...
- **GitCommitGateway 无 pathspec commit（方案 A+ 治本）**：`_commit_locked` 和 `_commit_auto` 统一用 [`_commit_with_file_message`](file:///d:/ZephyrAlpha/src/zephyr/governance/git_commit_gateway.py)（内置 [`_verify_staged_is_clean`](file:///d:/ZephyrAlpha/src/zephyr/governance/git_commit_gateway.py) + 无 pathspec）替代 `--pathspec-from-file`。根因：`git commit --pathspec-from-file` 对 staged rename（R100）拆分为独立 add+delete，只提交 pathspec 匹配部分，破坏 rename。治本：消除 pathspec 依赖，staged 验证内置到 commit 方法确保无法跳过，由 `_stash_other_files` + 内置 staged 验证双保险保证安全。`_collect_non_target_rel` 已修复 rename 格式 `R old -> new` 的路径解析（提取新路径），确保其他 session 的 staged rename 能被正确 stash。
