...
- **目录契约门禁**（GATE-DIRECTORY-CONTRACT）→ 文件放错目录 = AI 找不到 + 真源散落 + 规则无法自动执行。已加自动化门禁强制文件目录归属合规：
  ...
  - **post-commit 审计 reconciler**（warn-only）：修改上述 5 个规则文件会触发 `GATE-RULE-FILE-AUDIT`（reconciliation_registry.py `make_rule_file_audit_reconciler`，priority=700），落盘审计报告到 `.runtime/reconcile_reports/rule_file_audit_*.json`。单独提交豁免区（`docs/_working/`等）下带 frontmatter 的文件会触发 `GATE-EXEMPT-ZONE-FM`（`make_exempt_zone_frontmatter_reconciler`，priority=710），检测本应放正式目录却被塞进豁免区的文件。两者均 warn-only（不阻断），报告供人工审查。
  - **新建 .py CapabilityLookup 提示门禁**（warn-only）：commit 时 `CAPABILITY-OVERLAP` gate（commit_gates/capability_overlap_gate.py，priority=200）自动检测新建 .py 文件名是否与 capability_canonical_file_registry.yaml 已注册能力 aliases token 重叠，命中则 logger.info 告警。同时检测 `_registry/contracts|vocabularies|catalogs/` 下新增 .yaml 文件名是否与同目录现有 .yaml token 重叠（≥2 token = 高置信度第二真源）。warn-only 不阻断——文件名匹配是启发式，AI 看到 warning 后自行判断是扩展还是新建。
...
- 不要创建新模块而不注册到大脑
- **reconciler auto-commit 入口选择（2026-06-30 治本修订，向内收原则）**：两种入口都合法，按保护需求选择。① [`_commit_auto()`](file:///d:/ZephyrAlpha/src/zephyr/governance/git_commit_gateway.py)——需要全套保护（ttl/锁/GW标记）的 reconciler 入口，机器生成文件白名单内可跳过 ttl 校验；② 裸调 [`_run_git(["git","commit",...])`](file:///d:/ZephyrAlpha/src/zephyr/governance/git_commit_gateway.py) + 手动追加 `[GW:{session_id}:auto]` 标记——只需 GW 标记的 reconciler 入口（机器生成文件无 ttl 问题，git commit 原子性已足够，reconciler 在锁外运行不需应用层锁）。**现状**：8 个 reconciler（manifest/rule_catalog/registry_index/working_docs/domain_doc/arch_model/ttl rejudge/vocab_change）均走入口②，`_commit_auto` 0 调用方（活代码未被调用，保留作为未来需要全套保护的 reconciler 入口）。**禁止**：reconciler 不加任何 GW 标记的裸调 `_run_git`（审计断裂）。原"必须经 _commit_auto 统一入口"约束过严（机器生成文件无 ttl 问题，强制走 _commit_auto 需白名单设计，复杂度收益不匹配），修订为按需选择。
...
- **GitCommitGateway REPO_ROOT 门禁**：commit 时自动调 [`_check_repo_root_usage`](file:///d:/ZephyrAlpha/src/zephyr/governance/git_commit_gateway.py) 检测 `.py` 文件两类违规——① `parents[N]` 反模式（`Path(__file__).resolve().parents[N]` / `.parent.parent...` 推算仓库根）；② **AST 检测 missing import**（P2 治本 2026-06-29）：用 `ast.parse` + `ast.walk` 检测使用 `REPO_ROOT`/`DB_PATH` 但未 `from zephyr.shared.io.paths import` 或 import 顺序错误（import 在使用之后）——背景：P2 遗留 5 个 src 文件 missing import 导致 9 个 collection error，此门禁防止复发。豁免：`from zephyr.shared.io.paths import *` 覆盖。`--no-verify` 绕不过（约定见 §7 REPO_ROOT 真源归一）。
