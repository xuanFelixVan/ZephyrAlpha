...
- **自定义 noqa 豁免机制（#ARCH-NOQA-GOV-001 治本，2026-07-17）**——项目使用5种自定义 `# noqa: <marker>` 标记豁免 M 系列指标检测，5种标记的适用场景由 [noqa_exempt_registry.yaml](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/_registry/catalogs/noqa_exempt_registry.yaml) SSoT 定义。使用规则：
  ...
  - **5种合法标记**：`m02-manual`（manual-only 永久脚本豁免——CLI 启动的常驻服务）/`m03-duplicate`（重复簇函数豁免——AI 趋同演化非复制粘贴）/`m07-orphan`（死代码/orphan 模块豁免——动态引用无法被静态扫描发现）/`m10-time-trigger`（时间触发残留豁免——注释/枚举值/锁等待等非真实时间触发）/`gate-vocab`（GATE-VOCAB 词表硬编码豁免——DDL-as-Code 枚举/schema 合法值）——详细适用场景见 registry `applicable_conditions` 字段
