...
- 不要创建新模块而不注册到大脑
- **reconciler 不要裸调 `_run_git(["git", "commit", ...])`**——必须经 [`_commit_auto()`](file:///d:/ZephyrAlpha/src/zephyr/governance/git_commit_gateway.py) 统一入口（锁 + ttl 校验 + GW 标记），否则 ttl 防御被绕过（详见 §7 TTL 校验统一拦截点）。**现状（2026-06-30 调研发现）**：8 个 reconciler（manifest/rule_catalog/registry_index/working_docs/domain_doc/arch_model/ttl rejudge/vocab_change）仍裸调 `gateway._run_git(["git","commit",...])` 绕过 `_commit_auto`，`_commit_auto` 0 调用方（活代码未被调用，非死代码）。治本方向：reconciler 改调 `_commit_auto` + 机器生成文件白名单（跳过 ttl 校验，保留锁+GW标记），Phase B 施工。
