...
- **5.96 维度防御门禁 GATE-DEBT-BRIDGE（架构债务防复发，R67 引入 2026-07-06）**：[`scan_debt.py`](file:///d:/ZephyrAlpha/scripts/governance/d7_code/scan_debt.py) 是 pre-commit hook（`.pre-commit-config.yaml` gate-debt-bridge，`stages: [manual]`），检测 3 类 AI 易复发的代码异味（5.96 维度病根）。**3 条硬规则（违反即视为新债，需登记 architecture_debt_registry.md 后修复）**：
  ...
  - **DEBT-2 函数布尔参数蔓延**：函数（含方法）含 ≥3 个 `bool` 参数（不含 `self`/`cls`）→ MUST 改为 `dict[str, bool]` 或 keyword-only 参数（5.96.3 病根：`_calculate_trust(git_ok, test_ok, audit_ok)` 三布尔参数顺序易传错，R67 已修复为 `_calculate_trust(checks: dict[str, bool])`）。
  - **DEBT-3 class action+bool 共存**：任意 `class`（不限 `@dataclass`）含 `action: str` + `should_*`/`*_allowed: bool` 字段 → 同 DEBT-1 治本方案。
  **检测机制**：[`scan_debt.py`](file:///d:/ZephyrAlpha/scripts/governance/d7_code/scan_debt.py) 纯 stdlib AST 扫描（不依赖 ruff/mypy 是否安装），3 段检测：① `_is_action_field` + `_is_bool_flag_field` 识别字段语义 ② `DebtScanner.visit_ClassDef` 检测 DEBT-1/3 ③ `DebtScanner._check_function_args` 检测 DEBT-2。误报优先（宁可放过不可误伤 commit）。**阶段1（当前）**：`stages: [manual]` 不阻断常规 commit，手动 `pre-commit run gate-debt-bridge` 触发——现存 13 处 DEBT-2 违规（hallucination_detector.py:445 should_trigger 6 个 bool 等），强制阻断会卡死工作流。**阶段2（待定）**：存量违规清零后移除 `stages: [manual]`，commit 时自动阻断新增违规。**阶段3（待定）**：CI 集成 `pre-commit run gate-debt-bridge --all-files`。**使用**：`python scripts/governance/d7_code/scan_debt.py --src src/zephyr`（warn-only）/ `--ci`（hard block）。**R67 验证基线**：DEBT-1=0 / DEBT-2=13 / DEBT-3=0——修复后的 TriggerDecision + _calculate_trust 不被误报（门闸精确），13 处 DEBT-2 是新发现的可治理债。**TODO（capability 登记）**：后续需在 [`capability_canonical_file_registry.yaml`](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/_registry/catalogs/capability_canonical_file_registry.yaml) 登记 `architecture_debt_scanner` capability，让新 AI 想做"代码异味检测/bool 参数扫描/dataclass 冗余"前 CapabilityLookup 能反查阻止重复造轮子。
