...
- **post-commit reconciler**：`make_registry_sync_reconciler`（AD-GOV-001 合并后；旧 `make_baseline_aware_reconciler` 已并入 `GATE-REGISTRY-SYNC`，见 [reconciliation_registry.py](file:///d:/ZephyrAlpha/src/zephyr/governance/reconciliation_registry.py)）在 commit 后自动触发，用 `--files <committed_files>` 精确扫描本次提交文件
...
- **`docs/_working/` 新增 .md 文件 MUST 在 frontmatter 声明 `completes_when` 字段**（可验证的完成条件），GitCommitGateway commit 时自动拦截缺失该字段的新文档；规则真源见 [trae_028 §归档与废弃流程](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/rules/trae_028_doc_structure_naming.yaml)。
  - 自动归档由 [GATE-DELETE-AUDIT reconciler（含原 WORKING-DOCS 功能）](file:///d:/ZephyrAlpha/src/zephyr/governance/reconciliation_registry.py) post-commit 事件驱动（capability_id：`working_docs_ghost_ref_archiver`，反查用法见 §9）。
- **读取 `docs/_working/` 下任何 .md 前 MUST 验证文档引用的脚本/YAML/blueprint_id 是否仍存在**（防幽灵引用漂移），细则见 [GATE-DELETE-AUDIT reconciler（含原 WORKING-DOCS 功能）](file:///d:/ZephyrAlpha/src/zephyr/governance/reconciliation_registry.py)（`scan_and_archive_working_docs` 幽灵引用检测）。
