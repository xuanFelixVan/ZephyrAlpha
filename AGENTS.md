...
>
> **全图全库对齐铁律（D38，2026-09-07 Owner 裁定）**：全景图已扩至七图（+frontend_map+trading_decision_map），业务资产库 16 个全部挂 TDM 交叉轴（`_XREF_SPECS` 表驱动，门禁 R3-R36）。**以后新上的图和库都必须挂上去打通**——①新建业务资产库：同 commit 在 `src/zephyr/trading/decision_map.py` `_XREF_SPECS` 加轴（两行）+ 真源挂载，治理类库在 `tests/trading/test_decision_map.py::TestNewRegistryGate._GOVERNANCE_EXEMPT` 登记豁免，二选一必做（`tests/trading/test_decision_map.py` 门禁测试强制）；②新建全景图：以 module_id 为对齐 key 挂入七图体系（MOD 总线两跳互通，不搞 N² 直连）并在 alignment_checklist §3 登记。规则真源见 [alignment_checklist.md](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/sop/alignment_checklist.md) §4 统一原则 5/6。
>
