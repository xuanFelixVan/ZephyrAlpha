...
- **5.96 维度防御门禁 GATE-DEBT-BRIDGE（架构债务防复发，R67 引入 2026-07-06，R68 转硬阻断 2026-07-06）**：[`scan_debt.py`](file:///d:/ZephyrAlpha/scripts/governance/d7_code/scan_debt.py) 是 pre-commit hook（`.pre-commit-config.yaml` gate-debt-bridge，**阶段2 已转硬阻断——commit 时自动检测**），检测 3 类 AI 易复发的代码异味（5.96 维度病根）。**3 条硬规则（违反即视为新债，需登记 architecture_debt_registry.md 后修复；commit 阶段 hard block）**：
