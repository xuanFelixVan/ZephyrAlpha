...
- **废弃目录门禁 GATE-DEPRECATED-DIR（09_audit 治本加固，红蓝对抗修复）**：双层防御（① 层缺口待补全，② 层已修复 2026-06-30）——① [`directory_contract_gate`](file:///d:/ZephyrAlpha/src/zephyr/governance/commit_gates/directory_contract_gate.py)（priority=30，注册制 gate）通过 subprocess 调用 [`check_directory_contract.py`](file:///d:/ZephyrAlpha/scripts/governance/d1_structure/check_directory_contract.py)，但 `check_directory_contract.py::scan_files` **未实现** deprecated_directories 检测（原 `_check_deprecated_directories` 已在 AD-001 阶段3 删除，**未迁移**到 directory_contract_gate——本节原描述"迁移到 directory_contract_gate"有误，check_directory_contract.py 的 scan_files 只检测 doc_type/extension/root_whitelist/ttl_zone 四项）——① 层阻断缺口待补全（在 check_directory_contract.py 添加 deprecated_directories 检测，受 AD-GOV-001 收敛期约束暂缓）；② [`make_deprecated_directory_reconciler`](file:///d:/ZephyrAlpha/src/zephyr/governance/reconciliation_registry.py)（priority=600）post-commit **自动修复**（2026-06-30 治本：从 [`directory_contract.yaml`](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/_registry/contracts/directory_contract.yaml) §7 `deprecated_directories` 字段动态加载，当前含 `docs/09_audit/`）：检测到废弃目录存在时自动迁移文件到 `docs/_working/audit/` + 删除空目录。报告落盘 `.runtime/reconcile_reports/deprecated_directory_<ts>.json`。**注意**：原 reconciler 因 `_DEPRECATED_DIRS` 已删除降级为 no-op，2026-06-30 修复为直接读契约真源，② 层自动迁移已恢复。
