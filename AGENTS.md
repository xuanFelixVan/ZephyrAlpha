...
- **ZephyrBaseError 真源归一 + 循环依赖治本（P3 前置，2026-06-29）**——[`zephyr.shared.foundation.errors`](file:///d:/ZephyrAlpha/src/zephyr/shared/foundation/errors.py) 是 `ZephyrBaseError` 体系（13 个 Error 类）唯一真源。[`integration.shared_08.foundation.errors`](file:///d:/ZephyrAlpha/src/zephyr/integration/shared_08/foundation/errors.py) 改为 re-export shim（`from zephyr.shared.foundation.errors import *`），两个路径指向同一类对象（`A is B == True`），消除 breaking change 风险。**循环依赖根因**：`shared.infra_06.{idempotency,lock,limiter,outbox}` 越层引用 `integration.shared_08.foundation.errors.ZephyrBaseError`，而 `integration.shared_08._patterns` 反向引用 `shared.infra_06.*`，形成双向闭合循环（import-order-dependent 脆性态，先 import infra_06 触发 ImportError）。**治本**：4 个 infra_06 文件改引同层 `shared.foundation.errors.ZephyrBaseError`，切断 `shared→integration` 方向，`_patterns→infra_06` 保留为合法单向向下依赖。**验证**：两种 import 顺序均通过（先 infra_06 / 先 integration.shared_08），`health.py` 第二循环入口自动消除。**新 AI 引导**：新增 Error 子类改 `shared.foundation.errors`（真源），integration shim 自动 re-export；`shared` 层禁止 import `integration.*`（向下依赖原则）。
...
- **归档区 `docs/_archive/`（永久保留历史文件）**——已完成历史使命但需永久保留的文件（如架构决策记录、施工方案、阶段总结）MUST 移到 [`docs/_archive/`](file:///d:/ZephyrAlpha/docs/_archive/)。归档操作：`git mv <file> docs/_archive/` + frontmatter `ttl: permanent` + `status: deprecated`。归档区在 [`directory_contract.yaml`](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/_registry/contracts/directory_contract.yaml) `directory_zones.permanent.paths` 中（default_ttl=permanent），`validate_document_ttl.py --list-all-non-permanent` 不列为清理候选。**`docs/_archive/` 是唯一合法归档区**——禁止在 `docs/` 下创建其他归档目录（如 `_working/archive/`、`09_archive/`）。
...
- **批量改 DB（多 op 原子事务）** → `python scripts/governance/apply_depgraph.py --batch changes.json`（先 `--dry-run` 预览）。支持 12 种 op（4 节点级 + 8 域级含 `rename_domain`），真源清单见 [`cmd_batch` docstring](file:///d:/ZephyrAlpha/scripts/governance/apply_depgraph.py)；所有 op 共享单一 PostgreSQL 事务，全部成功才 commit，任一失败全部 rollback。批量重命名域 ID 时**禁止**手写 `_tmp_batch_rename.py` 调 `--rename-domain` 单命令循环（失去原子性，部分失败留半成品数据）。
