...
# Manifest format: dirname.manifest.md contains paths relative to dirname/
manifest_files = set()
manifest_dir = os.path.join(base_docs, '_manifests')
if os.path.isdir(manifest_dir):
    for mf in os.listdir(manifest_dir):
        if mf.endswith('.manifest.md'):
            # Extract directory name from manifest filename
            ...
            with open(os.path.join(manifest_dir, mf), 'r', encoding='utf-8') as fh:
                for line in fh:
                    ...
                    if line.startswith('- `'):
                        ...
                        if len(parts) >= 1:
                            path = parts[0]
                            # Manifest paths are relative to the directory
                            # So we need to prepend the directory name
