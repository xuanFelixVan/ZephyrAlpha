...
> **v3.1.0 更新（2026-06-26）**：各子目录文件数——`catalogs/` **26**、`contracts/` **4**、`vocabularies/` **30**、`schemas/` **3**（合计 **63**）；若**排除**四个子目录各自 `index.md` 后合计 **59** 个体系数据文件。

## 子目录结构

| 子目录 | 用途 | 文件数 | 索引 |
|--------|------|:---:|------|
| [`catalogs/`](../_registry/catalogs/index.md) | 登记表集中存储——YAML/MD 格式的注册表/清单 | 26 | index.md |
| [`contracts/`](../_registry/contracts/) | 验证契约——pre-commit/CI 消费的 YAML 契约规则 | 4 | index.md |
| [`vocabularies/`](../_registry/vocabularies/) | 受控词表——doc_type/status/rule_form/ttl/layer 等 **29** 个词表 + index | 30 | index.md |
