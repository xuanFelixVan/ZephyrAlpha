...
| META-018 | 禁止在废弃路径下写入新文件 | ABS-18 | doc | `governance/document/trae_028_doc_structure_naming.yaml doc_004` |
...
| META-033 | 禁止删除或篡改审计日志 | ABS-33 | code | `src/zephyr/llm-security/behavior_audit_logger.py` |
| META-034 | 禁止 AI 修改审计日志完整性校验 | ABS-34 | code | `src/zephyr/llm-security/behavior_audit_logger.py` |
...
| META-038 | 禁止 AI 在沙箱外执行不可信代码 | ABS-38 | code | `src/zephyr/llm-security/process_sandbox.py` |
...
| META-043 | 禁止使用 shell=True 执行子进程 | ABS-43 | code | `src/zephyr/llm-security/process_sandbox.py` |
...
| ARCH-023 | H/CRITICAL 级 FLE 提案强制 Owner 审批 | — | doc | `src/zephyr/feedback-loop/auto_evolution.py` L15 |
...
| ARCH-027 | 激活条件触发前禁止向架构视图添加实质内容 | — | doc | `02_enterprise_architecture/target-architecture/08-operations_architecture.md` L53 |
...
| 登记号 | 规则内容 | 对应 COND | 强制方式 | 来源路径 |
|--------|---------|----------|---------|---------|
| DOC-001 | 文件名禁止使用大写字母（历史遗留白名单除外） | — | doc | `governance/document/trae_028_doc_structure_naming.yaml doc_003 + gov_doc_003_naming_ssot` |
| DOC-002 | 文件名禁止使用版本号后缀 | — | doc | `governance/document/trae_028_doc_structure_naming.yaml doc_003 + gov_doc_003_naming_ssot` |
| DOC-003 | 文件名禁止使用日期后缀（LATEST 文件除外） | — | doc | `governance/document/trae_028_doc_structure_naming.yaml doc_003 + gov_doc_003_naming_ssot` |
| DOC-004 | 文件名禁止使用空格和特殊字符 | — | doc | `governance/document/trae_028_doc_structure_naming.yaml doc_003 + gov_doc_003_naming_ssot` |
...
| DOC-007 | B 轨禁止反向依赖 C 轨 | COND-07 | doc | `governance/document/trae_028_doc_structure_naming.yaml doc_002` |
| DOC-008 | C 轨内部禁止反向依赖 | COND-08 | doc | `governance/document/trae_028_doc_structure_naming.yaml doc_002` |
...
| SCRIPT-013 | 脚本 MUST 注册在 script-manifest.yaml（D-F-04） | — | manual | `scripts/governance/quality-standard.md` §3 D-F-04 |
...
| TRAE-011 | RULE-TEN 域归属铁律——模块归入已有 35 域，不新建域。新建功能域/子域 MUST 经 Owner 书面审批。AI 遇无域归属模块时优先根据 `[BLUEPRINT]` 字段归入已有域 | doc | `.trae/rules/project_rules.md` RULE-TEN 域归属铁律 |
| TRAE-012 | RULE-SEVENTEEN 禁止 PowerShell 语法——RunCommand 只允许裸命令（python/git/pytest）。禁止管道\|、引号嵌套、$变量、cmdlet、>重定向、;串联。文件操作用 Read/Write/Edit/Glob/Grep/DeleteFile 工具。复杂逻辑写 .py 脚本。根因：PowerShell 引号转义+非UTF8编码=乱码/文件损坏 | doc | `.trae/rules/project_rules.md` RULE-SEVENTEEN |
| TRAE-013 | RULE-ELEVEN 跨蓝图变更通知——修改蓝图接口契约（Collection名/API签名/数据格式/依赖方向）时MUST三步：识别消费方→同步更新→端到端验证。禁止只改提供方不改消费方 | doc | `.trae/rules/project_rules.md` RULE-ELEVEN |
| TRAE-014 | RULE-TWELVE 项目瘦身与自动清理——发现候选清理物→先判定价值→有价值接通系统/无价值走RULE-THREE审判后删除。禁止仅凭"零消费者"或"版本旧"判定删除 | doc | `.trae/rules/project_rules.md` RULE-TWELVE |
| TRAE-015 | RULE-THIRTEEN 任务卡粒度铁律——一卡一任务，独立可验证。R1-R6拆卡规则（deliverables≤1/files_in_scope≤3/acceptance≤1/施工目标≤1/结构词/字数≥100）+超粒度自动拆分 | doc | `.trae/rules/project_rules.md` RULE-THIRTEEN |
| TRAE-016 | RULE-FOURTEEN 根目录清爽铁律——根目录白名单门禁，只允许白名单目录/文件存在。禁止创建永久性.py/.md/.yaml/.json/.txt文件和子目录。Session关门时强制根目录审计 | doc | `.trae/rules/project_rules.md` RULE-FOURTEEN |
| TRAE-017 | RULE-FIFTEEN 自动化双轨判定——新建/改造自动化系统MUST两轨分类（🕐定时/⚡事件）+实现验证。定时轨注册circadian_scheduler，事件轨注册hook_registry。单轨实现=未完成 | doc | `.trae/rules/project_rules.md` RULE-FIFTEEN |
| TRAE-018 | RULE-SIXTEEN depgraph 程序化访问协议——depgraph存SQLite（157MB/55M tokens），禁止AI直接Read/Write，必须通过extract_depgraph.py/apply_depgraph.py操作。禁止拆分为30个域文件 | doc | `.trae/rules/project_rules.md` RULE-SIXTEEN |
| TRAE-019 | RULE-EIGHTEEN 连续两次审查零问题——任何文件变更声明完成前MUST连续两次审查零问题。适用任务卡COMPLETED/代码交付/规则修改/文档更新/配置变更/Session关门。第二次审查必须完整执行（非仅查修复点）。YAML真源: trae_042 §std_011_dual_review_protocol | doc | `.trae/rules/project_rules.md` RULE-EIGHTEEN + `docs/01_policies_and_standards/rules/trae_042_meta_rule_standard.yaml` §std_011_dual_review_protocol |
| TRAE-020 | RULE-NINETEEN 先裁定后确认（MTH-009 显化）——AI遇到任何决策（方案选择/范围裁定/触发条件判定/多选项权衡）MUST先给出专业裁定+理由，再请Owner确认。禁止直接问"你选哪个"把决策权推给Owner。YAML真源: trae_025 §mth_009 | doc | `.trae/rules/project_rules.md` RULE-NINETEEN + `docs/01_policies_and_standards/rules/trae_025_methodology_decision.yaml` §mth_009 |
...
| 1.4.9 | 2026-06-22 | 补登 TRAE-019~020。(1) 登记 RULE-EIGHTEEN（连续两次审查零问题，YAML真源 trae_042 §std_011_dual_review_protocol）/RULE-NINETEEN（先裁定后确认 MTH-009 显化，YAML真源 trae_025 §mth_009）共2条规则。(2) 统计更新：142→144条（+2 TRAE → 20）。版本号 patch +1。 |
| 1.4.8 | 2026-06-19 | 补登 TRAE-013~018。(1) 登记 RULE-ELEVEN（跨蓝图变更通知）/RULE-TWELVE（项目瘦身与自动清理）/RULE-THIRTEEN（任务卡粒度铁律）/RULE-FOURTEEN（根目录清爽铁律）/RULE-FIFTEEN（自动化双轨判定）/RULE-SIXTEEN（depgraph程序化访问协议）共6条规则。(2) 修复 sync_rule_registry.py：regex去掉🔴emoji要求+重构build_mapping为反向解析（不再依赖number_words硬映射）。(3) 统计更新：136→142条（+6 TRAE → 18）。(4) 修复 frontmatter version 1.4.6→1.4.8（对齐变更记录）。版本号 patch +1。 |
| 1.4.7 | 2026-06-18 | 新增 TRAE-012。(1) 登记 RULE-SEVENTEEN：禁止 PowerShell 语法——RunCommand 只允许裸命令（python/git/pytest），禁止管道/引号嵌套/$变量/cmdlet/>重定向。文件操作用专用工具。复杂逻辑写 .py 脚本。根因：PowerShell 引号转义+非UTF8编码=乱码/文件损坏。(2) 统计更新：135→136 条（+1 TRAE → 12）。版本号 patch +1。 |
