...
| 域/目录/依赖 | 5 | `functional_domain_registry.yaml`、`directory_registry.yaml`、`cross_module_dependency_registry.yaml`、`governance_convergence_map.yaml`、`battle_map_domain_policy.yaml` | 功能域/目录/跨模块依赖/契约（interface_contract/business_streams 已于 2026-09-16 裁定#259 归档至 `_archive/`） |
| AI 治理 | 3 | `ai_autonomy_authority_registry.yaml`、`ai_risk_register.yaml`、`trust_boundary_surface_registry.yaml` | AI 权限/风险/信任边界（ai_session/frontier_llm 已于 2026-09-16 裁定#258 归档至 `_archive/`） |
| 元数据/文档治理 | 6 | `frontmatter_field_registry.yaml`、`terminology_glossary.yaml`（术语仲裁源）、`task_card_meta_registry.yaml`、`external_contract_verification_registry.yaml`、`depgraph_scan_exclusions.yaml`、`infrastructure_registry.yaml` | frontmatter 字段/术语/任务卡/知识条目（knowledge_article/declarative_contract_tracker 已于 2026-09-16 裁定#258/#259 归档至 `_archive/`） |
...
> 另有 `_index.yaml`（TRAE 规则高级别名表 PS-REG-001，RULE-ZERO..RULE-TWENTY）与 `_archive/`（归档区：candidate_module_registry_harvest_archive.yaml 5283 条 HARVEST 候选归档；2026-09-16 裁定#258/#259 新增六库归档——ai_session_registry / knowledge_article_registry / frontier_llm_benchmark_ranking / business_streams_registry / declarative_contract_tracker_registry / interface_contract_registry）。
