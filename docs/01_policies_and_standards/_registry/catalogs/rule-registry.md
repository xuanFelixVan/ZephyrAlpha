...
summary: "ZephyrAlpha 项目全部规则的集中登记表。记录所有因代码、架构、文档、合规等原因产生的规则，不限于元规则。每条规则包含：登记号、规则内容、强制方式、来源路径、锁定状态。本登记表是规则发现入口，不是规则定义——规则定义权属于各自的源文件。v1.4.0：从 meta/ 迁移至 _registry/catalogs/。"
ttl: permanent
...
| 1.2.1 | 2026-05-01 | meta/ 最终审查。(1) depends_on 格式 V2 确认（结构化行级）。(2) META-V17 违规 ID 冲突已解决：PS-STD-012 §2.1 不再错误复用 META-V17（该 ID 在 PS-STD-001 §14.1 中定义为 blueprint_refs 废弃蓝图检查），改为待分配占位。版本号 patch +1。 |
