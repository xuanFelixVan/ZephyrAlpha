...
summary: "ZephyrAlpha 2.0 全系统统一编号规范——定义 module_id、task_id、ADR 编号、registry_id 的格式与分配规则。"
---
...
本规范建立 ZephyrAlpha 2.0 全系统**唯一的编号体系**，确保：

- 任何人或 AI 读到一个编号，能**唯一定位**到对应的架构层、代码目录、文档区域
- 消除当前两套编号并行导致的 7 处 SSoT 矛盾
- 为 5-10 年的系统演化预留充足的编号空间
...
| Mermaid 架构图节点 ID | `L00`, `L04`, `FE` |
| ADR 引用 | "本决策影响 L04 Risk Management 层" |
| 施工图命名 | `construction-plan-l00-data-source.md` |
...
| 来源 | 对标内容 |
|------|---------|
| ISO 9001 §7.5.2 | "文件化信息应包含唯一标识"——本文的 module_id 即此要求的落地 |
| K8s API Group Naming | `apps/v1/deployments`——层次化命名空间让资源可唯一定位——本文的 `{DOMAIN}-{SUB}-{NNN}` 前缀体系基于此模式 |
| ITIL SACM → Identification | 配置项必须有唯一标识符，标识符应携带语义信息（如位置/类型） |
| Unicode CID（Character-ID）惯例 | "ID 永不回收"——本文的 append-only 原则基于此 |

### 1.5 唯一真源声明

> **SSoT**: 本规范的唯一真源是 `docs/02_enterprise_architecture/target-architecture/architecture-model/_index.yaml` 中的 `partitions` 列表。
>
> 本文档是该 SSoT 的**人类可读解释**，不是独立真源。若本文档与 `_index.yaml` 冲突，以 `_index.yaml` 为准。
...
| 大写 L 前缀 | 在文本/图表/ID 中引用时使用大写 | `L00`, `L11` | `l00`, `layer00` |
| 小写目录名 | 在文件系统路径中使用小写 | `l00_data_source/` | `L00_Data_Source/` |
| kebab-case YAML | 在 YAML 文件名中使用 kebab-case | `l00-data-source.yaml` | `l00_data_source.yaml` |
| snake_case 目录 | 在 Python 包目录中使用 snake_case | `l00_data_source/` | `l00-data-source/` |

### 2.2 完整层编号注册表

以下是 ZephyrAlpha 2.0 全系统的完整层编号，按架构分层顺序排列：
...
| **contracts** | Contracts | P0/P1 跨层数据契约、OCP 扩展点 | `architecture-model/contracts/` |
| **events** | Events | 22 条领域事件 | `architecture-model/events/` |
...
```
L00-L13    已分配（14 个业务/技术层）
L14-L19    近期预留（3-5 年内可能新增的层）
L20-L49    中期预留（5-10 年扩展空间）
L50-L89    远期预留（10 年以上或重大架构变更）
...
1. 提交 ADR 说明新增层的必要性
2. 确认无法通过现有层的子模块解决
3. 在 `_index.yaml` 的 `partitions` 中注册
4. 同步更新本规范文档
...
> **docs 目录编号（00-19, 99）是信息架构的物理路径编号，不是架构层编号。**
>
> 它们在文件系统中保留，但在任何架构语义场景中，必须使用 L{XX} 层编号。

### 3.2 两类编号的本质区别

| 维度 | L{XX} 层编号 | docs 目录编号 |
|------|-------------|-------------|
| **本质** | 架构层标识（Architecture Layer ID） | 信息资产抽屉编号（Drawer Number） |
| **SSoT** | `_index.yaml` | `02-information-architecture.md` |
| **分类逻辑** | 按量化投资价值链分层 | 按治理属性混合分类 |
| **对应代码** | 1:1 对应 `src/zephyr/l{xx}_*/` | 无代码对应 |
| **使用场景** | 架构设计、代码、蓝图、ADR、图表 | 仅用于 `docs/` 物理目录路径 |
| **是否可在架构图中使用** | ✅ 是 | ❌ 否 |
...
├── ...
├── l11_ml_platform/    # L11（注：目录名为 strategic_decision，层名为 ML Platform）
...
```markdown
<!-- 正确 -->
本决策影响 **L04 Risk Management** 层的 Kill Switch 模块。

<!-- 错误 -->
本决策影响 17_risk_and_controls 目录下的风控模块。
```
...
| 审查项 ID | 审查内容 | 触发条件 |
|----------|---------|---------|
| NUM-R01 | 新增层编号是否经过 ADR 审批 | 任何 `L14+` 编号首次出现 |
| NUM-R02 | 前端子层编号是否合理 | `FE-L5+` 编号首次出现 |
| NUM-R03 | 跨层契约是否正确引用两端层编号 | 新增 `CTR-*` 条目 |
...
- **不改变 docs 物理目录结构**：`docs/09_data_platform/` 等目录保持不变
- **只改变架构语义引用**：所有架构文档、图表、ADR 中的引用统一为 L{XX}
- **渐进式迁移**：新文档必须遵守本规范；存量文档在下次编辑时顺带修正

### 7.2 迁移检查清单

- [ ] 所有 Mermaid 图节点 ID 改为 `L{XX}` 格式
- [ ] 所有蓝图 frontmatter `layer` 字段改为 `l{xx}_*` 格式
- [ ] 所有施工图文件名改为 `construction-plan-l{xx}-*.md`
- [ ] 所有模块 ID 改为 `L{XX}-{ABBR}-{NNN}` 格式
- [ ] 所有 ADR 中对架构层的引用改为 L{XX}
- [ ] 移除所有正文中的"抽屉 09"等旧编号称呼
...
| 规则 | 与本标准的关系 |
|------|-------------|
| file-naming-standard.md（GOV-DOC-003） | 文件命名规范消费本标准的编号前缀（如 `GOV-DOC-`、`PS-STD-`、`DOM-L{XX}-`）来构造 module_id |
| directory-structure-standard.md（GOV-DOC-002） | 目录结构规范中每个子目录的 module_id 前缀由本标准定义 |
| document-lifecycle-standard.md（GOV-DOC-006） | 生命周期状态管理依赖 module_id 来唯一标识文件——各编号域的文件可能有不同的 TTL 和状态转换要求 |
| document-discovery-policy.md（GOV-DOC-010） | module_id 搜索范式以本标准的编号前缀体系为基础——§4.2 前缀速查表引用 |
| AR-ARCH-027 | 架构蓝图的 7 维度文档治理框架定义了编号体系要服务的具体维度 |
| ADR-0006 | 统一编号体系的设计决策记录 |
