...
>
> **老树教训**：老树中文档没有生命周期管理，导致过期文档、临时文件、状态快照大量堆积，AI 无法判断哪些文档是当前有效的，哪些是历史遗留。
...
> **老树教训**：老树中每次扫描都新建一个带日期的文件（如 `scan-20260413.json`、`scan-20260414.json`），导致目录中堆积大量历史快照，AI 不知道哪个是最新的。
