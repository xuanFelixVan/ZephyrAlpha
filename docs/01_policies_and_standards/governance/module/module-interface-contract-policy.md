...
ttl: permanent
summary: "定义模块间接口契约要求、契约注册表、版本兼容规则、一致性验证、破坏性变更处理、退役级联规则及模块间运行时交互规则。v1.0.0 升格：被 19 处引用（6 个文件），实质已达 active 成熟度。draft→active 升格由 2026-05-02 审计驱动。"
...
---
## 1. 目的与范围
本策略定义 ZephyrAlpha 系统中模块间交互必须定义的接口契约规则。适用于：
- 所有 P0 模块的接口定义
- 跨层模块间的交互接口
- 模块对外暴露的 API / 事件 / 数据接口
本策略**不适用于**：临时草案（ttl: session）、未注入的模块接口讨论稿。

## 2. SSoT 声明

本文档是 ZephyrAlpha 系统中**模块接口契约**的唯一真源（SSoT）。

**本文档定义了**：
- 接口契约的 7 个必填字段（IFC-001）及唯一标识规则（IFC-002）
- 语义化版本规则（IFC-003）：MAJOR/MINOR/PATCH 判定标准
...
- Provider 退役时的契约级联废弃（IFC-007，由 GOV-MOD-003 MLC-003 触发）
- 模块间运行时交互规则

**本文档与以下文件互补**（非取代关系）：
- GOV-MOD-001 MOD-P4：接口可定义性筛选——本策略定义的契约是在 MOD-P4 通过之后的工作
- GOV-MOD-003 MLC-003：退役流程——触发 IFC-007 契约级联
...
| 枚举值 | 含义 | 定义位置 | 说明 |
|--------|------|---------|------|
| `draft` | 契约草稿 | §11（IFC-005） | 可变更，仅 provider 消费 |
| `frozen` | 契约冻结 | §11（IFC-005） | 不可变更，consumer 可接入——P0 模块注入前置条件 |
| `deprecated` | 契约废弃 | §11（IFC-005）+ §12（IFC-007） | 即将移除，consumer 应迁移 |

**受控约束**：新增契约状态或修改状态转换规则需要创建 ADR（见 §17 修改条件）。GOV-MOD-005 INJ-003 的契约状态检查消费本枚举。
...
| 消费者 | 文件 | Tier | 依赖内容 |
|--------|------|:---:|---------|
| GOV-MOD-005 | module-injection-rules.yaml | 1 | INJ-003 检查 `cross-layer-contracts.yaml` 是否存在——消费本规范的契约注册表格式 |
| GOV-MOD-003 | module-lifecycle-policy.md | 1 | MLC-003 步骤 7（契约级联废弃）——消费 IFC-007 的级联规则 |
| GOV-MOD-001 | module-admission-policy.md | 1 | MAD-005/MOD-P4 检查接口可定义性——消费本规范的契约是否已 frozen 的判断 |
...
任何已 `frozen` 的接口契约发生变更，必须创建 ADR 记录变更决策。
...
以下行为在本策略管辖范围内**严格禁止**，违反将导致注入拒绝或契约撤销：

| 禁止行为 | 原因 | 替代方案 |
|---------|------|---------|
| P0 模块未在 `cross-layer-contracts.yaml` 中定义接口契约就注入 | 接口未定义 → 消费者无法接入 → 运行时错误 | 按 IFC-001 定义全部 7 个必填字段后注入 |
| 已 `frozen` 的接口契约未经 ADR 直接变更 | 契约变更是跨模块影响——绕过 ADR 导致消费者不知情，契约与实际实现漂移 | 按 IFC-002 创建 ADR，破坏性变更须含迁移方案和影响分析 |
| 破坏性变更不提供双版本并行过渡期（≥30 天） | 消费者集中迁移 → 停机窗口 → 数据不一致 | 按 §8 双版本并行运行 ≥30 天后再下线旧版本 |
| 跨层调用绕过 `frozen` 契约直接操作 | 绕契约即绕治理 → 接口漂移 → 架构污染不可恢复 | 通过 `cross-layer-contracts.yaml` 中的 `frozen` 契约调用 |
| 模块间形成循环调用（A → B → A） | 循环依赖 → 死锁风险 + 不可测试 → 架构腐化 | 依赖注入/事件总线解耦；设计阶段 MAD-003 应捕获 |
| 手动编辑 `cross-layer-contracts.yaml` 注册表 | 注册表是脚本同步的镜像——手动编辑导致与实际契约不一致 | 使用 `validate_module_schema.py --sync-contracts` 自动同步 |
| Provider 退役时不触发 IFC-007 契约级联废弃 | 消费者引用已废弃的契约 → 运行时错误 + 断链累积 | 退役前完成 MLC-003 全部 7 步——含契约级联废弃通知 |
...
- 请求/响应示例
- 错误码清单
- 性能约束（P0 模块必填：延迟 p99 ≤ **TBD**（待 Phase 1 性能基线确定后填入具体值），吞吐 ≥ Y req/s；非 P0 选填）
...
本策略 `stability: evolving`——接口契约规范会随 Phase 边界变化。以下矩阵定义变更类型与消费者同步要求：

| 变更类型 | 影响范围 | 同步动作 | 时机 |
|---------|---------|---------|------|
| 修改 IFC-001 必填字段列表 | cross-layer-contracts.yaml（Tier 1） | 更新数据文件 schema | 同 commit |
| 新增/删除契约状态（§3 枚举） | GOV-MOD-005（Tier 1） | 更新 INJ-003 契约状态检查 | 同 commit |
| 修改 IFC-004/005 审批流程 | GOV-MOD-003（Tier 1） | 更新 MLC-001 in_design 契约冻结时序 | 同 commit |
| 修改 IFC-007 级联废弃规则 | GOV-MOD-003（Tier 1） | 更新 MLC-003 步骤 7 | 同 commit |
| 修改 IFC-002 字段类型 | 全部 Tier 1 | 数据迁移 + 脚本更新 | 同 PR，不同 commit（需 migration commit 先行） |
...
本策略 `ai_autonomy: human_gated`——接口契约规范不可由 AI 自主修改：

| 级别 | 变更范围 | 审批方 | 要求 |
|:---:|---------|--------|------|
| L0 | 错别字、措辞优化、格式调整 | AI 自批 | Session Log 记录 |
| L1 | 增加非破坏性字段（如新增一个选填字段到 IFC-001 表） | AI 可建议，Owner 确认 | Session Log 提案 |
| L2 | 修改 IFC 规则内容 / 契约状态枚举 | Owner 审批 | 必须创建 ADR |
| L3 | 删除已有 IFC 规则 / 修改契约状态转换规则 | Owner 审批 | 必须创建 ADR + 全部 Tier 1 消费者同步 |
| — | `cross-layer-contracts.yaml` 数据文件字段结构 | Owner 唯一 | 数据文件 schema 变更影响所有模块，仅 Owner 可操作 |
...
| 豁免场景 | 豁免内容 | 约束 |
|---------|---------|------|
| Phase 0 原型阶段 | 免 IFC-001 强制契约要求 | Owner 审批，Phase 0→1 边界自动回退，必须在 Phase 1 开始前补齐 |
...
| 本标准字段 | 引用此字段的其他标准/文件 | 引用方式 |
|-----------|------------------------|---------|
| `contract_id` | cross-layer-contracts.yaml | 数据文件的主键——每个契约条目使用此字段 |
| `provider` | cross-layer-contracts.yaml / GOV-MOD-005 INJ-003 | 数据文件的必填列——注入时检查 provider 是否为合法模块 ID |
| `consumers` | cross-layer-contracts.yaml / GOV-MOD-005 INJ-003 | 数据文件的必填列——注入时校验所有 consumer 模块 ID 存在 |
| `interface_schema` | cross-layer-contracts.yaml / GOV-MOD-005 INJ-006 | 数据文件的必填列——INJ-006 检查 schema 字段类型与契约定义一致 |

> 字段改名时，必须按 §16 变更同步规则同步更新 `cross-layer-contracts.yaml` 和 GOV-MOD-005 INJ 规则中所有引用。
...
- [ ] §1 目的与范围：明确覆盖 P0 模块，说明 P1/P2 的选填策略
- [ ] §2 SSoT 声明：互补关系覆盖 GOV-MOD-005/MOD-003/MOD-001
- [ ] §3 受控枚举：3 契约状态枚举完整
- [ ] §4 消费者注册表：全部 Tier 1 消费者已列出
- [ ] §5 接口定义要求：IFC-001 字段表完整（7 字段）
- [ ] §6 禁止行为：7 条禁止行为覆盖接口契约全部风险面——含禁止+原因+替代方案三列
- [ ] §10 契约注册表：生消流程明确（禁止手动编辑注册表）
- [ ] §11 契约生命周期：draft→frozen→deprecated 状态转换完整
- [ ] §12 契约一致性验证：IFC-006 验证命令和失败动作明确
- [ ] §13 模块间交互规则：跨模块约束补充单模块治理的空白
- [ ] §14 标准间引用：normative 和 informative 分类正确
- [ ] §16 变更同步规则：每种变更类型有明确的同步动作和时机
- [ ] §18 废弃流程：覆盖本策略自身被取代的处置
- [ ] §19 异常豁免：3 场景含约束和过期回退
...
| 2026-05-01 | 0.6.3 | P1 结构性添加：§6 禁止行为（7 条——覆盖未定义契约注入/frozen 绕过 ADR/破坏性变更无过渡期/绕契约跨层调用/循环调用/手动编辑注册表/退役不级联废弃）+ 全部后续章节 §7→§8…§22→§23 重编号 + 自引用 § 编号同步修正（~25 处） + §15 AI 可消费性最小必读路径更新 |
