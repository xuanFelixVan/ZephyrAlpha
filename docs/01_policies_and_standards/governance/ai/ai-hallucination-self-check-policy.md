...
>
> **老树教训**：AI 在不知道完整架构的情况下，会幻觉补全接口（超时、重试、异常类型）、编造不存在的文件路径、推断错误的依赖关系，导致架构污染。
...
> 来源：老树 scaffold~2 施工中实际发生过的 AI 幻觉。每次自检时对照本清单，确认自己是否落入了同一种模式。

| # | 幻觉模式 | 典型表现 | 后果实例 |
|---|---------|---------|---------|
| 1 | **路径幻觉** | AI 引用了一个语法正确但不存在的文件路径（如 `ssot-authority-map.md`、`module-id-registry.json`） | 下游操作全部基于不存在的文件，需要回滚整次 session |
| 2 | **接口补全幻觉** | AI 在未读取接口定义的情况下，"合理推断"了参数类型、超时值（300ms）、异常类型（TimeoutError） | 代码运行时报 TypeError，因为实际参数是 dict 不是 int |
| 3 | **编号格式幻觉** | AI 假设模块 ID 是 `MOD-XXX`、层编号是 `L00-L13`，但实际规范已变为 `GOV-XXX`/`PS-XXX` 命名空间体系 | 注册表中找不到引用，产生孤儿引用链 |
| 4 | **上下文溢出幻觉** | Session 后半段（60%+ context），AI 不再读取文件而是凭 session 早期读到的残片"记忆补全" | 路径引用开始出现老树相对路径（"在上级目录中"），违反路径不可漂移原则 |
| 5 | **权限假设幻觉** | AI 在没有查 `ai-autonomy-authority-registry.md` 的情况下，自行判断某模块"看起来应该是 AI-Modifiable" | 越权修改了 Immutable Core 组件，触发 pre-commit 拒绝 |

**AI 自检时请逐条对照**：本次 session 是否复现了上述任何一种模式？如有，立即标记不确定性级别为 L3（高风险幻觉）并走事故路由。
