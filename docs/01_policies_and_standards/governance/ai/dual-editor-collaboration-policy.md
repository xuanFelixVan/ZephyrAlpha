...
ttl: permanent
summary: "定义 Cursor 和 Trae 两个编辑器的分工规则、文件锁定机制和编码安全规范。"
...
> **目的**：定义 Cursor 和 Trae 两个编辑器的分工规则、文件锁定机制和编码安全规范，防止双编辑器并行工作导致的编码损坏和文件冲突。
>
> **老树教训**：Trae 的 `files.autoGuessEncoding` 选项将 UTF-8 文件误判为 GBK/Latin-1，保存时以错误编码写回，产生双重编码乱码（表现为文件末尾出现阿拉伯文/西里尔文字符）。
...
| 在 Trae 中编辑 `.cursor/rules/` 下的文件 | Trae 无权修改 Cursor 规则 |
| 使用 `echo` 重定向创建 `.md` 文件 | Windows 默认 UTF-16 LE 或 GBK |
| 使用 PowerShell `Out-File` 默认参数创建文件 | 默认编码非 UTF-8 |
...
- 故障后切换到另一编辑器继续工作前，必须先完成上述恢复步骤和编码扫描
- 故障事件必须写入 Session Log，供下一个 session 确认状态
- 如果同一编辑器在 1 小时内崩溃 ≥ 2 次，暂停工作并通知 Owner
