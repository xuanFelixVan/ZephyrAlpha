...
>
> 定义 AI Agent Session 的生命周期状态机，确保所�?Session 遵循统一的状态转换规则�?
## 一、状态定�?
| 状�?| 说明 | 入口动作 | 出口动作 |
|------|------|---------|---------|
...
## 七、与 YAML 规则的对�?
�?Markdown 文档�?`D:\ZephyrAlpha\config\session_state_machine.yaml` 的人类可读版本�?YAML 文件是机器可执行的权威来源，本文档是解释性参考�?
差异说明�?- YAML 中使�?`idle` 状态，本文档使�?`INIT`（语义相同，命名对齐 ADR-0035 三阶段模型）
- YAML 中有 `archived` 状态，本文档将其归�?`COMPLETED` 的出口动作（归档是完成后的自动行为，非独立状态）
