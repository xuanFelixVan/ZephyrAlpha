...
| #      | 禁止行为                                       | 原因                                   | 替代方案                                          | 来源        |
| ------ | ------------------------------------------ | ------------------------------------ | --------------------------------------------- | --------- |
| ABS-44 | 使用 `_DO_NOT_USE_old_tree/` 下的文件作为规则来源或操作目标 | 废墟文件已归档，其中的规则可能已过时或与新树矛盾，引用将产生不可预测行为 | 仅作为迁移参考，新树规则以 `01_policies_and_standards/` 为准 | AGENTS.md |
...
| COND-35 | 生产环境关闭门禁 disable 开关   | 生产环境启动时       | `TaskRepository(enable_gate=False)` 仅限单元测试/Phase 0 补录 | gate-strategy-standard.md |
...
| COND-49 | Phase 2 接入真实资金前未升级容器隔离  | Phase 2 接入真实资金时 | 必须从 Windows ACL 升级到完整容器隔离     | adr-0018                      |
