...
date: "2026-05-06"
ttl: permanent
summary: "定义 ZephyrAlpha 治理决策的方法论——总则（两条最高原则：最优先行 + 项目清爽）+ 十三条核心原则（MTH-001~013）。v2.7.0：唯一真源从'声明式原则'升格为'操作化体系'——总则唯一真源规则增加事前检查维度（创建前查 GOV-MOD-001 §7 #5 功能域重叠扫描 → PS-STD-005 §10 #7 禁止平行蓝图）+ 禁止行为新增第 4 条（禁止创建平行文件后合并）。治根逻辑：闭环 唯一真源原则 → 准入门控（GOV-MOD-001 §7 #5）→ 架构标准（PS-STD-005 §10 #7）→ 生命周期（GOV-MOD-003 MLC-001）→ 数据字段（responsibility_domain）五层操作化链路。根源案例：MOD-INF-003/004→006 的反模式——事后 MTH-008 冲突裁决治标不治根。"
