...
4. 计数/评级类提前放行（裁定#253，2026-09-15，夜班自裁协议）：§9.2 定性经数据实证后
   放行 EXP-04/06 不等干净窗先行评估——600519 IS 窗（2019-2023）n_reports 48 值/
   n_orgs 20 值/rating_score_mean 104 值时变真实 vs 同窗 eps_consensus 3 值退化；
   评级年度非空覆盖 92-98%。出证（真实数据，均不达标=noise 非 data-gap）：
   EXP-06 IS RankIC -0.0035（p=0.53）+窄测 cost-fragile（EXP-FACTOR-EVAL-003）；
   EXP-04 IS +0.0043（p=0.72）+OOS 符号翻转+窄测全负（EXP-FACTOR-EVAL-002）。
   该问号已关闭（数据侧无可疑、因子侧本身弱）；改进旋钮留 C4 历史修复完成后统一复评。
   值类因子（EXP-01/03/05 及 EXP-02 复核）不受益于本裁定，仍守本条 2 款双源策略。
