...
> **配套编排**：修复循环、退出条件、时间盒见 [industry_chain_data_audit_sop](../sop/data_audit_sop/industry_chain_data_audit_policy.md) §11（v1.5.0 起）。
...
| S25 | 链名结构完整（2026-09-10 增，SOP §4.7.0 链名分类学） | 活跃链名命中任一即违规：括号不闭合（"碳化硅（SiC"）/虚词悬空尾（"中国算力产业崛起的"）/外文缩写裸名（IC、PVD、VR——缩写进 aliases）/报告指数词（指数/白皮书/研究报告/年鉴） | 改规范中文名（逐簇梳理时定映射）；写入工具 CHAIN_STRUCT_RE 同步拒绝新增 |

## 3. 节点层合格线（S-NODE，4 条）

| # | 标准 | 判定规则 | 修复方案 |
|---|---|---|---|
| S6 | function_role 八值词表（**2026-09-12 tier 检查退役**） | function_role 非空且不在八值词表 {生产设备,生产原料,辅助材料,辅助设备,加工工艺,技术服务,产品业务,销售渠道} 违规。**tier 检查段废止**（Owner 2026-09-12 第一性原理裁定：tier 相对位置无机械判定依据，AI 标注口径漂移——686 条 unspecified+306 条治理债实证；层位改由 S21 拓扑口径与前端分层派生，存量 tier 值保留作历史快照不清洗不检查） | 语义归一或清空 function_role；tier 一律不填不停修 |
| S7 | 节点名零 "-tier" 后缀 | name 以 -(上游|中游|下游|设备|材料|零部件|原材料|辅材|unspecified) 结尾违规 | **双轨**：前端/查询侧显示剥离（立刻生效）；数据层改名随迁移脚本慢做（改名牵 node_id，需专项） |
| S8 | 零孤岛节点 | 无边且无落位的节点违规（豁免：行业聚合节点及 child\_chain_id 挂接环节——挂子链=有下钻内容非孤岛，2026-09-10 补；墓碑节点过滤与全引擎同口径；**2026-09-12 起 valid_to 非空的 PIT 已关闭节点不参与审查**——ig_node.valid_to 列同日增配，孤岛唯一处置=node_close 通道） | 有据补边/补落位；无据 node_close PIT 关闭（禁 DELETE） |
...
| # | 标准 | 判定规则（大白话） |
|---|---|---|
| S21 | 流程主干连通 | 活跃链（**实质节点数≥3**，2026-09-10 口径修正：墓碑节点'（已并入'标记不计入节点数与起讫集——与 S8 历史快照豁免同源；去墓碑后实质节点<3 或实质节点名全等链名(±'行业'/'行业聚合')的僵尸/锚点链跳过，由 S24 收口）：**链内拓扑端点**（2026-09-12 tier 退役裁定：入度0=源头起讫集，出度0=终端目标集；链内成环/散点=无拓扑端点=违规）间存在连通路径（structure 流程边与 supply/supplies_to 供应边均计入连通路径，无向 BFS）=合规；断链=违规。附带指标：各链 structure 边占比。**先进度指标后硬闸**：样板链验收前只报告不计违规，验收后转硬闸。**口径修正依据（2026-09-10）**：首轮 advisory 336 中 93.8%（315/336）为墓碑残留伪断链，真缺口被淹没；**tier 起讫废止依据（2026-09-12）**：修正后 15 条 advisory 全为"缺上游/下游 tier 层标记"型——tier 标注漂移制造伪缺口，拓扑端点=结构事实更客观 |
...
- 1.6.0（2026-09-10）：**S25 链名结构完整性入引擎**（SOP v1.7.0 链名分类学配套，Owner 2026-09-10"命名专业化"指令）——括号不闭合/虚词悬空尾/外文缩写裸名/报告指数词四类链名结构违规硬线；写入工具 CHAIN_STRUCT_RE 同步拒绝新增（引擎查存量+工具拒新增同正则防漂移）；首轮存量 23 条（15 裸缩写+3 截断+3 悬空尾+2 指数名）待逐簇梳理清零。
