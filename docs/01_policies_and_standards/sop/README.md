...
| `construction_sop/` | **施工族**：15 步施工闭环真源（construction_workflow）+ 前端拆件 8 步闭环（frontend_component_split）+ 文档七轮审查法（document_review_and_optimization） | 写第一行业务代码/新模块施工前（宪法 RULE-CAPABILITY-LOOKUP 强制）；前端拆组件前；重要文档定稿前 |
...
| `trading_decision_map_sop/` | **TDM 地图族**：逐层讨论六步法（trading_decision_map_layering：四道前置检查防撞车——资产盘点/四路调研/上层完整性/枝干分级） | TDM 地图结构讨论、血肉填充任务开工前 |
| `ops_sop/` | **运维协作与应急族**：冲突三分法（merge_conflict_resolution：全项目冲突处理唯一真源）+ worktree 四证清理（worktree_cleanup）+ 保命轨人工 Runbook（2026-08-22-emergency-runbook，D-L1~D-L3） | 合并冲突时；清理 worktree 前（四证缺一不可）；系统应急时 |
| `data_audit_sop/` | **数据审计族**：产业链数据审计修复循环（industry_chain_data_audit，配套标准=policies/graph_quality_standard.md） | 图谱数据质量修复、数据审计班次开工前 |
