...
title: 车道施工纪律 SOP——多车道并发施工全纪律一页册（冷启动/提交正门/新建三件套/热注册表/门禁红线/并发安全/EOL/判据交付）
...
# 车道施工纪律 SOP——多车道并发施工一页册（真源）

> **一句话**：本册把本机/本仓多车道并发施工已实证的坑压成一册：冷启动 → 提交正门（含入队前门禁预跑）→ 新建文件三件套 → 热注册表并发写 → 代码红线 → 并发安全 → 判据与交付标准，照着做可省 3-5 轮试错；违反即提交被拦或成果被吞。
> **定位**：[construction_workflow_policy.md](construction_workflow_policy.md)（15 步施工闭环，单线施工编排真源）在**并发维度**的纪律补册，与前端拆件、文档审查两册并列于施工族；语义冲突时以宪法 `AGENTS.md` 与各门禁真源为准。
> **晋升**：2026-09-21 由战役工作册晋升正典（见下方晋升声明与净零申报）。
> **用语约定**：正文沿原册保留"本役/总包/车道"及 B15、R-065a、G3、L3 等代号——"役"泛指产出这些实证的施工战役（原全流通战役，总包 st-fullflow-20260918，2026-09-18 起），"总包"=战役总控包，"车道"=并发施工线；代号是可回查的历史证据锚，不作通用名词使用。

## 晋升声明（2026-09-21）

- **正典化日期**：2026-09-21。
- **来源**：`docs/_working/fullflow_campaign/CONSTRUCTION_DISCIPLINE.md`（全流通战役·子代理施工纪律速查，总包 st-fullflow-20260918 发布，ttl=task_bound）全量晋升为本册；**原册冻结随档**，作为历史证据归档，不再单独维护。
- **净零声明**：本册吸收合并原册全量内容，并收编各战役散落于车道任务书、战役台账、死信复盘中的车道纪律知识，替代其发现面职能；正典资产净增 1 册，对价=原册及散落条目的替代合并（详见册末版本记录），符合宪法 §4 全资产净零。
- **适用范围**：一切多车道/多会话并发施工场景——战役总控（总包）作业、并发施工线（车道）作业、worktree 隔离施工、提交队列作业；单线 15 步闭环编排仍以 construction_workflow_policy.md 为真源，本册不复制其内容。
...
### 入队前标准一步：进程内门禁预跑（原全流通战役 B19 载体，R-065a 手法保留）
```bash
...
```
- **为什么必须是标准一步**：`run_gate_chain.py` 只聚合**脚本型**子门禁，**预跑不到该役任何一条死因门**
...
- token 载体 `docs/01_policies_and_standards/_registry/catalogs/capability_canonical_file_registry.yaml` 与代码同批提交的**精确判据**（门源码定论，z-orphan 反例校正）：
  ...
  **「token 必须对那棵工作树可见」= ①已在 HEAD，或 ②随本批入提交面，二者必居其一**。
  ...
  → 推论：他车道已把 token 送进 HEAD 时**不必再带注册表**（该役 G3 车道即此）；
...
- ★ **日期函数编号序不得信文档，必须实测**（该役 B15 批实弹新增失效型）：
...
- 注册表被 REGISTRY-MASS-DELETION / HOT-FILE-BASE-FRESHNESS 拦：**先分诊，两种病处方相反**（z-testint 实测 + 总包 22:1x 复现）：
  ...
  ```
  · **B 型（该役最常见）**：工作区 `+N/-0` 纯插入、index 有删除列 ⇒ **index 存着他会话的陈旧快照**。
...
- 入队热注册表必带**基底校验**：旗在 **`scripts/commit_queue.py enqueue --base-head $(git rev-parse dev)`** 上；
  ★ **`git_commit.py` 没有 `--base-head` 这个旗**（该役实测：`git_commit.py --help | grep -c base-head` = 0，`commit_queue.py enqueue --help` 有）⇒ 走 `git_commit.py --enqueue` 时不要带它，会报未知参数；  热册且需要基底校验的批次改用 `commit_queue.py enqueue --base-head …` 直入队。
...
- 数据库访问一律 `DatabaseService`（`zephyr.infrastructure.database_service`），**禁裸 duckdb.connect**。
  ...
  `DatabaseService` 自身也无 `.query()`（该役三条车道各猜一次）。
...
- ★ **该役（全流通战役）8 条车道死于 150 轮上限且未交回报**（2026-09-18 夜实测）。三条保命动作**每次都做，缺一条就是永久损失**：
...
- **写盘一律显式 `newline="\n"`**（`Path.write_text` / `open(p,"w")` / `safe_write_text` 三个入口同规）。
  致病面不是某个生成器，而是**任何一次 `newline=None` 的文本写盘**——该役总包自己落地的那件
...
- **队列落地后 index 恒陈旧（该役第 8、9 次现场复现，含 `68b80cd512` 那 16 件）**：
...
| 日期 | 版本 | 变更 | 依据 |
|------|------|------|------|
| 2026-09-21 | 1.0.0 | 由 `docs/_working/fullflow_campaign/CONSTRUCTION_DISCIPLINE.md`（ttl=task_bound）晋升正典；内容全量随迁零删减，仅战役专名改通用表述（原专名括注保留）；原册冻结随档转历史证据，不再单独维护 | SOP 晋升令；宪法 §6 检索序（construction_sop 族） |
