...
> 本文件是替换前的 AGENTS.md 完整原文归档（#ARCH-310 R3 切换）。所有历史规则细节以此处可检索；
> 现行宪法=agent_constitution_l0.md。规则细节的正式真源仍是 docs/01_policies_and_standards/rules/*.yaml。
...
> **新 AI 必读三件套**（2026-08-31 新增；2026-09-04 真源接通铁律增补；2026-09-05 "全图全库对齐"简称裁定 #ARCH-ALIGN-NAMING-001）：①[全项目对齐清单](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/sop/governance_sop/alignment_checklist.md)（**全图全库对齐**——现八图+49 注册表+代码文档三层对齐规则，计数无关简称不随增长腐化；trading_decision_map 第七图已入表；industry_chain_map 第八图已入表（2026-09-11 八图满贯：BUSINESS-REGISTRY gate 139 同批扩至 19 文件/21 段+字典 FK+CAND 转正链+治理双向，统一入口 align_all 七节））②[施工流程 SOP](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/sop/construction_sop/construction_workflow_sop.md)（端到端 15 步施工闭环；**Step 3.5 后端盘点与拆件判定=前端施工强制闸**）③前端施工前必读：[前端技术手册](file:///d:/ZephyrAlpha/docs/03_modules/_domain_frontend/frontend_handbook/)（避坑事实库）+ [TRAE-086 前端功能模块施工铁律](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/rules/trae_086_frontend_module_construction.yaml)（拆件判据/命名/目录归属/四件套闭环/**§truth_source_wiring 后端盘点与真源接通——前端不许自建数据世界**）+ [前端拆件 SOP](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/sop/construction_sop/frontend_component_split_sop.md)（有得拆时的 8 步闭环）——**不知道要对齐什么=施工必有漏项；不知道拆件铁律=前端必回大杂烩；不盘点就造数据=前后端幻觉漂移（FRONTEND-TRUTH-SOURCE gate commit 时点名）**
...
>
> **全图全库对齐铁律（D38，2026-09-07 Owner 裁定；2026-09-11 八图满贯扩容）**：全景图已扩至八图（+frontend_map+trading_decision_map+industry_chain_map 产业链全景图，图 8 git 侧=INDUSTRY-CHAIN-MAP gate 141、数据层=graph_quality_check 引擎判定），业务资产库 16 个全部挂 TDM 交叉轴（`_XREF_SPECS` 表驱动，门禁 R3-R36）。**以后新上的图和库都必须挂上去打通**——①新建业务资产库：同 commit 在 `src/zephyr/trading/decision_map.py` `_XREF_SPECS` 加轴（两行）+ 真源挂载，治理类库在 `tests/trading/test_decision_map.py::TestNewRegistryGate._GOVERNANCE_EXEMPT` 登记豁免，二选一必做（`tests/trading/test_decision_map.py` 门禁测试强制）；②新建全景图：以 module_id 为对齐 key 挂入八图体系（MOD 总线两跳互通，不搞 N² 直连）并在 alignment_checklist §3 登记。规则真源见 [alignment_checklist.md](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/sop/governance_sop/alignment_checklist.md) §4 统一原则 5/6。
>
