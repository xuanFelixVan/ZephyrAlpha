...
depends_on:
  ...
  - trading_decision_map_pathfinding_sop
...
### 4.7 命名与词表标准（2026-09-07 增补；2026-09-10 v1.7.0 链名分类学升级）

#### 4.7.0 链名分类学（Owner 2026-09-10 指令："命名专业化+逐簇梳理"——五类链模板+禁令+逐簇清理规程）

> 背景实测：半导体相关 59 条链中约 25 条为单环节空壳（"碳化硅（SiC"截断名挂 23 家公司）、报告名垃圾链（"中国绿色工厂科技创新指数"挂 98 家）、外文缩写裸名（"IC""PVD"）。命名乱的根因=没有链的类型边界。

**§4.7.0.1 专业对标结论（Owner 2026-09-10 指令全网调研：专业机构/量化社区/开源三路，用途=回测/新闻检索/盘中决策）**

| 机构/社区 | 体系 | 关键做法 | 对本项目的启示 |
|---|---|---|---|
| MSCI/S&P GICS | 自上而下 4 层分类树（部门→行业组→行业→子行业，8 位编码） | 命名=标准化分类学词汇，按"公司主要收入来源"归类 | **行业锚点链=分类树**，命名跟申万词表（我们 category 已是申万一级），禁自创 |
| FactSet RBICS | 自下而上 6 层、按产品服务、1700+ 组；RBICS with Revenue 把分部营收映射到细分行业 | 公司→产品→细分行业组，量化 peer grouping 首选 | 营收归因对标 ig_product_revenue（§4.9）；产品词命名参考其粒度 |
| FactSet Revere 供应链 | 供应链关系图（customer-supplier）是**独立产品**，与分类树分离 | **分类与供应图分离**——供应关系天然跨行业，不能拿它当分类 | **病根确认**：我们的 galaxy 聚类把行业锚点链当节点吸簇（化工簇混进公路运输/游艇=跨行业供应边焊接），专业做法是分类轨与群落轨分离 |
| 申万 2021 | 31 一级/134 二级/346 三级分类树，父代码建树 | 四原则：盈利驱动/估值聚类/物理形态/使用习惯 | 行业命名=分类学词汇封闭词表；锚点链名=THS/申万标准词，禁改写 |
| 中证指数《资本市场产业链图谱业务标准及数据标准研究》 | 行业级图谱标准课题 | 图谱粒度/数据质量需行业标准 | 我们 S 系列合格线同思路；图谱粒度=链/环节/公司三层 |
| Wind 产业链库 | 16 万条上下游图谱，与行业分类**并存两套** | 产业链=环节上下游图谱；行业=分类树 | 簇内"子类目"应展示产业链与环节，行业锚点链按分类树另轴 |
| ChainKnowledgeGraph（开源） | 公司/行业/产品**三实体**+六关系 | 行业=树（上级关系），产品=网（上下游关系），公司两边挂 | 我们三层模型对标：链（分类+产业两轨）/环节/公司，产品层=ig_product_revenue |
| 量化社区/学术 | customer-supplier momentum 文献 | 供应边命名 supplies_to/customer_of 方向恒定、revenue_pct 优先 | §4.8 已对标（PIT 三时间戳+revenue_pct），命名侧沿用 |

**落地三原则（从对标收敛）**：
1. **双轨分离**：行业分类轨（锚点链，申万/THS 词表命名，静态树）与产业群落轨（产业链/子链，环节链命名，聚类分群）严格分离——**行业锚点链退出 galaxy 聚类参与**（api_server 修正，另批 ACC 声明），簇=纯产业群落，簇内"子类目"只出产业链与环节。
2. **命名三段法**（产业链标准名）：`[区域/市场前缀(仅全球链"全球")] + 核心产品或工艺的标准中文名 + 产业链`。核心名依据：国标/行业术语标准/申万词汇（如"碳化硅""光刻胶""电子特种气体"），禁文章句式、禁现状描述（"XX发展现状/供需格局"是文章名不是链名）。
3. **环节命名**：链内环节=标准工序/部件名词（≤10 字名词性，如"衬底""外延""晶圆制造"），上游→中游→下游只作 tier 位置不作名字成分（禁"XX上游"后缀——S7 已拦）。

#### 4.7.0.2 五类链命名模板（封闭分类，每条活跃链必须能归入且仅归入一类）

| 类 | 命名模板 | 例 | 结构要求 | 判据 |
|---|---|---|---|---|
| 行业锚点链 | `{THS二级行业}行业` | 半导体行业、食品加工制造行业 | 行业聚合节点结构（by-design 无产业链路），承载 THS 落位 | 有"行业聚合"节点 |
| 产业链 | `{核心产业}产业链` | 半导体产业链、光刻胶产业链 | **必须有完整环节路**：实质节点≥3+上中下游层级+structure 边连通（S21 口径） | 有真实环节结构 |
| 子链（下钻） | `{环节或细分}产业链` | 划片机产业链、先进封装产业链 | **必须挂接母链环节**（node.child_chain_id+drill_status='child'，§6 深度体系）——不挂接的细分链=待归位 | child_chain_id 非空 |
| 全球主干链 | `全球{产业}产业链` | 全球半导体制造产业链 | market='global'；与 cn 链靠"全球"前缀区分（大行业套大行业的观感=缺子链挂接所致，正解=挂接不是改名） | market=global |
| 废弃链 | —（merged_into 指向吸收者） | — | status='deprecated'，唯一出口 | — |

**命名禁令（引擎 S25 检查项+写入工具双重拦截）**：

1. 禁报告/文章/指数名（S1 已拦标题腔，扩展：名称含"指数/白皮书/研究报告/年鉴"即违规）。
2. 禁截断/悬空名：括号不闭合（"碳化硅（SiC"）、以"的/了/与/及"等虚词结尾（"中国算力产业崛起的"）。
3. 禁外文缩写裸名：链名须为中文产业名；缩写（IC/PVD/ASIC/MEMS）一律进 aliases，不进 name。
4. 禁环节名当链名：单环节主题（"电子特气与电子大宗气体"）要么补环节升格为产业链、要么并入母链相应环节——挂着一堆公司却只有链名自身一个节点的链=S24 僵尸链重点来源。
5. 禁父链缺挂接：细分链与母链同 category 且语义为子集关系时，必须显式挂接（child_chain_id），否则前端"大行业套大行业"观感无解。

**逐簇梳理规程（"一集一集"，Owner 2026-09-10 定节奏）**：

每集=一个 galaxy 簇（C01~C47，γ=3.5 聚类）。单集闭环六步：

1. **清点**：簇内全部活跃链按五分类归位，产出分类清单。
2. **裁定 dry-run**：逐链给出处置（合规保留/改名升格=新建规范链/并入=deprecated+merged_into/废弃=同并入/挂子链=child_chain_id），涉及存量行的废弃与并入须 Owner 按集批准（样板批准后同款规则批量化，§4.7.4 先例）。
3. **执行**（全部走 ingest 通道）：新建规范链（补环节+structure 边）→ 落位迁移（旧链落位 PIT 关闭+新链重挂）→ 旧链 deprecated+merged_into → 母链挂接子链（node 记录带 child_chain_id）。
4. **引擎复跑**：S1~S25+S24 该簇零违规，advisory 递减。
5. **汇报**：第 N 集完成（簇名/处置清单/数据增量/残留缺口）。
6. **顺序**：按簇内垃圾链密度降序排集（半导体簇=第一集，垃圾密度 25/59 最高），逐集推进直至全 47 簇干净。
...
> **【2026-09-12 退役】** Owner 第一性原理裁定：tier 停止人工填写——相对位置（上/中/下游）随链相对、无机械判定依据，AI 标注口径漂移（实证：686 条 unspecified、306 条治理债、S21 15 条"缺 tier 层标记"伪缺口）。层位改由 ig_edge 链内拓扑派生（消费/展示侧推导：入度0=源头、出度0=终端，不落库）；引擎 S6 tier 检查废止、S21 改拓扑起讫；**websearch/ckg 新写入一律不填 tier**；存量值保留作历史快照不清洗。下词表仅为存量兼容存档：
...
| ⑥事件种子 | 什么事件正在冲击本环节？ | 宽度三闸事件传导路径 | `"{公司/环节} 断供 扩产 涨价 2026"` | 事件边（PIT 三时间戳）+source_doc 新闻 URL |

**防噪音四道闸（写入前逐条过，任一不过=登记缺口不落库）**：

1. **来源可溯闸**：§4.2 source_doc 三段式（查询词|URL|日期）100%——已有硬校验，升格为闸 1。
2. **交叉验证闸**：关键边/锚点边 ≥2 独立来源（§7.6 多 agent 投票）；单来源 confidence 0.5 + 状态待核，不入 P1 消费口径。
3. **适配与词表闸**：A 股口径+词表硬约束集中检查——edge_type v2 词表/tier 9 值（websearch 禁 unspecified）/挂链≤20/股权禁入业务边表/UNLISTED 唯一 UE- 格式。
4. **可验证闸**：evidence_text 原文摘录一句（反幻觉锚）+ PIT 三时间戳（§4.8）+ 抽取走 JSON schema 约束（§4.9）——回读原文找不到依据的零容忍。

**盲点自查机制（每夜收尾新增，与宽度三闸并列）**：第 7 轮收尾前跑三项——①六向是否有向未探（按 P1 链环节抽样）②四闸是否有闸被绕（抽样新批次核对 evidence/PIT/词表）③反幻觉抽验（随机 ≥3 条本夜新边，回读 source_doc URL 复核原文依据是否真存在）。结果进夜班报告「寻路盲点自查」节，缺口进下夜班种子清单；T4 半年强制保鲜轮加跑一次全量盲点自查。
