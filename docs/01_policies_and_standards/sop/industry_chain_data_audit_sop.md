...
**下夜班主攻顺序**（按数据增量价值排）：
1. **R3 覆盖冲刺**（47.5%→目标 80%，cn 落位公司 2476→~4100）——数据增量最大主战场，按行业组并发。
2. **R1 治理执行**（前提：Owner 醒后批量批准 dry-run 方案）——改名 46+并入 73+废弃 29（含 4 条改判）→ 合并方案 A-D 执行 → tier 380 条落地（306 待处置留 Owner）。
3. **光伏链重建**（环节名垃圾残留，依赖 tier/改名先落地）。
4. R5 年报专项（P1 链 TOP5 客户/供应商带 revenue_pct）。
...
□ 开工
  ...
  □ backup 九表（superuser 通道）
  □ stats 基线 + 缺口总账七清单（含命名不规范/过期链）
  □ 链工作清单排优先级（P1/P2/P3）
□ 大循环 N（§8.4：跑完不结束，连续两个大循环零发现才结束）
  □ 第1轮 存量质量：重复链核验（dry-run 方案）/ 死映射 / 孤岛节点 / category 补全
  ...
  □ 第7轮 收尾（必跑）：stats 终态对比 / 报告落盘 / progress 终态 / git 干净
  □ 记录本循环发现数 → >0 则开下一个大循环；连续两个零发现 → 终态汇报
□ 收敛三闸：发现数严格递减（振荡即停）/ 07:00 时间盒优先 / 单夜 ≤4 大循环
□ 全程纪律：每批一事务 / 只增不删 / 不问用户 / 子代理最大并发 / progress 总控独写
```
