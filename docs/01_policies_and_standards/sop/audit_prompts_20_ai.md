...
# AI 自主审计+治本修复闭环提示词（v3.5，2026-08-28 新增公共指令 3.5 封装与集成模式审查七维（封装质量/集成模式/管道契约/僵尸模块/并发安全/错误一致性/配置正确性）；v3.4 新增公共指令 10.4 六者交叉语义核对 + AI-21 五图表头语义对齐横切域；v3.3 吸收夜班实证三教训：双份承载对齐/错误码登记完整性/重号处置规则；v3.2 吸收会话审查指令精华；v3.1 在途自适应；v3 自主化；v2 范围+清单双升级）
...
# 域自主审计+治本修复指令（v3）
 ...
 3.5 封装与集成模式审查 [适用:全类·每轮必做]：
 - ①封装质量：模块公共 API 是否最小化（只暴露调用方真正需要的接口）；内部实现细节是否被隐藏（private 函数/变量不泄漏）；是否存在"上帝模块"（单文件承担 ≥3 个不相关职责）；模块间是否通过明确定义的接口/契约通信，而非直接 import 内部实现。
 - ②集成模式最优性：模块间通信方式是否与场景匹配——松耦合场景用事件驱动，强一致场景用直接调用；是否存在"旁路调用"（绕过封装层直接操作内部状态/内部函数）；数据流是否单向可预测（禁止环状依赖导致的双向数据流）。
 - ③管道阶段契约：数据在管道各阶段流转时，每个阶段的输入/输出契约是否明确（类型/格式/边界）；错误在管道中的传播方式是否一致（禁止有的阶段抛异常有的阶段静默吞掉）；是否存在"管道泄漏"（中间处理状态被外部直接读取而非通过管道末端输出）。
 - ④僵尸模块检测：本域内是否存在"活着但没人用"的模块——有代码有表头有蓝图，但全仓 grep 无任何其他模块 import 它、无 battle_map 环节锚定它、无 boot_hooks 注册它。命中则评估：功能是否已被其他模块覆盖？是否应走退役流程？
 - ⑤并发安全：共享状态（注册表/DB/文件系统）的读写是否有竞态风险；TOCTOU 模式（先检查后操作）是否有原子化保护；多会话并发场景下的文件锁/session 隔离是否完备。
 - ⑥错误处理一致性：同一调用链上，各环节的错误处理风格是否统一（全抛异常 or 全返回错误码 or 全 log+continue，禁止混搭）；错误是否携带足够上下文（哪个模块/什么操作/什么原因）；禁止静默吞异常（except: pass / except: continue）。
 - ⑦配置值正确性：YAML 注册表/契约/门禁配置中的值是否与实物一致——域标签拼写、文件路径引用真实存在、枚举值用词表合法 key 而非自由文本、阈值数值在合理范围。
 - 执行方法：抽样本域 3-5 个核心模块，逐项检查①②③；全量扫描本域④⑦；⑤⑥按改动涉及面触发。发现问题记入问题清单。
