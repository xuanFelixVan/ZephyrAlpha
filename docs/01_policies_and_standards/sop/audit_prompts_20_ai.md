...
# 全仓打扫卫生 + 自主审计治本闭环（v5，2026-09-20，裁定#375 内收扩面）
...
3.6 **内收审查义务条** [全类·v5 新增，裁定#375]：宪法 §4 已升"全资产净零"——每轮审计对本域触碰/新增的每个资产（规则/gate/脚本/配置册/模块）逐件过四判据：**同真源可派生→必并｜零触发零消费→退役｜同域重复簇→收敛唯一｜跨域不同对象→不并**。判定须留消费方证据（grep+计划任务+注册表三查），禁凭印象判"没人用"；命中必并/退役的进域修复清单并计入第 4 章内收指标，与季度合并审计（W5-3 机制）同源对账。
...
> **内收节（v5）**：域子代理任务书另附 3.6 义务条+本域近一季度合并/退役台账（若有），域模板内收发现单列一节汇报。
...
| **内收执行数（合并并掉/退役删掉，v5）** | 本轮 3.6 判定落地计数（commit 实证） | 现场计 |
