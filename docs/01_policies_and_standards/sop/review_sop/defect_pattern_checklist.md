...
| 4 | 双份承载漂移 | N-16 豁免名单 YAML↔兜底常量（audit_prompts §2.2 双份承载对齐） | 配置几处承载？改真源后副本同步？一致性红未搁置？ | D |
| 5 | 编号失控 | 43 码未登记+5 重号（audit_prompts §2.5 错误码双查，127f12d82d 再犯；该批存量已闭合，`error_code_registry.known_duplicates=0`，留作先例）；号段竞速撞车（5697da2892/20065c08f1） | 新码新号全仓 grep 唯一入册？分配前查最大号？ | D |
