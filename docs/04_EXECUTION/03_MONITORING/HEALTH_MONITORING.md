...
class SimpleHealthCheck:
    ...
    def __init__(self):
        self.checks = [
            ...
            self.check_data_fresh,    # 数据新鲜度
            self.check_disk_space,     # 磁盘空间
            self.check_error_logs,     # 错误日志
            self.check_pipeline_status # 流水线状态
        ]
...
def check_error_logs(self) -> CheckResult:
    ...
    for line in lines[-100:]:  # 只检查最近100行
        if 'ERROR' in line:
            try:
    ...
    return CheckResult(
        name='error_logs',
        ok=len(recent_errors) < 10,  # 少于10个错误认为正常
...
正常项: {sum(1 for r in report.checks if r.ok)}
异常项: {sum(1 for r in report.checks if not r.ok)}
"""
...
- 父目录: [04_EXECUTION/03_MONITORING/README.md](./README.md)
