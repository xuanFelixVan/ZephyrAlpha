...
```
模拟交易系统
├── 模拟环境管理器 (Simulation Environment)
│   ├── 市场模拟器
│   ├── 订单模拟器
│   ├── 成交模拟器
│   └── 滑点模型
...
class SimulationConfig:
    ...
    commission_rate: float = 0.0003      # 佣金万分之3
    stamp_tax_rate: float = 0.001        # 印花税千分之1
    slippage_model: str = "percentage"   # 滑点模型
    slippage_rate: float = 0.0005        # 滑点率0.05%
...
class MarketSimulator:
    ...
    def simulate_fill(
        ...
        if np.random.random() > fill_prob:
            return 0, 0, OrderStatus.PENDING, "成交概率未满足"
        ...
        if np.random.random() < self.config.partial_fill_ratio:
            ...
            if filled_qty > 0:
                ...
                return 0, 0, OrderStatus.PENDING, "部分成交数量为0"
...
class SimulationEngine:
    ...
    def generate_daily_report(self, date: date) -> str:
        ...
        lines.append("")
        lines.append(f"今日成交: {len(day_trades)}笔")
...
def example_simulation():
    ...
    print(f"总盈亏: {account.total_pnl:,.2f} ({account.total_pnl_pct:.2%})")
```
