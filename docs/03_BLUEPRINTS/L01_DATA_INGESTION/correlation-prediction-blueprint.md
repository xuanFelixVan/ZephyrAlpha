...
- 全库 API 与事件约定真源：`API_Contract.md`。相关性预测结果（协方差/相关矩阵）向组合优化或风控暴露时的字段口径须在该真源或本文后续接口说明中闭合。
...
- 能从本文中明确“输入数据 → 相关性/协方差预测 → 输出格式 → 下游使用方（组合优化/风控）”的最小闭环，并能指向 `API_Contract.md` 的对应契约入口或补全计划。
...
- 指标口径与频率需在施工文档阶段与组合优化/风控模块对齐；以本节门禁为准。
