...
responsibility:
- 策略选择
module_id: STRATEGY_SELECTION_001_4783
version: 1.0.0
status: Active
...
1. **功能完整性**: 确保STRATEGY SELECTION功能完整，满足业务需求
...
采用STRATEGY SELECTION化设计，分层架构实现。
...
| 已有模块 | 集成方式 | 接口定义 |
|----------|----------|----------|
| **BatchEvaluation系统** | 绩效数据输入 | 获取策略历史绩效数据 |
?|
...
"""
        return {
            ...
            'complexity_score': 0.05,   # 复杂度：策略简单性
...
class DynamicWeightOptimizer:
    ...
    def _calculate_base_weights(self, market_state: MarketState) -> Dict[str, float]:
        ...
        # 根据市场状态强度调整权重
        strength_factor = market_state.strength  # 0-1 之间的强度
        adjusted_weights = {}
    ...
    def _adjust_for_user_preferences(self, base_weights: Dict[str, float],
        ...
        # 风险偏好调整
        risk_tolerance = user_prefs.risk_tolerance  # 1-5（1 为极度保守，5 为极度激进）
        
        if risk_tolerance <= 2:  # 保守用户
...
class TimeWeightedPerformanceScorer:
    ...
    def _time_weighted_annual_return(self, equity_curve: pd.Series) -> float:
        ...
        # 将权益曲线分段计算收益
        n_segments = min(10, len(equity_curve) // 30)  # 每段约 30 个交易日
        segment_returns = []
...
class AIStrategyRecommender:
    ...
    def _construct_ai_portfolio(self, strategies: List[Strategy],
        ...
        # 根据用户风险偏好确定组合规模
        if user_prefs.risk_tolerance <= 2:  # 保守用户
    ...
    def _generate_explanation(self, portfolio: Portfolio,
        ...
        explanations.append(
            f"组合风险等级: {risk_level}，最大回撤预估 {risk_assessment.expected_max_dd:.1%}"
        )
...
- **对外接口边界**：本模块对外提供策略筛选/评分/选择结果的产出能力；不负责交易执行，不替代策略研究对策略池与口径的最终定义。

## 验收标准（可检查）

- 在给定策略池与评价指标输入时，能够产出可复核的策略选择结果（含评分/排序与理由摘要），并记录输入摘要与版本信息以便追溯。

## 已知限制

- 选择结果对评价指标与数据质量敏感；实施阶段需在契约真源或子契约中固化指标口径、数据缺失处理与降级策略。
