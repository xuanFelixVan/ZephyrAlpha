...
doc_type: index
ttl: permanent
title: "21个AI审计一键复制提示词"
---

# AI 自主审计+治本修复闭环提示词（v3.4，2026-08-22 新增公共指令 10.4 六者交叉语义核对 + AI-21 五图表头语义对齐横切域：结构层 align_all.py 机检已覆盖，AI 只审机检盲区语义层；v3.3 吸收夜班实证三教训：双份承载对齐/错误码登记完整性/重号处置规则；v3.2 吸收会话审查指令精华；v3.1 在途自适应；v3 自主化；v2 范围+清单双升级）

> **⚠️ 禁止删除本文件（PROTECTED FILE）** ⚠️
> - 用途：全项目"自动审计→自动治本修复→复检循环→零问题闭环"的总控+21 域子代理提示词
...
> **使用方式（v3 主用法）**：对本项目 AI 说一句"按 docs/audit_prompts_20_ai.md 执行"即可——该 AI 扮演总控 AI-00：读取本文件 → 并发启动 21 个域子代理 → 各域自主审计+治本修复+复检循环 → 总控共享收口 → 全局复审 → 未零则再派单，直到全局问题=0 才向用户一次性汇报。中途不问用户、不汇报中间态。
> - AI-00 = 总控编排指令；AI-01~20 = 20 个文件域子代理 + AI-21 = 五图表头语义对齐横切域（自包含，由总控复制全文派发，子代理无对话上下文）
...
## 1.A MODE-A 只审波（零写入）
- 并发派 21 个"只审不修"子代理：任务描述 = 对应 AI-XX section 全文 + 末尾附加一行"本轮 MODE-A 只审不修：禁止任何文件写入/提交/建 worktree，按第十五条格式返回问题清单+修复建议（不执行修复）"
- 回收 21 域问题清单（这是修复波的作战地图）
...
- 用 Task 工具在单条消息内并发启动 AI-01~AI-21 全部 21 个子代理（平台并发上限内全并发；若超限则分批，批内仍并发）
- AI-21 并发纪律：AI-21 为横切域，与 20 文件域同波并发——文件级重叠靠避让登记+总控串行 merge 吸收；若实测冲突率高的文件，AI-21 记避让登记顺延复检轮，禁止抢改
- 每个子代理的任务描述 = 本文件对应 AI-XX section 的完整全文复制（提示词自包含；子代理看不到本对话，禁止只给摘要）
- 子代理类型：通用编码代理（需读写+执行权限）

## 3. 回收汇总
- 收齐 21 份结构化结果（各域第十五条格式），汇总四张表：已修复清单 / 自主裁定清单 / 共享收口清单 / 遗留项清单
...
# 域自主审计+治本修复指令（v3）
 ...
 2.4A 僵尸系统实证审查（2026-08-28 ide_health_daemon 事故实证；守护/监控/清理/看门狗/健康检查/nanny/daemon/reaper 类功能专项）：禁止只看代码设计意图，必须做生命周期实证，五信号任一命中即判僵尸系统并启动 salvage 流程：①零调用者——grep 全项目生产调用点=0（仅自身/测试/__init__导出引用），登记式机制（track/register 类）尤为高危，肇事者恰恰是最不会自觉登记的；②守护已死——PID 文件 stale/计划任务不存在/进程实测不在运行且无人发现（谁守护守护进程=递归缺陷：常驻守护自身也是残留风险源，OS 托管 one-shot 优于进程内常驻）；③君子协定失效——依赖"AI 会话自觉执行"的冷启动/登记/清理步骤实证无人执行（100% AI 开发下自觉=不可依赖，须由门禁或 OS 强制执行）；④静默失效——名义在跑但端到端实测从未生效（注入一个检测目标验证系统是否真报警；实证：psutil name 精确匹配 bug 致幽灵窗口扫描永远扫空）；⑤无人敢接线——模块写好但阈值/副作用危险（如一刀切超龄即杀会误杀永久服务），白名单优先于黑名单+fail-safe 退化方向（故障只许退化为不清理、不许退化为误杀）是此类功能的可接线前提。salvage 流程（判定僵尸后执行，禁止直接删文件）：①迁移活能力——grep 每个公开函数的真实消费方，有活消费方的函数迁入新系统或消费方改引用；②解接线——boot_hooks/事件订阅/注册脚本中的注册点逐一解除；③删文件及其随属测试；④同步注册表（translation/log/capability registry 删死条目）与引用方头部[CONSUMERS]注释（防治理 validator 告警）；⑤验证——import 链路+消费方测试+dry-run 实测新系统真能端到端检测到目标。
 ...
 10.4 六者交叉语义核对 [适用:全类·每轮必做，覆盖存量文件]（机检盲区的语义层；结构层对齐以 align_all.py 机检为准，禁止人工重复机检已覆盖项）：
 - 六者 = 代码表头（物理真源）/ 蓝图 blueprint.md（设计真源）/ depgraph / dataflowgraph / decisiongraph / battle_map（四图派生投影）。发现漂移只改真源（表头/蓝图/登记条目），派生图一律重跑生成器重建，禁止手改派生文档。
 - ①表头声明真实性：[DEPENDENCIES] 与文件真实 import 逐一对齐（多头/漏头/死依赖）；[CONSUMERS] 与全仓反向引用核实；[MATURITY] 与物理存在性一致；[DOMAIN] 与 depgraph 节点域归属一致。
 - ②表头↔蓝图双向锚定：[BLUEPRINT] 指向的蓝图必须存在，且该蓝图 §0.1 锚定清单回含本文件；蓝图 §0.6 五图对齐视图与实物（depgraph 节点/数据流/决策流/battle_map 环节）一致。
 - ③蓝图叙事真实性：蓝图"功能/职责/接口"描述与代码真实行为一致（读代码验证，禁止凭蓝图自述背书）；叙事过期=漂移，治本=更新叙事（设计真源），禁止改代码迎合过期叙事。
 - ④battle_map 叙事（本域涉及 battle_map 环节时）：环节叙事与锚定模块真实职责一致；机检软问题（孤儿环节/缺失叙事 BM-INV-003/域漂移 BM-INV-004）落在本域的，查清根因并修叙事真源（module_translation_registry 叙事条目）。
 - 抽样纪律：本域文件 ≤30 全量；>30 按"机检软问题清单命中者优先 → 近 30 天变更者优先 → 其余随机"抽满 30，抽样依据记入结果。
 - 存量无表头文件记入问题清单（锚定缺失）；发现代码行为本身错误（非声明漂移）照常走本域问题清单治本修复；发现跨域漂移（别域蓝图/叙事与本域代码矛盾）记入共享收口清单交总控转派。
...
```
你是项目审计修复 AI-21，负责以下区域（五图+表头语义对齐，横切域）：

横切对象（无自有文件区，责任对象是"六者语义一致性"本身）：
- 代码表头：全仓 .py/.ps1/.yaml/.json/.md 治理锚定表头（trae_047 15字段族）
- 蓝图：d:\ZephyrAlpha\docs\03_modules\ 下全部 blueprint.md（含 §0.1 锚定清单 / §0.6 五图对齐视图）
- 四图真源：PostgreSQL depgraph / dataflowgraph / decisiongraph 三图节点 + battle_map 三表（steps/anchors/edges）
- 派生文档（只读核对用，禁止手改）：d:\ZephyrAlpha\docs\02_enterprise_architecture\ 下 01_global_architecture_diagram / 05_dataflow_architecture / 06_decision_architecture / 07_trading_decision_architecture\battle_map

审计重点：机检（align_all.py）覆盖不了的语义层漂移——表头声明 vs 真实 import/反向引用、蓝图叙事 vs 代码真实行为、battle_map 叙事 vs 锚定模块职责、生成器保真度（派生图是否如实反映真源）。结构层（孤儿/域不一致/幽灵锚点/悬空边/状态漂移计数）以机检为准，本子代理禁止人工重复结构层审查。

专属执行序列（先于通用指令各条执行，后续轮次照旧循环）：
1. 结构层基线：python scripts/governance/d5_architecture/generators/align_all.py --no-report；exit 非 0（硬问题未清零）→ 直接返回"不通过+硬问题清单"，语义层不开工（结构未稳审语义=审流沙）
2. 软问题清单（panorama_alignment_report.md + battle_map_alignment_report.md）= 语义审计线索图：命中的 module_id/step_id 优先深查
3. 按域分批语义核对（20 域逐域过）：表头真实性抽样 ≥10 文件/域（抽样规则同公共指令 10.4）+ 蓝图叙事 vs 代码行为逐模块 + battle_map 叙事逐环节
4. 生成器保真度抽查：随机 3 个生成器（generate_dataflow_diagram/generate_decision_diagram/generate_battle_map_diagram 等），核对输出文档与其 DB/真源输入是否一致（生成器撒谎=派生图整体不可信，属硬问题级发现）
5. 修复纪律：只修真源（表头字段值/蓝图叙事文字/翻译叙事登记条目）；派生图重跑生成器重建；depgraph/DB/registry 等主仓共享状态写需求记共享收口清单交总控；与 AI-18（模块文档工作区）及其他域文件级重叠时仅改语义漂移行，同文件已被域代理列入修复清单的记避让登记、顺延复检轮
6. 产出专章（第十五条结果内附加）：六者语义漂移台账——每条含 对象/六者中哪两者漂移/证据（绝对路径+行号）/治本修复/commit hash

上述区域为你的责任区：按专属执行序列+下述通用指令执行全量审查，发现问题直接治本修复，修复后复检，循环直到零问题。禁止创建任何报告文件；中途不问用户；最终结果按第十五条格式返回总控。
