...
# AI 自主审计+治本修复闭环提示词（v3，2026-08-16 自主化升级；v2 范围+清单双升级）
...
## 1. 开工准备
- 查在途 session/worktree：python scripts/session_worktree.py list ——记录各在途任务的 held_files 作为避让清单
- 确认主仓 git status 干净；若有悬挂变更先判明归属再动
...
# 域自主审计+治本修复指令（v3）
 ...
 2.2 真源唯一：是否存在多真源同步（YAML↔DB↔代码常量↔文档）。多真源必须收敛为单真源+派生缓存，禁止双向同步。重点：是否存在第二决策点（如第二个退出码分支、第二个校验入口）。
 ...
 3.3 原则③第一性原理治本 [适用:全类]：是否治本而非治标？重复簇（原子写入/加载YAML/解析frontmatter等散落多处）是否收敛为唯一实现？
