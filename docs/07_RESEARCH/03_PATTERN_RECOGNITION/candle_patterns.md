...
| **两根组合** | 吞没形态(看涨/看跌)、乌云盖顶、刺透形态 | 趋势反转 |
| **三根组合** | 早晨之星、黄昏之星、三兵形态(红/黑) | 趋势反转/延续 |
| **持续形态** | 跳空缺口、上升三法、下降三法 | 趋势持续 |
...
**定义**: 出现在下跌趋势底部的小实体蜡烛，下影线长度至少是实体的2倍，上影线很短。

**识别参数**:
```python
@dataclass
class HammerConfig:
    min_lower_shadow_ratio: float = 2.0  # 下影线/实体 最小比例
    max_upper_shadow_ratio: float = 0.5    # 上影线/实体 最大比例
    min_body_ratio: float = 0.1           # 最小实体占比
    max_body_ratio: float = 0.4          # 最大实体占比
```
...
def _calculate_hammer_confidence(self, candle: Candle) -> float:
    ...
    score += min(lower_ratio / 5, 0.4)  # 下影线是实体的5倍以上给0.4分
...
**定义**: 出现在上涨趋势顶部，上影线长度至少是实体的2倍，下影线很短。

**识别参数**:
```python
@dataclass
class ShootingStarConfig:
    min_upper_shadow_ratio: float = 2.0   # 上影线/实体 最小比例
    max_lower_shadow_ratio: float = 0.3  # 下影线/实体 最大比例
    min_body_ratio: float = 0.1           # 最小实体占比
    max_body_ratio: float = 0.4           # 最大实体占比
```
...
@dataclass
class DojiConfig:
    max_body_ratio: float = 0.05      # 最大实体占比(5%)
    min_shadow_ratio: float = 0.3     # 最小影线占比(影线要比较明显)
```
...
def recognize_doji(self, candle: Candle) -> Optional[Signal]:
    ...
    return Signal(
        pattern="doji",
        pattern_type=doji_type,  # 长腿/墓碑/蜻蜓/普通
...
def recognize_dark_cloud_cover(self, c1: Candle, c2: Candle) -> Optional[Signal]:
    ...
    return Signal(
        ...
        direction="bearish",
        confidence=0.6 + cloud_penetration * 0.2,  # 乌云越深置信度越高
        entry_price=c2.close,
        stop_loss=c2.high,
        target=self._calculate_target(c2, "bearish", c1),
...
**定义**: 下跌趋势中出现的三根蜡烛组合，中间一根为小实体(星线)，第三根阳线向上深入第一根阴线内部。
