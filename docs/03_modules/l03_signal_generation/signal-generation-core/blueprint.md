...
actual_disk_path: "src/zephyr/l03_signal_generation/"
...
depends_on:
  ...
  - target: ALPHA-SIGNAL-DOMAIN-001
    ...
    why: Alpha信号域集成
references:
  - path: "D:\\ZephyrAlpha\\docs\\02_enterprise_architecture\\target-architecture\\architecture-model\\layers\\l03_signal_generation.yaml"
    section: "§1"
    why: SSoT YAML
summary: "信号生成层C轨占位。4个OCP扩展点+2个默认实现。Phase B骨架已就位，业务层未开放，禁止施工。"
ssot_yaml: "architecture-model/layers/l03_signal_generation.yaml"
...
> module_id: MOD-L03-001 | version: 2.2.0 | status: active | layer: l03_signal_generation
> actual_disk_path: src/zephyr/l03_signal_generation/ | generation: 2 | construction_progress: partially_implemented
...
| 验证项 | 验证方法 | 结果 |
|--------|---------|:---:|
| construction_progress = partially_implemented → 已实现章节的代码存在 | `ls D:\ZephyrAlpha\src\zephyr\l03_signal_generation\` 逐文件核对 | ☐ |
| 蓝图描述的类名 = 代码中的类名 | `grep "class" D:\ZephyrAlpha\src\zephyr\l03_signal_generation\*.py` | ☐ |
...
> 强制 Pydantic V2 BaseModel（ADR-0040），禁止 @dataclass。实际契约类型为 codegen 生成的 frozen dataclass（CTR-002/CTR-P1-015），蓝图只保留接口签名。
...
| 4 | 导入源 | zephyr.l04_* / zephyr.l05_* | 仅消费 CTR 契约类型 | 分层约束，L03 不依赖 L04/L05 |
...
| MOD-L02-001 Alpha Factor | 必须 | CTR-002 FactorSignal + 因子计算结果 | — | `D:\ZephyrAlpha\docs\03_modules\l02_alpha_factor\alpha-factor-core\blueprint.md` |
| MOD-INF-015 Telemetry | 可选 | 信号生成监控 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\system-telemetry\blueprint.md` |
...
| 产出物类型 | 存放完整绝对路径 | 说明 |
|----------|---------------|------|
| 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l03_signal_generation\signal-generation-core\blueprint.md` | 本文件 |
| 业务代码 | `D:\ZephyrAlpha\src\zephyr\l03_signal_generation\` | Python 源码 |
| 测试代码 | `D:\ZephyrAlpha\tests\l03_signal_generation\` | 测试用例（待创建） |
...
| 2 | 模块 ID 注册表 | `D:\ZephyrAlpha\docs\02_enterprise_architecture\target-architecture\architecture-model\module-id-registry.yaml` | 版本号更新 | 蓝图升级 |
| 3 | 依赖图 | `D:\ZephyrAlpha\docs\02_enterprise_architecture\system-dependency-map.md` | L03 输出契约 CTR-008→CTR-P1-015 对齐 | 契约ID不一致 |
| 4 | 代码文件头部 | `D:\ZephyrAlpha\src\zephyr\l03_signal_generation\*.py` | [BLUEPRINT] 字段指向 MOD-L03-001 | 当前指向 ALPHA-SIGNAL-DOMAIN-001 |
...
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l03_signal_generation\implementations\default_signal_aggregator.py` |
| 验收标准 | import 成功 + 单元测试通过 |
| 验证命令 | `python -m pytest tests/l03_signal_generation/test_default_signal_aggregator.py -v` |
...
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l03_signal_generation\implementations\default_capital_allocator.py` |
| 验收标准 | import 成功 + 单元测试通过 |
| 验证命令 | `python -m pytest tests/l03_signal_generation/test_default_capital_allocator.py -v` |
...
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l03_signal_generation\signal_quality_metrics.py` |
| 验收标准 | L04/L08 可消费 |
| 验证命令 | `python -m pytest tests/l03_signal_generation/test_signal_quality_metrics.py -v` |
...
| 步骤 | 如果出问题 | 回滚操作 |
|------|----------|---------|
| 1 | DefaultSignalAggregator 修改破坏已有功能 | `git checkout -- src/zephyr/l03_signal_generation/implementations/default_signal_aggregator.py` |
| 2 | DefaultCapitalAllocator 修改破坏已有功能 | `git checkout -- src/zephyr/l03_signal_generation/implementations/default_capital_allocator.py` |
...
|---|:----:|------|----------|----------|----------|
| 1 | 命令 | `python -m pytest tests/l03_signal_generation/ -v` | 运行全部L03测试 | — | 0 failed |
...
| 代码文件数 | 7 | `find src/zephyr/l03_signal_generation/ -name "*.py"` |
...
| 3 | 无测试文件（tests/l03_signal_generation/ 不存在） | 高 | C轨占位未创建测试 | C轨解除后优先创建 | §9 | 待解决 |
...
| 9 | 跨层契约 SSoT | — | — | `D:\ZephyrAlpha\architecture-model\contracts\cross-layer-contracts.yaml` | CTR 契约定义 |
...
|---|---------|------------|----------|-------------|
| 1 | L02 Alpha Factor | `D:\ZephyrAlpha\docs\03_modules\l02_alpha_factor\alpha-factor-core\blueprint.md` | 因子信号产出 | L02 是因子计算层，L03 是信号聚合层，职责不同 |
...
|---|---------|------------|------|---------|
| 1 | 信号生成代码 | `D:\ZephyrAlpha\src\zephyr\l03_signal_generation\` | 修改 | 蓝图描述的核心代码 |
| 2 | 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l03_signal_generation\signal-generation-core\blueprint.md` | 修改 | 本文件 |
| 3 | 测试代码 | `D:\ZephyrAlpha\tests\l03_signal_generation\` | 新建 | 测试用例（待创建） |
...
| Tier | 消费者 | 依赖内容 |
|:----:|--------|---------|
| Tier 1 | `D:\ZephyrAlpha\docs\03_modules\l04_portfolio_construction\portfolio-construction-core\blueprint.md` | §4 接口契约、§10 依赖关系 |
| Tier 1 | `D:\ZephyrAlpha\docs\03_modules\l05_risk_management\risk-management-core\blueprint.md` | §4 接口契约、§10 依赖关系 |
| Tier 2 | `D:\ZephyrAlpha\src\zephyr\l04_portfolio_construction\` | §4 数据模型、§11 产出物路径 |
| Tier 2 | `D:\ZephyrAlpha\src\zephyr\l05_risk_management\` | §4 数据模型、§11 产出物路径 |
