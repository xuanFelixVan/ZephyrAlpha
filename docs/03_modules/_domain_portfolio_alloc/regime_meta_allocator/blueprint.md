...
**新 AI session 读取顺序**：
1. 读本蓝图 §1.1（核心裁定：移除 RegimeScore）→ 理解"regime 只节流不择时"
2. 读 §3.1（分配公式）→ 理解"global_shrinkage 在外控制总暴露，SampleShrinkage 在内影响相对分配"
3. 读 §3.2（Shrinkage 二维公式）→ 理解 ConfidenceSignal × RiskSignal
4. 读 §3.3（PerformanceScore）→ 理解"后验捕获策略亲和性"
5. 读 §2.3（BudgetAllocation 契约）→ 理解"和上下游的接口"
