...
<!-- AUTOGEN: source=depgraph+dataflow+decision, generator=generate_blueprint_panorama.py -->

| 图 | 位置 | 状态 | 链接 |
|----|------|------|------|
| 依赖图 (depgraph) | `blueprint_id=MOD-SIG-056` 文件节点 node_id=9823087 | design | `extract_depgraph.py --modules MOD-SIG-056` |
| 数据流图 (dataflow) | DS-080 market_data.lhb_detail（JOB-076 生产） | candidate | `apply_dataflowgraph.py --list-datasets` |
| 决策架构图 (decision) | 未登记（晋升后服务 L2 买入信号决策） | — | — |
...
- [x] depgraph design 节点（node_id=9823087）
