...
actual_disk_path: "src/zephyr/l13_experimentation/"
...
references:
  - path: "D:\\ZephyrAlpha\\architecture-model\\layers\\l13_experimentation.yaml"
    section: "§0"
    why: SSoT YAML 真源
  - path: "D:\\ZephyrAlpha\\docs\\02_enterprise_architecture\\target-architecture\\03-application-architecture.md"
    section: "§4.3"
...
> module_id: MOD-L13-001 | version: 2.1.0 | status: active | layer: l13_experimentation
> actual_disk_path: src/zephyr/l13_experimentation/ | generation: 2 | construction_progress: partially_implemented
...
| 验证项 | 验证方法 | 结果 |
|--------|---------|:---:|
| construction_progress = partially_implemented → 已实现章节的代码存在 | `ls D:\ZephyrAlpha\src\zephyr\l13_experimentation\` | ☐ |
...
| 数据模型 | dataclass | Pydantic V2 BaseModel | ADR-0040不符 | P2 |
...
> 强制 **Pydantic V2 BaseModel**（ADR-0040），当前使用 dataclass 为过渡态（GAP-003）。
...
| 3 | 编码模式 | 使用 @dataclass 做输出契约 | Pydantic V2 BaseModel | ADR-0040（过渡期豁免，GAP-003） |
| 4 | 导入源 | zephyr.l05_portfolio_construction.* | 通过 CTR 契约间接交互 | 分层约束 |
| 5 | 导入源 | zephyr.l03_signal_generation.* | 通过 CTR 契约间接交互 | 分层约束 |
...
|---------|---------|---------|---------|---------|
| MOD-L11-001 ML Platform | 必须 | CTR-011 ModelCheckpoint + CTR-P1-004/005 ModelServing | — | `D:\ZephyrAlpha\docs\03_modules\l11_ml_platform\ml-core\blueprint.md` |
| MOD-L09-001 Research | 可选 | CTR-010 ExperimentMetric 上报 + CTR-P1-014 ExperimentResult 消费 | — | `D:\ZephyrAlpha\docs\03_modules\l09_research\research-core\blueprint.md` |
...
| 产出物类型 | 存放完整绝对路径 | 说明 |
|----------|---------------|------|
| 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l13_experimentation\experiment-core\blueprint.md` | 本文件 |
| 业务代码（基类+数据类） | `D:\ZephyrAlpha\src\zephyr\l13_experimentation\pipeline_base.py` | ExperimentPipelineBase + ScoutAgentBase + ExperimentConfig + ExperimentMetric |
| 业务代码（默认实现） | `D:\ZephyrAlpha\src\zephyr\l13_experimentation\implementations\default_experiment_pipeline.py` | DefaultExperimentPipeline |
| 测试代码 | `D:\ZephyrAlpha\tests\l13_experimentation\` | 测试用例（待创建） |
...
| 3 | 跨层契约 | `D:\ZephyrAlpha\docs\02_enterprise_architecture\target-architecture\architecture-model\cross-layer-contracts.yaml` | 新增 CTR-009~012 契约定义 | 契约注册 |
| 4 | 模块 ID 注册表 | `D:\ZephyrAlpha\docs\02_enterprise_architecture\target-architecture\architecture-model\module-id-registry.yaml` | 确认 MOD-L13-001 注册 | 编号验证 |
...
| 5 | ExperimentConfig 使用 dataclass 而非 Pydantic BaseModel | 低 | 低 | Phase C 迁移至 Pydantic V2（ADR-0040） | 风险 |
...
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l13_experimentation\implementations\default_experiment_pipeline.py` |
| 验收标准 | import 成功 + 单元测试通过 |
| 验证命令 | `python -m pytest tests/l13_experimentation/ -k test_default -v` |
...
|------|------|
| 对应蓝图契约 | §4.3 CTR-009 |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l13_experimentation\pipeline_base.py` 扩展 |
| 验收标准 | L11 可消费 ExperimentConfig |
| 验证命令 | `python -m pytest tests/l13_experimentation/ -k test_ctr009 -v` |
| G7 检查项 | CTR-009 契约格式与 cross-layer-contracts.yaml 一致 |
| AI 自治范围 | ai_modifiable |
...
| 验证命令 | `python -m pytest tests/l13_experimentation/ -k test_ctr010 -v` |
| AI 自治范围 | ai_modifiable |
...
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l13_experimentation\pipeline_base.py` 扩展 |
| 验收标准 | L11 检查点可导入 |
| 验证命令 | `python -m pytest tests/l13_experimentation/ -k test_ctr011 -v` |
| AI 自治范围 | ai_modifiable |
...
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l13_experimentation\pipeline_base.py` 扩展 |
| 验收标准 | INF-012 可归档 |
| 验证命令 | `python -m pytest tests/l13_experimentation/ -k test_ctr012 -v` |
| AI 自治范围 | ai_modifiable |
...
|------|------|
| 对应蓝图契约 | §12 集成目标 |
| 产出位置 | `D:\ZephyrAlpha\tests\l13_experimentation\` |
| 验收标准 | 端到端通过 |
| 验证命令 | `python -m pytest tests/l13_experimentation/ -v` |
| AI 自治范围 | ai_modifiable |
| 检查点 | 全部集成测试通过 |
...
| 步骤 | 如果出问题 | 回滚操作 |
|------|----------|---------|
| 1 | DefaultExperimentPipeline 修改破坏已有功能 | `git checkout -- src/zephyr/l13_experimentation/implementations/` |
...
| 7 | 测试代码已创建 | tests/l13_experimentation/ 非空 | 完成 | ☐ |
...
|---|:----:|------|----------|----------|----------|
| 1 | 命令 | `python -m pytest tests/l13_experimentation/` | 运行测试 | `-k`: 过滤用例 | exit 0=通过 |
...
| GAP-003 | dataclass 非 Pydantic | 迁移至 Pydantic V2 BaseModel | P2 | ADR-0040 强制时 | v3.0.0 | 待施工 |
...
| 3 | dataclass不符合ADR-0040 | 低 | 骨架阶段快速实现 | GAP-003迁移Pydantic V2 | §5.7 #3 | 待解决 |
...
| 数据模型 | evolving | 中 | ADR-0040强制迁移 | frozen dataclass→Pydantic V2 |
...
|---|---------|------------|------|---------|
| 1 | 实验层代码 | `D:\ZephyrAlpha\src\zephyr\l13_experimentation\` | 修改 | 蓝图描述的核心代码 |
| 2 | 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l13_experimentation\experiment-core\blueprint.md` | 修改 | 本文件 |
...
| Tier | 消费者 | 依赖内容 |
|:----:|--------|---------|
| Tier 1 | `D:\ZephyrAlpha\docs\03_modules\l11_ml_platform\ml-core\blueprint.md` (MOD-L11-001) | §4 接口契约、§10 依赖关系 |
| Tier 1 | `D:\ZephyrAlpha\docs\03_modules\l09_research\research-core\blueprint.md` (MOD-L09-001) | §4 接口契约、§10 依赖关系 |
| Tier 2 | `D:\ZephyrAlpha\src\zephyr\l13_experimentation\` (代码) | §4 数据模型、§11 产出物路径 |
