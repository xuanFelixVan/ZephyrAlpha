...
depends_on:
  ...
  - target: "REM-PHYSTR-001"
    at: "全文"
    why: "物理结构1500模块可扩展性修复方案——目录结构/分片架构/模块隔离规范权威来源"
  - target: "MOD-INF-007"
    at: "§3"
...
| 审计模块 | `src/zephyr/audit_trail/` | 无 | ✅ 唯一 |
...
脚本注册表性能预算:
  ...
  第一层: 预编译 manifest 缓存
    - generate_script_manifest.py 产出:
      script_manifest.yaml (SSoT, 人类可读)
...
治理结果推送方案: MCP Push Notification（基于现有 MCP 扩展）
  ...
  实现: src/zephyr/governance/mcp_result_push.py（新建）
```
...
| **NEW-02** | `scripts/governance/_resource_guard.py` | D-GAP-14 | Worker 内存限制 + 资源监控 | ✅ 已实现（扩展 drift_detector.resource_guard） |
...
| **NEW-13** | `src/zephyr/governance/mcp_result_push.py` | D-GAP-26 | MCP 异步结果推送 | ✅ 已实现（callback/event_bus/file_watcher 三模式） |
...
| 归属模块 | 文件数 | 目标目录 | 代表性文件 |
|---------|:---:|---------|---------|
| MOD-INF-018 Agent RBAC | 9 | `src/zephyr/agent_rbac/` | defense_depth.py, kill_switch.py, anti_pattern_guard.py |
| MOD-INF-019 Agent Spec | 6 | `src/zephyr/agent_spec/` | agent_debate.py, agent_dispatch.py, vibe_coding_enforcer.py |
| MOD-INF-020 Audit Trail | 13 | `src/zephyr/audit_trail/` | provenance_tracker.py, changelog_manager.py, sbom_generator.py |
| MOD-INF-021 Rollback System | 9 | `src/zephyr/rollback/` | startup_shutdown.py, phase_manager.py, fault_tolerance.py |
| MOD-INF-022 Escalation Protocol | 10 | `src/zephyr/escalation_engine/` | incident_response.py, risk_matrix.py, spof_checker.py |
| MOD-INF-023 Drift Detector | 15 | `src/zephyr/behavioral_auditor/` | model_drift_monitor.py, architecture_contracts.py, data_quality.py |
| MOD-INF-024 Budget Enforcer | 9 | `src/zephyr/budget_enforcer/` | token_budget.py, cost_router.py, tco_model.py |
| MOD-INF-025 A2A Protocol | 8 | `src/zephyr/l01_infrastructure/a2a_protocol/` | multi_model_consensus.py, prompt_lifecycle.py, offline_autonomy.py |
...
```
方向：PS-STD-011 (MTH-006) → L0 project_rules.md + L1 onboarding_detail.md + skill_registry.yaml
触发时机：sync_rule_registry.py 每次运行时自动验证
契约定义：
  MTH-006（5 Whys 根源分析原则）的触发器 MUST 同时存在于三个位置：
    1. L0 project_rules.md — 产出五条中包含 "MTH-006" + "5 Whys" 关键词
    2. L1 onboarding_detail.md — §八包含 "治根" + "治标" + "MTH-006" 判定标准
    3. skill_registry.yaml — task_keywords 包含 "根因" + "5 Whys" + "root_cause" 路由
...
契约定义：
  ...
  SSoT：src/zephyr/agent_spec/skill_registry.yaml
...
| 类型 | 风险 | 影响 | 缓解 |
|------|------|------|------|
| 风险 | 八件套施工进度不统一 | agent_rbac 100%, 其余 7 模块 50%——集成契约已定义但施工未完成 | Phase 2 完成后→Phase 3 推进 |
...
| REM-PHYSTR-001 | 可选 | 物理结构可扩展性修复方案——目录结构/分片架构规范 | — | `D:\ZephyrAlpha\docs\03_modules\...` |
...
| 9 | AI产出物压缩工作流标准 | REG-STD-004 / GOV-DOC-011 | — | `D:\ZephyrAlpha\docs\01_policies_and_standards\governance\document\compression-workflow-standard.md` | 转化优先/15不可删/6砍错模式 |
...
| 11 | 会话门禁检查清单 | REG-STD-006 / OPS-VC-005 | — | `D:\ZephyrAlpha\docs\01_policies_and_standards\operational\vibe_coding\vibe-coding-gate-checklist.md` | 12项门禁检查 |
| 12 | AI事故响应手册 | REG-STD-007 / OPS-VC-004 | — | `D:\ZephyrAlpha\docs\01_policies_and_standards\operational\vibe_coding\ai-incident-and-emergency-playbook.md` | P0/P1/P2事故分级 |
...
| skill_registry.yaml | `src/zephyr/agent_spec/skill_registry.yaml` | Active |
| Agent RBAC 模块 | `src/zephyr/l01_infrastructure/agent_rbac/` | phase_2_complete |
| Audit Trail 模块 | `src/zephyr/l01_infrastructure/audit_trail/` | phase_2_complete |
| Rollback 模块 | `src/zephyr/l01_infrastructure/rollback_system/` | phase_2_complete |
| Escalation 模块 | `src/zephyr/l01_infrastructure/escalation_protocol/` | phase_2_complete |
| Drift Detector 模块 | `src/zephyr/l01_infrastructure/drift_detector/` | completed |
| Budget Enforcer 模块 | `src/zephyr/l01_infrastructure/budget_enforcer/` | phase_2_complete |
| A2A Protocol 模块 | `src/zephyr/l01_infrastructure/a2a_protocol/` | phase_2_complete |
| Agent Spec 模块 | `src/zephyr/l01_infrastructure/agent_spec/` | phase_2_complete |
...
| 新增治理脚本 | script_manifest.yaml + 蓝图 §11 产出物 | scripts/ + 本蓝图 |
