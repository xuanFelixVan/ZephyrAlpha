...
契约定义：
  ...
  - 不再需要 L0 ProcessLock（已降级为 Config Read Lock，仅保护 script_manifest.yaml 读取）
...
使用方：
  ...
  - behavioral_auditor/repair.py 漂移修复写入
...
```
方向：治理脚本 → arbitrate_findings.py → 任务系统（MOD-INF-006）
