...
actual_disk_path: "D:\\ZephyrAlpha\\docs\\03_modules\\_master-blueprint\\blueprint-capacity.md"
...
> actual_disk_path: D:\ZephyrAlpha\docs\03_modules\_master-blueprint\blueprint-capacity.md | generation: 3 | construction_progress: partially_implemented
...
| 2 | CT-*集成契约定义 | MOD-MASTER-002 (blueprint-baseline.md §二) |
...
| GAP-M03 | KB 分区：按 layer 拆 ChromaDB collection + metadata 预过滤 + 混合搜索 | `src/zephyr/kb/partition_router.py` | 8M vectors 查询 P99 < 1s |
| GAP-M04 | CE tiering：blueprint relevance scoring + 四级注入 + 预算分配器 | `src/zephyr/context_engine/blueprint_ranker.py` | 1,500 蓝图场景 → token 预算内注入 → AI 引用率 > 60% |

### Phase B：运维稳定加固 — 目标 v1.1.0-beta

| 缺口 | 施工内容 | 产出 | 验证方式 |
|:---:|---------|------|------|
| GAP-M05 | 脚本生命周期：状态机 + SQLite 表 + 90 天自动流转 + 质量分级 cron | `src/zephyr/script_system/script_lifecycle.py` | 标记 30% 脚本为 DEPRECATED → 验证 ARCHIVED 不参与全量扫描 |
| GAP-M06 | 冷启动编排：四层 DAG + health probe 链 + warm-up 策略 | `src/zephyr/runtime/startup_orchestrator.py` | 冷启动 → 95s 内 readyz=200 |
...
| GAP-M09 | Saga 协调器：SagaCoordinator + saga_state 表 + 崩溃恢复 | `src/zephyr/orchestrator/saga_coordinator.py` | 模拟 step_3 崩溃 → 自动回滚 step_1+2 → 最终一致 |
...
construction_sequence:
  P0_emergency:
    ...
    - {file: "src/zephyr/runtime/runtime_config.py", change: "max_parallel_l1:1→20, l2:3→10, l3:2→5", hours: 0.5}
    - {file: "src/zephyr/pipeline/pipeline_orchestrator.py", change: "token_budget:200K→2M", hours: 0.5}
  ...
  P2_multi_agent:
    - {file: "src/zephyr/orchestrator/agent_session.py", change: "新建——Agent Session生命周期管理", hours: 8}
    - {file: "src/zephyr/orchestrator/concurrency_license.py", change: "新建——80slot许可证+优先级抢占", hours: 8}
    - {file: "src/zephyr/orchestrator/capacity_scheduler.py", change: "新建——4级优先级队列+公平分配", hours: 10}
    - {file: "src/zephyr/orchestrator/session_conflict.py", change: "新建——文件锁+乐观锁+冲突解决", hours: 6}
  ...
  P4_production_hardening:
    ...
    - {file: "src/zephyr/orchestrator/deferred_queue.py", change: "内存EventBus→Redis Pub/Sub适配层(可插拔)", hours: 6}
...
| 5 | 基线蓝图 | MOD-MASTER-002 | — | `D:\ZephyrAlpha\docs\03_modules\_master-blueprint\blueprint-baseline.md` | 现存设计 |
...
|---|---------|------------|------|---------|
| 1 | 容量蓝图 | `D:\ZephyrAlpha\docs\03_modules\_master-blueprint\blueprint-capacity.md` | 修改 | 本文件 |
| 2 | 基线蓝图 | `D:\ZephyrAlpha\docs\03_modules\_master-blueprint\blueprint-baseline.md` | 读取 | 依赖 |
