...
> module_id: MOD-TRIG-001 | version: 0.1.0 | status: Active | layer: L2_domain (trading)
...
> 自动生成（五图对齐，ARCH-056），禁止手写。生成命令：`python scripts/governance/d5_architecture/generators/align_panoramas.py --module MOD-TRIG-001`

| 图 | 位置 | 状态 | 链接 |
|----|------|------|------|
| 依赖图 (depgraph) | `blueprint_id=MOD-TRIG-001` 的 file 节点 | production | `extract_depgraph.py --modules MOD-TRIG-001` |
| 数据流图 (dataflow) | 消费各 spec condition→产出 TriggeredEvent | active | `apply_dataflowgraph.py --list-datasets` |
| 决策架构图 (decision) | BM-BUY-03 决策编排（TriggerList 承载） | production | `generate_decision_diagram.py` |
| 蓝图 (blueprint) | 本文件 | Active | — |

| 字段 | depgraph 值（真源） | 蓝图 frontmatter 值（声明） | 是否一致 |
|------|-------------------|--------------------------|:-------:|
| module_id | MOD-TRIG-001 | MOD-TRIG-001 | ✅ |
| domain_id | trading | trading | ✅ |
