...
actual_disk_path: "src/zephyr/l08_human_ai_interface/"
...
> module_id: MOD-L08-001 | version: 2.1.0 | status: active | layer: l08_human_ai_interface
> actual_disk_path: src/zephyr/l08_human_ai_interface/ | generation: 2 | construction_progress: not_started
...
| 8 | implementations/default_notification_manager.py | §4.1 | DefaultNotificationManager | 未实现 |
| 9 | implementations/default_approval_gateway.py | §4.1 | DefaultApprovalGateway | 未实现 |

### §0.2 对齐验证矩阵

| 验证项 | 验证方法 | 结果 |
|--------|---------|:---:|
| construction_progress = not_started → 代码目录存在但业务实现不完整 | `ls src/zephyr/l08_human_ai_interface/` | ☐ |
...
| 11 | ❌ 排除 | Fitness Functions 计算 | feedback_loop/fitness_functions.py | MOD-INF-010 |
...
> ⚠️ 强制 **Pydantic V2 BaseModel**（ADR-0040），禁止 `@dataclass`。
...
| MOD-L04-001 Risk Management | 可选 | CTR-P1-008 RiskDashboardSnapshot | — | `D:\ZephyrAlpha\docs\03_modules\l04_risk_management\risk-core\blueprint.md` |
| MOD-L07-001 Post-Trade Analytics | 可选 | CTR-P1-009 PerformanceAttributionReport | — | `D:\ZephyrAlpha\docs\03_modules\l07_post_trade_analytics\analytics-core\blueprint.md` |
| MOD-INF-035 系统大脑 | 可选 | 运维可视化 | — | `D:\ZephyrAlpha\docs\03_modules\_cross_layer\auto-runtime-core\blueprint.md` |
| MOD-INF-015 系统遥测 | 可选 | 告警通道 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\system-telemetry\blueprint.md` |
...
| 产出物类型 | 存放完整绝对路径 | 说明 |
|----------|---------------|------|
| 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l08_human_ai_interface\hmi-core\blueprint.md` | 本文件 |
| 业务代码 | `D:\ZephyrAlpha\src\zephyr\l08_human_ai_interface\` | Python 源码 |
| 测试代码 | `D:\ZephyrAlpha\tests\l08_human_ai_interface\` | 测试用例 |
...
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l08_human_ai_interface\implementations\default_notification_manager.py` |
| 验收标准 | import 成功，send 返回 bool，channels 返回 list[str] |
| 验证命令 | `python -c "from zephyr.l08_human_ai_interface.implementations.default_notification_manager import DefaultNotificationManager"` |
...
|-----------|--------|----------|------------|
| MOD-L08-001 | default_notification_manager.py | code | `D:\ZephyrAlpha\src\zephyr\l08_human_ai_interface\implementations\default_notification_manager.py` |
...
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l08_human_ai_interface\implementations\default_approval_gateway.py` |
| 验收标准 | import 成功，submit 返回 request_id，decide 返回 bool，pending 返回 list |
| 验证命令 | `python -c "from zephyr.l08_human_ai_interface.implementations.default_approval_gateway import DefaultApprovalGateway"` |
| G7 检查项 | 上游 interface_base.py 存在，下游可调用 |
| AI 自治范围 | human_gated |
| 检查点 | 文件存在且非空 |
...
| module_id | 文件名 | doc_type | 完整绝对路径 |
|-----------|--------|----------|------------|
| MOD-L08-001 | default_approval_gateway.py | code | `D:\ZephyrAlpha\src\zephyr\l08_human_ai_interface\implementations\default_approval_gateway.py` |
...
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l08_human_ai_interface\dashboard\app.py` |
| 验收标准 | 新增风险/归因 Dashboard 页面，L04/L07 数据可展示 |
| 验证命令 | `python -m pytest tests/l08_human_ai_interface/ -k dashboard` |
...
|---|:----:|------|----------|----------|----------|
| 1 | 命令 | `streamlit run src/zephyr/l08_human_ai_interface/dashboard/app.py` | 启动 Dashboard | — | Streamlit Web UI |
| 2 | 命令 | `python -m pytest tests/l08_human_ai_interface/` | 运行测试 | `-k {pattern}` | pytest 输出 |
...
| 4 | D-L08-04 | Notification/Approval 使用 dataclass | A: Pydantic BaseModel / B: dataclass(frozen=True) | B | 历史遗留；新模型MUST用Pydantic(ADR-0040) | 2026-05-05 |
...
|---|---------|------------|------|---------|
| 1 | interface_base.py | `D:\ZephyrAlpha\src\zephyr\l08_human_ai_interface\interface_base.py` | 读取 | 无变更 |
| 2 | dashboard/ | `D:\ZephyrAlpha\src\zephyr\l08_human_ai_interface\dashboard\` | 修改 | 完善组件 |
| 3 | implementations/ | `D:\ZephyrAlpha\src\zephyr\l08_human_ai_interface\implementations\` | 新建 | 默认实现(C轨开放后) |
