...
| 日期 | 版本 | 变更内容 |
|------|------|---------|
| 2026-05-10 | 3.5.3 | **SRC-0037: event_bus_upgrade 版本分叉解决 — l01版重命名为 shared/upgrade_strategy.py ✅**：`l01_infrastructure/event_bus_upgrade.py`（升级策略，生成升级计划）已通过 `git mv` 重命名为 `shared/upgrade_strategy.py`，原位置保留为 compat shim（re-export + DeprecationWarning）。`shared/event_bus_upgrade.py`（事件版本化+增量升级）保持不变。旧 import 路径全项目零残留。event_bus 相关测试 3/0 passed，upgrade 相关测试 18/0 passed。Git commit: `0a9e14b11`。 |
