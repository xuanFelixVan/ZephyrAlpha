...
actual_disk_path: "src/zephyr/l10_compliance/"
...
> actual_disk_path: src/zephyr/l10_compliance/ | generation: 2 | construction_progress: partially_implemented
...
| 场景 | 触发 | 处理流程 | 输出 |
|------|------|---------|------|
| AI指令安全拦截 | AI指令执行请求 | SecurityGateway.pre_filter → security_scan → decide | AuditDecision(allow/deny/flag) |
...
|---|-------------|---------|---------|---------|------------|
| 1 | default_security_gateway.py（根目录） | `D:\ZephyrAlpha\src\zephyr\l10_compliance\default_security_gateway.py` | 删除 | 迁移+重定向→标记deprecated→Phase4物理删除 | Grep全项目import引用→更新为implementations/路径 |
...
| 3 | 编码模式 | `@dataclass` 用于数据模型 | `Pydantic V2 BaseModel` | ADR-0040 |
| 4 | 导入源 | `from zephyr.l04_risk.*` 直接导入 | 通过 CTR-P1-012 契约交互 | 分层约束 |
| 5 | 导入源 | `from zephyr.l06_execution.*` 直接导入 | 通过 CTR-P1-012 契约交互 | 分层约束 |
...
| l10_compliance_evaluation_total | Counter | 自动埋点 | 不合规率>50% | P2 |
...
|---------|---------|---------|---------|---------|
| INF-012 Database | 可选 | 合规规则存储 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\database\blueprint.md` |
| INF-020 Audit Trail | 必须 | 审计决策写入 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\audit-trail\blueprint.md` |
| INF-018 Agent RBAC | 可选 | 权限联动 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\agent-rbac\blueprint.md` |
...
| §10.1 依赖声明 ↔ cross-layer-contracts.yaml | 未对齐 | 待验证 |
...
| 产出物类型 | 存放完整绝对路径 | 说明 |
|----------|---------------|------|
| 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l10_compliance\compliance-core\blueprint.md` | 本文件（含设计和施工指引） |
| 业务代码 | `D:\ZephyrAlpha\src\zephyr\l10_compliance\` | Python 源码 |
| 测试代码 | `D:\ZephyrAlpha\tests\l10_compliance\` | 测试用例 |

| 产出物 | 绝对路径 |
|--------|---------|
| SecurityGateway + ComplianceEngine + AuditAction + AuditDecision | `D:\ZephyrAlpha\src\zephyr\l10_compliance\security_gateway_base.py` |
| ComplianceManagerBase + ComplianceRule | `D:\ZephyrAlpha\src\zephyr\l10_compliance\compliance_manager.py` |
| AISGSandbox | `D:\ZephyrAlpha\src\zephyr\l10_compliance\aisg_sandbox.py` |
| ArtifactScanner + ArtifactFinding + ScanReport | `D:\ZephyrAlpha\src\zephyr\l10_compliance\artifact_scanner.py` |
| DefaultSecurityGateway（导出兼容层） | `D:\ZephyrAlpha\src\zephyr\l10_compliance\default_security_gateway.py` |
| DefaultSecurityGateway（实际实现） | `D:\ZephyrAlpha\src\zephyr\l10_compliance\implementations\default_security_gateway.py` |
...
|------|------|
| 对应蓝图契约 | §4.1 |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l10_compliance\implementations\default_security_gateway.py` |
| 验收标准 | L1/L2/L3拦截可运行 |
| 验证命令 | `python -m pytest tests/l10_compliance/test_default_security_gateway.py -v` |
...
|------|------|
| 对应蓝图契约 | §4.1 |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l10_compliance\compliance_manager.py` |
| 验收标准 | CTR-P1-012可被L04/L06消费 |
| 验证命令 | `python -m pytest tests/l10_compliance/test_compliance_manager.py -v` |
...
| 步骤 | 如果出问题 | 回滚操作 |
|------|----------|---------|
| 1 | DefaultSecurityGateway三层防御实现失败 | `git checkout -- src/zephyr/l10_compliance/implementations/` |
| 2 | YAML注册导致格式错误 | `git checkout -- architecture-model/layers/l10_compliance.yaml` |
| 3 | ComplianceManagerBase修改导致接口断裂 | `git checkout -- src/zephyr/l10_compliance/compliance_manager.py` |

### 16.5 施工完成标准

| # | 产出物 | 存放完整绝对路径 | 是否存在 | 内容非空 | §0对齐 |
|---|--------|---------------|:---:|:---:|:---:|
| 1 | DefaultSecurityGateway | `D:\ZephyrAlpha\src\zephyr\l10_compliance\implementations\default_security_gateway.py` | ☐ | ☐ | ☐ |
| 2 | l10_compliance.yaml | `D:\ZephyrAlpha\architecture-model\layers\l10_compliance.yaml` | ☐ | ☐ | ☐ |
| 3 | ComplianceManagerBase | `D:\ZephyrAlpha\src\zephyr\l10_compliance\compliance_manager.py` | ☐ | ☐ | ☐ |
...
| # | 规格名称 | 类型 | 规格内容 | 对应代码 |
|---|---------|------|---------|---------|
| 1 | DefaultSecurityGateway三层防御 | 算法 | L1: _L1_PATTERNS正则匹配→InputSanitizer净化→L2: _L2_PATTERNS正则匹配+AISGSandbox.scan_content→L3: 汇总findings+LSG扫描→生成AuditDecision | `src/zephyr/l10_compliance/implementations/default_security_gateway.py` |
| 2 | AuditDecision生成逻辑 | 算法 | errors存在或lsg_blocked→BLOCK; warnings存在或l1_clean=False→FLAG; 否则→ALLOW | `src/zephyr/l10_compliance/implementations/default_security_gateway.py` |

### 16.8 施工参考卡

| # | 类型 | 名称 | 用途/说明 | 参数/字段 | 输出/约束 |
|---|:----:|------|----------|----------|----------|
| 1 | 命令 | `python -m pytest tests/l10_compliance/` | 运行L10全部测试 | `-v`: 详细输出; `-k`: 过滤 | exit 0=通过 |
...
|--------|---------|---------|----------|:---:|
| AsyncInterceptQueue | GAP-L10-001 | `D:\ZephyrAlpha\src\zephyr\l10_compliance\async_intercept_queue.py` | Phase 2 | 待施工 |
| ParallelScanner | GAP-L10-002 | `D:\ZephyrAlpha\src\zephyr\l10_compliance\parallel_scanner.py` | Phase 2 | 待施工 |
...
|---|---------------|------------|---------|---------|------------|
| 1 | default_security_gateway.py（根目录导出兼容层） | `D:\ZephyrAlpha\src\zephyr\l10_compliance\default_security_gateway.py` | 迁移型 | `D:\ZephyrAlpha\src\zephyr\l10_compliance\implementations\default_security_gateway.py` | 确认所有import路径指向implementations/后→标记deprecated→Phase4物理删除 |
...
|---|---------|------------|------|---------|
| 1 | security_gateway_base.py | `D:\ZephyrAlpha\src\zephyr\l10_compliance\security_gateway_base.py` | 修改 | 完善SecurityGateway抽象 |
| 2 | compliance_manager.py | `D:\ZephyrAlpha\src\zephyr\l10_compliance\compliance_manager.py` | 修改 | 完善ComplianceManagerBase |
| 3 | aisg_sandbox.py | `D:\ZephyrAlpha\src\zephyr\l10_compliance\aisg_sandbox.py` | 读取 | AISG模式匹配 |
| 4 | artifact_scanner.py | `D:\ZephyrAlpha\src\zephyr\l10_compliance\artifact_scanner.py` | 修改 | 注册到YAML SSoT |
| 5 | default_security_gateway.py（根目录） | `D:\ZephyrAlpha\src\zephyr\l10_compliance\default_security_gateway.py` | 废弃 | 导出兼容层→迁移 |
| 6 | implementations/default_security_gateway.py | `D:\ZephyrAlpha\src\zephyr\l10_compliance\implementations\default_security_gateway.py` | 修改 | 完善三层防御实现 |
