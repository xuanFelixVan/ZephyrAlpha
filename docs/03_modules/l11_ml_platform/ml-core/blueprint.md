...
actual_disk_path: "src/zephyr/l11_ml_platform/"
...
> module_id: MOD-L11-001 | version: 2.1.0 | status: active | layer: l11_ml_platform
> actual_disk_path: src/zephyr/l11_ml_platform/ | generation: 2 | construction_progress: partially_implemented
...
| 验证项 | 验证方法 | 结果 |
|--------|---------|:---:|
| construction_progress = partially_implemented → 已实现章节的代码存在 | `ls D:\ZephyrAlpha\src\zephyr\l11_ml_platform\` | ☐ |
...
> ⚠️ ModelMetadata 使用 `@dataclass(frozen=True)` 而非 Pydantic V2 BaseModel（ADR-0040），属于技术债务，C轨激活时 MUST 迁移为 Pydantic V2 BaseModel。
...
| # | 类型 | 禁止项 | 替代/允许项 | 原因 |
|---|:----:|--------|-----------|------|
| 1 | 编码模式 | `@dataclass` 用于新数据模型 | `Pydantic V2 BaseModel` | ADR-0040；ModelMetadata为技术债务，待迁移 |
...
| L02 Alpha Factor | 可选 | 特征输入(CTR-001) | — | `D:\ZephyrAlpha\docs\03_modules\l02_alpha_factor\alpha-factor-core\blueprint.md` |
| MOD-INF-015 System Telemetry | 可选 | 模型监控(CTR-P1-013) | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\system-telemetry\blueprint.md` |
| L06 Trade Execution | 可选 | 持仓数据(CTR-006) | — | `D:\ZephyrAlpha\docs\03_modules\l06_trade_execution\execution-core\blueprint.md` |
...
| 产出物类型 | 存放完整绝对路径 | 说明 |
|----------|---------------|------|
| 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l11_ml_platform\ml-core\blueprint.md` | 本文件 |
| 业务代码 | `D:\ZephyrAlpha\src\zephyr\l11_ml_platform\` | Python 源码 |
| 测试代码 | `D:\ZephyrAlpha\tests\unit\l11_ml_platform\` | 测试用例 |
...
| 2 | 模块ID注册表 | `D:\ZephyrAlpha\docs\02_enterprise_architecture\target-architecture\architecture-model\module-id-registry.yaml` | 版本信息 | 蓝图升级 |
...
|------|------|
| 对应蓝图契约 | §4.1 |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l11_ml_platform\implementations\default_inference_engine.py` |
| 验收标准 | import成功 + predict()返回有效ModelServingResponse |
| 验证命令 | `python -c "from zephyr.l11_ml_platform.implementations.default_inference_engine import DefaultInferenceEngine; print('OK')"` |
...
|------|------|
| 对应蓝图契约 | §4.1 |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l11_ml_platform\inference_base.py` |
| 验收标准 | 版本注册/查找/废弃可用 |
| 验证命令 | `python -c "from zephyr.l11_ml_platform.inference_base import ModelRegistry; print('OK')"` |
...
| 步骤 | 如果出问题 | 回滚操作 |
|------|----------|---------|
| 1 | DefaultInferenceEngine修改破坏已有功能 | `git checkout -- src/zephyr/l11_ml_platform/implementations/` |
| 2 | ModelRegistry扩展破坏注册逻辑 | `git checkout -- src/zephyr/l11_ml_platform/inference_base.py` |
...
|---|:----:|------|----------|----------|----------|
| 1 | 命令 | `python -m pytest tests/unit/l11_ml_platform/` | 单元测试 | — | 9 passed |
...
|---|---------|------------|------|---------|
| 1 | ML平台核心代码 | `D:\ZephyrAlpha\src\zephyr\l11_ml_platform\` | 修改 | 蓝图描述的核心代码 |
| 2 | 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l11_ml_platform\ml-core\blueprint.md` | 修改 | 本文件 |
