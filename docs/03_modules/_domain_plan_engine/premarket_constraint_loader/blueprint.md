...
> module_id: MOD-PLAN-002 | version: 0.1.0 | status: Active | layer: L2_domain (plan_engine)
...
> 自动生成（五图对齐，ARCH-056），禁止手写。生成命令：`python scripts/governance/d5_architecture/generators/align_panoramas.py --module MOD-PLAN-002`

| 图 | 位置 | 状态 | 链接 |
|----|------|------|------|
| 依赖图 (depgraph) | `blueprint_id=MOD-PLAN-002` 的 file 节点 | production | `extract_depgraph.py --modules MOD-PLAN-002` |
| 数据流图 (dataflow) | 消费 TomorrowBoundary→产出 ConstraintState | active | `apply_dataflowgraph.py --list-datasets` |
| 决策架构图 (decision) | BM-PLAN-02 盘前预案加载环节 | production | `generate_decision_diagram.py` |
...
| 操作 | 触发方式 | 调度策略 | 当前状态 |
|------|---------|---------|---------|
| load_constraint（盘前加载） | auto_scheduled | 次日 9:00 加载边界 + 9:25 竞价数据到达后匹配 | ✅已实现 |
### §5.7 禁止模式与导入约束
| # | 类型 | 禁止项 | 替代/允许项 | 原因 |
|---|:----:|--------|-----------|------|
