...
actual_disk_path: "src/zephyr/l04_risk_management/"
...
depends_on:
  - target: "architecture-model/layers/l04_risk_management.yaml"
    at: "全篇"
    why: "YAML SSoT——本蓝图真源"
  - target: "MOD-L02-001"
    at: "CTR-002"
...
references:
  - path: "D:\\ZephyrAlpha\\architecture-model\\layers\\l04_risk_management.yaml"
    section: "全篇"
    why: "YAML SSoT"
...
priority: P0
ssot_yaml: "architecture-model/layers/l04_risk_management.yaml"
---
...
> actual_disk_path: src/zephyr/l04_risk_management/ (10 .py files)
...
| §0.1 文件清单 10 个文件全部存在于磁盘 | `ls D:\ZephyrAlpha\src\zephyr\l04_risk_management\` | ☐ |
| implementations/ 子目录包含 5 个 default_* 文件 | `ls D:\ZephyrAlpha\src\zephyr\l04_risk_management\implementations\` | ☐ |
...
| 角色 | 关注点 | 参与阶段 | 约束 |
|------|--------|---------|------|
| Owner | 架构决策+风控参数审批 | 设计+施工 | L04 为 Immutable Core（GOV-AI-001），变更需 Owner+ADR |
...
> ⚠️ 代码中 risk_manager_base.py 使用了 `@dataclass(frozen=True)` 定义 RiskCheckResult/RiskReport，与 ADR-0040 不一致——Phase 2 需迁移为 Pydantic BaseModel。
...
|---|-------------|---------|---------|---------|------------|
| 1 | YAML SSoT files 列表 | `D:\ZephyrAlpha\architecture-model\layers\l04_risk_management.yaml` | 同位置 | 更新 files 列表反映 implementations/ 子目录结构 | 搜索全项目引用并更新 |
| 2 | RiskCheckResult/RiskReport @dataclass | `src/zephyr/l04_risk_management/risk_manager_base.py` | 同文件 | 迁移为 Pydantic BaseModel（ADR-0040） | Phase 2 执行 |
...
| # | 类型 | 禁止项 | 替代/允许项 | 原因 |
|---|:----:|--------|-----------|------|
| 1 | 编码模式 | `@dataclass` 用于数据模型 | Pydantic BaseModel | ADR-0040 强制 Pydantic V2 |
| 2 | 编码模式 | Kill Switch 中包含 I/O 操作 | 纯内存操作 | INV-001 延迟 < 1ms |
| 3 | 编码模式 | HALT 降级为 WARNING | 硬编码禁止降级 | 资金安全 |
| 4 | 导入源 | `from zephyr.l05_* import *` | 通过 CTR-003/CTR-004 契约消费 | 分层约束——L04 不直接导入 L05 |
| 5 | 导入源 | `from zephyr.l06_* import *` | 通过 CTR-006 契约消费 | 分层约束——L04 不直接导入 L06 |
...
> ⚠️ 当前测试目录 `tests/l04_risk_management/` 不存在——违反防幻觉铁律 #14（新代码必测）。
...
|---------|---------|---------|---------|---------|
| MOD-L00-001 | 可选 | CTR-001 NormalizedMarketData | v1.0.0 | `D:\ZephyrAlpha\docs\03_modules\l00_data_source\data-source-core\blueprint.md` |
| MOD-L02-001 | 必须 | CTR-002 FactorSignal | v1.0.0 | `D:\ZephyrAlpha\docs\03_modules\l02_alpha_factor\alpha-factor-core\blueprint.md` |
| MOD-L03-001 | 必须 | CTR-P1-015 SynthesizedSignal | v1.0.0 | `D:\ZephyrAlpha\docs\03_modules\l03_signal_generation\signal-generation-core\blueprint.md` |
| MOD-L05-001 | 必须 | CTR-004 Order（消费） | v1.0.0 | `D:\ZephyrAlpha\docs\03_modules\l05_portfolio\portfolio-construction\blueprint.md` |
| MOD-L06-001 | 必须 | CTR-006 PositionSnapshot（消费） | v1.0.0 | `D:\ZephyrAlpha\docs\03_modules\l06_execution\execution-engine\blueprint.md` |
| MOD-L08-001 | 可选 | CTR-P1-008 RiskDashboardSnapshot（产出） | v1.0.0 | `D:\ZephyrAlpha\docs\03_modules\l08_interface\dashboard\blueprint.md` |
...
| 产出物类型 | 存放完整绝对路径 | 说明 |
|----------|---------------|------|
| 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l04_risk_management\risk-management-core\blueprint.md` | 本文件（含设计和施工指引） |
| 接口定义 | `D:\ZephyrAlpha\src\zephyr\l04_risk_management\*.py` | ABC 基类 + 数据模型 |
| 默认实现 | `D:\ZephyrAlpha\src\zephyr\l04_risk_management\implementations\` | 5 个 default_* 实现类 |
...
| 测试代码 | `D:\ZephyrAlpha\tests\l04_risk_management\` | 单元测试 + 集成测试（待创建） |
| 交易门禁（规则真源） | `D:\ZephyrAlpha\src\zephyr\gates\g7_position_limits.yaml` + `g8_leverage.yaml` | G10持仓限制 + G11杠杆限制——消费 CTR-003 RiskLimits 契约（实现归属 MOD-INF-007） |
...
|---|------------|------------|---------|---------|
| 1 | YAML SSoT | `D:\ZephyrAlpha\architecture-model\layers\l04_risk_management.yaml` | files 列表应反映 implementations/ 子目录结构 | 磁盘路径与 YAML 不一致 |
| 2 | 蓝图注册表 | `D:\ZephyrAlpha\docs\03_modules\blueprint-registry.yaml` | construction_progress 更新为 phase_1_partial | 进度同步 |
| 3 | 跨层契约 | `D:\ZephyrAlpha\architecture-model\cross-layer-contracts.yaml` | 确认 CTR-003/CTR-ERR-004/CTR-P1-008 状态 | 契约状态确认 |
...
| 7 | 代码 [AI_AUTONOMY]=ai_modifiable 与 GOV-AI-001 声明 L04=Immutable Core 不一致 | 高 | 高 | 代码头部需修正为 human_gated，变更需 Owner+ADR | 风险 |
...
| 5 | L04 为 Immutable Core（GOV-AI-001），变更需 Owner+ADR | 确认审批链 | ☐ |
...
|------|------|
| 对应蓝图契约 | §4 |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l04_risk_management\` |
| 验收标准 | 5 个 ABC 基类 + 5 个 default_* 实现 + 4 种止损模式 + Kill Switch |
| 验证命令 | `python -m pytest tests/l04_risk_management/ -k "test_risk" -v` |
...
| 验证命令 | `python -m pytest tests/l04_risk_management/ -k "test_integration or test_performance" -v` |
| G7 检查项 | 上游文件全部列出？下游产出物路径精确？回滚方案可执行？ |
| AI 自治范围 | human_gated（Immutable Core）——需 Owner 审批 |
...
| # | 检查项 | 通过标准 | 类型 | 状态 |
|---|--------|---------|:----:|:----:|
| 1 | OCP 扩展点骨架存在 | `ls D:\ZephyrAlpha\src\zephyr\l04_risk_management\*.py` exit 0 | 完成 | ☐ |
| 2 | 默认实现存在 | `ls D:\ZephyrAlpha\src\zephyr\l04_risk_management\implementations\` exit 0 | 完成 | ☐ |
...
|---|:----:|------|----------|----------|----------|
| 1 | 命令 | `python -m pytest tests/l04_risk_management/ -v` | 运行全部测试 | — | exit 0 = 通过 |
| 2 | 命令 | `python -m pytest tests/l04_risk_management/ -k "test_integration" -v` | 集成测试 | — | exit 0 = 通过 |
| 3 | 命令 | `python -m pytest tests/l04_risk_management/ -k "test_performance" -v` | 性能测试（INV-001） | — | 延迟 < 1ms |
...
|---|:----:|------|---------|----------|----------|----------|
| 1 | 施工 | ABC 基类编译失败 | 类型注解错误 | `python -c "from zephyr.l04_risk_management import *"` | 修复类型注解 | import 成功 |
...
| GAP-L04-004 | 测试目录不存在 | 创建 tests/l04_risk_management/ | P0 | 立即 | v2.2.0 | 待施工 |
...
| L05/L06 集成测试 | GAP-L04-002 | `tests/l04_risk_management/test_integration.py` | Phase 2 | 待施工 |
| INV-001 延迟基准测试 | GAP-L04-003 | `tests/l04_risk_management/test_performance.py` | Phase 2 | 待施工 |
| 单元测试 | GAP-L04-004 | `tests/l04_risk_management/test_*.py` | Phase 2 | 待施工 |
...
|---|------|:------:|------|---------|---------|:----:|
| 1 | 测试目录不存在 | 高 | Phase 1 未创建测试 | 创建 tests/l04_risk_management/ + 单元测试 | 铁律#14 | 待解决 |
| 2 | 代码 AI_AUTONOMY=ai_modifiable 与 GOV-AI-001=Immutable Core 不一致 | 高 | 代码头部未同步注册表声明 | 修正代码头部为 human_gated | GOV-AI-001 | 待解决 |
| 3 | RiskCheckResult/RiskReport 使用 @dataclass 而非 Pydantic BaseModel | 中 | Phase 1 快速实现 | 迁移为 Pydantic BaseModel | ADR-0040 | 待解决 |
...
| 9 | YAML SSoT | — | — | `D:\ZephyrAlpha\architecture-model\layers\l04_risk_management.yaml` | 本蓝图真源 |
...
|---|---------|------------|------|---------|
| 1 | 风险管理模块 | `D:\ZephyrAlpha\src\zephyr\l04_risk_management\` | 读取 | 确认文件存在 |
| 2 | 默认实现目录 | `D:\ZephyrAlpha\src\zephyr\l04_risk_management\implementations\` | 读取 | 确认文件存在 |
| 3 | 契约 SSoT | `D:\ZephyrAlpha\src\zephyr\shared\contracts\risk\` | 读取 | 确认契约状态 |
| 4 | YAML SSoT | `D:\ZephyrAlpha\architecture-model\layers\l04_risk_management.yaml` | 读取 | 蓝图真源 |
| 5 | 蓝图注册表 | `D:\ZephyrAlpha\docs\03_modules\blueprint-registry.yaml` | 修改 | construction_progress 同步 |
| 6 | 跨层契约 | `D:\ZephyrAlpha\architecture-model\cross-layer-contracts.yaml` | 修改 | 确认契约状态 |
...
| Tier | 消费者 | 依赖内容 |
|:----:|--------|---------|
| Tier 1 | `D:\ZephyrAlpha\docs\03_modules\l05_portfolio\portfolio-construction\blueprint.md` | §4 接口契约、§10 依赖关系 |
| Tier 1 | `D:\ZephyrAlpha\docs\03_modules\l06_execution\execution-engine\blueprint.md` | §4 接口契约、§10 依赖关系 |
| Tier 2 | `D:\ZephyrAlpha\docs\03_modules\l08_interface\dashboard\blueprint.md` | §12 集成点 |
| Tier 3 | `D:\ZephyrAlpha\src\zephyr\l04_risk_management\` | §4 数据模型、§11 产出物路径 |
