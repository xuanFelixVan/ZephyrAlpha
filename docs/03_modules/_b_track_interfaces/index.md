...
| Yes（本目录管） | No（不管 → 正确位置） |
|:--|:--|
| 6 大 B 轨核心服务的接口合同（Protocol + Pydantic Schemas + API 签名） | 模块蓝图 → `docs/03_modules/_domain_infra_ops/{module}/blueprint.md` **或** `docs/03_modules/_cross_layer/{module}/blueprint.md`（见 `module_registry.yaml` 的 path） |
