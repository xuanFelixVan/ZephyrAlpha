...
ttl: permanent
actual_disk_path: "src/zephyr/kb/"
...
- 新增 Module ID 映射 → 对齐 `module-id-registry.yaml` 的 MOD-XXX-NNN 格式
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|:---:|------|
| `module_id` | str | ✅ | 对齐 `module-id-registry.yaml` |
...
| # | 新增文件 | 用途 | 优先级 |
|:--:|---------|------|:---:|
| 1 | `src/zephyr/kb/script_registry.py` | 脚本元数据注册 + Script CRUD | P0 |
| 2 | `src/zephyr/kb/module_registry.py` | 模块注册 + Module-Script-KE 映射 | P0 |
| 3 | `src/zephyr/kb/script_scheduler.py` | 脚本优先级调度 + Worker Pool | P0 |
| 4 | `src/zephyr/kb/script_executor.py` | 脚本子进程管理 + 沙箱隔离 | P0 |
| 5 | `src/zephyr/kb/incremental_scanner.py` | 增量扫描：文件变更 → 受影响脚本 | P0 |
| 6 | `src/zephyr/kb/dependency_graph.py` | 脚本依赖图 + 模块依赖图（NetworkX） | P0 |
| 7 | `src/zephyr/kb/event_bus.py` | 事件总线（asyncio.Queue + SQLite持久化） | P1 |
| 8 | `src/zephyr/kb/connection_pool.py` | SQLite + ChromaDB 连接池管理 | P0 |
| 9 | `src/zephyr/kb/rate_limiter.py` | API 速率限制（Token Bucket 算法） | P1 |
| 10 | `src/zephyr/kb/dead_letter_queue.py` | 脚本执行失败重试 + 死信队列 | P1 |
| 11 | `src/zephyr/kb/result_aggregator.py` | 多脚本执行结果聚合 + 去重 | P1 |
| 12 | `src/zephyr/kb/full_scan_orchestrator.py` | 全量扫描分片编排 + 周检报告生成 | P1 |
| 13 | `src/zephyr/kb/ke_module_mapping.py` | KE↔Module 双向映射 + 跨模块隔离检索 | P2 |
| 14 | `src/zephyr/kb/shard_manager.py` | ChromaDB Collection 分片管理（≥5万向量时） | P2 |
...
| 7 | ❌排除 | 上下文引擎的 Token 预算追踪 | context_engine/ 模块（ADR-0015） |
...
| 8 | ❌排除 | 上下文引擎的 Token 预算追踪和注入策略 | context_engine/ 模块（ADR-0015） |
...
| 3 | `src/zephyr/kb/unified_memory_api.py` | RI-02 统一内存 API——remember/learn/forget/recall (deprecated → VMS) |
| 4 | `src/zephyr/kb/chromadb_init.py` | ChromaDB 4 Collection 初始化 (deprecated → VMS) |
...
> 来源：Session Log / AGENTS.md / ADR / 门禁阻断 / pre-commit hooks。提取优先级：自动。
...
**ADR 已于 2026-04-27 裁定废弃（R72）**。氛围编程下决策在聊天中发生，不需要传统 ADR 模板。
...
**旧 ADR 迁移**：36 份旧 ADR → `adr_migrate.py`（beta 单次执行）→ ≤200字结论→AGENTS.md §10; 含对比表→KE（A2）G1-G5。原文件归档 `docs/_archive/old_adr/`。
...
| 步骤 | 操作 | 产出 |
|:---:|------|------|
| S1 | 存量文档全量扫描：AGENTS.md + ADR/ + blueprints/ + session-logs/ | ~200-500 文档片段 |
...
| 3: 决策记录+蓝图变更 | 聊天决策信号 / bp version bump | per-source-type templates | G1→G2(ADR→A2, 蓝图→A3, 工具→A5)→G3 | HIGH→直接KE |
...
| ADR.status→ACCEPTED | ADR status watcher (git hook + YAML parse) | G1-G5 → KE(A2) | 零 |
...
| 知识文章注册表 | `D:\ZephyrAlpha\docs\01_policies_and_standards\_registry\catalogs\knowledge-article-registry.yaml` | 知识文章索引（REG-KB-001，待填充） |
...
| 2 | 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\knowledge-base\blueprint.md` | 修改 | 本文件 |
...
| 3 | `zephyr.l01_infrastructure.a2a_protocol.*` | 直接 `import chromadb` | 通过 `kb_repo.py` 封装访问 |
...
| MOD-INF-006 Task System | 必须 | context_assembler 知识注入接口 + 任务状态机 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\runtime-integration\blueprint.md` |
| MOD-INF-005 Script System | 必须 | 审计数据来源 + 标签分类体系 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\script-system\blueprint.md` |
| MOD-INF-026 Asset Inventory | 可选 | 资产盘点 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\asset-inventory\blueprint.md` |
| MOD-INF-008 Context Engine | 可选 | CE build 阶段从 KB 检索 KE | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\context-engine\blueprint.md` |
| MOD-INF-011 Vector Memory | 可选 | KE 写入时同步向量化 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\vector-memory\blueprint.md` |
| MOD-INF-007 Gate Engine | 可选 | G1-G5 KMS 决策门 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\gate-engine\blueprint.md` |
| MOD-INF-010 Feedback Loop | 可选 | 知识演化回路 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\feedback-loop\blueprint.md` |
...
| `src/zephyr/kb/bootstrap.py` | ✅ 已实现 | |
| `src/zephyr/kb/chromadb_init.py` | ✅ 已实现 | |
| `src/zephyr/kb/embedding_migrate.py` | ✅ 已实现 | |
| `src/zephyr/kb/extract.py` | ✅ 已实现 | |
| `src/zephyr/kb/freeze.py` | ✅ 已实现 | |
| `src/zephyr/kb/graph_validator.py` | ✅ 已实现 | |
| `src/zephyr/kb/ingest.py` | ✅ 已实现 | |
| `src/zephyr/kb/integrity.py` | ✅ 已实现 | |
| `src/zephyr/kb/kb_gate_task.py` | ✅ 已实现 | |
| `src/zephyr/kb/kb_repo.py` | ✅ 已实现 | |
| `src/zephyr/kb/ke_tombstone.py` | ✅ 已实现 | |
| `src/zephyr/kb/load_bearing.py` | ✅ 已实现 | |
| `src/zephyr/kb/migration/embedding_migrate.py` | ✅ 已实现 | |
| `src/zephyr/kb/migration/kb_gate_task.py` | ✅ 已实现 | |
| `src/zephyr/kb/pipeline/activate.py` | ✅ 已实现 | |
| `src/zephyr/kb/pipeline/analyze.py` | ✅ 已实现 | |
| `src/zephyr/kb/pipeline/batch_ingest.py` | ✅ 已实现 | |
| `src/zephyr/kb/pipeline/extract.py` | ✅ 已实现 | |
| `src/zephyr/kb/pipeline/ingest.py` | ✅ 已实现 | |
| `src/zephyr/kb/pipeline/triage.py` | ✅ 已实现 | |
| `src/zephyr/kb/quiet_period_monitor.py` | ✅ 已实现 | |
| `src/zephyr/kb/reranker.py` | ✅ 已实现 | |
| `src/zephyr/kb/safety_brake.py` | ✅ 已实现 | |
| `src/zephyr/kb/self_test.py` | ✅ 已实现 | |
| `src/zephyr/kb/storage/chromadb_init.py` | ✅ 已实现 | |
| `src/zephyr/kb/storage/graph_validator.py` | ✅ 已实现 | |
| `src/zephyr/kb/storage/kb_repo.py` | ✅ 已实现 | |
| `src/zephyr/kb/storage/unified_memory_api.py` | ✅ 已实现 | |
| `src/zephyr/kb/triage.py` | ✅ 已实现 | |
| `src/zephyr/kb/unified_memory_api.py` | ✅ 已实现 | |
| `src/zephyr/kb/verify.py` | ✅ 已实现 | |
| `src/zephyr/kb/vms_memory_backend.py` | ✅ 已实现 | |
