...
actual_disk_path: "src/zephyr/system_telemetry/"
...
> module_id: MOD-INF-015 | version: 2.0.1 | status: Active | layer: L01
...
| 4 | 已删除副本 | src/zephyr/l12_system_telemetry/（2026-05-16 RULE-THREE审判删除） |
...
> 强制 **Pydantic V2 BaseModel**（ADR-0040），禁止 `@dataclass`。已实现代码仅保留接口签名。
...
|---------|---------|---------|---------|---------|
| MOD-INF-016 | 必须 | shared/logging / lifecycle / flags / observer / contracts | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\shared-core\blueprint.md` |
| MOD-INF-012 | 必须 | olap_engine 持久化 FLE 时序分析结果 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\database\blueprint.md` |
| MOD-INF-024 | 必须 | Budget Enforcer 成本 metrics 聚合 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\budget-enforcer\blueprint.md` |
| MOD-INF-022 | 必须 | Escalation Protocol 告警升级 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\escalation-protocol\blueprint.md` |
| MOD-INF-018 | 必须 | Agent RBAC L6 Observability 消费 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\agent-rbac\blueprint.md` |
| MOD-INF-014 | 必须 | LLM Security AI 行为安全事件联动 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\llm-security-gateway\blueprint.md` |
| MOD-INF-010 | 可选 | FLE 消费 metrics/logs | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\feedback-loop\blueprint.md` |
| MOD-INF-020 | 可选 | 审计写入遥测-derived 事件 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\audit-trail\blueprint.md` |
...
|----------|---------------|------|:-----------:|---------|
| 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\system-telemetry\blueprint.md` | 本文件（含设计和施工指引） | ≥0 | blueprint-registry.yaml |
...
| 3 | 跨层契约 | `D:\ZephyrAlpha\docs\02_enterprise_architecture\target-architecture\architecture-model\contracts\cross-layer-contracts.yaml` | MCP 接口契约 | AI 可消费性设计落地 |
...
|---|---------|---------|---------|------------|
| 1 | `src/zephyr/system_telemetry/facade.py` | scaffold.py module | `__init__.py` __all__ | Telemetry 门面类，聚合9子系统，实现 auto_boot/auto_event/auto_schedule |
| 2 | `src/zephyr/system_telemetry/metrics/__init__.py` | scaffold.py module | `__init__.py` __all__ | MetricsSubsystem: counter/gauge/histogram/timer，FQMN格式 `{module}.{subsystem}.{metric}` |
| 3 | `src/zephyr/system_telemetry/logs/__init__.py` | scaffold.py module | `__init__.py` __all__ | LogsSubsystem: info/warning/error/critical，结构化JSONL输出 |
| 4 | `src/zephyr/system_telemetry/traces/__init__.py` | scaffold.py module | `__init__.py` __all__ | TracesSubsystem: span/start_span/finish_span，OTel兼容 |
| 5 | `src/zephyr/system_telemetry/ai_behavior/__init__.py` | scaffold.py module | `__init__.py` __all__ | AIBehaviorSubsystem: record/self_correct，AIBehaviorEvent数据类 |
| 6 | `src/zephyr/system_telemetry/archive/__init__.py` | scaffold.py module | `__init__.py` __all__ | ArchiveSubsystem: archive/query，冷热分层存储 |
| 7 | `src/zephyr/system_telemetry/profiles/__init__.py` | scaffold.py module | `__init__.py` __all__ | ProfilesSubsystem: record_profile/get_profile，模块遥测画像 |
| 8 | `src/zephyr/system_telemetry/health/__init__.py` | scaffold.py module | `__init__.py` __all__ | HealthSubsystem: register/set_unhealthy/set_healthy/status，健康探针聚合 |
| 9 | `src/zephyr/system_telemetry/alerts/__init__.py` | scaffold.py module | `__init__.py` __all__ | AlertsSubsystem: add_rule/check_alerts/get_active，规则引擎+阈值告警 |
| 10 | `src/zephyr/system_telemetry/schema/__init__.py` | scaffold.py module | `__init__.py` __all__ | SchemaSubsystem: register_schema/check_compatibility/get_schema，版本兼容检查 |
| 11 | `src/zephyr/system_telemetry/auto_bootstrap.py` | scaffold.py module | `__init__.py` __all__ | 自动初始化：import时触发register_module+monkey-patch |
| 12 | `src/zephyr/system_telemetry/watchdog.py` | scaffold.py script | script_manifest.yaml | 独立watchdog进程：健康巡检+告警触发+自愈 |
...
| 验证命令 | `python -c "import yaml; d=yaml.safe_load(open('.../cross-layer-contracts.yaml')); assert any(c['id']=='CT-TEL-001' for c in d['contracts'])"` |
...
| 验证命令 | `python src/zephyr/gates/triple_alignment.py --module MOD-INF-015` |
...
| 2 | CT-TEL 注册失败 | 删除 cross-layer-contracts.yaml 中 4 条条目 |
...
| 5 | FQMN 自动注入 | 算法 | Telemetry(module_id)→metrics.counter(name)→内部生成 module_id::name | src/zephyr/system_telemetry/facade.py |
...
| 5 | 压缩工作流标准 | GOV-DOC-011 | `D:\ZephyrAlpha\docs\01_policies_and_standards\governance\document\compression-workflow-standard.md` | 规格化流程 |
...
| 8 | 模块ID注册表 | — | `D:\ZephyrAlpha\docs\02_enterprise_architecture\target-architecture\architecture-model\module-id-registry.yaml` | 编号注册 |
...
L2/L3 独立进程模式（可选）：`python -m zephyr.l01_infrastructure.system_telemetry.watchdog --id wd-1`，用于部署层需要独立进程监控的场景。
