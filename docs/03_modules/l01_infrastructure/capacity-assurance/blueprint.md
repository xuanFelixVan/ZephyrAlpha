...
|---|---------|------------|------|:-----:|-------------------|
| 28 | `src/zephyr/shared/error_budget_tracker.py` | §8 | Error Budget 五级响应追踪（M-21） | 已实现 | |
| 29 | `src/zephyr/shared/sandbox_executor.py` | §10 | 高风险操作沙箱隔离（M-23） | 已实现 | |
| 30 | `src/zephyr/shared/degradation_chain.py` | §11 | 模型降级链（M-24） | 已实现 | |
| 31 | `src/zephyr/shared/reasoning_spans.py` | §12 | OTel GenAI 语义规范对齐（M-25） | 已实现 | |
| 32 | `src/zephyr/shared/cost_estimator.py` | §9 | 执行前成本预估（M-26） | 已实现 | |

**外部 re-export wrapper（shared/）— 实际归属其他蓝图**

| # | 文件路径 | 对应蓝图章节 | 职责 | 实际归属 | 存在性 | 阻塞原因（仅已阻塞） |
|---|---------|------------|------|---------|:-----:|-------------------|
| 33 | `src/zephyr/shared/kill_switch.py` | §10 | re-export wrapper | MOD-INF-021 | 已实现 | |
| 34 | `src/zephyr/shared/semantic_cache.py` | §11 | re-export wrapper | MOD-INF-032 | 已实现 | |

**外部依赖（非本蓝图代码）**

| # | 文件路径 | 对应蓝图章节 | 职责 | 存在性 | 阻塞原因（仅已阻塞） |
|---|---------|------------|------|:-----:|-------------------|
| 35 | `src/zephyr/llm_security/behavior_audit_logger.py` | §8 | AI 修改审计日志（M-17） | 已阻塞 | 依赖MOD-INF-015完整实现 |
...
| 原蓝图路径 | 问题 | 实际路径 |
|-----------|------|---------|
| `scripts/governance/d5_architecture/validate_ssot.py`（M-03） | 路径不存在 | `scripts/governance/d5_architecture/validators/validate_ssot.py`（归属治理蓝图） |
...
| M-22 | kill_switch.py | 全局一键熔断 | `src/zephyr/shared/kill_switch.py` | Human-Gated | ✅ 已实现 |
...
| M-27 | semantic_cache.py | 语义缓存（复用 ChromaDB） | `src/zephyr/shared/semantic_cache.py` | AI-Modifiable | ✅ 已实现 |
...
|---------|---------|---------|---------|---------|
| MOD-INF-016 | 必须 | Shared Core 基础设施依赖 | ≥1.0 | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\shared-core\blueprint.md` |
| MOD-INF-015 | 必须 | 系统遥测消费容量指标 | ≥1.0 | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\system-telemetry\blueprint.md` |
| MOD-INF-032 | 必须 | 预算执行器消费预算策略 | ≥1.0 | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\budget-executor\blueprint.md` |
| MOD-INF-021 | 可选 | Kill Switch re-export wrapper | ≥1.0 | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\kill-switch\blueprint.md` |
...
| infrastructure-registry.yaml | 可选 | 基础设施组件 SLA 声明 | — | `D:\ZephyrAlpha\docs\01_policies_and_standards\_registry\catalogs\infrastructure-registry.yaml` |
| ai-risk-register.yaml | 可选 | AI 操作风险登记 | — | `D:\ZephyrAlpha\docs\01_policies_and_standards\_registry\catalogs\ai-risk-register.yaml` |
...
| 5 | 文档压缩工作流标准 | `D:\ZephyrAlpha\docs\01_policies_and_standards\governance\document\compression-workflow-standard.md` |
| 6 | AI 压缩工作流标准 | `D:\ZephyrAlpha\docs\01_policies_and_standards\governance\document\compression-workflow-standard.md` |
| 7 | 依赖图 | `D:\ZephyrAlpha\docs\02_enterprise_architecture\system-dependency-map.md` |
...
```
新 AI → project_rules.md → registry-of-registries.yaml → MOD-INF-001 → 本蓝图
...
| L12 Metrics 骨架 | `src/zephyr/system_telemetry/metrics/__init__.py` | — | — | system-telemetry |
