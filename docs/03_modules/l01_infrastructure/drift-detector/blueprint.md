...
actual_disk_path: "src/zephyr/behavioral_auditor/"
...
> actual_disk_path: src/zephyr/behavioral_auditor/ (54 .py files) | generation: 4 | construction_progress: completed
...
| 验证项 | 验证方法 | 结果 |
|--------|---------|:---:|
| construction_progress = completed → 代码文件清单 100% 存在 | `ls D:\ZephyrAlpha\src\zephyr\behavioral_auditor\` 逐文件核对 | ✅ |
| 蓝图描述的类/函数名 = 代码中的类/函数名 | `grep "class\|def" D:\ZephyrAlpha\src\zephyr\behavioral_auditor\*.py` | ✅ |
| actual_disk_path = §11 业务代码路径 | `D:\ZephyrAlpha\src\zephyr\behavioral_auditor\` 存在 | ✅ |
...
| `src/zephyr/behavioral_auditor/` 代码 | ✅ 是 | — | MOD-INF-033 已改 actual_disk_path 为 behavioral_audit/，冲突已消除 |
| `src/zephyr/drift_detector/` 代码 | ✅ 已清理 | — | 消费者已迁移至 behavioral_auditor |

### §0.5 代码目录唯一性声明

| 声明项 | 状态 | 详情 |
|--------|:----:|------|
| `actual_disk_path` 无冲突 | ✅ 无冲突 | MOD-INF-033 已改为 `behavioral_audit/`，本蓝图独占 `behavioral_auditor/` |
| 代码副本存在 | ✅ 已清理 | 消费者已迁移至 behavioral_auditor |
...
| detector_dispatcher | DetectorDispatcher | _detector_registry.yaml | ✅ |
...
|--------|----------|---------|---------|---------|
| 检测器扩展 | `_detector_registry.yaml` 条目 | 39 内置检测器 | YAML 声明式注册：id/name/dimension/severity/auto_fixable/status/script_path/timeout_s | YAML 文件添加条目 |
...
| 3 | 导入源 | `from zephyr.l00_* import *` | `from zephyr.behavioral_auditor import *` | 分层约束 |
...
|---------|---------|---------|---------|---------|
| MOD-INF-021 Rollback | 必须 | 漂移→回滚桥接(G-CT-005) | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\rollback-system\blueprint.md` |
| MOD-INF-022 Escalation | 必须 | 漂移预算耗尽升级(G-CT-006) | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\escalation-protocol\blueprint.md` |
| MOD-INF-020 Audit Trail | 必须 | 漂移事件审计 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\audit-trail\blueprint.md` |
| MOD-INF-018 Agent RBAC | 必须 | 检测器权限控制 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\agent-rbac\blueprint.md` |
| MOD-INF-007 Gate Engine | 必须 | 门禁持久化 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\gate-engine\blueprint.md` |
| MOD-INF-016 Shared | 必须 | AiAuditLogger + AbstractLock | — | `D:\ZephyrAlpha\docs\03_modules\_cross_layer\shared-core\blueprint.md` |
| MOD-INF-012 DB | 必须 | 基线+漂移结果持久化 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\db\blueprint.md` |
...
> 全局依赖图 SSoT：[system-dependency-map.md](file:///d:/ZephyrAlpha/docs/02_enterprise_architecture/system-dependency-map.md) 线3:治理闭环
...
|----------|---------------|------|-------------|
| 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\drift-detector\blueprint.md` | 本文件（含设计和施工指引） | AI session |
| 业务代码 | `D:\ZephyrAlpha\src\zephyr\behavioral_auditor\` | Python 源码（54 .py 文件） | MOD-INF-021/022/020/018/007/016 |
| 测试代码 | `D:\ZephyrAlpha\tests\behavioral_auditor\` | 单元+集成+红白对抗测试 | CI pipeline |
| 事件定义 | `D:\ZephyrAlpha\src\zephyr\behavioral_auditor\events.py` | 漂移事件+异常类 | MOD-INF-020/021 |
...
|---|------------|------------|---------|---------|
| 1 | 模块 ID 注册表 | `D:\ZephyrAlpha\docs\02_enterprise_architecture\target-architecture\architecture-model\module-id-registry.yaml` | MOD-INF-023 版本更新 | 版本升级同步 |
| 2 | 蓝图注册表 | `D:\ZephyrAlpha\docs\03_modules\blueprint-registry.yaml` | MOD-INF-023 条目更新 | 版本升级同步 |
| 3 | 治理资产清单 | `D:\ZephyrAlpha\docs\01_policies_and_standards\_registry\catalogs\document-metadata-index.yaml` | MOD-INF-023 元数据更新 | 版本升级同步 |
...
| # | 规格名称 | 类型 | 规格内容 | 对应代码 |
|---|---------|------|---------|---------|
| 1 | 漂移状态机存储 | SQL | `CREATE TABLE drift_events (event_id TEXT PK, module_id TEXT, detector_id TEXT, state TEXT, severity TEXT, detected_at TIMESTAMP, updated_at TIMESTAMP, auto_fixable BOOLEAN, hotfix BOOLEAN DEFAULT 0)` + `CREATE TRIGGER prevent_update BEFORE UPDATE ON drift_events BEGIN SELECT RAISE(ABORT, 'append-only'); END` | `D:\ZephyrAlpha\src\zephyr\behavioral_auditor\state_machine.py` |
| 2 | 基线快照存储 | 文件格式 | `data/drift_baselines/{module_id}/baseline_v{N}.json` 含 tree_hash/interface_snapshot/import_graph/config_snapshot + `integrity_manifest.yaml` 含 SHA256 chain | `D:\ZephyrAlpha\src\zephyr\behavioral_auditor\baseline_manager.py` |
| 3 | 检测器调度并行 | 算法 | `ThreadPoolExecutor(max_workers=8)` + per-detector checkpoint + 超时终止 + 结果缓存 | `D:\ZephyrAlpha\src\zephyr\behavioral_auditor\detector_dispatcher.py` |
| 4 | 告警去重与聚合 | 算法 | pattern_hash = SHA256(module_id + dimension + target_path) → 相同 hash 去重 → 同维度聚合 → 同模块聚合 | `D:\ZephyrAlpha\src\zephyr\behavioral_auditor\alert_router.py` |

### 16.8 施工参考卡

| # | 类型 | 名称 | 用途/说明 | 参数/字段 | 输出/约束 |
|---|:----:|------|----------|----------|----------|
| 1 | 命令 | `python -m zephyr.behavioral_auditor` | 漂移检测 CLI | `--scope: 模块ID列表` `--level: LIGHT/STANDARD/DEEP` | ScanResult |
| 2 | 命令 | `python -m zephyr.behavioral_auditor --baseline-update` | 基线更新 | `--module-id: 模块ID` `--force: 跳过投毒检测` | BaselineSnapshot |
| 3 | 配置 | `_detector_registry.yaml` | 检测器注册表 | id/name/dimension/severity/auto_fixable/status/script_path/timeout_s | YAML 格式 |
...
| 资源 | 当前基线 | 测量方式 |
|------|---------|---------|
| 检测器数 | 39 | `_detector_registry.yaml` 条目数 |
...
本蓝图涉及迁移：governance/ 根级 15 孤儿文件 → behavioral_auditor/。迁移方案见 §5.3。
...
| 4 | 模块 ID 注册表 | — | `D:\ZephyrAlpha\docs\02_enterprise_architecture\target-architecture\architecture-model\module-id-registry.yaml` | 编号注册 |
...
| 7 | AI 压缩工作流标准 | GOV-DOC-011 | `D:\ZephyrAlpha\docs\01_policies_and_standards\governance\document\compression-workflow-standard.md` | 压缩规则 |
...
|---|---------|------------|----------|-------------|
| 1 | governance/ 根级 15 孤儿脚本 | `D:\ZephyrAlpha\src\zephyr\governance\` | 部分漂移检测功能 | 已整合到 behavioral_auditor/，governance/ 版本为旧版 |
...
|---|---------|------------|------|---------|
| 1 | 漂移检测代码 | `D:\ZephyrAlpha\src\zephyr\behavioral_auditor\` | 修改 | 蓝图描述的核心代码 |
| 2 | 测试代码 | `D:\ZephyrAlpha\tests\behavioral_auditor\` | 修改 | 测试用例 |
| 3 | 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\drift-detector\blueprint.md` | 修改 | 本文件 |
| 4 | 旧治理脚本 | `D:\ZephyrAlpha\src\zephyr\governance\` | 迁移 | 孤儿文件迁移到 behavioral_auditor/ |
...
| Tier 1 | MOD-INF-007 Gate Engine | gates/drift_detector.py + gate_engine.py + ct_drift_budget.py |
| Tier 1 | MOD-INF-021 Rollback | rollback/drift_fix.py |
| Tier 2 | MOD-INF-020 Audit Trail | audit_trail/drift_bridge.py |
| Tier 2 | MOD-INF-013 MCP Server | mcp/governance_server.py |
| Tier 2 | governance/ shim | drift_detector/__init__.py + phase_check_registry.py |
| Tier 3 | `D:\ZephyrAlpha\src\zephyr\behavioral_auditor\` 代码 | §4 文件归属 |
...
| behavioral_auditor/ 文件迁移 | 读取 §5.3 孤儿文件清单 + §5 约束条件 |
| 治理域子蓝图查询 | 从 DOM-GOV-001 §2 路由到本文件 |

### 导航路径

```
registry-of-registries.yaml → blueprint-registry.yaml → DOM-GOV-001 → §2 → MOD-INF-023
```
