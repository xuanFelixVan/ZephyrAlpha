...
actual_disk_path: "src/zephyr/l01_infrastructure/code_dedup_engine/"
...
| 目录                                                 | 唯一职责                    | 冲突目录 | 判定   |
| -------------------------------------------------- | ----------------------- | ---- | ---- |
| `src/zephyr/l01_infrastructure/code_dedup_engine/` | 代码去重检测+修复+爆炸半径防护 | 无    | ✅ 唯一 |
| `data/cache/function_cache.json`                   | 函数签名指纹缓存                | 无    | ✅ 唯一 |
...
| 4 | @dataclass                        | Pydantic V2 BaseModel                                         | ADR-0040   |
...
| 全量扫描 | on_demand | `python -m zephyr.l01_infrastructure.code_dedup_engine.cli scan --full` | 半自动 |
| 修复执行 | on_demand | `python -m zephyr.l01_infrastructure.code_dedup_engine.cli fix` | 半自动(需--fix) |
...
| ----------- | ---- | -------------------------------- | -------- | ------------------------------------------------------------------------------- |
| MOD-INF-005 | 必须   | 退出码约定 0/1/2/3/4 + Finding Schema | ≥ 0.5.0  | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\script-system\blueprint.md`  |
| MOD-INF-007 | 必须   | GATE-DEDUP 门禁判定逻辑                | ≥ 0.3.0  | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\gate-engine\blueprint.md`    |
| MOD-INF-008 | 必须   | Context Engine 影子清单注入            | ≥ 0.2.0  | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\context-engine\blueprint.md` |
| MOD-INF-010 | 可选   | FLE 进化闭环                         | ≥ 0.2.0  | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\feedback-loop\blueprint.md`  |
| MOD-INF-016 | 必须   | SSoT Guard + shared 目录           | ≥ 0.14.0 | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\shared-core\blueprint.md`    |
| MOD-INF-012 | 可选   | KB 持久化 API                       | ≥ 0.1.0  | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\knowledge-base\blueprint.md` |
...
| ------ | ---------------------------------------------------------------------------------------- | ------------------------------------------------------ | ------------ |
| 蓝图文件   | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\code-dedup-engine\blueprint.md`       | 本文件                                                    | AI session / 治理审计 |
| 业务代码   | `D:\ZephyrAlpha\src\zephyr\l01_infrastructure\code_dedup_engine\`                        | 68 个 .py 文件                                            | CLI / Gate Engine / AutoRuntime |
| CLI 脚本 | `D:\ZephyrAlpha\src\zephyr\l01_infrastructure\code_dedup_engine\cli.py` | CLI 入口：`python -m zephyr.l01_infrastructure.code_dedup_engine.cli` | pre-commit / CI / Owner |
...
| 缓存数据   | `D:\ZephyrAlpha\data\cache\function_cache.json`                                          | 运行时生成（.gitignore + \_integrity 校验）                     | scanner / signature_matcher |
| 策略树配置  | `D:\ZephyrAlpha\src\zephyr\l01_infrastructure\code_dedup_engine\config\policy_tree.yaml` | Wave 3 落地                                              | config.py / GATE-DEDUP |
...
| ---------------------------- | ---------------------------------------------------------------- | ---------------------------------------------------- | ------------------------- |
| Script System (MOD-INF-005)  | 去重脚本注册到 manifest，遵循退出码约定                                         | `cli.py` → `script_manifest.yaml` | G4 入库验收 + exit code 映射    |
...
| Context Engine (MOD-INF-008) | 影子清单注入 system prompt + 渐进式三层记忆 + 消费验证回环                          | `shadow_api_manifest.yaml` → CE build → inject       | CE build log 包含影子清单       |
...
| 2 | script\_manifest.yaml     | `D:\ZephyrAlpha\scripts\governance\script_manifest.yaml`       | 注册 CLI 入口 + exit code 4 + 特殊标志                    | Wave 1 产出 CLI |
| 3 | AGENTS.md §5.1            | `D:\ZephyrAlpha\AGENTS.md`                                     | 影子清单锚点 + 三层记忆注入 + Deprecated 标记                   | 生成时预防落地       |
| 4 | Gate Engine YAML          | `D:\ZephyrAlpha\src\zephyr\gates\g6_blueprint_compliance.yaml` | GATE-DEDUP 门禁规则                                   | Wave 1 门禁落地   |
...
| W3-4 | 策略树 YAML 正式落地               | `config/policy_tree.yaml` 5条规则R001-R005+load_policy_tree() | ✅ 已完成 |
...
| Wave 2 | `atomic_fixer recover_from_crash()` | `python -m zephyr.l01_infrastructure.code_dedup_engine.cli verify` |
| Wave 3 | `git revert <wave3-commit>` | `python -m zephyr.l01_infrastructure.code_dedup_engine.cli scan --full` |

### 16.5 验证流程（每次施工后必执行）

1. `python -m pytest tests/unit/ -q` —— 确认全部测试全绿
2. `python -m zephyr.l01_infrastructure.code_dedup_engine.cli scan --full --warn-only` —— 确认扫描可正常输出含 Health Score
...
| W3-4 | 策略树 YAML 正式落地 | config/policy_tree.yaml 5条规则R001-R005 | ✅ 已完成 | — |
| W3-5 | GATE-DEDUP 正式版 | DeduplicationHandler+_GATE_FILES+skip set移除 | ✅ 已完成 | — |
...
| 策略树 YAML 落地 R001-R005 | config/policy_tree.yaml + config.py | P2 |
...
| 任务 | 堵塞依赖 | 优先级 | 解堵条件 |
|------|---------|:---:|---------|
| MOD-INF-031 代码物理迁移 | auto_fix_engine/ 目录不存在 | P1 | MOD-INF-031 蓝图 Step 1-3 落地 |
...
```
python -m zephyr.l01_infrastructure.code_dedup_engine.cli <subcommand> [OPTIONS]
...
| 影子清单                | shadow\_api\_manifest.yaml——AI生成时注入的共享API列表 | SSoT       | 影子清单=AI预防用；SSoT=运行时注册用                            |
...
| 2 | 压缩工作流标准           | `D:\ZephyrAlpha\docs\01_policies_and_standards\governance\document\compression-workflow-standard.md`  |
...
| 7 | Gate Engine 蓝图    | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\gate-engine\blueprint.md`                          |
| 8 | Context Engine 蓝图 | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\context-engine\blueprint.md`                       |
| 9 | Shared Core 蓝图    | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\shared-core\blueprint.md`                          |
...
| 目录                                                                | 文件数 | 说明     |
| ----------------------------------------------------------------- | :-: | ------ |
| `D:\ZephyrAlpha\src\zephyr\l01_infrastructure\code_dedup_engine\` |  68 | 核心源码   |
