...
|---|---------|------------|------|:-----:|-------------------|
| 1 | `src/zephyr/script_system/__init__.py` | §4 接口契约 | 包初始化 | 已实现 | — |
| 2 | `src/zephyr/script_system/finding.py` | §4 接口契约 | Finding Schema 数据模型 | 已实现 | — |
...
| 蓝图描述的类/函数名 = 代码中的类/函数名 | `grep "class\|def" src/zephyr/script_system/finding.py` | ☐ |
| script_manifest.yaml 与蓝图 §3 维度分类一致 | `python scripts/governance/run_all.py --list` | ☐ |
...
| 3 | 脚本清单 | `D:\ZephyrAlpha\scripts\governance\script_manifest.yaml` | 修改 | 脚本注册表 |
| 4 | 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\governance-automation\blueprint.md` | 修改 | 本文件 |
...
| 5 | 脚本清单 | script_manifest.yaml | `D:\ZephyrAlpha\scripts\governance\script_manifest.yaml` | 脚本的完整注册表（SSoT，以实际生成为准）——REG-SCRIPT-001 主清单 + REG-SCRIPT-002 Governance 子集 |
...
| 9 | 脚本健康注册表 | REG-SCRIPT-HEALTH-001 | `D:\ZephyrAlpha\docs\01_policies_and_standards\_registry\catalogs\script-health-registry.yaml` | 39 个脚本健康状态 |
...
| 模块归属（directory-registry.yaml） | 2 | 构建时 |
...
scripts/governance/
  ...
  manifest/
    d1_structure_manifest.yaml            ← per-dimension manifest
    ...
    meta_manifest.yaml
  script_manifest.yaml                    ← 全量 SSoT（per-dimension manifest 派生）
```

**加载策略**: `--list`→只读 L0 index(<10ms) / `--dimensions d3,d5`→读 index+D3+D5 manifest(<100ms) / `--full`→全量 manifest(~5s，仅周检)。

**script_manifest.yaml 仍是全量 SSoT**——per-dimension manifest 由 `generate_script_manifest.py` 自动派生，不改 AI 的脚本注册流程。
...
| Kill Switch 全局冻结 | run_all.py 启动时读取 kill_switch_state.yaml | 拒绝执行新脚本（exit 2） | 2 |
...
| 治理脚本数 | 366 | script_manifest.yaml total_scripts |
...
|---------|---------|---------|---------|---------|
| MOD-INF-001 (capacity-assurance) | runtime | 容量预算检查 + SLO 监控 + Error Budget + Kill Switch | 2.0.0 | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\capacity-assurance\blueprint.md` |
| MOD-INF-003 (task-card-kms) | runtime | Finding → CRITICAL 自动创建任务卡 | 1.0.0 | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\task-card-kms\blueprint.md` |
| MOD-INF-004 (vibe-coding-pipelines) | contract | 脚本系统是双管线审计侧的脚本基础设施 | 1.0.0 | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\vibe-coding-pipelines\blueprint.md` |
| **MOD-INF-006 (task-system)** | **contract** | **G0-G7门禁体系 + M1-M11管线节点** | **0.3.0** | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\task-system\blueprint.md` |
...
| NC3 | **学习成本**——Owner 需理解 Finding Schema 字段含义 | 中 | 低 | QUICKSTART.md + HUMAN_MEMORY_CARD.md | 负面后果 |
...
| 4 | DD-SCRIPT-004 | Manifest 分层方式 | A:生成物分层 / B:源数据分层 | A | script_manifest.yaml 仍是全量 SSoT；per-dimension manifest 自动派生；不改 AI 注册流程 | 2026-05-03 |
...
| 修改本蓝图 | MUST 同步更新 |
|-----------|-------------|
| §4 接口契约变更 | script_manifest.yaml + 下游消费者 |
| §3 维度分类变更 | script_manifest.yaml + run_all.py |
...
```
registry-of-registries.yaml → blueprint-registry.yaml → MOD-INF-005 → 本蓝图
...
| 产出物类型 | 存放完整绝对路径 | 说明 |
|----------|---------------|------|
| 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\governance-automation\blueprint.md` | 本文件 |
| 业务代码 | `D:\ZephyrAlpha\src\zephyr\script_system\` | 脚本系统核心 |
| 治理脚本 | `D:\ZephyrAlpha\scripts\governance\` | 80+ 治理脚本 |
| 脚本注册表 | `D:\ZephyrAlpha\scripts\governance\script_manifest.yaml` | 脚本登记 SSoT（REG-SCRIPT-001 主清单 + REG-SCRIPT-002 Governance 子集） |
| 脚本健康注册表 | `D:\ZephyrAlpha\docs\01_policies_and_standards\_registry\catalogs\script-health-registry.yaml` | 39 个脚本健康状态（REG-SCRIPT-HEALTH-001） |
...
| **B68** | **人类知识巴士系数** | 1 个人的脑子里装着所有隐式设计决策（"为什么 D5 有 45 个脚本但 D2 只有 2 个"、"为什么不直接用 ruff 替代 D7 全部脚本"）。这些决策理由没有文档化 | 在 `HUMAN_MEMORY_CARD.md` 中补充「设计决策日志」——每个反直觉决策一句话解释。在蓝图 §3.1 的各维度描述中补充「为什么这个维度脚本多/少」的脚注.
...
| # | 盲点 | 为什么重要 | 缓解策略 |
|---|------|-----------|---------|
| **B83** | **脚本-系统版本兼容矩阵** | 蓝图升级后脚本检查逻辑是否仍然有效？没有版本兼容矩阵 → 无法判断 | `script_manifest.yaml` 新增 `compatible_blueprint_version` 字段（min/max 蓝图版本）。`run_all.py` 启动时对比蓝图版本 vs 脚本声明兼容范围 → 不兼容标记 `[VERSION_MISMATCH]` |
...
**新增文件**：`src/zephyr/db/write_batcher.py`——`WriteBatcher` 类，接口：`submit(table, row)` / `submit_many(table, rows)` / `flush()` / `start()` / `stop()`。
...
| 步骤 | 操作 | 回滚 |
|:---:|------|------|
| 1 | 新增 `src/zephyr/db/write_batcher.py` | 删除文件 |
