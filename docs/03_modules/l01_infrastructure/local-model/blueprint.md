...
title: Local Model 蓝图 — 本地模型推理基础设施·BGE-M3嵌入+Ollama推理+调度+缓存
version: 0.1.0
status: Active
layer: l01_infrastructure
construction_progress: phase_0_complete
actual_disk_path: src/zephyr/local_model/
...
| # | 声明项 | 值 |
|---|--------|-----|
| 1 | 主代码目录 | `src/zephyr/local_model/` |
...
| 目标态 | 独立模块，精确依赖，VMS/agent_spec/pipeline/runtime均依赖local_model |
...
```
消费者(agent_spec/pipeline/runtime/VMS)
...
| 类 | 方法 | 输入 | 输出 | 契约ID |
|----|------|------|------|--------|
| EmbeddingRouter | `embed(text, collection)` | str, str | np.ndarray | CT-039-001 |
| EmbeddingRouter | `embed_batch(texts, collection)` | List[str], str | np.ndarray | CT-039-002 |
| EmbeddingRouter | `warmup()` | — | None | CT-039-003 |
| OllamaEmbedder | `embed(text)` | str | np.ndarray | CT-039-004 |
| OllamaChat | `chat(prompt)` | str | str | CT-039-005 |
| LocalModelScheduler | `dispatch(task_type, payload)` | str, dict | dict | CT-039-006 |
| CacheLayer | `get(key)` / `set(key, value)` | str, np.ndarray | Optional[np.ndarray] | CT-039-007 |
...
|-----------|:-------:|------|--------|---------|
| MOD-INF-011 | 可选 | VMS消费嵌入服务 | CT-039-001 | `docs/03_modules/l01_infrastructure/vector-memory/blueprint.md` |
| MOD-INF-009 | 可选 | Pipeline消费嵌入+rerank | CT-039-001 | `docs/03_modules/_cross_layer/pipeline/blueprint.md` |
| MOD-INF-019 | 可选 | Agent Spec语义路由 | CT-039-001 | `docs/03_modules/l01_infrastructure/agent-spec/blueprint.md` |
...
|---|--------|:---:|---------|---------|---------|
| 1 | 039→无外部依赖 | 0 | L0 | 不适用 | 不适用 |

## §11 产出物存放目录 {temporal_type=permanent}

| 产出类型 | 路径 | consumer_min |
|---------|------|-------------|
| 代码 | `src/zephyr/local_model/` | agent_spec, pipeline, runtime, vector_memory |
| 测试 | `tests/unit/local_model/` | CI |
...
| # | 集成目标 | 契约ID | 当前状态 |
|---|---------|--------|:-------:|
| 1 | MOD-INF-011 VMS | CT-039-001 | ✅已集成 |
| 2 | MOD-INF-009 Pipeline | CT-039-001 | ✅已集成 |
| 3 | MOD-INF-019 Agent Spec | CT-039-001 | ✅已集成 |
| 4 | MOD-INF-002 Runtime | CT-039-003 | ✅已集成 |
