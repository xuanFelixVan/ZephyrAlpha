...
depends_on:
  ...
  - {target: "src/zephyr/db/task_repo.py", at: "全篇", why: "SQLite CRUD+10状态机+N:N task_files——数据层真源"}
  - {target: ADR-0038, at: "全篇", why: "File-as-Task范式——文件与任务1:1双向映射"}
  - {target: ADR-0040, at: "全篇", why: "Pydantic V2强制——所有模型基座"}
  - {target: ADR-0030, at: "全篇", why: "SQLite元数据层——tasks/events/gates四表"}
priority: P0
---

# Task System 蓝图 — 全链路任务卡生命周期管理

> module_id: MOD-INF-006 | version: 0.9.3 | status: active | layer: L01
...
| 蓝图→任务卡拆解 | blueprint_decomposer.py | RULE-ZERO-TASK：唯一合法路径 |
...
| src/zephyr/db/task_repo.py | MOD-INF-006（业务接口）/ MOD-INF-012（物理存储） | 业务接口定义权归 MOD-INF-006 |
| src/zephyr/core/models.py | MOD-INF-006 | TaskCard 模型 |
| src/zephyr/core/blueprint_decomposer.py | MOD-INF-006 | 蓝图拆解器 |
| src/zephyr/mcp/task_manager_server.py | MOD-INF-006（业务逻辑）/ MOD-INF-013（协议层） | 见 §0.4 SSoT 声明 |
...
| 4 | ✅包含 | **task_id 格式统一为 `{NAMESPACE}-{SEQ}`** | ADR-001 / STD-005 / SRC-042 |
...
> ⛔ **强制规则（RULE-ZERO-TASK）**：蓝图拆解是生成任务卡的**唯一合法路径**。
...
| # | 上游 | 处理逻辑 | 下游 | 数据格式 |
|---|--------|---------|---------|---------|
| 1 | 蓝图.md | BlueprintDecomposer 解析§16 | task_repo(SQLite) | TaskCard Pydantic |
...
> ⚠️ **B-20 铁律**：§0.1 标记`已实现`的模块，蓝图只保留接口签名（§4），不复制实现代码。完整 TaskCard 字段定义见 `src/zephyr/task_system/core/models.py`。
...
| TaskNamespace | ADR/CP/KE/STD/DW/SRC/OPS |
...
| 模型 | 关键字段 |
|------|---------|
| DecompositionResult | total_tasks, tasks: list[TaskCard], dependency_graph, unassigned_items, warnings |
...
| 6 | 门禁执行时机 | 门禁在状态转换前执行 |
| 7 | 任务卡编号 | `{NAMESPACE}-{SEQ}`（ADR/CP/KE/STD/DW/SRC/OPS） |
...
| 3 | `zephyr.core.shared_schemas` | `zephyr.l01_infrastructure.a2a_protocol.*` | A2A 是通信层，任务系统不应直接依赖 |
...
| 4 | BlueprintDecomposer | HALT | 暂停任务创建，等待 Decomposer 恢复 | Decomposer 恢复后重新解析验证 | RULE-ZERO-TASK：禁止手动创建 TaskCard |
...
| GOV-DOC-002 | 必须 | §5.1.2 路径映射 | — | `docs/01_policies_and_standards/governance/document/directory-structure-standard.md` |
...
| MOD-INF-005 | 必须 | 脚本系统 12 维度 | ≥3.0.0 | `docs/03_modules/l01_infrastructure/script-system/blueprint.md` |
...
| task_repo.py | 必须 | SQLite CRUD + 10状态机 + N:N + BatchCoordination | 现有代码 | `src/zephyr/db/task_repo.py` |
...
|---|---------|---------|-------------|------|
| 1 | 依赖图自动生成 | AST解析import + manifest字段 | asset_inventory/dependency.py | 不覆盖scripts/ |
...
| MOD-INF-010→MOD-INF-006(X-04) | 2 | L2(中) | 自愈任务走 RULE-ZERO-TASK 唯一路径 |
...
|----------|---------------|:---:|------|
| 蓝图文件 | `docs/03_modules/l01_infrastructure/task-system/blueprint.md` | 0 | 本文件 |
| 业务代码 | `src/zephyr/task_system/` | 1 | TaskSystem 包 |
| 业务代码 | `src/zephyr/orchestrator/` | 1 | 管线调度器 |
| 业务代码 | `src/zephyr/core/models.py` | 1 | TaskCard 模型 |
| 业务代码 | `src/zephyr/core/blueprint_decomposer.py` | 1 | 蓝图拆解器 |
| 数据层 | `src/zephyr/db/task_repo.py` | 5 | SQLite CRUD + 状态机 |
| 数据层 | `src/zephyr/db/sqlite_schema.py` | 5 | Schema + 迁移链 |
| MCP 接口 | `src/zephyr/mcp/task_manager_server.py` | 2 | MCP 5 Tool |
| MCP 契约 | `src/zephyr/mcp/tool_contracts.yaml` | 2 | Tool Schema |
| 门禁 | `src/zephyr/gates/task_completion_gate.py` | 1 | G7 门禁 |
| 管线 | `src/zephyr/pipeline/pipeline_orchestrator.py` | 1 | 管线编排器 |
| 上下文 | `src/zephyr/context_engine/context_assembler.py` | 1 | 上下文装配器 |
| 任务卡（SQLite 真源）| `data/zalpha_metadata.db` — tasks 表 | 5 | SQLite |
| Task 模型基座 | `src/zephyr/shared/schemas.py` | 5 | 31 字段基座 |
| N:N 文件映射 | `src/zephyr/orchestrator/file_task_mapper.py` | 1 | 文件-任务映射 |
| 知识审阅池 | `src/zephyr/kb/triage.py` | 1 | Triage |
...
| 2 | 任务卡元注册表 | `docs/01_policies_and_standards/_registry/catalogs/task-card-meta-registry.yaml` | 迁移状态更新 | v0.2.0→v0.3.0 |
| 3 | core/models.py | `src/zephyr/core/models.py` | TaskCard 继承 Task | 基座对齐 |
| 4 | blueprint_decomposer.py | `src/zephyr/core/blueprint_decomposer.py` | 对接 task_repo | 数据层真源 |
| 5 | task_manager_server.py | `src/zephyr/mcp/task_manager_server.py` | MCP 5 Tool | 接入 SQLite |
| 6 | task_completion_gate.py | `src/zephyr/gates/task_completion_gate.py` | G7 门禁逻辑同步 | 约束对齐 |
...
#### 步骤 2：同步 task-card-meta-registry.yaml
...
| AI 自治范围 | ai_modifiable |
| 验证命令 | `python -c "import yaml; d=yaml.safe_load(open('docs/01_policies_and_standards/_registry/catalogs/task-card-meta-registry.yaml')); print('OK')"` |
| 检查点 | task-card-meta-registry.yaml 包含 MOD-INF-006 迁移记录 |
| G7 检查项 | 迁移状态是否准确？ |
...
|------|------|
| 对应蓝图契约 | §4.2.1 |
| 产出位置 | `src/zephyr/core/models.py` |
...
| 验收标准 | ① isinstance(TaskCard(...), Task) == True；② task_id pattern `^(ADR/|CP/|KE/|STD/|DW/|SRC/|OPS)-//d+$`；③ status ∈ TaskStatus enum |
| AI 自治范围 | human_gated |
...
|------|------|
| 对应蓝图契约 | §4.1.1 |
| 产出位置 | `src/zephyr/core/blueprint_decomposer.py` |
...
|------|------|
| 对应蓝图契约 | §4.5 |
| 产出位置 | `src/zephyr/mcp/task_manager_server.py` |
...
| M8 | B区 | 标准合规——PS/GOV/ADR | DeepSeek V4 Pro |
...
| 2 | task-card-meta-registry.yaml | `docs/01_policies_and_standards/_registry/catalogs/task-card-meta-registry.yaml` | ☐ | ☐ | ☐ |
| 3 | core/models.py | `src/zephyr/core/models.py` | ☐ | ☐ | ☐ |
| 4 | blueprint_decomposer.py | `src/zephyr/core/blueprint_decomposer.py` | ☐ | ☐ | ☐ |
| 5 | task_manager_server.py | `src/zephyr/mcp/task_manager_server.py` | ☐ | ☐ | ☐ |
...
> B-20 铁律：已实现代码不在蓝图中重复。完整 SQL 见 `src/zephyr/db/task_repo.py`。
...
| `claim_next` | `(batch_id, worker_id) -> TaskCard /| None` | 原子认领 |
| `recover_stale_claims` | `(batch_id, timeout_minutes=30) -> int` | 超时回收 |
| `batch_progress` | `(batch_id) -> dict[str, int]` | 批量进度聚合 |

#### §16.7.4 BatchOrchestrator 使用模式

> B-20 铁律：已实现代码不在蓝图中重复。完整使用示例见 `src/zephyr/orchestrator/batch_orchestrator.py`。
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `db_path` | str | `data/zalpha_metadata.db` | SQLite 数据库路径 |
...
| # | 决策ID | 决策 | 选项 | 选中 | 依据 | 日期 |
|---|--------|------|------|------|------|------|
| 1 | D-INF006-01 | TaskCard 继承 shared/schemas.py Task | A:独立模型/B:继承Task | B | ADR-0040 Pydantic V2 强制 + SSoT 唯一 | 2026-05-02 |
| 2 | D-INF006-02 | task_id 格式 {NAMESPACE}-{SEQ} | A:UUID/B:NAMESPACE-SEQ | B | ADR-001/SRC-042——自文档 | 2026-05-02 |
| 3 | D-INF006-03 | SQLite 作为唯一持久化层 | A:文件系统/B:SQLite/C:PostgreSQL | B | ADR-0030——零依赖+单机部署 | 2026-05-02 |
| 4 | D-INF006-04 | File-as-Task 范式 | A:纯任务/B:文件-任务1:1 | B | ADR-0038——双向映射 | 2026-05-02 |
...
| TaskNamespace | 任务编号命名空间枚举：ADR/CP/KE/STD/DW/SRC/OPS。task_id 格式 `{NAMESPACE}-{SEQ}` |
...
| TaskManagerServer | MCP 接口服务器——5 Tool 接口，数据真源为 SQLite |
...
|---|---------------|------------|---------|---------|------------|
| 1 | MOD-INF-003 任务卡KMS蓝图 | `docs/03_modules/l01_infrastructure/task-card-kms/blueprint.md` | 覆盖型 | 本蓝图 | 已标记 deprecated→物理删除 |
| 2 | MOD-INF-004 双管线蓝图 | `docs/03_modules/l01_infrastructure/vibe-coding-pipelines/blueprint.md` | 覆盖型 | 本蓝图 | 已标记 deprecated→物理删除 |
| 3 | v0.2.0 TaskCard 模型 | `src/zephyr/core/models.py` | 覆盖型 | v0.3.0 TaskCard | 重写对齐新契约 |
...
| 2 | 目录结构标准 | GOV-DOC-002 | — | `docs/01_policies_and_standards/governance/document/directory-structure-standard.md` | 路径映射、边界判据 |
...
| 4 | 脚本系统蓝图 | MOD-INF-005 | 3.0.0+ | `docs/03_modules/l01_infrastructure/script-system/blueprint.md` | 审计消费方 |
...
| 13 | task_repo.py | — | 现有代码 | `src/zephyr/db/task_repo.py` | SQLite CRUD + 10状态机——数据层真源 |
| 14 | 任务卡元注册表 | task-card-meta-registry | V-13 | `docs/01_policies_and_standards/_registry/catalogs/task-card-meta-registry.yaml` | 迁移状态追踪 |
...
|---|-------------|------------|----------|-------------|
| 1 | MOD-INF-003（旧蓝图层） | `docs/03_modules/l01_infrastructure/task-card-kms/blueprint.md` | 任务卡制度+KMS体系 | deprecated——已被本蓝图合并 |
| 2 | MOD-INF-004（旧双管线） | `docs/03_modules/l01_infrastructure/vibe-coding-pipelines/blueprint.md` | 双管线流程+M模块 | deprecated——已被本蓝图合并 |
| 3 | Task 模型（shared/schemas.py） | `src/zephyr/shared/schemas.py` | 语义28+追踪3=31 字段 | ✅ 可复用——本蓝图 TaskCard 继承此模型 |
| 4 | task_repo.py（SQLite CRUD） | `src/zephyr/db/task_repo.py` | 创建/查询/更新/删除/状态转换 | ✅ 可复用——本蓝图数据层使用此代码 |
...
|---|---------|------------|------|---------|
| 1 | 本蓝图 | `docs/03_modules/l01_infrastructure/task-system/blueprint.md` | 真源 | 重写 v0.9.4 |
| 2 | Change Folder | `docs/03_modules/l01_infrastructure/task-system/changes/` | 新建 | 存放任务卡 .md 文件 |
...
| 5 | task_repo.py | `src/zephyr/db/task_repo.py` | 依赖 | 数据层真源 |
| 6 | core/models.py | `src/zephyr/core/models.py` | 重写 | 对齐到 shared/schemas.py Task 继承 |
| 7 | blueprint_decomposer.py | `src/zephyr/core/blueprint_decomposer.py` | 重写 | 输出改为 task_repo(SQLite) + .md |
| 8 | task_manager_server.py | `src/zephyr/mcp/task_manager_server.py` | 重写 | 接入 task_repo(SQLite) 真源 |
| 9 | task_completion_gate.py | `src/zephyr/gates/task_completion_gate.py` | 读取 | 需同步 G7 门禁 |
| 10 | metadata-registry.md | `docs/01_policies_and_standards/meta/metadata-registry.md` | 读取 | §7 字段真源 |
| 11 | task-card-meta-registry.yaml | `docs/01_policies_and_standards/_registry/catalogs/task-card-meta-registry.yaml` | 修改 | 更新迁移状态 |
...
| 文件路径 | 实现状态 | 说明 |
|---------|:---:|------|
| `src/zephyr/core/adaptation/execution_tuner.py` | ✅ 已实现 | |
| `src/zephyr/core/adaptation/prompt_version_manager.py` | ✅ 已实现 | |
| `src/zephyr/core/blueprint_code_sync.py` | ✅ 已实现 | |
| `src/zephyr/core/blueprint_decomposer.py` | ✅ 已实现 | |
| `src/zephyr/core/compensation/saga_compensator.py` | ✅ 已实现 | |
| `src/zephyr/core/context_engine.py` | ✅ 已实现 | |
| `src/zephyr/core/dependency/dependency_graph.py` | ✅ 已实现 | |
| `src/zephyr/core/draft/draft_assistant.py` | ✅ 已实现 | |
| `src/zephyr/core/events/event_bus.py` | ✅ 已实现 | |
| `src/zephyr/core/events/event_reactor.py` | ✅ 已实现 | |
| `src/zephyr/core/events/event_store.py` | ✅ 已实现 | |
| `src/zephyr/core/events/hook_dispatcher.py` | ✅ 已实现 | |
| `src/zephyr/core/healthcheck_service.py` | ✅ 已实现 | |
| `src/zephyr/core/impact/impact_propagator.py` | ✅ 已实现 | |
| `src/zephyr/core/impact/llm_impact_analyzer.py` | ✅ 已实现 | |
| `src/zephyr/core/knowledge/ke_linker.py` | ✅ 已实现 | |
| `src/zephyr/core/knowledge/ke_structurer.py` | ✅ 已实现 | |
| `src/zephyr/core/knowledge/kms_interface.py` | ✅ 已实现 | |
| `src/zephyr/core/lifecycle/scope_guard.py` | ✅ 已实现 | |
| `src/zephyr/core/lifecycle/task_lifecycle_manager.py` | ✅ 已实现 | |
| `src/zephyr/core/maintenance/autonomy_monitor.py` | ✅ 已实现 | |
| `src/zephyr/core/maintenance/dogfooding.py` | ✅ 已实现 | |
| `src/zephyr/core/maintenance/handbook.py` | ✅ 已实现 | |
| `src/zephyr/core/maintenance/zero_config.py` | ✅ 已实现 | |
| `src/zephyr/core/models.py` | ✅ 已实现 | |
| `src/zephyr/core/observability/cli_summary.py` | ✅ 已实现 | |
| `src/zephyr/core/observability/cost_tracker.py` | ✅ 已实现 | |
| `src/zephyr/core/observability/failure_matcher.py` | ✅ 已实现 | |
| `src/zephyr/core/observability/notifier.py` | ✅ 已实现 | |
| `src/zephyr/core/observability/trace_decorator.py` | ✅ 已实现 | |
| `src/zephyr/core/quality/quality_monitor.py` | ✅ 已实现 | |
| `src/zephyr/core/queue/task_queue.py` | ✅ 已实现 | |
| `src/zephyr/core/queue/task_scheduler.py` | ✅ 已实现 | |
| `src/zephyr/core/reliability/circuit_breaker.py` | ✅ 已实现 | |
| `src/zephyr/core/reliability/context_guard.py` | ✅ 已实现 | |
| `src/zephyr/core/reliability/diff_planner.py` | ✅ 已实现 | |
| `src/zephyr/core/reliability/retry_handler.py` | ✅ 已实现 | |
| `src/zephyr/core/session/session_boundary.py` | ✅ 已实现 | |
| `src/zephyr/core/session/session_continuity.py` | ✅ 已实现 | |
| `src/zephyr/core/session_continuity.py` | ✅ 已实现 | |
| `src/zephyr/core/sla/sla_monitor.py` | ✅ 已实现 | |
| `src/zephyr/core/sync/blueprint_code_sync.py` | ✅ 已实现 | |
...
**审计范围**：蓝图 v0.3.2 → v0.6.0 + ADR-0038/0040/0030 + 全部核心源码 + 治理文档
