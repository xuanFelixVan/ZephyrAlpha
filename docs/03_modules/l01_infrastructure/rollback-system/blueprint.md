...
actual_disk_path: "src/zephyr/rollback/"
...
| 回滚触发链路 | **本文档 §10.4** | behavioral_auditor/rollback_bridge.py（消费端） |
...
| MCP 入口 | `src/zephyr/mcp/governance_server.py:_execute_rollback()`（调用 RollbackExecutor）|
| 已降级遗留 | `src/zephyr/orchestrator/rollback_manager.py`（仅调试场景手动 DB 快照）|
| 影子实现（待清理）| `src/zephyr/governance/rollback/`（同名类不同实现）、`src/zephyr/orchestrator/resilience/rollback_manager.py`（旧设计）|
...
| 40 | ModelDriftDetector | drift_detector 每日对比 AI 输出质量 | — | ✅ |
...
|---------|---------|---------|---------|---------|
| MOD-INF-020 | 必须 | Audit Trail——回滚操作写入审计日志 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\audit-trail\blueprint.md` |
| MOD-INF-018 | 必须 | Agent RBAC——auto_guard 后验失败触发自动回滚 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\agent-rbac\blueprint.md` |
| MOD-INF-007 | 必须 | Gate Engine——回滚后跑 G0 门禁验证 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\gate-engine\blueprint.md` |
| MOD-MASTER-001 | 必须 | CT-RBK-GATE-001 集成契约 | — | `D:\ZephyrAlpha\docs\03_modules\_sys-master\blueprint.md` |
| MOD-INF-016 | 可选 | Shared Core 承载 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\shared-core\blueprint.md` |
| MOD-INF-012 | 必须 | Shared Core 数据脊——rollback_metrics.db/JSONL 快照路径解析 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\shared-core\blueprint.md` |
...
| `governance/rollback/budget_tracker.py` | `RollbackBudgetTracker` | G-CT-009 契约——回滚成本计入预算 | 保留，属于 budget_enforcer 职责 |
| `orchestrator/resilience/rollback_manager.py` | `RollbackManager` | 旧设计（ADR-0038），仅 DB 恢复无 git-native | **待迁移**——降级为 thin shim 调用 RollbackExecutor |
...
| `drift_detector/` 下 6 个文件 | `rollback_fix()`/`trigger_rollback()` 等 | 与 behavioral_auditor/ 近似重复 | **待清理**——蓝图已标注"待清理" |
...
| 产出物类型 | 存放完整绝对路径 | 说明 |
|----------|---------------|------|
| 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\rollback-system\blueprint.md` | 本文件 |
...
|---|------------|------------|---------|---------|
| 1 | 模块 ID 注册表 | `D:\ZephyrAlpha\docs\02_enterprise_architecture\target-architecture\architecture-model\module-id-registry.yaml` | MOD-INF-021 版本+generation | 蓝图升级 |
| 2 | 蓝图注册表 | `D:\ZephyrAlpha\docs\03_modules\blueprint-registry.yaml` | MOD-INF-021 版本+路径 | 蓝图升级 |
| 3 | 治理资产清单 | `D:\ZephyrAlpha\docs\01_policies_and_standards\_registry\catalogs\document-metadata-index.yaml` | MOD-INF-021 元数据 | 蓝图升级 |
...
| 步骤 | 如果出问题 | 回滚操作 |
|------|----------|---------|
| 1 | rollback_executor.py 语法错误 | git checkout -- src/zephyr/rollback/rollback_executor.py |
...
| 2 | resilience/rollback_manager.py | `D:\ZephyrAlpha\src\zephyr\orchestrator\resilience\rollback_manager.py` | 完整 checkpoint/rollback_to 链路 | 旧设计（ADR-0038），仅 DB 恢复无 git-native——**待迁移为 thin shim** |
...
| 3 | 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\rollback-system\blueprint.md` | 蓝图 | 修改 |
