...
> module_id: MOD-L02-001 | version: 4.0.3 | status: active | domain: factor
...
> 查询最新文件清单：`python scripts/governance/extract_depgraph.py --modules MOD-L02-001`
...
> 生成命令：`python scripts/governance/d5_architecture/generators/generate_blueprint_panorama.py MOD-L02-001`

#### 四图位置

| 图 | 位置 | 状态 | 链接 |
|----|------|------|------|
| 依赖图 (depgraph) | `blueprint_id=MOD-L02-001` 的 9 个 file 节点 | prototype | `extract_depgraph.py --modules MOD-L02-001` |
| 数据流图 (dataflow) | 2 个 Dataset / 2 个 Job | active | `apply_dataflowgraph.py --list-datasets` |
