...
**负向责任**: 不涉及应用层业务逻辑 / 数据库 Schema 设计（→ MOD-INF-012）/ 契约定义（→ MOD-INF-016-CONTRACTS）。
...
所有 L01-L13 模块均直接消费 shared/ 基础设施。详见 MOD-INF-016 集成蓝图 §7 反向依赖索引。

## §3 关联

- 父蓝图: MOD-INF-016 (Shared+Core 集成蓝图)
- 兄弟蓝图: MOD-INF-016-CONTRACTS / MOD-INF-016-CORE
- SSoT 映射: `architecture-model/layers/b_shared.yaml`
