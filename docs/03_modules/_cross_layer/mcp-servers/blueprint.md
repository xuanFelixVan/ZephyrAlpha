...
| tool_contracts.yaml 工具名 = 代码中注册的工具名 | `grep "def .*_tool" *.py` | ☐ |
...
| R50 | tool_contracts 无跨 tool 前置约束声明（B103） | 中 | 🔴高 | 新增 `depends_on: [{tool_id, required_outcome}]` 字段 | 风险 |
...
| R100 | 零工具执行超时——慢/死工具可永久挂起（B153） | 中 | 🔴高 | tool_contracts.yaml 新增 `timeout_ms` 字段 | 风险 |
...
|---------|---------|---------|---------|---------|
| MOD-INF-006 | 必须 | task_manager MCP——decompose_blueprint接口 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\task-repo\blueprint.md` |
| MOD-KB-001 | 必须 | knowledge_base MCP——KE查询接口 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\knowledge-base\blueprint.md` |
| MOD-INF-007 | 必须 | gate_engine MCP——Gate判定接口 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\gate-engine\blueprint.md` |
...
|---|---------|---------|-------------|------|
| 1 | 依赖图自动生成 | AST解析import + manifest字段 | asset_inventory/dependency.py | 不覆盖mcp/目录 |
...
| `src/zephyr/mcp/audit_logger.py` | ✅ 已实现 | |
| `src/zephyr/mcp/blueprint_search_server.py` | ✅ 已实现 | |
| `src/zephyr/mcp/doc_guard_server.py` | ✅ 已实现 | |
| `src/zephyr/mcp/error_codes.py` | ✅ 已实现 | |
| `src/zephyr/mcp/gate_engine_server.py` | ✅ 已实现 | |
| `src/zephyr/mcp/gateway_server.py` | ✅ 已实现 | |
| `src/zephyr/mcp/governance_server.py` | ✅ 已实现 | |
| `src/zephyr/mcp/handoff_auto_loader.py` | ✅ 已实现 | |
| `src/zephyr/mcp/knowledge_base_server.py` | ✅ 已实现 | |
| `src/zephyr/mcp/prompt_provider.py` | ✅ 已实现 | |
| `src/zephyr/mcp/rate_limiter.py` | ✅ 已实现 | |
| `src/zephyr/mcp/resource_provider.py` | ✅ 已实现 | |
| `src/zephyr/mcp/sandbox_server.py` | ✅ 已实现 | |
| `src/zephyr/mcp/sentinel_server.py` | ✅ 已实现 | |
| `src/zephyr/mcp/task_manager_server.py` | ✅ 已实现 | |
| `src/zephyr/mcp/telemetry_server.py` | ✅ 已实现 | |
| `src/zephyr/mcp/tool_contracts.yaml` | ✅ 已实现 | |
| `src/zephyr/mcp/vector_memory_server.py` | ✅ 已实现 | |
...
| MCP 施工步骤 | **本文档 §11** | — |
| MCP 接口契约 | **本文档 §3** | tool_contracts.yaml（派生） |
...
```
新 AI → registry-of-registries.yaml → MOD-INF-013 → 本蓝图
     → AGENTS.md §8.2 MCP 任务菜单 → 本蓝图 §11 施工指引
     → src/zephyr/mcp/tool_contracts.yaml → 本蓝图 §3 契约定义
```
...
| 修改本蓝图 | MUST 同步更新 |
|-----------|-------------|
| §3 接口契约 | `src/zephyr/mcp/tool_contracts.yaml` + 下游消费者蓝图 |
| §2 MCP 服务端列表 | `src/zephyr/mcp/__init__.py` + `architecture-model/layers/b_mcp.yaml` |
...
| 3 | 工具契约 | `D:\ZephyrAlpha\src\zephyr\mcp\tool_contracts.yaml` | 读取 | 契约 SSoT |
