...
| §18 | CLI 接口 | 8 个命令行入口 |
| §19 | 配置系统 | config/orphan_judge.yaml |
...
| 文件 | Phase | 说明 |
|------|-------|------|
| `src/zephyr/orphan_judge/__init__.py` | 1 | 包初始化 + __all__ |
| `src/zephyr/orphan_judge/models.py` | 1 | 数据模型 |
| `src/zephyr/orphan_judge/judge.py` | 1 | OrphanJudge 主控 |
| `src/zephyr/orphan_judge/registration_checker.py` | 0 | L0 注册检查 |
| `src/zephyr/orphan_judge/reference_graph_engine.py` | 0 | L1 引用图 |
| `src/zephyr/orphan_judge/duplicate_detector.py` | 0 | L2 功能重复 |
| `src/zephyr/orphan_judge/unique_analyzer.py` | 0 | L3 独特价值 |
| `src/zephyr/orphan_judge/standalone_evaluator.py` | 0 | L4 独立价值 |
| `src/zephyr/orphan_judge/decision_table.py` | 0 | 决策表 |
| `src/zephyr/orphan_judge/safety_fence.py` | 0 | 安全围栏 |
| `src/zephyr/orphan_judge/deprecation_tracker.py` | 0 | 废弃追踪 |
| `src/zephyr/orphan_judge/cascade_analyzer.py` | 0 | 级联清理 |
| `src/zephyr/orphan_judge/orphan_collector.py` | 1 | 统一收集器 |
| `src/zephyr/orphan_judge/report_generator.py` | 1 | 报告生成 |
| `src/zephyr/orphan_judge/config_loader.py` | 0 | 配置加载 |
| `src/zephyr/orphan_judge/__main__.py` | 1 | CLI 入口 |
| `src/zephyr/orphan_judge/drift_bridge.py` | 2 | Drift 桥接 |
| `src/zephyr/orphan_judge/escalation_bridge.py` | 2 | Escalation 桥接 |
| `src/zephyr/orphan_judge/rbac_bridge.py` | 2 | RBAC 桥接 |
| `src/zephyr/orphan_judge/kb_bridge.py` | 2 | KB 桥接 |
| `src/zephyr/orphan_judge/mcp_integration.py` | 2 | MCP 集成 |
| `config/orphan_judge.yaml` | 0 | 配置文件 |
| `config/orphan_judge_entry_points.yaml` | 0 | 入口点配置 |
| `tests/orphan_judge/` | 3 | 测试目录 |
...
3. Phase 2：Agent Skill 注册 + orphan_judge.md
