...
> ⚠️ **门禁逻辑不适用（#ARCH-048, 2026-07-05）**：本节原"需求门禁触发演进"逻辑已被母蓝图 MOD-ARCH_BIZDB §3.1 弃用，改为"硬性边界二元判定：能造现在就造"。Redis/EventStore 施工优先级降为 P2（待实盘需求触发）。ClickHouse 已于 2026-07-01 部署（INFRA-DB-006），不再受"AUM>200万"门禁约束。以下门禁表保留作为历史参考，**不作为当前施工依据**。
