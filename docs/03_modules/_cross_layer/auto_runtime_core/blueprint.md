...
| Q2 | **worktree 泄漏**——AI 进程崩溃未 cleanup，worktree 残留占磁盘 | 高 | ① WorktreeManager 启动时扫描 `git worktree list --porcelain` 检测无主 worktree；② 超过 TTL（默认 24h）的 session worktree 自动 cleanup；③ spawn_agent.ps1 的 `finally` 块保证退出时清理；④ 事件驱动 reconciler `GATE-WORKTREE-LIFECYCLE` 在每次 commit/merge 后自动 sweep stale worktree（替代废弃的 `worktree_gc.py` 守护进程时间触发方案，符合"永久系统必须全自动/事件触发"铁律） | session_worktree.py（session_worktree_sweep）+ reconciliation_registry.py（GATE-WORKTREE-LIFECYCLE） |
