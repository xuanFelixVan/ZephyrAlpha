...
> module_id: MOD-INF-035 | version: 6.0.1 | status: active | layer: cross_layer
...
> 查询最新文件清单：`python scripts/governance/extract_depgraph.py --modules MOD-INF-035`
...
> 生成命令：`python scripts/governance/d5_architecture/generators/generate_blueprint_panorama.py MOD-INF-035`

#### 四图位置

| 图 | 位置 | 状态 | 链接 |
|----|------|------|------|
| 依赖图 (depgraph) | `blueprint_id=MOD-INF-035` 的 66 个 file 节点 | prototype | `extract_depgraph.py --modules MOD-INF-035` |
...
| `src/zephyr/gov_enforcement/rule_bridge/git_commit_gateway.py` | ✅ 已实现 | |
| `src/zephyr/governance/audit/reconciliation_registry.py` | ✅ 已实现 | |
...
| `src/zephyr/shared/contracts/runtime_types.py` | ✅ 已实现 | |
| `src/zephyr/trading/__main__.py` | ✅ 已实现 | |
| `src/zephyr/trading/action_dispatcher.py` | ✅ 已实现 | |
| `src/zephyr/trading/ai_audit_logger.py` | ✅ 已实现 | |
| `src/zephyr/trading/auto_integrator.py` | ✅ 已实现 | |
| `src/zephyr/trading/auto_runtime_core.py` | ✅ 已实现 | |
| `src/zephyr/trading/auto_task_generator.py` | ✅ 已实现 | |
| `src/zephyr/trading/boot_hooks.py` | ✅ 已实现 | |
| `src/zephyr/trading/capability_card.py` | ✅ 已实现 | |
| `src/zephyr/trading/capability_registry.py` | ✅ 已实现 | |
| `src/zephyr/trading/capability_sync.py` | ✅ 已实现 | |
| `src/zephyr/trading/dream_cycle.py` | ✅ 已实现 | |
| `src/zephyr/trading/finalizer.py` | ✅ 已实现 | |
| `src/zephyr/trading/health_monitor.py` | ✅ 已实现 | |
| `src/zephyr/trading/integration_registry.py` | ✅ 已实现 | |
| `src/zephyr/trading/lifecycle_manager.py` | ✅ 已实现 | |
| `src/zephyr/trading/module_onboarding_scanner.py` | ✅ 已实现 | |
| `src/zephyr/trading/night_shift_queue.py` | ✅ 已实现 | |
| `src/zephyr/trading/orphan_detector.py` | ✅ 已实现 | |
| `src/zephyr/trading/ports.py` | ✅ 已实现 | |
| `src/zephyr/trading/resource_optimization.py` | ✅ 已实现 | |
| `src/zephyr/trading/runtime/async_runtime.py` | ✅ 已实现 | |
| `src/zephyr/trading/runtime_config.py` | ✅ 已实现 | |
| `src/zephyr/trading/staging_area.py` | ✅ 已实现 | |
| `src/zephyr/trading/status_dashboard.py` | ✅ 已实现 | |
| `src/zephyr/trading/stop_gate.py` | ✅ 已实现 | |
| `src/zephyr/trading/task_gate.py` | ✅ 已实现 | |
| `src/zephyr/trading/windows_service.py` | ✅ 已实现 | |
| `src/zephyr/trading/work_dag.py` | ✅ 已实现 | |
| `src/zephyr/trading/work_orchestrator.py` | ✅ 已实现 | |
...
| `tests/trading/test_trading_session_lifecycle.py` | ✅ 已实现 | |
...
| Q2 | **worktree 泄漏**——AI 进程崩溃未 cleanup，worktree 残留占磁盘 | 高 | ① WorktreeManager 启动时扫描 `git worktree list --porcelain` 检测无主 worktree；② 超过 TTL（默认 24h）的 session worktree 自动 cleanup；③ spawn_agent.ps1 的 `finally` 块保证退出时清理；④ 事件驱动 reconciler `GATE-WORKTREE-LIFECYCLE` 在每次 commit/merge 后自动 sweep stale worktree（替代废弃的 `worktree_gc.py` 守护进程时间触发方案，符合"永久系统必须全自动/事件触发"铁律）；⑤ 阶段2治本（#ARCH-WORKTREE-001 第三期，2026-07-18）：sweep 判据3 扩展 `_branch_commits_superseded` 两维度取代判定（patch-id 等价 `git cherry` + message 主体匹配 HEAD 近 200 条历史），死 session 的未合并提交若已通过其他路径合并到 HEAD 则安全清理，治"未合并提交陷阱"导致的永久堆积（100% AI 开发场景高发） | session_worktree.py（session_worktree_sweep + _branch_commits_superseded）+ reconciliation_registry.py（GATE-WORKTREE-LIFECYCLE） |
| Q3 | **merge 冲突如何处理**——多 worktree 改了同一文件，merge 回主分支冲突 | 高 | ① merge_session_worktree 检测冲突时**不强制 merge**，保留 worktree 供人工解决；② 在 `.runtime/worktree_conflicts/{session_id}.json` 记录冲突文件清单；③ spawn_agent.ps1 输出冲突提示，AI 自行决定重试或人工介入；④ 不自动 abort，避免丢失工作 | session_worktree.py（session_worktree_merge） |
