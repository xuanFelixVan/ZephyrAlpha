...
layer: cross_layer
summary: >
  cross_layer 模块物理真源已从 infra_ops/ 迁移至 _cross_layer/（Phase 5 执行完毕）。
...
> **Phase 5 迁移完成（2026-05-06）**：8 个 `layer: cross_layer` 模块已从 `infra_ops/` 迁移至本目录。
...
| MOD-INF-030 | RedBlue Validator | [blueprint](red-blue-validator/blueprint.md) |
...
- **2026-05-06 Phase 5**：物理目录从 `infra_ops/` 迁至 `_cross_layer/`。
