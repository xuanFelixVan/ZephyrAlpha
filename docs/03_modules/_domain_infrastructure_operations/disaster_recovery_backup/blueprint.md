...
title: "灾备备份系统蓝图 — 一键触发→DB dump→Restic去重备份→保留清理→校验→报告"
doc_type: blueprint
...
summary: "灾备备份系统——一键触发的Restic去重备份流水线，覆盖代码+配置+数据库+不可替代数据，目标盘F:(SanDisk 2TB)，遵循3-2-1原则与数据最小化原则"
tags: [backup, disaster-recovery, restic, pg-dump, sqlite-backup, clickhouse-backup, 3-2-1-rule]
priority: P1
...
---
> module_id: MOD-INF-027 | version: 1.0.0 | status: active | layer: L0_infrastructure
> actual_disk_path: scripts/backup/ | generation: 1 | construction_progress: planned

# 灾备备份系统蓝图 — 一键触发→DB dump→Restic去重备份→保留清理→校验→报告

## 概述

灾备备份系统是 ZephyrAlpha 的数据安全底线——解决"单点故障=项目死亡"的核心风险。系统采用 Restic 去重备份工具，一键触发后自动执行：预检→数据库dump→Restic备份→保留策略清理→完整性校验→报告输出。备份目标为外置盘 F:(SanDisk 2TB NTFS)。备份内容遵循数据最小化原则（ISO/IEC 27001）：不可替代数据必备份，可重建数据排除，可重新下载数据由Restic去重红利决定包含。系统遵循3-2-1原则的本地实现（3副本=原盘+Restic快照+git历史，2介质=SSD+外置盘，1异地=未来扩展云备份）。
...
|---|--------|------------|------|:-----:|-------------------|
| 1 | `backup.ps1` | §3.1 | 主备份脚本（预检+dump+restic+清理+校验+报告） | 待实现 | |
| 2 | `backup_config.yaml` | §3.2 | 备份配置（路径/保留策略/排除规则） | 待实现 | |
| 3 | `一键备份.bat` | §3.3 | 双击触发入口（调用backup.ps1） | 待实现 | |
| 4 | `restore.ps1` | §3.4 | 恢复脚本（查看快照/恢复指定快照/灾难恢复） | 待实现 | |
| 5 | `README.md` | §3.5 | 使用说明（首次初始化/日常备份/灾难恢复） | 待实现 | |
...
手动一键触发，无计划任务（符合用户选择）：
- 双击 `scripts/backup/一键备份.bat`
- 或右键 `backup.ps1` → "用PowerShell运行"
- 或终端：`powershell -ExecutionPolicy Bypass -File scripts\backup\backup.ps1`
...
### §3.2 backup_config.yaml — 备份配置

**职责**：集中管理路径、保留策略、排除规则（脚本读取，避免硬编码）。
