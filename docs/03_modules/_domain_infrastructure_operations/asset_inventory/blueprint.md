...
---
> module_id: MOD-INF-026 | version: 3.1.9 | status: active | layer: L0_infrastructure
...
> 查询最新文件清单：`python scripts/governance/extract_depgraph.py --modules MOD-INF-026`
...
> 生成命令：`python scripts/governance/d5_architecture/generators/generate_blueprint_panorama.py MOD-INF-026`
...
| 模块 | 关系 |
|------|------|
| [MOD-INF-020 audit-trail](../../_domain_governance/audit_trail/blueprint.md) | **兄弟模块**——本模块产出资产事件，MOD-INF-020 做不可变审计记录 |
...
| [MOD-INF-005 governance-automation](../../_domain_governance/governance_automation/blueprint.md) | **调度依赖**——`generate_asset_index.py` 作为治理脚本 |
...
| GOV-CMP-003 审计协议 | **治理依赖**——盘点结果纳入 12 维度审计清单 |
...
> **派生面退役（2026-09-06 Owner 批二③+批四收尾）**：`dashboard.json`、
> `reconciliation-report.md`、`classified-assets.json`、`raw-asset-scan.json`
> 四个派生产物文件已 git rm（前三者零活消费方；raw-asset-scan 为同族管线产物、
> 内容停 2026-08-16 窄口径，MCP 资源声明同步移除）。下述 scan/classify/reconcile/
> dashboard 命令保留为手动善后工具（GATE-ASSET-INDEX post-commit 自动触发链已禁写，
> 正常流程不再产出四文件；手动运行产出物自行管理）；索引真源唯一写者 =
> `scripts/governance/generate_asset_index.py`。
