...
| 图 | 位置 | 状态 | 链接 |
|----|------|------|------|
| 依赖图 (depgraph) | `blueprint_id=MOD-POS-022` 的 1 个 file 节点 | design | `extract_depgraph.py --modules MOD-POS-022` |
...
**新 AI session 读取顺序**：
1. 读本蓝图 §1.1（核心原则：budget 是硬约束，策略不能拒绝）→ 理解三级升级的存在理由
2. 读 §3（三级升级规则）→ 理解 Tier 1/2/3 各自的触发、动作、性质
3. 读 §4（状态机）→ 理解 Tier 升级流转全路径
4. 读 §3.6（convergence_window）→ 理解按换手率差异化的等待时间
