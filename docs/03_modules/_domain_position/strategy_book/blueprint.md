...
| 图 | 位置 | 状态 | 链接 |
|----|------|------|------|
| 依赖图 (depgraph) | `blueprint_id=MOD-POS-020` 的 1 个 file 节点 | production | `extract_depgraph.py --modules MOD-POS-020` |
...
**新 AI session 读取顺序**：
1. 读本蓝图 §1.1（分层边界）→ 知道"StrategyBook 做什么、不做什么"
2. 读 §2（输入输出）→ 知道"和上下游的契约"
3. 读 §8（施工 Phase）→ 知道"下一步该做什么"
