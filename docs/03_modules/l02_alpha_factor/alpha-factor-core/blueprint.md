...
actual_disk_path: "src/zephyr/l02_alpha_factor/"
...
> actual_disk_path: src/zephyr/l02_alpha_factor/ (6 .py files)

# Alpha Factor Core 蓝图+施工图 — 因子计算引擎·OCP扩展点

> module_id: MOD-L02-001 | version: 3.0.0 | status: active | layer: l02_alpha_factor
> actual_disk_path: src/zephyr/l02_alpha_factor/ | generation: 2 | construction_progress: partially_implemented
...
|---|-------------|---------|---------|---------|------------|
| 1 | base.py(旧版FactorBase) | `src/zephyr/l02_alpha_factor/base.py` | 删除 | 标记deprecated→Phase2物理删除 | Grep全项目引用base.py→更新为factor_base.py |
...
| 3 | 导入源 | zephyr.l03_signal_generation.* | zephyr.l02_alpha_factor.* | 分层约束：L02不依赖L03 |
...
| MOD-INF-015 Telemetry | 可选 | 因子计算监控 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\system-telemetry\blueprint.md` |
| ALPHA-SIGNAL-DOMAIN-001 | 必须 | 因子→信号域集成 | — | `D:\ZephyrAlpha\docs\03_modules\_alpha-signal-domain\blueprint.md` |
| MOD-L11-001 ML Platform | 可选 | ModelPrediction因子增强 | — | `D:\ZephyrAlpha\docs\03_modules\l11_ml_platform\ml-core\blueprint.md` |
...
| 产出物类型 | 存放完整绝对路径 | 说明 |
|----------|---------------|------|
| 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l02_alpha_factor\alpha-factor-core\blueprint.md` | 本文件 |
| 业务代码 | `D:\ZephyrAlpha\src\zephyr\l02_alpha_factor\` | Python 源码 |
| 测试代码 | `D:\ZephyrAlpha\tests\l02_alpha_factor\` | 测试用例 |
...
| 3 | 代码文件头部 | `D:\ZephyrAlpha\src\zephyr\l02_alpha_factor\*.py` | [BLUEPRINT]指向MOD-L02-001 | 当前指向ALPHA-SIGNAL-DOMAIN-001，漂移 |
...
| 6 | FactorMeta使用@dataclass而非Pydantic BaseModel | 中 | 违反ADR-0040 | 迁移至Pydantic | 风险 |
...
|------|------|
| 对应蓝图契约 | §0.2 |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l02_alpha_factor\*.py` |
| 验收标准 | 所有.py文件[BLUEPRINT]字段指向MOD-L02-001 |
| 验证命令 | `grep -r "BLUEPRINT" src/zephyr/l02_alpha_factor/` |
| G7 检查项 | 所有6个.py文件已修正 |
...
|------|------|
| 对应蓝图契约 | §5.3 |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l02_alpha_factor\base.py` |
| 验收标准 | base.py标记deprecated，无其他模块引用 |
| 验证命令 | `grep -r "from.*base import\|from.*l02_alpha_factor.base" src/zephyr/` |
| G7 检查项 | 无外部引用base.py |
| AI 自治范围 | human_gated |
| 检查点 | grep无结果 |
...
|------|------|
| 对应蓝图契约 | §4.2 |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l02_alpha_factor\factor_base.py` |
| 验收标准 | FactorMeta继承BaseModel，所有字段有Field(description) |
| 验证命令 | `python -c "from zephyr.l02_alpha_factor.factor_base import FactorMeta; print(FactorMeta.model_fields)"` |
...
|---|:----:|------|----------|----------|----------|
| 1 | 命令 | `python -c "from zephyr.l02_alpha_factor import FactorRegistry; FactorRegistry.list_all()"` | 列出所有注册因子 | — | FactorMeta列表 |
| 2 | 命令 | `python -c "from zephyr.l02_alpha_factor import autodiscover_factors; autodiscover_factors()"` | 触发因子自动发现 | `package_path: 可选` | 无输出，副作用注册 |
...
| GAP-L02-002 | FactorMeta使用@dataclass | 迁移至Pydantic BaseModel | P1 | ADR-0040合规 | v3.0.0 | 待施工 |
...
| 4 | D-L02-04 | FactorMeta使用@dataclass(待迁移Pydantic) | @dataclass/Pydantic | @dataclass→Pydantic | ADR-0040强制Pydantic V2 | 2026-05-05 |
...
| 3 | FactorMeta使用@dataclass违反ADR-0040 | 中 | 初始实现未遵循Pydantic要求 | 迁移至Pydantic BaseModel | §4.2 | 待解决 |
...
|---|---------------|------------|---------|------------|
| 1 | base.py | `D:\ZephyrAlpha\src\zephyr\l02_alpha_factor\base.py` | 废弃型 | 标记deprecated→确认无引用→Phase2物理删除 |
...
|---|---------|------------|------|---------|
| 1 | factor_base.py | `D:\ZephyrAlpha\src\zephyr\l02_alpha_factor\factor_base.py` | 修改 | FactorMeta迁移Pydantic |
| 2 | base.py | `D:\ZephyrAlpha\src\zephyr\l02_alpha_factor\base.py` | 废弃 | 标记deprecated |
| 3 | factors/momentum_factor.py | `D:\ZephyrAlpha\src\zephyr\l02_alpha_factor\factors\momentum_factor.py` | 修改 | [BLUEPRINT]字段修正 |
| 4 | factors/value_factor.py | `D:\ZephyrAlpha\src\zephyr\l02_alpha_factor\factors\value_factor.py` | 修改 | [BLUEPRINT]字段修正 |
| 5 | __init__.py | `D:\ZephyrAlpha\src\zephyr\l02_alpha_factor\__init__.py` | 修改 | [BLUEPRINT]字段修正 |
| 6 | factors/__init__.py | `D:\ZephyrAlpha\src\zephyr\l02_alpha_factor\factors\__init__.py` | 修改 | [BLUEPRINT]字段修正 |
