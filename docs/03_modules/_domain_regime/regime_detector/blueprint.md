...
- **HMM 组件锚定**（裁定#304，2026-09-17 重校批）：fit() 后按训练窗特征统计把组件确定性重排到固定语义槽位
  （r4=斜率均值最小组件；r1/r2/r3=其余按波动率均值升序），跨 refit 态身份可比，
  label switching 结构性消除；锚定只用训练窗（PIT），不改似然/概率集合
  （依据：预注册协议 docs/_working/regime_recal/regime_recal_protocol_2026_09_17.md R1 +
  VAL-P0-20260916-230726 实证诊断——r4 名义熊市但 fwd20 两段均正、季符号一致率 0.571、
  相邻季翻转率 0.667；r1/r3 方向 IS→OOS 完全反转）。
  ⚠️ 锚定恢复标签语义稳定性，不恢复"态→前向收益方向"预测力（暴跌反弹溢价为市场结构
  性质）；方向判别职责归因子层，态层职责=风险分档（同 #229/#230 精神）。消费方禁把
  r4/r10 名义语义直译"看空"。
...
| 文件路径 | 实现状态 | 说明 |
|---------|:---:|------|
| `src/zephyr/regime/__init__.py` | ✅ 已实现 | |
| `src/zephyr/regime/core/__init__.py` | ✅ 已实现 | |
| `src/zephyr/regime/core/anchored_state_machine.py` | ✅ 已实现 | |
...
| `tests/regime/test_rb_stats_regime_failopen.py` | ✅ 已实现 | |
