...
actual_disk_path: "src/zephyr/l00_data_source/"
...
references:
  - path: "D:\\ZephyrAlpha\\docs\\02_enterprise_architecture\\target-architecture\\architecture-model\\layers\\l00-data-source.yaml"
    section: ""
    why: "架构层YAML真源"
depends_on:
  - target: MOD-INF-007
...
> actual_disk_path: src/zephyr/l00_data_source/ (6 .py files + 2 test files)

# Data Source Core 蓝图+施工图 — 数据接入层·C轨占位

> module_id: MOD-L00-001 | version: 3.0.0 | status: active | layer: l00_data_source
> actual_disk_path: src/zephyr/l00_data_source/ | generation: 3 | construction_progress: partially_implemented
...
| 验证项 | 验证方法 | 结果 |
|--------|---------|:---:|
| construction_progress = partially_implemented → 已实现章节的代码存在 | `ls src/zephyr/l00_data_source/` 逐文件核对 | ☐ |
...
> ⚠️ 代码实际使用 `pd.DataFrame` 而非 Pydantic BaseModel。这是 C轨占位阶段的实现选择，待正式施工时按 ADR-0040 迁移为 Pydantic V2。
...
| 4 | 导入源 | zephyr.l02_* / zephyr.l03_* | 仅允许被下游导入 | 分层约束：L00不依赖L02+ |
...
|--------|------|---------|---------|---------|
| l00_fetch_total | Counter | 自动埋点 | — | — |
| l00_fetch_error_total | Counter | 自动埋点 | >5%错误率 | P2 |
| l00_quality_pass_rate | Gauge | 质检统计 | <90% | P1 |
| l00_fetch_latency_seconds | Histogram | 计时 | P95>10s | P2 |
...
|---|---------|---------|------------|---------|
| 1 | 单元测试 | DataSourceBase | 注册机制+validate_schema | tests/unit/l00_data_source/test_provider_base_contract.py |
| 2 | 单元测试 | DataQualityGate | QualityReport+QualityFailureReason+is_within_normal_range+QUALITY_THRESHOLD | tests/unit/l00_data_source/test_quality_gate.py |
...
| MOD-INF-015 Telemetry | 必须 | 数据摄取监控 | — | `D:\ZephyrAlpha\docs\03_modules\l01_infrastructure\system-telemetry\blueprint.md` |
...
| 3 | §0 代码文件清单 ↔ 架构层YAML l00-data-source.yaml | 子模块映射 | 已确认不对齐(C轨占位) | YAML定义5子模块(connectors/normalizers/storage/cache/quality)，代码为provider_base/quality_gate/implementations/结构 |
...
| 产出物类型 | 存放完整绝对路径 | 说明 |
|----------|---------------|------|
| 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l00_data_source\datasource-core\blueprint.md` | 本文件 |
| 业务代码 | `D:\ZephyrAlpha\src\zephyr\l00_data_source\` | Python 源码 |
| 测试代码 | `D:\ZephyrAlpha\tests\unit\l00_data_source\` | 测试用例 |
...
| 2 | 架构层YAML | `D:\ZephyrAlpha\docs\02_enterprise_architecture\target-architecture\architecture-model\layers\l00-data-source.yaml` | 补充implementations/子目录文件 | 文件清单同步 |
...
|------|------|
| 对应蓝图契约 | §4.1 |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l00_data_source\implementations\akshare_provider.py` |
| 验收标准 | import成功，fetch_historical返回OHLCV DataFrame |
| 验证命令 | `python -c "from zephyr.l00_data_source.implementations.akshare_provider import AkshareProvider"` |
...
|------|------|
| 对应蓝图契约 | §4.1 |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l00_data_source\implementations\default_quality_gate.py` |
| 验收标准 | 校验规则可配置，check返回QualityReport |
| 验证命令 | `python -c "from zephyr.l00_data_source.implementations.default_quality_gate import DefaultQualityGate"` |
...
|---|:----:|------|----------|----------|----------|
| 1 | 命令 | `python -c "from zephyr.l00_data_source.provider_base import DataSourceBase"` | 验证DataSourceBase可导入 | — | 无报错 |
| 2 | 命令 | `python -c "from zephyr.l00_data_source.quality_gate import DataQualityGate"` | 验证DataQualityGate可导入 | — | 无报错 |
...
| GAP-L00-003 | DataFrame→Pydantic迁移 | 按ADR-0040迁移 | P2 | L02消费端要求 | v2.0.0 | 待施工 |
...
| 2 | D-L00-02 | 数据输出格式为DataFrame(非Pydantic) | dict/Pydantic/dataclass/DataFrame | DataFrame | C轨占位阶段DataFrame更灵活，待正式施工按ADR-0040迁移Pydantic | 2026-05-05 |
...
| 2 | DataFrame未按ADR-0040使用Pydantic | 中 | C轨占位阶段选择灵活实现 | 待v2.0.0迁移 | §5.1 #2 | 待解决 |
...
| 9 | 架构层YAML | — | — | `D:\ZephyrAlpha\docs\02_enterprise_architecture\target-architecture\architecture-model\layers\l00-data-source.yaml` | 子模块定义真源 |
...
|---|---------|------------|------|---------|
| 1 | provider_base.py | `D:\ZephyrAlpha\src\zephyr\l00_data_source\provider_base.py` | 读取 | 无变更 |
| 2 | quality_gate.py | `D:\ZephyrAlpha\src\zephyr\l00_data_source\quality_gate.py` | 读取 | 无变更 |
| 3 | implementations/ | `D:\ZephyrAlpha\src\zephyr\l00_data_source\implementations\` | 读取 | 无变更 |
| 4 | 测试代码 | `D:\ZephyrAlpha\tests\unit\l00_data_source\` | 读取 | 无变更 |
