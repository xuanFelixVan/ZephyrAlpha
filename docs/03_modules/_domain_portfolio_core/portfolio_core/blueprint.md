...
depends_on:
  - target: MOD-L03-001
    at: CTR-P1-015
    why: 消费 SynthesizedSignal（source_layer=L03）
  - target: MOD-L04-001
    at: CTR-003
    why: 消费 RiskLimits
references:
...
本蓝图描述 ZephyrAlpha 组合构建层——它解决了从信号到委托指令的转换问题。核心职责包括：策略注册与发现（StrategyBase OCP-002 扩展点）、目标权重生成（等权/信号加权/最小方差/风险平价）、订单列表生成（target_weights → 增量 Order）、风控约束应用。当前规模 1 种策略（DefaultEquityStrategy），目标容量 4 种策略 + 500 标的池。上游依赖 L03 SynthesizedSignal + L04 RiskLimits，下游被 L06 ExecutionBroker / L07 Analytics / L10 Compliance 消费。
...
>
> **source_layer 修正**：YAML 中 source_layer 原标注为 L02，实际信号来源为 L03（SynthesizedSignal）。本蓝图以 L03 为准。
>
> **ARB-20 裁定**：L05 产出 CTR-007（TargetPortfolio），非 CTR-004（Order）。L06-oms 将 CTR-007 转化为 CTR-004。当前代码 DefaultEquityStrategy.generate_target_weights() 返回 list[Order]（CTR-004），待 Phase 2 重构为返回 TargetPortfolio（CTR-007）。
...
ZephyrAlpha 量化架构需要从信号层（L03）和风控层（L04）的输出，转化为可执行的委托指令。当前无组合构建模块，信号和风控输出无法自动转化为交易指令。L05 组合构建层填补这一空白，作为信号→执行的桥梁。
...
| 3 | ✅ 包含 | 订单生成：目标权重 → 增量订单 | CTR-004 Order 产出至 L06（待重构为 CTR-007 TargetPortfolio） |
| 4 | ✅ 包含 | 策略生命周期：CTR-P1-006 StrategyLifecycleEvent | 事件产出至 L07/L10 |
| 5 | ❌ 排除 | 信号合成 | L03 职责 |
| 6 | ❌ 排除 | 风险限额计算 | L04 职责 |
| 7 | ❌ 排除 | 订单执行 | L06 职责 |
...
| L06 消费者 | CTR-007/CTR-004 接口兼容 | 集成 | 接口变更需通知 |
| L07/L10 消费者 | CTR-P1-006 事件格式 | 集成 | 事件变更需通知 |
...
| 信号加权调仓 | L03 信号更新 | SynthesizedSignal → 信号排序 → 加权分配 → 风控截断 → Order 列表 | CTR-004 Orders |
...
| 4 | ✅ 包含 | 风控约束应用 | 从 L04 RiskLimits 获取约束，确保目标权重不违反限额 | 本模块 |
| 5 | ❌ 排除 | 信号合成 | L03（SynthesizedSignal） | L03 |
| 6 | ❌ 排除 | 风险限额计算 | L04（RiskLimits） | L04 |
| 7 | ❌ 排除 | 订单执行 | L06（ExecutionBroker） | L06 |
| 8 | ❌ 排除 | 策略回测 | 独立回测模块 | L09 |
...
| # | 上游 | 处理逻辑 | 下游 | 数据格式 |
|---|--------|---------|---------|---------|
| 1 | L03 SynthesizedSignal + L04 RiskLimits | StrategyBase.generate_target_weights() → target_weights / Order 列表 | 内部 / L06 | Pydantic Model |
| 2 | target_weights | DefaultEquityStrategy._weights_to_orders() → Order 列表 | L06 ExecutionBroker | CTR-004 Order |
| 3 | 策略状态变更 | StrategyLifecycleEvent 产出 | L07 分析 / L10 合规 | CTR-P1-006 |

> **ARB-20 修正**：终局数据流应为 L05 → CTR-007 TargetPortfolio → L06-oms 转换为 CTR-004 Order。当前代码直接产出 CTR-004 Order，待 Phase 2 重构。
...
| Discovered | 策略实例化 | Active | 依赖的 L03/L04 契约可用 |
...
| 契约 ID | 名称 | 来源层 | 用途 |
|---------|------|--------|------|
| CTR-P1-015 | SynthesizedSignal | **L03** | 信号加权策略输入（source_layer=L03 非 L02） |
| CTR-003 | RiskLimits | L04 | 风控约束（max_single_position 等） |
| CTR-ERR-004 | RiskLimitViolationError | L04 | 风控硬错误 |
| CTR-002 | FactorSignal | L02 | 因子依赖 |
| CTR-ERR-003 | SignalDegradationWarning | L03 | 信号降级 |
| CTR-ERR-005 | ExecutionRejectionError | L06 | 执行拒绝 |
| CTR-P1-003 | CapitalAllocationResult | L03 | 资金分配 |
| CTR-P1-011 | RiskMetricsReport | L04 | 风险指标 |
| CTR-P1-010 | SystemConfiguration | L01 | 系统配置 |
| CTR-P1-013 | TelemetryEmitter | L12 | 遥测 |

### 4.4 输出契约

| 契约 ID | 名称 | 目标层 | 用途 | ARB-20 修正 |
|---------|------|--------|------|------------|
| CTR-004 | Order | L06 | 委托指令（当前实现） | 终局应为 CTR-007 TargetPortfolio |
| CTR-P1-006 | StrategyLifecycleEvent | L07, L10 | 策略生命周期事件 | — |
...
| 修改 CTR-004 Order 结构 | ❌ 破坏性 | 需 Owner 审批 + L06 同步更新 |
| 新增 CTR-P1-006 事件类型 | ✅ 向后兼容 | 不影响已有消费者 |
| CTR-004 → CTR-007 重构 | ❌ 破坏性 | 需 Owner 审批 + L06 同步更新（ARB-20） |
...
| 2 | 风控约束 MUST 从 L04 RiskLimits 获取 | L04 RiskLimits |
...
| # | 类型 | 禁止项 | 替代/允许项 | 原因 |
|---|:----:|--------|-----------|------|
| 1 | 编码模式 | 策略内部硬编码风控限额 | 从 L04 RiskLimits 获取 | 策略不可自行放宽限额 |
...
| 2 | 风控约束违反（权重超限） | CTR-ERR-004 RiskLimitViolationError | 截断权重至限额内 + 记录告警 | L06 订单执行 |
| 3 | 信号降级 | CTR-ERR-003 SignalDegradationWarning | 降级为等权策略 + 记录遥测 | 权重质量 |
| 4 | 执行拒绝 | CTR-ERR-005 ExecutionRejectionError | 重新计算权重（排除被拒标的） | L06 订单执行 |
| 5 | 策略依赖的 L03/L04 契约不可用 | 导入/调用失败 | 抛出明确异常，策略保持 Degraded 状态 | 策略生命周期 |
...
|------|-------------|-----------|---------|---------|
| L03 SynthesizedSignal | 等权策略 | 信号加权策略 | 降级为等权 | L03 恢复健康 |
| L04 RiskLimits | 无风控约束的权重 | 风控截断 | 策略保持 Degraded，禁止下单 | L04 恢复健康 |
...
| # | 威胁 | 影响 | 缓解措施 | 验证方式 |
|---|------|------|---------|---------|
| 1 | 策略自行放宽风控限额 | 高 | 风控约束 MUST 从 L04 RiskLimits 获取，禁止策略内部硬编码或放宽限额 | 代码审查 + 单元测试验证 RiskLimits 来源 |
...
| 3 | 集成测试 | L04 风控约束 → L05 权重生成 → L06 订单 | RiskLimits 约束传递；Order 结构符合 CTR-004；幂等键传递 | 端到端通过 |
...
|------------|---------|--------|---------|:---:|
| L06 交易执行 | 新增接口 | CTR-004 Order → SimulationBroker 消费（终局 CTR-007 → L06-oms 转换） | 端到端测试：权重→订单→执行 | 部分集成 |
| L04 风控约束 | 修改现有接口 | DefaultEquityStrategy 消费 RiskLimits | 单元测试验证约束传递 | 部分集成 |
| L07 分析 | 事件订阅 | CTR-P1-006 StrategyLifecycleEvent | 集成测试验证事件产出 | 待集成 |
| L10 合规 | 事件订阅 | CTR-P1-006 StrategyLifecycleEvent | 集成测试验证事件产出 | 待集成 |
...
| 2 | source_layer 标注错误（L02 → 应为 L03） | 高 | 中 | 本蓝图已修正，YAML 待同步 | 风险 |
| 3 | YAML 文件路径与磁盘不一致 | 中 | 低 | 以磁盘为准，YAML 待同步 | 风险 |
| 4 | 策略注册冲突 | 低 | 中 | StrategyRegistry.register 检测重复 strategy_id 并抛出 ValueError | 风险 |
| 5 | CTR-004 → CTR-007 重构影响 L06 | — | 高 | 需 Owner 审批 + L06 同步更新（ARB-20） | 负面后果 |
| 6 | L04 不可用时策略无法运行 | — | 高 | 策略保持 Degraded 状态 | 负面后果 |
...
| G7 检查项 | 上游 L03/L04 契约引用已列出；下游 CTR-004 Order 产出路径精确；回滚方案可执行 |
...
| G7 检查项 | L03 SynthesizedSignal 输入正确；L04 RiskLimits 约束应用；CTR-004 Order 产出格式正确 |
...
| 验收标准 | generate_target_weights() 返回 TargetPortfolio(CTR-007) 而非 list[Order]；L06-oms 负责转换 |
...
| 5 | StrategyLifecycleEvent 结构不符合 L07/L10 预期 | 删除事件文件，恢复策略为不产出事件状态 |
| 6 | CTR-007 重构导致 L06 不兼容 | 回退到 CTR-004 输出，L05 直接产出 Order |
| 7 | L06 集成测试失败 | 回退到步骤 3 状态，L05 仅产出 Order 不产出事件 |
...
| 2 | 运行 | L04 不可用 | RiskLimits 导入失败 | 检查 L04 模块健康 | 策略保持 Degraded | L04 恢复 |
| 3 | 运行 | 信号降级 | CTR-ERR-003 | 检查 L03 信号质量 | 降级为等权 | L03 恢复 |
...
| TargetPortfolio | CTR-007，L05 终局产出格式 | Order (CTR-004) | TargetPortfolio 是目标组合，Order 是委托指令；L06-oms 负责转换 |
...
| 场景 | 判定 | 理由 |
|------|------|------|
| L05 新增风险平价策略 | 原地升级 | 同属组合构建 |
| L05 新增回测引擎 | 拆分独立蓝图 | 回测≠组合构建 |
| L05 新增订单执行逻辑 | 拆分独立蓝图 | L06 已覆盖 |
