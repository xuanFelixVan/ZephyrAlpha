...
> module_id: MOD-L00-001 | version: 4.0.6 | status: active | domain: data
...
本蓝图描述 ZephyrAlpha 数据接入层——它是业务数据库母蓝图([MOD-ARCH_BIZDB](file:///d:/ZephyrAlpha/docs/03_modules/_cross_layer/database/business_data_architecture.md))的**上游**，为 C1/C2/C3 仓库提供原料数据，解决外部数据源格式各异、API限流、数据质量参差不齐的标准化接入问题。
...
> 查询最新文件清单：`python scripts/governance/extract_depgraph.py --modules MOD-L00-001`
...
> 生成命令：`python scripts/governance/d5_architecture/generators/generate_blueprint_panorama.py MOD-L00-001`

#### 全景位置

| 图 | 位置 | 状态 | 链接 |
|----|------|------|------|
| 依赖图 (depgraph) | `blueprint_id=MOD-L00-001` 的 15 个 file 节点 | production | `extract_depgraph.py --modules MOD-L00-001` |
...
ZephyrAlpha 业务数据库母蓝图(MOD-ARCH_BIZDB §5)定义了 **69 个数据品类全景**：行情数据(10)、基本面(10)、另类(10)、宏观(11)、因子值(5大类)、技术指标(1组)、图形识别(6类)、信号历史(1)、主力行为(1)、板块强度(1)、知识图谱(5类)、回测结果(2)、交易事务(3)、补充品类(9)。数据源接入层作为业务数据库的**上游**，需为 C1/C2/C3 仓库提供这 69 品类的原料数据摄取能力。
...
| 8 | ❌ 排除 | 数据存储(C1~C4仓库) | 母蓝图 MOD-ARCH_BIZDB 负责 |
...
| 母蓝图 MOD-ARCH_BIZDB | 品类全景/插拔机制/调度策略上游对齐 | 设计 | 数据源接入层为其上游 |
...
| 7 | ❌ 排除 | 数据持久化(C1~C4仓库) | 母蓝图 MOD-ARCH_BIZDB 负责 | 母蓝图 |
...
> 🚧 v4.0.0 重建施工指引——对接母蓝图 MOD-ARCH_BIZDB Spiral 开发顺序。抽象层+实现层已重建，多品类扩展(对接母蓝图§6插拔机制)为未来 Spiral 工作。
...
| 3 | 母蓝图 MOD-ARCH_BIZDB 品类注册表 | soft | 设计完成 | ☐ |
...
| 验收标准 | AkshareIngestProvider/DefaultQualityGate/MemoryProvider 可导入并实例化 |
| 验证命令 | `python -c "from zephyr.data.implementations.akshare_provider import AkshareIngestProvider"` |
| 状态 | ✅ 已重建 |
...
| 母蓝图 | 业务数据库顶层架构设计书(MOD-ARCH_BIZDB) | — | 本数据源接入层的上游设计真源 |
...
| 10 | 业务数据库母蓝图 | MOD-ARCH_BIZDB | 1.0.0 | `D:\ZephyrAlpha\docs\03_modules\_cross_layer\database\business_data_architecture.md` | 上游设计真源(§5品类/§6插拔/§7调度/§8边界) |
...
| 文件路径 | 实现状态 | 说明 |
|---------|:---:|------|
| `src/zephyr/data/news_collector.py` | ✅ 已实现 | |
| `src/zephyr/data/satellite_geospatial_engine/__init__.py` | ✅ 已实现 | |
| `src/zephyr/data/tick_subscriber.py` | ✅ 已实现 | |
...
| `src/zephyr/governance/data_governance/akshare_quote_provider.py` | ✅ 已实现 | |
...
| 69品类全景/插拔机制/调度策略 | **母蓝图 MOD-ARCH_BIZDB** | 本蓝图（上游对接视图） |
