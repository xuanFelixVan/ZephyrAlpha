...
actual_disk_path: "src/zephyr/l07_post_trade_analytics/"
...
references:
  - path: "D:\\ZephyrAlpha\\docs\\02_enterprise_architecture\\target-architecture\\architecture-model\\layers\\l07_post_trade_analytics.yaml"
    section: ""
    why: "架构层YAML真源"
...
> module_id: MOD-L07-001 | version: 2.1.0 | status: active | layer: l07_post_trade_analytics
> actual_disk_path: src/zephyr/l07_post_trade_analytics/ | generation: 2 | construction_progress: not_started
...
| 4 | tests/l07_post_trade_analytics/ | §9 | 测试用例 | 已阻塞 | C轨未开放，测试目录未创建 |

### §0.2 对齐验证矩阵

| 验证项 | 验证方法 | 结果 |
|--------|---------|:---:|
| construction_progress = not_started → 代码目录存在但业务逻辑为骨架 | `ls src/zephyr/l07_post_trade_analytics/` | ☐ |
...
> ⚠️ 当前代码使用 `@dataclass(frozen=True)`（codegen 自动生成），非 Pydantic BaseModel。待 ADR-0040 全面落地后迁移。

```python
@dataclass(frozen=True)
class ExecutionReport:
    """执行报告 (CTR-P1-007) — SSoT: cross-layer-contracts.yaml"""
    order_id: str
    symbol: str
    direction: str
...
class PerformanceAttributionReport:
    """归因报告 (CTR-P1-009) — SSoT: cross-layer-contracts.yaml"""
    portfolio_id: str
    period_start: str
    period_end: str
    total_return: float
...
| 3 | 导入源 | zephyr.l06_trade_execution.* | zephyr.shared.contracts.execution.* | 分层约束，通过契约解耦 |
...
> ⚠️ 测试目录 `tests/l07_post_trade_analytics/` 尚未创建，C轨开放后 MUST 创建。
...
|---------|---------|---------|---------|---------|
| MOD-L06-001 Trade Execution | 必须 | CTR-005 Fill + CTR-004 Order | — | `D:\ZephyrAlpha\docs\03_modules\l06_trade_execution\execution-core\blueprint.md` |
| MOD-L05-001 Portfolio Construction | 必须 | CTR-006 PositionSnapshot | — | `D:\ZephyrAlpha\docs\03_modules\l05_portfolio_construction\portfolio-core\blueprint.md` |
| MOD-L04-001 Risk Management | 可选 | CTR-P1-011 RiskMetricsReport | — | `D:\ZephyrAlpha\docs\03_modules\l04_risk_management\risk-management-core\blueprint.md` |
| MOD-L02-001 Alpha Factor | 可选 | CTR-P1-001 FactorMonitorReport | — | `D:\ZephyrAlpha\docs\03_modules\l02_alpha_factor\alpha-factor-core\blueprint.md` |

### 10.2 依赖图对齐声明

| # | 对齐项 | 对齐方式 | 对齐状态 | 验证命令 |
|---|--------|---------|:-------:|---------|
| 1 | §10.1 依赖声明 ↔ system-dependency-map.md §5 MOD-L07-001 行 | 逐项核对 | 已对齐 | `python scripts/governance/d5_architecture/validators/validate_path_alignment.py --blueprint MOD-L07-001` |
...
| 产出物类型 | 存放完整绝对路径 | 说明 |
|----------|---------------|------|
| 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l07_post_trade_analytics\analytics-core\blueprint.md` | 本文件 |
| 业务代码 | `D:\ZephyrAlpha\src\zephyr\l07_post_trade_analytics\` | Python 源码 |
| 测试代码 | `D:\ZephyrAlpha\tests\l07_post_trade_analytics\` | 测试用例（待创建） |
...
|---|------------|------------|---------|---------|
| 1 | 蓝图注册表 | `D:\ZephyrAlpha\docs\03_modules\blueprint-registry.yaml` | construction_progress 更新 | 进度变更 |
| 2 | 架构层 YAML | `D:\ZephyrAlpha\docs\02_enterprise_architecture\target-architecture\architecture-model\layers\l07_post_trade_analytics.yaml` | 确认 files 列表与磁盘一致 | 文件清单同步 |
...
|------|------|
| 对应蓝图契约 | §4.1 |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l07_post_trade_analytics\implementations\default_tca_engine.py` |
| 验收标准 | import 成功，slippage_bps 计算使用 Decimal |
| 验证命令 | `python -c "from zephyr.l07_post_trade_analytics.implementations.default_tca_engine import DefaultTCAEngine"` |
...
|-----------|--------|----------|------------|
| MOD-L07-001 | default_tca_engine.py | code | `D:\ZephyrAlpha\src\zephyr\l07_post_trade_analytics\implementations\default_tca_engine.py` |
...
|------|------|
| 对应蓝图契约 | §4.1 |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l07_post_trade_analytics\implementations\default_attribution_engine.py` |
| 验收标准 | import 成功，Brinson 三因子分解正确 |
| 验证命令 | `python -c "from zephyr.l07_post_trade_analytics.implementations.default_attribution_engine import DefaultAttributionEngine"` |
| G7 检查项 | 上游 analytics_base.py 存在，下游 L08/L10 可消费 |
| AI 自治范围 | ai_modifiable |
...
| module_id | 文件名 | doc_type | 完整绝对路径 |
|-----------|--------|----------|------------|
| MOD-L07-001 | default_attribution_engine.py | code | `D:\ZephyrAlpha\src\zephyr\l07_post_trade_analytics\implementations\default_attribution_engine.py` |
...
|------|------|
| 对应蓝图契约 | §4.1 + §9 |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l07_post_trade_analytics\implementations\` + `D:\ZephyrAlpha\tests\l07_post_trade_analytics\` |
| 验收标准 | PerformanceAttributionReport 可被 L08/L10 消费，测试通过 |
| 验证命令 | `python -m pytest tests/l07_post_trade_analytics/ -k attribution -v` |
...
|---|:----:|------|----------|----------|----------|
| 1 | 命令 | `python -c "from zephyr.l07_post_trade_analytics import TCAEngineBase"` | 验证 import | — | exit 0 |
...
| TestSuite | GAP-L07-002 | tests/l07_post_trade_analytics/ | Phase 3 | 待施工 |
...
|---|------|-----------|------|------------|----------|
| 1 | 元数据注册表 | PS-STD-001 | 当前版本 | `D:\ZephyrAlpha\docs\01_policies_and_standards\meta\metadata-registry.md` | 编号规则、doc_type 词表 |
| 2 | 目录结构标准 | GOV-DOC-002 | 当前版本 | `D:\ZephyrAlpha\docs\01_policies_and_standards\governance\document\directory-structure-standard.md` | 路径映射 |
| 3 | 治理方法论 | PS-STD-011 | 当前版本 | `D:\ZephyrAlpha\docs\01_policies_and_standards\meta\governance-methodology-standard.md` | MTH-012/MTH-013 |
...
| 8 | AI 自治权限注册表 | GOV-AI-001 | 当前版本 | `D:\ZephyrAlpha\docs\01_policies_and_standards\_registry\catalogs\ai-autonomy-authority-registry.md` | AI 操作权限 |
...
|---|---------|------------|------|---------|
| 1 | analytics_base.py | `D:\ZephyrAlpha\src\zephyr\l07_post_trade_analytics\analytics_base.py` | 读取 | 无变更 |
| 2 | implementations/ | `D:\ZephyrAlpha\src\zephyr\l07_post_trade_analytics\implementations\` | 修改 | 完善实现（C轨开放后） |
