...
actual_disk_path: "src/zephyr/l06_trade_execution/"
...
ssot_yaml: "architecture-model/layers/l06_trade_execution.yaml"
...
priority: P0
depends_on:
  - target: "architecture-model/layers/l06_trade_execution.yaml"
    at: "全篇"
    why: "YAML SSoT——本蓝图真源"
  - target: "MOD-L05-001"
    at: "CTR-004"
...
references:
  - path: "D:\\ZephyrAlpha\\architecture-model\\layers\\l06_trade_execution.yaml"
    section: "全篇"
    why: "YAML SSoT"
---

> module_id: MOD-L06-001 | version: 2.1.0 | status: Active | layer: l06
> actual_disk_path: src/zephyr/l06_trade_execution/ | generation: 2 | construction_progress: partially_implemented
...
|---|-------------|---------|---------|---------|------------|
| 1 | DefaultRiskValidator 硬编码依赖 | `D:\ZephyrAlpha\src\zephyr\l06_trade_execution\execution_engine.py` | 构造函数注入 RiskValidator ABC | 迁移+解耦 | Grep `DefaultRiskValidator` 引用 → 改为 RiskValidator ABC |
...
| 4 | 导入源 | `from zephyr.l04_risk_management.implementations.*` | `from zephyr.l04_risk_management.contracts.*` | 分层约束——L06 不依赖 L04 实现 |
...
|--------|------|---------|---------|---------|
| l06_orders_total | Counter | 自动埋点 | — | — |
| l06_orders_rejected_total | Counter | 自动埋点 | >10/min | P1 |
| l06_fill_latency_ms | Histogram | 自动埋点 | P95>200ms | P2 |
| l06_broker_score | Gauge | 手动上报 | <0.5 | P2 |
| l06_position_snapshot_staleness_ms | Gauge | 自动埋点 | >5000ms | P1 |
...
|---------|---------|---------|---------|---------|
| MOD-L05-001 | 必须 | CTR-004 Order | v1.0.0+ | `D:\ZephyrAlpha\docs\03_modules\l05_portfolio_construction\portfolio-core\blueprint.md` |
| MOD-L04-001 | 必须 | CTR-ERR-004 RiskLimitViolationError | v1.0.0+ | `D:\ZephyrAlpha\docs\03_modules\l04_risk_management\risk-core\blueprint.md` |
| MOD-L07-001 | 可选 | CTR-005 Fill / CTR-P1-007 ExecutionReport 消费 | v1.0.0+ | `D:\ZephyrAlpha\docs\03_modules\l07_analytics\analytics-core\blueprint.md` |
| MOD-L11-001 | 可选 | CTR-006 PositionSnapshot 消费 | v1.0.0+ | `D:\ZephyrAlpha\docs\03_modules\l11_ml_platform\ml-core\blueprint.md` |
...
| 产出物类型 | 存放完整绝对路径 | 说明 |
|----------|---------------|------|
| 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l06_trade_execution\execution-core\blueprint.md` | 本文件（含设计和施工指引） |
| 业务代码 | `D:\ZephyrAlpha\src\zephyr\l06_trade_execution\` | Python 源码 |
| 券商适配器 | `D:\ZephyrAlpha\src\zephyr\l06_trade_execution\adapters\` | 适配器实现 |
| 契约 SSoT | `D:\ZephyrAlpha\src\zephyr\shared\contracts\execution\` | 契约数据模型 |
| 测试代码 | `D:\ZephyrAlpha\tests\unit\l06_trade_execution\` | 测试用例 |
...
|---|------------|------------|---------|---------|
| 1 | l06_trade_execution.yaml | `D:\ZephyrAlpha\architecture-model\layers\l06_trade_execution.yaml` | files 列表应反映 `adapters/` 子目录结构 | YAML SSoT 与磁盘不一致 |
...
| 3 | cross-layer-contracts.yaml | `D:\ZephyrAlpha\src\zephyr\shared\contracts\cross-layer-contracts.yaml` | 确认 CTR-005/CTR-006/CTR-ERR-005/CTR-P1-007 状态 | 契约状态同步 |
...
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l06_trade_execution\broker_interface.py` |
| 验收标准 | BrokerInterface ABC 定义完整，submit_order/cancel_order 抽象方法 |
| 验证命令 | `python -m pytest tests/unit/l06_trade_execution/ -k broker_interface -v` |
...
|------|------|
| 对应蓝图契约 | §3.3 状态生命周期 |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l06_trade_execution\order_manager.py` |
| 验收标准 | VALID_TRANSITIONS 全部实现，非法转换抛异常 |
| 验证命令 | `python -m pytest tests/unit/l06_trade_execution/ -k order_manager -v` |
...
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l06_trade_execution\execution_engine.py` |
| 验收标准 | TWAP/VWAP/冰山单算法实现，SOR 评分机制实现 |
| 验证命令 | `python -m pytest tests/unit/l06_trade_execution/ -k execution_engine -v` |
...
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l06_trade_execution\adapters\simulation_broker.py` |
| 验收标准 | 模拟成交 + 滑点 + 佣金 + 持仓维护 |
| 验证命令 | `python -m pytest tests/unit/l06_trade_execution/ -k simulation_broker -v` |
...
| 验证命令 | `python -m pytest tests/unit/l06_trade_execution/ -k risk_validator -v` |
...
| 验证命令 | `python -m pytest tests/unit/l06_trade_execution/ -k execution_report -v` |
...
|------|------|
| 对应蓝图契约 | §4.1 BrokerInterface 子类 |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l06_trade_execution\adapters\` |
| 验收标准 | 继承 BrokerInterface，实现 submit_order/cancel_order |
| 验证命令 | `python -m pytest tests/unit/l06_trade_execution/ -k broker_adapter -v` |
...
|------|------|
| 对应蓝图契约 | §12 集成目标 |
| 产出位置 | `D:\ZephyrAlpha\tests\unit\l06_trade_execution\` |
| 验收标准 | L04 风控 + L07 分析端到端测试通过 |
| 验证命令 | `python -m pytest tests/unit/l06_trade_execution/ -k integration -v` |
...
|---|:----:|------|----------|----------|----------|
| 1 | 命令 | `python -m pytest tests/unit/l06_trade_execution/ -v` | 运行 L06 全部测试 | — | exit 0 = 通过 |
...
| 9 | YAML SSoT | — | — | `D:\ZephyrAlpha\architecture-model\layers\l06_trade_execution.yaml` | 本蓝图真源 |
...
|---|---------|------------|------|---------|
| 1 | broker_interface.py | `D:\ZephyrAlpha\src\zephyr\l06_trade_execution\broker_interface.py` | 修改 | RiskValidator ABC 注入解耦 |
| 2 | execution_engine.py | `D:\ZephyrAlpha\src\zephyr\l06_trade_execution\execution_engine.py` | 修改 | ExecutionReport 产出 + RiskValidator 解耦 |
| 3 | order_manager.py | `D:\ZephyrAlpha\src\zephyr\l06_trade_execution\order_manager.py` | 读取 | 订单状态机参考 |
| 4 | adapters/simulation_broker.py | `D:\ZephyrAlpha\src\zephyr\l06_trade_execution\adapters\simulation_broker.py` | 读取 | 模拟券商参考 |
| 5 | adapters/__init__.py | `D:\ZephyrAlpha\src\zephyr\l06_trade_execution\adapters\__init__.py` | 修改 | 新增适配器注册 |
| 6 | __init__.py | `D:\ZephyrAlpha\src\zephyr\l06_trade_execution\__init__.py` | 修改 | CTR 声明更新 |
...
| Tier | 消费者 | 依赖内容 |
|:----:|--------|---------|
| Tier 1 | `D:\ZephyrAlpha\docs\03_modules\l07_analytics\analytics-core\blueprint.md` | §4 接口契约（CTR-005 Fill, CTR-P1-007 ExecutionReport） |
| Tier 1 | `D:\ZephyrAlpha\docs\03_modules\l04_risk_management\risk-core\blueprint.md` | §4 接口契约（CTR-006 PositionSnapshot） |
| Tier 2 | `D:\ZephyrAlpha\docs\03_modules\l11_ml_platform\ml-core\blueprint.md` | §4 接口契约（CTR-006 PositionSnapshot） |
| Tier 3 | `D:\ZephyrAlpha\src\zephyr\l06_trade_execution\` | §4 数据模型、§11 产出物路径 |
