...
actual_disk_path: "src/zephyr/l05_portfolio_construction/"
...
> actual_disk_path: src/zephyr/l05_portfolio_construction/ | generation: 2 | construction_progress: partially_implemented
...
| 6 | `strategies/min_variance_strategy.py` | §3.1 | 最小方差策略 | 已阻塞 | C轨blocked |
| 7 | `strategies/risk_parity_strategy.py` | §3.1 | 风险平价策略 | 已阻塞 | C轨blocked |
| 8 | `shared/contracts/portfolio/strategy_lifecycle.py` | §4.4 | CTR-P1-006 StrategyLifecycleEvent | 已阻塞 | C轨blocked |
...
| 蓝图描述的类/函数名 = 代码中的类/函数名 | `grep "class\|def" D:\ZephyrAlpha\src\zephyr\l05_portfolio_construction\*.py` | ✅ |
...
> 强制 **Pydantic V2 BaseModel**（ADR-0040），禁止 `@dataclass`。当前 StrategyMeta 使用 @dataclass(frozen=True)，待重构为 Pydantic BaseModel。
...
| 5 | ADR-0040：数据模型强制 Pydantic V2 BaseModel | StrategyMeta 当前为 @dataclass，待重构 |
...
| 3 | 导入源 | `from zephyr.l06_* import *` | 通过 CTR-004 契约交互 | 分层约束 |
| 4 | 数据模型 | `@dataclass` 用于新数据模型 | Pydantic V2 BaseModel | ADR-0040 |
...
|--------|------|---------|---------|---------|
| l05_strategy_registry_count | Gauge | StrategyRegistry.count() | <1（无策略注册） | P2 |
| l05_weight_calc_duration_ms | Histogram | generate_target_weights() 计时 | P95>50ms | P2 |
| l05_order_gen_duration_ms | Histogram | _weights_to_orders() 计时 | P95>20ms | P3 |
| l05_risk_truncation_total | Counter | 风控截断事件 | >10次/调仓周期 | P1 |
...
> **测试路径**：`D:\ZephyrAlpha\tests\unit\l05_portfolio_construction\`（非蓝图原写的 `tests/l05_portfolio_construction/`）
...
|---------|---------|---------|---------|---------|
| MOD-L03-001 | 必须 | CTR-P1-015 SynthesizedSignal, CTR-P1-003 CapitalAllocationResult, CTR-ERR-003 | — | `D:\ZephyrAlpha\docs\03_modules\l03_signal_synthesis\signal-core\blueprint.md` |
| MOD-L04-001 | 必须 | CTR-003 RiskLimits, CTR-ERR-004, CTR-P1-011 | — | `D:\ZephyrAlpha\docs\03_modules\l04_risk_management\risk-core\blueprint.md` |
| MOD-L02-001 | 可选 | CTR-002 FactorSignal | — | `D:\ZephyrAlpha\docs\03_modules\l02_factor_processing\factor-core\blueprint.md` |
| MOD-L06-001 | 下游 | CTR-004 Order 消费方（终局 CTR-007 TargetPortfolio） | — | `D:\ZephyrAlpha\docs\03_modules\l06_trade_execution\execution-core\blueprint.md` |
| MOD-L07-001 | 下游 | CTR-P1-006 StrategyLifecycleEvent 消费方 | — | `D:\ZephyrAlpha\docs\03_modules\l07_analytics\analytics-core\blueprint.md` |
| MOD-L10-001 | 下游 | CTR-P1-006 StrategyLifecycleEvent 消费方 | — | `D:\ZephyrAlpha\docs\03_modules\l10_compliance\compliance-core\blueprint.md` |
...
| 产出物类型 | 存放完整绝对路径 | 说明 |
|----------|---------------|------|
| 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l05_portfolio_construction\portfolio-core\blueprint.md` | 本文件（含设计和施工指引） |
| 接口定义 | `D:\ZephyrAlpha\src\zephyr\l05_portfolio_construction\strategy_base.py` | StrategyBase + StrategyMeta + StrategyRegistry |
| 注册表卫星 | `D:\ZephyrAlpha\src\zephyr\l05_portfolio_construction\strategy_registry.py` | re-export 卫星模块 |
| 策略实现 | `D:\ZephyrAlpha\src\zephyr\l05_portfolio_construction\strategies\` | 策略子目录 |
| 契约 SSoT | `D:\ZephyrAlpha\src\zephyr\shared\contracts\execution\order.py` | CTR-004 Order |
| 测试代码 | `D:\ZephyrAlpha\tests\unit\l05_portfolio_construction\` | 测试用例 |
...
| 3 | cross-layer-contracts.yaml | `D:\ZephyrAlpha\architecture-model\cross-layer-contracts.yaml` | 确认 CTR-004/CTR-007/CTR-P1-006 状态 | 契约状态确认 |
...
|------|------|
| 对应蓝图契约 | §4.1 公共 API |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l05_portfolio_construction\strategy_base.py` |
| 验收标准 | StrategyBase 抽象类定义 generate_target_weights()；StrategyMeta 包含 strategy_id / name / strategy_type / version / description |
| 验证命令 | `python -m pytest D:\ZephyrAlpha\tests\unit\l05_portfolio_construction\ -k test_strategy_base -v` |
...
|------|------|
| 对应蓝图契约 | §4.1 公共 API |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l05_portfolio_construction\strategy_registry.py` |
| 验收标准 | register 装饰器、get()、list_all()、count()、clear()、autodiscover_strategies() 均可正常工作；重复注册抛出 ValueError |
| 验证命令 | `python -m pytest D:\ZephyrAlpha\tests\unit\l05_portfolio_construction\ -k test_strategy_registry -v` |
...
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l05_portfolio_construction\strategies\default_equity_strategy.py` |
| 验收标准 | 等权模式：权重均匀分配；信号加权模式：按 SynthesizedSignal 加权；风控约束截断生效 |
| 验证命令 | `python -m pytest D:\ZephyrAlpha\tests\unit\l05_portfolio_construction\ -k test_default_equity -v` |
...
|------|------|
| 对应蓝图契约 | §4.1 公共 API |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l05_portfolio_construction\strategies\` |
| 验收标准 | MinVarianceStrategy + RiskParityStrategy 继承 StrategyBase 并注册；权重计算正确 |
| 验证命令 | `python -m pytest D:\ZephyrAlpha\tests\unit\l05_portfolio_construction\ -k test_min_variance -v` |
...
| 验证命令 | `python -m pytest D:\ZephyrAlpha\tests\unit\l05_portfolio_construction\ -k test_lifecycle_event -v` |
...
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l05_portfolio_construction\` |
| 验收标准 | generate_target_weights() 返回 TargetPortfolio(CTR-007) 而非 list[Order]；L06-oms 负责转换 |
| 验证命令 | `python -m pytest D:\ZephyrAlpha\tests\unit\l05_portfolio_construction\ -k test_target_portfolio -v` |
...
|------|------|
| 对应蓝图契约 | §12 集成目标 |
| 产出位置 | `D:\ZephyrAlpha\tests\unit\l05_portfolio_construction\test_l06_integration.py` |
| 验收标准 | L05 → L06 端到端：权重生成 → 订单生成 → SimulationBroker 消费 |
| 验证命令 | `python -m pytest D:\ZephyrAlpha\tests\unit\l05_portfolio_construction\test_l06_integration.py -v` |
| G7 检查项 | CTR-004 Order 格式与 L06 消费方一致；幂等键传递；风控约束传递 |
...
| G7 检查项 | YAML 与蓝图一致；YAML 与磁盘一致；cross-layer-contracts.yaml 状态同步 |
...
|---|:----:|------|----------|----------|----------|
| 1 | 命令 | `python -m pytest tests/unit/l05_portfolio_construction/ -v` | 运行 L05 全部单元测试 | — | 12 tests passed |
| 2 | 命令 | `python -c "from zephyr.l05_portfolio_construction.strategy_base import autodiscover_strategies; autodiscover_strategies()"` | 策略自动发现 | `package_path`: 默认 `zephyr.l05_portfolio_construction.strategies` | 发现策略数 |
...
| 2 | D-L05-02 | StrategyMeta 使用 @dataclass | A: Pydantic BaseModel / B: @dataclass(frozen=True) | B | frozen 语义简单，Phase 1 快速实现；ADR-0040 要求后续迁移 | 2026-05-05 |
...
|---|---------|------------|------|---------|
| 1 | strategy_base.py | `D:\ZephyrAlpha\src\zephyr\l05_portfolio_construction\strategy_base.py` | 读取 | 已存在 |
| 2 | strategy_registry.py | `D:\ZephyrAlpha\src\zephyr\l05_portfolio_construction\strategy_registry.py` | 读取 | 已存在 |
| 3 | default_equity_strategy.py | `D:\ZephyrAlpha\src\zephyr\l05_portfolio_construction\strategies\default_equity_strategy.py` | 读取 | 已存在 |
| 4 | __init__.py | `D:\ZephyrAlpha\src\zephyr\l05_portfolio_construction\__init__.py` | 读取 | 已存在 |
...
| 8 | cross-layer-contracts.yaml | `D:\ZephyrAlpha\architecture-model\cross-layer-contracts.yaml` | 修改 | 确认 CTR-004/CTR-007/CTR-P1-006 状态 |
...
| Tier | 消费者 | 依赖内容 |
|:----:|--------|---------|
| Tier 1 | `D:\ZephyrAlpha\docs\03_modules\l06_trade_execution\execution-core\blueprint.md` | §4 接口契约、§10 依赖关系 |
| Tier 1 | `D:\ZephyrAlpha\docs\03_modules\l07_analytics\analytics-core\blueprint.md` | §4 接口契约（CTR-P1-006） |
| Tier 1 | `D:\ZephyrAlpha\docs\03_modules\l10_compliance\compliance-core\blueprint.md` | §4 接口契约（CTR-P1-006） |
| Tier 2 | `D:\ZephyrAlpha\architecture-model\cross-layer-contracts.yaml` | §12 集成点 |
| Tier 3 | `D:\ZephyrAlpha\src\zephyr\l05_portfolio_construction\strategy_base.py` | §4 数据模型、§11 产出物路径 |
| Tier 3 | `D:\ZephyrAlpha\src\zephyr\l05_portfolio_construction\strategy_registry.py` | §4 数据模型、§11 产出物路径 |
