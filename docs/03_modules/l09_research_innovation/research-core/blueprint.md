...
actual_disk_path: "src/zephyr/l09_research_innovation/"
...
references:
  - path: "D:\\ZephyrAlpha\\docs\\02_enterprise_architecture\\target-architecture\\architecture-model\\layers\\l09-research-innovation.yaml"
    section: ""
    why: "架构层YAML真源"
...
> module_id: MOD-L09-001 | version: 2.1.0 | status: active | layer: l09_research_innovation
> actual_disk_path: src/zephyr/l09_research_innovation/ | generation: 2 | construction_progress: partially_implemented
...
> YAML 真源（l09-research-innovation.yaml）定义3个子模块（experiments/notebooks/prototypes），均为 planned 状态。当前代码实现的是回测引擎，与YAML子模块划分不同——这是 ARB-23 裁定的"YAML-磁盘双重现实"问题，待 GOV-FSTR-001 统一。
...
| 3 | 导入源 | zephyr.l03_signal_generation.* | 通过CTR-002消费 | 分层约束 |
...
|---------|---------|---------|---------|---------|
| MOD-L00-001 Data Source | 必须 | CTR-001 NormalizedMarketData | — | `D:\ZephyrAlpha\docs\03_modules\l00_data_source\datasource-core\blueprint.md` |
| MOD-L02-001 Alpha Factor | 必须 | CTR-002 FactorSignal | — | `D:\ZephyrAlpha\docs\03_modules\l02_alpha_factor\alpha-factor-core\blueprint.md` |
| MOD-L07-001 Post-Trade Analytics | 可选 | 盘后分析→研究输入 | — | `D:\ZephyrAlpha\docs\03_modules\l07_post_trade_analytics\analytics-core\blueprint.md` |
| MOD-L13-001 Experiment Pipeline | 可选 | CTR-P1-014 ExperimentResult | — | `D:\ZephyrAlpha\docs\03_modules\l13_experimentation\experiment-core\blueprint.md` |
...
| 2 | §10.1 依赖声明 ↔ l09-research-innovation.yaml | YAML接口声明与蓝图依赖一致 | 已对齐 | 逐项核对 |
| 3 | §0 代码文件清单 ↔ 依赖图节点 code_path | 节点存在 | 已对齐 | `ls src/zephyr/l09_research_innovation/` |
...
| 产出物类型 | 存放完整绝对路径 | 说明 |
|----------|---------------|------|
| 蓝图文件 | `D:\ZephyrAlpha\docs\03_modules\l09_research_innovation\research-core\blueprint.md` | 本文件 |
| 业务代码 | `D:\ZephyrAlpha\src\zephyr\l09_research_innovation\` | Python 源码 |
| 测试代码 | `D:\ZephyrAlpha\tests\unit\l09_research_innovation\` | 测试用例 |
...
| 2 | 架构层YAML | `D:\ZephyrAlpha\docs\02_enterprise_architecture\target-architecture\architecture-model\layers\l09-research-innovation.yaml` | 确认files列表与磁盘一致 | ARB-23双重现实 |
...
|------|------|
| 对应蓝图契约 | §4.1 |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l09_research_innovation\implementations\default_backtest_engine.py` |
| 验收标准 | import成功，run返回BacktestResult |
| 验证命令 | `python -c "from zephyr.l09_research_innovation.implementations.default_backtest_engine import DefaultBacktestEngine"` |
...
|------|------|
| 对应蓝图契约 | §3.3 |
| 产出位置 | `D:\ZephyrAlpha\src\zephyr\l09_research_innovation\backtest_base.py` |
| 验收标准 | candidate→validated→promoted/rejected状态转换正确 |
| 验证命令 | `python -m pytest tests/unit/l09_research_innovation/ -k factor_discovery` |
| G7 检查项 | 状态守卫条件完整 |
| AI 自治范围 | ai_modifiable |
| 检查点 | 状态转换测试通过 |
...
|------|------|
| 对应蓝图契约 | §4 |
| 产出位置 | `D:\ZephyrAlpha\docs\02_enterprise_architecture\target-architecture\architecture-model\cross-layer-contracts.yaml` |
| 验收标准 | BacktestResult注册为producer契约，L13可消费 |
| 验证命令 | `grep "CTR-P1-014" cross-layer-contracts.yaml` |
| G7 检查项 | 下游L13可消费 |
| AI 自治范围 | human_gated |
| 检查点 | 契约注册完成 |
...
| 3 | CTR-P1-014注册失败 | 还原cross-layer-contracts.yaml |
...
| # | 规格名称 | 类型 | 规格内容 | 对应代码 |
|---|---------|------|---------|---------|
| 1 | 向量化回测算法 | 算法 | 遍历日期→信号权重归一化→调仓(先卖后买)→NAV计算→指标计算(sharpe/sortino/calmar/maxDD) | `src/zephyr/l09_research_innovation/implementations/default_backtest_engine.py` |
...
|---|:----:|------|----------|----------|----------|
| 1 | 命令 | `python -c "from zephyr.l09_research_innovation.backtest_base import BacktestEngineBase"` | 验证基类可导入 | — | 无报错 |
...
|--------|---------|---------|----------|:---:|
| CTR-P1-014注册 | GAP-L09-001 | cross-layer-contracts.yaml | Phase 3 | 待施工 |
| FactorDiscovery状态机 | GAP-L09-002 | backtest_base.py | Phase 2 | 待施工 |
| YAML-代码统一 | GAP-L09-003 | l09-research-innovation.yaml | Phase 4 | 待施工 |
...
|---|------|-----------|------|------------|----------|
| 1 | 元数据注册表 | PS-STD-001 | 当前版本 | `D:\ZephyrAlpha\docs\01_policies_and_standards\meta\metadata-registry.md` | 编号规则、doc_type词表 |
...
|---|---------|------------|------|---------|
| 1 | backtest_base.py | `D:\ZephyrAlpha\src\zephyr\l09_research_innovation\backtest_base.py` | 读取/修改 | 补充状态机 |
| 2 | implementations/ | `D:\ZephyrAlpha\src\zephyr\l09_research_innovation\implementations\` | 修改 | 完善实现 |
| 3 | __init__.py | `D:\ZephyrAlpha\src\zephyr\l09_research_innovation\__init__.py` | 读取 | 导出验证 |
