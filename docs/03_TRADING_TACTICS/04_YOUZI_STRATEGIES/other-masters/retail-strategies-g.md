...
class ZexiReverseThinkingQuantifier(BaseStrategy):
    ...
    def detect_reverse_timing(self, market_data):
        ...
        if market_data.get('报告一致性', 0) > 0.8:
            signals['一致预期'] = -0.2
...
class FamousAssassinSystem(BaseStrategy):
    ...
    def analyze_limitup_stocks(self, stock_data):
        ...
        if not is_limitup or continuous_days < params['min_limitup_days']:
            return {'can_buy': False, 'reason': '非涨停或非连板'}
        ...
        volume_ratio = stock_data['成交量'] / stock_data.get('均量', stock_data['成交量'])
...
class LimitUpMorningStar(BaseStrategy):
    ...
    def detect_morning_star(self, market_data, stock_data):
        ...
        if not (continuous_decline and few_3board):
            return {'is_morning_star': False, 'reason': '未满足市场条件'}
        ...
        return {'is_morning_star': False, 'reason': '个股条件不满足'}
```
...
- 买入：二板定龙头，10点前封板，跟风股出现
- 卖出：亏损7%止损，涨15-30%卖一半
...
class ComprehensiveChecklist(BaseStrategy):
    ...
    def check_stock_selection(self, stock_data):
        ...
        checks['放量阳线'] = (
            ...
            stock_data.get('成交量', 0) > stock_data.get('均量', 0) * params['min_volume_ratio']
        )
        checks['涨停优先'] = stock_data.get('is_limit_up', False)
        checks['相对低位'] = stock_data.get('距年内低点', 0) > 0.2
    ...
    def check_buy_conditions(self, stock_data, market_data):
        ...
        checks['有跟风'] = market_data.get('跟风涨停数', 0) >= 1
        checks['换手充分'] = params['min_turnover_rate'] < stock_data.get('换手率', 0) < params['max_turnover_rate']
    ...
    def check_sell_conditions(self, holding_stock):
        ...
        checks['涨15%卖一半'] = profit_ratio >= params['profit_take1_ratio']
        checks['涨30%清仓'] = profit_ratio >= params['profit_take2_ratio']
        checks['回落8%清仓'] = trail_ratio >= params['trail_stop_ratio']

        if checks['亏损7%止损']:
            return {'action': '止损', 'reason': '亏损超过7%'}
        if checks['涨30%清仓'] or checks['回落8%清仓']:
            return {'action': '清仓', 'reason': '达到目标位或回落'}
        if checks['涨15%卖一半']:
            return {'action': '卖出一半', 'reason': '涨15%部分止盈'}
...
**核心理念**：赢面>50%才操作，赢面决定仓位

**量化规则**：
- 赢面计算：市场趋势+赚钱效应+个股动量+催化剂
- 赢面>80%：满仓
- 赢面>60%：半仓以上
- 赢面<50%：空仓
...
class WanShiHuTimingSystem(BaseStrategy):
    ...
    CORE_PRINCIPLES = {
        '核心原则': '赢面 > 50%才操作',
        '仓位依据': '赢面决定仓位',
        '操作频率': '行情好：多操作；行情差：少操作'
    }

    def __init__(self):
        super().__init__("万狮虎择时系统", "S054")
        self.market_states = [MarketState.ANY]
        self.parameters = {
    ...
    def calc_win_probability(self, market_data, stock_data):
        ...
        if stock_data.get('催化剂'):
            factors['催化剂'] = params['catalyst_weight']
        else:
            factors['催化剂'] = 0
...
class ProfessionalTraderSystem(BaseStrategy):
    ...
    def confirm_dragon(self, stock_data, sector_data):
        ...
        follow_stocks = sector_data.get('跟风涨停数', 0)
...
**核心理念**：新生代游资手法，手法激进、反应迅速

**量化规则**：
- 二板四种买入方式：低吸/半路/打板/竞价
- 首板筛选：题材/位置/换手
...
class YoungCrocodileSupplement(BaseStrategy):
    ...
    def select_entry_method(self, stock_data, intraday_data):
        ...
        if intraday_data.get('距涨停', 1) < params['near_limit_max']:
...
class FanbeiSubNewStock(BaseStrategy):
    ...
    def check_new_stock(self, stock_data):
        ...
        if turnover_rate < params['min_turnover_rate']:
            return {'is_new_stock': False, 'reason': '换手率不足'}
...
| 席位 | 风格 | 操作 | 条件 |
|------|------|------|------|
| 欢乐海岸 | 高位锁仓 | 龙头战法 | 连板≥3天 + 席位净买入 |
| 成都帮 | 首板低位 | 低位挖掘 | 首板 + 席位净买入≥5000万 |
| 金田路 | 涨停敢死队 | 龙头 | 连板≥3天 |
| 佛山 | 一夜情 | 超短线 | 消息刺激 + 首板 |
...
|------|---------|------|---------|------|---------|---------|
| S050 | 泽熙逆向思维 | 徐翔/泽熙 | 任何市场 | 中 | 5-30天 | 别人恐惧我贪婪 |
| S051 | 著名刺客实战 | 著名刺客 | 强势市场 | 高 | 1-3天 | 跟庄操作 |
| S052 | 涨停启明星 | 综合 | 弱势市场 | 中 | 3-10天 | 转折点搏击 |
| S053 | 综合量化清单 | 综合 | 任何市场 | 中 | 1-10天 | checklist体系 |
| S054 | 万狮虎择时 | 万狮虎 | 任何市场 | 中 | 1-10天 | 赢面决定仓位 |
| S055 | 职业炒手 | 王元杰 | 强势市场 | 高 | 1-3天 | 龙头战法 |
| S056 | 小鳄鱼补充 | 小鳄鱼 | 强势市场 | 高 | 1-3天 | 新生代手法 |
| S057 | 凡倍无名次新 | 凡倍无名 | 强势市场 | 高 | 1-3天 | 次新股二板 |
| S058 | 顶级游资汇总 | 欢乐海岸等 | 强势市场 | 高 | 1-3天 | 席位资金跟踪 |
...
| 战术模块 | 关联策略 |
|---------|---------|
| [T.00.MR001.市场趋势识别](../02_TACTICS/T.00.MR001.市场趋势识别.md) | S050/S052/S054 |
| [T.01.SN001.资金流向](../02_TACTICS/T.01.SN001.资金流向.md) | S051/S055/S058 |
| [T.01.SN002.龙虎榜跟庄](../02_TACTICS/T.01.SN002.龙虎榜跟庄.md) | S051/S058 |
| [T.04.EX004.盘前计划与买入模式](../02_TACTICS/T.04.EX004.盘前计划与买入模式.md) | S053/S055/S056/S057 |