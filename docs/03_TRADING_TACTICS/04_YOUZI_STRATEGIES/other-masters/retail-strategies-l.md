...
> **说明**：这些策略来自28位游资悟道心法补充内容，已抽象为量化规则
...
class QiaoBangZhuBoomPoint:
    ...
    def detect_boom_point(self, stock_data):
        ...
        return {'is_boom': False, 'reason': '不符合起爆条件'}
```
...
class QiaoBangZhuBottom:
    ...
    def check_bottom_opportunity(self, stock_data, sector_data):
        ...
        decline_ratio = (stock_data['最高价'] - stock_data['当前价']) / stock_data['最高价']
        ...
        if is_sector_leader and is_oversold and volume_stable:
            return {'opportunity': True, 'confidence': 0.8, 'action': '可抄底'}
...
class SmallCrocodileSecondBoard:
    ...
    NEXT_DAY_OPEN = {
        '一字涨停板': {'action': '可放弃', 'reason': '无法买入'},
        '大幅高开(7%+)缩量秒板': {'action': '视情况', 'reason': '谨慎参与'},
        '大幅高开(7%+)下杀': {'action': '低吸、打板', 'reason': '盘中机会'},
        '小幅高开(3%-7%)': {'action': '低吸、打板、竞价', 'reason': '正常参与'},
        '平开、低开': {'action': '视情况', 'reason': '需要分析'}
    }
    ...
    def analyze_second_board(self, stock_data):
        ...
        open_count = stock_data.get('开板次数', 0)

        if not is_limit_up:
            return {'type': '非涨停', 'action': '不适用'}

        if open_count == 0:
            return {'type': '一字板', 'action': '可放弃'}
        elif open_count <= 2:
            return {'type': '实体板', 'action': '可参与'}
        else:
...
class AskingQuantifier:
    ...
    def check_trade_eligibility(self, stock_data, market_data):
        ...
        is_hot = stock_data['所属板块'] in market_data['热点板块']
        has_signal = self.detect_buy_signal(stock_data)

        if is_strong and is_hot and has_signal:
            return {'eligible': True, 'action': '可交易', 'max_position': 0.2}
    ...
    def detect_buy_signal(self, stock_data):
        ...
        if stock_data['成交量'] > stock_data['均量'] * 1.5:
            if stock_data['价格'] > stock_data['压力位']:
                signals['放量突破'] = 0.4

        if stock_data['竞价涨幅'] > 0.05:
            signals['竞价强势'] = 0.3

        if stock_data['分时距均线'] > -0.02:
            signals['均线支撑'] = 0.3
...
class LingHuChongQuantifier:
    ...
    def detect_bottom_signal(self, stock_data):
        ...
        if stock_data['成交量'] < stock_data['均量'] * 0.5:
            signals['地量'] = 0.3
        ...
        if total_score >= 0.6:
            return {'is_bottom': True, 'score': total_score, 'action': '可买入'}

        return {'is_bottom': False, 'score': total_score, 'action': '等待'}

    SELL_STRATEGY = {
        '目标位': {'ma5': '+5%', 'ma10': '+10%', '前期支撑位': '+15%'},
        '卖出信号': {'放量大阴线': '立即卖出', '滞涨': '高开后低走'}
    }
...
class ValueInvestorQuantifier:
    ...
    def evaluate_valuation(self, stock_data):
        ...
        if score >= 0.6:
            return {'undervalued': True, 'score': score, 'action': '可买入'}

        return {'undervalued': False, 'score': score, 'action': '等待'}

    HOLD_RULES = {
        '持有条件': {'基本面稳定': True, '估值未高估': 'PE<20'},
        '卖出标准': {'高估': 'PE>30', '泡沫': '市净率>3'}
    }
    ...
    def should_hold(self, stock_data, original_data):
        ...
        判断是否继续持有
        """
        current_pb = stock_data['市净率']
        original_pb = original_data['市净率']

        if current_pb >= original_pb * 2:
            return {'action': '减仓50%', 'reason': '估值翻倍'}

        if stock_data['业绩增速'] < -0.20:
            return {'action': '清仓', 'reason': '基本面恶化'}
...
class SwingTraderQuantifier:
    ...
    def find_entry_point(self, stock_data, trend_info):
        ...
        if trend_info['trend'] == '上升':
            ...
            if stock_data['当前价'] >= support * 0.98:
                return {
                    'action': '买入',
                    'entry': support,
...
class FirstLimitUpQuantifier:
    ...
    def analyze_first_limitup(self, stock_data):
        ...
        return {'is_good': score >= 0.6, 'score': score, 'recommendation': '可参与' if score >= 0.6 else '观望'}

    NEXT_DAY_RULES = {
        '高开操作': {
            '高开>7%': {'action': '开盘卖出', 'reason': '溢价高'},
            '高开5-7%': {'action': '持有观察', 'reason': '等待连板'},
            '高开<5%': {'action': '视情况', 'reason': '结合市场'}
        },
        '低开操作': {
...
class EmotionCycleQuantifier:
    ...
    def detect_emotion_cycle(self, market_data):
        ...
        elif '高潮期' in indicators:
            return {'cycle': '高潮期', 'action': '分批卖出', 'confidence': indicators['高潮期']}
        elif '发酵期' in indicators:
            return {'cycle': '发酵期', 'action': '持有', 'confidence': indicators['发酵期']}

        return {'cycle': '启动期', 'action': '积极买入', 'confidence': 0.5}

    LEADER_TOP_PATTERNS = {
        '缩量加速': {'特征': '连续3天缩量涨停', '信号强度': 0.8, '操作': '卖出'},
        '尾盘炸板': {'特征': '14:30后开板', '信号强度': 0.7, '操作': '立即卖出'},
        '地天板': {'特征': '从跌停拉到涨停', '信号强度': 0.9, '操作': '卖出'}
    }
    ...
    def detect_leader_top(self, leader_stock):
        ...
        if leader_stock['涨幅'] > 0.09 and leader_stock['最低价'] == leader_stock['跌停价']:
...
class Ningbo敢死队Quantifier:
    ...
    LIMIT_UP_TYPES = {
        '一字板': {'action': '不参与', 'reason': '无法买入'},
        '实体板': {'action': '最佳买点', 'reason': '可参与'},
        '洗盘板': {'action': '可参与', 'reason': '涨停后开板洗盘'}
    }

    LIMIT_UP_NEXT_DAY = {
        '高开>5%': {'action': '开盘卖出一半', 'reason': '锁定利润'},
        '平开或低开': {'action': '等待冲高卖出', 'reason': '寻找高点'},
        '收盘价<涨停价80%': {'action': '止损', 'reason': '形态破坏'}
    }
    ...
    def analyze_limit_up_type(self, stock_data):
        ...
        if open_count <= 2 and turnover_rate < 0.25:
            return self.LIMIT_UP_TYPES['洗盘板']
        return self.LIMIT_UP_TYPES['实体板']
```
...
class RuiHeXianBearMarket:
    ...
    def check_entry_opportunity(self, market_data):
        ...
        if market_data.get('kline_type') == '十字星':
            volume_ratio = market_data['成交量'] / market_data['均量']
            if volume_ratio < 0.8:
                signals['缩量十字星'] = 0.4
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v2.0 | 2026-03-28 | 新增：乔帮主完整策略(S106)、小鳄鱼补充(S107)、Asking炒单(S108)、令胡冲超跌(S109)、92科比价值(S110)、波段操作(S111)、首板战法(S112)、情绪周期(S113)、宁波敢死队(S114)、瑞鹤仙熊市(S115)、章盟主三线(S116)、综合要点(S117-S120) |
