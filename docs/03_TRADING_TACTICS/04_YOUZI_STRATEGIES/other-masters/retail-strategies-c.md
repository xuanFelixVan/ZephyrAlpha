...
class NewThemeQuantifier(BaseStrategy):
    ...
    def identify_new_theme(self, market_data, theme_data):
        ...
        has_capital = theme_data.get('板块成交额', 0) > params['min_turnover']
        has_recognition = theme_data.get('板块涨停数', 0) >= params['min_limit_up_count']
    ...
    def check_theme_story(self, theme_data, market_data):
        ...
        检查题材是否有故事（政策、业绩等）
        """
        policy_score = theme_data.get('政策关联度', 0)
...
| 阶段 | 涨停家数 | 跌停家数 | 炸板率 | 连板股数 | 操作策略 |
|------|----------|----------|--------|----------|----------|
| 冰点 | <20 | >30 | >50% | 0 | 观望，等待转折 |
...
class EmotionCycleFiveStagesStrategy(BaseStrategy):
    ...
    def identify_stage(self, market_data):
        ...
        break_rate = market_data.get('炸板率', 0)
...
**核心理念**：主力行为四步循环：吸筹→洗盘→拉升→出货

**量化规则**：

| 阶段 | 特征 | 量化信号 | 散户应对 |
|------|------|----------|----------|
| 吸筹 | 低位悄悄买 | 缩量横盘/地量 | 别割肉 |
| 洗盘 | 把散户洗出去 | 缩量下跌/尾盘打压 | 别被吓走 |
| 拉升 | 快速拉高赚钱 | 放量突破/缩量回调 | 拿稳 |
| 出货 | 高位偷偷卖 | 高位放量滞涨 | 快跑 |
...
**核心理念**：做T是A股重要的超额收益来源，需要分钟级数据和特殊因子支持

**量化规则**：
- 基础比例：日线仓位 × 20-30%
- 置信度调整：基础仓位 × 分钟预测置信度
- 市场状态调整：牛市上限50%，熊市上限20%
- 硬性限制：单日做T仓位 ≤ 总资产50%
- 最大做T次数：≤10次/日
- 最小间隔时间：≥15分钟
- 冷却机制：连续3次亏损暂停做T
...
- 资金信号：分钟级资金持续流入（连续3个5分钟净流入）
- 波动信号：波动率回归到均值以下（波动率 < 20日均值×0.8）
...
**不做T条件**：
- 流动性不足：买卖价差 > 0.2%
- 波动率过低：5分钟ATR < 20日均值×0.6
- 重大事件窗口：财报/宏观数据发布前后1小时
- 系统风险预警：大盘下跌 > 2%
...
class DayTradingDecisionStrategy(BaseStrategy):
    ...
    def check_avoid_conditions(self, market_data):
        ...
        if market_data.get('5分钟ATR', 0) < market_data.get('ATR均值', 1) * params['low_volatility_threshold']:
            reasons.append('波动率过低')

        if market_data.get('重大事件窗口', False):
            reasons.append('重大事件窗口')

        if market_data.get('大盘涨跌幅', 0) < -params['market_drop_warning']:
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v1.0 | 2026-03-28 | 整合二板定龙头(S021-S022)、情绪周期五阶段(S023)、主力伏击(S024-S025)、一夜持股法(S026)、做T策略(S027-S028) |
