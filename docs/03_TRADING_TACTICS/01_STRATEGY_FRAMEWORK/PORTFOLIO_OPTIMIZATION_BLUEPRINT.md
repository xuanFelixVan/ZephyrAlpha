...
> 清风量化交易系统 v5.3 - 策略组合优化系统详细技术设计
> **索引**: `STRAT.PORTFOLIO.OPT.001`
> **开发周期**: 140小时（胶合代码开发）
> **核心定位**: 策略工厂核心组件，支持多策略组合优化、资金分配、风险预算管理、动态调仓的智能组合系统
> **参考开源**: PyPortfolioOpt + CVXPY + Riskfolio-Lib + 桥水全天候组合优化思想
> **补充文档**: 本蓝图是[STRATEGY_SELECTION_BLUEPRINT.md](./STRATEGY_SELECTION_BLUEPRINT.md)的后续组件，专注于多策略组合构建与优化
...
| 目标 | 优先级 | 技术实现 |
|------|--------|----------|
| **多策略组合优化** | P0 | 均值-方差优化、风险平价、最大夏普比率、最小回撤等多种优化目标 |
| **智能资金分配** | P0 | 基于策略绩效、相关性、风险贡献度的动态资金分配算法 |
...
1. **可解释性原则**：组合权重调整必须有明确的数学逻辑和业务解释
2. **风险可控原则**：组合总风险不超过预设阈值，单策略风险贡献度可控
3. **交易成本意识**：调仓决策考虑交易成本，避免频繁无效调仓
4. **实盘友好原则**：优化结果必须考虑实盘执行可行性（流动性、冲击成本等）
5. **开源优先原则**：优先使用成熟开源优化库（PyPortfolioOpt、CVXPY、Riskfolio-Lib）

### 1.3 与现有系统集成

| 已有模块 | 集成方式 | 接口定义 |
|----------|----------|----------|
| **策略排名与选择系统** | 输入源 | 接收已选择的策略列表及其绩效数据 |
| **批量评估系统** | 数据源 | 获取策略历史表现、相关性矩阵、风险指标 |
...
### 3.2 均值-方差优化器（MeanVarianceOptimizer）

```python
class MeanVarianceOptimizer:
    """
    均值-方差优化器 - 基于马科维茨现代投资组合理论
    使用PyPortfolioOpt库实现
    """
...
class RiskParityOptimizer:
    ...
    def __init__(self, risk_measure: str = 'CVaR', alpha: float = 0.05):
        self.risk_measure = risk_measure  # CVaR, VaR, CDaR, EDaR等
...
class ConstraintProcessor:
    ...
    def _handle_position_limit(self, weights: pd.Series, limit_config: Dict) -> pd.Series:
        ...
        处理单策略仓位限制
        """
        max_position = limit_config.get('max_position', 0.3)  # 默认单策略最大仓位30%
        min_position = limit_config.get('min_position', 0.01) # 默认最小仓位1%
    ...
    def _handle_liquidity_constraint(self, weights: pd.Series, liquidity_config: Dict) -> pd.Series:
        ...
        portfolio_value = liquidity_config.get('portfolio_value', 1e6)  # 默认组合规模100万
        ...
        for strategy_id, weight in weights.items():
            if strategy_id in liquidity_data:
                ...
                if position_value > max_daily_trade * 3:  # 分3天建仓
                    # 调整权重至流动性允许范围
...
class RLRebalancer:
    ...
    def __init__(self, state_dim: int = 20, action_dim: int = 10):
        self.state_dim = state_dim  # 状态维度：市场状态 + 策略表现 + 风险指标
    ...
    def _format_state_to_observation(self, state: Dict) -> np.ndarray:
        ...
        observation = np.concatenate([
            market_features,
            strategy_features[:min(len(strategy_features), 8)],  # 最多8个策略特征
            risk_features
        ])
...
pypfopt_config:
  ...
  constraints:
    weight_bounds: [0.01, 0.3]  # 单策略权重范围1%-30%
...
class MinDrawdownOptimizer:
    ...
    def optimize(self, returns: pd.DataFrame, max_positions: int = 10) -> pd.Series:
        ...
        constraints = [
            ...
            w >= 0,  # 不允许做空
            w <= 0.3,  # 单资产最大权重30%
            cp.norm(w, 0) <= max_positions  # 最多持有max_positions个策略
        ]
...
portfolio_optimization:
  ...
  risk_budget:
    ...
    max_drawdown_limit: 0.20  # 最大回撤不超过20%
    var_95_limit: 0.05  # 95% VaR不超过5%
    cvar_95_limit: 0.07  # 95% CVaR不超过7%
    
    strategy_limits:
      max_single_strategy_risk: 0.25  # 单策略风险贡献不超过25%
      min_strategies: 5  # 最少持有5个策略
      max_strategies: 12  # 最多持有12个策略
  
  # 交易约束配置
  trading_constraints:
    position_limits:
      max_single_position: 0.30  # 单策略最大仓位30%
      min_position: 0.01  # 最小仓位1%
    
    turnover_limits:
      max_annual_turnover: 6.0  # 年化换手率不超过6倍
      max_single_rebalance: 0.10  # 单次调仓不超过10%
  ...
  monitoring:
    ...
    performance_check_frequency: "daily"  # 每日检查绩效
    risk_monitoring_frequency: "intraday"  # 日内风险监控
    alert_thresholds:
      drawdown_alert: 0.08  # 回撤超过8%预警
      volatility_alert: 0.18  # 波动率超过18%预警
      concentration_alert: 0.35  # 集中度超过35%预警
```
...
| 里程碑 | 预计完成时间 | 交付物 | 成功标准 |
|--------|--------------|--------|----------|
| **M1：基础优化框架** | 第4周结束 | 1. 组合优化控制器<br>2. 均值-方差优化器<br>3. 基本约束处理器 | 支持5种策略组合优化，结果可复现 |
| **M2：风险平价实现** | 第10周结束 | 1. 风险平价优化器<br>2. 风险贡献度分析<br>3. 多风险度量支持 | 实现经典风险平价和风险预算模型 |
| **M3：AI调仓系统** | 第18周结束 | 1. 强化学习调仓器<br>2. 市场环境分类器<br>3. 自适应优化框架 | AI调仓策略表现优于静态优化10% |
| **M4：实盘就绪** | 第24周结束 | 1. 完整实盘约束处理<br>2. 高性能优化引擎<br>3. 监控告警系统 | 通过3个月模拟盘测试，年化换手率<8倍 |
...
| **计算性能瓶颈** | 高 | 中 | 1. 并行计算优化<br>2. 缓存机制实现<br>3. 增量计算算法 |
...
1. **PyPortfolioOpt** - Python投资组合优化库
   - GitHub: https://github.com/robertmartin8/PyPortfolioOpt
   - 特点：实现马科维茨现代投资组合理论，支持均值-方差优化、黑利特曼模型等
   - 集成方式：直接pip安装，作为核心优化引擎
...
9. **Qlib** - 微软量化投资平台
   ...
   - 特点：完整的量化研究框架，包含组合优化模块
   - 参考价值：架构设计、模块组织

10. **QuantConnect** - 开源量化平台
...
本蓝图详细设计了策略组合优化系统的完整技术方案，涵盖从基础均值-方差优化到高级风险平价模型，再到AI增强的动态调仓系统。系统设计遵循以下核心原则：

1. **开源优先原则**：最大限度利用成熟开源库，减少自研代码量
2. **模块化设计**：各优化算法独立封装，支持灵活组合和扩展
3. **实盘友好**：充分考虑交易成本、流动性、监管限制等实盘约束
4. **风险可控**：多层次风险预算体系，确保组合风险在预设范围内
5. **AI增强**：利用强化学习和机器学习优化动态调仓决策

对于个人开发者（不懂编程）而言，本系统的主要开发工作将是：
- **胶合代码编写**：将各开源模块集成到统一框架中（约40%工作量）
- **配置管理**：设计灵活的配置文件系统，支持不同优化场景（约30%工作量）
- **测试验证**：验证优化结果的有效性和稳定性（约20%工作量）
- **文档与界面**：创建用户友好的命令行和Web界面（约10%工作量）
