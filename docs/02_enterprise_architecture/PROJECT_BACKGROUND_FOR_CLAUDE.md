...
[git_commit_gateway.py](file:///d:/ZephyrAlpha/src/zephyr/governance/rule_bridge/git_commit_gateway.py)：
...
[capability_lookup.py](file:///d:/ZephyrAlpha/src/zephyr/governance/capability_lookup.py)，被 76+ 消费者使用。能力 → 真源文件反查，134 条能力。
...
完整技术决策清单见 [tech_stack_manifest.yaml](file:///d:/ZephyrAlpha/config/tech_stack_manifest.yaml)。典型决策：
...
代码唯一存放于 [src/zephyr/backtest/](file:///d:/ZephyrAlpha/src/zephyr/backtest/)。

### 6.2 双模式架构

| 引擎 | 文件 | 推进单位 | 适用场景 |
|------|------|---------|---------|
| `DefaultBacktestEngine`（向量化） | [vectorized_engine.py](file:///d:/ZephyrAlpha/src/zephyr/backtest/implementations/vectorized_engine.py) | 日 bar | 快速筛选因子 IC/IR |
| `EventDrivenEngine`（事件驱动） | [event_driven_engine.py](file:///d:/ZephyrAlpha/src/zephyr/backtest/implementations/event_driven_engine.py) | Tick | 精确验证策略 PnL，做 T 专用 |

### 6.3 BacktestResult — CTR-P1-016 冻结契约

[engine_base.py](file:///d:/ZephyrAlpha/src/zephyr/backtest/core/engine_base.py) 中的 `@dataclass(frozen=True) BacktestResult`，由 codegen 从 `cross_layer_contracts.yaml` 自动生成。
...
[pit_manager.py](file:///d:/ZephyrAlpha/src/zephyr/backtest/core/pit_manager.py)，**PIT 三公理**：
...
[walk_forward.py](file:///d:/ZephyrAlpha/src/zephyr/backtest/core/walk_forward.py)：
...
[overfitting_detector.py](file:///d:/ZephyrAlpha/src/zephyr/backtest/core/overfitting_detector.py)：
...
[decision_gate.py](file:///d:/ZephyrAlpha/src/zephyr/backtest/core/decision_gate.py)，**3 阶段不可跳级**：
...
两个引擎共用 `MatchingLogic` 模块（[matching_logic.py](file:///d:/ZephyrAlpha/src/zephyr/backtest/core/matching_logic.py)），撮合规则与 D_EX_CORE 的 MiniQMT Broker 保持一致。
...
[src/zephyr/frontend/](file:///d:/ZephyrAlpha/src/zephyr/frontend/) 已有 Streamlit dashboard 骨架：

- [app.py](file:///d:/ZephyrAlpha/src/zephyr/frontend/dashboard/app.py)：5 个页面（任务进度/知识库/门禁/Fitness/OLAP）
- [components/](file:///d:/ZephyrAlpha/src/zephyr/frontend/dashboard/components/)：10 个组件（backtest_results / fitness_functions / gate_statistics / knowledge_overview / olap_trend / order_book / position_monitor / task_progress / tick_replay / trade_panel）

[backtest_results.py](file:///d:/ZephyrAlpha/src/zephyr/frontend/dashboard/components/backtest_results.py)（v2.2.0）已实现回测可视化组件，含 `BacktestResultData` / `BacktestMetrics` / `BacktestGateStatus` dataclass + `fetch_backtest_results()` + `render_backtest_results()`，但**尚未接入 app.py**。

### 7.2 长期前端终局

[frontend_architecture.md](file:///d:/ZephyrAlpha/docs/02_enterprise_architecture/target_architecture/frontend_architecture.md)（v1.1.1）定义了 7 条前端铁律：
...
[architecture_issue_registry.yaml](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/_registry/catalogs/architecture_issue_registry.yaml)（REG-ARCH-ISSUE-001）：
...
[architecture_debt_registry.md](file:///d:/ZephyrAlpha/docs/02_enterprise_architecture/architecture_debt_registry.md) 执行摘要：
...
### 顶层政策
- [README.md](file:///d:/ZephyrAlpha/README.md)
- [AGENTS.md](file:///d:/ZephyrAlpha/AGENTS.md)
- [.trae/rules/project_rules.md](file:///d:/ZephyrAlpha/.trae/rules/project_rules.md)
- [trae_057_ai_consumer_first.yaml](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/rules/trae_057_ai_consumer_first.yaml)
- [trae_060_inward_consolidation.yaml](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/rules/trae_060_inward_consolidation.yaml)
- [trae_053_automation_dual_track.yaml](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/rules/trae_053_automation_dual_track.yaml)

### 架构
- [architecture_pritecture_principles.md](file:///d:/ZephyrAlpha/docs/02_enterprise_architecture/04_architecture_principles_decisions/architecture_pritecture_principles.md)
- [dependency_path_panorama.md](file:///d:/ZephyrAlpha/docs/02_enterprise_architecture/04_architecture_principles_decisions/dependency_path_panorama.md)
- [target_architecture/index.md](file:///d:/ZephyrAlpha/docs/02_enterprise_architecture/target_architecture/index.md)
- [target_architecture/overview.md](file:///d:/ZephyrAlpha/docs/02_enterprise_architecture/target_architecture/overview.md)
- [application_architecture.md](file:///d:/ZephyrAlpha/docs/02_enterprise_architecture/target_architecture/application_architecture.md)
- [frontend_architecture.md](file:///d:/ZephyrAlpha/docs/02_enterprise_architecture/target_architecture/frontend_architecture.md)
- [architecture_debt_registry.md](file:///d:/ZephyrAlpha/docs/02_enterprise_architecture/architecture_debt_registry.md)

### 治理代码
- [git_commit_gateway.py](file:///d:/ZephyrAlpha/src/zephyr/governance/rule_bridge/git_commit_gateway.py)
- [session_worktree.py](file:///d:/ZephyrAlpha/src/zephyr/governance/rule_bridge/session_worktree.py)
- [depgraph_schema.py](file:///d:/ZephyrAlpha/src/zephyr/governance/depgraph_schema.py)
- [capability_lookup.py](file:///d:/ZephyrAlpha/src/zephyr/governance/capability_lookup.py)
- [task_repo.py](file:///d:/ZephyrAlpha/src/zephyr/governance/persistence/task_repo.py)
- [commit_gates/](file:///d:/ZephyrAlpha/src/zephyr/governance/commit_gates/)

### 注册表
- [capability_canonical_file_registry.yaml](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/_registry/catalogs/capability_canonical_file_registry.yaml)
- [architecture_issue_registry.yaml](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/_registry/catalogs/architecture_issue_registry.yaml)
- [infrastructure_registry.yaml](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/_registry/catalogs/infrastructure_registry.yaml)
- [blueprint_registry.yaml](file:///d:/ZephyrAlpha/docs/03_modules/blueprint_registry.yaml)

### 回测子系统
- [_domain_backtest/blueprint.md](file:///d:/ZephyrAlpha/docs/03_modules/_domain_backtest/blueprint.md)
- [src/zephyr/backtest/](file:///d:/ZephyrAlpha/src/zephyr/backtest/)
- [engine_base.py](file:///d:/ZephyrAlpha/src/zephyr/backtest/core/engine_base.py)
- [decision_gate.py](file:///d:/ZephyrAlpha/src/zephyr/backtest/core/decision_gate.py)
- [overfitting_detector.py](file:///d:/ZephyrAlpha/src/zephyr/backtest/core/overfitting_detector.py)
- [walk_forward.py](file:///d:/ZephyrAlpha/src/zephyr/backtest/core/walk_forward.py)
- [pit_manager.py](file:///d:/ZephyrAlpha/src/zephyr/backtest/core/pit_manager.py)

### 技术栈
- [pyproject.toml](file:///d:/ZephyrAlpha/pyproject.toml)
- [tech_stack_manifest.yaml](file:///d:/ZephyrAlpha/config/tech_stack_manifest.yaml)
