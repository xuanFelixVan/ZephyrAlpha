...
| 文档 | 完整路径 | 作用 |
|------|---------|------|
| 依赖与架构全景图能力定位书 | `D:\ZephyrAlpha\docs\02_enterprise_architecture\依赖与架构全景图能力定位书.md` | 全景图能力定位、设计决策、裁定记录、七批次施工规格（§5-§22） |
...
2. 数据库全景图（depgraph.db）是整个项目蓝图，包含所有文件夹/文件、
   功能域依赖关系和架构关系
   ← 依赖与架构全景图能力定位书.md §一：依赖全景图管"谁依赖谁"，架构全景图管"放在哪"
...
5. 修改全景图生成器代码，对齐实际目录结构和数据库全景图；
   在此之前不运行生成器，否则会覆盖全景图
   ← 依赖与架构全景图能力定位书.md §4.4：生成器 DELETE+INSERT 运营态，
