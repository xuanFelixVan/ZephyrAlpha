...
| 决策 | 推荐方案 | 实施依赖 | 优先级 |
|------|---------|---------|:---:|
| T6 事件类型体系 | 选项C: 注册表+Enum | R1-2 AsyncEventBus | 中 |
| T7 三级配置结构 | 选项C: 混合结构 | 阶段1 STEP 1（已就绪） | 高 |
| T17 模块DOMAIN字段 | 选项B: 可选声明 | 生成器修改+100-150模块添加字段 | 中 |
| T18 依赖图真源裁定 | 选项B: 分层真源（设计态YAML化） | **暂缓** — 转为阶段8生产环节，按域生产蓝图时重启 | 低（暂缓） |

**审批请求**：请 Owner 审批以上 4 项推荐方案。审批后创建对应任务卡实施。
