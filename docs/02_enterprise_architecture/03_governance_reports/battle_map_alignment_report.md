...
- 生成时间: 2026-08-04 19:05:50
- 数据源: depgraph (PostgreSQL)
- 三表统计: steps=302 / anchors=392 / edges=119 / 叙事真源=285
- 业务域模块: 1764（BM-INV-007 扫描范围，业务域白名单内 depgraph 节点）
- 问题总数: 1172
  - 孤儿环节（BM-INV-001，无锚点=悬空决策）: 17
  - 幽灵锚点（BM-INV-002，target_id 找不到）: 0
  - 缺失叙事（BM-INV-003，翻译真源无环节）: 17
  - 悬空边（edge 指向不存在环节）: 0
  - 域漂移（BM-INV-004，target domain 不在 flow_stage 允许列表）: 0
  - 父子嵌套问题（BM-INV-006，父不存在/跨阶段/成环/depth超限）: 0
  - 孤儿模块（BM-INV-007，业务域模块无作战锚点=造出来没用上）: 1138
