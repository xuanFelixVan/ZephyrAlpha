...
- 生成时间: 2026-08-05 00:23:33
- 数据源: depgraph (PostgreSQL)
- 三表统计: steps=324 / anchors=496 / edges=119 / 叙事真源=324
- 业务域模块: 1767（BM-INV-007 扫描范围，业务域白名单内 depgraph 节点）
- 问题总数: 159
  - 孤儿环节（BM-INV-001，无锚点=悬空决策）: 25
  ...
  - 孤儿模块（BM-INV-007，业务域模块无作战锚点=造出来没用上）: 134
...
> 裁定原则：域归属看"承载什么决策"，不看"被谁调用"。supplement 角色（工具被用到，不承载业务决策）豁免域漂移检查；仅 primary（承载决策）严查域。故本表只列 primary 锚点的漂移。
