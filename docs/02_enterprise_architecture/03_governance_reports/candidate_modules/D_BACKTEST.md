...
| CAND-BT-001 | Backtest v2.0 Auxiliary Modules / 回测v2.0辅助模块 | 回测需批量调度/衰减监控/自动报告/结果缓存时,无对应辅助模块 | D_BACKTEST | 延后（deferred） | q2 无需求驱动 | P2 | 回测需批量调度多策略(当前单策略) 等4条 | 2027-07-31 |
| CAND-WFO-001 | Walk-Forward Optimizer / 滚动前进优化器 | 回测参数过拟合风险——单一全样本优化容易拟合历史噪声,实盘表现衰退 | D_BACKTEST | 延后（deferred） | q2 无需求驱动 | P2 | 实盘策略表现与回测显著背离(过拟合迹象) 等3条 | 2027-01-31 |
