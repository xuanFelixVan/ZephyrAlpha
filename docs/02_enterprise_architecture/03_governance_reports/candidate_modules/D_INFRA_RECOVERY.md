...
|------|------|------|------|------|------|:---:|------|------|
| CAND-DR-001 | Offsite Backup / 异地备份 | 本地备份之外再加一份云上/异地副本（3-2-1原则）。用户已裁定：单用户场景本地外接盘F盘就够，不预埋异地备份。 | D_INFRA_RECOVERY | 否决（rejected） | q2 无需求驱动 | P2 | 系统升级为多用户/多站点场景(当前单用户) 等3条 | 2027-07-31 |
...
|------|------|------|------|------|------|
| CAND-DR-001 | Offsite Backup / 异地备份 | 本地备份之外再加一份云上/异地副本（3-2-1原则）。用户已裁定：单用户场景本地外接盘F盘就够，不预埋异地备份。 | D_INFRA_RECOVERY | rejected,overruled_by_user #ARCH-CH-032。除非场景升级为多用户/多站点,否则不再评估 | 本地外接盘备份(F:\restic-zephyr,主库+外接盘=2份)。代价:无,用户已确认足够 |
