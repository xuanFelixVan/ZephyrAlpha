---
doc_type: audit_report
title: 架构约束违规报告
version: "1.0"
status: active
date: auto-generated
owner: auto-generator
ttl: permanent
---

# 架构约束违规报告

> **文档作用 / Purpose**: 展示架构约束违规情况，包括跨层依赖、循环依赖、命名违规等，为架构治理提供修复清单。

> 本文档由 generate_constraint_violations.py 从 depgraph (PostgreSQL) 自动生成
> 最后更新以 git log 为准
> 数据源: depgraph (PostgreSQL) arch_constraints表

## 统计概览

| 指标 / Metric | 值 / Value |
|------|-----|
| 约束总数 | 125 |
| Open（未解决） | 125 |
| Resolved（已解决） | 0 |
| 其他状态 | 0 |

## 按严重程度分组

| 严重程度 / Severity | 数量 / Count |
|---------|:---:|
| error | 117 |
| hard | 8 |

## 按约束类型分组

| 约束类型 / Constraint Type | 数量 / Count |
|---------|:---:|
| architecture_contract | 1 |
| capacity_exceeded | 8 |
| cross_domain_violation | 92 |
| hard_limit_exceeded | 8 |
| layer_violation | 16 |

## Open 违规清单（需处理）

| 约束ID / Constraint ID | 名称 / Name | 类型 / Type | 源域 / From Domain | 目标域 / To Domain | 严重程度 / Severity | 执行方式 / Enforcement | 描述 / Description |
|--------|------|------|------|--------|---------|---------|------|
| V-CAP-D_DATA | 容量超限: D_DATA | capacity_exceeded | D_DATA |  | hard | gate | 域 D_DATA(数据接入层) production 节点 172 超过上限 150，需拆分或提升上限 (ARCH-CA... |
| V-CAP-D_GOVERNANCE | 容量超限: D_GOVERNANCE | capacity_exceeded | D_GOVERNANCE |  | hard | gate | 域 D_GOVERNANCE(生命周期管理) production 节点 451 超过上限 150，需拆分或提升上限 (... |
| V-CAP-D_GOV_AUDIT | 容量超限: D_GOV_AUDIT | capacity_exceeded | D_GOV_AUDIT |  | hard | gate | 域 D_GOV_AUDIT(审计追踪) production 节点 192 超过上限 150，需拆分或提升上限 (ARC... |
| V-CAP-D_GOV_CODE_QUALITY | 容量超限: D_GOV_CODE_QUALITY | capacity_exceeded | D_GOV_CODE_QUALITY |  | hard | gate | 域 D_GOV_CODE_QUALITY(代码质量治理) production 节点 215 超过上限 150，需拆分或... |
| V-CAP-D_GOV_SCRIPTS | 容量超限: D_GOV_SCRIPTS | capacity_exceeded | D_GOV_SCRIPTS |  | hard | gate | 域 D_GOV_SCRIPTS(脚本治理) production 节点 422 超过上限 150，需拆分或提升上限 (A... |
| V-CAP-D_INFRA_RUNTIME | 容量超限: D_INFRA_RUNTIME | capacity_exceeded | D_INFRA_RUNTIME |  | hard | gate | 域 D_INFRA_RUNTIME(运行时集成) production 节点 171 超过上限 150，需拆分或提升上限... |
| V-CAP-D_SECURITY | 容量超限: D_SECURITY | capacity_exceeded | D_SECURITY |  | hard | gate | 域 D_SECURITY(对抗验证) production 节点 171 超过上限 150，需拆分或提升上限 (ARCH... |
| V-CAP-D_SHARED | 容量超限: D_SHARED | capacity_exceeded | D_SHARED |  | hard | gate | 域 D_SHARED(共享服务) production 节点 177 超过上限 150，需拆分或提升上限 (ARCH-C... |
|  | procedural policy 必须可验证（不能是 inspection） | architecture_contract |  |  | error | code |  |
| V-CROSS-D_AUTONOMY_CORE-D_SECURITY | 跨域违规: D_AUTONOMY_CORE -> D_SECURITY | cross_domain_violation | D_AUTONOMY_CORE | D_SECURITY | error | gate | 跨域依赖未声明: D_AUTONOMY_CORE -> D_SECURITY |
| V-CROSS-D_AUTONOMY_PERM-D_GOV_SCRIPTS | 跨域违规: D_AUTONOMY_PERM -> D_GOV_SCRIPTS | cross_domain_violation | D_AUTONOMY_PERM | D_GOV_SCRIPTS | error | gate | 跨域依赖未声明: D_AUTONOMY_PERM -> D_GOV_SCRIPTS |
| V-CROSS-D_BACKTEST-D_FACTOR | 跨域违规: D_BACKTEST -> D_FACTOR | cross_domain_violation | D_BACKTEST | D_FACTOR | error | gate | 跨域依赖未声明: D_BACKTEST -> D_FACTOR |
| V-CROSS-D_BACKTEST-D_INFRA_RUNTIME | 跨域违规: D_BACKTEST -> D_INFRA_RUNTIME | cross_domain_violation | D_BACKTEST | D_INFRA_RUNTIME | error | gate | 跨域依赖未声明: D_BACKTEST -> D_INFRA_RUNTIME |
| V-CROSS-D_COMPLIANCE-D_SECURITY | 跨域违规: D_COMPLIANCE -> D_SECURITY | cross_domain_violation | D_COMPLIANCE | D_SECURITY | error | gate | 跨域依赖未声明: D_COMPLIANCE -> D_SECURITY |
| V-CROSS-D_EX_CORE-D_FACTOR | 跨域违规: D_EX_CORE -> D_FACTOR | cross_domain_violation | D_EX_CORE | D_FACTOR | error | gate | 跨域依赖未声明: D_EX_CORE -> D_FACTOR |
| V-CROSS-D_EX_SOR-D_INFRASTRUCTURE | 跨域违规: D_EX_SOR -> D_INFRASTRUCTURE | cross_domain_violation | D_EX_SOR | D_INFRASTRUCTURE | error | gate | 跨域依赖未声明: D_EX_SOR -> D_INFRASTRUCTURE |
| V-CROSS-D_EX_SOR-D_SHARED | 跨域违规: D_EX_SOR -> D_SHARED | cross_domain_violation | D_EX_SOR | D_SHARED | error | gate | 跨域依赖未声明: D_EX_SOR -> D_SHARED |
| V-CROSS-D_FACTOR-D_INFRA_RUNTIME | 跨域违规: D_FACTOR -> D_INFRA_RUNTIME | cross_domain_violation | D_FACTOR | D_INFRA_RUNTIME | error | gate | 跨域依赖未声明: D_FACTOR -> D_INFRA_RUNTIME |
| V-CROSS-D_FACTOR-D_SHARED | 跨域违规: D_FACTOR -> D_SHARED | cross_domain_violation | D_FACTOR | D_SHARED | error | gate | 跨域依赖未声明: D_FACTOR -> D_SHARED |
| V-CROSS-D_FEEDBACK_LOOP-D_SECURITY | 跨域违规: D_FEEDBACK_LOOP -> D_SECURITY | cross_domain_violation | D_FEEDBACK_LOOP | D_SECURITY | error | gate | 跨域依赖未声明: D_FEEDBACK_LOOP -> D_SECURITY |
| V-CROSS-D_GOVERNANCE-D_AUTONOMY_CORE | 跨域违规: D_GOVERNANCE -> D_AUTONOMY_CORE | cross_domain_violation | D_GOVERNANCE | D_AUTONOMY_CORE | error | gate | 跨域依赖未声明: D_GOVERNANCE -> D_AUTONOMY_CORE |
| V-CROSS-D_GOVERNANCE-D_BACKTEST | 跨域违规: D_GOVERNANCE -> D_BACKTEST | cross_domain_violation | D_GOVERNANCE | D_BACKTEST | error | gate | 跨域依赖未声明: D_GOVERNANCE -> D_BACKTEST |
| V-CROSS-D_GOVERNANCE-D_EX_CORE | 跨域违规: D_GOVERNANCE -> D_EX_CORE | cross_domain_violation | D_GOVERNANCE | D_EX_CORE | error | gate | 跨域依赖未声明: D_GOVERNANCE -> D_EX_CORE |
| V-CROSS-D_GOVERNANCE-D_FACTOR | 跨域违规: D_GOVERNANCE -> D_FACTOR | cross_domain_violation | D_GOVERNANCE | D_FACTOR | error | gate | 跨域依赖未声明: D_GOVERNANCE -> D_FACTOR |
| V-CROSS-D_GOVERNANCE-D_FBL_DETECTORS | 跨域违规: D_GOVERNANCE -> D_FBL_DETECTORS | cross_domain_violation | D_GOVERNANCE | D_FBL_DETECTORS | error | gate | 跨域依赖未声明: D_GOVERNANCE -> D_FBL_DETECTORS |
| V-CROSS-D_GOVERNANCE-D_FBL_DIAGNOSERS | 跨域违规: D_GOVERNANCE -> D_FBL_DIAGNOSERS | cross_domain_violation | D_GOVERNANCE | D_FBL_DIAGNOSERS | error | gate | 跨域依赖未声明: D_GOVERNANCE -> D_FBL_DIAGNOSERS |
| V-CROSS-D_GOVERNANCE-D_FEEDBACK_LOOP | 跨域违规: D_GOVERNANCE -> D_FEEDBACK_LOOP | cross_domain_violation | D_GOVERNANCE | D_FEEDBACK_LOOP | error | gate | 跨域依赖未声明: D_GOVERNANCE -> D_FEEDBACK_LOOP |
| V-CROSS-D_GOVERNANCE-D_FUNDAMENTAL_SIGNAL | 跨域违规: D_GOVERNANCE -> D_FUNDAMENTAL_SIGNAL | cross_domain_violation | D_GOVERNANCE | D_FUNDAMENTAL_SIGNAL | error | gate | 跨域依赖未声明: D_GOVERNANCE -> D_FUNDAMENTAL_SIGNAL |
| V-CROSS-D_GOVERNANCE-D_GOV_DRIFT | 跨域违规: D_GOVERNANCE -> D_GOV_DRIFT | cross_domain_violation | D_GOVERNANCE | D_GOV_DRIFT | error | gate | 跨域依赖未声明: D_GOVERNANCE -> D_GOV_DRIFT |
| V-CROSS-D_GOVERNANCE-D_INFRA_A2A | 跨域违规: D_GOVERNANCE -> D_INFRA_A2A | cross_domain_violation | D_GOVERNANCE | D_INFRA_A2A | error | gate | 跨域依赖未声明: D_GOVERNANCE -> D_INFRA_A2A |
| V-CROSS-D_GOVERNANCE-D_INFRA_RECOVERY | 跨域违规: D_GOVERNANCE -> D_INFRA_RECOVERY | cross_domain_violation | D_GOVERNANCE | D_INFRA_RECOVERY | error | gate | 跨域依赖未声明: D_GOVERNANCE -> D_INFRA_RECOVERY |
| V-CROSS-D_GOVERNANCE-D_PF_ALLOC | 跨域违规: D_GOVERNANCE -> D_PF_ALLOC | cross_domain_violation | D_GOVERNANCE | D_PF_ALLOC | error | gate | 跨域依赖未声明: D_GOVERNANCE -> D_PF_ALLOC |
| V-CROSS-D_GOVERNANCE-D_RISK | 跨域违规: D_GOVERNANCE -> D_RISK | cross_domain_violation | D_GOVERNANCE | D_RISK | error | gate | 跨域依赖未声明: D_GOVERNANCE -> D_RISK |
| V-CROSS-D_GOVERNANCE-D_SIMULATION | 跨域违规: D_GOVERNANCE -> D_SIMULATION | cross_domain_violation | D_GOVERNANCE | D_SIMULATION | error | gate | 跨域依赖未声明: D_GOVERNANCE -> D_SIMULATION |
| V-CROSS-D_GOV_AUDIT-D_GOV_CODE_QUALITY | 跨域违规: D_GOV_AUDIT -> D_GOV_CODE_QUALITY | cross_domain_violation | D_GOV_AUDIT | D_GOV_CODE_QUALITY | error | gate | 跨域依赖未声明: D_GOV_AUDIT -> D_GOV_CODE_QUALITY |
| V-CROSS-D_GOV_AUDIT-D_INFRA_RECOVERY | 跨域违规: D_GOV_AUDIT -> D_INFRA_RECOVERY | cross_domain_violation | D_GOV_AUDIT | D_INFRA_RECOVERY | error | gate | 跨域依赖未声明: D_GOV_AUDIT -> D_INFRA_RECOVERY |
| V-CROSS-D_GOV_AUDIT-D_OPS | 跨域违规: D_GOV_AUDIT -> D_OPS | cross_domain_violation | D_GOV_AUDIT | D_OPS | error | gate | 跨域依赖未声明: D_GOV_AUDIT -> D_OPS |
| V-CROSS-D_GOV_AUDIT-D_TRADING | 跨域违规: D_GOV_AUDIT -> D_TRADING | cross_domain_violation | D_GOV_AUDIT | D_TRADING | error | gate | 跨域依赖未声明: D_GOV_AUDIT -> D_TRADING |
| V-CROSS-D_GOV_CODE_QUALITY-D_DATA | 跨域违规: D_GOV_CODE_QUALITY -> D_DATA | cross_domain_violation | D_GOV_CODE_QUALITY | D_DATA | error | gate | 跨域依赖未声明: D_GOV_CODE_QUALITY -> D_DATA |
| V-CROSS-D_GOV_CODE_QUALITY-D_FBL_VERIFICATION | 跨域违规: D_GOV_CODE_QUALITY -> D_FBL_VERIFICATION | cross_domain_violation | D_GOV_CODE_QUALITY | D_FBL_VERIFICATION | error | gate | 跨域依赖未声明: D_GOV_CODE_QUALITY -> D_FBL_VERIFICATION |
| V-CROSS-D_GOV_CODE_QUALITY-D_GOV_AUDIT | 跨域违规: D_GOV_CODE_QUALITY -> D_GOV_AUDIT | cross_domain_violation | D_GOV_CODE_QUALITY | D_GOV_AUDIT | error | gate | 跨域依赖未声明: D_GOV_CODE_QUALITY -> D_GOV_AUDIT |
| V-CROSS-D_GOV_CODE_QUALITY-D_GOV_DRIFT | 跨域违规: D_GOV_CODE_QUALITY -> D_GOV_DRIFT | cross_domain_violation | D_GOV_CODE_QUALITY | D_GOV_DRIFT | error | gate | 跨域依赖未声明: D_GOV_CODE_QUALITY -> D_GOV_DRIFT |
| V-CROSS-D_GOV_CODE_QUALITY-D_GOV_OPS_RESILIENCE | 跨域违规: D_GOV_CODE_QUALITY -> D_GOV_OPS_RESILIENCE | cross_domain_violation | D_GOV_CODE_QUALITY | D_GOV_OPS_RESILIENCE | error | gate | 跨域依赖未声明: D_GOV_CODE_QUALITY -> D_GOV_OPS_RESILIENCE |
| V-CROSS-D_GOV_CODE_QUALITY-D_GOV_RULE | 跨域违规: D_GOV_CODE_QUALITY -> D_GOV_RULE | cross_domain_violation | D_GOV_CODE_QUALITY | D_GOV_RULE | error | gate | 跨域依赖未声明: D_GOV_CODE_QUALITY -> D_GOV_RULE |
| V-CROSS-D_GOV_CODE_QUALITY-D_GOV_SCRIPTS | 跨域违规: D_GOV_CODE_QUALITY -> D_GOV_SCRIPTS | cross_domain_violation | D_GOV_CODE_QUALITY | D_GOV_SCRIPTS | error | gate | 跨域依赖未声明: D_GOV_CODE_QUALITY -> D_GOV_SCRIPTS |
| V-CROSS-D_GOV_CODE_QUALITY-D_SECURITY | 跨域违规: D_GOV_CODE_QUALITY -> D_SECURITY | cross_domain_violation | D_GOV_CODE_QUALITY | D_SECURITY | error | gate | 跨域依赖未声明: D_GOV_CODE_QUALITY -> D_SECURITY |
| V-CROSS-D_GOV_DOCS-D_GOV_SCRIPTS | 跨域违规: D_GOV_DOCS -> D_GOV_SCRIPTS | cross_domain_violation | D_GOV_DOCS | D_GOV_SCRIPTS | error | gate | 跨域依赖未声明: D_GOV_DOCS -> D_GOV_SCRIPTS |
| V-CROSS-D_GOV_DRIFT-D_SECURITY | 跨域违规: D_GOV_DRIFT -> D_SECURITY | cross_domain_violation | D_GOV_DRIFT | D_SECURITY | error | gate | 跨域依赖未声明: D_GOV_DRIFT -> D_SECURITY |
| V-CROSS-D_GOV_ENFORCEMENT-D_GOV_CODE_QUALITY | 跨域违规: D_GOV_ENFORCEMENT -> D_GOV_CODE_QUALITY | cross_domain_violation | D_GOV_ENFORCEMENT | D_GOV_CODE_QUALITY | error | gate | 跨域依赖未声明: D_GOV_ENFORCEMENT -> D_GOV_CODE_QUALITY |
| V-CROSS-D_GOV_ENFORCEMENT-D_GOV_RULE | 跨域违规: D_GOV_ENFORCEMENT -> D_GOV_RULE | cross_domain_violation | D_GOV_ENFORCEMENT | D_GOV_RULE | error | gate | 跨域依赖未声明: D_GOV_ENFORCEMENT -> D_GOV_RULE |
| V-CROSS-D_GOV_ENFORCEMENT-D_GOV_SCRIPTS | 跨域违规: D_GOV_ENFORCEMENT -> D_GOV_SCRIPTS | cross_domain_violation | D_GOV_ENFORCEMENT | D_GOV_SCRIPTS | error | gate | 跨域依赖未声明: D_GOV_ENFORCEMENT -> D_GOV_SCRIPTS |
| V-CROSS-D_GOV_OPS_RESILIENCE-D_GOV_CODE_QUALITY | 跨域违规: D_GOV_OPS_RESILIENCE -> D_GOV_CODE_QUALITY | cross_domain_violation | D_GOV_OPS_RESILIENCE | D_GOV_CODE_QUALITY | error | gate | 跨域依赖未声明: D_GOV_OPS_RESILIENCE -> D_GOV_CODE_QUALITY |
| V-CROSS-D_GOV_OPS_RESILIENCE-D_GOV_REPAIR | 跨域违规: D_GOV_OPS_RESILIENCE -> D_GOV_REPAIR | cross_domain_violation | D_GOV_OPS_RESILIENCE | D_GOV_REPAIR | error | gate | 跨域依赖未声明: D_GOV_OPS_RESILIENCE -> D_GOV_REPAIR |
| V-CROSS-D_GOV_RULE-D_GOV_SCRIPTS | 跨域违规: D_GOV_RULE -> D_GOV_SCRIPTS | cross_domain_violation | D_GOV_RULE | D_GOV_SCRIPTS | error | gate | 跨域依赖未声明: D_GOV_RULE -> D_GOV_SCRIPTS |
| V-CROSS-D_GOV_SCRIPTS-D_GOV_CODE_QUALITY | 跨域违规: D_GOV_SCRIPTS -> D_GOV_CODE_QUALITY | cross_domain_violation | D_GOV_SCRIPTS | D_GOV_CODE_QUALITY | error | gate | 跨域依赖未声明: D_GOV_SCRIPTS -> D_GOV_CODE_QUALITY |
| V-CROSS-D_GOV_SCRIPTS-D_GOV_DRIFT | 跨域违规: D_GOV_SCRIPTS -> D_GOV_DRIFT | cross_domain_violation | D_GOV_SCRIPTS | D_GOV_DRIFT | error | gate | 跨域依赖未声明: D_GOV_SCRIPTS -> D_GOV_DRIFT |
| V-CROSS-D_GOV_SCRIPTS-D_GOV_OPS_RESILIENCE | 跨域违规: D_GOV_SCRIPTS -> D_GOV_OPS_RESILIENCE | cross_domain_violation | D_GOV_SCRIPTS | D_GOV_OPS_RESILIENCE | error | gate | 跨域依赖未声明: D_GOV_SCRIPTS -> D_GOV_OPS_RESILIENCE |
| V-CROSS-D_GOV_SCRIPTS-D_GOV_REPAIR | 跨域违规: D_GOV_SCRIPTS -> D_GOV_REPAIR | cross_domain_violation | D_GOV_SCRIPTS | D_GOV_REPAIR | error | gate | 跨域依赖未声明: D_GOV_SCRIPTS -> D_GOV_REPAIR |
| V-CROSS-D_GOV_SCRIPTS-D_GOV_RULE | 跨域违规: D_GOV_SCRIPTS -> D_GOV_RULE | cross_domain_violation | D_GOV_SCRIPTS | D_GOV_RULE | error | gate | 跨域依赖未声明: D_GOV_SCRIPTS -> D_GOV_RULE |
| V-CROSS-D_GOV_SCRIPTS-D_ORCHESTRATOR | 跨域违规: D_GOV_SCRIPTS -> D_ORCHESTRATOR | cross_domain_violation | D_GOV_SCRIPTS | D_ORCHESTRATOR | error | gate | 跨域依赖未声明: D_GOV_SCRIPTS -> D_ORCHESTRATOR |
| V-CROSS-D_INFRASTRUCTURE-D_SHARED | 跨域违规: D_INFRASTRUCTURE -> D_SHARED | cross_domain_violation | D_INFRASTRUCTURE | D_SHARED | error | gate | 跨域依赖未声明: D_INFRASTRUCTURE -> D_SHARED |
| V-CROSS-D_INFRA_OPS-D_DATA | 跨域违规: D_INFRA_OPS -> D_DATA | cross_domain_violation | D_INFRA_OPS | D_DATA | error | gate | 跨域依赖未声明: D_INFRA_OPS -> D_DATA |
| V-CROSS-D_INFRA_OPS-D_GOV_AUDIT | 跨域违规: D_INFRA_OPS -> D_GOV_AUDIT | cross_domain_violation | D_INFRA_OPS | D_GOV_AUDIT | error | gate | 跨域依赖未声明: D_INFRA_OPS -> D_GOV_AUDIT |
| V-CROSS-D_INFRA_RECOVERY-D_SECURITY | 跨域违规: D_INFRA_RECOVERY -> D_SECURITY | cross_domain_violation | D_INFRA_RECOVERY | D_SECURITY | error | gate | 跨域依赖未声明: D_INFRA_RECOVERY -> D_SECURITY |
| V-CROSS-D_INFRA_RUNTIME-D_AUTONOMY_CORE | 跨域违规: D_INFRA_RUNTIME -> D_AUTONOMY_CORE | cross_domain_violation | D_INFRA_RUNTIME | D_AUTONOMY_CORE | error | gate | 跨域依赖未声明: D_INFRA_RUNTIME -> D_AUTONOMY_CORE |
| V-CROSS-D_INFRA_RUNTIME-D_DATA | 跨域违规: D_INFRA_RUNTIME -> D_DATA | cross_domain_violation | D_INFRA_RUNTIME | D_DATA | error | gate | 跨域依赖未声明: D_INFRA_RUNTIME -> D_DATA |
| V-CROSS-D_INFRA_RUNTIME-D_FACTOR | 跨域违规: D_INFRA_RUNTIME -> D_FACTOR | cross_domain_violation | D_INFRA_RUNTIME | D_FACTOR | error | gate | 跨域依赖未声明: D_INFRA_RUNTIME -> D_FACTOR |
| V-CROSS-D_INFRA_RUNTIME-D_FEEDBACK_LOOP | 跨域违规: D_INFRA_RUNTIME -> D_FEEDBACK_LOOP | cross_domain_violation | D_INFRA_RUNTIME | D_FEEDBACK_LOOP | error | gate | 跨域依赖未声明: D_INFRA_RUNTIME -> D_FEEDBACK_LOOP |
| V-CROSS-D_INFRA_RUNTIME-D_GOV_REPAIR | 跨域违规: D_INFRA_RUNTIME -> D_GOV_REPAIR | cross_domain_violation | D_INFRA_RUNTIME | D_GOV_REPAIR | error | gate | 跨域依赖未声明: D_INFRA_RUNTIME -> D_GOV_REPAIR |
| V-CROSS-D_INFRA_RUNTIME-D_GOV_SCRIPTS | 跨域违规: D_INFRA_RUNTIME -> D_GOV_SCRIPTS | cross_domain_violation | D_INFRA_RUNTIME | D_GOV_SCRIPTS | error | gate | 跨域依赖未声明: D_INFRA_RUNTIME -> D_GOV_SCRIPTS |
| V-CROSS-D_INFRA_RUNTIME-D_INFRASTRUCTURE | 跨域违规: D_INFRA_RUNTIME -> D_INFRASTRUCTURE | cross_domain_violation | D_INFRA_RUNTIME | D_INFRASTRUCTURE | error | gate | 跨域依赖未声明: D_INFRA_RUNTIME -> D_INFRASTRUCTURE |
| V-CROSS-D_INFRA_RUNTIME-D_INFRA_RECOVERY | 跨域违规: D_INFRA_RUNTIME -> D_INFRA_RECOVERY | cross_domain_violation | D_INFRA_RUNTIME | D_INFRA_RECOVERY | error | gate | 跨域依赖未声明: D_INFRA_RUNTIME -> D_INFRA_RECOVERY |
| V-CROSS-D_INFRA_RUNTIME-D_INTEGRATION | 跨域违规: D_INFRA_RUNTIME -> D_INTEGRATION | cross_domain_violation | D_INFRA_RUNTIME | D_INTEGRATION | error | gate | 跨域依赖未声明: D_INFRA_RUNTIME -> D_INTEGRATION |
| V-CROSS-D_INFRA_RUNTIME-D_INTELLIGENCE | 跨域违规: D_INFRA_RUNTIME -> D_INTELLIGENCE | cross_domain_violation | D_INFRA_RUNTIME | D_INTELLIGENCE | error | gate | 跨域依赖未声明: D_INFRA_RUNTIME -> D_INTELLIGENCE |
| V-CROSS-D_INFRA_RUNTIME-D_OPS | 跨域违规: D_INFRA_RUNTIME -> D_OPS | cross_domain_violation | D_INFRA_RUNTIME | D_OPS | error | gate | 跨域依赖未声明: D_INFRA_RUNTIME -> D_OPS |
| V-CROSS-D_INFRA_RUNTIME-D_ORCHESTRATOR | 跨域违规: D_INFRA_RUNTIME -> D_ORCHESTRATOR | cross_domain_violation | D_INFRA_RUNTIME | D_ORCHESTRATOR | error | gate | 跨域依赖未声明: D_INFRA_RUNTIME -> D_ORCHESTRATOR |
| V-CROSS-D_INFRA_RUNTIME-D_SECURITY | 跨域违规: D_INFRA_RUNTIME -> D_SECURITY | cross_domain_violation | D_INFRA_RUNTIME | D_SECURITY | error | gate | 跨域依赖未声明: D_INFRA_RUNTIME -> D_SECURITY |
| V-CROSS-D_INFRA_RUNTIME-D_TRADING | 跨域违规: D_INFRA_RUNTIME -> D_TRADING | cross_domain_violation | D_INFRA_RUNTIME | D_TRADING | error | gate | 跨域依赖未声明: D_INFRA_RUNTIME -> D_TRADING |
| V-CROSS-D_INTEGRATION-D_TRADING | 跨域违规: D_INTEGRATION -> D_TRADING | cross_domain_violation | D_INTEGRATION | D_TRADING | error | gate | 跨域依赖未声明: D_INTEGRATION -> D_TRADING |
| V-CROSS-D_MKT_DATA-D_DATA | 跨域违规: D_MKT_DATA -> D_DATA | cross_domain_violation | D_MKT_DATA | D_DATA | error | gate | 跨域依赖未声明: D_MKT_DATA -> D_DATA |
| V-CROSS-D_MKT_DATA-D_SHARED | 跨域违规: D_MKT_DATA -> D_SHARED | cross_domain_violation | D_MKT_DATA | D_SHARED | error | gate | 跨域依赖未声明: D_MKT_DATA -> D_SHARED |
| V-CROSS-D_OPS-D_GOV_SCRIPTS | 跨域违规: D_OPS -> D_GOV_SCRIPTS | cross_domain_violation | D_OPS | D_GOV_SCRIPTS | error | gate | 跨域依赖未声明: D_OPS -> D_GOV_SCRIPTS |
| V-CROSS-D_ORCHESTRATOR-D_SECURITY | 跨域违规: D_ORCHESTRATOR -> D_SECURITY | cross_domain_violation | D_ORCHESTRATOR | D_SECURITY | error | gate | 跨域依赖未声明: D_ORCHESTRATOR -> D_SECURITY |
| V-CROSS-D_PF_ALLOC-D_SHARED | 跨域违规: D_PF_ALLOC -> D_SHARED | cross_domain_violation | D_PF_ALLOC | D_SHARED | error | gate | 跨域依赖未声明: D_PF_ALLOC -> D_SHARED |
| V-CROSS-D_PF_CORE-D_BACKTEST | 跨域违规: D_PF_CORE -> D_BACKTEST | cross_domain_violation | D_PF_CORE | D_BACKTEST | error | gate | 跨域依赖未声明: D_PF_CORE -> D_BACKTEST |
| V-CROSS-D_PF_CORE-D_FACTOR | 跨域违规: D_PF_CORE -> D_FACTOR | cross_domain_violation | D_PF_CORE | D_FACTOR | error | gate | 跨域依赖未声明: D_PF_CORE -> D_FACTOR |
| V-CROSS-D_PF_CORE-D_GOVERNANCE | 跨域违规: D_PF_CORE -> D_GOVERNANCE | cross_domain_violation | D_PF_CORE | D_GOVERNANCE | error | gate | 跨域依赖未声明: D_PF_CORE -> D_GOVERNANCE |
| V-CROSS-D_PF_CORE-D_INFRASTRUCTURE | 跨域违规: D_PF_CORE -> D_INFRASTRUCTURE | cross_domain_violation | D_PF_CORE | D_INFRASTRUCTURE | error | gate | 跨域依赖未声明: D_PF_CORE -> D_INFRASTRUCTURE |
| V-CROSS-D_PF_CORE-D_REPORTING | 跨域违规: D_PF_CORE -> D_REPORTING | cross_domain_violation | D_PF_CORE | D_REPORTING | error | gate | 跨域依赖未声明: D_PF_CORE -> D_REPORTING |
| V-CROSS-D_PF_CORE-D_RISK | 跨域违规: D_PF_CORE -> D_RISK | cross_domain_violation | D_PF_CORE | D_RISK | error | gate | 跨域依赖未声明: D_PF_CORE -> D_RISK |
| V-CROSS-D_PF_CORE-D_SHARED | 跨域违规: D_PF_CORE -> D_SHARED | cross_domain_violation | D_PF_CORE | D_SHARED | error | gate | 跨域依赖未声明: D_PF_CORE -> D_SHARED |
| V-CROSS-D_POSITION-D_INFRASTRUCTURE | 跨域违规: D_POSITION -> D_INFRASTRUCTURE | cross_domain_violation | D_POSITION | D_INFRASTRUCTURE | error | gate | 跨域依赖未声明: D_POSITION -> D_INFRASTRUCTURE |
| V-CROSS-D_POSITION-D_SHARED | 跨域违规: D_POSITION -> D_SHARED | cross_domain_violation | D_POSITION | D_SHARED | error | gate | 跨域依赖未声明: D_POSITION -> D_SHARED |
| V-CROSS-D_REPORTING-D_EX_CORE | 跨域违规: D_REPORTING -> D_EX_CORE | cross_domain_violation | D_REPORTING | D_EX_CORE | error | gate | 跨域依赖未声明: D_REPORTING -> D_EX_CORE |
| V-CROSS-D_REPORTING-D_SHARED | 跨域违规: D_REPORTING -> D_SHARED | cross_domain_violation | D_REPORTING | D_SHARED | error | gate | 跨域依赖未声明: D_REPORTING -> D_SHARED |
| V-CROSS-D_REPORTING-D_TRADING | 跨域违规: D_REPORTING -> D_TRADING | cross_domain_violation | D_REPORTING | D_TRADING | error | gate | 跨域依赖未声明: D_REPORTING -> D_TRADING |
| V-CROSS-D_RISK-D_SHARED | 跨域违规: D_RISK -> D_SHARED | cross_domain_violation | D_RISK | D_SHARED | error | gate | 跨域依赖未声明: D_RISK -> D_SHARED |
| V-CROSS-D_SELL_DECISION-D_SHARED | 跨域违规: D_SELL_DECISION -> D_SHARED | cross_domain_violation | D_SELL_DECISION | D_SHARED | error | gate | 跨域依赖未声明: D_SELL_DECISION -> D_SHARED |
| V-CROSS-D_SHARED-D_INFRA_RUNTIME | 跨域违规: D_SHARED -> D_INFRA_RUNTIME | cross_domain_violation | D_SHARED | D_INFRA_RUNTIME | error | gate | 跨域依赖未声明: D_SHARED -> D_INFRA_RUNTIME |
| V-CROSS-D_SHARED-D_ML_TRAIN | 跨域违规: D_SHARED -> D_ML_TRAIN | cross_domain_violation | D_SHARED | D_ML_TRAIN | error | gate | 跨域依赖未声明: D_SHARED -> D_ML_TRAIN |
| V-CROSS-D_SIMULATION-D_SHARED | 跨域违规: D_SIMULATION -> D_SHARED | cross_domain_violation | D_SIMULATION | D_SHARED | error | gate | 跨域依赖未声明: D_SIMULATION -> D_SHARED |
| V-HARD150-D_DATA | 硬上限违规: D_DATA | hard_limit_exceeded | D_DATA |  | error | gate | 域 D_DATA(数据接入层) production 节点 172 超过硬上限 150 (ARCH-CAP-002 v1... |
| V-HARD150-D_GOVERNANCE | 硬上限违规: D_GOVERNANCE | hard_limit_exceeded | D_GOVERNANCE |  | error | gate | 域 D_GOVERNANCE(生命周期管理) production 节点 451 超过硬上限 150 (ARCH-CAP... |
| V-HARD150-D_GOV_AUDIT | 硬上限违规: D_GOV_AUDIT | hard_limit_exceeded | D_GOV_AUDIT |  | error | gate | 域 D_GOV_AUDIT(审计追踪) production 节点 192 超过硬上限 150 (ARCH-CAP-00... |
| V-HARD150-D_GOV_CODE_QUALITY | 硬上限违规: D_GOV_CODE_QUALITY | hard_limit_exceeded | D_GOV_CODE_QUALITY |  | error | gate | 域 D_GOV_CODE_QUALITY(代码质量治理) production 节点 215 超过硬上限 150 (AR... |
| V-HARD150-D_GOV_SCRIPTS | 硬上限违规: D_GOV_SCRIPTS | hard_limit_exceeded | D_GOV_SCRIPTS |  | error | gate | 域 D_GOV_SCRIPTS(脚本治理) production 节点 422 超过硬上限 150 (ARCH-CAP-... |
| V-HARD150-D_INFRA_RUNTIME | 硬上限违规: D_INFRA_RUNTIME | hard_limit_exceeded | D_INFRA_RUNTIME |  | error | gate | 域 D_INFRA_RUNTIME(运行时集成) production 节点 171 超过硬上限 150 (ARCH-C... |
| V-HARD150-D_SECURITY | 硬上限违规: D_SECURITY | hard_limit_exceeded | D_SECURITY |  | error | gate | 域 D_SECURITY(对抗验证) production 节点 171 超过硬上限 150 (ARCH-CAP-002... |
| V-HARD150-D_SHARED | 硬上限违规: D_SHARED | hard_limit_exceeded | D_SHARED |  | error | gate | 域 D_SHARED(共享服务) production 节点 177 超过硬上限 150 (ARCH-CAP-002 v... |
| V-LAYER-D_GOV_CODE_QUALITY-D_GOV_SCRIPTS | 层级违规: L1_foundation -> L2_domain | layer_violation | D_GOV_CODE_QUALITY | D_GOV_SCRIPTS | error | gate | 层级违规: 8730995 -> 8731152 (L1_foundation -> L2_domain) |
| V-LAYER-D_INFRA_OPS-D_DATA | 层级违规: L0_infrastructure -> L1_foundation | layer_violation | D_INFRA_OPS | D_DATA | error | gate | 层级违规: 8731186 -> 8731179 (L0_infrastructure -> L1_foundation... |
| V-LAYER-D_INFRA_RUNTIME-D_AUTONOMY_CORE | 层级违规: L0_infrastructure -> L1_foundation | layer_violation | D_INFRA_RUNTIME | D_AUTONOMY_CORE | error | gate | 层级违规: 8730530 -> 8728020 (L0_infrastructure -> L1_foundation... |
| V-LAYER-D_INFRA_RUNTIME-D_DATA | 层级违规: L0_infrastructure -> L1_foundation | layer_violation | D_INFRA_RUNTIME | D_DATA | error | gate | 层级违规: 8730001 -> 8728111 (L0_infrastructure -> L1_foundation... |
| V-LAYER-D_INFRA_RUNTIME-D_FACTOR | 层级违规: L0_infrastructure -> L2_domain | layer_violation | D_INFRA_RUNTIME | D_FACTOR | error | gate | 层级违规: 8730001 -> 8728256 (L0_infrastructure -> L2_domain) |
| V-LAYER-D_INFRA_RUNTIME-D_GOV_REPAIR | 层级违规: L0_infrastructure -> L2_domain | layer_violation | D_INFRA_RUNTIME | D_GOV_REPAIR | error | gate | 层级违规: 8729502 -> 8728780 (L0_infrastructure -> L2_domain) |
| V-LAYER-D_INFRA_RUNTIME-D_GOV_SCRIPTS | 层级违规: L0_infrastructure -> L2_domain | layer_violation | D_INFRA_RUNTIME | D_GOV_SCRIPTS | error | gate | 层级违规: 8730530 -> 8730714 (L0_infrastructure -> L2_domain) |
| V-LAYER-D_INFRA_RUNTIME-D_INTEGRATION | 层级违规: L0_infrastructure -> L1_foundation | layer_violation | D_INFRA_RUNTIME | D_INTEGRATION | error | gate | 层级违规: 8730550 -> 8730257 (L0_infrastructure -> L1_foundation... |
| V-LAYER-D_INFRA_RUNTIME-D_OPS | 层级违规: L0_infrastructure -> L1_foundation | layer_violation | D_INFRA_RUNTIME | D_OPS | error | gate | 层级违规: 8730530 -> 8728838 (L0_infrastructure -> L1_foundation... |
| V-LAYER-D_INFRA_RUNTIME-D_ORCHESTRATOR | 层级违规: L0_infrastructure -> L1_foundation | layer_violation | D_INFRA_RUNTIME | D_ORCHESTRATOR | error | gate | 层级违规: 8730530 -> 8729851 (L0_infrastructure -> L1_foundation... |
| V-LAYER-D_INFRA_RUNTIME-D_TRADING | 层级违规: L0_infrastructure -> L2_domain | layer_violation | D_INFRA_RUNTIME | D_TRADING | error | gate | 层级违规: 8730546 -> 8730540 (L0_infrastructure -> L2_domain) |
| V-LAYER-D_INTEGRATION-D_TRADING | 层级违规: L1_foundation -> L2_domain | layer_violation | D_INTEGRATION | D_TRADING | error | gate | 层级违规: 8729678 -> 8730519 (L1_foundation -> L2_domain) |
| V-LAYER-D_OPS-D_GOV_SCRIPTS | 层级违规: L1_foundation -> L2_domain | layer_violation | D_OPS | D_GOV_SCRIPTS | error | gate | 层级违规: 8731078 -> 8731151 (L1_foundation -> L2_domain) |
| V-LAYER-D_REPORTING-D_EX_CORE | 层级违规: L1_foundation -> L2_domain | layer_violation | D_REPORTING | D_EX_CORE | error | gate | 层级违规: 8729953 -> 8728229 (L1_foundation -> L2_domain) |
| V-LAYER-D_REPORTING-D_TRADING | 层级违规: L1_foundation -> L2_domain | layer_violation | D_REPORTING | D_TRADING | error | gate | 层级违规: 8729953 -> 8730545 (L1_foundation -> L2_domain) |
| V-LAYER-D_SHARED-D_ML_TRAIN | 层级违规: L0_infrastructure -> L2_domain | layer_violation | D_SHARED | D_ML_TRAIN | error | gate | 层级违规: 8730452 -> 8729809 (L0_infrastructure -> L2_domain) |

## 完整约束清单

| 约束ID / Constraint ID | 名称 / Name | 类型 / Type | 源域 / From Domain | 目标域 / To Domain | 严重程度 / Severity | 状态 / Status |
|--------|------|------|------|--------|---------|------|
| V-CAP-D_DATA | 容量超限: D_DATA | capacity_exceeded | D_DATA |  | hard | open |
| V-CAP-D_GOVERNANCE | 容量超限: D_GOVERNANCE | capacity_exceeded | D_GOVERNANCE |  | hard | open |
| V-CAP-D_GOV_AUDIT | 容量超限: D_GOV_AUDIT | capacity_exceeded | D_GOV_AUDIT |  | hard | open |
| V-CAP-D_GOV_CODE_QUALITY | 容量超限: D_GOV_CODE_QUALITY | capacity_exceeded | D_GOV_CODE_QUALITY |  | hard | open |
| V-CAP-D_GOV_SCRIPTS | 容量超限: D_GOV_SCRIPTS | capacity_exceeded | D_GOV_SCRIPTS |  | hard | open |
| V-CAP-D_INFRA_RUNTIME | 容量超限: D_INFRA_RUNTIME | capacity_exceeded | D_INFRA_RUNTIME |  | hard | open |
| V-CAP-D_SECURITY | 容量超限: D_SECURITY | capacity_exceeded | D_SECURITY |  | hard | open |
| V-CAP-D_SHARED | 容量超限: D_SHARED | capacity_exceeded | D_SHARED |  | hard | open |
|  | procedural policy 必须可验证（不能是 inspection） | architecture_contract |  |  | error | open |
| V-CROSS-D_AUTONOMY_CORE-D_SECURITY | 跨域违规: D_AUTONOMY_CORE -> D_SECURITY | cross_domain_violation | D_AUTONOMY_CORE | D_SECURITY | error | open |
| V-CROSS-D_AUTONOMY_PERM-D_GOV_SCRIPTS | 跨域违规: D_AUTONOMY_PERM -> D_GOV_SCRIPTS | cross_domain_violation | D_AUTONOMY_PERM | D_GOV_SCRIPTS | error | open |
| V-CROSS-D_BACKTEST-D_FACTOR | 跨域违规: D_BACKTEST -> D_FACTOR | cross_domain_violation | D_BACKTEST | D_FACTOR | error | open |
| V-CROSS-D_BACKTEST-D_INFRA_RUNTIME | 跨域违规: D_BACKTEST -> D_INFRA_RUNTIME | cross_domain_violation | D_BACKTEST | D_INFRA_RUNTIME | error | open |
| V-CROSS-D_COMPLIANCE-D_SECURITY | 跨域违规: D_COMPLIANCE -> D_SECURITY | cross_domain_violation | D_COMPLIANCE | D_SECURITY | error | open |
| V-CROSS-D_EX_CORE-D_FACTOR | 跨域违规: D_EX_CORE -> D_FACTOR | cross_domain_violation | D_EX_CORE | D_FACTOR | error | open |
| V-CROSS-D_EX_SOR-D_INFRASTRUCTURE | 跨域违规: D_EX_SOR -> D_INFRASTRUCTURE | cross_domain_violation | D_EX_SOR | D_INFRASTRUCTURE | error | open |
| V-CROSS-D_EX_SOR-D_SHARED | 跨域违规: D_EX_SOR -> D_SHARED | cross_domain_violation | D_EX_SOR | D_SHARED | error | open |
| V-CROSS-D_FACTOR-D_INFRA_RUNTIME | 跨域违规: D_FACTOR -> D_INFRA_RUNTIME | cross_domain_violation | D_FACTOR | D_INFRA_RUNTIME | error | open |
| V-CROSS-D_FACTOR-D_SHARED | 跨域违规: D_FACTOR -> D_SHARED | cross_domain_violation | D_FACTOR | D_SHARED | error | open |
| V-CROSS-D_FEEDBACK_LOOP-D_SECURITY | 跨域违规: D_FEEDBACK_LOOP -> D_SECURITY | cross_domain_violation | D_FEEDBACK_LOOP | D_SECURITY | error | open |
| V-CROSS-D_GOVERNANCE-D_AUTONOMY_CORE | 跨域违规: D_GOVERNANCE -> D_AUTONOMY_CORE | cross_domain_violation | D_GOVERNANCE | D_AUTONOMY_CORE | error | open |
| V-CROSS-D_GOVERNANCE-D_BACKTEST | 跨域违规: D_GOVERNANCE -> D_BACKTEST | cross_domain_violation | D_GOVERNANCE | D_BACKTEST | error | open |
| V-CROSS-D_GOVERNANCE-D_EX_CORE | 跨域违规: D_GOVERNANCE -> D_EX_CORE | cross_domain_violation | D_GOVERNANCE | D_EX_CORE | error | open |
| V-CROSS-D_GOVERNANCE-D_FACTOR | 跨域违规: D_GOVERNANCE -> D_FACTOR | cross_domain_violation | D_GOVERNANCE | D_FACTOR | error | open |
| V-CROSS-D_GOVERNANCE-D_FBL_DETECTORS | 跨域违规: D_GOVERNANCE -> D_FBL_DETECTORS | cross_domain_violation | D_GOVERNANCE | D_FBL_DETECTORS | error | open |
| V-CROSS-D_GOVERNANCE-D_FBL_DIAGNOSERS | 跨域违规: D_GOVERNANCE -> D_FBL_DIAGNOSERS | cross_domain_violation | D_GOVERNANCE | D_FBL_DIAGNOSERS | error | open |
| V-CROSS-D_GOVERNANCE-D_FEEDBACK_LOOP | 跨域违规: D_GOVERNANCE -> D_FEEDBACK_LOOP | cross_domain_violation | D_GOVERNANCE | D_FEEDBACK_LOOP | error | open |
| V-CROSS-D_GOVERNANCE-D_FUNDAMENTAL_SIGNAL | 跨域违规: D_GOVERNANCE -> D_FUNDAMENTAL_SIGNAL | cross_domain_violation | D_GOVERNANCE | D_FUNDAMENTAL_SIGNAL | error | open |
| V-CROSS-D_GOVERNANCE-D_GOV_DRIFT | 跨域违规: D_GOVERNANCE -> D_GOV_DRIFT | cross_domain_violation | D_GOVERNANCE | D_GOV_DRIFT | error | open |
| V-CROSS-D_GOVERNANCE-D_INFRA_A2A | 跨域违规: D_GOVERNANCE -> D_INFRA_A2A | cross_domain_violation | D_GOVERNANCE | D_INFRA_A2A | error | open |
| V-CROSS-D_GOVERNANCE-D_INFRA_RECOVERY | 跨域违规: D_GOVERNANCE -> D_INFRA_RECOVERY | cross_domain_violation | D_GOVERNANCE | D_INFRA_RECOVERY | error | open |
| V-CROSS-D_GOVERNANCE-D_PF_ALLOC | 跨域违规: D_GOVERNANCE -> D_PF_ALLOC | cross_domain_violation | D_GOVERNANCE | D_PF_ALLOC | error | open |
| V-CROSS-D_GOVERNANCE-D_RISK | 跨域违规: D_GOVERNANCE -> D_RISK | cross_domain_violation | D_GOVERNANCE | D_RISK | error | open |
| V-CROSS-D_GOVERNANCE-D_SIMULATION | 跨域违规: D_GOVERNANCE -> D_SIMULATION | cross_domain_violation | D_GOVERNANCE | D_SIMULATION | error | open |
| V-CROSS-D_GOV_AUDIT-D_GOV_CODE_QUALITY | 跨域违规: D_GOV_AUDIT -> D_GOV_CODE_QUALITY | cross_domain_violation | D_GOV_AUDIT | D_GOV_CODE_QUALITY | error | open |
| V-CROSS-D_GOV_AUDIT-D_INFRA_RECOVERY | 跨域违规: D_GOV_AUDIT -> D_INFRA_RECOVERY | cross_domain_violation | D_GOV_AUDIT | D_INFRA_RECOVERY | error | open |
| V-CROSS-D_GOV_AUDIT-D_OPS | 跨域违规: D_GOV_AUDIT -> D_OPS | cross_domain_violation | D_GOV_AUDIT | D_OPS | error | open |
| V-CROSS-D_GOV_AUDIT-D_TRADING | 跨域违规: D_GOV_AUDIT -> D_TRADING | cross_domain_violation | D_GOV_AUDIT | D_TRADING | error | open |
| V-CROSS-D_GOV_CODE_QUALITY-D_DATA | 跨域违规: D_GOV_CODE_QUALITY -> D_DATA | cross_domain_violation | D_GOV_CODE_QUALITY | D_DATA | error | open |
| V-CROSS-D_GOV_CODE_QUALITY-D_FBL_VERIFICATION | 跨域违规: D_GOV_CODE_QUALITY -> D_FBL_VERIFICATION | cross_domain_violation | D_GOV_CODE_QUALITY | D_FBL_VERIFICATION | error | open |
| V-CROSS-D_GOV_CODE_QUALITY-D_GOV_AUDIT | 跨域违规: D_GOV_CODE_QUALITY -> D_GOV_AUDIT | cross_domain_violation | D_GOV_CODE_QUALITY | D_GOV_AUDIT | error | open |
| V-CROSS-D_GOV_CODE_QUALITY-D_GOV_DRIFT | 跨域违规: D_GOV_CODE_QUALITY -> D_GOV_DRIFT | cross_domain_violation | D_GOV_CODE_QUALITY | D_GOV_DRIFT | error | open |
| V-CROSS-D_GOV_CODE_QUALITY-D_GOV_OPS_RESILIENCE | 跨域违规: D_GOV_CODE_QUALITY -> D_GOV_OPS_RESILIENCE | cross_domain_violation | D_GOV_CODE_QUALITY | D_GOV_OPS_RESILIENCE | error | open |
| V-CROSS-D_GOV_CODE_QUALITY-D_GOV_RULE | 跨域违规: D_GOV_CODE_QUALITY -> D_GOV_RULE | cross_domain_violation | D_GOV_CODE_QUALITY | D_GOV_RULE | error | open |
| V-CROSS-D_GOV_CODE_QUALITY-D_GOV_SCRIPTS | 跨域违规: D_GOV_CODE_QUALITY -> D_GOV_SCRIPTS | cross_domain_violation | D_GOV_CODE_QUALITY | D_GOV_SCRIPTS | error | open |
| V-CROSS-D_GOV_CODE_QUALITY-D_SECURITY | 跨域违规: D_GOV_CODE_QUALITY -> D_SECURITY | cross_domain_violation | D_GOV_CODE_QUALITY | D_SECURITY | error | open |
| V-CROSS-D_GOV_DOCS-D_GOV_SCRIPTS | 跨域违规: D_GOV_DOCS -> D_GOV_SCRIPTS | cross_domain_violation | D_GOV_DOCS | D_GOV_SCRIPTS | error | open |
| V-CROSS-D_GOV_DRIFT-D_SECURITY | 跨域违规: D_GOV_DRIFT -> D_SECURITY | cross_domain_violation | D_GOV_DRIFT | D_SECURITY | error | open |
| V-CROSS-D_GOV_ENFORCEMENT-D_GOV_CODE_QUALITY | 跨域违规: D_GOV_ENFORCEMENT -> D_GOV_CODE_QUALITY | cross_domain_violation | D_GOV_ENFORCEMENT | D_GOV_CODE_QUALITY | error | open |
| V-CROSS-D_GOV_ENFORCEMENT-D_GOV_RULE | 跨域违规: D_GOV_ENFORCEMENT -> D_GOV_RULE | cross_domain_violation | D_GOV_ENFORCEMENT | D_GOV_RULE | error | open |
| V-CROSS-D_GOV_ENFORCEMENT-D_GOV_SCRIPTS | 跨域违规: D_GOV_ENFORCEMENT -> D_GOV_SCRIPTS | cross_domain_violation | D_GOV_ENFORCEMENT | D_GOV_SCRIPTS | error | open |
| V-CROSS-D_GOV_OPS_RESILIENCE-D_GOV_CODE_QUALITY | 跨域违规: D_GOV_OPS_RESILIENCE -> D_GOV_CODE_QUALITY | cross_domain_violation | D_GOV_OPS_RESILIENCE | D_GOV_CODE_QUALITY | error | open |
| V-CROSS-D_GOV_OPS_RESILIENCE-D_GOV_REPAIR | 跨域违规: D_GOV_OPS_RESILIENCE -> D_GOV_REPAIR | cross_domain_violation | D_GOV_OPS_RESILIENCE | D_GOV_REPAIR | error | open |
| V-CROSS-D_GOV_RULE-D_GOV_SCRIPTS | 跨域违规: D_GOV_RULE -> D_GOV_SCRIPTS | cross_domain_violation | D_GOV_RULE | D_GOV_SCRIPTS | error | open |
| V-CROSS-D_GOV_SCRIPTS-D_GOV_CODE_QUALITY | 跨域违规: D_GOV_SCRIPTS -> D_GOV_CODE_QUALITY | cross_domain_violation | D_GOV_SCRIPTS | D_GOV_CODE_QUALITY | error | open |
| V-CROSS-D_GOV_SCRIPTS-D_GOV_DRIFT | 跨域违规: D_GOV_SCRIPTS -> D_GOV_DRIFT | cross_domain_violation | D_GOV_SCRIPTS | D_GOV_DRIFT | error | open |
| V-CROSS-D_GOV_SCRIPTS-D_GOV_OPS_RESILIENCE | 跨域违规: D_GOV_SCRIPTS -> D_GOV_OPS_RESILIENCE | cross_domain_violation | D_GOV_SCRIPTS | D_GOV_OPS_RESILIENCE | error | open |
| V-CROSS-D_GOV_SCRIPTS-D_GOV_REPAIR | 跨域违规: D_GOV_SCRIPTS -> D_GOV_REPAIR | cross_domain_violation | D_GOV_SCRIPTS | D_GOV_REPAIR | error | open |
| V-CROSS-D_GOV_SCRIPTS-D_GOV_RULE | 跨域违规: D_GOV_SCRIPTS -> D_GOV_RULE | cross_domain_violation | D_GOV_SCRIPTS | D_GOV_RULE | error | open |
| V-CROSS-D_GOV_SCRIPTS-D_ORCHESTRATOR | 跨域违规: D_GOV_SCRIPTS -> D_ORCHESTRATOR | cross_domain_violation | D_GOV_SCRIPTS | D_ORCHESTRATOR | error | open |
| V-CROSS-D_INFRASTRUCTURE-D_SHARED | 跨域违规: D_INFRASTRUCTURE -> D_SHARED | cross_domain_violation | D_INFRASTRUCTURE | D_SHARED | error | open |
| V-CROSS-D_INFRA_OPS-D_DATA | 跨域违规: D_INFRA_OPS -> D_DATA | cross_domain_violation | D_INFRA_OPS | D_DATA | error | open |
| V-CROSS-D_INFRA_OPS-D_GOV_AUDIT | 跨域违规: D_INFRA_OPS -> D_GOV_AUDIT | cross_domain_violation | D_INFRA_OPS | D_GOV_AUDIT | error | open |
| V-CROSS-D_INFRA_RECOVERY-D_SECURITY | 跨域违规: D_INFRA_RECOVERY -> D_SECURITY | cross_domain_violation | D_INFRA_RECOVERY | D_SECURITY | error | open |
| V-CROSS-D_INFRA_RUNTIME-D_AUTONOMY_CORE | 跨域违规: D_INFRA_RUNTIME -> D_AUTONOMY_CORE | cross_domain_violation | D_INFRA_RUNTIME | D_AUTONOMY_CORE | error | open |
| V-CROSS-D_INFRA_RUNTIME-D_DATA | 跨域违规: D_INFRA_RUNTIME -> D_DATA | cross_domain_violation | D_INFRA_RUNTIME | D_DATA | error | open |
| V-CROSS-D_INFRA_RUNTIME-D_FACTOR | 跨域违规: D_INFRA_RUNTIME -> D_FACTOR | cross_domain_violation | D_INFRA_RUNTIME | D_FACTOR | error | open |
| V-CROSS-D_INFRA_RUNTIME-D_FEEDBACK_LOOP | 跨域违规: D_INFRA_RUNTIME -> D_FEEDBACK_LOOP | cross_domain_violation | D_INFRA_RUNTIME | D_FEEDBACK_LOOP | error | open |
| V-CROSS-D_INFRA_RUNTIME-D_GOV_REPAIR | 跨域违规: D_INFRA_RUNTIME -> D_GOV_REPAIR | cross_domain_violation | D_INFRA_RUNTIME | D_GOV_REPAIR | error | open |
| V-CROSS-D_INFRA_RUNTIME-D_GOV_SCRIPTS | 跨域违规: D_INFRA_RUNTIME -> D_GOV_SCRIPTS | cross_domain_violation | D_INFRA_RUNTIME | D_GOV_SCRIPTS | error | open |
| V-CROSS-D_INFRA_RUNTIME-D_INFRASTRUCTURE | 跨域违规: D_INFRA_RUNTIME -> D_INFRASTRUCTURE | cross_domain_violation | D_INFRA_RUNTIME | D_INFRASTRUCTURE | error | open |
| V-CROSS-D_INFRA_RUNTIME-D_INFRA_RECOVERY | 跨域违规: D_INFRA_RUNTIME -> D_INFRA_RECOVERY | cross_domain_violation | D_INFRA_RUNTIME | D_INFRA_RECOVERY | error | open |
| V-CROSS-D_INFRA_RUNTIME-D_INTEGRATION | 跨域违规: D_INFRA_RUNTIME -> D_INTEGRATION | cross_domain_violation | D_INFRA_RUNTIME | D_INTEGRATION | error | open |
| V-CROSS-D_INFRA_RUNTIME-D_INTELLIGENCE | 跨域违规: D_INFRA_RUNTIME -> D_INTELLIGENCE | cross_domain_violation | D_INFRA_RUNTIME | D_INTELLIGENCE | error | open |
| V-CROSS-D_INFRA_RUNTIME-D_OPS | 跨域违规: D_INFRA_RUNTIME -> D_OPS | cross_domain_violation | D_INFRA_RUNTIME | D_OPS | error | open |
| V-CROSS-D_INFRA_RUNTIME-D_ORCHESTRATOR | 跨域违规: D_INFRA_RUNTIME -> D_ORCHESTRATOR | cross_domain_violation | D_INFRA_RUNTIME | D_ORCHESTRATOR | error | open |
| V-CROSS-D_INFRA_RUNTIME-D_SECURITY | 跨域违规: D_INFRA_RUNTIME -> D_SECURITY | cross_domain_violation | D_INFRA_RUNTIME | D_SECURITY | error | open |
| V-CROSS-D_INFRA_RUNTIME-D_TRADING | 跨域违规: D_INFRA_RUNTIME -> D_TRADING | cross_domain_violation | D_INFRA_RUNTIME | D_TRADING | error | open |
| V-CROSS-D_INTEGRATION-D_TRADING | 跨域违规: D_INTEGRATION -> D_TRADING | cross_domain_violation | D_INTEGRATION | D_TRADING | error | open |
| V-CROSS-D_MKT_DATA-D_DATA | 跨域违规: D_MKT_DATA -> D_DATA | cross_domain_violation | D_MKT_DATA | D_DATA | error | open |
| V-CROSS-D_MKT_DATA-D_SHARED | 跨域违规: D_MKT_DATA -> D_SHARED | cross_domain_violation | D_MKT_DATA | D_SHARED | error | open |
| V-CROSS-D_OPS-D_GOV_SCRIPTS | 跨域违规: D_OPS -> D_GOV_SCRIPTS | cross_domain_violation | D_OPS | D_GOV_SCRIPTS | error | open |
| V-CROSS-D_ORCHESTRATOR-D_SECURITY | 跨域违规: D_ORCHESTRATOR -> D_SECURITY | cross_domain_violation | D_ORCHESTRATOR | D_SECURITY | error | open |
| V-CROSS-D_PF_ALLOC-D_SHARED | 跨域违规: D_PF_ALLOC -> D_SHARED | cross_domain_violation | D_PF_ALLOC | D_SHARED | error | open |
| V-CROSS-D_PF_CORE-D_BACKTEST | 跨域违规: D_PF_CORE -> D_BACKTEST | cross_domain_violation | D_PF_CORE | D_BACKTEST | error | open |
| V-CROSS-D_PF_CORE-D_FACTOR | 跨域违规: D_PF_CORE -> D_FACTOR | cross_domain_violation | D_PF_CORE | D_FACTOR | error | open |
| V-CROSS-D_PF_CORE-D_GOVERNANCE | 跨域违规: D_PF_CORE -> D_GOVERNANCE | cross_domain_violation | D_PF_CORE | D_GOVERNANCE | error | open |
| V-CROSS-D_PF_CORE-D_INFRASTRUCTURE | 跨域违规: D_PF_CORE -> D_INFRASTRUCTURE | cross_domain_violation | D_PF_CORE | D_INFRASTRUCTURE | error | open |
| V-CROSS-D_PF_CORE-D_REPORTING | 跨域违规: D_PF_CORE -> D_REPORTING | cross_domain_violation | D_PF_CORE | D_REPORTING | error | open |
| V-CROSS-D_PF_CORE-D_RISK | 跨域违规: D_PF_CORE -> D_RISK | cross_domain_violation | D_PF_CORE | D_RISK | error | open |
| V-CROSS-D_PF_CORE-D_SHARED | 跨域违规: D_PF_CORE -> D_SHARED | cross_domain_violation | D_PF_CORE | D_SHARED | error | open |
| V-CROSS-D_POSITION-D_INFRASTRUCTURE | 跨域违规: D_POSITION -> D_INFRASTRUCTURE | cross_domain_violation | D_POSITION | D_INFRASTRUCTURE | error | open |
| V-CROSS-D_POSITION-D_SHARED | 跨域违规: D_POSITION -> D_SHARED | cross_domain_violation | D_POSITION | D_SHARED | error | open |
| V-CROSS-D_REPORTING-D_EX_CORE | 跨域违规: D_REPORTING -> D_EX_CORE | cross_domain_violation | D_REPORTING | D_EX_CORE | error | open |
| V-CROSS-D_REPORTING-D_SHARED | 跨域违规: D_REPORTING -> D_SHARED | cross_domain_violation | D_REPORTING | D_SHARED | error | open |
| V-CROSS-D_REPORTING-D_TRADING | 跨域违规: D_REPORTING -> D_TRADING | cross_domain_violation | D_REPORTING | D_TRADING | error | open |
| V-CROSS-D_RISK-D_SHARED | 跨域违规: D_RISK -> D_SHARED | cross_domain_violation | D_RISK | D_SHARED | error | open |
| V-CROSS-D_SELL_DECISION-D_SHARED | 跨域违规: D_SELL_DECISION -> D_SHARED | cross_domain_violation | D_SELL_DECISION | D_SHARED | error | open |
| V-CROSS-D_SHARED-D_INFRA_RUNTIME | 跨域违规: D_SHARED -> D_INFRA_RUNTIME | cross_domain_violation | D_SHARED | D_INFRA_RUNTIME | error | open |
| V-CROSS-D_SHARED-D_ML_TRAIN | 跨域违规: D_SHARED -> D_ML_TRAIN | cross_domain_violation | D_SHARED | D_ML_TRAIN | error | open |
| V-CROSS-D_SIMULATION-D_SHARED | 跨域违规: D_SIMULATION -> D_SHARED | cross_domain_violation | D_SIMULATION | D_SHARED | error | open |
| V-HARD150-D_DATA | 硬上限违规: D_DATA | hard_limit_exceeded | D_DATA |  | error | open |
| V-HARD150-D_GOVERNANCE | 硬上限违规: D_GOVERNANCE | hard_limit_exceeded | D_GOVERNANCE |  | error | open |
| V-HARD150-D_GOV_AUDIT | 硬上限违规: D_GOV_AUDIT | hard_limit_exceeded | D_GOV_AUDIT |  | error | open |
| V-HARD150-D_GOV_CODE_QUALITY | 硬上限违规: D_GOV_CODE_QUALITY | hard_limit_exceeded | D_GOV_CODE_QUALITY |  | error | open |
| V-HARD150-D_GOV_SCRIPTS | 硬上限违规: D_GOV_SCRIPTS | hard_limit_exceeded | D_GOV_SCRIPTS |  | error | open |
| V-HARD150-D_INFRA_RUNTIME | 硬上限违规: D_INFRA_RUNTIME | hard_limit_exceeded | D_INFRA_RUNTIME |  | error | open |
| V-HARD150-D_SECURITY | 硬上限违规: D_SECURITY | hard_limit_exceeded | D_SECURITY |  | error | open |
| V-HARD150-D_SHARED | 硬上限违规: D_SHARED | hard_limit_exceeded | D_SHARED |  | error | open |
| V-LAYER-D_GOV_CODE_QUALITY-D_GOV_SCRIPTS | 层级违规: L1_foundation -> L2_domain | layer_violation | D_GOV_CODE_QUALITY | D_GOV_SCRIPTS | error | open |
| V-LAYER-D_INFRA_OPS-D_DATA | 层级违规: L0_infrastructure -> L1_foundation | layer_violation | D_INFRA_OPS | D_DATA | error | open |
| V-LAYER-D_INFRA_RUNTIME-D_AUTONOMY_CORE | 层级违规: L0_infrastructure -> L1_foundation | layer_violation | D_INFRA_RUNTIME | D_AUTONOMY_CORE | error | open |
| V-LAYER-D_INFRA_RUNTIME-D_DATA | 层级违规: L0_infrastructure -> L1_foundation | layer_violation | D_INFRA_RUNTIME | D_DATA | error | open |
| V-LAYER-D_INFRA_RUNTIME-D_FACTOR | 层级违规: L0_infrastructure -> L2_domain | layer_violation | D_INFRA_RUNTIME | D_FACTOR | error | open |
| V-LAYER-D_INFRA_RUNTIME-D_GOV_REPAIR | 层级违规: L0_infrastructure -> L2_domain | layer_violation | D_INFRA_RUNTIME | D_GOV_REPAIR | error | open |
| V-LAYER-D_INFRA_RUNTIME-D_GOV_SCRIPTS | 层级违规: L0_infrastructure -> L2_domain | layer_violation | D_INFRA_RUNTIME | D_GOV_SCRIPTS | error | open |
| V-LAYER-D_INFRA_RUNTIME-D_INTEGRATION | 层级违规: L0_infrastructure -> L1_foundation | layer_violation | D_INFRA_RUNTIME | D_INTEGRATION | error | open |
| V-LAYER-D_INFRA_RUNTIME-D_OPS | 层级违规: L0_infrastructure -> L1_foundation | layer_violation | D_INFRA_RUNTIME | D_OPS | error | open |
| V-LAYER-D_INFRA_RUNTIME-D_ORCHESTRATOR | 层级违规: L0_infrastructure -> L1_foundation | layer_violation | D_INFRA_RUNTIME | D_ORCHESTRATOR | error | open |
| V-LAYER-D_INFRA_RUNTIME-D_TRADING | 层级违规: L0_infrastructure -> L2_domain | layer_violation | D_INFRA_RUNTIME | D_TRADING | error | open |
| V-LAYER-D_INTEGRATION-D_TRADING | 层级违规: L1_foundation -> L2_domain | layer_violation | D_INTEGRATION | D_TRADING | error | open |
| V-LAYER-D_OPS-D_GOV_SCRIPTS | 层级违规: L1_foundation -> L2_domain | layer_violation | D_OPS | D_GOV_SCRIPTS | error | open |
| V-LAYER-D_REPORTING-D_EX_CORE | 层级违规: L1_foundation -> L2_domain | layer_violation | D_REPORTING | D_EX_CORE | error | open |
| V-LAYER-D_REPORTING-D_TRADING | 层级违规: L1_foundation -> L2_domain | layer_violation | D_REPORTING | D_TRADING | error | open |
| V-LAYER-D_SHARED-D_ML_TRAIN | 层级违规: L0_infrastructure -> L2_domain | layer_violation | D_SHARED | D_ML_TRAIN | error | open |
