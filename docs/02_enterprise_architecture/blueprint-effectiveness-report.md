...
| 0.3.0 | 2026-05-04 | beta 硬合规激活——G6 门禁 YAML 创建（severity=error→P0阻断）；`gate_engine.py` 注册 G6 + `hard_compliance` 参数；`blueprint_routing.yaml` 修复 4 类 keyword 误匹配（Gate Engine +12 keywords, LLM Security +8 keywords, cross_read_hint 互引）；`blueprint_search_server.py` MCP 默认 top-3 + MUST read 语言 + `cross_read_hints` 传播；`session_simulator.py` beta 升级 WARNING→REJECT。项目目录结构扁平化至根目录（消除子目录层级）。30 session P2 模拟结果：20 PASS / 10 REJECT (33.3%)——违规率不变但严重度从 P1 升级为 P0。 |
