---
doc_type: capacity_report
title: 域容量报告
version: "1.0"
status: active
date: 2026-06-23
owner: auto-generator
ttl: permanent
---

# 域容量报告

> 本文档由 generate_capacity_report.py 从 depgraph.db 自动生成
> 最后更新: 2026-06-23 13:32:18
> 数据源: depgraph.db domains表 + nodes表

## 统计概览

| 指标 | 值 |
|------|-----|
| 域总数 | 52 |
| 超容域 | 19 |
| 接近超容域（>80%） | 6 |
| 空域（0模块） | 9 |

## 超容域清单（需拆分）

| 域ID | 域名称 | 实际模块数 | 上限 | 超出 |
|------|--------|:---:|:---:|:---:|
| D-AUTONOMY_CORE | 自治核心 | 650 | 150 | +500 |
| D-AUTONOMY_PERM | 自治保护 | 206 | 150 | +56 |
| D-COMPLIANCE | 合规 | 916 | 150 | +766 |
| D-FACTOR | 因子 | 320 | 150 | +170 |
| D-FRONTEND | 前端 | 237 | 150 | +87 |
| D-GOVERNANCE | lifecycle_management | 4289 | 200 | +4089 |
| D-INFRA_OPS | 基础设施运维 | 404 | 150 | +254 |
| D-INFRA_RUNTIME | runtime_integration | 726 | 150 | +576 |
| D-INTEGRATION | pipeline_routing | 706 | 150 | +556 |
| D-INTELLIGENCE | context_management | 273 | 150 | +123 |
| D-KNOWLEDGE | knowledge_management | 160 | 150 | +10 |
| D-MKT_DATA | 行情数据(接入+存储) | 266 | 150 | +116 |
| D-OPS | feedback-loop | 641 | 150 | +491 |
| D-PF_CORE | 组合核心 | 202 | 150 | +52 |
| D-RISK | 风控 | 775 | 150 | +625 |
| D-SECURITY | adversarial_validation | 849 | 200 | +649 |
| D-SHARED | shared_services | 288 | 150 | +138 |
| D-SIGNAL | 信号 | 476 | 150 | +326 |
| D-TRADING | 交易运营 | 249 | 150 | +99 |

## 接近超容域清单（>80%，需关注）

| 域ID | 域名称 | 实际模块数 | 上限 | 使用率 |
|------|--------|:---:|:---:|:---:|
| D-DATA_ENG | 数据工程(增值+融合+知识) | 147 | 150 | 98.0% |
| D-EX_CORE | 执行核心 | 135 | 150 | 90.0% |
| D-EX_SOR | 执行路由 | 131 | 150 | 87.3% |
| D-GOV_RULE | 规则治理 | 175 | 200 | 87.5% |
| D-REPORTING | 报告 | 132 | 150 | 88.0% |
| D-SIMULATION | 仿真 | 128 | 150 | 85.3% |

## 空域清单（0模块，待开发）

| 域ID | 域名称 | 架构层 | 上限 |
|------|--------|--------|:---:|
| D-AUTONOMY-CORE | agent_communication |  | 200 |
| D-AUTONOMY-PERM | escalation |  | 200 |
| D-GOV-ENFORCEMENT | rule_enforcement |  | 200 |
| D-GOV-REPAIR | rollback |  | 200 |
| D-GOV-SCRIPTS | code_dedup |  | 200 |
| D-INFRA-OPS | resource_optimization |  | 200 |
| D-INTEGRATION-GATEWAY | mcp_servers |  | 200 |
| D-ML-TRAIN | model_profiling |  | 200 |
| D-SECURITY-LLM | llm_defense |  | 200 |

## 完整域容量清单

| 域ID | 域名称 | 架构层 | 实际模块数 | 上限 | 使用率 | 状态 |
|------|--------|--------|:---:|:---:|:---:|------|
| D-ALT_DATA | 另类数据 | L1_foundation | 68 | 150 | 45.3% | 正常 |
| D-AUTONOMY-CORE | agent_communication |  | 0 | 200 | 0.0% | 空 |
| D-AUTONOMY-PERM | escalation |  | 0 | 200 | 0.0% | 空 |
| D-AUTONOMY_CORE | 自治核心 | L1_platform | 650 | 150 | 433.3% | 超容 |
| D-AUTONOMY_PERM | 自治保护 | L2_domain | 206 | 150 | 137.3% | 超容 |
| D-BACKTEST | 回测 | L2_domain | 9 | 150 | 6.0% | 正常 |
| D-BEHAVIORAL_AUDIT | 行为审计 | L1_foundation | 60 | 150 | 40.0% | 正常 |
| D-COMPLIANCE | 合规 | L2_domain | 916 | 150 | 610.7% | 超容 |
| D-CROSS_ASSET | 跨资产 | L2_domain | 79 | 150 | 52.7% | 正常 |
| D-DATA_ENG | 数据工程(增值+融合+知识) | L1_foundation | 147 | 150 | 98.0% | 接近超容 |
| D-DATA_GOV | 数据治理(质量+血缘+参考) | L1_foundation | 38 | 150 | 25.3% | 正常 |
| D-DATA_SEC | 数据安全与契约 | L1_foundation | 30 | 150 | 20.0% | 正常 |
| D-DIGITAL_TWIN | 数字孪生 | L2_domain | 13 | 150 | 8.7% | 正常 |
| D-EXEC_SIM | 执行仿真 | L2_domain | 8 | 150 | 5.3% | 正常 |
| D-EX_CORE | 执行核心 | L2_domain | 135 | 150 | 90.0% | 接近超容 |
| D-EX_SOR | 执行路由 | L2_domain | 131 | 150 | 87.3% | 接近超容 |
| D-FACTOR | 因子 | L2_domain | 320 | 150 | 213.3% | 超容 |
| D-FRONTEND | 前端 | L1_platform | 237 | 150 | 158.0% | 超容 |
| D-GOV-ENFORCEMENT | rule_enforcement |  | 0 | 200 | 0.0% | 空 |
| D-GOV-REPAIR | rollback |  | 0 | 200 | 0.0% | 空 |
| D-GOV-SCRIPTS | code_dedup |  | 0 | 200 | 0.0% | 空 |
| D-GOVERNANCE | lifecycle_management | L2_domain | 4289 | 200 | 2144.5% | 超容 |
| D-GOV_AUDIT | audit-trail | L2_domain | 69 | 200 | 34.5% | 正常 |
| D-GOV_DRIFT | drift_detection | L2_domain | 22 | 200 | 11.0% | 正常 |
| D-GOV_RULE | 规则治理 | L2_domain | 175 | 200 | 87.5% | 接近超容 |
| D-INFRA-OPS | resource_optimization |  | 0 | 200 | 0.0% | 空 |
| D-INFRA_OPS | 基础设施运维 | L0_infrastructure | 404 | 150 | 269.3% | 超容 |
| D-INFRA_RUNTIME | runtime_integration | L0_infrastructure | 726 | 150 | 484.0% | 超容 |
| D-INTEGRATION | pipeline_routing | L1_platform | 706 | 150 | 470.7% | 超容 |
| D-INTEGRATION-GATEWAY | mcp_servers |  | 0 | 200 | 0.0% | 空 |
| D-INTELLIGENCE | context_management | L2_domain | 273 | 150 | 182.0% | 超容 |
| D-KNOWLEDGE | knowledge_management | L2_domain | 160 | 150 | 106.7% | 超容 |
| D-MKT_DATA | 行情数据(接入+存储) | L1_foundation | 266 | 150 | 177.3% | 超容 |
| D-ML-TRAIN | model_profiling |  | 0 | 200 | 0.0% | 空 |
| D-ML_SERVE | 推理 | L2_domain | 69 | 150 | 46.0% | 正常 |
| D-ML_TRAIN | 训练 | L2_domain | 118 | 150 | 78.7% | 正常 |
| D-OPS | feedback-loop | L1_platform | 641 | 150 | 427.3% | 超容 |
| D-PF_ALLOC | 组合分配 | L2_domain | 114 | 150 | 76.0% | 正常 |
| D-PF_CORE | 组合核心 | L2_domain | 202 | 150 | 134.7% | 超容 |
| D-POSITION | 仓位管理 | L2_domain | 77 | 150 | 51.3% | 正常 |
| D-REPORTING | 报告 | L1_platform | 132 | 150 | 88.0% | 接近超容 |
| D-RISK | 风控 | L2_domain | 775 | 150 | 516.7% | 超容 |
| D-SECURITY | adversarial_validation | L1_platform | 849 | 200 | 424.5% | 超容 |
| D-SECURITY-LLM | llm_defense |  | 0 | 200 | 0.0% | 空 |
| D-SELL_DECISION | 卖出决策 | L2_domain | 64 | 150 | 42.7% | 正常 |
| D-SHARED | shared_services | L1_platform | 288 | 150 | 192.0% | 超容 |
| D-SIGNAL | 信号 | L2_domain | 476 | 150 | 317.3% | 超容 |
| D-SIGNAL_ASHARE | A股特色信号 | L2_domain | 27 | 150 | 18.0% | 正常 |
| D-SIGNAL_FUNDAMENTAL | 基本面信号 | L2_domain | 24 | 150 | 16.0% | 正常 |
| D-SIGNAL_QUALITY | 信号质量 | L2_domain | 18 | 150 | 12.0% | 正常 |
| D-SIMULATION | 仿真 | L2_domain | 128 | 150 | 85.3% | 接近超容 |
| D-TRADING | 交易运营 | L2_domain | 249 | 150 | 166.0% | 超容 |
