...
- 17-core-selections
summary: 技术架构永恒原则文档。timeless 方法论——技术栈决策框架（ThoughtWorks Radar 四象限 + Build 自研扩展 = 五象限；17 项 AI 基础设施聚焦）、单进程演进式运行时拓扑、experimental vs Post-Activation 部署框架、DR/BCP RTO/RPO 分层矩阵、4 环境矩阵与晋级门禁、可观测性三支柱（Metrics/Logs/Traces + OTel）。派生数据（具体技术清单、容量数字、成本预算）不在本文档，由 technology_landscape.yaml + 自动化系统维护。
date: '2026-07-19'
ttl: permanent
---
...
### 2.1 ThoughtWorks Technology Radar 四象限 + Build 自研扩展（永恒）

本项目技术栈采用 **ThoughtWorks Technology Radar** 风格分类管理，在标准四象限基础上扩展 **Build**（自研）象限，共五象限：
...
| **Hold** | 暂不采用，有特定原因 | 暂缓 |
| **Build** | 无合适开源替代，自研模块 | 自研实现 |

> **注**：技术栈完整清单（43 项 = Adopt 12 / Trial 9 / Assess 10 / Hold 6 / Build 6）见 `architecture_model/technology/technology_landscape.yaml`（真源），不在本文档硬编码。

### 2.2 Vibe Coding 2.0 AI 基础设施聚焦视图（永恒——17 项强约束）

本视图是 AI 基础设施的**强约束选型**（17 项，源自 Qwen 技术选型共识）。
...
| 分类方式 | 按服务分组 + 升级阈值看板 + KB 决策记录对应 | ThoughtWorks Radar 五象限 |
...
| 服务 | 项数 | 职责 |
|------|:----:|------|
| Context Engine | 3 | 上下文图存储 / 上下文压缩 / 降级回退 |
| Vector Memory | 3 | 向量库 / 嵌入模型 / 分块策略 |
| Agent Orchestrator | 5 | 状态持久化 / 异步队列 / 状态机 / 进程互斥 / 任务派发 |
| Agent Sandbox | 1 | 沙箱隔离与权限边界 |
| Feedback Loop | 2 | 时间序列存储 / 异常检测 |
| LLM Security Gateway | 3 | 输入校验 / 模式拦截 / 密钥保护 |

> **注**：每项的具体选型、版本号与升级阈值看板见 `vibe_coding_infrastructure_tech_stack.yaml` 真源，不在本文档硬编码。
...
**架构定位为单进程演进式**：所有层在同一 Python 进程内运行，通过函数调用传递契约对象。
...
| 进程类型 | 运行环境 | 职责 | 启动方式 |
|---------|---------|------|---------|
| **Main Process** | Windows / Linux (Python) | 运行 全域全链路主业务逻辑 | `zephyr` CLI（详见 `pyproject.toml` `[project.scripts]`） |
| **Pre-commit Guard** | Git hook（本地） | 文件治理检查 | `git commit` 触发 |
| **CI Audit Process** | CI（GitHub Actions） | 全仓库审计扫描 | push / PR 触发 |
...
- experimental 阶段无入站监听；Post-Activation 阶段启用
...
| 维度 | experimental | Post-Activation |
|------|----------------|----------------|
| 运行环境 | 本地单机 | 云端 / 容器化 |
| 数据存储 | 嵌入式/文件存储（具体选型见 YAML）| 云端 DB + 对象存储 |
| 可观测性 | 本地日志文件 | OTel Collector + Metrics/Logs/Traces 栈 |
| 调度 | 手动 / 简单定时 | 工作流调度器 |
| CI/CD | lint/audit 流水线 | 完整 CI/CD Pipeline + 回滚 |

> **注**：各阶段具体技术选型（数据库、调度器、可观测性栈、CI/CD 工具）见 `architecture_model/technology/technology_landscape.yaml`，本文仅保留二阶段部署框架。
...
| `D_OPS` 反馈循环 | 接入真实券商 API / 可用性达标 / 多 Agent 达标 |
| `D_SECURITY_LLM` LLM防御 | LLM 调用量达阈值后激活 |

> **注**：具体激活阈值（可用性百分比、Agent 数量、LLM 调用量等）为决策快照，详见 `architecture_model/technology/technology_landscape.yaml` 各条目 `reevaluate_trigger` / `decision_trigger` 字段。
...
> **注**：域名级 7 行详细 RTO/RPO 矩阵（含市场/非市场时段 RTO、当前阶段成本）见 `architecture_model/technology/dr_bcp_matrix.yaml`。
...
| **Tier 3 冷备** | 数据快照归档 + Git（具体工具见 `dr_bcp_matrix.yaml`）|
...
| 门禁 | 必须满足 |
|------|---------|
| **Gate 1（Dev→UAT）** | 单元测试全通过 + 回测达标 + 风险调整收益达标 |
| **Gate 2（UAT→Staging）** | UAT 模拟盘无重大异常 + Code Review + KB 决策记录 |
| **Gate 3（Staging→Prod）** | Shadow Trading 达标 + SLO 验证 + 书面风险确认 + Runbook |

> **注**：门禁具体阈值（回测年限、Sharpe、模拟盘周期、Shadow Trading 周期等数值）为决策快照，详见 `architecture_model/technology/dr_bcp_matrix.yaml` 环境维度矩阵。
...
| 级别 | 日志类型 | 保留策略 |
|------|---------|---------|
| **L1 应用日志** | 业务运行事件 | 限期本地 + 聚合系统归档 |
| **L2 审计日志** | 决策与操作记录 | **不可删除（append-only，KBG-0002）**；长期留存 |
| **L3 安全日志** | 认证、API Key 使用 | 限期本地；Prod 加密存储 |

**永恒约束**：
- 日志格式：结构化 JSON（含 `trace_id` / `span_id` / `layer` 字段）
- L2 审计日志 append-only，禁用删除

> **注**：具体保留周期、日志聚合工具（Loki 等）见 `architecture_model/technology/technology_landscape.yaml`（决策快照）。
...
> **注**：具体采样比例为决策快照，详见 `architecture_model/technology/dr_bcp_matrix.yaml` 可观测性指标目录。

### 7.4 OTel 集成框架（永恒）

可观测性栈采用 OpenTelemetry 标准统一采集三支柱，由 Collector 接收路由至 Metrics/Logs/Traces 存储后端，最终汇聚至统一看板。

> **注**：具体组件选型（OTel SDK / Collector / Prometheus / Tempo / Loki / Grafana）见 `architecture_model/technology/technology_landscape.yaml`（T-A11/T-A12, T-T05~T-T08, T-AS08），本文仅保留三支柱集成框架。
...
- ThoughtWorks Radar 五象限分类与 17 项 AI 基础设施聚焦视图（§2）
- 单进程演进式运行时拓扑原则（§3）
