...
- security-principles
- domain-driven
summary: ZephyrAlpha 架构原则+方法论集中 SSoT。当前 v2.0.1：清理幻觉引用 + 量化估算改定性 + 历史修订记录精简。原则内容（4 条安全红线 + 5 条开源优先子原则 + 核心架构决策）保留。
date: '2026-07-19'
ttl: permanent
---
...
| 日期 | 版本 | 说明 |
|------|------|------|
| 2026-07-19 | 2.0.1 | 当前版本。清理幻觉引用（KBG/ADR/D_XXX/ARB/gov_kb 等不存在的引用全删）+ 量化估算数字（10-50倍/3-10倍）改定性 + 历史修订记录清理（v1.x/v2.0.0 噪音删除）。原则内容保留。 |