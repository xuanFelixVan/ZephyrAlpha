---
module_id: VIEW-CODE-WIKI
title: "ZephyrAlpha Code Wiki / 代码百科"
...
summary: "ZephyrAlpha 项目结构化代码百科——整体架构、模块职责、关键类与函数、依赖关系、运行方式。半自动维护：手工叙述 + 自动同步统计数据（depgraph DB）。"
tags: [code_wiki, architecture_overview, onboarding, semi_auto]
---
...
> 自动区用 `<!-- AUTO-START:block_name -->` / `<!-- AUTO-END:block_name -->` 标记，生成器只更新标记内内容。
> 生成器（待实现）：`scripts/governance/d5_architecture/generators/generate_code_wiki_stats.py`，挂在 `generate_project_depgraph.py` 之后触发。
...
*半自动（层级定义手工 + 各层域清单自动同步 | 数据源：depgraph DB domains 表）*

项目域按 `layer_id` 属性分为 4 个层级（真源：`depgraph_schema.py` DB trigger）：

| 层级 | 含义 | 代表域 |
|------|------|--------|
| `L0_infrastructure` | 基础设施层 / Infrastructure | D_INFRA_RUNTIME / D_INFRA_RECOVERY |
| `L1_foundation` | 基础层 / Foundation | D_SHARED / D_GOVERNANCE |
...
<!-- 生成器从文件系统扫描 src/zephyr/ 一级目录 + scripts/governance/ 维度目录自动生成此树 -->
<!-- 注释列格式：中文说明（英文说明）；中英文均手工维护 -->
<!-- AUTO-END:directory_tree -->

```
ZephyrAlpha/
├── src/zephyr/                  # 核心源码（唯一 Python 包根）/ Core source (sole Python package root)
│   ├── trading/                 # AutoRuntime Core 系统大脑 / AutoRuntime Core (system brain)
...
│   ├── signal_fundamental/      # 信号合成管线 / Signal synthesis pipeline
│   ├── signal_quality/          # 信号质量评估 / Signal quality evaluation
│   ├── risk/                    # 风险管理（止损/限仓/熔断）/ Risk management (stop-loss/limit/circuit breaker)
│   ├── ex_core/                 # 执行核心（订单/SOR）/ Execution core (order/SOR)
│   ├── pf_core/                 # 组合核心（策略基类/注册表）/ Portfolio core (strategy base/registry)
│   ├── market_data/             # 行情数据契约 / Market data contracts
│   ├── shared/                  # 共享层（types/errors/io/event_bus）/ Shared layer (types/errors/io/event_bus)
...
├── architecture_model/          # 架构模型 YAML SSoT（53域+契约+事件）/ Architecture model YAML SSoT (53 domains)
├── pyproject.toml               # 项目元数据与依赖 / Project metadata & dependencies
├── docker-compose.yml           # 容器编排 / Container orchestration
├── Dockerfile                   # 容器构建 / Container build
└── AGENTS.md                    # AI 接入宪法 / AI onboarding constitution
```
...
**位置**：[src/zephyr/trading/](../../../src/zephyr/trading/) | **蓝图**：MOD-INF-035 | **层级**：infrastructure
...
| 类 | 文件 | 职责 |
|----|------|------|
| `AutoRuntimeCore` | [auto_runtime_core.py](../../../src/zephyr/trading/auto_runtime_core.py) | 三层运行时运营中心，组合 20+ 子组件 |
| `Conductor` | [conductor.py](../../../src/zephyr/trading/conductor.py) | 全自动指挥官，任务认领+冲突检测+并行分组 |
| `AutoPilot` | [autopilot.py](../../../src/zephyr/trading/autopilot.py) | 自动驾驶，扫描+原子认领待处理任务 |
| `WorkOrchestrator` | [work_orchestrator.py](../../../src/zephyr/trading/work_orchestrator.py) | 工作编排子系统，决定 what/when/which model/order |
| `CapabilityRegistry` | [capability_registry.py](../../../src/zephyr/trading/capability_registry.py) | 能力注册表，解决"AI 不知道功能存在"问题 |
| `LifecycleManager` | [lifecycle_manager.py](../../../src/zephyr/trading/lifecycle_manager.py) | 子组件生命周期管理（boot/shutdown 序列） |
...
`register_boot_hooks()`（[boot_hooks.py](../../../src/zephyr/trading/boot_hooks.py)）幂等注册以下钩子：
...
*半自动（类表自动扫描 AST + 职责描述手工）| 数据源：src/zephyr/governance/*.py*

**位置**：[src/zephyr/governance/](../../../src/zephyr/governance/) | **蓝图**：DOM-GOV-001
...
**位置**：[src/zephyr/gov_audit/](../../../src/zephyr/gov_audit/) | **蓝图**：MOD-INF-020 | **域**：D_GOV_AUDIT

不可篡改审计追踪，带加密溯源与 Agent 签名。62 文件物理平铺但逻辑聚类（ARCH-042：10 前缀簇）。

| 类 | 文件 | 职责 |
|----|------|------|
| `AuditIndexer` | [indexer.py](../../../src/zephyr/gov_audit/indexer.py) | 增量非破坏性索引构建（按维度/严重度/类型） |
| `AuditReportWriter` | [writer.py](../../../src/zephyr/gov_audit/writer.py) | 原子报告写入（temp+fsync+replace，防崩溃损坏） |
| `AuditQueryEngine` | [query.py](../../../src/zephyr/gov_audit/query.py) | 只读查询引擎（失败返回空，不抛异常） |
| `AuditType`/`Severity`/`Priority` | [models.py](../../../src/zephyr/gov_audit/models.py) | Pydantic 数据模型与枚举（不可变分类） |

#### 4.2.3 漂移检测（gov_drift）

*半自动（类表自动 + 检测器数自动同步 + 职责描述手工）| 数据源：src/zephyr/gov_drift/ + 检测器注册表*

**位置**：[src/zephyr/gov_drift/](../../../src/zephyr/gov_drift/) | **蓝图**：MOD-INF-023 | **域**：D_GOV_DRIFT

Git 原生运行时漂移检测，39 个强制检测器（全部必须执行）+ 自动调和闭环。

| 类 | 文件 | 职责 |
|----|------|------|
| `DriftDetector` | [drift_detector.py](../../../src/zephyr/gov_drift/drift_detector.py) | 兼容别名，同步漂移检测（欧氏距离+事件发射） |
| `DriftEngine` | [drift_engine.py](../../../src/zephyr/gov_drift/drift_engine.py) | 编排核心：检测器发现→调度→聚合→反馈（Safety H） |
...
*自动化同步（维度脚本数表）+ 手工维护（退出码说明）| 数据源：scripts/governance/d*/ 目录扫描*
...
| d11_compliance | 合规检查 / Compliance check | 21 |
...
| d6_security | 安全扫描 / Security scan | 14 |
| d7_code | d7_code / d7_code | 40 |
| d8_doc_sync | 文档一致性 / Doc consistency | 11 |
...
**位置**：[src/zephyr/data/](../../../src/zephyr/data/) | **蓝图**：MOD-L00-004 | **CLI**：`integrator`

统一管理多数据源（iFind / miniQMT / AKShare）的自动下载、调度与 ClickHouse 持久化。

| 类 | 文件 | 职责 |
|----|------|------|
| `DataSourceBase` | [provider_base.py](../../../src/zephyr/data/provider_base.py) | Provider 抽象基类（connect/health_check/fetch/disconnect） |
| `IntegratorScheduler` | [scheduler.py](../../../src/zephyr/data/scheduler.py) | APScheduler 编排器（5 cron 时段 + DAG 依赖 + 断点续传） |
| `ClickHouseWriter` | [ch_writer.py](../../../src/zephyr/data/ch_writer.py) | ClickHouse 写入（TCP:9000 查询 + HTTP:8123 写入 + 本地落盘回退） |
| `Alerter` | [alerter.py](../../../src/zephyr/data/alerter.py) | 失败告警 |
| `ProgressStore` | [progress_store.py](../../../src/zephyr/data/progress_store.py) | 断点续传存储 |

**数据流**：`CLI → get_integrator() → IntegratorScheduler → DataSourceBase providers → ch_writer.write_result() → ClickHouse`

**配置**：[config/tasks.yaml](../../../src/zephyr/data/config/tasks.yaml) 定义 10 个任务（3 个 provider × 2 调度时段），每个任务声明 `task_id/table/source/schedule/incremental/dependencies/capability`。
...
*半自动（类表自动扫描 AST + 职责描述手工）| 数据源：src/zephyr/backtest/core/*.py*

**位置**：[src/zephyr/backtest/](../../../src/zephyr/backtest/) | **蓝图**：MOD-BT-001 | **域**：D_BACKTEST

| 类 | 文件 | 职责 |
|----|------|------|
| `BacktestEngineBase` | [core/engine_base.py](../../../src/zephyr/backtest/core/engine_base.py) | 回测引擎抽象基类（OCP 扩展点） |
| `MatchingEngine` | [core/matching_engine.py](../../../src/zephyr/backtest/core/matching_engine.py) | 撮合引擎（委托纯函数 MatchingLogic，保证回测=实盘；A 股 T+1/涨跌停/停牌/100 股） |
| `DecisionGate` | [core/decision_gate.py](../../../src/zephyr/backtest/core/decision_gate.py) | 3 阶段门控 IS→WFA→OOS（不可跳过；IS Sharpe>0.5 准入；OOS Sharpe≥70% IS） |
| `PITManager` | [core/pit_manager.py](../../../src/zephyr/backtest/core/pit_manager.py) | PIT 铁律（AS-OF JOIN + Embargo 期，零前瞻） |
| `WalkForward` | [core/walk_forward.py](../../../src/zephyr/backtest/core/walk_forward.py) | Walk-Forward 分析（滚动/锚定/扩展 + White's Reality Check） |
| `Portfolio` | [core/portfolio.py](../../../src/zephyr/backtest/core/portfolio.py) | 组合（T+1 锁 + NAV 曲线） |
| `calculate_metrics()` | [core/metrics.py](../../../src/zephyr/backtest/core/metrics.py) | 指标计算（中国 10 年国债 2.5% 无风险；样本<60 不算 Sharpe；DSR 修正） |

#### 4.4.2 因子框架（factor）

**位置**：[src/zephyr/factor/](../../../src/zephyr/factor/) | **蓝图**：MOD-L02-001 | **域**：D_FACTOR

| 类 | 文件 | 职责 |
|----|------|------|
| `FactorBase` | [factor_base.py](../../../src/zephyr/factor/factor_base.py) | 因子抽象基类（OCP 扩展点；`compute(data)->Series`） |
| `FactorMeta` | [factor_base.py](../../../src/zephyr/factor/factor_base.py) | 因子元数据（factor_id/name/domain/version/dependencies） |
| `FactorRegistry` | [factor_base.py](../../../src/zephyr/factor/factor_base.py) | 因子注册表单例（装饰器自注册） |
| `ValueFactor` | [value_factor.py](../../../src/zephyr/factor/value_factor.py) | 示例价值因子（PE 倒数） |
| `Momentum20d` | [momentum_factor.py](../../../src/zephyr/factor/momentum_factor.py) | 示例动量因子（20 日收益率） |

#### 4.4.3 信号管线（signal_fundamental）

*半自动（类签名自动扫描 + 职责描述手工）| 数据源：src/zephyr/signal_fundamental/pipeline.py*

**位置**：[src/zephyr/signal_fundamental/pipeline.py](../../../src/zephyr/signal_fundamental/pipeline.py) | **域**：D_SIGLEGACY
...
*半自动（类表自动扫描 AST + 职责描述手工）| 数据源：src/zephyr/risk/*.py*

**位置**：[src/zephyr/risk/](../../../src/zephyr/risk/) | **蓝图**：MOD-L04-001 | **域**：D_RISK

| 类 | 文件 | 职责 |
|----|------|------|
| `RiskManagerBase` | [risk_manager.py](../../../src/zephyr/risk/risk_manager.py) | 顶级风险编排（预+后+熔断） |
| `RiskLimitsCalculator` | [risk_limits.py](../../../src/zephyr/risk/risk_limits.py) | 限仓计算（L1 硬限/L2 软限/L3 熔断） |
| `RiskValidator` | [risk_validator.py](../../../src/zephyr/risk/risk_validator.py) | 预交易+组合验证（HALT 抛异常，WARNING 仅日志） |
| `evaluate_stop_loss()` | [stop_loss.py](../../../src/zephyr/risk/stop_loss.py) | 止损评估（fixed_pct/trailing/time_based/volatility） |

不变量：熔断延迟 <1ms（INV-001）、日亏硬限（INV-004）、所有调用携带 idempotency_key（INV-007）。

#### 4.4.5 执行核心（ex_core）

*半自动（类表自动扫描 AST + 职责描述手工）| 数据源：src/zephyr/ex_core/*.py*

**位置**：[src/zephyr/ex_core/](../../../src/zephyr/ex_core/) | **蓝图**：MOD-L06-001 | **域**：D_EX_CORE

| 类 | 文件 | 职责 |
|----|------|------|
| `ExecutionEngine` | [execution_engine.py](../../../src/zephyr/ex_core/execution_engine.py) | 算法执行+SOR 编排（TWAP/VWAP/冰山） |
| `OrderManager` | [order_manager.py](../../../src/zephyr/ex_core/order_manager.py) | 订单状态机生命周期（PENDING→SUBMITTED→FILLED） |
| `BrokerInterface` | [broker_interface.py](../../../src/zephyr/trading/trading_contracts/broker_interface.py) | Broker 接口（OCP-003 扩展点，canonical 路径 ARCH-GOV-SHIM-001） |

#### 4.4.6 组合核心（pf_core）

*半自动（类表自动扫描 AST + 职责描述手工）| 数据源：src/zephyr/pf_core/*.py + governance/strategies/*.py*

**位置**：[src/zephyr/pf_core/](../../../src/zephyr/pf_core/) | **蓝图**：MOD-L05-001 | **域**：D_PORTFOLIO_CORE

| 类 | 文件 | 职责 |
|----|------|------|
| `StrategyBase` | [governance/strategies/strategy_base.py](../../../src/zephyr/governance/strategies/strategy_base.py) | 策略抽象基类（`generate_target_weights`，OCP-002 扩展点） |
| `StrategyRegistry` | [governance/strategies/strategy_base.py](../../../src/zephyr/governance/strategies/strategy_base.py) | 策略注册表单例（装饰器自注册） |
...
*半自动（类表自动扫描 AST + 职责描述手工）| 数据源：src/zephyr/shared/**/*.py*

**位置**：[src/zephyr/shared/](../../../src/zephyr/shared/) | **蓝图**：MOD-SHR_shared

跨层基础能力：types / errors / constants / IO / event_bus / contracts / observability。

| 类/函数 | 文件 | 职责 |
|---------|------|------|
| `EventBus` / `DomainEvent` | [event_bus.py](../../../src/zephyr/shared/event_bus.py) | 异步事件总线+背压控制（CAP-006=500 队列深度） |
| `ZephyrBaseError` + 11 子类 | [foundation/errors.py](../../../src/zephyr/shared/foundation/errors.py) | 统一异常树根（ZA-XX-NNNN 错误码） |
| `REPO_ROOT` / `find_repo_root()` | [io/paths.py](../../../src/zephyr/shared/io/paths.py) | 仓库根常量 SSoT（基于 .git marker 向上搜索） |
| `load_vocabulary_values()` | [io/yaml_utils.py](../../../src/zephyr/shared/io/yaml_utils.py) | 词表合法值加载器（strict=True fail-fast） |
| `Task` / `TaskCard` / `DecompositionResult` | [foundation/models.py](../../../src/zephyr/shared/foundation/models.py) | 任务系统核心数据模型（Pydantic v2） |
| NewType 别名 | [foundation/types.py](../../../src/zephyr/shared/foundation/types.py) | TaskId/ModuleId/FilePath 等零开销语义标识 |

> **向下依赖原则**：`shared` 禁止 import `integration.*`。

#### 4.5.2 基础设施层（infrastructure）

*半自动（类表自动扫描 AST + 职责描述手工）| 数据源：src/zephyr/infrastructure/**/*.py*

**位置**：[src/zephyr/infrastructure/](../../../src/zephyr/infrastructure/) | **蓝图**：MOD-INFRA_RUNTIME

| 类 | 文件 | 职责 |
|----|------|------|
| `CostTracker` | [cost_tracker.py](../../../src/zephyr/infrastructure/cost_tracker.py) | AI Agent 执行成本追踪（token/API 调用/费用，日预算告警） |
| `EventStore` | [event_store.py](../../../src/zephyr/infrastructure/event_store.py) | 不可篡改审计日志（SQLite WAL + SHA256 checksum） |
| `SLAMonitor` | [sla/sla_monitor.py](../../../src/zephyr/infrastructure/sla/sla_monitor.py) | RTO/RPO 自动记录（事件驱动：pipeline_failed→rollback_completed） |
...
*半自动（类表自动扫描 + 服务器清单自动同步 + 职责描述手工）*

#### 4.6.1 集成层（integration）

*半自动（类表自动扫描 AST + MCP 服务器清单自动同步 + 职责描述手工）| 数据源：src/zephyr/integration/*.py + config/mcp.json*

**位置**：[src/zephyr/integration/](../../../src/zephyr/integration/) | **蓝图**：MOD-INT_integration

外部集成边界：MCP 服务器、LLM 桥、端口协议。

| 类/函数 | 文件 | 职责 |
|---------|------|------|
| `BaseMCPServer` | [mcp/_base_server.py](../../../src/zephyr/integration/mcp/_base_server.py) | 自建 JSON-RPC 2.0 over stdio MCP 基类（MCP/0.3 spec） |
...
*半自动（类表自动扫描 AST + 防御层清单自动同步 + 职责描述手工）| 数据源：src/zephyr/security/**/*.py*

**位置**：[src/zephyr/security/](../../../src/zephyr/security/) | **蓝图**：MOD-SEC_security

LLM 防御 L0-L8 九层纵深防御栈。

| 类 | 文件 | 职责 |
|----|------|------|
| `LLMSecurityProtocol` | [llm_defense/llm_security/protocol.py](../../../src/zephyr/security/llm_defense/llm_security/protocol.py) | 防御层抽象基类（fail-closed：层不可用→BLOCK） |
| `LSGSecurityGateway` | [llm_defense/llm_security/gateway.py](../../../src/zephyr/security/llm_defense/llm_security/gateway.py) | LSG 安全网关编排器（所有 LLM 调用必经） |
| `InputSanitizer` | [llm_defense/llm_security/input_sanitizer.py](../../../src/zephyr/security/llm_defense/llm_security/input_sanitizer.py) | 输入消毒（路径穿越/命令注入/上下文注入） |
| `AuditLogger` | [llm_defense/llm_security/behavior_audit_logger.py](../../../src/zephyr/security/llm_defense/llm_security/behavior_audit_logger.py) | 行为审计日志（带轮转） |
...
*半自动（类签名自动扫描 AST + 注释解读手工）| 数据源：对应模块 .py 文件 AST 解析*
...
*自动化同步（统计数据）+ 手工维护（依赖流图）*

<!-- AUTO-START:dependency_stats -->
<!-- 数据源：depgraph (PostgreSQL) | 最后同步：2026-07-21 -->

| 指标 | 值 |
|------|----|
| 域总数 / Total domains | 62 |
| 节点总数 / Total nodes | 5461 |
| 依赖边总数 / Total edges | 7469 |
...
*自动化同步（依赖表）| 数据源：pyproject.toml [project.dependencies]*
...
*半自动（命令表自动同步 + 用途说明手工）| 数据源：pyproject.toml [project.scripts]*

安装后提供两个控制台命令（[pyproject.toml](../../../pyproject.toml) `[project.scripts]`）：
...
启动 4 个服务（[docker-compose.yml](../../../docker-compose.yml)）：
...
复制 [.env.example](../../../.env.example) → `.env` 并填入：
...
*半自动（子目录数自动同步 + 命令手工）| 数据源：tests/ 目录扫描*
...
| [config/capacity_slo.yaml](../../../config/capacity_slo.yaml) | 容量 SLO |
| [src/zephyr/data/config/tasks.yaml](../../../src/zephyr/data/config/tasks.yaml) | 数据任务清单（10 任务） |
| [architecture_model/index.yaml](../../../architecture_model/index.yaml) | 架构模型 SSoT（53 域） |

### 8.3 Hot/Warm/Cold 三层存储架构

| 层 | 职责 | 存储 | 场景 |
|----|------|------|------|
| Hot | 高频实时数据 / High-freq realtime | ClickHouse MergeTree + 常驻内存 / In-memory | tick_data / index_quote |
| Warm | 中频历史数据 / Mid-freq historical | DuckDB + Parquet | daily_kline 等 |
| Cold | 长期归档（7 年合规）/ Long-term archive (7y compliance) | E 盘 Parquet / E-drive Parquet (reserved) | 合规归档 / Compliance archive |
...
| 铁律 | 要点 |
|------|------|
| SSoT 真源分类 / SSoT classification | 规则数据→YAML（sync 到 DB）；架构数据→PostgreSQL DB（apply_*.py 直接写）/ Rules→YAML; Architecture→PostgreSQL |
| 依赖关系先行 / Dependency-first | 施工前 MUST 先 `apply_depgraph.py --add-design-node` 登记依赖 / Register deps before coding |
| 任务系统 SSoT / Task system SSoT | 查询任务状态 MUST 通过 TaskRepository，禁止直接读 tasks/*.md / Query via TaskRepository only |
| LLM 安全 / LLM security | 所有 LLM 调用必经 LSGSecurityGateway，禁止裸调 / All LLM calls via LSGSecurityGateway |
...
> **文档维护机制**：本文档采用半自动维护，每个小标题下的斜体标注行标识维护方式：
> - **自动化同步**：生成器从数据源全自动刷新（用 `<!-- AUTO-START/END -->` 标记块）
> - **半自动**：清单/签名/数量自动同步 + 职责描述/解读手工维护
> - **手工维护**：叙述/设计图/概念定义，不自动
>
> **双语要求**：所有说明性内容（目录注释、表格职责列、铁律要点）均采用 `中文 / English` 双语格式，便于 AI 检索与国际协作。
>
> 生成器（待实现）：`scripts/governance/d5_architecture/generators/generate_code_wiki_stats.py`，挂在 `generate_project_depgraph.py` 之后触发。
