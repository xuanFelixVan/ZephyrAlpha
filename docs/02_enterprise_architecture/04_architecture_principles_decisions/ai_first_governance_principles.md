...
全部历史违规问题（去重后 3193 个）归因于 5 个根因。每个根因保留抽象概念（详细 5W 追问与证据链见归档文档 git log）：
...
| 根因 | 影响问题类数 | 代表性问题 |
|---|:---:|---|
| 1. 静态快照未动态更新 | ~15 类 | 文件复制对 / 时间触发残留 / 重复簇 |
| 2. 词表消费链机械盲区 | ~12 类 | 词表硬编码 / stability 值域错位 / 未检出盲区 |
| 3. CapabilityLookup 建议性 | ~10 类 | GATE 无反查 / 重复造轮子 / 重复簇新建 |
| 4. manual 例外开口过大 | ~10 类 | manual 脚本 / 永久治理功能 manual-only |
...
**强制机制**：[`RULE-EXECUTION-PAIRING`](file:///d:/ZephyrAlpha/src/zephyr/gov_enforcement/commit_gates/rule_execution_pairing_gate.py) gate（priority=61）已落地——`trae_*.yaml` 规则文件 MUST 有 `enforcement.paired_gate_id` 字段（null 允许文档型规则，字符串须在 gate_registry 注册，`[no-pairing:reason]` 逃生通道）。本裁定将该 commit-time 机制上升为战略原则。
...
**已实现**：[`architecture_health_dashboard.py`](file:///d:/ZephyrAlpha/scripts/governance/architecture_health_dashboard.py) + post-commit reconciler，M01-M31 指标快照落盘 `data/architecture_health/`。后续增量违规由仪表盘实时基线自动发现，不再依赖人工调研快照。
...
**当前状态**：全部 DEFERRED-PERMANENT 项已完成逐项裁定（裁定#222 确认 40 项 wontfix，登记为 #ARCH-DEBT-001~006，详见 [`architecture_issue_registry.yaml`](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/_registry/catalogs/architecture_issue_registry.yaml)）。

---

## 四、治本施工框架（4 期）

> 依赖铁律：Phase 1 不阻塞其他 Phase，作为防复发层贯穿全程；每完成一类存量修复 MUST 配套落地对应 AST 门禁，形成"修复 + 防复发"闭环。
...
- **新增议题**：MUST 在 [`architecture_issue_registry.yaml`](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/_registry/catalogs/architecture_issue_registry.yaml) 登记 #ARCH-XXX 条目（铁律#6）。
