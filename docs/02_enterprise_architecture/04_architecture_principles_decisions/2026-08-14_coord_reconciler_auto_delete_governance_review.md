...
### T2 · 观测与准入（P1）✅ 已落地（2026-08-14 治理批②）

1. ~~worker 启动三证~~：`_check_worker_admission`（reconcile_worker.py）——payload 锚定 worktree 存活 + started_at 新鲜度（15min TTL）+ 发起 session 活性，缺一拒启；TestWorkerAdmission 覆盖。
2. ~~reconciler 动作全量落盘~~：删除/移动类动作（含 auto_committed/fix-in-place）全量 [DELETE-AUDIT] stderr 落盘。
3. 文件删除审计覆盖率指标化：ops_guard 审计 / 实际删除 比=100% 入 RECONCILER-HEALTH。⏳ 留后续批（覆盖率指标采集依赖 S1 盲区收口进度）

### T3 · 文档保护区（P1）🔄 部分落地（2026-08-14 治理批②）

1. docs/_working 分级：统筹文件迁 `_working/_coordination/` 或 frontmatter `protection: coordinator`，归档/清理器禁区。⏳ 留后续批（tracker 已迁 design_memos 永久区先行止血）
2. docs/ 下 untracked 文件的删除/移动需人工确认（guard 规则）——清风类损失归零。⏳ 留后续批（T1 doc_lifecycle 零物理删除已兜底：归档必走 30 天回收站可恢复）
3. ~~本裁定书与 wipe 裁定书 ttl 转 permanent~~（已落地，双裁定书 frontmatter ttl: permanent + doc_type: audit_report；2026-08-14 随 commit 自 docs/_working/ 临时区迁入 02/04_architecture_principles_decisions 永久区——FILE-PLACEMENT-TTL 门禁对齐，全部入站引用已同步）。

### T4 · #55 治本（P1）✅ 已落地（2026-08-14 治理批②）

1. ~~门禁审计写入迁出 tracked 区~~：flags.py 默认审计路径 → `.runtime/audit/feature_flags.jsonl`；`data/audit_logs/` 全目录 gitignore；5 个历史 tracked 审计文件（含 51MB feature_flags.jsonl）git rm --cached 退跟踪。
2. 65 号 §10 "pre-commit 框架 stash 适配"已立项 → **#ARCH-PRECOMMIT-STASH-ADAPT-001**（验收：干净工作区裸 `git commit` pre-commit 全过 + hook 前后 tracked 区零漂移）。

### T5 · 告警卫生（P2）✅ 已落地（2026-08-14 治理批②）

1. ~~TTL cleanup errors 聚类归因~~：GATE-RUNTIME-CLEANUP / GATE-TMP-CLEANUP 双 reconciler 落地"锁定跳过=clean 语义"——PermissionError（WinError 32/5 文件占用）计入 locked_skipped 不触发 warn，仅真异常 OSError 计入 errors；原 rmtree `except OSError: pass` 静默吞真异常顺手修为可观测。
2. ~~RECONCILER-HEALTH 横幅 24h dedup~~：按告警内容签名（gate_id+detail 哈希）去重，同签名 24h 内只 print 一次（logger 留痕不断），新签名照常打印；状态落盘 `.runtime/reconciler_health_warn_dedup.json`，IO 异常 fail-open 保告警可见性。

### T6 · 登记与文本对齐（随批）✅ 已落地（2026-08-14 治理批②）

1. ~~I-GOV-2 表述对齐实现~~：architecture_issue_registry.yaml I-GOV-2 行补对齐注记（auto-commit 白名单=非删除派生写回+满 7 天宽限生命周期归档，其余删除类禁止）。
2. ~~wipe 裁定书排除清单勘误~~：2026-08-14_ai-liq-001 裁定书排除项上行结论已勘误（GATE-WORKING-DOCS 归档器具备实质删除能力）。
3. ~~tracker #53 闭环路径更新~~ + ~~本裁定书 ARCH 登记~~（#ARCH-RECONCILER-AUTO-DELETE-GOV-001，fix_phase 已同步 T2-T6 落地态）。
