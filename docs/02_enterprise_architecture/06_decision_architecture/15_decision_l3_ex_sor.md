...
flowchart TD
    ...
    N72["(设计态 / design) 订单路由决策 / Order Routing<br/>Decision<br/>决定订单发往哪个交易通道或券商，优化成交质量<br/>文件: decision/ex_sor/ex_16"]
    LL3 --- N72
    N73["(设计态 / design) SOR路由决策延迟 / SOR Routing<br/>Latency<br/>监控智能订单路由决策的延迟，确保路由不成为执行瓶<br/>颈<br/>文件: decision/ex_sor/ex_17"]
    LL3 --- N73
    N75["(设计态 / design) 交易通道熔断人工恢复 /<br/>Trading Channel Manual Recovery<br/>交易通道熔断后需人工确认才能恢复，防止自动恢复引<br/>发二次事故<br/>文件: decision/ex_sor/ex_19"]
    LL3 --- N75
    N77["(设计态 / design) Kill-Switch四级阶梯 /<br/>Kill-Switch 4-Level Cascade<br/>急停开关分四级阶梯式触发，从警告到全面停止渐进升<br/>级<br/>文件: decision/ex_sor/ex_21"]
    LL3 --- N77
    N78["(设计态 / design) 熔断器矩阵 / Circuit Breaker<br/>Matrix<br/>多维度熔断器组成矩阵协同防护，覆盖不同粒度的异常<br/>文件: decision/ex_sor/ex_22"]
    LL3 --- N78
    ...
    N74["(设计态 / design) 券商连接熔断+故障转移 /<br/>Broker Circuit Breaker<br/>券商连接异常时熔断并切换到备用券商，保证交易通道<br/>可用<br/>文件: decision/ex_sor/ex_18"]
    LL4 --- N74
    N76["(设计态 / design) Pre-Trade合规检查流水线 /<br/>Pre-Trade Compliance Pipeline<br/>下单前合规检查的流水线化处理，串联多项合规校验<br/>文件: decision/ex_sor/ex_20"]
    LL4 --- N76
    N79["(设计态 / design) 行为准入门禁 / Behavioral<br/>Admission Gateway<br/>交易行为准入的门禁检查，拦截不符合行为准则的交易<br/>文件: decision/ex_sor/ex_23"]
    LL4 --- N79
