...
> **[可缩放 HTML 版 / Zoomable HTML](http://localhost:8765/docs/02_enterprise_architecture/06_decision_architecture/_zoomable_html/06_decision_l2a_data.html)** — Ctrl+滚轮缩放 ｜ 双击重置 ｜ Ctrl+Shift+D 切换拖动/选择模式
...
> 展示全部 3 个决策节点（运营态 0 + 设计态 3），含跨域依赖外部节点。
...
flowchart TD
    ...
    N192["(设计态 / design) Multi-Source Priority Router<br/>多源优先级路由 / Multi-Source Priority Router<br/>多源优先级路由<br/>信号节点·Multi-Source Priority Router<br/>多源优先级路由<br/>文件: decision/data/dt_01"]
    LL2A --- N192
    N193["(设计态 / design) Cross-Source Reconciler<br/>多源对账 / Cross-Source Reconciler 多源对账<br/>信号节点·Cross-Source Reconciler 多源对账<br/>文件: decision/data/dt_02"]
    LL2A --- N193
    N194["(设计态 / design) Multi-Timeframe Fusion<br/>跨频率融合 / Multi-Timeframe Fusion 跨频率融合<br/>信号节点·Multi-Timeframe Fusion 跨频率融合<br/>文件: decision/data/dt_03"]
    LL2A --- N194
    ...
    class LL2A,N192,N193,N194,LL2B,LL2C,LL2D,LL5,LL6 design
```
...
> 仅展示蓝图阶段、代码未写的设计态决策节点（共 3 个），不含跨域外部节点。
...
%%{init: {'theme': 'base', 'themeVariables': {'primaryColor': '#eaeaea', 'primaryTextColor': '#333333', 'primaryBorderColor': '#666666', 'lineColor': '#666666', 'secondaryColor': '#eaeaea', 'tertiaryColor': '#eaeaea', 'clusterBkg': 'transparent', 'clusterBorder': 'transparent', 'fontSize': '14px'}}}%%
flowchart TD
    SELF["(设计态 / design) 数据 / data<br/>多源行情/基本面数据接入、优先级路由、跨源对账与<br/>跨频率融合<br/>跨域节点 / cross-domain"]
    ...
    classDef external_design fill:#fff8e7,stroke:#ef6c00,stroke-width:1px,color:#000,stroke-dasharray: 5 5
    class SELF design
    class EXT_factor external_design
```
