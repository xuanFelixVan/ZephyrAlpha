...
flowchart TD
    ...
    N38["(设计态 / design) 仓位唯一裁决中心 / C-047<br/>Position Sole Arbiter<br/>全系统唯一的仓位最终裁决者，保证仓位指令不冲突<br/>文件: decision/position/pos_01"]
    LL3 --- N38
    N39["(设计态 / design) 持仓状态机 / Position State<br/>Machine<br/>管理持仓的生命周期状态转换，防止非法状态跳转<br/>文件: decision/position/pos_02"]
    LL3 --- N39
    N40["(设计态 / design) 仓位漂移监控 / Position Drift<br/>Monitor<br/>监控实际仓位与目标的偏差，漂移超阈值时报警触发再<br/>平衡<br/>文件: decision/position/pos_03"]
    LL3 --- N40
    N41["(设计态 / design) Kelly仓位决策 / Kelly<br/>Position Decision<br/>用凯利公式计算理论最优仓位，作为仓位决策的数学基<br/>准<br/>文件: decision/position/pos_04"]
    LL3 --- N41
    N42["(设计态 / design) 风险配额 / Risk Quota<br/>为每个仓位分配风险额度上限，防止单仓位占用过多风<br/>险预算<br/>文件: decision/position/pos_05"]
    LL3 --- N42
    N43["(设计态 / design) 11种市场状态→仓位上限 /<br/>Market State Position Cap<br/>按市场状态分类设置仓位上限，熊市自动降仓控风险<br/>文件: decision/position/pos_06"]
    LL3 --- N43
    N44["(设计态 / design) 组合层决策 / Portfolio Layer<br/>Decision<br/>组合层面决定整体仓位水平，自顶向下分配风险<br/>文件: decision/position/pos_07"]
    LL3 --- N44
    N45["(设计态 / design) 策略层决策 / Strategy Layer<br/>Decision<br/>策略层面决定单策略仓位，承接组合层分配<br/>文件: decision/position/pos_08"]
    LL3 --- N45
    N46["(设计态 / design) 标层决策 / Instrument Layer<br/>Decision<br/>单个标的层面决定具体仓位，承接策略层指令<br/>文件: decision/position/pos_09"]
    LL3 --- N46
    N47["(设计态 / design) 动态层决策 / Dynamic Layer<br/>Decision<br/>动态调整各层决策的权重分配，适配市场变化<br/>文件: decision/position/pos_10"]
    LL3 --- N47
    N48["(设计态 / design) 再平衡触发 / Rebalance Trigger<br/>判定是否触发再平衡的条件检查，避免频繁或滞后再平<br/>衡<br/>文件: decision/position/pos_11"]
    LL3 --- N48
    N49["(设计态 / design) 仓位上限硬约束 / Position Cap<br/>Hard Constraint<br/>不可违反的仓位上限硬约束，兜底防止超仓<br/>文件: decision/position/pos_12"]
    LL3 --- N49
    N50["(设计态 / design) REDUCING→EXITING状态转换 /<br/>REDUCING to EXITING<br/>从减仓状态切换到清仓状态的决策，规范减仓到清仓的<br/>过渡<br/>文件: decision/position/pos_13"]
    LL3 --- N50
    N51["(设计态 / design) 风险预算→Kelly决策 / Risk<br/>Budget to Kelly<br/>把风险预算转换为凯利仓位参数，打通风险到仓位的映<br/>射<br/>文件: decision/position/pos_14"]
    LL3 --- N51
    N52["(设计态 / design) 半Kelly硬上限 / Half-Kelly<br/>Hard Cap<br/>凯利仓位减半作为硬上限，保守控制单笔风险敞口<br/>文件: decision/position/pos_15"]
    LL3 --- N52
    N53["(设计态 / design) 仓位降级 / Position<br/>Degradation<br/>风险升高时主动降低仓位等级，渐进式风险控制<br/>文件: decision/position/pos_16"]
    LL3 --- N53
    N54["(设计态 / design) 持仓状态→卖出阈值 / Position<br/>State to Sell Threshold<br/>按持仓状态动态调整卖出阈值，不同状态用不同止盈止<br/>损标准<br/>文件: decision/position/pos_17"]
    LL3 --- N54
    N55["(设计态 / design) 仓位四轨决策 / Position<br/>Four-Track Decision<br/>四轨对仓位的不同要求进行融合决策，处理多轨仓位冲<br/>突<br/>文件: decision/position/pos_18"]
    LL3 --- N55
    N56["(设计态 / design) 仓位裁决→执行 / Position<br/>Arbitration to Execution<br/>仓位裁决结果下发到执行层，完成决策到执行的闭环<br/>文件: decision/position/pos_19"]
    LL3 --- N56
