...
- orthogonal-view
- j1
summary: ZephyrAlpha 2.0 TOGAF 第 10 个架构视图——前端架构。承载 ADR-0007 拍板的方案 D（前端独立 `frontend/`
  顶级目录，与 `src/zephyr/` 14 层 Python 后端完全异构），对标 Bloomberg Terminal / Refinitiv Workspace
...
| 问题 | 归属视图 |
|------|---------|
| 后端 `src/zephyr/` 14 层分层与模块职责 | `03-application-architecture.md`（AA） |
| 前端依赖的业务数据对象（Order / Signal / Position 等）| `05-data-architecture.md`（DA） |
| 前端 ↔ 后端 REST/WebSocket/OpenAPI 字段级契约清单 | `architecture-model/contracts/cross-layer-contracts.yaml` |
| 前端运行时依赖的 Redis / PostgreSQL / LLM 技术栈选型 | `04-technology-architecture.md`（TA）|
| 前端安全策略（CSP / CORS / CSRF / XSS / OAuth）| `06-security-architecture.md`（SEC，当前 skeleton，后续深化时必须包含前端安全一节）|
| 前端与外部集成拓扑（API Gateway / CDN / 第三方登录）| `07-integration-architecture.md`（INTEG）|
| 前端运维（监控/日志/告警/发布流水线）| `08-operations-architecture.md`（OPS）|
...
- **ADR-0007** 前端层不进 src/ 14 层，作为独立 `frontend/` 平台层（2026-04-18 accepted）
- **ADR-0008** 四架构联邦制（Federated-Light）与 Metamodel 桥梁（2026-04-18 accepted）
...
| # | 原则 | 含义 | 违反后果 |
|---|------|------|---------|
| **FE-P1** | **技术栈异构隔离**（Heterogeneous Stack Isolation）| TypeScript / React / Vite / pnpm 技术栈与后端 Python / uv / pytest 完全隔离，不得互相侵入（无 transpile、无 embed、无 shared build tool）| CI 管线交叉污染；发布不可独立；打破 ADR-0007 |
...
> **📋 SSoT 声明**：前端模块清单（模块 ID / 状态 / 优先级 / runtime_plane / 激活条件等）的**唯一权威来源（Single Source of Truth）**是 [`frontend-model.yaml`](architecture-model/frontend/frontend-model.yaml)。本节仅提供架构级分层概览与职责说明；如需查询具体模块列表、状态或优先级，请以 YAML 为准。
...
> **📊 微前端拓扑图**：见 [`diagrams/frontend-mfe-topology.mmd`](diagrams/frontend-mfe-topology.mmd)

### 4.2 MFE 技术选型（Activation 时定）

> **📋 SSoT 声明**：前端技术栈选型（框架、构建工具、状态管理库、图表引擎等）的权威定义见 [`frontend-model.yaml`](architecture-model/frontend/frontend-model.yaml) 各模块的 `description` 字段。本节提供架构级选型决策与对比分析。
...
> **📊 前端构建管线图**：见 [`diagrams/frontend-build-pipeline.mmd`](diagrams/frontend-build-pipeline.mmd)
...
> **定位**：本节把前端 4 层（FE-L1 Apps / FE-L2 Platform / FE-L3 Packages / FE-L4 Tools）在 `04bis-runtime-planes.md` 正交视图的 **Hot / Warm / Cold 三平面**上做归属标注。**FE-L4 Tools 不入运行平面**（纯 dev-time 工具链，无运行时代码）。
> **SSoT**：`04bis-runtime-planes.md` §3.4 "前端平面归属" 是本标注的规范源头；本节只做前端视角的索引与澄清。

#### 7.5.1 前端模块三平面归属快查表

> **📋 SSoT 声明**：各模块的 `runtime_plane` 权威值定义在 [`frontend-model.yaml`](architecture-model/frontend/frontend-model.yaml)。下表为架构视角的快查索引。
...
#### 7.5.4 与 `04bis-runtime-planes.md` 的同步规则

| 当发生这些变动时 | 必须联动更新 |
|----------------|------------|
| 新增前端 App | 本节 §7.5.1 加一行（标注 Warm / Hot-adjacent / Cold）+ `04bis` §3.4 前端平面归属表同步 |
| 前端某模块延迟预算从 Warm 升级到 Hot-adjacent（或降级） | 本节 §7.5.2 Hot-adjacent 表更新 + `04bis` §5 跨面通信协议章节同步 + ADR 审批（如果是 Hot-adjacent 首次引入一整类场景）|
...
| "交易信号页面嵌到 L05 里" | 违反 ADR-0007 + FE-P1/FE-P2 | 强制踢回 G0 或 G1，不允许临时破例 |
...
| 档位 | 需起草的 ADR | 需更新的视图 |
|------|------------|-----------|
| G1 | ADR-0007-A（API 契约 OpenAPI 版本 / 错误格式 / 认证）| 本视图 v1.0.0 → v1.1.0（G1 运行反馈回注）|
| G2 | ADR-0007-B（前端治理扩展：ESLint / MFE 约束 / 共享包）| 本视图 §4 更新实际选型方案 |
| G3 | ADR-0007-C（团队协作 / Release Train / Design System v2）| 本视图 §4 扩展前端治理章节 |
| G4 | ADR-xxxx（AI Operator Frontend 整合）| OQ-063 F-1/F-2/F-3 激活闭环 |
