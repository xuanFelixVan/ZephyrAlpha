...
| S7 | **SRE / Ops / 运维** | 部署、监控、容量、成本、DR | 可用性、Runbook 可演练、成本可控 | you（deferred，`08-operations-architecture.md` skeleton 激活后独立）|
...
> **📊 业务价值流图**：见 [`diagrams/business-value-stream.mmd`](diagrams/business-value-stream.mmd)
...
| **Security / 安全性** | Personal scale；密钥管理、无公开暴露 | `06-security-architecture.md` skeleton，激活条件见其 §8 | — |
...
| **SLO-7** | **Data Quality**<br/>数据质量 | PIT / survivorship / lineage 三断言 + 完整度 / 一致性 / 及时性三维度 | 完整度（Completeness）：≥ **99.5%**（缺失率 ≤ 0.5%）<br/>一致性（Consistency，PIT 铁律不被破坏）：≥ **99.9%**<br/>及时性（Timeliness，同 SLO-1）：见 SLO-1<br/>F21-F25 fitness functions（`OQ-027` + `OQ-075` 三自研断言）：**100% pass**（任何失败阻塞上线）| Great Expectations / Soda 断言结果；pytest fitness function pass ratio；自研 `test_no_lookahead_bias.py` / `test_no_survivorship_bias.py` / `test_lineage_completeness.py` | 📌 → 04-TA §10 (TODO)<br/>`data_quality_assertion_pass_ratio{assertion,dataset}` | 任何断言失败阻塞因子上线（HO-2 handoff gate）|
| **SLO-Audit** | **Auditability**<br/>可审计性 | ADR 决策链完整、AI 协作决策日志七维度覆盖、factor/strategy 变更均可回溯 | ADR append-only：100%（不可变）<br/>AI 决策日志七维度（`OQ-063`）覆盖率：≥ **99%**（允许 &lt; 1% extensions 兜底）<br/>Git commit → ADR 双向索引完整度：100% | pre-commit hook + CI audit（`scripts/ci_audit/`）+ decision log schema validation | 📌 → 04-TA §10 (TODO)<br/>`adr_append_only_violations_total` / `decision_log_coverage_ratio` | **0 容忍**：任何违约阻塞 merge |
...
|---|---|---|---|---|
| RW-BA-01 | 投委会支持工具 — 为多人协作场景提供议事流程、审议记录、AI 辅助决策仪表盘 | `§2 Stakeholder S1-S12` + `l11_ml_platform/` | 系统从个人扩展到多人协作团队（≥2 位基金经理共同管理）| P3-STR-002 [待创建] |
