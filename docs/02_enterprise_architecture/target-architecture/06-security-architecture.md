...
summary: TOGAF Security Architecture 视图（v1.0.0 完整版，从 skeleton 升格）。本视图是 D6 安全架构（12 维审计基线 2.2/10 P0 红线）的真源文档。核心焦点：AI 安全（Prompt Injection / Agent Sandbox / LLM Security Gateway）+ Secret Scanning + 分阶段 IAM。LSG 接口契约 + Windows ACL Sandbox + git-secrets 扫描为三大 Phase 1 落地基石。
date: '2026-04-24'
ttl: permanent
---
...
- 给 ADR-0018（Agent Sandbox）和 ADR-0020（LLM Security Gateway）提供架构上下文

### 0.2 本文档不是

- ❌ **LLM Security Gateway 接口实现文档** — 见 [`03_modules/_b_track_interfaces/llm-security-gateway-interface.md`](../../03_modules/_b_track_interfaces/llm-security-gateway-interface.md)
- ❌ **密钥具体轮换操作手册** — 见 `docs/10_operations_and_sre/`（Phase 2 产出）
- ❌ **合规审计细则**（CSRC / MAS / SOX）— 未到接入真实资金阶段，不展开
- ❌ **代码级安全规范** — 见 `.cursor/rules/code-conventions.mdc` 与 `encoding-tool-guard.mdc`
- ❌ **SRE 事件响应 Runbook** — Phase 2 启动时另行产出
...
| 单人开发者（当前阶段）| §6 Secret Mgmt, §7 IAM Phase 1 |
...
1. **威胁在哪**：面对 AI 编码协作场景，主要攻击面是什么？（§3 Threat Model）
2. **怎么防御**：6 大核心服务如何协同防御？（§4 LSG + §5 Sandbox）
3. **密钥安全**：API Key / Token / 凭证如何不泄露到代码和日志？（§6 Secret Mgmt）
4. **谁能做什么**：当前单人场景到 Phase 3 多用户场景如何演进？（§7 IAM）
...
**基线评分**：12 维架构审计（`12-dimension-audit-matrix.md`）中 D6 Security 当前得分 **2.2/10（P0 红线）**。本文档升格（skeleton→active）是消除该红线的**前置条件**。Phase 1 末目标：D6 ≥ 5.5/10。
...
本系统采用 **"AI 协作域突出 + 渐进扩展"** 的域划分：

| 域 ID | 域名 | 信任级别 | 边界关口 | Phase 1 状态 |
|-------|------|:--------:|---------|:------------:|
| D-EXT | **外部接入域** | 不可信 | `l00_data_source/connectors/` ACL | ✅ 已有 |
...
| D-SECRET | **密钥管理域** | 最高信任 | `.env` + git-secrets + 1Password CLI | 🟡 需加固（§6）|
| D-MGMT | **管理/审计域** | 最高信任 | SQLite WAL + Session Log | 🔴 P0 待建 |

**关键决策**：Phase 1 单人阶段，**D-AI 和 D-AGT 是新增的 P0 安全域**，是传统量化系统没有的。这是 Vibe Coding 2.0 的核心安全挑战。
...
### 2.3 零信任原则（Phase 2 起启用）

**Phase 1 简化**：D-INT / D-STORE 之间不强制边界校验（单进程、单机、单人）。

**Phase 2 及以后**：接入真实券商后，必须升级为零信任：
...
| OWASP ID | 威胁 | 本系统暴露面 | Phase 1 防御 | 参考 |
|----------|------|-------------|-------------|------|
...
## §4 LLM Security Gateway (LSG) / LLM 安全闸

### 4.1 锚点声明

**本节是架构锚点**，实现细节见 [`03_modules/_b_track_interfaces/llm-security-gateway-interface.md`](../../03_modules/_b_track_interfaces/llm-security-gateway-interface.md)（v1.0.0，814 行）。
...
| 指标 | Phase 1 SLO | Phase 2 目标 | 当前基线 |
|------|:----------:|:------------:|:--------:|
| 误拦率（合法请求被拒）| < 2% | < 0.5% | 未测 |
| 漏拦率（攻击被放行）| < 5% | < 1% | 未测 |
...
**红队评估**：Phase 1 末必须跑一次红队评估（模拟 OWASP LLM01/02/06/08/09 攻击），记录漏拦率。阈值 > 5% 触发 **TECH-16 升级**（见 `technology-landscape.yaml upgrade_watchboard`）。
...
### 5.2 Phase 1 实现：Windows ACL + 只读挂载
...
**已知局限**：Windows ACL 不如 Linux namespace 隔离严格，Phase 2 接入真实资金前**必须**升级为 Docker 沙箱。
...
| Broker API Key（未来）| 券商 | `.env`（Phase 1 不用）| N/A | — |
...
- `scripts/hooks/git-secrets-setup.sh`（Phase 1 新增）
...
- `scripts/governance/secret_leak_scan.py` — 全库扫描 + 对比历史快照
...
- **PII 豁免**：当前系统无用户 PII；未来接入 KYC 数据时触发 Phase 2 升级
...
| Broker 交易日志（未来）| 7 年 | 合规要求（Phase 2 定）|
...
- 异地只读副本（Phase 2 upgrade_watchboard 触发）
...
|:-----:|-------|:--------:|-------------|
| Phase 0 | 基础奠基 | 2.2 → 3.5 | §6 L1+L2（.env + git-secrets）+ §9 SQLite audit schema |
| Phase 1 | 核心服务上线 | 3.5 → 5.5 | §4 LSG + §5 Agent Sandbox + §6 L3 + §9 Session Log |
| Phase 2 | 接入真实券商 | 5.5 → 7.0 | §7 RBAC 启用 + §6 1Password 迁移 + §8 数据分级落地 + 合规审计 |
| Phase 3 | 多用户/部分自动化 | 7.0 → 8.0 | 零信任 + OIDC + WAF + 多区审计 |
| Phase 4 | 机构级/全自动 | 8.0 → 9.0+ | SOC2/ISO27001 合规 + 专职安全团队 |

### 10.2 Phase 门禁（不可越级）

**Phase 0 → Phase 1 出口门禁**（必须 ALL PASS）：

- [ ] `scripts/hooks/git-secrets-setup.sh` 已部署，所有 commit 必须过检
- [ ] 历史 git 全库 trufflehog 扫描 0 finding
- [ ] SQLite audit.db schema 定稿 + 写入一条测试事件
- [ ] 本视图 `status: active` + 被 `00-overview.md §5` 引用

**Phase 1 → Phase 2 出口门禁**：
...
## §11 Incident Response / 事件响应（Phase 1 启动）
...
**Runbook 正文**：Phase 1 在 `docs/10_operations_and_sre/runbooks/` 产出（不在本视图中展开）。
...
| 2026-04-24 | **v1.0.0**：B-d-4 完整重写。从 skeleton 升格为 active。新增 §0 读者指南、§2 安全域 7 域模型（含 D-AI/D-AGT AI 特有域）、§3 STRIDE-Lite + OWASP LLM Top 10 威胁映射 + P0 攻击树、§4 LSG 架构锚点、§5 Agent Sandbox 锚点、§6 Secret 三道防线 fail-closed、§7 Phase-based IAM、§8 数据分级与保护、§9 审计日志 SQLite WAL + Session Log 双轨、§10 Phase 路线（D6 目标分 2.2→9.0）、§11 事件响应 Phase 1 启动。消除 12 维审计 D6 P0 红线。依赖 ADR-0018（Sandbox）+ ADR-0020（LSG）。 |
...
| ID | 问题 | 决定期限 |
|----|------|---------|
| SEC-OQ-01 | LSG 的红队评估工具选型（内部脚本 vs 外部工具 garak / promptbench）| Phase 1 初 |
| SEC-OQ-02 | Session Log 归档加密选型（AES-256-GCM vs age）| Phase 1 末 |
| SEC-OQ-03 | Phase 2 Sandbox 升级路径：Docker Desktop vs WSL2 vs Firejail（WSL 下）| Phase 1 末 |
| SEC-OQ-04 | Phase 2 IAM 身份提供者：本地 LDAP vs OIDC vs Authentik | Phase 2 初 |
| SEC-OQ-05 | 合规审计冻结期限（7 年？合规机构决定）| Phase 2 初 |
