...
summary: TOGAF Security Architecture 视图（v1.0.0 完整版，从 skeleton 升格）。本视图是 D6 安全架构（12 维审计基线 2.2/10 P0 红线）的真源文档。核心焦点：AI 安全（Prompt Injection / Agent Sandbox / LLM Security Gateway）+ Secret Scanning + 分阶段 IAM。LSG 接口契约 + Windows ACL Sandbox + git-secrets 扫描为三大 experimental 落地基石。
date: '2026-04-24'
ttl: permanent
---
...
**基线评分**：12 维架构审计（`12-dimension-audit-matrix.md`）中 D6 Security 当前得分 **2.2/10（P0 红线）**。本文档升格（skeleton→active）是消除该红线的**前置条件**。experimental 末目标：D6 ≥ 5.5/10。
...
**关键决策**：experimental 单人阶段，**D-AI 和 D-AGT 是新增的 P0 安全域**，是传统量化系统没有的。这是 Vibe Coding 2.0 的核心安全挑战。
