...
summary: TOGAF Information Architecture 视图。回答：docs/ 有哪些信息资产抽屉、分类依据、抽屉间关系、文档生命周期（workspace→review→canonical）、元数据标准。从原
  target-information-architecture.md §3 完整迁移。v1.2.0：K1 SRE 抽屉转 planned（含激活条件）。
date: '2026-04-22'
ttl: permanent
---
...
- `03_modules` — Module lifecycle documents: blueprint + construction plan + delivery records / 模块生命周期文档：蓝图+施工图+交付记录
  ...
  - `l00_data_source` — L00 blueprints / L00 蓝图
  - `l01_infrastructure` — L01 blueprints / L01 蓝图
  - `l02_alpha_factor` — L02 blueprints / L02 蓝图
  - `l03_signal_generation` — L03 blueprints / L03 蓝图
  - `l04_risk_management` — L04 blueprints / L04 蓝图
  - `l05_portfolio_construction` — L05 blueprints / L05 蓝图
  - `l06_trade_execution` — L06 blueprints / L06 蓝图
  - `l07_post_trade_analytics` — L07 blueprints / L07 蓝图
  - `l08_human_ai_interface` — L08 blueprints / L08 蓝图
  - `l09_research_innovation` — L09 blueprints / L09 蓝图
  - `l10_compliance` — L10 blueprints / L10 蓝图
  - `l11_ml_platform` — L11 documents / L11 文档
  - `l12_system_telemetry` — L12 documents / L12 文档
  - `l13_experimentation` — L13 documents / L13 文档
...
> **📊 文档拓扑图**：见 [`diagrams/docs-drawer-topology.mmd`](diagrams/docs-drawer-topology.mmd) — docs/ 20 抽屉分类拓扑
...
| `19_development_workspace/structure-and-mapping/terminology-mapping.md` | Terminology mapping / 术语映射表 |
