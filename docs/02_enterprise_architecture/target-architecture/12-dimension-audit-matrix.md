...
- ❌ 6 大核心服务的详细架构 → 见 `03-application-architecture.md §4A`
...
| D3 应用架构 | **5.2** | 6 大核心服务新定义 | Phase 1 未落地前为规范级 |
...
| D12 治理架构 | **5.5** | SSoT + ADR 体系有骨架 | SSoT Validator 未实现（Phase 0 任务）|
| **加权合计** | **4.73** | — | 综合分 6 分 passing 线之下，需 Phase 1-2 整体提升（v1.2.0 GATE-SUM 校正：原报 4.49 为 GLM 舍入，精确值 4.732） |
...
| 维度 | 分数 | 红线判定 | 处置 |
|------|------|---------|------|
| **D6 安全架构** | 2.2 | ❗ P0 红线（< 3.0）| Phase 1-2 必须升至 ≥ 6.0 |
| **D10 运维架构** | 2.8 | ❗ P0 红线（< 3.0）| Phase 3-4 必须升至 ≥ 6.0 |
| D11 安全运营 | 3.5 | ⚠️ P1 警戒（< 4.0）| Phase 1 升至 ≥ 5.0 |
| D5 MCP 集成 | 4.0 | ⚠️ P1 警戒 | Phase 1 升至 ≥ 6.0 |
| D7 Agent 编排 | 4.0 | ⚠️ P1 警戒 | Phase 1 升至 ≥ 6.0 |
| D8 反馈闭环 | 4.0 | ⚠️ P1 警戒 | Phase 2 升至 ≥ 6.0 |
...
| Phase | 重点提升维度 | 关键动作 |
|-------|------------|---------|
| Phase 1 | D3 / D6 / D7 / D12 | 6 大核心服务 InProcess* 落地 + LSG L1-L4 起步 + SSoT Validator 就位 |
| Phase 2 | D6 / D7 / D8 | 红队语料库 ≥ 150 条 + 幻觉检测规则完善 + FLE 13 项指标上线 |
| Phase 3 | D5 / D10 | Remote* 服务化 + OpenTelemetry 集成 |
| Phase 4 | D10 / D11 | 生产级可观测性 + 季度红队演练 + 供应链审计 |

### 3.3 里程碑门（最低合格线）

| 里程碑 | 综合分 ≥ | 关键单维度 ≥ |
|--------|:-------:|------------|
| Phase 1 退出门 | 6.0 | D6 ≥ 5.0, D12 ≥ 7.0 |
| Phase 2 退出门 | 7.0 | D6 ≥ 6.0, D7 ≥ 7.0, D8 ≥ 6.5 |
| Phase 3 退出门 | 7.5 | D5 ≥ 7.5, D10 ≥ 6.5 |
| Phase 4 稳定态 | 8.0 | D6 ≥ 8.0, D10 ≥ 8.0 |
...
| 零外部服务依赖（Phase 1）| `python -c "from zephyr import *"` 不联网 | 0.3 |
...
|:-----:|------|------|
| ≥ 8.0 | A | 生产级 |
| 6.0-7.9 | B | 可交付（Phase 1-3 合格）|
| 4.0-5.9 | C | 开发中（Phase 0-1 过渡期）|
| < 4.0 | D | 不合格（必须整改）|
...
| OQ | 议题 | 何时闭合 |
|----|------|---------|
| OQ-AM-01 | 每维度子指标的权重是否需要随 Phase 微调 | Phase 2 |
| OQ-AM-02 | 是否需要加入"技术债"维度 D13 | Phase 3 |
| OQ-AM-03 | 社区标杆对比（如 Citadel / Jane Street 内部 EA 评分）| Phase 4 |
