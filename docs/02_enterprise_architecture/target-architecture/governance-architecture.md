...
>
> **拍板依据**: `archive/reorg-2026-04-24/realized-as-adr/working-designs/governance-three-layer-boundary-design.md` v1.0.0（ARC-20260424-004，已落地为 ADR-0010）+ ADR-0010 v1.0.0；v1.1.0 增量 ADR-0009 v1.1.0 + OQ-076/079/080/081。
...
| 边界 | 落在哪个视图 | 本视图如何引用 |
|---|---|---|
| docs/ 文档抽屉治理规则 | `02-information-architecture.md` | §4 A-01/A-03 引用 |
| src/ 14 层代码分层规则 | `03-application-architecture.md` | §4 A-16 引用 |
| scripts/ 治理代码拓扑 | `03-application-architecture.md §5` | §2 Factory 层引用 |
| 数据层治理（PIT / Survivorship / Lineage）| `05-data-architecture.md §8` | §4 A-07 F 函数引用 |
| 集成契约治理 | `07-integration-architecture.md §6` | §4 A-15 OCP 引用 |
| 安全威胁治理（IAM / KMS / Audit）| `06-security-architecture.md`（skeleton）| §4 A-10 引用 |
| 运维治理（监控 / Runbook / DR）| `08-operations-architecture.md`（skeleton）| §6 T1-T6 联动 |
| 前端治理（ESLint / TypeScript strict / A11y）| `10-frontend-architecture.md` | §4 A-05 扩展 |
| **运行平面切分（Hot/Warm/Cold）** | **`04bis-runtime-planes.md`** | **§1.2bis 铁律** |
...
> **详细路径清单** → See `architecture-model/scripts/scripts-model.yaml`（governance/arch_guard/quality 三域）
...
> **📊 治理三层全景图**：见 [`diagrams/governance-three-layers.mmd`](diagrams/governance-three-layers.mmd)
...
> **📊 D2-B 闭环接口图**：见 [`diagrams/governance-d2b-loop.mmd`](diagrams/governance-d2b-loop.mmd)
...
| A-14 | Kill switch | Runtime | — | Sprint 9（l10_compliance）|
...
> **📊 治理系统激活时间表**：见 [`diagrams/governance-activation-gantt.mmd`](diagrams/governance-activation-gantt.mmd)
...
**本视图只做**：定义三层分层边界 + 承载 D1-D4 拍板 + 关闭 OQ-026 + 预留 AI 口子 + 锁定激活时间表 + 作为 ADR-0010 + ADR-0021 同源视图 + 定义 6 大核心服务治理归属。

**本视图不做**：不新建目录、不写脚本、不激活检查器、不实施 D3-B 口子、不动 39 系统内部结构、不动 src/ 14 层、不动 ADR-0001~0009。
...
> 完整修订历史：`git log --oneline -- 09-governance-architecture.md`
