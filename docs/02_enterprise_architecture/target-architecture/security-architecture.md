...
- �?ADR-0018（Agent Sandbox）和 ADR-0020（LLM Security Gateway）提供架构上下文
...
**驱动关系**：本视图受应用架构（§03-application-architecture.md §4A 6 大核心服务）驱动，并**反向约束**应用架构（拒绝不满足安全门禁的服务上线）�?

**基线评分**�?2 维架构审计（`12-dimension-audit-matrix.md`）中 D6 Security 当前得分 **2.2/10（P0 红线�?*。本文档升格（skeleton→active）是消除该红线的**前置条件**。experimental 末目标：D6 �?5.5/10�?
...
|-------|------|:--------:|---------|:------------:|
| D-EXT | **外部接入�?* | 不可�?| `l00_data_source/connectors/` ACL | �?已有 |
...
| LLM05 | **Supply Chain** | 第三�?MCP Tool 不可�?| Agent Sandbox 白名单（§5）| ADR-0018 |
...
**红队评估**：experimental 末必须跑一次红队评估（模拟 OWASP LLM01/02/06/08/09 攻击），记录漏拦率。阈�?> 5% 触发 **TECH-16 升级**（见 `technology-landscape.yaml upgrade_watchboard`）�?
...
**本节是架构锚�?*，实现细节见 [`03_modules/_b_track_interfaces/agent-orchestrator-interface.md §7 Sandbox`](../../03_modules/_b_track_interfaces/agent-orchestrator-interface.md)，技术选型�?ADR-0018�?
...
- [ ] 本视�?`status: active` + �?`00-overview.md §5` 引用
...
| 2026-04-24 | **v1.0.0**：B-d-4 完整重写。从 skeleton 升格�?active。新�?§0 读者指南、�? 安全�?7 域模型（�?D-AI/D-AGT AI 特有域）、�? STRIDE-Lite + OWASP LLM Top 10 威胁映射 + P0 攻击树、�? LSG 架构锚点、�? Agent Sandbox 锚点、�? Secret 三道防线 fail-closed、�? Phase-based IAM、�? 数据分级与保护、�? 审计日志 SQLite WAL + Session Log 双轨、�?0 Phase 路线（D6 目标�?2.2�?.0）、�?1 事件响应 experimental 启动。消�?12 维审�?D6 P0 红线。依�?ADR-0018（Sandbox�? ADR-0020（LSG）�?|
