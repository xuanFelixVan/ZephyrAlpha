...
> | §23.5-1 消除project\_rules↔onboarding重复 | 规则文件优化        | ✅ 已完成                                                            | L0每个RULE标注YAML真源引用，L1开头标注"本文件是L0的详细补充"，去重完成                | 无               |
> | §23.5-2 重构三层职责边界                      | 规则文件优化        | ✅ 已完成                                                            | L0瘦身(v0.20.0)/L1聚焦(施工指导)/AGENTS区分(Agent接入宪法)，职责边界已重构            | 无               |
> | §23.5-3 创建缺失标准文档                      | ~~可提前推进~~ 已过时 | D56 ruling后YAML是唯一SSoT，trae\_010/011/012已包含完整标准，再创建独立MD文档违反真源唯一 | 已过时，不执行                                                |                 |
> | §23.5-4 YAML frontmatter大改            | 规则文件优化        | ✅ 已完成                                                            | 58个规则YAML frontmatter字段统一(rule_id/title/version/layer/module_id/depends_on/tags/stability/safety_level/ai_autonomy/aliases)，48张门禁DM-100275~322全部COMPLETED | 无               |
...
> - 阶段7分两类：运行时升级类（R7/R8/测试，依赖阶段5/6）+ 规则文件优化类（§23.5第1/2/4/5项已完成，第3项已过时）
...
| 阶段7 R7/R8+测试   |    未开始   | §23.5第1/2/4/5项已完成；运行时升级类依赖阶段5/6                |
...
### 23.5 不做的事项（原推迟到阶段7，第1/2/4/5项已完成，第3项已过时）

> **2026-06-23 更新**：§23.5第1/2/4/5项已完成，第3项已过时。
