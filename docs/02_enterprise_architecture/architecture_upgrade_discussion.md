...
>
> **剩余架构债务（2026-06-22 清理后剩余）**：
>
> | 债务ID                                  | 类型            | 状态                                                              | 说明                                                     | 阻塞条件            |
> | ------------------------------------- | ------------- | --------------------------------------------------------------- | ------------------------------------------------------ | --------------- |
> | \_yaml\_to\_md.py                     | 工具缺失          | 非阻塞                                                             | YAML→MD反向生成工具，计划中未创建。D56 ruling已决定YAML是唯一SSoT，MD副本不再需要 | 如需MD副本可创建，当前非阻塞 |
> | §23.5-1 消除project\_rules↔onboarding重复 | 规则文件优化        | 可提前推进                                                           | 无运行时依赖，§23.5推迟理由是"统一做"（组织决策非技术依赖）                      | 无               |
> | §23.5-2 重构三层职责边界                      | 规则文件优化        | 可提前推进                                                           | L0瘦身/L1聚焦/AGENTS区分，无运行时依赖                              | 无               |
> | §23.5-3 创建缺失标准文档                      | ~~可提前推进~~ 已过时 | D56 ruling后YAML是唯一SSoT，trae\_010/011/012已包含完整标准，再创建独立MD文档违反真源唯一 | 已过时，不执行                                                |                 |
> | §23.5-4 YAML frontmatter大改            | 规则文件优化        | 可提前推进                                                           | 规则文件元数据统一，无运行时依赖                                       | 无               |
> | R7安全升级                                | 运行时升级         | 推迟到阶段7                                                          | 依赖阶段5/6完成                                              | 阶段5→6完成         |
> | R8基础设施升级                              | 运行时升级         | 推迟到阶段7                                                          | 依赖阶段5/6完成                                              | 阶段5→6完成         |
> | 全量功能测试                                | 测试            | 推迟到阶段7                                                          | 需R3-R8升级完成                                             | 阶段5→6→7完成       |
>
> **本次session已清理的债务（2026-06-22）**：
>
> - DM-100260 Phase F-D1D2 → COMPLETED
> - 38个VERIFIED任务 → COMPLETED
> - g1-ingest.yaml门禁文件 → 已修复
> - shared\_08.\_contracts导入链 → 已修复
> - 921张任务卡execution\_model → 已修复
> - BLUEPRINT路径连字符 → 已修复（3833个文件）
> - cleanup\_stale\_tasks.py → 已删除
> - OPS-2026061803 persistence引用 → 已完成
>
> **执行顺序更新（2026-06-22 Owner裁定）**：
>
> - DM-100260提前推进（原blocked\_by阶段7，确认无运行时依赖）
> - 阶段7分两类：运行时升级类（R7/R8/测试，依赖阶段5/6）+ 规则文件优化类（§23.5的4项，无依赖可提前推进）
...
阶段7：R7/R8 + 全量功能测试 + 规则文件格式升级（§23.5第1-4项可提前推进，见§23.5）
...
| 问题              | 裁定                                     | 状态 |
| --------------- | -------------------------------------- | :---: |
| 无模型版本管理         | 建立models/registry.yaml，记录版本+路径+GPU显存需求 | 未完成 |
| 无按需加载机制         | 实现ModelManager，按需加载+LRU淘汰              | 未完成 |
| 未纳入Git LFS      | .gitignore排除大文件，首次运行自动下载               | 未完成 |
| 迁移到data/models/ | 与数据目录统一管理                              | ✅ 已迁移 |

> **推迟说明（2026-06-22）**：前3项（模型版本管理/按需加载/Git LFS）属功能实现工作，非搬家对齐范畴。推迟到阶段7b之后执行，不影响当前 P1 治理瘦身推进。搬家部分（迁移到 data/models/）已完成。
...
| D77 | D-TEST 【已废止：2026-06-22 测试文件无依赖关系，不需要功能域管理，D-TEST不创建】保留为第40域                         | **独立保留，不归入其他域**                                                                                                        | 2104个test类型边界清晰，拆分会破坏测试组织                                                  |
...
| ------------- | :----: | :-: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------: | ------------------- |
| governance.db |  11MB  |  26 | 1,220 tasks(1,016 COMPLETED + 188 CANCELLED + 16 PENDING), 6401 gates, 398 knowledge, 23687 audit\_entries, 386 drift\_events, 99613 fle\_metrics                                                |    v27   | sqlite\_schema.py   |
...
| 阶段7 R7/R8+测试   |    未开始   | §23.5可提前推进；运行时升级类依赖阶段5/6                |
...
| governance.db 任务 | 1,220（1,016 COMPLETED + 188 CANCELLED + 16 PENDING） |
...
> **原则**：只修复"不依赖搬家且 SSoT 已明确"的漂移；结构优化/重复消除见§23.5（2026-06-22 Owner裁定可提前推进）。
...
### 23.5 不做的事项（原推迟到阶段7，2026-06-22 Owner裁定可提前推进）

> **2026-06-22 Owner裁定**：§23.5第1-4项无运行时依赖（前置条件"路径稳定"已满足），可提前推进。第5项已完成。

|  #  | 事项                                             | 原推迟理由                       | 当前状态                                                                                |
| :-: | ---------------------------------------------- | --------------------------- | ----------------------------------------------------------------------------------- |
|  1  | 大规模消除 project\_rules ↔ onboarding 重复           | 阶段7规则文件格式升级时统一做             | 可提前推进（无运行时依赖）                                                                       |
|  2  | 重构三层职责边界（L0瘦身/L1聚焦/AGENTS区分）                   | 搬家后路径稳定再做                   | 可提前推进（阶段4搬家已完成）                                                                     |
|  3  | ~~创建缺失标准文档（code-construction-standards.md 等）~~ | ~~待阶段7~~                    | **已过时（2026-06-22）**：D56 ruling后YAML是唯一SSoT，trae\_010/011/012已包含完整标准，再创建独立MD文档违反真源唯一 |
|  4  | YAML frontmatter 大改                            | 待阶段7                        | 可提前推进（⚠️需等48张门禁DM-100275\~322完成，避免同时读写规则YAML）                                       |
...
> **工作内容**：P0（搬家对齐）✅已完成 | P1-1（审计脚本消费者）✅已完成 | P1-3（合并重复规则）✅已完成（CP-9002~9004） | P1-2（审计门禁拦截率）⏸️暂缓（依赖P2-1） | P1-4（冷启动步骤精简）❌取消（与trae_057 §7冲突） | P2-P3 待阶段7后启动
...
| 门禁数              | 21个YAML（48张门禁任务卡DM-100275\~322创建中） | CI/CD 通常 5-15 个        | 3 倍     |
