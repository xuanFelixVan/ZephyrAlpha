...
flowchart TD
    ...
    DS31191["(生产态 / production) signal.composite /<br/>信号.合成信号<br/>合成交易信号（多因子加权/截面排名<br/>/置信度），CTR-P1-015 SynthesizedSignal<br/>契约: CTR-P1-015 · 域: 信号遗留设计态<br/>跨域节点 / cross-domain"]
    DS31192["(生产态 / production) risk.limits / 风险.限额<br/>风险限额（max_position/max_drawdown/exposure_<br/>limits），CTR-003 RiskLimits<br/>契约: CTR-003 · 域: 风控<br/>跨域节点 / cross-domain"]
    ...
    JOB757629 ~~~ JOB1154367
    JOB1154367 ~~~ JOB757627
    JOB757627 ~~~ JOB757630
    JOB757630 ~~~ JOB757632
    JOB757632 ~~~ JOB757631
    JOB757631 ~~~ JOB757634
    ...
    class DS31194,DS31193,DS31195,JOB1154368,JOB1154367 production
    ...
    class DS31191,DS31192 external_prod
```
...
| DS-31194 | fill.executed / 成交.已成交 | production / 生产 | D_EX_CORE / 执行核心 | production / 生产 | MOD-L06-001 | 成交回报（symbol/quantity/price/commission/timestamp），CTR-005 Fill |
| DS-31193 | order.target / 订单.目标订单 | production / 生产 | D_PF_CORE / 组合核心 | production / 生产 | MOD-L05-001 | 目标订单（symbol/side/quantity/price/order_type），CTR-004 Order |
...
| DS-31195 | position.snapshot / 持仓.快照 | production / 生产 | D_EX_CORE / 执行核心 | production / 生产 | MOD-L06-001 | 持仓快照（symbol/quantity/avg_cost/market_value/timestamp），CTR-006 PositionSnapshot |
...
| JOB-1154368 | execute.order / 执行.订单 | event_driven / 事件驱动 | production / 生产 | MOD-L06-001 | 执行订单（实盘/模拟），产出DS-008 fill.executed + DS-009 position.snapshot |
| JOB-1154367 | generate.order / 生成.订单 | event_driven / 事件驱动 | production / 生产 | MOD-L05-001 | 根据信号+风险限额生成目标订单，产出DS-007 order.target |
