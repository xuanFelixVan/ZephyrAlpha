...
| 数据源 | 任务数 | 主要采什么 |
|--------|--------|-----------|
| **akshare**（AKShare） | 62 | 估值、融资融券、龙虎榜、大宗交易、宏观 |
...
### 1. akshare（AKShare）— 62 个任务，主力数据源
...
flowchart TD
    subgraph ext_sources["外部数据源 / External Data Sources"]
        S0["(生产态 / production) akshare / AKShare<br/>开源数据源，采估值、融资融券、龙虎榜、大宗交易、<br/>宏观数据、限售解禁等事件类数据。<br/>数据源 / data-source（62任务）"]
        S1["(生产态 / production) miniqmt / 迅投QMT<br/>主力数据源，采 A股/港股/期货的 K线行情（日/周<br/>/月/分钟级）和财务报表、股东数据、期权可转债等。<br/>数据源 / data-source（60任务）"]
