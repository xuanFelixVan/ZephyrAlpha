...
flowchart LR
    DS11213["[design]factor.ashare_alpha87<br/>A股Alpha#87因子信号<br/>（多因子截面排名）"]
    DS11214["[design]factor.ashare_capital_flow<br/>A股资金流向因子<br/>（主力资金净流入/流出）"]
    DS11215["[design]factor.ashare_cross_market<br/>A股跨市场因子<br/>（AH股溢价/跨市套利信号）"]
    DS11216["[design]factor.ashare_fundamental<br/>A股基本面因子<br/>（PE/PB/ROE/股息率等）"]
    DS11217["[design]factor.ashare_institutional<br/>A股机构持仓变动因子<br/>（基金/外资持仓变化）"]
    DS11218["[design]factor.ashare_intraday<br/>A股日内动量因子<br/>（开盘/尾盘效应）"]
    DS11219["[design]factor.ashare_irl<br/>A股IRL因子<br/>（逆强化学习推导的交易偏好信号）"]
    DS11220["[design]factor.ashare_market_structure<br/>A股市场结构因子<br/>（支撑压力/趋势结构）"]
    DS11221["[design]factor.ashare_microstructure<br/>A股微观结构因子<br/>（订单簿不平衡/买卖价差）"]
    DS11222["[design]factor.ashare_pattern_signal<br/>A股K线形态因子<br/>（技术形态识别信号）"]
    DS11223["[design]factor.ashare_ps_liquidity<br/>A股PS流动性因子<br/>（换手率/成交额流动性指标）"]
    DS11224["[design]factor.ashare_sector<br/>A股板块轮动因子<br/>（行业板块动量/资金流）"]
    DS11225["[design]factor.ashare_smc<br/>A股SMC因子<br/>（智能货币概念/机构筹码分布）"]
    DS11226["[design]factor.ashare_technical_indicator<br/>A股技术指标因子<br/>（MACD/RSI/KDJ等）"]
    JOB757577("[design]compute.ashare_alpha87<br/>计算Alpha#87因子<br/>（消费OHLC K线，产出因子信号）")
    JOB757578("[design]compute.ashare_capital_flow<br/>计算资金流因子<br/>（消费OHLC K线，产出因子信号）")
    JOB757579("[design]compute.ashare_cross_market<br/>计算跨市场因子<br/>（消费OHLC K线，产出因子信号）")
    JOB757580("[design]compute.ashare_fundamental<br/>计算基本面因子<br/>（消费OHLC K线，产出因子信号）")
    JOB757581("[design]compute.ashare_institutional<br/>计算机构行为因子<br/>（消费OHLC K线，产出因子信号）")
    JOB757582("[design]compute.ashare_intraday<br/>计算日内因子<br/>（消费OHLC K线，产出因子信号）")
    JOB757583("[design]compute.ashare_irl<br/>计算逆强化学习因子<br/>（消费OHLC K线，产出因子信号）")
    JOB757584("[design]compute.ashare_market_structure<br/>计算市场结构因子<br/>（消费OHLC K线，产出因子信号）")
    JOB757585("[design]compute.ashare_microstructure<br/>计算微观结构因子<br/>（消费OHLC K线，产出因子信号）")
    JOB757586("[design]compute.ashare_pattern_signal<br/>计算形态信号因子<br/>（消费OHLC K线，产出因子信号）")
    JOB757587("[design]compute.ashare_ps_liquidity<br/>计算流动性因子<br/>（消费OHLC K线，产出因子信号）")
    JOB757588("[design]compute.ashare_sector<br/>计算板块因子<br/>（消费OHLC K线，产出因子信号）")
    JOB757589("[design]compute.ashare_smc<br/>计算智能货币概念因子<br/>（消费OHLC K线，产出因子信号）")
    JOB757590("[design]compute.ashare_technical_indicator<br/>计算技术指标因子<br/>（消费OHLC K线，产出因子信号）")
