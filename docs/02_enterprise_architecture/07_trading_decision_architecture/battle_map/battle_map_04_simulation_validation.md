...
| 要素 | 内容 |
|---|---|
| ① 触发条件 | BM-BT-07 回测通过/研究员配置 |
| ② 消费数据/因子 | BM-BT-01 回测引擎+策略代码 |
| ③ 参数 | 订单簿仿真、价格生成、微观结构模拟、逐Tick回放、时间压缩 |
| ④ 数据流 | 策略+仿真市场→撮合仿真→仿真成交→仿真结果→BM-SIM-06分析 |
| ⑤ 代码映射 | D-SIMULATION-01/07（planned） |
| ⑥ 降级/中止 | 市场仿真器未就绪→仅历史回测(无what-if能力) |
...
📌 概念覆盖清单（草稿H3逐字登记）：§29.13 市场微观结构深度建模；§29.23 数字孪生市场仿真（v5.1新增）；§6.1 不操纵市场
...
| 要素 | 内容 |
|---|---|
| ① 触发条件 | BM-SIM-01 市场仿真就绪 |
| ② 消费数据/因子 | 策略代码+BM-SIM-01 仿真市场 |
| ③ 参数 | 策略沙箱、信号模拟、组合模拟 |
| ④ 数据流 | 策略+仿真市场→策略沙箱→信号模拟+组合模拟→仿真PnL→BM-SIM-06 |
| ⑤ 代码映射 | D-SIMULATION-02（planned） |
| ⑥ 降级/中止 | 策略仿真器未就绪→仅回测(无沙箱隔离) |
...
| 要素 | 内容 |
|---|---|
| ① 触发条件 | BM-SIM-02 策略仿真后/研究员配置 |
| ② 消费数据/因子 | BM-SIM-02 仿真结果+场景定义 |
| ③ 参数 | 蒙特卡洛路径数、GPU加速、历史场景库、自定义场景 |
| ④ 数据流 | 场景定义→蒙特卡洛百万路径→仿真结果分布→BM-SIM-06分析 |
| ⑤ 代码映射 | D-SIMULATION-05/06（planned） |
| ⑥ 降级/中止 | 蒙特卡洛未就绪→少量场景手动跑(无统计意义) |
...
| 要素 | 内容 |
|---|---|
| ① 触发条件 | BM-SIM-03 蒙特卡洛完成/风控参数调整 |
| ② 消费数据/因子 | BM-SIM-01 仿真市场+BM-SIM-03 蒙特卡洛路径 |
| ③ 参数 | VaR模拟、回撤模拟、熔断模拟 |
| ④ 数据流 | 仿真市场+MC路径→风控仿真→VaR/回撤/熔断评估→BM-SIM-06分析+D-RISK风控参数 |
| ⑤ 代码映射 | MOD-SIM-003 risk_simulator.py（stable） |
| ⑥ 降级/中止 | 风控仿真器未就绪→仅历史VaR(无蒙特卡洛VaR) |
...
📌 概念覆盖清单（草稿H3逐字登记）：§17.13 D-RISK 风控域缺失模块
...
| 要素 | 内容 |
|---|---|
| ① 触发条件 | BM-SIM-03 场景生成后/定时压力测试 |
| ② 消费数据/因子 | BM-SIM-03 极端场景+历史极端事件库 |
| ③ 参数 | 历史情景(2008/2015/2020)、假设情景、反向压力测试、敏感性分析、传染效应 |
| ④ 数据流 | 极端场景→压力测试→策略表现→BM-SIM-06分析+D-RISK风控参数调整 |
| ⑤ 代码映射 | D-SIMULATION-04（testing）+D-SIMULATION-10（planned） |
| ⑥ 降级/中止 | 极端事件仿真器未就绪→仅历史重放(无黑天鹅模拟) |
...
|---|---|
| ① 触发条件 | 架构变更前/混沌实验 |
| ② 消费数据/因子 | depgraph 依赖图+模块状态 |
| ③ 参数 | what-if仿真、批量/流式/实时三模式、依赖拓扑实时映射、变更预演 |
| ④ 数据流 | depgraph→数字孪生→what-if仿真→变更影响预测→ADR决策 |
| ⑤ 代码映射 | D-SIMULATION-13/14（planned） |
| ⑥ 降级/中止 | 数字孪生未就绪→人工评估变更影响(精度低) |
...
| 要素 | 内容 |
|---|---|
| ① 触发条件 | BM-SIM-01~05 仿真完成 |
| ② 消费数据/因子 | 仿真成交+PnL+场景结果 |
| ③ 参数 | 统计检验、可视化、报告生成 |
| ④ 数据流 | 仿真结果→统计检验+可视化→SimulationResult事件→D-RISK风控参数+D-PF-CORE组合参考 |
| ⑤ 代码映射 | D-SIMULATION-12（planned） |
| ⑥ 降级/中止 | 结果分析器未就绪→原始仿真数据(无统计检验,人工分析) |
