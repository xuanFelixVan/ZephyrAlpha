...
> 展示 回测验证 阶段全部 49 个环节及流转边，颜色区分五态。
...
| 要素 | 内容 |
|---|---|
| ① 触发条件 | 研究员提交策略/自动调度(BM-BT-07) |
| ② 消费数据/因子 | BM-RES-01 特征(PIT)+策略代码 |
| ③ 参数 | 向量化vs事件驱动、市价/限价/滑点/Tick级5档撮合、A股T+1/万三/5元/1bp滑点 |
| ④ 数据流 | 策略+历史数据→撮合引擎→成交记录→BacktestResult→BM-BT-02 |
| ⑤ 代码映射 | BT-01~BT-04（stable, production） |
| ⑥ 降级/中止 | 事件驱动引擎未就绪→仅向量化回测(精度低) |
...
|---|---|
| ① 触发条件 | BM-BT-01 引擎启动 |
| ② 消费数据/因子 | D-DATA MiniQMT Provider + ClickHouse c1_market |
| ③ 参数 | 持仓/现金/PnL/净值曲线计算、多源数据切换 |
| ④ 数据流 | 多源数据→data_handler→portfolio→BacktestResult |
| ⑤ 代码映射 | BT-05/06（stable, production） |
| ⑥ 降级/中止 | Tick数据缺失→降级日线回测(精度低) |
...
|---|---|
| ① 触发条件 | BM-BT-01 回测完成 |
| ② 消费数据/因子 | BacktestResult 净值曲线+成交记录 |
| ③ 参数 | Sharpe/Sortino/MaxDD/IC/IR/胜率、Tick回放秒级/30秒/5秒 |
| ④ 数据流 | BacktestResult→metrics计算+Tick回放→绩效报告→BM-BT-05过拟合检测 |
| ⑤ 代码映射 | BT-07/08/09（stable, production） |
| ⑥ 降级/中止 | Tick回放未就绪→仅日线指标(无秒级验证) |
...
|---|---|
| ① 触发条件 | BM-BT-01 数据接入时 |
| ② 消费数据/因子 | BM-RES-01 特征存储(PIT) |
| ③ 参数 | PIT三公理、AS OF JOIN、Embargo期 |
| ④ 数据流 | 特征请求→PIT校验→AS OF JOIN→当时已知值→回测引擎 |
| ⑤ 代码映射 | BT-10 pit_manager（stable, production） |
| ⑥ 降级/中止 | PIT管理器未就绪→回测不可信(硬阻断,禁止上线) |
...
|---|---|
| ① 触发条件 | BM-BT-03 绩效产出后 |
| ② 消费数据/因子 | BacktestResult+样本内外对比 |
| ③ 参数 | 三维度(样本内外/参数敏感性/多重比较)+三层(统计/经济/稳健) |
| ④ 数据流 | BacktestResult→过拟合检测→OverfittingDetected事件→BM-BT-07决策门控 |
| ⑤ 代码映射 | BT-11 overfitting_detector（stable, production） |
| ⑥ 降级/中止 | 过拟合检测未就绪→人工review(无自动门禁,风险高) |
...
| 要素 | 内容 |
|---|---|
| ① 触发条件 | BM-BT-05 过拟合检测后 |
| ② 消费数据/因子 | BacktestResult+参数空间 |
| ③ 参数 | 滚动窗口大小、样本外验证、参数稳定性区域 |
| ④ 数据流 | 参数空间→滚动窗口回测→样本外验证→参数稳定性区域→BM-BT-07 |
| ⑤ 代码映射 | BT-12 walk_forward（stable, production） |
| ⑥ 降级/中止 | WFO未就绪→单次回测(无稳健性验证) |
...
| 要素 | 内容 |
|---|---|
| ① 触发条件 | BM-BT-05/06 检测通过 |
| ② 消费数据/因子 | 过拟合检测+WFO结果+参数稳定性 |
| ③ 参数 | IS→WFA→OOS三阶段不可跳级、参数稳定性区域、BacktestRunArtifact持久化 |
| ④ 数据流 | 检测结果→决策门控→BacktestPassed事件→BM-SIM-01仿真/D-ML-SERVE影子验证 |
| ⑤ 代码映射 | BT-13/14/15/16（stable, production） |
| ⑥ 降级/中止 | 决策门控未就绪→人工审批(无自动门禁,风险高) |
...
| ① 触发条件 | BM-BT-01 引擎初始化 |
| ② 消费数据/因子 | 策略代码+配置 |
| ③ 参数 | BacktestEngineBase ABC、BacktestResult契约(CTR-P1-016) |
| ④ 数据流 | 策略+配置→engine_base初始化→BacktestEngineBase实例→子引擎 |
| ⑤ 代码映射 | BT-01 core/engine_base.py（stable, production） |
| ⑥ 降级/中止 | 引擎基座未就绪→回测不可启动(硬阻断) |
...
|---|---|
| ① 触发条件 | 策略提交+向量化模式选择 |
| ② 消费数据/因子 | BM-RES-01 特征(PIT)+策略信号 |
| ③ 参数 | 向量化模式、批量计算 |
| ④ 数据流 | 信号矩阵→向量化持仓→收益矩阵→BacktestResult |
| ⑤ 代码映射 | BT-02 implementations/vectorized_engine.py（stable, production） |
| ⑥ 降级/中止 | 向量化引擎不可用→无快速筛选(仅事件驱动,慢10x) |
...
|---|---|
| ① 触发条件 | 事件驱动回测中订单提交 |
| ② 消费数据/因子 | Tick数据+订单簿深度 |
| ③ 参数 | 市价/限价/滑点模型/Tick级5档撮合 |
| ④ 数据流 | 订单→撮合引擎→成交记录→BacktestResult |
| ⑤ 代码映射 | BT-03 core/matching_engine.py（stable, production） |
| ⑥ 降级/中止 | 撮合引擎不可用→用简单收盘价成交(精度低) |
...
| ① 触发条件 | 撮合引擎执行时自动应用 |
| ② 消费数据/因子 | 交易规则配置 |
| ③ 参数 | T+1、佣金万三、最低5元、滑点1bp |
| ④ 数据流 | 成交信号→约束应用→实际成交价+费用→BacktestResult |
| ⑤ 代码映射 | BT-04 core/matching_logic.py（stable, production） |
| ⑥ 降级/中止 | 约束模块不可用→无A股约束(回测失真) |
...
| ② 消费数据/因子 | 策略参数网格+回测配置 |
| ③ 参数 | 批量大小、参数网格、队列策略 |
| ④ 数据流 | 参数网格→调度器→批量回测→结果聚合 |
| ⑤ 代码映射 | BT-17 services/scheduler.py（planned） |
| ⑥ 降级/中止 | 调度器未就绪→手动逐个回测(效率低) |
...
|---|---|
| ① 触发条件 | 大批量参数网格回测请求 |
| ② 消费数据/因子 | 策略参数网格+历史数据缓存 |
| ③ 参数 | 并行度、缓存策略、数据预加载 |
| ④ 数据流 | 参数网格→并行调度→向量化回测→缓存命中检查→结果聚合 |
| ⑤ 代码映射 | BT-01~BT-20 加速层（planned） |
| ⑥ 降级/中止 | 加速层未就绪→串行回测(慢但正确) |
...
| ② 消费数据/因子 | 成交记录+初始资金 |
| ③ 参数 | 初始资金、持仓更新逻辑 |
| ④ 数据流 | 成交→持仓更新→净值曲线→BacktestResult |
| ⑤ 代码映射 | BT-05 core/portfolio.py（stable, production） |
| ⑥ 降级/中止 | 组合管理不可用→无法记录持仓(回测中断) |
...
|---|---|
| ① 触发条件 | 回测引擎请求数据 |
| ② 消费数据/因子 | miniQMT Tick + ClickHouse 日线 |
| ③ 参数 | 数据源选择、PIT对齐 |
| ④ 数据流 | 数据源→data_handler→回测引擎→BacktestResult |
| ⑤ 代码映射 | BT-06 core/data_handler.py（stable, production） |
| ⑥ 降级/中止 | 数据源不可用→降级到单一数据源(覆盖度降低) |
...
| ② 消费数据/因子 | 回测参数+历史结果 |
| ③ 参数 | 缓存策略、过期时间 |
| ④ 数据流 | 回测请求→缓存检查→命中/未命中→结果 |
| ⑤ 代码映射 | BT-20 services/cache_manager.py（planned, P2） |
| ⑥ 降级/中止 | 缓存不可用→每次重新计算(慢但正确) |
...
| ② 消费数据/因子 | 原始市场数据 |
| ③ 参数 | 缺失阈值、异常检测规则 |
| ④ 数据流 | 原始数据→质量检查→清洗后数据→回测引擎 |
| ⑤ 代码映射 | BT-22 services/data_quality_checker.py（planned） |
| ⑥ 降级/中止 | 检查器不可用→跳过检查(数据质量风险) |
...
|---|---|
| ① 触发条件 | 回测数据加载时 |
| ② 消费数据/因子 | 历史成分股数据+退市股票数据 |
| ③ 参数 | 成分股还原时点、退市数据处理 |
| ④ 数据流 | 当前成分股→历史还原→全样本(含退市)→回测引擎 |
| ⑤ 代码映射 | BT-06 data_handler 扩展（planned） |
| ⑥ 降级/中止 | 退市数据缺失→仅用存活股票(收益虚高风险) |
...
| ② 消费数据/因子 | 净值曲线+成交记录 |
| ③ 参数 | 指标集合(年化/夏普/回撤/胜率) |
| ④ 数据流 | 净值曲线→metrics计算→绩效指标→BacktestResult |
| ⑤ 代码映射 | BT-07 core/metrics.py（generated） |
| ⑥ 降级/中止 | 指标计算不可用→仅输出原始净值(无绩效评估) |
...
|---|---|
| ① 触发条件 | 事件驱动回测启动 |
| ② 消费数据/因子 | miniQMT Tick 历史数据 |
| ③ 参数 | 回放速度、Tick过滤 |
| ④ 数据流 | Tick历史→tick_replay→事件驱动引擎→BacktestResult |
| ⑤ 代码映射 | BT-08 core/tick_replay.py（stable, production） |
| ⑥ 降级/中止 | Tick回放不可用→用日线数据(精度降低) |
...
|---|---|
| ① 触发条件 | 策略提交+事件驱动模式选择 |
| ② 消费数据/因子 | Tick回放数据+策略信号 |
| ③ 参数 | 事件驱动模式、撮合精度 |
| ④ 数据流 | Tick事件→策略信号→订单→撮合→成交→BacktestResult |
| ⑤ 代码映射 | BT-09 implementations/event_driven_engine.py（stable, production） |
| ⑥ 降级/中止 | 事件驱动引擎不可用→仅向量化回测(精度低) |
...
| ② 消费数据/因子 | 原始指标计算结果 |
| ③ 参数 | NaN填充策略（前值/均值/插值） |
| ④ 数据流 | 指标计算→NaN检测→填充/清洗→清洁指标 |
| ⑤ 代码映射 | BT-26 services/nan_processor.py（planned） |
| ⑥ 降级/中止 | 处理器不可用→跳过NaN(指标可能异常) |
...
|---|---|
| ① 触发条件 | 密度预测模型训练完成后 |
| ② 消费数据/因子 | 模型概率预测+真实收益分布 |
| ③ 参数 | CRPS阈值、校准度指标 |
| ④ 数据流 | 模型预测分布→密度回测→CRPS/校准度→评估报告 |
| ⑤ 代码映射 | BT-07 metrics 扩展（planned） |
| ⑥ 降级/中止 | 密度回测未就绪→仅点预测回测(概率评估缺失) |

**指标文案（翻译真源 indicators_zh）**：

①触发：密度预测模型训练完成后；②消费：模型概率预测+真实收益分布；③参数：CRPS阈值、校准度指标、🆕密度预测模型V1~V5验证标准（V1方向准确率/V2校准度/V3 CRPS/V4策略PnL/端到端+V5共形分布无关覆盖率检查，实际覆盖率偏差<5%，⑦V1-V5验证层级全部可执行）；④数据流：模型预测分布→密度回测→CRPS/校准度→评估报告；⑤代码：BT-07 metrics 扩展（planned）；⑥降级：密度回测未就绪→仅点预测回测(概率评估缺失)。
...
| ① 触发条件 | 回测数据加载时自动应用 |
| ② 消费数据/因子 | 特征数据+时间戳 |
| ③ 参数 | PIT三公理、AS OF JOIN策略 |
| ④ 数据流 | 原始数据→pit_manager→PIT对齐数据→回测引擎 |
| ⑤ 代码映射 | BT-10 core/pit_manager.py（generated） |
| ⑥ 降级/中止 | PIT管理不可用→前视偏差风险(回测结果不可信) |
...
| 要素 | 内容 |
|---|---|
| ① 触发条件 | Walk-Forward/WFO 样本分割时 |
| ② 消费数据/因子 | 样本分割配置 |
| ③ 参数 | Embargo期长度 |
| ④ 数据流 | 样本分割→Embargo应用→隔离后样本→回测验证 |
| ⑤ 代码映射 | BT-10 core/pit_manager.py（generated） |
| ⑥ 降级/中止 | Embargo管理不可用→无隔离期(数据泄漏风险) |
...
| ① 触发条件 | 模型交叉验证阶段 |
| ② 消费数据/因子 | PIT特征+标签序列 |
| ③ 参数 | K折叠数、Purge窗口、Embargo期 |
| ④ 数据流 | 时序数据→Purged K-Fold分割→训练/验证折叠→评估 |
| ⑤ 代码映射 | BT-04 PIT管理扩展（planned） |
| ⑥ 降级/中止 | Purged K-Fold未就绪→普通K-Fold(泄漏风险) |
...
| ① 触发条件 | 回测完成后自动检测 |
| ② 消费数据/因子 | IS绩效+OOS绩效 |
| ③ 参数 | 过拟合阈值(IS vs OOS差异) |
| ④ 数据流 | IS/OOS绩效→对比检测→过拟合告警→决策门控 |
| ⑤ 代码映射 | BT-11 core/overfitting_detector.py（generated） |
| ⑥ 降级/中止 | 检测器不可用→人工判断(漏检风险) |
...
| ② 消费数据/因子 | 参数空间+回测结果 |
| ③ 参数 | 参数扫描范围、敏感度阈值 |
| ④ 数据流 | 参数扫描→多组回测→敏感度计算→告警→决策门控 |
| ⑤ 代码映射 | BT-11 core/overfitting_detector.py（generated） |
| ⑥ 降级/中止 | 检测器不可用→无参数敏感性分析(过拟合漏检) |
...
| ① 触发条件 | 批量回测后自动校正 |
| ② 消费数据/因子 | 多策略回测结果 |
| ③ 参数 | 校正方法(Bonferroni/Holm)、显著性水平 |
| ④ 数据流 | 多策略绩效→多重比较校正→校正后p值→决策门控 |
| ⑤ 代码映射 | BT-11 core/overfitting_detector.py（generated） |
| ⑥ 降级/中止 | 校正不可用→未校正p值(假阳性风险) |
...
| ② 消费数据/因子 | 实盘/回测绩效时序 |
| ③ 参数 | 衰减检测阈值、监控频率 |
| ④ 数据流 | 绩效时序→衰减检测→告警→策略退役流程 |
| ⑤ 代码映射 | BT-18 services/decay_monitor.py（planned） |
| ⑥ 降级/中止 | 监控不可用→人工定期检查(漏检风险) |
...
| ② 消费数据/因子 | 参数网格回测结果 |
| ③ 参数 | 显著性水平、过拟合阈值 |
| ④ 数据流 | 参数网格结果→显著性检验→过拟合评估→最优参数 |
| ⑤ 代码映射 | BT-21 services/param_analyzer.py（planned） |
| ⑥ 降级/中止 | 分析器不可用→人工选择参数(过拟合风险) |
...
| ② 消费数据/因子 | 策略收益序列 |
| ③ 参数 | 置换次数、显著性水平 |
| ④ 数据流 | 真实收益→置换重排→经验分布→p值→显著性判定 |
| ⑤ 代码映射 | BT-07 metrics 扩展（planned） |
| ⑥ 降级/中止 | 置换检验未就绪→仅样本内外对比(假阳性风险) |
...
| ② 消费数据/因子 | 策略夏普比率+试验次数 |
| ③ 参数 | 试验次数、相关性结构 |
| ④ 数据流 | 原始夏普→试验次数校正→Deflated Sharpe→显著性判定 |
| ⑤ 代码映射 | BT-07 metrics 扩展（planned） |
| ⑥ 降级/中止 | DSR未就绪→仅原始夏普(多重比较假阳性风险) |
...
|---|---|
| ① 触发条件 | 策略上线后持续监控 |
| ② 消费数据/因子 | 回测预期绩效+实盘实际绩效 |
| ③ 参数 | 偏差阈值、监控频率 |
| ④ 数据流 | 回测预期+实盘实际→偏差计算→阈值判定→告警 |
| ⑤ 代码映射 | BT-18 decay_monitor 扩展（planned） |
| ⑥ 降级/中止 | 监控未就绪→人工对账(漏检风险) |
...
|---|---|
| ① 触发条件 | 组合策略评估阶段 |
| ② 消费数据/因子 | 组合收益+单策略收益+相关性矩阵 |
| ③ 参数 | 组合过拟合阈值、相关性稳定性窗口 |
| ④ 数据流 | 组合+单策略收益→组合级检验→相关性稳定性→过拟合判定 |
| ⑤ 代码映射 | BT-07 metrics 扩展（planned） |
| ⑥ 降级/中止 | 组合级检测未就绪→仅单策略检测(组合过拟合漏检) |
...
| ② 消费数据/因子 | 试验日志+参数调整记录 |
| ③ 参数 | 试验追踪粒度、报告阈值 |
| ④ 数据流 | 试验/参数调整→日志记录→p-hacking检测→审计报告 |
| ⑤ 代码映射 | BT-21 param_analyzer 扩展（planned） |
| ⑥ 降级/中止 | 追踪未就绪→无审计(选择性报告风险) |
...
| ② 消费数据/因子 | 历史数据+窗口配置 |
| ③ 参数 | 训练窗长度、测试窗长度、滚动步长 |
| ④ 数据流 | 数据→窗口分割→训练→预测→滚动→汇总绩效 |
| ⑤ 代码映射 | BT-12 core/walk_forward.py（generated） |
| ⑥ 降级/中止 | WFO不可用→单段回测(无滚动验证) |
...
| ② 消费数据/因子 | 各窗口OOS绩效+参数 |
| ③ 参数 | 稳定性区域阈值、OOS通过标准 |
| ④ 数据流 | 各窗口OOS绩效→稳定性分析→稳定性区域→决策门控 |
| ⑤ 代码映射 | BT-12 core/walk_forward.py（generated） |
| ⑥ 降级/中止 | 稳定性分析不可用→无参数稳定性保障(策略可靠性存疑) |
...
| 要素 | 内容 |
|---|---|
| ① 触发条件 | Walk-Forward优化阶段 |
| ② 消费数据/因子 | 市场状态识别+策略参数 |
| ③ 参数 | 自适应窗口规则、市场状态分类 |
| ④ 数据流 | 市场状态→窗口自适应→Walk-Forward回测→参数稳定性 |
| ⑤ 代码映射 | BT-01~BT-09 扩展（planned） |
| ⑥ 降级/中止 | 自适应未就绪→固定窗口WFO(市场切换适应差) |
...
|---|---|
| ① 触发条件 | BM-BT-05/06 检测通过 |
| ② 消费数据/因子 | 过拟合检测结果+WFO结果+参数稳定性 |
| ③ 参数 | IS→WFA→OOS三阶段不可跳级、参数稳定性区域 |
| ④ 数据流 | 检测结果→三阶段门控→BacktestPassed事件→BM-SIM-01仿真 |
| ⑤ 代码映射 | BT-16 core/decision_gate.py（generated） |
| ⑥ 降级/中止 | 决策门控未就绪→人工审批(无自动门禁,风险高) |
...
|---|---|
| ① 触发条件 | 回测完成+决策门控通过 |
| ② 消费数据/因子 | BacktestResult |
| ③ 参数 | 可视化数据格式 |
| ④ 数据流 | BacktestResult→result_sink→BacktestSinkData→前端/报告 |
| ⑤ 代码映射 | BT-13 io/backtest_result_sink.py（generated） |
| ⑥ 降级/中止 | Sink不可用→原始结果(无可视化) |
...
|---|---|
| ① 触发条件 | 回测完成+决策门控通过 |
| ② 消费数据/因子 | BacktestResult+BacktestSinkData |
| ③ 参数 | BacktestRunArtifact(CTR-P1-017)、持久化策略 |
| ④ 数据流 | BacktestResult→result_repository→BacktestRunArtifact→DB |
| ⑤ 代码映射 | BT-14 io/result_repository.py（generated） |
| ⑥ 降级/中止 | 持久化不可用→结果仅内存(重启丢失) |
...
| ① 触发条件 | 回测完成+决策门控通过 |
| ② 消费数据/因子 | BacktestResult |
| ③ 参数 | decisiongraph L5节点格式 |
| ④ 数据流 | BacktestResult→decisiongraph_adapter→L5决策节点→decisiongraph |
| ⑤ 代码映射 | BT-15 io/decisiongraph_adapter.py（stable, production） |
| ⑥ 降级/中止 | 适配器不可用→回测结论不进入决策流(人工衔接) |
...
| ② 消费数据/因子 | BacktestResult+BacktestSinkData |
| ③ 参数 | 报告格式(PDF/HTML)、模板 |
| ④ 数据流 | BacktestResult→报告生成→PDF/HTML报告 |
| ⑤ 代码映射 | BT-19 services/report_generator.py（planned, P2） |
| ⑥ 降级/中止 | 生成器不可用→手动导出数据(无格式化报告) |
...
| ② 消费数据/因子 | 错误日志+回测上下文 |
| ③ 参数 | 诊断规则、修复建议库 |
| ④ 数据流 | 错误日志→异常诊断→修复建议→用户 |
| ⑤ 代码映射 | BT-23 services/anomaly_diagnoser.py（planned, P2） |
| ⑥ 降级/中止 | 诊断器不可用→人工排查(效率低) |
...
|---|---|
| ① 触发条件 | 多次回测完成后 |
| ② 消费数据/因子 | 多组BacktestResult |
| ③ 参数 | 对比维度、差异阈值 |
| ④ 数据流 | 多组结果→差异分析→对比报告 |
| ⑤ 代码映射 | BT-24 services/result_comparator.py（planned, P2） |
| ⑥ 降级/中止 | 对比器不可用→人工对比(耗时) |
...
|---|---|
| ① 触发条件 | 决策门控通过+用户确认 |
| ② 消费数据/因子 | BacktestRunArtifact+策略代码 |
| ③ 参数 | 部署目标、配置 |
| ④ 数据流 | 验证通过策略→部署器→实盘环境 |
| ⑤ 代码映射 | BT-25 services/result_deployer.py（planned） |
| ⑥ 降级/中止 | 部署器不可用→手动部署(操作风险) |
...
| ② 消费数据/因子 | 各层验证指标+策略绩效 |
| ③ 参数 | V1-V6各层阈值 |
| ④ 数据流 | 策略→V1基础检验→V2样本外→V3稳健性→V4压力测试→V5模拟盘→V6小仓位实盘→上线 |
| ⑤ 代码映射 | BT-07 门控扩展（planned） |
| ⑥ 降级/中止 | 分层门控未就绪→三阶段门控(粒度粗) |
...
| 要素 | 内容 |
|---|---|
| ① 触发条件 | BM-BT-07 回测验证通过后、策略全量上线前 |
| ② 消费数据/因子 | 回测通过策略 + 实盘环境配置 + BM-BT-07-I 分层验证门控(V3+V4)结果 |
| ③ 参数 | 试运行流水线、影子运行(shadow)、小仓位实盘验证、输出契约、灰度切换门禁 |
| ④ 数据流 | 回测通过→影子运行→小仓位实盘验证→门禁评估→全量上线 / 回退迭代 |
