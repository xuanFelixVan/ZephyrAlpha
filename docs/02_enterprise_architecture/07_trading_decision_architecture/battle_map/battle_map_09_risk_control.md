...
> battle_map §risk_control 阶段，50 环节（66 锚点）。
...
| 要素 | 内容 |
|---|---|
| ① 触发条件 | Agent自治行为发生时/盘后审计 |
| ② 消费数据/因子 | Agent决策日志+自治边界配置+治理基线 |
| ③ 参数 | 有界自治(Bounded Autonomy)、保障缺口管理、治理漂移防护、Agent行为监控、ARS双轨结算模型 |
...
①触发：Agent自治行为发生时/盘后审计；②消费：Agent决策日志+自治边界配置+治理基线；③参数：有界自治(Bounded Autonomy)、保障缺口管理、治理漂移防护、Agent行为监控、ARS双轨结算模型；④数据流：Agent行为→监控→漂移检测→保障缺口评估→否决/Kill Switch；⑤代码映射：待开发（planned，D_RISK域）；⑥降级：漂移超限→触发BM-RC-03 Kill Switch熔断。；📌 概念覆盖清单（草稿H3逐字登记）：§29.11 大语言模型Agent用于基本面分析；§11.2.2 数据血缘（Lineage）；§1.5 AI/Agent特有风险
...
| 要素 | 内容 |
|---|---|
| ① 触发条件 | 风控官配置策略/定时review |
| ② 消费数据/因子 | D-EX-CORE 持仓快照+D-FACTOR 因子暴露 |
| ③ 参数 | 策略状态机、9种限额类型、消耗追踪、预警分级、审批流 |
| ④ 数据流 | 策略配置→限额管理→CTR-003 RiskLimits→BM-RC-02盘前检查+BM-RC-04盘中监控 |
| ⑤ 代码映射 | RK-01 risk_manager（stable, production）+RK-06 default_position_limit_checker（stable, production） |
| ⑥ 降级/中止 | 限额管理器未就绪→硬编码保守限额(无动态调整) |

**指标文案（翻译真源 indicators_zh）**：

①触发：风控官配置策略/定时review；②消费：D-EX-CORE 持仓快照+D-FACTOR 因子暴露；③参数：策略状态机、9种限额类型、消耗追踪、预警分级、审批流、风控验证规则=13条主规则(VR-001~VR-013)+2条子规则(VR-002a/002b)+熔断器模式+4层隔离(代码/数据/权限/审计)、HC-RISK-01~08硬约束（风险架构）、🎯监管框架参考（规则设计依据，非本系统组件）：BCBS/Basel 巴塞尔协议、IFRS 9 ECL 预期信用损失、Merton 模型、Millennium 风控案例、ESRB、Treasury/FSB、IMDA/CSA/CISA、RWC/SA-BCP；④数据流：策略配置→限额管理→CTR-003 RiskLimits→BM-RC-02盘前检查+BM-RC-04盘中监控；⑤代码：RK-01 risk_manager（stable, production）+RK-06 default_position_limit_checker（stable, production）；⑥降级：限额管理器未就绪→硬编码保守限额(无动态调整)。
📌 概念覆盖清单（草稿H3逐字登记）：§8.2 批流分离设计；§5.2 风控参数版本管理；§6.3 涨跌停约束
...
①触发：极端市场事件发生时 / 盘后黑天鹅压力测试；②消费：历史极端事件数据 + BM-RC-08-C 压力测试结果 + 跨市场行情数据；③参数：黑天鹅模式库、跨市场传导模型、流动性危机模拟、二阶效应与传染模型、反向压力测试；④数据流：极端事件→模式库匹配→跨市场传导建模→流动性危机模拟→传染效应评估→BM-RC-03 Kill Switch；⑤代码映射：待开发（planned，D_RISK 域）；⑥降级：极端事件超出模型范围→触发 BM-RC-03 Kill Switch 熔断（安全优先）。；📌 概念覆盖清单（草稿H3逐字登记）：§6.3 C-039 跨市场传导量化模型；模块37 系统性风险分级预警与尾部风险管理模型（Systemic Risk Tiered Alert；§14.2 跨市场传导模型；§14.5 二阶效应与传染模型

**锚点（环节↔模块双向关联）**：

| 目标图 | 目标ID | 角色 | 状态快照 | 真实build_status |
|---|---|---|---|---|
| candidate | CAND-HARVEST-0667 | primary | — | — |
...
| 要素 | 内容 |
|---|---|
| ① 触发条件 | BM-BUY/BM-SELL 下单请求 |
| ② 消费数据/因子 | BM-RC-01 RiskLimits+持仓+Kill Switch状态 |
| ③ 参数 | 5步检查链、幂等、Fail-Closed、50ms SLA |
| ④ 数据流 | 下单请求→5步检查→通过/拒绝(E-RK-04 PreTradeRejected)→BM-EXE执行/拦截 |
| ⑤ 代码映射 | RK-02 risk_validator（stable, production） |
| ⑥ 降级/中止 | 检查超时→Fail-Closed拒单(保守,不放过) |
...
📌 概念覆盖清单（草稿H3逐字登记）：§8.3 新鲜度检查点与延迟预算；§6.2 持仓限额
...
| 要素 | 内容 |
|---|---|
| ① 触发条件 | 回撤>EMERGENCY/VaR超限+无法减仓/Owner手动 |
| ② 消费数据/因子 | BM-RC-04 盘中监控信号+Owner指令 |
| ③ 参数 | 状态机(OPEN/CLOSED)、3种触发条件、冷却期30min、Owner确认重置 |
| ④ 数据流 | 触发条件→Kill Switch CLOSED→停止新开仓+多域通知→冷却30min→Owner确认→重置 |
| ⑤ 代码映射 | RK-17 kill_switch（stable, production） |
| ⑥ 降级/中止 | Kill Switch未就绪→人工紧急停盘(响应慢) |
...
|---|---|
| ① 触发条件 | 盘中实时(每Tick/定时) |
| ② 消费数据/因子 | D-EX-CORE 持仓快照+D-MKT-DATA 行情 |
| ③ 参数 | VaR计算、回撤三级阈值(-5%/-10%/-15%)、因子暴露、相关性矩阵、CTR-P1-008 |
| ④ 数据流 | 持仓+行情→VaR+回撤+因子暴露→告警(E-RK-01/E-RK-03)→BM-RC-03 Kill Switch判定 |
| ⑤ 代码映射 | RK-03 risk_limits（stable, production）+RK-11（planned） |
...
📌 概念覆盖清单（草稿H3逐字登记）：§29.33 KAN Kolmogorov-Arnold网络（v6.0新增）；§1.3 流动性风险；§2.1 VaR/CVaR/ES；§2.3 密度感知VaR/共形VaR；§15.4 Agent行为监控
...
| 目标图 | 目标ID | 角色 | 状态快照 | 真实build_status |
|---|---|---|---|---|
| candidate | CAND-HARVEST-0973 | primary | — | — |
...
OWASP Agentic Security Top10扫描（ASI01-10：目标劫持/工具滥用/身份权限滥用/供应链/意外代码执行/记忆投毒等10类）+AST01-10对抗测试+MCP01-10协议安全映射+红队FCFT演练。承载模块: (depgraph无实现-设计态)。出处: 依赖图/01-跨域交叉点与因果链.md L253
...
| 目标图 | 目标ID | 角色 | 状态快照 | 真实build_status |
|---|---|---|---|---|
| depgraph | MOD-RK-22 | primary | planned | planned |
...
| 要素 | 内容 |
|---|---|
| ① 触发条件 | 持仓跌破止损条件/亏损限额触达 |
| ② 消费数据/因子 | BM-RC-04 盘中监控+个股行情 |
| ③ 参数 | 6种A股止损模式、亏损限额(日2%/周5%/月10%)、强制停盘/复盘 |
| ④ 数据流 | 止损条件→止损引擎→止损单→BM-SELL卖出流程+Kill Switch联动 |
| ⑤ 代码映射 | RK-04 stop_loss（stable, production）+RK-09（planned） |
...
|---|---|
| ① 触发条件 | 盘中5信号扫描/尾部异常 |
| ② 消费数据/因子 | 融资余额+流动性+政策新闻+外围指数+尾部数据 |
| ③ 参数 | 5大信号、三级警报(1停/2降30%/≥3清仓)、EVT/POT模型、Copula尾部依赖、FRTB加价 |
| ④ 数据流 | 5信号+尾部数据→系统性风险检测→三级警报→BM-RC-03 Kill Switch+清仓执行 |
| ⑤ 代码映射 | RK-10/RK-15（planned） |
...
📌 概念覆盖清单（草稿H3逐字登记）：§20.15 熔断检测职责分配；P0 — 核心能力（没有它系统不可用，共14项）；P1 — 重要能力（增强系统价值）；§29.21 学习系统桥接声明；系统性风险分级预警与尾部风险管理模型（Systemic Risk Tiered Alert & Ta；§7.1 事前PSI检测
...
拥挤度计算(因子拥挤+策略逻辑相似度)+去杠杆+悖论防护。因子/策略拥挤度深度检测（🆕v8.2 C-045深度增强，§29.39裁定16）：策略逻辑相似度（超越因子暴露相似度的表层比较，深入策略代码逻辑判断实质等价，如两策略因子不同但交易逻辑同构→仍判定为拥挤）+五种去杠杆原型匹配+拥挤度悖论防护+行业研报因子推荐频率监控。承载模块: (depgraph无实现-设计态)。出处: 依赖图/01-跨域交叉点与因果链.md L248
...
①触发：盘中定时/盘后（阈值: 因子/策略拥挤度+深度增强(策略逻辑相似度+去杠杆+悖论防护)）；②消费：因子暴露+策略持仓（来自 D-FACTOR / D-PF-CORE）；③参数：—；④数据流：因子+策略持仓→拥挤度计算(因子拥挤+策略逻辑相似度)+去杠杆+悖论防护→拥挤度预警→BM-RC-06-C 三级警报；⑤代码：(depgraph无实现-设计态) / 依赖图/01-跨域交叉点与因果链.md L248；⑥降级：拥挤度检测未就绪→跳过拥挤度检查。；📌 概念覆盖清单（草稿H3逐字登记）：模块40 板块拥挤度与启动条件量化模型（Sector Crowding & Launch Condi

**锚点（环节↔模块双向关联）**：

| 目标图 | 目标ID | 角色 | 状态快照 | 真实build_status |
|---|---|---|---|---|
| candidate | CAND-HARVEST-2033 | primary | — | — |
...
|---|---|
| ① 触发条件 | 盘前预算分配/盘中VaR重算 |
| ② 消费数据/因子 | BM-RC-04 持仓+BM-RC-01 限额 |
| ③ 参数 | VaR三阶段(参数法/蒙特卡洛/Basel III三角)、GPU CuPy/RTX3090、风险预算优化求解器、风险贡献计算 |
| ④ 数据流 | 持仓+限额→VaR计算+风险预算分配→再平衡触发→BM-POS仓位调整 |
| ⑤ 代码映射 | RK-05/RK-08（planned） |
...
|---|---|
| ① 触发条件 | 盘后定时/合规要求 |
| ② 消费数据/因子 | D-EX-CORE 成交记录+BM-RC-04 盘中监控+持仓快照 |
| ③ 参数 | Brinson归因、日终PnL对账、合规报告、历史情景(2008/2015/2020)、反向压力测试、敏感性分析、传染效应 |
| ④ 数据流 | 成交+持仓→PnL对账+归因+合规报告(CTR-P1-011)→BM-RES-07策略迭代+BM-REC对账 |
| ⑤ 代码映射 | RK-16/RK-20（planned）+RK-12（planned） |
| ⑥ 降级/中止 | 盘后审计器未就绪→人工Excel对账(效率低,易错) |
...
📌 概念覆盖清单（草稿H3逐字登记）：§12.5 通知与告警(C-015) + 审计与合规(C-043) + 成本治理(C-044)（→A；§1.2 模型风险；§1.4 操作风险
...
| 目标图 | 目标ID | 角色 | 状态快照 | 真实build_status |
|---|---|---|---|---|
| candidate | CAND-HARVEST-0141 | primary | — | — |
...
| 目标图 | 目标ID | 角色 | 状态快照 | 真实build_status |
|---|---|---|---|---|
| candidate | CAND-HARVEST-0978 | primary | — | — |
...
| 目标图 | 目标ID | 角色 | 状态快照 | 真实build_status |
|---|---|---|---|---|
| candidate | CAND-RSK-014 | primary | — | — |
...
| 目标图 | 目标ID | 角色 | 状态快照 | 真实build_status |
|---|---|---|---|---|
| candidate | CAND-HARVEST-1440 | primary | — | — |
...
| 目标图 | 目标ID | 角色 | 状态快照 | 真实build_status |
|---|---|---|---|---|
| candidate | CAND-HARVEST-3408 | primary | — | — |
