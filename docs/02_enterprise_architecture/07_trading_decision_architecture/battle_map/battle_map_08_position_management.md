...
①触发：编排后决策（买/卖）就绪；②消费：BM-BUY-03 编排决策 + BM-SELL-02 卖出决策 + BM-BUY-04 分批方案；③参数：position_cap=目标仓位；④数据流：买/卖决策+分批方案→C-047唯一裁决→最终仓位指令→BM-EXE-01，策略未覆盖/冲突时仓位轨走v7.0因子直通（Kelly+风险预算直接产生仓位决策，L827）；⑤代码：C-047 §1.8（v4.0 P0）；⑥降级：C-047 不可用→固定比例仓位查表。
...
| depgraph | MOD-POS_SERVICES | primary | stable | generated |
...
| depgraph | MOD-POS-019 | supplement | planned | planned |
...
| depgraph | MOD-POS-005 | supplement | planned | planned |
...
| ③ 参数 | 行业偏离±10%/叠加态±15%/绝对30%、corr<0.7、半Kelly硬上限 |
...
| ③ 参数 | 策略集成员、权重分配(A30%/B25%/C20%)、准入门禁 |
