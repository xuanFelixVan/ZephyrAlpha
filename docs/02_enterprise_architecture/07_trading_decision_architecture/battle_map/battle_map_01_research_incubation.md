...
> battle_map §research_incubation 阶段，33 环节（49 锚点）。
...
### BM-RES-08 知识清洗与结构化 / Knowledge Cleaning & Structuring

> **大白话**：把采集来的脏知识洗干净、结构化，去掉噪音和重复，变成可用的结构化知识。

**机制说明**：

S1知识清洗与结构化层。把多模态采集的脏知识(含噪音/重复/格式不一)经清洗流水线处理为结构化知识。是研究孵化的第一步。
...
①触发：BM-RES-06 LLM研究Agent采集多模态材料后；②消费：原始研究材料（论文/研报/新闻/财报文本）；③参数：NLP清洗流水线、实体抽取、结构化模板、去重去噪、知识契约=RawKnowledgePacket→StructuredKnowledgeFragment（实体+关系+时序边→支持多跳推理）；④数据流：原始材料→清洗→结构化→BM-RES-03假设管理沉淀；⑤代码映射：待开发（planned，D_RESEARCH/D_INTELLIGENCE域）；⑥降级：清洗失败→保留原始材料人工处理。；📌 概念覆盖清单（草稿H3逐字登记）：§4.1 清洗流水线；§4.2 输出契约

**锚点（环节↔模块双向关联）**：

| 目标图 | 目标ID | 角色 | 状态快照 | 真实build_status |
|---|---|---|---|---|
| candidate | CAND-HARVEST-1916 | primary | — | — |

**有效状态**：🟨 候选态（候选池） ｜ **环节自报**：design ｜ **层**：L1 ｜ **阶段**：research_incubation

### BM-RES-09 知识分类与策略提取 / Knowledge Classification & Strategy Extraction

> **大白话**：把结构化知识按类型分类，从中提取可执行的策略知识。

**机制说明**：

S2知识分类与策略提取层。把结构化知识按类型分类(事实/规则/策略/案例/元知识)，从中提取可执行策略知识喂给后续模块。
...
①触发：BM-RES-08 知识清洗完成后；②消费：结构化知识+历史策略库+BM-RES-03假设；③参数：知识类型分类体系(6类)、策略提取流程、输出契约=ClassifiedKnowledgePackage、因子矿工（Factor Mining Agent，v7.0 S4）；④数据流：结构化知识→分类→策略提取→BM-RES-07策略迭代升级；⑤代码映射：待开发（planned，D_RESEARCH/D_ML_TRAIN域）；⑥降级：提取失败→人工标注策略要素。；📌 概念覆盖清单（草稿H3逐字登记）：§5.2 策略提取流程；§5.3 输出契约；3.1 因子分类体系；6.1 图谱类型体系；§3.3 否决与策略逻辑的隔离

**锚点（环节↔模块双向关联）**：

| 目标图 | 目标ID | 角色 | 状态快照 | 真实build_status |
|---|---|---|---|---|
| candidate | CAND-HARVEST-1918 | primary | — | — |
...
①触发：BM-RES-09 策略提取后/新模块需求触发；②消费：策略规格+现有模块工厂库+BM-MT-01训练基座契约；③参数：模块工厂架构、映射匹配规则、与现有工厂关系、输出契约=NewModule/ModuleRegistry/ModuleMappingResult；④数据流：策略规格→工厂匹配→模块规格→BM-MT-01-B AI辅助代码生成；⑤代码映射：待开发（planned，D_RESEARCH/D_ML_TRAIN域）；⑥降级：匹配失败→全新建模块(走BM-MT-01-B AI代码生成)。；📌 概念覆盖清单（草稿H3逐字登记）：§5.1 三层递进架构；§12.2 C-008 AI自治运维（→A9运维架构）；分层架构总览；§29.G v5.1+架构增强；§29.20 架构增强二元结论与实现路径；§30.1 核心价值链域缺失模块；§30.2 增强与扩展域缺失模块；P0 模块明细；❌不能建模块门禁条件分布；§30.5 自治与基础设施域缺失模块；§30.6 运维安全治理域缺失模块；§0.1 学习系统在全局架构中的位置；§6.3 输出契约；§7.1 模块创建流程；§11.1 双存储架构；§17.8 D-SECURITY 安全域缺失模块；§17.11 跨域基础设施缺失模块；§17.18 D-INFRA-OPS 运维基础设施域缺失模块；§17.20 D-RESEARCH 研究基础设施域缺失模块；§17.24 D-OPS 运维域缺失模块；§17.26 补充模块建设结论汇总；§17.1 D-RISK域模块映射

**锚点（环节↔模块双向关联）**：

| 目标图 | 目标ID | 角色 | 状态快照 | 真实build_status |
|---|---|---|---|---|
| candidate | CAND-HARVEST-1921 | primary | — | — |
...
📌 概念覆盖清单（草稿H3逐字登记）：能力血缘关系图；§29.2 特征存储 (Feature Store)；§11.3.2 PIT正确性详解
...
> **大白话**：从多模态来源（文本/图表/音频/视频）采集知识，统一入库。

**机制说明**：

S0多模态知识采集层。从文本(新闻/公告/研报)、图表(K线/技术图)、音频(电话会)、视频(路演)等多模态来源采集知识，统一入库供后续清洗。采集通道：语音(Whisper转写)+视频(Whisper+OCR)+PDF解析+网址爬虫+文字直入；🆕VLM图表视觉理解（K线/技术图表视觉语义）+🆕Point-in-Time门控+🆕时序基础模型骨干(TimesFM)。
...
①触发：定时采集/研究员触发/事件驱动；②消费：外部数据源(论文库/新闻/研报/财报/另类数据)；③参数：采集源分类、采集调度、采集增强能力(v4.0)、输出契约；🎯对标：Captide 持续信息提取/MountainLion 图表视觉理解（见battle_map_12 对标清单）；④数据流：外部源→采集→分类→调度→BM-RES-06 LLM研究Agent/论文追踪；⑤代码映射：待开发（planned，D_RESEARCH/D_INTELLIGENCE域）；⑥降级：采集源故障→降级缓存数据+告警。；📌 概念覆盖清单（草稿H3逐字登记）：§3.4 因子分类与数据源映射；§29.38 多模态金融推理（v6.0新增）；§3.1 采集源分类；§3.2 采集调度；§3.4 输出契约
...
离线特征(批量→历史) + 在线特征(实时→最新) + PIT AS OF JOIN + 特征注册表 + 特征血缘（OpenLineage 血缘标准）。🆕v8.1 UFL确定性事实层（Feature Store子集，is_deterministic=True，§29.24）——确定性事实（财报数值/公告字段）与统计因子分层存储，供 L6 VeNRA 零幻觉锚定消费。声明式因子定义+一致性引擎（数据架构）：因子以声明式定义注册，滑动窗口类因子避免全量重算，日终离线/在线偏差检测保证两套特征一致。承载模块: D-RESEARCH-02。出处: 20-D-RESEARCH §1 + §12.0 R-68/R-69
...
📌 概念覆盖清单（草稿H3逐字登记）：§29.32 LLM进化式策略搜索（v6.0新增）
...
### BM-RES-08-A 知识清洗流水线 / Knowledge Cleaning Pipeline

> **大白话**：清洗流水线——去重/去噪/格式标准化/实体链接/质量评分，分阶段处理脏数据。

**机制说明**：

清洗流水线5阶段：①去重(语义+精确)②去噪(规则+ML过滤)③格式标准化(统一schema)④实体链接(知识图谱对齐)⑤质量评分(可信度/时效性/来源权威性)。
...
①触发：BM-RES-11多模态采集产出原始知识后，需清洗流水线去噪/去重/结构化；②消费：原始多模态知识(文本/表格/图像/PDF) + 采集元数据(来源/时间/置信度)；③参数：清洗流水线：格式归一化 + 去重(哈希+语义相似度) + 去噪(低质量过滤) + 实体抽取 + 关系抽取 + 结构化输出(知识三元组)；④数据流：原始知识→清洗→结构化三元组→输出契约(知识图谱节点/边)→下游BM-RES-09知识分类；⑤代码映射：待开发（planned，D_RESEARCH/D_INTELLIGENCE域）；⑥降级：清洗子步骤失效→保留原始知识+标记未清洗，由下游人工兜底。

**锚点（环节↔模块双向关联）**：

| 目标图 | 目标ID | 角色 | 状态快照 | 真实build_status |
|---|---|---|---|---|
| candidate | CAND-HARVEST-0751 | primary | — | — |

**有效状态**：🟨 候选态（候选池） ｜ **环节自报**：design ｜ **层**：S1 ｜ **阶段**：research_incubation

### BM-RES-09-A 知识类型分类体系 / Knowledge Type Classification System

> **大白话**：知识类型分类体系——定义知识类型（事实/规则/策略/案例/元知识），按类型组织管理。

**机制说明**：

知识类型分类体系：①事实知识(市场数据/财务数据)②规则知识(交易规则/风控规则)③策略知识(选股/择时/仓位策略)④案例知识(历史交易案例)⑤元知识(关于知识本身的知识)。
...
①触发：清洗后的结构化知识需按类型分类以驱动后续策略提取；②消费：结构化知识三元组(来自RES-08-A)；③参数：知识分类体系：事实型/规则型/模式型/案例型/方法论型5类 + 分类置信度 + 跨类型关联；④数据流：知识三元组→分类→类型化知识库→下游BM-RES-09策略提取；⑤代码映射：待开发（planned，D_RESEARCH/D_ML_TRAIN域）；⑥降级：分类失效→默认归入事实型(最安全)，人工复核。

**锚点（环节↔模块双向关联）**：

| 目标图 | 目标ID | 角色 | 状态快照 | 真实build_status |
|---|---|---|---|---|
| candidate | CAND-HARVEST-2624 | primary | — | — |

**有效状态**：🟨 候选态（候选池） ｜ **环节自报**：design ｜ **层**：S2 ｜ **阶段**：research_incubation

### BM-RES-10-A 模块工厂架构 / Module Factory Architecture

> **大白话**：模块工厂架构——标准创建→注册→接入→验证4步流程，产出规范+版本管理+退役流程。

**机制说明**：

模块工厂架构。标准创建流程(4步：创建→注册→接入→验证)+输出契约(产出规范+版本管理+退役流程)。与研究发现对接，自动匹配或创建模块。

**6 件套（结构化，DB indicators JSONB）**：

| 要素 | 内容 |
|---|---|
| ① 触发条件 | 研究产出新模块/新能力时，模块工厂匹配现有模块或创建新模块注册 |
| ② 消费数据/因子 | 研究发现 + 模块注册表 + 模块映射规则 + 能力卡片 |
| ③ 参数 | 模块创建流程(标准创建→注册→接入→验证4步) + 输出契约(模块产出规范+版本管理+退役流程) + §7 S4模块创建与接入层: 模块创建流程(标准创建→注册→接入→验证4步) + 输出契约(模块产出规范+版本管理+退役流程) |
| ④ 数据流 | 研究发现→模块工厂匹配→注册新模块/复用现有→能力卡片更新→部署 |
| ⑤ 代码映射 | 待开发（planned，D_RESEARCH域） |
| ⑥ 降级/中止 | 模块工厂不可用→人工创建模块+手动注册 |

**指标文案（翻译真源 indicators_zh）**：

①触发：研究产出新模块/新能力时，模块工厂匹配现有模块或创建新模块注册；②消费：研究发现 + 模块注册表 + 模块映射规则 + 能力卡片；③参数：模块创建流程(标准创建→注册→接入→验证4步) + 输出契约(模块产出规范+版本管理+退役流程) + §7 S4模块创建与接入层: 模块创建流程(标准创建→注册→接入→验证4步) + 输出契约(模块产出规范+版本管理+退役流程)；④数据流：研究发现→模块工厂匹配→注册新模块/复用现有→能力卡片更新→部署；⑤代码映射：待开发（planned，D_RESEARCH域）；⑥降级：模块工厂不可用→人工创建模块+手动注册。；📌 概念覆盖清单（草稿H3逐字登记）：§7.3 输出契约

**锚点（环节↔模块双向关联）**：

| 目标图 | 目标ID | 角色 | 状态快照 | 真实build_status |
|---|---|---|---|---|
| candidate | CAND-HARVEST-2623 | primary | — | — |

**有效状态**：🟨 候选态（候选池） ｜ **环节自报**：design ｜ **层**：S3 ｜ **阶段**：research_incubation

### BM-RES-11-A 采集源分类与调度 / Source Classification & Scheduling

> **大白话**：采集源分类（官方/新闻/另类/社交）+调度策略（限流/优先级/去重）。

**机制说明**：

采集源分类(官方数据源/新闻媒体/另类数据/社交媒体)+采集调度(限流策略/QPS分配/优先级队列/去重策略)。确保采集不漏不重不超限。
...
①触发：研究知识采集需按源类型分类并按调度策略拉取，避免QPS超限/数据缺失；②消费：多源接入配置(研报/新闻/公告/财报/社交媒体/另类数据) + QPS配额；③参数：采集源分类6类(研报/新闻/公告/财报/社交/另类) + 调度策略(优先级+QPS分配+重试+增量) + v4.0采集增强(智能去重/相关性预筛) + 输出契约(原始知识+元数据)；④数据流：源配置→调度拉取→原始多模态知识+元数据→下游BM-RES-08-A清洗；⑤代码映射：待开发（planned，D_RESEARCH/D_INTELLIGENCE域，C-022/C-044 iFind QPS协同）；⑥降级：主源失效→自动切换备用源(akshare/tushare)；调度超限→降级QPS+延后非优先源。

**锚点（环节↔模块双向关联）**：

| 目标图 | 目标ID | 角色 | 状态快照 | 真实build_status |
|---|---|---|---|---|
| depgraph | MOD-DAT-fred_ingest | supplement | generated | generated |

**有效状态**：🟦 运营态（已建） ｜ **环节自报**：design ｜ **层**：S0 ｜ **阶段**：research_incubation
