...
- 组合结构：持仓角色分层（底仓/进攻/机动/备用+层间资金禁互相救援"不补底仓亏损"）；板块梯队标签（龙头/跟风角色，跟风止损严于龙头，板块≥2 天地板全梯队降仓（与 §35A 素材"≥3 只天地板全线清仓"的阈值与动作双偏差：刻意收紧还是笔误待 Owner 确认，外审 G-10/m-39））；组合加仓预算池（多票同时加仓总闸，池空转报备待批）；持仓只数上限=情绪周期函数（试错期多标的分散/主升期集中 1-2 只/退潮 0 只）；账龄字段+超期升级
...
> 待裁定（外审修复 2026-09-07，M-21）：竞价段（9:20-9:25 挂单）是否豁免 D72 恐慌拦截 30 分钟窗口；D72 豁免清单（KillSwitch/黑天鹅/跌停封死）与 S1-06 四触发（含 K≥3 破位/主力弃庄）的对齐。
...
> 待裁定（外审修复 2026-09-07，m-37/G-04/G-05）：档内自动动作范围=减仓/冻结/降权类风险动作；清退归零属评审制人工裁定输出（D79×D84 边界需 Owner 明示）。
