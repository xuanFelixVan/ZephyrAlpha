...
> ## 结案报告（2026-08-16 补记）
>
> **实际开发**：2026-08-13 第一批施工（会话 AI-POS-001），按本档落码"策略层粗仓位 + 公司层 Kelly 精裁决"分层体系——模块 MOD-POS-001/MOD-POS-020/MOD-POS-021，外加本档 §2.8 持仓漂移再平衡，3 笔单域拆分提交合并（merge，即代码合入 dev 主线）回 dev。
>
> **最终成果**：25 个新测试两轮全绿；MOD-POS-020/021 经依赖图确认为稳定+生产态（production）；策略层等权/风险平价与公司层 Kelly 裁决的接口契约随代码落地。
>
> **未做事项及原因**：
> - T+1 可卖持仓口径（current_holdings 应按 T+1 可卖权重供数）未对齐——供数方口径未就绪，待数据侧对齐后关闭（跟踪表遗留 #30）。
> - 2 个既有时间炸弹测试失败未修——存量测试债，非本档施工引入，归测试债清偿批处理（遗留 #18）。
...
- **⚠️ 胖尾场景下半 Kelly 的理论不稳定性（Taleb 论点，2026-08-10 审查补充，[convexly 2026-03](https://www.convexly.app/blog/kelly-criterion-explained)）**：Kelly 公式（二值/精确/Merton 均）隐含依赖**有限方差**假设。Taleb *Statistical Consequences of Fat Tails*（2020 ch.10）论证：尾部幂律且 **Hill 尾指数 α<2** 时样本方差发散（不收敛），任何依赖方差的 sizing 规则（含半 Kelly）都在估计不存在的量。convexly 对 Polymarket 8656 钱包实测 α=1.28（95%CI 1.20-1.36），遂改用 **quarter-Kelly（0.25×f*）+ barbell**（投机腿总暴露封顶 10-20%，腿内每仓位 quarter-Kelly）。**对本项目校准启示**：① A 股个股尾指数经验值 α≈2-4（中等厚尾），单票 α<2 风险主要在次新股/事件冲击期（已被 §2.2.2 σ_i 异常判定第 3/4 项降级覆盖）；② **不把半 Kelly 降为 quarter-Kelly**——本设计不依赖半 Kelly 单层防护，而叠加 §2.3.3 分布感知（VaR/CVaR 下调）+ §2.3.4 多约束取最小 + §2.4 硬上限 + regime Shrinkage 四层独立兜底，尾部场景有效仓位已远低于半 Kelly 名义值（与 Taleb barbell 异曲同工：多层硬约束等效"投机腿封顶+腿内 quarter-Kelly"）；③ **校准参数**：G04 首批策略 50+ trades 后用 Hill 估计器测各策略 PnL 的 α，α<2 则 Kelly 倍数 0.5×→0.25×（或维持 0.5× 但收紧 §2.3.3 VaR/CVaR 阈值），与 §6 EVT-Based Tail Budgeting 同源（GPD 拟合同产 α 估计）。此论点为四层防御栈提供**理论背书**——保留多层硬约束而非简化为纯半 Kelly，正是 Taleb 胖尾批判要求的风险架构
...
- **⚠️ Conformal Kelly lockbox 样本外负结果**（[arXiv:2608.01494v1](https://arxiv.org/abs/2608.01494)）：开发窗口 28.5% 年化/Sharpe 1.34/MaxDD 27.7%，但 **lockbox OOS（2022+）增长未持仓**——8.5%/7.0% 年化低于被动基准。**关键警示**：calibration 持仓（0.745 vs 0.750）≠ growth 持仓。正面结果：风险控制策略（下行 miss→削杠杆）MaxDD 27.7%→20.3% Sharpe 提升 p=1/41。**启示**：① lockbox 负结果警示开发窗口过拟合风险，引入须独立 lockbox 验证；② "模型失效检测+自动降杠杆"借鉴为降级触发第 5 项（与四检查链互补，检测渐变失效）。**第 5 项施工参数**：① **N = 20 交易日**（对齐浙商"10 日优于 30 日"短期窗口，月级为稳健上界）；② **historical rate = 滚动 60 日下行 miss 率 P95**（动态阈值自适应不同标的/regime baseline）；③ **下行 miss 定义**：actual < lower_bound（75% conformal 区间下界被突破）；④ **降级动作**：Kelly 分数 ×0.5，持续至连续 10 日 miss 率回落至 historical rate 以下。G04 首批 50+ trades 后用实盘 miss 率分布校准 N 和分位数
- **A 股 Kelly 不做 regime 检测的本土负结果印证**（[中邮证券 2026-07-09](https://pdf.dfcfw.com/pdf/H3_AP202607091826846688_1.pdf)）：LSTM+GHMM 5 状态做 A 股择时+动态仓位，8 指数均正超额。**关键失败**：2021/2026 结构性行情适应性弱——归因发现**问题不在状态识别层，而在仓位执行层**：Kelly 在特定状态"均值回归"保守，对"假摔反包"响应不足。**启示**：① Kelly 层不承担择时职责（归 sleeve alpha 情绪周期，与 [30号 §2.2](30_multi_strategy_concurrency.md) 正交分工一致）；② regime→Shrinkage 节流路径正确，避免 Kelly 做 regime-conditioned 检测；③ 印证 Kelly 不内置 regime 切换（§2.7）
...
>
> **⚠️ 纯 vol-targeting 的系统性风险警示**：Michael Burry 2026-08 警告（[insta-forex](https://www.insta-forex.com/in/forex_analysis/453380)）——约 $5000 亿 vol-targeting 基金构成**机械级联抛售风险**：S&P 500 跌 2.5% 即触发从 77%→50% 股权配置削减，形成"下跌→vol 上升→机械减仓→进一步下跌"恶性循环（与 1987 portfolio insurance 同构）。**启示**：纯连续 vol-targeting 在极端行情与全市场同类策略同步减仓形成踩踏；本项目 regime Shrinkage **离散分档**（9+3 态）+ HMM **提前识别**可部分规避——regime 转换信号早于 vol 飙升，减仓时点分散于不同切换事件。但若全市场 regime 检测器同质化同样会同步减仓。**缓释**：① 分档阈值差异化（避免整数阈值扎堆）② 减仓速度受限（33号 convergence_window 防抖）③ 保留现金储备（§2.5 最低储备金+机会储备）
>
> **⚠️ regime-conditional 重分配的换手率风险**：[MDPI Economies](https://www.mdpi.com/2227-7099/14/7/268) 2026-07 实证（欧洲 10 资产 2000-2026 walk-forward）：naive regime-conditional CVaR 分配**换手率 ~226%/年**，现实交易成本下净表现**低于简单基准**；实现感知替代方案（weight bands 限单次变动幅度）~29% 换手率恢复差距（net Sharpe 差 0.009）。**核心发现**："瓶颈不是 regime 检测，而是透明、稳定、成本感知的决策规则设计"。**校准**：① 离散分档天然换手率更低（regime 切换月/季级，非每日重分配）；② 33号 convergence_window+防抖是控制换手率关键机制；③ 硬上限"只减不增"单向约束换手率天然可控；④ **待校准**：G04 后实测换手率，若 >100%/年评估 weight bands
>
> **⚠️ 闭环比例控制 vol-targeting（BlackRock AI Lab 2026-03）**：[arXiv:2603.01298](https://arxiv.org/abs/2603.01298) 提出**比例反馈控制替代开环 `L_t = σ*/σ_t`**——开环三缺陷（turnover 尖峰/leverage spikes/σ 估计误差敏感），闭环反馈**显式修正跟踪误差**：
> - **跟踪误差**：`e_k = log(σ̂_k^ind / σ^tar)`（实现 vol vs 目标 vol）
> - **比例控制律**：`w_k = w_{k-1} - K_p · e_k`（跟踪误差正→减仓；负→加仓），天然平滑权重震荡
> - **drawdown suppression 扩展**：回撤加深时额外降杠杆——与 [35号回撤 Protocol](35_drawdown_protocol_impl.md) 四级回撤控制**目标同构**（BlackRock 连续反馈版 vs 35号离散分档版）
> - **实证**：比例控制在持续达成目标波动率上优于开环方案
>
> **定位（远期候选，非 MVP 替代）**：regime Shrinkage 离散分档 vs BlackRock 连续闭环是 vol-targeting 频谱两端。MVP 不切换理由：① regime Shrinkage 已定稿，切换是范式替代非增量改进；② BlackRock 单资产模型，多资产推广需重设跟踪误差聚合口径；③ 连续闭环与 T+1 + convergence_window 低频再平衡哲学有张力。**可借鉴**：① drawdown suppression 同构为离散分档提供连续版参照（G04 后若阶梯跳变过强评估引入平滑项）；② 跟踪误差 e_k 作 [55号监控](55_monitoring_review.md) 维度（实现 vs 目标 vol 偏离度告警）；③ 闭环反馈缓释 Burry 级联风险（减仓速度受限，与 convergence_window 防抖一致）。**Phase 2+ 远期候选**：G04 实盘数据积累后若发现离散分档"阶梯跳变"问题，评估引入比例控制平滑项（离散分档+连续微调混合范式）
...
- **κ 推荐值与施工参数（Sukhov 2026-06 Monte Carlo 校准）**：**swing trading κ=30**（30 笔达半 Kelly，本项目各策略均属 swing 级）。含义：n_eff=30 时 f*_bayes 收缩到 plug-in Kelly 的 50%（等价半 Kelly），n_eff=60→67%，n_eff→∞→100%。**额外约束**：correlated portfolio 设 `f_max=0.15`（与 §2.4.1 单票 8% 叠加取最小）。**等价关系**：n_eff=κ=30 时精确等价半 Kelly，样本更多自动放宽，样本更少自动收紧。**Monte Carlo 实证**（Sukhov p=0.55, n=50, 10000 路径）：Bayesian Kelly 2.47× 增长 9.1% MaxDD 24.6% Sharpe 0.89 破产率 0.8%，优于 Half Kelly 2.21× 增长 7.9% MaxDD 31.2% Sharpe 0.72 破产率 4.2%——捕获 Full Kelly 87% 增长 + 回撤减 60% + 破产风险降 96%
...
| 2026-08-10 | 1.3.0 | 已施工算法 why 回填 + 代码-设计差异协调 | §2.3.1 加实现现状差异框（连续 vs 二值 Kelly，BM-SEL-13 产出 p/b）；§2.3.3 加 C10 未实现标注；§2.4.1 加 8%/5% 三层口径澄清（MOD-POS-001 5%/021 8%/010 5%，冗余待统一）；§2.4.3 补三特殊态+overlay 编号修正；§4.1 更新；新增 §4.4 过度工程审查；§5 新增 3 项待裁定；§7.4 补 10 条 2026 实证 |
| 2026-08-10 | 1.4.0 | 精确 Kelly 公式统一三种形式 | §2.3.1 重写引入精确 Kelly（arXiv:2604.24723），证明二值/精确/Merton 三者统一，大 edge 自动饱和到 1，消除形式冲突；§3.4 补理论支撑；§4.2 阶段 3 更新；§5 "Kelly 形式统一"✅关闭；§7.4 补 6 条实证 |
| 2026-08-10 | 1.5.0 | binding constraint 显式化 + HRP 评估 + Empirical Kelly UQ | §2.3.4 补 binding constraint（5 约束取最小+`sizing_basis`，deadeye-rs 模式）；§2.4.3 补 regime Shrinkage≈vol-targeting 等价关系；§3.7 新增 HRP 评估（拒绝记远期候选）；§5 升级"Empirical Kelly with UQ"；§6 新增 `sizing_basis` 待定；§7.4 补 8 条实证 |
| 2026-08-10 | 1.6.0 | Kelly 降级四检查链 + Bayesian Kelly 评估 + Diffusion 远期候选 | §2.3.2 补 Kelly 降级触发四检查链+param_source 标记；§2.4.1 补冷启动 ×30% 执行时机；§3.8 新增 Bayesian Kelly 评估（Sukhov Beta 共轭先验，Phase 2 候选）；§5 新增 Factor-Based Diffusion 远期候选；§7.4 补 3 条实证 |
| 2026-08-10 | 1.7.0 | HRP-μ 信号感知变体补充 | §3.7 修正"HRP 无 μ"判断：Wuebben 2026-04 提出 HRP-μ 信号感知变体（γ=0 且 μ=1 退化为标准 HRP），但不改变核心拒绝理由；远期候选条件更新为评估 HRP-μ |
| 2026-08-10 | 1.8.0 | MARCD regime-conditioned diffusion 远期候选 | §5 新增 MARCD 远期候选（arXiv:2510.10807v2：HMM+regime-conditioned diffusion+CVaR QP，MaxDD 降 34%，与本项目架构契合）；§7.4 补 4 条实证 |
| 2026-08-10 | 1.9.0 | vol-targeting 级联警示 + regime CVaR 换手率风险 + Diffolio 远期候选 | §2.4.3 补 Burry vol-targeting 级联警示（$5000 亿基金 2.5% 跌幅触发机械减仓）+ MDPI regime CVaR 换手率风险（226%/年）；§5 新增 Diffolio 远期候选（WSDM 2026，直接学习组合分布，ARR +12.1pp）+ MARCD 更新 v2→v3；§7.4 补 4 条实证 |
| 2026-08-10 | 1.10.0 | A 股 Kelly 不做 regime 检测的本土负结果印证 | §2.3.2 补中邮证券 LSTM-GHMM 负结果（Kelly 在特定状态"均值回归"保守，问题在仓位执行层非状态识别层——印证 Kelly 不做 regime 检测的正交设计）；§7.4 补 1 条实证 |
| 2026-08-10 | 1.11.0 | Conformal Kelly lockbox 负结果 + 降级触发第 5 项 | §2.3.2 补 Conformal Kelly lockbox 负结果（OOS 8.5%/7.0% 低于基准，calibration≠growth）+ 降级触发第 5 项（下行 miss→削杠杆）；§7.4 更新 Conformal Kelly 条目 |
| 2026-08-10 | 1.12.0 | Conformal VaR/ES + MPC 待裁定 + VaR/CVaR 计算方法待定 | §5 新增 2 项待裁定：① Conformal Prediction for VaR/ES（arXiv:2602.03903，有限样本覆盖+regime 契合，首选远期方案）② MPC 动态风险调整（arXiv:2604.00415 DLP-SMPC，替代范式远期候选）；§6 新增 VaR/CVaR 计算方法待定问题；§7.4 补 5 条实证 |
| 2026-08-10 | 1.13.0 | Bayesian Kelly 理论根基补强 | §5 Bayesian Kelly 补 Browne & Whitt 1995 理论根基（Sukhov 2026 是其离散化闭式解）；§7.4 补 2 条实证 |
| 2026-08-10 | 1.14.0 | 施工参数缺口补全（σ_i 异常/降级第 5 项/κ 推荐值） | §2.2.2 补 σ_i 异常判定 4 检查链施工算法；§2.3.2 补降级第 5 项施工参数（N=20/60 日 P95/降级 ×0.5/10 日恢复）；§3.8 补 Sukhov κ=30/f_max=0.15 施工参数（n_eff=κ 等价半 Kelly）；§5 Bayesian Kelly 补施工参数引用 |
| 2026-08-10 | 1.15.0 | 资金利用率 70-90% 指引 + EVT Tail Budgeting 候选 | §2.5 补资金利用率 70-90% 指引（quanthedgeai 实证，现金储备缓释同步减仓踩踏）；§6 VaR/CVaR 计算方法补第 4 候选 EVT-Based Tail Budgeting；施工算法完整性闭合 |
| 2026-08-10 | 1.16.0 | Taleb 胖尾 quarter-Kelly 理论背书 | §2.3.1 补 Taleb 胖尾论点（α<2 方差发散→quarter-Kelly+barbell；本项目不降半 Kelly——四层独立兜底等效 barbell；校准：G04 后 Hill 估计 α，α<2 则 0.5×→0.25×）；理论背书非算法变更 |
| 2026-08-10 | 1.17.0 | §3.9 新增 Tepelyan 多元 Kelly sigmoid 标度律（Phase 3 远期候选） | Tepelyan 2026-04 arXiv:2604.11550：积分变换将多元 Kelly 从 O(2^N)→O(N)+O(N²)，sigmoid 标度律（低相关≈独立/高相关≈半 Kelly）。解决拒绝理由②（复杂度），部分缓解①（仅需 pairwise ρ），但③（regime 转折 ρ 不稳定）仍成立。远期升级路径非 MVP |
| 2026-08-10 | 1.18.0 | §2.4.4 流动性硬上限（ADV 口径）+ §2.3.4 binding constraint 栈第 6 项 | 发现流动性口径硬上限缺失（A 股打板+T+1+涨跌停板下比资金口径更先 binding）。§2.4.4 新增 ADV 三档流动性硬上限施工算法（>20% 削到 20%/>10% 削半，20 日 P25，MVP 必做）；§2.3.4 binding constraint 栈同步更新（5→6 约束、sizing_basis +2 值） |
| 2026-08-10 | 1.19.0 | §2.4.4 盘后固定价格交易缓解路径 + 2026-07-06 A 股新规整合 | ①§2.4.4 补盘后固定价格交易缓解路径（2026-07-06 新规扩展至全市场，衔接 33号 Tier 3 fallback，liquidity_cap_severe 时优先盘后交易）；②33号三级升级机制完整性验证通过；③ST 股 ±5%→±10% 不影响（21号已排除） |
| 2026-08-10 | 1.20.0 | §2.4.3 补 BlackRock 闭环 vol-targeting 远期候选 | §2.4.3 补 BlackRock 比例反馈控制（arXiv:2603.01298，闭环跟踪误差反馈替代开环 σ*/σ_t，远期候选非 MVP 替代——离散分档 vs 连续闭环频谱两端；可借鉴：drawdown suppression 与 35号同构、e_k 作 55号监控维度、缓释 Burry 级联风险） |
| 2026-08-10 | 1.21.0 | §3.10 新增 DCVaR RNN 多期组合优化（Phase 4 远期候选） | §3.10 新增 DCVaR RNN 多期组合优化（arXiv:2604.14439 SCOR：DCVaR 偏差度量+RNN 绕开动态规划，Phase 4——分层裁定 vs 统一优化器范式不兼容+RNN 不可解释+时间不一致性+SCOR 场景差异；DCVaR 偏差度量可先于 RNN 在 36 号 ES 监控借鉴） |
| 2026-08-10 | 1.22.0 | §3.10 补路径依赖警示——"凸性才创造价值" | §3.10 补 Noguer 路径依赖警示（arXiv:2608.02355："凸性才创造价值"——全部增益在对称块非路径依赖块；DCVaR RNN 评估须验证路径信号是否对称化后消失+与对称块 ablation 对比）。不改变 Phase 4 定位 |
| 2026-08-10 | 1.23.0 | §2.6 FirmTargetPortfolio 跨文档数据结构同步修复 | 跨文档审计发现 `FirmTargetPortfolio` 31号 4 字段 vs 32号 10 字段零交集（v1.0.0 遗留 vs v1.0.x 权威版）。修复：31号 §2.6 替换为 32号 §2.7 权威版+字段映射说明，契约纪律更新。StrategyTarget 两文档一致无需同步 |
| 2026-08-12 | 1.24.0 | 新增 §4.5 已施工设施盘点 + §6 两项跨文档同步待定问题 | ①§4.5 盘点分层裁定全链路代码资产（MOD-POS-001/020/021/022 production）；登记 4 项未施工演进项；②§6 新增 2 项跨文档同步待定问题（30号 §2.2 矛盾描述、64号域文档滞后） |
| 2026-08-12 | 1.24.1 | 作战地图全覆盖补丁——BM-POS-03 / BM-POS-07 / BM-SEL-21-C | §2.8 新增"持仓漂移与再平衡"（3 环节闭合）：①§2.8.1 BM-POS-03 状态机+双档漂移阈值（±2%/±3%）+边界澄清（42号 OBSERVING 同名不同域/25号层级分工）；②§2.8.2 BM-POS-07 双驱动再平衡执行链（>2×成本/恶化×1.5/偏差<1%）+与 33号分工（budget 驱动 vs 漂移驱动正交）；③§2.8.3 BM-SEL-21-C PF-CORE 调度 why 层（同一参数面唯一真源） |
| 2026-08-12 | 1.24.2 | 作战地图环节映射补强——锚定 BM-RC-02-A、BM-RC-02-B | §2.4.2 末尾补映射块，环节级可追溯 |
| 2026-08-13 | 1.25.0 | σ_i 异常判定 4 检查链落码 + MOD-POS-020 测试补建（施工会话 AI-POS-001） | ①核查确认 §2.2/§2.3/§2.8 全链路已落码；②唯一缺口 σ_i 异常判定 4 检查链补落码（`VolatilityInfo`+`_is_vol_anomaly`+部分降级，向后兼容）；③补 ALGO_FLOW docstring；④测试基建修正（"70 测试"失实→新建 25 测试全绿）；⑤§4.5 缺口标已施工+测试声明修正+ADV 落码注记 |
| 2026-08-15 | 1.25.1 | 第二轮循环压缩：可压缩点收敛=0（AI-DC2-09） | §9 修订记录 1.3.0~1.25.0 各行过程性理由压缩为一行结论（删"全网搜索...评估选项外更好答案"样板文/审查轮次叙述/重复完整性结论）；§2.3.2 Conformal Kelly lockbox 与中邮证券两长段要点化。标题/frontmatter/公式/参数/裁定/开放问题/BM-XXX/跨文档链接零丢失 |
| 2026-08-16 | 1.25.2 | 勘误（双轮审查 D2 对账，AI-RFIX-001）：v1.25.0 ①"§2.3 全链路已落码"表述失准 | §2.3.3 `dist_adj`（C10 偏度/峰度分布感知调整因子，含"正偏例外 ≤1.1"）src 全库零匹配**未施工**——§4.5 未施工表此前已正确登记该缺口，v1.25.0 核查结论与之矛盾，以 §4.5 为准；§2.2/§2.8 落码确认无误。`dist_adj` 施工阻塞条件不变（BM-SEL-13 密度 PDF 高阶矩输出，阶段 3） |
