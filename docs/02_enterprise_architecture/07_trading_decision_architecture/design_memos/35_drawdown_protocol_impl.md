...
> **v1.38.1 新增**：作战地图全覆盖补丁——BM-RC-10（风险否决权，L4 风控域，design）与 BM-RC-10-A（否决执行引擎，BM-RC-10 子环节，design）在本备忘落地。BM 环节定义真源：风险架构.md §3 风险否决权 / §3.2 否决执行机制。
...
2. **当前代码已覆盖否决的 MVP 子集**：`DefaultRiskValidator.validate_order` 在 kill_switch_active 时拒绝全部新订单（severity=HALT，§3.5 执行路径已落码）即是"单规则否决"的已实现形态；BM-RC-10-A 的完整否决规则集（多规则集结构 + 熔断器 + 多路径激活）是 §6.5 RiskOrchestrator 施工时的扩展，MVP 阶段单独建引擎属过度工程。
3. **与 §3.5 既有施工缺口同批次衔接**：§3.5 v1.38.0 口径修正已标注"平仓所有持仓+撤所有挂单"执行链路未落码（§6.11/§6.14 待裁定）——否决引擎（增量拦截）与 KS 平仓链路（存量出清）同属 D_RISK 执行层缺口，须在同一施工批次统一设计（共用 execution_broker 接口 + 审计通道），避免分两批施工产生两套订单拦截点、旁路风险翻倍。
...
> **与 §3.5.1 4 层架构的关系**：否决执行引擎是 L1 代码层"锁定新开仓"能力的**规则化扩展**——当前 `validate_order` 只有 kill_switch 单条规则，BM-RC-10-A 将其泛化为可配置规则集。L3 看门狗层（独立进程）施工时须同步校验"否决引擎未被绕过"（混沌工程验证：定期注入故障验证不可绕过性，BM-RC-10-A params 隔离策略三机制之一）。
...
> **v1.38.1 补——周/月两级亏损限额与强制复盘链路（作战地图 BM-RC-05-C 闭合，design）**：
>
...
>
> **裁定**：在既有"组合日亏 >4% 停开仓 1 天"（日级，本节前表已裁决）基础上**补周/月两级组合层限额**：**周累计亏损 >5% → 停开仓 2 天；月累计亏损 >10% → 停开仓 3 天**。三级联动取最严（日/周/月任一触发即生效，停盘天数取 max）。理由：① 日级熔断管"单日急跌"，周/月级管"阴跌累积"——A 股 T+1 下阴跌型亏损不触发日级熔断但同样侵蚀本金，周/月两级补足时间维度覆盖；② 日级口径层级分工维持既有裁决——BM 阈值表"日 2%/周 5%/月 10%"中的日 2% 是 CAND-HARVEST-0135 **单标的止损层级**（ashare 引擎默认 2% 已实现，本节附注已记录），组合层日级维持 §2.5.1 框架 4%；周 5%/月 10% 为组合层口径与 BM 一致，直接采用；③ 停盘天数 1/2/3 天对齐 BM"强制停盘 1-3 天"区间按级别递进，与 Kill Switch"强制休息 5 天"（§3.2 Level 4）保持梯度（熔断停盘 < Kill Switch 休息）。**重评条件**：实盘 ≥6 月后若周/月级触发频次 >2 次/季度（过敏感致踏空）或同期组合回撤超 15% 而两级均未触发（过宽松未兜底），回测校准阈值。
>
> **契约/参数/接口**：周亏损 = 自然周（周一开盘 NAV → 当前 NAV）累计收益率；月亏损 = 自然月口径同理；数据源 = D-EX-CORE 累计盈亏（BM-RC-05-C consumes）；触发动作 = `DISABLE_NEW`（停开仓）+ 天数冷却——承载机制复用 `trading_kill_switch.py`（MOD-INF-016）的 cooldown 模式扩展 `WEEKLY_LOSS` / `MONTHLY_LOSS` 两类型（对齐既有 `DAILY_LOSS` 的 CANCEL_ALL + DISABLE_NEW + auto_reenable=False 语义，周/月级不 CANCEL_ALL 仅 DISABLE_NEW）；降级 = 限额引擎未就绪 → 人工监控停盘（BM-RC-05-C degradation 原值）。
>
> **强制复盘链路**：三级限额任一触发 → 除停盘外**强制复盘**，由 [55_monitoring_review](55_monitoring_review.md) §3.6 复盘编排器承载——日级触发并入当日自动复盘（四段模板：盈亏归因/偏离告警/参数变更/action items）；周级触发强制升档为**人工周复盘**（编排器"日自动/周人工"频率裁剪下，熔断触发的周复盘不可裁剪跳过）；月级触发除人工月复盘外，须经 §3.16 回撤归因流程产出归因报告后方可申请恢复开仓——月级停盘后的恢复视同一次"软 Kill Switch 复位"，走 §3.14 ResetConfirmation 确认链（owner 确认 + 根因分析留痕）。
...
>
> **v1.38.1 补——多域通知 Saga 裁定（作战地图 BM-RC-03-C 闭合，production 补强）**：BM-RC-03-C（Owner 确认重置与多域通知，L4，production，MOD-INF-018）定义 Owner 确认重置后的**多域通知 Saga**（D-AUTONOMY→D-EXECUTION→D-PORTFOLIO，无补偿），输出 KillSwitch=OPEN 恢复开仓 → BM-RC-02-E 盘前检查放行。**裁定**：MVP 阶段**不建独立 Saga 编排器，多域通知由现公告警通道承载**——复位确认通过本节阶段 0 校验后，`daily_auditor.log_*` 审计日志（D_AUTONOMY 侧留痕）+ `alert()` 告警推送（D_EXECUTION / D_PORTFOLIO 侧通知）+ `state_store.record_reset` 持久化（跨重启可查，本节伪代码已含）三通道同步完成"通知"语义；显式 Saga 编排（Saga 状态机 + 无补偿语义保证）登记远期，归 §6.5 RiskOrchestrator 施工批次统一设计。理由：三域通知均为"写日志/发告警"幂等动作，无补偿 Saga 的编排价值在 MVP 单进程架构下不显著，且 Owner 人工确认本身就是最强的一致性闸门（通知失败可在复位当场被人工发现）。**重评条件**：§6.5 RiskOrchestrator 落地或 L3 看门狗层独立进程化（§6.11）后，通知链路升级为显式 Saga 并补失败重放。
...
| 2026-08-12 | 1.38.1 | 作战地图全覆盖补丁——BM-RC-10 / BM-RC-10-A / BM-RC-05-C / BM-RC-03-C | 作战地图 12 环节闭合施工（本篇 4 环节）：① §3.5.2 新增风险否决权与否决执行引擎（BM-RC-10/10-A，design）——否决规则集结构/策略逻辑隔离/同步拦截 P99<50ms/不可绕过（HC-RISK-03，Kill Switch 单一通道原则延伸）/熔断器模式（5 次失败 60 秒→OPEN 全拒→30 秒 HALF-OPEN→CLOSED）/多路径激活（AI<1ms/人工<100ms/定时 5 秒无心跳/外部<1s）/否决审计追踪五字段；裁定登记设计裁决纳入 §6.11/§6.14 施工批次，与 §3.5 既有"KS 平仓链路未落码"缺口同批次统一设计（增量拦截+存量出清共用 execution_broker 接口与审计通道）；② §3.6 补周 5%/月 10% 两级组合层亏损限额+强制复盘链路（BM-RC-05-C，design——停盘 1/2/3 天递进取最严，承载复用 trading_kill_switch cooldown 扩展 WEEKLY_LOSS/MONTHLY_LOSS，复盘由 55号 §3.6 复盘编排器承载，月级恢复走 §3.14 ResetConfirmation 确认链）；③ §3.14 补多域通知 Saga 裁定（BM-RC-03-C，production 补强——MVP 不建 Saga 编排器，由 daily_auditor 审计+alert 告警+state_store 三通道承载通知语义，显式 Saga 登记远期归 §6.5 RiskOrchestrator 批次） |
| 2026-08-12 | 1.38.2 | 作战地图环节映射补强——锚定 BM-POS-05、BM-RC-03-A、BM-RC-05、BM-RC-05-B | §3.7/§3.20 末尾补映射块，环节级可追溯 |
