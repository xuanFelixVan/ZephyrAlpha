...
| 来源 | 条件 | 模块 |
|---|---|---|
| 回撤 | 组合回撤 > 25%（§2.5.1 Level 4）或 drawdown_tracker EMERGENCY（15%，更紧） | drawdown_tracker / drawdown_controller |
...
执行路径：
```
触发源 → drawdown_controller.kill_switch_advised=True
       → stop_loss.trigger_kill_switch(reason, scope="all")
       → DefaultRiskValidator.trigger_kill_switch() 置状态
       → 平仓所有持仓 + 撤所有挂单 + 锁定新开仓
       → requires_manual_reset: True（人工复位才能恢复）
```

> 代码已实现 `trigger_kill_switch` 返回 `requires_manual_reset: True`，状态由 `DefaultRiskValidator` 管理。`daily_auditor` 日终检查清单第 4 项验证 Kill Switch 终态 == CLOSED。
...
| 层 | 职责 | 触发 | 本项目现状 |
|---|---|---|---|
| L1 代码层 | `stop_loss.trigger_kill_switch` 发市价平仓 + 撤挂单 | drawdown_controller.kill_switch_advised | ✅ 已实现 |
...
> **关键启示**：2026-07 量化私募集体回撤**不是模型失效，而是风险约束不足**——"短期回撤不等于模型失效，不能因为一次极端行情就推翻长期验证过的策略"，但"部分模型的风格敞口过于集中"是可改进的风控缺陷。这直接印证项目 [project_memory 风险优先原则](file:///d:/ZephyrAlpha/docs/02_enterprise_architecture/07_trading_decision_architecture/design_memos/35_drawdown_protocol_impl.md)："回撤 Protocol 施工优先级：风险相关模块（drawdown_controller/var_calculator/kill_switch）先于策略模块施工至 production"——先建风控红线再迭代 alpha，正是量化私募 7 月回撤教训的工程化回应。
...
> **优先级**：P0=最小补丁（立即可做，低依赖）｜P1=短期（实盘 1-3 月后）｜P2=中期（实盘 6 月+）｜P3=远期（实盘 1 年+）
