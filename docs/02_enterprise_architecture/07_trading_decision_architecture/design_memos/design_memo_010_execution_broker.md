...
- **MiniQMT 收紧趋势**（v1.6.0 补全，v1.6.1 订正）：部分券商收紧 miniQMT 新增开通（少数券商关闭新开通道），但**存量用户权限稳定、交易日正常使用**（周末维护属正常系统维护），多数券商仍正常开放。风险等级为"中长期关注"非"生存级紧急"——记录迁移路径作为预案（详见决策⑲）。来源：搜狐两融小顾问 2026-07 行业调研（四类情况：少数收紧新开存量稳定/部分暂停新开存量正常/多数正常开放/少部分主动确认正常）
...
### 2.20 决策⑲：MiniQMT 收紧趋势与迁移路径预案（Platform Tightening & Migration Contingency）

**决策**：记录 MiniQMT 收紧趋势为"中长期关注"风险（非生存级紧急），规划"BrokerInterface 抽象层隔离 + 大 QMT 迁移路径"作为预案，MVP 阶段继续用 MiniQMT（存量用户交易日正常使用）。

> v1.6.0 新增，v1.6.1 订正。v1.6.0 原引 licai.cofool.com（券商营销网站）单一来源称"正在有序关停/存量将陆续无法连接"，经搜狐两融小顾问 2026-07 行业调研订正为**夸大表述**。实际：部分券商收紧新增开通，但存量用户权限稳定、交易日正常使用，多数券商仍正常开放。降级风险等级并保留迁移路径预案。

**收紧现状**（搜狐两融小顾问 2026-07-16 行业调研，四类情况）：
1. **少数券商收紧新开权限，存量暂无关停计划**："现有存量 miniQMT 用户的交易权限、功能使用未受影响，暂无明确的存量关停、清理的相关通知和计划"
2. **部分券商暂停新开，存量正常使用**："仅限制增量，所有存量用户可正常登录、运行量化策略、执行交易，权限保持稳定"
3. **大部分券商未出台收紧政策，可正常开通**："个人投资者满足基础开通条件后，可正常申请开通 miniQMT 业务"
4. **少部分券商主动确认业务正常**

> **国金证券情况**：55188 论坛 2026-07-01 有用户称"收到国金通知 miniQMT 要关了"（指独立行情取消），但迅投论坛 2026-07-16/22 仍有国金用户正常使用 miniqmt xtquant。用户确认"周末两天维护，交易日都是正常使用的"——周末维护属正常系统维护，非关停信号。
...
- **MiniQMT 收紧趋势**（v1.6.0，v1.6.1 订正）：部分券商收紧新增但存量稳定、交易日正常，风险等级"中长期关注"，BrokerInterface 抽象层是迁移防线（决策⑲）
