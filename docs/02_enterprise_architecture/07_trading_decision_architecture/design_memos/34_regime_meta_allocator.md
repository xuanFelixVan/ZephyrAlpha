...
- **⚠️ Sortino 样本量要求（2026-08 四次审查补充）**：Sortino 的下行偏差只统计 `R_daily < R_target` 的日子，A 股 60 交易日中下跌日约 40% ≈ 24 日。ecassets 2026-05 / foliolab 2026 警告：**下行样本不足时 Sortino 系统性偏高（inflated values）**，高估策略表现。防护四件套：
...
- **⚠️ 60 日 vs Sortino 36 个月机构标准（2026-08 六次审查重大修正）**：两个不同来源的"36 个月"须区分——
...
- **⚠️ 加权方式决策（等权 vs EMA，2026-08 八次审查补充）**：
...
- **⚠️ 多策略 PerformanceScore 同向变动说明（2026-08 八次审查补充）**：当 regime 切换（如 r3 牛市→r4 熊市）时，**所有策略的 Sortino 可能同时下降** → 所有 PerformanceScore 同时降低 → normalize 后 allocation 比例可能几乎不变。**这是 feature 不是 bug**：① PerformanceScore 的设计意图就是"后验捕获 regime 亲和性"——所有策略同向变动说明 regime 影响是"市场级"而非"策略级"，此时 allocation 比例稳定是正确的（市场级切换不重定向资金，RegimeScore alpha 择时已被 §2.2 裁定拒绝）；② **"多谨慎"由 global_shrinkage 回答**——regime 切换时 global_shrinkage 降低 → effective_budget = allocation × global_shrinkage 整体同比例收缩，正是 allocation/global_shrinkage 解耦设计的体现；③ **floor/cap 防同向极端**仍生效；④ 打板策略 Sortino 波动远大于多因子，**实际同向变动不会完全同步**（完全同步只在"所有策略对 regime 同等敏感"时发生，实际不会）。
...
- 实现：✅ 代码 `_normalize_and_clip()` 用 **water-filling 投影**（固定越界值 + 按比例重分配未越界部分，避免被裁剪值在再归一化时拉回越界区间——比原"裁剪+全局再归一化"伪代码更稳，见 §9 v2.7.0）。**⚠️ floor/cap 无解兜底（2026-08 六次审查补充）**：floor+cap 约束在策略数 N 较小时可能**数学无解**——如 N=2 + floor=5% + cap=40%：两策略都 ≥5% 且 ≤40%，Σ=1.0 → 一个 ≥60% 必然违 cap。无解场景的兜底（参考 AIMS Mathematics 2026, 11(2):3647 Lkhagvasuren et al. feasibility restoration 思想——当约束无解时找"最近可行解"）：
...
（远期演进路径排序见第十八条末的十八次审查更新版——含全部条目，为当前权威版本）
...
- **①.4 近期**（不换架构，**最高优先级**，十八次审查新增）：**CHMM-t（Student-t emission）**——仅改 emission 分布 ~50 行，解决分布性问题，**优先于 Sticky HMM**
...
> **过度工程审查更新（十八次审查）**：CHMM-t 是所有远期候选中**实现成本最低**（~50 行 emission 替换，不换检测器/不换状态划分/不重写 C1）且**证据最颠覆**的路径。**升级优先级更新**：CHMM-t（分布性，最高优先级）> Sticky HMM（时间性-先验）> 5 态结构（时间性-过渡态）> Hybrid HMM Poisson（时间性-持续期+绕过 EM）> JM（时间性-完全重写）。所有远期候选均登记非施工算法缺失，MVP 用 Gaussian HMM 4 态（C1 已验证有效）。
...
| Invesco+清华《Dynamic global asset allocation》(2026-04) + I-am-Uchenna/regime-allocation-strategy (2026-01) + donarduka/regime-switching-portfolio (2026) | regime-based 动态分配实证：Invesco Markov Regime-Switching"higher returns, better risk-adjusted performance, shallower drawdowns"；I-am-Uchenna Regime Strategy Sharpe 1.220 / MaxDD -19.54% vs Equal Weight 0.660 / -23.45%（Sharpe 近翻倍，MaxDD 改善 4pp）；donarduka Sharpe +0.026、MaxDD 改善 5.5pp（与我们 C1 的 7.36pp 同量级，独立印证 regime 节流效果） |
| arxiv 2410.14841《Dynamic Factor Allocation Leveraging Regime-Switching Signals》 + 华安证券《基于状态切换信号的动态因子配置》(2025-10-21, SJM+BL) | regime analysis ≠ factor timing（不预测何时切换）；BL + regime 信号 IR 0.05→0.4（华安 SJM 特征加权识别单因子牛熊态整合 BL，IR 同量级 8 倍提升，双重印证）——支撑"regime 用于风险节流/分配调整，不用于择时"；我们拒绝 BL（§4.5），但 SJM 特征选择思想可吸收到 RiskSignal 13 参数权重优化 |
| misango《Regime-Based Portfolio Strategies》(2026-03) + preprints.org《When to Route? Regime-Adaptive Meta-Policies》(2026-05) + loic-mmt/quant-portfolio (2026-08-07) | misango：60/40 在交易成本后优于复杂 ML 方案（simple often outperforms complex），支撑"三因子乘法"而非 RL/优化器（§4.4）；preprints：层次化决策"direct optimization safer in low-signal"——印证"低信号时直接部署（Shrinkage 节流）比试图路由（alpha 择时）更安全"；loic-mmt：哲学"不预测未来，只检测状态+调整暴露"——与我们"regime 只做风险节流不做择时"完全一致 |
...
| stockalpha.ai《AI for Portfolio Optimization》(2026-01) + arxiv MM-ARC 2509.05080v3 (2026-07-27) + arxiv 2510.14986《RegimeFolio》(2026-10) | stockalpha："improving covariance/return estimates yields outsized benefits vs tuning optimizer knobs"——我们用 PerformanceScore 后验代替协方差估计（该建议的轻量替代）；MM-ARC："regime-conditioned strategy pools"与我们 regime→Shrinkage 一致，但复杂度高，不选 RABO；RegimeFolio：Ledoit-Wolf 协方差 + regime-conditioned MVO（Sharpe 1.17、MaxDD 改善 12%）——A 模型明确拒绝 MVO，但"regime 分割+分别建模"思想与 Shrinkage 节流一致 |
| arxiv 2603.04441《Explainable Regime-Aware Investing》(2026-02) + arXiv:2507.19824《Regime-Switching Induced Stock Price Jumps》(2025-07) + arXiv:2602.16952 HyRA(2026-02-18)+ewinnington(2025-04-16) | 2603.04441：Wasserstein HMM 严格因果滚动 + 模板身份追踪，核心论断"**regime inference stability—particularly identity preservation—is a first-order determinant of portfolio drawdown**"（Sharpe 2.18 vs SPX 1.18）——印证 Shrinkage 路径，记为 10 号 Phase 3 增强候选（身份保持机制）；2507.19824：**制度切换时股价本身发生跳跃**（A 股牛熊转换常伴跳跃，如 2024-09 政策底单日 +8%）——ConfidenceSignal 四档映射在切换瞬间可能来不及反应，印证 §3.3 日频更新 + D-SIGNAL-68 盘中重算设计；HyRA/ewinnington：加 floor/关系约束时迭代 water-filling 失效→MIQP（需求解器）——当前 `_normalize_and_clip` water-filling 对 N=3-5 有效，MIQP 记为远期升级路径 |
| advisingalpha.com《Sharpe vs Sortino》(2026-05) + fastercapital.com(2026-05) + moneylume.com(2026) | "Sortino is a better measure of risk-adjusted return for most individual investors because it only penalizes downside variability"（advisingalpha）；"investors often care more about avoiding losses than maximizing gains"（fastercapital）；"Not ideal for: options, leveraged ETFs, or crypto—use Sortino ratio instead"（moneylume——A 股打板/事件驱动非对称收益分布类似 options payoff）。支撑 §3.2.2 Sortino 切换 + downside-focus 与风险节流目的对齐 |
| equiscale.ai《Sharpe vs Sortino》(2026) + portfoliogenius.ai《Sortino Ratio》(2026) + fxroboteasy.com(2026-08-04) | "Use Sortino for high-volatility strategies—assets have massive good volatility that would unfairly lower a Sharpe score"（equiscale——A 股打板涨停板 = massive good volatility，直接支撑 Sortino 选型）；Sortino 解读区间（<0 失败 / 0-0.5 低于平均 / 0.5-1.0 适中 / 1.0-2.0 很好 / >2.0 卓越，portfoliogenius——支撑 §3.2.2 映射区间 [0.5,1.5] 语义）；"When Sortino is preferred: trend-followers, breakout systems"（fxroboteasy——A 股打板=breakout system，实用区间 1.0-1.5 good / >2.5 excellent 与我们映射语义一致） |
| recessionistpro.com《Capital Preservation》(2026-02) + protraderdaily.com《Recession Portfolio 2026》(2026-08-09) | recessionistpro："Define your drawdown limit first—5%/10%/15% determines everything"——支撑 §3.2.2 熊市最低总暴露设计（A 股 cash=防御资产，9% floor 对应"极端防御"档位）；protraderdaily：60% bonds/25% dividend/15% alts 组合 MaxDD -8.2% vs growth -23.7%——多资产防御组合对照，A 股单市场无此 sleeve，用高 cash（低 global_shrinkage）等效防御 |
| ecassets.com《Sortino Ratio》(2026-05-28) + foliolab.ai《Sortino Ratio》(2026) | **关键警示**："downside sample is small → statistically unreliable" + "inflated values for strategies with few downside observations"（ecassets）；"Requires more data than Sharpe for stable estimates" + "Can produce very high values for strategies with few downside observations"（foliolab）。直接支撑 §3.2.2 Sortino 样本量门槛（downside<15 强制中性）+ inflated values 防护三件套（样本门槛 + cap 兜底 + gap 监控） |
| Oliveira et al.《Tactical asset allocation with macroeconomic regime detection》(Quantitative Finance 2026-06-11, Oxford-Man Institute) + Pei et al.《Market Regime Council for Dynamic Credit Assignment》(arXiv:2605.24490, 2026-05-23) | Oliveira：FRED-MD 宏观数据集 + modified k-means regime 检测 + 预测 regime 分布→仓位映射（优于 equal-weight/buy-and-hold）——"regime 预测分布→仓位映射"思想与 Shrinkage 节流一致，记为 10 号前沿演进候选（宏观特征补充）；Pei（MRC）：**贝叶斯自适应混合**稳定早期冷启动（`w_prior×1.0 + w_data×Sortino` 权重随样本量渐变）比我们 30 日阈值硬切换更平滑——记为 §3.2.2 冷启动远期候选（MVP 不采纳）；Shapley credits 需 O(2^N) 评估，N=5 过重不采纳（我们的 O(N) 乘法是正确替代） |
| forex-basics.com《Sortino Ratio Basics》(2026-05-17, verified 2026-05-28) + financefriend24.com《Sortino Ratio Explained》(2026) | **关键机构标准**："The institutional standard is a minimum of thirty-six months"（forex-basics）；"With fewer than 36 monthly observations (3 years), the result is highly sensitive to individual bad months" + "Short-period Sortino Ratios—particularly from strategies that rarely lose money—should be treated with caution"（financefriend24）。直接支撑 §3.2.2 重大修正——Sortino 有 36 个月机构标准，我们 60 日远低于标准，是已知妥协，需防护四件套 + 实盘校准触发条件 |
| superglobalcalculator.com《Sortino Ratio Calculator》(2026) + getzenquery.com(2026) | "Need ≥ 30 periods for stability"（superglobalcalculator）；"For more reliable results, consider using 20-30+ return periods"（getzenquery）。支撑 §3.2.2 downside 样本量门槛（我们 60 日 downside ~24 日接近 30 阈值，需防护） |
| Alkhudaydi & Althobaity《GATE-WPCA-PI》(AIMS Mathematics 2026, 11(2):3647-3702) + Lkhagvasuren et al.(JIMO 2026, 22(4):1672-1692) | GATE-WPCA-PI：**entropy floor**（防集中）+ **sleeve caps**（限制单资产）与我们 floor 5%/cap 40% 同构（§3.2.4 学术级印证），PI 控制器记为远期候选（MVP 不采纳）；Lkhagvasuren：feasibility restoration——约束无解时找"最近可行解"，直接支撑 §3.2.4 floor/cap 无解兜底设计（N=2 时"优先保 floor 降 cap"策略） |
| AI Finance Labs / Lopez-Lira《Claude AI-Managed Portfolio》(2026-03 至 2026-08) + sooktrading/whalesbook《KOSPI Collapse 2026》(2026-08-03/05) | Claude AI 管理 $50K 组合 5 个月回报 19.04% vs S&P 12.24%，最大持仓 = 0-3 月国债 ETF（最防御）——**AI 无人值守系统天然倾向防御**，印证"风险优先 + cash 防御 + 低 global_shrinkage floor 9%"设计；KOSPI 2026-07 六周跌 40%、杠杆 ETF + 集中三星/SK海力士（>50% 权重）= 多米诺崩塌、32 万账户强平——印证 cap 40% 防集中 + 禁杠杆必要性（A 股打板天然高集中，cap 40% 是防"KOSPI 式集中崩塌"的硬约束） |
| Yang et al.《RMATS: Recursive Multi-Agent Trading System》(arXiv:2605.25311, 2026-05-25, APAM 2026) | 4 specialized agents + recursive Manager。MaxDD 9.62%（vs MVO 15.49%）。**Risk Agent 独立于策略 agent**。与 34号 的关联：印证 regime 层应独立于策略层解耦，但**不采纳多 agent 递归架构**（个人项目过度工程审查，见 §4.4/§3.2.7）；RMATS 的 adaptive circuit breaker 与我们 ConfidenceSignal 四档 + §3.2.7 外部信号冲突降档思想一致 |
| [YoungCan-Wang/Wyckoff-Analysis](https://github.com/YoungCan-Wang/Wyckoff-Analysis) v2.1.x（2026-04 实证） | A 股大盘水温 5 档仓位控制：NEUTRAL 100% / RISK_ON 50% / PANIC_REPAIR 50% / RISK_OFF 30% / CRASH 0%。实测：NEUTRAL +1.17%（唯一正收益）。核心结论"选股选得好不如市场选得对，水温仓控是性价比最高的风控手段"。直接支撑 §3.2.7 5 档水温作为 HMM 4 态的外部交叉验证信号 |
| [YoungCan-Wang/WyckoffTradingAgent](https://github.com/YoungCan-Wang/WyckoffTradingAgent) Wiki《04_Finance_Sector_Rotation_Regime》（2026-04 实证） | 板块轮动状态 5 分类 + watch_score 加减分机制。实测：共识高潮后 3 日下跌 >2% 概率 29.8%。支撑 §3.2.7 板块轮动状态作为 regime 检测的辅助信号——DISTRIBUTION_RISK 是"最危险状态"，与 HMM r4（熊市）形成交叉印证 |
| pooyagolchian《Portfolio Risk Management: VaR, CVaR, and Kelly Criterion for 2026》(2026-04-13) | Fractional Kelly 实证：Full Kelly CAGR 18.2%/MaxDD −62% / Half Kelly 14.1%/−38% / **Quarter Kelly 10.8%/−22%**（85% 增长 / 35% 回撤）。直接支撑 §3.2.3 Shrinkage 节流与 Quarter Kelly 同构——"适度收缩风险预算 → 以小得多的回撤代价获大部分增长"是 §2.2 regime 风险节流裁定的实证依据 |
| Nystrup/Boyd/Lindström/Madsen《Multi-period portfolio selection with drawdown control》(Annals of Operations Research 282(2):1-27, 2019) | MPC 动态优化 + 多变量 HMM 多期均值/协方差预测 + **根据已实现回撤调整风险厌恶系数**。记为 §5.2 远期候选——完整 MPC 框架需协方差估计 + 凸优化求解器；但"realized drawdown → γ 动态"思想可先吸收为 ConfidenceSignal 的回撤通道增强（§3.2.7/§6 待裁定），不换架构 |
| Grube Martín-Lunas et al.《From Regime Detection to Decision Rules》(MDPI Economies 14(7):268, 2026-07-09) + Verma/Putri/Lesupi(arXiv:2605.27848, 2026-05-27) + youcanbuildthings.com(2026-05-06) | Grube：4-state HMM + CVaR 优化 + 严格 walk-forward——naive regime-conditional CVaR 年换手 ~226%，regime-constrained weight bands 才是关键（"**bottleneck is not regime detection but transparent, stable, cost-aware decision-rule design**"）——支撑 Shrinkage 只缩放总暴露（低换手）而非 regime-conditional MVO（高换手），印证 §4.2 MVO 拒绝；Verma：3-state HMM + RL 分配 SPY/TLT/GLD，**one-day execution lag 避免 look-ahead bias**（与我们 T+1 结算窗口一致），RL MVP 不采纳（§4.4）；youcanbuildthings：risk-parity（inverse-vol）+ **90-day correlation drop rule** + per-strategy circuit breaker（15% half / 25% zero）——与我们多策略架构高度相似，correlation drop rule 是 [30号] 策略独立性可借鉴的量化判定 |
| Shu/Yu/Mulvey《Downside risk reduction using regime-switching signals: a statistical jump model approach》(Journal of Asset Management 25(5):493-507, 2024；arXiv:2402.05272) | **Statistical Jump Model（JM）原始论文**——显式 jump penalty λ 增强 regime 持续性；特征集 DD_10+Sortino_20+Sortino_60（**与我们 PerformanceScore Sortino 同源**）；US/Germany/Japan 1990-2023 JM-guided 在波动率/MaxDD/Sharpe 全面优于 HMM 与 buy-and-hold。直接支撑 §5.2 JM 远期候选 |
| 中金公司《量化配置模型系列（2）：基于统计跳跃的系统性风险预警模型》(2026-06-24, finance.sina.com.cn 报道) | **JM 应用于 A 股（股-债-金八资产）的关键实证**——跳跃惩罚 λ：权益/黄金=50，债券=25；强制避险持续≥60 交易日。实证：三资产风险平价 MaxDD -7.07%→-3.23%、卡玛比率 0.77→1.59。**关键启示**：① JM 在中国市场有效；② 60 交易日持续期与我们 Sortino 60 日窗口巧合一致；③ "多资产确认+强制持续期"降低 false positive——我们单市场用 §3.2.7 外部信号交叉验证起类似多源确认作用。支撑 §5.2 JM 远期候选 + §3.2.2 60 日窗口选择 |
| Cortese/Kolm/Lindström《Generalized information criteria for high-dimensional sparse statistical jump models》(AStA 2026-06) + Li/Chen/Tao/Ji《JM+MPC》(Mathematics 13(17):2837, 2025) | Cortese：**JM 超参数选择的学术标准**——广义信息准则用于 jump penalty λ 选优，MSCI 实证 3 态最优（与我们 4 态 BIC 同量级，JM 不强制减少状态数）；Li：**JM-MPC 混合框架**——JM 识别 regime + MPC 滚动优化，高波动期显著降回撤，支撑 §5.2 远期演进路径排序③；MVP 均不采纳（需协方差估计问题先解决） |
| dataloopr《Regime-Aware Portfolio Strategies》(2026-03-09) + Soloviov《Asymmetry, Fat Tails, and the Cost of the Wrong Innovation》(2026-07, marketmaker.cc) + Park/Kim《Deep Generative AI for Portfolio Management》(Columbia, 2025) | RiskSignal 波动建模远期参考：dataloopr 集成框架（GARCH(1,1)+Student-t 肥尾+Markov switching+动态波动率目标仓位）——GARCH+Student-t 是 realized_vol 分位数的进阶建模，动态波动率目标与 Shrinkage 同构；Soloviov 受控实验（GJR-GARCH(1,1)+Student-t(ν=5)，已知 ground truth）：**Student-t innovations 修复几乎所有 VaR 覆盖误差（99% VaR 违规率 1.58%→1.03%），尾形效应比不对称效应大一个数量级**——RiskSignal 若升级 GARCH，Student-t innovations 必选，ν 估计是关键参数；Park/Kim：深度生成模型（Student-t Glow/Flow Matching/Score-Based）做 CVaR 优化——Phase 5+ 前沿路径，MVP 不采纳（训练成本+黑箱） |
| 汇安基金柳预才《大小盘动态量化新框架》(2026-08-07, cnfol.com 报道) | **A 股本土双层动态量化实证**——顶层市值风格判别模型（**20+ 量化因子**）+ 底层双独立选股池 + **切换阈值过滤日间噪音** + **底仓对冲误判风险**。与 34号 的关联：① 顶层风格判别 ≈ 我们的 regime 检测器 + Shrinkage，但汇安做的是**大小盘 alpha 择时**，我们明确拒绝 RegimeScore 择时（§4.1）；② 双独立选股池 ≈ StrategyBook 独立 sleeve；③ **切换阈值过滤噪音**与 ConfidenceSignal 四档同构；④ **底仓对冲误判**与 floor≥5% 同构。A 股 2026H1 量化超额从 14.17% 降至 3.11%（[新浪财经 2026-07-11](https://finance.sina.com.cn/jjxw/2026-07-11/doc-inihmkxc5002361.shtml)）印证"风险节流不做 alpha 择时"裁定的正确性 |
| firestrand/marketregimeml《Model Comparison》(2025-09, GitHub) | **regime 检测模型对比基准**——快速 SVM 集成（RBF+Linear）RQI 83.6-86.9 vs HMM 67.8-76.6 vs LSTM 49.1-64.9（最差）。**10 个优化特征胜过 35+ 特征**，n_regimes>3 过拟合。与 34号 的关联：① HMM 非最优但 C1 已证明有效，MVP 不换；② SVM 集成为 §6 HMM→JM 替换提供更轻量替代路径（无需 jump penalty 调参）；③ **10 特征原则**印证我们 6 因子 HMM 输入的精简设计正确 |
| quantt.co.uk《Sortino Ratio Explained》(2026-04) + CFA Institute《The Sortino Ratio》(Deborah Kidd, CFA, rpc.cfainstitute.org) | quantt：**Sortino ≈ 1.3-1.5 × Sharpe 正常范围基准**（支撑 §3.2.2 gap 监控阈值校准）+ **下行偏差分母用总样本量 N（非下行观测数）的独立验证**——Step 5 明确"divide by the total number of observations (N), not just the number of below-target ones"，"common source of confusion"——独立验证施工要点 #13 的 Sortino 分母修复（n_downside-1→n-1）；CFA Institute：Sortino 权威原始定义来源，确认 downside deviation 分母争议"easy to understand but also easy to miscalculate" + 引 Sortino-Forsey 1996 bootstrap 方法——为施工要点 #14 bootstrap 远期候选提供原始理论溯源 |
| RPubs《Market Regime Detection using HMM Walk-Forward》(2026-04) | **HMM walk-forward 三策略对比实证**——① expanding window + hard switch **失败**（crisis-scarred 训练数据导致 HMM 过度保守）；② rolling 3yr + hard switch 纠正；③ **Strategy C（soft allocation，概率加权敞口）最优 MaxDD -30.5%（vs hard expanding -52.4%）**。与 34号 的关联：① **直接验证 ConfidenceSignal 四档软映射设计**（非 binary 开关），rpubs 实证 MaxDD 改善 22pp 为软映射优于硬切换提供量化证据；② **expanding window 失败警示**——HMM 训练窗口应用 rolling 而非 expanding，归 [10号] HMM refit 策略设计约束 |
| GARCH-ARJI《Jump Persistence in Financial Markets》(Int J Forecasting 42(3):833-852, 2026) + M-ROLL《AI-Markov Hybrid Portfolio Framework》(IJFMR 2026-03/04) | GARCH-ARJI：**跳跃强度的时变持续性**——RiskSignal realized_vol 若升级 GARCH 建模**须纳入 ARJI 跳跃强度**（常数跳跃强度低估尾部风险），为 §5.2 JM 远期候选提供跳跃持续性理论支撑；M-ROLL：离散 Markov 链 + PSO 散度最小化（Sharpe 1.87 较等权 +260%，MaxDD 44.2%→28.1%）——不换检测器（仍 Markov）换优化器（PSO 替代三因子乘法）的第三条远期路径，MVP 不采纳（PSO 增加复杂度且三因子乘法 C1 已验证有效） |
| Acanto 8A《Momentum + Risk Parity White Paper》(2026-02) + arXiv:2606.09478《Volatility Forecasting under Market Regimes: High-Frequency Chinese Equity Data》(2026-06) | Acanto 8A：21 资产 + 月度再平衡 + 双动量 + 风险平价，2008-2026 回测 Sharpe 1.19、**Sortino 1.21**、MaxDD 仅 -7.6%——Sortino 1.21 是动量+风险平价**机构级基准**（首批策略实盘后 Sortino 达 1.0-1.5 是合理预期），双动量（绝对+相对）与我们 Base 先验 + PerformanceScore 后验同构；arXiv:2606.09478：**A 股高频实证** regime-augmented HARQ + Markov-switching GJR-GARCH 波动率预测持续优于 baseline，严格 walk-forward 与 C1 协议一致——支撑 RiskSignal realized_vol 升级 regime-augmented GARCH 远期候选的 A 股适用性，MVP 不换（分位数已验证有效） |
| arXiv:2604.09060《Taming the Black Swan: A Momentum-Gated Hierarchical Optimisation Framework for Asymmetric Alpha Generation》(2026-04, Chakraborty & Singh) | **AEGIS 框架：动量门控 + 最小最大化相关 + SLSQP 直接优化 Sortino**。20 年 walk-forward 回测：熊市降低崩盘强度，牛市保留不对称上行参与。与 34号 的关联：① **直接优化 Sortino** 印证我们 PerformanceScore 选 Sortino 的正确性；② minimax 相关强制分散与我们的 floor 5%/cap 40% 防集中同构。MVP 不采纳 SLSQP 优化器（需协方差估计），记为远期候选 |
| arXiv:2603.10202《Hybrid Hidden Markov Model for Modeling Equity Excess Growth Rate Dynamics: A Discrete-State Approach with Jump-Diffusion》(v2 2026-04-02, Alswaidan & Varner, Cornell) | **混合 HMM + Poisson 驱动跳跃持续期机制**——Laplace 分位数定义状态 + Poisson jump-duration 强制尾部态驻留 + 直接转移计数绕过 Baum-Welch EM。SPY 10 年 KS/AD pass rate >97%/91%（样本内）、>94%（样本外）。与 34号 的关联：**直接支撑 §5.2 第十七条远期候选**——独特优势是**参数估计比当前 HMM 更简单**，同时通过 Poisson jump-duration 获得与 JM jump penalty 同等的持续性增强。MVP 不换（C1 已验证 HMM 4 态有效） |
| Oliveira/Guzman/Firooziye《(Non-Parametric) Bootstrap Robust Optimization for Portfolios and Trading Strategies》(arXiv:2510.12725, 2025-10-14, USP/UCL) | **非参数 bootstrap 鲁棒优化**——把效用当作随机变量做百分位优化，非参数 bootstrap 重采样构造数据驱动的置信区间。与 34号 的关联：① 直接支撑施工要点 #14 bootstrap CI 远期候选（PerformanceScore 从"点估计映射"升级为"bootstrap 下分位映射"）；② **CFA Institute 2026 共识**：downside deviation 分母必须用全部 n（非 n_downside）——直接支撑施工要点 #13 的 CRITICAL bug 修复 |
| quarcc.com《Endogenous Regime-Switching with Duration Dependence》(2026-03-08) + arXiv:2604.27991《Stochastic Inertia in Regime-Switching Models》(2026-04-30) | quarcc：**negative-binomial hazard duration dependence**——比 Poisson 更灵活处理过离散停留时间（A 股熊市停留时间方差 > 均值时 Poisson 不够）+ skewed-t 印证 Soloviov"Student-t innovations 必选"结论，记为远期候选池第五条路径；Stochastic Inertia：噪声反而能增强 regime 寿命——regime 持续性增强的理论基础（最轻量路径：仅在转移概率上加噪声），MVP 不采纳 |
| Soloviov《Do Bootstrap Confidence Intervals for Backtest Statistics Cover? A Controlled Study Under Serial Dependence》(bootstrap.marketmaker.cc, 2026-06-10, arXiv-ready; [GitHub](https://github.com/suenot/bootstrap-coverage)) | **Bootstrap CI 覆盖率受控实验**——5 种已知真实 Sharpe 的 DGP × 3 种样本长度 × 14 种区间方法，共 6000 次实验。**关键发现**：① **BCa 仅在 iid 下有效**（覆盖率 0.954），AR(1) φ=0.3 下失效（0.838 vs 名义 0.95）——"失败的是 resampling scheme 而非区间公式"；② GARCH 与 regime 波动率对 Sharpe 覆盖率损害很小；③ **最大回撤分位数全部乐观偏差**；④ 实践配方：默认用自动块长 block bootstrap 或 Lo HAC SE。**直接支撑施工要点 #14 十六次审查 CRITICAL 修正**——须升级为 stationary block bootstrap（Politis-Romano 自动块长），AR(1) 下覆盖率恢复 0.946 |
| Staures & Kabašinskas《Identifiable Regime Detection in Pension Fund Networks via Sticky Hidden Markov Models》(preprints.org 2026-06-02, DOI:10.20944/preprints202606.0111.v1; Mathematics 2026, 14(14):2463) | **Sticky HMM Dirichlet 自转移先验**——Bayesian sticky 转移先验 `π_k ~ Dirichlet(α·1 + κ·e_k)`，κ 控制自转移持续性。识别 3 个潜态，高风险期 cluster 跟踪误差放大 1.09×-1.23×。**直接支撑 §5.2 第十六条远期候选**——实现侵入性极低（转移矩阵加 1 行先验），统计上最规范。MVP 不采纳，若 HMM 实盘后发现状态高频抖动，Sticky HMM 是比 JM 更轻量的第一升级路径 |
| usepancake/batter《Pancake Engine Bootstrap CI + Permutation Test》(GitHub math-audit-0.4.md, 2026-05-26) | **Percentile bootstrap CI 工程实现 + CI_TOO_WIDE 守卫**——percentile bootstrap（10000 次重采样）+ `CI_TOO_WIDE` 守卫（阈值 5×：当 `(ci_high - ci_low)/|point_estimate| > 5.0` 时该指标不可信）。**直接支撑施工要点 #14 CI_TOO_WIDE 守卫**——5× 阈值来自 Ding & Martin (2017) 对 Sharpe ratio 的校准。当 PerformanceScore 的 bootstrap CI 触发 CI_TOO_WIDE 时，应触发更强 Shrinkage 或强制中性 |
| 中邮证券/黄子崟《市场脉搏（2）：基于 LSTM~HMM 混合方案的量化择时与动态仓位管理》(2026-07-09, SAC S1340523090002) | **A 股本土 LSTM 自编码器 + GHMM 5 态方案**——LSTM 自编码器（90 日 × 25 维 → 10 维压缩）+ 高斯 HMM 5 态（1 过渡态 + 4 稳态），状态切换经过渡态完成减少稳态间直接跳跃。2021 年以来多指数回测控制回撤并积累超额；**2026 年 K 型极端分化行情适应性偏弱**。**直接支撑 §5.2 A 股本土对标 + §6 待裁定 5 态结构**——2026 K 型分化失效（2026H1 量化超额 14.17%→3.11%）印证"风险节流不做 alpha 择时"裁定。MVP 不采纳 |
| 华安证券/严佳炜、钱静闲《自适应市场状态的强化学习在资产配置中的应用》(2026-05-01) + 湘财证券/仇华《2026年8月大类资产配置展望》(2026-07-26) | 华安 RL：KMeans/GMM/HMM 三机制（稳定/中性/危机，可作 4 态 HMM 简化对照）+ Transformer PPO Sharpe 1.43、Sortino 1.59——**奖励裁剪是关键**（可嵌入 PerformanceScore 防极端），**机制信号互信息 0.1020** 是可量化的信号强度基准（首批实盘后可测 HMM 信号与策略 PnL 互信息对照）；MVP 不采纳 RL（§4.4）。湘财：A 股本土波动率锚定风险平价三档（目标年化波动 ≤4% / 8-10% / 15-18%，进取型权益 61%）——可参考作为 Base 因子 A 股校准基准（MVP 用等权 1/N，远期可升级 inverse-volatility 或 risk parity） |
| CSDN/mokamo《A股市场状态识别：HMM + Optuna 超参优化》(2026-05-16) + arXiv:2606.06190《Multi-Scale MS-GARCH EUR/USD》(2026-06-04) | CSDN：A 股 3 态 GaussianHMM 实战——A 股年化波动率 20-30%（远高于标普 15%）、60%+ 时间震荡，**3 态是"过拟合与表达力"的折中（2 态太粗、4 态过拟合）**——直接质疑 4 态选择，记为 §6"3 态 vs 4 态 ablation"项（[11号] C1 BIC 稳定收敛 4 态但未做 3 态样本外对比）；arXiv:2606.06190：4H/1H 上 TVTP 强烈受支持，**1D（日频）上静态转移概率更优**——支撑我们 HMM 4 态用静态转移概率的选择，MVP 不采纳多尺度 |
| arXiv:2512.03777《iHMM 初始化策略对比》(2026-06-12 v2, Cortese & Rossini) + kooexperience.com《HMM Regime Detection》(2026-03) | iHMM：**distance-based clustering（KMeans++）初始化一致优于 model-based 和 uniform**——对当前 HMM 训练直接有用（归 [10号] HMM EM 训练初始化策略）；iHMM 自动推断状态数可解决"4 态是否最优"元问题但贝叶斯非参推断成本高 MVP 不采纳。kooexperience：3 态教程实证"three is the sweet spot"（two too coarse, four+ overfitting noise）——与 CSDN 同向质疑 4 态，支撑 §6"3 态 sweet spot 佐证"项；但 [11号] C1 BIC 选优稳定收敛到 4 态且有语义基础（r1-r4），待 ablation 定论 |
| susanpotter.net《Bootstrap Methods for Strategy Robustness》(2026-05-23) + metricgate.com《Choosing a Resampling Scheme for Dependent Time Series》(2026-06) | susanpotter：naive i.i.d. bootstrap 在金融时序上三重失效（破坏自相关/波动率聚类/截面相关），block bootstrap（Künsch 1989 / Liu-Singh 1992 / Politis-Romano stationary）保留块内依赖结构解决；metricgate：四种依赖时序 bootstrap 对比决策框架，**AR(1) φ=0.7 下 naive i.i.d. bootstrap 标准误仅为真实值 ~40%（灾难性低估）**，stationary bootstrap 对单一块长选择不敏感更鲁棒。**直接支撑施工要点 #14 stationary block bootstrap 选择**（A 股日频收益率的正确 resampling scheme） |
| Alswaidan, Jin & Varner《Continuous Hidden Markov Models for Equity Returns: Heavy-Tail Emission Families and Regime-Conditional Value-at-Risk》(arXiv:2606.23492, 2026-06, West Virginia University) | **CHMM-t（Student-t 发射的连续 HMM）颠覆性发现**——"HMM 无法复现收益率绝对值自相关的慢衰减"是**分布性**问题而非**时间性**问题——重尾边际分布（Student-t）而非更多 decay modes 弥合了大部分拟合差距，**无需调超参数**。产出 regime-conditional VaR，通过 Christoffersen 条件覆盖检验。**直接支撑 §5.2 第十八条远期候选**——实现成本最低（~50 行 emission 替换），证据最颠覆（分布性修复优先于时间性修复）。MVP 不换（C1 已验证 Gaussian HMM 4 态有效），若实盘后发现分布性问题则 CHMM-t 是第一升级路径 |
| Pav《Post-Selection Estimation of Sharpe Ratios》(arXiv:2606.01650v1, 2026-06-02) | **选中最高 in-sample Sharpe 策略后的真实 Sharpe 估计**——系统测试 5 种修正估计器。**关键发现**：James-Stein 估计器在多数现实参数下最优，紧随其后是 GMLEB 经验贝叶斯。收缩因子 `s = (1 - (k-2)·σ²/‖ζ̂‖²)₊`（positive-part，k≥3 生效）。**直接支撑施工要点 #15 PerformanceScore 选择偏差收缩远期候选**——James-Stein 收缩是正交于四件套绝对值防护的多策略相对值防护。MVP 不进，若实盘后发现"上月 Sortino 最高策略次月反转"则 James-Stein 是第一修正路径 |
| Ryan《Conformal Kelly: Conformal Prediction Intervals as the Scale in Fractional Kelly Position Sizing》(arXiv:2608.01494v1, 2026-08-02, ACS Athens) | **保形预测区间宽度作为 fractional Kelly 的 σ**——`f* ≈ μ/σ²` 中 σ 取自 75% 保形区间半宽。开发窗口年化净对数增长 28.5%、Sharpe 1.34、MaxDD 27.7%；风险控制层 MaxDD 27.7%→20.3%。**关键设计原则**：最简单的慢/无权重/逐资产 rolling 保形分位数胜过所有快速自适应方案。**样本外警示**：2022+ 校准保持（0.745 vs 0.750 目标）但增长未保持（8.5%/7.0% 低于被动基准）。**直接支撑 §4.5 Conformal Kelly 远期精炼登记**——不推翻 Kelly 拒绝（仍是 Kelly 族），登记为 [31号] Phase 4+ 单策略仓位尺寸远期精炼路径 |
| marketmaker.cc《Objective-Function Design: The Metric You Optimize Secretly Picks Your Strategy》(2026-06-28，含配套论文+开源代码） | **目标函数口径受控实验**——600 种子、已知 ground truth 合成市场：按全时间线计算的 Sharpe 永不退化（OOS 1.71），按"仅交易 bar"per-trade 口径 57% 概率选出退化策略；`conf_k = n/(n+k)` 交易数收缩 + exposure floor 的鲁棒目标可追平全时间线 Sharpe（1.70）且退化率仅 0.2%。与 34号 的关联：① **印证 PerformanceScore 全时间线口径纪律**——我们 60 日 Sortino 按策略每日收益序列计算（含未持仓日收益 0），不踩 per-trade 口径陷阱；② `conf_k = n/(n+k)` 连续置信收缩登记为 ConfidenceSignal 四档阶梯的远期替代形状（§6 待裁定） |
| Whelan《Kelly Strategies and the Return on Capital Deployed》(UCD, 2026-05) | **fractional Kelly 单位资本回报新证**——二阶近似重证 Ethier–Tavaré：full Kelly 单位投放资本回报仅约均值一半，半 Kelly 3µ/4、1/4 Kelly 7µ/8。与 34号 的关联：为 Shrinkage ≤1.0 只减不增的折扣哲学提供 2026 学术背书——折扣不是损失，按投放资本口径反而更优（与 §3.2.3 Quarter Kelly 同构印证互补） |
| arXiv:2603.21330《FinRL-X》(2026-03-22) + Candriam Alternative Multi-Strategies 转型（AOF/Boursorama 2026-03-23 + 官网 Q&A 2026-04-16） | FinRL-X：分层权重中心架构，组合级**风险覆盖层独立于信号层**做成可组合管道——印证我们 regime Shrinkage 正是独立于策略层的风险覆盖层（§3.1 解耦设计），RL 分配器不采纳（§4.4）；Candriam：机构级多策略基金按 risk-on/risk-off regime 主动动态调整各策略资金权重 + 单一交易账簿统一管理——行业实践侧印证"regime 驱动策略间资金再分配"是 2026 机构在用范式，我们用三因子乘法 O(N) 实现同目标 |
| cluttmann/multi_strategy_portfolio（GitHub 个人实盘，2026-05~08 持续提交） | **反例弱证据（诚实登记）**——真实运行的个人 7-sleeve 组合采用固定目标权重 + 月度再平衡，2026-05 退役其 "Regime World" 策略（无业绩归因披露）。与 34号 的关联：个人系统简化到固定权重的反例——但我们不据此改 MVP：① 无业绩对比无法判断 regime 失效还是实现问题；② 我们 C1 已实证 Shrinkage 有效（MaxDD 改善 7.36pp）；③ 我们第一阶段本就是固定 budget（§4.3），RegimeMetaAllocator 是第二阶段可选增强。登记为"regime 模块个人系统适配性"的持续观察项 |
