...
> 代码映射：[regime_meta_allocator.py](../../../../src/zephyr/pf_alloc/core/regime_meta_allocator.py) MOD-PA-007（**v1.0.0 production**，MATURITY=production），`FLOOR=0.05` / `CAP=0.40`，`allocate()` / `_compute_shrinkage()` / `_compute_confidence_signal()` / `_compute_risk_signal()` / `_compute_raw_allocation()` / `_normalize_and_clip()` / `compute_performance_score()`（静态方法，供上游计算 Sortino→[0.5,1.5]）。55 测试用例曾全绿（⚠️ **2026-08-11 git 灾难**：`tests/pf_alloc/test_regime_meta_allocator.py` 从未提交、被 `git clean -fd` 删除且 git 历史无记录，**测试套件待重建**——见 §3.5 已施工设施盘点 + §6 待裁定；代码本体已提交幸存，commit 81c7687540）。
...
- **更新频率**：effective_budget 日频重算（PerformanceScore 60 日滚动日频更新 + Shrinkage 日频更新）；盘中 regime 突变（D-SIGNAL-68 overlay 触发）时盘中重算 Shrinkage → effective_budget。日频更新意味着 budget 可能日频变动，这正是 [33号](33_budget_change_handler.md) §3.3 防抖双层（2026-08-12 重建后锚点已恢复）的必要性来源
...
> 全面扫描项目代码/配置/测试/蓝图/上下游配套，盘点与本备忘（G15 RegimeMetaAllocator）直接相关的已建设施。目的：清楚有什么 → 才知道怎么改 → 才知道该删除/退役什么。扫描基线：2026-08-12 工作树 + git 历史取证。
...
| marketmaker.cc《Objective-Function Design: The Metric You Optimize Secretly Picks Your Strategy》(2026-06-28，含配套论文+开源代码） | **目标函数口径受控实验**——600 种子、已知 ground truth 合成市场：按全时间线计算的 Sharpe 永不退化（OOS 1.71），按"仅交易 bar"per-trade 口径 57% 概率选出退化策略；`conf_k = n/(n+k)` 交易数收缩 + exposure floor 的鲁棒目标可追平全时间线 Sharpe（1.70）且退化率仅 0.2%。与 34号 的关联：① **印证 PerformanceScore 全时间线口径纪律**——我们 60 日 Sortino 按策略每日收益序列计算（含未持仓日收益 0），不踩 per-trade 口径陷阱；② `conf_k = n/(n+k)` 连续置信收缩登记为 ConfidenceSignal 四档阶梯的远期替代形状（§6 待裁定）——四档悬崖型时首选 |
| Whelan《Kelly Strategies and the Return on Capital Deployed》(UCD, 2026-05) | **fractional Kelly 单位资本回报新证**——二阶近似重证 Ethier–Tavaré：full Kelly 单位投放资本回报仅约均值一半，半 Kelly 3µ/4、1/4 Kelly 7µ/8；过度自信进一步降低实际回报。与 34号 的关联：为 Shrinkage ≤1.0 只减不增的折扣哲学提供 2026 学术背书——折扣不是损失，按投放资本口径反而更优（与 §3.2.3 Quarter Kelly 同构印证互补） |
| arXiv:2603.21330《FinRL-X: An AI-Native Modular Infrastructure for Quantitative Trading》(2026-03-22) | **分层权重中心架构**——数据/策略/回测/执行分层 + 统一接口，组合级**风险覆盖层独立于信号层**做成可组合管道，支持 RL 分配器与 LLM 信号且不改变下游执行语义。与 34号 的关联：印证"组合级风险覆盖层独立于信号层"架构——我们 regime Shrinkage 正是独立于策略层的风险覆盖层（§3.1 解耦设计），RL 分配器部分不采纳（§4.4 已拒绝） |
| Candriam Alternative Multi-Strategies 转型（AOF/Boursorama 2026-03-23 + 官网 Q&A 2026-04-16） | **机构级多策略基金 regime 动态分配实践**——三策略族（方向性 L/S、市场中性、Upside Alpha）+ 按 risk-on/risk-off regime 主动动态调整各策略资金权重 + 单一交易账簿统一管理风险流动性。与 34号 的关联：行业实践侧印证"regime 驱动策略间资金再分配"是 2026 机构在用范式（他们用量化+主观，未披露公式）；我们用三因子乘法 O(N) 实现同目标 |
