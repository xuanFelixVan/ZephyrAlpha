...
| 表名 | 内容 | 潜在消费场景 | 当前状态 |
|---|---|---|---|
| `block_trade_detail`（大宗交易明细） | 个股大宗交易逐笔明细（成交价/成交量/折溢价率/买卖营业部） | 机构折价大宗信号：大宗买入成交价 ≥ 当日 ask（卖一价）→ 短期正向（[2026 实证](https://equity-insider.com/behind-the-big-moves-how-block-trade-alerts-are-reshaping-equity-markets/)）；深折价大宗→抛压预警，与 §3.13#4 持仓监控/§3.14 微结构监控联动；候选输入席位过滤（§3.10 龙虎榜量化席位双阈值同源营业部数据） | **未消费登记**（2026-08-20 实证：src/zephyr 引用 4 次，代码活跃；消费语义未落本文档） |

> 关联挂起：`auction_book`（集合竞价委托簿，63 号批次 C 标注"配合 block_trade_detail"）随本表一并挂起登记——2026-08-20 实证 src/zephyr 引用 4 次；竞价三维（§3.9）现用盘口快照口径，后续接入时与大宗信号同批补 §7.0.1 六字段小节。
