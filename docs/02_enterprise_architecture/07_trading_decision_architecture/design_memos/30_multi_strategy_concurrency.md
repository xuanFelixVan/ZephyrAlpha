...
>
> **选项外更优算法：Bayesian Kelly Criterion（2026-08-10 算法审查补充）**：固定 Fractional Kelly 25-50% 是"一刀切"收缩——大样本时过度保守（牺牲 alpha），小样本时仍嫌激进（edge 估计噪声大）。Sukhov 2026-06 提出 **Bayesian Kelly with Parameter Uncertainty**：用后验加权替代 plug-in Kelly 分数，收缩系数随有效样本量自适应——
>
> ```
> f* = (p̄ − (1−p̄)/b) · n_eff/(n_eff + κ)
> ```
>
> 其中 p̄=α/(α+β) 是 Beta 共轭先验下的后验均值（胜率估计），n_eff=α+β 是有效样本量，κ 是正则化强度。**关键性质**：n_eff 小（样本少）→ n_eff/(n_eff+κ) → 0，自动深收缩；n_eff 大（样本多）→ n_eff/(n_eff+κ) → 1，自动放宽到 Full Kelly。这比固定 25-50% 更 principled——直接解决"edge 估计含误差"问题，无需手动调 fraction。
>
> | 方案 | 收缩机制 | 样本自适应 | 实现复杂度 | 推荐 |
> |---|---|---|---|---|
> | **固定 Fractional Kelly 25-50%** | 固定比例 | ❌ | 最简 | Phase 1（当前，[31 号](31_position_sizing.md) 已用半 Kelly 硬上限） |
> | **Bayesian Kelly** | n_eff/(n_eff+κ) 后验收缩 | ✅ 小样本深收缩/大样本放宽 | 中（需维护 Beta 先验+后验更新） | **Phase 2 演进**（首批策略 3-6 月 PnL 后，有足够样本标定 κ） |
>
> **定位**：Bayesian Kelly 不是替代 Fractional Kelly，而是其**自适应演进**——Phase 1 用固定 fraction（简单可控），Phase 2 数据积累后升级为 Bayesian Kelly（样本自适应）。两者都遵循"风险优先"原则，Bayesian Kelly 只是在"收缩多少"上更精细。详见 [31 仓位算法](31_position_sizing.md) G12 讨论。
>
> **选项外更优算法：Conformal Kelly（2026-08-10 四次算法审查补充）**：Bayesian Kelly 仍依赖"胜率 p + 赔率 b"的点估计（虽用后验收缩），而 Conformal Kelly 直接从**预测区间宽度**推导缩放因子——绕过 p/b 估计难题。[arXiv:2608.01494v1（Ryan 2026-08-02）](https://arxiv.org/html/2608.01494v1)提出用 Conformal Prediction（保形预测）的 75% 区间宽度作为 Fractional Kelly 的 scale：区间变宽→预测不确定→收缩仓位；区间变窄→预测确定→放大仓位。
>
> ```
> f_conformal = f_kelly × (w_ref / w_t)
> ```
>
> 其中 w_t 是当前 conformal 区间宽度，w_ref 是基准宽度（如历史中位数）。**核心实证**：6 年开发窗口（2016-2021）含交易成本+1 日执行延迟+严格杠杆上限，年化净 log 增长 28.5%、Sharpe 1.34、MaxDD 27.7%（vs S&P 500 15.9%、同杠杆被动 21-22%）；区间命中率 74.8%（目标 75%）。
>
> **反直觉发现（关键，印证 §2.2 regime 哲学）**：让区间更快适应当前市场条件/regime 的每次调整都会**损失 0.7-5.3 个百分点的年增长**——最优是最简单的 slow/unweighted/per-asset rolling conformal quantiles。这印证了"regime 做风险节流不做 alpha 择时"（§2.2）——仓位缩放不需要快速适应当前 regime，**稳定性比局部锐度更重要**。
>
> **回撤控制集成（drawdown dial）**：当 conformal 区间在下行方向 miss 远超历史比率时，视为模型失效信号，削减杠杆——开发窗口把 MaxDD 从 27.7% 降到 20.3% 同时**提升** Sharpe，timing 击败所有 40 个 placebo 版本（rank-based p=1/41≈0.024）。这与 §2.5 四级回撤 Protocol 思路同源——都是"回撤驱动风险节流"，但 Conformal Kelly 用连续 conformal miss 率替代离散阈值。
>
> **诚实性披露**：Lockbox（2022+ OOS）calibration 保持（0.745 vs 0.750 目标），但**增长未保持**（8.5% 和 7.0%/年，低于被动基准）——说明方法对开发窗口敏感，实盘需谨慎。预注册+honesty ledger 设计值得本项目借鉴。
>
> | 方案 | 缩放机制 | 依赖估计 | 回撤集成 | OOS 诚实性 | 推荐 |
> |---|---|---|---|---|---|
> | **固定 Fractional Kelly 25-50%** | 固定比例 | 胜率/赔率 | ❌ 独立 | — | Phase 1（当前） |
> | **Bayesian Kelly** | n_eff/(n_eff+κ) 后验收缩 | Beta 先验+胜率 | ❌ 独立 | — | Phase 2 演进 |
> | **Conformal Kelly** | conformal 区间宽度比 | 预测模型（不需 p/b） | ✅ drawdown dial | ✅ Lockbox 披露 | **Phase 3 远期候选** |
>
> **为何列为远期而非 Phase 2**：① Conformal Kelly 需可靠的预测模型产出 return 区间（本项目当前 alpha 信号输出的是仓位/target_portfolio，非 return 分布），需先建预测层；② Lockbox OOS 增长未保持说明方法敏感，需充分 OOS 验证；③ Bayesian Kelly 已解决"样本自适应"核心问题，Conformal Kelly 的边际收益在"绕过 p/b 估计+回撤集成"上，属精细化演进。**重评条件**：Phase 2 Bayesian Kelly 跑 6+ 月后，若 p/b 估计噪声导致 Kelly 分数不稳定，则评估 Conformal Kelly 作为替代。详见 [31 仓位算法](31_position_sizing.md) G12 讨论。
...
>
> **⚠️ 字段名三方漂移（2026-08-12 代码核对，v2.6.0 补）**：代码真源 `TargetPortfolio`（strategy_book.py L102-119）字段为 **`positions: dict[str, TargetWeight]` + `budget`**（非本文档伪代码的 `target_portfolio` / `budget_used`，且 `positions` 值是 `TargetWeight` 对象非裸 float）；`firm_risk_aggregator.py._sum_by_symbol` 按 `target_portfolio`/`budget_used` duck-typing 取值——**直接传入 TargetPortfolio 对象会静默取空默认值得出全现金组合**（断裂不报错）。权重口径声明本身已在代码落实（`TargetWeight` docstring L92 明示"相对 strategy_budget 的占比"），但字段名对齐属 P0 接口修复项，登记 [32号 §6](32_firm_risk_aggregator.md) 开放问题
>
...
> **多策略组合预期 Sharpe 基准（2026-08-10 行业实证补充）**：A 模型"加法替代优化器"的预期收益可从 2026 系统化股权交易全景校准。[vzeman/trading-autoresearch (2026-05)](https://github.com/vzeman/trading-autoresearch/blob/main/systematic_equity_trading_research.md) 综合 ~40 篇一手来源（Jegadeesh & Titman 2023 / Hou-Xue-Zhang 2020 / Daniel & Moskowitz 2016 / Frazzini-Israel-Moskowitz 2018 / AQR / Robeco / Two Sigma / Man AHL / Alpha Architect / Research Affiliates）给出 2026 equity systematic trading 基准：
> - **Cross-sectional momentum (12-1)**：gross Sharpe ~0.85，net ~0.65 after costs（月度换手 ~140%/yr 导致 20-30% 净值折扣）；volatility-scaled（Barroso & Santa-Clara）风险调整收益**翻倍**
> - **Time-series momentum / dual momentum**（index/ETF level）：net Sharpe 0.6-0.8
> - **警示**：学术回测 headline figures 在实盘应打 20-50% 折扣
>
> **校准意义**：Model A 多策略组合（打板/多因子/事件驱动）的预期 net Sharpe 基准在 **0.6-0.85** 区间——"加法替代优化器"在不引入协方差估计误差下的合理上限。A 股 T+1+涨跌停+不能做空使预期较美股全景略低，但 volatility-scaled（§2.1 Fractional Kelly + §2.2 Shrinkage）的"风险调整收益翻倍"效应仍适用。实盘 net Sharpe <0.5 提示策略间相关性过高（§6.2 必做验证）或单策略 alpha 不足；>1.0 需警惕过拟合。与 [Morwane OOS 2013-2026](§7.4) risk-throttle Sharpe 1.43（两 sleeve 弱相关 ρ=+0.03）形成对照——弱相关 sleeve 组合可达 Sharpe >1，但那需 sleeve 数更多且相关性极低，本项目 3 策略起步阶段以 0.6-0.85 为合理预期。

> **A 股 2026 上半年量化超额衰减警示（2026-08-10 市场环境校准补充）**：[新浪财经 2026-07-11](https://finance.sina.com.cn/jjxw/2026-07-11/doc-inihmkxc5002361.shtml) 报道，2026 年上半年 A 股股票量化多头产品平均收益 16.25%（与 2025 同期 17.32% 持平），但**平均超额收益从 14.17% 骤降至 3.11%**——收益主要来自 Beta，Alpha 获取难度显著提升。三大根因：① **K 型行情压制分散持仓**——4-5 成成交额集中在前 5% 热门科技股，量化指增的"广泛撒网+高度分散"策略大量非热点中小盘滞涨甚至下跌；② **因子失效+策略同质化**——动量因子虽强但反转/均值回归/小市值因子反向，机构共用相似价量因子框架使超额被海量资金摊薄；③ **风格错配**——资金高度偏好科技成长与动量，低估值低波因子弱势，缺乏明确主线轮动时量化多头难跑赢紧抱科技龙头的宽基指增。**对本项目的校准意义**：A 股 2026 环境下"纯横截面多因子 alpha"获取难度飙升（中证 500 指增上半年平均超额仅 0.85%），印证 [34 §4.1](34_regime_meta_allocator.md) "regime 做 alpha 择时降 Sharpe"的风险——量化超额衰减期做 alpha 择时更易亏损。Model A 的"风险节流不做 alpha 择时"+ §6.2 相关性验证 + [28 情绪周期](28_sentiment_cycle_trading.md) sleeve 内择时 的组合架构在此环境下更稳健：多因子 sleeve 的超额预期应从"美股全景 0.6-0.85 net Sharpe"**下调**（A 股 2026 环境 alpha 衰减），但打板 sleeve 的情绪周期择时在连板回暖期仍可获超额（与横截面 alpha 正交）。**实盘预期管理**：首批 3 策略实盘 6 个月后，若组合 net Sharpe <0.3（低于 0.6-0.85 基准的一半），优先排查策略间相关性（§6.2）而非单策略 alpha 不足——K 型行情下"相关性飙升"比"alpha 不足"更可能是根因。

> **叠加超限裁剪算法（2026-08-10 补充）**：自然叠加后若超过 firm 层单票硬上限（如 8%），须裁剪。裁剪算法（FirmRiskAggregator `MOD-POS-021` 待实现）：
>
> | 裁剪方式 | 算法 | 优点 | 缺点 | 适用场景 |
> |---|---|---|---|---|
...
>
> **选项外更优算法：Clipped Water-Filling（2026-08-10 算法审查补充）**：pro-rata 裁剪"所有策略同比例缩减"在 minimax 意义下并非最优。arXiv:2603.26893（2026-03）证明 **Water-Filling（注水算法）在在线可分资源分配中普遍 minimax 最优**——对任何 Schur-单调目标函数最优；arXiv:2603.15963（2026-03）在多资产交叉保证金场景提出 Clipped Water-Filling，实证优于 pro-rata。直觉：把预算"注"到各策略直到水位齐平（max-min fairness），而非按比例削。
>
> | 裁剪方式 | minimax 最优性 | 与 A 模型哲学一致性 | 复杂度 | 裁定 |
> |---|---|---|---|---|
> | **pro-rata 按比例缩放** | 次优 | ✅ 完全一致（不偏袒） | O(N) | **Phase 1 首选**（当前裁定） |
> | **Clipped Water-Filling** | ✅ minimax 最优 | ⚠️ 部分——注水"填平"会优先满足小仓位策略完整保留，大仓位策略多承担裁剪，隐含"小仓位优先"倾向 | O(N log N) | **Phase 2 候选**（若 pro-rata 显示信号失真） |
>
> **为何 Phase 1 仍选 pro-rata**：Water-Filling 的 minimax 最优性在"资源不足"场景成立，但 A 模型叠加超限是"轻微超限"（如 8% 上限超出 1-2%），此时 pro-rata 与 Water-Filling 差异极小；且 pro-rata 与 A 模型"不偏袒"哲学完全一致，Water-Filling 的"填平"隐含优先级。**重评条件**：实盘跑后若 pro-rata 裁剪导致高信心策略被过度缩减、归因显示"裁剪失真"，则升级到 Water-Filling。
...
> ```
>
> **为何 ε_pos=5%**：A 股 T+1 使策略无法当日即时调仓（买入需次日才能卖），5% 容差给策略 1-2 日的自然调仓空间（打板高换手 1-2 天自然收敛，[20 §6.4](20_first_batch_strategies.md)）。容差太小（如 1%）→ Tier 3 频繁误触发，强裁成本高；容差太大（如 15%）→ budget 下调形同虚设。5% 是"足够松让 T+1 自然收敛，足够紧让 budget 约束有效"的平衡点，待首批策略实盘后校准。
>
...
> **施工状态与 why 补全（2026-08-10 核对源码）**：本 Protocol 相关模块**已先于 §2.2/§2.4 三模块施工至 production**——`drawdown_controller.py`（MOD-POS-008，488 行，production）、`drawdown_tracker.py`（production）、`var_calculator.py`（production）、`kill_switch.py`（production）。为何回撤风控先于 alpha 账本施工（why）：用户原则"风险优先于收益"——生存底线是 alpha 迭代的前提，账户爆掉再好的选股也无意义；且回撤风控不依赖 StrategyBook 选股逻辑，可独立先建。**为何用四级硬阈值而非软优化（why）**：硬阈值=fail-closed 生存底线，软优化=fail-open 风险（优化器可能"优化"掉保护）；四级 8/15/20/25% 是行业基准（LedgerMind/ARKA/Sina FOF 2026 共识），非自创。
>
...
> | 50% | 50% | +100% | （不可承受——需翻倍） |
>
> 来源：tradingengineeringlab 2026-06 / astuteinvestorscalculus 2026-06 / metatronics 2026-03 三源一致。**为何阈值定在 25% 而非 30%/40%**：25% 需 +33% 恢复（按 10% 年化约 3 年），尚可承受；30% 需 +43%（约 4 年），50% 需 +100%（约 7 年）——恢复成本呈凸性增长，25% 是"恢复尚可行"与"防止灾难性损失"的临界点。Level 4 后强制休息 5 天正是给"恢复 +33%"留心理空间，避免情绪化加码。
...
**总体裁定**：回撤风控已先于 alpha 账本施工至 production，方向正确（生存优先）。但 production 不等于"定稿"——5 级 VaR 节流 + 7 黑天鹅模式是从机构级框架继承的完整集，对个人项目存在"建了未必用"的过度风险。遵循 charter 约束五"少而精"+ project_memory"56 条硬边界砍到 10 条真红线"原则，建议在 G13/G14 风控细节讨论中：① Kill Switch + 四级回撤 = 真红线（mandatory）；② VaR 5 级 + 7 黑天鹅 = 监控层（先全建+全 log，实盘验证后裁剪未触发项）。这与 [Trium Capital 2026-06](https://wealthdfm.com/viewpoint-not-all-multi-strategy-funds-are-built-the-same/) 的"止损是 review 信号"形成对照——个人系统取 pod 的硬纪律（防 AI-dev 软弱），但用"监控 vs 红线"分层避免机构级全量自动化的维护负担。
...
- **远期（可选，非当前 scope）**：Relaxed Risk Parity——从三因子乘法升级到松弛风险预算。
  > **2026 新实证**：[ericxuzhesheng/Relaxed-Risk-Parity-Research](https://github.com/ericxuzhesheng/Relaxed-Risk-Parity-Research)（2026-08-07 更新，410 commits）提出 Relaxed Risk Budgeting——软化严格等风险贡献约束 + 凸自适应重构（改进求解器稳定性）+ CVaR 约束（控制尾部损失）+ Turnover penalty（内化交易成本）。这是"三因子乘法（当前）→ MVO（已否决）"之间的中间态：比三因子乘法更精细（考虑策略间协方差结构），比 MVO 更稳定（不直接反转协方差矩阵，用松弛约束+凸优化）。
  >
  > **为何列为远期而非第二阶段**：① 当前三因子乘法 O(N) 已满足 3-5 策略需求，Relaxed Risk Parity 的边际收益在策略数少时有限；② 松弛约束+凸优化需引入 cvxpy 等求解器依赖，增加复杂度；③ CVaR 约束需策略 PnL 尾部分布估计，需更长 track record。**重评条件**：策略数 >5 且三因子乘法显示"等权 risk contribution 假设不成立"（如某策略波动率显著高于其他策略但 PerformanceScore 未充分调整）。
  >
  > **远期演进选项 B：MPC 多期预测（2026-08-10 算法审查补充）**：[Nystrup-Boyd-Lindström-Madsen (Annals of Operations Research 282(1):245-271, 2019; [Stanford Boyd page 2026-06 维护](https://stanford.edu/~boyd/papers/multiperiod_portfolio_drawdown.html))] 提出 MPC（Model Predictive Control）+ HMM 多期预测的组合优化框架——基于多变量 HMM（时变参数）的多期均值/协方差预测，**根据已实现回撤动态调整风险厌恶**（realized drawdown → risk aversion adjustment），成功控制回撤且几乎不牺牲 mean-variance efficiency；交易/持仓成本作为估计误差处理 + 正则化优化问题。
  >
  > **为何与 Model A "加法替代优化器"哲学一致**：MPC 的核心是"根据已实现回撤调整风险厌恶"——这与 Model A 的 §2.5 Drawdown Protocol（四级回撤阈值触发减仓/停仓/清仓）思路**同源**，都是"回撤驱动风险节流"。差别在于 MPC 用连续优化器（需协方差预测），Model A 用离散硬阈值（O(N) 加法）。MPC 可视为 Model A 在"有可靠多期预测"时的连续化演进——与 §2.5.7 CDaR 离线度量、§4.2 Relaxed Risk Parity 同属"加法替代优化器的精细化演进谱系"。
  >
  > **为何列为远期而非第二阶段**：① MPC 需多变量 HMM 的多期均值/协方差预测，这是 [10号](10_regime_detector_spec.md) regime 检测器的扩展（当前 4 态 HMM 仅做 regime 识别，不做多期收益预测）；② 连续优化器引入协方差估计（§3.1 否决 MVO 的理由），需先验证 HMM 预测稳定性；③ Model A 的离散硬阈值 + Shrinkage 已满足当前需求。**重评条件**：[10号] regime 检测器升级到多期预测 + 首批策略 PnL 验证四级阈值有效性后，可评估 MPC 作为连续化演进。

### 4.3 为何这是上限而非妥协
- ~~Citadel/Millennium 的 pod 模型本质就是 A（独立账本 + firm 风险聚合），**没有统一 MVO**~~
  > **修正（2026-08-10，[20 §1.4/§5 待裁定-2](20_first_batch_strategies.md) 裁定）**：原表述是误标。Model A **不是** Citadel pod 式，是 **Morwane 式"统一风险框架（firm 求和+裁剪+Kelly）+ 独立 alpha sleeve + regime 风险节流"**。Citadel pod = 几十个互不相关独立 PM + 被动风险聚合 + PM 间不共享不协同；Model A = 统一 firm 层 + 统一 StrategyBook 接口 + 统一信号工厂（G05）+ 少而精（3-5 个）差异化 sleeve。**2026 实证支撑**：
  > - [resonanzcapital 2026-04](https://resonanzcapital.com/insights/systematic-multi-strategy-vs.-discretionary-pod-platforms-different-engines-different-risk)：明确区分"systematic multi-strategy"（中央设计信号库+统一风险框架，判断内嵌系统）vs"discretionary pod"（独立 PM 各自为政）。**Model A 正是前者**——与 charter §3 约束二"统一框架派"一致。
  > - [Trium Capital 2026-06](https://wealthdfm.com/viewpoint-not-all-multi-strategy-funds-are-built-the-same/)："Not all multi-strategy funds are built the same"——pod 平台=风险主导+止损即自动砍仓；传统多策略=收益主导+止损是 review 信号+回撤中可加仓。Model A 的"统一框架+少而精"属传统多策略谱系，非 pod 平台。
  > - [algoalpha 2026-06](https://www.algoalpha.co/join/blog/multi-strategy-pod-shops-explained)：个人投资者可借鉴的是 pod 模型的**过程**（分散下注、先定风险再定仓位、保持系统化），**非规模**——正好对应 Model A 借鉴 pod 的风险纪律但不上 pod 的组织复杂度。
  > - [RMATS (arXiv:2605.25311, 2026-05, APAM 2026)](https://arxiv.org/pdf/2605.25311)：Recursive Multi-Agent Trading System——4 个专门 agent（Sentiment / Report / Analysis / Risk）+ 递归 Manager Agent（typed message passing + 收敛保证 ‖w^(r+1)−w^(r)‖₂<ε）。实证（561 交易日 / 24 资产多类别）：MaxDD **9.62%** < MVO 15.49% < FinBERT 15.28%，5 个地缘政治压力场景中 3 个事件期回撤最低；消融实验确认每个 agent 组件贡献。**关键印证**：RMATS 的独立 Risk Agent（CVaR 估计 + 地缘压力测试 + 自适应断路器 circuit breaker）与 Model A 的 FirmRiskAggregator（独立 firm 风险层）思路一致——**将风险控制从 alpha 生成中解耦为独立层**，验证 Model A 分层设计正确性。Analysis Agent 用 HMM regime 分类 + Kalman 滤波融合，与 [10号](10_regime_detector_spec.md) regime 检测器 + [34号](34_regime_meta_allocator.md) Shrinkage 节流呼应。**个人项目不采用多 agent 递归协调**（§5 已登记 LLM 多 Agent 辩论为暂缓项——AI 写 AI 的失控风险高 + 投入产出比对个人系统极低），但 RMATS 的"独立 Risk Agent"架构印证了 Model A 的 FirmRiskAggregator 独立分层——A 模型用更简的方式（firm 层求和+裁剪+硬阈值，非递归多 agent）实现了同等的"风险解耦"目标。
...
>
> **v1.4.0 处置说明**：[20 §5](20_first_batch_strategies.md) 登记的 4 项与本备忘相关的待裁定，已在本版处置：待裁定-2（§4.3 pod 误标）✅ 已修正；待裁定-3（§1.1 5 候选清单同步）✅ 已修正；待裁定-4（情绪周期 vs regime 边界）✅ 已由 [28 §3.1](28_sentiment_cycle_trading.md) 定型；待裁定-1（charter §3 措辞修订）⏳ 属 04 域 owner，非本备忘范围。下方 6 项为本备忘原生暂缓项，状态未变。
...
**审查发现**：§2.2 RegimeMetaAllocator 提到"新策略冷启动只用 Base_i（先验权重）"，但**未明确定义新 StrategyBook 上线时的初始执行比例**（如 ×30% 初始执行，逐步放大到 full size）。[25 §3.5/§4.1](25_multifactor_strategy_detail.md) 因子治理有灰度阶梯 10%→30%→100%（因子级），但**策略级冷启动执行比例在本备忘中缺失**。

**行业标准**（[quanthedgeai 2026-07](https://www.quanthedgeai.com/blog/implementing-a-multi-strategy-portfolio-end-to-end/)）：research candidate → paper portfolio（6 月，PnL 偏离回测预期 >30% 则调查）→ half-sized live（6 月，rolling DSR 确认信号）→ full size。即"先纸面、再半仓、后满仓"的三段式渐进上线。
...
**与因子治理灰度的差异**：因子级灰度（10%→30%→100%）是单因子权重的渐进暴露；策略级冷启动是整个 StrategyBook 资金预算的渐进放大——周期更长（策略 PnL 波动远大于因子 IC），且需与 §2.4 BudgetChangeHandler 三级升级联动（冷启动期 budget 上调用 Tier 1+2 给时间，不触发 Tier 3 强裁）。

**为何缺失需补**：冷启动期是新策略最脆弱阶段（无 track record、PerformanceScore 无效、Base_i 等权可能高估未验证策略的 budget）。无明确执行比例 → 要么过度暴露（新策略上线即满仓，若 alpha 不成立则亏损），要么过度保守（长期 ×10% 使策略永远无法积累足够 PnL 验证）。×30% 起步是"足够小让单策略故障不致命，足够大让 PnL 信号可观测"的平衡点。需首批策略确定后校准。

**裁定结论（v2.1.0 定型，2026-08-10 第三十七轮审查）**：

经三十七轮施工算法完整性审查，§6.7 是 30 号文档**唯一"设计待定"项**——初拟方案已有完整三段式 + 晋升条件 + 行业标准支撑 + 与 BudgetChangeHandler 联动设计，符合风险优先原则（新策略最脆弱阶段用 30% 起步确保单策略故障不致命）。裁定如下：

1. **MVP 基线采纳**：三段式 ×30%（1 月）→ ×60%（1 月）→ ×100%，晋升条件如表所述（无 firm 风险违例 + PnL 不偏离回测预期 >30% + PerformanceScore 稳定）。**与 53 号模拟实盘迁移路径 PARALLEL→SHADOW→GRAY_RAMP 同构**——冷启动是策略级的灰度迁移，53 号是系统级的灰度迁移，两者共用"渐进放大"哲学
2. **参数待 C1 校准**：×30%/×60%/100% 为 MVP 基线参数，待首批策略确定后按实盘 PnL 波动率校准（若策略 Sharpe >1.5 且 MaxDD <5% 可加速到 ×50%→×100% 两段式；若 Sharpe <0.8 则延长冷启动期至 2 月）
3. **与 §2.4 BudgetChangeHandler 联动**：冷启动期 budget 上调用 Tier 1+2 给时间（自平衡），不触发 Tier 3 强裁——冷启动策略的 budget 预算本身就是 ×30%/×60% 缩放后的，Tier 3 强裁门槛相应缩放
4. **与 25 号因子治理灰度的区分**：因子级灰度（10%→30%→100%）是单因子权重的渐进暴露（周期短，因子 IC 稳定快）；策略级冷启动是整个 StrategyBook 资金预算的渐进放大（周期长，策略 PnL 波动远大于因子 IC）——两者正交，不冲突
5. **风险红线**：冷启动期 Kill Switch 仍全效（Kill Switch 是系统级非策略级，不受冷启动缩放影响），确保新策略故障在 30% 暴露度下即可被 Kill Switch 截断
...
- [vzeman/trading-autoresearch — Systematic Equity Trading: What Works in 2026 (2026-05)](https://github.com/vzeman/trading-autoresearch/blob/main/systematic_equity_trading_research.md)：综合 ~40 篇一手来源（Jegadeesh & Titman 2023 / Hou-Xue-Zhang 2020 / Daniel & Moskowitz 2016 / Frazzini-Israel-Moskowitz 2018 / AQR / Robeco / Two Sigma / Man AHL / Alpha Architect / Research Affiliates / Carver / López de Prado / Antonacci / Gray）的 2026 equity systematic trading 全景。cross-sectional momentum (12-1) gross Sharpe ~0.85 / net ~0.65 after costs（月度换手 ~140%/yr），volatility-scaled（Barroso & Santa-Clara）风险调整收益翻倍；time-series momentum / dual momentum at index/ETF level net Sharpe 0.6-0.8；学术回测 headline figures 实盘应打 20-50% 折扣。**支撑 §2.3 多策略组合预期 Sharpe 基准 0.6-0.85**
- [Nystrup-Boyd-Lindström-Madsen — Multi-Period Portfolio Selection with Drawdown Control (Annals of Operations Research 282(1):245-271, 2019; [Stanford Boyd page 2026-06 维护](https://stanford.edu/~boyd/papers/multiperiod_portfolio_drawdown.html))]：MPC（Model Predictive Control）+ 多变量 HMM（时变参数）多期预测，根据已实现回撤动态调整风险厌恶，控制回撤且几乎不牺牲 mean-variance efficiency；交易/持仓成本作为估计误差处理 + 正则化。**支撑 §4.2 MPC 远期演进——与 Model A 回撤驱动风险节流（§2.5 Drawdown Protocol）同源**
- [Yang et al. — RMATS: Recursive Multi-Agent Trading System (arXiv:2605.25311, 2026-05, APAM 2026)](https://arxiv.org/pdf/2605.25311)：4 agent（Sentiment/Report/Analysis/Risk）+ 递归 Manager（typed message passing + 收敛保证），Analysis Agent 用 HMM regime 分类 + Kalman 滤波，Risk Agent 用 CVaR + 地缘压力测试 + 自适应断路器。561 交易日/24 资产实证 MaxDD 9.62% < MVO 15.49% < FinBERT 15.28%，5 个地缘政治压力场景中 3 个事件期回撤最低，消融实验确认每个 agent 组件贡献。**支撑 §4.3 Model A FirmRiskAggregator 独立分层设计正确性（独立 Risk Agent 印证）**
...
- [Conformal Kelly: Conformal Prediction Intervals as the Scale in Fractional Kelly Position Sizing (arXiv:2608.01494v1, 2026-08-02)](https://arxiv.org/html/2608.01494v1)：用 conformal prediction 75% 区间宽度作为 Fractional Kelly 缩放因子——区间变宽收缩仓位、变窄放大仓位。6 年开发窗口（2016-2021）含成本+1 日延迟+杠杆上限，年化净 log 增长 28.5%/Sharpe 1.34/MaxDD 27.7%（vs S&P 15.9%）。**关键反直觉发现**：让区间更快适应当前 regime 每次调整损失 0.7-5.3pp 年增长，最优是最简单的 slow/unweighted conformal quantiles——印证 §2.2 "regime 做风险节流不做 alpha 择时"。**drawdown dial**：conformal 下行 miss 率超历史→削杠杆，MaxDD 27.7%→20.3% 同时提升 Sharpe（p=1/41≈0.024）。Lockbox OOS calibration 保持（0.745 vs 0.750）但增长未保持（8.5%/7.0%）。**支撑 §2.1 Conformal Kelly 为 Bayesian Kelly 的 Phase 3 远期候选——绕过 p/b 估计+回撤集成+OOS 诚实性**
