...
| **治理批 A 包（2026-08-17 Owner 裁定派单，与 RFIX-001 零交集实证）** | Z1-Z3（CAND-WORKTREE-001 退役脏工作区强制 patch 存证+三分类 / CAND-DAEMON-001 daemon 失锚自退）+ #ARCH-114 治理脚本命名前缀/EXIT 常量治本 + tracker #102 regime_detector 文件头 MATURITY 顺手修 + #115/#116 xfail 标记清除顺手项 + watchdog RestartOnFailure 退避（#99 ②③）+ fail-open 敞口分析出方案（Owner 裁定随批出方案再拍） | 🔄 派单中 |
