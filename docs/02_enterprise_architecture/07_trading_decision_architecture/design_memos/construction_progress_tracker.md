...
| 104 | GOV 包③三条 open 项（merge 撞号重编后）【遗留表序号注记：初登 #100 与上方 PG 5432 行撞号，2026-08-16 第五统筹重编 #104】 | AI-TD2-GOV-001（5e486bebed） | **#ARCH-114**（~20 治理脚本缺合法命名前缀+17 处裸 return 未用 EXIT_* 常量——双门抓真实违例，治本=批量改名/常量跟进，随治理批）；**#ARCH-115**（PG 依赖 28 项环境失败——PG 已 UAC 恢复，GOV 报 6 项自动 XPASS，strict=False 容错按预期；余项待全量复跑确认，倾向 OBE）；**#ARCH-116**（worktree_pool 3 项 lease 主仓残留修改——主仓派生淤积收口后统筹复跑 **3 项 xpassed**，事实 OBE 转正） | ⏳ #114 随治理批 / ✅ #115/#116 倾向 OBE（全量复跑终证） |
