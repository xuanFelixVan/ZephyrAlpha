...
| 100 | PG 5432 停服疑点核查 | AI-RRESIL-001 反馈 | **核查实锤**（2026-08-16 统筹）：postgresql-x64-16 服务 Stopped（Automatic 启动型但未自启）+5432 TCP 不通——与 #99 内存耗尽事故（800705AF，119.8/130GB）时段重叠，判为事故次生灾害（OOM 连锁）。**恢复需管理员权限**（Start-Service Access denied），待用户手动启动或管理员会话执行；**fail-open 敞口登记治理批**：PG 离线时 depgraph 类门禁静默放行（fail-open）是设计内降级还是敞口，随 RestartOnFailure 退避项一并裁定【编号注记：初登误编 #99 与 watchdog 事故撞号，重编 #100】 | ⏳ 待用户启动 PG（管理员权限） |
