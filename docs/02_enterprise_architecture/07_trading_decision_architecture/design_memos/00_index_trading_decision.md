...
| 2026-08-17 | 2.11.1 | **AI-17 审计计数修正**——§0 标题"48 篇"更正为"49 篇"（v2.11.0 补登 29 号后标题计数未联动；实测口径=§0 表 52 行 − 00 自身 − 02/67 已迁出 2 行 = 49 篇编号文档，与表内行一一对应，覆盖物理 53 文件中 50 个编号文档 + 2 个已迁出 SOP 指针；AI_review_instructions/construction_progress_tracker/handoff_construction_coordinator 3 篇按 §0 注记不入编号） | AI-17 政策架构文档域全量审计：索引计数与表内实测行数对齐，无主题组/决策变更 |