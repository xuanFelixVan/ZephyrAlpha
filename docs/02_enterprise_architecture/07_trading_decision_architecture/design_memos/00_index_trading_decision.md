...
| [69_trading_decision_map.md](69_trading_decision_map.md) | 交易决策地图（跨切治理层；**决策内容索引层=项目第 7 张地图性质**：三流骨架[建仓 L1-L4 内嵌漏斗+持仓 P1-P3+离场 S1-S2+风控横切]+L1 四路传感器阵列+环节×状态矩阵带置信度分级+双市场同 schema；图谱单真源多视图投影；策略/因子/数据按稳定标识符引用既有注册表不复制；Owner D1-D6 裁定+27 项论断交叉验证；V0 后端骨架已施工=MOD-TRADING-015 config/trading_decision_map.yaml+src/zephyr/trading/decision_map.py R1-R8 校验 19 测试全绿；V1 待做=API 端点+前端泳道/矩阵视图+depgraph/ClickHouse 实时缺口检查；V2=整装回测+情绪动态判别） | active v1.0.0 |
