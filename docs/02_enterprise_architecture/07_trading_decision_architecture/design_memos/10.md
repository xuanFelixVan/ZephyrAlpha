...
> **性质**：active v0.2.0。本文是冷数据归档（ClickHouse → E盘 Parquet）的施工执行计划。
...
| 铁律 | 原文 | 冲突点 |
|------|------|--------|
| INV-RET-001 | "所有数据永不删除——禁止 DROP TABLE / DELETE WHERE / TTL" | DROP PARTITION 是否算"删除"？ |
| INV-RET-002 | "进 Cold 层必须手动触发——archiver.py 仅提供手动 CLI" | archiver.py 未实现 |
| INV-RET-003 | "所有表 Hot 层无 TTL，永久保留" | "永久保留"是否意味着不能 DROP PARTITION？ |

**裁定**：铁律精神是"数据不丢失"（保留或归档），不是"ClickHouse 必须保留全部数据"。归档到 Parquet 并验证行数一致后的 DROP PARTITION，数据仍保留在 Parquet，不违反铁律精神。需修订铁律措辞以消除歧义。

### 1.3 目标

1. 腾出 D 盘 ≥ 135 GB 空间（83 GB → 218 GB+），足够装 6 周期回算 198 GB + 20 GB 缓冲
2. 落地 archiver.py，实现契约 INV-RET-002 定义的手动冷归档 CLI
3. 修订铁律措辞，明确"归档后 DROP PARTITION"的合法性
4. 保留回测所需的热数据（Tick 近期 + K线一个牛熊周期）
...
| 依赖 | 来源 | 用途 |
|------|------|------|
| clickhouse-driver | 已安装 | 读 ClickHouse 分区数据 |
| pyarrow 19.0.1 | 已安装 | 写 Parquet 文件 |
...
1. **行数一致性**：每个分区 Parquet 行数 = ClickHouse 归档前行数
2. **抽样字段值**：随机 100 行比对 symbol/open/high/low/close/volume
3. **归档清单完整性**：`archive_manifest.jsonl` 每条记录 `verified=true` 且 `dropped=true`
4. **ClickHouse 分区已删除**：`SELECT count() FROM system.parts WHERE table='tick_data' AND partition='202201'` 返回 0

### 6.2 D 盘空间

| 检查项 | 归档前 | 归档后（预期）|
|--------|--------|-------------|
| D 盘可用 | 83.2 GB | ≥ 231 GB |
| tick_data | 204.2 GiB | ~111 GiB（保留 2025-2026）|
| kline_1min | 88.7 GiB | ~48 GiB（保留 2019-2026）|
| 其他 K 线表 | 44.3 GiB | ~24 GiB |
...
- id: "#ARCH-DATA-COLD-001"
  ...
  decision: "两层归档：Tick 2022-2024 + K线 2019年前，腾出 157.9 GiB"
  depends_on: ["#ARCH-DATA-TI-001"]
```

### 8.2 模块登记

- `capability_canonical_file_registry.yaml`：登记 archiver.py creation_token
- `module_translation_registry.yaml`：登记 plain_zh 翻译条目（cold_archive → 冷数据归档）
...
| [16_technical_indicator_build_plan.md](16_technical_indicator_build_plan.md) | 技术指标回算施工图，本文为其腾出存储空间 |
