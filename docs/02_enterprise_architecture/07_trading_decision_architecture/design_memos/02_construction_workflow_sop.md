...
> **管理规范**：[01_design_memo_management_spec](01_design_memo_management_spec.md)。
> **关联**：[AI_review_instructions](AI_review_instructions.md)（文档审查指令集，Step 1 真源）｜ [60_cross_cutting_cleanup](60_cross_cutting_cleanup.md)｜ [65_git_safety_governance](65_git_safety_governance.md)｜ [66_commit_queue_serialization](66_commit_queue_serialization.md)
...
| 优先级 | P0（施工前置依赖） |
| 状态 | active v1.0.0 |
| 上游 | [01_design_memo_management_spec](01_design_memo_management_spec.md)（设计备忘管理规范） |
...
| 现有文档 | 覆盖环节 | 缺口 |
|---|---|---|
| [trae_035_task_construction_verification.yaml](../../../01_policies_and_standards/rules/trae_035_task_construction_verification.yaml) | 搬家规则/前置检查/循环验收/全景图对齐/门禁命令 | ❌ 缺文档审查+长清单审查+施工完毕文档更新+worktree 合并+只清理自己 |
| [trae_056_module_creation_workflow.yaml](../../../01_policies_and_standards/rules/trae_056_module_creation_workflow.yaml) | 模块创建 10 phase 完整工作流（冷启动→搜索→设计态→准入→蓝图→文件→路径→文件头→启动→注册表→三方对齐） | ❌ 仅"新建模块"流程，不含文档审查/测试/commit/清理/merge |
| [trae_080_panorama_alignment.yaml](../../../01_policies_and_standards/rules/trae_080_panorama_alignment.yaml) | 五图对齐铁律（设计态先行+派生+对齐验证） | ❌ 仅五图对齐环节 |
| [AI_review_instructions.md](AI_review_instructions.md) | 文档审查指令集 | ❌ 只是审查指令，不是端到端施工流程 |
| [65_git_safety_governance.md](65_git_safety_governance.md) | git 安全防护层 | ❌ 只管 git 安全 |
| [66_commit_queue_serialization.md](66_commit_queue_serialization.md) | 多 AI 并发提交队列 | ❌ 只管提交期串行化 |
| [01_design_memo_management_spec.md](01_design_memo_management_spec.md) | 设计备忘三层分治原则 | ❌ 是文档管理规范，不是施工流程 |
...
**引用真源**：[trae_056_module_creation_workflow.yaml](../../../01_policies_and_standards/rules/trae_056_module_creation_workflow.yaml) §phase_0_cold_start / [trae_048_ops_vibe_coding_session.yaml](../../../01_policies_and_standards/rules/trae_048_ops_vibe_coding_session.yaml) §ops_vc_005
**执行命令**：
...
# - 本 SOP（02_construction_workflow_sop.md）
...
**引用真源**：[AI_review_instructions.md](AI_review_instructions.md)（审查指令集真源，不重复内容）
**执行要点**：
...
**引用真源**：[trae_056_module_creation_workflow.yaml](../../../01_policies_and_standards/rules/trae_056_module_creation_workflow.yaml) §phase_1_search / [trae_002_anti_orphan_search_first.yaml](../../../01_policies_and_standards/rules/trae_002_anti_orphan_search_first.yaml) §rule_eight
**执行要点**：
1. 关键词全局搜索（SearchCodebase + Grep on scripts/+src/zephyr/+tests/）
2. 注册表精确匹配（读 [docs/registry_of_registries.yaml](../../../01_policies_and_standards/_registry/catalogs/registry_master_index.yaml) → 对应 REG-* → 对照）
...
**引用真源**：[trae_036_arch_gate_transition.yaml](../../../01_policies_and_standards/rules/trae_036_arch_gate_transition.yaml) §gov_arch_002
**执行要点**：
...
**引用真源**：[trae_080_panorama_alignment.yaml](../../../01_policies_and_standards/rules/trae_080_panorama_alignment.yaml) §panorama_alignment / [trae_056_module_creation_workflow.yaml](../../../01_policies_and_standards/rules/trae_056_module_creation_workflow.yaml) §phase_2_design_state / [trae_032_module_lifecycle.yaml](../../../01_policies_and_standards/rules/trae_032_module_lifecycle.yaml) §mod_001
**执行命令**：
...
**架构升级期保护**：depgraph 生成器（generate_project_depgraph.py）在重建运营态时保护设计态节点不被覆盖——`restore_design_data` 恢复 design edges，DELETE 只删非设计态且设计态有同路径的节点（`scripts/governance/generate_project_depgraph.py` §restore_design_data L3094）。但**架构升级期仍禁止运行** `--force`，会覆盖全景图（[trae_005_modification_governance.yaml](../../../01_policies_and_standards/rules/trae_005_modification_governance.yaml)）
...
**引用真源**：[trae_080_panorama_alignment.yaml](../../../01_policies_and_standards/rules/trae_080_panorama_alignment.yaml) §panorama_alignment
**执行命令**：
...
**引用真源**：
- [trae_056_module_creation_workflow.yaml](../../../01_policies_and_standards/rules/trae_056_module_creation_workflow.yaml) §phase_4_blueprint ~ §phase_9
- [trae_047_engineering_file_header.yaml](../../../01_policies_and_standards/rules/trae_047_engineering_file_header.yaml) §gov_eng_002
- [trae_053_automation_dual_track.yaml](../../../01_policies_and_standards/rules/trae_053_automation_dual_track.yaml) §rule_fifteen
- [trae_064_git_call_budget.yaml](../../../01_policies_and_standards/rules/trae_064_git_call_budget.yaml) §git_call_budget
- 各设计备忘 §施工算法章节

**执行要点**：
1. **蓝图创建**（仅新建模块）：按 [blueprint_construction_template.md](../../../01_policies_and_standards/templates/blueprint_construction_template.md) 模板
2. **文件创建**：用 scaffold.py + 锁协议（[trae_001_file_operation_security.yaml](../../../01_policies_and_standards/rules/trae_001_file_operation_security.yaml) RULE-ZERO）
3. **路径多维审查**：名字+路径+上层文件夹+容量对齐（[trae_055_arch_domain_capacity.yaml](../../../01_policies_and_standards/rules/trae_055_arch_domain_capacity.yaml) ARCH-CAP-002：≤150 通过 / >150 必须拆分）
...
6. **多注册表同步**：MRS-001~003（[trae_033_module_registration_sync.yaml](../../../01_policies_and_standards/rules/trae_033_module_registration_sync.yaml)）
...
**任务卡系统**（[trae_034_task_card_standard.yaml](../../../01_policies_and_standards/rules/trae_034_task_card_standard.yaml)，若施工需要任务卡载体）：
...
**引用真源**：[trae_035_task_construction_verification.yaml](../../../01_policies_and_standards/rules/trae_035_task_construction_verification.yaml) §task_003_circular_acceptance / [trae_084_precommit_incremental_discipline.yaml](../../../01_policies_and_standards/rules/trae_084_precommit_incremental_discipline.yaml)
**执行命令**：
...
**pre-commit 增量守门**（[trae_084](../../../01_policies_and_standards/rules/trae_084_precommit_incremental_discipline.yaml)）：
...
**测试残留清理**（[trae_071_temporary_file_lifecycle.yaml](../../../01_policies_and_standards/rules/trae_071_temporary_file_lifecycle.yaml) §test_residue_reclaim L414）：
...
**引用真源**：本 SOP §附录 A（全文收录用户提供的 12 节审查清单）+ [trae_081_audit_dimensions_framework.yaml](../../../01_policies_and_standards/rules/trae_081_audit_dimensions_framework.yaml)（54 维度基座，长清单是子集）
**执行要点**：
...
5. **跨蓝图变更通知**（RULE-ELEVEN，[trae_052](../../../01_policies_and_standards/rules/trae_052_cross_blueprint_change_cleanup.yaml)）：修改任何蓝图的接口契约（Collection 名/API 签名/数据格式/依赖方向）时
...
**引用真源**：[trae_080_panorama_alignment.yaml](../../../01_policies_and_standards/rules/trae_080_panorama_alignment.yaml) §panorama_alignment（转正流程） / [trae_056_module_creation_workflow.yaml](../../../01_policies_and_standards/rules/trae_056_module_creation_workflow.yaml) §phase_10 / [trae_035_task_construction_verification.yaml](../../../01_policies_and_standards/rules/trae_035_task_construction_verification.yaml) §task_003_panorama_alignment
**执行命令**：
...
**产出物**：全景图对齐通过报告
**禁止**：架构升级期运行 `generate_project_depgraph.py --force`（会覆盖 depgraph 全景图，详见 [trae_005_modification_governance.yaml](../../../01_policies_and_standards/rules/trae_005_modification_governance.yaml)）
...
**引用真源**：[65_git_safety_governance.md](65_git_safety_governance.md) §备份先行 / project_memory #ARCH-GIT-CLEAN-GUARD-FIX 灾难教训
**执行命令**：
...
**引用真源**：[66_commit_queue_serialization.md](66_commit_queue_serialization.md) / [trae_075_stash_lifecycle.yaml](../../../01_policies_and_standards/rules/trae_075_stash_lifecycle.yaml) / [trae_084_precommit_incremental_discipline.yaml](../../../01_policies_and_standards/rules/trae_084_precommit_incremental_discipline.yaml)
**执行命令**：
...
**引用真源**：[trae_071_temporary_file_lifecycle.yaml](../../../01_policies_and_standards/rules/trae_071_temporary_file_lifecycle.yaml) / [trae_035_task_construction_verification.yaml](../../../01_policies_and_standards/rules/trae_035_task_construction_verification.yaml) §task_closure_standard 清扫三步法
**三级分类**（[trae_071](../../../01_policies_and_standards/rules/trae_071_temporary_file_lifecycle.yaml)）：
...
**force merge 安全分类**（[trae_078](../../../01_policies_and_standards/rules/trae_078_force_merge_safety.yaml)）：
- **不可绕过类**（force=True 也不跳过）：
  - commit 持久性验证（_read_commit_persisted_marker + tip hash 对比，[trae_076](../../../01_policies_and_standards/rules/trae_076_worktree_commit_persistence.yaml)）
...
**commit 持久性标记**（[trae_076](../../../01_policies_and_standards/rules/trae_076_worktree_commit_persistence.yaml)）：
...
- **不替代**任务卡系统（[trae_034_task_card_standard.yaml](../../../01_policies_and_standards/rules/trae_034_task_card_standard.yaml)，33 字段+粒度约束）
...
| 5 | 是否需要把 SOP 注册到 [registry_of_registries.yaml](../../../01_policies_and_standards/_registry/catalogs/registry_master_index.yaml) | 待讨论 |
...
> 本附录是长清单审查（Step 6）的真源清单，源自用户 2026-08-12 提供。54 维度基座见 [trae_081_audit_dimensions_framework.yaml](../../../01_policies_and_standards/rules/trae_081_audit_dimensions_framework.yaml)，本清单是其子集。
...
| **测试** | test_residue_reclaim | 测试残留清理（[trae_071](../../../01_policies_and_standards/rules/trae_071_temporary_file_lifecycle.yaml) §test_residue_reclaim） |
