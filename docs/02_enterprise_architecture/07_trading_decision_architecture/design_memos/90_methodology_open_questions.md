...
| 2026-08-12 | 2.0.1 | 作战地图全覆盖补丁——BM-SEL-05-D / BM-SEL-05-E / BM-SEL-06 / BM-SEL-10 / BM-SEL-26。新增 §22 作战地图选股层补充环节 5 条开放问题（均 design 态、登记远期/Phase 2 候选、MVP 不建，各含定位→裁定理由+重评条件→契约/参数→边界消歧四层）：① 22.1 BM-SEL-05-D 主力行为自迭代推演（行为分类 5 态+出货派发概率+假动作 6 类+模式库自迭代；边界=25 号资金性质 5 类已 production 是输入特征层，本环节是行为推演层）；② 22.2 BM-SEL-05-E 庄家行为识别与模拟（庄家 4 阶段建仓/洗盘/拉升/出货+控盘度+筹码锁定+博弈意图模拟；边界同上，为 22.1 庄家特化分支）；③ 22.3 BM-SEL-06 跨市场传导感知（C-039 传导系数+A股影响幅度量化预测；边界=36 号 §3 BS-005 是风控前兆监控已 production，本环节是 alpha 侧传导量化）；④ 22.4 BM-SEL-10 行情生命周期阶段（春夏秋冬 4 阶段+新高占比趋势+冬季禁抄底/秋季强制离场；边界=28 号情绪周期 4+1 是 sleeve 内择时、10 号 regime 是市场级节流，本环节是低频季节判定，三者不同物）；⑤ 22.5 BM-SEL-26 决策可解释性与人机协作（C-030 决策溯源链+人机协作接口；边界=§12/§19 已裁定 C-031 置信度分层与审批频次，本环节补溯源链呈现侧） |
