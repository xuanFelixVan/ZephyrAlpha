...
> - #7 T+1 8 态预测——v2.0.0 裁定"暂缓建设"经 v2.1.0（2026-08-28）**口径回切**：预测模块已按窄口径（只出概率分布+置信度，不出点位/买卖信号）落码 production（MOD-SIG-037），登记为既有模块；维持暂缓的对象=8 态→决策消费链（重启条件未达）。#10 密度预测——远期维持（重启条件未达，91 号承载规划态登记）；裁定=过度工程（当前阶段）。
...
> **仍真实未完工**：P-1~P-5 五项待人拍板（设计内）；§22 远期候选未建（设计内）。
> **2026-08-28 口径回切（v2.1.0）**：#7 T+1 8 态预测"暂缓建设"裁定文字与代码现状的出入已闭环——MOD-SIG-037（signal_ashare/next_day_8state_forecast.py）已按窄口径落码 production，登记为既有模块；维持暂缓的对象收敛为 8 态→决策消费链（详见 §7 口径回切注记）。
...
| 约束九 | T+1次日预测(8态) | G02 | 8态(BM-SEL-04)预测模块已窄口径落码（MOD-SIG-037 production）；8态→直接决策映射过时(Model A)，决策消费链未建 | ❌ 暂缓建设（52-53%天花板未突破，远期窄目标重启条件已定义）→ **v2.1.0 口径回切**：预测模块登记为既有模块（窄口径：只出概率分布不出点位/信号），暂缓对象收敛为 8 态→决策消费链（见 §7） |
...
> 对应 G02（regime spec，已定稿）｜ ⚠️ **纠正（v0.2.0）：原稿"8态已被12态替代"不准确** ｜ ❌ **v2.0.0 裁定：暂缓建设（BM-SEL-04 降级远期）** ｜ 📝 **v2.1.0 口径回切（2026-08-28）：预测模块已按窄口径落码 production（MOD-SIG-037），登记为既有模块；维持暂缓的对象=8 态→决策消费链（重启条件不变）**
...
**📝 v2.1.0 口径回切（2026-08-28，路线图 A18 闭环）**：

- **回切原因**：v2.0.0 裁定文字"暂缓建设/全部未建"与代码现状矛盾——`signal_ashare/next_day_8state_forecast.py`（MOD-SIG-037）已按窄口径落码并流转 production（一阶马尔可夫链+Laplace 平滑+平稳分布修正，输出 8 态概率分布+置信度；模块边界声明与本裁定一致：**只出概率分布，不出点位、不出买卖信号**）。
- **回切后口径**：① **预测模块本体=既有模块**（窄口径概率输出层，登记不删除——落码实现与本裁定"52-53% 天花板/T+1 兑现悖论"论据自洽：不做点预测、不接直接决策）；② **维持暂缓的对象=8 态→决策消费链**（8 态→直接决策映射确认废弃不变；BM-BUY-01 多情景对策/下游风控节流层信号权重修正的消费接线未建）；③ **远期重启条件不变**（系统稳定盈利+目标收窄开盘 30 分钟+概率仅接仓位微调，三者全满足才评估消费链建设）。
- **历史裁定保留**：上方 v2.0.0 裁定结论原文不删（含 52-53% 天花板论据与废弃映射记录），本节为口径修正注记，非翻案。
...
| ⏸️ | #7 T+1 8态 / #10 密度预测 | ❌ 暂缓/远期 | #7 预测模块已窄口径落码 production（MOD-SIG-037，v2.1.0 口径回切登记为既有模块），暂缓对象=决策消费链（重启条件见 §7）；#10 不施工（91号待回填） |
...
| 2026-08-28 | 2.1.0 | #7 T+1 8 态预测口径回切：v2.0.0"暂缓建设/全部未建"裁定文字 vs 代码现状（MOD-SIG-037 已 production）矛盾闭环——预测模块按窄口径（只出概率分布+置信度，不出点位/买卖信号）登记为既有模块；维持暂缓对象收敛为 8 态→决策消费链，远期重启条件不变；历史裁定原文保留（§7 加口径回切注记段）；总览表/已施工设施盘点/施工优先级表/结案报告回填四处同步 | 2026-08-28 剩余施工路线图 A18 项（session AI-WAVE0-001）：裁定文字与代码现状对齐，防后续会话按"未建"口径重复建设或误删既有模块 |
