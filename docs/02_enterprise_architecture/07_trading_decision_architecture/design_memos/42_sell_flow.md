...
> ⚠ 勘误（2026-09-07 D104）：本表口径作废，以 D58 为准=WATCH 秒级/MONITOR 1 分钟/HOLD 5 分钟。
...
#### 3.8.1 ST 主板涨跌幅 5%→10% 联动参数重校判定（2026-09-07 清偿，#ARCH-DATA-020 ③）

背景：2026-07-06 起主板 ST/*ST 日涨跌幅 5%→10%（沪深交易所《交易规则（2026年修订）》，上交所上证发〔2026〕41号），数据管道已按生效日切片修复回填（#ARCH-DATA-020 ①）；TDM-X-S2-02 挂账"联动参数重校欠账：竞价即决封板/溢价映射、跌停排队、封成比阈值按 10% 重校"。本节为该欠账的逐项判定落账。

**总结论：交易决策层零值变更——全部无需重校，亦无需做成时变参数。** 第一性原理：竞价即决矩阵/溢价映射/封成比分档的校准宇宙本就是 10% 涨跌幅板（主板非 ST 打板样本 + 社区 428 笔弱转强回测锚），2026-07-06 后主板 ST 是**加入**该校准宇宙（同口径全域适用），属"适用域修复"而非"参数漂移"——旧 5% 制下高位档（+5~9% 预期高开、≥+9% 一字位）对 ST 结构性不可达（矩阵对 ST 被截断），重校方向是恢复全域适用而非改值；改值反而为同一语义引入 ST 双真源。逐项判定：

| 判定项 | 现值 | 判定 | 依据 |
|---|---|---|---|
| 封成比四档（>10 稳定/3-10 强/1-3 一般/<1 不牢，`score_seal_structure`） | 10/1 边界 | 无需重校 | 封单量/成交量比值，量纲与涨跌幅限制无函数关系；分档校准宇宙=主板 10% 打板样本（24 号 §3.11① C11），ST 拉平后同口径适用 |
| 溢价×量能十行矩阵（≥+9% 一字位…<-5% 核按钮，69 号 §2.30 D67 真源） | 绝对溢价带 | 无需重校 | 绝对损益（PnL）语义不随板幅缩放；校准=10% 板，ST 10% 后全域可达；为同一语义分叉 ST 专属值=双真源漂移温床 |
| 封板时间→次日预期高开映射（秒板 5-9%/10:00-11:00 3-6%/13:00-14:00 0-3%/烂板 0 至 -3%） | 5-9% 等 | 无需重校 | 同上；旧 5% 制下该映射高位档对 ST 不可达（次日竞价 gap 被 ±5% 封顶截断），新制恢复全域适用 |
| 竞价量能三口径+弱转强回测锚（占昨日总量 5-10% 健康/<3% 缩量/>15% 爆量；428 笔锚=高开 2-5%+量比>5+竞价额>1000 万） | 量比值 | 无需重校 | 成交量比值口径与涨跌幅无关；样本宇宙=主板 10% 板 |
| 高开/低开出场档位（`NextDayExitDecision`：-5% 核按钮闷杀/+3% 竞价卖半/+5% 全卖） | -0.05/0.03/0.05 | 无需重校 | 相对成本基的绝对损益档位，风险管理语义与板幅无关；10% 制下 ST 新增可达尾部（±8% 级低/高开）落入既档位行为正确——回归锚测试已锁定（tests/ex_core/test_daban_exit_decision.py::TestST10PctRecalibrationBaseline） |
| 昨涨停竞价溢价映射（`scenario_planner._compute_limit_up_premium` + `llm_premarket_analysis._SQL_LIMIT_UP_SYMBOLS` 同构） | 数据驱动 | 无需重校 | 涨停名单=close≥stk_limit.limit_up（JOIN 数据真源零硬编码 pct），溢价=(竞价匹配价-昨收)/昨收 实测值；stk_limit 已按生效日切片回填（#ARCH-DATA-020 ①），2026-07-06 前无 stk_limit 行→优雅降级 (None,0) 不阻塞主流程 |
| 跌停排队/卖出执行（`sell_execution_planner` LIMIT_DOWN_QUEUE + `rank_limit_down_orders` + `simulation/limit_board_queue.py`） | 注入式 | 无需重校 | is_limit_down 由调用方判定（布尔输入），排队排序键=紧迫度/亏损/仓位金额，limit_pct 由调用方注入（`涨/跌停价=昨收×(1±limit_pct)` 无板幅假设）——零 5% 残留 |
...
> ⚠ 口径冲突存疑（外审修复 2026-09-07，M-57）：本节与 69 号 D58（≤15 笔/秒+日撤单率≤15%）正面冲突；可能一个是高频认定线、一个是异常交易监控线——以交易所细则原文裁决为准。（2026-09-07 D102 终裁：300 笔/秒+日 2 万笔=高频认定线，本系统不适用；15 笔/秒+日撤单率≤15% 保留为内部自评红线）
