...
|---|---|---|
| 全库合规债务分批修复 | 47 篇受检 32 篇违规：G 类版本不同步 17 / D 类 title·H1 骨架标记残留 8 / F 类缺开放问题节 6（16 两篇/18/51 等）/ A 类字段顺序与白名单外字段 4（11/12/13/14，明细见下行）/ E 类修订记录不在末尾 3（10/62/64，64 表为空） | 各文档归口 AI 在处理各自文档时顺带修复；不在本规范越界改 |
...
| 2026-08-15 | 1.3.1 | 第二轮循环压缩：可压缩点收敛=0（AI-DC2-11）。仅 §7 首行扫描摘要冗余修饰清理（类名自明括注删、11-14 字段明细指针化归下行），全篇通读无其他可压缩点 | 第二轮循环压缩门禁；不为压而压，规范正文（定义/纪律/模板）均为当前态真源，零实质改动 |
