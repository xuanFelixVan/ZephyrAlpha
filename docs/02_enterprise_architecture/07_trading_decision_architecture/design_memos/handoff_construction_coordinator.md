...
- **🔄 在途四路并发**：①**AI-GOVA-001 治理批 A 包**（Z1-Z3 退役存证/分类器/daemon 失锚自退+#ARCH-114 脚本命名/EXIT 常量+tracker #102 顺手修+#115/116 xfail 清除+watchdog RestartOnFailure 退避+fail-open 出方案）指令=docs/_working/dispatch/2026-08-17-gova001-governance-batch-a-order.md；②**AI-LVL3-001 37 号 LEVEL_3 接线**（LEVEL_3 检测→逃生指令→Kill Switch 生产链路）指令=docs/_working/dispatch/2026-08-17-lvl3001-liquidity-level3-wiring-order.md；③**AI-THD-001 阈值统读**（#87 存量 9 模块码内阈值→REG-ATH-001 fail-closed 统读+一致性测试演化）指令=docs/_working/dispatch/2026-08-17-thd001-threshold-unification-order.md；④**AI-DGR-001 五态降级机**（#101/QUANT-003 53 号 §3.8 五态 FSM 落码+晋级 NORMAL 耦合+fail-closed 持久化）指令=docs/_working/dispatch/2026-08-17-dgr001-degradation-fsm-order.md。四路文件级零交集两两实证。
- **AI-REDIS-001 ✅ 已 merge 闭环（2026-08-17 第六统筹）**：d95675322e（Redis 后端同接口+30 新测试）+8470e899（#ARCH-118 拆分续作，统筹代执行）→e9d49313 merge（冲突 9 处=8 派生取 dev 侧+capability token 并集）；统筹独立复跑 65×2+消费方 28 全绿；**副产品=trae_010 ≤300 行规则第一性原理重裁 v1.3.0**（考古：零门禁执行+21.4% 违例死条文→分层裁量：≤300 通过/301-500 高内聚豁免/>500 须拆或登记豁免/并发热点降一档，#ARCH-118，Owner 裁定"规则可以为正确选择让路"）；db15 测试库约定登记 redis_config D3 行（2e20d808）；depgraph 全量重扫已刷（state_store_redis node_id=9702462 stable+production）；worktree 已清理。
...
- **待推进（五路并发期）**：①#ARCH-ERRCODE-001 错误码改号（登记时自定"择无并发窗口"——五路完工收口窗口独占执行）；②全量测试复跑终证（#115/116 已 OBE，全量背书可待）；③QUANT-002 registry 流转收口（统筹动作，REDIS 已 merge 可顺手标注）；④#61-64 专项裁定挂起；⑤#96 观察项；⑥小项顺手批（#8 capability 补登/#10 UNI-BASKET 补登/#11 16 号对齐/#12 data_asset 锚点/#14 DSR 阈值裁定等 1-3 行级改动，收口窗口执行）。
