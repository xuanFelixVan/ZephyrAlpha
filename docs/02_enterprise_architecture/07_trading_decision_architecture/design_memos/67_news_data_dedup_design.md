...
| 问题 | 建议 | 决策状态 |
| --- | --- | --- |
| 老批单行 1,400 个 id 的 +8h 修正重插是否随本轨做 | 做——量小且同脚本一次完成 | **已执行**（引发事故链，见 §8；误伤行已在重建中 -8h 反演归位） |
| news/announcement \~760 万行 8h 偏移专项的优先级 | 建议 P0——前视偏差直接污染回测结论，先于一切依赖新闻时间戳的策略开发 | **已立项** CAND-DAT-026（含 stuck 35 行 16:00 指纹） |
| 清扫执行窗口 | 常态采集为增量小流量，无需停写，但建议避开备份窗口 | 已执行 |

## 8. 事故记录与结局（2026-08-28，v2.0.0）

### 8.1 事故链（清扫执行引爆既有深层炸弹）

1. purge 清扫的 ALTER DELETE mutation 在重写 part 时**折叠吞数据**：实证 202506 分区 409 个不同键 OPTIMIZE 后剩 32 行
2. 全新干净表同款引擎白盒对照**完全正常** → CH 26.6.1 引擎无恙；根因=**本表 part 物理排序与排序键脱节**（时区迁移 #ARCH-CH-022 原地改键列 publish\_time 的后遗症），merge/mutation 在坏 part 上按错误假设把不同键的行折叠丢弃
3. 表持续萎缩（ids 146,519→118,214→真实 3,404，8 小时）；`system.parts`/物理 DISTINCT 在腐败态**虚报行数**（元数据 410 实际 32），FINAL 一度反而是准的
4. category 标签在事故链中大量丢失：修正重插行携老批 `general` 标成为最新版本，argMax 去重后 143,122 行研报源行 category 错标

### 8.2 处置（全部完成，终验通过）

| 步 | 动作 | 结果 |
| --- | --- | --- |
| 1 | 旧表封刀（禁一切 mutation/OPTIMIZE） | 止血 |
| 2 | 换表重建 rebuild\_news\_data.py：逐月 argMax 按键去重 + 误伤批 -8h 精确反演 + 逐月对账 + 原子双 RENAME + 窗口增量补捞 | 190 月全过；旧表保留为 news\_data\_corrupt\_20260828 观察 |
| 3 | tag\_news\_category 重刷 category | research\_report 146,561 行归位 |
| 4 | 终验 | **FINAL == 物理 == 键数**（146,561/146,526 id）；publish\_time 全洗 00:00 健康语义（stuck 35 行 16:00 指纹除外）；2025 段 17,007 id |

### 8.3 教训（硬约束级）

1. **ReplacingMergeTree 键列禁止原地改值**：改键列=merge 语义死亡，必须重建表并验证 part 排序（白盒小表对照法）
2. **mutation 谓词禁止同表 IN 子查询**：逐 part 重估子查询导致不同 part 适用不同集合，行为不可控；id 集须先物化（临时表）或谓词仅依赖本行字段
3. **腐败态下元数据不可信**：`system.parts.rows`/count()/DISTINCT 均可能虚报；以白盒对照+多口径交叉验证为准
4. 备份 inc.zip 每日覆盖——破坏性操作前的"近 24h 备份"若恰在事故后执行，事故前状态即永久丢失；大手术应额外留**带外快照**（本轨靠 akshare 确定性 id 重采兜底，实属侥幸）

### 8.4 结局对账

* 研报 id 全集 146,526（vs 评级产物 146,519，+7 零星新采）；按年分布与事故前 uniq 口径逐年在 ±5 内对齐
* 35 个全老批 stuck id（16:00 指纹，publish\_time 偏 8h）转入 CAND-DAT-026 一并处理
* 全量 akshare 重采**未执行**（行全集在，仅标签丢失，无需重采）；backfill 已年份参数化备用
