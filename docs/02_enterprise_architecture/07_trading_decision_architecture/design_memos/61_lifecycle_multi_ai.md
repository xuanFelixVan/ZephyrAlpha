...
> **策略规格产出物**（§3.1 ①孵化阶段退出条件"策略规格产出"的承接）：以 [20_first_batch_strategies](20_first_batch_strategies.md)（G04，active v1.2.4）为首批范式——sleeve 定位 / 容量测算 / 持仓周期 / 风控参数四要素齐备才算"规格产出"，方可进入 ② 训练阶段。
