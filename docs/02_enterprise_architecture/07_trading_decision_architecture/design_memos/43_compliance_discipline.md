...
| 日期 | 版本 | 改动 | 理由 |
|---|---|---|---|
| 2026-08-12 | 0.1.0 | 新建——作战地图全覆盖补丁，承载 BM-BUY-08-A/08-B/09/12/15 五环节：08-A 四时点必做清单检查器；08-B 四项严禁阈值+算法+KillSwitchLite；09 重定义为数据源授权合规（登记表+使用审计+违规处置三级）；12 功能裁定清单结构+上线门禁（与 30 号 charter 红线消歧）；15 补市场操纵 4 类检测+程序化报告 6 项义务+50μs 时间锁不适用裁定 | 作战地图 339 环节全覆盖审计发现合规域 5 环节无设计载体，经用户裁定新建本篇统一承载，作 D_COMPLIANCE 域设计真源 |
