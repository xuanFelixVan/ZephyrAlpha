...
| # | 问题 | 状态 | 说明 |
|---|------|------|------|
| Q1 | 模块工厂 Phase 0→1 的施工优先级？ | ✅ 已裁定（2026-08-29，#ARCH-285） | **批准转入 GP1 排期批第一波**。三判据全满足：①前置就绪（U8 实测 active、19号 SOP 首实例 FCT-SENT-028 全链路走通、M0 已过）；②Phase 1 只自动化纯机械环节（LLM 分类+embedding 查重），生成动作仍留 Phase 2+人审，风险面可控；③人审闸门在位（L4/62号 G8 不可降级）。Phase 1 范围=knowledge_classifier + module_mapper 两模块，验收按 §4.8（一致率≥85%/≥5 条半自动入库） |
| Q2 | 11 号技能库接口假设待确认 | ✅ 已裁定（2026-08-29，#ARCH-286） | **真源裁定：接口真源=11 号文（接口提供方持有真源，打破两文互让环路）**。11号文已填充（v0.3.0），其 §4.6 口径=技能库不为模块工厂单独建索引、复用 skill_discovery/skill_router 既有设施。本文 §4.6 按此对齐修订。实证差异登记：既有设施无"按任务描述检索技能候选列表"的精确函数（skill_router.route 返回 role/domain 路由而非候选集）——字段级契约在 Phase 1 P1-S3 开工时冻结（届时以 11号文为真源落接口），不阻塞排期 |
| Q3 | 12 号自反 Agent 接口假设待确认 | ✅ 已关闭（2026-08-29，#ARCH-286） | 12号文已填充（v0.3.0）且其 Q3 已逐条核对声明"13号文 §4.7 假设与本文 §3.1/§3.2/§3.4 设计一致，待人确认后双关"——本裁定即人确认，双关成立。字段增强项（EvaluationReport 增 source=验证失败/人审驳回 区分字段）登记为 Phase 1 顺手项，不阻塞 |
| Q4 | 62 号文档 §6 标题"P1 待施工七注册表"与落盘状态不一致 | ✅ 已裁定（2026-08-29，#ARCH-286） | **以落盘实测为准**（注册表 YAML active 即事实）；62号文 §6 标题修订转交易决策侧顺手项，不构成阻塞，随该侧下次维护批一并处理 |
| Q5 | discovery_agent 枚举是否扩展 `module_factory` 值 | ✅ 已裁定（2026-08-29，#ARCH-286） | **批准扩展**——模块工厂是真实发现源，`other` 占位损失审计粒度；62 侧 schema 小变更随 Phase 1 入库施工一并执行 |
| Q6 | 模块工厂自身的 depgraph 节点编号与域归属 | ✅ 已裁定（2026-08-29，#ARCH-286） | 前置已解锁（03号文域边界 #ARCH-169 已于 2026-08-22 裁定）。**归域=D_AUTONOMY_CORE**——"知识→模块创建"是自治核心能力，与 reflexion_agent/execution entries 同域；编号按既有 MOD- 系列在 P1-S1 登记时分配（knowledge_classifier/module_mapper 各一节点） |
| Q7 | 潘潘课程剩余约 260 条是否作为 Phase 0 首批手动实例 | ✅ 已裁定（2026-08-29，#ARCH-286） | **批准**——剩余 260 条正是 Phase 1"≥5 条半自动链路入库"验收的真实负载来源；首批仍按 19号 SOP 手动形态走，随 Phase 1 分类器/映射引擎就绪后切半自动对照 |
| Q8 | S0 采集增强候选与本文"文本优先"裁定的口径差异 | ✅ 已裁定（2026-08-29，#ARCH-286） | **拆分裁定**：③PIT 门控**吸收**为数据纪律项（采集数据时间戳标注+财务数据延迟报告期=防前视正确性刚需，属"采集到的数据如何标"而非"自动采集"，与 §3.1 文本优先裁定不冲突，随 Phase 1 入库 schema 一并落地）；①ADWIN 漂移调度、②VLM 图表理解**维持不进施工计划**（产能扩张项，当前瓶颈不在采集速度；登记 Phase 2+ 远期候选） |
| Q9 | MAML/EWC/LoRA"小参数可行"反证 vs 01 号文"用 ICL 替代"裁定 | ✅ 已裁定（2026-08-29，#ARCH-286） | **维持 01号文 ICL 既定裁定不动**；草稿折中口径（小样本 ICL/大样本 MAML，R-62）登记为 Phase 3 远期观察项。理由：Phase 3 本身远期不承诺时间表，当前无任何施工依赖该选择；推翻既定裁定需要的不只是"可行性反证"而是"必要性证据"（ICL 案例库被实测证明不足），该证据不存在 |
