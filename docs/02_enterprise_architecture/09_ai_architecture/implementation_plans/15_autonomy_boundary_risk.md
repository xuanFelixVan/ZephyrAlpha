...
**关于"<1ms 自动触发"的可行性边界（2026-08-18 实测完成）**：实测环境=本机 Python 3.12.8 / Windows nt；采样方法=最小计时脚本，1000 次采样取 P50/P95/P99。①**决策点内联拦截**（同进程同步检查）：P50=0.20μs / P95=0.30μs / P99=0.30μs / max=1.70μs——微秒级成立；②**文件标志位轮询**（存在性检查+读取小文件）：P50=0.032ms / P95=0.045ms / P99=0.110ms / max=0.261ms——优于原 10ms~1s 预期（本地缓存读），但该数据是同机本地文件读，跨进程/跨会话传播仍不承诺毫秒级语义。"AI 自动 <1ms"经实测确认为**决策点内联拦截**（下单/写文件前的同步检查，等效 0 附加延迟），不能理解为"外部信号 1ms 内传遍全系统"。

**Phase 0 运行时 gate 延迟实测（2026-08-22 补测，实测完成）**：实测环境=本机 Python 3.12.8 / Windows；采样方法=最小计时脚本，1000 次采样取 P50/P95/P99。①**gate 热路径**（三分类判定+审计留痕）：P50=54.1µs / P95=313.5µs / P99=902.8µs；②**gate 纯判定**（不含留痕）：P50=22.1µs / P95=111.9µs；③**gate BLOCK+告警**：P95=530.5µs；④**KS 内联拦截查询**：P50=0.4µs / P95≤0.8µs / P99≤0.9µs。四组链路全部 P95<1ms 达标——§4.1 S0.4 验收口径满足，§3.4「<1ms=决策点内联拦截」工程语义无需修订。实测明细见 `docs/_working/reports/2026-08-22-autonomy-gate-latency.md`。
### 2.4 已施工设施盘点
...
| Agent Challenge | 检测器判"像漂移"不等于真漂移；让 Agent 自证（复述原始任务意图并对齐当前动作）可把判定成本转移给被检测方，降低误报停工 | 无现成实现——实现方式已裁定（2026-08-18，Q2 关闭）：交叉会话复审为主形态，challenge 工单为统一落盘载体与降级形态，人兜底不可省（§4.3） |
...
**三套既有标尺的映射（已定稿，2026-08-18 裁定，Q3 关闭）**：00_index 有界自治 L0~L3（+L4 保留不启用）为项目唯一主标尺（管「Agent 能做什么」），其余标尺向其映射或保持正交：

| 00_index 有界自治主标尺（可操作档） | PS-VOC-021 规划级 | AutonomyMaturity 运行时信任 | 语义对齐点 |
|---|---|---|---|
| L0 人工 | l0（无自治） | L0 OBSERVE | 只观察记录，不产动作 |
| L1 建议 | l1 | L1 NOTIFY + L2 SUGGEST（L1 的两个亚档） | 产建议，人决策人执行 |
| L2 低风险 | l2 | L3 AUTO_MINOR | 低风险动作自动，留痕可回滚 |
| L3 中风险 | l3（描述已与主标尺封顶对齐：中风险封顶，全权自主不启用） | 无启用对应档（L4 AUTO_FULL 保留不启用，不入映射；L3 内仍带熔断兜底） | 中风险自动 + regressor 回归兜底 |

原两个已知张力已裁定（2026-08-18）：①PS-VOC-021 的 l3 语义原为"全权自主"，比主标尺 L3"中风险"更激进——裁定：l3 描述与主标尺封顶对齐（词表已改为"中风险封顶，Owner 授权范围内自主；全权自主不启用"）；②AutonomyMaturity 5 档（L0~L4）vs 可操作 4 档——裁定：L1 建议对应 NOTIFY+SUGGEST 两个亚档，L4 AUTO_FULL 保留不启用、不入映射。AutonomyGuard 的 level1~3 是正交维度（Owner 缺位时的运维动作白名单，非信任阶梯），不纳入映射、保持不变。16 号文 A-L0~A-L4 系管「运维闭环自治深度」，是另一根轴——不建映射，只做交叉引用（见 16 号文 §3.7）。
...
**"<1ms"的工程语义（2026-08-18 实测确认）**：按 §2.3 的实测口径，"AI 自动 <1ms"落地为**决策点内联拦截**——下单/写文件/注册表变更前的同步检查，与被拦截动作同进程同线程，实测 P50=0.20μs / P95=0.30μs / P99=0.30μs，微秒级成立；不承诺"外部信号 1ms 传遍全系统"（Windows 文件标志位轮询实测 P50=0.032ms / P95=0.045ms / P99=0.110ms，优于原 10ms~1s 预期，但系同机本地缓存读，跨进程传播不承诺毫秒级——§2.3）。约束四给了这个解读的底气：Tick=3 秒、miniQMT 10 笔/秒，决策点拦截的时间预算本就宽裕。
...
> 本 Phase 启动以 Q2/Q3 裁定为前提（两问均已于 2026-08-18 裁定），属远期工程，不展开细排。

- Agent Challenge（实现形式已裁定 2026-08-18）：交叉会话复审为主形态（另一 AI 会话做对齐判定），challenge 工单为统一落盘载体与降级形态（交叉会话不可用/超时时直接进人审队列），人兜底不可省（61 号备忘「交接落盘可追溯」与约束⑥交叉验证范式）。工单字段：原始任务意图复述 + 当前动作链对齐说明 + 检测器证据；流转：检测器判疑 → 落盘工单 → 交叉会话复审 → 争议/RED/超时 → 人审。
...
| # | 问题 | 状态 | 说明 |
|---|------|------|------|
| Q1 | Kill Switch "<1ms" 语义与 Windows 实测 | 已实测+已裁定 2026-08-18 | 实测完成（2026-08-18 本机，Python 3.12.8/Windows nt，最小计时脚本 1000 采样）：①决策点内联拦截 P50=0.20μs/P95=0.30μs/P99=0.30μs/max=1.70μs——微秒级成立；②文件标志位轮询 P50=0.032ms/P95=0.045ms/P99=0.110ms/max=0.261ms——优于原 10ms~1s 预期（本地缓存读），跨进程传播仍不承诺毫秒级。数据已写回 §2.3/§3.4；内联拦截 P95 远低于 1ms，裁定确认"<1ms"=决策点内联拦截语义成立，00_index §3.2 口径已按裁定修订（00_index Q5 同步关闭） |
| Q2 | Agent Challenge 的实现形式 | 已裁定 2026-08-18 | 裁定：交叉会话复审为主形态（另一 AI 会话做对齐判定），challenge 工单为统一落盘载体与降级形态（交叉会话不可用/超时时直接进人审队列），人兜底不可省（61 号备忘「交接落盘可追溯」与约束⑥交叉验证范式）。§4.3 Phase 2 已按裁定细化（工单字段=原始任务意图复述+当前动作链对齐说明+检测器证据；流转=检测器判疑→落盘工单→交叉会话复审→争议/RED/超时→人审） |
| Q3 | 三套自治等级标尺的统一方案 | 已裁定 2026-08-18 | 裁定：①00_index 有界自治 L0~L3（+L4 保留不启用）为项目唯一主标尺（管「Agent 能做什么」）；②PS-VOC-021 词表 l3 描述与主标尺封顶对齐（已改：中风险封顶，全权自主不启用）；③AutonomyMaturity 映射定稿——L0 OBSERVE=L0；L1 NOTIFY+L2 SUGGEST=L1 两个亚档；L3 AUTO_MINOR=L2；L4 AUTO_FULL=保留不启用；④AutonomyGuard level1~3 为正交维度（Owner 缺位运维白名单）不入映射；⑤16 号文 A-L 系管「运维闭环自治深度」，另一根轴，不建映射只做交叉引用。§3.3 映射表已转定稿，00_index §3.2 已补齐「L4 保留不启用」口径；13 号文模块工厂定级口径按主标尺执行 |
...
| 2026-08-18 | 0.2.3 | Owner 裁定回填（裁定 6/7/8 落点本文）：①裁定 6——§2.3「<1ms 可行性边界」将来时改实测完成时态并写入两组实测数据（2026-08-18 本机 1000 采样：内联拦截 P50=0.20μs/P95=0.30μs/P99=0.30μs；文件标志位轮询 P50=0.032ms/P95=0.045ms/P99=0.110ms，优于原 10ms~1s 预期但跨进程传播不承诺毫秒级）；§3.4「待实测」表述同步改实测确认；§6 Q1 关闭；②裁定 7——§3.2 Agent Challenge「实现方式待定」改裁定口径（交叉会话复审为主形态+challenge 工单统一落盘/降级+人兜底不可省）；§4.3 Phase 2 对应行细化（工单字段+流转）；§6 Q2 关闭；③裁定 8——§3.3 候选映射表转定稿（00_index 主标尺 L0~L3+L4 保留 / l3 描述封顶对齐 / AutonomyMaturity 映射定稿 / AutonomyGuard 正交不入映射 / 16 号文 A-L 系交叉引用）；§6 Q3 关闭 | Owner 2026-08-18 裁定批（AI-ADJ-003 回填；00_index/16 号文同批复位） |
| 2026-08-22 | 0.2.4 | 阶段三 GP0 施工后延迟实测回写：§2.3 补「Phase 0 运行时 gate 延迟实测」段（2026-08-22 本机 Python 3.12.8/Windows，最小计时脚本 1000 采样——gate 热路径（判定+审计留痕）P50=54.1µs/P95=313.5µs/P99=902.8µs；gate 纯判定 P50=22.1µs/P95=111.9µs；gate BLOCK+告警 P95=530.5µs；KS 内联拦截查询 P50=0.4µs/P95≤0.8µs/P99≤0.9µs），全部 P95<1ms 达标，§4.1 S0.4 验收口径满足、§3.4 无需修订；实测明细引用 `docs/_working/reports/2026-08-22-autonomy-gate-latency.md` | 阶段三 GP0 施工批实证回写（E0-3，#ARCH-160；S0.4 延迟实测数据落盘） |
