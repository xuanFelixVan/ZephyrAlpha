...
> | F33 | D_INFRA_OPS | D-AUTONOMY_CORE | 基础设施非自治逻辑 |
> | F36 | D-GOV_AUDIT | D-SECURITY | 专用审计域更精确 |
> | F37 | D_INFRA_OPS | D-AUTONOMY_CORE | 基础设施运维 |
...
| F25 | 数据库集成层 | 运营态 | `src/zephyr/infrastructure/db/` | 14 |
| F26 | 运行时集成 | 运营态 ⚠️ | `src/zephyr/trading/boot_hooks.py` + `boot_cron_jobs.py` + `lifecycle_manager.py`（蓝图声明`governance/lifecycle_manager/`，实际在`trading/`） | 3 |
| **L1 守护层** | | | | |
| F21 | IDE健康守护 | 运营态 | `src/zephyr/trading/ide_health_daemon.py` | 1+ |
| **L2 调度层** | | | | |
| F1 | 自动驾驶/大脑 | 运营态 | `src/zephyr/trading/autopilot.py` + `scripts/construction/start_brain.py` | 2 |
...
| F4 | 预算执行器 | 运营态 | `src/zephyr/governance/budget_engine.py` | 6 |
...
| F32 | 状态机引擎 | 运营态 | `src/zephyr/shared/state_machine.py` | 1 |
| **L4 执行层** | | | | |
| F3 | 任务系统 | 运营态 | `src/zephyr/governance/task_repo.py` | 5 |
| F11 | 上下文引擎 | 运营态 | `src/zephyr/shared/context_engine.py` | 1 |
| F12 | 知识库 | 运营态 | `src/zephyr/integration/mcp/knowledge_base_server.py` | 1 |
| F13 | MCP集群 | 运营态 | `src/zephyr/integration/mcp_server.py` + `src/zephyr/integration/mcp/` | 25+ |
| F14 | 管线编排 | 运营态 | `src/zephyr/integration/pipeline_orchestrator.py` | 5 |
| F24 | Agent Spec/Skill | 运营态 | `src/zephyr/autonomy_core/skill_*.py` | 60+ |
| F33 | 本地模型 | 运营态 | `src/zephyr/integration/local_model/` | 7 |
| **L5 治理层** | | | | |
| F6 | 漂移检测 | 运营态 | `src/zephyr/governance/drift_detector.py` | 9 |
...
| **L6 应急层** | | | | |
| F5 | 升级/A2A | 运营态 | `src/zephyr/governance/escalation_engine.py` | 7 |
| F9 | 回滚系统 | 运营态 | `src/zephyr/infrastructure/rollback/rollback_executor.py` | 80+ |
...
| 功能 | 代码文件完整路径 |
|:---:|------|
| F1 | `src/zephyr/trading/autopilot.py`（主入口：自动领取任务→执行→完成循环）<br>`scripts/construction/start_brain.py`（大脑系统启动脚本，MAPE-K调和循环+MCP集群自愈） |
...
| F4 | `src/zephyr/governance/budget_engine.py`（主入口：Token/成本/时间三维预算）<br>`src/zephyr/governance/budget_*.py`（预算管控辅助模块：budget_enforcement/budget_tracker/budget_profile_manager/budget_models/budget_handler，6文件） |
...
| F8 | `src/zephyr/security/access_control/rbac_guard.py`（主入口：身份注册+权限校验）<br>`src/zephyr/security/access_control/`（目录：80+文件，其中RBAC/权限相关11文件，其余属F2/F7/F27等） |
| F27 | `src/zephyr/infrastructure/capacity_assurance/`（目录：SLI/SLO框架+Error Budget五级响应+Kill Switch熔断，31+文件） |
| F32 | `src/zephyr/shared/state_machine.py`（主入口：通用状态机引擎+FSM验证器，1文件） |

**L4 执行层**

| 功能 | 代码文件完整路径 |
|:---:|------|
| F3 | `src/zephyr/governance/task_repo.py`（主入口：任务卡增删改查+状态机流转）<br>`src/zephyr/governance/persistence/task_repo.py`（持久化层）<br>`src/zephyr/shared/contracts/task_repository_protocol.py`（Protocol抽象接口）<br>`src/zephyr/shared/task_types.py`（类型定义）<br>`src/zephyr/shared/task_heartbeat.py`（心跳），5文件 |
| F11 | `src/zephyr/shared/context_engine.py`（主入口：AI对话上下文压缩和传递，1文件） |
| F12 | `src/zephyr/integration/mcp/knowledge_base_server.py`（主入口：AI长期记忆存取，1文件） |
| F13 | `src/zephyr/integration/mcp_server.py`（主入口：10个MCP服务器统一管理）<br>`src/zephyr/integration/mcp/`（目录：19个MCP服务器实现文件，25+文件） |
| F14 | `src/zephyr/integration/pipeline_orchestrator.py`（主入口：串联处理阶段形成闭环）<br>`src/zephyr/integration/pipeline_*.py`（管线辅助模块：pipeline_routing/pipeline_roadmap/pipeline_lock/pipeline_agent_bridge，5文件） |
...
| 功能 | 代码文件完整路径 |
|:---:|------|
| F6 | `src/zephyr/governance/drift_detector.py`（主入口：监控代码/配置偏离蓝图）<br>`src/zephyr/governance/drift_fix.py`（漂移修复）<br>`src/zephyr/governance/drift_detection/`（目录：51文件，其中7个核心文件：drift_engine/drift_models/drift_infrastructure/drift_training/drift_result_types/drift_hotfix_bypass/drift_cron_scheduler），共9关键文件 |
...
| F18 | `src/zephyr/governance/phase_manager.py`（主入口：12维度治理脚本自动执行）<br>`src/zephyr/governance/phase_*.py`（治理脚本辅助模块：phase_check_registry/phase_executor，3文件） |
...
| F29 | `src/zephyr/governance/semantic_audit/`（目录：规则文档LLM桥接+语义缓存，18文件） |
| F30 | `src/zephyr/security/adversarial_validation/`（目录：红蓝对抗验证器+修复有效性确认，23文件） |
...
| F37 | `src/zephyr/shared/lifecycle/resource_optimization_engine.py`（主入口：MAPE-K闭环+进程池化+I/O缓存+GPU监控）<br>`src/zephyr/shared/lifecycle/`（资源优化辅助模块，11文件） |

**L6 应急层**

| 功能 | 代码文件完整路径 |
|:---:|------|
| F5 | `src/zephyr/governance/escalation_engine.py`（主入口：AI间互相委托任务安全网）<br>`src/zephyr/governance/escalation_*.py`（升级/A2A辅助模块：escalation_models/escalation_metrics/escalation_loop_detector/escalation_fatigue_manager/escalation_api/escalation_smoke_tests，7文件） |
| F9 | `src/zephyr/infrastructure/rollback/rollback_executor.py`（主入口：一键撤销）<br>`src/zephyr/infrastructure/rollback/`（目录：回滚系统，80+文件） |
...
| 0.3.6 | 2026-06-24 | 功能域审查修复：①P1域ID命名格式统一为下划线（D_INFRA_OPS/D-AUTONOMY_CORE/D-GOV_AUDIT，共8处连字符→下划线，与depgraph.db真源对齐）②P2添加规划差异说明（F28/F32/F33/F36/F37共5个功能设计域归属与depgraph现状不一致，本文档为规划目标，depgraph需迁移） |
