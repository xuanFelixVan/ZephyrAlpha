...
graph LR
    ...
    subgraph L1_foundation[L1_foundation]
        ...
        D_FEEDBACK_LOOP["D_FEEDBACK_LOOP<br/>feedback_loop_engine<br/>(358模块)"]
        D_FRONTEND["D_FRONTEND<br/>前端<br/>(46模块)"]
        D_GOV_CODE_QUALITY["D_GOV_CODE_QUALITY<br/>code_quality_governance<br/>(107模块)"]
        ...
        D_ORCHESTRATOR["D_ORCHESTRATOR<br/>agent_orchestrator<br/>(79模块)"]
    ...
    subgraph L2_domain[L2_domain]
        ...
        D_GOVERNANCE["D_GOVERNANCE<br/>registry_management<br/>(1074模块)"]
        D_GOV_AUDIT["D_GOV_AUDIT<br/>audit_orchestration<br/>(293模块)"]
        D_GOV_DOCS["D_GOV_DOCS<br/>architecture_docs<br/>(96模块)"]
        D_GOV_DRIFT["D_GOV_DRIFT<br/>drift_detection<br/>(76模块)"]
        ...
        D_GOV_RULE["D_GOV_RULE<br/>rule_governance<br/>(33模块)"]
    ...
    subgraph unknown[unknown]
        D_BEHAVIORAL_AUDIT["D_BEHAVIORAL_AUDIT<br/>drift_detector_core<br/>(1模块)"]
        D_COMPLIANCE["D_COMPLIANCE<br/>compliance_gate<br/>(24模块)"]
        D_DATA["D_DATA<br/>data_source_integrator<br/>(53模块)"]
    ...
    D_INFRA_RUNTIME -->|164条 config_depends| D_SHARED
    D_AUTONOMY_PERM -->|130条 import_depends| D_SECURITY
    D_GOVERNANCE -->|107条 import_depends| D_SHARED
    D_GOV_SCRIPTS -->|96条 test_depends| D_GOV_RULE
    D_GOVERNANCE -->|95条 import_depends| D_FEEDBACK_LOOP
    D_GOVERNANCE -->|89条 import_depends| D_GOV_CODE_QUALITY
    D_GOV_AUDIT -->|81条 config_depends| D_GOVERNANCE
    D_FEEDBACK_LOOP -->|69条 import_depends| D_GOVERNANCE
    ...
    D_GOVERNANCE -->|28条 import_depends| D_FBL_VERIFICATION
    D_GOV_AUDIT -->|27条 import_depends| D_GOV_OPS_RESILIENCE
    D_AUTONOMY_CORE -->|26条 runtime| D_GOVERNANCE
    ...
    D_SHARED -->|23条 test_depends| D_GOVERNANCE
    D_INFRA_RUNTIME -->|23条 import_depends| D_GOVERNANCE
    D_GOVERNANCE -->|22条 import_depends| D_AUTONOMY_CORE
    ...
    D_FEEDBACK_LOOP -->|17条 import_depends| D_FBL_VERIFICATION
    D_GOVERNANCE -->|17条 contract| D_SECURITY
    ...
    D_FRONTEND -->|12条 import_depends| D_GOVERNANCE
    ...
    D_GOV_SCRIPTS -->|10条 config_depends| D_GOVERNANCE
    D_AUTONOMY_CORE -->|10条 test_depends| D_GOV_OPS_RESILIENCE
    D_AUTONOMY_CORE -->|10条 import_depends| D_SECURITY
    D_COMPLIANCE -->|10条 import_depends| D_GOV_AUDIT
    D_AUTONOMY_CORE -->|9条 contract| D_GOV_DOCS
    D_GOV_DRIFT -->|9条 import_depends| D_GOVERNANCE
    D_AUTONOMY_CORE -->|9条 test_depends| D_ORCHESTRATOR
    D_GOV_ENFORCEMENT -->|9条 import_depends| D_GOVERNANCE
    D_INTELLIGENCE -->|8条 import_depends| D_GOVERNANCE
    ...
    %% Top 10 依赖对
    %% 1. D_INFRA_RUNTIME -> D_SHARED: 164 条
    %% 2. D_AUTONOMY_PERM -> D_SECURITY: 130 条
    %% 3. D_GOVERNANCE -> D_SHARED: 107 条
    %% 4. D_GOV_SCRIPTS -> D_GOV_RULE: 96 条
    %% 5. D_GOVERNANCE -> D_FEEDBACK_LOOP: 95 条
    %% 6. D_GOVERNANCE -> D_GOV_CODE_QUALITY: 89 条
    %% 7. D_GOV_AUDIT -> D_GOVERNANCE: 81 条
    %% 8. D_FEEDBACK_LOOP -> D_GOVERNANCE: 69 条
    %% 9. D_AUTONOMY_CORE -> D_FBL_VERIFICATION: 67 条
    %% 10. D_AUTONOMY_CORE -> D_FEEDBACK_LOOP: 66 条
