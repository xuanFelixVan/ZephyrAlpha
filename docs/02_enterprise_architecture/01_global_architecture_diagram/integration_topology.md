...
graph LR
    ...
    subgraph L0_infrastructure[L0_infrastructure]
        ...
        D_INFRA_RUNTIME["D_INFRA_RUNTIME<br/>runtime_core<br/>(326模块)"]
    ...
    subgraph L1_foundation[L1_foundation]
        ...
        D_GOV_CODE_QUALITY["D_GOV_CODE_QUALITY<br/>code_quality_governance<br/>(116模块)"]
        ...
        D_SHARED["D_SHARED<br/>shared_services<br/>(294模块)"]
    end
    subgraph L2_domain[L2_domain]
        D_ASHARE_SIGNAL["D_ASHARE_SIGNAL<br/>ashare_signal<br/>(8模块)"]
        D_AUDITTEST["D_AUDITTEST<br/>audit_test_suite<br/>(12模块)"]
        D_AUTONOMY_PERM["D_AUTONOMY_PERM<br/>budget_enforcement<br/>(41模块)"]
        ...
        D_EX_CORE["D_EX_CORE<br/>执行核心<br/>(21模块)"]
        ...
        D_GOVERNANCE["D_GOVERNANCE<br/>registry_management<br/>(688模块)"]
        D_GOV_AUDIT["D_GOV_AUDIT<br/>audit_orchestration<br/>(276模块)"]
        D_GOV_DOCS["D_GOV_DOCS<br/>architecture_docs<br/>(97模块)"]
    ...
    subgraph unknown[unknown]
        ...
        D_DATA["D_DATA<br/>data_source_integrator<br/>(65模块)"]
        D_INFRASTRUCTURE["D_INFRASTRUCTURE<br/>shared_contracts<br/>(62模块)"]
        D_SIGLEGACY["D_SIGLEGACY<br/>signal_legacy<br/>(15模块)"]
    end

    %% 跨域依赖（按依赖数排序，最多显示 100 条）
    D_INFRA_RUNTIME -->|189条 config_depends| D_SHARED
    D_GOV_SCRIPTS -->|164条 config_depends| D_GOVERNANCE
    D_AUTONOMY_PERM -->|118条 test_depends| D_SECURITY
    D_GOV_SCRIPTS -->|102条 import_depends| D_GOV_RULE
    D_GOVERNANCE -->|90条 import_depends| D_GOV_CODE_QUALITY
    D_GOVERNANCE -->|84条 import_depends| D_SHARED
    D_GOV_SCRIPTS -->|79条 import_depends| D_SHARED
    D_GOVERNANCE -->|66条 import_depends| D_GOV_OPS_RESILIENCE
    D_AUTONOMY_CORE -->|66条 test_depends| D_FBL_VERIFICATION
    D_AUTONOMY_CORE -->|65条 runtime| D_FEEDBACK_LOOP
    D_INTEGRATION -->|65条 import_depends| D_SHARED
    D_FEEDBACK_LOOP -->|58条 import_depends| D_FBL_DIAGNOSERS
    D_GOV_AUDIT -->|58条 import_depends| D_GOV_DRIFT
    D_GOVERNANCE -->|53条 contract| D_GOV_ENFORCEMENT
    D_GOV_AUDIT -->|50条 import_depends| D_SHARED
    D_INFRASTRUCTURE -->|50条 config_depends| D_SHARED
    D_AUTONOMY_CORE -->|48条 import_depends| D_INFRA_RUNTIME
    D_GOV_ENFORCEMENT -->|48条 import_depends| D_GOV_CODE_QUALITY
    D_GOV_CODE_QUALITY -->|46条 import_depends| D_GOV_ENFORCEMENT
    D_COMPLIANCE -->|45条 import_depends| D_GOV_DRIFT
    D_GOVERNANCE -->|45条 config_depends| D_GOV_AUDIT
    D_GOV_REPAIR -->|44条 config_depends| D_GOVERNANCE
    ...
    D_GOV_AUDIT -->|28条 import_depends| D_GOV_OPS_RESILIENCE
    D_AUTONOMY_CORE -->|26条 import_depends| D_SHARED
    D_SHARED -->|25条 config_depends| D_INFRA_RUNTIME
    D_DATA -->|25条 import_depends| D_SHARED
    D_GOV_AUDIT -->|24条 test_depends| D_FBL_DIAGNOSERS
    D_GOVERNANCE -->|23条 config_depends| D_INFRA_RUNTIME
    ...
    D_GOV_AUDIT -->|20条 import_depends| D_GOVERNANCE
    D_GOVERNANCE -->|20条 runtime| D_AUTONOMY_CORE
    D_SECURITY_LLM -->|20条 test_depends| D_SHARED
    D_GOVERNANCE -->|19条 import_depends| D_GOV_RULE
    D_SECURITY_LLM -->|19条 test_depends| D_INFRA_RUNTIME
    D_INTELLIGENCE -->|19条 import_depends| D_SHARED
    D_TRADING -->|18条 import_depends| D_SHARED
    ...
    D_INFRA_RUNTIME -->|18条 import_depends| D_INTEGRATION
    D_GOVERNANCE -->|18条 import_depends| D_INTEGRATION
    ...
    D_GOVERNANCE -->|15条 import_depends| D_SECURITY
    D_FRONTEND -->|15条 import_depends| D_FEEDBACK_LOOP
    D_INFRA_RUNTIME -->|15条 import_depends| D_INFRA_A2A
    ...
    D_GOVERNANCE -->|14条 import_depends| D_INTELLIGENCE
    D_GOVERNANCE -->|14条 import_depends| D_OPS
    D_GOVERNANCE -->|14条 config_depends| D_INFRA_A2A
    D_GOVERNANCE -->|14条 config_depends| D_INFRASTRUCTURE
    D_INTEGRATION -->|14条 import_depends| D_INFRA_RUNTIME
    D_GOVERNANCE -->|14条 contract| D_GOV_DRIFT
    D_DATA -->|14条 config_depends| D_GOVERNANCE
    ...
    D_AUTONOMY_CORE -->|12条 data| D_GOVERNANCE
    D_GOV_REPAIR -->|11条 import_depends| D_INFRA_RUNTIME
    D_COMPLIANCE -->|11条 import_depends| D_GOV_AUDIT
    D_TRADING -->|11条 import_depends| D_INFRASTRUCTURE
    D_GOV_DRIFT -->|10条 import_depends| D_GOVERNANCE
    D_GOV_DOCS -->|10条 contract| D_GOVERNANCE
    D_GOV_SCRIPTS -->|10条 import_depends| D_GOV_REPAIR
    ...
    D_GOV_AUDIT -->|10条 test_depends| D_INFRA_RUNTIME
    ...
    D_GOVERNANCE -->|8条 config_depends| D_GOV_KB
    ...
    %% Top 10 依赖对
    %% 1. D_INFRA_RUNTIME -> D_SHARED: 189 条
    %% 2. D_GOV_SCRIPTS -> D_GOVERNANCE: 164 条
    %% 3. D_AUTONOMY_PERM -> D_SECURITY: 118 条
    %% 4. D_GOV_SCRIPTS -> D_GOV_RULE: 102 条
    %% 5. D_GOVERNANCE -> D_GOV_CODE_QUALITY: 90 条
    %% 6. D_GOVERNANCE -> D_SHARED: 84 条
    %% 7. D_GOV_SCRIPTS -> D_SHARED: 79 条
    %% 8. D_GOVERNANCE -> D_GOV_OPS_RESILIENCE: 66 条
    %% 9. D_AUTONOMY_CORE -> D_FBL_VERIFICATION: 66 条
    %% 10. D_AUTONOMY_CORE -> D_FEEDBACK_LOOP: 65 条
