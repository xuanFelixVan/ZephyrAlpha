...
> 规模：18 节点｜🔴设计态（红节点）10｜📄paper 实盘执行 6｜图例：橙虚线=设计态，蓝底=📄paper 实盘执行节点（D18 治理阶梯）。
...
flowchart TD
  ...
  class TDM_X_S1,TDM_X_S2,TDM_X_R1,TDM_X_R1_01,TDM_X_R1_02,TDM_X_R1_03,TDM_X_S1_06,TDM_X_S2_02,TDM_X_S2_03,TDM_X_S2_04 design;
...
| TDM-X-R1-01🔴 | 熔断分级判定 | 熔断五级状态机（判定轴=组合日盈亏+组合回撤双轴，与六段情绪的市场轴正交）： L0 正常 / L1 警戒（日亏≥2%：禁加仓） / L2 禁开仓（日亏≥4%：停新开仓+白名单窄门开启） / L3 减仓（日亏≥6%：梯度减仓） / L4 保命（组合回撤≥25% 或生死线：清仓+Owner 人工接管）。迟滞解除：当日无新低+修复当日跌幅 50% 才降一级，禁 V 型直接回满。 | 持续 | auto | — |
| TDM-X-R1-02🔴 | 熔断期减仓与持仓处置 | L3 减仓顺序（梯度非一刀切）：重亏仓（浮亏最深优先砍）→高波动仓（ATR 大）→压舱石保留（浮盈护盘属性资产最后动）。减仓指令路由 P2-04 执行；恢复规则=组合回撤修复 50% 后逐级加回风险预算，禁 V 型回满。L4 全清仓走 S1-06 绕过通道（既有边不动）。 | 盘中 | paper | — |
| TDM-X-R1-03🔴 | 护盘资产定向加仓白名单 | 三重门全过才开窄门：①处于 L2/L3 状态；②D110 超跌反转信号触发（或国家队明牌信号=ETF 天量成交/官方增持公告）； ③金字塔法分批（首笔 1/3 预算，确认收复再加）。白名单：宽基 ETF（沪深300/中证500/1000/红利）优先，银行/高股息次之；仓位=D107 尾部弹药预算，上限总资金 5-10%。风险声明：政策底≠市场底（历史滞后 40 天~半年）——分批+确认信号是铁律，严禁一次到位。 | 盘中 | paper | — |
...
**机制（怎么算）**：熔断五级状态机（判定轴=组合日盈亏+组合回撤双轴，与六段情绪的市场轴正交）： L0 正常 / L1 警戒（日亏≥2%：禁加仓） / L2 禁开仓（日亏≥4%：停新开仓+白名单窄门开启） / L3 减仓（日亏≥6%：梯度减仓） / L4 保命（组合回撤≥25% 或生死线：清仓+Owner 人工接管）。迟滞解除：当日无新低+修复当日跌幅 50% 才降一级，禁 V 型直接回满。

**依据锚**：风险限额 RLM-KILL-SWITCH-006、RLM-KILLSW-005、RLM-KILL-SWITCH-004、RLM-KILLSW-003 ｜ 阈值 THD-DRAWDOWN-001、THD-DRAWDOWN-002、THD-DRAWDOWN-003
**治理**：激活=continuous ｜ 失效=L4 触发后仅 Owner 人工解除可回 L0（fail-closed） ｜ 档位=auto ｜ 兜底=状态判定数据断流→维持当前级别不降级（fail-closed，D103 语义）
...
- M-55 登记（D117）：人工干预入口欠账——paper 升档前在图上增设 Owner 手动接管正式入口节点（当前仅 kill switch 人工解除一种人工通道）
- ── X-R1 血肉（D112-D114，Owner 2026-09-07 终裁；熔断五级状态机分叉，树深 3）──
- M-36 收口：熔断从模糊词变等级表（判定=R1-01 离散状态机，D108 设计原则）；梯度减仓=R1-02；
- 护盘资产定向加仓窄门=R1-03（proposed 假说，回测验证前不生效）
- M-36 收口（D112）：全系统"熔断禁加期"统一定义=R1-01 状态机 L1 及以上持续期间（P3-01 资格门④消费本节点输出）；
- 解除=熔断本身解除即解除，无独立计时器；梯度减仓表参考机构软去杠杆阶梯（breakingalpha：DD 10-12%→80% 仓/12-15%→60%/15-18%→40%）

### TDM-X-R1-02 熔断期减仓与持仓处置 🔴

**问**：L3 级减仓先减谁、减多少、怎么逐步恢复

**机制（怎么算）**：L3 减仓顺序（梯度非一刀切）：重亏仓（浮亏最深优先砍）→高波动仓（ATR 大）→压舱石保留（浮盈护盘属性资产最后动）。减仓指令路由 P2-04 执行；恢复规则=组合回撤修复 50% 后逐级加回风险预算，禁 V 型回满。L4 全清仓走 S1-06 绕过通道（既有边不动）。

**治理**：激活=intraday ｜ 档位=paper ｜ 兜底=减仓指令通道故障→降级 X-S1-06 单仓强清通道

**设计备注（裁定/欠账原文）**：
- D113：减仓顺序=重亏仓→高波动仓→压舱石（私募实践：核心仓=大市值高分红低波减缓下跌斜率，风浪来时压舱）；
- 梯度减仓参考机构软去杠杆阶梯；恢复=修复 50% 回撤逐级加回（机构标准 recover-50% 规则）
...
**治理**：激活=intraday ｜ 失效=白名单买入后窄门条件消失（信号失效/状态升 L4）→停止后续加仓批次，存量按 X 流信号管理 ｜ 档位=paper ｜ 兜底=尾部弹药预算未启用（D107 回测验证前）→本节点整节点休眠

**设计备注（裁定/欠账原文）**：
- D114 proposed 假说（回测验证前不生效）：证据链=NT 买入降低标的波动率 3.45-5.65%/崩盘风险 -30~45%（JFQA 2023、
- La Trobe 2025）；护盘范式切换实证（2026 半年报：证金前十股东 77→22 家退出个股，诚通/国新借道宽基 ETF 维稳，
- NT 总持仓 80% 仍是银行底仓）；九次出手七次见底但政策底滞后市场底 40 天~半年（财联社复盘）→金字塔分批+确认信号铁律；
- 数据地基=ETF 成交量天量检测（需 ETF 日线数据登记）+银行指数 5 日超额（D109 已有 DS-170）
...
- 0.01 元低价股跌停仅 1 分钱秒级排队/强制限价申报）（✅ 已核实 2026-09-07 另案结论：规则已如期生效、盘面实证 10% 口径（18 个交易日 >5% 波动）；矛盾根因=stk_limit 管道口径滞后——修表欠账已清偿（2026-09-07 #ARCH-DATA-020）：生成侧 _limit_pct_of/daban rule_limit_price 按 trade_date 切片（≥2026-07-06 主板 ST=10%、此前 5%），ST 行重算回填 246406 行+周度 0.05 清零+Decimal 精确对账通过；联动参数重校已清偿（2026-09-07，判定=交易决策层零值变更、全部无需重校——D67 矩阵/封板时间→溢价映射/封成比四档的校准宇宙本为 10% 涨跌幅板，ST 主板拉平后系"恢复全域适用"非参数漂移，改值反成 ST 双真源；scenario_planner 昨涨停溢价映射数据驱动（stk_limit 已切片回填）、跌停排队注入式零 pct 假设；逐项判定表见 42 号 §3.8.1，回归锚测试 TestST10PctRecalibrationBaseline 已锁基线；matching_engine._infer_limit_pct/miniqmt_provider 竞价簿 ±10% 维持 #ARCH-DATA-020 重评条件②另案；ST 日累计买入 50 万股上限（规则 4.4.10）保留）；④碎股整单卖出拆分废单+科创板 200 股基准
...
- 分母改除权参考价（D68 联动）；ST 主板 2026-07-06 起 ±10% 同口径全域适用本矩阵/映射/量能三口径
- （校准宇宙本为 10% 涨跌幅板，此前 5% 制下高位档对 ST 结构性不可达，零值变更判定与逐项表见 42 号 §3.8.1）
- D116 新股排除（M-30 收口）：上市首日无昨收/历史量能基准→十行矩阵全部"无法计算→默认观察不动作"，首日禁开新仓；次日起正常规则
- M-24 收口：跳空分档预案挂载=本节点（大幅低开 3%/6% 分档动作表），施工归 position 域隔夜风险批次（69 号 §2.28 欠账）
