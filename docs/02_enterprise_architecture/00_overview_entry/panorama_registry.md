...
| 依赖图 depgraph | `nodes` | 6233 | 模块节点——每个 .py/.yaml/.md 文件作为一个节点（module_id/path/build_status/design_maturity），6233 个 |
| 依赖图 depgraph | `edges` | 9551 | 依赖边——节点间的依赖关系（import/契约/事件订阅），9551 条 |
| 数据流图 dataflowgraph | `dataflow_datasets` | 76 | 数据集——数据流转的「货物」（如 market_data.tick / factor.value_factor），含 scope/domain/pit_policy |
| 数据流图 dataflowgraph | `dataflow_jobs` | 1014 | 作业——处理数据的「加工者」（如 ingest.ifind_kline / compute.value_factor），含 trigger_type/run_context |
...
| 资产配置 assets（YAML→DB 同步，DB 为只读缓存） | `data_source_assets` | 18 | 外部数据源——行情/交易/风控等外部数据源资产，真源 data_sources_registry.yaml，18 个 |
...
| ID | 名称 | 状态 | 来自架构图 | 内容描述 | 真源/规划 |
|------|------|:---:|------|------|------|
| PAN-BUILT-03 | 手绘 Mermaid 图（时序/治理） | ✅已建 | 手工 | 端到端时序图（application_flows）+ 治理 d2b 闭环（governance_views）。拓扑/数据流已由生成视图 integration_topology.md / dataflow_index.md 取代（v3.4.0 删除 topology_views.md / dataflow_views.md）；C4-L3 域组件图已删除（v3.5.0 删除 c4_component_views.md，有效组件模式已登记 depgraph 设计态） | 真源：手工<br>生成器：`(手工维护)`<br>产物：[`target_architecture/application_flows.md`](../target_architecture/application_flows.md) |
