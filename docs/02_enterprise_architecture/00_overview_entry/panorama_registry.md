...
| 依赖图 depgraph | `nodes` | 4981 | 模块节点——每个 .py/.yaml/.md 文件作为一个节点（module_id/path/build_status/design_maturity），4986 个 |
| 依赖图 depgraph | `edges` | 5982 | 依赖边——节点间的依赖关系（import/契约/事件订阅），5946 条 |
...
| 决策流图 decisiongraph | `decision_layers` | 10 | 决策层——L0-L6 七层决策链（如 L0 信号源 / L3 组合优化 / L6 执行），214 个节点的分层归属 |
