...
>
> **自动生成**：本文件由 `generate_navigation_index.py` 自动生成，每次全景图更新后自动刷新。最后更新：2026-08-04 17:41:44
...
| `02_domain_architecture_docs/` | 每个功能域的详细文档和依赖图，共 73 个文件 | 自动生成 | 全景图更新时 |
...
| 层级 | 域数量 | 代表域 |
|------|:---:|--------|
| 基础设施层 | 8 | D_CONTRACTS（共享契约）、D_INFRASTRUCTURE（跨层契约基础设施）、D_INFRA_A2A（A2A通信） 等 |
| 基础层 | 20 | D_ALT_DATA（另类数据）、D_AUTONOMY_CORE（自治核心）、D_DATA（数据接入层） 等 |
| 业务域层 | 44 | D_ARCHIVE_SCRIPTS（Archived Scripts）、D_ARCH_GUARD（架构守护脚本）、D_ARCH_SCRIPTS（架构治理脚本） 等 |
