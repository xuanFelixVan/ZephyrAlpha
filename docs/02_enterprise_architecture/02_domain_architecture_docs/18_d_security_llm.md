...
graph TD
    ...
    D_GOVERNANCE -.->|contract / contract| src_zephyr_security_llm_defense_llm_security_poisoning_monitor_py
    D_GOVERNANCE -.->|contract / contract| src_zephyr_security_llm_defense_llm_security_poisoning_monitor_py
    D_AUTONOMY_CORE["(生产态 / production) D_AUTONOMY_CORE"]
    D_AUTONOMY_CORE -->|runtime / runtime| src_zephyr_security_llm_defense_llm_security_poisoning_monitor_py
    D_INFRA_TELEMETRY["(生产态 / production) D_INFRA_TELEMETRY"]
    D_INFRA_TELEMETRY -->|runtime / runtime| src_zephyr_security_llm_defense_llm_security_poisoning_monitor_py
    D_INTEGRATION_GATEWAY["(生产态 / production) D_INTEGRATION_GATEWAY"]
    ...
    class D_SHARED,D_GOVERNANCE,D_AUTONOMY_CORE,D_INFRA_TELEMETRY,D_INTEGRATION_GATEWAY external_prod
    class D_AUDITTEST external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_AUTONOMY_CORE,D_INFRA_TELEMETRY,D_INTEGRATION,D_INTEGRATION_GATEWAY,D_TRADING external_prod
```
...
| 41 | D_AUTONOMY_CORE 自治核心: MOD-INF-019: Agent Spec — Agent Observability ... | → | poisoning_monitor.py — Embed 污染检测 (DD97, T... | runtime / runtime |
| 42 | D_AUTONOMY_CORE 自治核心: ContextInjector: retrieve and inject relevant k... | → | gateway.py | 导入依赖 / import_depends |
| 43 | D_GOVERNANCE 生命周期管理: blueprint.md | → | poisoning_monitor.py — Embed 污染检测 (DD97, T... | contract / contract |
| 44 | D_GOVERNANCE 生命周期管理: blueprint.md | → | poisoning_monitor.py — Embed 污染检测 (DD97, T... | contract / contract |
...
| 50 | D_INFRA_TELEMETRY 可观测性: otel_instrumentation.py — 全链路 OTel (B12, DD... | → | poisoning_monitor.py — Embed 污染检测 (DD97, T... | runtime / runtime |
