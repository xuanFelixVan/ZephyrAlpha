...
flowchart TD
    ...
    src_zephyr_sell_decision_core_breakout_failure_detector_py["core/breakout_failure_detector<br/>Breakout Failure Detector — 突破成败检测器<br/>(MOD-SELL-003)<br/>⛔ 卖出决策域，设计已就绪，等待开发排期<br/>文件: core/breakout_failure_detector.py<br/>(设计态 / design)"]
    ...
    src_zephyr_sell_decision_core_sell_urgency_scorer_py["core/sell_urgency_scorer<br/>Sell Urgency Scorer — 卖出紧迫度评分器<br/>(MOD-SELL-009)<br/>⛔ 卖出决策域，设计已就绪，等待开发排期<br/>文件: core/sell_urgency_scorer.py<br/>(设计态 / design)"]
    ...
    src_zephyr_sell_decision_core_buy_sell_conflict_arbitrator_py["core/buy_sell_conflict_arbitrator<br/>卖出决策/核心包的buy_sell_conflict_arbitrator模<br/>块<br/>⛔ 卖出决策域，设计已就绪，等待开发排期<br/>文件: core/buy_sell_conflict_arbitrator.py<br/>(设计态 / design)"]
    ...
    src_zephyr_sell_decision_core_sell_signal_fusion_engine_py["core/sell_signal_fusion_engine<br/>Sell Signal Fusion Engine — 卖出信号融合引擎<br/>(MOD-SELL-007)<br/>⛔ 卖出决策域，设计已就绪，等待开发排期<br/>文件: core/sell_signal_fusion_engine.py<br/>(设计态 / design)"]
    ...
    src_zephyr_sell_decision_core_sell_signal_collector_py["core/sell_signal_collector<br/>Sell Signal Collector — 卖出信号收集器<br/>(MOD-SELL-001)<br/>⛔ 卖出决策域，设计已就绪，等待开发排期<br/>文件: core/sell_signal_collector.py<br/>(设计态 / design)"]
...
|:--:|---------|:--:|---------|---------|
| 1 | Sell Signal Fusion Engine — 卖出信号融合引擎 (MOD-SELL-0... | → | D_POSITION 仓位管理: Sell-Position Bidirectional Link — 卖出-仓位双向链接 (MO... | runtime / runtime |
| 2 | Replacement & Rebalance Seller — 置换与再平衡卖出 (MOD-S... | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Traditional Exce... | 导入依赖 / import_depends |
| 3 | Sell Conflict Arbitrator — 买卖冲突仲裁器 (MOD-SELL-008)... | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Traditional Exce... | 导入依赖 / import_depends |
