...
| 2 | Sell Conflict Arbitrator — 买卖冲突仲裁器 (MOD-SELL-008)... | → | D_SHARED 共享服务: 错误 / errors (foundation/errors.py) | 导入依赖 / import_depends |
...
> 本域与 3 个外部域直接连接（出边 2 条 + 入边 4 条 = 6 条）。只显示直接连接的域，不展开具体节点。
