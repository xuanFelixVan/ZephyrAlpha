...
flowchart TD
    ...
    src_zephyr_sell_decision_core_sell_signal_accuracy_monitor_py["卖出信号准确率监控器<br/>统计每类卖出信号触发后N天的准确率，分析假阳性<br/>(卖了又涨=止损过紧)和假阴性<br/>(没卖继续跌=信号漏判)，按信号类型和策略类型分组<br/>，动态调整信号权重让准的信号更重、不准的更轻。<br/>文件: core/sell_signal_accuracy_monitor.py<br/>(设计态 / design)"]
    ...
    src_zephyr_sell_decision_core_sell_execution_quality_tracker_py["卖出执行质量追踪器<br/>追踪每笔卖出执行的滑点、冲击成本、执行延迟、分批<br/>执行效果，计算执行质量评分反馈给卖出执行策略，让<br/>下次卖得更便宜更快。<br/>文件: core/sell_execution_quality_tracker.py<br/>(设计态 / design)"]
    src_zephyr_sell_decision_core_sell_strategy_ab_tester_py["卖出策略A/B测试器<br/>把不同止盈线(8%vs10%)、止损范式<br/>(移动vs固定)、退出方式(分批vs一次性)做A<br/>/B对照实验，统计显著性检验后产出参数调整建议，让<br/>卖出策略参数靠数据说话而非拍脑袋。<br/>文件: core/sell_strategy_ab_tester.py<br/>(设计态 / design)"]
