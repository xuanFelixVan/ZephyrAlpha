...
| 5 | src/zephyr/pf_core/vwap_reversion_strategy.py | D_PORTFOLIO_CORE — VWAP 回归做T策略（路径 B 策略） | 生产态 / production |  |
...
| 4 | tests/pf_core/test_vwap_reversion_strategy.py | VWAPReversionStrategy 单元测试（路径 B 均值回归... | 生产态 / production |  |
...
| 6 | VWAPReversionStrategy 单元测试（路径 B 均值回归... | → | D_BACKTEST 回测: 共享撮合逻辑模块（回测=实盘一致性核心） (matchi... | 测试依赖 / test_depends |
| 7 | D_PORTFOLIO_CORE — StrategyRunner 策略运行器（... | → | D_FACTOR 因子: D-FACTOR-ANA-10 多因子合成——将多个因子值合成.... | 导入依赖 / import_depends |
| 8 | D_PORTFOLIO_CORE — StrategyRunner 策略运行器（... | → | D_FACTOR 因子: D-FACTOR-03 因子评估回测运行器——端到端因子评... | 导入依赖 / import_depends |
| 9 | D_PORTFOLIO_CORE — StrategyRunner 策略运行器（... | → | D_FACTOR 因子: ZephyrAlpha — D_FACTOR Alpha Factor Layer (fac... | 导入依赖 / import_depends |
| 10 | D_PORTFOLIO_CORE — StrategyRunner 策略运行器（... | → | D_GOVERNANCE 生命周期管理: D_PORTFOLIO_CORE — StrategyBase + StrategyMeta... | 导入依赖 / import_depends |
| 11 | D_PORTFOLIO_CORE — Portfolio Construction Stra... | → | D_PF_ALLOC 组合分配: D_PORTFOLIO_CORE — Default Equity Long-Only St... | 导入依赖 / import_depends |
...
> 本域与 7 个外部域直接连接（出边 13 条 + 入边 1 条 = 14 条）。只显示直接连接的域，不展开具体节点。
