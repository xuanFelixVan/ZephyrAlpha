...
flowchart TD
    ...
    src_zephyr_pf_core_multi_track_fusion_py ~~~ src_zephyr_pf_core_optimizer
    src_zephyr_pf_core_optimizer ~~~ src_zephyr_pf_core_strategy_engine_init_py
    ...
    src_zephyr_pf_core_meta_router -.->|import / import| src_zephyr_pf_core_optimizer
    src_zephyr_pf_core_meta_router -.->|import / import| src_zephyr_pf_core_multi_track_fusion_py
    ...
    src_zephyr_pf_core_decision_orchestrator_py -.->|import / import| src_zephyr_pf_core_optimizer
    ...
    src_zephyr_pf_core_multi_track_fusion_py -.->|contract / contract| D_RISK
    D_INFRASTRUCTURE["跨层契约基础设施<br/>跨层契约基础设施，负责跨层契约定义、共享契约管理<br/>和契约校验<br/>Cross-Layer Contract Infrastructure<br/>跨域节点 / cross-domain<br/>(生产态 / production)"]
    src_zephyr_pf_core_optimizer -.->|contract / contract| D_INFRASTRUCTURE
    ...
    src_zephyr_pf_core_multi_track_fusion_py -.->|导入依赖 / import_depends| D_PF_ALLOC
    D_GOVERNANCE["生命周期管理<br/>生命周期管理，负责蓝图/模块<br/>/任务的声明周期管理和元数据治理<br/>Lifecycle Management<br/>跨域节点 / cross-domain<br/>(生产态 / production)"]
    src_zephyr_pf_core_optimizer -.->|导入依赖 / import_depends| D_GOVERNANCE
...
| 14 | 优化器 (optimizer/) | → | D_GOVERNANCE 生命周期管理: 策略基类 / D_PORTFOLIO_CORE — StrategyBase + StrategyMet... | 导入依赖 / import_depends |
...
| 16 | 决策编排器 (pf_core/decision_orchestrator.py) | → | D_INFRASTRUCTURE 跨层契约基础设施: 绩效attribution报告 / performance_attribution_report (con... | contract / contract |
| 17 | 元路由器 (meta_router/) | → | D_INFRASTRUCTURE 跨层契约基础设施: contracts/target_portfolio.py | contract / contract |
| 18 | 优化器 (optimizer/) | → | D_INFRASTRUCTURE 跨层契约基础设施: 策略生命周期事件 / strategy_lifecycle_event (contracts/st... | contract / contract |
| 19 | 决策编排器 (pf_core/decision_orchestrator.py) | → | D_PF_ALLOC 组合分配: Strategy Correlation Gate — 策略相关性门禁 (MOD-PA-004) ... | 导入依赖 / import_depends |
| 20 | 多轨道融合器 (pf_core/multi_track_fusion.py) | → | D_PF_ALLOC 组合分配: Strategy Correlation Gate — 策略相关性门禁 (MOD-PA-004) ... | 导入依赖 / import_depends |
...
| 25 | 决策编排器 (pf_core/decision_orchestrator.py) | → | D_RISK 风控: Risk Decomposition Engine — 风险分解引擎 (MOD-RK-16) (co... | 导入依赖 / import_depends |
| 26 | 元路由器 (meta_router/) | → | D_RISK 风控: Risk Budget Allocator — 风险预算分配器 (MOD-RK-08) (core... | 导入依赖 / import_depends |
| 27 | 元路由器 (meta_router/) | → | D_RISK 风控: Risk Decomposition Engine — 风险分解引擎 (MOD-RK-16) (co... | 导入依赖 / import_depends |
| 28 | 多轨道融合器 (pf_core/multi_track_fusion.py) | → | D_RISK 风控: 风险limits / D_RISK — Risk Limits Calculator (risk/risk_... | contract / contract |
| 29 | 优化器 (optimizer/) | → | D_RISK 风控: 风险limits / D_RISK — Risk Limits Calculator (risk/risk_... | 导入依赖 / import_depends |
