...
| 5 | src/zephyr/shared/_cross_layer/ml_experiment_pipeline.py | MLExperimentPipeline D_ML_TRAIN->实验跨层集成管道 | 生产态 / production |  |
...
> 展示全部 5 个模块（生产态 4 + 设计态 1），标签标注成熟度。
...
graph TD
    ...
    src_zephyr_shared_cross_layer_ml_experiment_pipeline_py -->|导入依赖 / import_depends| src_zephyr_ml_train_trainer_base_py
    ...
    src_zephyr_ml_train_inference_base_py -->|导入依赖 / import_depends| D_CONTRACTS
    src_zephyr_ml_train_implementations_default_inference_engine_py -->|导入依赖 / import_depends| D_CONTRACTS
    D_SHARED["(生产态 / production) D_SHARED"]
    src_zephyr_ml_train_implementations_default_inference_engine_py -->|导入依赖 / import_depends| D_SHARED
    src_zephyr_shared_cross_layer_ml_experiment_pipeline_py -->|导入依赖 / import_depends| D_SHARED
    src_zephyr_shared_cross_layer_ml_experiment_pipeline_py -->|导入依赖 / import_depends| D_SHARED
    ...
    class src_zephyr_ml_train_implementations_default_inference_engine_py,src_zephyr_ml_train_inference_base_py,src_zephyr_ml_train_trainer_base_py,src_zephyr_shared_cross_layer_ml_experiment_pipeline_py production
    class docs_03_modules_cross_layer_model_profiler_blueprint_md design
    class D_TRADING,D_CONTRACTS,D_SHARED,D_RISK,D_INTELLIGENCE external_prod
```

### 运营态子图（仅 design_maturity=production 的模块和依赖）

> 仅展示已上线运行的模块（共 4 个，4 条域内依赖）。
...
| 2 | D_ML_TRAIN — ML Inference Base (inference_base.py) | → | D_CONTRACTS 共享契约: model_serving_response.py | 导入依赖 / import_depends |
| 3 | D_ML_TRAIN — Default Inference Engine (default... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 4 | MLExperimentPipeline D_ML_TRAIN->实验跨层集成管... | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 导入依赖 / import_depends |
| 5 | MLExperimentPipeline D_ML_TRAIN->实验跨层集成管... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
...
| 5 | D_RISK 风控: ml_experiment_pipeline.py | → | MLExperimentPipeline D_ML_TRAIN->实验跨层集成管... | 导入依赖 / import_depends |
