...
graph TD
    ...
    class D_ORCHESTRATOR,D_GOV_KB,D_INTELLIGENCE external_design
```
...
| 3 | D_GOV_KB 知识库治理: __init__.py | → | citation_walker.py — 引用行走 (DD117, TASK-020... | config_depends / config_depends |
...
> 本域与 12 个外部域直接连接（出边 62 条 + 入边 13 条 = 75 条）。只显示直接连接的域，不展开具体节点。
