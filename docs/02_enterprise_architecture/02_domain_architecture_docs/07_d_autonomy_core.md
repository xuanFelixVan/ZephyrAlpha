...
graph TD
    ...
    class D_SECURITY,D_INTEGRATION,D_TRADING external_prod
    class D_GOVERNANCE,D_GOV_ENFORCEMENT,D_GOV_DRIFT,D_INFRA_A2A,D_SECURITY_LLM,D_KNOWLEDGE,D_INTELLIGENCE,D_AUDITTEST external_design
```
...
graph TD
    ...
    class D_GOVERNANCE,D_GOV_ENFORCEMENT,D_GOV_DRIFT,D_INFRA_A2A,D_SECURITY_LLM,D_KNOWLEDGE external_design
```
...
| 11 | file_autoregister.py | → | D_GOVERNANCE 生命周期管理: GateEventAdapter — GateRepo 事件适配器（DW-000... | runtime / runtime |
...
| 18 | file_autoregister.py | → | D_INFRA_A2A A2A通信: audit_logger.py | runtime / runtime |
...
| 35 | file_autoregister.py | → | D_SECURITY 对抗验证: DryRun — 权限模拟与影响分析. (dry_run.py) | runtime / runtime |
