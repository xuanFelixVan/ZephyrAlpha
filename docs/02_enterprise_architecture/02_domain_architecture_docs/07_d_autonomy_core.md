...
graph TD
    ...
    src_zephyr_autonomy_core_skills_init_py -.->|config_depends / config_depends| src_zephyr_autonomy_core_skills_skill_calibration_py
...
graph TD
    ...
    class D_INTEGRATION,D_GOV_SCRIPTS,D_EX_CORE,D_INFRASTRUCTURE external_design
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_FBL_VERIFICATION,D_ORCHESTRATOR,D_INFRA_RECOVERY,D_FBL_DETECTORS,D_INTEGRATION,D_SECURITY external_prod
```
...
graph TD
    ...
    class D_FBL_VERIFICATION,D_FBL_DIAGNOSERS,D_GOV_AUDIT,D_FBL_DETECTORS,D_INFRA_RUNTIME,D_FEEDBACK_LOOP,D_INFRA_RECOVERY,D_SECURITY,D_ORCHESTRATOR external_prod
    class D_INTEGRATION external_design
```
...
> 本域与 26 个外部域直接连接（出边 312 条 + 入边 47 条 = 359 条）。只显示直接连接的域，不展开具体节点。
