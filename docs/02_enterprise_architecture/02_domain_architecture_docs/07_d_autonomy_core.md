...
graph TD
    ...
    class D_FBL_VERIFICATION,D_GOV_DOCS,D_INFRA_A2A,D_COMPLIANCE,D_GOV_DRIFT,D_GOVERNANCE,D_FEEDBACK_LOOP,D_KNOWLEDGE,D_INFRA_RUNTIME,D_GOV_AUDIT,D_EX_CORE,D_ORCHESTRATOR,D_INTELLIGENCE external_design
```
...
graph TD
    ...
    class D_SHARED,D_FBL_DIAGNOSERS,D_INFRA_RUNTIME,D_GOV_AUDIT,D_FEEDBACK_LOOP,D_INFRA_RECOVERY,D_FBL_DETECTORS,D_SECURITY,D_ORCHESTRATOR,D_FBL_VERIFICATION,D_INTEGRATION external_prod
    class D_GOV_DOCS,D_GOVERNANCE,D_INFRASTRUCTURE,D_GOV_SCRIPTS,D_EX_CORE external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOV_OPS_RESILIENCE,D_GOV_ENFORCEMENT,D_INTEGRATION,D_OPS,D_FEEDBACK_LOOP,D_EX_CORE,D_SECURITY,D_FBL_VERIFICATION external_prod
```
...
graph TD
    ...
    class D_INFRA_RECOVERY,D_INTEGRATION,D_FBL_DIAGNOSERS,D_FEEDBACK_LOOP external_prod
```
...
graph TD
    ...
    class D_SECURITY,D_FEEDBACK_LOOP,D_ORCHESTRATOR external_prod
```
...
graph TD
    ...
    class D_INTELLIGENCE,D_INFRA_RUNTIME,D_FEEDBACK_LOOP,D_GOV_RULE,D_INTEGRATION,D_GOVERNANCE external_prod
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_INTEGRATION,D_GOV_AUDIT,D_SECURITY,D_INTELLIGENCE external_prod
...
| 12 | D_GOVERNANCE 生命周期管理: model_provider_data.py | → | adversarial_robustness.py — 对抗鲁棒性 (B8, DD... | contract / contract |
...
| 37 | D_GOV_DOCS 架构文档治理: blueprint.md | → | adversarial_robustness.py — 对抗鲁棒性 (B8, DD... | contract / contract |
...
| 45 | D_INFRA_RUNTIME 运行时集成: 包 shared.dependency 的初始化文件。 (__init__.py) | → | dependency_tracker.py — 依赖追踪 (DD116, TASK-... | config_depends / config_depends |
| 46 | D_INFRA_RUNTIME 运行时集成: shared.io — auto-generated package init. (__in... | → | cache_invalidation.py — 缓存一致性 (DD113, TAS... | config_depends / config_depends |
| 47 | D_INFRA_RUNTIME 运行时集成: boot_hooks.py | → | MOD-INF-019: Agent Spec — Skill Freshness Exte... | 导入依赖 / import_depends |
| 48 | D_INFRA_RUNTIME 运行时集成: boot_hooks.py | → | MOD-INF-019: Agent Spec — Skill Lifecycle (ski... | 导入依赖 / import_depends |
| 49 | D_INFRA_RUNTIME 运行时集成: test_cold_start_booster.py | → | cold_start_booster.py — 冷启动 (DD107, TASK-01... | 测试依赖 / test_depends |
| 50 | D_INTEGRATION 管线路由: SentinelServer: 意图路由哨兵 MCP Server (sentin... | → | IntentKeywordMapper - Stage 1 of three-stage in... | 导入依赖 / import_depends |
| 51 | D_INTEGRATION 管线路由: PipelineOrchestrator — M1-M11 管线协调器 (pipe... | → | PipelineSkillBridge — Agent Spec -> Pipeline .... | 导入依赖 / import_depends |
| 52 | D_INTEGRATION 管线路由: PipelineOrchestrator — M1-M11 管线协调器 (pipe... | → | MOD-INF-019: Agent Spec — Skill Feedback Loop ... | 导入依赖 / import_depends |
| 53 | D_INTELLIGENCE 上下文管理: KB->VMS 同步引擎 — sync_to_vms() 生产者 (sync_... | → | VectorBridge — CE↔VMS 检索桥接 (Connect CT-CE... | 导入依赖 / import_depends |
| 54 | D_INTELLIGENCE 上下文管理: test_pipeline_bridge.py | → | PipelineSkillBridge — Agent Spec -> Pipeline .... | 测试依赖 / test_depends |
| 55 | D_INTELLIGENCE 上下文管理: test_pipeline_bridge.py | → | trigger_router.py | 测试依赖 / test_depends |
| 56 | D_KNOWLEDGE 知识管理: test_knowledge_distiller.py | → | knowledge_distiller.py — 知识蒸馏 (B10, DD84, ... | 测试依赖 / test_depends |
| 57 | D_ORCHESTRATOR 代理编排器: Orc->CE 上下文桥接 — request_context() 生产者 ... | → | CE 向量写入器 — vectorize_and_store() 生产者 (... | 导入依赖 / import_depends |
| 58 | D_ORCHESTRATOR 代理编排器: Orc->VMS 记忆写入器 (memory_writer.py) | → | VectorBridge — CE↔VMS 检索桥接 (Connect CT-CE... | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 29 个外部域直接连接（出边 343 条 + 入边 58 条 = 401 条）。只显示直接连接的域，不展开具体节点。
