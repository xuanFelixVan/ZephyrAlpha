...
graph TD
    ...
    class D_SHARED,D_INTEGRATION,D_INFRA_RUNTIME external_prod
    class D_COMPLIANCE,D_GOV_DRIFT,D_GOVERNANCE,D_GOV_DOCS,D_FEEDBACK_LOOP,D_SECURITY_LLM,D_KNOWLEDGE,D_GOV_AUDIT,D_EX_CORE,D_ORCHESTRATOR,D_INTELLIGENCE external_design
```
...
graph TD
    ...
    class D_SHARED,D_FEEDBACK_LOOP,D_INFRA_RECOVERY,D_FBL_DETECTORS,D_SECURITY,D_ORCHESTRATOR,D_INFRA_RUNTIME,D_GOV_AUDIT,D_FBL_DIAGNOSERS,D_FBL_VERIFICATION external_prod
    class D_INTEGRATION,D_EX_CORE,D_GOV_SCRIPTS,D_INFRASTRUCTURE external_design
```
...
graph TD
    ...
    class D_TRADING,D_GOV_OPS_RESILIENCE,D_INFRA_RUNTIME,D_INFRA_RECOVERY,D_FEEDBACK_LOOP,D_GOV_CODE_QUALITY,D_FBL_DIAGNOSERS external_prod
...
graph TD
    ...
    class D_FBL_VERIFICATION,D_ORCHESTRATOR,D_GOV_DRIFT,D_GOVERNANCE,D_INFRA_RUNTIME,D_SHARED external_prod
```
...
graph TD
    ...
    class D_GOV_OPS_RESILIENCE,D_GOV_ENFORCEMENT,D_INTEGRATION,D_SHARED,D_FEEDBACK_LOOP,D_EX_CORE,D_SECURITY,D_FBL_VERIFICATION external_prod
```
...
graph TD
    ...
    class D_FEEDBACK_LOOP,D_FBL_DIAGNOSERS,D_FBL_DETECTORS,D_INTEGRATION external_prod
```
...
graph TD
    ...
    class D_FEEDBACK_LOOP,D_INFRA_RUNTIME,D_GOVERNANCE,D_INTEGRATION,D_GOV_RULE,D_INTELLIGENCE external_prod
```
...
graph TD
    ...
    src_zephyr_autonomy_core_context_context_budget_tracker_py -->|导入依赖 / import_depends| D_INFRA_RUNTIME
    D_GOVERNANCE["(原型态 / prototype) D_GOVERNANCE"]
    D_GOVERNANCE -.->|runtime / runtime| src_zephyr_autonomy_core_context_atomic_injector_py
    D_GOVERNANCE -.->|contract / contract| src_zephyr_autonomy_core_context_atomic_injector_py
    D_EX_CORE["(原型态 / prototype) D_EX_CORE"]
    D_EX_CORE -.->|测试依赖 / test_depends| src_zephyr_autonomy_core_context_ce_bootstrap_py
    D_GOV_AUDIT -.->|测试依赖 / test_depends| src_zephyr_autonomy_core_self_evolution_fidelity_gate_py
    D_GOV_AUDIT -.->|测试依赖 / test_depends| src_zephyr_autonomy_core_phase_planner_py
    ...
    class D_GOV_AUDIT,D_INFRA_RUNTIME,D_INTELLIGENCE,D_GOV_KB external_prod
    class D_INTEGRATION,D_SHARED,D_GOVERNANCE,D_EX_CORE,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_INFRA_A2A,D_GOV_DOCS,D_FBL_VERIFICATION,D_INFRA_RUNTIME,D_KNOWLEDGE,D_GOVERNANCE,D_SECURITY_LLM,D_FEEDBACK_LOOP,D_GOV_DRIFT,D_COMPLIANCE,D_ORCHESTRATOR external_design
```
...
| 90 | Agent Spec -> Pipeline 集成桥接层 (__init__.py) | → | D_FEEDBACK_LOOP 反馈循环引擎: FLE 持久化写入器 — 写 metrics/alerts/dispatch_... | runtime / runtime |
...
| 95 | test_auto_bootstrap.py | → | D_FEEDBACK_LOOP 反馈循环引擎: FLE 持久化写入器 — 写 metrics/alerts/dispatch_... | runtime / runtime |
...
| 158 | Agent Spec -> Pipeline 集成桥接层 (__init__.py) | → | D_GOVERNANCE 生命周期管理: depgraph_reader.py — 依赖图数据库查询工具模块 ... | data / data |
...
| 162 | test_auto_bootstrap.py | → | D_GOVERNANCE 生命周期管理: depgraph_reader.py — 依赖图数据库查询工具模块 ... | data / data |
...
| 315 | Agent Spec -> Pipeline 集成桥接层 (__init__.py) | → | D_SECURITY_LLM LLM防御: LLM Security Gateway Dashboard Module. (__init_... | runtime / runtime |
| 316 | test_auto_bootstrap.py | → | D_SECURITY_LLM LLM防御: LLM Security Gateway Dashboard Module. (__init_... | runtime / runtime |
...
| 7 | D_FEEDBACK_LOOP 反馈循环引擎: FLE 持久化写入器 — 写 metrics/alerts/dispatch_... | → | test_fl_scheduler_act.py | runtime / runtime |
| 8 | D_FEEDBACK_LOOP 反馈循环引擎: FLE 持久化写入器 — 写 metrics/alerts/dispatch_... | → | test_fl_scheduler_collect_detect.py | runtime / runtime |
| 9 | D_FEEDBACK_LOOP 反馈循环引擎: FLE 持久化写入器 — 写 metrics/alerts/dispatch_... | → | test_fl_scheduler_health.py | runtime / runtime |
| 10 | D_FEEDBACK_LOOP 反馈循环引擎: FLE 持久化写入器 — 写 metrics/alerts/dispatch_... | → | test_fl_scheduler_safety.py | runtime / runtime |
| 11 | D_FEEDBACK_LOOP 反馈循环引擎: FLE 全链路调度器 —— collect->detect->diagnose... | → | VectorBridge — CE↔VMS 检索桥接 (Connect CT-CE... | 导入依赖 / import_depends |
| 12 | D_GOVERNANCE 生命周期管理: model_provider_data.py | → | atomic_injector.py — 原子注入 (DD101, TASK-019... | runtime / runtime |
| 13 | D_GOVERNANCE 生命周期管理: model_provider_data.py | → | test_auto_bootstrap.py | runtime / runtime |
| 14 | D_GOVERNANCE 生命周期管理: Self-Benchmark (W3-7) — 5 组已知对自验证 + 引.... | → | atomic_injector.py — 原子注入 (DD101, TASK-019... | contract / contract |
| 15 | D_GOVERNANCE 生命周期管理: test_capability_check.py | → | G-CT-003: Agent Spec -> RBAC capability check. ... | 测试依赖 / test_depends |
| 16 | D_GOVERNANCE 生命周期管理: test_context_assembler_root.py | → | ContextAssembler — 上下文装配、校验、影子留档 ... | 测试依赖 / test_depends |
| 17 | D_GOVERNANCE 生命周期管理: test_context_budget_tracker.py | → | ContextBudgetTracker: token budget management w... | 测试依赖 / test_depends |
| 18 | D_GOVERNANCE 生命周期管理: Tests for zephyr.autonomy_core.context.context_... | → | context_debt_score.py — 上下文债务评分 (B19, D... | 测试依赖 / test_depends |
| 19 | D_GOVERNANCE 生命周期管理: test_context_evaluator_root.py | → | context_evaluator.py — AI 引用率评估 (TASK-014... | 测试依赖 / test_depends |
| 20 | D_GOVERNANCE 生命周期管理: test_context_evictor_root.py | → | context_evictor.py — 三维逐出器 (DD9, TASK-014... | 测试依赖 / test_depends |
| 21 | D_GOVERNANCE 生命周期管理: test_context_health_score.py | → | ContextHealthScore.py — 统一健康分 (B6, DD80, ... | 测试依赖 / test_depends |
| 22 | D_GOVERNANCE 生命周期管理: test_context_injector_root.py | → | ContextInjector: retrieve and inject relevant k... | 测试依赖 / test_depends |
| 23 | D_GOVERNANCE 生命周期管理: test_context_model_strategy.py | → | context_model_strategy.py — 模型选择策略 (DD11... | 测试依赖 / test_depends |
| 24 | D_GOVERNANCE 生命周期管理: test_context_outcome_tracker.py | → | context_outcome_tracker.py — 因果链追踪 (B14, ... | 测试依赖 / test_depends |
| 25 | D_GOVERNANCE 生命周期管理: F11 ContextPipeline 三层自动化机制测试 (test_co... | → | context_pipeline_auto.py — ContextPipeline 三.... | 测试依赖 / test_depends |
| 26 | D_GOVERNANCE 生命周期管理: test_context_pipeline_root.py | → | ContextAssembler — 上下文装配、校验、影子留档 ... | 测试依赖 / test_depends |
| 27 | D_GOVERNANCE 生命周期管理: test_context_pipeline_root.py | → | context_pipeline — Context Engine **四段流水线... | 测试依赖 / test_depends |
| 28 | D_GOVERNANCE 生命周期管理: test_context_playground.py | → | context_playground.py — 上下文沙箱 dry-run (B5... | 测试依赖 / test_depends |
| 29 | D_GOVERNANCE 生命周期管理: test_context_rot_model_root.py | → | context_rot_model.py — n² Attention 衰减数学.... | 测试依赖 / test_depends |
| 30 | D_GOVERNANCE 生命周期管理: test_context_rule_registry_root.py | → | context_rule_registry.py | 测试依赖 / test_depends |
| 31 | D_GOVERNANCE 生命周期管理: test_context_rule_registry_unit.py | → | context_rule_registry.py | 测试依赖 / test_depends |
| 32 | D_GOVERNANCE 生命周期管理: test_context_value_attribution.py | → | context_value_attribution.py — KE 级 ROI 归因 ... | 测试依赖 / test_depends |
| 33 | D_GOVERNANCE 生命周期管理: test_governance_capability_check.py | → | G-CT-003: Agent Spec -> RBAC capability check. ... | 测试依赖 / test_depends |
| 34 | D_GOV_AUDIT 审计追踪: test_ba_main.py | → | agent-spec MOD-INF-019 CLI — 蓝图->Skill 升级.... | 测试依赖 / test_depends |
| 35 | D_GOV_AUDIT 审计追踪: test_phase_planner.py | → | MOD-INF-019: Agent Spec — Phase Planner (phase... | 测试依赖 / test_depends |
| 36 | D_GOV_AUDIT 审计追踪: test_self_evolution_fidelity_gate.py | → | MOD-INF-019: Agent Spec — Self Evolution Fidel... | 测试依赖 / test_depends |
| 37 | D_GOV_CODE_QUALITY 代码质量治理: 集成协调器 — 24集成+19更新+16GitHub整合. (inte... | → | context_rule_registry.py | 导入依赖 / import_depends |
...
| 45 | D_INFRA_RUNTIME 运行时集成: shared.io — auto-generated package init. (__in... | → | cache_invalidation.py — 缓存一致性 (DD113, TAS... | config_depends / config_depends |
...
> 本域与 30 个外部域直接连接（出边 343 条 + 入边 57 条 = 400 条）。只显示直接连接的域，不展开具体节点。
