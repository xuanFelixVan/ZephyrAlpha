...
graph TD
    ...
    class D_INFRA_RUNTIME,D_SECURITY,D_INTEGRATION,D_GOV_KB,D_INTELLIGENCE,D_GOV_CODE_QUALITY external_prod
...
graph TD
    ...
    class D_FBL_VERIFICATION,D_GOV_DOCS,D_INFRA_A2A,D_COMPLIANCE,D_GOV_DRIFT,D_GOVERNANCE,D_FEEDBACK_LOOP,D_SECURITY_LLM,D_KNOWLEDGE,D_GOV_AUDIT,D_EX_CORE,D_ORCHESTRATOR,D_INTELLIGENCE external_design
```
...
graph TD
    ...
    class D_SHARED,D_FBL_VERIFICATION,D_FEEDBACK_LOOP,D_ORCHESTRATOR,D_GOV_AUDIT,D_SECURITY,D_FBL_DIAGNOSERS,D_FBL_DETECTORS,D_INFRA_RECOVERY external_prod
...
graph TD
    ...
    class D_FBL_VERIFICATION,D_INFRA_RUNTIME,D_GOV_CODE_QUALITY external_prod
    class D_GOVERNANCE,D_INFRA_A2A,D_GOV_DOCS,D_SECURITY_LLM,D_FEEDBACK_LOOP,D_COMPLIANCE,D_GOV_DRIFT external_design
```
...
graph TD
    ...
    class D_ORCHESTRATOR,D_INFRA_RUNTIME,D_FBL_VERIFICATION,D_GOV_DRIFT,D_GOVERNANCE,D_SHARED external_prod
```
...
graph TD
    ...
    tests_escalation_test_order_state_escalator_py -.->|测试依赖 / test_depends| D_EX_CORE
    ...
    class D_FEEDBACK_LOOP,D_SHARED,D_OPS,D_EX_CORE,D_GOV_OPS_RESILIENCE,D_FBL_VERIFICATION,D_INTEGRATION,D_SECURITY external_prod
```
...
graph TD
    ...
    D_FEEDBACK_LOOP["(生产态 / production) D_FEEDBACK_LOOP"]
    tests_federated_learning_test_fl_evolution_engine_py -.->|测试依赖 / test_depends| D_FEEDBACK_LOOP
...
graph TD
    ...
    class D_FBL_VERIFICATION,D_SHARED,D_FEEDBACK_LOOP,D_FBL_DIAGNOSERS,D_FBL_DETECTORS external_prod
```
...
graph TD
    ...
    class D_FEEDBACK_LOOP,D_FBL_DIAGNOSERS,D_INTEGRATION,D_ORCHESTRATOR,D_SECURITY external_prod
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_GOVERNANCE,D_GOV_RULE,D_INTEGRATION,D_FEEDBACK_LOOP,D_INTELLIGENCE external_prod
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_KNOWLEDGE,D_GOVERNANCE,D_GOV_DOCS,D_SECURITY_LLM,D_FEEDBACK_LOOP,D_GOV_DRIFT,D_COMPLIANCE,D_INFRA_A2A,D_ORCHESTRATOR external_design
```
...
| 23 | Agent Spec -> Pipeline 集成桥接层 (__init__.py) | → | D_FBL_VERIFICATION 反馈验证: _operational_gates.py | runtime / runtime |
...
| 38 | D_GOV_KB 知识库治理: __init__.py | → | citation_walker.py — 引用行走 (DD117, TASK-020... | config_depends / config_depends |
| 39 | D_GOV_REPAIR 治理修复: budget_enforcement.py | → | skill_executor.py | 导入依赖 / import_depends |
| 40 | D_GOV_SCRIPTS 脚本治理: G9 四蓝图跨模块集成合规门禁执行器. (g9_complian... | → | autonomy_core 包结构指引（ARCH-033 治本）： (__... | 导入依赖 / import_depends |
| 41 | D_GOV_SCRIPTS 脚本治理: [INVARIANTS] agent-spec 审计完整性 (audit_agent... | → | autonomy_core 包结构指引（ARCH-033 治本）： (__... | 导入依赖 / import_depends |
| 42 | D_GOV_SCRIPTS 脚本治理: test_dependency_tracker.py | → | dependency_tracker.py — 依赖追踪 (DD116, TASK-... | 测试依赖 / test_depends |
| 43 | D_INFRASTRUCTURE: test_config_safety_guard.py | → | config_safety_guard.py — 配置自毁防护 (B16, DD... | 测试依赖 / test_depends |
...
> 本域与 30 个外部域直接连接（出边 340 条 + 入边 57 条 = 397 条）。只显示直接连接的域，不展开具体节点。
