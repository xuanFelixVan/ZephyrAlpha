...
graph TD
    ...
    class D_SHARED,D_INTELLIGENCE,D_KNOWLEDGE,D_GOV_KB external_design
```
...
graph TD
    ...
    class D_FBL_DETECTORS,D_FBL_DIAGNOSERS,D_GOV_CODE_QUALITY,D_ORCHESTRATOR,D_GOV_AUDIT,D_FEEDBACK_LOOP,D_INFRA_RUNTIME external_prod
```
...
graph TD
    ...
    class D_INTELLIGENCE,D_INFRA_RUNTIME,D_GOVERNANCE,D_GOV_RULE,D_INTEGRATION external_prod
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_FBL_DIAGNOSERS,D_FBL_DETECTORS,D_FBL_VERIFICATION,D_FEEDBACK_LOOP,D_GOV_AUDIT,D_INFRA_RECOVERY,D_SECURITY,D_ORCHESTRATOR external_prod
...
| 33 | D_GOV_KB 知识库治理: __init__.py | → | citation_walker.py — 引用行走 (DD117, TASK-020... | config_depends / config_depends |
| 34 | D_GOV_REPAIR 治理修复: budget_enforcement.py | → | skill_executor.py | 导入依赖 / import_depends |
...
> 本域与 25 个外部域直接连接（出边 306 条 + 入边 48 条 = 354 条）。只显示直接连接的域，不展开具体节点。
