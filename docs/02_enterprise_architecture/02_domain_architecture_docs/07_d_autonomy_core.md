...
graph TD
    ...
    class D_GOV_SCRIPTS,D_GOV_AUDIT,D_EX_CORE,D_GOVERNANCE external_design
```
...
graph TD
    ...
    class D_GOV_AUDIT,D_GOV_RULE,D_SHARED,D_GOV_REPAIR,D_INFRA_RUNTIME external_prod
...
graph TD
    ...
    D_GOV_DOCS -.->|contract / contract| src_zephyr_security_llm_defense_llm_security_adversarial_robustness_py
    D_GOV_DOCS -.->|runtime / runtime| src_zephyr_security_llm_defense_llm_security_adversarial_robustness_py
    D_INFRA_RUNTIME -->|runtime / runtime| src_zephyr_security_llm_defense_llm_security_adversarial_robustness_py
    ...
    class D_INTEGRATION,D_INFRA_RUNTIME,D_FBL_DETECTORS,D_FBL_DIAGNOSERS,D_FBL_VERIFICATION,D_GOV_AUDIT,D_FEEDBACK_LOOP external_prod
    class D_SHARED,D_GOV_DOCS,D_ORCHESTRATOR,D_EX_CORE,D_INFRASTRUCTURE,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_ORCHESTRATOR,D_GOV_AUDIT,D_FEEDBACK_LOOP,D_FBL_DETECTORS,D_FBL_DIAGNOSERS,D_INFRA_RUNTIME,D_GOV_CODE_QUALITY,D_TRADING,D_FBL_VERIFICATION external_prod
```
...
graph TD
    ...
    class D_FBL_VERIFICATION,D_ORCHESTRATOR,D_GOVERNANCE,D_GOV_DRIFT,D_INFRA_RUNTIME,D_SHARED external_prod
```
...
graph TD
    ...
    class D_GOV_OPS_RESILIENCE,D_INTEGRATION,D_SHARED,D_INFRA_RUNTIME,D_GOVERNANCE,D_GOV_ENFORCEMENT,D_OPS,D_SECURITY external_prod
```
...
graph TD
    ...
    tests_federated_learning_test_fl_slo_manager_py -.->|测试依赖 / test_depends| D_FEEDBACK_LOOP
    D_GOV_DOCS["(设计态 / design) D_GOV_DOCS"]
    ...
    class D_FEEDBACK_LOOP,D_FBL_DETECTORS,D_FBL_DIAGNOSERS,D_FBL_VERIFICATION external_prod
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_INTEGRATION,D_GOV_AUDIT,D_GOV_KB,D_INTELLIGENCE external_prod
    class D_SHARED,D_GOV_DOCS,D_INFRASTRUCTURE,D_GOV_SCRIPTS,D_EX_CORE,D_GOVERNANCE external_design
```
...
graph TD
    ...
    class D_SHARED,D_INFRA_RUNTIME,D_FBL_DETECTORS,D_FBL_DIAGNOSERS,D_FBL_VERIFICATION,D_GOV_AUDIT,D_FEEDBACK_LOOP,D_SECURITY,D_INFRA_RECOVERY external_prod
...
| 272 | test_task_model_learner.py | → | D_INTELLIGENCE 上下文管理: Model Profiling — 本地 + 远程模型性能基准测试 ... | 测试依赖 / test_depends |
...
| 35 | D_GOV_DOCS 架构文档治理: blueprint.md | → | adversarial_robustness.py — 对抗鲁棒性 (B8, DD... | contract / contract |
| 36 | D_GOV_DOCS 架构文档治理: blueprint.md | → | adversarial_robustness.py — 对抗鲁棒性 (B8, DD... | runtime / runtime |
...
| 43 | D_INFRA_RUNTIME 运行时集成: ZephyrAlpha — system-telemetry/contract_metric... | → | adversarial_robustness.py — 对抗鲁棒性 (B8, DD... | runtime / runtime |
...
> 本域与 27 个外部域直接连接（出边 317 条 + 入边 57 条 = 374 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_INFRA_RUNTIME -->|6条 config_depends / config_depends, 导入依赖 / import_depends, runtime / runtime, 测试依赖 / test_depends| D_AUTONOMY_CORE
