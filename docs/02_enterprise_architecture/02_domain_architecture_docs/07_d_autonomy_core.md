...
graph TD
    ...
    class D_SHARED,D_INFRA_RUNTIME,D_INTEGRATION,D_FEEDBACK_LOOP,D_SECURITY external_prod
...
graph TD
    ...
    class D_SHARED,D_INTEGRATION,D_INFRA_RUNTIME,D_FEEDBACK_LOOP,D_GOV_CODE_QUALITY,D_SECURITY,D_GOV_REPAIR external_prod
...
| 3 | D_GOV_KB 知识库治理: __init__.py | → | embedding_version_lock.py — 嵌入模型版本锁 (B1... | config_depends / config_depends |
...
> 本域与 12 个外部域直接连接（出边 63 条 + 入边 13 条 = 76 条）。只显示直接连接的域，不展开具体节点。
