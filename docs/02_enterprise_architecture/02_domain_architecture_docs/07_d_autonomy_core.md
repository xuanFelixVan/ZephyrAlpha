...
graph TD
    ...
    class D_INFRA_RUNTIME,D_INTEGRATION,D_GOVERNANCE,D_INTELLIGENCE,D_SHARED,D_SECURITY_LLM external_prod
...
graph TD
    ...
    class D_SECURITY,D_SECURITY_LLM,D_INTEGRATION,D_TRADING external_prod
...
| 8 | file_autoregister.py | → | D_GOVERNANCE 生命周期管理: blueprint.md | contract / contract |
| 9 | file_autoregister.py | → | D_GOVERNANCE 生命周期管理: Blueprint Bloat Monitor — v0.11.0 蓝图膨胀监控... | runtime / runtime |
| 10 | file_autoregister.py | → | D_GOVERNANCE 生命周期管理: GateEventAdapter — GateRepo 事件适配器（DW-000... | runtime / runtime |
...
| 17 | file_autoregister.py | → | D_INFRA_A2A A2A通信: G-CT-008 契约：A2A -> Audit 审计 Agent 间通信. ... | runtime / runtime |
...
| 24 | file_autoregister.py | → | D_INFRA_RUNTIME 运行时集成: blueprint.md | runtime / runtime |
...
| 35 | file_autoregister.py | → | D_SECURITY 对抗验证: InputGuard — 输入参数守卫. (input_guard.py) | runtime / runtime |
| 36 | ContextInjector: retrieve and inject relevant k... | → | D_SECURITY_LLM LLM防御: gateway.py | 导入依赖 / import_depends |
| 37 | file_autoregister.py | → | D_SECURITY_LLM LLM防御: adversarial_robustness.py — 对抗鲁棒性 (B8, DD... | runtime / runtime |
