...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_ENFORCEMENT,D_INTEGRATION,D_TRADING external_prod
...
graph TD
    ...
    class D_GOVERNANCE,D_AUDITTEST,D_GOV_ENFORCEMENT,D_GOV_DRIFT,D_INFRA_A2A,D_SECURITY_LLM external_design
```
...
| 12 | file_autoregister.py | → | D_GOVERNANCE 生命周期管理: Construction Verifier — 施工验证器: 任务卡完成... | runtime / runtime |
| 13 | skill_executor.py | → | D_GOVERNANCE 生命周期管理: writer.py | 导入依赖 / import_depends |
| 14 | MOD-INF-019: Agent Spec — Skill Sandbox (skill... | → | D_GOVERNANCE 生命周期管理: bridge.py | 导入依赖 / import_depends |
| 15 | MOD-INF-019: Agent Spec — SpecEngine 蓝图->Ski... | → | D_GOVERNANCE 生命周期管理: writer.py | 导入依赖 / import_depends |
| 16 | file_autoregister.py | → | D_GOV_DRIFT 漂移检测: blueprint.md | runtime / runtime |
| 17 | file_autoregister.py | → | D_GOV_ENFORCEMENT 规则执行: Audit Trail — MOD-INF-020 (__init__.py) | runtime / runtime |
| 18 | skill_executor.py | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 导入依赖 / import_depends |
| 19 | file_autoregister.py | → | D_INFRA_A2A A2A通信: G-CT-008 契约：A2A -> Audit 审计 Agent 间通信. ... | runtime / runtime |
...
| 27 | ContextAssembler — 上下文装配、校验、影子留档 ... | → | D_INTEGRATION 管线路由: schemas.py | 导入依赖 / import_depends |
| 28 | ContextInjector: retrieve and inject relevant k... | → | D_INTEGRATION 管线路由: schemas.py | 导入依赖 / import_depends |
| 29 | context_pipeline — Context Engine **四段流水线... | → | D_INTEGRATION 管线路由: schemas.py | 导入依赖 / import_depends |
| 30 | PromptRegistry: YAML-driven Prompt 模板注册表 (... | → | D_INTEGRATION 管线路由: schemas.py | 导入依赖 / import_depends |
| 31 | skill-registry.py —— Skill 注册基座（Phase 14... | → | D_INTEGRATION 管线路由: schemas.py | 导入依赖 / import_depends |
| 32 | skill_router.py | → | D_INTEGRATION 管线路由: EmbeddingRouter — MOD-INF-011 双嵌入维度路由 (... | 导入依赖 / import_depends |
| 33 | ContextAssembler — 上下文装配、校验、影子留档 ... | → | D_INTELLIGENCE 上下文管理: Cross-Encoder 重排序层 — BGE-reranker-v2-m3（T... | 导入依赖 / import_depends |
| 34 | ContextAssembler — 上下文装配、校验、影子留档 ... | → | D_INTELLIGENCE 上下文管理: UnifiedMemoryAPI — RI-02 统一记忆 API（M2 跨模... | 导入依赖 / import_depends |
| 35 | file_autoregister.py | → | D_KNOWLEDGE 知识管理: blueprint.md | contract / contract |
| 36 | ContextInjector: retrieve and inject relevant k... | → | D_SECURITY_LLM LLM防御: gateway.py | 导入依赖 / import_depends |
| 37 | file_autoregister.py | → | D_SECURITY_LLM LLM防御: LLM Security Gateway Dashboard Module. (__init_... | runtime / runtime |
...
> 本域与 13 个外部域直接连接（出边 51 条 + 入边 139 条 = 190 条）。只显示直接连接的域，不展开具体节点。
