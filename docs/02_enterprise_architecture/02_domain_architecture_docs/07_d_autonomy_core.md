...
| 跨域出边 | 329 | Cross-domain Outgoing | 329 |
...
graph TD
    ...
    src_zephyr_autonomy_core_trigger_router_py -.->|runtime / runtime| D_FBL_VERIFICATION
    D_GOV_DOCS["(设计态 / design) D_GOV_DOCS"]
    src_zephyr_autonomy_core_trigger_router_py -.->|runtime / runtime| D_GOV_DOCS
    src_zephyr_autonomy_core_trigger_router_py -.->|runtime / runtime| D_GOV_DOCS
    D_INFRA_A2A["(原型态 / prototype) D_INFRA_A2A"]
    src_zephyr_autonomy_core_trigger_router_py -.->|runtime / runtime| D_INFRA_A2A
    D_COMPLIANCE["(原型态 / prototype) D_COMPLIANCE"]
    src_zephyr_autonomy_core_trigger_router_py -.->|runtime / runtime| D_COMPLIANCE
    D_GOV_DRIFT["(设计态 / design) D_GOV_DRIFT"]
    src_zephyr_autonomy_core_trigger_router_py -.->|runtime / runtime| D_GOV_DRIFT
    D_GOVERNANCE["(原型态 / prototype) D_GOVERNANCE"]
    src_zephyr_autonomy_core_trigger_router_py -.->|runtime / runtime| D_GOVERNANCE
    src_zephyr_autonomy_core_trigger_router_py -.->|contract / contract| D_GOV_DOCS
    D_FEEDBACK_LOOP["(原型态 / prototype) D_FEEDBACK_LOOP"]
    src_zephyr_autonomy_core_trigger_router_py -.->|runtime / runtime| D_FEEDBACK_LOOP
    src_zephyr_autonomy_core_trigger_router_py -.->|data / data| D_GOV_DOCS
    D_SECURITY["(原型态 / prototype) D_SECURITY"]
    src_zephyr_autonomy_core_trigger_router_py -.->|runtime / runtime| D_SECURITY
    src_zephyr_autonomy_core_trigger_router_py -.->|runtime / runtime| D_GOV_DOCS
    src_zephyr_autonomy_core_trigger_router_py -.->|runtime / runtime| D_GOVERNANCE
    src_zephyr_autonomy_core_trigger_router_py -.->|runtime / runtime| D_GOVERNANCE
    D_GOV_DOCS -.->|contract / contract| src_zephyr_autonomy_core_trigger_router_py
    ...
    class D_FBL_VERIFICATION,D_GOV_DOCS,D_INFRA_A2A,D_COMPLIANCE,D_GOV_DRIFT,D_GOVERNANCE,D_FEEDBACK_LOOP,D_SECURITY,D_KNOWLEDGE,D_INTELLIGENCE,D_GOV_KB external_design
```
...
graph TD
    ...
    class D_GOV_DOCS,D_INFRA_A2A,D_SECURITY,D_COMPLIANCE,D_GOV_DRIFT external_design
```
...
graph TD
    ...
    src_zephyr_autonomy_core_trigger_router_py -.->|runtime / runtime| D_INFRA_RUNTIME
    D_KNOWLEDGE["(设计态 / design) D_KNOWLEDGE"]
    src_zephyr_autonomy_core_trigger_router_py -.->|contract / contract| D_KNOWLEDGE
    D_GOVERNANCE["(原型态 / prototype) D_GOVERNANCE"]
    src_zephyr_autonomy_core_trigger_router_py -.->|runtime / runtime| D_GOVERNANCE
    src_zephyr_autonomy_core_trigger_router_py -.->|runtime / runtime| D_GOVERNANCE
    D_GOV_DOCS["(设计态 / design) D_GOV_DOCS"]
    ...
    class D_INFRA_RUNTIME,D_KNOWLEDGE,D_GOVERNANCE,D_GOV_DOCS,D_SECURITY,D_FEEDBACK_LOOP,D_GOV_DRIFT,D_COMPLIANCE,D_INFRA_A2A,D_INTELLIGENCE,D_GOV_AUDIT,D_EX_CORE external_design
```
...
graph TD
    ...
    class D_INTEGRATION,D_GOV_DOCS external_design
```
...
|:--:|---------|:--:|---------|---------|
| 1 | trigger_router.py | → | D_COMPLIANCE 合规: Audit Trail — MOD-INF-020 (__init__.py) | runtime / runtime |
...
| 4 | trigger_router.py | → | D_FBL_VERIFICATION 反馈验证: _operational_gates.py | runtime / runtime |
...
| 37 | trigger_router.py | → | D_FEEDBACK_LOOP 反馈循环引擎: FLE->Orc 告警分派器 — dispatch() 生产者 (alert... | runtime / runtime |
...
| 103 | trigger_router.py | → | D_GOVERNANCE 生命周期管理: Construction Verifier — 施工验证器: 任务卡完成... | runtime / runtime |
| 104 | trigger_router.py | → | D_GOVERNANCE 生命周期管理: model_provider_data.py | runtime / runtime |
| 105 | trigger_router.py | → | D_GOVERNANCE 生命周期管理: G-CT-008 契约：A2A -> Audit 审计 Agent 间通信. ... | runtime / runtime |
...
| 135 | trigger_router.py | → | D_GOV_DOCS 架构文档治理: blueprint.md | runtime / runtime |
| 136 | trigger_router.py | → | D_GOV_DOCS 架构文档治理: blueprint.md | data / data |
| 137 | trigger_router.py | → | D_GOV_DOCS 架构文档治理: blueprint.md | runtime / runtime |
| 138 | trigger_router.py | → | D_GOV_DOCS 架构文档治理: blueprint.md | runtime / runtime |
| 139 | trigger_router.py | → | D_GOV_DOCS 架构文档治理: blueprint.md | contract / contract |
...
| 144 | trigger_router.py | → | D_GOV_DRIFT 漂移检测: blueprint.md | runtime / runtime |
...
| 162 | trigger_router.py | → | D_INFRA_A2A A2A通信: test_a2a_check.py | runtime / runtime |
...
| 173 | PromptRegistry: YAML-driven Prompt 模板注册表 (... | → | D_INFRA_RUNTIME 运行时集成: token_budget.py — Token 估算工具 SSoT (token_b... | 导入依赖 / import_depends |
| 174 | trigger_router.py | → | D_INFRA_RUNTIME 运行时集成: blueprint.md | runtime / runtime |
...
| 251 | trigger_router.py | → | D_KNOWLEDGE 知识管理: blueprint.md | contract / contract |
...
| 263 | trigger_router.py | → | D_SECURITY 对抗验证: LLM Security Gateway - Streamlit Dashboard. (ap... | runtime / runtime |
| 264 | test_agent_creation_policy.py | → | D_SECURITY 对抗验证: AgentCreationPolicy — Agent 创建策略. (agent_c... | 测试依赖 / test_depends |
| 265 | test_auto_bootstrap.py | → | D_SECURITY 对抗验证: LLM Security Gateway - Streamlit Dashboard. (ap... | runtime / runtime |
| 266 | test_auto_maintenance.py | → | D_SECURITY 对抗验证: AutoMaintenance — 自动维护与规则健康仪表盘. (a... | 测试依赖 / test_depends |
| 267 | test_escalation_handler.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.esc... | 测试依赖 / test_depends |
| 268 | test_intent_binder_root.py | → | D_SECURITY 对抗验证: IntentBinder — 意图绑定与漂移检测. (intent_bin... | 测试依赖 / test_depends |
| 269 | test_memory_guard.py | → | D_SECURITY 对抗验证: MemoryGuard — 内存访问守卫. (memory_guard.py) | 测试依赖 / test_depends |
| 270 | test_memory_provenance_guard.py | → | D_SECURITY 对抗验证: MemoryProvenanceGuard — 记忆来源溯源守卫. (mem... | 测试依赖 / test_depends |
| 271 | test_session_lifecycle.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.ses... | 测试依赖 / test_depends |
| 272 | checkpoint_manager.py — Inject 前快照 (DD100, ... | → | D_SHARED 共享服务: serialization.py —— 统一序列化/反序列化基础设... | 导入依赖 / import_depends |
| 273 | ContextBudgetTracker: token budget management w... | → | D_SHARED 共享服务: Zero-dependency Observer pattern (subscribe/emi... | 导入依赖 / import_depends |
| 274 | ContextBudgetTracker: token budget management w... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 275 | ContextInjector: retrieve and inject relevant k... | → | D_SHARED 共享服务: async_utils.py — async/sync 边界桥接（5.12.8 .... | 导入依赖 / import_depends |
| 276 | context_pipeline_auto.py — ContextPipeline 三.... | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 导入依赖 / import_depends |
| 277 | file_autoregister.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 278 | PromptRegistry: YAML-driven Prompt 模板注册表 (... | → | D_SHARED 共享服务: constants.py —— 共享枚举 & 常量集中 re-export... | 导入依赖 / import_depends |
| 279 | skill_factory.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 280 | MOD-INF-019: Agent Spec — Skill Feedback Loop ... | → | D_SHARED 共享服务: serialization.py —— 统一序列化/反序列化基础设... | 导入依赖 / import_depends |
| 281 | MOD-INF-019: Agent Spec — Skill Freshness Exte... | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 导入依赖 / import_depends |
| 282 | skill-registry.py —— Skill 注册基座（Phase 14... | → | D_SHARED 共享服务: constants.py —— 共享枚举 & 常量集中 re-export... | 导入依赖 / import_depends |
| 283 | PatternLibrary · 成功模式库（KB refactor 后独.... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 284 | SystemSnapshotter — M1 系统状态镜像（CL-017 RI... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 285 | SystemSnapshotter — M1 系统状态镜像（CL-017 RI... | → | D_SHARED 共享服务: SQLite 连接工厂真源（SSoT） (sqlite_factory.py) | 导入依赖 / import_depends |
| 286 | DocCompressor — 文档压缩服务（CL-018 RI 扩展模... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 287 | DocCompressor — 文档压缩服务（CL-018 RI 扩展模... | → | D_SHARED 共享服务: CBAC 能力检查器 (Capability-Based Access Contro... | 导入依赖 / import_depends |
| 288 | test_auto_split.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 289 | test_ide_watcher.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 290 | test_mgmt_context_budget_tracker.py | → | D_SHARED 共享服务: Zero-dependency Observer pattern (subscribe/emi... | 测试依赖 / test_depends |
| 291 | test_escalation_contracts.py | → | D_SHARED 共享服务: budget_alert.py | 测试依赖 / test_depends |
| 292 | test_escalation_gov_budget_handler.py | → | D_SHARED 共享服务: budget_alert.py | 测试依赖 / test_depends |
| 293 | test_escalation_gov_contracts.py | → | D_SHARED 共享服务: budget_alert.py | 测试依赖 / test_depends |
| 294 | test_escalation_gov_rbac_bridge.py | → | D_SHARED 共享服务: permission.py | 测试依赖 / test_depends |
| 295 | F1 事件触发启动测试 (test_f1_event_trigger.py) | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 测试依赖 / test_depends |
...
| 34 | D_GOV_AUDIT 审计追踪: budget_enforcement.py | → | skill_executor.py | 导入依赖 / import_depends |
...
| 39 | D_GOV_DOCS 架构文档治理: blueprint.md | → | trigger_router.py | contract / contract |
| 40 | D_GOV_DOCS 架构文档治理: blueprint.md | → | test_auto_bootstrap.py | runtime / runtime |
| 41 | D_GOV_KB 知识库治理: __init__.py | → | citation_walker.py — 引用行走 (DD117, TASK-020... | config_depends / config_depends |
...
| 44 | D_INFRA_RUNTIME 运行时集成: 包 shared.dependency 的初始化文件。 (__init__.py) | → | dependency_tracker.py — 依赖追踪 (DD116, TASK-... | config_depends / config_depends |
...
> 本域与 25 个外部域直接连接（出边 329 条 + 入边 56 条 = 385 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_AUTONOMY_CORE -->|10条 导入依赖 / import_depends, runtime / runtime, 测试依赖 / test_depends| D_SECURITY
