...
graph TD
    ...
    src_zephyr_autonomy_core_agent_observability_py -->|runtime / runtime| src_zephyr_autonomy_core_context_ce_file_lister_py
    ...
    src_zephyr_autonomy_core_agent_observability_py -.->|runtime / runtime| D_AUDITTEST
    D_GOV_AUDIT["(设计态 / design) D_GOV_AUDIT"]
    src_zephyr_autonomy_core_agent_observability_py -.->|runtime / runtime| D_GOV_AUDIT
    ...
    src_zephyr_autonomy_core_agent_observability_py -.->|runtime / runtime| D_INFRA_RUNTIME
    D_GOVERNANCE -.->|contract / contract| src_zephyr_autonomy_core_context_ce_file_lister_py
    D_GOVERNANCE -.->|contract / contract| src_zephyr_autonomy_core_agent_observability_py
    D_GOVERNANCE -.->|runtime / runtime| src_zephyr_autonomy_core_context_ce_file_lister_py
    D_INFRA_TELEMETRY["(生产态 / production) D_INFRA_TELEMETRY"]
    D_INFRA_TELEMETRY -->|runtime / runtime| src_zephyr_autonomy_core_context_ce_file_lister_py
    ...
    class D_GOVERNANCE,D_AUDITTEST,D_GOV_AUDIT,D_GOV_DRIFT,D_INFRA_A2A,D_KNOWLEDGE,D_INFRA_RUNTIME,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    D_GOVERNANCE["(设计态 / design) D_GOVERNANCE"]
    D_GOVERNANCE -.->|runtime / runtime| src_zephyr_autonomy_core_self_evolution_fidelity_gate_py
...
graph TD
    ...
    class D_SECURITY_LLM,D_INFRA_TELEMETRY,D_INTEGRATION,D_TRADING external_prod
    class D_GOVERNANCE,D_AUDITTEST,D_GOV_AUDIT,D_GOV_DRIFT,D_INFRA_A2A,D_KNOWLEDGE,D_INFRA_RUNTIME,D_INTELLIGENCE,D_GOV_SCRIPTS external_design
```
...
|:--:|---------|:--:|---------|---------|
| 1 | MOD-INF-019: Agent Spec — Agent Observability ... | → | D_AUDITTEST 审计测试套件: test_a2a_check.py | runtime / runtime |
| 2 | MOD-INF-019: Agent Spec — Agent Observability ... | → | D_GOVERNANCE 生命周期管理: blueprint.md | data / data |
...
| 8 | MOD-INF-019: Agent Spec — Agent Observability ... | → | D_GOVERNANCE 生命周期管理: blueprint.md | runtime / runtime |
| 9 | MOD-INF-019: Agent Spec — Agent Observability ... | → | D_GOVERNANCE 生命周期管理: blueprint.md | contract / contract |
| 10 | skill_executor.py | → | D_GOVERNANCE 生命周期管理: writer.py | 导入依赖 / import_depends |
| 11 | MOD-INF-019: Agent Spec — Skill Sandbox (skill... | → | D_GOVERNANCE 生命周期管理: bridge.py | 导入依赖 / import_depends |
| 12 | MOD-INF-019: Agent Spec — SpecEngine 蓝图->Ski... | → | D_GOVERNANCE 生命周期管理: writer.py | 导入依赖 / import_depends |
| 13 | MOD-INF-019: Agent Spec — Agent Observability ... | → | D_GOV_AUDIT 审计追踪: blueprint.md | runtime / runtime |
| 14 | MOD-INF-019: Agent Spec — Agent Observability ... | → | D_GOV_DRIFT 漂移检测: blueprint.md | runtime / runtime |
| 15 | skill_executor.py | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 导入依赖 / import_depends |
| 16 | ContextAssembler — 上下文装配、校验、影子留档 ... | → | D_GOV_KB 知识库治理: 冷启动引导引擎 — 从存量文档自动生成首批KE（T-M... | 导入依赖 / import_depends |
| 17 | MOD-INF-019: Agent Spec — Agent Observability ... | → | D_INFRA_A2A A2A通信: audit_logger.py | runtime / runtime |
| 18 | MOD-INF-019: Agent Spec — Agent Observability ... | → | D_INFRA_RUNTIME 运行时集成: blueprint.md | runtime / runtime |
...
| 35 | MOD-INF-019: Agent Spec — Agent Observability ... | → | D_SECURITY_LLM LLM防御: poisoning_monitor.py — Embed 污染检测 (DD97, T... | runtime / runtime |
| 36 | ContextInjector: retrieve and inject relevant k... | → | D_SECURITY_LLM LLM防御: gateway.py | 导入依赖 / import_depends |
...
| 128 | D_GOVERNANCE 生命周期管理: blueprint.md | → | MOD-INF-019: Agent Spec — Self Evolution Fidel... | runtime / runtime |
| 129 | D_GOVERNANCE 生命周期管理: blueprint.md | → | MOD-INF-019: Agent Spec — Agent Observability ... | contract / contract |
| 130 | D_GOVERNANCE 生命周期管理: blueprint.md | → | list_ce_files.py — CE 文件清单生成器 (ce_file_... | runtime / runtime |
| 131 | D_GOVERNANCE 生命周期管理: blueprint.md | → | list_ce_files.py — CE 文件清单生成器 (ce_file_... | contract / contract |
...
| 136 | D_INFRA_TELEMETRY 可观测性: otel_instrumentation.py — 全链路 OTel (B12, DD... | → | list_ce_files.py — CE 文件清单生成器 (ce_file_... | runtime / runtime |
| 137 | D_INTEGRATION 管线路由: PipelineOrchestrator — M1-M11 管线协调器 (pipe... | → | PipelineSkillBridge — Agent Spec -> Pipeline .... | 导入依赖 / import_depends |
| 138 | D_INTEGRATION 管线路由: PipelineOrchestrator — M1-M11 管线协调器 (pipe... | → | MOD-INF-019: Agent Spec — Skill Feedback Loop ... | 导入依赖 / import_depends |
| 139 | D_INTELLIGENCE 上下文管理: KB->VMS 同步引擎 — sync_to_vms() 生产者 (sync_... | → | VectorBridge — CE↔VMS 检索桥接 (Connect CT-CE... | 导入依赖 / import_depends |
| 140 | D_TRADING 交易运营: boot_hooks.py | → | MOD-INF-019: Agent Spec — Skill Freshness Exte... | 导入依赖 / import_depends |
| 141 | D_TRADING 交易运营: boot_hooks.py | → | MOD-INF-019: Agent Spec — Skill Lifecycle (ski... | 导入依赖 / import_depends |
| 142 | D_TRADING 交易运营: FLE 全链路调度器 —— collect->detect->diagnose... | → | VectorBridge — CE↔VMS 检索桥接 (Connect CT-CE... | 导入依赖 / import_depends |
| 143 | D_TRADING 交易运营: Orc->VMS 记忆写入器 (memory_writer.py) | → | VectorBridge — CE↔VMS 检索桥接 (Connect CT-CE... | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 16 个外部域直接连接（出边 50 条 + 入边 143 条 = 193 条）。只显示直接连接的域，不展开具体节点。
