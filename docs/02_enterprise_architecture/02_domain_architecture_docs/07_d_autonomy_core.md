...
graph TD
    ...
    class D_SHARED,D_FBL_VERIFICATION,D_GOV_DOCS,D_INFRA_A2A,D_COMPLIANCE,D_GOV_DRIFT,D_GOVERNANCE,D_FEEDBACK_LOOP,D_INTELLIGENCE,D_ORCHESTRATOR,D_EX_CORE,D_GOV_AUDIT external_design
```
...
graph TD
    ...
    class D_GOV_AUDIT,D_GOV_RULE,D_SHARED,D_GOV_REPAIR,D_INTEGRATION,D_INFRA_RUNTIME external_prod
```
...
graph TD
    ...
    D_GOV_DOCS["(设计态 / design) D_GOV_DOCS"]
    D_GOV_DOCS -.->|contract / contract| src_zephyr_security_llm_defense_llm_security_alignment_scorer_py
    D_GOVERNANCE["(原型态 / prototype) D_GOVERNANCE"]
    ...
    class D_INTEGRATION,D_FBL_DIAGNOSERS,D_INFRA_RUNTIME,D_FBL_DETECTORS,D_FBL_VERIFICATION,D_GOV_AUDIT,D_SECURITY external_prod
...
graph TD
    ...
    class D_ORCHESTRATOR,D_FEEDBACK_LOOP,D_GOV_AUDIT,D_FBL_DETECTORS external_prod
    class D_GOVERNANCE,D_INFRA_A2A,D_COMPLIANCE,D_GOV_DRIFT,D_GOV_DOCS external_design
```
...
graph TD
    ...
    class D_FBL_VERIFICATION,D_GOV_DRIFT,D_ORCHESTRATOR,D_INFRA_RUNTIME,D_SHARED external_prod
```
...
graph TD
    ...
    class D_GOV_OPS_RESILIENCE,D_SHARED,D_INFRA_RUNTIME,D_GOV_ENFORCEMENT,D_OPS,D_GOVERNANCE external_prod
```
...
graph TD
    ...
    class D_GOVERNANCE,D_SECURITY,D_FBL_DIAGNOSERS,D_INTEGRATION external_prod
```
...
graph TD
    ...
    class D_INTEGRATION,D_INFRA_RUNTIME,D_GOV_AUDIT,D_INTELLIGENCE,D_GOV_KB,D_SECURITY,D_FEEDBACK_LOOP,D_GOV_REPAIR,D_GOV_CODE_QUALITY external_prod
    class D_SHARED,D_GOV_DOCS,D_GOVERNANCE,D_ORCHESTRATOR external_design
```
...
| 23 | Agent Spec -> Pipeline 集成桥接层 (__init__.py) | → | D_FBL_VERIFICATION 反馈验证: _safety_gates.py | runtime / runtime |
...
| 125 | Agent Spec -> Pipeline 集成桥接层 (__init__.py) | → | D_GOVERNANCE 生命周期管理: session_manager.py | runtime / runtime |
...
| 129 | test_auto_bootstrap.py | → | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | contract / contract |
| 130 | test_auto_bootstrap.py | → | D_GOVERNANCE 生命周期管理: Batch2 治理层契约 — 15条 Pydantic v2 Schema（P... | runtime / runtime |
| 131 | test_auto_split.py | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 测试依赖 / test_depends |
| 132 | test_escalation_adapter.py | → | D_GOVERNANCE 生命周期管理: Escalation Adapter — MOD-INF-022 统一集成入口.... | 测试依赖 / test_depends |
| 133 | test_escalation_gov_a2a_failure.py | → | D_GOVERNANCE 生命周期管理: G-CT-008 消费端 — Escalation.on_a2a_failure() ... | 测试依赖 / test_depends |
| 134 | test_escalation_gov_rbac_bridge.py | → | D_GOVERNANCE 生命周期管理: G-CT-007 契约：Budget -> RBAC 配额限制. (rbac_b... | 测试依赖 / test_depends |
| 135 | test_memory_provenance.py | → | D_GOVERNANCE 生命周期管理: Memory Provenance — v0.9.0 记忆溯源追踪: 每条m... | 测试依赖 / test_depends |
| 136 | DM-202918: transition(COMPLETED)自动git commit.... | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 测试依赖 / test_depends |
| 137 | skill_executor.py | → | D_GOV_AUDIT 审计追踪: writer.py | 导入依赖 / import_depends |
| 138 | MOD-INF-019: Agent Spec — Skill Sandbox (skill... | → | D_GOV_AUDIT 审计追踪: bridge.py | 导入依赖 / import_depends |
| 139 | MOD-INF-019: Agent Spec — SpecEngine 蓝图->Ski... | → | D_GOV_AUDIT 审计追踪: writer.py | 导入依赖 / import_depends |
| 140 | test_action_history.py | → | D_GOV_AUDIT 审计追踪: ActionHistory — 操作历史持久化审计 + 去重 + 循... | 测试依赖 / test_depends |
| 141 | test_agent_signer.py | → | D_GOV_AUDIT 审计追踪: audit-trail.agent_signer — MOD-INF-020 · Agen... | 测试依赖 / test_depends |
| 142 | test_auto_fixer.py | → | D_GOV_CODE_QUALITY 代码质量治理: 安全自动修复引擎——五直接开关+五间接约束. (aut... | 测试依赖 / test_depends |
...
| 145 | Agent Spec -> Pipeline 集成桥接层 (__init__.py) | → | D_GOV_DOCS 架构文档治理: blueprint.md | runtime / runtime |
| 146 | Agent Spec -> Pipeline 集成桥接层 (__init__.py) | → | D_GOV_DOCS 架构文档治理: blueprint.md | contract / contract |
| 147 | test_auto_bootstrap.py | → | D_GOV_DOCS 架构文档治理: blueprint.md | runtime / runtime |
...
| 14 | D_GOVERNANCE 生命周期管理: model_provider_data.py | → | alignment_scorer.py — 对齐评分 (B11, DD85, TAS... | contract / contract |
...
| 39 | D_GOV_DOCS 架构文档治理: blueprint.md | → | alignment_scorer.py — 对齐评分 (B11, DD85, TAS... | contract / contract |
...
| 42 | D_GOV_REPAIR 治理修复: budget_enforcement.py | → | skill_executor.py | 导入依赖 / import_depends |
...
> 本域与 28 个外部域直接连接（出边 332 条 + 入边 56 条 = 388 条）。只显示直接连接的域，不展开具体节点。
