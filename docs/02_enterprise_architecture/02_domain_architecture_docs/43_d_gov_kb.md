...
> 展示全部 31 个模块（生产态 18 + 设计态 0 + 原型态 13），标签标注成熟度。
...
graph TD
    ...
    class D_GOVERNANCE,D_GOV_RULE,D_GOV_OPS_RESILIENCE,D_INTEGRATION,D_INTELLIGENCE,D_AUTONOMY_CORE external_prod
...
graph TD
    ...
    class D_INTEGRATION,D_SHARED,D_GOVERNANCE,D_GOV_RULE,D_GOV_OPS_RESILIENCE,D_INTELLIGENCE,D_AUTONOMY_CORE external_prod
```
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 13 个，2 条域内依赖）。
...
graph TD
    ...
    class D_SHARED,D_INTEGRATION,D_GOV_OPS_RESILIENCE external_design
```
