...
graph TD
    ...
    class D_GOVERNANCE,D_AUTONOMY_CORE,D_GOV_RULE,D_INTEGRATION,D_GOV_REPAIR,D_INTELLIGENCE,D_GOV_OPS_RESILIENCE external_prod
...
graph TD
    ...
    class D_SHARED,D_INTEGRATION,D_GOVERNANCE,D_GOV_RULE,D_GOV_REPAIR,D_INTELLIGENCE,D_GOV_OPS_RESILIENCE external_prod
...
| 29 | D_KNOWLEDGE 知识管理: test_kb_storage_backend.py | → | Backend protocol & shared data classes for the ... | 测试依赖 / test_depends |
...
> 本域与 10 个外部域直接连接（出边 33 条 + 入边 35 条 = 68 条）。只显示直接连接的域，不展开具体节点。
