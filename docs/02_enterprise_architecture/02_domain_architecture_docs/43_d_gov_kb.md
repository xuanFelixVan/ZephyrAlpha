...
| 2 | src/zephyr/gov_kb/_backend_protocol.py | Re-export shim. True source: src/zephyr/intelli... | 生产态 / production | [MOD-KB-001](../../03_modules/_domain_knowledge/knowledge_base/blueprint.md) |
...
| 26 | src/zephyr/gov_kb/storage/_backend_protocol.py | Re-export shim. True source: src/zephyr/intelli... | 生产态 / production | [MOD-KB-001](../../03_modules/_domain_knowledge/knowledge_base/blueprint.md) |
...
| 31 | src/zephyr/gov_kb/vms_memory_backend.py | Re-export shim. True source: src/zephyr/integra... | 生产态 / production | [MOD-KB-001](../../03_modules/_domain_knowledge/knowledge_base/blueprint.md) |
...
graph TD
    subgraph D_GOV_KB["D_GOV_KB 知识库治理"]
        ...
        src_zephyr_gov_kb_backend_protocol_py["(生产态 / production) Re-export shim. True source: src/zephyr/intelli...<br/>文件: _backend_protocol.py"]
        ...
        src_zephyr_gov_kb_storage_backend_protocol_py["(生产态 / production) Re-export shim. True source: src/zephyr/intelli...<br/>文件: _backend_protocol.py"]
    ...
    class D_GOVERNANCE,D_SHARED,D_GOV_RULE,D_INTEGRATION,D_INTELLIGENCE,D_AUTONOMY_CORE external_prod
...
graph TD
    subgraph D_GOV_KB["D_GOV_KB 知识库治理"]
        src_zephyr_gov_kb_vms_memory_backend_py["(生产态 / production) Re-export shim. True source: src/zephyr/integra...<br/>文件: vms_memory_backend.py"]
    end
    D_INTEGRATION["(生产态 / production) D_INTEGRATION"]
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_RULE,D_GOV_OPS_RESILIENCE,D_INTELLIGENCE,D_AUTONOMY_CORE external_prod
```
...
| 11 | Re-export shim. True source: src/zephyr/integra... | → | D_INTEGRATION 管线路由: BridgeLayer — MOD-INF-011 kb/ ↔ VMS 过渡桥接 ... | 导入依赖 / import_depends |
| 12 | Re-export shim. True source: src/zephyr/integra... | → | D_INTEGRATION 管线路由: InProcessVectorMemory — MOD-INF-011 VMS 统一入... | 导入依赖 / import_depends |
...
| 9 | D_INTELLIGENCE 上下文管理: UnifiedMemoryAPI — RI-02 统一记忆 API（M2 跨模... | → | Re-export shim. True source: src/zephyr/intelli... | 导入依赖 / import_depends |
| 10 | D_INTELLIGENCE 上下文管理: UnifiedMemoryAPI — RI-02 统一记忆 API（M2 跨模... | → | Re-export shim. True source: src/zephyr/integra... | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 7 个外部域直接连接（出边 32 条 + 入边 10 条 = 42 条）。只显示直接连接的域，不展开具体节点。
