...
| 5 | src/zephyr/data/buffered_writer.py | 批量聚合写入器（MOD-L00-004 §18.3 裁定 #ARCH-CH-003 | 原型态 / prototype |  |
...
graph TD
    subgraph D_DATA["D_DATA data_source_integrator"]
        ...
        src_zephyr_data_buffered_writer_py["(原型态 / prototype) 批量聚合写入器（MOD-L00-004 §18.3 裁定 #ARCH-CH-003<br/>文件: buffered_writer.py"]
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_INTEGRATION,D_FBL_DIAGNOSERS,D_FBL_VERIFICATION external_prod
```
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_INTEGRATION,D_FBL_DIAGNOSERS,D_FBL_VERIFICATION,D_BACKTEST external_prod
...
| 27 | 本地落盘兜底 + 自动回灌（裁定 #ARCH-CH-013 Phas... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
...
> 本域与 11 个外部域直接连接（出边 43 条 + 入边 11 条 = 54 条）。只显示直接连接的域，不展开具体节点。
