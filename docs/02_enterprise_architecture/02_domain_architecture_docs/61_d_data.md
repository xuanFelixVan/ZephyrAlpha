...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_RULE,D_FBL_VERIFICATION external_prod
```
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_RULE,D_FBL_VERIFICATION,D_BACKTEST external_prod
...
| 33 | tests/db/ 共享 fixture — 收敛 governance.db 路... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
...
> 本域与 10 个外部域直接连接（出边 44 条 + 入边 11 条 = 55 条）。只显示直接连接的域，不展开具体节点。
