...
graph TD
    ...
    src_zephyr_data_implementations_init_py -.->|data / data| D_MKT_DATA
    ...
    class D_MKT_DATA,D_GOVERNANCE,D_GOV_SCRIPTS,D_INFRA_RUNTIME,D_GOV_CODE_QUALITY external_design
```
...
graph TD
    ...
    class D_FBL_DIAGNOSERS,D_GOVERNANCE,D_SHARED,D_FBL_VERIFICATION,D_FEEDBACK_LOOP external_prod
```
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 56 个，71 条域内依赖）。
...
graph TD
    ...
    src_zephyr_data_implementations_init_py -.->|导入依赖 / import_depends| src_zephyr_data_implementations_ifind_provider_py
    src_zephyr_data_implementations_init_py -.->|导入依赖 / import_depends| src_zephyr_data_implementations_miniqmt_provider_py
    src_zephyr_data_implementations_init_py -.->|contract / contract| src_zephyr_data_satellite_geospatial_engine_init_py
    src_zephyr_data_satellite_geospatial_engine_init_py -.->|导入依赖 / import_depends| src_zephyr_data_provider_base_py
    D_FBL_DIAGNOSERS["(生产态 / production) D_FBL_DIAGNOSERS"]
    tests_data_test_data_volume_growth_monitor_py -.->|测试依赖 / test_depends| D_FBL_DIAGNOSERS
    D_GOVERNANCE["(生产态 / production) D_GOVERNANCE"]
    ...
    class D_FBL_DIAGNOSERS,D_GOVERNANCE,D_INTEGRATION,D_SHARED,D_BACKTEST external_prod
    class D_GOV_SCRIPTS,D_INFRA_RUNTIME,D_GOV_CODE_QUALITY external_design
```
...
| 22 | 数据源 Provider 实现集合（MOD-L00-004 §4.3）。... | → | D_MKT_DATA 行情数据:  | data / data |
...
> 本域与 13 个外部域直接连接（出边 48 条 + 入边 15 条 = 63 条）。只显示直接连接的域，不展开具体节点。
