...
graph TD
    ...
    class D_INFRA_RUNTIME,D_GOV_SCRIPTS,D_GOV_CODE_QUALITY,D_GOVERNANCE external_design
```
...
graph TD
    ...
    src_zephyr_data_scheduler_py -.->|contract / contract| D_GOVERNANCE
    D_SHARED["(生产态 / production) D_SHARED"]
    ...
    class D_MKT_DATA,D_GOVERNANCE,D_GOV_ENFORCEMENT external_design
```
...
graph TD
    ...
    class D_SHARED,D_BACKTEST external_prod
    class D_MKT_DATA,D_GOVERNANCE,D_GOV_ENFORCEMENT,D_INFRA_RUNTIME,D_GOV_CODE_QUALITY,D_GOV_SCRIPTS external_design
```
...
|:--:|---------|:--:|---------|---------|
| 1 | 数据源调度编排层（MOD-L00-004 §6）。 (schedule... | → | D_GOVERNANCE 生命周期管理:  | contract / contract |
| 2 | D_DATA Data Source (__init__.py) | → | D_GOV_ENFORCEMENT 规则执行: D_DATA — Data Quality Gate (quality_gate.py) | 导入依赖 / import_depends |
| 3 | 数据源调度编排层（MOD-L00-004 §6）。 (schedule... | → | D_MKT_DATA 行情数据:  | data / data |
| 4 | 告警管理（MOD-L00-004 §6.5 失败重试与告警 + §... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 5 | 告警管理（MOD-L00-004 §6.5 失败重试与告警 + §... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 6 | ClickHouse 连接配置单真源加载器（裁定... (ch_co... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 7 | ClickHouse 连接配置单真源加载器（裁定... (ch_co... | → | D_SHARED 共享服务: secrets.py —— Secrets 管理抽象（Phase 7 新增 ... | 导入依赖 / import_depends |
...
> 本域与 8 个外部域直接连接（出边 17 条 + 入边 10 条 = 27 条）。只显示直接连接的域，不展开具体节点。
