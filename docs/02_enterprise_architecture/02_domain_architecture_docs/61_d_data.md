...
> 展示全部 64 个模块（生产态 9 + 设计态 0 + 原型态 55），标签标注成熟度。
...
graph TD
    ...
    classDef external_prod fill:#e8f5e9,stroke:#1b5e20,stroke-width:1px,color:#000
    classDef external_design fill:#fce4ec,stroke:#880e4f,stroke-width:1px,color:#000,stroke-dasharray: 5 5
    class tests_io_test_io_serialization_py,tests_io_test_mcp_launcher_py,tests_io_test_mcp_task_claim_py,tests_zephyr_data_test_tick_subscriber_py design
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 55 个，71 条域内依赖）。
...
graph TD
    ...
    class D_GOVERNANCE,D_SHARED,D_FBL_VERIFICATION,D_FBL_DIAGNOSERS,D_BACKTEST external_prod
...
| 22 | 批量聚合写入器（MOD-L00-004 §18.3 裁定... (buf... | → | D_MKT_DATA 行情数据:  | data / data |
...
| 8 | D_GOV_SCRIPTS 脚本治理: tick_data 表真重复检查工具（RULE-DATA-OPS 配套.... | → | ClickHouse 统一读取层（裁定 #ARCH-CH-007）。 (c... | 导入依赖 / import_depends |
