...
graph TD
    ...
    src_zephyr_data_ch_config_py -.->|data / data| D_MKT_DATA
...
graph TD
    ...
    src_zephyr_data_ch_config_py -.->|contract / contract| src_zephyr_data_satellite_geospatial_engine_init_py
    src_zephyr_data_buffered_writer_py -.->|导入依赖 / import_depends| src_zephyr_data_ch_writer_py
...
| 2 | ClickHouse 连接配置单真源加载器（裁定... (ch_co... | → | D_MKT_DATA 行情数据:  | data / data |
