...
graph TD
    ...
    src_zephyr_data_capability_validator_py -.->|data / data| D_MKT_DATA
    ...
    class D_MKT_DATA,D_GOVERNANCE,D_GOV_SCRIPTS,D_GOV_CODE_QUALITY,D_INFRA_RUNTIME external_design
```
...
graph TD
    ...
    class D_FBL_VERIFICATION,D_GOVERNANCE,D_SHARED,D_GOV_RULE,D_FBL_DIAGNOSERS external_prod
```
...
graph TD
    ...
    src_zephyr_data_capability_validator_py -.->|contract / contract| src_zephyr_data_satellite_geospatial_engine_init_py
    src_zephyr_data_buffered_writer_py -.->|导入依赖 / import_depends| src_zephyr_data_ch_writer_py
    ...
    class D_FBL_VERIFICATION,D_GOVERNANCE,D_SHARED,D_INTEGRATION,D_BACKTEST external_prod
    class D_GOV_SCRIPTS,D_GOV_CODE_QUALITY,D_INFRA_RUNTIME external_design
```
...
| 11 | DM-100021: 事件驱动自动启动检查+自动运行检查 (t... | → | D_GOVERNANCE 生命周期管理: DatabaseService 真源收敛（AI-14 审计 P1 修复） ... | 测试依赖 / test_depends |
...
| 22 | Provider Capability 行为契约校验器（裁定... (ca... | → | D_MKT_DATA 行情数据:  | data / data |
