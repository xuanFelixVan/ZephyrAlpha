...
graph TD
    ...
    src_zephyr_data_implementations_init_py -.->|contract / contract| D_GOVERNANCE
    ...
    class D_MKT_DATA,D_GOVERNANCE,D_INFRA_RUNTIME,D_GOV_SCRIPTS,D_GOV_CODE_QUALITY external_design
```
...
graph TD
    ...
    class D_SHARED,D_BACKTEST external_prod
    class D_MKT_DATA,D_GOVERNANCE,D_GOV_ENFORCEMENT,D_INFRA_RUNTIME,D_GOV_SCRIPTS,D_GOV_CODE_QUALITY external_design
```
...
|:--:|---------|:--:|---------|---------|
| 1 | 数据源 Provider 实现集合（MOD-L00-004 §4.3）。... | → | D_GOVERNANCE 生命周期管理:  | contract / contract |
| 2 | D_DATA Data Source (__init__.py) | → | D_GOV_ENFORCEMENT 规则执行: D_DATA — Data Quality Gate (quality_gate.py) | 导入依赖 / import_depends |
| 3 | 数据源 Provider 实现集合（MOD-L00-004 §4.3）。... | → | D_MKT_DATA 行情数据:  | data / data |
