...
graph TD
    ...
    class D_GOV_AUDIT,D_SHARED,D_GOVERNANCE,D_INFRA_RECOVERY,D_INTEGRATION,D_ORCHESTRATOR,D_FEEDBACK_LOOP,D_OPS external_prod
...
> 本域与 20 个外部域直接连接（出边 46 条 + 入边 194 条 = 240 条）。只显示直接连接的域，不展开具体节点。
