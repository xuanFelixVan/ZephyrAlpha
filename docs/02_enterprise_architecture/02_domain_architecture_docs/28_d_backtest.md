...
|:--:|---------|:--:|---------|---------|
| 1 |  | → | D_GOVERNANCE 生命周期管理:  | 导入依赖 / import_depends |
...
| 6 | result_repository · 回测产物持久化/检索模块（v... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
...
> 本域与 7 个外部域直接连接（出边 6 条 + 入边 20 条 = 26 条）。只显示直接连接的域，不展开具体节点。
