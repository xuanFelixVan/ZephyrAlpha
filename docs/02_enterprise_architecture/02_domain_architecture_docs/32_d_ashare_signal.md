...
flowchart TD
    src_zephyr_signal_ashare_init_py["zephyr/signal_ashare 包入口<br/>信号的包入口，把这一层的子模块归到一起统一管理，<br/>用到谁才加载谁，避免一次性全加载拖慢启动<br/>文件: signal_ashare/__init__.py<br/>(生产态 / production)"]
    src_zephyr_signal_ashare_extensions_init_py["signal_ashare/_extensions 包入口<br/>信号的包入口，把这一层的子模块归到一起统一管理，<br/>用到谁才加载谁，避免一次性全加载拖慢启动<br/>文件: _extensions/__init__.py<br/>(生产态 / production)"]
    src_zephyr_signal_ashare_api_init_py["signal_ashare/api 包入口<br/>接口的包入口，把这一层的子模块归到一起统一管理，<br/>用到谁才加载谁，避免一次性全加载拖慢启动<br/>文件: api/__init__.py<br/>(生产态 / production)"]
    src_zephyr_signal_ashare_capital_flow_pattern_analyzer_py["资本流模式分析器<br/>信号的分析器，分析数据找出问题或规律<br/>⛔ A股信号域，设计已就绪，等待开发排期<br/>capital_flow_pattern_analyzer<br/>文件: signal_ashare/capital_flow_pattern_<br/>analyzer.py<br/>(设计态 / design)"]
    src_zephyr_signal_ashare_core_init_py["signal_ashare/core 包入口<br/>信号的包入口，把这一层的子模块归到一起统一管理，<br/>用到谁才加载谁，避免一次性全加载拖慢启动<br/>文件: core/__init__.py<br/>(生产态 / production)"]
    src_zephyr_signal_ashare_dual_engine_fusion_decision_engine_py["双引擎融合决策引擎<br/>信号的引擎，执行核心逻辑的处理引擎<br/>⛔ A股信号域，设计已就绪，等待开发排期<br/>dual_engine_fusion_decision_engine<br/>文件: signal_ashare/dual_engine_fusion_decision_<br/>engine.py<br/>(设计态 / design)"]
    src_zephyr_signal_ashare_infrastructure_init_py["signal_ashare/infrastructure 包入口<br/>基础设施的包入口，把这一层的子模块归到一起统一管<br/>理，用到谁才加载谁，避免一次性全加载拖慢启动<br/>文件: infrastructure/__init__.py<br/>(生产态 / production)"]
    src_zephyr_signal_ashare_institutional_behavior_analyzer_py["机构行为分析器<br/>信号的分析器，分析数据找出问题或规律<br/>⛔ A股信号域，设计已就绪，等待开发排期<br/>institutional_behavior_analyzer<br/>文件: signal_ashare/institutional_behavior_<br/>analyzer.py<br/>(设计态 / design)"]
    src_zephyr_signal_ashare_intraday_buy_sell_point_analyzer_py["日内买卖点分析器<br/>信号的分析器，分析数据找出问题或规律<br/>⛔ A股信号域，设计已就绪，等待开发排期<br/>intraday_buy_sell_point_analyzer<br/>文件: signal_ashare/intraday_buy_sell_point_<br/>analyzer.py<br/>(设计态 / design)"]
    src_zephyr_signal_ashare_market_sentiment_analyzer_py["市场情绪分析器<br/>信号的分析器，分析数据找出问题或规律<br/>⛔ A股信号域，设计已就绪，等待开发排期<br/>market_sentiment_analyzer<br/>文件: signal_ashare/market_sentiment_analyzer.py<br/>(设计态 / design)"]
    ...
    src_zephyr_signal_ashare_models_init_py["signal_ashare/models 包入口<br/>模型的包入口，把这一层的子模块归到一起统一管理，<br/>用到谁才加载谁，避免一次性全加载拖慢启动<br/>文件: models/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_signal_ashare_sector_analyzer_py["板块分析器<br/>信号的分析器，分析数据找出问题或规律<br/>⛔ A股信号域，设计已就绪，等待开发排期<br/>sector_analyzer<br/>文件: signal_ashare/sector_analyzer.py<br/>(设计态 / design)"]
    src_zephyr_signal_ashare_services_init_py["signal_ashare/services 包入口<br/>信号的包入口，把这一层的子模块归到一起统一管理，<br/>用到谁才加载谁，避免一次性全加载拖慢启动<br/>文件: services/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_signal_ashare_quant_short_term_strength_engine_py["量化短期强度引擎<br/>信号的引擎，执行核心逻辑的处理引擎<br/>⛔ A股信号域，设计已就绪，等待开发排期<br/>quant_short_term_strength_engine<br/>文件: signal_ashare/quant_short_term_strength_<br/>engine.py<br/>(设计态 / design)"]
    src_zephyr_signal_ashare_youzi_relay_emotion_engine_py["游资中继情绪引擎<br/>信号的引擎，执行核心逻辑的处理引擎<br/>⛔ A股信号域，设计已就绪，等待开发排期<br/>youzi_relay_emotion_engine<br/>文件: signal_ashare/youzi_relay_emotion_<br/>engine.py<br/>(设计态 / design)"]
