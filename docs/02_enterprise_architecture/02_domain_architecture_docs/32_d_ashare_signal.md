...
flowchart TD
    src_zephyr_signal_ashare_init_py["(生产态 / production) 包入口 / __init__<br/>信号的包入口，把这一层的子模块归到一起统<br/>一管理，用到谁才加载谁，避免一次性全加载<br/>拖慢启动。<br/>文件: signal_ashare/__init__.py"]
    src_zephyr_signal_ashare_extensions_init_py["(生产态 / production) 包入口 / __init__<br/>信号的包入口，把这一层的子模块归到一起统<br/>一管理，用到谁才加载谁，避免一次性全加载<br/>拖慢启动。<br/>文件: _extensions/__init__.py"]
    src_zephyr_signal_ashare_api_init_py["(生产态 / production) 包入口 / __init__<br/>接口的包入口，把这一层的子模块归到一起统<br/>一管理，用到谁才加载谁，避免一次性全加载<br/>拖慢启动。<br/>文件: api/__init__.py"]
    src_zephyr_signal_ashare_capital_flow_pattern_analyzer_py["(设计态 / design) 资本流模式分析器 /<br/>capital_flow_pattern_analyzer<br/>资本流模式分析器，信号的分析器，分析数据<br/>找出问题或规律。<br/>文件: signal_ashare/capital_flow_<br/>pattern_analyzer.py<br/>⛔ A股信号域，设计已就绪，等待开发排期"]
    src_zephyr_signal_ashare_core_init_py["(生产态 / production) 包入口 / __init__<br/>信号的包入口，把这一层的子模块归到一起统<br/>一管理，用到谁才加载谁，避免一次性全加载<br/>拖慢启动。<br/>文件: core/__init__.py"]
    src_zephyr_signal_ashare_dual_engine_fusion_decision_engine_py["(设计态 / design) 双引擎融合决策引擎 /<br/>dual_engine_fusion_decision_engine<br/>双引擎融合决策引擎，信号的引擎，执行核心<br/>逻辑的处理引擎。<br/>文件: signal_ashare/dual_engine_fusion_<br/>decision_engine.py<br/>⛔ A股信号域，设计已就绪，等待开发排期"]
    src_zephyr_signal_ashare_infrastructure_init_py["(生产态 / production) 包入口 / __init__<br/>基础设施的包入口，把这一层的子模块归到一<br/>起统一管理，用到谁才加载谁，避免一次性全<br/>加载拖慢启动。<br/>文件: infrastructure/__init__.py"]
    src_zephyr_signal_ashare_institutional_behavior_analyzer_py["(设计态 / design) 机构行为分析器 /<br/>institutional_behavior_analyzer<br/>机构行为分析器，信号的分析器，分析数据找<br/>出问题或规律。<br/>文件: signal_ashare/institutional_<br/>behavior_analyzer.py<br/>⛔ A股信号域，设计已就绪，等待开发排期"]
    src_zephyr_signal_ashare_intraday_buy_sell_point_analyzer_py["(设计态 / design) 日内买卖点分析器 /<br/>intraday_buy_sell_point_analyzer<br/>日内买卖点分析器，信号的分析器，分析数据<br/>找出问题或规律。<br/>文件: signal_ashare/intraday_buy_sell_<br/>point_analyzer.py<br/>⛔ A股信号域，设计已就绪，等待开发排期"]
    src_zephyr_signal_ashare_market_sentiment_analyzer_py["(设计态 / design) 市场情绪分析器 /<br/>market_sentiment_analyzer<br/>市场情绪分析器，信号的分析器，分析数据找<br/>出问题或规律。<br/>文件: signal_ashare/market_sentiment_<br/>analyzer.py<br/>⛔ A股信号域，设计已就绪，等待开发排期"]
    src_zephyr_signal_ashare_models_init_py["(生产态 / production) 包入口 / __init__<br/>模型的包入口，把这一层的子模块归到一起统<br/>一管理，用到谁才加载谁，避免一次性全加载<br/>拖慢启动。<br/>文件: models/__init__.py"]
    src_zephyr_signal_ashare_sector_analyzer_py["(设计态 / design) 板块分析器 / sector_<br/>analyzer<br/>板块分析器，信号的分析器，分析数据找出问<br/>题或规律。<br/>文件: signal_ashare/sector_analyzer.py<br/>⛔ A股信号域，设计已就绪，等待开发排期"]
    src_zephyr_signal_ashare_services_init_py["(生产态 / production) 包入口 / __init__<br/>信号的包入口，把这一层的子模块归到一起统<br/>一管理，用到谁才加载谁，避免一次性全加载<br/>拖慢启动。<br/>文件: services/__init__.py"]
    src_zephyr_signal_ashare_short_term_stock_selector_py["(设计态 / design) 短期股票选择器 /<br/>short_term_stock_selector<br/>短期股票选择器，信号的选择器，按条件选择<br/>最优项。<br/>文件: signal_ashare/short_term_stock_<br/>selector.py<br/>⛔ A股信号域，设计已就绪，等待开发排期"]
    ...
    src_zephyr_signal_ashare_quant_short_term_strength_engine_py["(设计态 / design) 量化短期强度引擎 /<br/>quant_short_term_strength_engine<br/>量化短期强度引擎，信号的引擎，执行核心逻<br/>辑的处理引擎。<br/>文件: signal_ashare/quant_short_term_<br/>strength_engine.py<br/>⛔ A股信号域，设计已就绪，等待开发排期"]
    src_zephyr_signal_ashare_youzi_relay_emotion_engine_py["(设计态 / design) 游资中继情绪引擎 /<br/>youzi_relay_emotion_engine<br/>游资中继情绪引擎，信号的引擎，执行核心逻<br/>辑的处理引擎。<br/>文件: signal_ashare/youzi_relay_emotion_<br/>engine.py<br/>⛔ A股信号域，设计已就绪，等待开发排期"]
    ...
    D_FUNDAMENTAL_SIGNAL["(设计态 / design) 基本面信号 /<br/>Fundamental Signal<br/>基本面信号，负责基于财务数据的基本面信号<br/>生成<br/>跨域节点 / cross-domain"]
