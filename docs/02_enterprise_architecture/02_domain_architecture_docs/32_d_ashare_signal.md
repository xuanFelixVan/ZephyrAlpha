...
flowchart TD
    src_zephyr_signal_ashare_init_py["zephyr/signal_ashare 包入口<br/>zephyr/signal_ashare<br/>包入口，管理该层子模块的统一加载和懒导入<br/>文件: signal_ashare/__init__.py<br/>(生产态 / production)"]
    src_zephyr_signal_ashare_extensions_init_py["signal_ashare/_extensions 包入口<br/>signal_ashare/_extensions<br/>包入口，管理该层子模块的统一加载和懒导入<br/>文件: _extensions/__init__.py<br/>(生产态 / production)"]
    src_zephyr_signal_ashare_api_init_py["signal_ashare/api 包入口<br/>signal_ashare/api<br/>包入口，管理该层子模块的统一加载和懒导入<br/>文件: api/__init__.py<br/>(生产态 / production)"]
    src_zephyr_signal_ashare_capital_flow_pattern_analyzer_py["资本流模式分析器<br/>signal ashare包的capital_flow_pattern_<br/>analyzer模块<br/>⛔ A股信号域，设计已就绪，等待开发排期<br/>文件: signal_ashare/capital_flow_pattern_<br/>analyzer.py<br/>(设计态 / design)"]
    src_zephyr_signal_ashare_core_init_py["signal_ashare/core 包入口<br/>signal_ashare/core<br/>包入口，管理该层子模块的统一加载和懒导入<br/>文件: core/__init__.py<br/>(生产态 / production)"]
    src_zephyr_signal_ashare_dual_engine_fusion_decision_engine_py["双引擎融合决策引擎<br/>signal ashare包的dual_engine_fusion_decision_<br/>engine模块<br/>⛔ A股信号域，设计已就绪，等待开发排期<br/>文件: signal_ashare/dual_engine_fusion_decision_<br/>engine.py<br/>(设计态 / design)"]
    src_zephyr_signal_ashare_infrastructure_init_py["signal_ashare/infrastructure 包入口<br/>signal_ashare/infrastructure<br/>包入口，管理该层子模块的统一加载和懒导入<br/>文件: infrastructure/__init__.py<br/>(生产态 / production)"]
    src_zephyr_signal_ashare_institutional_behavior_analyzer_py["机构行为分析器<br/>signal ashare包的institutional_behavior_<br/>analyzer模块<br/>⛔ A股信号域，设计已就绪，等待开发排期<br/>文件: signal_ashare/institutional_behavior_<br/>analyzer.py<br/>(设计态 / design)"]
    src_zephyr_signal_ashare_intraday_buy_sell_point_analyzer_py["日内买卖点分析器<br/>signal ashare包的intraday_buy_sell_point_<br/>analyzer模块<br/>⛔ A股信号域，设计已就绪，等待开发排期<br/>文件: signal_ashare/intraday_buy_sell_point_<br/>analyzer.py<br/>(设计态 / design)"]
    src_zephyr_signal_ashare_market_sentiment_analyzer_py["市场情绪分析器<br/>signal ashare包的market_sentiment_analyzer模块<br/>⛔ A股信号域，设计已就绪，等待开发排期<br/>文件: signal_ashare/market_sentiment_analyzer.py<br/>(设计态 / design)"]
    ...
    src_zephyr_signal_ashare_models_init_py["signal_ashare/models 包入口<br/>signal_ashare/models<br/>包入口，管理该层子模块的统一加载和懒导入<br/>文件: models/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_signal_ashare_sector_analyzer_py["板块分析器<br/>signal ashare包的sector_analyzer模块<br/>⛔ A股信号域，设计已就绪，等待开发排期<br/>文件: signal_ashare/sector_analyzer.py<br/>(设计态 / design)"]
    src_zephyr_signal_ashare_services_init_py["signal_ashare/services 包入口<br/>signal_ashare/services<br/>包入口，管理该层子模块的统一加载和懒导入<br/>文件: services/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_signal_ashare_quant_short_term_strength_engine_py["量化短期强度引擎<br/>signal ashare包的quant_short_term_strength_<br/>engine模块<br/>⛔ A股信号域，设计已就绪，等待开发排期<br/>文件: signal_ashare/quant_short_term_strength_<br/>engine.py<br/>(设计态 / design)"]
    src_zephyr_signal_ashare_youzi_relay_emotion_engine_py["游资中继情绪引擎<br/>signal ashare包的youzi_relay_emotion_engine模块<br/>⛔ A股信号域，设计已就绪，等待开发排期<br/>文件: signal_ashare/youzi_relay_emotion_<br/>engine.py<br/>(设计态 / design)"]
