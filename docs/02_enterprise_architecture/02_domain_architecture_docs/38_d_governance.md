...
> 展示全部 693 个模块（生产态 141 + 设计态 1 + 原型态 551），标签标注成熟度。
...
graph TD
    ...
    class D_INTELLIGENCE,D_SHARED,D_SECURITY,D_INFRA_RUNTIME,D_INFRASTRUCTURE external_prod
...
graph TD
    ...
    class D_GOV_OPS_RESILIENCE,D_TRADING,D_SECURITY,D_INFRA_RUNTIME,D_INTEGRATION,D_INTELLIGENCE external_prod
    class D_INFRASTRUCTURE,D_SHARED,D_INTEGRATION_GATEWAY,D_EX_CORE,D_GOV_REPAIR,D_INFRA_A2A,D_GOV_AUDIT,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_GOV_ENFORCEMENT,D_INTEGRATION,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_BACKTEST,D_EX_CORE,D_FRONTEND,D_SECURITY external_design
```
...
graph TD
    subgraph D_GOVERNANCE["D_GOVERNANCE 生命周期管理"]
        ...
        src_zephyr_governance_intelligence_governance_memory_provider_py["(原型态 / prototype) D_DATA — Memory Provider<br/>文件: memory_provider.py"]
    ...
    class D_SECURITY,D_OPS,D_GOV_CODE_QUALITY,D_GOV_OPS_RESILIENCE,D_GOV_RULE,D_GOV_REPAIR,D_INFRA_RUNTIME external_prod
...
graph TD
    ...
    class D_SHARED,D_GOV_RULE,D_INTEGRATION,D_GOV_OPS_RESILIENCE,D_GOV_ENFORCEMENT,D_FEEDBACK_LOOP,D_FRONTEND,D_OPS,D_GOV_DRIFT external_prod
    class D_GOV_AUDIT,D_SECURITY external_design
```
...
graph TD
    ...
    class D_SECURITY,D_INFRA_RUNTIME,D_GOV_ENFORCEMENT,D_GOV_CODE_QUALITY,D_INTELLIGENCE,D_AUTONOMY_CORE external_prod
```
...
graph TD
    ...
    class D_GOV_AUDIT,D_TRADING,D_OPS,D_GOV_OPS_RESILIENCE external_prod
```
...
graph TD
    ...
    class D_GOV_ENFORCEMENT,D_GOV_CODE_QUALITY,D_GOV_REPAIR external_prod
```
...
graph TD
    ...
    class D_GOV_CODE_QUALITY,D_GOV_OPS_RESILIENCE,D_GOV_AUDIT,D_SHARED,D_GOV_REPAIR external_prod
```
...
graph TD
    ...
    class D_INFRA_RECOVERY,D_INTEGRATION,D_SHARED,D_GOV_ENFORCEMENT,D_GOV_DRIFT,D_GOV_OPS_RESILIENCE,D_INFRA_RUNTIME,D_FRONTEND,D_GOV_CODE_QUALITY external_prod
```
...
graph TD
    ...
    class D_GOV_ENFORCEMENT,D_GOV_SCRIPTS,D_GOV_RULE,D_SHARED,D_SECURITY,D_GOV_DRIFT external_prod
```
...
graph TD
    ...
    class D_GOV_SCRIPTS,D_GOV_OPS_RESILIENCE,D_GOV_CODE_QUALITY,D_INFRA_RUNTIME,D_INFRA_RECOVERY,D_SHARED,D_INTEGRATION external_prod
```
...
graph TD
    ...
    class D_GOV_OPS_RESILIENCE,D_SHARED,D_SECURITY,D_INFRA_RUNTIME,D_GOV_RULE,D_FEEDBACK_LOOP,D_FRONTEND,D_GOV_REPAIR external_prod
```
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 551 个，102 条域内依赖）。
...
| 327 | __init__.py | → | D_GOV_SCRIPTS 脚本治理: analyze_contract_impact.py — 契约变更影响分析... | config_depends / config_depends |
| 328 | __init__.py | → | D_GOV_SCRIPTS 脚本治理: validate_session_log_updated.py — Session Log ... | config_depends / config_depends |
...
| 336 | backup.ps1 | → | D_INFRASTRUCTURE: backup_reconciler.py — 灾备备份系统事件触发器.... | config_depends / config_depends |
| 337 | backup_config.yaml | → | D_INFRASTRUCTURE: backup_reconciler.py — 灾备备份系统事件触发器.... | config_depends / config_depends |
| 338 | backup_manual.ps1 | → | D_INFRASTRUCTURE: backup_reconciler.py — 灾备备份系统事件触发器.... | config_depends / config_depends |
| 339 | restore.ps1 | → | D_INFRASTRUCTURE: backup_reconciler.py — 灾备备份系统事件触发器.... | config_depends / config_depends |
| 340 | 一键备份.ps1 | → | D_INFRASTRUCTURE: backup_reconciler.py — 灾备备份系统事件触发器.... | config_depends / config_depends |
...
| 527 | decisiongraph Schema DDL + 不变量声明 (decision... | → | D_SHARED 共享服务: yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
| 528 | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 529 | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | → | D_SHARED 共享服务: SQLite 连接工厂真源（SSoT） (sqlite_factory.py) | 导入依赖 / import_depends |
| 530 | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 531 | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | → | D_SHARED 共享服务: task_types — 任务系统核心类型 re-export 层 (ta... | 导入依赖 / import_depends |
| 532 | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
...
> 本域与 40 个外部域直接连接（出边 561 条 + 入边 372 条 = 933 条）。只显示直接连接的域，不展开具体节点。
