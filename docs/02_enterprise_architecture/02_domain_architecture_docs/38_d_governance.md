...
graph TD
    ...
    class D_SECURITY,D_INFRA_RUNTIME,D_INTELLIGENCE,D_INFRASTRUCTURE,D_SHARED external_prod
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_BACKTEST,D_EX_CORE,D_FRONTEND,D_DATA,D_GOV_AUDIT,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_GOV_RULE,D_SHARED,D_OPS,D_GOV_CODE_QUALITY,D_SECURITY,D_GOV_AUDIT,D_GOV_OPS_RESILIENCE external_prod
...
graph TD
    ...
    class D_SHARED,D_GOV_DRIFT,D_GOV_KB external_prod
    class D_INFRA_A2A,D_INFRA_RUNTIME,D_AUTONOMY_CORE,D_GOV_AUDIT,D_DATA,D_PF_CORE,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOV_CODE_QUALITY,D_INFRA_RECOVERY,D_GOV_AUDIT external_prod
```
...
graph TD
    ...
    class D_GOV_CODE_QUALITY,D_GOV_OPS_RESILIENCE,D_GOV_REPAIR,D_INFRA_RECOVERY,D_GOV_AUDIT,D_SHARED external_prod
```
...
graph TD
    ...
    class D_GOV_RULE,D_SHARED,D_SECURITY,D_GOV_DRIFT,D_GOV_CODE_QUALITY,D_GOV_OPS_RESILIENCE external_prod
    class D_INFRA_RUNTIME,D_INTEGRATION_GATEWAY,D_GOV_SCRIPTS,D_DATA external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOV_CODE_QUALITY,D_INFRA_RUNTIME,D_INTELLIGENCE,D_AUTONOMY_CORE,D_GOV_REPAIR external_prod
    class D_PF_CORE,D_GOV_SCRIPTS,D_AUTONOMY_PERM external_design
```
...
| 130 | test_pure_assertion_gate.py — pure_assertion_g... | → | D_GOV_CODE_QUALITY 代码质量治理: pure_assertion_gate.py — 纯陈述原则阻断门禁（P... | 测试依赖 / test_depends |
...
| 197 | test_data_task_completeness_gate.py — 数据任务... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
...
| 237 | kb.storage — auto-generated package init. (__i... | → | D_GOV_KB 知识库治理: UnifiedMemoryAPI — RI-02 统一记忆 API（M2 跨模... | config_depends / config_depends |
...
| 240 | G-CT-008 消费端 — Escalation.on_a2a_failure() ... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: G-CT-003 消费端 — Escalation.on_rollback_failu... | 导入依赖 / import_depends |
| 241 | default_security_gateway.py | → | D_GOV_OPS_RESILIENCE 运维弹性治理: DefaultSecurityGateway — SecurityGateway 三层.... | 导入依赖 / import_depends |
| 242 | Delegation Engine — MOD-INF-022 (delegation_en... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Escalation Protocol data models — MOD-INF-022 ... | 导入依赖 / import_depends |
| 243 | Escalation Protocol Self-Test — MOD-INF-022. (... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Escalation Engine — MOD-INF-022 (escalation_en... | 导入依赖 / import_depends |
| 244 | Escalation Protocol Self-Test — MOD-INF-022. (... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Escalation Protocol data models — MOD-INF-022 ... | 导入依赖 / import_depends |
| 245 | Escalation Protocol Self-Test — MOD-INF-022. (... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Circuit Breaker — MOD-INF-022 (circuit_breaker.py) | 导入依赖 / import_depends |
| 246 | transition — 状态机转换 Mixin（从 task_repo.py... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: EventHook — 声明式任务系统事件订阅 (event_hook.py) | 导入依赖 / import_depends |
| 247 | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: EventHook — 声明式任务系统事件订阅 (event_hook.py) | 导入依赖 / import_depends |
...
| 395 | DatabaseService 真源收敛（AI-14 审计 P1 修复） ... | → | D_INFRA_RUNTIME 运行时集成: DatabaseService: 统一管理数据库的连接池、生命周... | 导入依赖 / import_depends |
...
| 544 | test_integrity_root.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
...
| 122 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | DatabaseService 真源收敛（AI-14 审计 P1 修复） ... | 导入依赖 / import_depends |
...
| 132 | D_GOV_REPAIR 治理修复: __init__.py | → | Confidence Estimator — D-022-05 置信度评估器: ... | config_depends / config_depends |
...
> 本域与 40 个外部域直接连接（出边 559 条 + 入边 374 条 = 933 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_GOVERNANCE -->|82条 导入依赖 / import_depends, 测试依赖 / test_depends| D_SHARED
