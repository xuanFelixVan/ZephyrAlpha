...
graph TD
    ...
    class D_INFRA_RUNTIME,D_INTELLIGENCE,D_GOV_ENFORCEMENT,D_INTEGRATION,D_SHARED,D_SECURITY,D_INFRASTRUCTURE,D_DATA external_prod
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_INTEGRATION,D_TRADING,D_INFRA_RECOVERY,D_GOV_AUDIT external_prod
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_SHARED,D_GOV_CODE_QUALITY,D_GOV_RULE external_prod
...
graph TD
    ...
    class D_SHARED,D_GOV_RULE,D_GOV_AUDIT,D_GOV_CODE_QUALITY,D_GOV_OPS_RESILIENCE,D_GOV_DRIFT,D_TRADING,D_FEEDBACK_LOOP external_prod
...
graph TD
    ...
    class D_SHARED,D_GOV_SCRIPTS,D_OPS,D_GOV_ENFORCEMENT,D_GOV_DRIFT,D_GOV_OPS_RESILIENCE,D_AUTONOMY_CORE,D_PF_ALLOC,D_INTEGRATION external_prod
    class D_FACTOR,D_GOV_DOCS,D_INFRA_RUNTIME external_design
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_SHARED,D_GOV_RULE,D_OPS,D_GOV_AUDIT,D_GOV_OPS_RESILIENCE,D_GOV_CODE_QUALITY,D_INTEGRATION,D_GOV_DRIFT,D_TRADING external_prod
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_INTELLIGENCE,D_GOV_SCRIPTS,D_GOV_ENFORCEMENT,D_GOV_OPS_RESILIENCE,D_GOV_CODE_QUALITY,D_INTEGRATION,D_AUTONOMY_CORE external_prod
    class D_FACTOR,D_GOV_DOCS,D_SHARED,D_EX_CORE external_design
```
...
| 126 | DatabaseManager — 连接池 + 健康检查 + 自动备份... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 127 | decisiongraph Schema DDL + 不变量声明 (decision... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 128 | decisiongraph Schema DDL + 不变量声明 (decision... | → | D_SHARED 共享服务: yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
| 129 | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 130 | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | → | D_SHARED 共享服务: SQLite 连接工厂真源（SSoT） (sqlite_factory.py) | 导入依赖 / import_depends |
| 131 | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 132 | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | → | D_SHARED 共享服务: severity_types.py | 导入依赖 / import_depends |
| 133 | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | → | D_SHARED 共享服务: task_types — 任务系统核心类型 re-export 层 (ta... | 导入依赖 / import_depends |
| 134 | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 135 | Escalation Adapter — MOD-INF-022 统一集成入口.... | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 导入依赖 / import_depends |
| 136 | A2A GovernanceAdapter — Phase 4 治理集成桥接器... | → | D_SHARED 共享服务: security_decision.py | 导入依赖 / import_depends |
| 137 | A2A GovernanceAdapter — Phase 4 治理集成桥接器... | → | D_SHARED 共享服务: async_utils.py — async/sync 边界桥接（5.12.8 .... | 导入依赖 / import_depends |
| 138 | G-CT-008 — A2ACommunication Pydantic V2 BaseMo... | → | D_SHARED 共享服务: Core A2A Protocol interface and governance data... | 导入依赖 / import_depends |
...
|:--:|---------|:--:|---------|---------|
| 1 | D_AUTONOMY_CORE 自治核心: MOD-INF-019: Agent Spec — All Skill Modules (a... | → | G-CT-008 契约：A2A -> Audit 审计 Agent 间通信. ... | runtime / runtime |
| 2 | D_BACKTEST 回测:  | → |  | 导入依赖 / import_depends |
| 3 | D_BACKTEST 回测:  | → |  | 导入依赖 / import_depends |
| 4 | D_BACKTEST 回测: BacktestResult -> decisiongraph 适配器（TRAE-06... | → | decisiongraph Schema DDL + 不变量声明 (decision... | 导入依赖 / import_depends |
...
| 73 | D_GOV_SCRIPTS 脚本治理: apply_dataflowgraph.py — dataflowgraph 变更写.... | → | dataflowgraph Schema DDL + 连接入口 (dataflowgr... | 导入依赖 / import_depends |
...
> 本域与 32 个外部域直接连接（出边 149 条 + 入边 130 条 = 279 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_GOV_DOCS -->|4条 data / data, runtime / runtime| D_GOVERNANCE
    ...
    D_AUTONOMY_CORE -->|1条 runtime / runtime| D_GOVERNANCE
```
