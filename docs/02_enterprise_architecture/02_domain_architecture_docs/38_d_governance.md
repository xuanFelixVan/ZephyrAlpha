...
graph TD
    ...
    class D_SHARED,D_SECURITY,D_INFRASTRUCTURE,D_INTEGRATION,D_INFRA_RUNTIME,D_GOV_ENFORCEMENT,D_INTELLIGENCE,D_DATA external_prod
```
...
graph TD
    ...
    class D_GOV_DOCS,D_SHARED,D_INFRASTRUCTURE,D_GOV_SCRIPTS,D_EX_CORE external_design
```
...
graph TD
    ...
    src_zephyr_governance_depgraph_schema_py -->|导入依赖 / import_depends| D_SHARED
    D_GOV_SCRIPTS["(原型态 / prototype) D_GOV_SCRIPTS"]
    D_GOV_SCRIPTS -.->|contract / contract| src_zephyr_governance_data_governance_akshare_provider_py
    ...
    class D_GOV_ENFORCEMENT,D_INTEGRATION,D_GOV_SCRIPTS,D_BACKTEST,D_EX_CORE,D_FRONTEND external_design
```
...
graph TD
    ...
    D_GOV_DOCS["(设计态 / design) D_GOV_DOCS"]
    D_GOV_DOCS -.->|runtime / runtime| src_zephyr_governance_implementations_default_security_gateway_py
    ...
    class D_SECURITY,D_OPS,D_GOV_OPS_RESILIENCE,D_INTELLIGENCE,D_GOV_REPAIR,D_GOV_AUDIT,D_INFRA_RUNTIME external_prod
    class D_SHARED,D_DATA,D_GOV_DOCS external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOV_RULE,D_GOV_OPS_RESILIENCE,D_GOV_CODE_QUALITY,D_GOV_AUDIT,D_GOV_KB,D_OPS,D_GOV_DRIFT external_prod
...
graph TD
    ...
    class D_SHARED,D_OPS,D_GOV_RULE,D_SECURITY,D_GOV_OPS_RESILIENCE,D_GOV_DRIFT,D_GOV_AUDIT,D_GOV_KB,D_INFRA_RUNTIME external_prod
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_GOV_OPS_RESILIENCE,D_GOV_CODE_QUALITY,D_OPS,D_INTEGRATION,D_GOV_ENFORCEMENT external_prod
    class D_GOV_DOCS,D_SHARED,D_GOV_SCRIPTS,D_EX_CORE external_design
```
...
| 10 | post_sync_validator — post_sync_standard 命令.... | → | D_GOV_DOCS 架构文档治理: blueprint.md | contract / contract |
| 11 | GovernanceServer: 治理域统一MCP入口 (governance... | → | D_GOV_DRIFT 漂移检测: Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | 导入依赖 / import_depends |
| 12 | GovernanceServer: 治理域统一MCP入口 (governance... | → | D_GOV_DRIFT 漂移检测: Drift Detector 基础设施 — drift_infrastructure... | 导入依赖 / import_depends |
| 13 | GovernanceServer: 治理域统一MCP入口 (governance... | → | D_GOV_DRIFT 漂移检测: Drift Detector 数据模型 — drift_models.py (dri... | 导入依赖 / import_depends |
| 14 | git_commit.py — GitCommitGateway CLI 封装（OPS... | → | D_GOV_ENFORCEMENT 规则执行: GitCommitGateway — 全项目唯一合法 git commit .... | 导入依赖 / import_depends |
| 15 | ZephyrAlpha — D_COMPLIANCE Compliance Layer —... | → | D_GOV_ENFORCEMENT 规则执行: Re-export shim — ComplianceRule 真源已合并至 z... | 导入依赖 / import_depends |
| 16 | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | → | D_GOV_ENFORCEMENT 规则执行: GitCommitGateway — 全项目唯一合法 git commit .... | 导入依赖 / import_depends |
| 17 | session 隔离 stash 红蓝对抗极限测试。 (test_ses... | → | D_GOV_ENFORCEMENT 规则执行: GitCommitGateway — 全项目唯一合法 git commit .... | 测试依赖 / test_depends |
...
| 21 | test_task_repo_gateway_e2e.py — 端到端链路测试... | → | D_GOV_ENFORCEMENT 规则执行: GitCommitGateway — 全项目唯一合法 git commit .... | 测试依赖 / test_depends |
| 22 | KB 13项一键体检 — CLI入口薄包装 (self_test.py) | → | D_GOV_KB 知识库治理: KB 13项一键体检 + --self-test入口 (self_test.py) | 导入依赖 / import_depends |
...
| 41 | A2A Protocol 全链路满分验证脚本 (a2a_full_verif... | → | D_INFRASTRUCTURE: ZephyrAlpha — 基础设施 Infrastructure Layer —... | 导入依赖 / import_depends |
| 42 | local_layer_daemon.py — L2 本地模型层守护进程.... | → | D_INFRASTRUCTURE: ZephyrAlpha — 基础设施 Infrastructure Layer —... | 导入依赖 / import_depends |
| 43 | D_EXECUTION_CORE — Risk Validation Bridge (DW-... | → | D_INFRASTRUCTURE: risk_limits.py | 导入依赖 / import_depends |
| 44 | D_EXECUTION_CORE — Simulation Broker Adapter (... | → | D_INFRASTRUCTURE: fill.py | 导入依赖 / import_depends |
| 45 | D_EXECUTION_CORE — Simulation Broker Adapter (... | → | D_INFRASTRUCTURE: order.py | 导入依赖 / import_depends |
| 46 | D_EXECUTION_CORE — Simulation Broker Adapter (... | → | D_INFRASTRUCTURE: position.py | 导入依赖 / import_depends |
...
| 60 | post_sync_validator — post_sync_standard 命令.... | → | D_INFRA_RUNTIME 运行时集成: RI-15 CostTracker — 成本追踪器 (cost_tracker.py) | runtime / runtime |
...
| 35 | D_GOV_DOCS 架构文档治理: blueprint.md | → | default_security_gateway.py | runtime / runtime |
...
| 85 | D_GOV_SCRIPTS 脚本治理: G-acqflow: 从 tasks.yaml 生成业务数据采集流图 M... | → | D_DATA — Akshare Data Provider (akshare_provid... | contract / contract |
...
| 107 | D_INFRA_RUNTIME 运行时集成: RI-15 CostTracker — 成本追踪器 (cost_tracker.py) | → | Batch2 治理层契约 — 15条 Pydantic v2 Schema（P... | runtime / runtime |
...
> 本域与 30 个外部域直接连接（出边 149 条 + 入边 131 条 = 280 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_GOV_SCRIPTS -->|40条 contract / contract, 导入依赖 / import_depends| D_GOVERNANCE
