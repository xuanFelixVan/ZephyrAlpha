...
graph TD
    ...
    class D_GOV_DOCS,D_SHARED,D_GOV_KB,D_INTELLIGENCE,D_INFRASTRUCTURE,D_INTEGRATION_GATEWAY,D_GOV_DRIFT,D_AUTONOMY_CORE,D_EX_CORE,D_GOV_AUDIT,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_FACTOR,D_GOV_DOCS,D_SHARED,D_GOV_ENFORCEMENT,D_INTEGRATION,D_FRONTEND,D_GOV_SCRIPTS,D_DATA,D_INFRASTRUCTURE external_design
```
...
graph TD
    ...
    class D_AUTONOMY_CORE,D_OPS,D_GOV_AUDIT,D_GOV_CODE_QUALITY,D_GOV_SCRIPTS,D_GOV_OPS_RESILIENCE external_prod
    class D_GOV_DOCS,D_FBL_VERIFICATION,D_FEEDBACK_LOOP,D_COMPLIANCE,D_GOV_DRIFT,D_ML_TRAIN,D_INTELLIGENCE,D_SHARED external_design
```
...
graph TD
    ...
    class D_INFRA_A2A,D_FACTOR,D_GOV_DOCS,D_AUTONOMY_CORE,D_GOV_SCRIPTS,D_TRADING,D_DATA external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOV_AUDIT,D_SECURITY,D_GOV_RULE,D_GOV_OPS_RESILIENCE,D_GOV_DRIFT,D_GOV_REPAIR,D_INFRA_RUNTIME,D_FEEDBACK_LOOP,D_GOV_CODE_QUALITY external_prod
    class D_GOV_DOCS,D_INTEGRATION,D_GOV_SCRIPTS,D_GOV_ENFORCEMENT,D_TRADING,D_DATA,D_INTELLIGENCE external_design
```
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 556 个，110 条域内依赖）。
...
|:--:|---------|:--:|---------|---------|
| 1 | model_provider_data.py | → | D_AUTONOMY_CORE 自治核心: sensitivity_classifier.py — 数据分级 (B9, DD83... | contract / contract |
...
| 28 | test_scheduler_act.py | → | D_FBL_DETECTORS: R519: GuardOscillationDetector (guard_oscillati... | 测试依赖 / test_depends |
...
| 31 | test_scheduler_act.py | → | D_FBL_DIAGNOSERS: Self-Bottleneck Detector — v0.38.0 R479 (self_... | 测试依赖 / test_depends |
| 32 | test_scheduler_act.py | → | D_FBL_DIAGNOSERS: R506: ContextWindowPressureManager (context_win... | 测试依赖 / test_depends |
| 33 | Self-Benchmark (W3-7) — 5 组已知对自验证 + 引.... | → | D_FBL_VERIFICATION 反馈验证: _governance_gates.py | runtime / runtime |
...
| 37 | test_scheduler_act.py | → | D_FEEDBACK_LOOP 反馈循环引擎: R522: SelfModificationRateLimiter (self_modific... | 测试依赖 / test_depends |
| 38 | test_scheduler_act.py | → | D_FEEDBACK_LOOP 反馈循环引擎: Graceful Degradation Planner — v0.40.0 R496 (g... | 测试依赖 / test_depends |
| 39 | test_scheduler_act.py | → | D_FEEDBACK_LOOP 反馈循环引擎: Self API Throttle Defense — v0.39.0 R491 (self... | 测试依赖 / test_depends |
| 40 | test_scheduler_act.py | → | D_FEEDBACK_LOOP 反馈循环引擎: scheduler_act.py | 测试依赖 / test_depends |
| 41 | test_app_panel_unit · app_panel.py 单元测试（v... | → | D_FRONTEND 前端: app_panel · Panel 仪表盘主应用入口（v3.1.0, #A... | 测试依赖 / test_depends |
| 42 | test_app_panel_unit · app_panel.py 单元测试（v... | → | D_FRONTEND 前端: backtest_results · 回测结果可视化组件（v3.0.0 ... | 测试依赖 / test_depends |
| 43 | test_p1_components_unit · 5 个 P1 交易/回测组.... | → | D_FRONTEND 前端: backtest_results · 回测结果可视化组件（v3.0.0 ... | 测试依赖 / test_depends |
| 44 | test_p1_components_unit · 5 个 P1 交易/回测组.... | → | D_FRONTEND 前端: order_book · 5档盘口实时展示组件（v3.0.0 Panel... | 测试依赖 / test_depends |
| 45 | test_p1_components_unit · 5 个 P1 交易/回测组.... | → | D_FRONTEND 前端: position_monitor · 实盘持仓监控组件（v3.0.0 Pa... | 测试依赖 / test_depends |
| 46 | test_p1_components_unit · 5 个 P1 交易/回测组.... | → | D_FRONTEND 前端: tick_replay · Tick 回放可视化组件（v3.0.0 Pane... | 测试依赖 / test_depends |
| 47 | test_p1_components_unit · 5 个 P1 交易/回测组.... | → | D_FRONTEND 前端: trade_panel · 实盘交易面板组件（v3.0.0 Panel+H... | 测试依赖 / test_depends |
| 48 | C-track 端到端演示 —— 全流水线一次性运行 (dem... | → | D_FUNDAMENTAL_SIGNAL 基本面信号: D_SIGNAL Signal Domain (__init__.py) | 导入依赖 / import_depends |
| 49 | apply_verification_results.py — 第32轮验证结果... | → | D_GOV_AUDIT 审计追踪: [INVARIANTS] 按path精确匹配+按功能名模糊匹配; .... | config_depends / config_depends |
| 50 | ProjectionEngine — 事件折叠为当前状态（DW-0003... | → | D_GOV_AUDIT 审计追踪: EventStore — Event Sourcing 事件追加与回放（DW... | 导入依赖 / import_depends |
| 51 | DatabaseManager — 连接池 + 健康检查 + 自动备份... | → | D_GOV_AUDIT 审计追踪: audit_schema — 审计视图与查询入口（SH-DB-001 v... | 导入依赖 / import_depends |
| 52 | GovernanceServer: 治理域统一MCP入口 (governance... | → | D_GOV_AUDIT 审计追踪: writer.py | 导入依赖 / import_depends |
| 53 | test_anomaly.py | → | D_GOV_AUDIT 审计追踪: anomaly.py | 测试依赖 / test_depends |
| 54 | test_anomaly.py | → | D_GOV_AUDIT 审计追踪: models.py | 测试依赖 / test_depends |
...
| 267 | kb.pipeline — auto-generated package init. (__... | → | D_GOV_KB 知识库治理: G4 Activate 门禁 — 人工激活（T-2-13-D） (activ... | config_depends / config_depends |
...
| 498 | test_capability_check.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.cap... | 测试依赖 / test_depends |
...
| 502 | test_governance_a2a_check.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.a2a... | 测试依赖 / test_depends |
| 503 | test_governance_approver_check.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.app... | 测试依赖 / test_depends |
| 504 | test_governance_bootstrap_superadmin.py | → | D_SECURITY 对抗验证: BootstrapSuperadmin — Superadmin 账户启动器. (... | 测试依赖 / test_depends |
| 505 | test_governance_capability_check.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.cap... | 测试依赖 / test_depends |
| 506 | test_governance_contracts.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.con... | 测试依赖 / test_depends |
...
| 148 | D_GOV_REPAIR 治理修复: __init__.py | → | context_recycling.py | config_depends / config_depends |
...
| 153 | D_GOV_REPAIR 治理修复: __init__.py | → | AISG Sandbox Testing — AI Security Gateway 沙.... | config_depends / config_depends |
| 154 | D_GOV_REPAIR 治理修复: __init__.py | → | migration_strategy.py | config_depends / config_depends |
| 155 | D_GOV_REPAIR 治理修复: __init__.py | → | Objective Tracker — v0.9.0 目标漂移检测器: age... | config_depends / config_depends |
...
| 158 | D_GOV_REPAIR 治理修复: Re-export wrapper: true source is zephyr.pf_cor... | → | D_PORTFOLIO_CORE — StrategyBase + StrategyMeta... | config_depends / config_depends |
...
| 261 | D_GOV_SCRIPTS 脚本治理: validate_static_manifest_drift.py — GATE-21 静... | → | __init__.py | config_depends / config_depends |
