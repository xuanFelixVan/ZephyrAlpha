...
| 71 | tests/orchestrator/test_deferred_queue.py | test_deferred_queue.py | 原型态 / prototype | [MOD-INF-039](../../03_modules/_cross_layer/agent_orchestrator/blueprint.md) |
| 72 | tests/orchestrator/test_orchestrator_data_lifecycle.py | test_orchestrator_data_lifecycle.py | 原型态 / prototype | [MOD-INF-039](../../03_modules/_cross_layer/agent_orchestrator/blueprint.md) |
| 73 | tests/orchestrator/test_orchestrator_failure_matcher.py | test_orchestrator_failure_matcher.py | 原型态 / prototype | [MOD-INF-039](../../03_modules/_cross_layer/agent_orchestrator/blueprint.md) |
| 74 | tests/orchestrator/test_orchestrator_hallucination_detect... | test_orchestrator_hallucination_detector.py | 原型态 / prototype | [MOD-INF-039](../../03_modules/_cross_layer/agent_orchestrator/blueprint.md) |
| 75 | tests/orchestrator/test_orchestrator_model_registry.py | test_orchestrator_model_registry.py | 原型态 / prototype | [MOD-INF-039](../../03_modules/_cross_layer/agent_orchestrator/blueprint.md) |
| 76 | tests/orchestrator/test_orchestrator_rollback_manager.py | test_orchestrator_rollback_manager.py | 原型态 / prototype | [MOD-INF-039](../../03_modules/_cross_layer/agent_orchestrator/blueprint.md) |
| 77 | tests/orchestrator/test_orchestrator_task_queue.py | test_orchestrator_task_queue.py | 原型态 / prototype | [MOD-INF-039](../../03_modules/_cross_layer/agent_orchestrator/blueprint.md) |
| 78 | tests/orchestrator/test_orchestrator_trigger_router.py | test_orchestrator_trigger_router.py | 原型态 / prototype | [MOD-INF-039](../../03_modules/_cross_layer/agent_orchestrator/blueprint.md) |
| 79 | tests/orchestrator/test_orchestrator_wave_generator.py | test_orchestrator_wave_generator.py | 原型态 / prototype | [MOD-INF-039](../../03_modules/_cross_layer/agent_orchestrator/blueprint.md) |
...
> 展示全部 79 个模块（生产态 58 + 设计态 0 + 原型态 21），标签标注成熟度。
...
graph TD
    ...
    class D_INTEGRATION,D_SHARED,D_GOV_OPS_RESILIENCE,D_SECURITY,D_INFRA_RUNTIME,D_GOVERNANCE external_prod
    class D_INFRASTRUCTURE,D_FEEDBACK_LOOP,D_TRADING,D_AUTONOMY_CORE,D_INFRA_RECOVERY external_design
```
...
graph TD
    ...
    class D_AUTONOMY_CORE,D_GOV_ENFORCEMENT,D_INFRA_RECOVERY,D_GOV_SCRIPTS,D_RISK,D_TRADING external_design
```
...
graph TD
    ...
    class D_TRADING,D_KNOWLEDGE,D_GOV_SCRIPTS,D_GOVERNANCE external_design
```
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 21 个，3 条域内依赖）。
...
| 12 | D_GOVERNANCE 生命周期管理: __init__.py | → | Orc 告警接收器 — handle_alert() 消费者 (alert_... | 导入依赖 / import_depends |
| 13 | D_GOVERNANCE 生命周期管理: __init__.py | → | Orc->CE 上下文桥接 — request_context() 生产者 ... | 导入依赖 / import_depends |
| 14 | D_GOVERNANCE 生命周期管理: __init__.py | → | Orc->VMS 记忆写入器 (memory_writer.py) | 导入依赖 / import_depends |
| 15 | D_GOVERNANCE 生命周期管理: __init__.py | → | Orc->Script 脚本执行器 — run_audit() 生产者 (s... | 导入依赖 / import_depends |
| 16 | D_GOVERNANCE 生命周期管理: orchestrator.core — auto-generated package ini... | → | ActiveTaskQueue — 后台任务轮询与自动分发 (task... | config_depends / config_depends |
| 17 | D_GOVERNANCE 生命周期管理: orchestrator.resilience — auto-generated packa... | → | FailurePatternMatcher — 任务失败模式识别与纠正... | 导入依赖 / import_depends |
| 18 | D_GOV_AUDIT 审计追踪: test_phase_executor_root.py | → | Phase 执行引擎（Phase Executor） (phase_executo... | 测试依赖 / test_depends |
| 19 | D_GOV_ENFORCEMENT 规则执行: test_capacity_budget_root.py | → | 全局容量预算控制器（Capacity Budget Controller... | 测试依赖 / test_depends |
| 20 | D_GOV_SCRIPTS 脚本治理: test_blueprint_scorer.py | → | BlueprintScorer — 蓝图路由统一打分逻辑 (bluepr... | 测试依赖 / test_depends |
| 21 | D_GOV_SCRIPTS 脚本治理: test_dependency_lock.py | → | 外部依赖版本锁（CT-DEPS）——Python包版本锁定+h... | 测试依赖 / test_depends |
| 22 | D_INFRASTRUCTURE: test_contract_registry_root.py | → | 集成契约注册表（Contract Registry） (contract_r... | 测试依赖 / test_depends |
| 23 | D_INFRASTRUCTURE: test_contract_router_root.py | → | 集成契约注册表（Contract Registry） (contract_r... | 测试依赖 / test_depends |
| 24 | D_INFRASTRUCTURE: test_contract_router_root.py | → | 契约路由（Contract Router） (contract_router.py) | 测试依赖 / test_depends |
| 25 | D_INFRA_RECOVERY 回滚恢复: test_canary_manager.py | → | 金丝雀发布管理器（CT-CANARY）——权重分流+指标.... | 测试依赖 / test_depends |
| 26 | D_INFRA_RECOVERY 回滚恢复: test_chaos_engine.py | → | Chaos 故障注入引擎（CT-CHAOS-001）——4注入点×... | 测试依赖 / test_depends |
| 27 | D_INFRA_RECOVERY 回滚恢复: test_chaos_engine_ops.py | → | Chaos 故障注入引擎（CT-CHAOS-001）——4注入点×... | 测试依赖 / test_depends |
| 28 | D_INFRA_RECOVERY 回滚恢复: test_chaos_hooks.py | → | Chaos 故障注入引擎（CT-CHAOS-001）——4注入点×... | 测试依赖 / test_depends |
| 29 | D_INFRA_RECOVERY 回滚恢复: test_chaos_hooks.py | → | ChaosHook — integrates ChaosEngine with the or... | 测试依赖 / test_depends |
| 30 | D_INFRA_RUNTIME 运行时集成: boot_hooks.py | → | Orc->VMS 记忆写入器 (memory_writer.py) | 导入依赖 / import_depends |
| 31 | D_INTELLIGENCE 上下文管理: test_pipeline_agent_bridge.py | → | AgentOrchestrator · 多角色 Agent 路由、工具链.... | 测试依赖 / test_depends |
| 32 | D_KNOWLEDGE 知识管理: test_ke_quality.py | → | 知识质量评分契约（CT-KE-QUALITY）——KE完整性+.... | 测试依赖 / test_depends |
| 33 | D_KNOWLEDGE 知识管理: test_knowledge_freshness.py | → | 知识新鲜度废止管理器（CT-KNOWLEDGE-FRESHNESS）.... | 测试依赖 / test_depends |
| 34 | D_RISK 风控: test_risk_registry_root.py | → | risk_registry.py | 测试依赖 / test_depends |
| 35 | D_SECURITY_LLM LLM防御: test_cross_module_integration_llm_security.py | → | AgentOrchestrator · 多角色 Agent 路由、工具链.... | 测试依赖 / test_depends |
| 36 | D_SHARED 共享服务: test_file_task_mapper_root.py | → | FileTaskMapper — 文件路径 ↔ Task N:N 映射器（... | 测试依赖 / test_depends |
| 37 | D_SHARED 共享服务: test_path_index.py | → | path_index.py | 测试依赖 / test_depends |
| 38 | D_TRADING 交易运营: AutoDispatcher — 守护进程内的轻量 PipelineDisp... | → | ActiveTaskQueue — 后台任务轮询与自动分发 (task... | 导入依赖 / import_depends |
| 39 | D_TRADING 交易运营: AutoDispatcher — 守护进程内的轻量 PipelineDisp... | → | Orc->CE 上下文桥接 — request_context() 生产者 ... | 导入依赖 / import_depends |
| 40 | D_TRADING 交易运营: AutoDispatcher — 守护进程内的轻量 PipelineDisp... | → | Orc->Script 脚本执行器 — run_audit() 生产者 (s... | 导入依赖 / import_depends |
| 41 | D_TRADING 交易运营: test_batch_orchestrator.py | → | BatchOrchestrator — 多 Worker 批量任务协调器（... | 测试依赖 / test_depends |
| 42 | D_TRADING 交易运营: test_benchmark_runner.py | → | benchmark_runner.py | 测试依赖 / test_depends |
| 43 | D_TRADING 交易运营: test_blind_spot_closure.py | → | blind_spot_closure.py | 测试依赖 / test_depends |
| 44 | D_TRADING 交易运营: test_bulkhead_manager.py | → | bulkhead_manager.py | 测试依赖 / test_depends |
| 45 | D_TRADING 交易运营: test_construction_guide.py | → | 施工指南引擎（Construction Guide） (constructio... | 测试依赖 / test_depends |
| 46 | D_TRADING 交易运营: test_degrade_cascade.py | → | degrade_cascade.py | 测试依赖 / test_depends |
| 47 | D_TRADING 交易运营: test_design_decisions_root.py | → | design_decisions.py | 测试依赖 / test_depends |
| 48 | D_TRADING 交易运营: test_disk_guard.py | → | disk_guard.py | 测试依赖 / test_depends |
| 49 | D_TRADING 交易运营: test_dlq_manager_root.py | → | DLQ 管理器（Dead Letter Queue Manager — CT-DLQ... | 测试依赖 / test_depends |
| 50 | D_TRADING 交易运营: test_fault_types.py | → | Fault type registry and preset templates for ch... | 测试依赖 / test_depends |
| 51 | D_TRADING 交易运营: test_feature_flag.py | → | FeatureFlag 管理器（CT-FEATUREFLAG-001）——CT-... | 测试依赖 / test_depends |
| 52 | D_TRADING 交易运营: test_finding_bridge.py | → | CT-ORC-SCRIPT-001 运行时桥接 (finding_bridge.py) | 测试依赖 / test_depends |
| 53 | D_TRADING 交易运营: test_housekeeping.py | → | 文件卫生保洁管理器（CT-HOUSEKEEPING）——临时文... | 测试依赖 / test_depends |
| 54 | D_TRADING 交易运营: test_incident_postmortem.py | → | 事件复盘管理器（CT-INCIDENT）——incident记录+t... | 测试依赖 / test_depends |
| 55 | D_TRADING 交易运营: test_lean_scanner.py | → | 死代码/孤儿文件/僵尸引用三扫描（CT-LEAN）——三... | 测试依赖 / test_depends |
| 56 | D_TRADING 交易运营: test_network_partition.py | → | 网络分区容忍（CT-NETWORK-PARTITION）——CAP定理... | 测试依赖 / test_depends |
| 57 | D_TRADING 交易运营: test_reconciliation_loop.py | → | reconciliation_loop.py | 测试依赖 / test_depends |
| 58 | D_TRADING 交易运营: test_rolling_upgrade.py | → | 零停机滚动升级（CT-DEPLOY）——graceful shutdow... | 测试依赖 / test_depends |
| 59 | D_TRADING 交易运营: test_schema_migration.py | → | 数据库 Schema 演化契约（CT-SCHEMA-MIGRATE）——... | 测试依赖 / test_depends |
| 60 | D_TRADING 交易运营: test_stability_guard.py | → | API 稳定性守护（CT-STABILITY）——public API签.... | 测试依赖 / test_depends |
| 61 | D_TRADING 交易运营: test_startup_sequencer.py | → | startup_sequencer.py | 测试依赖 / test_depends |
| 62 | D_TRADING 交易运营: test_state_propagation_root.py | → | 全局状态传播链（State Propagation Chain） (stat... | 测试依赖 / test_depends |
| 63 | D_TRADING 交易运营: test_state_synchronizer_root.py | → | StateSynchronizer — 同步 SQLite 状态与文件系统... | 测试依赖 / test_depends |
| 64 | D_TRADING 交易运营: test_system_transfer.py | → | 系统移交恢复（CT-TRANSFER）——系统Owner变更+配... | 测试依赖 / test_depends |
| 65 | D_TRADING 交易运营: test_teardown_manager.py | → | teardown_manager.py | 测试依赖 / test_depends |
| 66 | D_TRADING 交易运营: test_version_manifest.py | → | version_manifest.py | 测试依赖 / test_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 19 个外部域直接连接（出边 52 条 + 入边 66 条 = 118 条）。只显示直接连接的域，不展开具体节点。
