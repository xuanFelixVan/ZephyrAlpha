...
flowchart TD
    scripts_calibrate_model_diff_py["(生产态 / production)<br/>模型能力差异校准脚本（P1-3 治本）。 /<br/>calibrate_model_diff<br/>模型能力差异校准脚本（P1-3 治本）。<br/>文件: scripts/calibrate_model_diff.py"]
    scripts_quick_profile_py["(生产态 / production) quick画像 / quick_<br/>profile<br/>模型快速能力画像脚本 (P2 三级模式 Quick<br/>入口)。<br/>文件: scripts/quick_profile.py"]
    src_zephyr_intelligence_model_drift_detector_py["(生产态 / production) 模型漂移检测器 /<br/>model_drift_detector<br/>ModelDriftDetector — LLM<br/>模型行为漂移检测。<br/>文件: intelligence/model_drift_<br/>detector.py"]
    ...
    src_zephyr_intelligence_model_evaluation_implementations_default_inference_engine_py["(生产态 / production) 默认推理引擎 / D_<br/>ML_TRAIN — Default Inference Engine<br/>默认推理引擎。D_ML_TRAIN — Default<br/>Inference Engine<br/>文件: implementations/default_inference_<br/>engine.py"]
    src_zephyr_intelligence_model_evaluation_inference_base_py["(生产态 / production) 推理基类 /<br/>inference_base<br/>推理基类，模型的基类，定义抽象接口供子类<br/>实现。<br/>文件: model_evaluation/inference_base.py"]
    src_zephyr_intelligence_model_evaluation_reranker_py["(生产态 / production) Cross-Encoder<br/>重排序层 — BGE-reranker-v2-m3 / reranker<br/>Cross-Encoder 重排序层 —<br/>BGE-reranker-v2-m3<br/>文件: model_evaluation/reranker.py"]
    src_zephyr_intelligence_model_evaluation_unified_memory_api_py["(生产态 / production) unified记忆API /<br/>unified_memory_api<br/>UnifiedMemoryAPI — RI-02 统一记忆 API<br/>（M2 跨模块封装）<br/>文件: model_evaluation/unified_memory_<br/>api.py"]
    ...
    src_zephyr_intelligence_model_profiling_deepseek_v4_chat_py["(生产态 / production) DeepSeekV4Chat<br/>--- DeepSeek V4 系列模型 API  /<br/>deepseek_v4_chat<br/>DeepSeekV4Chat --- DeepSeek V4 系列模型<br/>API 客户端<br/>文件: model_profiling/deepseek_v4_<br/>chat.py"]
    ...
    src_zephyr_intelligence_model_profiling_pipeline_routing_task_model_learner_py["(生产态 / production) 任务模型学习器 /<br/>task_model_learner<br/>ModelTaskMatrix — 任务×模型性能学习引擎<br/>文件: pipeline_routing/task_model_<br/>learner.py"]
    src_zephyr_intelligence_model_profiling_task_model_learner_py["(生产态 / production) 任务模型学习器 /<br/>task_model_learner<br/>ModelTaskMatrix — 任务×模型性能学习引擎<br/>文件: model_profiling/task_model_<br/>learner.py"]
    ...
    src_zephyr_intelligence_model_evaluation_memory_backend_py["(生产态 / production) 记忆后端 /<br/>Backend protocol & shared data classes<br/>for the unified memor<br/>记忆后端。Backend protocol & shared<br/>data classes for the unified memory<br/>layer.<br/>文件: model_evaluation/_memory_<br/>backend.py"]
    src_zephyr_intelligence_model_profiling_exam_orchestrator_py["(生产态 / production) exam编排器 / exam_<br/>orchestrator<br/>ExamOrchestrator --- 五轴入职考试主控<br/>文件: model_profiling/exam_<br/>orchestrator.py"]
    src_zephyr_intelligence_model_profiling_pipeline_routing_results_writer_py["(生产态 / production) results写入器 /<br/>results_writer<br/>Results Writer — 持久化 benchmark<br/>结果，支持历史对比（漂移检测）<br/>文件: pipeline_routing/results_writer.py"]
    src_zephyr_intelligence_model_profiling_results_writer_py["(生产态 / production) results写入器 /<br/>results_writer<br/>Results Writer — 持久化 benchmark<br/>结果，支持历史对比（漂移检测）<br/>文件: model_profiling/results_writer.py"]
    ...
    src_zephyr_intelligence_model_profiling_exam_checks_py["(生产态 / production) examchecks.py —<br/>考试检测纯函数模块（Stage 4 试点：从  /<br/>exam_checks<br/>考试检测纯函数模块（Stage 4 试点：从<br/>exam_orchestrator 提取）<br/>文件: model_profiling/exam_checks.py"]
    src_zephyr_intelligence_model_profiling_exam_executor_py["(生产态 / production) exam执行器 / exam_<br/>executor<br/>ExamExecutor --- 执行式代码评测<br/>（HumanEval pass@1 风格，v3.0.5）。<br/>文件: model_profiling/exam_executor.py"]
    src_zephyr_intelligence_model_profiling_exam_judge_py["(生产态 / production) exam判定 / exam_<br/>judge<br/>ExamJudge --- LLM-as-judge 评分器<br/>文件: model_profiling/exam_judge.py"]
    src_zephyr_intelligence_model_profiling_exam_rubric_py["(生产态 / production) ExamRubric ---<br/>奥赛题结构化多维清单评分（v3.0.5）。 /<br/>exam_rubric<br/>ExamRubric ---<br/>奥赛题结构化多维清单评分（v3.0.5）。<br/>文件: model_profiling/exam_rubric.py"]
    src_zephyr_intelligence_model_profiling_job_matcher_py["(生产态 / production) job匹配器 / job_<br/>matcher<br/>JobMatcher --- 模型岗位匹配器<br/>文件: model_profiling/job_matcher.py"]
    src_zephyr_intelligence_model_profiling_pipeline_routing_profiler_py["(生产态 / production) 性能分析器 /<br/>profiler<br/>ModelProfiler — 核心性能分析引擎<br/>文件: pipeline_routing/profiler.py"]
    src_zephyr_intelligence_model_profiling_profiler_py["(生产态 / production) 性能分析器 /<br/>profiler<br/>ModelProfiler — 核心性能分析引擎<br/>文件: model_profiling/profiler.py"]
    ...
    src_zephyr_intelligence_model_profiling_benchmark_suite_py["(生产态 / production) 基准suite /<br/>benchmark_suite<br/>BenchmarkSuite —<br/>多维度模型性能测试用例集<br/>文件: model_profiling/benchmark_suite.py"]
    src_zephyr_intelligence_model_profiling_capability_passport_py["(生产态 / production) 能力passport /<br/>capability_passport<br/>CapabilityPassport --- AI 模型能力护照<br/>文件: model_profiling/capability_<br/>passport.py"]
    src_zephyr_intelligence_model_profiling_exam_test_cases_py["(生产态 / production) exam测试cases /<br/>exam_test_cases<br/>exam测试cases.0.5 扩展考试题库（96 题 /<br/>29 能力 / 5 难度）<br/>文件: model_profiling/exam_test_cases.py"]
    src_zephyr_intelligence_model_profiling_model_discovery_py["(生产态 / production) 模型discovery /<br/>model_discovery<br/>ModelDiscovery — 枚举所有本地 Ollama<br/>模型 + 远程 API 模型<br/>文件: model_profiling/model_discovery.py"]
    src_zephyr_intelligence_model_profiling_pipeline_routing_benchmark_suite_py["(生产态 / production) 基准suite /<br/>benchmark_suite<br/>BenchmarkSuite —<br/>多维度模型性能测试用例集<br/>文件: pipeline_routing/benchmark_<br/>suite.py"]
    ...
    src_zephyr_intelligence_model_profiling_case_assembler_py["(生产态 / production)<br/>真实多文件注入装配器（Phase 3<br/>极限深度）。 / case_assembler<br/>真实多文件注入装配器（Phase 3<br/>极限深度）。<br/>文件: model_profiling/case_assembler.py"]
    src_zephyr_intelligence_model_profiling_provider_data_py["(生产态 / production) 提供器数据 /<br/>provider_data<br/>提供器数据，供src.zephyr.infrastructure.<br/>budg使用<br/>文件: model_profiling/provider_data.py"]
