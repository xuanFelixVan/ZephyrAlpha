...
flowchart TD
    ...
    src_zephyr_trading_trading_contracts_market_instrument_py["标的合约<br/>供data ; factor ; pf_core ; ex_c使用<br/>instrument<br/>文件: market/instrument.py<br/>(生产态 / production)"]
...
| 59 | 过渡兼容层（DEPRECATED）—— Money 契约 canonical 真 / mo... | → | D_SHARED 共享服务: 金额精度异常 / money (portfolio/money.py) | 导入依赖 / import_depends |
