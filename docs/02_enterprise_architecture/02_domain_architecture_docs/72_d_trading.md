...
flowchart TD
    ...
    src_zephyr_trading_pnl_calculator_py -->|导入依赖 / import_depends| D_REPORTING
    ...
    class D_REPORTING,D_POSITION,D_EX_CORE,D_SHARED,D_INFRASTRUCTURE,D_GOV_AUDIT,D_INFRA_RUNTIME,D_FUNDAMENTAL_SIGNAL,D_RISK,D_ML_TRAIN,D_SIGQC,D_GOVERNANCE external_prod
```
...
| 37 | D_TRADING — PnL Calculator (盈亏计算器) (trading/pnl_cal... | → | D_REPORTING 报告: 实时盈亏仪表盘 / realtime_pnl_dashboard (reporting/realti... | 导入依赖 / import_depends |
| 38 | 风险仪表盘快照 / risk_dashboard_snapshot (risk/risk_dashb... | → | D_REPORTING 报告: 实时盈亏仪表盘 / realtime_pnl_dashboard (reporting/realti... | contract / contract |
...
| 26 | D_RISK 风控: 风控管理器 / risk_manager (risk/risk_manager.py) | → | 风险仪表盘快照 / risk_dashboard_snapshot (risk/risk_dashb... | 导入依赖 / import_depends |
| 27 | D_RISK 风控: 风控管理器 / risk_manager (risk/risk_manager.py) | → | 风险限制违规错误 / risk_limit_violation_error (risk/risk_... | 导入依赖 / import_depends |
| 28 | D_RISK 风控: 风控管理器 / risk_manager (risk/risk_manager.py) | → | 风险指标 / risk_metrics (risk/risk_metrics.py) | 导入依赖 / import_depends |
| 29 | D_SIGQC 信号质量控制: 退化监控基类 / D_SIGQC — Signal Quality Degradation Moni... | → | 信号退化警告 / signal_degradation_warning (market/signal_... | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 16 个外部域直接连接（出边 64 条 + 入边 29 条 = 93 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_TRADING -->|2条 contract / contract, 导入依赖 / import_depends| D_REPORTING
