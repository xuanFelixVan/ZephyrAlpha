...
flowchart TD
    src_zephyr_trading_action_dispatcher_init_py["(生产态 / production) 包入口 / __init__<br/>BRAIN 注释块文本编辑器<br/>（ActionDispatcher<br/>协作者，职责簇：纯文本块构建/插入<br/>/更新，无 I/O 无状态）。<br/>文件: action_dispatcher/__init__.py"]
    src_zephyr_trading_admission_controller_py["(生产态 / production) 准入控制器 /<br/>admission_controller<br/>5.171 修复：admit(event: Any) Any<br/>滥用——定义 VerdictEvent Protocol<br/>文件: trading/admission_controller.py"]
    src_zephyr_trading_auto_dispatcher_py["(生产态 / production) 自动分发器 / auto_<br/>dispatcher<br/>AutoDispatcher — 守护进程内的轻量<br/>PipelineDispatcher<br/>文件: trading/auto_dispatcher.py"]
    src_zephyr_trading_conductor_py["(生产态 / production) Conductor — AI<br/>session 全自动指挥官。 / conductor<br/>Conductor — AI session 全自动指挥官。<br/>文件: trading/conductor.py"]
    src_zephyr_trading_corporate_action_processor_py["(设计态 / design) 公司行为处理器 /<br/>corporate_action_processor<br/>公司行为处理器，交易的处理器，处理加工数<br/>据。<br/>文件: trading/corporate_action_<br/>processor.py<br/>⛔ 交易域，设计已就绪，等待开发排期"]
    src_zephyr_trading_gpu_consensus_scheduler_py["(生产态 / production) GPU共识调度器 /<br/>gpu_consensus_scheduler<br/>GPU共识调度器，交易的调度器，按时间或优<br/>先级安排任务。<br/>文件: trading/gpu_consensus_scheduler.py"]
    src_zephyr_trading_gpu_monitor_py["(生产态 / production) GPU监控 / gpu_<br/>monitor<br/>NVIDIA GPU 状态采集器<br/>文件: trading/gpu_monitor.py"]
    src_zephyr_trading_ide_health_daemon_py["(生产态 / production) ide健康daemon /<br/>ide_health_daemon<br/>TRAE IDE 幽灵窗口守护线程<br/>文件: trading/ide_health_daemon.py"]
    src_zephyr_trading_pnl_calculator["(设计态 / design) 盈亏计算器<br/>盈亏计算器，盈亏计算的子目录，归集相关子<br/>模块。<br/>文件: pnl_calculator/<br/>⛔ 交易域，设计已就绪，等待开发排期"]
    src_zephyr_trading_runtime_async_runtime_py["(生产态 / production) 异步运行时 /<br/>async_runtime<br/>事件循环引导 + run_in_executor 桥接。<br/>文件: runtime/async_runtime.py"]
    src_zephyr_trading_settlement_reconciliation_py["(设计态 / design) 结算对账 / settlement_<br/>reconciliation<br/>结算对账（settlement_reconciliation.py）<br/>文件: trading/settlement_<br/>reconciliation.py<br/>⛔ 交易域，设计已就绪，等待开发排期"]
    src_zephyr_trading_speed_baseline_checker_py["(生产态 / production) 测速基线检查器 /<br/>speed_baseline_checker<br/>speed基线检查器，交易的检查器，检查条件<br/>是否满足。<br/>文件: trading/speed_baseline_checker.py"]
    src_zephyr_trading_trading_contracts_broker_interface_py["(生产态 / production) 经纪人接口 / D_<br/>EXECUTION_CORE — BrokerInterface<br/>经纪人接口。D_EXECUTION_CORE —<br/>BrokerInterface<br/>文件: trading_contracts/broker_<br/>interface.py"]
    src_zephyr_trading_trading_contracts_execution_capital_allocation_result_py["(生产态 / production) 资本分配结果 /<br/>capital_allocation_result<br/>资本allocation结果，执行的结果，封装操作<br/>结果的数据结构。<br/>文件: execution/capital_allocation_<br/>result.py"]
    src_zephyr_trading_trading_contracts_execution_execution_rejection_error_py["(生产态 / production) 执行拒绝错误 /<br/>execution_rejection_error<br/>执行拒绝错误，执行的异常，定义本模块的异<br/>常类型。<br/>文件: execution/execution_rejection_<br/>error.py"]
    src_zephyr_trading_trading_contracts_execution_execution_report_py["(生产态 / production) 执行报告 /<br/>execution_report<br/>Re-export wrapper: ExecutionReport<br/>真源在 zephyr.shared.contracts.execution<br/>_report（CTR-P1-007 codegen）<br/>文件: execution/execution_report.py"]
    src_zephyr_trading_trading_contracts_execution_fill_py["(生产态 / production) 成交 / fill<br/>Re-export wrapper: Fill 真源在<br/>zephyr.shared.contracts.fill（CTR-005<br/>codegen）<br/>文件: execution/fill.py"]
    src_zephyr_trading_trading_contracts_execution_model_serving_request_py["(生产态 / production) 模型服务请求 /<br/>model_serving_request<br/>模型服务请求，执行的模型，定义数据结构和<br/>字段。<br/>文件: execution/model_serving_request.py"]
    src_zephyr_trading_trading_contracts_execution_position_py["(生产态 / production) 持仓 / position<br/>Re-export wrapper: PositionSnapshot<br/>真源在 zephyr.shared.contracts.position<br/>（CTR-006 codegen）<br/>文件: execution/position.py"]
    ...
    src_zephyr_trading_trading_contracts_market_instrument_py["(生产态 / production) 标的合约 /<br/>instrument<br/>标的合约，供data ; factor ; pf_core ;<br/>ex_c使用<br/>文件: market/instrument.py"]
    src_zephyr_trading_trading_contracts_market_signal_degradation_warning_py["(生产态 / production) 信号退化警告 /<br/>signal_degradation_warning<br/>信号退化警告，供signal; risk; pf_<br/>core使用<br/>文件: market/signal_degradation_<br/>warning.py"]
    src_zephyr_trading_trading_contracts_portfolio_contracts_money_py["(生产态 / production) 过渡兼容层<br/>（DEPRECATED）—— Money 契约 canonical<br/>真 / money<br/>过渡兼容层（DEPRECATED）—— Money 契约<br/>canonical 真源已收敛至 shared 侧。<br/>文件: contracts/money.py"]
    src_zephyr_trading_trading_contracts_portfolio_contracts_performance_attribution_report_py["(生产态 / production)<br/>绩效attribution报告 / performance_<br/>attribution_report<br/>Re-export shim — 真源已收敛至<br/>zephyr.shared.contracts.performance_<br/>attribution_report.<br/>文件: contracts/performance_attribution_<br/>report.py"]
    src_zephyr_trading_trading_contracts_portfolio_contracts_strategy_lifecycle_event_py["(生产态 / production) 策略生命周期事件<br/>/ strategy_lifecycle_event<br/>策略生命周期事件，组合的事件，定义和分发<br/>事件。<br/>文件: contracts/strategy_lifecycle_<br/>event.py"]
    src_zephyr_trading_trading_contracts_risk_compliance_rule_py["(生产态 / production) 合规规则 /<br/>compliance_rule<br/>合规规则，供l10-compliance使用<br/>文件: risk/compliance_rule.py"]
    src_zephyr_trading_trading_contracts_risk_risk_limit_violation_error_py["(生产态 / production) 风险限制违规错误<br/>/ risk_limit_violation_error<br/>风险限制违规错误，风控的异常，定义本模块<br/>的异常类型。<br/>文件: risk/risk_limit_violation_error.py"]
    src_zephyr_trading_trading_contracts_risk_risk_validator_protocol_py["(生产态 / production) 风险校验器协议 /<br/>risk_validator_protocol<br/>风险校验器协议，风控的校验器，检查输入是<br/>否符合规则。<br/>文件: risk/risk_validator_protocol.py"]
    src_zephyr_trading_trading_contracts_risk_trading_kill_switch_py["(生产态 / production) 交易终止开关 /<br/>trading_kill_switch<br/>交易终止开关，供MOD-INF-022 ;<br/>MOD-INF-020使用<br/>文件: risk/trading_kill_switch.py"]
    ...
    src_zephyr_trading_action_dispatcher_annotation_writer_py["(生产态 / production) annotation写入器<br/>/ _annotation_writer<br/>注释注解写入器（从 ActionDispatcher._<br/>annotate_py_file/_tag_module/_annotate_<br/>blueprint 提取）。<br/>文件: action_dispatcher/_annotation_<br/>writer.py"]
    src_zephyr_trading_action_dispatcher_audit_log_writer_py["(生产态 / production) 审计日志写入器 / _<br/>audit_log_writer<br/>审计日志写入器（从 ActionDispatcher._<br/>write_triage_log 提取）。<br/>文件: action_dispatcher/_audit_log_<br/>writer.py"]
    src_zephyr_trading_action_dispatcher_file_lifecycle_manager_py["(生产态 / production)<br/>文件生命周期管理器 / _file_lifecycle_<br/>manager<br/>文件生命周期管理器（从<br/>ActionDispatcher._create_file / _delete_<br/>file / _version_backup 提取）。<br/>文件: action_dispatcher/_file_lifecycle_<br/>manager.py"]
    src_zephyr_trading_action_dispatcher_search_replace_engine_py["(生产态 / production) searchreplace引擎<br/>/ _search_replace_engine<br/>搜索替换引擎（从 ActionDispatcher._<br/>search_replace_file<br/>及两个底层方法提取）。<br/>文件: action_dispatcher/_search_replace_<br/>engine.py"]
    src_zephyr_trading_autopilot_py["(生产态 / production) AutoPilot — AI<br/>session 自动找活干、认领任务。 /<br/>autopilot<br/>AutoPilot — AI session<br/>自动找活干、认领任务。<br/>文件: trading/autopilot.py"]
    src_zephyr_trading_trading_contracts_execution_order_py["(生产态 / production) 订单 / order<br/>Re-export wrapper: Order 真源在<br/>zephyr.shared.contracts.order（CTR-004<br/>codegen）<br/>文件: execution/order.py"]
    src_zephyr_trading_trading_contracts_risk_risk_dashboard_snapshot_py["(生产态 / production) 风险仪表盘快照 /<br/>risk_dashboard_snapshot<br/>风险仪表盘快照，供risk; ops使用<br/>文件: risk/risk_dashboard_snapshot.py"]
    src_zephyr_trading_trading_contracts_risk_risk_limits_py["(生产态 / production) 风险limits / risk_<br/>limits<br/>风险limits，供risk; pf_core使用<br/>文件: risk/risk_limits.py"]
    src_zephyr_trading_trading_contracts_risk_risk_metrics_py["(生产态 / production) 风险指标 / risk_<br/>metrics<br/>风险指标，风控的报告器，汇总数据生成报告<br/>。<br/>文件: risk/risk_metrics.py"]
    src_zephyr_trading_verdict_engine_py["(生产态 / production) 裁定引擎 /<br/>verdict_engine<br/>verdict引擎，交易的事件，定义和分发事件<br/>。<br/>文件: trading/verdict_engine.py"]
    ...
    src_zephyr_trading_protection_index_py["(生产态 / production) 保护索引 /<br/>protection_index<br/>保护索引，供zephyr.trading.verdict_<br/>engine;使用<br/>文件: trading/protection_index.py"]
    ...
    D_POSITION["(生产态 / production) 仓位管理 /<br/>Position Management<br/>仓位管理，负责持仓跟踪、仓位计算和盈亏分<br/>析<br/>跨域节点 / cross-domain"]
    ...
    D_DATA["(设计态 / design) 数据接入层 / Data<br/>Access Layer<br/>数据接入层，负责数据源接入、数据集成和数<br/>据标准化<br/>跨域节点 / cross-domain"]
    ...
    D_FUNDAMENTAL_SIGNAL["(生产态 / production) 基本面信号 /<br/>Fundamental Signal<br/>基本面信号，负责基于财务数据的基本面信号<br/>生成<br/>跨域节点 / cross-domain"]
