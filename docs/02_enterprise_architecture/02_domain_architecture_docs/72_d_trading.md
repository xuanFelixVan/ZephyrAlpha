...
flowchart TD
    ...
    src_zephyr_trading_conductor_py["Conductor — AI session 全自动指挥官。<br/>文件: trading/conductor.py<br/>(生产态 / production)"]
    ...
    src_zephyr_trading_settlement_reconciliation_py["结算对账<br/>结算对账（settlement_reconciliation.py）<br/>⛔ 交易域，设计已就绪，等待开发排期<br/>文件: trading/settlement_reconciliation.py<br/>(设计态 / design)"]
    src_zephyr_trading_speed_baseline_checker_py["测速基线检查器<br/>speed基线检查器，交易的检查器，检查条件是否满足<br/>speed_baseline_checker<br/>文件: trading/speed_baseline_checker.py<br/>(生产态 / production)"]
    src_zephyr_trading_trading_contracts_broker_interface_py["经纪人接口<br/>经纪人接口。D_EXECUTION_CORE — BrokerInterface<br/>文件: trading_contracts/broker_interface.py<br/>(生产态 / production)"]
    ...
    src_zephyr_trading_trading_contracts_execution_execution_report_py["执行报告<br/>重导出包装，真源:<br/>zephyr.shared.contracts.execution_report<br/>（CTR-P1-007 codegen）<br/>文件: execution/execution_report.py<br/>(生产态 / production)"]
    src_zephyr_trading_trading_contracts_execution_fill_py["成交<br/>重导出包装，真源: zephyr.shared.contracts.fill<br/>（CTR-005 codegen）<br/>文件: execution/fill.py<br/>(生产态 / production)"]
    ...
    src_zephyr_trading_trading_contracts_execution_position_py["持仓<br/>重导出包装，真源:<br/>zephyr.shared.contracts.position（CTR-006<br/>codegen）<br/>文件: execution/position.py<br/>(生产态 / production)"]
    ...
    src_zephyr_trading_trading_contracts_market_instrument_py["标的合约<br/>instrument<br/>文件: market/instrument.py<br/>(生产态 / production)"]
    ...
    src_zephyr_trading_trading_contracts_portfolio_contracts_money_py["过渡兼容层（DEPRECATED）—— Money 契约 canonical<br/>真<br/>文件: contracts/money.py<br/>(生产态 / production)"]
    src_zephyr_trading_trading_contracts_portfolio_contracts_performance_attribution_report_py["绩效attribution报告<br/>重导出垫片，真源:<br/>zephyr.shared.contracts.performance_<br/>attributio...<br/>performance_attribution_report<br/>文件: contracts/performance_attribution_<br/>report.py<br/>(生产态 / production)"]
    ...
    src_zephyr_trading_autopilot_py["AutoPilot — AI session 自动找活干、认领任务。<br/>文件: trading/autopilot.py<br/>(生产态 / production)"]
    src_zephyr_trading_trading_contracts_execution_order_py["订单<br/>重导出包装，真源:<br/>zephyr.shared.contracts.order（CTR-004 codegen）<br/>文件: execution/order.py<br/>(生产态 / production)"]
