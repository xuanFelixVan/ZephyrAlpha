...
flowchart TD
    ...
    src_zephyr_ex_core_auction_deviation_executor_py["(设计态 / design) 拍卖偏差执行器 / auction_deviation_executor<br/>拍卖偏差执行器，执行核心的执行器，执行具体操作。<br/>文件: ex_core/auction_deviation_executor.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    ...
    src_zephyr_ex_core_batch_take_profit_executor_py["(设计态 / design) 批次止盈利润执行器 / batch_take_profit_executor<br/>批次止盈利润执行器，执行核心的执行器，执行具体操作。<br/>文件: ex_core/batch_take_profit_executor.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    src_zephyr_ex_core_conditional_order_manager_py["(设计态 / design) conditional订单管理器 / conditional_order_manager<br/>conditional订单管理器，ex_core的管理器，统一管理一类资源的生命周期。<br/>文件: ex_core/conditional_order_manager.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    ...
    src_zephyr_ex_core_execution_mcp_server_py["(设计态 / design) 执行MCP服务端 / execution_mcp_server<br/>执行MCP服务端，ex_core的服务端，接收并处理请求。<br/>文件: ex_core/execution_mcp_server.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    src_zephyr_ex_core_fill_handler_py["(设计态 / design) 成交处理器 / fill_handler<br/>成交处理器，ex_core的处理器，处理特定类型的事件或请求。<br/>文件: ex_core/fill_handler.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    ...
    src_zephyr_ex_core_live_simulation_switcher_py["(设计态 / design) 实盘仿真切换器 / live_simulation_switcher<br/>实盘仿真切换器（live_simulation_switcher.py）<br/>文件: ex_core/live_simulation_switcher.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    ...
    src_zephyr_ex_core_sell_priority_scheduler_py["(设计态 / design) 卖出优先级调度器 / sell_priority_scheduler<br/>卖priority调度器，ex_core的调度器，按时间或优先级安排任务执行。<br/>文件: ex_core/sell_priority_scheduler.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    ...
    src_zephyr_ex_core_stop_loss_take_profit_executor_py["(设计态 / design) 停止亏损止盈利润执行器 / stop_loss_take_profit_executor<br/>停止亏损止盈利润执行器，执行核心的执行器，执行具体操作。<br/>文件: ex_core/stop_loss_take_profit_executor.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    src_zephyr_ex_core_trading_session_py["(设计态 / design) 交易会话 / trading_session<br/>D_EXECUTION_CORE — TradingSession 盘中实时调仓编排器<br/>文件: ex_core/trading_session.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
