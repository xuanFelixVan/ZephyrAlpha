...
flowchart TD
    ...
    src_zephyr_ex_core_adapters_risk_validation_bridge_py["风控验证桥接<br/>执行核心的Re-export wrapper:<br/>risk_validation_bridge 真源在<br/>zephyr.governance.adapters.risk_validation_bridg<br/>e，Re-export wrapper: risk_validation_bridge<br/>真源在 zephyr.governance.adapters.risk_validatio<br/>n_bridge相关功能的实现。<br/>文件: adapters/risk_validation_bridge.py<br/>(生产态 / production)"]
    ...
    src_zephyr_ex_core_live_simulation_switcher_py["实盘仿真切换器<br/>执行核心的实盘仿真切换器<br/>（live_simulation_switcher.py），实盘仿真切换器<br/>（live_simulation_switcher.py）相关功能的实现。<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期<br/>文件: ex_core/live_simulation_switcher.py<br/>(设计态 / design)"]
    src_zephyr_ex_core_order_splitter_py["订单拆分器<br/>执行核心的订单拆分器<br/>（order_splitter.py），订单拆分器<br/>（order_splitter.py）相关功能的实现。<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期<br/>文件: ex_core/order_splitter.py<br/>(设计态 / design)"]
    ...
    src_zephyr_ex_core_services_live_portfolio_py["实时组合<br/>执行核心的实时组合<br/>（live_portfolio.py），实时组合<br/>（live_portfolio.py）相关功能的实现。<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期<br/>文件: services/live_portfolio.py<br/>(设计态 / design)"]
    ...
    src_zephyr_ex_core_adapters_simulation_broker_py["模拟经纪人<br/>执行核心的Re-export wrapper: simulation_broker<br/>真源在 zephyr.governance.adapters.simulation_bro<br/>ker，Re-export wrapper: simulation_broker<br/>真源在 zephyr.governance.adapters.simulation_bro<br/>ker相关功能的实现。<br/>文件: adapters/simulation_broker.py<br/>(生产态 / production)"]
