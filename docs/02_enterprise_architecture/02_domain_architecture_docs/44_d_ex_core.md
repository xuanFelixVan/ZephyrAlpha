...
flowchart TD
    src_zephyr_ex_core_adapters_init_py["(生产态 / production) 包入口 / __init__<br/>D_EX_CORE adapters — 券商/风控适配器 re-export<br/>wrapper<br/>文件: adapters/__init__.py"]
    src_zephyr_ex_core_adapters_risk_validation_bridge_py["(生产态 / production) 风控验证桥接 / risk_<br/>validation_bridge<br/>Re-export wrapper: risk_validation_bridge<br/>真源在 zephyr.governance.adapters.risk_<br/>validation_bridge<br/>文件: adapters/risk_validation_bridge.py"]
    src_zephyr_ex_core_auction_deviation_executor_py["(设计态 / design) 拍卖偏差执行器 / auction_<br/>deviation_executor<br/>拍卖偏差执行器，执行核心的执行器，执行具体操作。<br/>文件: ex_core/auction_deviation_executor.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    src_zephyr_ex_core_batch_executor_py["(设计态 / design) 批次执行器 / batch_executor<br/>批次执行器，执行核心的执行器，执行具体操作。<br/>文件: ex_core/batch_executor.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    src_zephyr_ex_core_batch_take_profit_executor_py["(设计态 / design) 批次止盈利润执行器 / batch_<br/>take_profit_executor<br/>批次止盈利润执行器，执行核心的执行器，执行具体操<br/>作。<br/>文件: ex_core/batch_take_profit_executor.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    src_zephyr_ex_core_conditional_order_manager_py["(设计态 / design) conditional订单管理器 /<br/>conditional_order_manager<br/>conditional订单管理器，ex_<br/>core的管理器，统一管理一类资源的生命周期。<br/>文件: ex_core/conditional_order_manager.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    src_zephyr_ex_core_execution_engine_py["(生产态 / production) 执行引擎 / D_EXECUTION_<br/>CORE — Execution Engine<br/>执行引擎。D_EXECUTION_CORE — Execution Engine<br/>文件: ex_core/execution_engine.py"]
    src_zephyr_ex_core_execution_mcp_server_py["(设计态 / design) 执行MCP服务端 / execution_mcp_<br/>server<br/>执行MCP服务端，ex_core的服务端，接收并处理请求。<br/>文件: ex_core/execution_mcp_server.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    src_zephyr_ex_core_fill_handler_py["(设计态 / design) 成交处理器 / fill_handler<br/>成交处理器，ex_<br/>core的处理器，处理特定类型的事件或请求。<br/>文件: ex_core/fill_handler.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    src_zephyr_ex_core_fill_processor_py["(设计态 / design) 成交处理器 / fill_processor<br/>成交处理器，执行核心的处理器，处理加工数据。<br/>文件: ex_core/fill_processor.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    src_zephyr_ex_core_live_simulation_switcher_py["(设计态 / design) 实盘仿真切换器 / live_<br/>simulation_switcher<br/>实盘仿真切换器（live_simulation_switcher.py）<br/>文件: ex_core/live_simulation_switcher.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    src_zephyr_ex_core_order_splitter_py["(设计态 / design) 订单拆分器 / order_splitter<br/>订单拆分器（order_splitter.py）<br/>文件: ex_core/order_splitter.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    src_zephyr_ex_core_redis_idempotency["(设计态 / design) redis幂等性<br/>redis幂等性，执行核心的子目录，归集相关子模块。<br/>文件: redis_idempotency/<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    src_zephyr_ex_core_sell_priority_scheduler_py["(设计态 / design) 卖出优先级调度器 / sell_<br/>priority_scheduler<br/>卖priority调度器，ex_<br/>core的调度器，按时间或优先级安排任务执行。<br/>文件: ex_core/sell_priority_scheduler.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    src_zephyr_ex_core_services_live_portfolio_py["(设计态 / design) 实时组合 / live_portfolio<br/>实时组合（live_portfolio.py）<br/>文件: services/live_portfolio.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    src_zephyr_ex_core_signal_providers_py["(生产态 / production) 信号提供器 / signal_<br/>providers<br/>D_EXECUTION_CORE — 信号源 / 价格源 callable 工厂<br/>文件: ex_core/signal_providers.py"]
    src_zephyr_ex_core_stop_loss_take_profit_executor_py["(设计态 / design) 停止亏损止盈利润执行器 / stop_<br/>loss_take_profit_executor<br/>停止亏损止盈利润执行器，执行核心的执行器，执行具<br/>体操作。<br/>文件: ex_core/stop_loss_take_profit_executor.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    src_zephyr_ex_core_trading_session_py["(设计态 / design) 交易会话 / trading_session<br/>D_EXECUTION_CORE — TradingSession<br/>盘中实时调仓编排器<br/>文件: ex_core/trading_session.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    src_zephyr_governance_escalation_order_state_escalator_py["(生产态 / production) 订单状态escalator / order_<br/>state_escalator<br/>Order State Escalator — v0.10.0<br/>订单状态机升级器。<br/>文件: escalation/order_state_escalator.py"]
    ...
    src_zephyr_ex_core_adapters_miniqmt_broker_py["(生产态 / production) miniqmt券商 / miniqmt_<br/>broker<br/>MiniQMT 实盘券商适配器（对接<br/>xttrader，A股实盘交易）<br/>文件: adapters/miniqmt_broker.py"]
    src_zephyr_ex_core_adapters_simulation_broker_py["(生产态 / production) 模拟经纪人 / simulation_<br/>broker<br/>Re-export wrapper: simulation_broker 真源在<br/>zephyr.governance.adapters.simulation_broker<br/>文件: adapters/simulation_broker.py"]
    src_zephyr_ex_core_execution_report_py["(设计态 / design) 执行报告 / execution_report<br/>执行报告，ex_core的报告器，汇总数据生成报告。<br/>文件: ex_core/execution_report.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    src_zephyr_ex_core_order_manager_py["(生产态 / production) 订单管理器 / D_EXECUTION_<br/>CORE — Order Manager<br/>订单管理器。D_EXECUTION_CORE — Order Manager<br/>文件: ex_core/order_manager.py"]
    src_zephyr_ex_core_position_tracker["(设计态 / design) 持仓追踪器<br/>持仓追踪器，持仓的子目录，归集相关子模块。<br/>文件: position_tracker/<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
    ...
    src_zephyr_ex_core_audit_journal["(设计态 / design) 审计日志<br/>审计日志，审计日志的子目录，归集相关子模块。<br/>文件: audit_journal/<br/>⛔ 交易执行核心域，设计已就绪，等待开发排期"]
