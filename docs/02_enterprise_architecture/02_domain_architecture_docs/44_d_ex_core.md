...
flowchart TD
    src_zephyr_ex_core_adapters_init_py["(生产态 / production) 包入口 / __init__<br/>D_EX_CORE adapters — 券商/风控适配器<br/>re-export wrapper<br/>文件: adapters/__init__.py"]
    src_zephyr_ex_core_adapters_risk_validation_bridge_py["(生产态 / production) 风控验证桥接 /<br/>risk_validation_bridge<br/>Re-export wrapper: risk_validation_<br/>bridge 真源在<br/>zephyr.governance.adapters.risk_<br/>validation_bridge<br/>文件: adapters/risk_validation_bridge.py"]
    src_zephyr_ex_core_auction_deviation_executor_py["(设计态 / design) 拍卖偏差执行器 /<br/>auction_deviation_executor<br/>拍卖偏差执行器，执行核心的执行器，执行具<br/>体操作。<br/>文件: ex_core/auction_deviation_<br/>executor.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排<br/>期"]
    src_zephyr_ex_core_batch_executor_py["(设计态 / design) 批次执行器 / batch_<br/>executor<br/>批次执行器，执行核心的执行器，执行具体操<br/>作。<br/>文件: ex_core/batch_executor.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排<br/>期"]
    src_zephyr_ex_core_batch_take_profit_executor_py["(设计态 / design) 批次止盈利润执行器 /<br/>batch_take_profit_executor<br/>批次止盈利润执行器，执行核心的执行器，执<br/>行具体操作。<br/>文件: ex_core/batch_take_profit_<br/>executor.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排<br/>期"]
    src_zephyr_ex_core_conditional_order_manager_py["(设计态 / design) conditional订单管理器<br/>/ conditional_order_manager<br/>conditional订单管理器，ex_<br/>core的管理器，统一管理一类资源的生命周期<br/>。<br/>文件: ex_core/conditional_order_<br/>manager.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排<br/>期"]
    src_zephyr_ex_core_execution_engine_py["(生产态 / production) 执行引擎 / D_<br/>EXECUTION_CORE — Execution Engine<br/>执行引擎。D_EXECUTION_CORE — Execution<br/>Engine<br/>文件: ex_core/execution_engine.py"]
    src_zephyr_ex_core_execution_mcp_server_py["(设计态 / design) 执行MCP服务端 /<br/>execution_mcp_server<br/>执行MCP服务端，ex_<br/>core的服务端，接收并处理请求。<br/>文件: ex_core/execution_mcp_server.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排<br/>期"]
    src_zephyr_ex_core_fill_handler_py["(设计态 / design) 成交处理器 / fill_<br/>handler<br/>成交处理器，ex_<br/>core的处理器，处理特定类型的事件或请求。<br/>文件: ex_core/fill_handler.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排<br/>期"]
    src_zephyr_ex_core_fill_processor_py["(设计态 / design) 成交处理器 / fill_<br/>processor<br/>成交处理器，执行核心的处理器，处理加工数<br/>据。<br/>文件: ex_core/fill_processor.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排<br/>期"]
    src_zephyr_ex_core_live_simulation_switcher_py["(设计态 / design) 实盘仿真切换器 / live_<br/>simulation_switcher<br/>实盘仿真切换器（live_simulation_<br/>switcher.py）<br/>文件: ex_core/live_simulation_<br/>switcher.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排<br/>期"]
    src_zephyr_ex_core_order_splitter_py["(设计态 / design) 订单拆分器 / order_<br/>splitter<br/>订单拆分器（order_splitter.py）<br/>文件: ex_core/order_splitter.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排<br/>期"]
    src_zephyr_ex_core_redis_idempotency["(设计态 / design) redis幂等性<br/>redis幂等性，执行核心的子目录，归集相关<br/>子模块。<br/>文件: redis_idempotency/<br/>⛔ 交易执行核心域，设计已就绪，等待开发排<br/>期"]
    src_zephyr_ex_core_sell_priority_scheduler_py["(设计态 / design) 卖出优先级调度器 /<br/>sell_priority_scheduler<br/>卖priority调度器，ex_<br/>core的调度器，按时间或优先级安排任务执行<br/>。<br/>文件: ex_core/sell_priority_scheduler.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排<br/>期"]
    src_zephyr_ex_core_services_live_portfolio_py["(设计态 / design) 实时组合 / live_<br/>portfolio<br/>实时组合（live_portfolio.py）<br/>文件: services/live_portfolio.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排<br/>期"]
    src_zephyr_ex_core_signal_providers_py["(生产态 / production) 信号提供器 /<br/>signal_providers<br/>D_EXECUTION_CORE — 信号源 / 价格源<br/>callable 工厂<br/>文件: ex_core/signal_providers.py"]
    src_zephyr_ex_core_stop_loss_take_profit_executor_py["(设计态 / design)<br/>停止亏损止盈利润执行器 / stop_loss_take_<br/>profit_executor<br/>停止亏损止盈利润执行器，执行核心的执行器<br/>，执行具体操作。<br/>文件: ex_core/stop_loss_take_profit_<br/>executor.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排<br/>期"]
    src_zephyr_ex_core_trading_session_py["(设计态 / design) 交易会话 / trading_<br/>session<br/>D_EXECUTION_CORE — TradingSession<br/>盘中实时调仓编排器<br/>文件: ex_core/trading_session.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排<br/>期"]
    src_zephyr_governance_escalation_order_state_escalator_py["(生产态 / production) 订单状态escalator<br/>/ order_state_escalator<br/>Order State Escalator — v0.10.0<br/>订单状态机升级器。<br/>文件: escalation/order_state_<br/>escalator.py"]
    ...
    src_zephyr_ex_core_adapters_miniqmt_broker_py["(生产态 / production) miniqmt券商 /<br/>miniqmt_broker<br/>MiniQMT 实盘券商适配器（对接<br/>xttrader，A股实盘交易）<br/>文件: adapters/miniqmt_broker.py"]
    src_zephyr_ex_core_adapters_simulation_broker_py["(生产态 / production) 模拟经纪人 /<br/>simulation_broker<br/>Re-export wrapper: simulation_broker<br/>真源在 zephyr.governance.adapters.simula<br/>tion_broker<br/>文件: adapters/simulation_broker.py"]
    src_zephyr_ex_core_execution_report_py["(设计态 / design) 执行报告 / execution_<br/>report<br/>执行报告，ex_<br/>core的报告器，汇总数据生成报告。<br/>文件: ex_core/execution_report.py<br/>⛔ 交易执行核心域，设计已就绪，等待开发排<br/>期"]
    src_zephyr_ex_core_order_manager_py["(生产态 / production) 订单管理器 / D_<br/>EXECUTION_CORE — Order Manager<br/>订单管理器。D_EXECUTION_CORE — Order<br/>Manager<br/>文件: ex_core/order_manager.py"]
    src_zephyr_ex_core_position_tracker["(设计态 / design) 持仓追踪器<br/>持仓追踪器，持仓的子目录，归集相关子模块<br/>。<br/>文件: position_tracker/<br/>⛔ 交易执行核心域，设计已就绪，等待开发排<br/>期"]
    ...
    src_zephyr_ex_core_audit_journal["(设计态 / design) 审计日志<br/>审计日志，审计日志的子目录，归集相关子模<br/>块。<br/>文件: audit_journal/<br/>⛔ 交易执行核心域，设计已就绪，等待开发排<br/>期"]
    ...
    D_REPORTING["(设计态 / design) 报告 / Reporting<br/>报告，负责投资报告、风险报告和合规报告的<br/>生成与分发<br/>跨域节点 / cross-domain"]
    ...
    D_EX_SOR["(设计态 / design) 执行路由 / Execution<br/>Routing<br/>执行路由，负责订单路由、智能拆单和执行场<br/>所选择<br/>跨域节点 / cross-domain"]
