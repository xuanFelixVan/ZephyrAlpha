...
graph TD
    ...
    src_zephyr_factor_bus_factor_defense_py -.->|contract / contract| D_GOVERNANCE
    ...
    D_GOV_DRIFT -.->|runtime / runtime| src_zephyr_factor_bus_factor_defense_py
    D_INFRA_TELEMETRY["(生产态 / production) D_INFRA_TELEMETRY"]
    D_INFRA_TELEMETRY -->|runtime / runtime| src_zephyr_factor_bus_factor_defense_py
    D_GOVERNANCE -.->|runtime / runtime| src_zephyr_factor_bus_factor_defense_py
    D_GOVERNANCE -.->|contract / contract| src_zephyr_factor_bus_factor_defense_py
    D_AUTONOMY_CORE["(原型态 / prototype) D_AUTONOMY_CORE"]
    D_AUTONOMY_CORE -.->|runtime / runtime| src_zephyr_factor_bus_factor_defense_py
    ...
    class D_FUNDAMENTAL_SIGNAL,D_INFRA_TELEMETRY external_prod
    class D_GOVERNANCE,D_INFRA_RUNTIME,D_GOV_DRIFT,D_AUTONOMY_CORE external_design
```
...
graph TD
    ...
    classDef external_design fill:#fce4ec,stroke:#880e4f,stroke-width:1px,color:#000,stroke-dasharray: 5 5
    class src_zephyr_factor_init_py,src_zephyr_factor_base_py,src_zephyr_factor_bus_factor_defense_py,src_zephyr_factor_factor_base_py production
    class D_INFRA_TELEMETRY,D_FUNDAMENTAL_SIGNAL external_prod
...
| 2 | alpha_signal_pipeline.py | → | D_GOVERNANCE 生命周期管理: blueprint.md | runtime / runtime |
| 3 | bus_factor_defense.py | → | D_GOVERNANCE 生命周期管理: blueprint.md | contract / contract |
| 4 | alpha_signal_pipeline.py | → | D_INFRA_RUNTIME 运行时集成: Batch2 治理层契约 — 15条 Pydantic v2 Schema（P... | runtime / runtime |

### 依赖本域的其他域（入边）/ Depended By

| # | 外部域-源模块 / Source Module | → | 本域模块 / Target Module | 依赖类型 / Type |
|:--:|---------|:--:|---------|---------|
| 1 | D_AUTONOMY_CORE 自治核心: file_autoregister.py | → | bus_factor_defense.py | runtime / runtime |
| 2 | D_FUNDAMENTAL_SIGNAL 基本面信号: D_SIGNAL Signal Domain (__init__.py) | → | D_FACTOR — Momentum Factor (momentum_factor.py) | contract / contract |
| 3 | D_FUNDAMENTAL_SIGNAL 基本面信号: AlphaSignalPipeline D_FACTOR->D_SIGNAL跨层集成... | → | ZephyrAlpha — D_FACTOR Alpha Factor Layer (fac... | 导入依赖 / import_depends |
| 4 | D_GOVERNANCE 生命周期管理: blueprint.md | → | bus_factor_defense.py | runtime / runtime |
| 5 | D_GOVERNANCE 生命周期管理: blueprint.md | → | bus_factor_defense.py | contract / contract |
| 6 | D_GOVERNANCE 生命周期管理: post_sync_validator — post_sync_standard 命令.... | → | alpha_signal_pipeline.py | runtime / runtime |
| 7 | D_GOVERNANCE 生命周期管理: ZephyrAlpha — governance.base re-export shim. ... | → | ZephyrAlpha — D_FACTOR Alpha Factor Layer (fac... | 导入依赖 / import_depends |
| 8 | D_GOV_DRIFT 漂移检测: blueprint.md | → | bus_factor_defense.py | runtime / runtime |
| 9 | D_INFRA_TELEMETRY 可观测性: health subsystem — 模块健康注册与 LifecycleMan... | → | bus_factor_defense.py | runtime / runtime |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 6 个外部域直接连接（出边 4 条 + 入边 9 条 = 13 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_AUTONOMY_CORE -->|1条 runtime / runtime| D_FACTOR
    D_GOV_DRIFT -->|1条 runtime / runtime| D_FACTOR
    D_INFRA_TELEMETRY -->|1条 runtime / runtime| D_FACTOR
```
