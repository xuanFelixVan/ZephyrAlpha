...
flowchart TD
    src_zephyr_feedback_loop_gates_governance_gates_py["治理门禁<br/>门禁的门禁，在关键节点检查是否放行<br/>_governance_gates<br/>文件: gates/_governance_gates.py<br/>(生产态 / production)"]
    ...
    src_zephyr_feedback_loop_gates_safety_gates_py["安全门禁<br/>门禁的门禁，在关键节点检查是否放行<br/>_safety_gates<br/>文件: gates/_safety_gates.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_gates_security_gates_py["安全门禁<br/>门禁的门禁，在关键节点检查是否放行<br/>_security_gates<br/>文件: gates/_security_gates.py<br/>(生产态 / production)"]
    ...
    src_zephyr_feedback_loop_gates_autonomy_credit_py["自治信用<br/>管理自治信用积分，支持衰减机制防止信任固化<br/>Autonomy Credit System — v0.7.0 R87<br/>文件: gates/autonomy_credit.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_gates_autonomy_maturity_py["自治成熟度<br/>评估自治成熟度等级，按等级分级授权自治权限<br/>Autonomy Maturity Ladder — v0.7.0 R86<br/>文件: gates/autonomy_maturity.py<br/>(生产态 / production)"]
    ...
    src_zephyr_feedback_loop_gates_deployment_suppression_py["部署抑制<br/>门禁的门禁，在关键节点检查是否放行<br/>Deployment Suppression — v0.37.0 R464<br/>文件: gates/deployment_suppression.py<br/>(生产态 / production)"]
    ...
    src_zephyr_feedback_loop_verifiers_digital_twin_sandbox_py["数字孪生沙箱<br/>反馈闭环的数据库，持久化存取结构化数据<br/>Digital Twin Sandbox — v0.6.0 R55<br/>文件: verifiers/digital_twin_sandbox.py<br/>(生产态 / production)"]
    ...
    src_zephyr_feedback_loop_verifiers_toctou_revalidation_py["TOCTOU重新验证<br/>反馈闭环的结果，封装操作结果的数据结构<br/>TOCTOU Revalidation — v0.37.0 R458<br/>文件: verifiers/toctou_revalidation.py<br/>(生产态 / production)"]
