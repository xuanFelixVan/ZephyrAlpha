...
| 容量 | 156/150 (超容) | Capacity | 156/150 (超容) |
...
> 展示全部 158 个模块（生产态 156 + 设计态 2），含跨域依赖外部节点。节点含成熟度+名称+大白话/简介+文件路径。
...
> 仅展示已上线运行的模块（共 156 个），不含跨域外部节点。跨域依赖见下方跨域依赖章节。
