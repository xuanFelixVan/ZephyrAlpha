...
flowchart TD
    ...
    src_zephyr_infrastructure_pipeline_pipeline_agent_bridge_py["管线代理桥接<br/>真源：MOD-INF-009 第四轮审计+<br/>pipeline_agent_bridge<br/>文件: pipeline/pipeline_agent_bridge.py<br/>(生产态 / production)"]
    src_zephyr_infrastructure_pipeline_pipeline_lock_py["管线锁<br/>真源：MOD-INF-009 蓝图 §施工完成标准 第 2 项<br/>pipeline_lock<br/>文件: pipeline/pipeline_lock.py<br/>(生产态 / production)"]
    ...
    src_zephyr_infrastructure_system_telemetry_metrics_blueprint_metrics_py["蓝图指标<br/>关联蓝图 : MOD-INF-015 §4（metrics 子系统, SLI<br/>新增）<br/>blueprint_metrics<br/>文件: metrics/blueprint_metrics.py<br/>(生产态 / production)"]
    src_zephyr_trading_main_py["主入口<br/>noqa: m02-manual  M02豁免:<br/>交易系统主入口常驻服务(python -m<br/>zephyr.trading),CLI触发启动,启动后自动运行reconc<br/>ile循环;非reconciler无需事件触发<br/>__main__<br/>文件: trading/__main__.py<br/>(生产态 / production)"]
...
| 4 | tick → Redis tick:{symbol}:latest 双写器（D-DATA → H1 ... | → | D_FACTOR 因子: core/intraday_factor_loop.py | import / import |
