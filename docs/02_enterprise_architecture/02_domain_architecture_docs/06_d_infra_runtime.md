...
flowchart TD
    ...
    src_zephyr_infrastructure_auto_fix_engine_all_completer_py["all补全器<br/>提供解析all、extractpublicsymbols、scan等方法<br/>all_completer<br/>文件: auto_fix_engine/all_completer.py<br/>(生产态 / production)"]
    src_zephyr_infrastructure_auto_fix_engine_config_fixer_py["配置修复器<br/>配置修复器，提供fixtrailingwhitespace、fixtabs、<br/>fixmergeconflicts等方法<br/>config_fixer<br/>文件: auto_fix_engine/config_fixer.py<br/>(生产态 / production)"]
    src_zephyr_infrastructure_auto_fix_engine_dedup_extractor_py["去重提取器<br/>去重提取器，提供normalizecode、minoccurrences、m<br/>inoccurrences等方法<br/>dedup_extractor<br/>文件: auto_fix_engine/dedup_extractor.py<br/>(生产态 / production)"]
    src_zephyr_infrastructure_auto_fix_engine_dep_version_fixer_py["dep版本修复器<br/>主要提供ishigher、扫描、修复等功能<br/>dep_version_fixer<br/>文件: auto_fix_engine/dep_version_fixer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_infrastructure_auto_fix_engine_fix_diff_py["修复差异<br/>修复差异，提供计算、计算text、reverse等方法<br/>fix_diff<br/>文件: auto_fix_engine/fix_diff.py<br/>(生产态 / production)"]
    ...
    src_zephyr_infrastructure_auto_fix_engine_import_fixer_py["导入修复器<br/>导入修复器，主要提供try修复模块、扫描、修复等功<br/>能<br/>import_fixer<br/>文件: auto_fix_engine/import_fixer.py<br/>(生产态 / production)"]
    src_zephyr_infrastructure_auto_fix_engine_interrupt_guard_py["中断守卫<br/>中断守卫，提供waldir、waldir、dbpath等方法<br/>interrupt_guard<br/>文件: auto_fix_engine/interrupt_guard.py<br/>(生产态 / production)"]
    src_zephyr_infrastructure_auto_fix_engine_llm_fix_adapter_py["llm修复适配器<br/>提供secretguard、secretguard、llmbridge等方法<br/>llm_fix_adapter<br/>文件: auto_fix_engine/llm_fix_adapter.py<br/>(生产态 / production)"]
    ...
    src_zephyr_infrastructure_auto_fix_engine_self_heal_agent_py["selfheal代理<br/>自heal代理，主要提供最大rounds、熔断阈值、consec<br/>utivefailures等功能<br/>self_heal_agent<br/>文件: auto_fix_engine/self_heal_agent.py<br/>(生产态 / production)"]
    ...
    src_zephyr_infrastructure_capacity_assurance_sli_instrumentation_py["SLI instrumentation — SLI采集插桩点（对标蓝图<br/>§13<br/>基础设施/capacity assurance包的sli_<br/>instrumentation模块<br/>文件: capacity_assurance/sli_instrumentation.py<br/>(生产态 / production)"]
    ...
    src_zephyr_infrastructure_h1_redis_hot_init_py["infrastructure/h1_redis_hot 包入口<br/>管理infrastructure.h1_redis_<br/>hot子包的加载和懒导入<br/>文件: h1_redis_hot/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_infrastructure_kill_switch_sim_py["终止开关仿真<br/>基础设施包的kill_switch_sim模块<br/>Kill Switch T0 Hardware Simulator<br/>文件: infrastructure/kill_switch_sim.py<br/>(生产态 / production)"]
    ...
    src_zephyr_infrastructure_pipeline_circuit_breaker_manager_py["熔断断路器管理器<br/>基础设施/pipeline包的circuit_breaker_manager模块<br/>CircuitBreakerManager -- standalone circuit<br/>breaker manager (Netflix Hystrix equivalent).<br/>文件: pipeline/circuit_breaker_manager.py<br/>(生产态 / production)"]
    ...
    src_zephyr_trading_ports_py["端口<br/>端口，交易的分发器，把任务/事件分发给处理方<br/>Protocol-based interface layer for<br/>runtime->pipeline dependency abstraction.<br/>文件: trading/ports.py<br/>(生产态 / production)"]
    src_zephyr_trading_staging_area_py["StagingArea —<br/>多AI并发草稿写入+提交+冲突检测模块（CT-SES<br/>交易包的staging_area模块<br/>文件: trading/staging_area.py<br/>(生产态 / production)"]
    ...
    src_zephyr_infrastructure_asset_inventory_registry_adapter_py["MOD-INF-026 §17 — 24 个异构注册表统一解析适配器<br/>基础设施/asset inventory包的registry_adapter模块<br/>文件: asset_inventory/registry_adapter.py<br/>(生产态 / production)"]
    ...
    src_zephyr_infrastructure_system_telemetry_budget_telemetry_bridge_py["预算遥测桥接<br/>基础设施/system telemetry包的budget_telemetry_<br/>bridge模块<br/>_budget_telemetry_bridge<br/>文件: system_telemetry/_budget_telemetry_<br/>bridge.py<br/>(生产态 / production)"]
    ...
    src_zephyr_infrastructure_auto_fix_engine_batch_fixer_py["批次修复器<br/>批次修复器，主要提供conflict解析器、conflict解析<br/>器、idempotency等功能<br/>batch_fixer<br/>文件: auto_fix_engine/batch_fixer.py<br/>(生产态 / production)"]
    src_zephyr_infrastructure_auto_fix_engine_compliance_auditor_py["合规审计器<br/>合规审计器，提供retentiondays、retentiondays、au<br/>ditfix等方法<br/>compliance_auditor<br/>文件: auto_fix_engine/compliance_auditor.py<br/>(生产态 / production)"]
    src_zephyr_infrastructure_auto_fix_engine_escalation_bridge_py["升级桥接<br/>升级桥接，提供escalate、escalatedeadletter、获取<br/>escalationhistory等方法<br/>escalation_bridge<br/>文件: auto_fix_engine/escalation_bridge.py<br/>(生产态 / production)"]
    src_zephyr_infrastructure_auto_fix_engine_fix_health_check_py["修复健康检查<br/>提供检查config、dbpath、dbpath等方法<br/>fix_health_check<br/>文件: auto_fix_engine/fix_health_check.py<br/>(生产态 / production)"]
    src_zephyr_infrastructure_auto_fix_engine_fix_pattern_miner_py["修复patternminer<br/>提供dbpath、dbpath、patterncache等方法<br/>fix_pattern_miner<br/>文件: auto_fix_engine/fix_pattern_miner.py<br/>(生产态 / production)"]
    src_zephyr_infrastructure_auto_fix_engine_fix_report_py["修复报告<br/>修复报告，提供history、history、生成等方法<br/>fix_report<br/>文件: auto_fix_engine/fix_report.py<br/>(生产态 / production)"]
    src_zephyr_infrastructure_auto_fix_engine_fix_safety_py["修复安全<br/>修复安全，提供enabled、enabled、检查等方法<br/>fix_safety<br/>文件: auto_fix_engine/fix_safety.py<br/>(生产态 / production)"]
    src_zephyr_infrastructure_auto_fix_engine_shadow_workspace_py["影子工作区<br/>影子工作区，主要提供运行type检查、运行测试、运行<br/>ruff等功能<br/>shadow_workspace<br/>文件: auto_fix_engine/shadow_workspace.py<br/>(生产态 / production)"]
    ...
    src_zephyr_infrastructure_auto_fix_engine_fix_budget_py["修复预算<br/>修复预算，提供dailylimit、monthlylimit、llmtoken<br/>limit等方法<br/>fix_budget<br/>文件: auto_fix_engine/fix_budget.py<br/>(生产态 / production)"]
    src_zephyr_infrastructure_auto_fix_engine_fix_reliability_py["修复可靠性<br/>修复可靠性，提供ttl、ttl、检查等方法<br/>fix_reliability<br/>文件: auto_fix_engine/fix_reliability.py<br/>(生产态 / production)"]
    ...
    src_zephyr_infrastructure_system_telemetry_watchdog_py["三冗余 Watchdog（CT-WATCHDOG-001）——互检+Panic<br/>基础设施/system telemetry包的watchdog模块<br/>文件: system_telemetry/watchdog.py<br/>(生产态 / production)"]
    ...
    src_zephyr_trading_capability_sync_py["能力同步<br/>能力同步，主要提供注册表、注册表、同步A2A等功能<br/>capability_sync<br/>文件: trading/capability_sync.py<br/>(生产态 / production)"]
    ...
    class D_SHARED,D_SECURITY,D_FEEDBACK_LOOP,D_GOV_RULE,D_INTELLIGENCE,D_AUTONOMY_CORE,D_GOV_SCRIPTS,D_BACKTEST,D_INTEGRATION,D_TRADING external_prod
```
