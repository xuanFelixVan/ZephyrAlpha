...
flowchart TD
    ...
    class D_DATA,D_SHARED,D_GOV_RULE,D_SECURITY,D_AUTONOMY_CORE,D_BACKTEST,D_GOV_SCRIPTS,D_INTELLIGENCE,D_FEEDBACK_LOOP,D_TRADING,D_INTEGRATION external_prod
...
| 4 | TickRedisCache 单元测试——tick→Redis tick:{symbol}:late... | → | D_DATA 数据接入层: 逐笔订阅器 / tick_subscriber (data/tick_subscriber.py) | 测试依赖 / test_depends |
