...
flowchart TD
    ...
    scripts_setup_dev_env_py["开发环境一次性初始化（裁定<br/>#ARCH-PYTHON-SITECUSTOMIZE<br/>scripts包的setup_dev_env模块<br/>文件: scripts/setup_dev_env.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_ops_governance_budget_profile_manager_py["预算档案管理器<br/>治理包的budget_profile_manager模块<br/>文件: ops_governance/budget_profile_manager.py<br/>(生产态 / production)"]
