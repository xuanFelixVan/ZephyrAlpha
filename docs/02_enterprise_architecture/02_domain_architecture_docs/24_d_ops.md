...
flowchart TD
    scripts_governance_observability_gate_cache_py["门禁缓存<br/>gate_cache模块<br/>Module docstring — see module-level docstring<br/>for details.<br/>文件: observability/gate_cache.py<br/>(生产态 / production)"]
    scripts_setup_dev_env_py["开发环境一次性初始化（裁定<br/>#ARCH-PYTHON-SITECUSTOMIZE<br/>setup_dev_env模块<br/>文件: scripts/setup_dev_env.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_ops_governance_budget_profile_manager_py["预算档案管理器<br/>治理/ops governance包的budget_profile_<br/>manager模块<br/>文件: ops_governance/budget_profile_manager.py<br/>(生产态 / production)"]
