...
flowchart TD
    scripts_governance_observability_gate_cache_py["(生产态 / production) 门禁缓存 / Module<br/>docstring — see module-level docstring for<br/>details.<br/>门禁缓存。Module docstring — see module-level<br/>docstring for details.<br/>文件: observability/gate_cache.py"]
    scripts_setup_dev_env_py["(生产态 / production) 开发环境一次性初始化<br/>（裁定 #ARCH-PYTHON-SITECUSTOMIZE / setup_dev_<br/>env<br/>开发环境一次性初始化（裁定<br/>#ARCH-PYTHON-SITECUSTOMIZE）。<br/>文件: scripts/setup_dev_env.py"]
    src_zephyr_governance_observability_governance_observability_dashboard_py["(生产态 / production) 可观测性仪表盘 /<br/>observability_dashboard<br/>observability仪表盘，治理的核心类，封装Dashboard<br/>Panel相关逻辑。<br/>文件: observability_governance/observability_<br/>dashboard.py"]
    src_zephyr_governance_ops_governance_budget_engine_py["(生产态 / production) 预算引擎 / Budget<br/>Enforcer core engine — MOD-INF-024<br/>预算引擎。Budget Enforcer core engine —<br/>MOD-INF-024<br/>文件: ops_governance/budget_engine.py"]
    src_zephyr_governance_ops_governance_budget_handler_py["(生产态 / production) 预算处理器 / budget_<br/>handler<br/>G-CT-006 消费端 — Escalation.on_budget_alert()<br/>预算告急升级处理.<br/>文件: ops_governance/budget_handler.py"]
    src_zephyr_governance_ops_governance_budget_profile_manager_py["(生产态 / production) 预算档案管理器 / budget_<br/>profile_manager<br/>预算档案管理器，提供包入口和模块加载功能<br/>文件: ops_governance/budget_profile_manager.py"]
    src_zephyr_governance_ops_governance_budget_tracker_py["(生产态 / production) 预算追踪器 / budget_<br/>tracker<br/>预算追踪器，治理的追踪器，持续跟踪某项指标或状态<br/>的变化。<br/>文件: ops_governance/budget_tracker.py"]
    src_zephyr_governance_ops_governance_cost_budget_py["(生产态 / production) 成本预算 / cost_budget<br/>— AI 成本预算与强制熔断（Phase 11 / 盲点 B26）<br/>文件: ops_governance/cost_budget.py"]
    src_zephyr_governance_ops_governance_meta_observability_py["(生产态 / production) 元可观测性 / meta_<br/>observability<br/>Meta Observability — v0.10.0 协议自身可观测性:<br/>self loop latency+p99+edge case rate。<br/>文件: ops_governance/meta_observability.py"]
    src_zephyr_governance_ops_governance_token_budget_py["(生产态 / production) 令牌预算 / token_budget<br/>三级 Token Budget 管理器。<br/>文件: ops_governance/token_budget.py"]
    ...
    src_zephyr_governance_ops_governance_budget_models_py["(生产态 / production) 预算模型 / Budget<br/>Enforcer data models — MOD-INF-024<br/>预算模型。Budget Enforcer data models —<br/>MOD-INF-024<br/>文件: ops_governance/budget_models.py"]
