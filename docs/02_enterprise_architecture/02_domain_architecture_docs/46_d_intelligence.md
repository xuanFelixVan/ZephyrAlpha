...
graph TD
    ...
    class D_SHARED,D_GOV_RULE,D_INTEGRATION,D_ML_TRAIN,D_GOVERNANCE,D_GOV_OPS_RESILIENCE,D_INFRA_RUNTIME,D_GOV_SCRIPTS external_prod
```
...
> 本域与 10 个外部域直接连接（出边 28 条 + 入边 20 条 = 48 条）。只显示直接连接的域，不展开具体节点。
