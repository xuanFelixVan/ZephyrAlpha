...
graph TD
    ...
    class D_COMPLIANCE,D_AUTONOMY_PERM,D_INFRA_RECOVERY external_design
```
...
graph TD
    ...
    class D_AUTONOMY_PERM,D_INTELLIGENCE,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    D_GOV_DRIFT -.->|config_depends / config_depends| src_zephyr_gov_drift_alert_router_py
    ...
    class D_SHARED,D_FBL_VERIFICATION,D_GOV_DRIFT,D_FEEDBACK_LOOP,D_INFRA_RUNTIME,D_GOV_AUDIT,D_GOVERNANCE,D_GOV_ENFORCEMENT external_prod
...
| 163 | D_GOV_DRIFT 漂移检测: __init__.py | → | Alert Router — alert_router.py (alert_router.py) | config_depends / config_depends |
...
| 185 | D_INFRA_A2A A2A通信: test_a2a_check.py | → | A2A 通信对验证——校验两个 agent 之间是否允许通... | 测试依赖 / test_depends |
...
> 本域与 22 个外部域直接连接（出边 142 条 + 入边 248 条 = 390 条）。只显示直接连接的域，不展开具体节点。
