...
graph TD
    ...
    class D_GOVERNANCE,D_GOV_AUDIT,D_COMPLIANCE,D_AUTONOMY_PERM,D_INFRA_RECOVERY external_design
```
...
graph TD
    ...
    class D_AUTONOMY_PERM,D_INFRASTRUCTURE,D_GOV_AUDIT,D_INTELLIGENCE,D_GOVERNANCE,D_AUTONOMY_CORE external_design
```
...
graph TD
    ...
    class D_SHARED,D_FBL_VERIFICATION,D_GOV_KB external_prod
```
...
graph TD
    ...
    class D_SHARED,D_GOV_AUDIT,D_AUTONOMY_PERM,D_INFRASTRUCTURE,D_SECURITY_LLM,D_GOV_SCRIPTS,D_GOVERNANCE external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOV_DRIFT,D_FBL_VERIFICATION,D_INFRA_RUNTIME,D_GOV_AUDIT,D_GOV_ENFORCEMENT external_prod
...
> 本域与 22 个外部域直接连接（出边 140 条 + 入边 248 条 = 388 条）。只显示直接连接的域，不展开具体节点。
