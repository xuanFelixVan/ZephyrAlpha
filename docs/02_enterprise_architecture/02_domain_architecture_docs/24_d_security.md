...
graph TD
    ...
    class D_INFRA_RUNTIME,D_GOV_SCRIPTS,D_GOVERNANCE,D_GOV_ENFORCEMENT external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOV_RULE,D_INTEGRATION,D_GOV_AUDIT,D_GOVERNANCE,D_GOV_OPS_RESILIENCE,D_INFRA_RUNTIME,D_AUTONOMY_CORE,D_ORCHESTRATOR,D_GOV_ENFORCEMENT,D_FEEDBACK_LOOP external_prod
```
...
graph TD
    ...
    class D_SHARED,D_GOV_DRIFT,D_GOVERNANCE,D_AUTONOMY_CORE,D_GOV_AUDIT,D_INFRA_RUNTIME,D_INTEGRATION,D_GOV_OPS_RESILIENCE external_prod
...
> 本域与 16 个外部域直接连接（出边 95 条 + 入边 39 条 = 134 条）。只显示直接连接的域，不展开具体节点。
