...
graph TD
    ...
    class D_AUTONOMY_PERM,D_COMPLIANCE,D_INFRA_A2A,D_GOV_AUDIT,D_INFRA_RECOVERY,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    D_INFRA_RUNTIME -.->|测试依赖 / test_depends| src_zephyr_security_access_control_cold_start_lock_py
    D_AUTONOMY_PERM -.->|测试依赖 / test_depends| src_zephyr_security_access_control_guards_input_guard_py
...
graph TD
    ...
    D_AUTONOMY_PERM -.->|测试依赖 / test_depends| src_zephyr_security_access_control_guards_output_guard_py
    D_AUTONOMY_PERM -.->|测试依赖 / test_depends| src_zephyr_security_access_control_guards_path_guard_py
    ...
    class D_INFRA_RUNTIME,D_AUTONOMY_PERM external_design
```
...
graph TD
    ...
    class D_SHARED,D_INFRA_RUNTIME,D_INTELLIGENCE,D_FEEDBACK_LOOP,D_GOV_OPS_RESILIENCE,D_GOV_RULE,D_GOV_ENFORCEMENT external_prod
    class D_GOV_DRIFT,D_GOV_AUDIT,D_GOVERNANCE,D_AUTONOMY_PERM,D_GOV_SCRIPTS,D_SECURITY_LLM,D_AUTONOMY_CORE external_design
```
...
graph TD
    ...
    class D_INTEGRATION,D_SHARED,D_GOV_RULE,D_INFRA_RUNTIME,D_FBL_VERIFICATION,D_GOV_ENFORCEMENT external_prod
...
graph TD
    ...
    D_SECURITY_LLM -.->|测试依赖 / test_depends| src_zephyr_security_llm_defense_llm_security_protocol_py
    D_SECURITY_LLM -.->|测试依赖 / test_depends| src_zephyr_security_llm_defense_llm_security_layers_l4_agent_py
    D_SECURITY_LLM -.->|测试依赖 / test_depends| src_zephyr_security_llm_defense_llm_security_protocol_py
    classDef production fill:#e1f5fe,stroke:#01579b,stroke-width:2px,color:#000
    classDef design fill:#fff3e0,stroke:#e65100,stroke-width:2px,color:#000,stroke-dasharray: 5 5
    classDef external_prod fill:#e8f5e9,stroke:#1b5e20,stroke-width:1px,color:#000
    ...
    class D_GOV_AUDIT,D_SHARED,D_INTEGRATION,D_GOV_OPS_RESILIENCE,D_FEEDBACK_LOOP external_prod
...
graph TD
    ...
    class D_GOV_AUDIT,D_SHARED,D_INFRA_RUNTIME,D_GOV_ENFORCEMENT external_prod
...
graph TD
    ...
    class D_FBL_VERIFICATION,D_INTEGRATION,D_SHARED,D_GOV_DRIFT,D_GOVERNANCE,D_GOV_AUDIT,D_INFRA_RUNTIME,D_GOV_ENFORCEMENT external_prod
...
| 2 | D_AUTONOMY_CORE 自治核心: Agent Spec -> Pipeline 集成桥接层 (__init__.py) | → | SequenceGuard — 操作序列守卫. (sequence_guard.py) | runtime / runtime |
| 3 | D_AUTONOMY_CORE 自治核心: test_agent_creation_policy.py | → | AgentCreationPolicy — Agent 创建策略. (agent_c... | 测试依赖 / test_depends |
| 4 | D_AUTONOMY_CORE 自治核心: test_auto_bootstrap.py | → | SequenceGuard — 操作序列守卫. (sequence_guard.py) | contract / contract |
...
| 156 | D_GOV_AUDIT 审计追踪: test_audit_log_guard.py | → | Stub module: zephyr.security.access_control.gua... | 测试依赖 / test_depends |
...
| 166 | D_GOV_DOCS 架构文档治理: blueprint.md | → | SequenceGuard — 操作序列守卫. (sequence_guard.py) | contract / contract |
| 167 | D_GOV_DOCS 架构文档治理: blueprint.md | → | SequenceGuard — 操作序列守卫. (sequence_guard.py) | runtime / runtime |
| 168 | D_GOV_DRIFT 漂移检测: blueprint.md | → | SequenceGuard — 操作序列守卫. (sequence_guard.py) | runtime / runtime |
...
> 本域与 23 个外部域直接连接（出边 138 条 + 入边 253 条 = 391 条）。只显示直接连接的域，不展开具体节点。
