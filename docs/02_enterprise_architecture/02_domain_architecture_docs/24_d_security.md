...
> 展示全部 165 个模块（生产态 99 + 设计态 0 + 原型态 66），标签标注成熟度。
...
graph TD
    ...
    class D_AUTONOMY_CORE,D_GOV_DRIFT,D_GOVERNANCE,D_GOV_AUDIT,D_INFRA_RECOVERY external_prod
...
graph TD
    ...
    class D_FEEDBACK_LOOP,D_SHARED,D_GOV_OPS_RESILIENCE,D_GOVERNANCE,D_GOV_RULE,D_INFRA_RUNTIME external_prod
...
graph TD
    ...
    D_AUTONOMY_CORE["(生产态 / production) D_AUTONOMY_CORE"]
    D_AUTONOMY_CORE -.->|runtime / runtime| src_zephyr_security_access_control_phase_executor_py
    ...
    class D_SHARED,D_INTELLIGENCE,D_AUTONOMY_CORE,D_GOV_OPS_RESILIENCE,D_GOV_AUDIT external_prod
...
graph TD
    ...
    class D_AUTONOMY_CORE,D_SHARED,D_GOVERNANCE,D_INFRA_RUNTIME,D_INTEGRATION,D_GOV_OPS_RESILIENCE external_prod
    class D_GOV_AUDIT,D_GOV_SCRIPTS,D_GOV_ENFORCEMENT external_design
```
...
graph TD
    ...
    class D_GOV_DRIFT,D_FEEDBACK_LOOP,D_SHARED,D_GOVERNANCE,D_AUTONOMY_CORE,D_INFRA_RUNTIME,D_GOV_AUDIT,D_GOV_ENFORCEMENT external_prod
...
|:--:|---------|:--:|---------|---------|
| 1 | D_AUTONOMY_CORE 自治核心: MOD-INF-019: Agent Spec — All Skill Modules (a... | → | phase_executor.py | runtime / runtime |
| 2 | D_AUTONOMY_CORE 自治核心: MOD-INF-019: Agent Spec — All Skill Modules (a... | → | LLM Security Gateway - Streamlit Dashboard. (ap... | runtime / runtime |
| 3 | D_AUTONOMY_CORE 自治核心: ContextInjector: retrieve and inject relevant k... | → | gateway.py | 导入依赖 / import_depends |
| 4 | D_COMPLIANCE 合规: __init__.py | → | Alert Router — alert_router.py (alert_router.py) | 导入依赖 / import_depends |
| 5 | D_COMPLIANCE 合规: __init__.py | → | Cold Start Bootstrapper — 冷启动引导 §6.31。 ... | 导入依赖 / import_depends |
| 6 | D_COMPLIANCE 合规: __init__.py | → | G-CT-005 — ManagedDriftEvent Pydantic V2 BaseM... | 导入依赖 / import_depends |
| 7 | D_COMPLIANCE 合规: __init__.py | → | Auto Reconciler — reconciler.py (reconciler.py) | 导入依赖 / import_depends |
| 8 | D_COMPLIANCE 合规: __init__.py | → | Drift Runbook Generator — 漂移演练手册自动生成... | 导入依赖 / import_depends |
| 9 | D_FEEDBACK_LOOP 反馈循环引擎: evolution_engine.py | → | gateway.py | 导入依赖 / import_depends |
| 10 | D_GOVERNANCE 生命周期管理: git_commit.py — GitCommitGateway CLI 封装（OPS... | → | Session 级并发协调模块（P2-SES 落地）。 (sessio... | 导入依赖 / import_depends |
| 11 | D_GOVERNANCE 生命周期管理: G-CT-007 契约：Budget -> RBAC 配额限制. (rbac_b... | → | PermissionGuard — 七层权限编排器. (permission_... | 导入依赖 / import_depends |
| 12 | D_GOVERNANCE 生命周期管理: Delegation Engine — MOD-INF-022 (delegation_en... | → | gateway.py | 导入依赖 / import_depends |
| 13 | D_GOVERNANCE 生命周期管理: GovernanceServer: 治理域统一MCP入口 (governance... | → | Cold Start Bootstrapper — 冷启动引导 §6.31。 ... | 导入依赖 / import_depends |
| 14 | D_GOVERNANCE 生命周期管理: GovernanceServer: 治理域统一MCP入口 (governance... | → | PermissionGuard — 七层权限编排器. (permission_... | 导入依赖 / import_depends |
| 15 | D_GOVERNANCE 生命周期管理: session 隔离 stash 红蓝对抗极限测试。 (test_ses... | → | Session 级并发协调模块（P2-SES 落地）。 (sessio... | 测试依赖 / test_depends |
| 16 | D_GOV_AUDIT 审计追踪: cli.py | → | judge.py | 导入依赖 / import_depends |
| 17 | D_GOV_AUDIT 审计追踪: cli.py | → | validator.py | 导入依赖 / import_depends |
| 18 | D_GOV_AUDIT 审计追踪: reconciliation_registry.py — GitCommitGateway ... | → | Session 级并发协调模块（P2-SES 落地）。 (sessio... | 导入依赖 / import_depends |
| 19 | D_GOV_DOCS 架构文档治理: blueprint.md | → | phase_executor.py | contract / contract |
| 20 | D_GOV_DOCS 架构文档治理: blueprint.md | → | LLM Security Gateway - Streamlit Dashboard. (ap... | contract / contract |
...
graph LR
    ...
    D_GOV_DOCS -->|4条 contract / contract, runtime / runtime| D_SECURITY
    D_GOV_DRIFT -->|4条 导入依赖 / import_depends, runtime / runtime| D_SECURITY
    D_AUTONOMY_CORE -->|3条 导入依赖 / import_depends, runtime / runtime| D_SECURITY
