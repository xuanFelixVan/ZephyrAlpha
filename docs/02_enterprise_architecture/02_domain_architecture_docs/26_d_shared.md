...
graph TD
    ...
    class D_INFRASTRUCTURE,D_INTEGRATION,D_AUTONOMY_CORE,D_GOVERNANCE external_design
```
...
graph TD
    ...
    class D_INFRASTRUCTURE,D_TRADING,D_GOV_OPS_RESILIENCE,D_SECURITY,D_INTEGRATION external_prod
    class D_INFRA_RUNTIME,D_SECURITY_LLM,D_INFRA_RECOVERY,D_AUTONOMY_CORE external_design
```
...
graph TD
    ...
    class D_GOV_OPS_RESILIENCE,D_GOV_AUDIT external_design
```
...
graph TD
    ...
    class D_FEEDBACK_LOOP,D_GOV_AUDIT,D_SECURITY,D_INFRA_RUNTIME,D_GOV_OPS_RESILIENCE,D_INTEGRATION external_prod
    class D_ORCHESTRATOR,D_SECURITY_LLM external_design
```
...
graph TD
    ...
    class D_SIMULATION,D_SECURITY,D_GOV_OPS_RESILIENCE,D_GOVERNANCE,D_FBL_DIAGNOSERS,D_FBL_VERIFICATION,D_FUNDAMENTAL_SIGNAL,D_FBL_DETECTORS,D_INFRA_RUNTIME,D_FEEDBACK_LOOP,D_ORCHESTRATOR,D_TRADING,D_INTEGRATION,D_GOV_AUDIT,D_INTELLIGENCE,D_GOV_DRIFT external_prod
```
...
graph TD
    ...
    class D_GOVERNANCE,D_GOV_DRIFT,D_GOV_ENFORCEMENT,D_GOV_AUDIT,D_POSITION,D_GOV_OPS_RESILIENCE external_prod
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_FBL_DETECTORS,D_ORCHESTRATOR,D_GOV_CODE_QUALITY,D_INFRA_A2A,D_GOVERNANCE,D_GOV_RULE external_prod
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_GOVERNANCE,D_GOV_DRIFT,D_DATA,D_FBL_DETECTORS,D_ORCHESTRATOR,D_GOV_ENFORCEMENT,D_SIMULATION,D_INTEGRATION,D_TRADING external_prod
...
| 558 | D_INFRA_RUNTIME 运行时集成: 包 shared.adaptation 的初始化文件。 (__init__.py) | → | Prompt Version Manager — 版本化 Prompt 治理。 ... | config_depends / config_depends |
...
| 561 | D_INFRA_RUNTIME 运行时集成: 包 shared.capacity_governance 的初始化文件。 (_... | → | adaptive_sampler.py | config_depends / config_depends |
...
| 563 | D_INFRA_RUNTIME 运行时集成: 包 shared.dependency 的初始化文件。 (__init__.py) | → | Dependency Graph — 任务卡依赖关系管理。 (depen... | config_depends / config_depends |
| 564 | D_INFRA_RUNTIME 运行时集成: 包 shared.draft 的初始化文件。 (__init__.py) | → | Draft Assistant — 想法 -> MTH-012 蓝图骨架生成... | config_depends / config_depends |
| 565 | D_INFRA_RUNTIME 运行时集成: __init__.py | → | CT-DLQ-001: DeadLetterQueue -> System Event Bus... | 导入依赖 / import_depends |
| 566 | D_INFRA_RUNTIME 运行时集成: __init__.py | → | cache.py —— 统一缓存抽象（Phase 8 新增 | 盲点... | 导入依赖 / import_depends |
| 567 | D_INFRA_RUNTIME 运行时集成: __init__.py | → | ProcessLifecycleGateway — 进程生命周期统一入口... | 导入依赖 / import_depends |
| 568 | D_INFRA_RUNTIME 运行时集成: 包 shared.knowledge 的初始化文件。 (__init__.py) | → | KE Linker — 知识条目关联图。 (ke_linker.py) | config_depends / config_depends |
| 569 | D_INFRA_RUNTIME 运行时集成: 包 shared.maintenance 的初始化文件。 (__init__.py) | → | code_economy_analyzer.py | config_depends / config_depends |
...
| 577 | D_INFRA_RUNTIME 运行时集成: shared.security — auto-generated package init.... | → | idempotency.py —— Re-export wrapper -> canoni... | config_depends / config_depends |
