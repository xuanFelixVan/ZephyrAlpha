...
graph TD
    ...
    class D_GOV_RULE,D_INFRA_RUNTIME,D_GOVERNANCE,D_GOV_AUDIT external_prod
...
graph TD
    ...
    class D_GOV_OPS_RESILIENCE,D_GOV_DRIFT,D_FBL_DETECTORS,D_GOVERNANCE,D_GOV_CODE_QUALITY,D_INFRA_RECOVERY,D_SECURITY,D_FUNDAMENTAL_SIGNAL,D_INTELLIGENCE,D_FEEDBACK_LOOP,D_FBL_VERIFICATION,D_INFRA_RUNTIME,D_INFRA_A2A,D_FRONTEND,D_AUTONOMY_CORE,D_INTEGRATION,D_GOV_AUDIT,D_ORCHESTRATOR external_prod
```
...
graph TD
    ...
    class D_FEEDBACK_LOOP,D_GOV_AUDIT,D_INFRA_RUNTIME,D_GOV_OPS_RESILIENCE external_prod
    class D_GOVERNANCE,D_GOV_SCRIPTS,D_INTEGRATION,D_AUTONOMY_CORE external_design
```
...
graph TD
    ...
    class D_GOVERNANCE,D_INFRA_RUNTIME,D_OPS,D_GOV_AUDIT,D_GOV_OPS_RESILIENCE,D_DATA,D_FBL_DETECTORS,D_GOV_RULE,D_INTEGRATION,D_ORCHESTRATOR,D_TRADING external_prod
    class D_INFRASTRUCTURE external_design
```
...
| 562 | D_INFRA_RUNTIME 运行时集成: 包 shared.knowledge 的初始化文件。 (__init__.py) | → | KE Structurer — 知识条目结构化提取。 (ke_struc... | config_depends / config_depends |
...
> 本域与 37 个外部域直接连接（出边 120 条 + 入边 851 条 = 971 条）。只显示直接连接的域，不展开具体节点。
