...
graph TD
    ...
    class D_FEEDBACK_LOOP,D_GOV_AUDIT,D_GOV_OPS_RESILIENCE,D_INFRA_RUNTIME,D_INTEGRATION,D_GOV_DRIFT,D_ORCHESTRATOR,D_SECURITY,D_GOV_RULE external_prod
...
graph TD
    ...
    class D_SIMULATION,D_FBL_VERIFICATION,D_GOV_OPS_RESILIENCE,D_FUNDAMENTAL_SIGNAL,D_GOV_CODE_QUALITY,D_SECURITY,D_GOVERNANCE,D_FBL_DETECTORS,D_FBL_DIAGNOSERS,D_INFRA_RUNTIME,D_INTELLIGENCE,D_INTEGRATION,D_GOV_AUDIT,D_INFRA_RECOVERY,D_TRADING,D_ORCHESTRATOR external_prod
...
graph TD
    ...
    class D_GOV_DRIFT,D_INFRA_RUNTIME,D_FEEDBACK_LOOP,D_GOV_OPS_RESILIENCE,D_GOVERNANCE,D_SIMULATION,D_INTEGRATION,D_ORCHESTRATOR external_prod
...
| 292 | D_GOV_RULE 规则治理: ZephyrAlpha — gates/ai_capability_guard.py (ai... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
...
| 305 | D_GOV_RULE 规则治理: constitutional_update.py —— 宪法自愈（Phase 1... | → | file_utils.py —— 安全文件操作工具（Phase 3 新... | 导入依赖 / import_depends |
| 306 | D_GOV_RULE 规则治理: constitutional_update.py —— 宪法自愈（Phase 1... | → | session_audit.py —— Session 审计轨迹（Phase 1... | 导入依赖 / import_depends |
| 307 | D_GOV_SCRIPTS 脚本治理: DM-106: P2-B 迁移全量验证脚本 (dm106_p2b_verifi... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 308 | D_GOV_SCRIPTS 脚本治理: analyze_orphan_consumers.py | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 309 | D_GOV_SCRIPTS 脚本治理: audit_post_sync_commands.py — post_sync_standa... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 310 | D_GOV_SCRIPTS 脚本治理: DM-105: depgraph 未分配节点三策略处理脚本 (dm10... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 311 | D_GOV_SCRIPTS 脚本治理: [INVARIANTS] 只读访问 depgraph（mode=ro）；禁止... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 312 | D_GOV_SCRIPTS 脚本治理: constants.py — 审计脚本共享常量 (constants.py) | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 313 | D_GOV_SCRIPTS 脚本治理: _shared/file_utils.py — 原子写入共享工具（ARCH... | → | file_utils.py —— 安全文件操作工具（Phase 3 新... | 导入依赖 / import_depends |
| 314 | D_GOV_SCRIPTS 脚本治理: _shared/yaml_utils.py — YAML 文件加载共享工具 ... | → | yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
| 315 | D_GOV_SCRIPTS 脚本治理: [INVARIANTS] pg_advisory_lock 写锁; build_statu... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 316 | D_GOV_SCRIPTS 脚本治理: [INVARIANTS] 原子写入（RULE-ONE）；变更前验证；... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 317 | D_GOV_SCRIPTS 脚本治理: [INVARIANTS] 原子写入（RULE-ONE）；变更前验证；... | → | yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
| 318 | D_GOV_SCRIPTS 脚本治理: GATE-SSOT: SSoT 创建门禁（pre-commit hook 双保.... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 319 | D_GOV_SCRIPTS 脚本治理: 测试结构合规门禁——检查 test_*.py 文件结构，防... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 320 | D_GOV_SCRIPTS 脚本治理: GATE-SSOT-SINGLESOURCE: SSoT 单一真源门禁（Phas... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 321 | D_GOV_SCRIPTS 脚本治理: # [BLUEPRINT] MOD-INF-005 | scripts/governance/... | → | yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
| 322 | D_GOV_SCRIPTS 脚本治理: G13: 从 depgraph (PostgreSQL) 生成资产清单全景... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 323 | D_GOV_SCRIPTS 脚本治理: G12: 从 depgraph (PostgreSQL) 生成契约目录全景... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 324 | D_GOV_SCRIPTS 脚本治理: generate_contracts.py -- SSoT to Codegen pipeli... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 325 | D_GOV_SCRIPTS 脚本治理: G-panorama-registry: 自动生成全景图清单总表 (ge... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 326 | D_GOV_SCRIPTS 脚本治理: validate_module_lifecycle.py — 模块生命周期校... | → | yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
| 327 | D_GOV_SCRIPTS 脚本治理: validate_interface_contracts.py — 接口契约校验... | → | yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
| 328 | D_GOV_SCRIPTS 脚本治理: extract_decisiongraph - decisiongraph on-demand... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 329 | D_GOV_SCRIPTS 脚本治理: [INVARIANTS] 禁止AI直接Read 157MB depgraph文件.... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 330 | D_GOV_SCRIPTS 脚本治理: [INVARIANTS] YAML 是唯一真源; DB 为只读缓存; 同... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 331 | D_GOV_SCRIPTS 脚本治理: # [BLUEPRINT] MOD-INF-005 | scripts/governance/... | → | yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
| 332 | D_GOV_SCRIPTS 脚本治理: check_gate_inventory_drift.py — commit_gates .... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 333 | D_GOV_SCRIPTS 脚本治理: _concurrency.py | → | process_pool.py - Shared process pool for MCP s... | 导入依赖 / import_depends |
| 334 | D_GOV_SCRIPTS 脚本治理: create_task_from_finding.py — Finding → 任务.... | → | ZephyrAlpha 任务系统核心数据模型 (models.py) | 导入依赖 / import_depends |
| 335 | D_GOV_SCRIPTS 脚本治理: create_task_from_finding.py — Finding → 任务.... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 336 | D_GOV_SCRIPTS 脚本治理: SQLite → PostgreSQL 数据迁移脚本 (migrate_data.py) | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 337 | D_GOV_SCRIPTS 脚本治理: concurrent_commit_test.py — 幽灵提交红蓝对抗脚... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 338 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_003 for rule TRAE-003 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 339 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_004 for rule TRAE-004 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 340 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_006 for rule TRAE-006 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 341 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_007 for rule TRAE-007 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 342 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_008 for rule TRAE-008 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 343 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_009 for rule TRAE-009 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 344 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_010 for rule TRAE-010 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 345 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_011 for rule TRAE-011 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 346 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_012 for rule TRAE-012 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 347 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_016 for rule TRAE-016 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 348 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_017 for rule TRAE-017 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 349 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_018 for rule TRAE-018 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 350 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_020 for rule TRAE-020 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 351 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_021 for rule TRAE-021 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 352 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_022 for rule TRAE-022 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 353 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_023 for rule TRAE-023 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 354 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_024 for rule TRAE-024 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 355 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_025 for rule TRAE-025 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 356 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_026 for rule TRAE-026 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 357 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_027 for rule TRAE-027 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 358 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_028 for rule TRAE-028 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 359 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_029 for rule TRAE-029 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 360 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_030 for rule TRAE-030 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 361 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_031 for rule TRAE-031 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 362 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_032 for rule TRAE-032 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 363 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_033 for rule TRAE-033 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 364 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_034 for rule TRAE-034 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 365 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_035 for rule TRAE-035 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 366 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_036 for rule TRAE-036 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 367 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_037 for rule TRAE-037 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 368 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_038 for rule TRAE-038 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 369 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_039 for rule TRAE-039 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 370 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_040 for rule TRAE-040 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 371 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_041 for rule TRAE-041 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 372 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_042 for rule TRAE-042 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 373 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_043 for rule TRAE-043 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 374 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_044 for rule TRAE-044 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 375 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_045 for rule TRAE-045 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 376 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_046 for rule TRAE-046 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 377 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_047 for rule TRAE-047 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 378 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_048 for rule TRAE-048 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 379 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_049 for rule TRAE-049 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 380 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_050 for rule TRAE-050 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 381 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_051 for rule TRAE-051 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 382 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_052 for rule TRAE-052 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 383 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_053 for rule TRAE-053 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 384 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_054 for rule TRAE-054 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 385 | D_GOV_SCRIPTS 脚本治理: Test gate g_trae_055 for rule TRAE-055 — calls... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 386 | D_INFRASTRUCTURE: ZephyrAlpha — shared/contracts/ (__init__.py) | → | ZephyrAlpha — shared/contracts/enforcer.py (en... | 导入依赖 / import_depends |
| 387 | D_INFRASTRUCTURE: ZephyrAlpha — shared/contracts/ (__init__.py) | → | shared/contracts/factories.py — 跨层数据契约工... | 导入依赖 / import_depends |
| 388 | D_INFRASTRUCTURE: ZephyrAlpha — shared/contracts/ (__init__.py) | → | ZephyrAlpha — shared/contracts/registry.py (re... | 导入依赖 / import_depends |
| 389 | D_INFRASTRUCTURE: ZephyrAlpha — shared/contracts/ (__init__.py) | → | ZephyrAlpha — shared/contracts/runtime_plane_t... | 导入依赖 / import_depends |
| 390 | D_INFRASTRUCTURE: ZephyrAlpha — shared/contracts/ (__init__.py) | → | system_configuration.py | 导入依赖 / import_depends |
| 391 | D_INFRASTRUCTURE: ZephyrAlpha — shared/contracts/ (__init__.py) | → | ZephyrAlpha — shared/contracts/timestamp.py (t... | 导入依赖 / import_depends |
| 392 | D_INFRASTRUCTURE: ZephyrAlpha — shared/contracts/ (__init__.py) | → | trace_context.py | 导入依赖 / import_depends |
| 393 | D_INFRASTRUCTURE: ZephyrAlpha — shared/contracts/ (__init__.py) | → | experiment_result.py | 导入依赖 / import_depends |
| 394 | D_INFRASTRUCTURE: ZephyrAlpha — shared/contracts/ (__init__.py) | → | model_serving_response.py | 导入依赖 / import_depends |
| 395 | D_INFRASTRUCTURE: ZephyrAlpha — shared/contracts/ (__init__.py) | → | LLMGatewayProtocol — LLM 网关抽象接口 (llm_gat... | 导入依赖 / import_depends |
| 396 | D_INFRASTRUCTURE: ZephyrAlpha — shared/contracts/ (__init__.py) | → | orchestration_protocol.py | 导入依赖 / import_depends |
| 397 | D_INFRASTRUCTURE: ZephyrAlpha — shared/contracts/ (__init__.py) | → | money.py | 导入依赖 / import_depends |
| 398 | D_INFRASTRUCTURE: ZephyrAlpha — shared/contracts/ (__init__.py) | → | Re-export shim — 真源已收敛至 zephyr.shared.co... | 导入依赖 / import_depends |
| 399 | D_INFRASTRUCTURE: ZephyrAlpha — shared/contracts/ (__init__.py) | → | skill_protocol.py | 导入依赖 / import_depends |
| 400 | D_INFRASTRUCTURE: ZephyrAlpha — shared/contracts/ (__init__.py) | → | TaskRepositoryProtocol — TaskRepository 的 Pro... | 导入依赖 / import_depends |
| 401 | D_INFRASTRUCTURE: Auto-generated contracts package — backpressur... | → | pause.py | 导入依赖 / import_depends |
| 402 | D_INFRASTRUCTURE: Auto-generated contracts package — backpressur... | → | resume.py | 导入依赖 / import_depends |
| 403 | D_INFRASTRUCTURE: Auto-generated contracts package — backpressur... | → | throttle.py | 导入依赖 / import_depends |
| 404 | D_INFRASTRUCTURE: shared.contracts.core — auto-generated package... | → | BaseEvent — 跨层事件基类 (base_event.py) | 导入依赖 / import_depends |
| 405 | D_INFRASTRUCTURE: shared.contracts.core — auto-generated package... | → | gate_types.py | 导入依赖 / import_depends |
| 406 | D_INFRASTRUCTURE: Auto-generated contracts package — errors (__i... | → | contract_violation_error.py | 导入依赖 / import_depends |
| 407 | D_INFRASTRUCTURE: Auto-generated contracts package — errors (__i... | → | CTR-ERR-001: DataQualityError / 行情质量门禁不.... | 导入依赖 / import_depends |
| 408 | D_INFRASTRUCTURE: Auto-generated contracts package — errors (__i... | → | execution_rejection_error.py | 导入依赖 / import_depends |
| 409 | D_INFRASTRUCTURE: Auto-generated contracts package — errors (__i... | → | CTR-ERR-002: FactorComputationError / 因子计算.... | 导入依赖 / import_depends |
| 410 | D_INFRASTRUCTURE: Auto-generated contracts package — errors (__i... | → | risk_limit_violation_error.py | 导入依赖 / import_depends |
| 411 | D_INFRASTRUCTURE: Auto-generated contracts package — errors (__i... | → | signal_degradation_warning.py | 导入依赖 / import_depends |
| 412 | D_INFRASTRUCTURE: __init__.py | → | budget_alert.py | 导入依赖 / import_depends |
| 413 | D_INFRASTRUCTURE: shared.contracts.experiment — auto-generated p... | → | model_serving_response.py | config_depends / config_depends |
| 414 | D_INFRASTRUCTURE: experiment_result.py | → | trace_context.py | 导入依赖 / import_depends |
| 415 | D_INFRASTRUCTURE: factor_signal.py | → | trace_context.py | 导入依赖 / import_depends |
| 416 | D_INFRASTRUCTURE: fill.py | → | trace_context.py | 导入依赖 / import_depends |
| 417 | D_INFRASTRUCTURE: __init__.py | → | agent_identity.py | 导入依赖 / import_depends |
| 418 | D_INFRASTRUCTURE: __init__.py | → | permission.py | 导入依赖 / import_depends |
| 419 | D_INFRASTRUCTURE: Backward-compat shim — only instrument remains... | → | Backward-compat shim — canonical location is z... | 导入依赖 / import_depends |
...
| 514 | D_INFRA_RUNTIME 运行时集成: file_watcher.py | → | EventBus — 事件总线（带背压控制）(M-07) (event... | 导入依赖 / import_depends |
...
| 564 | D_INFRA_RUNTIME 运行时集成: 包 shared.knowledge 的初始化文件。 (__init__.py) | → | KMS Interface — KE 推送契约 + 生命周期关联。 (... | config_depends / config_depends |
| 565 | D_INFRA_RUNTIME 运行时集成: 包 shared.maintenance 的初始化文件。 (__init__.py) | → | code_economy_analyzer.py | config_depends / config_depends |
...
| 573 | D_INFRA_RUNTIME 运行时集成: shared.security — auto-generated package init.... | → | idempotency.py —— Re-export wrapper -> canoni... | config_depends / config_depends |
...
| 742 | D_INTELLIGENCE 上下文管理: test_model_discovery.py | → | constants.py —— 共享枚举 & 常量集中 re-export... | 测试依赖 / test_depends |
...
> 本域与 36 个外部域直接连接（出边 119 条 + 入边 857 条 = 976 条）。只显示直接连接的域，不展开具体节点。
