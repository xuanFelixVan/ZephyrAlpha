...
| 原型态模块 | 190 | Prototype Modules | 190 |
...
> 展示全部 299 个模块（生产态 109 + 设计态 0 + 原型态 190），标签标注成熟度。
...
graph TD
    ...
    class D_BACKTEST,D_INFRA_RUNTIME,D_PF_ALLOC external_prod
...
graph TD
    ...
    class D_GOV_OPS_RESILIENCE,D_GOVERNANCE,D_SECURITY,D_INTEGRATION,D_INFRA_RUNTIME external_prod
    class D_INFRASTRUCTURE,D_ML_TRAIN,D_TRADING,D_ORCHESTRATOR external_design
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_GOVERNANCE,D_INFRA_RECOVERY,D_GOV_OPS_RESILIENCE external_prod
    class D_INTEGRATION,D_GOV_ENFORCEMENT,D_AUTONOMY_CORE,D_GOV_AUDIT,D_SECURITY,D_TRADING,D_INTELLIGENCE,D_SECURITY_LLM external_design
```
...
graph TD
    ...
    class D_GOV_RULE,D_INFRA_RUNTIME,D_ORCHESTRATOR,D_FEEDBACK_LOOP,D_INTEGRATION,D_INFRA_RECOVERY external_prod
...
graph TD
    ...
    class D_FEEDBACK_LOOP,D_GOV_AUDIT,D_INFRA_RUNTIME,D_TRADING external_prod
    class D_GOVERNANCE,D_GOV_KB,D_INTEGRATION,D_DATA external_design
```
...
graph TD
    ...
    class D_GOV_OPS_RESILIENCE,D_GOV_DRIFT,D_FBL_DETECTORS,D_GOVERNANCE,D_GOV_CODE_QUALITY,D_INFRA_RECOVERY,D_SECURITY,D_FUNDAMENTAL_SIGNAL,D_INTELLIGENCE,D_FEEDBACK_LOOP,D_FBL_VERIFICATION,D_GOV_AUDIT,D_INFRA_RUNTIME,D_INFRA_A2A,D_FRONTEND,D_AUTONOMY_CORE,D_INTEGRATION,D_ORCHESTRATOR external_prod
```
...
graph TD
    ...
    class D_GOVERNANCE,D_OPS,D_GOV_AUDIT,D_GOV_OPS_RESILIENCE,D_GOV_ENFORCEMENT,D_POSITION external_prod
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_FBL_DETECTORS,D_GOV_RULE,D_ORCHESTRATOR,D_GOV_CODE_QUALITY,D_GOV_DRIFT,D_INFRA_A2A,D_FEEDBACK_LOOP,D_GOVERNANCE external_prod
```
...
graph TD
    ...
    D_GOVERNANCE["(生产态 / production) D_GOVERNANCE"]
    D_GOVERNANCE -->|导入依赖 / import_depends| src_zephyr_shared_contracts_identity_permission_py
    D_INFRA_RUNTIME -.->|导入依赖 / import_depends| src_zephyr_shared_utils_context_py
    ...
    class D_FEEDBACK_LOOP,D_GOV_AUDIT,D_INFRA_RUNTIME,D_GOVERNANCE,D_TRADING external_prod
    class D_AUTONOMY_CORE,D_INFRASTRUCTURE,D_INTELLIGENCE,D_GOV_DRIFT external_design
```
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 190 个，23 条域内依赖）。
...
graph TD
    ...
    class D_GOVERNANCE,D_INFRA_RUNTIME,D_OPS,D_GOV_AUDIT,D_GOV_OPS_RESILIENCE,D_DATA,D_FBL_DETECTORS,D_INTEGRATION,D_ORCHESTRATOR,D_TRADING external_prod
    class D_INFRASTRUCTURE external_design
```
...
| 130 | D_GOVERNANCE 生命周期管理: decisiongraph Schema DDL + 不变量声明 (decision... | → | yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
...
| 408 | D_INFRASTRUCTURE: shared.contracts.experiment — auto-generated p... | → | model_serving_response.py | config_depends / config_depends |
...
| 508 | D_INFRA_RUNTIME 运行时集成: DEPRECATED: 此文件已废弃。 (event_bus_upgrade.py) | → | EventBus 升级策略引擎 (upgrade_strategy.py) | 导入依赖 / import_depends |
| 509 | D_INFRA_RUNTIME 运行时集成: RI-13 EventStore — 事件存储 (event_store.py) | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 510 | D_INFRA_RUNTIME 运行时集成: RI-13 EventStore — 事件存储 (event_store.py) | → | SQLite 连接工厂真源（SSoT） (sqlite_factory.py) | 导入依赖 / import_depends |
| 511 | D_INFRA_RUNTIME 运行时集成: Event Store — 事件持久化存储。 (event_store.py) | → | EventBus — 事件总线（带背压控制）(M-07) (event... | 导入依赖 / import_depends |
| 512 | D_INFRA_RUNTIME 运行时集成: Event Store — 事件持久化存储。 (event_store.py) | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 513 | D_INFRA_RUNTIME 运行时集成: file_watcher.py | → | ZephyrAlpha 蓝图拆解器 (blueprint_decomposer.py) | 导入依赖 / import_depends |
...
| 555 | D_INFRA_RUNTIME 运行时集成: 包 shared.blueprint_tools 的初始化文件。 (__ini... | → | ai_understandability_constraint.py | config_depends / config_depends |
...
| 572 | D_INFRA_RUNTIME 运行时集成: shared.security — auto-generated package init.... | → | idempotency.py —— Re-export wrapper -> canoni... | config_depends / config_depends |
| 573 | D_INFRA_RUNTIME 运行时集成: 包 shared.session 的初始化文件。 (__init__.py) | → | Session Boundary — 会话边界管理。 (session_bou... | config_depends / config_depends |
...
> 本域与 37 个外部域直接连接（出边 120 条 + 入边 852 条 = 972 条）。只显示直接连接的域，不展开具体节点。
