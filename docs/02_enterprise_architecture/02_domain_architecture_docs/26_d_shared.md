...
graph TD
    ...
    class D_INFRASTRUCTURE,D_INFRA_RUNTIME,D_AUTONOMY_CORE,D_GOV_AUDIT,D_SECURITY,D_INFRA_RECOVERY,D_ORCHESTRATOR,D_INTELLIGENCE,D_SECURITY_LLM external_design
```
...
graph TD
    ...
    class D_FEEDBACK_LOOP,D_GOV_AUDIT,D_TRADING,D_GOV_RULE,D_SECURITY,D_GOV_DRIFT external_prod
    class D_GOV_ENFORCEMENT,D_INFRA_RUNTIME,D_GOVERNANCE,D_SECURITY_LLM external_design
```
...
graph TD
    ...
    class D_SIMULATION,D_FBL_VERIFICATION,D_GOVERNANCE,D_SECURITY,D_FBL_DIAGNOSERS,D_FBL_DETECTORS,D_FUNDAMENTAL_SIGNAL,D_GOV_OPS_RESILIENCE,D_GOV_CODE_QUALITY,D_INFRA_RUNTIME,D_INTEGRATION,D_GOV_AUDIT,D_GOV_DRIFT,D_GOV_KB,D_TRADING,D_INTELLIGENCE external_prod
...
graph TD
    ...
    class D_GOV_DRIFT,D_GOV_OPS_RESILIENCE,D_POSITION,D_GOVERNANCE,D_GOV_ENFORCEMENT,D_GOV_AUDIT external_prod
```
...
graph TD
    ...
    class D_GOV_RULE,D_INFRA_RUNTIME,D_GOVERNANCE,D_FBL_DETECTORS,D_FEEDBACK_LOOP,D_SECURITY,D_GOV_CODE_QUALITY external_prod
```
...
graph TD
    ...
    class D_GOV_RULE,D_SIMULATION,D_GOV_DRIFT,D_INFRA_RUNTIME,D_GOV_OPS_RESILIENCE,D_POSITION,D_INTEGRATION,D_ORCHESTRATOR,D_GOVERNANCE,D_TRADING external_prod
...
| 289 | D_GOV_RULE 规则治理: ZephyrAlpha — gates/ai_capability_guard.py (ai... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
...
| 622 | D_INFRA_RUNTIME 运行时集成: zombie_scanner.py — 僵尸 Python 进程检测与自动... | → | paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
...
| 766 | D_ORCHESTRATOR 代理编排器: FileTaskMapper — 文件路径 ↔ Task N:N 映射器（... | → | yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
...
| 774 | D_ORCHESTRATOR 代理编排器: StateSynchronizer — 同步 SQLite 状态与文件系统... | → | yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
...
| 791 | D_SECURITY 对抗验证: CommitTrigger — 事件驱动红蓝对抗触发器 (MOD-IN... | → | EventBus — 事件总线（带背压控制）(M-07) (event... | 导入依赖 / import_depends |
...
| 849 | D_TRADING 交易运营: Re-export wrapper: Order 真源在 zephyr.shared.c... | → | OrderSide/OrderStatus/OrderType — 交易枚举真源... | 导入依赖 / import_depends |
...
> 本域与 36 个外部域直接连接（出边 120 条 + 入边 851 条 = 971 条）。只显示直接连接的域，不展开具体节点。
