...
graph TD
    ...
    class D_GOVERNANCE,D_PF_ALLOC,D_INFRA_RUNTIME external_prod
...
graph TD
    ...
    D_INFRASTRUCTURE -.->|导入依赖 / import_depends| src_zephyr_shared_contracts_portfolio_position_py
    ...
    class D_INFRASTRUCTURE,D_ORCHESTRATOR external_prod
    class D_INFRA_RUNTIME,D_GOV_AUDIT,D_SECURITY,D_GOVERNANCE,D_ML_TRAIN,D_SECURITY_LLM,D_INFRA_RECOVERY,D_TRADING external_design
```
...
graph TD
    ...
    class D_GOV_RULE,D_INFRA_RUNTIME,D_INFRA_A2A external_prod
    class D_GOV_AUDIT,D_GOV_OPS_RESILIENCE external_design
```
...
graph TD
    ...
    class D_FEEDBACK_LOOP,D_GOV_AUDIT,D_INTEGRATION,D_GOVERNANCE,D_GOV_OPS_RESILIENCE,D_INTELLIGENCE,D_ORCHESTRATOR external_prod
```
...
graph TD
    ...
    class D_FBL_DETECTORS,D_GOV_OPS_RESILIENCE,D_GOVERNANCE,D_FBL_DIAGNOSERS,D_FBL_VERIFICATION,D_SECURITY,D_FUNDAMENTAL_SIGNAL,D_GOV_CODE_QUALITY,D_SIMULATION,D_TRADING,D_ORCHESTRATOR,D_INFRA_RUNTIME,D_GOV_KB,D_INTEGRATION,D_GOV_DRIFT,D_FEEDBACK_LOOP,D_INFRA_RECOVERY external_prod
...
graph TD
    ...
    class D_GOV_ENFORCEMENT,D_GOV_OPS_RESILIENCE,D_GOV_DRIFT,D_GOVERNANCE,D_POSITION external_prod
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_GOV_CODE_QUALITY,D_INFRA_A2A,D_ORCHESTRATOR,D_FBL_DETECTORS,D_FEEDBACK_LOOP external_prod
```
...
graph TD
    ...
    class D_GOV_ENFORCEMENT,D_INFRA_RUNTIME,D_FBL_DETECTORS,D_GOV_OPS_RESILIENCE,D_GOV_CODE_QUALITY,D_GOVERNANCE,D_GOV_DRIFT,D_GOV_RULE,D_INTEGRATION,D_ORCHESTRATOR,D_TRADING external_prod
...
| 559 | D_INFRA_RUNTIME 运行时集成: 包 shared.blueprint_tools 的初始化文件。 (__ini... | → | ai_understandability_constraint.py | config_depends / config_depends |
...
| 568 | D_INFRA_RUNTIME 运行时集成: __init__.py | → | CT-HEALTH-001: System-wide Health Discovery Reg... | config_depends / config_depends |
| 569 | D_INFRA_RUNTIME 运行时集成: 包 shared.maintenance 的初始化文件。 (__init__.py) | → | Dogfooding — 自举测试：用 TaskCard 管理 TaskCa... | config_depends / config_depends |
| 570 | D_INFRA_RUNTIME 运行时集成: A2A Protocol — shared interface definitions. (... | → | A2A Coordination — shared interface definition... | 导入依赖 / import_depends |
| 571 | D_INFRA_RUNTIME 运行时集成: A2A Protocol — shared interface definitions. (... | → | Core A2A Protocol interface and governance data... | 导入依赖 / import_depends |
| 572 | D_INFRA_RUNTIME 运行时集成: A2A Protocol — shared interface definitions. (... | → | A2A Registry and Agent Card contracts — discov... | 导入依赖 / import_depends |
| 573 | D_INFRA_RUNTIME 运行时集成: A2A Protocol — shared interface definitions. (... | → | A2A data structure contracts — Message, Task, ... | 导入依赖 / import_depends |
| 574 | D_INFRA_RUNTIME 运行时集成: A2A Layer3 Coordination — shared Protocol inte... | → | A2A Coordination — shared interface definition... | 导入依赖 / import_depends |
| 575 | D_INFRA_RUNTIME 运行时集成: 包 shared.reliability 的初始化文件。 (__init__.py) | → | Diff Planner — 最小增量变更规划器。 (diff_plan... | config_depends / config_depends |
| 576 | D_INFRA_RUNTIME 运行时集成: shared.schema — auto-generated package init. (... | → | task_types — 任务系统核心类型 re-export 层 (ta... | config_depends / config_depends |
...
| 578 | D_INFRA_RUNTIME 运行时集成: 包 shared.session 的初始化文件。 (__init__.py) | → | Session Boundary — 会话边界管理。 (session_bou... | config_depends / config_depends |
