...
flowchart TD
    src_zephyr_signal_quality_init_py["(生产态 / production) 包入口 / D_SIGQC — Signal<br/>Quality Domain<br/>包入口。D_SIGQC — Signal Quality Domain<br/>文件: signal_quality/__init__.py"]
    src_zephyr_signal_quality_degradation_monitor_base_py["(生产态 / production) 退化监控基类 / D_SIGQC —<br/>Signal Quality Degradation Monitor Base<br/>退化监控基类。D_SIGQC — Signal Quality<br/>Degradation Monitor Base<br/>文件: signal_quality/degradation_monitor_base.py"]
