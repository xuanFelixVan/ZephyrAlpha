...
graph TD
    ...
    class D_GOVERNANCE,D_INFRA_RUNTIME,D_GOV_OPS_RESILIENCE,D_SIGLEGACY external_design
```
...
graph TD
    ...
    class D_GOVERNANCE,D_INFRA_RUNTIME,D_SIGLEGACY external_design
```
...
| 3 | D_GOV_OPS_RESILIENCE 运维弹性治理: bus_factor_defense.py | → | bus_factor_defense.py | 导入依赖 / import_depends |
| 4 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | ZephyrAlpha — D_FACTOR Alpha Factor Layer (fac... | 导入依赖 / import_depends |
| 5 | D_SIGLEGACY 信号遗留设计态: __init__.py | → | D_FACTOR — Momentum Factor (momentum_factor.py) | contract / contract |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 6 个外部域直接连接（出边 3 条 + 入边 5 条 = 8 条）。只显示直接连接的域，不展开具体节点。
