...
|:--:|---------|:--:|---------|---------|
| 1 | alpha_signal_pipeline.py | → | D_FUNDAMENTAL_SIGNAL 基本面信号: pipeline.py | 导入依赖 / import_depends |
...
> 本域与 3 个外部域直接连接（出边 1 条 + 入边 2 条 = 3 条）。只显示直接连接的域，不展开具体节点。
