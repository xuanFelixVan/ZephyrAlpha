...
graph TD
    ...
    class D_GOV_DOCS,D_GOV_AUDIT,D_AUTONOMY_CORE,D_FRONTEND external_design
```
...
graph TD
    ...
    class D_FEEDBACK_LOOP,D_AUTONOMY_CORE,D_GOV_AUDIT,D_GOV_ENFORCEMENT,D_INFRA_RECOVERY,D_GOVERNANCE,D_SHARED,D_INTEGRATION external_design
```
...
| 55 | D_GOV_DOCS 架构文档治理: blueprint.md | → | __init__.py | runtime / runtime |
...
> 本域与 15 个外部域直接连接（出边 3 条 + 入边 71 条 = 74 条）。只显示直接连接的域，不展开具体节点。
