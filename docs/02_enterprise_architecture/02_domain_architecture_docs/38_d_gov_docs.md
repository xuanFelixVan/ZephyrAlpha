...
graph TD
    ...
    class D_INFRA_RUNTIME,D_FBL_VERIFICATION,D_COMPLIANCE,D_GOV_CODE_QUALITY,D_INFRA_A2A,D_GOV_DRIFT,D_GOVERNANCE,D_INTEGRATION,D_FEEDBACK_LOOP,D_GOV_AUDIT external_design
```
...
| 2 | D_AUTONOMY_CORE 自治核心: Agent Spec -> Pipeline 集成桥接层 (__init__.py) | → | blueprint.md | data / data |
| 3 | D_AUTONOMY_CORE 自治核心: Agent Spec -> Pipeline 集成桥接层 (__init__.py) | → | blueprint.md | runtime / runtime |
| 4 | D_AUTONOMY_CORE 自治核心: Agent Spec -> Pipeline 集成桥接层 (__init__.py) | → | blueprint.md | runtime / runtime |
| 5 | D_AUTONOMY_CORE 自治核心: Agent Spec -> Pipeline 集成桥接层 (__init__.py) | → | blueprint.md | contract / contract |
...
| 12 | D_GOVERNANCE 生命周期管理: Construction Verifier — 施工验证器: 任务卡完成... | → | blueprint.md | contract / contract |
| 13 | D_GOVERNANCE 生命周期管理: post_sync_validator — post_sync_standard 命令.... | → | blueprint.md | contract / contract |
| 14 | D_GOVERNANCE 生命周期管理: model_provider_data.py | → | blueprint.md | runtime / runtime |
| 15 | D_GOVERNANCE 生命周期管理: model_provider_data.py | → | blueprint.md | data / data |
...
| 19 | D_GOV_CODE_QUALITY 代码质量治理: 金丝雀工厂——生成已知oracle 文件 用于引擎检出+... | → | blueprint.md | data / data |
| 20 | D_GOV_CODE_QUALITY 代码质量治理: 金丝雀工厂——生成已知oracle 文件 用于引擎检出+... | → | blueprint.md | contract / contract |
...
> 本域与 11 个外部域直接连接（出边 23 条 + 入边 24 条 = 47 条）。只显示直接连接的域，不展开具体节点。
