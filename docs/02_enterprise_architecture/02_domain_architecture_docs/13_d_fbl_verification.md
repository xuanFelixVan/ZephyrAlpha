...
graph TD
    ...
    D_AUTONOMY_CORE -.->|测试依赖 / test_depends| src_zephyr_feedback_loop_gates_adversarial_validation_py
    D_AUTONOMY_CORE -.->|测试依赖 / test_depends| src_zephyr_feedback_loop_gates_blueprint_validator_py
    D_AUTONOMY_CORE -.->|测试依赖 / test_depends| src_zephyr_feedback_loop_gates_config_governance_py
    D_AUTONOMY_CORE -.->|测试依赖 / test_depends| src_zephyr_feedback_loop_gates_cve_scanner_py
    D_AUTONOMY_CORE -.->|测试依赖 / test_depends| src_zephyr_feedback_loop_gates_concurrent_change_deconfliction_py
    ...
    D_AUTONOMY_CORE -.->|测试依赖 / test_depends| src_zephyr_feedback_loop_gates_parameterized_safety_gate_py
    D_AUTONOMY_CORE -.->|测试依赖 / test_depends| src_zephyr_feedback_loop_gates_conflict_arbitration_py
    D_SECURITY_LLM["(原型态 / prototype) D_SECURITY_LLM"]
    D_SECURITY_LLM -.->|测试依赖 / test_depends| src_zephyr_feedback_loop_gates_llm_cost_router_py
    D_AUTONOMY_CORE -.->|测试依赖 / test_depends| src_zephyr_feedback_loop_gates_dynamic_llm_cost_router_py
    ...
    class D_SECURITY,D_AUTONOMY_CORE,D_FEEDBACK_LOOP,D_SECURITY_LLM external_design
```
...
graph TD
    ...
    D_AUTONOMY_CORE -.->|测试依赖 / test_depends| src_zephyr_feedback_loop_gates_safety_gate_l56_l57_py
    D_GOV_AUDIT -.->|测试依赖 / test_depends| src_zephyr_feedback_loop_verifiers_build_reproducibility_verifier_py
    D_SECURITY["(原型态 / prototype) D_SECURITY"]
    ...
    class D_FEEDBACK_LOOP,D_AUTONOMY_CORE,D_SECURITY external_design
```
...
graph TD
    ...
    class D_GOV_AUDIT,D_INFRA_RECOVERY,D_FEEDBACK_LOOP external_design
```
...
graph TD
    ...
    class D_GOV_AUDIT external_prod
    class D_SECURITY,D_FEEDBACK_LOOP,D_AUTONOMY_CORE external_design
```
...
| 71 | D_FEEDBACK_LOOP 反馈循环引擎: FLE 持久化写入器 — 写 metrics/alerts/dispatch_... | → | _governance_gates.py | runtime / runtime |
