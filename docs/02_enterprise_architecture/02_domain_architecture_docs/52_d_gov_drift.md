...
| 60 | Truth源验证器 / Truth Source Validator (rule_enforcement/... | → | D_SHARED 共享服务: Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 61 | D_GOV_AUDIT 审计追踪: —真源优先级裁决器 / Test Truth Source Validator (audit/t... | → | Truth源验证器 / Truth Source Validator (rule_enforcement/... | 测试依赖 / test_depends |
...
| 63 | D_GOV_CODE_QUALITY 代码质量治理: BreakingChange检测器测试 / Test Breaking Change Detector ... | → | —字段删除/类型变更->CI FAIL / Breaking Change Detector (... | 测试依赖 / test_depends |
...
| 73 | D_OPS 反馈循环: —5.133.2 DI 注入契约 / Budget Engine (ops_governance/bud... | → | 漂移基础设施 / Drift Infrastructure (gov_drift/drift_infr... | 导入依赖 / import_depends |
...
| 76 | D_SECURITY 对抗验证: Main (gov_drift/__main__.py) | → | 漂移引擎 / Drift Engine (gov_drift/drift_engine.py) | 导入依赖 / import_depends |
| 77 | D_SECURITY 对抗验证: Main (gov_drift/__main__.py) | → | 漂移基础设施 / Drift Infrastructure (gov_drift/drift_infr... | 导入依赖 / import_depends |
| 78 | D_SECURITY 对抗验证: Main (gov_drift/__main__.py) | → | Self检查 / Self Check (gov_drift/self_check.py) | 导入依赖 / import_depends |
...
| 81 | D_SECURITY 对抗验证: Analysis (gov_drift/_analysis.py) | → | Credibility引擎 / Credibility Engine (gov_drift/credibili... | 导入依赖 / import_depends |
...
| 87 | D_SECURITY 对抗验证: Analysis (gov_drift/_analysis.py) | → | Self检查 / Self Check (gov_drift/self_check.py) | 导入依赖 / import_depends |
...
| 89 | D_SECURITY 对抗验证: Analysis (gov_drift/_analysis.py) | → | TamperProof审计 / Tamper Proof Audit (gov_drift/tamper_pr... | 导入依赖 / import_depends |
...
| 109 | D_SECURITY 对抗验证: Scanners (gov_drift/_scanners.py) | → | NamingMagic检查器 / Naming Magic Checker (gov_drift/namin... | 导入依赖 / import_depends |
| 110 | D_SECURITY 对抗验证: Scanners (gov_drift/_scanners.py) | → | 孤儿扫描器 / Orphan Scanner (gov_drift/orphan_scanner.py) | 导入依赖 / import_depends |
...
| 113 | D_SECURITY 对抗验证: Scanners (gov_drift/_scanners.py) | → | Symlink检查器 / Symlink Checker (gov_drift/symlink_checke... | 导入依赖 / import_depends |
