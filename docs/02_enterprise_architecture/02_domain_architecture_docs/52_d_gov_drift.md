...
| 跨域入边 | 120 | Cross-domain Incoming | 120 |
...
flowchart TD
    ...
    class D_SHARED,D_GOVERNANCE,D_INTEGRATION,D_GOV_SCRIPTS,D_SECURITY,D_INFRA_RECOVERY,D_COMPLIANCE,D_INFRA_RUNTIME external_prod
```
...
| 2 | Dashboard (gov_drift/dashboard.py) | → | D_GOVERNANCE 生命周期管理: sqlite结构 / sqlite_schema (persistence/sqlite_schema.py) | 导入依赖 / import_depends |
| 3 | Drift Engine (gov_drift/drift_engine.py) | → | D_GOVERNANCE 生命周期管理: sqlite结构 / sqlite_schema (persistence/sqlite_schema.py) | 导入依赖 / import_depends |
...
| 6 | Tamper Proof Audit (gov_drift/tamper_proof_audit.py) | → | D_GOVERNANCE 生命周期管理: sqlite结构 / sqlite_schema (persistence/sqlite_schema.py) | 导入依赖 / import_depends |
...
| 9 | Drift Engine (gov_drift/drift_engine.py) | → | D_GOV_AUDIT 审计追踪: 发现ingest / finding_ingest (gov_audit/finding_ingest.py) | 导入依赖 / import_depends |
| 10 | Drift Engine (gov_drift/drift_engine.py) | → | D_GOV_AUDIT 审计追踪: 发现模型 / finding_model (gov_audit/finding_model.py) | 导入依赖 / import_depends |
| 11 | Truth Source Validator (rule_enforcement/truth_source_val... | → | D_GOV_AUDIT 审计追踪: 写入核心审计链——治本（裁定#18 G7 + 5.37.1） / bridge (g... | 导入依赖 / import_depends |
| 12 | Integrity (governance/integrity.py) | → | D_GOV_AUDIT 审计追踪: audit-trail.merkle每小时 / merkle_hourly (gov_audit/merkl... | 导入依赖 / import_depends |
| 13 | Integrity (governance/integrity.py) | → | D_GOV_AUDIT 审计追踪: 审计事件类型枚举——治本（裁定#18 G2）：转为真 Enu / mode... | 导入依赖 / import_depends |
| 14 | Integrity (governance/integrity.py) | → | D_GOV_AUDIT 审计追踪: 信任桥接 / trust_bridge (gov_audit/trust_bridge.py) | 导入依赖 / import_depends |
| 15 | Tamper Proof Audit (gov_drift/tamper_proof_audit.py) | → | D_GOV_ENFORCEMENT 规则执行: 全项目唯一合法 git commit 入口 / Git Commit Gateway (rule... | 导入依赖 / import_depends |
...
| 23 | 只读：audit_dir / Drift Hotfix Bypass (gov_drift/drift_ho... | → | D_INTEGRATION 管线路由: Protocols (contracts/protocols.py) | 导入依赖 / import_depends |
| 24 | Brain Integration (gov_drift/brain_integration.py) | → | D_SECURITY 对抗验证: Cold Start (gov_drift/cold_start.py) | 导入依赖 / import_depends |
...
| 26 | ``drift_detected`` 触发器恢复入口 / Drift Detector (rule_... | → | D_SECURITY 对抗验证: Reconciler (gov_drift/reconciler.py) | 导入依赖 / import_depends |
| 27 | Self Monitor (gov_audit/self_monitor.py) | → | D_SHARED 共享服务: 注册 datetime/date→sqlite3 str 适配器 / Time Utils (util... | 导入依赖 / import_depends |
| 28 | Absence Manager (gov_drift/absence_manager.py) | → | D_SHARED 共享服务: 序列化/反序列化过程中类型不兼容或格式错误 / Serialization... | 导入依赖 / import_depends |
| 29 | Baseline Poisoning Guard (gov_drift/baseline_poisoning_gu... | → | D_SHARED 共享服务: 返回 Windows 无窗口 creationflags；POSIX 返回 0 / Process... | 导入依赖 / import_depends |
| 30 | Brain Integration (gov_drift/brain_integration.py) | → | D_SHARED 共享服务: 返回 Windows 无窗口 creationflags；POSIX 返回 0 / Process... | 导入依赖 / import_depends |
| 31 | Brain Integration (gov_drift/brain_integration.py) | → | D_SHARED 共享服务: 从当前文件向上查找项目根目录 / Paths (io/paths.py) | 导入依赖 / import_depends |
| 32 | Brain Integration (gov_drift/brain_integration.py) | → | D_SHARED 共享服务: async/sync 边界桥接 / Async Utils (utils/async_utils.py) | 导入依赖 / import_depends |
| 33 | Canary Controller (gov_drift/canary_controller.py) | → | D_SHARED 共享服务: 序列化/反序列化过程中类型不兼容或格式错误 / Serialization... | 导入依赖 / import_depends |
| 34 | Cascade Detector (gov_drift/cascade_detector.py) | → | D_SHARED 共享服务: 序列化/反序列化过程中类型不兼容或格式错误 / Serialization... | 导入依赖 / import_depends |
| 35 | Chaos Injector (gov_drift/chaos_injector.py) | → | D_SHARED 共享服务: 序列化/反序列化过程中类型不兼容或格式错误 / Serialization... | 导入依赖 / import_depends |
| 36 | Chaos Injector (gov_drift/chaos_injector.py) | → | D_SHARED 共享服务: async/sync 边界桥接 / Async Utils (utils/async_utils.py) | 导入依赖 / import_depends |
| 37 | Dashboard (gov_drift/dashboard.py) | → | D_SHARED 共享服务: 从当前文件向上查找项目根目录 / Paths (io/paths.py) | 导入依赖 / import_depends |
...
| 40 | Drift Engine (gov_drift/drift_engine.py) | → | D_SHARED 共享服务: 从当前文件向上查找项目根目录 / Paths (io/paths.py) | 导入依赖 / import_depends |
| 41 | Drift Infrastructure (gov_drift/drift_infrastructure.py) | → | D_SHARED 共享服务: 返回 Windows 无窗口 creationflags；POSIX 返回 0 / Process... | 导入依赖 / import_depends |
| 42 | Drift Infrastructure (gov_drift/drift_infrastructure.py) | → | D_SHARED 共享服务: 从当前文件向上查找项目根目录 / Paths (io/paths.py) | 导入依赖 / import_depends |
| 43 | Drift Models (gov_drift/drift_models.py) | → | D_SHARED 共享服务: 注册 datetime/date→sqlite3 str 适配器 / Time Utils (util... | 导入依赖 / import_depends |
...
| 51 | Headless Scanner (gov_drift/headless_scanner.py) | → | D_SHARED 共享服务: 返回 Windows 无窗口 creationflags；POSIX 返回 0 / Process... | 导入依赖 / import_depends |
...
| 54 | Tamper Proof Audit (gov_drift/tamper_proof_audit.py) | → | D_SHARED 共享服务: 序列化/反序列化过程中类型不兼容或格式错误 / Serialization... | 导入依赖 / import_depends |
...
| 59 | 从 YAML 真源加载契约文件路径 / En 002 Enforcement Validat... | → | D_SHARED 共享服务: Schemas (schema/schemas.py) | 导入依赖 / import_depends |
| 60 | Truth Source Validator (rule_enforcement/truth_source_val... | → | D_SHARED 共享服务: Schemas (schema/schemas.py) | 导入依赖 / import_depends |

### 依赖本域的其他域（入边）/ Depended By

| # | 外部域-源模块 / Source Module | → | 本域模块 / Target Module | 依赖类型 / Type |
|:--:|---------|:--:|---------|---------|
| 1 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Absence Manager (gov_drift/absence_manager.py) | 导入依赖 / import_depends |
| 2 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Ai Construction Detectors (gov_drift/ai_construction_dete... | 导入依赖 / import_depends |
| 3 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Ai Context Injector (gov_drift/ai_context_injector.py) | 导入依赖 / import_depends |
| 4 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Backcompat Checker (gov_drift/backcompat_checker.py) | 导入依赖 / import_depends |
...
| 6 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Baseline Poisoning Guard (gov_drift/baseline_poisoning_gu... | 导入依赖 / import_depends |
| 7 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Canary Controller (gov_drift/canary_controller.py) | 导入依赖 / import_depends |
| 8 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Cascade Detector (gov_drift/cascade_detector.py) | 导入依赖 / import_depends |
| 9 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Chaos Injector (gov_drift/chaos_injector.py) | 导入依赖 / import_depends |
| 10 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Config Consistency (gov_drift/config_consistency.py) | 导入依赖 / import_depends |
...
| 13 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Credibility Engine (gov_drift/credibility_engine.py) | 导入依赖 / import_depends |
...
| 15 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Dashboard (gov_drift/dashboard.py) | 导入依赖 / import_depends |
...
| 17 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Drift Engine (gov_drift/drift_engine.py) | 导入依赖 / import_depends |
...
| 19 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Drift Infrastructure (gov_drift/drift_infrastructure.py) | 导入依赖 / import_depends |
| 20 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Drift Models (gov_drift/drift_models.py) | 导入依赖 / import_depends |
...
| 23 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | File Attr Checker (gov_drift/file_attr_checker.py) | 导入依赖 / import_depends |
...
| 27 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Gitignore Auditor (gov_drift/gitignore_auditor.py) | 导入依赖 / import_depends |
...
| 29 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Headless Scanner (gov_drift/headless_scanner.py) | 导入依赖 / import_depends |
...
| 31 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Naming Magic Checker (gov_drift/naming_magic_checker.py) | 导入依赖 / import_depends |
| 32 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Orphan Scanner (gov_drift/orphan_scanner.py) | 导入依赖 / import_depends |
...
| 34 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Resource Guard (gov_drift/resource_guard.py) | 导入依赖 / import_depends |
...
| 38 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Self Check (gov_drift/self_check.py) | 导入依赖 / import_depends |
...
| 40 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Symlink Checker (gov_drift/symlink_checker.py) | 导入依赖 / import_depends |
| 41 | D_COMPLIANCE 合规: 包入口 / __init__ (behavioral_auditor/__init__.py) | → | Tamper Proof Audit (gov_drift/tamper_proof_audit.py) | 导入依赖 / import_depends |
...
| 44 | D_FEEDBACK_LOOP 反馈循环引擎: 调度器 / scheduler (feedback_loop/scheduler.py) | → | Drift Engine (gov_drift/drift_engine.py) | 导入依赖 / import_depends |
| 45 | D_FEEDBACK_LOOP 反馈循环引擎: 调度器 / scheduler (feedback_loop/scheduler.py) | → | Integrity (governance/integrity.py) | 导入依赖 / import_depends |
| 46 | D_GOVERNANCE 生命周期管理: 治理服务端 / governance_server (mcp/governance_server.py) | → | Drift Engine (gov_drift/drift_engine.py) | 导入依赖 / import_depends |
| 47 | D_GOVERNANCE 生命周期管理: 治理服务端 / governance_server (mcp/governance_server.py) | → | Drift Infrastructure (gov_drift/drift_infrastructure.py) | 导入依赖 / import_depends |
| 48 | D_GOVERNANCE 生命周期管理: 治理服务端 / governance_server (mcp/governance_server.py) | → | Drift Models (gov_drift/drift_models.py) | 导入依赖 / import_depends |
| 49 | D_GOVERNANCE 生命周期管理: Test Reward Hacking Rebound Detector (adversarial/test_re... | → | Reward Hacking Rebound Detector (gov_drift/reward_hacking... | 测试依赖 / test_depends |
| 50 | D_GOVERNANCE 生命周期管理: Test Vigil Runtime (adversarial/test_vigil_runtime.py) | → | 只读：override_expiry / Vigil Runtime (gov_drift/vigil_ru... | 测试依赖 / test_depends |
| 51 | D_GOVERNANCE 生命周期管理: Test Bootstrapping Calibrator (lifecycle/test_bootstrappi... | → | Bootstrapping Calibrator (gov_drift/bootstrapping_calibra... | 测试依赖 / test_depends |
| 52 | D_GOVERNANCE 生命周期管理: Test Silence Detector (resilience/test_silence_detector.py) | → | 只读：last_activity / Silence Detector (gov_drift/silence... | 测试依赖 / test_depends |
| 53 | D_GOVERNANCE 生命周期管理: Test Spiral Ews (resilience/test_spiral_ews.py) | → | Spiral Ews (gov_drift/spiral_ews.py) | 测试依赖 / test_depends |
...
| 55 | D_GOV_AUDIT 审计追踪: 编排器兼容 / _orchestrator_compat (gov_audit/_orchestrato... | → | Self Monitor (gov_audit/self_monitor.py) | 导入依赖 / import_depends |
...
| 57 | D_GOV_AUDIT 审计追踪: 审计漂移桥接 / audit_drift_bridge (bridges/audit_drift_br... | → | Drift Engine (gov_drift/drift_engine.py) | 导入依赖 / import_depends |
| 58 | D_GOV_AUDIT 审计追踪: 审计漂移桥接 / audit_drift_bridge (bridges/audit_drift_br... | → | Drift Models (gov_drift/drift_models.py) | 导入依赖 / import_depends |
| 59 | D_GOV_AUDIT 审计追踪: 命令行 / cli (gov_audit/cli.py) | → | Drift Engine (gov_drift/drift_engine.py) | 导入依赖 / import_depends |
| 60 | D_GOV_AUDIT 审计追踪: 命令行 / cli (gov_audit/cli.py) | → | Integrity (governance/integrity.py) | 导入依赖 / import_depends |
| 61 | D_GOV_AUDIT 审计追踪: —真源优先级裁决器 / Test Truth Source Validator (audit/t... | → | Truth Source Validator (rule_enforcement/truth_source_val... | 测试依赖 / test_depends |
| 62 | D_GOV_CODE_QUALITY 代码质量治理: Test En 002 Enforcement Validator (invariants/test_en_002... | → | 从 YAML 真源加载契约文件路径 / En 002 Enforcement Validat... | 测试依赖 / test_depends |
| 63 | D_GOV_CODE_QUALITY 代码质量治理: Test Breaking Change Detector (rule_enforcement/test_brea... | → | —字段删除/类型变更->CI FAIL / Breaking Change Detector (... | 测试依赖 / test_depends |
| 64 | D_GOV_RULE 规则治理: 门禁引擎模块集 / Gate Engine Package (gate_engine/__init_... | → | —per-gate SLI 报告、误报率、延迟分布、1人+AI运维视图 / G... | config_depends / config_depends |
| 65 | D_GOV_RULE 规则治理: 门禁裁决引擎 / Gate Engine (gate_engine/gate_engine.py) | → | Drift Infrastructure (gov_drift/drift_infrastructure.py) | 导入依赖 / import_depends |
| 66 | D_GOV_RULE 规则治理: 门禁裁决引擎 / Gate Engine (gate_engine/gate_engine.py) | → | 从 YAML 真源加载契约文件路径 / En 002 Enforcement Validat... | 导入依赖 / import_depends |
| 67 | D_GOV_SCRIPTS 脚本治理: P0：frontmatter 解析 / Test Validate Ssot Governance (scr... | → | SSoT 文件头一致性校验器. / Validate Ssot (validators/vali... | 测试依赖 / test_depends |
| 68 | D_GOV_SCRIPTS 脚本治理: P0：frontmatter 解析 / Test Validate Ssot Unit (scripts_g... | → | SSoT 文件头一致性校验器. / Validate Ssot (validators/vali... | 测试依赖 / test_depends |
...
| 70 | D_GOV_SCRIPTS 脚本治理: 在给定路径写入带 frontmatter 的 markdown 文件 / Test Vali... | → | 单条决策记录 / Validate Truth Source Cascade (d11_complia... | 测试依赖 / test_depends |
| 71 | D_INFRA_RUNTIME 运行时集成: State Machine (auto_fix_engine/state_machine.py) | → | Drift Models (gov_drift/drift_models.py) | 导入依赖 / import_depends |
| 72 | D_INFRA_RUNTIME 运行时集成: 只读：sla_buffer / Contract Metrics (system_telemetry/con... | → | 契约漂移检测器 / Contract Drift Detector (gov_drift/contr... | 导入依赖 / import_depends |
| 73 | D_INFRA_RUNTIME 运行时集成: Lifecycle Manager (trading/lifecycle_manager.py) | → | Self Monitor (gov_audit/self_monitor.py) | 导入依赖 / import_depends |
| 74 | D_OPS 反馈循环: —5.133.2 DI 注入契约 / Budget Engine (ops_governance/bud... | → | Drift Infrastructure (gov_drift/drift_infrastructure.py) | 导入依赖 / import_depends |
| 75 | D_OPS 反馈循环: —5.133.2 DI 注入契约 / Budget Engine (ops_governance/bud... | → | Spiral Ews (gov_drift/spiral_ews.py) | 导入依赖 / import_depends |
| 76 | D_ORCHESTRATOR 代理编排器: 触发路由审计日志 duck-typed 接口 / Trigger Router (execut... | → | ``drift_detected`` 触发器恢复入口 / Drift Detector (rule_... | 导入依赖 / import_depends |
| 77 | D_SECURITY 对抗验证: Main (gov_drift/__main__.py) | → | Drift Engine (gov_drift/drift_engine.py) | 导入依赖 / import_depends |
| 78 | D_SECURITY 对抗验证: Main (gov_drift/__main__.py) | → | Drift Infrastructure (gov_drift/drift_infrastructure.py) | 导入依赖 / import_depends |
| 79 | D_SECURITY 对抗验证: Main (gov_drift/__main__.py) | → | Self Check (gov_drift/self_check.py) | 导入依赖 / import_depends |
| 80 | D_SECURITY 对抗验证: Main (gov_drift/__main__.py) | → | 只读：base_dir / Self Test Verifier (gov_drift/self_test_... | 导入依赖 / import_depends |
| 81 | D_SECURITY 对抗验证: Analysis (gov_drift/_analysis.py) | → | 只读：db_path / Correlation Engine (gov_drift/correlation... | 导入依赖 / import_depends |
| 82 | D_SECURITY 对抗验证: Analysis (gov_drift/_analysis.py) | → | Credibility Engine (gov_drift/credibility_engine.py) | 导入依赖 / import_depends |
| 83 | D_SECURITY 对抗验证: Analysis (gov_drift/_analysis.py) | → | 只读：history / Cross Module Score (gov_drift/cross_modul... | 导入依赖 / import_depends |
| 84 | D_SECURITY 对抗验证: Analysis (gov_drift/_analysis.py) | → | 重放baseline历史，重构时间线 / Forensics Engine (gov_drif... | 导入依赖 / import_depends |
| 85 | D_SECURITY 对抗验证: Analysis (gov_drift/_analysis.py) | → | 只读：cache / Git Bisector (gov_drift/git_bisector.py) | 导入依赖 / import_depends |
| 86 | D_SECURITY 对抗验证: Analysis (gov_drift/_analysis.py) | → | 只读：effort_feedback / Roi Engine (gov_drift/roi_engine.py) | 导入依赖 / import_depends |
| 87 | D_SECURITY 对抗验证: Analysis (gov_drift/_analysis.py) | → | 行为漂移->回滚触发. / Rollback Bridge (gov_drift/rollback... | 导入依赖 / import_depends |
| 88 | D_SECURITY 对抗验证: Analysis (gov_drift/_analysis.py) | → | Self Check (gov_drift/self_check.py) | 导入依赖 / import_depends |
| 89 | D_SECURITY 对抗验证: Analysis (gov_drift/_analysis.py) | → | 只读：patterns / Suppression Learner (gov_drift/suppressi... | 导入依赖 / import_depends |
| 90 | D_SECURITY 对抗验证: Analysis (gov_drift/_analysis.py) | → | Tamper Proof Audit (gov_drift/tamper_proof_audit.py) | 导入依赖 / import_depends |
| 91 | D_SECURITY 对抗验证: Analysis (gov_drift/_analysis.py) | → | 只读：archive_dir / Trend Analyzer (gov_drift/trend_analy... | 导入依赖 / import_depends |
| 92 | D_SECURITY 对抗验证: Core (gov_drift/_core.py) | → | Config Consistency (gov_drift/config_consistency.py) | 导入依赖 / import_depends |
| 93 | D_SECURITY 对抗验证: Core (gov_drift/_core.py) | → | Drift Engine (gov_drift/drift_engine.py) | 导入依赖 / import_depends |
| 94 | D_SECURITY 对抗验证: Core (gov_drift/_core.py) | → | Drift Models (gov_drift/drift_models.py) | 导入依赖 / import_depends |
| 95 | D_SECURITY 对抗验证: Drift (gov_drift/_drift.py) | → | 契约漂移检测器 / Contract Drift Detector (gov_drift/contr... | 导入依赖 / import_depends |
| 96 | D_SECURITY 对抗验证: Drift (gov_drift/_drift.py) | → | 只读：audit_dir / Drift Hotfix Bypass (gov_drift/drift_ho... | 导入依赖 / import_depends |
| 97 | D_SECURITY 对抗验证: Drift (gov_drift/_drift.py) | → | Drift Infrastructure (gov_drift/drift_infrastructure.py) | 导入依赖 / import_depends |
| 98 | D_SECURITY 对抗验证: Drift (gov_drift/_drift.py) | → | 语义漂移检测结果 / Drift Result Types (gov_drift/drift_re... | 导入依赖 / import_depends |
| 99 | D_SECURITY 对抗验证: Drift (gov_drift/_drift.py) | → | 从重复漂移事件中提取的可训练模式 / Drift Training (gov_dr... | 导入依赖 / import_depends |
| 100 | D_SECURITY 对抗验证: Infrastructure (gov_drift/_infrastructure.py) | → | Absence Manager (gov_drift/absence_manager.py) | 导入依赖 / import_depends |
| 101 | D_SECURITY 对抗验证: Infrastructure (gov_drift/_infrastructure.py) | → | Ai Context Injector (gov_drift/ai_context_injector.py) | 导入依赖 / import_depends |
| 102 | D_SECURITY 对抗验证: Infrastructure (gov_drift/_infrastructure.py) | → | 只读：baselines_root / Baseline Manager (gov_drift/baseli... | 导入依赖 / import_depends |
| 103 | D_SECURITY 对抗验证: Infrastructure (gov_drift/_infrastructure.py) | → | Canary Controller (gov_drift/canary_controller.py) | 导入依赖 / import_depends |
| 104 | D_SECURITY 对抗验证: Infrastructure (gov_drift/_infrastructure.py) | → | Config Consistency (gov_drift/config_consistency.py) | 导入依赖 / import_depends |
| 105 | D_SECURITY 对抗验证: Infrastructure (gov_drift/_infrastructure.py) | → | Dashboard (gov_drift/dashboard.py) | 导入依赖 / import_depends |
| 106 | D_SECURITY 对抗验证: Infrastructure (gov_drift/_infrastructure.py) | → | 只读：project_root / Gate Persistence (gov_drift/gate_per... | 导入依赖 / import_depends |
| 107 | D_SECURITY 对抗验证: Infrastructure (gov_drift/_infrastructure.py) | → | 构建跨Session交接包 / Handoff Manager (gov_drift/handoff_... | 导入依赖 / import_depends |
| 108 | D_SECURITY 对抗验证: Infrastructure (gov_drift/_infrastructure.py) | → | Resource Guard (gov_drift/resource_guard.py) | 导入依赖 / import_depends |
| 109 | D_SECURITY 对抗验证: Scanners (gov_drift/_scanners.py) | → | 只读：project_root / Incremental Scanner (gov_drift/incre... | 导入依赖 / import_depends |
| 110 | D_SECURITY 对抗验证: Scanners (gov_drift/_scanners.py) | → | Naming Magic Checker (gov_drift/naming_magic_checker.py) | 导入依赖 / import_depends |
| 111 | D_SECURITY 对抗验证: Scanners (gov_drift/_scanners.py) | → | Orphan Scanner (gov_drift/orphan_scanner.py) | 导入依赖 / import_depends |
| 112 | D_SECURITY 对抗验证: Scanners (gov_drift/_scanners.py) | → | Python Compat (gov_drift/python_compat.py) | 导入依赖 / import_depends |
| 113 | D_SECURITY 对抗验证: Scanners (gov_drift/_scanners.py) | → | 只读：lock_dir / Scan Mutex (gov_drift/scan_mutex.py) | 导入依赖 / import_depends |
| 114 | D_SECURITY 对抗验证: Scanners (gov_drift/_scanners.py) | → | Symlink Checker (gov_drift/symlink_checker.py) | 导入依赖 / import_depends |
| 115 | D_SECURITY 对抗验证: Scanners (gov_drift/_scanners.py) | → | 检查测试夹具中硬编码数据结构是否与 ORM/pydantic schema 一... | 导入依赖 / import_depends |
| 116 | D_SECURITY 对抗验证: Cold Start (gov_drift/cold_start.py) | → | Drift Engine (gov_drift/drift_engine.py) | 导入依赖 / import_depends |
| 117 | D_SECURITY 对抗验证: Reconciler (gov_drift/reconciler.py) | → | Drift Models (gov_drift/drift_models.py) | 导入依赖 / import_depends |
| 118 | D_SECURITY 对抗验证: 构造 YAML frontmatter / Runbook Generator (gov_drift/runb... | → | Drift Models (gov_drift/drift_models.py) | 导入依赖 / import_depends |
| 119 | D_SECURITY 对抗验证: State Machine (gov_drift/state_machine.py) | → | Drift Models (gov_drift/drift_models.py) | 导入依赖 / import_depends |
| 120 | D_SECURITY 对抗验证: Drift Bridge (orphan_judge/drift_bridge.py) | → | ``drift_detected`` 触发器恢复入口 / Drift Detector (rule_... | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 15 个外部域直接连接（出边 60 条 + 入边 120 条 = 180 条）。只显示直接连接的域，不展开具体节点。
