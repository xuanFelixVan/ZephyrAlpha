...
flowchart TD
    docs_03_modules_domain_governance_drift_detector_blueprint_md["蓝图<br/>（blueprint.md）<br/>⛔ 漂移检测域，设计已就绪，等待开发排期<br/>文件: drift_detector/blueprint.md<br/>(设计态 / design)"]
    ...
    src_zephyr_gov_drift_baseline_manager_py["基线管理器<br/>治理漂移检测的报告器，汇总数据生成报告<br/>Baseline Manager — baseline_manager.py<br/>文件: gov_drift/baseline_manager.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_drift_git_bisector_py["Git二分器<br/>治理漂移检测的结果，封装操作结果的数据结构<br/>Git Bisector — git_bisector.py<br/>文件: gov_drift/git_bisector.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_drift_roi_engine_py["roi引擎<br/>治理漂移检测的引擎，执行核心逻辑<br/>ROI Engine — roi_engine.py<br/>文件: gov_drift/roi_engine.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_drift_self_test_verifier_py["自测试验证器<br/>治理漂移检测的结果，封装操作结果的数据结构<br/>Self Test Verifier — self_test_verifier.py<br/>文件: gov_drift/self_test_verifier.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_drift_suppression_learner_py["抑制学习器<br/>治理漂移检测的学习器，从数据中学习模式<br/>Suppression Learner — suppression_learner.py<br/>文件: gov_drift/suppression_learner.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_enforcement_rule_enforcement_drift_detector_py["漂移检测器<br/>Gate-side Drift Detector Recovery — zephyr.gov_<br/>enforcement.rule_enforcement.drift_detector<br/>文件: rule_enforcement/drift_detector.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_drift_correlation_engine_py["相关性引擎<br/>治理漂移检测的报告器，汇总数据生成报告<br/>Correlation Engine — correlation_engine.py<br/>文件: gov_drift/correlation_engine.py<br/>(生产态 / production)"]
    src_zephyr_gov_drift_credibility_engine_py["credibility引擎<br/>治理漂移检测的引擎，执行核心逻辑<br/>Credibility Engine — credibility_engine.py<br/>文件: gov_drift/credibility_engine.py<br/>(生产态 / production)"]
...
| 20 | 校验ssot / validate_ssot (validators/validate_ssot.py) | → | D_GOV_SCRIPTS 脚本治理: 编码安全工具 / encoding (_shared/encoding.py) | 导入依赖 / import_depends |
...
| 22 | 校验ssot / validate_ssot (validators/validate_ssot.py) | → | D_GOV_SCRIPTS 脚本治理: 目录遍历工具 / yaml_utils (_shared/yaml_utils.py) | 导入依赖 / import_depends |
