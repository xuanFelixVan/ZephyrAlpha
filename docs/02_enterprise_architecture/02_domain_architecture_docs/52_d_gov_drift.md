...
flowchart TD
    ...
    src_zephyr_gov_drift_ai_context_injector_py["ai上下文injector<br/>注入点：session_manager派发task时 + MCP<br/>discover_applicable_gates<br/>ai_context_injector<br/>文件: gov_drift/ai_context_injector.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_drift_brain_integration_py["brain集成<br/>漂移治理的brain集成。ProbeHierarchy - K8s<br/>3-Probe + Terraform<br/>Reconciliation，brain集成。ProbeHierarchy - K8s<br/>3-Probe + Terraform<br/>Reconciliation相关功能的实现。<br/>文件: gov_drift/brain_integration.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_drift_contract_drift_detector_py["契约漂移检测器<br/>从 MOD-INF-015 contract_metrics.py 迁移而来<br/>（v3.0.0 委托决策）<br/>contract_drift_detector<br/>文件: gov_drift/contract_drift_detector.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_drift_detector_core_init_py["gov_drift/detector_core 包入口<br/>漂移治理的包入口。MOD-INF-023 drift_detector<br/>core module.，包入口。MOD-INF-023<br/>drift_detector core module.相关功能的实现。<br/>文件: detector_core/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_drift_headless_scanner_py["headless扫描器<br/>漂移治理的LIGHT+DEEP 与会话日志<br/>_interrupt_log.jsonl 扫描。，LIGHT+DEEP<br/>与会话日志 _interrupt_log.jsonl<br/>扫描。相关功能的实现。<br/>Headless Scanner — headless_scanner.py<br/>文件: gov_drift/headless_scanner.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_drift_reward_hacking_rebound_detector_py["rewardhackingrebound检测器<br/>漂移治理的奖励hackingrebound检测器。Reward<br/>Hacking Rebound Detector — v0.14.0<br/>§2.37-D.，奖励hackingrebound检测器。Reward<br/>Hacking Rebound Detector — v0.14.0<br/>§2.37-D.相关功能的实现。<br/>文件: gov_drift<br/>/reward_hacking_rebound_detector.py<br/>(生产态 / production)"]
    ...
    class D_SECURITY,D_SHARED,D_GOV_SCRIPTS,D_GOV_AUDIT,D_INFRA_RECOVERY,D_COMPLIANCE,D_FEEDBACK_LOOP external_prod
```
