...
flowchart TD
    ...
    src_zephyr_ex_sor_core_algo_execution_selector_py["(设计态 / design) 算法执行选择器 / algo_execution_selector<br/>算法执行选择器，core的功能模块。<br/>文件: core/algo_execution_selector.py"]
    src_zephyr_ex_sor_core_broker_adapter_manager_py["(设计态 / design) 经纪人适配器管理器 / broker_adapter_manager<br/>经纪人适配器管理器，core的适配器，把外部接口适配成内部统一格式。<br/>文件: core/broker_adapter_manager.py"]
    src_zephyr_ex_sor_core_optimal_order_router_py["(设计态 / design) optimal订单路由器 / optimal_order_router<br/>optimal订单路由器，core的路由器，按规则把请求分发到对应处理方。<br/>文件: core/optimal_order_router.py"]
    ...
    src_zephyr_ex_sor_services_execution_quality_scorer_py["(设计态 / design) 执行质量评分器 / execution_quality_scorer<br/>执行质量评分器，services的功能模块。<br/>文件: services/execution_quality_scorer.py"]
    src_zephyr_ex_sor_services_slippage_analyzer_py["(设计态 / design) 滑点分析器 / slippage_analyzer<br/>滑点分析器，services的分析器，分析数据找出问题或规律。<br/>文件: services/slippage_analyzer.py"]
    src_zephyr_ex_sor_services_transaction_cost_optimizer_py["(设计态 / design) 交易成本优化器 / transaction_cost_optimizer<br/>交易成本优化器，services的功能模块。<br/>文件: services/transaction_cost_optimizer.py"]
    ...
    src_zephyr_ex_sor_core_execution_scheduler_py["(设计态 / design) 执行调度器 / execution_scheduler<br/>执行调度器，core的调度器，按时间或优先级安排任务执行。<br/>文件: core/execution_scheduler.py"]
    src_zephyr_ex_sor_core_algo_trading_engine_py["(设计态 / design) 算法交易引擎 / algo_trading_engine<br/>算法交易引擎，core的引擎，执行核心逻辑的处理引擎。<br/>文件: core/algo_trading_engine.py"]
    src_zephyr_ex_sor_core_broker_api_connector_py["(设计态 / design) 经纪人APIconnector / broker_api_connector<br/>经纪人APIconnector，core的功能模块。<br/>文件: core/broker_api_connector.py"]
    src_zephyr_ex_sor_core_api_rate_limiter_py["(设计态 / design) API率限制器 / api_rate_limiter<br/>API率限制器，core的功能模块。<br/>文件: core/api_rate_limiter.py"]
...
| 5 | 包入口 / __init__ (core/__init__.py) | → | D_MKT_DATA 行情数据: failover/ | runtime / runtime |
