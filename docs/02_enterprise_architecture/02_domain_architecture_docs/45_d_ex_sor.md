...
> 展示全部 18 个模块（生产态 17 + 设计态 1），含跨域依赖外部节点。节点含成熟度+名称+大白话/简介+文件路径。
...
flowchart TD
    ...
    src_zephyr_ex_sor_core_algo_execution_selector_py ~~~ src_zephyr_ex_sor_core_market_context_provider_py
    src_zephyr_ex_sor_core_market_context_provider_py ~~~ src_zephyr_ex_sor_infrastructure_init_py
    ...
    D_EX_CORE -->|导入依赖 / import_depends| src_zephyr_ex_sor_core_algo_trading_engine_py
    ...
    class src_zephyr_ex_sor_core_market_context_provider_py design
...
|:--:|---------|:--:|---------|---------|
| 1 | D_EX_CORE 执行核心: 执行引擎 / D_EXECUTION_CORE — Execution Engine (ex_core/... | → | 算法交易引擎 / algo_trading_engine (core/algo_trading_eng... | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 3 个外部域直接连接（出边 24 条 + 入边 1 条 = 25 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_EX_CORE -->|1条 导入依赖 / import_depends| D_EX_SOR
```
