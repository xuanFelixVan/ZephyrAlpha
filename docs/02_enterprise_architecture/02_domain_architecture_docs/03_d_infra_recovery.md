...
graph TD
    ...
    class D_GOV_AUDIT,D_SECURITY,D_GOV_OPS_RESILIENCE,D_OPS,D_INTEGRATION,D_GOV_RULE external_prod
    class D_SHARED,D_AUDITTEST,D_GOV_DRIFT external_design
```
...
graph TD
    ...
    class D_GOV_AUDIT,D_SECURITY,D_GOV_RULE,D_INTEGRATION,D_GOVERNANCE,D_FEEDBACK_LOOP external_prod
    class D_SHARED,D_GOV_DRIFT,D_AUDITTEST external_design
```
...
|:--:|---------|:--:|---------|---------|
| 1 | SqliteDumper — SQLite 双轨 Checkpoint 的 DB 层... | → | D_GOVERNANCE 生命周期管理: SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 2 | G-CT-004 契约：Rollback -> Audit 记录回滚操作. ... | → | D_GOV_AUDIT 审计追踪: contracts.py | 导入依赖 / import_depends |
| 3 | G-CT-002 Rollback 消费端 — on_audit_anomaly() ... | → | D_GOV_AUDIT 审计追踪: anomaly.py | 导入依赖 / import_depends |
| 4 | RollbackAbuseDetector — 回滚滥用检测。 (rollba... | → | D_GOV_AUDIT 审计追踪: query.py | 导入依赖 / import_depends |
| 5 | RollbackAuditNexus — 回滚审计记录聚合到 Nexus ... | → | D_GOV_AUDIT 审计追踪: writer.py | 导入依赖 / import_depends |
| 6 | RollbackExecutor — 回滚执行器核心封装。 (rollb... | → | D_GOV_AUDIT 审计追踪: writer.py | 导入依赖 / import_depends |
...
| 56 | D_GOVERNANCE 生命周期管理: GovernanceServer: 治理域统一MCP入口 (governance... | → | RollbackExecutor — 回滚执行器核心封装。 (rollb... | 导入依赖 / import_depends |
| 57 | D_GOV_DRIFT 漂移检测: Gate-side Drift Detector Recovery — zephyr.gov... | → | drift_fix.py | 导入依赖 / import_depends |
| 58 | D_GOV_RULE 规则治理: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | → | CT-RBK-GATE-001 集成契约落地——Rollback System... | 导入依赖 / import_depends |
| 59 | D_INFRA_RUNTIME 运行时集成: boot_hooks.py | → | RollbackBootIntegration — 回滚系统自动启动/关.... | 导入依赖 / import_depends |
| 60 | D_INTEGRATION 管线路由: PipelineOrchestrator — M1-M11 管线协调器 (pipe... | → | CT-RBK-GATE-001 集成契约落地——Rollback System... | 导入依赖 / import_depends |
| 61 | D_OPS 反馈循环: budget_tracker.py | → | G-CT-009 契约：Rollback -> Budget 回滚成本计入.... | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 12 个外部域直接连接（出边 26 条 + 入边 61 条 = 87 条）。只显示直接连接的域，不展开具体节点。
