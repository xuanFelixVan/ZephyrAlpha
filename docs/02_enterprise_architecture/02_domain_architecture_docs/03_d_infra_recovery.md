...
graph TD
    ...
    class D_ORCHESTRATOR,D_GOV_CODE_QUALITY,D_GOV_DRIFT,D_FBL_VERIFICATION,D_SECURITY,D_GOV_AUDIT,D_FEEDBACK_LOOP,D_GOVERNANCE external_prod
...
graph TD
    ...
    D_GOVERNANCE["(生产态 / production) D_GOVERNANCE"]
    tests_chaos_test_chaos_engineering_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    D_ORCHESTRATOR["(生产态 / production) D_ORCHESTRATOR"]
    ...
    class D_GOVERNANCE,D_ORCHESTRATOR,D_GOV_DRIFT,D_POSITION,D_FBL_VERIFICATION,D_SECURITY external_prod
```
...
graph TD
    ...
    class D_GOV_AUDIT,D_SHARED,D_GOV_OPS_RESILIENCE,D_ORCHESTRATOR,D_GOV_CODE_QUALITY,D_GOV_DRIFT,D_FBL_VERIFICATION,D_SECURITY,D_GOVERNANCE,D_POSITION,D_OPS,D_INFRA_RUNTIME external_prod
```
...
| 3 | SqliteDumper — SQLite 双轨 Checkpoint 的 DB 层... | → | D_GOVERNANCE 生命周期管理: SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 4 | test_chaos_engineering.py | → | D_GOVERNANCE 生命周期管理: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
