...
> 展示全部 54 个模块（生产态 48 + 设计态 0 + 原型态 6），标签标注成熟度。
...
graph TD
    ...
    src_zephyr_infrastructure_rollback_auditor_py -->|导入依赖 / import_depends| D_GOVERNANCE
    src_zephyr_infrastructure_rollback_drift_fix_py -->|导入依赖 / import_depends| D_GOVERNANCE
    src_zephyr_infrastructure_rollback_contracts_py -.->|导入依赖 / import_depends| D_GOVERNANCE
    ...
    src_zephyr_infrastructure_rollback_rollback_audit_nexus_py -->|导入依赖 / import_depends| D_GOVERNANCE
    src_zephyr_infrastructure_rollback_rollback_abuse_detector_py -->|导入依赖 / import_depends| D_GOVERNANCE
    ...
    D_GOV_OPS_RESILIENCE -.->|导入依赖 / import_depends| src_zephyr_infrastructure_rollback_budget_tracker_py
    D_GOV_ENFORCEMENT["(原型态 / prototype) D_GOV_ENFORCEMENT"]
    D_GOV_ENFORCEMENT -.->|导入依赖 / import_depends| src_zephyr_infrastructure_rollback_drift_fix_py
    D_GOV_ENFORCEMENT -->|导入依赖 / import_depends| src_zephyr_infrastructure_rollback_contract_py
    ...
    D_AUDITTEST -.->|测试依赖 / test_depends| src_zephyr_infrastructure_rollback_agent_cooldown_py
    D_AUDITTEST -.->|测试依赖 / test_depends| src_zephyr_infrastructure_rollback_auto_rollback_trigger_py
    D_AUDITTEST -.->|测试依赖 / test_depends| src_zephyr_infrastructure_rollback_kill_switch_py
    ...
    D_AUDITTEST -.->|测试依赖 / test_depends| src_zephyr_infrastructure_rollback_git_infra_snapshot_py
    D_AUDITTEST -.->|测试依赖 / test_depends| src_zephyr_infrastructure_rollback_credential_rotation_trigger_py
    D_AUDITTEST -.->|测试依赖 / test_depends| src_zephyr_infrastructure_rollback_hallucination_guard_py
    D_AUDITTEST -.->|测试依赖 / test_depends| src_zephyr_infrastructure_rollback_auditor_py
    ...
    class D_GOVERNANCE,D_GOV_OPS_RESILIENCE,D_INTEGRATION,D_TRADING external_prod
...
graph TD
    ...
    src_zephyr_infrastructure_rollback_rollback_integration_py -.->|导入依赖 / import_depends| D_SHARED
    D_GOVERNANCE["(生产态 / production) D_GOVERNANCE"]
    src_zephyr_infrastructure_rollback_rollback_executor_py -->|导入依赖 / import_depends| D_GOVERNANCE
    ...
    D_AUDITTEST["(原型态 / prototype) D_AUDITTEST"]
    D_AUDITTEST -.->|测试依赖 / test_depends| src_zephyr_infrastructure_rollback_rollback_wal_py
    ...
    D_INTEGRATION_GATEWAY -->|导入依赖 / import_depends| src_zephyr_infrastructure_rollback_rollback_executor_py
    ...
    D_AUDITTEST -.->|测试依赖 / test_depends| src_zephyr_infrastructure_rollback_rollback_lock_py
    D_AUDITTEST -.->|测试依赖 / test_depends| src_zephyr_infrastructure_rollback_rollback_lock_py
    D_AUDITTEST -.->|测试依赖 / test_depends| src_zephyr_infrastructure_rollback_rollback_target_staleness_py
    classDef production fill:#e1f5fe,stroke:#01579b,stroke-width:2px,color:#000
    classDef design fill:#fff3e0,stroke:#e65100,stroke-width:2px,color:#000,stroke-dasharray: 5 5
    classDef external_prod fill:#e8f5e9,stroke:#1b5e20,stroke-width:1px,color:#000
    classDef external_design fill:#fce4ec,stroke:#880e4f,stroke-width:1px,color:#000,stroke-dasharray: 5 5
    ...
    class D_GOVERNANCE,D_FEEDBACK_LOOP,D_INTEGRATION_GATEWAY external_prod
...
graph TD
    ...
    class D_GOVERNANCE,D_INTEGRATION,D_INTEGRATION_GATEWAY,D_FEEDBACK_LOOP external_prod
    class D_SHARED,D_GOV_ENFORCEMENT,D_AUDITTEST external_design
```
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 6 个，4 条域内依赖）。
...
graph TD
    ...
    classDef external_design fill:#fce4ec,stroke:#880e4f,stroke-width:1px,color:#000,stroke-dasharray: 5 5
    class src_zephyr_infrastructure_rollback_init_py,src_zephyr_infrastructure_rollback_manifest_py,src_zephyr_infrastructure_rollback_budget_tracker_py,src_zephyr_infrastructure_rollback_complexity_budget_py,src_zephyr_infrastructure_rollback_contracts_py,src_zephyr_infrastructure_rollback_rollback_boot_integration_py design
    class D_GOVERNANCE,D_SHARED,D_GOV_OPS_RESILIENCE,D_TRADING external_prod
```
...
|:--:|---------|:--:|---------|---------|
| 1 | G-CT-004 契约：Rollback -> Audit 记录回滚操作. ... | → | D_GOVERNANCE 生命周期管理: contracts.py | 导入依赖 / import_depends |
| 2 | G-CT-002 Rollback 消费端 — on_audit_anomaly() ... | → | D_GOVERNANCE 生命周期管理: anomaly.py | 导入依赖 / import_depends |
| 3 | drift_fix.py | → | D_GOVERNANCE 生命周期管理: G-CT-005 — ManagedDriftEvent Pydantic V2 BaseM... | 导入依赖 / import_depends |
| 4 | RollbackAbuseDetector — 回滚滥用检测。 (rollba... | → | D_GOVERNANCE 生命周期管理: query.py | 导入依赖 / import_depends |
| 5 | RollbackAuditNexus — 回滚审计记录聚合到 Nexus ... | → | D_GOVERNANCE 生命周期管理: writer.py | 导入依赖 / import_depends |
| 6 | RollbackExecutor — 回滚执行器核心封装。 (rollb... | → | D_GOVERNANCE 生命周期管理: writer.py | 导入依赖 / import_depends |
| 7 | SqliteDumper — SQLite 双轨 Checkpoint 的 DB 层... | → | D_GOVERNANCE 生命周期管理: SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 8 | RollbackBootIntegration — 回滚系统自动启动/关.... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: EventHook — 声明式任务系统事件订阅 (event_hook.py) | 导入依赖 / import_depends |
...
| 21 | RollbackVerifier — 回滚后验证器。 (rollback_ve... | → | D_SHARED 共享服务: SQLite 连接工厂真源（SSoT） (sqlite_factory.py) | 导入依赖 / import_depends |
| 22 | S3 Snapshot Lifecycle Manager — 快照防生命周期... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 23 | SqliteDumper — SQLite 双轨 Checkpoint 的 DB 层... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 24 | SqliteDumper — SQLite 双轨 Checkpoint 的 DB 层... | → | D_SHARED 共享服务: SQLite 连接工厂真源（SSoT） (sqlite_factory.py) | 导入依赖 / import_depends |
| 25 | SqliteDumper — SQLite 双轨 Checkpoint 的 DB 层... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 26 | WarmStandby — 温备热切（git worktree 副本维护... | → | D_SHARED 共享服务: serialization.py —— 统一序列化/反序列化基础设... | 导入依赖 / import_depends |

### 依赖本域的其他域（入边）/ Depended By

| # | 外部域-源模块 / Source Module | → | 本域模块 / Target Module | 依赖类型 / Type |
|:--:|---------|:--:|---------|---------|
| 1 | D_AUDITTEST 审计测试套件: test_agent_cooldown.py | → | AgentCooldown — Agent 冷却隔离器。 (agent_cool... | 测试依赖 / test_depends |
| 2 | D_AUDITTEST 审计测试套件: test_auto_rollback_trigger.py | → | AutoRollbackTrigger — 自动回滚触发器。 (auto_r... | 测试依赖 / test_depends |
| 3 | D_AUDITTEST 审计测试套件: test_ce_kill_switch.py | → | KillSwitchManager — 三级 Kill Switch 管理器。 ... | 测试依赖 / test_depends |
| 4 | D_AUDITTEST 审计测试套件: test_cross_platform_shell.py | → | CrossPlatformShell — 跨平台 Shell 脚本双输出。... | 测试依赖 / test_depends |
| 5 | D_AUDITTEST 审计测试套件: test_drift_fix.py | → | drift_fix.py | 测试依赖 / test_depends |
| 6 | D_AUDITTEST 审计测试套件: test_external_merkle_proof.py | → | External Merkle Proof — 外部可验证回滚完整性证... | 测试依赖 / test_depends |
| 7 | D_AUDITTEST 审计测试套件: test_git_infra_snapshot.py | → | GitInfraSnapshot — Git 基础设施快照与污染防护... | 测试依赖 / test_depends |
| 8 | D_AUDITTEST 审计测试套件: test_credential_rotation_trigger.py | → | CredentialRotationTrigger — 凭据自动轮替。 (cr... | 测试依赖 / test_depends |
| 9 | D_AUDITTEST 审计测试套件: test_secret_rotation_aware.py | → | SecretRotationAware — 密钥轮替感知器。 (secret... | 测试依赖 / test_depends |
| 10 | D_AUDITTEST 审计测试套件: test_hallucination_guard.py | → | HallucinationGuard — AI 幻觉防护：回滚后强制状... | 测试依赖 / test_depends |
| 11 | D_AUDITTEST 审计测试套件: test_auditor.py | → | G-CT-004 契约：Rollback -> Audit 记录回滚操作. ... | 测试依赖 / test_depends |
| 12 | D_AUDITTEST 审计测试套件: test_forensic.py | → | Forensic Engine — 取证基础设施（Phase 8 完整实... | 测试依赖 / test_depends |
| 13 | D_AUDITTEST 审计测试套件: test_governance_auditor.py | → | G-CT-004 契约：Rollback -> Audit 记录回滚操作. ... | 测试依赖 / test_depends |
| 14 | D_AUDITTEST 审计测试套件: test_right_to_be_forgotten.py | → | Right to be Forgotten — GDPR 遗忘权合规检查器... | 测试依赖 / test_depends |
| 15 | D_AUDITTEST 审计测试套件: test_s3_snapshot_lifecycle.py | → | S3 Snapshot Lifecycle Manager — 快照防生命周期... | 测试依赖 / test_depends |
| 16 | D_AUDITTEST 审计测试套件: test_sqlite_dumper.py | → | SqliteDumper — SQLite 双轨 Checkpoint 的 DB 层... | 测试依赖 / test_depends |
| 17 | D_AUDITTEST 审计测试套件: test_contract.py | → | CT-RBK-GATE-001 集成契约落地——Rollback System... | 测试依赖 / test_depends |
| 18 | D_AUDITTEST 审计测试套件: test_submodule_sync.py | → | Submodule Sync — Submodule/Monorepo 多仓库同步... | 测试依赖 / test_depends |
| 19 | D_AUDITTEST 审计测试套件: test_checkpoint_gc.py | → | CheckpointGC — Checkpoint 垃圾回收。 (checkpoi... | 测试依赖 / test_depends |
| 20 | D_AUDITTEST 审计测试套件: test_venv_sync.py | → | VenvSync — venv/conda 版本同步保障。 (venv_syn... | 测试依赖 / test_depends |
| 21 | D_AUDITTEST 审计测试套件: test_env_watcher.py | → | EnvWatcher — 环境变量热重载监控器。 (env_watch... | 测试依赖 / test_depends |
| 22 | D_AUDITTEST 审计测试套件: test_runbook_generator.py | → | RunbookGenerator — 回滚操作 Runbook 自动生成。... | 测试依赖 / test_depends |
| 23 | D_AUDITTEST 审计测试套件: test_knowngoodstate_ledger.py | → | KnowngoodstateLedger — 已验证正确状态收据。 (k... | 测试依赖 / test_depends |
| 24 | D_AUDITTEST 审计测试套件: test_warm_standby.py | → | WarmStandby — 温备热切（git worktree 副本维护... | 测试依赖 / test_depends |
| 25 | D_AUDITTEST 审计测试套件: test_vulnerability_rescanner.py | → | VulnerabilityRescanner — 依赖漏洞复扫。 (vulne... | 测试依赖 / test_depends |
| 26 | D_AUDITTEST 审计测试套件: test_commit_quality_gate.py | → | CommitQualityGate — Commit 质量基础设施。 (com... | 测试依赖 / test_depends |
| 27 | D_AUDITTEST 审计测试套件: test_forward_fix_runner.py | → | ForwardFixRunner — Forward-Fix 执行器。 (forwa... | 测试依赖 / test_depends |
| 28 | D_AUDITTEST 审计测试套件: test_topology_change_log.py | → | TopologyChangeLog — 分支拓扑变更日志。 (topolo... | 测试依赖 / test_depends |
| 29 | D_AUDITTEST 审计测试套件: test_intent_archiver.py | → | IntentArchiver — 意图存档保护。 (intent_archiv... | 测试依赖 / test_depends |
| 30 | D_AUDITTEST 审计测试套件: test_rollback_abuse_detector.py | → | RollbackAbuseDetector — 回滚滥用检测。 (rollba... | 测试依赖 / test_depends |
| 31 | D_AUDITTEST 审计测试套件: test_rollback_audit_nexus.py | → | RollbackAuditNexus — 回滚审计记录聚合到 Nexus ... | 测试依赖 / test_depends |
| 32 | D_AUDITTEST 审计测试套件: test_rollback_bootstrap.py | → | RollbackBootstrap — 零依赖自举回滚器。 (rollba... | 测试依赖 / test_depends |
| 33 | D_AUDITTEST 审计测试套件: test_rollback_budget.py | → | RollbackBudget — 回滚预算管理器。 (rollback_bu... | 测试依赖 / test_depends |
| 34 | D_AUDITTEST 审计测试套件: Extreme tests for concurrent rollback (MOD-INF-... | → | RollbackExecutor — 回滚执行器核心封装。 (rollb... | 测试依赖 / test_depends |
| 35 | D_AUDITTEST 审计测试套件: Extreme tests for concurrent rollback (MOD-INF-... | → | RollbackLock — 全局回滚锁管理。 (rollback_lock.py) | 测试依赖 / test_depends |
| 36 | D_AUDITTEST 审计测试套件: test_rollback_context_restorer.py | → | RollbackContextRestorer — 上下文恢复器。 (roll... | 测试依赖 / test_depends |
| 37 | D_AUDITTEST 审计测试套件: test_rollback_dashboard.py | → | RollbackDashboard — 回滚仪表盘（零依赖 Markdow... | 测试依赖 / test_depends |
| 38 | D_AUDITTEST 审计测试套件: test_rollback_drill.py | → | RollbackDrill — 定期回滚演练调度器 (DiRT-style... | 测试依赖 / test_depends |
| 39 | D_AUDITTEST 审计测试套件: test_rollback_executor_root.py | → | RollbackExecutor — 回滚执行器核心封装。 (rollb... | 测试依赖 / test_depends |
| 40 | D_AUDITTEST 审计测试套件: test_rollback_integration.py | → | Rollback Integration — executor 集成增强层。 (... | 测试依赖 / test_depends |
| 41 | D_AUDITTEST 审计测试套件: test_rollback_lock.py | → | RollbackLock — 全局回滚锁管理。 (rollback_lock.py) | 测试依赖 / test_depends |
| 42 | D_AUDITTEST 审计测试套件: test_rollback_loop_detector.py | → | RollbackLoopDetector — 回滚循环检测器。 (rollb... | 测试依赖 / test_depends |
| 43 | D_AUDITTEST 审计测试套件: Extreme tests for partial_revert (MOD-INF-021 B... | → | RollbackExecutor — 回滚执行器核心封装。 (rollb... | 测试依赖 / test_depends |
| 44 | D_AUDITTEST 审计测试套件: Extreme tests for partial_revert (MOD-INF-021 B... | → | RollbackLock — 全局回滚锁管理。 (rollback_lock.py) | 测试依赖 / test_depends |
| 45 | D_AUDITTEST 审计测试套件: test_rollback_simulator.py | → | RollbackSimulator — 回滚模拟器（CI 集成）。 (r... | 测试依赖 / test_depends |
| 46 | D_AUDITTEST 审计测试套件: test_rollback_state_machine.py | → | RollbackStateMachine — 回滚步骤级状态机。 (rol... | 测试依赖 / test_depends |
| 47 | D_AUDITTEST 审计测试套件: test_rollback_target_staleness.py | → | RollbackTargetStaleness — 回滚目标陈旧度检测。... | 测试依赖 / test_depends |
| 48 | D_AUDITTEST 审计测试套件: test_rollback_verifier_root.py | → | RollbackVerifier — 回滚后验证器。 (rollback_ve... | 测试依赖 / test_depends |
| 49 | D_AUDITTEST 审计测试套件: test_rollback_wal.py | → | RollbackWAL — 回滚预写日志。 (rollback_wal.py) | 测试依赖 / test_depends |
| 50 | D_AUDITTEST 审计测试套件: test_semantic_rollback_tag.py | → | SemanticRollbackTag — 语义化 Rollback Tag 管理... | 测试依赖 / test_depends |
| 51 | D_AUDITTEST 审计测试套件: test_semantic_similar_detector.py | → | SemanticSimilarDetector — 语义变形攻击检测。 (... | 测试依赖 / test_depends |
| 52 | D_AUDITTEST 审计测试套件: test_temporal_context_adapter.py | → | TemporalContextAdapter — AI 时间上下文断裂修复... | 测试依赖 / test_depends |
| 53 | D_FEEDBACK_LOOP 反馈循环引擎: scheduler_act.py | → | RollbackExecutor — 回滚执行器核心封装。 (rollb... | 导入依赖 / import_depends |
| 54 | D_GOVERNANCE 生命周期管理: Rollback System CLI — MOD-INF-021 v0.10.0 Git-... | → | RollbackExecutor — 回滚执行器核心封装。 (rollb... | 导入依赖 / import_depends |
| 55 | D_GOVERNANCE 生命周期管理: Rollback System CLI — MOD-INF-021 v0.10.0 Git-... | → | RollbackVerifier — 回滚后验证器。 (rollback_ve... | 导入依赖 / import_depends |
| 56 | D_GOV_ENFORCEMENT 规则执行: Gate-side Drift Detector Recovery — zephyr.gov... | → | drift_fix.py | 导入依赖 / import_depends |
| 57 | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | → | CT-RBK-GATE-001 集成契约落地——Rollback System... | 导入依赖 / import_depends |
| 58 | D_GOV_OPS_RESILIENCE 运维弹性治理: budget_tracker.py | → | G-CT-009 契约：Rollback -> Budget 回滚成本计入.... | 导入依赖 / import_depends |
| 59 | D_INTEGRATION 管线路由: PipelineOrchestrator — M1-M11 管线协调器 (pipe... | → | CT-RBK-GATE-001 集成契约落地——Rollback System... | 导入依赖 / import_depends |
| 60 | D_INTEGRATION_GATEWAY 集成网关: GovernanceServer: 治理域统一MCP入口 (governance... | → | RollbackExecutor — 回滚执行器核心封装。 (rollb... | 导入依赖 / import_depends |
| 61 | D_TRADING 交易运营: boot_hooks.py | → | RollbackBootIntegration — 回滚系统自动启动/关.... | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 9 个外部域直接连接（出边 26 条 + 入边 61 条 = 87 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_INFRA_RECOVERY -->|7条 导入依赖 / import_depends| D_GOVERNANCE
