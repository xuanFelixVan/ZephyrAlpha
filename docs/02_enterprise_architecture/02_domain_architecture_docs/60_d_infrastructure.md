...
graph TD
    ...
    D_GOV_AUDIT -.->|导入依赖 / import_depends| src_zephyr_shared_contracts_capital_allocation_result_py
    D_GOV_AUDIT -.->|导入依赖 / import_depends| src_zephyr_shared_contracts_execution_report_py
    D_GOV_AUDIT -.->|导入依赖 / import_depends| src_zephyr_shared_contracts_fill_py
    D_GOV_AUDIT -.->|导入依赖 / import_depends| src_zephyr_shared_contracts_model_serving_request_py
...
graph TD
    ...
    class D_SHARED,D_GOV_DRIFT,D_FBL_VERIFICATION,D_GOVERNANCE,D_FEEDBACK_LOOP,D_GOV_CODE_QUALITY,D_GOV_OPS_RESILIENCE,D_INFRA_RUNTIME,D_AUTONOMY_CORE,D_SECURITY,D_INTEGRATION,D_ORCHESTRATOR,D_TRADING external_prod
```
...
graph TD
    ...
    class D_SHARED,D_EX_CORE,D_RISK,D_SIMULATION,D_INFRA_RUNTIME,D_TRADING external_prod
...
graph TD
    ...
    D_GOV_AUDIT -.->|导入依赖 / import_depends| src_zephyr_shared_contracts_fill_py
    D_GOV_AUDIT -.->|导入依赖 / import_depends| src_zephyr_shared_contracts_model_serving_request_py
    D_GOV_AUDIT -.->|导入依赖 / import_depends| src_zephyr_shared_contracts_order_py
    D_GOV_AUDIT -.->|导入依赖 / import_depends| src_zephyr_shared_contracts_position_py
    classDef production fill:#e1f5fe,stroke:#01579b,stroke-width:2px,color:#000
    classDef design fill:#fff3e0,stroke:#e65100,stroke-width:2px,color:#000,stroke-dasharray: 5 5
    classDef external_prod fill:#e8f5e9,stroke:#1b5e20,stroke-width:1px,color:#000
    classDef external_design fill:#fce4ec,stroke:#880e4f,stroke-width:1px,color:#000,stroke-dasharray: 5 5
...
| 2 | test_config_complexity_budget.py | → | D_FBL_VERIFICATION 反馈验证: Config Complexity Budget — v0.16.0 R227 (confi... | 测试依赖 / test_depends |
| 3 | test_config_governance.py | → | D_FBL_VERIFICATION 反馈验证: Config Governance — v0.3.0 R8 (config_governan... | 测试依赖 / test_depends |
| 4 | test_config_hot_reload_guard.py | → | D_FEEDBACK_LOOP 反馈循环引擎: Config Hot-Reload Guard — v0.40.0 R498 (config... | 测试依赖 / test_depends |
| 5 | test_api_version_contract.py | → | D_FEEDBACK_LOOP 反馈循环引擎: API Version Contract — v0.14.0 R188 (api_versi... | 测试依赖 / test_depends |
| 6 | test_config_drift.py | → | D_GOVERNANCE 生命周期管理: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
...
| 18 | D_GOV_AUDIT 审计追踪: zephyr.trading.trading_contracts — trading-dom... | → | capital_allocation_result.py | 导入依赖 / import_depends |
| 19 | D_GOV_AUDIT 审计追踪: zephyr.trading.trading_contracts — trading-dom... | → | execution_report.py | 导入依赖 / import_depends |
| 20 | D_GOV_AUDIT 审计追踪: zephyr.trading.trading_contracts — trading-dom... | → | fill.py | 导入依赖 / import_depends |
| 21 | D_GOV_AUDIT 审计追踪: zephyr.trading.trading_contracts — trading-dom... | → | model_serving_request.py | 导入依赖 / import_depends |
| 22 | D_GOV_AUDIT 审计追踪: zephyr.trading.trading_contracts — trading-dom... | → | order.py | 导入依赖 / import_depends |
| 23 | D_GOV_AUDIT 审计追踪: zephyr.trading.trading_contracts — trading-dom... | → | position.py | 导入依赖 / import_depends |
...
| 51 | D_TRADING 交易运营: risk_limit_violation_error.py | → | trace_context.py | 导入依赖 / import_depends |
| 52 | D_TRADING 交易运营: risk_validator_protocol.py | → | risk_limits.py | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 21 个外部域直接连接（出边 78 条 + 入边 53 条 = 131 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_INFRASTRUCTURE -->|2条 测试依赖 / test_depends| D_GOVERNANCE
