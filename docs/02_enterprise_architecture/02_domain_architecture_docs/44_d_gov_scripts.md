...
graph TD
    ...
    class D_AUDITTEST,D_GOV_AUDIT external_design
```
...
|:--:|---------|:--:|---------|---------|
| 1 | __init__.py | → | D_GOVERNANCE 生命周期管理: governance/task_show 脚本 — 任务卡详情查询 CLI... | config_depends / config_depends |
...
| 252 | D_GOV_AUDIT 审计追踪: [INVARIANTS] 按path精确匹配+按功能名模糊匹配; .... | → | apply_verification_results.py — 第32轮验证结果... | config_depends / config_depends |
| 253 | D_GOV_RULE 规则治理: generate_script_manifest.py — 脚本清单自动生成... | → | __init__.py | config_depends / config_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 7 个外部域直接连接（出边 25 条 + 入边 253 条 = 278 条）。只显示直接连接的域，不展开具体节点。
