...
graph TD
    ...
    src_zephyr_feedback_loop_alert_dispatcher_py -.->|runtime / runtime| D_GOV_CODE_QUALITY
    ...
    class D_ORCHESTRATOR,D_FBL_DETECTORS,D_FBL_DIAGNOSERS,D_COMPLIANCE,D_GOV_CODE_QUALITY,D_INTEGRATION,D_AUTONOMY_CORE,D_SECURITY,D_TRADING external_design
```
...
graph TD
    ...
    class D_AUTONOMY_CORE,D_GOV_ENFORCEMENT,D_GOV_AUDIT,D_KNOWLEDGE,D_FRONTEND external_design
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_SHARED,D_GOVERNANCE,D_AUTONOMY_CORE,D_GOV_DRIFT,D_INFRA_RECOVERY external_prod
    class D_FBL_VERIFICATION,D_FBL_DIAGNOSERS,D_FBL_DETECTORS,D_SECURITY_LLM,D_GOV_AUDIT,D_FRONTEND external_design
```
...
graph TD
    ...
    class D_SHARED,D_INFRA_RUNTIME,D_GOVERNANCE,D_FBL_VERIFICATION,D_AUTONOMY_CORE external_prod
    class D_FBL_DIAGNOSERS,D_FBL_DETECTORS,D_SECURITY,D_GOV_ENFORCEMENT,D_TRADING,D_SECURITY_LLM,D_GOV_AUDIT external_design
```
...
| 99 | FLE->Orc 告警分派器 — dispatch() 生产者 (alert... | → | D_FBL_VERIFICATION 反馈验证: Cross-Module Integration Verifier — v0.5.0 R39... | runtime / runtime |
...
| 150 | FLE->Orc 告警分派器 — dispatch() 生产者 (alert... | → | D_GOV_CODE_QUALITY 代码质量治理: encoding_gate.py — 编码安全校验门禁（治本：弥.... | runtime / runtime |
...
> 本域与 27 个外部域直接连接（出边 182 条 + 入边 147 条 = 329 条）。只显示直接连接的域，不展开具体节点。
