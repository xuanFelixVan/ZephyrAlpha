...
graph TD
    ...
    D_FBL_DIAGNOSERS -.->|runtime / runtime| src_zephyr_feedback_loop_collectors_init_py
    ...
    class D_ORCHESTRATOR,D_FBL_DIAGNOSERS,D_SECURITY external_design
```
...
graph TD
    ...
    src_zephyr_feedback_loop_core_py -.->|导入依赖 / import_depends| D_SHARED
    D_FBL_DIAGNOSERS["(原型态 / prototype) D_FBL_DIAGNOSERS"]
    D_FBL_DIAGNOSERS -.->|runtime / runtime| src_zephyr_feedback_loop_evolution_init_py
    ...
    class D_INFRA_RUNTIME,D_FBL_DIAGNOSERS,D_AUTONOMY_CORE external_design
```
...
graph TD
    ...
    D_FBL_DIAGNOSERS["(原型态 / prototype) D_FBL_DIAGNOSERS"]
    D_FBL_DIAGNOSERS -.->|runtime / runtime| src_zephyr_feedback_loop_forensic_automated_rca_postmortem_generator_py
...
graph TD
    ...
    src_zephyr_feedback_loop_gates_init_py -.->|导入依赖 / import_depends| D_FBL_VERIFICATION
    D_FBL_DIAGNOSERS -.->|runtime / runtime| src_zephyr_feedback_loop_resilience_init_py
    ...
    class D_SHARED,D_GOVERNANCE,D_AUTONOMY_CORE,D_INFRA_RECOVERY,D_GOV_DRIFT,D_INFRA_RUNTIME external_prod
...
graph TD
    ...
    src_zephyr_feedback_loop_verifiers_init_py -.->|导入依赖 / import_depends| D_FBL_VERIFICATION
    src_zephyr_feedback_loop_verifiers_init_py -.->|导入依赖 / import_depends| D_FBL_VERIFICATION
    D_FBL_DIAGNOSERS -.->|runtime / runtime| src_zephyr_feedback_loop_verifiers_init_py
    classDef production fill:#e1f5fe,stroke:#01579b,stroke-width:2px,color:#000
    classDef design fill:#fff3e0,stroke:#e65100,stroke-width:2px,color:#000,stroke-dasharray: 5 5
    classDef external_prod fill:#e8f5e9,stroke:#1b5e20,stroke-width:1px,color:#000
    classDef external_design fill:#fce4ec,stroke:#880e4f,stroke-width:1px,color:#000,stroke-dasharray: 5 5
...
graph TD
    ...
    class D_FBL_VERIFICATION,D_SHARED,D_GOVERNANCE,D_AUTONOMY_CORE,D_INFRA_RECOVERY,D_INFRA_RUNTIME,D_ORCHESTRATOR,D_GOV_AUDIT,D_FRONTEND external_prod
...
graph TD
    ...
    class D_ORCHESTRATOR,D_FBL_DIAGNOSERS external_design
```
...
| 6 | D_FBL_DIAGNOSERS 反馈诊断器: __init__.py | → | feedback-loop.collectors — auto-generated pack... | runtime / runtime |
| 7 | D_FBL_DIAGNOSERS 反馈诊断器: __init__.py | → | feedback-loop.evolution — auto-generated packa... | runtime / runtime |
| 8 | D_FBL_DIAGNOSERS 反馈诊断器: __init__.py | → | Automated RCA Postmortem Generator — v0.38.0 R... | runtime / runtime |
| 9 | D_FBL_DIAGNOSERS 反馈诊断器: __init__.py | → | feedback-loop.resilience — auto-generated pack... | runtime / runtime |
| 10 | D_FBL_DIAGNOSERS 反馈诊断器: __init__.py | → | feedback-loop.verifiers — auto-generated packa... | runtime / runtime |
| 11 | D_FRONTEND 前端: fitness_functions · Fitness Functions 仪表盘组... | → | fitness_functions.py | 导入依赖 / import_depends |
| 12 | D_GOV_AUDIT 审计追踪: feedback_bridge.py | → | Feedback Loop Engine — MOD-FEEDBACK_LOOP. (__i... | 导入依赖 / import_depends |
| 13 | D_INFRA_RUNTIME 运行时集成: AutoRuntimeCore — 三层运行时运营中心（系统大脑... | → | Feedback Loop Engine — MOD-FEEDBACK_LOOP. (__i... | 导入依赖 / import_depends |
| 14 | D_INFRA_RUNTIME 运行时集成: AutoRuntimeCore — 三层运行时运营中心（系统大脑... | → | FLE 全链路调度器 —— collect->detect->diagnose... | 导入依赖 / import_depends |
| 15 | D_INFRA_RUNTIME 运行时集成: lifecycle_manager.py | → | Feedback Loop Engine — MOD-FEEDBACK_LOOP. (__i... | 导入依赖 / import_depends |
| 16 | D_ORCHESTRATOR 代理编排器: TriggerRouter — RI-03 触发路由器（M3 跨模块触.... | → | Feedback Loop Decision Engine (decision_engine.py) | 导入依赖 / import_depends |
| 17 | D_SECURITY 对抗验证: feedback_bridge.py | → | Feedback Loop Engine — MOD-FEEDBACK_LOOP. (__i... | 导入依赖 / import_depends |
| 18 | D_SHARED 共享服务: secrets.py —— Secrets 管理抽象（Phase 7 新增 ... | → | Secret Rotation — v0.14.0 R189 (secret_rotatio... | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 15 个外部域直接连接（出边 84 条 + 入边 18 条 = 102 条）。只显示直接连接的域，不展开具体节点。
