...
graph TD
    ...
    class D_GOVERNANCE,D_SHARED,D_GOV_DRIFT,D_INFRA_RECOVERY,D_AUTONOMY_CORE,D_INFRA_RUNTIME external_prod
    class D_FBL_DETECTORS,D_FBL_VERIFICATION,D_FBL_DIAGNOSERS,D_INTEGRATION,D_GOV_DOCS external_design
```
...
graph TD
    ...
    class D_SHARED,D_FBL_VERIFICATION,D_GOVERNANCE,D_GOV_DRIFT,D_INFRA_RECOVERY,D_ORCHESTRATOR,D_INFRA_RUNTIME,D_GOV_AUDIT,D_FRONTEND external_prod
    class D_FBL_DETECTORS,D_FBL_DIAGNOSERS,D_INTEGRATION,D_AUTONOMY_CORE,D_SECURITY external_design
```
...
| 16 | D_INTEGRATION 管线路由: MCP Gateway 集中式治理节点（MOD-INF-013 §12 Ph... | → | API Version Contract — v0.14.0 R188 (api_versi... | 导入依赖 / import_depends |
| 17 | D_ORCHESTRATOR 代理编排器: TriggerRouter — RI-03 触发路由器（M3 跨模块触.... | → | Feedback Loop Decision Engine (decision_engine.py) | 导入依赖 / import_depends |
| 18 | D_SECURITY 对抗验证: feedback_bridge.py | → | Feedback Loop Engine — MOD-FEEDBACK_LOOP. (__i... | 导入依赖 / import_depends |
| 19 | D_SHARED 共享服务: secrets.py —— Secrets 管理抽象（Phase 7 新增 ... | → | Secret Rotation — v0.14.0 R189 (secret_rotatio... | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 16 个外部域直接连接（出边 84 条 + 入边 19 条 = 103 条）。只显示直接连接的域，不展开具体节点。
