...
graph TD
    ...
    src_zephyr_feedback_loop_core_py -.->|导入依赖 / import_depends| D_SHARED
    D_GOV_DOCS["(设计态 / design) D_GOV_DOCS"]
    D_GOV_DOCS -.->|runtime / runtime| src_zephyr_feedback_loop_evolution_auto_reward_py
...
graph TD
    ...
    class D_FBL_DIAGNOSERS,D_FBL_DETECTORS,D_FBL_VERIFICATION,D_INTEGRATION,D_GOV_DOCS external_design
```
...
graph TD
    ...
    class D_FBL_DIAGNOSERS,D_FBL_DETECTORS,D_GOV_DOCS,D_SECURITY external_design
```
...
| 9 | D_GOV_DOCS 架构文档治理: blueprint.md | → | Auto Reward — v0.7.0 R76 (auto_reward.py) | runtime / runtime |
...
| 12 | D_INFRA_RUNTIME 运行时集成: AutoRuntimeCore — 三层运行时运营中心（系统大脑... | → | Feedback Loop Engine — MOD-FEEDBACK_LOOP. (__i... | 导入依赖 / import_depends |
| 13 | D_INFRA_RUNTIME 运行时集成: AutoRuntimeCore — 三层运行时运营中心（系统大脑... | → | FLE 全链路调度器 —— collect->detect->diagnose... | 导入依赖 / import_depends |
| 14 | D_INFRA_RUNTIME 运行时集成: lifecycle_manager.py | → | Feedback Loop Engine — MOD-FEEDBACK_LOOP. (__i... | 导入依赖 / import_depends |
...
> 本域与 16 个外部域直接连接（出边 84 条 + 入边 17 条 = 101 条）。只显示直接连接的域，不展开具体节点。
