...
graph TD
    ...
    class D_FBL_DETECTORS,D_FBL_DIAGNOSERS,D_FBL_VERIFICATION,D_GOV_AUDIT external_prod
```
...
graph TD
    ...
    tests_feedback_test_resolution_tracker_py -.->|测试依赖 / test_depends| D_FBL_DETECTORS
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 120 个，10 条域内依赖）。
...
| 99 | FLE->Orc 告警分派器 — dispatch() 生产者 (alert... | → | D_FBL_VERIFICATION 反馈验证: GateVerdict — GateVerdict (parameterized_safet... | runtime / runtime |
...
| 158 | FeedbackLoop core — 反馈闭环核心类。 (core.py) | → | D_INTEGRATION 管线路由: schemas.py | 导入依赖 / import_depends |
| 159 | FeedbackCollector: collect task execution feedb... | → | D_INTEGRATION 管线路由: schemas.py | 导入依赖 / import_depends |
| 160 | FLE 全链路调度器 —— collect->detect->diagnose... | → | D_INTEGRATION 管线路由: InProcessVectorMemory — MOD-INF-011 VMS 统一入... | 导入依赖 / import_depends |
| 161 | FLE->Orc 告警分派器 — dispatch() 生产者 (alert... | → | D_ORCHESTRATOR 代理编排器: Orc 告警接收器 — handle_alert() 消费者 (alert_... | 导入依赖 / import_depends |
| 162 | evolution_engine.py | → | D_SECURITY 对抗验证: gateway.py | 导入依赖 / import_depends |
| 163 | API Version Contract — v0.14.0 R188 (api_versi... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 164 | FLE->Orc 告警分派器 — dispatch() 生产者 (alert... | → | D_SHARED 共享服务: capability.py —— Re-export wrapper -> canonic... | runtime / runtime |
...
> 本域与 26 个外部域直接连接（出边 180 条 + 入边 146 条 = 326 条）。只显示直接连接的域，不展开具体节点。
