...
graph TD
    ...
    class D_INFRA_RUNTIME,D_GOVERNANCE,D_SHARED,D_FBL_VERIFICATION,D_GOV_AUDIT external_prod
    class D_ORCHESTRATOR,D_FBL_DETECTORS,D_FBL_DIAGNOSERS,D_COMPLIANCE,D_INTEGRATION,D_AUTONOMY_CORE,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_INTEGRATION,D_GOVERNANCE,D_SHARED,D_ORCHESTRATOR external_prod
    class D_INFRA_RUNTIME,D_AUTONOMY_CORE,D_DATA,D_INTELLIGENCE external_design
```
...
graph TD
    ...
    D_FRONTEND -.->|测试依赖 / test_depends| src_zephyr_feedback_loop_forensic_fle_upgrade_safety_validator_py
    ...
    class D_SHARED,D_SECURITY,D_INTEGRATION external_prod
    class D_GOV_AUDIT,D_FRONTEND,D_AUTONOMY_CORE,D_GOV_ENFORCEMENT external_design
```
...
graph TD
    ...
    D_FBL_DIAGNOSERS["(原型态 / prototype) D_FBL_DIAGNOSERS"]
    src_zephyr_feedback_loop_scheduler_act_py -.->|导入依赖 / import_depends| D_FBL_DIAGNOSERS
    ...
    src_zephyr_feedback_loop_scheduler_health_py -.->|导入依赖 / import_depends| D_FBL_DIAGNOSERS
    src_zephyr_feedback_loop_scheduler_py -.->|导入依赖 / import_depends| D_SHARED
    ...
    class D_FBL_DETECTORS,D_FBL_DIAGNOSERS,D_FBL_VERIFICATION,D_INFRA_RUNTIME,D_AUTONOMY_CORE,D_SECURITY_LLM,D_FRONTEND external_design
```
...
graph TD
    ...
    class D_FBL_DIAGNOSERS,D_FBL_VERIFICATION external_prod
    class D_INTELLIGENCE,D_AUTONOMY_CORE,D_FRONTEND external_design
```
...
graph TD
    ...
    class D_FBL_DIAGNOSERS,D_FBL_VERIFICATION,D_GOV_AUDIT,D_FBL_DETECTORS external_prod
```
...
graph TD
    ...
    tests_feedback_test_positive_feedback_defense_py -.->|测试依赖 / test_depends| D_FBL_DETECTORS
    tests_feedback_test_recovery_time_stats_py -.->|测试依赖 / test_depends| D_FBL_DIAGNOSERS
    tests_feedback_test_recursive_diagnosis_trust_evaluator_py -.->|测试依赖 / test_depends| D_FBL_DETECTORS
    classDef production fill:#e1f5fe,stroke:#01579b,stroke-width:2px,color:#000
    classDef design fill:#fff3e0,stroke:#e65100,stroke-width:2px,color:#000,stroke-dasharray: 5 5
    classDef external_prod fill:#e8f5e9,stroke:#1b5e20,stroke-width:1px,color:#000
    classDef external_design fill:#fce4ec,stroke:#880e4f,stroke-width:1px,color:#000,stroke-dasharray: 5 5
...
graph TD
    ...
    class D_SHARED,D_INFRA_RUNTIME,D_GOVERNANCE,D_FBL_VERIFICATION external_prod
    class D_FBL_DETECTORS,D_FBL_DIAGNOSERS,D_GOV_AUDIT,D_AUTONOMY_CORE,D_SECURITY_LLM,D_INTELLIGENCE,D_FRONTEND external_design
```
...
graph TD
    ...
    class D_FBL_DIAGNOSERS,D_INTEGRATION,D_FBL_DETECTORS external_prod
    class D_FBL_VERIFICATION,D_GOVERNANCE,D_GOV_DOCS,D_AUTONOMY_CORE external_design
```
...
| 100 | FLE->Orc 告警分派器 — dispatch() 生产者 (alert... | → | D_FBL_VERIFICATION 反馈验证: Dry Run Sandbox — v0.3.0 R19 (dry_run_sandbox.py) | runtime / runtime |
...
| 90 | D_GOV_AUDIT 审计追踪: test_crypto_bootstrap.py | → | Cryptographic Bootstrap — v0.15.0 R204 (crypto... | 测试依赖 / test_depends |
| 91 | D_GOV_AUDIT 审计追踪: test_deterministic_replay.py | → | Deterministic Replay — v0.15.0 R206 (determini... | 测试依赖 / test_depends |
| 92 | D_GOV_AUDIT 审计追踪: test_point_in_time_reconstructor.py | → | Point-in-Time Reconstructor — v0.37.0 R465 (po... | 测试依赖 / test_depends |
| 93 | D_GOV_AUDIT 审计追踪: test_serialization_format_tracker.py | → | Serialization Format Tracker — v0.39.0 R488 (s... | 测试依赖 / test_depends |
| 94 | D_GOV_AUDIT 审计追踪: test_sub_agent_collusion.py | → | Sub-Agent Collusion Detector — v0.15.0 R213 (s... | 测试依赖 / test_depends |
| 95 | D_GOV_AUDIT 审计追踪: test_schema_evolution_root.py | → | Schema Evolution — v0.9.0 R111 (schema_evoluti... | 测试依赖 / test_depends |
| 96 | D_GOV_AUDIT 审计追踪: test_self_api_throttle_defense.py | → | Self API Throttle Defense — v0.39.0 R491 (self... | 测试依赖 / test_depends |
| 97 | D_GOV_AUDIT 审计追踪: test_self_diagnosis.py | → | self_diagnosis.py — 自我诊断 (DD120, TASK-020)... | 测试依赖 / test_depends |
| 98 | D_GOV_AUDIT 审计追踪: test_self_modification_audit.py | → | Self-Modification Audit — v0.15.0 R218 (self_m... | 测试依赖 / test_depends |
| 99 | D_GOV_AUDIT 审计追踪: test_self_modification_rate_limiter.py | → | R522: SelfModificationRateLimiter (self_modific... | 测试依赖 / test_depends |
...
> 本域与 26 个外部域直接连接（出边 182 条 + 入边 147 条 = 329 条）。只显示直接连接的域，不展开具体节点。
