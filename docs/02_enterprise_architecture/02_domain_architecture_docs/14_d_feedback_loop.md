...
graph TD
    ...
    class D_GOVERNANCE,D_INTEGRATION,D_SHARED,D_ORCHESTRATOR,D_FBL_DETECTORS external_prod
    class D_INFRA_RUNTIME,D_AUTONOMY_CORE,D_INTELLIGENCE,D_KNOWLEDGE,D_GOV_AUDIT,D_DATA external_design
```
...
graph TD
    ...
    class D_FRONTEND,D_GOV_ENFORCEMENT,D_GOV_AUDIT,D_AUTONOMY_CORE external_design
```
...
graph TD
    ...
    class D_FBL_DETECTORS,D_SHARED,D_GOV_DRIFT,D_GOVERNANCE,D_FBL_VERIFICATION external_prod
    class D_FBL_DIAGNOSERS,D_SECURITY,D_FRONTEND,D_AUTONOMY_CORE,D_GOV_AUDIT,D_INFRA_RUNTIME,D_INFRASTRUCTURE,D_INFRA_A2A external_design
```
...
graph TD
    ...
    class D_FRONTEND,D_INTELLIGENCE,D_AUTONOMY_CORE external_design
```
...
graph TD
    ...
    tests_feedback_test_scheduler_collect_detect_py -.->|测试依赖 / test_depends| D_FBL_DIAGNOSERS
    D_FBL_DETECTORS["(生产态 / production) D_FBL_DETECTORS"]
    tests_feedback_test_rumor_noise_filter_py -.->|测试依赖 / test_depends| D_FBL_DETECTORS
    tests_feedback_test_scheduler_collect_detect_py -.->|测试依赖 / test_depends| D_FBL_DIAGNOSERS
...
graph TD
    ...
    class D_FBL_VERIFICATION,D_FBL_DETECTORS,D_SHARED,D_GOV_DRIFT,D_GOVERNANCE,D_INFRA_RUNTIME external_prod
    class D_FBL_DIAGNOSERS,D_AUTONOMY_CORE,D_SECURITY,D_FRONTEND,D_GOV_AUDIT,D_GOV_ENFORCEMENT external_design
```
...
graph TD
    ...
    class D_FBL_VERIFICATION,D_GOV_AUDIT,D_FBL_DETECTORS,D_FBL_DIAGNOSERS external_prod
```
...
| 168 | FLE 全链路调度器 —— collect->detect->diagnose... | → | D_SHARED 共享服务: async_utils.py — async/sync 边界桥接（5.12.8 .... | 导入依赖 / import_depends |
| 169 | scheduler_act.py | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 导入依赖 / import_depends |
| 170 | scheduler_safety.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 171 | Secret Rotation — v0.14.0 R189 (secret_rotatio... | → | D_SHARED 共享服务: secrets.py —— Secrets 管理抽象（Phase 7 新增 ... | 导入依赖 / import_depends |
