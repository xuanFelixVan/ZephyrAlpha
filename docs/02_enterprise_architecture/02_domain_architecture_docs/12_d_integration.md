...
graph TD
    ...
    src_zephyr_integration_governance_init_py -.->|config_depends| src_zephyr_integration_governance_embedding_router_py
...
graph TD
    ...
    D_INTEGRATION_GATEWAY -.->|import_depends| src_zephyr_integration_vector_memory_in_process_vector_memory_py
    D_SECURITY["D_SECURITY prototype"]
    D_SECURITY -.->|import_depends| src_zephyr_integration_shared_schema_severity_types_py
    D_INTEGRATION_GATEWAY -.->|import_depends| src_zephyr_integration_vector_memory_collection_manager_py
    D_AUDITTEST["D_AUDITTEST prototype"]
    D_AUDITTEST -.->|test_depends| src_zephyr_integration_shared_schema_schemas_py
    ...
    class D_SHARED,D_GOVERNANCE,D_INTEGRATION_GATEWAY,D_SECURITY,D_AUDITTEST external_design
```
...
graph TD
    ...
    class D_SHARED,D_AUDITTEST,D_TRADING,D_INTEGRATION_GATEWAY external_design
```
...
> 域内模块依赖关系（共 71 条 / 71 edges）。按依赖类型分组，使用 → 表示方向。

```

┌──────────────────────────────────────────────────────────────────┐
│       依赖关系图 / Dependency Graph (共 71 条 / 71 edges)        │
├──────────────────────────────────────────────────────────────────┤
...
│   migrate_chroma_to_faiss.py → collection_manager.py             │
│   migrate_chroma_to_faiss.py → faiss_collection_manager.py       │
│   migrate_chroma_to_faiss.py → sqlite_metadata_store.py          │
│   sqlite_metadata_store.py → collection_manager.py               │
│   provenance_enforcer.py → collection_manager.py                 │
│   provenance_enforcer.py → vms_schemas.py                        │
│   vector_writer.py → context_ingest.py                           │
│   __init__.py → delegated_vector_memory.py                       │
│   ...还有 15 条 / 15 more edges                                  │
└──────────────────────────────────────────────────────────────────┘

**[config_depends]** (7 条 / edges) — 已达显示上限，省略 / limit reached

> (最多显示前 50 条依赖边，共 71 条)
