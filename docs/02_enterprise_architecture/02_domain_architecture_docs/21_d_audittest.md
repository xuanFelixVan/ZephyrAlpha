...
| 模块数 | 1705 | Module Count | 1705 |
| 域内依赖 | 11 | Internal Dependencies | 11 |
| 跨域入边 | 6 | Cross-domain Incoming | 6 |
| 跨域出边 | 2206 | Cross-domain Outgoing | 2206 |
| 设计态模块 | 0 | Design Modules | 0 |
| 原型态模块 | 1656 | Prototype Modules | 1656 |
...
> 按 architecture_layer 分组的模块清单（共 1705 个模块 / 1705 modules）。
...
### L2 领域层 / Domain Layer (1704 modules)
...
> 展示全部 1705 个模块（生产态 49 + 设计态 0 + 原型态 1656），标签标注成熟度。
...
graph TD
    ...
    class D_TRADING,D_GOVERNANCE,D_SECURITY,D_SECURITY_LLM external_prod
    class D_INFRA_RUNTIME,D_GOV_ENFORCEMENT,D_GOV_DRIFT external_design
```
...
graph TD
    ...
    class D_GOVERNANCE,D_INFRA_RUNTIME,D_SHARED,D_GOV_ENFORCEMENT,D_SECURITY external_prod
```
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 1656 个，11 条域内依赖）。
...
graph TD
    ...
    class D_INTEGRATION,D_GOVERNANCE,D_BACKTEST external_prod
```
...
| 212 | test_auto_bootstrap.py | → | D_GOVERNANCE 生命周期管理: blueprint.md | contract / contract |
...
| 215 | test_auto_bootstrap.py | → | D_GOVERNANCE 生命周期管理: Construction Verifier — 施工验证器: 任务卡完成... | runtime / runtime |
| 216 | test_auto_bootstrap.py | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 (depgraph_... | data / data |
...
| 545 | test_ttl_gate.py — ttl 字段校验门禁单元测试。 ... | → | D_GOVERNANCE 生命周期管理: ttl_gate.py — ttl 字段校验门禁（治本：弥补 --n... | 测试依赖 / test_depends |
| 546 | test_ttl_gate.py — ttl 字段校验门禁单元测试。 ... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 547 | test_unsafe_dict_spread_gate.py — ``**data`` .... | → | D_GOVERNANCE 生命周期管理: unsafe_dict_spread_gate.py — ``**data`` 直接展... | 测试依赖 / test_depends |
| 548 | test_unsafe_dict_spread_gate.py — ``**data`` .... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 549 | test_vocab_hardcode_gate.py — VOCAB-HARDCODE .... | → | D_GOVERNANCE 生命周期管理: vocab_hardcode_gate.py — 新增 .py 文件词表硬编... | 测试依赖 / test_depends |
| 550 | test_vocab_hardcode_gate.py — VOCAB-HARDCODE .... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 551 | test_human_factors.py | → | D_GOVERNANCE 生命周期管理: Human Factors — v0.7.0 人因工程: 通知疲劳管理+... | 测试依赖 / test_depends |
| 552 | test_load_bearing.py | → | D_GOVERNANCE 生命周期管理: 承重KE不可变性 + 承重墙自检 (load_bearing.py) | 测试依赖 / test_depends |
| 553 | test_quiet_period_monitor.py | → | D_GOVERNANCE 生命周期管理: 每日静默期检测 + 管道健康自检 (quiet_period_mon... | 测试依赖 / test_depends |
| 554 | test_thematic_clusterer.py | → | D_GOVERNANCE 生命周期管理: 主题聚类器 — 噪声信号比·告警疲劳缓解. (themat... | 测试依赖 / test_depends |
| 555 | test_command_chain_length_gate.py | → | D_GOVERNANCE 生命周期管理: Command Chain Length Gate — v0.13.0 命令体积De... | 测试依赖 / test_depends |
| 556 | test_cache_manager.py | → | D_GOVERNANCE 生命周期管理: Stage 0: 函数缓存管理器 — 增量扫描的加速核心. ... | 测试依赖 / test_depends |
| 557 | test_symbol_index.py | → | D_GOVERNANCE 生命周期管理: 符号索引 — 全局函数/类/import映射表. (symbol_i... | 测试依赖 / test_depends |
| 558 | test_behavioral_sampler.py | → | D_GOVERNANCE 生命周期管理: 行为采样验证器 — Stage 0.25 低成本快速验证. (b... | 测试依赖 / test_depends |
| 559 | test_behavioral_trust_checker.py | → | D_GOVERNANCE 生命周期管理: 行为信任检查器 — 行为漂移DIVERGED检测. (behavi... | 测试依赖 / test_depends |
| 560 | test_consequence_tracker.py | → | D_GOVERNANCE 生命周期管理: 后果追踪——记录每次修复操作对依赖方的影响. (co... | 测试依赖 / test_depends |
| 561 | test_delegation_engine.py | → | D_GOVERNANCE 生命周期管理: Escalation Protocol data models — MOD-INF-022 ... | 测试依赖 / test_depends |
| 562 | test_delegation_engine.py | → | D_GOVERNANCE 生命周期管理: Delegation Engine — MOD-INF-022 (delegation_en... | 测试依赖 / test_depends |
| 563 | test_parent_child_attributor.py | → | D_GOVERNANCE 生命周期管理: parent_child_attributor.py | 测试依赖 / test_depends |
| 564 | test_shadow_trust_validator.py | → | D_GOVERNANCE 生命周期管理: 影子信任验证器 — ImportError 防护回路. (shadow... | 测试依赖 / test_depends |
| 565 | test_trust_ring_manager.py | → | D_GOVERNANCE 生命周期管理: trust_ring_manager.py | 测试依赖 / test_depends |
| 566 | DM-100017: depgraph端到端功能测试（P2迁移后：Po... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 (depgraph_... | 测试依赖 / test_depends |
| 567 | DM-100026: 极端红蓝测试：depgraph生成器vs设计态... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 (depgraph_... | 测试依赖 / test_depends |
| 568 | test_dead_module_detector.py | → | D_GOVERNANCE 生命周期管理: 死共享模块检测器 — shared/子模块无人使用 -> DE... | 测试依赖 / test_depends |
| 569 | test_diff_detector.py | → | D_GOVERNANCE 生命周期管理: Stage 0: Git diff 变更检测器 — 函数粒度增量. (... | 测试依赖 / test_depends |
| 570 | test_ghost_scan.py | → | D_GOVERNANCE 生命周期管理: Ghost Scan — v0.8.0 幽灵进程检测: lingering pr... | 测试依赖 / test_depends |
| 571 | test_governance_drift_fix.py | → | D_GOVERNANCE 生命周期管理: G-CT-005 — ManagedDriftEvent Pydantic V2 BaseM... | 测试依赖 / test_depends |
| 572 | test_micro_clone_detector.py | → | D_GOVERNANCE 生命周期管理: 微型克隆检测器 — n-gram频率计数, 1-2行高频模式... | 测试依赖 / test_depends |
| 573 | test_stale_shared_detector.py | → | D_GOVERNANCE 生命周期管理: 过时共享函数检测器 — 无caller × 30天 -> STALE... | 测试依赖 / test_depends |
| 574 | test_alternative_path_blocker.py | → | D_GOVERNANCE 生命周期管理: Alternative Path Blocker — v0.13.0 替代工具路.... | 测试依赖 / test_depends |
| 575 | test_result_types.py | → | D_GOVERNANCE 生命周期管理: G-CT-003 — RollbackResult backward-compat re-e... | 测试依赖 / test_depends |
| 576 | test_annotations.py | → | D_GOVERNANCE 生命周期管理: 共享函数注解引擎 — @shared / @known_dup / @int... | 测试依赖 / test_depends |
| 577 | test_bare_repo_scanner.py | → | D_GOVERNANCE 生命周期管理: Bare Repo Scanner — v0.14.0 嵌入式裸仓库检测器... | 测试依赖 / test_depends |
| 578 | test_governance_result_types.py | → | D_GOVERNANCE 生命周期管理: G-CT-003 — RollbackResult backward-compat re-e... | 测试依赖 / test_depends |
| 579 | test_mock_duplicate_generator.py | → | D_GOVERNANCE 生命周期管理: 可控克隆生产器——零假阳性可期待引擎分子离散 (m... | 测试依赖 / test_depends |
| 580 | test_question_tracker.py | → | D_GOVERNANCE 生命周期管理: 问题追踪——扫描中发现需要人工处理的问题. (ques... | 测试依赖 / test_depends |
| 581 | test_api_response_sanitizer.py | → | D_GOVERNANCE 生命周期管理: API Response Sanitizer — v0.9.0 API响应清洗器:... | 测试依赖 / test_depends |
| 582 | test_bandwidth_optimizer.py | → | D_GOVERNANCE 生命周期管理: bandwidth_optimizer.py | 测试依赖 / test_depends |
| 583 | test_integration_hub.py | → | D_GOVERNANCE 生命周期管理: 集成协调器 — 24集成+19更新+16GitHub整合. (inte... | 测试依赖 / test_depends |
| 584 | test_integrations.py | → | D_GOVERNANCE 生命周期管理: 集成管理——预提交钩子+CI-only 扫描+超时边界. (... | 测试依赖 / test_depends |
| 585 | test_protocol_self_context.py | → | D_GOVERNANCE 生命周期管理: Protocol Self Context — v0.10.0 协议自维护上下... | 测试依赖 / test_depends |
| 586 | test_protocol_state_store.py | → | D_GOVERNANCE 生命周期管理: Protocol State Store — v0.10.0 协议运行时状态.... | 测试依赖 / test_depends |
| 587 | test_subagent_hook_propagator.py | → | D_GOVERNANCE 生命周期管理: Subagent Hook Propagator — v0.13.0 子Agent Hoo... | 测试依赖 / test_depends |
| 588 | test_bootstrapping_calibrator.py | → | D_GOVERNANCE 生命周期管理: bootstrapping_calibrator.py | 测试依赖 / test_depends |
| 589 | test_coldstart_manager.py | → | D_GOVERNANCE 生命周期管理: Coldstart Manager — v0.7.0 冷启动管理器: escal... | 测试依赖 / test_depends |
| 590 | test_maintenance_window_adapter.py | → | D_GOVERNANCE 生命周期管理: Maintenance Window Adapter — v0.10.0 计划维护.... | 测试依赖 / test_depends |
| 591 | test_time_sync.py | → | D_GOVERNANCE 生命周期管理: time_sync.py | 测试依赖 / test_depends |
| 592 | test_confidence_estimator.py | → | D_GOVERNANCE 生命周期管理: Confidence Estimator — D-022-05 置信度评估器: ... | 测试依赖 / test_depends |
| 593 | test_hotspot_tracker.py | → | D_GOVERNANCE 生命周期管理: 热点追踪器 — 90天滑动窗口 + 高频变动检测 + 新.... | 测试依赖 / test_depends |
| 594 | test_instruction_bloat_detector.py | → | D_GOVERNANCE 生命周期管理: InstructionBloatDetector — 指令膨胀检测 (instr... | 测试依赖 / test_depends |
| 595 | test_meta_confidence.py | → | D_GOVERNANCE 生命周期管理: Meta-Confidence — D-022-10 Agent对自身判定置信... | 测试依赖 / test_depends |
| 596 | test_meta_observability.py | → | D_GOVERNANCE 生命周期管理: Meta Observability — v0.10.0 协议自身可观测性:... | 测试依赖 / test_depends |
| 597 | test_report.py | → | D_GOVERNANCE 生命周期管理: 报告生成器 — YAML/JSON 输出 + 退出码判定 + Hea... | 测试依赖 / test_depends |
| 598 | test_clock_guard.py | → | D_GOVERNANCE 生命周期管理: Clock Guard — v0.8.0 时钟完整性防御: NTP漂移检... | 测试依赖 / test_depends |
| 599 | test_daily_ops.py | → | D_GOVERNANCE 生命周期管理: daily_ops.py | 测试依赖 / test_depends |
| 600 | test_exit_codes.py | → | D_GOVERNANCE 生命周期管理: 退出码定义模块——五档exit code 0-4枚举+描述+判... | 测试依赖 / test_depends |
| 601 | test_health_monitor.py | → | D_GOVERNANCE 生命周期管理: 健康仪表盘 — Dedup Health Score 0-100 + 趋势 +... | 测试依赖 / test_depends |
| 602 | test_success_validator.py | → | D_GOVERNANCE 生命周期管理: 成功验证——判断一次去重操作是否真正消灭了克隆.... | 测试依赖 / test_depends |
| 603 | test_verifier.py | → | D_GOVERNANCE 生命周期管理: 修复验证器 — import + 类型 + 行为采样验证. (ve... | 测试依赖 / test_depends |
| 604 | EngineSandbox — filesystem/network/boundary is... | → | D_GOVERNANCE 生命周期管理: EngineSandbox — D-022-08 OS-level sandboxing f... | 测试依赖 / test_depends |
| 605 | test_mvep_orchestrator.py | → | D_GOVERNANCE 生命周期管理: MVEP Orchestrator — v0.11.0 Minimum Viable Esc... | 测试依赖 / test_depends |
| 606 | test_objective_tracker.py | → | D_GOVERNANCE 生命周期管理: Objective Tracker — v0.9.0 目标漂移检测器: age... | 测试依赖 / test_depends |
| 607 | test_prioritizer.py | → | D_GOVERNANCE 生命周期管理: 修复优先级排序器 — 置信度×Impact×适配性 三因... | 测试依赖 / test_depends |
| 608 | test_think_time_model.py | → | D_GOVERNANCE 生命周期管理: think_time_model.py | 测试依赖 / test_depends |
| 609 | test_decisiongraph_schema_domain_id.py — decis... | → | D_GOVERNANCE 生命周期管理: decisiongraph Schema DDL + 不变量声明 (decision... | 测试依赖 / test_depends |
| 610 | test_deadlock_detector.py | → | D_GOVERNANCE 生命周期管理: Escalation Protocol data models — MOD-INF-022 ... | 测试依赖 / test_depends |
| 611 | test_deadlock_detector.py | → | D_GOVERNANCE 生命周期管理: Delegation Engine — MOD-INF-022 (delegation_en... | 测试依赖 / test_depends |
| 612 | test_deadlock_detector.py | → | D_GOVERNANCE 生命周期管理: Deadlock Detector — D-022-04 多Agent死锁+循环.... | 测试依赖 / test_depends |
| 613 | test_doom_loop_guard.py | → | D_GOVERNANCE 生命周期管理: Doom Loop 防护 — 修复升级阶梯 L0-L4 状态机. (d... | 测试依赖 / test_depends |
| 614 | test_fail_mode_manager.py | → | D_GOVERNANCE 生命周期管理: fail_mode_manager.py | 测试依赖 / test_depends |
| 615 | test_flash_crash_guard.py | → | D_GOVERNANCE 生命周期管理: Flash Crash Guard — v0.12.0 闪崩双轨熔断器。 (... | 测试依赖 / test_depends |
| 616 | test_interrupt_handler.py | → | D_GOVERNANCE 生命周期管理: Interrupt Handler — D-022-06 硬中断处理器: Own... | 测试依赖 / test_depends |
| 617 | test_last_resort_watchdog.py | → | D_GOVERNANCE 生命周期管理: Last Resort Watchdog — v0.8.0 终极逃生舱: 所有... | 测试依赖 / test_depends |
| 618 | test_observation_window_guard.py | → | D_GOVERNANCE 生命周期管理: 提取后稳定观察期守护 — 对标SDP 14天观察. (obse... | 测试依赖 / test_depends |
| 619 | test_policy_sandbox.py | → | D_GOVERNANCE 生命周期管理: policy_sandbox.py | 测试依赖 / test_depends |
| 620 | test_process_isolator.py | → | D_GOVERNANCE 生命周期管理: Process Isolator — v0.6.0 进程隔离器: engine运... | 测试依赖 / test_depends |
| 621 | test_provider_failover.py | → | D_GOVERNANCE 生命周期管理: Provider Failover — v0.7.0 多LLM Provider容灾:... | 测试依赖 / test_depends |
| 622 | test_recovery_manifest_writer.py | → | D_GOVERNANCE 生命周期管理: Recovery Manifest Writer — R2纯文本base64 Mani... | 测试依赖 / test_depends |
| 623 | test_silence_detector.py | → | D_GOVERNANCE 生命周期管理: Silence Detector — v0.8.0 静默窗口检测器: agen... | 测试依赖 / test_depends |
| 624 | test_spiral_ews.py | → | D_GOVERNANCE 生命周期管理: spiral_ews.py | 测试依赖 / test_depends |
| 625 | test_stream_abort_guard.py | → | D_GOVERNANCE 生命周期管理: StreamAbortGuard — 流式中断守卫 (stream_abort_... | 测试依赖 / test_depends |
| 626 | test_timeout_guard.py | → | D_GOVERNANCE 生命周期管理: timeout_guard.py | 测试依赖 / test_depends |
| 627 | test_witness_isolation.py | → | D_GOVERNANCE 生命周期管理: Witness Isolation — v0.8.0 Witness隔离: N版本d... | 测试依赖 / test_depends |
| 628 | test_commit_gate_registry.py — CommitGateRegis... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 629 | test_session_worktree.py — worktree 物理隔离端... | → | D_GOVERNANCE 生命周期管理: session_worktree.py — AI 对话 worktree 物理隔.... | 测试依赖 / test_depends |
| 630 | test_session_worktree.py — worktree 物理隔离端... | → | D_GOVERNANCE 生命周期管理: worktree_manager.py — session worktree 物理隔.... | 测试依赖 / test_depends |
| 631 | test_ssot_gate — SSoT 创建门禁红蓝变异测试。 (... | → | D_GOVERNANCE 生命周期管理: scaffold.py — ZephyrAlpha 唯一创建入口（RULE-T... | 测试依赖 / test_depends |
| 632 | test_ssot_gate — SSoT 创建门禁红蓝变异测试。 (... | → | D_GOVERNANCE 生命周期管理: CapabilityLookup — 能力->真源文件反查注册表的.... | 测试依赖 / test_depends |
| 633 | test_ssot_gate — SSoT 创建门禁红蓝变异测试。 (... | → | D_GOVERNANCE 生命周期管理: GitCommitGateway — 全项目唯一合法 git commit .... | 测试依赖 / test_depends |
| 634 | test_extraction_safety.py | → | D_GOVERNANCE 生命周期管理: 安全提取适配性评估器 — Suitability Score 0-100... | 测试依赖 / test_depends |
| 635 | test_github_api_guard.py | → | D_GOVERNANCE 生命周期管理: GitHub API Guard — v0.9.0 Comment and Control.... | 测试依赖 / test_depends |
| 636 | test_hooks_integrity_guard.py | → | D_GOVERNANCE 生命周期管理: Hooks Integrity Guard — v0.11.0 Hooks自编辑防.... | 测试依赖 / test_depends |
| 637 | test_import_surface_tracker.py | → | D_GOVERNANCE 生命周期管理: Import表面积负债追踪 — SBS 0-100 + shared burd... | 测试依赖 / test_depends |
| 638 | test_ipi_defense.py | → | D_GOVERNANCE 生命周期管理: ipi_defense.py | 测试依赖 / test_depends |
| 639 | test_monoculture_guard.py | → | D_GOVERNANCE 生命周期管理: Monoculture 免疫 — BRS 0-100 + 去重悖论检测. (... | 测试依赖 / test_depends |
| 640 | test_sbom_guard.py | → | D_GOVERNANCE 生命周期管理: SBOM Guard — v0.8.0 SBOM供应链防护: 依赖版本锁... | 测试依赖 / test_depends |
| 641 | test_security_config_scanner.py | → | D_GOVERNANCE 生命周期管理: Security Config Scanner — v0.13.0 缺失安全配置... | 测试依赖 / test_depends |
| 642 | test_sensitivity_sweeper.py | → | D_GOVERNANCE 生命周期管理: 敏感性扫荡——threshold扫描->固化成new baseline... | 测试依赖 / test_depends |
| 643 | test_signature_matcher.py | → | D_GOVERNANCE 生命周期管理: Stage 0.5: 签名指纹 SHA256[:12] O(1) 精确匹配. ... | 测试依赖 / test_depends |
| 644 | test_boot_hooks_unlock.py | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 测试依赖 / test_depends |
| 645 | test_shared_evolver.py | → | D_GOVERNANCE 生命周期管理: 共享函数自我进化引擎 — 自动升降级 + 行为漂移锁... | 测试依赖 / test_depends |
| 646 | test_shared_lifecycle_manager.py | → | D_GOVERNANCE 生命周期管理: 共享函数生命周期管理 — Active->Deprecated->Gra... | 测试依赖 / test_depends |
| 647 | test_rule_patterns.py — 治理规则正则 + 安全审.... | → | D_GOVERNANCE 生命周期管理: rule_patterns.py — 治理规则正则 + 安全审计模式... | 测试依赖 / test_depends |
| 648 | test_arbitrage_asymmetry_detector.py | → | D_GOVERNANCE 生命周期管理: Arbitrage Asymmetry Detector — v0.11.0 跨交易.... | 测试依赖 / test_depends |
| 649 | test_exchange_partition_detector.py | → | D_GOVERNANCE 生命周期管理: Exchange Partition Detector — v0.12.0 交易所网... | 测试依赖 / test_depends |
| 650 | test_exchange_reg_monitor.py | → | D_GOVERNANCE 生命周期管理: Exchange Reg Monitor — v0.11.0 交易所规则变更.... | 测试依赖 / test_depends |
| 651 | test_pricing_sync.py | → | D_GOVERNANCE 生命周期管理: pricing_sync.py | 测试依赖 / test_depends |
| 652 | test_strategy_scoper.py | → | D_GOVERNANCE 生命周期管理: Strategy Scoper — v0.6.0 策略范围隔离器: SIG/S... | 测试依赖 / test_depends |
| 653 | test_intent_keyword_mapper_root.py | → | D_GOVERNANCE 生命周期管理: IntentKeywordMapper - Stage 1 of three-stage in... | 测试依赖 / test_depends |
| 654 | test_intent_parser_root.py | → | D_GOVERNANCE 生命周期管理: IntentKeywordMapper - Stage 1 of three-stage in... | 测试依赖 / test_depends |
| 655 | test_intent_parser_root.py | → | D_GOVERNANCE 生命周期管理: IntentParser · 意图三阶段级联解析器（V-09） (i... | 测试依赖 / test_depends |
| 656 | test_mcp_task_claim.py | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 测试依赖 / test_depends |
| 657 | test_kb_analyze.py | → | D_GOVERNANCE 生命周期管理: G3 Evaluate 门禁 — 深度评估（T-2-13-C） (analy... | 测试依赖 / test_depends |
| 658 | test_kb_bootstrap.py | → | D_GOVERNANCE 生命周期管理: 冷启动引导引擎 — 从存量文档自动生成首批KE（T-M... | 测试依赖 / test_depends |
| 659 | test_kb_embedding_migrate.py | → | D_GOVERNANCE 生命周期管理: EmbeddingMigrate · Embedding 版本管理 + 迁移管... | 测试依赖 / test_depends |
| 660 | test_kb_extract.py | → | D_GOVERNANCE 生命周期管理: G5 Extract 门禁 — 知识升格（T-2-13-E） (extrac... | 测试依赖 / test_depends |
| 661 | test_kb_freeze.py | → | D_GOVERNANCE 生命周期管理: 紧急冻结/解冻/安全模式断路器 (freeze.py) | 测试依赖 / test_depends |
| 662 | test_kb_gate.py | → | D_GOVERNANCE 生命周期管理: audit-trail.kb_gate — MOD-INF-020 · KB 审计门... | 测试依赖 / test_depends |
| 663 | test_kb_gate_task.py | → | D_GOVERNANCE 生命周期管理: KB 五阶段门禁 evaluate 用的最小合法 Task（对齐 ... | 测试依赖 / test_depends |
| 664 | test_kb_graph_validator.py | → | D_GOVERNANCE 生命周期管理: 知识图谱完整性校验器（T-2-11-C） (graph_validat... | 测试依赖 / test_depends |
| 665 | test_kb_integrity.py | → | D_GOVERNANCE 生命周期管理: integrity.py | 测试依赖 / test_depends |
| 666 | test_kb_migration_embedding.py | → | D_GOVERNANCE 生命周期管理: EmbeddingMigrate · Embedding 版本管理 + 迁移管... | 测试依赖 / test_depends |
| 667 | test_kb_migration_gate.py | → | D_GOVERNANCE 生命周期管理: KB 五阶段门禁 evaluate 用的最小合法 Task（对齐 ... | 测试依赖 / test_depends |
| 668 | test_kb_self_test.py | → | D_GOVERNANCE 生命周期管理: KB 13项一键体检 + --self-test入口 (self_test.py) | 测试依赖 / test_depends |
| 669 | test_kb_storage_backend.py | → | D_GOVERNANCE 生命周期管理: Re-export shim — 真源在 zephyr.governance.kb.s... | 测试依赖 / test_depends |
| 670 | test_kb_triage.py | → | D_GOVERNANCE 生命周期管理: G2 Triage 门禁 — 知识分类评分（T-2-13-B） (tri... | 测试依赖 / test_depends |
| 671 | test_kb_unified_memory_api.py | → | D_GOVERNANCE 生命周期管理: Re-export shim — 真源在 zephyr.governance.kb.s... | 测试依赖 / test_depends |
| 672 | test_kb_verify.py | → | D_GOVERNANCE 生命周期管理: 确定性事实核查 — 取代AI猜测 (verify.py) | 测试依赖 / test_depends |
| 673 | test_kb_vms_memory_backend.py | → | D_GOVERNANCE 生命周期管理: Re-export shim — 真源在 zephyr.governance.kb.s... | 测试依赖 / test_depends |
| 674 | test_kb_vms_memory_backend.py | → | D_GOVERNANCE 生命周期管理: VMSMemoryBackend — UnifiedMemoryAPI 的 VMS 后.... | 测试依赖 / test_depends |
| 675 | test_ke_tombstone.py | → | D_GOVERNANCE 生命周期管理: SQLite墓碑表 + G2向量去重 (ke_tombstone.py) | 测试依赖 / test_depends |
| 676 | test_knowledge_distiller.py | → | D_GOVERNANCE 生命周期管理: knowledge_distiller.py — 知识蒸馏 (B10, DD84, ... | 测试依赖 / test_depends |
| 677 | test_cross_module_integration_llm_security.py | → | D_GOVERNANCE 生命周期管理: DefaultSecurityGateway — SecurityGateway 三层.... | 测试依赖 / test_depends |
| 678 | test_memory_poison_guard.py | → | D_GOVERNANCE 生命周期管理: Memory Poison Guard — v0.9.0 记忆投毒防护: Mem... | 测试依赖 / test_depends |
| 679 | test_memory_provenance.py | → | D_GOVERNANCE 生命周期管理: Memory Provenance — v0.9.0 记忆溯源追踪: 每条m... | 测试依赖 / test_depends |
| 680 | calibrate_model_diff.py 单元测试（P1-3 配套, 零... | → | D_GOVERNANCE 生命周期管理: 模型能力差异校准脚本（P1-3 治本）。 (calibrate_... | 测试依赖 / test_depends |
| 681 | test_model_drift_monitor.py | → | D_GOVERNANCE 生命周期管理: model_drift_monitor.py | 测试依赖 / test_depends |
| 682 | test_model_version_detector.py | → | D_GOVERNANCE 生命周期管理: Model Version Detector — v0.10.0 模型版本突变.... | 测试依赖 / test_depends |
| 683 | test_multi_turn_intent_analyzer.py | → | D_GOVERNANCE 生命周期管理: Multi-Turn Intent Analyzer — v0.13.0 多轮分布.... | 测试依赖 / test_depends |
| 684 | test_path_index_validator.py | → | D_GOVERNANCE 生命周期管理: 路径索引验证——验证 config 数据集相对路径表与.... | 测试依赖 / test_depends |
| 685 | DM-100027: 极端红蓝测试：路径树生成器vs设计态保... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 (depgraph_... | 测试依赖 / test_depends |
| 686 | test_resource_guard.py | → | D_GOVERNANCE 生命周期管理: Resource Guard — 资源上限与优雅降级 D-023-23 .... | 测试依赖 / test_depends |
| 687 | test_risk_matrix.py | → | D_GOVERNANCE 生命周期管理: risk_matrix.py | 测试依赖 / test_depends |
| 688 | test_risk_mitigation_tracker.py | → | D_GOVERNANCE 生命周期管理: 风险缓解追踪——捕获哪些克隆报告了但在N次扫描后... | 测试依赖 / test_depends |
| 689 | test_risk_mitigator.py | → | D_GOVERNANCE 生命周期管理: R1-R45全量风险缓解执行器 — 逐条检查缓解措施 + ... | 测试依赖 / test_depends |
| 690 | 红蓝对抗极端测试 — git_guard + concurrency_gua... | → | D_GOVERNANCE 生命周期管理: Git Guard — 拦截危险 git 命令，防止破坏其他 se... | 测试依赖 / test_depends |
| 691 | 并发红蓝极限对抗测试 — 多 AI 并发执行 git mv .... | → | D_GOVERNANCE 生命周期管理: Git Guard — 拦截危险 git 命令，防止破坏其他 se... | 测试依赖 / test_depends |
| 692 | test_rollback_bridge.py | → | D_GOVERNANCE 生命周期管理: G-CT-006 契约：Drift -> Rollback 漂移触发回滚. ... | 测试依赖 / test_depends |
...
| 695 | test_safety_brake.py | → | D_GOVERNANCE 生命周期管理: 冷静期引擎 + 魔鬼代言人 + 影响评估 (safety_brak... | 测试依赖 / test_depends |
| 696 | test_self_budget_tracker.py | → | D_GOVERNANCE 生命周期管理: self_budget_tracker.py | 测试依赖 / test_depends |
| 697 | test_self_check.py | → | D_GOVERNANCE 生命周期管理: Self-Drift Check — self_check.py (self_check.py) | 测试依赖 / test_depends |
| 698 | test_self_monitor.py | → | D_GOVERNANCE 生命周期管理: self_monitor.py | 测试依赖 / test_depends |
| 699 | test_self_scanner.py | → | D_GOVERNANCE 生命周期管理: 引擎自扫描器 — Dogfooding 检测引擎自身源码重复... | 测试依赖 / test_depends |
| 700 | test_self_test.py | → | D_GOVERNANCE 生命周期管理: Escalation Protocol Self-Test — MOD-INF-022. (... | 测试依赖 / test_depends |
| 701 | test_self_test_verifier.py | → | D_GOVERNANCE 生命周期管理: Self Test Verifier — self_test_verifier.py (se... | 测试依赖 / test_depends |
| 702 | test_self_validator.py | → | D_GOVERNANCE 生命周期管理: Self Validator — v0.10.0 升级协议自验证器: pro... | 测试依赖 / test_depends |
| 703 | blast_radius 单元测试 — BlastRadiusAnalyzer 全... | → | D_GOVERNANCE 生命周期管理: blast_radius — MOD-INF-028 §3.1 Stage 9 (blas... | 测试依赖 / test_depends |
| 704 | blast_radius 单元测试 — BlastRadiusAnalyzer 全... | → | D_GOVERNANCE 生命周期管理: 语义审计管线数据模型 — MOD-INF-028 §4.2 (mode... | 测试依赖 / test_depends |
| 705 | blast_radius 红蓝对抗测试 — 对抗性场景覆盖. (t... | → | D_GOVERNANCE 生命周期管理: blast_radius — MOD-INF-028 §3.1 Stage 9 (blas... | 测试依赖 / test_depends |
| 706 | blast_radius 红蓝对抗测试 — 对抗性场景覆盖. (t... | → | D_GOVERNANCE 生命周期管理: 语义审计管线数据模型 — MOD-INF-028 §4.2 (mode... | 测试依赖 / test_depends |
| 707 | test_semantic_auditor.py | → | D_GOVERNANCE 生命周期管理: models.py | 测试依赖 / test_depends |
| 708 | test_semantic_cache.py | → | D_GOVERNANCE 生命周期管理: semantic_cache.py | 测试依赖 / test_depends |
| 709 | DM-202918: transition(COMPLETED)自动git commit.... | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 测试依赖 / test_depends |
| 710 | test_task_repo_gateway_e2e.py — 端到端链路测试... | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 测试依赖 / test_depends |
| 711 | test_task_repo_gateway_e2e.py — 端到端链路测试... | → | D_GOVERNANCE 生命周期管理: GitCommitGateway — 全项目唯一合法 git commit .... | 测试依赖 / test_depends |
| 712 | test_decision_graph_reader — DecisionGraphRead... | → | D_GOVERNANCE 生命周期管理: decision_graph_reader.py — 决策流图数据库只读.... | 测试依赖 / test_depends |
| 713 | test_decision_graph_reader — DecisionGraphRead... | → | D_GOVERNANCE 生命周期管理: decisiongraph Schema DDL + 不变量声明 (decision... | 测试依赖 / test_depends |
| 714 | test_extract_decisiongraph — extract_decisiong... | → | D_GOVERNANCE 生命周期管理: decisiongraph Schema DDL + 不变量声明 (decision... | 测试依赖 / test_depends |
| 715 | tests.test_trading_session_lifecycle — F1 Sess... | → | D_GOVERNANCE 生命周期管理: session_lifecycle.py | 测试依赖 / test_depends |
| 716 | cli 单测（MOD-L00-004 阶段3）。 (test_cli.py) | → | D_GOVERNANCE 生命周期管理: 数据源集成器 CLI（MOD-L00-004 §8.4）。 (cli.py) | 测试依赖 / test_depends |
| 717 | cli 单测（MOD-L00-004 阶段3）。 (test_cli.py) | → | D_GOVERNANCE 生命周期管理: per-source 调用策略注册表（MOD-L00-004 §5）。 ... | 测试依赖 / test_depends |
| 718 | scheduler 单测（MOD-L00-004 阶段2）。 (test_sch... | → | D_GOVERNANCE 生命周期管理: per-source 调用策略注册表（MOD-L00-004 §5）。 ... | 测试依赖 / test_depends |
| 719 | test_auto_bootstrap.py | → | D_GOV_DRIFT 漂移检测: blueprint.md | runtime / runtime |
| 720 | test_ai_capability_guard.py | → | D_GOV_ENFORCEMENT 规则执行: ZephyrAlpha — gates/ai_capability_guard.py (ai... | 测试依赖 / test_depends |
| 721 | test_audit_chain_verifier.py | → | D_GOV_ENFORCEMENT 规则执行: 审计链验证工具——独立重放门禁判定+Hash链完整性... | 测试依赖 / test_depends |
| 722 | test_audit_chain_verifier.py | → | D_GOV_ENFORCEMENT 规则执行: 门禁上下文传播——GateContext 构建/序列化/跨模.... | 测试依赖 / test_depends |
| 723 | test_audit_red_blue_e2e.py | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 724 | test_auto_bootstrap.py | → | D_GOV_ENFORCEMENT 规则执行: Audit Trail — MOD-INF-020 (__init__.py) | runtime / runtime |
| 725 | test_auto_split.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 726 | test_ba_integration_test_runner.py | → | D_GOV_ENFORCEMENT 规则执行: 集成测试运行器（Integration Test Runner） (inte... | 测试依赖 / test_depends |
| 727 | test_ct_audit_findings_resolved.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 728 | test_ct_audit_findings_resolved.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 729 | test_ct_blueprint_read_check.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 730 | test_ct_blueprint_read_check.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 731 | test_ct_circuit_breaker.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 732 | test_ct_circuit_breaker.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 733 | test_ct_circular_dependency_scan.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 734 | test_ct_circular_dependency_scan.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 735 | test_ct_classification.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 736 | test_ct_classification.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 737 | test_ct_content_length.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 738 | test_ct_content_length.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 739 | test_ct_content_quality.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 740 | test_ct_content_quality.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 741 | test_ct_contract_compatibility_check.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 742 | test_ct_contract_compatibility_check.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 743 | test_ct_deduplication.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 744 | test_ct_deduplication.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 745 | test_ct_drift_budget.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 746 | test_ct_drift_budget.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 747 | test_ct_encoding.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 748 | test_ct_encoding.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 749 | test_ct_enforcement_mode_check.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 750 | test_ct_enforcement_mode_check.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 751 | test_ct_field_presence.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 752 | test_ct_field_presence.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 753 | test_ct_file_extension.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 754 | test_ct_file_extension.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 755 | test_ct_fle_gate.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 756 | test_ct_fle_gate.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 757 | test_ct_frontmatter.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 758 | test_ct_frontmatter.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 759 | test_ct_leverage_limit.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 760 | test_ct_leverage_limit.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 761 | test_ct_line_ending.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 762 | test_ct_line_ending.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 763 | test_ct_manual_approval.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 764 | test_ct_manual_approval.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 765 | test_ct_path_blacklist.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 766 | test_ct_path_blacklist.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 767 | test_ct_path_routing.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 768 | test_ct_path_routing.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 769 | test_ct_path_whitelist.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 770 | test_ct_path_whitelist.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 771 | test_ct_position_limit.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 772 | test_ct_position_limit.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 773 | test_ct_reference_check.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 774 | test_ct_reference_check.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 775 | test_ct_regex_pattern.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 776 | test_ct_regex_pattern.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 777 | test_ct_restructuring_safety.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 778 | test_ct_restructuring_safety.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 779 | test_ct_rollback_exit_code.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 780 | test_ct_rollback_exit_code.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 781 | test_ct_score_threshold.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 782 | test_ct_score_threshold.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 783 | test_ct_security_artifact_scan.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 784 | test_ct_security_artifact_scan.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 785 | test_ct_strategy_correlation.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 786 | test_ct_strategy_correlation.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 787 | test_ct_temporal.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 788 | test_ct_temporal.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 789 | test_ct_zero_residue_check.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 790 | test_ct_zero_residue_check.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 791 | test_db_transition.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 792 | test_e_gov_approval.py | → | D_GOV_ENFORCEMENT 规则执行: G-CT-004 — Backward-compat re-export of Approv... | 测试依赖 / test_depends |
| 793 | test_e_slo_contract.py | → | D_GOV_ENFORCEMENT 规则执行: SLO-Driven Escalation Contract — D-022-12. (sl... | 测试依赖 / test_depends |
| 794 | test_escalation_gov_approval.py | → | D_GOV_ENFORCEMENT 规则执行: G-CT-004 — Backward-compat re-export of Approv... | 测试依赖 / test_depends |
| 795 | test_fl_adversarial_validation.py | → | D_GOV_ENFORCEMENT 规则执行: AdversarialValidationGate — validates outputs ... | 测试依赖 / test_depends |
| 796 | test_file_task_mapper_root.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 797 | test_gate_context.py | → | D_GOV_ENFORCEMENT 规则执行: 门禁上下文传播——GateContext 构建/序列化/跨模.... | 测试依赖 / test_depends |
| 798 | test_gate_health.py | → | D_GOV_ENFORCEMENT 规则执行: 门禁健康仪表板——per-gate SLI 报告、误报率、延... | 测试依赖 / test_depends |
| 799 | test_gate_integrity_guard.py | → | D_GOV_ENFORCEMENT 规则执行: 门禁引擎完整性守卫——自检SHA-256校验+trust roo... | 测试依赖 / test_depends |
| 800 | test_gate_override.py | → | D_GOV_ENFORCEMENT 规则执行: Owner 紧急旁路——时间限定的门禁临时绕过 + 审计... | 测试依赖 / test_depends |
| 801 | test_gate_pipeline.py | → | D_GOV_ENFORCEMENT 规则执行: 门禁上下文传播——GateContext 构建/序列化/跨模.... | 测试依赖 / test_depends |
| 802 | test_gate_pipeline.py | → | D_GOV_ENFORCEMENT 规则执行: 门禁评估管线——排序解析、组合逻辑（AND/OR/NOT.... | 测试依赖 / test_depends |
| 803 | test_gate_simulator.py | → | D_GOV_ENFORCEMENT 规则执行: 门禁上下文传播——GateContext 构建/序列化/跨模.... | 测试依赖 / test_depends |
| 804 | test_gate_simulator.py | → | D_GOV_ENFORCEMENT 规则执行: 门禁评估管线——排序解析、组合逻辑（AND/OR/NOT.... | 测试依赖 / test_depends |
| 805 | test_gate_simulator.py | → | D_GOV_ENFORCEMENT 规则执行: 门禁模拟器——dry-run 全链路门禁演练，不修改任.... | 测试依赖 / test_depends |
| 806 | test_gate_types.py | → | D_GOV_ENFORCEMENT 规则执行: gate_types.py | 测试依赖 / test_depends |
| 807 | test_approval.py | → | D_GOV_ENFORCEMENT 规则执行: G-CT-004 — Backward-compat re-export of Approv... | 测试依赖 / test_depends |
| 808 | test_pre_flight_gate.py | → | D_GOV_ENFORCEMENT 规则执行: pre_flight_gate.py | 测试依赖 / test_depends |
| 809 | test_slo_contract.py | → | D_GOV_ENFORCEMENT 规则执行: SLO-Driven Escalation Contract — D-022-12. (sl... | 测试依赖 / test_depends |
| 810 | test_base_repo.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 811 | test_check_type_registry.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 812 | test_adversarial_gate_integration.py | → | D_GOV_ENFORCEMENT 规则执行: Adversarial sample generator and 5 attack strat... | 测试依赖 / test_depends |
| 813 | test_adversarial_gate_integration.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 814 | test_adversarial_gate_integration.py | → | D_GOV_ENFORCEMENT 规则执行: AdversarialValidationGate — validates outputs ... | 测试依赖 / test_depends |
| 815 | test_adversarial_validation.py | → | D_GOV_ENFORCEMENT 规则执行: AdversarialValidationGate — validates outputs ... | 测试依赖 / test_depends |
| 816 | test_adversarial_validation_gate.py | → | D_GOV_ENFORCEMENT 规则执行: AdversarialValidationGate — validates outputs ... | 测试依赖 / test_depends |
| 817 | test_en_001_circular_dependency.py | → | D_GOV_ENFORCEMENT 规则执行: EN-001 — Circular Dependency Scanner (en_001_c... | 测试依赖 / test_depends |
| 818 | test_en_002_enforcement_validator.py | → | D_GOV_ENFORCEMENT 规则执行: EN-002 — Enforcement Mode Validator (en_002_en... | 测试依赖 / test_depends |
| 819 | test_en_003_contract_compatibility.py | → | D_GOV_ENFORCEMENT 规则执行: EN-003 — Contract Compatibility Checker (en_00... | 测试依赖 / test_depends |
| 820 | test_en_process_lifecycle_gateway.py | → | D_GOV_ENFORCEMENT 规则执行: EN-process-lifecycle-gateway — 进程创建入口校.... | 测试依赖 / test_depends |
| 821 | test_post_doc_review.py | → | D_GOV_ENFORCEMENT 规则执行: PostDocReviewScanner — Session 关门时文档内容.... | 测试依赖 / test_depends |
| 822 | test_zero_residue_check.py | → | D_GOV_ENFORCEMENT 规则执行: zero_residue_check.py | 测试依赖 / test_depends |
| 823 | test_adaptive_threshold.py | → | D_GOV_ENFORCEMENT 规则执行: 自适应阈值——从历史 FAIL/PASS 数据学习门禁参数... | 测试依赖 / test_depends |
| 824 | test_adversarial_strategies.py | → | D_GOV_ENFORCEMENT 规则执行: Adversarial sample generator and 5 attack strat... | 测试依赖 / test_depends |
| 825 | test_breaking_change_detector.py | → | D_GOV_ENFORCEMENT 规则执行: Breaking Change 检测器（GATE-CDC-2）——字段删.... | 测试依赖 / test_depends |
| 826 | test_end_to_end_walkthrough.py | → | D_GOV_ENFORCEMENT 规则执行: 端到端场景走查验证器（End-to-End Walkthrough Va... | 测试依赖 / test_depends |
| 827 | test_integration_test_runner.py | → | D_GOV_ENFORCEMENT 规则执行: 集成测试运行器（Integration Test Runner） (inte... | 测试依赖 / test_depends |
| 828 | test_kiss_enforcer.py | → | D_GOV_ENFORCEMENT 规则执行: KISS 约束执行器（CT-KISS-001）——AI产出复杂度.... | 测试依赖 / test_depends |
| 829 | test_output_quality_gate.py | → | D_GOV_ENFORCEMENT 规则执行: output_quality_gate.py | 测试依赖 / test_depends |
| 830 | test_secrets_guard.py | → | D_GOV_ENFORCEMENT 规则执行: Secrets 守护（CT-SECRETS-001）——.env校验+git ... | 测试依赖 / test_depends |
| 831 | test_triple_alignment.py | → | D_GOV_ENFORCEMENT 规则执行: G-TRIPLE-ALIGN: 蓝图↔代码↔依赖图三方对齐门禁 ... | 测试依赖 / test_depends |
| 832 | test_preemption_manager.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 833 | test_kb_activate.py | → | D_GOV_ENFORCEMENT 规则执行: gate_types.py | 测试依赖 / test_depends |
| 834 | test_kb_analyze.py | → | D_GOV_ENFORCEMENT 规则执行: gate_types.py | 测试依赖 / test_depends |
| 835 | test_kb_extract.py | → | D_GOV_ENFORCEMENT 规则执行: gate_types.py | 测试依赖 / test_depends |
| 836 | test_kb_migration_gate.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 837 | test_db.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 838 | test_risk_ssot.py | → | D_GOV_ENFORCEMENT 规则执行: risk_ssot — 从 ``config/risk_params.yaml`` 加.... | 测试依赖 / test_depends |
| 839 | test_rule_canary_manager.py | → | D_GOV_ENFORCEMENT 规则执行: Rule Canary Manager — v0.10.0 规则金丝雀: 1%用... | 测试依赖 / test_depends |
| 840 | test_rule_debt_auditor.py | → | D_GOV_ENFORCEMENT 规则执行: Rule Debt Auditor — v0.7.0 规则债务审计器: 分.... | 测试依赖 / test_depends |
| 841 | test_rule_e2e.py | → | D_GOV_ENFORCEMENT 规则执行: RuleLoader — 规则加载核心 API (rule_engine.py) | 测试依赖 / test_depends |
| 842 | test_rule_integration.py | → | D_GOV_ENFORCEMENT 规则执行: RuleLoader — 规则加载核心 API (rule_engine.py) | 测试依赖 / test_depends |
| 843 | test_rule_shadow_runner.py | → | D_GOV_ENFORCEMENT 规则执行: Rule Shadow Runner — v0.10.0 规则影子模式: 新.... | 测试依赖 / test_depends |
| 844 | test_task_types.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 845 | Test gate g_trae_003 for rule TRAE-003 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 846 | Test gate g_trae_003 for rule TRAE-003 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 847 | Test gate g_trae_004 for rule TRAE-004 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 848 | Test gate g_trae_004 for rule TRAE-004 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 849 | Test gate g_trae_006 for rule TRAE-006 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 850 | Test gate g_trae_006 for rule TRAE-006 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 851 | Test gate g_trae_007 for rule TRAE-007 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 852 | Test gate g_trae_007 for rule TRAE-007 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 853 | Test gate g_trae_008 for rule TRAE-008 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 854 | Test gate g_trae_008 for rule TRAE-008 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 855 | Test gate g_trae_009 for rule TRAE-009 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 856 | Test gate g_trae_009 for rule TRAE-009 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 857 | Test gate g_trae_010 for rule TRAE-010 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 858 | Test gate g_trae_010 for rule TRAE-010 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 859 | Test gate g_trae_011 for rule TRAE-011 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 860 | Test gate g_trae_011 for rule TRAE-011 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 861 | Test gate g_trae_012 for rule TRAE-012 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 862 | Test gate g_trae_012 for rule TRAE-012 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 863 | Test gate g_trae_016 for rule TRAE-016 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 864 | Test gate g_trae_016 for rule TRAE-016 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 865 | Test gate g_trae_017 for rule TRAE-017 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 866 | Test gate g_trae_017 for rule TRAE-017 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 867 | Test gate g_trae_018 for rule TRAE-018 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 868 | Test gate g_trae_018 for rule TRAE-018 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 869 | Test gate g_trae_020 for rule TRAE-020 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 870 | Test gate g_trae_020 for rule TRAE-020 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 871 | Test gate g_trae_021 for rule TRAE-021 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 872 | Test gate g_trae_021 for rule TRAE-021 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 873 | Test gate g_trae_022 for rule TRAE-022 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 874 | Test gate g_trae_022 for rule TRAE-022 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 875 | Test gate g_trae_023 for rule TRAE-023 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 876 | Test gate g_trae_023 for rule TRAE-023 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 877 | Test gate g_trae_024 for rule TRAE-024 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 878 | Test gate g_trae_024 for rule TRAE-024 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 879 | Test gate g_trae_025 for rule TRAE-025 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 880 | Test gate g_trae_025 for rule TRAE-025 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 881 | Test gate g_trae_026 for rule TRAE-026 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 882 | Test gate g_trae_026 for rule TRAE-026 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 883 | Test gate g_trae_027 for rule TRAE-027 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 884 | Test gate g_trae_027 for rule TRAE-027 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 885 | Test gate g_trae_028 for rule TRAE-028 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 886 | Test gate g_trae_028 for rule TRAE-028 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 887 | Test gate g_trae_029 for rule TRAE-029 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 888 | Test gate g_trae_029 for rule TRAE-029 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 889 | Test gate g_trae_030 for rule TRAE-030 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 890 | Test gate g_trae_030 for rule TRAE-030 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 891 | Test gate g_trae_031 for rule TRAE-031 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 892 | Test gate g_trae_031 for rule TRAE-031 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 893 | Test gate g_trae_032 for rule TRAE-032 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 894 | Test gate g_trae_032 for rule TRAE-032 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 895 | Test gate g_trae_033 for rule TRAE-033 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 896 | Test gate g_trae_033 for rule TRAE-033 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 897 | Test gate g_trae_034 for rule TRAE-034 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 898 | Test gate g_trae_034 for rule TRAE-034 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 899 | Test gate g_trae_035 for rule TRAE-035 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 900 | Test gate g_trae_035 for rule TRAE-035 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 901 | Test gate g_trae_036 for rule TRAE-036 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 902 | Test gate g_trae_036 for rule TRAE-036 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 903 | Test gate g_trae_037 for rule TRAE-037 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 904 | Test gate g_trae_037 for rule TRAE-037 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 905 | Test gate g_trae_038 for rule TRAE-038 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 906 | Test gate g_trae_038 for rule TRAE-038 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 907 | Test gate g_trae_039 for rule TRAE-039 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 908 | Test gate g_trae_039 for rule TRAE-039 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 909 | Test gate g_trae_040 for rule TRAE-040 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 910 | Test gate g_trae_040 for rule TRAE-040 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 911 | Test gate g_trae_041 for rule TRAE-041 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 912 | Test gate g_trae_041 for rule TRAE-041 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 913 | Test gate g_trae_042 for rule TRAE-042 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 914 | Test gate g_trae_042 for rule TRAE-042 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 915 | Test gate g_trae_043 for rule TRAE-043 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 916 | Test gate g_trae_043 for rule TRAE-043 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 917 | Test gate g_trae_044 for rule TRAE-044 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 918 | Test gate g_trae_044 for rule TRAE-044 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 919 | Test gate g_trae_045 for rule TRAE-045 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 920 | Test gate g_trae_045 for rule TRAE-045 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 921 | Test gate g_trae_046 for rule TRAE-046 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 922 | Test gate g_trae_046 for rule TRAE-046 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 923 | Test gate g_trae_047 for rule TRAE-047 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 924 | Test gate g_trae_047 for rule TRAE-047 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 925 | Test gate g_trae_048 for rule TRAE-048 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 926 | Test gate g_trae_048 for rule TRAE-048 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 927 | Test gate g_trae_049 for rule TRAE-049 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 928 | Test gate g_trae_049 for rule TRAE-049 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 929 | Test gate g_trae_050 for rule TRAE-050 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 930 | Test gate g_trae_050 for rule TRAE-050 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 931 | Test gate g_trae_051 for rule TRAE-051 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 932 | Test gate g_trae_051 for rule TRAE-051 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 933 | Test gate g_trae_052 for rule TRAE-052 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 934 | Test gate g_trae_052 for rule TRAE-052 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 935 | Test gate g_trae_053 for rule TRAE-053 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 936 | Test gate g_trae_053 for rule TRAE-053 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 937 | Test gate g_trae_054 for rule TRAE-054 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 938 | Test gate g_trae_054 for rule TRAE-054 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 939 | Test gate g_trae_055 for rule TRAE-055 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 940 | Test gate g_trae_055 for rule TRAE-055 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 941 | test_utils_testing.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 942 | 单元测试：scripts/governance/d3_metadata/check_... | → | D_GOV_SCRIPTS 脚本治理: GATE-15: Frontmatter metadata validation（ttl +... | 测试依赖 / test_depends |
| 943 | test_ssot_gate — SSoT 创建门禁红蓝变异测试。 (... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | 测试依赖 / test_depends |
| 944 | test_a2a_card_registry.py | → | D_INFRA_A2A A2A通信: A2A Card Registry — 全局 Agent Card 注册单例 (... | 测试依赖 / test_depends |
| 945 | test_a2a_card_registry.py | → | D_INFRA_A2A A2A通信: A2A Registry — Agent Card 注册与发现 (a2a_regi... | 测试依赖 / test_depends |
| 946 | test_a2a_card_registry.py | → | D_INFRA_A2A A2A通信: Agent Card 模型 — A2A Layer 1 Discovery (agent... | 测试依赖 / test_depends |
| 947 | test_a2a_governance.py | → | D_INFRA_A2A A2A通信: A2A GovernanceAdapter — Phase 4 治理集成桥接器... | 测试依赖 / test_depends |
| 948 | test_a2a_governance.py | → | D_INFRA_A2A A2A通信: Phase 4 Hold — A2A Phase 4 锁定标记模块 与其他... | 测试依赖 / test_depends |
| 949 | test_a2a_governance.py | → | D_INFRA_A2A A2A通信: G-CT-008 — A2ACommunication Pydantic V2 BaseMo... | 测试依赖 / test_depends |
| 950 | test_a2a_layer1_discovery.py | → | D_INFRA_A2A A2A通信: A2A Registry — Agent Card 注册与发现 (a2a_regi... | 测试依赖 / test_depends |
| 951 | test_a2a_layer1_discovery.py | → | D_INFRA_A2A A2A通信: Agent Card 模型 — A2A Layer 1 Discovery (agent... | 测试依赖 / test_depends |
| 952 | test_a2a_layer1_discovery.py | → | D_INFRA_A2A A2A通信: Identity Verifier — JWT 身份验证器 (identity_v... | 测试依赖 / test_depends |
| 953 | test_a2a_negotiation.py | → | D_INFRA_A2A A2A通信: A2A 协商协议 — Agent 间资源/任务分配协商 (a2a_... | 测试依赖 / test_depends |
| 954 | test_a2a_saga.py | → | D_INFRA_A2A A2A通信: A2A Saga 事务协议 — 多 Agent 跨步分布式事务 (a... | 测试依赖 / test_depends |
| 955 | test_a2a_schemas.py | → | D_INFRA_A2A A2A通信: A2A Message/Part 系统 — Layer 2 Communication ... | 测试依赖 / test_depends |
| 956 | test_a2a_state.py | → | D_INFRA_A2A A2A通信: A2A Task 状态机 — Layer 2 Communication (a2a_s... | 测试依赖 / test_depends |
| 957 | test_a2a_voting.py | → | D_INFRA_A2A A2A通信: A2A 加权投票协议 — 多 Agent 共识达成机制 (a2a_... | 测试依赖 / test_depends |
| 958 | test_a2a_work_steal.py | → | D_INFRA_A2A A2A通信: A2A 工作窃取调度器 — 跨 Agent 负载均衡 (a2a_wo... | 测试依赖 / test_depends |
| 959 | test_f5_auto_shutdown.py | → | D_INFRA_A2A A2A通信: A2A 三级仲裁引擎 — priority -> rule -> escalat... | 测试依赖 / test_depends |
| 960 | F5 端到端集成测试 — boot→run→shutdown→resta... | → | D_INFRA_A2A A2A通信: A2A 三级仲裁引擎 — priority -> rule -> escalat... | 测试依赖 / test_depends |
| 961 | F5 红蓝对抗极端测试 — DM-201513 (test_f5_red_t... | → | D_INFRA_A2A A2A通信: A2A 三级仲裁引擎 — priority -> rule -> escalat... | 测试依赖 / test_depends |
| 962 | F5 红蓝对抗极端测试 — DM-201513 (test_f5_red_t... | → | D_INFRA_A2A A2A通信: 级联守卫——防止失败在Agent间级联 (cascade_guard.py) | 测试依赖 / test_depends |
| 963 | test_arbiter.py | → | D_INFRA_A2A A2A通信: A2A 三级仲裁引擎 — priority -> rule -> escalat... | 测试依赖 / test_depends |
| 964 | test_arbitrator.py | → | D_INFRA_A2A A2A通信: A2A 三级仲裁引擎 — priority -> rule -> escalat... | 测试依赖 / test_depends |
| 965 | test_cascade_guard.py | → | D_INFRA_A2A A2A通信: 级联守卫——防止失败在Agent间级联 (cascade_guard.py) | 测试依赖 / test_depends |
| 966 | test_conflict_detector.py | → | D_INFRA_A2A A2A通信: A2A 冲突检测引擎 — 语义+文本+资源三维冲突检测 ... | 测试依赖 / test_depends |
| 967 | test_deadlock_guard.py | → | D_INFRA_A2A A2A通信: P2: 死锁守卫 (deadlock_guard.py) | 测试依赖 / test_depends |
| 968 | test_livelock_detector.py | → | D_INFRA_A2A A2A通信: P2: 活锁检测器 (livelock_detector.py) | 测试依赖 / test_depends |
| 969 | test_mcp_adapter.py | → | D_INFRA_A2A A2A通信: A2A GovernanceAdapter — Phase 4 治理集成桥接器... | 测试依赖 / test_depends |
| 970 | test_message_router.py | → | D_INFRA_A2A A2A通信: A2A Message/Part 系统 — Layer 2 Communication ... | 测试依赖 / test_depends |
| 971 | test_message_router.py | → | D_INFRA_A2A A2A通信: Message Router — A2A 消息路由 (message_router.py) | 测试依赖 / test_depends |
| 972 | test_push_notifier.py | → | D_INFRA_A2A A2A通信: Push Notifier — A2A 推送通知 (push_notifier.py) | 测试依赖 / test_depends |
| 973 | test_streaming.py | → | D_INFRA_A2A A2A通信: Streaming — A2A 流式传输 (streaming.py) | 测试依赖 / test_depends |
| 974 | test_supervisor.py | → | D_INFRA_A2A A2A通信: A2A Task 状态机 — Layer 2 Communication (a2a_s... | 测试依赖 / test_depends |
| 975 | test_supervisor.py | → | D_INFRA_A2A A2A通信: Supervisor — A2A Layer 3 Coordination (supervi... | 测试依赖 / test_depends |
| 976 | test_trigger_monitor.py | → | D_INFRA_A2A A2A通信: 触发监控器 (trigger_monitor.py) | 测试依赖 / test_depends |
| 977 | test_cross_module_integration_llm_security.py | → | D_INFRA_A2A A2A通信: 基础设施 Infrastructure — A2A Protocol 模块 (M... | 测试依赖 / test_depends |
| 978 | test_multi_agent_root.py | → | D_INFRA_A2A A2A通信: multi_agent.py —— Multi-Agent 编排基座（Phase... | 测试依赖 / test_depends |
| 979 | test_agent_cooldown.py | → | D_INFRA_RECOVERY 回滚恢复: AgentCooldown — Agent 冷却隔离器。 (agent_cool... | 测试依赖 / test_depends |
| 980 | test_auto_rollback_trigger.py | → | D_INFRA_RECOVERY 回滚恢复: AutoRollbackTrigger — 自动回滚触发器。 (auto_r... | 测试依赖 / test_depends |
| 981 | test_ce_kill_switch.py | → | D_INFRA_RECOVERY 回滚恢复: KillSwitchManager — 三级 Kill Switch 管理器。 ... | 测试依赖 / test_depends |
| 982 | test_cross_platform_shell.py | → | D_INFRA_RECOVERY 回滚恢复: CrossPlatformShell — 跨平台 Shell 脚本双输出。... | 测试依赖 / test_depends |
| 983 | test_drift_fix.py | → | D_INFRA_RECOVERY 回滚恢复: drift_fix.py | 测试依赖 / test_depends |
| 984 | test_external_merkle_proof.py | → | D_INFRA_RECOVERY 回滚恢复: External Merkle Proof — 外部可验证回滚完整性证... | 测试依赖 / test_depends |
| 985 | test_git_infra_snapshot.py | → | D_INFRA_RECOVERY 回滚恢复: GitInfraSnapshot — Git 基础设施快照与污染防护... | 测试依赖 / test_depends |
| 986 | test_credential_rotation_trigger.py | → | D_INFRA_RECOVERY 回滚恢复: CredentialRotationTrigger — 凭据自动轮替。 (cr... | 测试依赖 / test_depends |
| 987 | test_secret_rotation_aware.py | → | D_INFRA_RECOVERY 回滚恢复: SecretRotationAware — 密钥轮替感知器。 (secret... | 测试依赖 / test_depends |
| 988 | test_hallucination_guard.py | → | D_INFRA_RECOVERY 回滚恢复: HallucinationGuard — AI 幻觉防护：回滚后强制状... | 测试依赖 / test_depends |
| 989 | test_auditor.py | → | D_INFRA_RECOVERY 回滚恢复: G-CT-004 契约：Rollback -> Audit 记录回滚操作. ... | 测试依赖 / test_depends |
| 990 | test_forensic.py | → | D_INFRA_RECOVERY 回滚恢复: Forensic Engine — 取证基础设施（Phase 8 完整实... | 测试依赖 / test_depends |
| 991 | test_governance_auditor.py | → | D_INFRA_RECOVERY 回滚恢复: G-CT-004 契约：Rollback -> Audit 记录回滚操作. ... | 测试依赖 / test_depends |
| 992 | test_right_to_be_forgotten.py | → | D_INFRA_RECOVERY 回滚恢复: Right to be Forgotten — GDPR 遗忘权合规检查器... | 测试依赖 / test_depends |
| 993 | test_s3_snapshot_lifecycle.py | → | D_INFRA_RECOVERY 回滚恢复: S3 Snapshot Lifecycle Manager — 快照防生命周期... | 测试依赖 / test_depends |
| 994 | test_sqlite_dumper.py | → | D_INFRA_RECOVERY 回滚恢复: SqliteDumper — SQLite 双轨 Checkpoint 的 DB 层... | 测试依赖 / test_depends |
| 995 | test_contract.py | → | D_INFRA_RECOVERY 回滚恢复: CT-RBK-GATE-001 集成契约落地——Rollback System... | 测试依赖 / test_depends |
| 996 | test_submodule_sync.py | → | D_INFRA_RECOVERY 回滚恢复: Submodule Sync — Submodule/Monorepo 多仓库同步... | 测试依赖 / test_depends |
| 997 | test_checkpoint_gc.py | → | D_INFRA_RECOVERY 回滚恢复: CheckpointGC — Checkpoint 垃圾回收。 (checkpoi... | 测试依赖 / test_depends |
| 998 | test_venv_sync.py | → | D_INFRA_RECOVERY 回滚恢复: VenvSync — venv/conda 版本同步保障。 (venv_syn... | 测试依赖 / test_depends |
| 999 | test_env_watcher.py | → | D_INFRA_RECOVERY 回滚恢复: EnvWatcher — 环境变量热重载监控器。 (env_watch... | 测试依赖 / test_depends |
| 1000 | test_runbook_generator.py | → | D_INFRA_RECOVERY 回滚恢复: RunbookGenerator — 回滚操作 Runbook 自动生成。... | 测试依赖 / test_depends |
| 1001 | test_knowngoodstate_ledger.py | → | D_INFRA_RECOVERY 回滚恢复: KnowngoodstateLedger — 已验证正确状态收据。 (k... | 测试依赖 / test_depends |
| 1002 | test_warm_standby.py | → | D_INFRA_RECOVERY 回滚恢复: WarmStandby — 温备热切（git worktree 副本维护... | 测试依赖 / test_depends |
| 1003 | test_vulnerability_rescanner.py | → | D_INFRA_RECOVERY 回滚恢复: VulnerabilityRescanner — 依赖漏洞复扫。 (vulne... | 测试依赖 / test_depends |
| 1004 | test_commit_quality_gate.py | → | D_INFRA_RECOVERY 回滚恢复: CommitQualityGate — Commit 质量基础设施。 (com... | 测试依赖 / test_depends |
| 1005 | test_forward_fix_runner.py | → | D_INFRA_RECOVERY 回滚恢复: ForwardFixRunner — Forward-Fix 执行器。 (forwa... | 测试依赖 / test_depends |
| 1006 | test_topology_change_log.py | → | D_INFRA_RECOVERY 回滚恢复: TopologyChangeLog — 分支拓扑变更日志。 (topolo... | 测试依赖 / test_depends |
| 1007 | test_intent_archiver.py | → | D_INFRA_RECOVERY 回滚恢复: IntentArchiver — 意图存档保护。 (intent_archiv... | 测试依赖 / test_depends |
| 1008 | test_rollback_abuse_detector.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackAbuseDetector — 回滚滥用检测。 (rollba... | 测试依赖 / test_depends |
| 1009 | test_rollback_audit_nexus.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackAuditNexus — 回滚审计记录聚合到 Nexus ... | 测试依赖 / test_depends |
| 1010 | test_rollback_bootstrap.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackBootstrap — 零依赖自举回滚器。 (rollba... | 测试依赖 / test_depends |
| 1011 | test_rollback_budget.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackBudget — 回滚预算管理器。 (rollback_bu... | 测试依赖 / test_depends |
| 1012 | Extreme tests for concurrent rollback (MOD-INF-... | → | D_INFRA_RECOVERY 回滚恢复: RollbackExecutor — 回滚执行器核心封装。 (rollb... | 测试依赖 / test_depends |
| 1013 | Extreme tests for concurrent rollback (MOD-INF-... | → | D_INFRA_RECOVERY 回滚恢复: RollbackLock — 全局回滚锁管理。 (rollback_lock.py) | 测试依赖 / test_depends |
| 1014 | test_rollback_context_restorer.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackContextRestorer — 上下文恢复器。 (roll... | 测试依赖 / test_depends |
| 1015 | test_rollback_dashboard.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackDashboard — 回滚仪表盘（零依赖 Markdow... | 测试依赖 / test_depends |
| 1016 | test_rollback_drill.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackDrill — 定期回滚演练调度器 (DiRT-style... | 测试依赖 / test_depends |
| 1017 | test_rollback_executor_root.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackExecutor — 回滚执行器核心封装。 (rollb... | 测试依赖 / test_depends |
| 1018 | test_rollback_integration.py | → | D_INFRA_RECOVERY 回滚恢复: Rollback Integration — executor 集成增强层。 (... | 测试依赖 / test_depends |
| 1019 | test_rollback_lock.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackLock — 全局回滚锁管理。 (rollback_lock.py) | 测试依赖 / test_depends |
| 1020 | test_rollback_loop_detector.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackLoopDetector — 回滚循环检测器。 (rollb... | 测试依赖 / test_depends |
| 1021 | Extreme tests for partial_revert (MOD-INF-021 B... | → | D_INFRA_RECOVERY 回滚恢复: RollbackExecutor — 回滚执行器核心封装。 (rollb... | 测试依赖 / test_depends |
| 1022 | Extreme tests for partial_revert (MOD-INF-021 B... | → | D_INFRA_RECOVERY 回滚恢复: RollbackLock — 全局回滚锁管理。 (rollback_lock.py) | 测试依赖 / test_depends |
| 1023 | test_rollback_simulator.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackSimulator — 回滚模拟器（CI 集成）。 (r... | 测试依赖 / test_depends |
| 1024 | test_rollback_state_machine.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackStateMachine — 回滚步骤级状态机。 (rol... | 测试依赖 / test_depends |
| 1025 | test_rollback_target_staleness.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackTargetStaleness — 回滚目标陈旧度检测。... | 测试依赖 / test_depends |
| 1026 | test_rollback_verifier_root.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackVerifier — 回滚后验证器。 (rollback_ve... | 测试依赖 / test_depends |
| 1027 | test_rollback_wal.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackWAL — 回滚预写日志。 (rollback_wal.py) | 测试依赖 / test_depends |
| 1028 | test_semantic_rollback_tag.py | → | D_INFRA_RECOVERY 回滚恢复: SemanticRollbackTag — 语义化 Rollback Tag 管理... | 测试依赖 / test_depends |
| 1029 | test_semantic_similar_detector.py | → | D_INFRA_RECOVERY 回滚恢复: SemanticSimilarDetector — 语义变形攻击检测。 (... | 测试依赖 / test_depends |
| 1030 | test_temporal_context_adapter.py | → | D_INFRA_RECOVERY 回滚恢复: TemporalContextAdapter — AI 时间上下文断裂修复... | 测试依赖 / test_depends |
| 1031 | test_state_machine.py | → | D_INFRA_RUNTIME 运行时集成: state_machine.py | 测试依赖 / test_depends |
| 1032 | test_auto_bootstrap.py | → | D_INFRA_RUNTIME 运行时集成: Batch2 治理层契约 — 15条 Pydantic v2 Schema（P... | runtime / runtime |
| 1033 | test_auto_diagnostics.py | → | D_INFRA_RUNTIME 运行时集成: RI-12 AutoDiagnostics — 自动诊断引擎 (auto_dia... | 测试依赖 / test_depends |
| 1034 | DM-202508 验收测试: F15注册到phase_manager实现.... | → | D_INFRA_RUNTIME 运行时集成: engine.py | 测试依赖 / test_depends |
| 1035 | DM-202508 验收测试: F15注册到phase_manager实现.... | → | D_INFRA_RUNTIME 运行时集成: fix_scheduler.py | 测试依赖 / test_depends |
| 1036 | F15 自动修复引擎 - 红蓝对抗极端测试 (test_auto_... | → | D_INFRA_RUNTIME 运行时集成: fix_budget.py | 测试依赖 / test_depends |
| 1037 | F15 自动修复引擎 - 红蓝对抗极端测试 (test_auto_... | → | D_INFRA_RUNTIME 运行时集成: fix_reliability.py | 测试依赖 / test_depends |
| 1038 | F15 自动修复引擎 - 红蓝对抗极端测试 (test_auto_... | → | D_INFRA_RUNTIME 运行时集成: fix_safety.py | 测试依赖 / test_depends |
| 1039 | F15 自动修复引擎 - 红蓝对抗极端测试 (test_auto_... | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1040 | F15 自动修复引擎 - 红蓝对抗极端测试 (test_auto_... | → | D_INFRA_RUNTIME 运行时集成: self_heal_agent.py | 测试依赖 / test_depends |
| 1041 | F15 自动修复引擎 - 红蓝对抗极端测试 (test_auto_... | → | D_INFRA_RUNTIME 运行时集成: shadow_workspace.py | 测试依赖 / test_depends |
| 1042 | F11 ContextPipeline 红蓝对抗极端测试 (test_cont... | → | D_INFRA_RUNTIME 运行时集成: kill_switch.py -- safety circuit breaker (DD110... | 测试依赖 / test_depends |
| 1043 | test_host_resource_governor.py | → | D_INFRA_RUNTIME 运行时集成: host_resource_governor.py — 主机资源治理 (B17,... | 测试依赖 / test_depends |
| 1044 | test_support_system_snapshot.py | → | D_INFRA_RUNTIME 运行时集成: SystemSnapshotter — M1 系统状态镜像（CL-017 RI... | 测试依赖 / test_depends |
| 1045 | test_system_snapshot_root.py | → | D_INFRA_RUNTIME 运行时集成: SystemSnapshotter — M1 系统状态镜像（CL-017 RI... | 测试依赖 / test_depends |
| 1046 | test_token_budget_root.py | → | D_INFRA_RUNTIME 运行时集成: token_budget.py — Token 估算工具 SSoT (token_b... | 测试依赖 / test_depends |
| 1047 | test_ba_state_machine.py | → | D_INFRA_RUNTIME 运行时集成: state_machine.py | 测试依赖 / test_depends |
| 1048 | test_budget_forecaster.py | → | D_INFRA_RUNTIME 运行时集成: budget_forecaster.py — Token 预算预测 (DD120-e... | 测试依赖 / test_depends |
| 1049 | test_config_validator.py | → | D_INFRA_RUNTIME 运行时集成: M-12 ConfigValidator — 配置参数校验器 (config_... | 测试依赖 / test_depends |
| 1050 | F11 ContextPipeline 三层自动化机制测试 (test_co... | → | D_INFRA_RUNTIME 运行时集成: kill_switch.py -- safety circuit breaker (DD110... | 测试依赖 / test_depends |
| 1051 | test_contract_tester.py | → | D_INFRA_RUNTIME 运行时集成: M-11 ContractTester — 契约测试框架 (contract_t... | 测试依赖 / test_depends |
| 1052 | test_ct_pipe_routing_root.py | → | D_INFRA_RUNTIME 运行时集成: CT-PIPE-ORC-001 — TaskCard -> 管线入口节点路由... | 测试依赖 / test_depends |
| 1053 | test_ct_pipe_routing_root.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline 数据模型 (models.py) | 测试依赖 / test_depends |
| 1054 | test_dependency_root.py | → | D_INFRA_RUNTIME 运行时集成: MOD-INF-026 §18 — 资产依赖图。 (dependency.py) | 测试依赖 / test_depends |
| 1055 | test_drift_fixer.py | → | D_INFRA_RUNTIME 运行时集成: drift_fixer.py | 测试依赖 / test_depends |
| 1056 | test_drift_fixer.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1057 | test_escalation_bridge.py | → | D_INFRA_RUNTIME 运行时集成: escalation_bridge.py | 测试依赖 / test_depends |
| 1058 | test_escalation_bridge.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1059 | test_event_bus_upgrade.py | → | D_INFRA_RUNTIME 运行时集成: DEPRECATED: 此文件已废弃。 (event_bus_upgrade.py) | 测试依赖 / test_depends |
| 1060 | test_event_hooks.py | → | D_INFRA_RUNTIME 运行时集成: event_hooks.py | 测试依赖 / test_depends |
| 1061 | test_event_hooks.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1062 | test_event_store.py | → | D_INFRA_RUNTIME 运行时集成: RI-13 EventStore — 事件存储 (event_store.py) | 测试依赖 / test_depends |
| 1063 | F21 自动关闭测试 — DM-201250 (test_f21_auto_sh... | → | D_INFRA_RUNTIME 运行时集成: health.py —— ZephyrAlpha 聚合健康检查 (health.py) | 测试依赖 / test_depends |
| 1064 | F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | D_INFRA_RUNTIME 运行时集成: health.py —— ZephyrAlpha 聚合健康检查 (health.py) | 测试依赖 / test_depends |
| 1065 | F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | D_INFRA_RUNTIME 运行时集成: CT-HEALTH-001: System-wide Health Discovery Reg... | 测试依赖 / test_depends |
| 1066 | F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | D_INFRA_RUNTIME 运行时集成: longevity_monitor.py | 测试依赖 / test_depends |
| 1067 | F21 事件启动测试 — DM-201250 (test_f21_event_d... | → | D_INFRA_RUNTIME 运行时集成: health.py —— ZephyrAlpha 聚合健康检查 (health.py) | 测试依赖 / test_depends |
| 1068 | test_lifecycle_hooks.py | → | D_INFRA_RUNTIME 运行时集成: hooks.py —— 模块生命周期钩子（Phase 2 新增 | ... | 测试依赖 / test_depends |
| 1069 | test_file_watcher.py | → | D_INFRA_RUNTIME 运行时集成: file_watcher.py | 测试依赖 / test_depends |
| 1070 | test_fix_budget.py | → | D_INFRA_RUNTIME 运行时集成: fix_budget.py | 测试依赖 / test_depends |
| 1071 | test_fix_budget.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1072 | test_fix_diff.py | → | D_INFRA_RUNTIME 运行时集成: fix_diff.py | 测试依赖 / test_depends |
| 1073 | test_fix_diff.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1074 | test_fix_health_check.py | → | D_INFRA_RUNTIME 运行时集成: fix_health_check.py | 测试依赖 / test_depends |
| 1075 | test_fix_health_check.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1076 | test_fix_pattern_miner.py | → | D_INFRA_RUNTIME 运行时集成: fix_pattern_miner.py | 测试依赖 / test_depends |
| 1077 | test_fix_pattern_miner.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1078 | test_fix_reliability.py | → | D_INFRA_RUNTIME 运行时集成: fix_reliability.py | 测试依赖 / test_depends |
| 1079 | test_fix_reliability.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1080 | test_fix_report.py | → | D_INFRA_RUNTIME 运行时集成: fix_report.py | 测试依赖 / test_depends |
| 1081 | test_fix_report.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1082 | test_fix_safety.py | → | D_INFRA_RUNTIME 运行时集成: fix_safety.py | 测试依赖 / test_depends |
| 1083 | test_fix_safety.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1084 | test_fix_scheduler.py | → | D_INFRA_RUNTIME 运行时集成: fix_scheduler.py | 测试依赖 / test_depends |
| 1085 | test_fix_scheduler.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1086 | test_classifier_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetClassifier — MOD-INF-026 L2 资产自动分类... | 测试依赖 / test_depends |
| 1087 | test_classifier_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetInventoryModels — MOD-INF-026 Pydantic V2... | 测试依赖 / test_depends |
| 1088 | test_cost_tracker.py | → | D_INFRA_RUNTIME 运行时集成: RI-15 CostTracker — 成本追踪器 (cost_tracker.py) | 测试依赖 / test_depends |
| 1089 | test_dashboard_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetDashboard — MOD-INF-026 资产健康仪表盘生... | 测试依赖 / test_depends |
| 1090 | test_dashboard_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetInventoryModels — MOD-INF-026 Pydantic V2... | 测试依赖 / test_depends |
| 1091 | test_dry_run_simulator.py | → | D_INFRA_RUNTIME 运行时集成: RI-14 DryRunSimulator — 干运行模拟器 (dry_run_... | 测试依赖 / test_depends |
| 1092 | test_finding_task_bridge.py | → | D_INFRA_RUNTIME 运行时集成: Finding->TaskCard 桥接器 (finding_task_bridge.py) | 测试依赖 / test_depends |
| 1093 | test_index_generator_root.py | → | D_INFRA_RUNTIME 运行时集成: UnifiedAssetIndex — MOD-INF-026 L3 统一资产索.... | 测试依赖 / test_depends |
| 1094 | test_index_generator_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetInventoryModels — MOD-INF-026 Pydantic V2... | 测试依赖 / test_depends |
| 1095 | test_infrastructure_base.py | → | D_INFRA_RUNTIME 运行时集成: 基础设施 — Infrastructure Layer Skeleton (infr... | 测试依赖 / test_depends |
| 1096 | test_kill_switch_sim.py | → | D_INFRA_RUNTIME 运行时集成: Kill Switch T0 Hardware Simulator (kill_switch_... | 测试依赖 / test_depends |
| 1097 | test_lifecycle_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetLifecycle — MOD-INF-026 L5 ITIL生命周期自... | 测试依赖 / test_depends |
| 1098 | test_lifecycle_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetInventoryModels — MOD-INF-026 Pydantic V2... | 测试依赖 / test_depends |
| 1099 | test_metadata.py | → | D_INFRA_RUNTIME 运行时集成: MOD-INF-026 §24-25 — Git 历史元数据提取 + 多 ... | 测试依赖 / test_depends |
| 1100 | test_preemption_manager.py | → | D_INFRA_RUNTIME 运行时集成: PreemptionManager -- 优先级抢占管理器 (preempti... | 测试依赖 / test_depends |
| 1101 | test_pydantic_v2_migrator.py | → | D_INFRA_RUNTIME 运行时集成: M-15 PydanticV2Migrator — Pydantic V2 迁移工具... | 测试依赖 / test_depends |
| 1102 | test_reconciler_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetInventoryModels — MOD-INF-026 Pydantic V2... | 测试依赖 / test_depends |
| 1103 | test_reconciler_root.py | → | D_INFRA_RUNTIME 运行时集成: ReconciliationEngine — MOD-INF-026 L4 注册表 v... | 测试依赖 / test_depends |
| 1104 | test_registry_adapter_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetInventoryModels — MOD-INF-026 Pydantic V2... | 测试依赖 / test_depends |
| 1105 | test_registry_adapter_root.py | → | D_INFRA_RUNTIME 运行时集成: MOD-INF-026 §17 — 24 个异构注册表统一解析适配... | 测试依赖 / test_depends |
| 1106 | [INVARIANTS] 功能域注册表是功能域声明的唯一真源... | → | D_INFRA_RUNTIME 运行时集成: Registry Governance — MOD-INF-037 (registry_go... | 测试依赖 / test_depends |
| 1107 | test_registry_governance_root.py | → | D_INFRA_RUNTIME 运行时集成: Registry Governance — MOD-INF-037 (registry_go... | 测试依赖 / test_depends |
| 1108 | test_scanner_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetInventoryModels — MOD-INF-026 Pydantic V2... | 测试依赖 / test_depends |
| 1109 | test_scanner_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetDiscoveryScanner — MOD-INF-026 L1 全量文.... | 测试依赖 / test_depends |
| 1110 | test_telemetry.py | → | D_INFRA_RUNTIME 运行时集成: AssetInventoryTelemetry — MOD-INF-026 自监控指... | 测试依赖 / test_depends |
| 1111 | test_trust_anchor_root.py | → | D_INFRA_RUNTIME 运行时集成: MOD-INF-026 §26 — 三重信任锚验证门 R20。 (tru... | 测试依赖 / test_depends |
| 1112 | test_warm_hot_gate.py | → | D_INFRA_RUNTIME 运行时集成: M-14 WarmHotGate — Warm->Hot 阻断门 (warm_hot_... | 测试依赖 / test_depends |
| 1113 | test_cross_module_integration_llm_security.py | → | D_INFRA_RUNTIME 运行时集成: MOD-INF-019: Agent Spec — LLM Gateway (llm_gat... | 测试依赖 / test_depends |
| 1114 | test_dep_version_fixer.py | → | D_INFRA_RUNTIME 运行时集成: __init__.py | 测试依赖 / test_depends |
| 1115 | test_dep_version_fixer.py | → | D_INFRA_RUNTIME 运行时集成: dep_version_fixer.py | 测试依赖 / test_depends |
| 1116 | test_dep_version_fixer.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1117 | test_engine_root.py | → | D_INFRA_RUNTIME 运行时集成: engine.py | 测试依赖 / test_depends |
| 1118 | test_engine_root.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1119 | test_interrupt_guard.py | → | D_INFRA_RUNTIME 运行时集成: interrupt_guard.py | 测试依赖 / test_depends |
| 1120 | test_llm_fix_adapter.py | → | D_INFRA_RUNTIME 运行时集成: llm_fix_adapter.py | 测试依赖 / test_depends |
| 1121 | test_llm_fix_adapter.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1122 | test_llm_gateway.py | → | D_INFRA_RUNTIME 运行时集成: MOD-INF-019: Agent Spec — LLM Gateway (llm_gat... | 测试依赖 / test_depends |
| 1123 | test_models_root.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1124 | test_scaffold_registrar.py | → | D_INFRA_RUNTIME 运行时集成: __init__.py | 测试依赖 / test_depends |
| 1125 | test_scaffold_registrar.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1126 | test_scaffold_registrar.py | → | D_INFRA_RUNTIME 运行时集成: scaffold_registrar.py | 测试依赖 / test_depends |
| 1127 | test_shadow_workspace.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1128 | test_shadow_workspace.py | → | D_INFRA_RUNTIME 运行时集成: shadow_workspace.py | 测试依赖 / test_depends |
| 1129 | test_zombie_cleaner.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1130 | test_zombie_cleaner.py | → | D_INFRA_RUNTIME 运行时集成: zombie_cleaner.py | 测试依赖 / test_depends |
| 1131 | test_model_router.py | → | D_INFRA_RUNTIME 运行时集成: ModelRouter — 模型路由与降级链管理 (model_rout... | 测试依赖 / test_depends |
| 1132 | test_observability_health.py | → | D_INFRA_RUNTIME 运行时集成: hooks.py —— 模块生命周期钩子（Phase 2 新增 | ... | 测试依赖 / test_depends |
| 1133 | test_pipeline_agent_bridge.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline 数据模型 (models.py) | 测试依赖 / test_depends |
| 1134 | test_pipeline_agent_bridge.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline -> Agent Bridge — 双编排器桥接层 (pip... | 测试依赖 / test_depends |
| 1135 | test_pipeline_cost_tracker.py | → | D_INFRA_RUNTIME 运行时集成: CostTracker —— LLM 调用成本追踪器（SRC-0025）... | 测试依赖 / test_depends |
| 1136 | test_pipeline_cost_tracker.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline 数据模型 (models.py) | 测试依赖 / test_depends |
| 1137 | test_pipeline_lock.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline Lock — 双管线并发锁 (pipeline_lock.py) | 测试依赖 / test_depends |
| 1138 | test_pipeline_models.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline 数据模型 (models.py) | 测试依赖 / test_depends |
| 1139 | DM-202010: PipelineOrchestrator 自动启动/周期运... | → | D_INFRA_RUNTIME 运行时集成: Pipeline 数据模型 (models.py) | 测试依赖 / test_depends |
| 1140 | test_pipeline_roadmap.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline 未来版本路线图——v0.10.0 -> v0.12.0 .... | 测试依赖 / test_depends |
| 1141 | test_resource_optimization.py | → | D_INFRA_RUNTIME 运行时集成: models.py - Pydantic data models for resource o... | 测试依赖 / test_depends |
| 1142 | test_backpressure_manager.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline — Backpressure Manager (backpressure_... | 测试依赖 / test_depends |
| 1143 | test_backpressure_manager.py | → | D_INFRA_RUNTIME 运行时集成: backpressure_types.py - Pipeline backpressure s... | 测试依赖 / test_depends |
| 1144 | test_backpressure_types.py | → | D_INFRA_RUNTIME 运行时集成: backpressure_types.py - Pipeline backpressure s... | 测试依赖 / test_depends |
| 1145 | test_circuit_breaker_manager.py | → | D_INFRA_RUNTIME 运行时集成: CircuitBreakerManager -- standalone circuit bre... | 测试依赖 / test_depends |
| 1146 | test_circuit_breaker_manager.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline 数据模型 (models.py) | 测试依赖 / test_depends |
| 1147 | test_dead_letter_queue.py | → | D_INFRA_RUNTIME 运行时集成: DeadLetterQueue — 死信队列 (dead_letter_queue.py) | 测试依赖 / test_depends |
| 1148 | test_dead_letter_queue.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline 数据模型 (models.py) | 测试依赖 / test_depends |
| 1149 | F14 管线编排/反馈环 — 红蓝对抗端到端极端测试 (... | → | D_INFRA_RUNTIME 运行时集成: Pipeline — Backpressure Manager (backpressure_... | 测试依赖 / test_depends |
| 1150 | F14 管线编排/反馈环 — 红蓝对抗端到端极端测试 (... | → | D_INFRA_RUNTIME 运行时集成: backpressure_types.py - Pipeline backpressure s... | 测试依赖 / test_depends |
| 1151 | F14 管线编排/反馈环 — 红蓝对抗端到端极端测试 (... | → | D_INFRA_RUNTIME 运行时集成: DeadLetterQueue — 死信队列 (dead_letter_queue.py) | 测试依赖 / test_depends |
| 1152 | F14 管线编排/反馈环 — 红蓝对抗端到端极端测试 (... | → | D_INFRA_RUNTIME 运行时集成: Pipeline 数据模型 (models.py) | 测试依赖 / test_depends |
| 1153 | test_routing_plugins.py | → | D_INFRA_RUNTIME 运行时集成: CT-PIPE-ORC-001 — TaskCard -> 管线入口节点路由... | 测试依赖 / test_depends |
| 1154 | test_routing_plugins.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline 数据模型 (models.py) | 测试依赖 / test_depends |
| 1155 | test_routing_plugins.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline Routing Plugin System — K8s Schedulin... | 测试依赖 / test_depends |
| 1156 | test_otel_instrumentation.py | → | D_INFRA_TELEMETRY 可观测性: otel_instrumentation.py — 全链路 OTel (B12, DD... | 测试依赖 / test_depends |
| 1157 | test_observability_health.py | → | D_INFRA_TELEMETRY 可观测性: health subsystem — 模块健康注册与 LifecycleMan... | 测试依赖 / test_depends |
| 1158 | test_auto_runtime_core.py | → | D_INTEGRATION 管线路由: DeepSeekChat — 通过 DeepSeek API 进行 LLM 推理... | 测试依赖 / test_depends |
| 1159 | test_auto_runtime_core.py | → | D_INTEGRATION 管线路由: EmbeddingRouter — MOD-INF-011 双嵌入维度路由 (... | 测试依赖 / test_depends |
| 1160 | test_auto_runtime_core.py | → | D_INTEGRATION 管线路由: LocalModelScheduler — L2 本地模型 24/7 调度循... | 测试依赖 / test_depends |
| 1161 | test_auto_runtime_core.py | → | D_INTEGRATION 管线路由: OllamaChat — 通过 Ollama HTTP API 进行本地 LLM... | 测试依赖 / test_depends |
| 1162 | test_auto_runtime_core.py | → | D_INTEGRATION 管线路由: InProcessVectorMemory — MOD-INF-011 VMS 统一入... | 测试依赖 / test_depends |
| 1163 | test_auto_split.py | → | D_INTEGRATION 管线路由: execution_model.py | 测试依赖 / test_depends |
| 1164 | test_auto_split.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1165 | test_ct_audit_findings_resolved.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1166 | test_ct_blueprint_read_check.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1167 | test_ct_circuit_breaker.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1168 | test_ct_circular_dependency_scan.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1169 | test_ct_classification.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1170 | test_ct_content_length.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1171 | test_ct_content_quality.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1172 | test_ct_contract_compatibility_check.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1173 | test_ct_deduplication.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1174 | test_ct_drift_budget.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1175 | test_ct_encoding.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1176 | test_ct_enforcement_mode_check.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1177 | test_ct_field_presence.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1178 | test_ct_file_extension.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1179 | test_ct_fle_gate.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1180 | test_ct_frontmatter.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1181 | test_ct_leverage_limit.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1182 | test_ct_line_ending.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1183 | test_ct_manual_approval.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1184 | test_ct_path_blacklist.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1185 | test_ct_path_routing.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1186 | test_ct_path_whitelist.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1187 | test_ct_position_limit.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1188 | test_ct_reference_check.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1189 | test_ct_regex_pattern.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1190 | test_ct_restructuring_safety.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1191 | test_ct_rollback_exit_code.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1192 | test_ct_score_threshold.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1193 | test_ct_security_artifact_scan.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1194 | test_ct_strategy_correlation.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1195 | test_ct_temporal.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1196 | test_ct_zero_residue_check.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1197 | test_gate_types.py | → | D_INTEGRATION 管线路由: schemas.py | 测试依赖 / test_depends |
| 1198 | test_schema_schema_registry.py | → | D_INTEGRATION 管线路由: schema_registry.py | 测试依赖 / test_depends |
| 1199 | test_schema_schemas.py | → | D_INTEGRATION 管线路由: schemas.py | 测试依赖 / test_depends |
| 1200 | test_schema_schemas.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1201 | test_boot_hooks_unlock.py | → | D_INTEGRATION 管线路由: execution_model.py | 测试依赖 / test_depends |
| 1202 | test_boot_hooks_unlock.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1203 | test_cross_module_integration_llm_security.py | → | D_INTEGRATION 管线路由: PipelineOrchestrator — M1-M11 管线协调器 (pipe... | 测试依赖 / test_depends |
| 1204 | test_db.py | → | D_INTEGRATION 管线路由: base_config.py | 测试依赖 / test_depends |
| 1205 | test_db.py | → | D_INTEGRATION 管线路由: execution_model.py | 测试依赖 / test_depends |
| 1206 | test_db.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1207 | DM-202208 红蓝对抗-知识污染与检索劫持测试 (test... | → | D_INTEGRATION 管线路由: HybridRetriever — MOD-INF-011 混合检索架构 (hy... | 测试依赖 / test_depends |
| 1208 | VMS 红蓝对抗测试 — 向量注入与投毒检测 (test_vm... | → | D_INTEGRATION 管线路由: EmbeddingRouter — MOD-INF-011 双嵌入维度路由 (... | 测试依赖 / test_depends |
| 1209 | VMS 红蓝对抗测试 — 向量注入与投毒检测 (test_vm... | → | D_INTEGRATION 管线路由: InMemoryFakeVMS — MOD-INF-011 · 零依赖测试双... | 测试依赖 / test_depends |
| 1210 | VMS 红蓝对抗测试 — 向量注入与投毒检测 (test_vm... | → | D_INTEGRATION 管线路由: ProvenanceEnforcer — MOD-INF-011 写入溯源强制... | 测试依赖 / test_depends |
| 1211 | VMS 红蓝对抗测试 — 向量注入与投毒检测 (test_vm... | → | D_INTEGRATION 管线路由: VMS 共享数据模型 — MOD-INF-011 · 蓝图 §6.1 .... | 测试依赖 / test_depends |
| 1212 | DM-202210 自动化机制-事件触发与定时任务测试 (te... | → | D_INTEGRATION 管线路由: CacheLayer — MOD-INF-011 嵌入缓存与查询结果 LR... | 测试依赖 / test_depends |
| 1213 | DM-202210 自动化机制-事件触发与定时任务测试 (te... | → | D_INTEGRATION 管线路由: CollectionManager — MOD-INF-011 八大 Collectio... | 测试依赖 / test_depends |
| 1214 | DM-202210 自动化机制-事件触发与定时任务测试 (te... | → | D_INTEGRATION 管线路由: InProcessVectorMemory — MOD-INF-011 VMS 统一入... | 测试依赖 / test_depends |
| 1215 | DM-202210 自动化机制-事件触发与定时任务测试 (te... | → | D_INTEGRATION 管线路由: IndexHealthMonitor — MOD-INF-011 索引健康自检.... | 测试依赖 / test_depends |
| 1216 | DM-202210 自动化机制-事件触发与定时任务测试 (te... | → | D_INTEGRATION 管线路由: RetrievalFeedback — MOD-INF-011 FLE 检索质量消... | 测试依赖 / test_depends |
| 1217 | DM-202209 自动化机制-启动与关闭生命周期测试 (te... | → | D_INTEGRATION 管线路由: EmbeddingRouter — MOD-INF-011 双嵌入维度路由 (... | 测试依赖 / test_depends |
| 1218 | DM-202209 自动化机制-启动与关闭生命周期测试 (te... | → | D_INTEGRATION 管线路由: InMemoryFakeVMS — MOD-INF-011 · 零依赖测试双... | 测试依赖 / test_depends |
| 1219 | DM-202209 自动化机制-启动与关闭生命周期测试 (te... | → | D_INTEGRATION 管线路由: InProcessVectorMemory — MOD-INF-011 VMS 统一入... | 测试依赖 / test_depends |
| 1220 | DM-202010: PipelineOrchestrator 自动启动/周期运... | → | D_INTEGRATION 管线路由: PipelineOrchestrator — M1-M11 管线协调器 (pipe... | 测试依赖 / test_depends |
| 1221 | test_pipeline_orchestrator_root.py | → | D_INTEGRATION 管线路由: PipelineOrchestrator — M1-M11 管线协调器 (pipe... | 测试依赖 / test_depends |
| 1222 | test_task_types.py | → | D_INTEGRATION 管线路由: base_config.py | 测试依赖 / test_depends |
| 1223 | test_task_types.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1224 | test_utils_testing.py | → | D_INTEGRATION 管线路由: schemas.py | 测试依赖 / test_depends |
| 1225 | test_utils_testing.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1226 | test_mcp_task_claim.py | → | D_INTEGRATION_GATEWAY 集成网关: ZephyrAlpha MCP Task Manager Server (task_manag... | 测试依赖 / test_depends |
| 1227 | test_cross_module_integration_llm_security.py | → | D_INTEGRATION_GATEWAY 集成网关: MCP Gateway 集中式治理节点（MOD-INF-013 §12 Ph... | 测试依赖 / test_depends |
| 1228 | test_capability_passport.py | → | D_INTELLIGENCE 上下文管理: CapabilityPassport --- AI 模型能力护照 (capabil... | 测试依赖 / test_depends |
| 1229 | test_cross_layer.py | → | D_INTELLIGENCE 上下文管理: inference_base.py | 测试依赖 / test_depends |
| 1230 | DM-202009: F10 红蓝对抗测试套件。 (test_f10_red... | → | D_INTELLIGENCE 上下文管理: CapabilityPassport --- AI 模型能力护照 (capabil... | 测试依赖 / test_depends |
| 1231 | DM-202009: F10 红蓝对抗测试套件。 (test_f10_red... | → | D_INTELLIGENCE 上下文管理: ExamOrchestrator --- 五轴入职考试主控 (exam_orc... | 测试依赖 / test_depends |
| 1232 | DM-202009: F10 红蓝对抗测试套件。 (test_f10_red... | → | D_INTELLIGENCE 上下文管理: ExamTestCases --- v3.0.5 扩展考试题库（96 题 / ... | 测试依赖 / test_depends |
| 1233 | DM-202009: F10 红蓝对抗测试套件。 (test_f10_red... | → | D_INTELLIGENCE 上下文管理: Results Writer — 持久化 benchmark 结果，支持历... | 测试依赖 / test_depends |
| 1234 | test_kb_activate.py | → | D_INTELLIGENCE 上下文管理: G4 Activate 门禁 — 人工激活（T-2-13-D） (activ... | 测试依赖 / test_depends |
| 1235 | test_kb_pipeline_activate.py | → | D_INTELLIGENCE 上下文管理: G4 Activate 门禁 — 人工激活（T-2-13-D） (activ... | 测试依赖 / test_depends |
| 1236 | test_kb_reranker.py | → | D_INTELLIGENCE 上下文管理: Cross-Encoder 重排序层 — BGE-reranker-v2-m3（T... | 测试依赖 / test_depends |
| 1237 | test_kb_unified_memory_api.py | → | D_INTELLIGENCE 上下文管理: UnifiedMemoryAPI — RI-02 统一记忆 API（M2 跨模... | 测试依赖 / test_depends |
| 1238 | test_benchmark_suite.py | → | D_INTELLIGENCE 上下文管理: BenchmarkSuite — 多维度模型性能测试用例集 (ben... | 测试依赖 / test_depends |
| 1239 | calibrate_model_diff.py 单元测试（P1-3 配套, 零... | → | D_INTELLIGENCE 上下文管理: CapabilityPassport --- AI 模型能力护照 (capabil... | 测试依赖 / test_depends |
| 1240 | test_cli.py | → | D_INTELLIGENCE 上下文管理: model-profiler.cli — 模型性能检测命令行入口 (c... | 测试依赖 / test_depends |
| 1241 | test_deepseek_v4_chat.py | → | D_INTELLIGENCE 上下文管理: DeepSeekV4Chat --- DeepSeek V4 系列模型 API 客... | 测试依赖 / test_depends |
| 1242 | test_exam_orchestrator.py | → | D_INTELLIGENCE 上下文管理: CapabilityPassport --- AI 模型能力护照 (capabil... | 测试依赖 / test_depends |
| 1243 | test_exam_orchestrator.py | → | D_INTELLIGENCE 上下文管理: ExamJudge --- LLM-as-judge 评分器 (exam_judge.py) | 测试依赖 / test_depends |
| 1244 | test_exam_orchestrator.py | → | D_INTELLIGENCE 上下文管理: ExamOrchestrator --- 五轴入职考试主控 (exam_orc... | 测试依赖 / test_depends |
| 1245 | test_exam_orchestrator.py | → | D_INTELLIGENCE 上下文管理: ExamTestCases --- v3.0.5 扩展考试题库（96 题 / ... | 测试依赖 / test_depends |
| 1246 | test_exam_test_cases.py | → | D_INTELLIGENCE 上下文管理: ExamTestCases --- v3.0.5 扩展考试题库（96 题 / ... | 测试依赖 / test_depends |
| 1247 | test_job_matcher.py | → | D_INTELLIGENCE 上下文管理: CapabilityPassport --- AI 模型能力护照 (capabil... | 测试依赖 / test_depends |
| 1248 | test_job_matcher.py | → | D_INTELLIGENCE 上下文管理: JobMatcher --- 模型岗位匹配器 (job_matcher.py) | 测试依赖 / test_depends |
| 1249 | test_model_discovery.py | → | D_INTELLIGENCE 上下文管理: ModelDiscovery — 枚举所有本地 Ollama 模型 + 远... | 测试依赖 / test_depends |
| 1250 | test_model_drift_detector.py | → | D_INTELLIGENCE 上下文管理: ModelDriftDetector — LLM 模型行为漂移检测。 (m... | 测试依赖 / test_depends |
| 1251 | test_profiler.py | → | D_INTELLIGENCE 上下文管理: BenchmarkSuite — 多维度模型性能测试用例集 (ben... | 测试依赖 / test_depends |
| 1252 | test_profiler.py | → | D_INTELLIGENCE 上下文管理: ModelProfiler — 核心性能分析引擎 (profiler.py) | 测试依赖 / test_depends |
| 1253 | test_provider_data.py | → | D_INTELLIGENCE 上下文管理: provider_data.py | 测试依赖 / test_depends |
| 1254 | test_results_writer.py | → | D_INTELLIGENCE 上下文管理: ModelProfiler — 核心性能分析引擎 (profiler.py) | 测试依赖 / test_depends |
| 1255 | test_results_writer.py | → | D_INTELLIGENCE 上下文管理: Results Writer — 持久化 benchmark 结果，支持历... | 测试依赖 / test_depends |
| 1256 | test_task_gate.py | → | D_INTELLIGENCE 上下文管理: CapabilityPassport --- AI 模型能力护照 (capabil... | 测试依赖 / test_depends |
| 1257 | test_task_model_learner.py | → | D_INTELLIGENCE 上下文管理: ModelTaskMatrix — 任务×模型性能学习引擎 (task... | 测试依赖 / test_depends |
| 1258 | F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | D_OPS 反馈循环: metrics.py —— 轻量级 Metrics 收集基础设施（Ph... | 测试依赖 / test_depends |
| 1259 | F21 事件启动测试 — DM-201250 (test_f21_event_d... | → | D_OPS 反馈循环: metrics.py —— 轻量级 Metrics 收集基础设施（Ph... | 测试依赖 / test_depends |
| 1260 | test_observability_metrics.py | → | D_OPS 反馈循环: metrics.py —— 轻量级 Metrics 收集基础设施（Ph... | 测试依赖 / test_depends |
| 1261 | test_observability_tracing.py | → | D_OPS 反馈循环: tracing.py —— OpenTelemetry 分布式追踪（Phase... | 测试依赖 / test_depends |
| 1262 | test_e_position_reconciler.py | → | D_POSITION 仓位管理: Position Reconciler — v0.10.1 持仓对账: execut... | 测试依赖 / test_depends |
| 1263 | test_position_reconciler.py | → | D_POSITION 仓位管理: Position Reconciler — v0.10.1 持仓对账: execut... | 测试依赖 / test_depends |
| 1264 | test_ml_experiment_pipeline.py | → | D_RISK 风控: ml_experiment_pipeline.py | 测试依赖 / test_depends |
| 1265 | test_a2a_check.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.a2a... | 测试依赖 / test_depends |
| 1266 | test_agent_creation_policy.py | → | D_SECURITY 对抗验证: AgentCreationPolicy — Agent 创建策略. (agent_c... | 测试依赖 / test_depends |
| 1267 | 测试 L2 ABACGuard — 五维属性权限判定 (test_aba... | → | D_SECURITY 对抗验证: ABACGuard — 基于属性的权限守卫. (abac_guard.py) | 测试依赖 / test_depends |
| 1268 | 测试 L2 ABACGuard — 五维属性权限判定 (test_aba... | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1269 | MOD-INF-018 test_adversarial.py — 对抗性测试: ... | → | D_SECURITY 对抗验证: CrossSessionDetector — 跨 Session 检测器. (cro... | 测试依赖 / test_depends |
| 1270 | MOD-INF-018 test_adversarial.py — 对抗性测试: ... | → | D_SECURITY 对抗验证: ReplayAttackGuard — 重放攻击防护. (replay_atta... | 测试依赖 / test_depends |
| 1271 | MOD-INF-018 test_adversarial.py — 对抗性测试: ... | → | D_SECURITY 对抗验证: MonotonicClock — 单调时钟. (monotonic_clock.py) | 测试依赖 / test_depends |
| 1272 | MOD-INF-018 test_adversarial.py — 对抗性测试: ... | → | D_SECURITY 对抗验证: NonRepudiation — 不可抵赖性审计签名. (non_repu... | 测试依赖 / test_depends |
| 1273 | test_adversarial_resilience.py | → | D_SECURITY 对抗验证: AdversarialResilience — 对抗性韧性与 OWASP 覆... | 测试依赖 / test_depends |
| 1274 | MOD-INF-018 跨模型一致性测试 — DeepSeek/GLM/Cl... | → | D_SECURITY 对抗验证: RBACRoleDeriver — RBAC 角色派生器. (derive_rba... | 测试依赖 / test_depends |
| 1275 | MOD-INF-018 跨模型一致性测试 — DeepSeek/GLM/Cl... | → | D_SECURITY 对抗验证: PermissionGuard — 七层权限编排器. (permission_... | 测试依赖 / test_depends |
| 1276 | MOD-INF-018 跨模型一致性测试 — DeepSeek/GLM/Cl... | → | D_SECURITY 对抗验证: RBACGuard — 基于角色的权限守卫. (rbac_guard.py) | 测试依赖 / test_depends |
| 1277 | MOD-INF-018 跨模型一致性测试 — DeepSeek/GLM/Cl... | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1278 | MOD-INF-018 跨模型一致性测试 — DeepSeek/GLM/Cl... | → | D_SECURITY 对抗验证: ImmutableCore — 不可变核心验证器. (immutable_c... | 测试依赖 / test_depends |
| 1279 | MOD-INF-018 跨模型一致性测试 — DeepSeek/GLM/Cl... | → | D_SECURITY 对抗验证: IntegritySelfCheck — 完整性自检. (integrity_se... | 测试依赖 / test_depends |
| 1280 | 跨切面 D 异常检测 + 蓝图保真 + 原生API守卫 + 内... | → | D_SECURITY 对抗验证: BlueprintFidelity — 蓝图保真度检查. (blueprint... | 测试依赖 / test_depends |
| 1281 | 跨切面 D 异常检测 + 蓝图保真 + 原生API守卫 + 内... | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.det... | 测试依赖 / test_depends |
| 1282 | 跨切面 D 异常检测 + 蓝图保真 + 原生API守卫 + 内... | → | D_SECURITY 对抗验证: MemoryGuard — 内存访问守卫. (memory_guard.py) | 测试依赖 / test_depends |
| 1283 | 跨切面 D 异常检测 + 蓝图保真 + 原生API守卫 + 内... | → | D_SECURITY 对抗验证: NativeApiGuard — 原生 API 守卫. (native_api_gu... | 测试依赖 / test_depends |
| 1284 | cybersec 2026 独立测试. (test_cybersec_2026.py) | → | D_SECURITY 对抗验证: Cybersec2026Guard — 2026 网络安全威胁检测. (cy... | 测试依赖 / test_depends |
| 1285 | 测试 DecisionExplainer — 结构化拒绝原因 (test_... | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.dec... | 测试依赖 / test_depends |
| 1286 | 决策注册表测试. (test_decisions.py) | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.dec... | 测试依赖 / test_depends |
| 1287 | MOD-INF-018 test_derive_rbac.py — RBAC 自动派.... | → | D_SECURITY 对抗验证: RBACGuard — 基于角色的权限守卫. (rbac_guard.py) | 测试依赖 / test_depends |
| 1288 | MOD-INF-018 test_derive_rbac.py — RBAC 自动派.... | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1289 | 测试 L7 DryRun — 权限模拟与影响分析 (test_dry_... | → | D_SECURITY 对抗验证: DryRun — 权限模拟与影响分析. (dry_run.py) | 测试依赖 / test_depends |
| 1290 | 测试 L7 DryRun — 权限模拟与影响分析 (test_dry_... | → | D_SECURITY 对抗验证: RBACGuard — 基于角色的权限守卫. (rbac_guard.py) | 测试依赖 / test_depends |
| 1291 | 测试 L7 DryRun — 权限模拟与影响分析 (test_dry_... | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1292 | 测试 L0 EngineDegradation — 权限引擎降级策略 (... | → | D_SECURITY 对抗验证: EngineDegradation — 引擎降级管理. (engine_degr... | 测试依赖 / test_depends |
| 1293 | 七项增强安全机制整合测试. (test_enhanced_securi... | → | D_SECURITY 对抗验证: AgentCreationPolicy — Agent 创建策略. (agent_c... | 测试依赖 / test_depends |
| 1294 | 七项增强安全机制整合测试. (test_enhanced_securi... | → | D_SECURITY 对抗验证: AutoMaintenance — 自动维护与规则健康仪表盘. (a... | 测试依赖 / test_depends |
| 1295 | 七项增强安全机制整合测试. (test_enhanced_securi... | → | D_SECURITY 对抗验证: CacheInvalidation — 缓存失效事件管理. (cache_i... | 测试依赖 / test_depends |
| 1296 | 七项增强安全机制整合测试. (test_enhanced_securi... | → | D_SECURITY 对抗验证: CrossSessionDetector — 跨 Session 检测器. (cro... | 测试依赖 / test_depends |
| 1297 | 七项增强安全机制整合测试. (test_enhanced_securi... | → | D_SECURITY 对抗验证: EmergencyOverride — 紧急覆盖令牌管理. (emergen... | 测试依赖 / test_depends |
| 1298 | 七项增强安全机制整合测试. (test_enhanced_securi... | → | D_SECURITY 对抗验证: PermissionHooks — 权限钩子注册表. (permission_... | 测试依赖 / test_depends |
| 1299 | 测试 AgentRbac 异常类型 (test_exceptions_agent_... | → | D_SECURITY 对抗验证: AgentRbac 异常类型. (exceptions.py) | 测试依赖 / test_depends |
| 1300 | 跨切面 B 取证审计 A 层——genesis/asymmetric/no... | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.asy... | 测试依赖 / test_depends |
| 1301 | 跨切面 B 取证审计 A 层——genesis/asymmetric/no... | → | D_SECURITY 对抗验证: GenesisBootstrap — RBAC系统启动引导器. (genesi... | 测试依赖 / test_depends |
| 1302 | 跨切面 B 取证审计 A 层——genesis/asymmetric/no... | → | D_SECURITY 对抗验证: NonRepudiation — 不可抵赖性审计签名. (non_repu... | 测试依赖 / test_depends |
| 1303 | 跨切面 B 取证审计 B 层——path/shell/rule_injec... | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.det... | 测试依赖 / test_depends |
| 1304 | 跨切面 B 取证审计 B 层——path/shell/rule_injec... | → | D_SECURITY 对抗验证: PathGuard — 路径守卫. (path_guard.py) | 测试依赖 / test_depends |
| 1305 | 跨切面 B 取证审计 B 层——path/shell/rule_injec... | → | D_SECURITY 对抗验证: RuleInjectionGuard — 规则注入守卫. (rule_injec... | 测试依赖 / test_depends |
| 1306 | 跨切面 B 取证审计 C 层——audit_log/replay/lega... | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.gua... | 测试依赖 / test_depends |
| 1307 | 跨切面 B 取证审计 C 层——audit_log/replay/lega... | → | D_SECURITY 对抗验证: ReplayAttackGuard — 重放攻击防护. (replay_atta... | 测试依赖 / test_depends |
| 1308 | 跨切面 B 取证审计 C 层——audit_log/replay/lega... | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.leg... | 测试依赖 / test_depends |
| 1309 | 跨切面 B 取证审计 C 层——audit_log/replay/lega... | → | D_SECURITY 对抗验证: MonotonicClock — 单调时钟. (monotonic_clock.py) | 测试依赖 / test_depends |
| 1310 | 跨切面 B 取证审计 C 层——audit_log/replay/lega... | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.rol... | 测试依赖 / test_depends |
| 1311 | 测试防护层模块 — ColdStartLock, AutoGuard, Esc... | → | D_SECURITY 对抗验证: GuardLayers — 权限守卫层组件. (guard_layers.py) | 测试依赖 / test_depends |
| 1312 | 测试防护层模块 — ColdStartLock, AutoGuard, Esc... | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1313 | 测试 AgentIdentity — 身份模型 (test_identity.py) | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1314 | 测试 L0 ImmutableCore — 硬编码不可变保护区 (te... | → | D_SECURITY 对抗验证: ImmutableCore — 不可变核心验证器. (immutable_c... | 测试依赖 / test_depends |
| 1315 | 测试 L3 InputGuard — 参数级护栏 (test_input_gu... | → | D_SECURITY 对抗验证: InputGuard — 输入参数守卫. (input_guard.py) | 测试依赖 / test_depends |
| 1316 | 集成 + 契约验证测试. (test_integration_agent_rb... | → | D_SECURITY 对抗验证: IntegrationManager — 系统集成注册与健康检查. (... | 测试依赖 / test_depends |
| 1317 | 集成 + 契约验证测试. (test_integration_agent_rb... | → | D_SECURITY 对抗验证: ContractVerifier — 契约验证器. (contract_verif... | 测试依赖 / test_depends |
| 1318 | test_integration_root.py | → | D_SECURITY 对抗验证: IntegrationManager — 系统集成注册与健康检查. (... | 测试依赖 / test_depends |
| 1319 | 完整性自检测试. (test_integrity_agent_rbac.py) | → | D_SECURITY 对抗验证: IntegritySelfCheck — 完整性自检. (integrity_se... | 测试依赖 / test_depends |
| 1320 | 测试 IntentBinder — 意图绑定与连续验证 (test_i... | → | D_SECURITY 对抗验证: IntentBinder — 意图绑定与漂移检测. (intent_bin... | 测试依赖 / test_depends |
| 1321 | 测试 L0 KillSwitch — 全局熔断机制 (test_kill_s... | → | D_SECURITY 对抗验证: KillSwitch — 熔断器. (kill_switch.py) | 测试依赖 / test_depends |
| 1322 | 新攻击 / cybersec 2026 专项测试. (test_novel_at... | → | D_SECURITY 对抗验证: Cybersec2026Guard — 2026 网络安全威胁检测. (cy... | 测试依赖 / test_depends |
| 1323 | 新攻击 / cybersec 2026 专项测试. (test_novel_at... | → | D_SECURITY 对抗验证: NovelAttackGuard — 新型攻击行为画像. (novel_at... | 测试依赖 / test_depends |
| 1324 | 测试 L6 Observability — 指标上报与异常检测 (te... | → | D_SECURITY 对抗验证: ObservabilityReporter — 指标上报与异常检测. (o... | 测试依赖 / test_depends |
| 1325 | 测试 L5 OutputGuard — 输出护栏 (test_output_gu... | → | D_SECURITY 对抗验证: OutputGuard — 输出内容守卫. (output_guard.py) | 测试依赖 / test_depends |
| 1326 | 测试 PermissionGuard — 七层统一编排 (test_perm... | → | D_SECURITY 对抗验证: PermissionGuard — 七层权限编排器. (permission_... | 测试依赖 / test_depends |
| 1327 | 测试 PermissionGuard — 七层统一编排 (test_perm... | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1328 | 测试 PermissionGuard — 七层统一编排 (test_perm... | → | D_SECURITY 对抗验证: ImmutableCore — 不可变核心验证器. (immutable_c... | 测试依赖 / test_depends |
| 1329 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: CanaryRolloutManager — 灰度发布管理器. (canary... | 测试依赖 / test_depends |
| 1330 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: FalseCompletionDetector — 虚假完成检测. (false... | 测试依赖 / test_depends |
| 1331 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: MultiAgentCollusionDetector — 多 agent 合谋检... | 测试依赖 / test_depends |
| 1332 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: DryRun — 权限模拟与影响分析. (dry_run.py) | 测试依赖 / test_depends |
| 1333 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: GuardLayers — 权限守卫层组件. (guard_layers.py) | 测试依赖 / test_depends |
| 1334 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: ABACGuard — 基于属性的权限守卫. (abac_guard.py) | 测试依赖 / test_depends |
| 1335 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: InputGuard — 输入参数守卫. (input_guard.py) | 测试依赖 / test_depends |
| 1336 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: MemoryProvenanceGuard — 记忆来源溯源守卫. (mem... | 测试依赖 / test_depends |
| 1337 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: OutputGuard — 输出内容守卫. (output_guard.py) | 测试依赖 / test_depends |
| 1338 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: PermissionGuard — 七层权限编排器. (permission_... | 测试依赖 / test_depends |
| 1339 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: SequenceGuard — 操作序列守卫. (sequence_guard.py) | 测试依赖 / test_depends |
| 1340 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: TOCTOUGuard — TOCTOU (Time-of-Check to Time-of... | 测试依赖 / test_depends |
| 1341 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1342 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: ImmutableCore — 不可变核心验证器. (immutable_c... | 测试依赖 / test_depends |
| 1343 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: KillSwitch — 熔断器. (kill_switch.py) | 测试依赖 / test_depends |
| 1344 | MOD-INF-018 test_post_action.py — L5 Post-Acti... | → | D_SECURITY 对抗验证: PermissionHooks — 权限钩子注册表. (permission_... | 测试依赖 / test_depends |
| 1345 | RBAC 自动启动/关闭生命周期集成测试. (test_rbac_... | → | D_SECURITY 对抗验证: zephyr.security.access_control — Agent RBAC 权... | 测试依赖 / test_depends |
| 1346 | RBAC 自动启动/关闭生命周期集成测试. (test_rbac_... | → | D_SECURITY 对抗验证: EngineDegradation — 引擎降级管理. (engine_degr... | 测试依赖 / test_depends |
| 1347 | RBAC 自动启动/关闭生命周期集成测试. (test_rbac_... | → | D_SECURITY 对抗验证: GenesisBootstrap — RBAC系统启动引导器. (genesi... | 测试依赖 / test_depends |
| 1348 | RBAC 自动启动/关闭生命周期集成测试. (test_rbac_... | → | D_SECURITY 对抗验证: ImmutableCore — 不可变核心验证器. (immutable_c... | 测试依赖 / test_depends |
| 1349 | RBAC 自动启动/关闭生命周期集成测试. (test_rbac_... | → | D_SECURITY 对抗验证: KillSwitch — 熔断器. (kill_switch.py) | 测试依赖 / test_depends |
| 1350 | 测试 L1 RBACGuard — 三层权限模型 (test_rbac_gu... | → | D_SECURITY 对抗验证: RBACGuard — 基于角色的权限守卫. (rbac_guard.py) | 测试依赖 / test_depends |
| 1351 | 测试 L1 RBACGuard — 三层权限模型 (test_rbac_gu... | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1352 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: AdversarialResilience — 对抗性韧性与 OWASP 覆... | 测试依赖 / test_depends |
| 1353 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: AgentCreationPolicy — Agent 创建策略. (agent_c... | 测试依赖 / test_depends |
| 1354 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: AutoMaintenance — 自动维护与规则健康仪表盘. (a... | 测试依赖 / test_depends |
| 1355 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: ColdStartLock — 冷启动锁. (cold_start_lock.py) | 测试依赖 / test_depends |
| 1356 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: CrossCutting — 横切面权限组件. (cross_cutting.py) | 测试依赖 / test_depends |
| 1357 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: ContextDriftDetector — 上下文漂移与范围蔓延检... | 测试依赖 / test_depends |
| 1358 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: CrossSessionDetector — 跨 Session 检测器. (cro... | 测试依赖 / test_depends |
| 1359 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: FalseCompletionDetector — 虚假完成检测. (false... | 测试依赖 / test_depends |
| 1360 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: MultiAgentCollusionDetector — 多 agent 合谋检... | 测试依赖 / test_depends |
| 1361 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: EmergencyOverride — 紧急覆盖令牌管理. (emergen... | 测试依赖 / test_depends |
| 1362 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: EngineDegradation — 引擎降级管理. (engine_degr... | 测试依赖 / test_depends |
| 1363 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: ABACGuard — 基于属性的权限守卫. (abac_guard.py) | 测试依赖 / test_depends |
| 1364 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: InputGuard — 输入参数守卫. (input_guard.py) | 测试依赖 / test_depends |
| 1365 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: OutputGuard — 输出内容守卫. (output_guard.py) | 测试依赖 / test_depends |
| 1366 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: PathGuard — 路径守卫. (path_guard.py) | 测试依赖 / test_depends |
| 1367 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: PermissionGuard — 七层权限编排器. (permission_... | 测试依赖 / test_depends |
| 1368 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: RBACGuard — 基于角色的权限守卫. (rbac_guard.py) | 测试依赖 / test_depends |
| 1369 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: ReplayAttackGuard — 重放攻击防护. (replay_atta... | 测试依赖 / test_depends |
| 1370 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: SequenceGuard — 操作序列守卫. (sequence_guard.py) | 测试依赖 / test_depends |
| 1371 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: TOCTOUGuard — TOCTOU (Time-of-Check to Time-of... | 测试依赖 / test_depends |
| 1372 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1373 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: ImmutableCore — 不可变核心验证器. (immutable_c... | 测试依赖 / test_depends |
| 1374 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: IntentBinder — 意图绑定与漂移检测. (intent_bin... | 测试依赖 / test_depends |
| 1375 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: KillSwitch — 熔断器. (kill_switch.py) | 测试依赖 / test_depends |
| 1376 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: MonotonicClock — 单调时钟. (monotonic_clock.py) | 测试依赖 / test_depends |
| 1377 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: NonRepudiation — 不可抵赖性审计签名. (non_repu... | 测试依赖 / test_depends |
| 1378 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: PermissionHooks — 权限钩子注册表. (permission_... | 测试依赖 / test_depends |
| 1379 | 风险缓解测试. (test_risk_mitigation_agent_rbac.py) | → | D_SECURITY 对抗验证: RiskMitigation — 风险评估与缓解策略. (risk_mit... | 测试依赖 / test_depends |
| 1380 | 测试 L4 SequenceGuard — 操作序列追踪与危险序列... | → | D_SECURITY 对抗验证: SequenceGuard — 操作序列守卫. (sequence_guard.py) | 测试依赖 / test_depends |
| 1381 | session 隔离 stash 红蓝对抗极限测试。 (test_ses... | → | D_SECURITY 对抗验证: Session 级并发协调模块（P2-SES 落地）。 (sessio... | 测试依赖 / test_depends |
| 1382 | 测试 TOCTOU Guard — 竞态防护 (test_toctou_guar... | → | D_SECURITY 对抗验证: TOCTOUGuard — TOCTOU (Time-of-Check to Time-of... | 测试依赖 / test_depends |
| 1383 | Vibe Coding / Novel Attack / Cybersec 2026 攻击... | → | D_SECURITY 对抗验证: Cybersec2026Guard — 2026 网络安全威胁检测. (cy... | 测试依赖 / test_depends |
| 1384 | Vibe Coding / Novel Attack / Cybersec 2026 攻击... | → | D_SECURITY 对抗验证: NovelAttackGuard — 新型攻击行为画像. (novel_at... | 测试依赖 / test_depends |
| 1385 | Vibe Coding / Novel Attack / Cybersec 2026 攻击... | → | D_SECURITY 对抗验证: VibeCodingGuard — Vibe Coding 攻击面检测. (vib... | 测试依赖 / test_depends |
| 1386 | test_audit_log_guard.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.gua... | 测试依赖 / test_depends |
| 1387 | [INVARIANTS] E2E tests cover DecisionTable 12-r... | → | D_SECURITY 对抗验证: cascade_analyzer.py | 测试依赖 / test_depends |
| 1388 | [INVARIANTS] E2E tests cover DecisionTable 12-r... | → | D_SECURITY 对抗验证: decision_table.py | 测试依赖 / test_depends |
| 1389 | [INVARIANTS] E2E tests cover DecisionTable 12-r... | → | D_SECURITY 对抗验证: deprecation_tracker.py | 测试依赖 / test_depends |
| 1390 | [INVARIANTS] E2E tests cover DecisionTable 12-r... | → | D_SECURITY 对抗验证: judge.py | 测试依赖 / test_depends |
| 1391 | [INVARIANTS] E2E tests cover DecisionTable 12-r... | → | D_SECURITY 对抗验证: safety_fence.py | 测试依赖 / test_depends |
| 1392 | test_auto_bootstrap.py | → | D_SECURITY 对抗验证: RuleInjectionGuard — 规则注入守卫. (rule_injec... | contract / contract |
...
| 1434 | test_auto_bootstrap.py | → | D_SECURITY_LLM LLM防御: alignment_scorer.py — 对齐评分 (B11, DD85, TAS... | runtime / runtime |
...
> 本域与 24 个外部域直接连接（出边 2206 条 + 入边 6 条 = 2212 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_AUDITTEST -->|568条 contract / contract, data / data, runtime / runtime, 测试依赖 / test_depends| D_GOVERNANCE
    D_AUDITTEST -->|562条 测试依赖 / test_depends| D_TRADING
    D_AUDITTEST -->|222条 runtime / runtime, 测试依赖 / test_depends| D_GOV_ENFORCEMENT
    D_AUDITTEST -->|169条 测试依赖 / test_depends| D_SHARED
    ...
    D_AUDITTEST -->|68条 测试依赖 / test_depends| D_INTEGRATION
    ...
    D_AUDITTEST -->|30条 测试依赖 / test_depends| D_INTELLIGENCE
    ...
    D_AUDITTEST -->|1条 测试依赖 / test_depends| D_EX_CORE
