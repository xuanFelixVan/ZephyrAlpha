...
| 模块数 | 1689 | Module Count | 1689 |
| 域内依赖 | 11 | Internal Dependencies | 11 |
| 跨域入边 | 6 | Cross-domain Incoming | 6 |
| 跨域出边 | 2261 | Cross-domain Outgoing | 2261 |
| 设计态模块 | 0 | Design Modules | 0 |
| 原型态模块 | 1640 | Prototype Modules | 1640 |
...
> 按 architecture_layer 分组的模块清单（共 1689 个模块 / 1689 modules）。
...
### L2 领域层 / Domain Layer (1688 modules)
...
> 展示全部 1689 个模块（生产态 49 + 设计态 0 + 原型态 1640），标签标注成熟度。
...
graph TD
    ...
    class D_TRADING,D_SECURITY external_prod
...
graph TD
    ...
    class D_TRADING,D_GOVERNANCE,D_AUTONOMY_CORE,D_SHARED,D_INFRA_RUNTIME external_prod
```
...
graph TD
    ...
    class D_GOVERNANCE,D_INFRA_TELEMETRY,D_SECURITY_LLM,D_AUTONOMY_CORE,D_SHARED external_prod
```
...
graph TD
    ...
    class D_AUTONOMY_CORE,D_TRADING,D_GOVERNANCE,D_INFRA_RUNTIME,D_SECURITY external_prod
```
...
graph TD
    ...
    class D_TRADING,D_GOVERNANCE,D_INFRA_RUNTIME,D_GOV_ENFORCEMENT external_prod
```
...
graph TD
    ...
    tests_governance_adversarial_test_shadow_verifier_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    classDef production fill:#e1f5fe,stroke:#01579b,stroke-width:2px,color:#000
    classDef design fill:#fff3e0,stroke:#e65100,stroke-width:2px,color:#000,stroke-dasharray: 5 5
    classDef external_prod fill:#e8f5e9,stroke:#1b5e20,stroke-width:1px,color:#000
    classDef external_design fill:#fce4ec,stroke:#880e4f,stroke-width:1px,color:#000,stroke-dasharray: 5 5
...
graph TD
    ...
    tests_governance_data_layer_test_symbol_index_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    tests_governance_delegation_test_behavioral_sampler_py -.->|测试依赖 / test_depends| D_GOVERNANCE
...
graph TD
    ...
    tests_governance_ops_test_daily_ops_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    tests_governance_ops_test_clock_guard_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    D_INFRA_RECOVERY["(生产态 / production) D_INFRA_RECOVERY"]
    tests_governance_ops_test_env_watcher_py -.->|测试依赖 / test_depends| D_INFRA_RECOVERY
    tests_governance_ops_test_exit_codes_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    tests_governance_ops_test_health_monitor_py -.->|测试依赖 / test_depends| D_GOVERNANCE
...
graph TD
    ...
    tests_governance_resilience_test_timeout_guard_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    tests_governance_resilience_test_spiral_ews_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    tests_governance_resilience_test_witness_isolation_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    ...
    tests_governance_rule_bridge_test_commit_gate_registry_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    tests_governance_rule_bridge_test_session_worktree_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    D_SHARED["(生产态 / production) D_SHARED"]
...
graph TD
    ...
    tests_governance_security_test_hooks_integrity_guard_py -.->|测试依赖 / test_depends| D_GOVERNANCE
...
graph TD
    ...
    D_INFRA_A2A["(生产态 / production) D_INFRA_A2A"]
    tests_infrastructure_test_livelock_detector_py -.->|测试依赖 / test_depends| D_INFRA_A2A
    tests_infrastructure_test_lifecycle_root_py -.->|测试依赖 / test_depends| D_INFRA_RUNTIME
    tests_infrastructure_test_lifecycle_root_py -.->|测试依赖 / test_depends| D_INFRA_RUNTIME
    tests_infrastructure_test_mcp_adapter_py -.->|测试依赖 / test_depends| D_INFRA_A2A
    tests_infrastructure_test_mcp_boot_hooks_integration_py -.->|测试依赖 / test_depends| D_SHARED
    D_TRADING["(生产态 / production) D_TRADING"]
...
graph TD
    ...
    D_GOVERNANCE["(生产态 / production) D_GOVERNANCE"]
    tests_intent_test_intent_keyword_mapper_root_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    D_TRADING["(生产态 / production) D_TRADING"]
    tests_intent_test_intent_driven_ops_py -.->|测试依赖 / test_depends| D_TRADING
    tests_intent_test_intent_parser_root_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    tests_intent_test_intent_parser_root_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    D_SHARED["(生产态 / production) D_SHARED"]
...
graph TD
    ...
    tests_kb_test_kb_self_test_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    tests_kb_test_kb_storage_backend_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    tests_kb_test_kb_triage_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    ...
    tests_kb_test_kb_verify_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    tests_kb_test_kb_vms_memory_backend_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    tests_kb_test_kb_vms_memory_backend_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    D_TRADING["(生产态 / production) D_TRADING"]
...
graph TD
    ...
    tests_llm_security_test_engine_root_py -.->|测试依赖 / test_depends| D_INFRA_RUNTIME
    D_SECURITY_LLM["(生产态 / production) D_SECURITY_LLM"]
    tests_llm_security_test_fail_closed_py -.->|测试依赖 / test_depends| D_SECURITY_LLM
    D_SHARED["(生产态 / production) D_SHARED"]
    tests_llm_security_test_fail_closed_py -.->|测试依赖 / test_depends| D_SHARED
    tests_llm_security_test_fail_closed_py -.->|测试依赖 / test_depends| D_SECURITY_LLM
    tests_llm_security_test_injection_patterns_py -.->|测试依赖 / test_depends| D_SECURITY_LLM
    tests_llm_security_test_input_sanitizer_llm_security_py -.->|测试依赖 / test_depends| D_SECURITY_LLM
    tests_llm_security_test_gateway_e2e_py -.->|测试依赖 / test_depends| D_SHARED
    tests_llm_security_test_gateway_e2e_py -.->|测试依赖 / test_depends| D_SECURITY_LLM
...
graph TD
    ...
    tests_llm_security_test_scaffold_registrar_py -.->|测试依赖 / test_depends| D_INFRA_RUNTIME
    tests_llm_security_test_scaffold_registrar_py -.->|测试依赖 / test_depends| D_INFRA_RUNTIME
    tests_llm_security_test_secrets_py -.->|测试依赖 / test_depends| D_SECURITY_LLM
    D_TRADING["(生产态 / production) D_TRADING"]
    tests_llm_security_test_secret_rotation_py -.->|测试依赖 / test_depends| D_TRADING
    tests_llm_security_test_security_py -.->|测试依赖 / test_depends| D_TRADING
    tests_llm_security_test_security_py -.->|测试依赖 / test_depends| D_TRADING
    tests_llm_security_test_security_py -.->|测试依赖 / test_depends| D_TRADING
    tests_llm_security_test_security_py -.->|测试依赖 / test_depends| D_TRADING
    tests_llm_security_test_security_py -.->|测试依赖 / test_depends| D_TRADING
...
graph TD
    ...
    D_TRADING["(生产态 / production) D_TRADING"]
    tests_model_test_model_health_py -.->|测试依赖 / test_depends| D_TRADING
    D_GOVERNANCE["(生产态 / production) D_GOVERNANCE"]
    tests_model_test_model_drift_monitor_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    tests_model_test_model_version_detector_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    tests_model_test_model_rotation_py -.->|测试依赖 / test_depends| D_TRADING
    tests_model_test_model_rotation_v2_py -.->|测试依赖 / test_depends| D_TRADING
    D_INFRA_RUNTIME["(生产态 / production) D_INFRA_RUNTIME"]
    ...
    tests_model_test_profiler_py -.->|测试依赖 / test_depends| D_INTELLIGENCE
    tests_model_test_model_version_semantic_drift_py -.->|测试依赖 / test_depends| D_TRADING
...
graph TD
    ...
    tests_orchestrator_test_orchestrator_rollback_manager_py -.->|测试依赖 / test_depends| D_TRADING
    tests_orchestrator_test_orchestrator_trigger_router_py -.->|测试依赖 / test_depends| D_TRADING
    tests_orchestrator_test_orchestrator_wave_generator_py -.->|测试依赖 / test_depends| D_TRADING
...
graph TD
    ...
    D_TRADING["(生产态 / production) D_TRADING"]
    tests_prompt_test_prompt_fingerprint_py -.->|测试依赖 / test_depends| D_TRADING
    D_AUTONOMY_CORE["(生产态 / production) D_AUTONOMY_CORE"]
    tests_prompt_test_prompt_registry_root_py -.->|测试依赖 / test_depends| D_AUTONOMY_CORE
    tests_prompt_test_prompt_optimization_regression_detector_py -.->|测试依赖 / test_depends| D_TRADING
    tests_prompt_test_prompt_sanitizer_py -.->|测试依赖 / test_depends| D_TRADING
    tests_prompt_test_prompt_version_py -.->|测试依赖 / test_depends| D_TRADING
    tests_prompt_test_prompt_self_optimization_loop_py -.->|测试依赖 / test_depends| D_TRADING
    D_GOVERNANCE["(生产态 / production) D_GOVERNANCE"]
    tests_resource_test_resource_guard_py -.->|测试依赖 / test_depends| D_GOVERNANCE
    D_INFRA_RUNTIME["(生产态 / production) D_INFRA_RUNTIME"]
    tests_resource_test_resource_optimization_py -.->|测试依赖 / test_depends| D_INFRA_RUNTIME
    tests_resource_test_resource_optimization_py -.->|测试依赖 / test_depends| D_TRADING
    tests_resource_test_resource_starvation_aware_py -.->|测试依赖 / test_depends| D_TRADING
    tests_risk_test_blast_radius_detector_py -.->|测试依赖 / test_depends| D_TRADING
...
graph TD
    ...
    tests_session_test_session_conflict_py -.->|测试依赖 / test_depends| D_TRADING
    tests_session_test_session_learner_py -.->|测试依赖 / test_depends| D_TRADING
    D_SECURITY["(生产态 / production) D_SECURITY"]
    tests_session_test_session_lifecycle_py -.->|测试依赖 / test_depends| D_SECURITY
    tests_session_test_session_manager_py -.->|测试依赖 / test_depends| D_TRADING
    D_AUTONOMY_CORE["(生产态 / production) D_AUTONOMY_CORE"]
...
graph TD
    ...
    tests_trading_test_benchmark_runner_py -.->|测试依赖 / test_depends| D_TRADING
    D_SHARED["(生产态 / production) D_SHARED"]
    tests_trading_test_behavioral_admission_py -.->|测试依赖 / test_depends| D_SHARED
    tests_trading_test_batch_orchestrator_py -.->|测试依赖 / test_depends| D_TRADING
    tests_trading_test_blind_spot_closure_py -.->|测试依赖 / test_depends| D_TRADING
    tests_trading_test_boot_cron_jobs_py -.->|测试依赖 / test_depends| D_TRADING
    tests_trading_test_boot_hooks_py -.->|测试依赖 / test_depends| D_TRADING
    tests_trading_test_bulkhead_manager_py -.->|测试依赖 / test_depends| D_TRADING
...
graph TD
    ...
    D_TRADING["(生产态 / production) D_TRADING"]
    tests_trading_test_module_onboarding_scanner_py -.->|测试依赖 / test_depends| D_TRADING
    tests_trading_test_module_onboarding_scanner_py -.->|测试依赖 / test_depends| D_TRADING
    tests_trading_test_module_onboarding_scanner_py -.->|测试依赖 / test_depends| D_TRADING
    D_SHARED["(生产态 / production) D_SHARED"]
    tests_trading_test_lifecycle_manager_py -.->|测试依赖 / test_depends| D_SHARED
    tests_trading_test_lifecycle_manager_py -.->|测试依赖 / test_depends| D_TRADING
    tests_trading_test_lifecycle_manager_py -.->|测试依赖 / test_depends| D_TRADING
    tests_trading_test_lifecycle_manager_py -.->|测试依赖 / test_depends| D_TRADING
    tests_trading_test_lifecycle_manager_py -.->|测试依赖 / test_depends| D_TRADING
    tests_trading_test_lifecycle_manager_py -.->|测试依赖 / test_depends| D_TRADING
...
graph TD
    ...
    D_GOV_ENFORCEMENT["(生产态 / production) D_GOV_ENFORCEMENT"]
    tests_trae_rules_test_g_trae_009_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    tests_trae_rules_test_g_trae_009_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    tests_trae_rules_test_g_trae_008_py -.->|测试依赖 / test_depends| D_SHARED
    tests_trae_rules_test_g_trae_008_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    tests_trae_rules_test_g_trae_008_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    tests_trae_rules_test_g_trae_010_py -.->|测试依赖 / test_depends| D_SHARED
    tests_trae_rules_test_g_trae_010_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    tests_trae_rules_test_g_trae_010_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    tests_trae_rules_test_g_trae_012_py -.->|测试依赖 / test_depends| D_SHARED
    tests_trae_rules_test_g_trae_012_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    tests_trae_rules_test_g_trae_012_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    tests_trae_rules_test_g_trae_011_py -.->|测试依赖 / test_depends| D_SHARED
    tests_trae_rules_test_g_trae_011_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    tests_trae_rules_test_g_trae_011_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    classDef production fill:#e1f5fe,stroke:#01579b,stroke-width:2px,color:#000
    classDef design fill:#fff3e0,stroke:#e65100,stroke-width:2px,color:#000,stroke-dasharray: 5 5
    classDef external_prod fill:#e8f5e9,stroke:#1b5e20,stroke-width:1px,color:#000
...
graph TD
    ...
    D_GOV_ENFORCEMENT["(生产态 / production) D_GOV_ENFORCEMENT"]
    tests_trae_rules_test_g_trae_044_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    tests_trae_rules_test_g_trae_044_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    tests_trae_rules_test_g_trae_042_py -.->|测试依赖 / test_depends| D_SHARED
    tests_trae_rules_test_g_trae_042_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    tests_trae_rules_test_g_trae_042_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    tests_trae_rules_test_g_trae_045_py -.->|测试依赖 / test_depends| D_SHARED
    tests_trae_rules_test_g_trae_045_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    tests_trae_rules_test_g_trae_045_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    tests_trae_rules_test_g_trae_043_py -.->|测试依赖 / test_depends| D_SHARED
    tests_trae_rules_test_g_trae_043_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    tests_trae_rules_test_g_trae_043_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    tests_trae_rules_test_g_trae_046_py -.->|测试依赖 / test_depends| D_SHARED
    tests_trae_rules_test_g_trae_046_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    tests_trae_rules_test_g_trae_046_py -.->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    classDef production fill:#e1f5fe,stroke:#01579b,stroke-width:2px,color:#000
    classDef design fill:#fff3e0,stroke:#e65100,stroke-width:2px,color:#000,stroke-dasharray: 5 5
    classDef external_prod fill:#e8f5e9,stroke:#1b5e20,stroke-width:1px,color:#000
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 1640 个，11 条域内依赖）。
...
| 484 | test_bare_sql_gate.py — NO-BARE-SQL 门禁单测 (... | → | D_GOVERNANCE 生命周期管理: bare_sql_gate.py — 裸SQL字面量阻断门禁（NO-BAR... | 测试依赖 / test_depends |
| 485 | test_bare_sql_gate.py — NO-BARE-SQL 门禁单测 (... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 486 | test_claim_required_gate.py — claim_files 前置... | → | D_GOVERNANCE 生命周期管理: claim_required_gate.py — claim_files 前置检查.... | 测试依赖 / test_depends |
| 487 | test_claim_required_gate.py — claim_files 前置... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 488 | test_create_guard.py — CREATE-GUARD 门禁单元测... | → | D_GOVERNANCE 生命周期管理: create_guard.py — 新建 .py / 非 rules/ .yaml .... | 测试依赖 / test_depends |
| 489 | test_create_guard.py — CREATE-GUARD 门禁单元测... | → | D_GOVERNANCE 生命周期管理: GitCommitGateway — 全项目唯一合法 git commit .... | 测试依赖 / test_depends |
| 490 | test_dangling_reference_gate.py — AGENTS.md §... | → | D_GOVERNANCE 生命周期管理: dangling_reference_gate.py — AGENTS.md §X.Y .... | 测试依赖 / test_depends |
| 491 | test_datetime_now_forbidden_gate.py — 生成器代... | → | D_GOVERNANCE 生命周期管理: datetime_now_forbidden_gate.py — 生成器代码 da... | 测试依赖 / test_depends |
| 492 | test_datetime_now_forbidden_gate.py — 生成器代... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 493 | test_directory_contract_gate.py — DCR-001~007 ... | → | D_GOVERNANCE 生命周期管理: directory_contract_gate.py — DCR-001~007 等效.... | 测试依赖 / test_depends |
| 494 | test_directory_contract_gate.py — DCR-001~007 ... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 495 | test_file_placement_ttl_gate.py — 文件放置与 T... | → | D_GOVERNANCE 生命周期管理: file_placement_ttl_gate.py — 文件放置与 TTL 一... | 测试依赖 / test_depends |
| 496 | test_foreign_change_gate.py — 外来变更检测门禁... | → | D_GOVERNANCE 生命周期管理: foreign_change_gate.py — 外来变更检测门禁（FOR... | 测试依赖 / test_depends |
| 497 | test_foreign_change_gate.py — 外来变更检测门禁... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 498 | test_god_class_gate.py — NO-GOD-CLASS 门禁单测... | → | D_GOVERNANCE 生命周期管理: god_class_gate.py — God Class 阻断门禁（NO-GOD... | 测试依赖 / test_depends |
| 499 | test_god_class_gate.py — NO-GOD-CLASS 门禁单测... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 500 | test_held_overlap_gate.py — 搭便车防护门禁单测... | → | D_GOVERNANCE 生命周期管理: held_overlap_gate.py — 搭便车防护门禁（HELD-OV... | 测试依赖 / test_depends |
| 501 | test_held_overlap_gate.py — 搭便车防护门禁单测... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 502 | test_high_complexity_gate.py — NO-HIGH-COMPLEX... | → | D_GOVERNANCE 生命周期管理: high_complexity_gate.py — 高循环复杂度阻断门禁... | 测试依赖 / test_depends |
| 503 | test_high_complexity_gate.py — NO-HIGH-COMPLEX... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 504 | test_long_param_list_gate.py — NO-LONG-PARAM-L... | → | D_GOVERNANCE 生命周期管理: long_param_list_gate.py — 长参数列表阻断门禁（... | 测试依赖 / test_depends |
| 505 | test_long_param_list_gate.py — NO-LONG-PARAM-L... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 506 | test_module_id_consistency_gate.py — module_id... | → | D_GOVERNANCE 生命周期管理: module_id_consistency_gate.py — module_id 三声... | 测试依赖 / test_depends |
| 507 | test_msg_exposure_gate.py — MSG-EXPOSURE 门禁... | → | D_GOVERNANCE 生命周期管理: msg_exposure_gate.py — 错误消息暴露敏感信息阻.... | 测试依赖 / test_depends |
| 508 | test_msg_exposure_gate.py — MSG-EXPOSURE 门禁... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 509 | test_msg_style_gate.py — MSG-STYLE 门禁单测 (t... | → | D_GOVERNANCE 生命周期管理: msg_style_gate.py — 错误消息标点/箭头风格阻断.... | 测试依赖 / test_depends |
| 510 | test_msg_style_gate.py — MSG-STYLE 门禁单测 (t... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 511 | test_panorama_alignment_gate.py — 三图模块对齐... | → | D_GOVERNANCE 生命周期管理: panorama_alignment_gate.py — 三图模块对齐 warn... | 测试依赖 / test_depends |
| 512 | test_r5_digit_suffix_gate.py — R5-DIGIT-SUFFIX... | → | D_GOVERNANCE 生命周期管理: r5_digit_suffix_gate.py — R5 数字后缀目录禁止.... | 测试依赖 / test_depends |
| 513 | test_r5_digit_suffix_gate.py — R5-DIGIT-SUFFIX... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 514 | test_ssot_redefinition_gate.py — SSoT 符号重复... | → | D_GOVERNANCE 生命周期管理: ssot_redefinition_gate.py — SSoT 符号重复定义.... | 测试依赖 / test_depends |
| 515 | test_ssot_redefinition_gate.py — SSoT 符号重复... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 516 | test_ttl_gate.py — ttl 字段校验门禁单元测试。 ... | → | D_GOVERNANCE 生命周期管理: ttl_gate.py — ttl 字段校验门禁（治本：弥补 --n... | 测试依赖 / test_depends |
| 517 | test_ttl_gate.py — ttl 字段校验门禁单元测试。 ... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 518 | test_unsafe_dict_spread_gate.py — ``**data`` .... | → | D_GOVERNANCE 生命周期管理: unsafe_dict_spread_gate.py — ``**data`` 直接展... | 测试依赖 / test_depends |
| 519 | test_unsafe_dict_spread_gate.py — ``**data`` .... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 520 | test_human_factors.py | → | D_GOVERNANCE 生命周期管理: Human Factors — v0.7.0 人因工程: 通知疲劳管理+... | 测试依赖 / test_depends |
| 521 | test_load_bearing.py | → | D_GOVERNANCE 生命周期管理: 承重KE不可变性 + 承重墙自检 (load_bearing.py) | 测试依赖 / test_depends |
| 522 | test_quiet_period_monitor.py | → | D_GOVERNANCE 生命周期管理: 每日静默期检测 + 管道健康自检 (quiet_period_mon... | 测试依赖 / test_depends |
| 523 | test_thematic_clusterer.py | → | D_GOVERNANCE 生命周期管理: 主题聚类器 — 噪声信号比·告警疲劳缓解. (themat... | 测试依赖 / test_depends |
| 524 | test_command_chain_length_gate.py | → | D_GOVERNANCE 生命周期管理: Command Chain Length Gate — v0.13.0 命令体积De... | 测试依赖 / test_depends |
| 525 | test_cache_manager.py | → | D_GOVERNANCE 生命周期管理: Stage 0: 函数缓存管理器 — 增量扫描的加速核心. ... | 测试依赖 / test_depends |
| 526 | test_symbol_index.py | → | D_GOVERNANCE 生命周期管理: 符号索引 — 全局函数/类/import映射表. (symbol_i... | 测试依赖 / test_depends |
| 527 | test_behavioral_sampler.py | → | D_GOVERNANCE 生命周期管理: 行为采样验证器 — Stage 0.25 低成本快速验证. (b... | 测试依赖 / test_depends |
| 528 | test_behavioral_trust_checker.py | → | D_GOVERNANCE 生命周期管理: 行为信任检查器 — 行为漂移DIVERGED检测. (behavi... | 测试依赖 / test_depends |
| 529 | test_consequence_tracker.py | → | D_GOVERNANCE 生命周期管理: 后果追踪——记录每次修复操作对依赖方的影响. (co... | 测试依赖 / test_depends |
| 530 | test_delegation_engine.py | → | D_GOVERNANCE 生命周期管理: Escalation Protocol data models — MOD-INF-022 ... | 测试依赖 / test_depends |
| 531 | test_delegation_engine.py | → | D_GOVERNANCE 生命周期管理: Delegation Engine — MOD-INF-022 (delegation_en... | 测试依赖 / test_depends |
| 532 | test_parent_child_attributor.py | → | D_GOVERNANCE 生命周期管理: parent_child_attributor.py | 测试依赖 / test_depends |
| 533 | test_shadow_trust_validator.py | → | D_GOVERNANCE 生命周期管理: 影子信任验证器 — ImportError 防护回路. (shadow... | 测试依赖 / test_depends |
| 534 | test_trust_ring_manager.py | → | D_GOVERNANCE 生命周期管理: trust_ring_manager.py | 测试依赖 / test_depends |
| 535 | DM-100017: depgraph端到端功能测试（P2迁移后：Po... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 (depgraph_... | 测试依赖 / test_depends |
| 536 | DM-100026: 极端红蓝测试：depgraph生成器vs设计态... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 (depgraph_... | 测试依赖 / test_depends |
| 537 | test_dead_module_detector.py | → | D_GOVERNANCE 生命周期管理: 死共享模块检测器 — shared/子模块无人使用 -> DE... | 测试依赖 / test_depends |
| 538 | test_diff_detector.py | → | D_GOVERNANCE 生命周期管理: Stage 0: Git diff 变更检测器 — 函数粒度增量. (... | 测试依赖 / test_depends |
| 539 | test_ghost_scan.py | → | D_GOVERNANCE 生命周期管理: Ghost Scan — v0.8.0 幽灵进程检测: lingering pr... | 测试依赖 / test_depends |
| 540 | test_governance_drift_fix.py | → | D_GOVERNANCE 生命周期管理: G-CT-005 — ManagedDriftEvent Pydantic V2 BaseM... | 测试依赖 / test_depends |
| 541 | test_micro_clone_detector.py | → | D_GOVERNANCE 生命周期管理: 微型克隆检测器 — n-gram频率计数, 1-2行高频模式... | 测试依赖 / test_depends |
| 542 | test_stale_shared_detector.py | → | D_GOVERNANCE 生命周期管理: 过时共享函数检测器 — 无caller × 30天 -> STALE... | 测试依赖 / test_depends |
| 543 | test_alternative_path_blocker.py | → | D_GOVERNANCE 生命周期管理: Alternative Path Blocker — v0.13.0 替代工具路.... | 测试依赖 / test_depends |
| 544 | test_result_types.py | → | D_GOVERNANCE 生命周期管理: G-CT-003 — RollbackResult backward-compat re-e... | 测试依赖 / test_depends |
| 545 | test_annotations.py | → | D_GOVERNANCE 生命周期管理: 共享函数注解引擎 — @shared / @known_dup / @int... | 测试依赖 / test_depends |
| 546 | test_bare_repo_scanner.py | → | D_GOVERNANCE 生命周期管理: Bare Repo Scanner — v0.14.0 嵌入式裸仓库检测器... | 测试依赖 / test_depends |
| 547 | test_governance_result_types.py | → | D_GOVERNANCE 生命周期管理: G-CT-003 — RollbackResult backward-compat re-e... | 测试依赖 / test_depends |
| 548 | test_mock_duplicate_generator.py | → | D_GOVERNANCE 生命周期管理: 可控克隆生产器——零假阳性可期待引擎分子离散 (m... | 测试依赖 / test_depends |
| 549 | test_question_tracker.py | → | D_GOVERNANCE 生命周期管理: 问题追踪——扫描中发现需要人工处理的问题. (ques... | 测试依赖 / test_depends |
| 550 | test_api_response_sanitizer.py | → | D_GOVERNANCE 生命周期管理: API Response Sanitizer — v0.9.0 API响应清洗器:... | 测试依赖 / test_depends |
| 551 | test_bandwidth_optimizer.py | → | D_GOVERNANCE 生命周期管理: bandwidth_optimizer.py | 测试依赖 / test_depends |
| 552 | test_integration_hub.py | → | D_GOVERNANCE 生命周期管理: 集成协调器 — 24集成+19更新+16GitHub整合. (inte... | 测试依赖 / test_depends |
| 553 | test_integrations.py | → | D_GOVERNANCE 生命周期管理: 集成管理——预提交钩子+CI-only 扫描+超时边界. (... | 测试依赖 / test_depends |
| 554 | test_protocol_self_context.py | → | D_GOVERNANCE 生命周期管理: Protocol Self Context — v0.10.0 协议自维护上下... | 测试依赖 / test_depends |
| 555 | test_protocol_state_store.py | → | D_GOVERNANCE 生命周期管理: Protocol State Store — v0.10.0 协议运行时状态.... | 测试依赖 / test_depends |
| 556 | test_subagent_hook_propagator.py | → | D_GOVERNANCE 生命周期管理: Subagent Hook Propagator — v0.13.0 子Agent Hoo... | 测试依赖 / test_depends |
| 557 | test_bootstrapping_calibrator.py | → | D_GOVERNANCE 生命周期管理: bootstrapping_calibrator.py | 测试依赖 / test_depends |
| 558 | test_coldstart_manager.py | → | D_GOVERNANCE 生命周期管理: Coldstart Manager — v0.7.0 冷启动管理器: escal... | 测试依赖 / test_depends |
| 559 | test_maintenance_window_adapter.py | → | D_GOVERNANCE 生命周期管理: Maintenance Window Adapter — v0.10.0 计划维护.... | 测试依赖 / test_depends |
| 560 | test_time_sync.py | → | D_GOVERNANCE 生命周期管理: time_sync.py | 测试依赖 / test_depends |
| 561 | test_confidence_estimator.py | → | D_GOVERNANCE 生命周期管理: Confidence Estimator — D-022-05 置信度评估器: ... | 测试依赖 / test_depends |
| 562 | test_hotspot_tracker.py | → | D_GOVERNANCE 生命周期管理: 热点追踪器 — 90天滑动窗口 + 高频变动检测 + 新.... | 测试依赖 / test_depends |
| 563 | test_instruction_bloat_detector.py | → | D_GOVERNANCE 生命周期管理: InstructionBloatDetector — 指令膨胀检测 (instr... | 测试依赖 / test_depends |
| 564 | test_meta_confidence.py | → | D_GOVERNANCE 生命周期管理: Meta-Confidence — D-022-10 Agent对自身判定置信... | 测试依赖 / test_depends |
| 565 | test_meta_observability.py | → | D_GOVERNANCE 生命周期管理: Meta Observability — v0.10.0 协议自身可观测性:... | 测试依赖 / test_depends |
| 566 | test_report.py | → | D_GOVERNANCE 生命周期管理: 报告生成器 — YAML/JSON 输出 + 退出码判定 + Hea... | 测试依赖 / test_depends |
| 567 | test_clock_guard.py | → | D_GOVERNANCE 生命周期管理: Clock Guard — v0.8.0 时钟完整性防御: NTP漂移检... | 测试依赖 / test_depends |
| 568 | test_daily_ops.py | → | D_GOVERNANCE 生命周期管理: daily_ops.py | 测试依赖 / test_depends |
| 569 | test_exit_codes.py | → | D_GOVERNANCE 生命周期管理: 退出码定义模块——五档exit code 0-4枚举+描述+判... | 测试依赖 / test_depends |
| 570 | test_health_monitor.py | → | D_GOVERNANCE 生命周期管理: 健康仪表盘 — Dedup Health Score 0-100 + 趋势 +... | 测试依赖 / test_depends |
| 571 | test_success_validator.py | → | D_GOVERNANCE 生命周期管理: 成功验证——判断一次去重操作是否真正消灭了克隆.... | 测试依赖 / test_depends |
| 572 | test_verifier.py | → | D_GOVERNANCE 生命周期管理: 修复验证器 — import + 类型 + 行为采样验证. (ve... | 测试依赖 / test_depends |
| 573 | EngineSandbox — filesystem/network/boundary is... | → | D_GOVERNANCE 生命周期管理: EngineSandbox — D-022-08 OS-level sandboxing f... | 测试依赖 / test_depends |
| 574 | test_mvep_orchestrator.py | → | D_GOVERNANCE 生命周期管理: MVEP Orchestrator — v0.11.0 Minimum Viable Esc... | 测试依赖 / test_depends |
| 575 | test_objective_tracker.py | → | D_GOVERNANCE 生命周期管理: Objective Tracker — v0.9.0 目标漂移检测器: age... | 测试依赖 / test_depends |
| 576 | test_prioritizer.py | → | D_GOVERNANCE 生命周期管理: 修复优先级排序器 — 置信度×Impact×适配性 三因... | 测试依赖 / test_depends |
| 577 | test_think_time_model.py | → | D_GOVERNANCE 生命周期管理: think_time_model.py | 测试依赖 / test_depends |
| 578 | test_decisiongraph_schema_domain_id.py — decis... | → | D_GOVERNANCE 生命周期管理: decisiongraph Schema DDL + 不变量声明 (decision... | 测试依赖 / test_depends |
| 579 | test_deadlock_detector.py | → | D_GOVERNANCE 生命周期管理: Escalation Protocol data models — MOD-INF-022 ... | 测试依赖 / test_depends |
| 580 | test_deadlock_detector.py | → | D_GOVERNANCE 生命周期管理: Delegation Engine — MOD-INF-022 (delegation_en... | 测试依赖 / test_depends |
| 581 | test_deadlock_detector.py | → | D_GOVERNANCE 生命周期管理: Deadlock Detector — D-022-04 多Agent死锁+循环.... | 测试依赖 / test_depends |
| 582 | test_doom_loop_guard.py | → | D_GOVERNANCE 生命周期管理: Doom Loop 防护 — 修复升级阶梯 L0-L4 状态机. (d... | 测试依赖 / test_depends |
| 583 | test_fail_mode_manager.py | → | D_GOVERNANCE 生命周期管理: fail_mode_manager.py | 测试依赖 / test_depends |
| 584 | test_flash_crash_guard.py | → | D_GOVERNANCE 生命周期管理: Flash Crash Guard — v0.12.0 闪崩双轨熔断器。 (... | 测试依赖 / test_depends |
| 585 | test_interrupt_handler.py | → | D_GOVERNANCE 生命周期管理: Interrupt Handler — D-022-06 硬中断处理器: Own... | 测试依赖 / test_depends |
| 586 | test_last_resort_watchdog.py | → | D_GOVERNANCE 生命周期管理: Last Resort Watchdog — v0.8.0 终极逃生舱: 所有... | 测试依赖 / test_depends |
| 587 | test_observation_window_guard.py | → | D_GOVERNANCE 生命周期管理: 提取后稳定观察期守护 — 对标SDP 14天观察. (obse... | 测试依赖 / test_depends |
| 588 | test_policy_sandbox.py | → | D_GOVERNANCE 生命周期管理: policy_sandbox.py | 测试依赖 / test_depends |
| 589 | test_process_isolator.py | → | D_GOVERNANCE 生命周期管理: Process Isolator — v0.6.0 进程隔离器: engine运... | 测试依赖 / test_depends |
| 590 | test_provider_failover.py | → | D_GOVERNANCE 生命周期管理: Provider Failover — v0.7.0 多LLM Provider容灾:... | 测试依赖 / test_depends |
| 591 | test_recovery_manifest_writer.py | → | D_GOVERNANCE 生命周期管理: Recovery Manifest Writer — R2纯文本base64 Mani... | 测试依赖 / test_depends |
| 592 | test_silence_detector.py | → | D_GOVERNANCE 生命周期管理: Silence Detector — v0.8.0 静默窗口检测器: agen... | 测试依赖 / test_depends |
| 593 | test_spiral_ews.py | → | D_GOVERNANCE 生命周期管理: spiral_ews.py | 测试依赖 / test_depends |
| 594 | test_stream_abort_guard.py | → | D_GOVERNANCE 生命周期管理: StreamAbortGuard — 流式中断守卫 (stream_abort_... | 测试依赖 / test_depends |
| 595 | test_timeout_guard.py | → | D_GOVERNANCE 生命周期管理: timeout_guard.py | 测试依赖 / test_depends |
| 596 | test_witness_isolation.py | → | D_GOVERNANCE 生命周期管理: Witness Isolation — v0.8.0 Witness隔离: N版本d... | 测试依赖 / test_depends |
| 597 | test_commit_gate_registry.py — CommitGateRegis... | → | D_GOVERNANCE 生命周期管理: commit_gate_registry.py — GitCommitGateway pre... | 测试依赖 / test_depends |
| 598 | test_session_worktree.py — worktree 物理隔离端... | → | D_GOVERNANCE 生命周期管理: session_worktree.py — AI 对话 worktree 物理隔.... | 测试依赖 / test_depends |
| 599 | test_session_worktree.py — worktree 物理隔离端... | → | D_GOVERNANCE 生命周期管理: worktree_manager.py — session worktree 物理隔.... | 测试依赖 / test_depends |
| 600 | test_ssot_gate — SSoT 创建门禁红蓝变异测试。 (... | → | D_GOVERNANCE 生命周期管理: scaffold.py — ZephyrAlpha 唯一创建入口（RULE-T... | 测试依赖 / test_depends |
| 601 | test_ssot_gate — SSoT 创建门禁红蓝变异测试。 (... | → | D_GOVERNANCE 生命周期管理: CapabilityLookup — 能力->真源文件反查注册表的.... | 测试依赖 / test_depends |
| 602 | test_ssot_gate — SSoT 创建门禁红蓝变异测试。 (... | → | D_GOVERNANCE 生命周期管理: GitCommitGateway — 全项目唯一合法 git commit .... | 测试依赖 / test_depends |
| 603 | test_extraction_safety.py | → | D_GOVERNANCE 生命周期管理: 安全提取适配性评估器 — Suitability Score 0-100... | 测试依赖 / test_depends |
| 604 | test_github_api_guard.py | → | D_GOVERNANCE 生命周期管理: GitHub API Guard — v0.9.0 Comment and Control.... | 测试依赖 / test_depends |
| 605 | test_hooks_integrity_guard.py | → | D_GOVERNANCE 生命周期管理: Hooks Integrity Guard — v0.11.0 Hooks自编辑防.... | 测试依赖 / test_depends |
| 606 | test_import_surface_tracker.py | → | D_GOVERNANCE 生命周期管理: Import表面积负债追踪 — SBS 0-100 + shared burd... | 测试依赖 / test_depends |
| 607 | test_ipi_defense.py | → | D_GOVERNANCE 生命周期管理: ipi_defense.py | 测试依赖 / test_depends |
| 608 | test_monoculture_guard.py | → | D_GOVERNANCE 生命周期管理: Monoculture 免疫 — BRS 0-100 + 去重悖论检测. (... | 测试依赖 / test_depends |
| 609 | test_sbom_guard.py | → | D_GOVERNANCE 生命周期管理: SBOM Guard — v0.8.0 SBOM供应链防护: 依赖版本锁... | 测试依赖 / test_depends |
| 610 | test_security_config_scanner.py | → | D_GOVERNANCE 生命周期管理: Security Config Scanner — v0.13.0 缺失安全配置... | 测试依赖 / test_depends |
| 611 | test_sensitivity_sweeper.py | → | D_GOVERNANCE 生命周期管理: 敏感性扫荡——threshold扫描->固化成new baseline... | 测试依赖 / test_depends |
| 612 | test_signature_matcher.py | → | D_GOVERNANCE 生命周期管理: Stage 0.5: 签名指纹 SHA256[:12] O(1) 精确匹配. ... | 测试依赖 / test_depends |
| 613 | test_boot_hooks_unlock.py | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 测试依赖 / test_depends |
| 614 | test_shared_evolver.py | → | D_GOVERNANCE 生命周期管理: 共享函数自我进化引擎 — 自动升降级 + 行为漂移锁... | 测试依赖 / test_depends |
| 615 | test_shared_lifecycle_manager.py | → | D_GOVERNANCE 生命周期管理: 共享函数生命周期管理 — Active->Deprecated->Gra... | 测试依赖 / test_depends |
| 616 | test_rule_patterns.py — 治理规则正则 + 安全审.... | → | D_GOVERNANCE 生命周期管理: rule_patterns.py — 治理规则正则 + 安全审计模式... | 测试依赖 / test_depends |
| 617 | test_arbitrage_asymmetry_detector.py | → | D_GOVERNANCE 生命周期管理: Arbitrage Asymmetry Detector — v0.11.0 跨交易.... | 测试依赖 / test_depends |
| 618 | test_exchange_partition_detector.py | → | D_GOVERNANCE 生命周期管理: Exchange Partition Detector — v0.12.0 交易所网... | 测试依赖 / test_depends |
| 619 | test_exchange_reg_monitor.py | → | D_GOVERNANCE 生命周期管理: Exchange Reg Monitor — v0.11.0 交易所规则变更.... | 测试依赖 / test_depends |
| 620 | test_pricing_sync.py | → | D_GOVERNANCE 生命周期管理: pricing_sync.py | 测试依赖 / test_depends |
| 621 | test_strategy_scoper.py | → | D_GOVERNANCE 生命周期管理: Strategy Scoper — v0.6.0 策略范围隔离器: SIG/S... | 测试依赖 / test_depends |
| 622 | test_intent_keyword_mapper_root.py | → | D_GOVERNANCE 生命周期管理: IntentKeywordMapper - Stage 1 of three-stage in... | 测试依赖 / test_depends |
| 623 | test_intent_parser_root.py | → | D_GOVERNANCE 生命周期管理: IntentKeywordMapper - Stage 1 of three-stage in... | 测试依赖 / test_depends |
| 624 | test_intent_parser_root.py | → | D_GOVERNANCE 生命周期管理: IntentParser · 意图三阶段级联解析器（V-09） (i... | 测试依赖 / test_depends |
| 625 | test_mcp_task_claim.py | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 测试依赖 / test_depends |
| 626 | test_kb_analyze.py | → | D_GOVERNANCE 生命周期管理: G3 Evaluate 门禁 — 深度评估（T-2-13-C） (analy... | 测试依赖 / test_depends |
| 627 | test_kb_bootstrap.py | → | D_GOVERNANCE 生命周期管理: 冷启动引导引擎 — 从存量文档自动生成首批KE（T-M... | 测试依赖 / test_depends |
| 628 | test_kb_embedding_migrate.py | → | D_GOVERNANCE 生命周期管理: EmbeddingMigrate · Embedding 版本管理 + 迁移管... | 测试依赖 / test_depends |
| 629 | test_kb_extract.py | → | D_GOVERNANCE 生命周期管理: G5 Extract 门禁 — 知识升格（T-2-13-E） (extrac... | 测试依赖 / test_depends |
| 630 | test_kb_freeze.py | → | D_GOVERNANCE 生命周期管理: 紧急冻结/解冻/安全模式断路器 (freeze.py) | 测试依赖 / test_depends |
| 631 | test_kb_gate.py | → | D_GOVERNANCE 生命周期管理: audit-trail.kb_gate — MOD-INF-020 · KB 审计门... | 测试依赖 / test_depends |
| 632 | test_kb_gate_task.py | → | D_GOVERNANCE 生命周期管理: KB 五阶段门禁 evaluate 用的最小合法 Task（对齐 ... | 测试依赖 / test_depends |
| 633 | test_kb_graph_validator.py | → | D_GOVERNANCE 生命周期管理: 知识图谱完整性校验器（T-2-11-C） (graph_validat... | 测试依赖 / test_depends |
| 634 | test_kb_integrity.py | → | D_GOVERNANCE 生命周期管理: integrity.py | 测试依赖 / test_depends |
| 635 | test_kb_migration_embedding.py | → | D_GOVERNANCE 生命周期管理: EmbeddingMigrate · Embedding 版本管理 + 迁移管... | 测试依赖 / test_depends |
| 636 | test_kb_migration_gate.py | → | D_GOVERNANCE 生命周期管理: KB 五阶段门禁 evaluate 用的最小合法 Task（对齐 ... | 测试依赖 / test_depends |
| 637 | test_kb_self_test.py | → | D_GOVERNANCE 生命周期管理: KB 13项一键体检 + --self-test入口 (self_test.py) | 测试依赖 / test_depends |
| 638 | test_kb_storage_backend.py | → | D_GOVERNANCE 生命周期管理: Re-export shim — 真源在 zephyr.governance.kb.s... | 测试依赖 / test_depends |
| 639 | test_kb_triage.py | → | D_GOVERNANCE 生命周期管理: G2 Triage 门禁 — 知识分类评分（T-2-13-B） (tri... | 测试依赖 / test_depends |
| 640 | test_kb_unified_memory_api.py | → | D_GOVERNANCE 生命周期管理: Re-export shim — 真源在 zephyr.governance.kb.s... | 测试依赖 / test_depends |
| 641 | test_kb_verify.py | → | D_GOVERNANCE 生命周期管理: 确定性事实核查 — 取代AI猜测 (verify.py) | 测试依赖 / test_depends |
| 642 | test_kb_vms_memory_backend.py | → | D_GOVERNANCE 生命周期管理: Re-export shim — 真源在 zephyr.governance.kb.s... | 测试依赖 / test_depends |
| 643 | test_kb_vms_memory_backend.py | → | D_GOVERNANCE 生命周期管理: VMSMemoryBackend — UnifiedMemoryAPI 的 VMS 后.... | 测试依赖 / test_depends |
| 644 | test_ke_tombstone.py | → | D_GOVERNANCE 生命周期管理: SQLite墓碑表 + G2向量去重 (ke_tombstone.py) | 测试依赖 / test_depends |
| 645 | test_knowledge_distiller.py | → | D_GOVERNANCE 生命周期管理: knowledge_distiller.py — 知识蒸馏 (B10, DD84, ... | 测试依赖 / test_depends |
| 646 | test_cross_module_integration_llm_security.py | → | D_GOVERNANCE 生命周期管理: DefaultSecurityGateway — SecurityGateway 三层.... | 测试依赖 / test_depends |
| 647 | test_memory_poison_guard.py | → | D_GOVERNANCE 生命周期管理: Memory Poison Guard — v0.9.0 记忆投毒防护: Mem... | 测试依赖 / test_depends |
| 648 | test_memory_provenance.py | → | D_GOVERNANCE 生命周期管理: Memory Provenance — v0.9.0 记忆溯源追踪: 每条m... | 测试依赖 / test_depends |
| 649 | calibrate_model_diff.py 单元测试（P1-3 配套, 零... | → | D_GOVERNANCE 生命周期管理: 模型能力差异校准脚本（P1-3 治本）。 (calibrate_... | 测试依赖 / test_depends |
| 650 | test_model_drift_monitor.py | → | D_GOVERNANCE 生命周期管理: model_drift_monitor.py | 测试依赖 / test_depends |
| 651 | test_model_version_detector.py | → | D_GOVERNANCE 生命周期管理: Model Version Detector — v0.10.0 模型版本突变.... | 测试依赖 / test_depends |
| 652 | test_multi_turn_intent_analyzer.py | → | D_GOVERNANCE 生命周期管理: Multi-Turn Intent Analyzer — v0.13.0 多轮分布.... | 测试依赖 / test_depends |
| 653 | test_path_index_validator.py | → | D_GOVERNANCE 生命周期管理: 路径索引验证——验证 config 数据集相对路径表与.... | 测试依赖 / test_depends |
| 654 | DM-100027: 极端红蓝测试：路径树生成器vs设计态保... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 (depgraph_... | 测试依赖 / test_depends |
| 655 | test_resource_guard.py | → | D_GOVERNANCE 生命周期管理: Resource Guard — 资源上限与优雅降级 D-023-23 .... | 测试依赖 / test_depends |
| 656 | test_risk_matrix.py | → | D_GOVERNANCE 生命周期管理: risk_matrix.py | 测试依赖 / test_depends |
| 657 | test_risk_mitigation_tracker.py | → | D_GOVERNANCE 生命周期管理: 风险缓解追踪——捕获哪些克隆报告了但在N次扫描后... | 测试依赖 / test_depends |
| 658 | test_risk_mitigator.py | → | D_GOVERNANCE 生命周期管理: R1-R45全量风险缓解执行器 — 逐条检查缓解措施 + ... | 测试依赖 / test_depends |
| 659 | 红蓝对抗极端测试 — git_guard + concurrency_gua... | → | D_GOVERNANCE 生命周期管理: Git Guard — 拦截危险 git 命令，防止破坏其他 se... | 测试依赖 / test_depends |
| 660 | 并发红蓝极限对抗测试 — 多 AI 并发执行 git mv .... | → | D_GOVERNANCE 生命周期管理: Git Guard — 拦截危险 git 命令，防止破坏其他 se... | 测试依赖 / test_depends |
| 661 | test_rollback_bridge.py | → | D_GOVERNANCE 生命周期管理: G-CT-006 契约：Drift -> Rollback 漂移触发回滚. ... | 测试依赖 / test_depends |
| 662 | test_rule_integration.py | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 (depgraph_... | 测试依赖 / test_depends |
| 663 | test_rule_red_blue.py | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 测试依赖 / test_depends |
| 664 | test_safety_brake.py | → | D_GOVERNANCE 生命周期管理: 冷静期引擎 + 魔鬼代言人 + 影响评估 (safety_brak... | 测试依赖 / test_depends |
| 665 | test_self_budget_tracker.py | → | D_GOVERNANCE 生命周期管理: self_budget_tracker.py | 测试依赖 / test_depends |
| 666 | test_self_check.py | → | D_GOVERNANCE 生命周期管理: Self-Drift Check — self_check.py (self_check.py) | 测试依赖 / test_depends |
| 667 | test_self_monitor.py | → | D_GOVERNANCE 生命周期管理: self_monitor.py | 测试依赖 / test_depends |
| 668 | test_self_scanner.py | → | D_GOVERNANCE 生命周期管理: 引擎自扫描器 — Dogfooding 检测引擎自身源码重复... | 测试依赖 / test_depends |
| 669 | test_self_test.py | → | D_GOVERNANCE 生命周期管理: Escalation Protocol Self-Test — MOD-INF-022. (... | 测试依赖 / test_depends |
| 670 | test_self_test_verifier.py | → | D_GOVERNANCE 生命周期管理: Self Test Verifier — self_test_verifier.py (se... | 测试依赖 / test_depends |
| 671 | test_self_validator.py | → | D_GOVERNANCE 生命周期管理: Self Validator — v0.10.0 升级协议自验证器: pro... | 测试依赖 / test_depends |
| 672 | blast_radius 单元测试 — BlastRadiusAnalyzer 全... | → | D_GOVERNANCE 生命周期管理: blast_radius — MOD-INF-028 §3.1 Stage 9 (blas... | 测试依赖 / test_depends |
| 673 | blast_radius 单元测试 — BlastRadiusAnalyzer 全... | → | D_GOVERNANCE 生命周期管理: 语义审计管线数据模型 — MOD-INF-028 §4.2 (mode... | 测试依赖 / test_depends |
| 674 | blast_radius 红蓝对抗测试 — 对抗性场景覆盖. (t... | → | D_GOVERNANCE 生命周期管理: blast_radius — MOD-INF-028 §3.1 Stage 9 (blas... | 测试依赖 / test_depends |
| 675 | blast_radius 红蓝对抗测试 — 对抗性场景覆盖. (t... | → | D_GOVERNANCE 生命周期管理: 语义审计管线数据模型 — MOD-INF-028 §4.2 (mode... | 测试依赖 / test_depends |
| 676 | test_semantic_auditor.py | → | D_GOVERNANCE 生命周期管理: models.py | 测试依赖 / test_depends |
| 677 | test_semantic_cache.py | → | D_GOVERNANCE 生命周期管理: semantic_cache.py | 测试依赖 / test_depends |
| 678 | DM-202918: transition(COMPLETED)自动git commit.... | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 测试依赖 / test_depends |
| 679 | test_task_repo_gateway_e2e.py — 端到端链路测试... | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 测试依赖 / test_depends |
| 680 | test_task_repo_gateway_e2e.py — 端到端链路测试... | → | D_GOVERNANCE 生命周期管理: GitCommitGateway — 全项目唯一合法 git commit .... | 测试依赖 / test_depends |
| 681 | test_decision_graph_reader — DecisionGraphRead... | → | D_GOVERNANCE 生命周期管理: decision_graph_reader.py — 决策流图数据库只读.... | 测试依赖 / test_depends |
| 682 | test_decision_graph_reader — DecisionGraphRead... | → | D_GOVERNANCE 生命周期管理: decisiongraph Schema DDL + 不变量声明 (decision... | 测试依赖 / test_depends |
| 683 | test_extract_decisiongraph — extract_decisiong... | → | D_GOVERNANCE 生命周期管理: decisiongraph Schema DDL + 不变量声明 (decision... | 测试依赖 / test_depends |
| 684 | tests.test_trading_session_lifecycle — F1 Sess... | → | D_GOVERNANCE 生命周期管理: session_lifecycle.py | 测试依赖 / test_depends |
| 685 | cli 单测（MOD-L00-004 阶段3）。 (test_cli.py) | → | D_GOVERNANCE 生命周期管理: 数据源集成器 CLI（MOD-L00-004 §8.4）。 (cli.py) | 测试依赖 / test_depends |
| 686 | cli 单测（MOD-L00-004 阶段3）。 (test_cli.py) | → | D_GOVERNANCE 生命周期管理: per-source 调用策略注册表（MOD-L00-004 §5）。 ... | 测试依赖 / test_depends |
| 687 | scheduler 单测（MOD-L00-004 阶段2）。 (test_sch... | → | D_GOVERNANCE 生命周期管理: per-source 调用策略注册表（MOD-L00-004 §5）。 ... | 测试依赖 / test_depends |
| 688 | test_auto_bootstrap.py | → | D_GOV_DRIFT 漂移检测: blueprint.md | runtime / runtime |
| 689 | test_ai_capability_guard.py | → | D_GOV_ENFORCEMENT 规则执行: ZephyrAlpha — gates/ai_capability_guard.py (ai... | 测试依赖 / test_depends |
| 690 | test_audit_chain_verifier.py | → | D_GOV_ENFORCEMENT 规则执行: 审计链验证工具——独立重放门禁判定+Hash链完整性... | 测试依赖 / test_depends |
| 691 | test_audit_chain_verifier.py | → | D_GOV_ENFORCEMENT 规则执行: 门禁上下文传播——GateContext 构建/序列化/跨模.... | 测试依赖 / test_depends |
| 692 | test_audit_red_blue_e2e.py | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 693 | test_auto_bootstrap.py | → | D_GOV_ENFORCEMENT 规则执行: Audit Trail — MOD-INF-020 (__init__.py) | runtime / runtime |
| 694 | test_auto_split.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 695 | test_ba_integration_test_runner.py | → | D_GOV_ENFORCEMENT 规则执行: 集成测试运行器（Integration Test Runner） (inte... | 测试依赖 / test_depends |
| 696 | test_ct_audit_findings_resolved.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 697 | test_ct_audit_findings_resolved.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 698 | test_ct_blueprint_read_check.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 699 | test_ct_blueprint_read_check.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 700 | test_ct_circuit_breaker.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 701 | test_ct_circuit_breaker.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 702 | test_ct_circular_dependency_scan.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 703 | test_ct_circular_dependency_scan.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 704 | test_ct_classification.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 705 | test_ct_classification.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 706 | test_ct_content_length.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 707 | test_ct_content_length.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 708 | test_ct_content_quality.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 709 | test_ct_content_quality.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 710 | test_ct_contract_compatibility_check.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 711 | test_ct_contract_compatibility_check.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 712 | test_ct_deduplication.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 713 | test_ct_deduplication.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 714 | test_ct_drift_budget.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 715 | test_ct_drift_budget.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 716 | test_ct_encoding.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 717 | test_ct_encoding.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 718 | test_ct_enforcement_mode_check.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 719 | test_ct_enforcement_mode_check.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 720 | test_ct_field_presence.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 721 | test_ct_field_presence.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 722 | test_ct_file_extension.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 723 | test_ct_file_extension.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 724 | test_ct_fle_gate.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 725 | test_ct_fle_gate.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 726 | test_ct_frontmatter.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 727 | test_ct_frontmatter.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 728 | test_ct_leverage_limit.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 729 | test_ct_leverage_limit.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 730 | test_ct_line_ending.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 731 | test_ct_line_ending.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 732 | test_ct_manual_approval.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 733 | test_ct_manual_approval.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 734 | test_ct_path_blacklist.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 735 | test_ct_path_blacklist.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 736 | test_ct_path_routing.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 737 | test_ct_path_routing.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 738 | test_ct_path_whitelist.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 739 | test_ct_path_whitelist.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 740 | test_ct_position_limit.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 741 | test_ct_position_limit.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 742 | test_ct_reference_check.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 743 | test_ct_reference_check.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 744 | test_ct_regex_pattern.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 745 | test_ct_regex_pattern.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 746 | test_ct_restructuring_safety.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 747 | test_ct_restructuring_safety.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 748 | test_ct_rollback_exit_code.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 749 | test_ct_rollback_exit_code.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 750 | test_ct_score_threshold.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 751 | test_ct_score_threshold.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 752 | test_ct_security_artifact_scan.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 753 | test_ct_security_artifact_scan.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 754 | test_ct_strategy_correlation.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 755 | test_ct_strategy_correlation.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 756 | test_ct_temporal.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 757 | test_ct_temporal.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 758 | test_ct_zero_residue_check.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 759 | test_ct_zero_residue_check.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 760 | test_db_transition.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 761 | test_e_gov_approval.py | → | D_GOV_ENFORCEMENT 规则执行: G-CT-004 — Backward-compat re-export of Approv... | 测试依赖 / test_depends |
| 762 | test_e_slo_contract.py | → | D_GOV_ENFORCEMENT 规则执行: SLO-Driven Escalation Contract — D-022-12. (sl... | 测试依赖 / test_depends |
| 763 | test_escalation_gov_approval.py | → | D_GOV_ENFORCEMENT 规则执行: G-CT-004 — Backward-compat re-export of Approv... | 测试依赖 / test_depends |
| 764 | test_fl_adversarial_validation.py | → | D_GOV_ENFORCEMENT 规则执行: AdversarialValidationGate — validates outputs ... | 测试依赖 / test_depends |
| 765 | test_file_task_mapper_root.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 766 | test_gate_context.py | → | D_GOV_ENFORCEMENT 规则执行: 门禁上下文传播——GateContext 构建/序列化/跨模.... | 测试依赖 / test_depends |
| 767 | test_gate_health.py | → | D_GOV_ENFORCEMENT 规则执行: 门禁健康仪表板——per-gate SLI 报告、误报率、延... | 测试依赖 / test_depends |
| 768 | test_gate_integrity_guard.py | → | D_GOV_ENFORCEMENT 规则执行: 门禁引擎完整性守卫——自检SHA-256校验+trust roo... | 测试依赖 / test_depends |
| 769 | test_gate_override.py | → | D_GOV_ENFORCEMENT 规则执行: Owner 紧急旁路——时间限定的门禁临时绕过 + 审计... | 测试依赖 / test_depends |
| 770 | test_gate_pipeline.py | → | D_GOV_ENFORCEMENT 规则执行: 门禁上下文传播——GateContext 构建/序列化/跨模.... | 测试依赖 / test_depends |
| 771 | test_gate_pipeline.py | → | D_GOV_ENFORCEMENT 规则执行: 门禁评估管线——排序解析、组合逻辑（AND/OR/NOT.... | 测试依赖 / test_depends |
| 772 | test_gate_simulator.py | → | D_GOV_ENFORCEMENT 规则执行: 门禁上下文传播——GateContext 构建/序列化/跨模.... | 测试依赖 / test_depends |
| 773 | test_gate_simulator.py | → | D_GOV_ENFORCEMENT 规则执行: 门禁评估管线——排序解析、组合逻辑（AND/OR/NOT.... | 测试依赖 / test_depends |
| 774 | test_gate_simulator.py | → | D_GOV_ENFORCEMENT 规则执行: 门禁模拟器——dry-run 全链路门禁演练，不修改任.... | 测试依赖 / test_depends |
| 775 | test_gate_types.py | → | D_GOV_ENFORCEMENT 规则执行: gate_types.py | 测试依赖 / test_depends |
| 776 | test_approval.py | → | D_GOV_ENFORCEMENT 规则执行: G-CT-004 — Backward-compat re-export of Approv... | 测试依赖 / test_depends |
| 777 | test_pre_flight_gate.py | → | D_GOV_ENFORCEMENT 规则执行: pre_flight_gate.py | 测试依赖 / test_depends |
| 778 | test_slo_contract.py | → | D_GOV_ENFORCEMENT 规则执行: SLO-Driven Escalation Contract — D-022-12. (sl... | 测试依赖 / test_depends |
| 779 | test_base_repo.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 780 | test_check_type_registry.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 781 | test_adversarial_gate_integration.py | → | D_GOV_ENFORCEMENT 规则执行: Adversarial sample generator and 5 attack strat... | 测试依赖 / test_depends |
| 782 | test_adversarial_gate_integration.py | → | D_GOV_ENFORCEMENT 规则执行: CheckTypeHandler — CheckTypeHandler (check_typ... | 测试依赖 / test_depends |
| 783 | test_adversarial_gate_integration.py | → | D_GOV_ENFORCEMENT 规则执行: AdversarialValidationGate — validates outputs ... | 测试依赖 / test_depends |
| 784 | test_adversarial_validation.py | → | D_GOV_ENFORCEMENT 规则执行: AdversarialValidationGate — validates outputs ... | 测试依赖 / test_depends |
| 785 | test_adversarial_validation_gate.py | → | D_GOV_ENFORCEMENT 规则执行: AdversarialValidationGate — validates outputs ... | 测试依赖 / test_depends |
| 786 | test_en_001_circular_dependency.py | → | D_GOV_ENFORCEMENT 规则执行: EN-001 — Circular Dependency Scanner (en_001_c... | 测试依赖 / test_depends |
| 787 | test_en_002_enforcement_validator.py | → | D_GOV_ENFORCEMENT 规则执行: EN-002 — Enforcement Mode Validator (en_002_en... | 测试依赖 / test_depends |
| 788 | test_en_003_contract_compatibility.py | → | D_GOV_ENFORCEMENT 规则执行: EN-003 — Contract Compatibility Checker (en_00... | 测试依赖 / test_depends |
| 789 | test_en_process_lifecycle_gateway.py | → | D_GOV_ENFORCEMENT 规则执行: EN-process-lifecycle-gateway — 进程创建入口校.... | 测试依赖 / test_depends |
| 790 | test_post_doc_review.py | → | D_GOV_ENFORCEMENT 规则执行: PostDocReviewScanner — Session 关门时文档内容.... | 测试依赖 / test_depends |
| 791 | test_zero_residue_check.py | → | D_GOV_ENFORCEMENT 规则执行: zero_residue_check.py | 测试依赖 / test_depends |
| 792 | test_adaptive_threshold.py | → | D_GOV_ENFORCEMENT 规则执行: 自适应阈值——从历史 FAIL/PASS 数据学习门禁参数... | 测试依赖 / test_depends |
| 793 | test_adversarial_strategies.py | → | D_GOV_ENFORCEMENT 规则执行: Adversarial sample generator and 5 attack strat... | 测试依赖 / test_depends |
| 794 | test_breaking_change_detector.py | → | D_GOV_ENFORCEMENT 规则执行: Breaking Change 检测器（GATE-CDC-2）——字段删.... | 测试依赖 / test_depends |
| 795 | test_end_to_end_walkthrough.py | → | D_GOV_ENFORCEMENT 规则执行: 端到端场景走查验证器（End-to-End Walkthrough Va... | 测试依赖 / test_depends |
| 796 | test_integration_test_runner.py | → | D_GOV_ENFORCEMENT 规则执行: 集成测试运行器（Integration Test Runner） (inte... | 测试依赖 / test_depends |
| 797 | test_kiss_enforcer.py | → | D_GOV_ENFORCEMENT 规则执行: KISS 约束执行器（CT-KISS-001）——AI产出复杂度.... | 测试依赖 / test_depends |
| 798 | test_output_quality_gate.py | → | D_GOV_ENFORCEMENT 规则执行: output_quality_gate.py | 测试依赖 / test_depends |
| 799 | test_secrets_guard.py | → | D_GOV_ENFORCEMENT 规则执行: Secrets 守护（CT-SECRETS-001）——.env校验+git ... | 测试依赖 / test_depends |
| 800 | test_triple_alignment.py | → | D_GOV_ENFORCEMENT 规则执行: G-TRIPLE-ALIGN: 蓝图↔代码↔依赖图三方对齐门禁 ... | 测试依赖 / test_depends |
| 801 | test_preemption_manager.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 802 | test_kb_activate.py | → | D_GOV_ENFORCEMENT 规则执行: gate_types.py | 测试依赖 / test_depends |
| 803 | test_kb_analyze.py | → | D_GOV_ENFORCEMENT 规则执行: gate_types.py | 测试依赖 / test_depends |
| 804 | test_kb_extract.py | → | D_GOV_ENFORCEMENT 规则执行: gate_types.py | 测试依赖 / test_depends |
| 805 | test_kb_migration_gate.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 806 | test_db.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 807 | test_risk_ssot.py | → | D_GOV_ENFORCEMENT 规则执行: risk_ssot — 从 ``config/risk_params.yaml`` 加.... | 测试依赖 / test_depends |
| 808 | test_rule_canary_manager.py | → | D_GOV_ENFORCEMENT 规则执行: Rule Canary Manager — v0.10.0 规则金丝雀: 1%用... | 测试依赖 / test_depends |
| 809 | test_rule_debt_auditor.py | → | D_GOV_ENFORCEMENT 规则执行: Rule Debt Auditor — v0.7.0 规则债务审计器: 分.... | 测试依赖 / test_depends |
| 810 | test_rule_e2e.py | → | D_GOV_ENFORCEMENT 规则执行: RuleLoader — 规则加载核心 API (rule_engine.py) | 测试依赖 / test_depends |
| 811 | test_rule_integration.py | → | D_GOV_ENFORCEMENT 规则执行: RuleLoader — 规则加载核心 API (rule_engine.py) | 测试依赖 / test_depends |
| 812 | test_rule_shadow_runner.py | → | D_GOV_ENFORCEMENT 规则执行: Rule Shadow Runner — v0.10.0 规则影子模式: 新.... | 测试依赖 / test_depends |
| 813 | test_task_types.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 814 | Test gate g_trae_003 for rule TRAE-003 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 815 | Test gate g_trae_003 for rule TRAE-003 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 816 | Test gate g_trae_004 for rule TRAE-004 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 817 | Test gate g_trae_004 for rule TRAE-004 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 818 | Test gate g_trae_006 for rule TRAE-006 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 819 | Test gate g_trae_006 for rule TRAE-006 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 820 | Test gate g_trae_007 for rule TRAE-007 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 821 | Test gate g_trae_007 for rule TRAE-007 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 822 | Test gate g_trae_008 for rule TRAE-008 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 823 | Test gate g_trae_008 for rule TRAE-008 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 824 | Test gate g_trae_009 for rule TRAE-009 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 825 | Test gate g_trae_009 for rule TRAE-009 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 826 | Test gate g_trae_010 for rule TRAE-010 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 827 | Test gate g_trae_010 for rule TRAE-010 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 828 | Test gate g_trae_011 for rule TRAE-011 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 829 | Test gate g_trae_011 for rule TRAE-011 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 830 | Test gate g_trae_012 for rule TRAE-012 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 831 | Test gate g_trae_012 for rule TRAE-012 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 832 | Test gate g_trae_016 for rule TRAE-016 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 833 | Test gate g_trae_016 for rule TRAE-016 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 834 | Test gate g_trae_017 for rule TRAE-017 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 835 | Test gate g_trae_017 for rule TRAE-017 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 836 | Test gate g_trae_018 for rule TRAE-018 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 837 | Test gate g_trae_018 for rule TRAE-018 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 838 | Test gate g_trae_020 for rule TRAE-020 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 839 | Test gate g_trae_020 for rule TRAE-020 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 840 | Test gate g_trae_021 for rule TRAE-021 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 841 | Test gate g_trae_021 for rule TRAE-021 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 842 | Test gate g_trae_022 for rule TRAE-022 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 843 | Test gate g_trae_022 for rule TRAE-022 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 844 | Test gate g_trae_023 for rule TRAE-023 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 845 | Test gate g_trae_023 for rule TRAE-023 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 846 | Test gate g_trae_024 for rule TRAE-024 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 847 | Test gate g_trae_024 for rule TRAE-024 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 848 | Test gate g_trae_025 for rule TRAE-025 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 849 | Test gate g_trae_025 for rule TRAE-025 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 850 | Test gate g_trae_026 for rule TRAE-026 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 851 | Test gate g_trae_026 for rule TRAE-026 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 852 | Test gate g_trae_027 for rule TRAE-027 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 853 | Test gate g_trae_027 for rule TRAE-027 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 854 | Test gate g_trae_028 for rule TRAE-028 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 855 | Test gate g_trae_028 for rule TRAE-028 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 856 | Test gate g_trae_029 for rule TRAE-029 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 857 | Test gate g_trae_029 for rule TRAE-029 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 858 | Test gate g_trae_030 for rule TRAE-030 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 859 | Test gate g_trae_030 for rule TRAE-030 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 860 | Test gate g_trae_031 for rule TRAE-031 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 861 | Test gate g_trae_031 for rule TRAE-031 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 862 | Test gate g_trae_032 for rule TRAE-032 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 863 | Test gate g_trae_032 for rule TRAE-032 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 864 | Test gate g_trae_033 for rule TRAE-033 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 865 | Test gate g_trae_033 for rule TRAE-033 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 866 | Test gate g_trae_034 for rule TRAE-034 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 867 | Test gate g_trae_034 for rule TRAE-034 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 868 | Test gate g_trae_035 for rule TRAE-035 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 869 | Test gate g_trae_035 for rule TRAE-035 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 870 | Test gate g_trae_036 for rule TRAE-036 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 871 | Test gate g_trae_036 for rule TRAE-036 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 872 | Test gate g_trae_037 for rule TRAE-037 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 873 | Test gate g_trae_037 for rule TRAE-037 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 874 | Test gate g_trae_038 for rule TRAE-038 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 875 | Test gate g_trae_038 for rule TRAE-038 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 876 | Test gate g_trae_039 for rule TRAE-039 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 877 | Test gate g_trae_039 for rule TRAE-039 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 878 | Test gate g_trae_040 for rule TRAE-040 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 879 | Test gate g_trae_040 for rule TRAE-040 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 880 | Test gate g_trae_041 for rule TRAE-041 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 881 | Test gate g_trae_041 for rule TRAE-041 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 882 | Test gate g_trae_042 for rule TRAE-042 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 883 | Test gate g_trae_042 for rule TRAE-042 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 884 | Test gate g_trae_043 for rule TRAE-043 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 885 | Test gate g_trae_043 for rule TRAE-043 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 886 | Test gate g_trae_044 for rule TRAE-044 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 887 | Test gate g_trae_044 for rule TRAE-044 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 888 | Test gate g_trae_045 for rule TRAE-045 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 889 | Test gate g_trae_045 for rule TRAE-045 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 890 | Test gate g_trae_046 for rule TRAE-046 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 891 | Test gate g_trae_046 for rule TRAE-046 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 892 | Test gate g_trae_047 for rule TRAE-047 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 893 | Test gate g_trae_047 for rule TRAE-047 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 894 | Test gate g_trae_048 for rule TRAE-048 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 895 | Test gate g_trae_048 for rule TRAE-048 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 896 | Test gate g_trae_049 for rule TRAE-049 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 897 | Test gate g_trae_049 for rule TRAE-049 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 898 | Test gate g_trae_050 for rule TRAE-050 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 899 | Test gate g_trae_050 for rule TRAE-050 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 900 | Test gate g_trae_051 for rule TRAE-051 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 901 | Test gate g_trae_051 for rule TRAE-051 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 902 | Test gate g_trae_052 for rule TRAE-052 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 903 | Test gate g_trae_052 for rule TRAE-052 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 904 | Test gate g_trae_053 for rule TRAE-053 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 905 | Test gate g_trae_053 for rule TRAE-053 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 906 | Test gate g_trae_054 for rule TRAE-054 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 907 | Test gate g_trae_054 for rule TRAE-054 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 908 | Test gate g_trae_055 for rule TRAE-055 — calls... | → | D_GOV_ENFORCEMENT 规则执行: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 测试依赖 / test_depends |
| 909 | Test gate g_trae_055 for rule TRAE-055 — calls... | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 910 | test_utils_testing.py | → | D_GOV_ENFORCEMENT 规则执行: task_types.py | 测试依赖 / test_depends |
| 911 | 单元测试：scripts/governance/d3_metadata/check_... | → | D_GOV_SCRIPTS 脚本治理: GATE-15: Frontmatter metadata validation（ttl +... | 测试依赖 / test_depends |
| 912 | test_ssot_gate — SSoT 创建门禁红蓝变异测试。 (... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | 测试依赖 / test_depends |
| 913 | test_a2a_card_registry.py | → | D_INFRA_A2A A2A通信: A2A Card Registry — 全局 Agent Card 注册单例 (... | 测试依赖 / test_depends |
| 914 | test_a2a_card_registry.py | → | D_INFRA_A2A A2A通信: A2A Registry — Agent Card 注册与发现 (a2a_regi... | 测试依赖 / test_depends |
| 915 | test_a2a_card_registry.py | → | D_INFRA_A2A A2A通信: Agent Card 模型 — A2A Layer 1 Discovery (agent... | 测试依赖 / test_depends |
| 916 | test_a2a_governance.py | → | D_INFRA_A2A A2A通信: A2A GovernanceAdapter — Phase 4 治理集成桥接器... | 测试依赖 / test_depends |
| 917 | test_a2a_governance.py | → | D_INFRA_A2A A2A通信: Phase 4 Hold — A2A Phase 4 锁定标记模块 与其他... | 测试依赖 / test_depends |
| 918 | test_a2a_governance.py | → | D_INFRA_A2A A2A通信: G-CT-008 — A2ACommunication Pydantic V2 BaseMo... | 测试依赖 / test_depends |
| 919 | test_a2a_layer1_discovery.py | → | D_INFRA_A2A A2A通信: A2A Registry — Agent Card 注册与发现 (a2a_regi... | 测试依赖 / test_depends |
| 920 | test_a2a_layer1_discovery.py | → | D_INFRA_A2A A2A通信: Agent Card 模型 — A2A Layer 1 Discovery (agent... | 测试依赖 / test_depends |
| 921 | test_a2a_layer1_discovery.py | → | D_INFRA_A2A A2A通信: Identity Verifier — JWT 身份验证器 (identity_v... | 测试依赖 / test_depends |
| 922 | test_a2a_negotiation.py | → | D_INFRA_A2A A2A通信: A2A 协商协议 — Agent 间资源/任务分配协商 (a2a_... | 测试依赖 / test_depends |
| 923 | test_a2a_saga.py | → | D_INFRA_A2A A2A通信: A2A Saga 事务协议 — 多 Agent 跨步分布式事务 (a... | 测试依赖 / test_depends |
| 924 | test_a2a_schemas.py | → | D_INFRA_A2A A2A通信: A2A Message/Part 系统 — Layer 2 Communication ... | 测试依赖 / test_depends |
| 925 | test_a2a_state.py | → | D_INFRA_A2A A2A通信: A2A Task 状态机 — Layer 2 Communication (a2a_s... | 测试依赖 / test_depends |
| 926 | test_a2a_voting.py | → | D_INFRA_A2A A2A通信: A2A 加权投票协议 — 多 Agent 共识达成机制 (a2a_... | 测试依赖 / test_depends |
| 927 | test_a2a_work_steal.py | → | D_INFRA_A2A A2A通信: A2A 工作窃取调度器 — 跨 Agent 负载均衡 (a2a_wo... | 测试依赖 / test_depends |
| 928 | test_f5_auto_shutdown.py | → | D_INFRA_A2A A2A通信: A2A 三级仲裁引擎 — priority -> rule -> escalat... | 测试依赖 / test_depends |
| 929 | F5 端到端集成测试 — boot→run→shutdown→resta... | → | D_INFRA_A2A A2A通信: A2A 三级仲裁引擎 — priority -> rule -> escalat... | 测试依赖 / test_depends |
| 930 | F5 红蓝对抗极端测试 — DM-201513 (test_f5_red_t... | → | D_INFRA_A2A A2A通信: A2A 三级仲裁引擎 — priority -> rule -> escalat... | 测试依赖 / test_depends |
| 931 | F5 红蓝对抗极端测试 — DM-201513 (test_f5_red_t... | → | D_INFRA_A2A A2A通信: 级联守卫——防止失败在Agent间级联 (cascade_guard.py) | 测试依赖 / test_depends |
| 932 | test_arbiter.py | → | D_INFRA_A2A A2A通信: A2A 三级仲裁引擎 — priority -> rule -> escalat... | 测试依赖 / test_depends |
| 933 | test_arbitrator.py | → | D_INFRA_A2A A2A通信: A2A 三级仲裁引擎 — priority -> rule -> escalat... | 测试依赖 / test_depends |
| 934 | test_cascade_guard.py | → | D_INFRA_A2A A2A通信: 级联守卫——防止失败在Agent间级联 (cascade_guard.py) | 测试依赖 / test_depends |
| 935 | test_conflict_detector.py | → | D_INFRA_A2A A2A通信: A2A 冲突检测引擎 — 语义+文本+资源三维冲突检测 ... | 测试依赖 / test_depends |
| 936 | test_deadlock_guard.py | → | D_INFRA_A2A A2A通信: P2: 死锁守卫 (deadlock_guard.py) | 测试依赖 / test_depends |
| 937 | test_livelock_detector.py | → | D_INFRA_A2A A2A通信: P2: 活锁检测器 (livelock_detector.py) | 测试依赖 / test_depends |
| 938 | test_mcp_adapter.py | → | D_INFRA_A2A A2A通信: A2A GovernanceAdapter — Phase 4 治理集成桥接器... | 测试依赖 / test_depends |
| 939 | test_message_router.py | → | D_INFRA_A2A A2A通信: A2A Message/Part 系统 — Layer 2 Communication ... | 测试依赖 / test_depends |
| 940 | test_message_router.py | → | D_INFRA_A2A A2A通信: Message Router — A2A 消息路由 (message_router.py) | 测试依赖 / test_depends |
| 941 | test_push_notifier.py | → | D_INFRA_A2A A2A通信: Push Notifier — A2A 推送通知 (push_notifier.py) | 测试依赖 / test_depends |
| 942 | test_streaming.py | → | D_INFRA_A2A A2A通信: Streaming — A2A 流式传输 (streaming.py) | 测试依赖 / test_depends |
| 943 | test_supervisor.py | → | D_INFRA_A2A A2A通信: A2A Task 状态机 — Layer 2 Communication (a2a_s... | 测试依赖 / test_depends |
| 944 | test_supervisor.py | → | D_INFRA_A2A A2A通信: Supervisor — A2A Layer 3 Coordination (supervi... | 测试依赖 / test_depends |
| 945 | test_trigger_monitor.py | → | D_INFRA_A2A A2A通信: 触发监控器 (trigger_monitor.py) | 测试依赖 / test_depends |
| 946 | test_cross_module_integration_llm_security.py | → | D_INFRA_A2A A2A通信: 基础设施 Infrastructure — A2A Protocol 模块 (M... | 测试依赖 / test_depends |
| 947 | test_multi_agent_root.py | → | D_INFRA_A2A A2A通信: multi_agent.py —— Multi-Agent 编排基座（Phase... | 测试依赖 / test_depends |
| 948 | test_agent_cooldown.py | → | D_INFRA_RECOVERY 回滚恢复: AgentCooldown — Agent 冷却隔离器。 (agent_cool... | 测试依赖 / test_depends |
| 949 | test_auto_rollback_trigger.py | → | D_INFRA_RECOVERY 回滚恢复: AutoRollbackTrigger — 自动回滚触发器。 (auto_r... | 测试依赖 / test_depends |
| 950 | test_ce_kill_switch.py | → | D_INFRA_RECOVERY 回滚恢复: KillSwitchManager — 三级 Kill Switch 管理器。 ... | 测试依赖 / test_depends |
| 951 | test_cross_platform_shell.py | → | D_INFRA_RECOVERY 回滚恢复: CrossPlatformShell — 跨平台 Shell 脚本双输出。... | 测试依赖 / test_depends |
| 952 | test_drift_fix.py | → | D_INFRA_RECOVERY 回滚恢复: drift_fix.py | 测试依赖 / test_depends |
| 953 | test_external_merkle_proof.py | → | D_INFRA_RECOVERY 回滚恢复: External Merkle Proof — 外部可验证回滚完整性证... | 测试依赖 / test_depends |
| 954 | test_git_infra_snapshot.py | → | D_INFRA_RECOVERY 回滚恢复: GitInfraSnapshot — Git 基础设施快照与污染防护... | 测试依赖 / test_depends |
| 955 | test_credential_rotation_trigger.py | → | D_INFRA_RECOVERY 回滚恢复: CredentialRotationTrigger — 凭据自动轮替。 (cr... | 测试依赖 / test_depends |
| 956 | test_secret_rotation_aware.py | → | D_INFRA_RECOVERY 回滚恢复: SecretRotationAware — 密钥轮替感知器。 (secret... | 测试依赖 / test_depends |
| 957 | test_hallucination_guard.py | → | D_INFRA_RECOVERY 回滚恢复: HallucinationGuard — AI 幻觉防护：回滚后强制状... | 测试依赖 / test_depends |
| 958 | test_auditor.py | → | D_INFRA_RECOVERY 回滚恢复: G-CT-004 契约：Rollback -> Audit 记录回滚操作. ... | 测试依赖 / test_depends |
| 959 | test_forensic.py | → | D_INFRA_RECOVERY 回滚恢复: Forensic Engine — 取证基础设施（Phase 8 完整实... | 测试依赖 / test_depends |
| 960 | test_governance_auditor.py | → | D_INFRA_RECOVERY 回滚恢复: G-CT-004 契约：Rollback -> Audit 记录回滚操作. ... | 测试依赖 / test_depends |
| 961 | test_right_to_be_forgotten.py | → | D_INFRA_RECOVERY 回滚恢复: Right to be Forgotten — GDPR 遗忘权合规检查器... | 测试依赖 / test_depends |
| 962 | test_s3_snapshot_lifecycle.py | → | D_INFRA_RECOVERY 回滚恢复: S3 Snapshot Lifecycle Manager — 快照防生命周期... | 测试依赖 / test_depends |
| 963 | test_sqlite_dumper.py | → | D_INFRA_RECOVERY 回滚恢复: SqliteDumper — SQLite 双轨 Checkpoint 的 DB 层... | 测试依赖 / test_depends |
| 964 | test_contract.py | → | D_INFRA_RECOVERY 回滚恢复: CT-RBK-GATE-001 集成契约落地——Rollback System... | 测试依赖 / test_depends |
| 965 | test_submodule_sync.py | → | D_INFRA_RECOVERY 回滚恢复: Submodule Sync — Submodule/Monorepo 多仓库同步... | 测试依赖 / test_depends |
| 966 | test_checkpoint_gc.py | → | D_INFRA_RECOVERY 回滚恢复: CheckpointGC — Checkpoint 垃圾回收。 (checkpoi... | 测试依赖 / test_depends |
| 967 | test_venv_sync.py | → | D_INFRA_RECOVERY 回滚恢复: VenvSync — venv/conda 版本同步保障。 (venv_syn... | 测试依赖 / test_depends |
| 968 | test_env_watcher.py | → | D_INFRA_RECOVERY 回滚恢复: EnvWatcher — 环境变量热重载监控器。 (env_watch... | 测试依赖 / test_depends |
| 969 | test_runbook_generator.py | → | D_INFRA_RECOVERY 回滚恢复: RunbookGenerator — 回滚操作 Runbook 自动生成。... | 测试依赖 / test_depends |
| 970 | test_knowngoodstate_ledger.py | → | D_INFRA_RECOVERY 回滚恢复: KnowngoodstateLedger — 已验证正确状态收据。 (k... | 测试依赖 / test_depends |
| 971 | test_warm_standby.py | → | D_INFRA_RECOVERY 回滚恢复: WarmStandby — 温备热切（git worktree 副本维护... | 测试依赖 / test_depends |
| 972 | test_vulnerability_rescanner.py | → | D_INFRA_RECOVERY 回滚恢复: VulnerabilityRescanner — 依赖漏洞复扫。 (vulne... | 测试依赖 / test_depends |
| 973 | test_commit_quality_gate.py | → | D_INFRA_RECOVERY 回滚恢复: CommitQualityGate — Commit 质量基础设施。 (com... | 测试依赖 / test_depends |
| 974 | test_forward_fix_runner.py | → | D_INFRA_RECOVERY 回滚恢复: ForwardFixRunner — Forward-Fix 执行器。 (forwa... | 测试依赖 / test_depends |
| 975 | test_topology_change_log.py | → | D_INFRA_RECOVERY 回滚恢复: TopologyChangeLog — 分支拓扑变更日志。 (topolo... | 测试依赖 / test_depends |
| 976 | test_intent_archiver.py | → | D_INFRA_RECOVERY 回滚恢复: IntentArchiver — 意图存档保护。 (intent_archiv... | 测试依赖 / test_depends |
| 977 | test_rollback_abuse_detector.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackAbuseDetector — 回滚滥用检测。 (rollba... | 测试依赖 / test_depends |
| 978 | test_rollback_audit_nexus.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackAuditNexus — 回滚审计记录聚合到 Nexus ... | 测试依赖 / test_depends |
| 979 | test_rollback_bootstrap.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackBootstrap — 零依赖自举回滚器。 (rollba... | 测试依赖 / test_depends |
| 980 | test_rollback_budget.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackBudget — 回滚预算管理器。 (rollback_bu... | 测试依赖 / test_depends |
| 981 | Extreme tests for concurrent rollback (MOD-INF-... | → | D_INFRA_RECOVERY 回滚恢复: RollbackExecutor — 回滚执行器核心封装。 (rollb... | 测试依赖 / test_depends |
| 982 | Extreme tests for concurrent rollback (MOD-INF-... | → | D_INFRA_RECOVERY 回滚恢复: RollbackLock — 全局回滚锁管理。 (rollback_lock.py) | 测试依赖 / test_depends |
| 983 | test_rollback_context_restorer.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackContextRestorer — 上下文恢复器。 (roll... | 测试依赖 / test_depends |
| 984 | test_rollback_dashboard.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackDashboard — 回滚仪表盘（零依赖 Markdow... | 测试依赖 / test_depends |
| 985 | test_rollback_drill.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackDrill — 定期回滚演练调度器 (DiRT-style... | 测试依赖 / test_depends |
| 986 | test_rollback_executor_root.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackExecutor — 回滚执行器核心封装。 (rollb... | 测试依赖 / test_depends |
| 987 | test_rollback_integration.py | → | D_INFRA_RECOVERY 回滚恢复: Rollback Integration — executor 集成增强层。 (... | 测试依赖 / test_depends |
| 988 | test_rollback_lock.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackLock — 全局回滚锁管理。 (rollback_lock.py) | 测试依赖 / test_depends |
| 989 | test_rollback_loop_detector.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackLoopDetector — 回滚循环检测器。 (rollb... | 测试依赖 / test_depends |
| 990 | Extreme tests for partial_revert (MOD-INF-021 B... | → | D_INFRA_RECOVERY 回滚恢复: RollbackExecutor — 回滚执行器核心封装。 (rollb... | 测试依赖 / test_depends |
| 991 | Extreme tests for partial_revert (MOD-INF-021 B... | → | D_INFRA_RECOVERY 回滚恢复: RollbackLock — 全局回滚锁管理。 (rollback_lock.py) | 测试依赖 / test_depends |
| 992 | test_rollback_simulator.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackSimulator — 回滚模拟器（CI 集成）。 (r... | 测试依赖 / test_depends |
| 993 | test_rollback_state_machine.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackStateMachine — 回滚步骤级状态机。 (rol... | 测试依赖 / test_depends |
| 994 | test_rollback_target_staleness.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackTargetStaleness — 回滚目标陈旧度检测。... | 测试依赖 / test_depends |
| 995 | test_rollback_verifier_root.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackVerifier — 回滚后验证器。 (rollback_ve... | 测试依赖 / test_depends |
| 996 | test_rollback_wal.py | → | D_INFRA_RECOVERY 回滚恢复: RollbackWAL — 回滚预写日志。 (rollback_wal.py) | 测试依赖 / test_depends |
| 997 | test_semantic_rollback_tag.py | → | D_INFRA_RECOVERY 回滚恢复: SemanticRollbackTag — 语义化 Rollback Tag 管理... | 测试依赖 / test_depends |
| 998 | test_semantic_similar_detector.py | → | D_INFRA_RECOVERY 回滚恢复: SemanticSimilarDetector — 语义变形攻击检测。 (... | 测试依赖 / test_depends |
| 999 | test_temporal_context_adapter.py | → | D_INFRA_RECOVERY 回滚恢复: TemporalContextAdapter — AI 时间上下文断裂修复... | 测试依赖 / test_depends |
| 1000 | test_state_machine.py | → | D_INFRA_RUNTIME 运行时集成: state_machine.py | 测试依赖 / test_depends |
| 1001 | test_auto_bootstrap.py | → | D_INFRA_RUNTIME 运行时集成: Batch2 治理层契约 — 15条 Pydantic v2 Schema（P... | runtime / runtime |
| 1002 | test_auto_diagnostics.py | → | D_INFRA_RUNTIME 运行时集成: RI-12 AutoDiagnostics — 自动诊断引擎 (auto_dia... | 测试依赖 / test_depends |
| 1003 | DM-202508 验收测试: F15注册到phase_manager实现.... | → | D_INFRA_RUNTIME 运行时集成: engine.py | 测试依赖 / test_depends |
| 1004 | DM-202508 验收测试: F15注册到phase_manager实现.... | → | D_INFRA_RUNTIME 运行时集成: fix_scheduler.py | 测试依赖 / test_depends |
| 1005 | F15 自动修复引擎 - 红蓝对抗极端测试 (test_auto_... | → | D_INFRA_RUNTIME 运行时集成: fix_budget.py | 测试依赖 / test_depends |
| 1006 | F15 自动修复引擎 - 红蓝对抗极端测试 (test_auto_... | → | D_INFRA_RUNTIME 运行时集成: fix_reliability.py | 测试依赖 / test_depends |
| 1007 | F15 自动修复引擎 - 红蓝对抗极端测试 (test_auto_... | → | D_INFRA_RUNTIME 运行时集成: fix_safety.py | 测试依赖 / test_depends |
| 1008 | F15 自动修复引擎 - 红蓝对抗极端测试 (test_auto_... | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1009 | F15 自动修复引擎 - 红蓝对抗极端测试 (test_auto_... | → | D_INFRA_RUNTIME 运行时集成: self_heal_agent.py | 测试依赖 / test_depends |
| 1010 | F15 自动修复引擎 - 红蓝对抗极端测试 (test_auto_... | → | D_INFRA_RUNTIME 运行时集成: shadow_workspace.py | 测试依赖 / test_depends |
| 1011 | F11 ContextPipeline 红蓝对抗极端测试 (test_cont... | → | D_INFRA_RUNTIME 运行时集成: kill_switch.py -- safety circuit breaker (DD110... | 测试依赖 / test_depends |
| 1012 | test_host_resource_governor.py | → | D_INFRA_RUNTIME 运行时集成: host_resource_governor.py — 主机资源治理 (B17,... | 测试依赖 / test_depends |
| 1013 | test_support_system_snapshot.py | → | D_INFRA_RUNTIME 运行时集成: SystemSnapshotter — M1 系统状态镜像（CL-017 RI... | 测试依赖 / test_depends |
| 1014 | test_system_snapshot_root.py | → | D_INFRA_RUNTIME 运行时集成: SystemSnapshotter — M1 系统状态镜像（CL-017 RI... | 测试依赖 / test_depends |
| 1015 | test_token_budget_root.py | → | D_INFRA_RUNTIME 运行时集成: token_budget.py — Token 估算工具 SSoT (token_b... | 测试依赖 / test_depends |
| 1016 | test_ba_state_machine.py | → | D_INFRA_RUNTIME 运行时集成: state_machine.py | 测试依赖 / test_depends |
| 1017 | test_budget_forecaster.py | → | D_INFRA_RUNTIME 运行时集成: budget_forecaster.py — Token 预算预测 (DD120-e... | 测试依赖 / test_depends |
| 1018 | test_config_validator.py | → | D_INFRA_RUNTIME 运行时集成: M-12 ConfigValidator — 配置参数校验器 (config_... | 测试依赖 / test_depends |
| 1019 | F11 ContextPipeline 三层自动化机制测试 (test_co... | → | D_INFRA_RUNTIME 运行时集成: kill_switch.py -- safety circuit breaker (DD110... | 测试依赖 / test_depends |
| 1020 | test_contract_tester.py | → | D_INFRA_RUNTIME 运行时集成: M-11 ContractTester — 契约测试框架 (contract_t... | 测试依赖 / test_depends |
| 1021 | test_ct_pipe_routing_root.py | → | D_INFRA_RUNTIME 运行时集成: CT-PIPE-ORC-001 — TaskCard -> 管线入口节点路由... | 测试依赖 / test_depends |
| 1022 | test_ct_pipe_routing_root.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline 数据模型 (models.py) | 测试依赖 / test_depends |
| 1023 | test_dependency_root.py | → | D_INFRA_RUNTIME 运行时集成: MOD-INF-026 §18 — 资产依赖图。 (dependency.py) | 测试依赖 / test_depends |
| 1024 | test_drift_fixer.py | → | D_INFRA_RUNTIME 运行时集成: drift_fixer.py | 测试依赖 / test_depends |
| 1025 | test_drift_fixer.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1026 | test_escalation_bridge.py | → | D_INFRA_RUNTIME 运行时集成: escalation_bridge.py | 测试依赖 / test_depends |
| 1027 | test_escalation_bridge.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1028 | test_event_bus_upgrade.py | → | D_INFRA_RUNTIME 运行时集成: DEPRECATED: 此文件已废弃。 (event_bus_upgrade.py) | 测试依赖 / test_depends |
| 1029 | test_event_hooks.py | → | D_INFRA_RUNTIME 运行时集成: event_hooks.py | 测试依赖 / test_depends |
| 1030 | test_event_hooks.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1031 | test_event_store.py | → | D_INFRA_RUNTIME 运行时集成: RI-13 EventStore — 事件存储 (event_store.py) | 测试依赖 / test_depends |
| 1032 | F21 自动关闭测试 — DM-201250 (test_f21_auto_sh... | → | D_INFRA_RUNTIME 运行时集成: health.py —— ZephyrAlpha 聚合健康检查 (health.py) | 测试依赖 / test_depends |
| 1033 | F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | D_INFRA_RUNTIME 运行时集成: health.py —— ZephyrAlpha 聚合健康检查 (health.py) | 测试依赖 / test_depends |
| 1034 | F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | D_INFRA_RUNTIME 运行时集成: CT-HEALTH-001: System-wide Health Discovery Reg... | 测试依赖 / test_depends |
| 1035 | F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | D_INFRA_RUNTIME 运行时集成: longevity_monitor.py | 测试依赖 / test_depends |
| 1036 | F21 事件启动测试 — DM-201250 (test_f21_event_d... | → | D_INFRA_RUNTIME 运行时集成: health.py —— ZephyrAlpha 聚合健康检查 (health.py) | 测试依赖 / test_depends |
| 1037 | test_lifecycle_hooks.py | → | D_INFRA_RUNTIME 运行时集成: hooks.py —— 模块生命周期钩子（Phase 2 新增 | ... | 测试依赖 / test_depends |
| 1038 | test_file_watcher.py | → | D_INFRA_RUNTIME 运行时集成: file_watcher.py | 测试依赖 / test_depends |
| 1039 | test_fix_budget.py | → | D_INFRA_RUNTIME 运行时集成: fix_budget.py | 测试依赖 / test_depends |
| 1040 | test_fix_budget.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1041 | test_fix_diff.py | → | D_INFRA_RUNTIME 运行时集成: fix_diff.py | 测试依赖 / test_depends |
| 1042 | test_fix_diff.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1043 | test_fix_health_check.py | → | D_INFRA_RUNTIME 运行时集成: fix_health_check.py | 测试依赖 / test_depends |
| 1044 | test_fix_health_check.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1045 | test_fix_pattern_miner.py | → | D_INFRA_RUNTIME 运行时集成: fix_pattern_miner.py | 测试依赖 / test_depends |
| 1046 | test_fix_pattern_miner.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1047 | test_fix_reliability.py | → | D_INFRA_RUNTIME 运行时集成: fix_reliability.py | 测试依赖 / test_depends |
| 1048 | test_fix_reliability.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1049 | test_fix_report.py | → | D_INFRA_RUNTIME 运行时集成: fix_report.py | 测试依赖 / test_depends |
| 1050 | test_fix_report.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1051 | test_fix_safety.py | → | D_INFRA_RUNTIME 运行时集成: fix_safety.py | 测试依赖 / test_depends |
| 1052 | test_fix_safety.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1053 | test_fix_scheduler.py | → | D_INFRA_RUNTIME 运行时集成: fix_scheduler.py | 测试依赖 / test_depends |
| 1054 | test_fix_scheduler.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1055 | test_classifier_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetClassifier — MOD-INF-026 L2 资产自动分类... | 测试依赖 / test_depends |
| 1056 | test_classifier_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetInventoryModels — MOD-INF-026 Pydantic V2... | 测试依赖 / test_depends |
| 1057 | test_cost_tracker.py | → | D_INFRA_RUNTIME 运行时集成: RI-15 CostTracker — 成本追踪器 (cost_tracker.py) | 测试依赖 / test_depends |
| 1058 | test_dashboard_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetDashboard — MOD-INF-026 资产健康仪表盘生... | 测试依赖 / test_depends |
| 1059 | test_dashboard_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetInventoryModels — MOD-INF-026 Pydantic V2... | 测试依赖 / test_depends |
| 1060 | test_dry_run_simulator.py | → | D_INFRA_RUNTIME 运行时集成: RI-14 DryRunSimulator — 干运行模拟器 (dry_run_... | 测试依赖 / test_depends |
| 1061 | test_finding_task_bridge.py | → | D_INFRA_RUNTIME 运行时集成: Finding->TaskCard 桥接器 (finding_task_bridge.py) | 测试依赖 / test_depends |
| 1062 | test_index_generator_root.py | → | D_INFRA_RUNTIME 运行时集成: UnifiedAssetIndex — MOD-INF-026 L3 统一资产索.... | 测试依赖 / test_depends |
| 1063 | test_index_generator_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetInventoryModels — MOD-INF-026 Pydantic V2... | 测试依赖 / test_depends |
| 1064 | test_infrastructure_base.py | → | D_INFRA_RUNTIME 运行时集成: 基础设施 — Infrastructure Layer Skeleton (infr... | 测试依赖 / test_depends |
| 1065 | test_kill_switch_sim.py | → | D_INFRA_RUNTIME 运行时集成: Kill Switch T0 Hardware Simulator (kill_switch_... | 测试依赖 / test_depends |
| 1066 | test_lifecycle_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetLifecycle — MOD-INF-026 L5 ITIL生命周期自... | 测试依赖 / test_depends |
| 1067 | test_lifecycle_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetInventoryModels — MOD-INF-026 Pydantic V2... | 测试依赖 / test_depends |
| 1068 | test_metadata.py | → | D_INFRA_RUNTIME 运行时集成: MOD-INF-026 §24-25 — Git 历史元数据提取 + 多 ... | 测试依赖 / test_depends |
| 1069 | test_preemption_manager.py | → | D_INFRA_RUNTIME 运行时集成: PreemptionManager -- 优先级抢占管理器 (preempti... | 测试依赖 / test_depends |
| 1070 | test_pydantic_v2_migrator.py | → | D_INFRA_RUNTIME 运行时集成: M-15 PydanticV2Migrator — Pydantic V2 迁移工具... | 测试依赖 / test_depends |
| 1071 | test_reconciler_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetInventoryModels — MOD-INF-026 Pydantic V2... | 测试依赖 / test_depends |
| 1072 | test_reconciler_root.py | → | D_INFRA_RUNTIME 运行时集成: ReconciliationEngine — MOD-INF-026 L4 注册表 v... | 测试依赖 / test_depends |
| 1073 | test_registry_adapter_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetInventoryModels — MOD-INF-026 Pydantic V2... | 测试依赖 / test_depends |
| 1074 | test_registry_adapter_root.py | → | D_INFRA_RUNTIME 运行时集成: MOD-INF-026 §17 — 24 个异构注册表统一解析适配... | 测试依赖 / test_depends |
| 1075 | [INVARIANTS] 功能域注册表是功能域声明的唯一真源... | → | D_INFRA_RUNTIME 运行时集成: Registry Governance — MOD-INF-037 (registry_go... | 测试依赖 / test_depends |
| 1076 | test_registry_governance_root.py | → | D_INFRA_RUNTIME 运行时集成: Registry Governance — MOD-INF-037 (registry_go... | 测试依赖 / test_depends |
| 1077 | test_scanner_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetInventoryModels — MOD-INF-026 Pydantic V2... | 测试依赖 / test_depends |
| 1078 | test_scanner_root.py | → | D_INFRA_RUNTIME 运行时集成: AssetDiscoveryScanner — MOD-INF-026 L1 全量文.... | 测试依赖 / test_depends |
| 1079 | test_telemetry.py | → | D_INFRA_RUNTIME 运行时集成: AssetInventoryTelemetry — MOD-INF-026 自监控指... | 测试依赖 / test_depends |
| 1080 | test_trust_anchor_root.py | → | D_INFRA_RUNTIME 运行时集成: MOD-INF-026 §26 — 三重信任锚验证门 R20。 (tru... | 测试依赖 / test_depends |
| 1081 | test_warm_hot_gate.py | → | D_INFRA_RUNTIME 运行时集成: M-14 WarmHotGate — Warm->Hot 阻断门 (warm_hot_... | 测试依赖 / test_depends |
| 1082 | test_cross_module_integration_llm_security.py | → | D_INFRA_RUNTIME 运行时集成: MOD-INF-019: Agent Spec — LLM Gateway (llm_gat... | 测试依赖 / test_depends |
| 1083 | test_dep_version_fixer.py | → | D_INFRA_RUNTIME 运行时集成: __init__.py | 测试依赖 / test_depends |
| 1084 | test_dep_version_fixer.py | → | D_INFRA_RUNTIME 运行时集成: dep_version_fixer.py | 测试依赖 / test_depends |
| 1085 | test_dep_version_fixer.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1086 | test_engine_root.py | → | D_INFRA_RUNTIME 运行时集成: engine.py | 测试依赖 / test_depends |
| 1087 | test_engine_root.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1088 | test_interrupt_guard.py | → | D_INFRA_RUNTIME 运行时集成: interrupt_guard.py | 测试依赖 / test_depends |
| 1089 | test_llm_fix_adapter.py | → | D_INFRA_RUNTIME 运行时集成: llm_fix_adapter.py | 测试依赖 / test_depends |
| 1090 | test_llm_fix_adapter.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1091 | test_llm_gateway.py | → | D_INFRA_RUNTIME 运行时集成: MOD-INF-019: Agent Spec — LLM Gateway (llm_gat... | 测试依赖 / test_depends |
| 1092 | test_models_root.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1093 | test_scaffold_registrar.py | → | D_INFRA_RUNTIME 运行时集成: __init__.py | 测试依赖 / test_depends |
| 1094 | test_scaffold_registrar.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1095 | test_scaffold_registrar.py | → | D_INFRA_RUNTIME 运行时集成: scaffold_registrar.py | 测试依赖 / test_depends |
| 1096 | test_shadow_workspace.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1097 | test_shadow_workspace.py | → | D_INFRA_RUNTIME 运行时集成: shadow_workspace.py | 测试依赖 / test_depends |
| 1098 | test_zombie_cleaner.py | → | D_INFRA_RUNTIME 运行时集成: models.py | 测试依赖 / test_depends |
| 1099 | test_zombie_cleaner.py | → | D_INFRA_RUNTIME 运行时集成: zombie_cleaner.py | 测试依赖 / test_depends |
| 1100 | test_model_router.py | → | D_INFRA_RUNTIME 运行时集成: ModelRouter — 模型路由与降级链管理 (model_rout... | 测试依赖 / test_depends |
| 1101 | test_observability_health.py | → | D_INFRA_RUNTIME 运行时集成: hooks.py —— 模块生命周期钩子（Phase 2 新增 | ... | 测试依赖 / test_depends |
| 1102 | test_pipeline_agent_bridge.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline 数据模型 (models.py) | 测试依赖 / test_depends |
| 1103 | test_pipeline_agent_bridge.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline -> Agent Bridge — 双编排器桥接层 (pip... | 测试依赖 / test_depends |
| 1104 | test_pipeline_cost_tracker.py | → | D_INFRA_RUNTIME 运行时集成: CostTracker —— LLM 调用成本追踪器（SRC-0025）... | 测试依赖 / test_depends |
| 1105 | test_pipeline_cost_tracker.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline 数据模型 (models.py) | 测试依赖 / test_depends |
| 1106 | test_pipeline_lock.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline Lock — 双管线并发锁 (pipeline_lock.py) | 测试依赖 / test_depends |
| 1107 | test_pipeline_models.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline 数据模型 (models.py) | 测试依赖 / test_depends |
| 1108 | DM-202010: PipelineOrchestrator 自动启动/周期运... | → | D_INFRA_RUNTIME 运行时集成: Pipeline 数据模型 (models.py) | 测试依赖 / test_depends |
| 1109 | test_pipeline_roadmap.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline 未来版本路线图——v0.10.0 -> v0.12.0 .... | 测试依赖 / test_depends |
| 1110 | test_resource_optimization.py | → | D_INFRA_RUNTIME 运行时集成: models.py - Pydantic data models for resource o... | 测试依赖 / test_depends |
| 1111 | test_backpressure_manager.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline — Backpressure Manager (backpressure_... | 测试依赖 / test_depends |
| 1112 | test_backpressure_manager.py | → | D_INFRA_RUNTIME 运行时集成: backpressure_types.py - Pipeline backpressure s... | 测试依赖 / test_depends |
| 1113 | test_backpressure_types.py | → | D_INFRA_RUNTIME 运行时集成: backpressure_types.py - Pipeline backpressure s... | 测试依赖 / test_depends |
| 1114 | test_circuit_breaker_manager.py | → | D_INFRA_RUNTIME 运行时集成: CircuitBreakerManager -- standalone circuit bre... | 测试依赖 / test_depends |
| 1115 | test_circuit_breaker_manager.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline 数据模型 (models.py) | 测试依赖 / test_depends |
| 1116 | test_dead_letter_queue.py | → | D_INFRA_RUNTIME 运行时集成: DeadLetterQueue — 死信队列 (dead_letter_queue.py) | 测试依赖 / test_depends |
| 1117 | test_dead_letter_queue.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline 数据模型 (models.py) | 测试依赖 / test_depends |
| 1118 | F14 管线编排/反馈环 — 红蓝对抗端到端极端测试 (... | → | D_INFRA_RUNTIME 运行时集成: Pipeline — Backpressure Manager (backpressure_... | 测试依赖 / test_depends |
| 1119 | F14 管线编排/反馈环 — 红蓝对抗端到端极端测试 (... | → | D_INFRA_RUNTIME 运行时集成: backpressure_types.py - Pipeline backpressure s... | 测试依赖 / test_depends |
| 1120 | F14 管线编排/反馈环 — 红蓝对抗端到端极端测试 (... | → | D_INFRA_RUNTIME 运行时集成: DeadLetterQueue — 死信队列 (dead_letter_queue.py) | 测试依赖 / test_depends |
| 1121 | F14 管线编排/反馈环 — 红蓝对抗端到端极端测试 (... | → | D_INFRA_RUNTIME 运行时集成: Pipeline 数据模型 (models.py) | 测试依赖 / test_depends |
| 1122 | test_routing_plugins.py | → | D_INFRA_RUNTIME 运行时集成: CT-PIPE-ORC-001 — TaskCard -> 管线入口节点路由... | 测试依赖 / test_depends |
| 1123 | test_routing_plugins.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline 数据模型 (models.py) | 测试依赖 / test_depends |
| 1124 | test_routing_plugins.py | → | D_INFRA_RUNTIME 运行时集成: Pipeline Routing Plugin System — K8s Schedulin... | 测试依赖 / test_depends |
| 1125 | test_otel_instrumentation.py | → | D_INFRA_TELEMETRY 可观测性: otel_instrumentation.py — 全链路 OTel (B12, DD... | 测试依赖 / test_depends |
| 1126 | test_observability_health.py | → | D_INFRA_TELEMETRY 可观测性: health subsystem — 模块健康注册与 LifecycleMan... | 测试依赖 / test_depends |
| 1127 | test_auto_split.py | → | D_INTEGRATION 管线路由: execution_model.py | 测试依赖 / test_depends |
| 1128 | test_auto_split.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1129 | test_ct_audit_findings_resolved.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1130 | test_ct_blueprint_read_check.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1131 | test_ct_circuit_breaker.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1132 | test_ct_circular_dependency_scan.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1133 | test_ct_classification.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1134 | test_ct_content_length.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1135 | test_ct_content_quality.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1136 | test_ct_contract_compatibility_check.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1137 | test_ct_deduplication.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1138 | test_ct_drift_budget.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1139 | test_ct_encoding.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1140 | test_ct_enforcement_mode_check.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1141 | test_ct_field_presence.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1142 | test_ct_file_extension.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1143 | test_ct_fle_gate.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1144 | test_ct_frontmatter.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1145 | test_ct_leverage_limit.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1146 | test_ct_line_ending.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1147 | test_ct_manual_approval.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1148 | test_ct_path_blacklist.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1149 | test_ct_path_routing.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1150 | test_ct_path_whitelist.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1151 | test_ct_position_limit.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1152 | test_ct_reference_check.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1153 | test_ct_regex_pattern.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1154 | test_ct_restructuring_safety.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1155 | test_ct_rollback_exit_code.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1156 | test_ct_score_threshold.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1157 | test_ct_security_artifact_scan.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1158 | test_ct_strategy_correlation.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1159 | test_ct_temporal.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1160 | test_ct_zero_residue_check.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1161 | test_gate_types.py | → | D_INTEGRATION 管线路由: schemas.py | 测试依赖 / test_depends |
| 1162 | test_schema_schema_registry.py | → | D_INTEGRATION 管线路由: schema_registry.py | 测试依赖 / test_depends |
| 1163 | test_schema_schemas.py | → | D_INTEGRATION 管线路由: schemas.py | 测试依赖 / test_depends |
| 1164 | test_schema_schemas.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1165 | test_boot_hooks_unlock.py | → | D_INTEGRATION 管线路由: execution_model.py | 测试依赖 / test_depends |
| 1166 | test_boot_hooks_unlock.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1167 | test_cross_module_integration_llm_security.py | → | D_INTEGRATION 管线路由: PipelineOrchestrator — M1-M11 管线协调器 (pipe... | 测试依赖 / test_depends |
| 1168 | test_db.py | → | D_INTEGRATION 管线路由: base_config.py | 测试依赖 / test_depends |
| 1169 | test_db.py | → | D_INTEGRATION 管线路由: execution_model.py | 测试依赖 / test_depends |
| 1170 | test_db.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1171 | DM-202208 红蓝对抗-知识污染与检索劫持测试 (test... | → | D_INTEGRATION 管线路由: HybridRetriever — MOD-INF-011 混合检索架构 (hy... | 测试依赖 / test_depends |
| 1172 | VMS 红蓝对抗测试 — 向量注入与投毒检测 (test_vm... | → | D_INTEGRATION 管线路由: EmbeddingRouter — MOD-INF-011 双嵌入维度路由 (... | 测试依赖 / test_depends |
| 1173 | VMS 红蓝对抗测试 — 向量注入与投毒检测 (test_vm... | → | D_INTEGRATION 管线路由: InMemoryFakeVMS — MOD-INF-011 · 零依赖测试双... | 测试依赖 / test_depends |
| 1174 | VMS 红蓝对抗测试 — 向量注入与投毒检测 (test_vm... | → | D_INTEGRATION 管线路由: ProvenanceEnforcer — MOD-INF-011 写入溯源强制... | 测试依赖 / test_depends |
| 1175 | VMS 红蓝对抗测试 — 向量注入与投毒检测 (test_vm... | → | D_INTEGRATION 管线路由: VMS 共享数据模型 — MOD-INF-011 · 蓝图 §6.1 .... | 测试依赖 / test_depends |
| 1176 | DM-202210 自动化机制-事件触发与定时任务测试 (te... | → | D_INTEGRATION 管线路由: CacheLayer — MOD-INF-011 嵌入缓存与查询结果 LR... | 测试依赖 / test_depends |
| 1177 | DM-202210 自动化机制-事件触发与定时任务测试 (te... | → | D_INTEGRATION 管线路由: CollectionManager — MOD-INF-011 八大 Collectio... | 测试依赖 / test_depends |
| 1178 | DM-202210 自动化机制-事件触发与定时任务测试 (te... | → | D_INTEGRATION 管线路由: InProcessVectorMemory — MOD-INF-011 VMS 统一入... | 测试依赖 / test_depends |
| 1179 | DM-202210 自动化机制-事件触发与定时任务测试 (te... | → | D_INTEGRATION 管线路由: IndexHealthMonitor — MOD-INF-011 索引健康自检.... | 测试依赖 / test_depends |
| 1180 | DM-202210 自动化机制-事件触发与定时任务测试 (te... | → | D_INTEGRATION 管线路由: RetrievalFeedback — MOD-INF-011 FLE 检索质量消... | 测试依赖 / test_depends |
| 1181 | DM-202209 自动化机制-启动与关闭生命周期测试 (te... | → | D_INTEGRATION 管线路由: EmbeddingRouter — MOD-INF-011 双嵌入维度路由 (... | 测试依赖 / test_depends |
| 1182 | DM-202209 自动化机制-启动与关闭生命周期测试 (te... | → | D_INTEGRATION 管线路由: InMemoryFakeVMS — MOD-INF-011 · 零依赖测试双... | 测试依赖 / test_depends |
| 1183 | DM-202209 自动化机制-启动与关闭生命周期测试 (te... | → | D_INTEGRATION 管线路由: InProcessVectorMemory — MOD-INF-011 VMS 统一入... | 测试依赖 / test_depends |
| 1184 | DM-202010: PipelineOrchestrator 自动启动/周期运... | → | D_INTEGRATION 管线路由: PipelineOrchestrator — M1-M11 管线协调器 (pipe... | 测试依赖 / test_depends |
| 1185 | test_pipeline_orchestrator_root.py | → | D_INTEGRATION 管线路由: PipelineOrchestrator — M1-M11 管线协调器 (pipe... | 测试依赖 / test_depends |
| 1186 | test_task_types.py | → | D_INTEGRATION 管线路由: base_config.py | 测试依赖 / test_depends |
| 1187 | test_task_types.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1188 | test_utils_testing.py | → | D_INTEGRATION 管线路由: schemas.py | 测试依赖 / test_depends |
| 1189 | test_utils_testing.py | → | D_INTEGRATION 管线路由: severity_types.py | 测试依赖 / test_depends |
| 1190 | test_mcp_task_claim.py | → | D_INTEGRATION_GATEWAY 集成网关: ZephyrAlpha MCP Task Manager Server (task_manag... | 测试依赖 / test_depends |
| 1191 | test_cross_module_integration_llm_security.py | → | D_INTEGRATION_GATEWAY 集成网关: MCP Gateway 集中式治理节点（MOD-INF-013 §12 Ph... | 测试依赖 / test_depends |
| 1192 | test_capability_passport.py | → | D_INTELLIGENCE 上下文管理: CapabilityPassport --- AI 模型能力护照 (capabil... | 测试依赖 / test_depends |
| 1193 | test_cross_layer.py | → | D_INTELLIGENCE 上下文管理: inference_base.py | 测试依赖 / test_depends |
| 1194 | DM-202009: F10 红蓝对抗测试套件。 (test_f10_red... | → | D_INTELLIGENCE 上下文管理: CapabilityPassport --- AI 模型能力护照 (capabil... | 测试依赖 / test_depends |
| 1195 | DM-202009: F10 红蓝对抗测试套件。 (test_f10_red... | → | D_INTELLIGENCE 上下文管理: ExamOrchestrator --- 五轴入职考试主控 (exam_orc... | 测试依赖 / test_depends |
| 1196 | DM-202009: F10 红蓝对抗测试套件。 (test_f10_red... | → | D_INTELLIGENCE 上下文管理: ExamTestCases --- v3.0.5 扩展考试题库（96 题 / ... | 测试依赖 / test_depends |
| 1197 | DM-202009: F10 红蓝对抗测试套件。 (test_f10_red... | → | D_INTELLIGENCE 上下文管理: Results Writer — 持久化 benchmark 结果，支持历... | 测试依赖 / test_depends |
| 1198 | test_kb_activate.py | → | D_INTELLIGENCE 上下文管理: G4 Activate 门禁 — 人工激活（T-2-13-D） (activ... | 测试依赖 / test_depends |
| 1199 | test_kb_pipeline_activate.py | → | D_INTELLIGENCE 上下文管理: G4 Activate 门禁 — 人工激活（T-2-13-D） (activ... | 测试依赖 / test_depends |
| 1200 | test_kb_reranker.py | → | D_INTELLIGENCE 上下文管理: Cross-Encoder 重排序层 — BGE-reranker-v2-m3（T... | 测试依赖 / test_depends |
| 1201 | test_kb_unified_memory_api.py | → | D_INTELLIGENCE 上下文管理: UnifiedMemoryAPI — RI-02 统一记忆 API（M2 跨模... | 测试依赖 / test_depends |
| 1202 | test_benchmark_suite.py | → | D_INTELLIGENCE 上下文管理: BenchmarkSuite — 多维度模型性能测试用例集 (ben... | 测试依赖 / test_depends |
| 1203 | calibrate_model_diff.py 单元测试（P1-3 配套, 零... | → | D_INTELLIGENCE 上下文管理: CapabilityPassport --- AI 模型能力护照 (capabil... | 测试依赖 / test_depends |
| 1204 | test_cli.py | → | D_INTELLIGENCE 上下文管理: model-profiler.cli — 模型性能检测命令行入口 (c... | 测试依赖 / test_depends |
| 1205 | test_deepseek_v4_chat.py | → | D_INTELLIGENCE 上下文管理: DeepSeekV4Chat --- DeepSeek V4 系列模型 API 客... | 测试依赖 / test_depends |
| 1206 | test_exam_orchestrator.py | → | D_INTELLIGENCE 上下文管理: CapabilityPassport --- AI 模型能力护照 (capabil... | 测试依赖 / test_depends |
| 1207 | test_exam_orchestrator.py | → | D_INTELLIGENCE 上下文管理: ExamJudge --- LLM-as-judge 评分器 (exam_judge.py) | 测试依赖 / test_depends |
| 1208 | test_exam_orchestrator.py | → | D_INTELLIGENCE 上下文管理: ExamOrchestrator --- 五轴入职考试主控 (exam_orc... | 测试依赖 / test_depends |
| 1209 | test_exam_orchestrator.py | → | D_INTELLIGENCE 上下文管理: ExamTestCases --- v3.0.5 扩展考试题库（96 题 / ... | 测试依赖 / test_depends |
| 1210 | test_exam_test_cases.py | → | D_INTELLIGENCE 上下文管理: ExamTestCases --- v3.0.5 扩展考试题库（96 题 / ... | 测试依赖 / test_depends |
| 1211 | test_job_matcher.py | → | D_INTELLIGENCE 上下文管理: CapabilityPassport --- AI 模型能力护照 (capabil... | 测试依赖 / test_depends |
| 1212 | test_job_matcher.py | → | D_INTELLIGENCE 上下文管理: JobMatcher --- 模型岗位匹配器 (job_matcher.py) | 测试依赖 / test_depends |
| 1213 | test_model_discovery.py | → | D_INTELLIGENCE 上下文管理: ModelDiscovery — 枚举所有本地 Ollama 模型 + 远... | 测试依赖 / test_depends |
| 1214 | test_model_drift_detector.py | → | D_INTELLIGENCE 上下文管理: ModelDriftDetector — LLM 模型行为漂移检测。 (m... | 测试依赖 / test_depends |
| 1215 | test_profiler.py | → | D_INTELLIGENCE 上下文管理: BenchmarkSuite — 多维度模型性能测试用例集 (ben... | 测试依赖 / test_depends |
| 1216 | test_profiler.py | → | D_INTELLIGENCE 上下文管理: ModelProfiler — 核心性能分析引擎 (profiler.py) | 测试依赖 / test_depends |
| 1217 | test_provider_data.py | → | D_INTELLIGENCE 上下文管理: provider_data.py | 测试依赖 / test_depends |
| 1218 | test_results_writer.py | → | D_INTELLIGENCE 上下文管理: ModelProfiler — 核心性能分析引擎 (profiler.py) | 测试依赖 / test_depends |
| 1219 | test_results_writer.py | → | D_INTELLIGENCE 上下文管理: Results Writer — 持久化 benchmark 结果，支持历... | 测试依赖 / test_depends |
| 1220 | test_task_gate.py | → | D_INTELLIGENCE 上下文管理: CapabilityPassport --- AI 模型能力护照 (capabil... | 测试依赖 / test_depends |
| 1221 | test_task_model_learner.py | → | D_INTELLIGENCE 上下文管理: ModelTaskMatrix — 任务×模型性能学习引擎 (task... | 测试依赖 / test_depends |
| 1222 | F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | D_OPS 反馈循环: metrics.py —— 轻量级 Metrics 收集基础设施（Ph... | 测试依赖 / test_depends |
| 1223 | F21 事件启动测试 — DM-201250 (test_f21_event_d... | → | D_OPS 反馈循环: metrics.py —— 轻量级 Metrics 收集基础设施（Ph... | 测试依赖 / test_depends |
| 1224 | test_observability_metrics.py | → | D_OPS 反馈循环: metrics.py —— 轻量级 Metrics 收集基础设施（Ph... | 测试依赖 / test_depends |
| 1225 | test_observability_tracing.py | → | D_OPS 反馈循环: tracing.py —— OpenTelemetry 分布式追踪（Phase... | 测试依赖 / test_depends |
| 1226 | test_e_position_reconciler.py | → | D_POSITION 仓位管理: Position Reconciler — v0.10.1 持仓对账: execut... | 测试依赖 / test_depends |
| 1227 | test_position_reconciler.py | → | D_POSITION 仓位管理: Position Reconciler — v0.10.1 持仓对账: execut... | 测试依赖 / test_depends |
| 1228 | test_ml_experiment_pipeline.py | → | D_RISK 风控: ml_experiment_pipeline.py | 测试依赖 / test_depends |
| 1229 | test_a2a_check.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.a2a... | 测试依赖 / test_depends |
| 1230 | test_agent_creation_policy.py | → | D_SECURITY 对抗验证: AgentCreationPolicy — Agent 创建策略. (agent_c... | 测试依赖 / test_depends |
| 1231 | 测试 L2 ABACGuard — 五维属性权限判定 (test_aba... | → | D_SECURITY 对抗验证: ABACGuard — 基于属性的权限守卫. (abac_guard.py) | 测试依赖 / test_depends |
| 1232 | 测试 L2 ABACGuard — 五维属性权限判定 (test_aba... | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1233 | MOD-INF-018 test_adversarial.py — 对抗性测试: ... | → | D_SECURITY 对抗验证: CrossSessionDetector — 跨 Session 检测器. (cro... | 测试依赖 / test_depends |
| 1234 | MOD-INF-018 test_adversarial.py — 对抗性测试: ... | → | D_SECURITY 对抗验证: ReplayAttackGuard — 重放攻击防护. (replay_atta... | 测试依赖 / test_depends |
| 1235 | MOD-INF-018 test_adversarial.py — 对抗性测试: ... | → | D_SECURITY 对抗验证: MonotonicClock — 单调时钟. (monotonic_clock.py) | 测试依赖 / test_depends |
| 1236 | MOD-INF-018 test_adversarial.py — 对抗性测试: ... | → | D_SECURITY 对抗验证: NonRepudiation — 不可抵赖性审计签名. (non_repu... | 测试依赖 / test_depends |
| 1237 | test_adversarial_resilience.py | → | D_SECURITY 对抗验证: AdversarialResilience — 对抗性韧性与 OWASP 覆... | 测试依赖 / test_depends |
| 1238 | MOD-INF-018 跨模型一致性测试 — DeepSeek/GLM/Cl... | → | D_SECURITY 对抗验证: RBACRoleDeriver — RBAC 角色派生器. (derive_rba... | 测试依赖 / test_depends |
| 1239 | MOD-INF-018 跨模型一致性测试 — DeepSeek/GLM/Cl... | → | D_SECURITY 对抗验证: PermissionGuard — 七层权限编排器. (permission_... | 测试依赖 / test_depends |
| 1240 | MOD-INF-018 跨模型一致性测试 — DeepSeek/GLM/Cl... | → | D_SECURITY 对抗验证: RBACGuard — 基于角色的权限守卫. (rbac_guard.py) | 测试依赖 / test_depends |
| 1241 | MOD-INF-018 跨模型一致性测试 — DeepSeek/GLM/Cl... | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1242 | MOD-INF-018 跨模型一致性测试 — DeepSeek/GLM/Cl... | → | D_SECURITY 对抗验证: ImmutableCore — 不可变核心验证器. (immutable_c... | 测试依赖 / test_depends |
| 1243 | MOD-INF-018 跨模型一致性测试 — DeepSeek/GLM/Cl... | → | D_SECURITY 对抗验证: IntegritySelfCheck — 完整性自检. (integrity_se... | 测试依赖 / test_depends |
| 1244 | 跨切面 D 异常检测 + 蓝图保真 + 原生API守卫 + 内... | → | D_SECURITY 对抗验证: BlueprintFidelity — 蓝图保真度检查. (blueprint... | 测试依赖 / test_depends |
| 1245 | 跨切面 D 异常检测 + 蓝图保真 + 原生API守卫 + 内... | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.det... | 测试依赖 / test_depends |
| 1246 | 跨切面 D 异常检测 + 蓝图保真 + 原生API守卫 + 内... | → | D_SECURITY 对抗验证: MemoryGuard — 内存访问守卫. (memory_guard.py) | 测试依赖 / test_depends |
| 1247 | 跨切面 D 异常检测 + 蓝图保真 + 原生API守卫 + 内... | → | D_SECURITY 对抗验证: NativeApiGuard — 原生 API 守卫. (native_api_gu... | 测试依赖 / test_depends |
| 1248 | cybersec 2026 独立测试. (test_cybersec_2026.py) | → | D_SECURITY 对抗验证: Cybersec2026Guard — 2026 网络安全威胁检测. (cy... | 测试依赖 / test_depends |
| 1249 | 测试 DecisionExplainer — 结构化拒绝原因 (test_... | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.dec... | 测试依赖 / test_depends |
| 1250 | 决策注册表测试. (test_decisions.py) | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.dec... | 测试依赖 / test_depends |
| 1251 | MOD-INF-018 test_derive_rbac.py — RBAC 自动派.... | → | D_SECURITY 对抗验证: RBACGuard — 基于角色的权限守卫. (rbac_guard.py) | 测试依赖 / test_depends |
| 1252 | MOD-INF-018 test_derive_rbac.py — RBAC 自动派.... | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1253 | 测试 L7 DryRun — 权限模拟与影响分析 (test_dry_... | → | D_SECURITY 对抗验证: DryRun — 权限模拟与影响分析. (dry_run.py) | 测试依赖 / test_depends |
| 1254 | 测试 L7 DryRun — 权限模拟与影响分析 (test_dry_... | → | D_SECURITY 对抗验证: RBACGuard — 基于角色的权限守卫. (rbac_guard.py) | 测试依赖 / test_depends |
| 1255 | 测试 L7 DryRun — 权限模拟与影响分析 (test_dry_... | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1256 | 测试 L0 EngineDegradation — 权限引擎降级策略 (... | → | D_SECURITY 对抗验证: EngineDegradation — 引擎降级管理. (engine_degr... | 测试依赖 / test_depends |
| 1257 | 七项增强安全机制整合测试. (test_enhanced_securi... | → | D_SECURITY 对抗验证: AgentCreationPolicy — Agent 创建策略. (agent_c... | 测试依赖 / test_depends |
| 1258 | 七项增强安全机制整合测试. (test_enhanced_securi... | → | D_SECURITY 对抗验证: AutoMaintenance — 自动维护与规则健康仪表盘. (a... | 测试依赖 / test_depends |
| 1259 | 七项增强安全机制整合测试. (test_enhanced_securi... | → | D_SECURITY 对抗验证: CacheInvalidation — 缓存失效事件管理. (cache_i... | 测试依赖 / test_depends |
| 1260 | 七项增强安全机制整合测试. (test_enhanced_securi... | → | D_SECURITY 对抗验证: CrossSessionDetector — 跨 Session 检测器. (cro... | 测试依赖 / test_depends |
| 1261 | 七项增强安全机制整合测试. (test_enhanced_securi... | → | D_SECURITY 对抗验证: EmergencyOverride — 紧急覆盖令牌管理. (emergen... | 测试依赖 / test_depends |
| 1262 | 七项增强安全机制整合测试. (test_enhanced_securi... | → | D_SECURITY 对抗验证: PermissionHooks — 权限钩子注册表. (permission_... | 测试依赖 / test_depends |
| 1263 | 测试 AgentRbac 异常类型 (test_exceptions_agent_... | → | D_SECURITY 对抗验证: AgentRbac 异常类型. (exceptions.py) | 测试依赖 / test_depends |
| 1264 | 跨切面 B 取证审计 A 层——genesis/asymmetric/no... | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.asy... | 测试依赖 / test_depends |
| 1265 | 跨切面 B 取证审计 A 层——genesis/asymmetric/no... | → | D_SECURITY 对抗验证: GenesisBootstrap — RBAC系统启动引导器. (genesi... | 测试依赖 / test_depends |
| 1266 | 跨切面 B 取证审计 A 层——genesis/asymmetric/no... | → | D_SECURITY 对抗验证: NonRepudiation — 不可抵赖性审计签名. (non_repu... | 测试依赖 / test_depends |
| 1267 | 跨切面 B 取证审计 B 层——path/shell/rule_injec... | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.det... | 测试依赖 / test_depends |
| 1268 | 跨切面 B 取证审计 B 层——path/shell/rule_injec... | → | D_SECURITY 对抗验证: PathGuard — 路径守卫. (path_guard.py) | 测试依赖 / test_depends |
| 1269 | 跨切面 B 取证审计 B 层——path/shell/rule_injec... | → | D_SECURITY 对抗验证: RuleInjectionGuard — 规则注入守卫. (rule_injec... | 测试依赖 / test_depends |
| 1270 | 跨切面 B 取证审计 C 层——audit_log/replay/lega... | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.gua... | 测试依赖 / test_depends |
| 1271 | 跨切面 B 取证审计 C 层——audit_log/replay/lega... | → | D_SECURITY 对抗验证: ReplayAttackGuard — 重放攻击防护. (replay_atta... | 测试依赖 / test_depends |
| 1272 | 跨切面 B 取证审计 C 层——audit_log/replay/lega... | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.leg... | 测试依赖 / test_depends |
| 1273 | 跨切面 B 取证审计 C 层——audit_log/replay/lega... | → | D_SECURITY 对抗验证: MonotonicClock — 单调时钟. (monotonic_clock.py) | 测试依赖 / test_depends |
| 1274 | 跨切面 B 取证审计 C 层——audit_log/replay/lega... | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.rol... | 测试依赖 / test_depends |
| 1275 | 测试防护层模块 — ColdStartLock, AutoGuard, Esc... | → | D_SECURITY 对抗验证: GuardLayers — 权限守卫层组件. (guard_layers.py) | 测试依赖 / test_depends |
| 1276 | 测试防护层模块 — ColdStartLock, AutoGuard, Esc... | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1277 | 测试 AgentIdentity — 身份模型 (test_identity.py) | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1278 | 测试 L0 ImmutableCore — 硬编码不可变保护区 (te... | → | D_SECURITY 对抗验证: ImmutableCore — 不可变核心验证器. (immutable_c... | 测试依赖 / test_depends |
| 1279 | 测试 L3 InputGuard — 参数级护栏 (test_input_gu... | → | D_SECURITY 对抗验证: InputGuard — 输入参数守卫. (input_guard.py) | 测试依赖 / test_depends |
| 1280 | 集成 + 契约验证测试. (test_integration_agent_rb... | → | D_SECURITY 对抗验证: IntegrationManager — 系统集成注册与健康检查. (... | 测试依赖 / test_depends |
| 1281 | 集成 + 契约验证测试. (test_integration_agent_rb... | → | D_SECURITY 对抗验证: ContractVerifier — 契约验证器. (contract_verif... | 测试依赖 / test_depends |
| 1282 | test_integration_root.py | → | D_SECURITY 对抗验证: IntegrationManager — 系统集成注册与健康检查. (... | 测试依赖 / test_depends |
| 1283 | 完整性自检测试. (test_integrity_agent_rbac.py) | → | D_SECURITY 对抗验证: IntegritySelfCheck — 完整性自检. (integrity_se... | 测试依赖 / test_depends |
| 1284 | 测试 IntentBinder — 意图绑定与连续验证 (test_i... | → | D_SECURITY 对抗验证: IntentBinder — 意图绑定与漂移检测. (intent_bin... | 测试依赖 / test_depends |
| 1285 | 测试 L0 KillSwitch — 全局熔断机制 (test_kill_s... | → | D_SECURITY 对抗验证: KillSwitch — 熔断器. (kill_switch.py) | 测试依赖 / test_depends |
| 1286 | 新攻击 / cybersec 2026 专项测试. (test_novel_at... | → | D_SECURITY 对抗验证: Cybersec2026Guard — 2026 网络安全威胁检测. (cy... | 测试依赖 / test_depends |
| 1287 | 新攻击 / cybersec 2026 专项测试. (test_novel_at... | → | D_SECURITY 对抗验证: NovelAttackGuard — 新型攻击行为画像. (novel_at... | 测试依赖 / test_depends |
| 1288 | 测试 L6 Observability — 指标上报与异常检测 (te... | → | D_SECURITY 对抗验证: ObservabilityReporter — 指标上报与异常检测. (o... | 测试依赖 / test_depends |
| 1289 | 测试 L5 OutputGuard — 输出护栏 (test_output_gu... | → | D_SECURITY 对抗验证: OutputGuard — 输出内容守卫. (output_guard.py) | 测试依赖 / test_depends |
| 1290 | 测试 PermissionGuard — 七层统一编排 (test_perm... | → | D_SECURITY 对抗验证: PermissionGuard — 七层权限编排器. (permission_... | 测试依赖 / test_depends |
| 1291 | 测试 PermissionGuard — 七层统一编排 (test_perm... | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1292 | 测试 PermissionGuard — 七层统一编排 (test_perm... | → | D_SECURITY 对抗验证: ImmutableCore — 不可变核心验证器. (immutable_c... | 测试依赖 / test_depends |
| 1293 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: CanaryRolloutManager — 灰度发布管理器. (canary... | 测试依赖 / test_depends |
| 1294 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: FalseCompletionDetector — 虚假完成检测. (false... | 测试依赖 / test_depends |
| 1295 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: MultiAgentCollusionDetector — 多 agent 合谋检... | 测试依赖 / test_depends |
| 1296 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: DryRun — 权限模拟与影响分析. (dry_run.py) | 测试依赖 / test_depends |
| 1297 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: GuardLayers — 权限守卫层组件. (guard_layers.py) | 测试依赖 / test_depends |
| 1298 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: ABACGuard — 基于属性的权限守卫. (abac_guard.py) | 测试依赖 / test_depends |
| 1299 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: InputGuard — 输入参数守卫. (input_guard.py) | 测试依赖 / test_depends |
| 1300 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: MemoryProvenanceGuard — 记忆来源溯源守卫. (mem... | 测试依赖 / test_depends |
| 1301 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: OutputGuard — 输出内容守卫. (output_guard.py) | 测试依赖 / test_depends |
| 1302 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: PermissionGuard — 七层权限编排器. (permission_... | 测试依赖 / test_depends |
| 1303 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: SequenceGuard — 操作序列守卫. (sequence_guard.py) | 测试依赖 / test_depends |
| 1304 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: TOCTOUGuard — TOCTOU (Time-of-Check to Time-of... | 测试依赖 / test_depends |
| 1305 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1306 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: ImmutableCore — 不可变核心验证器. (immutable_c... | 测试依赖 / test_depends |
| 1307 | 权限自动化测试——120+攻击向量/跨模型一致性/对.... | → | D_SECURITY 对抗验证: KillSwitch — 熔断器. (kill_switch.py) | 测试依赖 / test_depends |
| 1308 | MOD-INF-018 test_post_action.py — L5 Post-Acti... | → | D_SECURITY 对抗验证: PermissionHooks — 权限钩子注册表. (permission_... | 测试依赖 / test_depends |
| 1309 | RBAC 自动启动/关闭生命周期集成测试. (test_rbac_... | → | D_SECURITY 对抗验证: zephyr.security.access_control — Agent RBAC 权... | 测试依赖 / test_depends |
| 1310 | RBAC 自动启动/关闭生命周期集成测试. (test_rbac_... | → | D_SECURITY 对抗验证: EngineDegradation — 引擎降级管理. (engine_degr... | 测试依赖 / test_depends |
| 1311 | RBAC 自动启动/关闭生命周期集成测试. (test_rbac_... | → | D_SECURITY 对抗验证: GenesisBootstrap — RBAC系统启动引导器. (genesi... | 测试依赖 / test_depends |
| 1312 | RBAC 自动启动/关闭生命周期集成测试. (test_rbac_... | → | D_SECURITY 对抗验证: ImmutableCore — 不可变核心验证器. (immutable_c... | 测试依赖 / test_depends |
| 1313 | RBAC 自动启动/关闭生命周期集成测试. (test_rbac_... | → | D_SECURITY 对抗验证: KillSwitch — 熔断器. (kill_switch.py) | 测试依赖 / test_depends |
| 1314 | 测试 L1 RBACGuard — 三层权限模型 (test_rbac_gu... | → | D_SECURITY 对抗验证: RBACGuard — 基于角色的权限守卫. (rbac_guard.py) | 测试依赖 / test_depends |
| 1315 | 测试 L1 RBACGuard — 三层权限模型 (test_rbac_gu... | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1316 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: AdversarialResilience — 对抗性韧性与 OWASP 覆... | 测试依赖 / test_depends |
| 1317 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: AgentCreationPolicy — Agent 创建策略. (agent_c... | 测试依赖 / test_depends |
| 1318 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: AutoMaintenance — 自动维护与规则健康仪表盘. (a... | 测试依赖 / test_depends |
| 1319 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: ColdStartLock — 冷启动锁. (cold_start_lock.py) | 测试依赖 / test_depends |
| 1320 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: CrossCutting — 横切面权限组件. (cross_cutting.py) | 测试依赖 / test_depends |
| 1321 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: ContextDriftDetector — 上下文漂移与范围蔓延检... | 测试依赖 / test_depends |
| 1322 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: CrossSessionDetector — 跨 Session 检测器. (cro... | 测试依赖 / test_depends |
| 1323 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: FalseCompletionDetector — 虚假完成检测. (false... | 测试依赖 / test_depends |
| 1324 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: MultiAgentCollusionDetector — 多 agent 合谋检... | 测试依赖 / test_depends |
| 1325 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: EmergencyOverride — 紧急覆盖令牌管理. (emergen... | 测试依赖 / test_depends |
| 1326 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: EngineDegradation — 引擎降级管理. (engine_degr... | 测试依赖 / test_depends |
| 1327 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: ABACGuard — 基于属性的权限守卫. (abac_guard.py) | 测试依赖 / test_depends |
| 1328 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: InputGuard — 输入参数守卫. (input_guard.py) | 测试依赖 / test_depends |
| 1329 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: OutputGuard — 输出内容守卫. (output_guard.py) | 测试依赖 / test_depends |
| 1330 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: PathGuard — 路径守卫. (path_guard.py) | 测试依赖 / test_depends |
| 1331 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: PermissionGuard — 七层权限编排器. (permission_... | 测试依赖 / test_depends |
| 1332 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: RBACGuard — 基于角色的权限守卫. (rbac_guard.py) | 测试依赖 / test_depends |
| 1333 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: ReplayAttackGuard — 重放攻击防护. (replay_atta... | 测试依赖 / test_depends |
| 1334 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: SequenceGuard — 操作序列守卫. (sequence_guard.py) | 测试依赖 / test_depends |
| 1335 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: TOCTOUGuard — TOCTOU (Time-of-Check to Time-of... | 测试依赖 / test_depends |
| 1336 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: Agent identity — 角色与成熟度定义. (identity.py) | 测试依赖 / test_depends |
| 1337 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: ImmutableCore — 不可变核心验证器. (immutable_c... | 测试依赖 / test_depends |
| 1338 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: IntentBinder — 意图绑定与漂移检测. (intent_bin... | 测试依赖 / test_depends |
| 1339 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: KillSwitch — 熔断器. (kill_switch.py) | 测试依赖 / test_depends |
| 1340 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: MonotonicClock — 单调时钟. (monotonic_clock.py) | 测试依赖 / test_depends |
| 1341 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: NonRepudiation — 不可抵赖性审计签名. (non_repu... | 测试依赖 / test_depends |
| 1342 | MOD-INF-018 对抗性红队测试 — 专用 Agent 尝试绕... | → | D_SECURITY 对抗验证: PermissionHooks — 权限钩子注册表. (permission_... | 测试依赖 / test_depends |
| 1343 | 风险缓解测试. (test_risk_mitigation_agent_rbac.py) | → | D_SECURITY 对抗验证: RiskMitigation — 风险评估与缓解策略. (risk_mit... | 测试依赖 / test_depends |
| 1344 | 测试 L4 SequenceGuard — 操作序列追踪与危险序列... | → | D_SECURITY 对抗验证: SequenceGuard — 操作序列守卫. (sequence_guard.py) | 测试依赖 / test_depends |
| 1345 | session 隔离 stash 红蓝对抗极限测试。 (test_ses... | → | D_SECURITY 对抗验证: Session 级并发协调模块（P2-SES 落地）。 (sessio... | 测试依赖 / test_depends |
| 1346 | 测试 TOCTOU Guard — 竞态防护 (test_toctou_guar... | → | D_SECURITY 对抗验证: TOCTOUGuard — TOCTOU (Time-of-Check to Time-of... | 测试依赖 / test_depends |
| 1347 | Vibe Coding / Novel Attack / Cybersec 2026 攻击... | → | D_SECURITY 对抗验证: Cybersec2026Guard — 2026 网络安全威胁检测. (cy... | 测试依赖 / test_depends |
| 1348 | Vibe Coding / Novel Attack / Cybersec 2026 攻击... | → | D_SECURITY 对抗验证: NovelAttackGuard — 新型攻击行为画像. (novel_at... | 测试依赖 / test_depends |
| 1349 | Vibe Coding / Novel Attack / Cybersec 2026 攻击... | → | D_SECURITY 对抗验证: VibeCodingGuard — Vibe Coding 攻击面检测. (vib... | 测试依赖 / test_depends |
| 1350 | test_audit_log_guard.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.gua... | 测试依赖 / test_depends |
| 1351 | [INVARIANTS] E2E tests cover DecisionTable 12-r... | → | D_SECURITY 对抗验证: cascade_analyzer.py | 测试依赖 / test_depends |
| 1352 | [INVARIANTS] E2E tests cover DecisionTable 12-r... | → | D_SECURITY 对抗验证: decision_table.py | 测试依赖 / test_depends |
| 1353 | [INVARIANTS] E2E tests cover DecisionTable 12-r... | → | D_SECURITY 对抗验证: deprecation_tracker.py | 测试依赖 / test_depends |
| 1354 | [INVARIANTS] E2E tests cover DecisionTable 12-r... | → | D_SECURITY 对抗验证: judge.py | 测试依赖 / test_depends |
| 1355 | [INVARIANTS] E2E tests cover DecisionTable 12-r... | → | D_SECURITY 对抗验证: safety_fence.py | 测试依赖 / test_depends |
| 1356 | test_auto_bootstrap.py | → | D_SECURITY 对抗验证: DryRun — 权限模拟与影响分析. (dry_run.py) | contract / contract |
| 1357 | test_auto_maintenance.py | → | D_SECURITY 对抗验证: AutoMaintenance — 自动维护与规则健康仪表盘. (a... | 测试依赖 / test_depends |
| 1358 | test_blueprint_fidelity.py | → | D_SECURITY 对抗验证: BlueprintFidelity — 蓝图保真度检查. (blueprint... | 测试依赖 / test_depends |
| 1359 | test_canary_rollout_manager.py | → | D_SECURITY 对抗验证: CanaryRolloutManager — 灰度发布管理器. (canary... | 测试依赖 / test_depends |
| 1360 | test_capability_check.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.cap... | 测试依赖 / test_depends |
| 1361 | test_cold_start_lock.py | → | D_SECURITY 对抗验证: ColdStartLock — 冷启动锁. (cold_start_lock.py) | 测试依赖 / test_depends |
| 1362 | test_cold_start_lock.py | → | D_SECURITY 对抗验证: ImmutableCore — 不可变核心验证器. (immutable_c... | 测试依赖 / test_depends |
| 1363 | test_context_drift_detector.py | → | D_SECURITY 对抗验证: ContextDriftDetector — 上下文漂移与范围蔓延检... | 测试依赖 / test_depends |
| 1364 | test_abac_guard_root.py | → | D_SECURITY 对抗验证: ABACGuard — 基于属性的权限守卫. (abac_guard.py) | 测试依赖 / test_depends |
| 1365 | test_contract_verifier.py | → | D_SECURITY 对抗验证: ContractVerifier — 契约验证器. (contract_verif... | 测试依赖 / test_depends |
| 1366 | test_rbac_guard_root.py | → | D_SECURITY 对抗验证: RBACGuard — 基于角色的权限守卫. (rbac_guard.py) | 测试依赖 / test_depends |
| 1367 | test_cross_cutting.py | → | D_SECURITY 对抗验证: CrossCutting — 横切面权限组件. (cross_cutting.py) | 测试依赖 / test_depends |
| 1368 | test_cross_session_detector.py | → | D_SECURITY 对抗验证: CrossSessionDetector — 跨 Session 检测器. (cro... | 测试依赖 / test_depends |
| 1369 | test_decision_explainer_root.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.dec... | 测试依赖 / test_depends |
| 1370 | test_decision_registry.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.dec... | 测试依赖 / test_depends |
| 1371 | test_dependency_auditor.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.dep... | 测试依赖 / test_depends |
| 1372 | test_escalation_handler.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.esc... | 测试依赖 / test_depends |
| 1373 | test_compliance_mapper.py | → | D_SECURITY 对抗验证: Compliance Mapper — D-022-13 合规映射器: 操作-... | 测试依赖 / test_depends |
| 1374 | test_session_worktree.py — worktree 物理隔离端... | → | D_SECURITY 对抗验证: Session 级并发协调模块（P2-SES 落地）。 (sessio... | 测试依赖 / test_depends |
| 1375 | test_governance_a2a_check.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.a2a... | 测试依赖 / test_depends |
| 1376 | test_governance_approver_check.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.app... | 测试依赖 / test_depends |
| 1377 | test_governance_bootstrap_superadmin.py | → | D_SECURITY 对抗验证: BootstrapSuperadmin — Superadmin 账户启动器. (... | 测试依赖 / test_depends |
| 1378 | test_governance_capability_check.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.cap... | 测试依赖 / test_depends |
| 1379 | test_governance_contracts.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.con... | 测试依赖 / test_depends |
| 1380 | test_guard_layers_root.py | → | D_SECURITY 对抗验证: GuardLayers — 权限守卫层组件. (guard_layers.py) | 测试依赖 / test_depends |
| 1381 | test_intent_binder_root.py | → | D_SECURITY 对抗验证: IntentBinder — 意图绑定与漂移检测. (intent_bin... | 测试依赖 / test_depends |
| 1382 | test_orphan_detector.py | → | D_SECURITY 对抗验证: [INVARIANTS] 蓝图 §4 文件清单与代码双向对齐 (o... | 测试依赖 / test_depends |
| 1383 | test_memory_guard.py | → | D_SECURITY 对抗验证: MemoryGuard — 内存访问守卫. (memory_guard.py) | 测试依赖 / test_depends |
| 1384 | test_memory_provenance_guard.py | → | D_SECURITY 对抗验证: MemoryProvenanceGuard — 记忆来源溯源守卫. (mem... | 测试依赖 / test_depends |
| 1385 | test_multi_agent_collusion_detector.py | → | D_SECURITY 对抗验证: MultiAgentCollusionDetector — 多 agent 合谋检... | 测试依赖 / test_depends |
| 1386 | test_observability_root.py | → | D_SECURITY 对抗验证: ObservabilityReporter — 指标上报与异常检测. (o... | 测试依赖 / test_depends |
| 1387 | test_path_guard.py | → | D_SECURITY 对抗验证: PathGuard — 路径守卫. (path_guard.py) | 测试依赖 / test_depends |
| 1388 | test_rollback_sandbox.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.rol... | 测试依赖 / test_depends |
| 1389 | test_rule_injection_guard.py | → | D_SECURITY 对抗验证: RuleInjectionGuard — 规则注入守卫. (rule_injec... | 测试依赖 / test_depends |
| 1390 | test_async_monitor.py | → | D_SECURITY 对抗验证: async_monitor.py | 测试依赖 / test_depends |
| 1391 | test_commit_trigger.py | → | D_SECURITY 对抗验证: circuit_breaker.py | 测试依赖 / test_depends |
| 1392 | test_constitution_engine.py | → | D_SECURITY 对抗验证: constitution_engine.py | 测试依赖 / test_depends |
| 1393 | test_defense_runner.py | → | D_SECURITY 对抗验证: models.py | 测试依赖 / test_depends |
| 1394 | test_game_day_scheduler.py | → | D_SECURITY 对抗验证: models.py | 测试依赖 / test_depends |
| 1395 | test_phase_manager_integration.py | → | D_SECURITY 对抗验证: game_day_scheduler.py | 测试依赖 / test_depends |
| 1396 | test_self_heal_agent.py | → | D_SECURITY 对抗验证: zephyr.security.access_control — Agent RBAC 权... | 测试依赖 / test_depends |
| 1397 | test_session_lifecycle.py | → | D_SECURITY 对抗验证: Stub module: zephyr.security.access_control.ses... | 测试依赖 / test_depends |
| 1398 | test_auto_bootstrap.py | → | D_SECURITY_LLM LLM防御: LLM Security Gateway - Streamlit Dashboard. (ap... | runtime / runtime |
| 1399 | test_adversarial_robustness.py | → | D_SECURITY_LLM LLM防御: adversarial_robustness.py — 对抗鲁棒性 (B8, DD... | 测试依赖 / test_depends |
| 1400 | test_alignment_scorer.py | → | D_SECURITY_LLM LLM防御: alignment_scorer.py — 对齐评分 (B11, DD85, TAS... | 测试依赖 / test_depends |
| 1401 | test_lsg_pattern_tracker.py | → | D_SECURITY_LLM LLM防御: lsg_pattern_tracker.py — LSG 模式逃逸追踪 (B20... | 测试依赖 / test_depends |
| 1402 | test_poisoning_monitor.py | → | D_SECURITY_LLM LLM防御: poisoning_monitor.py — Embed 污染检测 (DD97, T... | 测试依赖 / test_depends |
| 1403 | test_sensitivity_classifier.py | → | D_SECURITY_LLM LLM防御: sensitivity_classifier.py — 数据分级 (B9, DD83... | 测试依赖 / test_depends |
| 1404 | test_solo_dev_safety_net.py | → | D_SECURITY_LLM LLM防御: solo_dev_safety_net.py — 单人无审查安全网 (B15... | 测试依赖 / test_depends |
| 1405 | test_adversarial_mutator.py | → | D_SECURITY_LLM LLM防御: adversarial_mutator.py | 测试依赖 / test_depends |
| 1406 | test_behavior_audit_logger.py | → | D_SECURITY_LLM LLM防御: behavior_audit_logger.py | 测试依赖 / test_depends |
| 1407 | test_code_integrity.py | → | D_SECURITY_LLM LLM防御: code_integrity.py | 测试依赖 / test_depends |
| 1408 | test_db.py | → | D_SECURITY_LLM LLM防御: InputSanitizer: path whitelist + command whitel... | 测试依赖 / test_depends |
| 1409 | test_fail_closed.py | → | D_SECURITY_LLM LLM防御: gateway.py | 测试依赖 / test_depends |
| 1410 | test_fail_closed.py | → | D_SECURITY_LLM LLM防御: protocol.py | 测试依赖 / test_depends |
| 1411 | test_gateway_e2e.py | → | D_SECURITY_LLM LLM防御: gateway.py | 测试依赖 / test_depends |
| 1412 | test_injection_patterns.py | → | D_SECURITY_LLM LLM防御: injection_patterns.py | 测试依赖 / test_depends |
| 1413 | test_input_sanitizer_llm_security.py | → | D_SECURITY_LLM LLM防御: InputSanitizer: path whitelist + command whitel... | 测试依赖 / test_depends |
| 1414 | test_isolation.py | → | D_SECURITY_LLM LLM防御: isolation.py | 测试依赖 / test_depends |
| 1415 | test_l0_supply_chain.py | → | D_SECURITY_LLM LLM防御: l0_supply_chain.py | 测试依赖 / test_depends |
| 1416 | test_l0_supply_chain.py | → | D_SECURITY_LLM LLM防御: protocol.py | 测试依赖 / test_depends |
| 1417 | test_l1_input_defense.py | → | D_SECURITY_LLM LLM防御: l1_input.py | 测试依赖 / test_depends |
| 1418 | test_l1_input_defense.py | → | D_SECURITY_LLM LLM防御: protocol.py | 测试依赖 / test_depends |
| 1419 | test_l2_prompt_protection.py | → | D_SECURITY_LLM LLM防御: l2_prompt_protection.py | 测试依赖 / test_depends |
| 1420 | test_l2_prompt_protection.py | → | D_SECURITY_LLM LLM防御: protocol.py | 测试依赖 / test_depends |
| 1421 | test_l2a_process_sandbox.py | → | D_SECURITY_LLM LLM防御: l2a_process_sandbox.py | 测试依赖 / test_depends |
| 1422 | test_l3_output_security.py | → | D_SECURITY_LLM LLM防御: l3_output.py | 测试依赖 / test_depends |
| 1423 | test_l3_output_security.py | → | D_SECURITY_LLM LLM防御: protocol.py | 测试依赖 / test_depends |
| 1424 | test_l4_agent_security.py | → | D_SECURITY_LLM LLM防御: l4_agent.py | 测试依赖 / test_depends |
| 1425 | test_l4_agent_security.py | → | D_SECURITY_LLM LLM防御: protocol.py | 测试依赖 / test_depends |
| 1426 | test_l5_resource_protection.py | → | D_SECURITY_LLM LLM防御: l5_resource_protection.py | 测试依赖 / test_depends |
| 1427 | test_l5_resource_protection.py | → | D_SECURITY_LLM LLM防御: protocol.py | 测试依赖 / test_depends |
| 1428 | test_l6_observability.py | → | D_SECURITY_LLM LLM防御: L6 Observability Layer — security event loggin... | 测试依赖 / test_depends |
| 1429 | test_l6_observability.py | → | D_SECURITY_LLM LLM防御: protocol.py | 测试依赖 / test_depends |
| 1430 | test_l7_red_team.py | → | D_SECURITY_LLM LLM防御: red_team_scanner.py | 测试依赖 / test_depends |
| 1431 | test_l7_validation.py | → | D_SECURITY_LLM LLM防御: l7_validation.py | 测试依赖 / test_depends |
| 1432 | test_l8_multi_agent.py | → | D_SECURITY_LLM LLM防御: l8_multi_agent.py | 测试依赖 / test_depends |
| 1433 | test_l8_multi_agent.py | → | D_SECURITY_LLM LLM防御: protocol.py | 测试依赖 / test_depends |
| 1434 | test_process_sandbox_llm_security.py | → | D_SECURITY_LLM LLM防御: L2a ProcessSandbox — subprocess 路径白名单沙箱... | 测试依赖 / test_depends |
| 1435 | test_runtime_interceptor.py — 运行时 LLM 裸调.... | → | D_SECURITY_LLM LLM防御: gateway.py | 测试依赖 / test_depends |
| 1436 | test_runtime_interceptor.py — 运行时 LLM 裸调.... | → | D_SECURITY_LLM LLM防御: protocol.py | 测试依赖 / test_depends |
| 1437 | test_runtime_interceptor.py — 运行时 LLM 裸调.... | → | D_SECURITY_LLM LLM防御: runtime_interceptor.py — 运行时 LLM 裸调拦截器... | 测试依赖 / test_depends |
| 1438 | test_secrets.py | → | D_SECURITY_LLM LLM防御: secrets.py | 测试依赖 / test_depends |
| 1439 | test_ai_capability_guard.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1440 | test_audit_red_blue_e2e.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1441 | test_auto_split.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1442 | test_architecture_context_loader.py | → | D_SHARED 共享服务: architecture_context_loader — 加载 ``generate_... | 测试依赖 / test_depends |
| 1443 | test_cache_invalidation.py | → | D_SHARED 共享服务: cache_invalidation.py — 缓存一致性 (DD113, TAS... | 测试依赖 / test_depends |
| 1444 | test_doc_compressor_root.py | → | D_SHARED 共享服务: DocCompressor — 文档压缩服务（CL-018 RI 扩展模... | 测试依赖 / test_depends |
| 1445 | test_ide_watcher.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1446 | test_mgmt_context_budget_tracker.py | → | D_SHARED 共享服务: Zero-dependency Observer pattern (subscribe/emi... | 测试依赖 / test_depends |
| 1447 | test_support_architecture_context_loader.py | → | D_SHARED 共享服务: architecture_context_loader — 加载 ``generate_... | 测试依赖 / test_depends |
| 1448 | test_support_doc_compressor.py | → | D_SHARED 共享服务: DocCompressor — 文档压缩服务（CL-018 RI 扩展模... | 测试依赖 / test_depends |
| 1449 | test_verify_paths.py | → | D_SHARED 共享服务: verify_paths.py — 代码路径索引验证 (TASK-012) ... | 测试依赖 / test_depends |
| 1450 | DM-201503: F4 事件驱动预算执行——超限/IPI/螺旋... | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 测试依赖 / test_depends |
| 1451 | test_budget_handler.py | → | D_SHARED 共享服务: budget_alert.py | 测试依赖 / test_depends |
| 1452 | test_ce_cache_invalidation.py | → | D_SHARED 共享服务: cache_invalidation.py — 缓存一致性 (DD113, TAS... | 测试依赖 / test_depends |
| 1453 | test_config_safety_guard.py | → | D_SHARED 共享服务: config_safety_guard.py — 配置自毁防护 (B16, DD... | 测试依赖 / test_depends |
| 1454 | F11 ContextPipeline 三层自动化机制测试 (test_co... | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 测试依赖 / test_depends |
| 1455 | test_abac_guard_root.py | → | D_SHARED 共享服务: agent_identity.py | 测试依赖 / test_depends |
| 1456 | test_alerts_bridge.py | → | D_SHARED 共享服务: budget_alert.py | 测试依赖 / test_depends |
| 1457 | test_rbac_guard_root.py | → | D_SHARED 共享服务: agent_identity.py | 测试依赖 / test_depends |
| 1458 | DM-100021: 事件驱动自动启动检查+自动运行检查 (t... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1459 | DM-100019: 双库集成测试+四方对齐验证 (test_db_i... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1460 | DM-100020: 红蓝对抗测试：数据库安全与韧性 (test... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1461 | test_dependency_tracker.py | → | D_SHARED 共享服务: dependency_tracker.py — 依赖追踪 (DD116, TASK-... | 测试依赖 / test_depends |
| 1462 | test_e_gov_budget_handler.py | → | D_SHARED 共享服务: budget_alert.py | 测试依赖 / test_depends |
| 1463 | test_e_gov_contracts.py | → | D_SHARED 共享服务: budget_alert.py | 测试依赖 / test_depends |
| 1464 | test_escalation_contracts.py | → | D_SHARED 共享服务: budget_alert.py | 测试依赖 / test_depends |
| 1465 | test_escalation_gov_budget_handler.py | → | D_SHARED 共享服务: budget_alert.py | 测试依赖 / test_depends |
| 1466 | test_escalation_gov_contracts.py | → | D_SHARED 共享服务: budget_alert.py | 测试依赖 / test_depends |
| 1467 | test_escalation_gov_rbac_bridge.py | → | D_SHARED 共享服务: permission.py | 测试依赖 / test_depends |
| 1468 | F18 治理脚本系统自动化测试. (test_f18_automatio... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1469 | F18 红蓝极限对抗测试. (test_f18_redblue.py) | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1470 | F1 事件触发启动测试 (test_f1_event_trigger.py) | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 测试依赖 / test_depends |
| 1471 | F21 自动关闭测试 — DM-201250 (test_f21_auto_sh... | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 测试依赖 / test_depends |
| 1472 | F21 事件启动测试 — DM-201250 (test_f21_event_d... | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 测试依赖 / test_depends |
| 1473 | F5 端到端集成测试 — boot→run→shutdown→resta... | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 测试依赖 / test_depends |
| 1474 | test_f5_event_startup.py | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 测试依赖 / test_depends |
| 1475 | test_circuit_breaker_types.py | → | D_SHARED 共享服务: severity_types.py | 测试依赖 / test_depends |
| 1476 | test_resilience_circuit_breaker.py | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 测试依赖 / test_depends |
| 1477 | test_resilience_circuit_breaker.py | → | D_SHARED 共享服务: circuit_breaker.py —— 轻量熔断器状态机（Phase... | 测试依赖 / test_depends |
| 1478 | DM-100026: 极端红蓝测试：depgraph生成器vs设计态... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1479 | test_schema_schema_registry.py | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 测试依赖 / test_depends |
| 1480 | test_session_worktree.py — worktree 物理隔离端... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1481 | test_boot_hooks_unlock.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1482 | DM-100016: governance.db端到端功能测试 (test_go... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1483 | audit_rename_completeness.py 回归测试（红蓝对抗... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1484 | F18 治理脚本系统红蓝对抗极端测试. (test_f18_gov... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1485 | test_infra_cache.py | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 测试依赖 / test_depends |
| 1486 | test_infra_cache.py | → | D_SHARED 共享服务: cache.py —— 统一缓存抽象（Phase 8 新增 | 盲点... | 测试依赖 / test_depends |
| 1487 | test_infra_idempotency.py | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 测试依赖 / test_depends |
| 1488 | test_infra_idempotency.py | → | D_SHARED 共享服务: idempotency.py —— 幂等性基础设施（Phase 8 新.... | 测试依赖 / test_depends |
| 1489 | test_infra_limiter.py | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 测试依赖 / test_depends |
| 1490 | test_infra_limiter.py | → | D_SHARED 共享服务: limiter.py —— Re-export wrapper -> canonical:... | 测试依赖 / test_depends |
| 1491 | test_infra_lock.py | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 测试依赖 / test_depends |
| 1492 | test_infra_lock.py | → | D_SHARED 共享服务: lock.py —— 分布式锁抽象（Phase 10 新增 | 盲点... | 测试依赖 / test_depends |
| 1493 | test_infra_observer.py | → | D_SHARED 共享服务: Zero-dependency Observer pattern (subscribe/emi... | 测试依赖 / test_depends |
| 1494 | test_infra_outbox.py | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 测试依赖 / test_depends |
| 1495 | test_infra_outbox.py | → | D_SHARED 共享服务: outbox.py —— 事务性 Outbox 模式（Phase 10 新.... | 测试依赖 / test_depends |
| 1496 | DM-202910: MCP boot_hooks 集成测试——验证10进.... | → | D_SHARED 共享服务: ProcessLifecycleGateway — 进程生命周期统一入口... | 测试依赖 / test_depends |
| 1497 | DM-202910: MCP boot_hooks 集成测试——验证10进.... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1498 | DM-202914: MCP boot→FLE→MCP→shutdown全链路E2... | → | D_SHARED 共享服务: ProcessLifecycleGateway — 进程生命周期统一入口... | 测试依赖 / test_depends |
| 1499 | DM-202914: MCP boot→FLE→MCP→shutdown全链路E2... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1500 | DM-202913: MCP _mcp_health_check死亡进程检测+re... | → | D_SHARED 共享服务: ProcessLifecycleGateway — 进程生命周期统一入口... | 测试依赖 / test_depends |
| 1501 | DM-202913: MCP _mcp_health_check死亡进程检测+re... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1502 | DM-202912: MCP idle_timeout 10分钟自动回收验证... | → | D_SHARED 共享服务: ProcessLifecycleGateway — 进程生命周期统一入口... | 测试依赖 / test_depends |
| 1503 | DM-202912: MCP idle_timeout 10分钟自动回收验证... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP s... | 测试依赖 / test_depends |
| 1504 | DM-202912: MCP idle_timeout 10分钟自动回收验证... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1505 | DM-202911: MCP SIGINT/SIGTERM 信号优雅关闭进程.... | → | D_SHARED 共享服务: ProcessLifecycleGateway — 进程生命周期统一入口... | 测试依赖 / test_depends |
| 1506 | DM-202911: MCP SIGINT/SIGTERM 信号优雅关闭进程.... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1507 | test_depgraph_schema.py — depgraph_schema.py D... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1508 | test_io_content_fingerprint.py | → | D_SHARED 共享服务: SHA-256 content fingerprint computation and ver... | 测试依赖 / test_depends |
| 1509 | test_io_file_utils.py | → | D_SHARED 共享服务: file_utils.py —— 安全文件操作工具（Phase 3 新... | 测试依赖 / test_depends |
| 1510 | test_io_frontmatter_utils.py | → | D_SHARED 共享服务: frontmatter_utils.py — Markdown/YAML frontmatt... | 测试依赖 / test_depends |
| 1511 | test_io_paths.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1512 | test_io_serialization.py | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 测试依赖 / test_depends |
| 1513 | test_io_serialization.py | → | D_SHARED 共享服务: serialization.py —— 统一序列化/反序列化基础设... | 测试依赖 / test_depends |
| 1514 | MCP集群launcher.py自动化测试——DAG拓扑/路径/dr... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1515 | test_mcp_task_claim.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1516 | test_verify_schema_health.py — verify_schema_h... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1517 | test_code_integrity.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1518 | test_fail_closed.py | → | D_SHARED 共享服务: security_decision.py | 测试依赖 / test_depends |
| 1519 | test_gateway_e2e.py | → | D_SHARED 共享服务: security_decision.py | 测试依赖 / test_depends |
| 1520 | test_interrupt_guard.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1521 | test_l0_supply_chain.py | → | D_SHARED 共享服务: security_decision.py | 测试依赖 / test_depends |
| 1522 | test_l1_input_defense.py | → | D_SHARED 共享服务: security_decision.py | 测试依赖 / test_depends |
| 1523 | test_l2_prompt_protection.py | → | D_SHARED 共享服务: security_decision.py | 测试依赖 / test_depends |
| 1524 | test_l3_output_security.py | → | D_SHARED 共享服务: security_decision.py | 测试依赖 / test_depends |
| 1525 | test_l4_agent_security.py | → | D_SHARED 共享服务: security_decision.py | 测试依赖 / test_depends |
| 1526 | test_l5_resource_protection.py | → | D_SHARED 共享服务: security_decision.py | 测试依赖 / test_depends |
| 1527 | test_l6_observability.py | → | D_SHARED 共享服务: security_decision.py | 测试依赖 / test_depends |
| 1528 | test_l7_validation.py | → | D_SHARED 共享服务: security_decision.py | 测试依赖 / test_depends |
| 1529 | test_process_sandbox_llm_security.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1530 | test_runtime_interceptor.py — 运行时 LLM 裸调.... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1531 | test_security_capability.py | → | D_SHARED 共享服务: CBAC 能力检查器 (Capability-Based Access Contro... | 测试依赖 / test_depends |
| 1532 | test_security_secrets.py | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 测试依赖 / test_depends |
| 1533 | test_security_secrets.py | → | D_SHARED 共享服务: secrets.py —— Secrets 管理抽象（Phase 7 新增 ... | 测试依赖 / test_depends |
| 1534 | test_security_ssot_guard.py | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 测试依赖 / test_depends |
| 1535 | test_security_ssot_guard.py | → | D_SHARED 共享服务: ssot_guard.py | 测试依赖 / test_depends |
| 1536 | test_observability_logging.py | → | D_SHARED 共享服务: logging.py —— ZephyrAlpha 结构化日志系统（Str... | 测试依赖 / test_depends |
| 1537 | test_observability_tracing.py | → | D_SHARED 共享服务: logging.py —— ZephyrAlpha 结构化日志系统（Str... | 测试依赖 / test_depends |
| 1538 | test_deferred_queue.py | → | D_SHARED 共享服务: Zero-dependency Observer pattern (subscribe/emi... | 测试依赖 / test_depends |
| 1539 | DM-100027: 极端红蓝测试：路径树生成器vs设计态保... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1540 | test_risk_ssot.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1541 | test_behavioral_admission.py | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 测试依赖 / test_depends |
| 1542 | test_lifecycle_manager.py | → | D_SHARED 共享服务: runtime_types.py | 测试依赖 / test_depends |
| 1543 | tests.test_trading_session_lifecycle — F1 Sess... | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 测试依赖 / test_depends |
| 1544 | Test gate g_trae_003 for rule TRAE-003 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1545 | Test gate g_trae_004 for rule TRAE-004 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1546 | Test gate g_trae_006 for rule TRAE-006 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1547 | Test gate g_trae_007 for rule TRAE-007 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1548 | Test gate g_trae_008 for rule TRAE-008 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1549 | Test gate g_trae_009 for rule TRAE-009 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1550 | Test gate g_trae_010 for rule TRAE-010 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1551 | Test gate g_trae_011 for rule TRAE-011 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1552 | Test gate g_trae_012 for rule TRAE-012 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1553 | Test gate g_trae_016 for rule TRAE-016 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1554 | Test gate g_trae_017 for rule TRAE-017 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1555 | Test gate g_trae_018 for rule TRAE-018 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1556 | Test gate g_trae_020 for rule TRAE-020 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1557 | Test gate g_trae_021 for rule TRAE-021 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1558 | Test gate g_trae_022 for rule TRAE-022 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1559 | Test gate g_trae_023 for rule TRAE-023 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1560 | Test gate g_trae_024 for rule TRAE-024 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1561 | Test gate g_trae_025 for rule TRAE-025 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1562 | Test gate g_trae_026 for rule TRAE-026 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1563 | Test gate g_trae_027 for rule TRAE-027 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1564 | Test gate g_trae_028 for rule TRAE-028 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1565 | Test gate g_trae_029 for rule TRAE-029 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1566 | Test gate g_trae_030 for rule TRAE-030 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1567 | Test gate g_trae_031 for rule TRAE-031 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1568 | Test gate g_trae_032 for rule TRAE-032 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1569 | Test gate g_trae_033 for rule TRAE-033 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1570 | Test gate g_trae_034 for rule TRAE-034 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1571 | Test gate g_trae_035 for rule TRAE-035 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1572 | Test gate g_trae_036 for rule TRAE-036 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1573 | Test gate g_trae_037 for rule TRAE-037 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1574 | Test gate g_trae_038 for rule TRAE-038 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1575 | Test gate g_trae_039 for rule TRAE-039 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1576 | Test gate g_trae_040 for rule TRAE-040 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1577 | Test gate g_trae_041 for rule TRAE-041 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1578 | Test gate g_trae_042 for rule TRAE-042 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1579 | Test gate g_trae_043 for rule TRAE-043 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1580 | Test gate g_trae_044 for rule TRAE-044 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1581 | Test gate g_trae_045 for rule TRAE-045 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1582 | Test gate g_trae_046 for rule TRAE-046 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1583 | Test gate g_trae_047 for rule TRAE-047 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1584 | Test gate g_trae_048 for rule TRAE-048 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1585 | Test gate g_trae_049 for rule TRAE-049 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1586 | Test gate g_trae_050 for rule TRAE-050 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1587 | Test gate g_trae_051 for rule TRAE-051 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1588 | Test gate g_trae_052 for rule TRAE-052 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1589 | Test gate g_trae_053 for rule TRAE-053 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1590 | Test gate g_trae_054 for rule TRAE-054 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1591 | Test gate g_trae_055 for rule TRAE-055 — calls... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 1592 | test_foundation_deprecation.py | → | D_SHARED 共享服务: deprecation.py —— ZephyrAlpha API 废弃策略 (d... | 测试依赖 / test_depends |
| 1593 | test_foundation_env.py | → | D_SHARED 共享服务: shared.foundation — auto-generated package ini... | 测试依赖 / test_depends |
| 1594 | test_foundation_errors.py | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 测试依赖 / test_depends |
| 1595 | test_foundation_flags.py | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 测试依赖 / test_depends |
| 1596 | test_foundation_flags.py | → | D_SHARED 共享服务: flags.py | 测试依赖 / test_depends |
| 1597 | test_resilience_fallback.py | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 测试依赖 / test_depends |
| 1598 | test_resilience_fallback.py | → | D_SHARED 共享服务: fallback.py —— 降级策略模式（Phase 2 新增 | .... | 测试依赖 / test_depends |
| 1599 | test_resilience_retry.py | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 测试依赖 / test_depends |
| 1600 | test_resilience_retry.py | → | D_SHARED 共享服务: retry.py —— 统一重试策略（Phase 2 新增 | 零依... | 测试依赖 / test_depends |
| 1601 | test_utils_context.py | → | D_SHARED 共享服务: context.py —— 结构化上下文传播（Phase 8 新增 ... | 测试依赖 / test_depends |
| 1602 | test_utils_diff_utils.py | → | D_SHARED 共享服务: diff_utils.py —— 统一 Diff/Patch 工具（Phase ... | 测试依赖 / test_depends |
| 1603 | test_utils_migration.py | → | D_SHARED 共享服务: migration.py —— Re-export wrapper -> canonica... | 测试依赖 / test_depends |
| 1604 | test_utils_pagination.py | → | D_SHARED 共享服务: pagination.py —— 通用分页工具（Phase 9 新增 |... | 测试依赖 / test_depends |
| 1605 | test_utils_testing.py | → | D_SHARED 共享服务: testing.py —— ZephyrAlpha 共享测试夹具/工厂 (... | 测试依赖 / test_depends |
| 1606 | test_utils_time_utils.py | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 测试依赖 / test_depends |
| 1607 | test_version.py | → | D_SHARED 共享服务: __version__.py —— ZephyrAlpha Shared 模块版本... | 测试依赖 / test_depends |
| 1608 | test_cross_layer.py | → | D_SIMULATION 仿真: 实验 — Experimentation Pipeline Layer (pipelin... | 测试依赖 / test_depends |
| 1609 | test_action_composition_health_monitor.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1610 | test_action_dispatcher.py | → | D_TRADING 交易运营: ActionDispatcher --- 大脑的"手" v2.0 (Phase 2) ... | 测试依赖 / test_depends |
| 1611 | test_action_efficacy_decay_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1612 | test_action_explainability.py | → | D_TRADING 交易运营: Action Explainability — v0.3.0 R15 (action_exp... | 测试依赖 / test_depends |
| 1613 | test_action_interaction_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1614 | test_action_reversibility.py | → | D_TRADING 交易运营: Action Reversibility — v0.15.0 R208 (action_re... | 测试依赖 / test_depends |
| 1615 | test_action_selector.py | → | D_TRADING 交易运营: action_selector.py | 测试依赖 / test_depends |
| 1616 | test_action_selector.py | → | D_TRADING 交易运营: protocols.py | 测试依赖 / test_depends |
| 1617 | test_action_side_effect_cumulative_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1618 | test_agent_health_monitor_root.py | → | D_TRADING 交易运营: AgentHealthMonitor · Agent 健康监控（三态 + 5 ... | 测试依赖 / test_depends |
| 1619 | test_agent_health_monitor_root.py | → | D_TRADING 交易运营: AgentOrchestrator · 多角色 Agent 路由、工具链.... | 测试依赖 / test_depends |
| 1620 | test_agent_lifecycle.py | → | D_TRADING 交易运营: Agent Lifecycle Manager — v0.12.0 R159c (agent... | 测试依赖 / test_depends |
| 1621 | test_agent_orchestrator_root.py | → | D_TRADING 交易运营: AgentOrchestrator · 多角色 Agent 路由、工具链.... | 测试依赖 / test_depends |
| 1622 | test_agent_quality.py | → | D_TRADING 交易运营: AI Agent 质量反馈闭环（CT-AGENT-QUALITY）——ta... | 测试依赖 / test_depends |
| 1623 | test_agent_skill_guard.py | → | D_TRADING 交易运营: Agent Skill Guard — v0.14.0 R201 (agent_skill_... | 测试依赖 / test_depends |
| 1624 | test_agent_trajectory_anomaly_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1625 | RBAC 自动启动/关闭生命周期集成测试. (test_rbac_... | → | D_TRADING 交易运营: AutoRuntimeCore — 三层运行时运营中心（系统大脑... | 测试依赖 / test_depends |
| 1626 | RBAC 自动启动/关闭生命周期集成测试. (test_rbac_... | → | D_TRADING 交易运营: boot_hooks.py | 测试依赖 / test_depends |
| 1627 | test_ai_audit_logger.py | → | D_TRADING 交易运营: AiAuditLogger — AI 行为审计日志 (ai_audit_logg... | 测试依赖 / test_depends |
| 1628 | test_ai_comment_veracity.py | → | D_TRADING 交易运营: AI Comment Veracity — v0.37.0 R459 (ai_comment... | 测试依赖 / test_depends |
| 1629 | test_ab_test.py | → | D_TRADING 交易运营: A/B Test Verifier — v0.9.0 R117 (ab_test.py) | 测试依赖 / test_depends |
| 1630 | test_amplification_guard.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1631 | test_api_dependency_metrics.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1632 | test_audit_spec_auditor.py | → | D_TRADING 交易运营: protocols.py | 测试依赖 / test_depends |
| 1633 | test_build_reproducibility_verifier.py | → | D_TRADING 交易运营: Build Reproducibility Verifier — v0.38.0 R484 ... | 测试依赖 / test_depends |
| 1634 | test_build_reproducibility_verifier_v2.py | → | D_TRADING 交易运营: Build Reproducibility Verifier — v0.38.0 R484 ... | 测试依赖 / test_depends |
| 1635 | test_burn_rate_alerter.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1636 | test_burnout_alarm.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1637 | test_causal_inference_engine.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1638 | test_cognitive_load_budget.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1639 | test_crypto_bootstrap.py | → | D_TRADING 交易运营: Cryptographic Bootstrap — v0.15.0 R204 (crypto... | 测试依赖 / test_depends |
| 1640 | test_deterministic_replay.py | → | D_TRADING 交易运营: Deterministic Replay — v0.15.0 R206 (determini... | 测试依赖 / test_depends |
| 1641 | test_diagnosis_kpi.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1642 | test_emergent_behavior_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1643 | test_global_health_map.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1644 | test_human_anomaly_flood_detector.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1645 | test_interactive_diagnosis.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1646 | test_intermittent_failure_pattern.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1647 | test_latency_slo.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1648 | test_mtti_tracker.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1649 | test_point_in_time_reconstructor.py | → | D_TRADING 交易运营: Point-in-Time Reconstructor — v0.37.0 R465 (po... | 测试依赖 / test_depends |
| 1650 | test_pre_flight_simulator.py | → | D_TRADING 交易运营: Pre-Flight Simulator — v0.12.0 R169b (pre_flig... | 测试依赖 / test_depends |
| 1651 | test_preventive_repair.py | → | D_TRADING 交易运营: Preventive Repair — v0.6.0 R69 (preventive_rep... | 测试依赖 / test_depends |
| 1652 | test_regime_gain_scheduling.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1653 | test_serialization_format_tracker.py | → | D_TRADING 交易运营: Serialization Format Tracker — v0.39.0 R488 (s... | 测试依赖 / test_depends |
| 1654 | test_sim2real_calibration.py | → | D_TRADING 交易运营: Sim2Real Calibration — v0.6.0 R56 (sim2real_ca... | 测试依赖 / test_depends |
| 1655 | test_socratic_questions.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1656 | test_statistical_hygiene_auditor.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1657 | test_sub_agent_collusion.py | → | D_TRADING 交易运营: Sub-Agent Collusion Detector — v0.15.0 R213 (s... | 测试依赖 / test_depends |
| 1658 | test_toctou_revalidation.py | → | D_TRADING 交易运营: TOCTOU Revalidation — v0.37.0 R458 (toctou_rev... | 测试依赖 / test_depends |
| 1659 | test_toil_quantification.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1660 | test_tone_adapter.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1661 | test_tone_adapter_v2.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1662 | test_traffic_replay_validator.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1663 | test_value_added_baseline.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1664 | test_verification_engine.py | → | D_TRADING 交易运营: verification_engine.py | 测试依赖 / test_depends |
| 1665 | test_zombie_fle_detector.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1666 | test_auto_diagnosis.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1667 | test_auto_evolution_root.py | → | D_TRADING 交易运营: auto_evolution.py | 测试依赖 / test_depends |
| 1668 | test_auto_evolution_root.py | → | D_TRADING 交易运营: evolution_engine.py | 测试依赖 / test_depends |
| 1669 | test_auto_integrator.py | → | D_TRADING 交易运营: AutoIntegrator — 自动接入器 (auto_integrator.py) | 测试依赖 / test_depends |
| 1670 | test_auto_integrator.py | → | D_TRADING 交易运营: CapabilityCard — 能力卡片数据模型 (capability_... | 测试依赖 / test_depends |
| 1671 | test_auto_integrator.py | → | D_TRADING 交易运营: CapabilityRegistry — 能力注册中心 (capability_... | 测试依赖 / test_depends |
| 1672 | test_auto_integrator.py | → | D_TRADING 交易运营: ModuleOnboardingScanner — 模块接入扫描器 (modu... | 测试依赖 / test_depends |
| 1673 | test_auto_reward.py | → | D_TRADING 交易运营: Auto Reward — v0.7.0 R76 (auto_reward.py) | 测试依赖 / test_depends |
| 1674 | test_auto_rollback.py | → | D_TRADING 交易运营: Auto Rollback — v0.8.0 R93 (auto_rollback.py) | 测试依赖 / test_depends |
| 1675 | test_auto_runtime_core.py | → | D_TRADING 交易运营: AutoRuntimeCore — 三层运行时运营中心（系统大脑... | 测试依赖 / test_depends |
| 1676 | test_auto_runtime_core.py | → | D_TRADING 交易运营: runtime_config.py | 测试依赖 / test_depends |
| 1677 | F1 AutoRuntimeCore 非mock端到端集成测试 (test_a... | → | D_TRADING 交易运营: AutoRuntimeCore — 三层运行时运营中心（系统大脑... | 测试依赖 / test_depends |
| 1678 | F1 AutoRuntimeCore 非mock端到端集成测试 (test_a... | → | D_TRADING 交易运营: CapabilityRegistry — 能力注册中心 (capability_... | 测试依赖 / test_depends |
| 1679 | F1 AutoRuntimeCore 非mock端到端集成测试 (test_a... | → | D_TRADING 交易运营: DreamCycle — 知识固化引擎 (dream_cycle.py) | 测试依赖 / test_depends |
| 1680 | F1 AutoRuntimeCore 非mock端到端集成测试 (test_a... | → | D_TRADING 交易运营: HealthMonitor — 健康监控 + 自愈 (health_monito... | 测试依赖 / test_depends |
| 1681 | F1 AutoRuntimeCore 非mock端到端集成测试 (test_a... | → | D_TRADING 交易运营: runtime_config.py | 测试依赖 / test_depends |
| 1682 | F1 AutoRuntimeCore 非mock端到端集成测试 (test_a... | → | D_TRADING 交易运营: WorkDAG + WorkItem — 工作编排数据模型 (work_da... | 测试依赖 / test_depends |
| 1683 | F1 AutoRuntimeCore 非mock端到端集成测试 (test_a... | → | D_TRADING 交易运营: work_orchestrator.py | 测试依赖 / test_depends |
| 1684 | AutoRuntimeCore → FeedbackLoopScheduler 自动启... | → | D_TRADING 交易运营: AutoRuntimeCore — 三层运行时运营中心（系统大脑... | 测试依赖 / test_depends |
| 1685 | AutoRuntimeCore → FeedbackLoopScheduler 自动启... | → | D_TRADING 交易运营: FLE 全链路调度器 —— collect->detect->diagnose... | 测试依赖 / test_depends |
| 1686 | AutoRuntimeCore → FeedbackLoopScheduler 自动启... | → | D_TRADING 交易运营: runtime_config.py | 测试依赖 / test_depends |
| 1687 | test_auto_task_generator.py | → | D_TRADING 交易运营: AutoTaskGenerator — 自动任务生成器 (auto_task_... | 测试依赖 / test_depends |
| 1688 | test_autonomy_credit.py | → | D_TRADING 交易运营: Autonomy Credit System — v0.7.0 R87 (autonomy_... | 测试依赖 / test_depends |
| 1689 | test_autonomy_guard.py | → | D_TRADING 交易运营: Owner 缺位分级自治（CT-AUTONOMY）——Owner离线-... | 测试依赖 / test_depends |
| 1690 | test_autonomy_maturity.py | → | D_TRADING 交易运营: Autonomy Maturity Ladder — v0.7.0 R86 (autonom... | 测试依赖 / test_depends |
| 1691 | test_dispatch_table_root.py | → | D_TRADING 交易运营: AI Agent 冷启动分派表（Dispatch Table） (dispat... | 测试依赖 / test_depends |
| 1692 | test_blueprint_code_reconciler.py | → | D_TRADING 交易运营: Blueprint-Code Reconciler — v0.14.0 R195 (blue... | 测试依赖 / test_depends |
| 1693 | test_blueprint_scorer.py | → | D_TRADING 交易运营: BlueprintScorer — 蓝图路由统一打分逻辑 (bluepr... | 测试依赖 / test_depends |
| 1694 | test_blueprint_validator.py | → | D_TRADING 交易运营: Blueprint Validator — v0.8.0 R108 (blueprint_v... | 测试依赖 / test_depends |
| 1695 | test_gen_inherited.py | → | D_TRADING 交易运营: _gen_inherited.py | 测试依赖 / test_depends |
| 1696 | test_bridges_spec_auditor.py | → | D_TRADING 交易运营: protocols.py | 测试依赖 / test_depends |
| 1697 | DM-201504: F4 BudgetEngine自动关闭——shutdown.... | → | D_TRADING 交易运营: boot_hooks.py | 测试依赖 / test_depends |
| 1698 | test_error_budget.py | → | D_TRADING 交易运营: Error Budget 状态机——monthly budget + burn_ra... | 测试依赖 / test_depends |
| 1699 | test_canary_manager.py | → | D_TRADING 交易运营: 金丝雀发布管理器（CT-CANARY）——权重分流+指标.... | 测试依赖 / test_depends |
| 1700 | test_canary_repair.py | → | D_TRADING 交易运营: Canary Repair — v0.8.0 R104b (canary_repair.py) | 测试依赖 / test_depends |
| 1701 | test_capability_card.py | → | D_TRADING 交易运营: CapabilityCard — 能力卡片数据模型 (capability_... | 测试依赖 / test_depends |
| 1702 | test_capability_registry.py | → | D_TRADING 交易运营: CapabilityCard — 能力卡片数据模型 (capability_... | 测试依赖 / test_depends |
| 1703 | test_capability_registry.py | → | D_TRADING 交易运营: CapabilityRegistry — 能力注册中心 (capability_... | 测试依赖 / test_depends |
| 1704 | test_capability_sync.py | → | D_TRADING 交易运营: CapabilityCard — 能力卡片数据模型 (capability_... | 测试依赖 / test_depends |
| 1705 | test_capability_sync.py | → | D_TRADING 交易运营: CapabilityRegistry — 能力注册中心 (capability_... | 测试依赖 / test_depends |
| 1706 | test_capability_sync.py | → | D_TRADING 交易运营: capability_sync.py | 测试依赖 / test_depends |
| 1707 | test_capacity_aware_repair.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1708 | test_capacity_budget_root.py | → | D_TRADING 交易运营: 全局容量预算控制器（Capacity Budget Controller... | 测试依赖 / test_depends |
| 1709 | test_capacity_forecast.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1710 | test_chaos_engine.py | → | D_TRADING 交易运营: Chaos 故障注入引擎（CT-CHAOS-001）——4注入点×... | 测试依赖 / test_depends |
| 1711 | test_chaos_engine_ops.py | → | D_TRADING 交易运营: Chaos 故障注入引擎（CT-CHAOS-001）——4注入点×... | 测试依赖 / test_depends |
| 1712 | test_chaos_engineering.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1713 | test_chaos_hooks.py | → | D_TRADING 交易运营: Chaos 故障注入引擎（CT-CHAOS-001）——4注入点×... | 测试依赖 / test_depends |
| 1714 | test_chaos_hooks.py | → | D_TRADING 交易运营: ChaosHook — integrates ChaosEngine with the or... | 测试依赖 / test_depends |
| 1715 | test_cold_start_conservative_mode.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1716 | test_config_complexity_budget.py | → | D_TRADING 交易运营: Config Complexity Budget — v0.16.0 R227 (confi... | 测试依赖 / test_depends |
| 1717 | test_config_drift.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1718 | test_config_governance.py | → | D_TRADING 交易运营: Config Governance — v0.3.0 R8 (config_governan... | 测试依赖 / test_depends |
| 1719 | test_config_hot_reload_guard.py | → | D_TRADING 交易运营: Config Hot-Reload Guard — v0.40.0 R498 (config... | 测试依赖 / test_depends |
| 1720 | test_context_truncation.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1721 | test_context_window_contamination_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1722 | test_context_window_pressure_manager.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1723 | test_api_version_contract.py | → | D_TRADING 交易运营: API Version Contract — v0.14.0 R188 (api_versi... | 测试依赖 / test_depends |
| 1724 | test_contract_registry_root.py | → | D_TRADING 交易运营: 集成契约注册表（Contract Registry） (contract_r... | 测试依赖 / test_depends |
| 1725 | test_contract_router_root.py | → | D_TRADING 交易运营: 集成契约注册表（Contract Registry） (contract_r... | 测试依赖 / test_depends |
| 1726 | test_contract_router_root.py | → | D_TRADING 交易运营: 契约路由（Contract Router） (contract_router.py) | 测试依赖 / test_depends |
| 1727 | test_cross_blueprint_contract_drift.py | → | D_TRADING 交易运营: Cross-Blueprint Contract Drift Monitor — v0.39... | 测试依赖 / test_depends |
| 1728 | test_cross_gen_validation.py | → | D_TRADING 交易运营: Cross-Gen Validation — v0.7.0 R78 (cross_gen_v... | 测试依赖 / test_depends |
| 1729 | test_cross_guard_conflict_detector.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1730 | test_cross_module_integration_root.py | → | D_TRADING 交易运营: Cross-Module Integration Verifier — v0.5.0 R39... | 测试依赖 / test_depends |
| 1731 | test_cross_session_consistency_validator.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1732 | test_cross_session_knowledge_integrity.py | → | D_TRADING 交易运营: Cross-Session Knowledge Integrity — v0.16.0 R2... | 测试依赖 / test_depends |
| 1733 | test_cross_signal_validator.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1734 | test_cross_system_correlator.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1735 | test_data_quality_gate.py | → | D_TRADING 交易运营: Data Quality Gate — v0.11.0 R143 (data_quality... | 测试依赖 / test_depends |
| 1736 | test_data_volume_growth_monitor.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1737 | Tests for zephyr.trading.feedback_loop.db_bridg... | → | D_TRADING 交易运营: FLE DB契约适配器 — 通过规范zephyr.governance.s... | 测试依赖 / test_depends |
| 1738 | test_db_integrity.py | → | D_TRADING 交易运营: DB Integrity Gate — v0.3.0 R17 (db_integrity.py) | 测试依赖 / test_depends |
| 1739 | test_decision_engine.py | → | D_TRADING 交易运营: Feedback Loop Decision Engine (decision_engine.py) | 测试依赖 / test_depends |
| 1740 | test_decision_engine.py | → | D_TRADING 交易运营: protocols.py | 测试依赖 / test_depends |
| 1741 | test_decision_provenance.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1742 | test_dependency_freshness_monitor.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1743 | test_dependency_lock.py | → | D_TRADING 交易运营: 外部依赖版本锁（CT-DEPS）——Python包版本锁定+h... | 测试依赖 / test_depends |
| 1744 | test_concept_drift.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1745 | test_schema_evolution_root.py | → | D_TRADING 交易运营: Schema Evolution — v0.9.0 R111 (schema_evoluti... | 测试依赖 / test_depends |
| 1746 | test_version_migrator.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1747 | test_incident_priority_triage_automator.py | → | D_TRADING 交易运营: Incident Priority Triage Automator — v0.37.0 R... | 测试依赖 / test_depends |
| 1748 | test_owner_absence_escalation.py | → | D_TRADING 交易运营: Owner Absence Escalation — v0.37.0 R462 (owner... | 测试依赖 / test_depends |
| 1749 | test_external_health.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1750 | test_external_validation_checkpoint.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1751 | test_external_verifier.py | → | D_TRADING 交易运营: External Verifier — v0.15.0 R203 (external_ver... | 测试依赖 / test_depends |
| 1752 | F21 自动运行测试 — DM-201250 (test_f21_auto_ru... | → | D_TRADING 交易运营: __init__.py | 测试依赖 / test_depends |
| 1753 | F21 自动运行测试 — DM-201250 (test_f21_auto_ru... | → | D_TRADING 交易运营: HealthMonitor — 健康监控 + 自愈 (health_monito... | 测试依赖 / test_depends |
| 1754 | F21 自动关闭测试 — DM-201250 (test_f21_auto_sh... | → | D_TRADING 交易运营: __init__.py | 测试依赖 / test_depends |
| 1755 | F21 自动关闭测试 — DM-201250 (test_f21_auto_sh... | → | D_TRADING 交易运营: Finalizer — 优雅清理器 (finalizer.py) | 测试依赖 / test_depends |
| 1756 | F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | D_TRADING 交易运营: __init__.py | 测试依赖 / test_depends |
| 1757 | F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | D_TRADING 交易运营: Finalizer — 优雅清理器 (finalizer.py) | 测试依赖 / test_depends |
| 1758 | test_flag_lifecycle.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1759 | test_openfeature.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1760 | test_fl_action_reversibility.py | → | D_TRADING 交易运营: Action Reversibility — v0.15.0 R208 (action_re... | 测试依赖 / test_depends |
| 1761 | test_fl_action_selector.py | → | D_TRADING 交易运营: action_selector.py | 测试依赖 / test_depends |
| 1762 | test_fl_action_selector.py | → | D_TRADING 交易运营: protocols.py | 测试依赖 / test_depends |
| 1763 | test_fl_agent_lifecycle.py | → | D_TRADING 交易运营: Agent Lifecycle Manager — v0.12.0 R159c (agent... | 测试依赖 / test_depends |
| 1764 | test_fl_anomaly_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1765 | test_fl_anomaly_detector.py | → | D_TRADING 交易运营: FeedbackCollector: collect task execution feedb... | 测试依赖 / test_depends |
| 1766 | test_fl_anomaly_detector.py | → | D_TRADING 交易运营: MetricsCollector: append-only metrics recording... | 测试依赖 / test_depends |
| 1767 | test_fl_anomaly_detector.py | → | D_TRADING 交易运营: protocols.py | 测试依赖 / test_depends |
| 1768 | test_fl_api_version_contract.py | → | D_TRADING 交易运营: API Version Contract — v0.14.0 R188 (api_versi... | 测试依赖 / test_depends |
| 1769 | test_fl_auto_evolution.py | → | D_TRADING 交易运营: auto_evolution.py | 测试依赖 / test_depends |
| 1770 | test_fl_auto_evolution.py | → | D_TRADING 交易运营: evolution_engine.py | 测试依赖 / test_depends |
| 1771 | test_fl_autonomy_credit.py | → | D_TRADING 交易运营: Autonomy Credit System — v0.7.0 R87 (autonomy_... | 测试依赖 / test_depends |
| 1772 | test_fl_autonomy_maturity.py | → | D_TRADING 交易运营: Autonomy Maturity Ladder — v0.7.0 R86 (autonom... | 测试依赖 / test_depends |
| 1773 | test_fl_backpressure_bridge.py | → | D_TRADING 交易运营: FLE -> Pipeline 背压桥接（CTR-BP-001~003） (bac... | 测试依赖 / test_depends |
| 1774 | test_fl_backpressure_bridge.py | → | D_TRADING 交易运营: evolution_engine.py | 测试依赖 / test_depends |
| 1775 | test_fl_blueprint_code_reconciler.py | → | D_TRADING 交易运营: Blueprint-Code Reconciler — v0.14.0 R195 (blue... | 测试依赖 / test_depends |
| 1776 | test_fl_blueprint_validator.py | → | D_TRADING 交易运营: Blueprint Validator — v0.8.0 R108 (blueprint_v... | 测试依赖 / test_depends |
| 1777 | test_fl_calendar_adapter.py | → | D_TRADING 交易运营: Calendar Adapter — v0.8.0 R102b (calendar_adap... | 测试依赖 / test_depends |
| 1778 | test_fl_checkpoint_manager.py | → | D_TRADING 交易运营: Checkpoint Manager — v0.3.0 R18 (checkpoint_ma... | 测试依赖 / test_depends |
| 1779 | test_fl_ci_cd_pre_scanner.py | → | D_TRADING 交易运营: CI/CD Pre-Scanner — v0.8.0 R107 (ci_cd_pre_sca... | 测试依赖 / test_depends |
| 1780 | test_fl_concurrent_change_deconfliction.py | → | D_TRADING 交易运营: Concurrent Change Deconfliction — v0.16.0 R230... | 测试依赖 / test_depends |
| 1781 | test_fl_config.py | → | D_TRADING 交易运营: config.py | 测试依赖 / test_depends |
| 1782 | test_fl_config_complexity_budget.py | → | D_TRADING 交易运营: Config Complexity Budget — v0.16.0 R227 (confi... | 测试依赖 / test_depends |
| 1783 | test_fl_config_governance.py | → | D_TRADING 交易运营: Config Governance — v0.3.0 R8 (config_governan... | 测试依赖 / test_depends |
| 1784 | test_fl_config_timeline.py | → | D_TRADING 交易运营: Config Timeline — v0.8.0 R99 (config_timeline.py) | 测试依赖 / test_depends |
| 1785 | test_fl_conflict_arbitration.py | → | D_TRADING 交易运营: Conflict Arbitration — v0.10.0 R130 (conflict_... | 测试依赖 / test_depends |
| 1786 | test_fl_cve_scanner.py | → | D_TRADING 交易运营: CVE Scanner — v0.8.0 R106 (cve_scanner.py) | 测试依赖 / test_depends |
| 1787 | test_fl_data_quality_gate.py | → | D_TRADING 交易运营: Data Quality Gate — v0.11.0 R143 (data_quality... | 测试依赖 / test_depends |
| 1788 | test_fl_data_quality_validator.py | → | D_TRADING 交易运营: Data Quality Validator — v0.9.0 R110 (data_qua... | 测试依赖 / test_depends |
| 1789 | test_fl_db_bridge.py | → | D_TRADING 交易运营: FLE DB契约适配器 — 通过规范zephyr.governance.s... | 测试依赖 / test_depends |
| 1790 | test_fl_db_integrity.py | → | D_TRADING 交易运营: DB Integrity Gate — v0.3.0 R17 (db_integrity.py) | 测试依赖 / test_depends |
| 1791 | test_fl_decision_engine.py | → | D_TRADING 交易运营: Feedback Loop Decision Engine (decision_engine.py) | 测试依赖 / test_depends |
| 1792 | test_fl_decision_engine.py | → | D_TRADING 交易运营: protocols.py | 测试依赖 / test_depends |
| 1793 | test_fl_deployment_suppression.py | → | D_TRADING 交易运营: Deployment Suppression — v0.37.0 R464 (deploym... | 测试依赖 / test_depends |
| 1794 | test_fl_dynamic_llm_cost_router.py | → | D_TRADING 交易运营: Dynamic LLM Cost Router — v0.8.0 R109 (dynamic... | 测试依赖 / test_depends |
| 1795 | test_fl_emergency_takeover.py | → | D_TRADING 交易运营: Emergency Takeover — v0.7.0 R88 (emergency_tak... | 测试依赖 / test_depends |
| 1796 | test_fl_error_budget.py | → | D_TRADING 交易运营: Error Budget 状态机——monthly budget + burn_ra... | 测试依赖 / test_depends |
| 1797 | test_fl_eval_harness.py | → | D_TRADING 交易运营: eval_harness.py | 测试依赖 / test_depends |
| 1798 | test_fl_evolution_engine.py | → | D_TRADING 交易运营: evolution_engine.py | 测试依赖 / test_depends |
| 1799 | test_fl_exceptions.py | → | D_TRADING 交易运营: exceptions.py | 测试依赖 / test_depends |
| 1800 | test_fl_federated_security.py | → | D_TRADING 交易运营: Federated Security — v0.10.0 R131 (federated_s... | 测试依赖 / test_depends |
| 1801 | test_fl_financial_stratification.py | → | D_TRADING 交易运营: Financial Stratification — v0.5.0 R50 (financi... | 测试依赖 / test_depends |
| 1802 | test_fl_fitness_functions.py | → | D_TRADING 交易运营: fitness_functions.py | 测试依赖 / test_depends |
| 1803 | test_fl_flag_lifecycle_manager.py | → | D_TRADING 交易运营: Flag Lifecycle Manager — v0.3.0 R11 (flag_life... | 测试依赖 / test_depends |
| 1804 | test_fl_generator.py | → | D_TRADING 交易运营: generator.py | 测试依赖 / test_depends |
| 1805 | test_fl_global_action_scheduler.py | → | D_TRADING 交易运营: Global Action Scheduler — v0.16.0 R226 (global... | 测试依赖 / test_depends |
| 1806 | test_fl_incident_priority_triage_automator.py | → | D_TRADING 交易运营: Incident Priority Triage Automator — v0.37.0 R... | 测试依赖 / test_depends |
| 1807 | test_fl_intent_driven_ops.py | → | D_TRADING 交易运营: Intent-Driven Ops — v0.12.0 R159 (intent_drive... | 测试依赖 / test_depends |
| 1808 | test_fl_kb_provenance.py | → | D_TRADING 交易运营: KB Provenance — v0.10.0 R136 (kb_provenance.py) | 测试依赖 / test_depends |
| 1809 | test_fl_license_compliance.py | → | D_TRADING 交易运营: License Compliance — v0.14.0 R198 (license_com... | 测试依赖 / test_depends |
| 1810 | test_fl_llm_cost_router.py | → | D_TRADING 交易运营: LLM Cost Router — v0.3.0 R20 (llm_cost_router.py) | 测试依赖 / test_depends |
| 1811 | test_fl_merkle_audit_root.py | → | D_TRADING 交易运营: Merkle Audit Root — v0.8.0 R104 (merkle_audit_... | 测试依赖 / test_depends |
| 1812 | test_fl_meta_performance_gate.py | → | D_TRADING 交易运营: Meta Performance Gate — v0.11.0 R158 (meta_per... | 测试依赖 / test_depends |
| 1813 | test_fl_multi_agent_orchestrator.py | → | D_TRADING 交易运营: Multi-Agent Orchestrator — v0.12.0 R159b (mult... | 测试依赖 / test_depends |
| 1814 | test_fl_notification_personalizer.py | → | D_TRADING 交易运营: Notification Personalizer — v0.6.0 R67 (notifi... | 测试依赖 / test_depends |
| 1815 | test_fl_owner_absence_escalation.py | → | D_TRADING 交易运营: Owner Absence Escalation — v0.37.0 R462 (owner... | 测试依赖 / test_depends |
| 1816 | test_fl_parameterized_safety_gate.py | → | D_TRADING 交易运营: GateVerdict — GateVerdict (parameterized_safet... | 测试依赖 / test_depends |
| 1817 | test_fl_protocols.py | → | D_TRADING 交易运营: protocols.py | 测试依赖 / test_depends |
| 1818 | test_fl_safety_gate_l1_l27.py | → | D_TRADING 交易运营: Safety Gates L1-L27 — Unified Pipeline (MOD-FE... | 测试依赖 / test_depends |
| 1819 | test_fl_scheduler.py | → | D_TRADING 交易运营: FLE 全链路调度器 —— collect->detect->diagnose... | 测试依赖 / test_depends |
| 1820 | test_fl_scheduler_act.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1821 | test_fl_scheduler_act.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1822 | test_fl_scheduler_act.py | → | D_TRADING 交易运营: R522: SelfModificationRateLimiter (self_modific... | 测试依赖 / test_depends |
| 1823 | test_fl_scheduler_act.py | → | D_TRADING 交易运营: Graceful Degradation Planner — v0.40.0 R496 (g... | 测试依赖 / test_depends |
| 1824 | test_fl_scheduler_act.py | → | D_TRADING 交易运营: Self API Throttle Defense — v0.39.0 R491 (self... | 测试依赖 / test_depends |
| 1825 | test_fl_scheduler_act.py | → | D_TRADING 交易运营: scheduler_act.py | 测试依赖 / test_depends |
| 1826 | test_fl_scheduler_collect_detect.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1827 | test_fl_scheduler_collect_detect.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1828 | test_fl_scheduler_collect_detect.py | → | D_TRADING 交易运营: FeedbackCollector: collect task execution feedb... | 测试依赖 / test_depends |
| 1829 | test_fl_scheduler_collect_detect.py | → | D_TRADING 交易运营: MetricsCollector: append-only metrics recording... | 测试依赖 / test_depends |
| 1830 | test_fl_scheduler_collect_detect.py | → | D_TRADING 交易运营: scheduler_collect_detect.py | 测试依赖 / test_depends |
| 1831 | test_fl_scheduler_health.py | → | D_TRADING 交易运营: scheduler_health.py | 测试依赖 / test_depends |
| 1832 | test_fl_scheduler_safety.py | → | D_TRADING 交易运营: scheduler_safety.py | 测试依赖 / test_depends |
| 1833 | test_fl_scope_creep_monitor.py | → | D_TRADING 交易运营: Scope Creep Monitor — v0.15.0 R220 (scope_cree... | 测试依赖 / test_depends |
| 1834 | test_fl_slo_manager.py | → | D_TRADING 交易运营: slo_manager.py | 测试依赖 / test_depends |
| 1835 | test_fl_template.py | → | D_TRADING 交易运营: template.py | 测试依赖 / test_depends |
| 1836 | test_fl_validator.py | → | D_TRADING 交易运营: template.py | 测试依赖 / test_depends |
| 1837 | test_fl_validator.py | → | D_TRADING 交易运营: validator.py | 测试依赖 / test_depends |
| 1838 | test_actors_init.py | → | D_TRADING 交易运营: feedback-loop.actors — auto-generated package ... | 测试依赖 / test_depends |
| 1839 | test_adaptive_param_tuning.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1840 | test_alert_desensitization_curve.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1841 | test_anomaly_clustering.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1842 | test_architectural_sod.py | → | D_TRADING 交易运营: Architectural SoD — v0.15.0 R205 (architectura... | 测试依赖 / test_depends |
| 1843 | test_automated_rca_postmortem_generator.py | → | D_TRADING 交易运营: Automated RCA Postmortem Generator — v0.38.0 R... | 测试依赖 / test_depends |
| 1844 | test_autoscale_remediation.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1845 | test_backpressure_bridge_root.py | → | D_TRADING 交易运营: FLE -> Pipeline 背压桥接（CTR-BP-001~003） (bac... | 测试依赖 / test_depends |
| 1846 | test_backpressure_bridge_root.py | → | D_TRADING 交易运营: evolution_engine.py | 测试依赖 / test_depends |
| 1847 | test_blast_radius_budget.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1848 | test_boot_integrity_attestation.py | → | D_TRADING 交易运营: Boot Integrity Attestation — v0.38.0 R487 (boo... | 测试依赖 / test_depends |
| 1849 | test_cascading_rollback_analyzer.py | → | D_TRADING 交易运营: Cascading Rollback Analyzer — v0.38.0 R482 (ca... | 测试依赖 / test_depends |
| 1850 | test_cognitive_load.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1851 | test_collaborative_learning.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1852 | test_collectors.py | → | D_TRADING 交易运营: Calendar Adapter — v0.8.0 R102b (calendar_adap... | 测试依赖 / test_depends |
| 1853 | test_collectors.py | → | D_TRADING 交易运营: Config Timeline — v0.8.0 R99 (config_timeline.py) | 测试依赖 / test_depends |
| 1854 | test_collectors.py | → | D_TRADING 交易运营: Data Quality Validator — v0.9.0 R110 (data_qua... | 测试依赖 / test_depends |
| 1855 | test_collectors.py | → | D_TRADING 交易运营: Financial Stratification — v0.5.0 R50 (financi... | 测试依赖 / test_depends |
| 1856 | test_collectors.py | → | D_TRADING 交易运营: KB Provenance — v0.10.0 R136 (kb_provenance.py) | 测试依赖 / test_depends |
| 1857 | test_collectors.py | → | D_TRADING 交易运营: Knowledge Capture — v0.4.0 R30 (knowledge_capt... | 测试依赖 / test_depends |
| 1858 | test_collectors.py | → | D_TRADING 交易运营: Knowledge Freshness — v0.5.0 R47 (knowledge_fr... | 测试依赖 / test_depends |
| 1859 | test_collectors.py | → | D_TRADING 交易运营: Knowledge Injection — v0.8.0 R102 (knowledge_i... | 测试依赖 / test_depends |
| 1860 | test_collectors.py | → | D_TRADING 交易运营: Knowledge Packaging — v0.9.0 R123 (knowledge_p... | 测试依赖 / test_depends |
| 1861 | test_collectors.py | → | D_TRADING 交易运营: Known-Unknown Registry — v0.16.0 R229 (known_u... | 测试依赖 / test_depends |
| 1862 | test_collectors.py | → | D_TRADING 交易运营: LLM Cost Accounting — v0.4.0 R35 (llm_cost_acc... | 测试依赖 / test_depends |
| 1863 | test_collectors.py | → | D_TRADING 交易运营: Market Calendar — v0.5.0 R48 (market_calendar.py) | 测试依赖 / test_depends |
| 1864 | test_collectors.py | → | D_TRADING 交易运营: Market Event Integrator — v0.14.0 R197 (market... | 测试依赖 / test_depends |
| 1865 | test_collectors.py | → | D_TRADING 交易运营: Notification Feedback — v0.9.0 R118 (notificat... | 测试依赖 / test_depends |
| 1866 | test_collectors.py | → | D_TRADING 交易运营: Schema Evolution — v0.9.0 R111 (schema_evoluti... | 测试依赖 / test_depends |
| 1867 | test_collectors.py | → | D_TRADING 交易运营: Schema Migration — v0.14.0 R190 (schema_migrat... | 测试依赖 / test_depends |
| 1868 | test_collectors.py | → | D_TRADING 交易运营: Temporal Event Store — v0.3.0 R9 (temporal_eve... | 测试依赖 / test_depends |
| 1869 | test_collectors.py | → | D_TRADING 交易运营: Token FinOps — v0.12.0 R162 (token_finops.py) | 测试依赖 / test_depends |
| 1870 | test_collectors.py | → | D_TRADING 交易运营: FeedbackCollector: collect task execution feedb... | 测试依赖 / test_depends |
| 1871 | test_collectors.py | → | D_TRADING 交易运营: MetricsCollector: append-only metrics recording... | 测试依赖 / test_depends |
| 1872 | test_confidence_decomposer.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1873 | test_config_feedback_loop.py | → | D_TRADING 交易运营: config.py | 测试依赖 / test_depends |
| 1874 | test_conformal_prediction.py | → | D_TRADING 交易运营: Conformal Prediction — v0.7.0 R74 (conformal_p... | 测试依赖 / test_depends |
| 1875 | test_counterfactual.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1876 | test_deadman_switch.py | → | D_TRADING 交易运营: Deadman Switch — v0.15.0 R212 (deadman_switch.py) | 测试依赖 / test_depends |
| 1877 | test_diagnosers.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1878 | test_diagnosis_engine.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1879 | test_digital_twin_sandbox.py | → | D_TRADING 交易运营: Digital Twin Sandbox — v0.6.0 R55 (digital_twi... | 测试依赖 / test_depends |
| 1880 | test_diminishing_returns_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1881 | test_docs_init.py | → | D_TRADING 交易运营: feedback-loop.docs — auto-generated package in... | 测试依赖 / test_depends |
| 1882 | test_docs_init.py | → | D_TRADING 交易运营: cold_start_manual.py | 测试依赖 / test_depends |
| 1883 | test_dr_automation.py | → | D_TRADING 交易运营: DR Automation — v0.14.0 R187 (dr_automation.py) | 测试依赖 / test_depends |
| 1884 | test_dr_resilience_metrics.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1885 | test_dry_run_sandbox.py | → | D_TRADING 交易运营: Dry Run Sandbox — v0.3.0 R19 (dry_run_sandbox.py) | 测试依赖 / test_depends |
| 1886 | test_dynamic_threshold.py | → | D_TRADING 交易运营: Dynamic Threshold — v0.7.0 R71 (dynamic_thresh... | 测试依赖 / test_depends |
| 1887 | test_e2e_integration_health.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1888 | test_ebpf_monitor.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1889 | test_ensemble_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1890 | test_ensemble_drift.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1891 | test_eval_harness_root.py | → | D_TRADING 交易运营: eval_harness.py | 测试依赖 / test_depends |
| 1892 | test_evolution_engine_root.py | → | D_TRADING 交易运营: evolution_engine.py | 测试依赖 / test_depends |
| 1893 | test_evolution_init.py | → | D_TRADING 交易运营: Auto Reward — v0.7.0 R76 (auto_reward.py) | 测试依赖 / test_depends |
| 1894 | test_evolution_init.py | → | D_TRADING 交易运营: Conformal Prediction — v0.7.0 R74 (conformal_p... | 测试依赖 / test_depends |
| 1895 | test_evolution_init.py | → | D_TRADING 交易运营: Dynamic Threshold — v0.7.0 R71 (dynamic_thresh... | 测试依赖 / test_depends |
| 1896 | test_evolution_init.py | → | D_TRADING 交易运营: R522: SelfModificationRateLimiter (self_modific... | 测试依赖 / test_depends |
| 1897 | test_evolution_init.py | → | D_TRADING 交易运营: Self Reflection — v0.7.0 R75 (self_reflection.py) | 测试依赖 / test_depends |
| 1898 | test_ewc_kb_review.py | → | D_TRADING 交易运营: EWC KB Review — v0.6.0 R51 (ewc_kb_review.py) | 测试依赖 / test_depends |
| 1899 | test_exceptions_feedback_loop.py | → | D_TRADING 交易运营: exceptions.py | 测试依赖 / test_depends |
| 1900 | test_failure_replay.py | → | D_TRADING 交易运营: Failure Replay — v0.7.0 R77 (failure_replay.py) | 测试依赖 / test_depends |
| 1901 | test_federated_protocol.py | → | D_TRADING 交易运营: Federated Protocol — v0.10.0 R129 (federated_p... | 测试依赖 / test_depends |
| 1902 | test_feedback_collector_root.py | → | D_TRADING 交易运营: FeedbackCollector: collect task execution feedb... | 测试依赖 / test_depends |
| 1903 | Test suite: feedback-loop core (FeedbackCollect... | → | D_TRADING 交易运营: FeedbackCollector: collect task execution feedb... | 测试依赖 / test_depends |
| 1904 | Test suite: feedback-loop core (FeedbackCollect... | → | D_TRADING 交易运营: FLE 全链路调度器 —— collect->detect->diagnose... | 测试依赖 / test_depends |
| 1905 | test_feedback_delay_compensator.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1906 | test_feedback_loop.py | → | D_TRADING 交易运营: Feedback Loop Engine — MOD-FEEDBACK_LOOP. (__i... | 测试依赖 / test_depends |
| 1907 | test_flapping_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1908 | test_gamification.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1909 | test_global_action_scheduler.py | → | D_TRADING 交易运营: Global Action Scheduler — v0.16.0 R226 (global... | 测试依赖 / test_depends |
| 1910 | test_golden_test_external.py | → | D_TRADING 交易运营: Golden Test External — v0.15.0 R214 (golden_te... | 测试依赖 / test_depends |
| 1911 | test_gradual_poisoning_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1912 | test_graduated_activation_protocol.py | → | D_TRADING 交易运营: Graduated Activation Protocol — v0.38.0 R485 (... | 测试依赖 / test_depends |
| 1913 | test_heisenbug_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1914 | test_hypernetwork.py | → | D_TRADING 交易运营: HyperNetwork — v0.7.0 R72 (hypernetwork.py) | 测试依赖 / test_depends |
| 1915 | test_impact_predictor.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1916 | test_incident_knowledge_injector.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1917 | test_infinite_loop_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1918 | test_interrupt_coherence_validator.py | → | D_TRADING 交易运营: R531: InterruptCoherenceValidator (interrupt_co... | 测试依赖 / test_depends |
| 1919 | test_known_unknown_registry.py | → | D_TRADING 交易运营: Known-Unknown Registry — v0.16.0 R229 (known_u... | 测试依赖 / test_depends |
| 1920 | test_log_anomaly.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1921 | test_maintenance_coordinator.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1922 | test_market_calendar.py | → | D_TRADING 交易运营: Market Calendar — v0.5.0 R48 (market_calendar.py) | 测试依赖 / test_depends |
| 1923 | test_market_event_integrator.py | → | D_TRADING 交易运营: Market Event Integrator — v0.14.0 R197 (market... | 测试依赖 / test_depends |
| 1924 | test_meta_guard_latency_budget.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1925 | test_metric_cardinality_guard.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1926 | test_metrics_collector.py | → | D_TRADING 交易运营: MetricsCollector: append-only metrics recording... | 测试依赖 / test_depends |
| 1927 | test_no_llm_degradation.py | → | D_TRADING 交易运营: No-LLM Degradation Mode — v0.8.0 R94 (no_llm_d... | 测试依赖 / test_depends |
| 1928 | test_nonstationary_effectiveness.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1929 | test_notification_feedback.py | → | D_TRADING 交易运营: Notification Feedback — v0.9.0 R118 (notificat... | 测试依赖 / test_depends |
| 1930 | test_notification_personalizer.py | → | D_TRADING 交易运营: Notification Personalizer — v0.6.0 R67 (notifi... | 测试依赖 / test_depends |
| 1931 | test_numerical_stability_guard.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1932 | test_online_feature_importance.py | → | D_TRADING 交易运营: Online Feature Importance — v0.7.0 R73 (online... | 测试依赖 / test_depends |
| 1933 | test_operational_seasonality.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1934 | test_oscillation_damping.py | → | D_TRADING 交易运营: Oscillation Damping — v0.37.0 R450 (oscillatio... | 测试依赖 / test_depends |
| 1935 | test_otel_adapter.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1936 | test_placebo_action_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1937 | test_positive_feedback_defense.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1938 | test_protocols.py | → | D_TRADING 交易运营: protocols.py | 测试依赖 / test_depends |
| 1939 | test_recovery_time_stats.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1940 | test_recursive_diagnosis_trust_evaluator.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1941 | test_regulatory_audit.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1942 | test_resolution_tracker.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1943 | test_retirement_planner.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1944 | test_rumor_noise_filter.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1945 | test_runbook_executor.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1946 | test_scheduler_collect_detect.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1947 | test_scheduler_collect_detect.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1948 | test_scheduler_collect_detect.py | → | D_TRADING 交易运营: FeedbackCollector: collect task execution feedb... | 测试依赖 / test_depends |
| 1949 | test_scheduler_collect_detect.py | → | D_TRADING 交易运营: MetricsCollector: append-only metrics recording... | 测试依赖 / test_depends |
| 1950 | test_scheduler_collect_detect.py | → | D_TRADING 交易运营: scheduler_collect_detect.py | 测试依赖 / test_depends |
| 1951 | test_scheduler_health.py | → | D_TRADING 交易运营: scheduler_health.py | 测试依赖 / test_depends |
| 1952 | Integration tests: FeedbackLoopScheduler start/... | → | D_TRADING 交易运营: FLE 全链路调度器 —— collect->detect->diagnose... | 测试依赖 / test_depends |
| 1953 | test_secondary_alert_channel.py | → | D_TRADING 交易运营: Secondary Alert Channel — v0.37.0 R461 (second... | 测试依赖 / test_depends |
| 1954 | test_silent_corruption_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1955 | test_slo_capacity_metrics.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1956 | test_slo_manager_root.py | → | D_TRADING 交易运营: slo_manager.py | 测试依赖 / test_depends |
| 1957 | test_state_migration_validator.py | → | D_TRADING 交易运营: State Migration Validator — v0.40.0 R497 (stat... | 测试依赖 / test_depends |
| 1958 | test_stochastic_diagnosis_verifier.py | → | D_TRADING 交易运营: Stochastic Diagnosis Verifier — v0.38.0 R483 (... | 测试依赖 / test_depends |
| 1959 | test_stochastic_diagnosis_verifier_v2.py | → | D_TRADING 交易运营: Stochastic Diagnosis Verifier — v0.38.0 R483 (... | 测试依赖 / test_depends |
| 1960 | test_synthetic_anomaly_generator.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1961 | test_system_entropy_monitor.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1962 | test_teacher_transfer.py | → | D_TRADING 交易运营: Teacher Transfer — v0.6.0 R53 (teacher_transfe... | 测试依赖 / test_depends |
| 1963 | test_timezone_semantic_reasoner.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1964 | test_token_finops.py | → | D_TRADING 交易运营: Token FinOps — v0.12.0 R162 (token_finops.py) | 测试依赖 / test_depends |
| 1965 | test_training_data_gov.py | → | D_TRADING 交易运营: Training Data Governance — v0.14.0 R191 (train... | 测试依赖 / test_depends |
| 1966 | test_trend_cycle_separator.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1967 | test_validator.py | → | D_TRADING 交易运营: template.py | 测试依赖 / test_depends |
| 1968 | test_validator.py | → | D_TRADING 交易运营: validator.py | 测试依赖 / test_depends |
| 1969 | test_vertical_self_assessment.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1970 | test_worm_write_integrity.py | → | D_TRADING 交易运营: WORM Write Integrity — v0.15.0 R216 (worm_writ... | 测试依赖 / test_depends |
| 1971 | test_file_task_mapper_root.py | → | D_TRADING 交易运营: FileTaskMapper — 文件路径 ↔ Task N:N 映射器（... | 测试依赖 / test_depends |
| 1972 | test_fle_anomaly_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1973 | test_fle_anomaly_detector.py | → | D_TRADING 交易运营: FeedbackCollector: collect task execution feedb... | 测试依赖 / test_depends |
| 1974 | test_fle_anomaly_detector.py | → | D_TRADING 交易运营: MetricsCollector: append-only metrics recording... | 测试依赖 / test_depends |
| 1975 | test_fle_anomaly_detector.py | → | D_TRADING 交易运营: protocols.py | 测试依赖 / test_depends |
| 1976 | test_fle_chaos_engineering.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1977 | test_fle_config.py | → | D_TRADING 交易运营: config.py | 测试依赖 / test_depends |
| 1978 | test_fle_dogfood_monitor.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1979 | test_fle_exceptions.py | → | D_TRADING 交易运营: exceptions.py | 测试依赖 / test_depends |
| 1980 | test_fle_feedback_collector.py | → | D_TRADING 交易运营: FeedbackCollector: collect task execution feedb... | 测试依赖 / test_depends |
| 1981 | test_fle_generator.py | → | D_TRADING 交易运营: generator.py | 测试依赖 / test_depends |
| 1982 | test_fle_metrics_collector.py | → | D_TRADING 交易运营: MetricsCollector: append-only metrics recording... | 测试依赖 / test_depends |
| 1983 | test_fle_performance_regression_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1984 | test_fle_protocols.py | → | D_TRADING 交易运营: protocols.py | 测试依赖 / test_depends |
| 1985 | test_fle_regime_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 1986 | test_fle_self_slo_metrics.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 1987 | test_fle_template.py | → | D_TRADING 交易运营: template.py | 测试依赖 / test_depends |
| 1988 | test_fle_upgrade_safety_validator.py | → | D_TRADING 交易运营: R529: FLEUpgradeSafetyValidator (fle_upgrade_sa... | 测试依赖 / test_depends |
| 1989 | test_fle_validator.py | → | D_TRADING 交易运营: template.py | 测试依赖 / test_depends |
| 1990 | test_fle_validator.py | → | D_TRADING 交易运营: validator.py | 测试依赖 / test_depends |
| 1991 | test_ci_cd_pre_scanner.py | → | D_TRADING 交易运营: CI/CD Pre-Scanner — v0.8.0 R107 (ci_cd_pre_sca... | 测试依赖 / test_depends |
| 1992 | test_concurrent_change_deconfliction.py | → | D_TRADING 交易运营: Concurrent Change Deconfliction — v0.16.0 R230... | 测试依赖 / test_depends |
| 1993 | test_conflict_arbitration.py | → | D_TRADING 交易运营: Conflict Arbitration — v0.10.0 R130 (conflict_... | 测试依赖 / test_depends |
| 1994 | test_cve_scanner.py | → | D_TRADING 交易运营: CVE Scanner — v0.8.0 R106 (cve_scanner.py) | 测试依赖 / test_depends |
| 1995 | test_deployment_suppression.py | → | D_TRADING 交易运营: Deployment Suppression — v0.37.0 R464 (deploym... | 测试依赖 / test_depends |
| 1996 | test_dynamic_llm_cost_router.py | → | D_TRADING 交易运营: Dynamic LLM Cost Router — v0.8.0 R109 (dynamic... | 测试依赖 / test_depends |
| 1997 | test_emergency_takeover.py | → | D_TRADING 交易运营: Emergency Takeover — v0.7.0 R88 (emergency_tak... | 测试依赖 / test_depends |
| 1998 | test_federated_security.py | → | D_TRADING 交易运营: Federated Security — v0.10.0 R131 (federated_s... | 测试依赖 / test_depends |
| 1999 | test_flag_lifecycle_manager.py | → | D_TRADING 交易运营: Flag Lifecycle Manager — v0.3.0 R11 (flag_life... | 测试依赖 / test_depends |
| 2000 | test_license_compliance.py | → | D_TRADING 交易运营: License Compliance — v0.14.0 R198 (license_com... | 测试依赖 / test_depends |
| 2001 | test_merkle_audit_root.py | → | D_TRADING 交易运营: Merkle Audit Root — v0.8.0 R104 (merkle_audit_... | 测试依赖 / test_depends |
| 2002 | test_meta_performance_gate.py | → | D_TRADING 交易运营: Meta Performance Gate — v0.11.0 R158 (meta_per... | 测试依赖 / test_depends |
| 2003 | test_parameterized_safety_gate.py | → | D_TRADING 交易运营: GateVerdict — GateVerdict (parameterized_safet... | 测试依赖 / test_depends |
| 2004 | test_scope_creep_monitor.py | → | D_TRADING 交易运营: Scope Creep Monitor — v0.15.0 R220 (scope_cree... | 测试依赖 / test_depends |
| 2005 | test_spec_auditor.py | → | D_TRADING 交易运营: protocols.py | 测试依赖 / test_depends |
| 2006 | test_verdict_engine.py | → | D_TRADING 交易运营: verdict_engine.py | 测试依赖 / test_depends |
| 2007 | test_guard_cascade_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 2008 | test_guard_complexity_budget.py | → | D_TRADING 交易运营: R523: GuardComplexityBudget (guard_complexity_b... | 测试依赖 / test_depends |
| 2009 | test_guard_configuration_drift_monitor.py | → | D_TRADING 交易运营: R521: GuardConfigurationDriftMonitor (guard_con... | 测试依赖 / test_depends |
| 2010 | test_guard_interaction_topology_mapper.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 2011 | test_guard_oscillation_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 2012 | test_guard_self_consistency_auditor.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 2013 | test_graceful_degradation_planner.py | → | D_TRADING 交易运营: Graceful Degradation Planner — v0.40.0 R496 (g... | 测试依赖 / test_depends |
| 2014 | DM-202910: MCP boot_hooks 集成测试——验证10进.... | → | D_TRADING 交易运营: __init__.py | 测试依赖 / test_depends |
| 2015 | DM-202914: MCP boot→FLE→MCP→shutdown全链路E2... | → | D_TRADING 交易运营: __init__.py | 测试依赖 / test_depends |
| 2016 | DM-202914: MCP boot→FLE→MCP→shutdown全链路E2... | → | D_TRADING 交易运营: AutoRuntimeCore — 三层运行时运营中心（系统大脑... | 测试依赖 / test_depends |
| 2017 | test_split_brain_quorum.py | → | D_TRADING 交易运营: Split-Brain Quorum — v0.37.0 R451 (split_brain... | 测试依赖 / test_depends |
| 2018 | test_intent_driven_ops.py | → | D_TRADING 交易运营: Intent-Driven Ops — v0.12.0 R159 (intent_drive... | 测试依赖 / test_depends |
| 2019 | test_ke_quality.py | → | D_TRADING 交易运营: 知识质量评分契约（CT-KE-QUALITY）——KE完整性+.... | 测试依赖 / test_depends |
| 2020 | test_knowledge_bus_factor_monitor.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 2021 | test_knowledge_capture.py | → | D_TRADING 交易运营: Knowledge Capture — v0.4.0 R30 (knowledge_capt... | 测试依赖 / test_depends |
| 2022 | test_knowledge_distillation.py | → | D_TRADING 交易运营: Knowledge Distillation — v0.6.0 R52 (knowledge... | 测试依赖 / test_depends |
| 2023 | test_knowledge_freshness.py | → | D_TRADING 交易运营: 知识新鲜度废止管理器（CT-KNOWLEDGE-FRESHNESS）.... | 测试依赖 / test_depends |
| 2024 | test_knowledge_injection.py | → | D_TRADING 交易运营: Knowledge Injection — v0.8.0 R102 (knowledge_i... | 测试依赖 / test_depends |
| 2025 | test_knowledge_injection_pre_flight_verifier.py | → | D_TRADING 交易运营: R515: KnowledgeInjectionPreFlightVerifier (know... | 测试依赖 / test_depends |
| 2026 | test_knowledge_market.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 2027 | test_knowledge_packaging.py | → | D_TRADING 交易运营: Knowledge Packaging — v0.9.0 R123 (knowledge_p... | 测试依赖 / test_depends |
| 2028 | test_cross_module_integration_llm_security.py | → | D_TRADING 交易运营: AgentOrchestrator · 多角色 Agent 路由、工具链.... | 测试依赖 / test_depends |
| 2029 | test_dep_cve_correlator.py | → | D_TRADING 交易运营: Dependency CVE Correlator — v0.14.0 R196 (dep_... | 测试依赖 / test_depends |
| 2030 | test_llm_cost_accounting.py | → | D_TRADING 交易运营: LLM Cost Accounting — v0.4.0 R35 (llm_cost_acc... | 测试依赖 / test_depends |
| 2031 | test_llm_cost_router.py | → | D_TRADING 交易运营: LLM Cost Router — v0.3.0 R20 (llm_cost_router.py) | 测试依赖 / test_depends |
| 2032 | test_llm_provider_integrity.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 2033 | test_llm_quality_regression.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 2034 | test_metric_prompt_scanner.py | → | D_TRADING 交易运营: Metric-Prompt Scanner — v0.15.0 R215 (metric_p... | 测试依赖 / test_depends |
| 2035 | test_orphan_detector.py | → | D_TRADING 交易运营: ModuleOnboardingScanner — 模块接入扫描器 (modu... | 测试依赖 / test_depends |
| 2036 | test_remote_attestation.py | → | D_TRADING 交易运营: Remote Attestation — v0.15.0 R211 (remote_atte... | 测试依赖 / test_depends |
| 2037 | test_secret_rotation.py | → | D_TRADING 交易运营: Secret Rotation — v0.14.0 R189 (secret_rotatio... | 测试依赖 / test_depends |
| 2038 | test_security.py | → | D_TRADING 交易运营: Agent Skill Guard — v0.14.0 R201 (agent_skill_... | 测试依赖 / test_depends |
| 2039 | test_security.py | → | D_TRADING 交易运营: Dependency CVE Correlator — v0.14.0 R196 (dep_... | 测试依赖 / test_depends |
| 2040 | test_security.py | → | D_TRADING 交易运营: Metric-Prompt Scanner — v0.15.0 R215 (metric_p... | 测试依赖 / test_depends |
| 2041 | test_security.py | → | D_TRADING 交易运营: Remote Attestation — v0.15.0 R211 (remote_atte... | 测试依赖 / test_depends |
| 2042 | test_security.py | → | D_TRADING 交易运营: Secret Rotation — v0.14.0 R189 (secret_rotatio... | 测试依赖 / test_depends |
| 2043 | test_security.py | → | D_TRADING 交易运营: Wireheading Prevention — v0.37.0 R486 (wirehea... | 测试依赖 / test_depends |
| 2044 | test_wireheading_prevention.py | → | D_TRADING 交易运营: Wireheading Prevention — v0.37.0 R486 (wirehea... | 测试依赖 / test_depends |
| 2045 | test_memory_self_check.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 2046 | test_model_health.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 2047 | test_model_rotation.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 2048 | test_model_rotation_v2.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 2049 | test_model_version_semantic_drift.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 2050 | test_multi_agent_orchestrator.py | → | D_TRADING 交易运营: Multi-Agent Orchestrator — v0.12.0 R159b (mult... | 测试依赖 / test_depends |
| 2051 | test_multi_instance_coord.py | → | D_TRADING 交易运营: Multi-Instance Coordinator — v0.14.0 R199 (mul... | 测试依赖 / test_depends |
| 2052 | test_multi_signal_correlator.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 2053 | test_trace_causal_bridge.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 2054 | test_deferred_queue.py | → | D_TRADING 交易运营: DeferredQueue: WAITING -> READY task scheduler.... | 测试依赖 / test_depends |
| 2055 | test_orchestrator_data_lifecycle.py | → | D_TRADING 交易运营: data_lifecycle.py | 测试依赖 / test_depends |
| 2056 | test_orchestrator_failure_matcher.py | → | D_TRADING 交易运营: FailurePatternMatcher — 任务失败模式识别与纠正... | 测试依赖 / test_depends |
| 2057 | test_orchestrator_hallucination_detector.py | → | D_TRADING 交易运营: HallucinationDetector · Chain-of-Verification.... | 测试依赖 / test_depends |
| 2058 | test_orchestrator_model_registry.py | → | D_TRADING 交易运营: model_registry.py | 测试依赖 / test_depends |
| 2059 | test_orchestrator_rollback_manager.py | → | D_TRADING 交易运营: RollbackManager — 仅调试用途的 DB-state 快照，... | 测试依赖 / test_depends |
| 2060 | test_orchestrator_task_queue.py | → | D_TRADING 交易运营: ActiveTaskQueue — 后台任务轮询与自动分发 (task... | 测试依赖 / test_depends |
| 2061 | test_orchestrator_trigger_router.py | → | D_TRADING 交易运营: TriggerRouter — RI-03 触发路由器（M3 跨模块触.... | 测试依赖 / test_depends |
| 2062 | test_orchestrator_wave_generator.py | → | D_TRADING 交易运营: WaveGenerator — 根据 Task 依赖图生成执行 Wave.... | 测试依赖 / test_depends |
| 2063 | test_path_index.py | → | D_TRADING 交易运营: path_index.py | 测试依赖 / test_depends |
| 2064 | test_phase_executor_root.py | → | D_TRADING 交易运营: Phase 执行引擎（Phase Executor） (phase_executo... | 测试依赖 / test_depends |
| 2065 | test_integration_test_pipeline.py | → | D_TRADING 交易运营: E2E Integration Test Pipeline — TASK-MOD-FEEDB... | 测试依赖 / test_depends |
| 2066 | test_pipeline_agent_bridge.py | → | D_TRADING 交易运营: AgentOrchestrator · 多角色 Agent 路由、工具链.... | 测试依赖 / test_depends |
| 2067 | test_prompt_factory_governance.py | → | D_TRADING 交易运营: Prompt Factory Governance — v0.16.0 R224 (prom... | 测试依赖 / test_depends |
| 2068 | test_prompt_fingerprint.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 2069 | test_prompt_optimization_regression_detector.py | → | D_TRADING 交易运营: R514: PromptOptimizationRegressionDetector (pro... | 测试依赖 / test_depends |
| 2070 | test_prompt_sanitizer.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 2071 | test_prompt_self_optimization_loop.py | → | D_TRADING 交易运营: R502: PromptSelfOptimizationLoop (prompt_self_o... | 测试依赖 / test_depends |
| 2072 | test_prompt_version.py | → | D_TRADING 交易运营: AI Prompt 版本控制（CT-PROMPT-VERSION）——prom... | 测试依赖 / test_depends |
| 2073 | test_resource_optimization.py | → | D_TRADING 交易运营: resource_optimization.py - MAPE-K autonomic res... | 测试依赖 / test_depends |
| 2074 | test_resource_starvation_aware.py | → | D_TRADING 交易运营: Resource Starvation Aware — v0.15.0 R209 (reso... | 测试依赖 / test_depends |
| 2075 | test_blast_radius_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 2076 | test_risk_registry_root.py | → | D_TRADING 交易运营: risk_registry.py | 测试依赖 / test_depends |
| 2077 | test_rollback_integrity.py | → | D_TRADING 交易运营: Rollback Integrity — v0.3.0 R18b (rollback_int... | 测试依赖 / test_depends |
| 2078 | test_attack_simulator.py | → | D_TRADING 交易运营: Attack Simulator — v0.6.0 R57 (attack_simulato... | 测试依赖 / test_depends |
| 2079 | test_safety_gate_l1_l27.py | → | D_TRADING 交易运营: Safety Gates L1-L27 — Unified Pipeline (MOD-FE... | 测试依赖 / test_depends |
| 2080 | test_scheduler_safety.py | → | D_TRADING 交易运营: scheduler_safety.py | 测试依赖 / test_depends |
| 2081 | test_self_api_throttle_defense.py | → | D_TRADING 交易运营: Self API Throttle Defense — v0.39.0 R491 (self... | 测试依赖 / test_depends |
| 2082 | test_self_audit.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 2083 | test_self_benchmark.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 2084 | test_self_bottleneck_detector.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 2085 | test_self_diagnosis.py | → | D_TRADING 交易运营: self_diagnosis.py — 自我诊断 (DD120, TASK-020)... | 测试依赖 / test_depends |
| 2086 | test_self_diagnosis_data_leak_detector.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 2087 | test_self_ha.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 2088 | test_self_health_monitor.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 2089 | test_self_llm_observability.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 2090 | test_self_modification_audit.py | → | D_TRADING 交易运营: Self-Modification Audit — v0.15.0 R218 (self_m... | 测试依赖 / test_depends |
| 2091 | test_self_modification_rate_limiter.py | → | D_TRADING 交易运营: R522: SelfModificationRateLimiter (self_modific... | 测试依赖 / test_depends |
| 2092 | test_self_reflection.py | → | D_TRADING 交易运营: Self Reflection — v0.7.0 R75 (self_reflection.py) | 测试依赖 / test_depends |
| 2093 | test_self_upgrade_canary.py | → | D_TRADING 交易运营: Self Upgrade Canary — v0.14.0 R194 (self_upgra... | 测试依赖 / test_depends |
| 2094 | test_semantic_intent_preservation_guard.py | → | D_TRADING 交易运营: R505: SemanticIntentPreservationGuard (semantic... | 测试依赖 / test_depends |
| 2095 | test_session_conflict.py | → | D_TRADING 交易运营: Session 冲突预防契约（CT-SESSION-CONFLICT）——... | 测试依赖 / test_depends |
| 2096 | test_session_learner.py | → | D_TRADING 交易运营: session_learner.py — 在线学习 (DD114, TASK-020... | 测试依赖 / test_depends |
| 2097 | test_session_manager.py | → | D_TRADING 交易运营: SessionManager — AI Agent 会话生命周期管理（CT... | 测试依赖 / test_depends |
| 2098 | test_task_gate.py | → | D_TRADING 交易运营: TaskGate --- 任务门控 (task_gate.py) | 测试依赖 / test_depends |
| 2099 | test_temporal_coherence_of_self_model.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 2100 | test_temporal_event_store.py | → | D_TRADING 交易运营: Temporal Event Store — v0.3.0 R9 (temporal_eve... | 测试依赖 / test_depends |
| 2101 | test_temporal_integrity_guard.py | → | D_TRADING 交易运营: feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 2102 | test_temporal_pattern.py | → | D_TRADING 交易运营: feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 2103 | miniqmt_broker 正式测试（原 scripts/tests/ 临时... | → | D_TRADING 交易运营: order.py | 测试依赖 / test_depends |
| 2104 | test_admission_controller.py | → | D_TRADING 交易运营: admission_controller.py | 测试依赖 / test_depends |
| 2105 | test_admission_controller.py | → | D_TRADING 交易运营: verdict_engine.py | 测试依赖 / test_depends |
| 2106 | test_batch_orchestrator.py | → | D_TRADING 交易运营: BatchOrchestrator — 多 Worker 批量任务协调器（... | 测试依赖 / test_depends |
| 2107 | test_benchmark_runner.py | → | D_TRADING 交易运营: benchmark_runner.py | 测试依赖 / test_depends |
| 2108 | test_blind_spot_closure.py | → | D_TRADING 交易运营: blind_spot_closure.py | 测试依赖 / test_depends |
| 2109 | test_boot_cron_jobs.py | → | D_TRADING 交易运营: __init__.py | 测试依赖 / test_depends |
| 2110 | test_boot_hooks.py | → | D_TRADING 交易运营: boot_hooks.py | 测试依赖 / test_depends |
| 2111 | test_bulkhead_manager.py | → | D_TRADING 交易运营: bulkhead_manager.py | 测试依赖 / test_depends |
| 2112 | test_construction_guide.py | → | D_TRADING 交易运营: 施工指南引擎（Construction Guide） (constructio... | 测试依赖 / test_depends |
| 2113 | test_degrade_cascade.py | → | D_TRADING 交易运营: degrade_cascade.py | 测试依赖 / test_depends |
| 2114 | test_design_decisions_root.py | → | D_TRADING 交易运营: design_decisions.py | 测试依赖 / test_depends |
| 2115 | test_disk_guard.py | → | D_TRADING 交易运营: disk_guard.py | 测试依赖 / test_depends |
| 2116 | test_dlq_manager_root.py | → | D_TRADING 交易运营: DLQ 管理器（Dead Letter Queue Manager — CT-DLQ... | 测试依赖 / test_depends |
| 2117 | test_dream_cycle.py | → | D_TRADING 交易运营: DreamCycle — 知识固化引擎 (dream_cycle.py) | 测试依赖 / test_depends |
| 2118 | F14 管线编排/反馈环 — 红蓝对抗端到端极端测试 (... | → | D_TRADING 交易运营: Error Budget 状态机——monthly budget + burn_ra... | 测试依赖 / test_depends |
| 2119 | F14 管线编排/反馈环 — 红蓝对抗端到端极端测试 (... | → | D_TRADING 交易运营: FLE 全链路调度器 —— collect->detect->diagnose... | 测试依赖 / test_depends |
| 2120 | F1 自动驾驶/运行时大脑 — 红蓝对抗端到端极端测... | → | D_TRADING 交易运营: DreamCycle — 知识固化引擎 (dream_cycle.py) | 测试依赖 / test_depends |
| 2121 | F1 自动驾驶/运行时大脑 — 红蓝对抗端到端极端测... | → | D_TRADING 交易运营: HealthMonitor — 健康监控 + 自愈 (health_monito... | 测试依赖 / test_depends |
| 2122 | F1 自动驾驶/运行时大脑 — 红蓝对抗端到端极端测... | → | D_TRADING 交易运营: WorkDAG + WorkItem — 工作编排数据模型 (work_da... | 测试依赖 / test_depends |
| 2123 | F1 自动驾驶/运行时大脑 — 红蓝对抗端到端极端测... | → | D_TRADING 交易运营: work_orchestrator.py | 测试依赖 / test_depends |
| 2124 | test_fault_types.py | → | D_TRADING 交易运营: Fault type registry and preset templates for ch... | 测试依赖 / test_depends |
| 2125 | test_feature_flag.py | → | D_TRADING 交易运营: FeatureFlag 管理器（CT-FEATUREFLAG-001）——CT-... | 测试依赖 / test_depends |
| 2126 | test_finalizer.py | → | D_TRADING 交易运营: Finalizer — 优雅清理器 (finalizer.py) | 测试依赖 / test_depends |
| 2127 | test_finding_bridge.py | → | D_TRADING 交易运营: CT-ORC-SCRIPT-001 运行时桥接 (finding_bridge.py) | 测试依赖 / test_depends |
| 2128 | test_gpu_consensus_scheduler.py | → | D_TRADING 交易运营: gpu_consensus_scheduler.py | 测试依赖 / test_depends |
| 2129 | test_gpu_consensus_scheduler.py | → | D_TRADING 交易运营: verdict_engine.py | 测试依赖 / test_depends |
| 2130 | test_housekeeping.py | → | D_TRADING 交易运营: 文件卫生保洁管理器（CT-HOUSEKEEPING）——临时文... | 测试依赖 / test_depends |
| 2131 | IdeHealthDaemon 测试. (test_ide_health_daemon.py) | → | D_TRADING 交易运营: ide_health_daemon.py — TRAE IDE 幽灵窗口守护线... | 测试依赖 / test_depends |
| 2132 | test_incident_postmortem.py | → | D_TRADING 交易运营: 事件复盘管理器（CT-INCIDENT）——incident记录+t... | 测试依赖 / test_depends |
| 2133 | test_integration_registry.py | → | D_TRADING 交易运营: IntegrationRegistry — 集成注册表 (integration_... | 测试依赖 / test_depends |
| 2134 | test_lean_scanner.py | → | D_TRADING 交易运营: 死代码/孤儿文件/僵尸引用三扫描（CT-LEAN）——三... | 测试依赖 / test_depends |
| 2135 | test_lifecycle_manager.py | → | D_TRADING 交易运营: CapabilityRegistry — 能力注册中心 (capability_... | 测试依赖 / test_depends |
| 2136 | test_lifecycle_manager.py | → | D_TRADING 交易运营: DreamCycle — 知识固化引擎 (dream_cycle.py) | 测试依赖 / test_depends |
| 2137 | test_lifecycle_manager.py | → | D_TRADING 交易运营: Feedback Loop Engine — MOD-FEEDBACK_LOOP. (__i... | 测试依赖 / test_depends |
| 2138 | test_lifecycle_manager.py | → | D_TRADING 交易运营: Finalizer — 优雅清理器 (finalizer.py) | 测试依赖 / test_depends |
| 2139 | test_lifecycle_manager.py | → | D_TRADING 交易运营: HealthMonitor — 健康监控 + 自愈 (health_monito... | 测试依赖 / test_depends |
| 2140 | test_lifecycle_manager.py | → | D_TRADING 交易运营: IntegrationRegistry — 集成注册表 (integration_... | 测试依赖 / test_depends |
| 2141 | test_lifecycle_manager.py | → | D_TRADING 交易运营: lifecycle_manager.py | 测试依赖 / test_depends |
| 2142 | test_lifecycle_manager.py | → | D_TRADING 交易运营: NightShiftQueue — 夜班登记表持久化 (night_shif... | 测试依赖 / test_depends |
| 2143 | test_lifecycle_manager.py | → | D_TRADING 交易运营: StopGate — 质量闸门 (stop_gate.py) | 测试依赖 / test_depends |
| 2144 | test_lifecycle_manager.py | → | D_TRADING 交易运营: work_orchestrator.py | 测试依赖 / test_depends |
| 2145 | test_module_onboarding_scanner.py | → | D_TRADING 交易运营: CapabilityCard — 能力卡片数据模型 (capability_... | 测试依赖 / test_depends |
| 2146 | test_module_onboarding_scanner.py | → | D_TRADING 交易运营: CapabilityRegistry — 能力注册中心 (capability_... | 测试依赖 / test_depends |
| 2147 | test_module_onboarding_scanner.py | → | D_TRADING 交易运营: ModuleOnboardingScanner — 模块接入扫描器 (modu... | 测试依赖 / test_depends |
| 2148 | test_network_partition.py | → | D_TRADING 交易运营: 网络分区容忍（CT-NETWORK-PARTITION）——CAP定理... | 测试依赖 / test_depends |
| 2149 | test_night_shift_queue.py | → | D_TRADING 交易运营: NightShiftQueue — 夜班登记表持久化 (night_shif... | 测试依赖 / test_depends |
| 2150 | test_protection_index.py | → | D_TRADING 交易运营: protection_index.py | 测试依赖 / test_depends |
| 2151 | test_protection_index.py | → | D_TRADING 交易运营: verdict_engine.py | 测试依赖 / test_depends |
| 2152 | test_reconciliation_loop.py | → | D_TRADING 交易运营: reconciliation_loop.py | 测试依赖 / test_depends |
| 2153 | test_rolling_upgrade.py | → | D_TRADING 交易运营: 零停机滚动升级（CT-DEPLOY）——graceful shutdow... | 测试依赖 / test_depends |
| 2154 | test_runtime_config.py | → | D_TRADING 交易运营: runtime_config.py | 测试依赖 / test_depends |
| 2155 | test_schema_migration.py | → | D_TRADING 交易运营: 数据库 Schema 演化契约（CT-SCHEMA-MIGRATE）——... | 测试依赖 / test_depends |
| 2156 | test_stability_guard.py | → | D_TRADING 交易运营: API 稳定性守护（CT-STABILITY）——public API签.... | 测试依赖 / test_depends |
| 2157 | test_staging_area.py | → | D_TRADING 交易运营: StagingArea — 多AI并发草稿写入+提交+冲突检测模... | 测试依赖 / test_depends |
| 2158 | test_startup_sequencer.py | → | D_TRADING 交易运营: startup_sequencer.py | 测试依赖 / test_depends |
| 2159 | test_state_propagation_root.py | → | D_TRADING 交易运营: 全局状态传播链（State Propagation Chain） (stat... | 测试依赖 / test_depends |
| 2160 | test_state_synchronizer_root.py | → | D_TRADING 交易运营: StateSynchronizer — 同步 SQLite 状态与文件系统... | 测试依赖 / test_depends |
| 2161 | test_status_dashboard.py | → | D_TRADING 交易运营: HealthMonitor — 健康监控 + 自愈 (health_monito... | 测试依赖 / test_depends |
| 2162 | test_status_dashboard.py | → | D_TRADING 交易运营: StatusDashboard — 实时状态面板 (status_dashboa... | 测试依赖 / test_depends |
| 2163 | test_stop_gate.py | → | D_TRADING 交易运营: StopGate — 质量闸门 (stop_gate.py) | 测试依赖 / test_depends |
| 2164 | test_system_transfer.py | → | D_TRADING 交易运营: 系统移交恢复（CT-TRANSFER）——系统Owner变更+配... | 测试依赖 / test_depends |
| 2165 | test_teardown_manager.py | → | D_TRADING 交易运营: teardown_manager.py | 测试依赖 / test_depends |
| 2166 | test_version_manifest.py | → | D_TRADING 交易运营: version_manifest.py | 测试依赖 / test_depends |
| 2167 | test_work_dag.py | → | D_TRADING 交易运营: WorkDAG + WorkItem — 工作编排数据模型 (work_da... | 测试依赖 / test_depends |
| 2168 | test_work_orchestrator.py | → | D_TRADING 交易运营: WorkDAG + WorkItem — 工作编排数据模型 (work_da... | 测试依赖 / test_depends |
| 2169 | test_work_orchestrator.py | → | D_TRADING 交易运营: work_orchestrator.py | 测试依赖 / test_depends |
...
> 本域与 26 个外部域直接连接（出边 2261 条 + 入边 6 条 = 2267 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_AUDITTEST -->|537条 contract / contract, data / data, runtime / runtime, 测试依赖 / test_depends| D_GOVERNANCE
