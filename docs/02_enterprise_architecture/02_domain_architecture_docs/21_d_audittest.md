...
graph TD
    ...
    class D_GOVERNANCE,D_TRADING,D_SECURITY_LLM,D_INTEGRATION,D_INTEGRATION_GATEWAY external_prod
```
...
graph TD
    ...
    class D_GOVERNANCE,D_SECURITY,D_AUTONOMY_CORE,D_TRADING,D_INTEGRATION external_prod
```
...
> 本域与 25 个外部域直接连接（出边 2289 条 + 入边 4 条 = 2293 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_GOVERNANCE -->|4条 runtime / runtime| D_AUDITTEST
```
