...
| 模块数 | 1707 | Module Count | 1707 |
| 域内依赖 | 11 | Internal Dependencies | 11 |
| 跨域入边 | 0 | Cross-domain Incoming | 0 |
| 跨域出边 | 2197 | Cross-domain Outgoing | 2197 |
| 设计态模块 | 0 | Design Modules | 0 |
| 原型态模块 | 1658 | Prototype Modules | 1658 |
...
> 按 architecture_layer 分组的模块清单（共 1707 个模块 / 1707 modules）。
...
### L2 领域层 / Domain Layer (1706 modules)
...
> 展示全部 1707 个模块（生产态 49 + 设计态 0 + 原型态 1658），标签标注成熟度。
...
graph TD
    ...
    class D_GOVERNANCE,D_TRADING,D_AUTONOMY_CORE,D_INFRA_RUNTIME,D_SECURITY external_prod
```
...
graph TD
    ...
    class D_TRADING,D_SECURITY,D_INFRA_A2A,D_GOVERNANCE,D_INFRA_TELEMETRY,D_INFRA_RUNTIME,D_SHARED,D_OPS external_prod
```
...
graph TD
    ...
    class D_GOVERNANCE,D_SHARED,D_TRADING,D_AUTONOMY_CORE,D_FUNDAMENTAL_SIGNAL,D_INFRA_RUNTIME,D_INTEGRATION external_prod
```
...
graph TD
    ...
    class D_GOV_ENFORCEMENT,D_INFRA_RECOVERY,D_GOVERNANCE,D_SECURITY,D_TRADING external_prod
```
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 1658 个，11 条域内依赖）。
...
> 本域与 23 个外部域直接连接（出边 2197 条 + 入边 0 条 = 2197 条）。只显示直接连接的域，不展开具体节点。
