...
graph TD
    ...
    class D_SHARED,D_TRADING,D_AUTONOMY_CORE,D_GOVERNANCE,D_GOV_RULE,D_GOV_AUDIT external_prod
    class D_GOV_DOCS,D_INTEGRATION_GATEWAY,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_SHARED,D_INTELLIGENCE,D_SECURITY,D_INFRA_RUNTIME,D_GOV_AUDIT,D_AUTONOMY_CORE,D_GOVERNANCE,D_GOV_RULE,D_GOV_KB external_prod
...
graph TD
    ...
    class D_GOV_KB,D_FEEDBACK_LOOP,D_AUTONOMY_CORE,D_INTELLIGENCE,D_GOVERNANCE,D_TRADING external_design
```
...
graph TD
    ...
    src_zephyr_integration_vector_memory_collection_manager_py -->|导入依赖 / import_depends| src_zephyr_integration_vector_memory_vms_errors_py
    ...
    class D_SHARED,D_TRADING,D_INTELLIGENCE,D_AUTONOMY_CORE,D_GOVERNANCE,D_SECURITY,D_INFRA_RUNTIME external_prod
    class D_INTEGRATION_GATEWAY,D_GOV_ENFORCEMENT,D_ORCHESTRATOR external_design
```
...
| 141 | D_TRADING 交易运营: verdict_engine.py | → | LocalModelScheduler — L2 本地模型 24/7 调度循... | 导入依赖 / import_depends |
| 142 | D_TRADING 交易运营: test_lifecycle_manager.py | → | runtime_types.py | 测试依赖 / test_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 24 个外部域直接连接（出边 112 条 + 入边 142 条 = 254 条）。只显示直接连接的域，不展开具体节点。
