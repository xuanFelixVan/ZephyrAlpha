...
graph TD
    ...
    class D_GOVERNANCE,D_SHARED,D_OPS,D_GOV_RULE,D_AUTONOMY_CORE,D_TRADING external_prod
    class D_INTELLIGENCE,D_INFRA_RUNTIME,D_GOV_DOCS external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOV_RULE,D_FEEDBACK_LOOP,D_GOV_OPS_RESILIENCE,D_GOV_DRIFT,D_GOV_ENFORCEMENT,D_AUTONOMY_CORE,D_INFRA_RUNTIME external_prod
...
graph TD
    ...
    class D_GOVERNANCE,D_SHARED,D_OPS,D_GOV_RULE,D_AUTONOMY_CORE,D_TRADING,D_GOV_OPS_RESILIENCE external_prod
    class D_INTELLIGENCE,D_INFRA_RUNTIME,D_ORCHESTRATOR external_design
```
...
> 本域与 17 个外部域直接连接（出边 97 条 + 入边 37 条 = 134 条）。只显示直接连接的域，不展开具体节点。
