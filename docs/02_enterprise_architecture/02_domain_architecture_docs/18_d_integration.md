...
graph TD
    ...
    class D_INTELLIGENCE,D_AUTONOMY_CORE,D_SHARED,D_SECURITY,D_OPS,D_GOVERNANCE,D_INFRA_RUNTIME,D_TRADING external_prod
```
...
graph TD
    ...
    class D_INTELLIGENCE,D_AUTONOMY_CORE,D_SHARED,D_SECURITY,D_OPS,D_GOVERNANCE,D_GOV_OPS_RESILIENCE,D_INFRA_RUNTIME,D_TRADING external_prod
...
> 本域与 16 个外部域直接连接（出边 100 条 + 入边 34 条 = 134 条）。只显示直接连接的域，不展开具体节点。
