...
graph TD
    ...
    class D_SHARED,D_REPORTING,D_SIMULATION,D_PF_ALLOC,D_EX_CORE external_prod
    class D_TRADING,D_GOVERNANCE,D_MKT_DATA external_design
```
...
graph TD
    ...
    class D_SHARED,D_FBL_VERIFICATION,D_FBL_DETECTORS,D_GOV_DRIFT,D_FEEDBACK_LOOP,D_AUTONOMY_CORE,D_GOV_CODE_QUALITY,D_GOV_OPS_RESILIENCE,D_INFRA_RUNTIME,D_GOVERNANCE,D_SECURITY,D_RISK,D_FUNDAMENTAL_SIGNAL external_prod
    class D_TRADING,D_PF_ALLOC,D_SIGLEGACY external_design
```
...
graph TD
    ...
    D_SECURITY["(生产态 / production) D_SECURITY"]
    tests_contracts_test_abac_guard_root_py -.->|测试依赖 / test_depends| D_SECURITY
    src_zephyr_shared_contracts_portfolio_init_py -.->|导入依赖 / import_depends| D_SHARED
    src_zephyr_shared_contracts_security_init_py -.->|导入依赖 / import_depends| D_SHARED
    src_zephyr_shared_contracts_backpressure_init_py -.->|导入依赖 / import_depends| D_SHARED
    src_zephyr_shared_contracts_core_init_py -.->|导入依赖 / import_depends| D_SHARED
    src_zephyr_shared_contracts_order_py -.->|导入依赖 / import_depends| D_SHARED
    ...
    class D_SHARED,D_INFRA_RUNTIME,D_SECURITY,D_GOV_CODE_QUALITY,D_ORCHESTRATOR,D_REPORTING,D_PF_ALLOC,D_EX_CORE,D_TRADING external_prod
...
> 本域与 25 个外部域直接连接（出边 73 条 + 入边 86 条 = 159 条）。只显示直接连接的域，不展开具体节点。
