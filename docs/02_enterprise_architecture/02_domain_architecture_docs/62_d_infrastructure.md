...
graph TD
    ...
    class D_SHARED,D_FBL_DETECTORS,D_FBL_VERIFICATION,D_GOV_DRIFT,D_AUTONOMY_CORE,D_FEEDBACK_LOOP,D_GOV_CODE_QUALITY,D_INFRA_RUNTIME,D_GOV_OPS_RESILIENCE,D_TRADING,D_ORCHESTRATOR,D_INTEGRATION,D_EX_CORE external_prod
...
graph TD
    ...
    class D_SHARED,D_TRADING,D_INFRA_RUNTIME,D_SIMULATION,D_RISK,D_EX_CORE external_prod
    class D_REPORTING,D_PF_ALLOC,D_GOVERNANCE external_design
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_ORCHESTRATOR,D_TRADING,D_PF_ALLOC,D_REPORTING,D_INTEGRATION external_prod
    class D_GOV_ENFORCEMENT,D_SHARED,D_GOVERNANCE external_design
```
...
| 10 | D_GOVERNANCE 生命周期管理: 一键备份.ps1 | → | backup_reconciler.py — 灾备备份系统事件触发器.... | config_depends / config_depends |
| 11 | D_GOVERNANCE 生命周期管理: local_layer_daemon.py — L2 本地模型层守护进程.... | → | ZephyrAlpha — 基础设施 Infrastructure Layer —... | 导入依赖 / import_depends |
| 12 | D_GOVERNANCE 生命周期管理: C-track 端到端演示 —— 全流水线一次性运行 (dem... | → | factor_signal.py | 导入依赖 / import_depends |
| 13 | D_GOVERNANCE 生命周期管理: C-track 端到端演示 —— 全流水线一次性运行 (dem... | → | fill.py | 导入依赖 / import_depends |
| 14 | D_GOVERNANCE 生命周期管理: C-track 端到端演示 —— 全流水线一次性运行 (dem... | → | model_serving_request.py | 导入依赖 / import_depends |
| 15 | D_GOVERNANCE 生命周期管理: C-track 端到端演示 —— 全流水线一次性运行 (dem... | → | order.py | 导入依赖 / import_depends |
...
| 22 | D_INTEGRATION 管线路由: PipelineOrchestrator — M1-M11 管线协调器 (pipe... | → | __init__.py | 导入依赖 / import_depends |
| 23 | D_MKT_DATA 行情数据: __init__.py | → | market_data.py | 导入依赖 / import_depends |
| 24 | D_ORCHESTRATOR 代理编排器: AgentOrchestrator · 多角色 Agent 路由、工具链.... | → | __init__.py | 导入依赖 / import_depends |
...
| 27 | D_REPORTING 报告: D_REPORTING Post-Trade Analytics (__init__.py) | → | performance_attribution_report.py | 导入依赖 / import_depends |
...
| 52 | D_TRADING 交易运营: zephyr.trading.trading_contracts — trading-dom... | → | performance_attribution_report.py | 导入依赖 / import_depends |
| 53 | D_TRADING 交易运营: D_EXECUTION_CORE — BrokerInterface (broker_int... | → | fill.py | 导入依赖 / import_depends |
| 54 | D_TRADING 交易运营: D_EXECUTION_CORE — BrokerInterface (broker_int... | → | order.py | 导入依赖 / import_depends |
| 55 | D_TRADING 交易运营: D_EXECUTION_CORE — BrokerInterface (broker_int... | → | position.py | 导入依赖 / import_depends |
...
> 本域与 21 个外部域直接连接（出边 78 条 + 入边 61 条 = 139 条）。只显示直接连接的域，不展开具体节点。
