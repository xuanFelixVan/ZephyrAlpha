...
graph TD
    ...
    class D_GOVERNANCE,D_GOV_ENFORCEMENT,D_REPORTING,D_MKT_DATA external_design
```
...
graph TD
    ...
    class D_SHARED,D_FBL_VERIFICATION,D_FBL_DETECTORS,D_GOV_DRIFT,D_FEEDBACK_LOOP,D_AUTONOMY_CORE,D_GOV_CODE_QUALITY,D_GOV_OPS_RESILIENCE,D_INFRA_RUNTIME,D_GOVERNANCE,D_SECURITY,D_TRADING,D_ORCHESTRATOR,D_INTEGRATION external_prod
...
graph TD
    ...
    class D_SHARED,D_INFRA_RUNTIME,D_TRADING,D_RISK,D_SIMULATION,D_EX_CORE external_prod
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 52 个，8 条域内依赖）。
...
graph TD
    ...
    class D_FBL_DETECTORS,D_FBL_VERIFICATION,D_GOV_DRIFT,D_ORCHESTRATOR,D_TRADING,D_PF_ALLOC external_prod
    class D_SHARED,D_GOVERNANCE,D_MKT_DATA,D_REPORTING external_design
```
...
| 15 | D_GOV_ENFORCEMENT 规则执行: Re-export shim — ComplianceRule 真源已合并至 z... | → | compliance_rule.py | 导入依赖 / import_depends |
| 16 | D_INFRA_RUNTIME 运行时集成: HealthMonitor — 健康监控 + 自愈 (health_monito... | → | telemetry_emitter.py | 导入依赖 / import_depends |
| 17 | D_INTEGRATION 管线路由: PipelineOrchestrator — M1-M11 管线协调器 (pipe... | → | __init__.py | 导入依赖 / import_depends |
| 18 | D_MKT_DATA 行情数据: __init__.py | → | market_data.py | 导入依赖 / import_depends |
| 19 | D_ORCHESTRATOR 代理编排器: AgentOrchestrator · 多角色 Agent 路由、工具链.... | → | __init__.py | 导入依赖 / import_depends |
| 20 | D_PF_ALLOC 组合分配: strategy_lifecycle_event.py | → | strategy_lifecycle_event.py | 导入依赖 / import_depends |
| 21 | D_PF_ALLOC 组合分配: D_PORTFOLIO_CORE — Default Equity Long-Only St... | → | order.py | 导入依赖 / import_depends |
| 22 | D_REPORTING 报告: D_REPORTING Post-Trade Analytics (__init__.py) | → | performance_attribution_report.py | 导入依赖 / import_depends |
| 23 | D_REPORTING 报告: D_REPORTING — Post-Trade Analytics Layer (anal... | → | execution_report.py | 导入依赖 / import_depends |
| 24 | D_REPORTING 报告: D_REPORTING — Post-Trade Analytics Layer (anal... | → | fill.py | 导入依赖 / import_depends |
| 25 | D_REPORTING 报告: D_REPORTING — Post-Trade Analytics Layer (anal... | → | order.py | 导入依赖 / import_depends |
| 26 | D_REPORTING 报告: D_REPORTING — Post-Trade Analytics Layer (anal... | → | performance_attribution_report.py | 导入依赖 / import_depends |
| 27 | D_REPORTING 报告: D_REPORTING — Default Attribution Engine (defa... | → | performance_attribution_report.py | 导入依赖 / import_depends |
| 28 | D_REPORTING 报告: D_REPORTING — Default TCA Engine (default_tca_... | → | execution_report.py | 导入依赖 / import_depends |
| 29 | D_REPORTING 报告: D_REPORTING — Default TCA Engine (default_tca_... | → | fill.py | 导入依赖 / import_depends |
| 30 | D_REPORTING 报告: D_REPORTING — Default TCA Engine (default_tca_... | → | order.py | 导入依赖 / import_depends |
| 31 | D_RISK 风控: D_RISK — Risk Limits Calculator (risk_limits.py) | → | risk_limits.py | 导入依赖 / import_depends |
| 32 | D_RISK 风控: ZephyrAlpha — D_RISK Risk Management Layer — ... | → | risk_limits.py | 导入依赖 / import_depends |
| 33 | D_RISK 风控: test_l04_risk_management.py | → | risk_limits.py | 测试依赖 / test_depends |
| 34 | D_SHARED 共享服务: ContractBus — 跨层通信抽象 + Pydantic v2 Schem... | → | ZephyrAlpha — shared/contracts/ (__init__.py) | config_depends / config_depends |
| 35 | D_SHARED 共享服务: ContractTester — 契约测试框架 (contract_tester.py) | → | ZephyrAlpha — shared/contracts/ (__init__.py) | config_depends / config_depends |
| 36 | D_SHARED 共享服务: Backward-compat shim — canonical location is z... | → | Backward-compat shim — canonical location is z... | config_depends / config_depends |
| 37 | D_SHARED 共享服务: Backward-compat shim — canonical location is z... | → | Backward-compat shim — canonical location is z... | config_depends / config_depends |
| 38 | D_SHARED 共享服务: Backward-compat shim — canonical location is z... | → | Backward-compat shim — canonical location is z... | config_depends / config_depends |
...
| 41 | D_SHARED 共享服务: ext_001.py | → | Auto-generated contracts package — external (_... | config_depends / config_depends |
| 42 | D_SHARED 共享服务: ext_002.py | → | Auto-generated contracts package — external (_... | config_depends / config_depends |
| 43 | D_SHARED 共享服务: ext_003.py | → | Auto-generated contracts package — external (_... | config_depends / config_depends |
| 44 | D_SHARED 共享服务: ext_004.py | → | Auto-generated contracts package — external (_... | config_depends / config_depends |
| 45 | D_SHARED 共享服务: Re-export shim — 真源已收敛至 zephyr.shared.co... | → | performance_attribution_report.py | 导入依赖 / import_depends |
| 46 | D_SIMULATION 仿真: 实验 — Experimentation Pipeline Layer (pipelin... | → | experiment_result.py | 导入依赖 / import_depends |
| 47 | D_TRADING 交易运营: zephyr.trading.trading_contracts — trading-dom... | → | performance_attribution_report.py | 导入依赖 / import_depends |
| 48 | D_TRADING 交易运营: D_EXECUTION_CORE — BrokerInterface (broker_int... | → | fill.py | 导入依赖 / import_depends |
| 49 | D_TRADING 交易运营: D_EXECUTION_CORE — BrokerInterface (broker_int... | → | order.py | 导入依赖 / import_depends |
| 50 | D_TRADING 交易运营: D_EXECUTION_CORE — BrokerInterface (broker_int... | → | position.py | 导入依赖 / import_depends |
| 51 | D_TRADING 交易运营: __init__.py | → | performance_attribution_report.py | 导入依赖 / import_depends |
| 52 | D_TRADING 交易运营: Re-export shim — 真源已收敛至 zephyr.shared.co... | → | performance_attribution_report.py | 导入依赖 / import_depends |
| 53 | D_TRADING 交易运营: strategy_lifecycle_event.py | → | strategy_lifecycle_event.py | 导入依赖 / import_depends |
| 54 | D_TRADING 交易运营: risk_limit_violation_error.py | → | trace_context.py | 导入依赖 / import_depends |
| 55 | D_TRADING 交易运营: risk_validator_protocol.py | → | risk_limits.py | 导入依赖 / import_depends |
| 56 | D_TRADING 交易运营: test_l07_post_trade_analytics.py | → | performance_attribution_report.py | 测试依赖 / test_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 21 个外部域直接连接（出边 77 条 + 入边 56 条 = 133 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_GOVERNANCE -->|10条 导入依赖 / import_depends| D_INFRASTRUCTURE
