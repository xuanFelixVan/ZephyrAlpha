...
graph TD
    ...
    class D_GOVERNANCE,D_GOV_ENFORCEMENT,D_GOV_REPAIR external_design
```
...
graph TD
    ...
    class D_SHARED,D_FBL_VERIFICATION,D_FEEDBACK_LOOP,D_FBL_DETECTORS,D_GOV_CODE_QUALITY,D_GOV_DRIFT,D_AUTONOMY_CORE,D_INFRA_RUNTIME,D_GOV_OPS_RESILIENCE,D_SECURITY,D_GOVERNANCE,D_TRADING,D_INTEGRATION,D_ORCHESTRATOR external_prod
```
...
graph TD
    ...
    class D_SHARED,D_EX_CORE,D_RISK,D_INFRA_RUNTIME,D_SIMULATION,D_TRADING external_prod
...
graph TD
    ...
    class D_GOV_CODE_QUALITY,D_EX_CORE external_prod
    class D_SHARED,D_GOVERNANCE,D_GOV_ENFORCEMENT,D_GOV_REPAIR,D_REPORTING external_design
```
...
| 53 | Auto-generated contracts package — external (_... | → | D_SHARED 共享服务: ext_002.py | config_depends / config_depends |
| 54 | factor_signal.py | → | D_SHARED 共享服务: trace_context.py | 导入依赖 / import_depends |
| 55 | fill.py | → | D_SHARED 共享服务: trace_context.py | 导入依赖 / import_depends |
| 56 | __init__.py | → | D_SHARED 共享服务: agent_identity.py | 导入依赖 / import_depends |
| 57 | __init__.py | → | D_SHARED 共享服务: permission.py | 导入依赖 / import_depends |
...
| 64 | market_data.py | → | D_SHARED 共享服务: trace_context.py | 导入依赖 / import_depends |
| 65 | order.py | → | D_SHARED 共享服务: trace_context.py | 导入依赖 / import_depends |
| 66 | order.py | → | D_SHARED 共享服务: OrderSide/OrderStatus/OrderType — 交易枚举真源... | 导入依赖 / import_depends |
| 67 | shared.contracts.portfolio — auto-generated pa... | → | D_SHARED 共享服务: Backward-compat shim — canonical location is z... | 导入依赖 / import_depends |
| 68 | position.py | → | D_SHARED 共享服务: trace_context.py | 导入依赖 / import_depends |
...
| 73 | Backward-compat shim — canonical location is z... | → | D_SHARED 共享服务: Backward-compat shim — canonical location is z... | 导入依赖 / import_depends |
| 74 | risk_limits.py | → | D_SHARED 共享服务: trace_context.py | 导入依赖 / import_depends |
| 75 | __init__.py | → | D_SHARED 共享服务: security_decision.py | 导入依赖 / import_depends |
| 76 | synthesized_signal.py | → | D_SHARED 共享服务: trace_context.py | 导入依赖 / import_depends |
| 77 | test_abac_guard_root.py | → | D_SHARED 共享服务: agent_identity.py | 测试依赖 / test_depends |
| 78 | test_alerts_bridge.py | → | D_SHARED 共享服务: budget_alert.py | 测试依赖 / test_depends |
| 79 | test_rbac_guard_root.py | → | D_SHARED 共享服务: agent_identity.py | 测试依赖 / test_depends |
...
| 24 | D_GOV_REPAIR 治理修复: zephyr.trading.trading_contracts — trading-dom... | → | position.py | 导入依赖 / import_depends |
...
| 29 | D_PF_ALLOC 组合分配: D_PORTFOLIO_CORE — Default Equity Long-Only St... | → | order.py | 导入依赖 / import_depends |
...
| 33 | D_REPORTING 报告: D_REPORTING — Default TCA Engine (default_tca_... | → | execution_report.py | 导入依赖 / import_depends |
| 34 | D_REPORTING 报告: D_REPORTING — Default TCA Engine (default_tca_... | → | fill.py | 导入依赖 / import_depends |
| 35 | D_REPORTING 报告: D_REPORTING — Default TCA Engine (default_tca_... | → | order.py | 导入依赖 / import_depends |
| 36 | D_RISK 风控: D_RISK — Risk Limits Calculator (risk_limits.py) | → | risk_limits.py | 导入依赖 / import_depends |
| 37 | D_RISK 风控: ZephyrAlpha — D_RISK Risk Management Layer — ... | → | risk_limits.py | 导入依赖 / import_depends |
...
| 50 | D_TRADING 交易运营: risk_limit_violation_error.py | → | trace_context.py | 导入依赖 / import_depends |
| 51 | D_TRADING 交易运营: risk_validator_protocol.py | → | risk_limits.py | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 22 个外部域直接连接（出边 79 条 + 入边 52 条 = 131 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_INFRASTRUCTURE -->|57条 config_depends / config_depends, 导入依赖 / import_depends, 测试依赖 / test_depends| D_SHARED
    ...
    D_SHARED -->|11条 config_depends / config_depends, 导入依赖 / import_depends| D_INFRASTRUCTURE
