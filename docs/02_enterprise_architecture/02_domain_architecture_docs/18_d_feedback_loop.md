...
flowchart TD
    ...
    src_zephyr_feedback_loop_gen_inherited_py["(生产态 / production) 生成inherited / _gen_inherited<br/>生成inherited，提供包入口和模块加载功能<br/>文件: feedback_loop/_gen_inherited.py"]
    ...
    src_zephyr_feedback_loop_backpressure_bridge_py["(生产态 / production) 背压桥接 / backpressure_bridge<br/>FLE -> Pipeline 背压桥接（CTR-BP-001~003）<br/>文件: feedback_loop/backpressure_bridge.py"]
    ...
    src_zephyr_feedback_loop_db_bridge_py["(生产态 / production) 数据库桥接 / db_bridge<br/>FLE DB契约适配器 — 通过规范zephyr.governance.sqlite_schema连接写入fle_metrics<br/>文件: feedback_loop/db_bridge.py"]
    ...
    src_zephyr_feedback_loop_error_budget_py["(生产态 / production) 错误预算 / error_budget<br/>Error Budget 状态机——monthly budget + burn_rate + exhaust_policy。<br/>文件: feedback_loop/error_budget.py"]
    ...
    src_zephyr_feedback_loop_fitness_functions_py["(生产态 / production) 适应度functions / fitness_functions<br/>适应度functions，提供包入口和模块加载功能<br/>文件: feedback_loop/fitness_functions.py"]
    ...
    src_zephyr_feedback_loop_session_learner_py["(生产态 / production) 会话学习器 / session_learner<br/>在线学习<br/>文件: feedback_loop/session_learner.py"]
    src_zephyr_feedback_loop_slo_manager_py["(生产态 / production) SLO管理器 / slo_manager<br/>5.39.6: SLOManager 进程级单例（boot_hooks 启动时实例化）。<br/>文件: feedback_loop/slo_manager.py"]
    ...
    src_zephyr_feedback_loop_actors_incident_priority_triage_automator_py["(生产态 / production) incident优先级分诊automator / Incident Priority Triage Automator — v0.37.0 R463<br/>incident优先级triageautomator，执行者的核心类，封装Severity相关逻辑。<br/>文件: actors/incident_priority_triage_automator.py"]
    ...
    src_zephyr_feedback_loop_actors_owner_absence_escalation_py["(生产态 / production) ownerabsence升级 / Owner Absence Escalation — v0.37.0 R462<br/>所有者absenceescalation。Owner Absence Escalation — v0.37.0 R462<br/>文件: actors/owner_absence_escalation.py"]
    src_zephyr_feedback_loop_actors_saga_compensator_py["(生产态 / production) Saga补偿器 / Saga Compensator — v0.3.0 R19b<br/>Saga补偿器，执行者的补偿器，对失败操作做补偿。<br/>文件: actors/saga_compensator.py"]
    ...
    src_zephyr_feedback_loop_collectors_calendar_adapter_py["(生产态 / production) 日历适配器 / Calendar Adapter — v0.8.0 R102b<br/>calendar适配器。Calendar Adapter — v0.8.0 R102b<br/>文件: collectors/calendar_adapter.py"]
    ...
    src_zephyr_feedback_loop_collectors_financial_stratification_py["(生产态 / production) 金融分层 / Financial Stratification — v0.5.0 R50<br/>金融分层，采集器的核心类，封装FinancialStratification相关逻辑。<br/>文件: collectors/financial_stratification.py"]
    src_zephyr_feedback_loop_collectors_kb_provenance_py["(生产态 / production) 知识库溯源 / KB Provenance — v0.10.0 R136<br/>知识库溯源，采集器的核心类，封装KBProvenance相关逻辑。<br/>文件: collectors/kb_provenance.py"]
    ...
    src_zephyr_feedback_loop_collectors_knowledge_injection_py["(生产态 / production) 知识注入 / Knowledge Injection — v0.8.0 R102<br/>知识injection。Knowledge Injection — v0.8.0 R102<br/>文件: collectors/knowledge_injection.py"]
    ...
    src_zephyr_feedback_loop_collectors_market_calendar_py["(生产态 / production) 行情日历 / Market Calendar — v0.5.0 R48<br/>市场calendar。Market Calendar — v0.5.0 R48<br/>文件: collectors/market_calendar.py"]
    src_zephyr_feedback_loop_collectors_market_event_integrator_py["(生产态 / production) 行情事件integrator / Market Event Integrator — v0.14.0 R197<br/>市场事件integrator。Market Event Integrator — v0.14.0 R197<br/>文件: collectors/market_event_integrator.py"]
    ...
    src_zephyr_feedback_loop_db_writer_py["(生产态 / production) db写入器 / db_writer<br/>FLE 持久化写入器 — 写 metrics/alerts/dispatch_log 到 SQLite<br/>文件: feedback_loop/db_writer.py"]
    src_zephyr_feedback_loop_docs_cold_start_manual_py["(生产态 / production) 冷启动手册 / cold_start_manual<br/>冷启动手册（cold_start_manual.py）<br/>文件: docs/cold_start_manual.py"]
    ...
    src_zephyr_feedback_loop_evolution_ewc_kb_review_py["(生产态 / production) ewc知识库审查 / EWC KB Review — v0.6.0 R51<br/>ewckb审查。EWC KB Review — v0.6.0 R51<br/>文件: evolution/ewc_kb_review.py"]
    src_zephyr_feedback_loop_evolution_failure_replay_py["(生产态 / production) 故障replay / Failure Replay — v0.7.0 R77<br/>failure回放，进化的核心类，封装FailureReplay相关逻辑。<br/>文件: evolution/failure_replay.py"]
    ...
    src_zephyr_feedback_loop_evolution_hypernetwork_py["(生产态 / production) 超网络 / HyperNetwork — v0.7.0 R72<br/>超网络，进化的核心类，封装超网络相关逻辑。<br/>文件: evolution/hypernetwork.py"]
    ...
    src_zephyr_feedback_loop_evolution_online_feature_importance_py["(生产态 / production) online特征importance / Online Feature Importance — v0.7.0 R73<br/>online功能importance。Online Feature Importance — v0.7.0 R73<br/>文件: evolution/online_feature_importance.py"]
    ...
    src_zephyr_feedback_loop_evolution_prompt_optimization_regression_detector_py["(生产态 / production) 提示优化回归检测器 / R514: PromptOptimizationRegressionDetector<br/>提示optimization回归检测器。R514: PromptOptimizationRegressionDetector<br/>文件: evolution/prompt_optimization_regression_detector.py"]
    src_zephyr_feedback_loop_evolution_prompt_self_optimization_loop_py["(生产态 / production) 提示自优化循环 / R502: PromptSelfOptimizationLoop<br/>提示自optimizationloop。R502: PromptSelfOptimizationLoop<br/>文件: evolution/prompt_self_optimization_loop.py"]
    ...
    src_zephyr_feedback_loop_evolution_self_upgrade_canary_py["(生产态 / production) selfupgrade金丝雀 / Self Upgrade Canary — v0.14.0 R194<br/>自upgradecanary。Self Upgrade Canary — v0.14.0 R194<br/>文件: evolution/self_upgrade_canary.py"]
    ...
    src_zephyr_feedback_loop_evolution_teacher_transfer_py["(生产态 / production) 教师迁移 / Teacher Transfer — v0.6.0 R53<br/>教师迁移，进化的核心类，封装TeacherTransfer相关逻辑。<br/>文件: evolution/teacher_transfer.py"]
    src_zephyr_feedback_loop_evolution_training_data_gov_py["(生产态 / production) training数据治理 / Training Data Governance — v0.14.0 R191<br/>training数据gov。Training Data Governance — v0.14.0 R191<br/>文件: evolution/training_data_gov.py"]
    src_zephyr_feedback_loop_evolution_engine_py["(生产态 / production) 进化引擎 / evolution_engine<br/>进化引擎，依赖网关工作<br/>文件: feedback_loop/evolution_engine.py"]
    src_zephyr_feedback_loop_forensic_architectural_sod_py["(生产态 / production) 架构职责分离 / Architectural SoD — v0.15.0 R205<br/>架构职责分离，取证的核心类，封装SoDRole相关逻辑。<br/>文件: forensic/architectural_sod.py"]
    ...
    src_zephyr_feedback_loop_forensic_crypto_bootstrap_py["(生产态 / production) 加密自举 / Cryptographic Bootstrap — v0.15.0 R204<br/>加密自举，取证的核心类，封装HashLink相关逻辑。<br/>文件: forensic/crypto_bootstrap.py"]
    ...
    src_zephyr_feedback_loop_forensic_guard_configuration_drift_monitor_py["(生产态 / production) 守卫配置漂移监控 / R521: GuardConfigurationDriftMonitor<br/>守卫configuration漂移监控。R521: GuardConfigurationDriftMonitor<br/>文件: forensic/guard_configuration_drift_monitor.py"]
    src_zephyr_feedback_loop_forensic_interrupt_coherence_validator_py["(生产态 / production) interruptcoherence校验器 / R531: InterruptCoherenceValidator<br/>中断coherence校验器。R531: InterruptCoherenceValidator<br/>文件: forensic/interrupt_coherence_validator.py"]
    src_zephyr_feedback_loop_forensic_knowledge_injection_pre_flight_verifier_py["(生产态 / production) 知识注入preflight验证器 / R515: KnowledgeInjectionPreFlightVerifier<br/>知识injection预flight验证器。R515: KnowledgeInjectionPreFlightVerifier<br/>文件: forensic/knowledge_injection_pre_flight_verifier.py"]
    ...
    src_zephyr_feedback_loop_forensic_self_modification_audit_py["(生产态 / production) selfmodification审计 / Self-Modification Audit — v0.15.0 R218<br/>自modification审计。Self-Modification Audit — v0.15.0 R218<br/>文件: forensic/self_modification_audit.py"]
    ...
    src_zephyr_feedback_loop_forensic_sub_agent_collusion_py["(生产态 / production) sub代理collusion / Sub-Agent Collusion Detector — v0.15.0 R213<br/>子代理collusion。Sub-Agent Collusion Detector — v0.15.0 R213<br/>文件: forensic/sub_agent_collusion.py"]
    ...
    src_zephyr_feedback_loop_resilience_dr_automation_py["(生产态 / production) 灾备自动化 / DR Automation — v0.14.0 R187<br/>灾备自动化，韧性的结果，封装操作结果的数据结构。<br/>文件: resilience/dr_automation.py"]
    ...
    src_zephyr_feedback_loop_resilience_resource_starvation_aware_py["(生产态 / production) 资源starvation感知 / Resource Starvation Aware — v0.15.0 R209<br/>resourcestarvation感知，韧性的核心类，封装ResourceBudget相关逻辑。<br/>文件: resilience/resource_starvation_aware.py"]
    ...
    src_zephyr_feedback_loop_scheduler_collect_detect_py["(生产态 / production) 调度器collectdetect / scheduler_collect_detect<br/>调度器collect检测，主要提供运行collect、运行检测、运行diagnose等功能，供调度器使用<br/>文件: feedback_loop/scheduler_collect_detect.py"]
    ...
    src_zephyr_feedback_loop_security_dep_cve_correlator_py["(生产态 / production) 依赖CVE关联器 / Dependency CVE Correlator — v0.14.0 R196<br/>依赖CVE关联器，安全的关联器，关联匹配相关数据。<br/>文件: security/dep_cve_correlator.py"]
    ...
    src_zephyr_feedback_loop_alert_dispatcher_py["(生产态 / production) alert分发器 / alert_dispatcher<br/>FLE->Orc 告警分派器 — dispatch() 生产者<br/>文件: feedback_loop/alert_dispatcher.py"]
    ...
    src_zephyr_feedback_loop_evolution_self_modification_rate_limiter_py["(生产态 / production) selfmodification速率限制器 / R522: SelfModificationRateLimiter<br/>自modification率限制器。R522: SelfModificationRateLimiter<br/>文件: evolution/self_modification_rate_limiter.py"]
    src_zephyr_feedback_loop_forensic_boot_integrity_attestation_py["(生产态 / production) boot完整性attestation / Boot Integrity Attestation — v0.38.0 R487<br/>启动完整性attestation。Boot Integrity Attestation — v0.38.0 R487<br/>文件: forensic/boot_integrity_attestation.py"]
    ...
    src_zephyr_feedback_loop_resilience_graceful_degradation_planner_py["(生产态 / production) gracefuldegradation规划器 / Graceful Degradation Planner — v0.40.0 R496<br/>graceful退化planner。Graceful Degradation Planner — v0.40.0 R496<br/>文件: resilience/graceful_degradation_planner.py"]
    src_zephyr_feedback_loop_resilience_oscillation_damping_py["(生产态 / production) 振荡阻尼 / Oscillation Damping — v0.37.0 R450<br/>振荡阻尼，韧性的状态机，管理状态流转。<br/>文件: resilience/oscillation_damping.py"]
    src_zephyr_feedback_loop_resilience_self_api_throttle_defense_py["(生产态 / production) 自API限流器防御 / Self API Throttle Defense — v0.39.0 R491<br/>自APIthrottle防御。Self API Throttle Defense — v0.39.0 R491<br/>文件: resilience/self_api_throttle_defense.py"]
    src_zephyr_feedback_loop_security_wireheading_prevention_py["(生产态 / production) 神经劫持防护 / Wireheading Prevention — v0.37.0 R486<br/>神经劫持防护，安全的状态机，管理状态流转。<br/>文件: security/wireheading_prevention.py"]
    ...
    src_zephyr_feedback_loop_actors_alert_router_py["(生产态 / production) 告警路由器 / alert_router.py — Severity-based alert channel router.<br/>告警路由器，执行者的路由器，按规则分发请求到处理方。<br/>文件: actors/alert_router.py"]
    ...
    class D_SHARED,D_GOV_OPS_RESILIENCE,D_FBL_DIAGNOSERS,D_FBL_VERIFICATION,D_GOVERNANCE,D_INFRA_RUNTIME,D_GOV_DRIFT,D_INFRA_RECOVERY,D_FRONTEND,D_FBL_DETECTORS,D_AUTONOMY_CORE,D_ORCHESTRATOR,D_GOV_AUDIT,D_SECURITY external_prod
```
