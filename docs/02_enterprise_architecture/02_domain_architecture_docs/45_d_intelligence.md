...
graph TD
    ...
    class D_INFRA_RUNTIME,D_GOV_RULE,D_FBL_VERIFICATION,D_GOV_DRIFT,D_GOVERNANCE,D_INTEGRATION external_prod
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_OPS,D_SHARED,D_FEEDBACK_LOOP,D_GOV_OPS_RESILIENCE,D_GOV_DRIFT external_prod
```
...
graph TD
    ...
    class D_GOVERNANCE,D_FEEDBACK_LOOP,D_SIGLEGACY,D_INFRA_RUNTIME,D_AUTONOMY_CORE,D_INTEGRATION external_prod
```
...
graph TD
    ...
    class D_AUTONOMY_CORE,D_GOVERNANCE,D_INTEGRATION,D_BACKTEST,D_SHARED,D_GOV_RULE,D_FBL_VERIFICATION,D_INFRA_RUNTIME external_prod
...
| 12 | test_decision_provenance.py | → | D_FEEDBACK_LOOP 反馈循环引擎: Decision Provenance — v0.12.0 R166 (decision_p... | 测试依赖 / test_depends |
| 13 | test_model_health.py | → | D_FEEDBACK_LOOP 反馈循环引擎: Model Health Monitor — v0.5.0 R40 (model_healt... | 测试依赖 / test_depends |
| 14 | test_model_rotation.py | → | D_FEEDBACK_LOOP 反馈循环引擎: Model Rotation — v0.9.0 R125 (model_rotation.py) | 测试依赖 / test_depends |
| 15 | test_model_rotation_v2.py | → | D_FEEDBACK_LOOP 反馈循环引擎: Model Rotation v2 — v0.10.0 R140 (model_rotati... | 测试依赖 / test_depends |
| 16 | test_model_version_semantic_drift.py | → | D_FEEDBACK_LOOP 反馈循环引擎: Model Version Semantic Drift Monitor — v0.39.0... | 测试依赖 / test_depends |
| 17 | test_integration_test_pipeline.py | → | D_FEEDBACK_LOOP 反馈循环引擎: E2E Integration Test Pipeline — TASK-MOD-FEEDB... | 测试依赖 / test_depends |
| 18 | KB->VMS 同步引擎 — sync_to_vms() 生产者 (sync_... | → | D_GOVERNANCE 生命周期管理: SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 19 | test_budget_enforcer_rbac_bridge.py | → | D_GOVERNANCE 生命周期管理: G-CT-007 契约：Budget -> RBAC 配额限制. (rbac_b... | 测试依赖 / test_depends |
| 20 | test_model_version_detector.py | → | D_GOVERNANCE 生命周期管理: Model Version Detector — v0.10.0 模型版本突变.... | 测试依赖 / test_depends |
...
| 55 | D_ML_TRAIN — ML Training Domain (__init__.py) | → | D_ML_TRAIN 训练: D_ML_TRAIN — ML Training Base (trainer_base.py) | config_depends / config_depends |
...
graph LR
    ...
    D_INTELLIGENCE -->|9条 测试依赖 / test_depends| D_FEEDBACK_LOOP
