...
graph TD
    ...
    class D_GOVERNANCE,D_SHARED,D_INTEGRATION,D_AUTONOMY_CORE,D_GOV_OPS_RESILIENCE external_prod
    class D_INFRA_RUNTIME,D_TRADING,D_GOV_SCRIPTS,D_FEEDBACK_LOOP external_design
```
...
graph TD
    ...
    class D_GOV_OPS_RESILIENCE,D_FEEDBACK_LOOP external_prod
    class D_SHARED,D_GOV_DRIFT,D_GOV_SCRIPTS external_design
```
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 14 个，2 条域内依赖）。
...
graph TD
    ...
    class D_AUTONOMY_CORE,D_INTEGRATION,D_GOVERNANCE external_prod
    class D_SHARED,D_INFRA_RUNTIME,D_FEEDBACK_LOOP,D_TRADING external_design
```
