...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_DRIFT,D_FEEDBACK_LOOP,D_SECURITY,D_GOV_SCRIPTS,D_TRADING,D_INFRA_RUNTIME,D_AUTONOMY_CORE external_prod
```
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_DRIFT,D_FEEDBACK_LOOP,D_GOV_SCRIPTS,D_TRADING,D_INFRA_RUNTIME,D_AUTONOMY_CORE external_prod
```
...
> 本域与 11 个外部域直接连接（出边 55 条 + 入边 7 条 = 62 条）。只显示直接连接的域，不展开具体节点。
