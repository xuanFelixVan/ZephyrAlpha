...
flowchart TD
    src_zephyr_signal_fundamental_init_py["基本面信号域包<br/>zephyr/signal_fundamental<br/>包入口，管理zephyr.signal_fundamental子包加载<br/>Fundamental Signal Domain Package<br/>Unified package for D_FUNDAMENTAL_SIGNAL<br/>domain, aggregating signal generation,<br/>strategy, synthesis, combiner, capital<br/>allocation and pipeline; lazily re-exports<br/>submodule main classes via __getattr__<br/>文件: signal_fundamental/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_signal_fundamental_capital_capital_allocation_result_py["资本分配结果（兼容导出）<br/>资本分配结果的兼容导出垫片。老代码从这导入不会断<br/>，真源在别处，这层只是转发，避免改一处坏一片。<br/>Capital Allocation Result (compat re-export)<br/>Compat re-export shim for<br/>CapitalAllocationResult, keeping old import<br/>paths working<br/>文件: capital/capital_allocation_result.py<br/>(生产态 / production)"]
    ...
    src_zephyr_signal_fundamental_capital_default_capital_allocator_py["默认资本分配器（兼容导出）<br/>默认资本分配器的兼容导出垫片，真源在 strategy<br/>目录，这层只转发保持老路径不断。<br/>Default Capital Allocator (compat re-export)<br/>Compat re-export shim for default capital<br/>allocator, real source in strategy dir<br/>文件: capital/default_capital_allocator.py<br/>(生产态 / production)"]
    ...
    src_zephyr_signal_fundamental_router_signal_priority_router_py["信号优先级路由器<br/>给信号排优先级的路由器。多个信号同时来时，按优先<br/>级决定先处理谁后处理谁，避免重要信号被淹没。<br/>⛔ 基本面信号域，设计已就绪，等待开发排期<br/>Signal Priority Router<br/>文件: router/signal_priority_router.py<br/>(设计态 / design)"]
    ...
    src_zephyr_signal_fundamental_router_signal_conflict_resolver_py["信号冲突解决器<br/>解决信号冲突的裁判。多个信号互相矛盾<br/>（一个说买一个说卖）时，裁定听谁的，避免信号打架<br/>系统无所适从。<br/>⛔ 基本面信号域，设计已就绪，等待开发排期<br/>Signal Conflict Resolver<br/>文件: router/signal_conflict_resolver.py<br/>(设计态 / design)"]
    ...
    src_zephyr_signal_fundamental_pipeline_py -->|data / data| D_INFRA_RUNTIME
    ...
    class D_INFRA_RUNTIME,D_INFRASTRUCTURE,D_TRADING,D_SHARED,D_FACTOR,D_GOVERNANCE external_prod
...
| 12 | 管线 / Alpha Signal Pipeline (signal_fundamental/pipeline... | → | D_INFRA_RUNTIME 运行时集成: Redis热数据缓存 (infrastructure/h1_redis_hot) | data / data |
| 13 | 管线 / Alpha Signal Pipeline (signal_fundamental/pipeline... | → | D_SHARED 共享服务: 错误 / errors (foundation/errors.py) | 导入依赖 / import_depends |
| 14 | 管线 / Alpha Signal Pipeline (signal_fundamental/pipeline... | → | D_SHARED 共享服务: 时间工具 / time_utils (utils/time_utils.py) | 导入依赖 / import_depends |
| 15 | 资本分配结果（兼容导出） / Capital Allocation Result (com... | → | D_TRADING 交易运营: 资本分配结果 / capital_allocation_result (execution/capit... | 导入依赖 / import_depends |
| 16 | 信号生成聚合基类 / Signal Generation Aggregator Base (gen... | → | D_TRADING 交易运营: 资本分配结果 / capital_allocation_result (execution/capit... | 导入依赖 / import_depends |
| 17 | 信号生成聚合基类 / Signal Generation Aggregator Base (gen... | → | D_TRADING 交易运营: 信号退化警告 / signal_degradation_warning (market/signal_... | 导入依赖 / import_depends |
| 18 | 策略资本分配器 / Strategy Capital Allocator (strategy/cap... | → | D_TRADING 交易运营: 资本分配结果 / capital_allocation_result (execution/capit... | 导入依赖 / import_depends |
| 19 | 策略默认资本分配器 / Strategy Default Capital Allocator (... | → | D_TRADING 交易运营: 资本分配结果 / capital_allocation_result (execution/capit... | 导入依赖 / import_depends |
...
> 本域与 7 个外部域直接连接（出边 19 条 + 入边 2 条 = 21 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_FUNDAMENTAL_SIGNAL -->|1条 data / data| D_INFRA_RUNTIME
