...
flowchart TD
    src_zephyr_signal_fundamental_init_py["zephyr/signal_fundamental 包入口<br/>D_SIGNAL Signal Domain<br/>文件: signal_fundamental/__init__.py<br/>(生产态 / production)"]
    src_zephyr_signal_fundamental_audit_signal_audit_logger_py["audit/signal_audit_logger<br/>审计包的signal_audit_logger模块<br/>⛔ 基本面信号域，设计已就绪，等待开发排期<br/>文件: audit/signal_audit_logger.py<br/>(设计态 / design)"]
    src_zephyr_signal_fundamental_capital_capital_allocation_result_py["capital/capital_allocation_result<br/>D_FUNDAMENTAL_SIGNAL — CapitalAllocationResult<br/>re-export shim<br/>文件: capital/capital_allocation_result.py<br/>(生产态 / production)"]
    src_zephyr_signal_fundamental_capital_capital_allocator_py["capital/capital_allocator<br/>D_SIGNAL — Capital Allocator（兼容 re-export<br/>shim）<br/>文件: capital/capital_allocator.py<br/>(生产态 / production)"]
    src_zephyr_signal_fundamental_capital_default_capital_allocator_py["capital/default_capital_allocator<br/>D_SIGNAL — Default Capital Allocator（兼容<br/>re-export shim）<br/>文件: capital/default_capital_allocator.py<br/>(生产态 / production)"]
    src_zephyr_signal_fundamental_gen_implementations_default_signal_aggregator_py["implementations/default_signal_aggregator<br/>D_SIGNAL — Default Signal Aggregator<br/>文件: implementations<br/>/default_signal_aggregator.py<br/>(生产态 / production)"]
    src_zephyr_signal_fundamental_pipeline_py["signal_fundamental/pipeline<br/>AlphaSignalPipeline<br/>D_FACTOR->D_SIGNAL跨层集成管道<br/>文件: signal_fundamental/pipeline.py<br/>(生产态 / production)"]
    src_zephyr_signal_fundamental_router_signal_priority_router_py["router/signal_priority_router<br/>router包的signal_priority_router模块<br/>⛔ 基本面信号域，设计已就绪，等待开发排期<br/>文件: router/signal_priority_router.py<br/>(设计态 / design)"]
    ...
    src_zephyr_signal_fundamental_router_signal_conflict_resolver_py["router/signal_conflict_resolver<br/>router包的signal_conflict_resolver模块<br/>⛔ 基本面信号域，设计已就绪，等待开发排期<br/>文件: router/signal_conflict_resolver.py<br/>(设计态 / design)"]
    src_zephyr_signal_fundamental_strategy_capital_allocator_py["strategy/capital_allocator<br/>D_FUNDAMENTAL_SIGNAL — Capital Allocator<br/>（兼容导出）<br/>文件: strategy/capital_allocator.py<br/>(生产态 / production)"]
    src_zephyr_signal_fundamental_strategy_implementations_default_capital_allocator_py["implementations/default_capital_allocator<br/>D_SIGNAL — Default Capital Allocator<br/>文件: implementations<br/>/default_capital_allocator.py<br/>(生产态 / production)"]
    src_zephyr_signal_fundamental_synth_signal_synthesizer_py["synth/signal_synthesizer<br/>D_SIGNAL — Signal Synthesizer<br/>文件: synth/signal_synthesizer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_signal_fundamental_gen_aggregator_base_py["gen/aggregator_base<br/>D_SIGNAL — Signal Generation Layer<br/>文件: gen/aggregator_base.py<br/>(生产态 / production)"]
    ...
    class D_SHARED,D_INFRASTRUCTURE,D_TRADING,D_FACTOR,D_GOVERNANCE external_prod
...
|:--:|---------|:--:|---------|---------|
| 1 | router/signal_conflict_resolver.py | → | D_ASHARE_SIGNAL A股特色信号: signal_ashare/institutional_behavior_analyzer.py | event / event |
| 2 | AlphaSignalPipeline D_FACTOR->D_SIGNAL跨层集成管道 (signa... | → | D_FACTOR 因子: ZephyrAlpha — D_FACTOR Alpha Factor Layer (factor/factor... | 导入依赖 / import_depends |
| 3 | D_SIGNAL — Signal Generation Layer (gen/aggregator_base.py) | → | D_INFRASTRUCTURE 跨层契约基础设施: contracts/factor_signal.py | 导入依赖 / import_depends |
| 4 | D_SIGNAL — Signal Generation Layer (gen/aggregator_base.py) | → | D_INFRASTRUCTURE 跨层契约基础设施: contracts/synthesized_signal.py | 导入依赖 / import_depends |
| 5 | D_SIGNAL — Default Signal Aggregator (implementations/de... | → | D_INFRASTRUCTURE 跨层契约基础设施: contracts/factor_signal.py | 导入依赖 / import_depends |
| 6 | D_SIGNAL — Default Signal Aggregator (implementations/de... | → | D_INFRASTRUCTURE 跨层契约基础设施: contracts/synthesized_signal.py | 导入依赖 / import_depends |
| 7 | AlphaSignalPipeline D_FACTOR->D_SIGNAL跨层集成管道 (signa... | → | D_INFRASTRUCTURE 跨层契约基础设施: contracts/factor_signal.py | 导入依赖 / import_depends |
| 8 | AlphaSignalPipeline D_FACTOR->D_SIGNAL跨层集成管道 (signa... | → | D_INFRASTRUCTURE 跨层契约基础设施: contracts/synthesized_signal.py | 导入依赖 / import_depends |
| 9 | D_SIGNAL — Default Capital Allocator (implementations/de... | → | D_INFRASTRUCTURE 跨层契约基础设施: contracts/synthesized_signal.py | 导入依赖 / import_depends |
| 10 | D_SIGNAL — Signal Synthesizer (synth/signal_synthesizer.py) | → | D_INFRASTRUCTURE 跨层契约基础设施: contracts/factor_signal.py | 导入依赖 / import_depends |
| 11 | D_SIGNAL — Signal Synthesizer (synth/signal_synthesizer.py) | → | D_INFRASTRUCTURE 跨层契约基础设施: contracts/synthesized_signal.py | 导入依赖 / import_depends |
| 12 | AlphaSignalPipeline D_FACTOR->D_SIGNAL跨层集成管道 (signa... | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Traditional Exce... | 导入依赖 / import_depends |
| 13 | AlphaSignalPipeline D_FACTOR->D_SIGNAL跨层集成管道 (signa... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 | 盲点 B19... | 导入依赖 / import_depends |
| 14 | D_FUNDAMENTAL_SIGNAL — CapitalAllocationResult re-export... | → | D_TRADING 交易运营: execution/capital_allocation_result.py | 导入依赖 / import_depends |
| 15 | D_SIGNAL — Signal Generation Layer (gen/aggregator_base.py) | → | D_TRADING 交易运营: execution/capital_allocation_result.py | 导入依赖 / import_depends |
| 16 | D_SIGNAL — Signal Generation Layer (gen/aggregator_base.py) | → | D_TRADING 交易运营: market/signal_degradation_warning.py | 导入依赖 / import_depends |
| 17 | D_FUNDAMENTAL_SIGNAL — Capital Allocator（兼容导出） (st... | → | D_TRADING 交易运营: execution/capital_allocation_result.py | 导入依赖 / import_depends |
| 18 | D_SIGNAL — Default Capital Allocator (implementations/de... | → | D_TRADING 交易运营: execution/capital_allocation_result.py | 导入依赖 / import_depends |
