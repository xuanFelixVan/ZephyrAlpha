...
flowchart TD
    ...
    src_zephyr_signal_fundamental_audit_signal_audit_logger_py["(设计态 / design) 信号审计日志器 / Signal Audit Logger<br/>给信号留操作录像的记账员。从因子变信号、信号变资金分配，每一步关键操作都记一笔，事后出问题能倒查是哪一步、哪个信号出了岔。<br/>文件: audit/signal_audit_logger.py<br/>⛔ 基本面信号域，设计已就绪，等待开发排期"]
    ...
    src_zephyr_signal_fundamental_router_signal_priority_router_py["(设计态 / design) 信号优先级路由器 / Signal Priority Router<br/>给信号排优先级的路由器。多个信号同时来时，按优先级决定先处理谁后处理谁，避免重要信号被淹没。<br/>文件: router/signal_priority_router.py<br/>⛔ 基本面信号域，设计已就绪，等待开发排期"]
    ...
    src_zephyr_signal_fundamental_router_signal_conflict_resolver_py["(设计态 / design) 信号冲突解决器 / Signal Conflict Resolver<br/>解决信号冲突的裁判。多个信号互相矛盾（一个说买一个说卖）时，裁定听谁的，避免信号打架系统无所适从。<br/>文件: router/signal_conflict_resolver.py<br/>⛔ 基本面信号域，设计已就绪，等待开发排期"]
