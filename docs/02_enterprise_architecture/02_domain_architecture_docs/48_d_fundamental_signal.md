...
flowchart TD
    src_zephyr_signal_fundamental_init_py["基本面信号域包<br/>zephyr/signal_fundamental 包入口，管理zephyr<br/>/signal_fundamental层子模块的统一加载和懒导入<br/>Fundamental Signal Domain Package<br/>Unified package for D_FUNDAMENTAL_SIGNAL<br/>domain, aggregating signal generation,<br/>strategy, synthesis, combiner, capital<br/>allocation and pipeline; lazily re-exports<br/>submodule main classes via __getattr__<br/>文件: signal_fundamental/__init__.py<br/>(生产态 / production)"]
