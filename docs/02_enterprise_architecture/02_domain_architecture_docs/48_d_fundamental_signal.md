...
flowchart TD
    ...
    src_zephyr_signal_fundamental_capital_capital_allocation_result_py["资本分配结果（兼容导出）<br/>资本分配结果的兼容导出垫片。老代码从这导入不会断<br/>，真源在别处，这层只是转发，避免改一处坏一片<br/>Capital Allocation Result (compat re-export)<br/>文件: capital/capital_allocation_result.py<br/>(生产态 / production)"]
    ...
    src_zephyr_signal_fundamental_capital_default_capital_allocator_py["默认资本分配器（兼容导出）<br/>默认资本分配器的兼容导出垫片，真源在 strategy<br/>目录，这层只转发保持老路径不断<br/>Default Capital Allocator (compat re-export)<br/>文件: capital/default_capital_allocator.py<br/>(生产态 / production)"]
