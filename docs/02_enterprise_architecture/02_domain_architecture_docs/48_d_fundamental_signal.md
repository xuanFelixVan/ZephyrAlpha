...
> 展示全部 13 个模块（生产态 11 + 设计态 2），含跨域依赖外部节点。节点含成熟度+名称+大白话/简介+文件路径。
