...
| 27 |  | → | D_GOVERNANCE 生命周期管理: local_first_arch.py | runtime / runtime |
...
| 33 | trade_panel · 实盘交易面板组件（v3.0.0 Panel+H... | → | D_TRADING 交易运营: order.py | 导入依赖 / import_depends |
...
> 本域与 9 个外部域直接连接（出边 33 条 + 入边 8 条 = 41 条）。只显示直接连接的域，不展开具体节点。
