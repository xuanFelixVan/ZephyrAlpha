...
flowchart TD
    ...
    src_zephyr_pf_alloc_core_signal_synthesis_combiner_py["(生产态 / production) Signal Synthesis Combiner<br/>— 信号合成器 (MOD-PA-002)<br/>文件: core/signal_synthesis_combiner.py"]
    ...
    src_zephyr_pf_alloc_core_signal_synthesis_combiner_py ~~~ src_zephyr_pf_alloc_multi_strategy_capital_allocator_py
    ...
    class src_zephyr_pf_alloc_core_signal_synthesis_combiner_py,src_zephyr_pf_alloc_strategy_lifecycle_event_py,src_zephyr_pf_core_default_equity_strategy_py production
...
flowchart TD
    ...
    src_zephyr_pf_alloc_strategy_lifecycle_event_py["(生产态 / production) 策略生命周期事件 /<br/>strategy_lifecycle_event<br/>策略生命周期事件，pf_<br/>alloc的事件，定义和分发事件。<br/>文件: pf_alloc/strategy_lifecycle_event.py"]
    src_zephyr_pf_core_default_equity_strategy_py["(生产态 / production) 默认权益策略 / D_<br/>PORTFOLIO_CORE — Default Equity Long-Only<br/>Strategy<br/>默认权益策略。D_PORTFOLIO_CORE — Default Equity<br/>Long-Only Strategy<br/>文件: pf_core/default_equity_strategy.py"]
    src_zephyr_pf_alloc_core_signal_synthesis_combiner_py ~~~ src_zephyr_pf_alloc_strategy_lifecycle_event_py
    src_zephyr_pf_alloc_strategy_lifecycle_event_py ~~~ src_zephyr_pf_core_default_equity_strategy_py
    classDef production fill:#e1f5fe,stroke:#01579b,stroke-width:2px,color:#000
    classDef design fill:#fff3e0,stroke:#e65100,stroke-width:2px,color:#000,stroke-dasharray: 5 5
    classDef external_prod fill:#e8f4fd,stroke:#0277bd,stroke-width:1px,color:#000
...
| 5 | 默认权益策略 / D_PORTFOLIO_CORE — Default Equity Long-On... | → | D_SHARED 共享服务: 订单枚举 / order_enums (enums/order_enums.py) | 导入依赖 / import_depends |
