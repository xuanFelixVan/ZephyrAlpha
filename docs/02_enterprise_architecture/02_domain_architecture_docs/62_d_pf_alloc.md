...
flowchart TD
    src_zephyr_pf_alloc_strategy_lifecycle_event_py["(生产态 / production) 策略生命周期事件 / Strategy Lifecycle Event<br/>Re-export from shared SSoT — zephyr.shared.contracts.strategy_lifecycle_event<br/>文件: pf_alloc/strategy_lifecycle_event.py"]
    src_zephyr_pf_core_default_equity_strategy_py["(生产态 / production) defaultequity策略 / Default Equity Strategy<br/>D_PORTFOLIO_CORE — Default Equity Long-Only Strategy<br/>文件: pf_core/default_equity_strategy.py"]
...
|:--:|---------|:--:|---------|---------|
| 1 | defaultequity策略 / Default Equity Strategy (pf_core/defa... | → | D_GOVERNANCE 生命周期管理: 策略基础 / Strategy Base (strategies/strategy_base.py) | 导入依赖 / import_depends |
...
| 3 | defaultequity策略 / Default Equity Strategy (pf_core/defa... | → | D_INFRASTRUCTURE 跨层契约基础设施: order / Order (contracts/order.py) | 导入依赖 / import_depends |
| 4 | defaultequity策略 / Default Equity Strategy (pf_core/defa... | → | D_SHARED 共享服务: orderenums / Order Enums (enums/order_enums.py) | 导入依赖 / import_depends |

### 依赖本域的其他域（入边）/ Depended By

| # | 外部域-源模块 / Source Module | → | 本域模块 / Target Module | 依赖类型 / Type |
|:--:|---------|:--:|---------|---------|
| 1 | D_PF_CORE 组合核心: 组合核心Strategy Engine包 / Pf Core Strategy Engine Packa... | → | defaultequity策略 / Default Equity Strategy (pf_core/defa... | 导入依赖 / import_depends |
