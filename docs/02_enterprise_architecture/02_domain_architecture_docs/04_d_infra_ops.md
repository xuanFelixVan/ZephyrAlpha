...
# 04_d_infra_ops / 基础设施运维域 / Asset Inventory
...
> **[可缩放 HTML 版 / Zoomable HTML](http://localhost:8765/docs/02_enterprise_architecture/02_domain_architecture_docs/_zoomable_html/04_d_infra_ops.html)** — Ctrl+滚轮缩放 ｜ 双击重置 ｜ Ctrl+Shift+D 切换拖动/选择模式
...
flowchart TD
    scripts_ops_cleanup_runtime_tmp_residue_py["ops/cleanup_runtime_tmp_residue<br/>cleanup_runtime_tmp_residue.py — 一次性清理<br/>.runtime/tmp/ 下测试残留目录。<br/>文件: ops/cleanup_runtime_tmp_residue.py<br/>(生产态 / production)"]
...
|:--:|---------|:--:|---------|---------|
| 1 | download_models.py — ARCH-MODEL-LIFECYCLE-001 Phase 3 (o... | → | D_DATA 数据接入层: 7×24 CH 健康探针守护进程（R4b，#ARCH-DR-CH-RESTART-001）... | config_depends / config_depends |
| 2 | cleanup_runtime_tmp_residue.py — 一次性清理 .runtime/tmp... | → | D_GOV_AUDIT 审计追踪: 对账注册表 / reconciliation_registry (audit/reconciliatio... | 导入依赖 / import_depends |
