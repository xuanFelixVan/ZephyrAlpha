...
graph TD
    ...
    D_GOVERNANCE -.->|runtime / runtime| src_zephyr_security_access_control_dry_run_py
    D_GOV_DRIFT["(设计态 / design) D_GOV_DRIFT"]
    D_GOV_DRIFT -.->|runtime / runtime| src_zephyr_security_access_control_dry_run_py
    D_AUDITTEST["(原型态 / prototype) D_AUDITTEST"]
    D_AUDITTEST -.->|contract / contract| src_zephyr_security_access_control_dry_run_py
    D_GOVERNANCE -.->|contract / contract| src_zephyr_security_access_control_dry_run_py
    D_AUTONOMY_CORE["(原型态 / prototype) D_AUTONOMY_CORE"]
    D_AUTONOMY_CORE -.->|runtime / runtime| src_zephyr_security_access_control_dry_run_py
...
| 128 | D_AUDITTEST 审计测试套件: test_auto_bootstrap.py | → | DryRun — 权限模拟与影响分析. (dry_run.py) | contract / contract |
...
| 170 | D_AUTONOMY_CORE 自治核心: file_autoregister.py | → | DryRun — 权限模拟与影响分析. (dry_run.py) | runtime / runtime |
...
| 185 | D_GOVERNANCE 生命周期管理: blueprint.md | → | DryRun — 权限模拟与影响分析. (dry_run.py) | contract / contract |
| 186 | D_GOVERNANCE 生命周期管理: blueprint.md | → | DryRun — 权限模拟与影响分析. (dry_run.py) | runtime / runtime |
...
| 195 | D_GOV_DRIFT 漂移检测: blueprint.md | → | DryRun — 权限模拟与影响分析. (dry_run.py) | runtime / runtime |
