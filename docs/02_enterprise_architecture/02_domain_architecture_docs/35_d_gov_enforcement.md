...
| 域内依赖 | 227 | Internal Dependencies | 227 |
...
graph TD
    ...
    D_AUDITTEST -.->|runtime| src_zephyr_compliance_audit_trail_bridges_init_py
    ...
    class D_GOVERNANCE,D_GOV_DRIFT,D_AUTONOMY_CORE,D_AUDITTEST,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_INTEGRATION,D_INFRA_RECOVERY,D_TRADING,D_SECURITY,D_PF_CORE external_prod
...
graph TD
    ...
    class D_SHARED,D_INFRA_RECOVERY,D_GOVERNANCE,D_INTEGRATION,D_SECURITY external_prod
...
|------|:---:|---------|
| D_AUDITTEST | 222 | runtime,test_depends |
...
> 域内模块依赖关系（共 227 条 / 227 edges）。按依赖类型分组，使用 → 表示方向。

```

┌──────────────────────────────────────────────────────────────────┐
│      依赖关系图 / Dependency Graph (共 227 条 / 227 edges)       │
├──────────────────────────────────────────────────────────────────┤
│   依赖类型数 / Dependency Types: 2                               │
│   [import_depends]: 139 条 / edges                               │
│   [config_depends]: 88 条 / edges                                │
└──────────────────────────────────────────────────────────────────┘
...
**[config_depends]** (88 条 / edges) — 已达显示上限，省略 / limit reached

> (最多显示前 50 条依赖边，共 227 条)
