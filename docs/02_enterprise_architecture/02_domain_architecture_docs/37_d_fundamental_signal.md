...
|:--:|---------|---------|:---:|:---:|
| 1 | src/zephyr/signal_fundamental/capital/capital_allocation_... | D_FUNDAMENTAL_SIGNAL — CapitalAllocationResult... | 原型态 / prototype | [MOD-L03-001](../../03_modules/_domain_signal/blueprint.md) |
...
| 4 | src/zephyr/signal_fundamental/gen/aggregator_base.py | aggregator_base.py | 原型态 / prototype | [MOD-L03-001](../../03_modules/_domain_signal/blueprint.md) |
| 5 | src/zephyr/signal_fundamental/gen/implementations/default... | default_signal_aggregator.py | 生产态 / production | [MOD-L03-001](../../03_modules/_domain_signal/blueprint.md) |
| 6 | src/zephyr/signal_fundamental/pipeline.py | pipeline.py | 生产态 / production | [MOD-L03-001](../../03_modules/_domain_signal/blueprint.md) |
| 7 | src/zephyr/signal_fundamental/strategy/capital_allocator.py | D_FUNDAMENTAL_SIGNAL — Capital Allocator（兼容... | 原型态 / prototype | [MOD-L03-001](../../03_modules/_domain_signal/blueprint.md) |
| 8 | src/zephyr/signal_fundamental/strategy/implementations/de... | default_capital_allocator.py | 生产态 / production | [MOD-L03-001](../../03_modules/_domain_signal/blueprint.md) |
| 9 | src/zephyr/signal_fundamental/synth/signal_synthesizer.py | signal_synthesizer.py | 生产态 / production | [MOD-L03-001](../../03_modules/_domain_signal/blueprint.md) |
...
> 展示全部 9 个模块（生产态 4 + 设计态 0 + 原型态 5），标签标注成熟度。
...
graph TD
    subgraph D_FUNDAMENTAL_SIGNAL["D_FUNDAMENTAL_SIGNAL 基本面信号"]
        ...
        src_zephyr_signal_fundamental_gen_aggregator_base_py["(原型态 / prototype) aggregator_base.py"]
        src_zephyr_signal_fundamental_gen_implementations_default_signal_aggregator_py["(生产态 / production) default_signal_aggregator.py"]
        src_zephyr_signal_fundamental_pipeline_py["(生产态 / production) pipeline.py"]
        ...
        src_zephyr_signal_fundamental_strategy_implementations_default_capital_allocator_py["(生产态 / production) default_capital_allocator.py"]
        src_zephyr_signal_fundamental_synth_signal_synthesizer_py["(生产态 / production) signal_synthesizer.py"]
    end
    ...
    class D_INFRASTRUCTURE,D_SIGLEGACY,D_INTELLIGENCE external_design
```
...
graph TD
    ...
    class src_zephyr_signal_fundamental_gen_implementations_default_signal_aggregator_py,src_zephyr_signal_fundamental_pipeline_py,src_zephyr_signal_fundamental_strategy_implementations_default_capital_allocator_py,src_zephyr_signal_fundamental_synth_signal_synthesizer_py production
    class D_SHARED,D_FACTOR,D_TRADING external_prod
    class D_INFRASTRUCTURE,D_INTELLIGENCE,D_SIGLEGACY external_design
```
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 5 个，2 条域内依赖）。
...
graph TD
    ...
    class D_INFRASTRUCTURE,D_SIGLEGACY external_design
```
...
|:--:|---------|:--:|---------|---------|
| 1 | pipeline.py | → | D_FACTOR 因子: ZephyrAlpha — D_FACTOR Alpha Factor Layer (fac... | 导入依赖 / import_depends |
| 2 | aggregator_base.py | → | D_INFRASTRUCTURE: factor_signal.py | 导入依赖 / import_depends |
| 3 | aggregator_base.py | → | D_INFRASTRUCTURE: synthesized_signal.py | 导入依赖 / import_depends |
| 4 | default_signal_aggregator.py | → | D_INFRASTRUCTURE: factor_signal.py | 导入依赖 / import_depends |
| 5 | default_signal_aggregator.py | → | D_INFRASTRUCTURE: synthesized_signal.py | 导入依赖 / import_depends |
| 6 | pipeline.py | → | D_INFRASTRUCTURE: factor_signal.py | 导入依赖 / import_depends |
| 7 | pipeline.py | → | D_INFRASTRUCTURE: synthesized_signal.py | 导入依赖 / import_depends |
| 8 | default_capital_allocator.py | → | D_INFRASTRUCTURE: synthesized_signal.py | 导入依赖 / import_depends |
| 9 | signal_synthesizer.py | → | D_INFRASTRUCTURE: factor_signal.py | 导入依赖 / import_depends |
| 10 | signal_synthesizer.py | → | D_INFRASTRUCTURE: synthesized_signal.py | 导入依赖 / import_depends |
| 11 | pipeline.py | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 导入依赖 / import_depends |
| 12 | pipeline.py | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 13 | D_FUNDAMENTAL_SIGNAL — CapitalAllocationResult... | → | D_TRADING 交易运营: capital_allocation_result.py | 导入依赖 / import_depends |
| 14 | aggregator_base.py | → | D_TRADING 交易运营: capital_allocation_result.py | 导入依赖 / import_depends |
| 15 | aggregator_base.py | → | D_TRADING 交易运营: signal_degradation_warning.py | 导入依赖 / import_depends |
| 16 | D_FUNDAMENTAL_SIGNAL — Capital Allocator（兼容... | → | D_TRADING 交易运营: capital_allocation_result.py | 导入依赖 / import_depends |
| 17 | default_capital_allocator.py | → | D_TRADING 交易运营: capital_allocation_result.py | 导入依赖 / import_depends |

### 依赖本域的其他域（入边）/ Depended By

| # | 外部域-源模块 / Source Module | → | 本域模块 / Target Module | 依赖类型 / Type |
|:--:|---------|:--:|---------|---------|
| 1 | D_FACTOR 因子: alpha_signal_pipeline.py | → | pipeline.py | 导入依赖 / import_depends |
| 2 | D_INTELLIGENCE 上下文管理: test_alpha_signal_pipeline.py | → | pipeline.py | 测试依赖 / test_depends |
| 3 | D_SHARED 共享服务: test_cross_layer.py | → | signal_synthesizer.py | 测试依赖 / test_depends |
| 4 | D_SIGLEGACY 信号遗留设计态: Signal Capital Allocation sub-package (__init__... | → | D_FUNDAMENTAL_SIGNAL — CapitalAllocationResult... | 导入依赖 / import_depends |
| 5 | D_SIGLEGACY 信号遗留设计态: Signal Capital Allocation sub-package (__init__... | → | D_SIGNAL — Capital Allocator（兼容 re-export s... | 导入依赖 / import_depends |
| 6 | D_SIGLEGACY 信号遗留设计态: Signal Capital Allocation sub-package (__init__... | → | D_SIGNAL — Default Capital Allocator（兼容 re-... | 导入依赖 / import_depends |
| 7 | D_SIGLEGACY 信号遗留设计态: __init__.py | → | aggregator_base.py | 导入依赖 / import_depends |
| 8 | D_SIGLEGACY 信号遗留设计态: __init__.py | → | D_FUNDAMENTAL_SIGNAL — Capital Allocator（兼容... | 导入依赖 / import_depends |
| 9 | D_SIGLEGACY 信号遗留设计态: __init__.py | → | signal_synthesizer.py | 导入依赖 / import_depends |
| 10 | D_SIGLEGACY 信号遗留设计态: D_SIGNAL — Signal Combiner Concrete Implementa... | → | default_signal_aggregator.py | 导入依赖 / import_depends |
| 11 | D_SIGLEGACY 信号遗留设计态: D_SIGNAL — Signal Combiner Concrete Implementa... | → | default_capital_allocator.py | 导入依赖 / import_depends |
| 12 | D_SIGLEGACY 信号遗留设计态: Signal Generation sub-package (__init__.py) | → | aggregator_base.py | config_depends / config_depends |
| 13 | D_SIGLEGACY 信号遗留设计态: D_SIGNAL — Signal Generation Concrete Implemen... | → | default_signal_aggregator.py | 导入依赖 / import_depends |
| 14 | D_SIGLEGACY 信号遗留设计态: Signal Strategy sub-package (__init__.py) | → | D_FUNDAMENTAL_SIGNAL — Capital Allocator（兼容... | 导入依赖 / import_depends |
| 15 | D_SIGLEGACY 信号遗留设计态: Signal Strategy Concrete Implementations (__ini... | → | default_capital_allocator.py | 导入依赖 / import_depends |
| 16 | D_SIGLEGACY 信号遗留设计态: Signal Synthesis sub-package (__init__.py) | → | signal_synthesizer.py | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 6 个外部域直接连接（出边 17 条 + 入边 16 条 = 33 条）。只显示直接连接的域，不展开具体节点。
