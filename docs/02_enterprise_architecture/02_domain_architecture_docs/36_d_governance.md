...
| 跨域入边 | 198 | Cross-domain Incoming | 198 |
| 跨域出边 | 502 | Cross-domain Outgoing | 502 |
...
graph TD
    ...
    scripts_governance_d3_metadata_validate_module_id_naming_py -.->|config_depends / config_depends| D_GOV_SCRIPTS
...
graph TD
    ...
    scripts_governance_meta_manage_error_budget_py -.->|config_depends / config_depends| D_GOV_SCRIPTS
...
graph TD
    ...
    D_AUDITTEST -.->|测试依赖 / test_depends| src_zephyr_governance_capability_lookup_py
...
graph TD
    ...
    class D_OPS,D_INTELLIGENCE,D_GOV_OPS_RESILIENCE,D_GOV_CODE_QUALITY,D_INFRA_RUNTIME,D_SHARED,D_GOV_RULE,D_BACKTEST,D_FRONTEND,D_GOV_AUDIT,D_GOV_KB external_prod
...
graph TD
    ...
    class D_GOV_OPS_RESILIENCE,D_SHARED,D_PF_ALLOC,D_EX_CORE,D_GOV_AUDIT,D_GOV_CODE_QUALITY,D_OPS,D_INFRA_RUNTIME,D_PF_CORE external_prod
    class D_GOV_ENFORCEMENT,D_TRADING external_design
```
...
graph TD
    ...
    class D_TRADING,D_INFRA_A2A,D_INFRA_RUNTIME,D_INTEGRATION_GATEWAY,D_PF_CORE,D_AUDITTEST external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOV_OPS_RESILIENCE,D_SECURITY,D_INFRA_RUNTIME,D_AUTONOMY_CORE,D_OPS,D_BACKTEST,D_FRONTEND,D_GOV_AUDIT,D_GOV_CODE_QUALITY external_prod
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 587 个，170 条域内依赖）。
...
graph TD
    ...
    class D_FACTOR,D_SHARED,D_GOV_OPS_RESILIENCE,D_OPS,D_GOV_CODE_QUALITY,D_REPORTING,D_EX_CORE,D_GOV_AUDIT external_prod
    class D_INFRA_RUNTIME,D_GOV_ENFORCEMENT,D_SECURITY external_design
```
...
| 5 | C-track 端到端演示 —— 全流水线一次性运行 (dem... | → | D_EX_CORE 执行核心: D_EXECUTION_CORE Trade Execution — Re-export w... | 导入依赖 / import_depends |
| 6 | ZephyrAlpha — governance.base re-export shim. ... | → | D_FACTOR 因子: ZephyrAlpha — D_FACTOR Alpha Factor Layer (fac... | 导入依赖 / import_depends |
| 7 | C-track 端到端演示 —— 全流水线一次性运行 (dem... | → | D_FUNDAMENTAL_SIGNAL 基本面信号: D_SIGNAL Signal Domain (__init__.py) | 导入依赖 / import_depends |
| 8 | Red/Blue Team Adversarial Test v3: SYS-MASTER-0... | → | D_GOV_AUDIT 审计追踪: SYS-MASTER-001 Compliance Checker (sys_master_c... | 导入依赖 / import_depends |
| 9 | scripts/governance/rebuild_audit_index.py — 重... | → | D_GOV_AUDIT 审计追踪: indexer.py | 导入依赖 / import_depends |
| 10 | ProjectionEngine — 事件折叠为当前状态（DW-0003... | → | D_GOV_AUDIT 审计追踪: EventStore — Event Sourcing 事件追加与回放（DW... | 导入依赖 / import_depends |
| 11 | DatabaseManager — 连接池 + 健康检查 + 自动备份... | → | D_GOV_AUDIT 审计追踪: audit_schema — 审计视图与查询入口（SH-DB-001 v... | 导入依赖 / import_depends |
| 12 | GovernanceServer: 治理域统一MCP入口 (governance... | → | D_GOV_AUDIT 审计追踪: writer.py | 导入依赖 / import_depends |
| 13 | Self-Benchmark (W3-7) — 5 组已知对自验证 + 引.... | → | D_GOV_CODE_QUALITY 代码质量治理: Stage 2: AST 级精确比对器. (ast_comparator.py) | 导入依赖 / import_depends |
| 14 | Self-Benchmark (W3-7) — 5 组已知对自验证 + 引.... | → | D_GOV_CODE_QUALITY 代码质量治理: 行为采样验证器 — Stage 0.25 低成本快速验证. (b... | 导入依赖 / import_depends |
| 15 | Self-Benchmark (W3-7) — 5 组已知对自验证 + 引.... | → | D_GOV_CODE_QUALITY 代码质量治理: 微型克隆检测器 — n-gram频率计数, 1-2行高频模式... | 导入依赖 / import_depends |
| 16 | git_commit.py — GitCommitGateway CLI 封装（OPS... | → | D_GOV_ENFORCEMENT 规则执行: GitCommitGateway — 全项目唯一合法 git commit .... | 导入依赖 / import_depends |
| 17 | AI写入前强制门禁钩子: lock协议检查+GateEngine P... | → | D_GOV_ENFORCEMENT 规则执行: ZephyrAlpha 门禁子包 (__init__.py) | 导入依赖 / import_depends |
| 18 | concurrent_commit_test.py — 幽灵提交红蓝对抗脚... | → | D_GOV_ENFORCEMENT 规则执行: GitCommitGateway — 全项目唯一合法 git commit .... | 导入依赖 / import_depends |
| 19 | ZephyrAlpha — D_COMPLIANCE Compliance Layer —... | → | D_GOV_ENFORCEMENT 规则执行: Re-export shim — ComplianceRule 真源已合并至 z... | 导入依赖 / import_depends |
| 20 | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | → | D_GOV_ENFORCEMENT 规则执行: GitCommitGateway — 全项目唯一合法 git commit .... | 导入依赖 / import_depends |
| 21 | D_DATA Data Source (__init__.py) | → | D_GOV_ENFORCEMENT 规则执行: D_DATA — Data Quality Gate (quality_gate.py) | 导入依赖 / import_depends |
| 22 | zephyr.trading.trading_contracts — trading-dom... | → | D_GOV_ENFORCEMENT 规则执行: Re-export shim — ComplianceRule 真源已合并至 z... | 导入依赖 / import_depends |
| 23 | session 隔离 stash 红蓝对抗极限测试。 (test_ses... | → | D_GOV_ENFORCEMENT 规则执行: GitCommitGateway — 全项目唯一合法 git commit .... | 测试依赖 / test_depends |
| 24 | test_git_commit_concurrent.py — 幽灵提交红蓝对... | → | D_GOV_ENFORCEMENT 规则执行: GitCommitGateway — 全项目唯一合法 git commit .... | 测试依赖 / test_depends |
| 25 | test_git_commit_extreme.py — GitCommitGateway ... | → | D_GOV_ENFORCEMENT 规则执行: GitCommitGateway — 全项目唯一合法 git commit .... | 测试依赖 / test_depends |
| 26 | test_git_commit_gateway.py — GitCommitGateway ... | → | D_GOV_ENFORCEMENT 规则执行: GitCommitGateway — 全项目唯一合法 git commit .... | 测试依赖 / test_depends |
| 27 | test_task_repo_gateway_e2e.py — 端到端链路测试... | → | D_GOV_ENFORCEMENT 规则执行: GitCommitGateway — 全项目唯一合法 git commit .... | 测试依赖 / test_depends |
| 28 | KB 13项一键体检 — CLI入口薄包装 (self_test.py) | → | D_GOV_KB 知识库治理: KB 13项一键体检 + --self-test入口 (self_test.py) | 导入依赖 / import_depends |
| 29 | [INVARIANTS] 预算健康检查不可跳过;检查结果必须.... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: __init__.py | 导入依赖 / import_depends |
| 30 | G-CT-008 消费端 — Escalation.on_a2a_failure() ... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: G-CT-003 消费端 — Escalation.on_rollback_failu... | 导入依赖 / import_depends |
| 31 | budget_enforcement.py | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Burn Rate Monitor — MOD-INF-024 (burn_rate_mon... | 导入依赖 / import_depends |
| 32 | budget_enforcement.py | → | D_GOV_OPS_RESILIENCE 运维弹性治理: degradation_manager.py | 导入依赖 / import_depends |
| 33 | budget_enforcement.py | → | D_GOV_OPS_RESILIENCE 运维弹性治理: timeout_guard.py | 导入依赖 / import_depends |
| 34 | default_security_gateway.py | → | D_GOV_OPS_RESILIENCE 运维弹性治理: DefaultSecurityGateway — SecurityGateway 三层.... | 导入依赖 / import_depends |
| 35 | Delegation Engine — MOD-INF-022 (delegation_en... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Escalation Protocol data models — MOD-INF-022 ... | 导入依赖 / import_depends |
| 36 | Escalation Protocol Self-Test — MOD-INF-022. (... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Escalation Engine — MOD-INF-022 (escalation_en... | 导入依赖 / import_depends |
| 37 | Escalation Protocol Self-Test — MOD-INF-022. (... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Escalation Protocol data models — MOD-INF-022 ... | 导入依赖 / import_depends |
| 38 | Escalation Protocol Self-Test — MOD-INF-022. (... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Circuit Breaker — MOD-INF-022 (circuit_breaker.py) | 导入依赖 / import_depends |
| 39 | transition — 状态机转换 Mixin（从 task_repo.py... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: EventHook — 声明式任务系统事件订阅 (event_hook.py) | 导入依赖 / import_depends |
| 40 | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: EventHook — 声明式任务系统事件订阅 (event_hook.py) | 导入依赖 / import_depends |
| 41 | Escalation Adapter — MOD-INF-022 统一集成入口.... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Escalation Engine — MOD-INF-022 (escalation_en... | 导入依赖 / import_depends |
| 42 | Escalation Adapter — MOD-INF-022 统一集成入口.... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Escalation Protocol data models — MOD-INF-022 ... | 导入依赖 / import_depends |
| 43 | GovernanceServer: 治理域统一MCP入口 (governance... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Escalation Engine — MOD-INF-022 (escalation_en... | 导入依赖 / import_depends |
| 44 | GovernanceServer: 治理域统一MCP入口 (governance... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Escalation Protocol data models — MOD-INF-022 ... | 导入依赖 / import_depends |
| 45 | CBG 熔断器重置 CLI (CircuitBreakerGateway Reset... | → | D_GOV_RULE 规则治理: CircuitBreakerGateway (CBG) — 模块间调用单向熔... | 导入依赖 / import_depends |
| 46 | CBG 熔断器重置 CLI (CircuitBreakerGateway Reset... | → | D_GOV_RULE 规则治理: CircuitBreakerGateway (CBG) — 模块间调用单向熔... | 导入依赖 / import_depends |
| 47 | create_task_from_finding.py — Finding → 任务.... | → | D_GOV_RULE 规则治理: task_types.py | 导入依赖 / import_depends |
| 48 | Gate Engine Bootstrap Self-Check — Quis custod... | → | D_GOV_RULE 规则治理: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 导入依赖 / import_depends |
| 49 | validate_gate_engine_external.py — Gate Engine... | → | D_GOV_RULE 规则治理: CircuitBreakerGateway (CBG) — 模块间调用单向熔... | 导入依赖 / import_depends |
| 50 | validate_gate_engine_external.py — Gate Engine... | → | D_GOV_RULE 规则治理: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 导入依赖 / import_depends |
| 51 | transition — 状态机转换 Mixin（从 task_repo.py... | → | D_GOV_RULE 规则治理: gate_types.py | 导入依赖 / import_depends |
| 52 | transition — 状态机转换 Mixin（从 task_repo.py... | → | D_GOV_RULE 规则治理: task_types.py | 导入依赖 / import_depends |
| 53 | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | → | D_GOV_RULE 规则治理: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | 导入依赖 / import_depends |
| 54 | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | → | D_GOV_RULE 规则治理: gate_types.py | 导入依赖 / import_depends |
| 55 | [INVARIANTS] 仅查询不修改; 连接失败→exit 1 (li... | → | D_GOV_SCRIPTS 脚本治理: analyze_orphan_consumers.py | config_depends / config_depends |
| 56 | phase_a_backup.py — 阶段A安全网 Tier0/Tier1 关... | → | D_GOV_SCRIPTS 脚本治理: analyze_orphan_consumers.py | config_depends / config_depends |
| 57 | rename_kebab_to_snake.py — 全项目文件名/目录名... | → | D_GOV_SCRIPTS 脚本治理: analyze_orphan_consumers.py | config_depends / config_depends |
| 58 | 命名规范白名单清理 - 全文替换脚本。 (rename_whi... | → | D_GOV_SCRIPTS 脚本治理: analyze_orphan_consumers.py | config_depends / config_depends |
| 59 | test_lock_scenarios.py — RULE-ZERO 锁协议场景 ... | → | D_GOV_SCRIPTS 脚本治理: analyze_orphan_consumers.py | config_depends / config_depends |
| 60 | [INVARIANTS] 设计态节点数>=1128; 规则表各表>0 (... | → | D_GOV_SCRIPTS 脚本治理: analyze_orphan_consumers.py | config_depends / config_depends |
| 61 | verify_rule_yaml_migration.py - 6-dimensional v... | → | D_GOV_SCRIPTS 脚本治理: analyze_orphan_consumers.py | config_depends / config_depends |
| 62 | encoding.py — UTF-8 编码安全工具 (encoding.py) | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 63 | libcst_docstring_adder.py — Lossless docstring... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 64 | 登记表主条目计数——与 generate_registry_master... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 65 | thresholds.py — 阈值集中配置加载器 (thresholds.py) | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 66 | walk.py — 目录遍历共享工具 (walk.py) | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 67 | [INVARIANTS] 仅查询不修改; 连接失败→exit 1 (li... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 68 | architecture_health_dashboard.py — 架构健康度.... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 69 | collect_system_threads.py — 全系统线程数快照采... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 70 | audit_registration.py — 孤儿注册检测（RULE-TWO... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 71 | CI Entry: Self-Check — Drift Detector 自身完整... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 72 | fix_shared_bypass.py - D-D-07 auto-fix tool (va... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 73 | validate_commit_gateway.py — GATE-COMMIT-GW 门... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 74 | validate_commit_message.py — Conventional Comm... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 75 | validate_exit_codes.py — 审计脚本退出码规范门... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 76 | validate_frozen_requirements.py — 依赖版本锁定... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 77 | Module docstring — see module-level docstring ... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 78 | validate_no_utf8_bom.py — UTF-8 BOM 检测门禁 (... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 79 | validate_script_naming.py — 审计脚本命名规范门... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 80 | validate_script_quality.py — 治理脚本质量合规... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 81 | validate_task_decomposition_bypass.py — Task D... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 82 | Module docstring — see module-level docstring ... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 83 | verify_audit_integrity.py — MOD-INF-020 · 零.... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 84 | ===============================================... | → | D_GOV_SCRIPTS 脚本治理: D12 AI 幻觉审计维度 (__init__.py) | config_depends / config_depends |
| 85 | validate_gate_prompt_conflict.py — Gate-Prompt... | → | D_GOV_SCRIPTS 脚本治理: D12 AI 幻觉审计维度 (__init__.py) | config_depends / config_depends |
| 86 | validate_session_budget.py — Session 操作预算.... | → | D_GOV_SCRIPTS 脚本治理: D12 AI 幻觉审计维度 (__init__.py) | config_depends / config_depends |
| 87 | validate_session_gate_check.py — Session 门禁.... | → | D_GOV_SCRIPTS 脚本治理: D12 AI 幻觉审计维度 (__init__.py) | config_depends / config_depends |
| 88 | audit_config_format.py — config/ 目录格式/注释... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 89 | audit_directory_integrity.py — 01_policies_and... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 90 | audit_directory_scalability.py -- 物理结构可扩.... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 91 | audit_findings_by_scope.py — 按目录范围筛选 Fi... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 92 | Batch create index.md for all directories under... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 93 | GATE-DIRECTORY-CONTRACT: Directory Contract val... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 94 | check_index_integrity.py — 索引完整性校验 (che... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 95 | cleanup_stash.py — git stash 堆积治理（OPS-202... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 96 | detect_orphan_py.py — 项目根目录孤儿 .py 文件... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 97 | detect_residual_files.py — 残留物检测 (detect_... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 98 | detect_temp_files.py | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 99 | 草稿区生命周期归档器 (Drafts Zone Lifecycle Arc... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 100 | generate_missing_index_md.py — 扫描目录树，为.... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 101 | run_script_smoke_test.py — 治理脚本冒烟测试运... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 102 | sync_index_from_manifest.py — 从 script_manife... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 103 | sync_policies_index.py — 从磁盘实际扫描，自动.... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 104 | validate_config_integrity.py — 运行时配置完整.... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 105 | validate_d1_output_sanity.py — D1 产出物合理性... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 106 | validate_immutable_core.py — immutable_core 文... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 107 | Module docstring — see module-level docstring ... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 108 | validate_read_before_write.py — 先读后写校验（... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 109 | 检测文档/数据文件中的断链与幽灵引用。 (audit_br... | → | D_GOV_SCRIPTS 脚本治理: D2 链接完整性 — 文档内/文档间交叉引用有效性审... | config_depends / config_depends |
| 110 | detect_relative_references.py — 相对路径引用检... | → | D_GOV_SCRIPTS 脚本治理: D2 链接完整性 — 文档内/文档间交叉引用有效性审... | config_depends / config_depends |
| 111 | GATE-INDEX: Validate and auto-fix index.md fact... | → | D_GOV_SCRIPTS 脚本治理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
| 112 | 批量回填 frontmatter doc_type 字段（doc_type 存... | → | D_GOV_SCRIPTS 脚本治理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
| 113 | 批量回填/重判 ttl 字段（6 格式统一入口，GATE-15... | → | D_GOV_SCRIPTS 脚本治理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
| 114 | [INVARIANTS] REQUIRED_SECTIONS 必须与蓝图+施工.... | → | D_GOV_SCRIPTS 脚本治理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
| 115 | GATE-VOCAB: 词表合法值硬编码检测（trae_060 §2... | → | D_GOV_SCRIPTS 脚本治理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
| 116 | 基于内容关键词的 ttl 精细分类审查脚本。 (classi... | → | D_GOV_SCRIPTS 脚本治理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
| 117 | deep_content_scanner.py — 深度内容扫描器 (deep... | → | D_GOV_SCRIPTS 脚本治理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
| 118 | generate_derived_files.py — 枚举自动派生生成器... | → | D_GOV_SCRIPTS 脚本治理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
| 119 | Scan docs/01_policies_and_standards and emit _r... | → | D_GOV_SCRIPTS 脚本治理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
| 120 | 批量迁移非法 doc_type 值（doc_type 存量治理 Sta... | → | D_GOV_SCRIPTS 脚本治理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
| 121 | validate_architecture.py - Validate rule files ... | → | D_GOV_SCRIPTS 脚本治理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
| 122 | Blueprint Provenance Gate - V-12: validate prov... | → | D_GOV_SCRIPTS 脚本治理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
| 123 | GATE-MODULEID: Validate module_id uniqueness an... | → | D_GOV_SCRIPTS 脚本治理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
| 124 | module_id / domain_id / submodule_id 格式校验真... | → | D_GOV_SCRIPTS 脚本治理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
| 125 | 登记表总索引自校验门禁 (Registry Master Index S... | → | D_GOV_SCRIPTS 脚本治理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
| 126 | Tool Contract 一致性校验脚本（MOD-INF-013 §9 R... | → | D_GOV_SCRIPTS 脚本治理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
| 127 | detect_deprecated_path_writes.py — 废弃路径写.... | → | D_GOV_SCRIPTS 脚本治理: D4 路径有效性 — 文件系统中路径引用/落位合规性.... | config_depends / config_depends |
| 128 | detect_excessive_file_moves.py — 文件过度搬迁... | → | D_GOV_SCRIPTS 脚本治理: D4 路径有效性 — 文件系统中路径引用/落位合规性.... | config_depends / config_depends |
| 129 | detect_ruins_references.py — 残骸/废弃路径引用... | → | D_GOV_SCRIPTS 脚本治理: D4 路径有效性 — 文件系统中路径引用/落位合规性.... | config_depends / config_depends |
| 130 | detect_split_delete_ref_commit.py — 删除引用分... | → | D_GOV_SCRIPTS 脚本治理: D4 路径有效性 — 文件系统中路径引用/落位合规性.... | config_depends / config_depends |
| 131 | analyze_contract_impact.py — 契约变更影响分析... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 132 | audit_depends_on_chain_depth.py — depends_on .... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 133 | measure_deprecation_cascade.py — 废弃级联影响... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 134 | CI Entry: Drift Detector E2E Pipeline Check (ch... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 135 | v2.4.0 — 2026-05-03 (check_architecture_gates.py) | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 136 | [INVARIANTS] 蓝图§5.5自动化触发机制状态列必须.... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 137 | [INVARIANTS] 代码[BLUEPRINT]头部module_id必须与... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 138 | [INVARIANTS] 蓝图模板合规检查不可绕过;52项检查.... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 139 | [INVARIANTS] 扫描 src/zephyr/ 下所有包; 检测跨.... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 140 | check_contract_code_drift.py —— 契约-代码双写... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 141 | check_contract_physical_path.py — GATE-CONTRAC... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 142 | check_dependency_direction.py — 依赖方向校验（... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 143 | check_g6_ctr_compliance.py - G6 CTR Contract Co... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 144 | [INVARIANTS] 扫描蓝图 §11 产出物 consumer_min;... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 145 | check_precommit_id_uniqueness.py — GATE-ID-UNI... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 146 | check_rule_four_way_alignment.py —— 规则四方.... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 147 | [INVARIANTS] 扫描所有蓝图 ssot_claims 字段; 检.... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 148 | check_trace_context_propagation.py — TraceCont... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 149 | GATE-VMS-SSOT: VMS 单一真源门禁——三重检测。 (... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 150 | G9-Detect: 架构约束违规检测器（对照 depgraph 实... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 151 | analyze_same_name_module_relations.py --- 同名.... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 152 | detect_depends_on_cycles.py - depends_on 环检测... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 153 | detect_deprecated_adr_references.py — 废弃 ADR... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 154 | detect_duplicate_module_names.py --- 同名模块语... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 155 | G-acqflow: 从 tasks.yaml 生成业务数据采集流图 M... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 156 | #183: 从 data_sources_registry.yaml 派生 polici... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 157 | 安全删除门禁脚本——RULE-THREE 强制执行器。 (pr... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 158 | 对标 HDEBT-01：rationale-log.md 体积 >150KB / .... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 159 | Strategy: (merge_readme_to_index.py) | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 160 | 对标：AGENTS.md §6.1 蓝图-代码同步强制约定 (sy... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 161 | sync_registry_from_blueprints.py -- 从 blueprin... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 162 | AGENTS.md §6.1 蓝图-代码同步强制约定的 CI 门禁... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 163 | AGENTS.md 6.4 铁律五 + 铁律六：construction_pro... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 164 | Module docstring — see module-level docstring ... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 165 | 蓝图物理位置与归属链完整性校验器 (Blueprint Pla... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 166 | GATE-TAG-UNIQUE - Blueprint tag uniqueness vali... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 167 | validate_lifecycle_refs.py — 生命周期引用约束.... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 168 | Module docstring — see module-level docstring ... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 169 | validate_session_log_updated.py — Session Log ... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 170 | validate_adr_frontmatter_consistency.py — ADR ... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 171 | validate_arch_review_gate.py — 架构评审门控校... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 172 | GATE-CONTRACT: CI gate for architecture_contrac... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 173 | validate_autonomy_gate.py — 变更级别 vs AI 自.... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 174 | validate_b_track_packages.py — B 轨包完整性校... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 175 | GATE-BS: Blind Spot Reality Check (validate_bli... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 176 | validate_code_yaml_alignment.py — GATE-A: 实际... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 177 | validate_cross_references.py — 架构模型 YAML +... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 178 | [INVARIANTS] 治理脚本执行正确 (validate_depende... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 179 | validate_depends_on_format.py — depends_on 条.... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 180 | validate_deprecated_dependents.py — 废弃文件活... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 181 | Module docstring — see module-level docstring ... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 182 | validate_field_ownership.py — frontmatter 字段... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 183 | Module docstring — see module-level docstring ... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 184 | validate_handoff_package.py — HandoffPackage .... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 185 | Module docstring — see module-level docstring ... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 186 | validate_module_schema.py — 模块 Schema 校验（... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 187 | Module docstring — see module-level docstring ... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 188 | validate_p0_module_contracts.py — P0 模块契约... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 189 | Module docstring — see module-level docstring ... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 190 | 对标：target_layer_vocabulary.yaml v1.0.0——ta... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 191 | validate_three_way_consistency.py — 三方一致性... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 192 | validate_md_yaml_number_drift.py — MD 视图与 Y... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 193 | validate_yaml_interface_uniqueness.py — YAML .... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 194 | v1.0.0 -- 2026-05-03 (validate_yaml_summaries.py) | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 195 | check_protected_paths.py — 受保护路径写入检查.... | → | D_GOV_SCRIPTS 脚本治理: D6 安全漏洞 — 代码/配置/依赖安全风险审计。 (__... | config_depends / config_depends |
| 196 | detect_anchor_file_deletion.py — 锚点文件删除... | → | D_GOV_SCRIPTS 脚本治理: D6 安全漏洞 — 代码/配置/依赖安全风险审计。 (__... | config_depends / config_depends |
| 197 | detect_git_dangerous.py — 危险 Git 命令检测 (d... | → | D_GOV_SCRIPTS 脚本治理: D6 安全漏洞 — 代码/配置/依赖安全风险审计。 (__... | config_depends / config_depends |
| 198 | detect_keywords_in_logs.py — 日志输出敏感关键.... | → | D_GOV_SCRIPTS 脚本治理: D6 安全漏洞 — 代码/配置/依赖安全风险审计。 (__... | config_depends / config_depends |
| 199 | detect_permanent_file_deletion.py — 永久文件删... | → | D_GOV_SCRIPTS 脚本治理: D6 安全漏洞 — 代码/配置/依赖安全风险审计。 (__... | config_depends / config_depends |
| 200 | detect_secrets.py — 密钥/Token/凭证硬编码检测 ... | → | D_GOV_SCRIPTS 脚本治理: D6 安全漏洞 — 代码/配置/依赖安全风险审计。 (__... | config_depends / config_depends |
| 201 | detect_shell_dangerous.py — 危险 Shell 命令检... | → | D_GOV_SCRIPTS 脚本治理: D6 安全漏洞 — 代码/配置/依赖安全风险审计。 (__... | config_depends / config_depends |
| 202 | detect_shell_true.py — shell=True 调用检测 (de... | → | D_GOV_SCRIPTS 脚本治理: D6 安全漏洞 — 代码/配置/依赖安全风险审计。 (__... | config_depends / config_depends |
| 203 | detect_threading_lock.py — threading.Lock 导入... | → | D_GOV_SCRIPTS 脚本治理: D6 安全漏洞 — 代码/配置/依赖安全风险审计。 (__... | config_depends / config_depends |
| 204 | detect_vague_terms.py — 模糊/不确定术语检测 (d... | → | D_GOV_SCRIPTS 脚本治理: D6 安全漏洞 — 代码/配置/依赖安全风险审计。 (__... | config_depends / config_depends |
| 205 | CI Entry: Adversarial Validation — Red-Blue Dr... | → | D_GOV_SCRIPTS 脚本治理: D6 安全漏洞 — 代码/配置/依赖安全风险审计。 (__... | config_depends / config_depends |
| 206 | 对标 architecture_principles.md §1bis R2 安全.... | → | D_GOV_SCRIPTS 脚本治理: D6 安全漏洞 — 代码/配置/依赖安全风险审计。 (__... | config_depends / config_depends |
| 207 | 对标 06-security_architecture.md §6.3 L3-Audit... | → | D_GOV_SCRIPTS 脚本治理: D6 安全漏洞 — 代码/配置/依赖安全风险审计。 (__... | config_depends / config_depends |
| 208 | validate_gate_discipline.py — 门禁纪律校验 (va... | → | D_GOV_SCRIPTS 脚本治理: D6 安全漏洞 — 代码/配置/依赖安全风险审计。 (__... | config_depends / config_depends |
| 209 | 行为说明 (check_ai_capability_boundary.py) | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 210 | check_encoding.py — 编码合规校验（INJ-007） (c... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 211 | check_idempotency.py — 幂等性缺失检查（HC-9） ... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 212 | check_pit_compliance.py — PIT 合规检查（HC-10... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 213 | check_pure_shim.py — GATE-NO-PURE-SHIM 检测器.... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 214 | detect_absolute_path_hardcoding.py — 绝对路径.... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 215 | detect_direct_llm_calls.py — 裸调 LLM API 检测... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 216 | detect_forward_reference — 前向引用检测扫描器... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 217 | detect_missing_encoding.py — open() 缺 encodin... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 218 | detect_pydantic_any_fields.py — Pydantic Any .... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 219 | detect_silent_degradation.py — 静默降级检测 (d... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 220 | N-06 module_id scope 前缀检测修复脚本。 (fix_n0... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 221 | N-12 KE 条目命名格式批量修复脚本。 (fix_n12_ke_... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 222 | N-13 YAML/JSON/MD 文件名 snake_case 批量修复脚... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 223 | N-14 __init__.py 缺少 __all__ 批量修复脚本。 (f... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 224 | N-15 BLUEPRINT 头部路径不存在批量修复脚本。 (fi... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 225 | fix_naming_manual — 手动修复少量命名违规(N-11/... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 226 | fix_orphan_exports.py — 批量修复孤儿模块导出（... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 227 | rewrite_imports.py — 批量重写 Python import 路... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 228 | validate_contracts_purity.py — 契约纯度校验 (v... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 229 | validate_docstring_coverage.py — Docstring 覆.... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 230 | validate_fle_action_metadata.py — FLE Action .... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 231 | validate_fle_imports.py — FLE import 接口合规... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 232 | validate_import_style.py — 导入风格一致性校验 ... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 233 | validate_init_all.py — __init__.py __all__ 完.... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 234 | validate_kb_write_provenance.py — 知识库写入 p... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 235 | validate_python_syntax.py — Python 语法完整性... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 236 | validate_test_assertion_depth.py — 测试断言深.... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 237 | validate_test_coverage.py — 测试覆盖率治理校验... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 238 | validate_type_annotation_coverage.py — 类型注.... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 239 | validate_unused_imports.py — 未使用导入检测 (v... | → | D_GOV_SCRIPTS 脚本治理: D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 240 | audit_rename_completeness.py — 改名完整性审计.... | → | D_GOV_SCRIPTS 脚本治理: D8 文档代码同步审计维度 (__init__.py) | config_depends / config_depends |
| 241 | 全自动注册表同步器 (auto_sync_all_registries.py) | → | D_GOV_SCRIPTS 脚本治理: D8 文档代码同步审计维度 (__init__.py) | config_depends / config_depends |
| 242 | detect_ai_products_in_docs.py — AI 产物位置检... | → | D_GOV_SCRIPTS 脚本治理: D8 文档代码同步审计维度 (__init__.py) | config_depends / config_depends |
| 243 | detect_dated_snapshots.py — 带日期快照文件检测... | → | D_GOV_SCRIPTS 脚本治理: D8 文档代码同步审计维度 (__init__.py) | config_depends / config_depends |
| 244 | Checks that every RULE-ZERO through RULE-N in .... | → | D_GOV_SCRIPTS 脚本治理: D8 文档代码同步审计维度 (__init__.py) | config_depends / config_depends |
| 245 | update_progress.py — 从 domain_progress.json .... | → | D_GOV_SCRIPTS 脚本治理: D8 文档代码同步审计维度 (__init__.py) | config_depends / config_depends |
| 246 | validate_document_lifecycle.py — 文档生命周期... | → | D_GOV_SCRIPTS 脚本治理: D8 文档代码同步审计维度 (__init__.py) | config_depends / config_depends |
| 247 | validate_document_ttl.py — 文档 TTL 过期检测 (... | → | D_GOV_SCRIPTS 脚本治理: D8 文档代码同步审计维度 (__init__.py) | config_depends / config_depends |
| 248 | detect_duplicated_normative_language.py — 规范... | → | D_GOV_SCRIPTS 脚本治理: D9 知识覆盖审计维度 (__init__.py) | config_depends / config_depends |
| 249 | detect_orphan_documents.py — 孤立文档检测 (det... | → | D_GOV_SCRIPTS 脚本治理: D9 知识覆盖审计维度 (__init__.py) | config_depends / config_depends |
| 250 | 从磁盘扫描生成路径全景图的tree段（运营态目录结.... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 251 | fix_module_manifest_layout.py — 校正治理脚本模... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 252 | generate_gate_registry.py — 门禁登记表自动生成... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 253 | generate_registry_master_index.py — 登记表总索... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 254 | inject_manifests.py — __manifest__ 批量注入器 ... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 255 | refresh_master_entries.py — 登记表总索引 entri... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 256 | sync_audit_protocol_numbers.py — 从 SSoT 注册.... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 257 | arbitrate_findings.py — Finding 仲裁器（跨脚本... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 258 | compute_sla_metrics.py — SLA/SLO 指标计算引擎.... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 259 | detect_config_deviation.py — 配置文件结构完整.... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 260 | detect_fix_oscillation.py — 自修复振荡检测（蓝... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 261 | detect_hallucinated_packages.py — 幻觉包（Slop... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 262 | detect_script_divergence.py — 脚本实现与蓝图规... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 263 | detect_script_rot.py — Script Rot（脚本静默失.... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 264 | env_check.py — 环境就绪检查门禁 (Environment R... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 265 | governance_watchdog.py | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 266 | manage_baseline.py — Finding 基线快照管理 (man... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 267 | manage_error_budget.py — Error Budget + Burn R... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 268 | manage_finding_timeseries.py — Finding 时序数.... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 269 | manage_script_ab_test.py — 脚本 A/B 对照模式 (... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 270 | manage_script_retirement.py — 脚本退役/废弃生.... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 271 | manage_shadow_mode.py — Shadow Mode 渐进激活管... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 272 | mutation_test_post_sync_validator.py — SSoT 变... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 273 | mutation_test_reconciliation_registry.py — Rec... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 274 | Phase E: AI context injection verification scri... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 275 | AI操作前准入控制器 — 写/删文件前的机械门禁检查... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 276 | score_script_effectiveness.py — 脚本有效性评分... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 277 | trace_finding_lifecycle.py — Finding C1→C5 全... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 278 | track_script_costs.py — 脚本执行 AI 费用追踪 (... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 279 | Module docstring — see module-level docstring ... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 280 | validate_cross_model_consensus.py — 多AI模型共... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 281 | validate_dependency_chain.py — 依赖链拓扑顺序... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 282 | validate_end_to_end_benchmark.py — END-TO-END ... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 283 | validate_environment_health.py — 脚本运行环境.... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 284 | validate_false_negatives.py — 假阴性检测引擎 (... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 285 | validate_mutation_testing.py — 变异测试引擎（.... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 286 | validate_rule_freshness.py — AI Session 注入文... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 287 | validate_rules_file_backdoor.py — Rules File B... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 288 | validate_rules_integrity.py — 规则文件完整性保... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 289 | Module docstring — see module-level docstring ... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 290 | validate_script_provenance.py — 脚本 Provenanc... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 291 | validate_script_system_health.py — 脚本系统健.... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 292 | validate_threshold_changes.py — 阈值变更审计日... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 293 | validate_trust_tier.py — Trust-Tier 门禁执行器... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 294 | verify_reconciliation_registry.py — Reconcilia... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 295 | run_gate_chain.py — 顺序运行多个门禁脚本，任一... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 296 | status.py — 审计系统状态仪表盘 (status.py) | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 297 | VMS 盲点闭合检查器 — MOD-INF-011 · R1(33) + R... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 298 | VMS Build Completion Check — MOD-INF-011 · TA... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 299 | VMS 跨文件内容一致性检查器 — MOD-INF-011 · TA... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 300 | VMS Phase 回滚方案 — MOD-INF-011 · TASK-INF-0... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 301 | VMS 版本同步检查器 — MOD-INF-011 · TASK-INF-0... | → | D_GOV_SCRIPTS 脚本治理: __init__.py | config_depends / config_depends |
| 302 | _base_server.py | → | D_INFRA_A2A A2A通信: A2A Protocol — MOD-INF-025 (__init__.py) | config_depends / config_depends |
| 303 | audit_logger.py | → | D_INFRA_A2A A2A通信: A2A Protocol — MOD-INF-025 (__init__.py) | config_depends / config_depends |
| 304 | G-CT-008 契约：A2A -> Audit 审计 Agent 间通信. ... | → | D_INFRA_A2A A2A通信: A2A Protocol — MOD-INF-025 (__init__.py) | config_depends / config_depends |
| 305 | error_codes.py | → | D_INFRA_A2A A2A通信: A2A Protocol — MOD-INF-025 (__init__.py) | config_depends / config_depends |
| 306 | policy_engine.py | → | D_INFRA_A2A A2A通信: A2A Protocol — MOD-INF-025 (__init__.py) | config_depends / config_depends |
| 307 | rate_limiter.py | → | D_INFRA_A2A A2A通信: A2A Protocol — MOD-INF-025 (__init__.py) | config_depends / config_depends |
| 308 | session_manager.py | → | D_INFRA_A2A A2A通信: A2A Protocol — MOD-INF-025 (__init__.py) | config_depends / config_depends |
| 309 | Rollback System CLI — MOD-INF-021 v0.10.0 Git-... | → | D_INFRA_RECOVERY 回滚恢复: RollbackExecutor — 回滚执行器核心封装。 (rollb... | 导入依赖 / import_depends |
| 310 | Rollback System CLI — MOD-INF-021 v0.10.0 Git-... | → | D_INFRA_RECOVERY 回滚恢复: RollbackVerifier — 回滚后验证器。 (rollback_ve... | 导入依赖 / import_depends |
| 311 | GovernanceServer: 治理域统一MCP入口 (governance... | → | D_INFRA_RECOVERY 回滚恢复: RollbackExecutor — 回滚执行器核心封装。 (rollb... | 导入依赖 / import_depends |
| 312 | A2A Protocol 全链路满分验证脚本 (a2a_full_verif... | → | D_INFRA_RUNTIME 运行时集成: __init__.py | 导入依赖 / import_depends |
| 313 | local_layer_daemon.py — L2 本地模型层守护进程.... | → | D_INFRA_RUNTIME 运行时集成: __init__.py | 导入依赖 / import_depends |
| 314 | start_brain.py — ZephyrAlpha 系统大脑一键启动 ... | → | D_INFRA_RUNTIME 运行时集成: AutoRuntimeCore — 三层运行时运营中心（系统大脑... | 导入依赖 / import_depends |
| 315 | start_brain.py — ZephyrAlpha 系统大脑一键启动 ... | → | D_INFRA_RUNTIME 运行时集成: AutoTaskGenerator — 自动任务生成器 (auto_task_... | 导入依赖 / import_depends |
| 316 | session_simulator — 30 个模拟开发 session 的蓝... | → | D_INFRA_RUNTIME 运行时集成: blueprint_metrics — 蓝图使用追踪 instrumentati... | 导入依赖 / import_depends |
| 317 | base.py — 审计脚本基类 (base.py) | → | D_INFRA_RUNTIME 运行时集成: __init__.py | 导入依赖 / import_depends |
| 318 | check_registry_consistency — 跨登记表一致性校... | → | D_INFRA_RUNTIME 运行时集成: __init__.py | 导入依赖 / import_depends |
| 319 | finding_state_machine.py — Finding 全生命周期.... | → | D_INFRA_RUNTIME 运行时集成: Finding Schema — 审计发现标准化数据模型 (findi... | 导入依赖 / import_depends |
| 320 | validate_emergency_bypass_log.py — 应急绕过审.... | → | D_INFRA_RUNTIME 运行时集成: __init__.py | 导入依赖 / import_depends |
| 321 | run_all.py — 脚本系统统一入口脚本 (run_all.py) | → | D_INFRA_RUNTIME 运行时集成: Finding->TaskCard 桥接器 (finding_task_bridge.py) | 导入依赖 / import_depends |
| 322 | run_all.py — 脚本系统统一入口脚本 (run_all.py) | → | D_INFRA_RUNTIME 运行时集成: Finding Schema — 审计发现标准化数据模型 (findi... | 导入依赖 / import_depends |
| 323 | IDE健康守护进程CLI包装器 (ide_health_service.py) | → | D_INFRA_RUNTIME 运行时集成: daemon_registry.py - unified daemon thread regi... | 导入依赖 / import_depends |
| 324 | context_budget.py —— 上下文预算管理与超预算截... | → | D_INFRA_RUNTIME 运行时集成: token_budget.py — Token 估算工具 SSoT (token_b... | 导入依赖 / import_depends |
| 325 | MiniQMT 实盘行情 Provider（Tick + 5档盘口） (mi... | → | D_INFRA_RUNTIME 运行时集成: DatabaseService: 统一管理数据库的连接池、生命周... | 导入依赖 / import_depends |
| 326 | Self-Benchmark (W3-7) — 5 组已知对自验证 + 引.... | → | D_INFRA_RUNTIME 运行时集成: AssetDiscoveryScanner — MOD-INF-026 L1 全量文.... | 导入依赖 / import_depends |
| 327 | Re-export bridge for layer3_coordination govern... | → | D_INFRA_RUNTIME 运行时集成: A2A 监控仪表盘 — Agent 集群运行状态可视化面板 ... | 导入依赖 / import_depends |
| 328 | Re-export bridge for layer3_coordination govern... | → | D_INFRA_RUNTIME 运行时集成: A2A 形式化验证 — 协议属性模型检查 (a2a_formal_... | 导入依赖 / import_depends |
| 329 | Re-export bridge for layer3_coordination govern... | → | D_INFRA_RUNTIME 运行时集成: A2A ANP 帧协商协议 — Agent Negotiation Protoco... | 导入依赖 / import_depends |
| 330 | Re-export bridge for layer3_coordination govern... | → | D_INFRA_RUNTIME 运行时集成: A2A 协议网关 — Agent 间请求分发与协议转换 (a2a... | 导入依赖 / import_depends |
| 331 | Re-export bridge for layer3_coordination govern... | → | D_INFRA_RUNTIME 运行时集成: A2A 分布式追踪 — 跨 Agent 请求链追踪 (Span-bas... | 导入依赖 / import_depends |
| 332 | Re-export bridge for layer3_coordination govern... | → | D_INFRA_RUNTIME 运行时集成: A2A Living Spec 同步 — 蓝图与实现的双向漂移管... | 导入依赖 / import_depends |
| 333 | service_layer_owners.yaml | → | D_INFRA_RUNTIME 运行时集成: ZephyrAlpha 核心包索引 + 模块懒加载器 (M-04) (_... | config_depends / config_depends |
| 334 | A2A Protocol 全链路满分验证脚本 (a2a_full_verif... | → | D_INTEGRATION 管线路由: __init__.py | 导入依赖 / import_depends |
| 335 | 初始化任务系统数据库 + 创建任务系统自身的施工任... | → | D_INTEGRATION 管线路由: __init__.py | 导入依赖 / import_depends |
| 336 | A2A 协议协调任务演示 (demo_a2a_coordination.py) | → | D_INTEGRATION 管线路由: __init__.py | 导入依赖 / import_depends |
| 337 | C-track 端到端演示 —— 全流水线一次性运行 (dem... | → | D_INTEGRATION 管线路由: __init__.py | 导入依赖 / import_depends |
| 338 | finalize_tasks.py | → | D_INTEGRATION 管线路由: __init__.py | 导入依赖 / import_depends |
| 339 | local_layer_daemon.py — L2 本地模型层守护进程.... | → | D_INTEGRATION 管线路由: LocalModelScheduler — L2 本地模型 24/7 调度循... | 导入依赖 / import_depends |
| 340 | start_brain.py — ZephyrAlpha 系统大脑一键启动 ... | → | D_INTEGRATION 管线路由: runtime_types.py | 导入依赖 / import_depends |
| 341 | test_event_hook.py | → | D_INTEGRATION 管线路由: __init__.py | 导入依赖 / import_depends |
| 342 | task_self_check.py — 任务系统自身健康检查 (tas... | → | D_INTEGRATION 管线路由: __init__.py | 导入依赖 / import_depends |
| 343 | create_task_from_finding.py — Finding → 任务.... | → | D_INTEGRATION 管线路由: __init__.py | 导入依赖 / import_depends |
| 344 | validate_gate_engine_external.py — Gate Engine... | → | D_INTEGRATION 管线路由: __init__.py | 导入依赖 / import_depends |
| 345 | Ollama 入职考试运行脚本 (run_ollama_exam.py) | → | D_INTEGRATION 管线路由: OllamaChat — 通过 Ollama HTTP API 进行本地 LLM... | 导入依赖 / import_depends |
| 346 | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | → | D_INTEGRATION 管线路由: severity_types.py | 导入依赖 / import_depends |
| 347 | embedding_router.py | → | D_INTEGRATION 管线路由: EmbeddingRouter — MOD-INF-011 双嵌入维度路由 (... | 导入依赖 / import_depends |
| 348 | EmbeddingRouter — MOD-INF-011 双嵌入维度路由 (... | → | D_INTEGRATION 管线路由: OllamaEmbedder — 通过 Ollama HTTP API 生成文本... | 导入依赖 / import_depends |
| 349 | GovernanceServer: 治理域统一MCP入口 (governance... | → | D_INTEGRATION 管线路由: BaseMCPServer: stdio 传输 + JSON-RPC 2.0 协议基... | 导入依赖 / import_depends |
| 350 | scaffold.py — ZephyrAlpha 唯一创建入口（RULE-T... | → | D_INTEGRATION_GATEWAY 集成网关: ZephyrAlpha MCP (Model Context Protocol) 子包。... | 导入依赖 / import_depends |
| 351 | C-track 端到端演示 —— 全流水线一次性运行 (dem... | → | D_INTELLIGENCE 上下文管理: D_ML_TRAIN — Default Inference Engine (default... | 导入依赖 / import_depends |
| 352 | 诊断 breadth_failed 能力的根因。 (diagnose_brea... | → | D_INTELLIGENCE 上下文管理: DeepSeekV4Chat --- DeepSeek V4 系列模型 API 客... | 导入依赖 / import_depends |
| 353 | 诊断 breadth_failed 能力的根因。 (diagnose_brea... | → | D_INTELLIGENCE 上下文管理: ExamOrchestrator --- 五轴入职考试主控 (exam_orc... | 导入依赖 / import_depends |
| 354 | 诊断 breadth_failed 能力的根因。 (diagnose_brea... | → | D_INTELLIGENCE 上下文管理: ExamTestCases --- v3.0.5 扩展考试题库（96 题 / ... | 导入依赖 / import_depends |
| 355 | 考试题库一致性检查——根因治本，防止"定义-注册.... | → | D_INTELLIGENCE 上下文管理: ExamTestCases --- v3.0.5 扩展考试题库（96 题 / ... | 导入依赖 / import_depends |
| 356 | DeepSeek V4 入职考试运行脚本 (run_deepseek_v4_e... | → | D_INTELLIGENCE 上下文管理: DeepSeekV4Chat --- DeepSeek V4 系列模型 API 客... | 导入依赖 / import_depends |
| 357 | DeepSeek V4 入职考试运行脚本 (run_deepseek_v4_e... | → | D_INTELLIGENCE 上下文管理: ExamOrchestrator --- 五轴入职考试主控 (exam_orc... | 导入依赖 / import_depends |
| 358 | Ollama 入职考试运行脚本 (run_ollama_exam.py) | → | D_INTELLIGENCE 上下文管理: ExamOrchestrator --- 五轴入职考试主控 (exam_orc... | 导入依赖 / import_depends |
| 359 | 考试系统评分逻辑单元测试（合成数据，零成本，不.... | → | D_INTELLIGENCE 上下文管理: CapabilityPassport --- AI 模型能力护照 (capabil... | 导入依赖 / import_depends |
| 360 | 考试系统评分逻辑单元测试（合成数据，零成本，不.... | → | D_INTELLIGENCE 上下文管理: ExamOrchestrator --- 五轴入职考试主控 (exam_orc... | 导入依赖 / import_depends |
| 361 | 考试系统评分逻辑单元测试（合成数据，零成本，不.... | → | D_INTELLIGENCE 上下文管理: ExamRubric --- 奥赛题结构化多维清单评分（v3.0.5... | 导入依赖 / import_depends |
| 362 | 考试系统评分逻辑单元测试（合成数据，零成本，不.... | → | D_INTELLIGENCE 上下文管理: ExamTestCases --- v3.0.5 扩展考试题库（96 题 / ... | 导入依赖 / import_depends |
| 363 | model_router.py | → | D_INTELLIGENCE 上下文管理: provider_data.py | 导入依赖 / import_depends |
| 364 | model_router.py | → | D_INTELLIGENCE 上下文管理: Results Writer — 持久化 benchmark 结果，支持历... | 导入依赖 / import_depends |
| 365 | budget_enforcement.py | → | D_OPS 反馈循环: Budget Enforcer core engine — MOD-INF-024 (bud... | 导入依赖 / import_depends |
| 366 | budget_enforcement.py | → | D_OPS 反馈循环: Budget Enforcer data models — MOD-INF-024 (bud... | 导入依赖 / import_depends |
| 367 | budget_enforcement.py | → | D_OPS 反馈循环: budget_tracker.py | 导入依赖 / import_depends |
| 368 | model_provider_data.py | → | D_OPS 反馈循环: Budget Enforcer data models — MOD-INF-024 (bud... | 导入依赖 / import_depends |
| 369 | model_router.py | → | D_OPS 反馈循环: Budget Enforcer data models — MOD-INF-024 (bud... | 导入依赖 / import_depends |
| 370 | GovernanceServer: 治理域统一MCP入口 (governance... | → | D_OPS 反馈循环: Budget Enforcer core engine — MOD-INF-024 (bud... | 导入依赖 / import_depends |
| 371 | GovernanceServer: 治理域统一MCP入口 (governance... | → | D_OPS 反馈循环: Budget Enforcer data models — MOD-INF-024 (bud... | 导入依赖 / import_depends |
| 372 | check_handoff_manifests.py — AI Session Handof... | → | D_ORCHESTRATOR 代理编排器: 集成契约注册表（Contract Registry） (contract_r... | 导入依赖 / import_depends |
| 373 | D_PORTFOLIO_CORE — Portfolio Construction Stra... | → | D_PF_ALLOC 组合分配: D_PORTFOLIO_CORE — Default Equity Long-Only St... | 导入依赖 / import_depends |
| 374 | Re-export wrapper: analytics_base canonical at ... | → | D_REPORTING 报告: D_REPORTING — Post-Trade Analytics Layer (anal... | 导入依赖 / import_depends |
| 375 | C-track 端到端演示 —— 全流水线一次性运行 (dem... | → | D_RISK 风控: D_RISK Risk Management (__init__.py) | 导入依赖 / import_depends |
| 376 | C-track 端到端演示 —— 全流水线一次性运行 (dem... | → | D_RISK 风控: ZephyrAlpha — D_RISK Risk Management Layer — ... | 导入依赖 / import_depends |
| 377 | C-track 端到端演示 —— 全流水线一次性运行 (dem... | → | D_RISK 风控: D_RISK — Stop-Loss & Kill Switch 兼容层 (stop_... | 导入依赖 / import_depends |
| 378 | git_commit.py — GitCommitGateway CLI 封装（OPS... | → | D_SECURITY 对抗验证: Session 级并发协调模块（P2-SES 落地）。 (sessio... | 导入依赖 / import_depends |
| 379 | AI写入前强制门禁钩子: lock协议检查+GateEngine P... | → | D_SECURITY 对抗验证: Session 级并发协调模块（P2-SES 落地）。 (sessio... | 导入依赖 / import_depends |
| 380 | G-CT-007 契约：Budget -> RBAC 配额限制. (rbac_b... | → | D_SECURITY 对抗验证: PermissionGuard — 七层权限编排器. (permission_... | 导入依赖 / import_depends |
| 381 | Delegation Engine — MOD-INF-022 (delegation_en... | → | D_SECURITY 对抗验证: gateway.py | 导入依赖 / import_depends |
| 382 | GovernanceServer: 治理域统一MCP入口 (governance... | → | D_SECURITY 对抗验证: Cold Start Bootstrapper — 冷启动引导 §6.31。 ... | 导入依赖 / import_depends |
| 383 | GovernanceServer: 治理域统一MCP入口 (governance... | → | D_SECURITY 对抗验证: PermissionGuard — 七层权限编排器. (permission_... | 导入依赖 / import_depends |
| 384 | session 隔离 stash 红蓝对抗极限测试。 (test_ses... | → | D_SECURITY 对抗验证: Session 级并发协调模块（P2-SES 落地）。 (sessio... | 测试依赖 / test_depends |
| 385 | C-track 端到端演示 —— 全流水线一次性运行 (dem... | → | D_SECURITY_LLM LLM防御: __init__.py | 导入依赖 / import_depends |
| 386 | DM-106: P2-B 迁移全量验证脚本 (dm106_p2b_verifi... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 387 | _e2e_check.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 388 | _e2e_deep.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 389 | reset_test_task.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 390 | generate_architecture_context.py — 预编译架构.... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 391 | 诊断 breadth_failed 能力的根因。 (diagnose_brea... | → | D_SHARED 共享服务: secrets.py —— Secrets 管理抽象（Phase 7 新增 ... | 导入依赖 / import_depends |
| 392 | audit_post_sync_commands.py — post_sync_standa... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 393 | DM-105: depgraph 未分配节点三策略处理脚本 (dm10... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 394 | constants.py — 审计脚本共享常量 (constants.py) | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 395 | _shared/file_utils.py — 原子写入共享工具（ARCH... | → | D_SHARED 共享服务: file_utils.py —— 安全文件操作工具（Phase 3 新... | 导入依赖 / import_depends |
| 396 | _shared/yaml_utils.py — YAML 文件加载共享工具 ... | → | D_SHARED 共享服务: yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
| 397 | [INVARIANTS] pg_advisory_lock 写锁; build_statu... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 398 | [INVARIANTS] 原子写入（RULE-ONE）；变更前验证；... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 399 | GATE-SSOT: SSoT 创建门禁（pre-commit hook 双保.... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 400 | GATE-SSOT-SINGLESOURCE: SSoT 单一真源门禁（Phas... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 401 | # [BLUEPRINT] MOD-INF-005 | scripts/governance/... | → | D_SHARED 共享服务: yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
| 402 | G13: 从 depgraph (PostgreSQL) 生成资产清单全景... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 403 | G12: 从 depgraph (PostgreSQL) 生成契约目录全景... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 404 | generate_contracts.py -- SSoT to Codegen pipeli... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 405 | G-panorama-registry: 自动生成全景图清单总表 (ge... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 406 | validate_module_lifecycle.py — 模块生命周期校... | → | D_SHARED 共享服务: yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
| 407 | validate_interface_contracts.py — 接口契约校验... | → | D_SHARED 共享服务: yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
| 408 | extract_decisiongraph - decisiongraph on-demand... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 409 | [INVARIANTS] 禁止AI直接Read 157MB depgraph文件.... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 410 | [INVARIANTS] YAML 是唯一真源; DB 为只读缓存; 同... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 411 | # [BLUEPRINT] MOD-INF-005 | scripts/governance/... | → | D_SHARED 共享服务: yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
| 412 | check_gate_inventory_drift.py — commit_gates .... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 413 | _concurrency.py | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP s... | 导入依赖 / import_depends |
| 414 | create_task_from_finding.py — Finding → 任务.... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 415 | concurrent_commit_test.py — 幽灵提交红蓝对抗脚... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 416 | IDE健康守护进程CLI包装器 (ide_health_service.py) | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP s... | 导入依赖 / import_depends |
| 417 | IDE健康守护进程CLI包装器 (ide_health_service.py) | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 418 | lock_files.py —— AI 对话文件锁协议（硬规则执.... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP s... | 导入依赖 / import_depends |
| 419 | lock_files.py —— AI 对话文件锁协议（硬规则执.... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 420 | MCP DAG 编排启动器（MOD-INF-013 §14 拓扑排序 +... | → | D_SHARED 共享服务: ProcessLifecycleGateway — 进程生命周期统一入口... | 导入依赖 / import_depends |
| 421 | 文件头部完整性校验（6 格式统一入口） (verify_he... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 422 | DeepSeek V4 入职考试运行脚本 (run_deepseek_v4_e... | → | D_SHARED 共享服务: secrets.py —— Secrets 管理抽象（Phase 7 新增 ... | 导入依赖 / import_depends |
| 423 | G-CT-007 契约：Budget -> RBAC 配额限制. (rbac_b... | → | D_SHARED 共享服务: agent_identity.py | 导入依赖 / import_depends |
| 424 | G-CT-007 契约：Budget -> RBAC 配额限制. (rbac_b... | → | D_SHARED 共享服务: permission.py | 导入依赖 / import_depends |
| 425 | G-CT-003 契约：Agent Spec -> RBAC 能力检查. (re... | → | D_SHARED 共享服务: skill_protocol.py | 导入依赖 / import_depends |
| 426 | G-CT-006 — BudgetAlert re-exported from shared... | → | D_SHARED 共享服务: budget_alert.py | 导入依赖 / import_depends |
| 427 | CapabilityLookup — 能力->真源文件反查注册表的.... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 428 | MiniQMT 实盘行情 Provider（Tick + 5档盘口） (mi... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 429 | pricing_sync.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 430 | depgraph Schema DDL + 版本化迁移框架 (depgraph_... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 431 | depgraph Schema DDL + 版本化迁移框架 (depgraph_... | → | D_SHARED 共享服务: secrets.py —— Secrets 管理抽象（Phase 7 新增 ... | 导入依赖 / import_depends |
| 432 | 实验 — Experimentation Pipeline Layer (pipelin... | → | D_SHARED 共享服务: experiment_result.py | 导入依赖 / import_depends |
| 433 | evidence_pack.py | → | D_SHARED 共享服务: serialization.py —— 统一序列化/反序列化基础设... | 导入依赖 / import_depends |
| 434 | AtomicTransactionManager — SQLite + 文件系统的... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 435 | AISG Sandbox Testing — AI Security Gateway 沙.... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 436 | Delegation Engine — MOD-INF-022 (delegation_en... | → | D_SHARED 共享服务: async_utils.py — async/sync 边界桥接（5.12.8 .... | 导入依赖 / import_depends |
| 437 | Self-Benchmark (W3-7) — 5 组已知对自验证 + 引.... | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 导入依赖 / import_depends |
| 438 | ProjectionEngine — 事件折叠为当前状态（DW-0003... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 439 | QueryMetrics — SQL 查询性能监控装饰器（SH-DB-0... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 440 | QueryMetrics — SQL 查询性能监控装饰器（SH-DB-0... | → | D_SHARED 共享服务: serialization.py —— 统一序列化/反序列化基础设... | 导入依赖 / import_depends |
| 441 | base_repo — 异常类、状态机常量、工具函数（从 t... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 442 | DatabaseManager — 连接池 + 健康检查 + 自动备份... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 443 | DatabaseService: 统一管理两个数据库的连接池、生... | → | D_SHARED 共享服务: DatabaseCRUDMixin: 共享的 governance.db + depgr... | 导入依赖 / import_depends |
| 444 | DatabaseService: 统一管理两个数据库的连接池、生... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 445 | decisiongraph Schema DDL + 不变量声明 (decision... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 446 | OLAPEngine — DuckDB OLAP 分析引擎 (olap_engine.py) | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 447 | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 448 | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | → | D_SHARED 共享服务: SQLite 连接工厂真源（SSoT） (sqlite_factory.py) | 导入依赖 / import_depends |
| 449 | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 450 | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 451 | Escalation Adapter — MOD-INF-022 统一集成入口.... | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 导入依赖 / import_depends |
| 452 | zephyr.trading.trading_contracts — trading-dom... | → | D_SHARED 共享服务: OrderSide/OrderStatus/OrderType — 交易枚举真源... | 导入依赖 / import_depends |
| 453 | zephyr.trading.trading_contracts — trading-dom... | → | D_SHARED 共享服务: execution_rejection_error.py | 导入依赖 / import_depends |
| 454 | A2A GovernanceAdapter — Phase 4 治理集成桥接器... | → | D_SHARED 共享服务: security_decision.py | 导入依赖 / import_depends |
| 455 | A2A GovernanceAdapter — Phase 4 治理集成桥接器... | → | D_SHARED 共享服务: async_utils.py — async/sync 边界桥接（5.12.8 .... | 导入依赖 / import_depends |
| 456 | G-CT-008 — A2ACommunication Pydantic V2 BaseMo... | → | D_SHARED 共享服务: Core A2A Protocol interface and governance data... | 导入依赖 / import_depends |
| 457 | A2A 治理适配器 — 连接 A2A 协议与 Governance 层... | → | D_SHARED 共享服务: security_decision.py | 导入依赖 / import_depends |
| 458 | A2A 治理适配器 — 连接 A2A 协议与 Governance 层... | → | D_SHARED 共享服务: async_utils.py — async/sync 边界桥接（5.12.8 .... | 导入依赖 / import_depends |
| 459 | Registry Governance — MOD-INF-037 (registry_go... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 460 | GovernanceServer: 治理域统一MCP入口 (governance... | → | D_SHARED 共享服务: agent_identity.py | 导入依赖 / import_depends |
| 461 | GovernanceServer: 治理域统一MCP入口 (governance... | → | D_SHARED 共享服务: skill_protocol.py | 导入依赖 / import_depends |
| 462 | GovernanceServer: 治理域统一MCP入口 (governance... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 463 | GovernanceServer: 治理域统一MCP入口 (governance... | → | D_SHARED 共享服务: async_utils.py — async/sync 边界桥接（5.12.8 .... | 导入依赖 / import_depends |
| 464 | test_git_commit_extreme.py — GitCommitGateway ... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 465 | test_depgraph_schema.py — depgraph_schema.py D... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 466 | test_verify_schema_health.py — verify_schema_h... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 467 | C-track 端到端演示 —— 全流水线一次性运行 (dem... | → | D_SIMULATION 仿真: 实验 Experimentation (__init__.py) | 导入依赖 / import_depends |
| 468 | IDE健康守护进程CLI包装器 (ide_health_service.py) | → | D_TRADING 交易运营: ide_health_daemon.py — TRAE IDE 幽灵窗口守护线... | 导入依赖 / import_depends |
| 469 | zephyr.trading.trading_contracts — trading-dom... | → | D_TRADING 交易运营: zephyr.trading.trading_contracts — trading-dom... | 导入依赖 / import_depends |
| 470 | zephyr.trading.trading_contracts — trading-dom... | → | D_TRADING 交易运营: factor_monitor_report.py | 导入依赖 / import_depends |
| 471 | zephyr.trading.trading_contracts — trading-dom... | → | D_TRADING 交易运营: factor_signal.py | 导入依赖 / import_depends |
| 472 | zephyr.trading.trading_contracts — trading-dom... | → | D_TRADING 交易运营: instrument.py | 导入依赖 / import_depends |
| 473 | zephyr.trading.trading_contracts — trading-dom... | → | D_TRADING 交易运营: macro_factor_signal.py | 导入依赖 / import_depends |
| 474 | zephyr.trading.trading_contracts — trading-dom... | → | D_TRADING 交易运营: market_data.py | 导入依赖 / import_depends |
| 475 | zephyr.trading.trading_contracts — trading-dom... | → | D_TRADING 交易运营: signal_degradation_warning.py | 导入依赖 / import_depends |
| 476 | zephyr.trading.trading_contracts — trading-dom... | → | D_TRADING 交易运营: synthesized_signal.py | 导入依赖 / import_depends |
| 477 | zephyr.trading.trading_contracts — trading-dom... | → | D_TRADING 交易运营: risk_dashboard_snapshot.py | 导入依赖 / import_depends |
| 478 | zephyr.trading.trading_contracts — trading-dom... | → | D_TRADING 交易运营: risk_limit_violation_error.py | 导入依赖 / import_depends |
| 479 | zephyr.trading.trading_contracts — trading-dom... | → | D_TRADING 交易运营: risk_limits.py | 导入依赖 / import_depends |
| 480 | zephyr.trading.trading_contracts — trading-dom... | → | D_TRADING 交易运营: risk_metrics.py | 导入依赖 / import_depends |
| 481 | zephyr.trading.trading_contracts — trading-dom... | → | D_TRADING 交易运营: risk_validator_protocol.py | 导入依赖 / import_depends |
| 482 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: capital_allocation_result.py | 导入依赖 / import_depends |
| 483 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: execution_rejection_error.py | 导入依赖 / import_depends |
| 484 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: execution_report.py | 导入依赖 / import_depends |
| 485 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: fill.py | 导入依赖 / import_depends |
| 486 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: model_serving_request.py | 导入依赖 / import_depends |
| 487 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: order.py | 导入依赖 / import_depends |
| 488 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: position.py | 导入依赖 / import_depends |
| 489 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: trading-contracts/factories.py — 交易域数据契.... | 导入依赖 / import_depends |
| 490 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: factor_monitor_report.py | 导入依赖 / import_depends |
| 491 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: factor_signal.py | 导入依赖 / import_depends |
| 492 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: instrument.py | 导入依赖 / import_depends |
| 493 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: macro_factor_signal.py | 导入依赖 / import_depends |
| 494 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: market_data.py | 导入依赖 / import_depends |
| 495 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: signal_degradation_warning.py | 导入依赖 / import_depends |
| 496 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: synthesized_signal.py | 导入依赖 / import_depends |
| 497 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: compliance_rule.py | 导入依赖 / import_depends |
| 498 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: risk_dashboard_snapshot.py | 导入依赖 / import_depends |
| 499 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: risk_limit_violation_error.py | 导入依赖 / import_depends |
| 500 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: risk_limits.py | 导入依赖 / import_depends |
| 501 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: risk_metrics.py | 导入依赖 / import_depends |
| 502 | Re-export shim — 真源已合并至 zephyr.trading.t... | → | D_TRADING 交易运营: risk_validator_protocol.py | 导入依赖 / import_depends |
...
| 102 | D_AUTONOMY_PERM 自治保护: check_kill_switch_latency.py — Kill Switch 延.... | → | Architecture Guard — 不变量适应度函数集 (__ini... | config_depends / config_depends |
| 103 | D_BACKTEST 回测:  | → |  | 导入依赖 / import_depends |
| 104 | D_BACKTEST 回测:  | → |  | 导入依赖 / import_depends |
| 105 | D_BACKTEST 回测: BacktestResult -> decisiongraph 适配器（TRAE-06... | → | decisiongraph Schema DDL + 不变量声明 (decision... | 导入依赖 / import_depends |
| 106 | D_EX_CORE 执行核心: D_EX_CORE adapters — 券商/风控适配器 re-export... | → | D_EXECUTION_CORE — Risk Validation Bridge (DW-... | 导入依赖 / import_depends |
| 107 | D_EX_CORE 执行核心: D_EX_CORE adapters — 券商/风控适配器 re-export... | → | D_EXECUTION_CORE — Simulation Broker Adapter (... | 导入依赖 / import_depends |
| 108 | D_EX_CORE 执行核心: D_EX_CORE adapters — 券商/风控适配器 re-export... | → | D_EXECUTION_CORE — BrokerInterface (broker_int... | 导入依赖 / import_depends |
| 109 | D_EX_CORE 执行核心: Re-export wrapper: broker_interface has migrate... | → | D_EXECUTION_CORE — BrokerInterface (broker_int... | 导入依赖 / import_depends |
| 110 | D_EX_CORE 执行核心: MiniQMT 实盘券商适配器（对接 xttrader，A股实盘.... | → | D_EXECUTION_CORE — BrokerInterface (broker_int... | 导入依赖 / import_depends |
| 111 | D_EX_CORE 执行核心:  | → |  | 导入依赖 / import_depends |
| 112 | D_EX_CORE 执行核心: Re-export wrapper: risk_validation_bridge has m... | → | D_EXECUTION_CORE — Risk Validation Bridge (DW-... | 导入依赖 / import_depends |
| 113 | D_EX_CORE 执行核心: Re-export wrapper: simulation_broker has migrat... | → | D_EXECUTION_CORE — Simulation Broker Adapter (... | 导入依赖 / import_depends |
| 114 | D_EX_CORE 执行核心: Re-export wrapper: broker_interface has migrate... | → | D_EXECUTION_CORE — BrokerInterface (broker_int... | 导入依赖 / import_depends |
| 115 | D_EX_CORE 执行核心: D_EXECUTION_CORE — Execution Engine (execution... | → | D_EXECUTION_CORE — Risk Validation Bridge (DW-... | 导入依赖 / import_depends |
| 116 | D_EX_CORE 执行核心: D_EXECUTION_CORE — Order Manager (order_manage... | → | D_EXECUTION_CORE — BrokerInterface (broker_int... | 导入依赖 / import_depends |
| 117 | D_FEEDBACK_LOOP 反馈循环引擎: FLE->Orc 告警分派器 — dispatch() 生产者 (alert... | → | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 118 | D_FEEDBACK_LOOP 反馈循环引擎: FLE DB契约适配器 — 通过规范zephyr.governance.s... | → | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 119 | D_FEEDBACK_LOOP 反馈循环引擎: FLE 持久化写入器 — 写 metrics/alerts/dispatch_... | → | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 120 | D_FEEDBACK_LOOP 反馈循环引擎: MetricsCollector: append-only metrics recording... | → | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 121 | D_FRONTEND 前端: ZephyrAlpha Dashboard · Streamlit 仪表盘 (app.py) | → | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 122 | D_FRONTEND 前端: ZephyrAlpha Dashboard · Streamlit 仪表盘 (app.py) | → | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 导入依赖 / import_depends |
| 123 | D_FRONTEND 前端: app_panel · Panel 仪表盘主应用入口（v3.1.0, #A... | → | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 124 | D_FRONTEND 前端: app_panel · Panel 仪表盘主应用入口（v3.1.0, #A... | → | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 导入依赖 / import_depends |
| 125 | D_FRONTEND 前端:  | → |  | 导入依赖 / import_depends |
| 126 | D_FRONTEND 前端:  | → |  | 导入依赖 / import_depends |
| 127 | D_GOV_AUDIT 审计追踪: reconciliation_registry.py — GitCommitGateway ... | → | depgraph Schema DDL + 版本化迁移框架 (depgraph_... | 导入依赖 / import_depends |
| 128 | D_GOV_AUDIT 审计追踪: SnapshotManager — Event Sourcing 快照管理（DW-... | → | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 129 | D_GOV_AUDIT 审计追踪: audit_schema — 审计视图与查询入口（SH-DB-001 v... | → | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 130 | D_GOV_AUDIT 审计追踪: EventStore — Event Sourcing 事件追加与回放（DW... | → | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 131 | D_GOV_AUDIT 审计追踪: audit-trail.evidence_pack — MOD-INF-020 · 证.... | → | evidence_pack.py | 导入依赖 / import_depends |
| 132 | D_GOV_AUDIT 审计追踪: audit-trail.kb_gate — MOD-INF-020 · KB 审计门... | → | rule_patterns.py — 治理规则正则 + 安全审计模式... | 导入依赖 / import_depends |
| 133 | D_GOV_AUDIT 审计追踪: audit-trail.privacy — MOD-INF-020 · PII 检测... | → | rule_patterns.py — 治理规则正则 + 安全审计模式... | 导入依赖 / import_depends |
| 134 | D_GOV_AUDIT 审计追踪: spec_auditor.py | → | G-CT-003 契约：Agent Spec -> RBAC 能力检查. (re... | 导入依赖 / import_depends |
| 135 | D_GOV_AUDIT 审计追踪: audit-trail.kb_gate — MOD-INF-020 · KB 审计门... | → | rule_patterns.py — 治理规则正则 + 安全审计模式... | 导入依赖 / import_depends |
| 136 | D_GOV_AUDIT 审计追踪: audit-trail.privacy — MOD-INF-020 · PII 检测... | → | rule_patterns.py — 治理规则正则 + 安全审计模式... | 导入依赖 / import_depends |
| 137 | D_GOV_CODE_QUALITY 代码质量治理: code-dedup-engine CLI——子命令映射+退出码+扫描... | → | Self-Benchmark (W3-7) — 5 组已知对自验证 + 引.... | 导入依赖 / import_depends |
| 138 | D_GOV_CODE_QUALITY 代码质量治理: capability_overlap_gate.py — 新建 .py 文件 Cap... | → | CapabilityLookup — 能力->真源文件反查注册表的.... | 导入依赖 / import_depends |
| 139 | D_GOV_CODE_QUALITY 代码质量治理: create_guard.py — 新建 .py / 非 rules/ .yaml .... | → | CapabilityLookup — 能力->真源文件反查注册表的.... | 导入依赖 / import_depends |
| 140 | D_GOV_CODE_QUALITY 代码质量治理: create_guard.py — 新建 .py / 非 rules/ .yaml .... | → | rule_patterns.py — 治理规则正则 + 安全审计模式... | 导入依赖 / import_depends |
| 141 | D_GOV_CODE_QUALITY 代码质量治理: ssot_redefinition_gate.py — SSoT 符号重复定义.... | → | CapabilityLookup — 能力->真源文件反查注册表的.... | 导入依赖 / import_depends |
| 142 | D_GOV_DRIFT 漂移检测: validate_ssot.py | → | 文件头部格式解析 SSoT（Single Source of Truth）... | 导入依赖 / import_depends |
| 143 | D_GOV_ENFORCEMENT 规则执行: Re-export wrapper: aisg_sandbox has migrated to... | → | AISG Sandbox Testing — AI Security Gateway 沙.... | 导入依赖 / import_depends |
| 144 | D_GOV_ENFORCEMENT 规则执行: Re-export wrapper: compliance_manager has migra... | → | ZephyrAlpha — D_COMPLIANCE Compliance Layer —... | 导入依赖 / import_depends |
| 145 | D_GOV_ENFORCEMENT 规则执行: Re-export wrapper: evidence_pack has migrated t... | → | evidence_pack.py | 导入依赖 / import_depends |
| 146 | D_GOV_ENFORCEMENT 规则执行: DLQ 重试策略 — 指数退避自动重试 (dlq_retry_pol... | → | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 147 | D_GOV_KB 知识库治理: SQLite墓碑表 + G2向量去重 (ke_tombstone.py) | → | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 148 | D_GOV_KB 知识库治理: KB 13项一键体检 + --self-test入口 (self_test.py) | → | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 149 | D_GOV_OPS_RESILIENCE 运维弹性治理: __init__.py | → | Delegation Engine — MOD-INF-022 (delegation_en... | 导入依赖 / import_depends |
| 150 | D_GOV_OPS_RESILIENCE 运维弹性治理: GovernanceAutoRunner — 治理脚本自动运行/自动关... | → | depgraph Schema DDL + 版本化迁移框架 (depgraph_... | 导入依赖 / import_depends |
| 151 | D_GOV_OPS_RESILIENCE 运维弹性治理: F5BootIntegration — F5 自动启动/关闭集成 (MOD-... | → | Delegation Engine — MOD-INF-022 (delegation_en... | 导入依赖 / import_depends |
| 152 | D_GOV_OPS_RESILIENCE 运维弹性治理: F5EventSubscriber — F5 事件启动机制 (MOD-INF-0... | → | Escalation Adapter — MOD-INF-022 统一集成入口.... | 导入依赖 / import_depends |
| 153 | D_GOV_OPS_RESILIENCE 运维弹性治理: F5ShutdownManager — F5 自动关闭/状态持久化/信.... | → | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 154 | D_GOV_OPS_RESILIENCE 运维弹性治理: DefaultSecurityGateway — SecurityGateway 三层.... | → | AISG Sandbox Testing — AI Security Gateway 沙.... | 导入依赖 / import_depends |
| 155 | D_GOV_RULE 规则治理: RuleLoader — 规则加载核心 API (rule_engine.py) | → | depgraph Schema DDL + 版本化迁移框架 (depgraph_... | 导入依赖 / import_depends |
| 156 | D_GOV_RULE 规则治理: G-TRIPLE-ALIGN: 蓝图↔代码↔依赖图三方对齐门禁 ... | → | depgraph Schema DDL + 版本化迁移框架 (depgraph_... | 导入依赖 / import_depends |
| 157 | D_GOV_SCRIPTS 脚本治理: blueprint_frontmatter_reconciler.py — 蓝图 fro... | → | depgraph Schema DDL + 版本化迁移框架 (depgraph_... | 导入依赖 / import_depends |
| 158 | D_GOV_SCRIPTS 脚本治理: P2-T6 PostgreSQL 40并发写入红蓝测试。 (p2_pg_co... | → | depgraph Schema DDL + 版本化迁移框架 (depgraph_... | 导入依赖 / import_depends |
| 159 | D_GOV_SCRIPTS 脚本治理: sync_panorama_module.py — 四图模块同步引擎（AR... | → | depgraph Schema DDL + 版本化迁移框架 (depgraph_... | 导入依赖 / import_depends |
| 160 | D_GOV_SCRIPTS 脚本治理: sync_panorama_module.py — 四图模块同步引擎（AR... | → | dataflowgraph Schema DDL + 连接入口 (dataflowgr... | 导入依赖 / import_depends |
| 161 | D_GOV_SCRIPTS 脚本治理: sync_panorama_module.py — 四图模块同步引擎（AR... | → | decisiongraph Schema DDL + 不变量声明 (decision... | 导入依赖 / import_depends |
| 162 | D_INFRA_A2A A2A通信: 基础设施 Infrastructure — A2A Protocol 模块 (M... | → | A2A GovernanceAdapter — Phase 4 治理集成桥接器... | 导入依赖 / import_depends |
| 163 | D_INFRA_A2A A2A通信: A2A Protocol — MOD-INF-025 (__init__.py) | → | Phase 4 Hold — A2A Phase 4 锁定标记模块 与其他... | 导入依赖 / import_depends |
| 164 | D_INFRA_A2A A2A通信: Layer 3: 协调+仲裁 — Coordinator, Living Spec ... | → | Re-export bridge for layer3_coordination govern... | 导入依赖 / import_depends |
| 165 | D_INFRA_RECOVERY 回滚恢复: SqliteDumper — SQLite 双轨 Checkpoint 的 DB 层... | → | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 166 | D_INFRA_RUNTIME 运行时集成: AssetDashboard — MOD-INF-026 资产健康仪表盘生... | → | depgraph Schema DDL + 版本化迁移框架 (depgraph_... | 导入依赖 / import_depends |
| 167 | D_INFRA_RUNTIME 运行时集成: escalation_bridge.py | → | Escalation Adapter — MOD-INF-022 统一集成入口.... | 导入依赖 / import_depends |
| 168 | D_INFRA_RUNTIME 运行时集成: ContractBus loader — 加载全部44条容量保障契约.... | → | Batch2 治理层契约 — 15条 Pydantic v2 Schema（P... | 导入依赖 / import_depends |
| 169 | D_INFRA_RUNTIME 运行时集成: DatabaseService: 统一管理数据库的连接池、生命周... | → | depgraph Schema DDL + 版本化迁移框架 (depgraph_... | 导入依赖 / import_depends |
| 170 | D_INFRA_RUNTIME 运行时集成: DatabaseService: 统一管理数据库的连接池、生命周... | → | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 171 | D_INFRA_RUNTIME 运行时集成: PreemptionManager -- 优先级抢占管理器 (preempti... | → | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 导入依赖 / import_depends |
| 172 | D_INFRA_RUNTIME 运行时集成: AutoRuntimeCore — 三层运行时运营中心（系统大脑... | → | model_router.py | 导入依赖 / import_depends |
| 173 | D_INFRA_RUNTIME 运行时集成: AutoRuntimeCore — 三层运行时运营中心（系统大脑... | → | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 导入依赖 / import_depends |
| 174 | D_INFRA_RUNTIME 运行时集成: AutoRuntimeCore — 三层运行时运营中心（系统大脑... | → | Escalation Adapter — MOD-INF-022 统一集成入口.... | 导入依赖 / import_depends |
| 175 | D_INFRA_RUNTIME 运行时集成: boot_hooks.py | → | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 导入依赖 / import_depends |
| 176 | D_INFRA_RUNTIME 运行时集成: resource_optimization.py - MAPE-K autonomic res... | → | capacity_governance_loop.py | 导入依赖 / import_depends |
| 177 | D_INTEGRATION 管线路由: __init__.py | → | embedding_router.py | config_depends / config_depends |
| 178 | D_INTEGRATION 管线路由: BaseMCPServer: stdio 传输 + JSON-RPC 2.0 协议基... | → | G-CT-007 契约：Budget -> RBAC 配额限制. (rbac_b... | 导入依赖 / import_depends |
| 179 | D_INTEGRATION 管线路由: MCP Gateway 集中式治理节点（MOD-INF-013 §12 Ph... | → | GovernanceServer: 治理域统一MCP入口 (governance... | 导入依赖 / import_depends |
| 180 | D_INTEGRATION 管线路由: PipelineOrchestrator — M1-M11 管线协调器 (pipe... | → | G-CT-007 契约：Budget -> RBAC 配额限制. (rbac_b... | 导入依赖 / import_depends |
| 181 | D_INTEGRATION_GATEWAY 集成网关: ZephyrAlpha MCP (Model Context Protocol) 子包。... | → | GovernanceServer: 治理域统一MCP入口 (governance... | 导入依赖 / import_depends |
| 182 | D_INTELLIGENCE 上下文管理: KB->VMS 同步引擎 — sync_to_vms() 生产者 (sync_... | → | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 183 | D_OPS 反馈循环: G-CT-006 消费端 — Escalation.on_budget_alert()... | → | Escalation Adapter — MOD-INF-022 统一集成入口.... | 导入依赖 / import_depends |
| 184 | D_ORCHESTRATOR 代理编排器: Orc 告警接收器 — handle_alert() 消费者 (alert_... | → | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 185 | D_ORCHESTRATOR 代理编排器: Orc 告警接收器 — handle_alert() 消费者 (alert_... | → | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 导入依赖 / import_depends |
| 186 | D_ORCHESTRATOR 代理编排器: CT-ORC-SCRIPT-001 运行时桥接 (finding_bridge.py) | → | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 导入依赖 / import_depends |
| 187 | D_PF_ALLOC 组合分配: D_PORTFOLIO_CORE — Default Equity Long-Only St... | → | D_PORTFOLIO_CORE — StrategyBase + StrategyMeta... | 导入依赖 / import_depends |
| 188 | D_PF_CORE 组合核心: Re-export wrapper: risk_limits canonical at zep... | → | Re-export shim — 真源已合并至 zephyr.trading.t... | 导入依赖 / import_depends |
| 189 | D_PF_CORE 组合核心: Re-export wrapper: strategy_base has migrated t... | → | D_PORTFOLIO_CORE — StrategyBase + StrategyMeta... | 导入依赖 / import_depends |
| 190 | D_PF_CORE 组合核心: Re-export wrapper: strategy_engine has migrated... | → | D_PORTFOLIO_CORE — Portfolio Construction Stra... | 导入依赖 / import_depends |
| 191 | D_PF_CORE 组合核心: Re-export wrapper: strategy_registry has migrat... | → | StrategyRegistry 卫星模块（OCP-002） (strategy_... | 导入依赖 / import_depends |
| 192 | D_SECURITY 对抗验证: Cold Start Bootstrapper — 冷启动引导 §6.31。 ... | → | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 193 | D_SECURITY 对抗验证: D_COMPLIANCE — Compliance Concrete Implementat... | → | default_security_gateway.py | 导入依赖 / import_depends |
| 194 | D_SECURITY 对抗验证: db.py | → | SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 195 | D_SHARED 共享服务: A2A Protocol — shared interface definitions. (... | → | A2A Governance — shared interface definitions ... | 导入依赖 / import_depends |
| 196 | D_SHARED 共享服务: A2A Layer3 Coordination — shared Protocol inte... | → | A2A Governance — shared interface definitions ... | 导入依赖 / import_depends |
| 197 | D_TRADING 交易运营: AutoDispatcher — 守护进程内的轻量 PipelineDisp... | → | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 导入依赖 / import_depends |
| 198 | D_TRADING 交易运营: ide_health_daemon.py — TRAE IDE 幽灵窗口守护线... | → | TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 34 个外部域直接连接（出边 502 条 + 入边 198 条 = 700 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_GOV_DRIFT -->|1条 导入依赖 / import_depends| D_GOVERNANCE
