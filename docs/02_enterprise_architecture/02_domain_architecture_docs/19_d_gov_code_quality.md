...
flowchart TD
    ...
    D_GOV_ENFORCEMENT -->|测试依赖 / test_depends| src_zephyr_gov_enforcement_commit_gates_claim_required_gate_py
    ...
    D_GOV_ENFORCEMENT -->|测试依赖 / test_depends| src_zephyr_gov_enforcement_commit_gates_bare_subprocess_gate_py
    ...
    class D_GOV_ENFORCEMENT,D_SHARED,D_GOV_AUDIT,D_GOVERNANCE,D_SECURITY external_prod
```
...
| 24 | BreakingChange检测器测试 / Test Breaking Change Detector ... | → | D_GOV_DRIFT 漂移检测: —字段删除/类型变更->CI FAIL / Breaking Change Detector (... | 测试依赖 / test_depends |
...
| 148 | baregetenv门禁 / bare_getenv_gate (commit_gates/bare_gete... | → | D_SHARED 共享服务: Secrets (security/secrets.py) | 导入依赖 / import_depends |
