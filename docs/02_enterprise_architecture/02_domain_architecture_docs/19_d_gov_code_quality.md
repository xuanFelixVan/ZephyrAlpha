...
flowchart TD
    ...
    src_zephyr_gov_enforcement_commit_gates_blueprint_amodule_cross_check_gate_py["蓝图amodule跨check门禁<br/>(BLUEPRINT) vs (A_module) 交叉校验门禁<br/>blueprint_amodule_cross_check_gate<br/>文件: commit_gates<br/>/blueprint_amodule_cross_check_gate.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_enforcement_commit_gates_exempt_zone_frontmatter_gate_py["exemptzonefrontmatter门禁<br/>豁免区 frontmatter 门禁（Phase 3<br/>reconciler->gate 收敛）<br/>exempt_zone_frontmatter_gate<br/>文件: commit_gates<br/>/exempt_zone_frontmatter_gate.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_enforcement_commit_gates_issue_resolved_integrity_gate_py["issueresolved完整性门禁<br/>ISSUE-RESOLVED-INTEGRITY warn-only 门禁<br/>issue_resolved_integrity_gate<br/>文件: commit_gates<br/>/issue_resolved_integrity_gate.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_enforcement_commit_gates_zephyr_env_direct_access_gate_py["zephyrenvdirect访问门禁<br/>ZEPHYR_ENV 直访硬阻断门禁<br/>zephyr_env_direct_access_gate<br/>文件: commit_gates<br/>/zephyr_env_direct_access_gate.py<br/>(生产态 / production)"]
    ...
    tests_governance_test_apply_dataflowgraph_smoke_py["测试applydataflowgraphsmoke<br/>测试应用dataflowgraphsmoke。test_apply_dataflowg<br/>raph_smoke.py — apply_dataflowgraph.py<br/>end-to-end smoke test<br/>文件: governance<br/>/test_apply_dataflowgraph_smoke.py<br/>(生产态 / production)"]
    tests_governance_test_apply_decisiongraph_smoke_py["测试applydecisiongraphsmoke<br/>测试应用decisiongraphsmoke。test_apply_decisiong<br/>raph_smoke.py — apply_decisiongraph.py<br/>end-to-end smoke test<br/>文件: governance<br/>/test_apply_decisiongraph_smoke.py<br/>(生产态 / production)"]
    tests_governance_test_apply_depgraph_smoke_py["测试applydepgraphsmoke<br/>测试应用依赖图smoke。test_apply_depgraph_smoke.p<br/>y — apply_depgraph.py end-to-end smoke test<br/>文件: governance/test_apply_depgraph_smoke.py<br/>(生产态 / production)"]
    ...
    tests_governance_test_generate_project_depgraph_smoke_py["测试generateprojectdepgraphsmoke<br/>测试生成project依赖图smoke。test_generate_projec<br/>t_depgraph_smoke.py —<br/>generate_project_depgraph.py e2e smoke test<br/>文件: governance<br/>/test_generate_project_depgraph_smoke.py<br/>(生产态 / production)"]
    ...
    tests_governance_test_run_silent_failure_regression_py["测试runsilent故障regression<br/>silent-failure 回归 runner 单元测试<br/>test_run_silent_failure_regression<br/>文件: governance<br/>/test_run_silent_failure_regression.py<br/>(生产态 / production)"]
    ...
    tests_governance_test_sync_yaml_to_depgraph_smoke_py["测试syncyamltodepgraphsmoke<br/>测试同步yamlto依赖图smoke。test_sync_yaml_to_dep<br/>graph_smoke.py — sync_yaml_to_depgraph.py e2e<br/>smoke test<br/>文件: governance<br/>/test_sync_yaml_to_depgraph_smoke.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_enforcement_commit_gates_capability_lookup_bypass_policy_py["capabilitylookup绕过策略<br/>CAPABILITY-LOOKUP bypass 策略共享模块<br/>capability_lookup_bypass_policy<br/>文件: commit_gates<br/>/capability_lookup_bypass_policy.py<br/>(生产态 / production)"]
