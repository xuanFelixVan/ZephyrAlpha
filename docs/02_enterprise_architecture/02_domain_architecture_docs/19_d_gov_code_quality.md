...
flowchart TD
    ...
    src_zephyr_gov_code_quality_code_dedup_behavioral_trust_checker_py["行为信任检查器 — 行为漂移DIVERGED检测.<br/>code dedup包的behavioral_trust_checker模块<br/>文件: code_dedup/behavioral_trust_checker.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_canary_register_py["金丝雀注册表维护器 — 注册/过期/腐败检测.<br/>code dedup包的canary_register模块<br/>文件: code_dedup/canary_register.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_contract_consistency_checker_py["API契约一致性检查器 — 存在性·行为·契约三维.<br/>code dedup包的contract_consistency_checker模块<br/>文件: code_dedup/contract_consistency_checker.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_fifteen_dimension_auditor_py["15维超综合审计首页 — 逐项证明'做过且做对'.<br/>code dedup包的fifteen_dimension_auditor模块<br/>文件: code_dedup/fifteen_dimension_auditor.py<br/>(生产态 / production)"]
    src_zephyr_gov_code_quality_code_dedup_file_creator_py["文件创建清单执行器 — 验证所有源/测试<br/>/数据文件存在性.<br/>code dedup包的file_creator模块<br/>文件: code_dedup/file_creator.py<br/>(生产态 / production)"]
    src_zephyr_gov_code_quality_code_dedup_function_discovery_py["共享函数主动发现 — 签名+语义双通道从被动到主动.<br/>code dedup包的function_discovery模块<br/>文件: code_dedup/function_discovery.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_observation_window_guard_py["提取后稳定观察期守护 — 对标SDP 14天观察.<br/>code dedup包的observation_window_guard模块<br/>文件: code_dedup/observation_window_guard.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_policy_tree_validator_py["策略树自动一致性校验器 — 虚线箭头影响分析.<br/>code dedup包的policy_tree_validator模块<br/>文件: code_dedup/policy_tree_validator.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_prioritizer_py["修复优先级排序器 — 置信度×Impact×适配性<br/>三因子排序.<br/>code dedup包的prioritizer模块<br/>文件: code_dedup/prioritizer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_shared_evolver_py["共享函数自我进化引擎 — 自动升降级 +<br/>行为漂移锁定.<br/>code dedup包的shared_evolver模块<br/>文件: code_dedup/shared_evolver.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_simplicity_auditor_py["引擎成本效益自审计器 — SAS 0-100 月度审计 + Tax<br/>报告.<br/>code dedup包的simplicity_auditor模块<br/>文件: code_dedup/simplicity_auditor.py<br/>(生产态 / production)"]
    src_zephyr_gov_code_quality_code_dedup_ssot_registrar_py["SSoT注册器 — 提取函数自动注册到 shared API清单.<br/>code dedup包的ssot_registrar模块<br/>文件: code_dedup/ssot_registrar.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_thematic_clusterer_py["主题聚类器 — 噪声信号比·告警疲劳缓解.<br/>code dedup包的thematic_clusterer模块<br/>文件: code_dedup/thematic_clusterer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_trackers_blind_spot_tracker_py["盲点关闭追踪器 — 自动验证各轮盲点是否已覆盖.<br/>trackers包的blind_spot_tracker模块<br/>文件: trackers/blind_spot_tracker.py<br/>(生产态 / production)"]
    src_zephyr_gov_code_quality_code_dedup_trackers_hotspot_tracker_py["热点追踪器 — 90天滑动窗口 + 高频变动检测 +<br/>新项目预热清单.<br/>trackers包的hotspot_tracker模块<br/>文件: trackers/hotspot_tracker.py<br/>(生产态 / production)"]
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_ENFORCEMENT,D_GOV_SCRIPTS,D_GOV_AUDIT external_prod
```
