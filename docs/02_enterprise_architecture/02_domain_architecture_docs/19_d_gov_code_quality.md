...
flowchart TD
    ...
    src_zephyr_gov_enforcement_commit_gates_precommit_offline_gate_py -->|导入依赖 / import_depends| D_GOV_ENFORCEMENT
...
> 本域与 14 个外部域直接连接（出边 157 条 + 入边 138 条 = 295 条）。只显示直接连接的域，不展开具体节点。
