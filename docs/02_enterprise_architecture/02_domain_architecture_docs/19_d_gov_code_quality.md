...
| 5 | Test Adversarial Validation (gate_engine/test_adversarial... | → | D_FBL_VERIFICATION 反馈验证: 对抗验证 / Adversarial Validation Gate — FLE-ADVERSARIAL... | 测试依赖 / test_depends |
...
| 14 | Test Atomic Transaction Manager Unit (governance_misc/tes... | → | D_GOVERNANCE 生命周期管理: atomic交易管理器 / atomic_transaction_manager (financial_... | 测试依赖 / test_depends |
...
| 19 | Test Sync Yaml To Depgraph Smoke (governance/test_sync_ya... | → | D_GOVERNANCE 生命周期管理: 依赖图模式 / depgraph_schema (governance/depgraph_schema.py) | 测试依赖 / test_depends |
...
| 23 | Test En 002 Enforcement Validator (invariants/test_en_002... | → | D_GOV_DRIFT 漂移检测: 从 YAML 真源加载契约文件路径 / En 002 Enforcement Validat... | 测试依赖 / test_depends |
| 24 | Test Breaking Change Detector (rule_enforcement/test_brea... | → | D_GOV_DRIFT 漂移检测: —字段删除/类型变更->CI FAIL / Breaking Change Detector (... | 测试依赖 / test_depends |
...
| 122 | Test Output Quality Gate (rule_enforcement/test_output_qu... | → | D_GOV_ENFORCEMENT 规则执行: 只读：rules / Output Quality Gate (rule_enforcement/outpu... | 测试依赖 / test_depends |
| 123 | worktree_ops_log 遥测完整性审计测试 / Test Audit Worktree... | → | D_GOV_ENFORCEMENT 规则执行: Session Worktree (rule_bridge/session_worktree.py) | 测试依赖 / test_depends |
| 124 | Test Bare Repo Scanner (governance_misc/test_bare_repo_sc... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Bare Repo Scanner (security_governance/bare_repo_scanner.py) | 测试依赖 / test_depends |
| 125 | Test Governance Result Types (governance_misc/test_govern... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Result Types (escalation/result_types.py) | 测试依赖 / test_depends |
| 126 | Test Adversarial Gate Integration (gate_engine/test_adver... | → | D_GOV_RULE 规则治理: 对抗样本生成器 / Adversarial Strategies (rule_enforcement... | 测试依赖 / test_depends |
| 127 | Test Adversarial Gate Integration (gate_engine/test_adver... | → | D_GOV_RULE 规则治理: 对抗验证门禁 / Adversarial Validation (gate_engine/advers... | 测试依赖 / test_depends |
| 128 | Test Adversarial Validation Gate (gate_engine/test_advers... | → | D_GOV_RULE 规则治理: 对抗验证门禁 / Adversarial Validation (gate_engine/advers... | 测试依赖 / test_depends |
| 129 | Test En 001 Circular Dependency (invariants/test_en_001_c... | → | D_GOV_RULE 规则治理: 循环依赖扫描器 / Circular Dependency Scanner (invariants/... | 测试依赖 / test_depends |
| 130 | Test En 003 Contract Compatibility (invariants/test_en_00... | → | D_GOV_RULE 规则治理: 契约兼容性检查器 / Contract Compatibility Checker (invari... | 测试依赖 / test_depends |
| 131 | Test En Process Lifecycle Gateway (invariants/test_en_pro... | → | D_GOV_RULE 规则治理: 进程生命周期网关 / Process Lifecycle Gateway (invariants/... | 测试依赖 / test_depends |
| 132 | Test Zero Residue Check (invariants/test_zero_residue_che... | → | D_GOV_RULE 规则治理: 零残留检查器 / Zero Residue Check (invariants/zero_residu... | 测试依赖 / test_depends |
| 133 | Test Adaptive Threshold (rule_enforcement/test_adaptive_t... | → | D_GOV_RULE 规则治理: 自适应阈值 / Adaptive Threshold (rule_enforcement/adaptiv... | 测试依赖 / test_depends |
| 134 | Test Adversarial Strategies (rule_enforcement/test_advers... | → | D_GOV_RULE 规则治理: 对抗样本生成器 / Adversarial Strategies (rule_enforcement... | 测试依赖 / test_depends |
| 135 | Test Integration Test Runner (rule_enforcement/test_integ... | → | D_GOV_RULE 规则治理: 集成测试运行器 / Integration Test Runner (rule_enforcemen... | 测试依赖 / test_depends |
| 136 | Test Kiss Enforcer (rule_enforcement/test_kiss_enforcer.py) | → | D_GOV_RULE 规则治理: KISS 约束执行器 / KISS Enforcer (rule_enforcement/kiss_en... | 测试依赖 / test_depends |
| 137 | Test Secrets Guard (rule_enforcement/test_secrets_guard.py) | → | D_GOV_RULE 规则治理: 密钥守卫 / Secrets Guard (rule_enforcement/secrets_guard.py) | 测试依赖 / test_depends |
| 138 | Test Triple Alignment (rule_enforcement/test_triple_align... | → | D_GOV_RULE 规则治理: 三方对齐门禁 / Triple Alignment (rule_enforcement/triple_... | 测试依赖 / test_depends |
...
| 4 | D_GOVERNANCE 生命周期管理: Test Shadow Verifier (adversarial/test_shadow_verifier.py) | → | 影子验证器 / shadow_verifier (code_dedup/shadow_verifier.py) | 测试依赖 / test_depends |
| 5 | D_GOVERNANCE 生命周期管理: Test Ast Comparator (code_quality/test_ast_comparator.py) | → | ast比较器 / ast_comparator (code_dedup/ast_comparator.py) | 测试依赖 / test_depends |
| 6 | D_GOVERNANCE 生命周期管理: Test Code Analyzer Runner (code_quality/test_code_analyze... | → | 代码分析器运行器 / code_analyzer_runner (code_dedup/code_... | 测试依赖 / test_depends |
| 7 | D_GOVERNANCE 生命周期管理: Test Code Dedup Engine (code_quality/test_code_dedup_engi... | → | 安全自动修复引擎——五直接开关+五间接约束. / auto_fixer (... | 测试依赖 / test_depends |
| 8 | D_GOVERNANCE 生命周期管理: Test Code Dedup Engine (code_quality/test_code_dedup_engi... | → | monoculture守卫 / monoculture_guard (code_dedup/monocultu... | 测试依赖 / test_depends |
| 9 | D_GOVERNANCE 生命周期管理: Test Code Dedup Engine Red Team (code_quality/test_code_d... | → | 决策审计器 / decision_auditor (code_dedup/decision_audito... | 测试依赖 / test_depends |
| 10 | D_GOVERNANCE 生命周期管理: Test Code Dedup Engine Red Team (code_quality/test_code_d... | → | 退出codes / exit_codes (code_dedup/exit_codes.py) | 测试依赖 / test_depends |
| 11 | D_GOVERNANCE 生命周期管理: Test Code Dedup Engine Red Team (code_quality/test_code_d... | → | 集成hub / integration_hub (code_dedup/integration_hub.py) | 测试依赖 / test_depends |
| 12 | D_GOVERNANCE 生命周期管理: Test Code Dedup Engine Red Team (code_quality/test_code_d... | → | monoculture守卫 / monoculture_guard (code_dedup/monocultu... | 测试依赖 / test_depends |
| 13 | D_GOVERNANCE 生命周期管理: Test Code Dedup Engine Red Team (code_quality/test_code_d... | → | 自扫描器 / self_scanner (code_dedup/self_scanner.py) | 测试依赖 / test_depends |
| 14 | D_GOVERNANCE 生命周期管理: Test Code Simulator (code_quality/test_code_simulator.py) | → | 代码模拟器 / code_simulator (code_dedup/code_simulator.py) | 测试依赖 / test_depends |
| 15 | D_GOVERNANCE 生命周期管理: Test Function Discovery (code_quality/test_function_disco... | → | 共享函数主动发现 — 签名+语义双通道从被动到主动. / functi... | 测试依赖 / test_depends |
| 16 | D_GOVERNANCE 生命周期管理: Test Simplicity Auditor (code_quality/test_simplicity_aud... | → | 引擎成本效益自审计器 — SAS 0-100 月度审计 + Tax 报告. / ... | 测试依赖 / test_depends |
...
| 18 | D_GOVERNANCE 生命周期管理: Test Thematic Clusterer (compliance/test_thematic_cluster... | → | 主题聚类器 — 噪声信号比·告警疲劳缓解. / thematic_cluste... | 测试依赖 / test_depends |
| 19 | D_GOVERNANCE 生命周期管理: Test Dedup Cache Manager (data_layer/test_dedup_cache_man... | → | 缓存管理器 / cache_manager (code_dedup/cache_manager.py) | 测试依赖 / test_depends |
| 20 | D_GOVERNANCE 生命周期管理: Test Symbol Index (data_layer/test_symbol_index.py) | → | symbol索引 / symbol_index (code_dedup/symbol_index.py) | 测试依赖 / test_depends |
| 21 | D_GOVERNANCE 生命周期管理: Test Behavioral Sampler (delegation/test_behavioral_sampl... | → | behavioral采样器 / behavioral_sampler (code_dedup/behavio... | 测试依赖 / test_depends |
| 22 | D_GOVERNANCE 生命周期管理: Test Behavioral Trust Checker (delegation/test_behavioral... | → | 行为信任检查器 — 行为漂移DIVERGED检测. / behavioral_trus... | 测试依赖 / test_depends |
| 23 | D_GOVERNANCE 生命周期管理: Test Consequence Tracker (delegation/test_consequence_tra... | → | 后果追踪——记录每次修复操作对依赖方的影响. / consequence... | 测试依赖 / test_depends |
| 24 | D_GOVERNANCE 生命周期管理: Test Shadow Trust Validator (delegation/test_shadow_trust... | → | 影子信任校验器 / shadow_trust_validator (code_dedup/shado... | 测试依赖 / test_depends |
| 25 | D_GOVERNANCE 生命周期管理: Test Dead Module Detector (drift/test_dead_module_detecto... | → | deadmodule检测器 / dead_module_detector (code_dedup/dead_... | 测试依赖 / test_depends |
| 26 | D_GOVERNANCE 生命周期管理: Test Diff Detector (drift/test_diff_detector.py) | → | 差异检测器 / diff_detector (code_dedup/diff_detector.py) | 测试依赖 / test_depends |
| 27 | D_GOVERNANCE 生命周期管理: Test Micro Clone Detector (drift/test_micro_clone_detecto... | → | microclone检测器 / micro_clone_detector (code_dedup/micro... | 测试依赖 / test_depends |
| 28 | D_GOVERNANCE 生命周期管理: Test Stale Shared Detector (drift/test_stale_shared_detec... | → | stale共享检测器 / stale_shared_detector (code_dedup/stale... | 测试依赖 / test_depends |
| 29 | D_GOVERNANCE 生命周期管理: Test Integration Hub (integration/test_integration_hub.py) | → | 集成hub / integration_hub (code_dedup/integration_hub.py) | 测试依赖 / test_depends |
| 30 | D_GOVERNANCE 生命周期管理: Test Integrations (integration/test_integrations.py) | → | 集成管理——预提交钩子+CI-only 扫描+超时边界. / integrati... | 测试依赖 / test_depends |
| 31 | D_GOVERNANCE 生命周期管理: Test Hotspot Tracker (observability/test_hotspot_tracker.py) | → | 热点追踪器 — 90天滑动窗口 + 高频变动检测 + 新项目预热清... | 测试依赖 / test_depends |
| 32 | D_GOVERNANCE 生命周期管理: Test Report (observability/test_report.py) | → | 报告 / report (code_dedup/report.py) | 测试依赖 / test_depends |
| 33 | D_GOVERNANCE 生命周期管理: Test Exit Codes (ops/test_exit_codes.py) | → | 退出codes / exit_codes (code_dedup/exit_codes.py) | 测试依赖 / test_depends |
| 34 | D_GOVERNANCE 生命周期管理: Test Health Monitor (ops/test_health_monitor.py) | → | 健康监控 / health_monitor (code_dedup/health_monitor.py) | 测试依赖 / test_depends |
| 35 | D_GOVERNANCE 生命周期管理: Test Success Validator (ops/test_success_validator.py) | → | 成功验证——判断一次去重操作是否真正消灭了克隆. / success... | 测试依赖 / test_depends |
| 36 | D_GOVERNANCE 生命周期管理: Test Verifier (ops/test_verifier.py) | → | 验证器 / verifier (code_dedup/verifier.py) | 测试依赖 / test_depends |
| 37 | D_GOVERNANCE 生命周期管理: Test Doom Loop Guard (resilience/test_doom_loop_guard.py) | → | doom循环守卫 / doom_loop_guard (code_dedup/doom_loop_guar... | 测试依赖 / test_depends |
| 38 | D_GOVERNANCE 生命周期管理: Test Observation Window Guard (resilience/test_observatio... | → | 提取后稳定观察期守护 — 对标SDP 14天观察. / observation_w... | 测试依赖 / test_depends |
| 39 | D_GOVERNANCE 生命周期管理: Test Recovery Manifest Writer (resilience/test_recovery_m... | → | 恢复清单写入器 / recovery_manifest_writer (code_dedup/rec... | 测试依赖 / test_depends |
| 40 | D_GOVERNANCE 生命周期管理: Test Extraction Safety (security/test_extraction_safety.py) | → | extraction安全 / extraction_safety (code_dedup/extraction... | 测试依赖 / test_depends |
| 41 | D_GOVERNANCE 生命周期管理: Test Import Surface Tracker (security/test_import_surface... | → | importsurface追踪器 / import_surface_tracker (trackers/im... | 测试依赖 / test_depends |
| 42 | D_GOVERNANCE 生命周期管理: Test Monoculture Guard (security/test_monoculture_guard.py) | → | monoculture守卫 / monoculture_guard (code_dedup/monocultu... | 测试依赖 / test_depends |
| 43 | D_GOVERNANCE 生命周期管理: Test Sensitivity Sweeper (security/test_sensitivity_sweep... | → | sensitivity清扫器 / sensitivity_sweeper (code_dedup/sensi... | 测试依赖 / test_depends |
| 44 | D_GOVERNANCE 生命周期管理: Test Signature Matcher (security/test_signature_matcher.py) | → | signature匹配器 / signature_matcher (code_dedup/signature... | 测试依赖 / test_depends |
| 45 | D_GOVERNANCE 生命周期管理: YAML + 环境变量覆盖 / Test App Config Yaml (shared/test_a... | → | 配置 / config (code_dedup/config.py) | 测试依赖 / test_depends |
| 46 | D_GOVERNANCE 生命周期管理: Test Shared Evolver (shared/test_shared_evolver.py) | → | 共享函数自我进化引擎 — 自动升降级 + 行为漂移锁定. / shar... | 测试依赖 / test_depends |
| 47 | D_GOVERNANCE 生命周期管理: Test Shared Lifecycle Manager (shared/test_shared_lifecyc... | → | 共享生命周期管理器 / shared_lifecycle_manager (code_dedup... | 测试依赖 / test_depends |
...
| 54 | D_GOV_AUDIT 审计追踪: Test False Negative Auditor (audit/test_false_negative_au... | → | falsenegative审计器 / false_negative_auditor (code_dedup/... | 测试依赖 / test_depends |
| 55 | D_GOV_AUDIT 审计追踪: Test Fifteen Dimension Auditor (audit/test_fifteen_dimens... | → | 15维超综合审计首页 — 逐项证明"做过且做对". / fifteen_dim... | 测试依赖 / test_depends |
...
| 59 | D_GOV_ENFORCEMENT 规则执行: Session Worktree (rule_bridge/session_worktree.py) | → | 包入口 / __init__ (commit_gates/__init__.py) | 导入依赖 / import_depends |
| 60 | D_GOV_ENFORCEMENT 规则执行: Session Worktree (rule_bridge/session_worktree.py) | → | capabilitylookuprequired门禁 / capability_lookup_required... | 导入依赖 / import_depends |
| 61 | D_GOV_ENFORCEMENT 规则执行: Session Worktree (rule_bridge/session_worktree.py) | → | 测试-源码符号一致性门禁 / Test Source Consistency Gate (c... | 导入依赖 / import_depends |
...
| 111 | D_GOV_ENFORCEMENT 规则执行: Test Open Without With Gate (commit_gates/test_open_witho... | → | openwithoutwith门禁 / open_without_with_gate (commit_gate... | 测试依赖 / test_depends |
...
| 120 | D_GOV_ENFORCEMENT 规则执行: Test Rule Execution Pairing Gate (commit_gates/test_rule_... | → | 规则-执行配对门禁 / Rule Execution Pairing Gate (commit_g... | 测试依赖 / test_depends |
...
| 139 | D_GOV_OPS_RESILIENCE 运维弹性治理: Test Debt Projector (budget/test_debt_projector.py) | → | debt投影器 / debt_projector (code_dedup/debt_projector.py) | 测试依赖 / test_depends |
| 140 | D_GOV_OPS_RESILIENCE 运维弹性治理: Test Degradation (budget/test_degradation.py) | → | 退化 / degradation (code_dedup/degradation.py) | 测试依赖 / test_depends |
| 141 | D_GOV_OPS_RESILIENCE 运维弹性治理: Test Prioritizer (orchestrator/test_prioritizer.py) | → | 修复优先级排序器 — 置信度×Impact×适配性 三因子排序. / ... | 测试依赖 / test_depends |
