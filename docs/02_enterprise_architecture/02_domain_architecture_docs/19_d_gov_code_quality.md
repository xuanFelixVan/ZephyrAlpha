...
| 模块数 | 205 | Module Count | 205 |
| 域内依赖 | 52 | Internal Dependencies | 52 |
| 跨域入边 | 135 | Cross-domain Incoming | 135 |
| 跨域出边 | 152 | Cross-domain Outgoing | 152 |
| 设计态模块 | 0 | Design Modules | 0 |
| 生产态模块 | 205 | Production Modules | 205 |
| 容量 | 206/150 (超容) | Capacity | 206/150 (超容) |
...
> 展示全部 205 个模块（生产态 205 + 设计态 0），含跨域依赖外部节点。节点含成熟度+名称+大白话/简介+文件路径。
...
flowchart TD
    ...
    tests_governance_rule_enforcement_test_end_to_end_walkthrough_py["rule_enforcement/test_end_to_end_walkthrough<br/>rule enforcement包的test_end_to_end_walkthrough<br/>模块<br/>文件: rule_enforcement<br/>/test_end_to_end_walkthrough.py<br/>(生产态 / production)"]
    ...
    tests_governance_rule_enforcement_test_breaking_change_detector_py ~~~ tests_governance_rule_enforcement_test_end_to_end_walkthrough_py
    tests_governance_rule_enforcement_test_end_to_end_walkthrough_py ~~~ tests_governance_rule_enforcement_test_integration_test_runner_py
    ...
    tests_governance_test_battle_map_execution_flow_py ~~~ tests_governance_test_generate_project_depgraph_smoke_py
    ...
    D_GOV_AUDIT -->|测试依赖 / test_depends| src_zephyr_gov_code_quality_code_dedup_false_negative_auditor_py
...
| 16 | test_sync_yaml_to_depgraph_smoke.py — sync_yaml_to_depgr... | → | D_GOVERNANCE 生命周期管理: 依赖图模式 / depgraph_schema (governance/depgraph_schema.py) | 测试依赖 / test_depends |
| 17 | panorama对齐门禁 / panorama_alignment_gate (commit_gates/... | → | D_GOV_AUDIT 审计追踪: 对账注册表 / reconciliation_registry (audit/reconciliatio... | 导入依赖 / import_depends |
| 18 | reconciler_health_gate.py — reconciler 健康度门禁（#ARCH... | → | D_GOV_AUDIT 审计追踪: 对账注册表 / reconciliation_registry (audit/reconciliatio... | 导入依赖 / import_depends |
| 19 | invariants/test_en_002_enforcement_validator.py | → | D_GOV_DRIFT 漂移检测: EN-002 — Enforcement Mode Validator (invariants/en_002_e... | 测试依赖 / test_depends |
| 20 | rule_enforcement/test_breaking_change_detector.py | → | D_GOV_DRIFT 漂移检测: Breaking Change 检测器（GATE-CDC-2）——字段删除/类型变更... | 测试依赖 / test_depends |
| 21 | reference辅助 / _reference_helpers (commit_gates/_referen... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 22 | archreference门禁 / arch_reference_gate (commit_gates/arc... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 23 | asynciorunin上下文门禁 / asyncio_run_in_context_gate (com... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 24 | baregetenv门禁 / bare_getenv_gate (commit_gates/bare_gete... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 25 | baresql门禁 / bare_sql_gate (commit_gates/bare_sql_gate.py) | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 26 | baresubprocess门禁 / bare_subprocess_gate (commit_gates/b... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 27 | 蓝图amoduleconsistency门禁 / blueprint_amodule_consistenc... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 28 | 蓝图amodule跨check门禁 / blueprint_amodule_cross_check_ga... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 29 | 蓝图format门禁 / blueprint_format_gate (commit_gates/blue... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 30 | 能力一致性门禁 / capability_consistency_gate (commit_gate... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 31 | capabilitylookuprequired门禁 / capability_lookup_required... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 32 | capabilityoverlap门禁 / capability_overlap_gate (commit_g... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 33 | ch批次大小门禁 / ch_batch_size_gate (commit_gates/ch_batc... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 34 | ch最终门禁 / ch_final_gate (commit_gates/ch_final_gate.py) | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 35 | ch版本col门禁 / ch_version_col_gate (commit_gates/ch_vers... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 36 | claimrequired门禁 / claim_required_gate (commit_gates/cla... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 37 | consumersaccuracy门禁 / consumers_accuracy_gate (commit_g... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 38 | 创建守卫 / create_guard (commit_gates/create_guard.py) | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 39 | danglingreference门禁 / dangling_reference_gate (commit_g... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 40 | 数据taskcompleteness门禁 / data_task_completeness_gate (c... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 41 | datetimenowforbidden门禁 / datetime_now_forbidden_gate (c... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 42 | depgraphfreshness门禁 / depgraph_freshness_gate (commit_g... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 43 | depgraph_pre_registration_gate.py — depgraph planned→pr... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 44 | depgraphwritepath门禁 / depgraph_write_path_gate (commit_... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 45 | derivationannotation门禁 / derivation_annotation_gate (co... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 46 | directorycontract门禁 / directory_contract_gate (commit_g... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 47 | docrefbroken门禁 / doc_ref_broken_gate (commit_gates/doc_... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 48 | 域fk门禁 / domain_fk_gate (commit_gates/domain_fk_gate.py) | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 49 | domainnamezhdirect访问门禁 / domain_name_zh_direct_access... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 50 | empty处理器门禁 / empty_handler_gate (commit_gates/empty_... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 51 | encoding门禁 / encoding_gate (commit_gates/encoding_gate.py) | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 52 | exemptzonefrontmatter门禁 / exempt_zone_frontmatter_gate ... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 53 | filecopy门禁 / file_copy_gate (commit_gates/file_copy_gat... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 54 | fileplacementttl门禁 / file_placement_ttl_gate (commit_ga... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 55 | folder容量hardlimit门禁 / folder_capacity_hard_limit_gate... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 56 | foreignchange门禁 / foreign_change_gate (commit_gates/for... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 57 | forgedgwmarker门禁 / forged_gw_marker_gate (commit_gates/... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 58 | 函数dup门禁 / function_dup_gate (commit_gates/function_du... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 59 | Gitcall预算门禁 / git_call_budget_gate (commit_gates/git_... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 60 | god类门禁 / god_class_gate (commit_gates/god_class_gate.py) | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 61 | hardcodedurl门禁 / hardcoded_url_gate (commit_gates/hardc... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 62 | heldoverlap门禁 / held_overlap_gate (commit_gates/held_ov... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 63 | highcomplexity门禁 / high_complexity_gate (commit_gates/h... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 64 | iduniqueness门禁 / id_uniqueness_gate (commit_gates/id_un... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 65 | importdirection门禁 / import_direction_gate (commit_gates... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 66 | 导入完整性门禁 / import_integrity_gate (commit_gates/impo... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 67 | issueresolved完整性门禁 / issue_resolved_integrity_gate (... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 68 | longparamlist门禁 / long_param_list_gate (commit_gates/lo... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 69 | 手册onlypermanent门禁 / manual_only_permanent_gate (commi... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 70 | MCP版本字段门禁 / mcp_version_field_gate (commit_gates/mc... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 71 | 模块id一致性门禁 / module_id_consistency_gate (commit_gat... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 72 | msg敞口门禁 / msg_exposure_gate (commit_gates/msg_exposur... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 73 | msgstyle门禁 / msg_style_gate (commit_gates/msg_style_gat... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 74 | mutableconstwithoutfinal门禁 / mutable_const_without_fina... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 75 | 新文件依赖图门禁 / new_file_depgraph_gate (commit_gates/n... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 76 | noimportsideeffect门禁 / no_import_side_effect_gate (comm... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 77 | noqa验证门禁 / noqa_validation_gate (commit_gates/noqa_va... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 78 | openwithoutwith门禁 / open_without_with_gate (commit_gate... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 79 | 孤儿module门禁 / orphan_module_gate (commit_gates/orphan_... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 80 | panorama对齐门禁 / panorama_alignment_gate (commit_gates/... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 81 | permtrigger门禁 / perm_trigger_gate (commit_gates/perm_tr... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 82 | precommitoffline门禁 / precommit_offline_gate (commit_gat... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 83 | pureassertion门禁 / pure_assertion_gate (commit_gates/pur... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 84 | pureshim门禁 / pure_shim_gate (commit_gates/pure_shim_gat... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 85 | r5digitsuffix门禁 / r5_digit_suffix_gate (commit_gates/r5... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 86 | reconciler_health_gate.py — reconciler 健康度门禁（#ARCH... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 87 | relative_path_literal_gate.py — 相对路径字面量硬阻断门禁... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 88 | rename_depgraph_sync_gate.py — 文件重命名后 depgraph 未... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 89 | rule_execution_pairing_gate.py — 规则-执行配对门禁（RULE... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 90 | rule_four_way_alignment_gate.py — 规则四方对齐门禁（RULE... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 91 | ruling_commit_verified_gate.py — 文档"已完成"声明 commit... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 92 | ruling_reference_gate.py — 裁定#NNN 悬空引用自动检测门禁... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 93 | schema_file_exists_gate.py — SCHEMA-FILE-EXISTS block 门... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 94 | scripts_import_integrity_gate.py — _shared.constants 符... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 95 | session_required_gate.py — session 注册强制门禁（SESSION... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 96 | snapshot_drift_gate.py — 运行时违规快照漂移阻断门禁（SNA... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 97 | ssot_redefinition_gate.py — SSoT 符号重复定义硬阻断门禁 ... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 98 | table_name_registry_gate.py — TABLE-NAME-REGISTRY block ... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 99 | test_source_consistency_gate.py — 测试-源码符号一致性门... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 100 | tests_coverage_gate.py — Gate 测试覆盖率校验 meta-gate（... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 101 | translation_coverage_gate.py — 新建 .py 文件大白话简介覆... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 102 | ttl_gate.py — ttl 字段校验门禁（治本：弥补 --no-verify ... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 103 | undefined_name_gate.py — UNDEFINED-NAME 门禁（F821 未定... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 104 | unsafe_dict_spread_gate.py — ``**data`` 直接展开模式 war... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 105 | vocab_chain_gate.py — SSoT 引用硬编码阻断门禁（VOCAB-CHA... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 106 | vocab_hardcode_gate.py — 新增 .py 文件词表硬编码阻断门禁... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 107 | zephyr_env_direct_access_gate.py — ZEPHYR_ENV 直访硬阻断... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 108 | gate_auto_registrar.py — YAML 驱动的 in-process gate 自... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 109 | rule_enforcement/test_output_quality_gate.py | → | D_GOV_ENFORCEMENT 规则执行: rule_enforcement/output_quality_gate.py | 测试依赖 / test_depends |
| 110 | test_audit_worktree_ops_telemetry.py — worktree_ops_log ... | → | D_GOV_ENFORCEMENT 规则执行: session_worktree.py — AI 对话 worktree 物理隔离 helper（... | 测试依赖 / test_depends |
| 111 | governance_misc/test_bare_repo_scanner.py | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Bare Repo Scanner — v0.14.0 嵌入式裸仓库检测器。 (securi... | 测试依赖 / test_depends |
| 112 | governance_misc/test_governance_result_types.py | → | D_GOV_OPS_RESILIENCE 运维弹性治理: G-CT-003 — RollbackResult backward-compat re-export faca... | 测试依赖 / test_depends |
| 113 | gate_engine/test_adversarial_gate_integration.py | → | D_GOV_RULE 规则治理: 对抗样本生成器 / Adversarial Strategies (rule_enforcement... | 测试依赖 / test_depends |
| 114 | gate_engine/test_adversarial_gate_integration.py | → | D_GOV_RULE 规则治理: 对抗验证门禁 / Adversarial Validation (gate_engine/advers... | 测试依赖 / test_depends |
| 115 | gate_engine/test_adversarial_validation_gate.py | → | D_GOV_RULE 规则治理: 对抗验证门禁 / Adversarial Validation (gate_engine/advers... | 测试依赖 / test_depends |
| 116 | invariants/test_en_001_circular_dependency.py | → | D_GOV_RULE 规则治理: 循环依赖扫描器 / Circular Dependency Scanner (invariants/... | 测试依赖 / test_depends |
| 117 | invariants/test_en_003_contract_compatibility.py | → | D_GOV_RULE 规则治理: 契约兼容性检查器 / Contract Compatibility Checker (invari... | 测试依赖 / test_depends |
| 118 | invariants/test_en_process_lifecycle_gateway.py | → | D_GOV_RULE 规则治理: 进程生命周期网关 / Process Lifecycle Gateway (invariants/... | 测试依赖 / test_depends |
| 119 | invariants/test_zero_residue_check.py | → | D_GOV_RULE 规则治理: 零残留检查器 / Zero Residue Check (invariants/zero_residu... | 测试依赖 / test_depends |
| 120 | rule_enforcement/test_adaptive_threshold.py | → | D_GOV_RULE 规则治理: 自适应阈值 / Adaptive Threshold (rule_enforcement/adaptiv... | 测试依赖 / test_depends |
| 121 | rule_enforcement/test_adversarial_strategies.py | → | D_GOV_RULE 规则治理: 对抗样本生成器 / Adversarial Strategies (rule_enforcement... | 测试依赖 / test_depends |
| 122 | rule_enforcement/test_end_to_end_walkthrough.py | → | D_GOV_RULE 规则治理: 端到端走查验证器 / End-to-End Walkthrough (rule_enforceme... | 测试依赖 / test_depends |
| 123 | rule_enforcement/test_integration_test_runner.py | → | D_GOV_RULE 规则治理: 集成测试运行器 / Integration Test Runner (rule_enforcemen... | 测试依赖 / test_depends |
| 124 | rule_enforcement/test_kiss_enforcer.py | → | D_GOV_RULE 规则治理: KISS 约束执行器 / KISS Enforcer (rule_enforcement/kiss_en... | 测试依赖 / test_depends |
| 125 | rule_enforcement/test_secrets_guard.py | → | D_GOV_RULE 规则治理: 密钥守卫 / Secrets Guard (rule_enforcement/secrets_guard.py) | 测试依赖 / test_depends |
| 126 | rule_enforcement/test_triple_alignment.py | → | D_GOV_RULE 规则治理: 三方对齐门禁 / Triple Alignment (rule_enforcement/triple_... | 测试依赖 / test_depends |
| 127 | 检查模块id一致性 / check_module_id_consistency (d7_code/c... | → | D_GOV_SCRIPTS 脚本治理: constants.py — 审计脚本共享常量 (_shared/constants.py) | 导入依赖 / import_depends |
| 128 | translation_coverage_gate.py — 新建 .py 文件大白话简介覆... | → | D_GOV_SCRIPTS 脚本治理: module_translation_loader.py — 模块级翻译共享加载器（SSo... | 导入依赖 / import_depends |
| 129 | 配置 / config (code_dedup/config.py) | → | D_INFRASTRUCTURE 跨层契约基础设施: app_config.py — 应用配置数据类与加载/热重载逻辑 (config/... | 导入依赖 / import_depends |
| 130 | 命令行 / cli (code_dedup/cli.py) | → | D_INFRA_RUNTIME 运行时集成: AssetDiscoveryScanner — MOD-INF-026 L1 全量文件系统扫描... | 导入依赖 / import_depends |
| 131 | forgedgwmarker门禁 / forged_gw_marker_gate (commit_gates/... | → | D_SECURITY 对抗验证: Session 级并发协调模块（P2-SES 落地）。 (access_control/s... | 导入依赖 / import_depends |
| 132 | 导入完整性门禁 / import_integrity_gate (commit_gates/impo... | → | D_SECURITY 对抗验证: Session 级并发协调模块（P2-SES 落地）。 (access_control/s... | 导入依赖 / import_depends |
| 133 | 缓存管理器 / cache_manager (code_dedup/cache_manager.py) | → | D_SHARED 共享服务: serialization.py —— 统一序列化/反序列化基础设施（Phase ... | 导入依赖 / import_depends |
| 134 | 差异检测器 / diff_detector (code_dedup/diff_detector.py) | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP servers and... | 导入依赖 / import_depends |
| 135 | reference辅助 / _reference_helpers (commit_gates/_referen... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP servers and... | 导入依赖 / import_depends |
| 136 | baregetenv门禁 / bare_getenv_gate (commit_gates/bare_gete... | → | D_SHARED 共享服务: secrets.py —— Secrets 管理抽象（Phase 7 新增 | 盲点 B12... | 导入依赖 / import_depends |
| 137 | 蓝图format门禁 / blueprint_format_gate (commit_gates/blue... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 138 | capabilitylookuprequired门禁 / capability_lookup_required... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 139 | 创建守卫 / create_guard (commit_gates/create_guard.py) | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 140 | 数据taskcompleteness门禁 / data_task_completeness_gate (c... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP servers and... | 导入依赖 / import_depends |
| 141 | encoding门禁 / encoding_gate (commit_gates/encoding_gate.py) | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP servers and... | 导入依赖 / import_depends |
| 142 | exemptzonefrontmatter门禁 / exempt_zone_frontmatter_gate ... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP servers and... | 导入依赖 / import_depends |
| 143 | 门禁repo / gate_repo (commit_gates/gate_repo.py) | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 144 | 门禁repo / gate_repo (commit_gates/gate_repo.py) | → | D_SHARED 共享服务: db_utils.py — SQLite 连接公共 API（SSoT: zephyr.governan... | 导入依赖 / import_depends |
| 145 | pureassertion门禁 / pure_assertion_gate (commit_gates/pur... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP servers and... | 导入依赖 / import_depends |
| 146 | pureshim门禁 / pure_shim_gate (commit_gates/pure_shim_gat... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP servers and... | 导入依赖 / import_depends |
| 147 | r5digitsuffix门禁 / r5_digit_suffix_gate (commit_gates/r5... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP servers and... | 导入依赖 / import_depends |
| 148 | ruling_commit_verified_gate.py — 文档"已完成"声明 commit... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP servers and... | 导入依赖 / import_depends |
| 149 | scripts_import_integrity_gate.py — _shared.constants 符... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 150 | test_source_consistency_gate.py — 测试-源码符号一致性门... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 151 | translation_coverage_gate.py — 新建 .py 文件大白话简介覆... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 152 | gate_auto_registrar.py — YAML 驱动的 in-process gate 自... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
...
| 17 | D_GOVERNANCE 生命周期管理: compliance/test_thematic_clusterer.py | → | 主题聚类器 — 噪声信号比·告警疲劳缓解. / thematic_cluste... | 测试依赖 / test_depends |
| 18 | D_GOVERNANCE 生命周期管理: data_layer/test_cache_manager.py | → | 缓存管理器 / cache_manager (code_dedup/cache_manager.py) | 测试依赖 / test_depends |
| 19 | D_GOVERNANCE 生命周期管理: data_layer/test_symbol_index.py | → | symbol索引 / symbol_index (code_dedup/symbol_index.py) | 测试依赖 / test_depends |
| 20 | D_GOVERNANCE 生命周期管理: delegation/test_behavioral_sampler.py | → | behavioral采样器 / behavioral_sampler (code_dedup/behavio... | 测试依赖 / test_depends |
| 21 | D_GOVERNANCE 生命周期管理: delegation/test_behavioral_trust_checker.py | → | 行为信任检查器 — 行为漂移DIVERGED检测. / behavioral_trus... | 测试依赖 / test_depends |
| 22 | D_GOVERNANCE 生命周期管理: delegation/test_consequence_tracker.py | → | 后果追踪——记录每次修复操作对依赖方的影响. / consequence... | 测试依赖 / test_depends |
| 23 | D_GOVERNANCE 生命周期管理: delegation/test_shadow_trust_validator.py | → | 影子信任校验器 / shadow_trust_validator (code_dedup/shado... | 测试依赖 / test_depends |
| 24 | D_GOVERNANCE 生命周期管理: drift/test_dead_module_detector.py | → | deadmodule检测器 / dead_module_detector (code_dedup/dead_... | 测试依赖 / test_depends |
| 25 | D_GOVERNANCE 生命周期管理: drift/test_diff_detector.py | → | 差异检测器 / diff_detector (code_dedup/diff_detector.py) | 测试依赖 / test_depends |
| 26 | D_GOVERNANCE 生命周期管理: drift/test_micro_clone_detector.py | → | microclone检测器 / micro_clone_detector (code_dedup/micro... | 测试依赖 / test_depends |
| 27 | D_GOVERNANCE 生命周期管理: drift/test_stale_shared_detector.py | → | stale共享检测器 / stale_shared_detector (code_dedup/stale... | 测试依赖 / test_depends |
| 28 | D_GOVERNANCE 生命周期管理: integration/test_integration_hub.py | → | 集成hub / integration_hub (code_dedup/integration_hub.py) | 测试依赖 / test_depends |
| 29 | D_GOVERNANCE 生命周期管理: integration/test_integrations.py | → | 集成管理——预提交钩子+CI-only 扫描+超时边界. / integrati... | 测试依赖 / test_depends |
| 30 | D_GOVERNANCE 生命周期管理: observability/test_hotspot_tracker.py | → | 热点追踪器 — 90天滑动窗口 + 高频变动检测 + 新项目预热清... | 测试依赖 / test_depends |
| 31 | D_GOVERNANCE 生命周期管理: observability/test_report.py | → | 报告 / report (code_dedup/report.py) | 测试依赖 / test_depends |
| 32 | D_GOVERNANCE 生命周期管理: ops/test_exit_codes.py | → | 退出codes / exit_codes (code_dedup/exit_codes.py) | 测试依赖 / test_depends |
| 33 | D_GOVERNANCE 生命周期管理: ops/test_health_monitor.py | → | 健康监控 / health_monitor (code_dedup/health_monitor.py) | 测试依赖 / test_depends |
| 34 | D_GOVERNANCE 生命周期管理: ops/test_success_validator.py | → | 成功验证——判断一次去重操作是否真正消灭了克隆. / success... | 测试依赖 / test_depends |
| 35 | D_GOVERNANCE 生命周期管理: ops/test_verifier.py | → | 验证器 / verifier (code_dedup/verifier.py) | 测试依赖 / test_depends |
| 36 | D_GOVERNANCE 生命周期管理: resilience/test_doom_loop_guard.py | → | doom循环守卫 / doom_loop_guard (code_dedup/doom_loop_guar... | 测试依赖 / test_depends |
| 37 | D_GOVERNANCE 生命周期管理: resilience/test_observation_window_guard.py | → | 提取后稳定观察期守护 — 对标SDP 14天观察. / observation_w... | 测试依赖 / test_depends |
| 38 | D_GOVERNANCE 生命周期管理: resilience/test_recovery_manifest_writer.py | → | 恢复清单写入器 / recovery_manifest_writer (code_dedup/rec... | 测试依赖 / test_depends |
| 39 | D_GOVERNANCE 生命周期管理: security/test_extraction_safety.py | → | extraction安全 / extraction_safety (code_dedup/extraction... | 测试依赖 / test_depends |
| 40 | D_GOVERNANCE 生命周期管理: security/test_import_surface_tracker.py | → | importsurface追踪器 / import_surface_tracker (trackers/im... | 测试依赖 / test_depends |
| 41 | D_GOVERNANCE 生命周期管理: security/test_monoculture_guard.py | → | monoculture守卫 / monoculture_guard (code_dedup/monocultu... | 测试依赖 / test_depends |
| 42 | D_GOVERNANCE 生命周期管理: security/test_sensitivity_sweeper.py | → | sensitivity清扫器 / sensitivity_sweeper (code_dedup/sensi... | 测试依赖 / test_depends |
| 43 | D_GOVERNANCE 生命周期管理: security/test_signature_matcher.py | → | signature匹配器 / signature_matcher (code_dedup/signature... | 测试依赖 / test_depends |
| 44 | D_GOVERNANCE 生命周期管理: shared/test_app_config_yaml.py | → | 配置 / config (code_dedup/config.py) | 测试依赖 / test_depends |
| 45 | D_GOVERNANCE 生命周期管理: shared/test_shared_evolver.py | → | 共享函数自我进化引擎 — 自动升降级 + 行为漂移锁定. / shar... | 测试依赖 / test_depends |
| 46 | D_GOVERNANCE 生命周期管理: shared/test_shared_lifecycle_manager.py | → | 共享生命周期管理器 / shared_lifecycle_manager (code_dedup... | 测试依赖 / test_depends |
| 47 | D_GOV_AUDIT 审计追踪: 对账注册表 / reconciliation_registry (audit/reconciliatio... | → | reference辅助 / _reference_helpers (commit_gates/_referen... | 导入依赖 / import_depends |
| 48 | D_GOV_AUDIT 审计追踪: 对账注册表 / reconciliation_registry (audit/reconciliatio... | → | capabilitylookup绕过策略 / capability_lookup_bypass_polic... | 导入依赖 / import_depends |
| 49 | D_GOV_AUDIT 审计追踪: 对账注册表 / reconciliation_registry (audit/reconciliatio... | → | consumersaccuracy门禁 / consumers_accuracy_gate (commit_g... | 导入依赖 / import_depends |
| 50 | D_GOV_AUDIT 审计追踪: 对账注册表 / reconciliation_registry (audit/reconciliatio... | → | scripts_import_integrity_gate.py — _shared.constants 符... | 导入依赖 / import_depends |
| 51 | D_GOV_AUDIT 审计追踪: 对账注册表 / reconciliation_registry (audit/reconciliatio... | → | undefined_name_gate.py — UNDEFINED-NAME 门禁（F821 未定... | 导入依赖 / import_depends |
| 52 | D_GOV_AUDIT 审计追踪: 对账注册表 / reconciliation_registry (audit/reconciliatio... | → | gate_auto_registrar.py — YAML 驱动的 in-process gate 自... | 导入依赖 / import_depends |
| 53 | D_GOV_AUDIT 审计追踪: audit/test_false_negative_auditor.py | → | falsenegative审计器 / false_negative_auditor (code_dedup/... | 测试依赖 / test_depends |
| 54 | D_GOV_AUDIT 审计追踪: audit/test_fifteen_dimension_auditor.py | → | 15维超综合审计首页 — 逐项证明"做过且做对". / fifteen_dim... | 测试依赖 / test_depends |
| 55 | D_GOV_AUDIT 审计追踪: test_undefined_name_baseline_reconciler.py — GATE-UNDEFI... | → | 包入口 / __init__ (commit_gates/__init__.py) | 测试依赖 / test_depends |
| 56 | D_GOV_AUDIT 审计追踪: test_undefined_name_baseline_reconciler.py — GATE-UNDEFI... | → | undefined_name_gate.py — UNDEFINED-NAME 门禁（F821 未定... | 测试依赖 / test_depends |
| 57 | D_GOV_ENFORCEMENT 规则执行: GitCommitGateway — 全项目唯一合法 git commit 入口（OPS-2... | → | gate_auto_registrar.py — YAML 驱动的 in-process gate 自... | 导入依赖 / import_depends |
| 58 | D_GOV_ENFORCEMENT 规则执行: session_worktree.py — AI 对话 worktree 物理隔离 helper（... | → | 包入口 / __init__ (commit_gates/__init__.py) | 导入依赖 / import_depends |
| 59 | D_GOV_ENFORCEMENT 规则执行: session_worktree.py — AI 对话 worktree 物理隔离 helper（... | → | capabilitylookuprequired门禁 / capability_lookup_required... | 导入依赖 / import_depends |
| 60 | D_GOV_ENFORCEMENT 规则执行: session_worktree.py — AI 对话 worktree 物理隔离 helper（... | → | test_source_consistency_gate.py — 测试-源码符号一致性门... | 导入依赖 / import_depends |
| 61 | D_GOV_ENFORCEMENT 规则执行: test_arch_reference_gate.py — #ARCH-NNN 悬空引用检测门禁... | → | reference辅助 / _reference_helpers (commit_gates/_referen... | 测试依赖 / test_depends |
| 62 | D_GOV_ENFORCEMENT 规则执行: test_arch_reference_gate.py — #ARCH-NNN 悬空引用检测门禁... | → | archreference门禁 / arch_reference_gate (commit_gates/arc... | 测试依赖 / test_depends |
| 63 | D_GOV_ENFORCEMENT 规则执行: test_asyncio_run_in_context_gate.py — asyncio API 误用硬... | → | asynciorunin上下文门禁 / asyncio_run_in_context_gate (com... | 测试依赖 / test_depends |
| 64 | D_GOV_ENFORCEMENT 规则执行: test_bare_getenv_gate.py — NO-BARE-GETENV 门禁单测 (comm... | → | baregetenv门禁 / bare_getenv_gate (commit_gates/bare_gete... | 测试依赖 / test_depends |
| 65 | D_GOV_ENFORCEMENT 规则执行: test_bare_sql_gate.py — NO-BARE-SQL 门禁单测 (commit_gat... | → | baresql门禁 / bare_sql_gate (commit_gates/bare_sql_gate.py) | 测试依赖 / test_depends |
| 66 | D_GOV_ENFORCEMENT 规则执行: test_bare_subprocess_gate.py — BARE-SUBPROCESS 门禁单测 ... | → | baresubprocess门禁 / bare_subprocess_gate (commit_gates/b... | 测试依赖 / test_depends |
| 67 | D_GOV_ENFORCEMENT 规则执行: test_blueprint_amodule_consistency_gate.py — BLUEPRINT-A... | → | 蓝图amoduleconsistency门禁 / blueprint_amodule_consistenc... | 测试依赖 / test_depends |
| 68 | D_GOV_ENFORCEMENT 规则执行: test_blueprint_amodule_cross_check_gate.py — BLUEPRINT-A... | → | 蓝图amodule跨check门禁 / blueprint_amodule_cross_check_ga... | 测试依赖 / test_depends |
| 69 | D_GOV_ENFORCEMENT 规则执行: test_capability_lookup_audit_log.py — capability_lookup ... | → | capabilitylookuprequired门禁 / capability_lookup_required... | 测试依赖 / test_depends |
| 70 | D_GOV_ENFORCEMENT 规则执行: test_capability_lookup_bypass_policy.py — CAPABILITY-LOO... | → | 包入口 / __init__ (commit_gates/__init__.py) | 测试依赖 / test_depends |
| 71 | D_GOV_ENFORCEMENT 规则执行: test_capability_lookup_bypass_policy.py — CAPABILITY-LOO... | → | capabilitylookup绕过策略 / capability_lookup_bypass_polic... | 测试依赖 / test_depends |
| 72 | D_GOV_ENFORCEMENT 规则执行: test_capability_lookup_required_gate.py — CAPABILITY-LOO... | → | capabilitylookuprequired门禁 / capability_lookup_required... | 测试依赖 / test_depends |
| 73 | D_GOV_ENFORCEMENT 规则执行: test_capability_overlap_gate.py — CAPABILITY-OVERLAP 门... | → | capabilityoverlap门禁 / capability_overlap_gate (commit_g... | 测试依赖 / test_depends |
| 74 | D_GOV_ENFORCEMENT 规则执行: test_ch_batch_size_gate.py — CH-BATCH-SIZE 门禁单测 (com... | → | ch批次大小门禁 / ch_batch_size_gate (commit_gates/ch_batc... | 测试依赖 / test_depends |
| 75 | D_GOV_ENFORCEMENT 规则执行: test_ch_version_col_gate.py — CH-VERSION-COL 门禁单测 (c... | → | ch版本col门禁 / ch_version_col_gate (commit_gates/ch_vers... | 测试依赖 / test_depends |
| 76 | D_GOV_ENFORCEMENT 规则执行: test_claim_required_gate.py — claim_files 前置检查门禁单... | → | claimrequired门禁 / claim_required_gate (commit_gates/cla... | 测试依赖 / test_depends |
| 77 | D_GOV_ENFORCEMENT 规则执行: test_consumers_accuracy_gate.py — CONSUMERS-ACCURACY 门... | → | consumersaccuracy门禁 / consumers_accuracy_gate (commit_g... | 测试依赖 / test_depends |
| 78 | D_GOV_ENFORCEMENT 规则执行: test_create_guard.py — CREATE-GUARD 门禁单元测试（2026-0... | → | 创建守卫 / create_guard (commit_gates/create_guard.py) | 测试依赖 / test_depends |
| 79 | D_GOV_ENFORCEMENT 规则执行: test_dangling_reference_gate.py — AGENTS.md §X.Y 悬空引... | → | danglingreference门禁 / dangling_reference_gate (commit_g... | 测试依赖 / test_depends |
| 80 | D_GOV_ENFORCEMENT 规则执行: test_datetime_now_forbidden_gate.py — 生成器代码 datetim... | → | datetimenowforbidden门禁 / datetime_now_forbidden_gate (c... | 测试依赖 / test_depends |
| 81 | D_GOV_ENFORCEMENT 规则执行: test_depgraph_freshness_gate.py — DEPGRAPH-FRESHNESS 门... | → | depgraphfreshness门禁 / depgraph_freshness_gate (commit_g... | 测试依赖 / test_depends |
| 82 | D_GOV_ENFORCEMENT 规则执行: test_depgraph_pre_registration_gate.py — DEPGRAPH-PRE-RE... | → | depgraph_pre_registration_gate.py — depgraph planned→pr... | 测试依赖 / test_depends |
| 83 | D_GOV_ENFORCEMENT 规则执行: test_doc_ref_broken_gate.py — DOC-REF-BROKEN 门禁单测 (c... | → | docrefbroken门禁 / doc_ref_broken_gate (commit_gates/doc_... | 测试依赖 / test_depends |
| 84 | D_GOV_ENFORCEMENT 规则执行: test_domain_fk_gate.py — GATE-DOMAIN-FK 门禁单测 (commit... | → | 域fk门禁 / domain_fk_gate (commit_gates/domain_fk_gate.py) | 测试依赖 / test_depends |
| 85 | D_GOV_ENFORCEMENT 规则执行: test_domain_name_zh_direct_access_gate.py — NO-DOMAIN-NA... | → | domainnamezhdirect访问门禁 / domain_name_zh_direct_access... | 测试依赖 / test_depends |
| 86 | D_GOV_ENFORCEMENT 规则执行: test_empty_handler_gate.py — EMPTY-HANDLER 门禁单测 (com... | → | empty处理器门禁 / empty_handler_gate (commit_gates/empty_... | 测试依赖 / test_depends |
| 87 | D_GOV_ENFORCEMENT 规则执行: test_exempt_zone_frontmatter_gate.py — EXEMPT-ZONE-FM 门... | → | exemptzonefrontmatter门禁 / exempt_zone_frontmatter_gate ... | 测试依赖 / test_depends |
| 88 | D_GOV_ENFORCEMENT 规则执行: test_file_copy_gate.py — FILE-COPY 门禁单测 (commit_gate... | → | filecopy门禁 / file_copy_gate (commit_gates/file_copy_gat... | 测试依赖 / test_depends |
| 89 | D_GOV_ENFORCEMENT 规则执行: test_foreign_change_gate.py — 外来变更检测门禁单测（FORE... | → | foreignchange门禁 / foreign_change_gate (commit_gates/for... | 测试依赖 / test_depends |
| 90 | D_GOV_ENFORCEMENT 规则执行: test_forged_gw_marker_gate.py — Forged GW Marker 前置检... | → | forgedgwmarker门禁 / forged_gw_marker_gate (commit_gates/... | 测试依赖 / test_depends |
| 91 | D_GOV_ENFORCEMENT 规则执行: test_function_dup_gate.py — FUNCTION-DUP 门禁单测 (commi... | → | 函数dup门禁 / function_dup_gate (commit_gates/function_du... | 测试依赖 / test_depends |
| 92 | D_GOV_ENFORCEMENT 规则执行: test_god_class_gate.py — NO-GOD-CLASS 门禁单测 (commit_g... | → | god类门禁 / god_class_gate (commit_gates/god_class_gate.py) | 测试依赖 / test_depends |
| 93 | D_GOV_ENFORCEMENT 规则执行: test_hardcoded_url_gate.py — NO-HARDCODED-URL 门禁单测 (... | → | hardcodedurl门禁 / hardcoded_url_gate (commit_gates/hardc... | 测试依赖 / test_depends |
| 94 | D_GOV_ENFORCEMENT 规则执行: test_held_overlap_gate.py — 搭便车防护门禁单测（HELD-OVE... | → | heldoverlap门禁 / held_overlap_gate (commit_gates/held_ov... | 测试依赖 / test_depends |
| 95 | D_GOV_ENFORCEMENT 规则执行: test_high_complexity_gate.py — NO-HIGH-COMPLEXITY 门禁单... | → | highcomplexity门禁 / high_complexity_gate (commit_gates/h... | 测试依赖 / test_depends |
| 96 | D_GOV_ENFORCEMENT 规则执行: test_id_uniqueness_gate.py — ID-UNIQUENESS 门禁单测 (com... | → | iduniqueness门禁 / id_uniqueness_gate (commit_gates/id_un... | 测试依赖 / test_depends |
| 97 | D_GOV_ENFORCEMENT 规则执行: test_import_direction_gate.py — NO-UPWARD-IMPORT 门禁单... | → | importdirection门禁 / import_direction_gate (commit_gates... | 测试依赖 / test_depends |
| 98 | D_GOV_ENFORCEMENT 规则执行: test_import_integrity_gate.py — IMPORT-INTEGRITY 门禁单... | → | 导入完整性门禁 / import_integrity_gate (commit_gates/impo... | 测试依赖 / test_depends |
| 99 | D_GOV_ENFORCEMENT 规则执行: test_long_param_list_gate.py — NO-LONG-PARAM-LIST 门禁单... | → | longparamlist门禁 / long_param_list_gate (commit_gates/lo... | 测试依赖 / test_depends |
| 100 | D_GOV_ENFORCEMENT 规则执行: test_manual_only_permanent_gate_noqa.py — MANUAL-ONLY-PE... | → | 手册onlypermanent门禁 / manual_only_permanent_gate (commi... | 测试依赖 / test_depends |
| 101 | D_GOV_ENFORCEMENT 规则执行: test_mcp_version_field_gate.py — MCP version 字段缺失硬... | → | MCP版本字段门禁 / mcp_version_field_gate (commit_gates/mc... | 测试依赖 / test_depends |
| 102 | D_GOV_ENFORCEMENT 规则执行: test_module_id_consistency_gate.py — module_id 三声明轨... | → | 模块id一致性门禁 / module_id_consistency_gate (commit_gat... | 测试依赖 / test_depends |
| 103 | D_GOV_ENFORCEMENT 规则执行: test_msg_exposure_gate.py — MSG-EXPOSURE 门禁单测 (commi... | → | msg敞口门禁 / msg_exposure_gate (commit_gates/msg_exposur... | 测试依赖 / test_depends |
| 104 | D_GOV_ENFORCEMENT 规则执行: test_msg_style_gate.py — MSG-STYLE 门禁单测 (commit_gate... | → | msgstyle门禁 / msg_style_gate (commit_gates/msg_style_gat... | 测试依赖 / test_depends |
| 105 | D_GOV_ENFORCEMENT 规则执行: test_mutable_const_without_final_gate.py — 可变常量缺 Fi... | → | mutableconstwithoutfinal门禁 / mutable_const_without_fina... | 测试依赖 / test_depends |
| 106 | D_GOV_ENFORCEMENT 规则执行: test_new_file_depgraph_gate.py — NEW-FILE-DEPGRAPH-ENFOR... | → | 新文件依赖图门禁 / new_file_depgraph_gate (commit_gates/n... | 测试依赖 / test_depends |
| 107 | D_GOV_ENFORCEMENT 规则执行: test_no_import_side_effect_gate.py — NO-IMPORT-SIDE-EFFE... | → | noimportsideeffect门禁 / no_import_side_effect_gate (comm... | 测试依赖 / test_depends |
| 108 | D_GOV_ENFORCEMENT 规则执行: test_open_without_with_gate.py — open() 未在 with 内硬阻... | → | openwithoutwith门禁 / open_without_with_gate (commit_gate... | 测试依赖 / test_depends |
| 109 | D_GOV_ENFORCEMENT 规则执行: test_orphan_module_gate.py — ORPHAN-MODULE 门禁单测 (com... | → | 孤儿module门禁 / orphan_module_gate (commit_gates/orphan_... | 测试依赖 / test_depends |
| 110 | D_GOV_ENFORCEMENT 规则执行: test_panorama_alignment_gate.py — 四图模块对齐门禁单测（... | → | panorama对齐门禁 / panorama_alignment_gate (commit_gates/... | 测试依赖 / test_depends |
| 111 | D_GOV_ENFORCEMENT 规则执行: test_perm_trigger_gate.py — PERM-TRIGGER 门禁单测 (commi... | → | permtrigger门禁 / perm_trigger_gate (commit_gates/perm_tr... | 测试依赖 / test_depends |
| 112 | D_GOV_ENFORCEMENT 规则执行: test_precommit_offline_gate.py — GATE-PRECOMMIT-OFFLINE ... | → | precommitoffline门禁 / precommit_offline_gate (commit_gat... | 测试依赖 / test_depends |
| 113 | D_GOV_ENFORCEMENT 规则执行: test_r5_digit_suffix_gate.py — R5-DIGIT-SUFFIX 门禁单元... | → | r5digitsuffix门禁 / r5_digit_suffix_gate (commit_gates/r5... | 测试依赖 / test_depends |
| 114 | D_GOV_ENFORCEMENT 规则执行: test_reconciler_health_gate.py — RECONCILER-HEALTH 门禁... | → | reconciler_health_gate.py — reconciler 健康度门禁（#ARCH... | 测试依赖 / test_depends |
| 115 | D_GOV_ENFORCEMENT 规则执行: test_rename_depgraph_sync_gate.py — RENAME-DEPGRAPH-SYNC... | → | rename_depgraph_sync_gate.py — 文件重命名后 depgraph 未... | 测试依赖 / test_depends |
| 116 | D_GOV_ENFORCEMENT 规则执行: Tests for RULE-EXECUTION-PAIRING gate (Phase 3.5). (commi... | → | rule_execution_pairing_gate.py — 规则-执行配对门禁（RULE... | 测试依赖 / test_depends |
| 117 | D_GOV_ENFORCEMENT 规则执行: test_rule_four_way_alignment_gate.py — RULE-FOUR-WAY-ALI... | → | rule_four_way_alignment_gate.py — 规则四方对齐门禁（RULE... | 测试依赖 / test_depends |
| 118 | D_GOV_ENFORCEMENT 规则执行: test_ruling_commit_verified_gate.py — RULING-COMMIT-VERI... | → | ruling_commit_verified_gate.py — 文档"已完成"声明 commit... | 测试依赖 / test_depends |
| 119 | D_GOV_ENFORCEMENT 规则执行: test_ruling_reference_gate.py — 裁定#NNN 悬空引用检测门... | → | ruling_reference_gate.py — 裁定#NNN 悬空引用自动检测门禁... | 测试依赖 / test_depends |
| 120 | D_GOV_ENFORCEMENT 规则执行: test_schema_file_exists_gate.py — SCHEMA-FILE-EXISTS 门... | → | schema_file_exists_gate.py — SCHEMA-FILE-EXISTS block 门... | 测试依赖 / test_depends |
| 121 | D_GOV_ENFORCEMENT 规则执行: test_scripts_import_integrity_gate.py — SCRIPTS-IMPORT-I... | → | scripts_import_integrity_gate.py — _shared.constants 符... | 测试依赖 / test_depends |
| 122 | D_GOV_ENFORCEMENT 规则执行: test_session_required_gate.py — SESSION-REQUIRED 门禁单... | → | session_required_gate.py — session 注册强制门禁（SESSION... | 测试依赖 / test_depends |
| 123 | D_GOV_ENFORCEMENT 规则执行: test_ssot_redefinition_gate.py — SSoT 符号重复定义硬阻断... | → | ssot_redefinition_gate.py — SSoT 符号重复定义硬阻断门禁 ... | 测试依赖 / test_depends |
| 124 | D_GOV_ENFORCEMENT 规则执行: test_test_source_consistency_gate.py — TEST-SOURCE-CONSI... | → | test_source_consistency_gate.py — 测试-源码符号一致性门... | 测试依赖 / test_depends |
| 125 | D_GOV_ENFORCEMENT 规则执行: test_translation_coverage_gate.py — TRANSLATION-COVERAGE... | → | translation_coverage_gate.py — 新建 .py 文件大白话简介覆... | 测试依赖 / test_depends |
| 126 | D_GOV_ENFORCEMENT 规则执行: test_undefined_name_gate.py — UNDEFINED-NAME 门禁单测 (c... | → | undefined_name_gate.py — UNDEFINED-NAME 门禁（F821 未定... | 测试依赖 / test_depends |
| 127 | D_GOV_ENFORCEMENT 规则执行: test_unsafe_dict_spread_gate.py — ``**data`` 直接展开 wa... | → | unsafe_dict_spread_gate.py — ``**data`` 直接展开模式 war... | 测试依赖 / test_depends |
| 128 | D_GOV_ENFORCEMENT 规则执行: test_vocab_hardcode_gate.py — VOCAB-HARDCODE 门禁单测 (c... | → | vocab_hardcode_gate.py — 新增 .py 文件词表硬编码阻断门禁... | 测试依赖 / test_depends |
| 129 | D_GOV_ENFORCEMENT 规则执行: test_zephyr_env_direct_access_gate.py — ZEPHYR_ENV 直访... | → | zephyr_env_direct_access_gate.py — ZEPHYR_ENV 直访硬阻断... | 测试依赖 / test_depends |
| 130 | D_GOV_ENFORCEMENT 规则执行: test_gate_auto_registrar.py — gate_auto_registrar 单元测... | → | gate_auto_registrar.py — YAML 驱动的 in-process gate 自... | 测试依赖 / test_depends |
| 131 | D_GOV_OPS_RESILIENCE 运维弹性治理: budget/test_debt_projector.py | → | debt投影器 / debt_projector (code_dedup/debt_projector.py) | 测试依赖 / test_depends |
| 132 | D_GOV_OPS_RESILIENCE 运维弹性治理: budget/test_degradation.py | → | 退化 / degradation (code_dedup/degradation.py) | 测试依赖 / test_depends |
| 133 | D_GOV_OPS_RESILIENCE 运维弹性治理: orchestrator/test_prioritizer.py | → | 修复优先级排序器 — 置信度×Impact×适配性 三因子排序. / ... | 测试依赖 / test_depends |
| 134 | D_GOV_SCRIPTS 脚本治理: scan_consumers_accuracy.py — CONSUMERS 字段准确性 baseli... | → | 差异辅助 / _diff_helpers (commit_gates/_diff_helpers.py) | 导入依赖 / import_depends |
| 135 | D_GOV_SCRIPTS 脚本治理: scan_consumers_accuracy.py — CONSUMERS 字段准确性 baseli... | → | consumersaccuracy门禁 / consumers_accuracy_gate (commit_g... | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 14 个外部域直接连接（出边 152 条 + 入边 135 条 = 287 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_GOV_CODE_QUALITY -->|90条 导入依赖 / import_depends, 测试依赖 / test_depends| D_GOV_ENFORCEMENT
    D_GOV_CODE_QUALITY -->|20条 导入依赖 / import_depends| D_SHARED
    D_GOV_CODE_QUALITY -->|14条 测试依赖 / test_depends| D_GOV_RULE
    D_GOV_CODE_QUALITY -->|11条 导入依赖 / import_depends, 测试依赖 / test_depends| D_GOVERNANCE
    ...
    D_GOV_ENFORCEMENT -->|74条 导入依赖 / import_depends, 测试依赖 / test_depends| D_GOV_CODE_QUALITY
    D_GOVERNANCE -->|46条 导入依赖 / import_depends, 测试依赖 / test_depends| D_GOV_CODE_QUALITY
