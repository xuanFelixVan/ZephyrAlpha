...
flowchart TD
    ...
    src_zephyr_gov_code_quality_init_py["zephyr/gov_code_quality 包入口<br/>代码质量治理的包入口。gov_code_quality domain<br/>package — code quality governance<br/>(D_GOV_CODE_QUALITY).，包入口。gov_code_quality<br/>domain package — code quality governance<br/>(D_GOV_CODE_QUALITY).相关功能的实现。<br/>文件: gov_code_quality/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_integrations_py["集成管理——预提交钩子+CI-only 扫描+超时边界.<br/>noqa: m10-time-trigger  M10豁免: 'cron'在注释<br/>/文档字符串中，非实际cron调用<br/>integrations<br/>文件: code_dedup/integrations.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_mock_duplicate_generator_py["可控克隆生产器——零假阳性可期待引擎分子离散<br/>代码质量治理的可控克隆生产器——零假阳性可期待引擎<br/>分子离散，可控克隆生产器——零假阳性可期待引擎分子<br/>离散相关功能的实现。<br/>mock_duplicate_generator<br/>文件: code_dedup/mock_duplicate_generator.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_recovery_manifest_writer_py["恢复清单写入器<br/>代码质量治理的Recovery Manifest Writer —<br/>R2纯文本base64 Manifest.，Recovery Manifest<br/>Writer — R2纯文本base64<br/>Manifest.相关功能的实现。<br/>recovery_manifest_writer<br/>文件: code_dedup/recovery_manifest_writer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_success_validator_py["成功验证——判断一次去重操作是否真正消灭了克隆.<br/>代码质量治理的成功验证——判断一次去重操作是否真正<br/>消灭了克隆.，成功验证——判断一次去重操作是否真正<br/>消灭了克隆.相关功能的实现。<br/>success_validator<br/>文件: code_dedup/success_validator.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_enforcement_commit_gates_exempt_zone_frontmatter_gate_py["exemptzonefrontmatter门禁<br/>从 make_exempt_zone_frontmatter_reconciler<br/>（post-commit warn）升级为 pre-commit 阻断 gate<br/>exempt_zone_frontmatter_gate<br/>文件: commit_gates<br/>/exempt_zone_frontmatter_gate.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_enforcement_commit_gates_id_uniqueness_gate_py["iduniqueness门禁<br/>从 make_precommit_id_uniqueness_reconciler<br/>（post-commit warn）升级为 pre-commit 阻断 gate<br/>id_uniqueness_gate<br/>文件: commit_gates/id_uniqueness_gate.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_enforcement_commit_gates_schema_file_exists_gate_py["结构fileexists门禁<br/>裁定  Phase 1：SSoT 存在性强制<br/>schema_file_exists_gate<br/>文件: commit_gates/schema_file_exists_gate.py<br/>(生产态 / production)"]
    ...
    tests_governance_test_apply_dataflowgraph_smoke_py["测试applydataflowgraphsmoke<br/>测试应用dataflowgraphsmoke模块<br/>文件: governance<br/>/test_apply_dataflowgraph_smoke.py<br/>(生产态 / production)"]
    tests_governance_test_apply_decisiongraph_smoke_py["测试applydecisiongraphsmoke<br/>测试应用decisiongraphsmoke模块<br/>文件: governance<br/>/test_apply_decisiongraph_smoke.py<br/>(生产态 / production)"]
    ...
    tests_governance_test_generate_project_depgraph_smoke_py["测试generateprojectdepgraphsmoke<br/>测试生成project依赖图smoke模块<br/>文件: governance<br/>/test_generate_project_depgraph_smoke.py<br/>(生产态 / production)"]
    ...
    tests_governance_test_sync_yaml_to_depgraph_smoke_py["测试syncyamltodepgraphsmoke<br/>测试同步yamlto依赖图smoke模块<br/>文件: governance<br/>/test_sync_yaml_to_depgraph_smoke.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_auto_fixer_py["安全自动修复引擎——五直接开关+五间接约束.<br/>代码质量治理的安全自动修复引擎——五直接开关+五间<br/>接约束.，安全自动修复引擎——五直接开关+五间接约束<br/>.相关功能的实现。<br/>auto_fixer<br/>文件: code_dedup/auto_fixer.py<br/>(生产态 / production)"]
