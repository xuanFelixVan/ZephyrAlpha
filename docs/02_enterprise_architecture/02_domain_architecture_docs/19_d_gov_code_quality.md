...
flowchart TD
    ...
    D_GOV_ENFORCEMENT -->|测试依赖 / test_depends| src_zephyr_gov_enforcement_commit_gates_ch_batch_size_gate_py
    ...
    class D_GOV_ENFORCEMENT,D_GOVERNANCE,D_SHARED,D_GOV_AUDIT,D_GOV_OPS_RESILIENCE external_prod
```
...
| 112 | worktree_required_gate.py — worktree 隔离强制门禁（WORKT... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 113 | zephyr_env_direct_access_gate.py — ZEPHYR_ENV 直访硬阻断... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 114 | gate_auto_registrar.py — YAML 驱动的 in-process gate 自... | → | D_GOV_ENFORCEMENT 规则执行: commit_gate_registry.py — GitCommitGateway pre-commit 门... | 导入依赖 / import_depends |
| 115 | rule_enforcement/test_output_quality_gate.py | → | D_GOV_ENFORCEMENT 规则执行: rule_enforcement/output_quality_gate.py | 测试依赖 / test_depends |
| 116 | test_audit_worktree_ops_telemetry.py — worktree_ops_log ... | → | D_GOV_ENFORCEMENT 规则执行: session_worktree.py — AI 对话 worktree 物理隔离 helper（... | 测试依赖 / test_depends |
| 117 | governance_misc/test_bare_repo_scanner.py | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Bare Repo Scanner — v0.14.0 嵌入式裸仓库检测器。 (securi... | 测试依赖 / test_depends |
| 118 | governance_misc/test_governance_result_types.py | → | D_GOV_OPS_RESILIENCE 运维弹性治理: G-CT-003 — RollbackResult backward-compat re-export faca... | 测试依赖 / test_depends |
| 119 | gate_engine/test_adversarial_gate_integration.py | → | D_GOV_RULE 规则治理: 对抗样本生成器 / Adversarial Strategies (rule_enforcement... | 测试依赖 / test_depends |
| 120 | gate_engine/test_adversarial_gate_integration.py | → | D_GOV_RULE 规则治理: 对抗验证门禁 / Adversarial Validation (gate_engine/advers... | 测试依赖 / test_depends |
| 121 | gate_engine/test_adversarial_validation_gate.py | → | D_GOV_RULE 规则治理: 对抗验证门禁 / Adversarial Validation (gate_engine/advers... | 测试依赖 / test_depends |
| 122 | invariants/test_en_001_circular_dependency.py | → | D_GOV_RULE 规则治理: 循环依赖扫描器 / Circular Dependency Scanner (invariants/... | 测试依赖 / test_depends |
| 123 | invariants/test_en_003_contract_compatibility.py | → | D_GOV_RULE 规则治理: 契约兼容性检查器 / Contract Compatibility Checker (invari... | 测试依赖 / test_depends |
| 124 | invariants/test_en_process_lifecycle_gateway.py | → | D_GOV_RULE 规则治理: 进程生命周期网关 / Process Lifecycle Gateway (invariants/... | 测试依赖 / test_depends |
| 125 | invariants/test_zero_residue_check.py | → | D_GOV_RULE 规则治理: 零残留检查器 / Zero Residue Check (invariants/zero_residu... | 测试依赖 / test_depends |
| 126 | rule_enforcement/test_adaptive_threshold.py | → | D_GOV_RULE 规则治理: 自适应阈值 / Adaptive Threshold (rule_enforcement/adaptiv... | 测试依赖 / test_depends |
| 127 | rule_enforcement/test_adversarial_strategies.py | → | D_GOV_RULE 规则治理: 对抗样本生成器 / Adversarial Strategies (rule_enforcement... | 测试依赖 / test_depends |
...
> 本域与 14 个外部域直接连接（出边 158 条 + 入边 141 条 = 299 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_GOV_CODE_QUALITY -->|93条 导入依赖 / import_depends, 测试依赖 / test_depends| D_GOV_ENFORCEMENT
