...
flowchart TD
    scripts_governance_d3_metadata_check_pure_assertion_py["(生产态 / production) 检查pureassertion<br/>/ check_pure_assertion<br/>GOV-DOC-016 纯陈述原则检测真源（SSoT）。<br/>文件: d3_metadata/check_pure_<br/>assertion.py"]
    scripts_governance_d7_code_check_module_id_consistency_py["(生产态 / production) 检查模块id一致性<br/>/ check_module_id_consistency<br/>module_id 全仓一致性扫描<br/>（--scan-existing 模式）.<br/>文件: d7_code/check_module_id_<br/>consistency.py"]
    src_zephyr_gov_code_quality_init_py["(生产态 / production) 包入口 / gov_code_<br/>quality domain package — code quality<br/>governance (D<br/>包入口。gov_code_quality domain package<br/>— code quality governance (D_GOV_CODE_<br/>QUALITY).<br/>文件: gov_code_quality/__init__.py"]
    src_zephyr_gov_code_quality_code_dedup_init_py["(生产态 / production) 包入口 / __init__<br/>code-dedup-engine 子包 —<br/>重复代码检测与治理引擎.<br/>文件: code_dedup/__init__.py"]
    src_zephyr_gov_code_quality_code_dedup_annotations_py["(生产态 / production) 共享函数注解引擎<br/>— @shared / @knowndup / @inten /<br/>annotations<br/>共享函数注解引擎 — @shared / @known_dup<br/>/ @intentional 三注解.<br/>文件: code_dedup/annotations.py"]
    src_zephyr_gov_code_quality_code_dedup_atomic_fixer_py["(生产态 / production) 原子性修复引擎 —<br/>WAL 式 PREFLIGHT -> CHECKPOINT  /<br/>atomic_fixer<br/>原子性修复引擎 — WAL 式 PREFLIGHT -><br/>CHECKPOINT -> APPLY -> RECOVER.<br/>文件: code_dedup/atomic_fixer.py"]
    src_zephyr_gov_code_quality_code_dedup_behavioral_sampler_py["(生产态 / production) behavioral采样器<br/>/ behavioral_sampler<br/>行为采样验证器 — Stage 0.25<br/>低成本快速验证.<br/>文件: code_dedup/behavioral_sampler.py"]
    src_zephyr_gov_code_quality_code_dedup_behavioral_trust_checker_py["(生产态 / production) 行为信任检查器 —<br/>行为漂移DIVERGED检测. / behavioral_<br/>trust_checker<br/>行为信任检查器 — 行为漂移DIVERGED检测.<br/>文件: code_dedup/behavioral_trust_<br/>checker.py"]
    src_zephyr_gov_code_quality_code_dedup_cache_manager_py["(生产态 / production) 缓存管理器 /<br/>cache_manager<br/>Stage 0: 函数缓存管理器 —<br/>增量扫描的加速核心.<br/>文件: code_dedup/cache_manager.py"]
    src_zephyr_gov_code_quality_code_dedup_canary_manager_py["(生产态 / production) 金丝雀管理器 /<br/>canary_manager<br/>金丝雀工厂——生成已知oracle 文件<br/>用于引擎检出+回归测试.<br/>文件: code_dedup/canary_manager.py"]
    src_zephyr_gov_code_quality_code_dedup_canary_register_py["(生产态 / production)<br/>金丝雀注册表维护器 — 注册/过期<br/>/腐败检测. / canary_register<br/>金丝雀注册表维护器 — 注册/过期/腐败检测.<br/>文件: code_dedup/canary_register.py"]
    ...
    src_zephyr_gov_code_quality_code_dedup_code_analyzer_runner_py["(生产态 / production) 代码分析器运行器<br/>/ code_analyzer_runner<br/>检查运行器——按照敏感基线运行三阶段+导出<br/>yaml 报告.<br/>文件: code_dedup/code_analyzer_runner.py"]
    src_zephyr_gov_code_quality_code_dedup_code_simulator_py["(生产态 / production) 代码模拟器 / code_<br/>simulator<br/>代码模拟器——播放录制的克隆演化序列，stre<br/>ss-test AST/baseline归一化.<br/>文件: code_dedup/code_simulator.py"]
    src_zephyr_gov_code_quality_code_dedup_contract_consistency_checker_py["(生产态 / production)<br/>API契约一致性检查器 —<br/>存在性·行为·契约三维. / contract_<br/>consistency_checker<br/>API契约一致性检查器 —<br/>存在性·行为·契约三维.<br/>文件: code_dedup/contract_consistency_<br/>checker.py"]
    src_zephyr_gov_code_quality_code_dedup_cross_boundary_detector_py["(生产态 / production) 跨boundary检测器<br/>/ cross_boundary_detector<br/>跨边界克隆感知——四大边界差异化检测+独立<br/>策略+跨边界保守auto_fix规则.<br/>文件: code_dedup/cross_boundary_<br/>detector.py"]
    src_zephyr_gov_code_quality_code_dedup_dead_module_detector_py["(生产态 / production) deadmodule检测器<br/>/ dead_module_detector<br/>死共享模块检测器 — shared<br/>/子模块无人使用 -> DEAD.<br/>文件: code_dedup/dead_module_detector.py"]
    src_zephyr_gov_code_quality_code_dedup_debt_projector_py["(生产态 / production) debt投影器 / debt_<br/>projector<br/>去重债务预测器 — weeks_to_payoff +<br/>intake_rate vs fix_rate 蒙特卡洛模拟.<br/>文件: code_dedup/debt_projector.py"]
    src_zephyr_gov_code_quality_code_dedup_decision_auditor_py["(生产态 / production) 决策审计器 /<br/>decision_auditor<br/>决策审计链 — DecisionFingerprint<br/>不可变追加日志.<br/>文件: code_dedup/decision_auditor.py"]
    src_zephyr_gov_code_quality_code_dedup_degradation_py["(生产态 / production) 退化 / degradation<br/>降级运行管理器 — 各 Stage 独立 try<br/>/except + degradation_level + exit code.<br/>文件: code_dedup/degradation.py"]
    src_zephyr_gov_code_quality_code_dedup_diff_detector_py["(生产态 / production) 差异检测器 / diff_<br/>detector<br/>Stage 0: Git diff 变更检测器 —<br/>函数粒度增量.<br/>文件: code_dedup/diff_detector.py"]
    src_zephyr_gov_code_quality_code_dedup_doom_loop_guard_py["(生产态 / production) doom循环守卫 /<br/>doom_loop_guard<br/>Doom Loop 防护 — 修复升级阶梯 L0-L4<br/>状态机.<br/>文件: code_dedup/doom_loop_guard.py"]
    src_zephyr_gov_code_quality_code_dedup_extraction_safety_py["(生产态 / production) extraction安全 /<br/>extraction_safety<br/>安全提取适配性评估器 — Suitability<br/>Score 0-100 + 不安全提取模式检测.<br/>文件: code_dedup/extraction_safety.py"]
    src_zephyr_gov_code_quality_code_dedup_false_negative_auditor_py["(生产态 / production)<br/>falsenegative审计器 / false_negative_<br/>auditor<br/>三层漏报盲审器 — L1 Sweep + L2 Canary +<br/>L3 Sampling.<br/>文件: code_dedup/false_negative_<br/>auditor.py"]
    src_zephyr_gov_code_quality_code_dedup_fifteen_dimension_auditor_py["(生产态 / production)<br/>15维超综合审计首页 —<br/>逐项证明'做过且做对'. / fifteen_<br/>dimension_auditor<br/>15维超综合审计首页 —<br/>逐项证明'做过且做对'.<br/>文件: code_dedup/fifteen_dimension_<br/>auditor.py"]
    src_zephyr_gov_code_quality_code_dedup_file_creator_py["(生产态 / production)<br/>文件创建清单执行器 — 验证所有源/测试<br/>/数据文件存在性. / file_creator<br/>文件创建清单执行器 — 验证所有源/测试<br/>/数据文件存在性.<br/>文件: code_dedup/file_creator.py"]
    src_zephyr_gov_code_quality_code_dedup_function_discovery_py["(生产态 / production) 共享函数主动发现<br/>— 签名+语义双通道从被动到主动. /<br/>function_discovery<br/>共享函数主动发现 —<br/>签名+语义双通道从被动到主动.<br/>文件: code_dedup/function_discovery.py"]
    ...
    src_zephyr_gov_code_quality_code_dedup_health_monitor_py["(生产态 / production) 健康监控 / health_<br/>monitor<br/>健康仪表盘 — Dedup Health Score 0-100 +<br/>趋势 + Session Log 写入.<br/>文件: code_dedup/health_monitor.py"]
    src_zephyr_gov_code_quality_code_dedup_integration_hub_py["(生产态 / production) 集成hub /<br/>integration_hub<br/>集成协调器 — 24集成+19更新+16GitHub整合.<br/>文件: code_dedup/integration_hub.py"]
    src_zephyr_gov_code_quality_code_dedup_integrations_py["(生产态 / production)<br/>集成管理——预提交钩子+CI-only<br/>扫描+超时边界. / integrations<br/>集成管理——预提交钩子+CI-only<br/>扫描+超时边界.<br/>文件: code_dedup/integrations.py"]
    src_zephyr_gov_code_quality_code_dedup_micro_clone_detector_py["(生产态 / production) microclone检测器<br/>/ micro_clone_detector<br/>微型克隆检测器 — n-gram频率计数,<br/>1-2行高频模式聚合.<br/>文件: code_dedup/micro_clone_detector.py"]
    src_zephyr_gov_code_quality_code_dedup_mock_duplicate_generator_py["(生产态 / production)<br/>可控克隆生产器——零假阳性可期待引擎分子离<br/>散 / mock_duplicate_generator<br/>可控克隆生产器——零假阳性可期待引擎分子离<br/>散<br/>文件: code_dedup/mock_duplicate_<br/>generator.py"]
    src_zephyr_gov_code_quality_code_dedup_monoculture_guard_py["(生产态 / production) monoculture守卫 /<br/>monoculture_guard<br/>Monoculture 免疫 — BRS 0-100 +<br/>去重悖论检测.<br/>文件: code_dedup/monoculture_guard.py"]
    src_zephyr_gov_code_quality_code_dedup_observation_window_guard_py["(生产态 / production)<br/>提取后稳定观察期守护 — 对标SDP<br/>14天观察. / observation_window_guard<br/>提取后稳定观察期守护 — 对标SDP 14天观察.<br/>文件: code_dedup/observation_window_<br/>guard.py"]
    src_zephyr_gov_code_quality_code_dedup_path_index_validator_py["(生产态 / production) 路径索引校验器 /<br/>path_index_validator<br/>路径索引验证——验证 config<br/>数据集相对路径表与实际文件系统同步.<br/>文件: code_dedup/path_index_validator.py"]
    src_zephyr_gov_code_quality_code_dedup_phase_executor_py["(生产态 / production) 阶段执行器 /<br/>phase_executor<br/>6Phase施工执行器 — Phase 0~5<br/>执行状态追踪.<br/>文件: code_dedup/phase_executor.py"]
    src_zephyr_gov_code_quality_code_dedup_policy_tree_validator_py["(生产态 / production)<br/>策略树自动一致性校验器 —<br/>虚线箭头影响分析. / policy_tree_<br/>validator<br/>策略树自动一致性校验器 —<br/>虚线箭头影响分析.<br/>文件: code_dedup/policy_tree_<br/>validator.py"]
    src_zephyr_gov_code_quality_code_dedup_pre_apply_integrity_gate_py["(生产态 / production) 预应用完整性门禁<br/>/ pre_apply_integrity_gate<br/>Pre-Apply 完整性门 — SHA256重新验证.<br/>文件: code_dedup/pre_apply_integrity_<br/>gate.py"]
    src_zephyr_gov_code_quality_code_dedup_prioritizer_py["(生产态 / production) 修复优先级排序器<br/>— 置信度×Impact×适配性 三因子排序. /<br/>prioritizer<br/>修复优先级排序器 — 置信度×Impact×适配性<br/>三因子排序.<br/>文件: code_dedup/prioritizer.py"]
    src_zephyr_gov_code_quality_code_dedup_recovery_manifest_writer_py["(生产态 / production) 恢复清单写入器 /<br/>recovery_manifest_writer<br/>Recovery Manifest Writer —<br/>R2纯文本base64 Manifest.<br/>文件: code_dedup/recovery_manifest_<br/>writer.py"]
    src_zephyr_gov_code_quality_code_dedup_risk_mitigator_py["(生产态 / production) 风险mitigator /<br/>risk_mitigator<br/>R1-R45全量风险缓解执行器 —<br/>逐条检查缓解措施 + mitigation_<br/>tracker.yaml.<br/>文件: code_dedup/risk_mitigator.py"]
    src_zephyr_gov_code_quality_code_dedup_self_scanner_py["(生产态 / production) 自扫描器 / self_<br/>scanner<br/>引擎自扫描器 — Dogfooding<br/>检测引擎自身源码重复.<br/>文件: code_dedup/self_scanner.py"]
    src_zephyr_gov_code_quality_code_dedup_sensitivity_sweeper_py["(生产态 / production) sensitivity清扫器<br/>/ sensitivity_sweeper<br/>敏感性扫荡——threshold扫描->固化成new<br/>baseline（零假阳性+触达率保险）.<br/>文件: code_dedup/sensitivity_sweeper.py"]
    src_zephyr_gov_code_quality_code_dedup_shadow_trust_validator_py["(生产态 / production) 影子信任校验器 /<br/>shadow_trust_validator<br/>影子信任验证器 — ImportError 防护回路.<br/>文件: code_dedup/shadow_trust_<br/>validator.py"]
    src_zephyr_gov_code_quality_code_dedup_shadow_verifier_py["(生产态 / production) 影子验证器 /<br/>shadow_verifier<br/>影子清单验证器 — size sanity check +<br/>semantic验证 + 覆盖度报告.<br/>文件: code_dedup/shadow_verifier.py"]
    src_zephyr_gov_code_quality_code_dedup_shared_evolver_py["(生产态 / production)<br/>共享函数自我进化引擎 — 自动升降级 +<br/>行为漂移锁定. / shared_evolver<br/>共享函数自我进化引擎 — 自动升降级 +<br/>行为漂移锁定.<br/>文件: code_dedup/shared_evolver.py"]
    src_zephyr_gov_code_quality_code_dedup_shared_lifecycle_manager_py["(生产态 / production)<br/>共享生命周期管理器 / shared_lifecycle_<br/>manager<br/>共享函数生命周期管理 —<br/>Active->Deprecated->Grace->Sunset->Retir<br/>ed 五阶段状态机.<br/>文件: code_dedup/shared_lifecycle_<br/>manager.py"]
    src_zephyr_gov_code_quality_code_dedup_signature_matcher_py["(生产态 / production) signature匹配器 /<br/>signature_matcher<br/>signature匹配器.5: 签名指纹 SHA256(:12)<br/>O(1) 精确匹配.<br/>文件: code_dedup/signature_matcher.py"]
    src_zephyr_gov_code_quality_code_dedup_simplicity_auditor_py["(生产态 / production)<br/>引擎成本效益自审计器 — SAS 0-100<br/>月度审计 + Tax 报告. / simplicity_<br/>auditor<br/>引擎成本效益自审计器 — SAS 0-100<br/>月度审计 + Tax 报告.<br/>文件: code_dedup/simplicity_auditor.py"]
    src_zephyr_gov_code_quality_code_dedup_ssot_registrar_py["(生产态 / production) SSoT注册器 —<br/>提取函数自动注册到 shared API清单. /<br/>ssot_registrar<br/>SSoT注册器 — 提取函数自动注册到 shared<br/>API清单.<br/>文件: code_dedup/ssot_registrar.py"]
    src_zephyr_gov_code_quality_code_dedup_stale_shared_detector_py["(生产态 / production) stale共享检测器 /<br/>stale_shared_detector<br/>过时共享函数检测器 — 无caller × 30天 -><br/>STALE标记.<br/>文件: code_dedup/stale_shared_<br/>detector.py"]
    src_zephyr_gov_code_quality_code_dedup_success_validator_py["(生产态 / production)<br/>成功验证——判断一次去重操作是否真正消灭了<br/>克隆. / success_validator<br/>成功验证——判断一次去重操作是否真正消灭了<br/>克隆.<br/>文件: code_dedup/success_validator.py"]
    src_zephyr_gov_code_quality_code_dedup_symbol_index_py["(生产态 / production) symbol索引 /<br/>symbol_index<br/>符号索引 — 全局函数/类/import映射表.<br/>文件: code_dedup/symbol_index.py"]
    src_zephyr_gov_code_quality_code_dedup_thematic_clusterer_py["(生产态 / production) 主题聚类器 —<br/>噪声信号比·告警疲劳缓解. / thematic_<br/>clusterer<br/>主题聚类器 — 噪声信号比·告警疲劳缓解.<br/>文件: code_dedup/thematic_clusterer.py"]
    src_zephyr_gov_code_quality_code_dedup_trackers_init_py["(生产态 / production) 包入口 / __init__<br/>tracker 族子包 — 风险/盲点<br/>/热点跟踪器集合.<br/>文件: trackers/__init__.py"]
    src_zephyr_gov_code_quality_code_dedup_trackers_blind_spot_tracker_py["(生产态 / production) 盲点关闭追踪器 —<br/>自动验证各轮盲点是否已覆盖. / blind_<br/>spot_tracker<br/>盲点关闭追踪器 —<br/>自动验证各轮盲点是否已覆盖.<br/>文件: trackers/blind_spot_tracker.py"]
    src_zephyr_gov_code_quality_code_dedup_trackers_consequence_tracker_py["(生产态 / production)<br/>后果追踪——记录每次修复操作对依赖方的影响<br/>. / consequence_tracker<br/>后果追踪——记录每次修复操作对依赖方的影响<br/>.<br/>文件: trackers/consequence_tracker.py"]
    src_zephyr_gov_code_quality_code_dedup_trackers_hotspot_tracker_py["(生产态 / production) 热点追踪器 —<br/>90天滑动窗口 + 高频变动检测 +<br/>新项目预热清单. / hotspot_tracker<br/>热点追踪器 — 90天滑动窗口 +<br/>高频变动检测 + 新项目预热清单.<br/>文件: trackers/hotspot_tracker.py"]
    ...
    src_zephyr_gov_code_quality_code_dedup_trackers_risk_mitigation_tracker_py["(生产态 / production)<br/>风险缓解追踪——捕获哪些克隆报告了但在N次<br/>扫描后仍未fix. / risk_mitigation_tracker<br/>风险缓解追踪——捕获哪些克隆报告了但在N次<br/>扫描后仍未fix.<br/>文件: trackers/risk_mitigation_<br/>tracker.py"]
    src_zephyr_gov_code_quality_code_dedup_verifier_py["(生产态 / production) 验证器 / verifier<br/>修复验证器 — import + 类型 +<br/>行为采样验证.<br/>文件: code_dedup/verifier.py"]
    src_zephyr_gov_enforcement_commit_gates_init_py["(生产态 / production) 包入口 / __init__<br/>commit_gates — GitCommitGateway<br/>pre-commit 门禁实现包。<br/>文件: commit_gates/__init__.py"]
    src_zephyr_gov_enforcement_commit_gates_arch_reference_gate_py["(生产态 / production) archreference门禁<br/>/ arch_reference_gate<br/>#ARCH-NNN / #ARCH-DOMAIN-NNN<br/>悬空引用自动检测门禁<br/>文件: commit_gates/arch_reference_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_asyncio_run_in_context_gate_py["(生产态 / production)<br/>asynciorunin上下文门禁 / asyncio_run_in_<br/>context_gate<br/>异步上下文误用硬阻断门禁<br/>文件: commit_gates/asyncio_run_in_<br/>context_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_bare_getenv_gate_py["(生产态 / production) baregetenv门禁 /<br/>bare_getenv_gate<br/>裸 os.getenv 读密钥阻断门禁<br/>（NO-BARE-GETENV，§5.17.10 治本）<br/>文件: commit_gates/bare_getenv_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_bare_sql_gate_py["(生产态 / production) baresql门禁 /<br/>bare_sql_gate<br/>裸SQL字面量阻断门禁<br/>（NO-BARE-SQL，§5.160.2 防复发）<br/>文件: commit_gates/bare_sql_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_bare_subprocess_gate_py["(生产态 / production)<br/>baresubprocess门禁 / bare_subprocess_<br/>gate<br/>裸 subprocess 调用硬阻断门禁<br/>文件: commit_gates/bare_subprocess_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_blueprint_amodule_consistency_gate_py["(生产态 / production)<br/>蓝图amoduleconsistency门禁 / blueprint_<br/>amodule_consistency_gate<br/>(A_module) 头部 module_id 格式一致性门禁<br/>文件: commit_gates/blueprint_amodule_<br/>consistency_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_blueprint_amodule_cross_check_gate_py["(生产态 / production)<br/>蓝图amodule跨check门禁 / blueprint_<br/>amodule_cross_check_gate<br/>(BLUEPRINT) vs (A_module) 交叉校验门禁<br/>文件: commit_gates/blueprint_amodule_<br/>cross_check_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_blueprint_format_gate_py["(生产态 / production) 蓝图format门禁 /<br/>blueprint_format_gate<br/>(BLUEPRINT) 头部 module_id<br/>格式阻断门禁（BLUEPRINT-FORMAT，裁定#214<br/>Phase 0 防蔓延）<br/>文件: commit_gates/blueprint_format_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_capability_consistency_gate_py["(生产态 / production) 能力一致性门禁 /<br/>capability_consistency_gate<br/>Provider 路由-meta 一致性门禁<br/>（CAP-CONSISTENCY，裁定 #ARCH-CH-022<br/>Phase 4.4）<br/>文件: commit_gates/capability_<br/>consistency_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_capability_lookup_required_gate_py["(生产态 / production)<br/>capabilitylookuprequired门禁 /<br/>capability_lookup_required_gate<br/>Capability Lookup 强制门禁<br/>（CAPABILITY-LOOKUP-REQUIRED，#ARCH-GOV-<br/>CONVERGENCE-META Phase<br/>文件: commit_gates/capability_lookup_<br/>required_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_capability_overlap_gate_py["(生产态 / production)<br/>capabilityoverlap门禁 / capability_<br/>overlap_gate<br/>新建 .py 文件 CapabilityLookup<br/>提示门禁（warn-only，2026-06-30 治本）<br/>文件: commit_gates/capability_overlap_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_ch_batch_size_gate_py["(生产态 / production) ch批次大小门禁 /<br/>ch_batch_size_gate<br/>CH 批量写入防回退门禁<br/>（CH-BATCH-SIZE，§18.4 防复发）<br/>文件: commit_gates/ch_batch_size_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_ch_final_gate_py["(生产态 / production) ch最终门禁 / ch_<br/>final_gate<br/>ch最终门禁.query() 直接调用阻断门禁<br/>（CH-FINAL-GATE，裁定 #ARCH-CH-007 B5）<br/>文件: commit_gates/ch_final_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_ch_version_col_gate_py["(生产态 / production) ch版本col门禁 /<br/>ch_version_col_gate<br/>CH version 列语义误用阻断门禁<br/>（CH-VERSION-COL，裁定 #ARCH-CH-009）<br/>文件: commit_gates/ch_version_col_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_claim_required_gate_py["(生产态 / production) claimrequired门禁<br/>/ claim_required_gate<br/>claim_files 前置检查门禁<br/>（CLAIM-REQUIRED，2026-06-30 治本）<br/>文件: commit_gates/claim_required_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_consumers_accuracy_gate_py["(生产态 / production)<br/>consumersaccuracy门禁 / consumers_<br/>accuracy_gate<br/>CONSUMERS 字段准确性 warn-only 门禁<br/>文件: commit_gates/consumers_accuracy_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_create_guard_py["(生产态 / production) 创建守卫 / create_<br/>guard<br/>新建 .py / 非 rules/ .yaml 文件<br/>creation_token 阻断门禁<br/>（CREATE-GUARD，2026-06-30 治本）<br/>文件: commit_gates/create_guard.py"]
    src_zephyr_gov_enforcement_commit_gates_dangling_reference_gate_py["(生产态 / production)<br/>danglingreference门禁 / dangling_<br/>reference_gate<br/>danglingreference门禁.md §X.Y<br/>悬空引用自动检测门禁<br/>文件: commit_gates/dangling_reference_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_data_task_completeness_gate_py["(生产态 / production)<br/>数据taskcompleteness门禁 / data_task_<br/>completeness_gate<br/>数据任务完整性门禁（warn 级，提醒型）<br/>文件: commit_gates/data_task_<br/>completeness_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_datetime_now_forbidden_gate_py["(生产态 / production)<br/>datetimenowforbidden门禁 / datetime_now_<br/>forbidden_gate<br/>时间戳约定硬阻断门禁<br/>文件: commit_gates/datetime_now_<br/>forbidden_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_depgraph_freshness_gate_py["(生产态 / production)<br/>depgraphfreshness门禁 / depgraph_<br/>freshness_gate<br/>depgraph 新鲜度门禁<br/>（dual-threshold，#ARCH-DEPGRAPH-RECONCI<br/>LER-FAILSILENT Phase 3.1）<br/>文件: commit_gates/depgraph_freshness_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_depgraph_write_path_gate_py["(生产态 / production)<br/>depgraphwritepath门禁 / depgraph_write_<br/>path_gate<br/>depgraph 写入路径白名单门禁<br/>文件: commit_gates/depgraph_write_path_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_derivation_annotation_gate_py["(生产态 / production)<br/>derivationannotation门禁 / derivation_<br/>annotation_gate<br/>派生关系声明真实性校验门禁<br/>文件: commit_gates/derivation_<br/>annotation_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_directory_contract_gate_py["(生产态 / production)<br/>directorycontract门禁 / directory_<br/>contract_gate<br/>DCR-001~007 等效校验门禁（治本：弥补<br/>--no-verify 绕过 pre-commit 的缺口）<br/>文件: commit_gates/directory_contract_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_doc_ref_broken_gate_py["(生产态 / production) docrefbroken门禁<br/>/ doc_ref_broken_gate<br/>文档相对路径断裂引用阻断门禁<br/>文件: commit_gates/doc_ref_broken_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_domain_fk_gate_py["(生产态 / production) 域fk门禁 / domain_<br/>fk_gate<br/>(DOMAIN) 头部域注册表 FK 校验门禁<br/>文件: commit_gates/domain_fk_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_domain_name_zh_direct_access_gate_py["(生产态 / production)<br/>domainnamezhdirect访问门禁 / domain_<br/>name_zh_direct_access_gate<br/>DOMAIN_NAME_ZH 字典直接访问硬阻断门禁<br/>文件: commit_gates/domain_name_zh_<br/>direct_access_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_empty_handler_gate_py["(生产态 / production) empty处理器门禁 /<br/>empty_handler_gate<br/>空事件 handler 函数阻断门禁<br/>文件: commit_gates/empty_handler_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_encoding_gate_py["(生产态 / production) encoding门禁 /<br/>encoding_gate<br/>编码安全校验门禁（治本：弥补<br/>--no-verify 绕过 pre-commit<br/>GATE-ENCODING 的缺口）<br/>文件: commit_gates/encoding_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_exempt_zone_frontmatter_gate_py["(生产态 / production)<br/>exemptzonefrontmatter门禁 / exempt_zone_<br/>frontmatter_gate<br/>豁免区 frontmatter 门禁（Phase 3<br/>reconciler->gate 收敛）<br/>文件: commit_gates/exempt_zone_<br/>frontmatter_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_file_copy_gate_py["(生产态 / production) filecopy门禁 /<br/>file_copy_gate<br/>新增 .py 文件复制检测阻断门禁<br/>（FILE-COPY，2026-07-03 Phase 1<br/>sub-task 3）<br/>文件: commit_gates/file_copy_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_file_placement_ttl_gate_py["(生产态 / production)<br/>fileplacementttl门禁 / file_placement_<br/>ttl_gate<br/>文件放置与 TTL 一致性门禁（治本<br/>#ARCH-049：防止临时文件乱放根目录）<br/>文件: commit_gates/file_placement_ttl_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_folder_capacity_hard_limit_gate_py["(生产态 / production)<br/>folder容量hardlimit门禁 / folder_<br/>capacity_hard_limit_gate<br/>文件夹容量硬上限门禁<br/>文件: commit_gates/folder_capacity_hard_<br/>limit_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_foreign_change_gate_py["(生产态 / production) foreignchange门禁<br/>/ foreign_change_gate<br/>外来变更检测门禁<br/>（FOREIGN-CHANGE-DETECTION，ARCH-054<br/>治本）<br/>文件: commit_gates/foreign_change_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_forged_gw_marker_gate_py["(生产态 / production)<br/>forgedgwmarker门禁 / forged_gw_marker_<br/>gate<br/>Forged GW Marker 前置检测门禁<br/>（FORGED-GW-MARKER，#ARCH-PREVENTABILITY<br/>-LAYER-001 Phase 2）<br/>文件: commit_gates/forged_gw_marker_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_function_dup_gate_py["(生产态 / production) 函数dup门禁 /<br/>function_dup_gate<br/>重复函数实现阻断门禁<br/>文件: commit_gates/function_dup_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_gate_repo_py["(生产态 / production) 门禁repo / gate_<br/>repo<br/>gates 表持久化仓库（AUDIT-07 P1-5: 从<br/>gate_engine.py 提取）<br/>文件: commit_gates/gate_repo.py"]
    src_zephyr_gov_enforcement_commit_gates_git_call_budget_gate_py["(生产态 / production) Gitcall预算门禁 /<br/>git_call_budget_gate<br/>Git 调用预算 warn-only 门禁<br/>（GIT-CALL-BUDGET，§ARCH-GIT-CALL-BUDGET<br/>P2.2）<br/>文件: commit_gates/git_call_budget_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_god_class_gate_py["(生产态 / production) god类门禁 / god_<br/>class_gate<br/>God Class 阻断门禁<br/>（NO-GOD-CLASS，§5.150 防复发）<br/>文件: commit_gates/god_class_gate.py"]
    ...
    src_zephyr_gov_enforcement_commit_gates_held_overlap_gate_py["(生产态 / production) heldoverlap门禁 /<br/>held_overlap_gate<br/>搭便车防护门禁（HELD-OVERLAP，2026-06-30<br/>治本）<br/>文件: commit_gates/held_overlap_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_high_complexity_gate_py["(生产态 / production)<br/>highcomplexity门禁 / high_complexity_<br/>gate<br/>高循环复杂度阻断门禁<br/>（NO-HIGH-COMPLEXITY，§5.158 防复发）<br/>文件: commit_gates/high_complexity_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_id_uniqueness_gate_py["(生产态 / production) iduniqueness门禁<br/>/ id_uniqueness_gate<br/>pre-commit hook ID 唯一性门禁（Phase 3<br/>reconciler->gate 收敛）<br/>文件: commit_gates/id_uniqueness_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_import_direction_gate_py["(生产态 / production)<br/>importdirection门禁 / import_direction_<br/>gate<br/>shared 层向上依赖阻断门禁<br/>（NO-UPWARD-IMPORT，§5.152 防复发）<br/>文件: commit_gates/import_direction_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_import_integrity_gate_py["(生产态 / production) 导入完整性门禁 /<br/>import_integrity_gate<br/>IMPORT-INTEGRITY 门禁（悬空 import<br/>硬阻断）<br/>文件: commit_gates/import_integrity_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_issue_resolved_integrity_gate_py["(生产态 / production)<br/>issueresolved完整性门禁 / issue_<br/>resolved_integrity_gate<br/>ISSUE-RESOLVED-INTEGRITY warn-only 门禁<br/>文件: commit_gates/issue_resolved_<br/>integrity_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_long_param_list_gate_py["(生产态 / production) longparamlist门禁<br/>/ long_param_list_gate<br/>长参数列表阻断门禁<br/>（NO-LONG-PARAM-LIST，§5.150 防复发）<br/>文件: commit_gates/long_param_list_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_manual_only_permanent_gate_py["(生产态 / production)<br/>手册onlypermanent门禁 / manual_only_<br/>permanent_gate<br/>永久系统脚本 manual<br/>触发无事件订阅阻断门禁<br/>（MANUAL-ONLY-PERMANENT，#ARCH-GOV-CONVE<br/>RGENCE-META Phase 3.6 补齐<br/>文件: commit_gates/manual_only_<br/>permanent_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_mcp_version_field_gate_py["(生产态 / production) MCP版本字段门禁 /<br/>mcp_version_field_gate<br/>MCP version 字段缺失硬阻断门禁<br/>文件: commit_gates/mcp_version_field_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_module_id_consistency_gate_py["(生产态 / production) 模块id一致性门禁<br/>/ module_id_consistency_gate<br/>module_id 三声明轨道一致性 + count 派生<br/>+ 跨文件唯一性门禁（Phase 3<br/>reconciler->gate 收敛）<br/>文件: commit_gates/module_id_<br/>consistency_gate.py"]
    ...
    src_zephyr_gov_enforcement_commit_gates_msg_style_gate_py["(生产态 / production) msgstyle门禁 /<br/>msg_style_gate<br/>错误消息标点/箭头风格阻断门禁<br/>文件: commit_gates/msg_style_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_mutable_const_without_final_gate_py["(生产态 / production)<br/>mutableconstwithoutfinal门禁 / mutable_<br/>const_without_final_gate<br/>可变常量缺 Final 标注硬阻断门禁<br/>文件: commit_gates/mutable_const_<br/>without_final_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_new_file_depgraph_gate_py["(生产态 / production) 新文件依赖图门禁<br/>/ new_file_depgraph_gate<br/>新建 .py 文件 depgraph 未登记硬阻断门禁<br/>文件: commit_gates/new_file_depgraph_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_no_import_side_effect_gate_py["(生产态 / production)<br/>noimportsideeffect门禁 / no_import_side_<br/>effect_gate<br/>模块导入零副作用门禁<br/>文件: commit_gates/no_import_side_<br/>effect_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_noqa_validation_gate_py["(生产态 / production) noqa验证门禁 /<br/>noqa_validation_gate<br/>自定义 noqa 标记合规性门禁<br/>（NOQA-VALIDATION，ARCH-NOQA-GOV-001<br/>治本）<br/>文件: commit_gates/noqa_validation_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_open_without_with_gate_py["(生产态 / production)<br/>openwithoutwith门禁 / open_without_with_<br/>gate<br/>openwithoutwith门禁() 未在 with<br/>内硬阻断门禁<br/>文件: commit_gates/open_without_with_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_orphan_module_gate_py["(生产态 / production) 孤儿module门禁 /<br/>orphan_module_gate<br/>孤儿模块（无 import 引用）阻断门禁<br/>文件: commit_gates/orphan_module_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_panorama_alignment_gate_py["(生产态 / production) panorama对齐门禁<br/>/ panorama_alignment_gate<br/>三图模块对齐门禁（四图模块对齐 Step<br/>4，ARCH-056 升级）<br/>文件: commit_gates/panorama_alignment_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_precommit_offline_gate_py["(生产态 / production)<br/>precommitoffline门禁 / precommit_<br/>offline_gate<br/>pre-commit 配置离线可运行检测门禁<br/>文件: commit_gates/precommit_offline_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_pure_assertion_gate_py["(生产态 / production) pureassertion门禁<br/>/ pure_assertion_gate<br/>纯陈述原则阻断门禁<br/>（PURE-ASSERTION，GOV-DOC-016 治本）<br/>文件: commit_gates/pure_assertion_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_pure_shim_gate_py["(生产态 / production) pureshim门禁 /<br/>pure_shim_gate<br/>纯 re-export shim 阻断门禁<br/>（PURE-SHIM，P6 治本 2026-07-09）<br/>文件: commit_gates/pure_shim_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_r5_digit_suffix_gate_py["(生产态 / production) r5digitsuffix门禁<br/>/ r5_digit_suffix_gate<br/>R5 数字后缀目录禁止门禁（治本：弥补<br/>--no-verify 绕过 pre-commit 的缺口）<br/>文件: commit_gates/r5_digit_suffix_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_reconciler_health_gate_py["(生产态 / production) 协调器健康门禁 /<br/>reconciler_health_gate<br/>reconciler 健康度门禁<br/>（#ARCH-DATAQUALITY-V1.7）<br/>文件: commit_gates/reconciler_health_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_relative_path_literal_gate_py["(生产态 / production)<br/>relativepathliteral门禁 / relative_path_<br/>literal_gate<br/>相对路径字面量硬阻断门禁<br/>文件: commit_gates/relative_path_<br/>literal_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_rename_depgraph_sync_gate_py["(生产态 / production)<br/>renamedepgraphsync门禁 / rename_<br/>depgraph_sync_gate<br/>文件重命名后 depgraph 未同步阻断门禁<br/>文件: commit_gates/rename_depgraph_sync_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_rule_execution_pairing_gate_py["(生产态 / production)<br/>规则执行pairing门禁 / rule_execution_<br/>pairing_gate<br/>规则-执行配对门禁<br/>（RULE-EXECUTION-PAIRING，Phase 3.5）<br/>文件: commit_gates/rule_execution_<br/>pairing_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_rule_four_way_alignment_gate_py["(生产态 / production)<br/>规则fourway对齐门禁 / rule_four_way_<br/>alignment_gate<br/>规则四方对齐门禁<br/>文件: commit_gates/rule_four_way_<br/>alignment_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_ruling_commit_verified_gate_py["(生产态 / production)<br/>rulingcommitverified门禁 / ruling_<br/>commit_verified_gate<br/>文档'已完成'声明 commit hash<br/>真实性硬验证门禁<br/>文件: commit_gates/ruling_commit_<br/>verified_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_ruling_reference_gate_py["(生产态 / production)<br/>rulingreference门禁 / ruling_reference_<br/>gate<br/>裁定#NNN 悬空引用自动检测门禁<br/>文件: commit_gates/ruling_reference_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_schema_file_exists_gate_py["(生产态 / production)<br/>结构fileexists门禁 / schema_file_exists_<br/>gate<br/>SCHEMA-FILE-EXISTS block 门禁<br/>文件: commit_gates/schema_file_exists_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_scripts_import_integrity_gate_py["(生产态 / production)<br/>脚本导入完整性门禁 / scripts_import_<br/>integrity_gate<br/>脚本导入完整性门禁.constants<br/>符号导入完整性门禁<br/>文件: commit_gates/scripts_import_<br/>integrity_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_session_required_gate_py["(生产态 / production) 会话required门禁<br/>/ session_required_gate<br/>会话 注册强制门禁<br/>（SESSION-REQUIRED，2026-07-01 治本<br/>FP-ISO.4B 件1改）<br/>文件: commit_gates/session_required_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_snapshot_drift_gate_py["(生产态 / production) 快照漂移门禁 /<br/>snapshot_drift_gate<br/>运行时违规快照漂移阻断门禁<br/>（SNAPSHOT-DRIFT，#ARCH-GOV-CONVERGENCE-<br/>META Phase 3.6 补齐 rc1 enforceability）<br/>文件: commit_gates/snapshot_drift_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_ssot_redefinition_gate_py["(生产态 / production)<br/>ssotredefinition门禁 / ssot_<br/>redefinition_gate<br/>SSoT 符号重复定义硬阻断门禁<br/>文件: commit_gates/ssot_redefinition_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_table_name_registry_gate_py["(生产态 / production)<br/>tablename注册表门禁 / table_name_<br/>registry_gate<br/>TABLE-NAME-REGISTRY block 门禁<br/>文件: commit_gates/table_name_registry_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_test_source_consistency_gate_py["(生产态 / production) 测试源一致性门禁<br/>/ test_source_consistency_gate<br/>测试-源码符号一致性门禁<br/>（TEST-SOURCE-CONSISTENCY，§5.178<br/>防复发）<br/>文件: commit_gates/test_source_<br/>consistency_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_tests_coverage_gate_py["(生产态 / production) testscoverage门禁<br/>/ tests_coverage_gate<br/>Gate 测试覆盖率校验 meta-gate<br/>（META-TESTS-COVERAGE，#ARCH-057）<br/>文件: commit_gates/tests_coverage_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_ttl_gate_py["(生产态 / production) 存活时间门禁 /<br/>ttl_gate<br/>ttl 字段校验门禁（治本：弥补<br/>--no-verify 绕过 pre-commit GATE-15<br/>的缺口）<br/>文件: commit_gates/ttl_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_undefined_name_gate_py["(生产态 / production) undefinedname门禁<br/>/ undefined_name_gate<br/>UNDEFINED-NAME 门禁（F821<br/>未定义符号硬阻断）<br/>文件: commit_gates/undefined_name_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_unsafe_dict_spread_gate_py["(生产态 / production)<br/>unsafedictspread门禁 / unsafe_dict_<br/>spread_gate<br/>``**data`` 直接展开模式 warn 级门禁<br/>文件: commit_gates/unsafe_dict_spread_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_vocab_chain_gate_py["(生产态 / production) vocabchain门禁 /<br/>vocab_chain_gate<br/>SSoT 引用硬编码阻断门禁<br/>（VOCAB-CHAIN，#ARCH-GOV-CONVERGENCE-MET<br/>A Phase 3.6 补齐 rc2 enforceability）<br/>文件: commit_gates/vocab_chain_gate.py"]
    src_zephyr_gov_enforcement_commit_gates_vocab_hardcode_gate_py["(生产态 / production) vocabhardcode门禁<br/>/ vocab_hardcode_gate<br/>新增 .py 文件词表硬编码阻断门禁<br/>（VOCAB-HARDCODE，2026-07-03 Phase 1）<br/>文件: commit_gates/vocab_hardcode_<br/>gate.py"]
    src_zephyr_gov_enforcement_commit_gates_zephyr_env_direct_access_gate_py["(生产态 / production)<br/>zephyrenvdirect访问门禁 / zephyr_env_<br/>direct_access_gate<br/>ZEPHYR_ENV 直访硬阻断门禁<br/>文件: commit_gates/zephyr_env_direct_<br/>access_gate.py"]
    src_zephyr_gov_enforcement_rule_bridge_gate_auto_registrar_py["(生产态 / production) 门禁自动registrar<br/>/ gate_auto_registrar<br/>YAML 驱动的 in-process gate 自动注册器<br/>（#ARCH-GATE-REGISTRY-AUTO-001 Phase 3）<br/>文件: rule_bridge/gate_auto_registrar.py"]
    tests_data_test_symbol_normalizer_py["(生产态 / production)<br/>测试symbol归一化器 / test_symbol_<br/>normalizer<br/>TRAE-082 symbol 标准化模块测试。<br/>文件: data/test_symbol_normalizer.py"]
    tests_governance_test_apply_dataflowgraph_smoke_py["(生产态 / production)<br/>测试applydataflowgraphsmoke / test_<br/>apply_dataflowgraph_smoke.py — apply_<br/>dataflowgraph.py e<br/>测试应用dataflowgraphsmoke。test_apply_<br/>dataflowgraph_smoke.py — apply_<br/>dataflowgraph.py end-to-end smoke test<br/>文件: governance/test_apply_<br/>dataflowgraph_smoke.py"]
    tests_governance_test_apply_decisiongraph_smoke_py["(生产态 / production)<br/>测试applydecisiongraphsmoke / test_<br/>apply_decisiongraph_smoke.py — apply_<br/>decisiongraph.py e<br/>测试应用decisiongraphsmoke。test_apply_<br/>decisiongraph_smoke.py — apply_<br/>decisiongraph.py end-to-end smoke test<br/>文件: governance/test_apply_<br/>decisiongraph_smoke.py"]
    tests_governance_test_apply_depgraph_smoke_py["(生产态 / production)<br/>测试applydepgraphsmoke / test_apply_<br/>depgraph_smoke.py — apply_depgraph.py<br/>end-to-end<br/>测试应用依赖图smoke。test_apply_<br/>depgraph_smoke.py — apply_depgraph.py<br/>end-to-end smoke test<br/>文件: governance/test_apply_depgraph_<br/>smoke.py"]
    tests_governance_test_audit_return_contract_usage_py["(生产态 / production)<br/>测试审计returncontractusage / test_<br/>audit_return_contract_usage<br/>返回契约 ok 键审计脚本单元测试<br/>文件: governance/test_audit_return_<br/>contract_usage.py"]
    tests_governance_test_audit_worktree_ops_telemetry_py["(生产态 / production)<br/>测试审计worktree运维遥测 / test_audit_<br/>worktree_ops_telemetry<br/>worktree_ops_log 遥测完整性审计测试<br/>文件: governance/test_audit_worktree_<br/>ops_telemetry.py"]
    tests_governance_test_generate_project_depgraph_smoke_py["(生产态 / production)<br/>测试generateprojectdepgraphsmoke / test_<br/>generate_project_depgraph_smoke.py —<br/>generate_project_d<br/>测试生成project依赖图smoke。test_<br/>generate_project_depgraph_smoke.py —<br/>generate_project_depgraph.py e2e smoke<br/>test<br/>文件: governance/test_generate_project_<br/>depgraph_smoke.py"]
    tests_governance_test_post_commit_guard_no_verify_threshold_py["(生产态 / production)<br/>测试提交提交守卫no校验阈值 / test_post_<br/>commit_guard_no_verify_threshold<br/>高基数 --no-verify 阈值阻断 e2e 测试<br/>文件: governance/test_post_commit_guard_<br/>no_verify_threshold.py"]
    tests_governance_test_run_silent_failure_regression_py["(生产态 / production)<br/>测试runsilent故障regression / test_run_<br/>silent_failure_regression<br/>silent-failure 回归 runner 单元测试<br/>文件: governance/test_run_silent_<br/>failure_regression.py"]
    tests_governance_test_session_startup_health_check_py["(生产态 / production)<br/>测试会话启动健康检查 / test_session_<br/>startup_health_check<br/>AI session 启动健康度自检单元测试<br/>文件: governance/test_session_startup_<br/>health_check.py"]
    tests_governance_test_sync_yaml_to_depgraph_smoke_py["(生产态 / production)<br/>测试syncyamltodepgraphsmoke / test_sync_<br/>yaml_to_depgraph_smoke.py — sync_yaml_<br/>to_depgraph.<br/>测试同步yamlto依赖图smoke。test_sync_<br/>yaml_to_depgraph_smoke.py — sync_yaml_<br/>to_depgraph.py e2e smoke test<br/>文件: governance/test_sync_yaml_to_<br/>depgraph_smoke.py"]
    ...
    src_zephyr_gov_code_quality_code_dedup_ast_comparator_py["(生产态 / production) ast比较器 / ast_<br/>comparator<br/>Stage 2: AST 级精确比对器.<br/>文件: code_dedup/ast_comparator.py"]
    src_zephyr_gov_code_quality_code_dedup_auto_fixer_py["(生产态 / production)<br/>安全自动修复引擎——五直接开关+五间接约束.<br/>/ auto_fixer<br/>安全自动修复引擎——五直接开关+五间接约束.<br/>文件: code_dedup/auto_fixer.py"]
    src_zephyr_gov_code_quality_code_dedup_config_py["(生产态 / production) 配置 / config<br/>配置管理 — 策略树 YAML 加载 +<br/>项目规模感知四 Tier 自适应阈值.<br/>文件: code_dedup/config.py"]
    src_zephyr_gov_code_quality_code_dedup_exit_codes_py["(生产态 / production) 退出codes / exit_<br/>codes<br/>退出码定义模块——五档exit code<br/>0-4枚举+描述+判定逻辑.<br/>文件: code_dedup/exit_codes.py"]
    src_zephyr_gov_code_quality_code_dedup_report_py["(生产态 / production) 报告 / report<br/>报告生成器 — YAML/JSON 输出 +<br/>退出码判定 + Health Score 聚合.<br/>文件: code_dedup/report.py"]
    src_zephyr_gov_code_quality_code_dedup_trackers_import_surface_tracker_py["(生产态 / production)<br/>importsurface追踪器 / import_surface_<br/>tracker<br/>Import表面积负债追踪 — SBS 0-100 +<br/>shared burden score.<br/>文件: trackers/import_surface_tracker.py"]
    src_zephyr_gov_enforcement_commit_gates_diff_helpers_py["(生产态 / production) 差异辅助 / _diff_<br/>helpers<br/>门禁 共享 diff 解析工具模块<br/>文件: commit_gates/_diff_helpers.py"]
    ...
    src_zephyr_gov_enforcement_commit_gates_capability_lookup_bypass_policy_py["(生产态 / production)<br/>capabilitylookup绕过策略 / capability_<br/>lookup_bypass_policy<br/>CAPABILITY-LOOKUP bypass 策略共享模块<br/>文件: commit_gates/capability_lookup_<br/>bypass_policy.py"]
    src_zephyr_gov_enforcement_commit_gates_perm_trigger_gate_py["(生产态 / production) permtrigger门禁 /<br/>perm_trigger_gate<br/>永久系统脚本时间触发模式无事件订阅阻断门<br/>禁<br/>文件: commit_gates/perm_trigger_gate.py"]
    ...
    src_zephyr_gov_code_quality_code_dedup_report_py ~~~ src_zephyr_gov_code_quality_code_dedup_trackers_import_surface_tracker_py
    src_zephyr_gov_code_quality_code_dedup_trackers_import_surface_tracker_py ~~~ src_zephyr_gov_enforcement_commit_gates_diff_helpers_py
    ...
    src_zephyr_gov_code_quality_code_dedup_trackers_init_py -->|config_depends / config_depends| src_zephyr_gov_code_quality_code_dedup_trackers_import_surface_tracker_py
