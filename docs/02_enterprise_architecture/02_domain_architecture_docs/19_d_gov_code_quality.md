...
flowchart TD
    ...
    src_zephyr_gov_code_quality_init_py["zephyr/gov_code_quality 包入口<br/>管理zephyr.gov_code_quality子包加载<br/>gov_code_quality domain package — code quality<br/>governance (D<br/>文件: gov_code_quality/__init__.py<br/>(生产态 / production)"]
    src_zephyr_gov_code_quality_code_dedup_init_py["gov_code_quality/code_dedup 包入口<br/>管理gov_code_quality.code_dedup子包加载<br/>文件: code_dedup/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_behavioral_trust_checker_py["行为信任检查器 — 行为漂移DIVERGED检测.<br/>gov code quality/code dedup包的behavioral_trust_<br/>checker模块<br/>文件: code_dedup/behavioral_trust_checker.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_canary_register_py["金丝雀注册表维护器 — 注册/过期/腐败检测.<br/>gov code quality/code dedup包的canary_<br/>register模块<br/>文件: code_dedup/canary_register.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_contract_consistency_checker_py["API契约一致性检查器 — 存在性·行为·契约三维.<br/>gov code quality/code dedup包的contract_<br/>consistency_checker模块<br/>文件: code_dedup/contract_consistency_checker.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_fifteen_dimension_auditor_py["15维超综合审计首页 — 逐项证明'做过且做对'.<br/>gov code quality/code dedup包的fifteen_<br/>dimension_auditor模块<br/>文件: code_dedup/fifteen_dimension_auditor.py<br/>(生产态 / production)"]
    src_zephyr_gov_code_quality_code_dedup_file_creator_py["文件创建清单执行器 — 验证所有源/测试<br/>/数据文件存在性.<br/>gov code quality/code dedup包的file_creator模块<br/>文件: code_dedup/file_creator.py<br/>(生产态 / production)"]
    src_zephyr_gov_code_quality_code_dedup_function_discovery_py["共享函数主动发现 — 签名+语义双通道从被动到主动.<br/>gov code quality/code dedup包的function_<br/>discovery模块<br/>文件: code_dedup/function_discovery.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_integrations_py["集成管理——预提交钩子+CI-only 扫描+超时边界.<br/>gov code quality/code dedup包的integrations模块<br/>文件: code_dedup/integrations.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_mock_duplicate_generator_py["可控克隆生产器——零假阳性可期待引擎分子离散<br/>gov code quality/code dedup包的mock_duplicate_<br/>generator模块<br/>文件: code_dedup/mock_duplicate_generator.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_observation_window_guard_py["提取后稳定观察期守护 — 对标SDP 14天观察.<br/>gov code quality/code dedup包的observation_<br/>window_guard模块<br/>文件: code_dedup/observation_window_guard.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_policy_tree_validator_py["策略树自动一致性校验器 — 虚线箭头影响分析.<br/>gov code quality/code dedup包的policy_tree_<br/>validator模块<br/>文件: code_dedup/policy_tree_validator.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_prioritizer_py["修复优先级排序器 — 置信度×Impact×适配性<br/>三因子排序.<br/>gov code quality/code dedup包的prioritizer模块<br/>文件: code_dedup/prioritizer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_shared_evolver_py["共享函数自我进化引擎 — 自动升降级 +<br/>行为漂移锁定.<br/>gov code quality/code dedup包的shared_<br/>evolver模块<br/>文件: code_dedup/shared_evolver.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_simplicity_auditor_py["引擎成本效益自审计器 — SAS 0-100 月度审计 + Tax<br/>报告.<br/>gov code quality/code dedup包的simplicity_<br/>auditor模块<br/>文件: code_dedup/simplicity_auditor.py<br/>(生产态 / production)"]
    src_zephyr_gov_code_quality_code_dedup_ssot_registrar_py["SSoT注册器 — 提取函数自动注册到 shared API清单.<br/>gov code quality/code dedup包的ssot_<br/>registrar模块<br/>文件: code_dedup/ssot_registrar.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_success_validator_py["成功验证——判断一次去重操作是否真正消灭了克隆.<br/>gov code quality/code dedup包的success_<br/>validator模块<br/>文件: code_dedup/success_validator.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_thematic_clusterer_py["主题聚类器 — 噪声信号比·告警疲劳缓解.<br/>gov code quality/code dedup包的thematic_<br/>clusterer模块<br/>文件: code_dedup/thematic_clusterer.py<br/>(生产态 / production)"]
    src_zephyr_gov_code_quality_code_dedup_trackers_init_py["code_dedup/trackers 包入口<br/>管理code_dedup.trackers子包加载<br/>文件: trackers/__init__.py<br/>(生产态 / production)"]
    src_zephyr_gov_code_quality_code_dedup_trackers_blind_spot_tracker_py["盲点关闭追踪器 — 自动验证各轮盲点是否已覆盖.<br/>gov code quality/trackers包的blind_spot_<br/>tracker模块<br/>文件: trackers/blind_spot_tracker.py<br/>(生产态 / production)"]
    src_zephyr_gov_code_quality_code_dedup_trackers_consequence_tracker_py["后果追踪——记录每次修复操作对依赖方的影响.<br/>gov code quality/trackers包的consequence_<br/>tracker模块<br/>文件: trackers/consequence_tracker.py<br/>(生产态 / production)"]
    src_zephyr_gov_code_quality_code_dedup_trackers_hotspot_tracker_py["热点追踪器 — 90天滑动窗口 + 高频变动检测 +<br/>新项目预热清单.<br/>gov code quality/trackers包的hotspot_tracker模块<br/>文件: trackers/hotspot_tracker.py<br/>(生产态 / production)"]
    src_zephyr_gov_code_quality_code_dedup_trackers_question_tracker_py["问题追踪——扫描中发现需要人工处理的问题.<br/>gov code quality/trackers包的question_<br/>tracker模块<br/>文件: trackers/question_tracker.py<br/>(生产态 / production)"]
    src_zephyr_gov_code_quality_code_dedup_trackers_risk_mitigation_tracker_py["风险缓解追踪——捕获哪些克隆报告了但在N次扫描后仍<br/>未fix.<br/>gov code quality/trackers包的risk_mitigation_<br/>tracker模块<br/>文件: trackers/risk_mitigation_tracker.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_enforcement_commit_gates_init_py["gov_enforcement/commit_gates 包入口<br/>管理gov_enforcement.commit_gates子包加载<br/>文件: commit_gates/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_enforcement_commit_gates_scripts_import_integrity_gate_py["脚本导入完整性门禁<br/>constants 符号导入完整性门禁<br/>scripts_import_integrity_gate<br/>文件: commit_gates/scripts_import_integrity_<br/>gate.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_code_quality_code_dedup_auto_fixer_py["安全自动修复引擎——五直接开关+五间接约束.<br/>gov code quality/code dedup包的auto_fixer模块<br/>文件: code_dedup/auto_fixer.py<br/>(生产态 / production)"]
