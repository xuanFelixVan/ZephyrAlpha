...
graph TD
    ...
    class D_KNOWLEDGE,D_FEEDBACK_LOOP,D_GOV_AUDIT,D_AUTONOMY_CORE external_design
```
...
graph TD
    ...
    class D_GOVERNANCE,D_SHARED,D_INFRA_RUNTIME,D_FEEDBACK_LOOP,D_AUTONOMY_CORE,D_GOV_ENFORCEMENT,D_GOV_AUDIT,D_SECURITY_LLM external_design
```
...
graph TD
    ...
    class D_FEEDBACK_LOOP,D_AUTONOMY_CORE,D_GOV_AUDIT,D_INTELLIGENCE external_design
```
...
graph TD
    ...
    class D_SHARED external_prod
    class D_GOVERNANCE,D_KNOWLEDGE,D_FEEDBACK_LOOP,D_GOV_AUDIT,D_INFRA_RUNTIME,D_AUTONOMY_CORE external_design
```
...
| 75 | D_GOVERNANCE 生命周期管理: test_scheduler_act.py | → | Self-Bottleneck Detector — v0.38.0 R479 (self_... | 测试依赖 / test_depends |
| 76 | D_GOVERNANCE 生命周期管理: test_scheduler_act.py | → | R506: ContextWindowPressureManager (context_win... | 测试依赖 / test_depends |
| 77 | D_GOV_AUDIT 审计追踪: test_amplification_guard.py | → | Amplification Guard — v0.10.0 R134 (amplificat... | 测试依赖 / test_depends |
| 78 | D_GOV_AUDIT 审计追踪: test_api_dependency_metrics.py | → | API Dependency Metrics — v0.17.0+ R237-R242 (a... | 测试依赖 / test_depends |
| 79 | D_GOV_AUDIT 审计追踪: test_burn_rate_alerter.py | → | Burn Rate Alerter — v0.14.0 R200 (burn_rate_al... | 测试依赖 / test_depends |
| 80 | D_GOV_AUDIT 审计追踪: test_burnout_alarm.py | → | Burnout Alarm — v0.8.0 R100 (burnout_alarm.py) | 测试依赖 / test_depends |
| 81 | D_GOV_AUDIT 审计追踪: test_causal_inference_engine.py | → | Causal Inference Engine — v0.3.0 R5-R7 (causal... | 测试依赖 / test_depends |
| 82 | D_GOV_AUDIT 审计追踪: test_cognitive_load_budget.py | → | Cognitive Load Budget — v0.16.0 R223 (cognitiv... | 测试依赖 / test_depends |
| 83 | D_GOV_AUDIT 审计追踪: test_diagnosis_kpi.py | → | Diagnosis KPI — v0.9.0 R116 (diagnosis_kpi.py) | 测试依赖 / test_depends |
| 84 | D_GOV_AUDIT 审计追踪: test_global_health_map.py | → | Global Health Map — v0.8.0 R103 (global_health... | 测试依赖 / test_depends |
| 85 | D_GOV_AUDIT 审计追踪: test_human_anomaly_flood_detector.py | → | Human Anomaly Flood Detector — v0.40.0 R500 (h... | 测试依赖 / test_depends |
| 86 | D_GOV_AUDIT 审计追踪: test_interactive_diagnosis.py | → | Interactive Diagnosis — v0.7.0 R80 (interactiv... | 测试依赖 / test_depends |
| 87 | D_GOV_AUDIT 审计追踪: test_latency_slo.py | → | Latency SLO Monitor — v0.14.0 R192 (latency_sl... | 测试依赖 / test_depends |
| 88 | D_GOV_AUDIT 审计追踪: test_mtti_tracker.py | → | MTTI Tracker — v0.16.0 R221 (mtti_tracker.py) | 测试依赖 / test_depends |
| 89 | D_GOV_AUDIT 审计追踪: test_regime_gain_scheduling.py | → | Regime Gain Scheduling — v0.37.0 R453 (regime_... | 测试依赖 / test_depends |
| 90 | D_GOV_AUDIT 审计追踪: test_socratic_questions.py | → | Socratic Questions — v0.7.0 R81 (socratic_ques... | 测试依赖 / test_depends |
| 91 | D_GOV_AUDIT 审计追踪: test_statistical_hygiene_auditor.py | → | Statistical Hygiene Auditor — v0.38.0 R476 (st... | 测试依赖 / test_depends |
| 92 | D_GOV_AUDIT 审计追踪: test_toil_quantification.py | → | Toil Quantification — v0.37.0 R457 (toil_quant... | 测试依赖 / test_depends |
| 93 | D_GOV_AUDIT 审计追踪: test_tone_adapter.py | → | Tone Adapter — v0.9.0 R127 (tone_adapter.py) | 测试依赖 / test_depends |
| 94 | D_GOV_AUDIT 审计追踪: test_tone_adapter_v2.py | → | Tone Adapter v2 — v0.10.0 R141 (tone_adapter_v... | 测试依赖 / test_depends |
| 95 | D_GOV_AUDIT 审计追踪: test_value_added_baseline.py | → | Value Added Baseline — v0.10.0 R138 (value_add... | 测试依赖 / test_depends |
| 96 | D_GOV_AUDIT 审计追踪: test_zombie_fle_detector.py | → | Zombie FLE Detector — v0.16.0 R222 (zombie_fle... | 测试依赖 / test_depends |
| 97 | D_GOV_AUDIT 审计追踪: test_self_benchmark.py | → | Self Benchmark — v0.9.0 R115 (self_benchmark.py) | 测试依赖 / test_depends |
| 98 | D_GOV_AUDIT 审计追踪: test_self_bottleneck_detector.py | → | Self-Bottleneck Detector — v0.38.0 R479 (self_... | 测试依赖 / test_depends |
| 99 | D_GOV_AUDIT 审计追踪: test_self_health_monitor.py | → | Self Health Monitor — v0.4.0 R29 (self_health_... | 测试依赖 / test_depends |
| 100 | D_GOV_AUDIT 审计追踪: test_self_llm_observability.py | → | Self LLM Observability — v0.12.0 R160 (self_ll... | 测试依赖 / test_depends |
| 101 | D_GOV_ENFORCEMENT 规则执行: test_capacity_aware_repair.py | → | Capacity Aware Repair — v0.9.0 R120 (capacity_... | 测试依赖 / test_depends |
| 102 | D_GOV_ENFORCEMENT 规则执行: test_guard_interaction_topology_mapper.py | → | R518: GuardInteractionTopologyMapper (guard_int... | 测试依赖 / test_depends |
| 103 | D_GOV_ENFORCEMENT 规则执行: test_guard_self_consistency_auditor.py | → | R512: GuardSelfConsistencyAuditor (guard_self_c... | 测试依赖 / test_depends |
| 104 | D_INFRA_RUNTIME 运行时集成: test_cold_start_conservative_mode.py | → | R509: ColdStartConservativeMode (cold_start_con... | 测试依赖 / test_depends |
| 105 | D_INFRA_RUNTIME 运行时集成: test_temporal_integrity_guard.py | → | Temporal Integrity Guard — v0.38.0 R478 (tempo... | 测试依赖 / test_depends |
| 106 | D_INTELLIGENCE 上下文管理: test_model_health.py | → | Model Health Monitor — v0.5.0 R40 (model_healt... | 测试依赖 / test_depends |
| 107 | D_INTELLIGENCE 上下文管理: test_model_rotation.py | → | Model Rotation — v0.9.0 R125 (model_rotation.py) | 测试依赖 / test_depends |
| 108 | D_INTELLIGENCE 上下文管理: test_model_rotation_v2.py | → | Model Rotation v2 — v0.10.0 R140 (model_rotati... | 测试依赖 / test_depends |
| 109 | D_INTELLIGENCE 上下文管理: test_model_version_semantic_drift.py | → | Model Version Semantic Drift Monitor — v0.39.0... | 测试依赖 / test_depends |
| 110 | D_KNOWLEDGE 知识管理: test_knowledge_bus_factor_monitor.py | → | Knowledge Bus Factor Monitor — v0.38.0 R481 (k... | 测试依赖 / test_depends |
| 111 | D_KNOWLEDGE 知识管理: test_knowledge_market.py | → | Knowledge Market — v0.9.0 R126 (knowledge_mark... | 测试依赖 / test_depends |
| 112 | D_SECURITY_LLM LLM防御: test_llm_provider_integrity.py | → | LLM Provider Integrity — v0.15.0 R217 (llm_pro... | 测试依赖 / test_depends |
| 113 | D_SECURITY_LLM LLM防御: test_llm_quality_regression.py | → | LLM Quality Regression — v0.12.0 R161 (llm_qua... | 测试依赖 / test_depends |
| 114 | D_SHARED 共享服务: test_cross_guard_conflict_detector.py | → | R513: CrossGuardConflictDetector (cross_guard_c... | 测试依赖 / test_depends |
| 115 | D_SHARED 共享服务: test_cross_session_consistency_validator.py | → | R510: CrossSessionConsistencyValidator (cross_s... | 测试依赖 / test_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 12 个外部域直接连接（出边 1 条 + 入边 115 条 = 116 条）。只显示直接连接的域，不展开具体节点。
