...
flowchart TD
    ...
    class D_GOV_DRIFT,D_GOV_AUDIT,D_SHARED,D_GOV_OPS_RESILIENCE,D_INTEGRATION,D_GOV_ENFORCEMENT,D_GOVERNANCE,D_GOV_CODE_QUALITY,D_ORCHESTRATOR,D_GOV_SCRIPTS,D_INFRA_RUNTIME,D_INFRA_RECOVERY external_prod
```
...
| 55 | judge.py | → | D_GOV_RULE 规则治理: 门禁类型定义——GateType 枚举与 gate 相关 datac... | 导入依赖 / import_depends |
...
| 58 | defense_runner.py | → | D_GOV_RULE 规则治理: 任务类型定义——Task model 是任务卡字段的 SSoT.... | 导入依赖 / import_depends |
