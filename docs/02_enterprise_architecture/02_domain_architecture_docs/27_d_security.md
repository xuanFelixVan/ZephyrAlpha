...
flowchart TD
    ...
    src_zephyr_security_adversarial_validation_commit_trigger_py["提交触发器<br/>（MOD-INF-030），把 GitCommitGateway 的<br/>post-commit 事件桥接到红蓝对抗验证器，锁内轻量<br/>emit 触发提交后对抗验证会话<br/>commit_trigger<br/>文件: adversarial_validation/commit_trigger.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_orphan_judge_db_py["数据库<br/>孤儿判定记录 SQLite 库，存每个文件的判决/置信度<br/>/原因/扫描时间/文件哈希，供报告与复查使用。<br/>db<br/>文件: orphan_judge/db.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_llm_defense_llm_security_patterns_injection_patterns_py["注入模式<br/>描述符（legacy），定义提示注入的匹配模式供测试与<br/>检测使用<br/>injection_patterns<br/>文件: patterns/injection_patterns.py<br/>(生产态 / production)"]
