...
flowchart TD
    ...
    src_zephyr_gov_drift_analysis_py["分析<br/>治理漂移包的analysis模块<br/>_analysis<br/>文件: gov_drift/_analysis.py<br/>(生产态 / production)"]
    src_zephyr_gov_drift_core_py["核心<br/>治理漂移包的core模块<br/>_core<br/>文件: gov_drift/_core.py<br/>(生产态 / production)"]
    src_zephyr_gov_drift_drift_py["漂移<br/>治理漂移包的drift模块<br/>_drift<br/>文件: gov_drift/_drift.py<br/>(生产态 / production)"]
    src_zephyr_gov_drift_infrastructure_py["基础设施<br/>治理漂移包的infrastructure模块<br/>_infrastructure<br/>文件: gov_drift/_infrastructure.py<br/>(生产态 / production)"]
    src_zephyr_gov_drift_scanners_py["扫描器<br/>治理漂移包的scanners模块<br/>_scanners<br/>文件: gov_drift/_scanners.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_asymmetric_audit_py["asymmetric审计<br/>安全/访问控制包的asymmetric_audit模块<br/>AsymmetricAudit - quorum-based approval for<br/>high-risk operations.<br/>文件: access_control/asymmetric_audit.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_cascading_failure_isolator_py["级联故障隔离器<br/>访问控制的隔离器，隔离故障防止扩散<br/>Stub module: zephyr.security.access_<br/>control.cascading_failure_isolator —<br/>implementation pending.<br/>文件: access_control/cascading_failure_<br/>isolator.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_derive_rbac_roles_py["RBACRoleDeriver — RBAC 角色派生器<br/>安全/访问控制包的derive_rbac_roles模块<br/>文件: access_control/derive_rbac_roles.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_intent_binder_py["IntentBinder — 意图绑定与漂移检测<br/>安全/访问控制包的intent_binder模块<br/>文件: access_control/intent_binder.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_key_hierarchy_py["密钥hierarchy<br/>安全/访问控制包的key_hierarchy模块<br/>Stub module: zephyr.security.access_control.key_<br/>hierarchy — implementation pending.<br/>文件: access_control/key_hierarchy.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_microstructure_defense_py["微结构防御——对抗做市<br/>/交易微结构攻击的策略与保真度因子<br/>安全/访问控制包的microstructure_defense模块<br/>文件: access_control/microstructure_defense.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_monotonic_clock_py["MonotonicClock — 单调时钟<br/>安全/访问控制包的monotonic_clock模块<br/>文件: access_control/monotonic_clock.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_non_repudiation_py["NonRepudiation — 不可抵赖性审计签名<br/>安全/访问控制包的non_repudiation模块<br/>文件: access_control/non_repudiation.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_orphan_judge_config_loader_py["配置加载器<br/>配置加载器，主要提供加载、save、配置等功能<br/>config_loader<br/>文件: orphan_judge/config_loader.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_orphan_judge_drift_bridge_py["漂移桥接<br/>漂移桥接，主要提供notify变更、isavailable等功能<br/>drift_bridge<br/>文件: orphan_judge/drift_bridge.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_orphan_judge_escalation_bridge_py["升级桥接<br/>escalation桥接，主要提供escalatejudgment、评估风<br/>险、isavailable等功能<br/>escalation_bridge<br/>文件: orphan_judge/escalation_bridge.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_orphan_judge_feedback_bridge_py["反馈桥接<br/>反馈桥接，主要提供报告misjudgment、isavailable等<br/>功能<br/>feedback_bridge<br/>文件: orphan_judge/feedback_bridge.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_orphan_judge_kb_bridge_py["kb桥接<br/>kb桥接，主要提供writejudgment、search历史、isava<br/>ilable等功能<br/>kb_bridge<br/>文件: orphan_judge/kb_bridge.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_orphan_judge_mcp_integration_py["MCP集成<br/>安全/孤儿判定包的mcp_integration模块<br/>文件: orphan_judge/mcp_integration.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_orphan_judge_orphan_detector_py["(INVARIANTS) 蓝图 §4 文件清单与代码双向对齐<br/>安全/孤儿判定包的orphan_detector模块<br/>文件: orphan_judge/orphan_detector.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_orphan_judge_rbac_bridge_py["RBAC桥接<br/>rbac桥接，主要提供检查删除权限、isavailable等功<br/>能<br/>rbac_bridge<br/>文件: orphan_judge/rbac_bridge.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_orphan_judge_registration_checker_py["扫描项目注册表，判断文件是否已登记在册<br/>安全/孤儿判定包的registration_checker模块<br/>文件: orphan_judge/registration_checker.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_orphan_judge_report_generator_py["报告生成器<br/>报告生成器，主要提供生成、摘要text等功能<br/>report_generator<br/>文件: orphan_judge/report_generator.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_orphan_judge_standalone_evaluator_py["六指标加权评分: 文件大小(15%) + 代码行数(20%) +<br/>定义数(20%<br/>安全/孤儿判定包的standalone_evaluator模块<br/>文件: orphan_judge/standalone_evaluator.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_orphan_judge_swid_tag_py["SWID标签<br/>swid标签，主要提供构建等功能<br/>swid_tag<br/>文件: orphan_judge/swid_tag.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_orphan_judge_unique_analyzer_py["AST节点比对，检测文件中的独特代码元素(类/函数<br/>/常量定义等)<br/>安全/孤儿判定包的unique_analyzer模块<br/>文件: orphan_judge/unique_analyzer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_permission_mode_manager_py["权限mode管理器<br/>安全/访问控制包的permission_mode_manager模块<br/>Stub module: zephyr.security.access_<br/>control.permission_mode_manager —<br/>implementation pending.<br/>文件: access_control/permission_mode_manager.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_phase_executor_py["阶段执行器<br/>安全/访问控制包的phase_executor模块<br/>文件: access_control/phase_executor.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_session_lifecycle_py["会话生命周期<br/>安全/访问控制包的session_lifecycle模块<br/>Stub module: zephyr.security.access_<br/>control.session_lifecycle — implementation<br/>pending.<br/>文件: access_control/session_lifecycle.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_verifiers_continuous_verifier_py["continuous验证器<br/>安全/验证器包的continuous_verifier模块<br/>Stub module: zephyr.security.access_<br/>control.verifiers.continuous_verifier —<br/>implementation pending.<br/>文件: verifiers/continuous_verifier.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_verifiers_micro_verifier_py["micro验证器<br/>安全/验证器包的micro_verifier模块<br/>Stub module: zephyr.security.access_<br/>control.verifiers.micro_verifier —<br/>implementation pending.<br/>文件: verifiers/micro_verifier.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_verifiers_post_action_verifier_py["提交动作验证器<br/>安全/验证器包的post_action_verifier模块<br/>Stub module: zephyr.security.access_<br/>control.verifiers.post_action_verifier —<br/>implementation pending.<br/>文件: verifiers/post_action_verifier.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_adversarial_validation_attack_registry_py["攻击注册表<br/>attack注册表，主要提供注册、查询by层、数量等功能<br/>attack_registry<br/>文件: adversarial_validation/attack_registry.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_adversarial_validation_constitution_engine_py["constitution引擎<br/>对抗验证的注册表，登记和查询已注册的条目<br/>constitution_engine<br/>文件: adversarial_validation/constitution_<br/>engine.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_game_day_scheduler_py["gameday调度器<br/>对抗验证的调度器，按时间或优先级安排任务执行<br/>game_day_scheduler<br/>文件: adversarial_validation/game_day_<br/>scheduler.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_injection_engine_py["注入引擎<br/>注入引擎，提供blastradius、blastradius、inject等<br/>方法<br/>injection_engine<br/>文件: adversarial_validation/injection_engine.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_mcp_endpoints_py["MCP端点<br/>安全/对抗验证包的mcp_endpoints模块<br/>文件: adversarial_validation/mcp_endpoints.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_orphan_judge_cascade_analyzer_py["删除级联分析器——分析删除文件对项目的影响<br/>安全/孤儿判定包的cascade_analyzer模块<br/>文件: orphan_judge/cascade_analyzer.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_orphan_judge_db_py["数据库<br/>数据库，主要提供insert、获取、列表byverdict等功<br/>能<br/>db<br/>文件: orphan_judge/db.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_orphan_judge_decision_table_py["五层判定结果 -> 处置动作映射表<br/>安全/孤儿判定包的decision_table模块<br/>文件: orphan_judge/decision_table.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_orphan_judge_deprecation_tracker_py["废弃文件追踪器——标记和追踪废弃文件的生命周期<br/>安全/孤儿判定包的deprecation_tracker模块<br/>文件: orphan_judge/deprecation_tracker.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_adversarial_validation_cli_py["命令行<br/>安全/对抗验证包的cli模块<br/>文件: adversarial_validation/cli.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_constitution_guard_py["constitution守卫<br/>安全/对抗验证包的constitution_guard模块<br/>文件: adversarial_validation/constitution_<br/>guard.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_convergence_checker_py["convergence检查器<br/>安全/对抗验证包的convergence_checker模块<br/>文件: adversarial_validation/convergence_<br/>checker.py<br/>(生产态 / production)"]
    src_zephyr_security_llm_defense_llm_security_behavior_audit_logger_py["行为审计日志器<br/>安全/llm security包的behavior_audit_logger模块<br/>文件: llm_security/behavior_audit_logger.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_llm_defense_llm_security_patterns_injection_patterns_py["注入模式<br/>注入模式，提供match等方法<br/>injection_patterns<br/>文件: patterns/injection_patterns.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_llm_defense_llm_security_self_protection_isolation_py["LSG 自身隔离策略<br/>安全/自保护包的isolation模块<br/>文件: self_protection/isolation.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_adversarial_validation_cold_start_py["冷启动<br/>安全/对抗验证包的cold_start模块<br/>文件: adversarial_validation/cold_start.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_game_day_runner_py["gameday运行器<br/>提供tier、blastradius等方法<br/>game_day_runner<br/>文件: adversarial_validation/game_day_runner.py<br/>(生产态 / production)"]
    src_zephyr_security_llm_defense_llm_security_layers_l0_supply_chain_py["l0supply链<br/>安全/层级包的l0_supply_chain模块<br/>文件: layers/l0_supply_chain.py<br/>(生产态 / production)"]
    src_zephyr_security_llm_defense_llm_security_layers_l1_input_py["输入来源类型<br/>安全/层级包的l1_input模块<br/>文件: layers/l1_input.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_llm_defense_llm_security_layers_l3_output_py["兼容旧接口的输出过滤层<br/>安全/层级包的l3_output模块<br/>文件: layers/l3_output.py<br/>(生产态 / production)"]
    src_zephyr_security_llm_defense_llm_security_layers_l4_agent_py["风险等级<br/>安全/层级包的l4_agent模块<br/>文件: layers/l4_agent.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_llm_defense_llm_security_layers_l6_observability_py["l6可观测性<br/>安全/层级包的l6_observability模块<br/>L6 Observability Layer — security event<br/>logging, alerting, and reporting.<br/>文件: layers/l6_observability.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_identity_py["Agent identity — 角色与成熟度定义<br/>安全/访问控制包的identity模块<br/>文件: access_control/identity.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_adversarial_validation_validator_py["校验器<br/>安全/对抗验证包的validator模块<br/>文件: adversarial_validation/validator.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_adversarial_validation_blast_radius_py["爆炸半径<br/>安全/对抗验证包的blast_radius模块<br/>文件: adversarial_validation/blast_radius.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_bypass_recorder_py["绕过记录器<br/>绕过记录器，提供recordbypass、querybypasses、esc<br/>alatedentries等方法<br/>bypass_recorder<br/>文件: adversarial_validation/bypass_recorder.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_cleanup_py["清理<br/>安全/对抗验证包的cleanup模块<br/>文件: adversarial_validation/cleanup.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_defense_runner_py["防御运行器<br/>安全/对抗验证包的defense_runner模块<br/>文件: adversarial_validation/defense_runner.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_scenario_loader_py["场景加载器<br/>场景加载器，提供scenariocount、加载、获取等方法<br/>scenario_loader<br/>文件: adversarial_validation/scenario_loader.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_steady_state_py["steady状态<br/>安全/对抗验证包的steady_state模块<br/>文件: adversarial_validation/steady_state.py<br/>(生产态 / production)"]
    ...
    class D_GOV_DRIFT,D_SHARED,D_INFRA_RUNTIME,D_GOVERNANCE,D_GOV_AUDIT,D_GOV_OPS_RESILIENCE,D_COMPLIANCE,D_GOV_ENFORCEMENT,D_INTEGRATION external_prod
```
