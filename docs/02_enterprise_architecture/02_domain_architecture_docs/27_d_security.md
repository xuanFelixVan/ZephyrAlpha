...
flowchart TD
    ...
    src_zephyr_governance_agent_rbac_contracts_py["契约<br/>py — G-CT-001 RBAC 契约（re-export）<br/>contracts<br/>文件: agent-rbac/contracts.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_compliance_matrix_py["合规矩阵<br/>Stub module: zephyr.security.access_<br/>control.compliance_matrix — implementation<br/>pending<br/>文件: access_control/compliance_matrix.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_decision_registry_py["决策注册表<br/>DecisionRegistry - decision log with query and<br/>stats<br/>文件: access_control/decision_registry.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_defense_depth_py["防御深度<br/>Stub module: zephyr.security.access_<br/>control.defense_depth — implementation pending<br/>文件: access_control/defense_depth.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_dependency_auditor_py["依赖审计器<br/>Stub module: zephyr.security.access_<br/>control.dependency_auditor — implementation<br/>pending<br/>文件: access_control/dependency_auditor.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_detectors_anomaly_detector_py["异常检测器<br/>AnomalyDetector - rolling z-score anomaly<br/>detection per field<br/>文件: detectors/anomaly_detector.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_environment_manager_py["环境管理器<br/>Stub module: zephyr.security.access_<br/>control.environment_manager — implementation<br/>pending<br/>文件: access_control/environment_manager.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_orphan_judge_config_loader_py["配置加载器<br/>主要提供加载、save、配置等功能，供orphan-judge.j<br/>udge.OrphanJudge使用<br/>config_loader<br/>文件: orphan_judge/config_loader.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_orphan_judge_drift_bridge_py["漂移桥接<br/>主要提供notify变更、isavailable等功能，供orphan-<br/>judge.judge.OrphanJudge使用<br/>drift_bridge<br/>文件: orphan_judge/drift_bridge.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_orphan_judge_feedback_bridge_py["反馈桥接<br/>主要提供报告misjudgment、isavailable等功能，供or<br/>phan-judge.judge.OrphanJudge使用<br/>feedback_bridge<br/>文件: orphan_judge/feedback_bridge.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_orphan_judge_kb_bridge_py["kb桥接<br/>主要提供writejudgment、search历史、isavailable等<br/>功能，供orphan-judge.__main__._cmd_rep使用<br/>kb_bridge<br/>文件: orphan_judge/kb_bridge.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_orphan_judge_mcp_integration_py["MCP集成<br/>供MCP Server Tool Registry; Fast使用<br/>mcp_integration<br/>文件: orphan_judge/mcp_integration.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_orphan_judge_rbac_bridge_py["RBAC桥接<br/>主要提供检查删除权限、isavailable等功能，供orpha<br/>n-judge.judge.OrphanJudge使用<br/>rbac_bridge<br/>文件: orphan_judge/rbac_bridge.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_orphan_judge_report_generator_py["报告生成器<br/>主要提供生成、摘要text等功能，供orphan-judge.__<br/>main__._cmd_rep使用<br/>report_generator<br/>文件: orphan_judge/report_generator.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_orphan_judge_swid_tag_py["SWID标签<br/>主要提供构建等功能，供orphan-judge.db.JudgmentDB<br/>; re使用<br/>swid_tag<br/>文件: orphan_judge/swid_tag.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_phase_executor_py["阶段执行器<br/>按阶段执行安全访问控制流程<br/>phase_executor<br/>文件: access_control/phase_executor.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_adversarial_validation_injection_engine_py["注入引擎<br/>提供blastradius、blastradius、inject等方法，供va<br/>lidator.py ; game_day_r使用<br/>injection_engine<br/>文件: adversarial_validation/injection_engine.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_mcp_endpoints_py["MCP端点<br/>对抗验证的异常，定义本模块的异常类型<br/>mcp_endpoints<br/>文件: adversarial_validation/mcp_endpoints.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_orphan_judge_db_py["数据库<br/>主要提供insert、获取、列表byverdict等功能，供orp<br/>han-judge.__main__._cmd_rep使用<br/>db<br/>文件: orphan_judge/db.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_adversarial_validation_cli_py["命令行<br/>供End users; CI/CD; MCP tool wra使用<br/>cli<br/>文件: adversarial_validation/cli.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_constitution_guard_py["constitution守卫<br/>对抗验证的异常，定义本模块的异常类型<br/>constitution_guard<br/>文件: adversarial_validation/constitution_<br/>guard.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_convergence_checker_py["convergence检查器<br/>对抗验证的异常，定义本模块的异常类型<br/>convergence_checker<br/>文件: adversarial_validation/convergence_<br/>checker.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_llm_defense_llm_security_input_sanitizer_py["输入清洗器<br/>基础设施异常基类（InputSanitizer<br/>所有异常由此派生）<br/>InputSanitizer: path whitelist + command<br/>whitelist + token budget guard.<br/>文件: llm_security/input_sanitizer.py<br/>(生产态 / production)"]
    src_zephyr_security_llm_defense_llm_security_patterns_injection_patterns_py["注入模式<br/>提供match等方法，供tests.llm_security.test_i使用<br/>injection_patterns<br/>文件: patterns/injection_patterns.py<br/>(生产态 / production)"]
    src_zephyr_security_llm_defense_llm_security_patterns_secrets_py["密钥<br/>依赖secrets工作<br/>文件: patterns/secrets.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_adversarial_validation_cold_start_py["冷启动<br/>供game_day_runner.py; validator.使用<br/>cold_start<br/>文件: adversarial_validation/cold_start.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_game_day_runner_py["gameday运行器<br/>提供tier、blastradius等方法，供game_day_<br/>scheduler.py; cl使用<br/>game_day_runner<br/>文件: adversarial_validation/game_day_runner.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_llm_defense_llm_security_layers_l8_multi_agent_py["l8多代理<br/>Represents a communication item between agents<br/>l8_multi_agent<br/>文件: layers/l8_multi_agent.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_llm_defense_llm_security_self_protection_l7_validation_py["l7验证<br/>Manages special risks for DeepSeek models<br/>l7_validation<br/>文件: self_protection/l7_validation.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_access_control_identity_py["身份定义<br/>Agent identity —<br/>角色与成熟度定义，管理Agent身份和权限等级<br/>文件: access_control/identity.py<br/>(生产态 / production)"]
    src_zephyr_security_access_control_immutable_core_py["身份定义<br/>ImmutableCore — 不可变核心验证器<br/>immutable_core<br/>文件: access_control/immutable_core.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_adversarial_validation_blast_radius_py["爆炸半径<br/>对抗验证的异常，定义本模块的异常类型<br/>blast_radius<br/>文件: adversarial_validation/blast_radius.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_bypass_recorder_py["绕过记录器<br/>提供recordbypass、querybypasses、escalatedentrie<br/>s等方法，供validator.py ; convergenc使用<br/>bypass_recorder<br/>文件: adversarial_validation/bypass_recorder.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_cleanup_py["清理<br/>对抗验证的异常，定义本模块的异常类型<br/>cleanup<br/>文件: adversarial_validation/cleanup.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_defense_runner_py["防御运行器<br/>对抗验证的异常，定义本模块的异常类型<br/>defense_runner<br/>文件: adversarial_validation/defense_runner.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_scenario_loader_py["场景加载器<br/>提供scenariocount、加载、获取等方法，供validator<br/>.py; attack_regi使用<br/>scenario_loader<br/>文件: adversarial_validation/scenario_loader.py<br/>(生产态 / production)"]
    src_zephyr_security_adversarial_validation_steady_state_py["steady状态<br/>对抗验证的异常，定义本模块的异常类型<br/>steady_state<br/>文件: adversarial_validation/steady_state.py<br/>(生产态 / production)"]
...
| 67 | 身份定义 / identity (access_control/identity.py) | → | D_SHARED 共享服务: 代理identity / agent_identity (identity/agent_identity.py) | 导入依赖 / import_depends |
| 68 | 身份定义 / immutable_core (access_control/immutable_core.py) | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
