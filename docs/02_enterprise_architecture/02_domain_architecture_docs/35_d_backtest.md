...
flowchart TD
    ...
    class D_EX_CORE,D_SHARED,D_INFRA_RUNTIME,D_FACTOR,D_DATA,D_GOVERNANCE,D_PF_CORE external_prod
```
...
| 6 | D-FACTOR → D-BACKTEST 数据流验证 / Test Backtest Factor ... | → | D_FACTOR 因子: D-FACTOR-03 因子评估指标——纯函数模块（无 IO 依赖）。 / ... | 测试依赖 / test_depends |
