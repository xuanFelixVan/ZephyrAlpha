...
> 展示全部 27 个模块（生产态 20 + 设计态 7），含跨域依赖外部节点。节点含成熟度+名称+大白话/简介+文件路径。
...
flowchart TD
    ...
    class D_EX_CORE,D_DATA,D_INFRA_RUNTIME,D_SHARED,D_GOVERNANCE,D_PF_CORE external_prod
```
...
> 本域与 6 个外部域直接连接（出边 11 条 + 入边 13 条 = 24 条）。只显示直接连接的域，不展开具体节点。
