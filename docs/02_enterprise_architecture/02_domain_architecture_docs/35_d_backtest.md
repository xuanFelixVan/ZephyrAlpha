...
flowchart TD
    ...
    src_zephyr_backtest_services_result_deployer_py["结果deployer<br/>结果deployer，回测的结果，封装操作结果的数据结构<br/>⛔ 受限：涉及实盘安全，需D-EX-CORE执行域就绪<br/>result_deployer<br/>文件: services/result_deployer.py<br/>(设计态 / design)"]
