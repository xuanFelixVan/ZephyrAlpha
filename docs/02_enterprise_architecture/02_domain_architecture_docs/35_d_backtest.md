...
flowchart TD
    ...
    src_zephyr_backtest_services_scheduler_py["D-BACKTEST BT-17<br/>回测自动调度器——批量+参数网格+队列管理+结<br/>服务包的scheduler模块<br/>文件: services/scheduler.py<br/>(生产态 / production)"]
    ...
    src_zephyr_backtest_services_data_quality_checker_py["数据质量检查器<br/>数据质量检查器，回测的检查器，检查某项条件是否满<br/>足。<br/>data_quality_checker<br/>文件: services/data_quality_checker.py<br/>(生产态 / production)"]
...
| 10 | 数据质量检查器 / data_quality_checker (services/data_qual... | → | D_SHARED 共享服务: 错误 / errors (foundation/errors.py) | 导入依赖 / import_depends |
