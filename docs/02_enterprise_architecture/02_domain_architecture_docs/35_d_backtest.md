...
flowchart TD
    src_zephyr_backtest_core_data_handler_py["(生产态 / production) 数据处理器 / data_handler<br/>回测数据处理器模块（v1.1.0 扩展：多源化 +<br/>ClickHouse 实现 + Tick 源）<br/>文件: core/data_handler.py"]
    src_zephyr_backtest_implementations_event_driven_engine_py["(生产态 / production) 事件driven引擎 / event_<br/>driven_engine<br/>事件驱动回测引擎（v1.1.0 新增，Tick 级回测核心）<br/>文件: implementations/event_driven_engine.py"]
    src_zephyr_backtest_io_init_py["(生产态 / production) 包入口 / __init__<br/>io · D_BACKTEST 可视化产物 io 子包（v1.3.0<br/>新增，#ARCH-047）<br/>文件: io/__init__.py"]
    src_zephyr_backtest_services_anomaly_diagnoser_py["(设计态 / design) 异常诊断器 / anomaly_diagnoser<br/>异常诊断器（anomaly_diagnoser.py）<br/>文件: services/anomaly_diagnoser.py<br/>⛔ 暂缓：P2优先级，当前回测失败率低"]
    src_zephyr_backtest_services_decay_monitor_py["(设计态 / design) decay监控器 / decay_monitor<br/>decay监控，回测的监控器，持续监视某项指标，异常<br/>时上报。<br/>文件: services/decay_monitor.py<br/>⛔ 暂缓：因子侧decay_monitor已覆盖IC衰减监控"]
    src_zephyr_backtest_services_nan_processor_py["(设计态 / design) nan处理器 / nan_processor<br/>nan处理器，服务的处理器，处理加工数据。<br/>文件: services/nan_processor.py<br/>⛔ 暂缓：P2优先级，当前数据缺失率低"]
    src_zephyr_backtest_services_param_analyzer_py["(设计态 / design) param分析器 / param_analyzer<br/>param分析器，回测的分析器，分析数据找出问题或规<br/>律。<br/>文件: services/param_analyzer.py<br/>⛔ 暂缓：scheduler已含best/worst/mean摘要"]
    src_zephyr_backtest_services_report_generator_py["(设计态 / design) 报告生成器 / report_generator<br/>报告生成器，回测的生成器，按规则生成所需的数据或<br/>报告。<br/>文件: services/report_generator.py<br/>⛔ 暂缓：P2优先级，当前无报告展示需求"]
    src_zephyr_backtest_services_result_comparator_py["(设计态 / design) 结果比较器 / result_comparator<br/>结果comparator，回测的结果，封装操作结果的数据结<br/>构。<br/>文件: services/result_comparator.py<br/>⛔ 暂缓：P2优先级，当前无多次对比需求"]
    src_zephyr_backtest_services_result_deployer_py["(设计态 / design) 结果deployer / result_deployer<br/>结果deployer，回测的结果，封装操作结果的数据结构<br/>。<br/>文件: services/result_deployer.py<br/>⛔ 受限：涉及实盘安全，需D-EX-CORE执行域就绪"]
    src_zephyr_backtest_services_scheduler_py["(生产态 / production) D-BACKTEST BT-17<br/>回测自动调度器——批量+参数网格+队列管理+结 /<br/>scheduler<br/>D-BACKTEST BT-17<br/>回测自动调度器——批量+参数网格+队列管理+结果聚合<br/>。<br/>文件: services/scheduler.py"]
    ...
    src_zephyr_backtest_core_pit_manager_py["(生产态 / production) pit管理器 / pit_manager<br/>pit管理器(Point-In-Time)铁律管理器模块<br/>文件: core/pit_manager.py"]
    src_zephyr_backtest_core_tick_replay_py["(生产态 / production) 逐笔replay / tick_replay<br/>Tick 回放引擎模块（v1.1.0 新增，秒级做T专用）<br/>文件: core/tick_replay.py"]
    src_zephyr_backtest_implementations_vectorized_engine_py["(生产态 / production) vectorized引擎 / L_<br/>BACKTEST — Vectorized Backtest Engine<br/>vectorized引擎。L_BACKTEST — Vectorized<br/>Backtest Engine<br/>文件: implementations/vectorized_engine.py"]
    src_zephyr_backtest_io_decisiongraph_adapter_py["(生产态 / production) decisiongraph适配器 /<br/>decisiongraph_adapter<br/>BacktestResult -> decisiongraph 适配器<br/>（TRAE-061 Phase 5）<br/>文件: io/decisiongraph_adapter.py"]
    src_zephyr_backtest_io_result_repository_py["(生产态 / production) 结果仓库 / result_<br/>repository<br/>result_repository · 回测产物持久化/检索模块<br/>（v1.3.0 新增，#ARCH-047）<br/>文件: io/result_repository.py"]
    src_zephyr_backtest_services_cache_manager_py["(设计态 / design) 缓存管理器 / cache_manager<br/>缓存管理器，回测的缓存，暂存常用数据加速访问。<br/>文件: services/cache_manager.py<br/>⛔ 暂缓：P2优先级，当前回测量不大"]
    src_zephyr_backtest_services_data_quality_checker_py["(设计态 / design) 数据质量检查器 / data_quality_<br/>checker<br/>数据质量检查器，回测的检查器，检查某项条件是否满<br/>足。<br/>文件: services/data_quality_checker.py<br/>⛔ 暂缓：data域已有quality_gate+integrity_checker"]
    ...
    src_zephyr_backtest_core_matching_engine_py["(生产态 / production) matching引擎 / matching_<br/>engine<br/>回测撮合引擎模块（v1.1.0 重构：委托<br/>MatchingLogic 保证回测=实盘一致性）<br/>文件: core/matching_engine.py"]
    src_zephyr_backtest_core_metrics_py["(生产态 / production) 回测绩效指标计算模块 /<br/>metrics<br/>回测绩效指标计算模块<br/>文件: core/metrics.py"]
    src_zephyr_backtest_core_walk_forward_py["(生产态 / production) walk前 / walk_forward<br/>Walk-Forward分析与多重比较偏差校正模块<br/>文件: core/walk_forward.py"]
    src_zephyr_backtest_io_backtest_result_sink_py["(生产态 / production) 回测结果sink / backtest_<br/>result_sink<br/>回测结果汇 · 回测结果数据落地模块（v1.3.0<br/>新增，#ARCH-047）<br/>文件: io/backtest_result_sink.py"]
    ...
    src_zephyr_backtest_core_engine_base_py["(生产态 / production) 引擎基类 / L_BACKTEST —<br/>Backtest Engine Layer<br/>引擎基类。L_BACKTEST — Backtest Engine Layer<br/>文件: core/engine_base.py"]
    ...
    src_zephyr_backtest_core_overfitting_detector_py["(生产态 / production) 过拟合检测模块(三维度 +<br/>三层) / overfitting_detector<br/>过拟合检测模块(三维度 + 三层)<br/>文件: core/overfitting_detector.py"]
