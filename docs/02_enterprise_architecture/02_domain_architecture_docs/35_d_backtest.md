...
flowchart TD
    ...
    tests_backtest_test_data_handler_pit_py -->|测试依赖 / test_depends| D_FACTOR
    src_zephyr_backtest_io_result_repository_py -->|导入依赖 / import_depends| D_SHARED
    ...
    class D_INFRA_RUNTIME,D_FACTOR,D_SHARED,D_DATA,D_EX_CORE,D_PF_CORE,D_GOVERNANCE external_prod
```
...
> 本域与 7 个外部域直接连接（出边 21 条 + 入边 14 条 = 35 条）。只显示直接连接的域，不展开具体节点。
