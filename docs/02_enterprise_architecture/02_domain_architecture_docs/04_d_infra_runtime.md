...
graph TD
    ...
    class D_KNOWLEDGE,D_GOV_DOCS,D_GOV_SCRIPTS,D_SECURITY_LLM,D_GOV_CODE_QUALITY,D_AUTONOMY_CORE external_design
```
...
graph TD
    ...
    class D_GOVERNANCE,D_INTEGRATION,D_BACKTEST,D_ORCHESTRATOR external_prod
    class D_SHARED,D_DATA,D_INFRASTRUCTURE,D_GOV_SCRIPTS,D_AUTONOMY_CORE external_design
```
...
graph TD
    ...
    class D_INFRASTRUCTURE,D_TRADING,D_AUTONOMY_CORE,D_SECURITY_LLM external_design
```
...
graph TD
    ...
    class D_GOV_OPS_RESILIENCE,D_INFRA_TELEMETRY,D_GOVERNANCE,D_INFRA_RECOVERY,D_GOV_SCRIPTS,D_ORCHESTRATOR external_design
```
...
graph TD
    ...
    class D_INFRA_TELEMETRY,D_TRADING,D_INTEGRATION external_prod
    class D_SHARED,D_GOV_AUDIT,D_GOVERNANCE,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_INFRA_A2A,D_SHARED,D_INTEGRATION,D_GOV_OPS_RESILIENCE,D_GOV_AUDIT,D_FBL_DIAGNOSERS,D_SECURITY,D_INFRA_RECOVERY,D_AUTONOMY_CORE,D_GOVERNANCE external_prod
...
graph TD
    ...
    class D_GOVERNANCE,D_FEEDBACK_LOOP,D_FBL_DETECTORS,D_SHARED,D_INFRA_RECOVERY,D_INFRA_A2A,D_GOV_DRIFT external_prod
```
...
graph TD
    ...
    class D_GOV_AUDIT,D_TRADING,D_AUTONOMY_CORE,D_INFRASTRUCTURE,D_GOV_SCRIPTS external_design
```
...
| 48 | phase_manager 命名空间桥——重导出 zephyr.gover... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: __init__.py | 导入依赖 / import_depends |
| 49 | phase_manager 命名空间桥——重导出 zephyr.gover... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Phase Manager — ZephyrAlpha 施工阶段门控引擎. ... | 导入依赖 / import_depends |
| 50 | auto_bootstrap — 全自动遥测注入钩子（MOD-INF-0... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Phase Manager — ZephyrAlpha 施工阶段门控引擎. ... | 导入依赖 / import_depends |
| 51 | AutoRuntimeCore — 三层运行时运营中心（系统大脑... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Coldstart Manager — v0.7.0 冷启动管理器: escal... | 导入依赖 / import_depends |
| 52 | boot_hooks.py | → | D_GOV_OPS_RESILIENCE 运维弹性治理: EventHook — 声明式任务系统事件订阅 (event_hook.py) | 导入依赖 / import_depends |
| 53 | boot_hooks.py | → | D_GOV_OPS_RESILIENCE 运维弹性治理: F5BootIntegration — F5 自动启动/关闭集成 (MOD-... | 导入依赖 / import_depends |
| 54 | boot_hooks.py | → | D_GOV_OPS_RESILIENCE 运维弹性治理: F5ShutdownManager — F5 自动关闭/状态持久化/信.... | 导入依赖 / import_depends |
| 55 | test_event_hook.py | → | D_GOV_OPS_RESILIENCE 运维弹性治理: EventHook — 声明式任务系统事件订阅 (event_hook.py) | 测试依赖 / test_depends |
...
| 215 | 包 shared.capacity_governance 的初始化文件。 (_... | → | D_SHARED 共享服务: capacity_calibrator.py | config_depends / config_depends |
...
| 223 | __init__.py | → | D_SHARED 共享服务: CT-HEALTH-001: System-wide Health Discovery Reg... | config_depends / config_depends |
| 224 | 包 shared.maintenance 的初始化文件。 (__init__.py) | → | D_SHARED 共享服务: code_economy_analyzer.py | config_depends / config_depends |
