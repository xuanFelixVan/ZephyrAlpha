...
graph TD
    ...
    class D_SHARED,D_GOV_SCRIPTS,D_AUTONOMY_CORE,D_ORCHESTRATOR external_design
```
...
graph TD
    ...
    class D_GOV_OPS_RESILIENCE,D_SHARED,D_INTELLIGENCE,D_GOVERNANCE,D_FEEDBACK_LOOP,D_SECURITY,D_INTEGRATION,D_TRADING external_prod
...
graph TD
    ...
    class D_GOV_OPS_RESILIENCE,D_INTELLIGENCE,D_GOVERNANCE,D_INTEGRATION external_prod
    class D_SHARED,D_ORCHESTRATOR,D_GOV_SCRIPTS,D_AUTONOMY_CORE,D_COMPLIANCE external_design
```
...
> 本域与 24 个外部域直接连接（出边 209 条 + 入边 72 条 = 281 条）。只显示直接连接的域，不展开具体节点。
