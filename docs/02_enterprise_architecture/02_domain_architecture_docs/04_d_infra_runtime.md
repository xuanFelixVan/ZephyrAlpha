...
graph TD
    ...
    class D_SHARED,D_KNOWLEDGE,D_GOV_DOCS,D_FACTOR,D_GOV_CODE_QUALITY,D_SECURITY,D_GOVERNANCE external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOV_RULE,D_GOVERNANCE,D_BACKTEST,D_INTELLIGENCE,D_INTEGRATION external_prod
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_DRIFT,D_INTEGRATION,D_FEEDBACK_LOOP,D_GOV_OPS_RESILIENCE external_prod
...
graph TD
    ...
    class D_AUTONOMY_CORE,D_SHARED,D_GOVERNANCE,D_SECURITY,D_GOV_OPS_RESILIENCE,D_GOV_DRIFT,D_INTEGRATION,D_TRADING external_prod
...
graph TD
    ...
    class D_SHARED,D_AUTONOMY_CORE,D_GOVERNANCE,D_SECURITY,D_GOV_OPS_RESILIENCE,D_INTEGRATION external_prod
    class D_ORCHESTRATOR,D_COMPLIANCE,D_GOV_SCRIPTS external_design
```
...
| 49 | D_INTEGRATION 管线路由: MCP Gateway 集中式治理节点（MOD-INF-013 §12 Ph... | → | Telemetry — 系统遥测门面类（MOD-INF-015 v2.1.0... | 导入依赖 / import_depends |
...
> 本域与 25 个外部域直接连接（出边 209 条 + 入边 74 条 = 283 条）。只显示直接连接的域，不展开具体节点。
