...
> 展示全部 329 个模块（生产态 149 + 设计态 1 + 原型态 179），标签标注成熟度。
...
graph TD
    ...
    class D_SHARED,D_FEEDBACK_LOOP,D_GOVERNANCE,D_GOV_AUDIT,D_GOV_REPAIR external_prod
    class D_GOV_OPS_RESILIENCE,D_GOV_CODE_QUALITY,D_SECURITY,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    D_AUTONOMY_CORE -.->|测试依赖 / test_depends| src_zephyr_infrastructure_auto_fix_engine_engine_py
    ...
    class D_GOV_AUDIT,D_COMPLIANCE,D_AUTONOMY_CORE,D_INTELLIGENCE external_design
```
...
graph TD
    ...
    class D_GOVERNANCE,D_INTEGRATION,D_AUTONOMY_CORE,D_BACKTEST,D_ORCHESTRATOR external_prod
...
graph TD
    ...
    class D_SHARED,D_GOV_RULE,D_FEEDBACK_LOOP,D_INTEGRATION,D_INTELLIGENCE external_prod
...
graph TD
    ...
    class D_ORCHESTRATOR,D_GOVERNANCE external_design
```
...
graph TD
    ...
    class D_AUTONOMY_CORE,D_INTEGRATION,D_INFRA_A2A,D_INFRA_TELEMETRY,D_TRADING external_prod
    class D_SHARED,D_FEEDBACK_LOOP external_design
```
...
graph TD
    ...
    class D_GOVERNANCE,D_AUTONOMY_CORE,D_AUTONOMY_PERM,D_INTELLIGENCE external_design
```
...
graph TD
    ...
    class D_SHARED,D_INTELLIGENCE,D_INTEGRATION,D_AUTONOMY_CORE,D_FBL_DIAGNOSERS,D_SECURITY,D_GOV_AUDIT,D_GOV_OPS_RESILIENCE,D_GOVERNANCE external_prod
...
graph TD
    ...
    class D_INFRA_RECOVERY,D_FEEDBACK_LOOP,D_SHARED,D_INFRA_A2A,D_GOVERNANCE external_prod
```
...
graph TD
    ...
    class D_FEEDBACK_LOOP,D_INFRA_A2A,D_INFRA_RECOVERY,D_SHARED,D_INFRA_TELEMETRY,D_SECURITY,D_FBL_DETECTORS,D_GOV_DRIFT external_prod
```
...
graph TD
    ...
    class D_SHARED,D_INTEGRATION,D_AUTONOMY_CORE,D_FEEDBACK_LOOP,D_GOV_REPAIR external_prod
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 179 个，19 条域内依赖）。
...
graph TD
    ...
    class D_SHARED,D_AUTONOMY_CORE,D_INTEGRATION,D_BACKTEST,D_GOV_OPS_RESILIENCE,D_INFRA_A2A,D_INFRA_TELEMETRY external_prod
    class D_GOVERNANCE,D_FEEDBACK_LOOP external_design
```
...
| 86 | 包 shared.knowledge 的初始化文件。 (__init__.py) | → | D_INTEGRATION 管线路由: kg_interface.py | config_depends / config_depends |
...
| 161 | DatabaseService: 统一管理数据库的连接池、生命周... | → | D_SHARED 共享服务: secrets.py —— Secrets 管理抽象（Phase 7 新增 ... | 导入依赖 / import_depends |
...
| 220 | 包 shared.reliability 的初始化文件。 (__init__.py) | → | D_SHARED 共享服务: Diff Planner — 最小增量变更规划器。 (diff_plan... | config_depends / config_depends |
