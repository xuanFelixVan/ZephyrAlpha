...
graph TD
    ...
    class D_GOVERNANCE,D_FEEDBACK_LOOP,D_INFRA_A2A,D_INFRA_RECOVERY,D_INFRA_TELEMETRY,D_SECURITY,D_SHARED,D_FBL_DETECTORS,D_GOV_DRIFT external_prod
```
...
| 195 | shared.schema — auto-generated package init. (... | → | D_SHARED 共享服务: schema_registry.py | config_depends / config_depends |
...
> 本域与 29 个外部域直接连接（出边 276 条 + 入边 223 条 = 499 条）。只显示直接连接的域，不展开具体节点。
