...
graph TD
    ...
    class D_SECURITY,D_GOV_SCRIPTS,D_AUTONOMY_CORE,D_SECURITY_LLM,D_GOV_CODE_QUALITY external_design
```
...
graph TD
    ...
    class D_COMPLIANCE,D_AUTONOMY_CORE,D_GOV_AUDIT,D_SECURITY_LLM external_design
```
...
graph TD
    ...
    class D_SHARED,D_INTEGRATION,D_GOVERNANCE,D_AUTONOMY_CORE,D_BACKTEST external_prod
...
graph TD
    ...
    class D_SHARED,D_INTEGRATION,D_FEEDBACK_LOOP external_prod
    class D_AUTONOMY_CORE,D_SECURITY_LLM,D_TRADING,D_INFRASTRUCTURE external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOV_OPS_RESILIENCE,D_INFRA_TELEMETRY,D_INFRA_RECOVERY external_prod
    class D_GOVERNANCE,D_ORCHESTRATOR,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_INTEGRATION,D_SHARED,D_INFRASTRUCTURE,D_FEEDBACK_LOOP,D_SECURITY,D_GOV_OPS_RESILIENCE,D_GOVERNANCE external_prod
    class D_AUTONOMY_CORE,D_TRADING,D_INTELLIGENCE,D_GOV_AUDIT,D_AUTONOMY_PERM external_design
```
...
graph TD
    ...
    class D_SHARED,D_INTELLIGENCE,D_FBL_DIAGNOSERS,D_AUTONOMY_CORE,D_SECURITY,D_GOV_OPS_RESILIENCE,D_GOVERNANCE,D_GOV_AUDIT,D_INFRA_A2A external_prod
...
graph TD
    ...
    class D_SHARED,D_INFRA_A2A,D_INFRA_RECOVERY,D_FEEDBACK_LOOP,D_GOVERNANCE external_prod
```
...
graph TD
    ...
    class D_INFRA_A2A,D_FEEDBACK_LOOP,D_SHARED,D_INFRA_TELEMETRY,D_SECURITY,D_FBL_DETECTORS,D_GOV_REPAIR,D_INFRA_RECOVERY external_prod
```
...
graph TD
    ...
    class D_SHARED,D_INTEGRATION,D_AUTONOMY_CORE,D_INFRA_RECOVERY external_prod
    class D_TRADING,D_COMPLIANCE external_design
```
...
graph TD
    ...
    class D_GOVERNANCE,D_SECURITY,D_ORCHESTRATOR,D_FEEDBACK_LOOP,D_GOV_SCRIPTS external_design
```
...
| 206 | 包 shared.capacity_governance 的初始化文件。 (_... | → | D_SHARED 共享服务: capacity_calibrator.py | config_depends / config_depends |
...
| 214 | 包 shared.maintenance 的初始化文件。 (__init__.py) | → | D_SHARED 共享服务: Dogfooding — 自举测试：用 TaskCard 管理 TaskCa... | config_depends / config_depends |
...
| 221 | shared.schema — auto-generated package init. (... | → | D_SHARED 共享服务: task_types — 任务系统核心类型 re-export 层 (ta... | config_depends / config_depends |
...
| 66 | D_GOVERNANCE 生命周期管理: context_budget.py —— 上下文预算管理与超预算截... | → | token_budget.py — Token 估算工具 SSoT (token_b... | 导入依赖 / import_depends |
| 67 | D_GOVERNANCE 生命周期管理: MiniQMT 实盘行情 Provider（Tick + 5档盘口） (mi... | → | DatabaseService: 统一管理数据库的连接池、生命周... | 导入依赖 / import_depends |
...
| 132 | D_INTEGRATION 管线路由: LocalModelScheduler — L2 本地模型 24/7 调度循... | → | resource_optimization_engine.py | 导入依赖 / import_depends |
...
> 本域与 30 个外部域直接连接（出边 307 条 + 入边 245 条 = 552 条）。只显示直接连接的域，不展开具体节点。
