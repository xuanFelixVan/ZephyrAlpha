...
graph TD
    ...
    class D_GOVERNANCE,D_FEEDBACK_LOOP,D_GOV_REPAIR external_prod
    class D_GOV_OPS_RESILIENCE,D_SHARED,D_AUTONOMY_CORE,D_GOV_CODE_QUALITY,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_GOV_AUDIT,D_AUTONOMY_CORE,D_SECURITY_LLM external_design
```
...
graph TD
    ...
    class D_SHARED,D_INTEGRATION,D_GOVERNANCE,D_AUTONOMY_CORE,D_ORCHESTRATOR external_prod
...
graph TD
    ...
    class D_INFRA_TELEMETRY,D_ORCHESTRATOR,D_GOVERNANCE,D_GOV_SCRIPTS,D_INFRA_RECOVERY external_design
```
...
graph TD
    ...
    class D_FEEDBACK_LOOP,D_GOVERNANCE,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_INTELLIGENCE,D_AUTONOMY_CORE,D_AUTONOMY_PERM,D_TRADING,D_GOVERNANCE external_design
```
...
graph TD
    ...
    class D_INFRA_RECOVERY,D_INTEGRATION,D_SHARED,D_AUTONOMY_CORE,D_GOV_AUDIT,D_INFRA_A2A external_prod
...
graph TD
    ...
    class D_GOV_DRIFT,D_SECURITY,D_INFRA_RECOVERY,D_FEEDBACK_LOOP,D_INFRA_TELEMETRY,D_INFRA_A2A,D_FBL_DETECTORS,D_GOVERNANCE,D_SHARED external_prod
```
...
graph TD
    ...
    class D_SHARED,D_AUTONOMY_CORE,D_INTELLIGENCE,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_SHARED,D_AUTONOMY_CORE,D_GOV_OPS_RESILIENCE,D_INFRA_TELEMETRY,D_GOVERNANCE external_prod
    class D_FEEDBACK_LOOP,D_ORCHESTRATOR,D_INTEGRATION,D_GOV_SCRIPTS external_design
```
...
| 207 | shared.api — auto-generated package init. (__i... | → | D_SHARED 共享服务: api_client.py —— 统一 API Client 基类（Phase ... | config_depends / config_depends |
...
| 214 | 包 shared.knowledge 的初始化文件。 (__init__.py) | → | D_SHARED 共享服务: KE Structurer — 知识条目结构化提取。 (ke_struc... | config_depends / config_depends |
| 215 | __init__.py | → | D_SHARED 共享服务: health.py —— ZephyrAlpha 聚合健康检查 (health.py) | config_depends / config_depends |
| 216 | 包 shared.maintenance 的初始化文件。 (__init__.py) | → | D_SHARED 共享服务: Autonomy Monitor — AI 自主等级监控与降级。 (au... | config_depends / config_depends |
...
| 223 | shared.schema — auto-generated package init. (... | → | D_SHARED 共享服务: schema_registry.py | config_depends / config_depends |
| 224 | shared.security — auto-generated package init.... | → | D_SHARED 共享服务: lock.py —— Re-export wrapper -> canonical: ze... | config_depends / config_depends |
...
| 85 | D_GOV_AUDIT 审计追踪: F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | Finalizer — 优雅清理器 (finalizer.py) | 测试依赖 / test_depends |
| 86 | D_GOV_AUDIT 审计追踪: test_lifecycle_hooks.py | → | hooks.py —— 模块生命周期钩子（Phase 2 新增 | ... | 测试依赖 / test_depends |
| 87 | D_GOV_CODE_QUALITY 代码质量治理: code-dedup-engine CLI——子命令映射+退出码+扫描... | → | AssetDiscoveryScanner — MOD-INF-026 L1 全量文.... | 导入依赖 / import_depends |
| 88 | D_GOV_DOCS 架构文档治理: blueprint.md | → | blueprint.md | runtime / runtime |
| 89 | D_GOV_OPS_RESILIENCE 运维弹性治理: Circuit Breaker — MOD-INF-022 (circuit_breaker.py) | → | Circuit Breaker — 熔断器：连续失败 -> OPEN -> ... | 导入依赖 / import_depends |
| 90 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | AssetClassifier — MOD-INF-026 L2 资产自动分类... | 导入依赖 / import_depends |
| 91 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | AssetDashboard — MOD-INF-026 资产健康仪表盘生... | 导入依赖 / import_depends |
| 92 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | MOD-INF-026 §18 — 资产依赖图。 (dependency.py) | 导入依赖 / import_depends |
| 93 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | UnifiedAssetIndex — MOD-INF-026 L3 统一资产索.... | 导入依赖 / import_depends |
| 94 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | AssetLifecycle — MOD-INF-026 L5 ITIL生命周期自... | 导入依赖 / import_depends |
| 95 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | MOD-INF-026 §24-25 — Git 历史元数据提取 + 多 ... | 导入依赖 / import_depends |
| 96 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | AssetInventoryModels — MOD-INF-026 Pydantic V2... | 导入依赖 / import_depends |
| 97 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | ReconciliationEngine — MOD-INF-026 L4 注册表 v... | 导入依赖 / import_depends |
| 98 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | MOD-INF-026 §17 — 24 个异构注册表统一解析适配... | 导入依赖 / import_depends |
| 99 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | MOD-INF-026 §26 — 三重信任锚验证门 R20。 (tru... | 导入依赖 / import_depends |
| 100 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | startup_shutdown.py | 导入依赖 / import_depends |
| 101 | D_GOV_RULE 规则治理: TaskCompletionGate: scan for residual files out... | → | Task Lifecycle Manager — G0-G7 任务生命周期门... | 导入依赖 / import_depends |
| 102 | D_GOV_SCRIPTS 脚本治理: session_simulator — 30 个模拟开发 session 的蓝... | → | blueprint_metrics — 蓝图使用追踪 instrumentati... | 导入依赖 / import_depends |
| 103 | D_GOV_SCRIPTS 脚本治理: base.py — 审计脚本基类 (base.py) | → | __init__.py | 导入依赖 / import_depends |
| 104 | D_GOV_SCRIPTS 脚本治理: check_registry_consistency — 跨登记表一致性校... | → | __init__.py | 导入依赖 / import_depends |
| 105 | D_GOV_SCRIPTS 脚本治理: finding_state_machine.py — Finding 全生命周期.... | → | Finding Schema — 审计发现标准化数据模型 (findi... | 导入依赖 / import_depends |
| 106 | D_GOV_SCRIPTS 脚本治理: validate_emergency_bypass_log.py — 应急绕过审.... | → | __init__.py | 导入依赖 / import_depends |
| 107 | D_GOV_SCRIPTS 脚本治理: run_all.py — 脚本系统统一入口脚本 (run_all.py) | → | Finding->TaskCard 桥接器 (finding_task_bridge.py) | 导入依赖 / import_depends |
| 108 | D_GOV_SCRIPTS 脚本治理: run_all.py — 脚本系统统一入口脚本 (run_all.py) | → | Finding Schema — 审计发现标准化数据模型 (findi... | 导入依赖 / import_depends |
...
> 本域与 29 个外部域直接连接（出边 309 条 + 入边 243 条 = 552 条）。只显示直接连接的域，不展开具体节点。
