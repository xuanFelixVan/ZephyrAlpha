...
graph TD
    ...
    class D_SHARED,D_FEEDBACK_LOOP,D_GOV_REPAIR external_prod
    class D_GOV_CODE_QUALITY,D_SECURITY,D_GOV_SCRIPTS,D_AUTONOMY_CORE,D_SECURITY_LLM external_design
```
...
graph TD
    ...
    class D_COMPLIANCE,D_GOV_AUDIT,D_AUTONOMY_CORE external_design
```
...
graph TD
    ...
    class D_SHARED,D_INTEGRATION,D_GOVERNANCE,D_ORCHESTRATOR,D_BACKTEST external_prod
...
graph TD
    ...
    class D_SHARED,D_GOV_RULE,D_INTELLIGENCE,D_INTEGRATION external_prod
...
graph TD
    ...
    class D_SHARED,D_GOV_DRIFT,D_INFRA_TELEMETRY,D_GOV_OPS_RESILIENCE,D_INFRA_RECOVERY,D_INTEGRATION external_prod
    class D_ORCHESTRATOR,D_GOVERNANCE,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_SHARED,D_INTEGRATION,D_INFRA_A2A,D_SECURITY external_prod
...
graph TD
    ...
    class D_INTELLIGENCE,D_SHARED,D_INTEGRATION,D_AUTONOMY_CORE,D_FBL_DIAGNOSERS,D_SECURITY,D_GOV_OPS_RESILIENCE,D_GOV_AUDIT,D_GOVERNANCE external_prod
...
graph TD
    ...
    class D_INFRA_RECOVERY,D_FEEDBACK_LOOP,D_SHARED,D_INFRA_A2A,D_GOVERNANCE,D_TRADING external_prod
```
...
graph TD
    ...
    class D_INFRA_A2A,D_FEEDBACK_LOOP,D_INFRA_RECOVERY,D_SHARED,D_INFRA_TELEMETRY,D_FBL_DETECTORS,D_GOV_DRIFT external_prod
```
...
| 177 | CT-PIPE-ORC-001 — TaskCard -> 管线入口节点路由... | → | D_SHARED 共享服务: yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
...
| 188 | Pipeline Routing Plugin System — K8s Schedulin... | → | D_SHARED 共享服务: yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
...
| 206 | 包 shared.adaptation 的初始化文件。 (__init__.py) | → | D_SHARED 共享服务: Prompt Version Manager — 版本化 Prompt 治理。 ... | config_depends / config_depends |
| 207 | 包 shared.blueprint_tools 的初始化文件。 (__ini... | → | D_SHARED 共享服务: ai_understandability_constraint.py | config_depends / config_depends |
| 208 | 包 shared.capacity_governance 的初始化文件。 (_... | → | D_SHARED 共享服务: adaptive_sampler.py | config_depends / config_depends |
| 209 | 包 shared.compensation 的初始化文件。 (__init__... | → | D_SHARED 共享服务: Saga Compensator — 补偿事务：多步操作任一失败 ... | config_depends / config_depends |
| 210 | 包 shared.dependency 的初始化文件。 (__init__.py) | → | D_SHARED 共享服务: Dependency Graph — 任务卡依赖关系管理。 (depen... | config_depends / config_depends |
| 211 | 包 shared.draft 的初始化文件。 (__init__.py) | → | D_SHARED 共享服务: Draft Assistant — 想法 -> MTH-012 蓝图骨架生成... | config_depends / config_depends |
| 212 | __init__.py | → | D_SHARED 共享服务: CT-DLQ-001: DeadLetterQueue -> System Event Bus... | 导入依赖 / import_depends |
| 213 | __init__.py | → | D_SHARED 共享服务: cache.py —— 统一缓存抽象（Phase 8 新增 | 盲点... | 导入依赖 / import_depends |
| 214 | __init__.py | → | D_SHARED 共享服务: ProcessLifecycleGateway — 进程生命周期统一入口... | 导入依赖 / import_depends |
| 215 | shared.io — auto-generated package init. (__in... | → | D_SHARED 共享服务: SHA-256 content fingerprint computation and ver... | config_depends / config_depends |
...
| 225 | shared.security — auto-generated package init.... | → | D_SHARED 共享服务: CBAC 能力检查器 (Capability-Based Access Contro... | config_depends / config_depends |
...
> 本域与 31 个外部域直接连接（出边 310 条 + 入边 245 条 = 555 条）。只显示直接连接的域，不展开具体节点。
