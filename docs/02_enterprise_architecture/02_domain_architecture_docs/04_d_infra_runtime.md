...
graph TD
    ...
    class D_GOV_AUDIT,D_INFRA_RECOVERY,D_INFRA_A2A,D_GOV_OPS_RESILIENCE,D_SHARED,D_GOVERNANCE,D_FBL_DIAGNOSERS,D_INTEGRATION,D_SECURITY,D_AUTONOMY_CORE external_prod
...
| 214 | 包 shared.blueprint_tools 的初始化文件。 (__ini... | → | D_SHARED 共享服务: ai_understandability_constraint.py | config_depends / config_depends |
...
| 224 | 包 shared.maintenance 的初始化文件。 (__init__.py) | → | D_SHARED 共享服务: Dogfooding — 自举测试：用 TaskCard 管理 TaskCa... | config_depends / config_depends |
...
| 231 | shared.schema — auto-generated package init. (... | → | D_SHARED 共享服务: task_types — 任务系统核心类型 re-export 层 (ta... | config_depends / config_depends |
...
| 233 | 包 shared.session 的初始化文件。 (__init__.py) | → | D_SHARED 共享服务: Session Boundary — 会话边界管理。 (session_bou... | config_depends / config_depends |
