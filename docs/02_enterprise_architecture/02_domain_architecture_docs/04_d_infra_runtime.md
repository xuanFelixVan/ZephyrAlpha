...
graph TD
    ...
    class D_GOV_OPS_RESILIENCE,D_GOVERNANCE,D_SECURITY_LLM,D_GOV_SCRIPTS,D_AUTONOMY_CORE,D_SECURITY external_design
```
...
graph TD
    ...
    class D_GOV_AUDIT,D_SECURITY_LLM,D_INTELLIGENCE,D_COMPLIANCE,D_AUTONOMY_CORE external_design
```
...
graph TD
    ...
    class D_INTEGRATION,D_GOVERNANCE,D_AUTONOMY_CORE,D_ORCHESTRATOR external_prod
    class D_SHARED,D_GOV_SCRIPTS,D_INFRASTRUCTURE external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOV_RULE,D_FEEDBACK_LOOP,D_INTEGRATION external_prod
    class D_INTELLIGENCE,D_TRADING,D_SECURITY_LLM external_design
```
...
graph TD
    ...
    class D_GOV_OPS_RESILIENCE,D_INTEGRATION,D_INFRA_RECOVERY,D_GOV_REPAIR external_prod
    class D_SHARED,D_INFRA_TELEMETRY,D_GOV_SCRIPTS,D_ORCHESTRATOR,D_GOVERNANCE external_design
```
...
graph TD
    ...
    class D_INFRA_TELEMETRY,D_FEEDBACK_LOOP,D_INTEGRATION external_prod
...
graph TD
    ...
    D_FEEDBACK_LOOP["(生产态 / production) D_FEEDBACK_LOOP"]
    src_zephyr_trading_lifecycle_manager_py -->|导入依赖 / import_depends| D_FEEDBACK_LOOP
    src_zephyr_trading_boot_hooks_py -->|导入依赖 / import_depends| D_SECURITY
    ...
    class D_SHARED,D_SECURITY,D_GOV_AUDIT,D_FEEDBACK_LOOP,D_TRADING,D_GOV_DRIFT external_prod
    class D_INTELLIGENCE,D_ORCHESTRATOR,D_AUTONOMY_CORE,D_GOVERNANCE external_design
```
...
graph TD
    ...
    class D_GOV_OPS_RESILIENCE,D_INFRA_A2A,D_SHARED,D_INFRA_RECOVERY,D_GOVERNANCE,D_AUTONOMY_CORE,D_SECURITY,D_INTEGRATION,D_GOV_AUDIT external_prod
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_INFRA_A2A,D_INFRA_RECOVERY external_prod
```
...
graph TD
    ...
    class D_SHARED,D_SECURITY,D_INFRA_A2A,D_FBL_DETECTORS,D_GOV_DRIFT,D_FEEDBACK_LOOP,D_INFRA_RECOVERY,D_FBL_DIAGNOSERS,D_INFRA_TELEMETRY,D_GOV_REPAIR external_prod
```
...
graph TD
    ...
    class D_INTEGRATION,D_SHARED,D_GOV_REPAIR,D_FEEDBACK_LOOP external_prod
    class D_INTELLIGENCE,D_TRADING,D_GOV_SCRIPTS,D_AUTONOMY_CORE,D_SECURITY_LLM,D_GOVERNANCE external_design
```
...
graph TD
    ...
    class D_GOVERNANCE,D_SECURITY,D_INFRA_A2A,D_FBL_DETECTORS,D_GOV_DRIFT,D_INFRA_TELEMETRY,D_FEEDBACK_LOOP external_prod
    class D_SHARED,D_GOV_OPS_RESILIENCE,D_ORCHESTRATOR,D_GOV_SCRIPTS,D_INTEGRATION external_design
```
...
| 205 | 包 shared.capacity_governance 的初始化文件。 (_... | → | D_SHARED 共享服务: capacity_calibrator.py | config_depends / config_depends |
...
| 212 | shared.io — auto-generated package init. (__in... | → | D_SHARED 共享服务: SHA-256 content fingerprint computation and ver... | config_depends / config_depends |
| 213 | 包 shared.knowledge 的初始化文件。 (__init__.py) | → | D_SHARED 共享服务: KE Structurer — 知识条目结构化提取。 (ke_struc... | config_depends / config_depends |
| 214 | 包 shared.maintenance 的初始化文件。 (__init__.py) | → | D_SHARED 共享服务: code_economy_analyzer.py | config_depends / config_depends |
...
| 222 | shared.security — auto-generated package init.... | → | D_SHARED 共享服务: lock.py —— Re-export wrapper -> canonical: ze... | config_depends / config_depends |
| 223 | 包 shared.session 的初始化文件。 (__init__.py) | → | D_SHARED 共享服务: Session Boundary — 会话边界管理。 (session_bou... | config_depends / config_depends |
...
| 130 | D_INTEGRATION 管线路由: __init__.py | → | A2A Layer3 Coordination — shared Protocol inte... | 导入依赖 / import_depends |
| 131 | D_INTEGRATION 管线路由: LocalModelScheduler — L2 本地模型 24/7 调度循... | → | resource_optimization_engine.py | 导入依赖 / import_depends |
...
> 本域与 29 个外部域直接连接（出边 307 条 + 入边 244 条 = 551 条）。只显示直接连接的域，不展开具体节点。
