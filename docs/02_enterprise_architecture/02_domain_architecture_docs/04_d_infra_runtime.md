...
graph TD
    ...
    class D_GOV_OPS_RESILIENCE,D_SECURITY_LLM,D_GOV_SCRIPTS,D_AUTONOMY_CORE,D_GOV_CODE_QUALITY external_design
```
...
graph TD
    ...
    class D_GOV_AUDIT,D_SECURITY_LLM,D_INTELLIGENCE,D_COMPLIANCE external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_AUTONOMY_CORE,D_ORCHESTRATOR external_prod
...
graph TD
    ...
    class D_SECURITY,D_TRADING,D_GOV_AUDIT,D_INTEGRATION,D_AUTONOMY_CORE external_design
```
...
graph TD
    ...
    class D_SECURITY_LLM,D_AUTONOMY_CORE,D_TRADING,D_INTELLIGENCE external_design
```
...
graph TD
    ...
    class D_GOVERNANCE,D_ORCHESTRATOR,D_INTEGRATION,D_GOV_SCRIPTS external_design
```
...
| 207 | shared.api — auto-generated package init. (__i... | → | D_SHARED 共享服务: dos_launcher.py | config_depends / config_depends |
| 208 | 包 shared.blueprint_tools 的初始化文件。 (__ini... | → | D_SHARED 共享服务: ai_understandability_constraint.py | config_depends / config_depends |
| 209 | 包 shared.capacity_governance 的初始化文件。 (_... | → | D_SHARED 共享服务: capacity_digital_twin.py | config_depends / config_depends |
...
| 215 | 包 shared.knowledge 的初始化文件。 (__init__.py) | → | D_SHARED 共享服务: KE Structurer — 知识条目结构化提取。 (ke_struc... | config_depends / config_depends |
...
| 222 | 包 shared.reliability 的初始化文件。 (__init__.py) | → | D_SHARED 共享服务: Retry Handler — 指数退避重试 + 可恢复/不可恢复... | config_depends / config_depends |
...
> 本域与 30 个外部域直接连接（出边 309 条 + 入边 246 条 = 555 条）。只显示直接连接的域，不展开具体节点。
