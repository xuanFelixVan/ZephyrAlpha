...
flowchart TD
    ...
    class D_INFRA_RUNTIME,D_SHARED,D_AUTONOMY_CORE,D_OPS,D_TRADING,D_GOV_SCRIPTS,D_GOVERNANCE,D_GOV_OPS_RESILIENCE,D_GOV_DRIFT,D_FEEDBACK_LOOP,D_GOV_ENFORCEMENT,D_INTELLIGENCE external_prod
```
...
| 19 | Pipeline Orchestrator (integration/pipeline_orchestrator.py) | → | D_INFRA_RUNTIME 运行时集成: B169 永久失败任务存储 / Dead Letter Queue (pipeline/dead_... | 导入依赖 / import_depends |
...
| 27 | Pipeline Orchestrator (integration/pipeline_orchestrator.py) | → | D_INTELLIGENCE 上下文管理: 重排器 / Reranker (model_evaluation/reranker.py) | 导入依赖 / import_depends |
...
| 33 | Ollama Chat (local_model/ollama_chat.py) | → | D_OPS 反馈循环: 5.133.2 DI 注入契约 / Budget Engine (ops_governance/budge... | 导入依赖 / import_depends |
| 34 | Pipeline Orchestrator (integration/pipeline_orchestrator.py) | → | D_OPS 反馈循环: 5.133.2 DI 注入契约 / Budget Engine (ops_governance/budge... | 导入依赖 / import_depends |
...
| 40 | 封装 /api/embed，兼容 SentenceTransformer.encode / Ollama... | → | D_SHARED 共享服务: Constants (foundation/constants.py) | 导入依赖 / import_depends |
...
| 56 | Task Manager Server (mcp/task_manager_server.py) | → | D_SHARED 共享服务: 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 67 | Pipeline Orchestrator (integration/pipeline_orchestrator.py) | → | D_SHARED 共享服务: 观察者 / Observer (infra/observer.py) | 导入依赖 / import_depends |
...
| 80 | Dlq Bridge (events/dlq_bridge.py) | → | D_SHARED 共享服务: 观察者 / Observer (infra/observer.py) | 导入依赖 / import_depends |
...
| 84 | Chunk Strategy Router (vector_memory/chunk_strategy_route... | → | D_SHARED 共享服务: 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 86 | Collection Manager (vector_memory/collection_manager.py) | → | D_SHARED 共享服务: 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 88 | Collection Schemas (vector_memory/collection_schemas.py) | → | D_SHARED 共享服务: 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 91 | Index Health Monitor (vector_memory/index_health_monitor.py) | → | D_SHARED 共享服务: 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 13 | D_GOVERNANCE 生命周期管理: Test Evals Unit (code_quality/test_evals_unit.py) | → | 评估 / Evals (evaluation/evals.py) | 测试依赖 / test_depends |
...
| 42 | D_INFRA_RUNTIME 运行时集成: Auto Runtime Core (trading/auto_runtime_core.py) | → | DI 注入契约 / Embedding Router (local_model/embedding_rou... | 导入依赖 / import_depends |
...
| 47 | D_INFRA_RUNTIME 运行时集成: 扫描项目 -> 生成推理任务 -> 送入调度器 / Auto Task Genera... | → | Local Model Scheduler (local_model/local_model_scheduler.py) | 导入依赖 / import_depends |
| 48 | D_INFRA_RUNTIME 运行时集成: 必填字段/类型/范围，失败 fail-fast / Runtime Config (trad... | → | Runtime Types (contracts/runtime_types.py) | 导入依赖 / import_depends |
