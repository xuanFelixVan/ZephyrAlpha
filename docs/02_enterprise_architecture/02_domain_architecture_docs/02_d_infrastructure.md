...
| 25 | D_EX_SOR 执行路由: 连接失败、断线、状态机非法跳转 / Broker Api Connector (ap... | → | Order (contracts/order.py) | 导入依赖 / import_depends |
...
| 31 | D_FACTOR 因子: FactorSignal 批量缓冲写入器 / Buffer (batch_output/buffer... | → | Factor Signal (contracts/factor_signal.py) | 导入依赖 / import_depends |
...
| 60 | D_MKT_DATA 行情数据: D_MKT_DATA→D_FACTOR 数据供给 / Producer (normalized_mark... | → | Market Data (contracts/market_data.py) | 导入依赖 / import_depends |
...
| 68 | D_PF_CORE 组合核心: 组合优化方法 / Portfolio Optimizer (core/portfolio_optimi... | → | Target Portfolio (contracts/target_portfolio.py) | contract / contract |
| 69 | D_PF_CORE 组合核心: 组合优化方法 / Portfolio Optimizer (core/portfolio_optimi... | → | Target Portfolio (contracts/target_portfolio.py) | 导入依赖 / import_depends |
