...
graph TD
    ...
    src_zephyr_shared_contracts_position_py -->|导入依赖 / import_depends| D_CONTRACTS
    src_zephyr_shared_contracts_risk_limits_py -->|导入依赖 / import_depends| D_CONTRACTS
    src_zephyr_shared_contracts_experiment_result_py -->|导入依赖 / import_depends| D_CONTRACTS
    src_zephyr_shared_contracts_market_data_py -->|导入依赖 / import_depends| D_CONTRACTS
    D_GOV_AUDIT["(生产态 / production) D_GOV_AUDIT"]
    scripts_backup_backup_reconciler_py -->|导入依赖 / import_depends| D_GOV_AUDIT
    src_zephyr_shared_contracts_synthesized_signal_py -->|导入依赖 / import_depends| D_CONTRACTS
    src_zephyr_shared_contracts_factor_signal_py -->|导入依赖 / import_depends| D_CONTRACTS
    src_zephyr_shared_contracts_fill_py -->|导入依赖 / import_depends| D_CONTRACTS
    src_zephyr_shared_contracts_order_py -->|导入依赖 / import_depends| D_CONTRACTS
    src_zephyr_shared_contracts_order_py -->|导入依赖 / import_depends| D_CONTRACTS
    D_MKT_DATA["(生产态 / production) D_MKT_DATA"]
    D_MKT_DATA -->|导入依赖 / import_depends| src_zephyr_shared_contracts_market_data_py
    D_TRADING["(生产态 / production) D_TRADING"]
    D_TRADING -->|导入依赖 / import_depends| src_zephyr_shared_contracts_telemetry_emitter_py
    ...
    D_CONTRACTS -->|导入依赖 / import_depends| src_zephyr_shared_contracts_performance_attribution_report_py
    ...
    D_GOV_SCRIPTS -->|导入依赖 / import_depends| src_zephyr_infrastructure_config_init_py
    ...
    class D_CONTRACTS,D_GOV_AUDIT,D_MKT_DATA,D_TRADING,D_SIGQC,D_FUNDAMENTAL_SIGNAL,D_GOV_CODE_QUALITY,D_GOV_SCRIPTS external_prod
```
...
|:--:|---------|:--:|---------|---------|
| 1 | experiment_result.py | → | D_CONTRACTS 共享契约: trace_context.py | 导入依赖 / import_depends |
| 2 | factor_signal.py | → | D_CONTRACTS 共享契约: trace_context.py | 导入依赖 / import_depends |
| 3 | fill.py | → | D_CONTRACTS 共享契约: trace_context.py | 导入依赖 / import_depends |
| 4 | market_data.py | → | D_CONTRACTS 共享契约: trace_context.py | 导入依赖 / import_depends |
| 5 | order.py | → | D_CONTRACTS 共享契约: trace_context.py | 导入依赖 / import_depends |
| 6 | order.py | → | D_CONTRACTS 共享契约: OrderSide/OrderStatus/OrderType — 交易枚举真源... | 导入依赖 / import_depends |
| 7 | position.py | → | D_CONTRACTS 共享契约: trace_context.py | 导入依赖 / import_depends |
| 8 | risk_limits.py | → | D_CONTRACTS 共享契约: trace_context.py | 导入依赖 / import_depends |
| 9 | synthesized_signal.py | → | D_CONTRACTS 共享契约: trace_context.py | 导入依赖 / import_depends |
| 10 | backup_reconciler.py — 灾备备份系统事件触发器.... | → | D_GOV_AUDIT 审计追踪: reconciliation_registry.py — GitCommitGateway ... | 导入依赖 / import_depends |

### 依赖本域的其他域（入边）/ Depended By

| # | 外部域-源模块 / Source Module | → | 本域模块 / Target Module | 依赖类型 / Type |
|:--:|---------|:--:|---------|---------|
| 1 | D_CONTRACTS 共享契约: Re-export shim — 真源已收敛至 zephyr.shared.co... | → | performance_attribution_report.py | 导入依赖 / import_depends |
| 2 | D_EX_CORE 执行核心: D_EXECUTION_CORE — Execution Engine (execution... | → | order.py | 导入依赖 / import_depends |
| 3 | D_EX_CORE 执行核心: D_EXECUTION_CORE — Execution Engine (execution... | → | risk_limits.py | 导入依赖 / import_depends |
| 4 | D_EX_CORE 执行核心: D_EXECUTION_CORE — Order Manager (order_manage... | → | fill.py | 导入依赖 / import_depends |
| 5 | D_EX_CORE 执行核心: D_EXECUTION_CORE — Order Manager (order_manage... | → | order.py | 导入依赖 / import_depends |
| 6 | D_FACTOR 因子:  | → | market_data.py | 导入依赖 / import_depends |
| 7 | D_FACTOR 因子: CTR-001 NormalizedMarketData 消费者——数据适配... | → | market_data.py | 导入依赖 / import_depends |
| 8 | D_FACTOR 因子: CTR-002 FactorSignal 生产者——信号适配层。 (co... | → | factor_signal.py | 导入依赖 / import_depends |
...
| 13 | D_FUNDAMENTAL_SIGNAL 基本面信号: AlphaSignalPipeline D_FACTOR->D_SIGNAL跨层集成... | → | factor_signal.py | 导入依赖 / import_depends |
| 14 | D_FUNDAMENTAL_SIGNAL 基本面信号: AlphaSignalPipeline D_FACTOR->D_SIGNAL跨层集成... | → | synthesized_signal.py | 导入依赖 / import_depends |
...
| 24 | D_GOV_SCRIPTS 脚本治理: A2A Protocol 全链路满分验证脚本 (a2a_full_verif... | → | ZephyrAlpha — 基础设施 Infrastructure Layer —... | 导入依赖 / import_depends |
| 25 | D_GOV_SCRIPTS 脚本治理: local_layer_daemon.py — L2 本地模型层守护进程.... | → | ZephyrAlpha — 基础设施 Infrastructure Layer —... | 导入依赖 / import_depends |
| 26 | D_MKT_DATA 行情数据: __init__.py | → | market_data.py | 导入依赖 / import_depends |
| 27 | D_MKT_DATA 行情数据:  | → | market_data.py | 导入依赖 / import_depends |
| 28 | D_PF_ALLOC 组合分配: strategy_lifecycle_event.py | → | strategy_lifecycle_event.py | 导入依赖 / import_depends |
...
| 42 | D_TRADING 交易运营: HealthMonitor — 健康监控 + 自愈 (health_monito... | → | telemetry_emitter.py | 导入依赖 / import_depends |
| 43 | D_TRADING 交易运营: D_EXECUTION_CORE — BrokerInterface (broker_int... | → | fill.py | 导入依赖 / import_depends |
| 44 | D_TRADING 交易运营: D_EXECUTION_CORE — BrokerInterface (broker_int... | → | order.py | 导入依赖 / import_depends |
| 45 | D_TRADING 交易运营: D_EXECUTION_CORE — BrokerInterface (broker_int... | → | position.py | 导入依赖 / import_depends |
| 46 | D_TRADING 交易运营: execution_rejection_error.py | → | trace_context.py | 导入依赖 / import_depends |
| 47 | D_TRADING 交易运营: Re-export wrapper: ExecutionReport 真源在 zephy... | → | execution_report.py | 导入依赖 / import_depends |
| 48 | D_TRADING 交易运营: Re-export wrapper: Fill 真源在 zephyr.shared.co... | → | fill.py | 导入依赖 / import_depends |
| 49 | D_TRADING 交易运营: Re-export wrapper: Order 真源在 zephyr.shared.c... | → | order.py | 导入依赖 / import_depends |
| 50 | D_TRADING 交易运营: Re-export wrapper: PositionSnapshot 真源在 zeph... | → | position.py | 导入依赖 / import_depends |
| 51 | D_TRADING 交易运营: trading-contracts/factories.py — 交易域数据契.... | → | factor_signal.py | 导入依赖 / import_depends |
| 52 | D_TRADING 交易运营: trading-contracts/factories.py — 交易域数据契.... | → | synthesized_signal.py | 导入依赖 / import_depends |
| 53 | D_TRADING 交易运营: signal_degradation_warning.py | → | trace_context.py | 导入依赖 / import_depends |
| 54 | D_TRADING 交易运营: Re-export shim — 真源已收敛至 zephyr.shared.co... | → | performance_attribution_report.py | 导入依赖 / import_depends |
| 55 | D_TRADING 交易运营: strategy_lifecycle_event.py | → | strategy_lifecycle_event.py | 导入依赖 / import_depends |
| 56 | D_TRADING 交易运营: risk_limit_violation_error.py | → | trace_context.py | 导入依赖 / import_depends |
| 57 | D_TRADING 交易运营: risk_limits.py | → | trace_context.py | 导入依赖 / import_depends |
| 58 | D_TRADING 交易运营: risk_validator_protocol.py | → | risk_limits.py | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 16 个外部域直接连接（出边 10 条 + 入边 58 条 = 68 条）。只显示直接连接的域，不展开具体节点。
