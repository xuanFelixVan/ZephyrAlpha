...
> 按 architecture_layer 分组的模块清单（共 359 个模块 / 359 modules）。
...
### L2 领域层 / Domain Layer (356 modules)
...
> 展示全部 359 个模块（生产态 10 + 设计态 2 + 原型态 347），标签标注成熟度。
...
graph TD
    ...
    scripts_governance_archive_prototype_adversarial_log_py -.->|config_depends / config_depends| scripts_governance_archive_prototype_generate_asset_index_py
    ...
    class D_SHARED,D_GOVERNANCE,D_INTELLIGENCE,D_INFRA_RUNTIME,D_GOV_AUDIT external_prod
```
...
graph TD
    ...
    class D_SECURITY,D_SHARED,D_DATA,D_GOVERNANCE external_prod
```
...
graph TD
    subgraph D_GOV_SCRIPTS["D_GOV_SCRIPTS 脚本治理"]
        ...
        scripts_governance_git_health_smoke_py["(原型态 / prototype) git_health_smoke.py — Git 健康度 smoke test（A...<br/>文件: git_health_smoke.py"]
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 347 个，25 条域内依赖）。
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_SECURITY,D_GOV_RULE,D_ORCHESTRATOR external_prod
    class D_GOV_AUDIT external_design
```
...
| 4 | D_GOV_AUDIT 审计追踪: red_blue_test.py | → | concurrent_commit_test.py — 幽灵提交红蓝对抗脚... | config_depends / config_depends |
| 5 | D_GOV_DRIFT 漂移检测: validate_ssot.py | → | 文件头部格式解析 SSoT（Single Source of Truth）... | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 13 个外部域直接连接（出边 91 条 + 入边 5 条 = 96 条）。只显示直接连接的域，不展开具体节点。
