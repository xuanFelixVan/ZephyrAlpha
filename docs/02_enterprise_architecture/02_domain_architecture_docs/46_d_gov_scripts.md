...
graph TD
    ...
    class D_GOV_AUDIT,D_INFRA_RUNTIME,D_SHARED,D_INTELLIGENCE external_prod
...
graph TD
    ...
    class D_GOVERNANCE,D_ORCHESTRATOR,D_GOV_RULE,D_SHARED external_prod
```
...
graph TD
    subgraph D_GOV_SCRIPTS["D_GOV_SCRIPTS 脚本治理"]
        ...
        scripts_governance_migrate_sqlite_to_pg_migrate_data_py["(原型态 / prototype) SQLite → PostgreSQL 运营数据迁移脚本<br/>文件: migrate_data.py"]
    ...
    class D_SHARED,D_GOVERNANCE,D_INFRA_RUNTIME,D_GOV_ENFORCEMENT external_prod
```
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 346 个，27 条域内依赖）。
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_AUDIT,D_GOV_OPS_RESILIENCE,D_INFRA_RUNTIME,D_SECURITY external_prod
    class D_DATA external_design
```
...
| 90 | SQLite → PostgreSQL 运营数据迁移脚本 (migrate_... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
