...
> 展示全部 356 个模块（生产态 10 + 设计态 2 + 原型态 344），标签标注成熟度。
...
graph TD
    ...
    class D_SHARED,D_INTELLIGENCE,D_GOV_AUDIT,D_INFRA_RUNTIME external_prod
...
graph TD
    ...
    class D_GOV_RULE,D_GOVERNANCE,D_SHARED external_prod
```
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_SECURITY,D_DATA external_prod
```
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 344 个，26 条域内依赖）。
