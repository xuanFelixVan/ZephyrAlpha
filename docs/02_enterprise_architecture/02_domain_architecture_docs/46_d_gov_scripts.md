...
graph TD
    ...
    class D_GOV_REPAIR,D_GOVERNANCE,D_GOV_AUDIT,D_SHARED,D_GOV_DRIFT external_prod
...
graph TD
    ...
    class D_SECURITY,D_GOV_ENFORCEMENT,D_DATA external_prod
...
graph TD
    ...
    class D_SHARED,D_GOV_REPAIR,D_INFRA_RUNTIME external_prod
    class D_GOVERNANCE external_design
```
...
graph TD
    ...
    class D_GOV_RULE,D_GOVERNANCE,D_SHARED,D_ORCHESTRATOR,D_SECURITY,D_AUTONOMY_CORE external_prod
```
...
> 本域与 20 个外部域直接连接（出边 393 条 + 入边 11 条 = 404 条）。只显示直接连接的域，不展开具体节点。
