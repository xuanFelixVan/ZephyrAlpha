...
graph TD
    ...
    class D_INFRA_RUNTIME,D_GOV_REPAIR,D_GOV_AUDIT,D_GOVERNANCE,D_GOV_DRIFT external_prod
...
graph TD
    ...
    class D_GOV_RULE,D_GOV_REPAIR,D_GOVERNANCE,D_ORCHESTRATOR external_prod
```
...
graph TD
    ...
    class D_SECURITY,D_SHARED,D_GOV_ENFORCEMENT external_prod
    class D_GOVERNANCE external_design
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_SHARED,D_GOVERNANCE,D_GOV_RULE external_prod
```
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_REPAIR,D_GOV_ENFORCEMENT external_prod
```
...
graph TD
    ...
    class D_INFRA_RECOVERY,D_SHARED,D_GOV_RULE,D_FEEDBACK_LOOP external_prod
```
...
graph TD
    ...
    tests_trae_rules_test_g_trae_038_py -.->|测试依赖 / test_depends| D_GOV_RULE
    tests_trae_rules_test_g_trae_039_py -.->|测试依赖 / test_depends| D_SHARED
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 438 个，148 条域内依赖）。
...
> 本域与 20 个外部域直接连接（出边 391 条 + 入边 9 条 = 400 条）。只显示直接连接的域，不展开具体节点。
