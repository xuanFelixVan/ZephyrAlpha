...
> 展示全部 454 个模块（生产态 13 + 设计态 2 + 原型态 439），标签标注成熟度。
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_GOV_REPAIR,D_GOV_AUDIT,D_GOVERNANCE,D_SHARED,D_GOV_DRIFT external_prod
```
...
graph TD
    ...
    class D_DATA,D_GOVERNANCE,D_SHARED external_prod
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_GOV_CODE_QUALITY,D_GOV_REPAIR,D_SHARED external_prod
    class D_GOVERNANCE external_design
```
...
graph TD
    ...
    class D_GOV_RULE,D_SHARED,D_GOV_DRIFT,D_GOVERNANCE,D_GOV_OPS_RESILIENCE,D_INFRA_RECOVERY external_prod
```
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 439 个，147 条域内依赖）。
