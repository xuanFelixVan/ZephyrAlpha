...
> 按 architecture_layer 分组的模块清单（共 455 个模块 / 455 modules）。
...
### L2 领域层 / Domain Layer (454 modules)
...
| 67 | scripts/governance/apply_dataflowgraph.py | apply_dataflowgraph.py — dataflowgraph 变更写... | 原型态 / prototype |  |
| 68 | scripts/governance/apply_decisiongraph.py | [INVARIANTS] pg_advisory_lock 写锁; build_statu... | 原型态 / prototype |  |
| 69 | scripts/governance/apply_depgraph.py | [INVARIANTS] 原子写入（RULE-ONE）；变更前验证；... | 原型态 / prototype |  |
...
| 72 | scripts/governance/batch_fix_5135.py | batch_fix_5135.py — 批量为5.135异常粒度过粗违... | 原型态 / prototype | [MOD-INF-005](../../03_modules/_domain_governance/governance_automation/blueprint.md) |
...
| 187 | scripts/governance/d5_architecture/generators/generate_bl... | G-panorama-gen: 蓝图 §0.6 四图对齐视图生成器（... | 原型态 / prototype |  |
...
> 展示全部 455 个模块（生产态 13 + 设计态 2 + 原型态 440），标签标注成熟度。
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_GOV_REPAIR,D_SHARED,D_GOV_AUDIT,D_GOVERNANCE,D_GOV_DRIFT external_prod
```
...
graph TD
    subgraph D_GOV_SCRIPTS["D_GOV_SCRIPTS 脚本治理"]
        ...
        scripts_governance_batch_fix_5135_py["(原型态 / prototype) batch_fix_5135.py — 批量为5.135异常粒度过粗违...<br/>文件: batch_fix_5135.py"]
...
graph TD
    ...
    class D_GOV_RULE,D_ORCHESTRATOR,D_GOV_REPAIR,D_GOVERNANCE external_prod
```
...
graph TD
    subgraph D_GOV_SCRIPTS["D_GOV_SCRIPTS 脚本治理"]
        ...
        scripts_governance_d5_architecture_generators_generate_blueprint_panorama_py["(原型态 / prototype) G-panorama-gen: 蓝图 §0.6 四图对齐视图生成器（...<br/>文件: generate_blueprint_panorama.py"]
    ...
    scripts_governance_d5_architecture_validators_blueprint_validate_blueprint_path_consistency_py -.->|config_depends / config_depends| D_GOVERNANCE
    ...
    class D_SHARED,D_GOV_ENFORCEMENT,D_DATA external_prod
    class D_GOVERNANCE external_design
```
...
graph TD
    subgraph D_GOV_SCRIPTS["D_GOV_SCRIPTS 脚本治理"]
        ...
        scripts_governance_d5_architecture_validators_validate_static_manifest_drift_py["(原型态 / prototype) validate_static_manifest_drift.py — GATE-21 静...<br/>文件: validate_static_manifest_drift.py"]
    ...
    scripts_governance_d5_architecture_validators_validate_load_path_integrity_py -.->|config_depends / config_depends| D_GOVERNANCE
    ...
    scripts_governance_d5_architecture_validators_validate_handoff_package_py -.->|config_depends / config_depends| D_GOVERNANCE
...
graph TD
    subgraph D_GOV_SCRIPTS["D_GOV_SCRIPTS 脚本治理"]
        ...
        scripts_governance_scan_exception_debt_py["(原型态 / prototype) scan_exception_debt.py — 一次性扫描5.135异常粒...<br/>文件: scan_exception_debt.py"]
...
graph TD
    ...
    classDef external_prod fill:#e8f5e9,stroke:#1b5e20,stroke-width:1px,color:#000
    classDef external_design fill:#fce4ec,stroke:#880e4f,stroke-width:1px,color:#000,stroke-dasharray: 5 5
    class tests_trae_rules_test_g_trae_051_py,tests_trae_rules_test_g_trae_052_py,tests_trae_rules_test_g_trae_053_py,tests_trae_rules_test_g_trae_054_py,tests_trae_rules_test_g_trae_055_py design
    class D_GOV_RULE,D_SHARED external_prod
```
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 440 个，147 条域内依赖）。
...
graph TD
    ...
    scripts_governance_scan_exception_debt_py -.->|导入依赖 / import_depends| D_GOV_CODE_QUALITY
    D_INFRA_RUNTIME["(生产态 / production) D_INFRA_RUNTIME"]
    scripts_governance_run_all_py -.->|导入依赖 / import_depends| D_INFRA_RUNTIME
    D_GOVERNANCE -.->|config_depends / config_depends| scripts_governance_d7_code_init_py
    ...
    class D_SHARED,D_GOV_CODE_QUALITY,D_INFRA_RUNTIME external_prod
    class D_GOVERNANCE,D_GOV_RULE external_design
```
...
| 71 | G-panorama-gen: 蓝图 §0.6 四图对齐视图生成器（... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 (depgraph_... | 导入依赖 / import_depends |
| 72 | G-panorama-gen: 蓝图 §0.6 四图对齐视图生成器（... | → | D_GOVERNANCE 生命周期管理: dataflowgraph Schema DDL + 连接入口 (dataflowgr... | 导入依赖 / import_depends |
| 73 | G-panorama-gen: 蓝图 §0.6 四图对齐视图生成器（... | → | D_GOVERNANCE 生命周期管理: decisiongraph Schema DDL + 不变量声明 (decision... | 导入依赖 / import_depends |
| 74 | G-acqflow: 从 tasks.yaml 生成业务数据采集流图 M... | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
| 75 | G-decision: 从 decisiongraph (PostgreSQL) 生成.... | → | D_GOVERNANCE 生命周期管理: decisiongraph Schema DDL + 不变量声明 (decision... | 导入依赖 / import_depends |
| 76 | #183: 从 data_sources_registry.yaml 派生 polici... | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
| 77 | panorama_common.py — 四图投票共享工具（ARCH-05... | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
| 78 | 安全删除门禁脚本——RULE-THREE 强制执行器。 (pr... | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
| 79 | 对标 HDEBT-01：rationale-log.md 体积 >150KB / .... | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
| 80 | blueprint_frontmatter_reconciler.py — 蓝图 fro... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 (depgraph_... | 导入依赖 / import_depends |
| 81 | Strategy: (merge_readme_to_index.py) | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
| 82 | 对标：AGENTS.md §6.1 蓝图-代码同步强制约定 (sy... | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
| 83 | sync_registry_from_blueprints.py -- 从 blueprin... | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
| 84 | AGENTS.md §6.1 蓝图-代码同步强制约定的 CI 门禁... | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
| 85 | AGENTS.md 6.4 铁律五 + 铁律六：construction_pro... | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
| 86 | Module docstring — see module-level docstring ... | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
| 87 | 蓝图物理位置与归属链完整性校验器 (Blueprint Pla... | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
| 88 | GATE-TAG-UNIQUE - Blueprint tag uniqueness vali... | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
| 89 | validate_lifecycle_refs.py — 生命周期引用约束.... | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
| 90 | Module docstring — see module-level docstring ... | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
...
| 111 | validate_static_manifest_drift.py — GATE-21 静... | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
...
| 177 | scan_exception_debt.py — 一次性扫描5.135异常粒... | → | D_GOV_CODE_QUALITY 代码质量治理: msg_exposure_gate.py — 错误消息暴露敏感信息阻.... | 导入依赖 / import_depends |
| 178 | test_git_bisector.py | → | D_GOV_DRIFT 漂移检测: Git Bisector — git_bisector.py (git_bisector.py) | 测试依赖 / test_depends |
...
|:--:|---------|:--:|---------|---------|
| 1 | D_GOVERNANCE 生命周期管理: __init__.py | → | validate_session_log_updated.py — Session Log ... | config_depends / config_depends |
| 2 | D_GOVERNANCE 生命周期管理: check_pure_shim.py — GATE-NO-PURE-SHIM 检测器.... | → | D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
...
| 5 | D_GOVERNANCE 生命周期管理: test_generate_gate_registry.py — generate_gate... | → | generate_gate_registry.py — 门禁登记表自动生成... | 测试依赖 / test_depends |
| 6 | D_GOVERNANCE 生命周期管理: test_ssot_gate — SSoT 创建门禁红蓝变异测试。 (... | → | __init__.py | 测试依赖 / test_depends |
| 7 | D_GOVERNANCE 生命周期管理: test_blueprint_frontmatter_reconciler.py — 蓝.... | → | blueprint_frontmatter_reconciler.py — 蓝图 fro... | 测试依赖 / test_depends |
| 8 | D_GOVERNANCE 生命周期管理: test_sync_panorama_module.py — 四图模块同步引.... | → | sync_panorama_module.py — 四图模块同步引擎（AR... | 测试依赖 / test_depends |
| 9 | D_GOV_CODE_QUALITY 代码质量治理: check_module_id_consistency.py — module_id 全.... | → | D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 10 | D_GOV_DRIFT 漂移检测: validate_ssot.py | → | 文件头部格式解析 SSoT（Single Source of Truth）... | 导入依赖 / import_depends |
| 11 | D_GOV_RULE 规则治理: generate_script_manifest.py — 脚本清单自动生成... | → | __init__.py | config_depends / config_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 20 个外部域直接连接（出边 394 条 + 入边 11 条 = 405 条）。只显示直接连接的域，不展开具体节点。
