...
graph TD
    ...
    class D_GOV_REPAIR,D_INFRA_RUNTIME,D_GOV_AUDIT,D_GOVERNANCE,D_GOV_DRIFT external_prod
...
graph TD
    ...
    class D_GOV_RULE,D_FEEDBACK_LOOP,D_SHARED external_prod
```
...
> 本域与 20 个外部域直接连接（出边 391 条 + 入边 10 条 = 401 条）。只显示直接连接的域，不展开具体节点。
