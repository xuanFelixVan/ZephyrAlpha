...
| 原型态模块 | 437 | Prototype Modules | 437 |
...
| 116 | scripts/governance/d1_structure/generate_missing_index_md.py | generate_missing_index_md.py | 原型态 / prototype |  |
...
| 130 | scripts/governance/d3_metadata/backfill_doctype_metadata.py | backfill_doctype_metadata.py | 原型态 / prototype |  |
| 131 | scripts/governance/d3_metadata/backfill_ttl_metadata.py | backfill_ttl_metadata.py | 原型态 / prototype |  |
...
| 133 | scripts/governance/d3_metadata/check_frontmatter_metadata.py | check_frontmatter_metadata.py | 生产态 / production | [MOD-INF-005](../../03_modules/_domain_governance/governance_automation/blueprint.md) |
...
| 139 | scripts/governance/d3_metadata/classify_ttl_by_content.py | classify_ttl_by_content.py | 原型态 / prototype |  |
...
| 142 | scripts/governance/d3_metadata/generate_rule_catalog.py | generate_rule_catalog.py | 原型态 / prototype |  |
| 143 | scripts/governance/d3_metadata/migrate_illegal_doctype.py | migrate_illegal_doctype.py | 原型态 / prototype |  |
| 144 | scripts/governance/d3_metadata/validate_architecture.py | validate_architecture.py | 原型态 / prototype |  |
...
| 199 | scripts/governance/d5_architecture/syncers/merge_readme_t... | merge_readme_to_index.py | 原型态 / prototype |  |
...
> 展示全部 452 个模块（生产态 13 + 设计态 2 + 原型态 437），标签标注成熟度。
...
graph TD
    ...
    class D_SHARED,D_GOV_AUDIT,D_INTELLIGENCE,D_GOVERNANCE external_prod
```
...
graph TD
    ...
    class D_GOV_REPAIR,D_INFRA_RUNTIME,D_GOVERNANCE,D_GOV_AUDIT,D_SHARED external_prod
```
...
graph TD
    ...
    D_GOVERNANCE -.->|config_depends / config_depends| scripts_governance_d5_architecture_validators_yaml_md_validate_yaml_interface_uniqueness_py
    classDef production fill:#e1f5fe,stroke:#01579b,stroke-width:2px,color:#000
    classDef design fill:#fff3e0,stroke:#e65100,stroke-width:2px,color:#000,stroke-dasharray: 5 5
    classDef external_prod fill:#e8f5e9,stroke:#1b5e20,stroke-width:1px,color:#000
    classDef external_design fill:#fce4ec,stroke:#880e4f,stroke-width:1px,color:#000,stroke-dasharray: 5 5
...
graph TD
    ...
    scripts_governance_d9_knowledge_init_py -.->|config_depends / config_depends| scripts_governance_d9_knowledge_detect_duplicated_normative_language_py
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_GOV_REPAIR,D_SHARED external_prod
...
graph TD
    ...
    class D_SHARED,D_GOV_OPS_RESILIENCE,D_INFRA_RUNTIME,D_ORCHESTRATOR,D_FBL_DETECTORS,D_INFRA_RECOVERY,D_GOV_RULE,D_AUTONOMY_CORE external_prod
```
...
graph TD
    ...
    D_SHARED["(生产态 / production) D_SHARED"]
    tests_trae_rules_test_g_trae_037_py -.->|测试依赖 / test_depends| D_SHARED
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 437 个，148 条域内依赖）。
...
graph TD
    ...
    class D_GOV_CODE_QUALITY,D_GOV_RULE,D_GOV_AUDIT external_design
```
...
| 28 | backfill_doctype_metadata.py | → | D_GOVERNANCE 生命周期管理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
| 29 | backfill_ttl_metadata.py | → | D_GOVERNANCE 生命周期管理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
...
| 33 | classify_ttl_by_content.py | → | D_GOVERNANCE 生命周期管理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
...
| 36 | generate_rule_catalog.py | → | D_GOVERNANCE 生命周期管理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
| 37 | migrate_illegal_doctype.py | → | D_GOVERNANCE 生命周期管理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
| 38 | validate_architecture.py | → | D_GOVERNANCE 生命周期管理: D3 元数据合规 — Markdown/YAML 文档元数据（fron... | config_depends / config_depends |
...
| 81 | merge_readme_to_index.py | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
...
| 85 | AGENTS.md 6.4 铁律五 + 铁律六：蓝图中声称的文件... | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
...
| 91 | validate_adr_frontmatter_consistency.py | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
...
| 100 | validate_depends_on_format.py | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
...
| 110 | validate_static_manifest_drift.py — GATE-21 静... | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
| 111 | 对标：target_layer_vocabulary.yaml v1.0.0——ta... | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
| 112 | validate_three_way_consistency.py | → | D_GOVERNANCE 生命周期管理: __init__.py | config_depends / config_depends |
...
| 2 | D_GOVERNANCE 生命周期管理: __init__.py | → | validate_yaml_interface_uniqueness.py — YAML .... | config_depends / config_depends |
...
| 5 | D_GOVERNANCE 生命周期管理: 单元测试：scripts/governance/d3_metadata/check_... | → | check_frontmatter_metadata.py | 测试依赖 / test_depends |
| 6 | D_GOVERNANCE 生命周期管理: test_generate_gate_registry.py — generate_gate... | → | generate_gate_registry.py — 门禁登记表自动生成... | 测试依赖 / test_depends |
| 7 | D_GOVERNANCE 生命周期管理: test_ssot_gate — SSoT 创建门禁红蓝变异测试。 (... | → | __init__.py | 测试依赖 / test_depends |
| 8 | D_GOVERNANCE 生命周期管理: test_blueprint_frontmatter_reconciler.py — 蓝.... | → | blueprint_frontmatter_reconciler.py — 蓝图 fro... | 测试依赖 / test_depends |
| 9 | D_GOVERNANCE 生命周期管理: test_sync_panorama_module.py — 四图模块同步引.... | → | sync_panorama_module.py — 四图模块同步引擎（AR... | 测试依赖 / test_depends |
| 10 | D_GOV_AUDIT 审计追踪: [INVARIANTS] 按path精确匹配+按功能名模糊匹配; .... | → | concurrent_commit_test.py — 幽灵提交红蓝对抗脚... | config_depends / config_depends |
| 11 | D_GOV_CODE_QUALITY 代码质量治理: check_module_id_consistency.py — module_id 全.... | → | D7 代码质量 — Python 代码静态分析与质量合规审... | config_depends / config_depends |
| 12 | D_GOV_RULE 规则治理: generate_script_manifest.py — 脚本清单自动生成... | → | __init__.py | config_depends / config_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 20 个外部域直接连接（出边 391 条 + 入边 12 条 = 403 条）。只显示直接连接的域，不展开具体节点。
