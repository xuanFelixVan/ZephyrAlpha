...
graph TD
    ...
    class D_GOVERNANCE,D_SHARED,D_GOV_AUDIT,D_INTELLIGENCE external_prod
```
...
graph TD
    ...
    class D_ORCHESTRATOR,D_GOV_REPAIR,D_GOV_RULE,D_GOVERNANCE external_prod
```
...
graph TD
    ...
    class D_SHARED,D_DATA,D_SECURITY,D_GOV_ENFORCEMENT external_prod
    class D_GOVERNANCE external_design
```
...
graph TD
    ...
    class D_GOVERNANCE,D_INFRA_RUNTIME,D_GOV_REPAIR,D_SHARED external_prod
```
...
graph TD
    ...
    class D_ORCHESTRATOR,D_GOV_RULE,D_SHARED,D_INFRA_RUNTIME,D_INFRA_RECOVERY,D_GOVERNANCE external_prod
```
...
> 本域与 20 个外部域直接连接（出边 394 条 + 入边 10 条 = 404 条）。只显示直接连接的域，不展开具体节点。
