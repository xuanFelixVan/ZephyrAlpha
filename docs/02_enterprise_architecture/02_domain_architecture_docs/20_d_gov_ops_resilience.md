...
| 61 | 对抗测试器 / Adversarial Tester (security_governance/adve... | → | D_OPS 反馈循环: —5.133.2 DI 注入契约 / Budget Engine (ops_governance/bud... | 导入依赖 / import_depends |
...
| 71 | PreFlight门禁测试 / Test Pre Flight Gate (budget/test_pre... | → | D_OPS 反馈循环: —5.133.2 DI 注入契约 / Budget Engine (ops_governance/bud... | 测试依赖 / test_depends |
...
| 80 | Environment管理器 / Environment Manager (ops_governance/e... | → | D_SHARED 共享服务: Secrets (security/secrets.py) | 导入依赖 / import_depends |
