...
flowchart TD
    ...
    src_zephyr_governance_ops_governance_tco_model_py ~~~ src_zephyr_governance_resilience_governance_bus_factor_defense_py
    ...
    tests_governance_budget_test_cost_budget_unit_py -->|测试依赖 / test_depends| D_OPS
    ...
    D_GOVERNANCE -->|测试依赖 / test_depends| src_zephyr_governance_security_governance_default_security_gateway_py
    ...
    D_GOVERNANCE -->|测试依赖 / test_depends| src_zephyr_governance_escalation_spof_checker_py
    ...
    class D_SHARED,D_OPS,D_GOV_ENFORCEMENT,D_INFRA_RECOVERY,D_GOVERNANCE,D_GOV_CODE_QUALITY,D_GOV_RULE,D_INFRA_A2A,D_SECURITY,D_GOV_REPAIR external_prod
...
|:--:|---------|:--:|---------|---------|
| 1 | Bus Factor Defense / Bus Factor Defense (resilience_gover... | → | D_FACTOR 因子: 总线因子防御 / bus_factor_defense (factor/bus_factor_defe... | 导入依赖 / import_depends |
...
| 10 | Test Budget Enforcer Smoke / Test Budget Enforcer Smoke (... | → | D_GOVERNANCE 生命周期管理: 模型路由器 / model_router (intelligence_governance/model_... | 测试依赖 / test_depends |
| 11 | Test Conversation Tax Detector / Test Conversation Tax De... | → | D_GOVERNANCE 生命周期管理: conversationtax检测器 / conversation_tax_detector (contex... | 测试依赖 / test_depends |
...
| 14 | Test Command Chain Length Gate / Test Command Chain Lengt... | → | D_GOVERNANCE 生命周期管理: 命令链长度门禁 / command_chain_length_gate (context_gover... | 测试依赖 / test_depends |
...
| 16 | Test Mvep Orchestrator / Test Mvep Orchestrator (orchestr... | → | D_GOVERNANCE 生命周期管理: mvep编排器 / mvep_orchestrator (intelligence_governance/m... | 测试依赖 / test_depends |
| 17 | Test Objective Tracker / Test Objective Tracker (orchestr... | → | D_GOVERNANCE 生命周期管理: objective追踪器 / objective_tracker (observability_govern... | 测试依赖 / test_depends |
| 18 | Test Think Time Model / Test Think Time Model (orchestrat... | → | D_GOVERNANCE 生命周期管理: thinktime模型 / think_time_model (context_governance/thin... | 测试依赖 / test_depends |
| 19 | P0 inflation guard + block_sessions_count + timeout exemp... | → | D_GOVERNANCE 生命周期管理: 任务repo / task_repo (persistence/task_repo.py) | 测试依赖 / test_depends |
...
| 24 | depgraph YAML 加载或结构校验失败.""" / Blast Radius (resi... | → | D_GOV_AUDIT 审计追踪: 语义审计管线数据模型 — MOD-INF-028 §4.2 / models (seman... | 导入依赖 / import_depends |
...
| 26 | Test Debt Projector / Test Debt Projector (budget/test_de... | → | D_GOV_CODE_QUALITY 代码质量治理: debt投影器 / debt_projector (code_dedup/debt_projector.py) | 测试依赖 / test_depends |
| 27 | Test Degradation / Test Degradation (budget/test_degradat... | → | D_GOV_CODE_QUALITY 代码质量治理: 退化 / degradation (code_dedup/degradation.py) | 测试依赖 / test_depends |
| 28 | Test Prioritizer / Test Prioritizer (orchestrator/test_pr... | → | D_GOV_CODE_QUALITY 代码质量治理: 修复优先级排序器 — 置信度×Impact×适配性 三因子排序. / ... | 测试依赖 / test_depends |
| 29 | 审计动作类型""" / Security Gateway Base (security_governa... | → | D_GOV_ENFORCEMENT 规则执行: ComplianceRule 真源已合并至 zephyr.shared.contracts.compl... | 导入依赖 / import_depends |
| 30 | Test Pre Flight Gate / Test Pre Flight Gate (budget/test_... | → | D_GOV_ENFORCEMENT 规则执行: 只读：engine / Pre Flight Gate (rule_enforcement/pre_flig... | 测试依赖 / test_depends |
| 31 | Test Budget Enforcer Smoke / Test Budget Enforcer Smoke (... | → | D_GOV_REPAIR 治理修复: 延迟导入 BudgetEngine 避免循环依赖.""" / Budget Enforceme... | 测试依赖 / test_depends |
| 32 | GCT-024 硬检查：验证 BudgetEngine 实例化、三维覆盖、策略... | → | D_GOV_REPAIR 治理修复: 延迟导入 BudgetEngine 避免循环依赖.""" / Budget Enforceme... | 测试依赖 / test_depends |
| 33 | Triage / Triage (escalation/triage.py) | → | D_GOV_RULE 规则治理: 门禁裁决引擎 / Gate Engine (gate_engine/gate_engine.py) | 导入依赖 / import_depends |
| 34 | Triage / Triage (escalation/triage.py) | → | D_GOV_RULE 规则治理: 门禁类型定义 / Gate Types (rule_enforcement/gate_types.py) | 导入依赖 / import_depends |
...
| 37 | P0 inflation guard + block_sessions_count + timeout exemp... | → | D_GOV_RULE 规则治理: 任务类型定义 / Task Types (rule_enforcement/task_types.py) | 测试依赖 / test_depends |
| 38 | 启动/关闭结果 / F5 Boot Integration (resilience_governanc... | → | D_INFRA_A2A A2A通信: priority -> rule -> escalation / Arbitrator (layer3_coord... | 导入依赖 / import_depends |
| 39 | 订阅操作结果 / F5 Event Subscriber (resilience_governance... | → | D_INFRA_A2A A2A通信: priority -> rule -> escalation / Arbitrator (layer3_coord... | 导入依赖 / import_depends |
| 40 | Offline Autonomy / Offline Autonomy (resilience_governanc... | → | D_INFRA_A2A A2A通信: Offline Autonomy / Offline Autonomy (a2a_protocol/offline... | 导入依赖 / import_depends |
| 41 | Offline Resilience / Offline Resilience (resilience_gover... | → | D_INFRA_A2A A2A通信: Offline Resilience / Offline Resilience (a2a_protocol/off... | 导入依赖 / import_depends |
| 42 | G-CT-003/004/006/008 消费端.""" / Contracts (escalation/c... | → | D_INFRA_RECOVERY 回滚恢复: Contracts / Contracts (rollback/contracts.py) | 导入依赖 / import_depends |
| 43 | 44 个阶段门控检查映射. / Phase Check Registry (ops_govern... | → | D_INFRA_RECOVERY 回滚恢复: Kill Switch / Kill Switch (rollback/kill_switch.py) | 导入依赖 / import_depends |
| 44 | 44 个阶段门控检查映射. / Phase Check Registry (ops_govern... | → | D_INFRA_RECOVERY 回滚恢复: Rollback Executor / Rollback Executor (rollback/rollback_... | 导入依赖 / import_depends |
| 45 | 只读：failure_count / Circuit Breaker (resilience_governa... | → | D_INFRA_RUNTIME 运行时集成: Circuit Breaker / Circuit Breaker (reliability/circuit_br... | 导入依赖 / import_depends |
| 46 | G-CT-003/004/006/008 消费端.""" / Contracts (escalation/c... | → | D_INTEGRATION 管线路由: Rollback Types / Rollback Types (contracts/rollback_types... | 导入依赖 / import_depends |
| 47 | Result Types / Result Types (escalation/result_types.py) | → | D_INTEGRATION 管线路由: Rollback Types / Rollback Types (contracts/rollback_types... | 导入依赖 / import_depends |
| 48 | 44 个阶段门控检查映射. / Phase Check Registry (ops_govern... | → | D_INTEGRATION 管线路由: Bridge Layer / Bridge Layer (vector_memory/bridge_layer.py) | 导入依赖 / import_depends |
| 49 | 44 个阶段门控检查映射. / Phase Check Registry (ops_govern... | → | D_INTEGRATION 管线路由: Collection Manager / Collection Manager (vector_memory/co... | 导入依赖 / import_depends |
| 50 | 44 个阶段门控检查映射. / Phase Check Registry (ops_govern... | → | D_INTEGRATION 管线路由: In Process Vector Memory / In Process Vector Memory (vect... | 导入依赖 / import_depends |
| 51 | 44 个阶段门控检查映射. / Phase Check Registry (ops_govern... | → | D_INTEGRATION 管线路由: Index Health Monitor / Index Health Monitor (vector_memor... | 导入依赖 / import_depends |
| 52 | 44 个阶段门控检查映射. / Phase Check Registry (ops_govern... | → | D_INTEGRATION 管线路由: Protocols / Protocols (contracts/protocols.py) | 导入依赖 / import_depends |
| 53 | 将 D-DATA 实现注册到 ServiceRegistry / Service Registrati... | → | D_INTEGRATION 管线路由: Collection Manager / Collection Manager (vector_memory/co... | 导入依赖 / import_depends |
| 54 | 将 D-DATA 实现注册到 ServiceRegistry / Service Registrati... | → | D_INTEGRATION 管线路由: In Process Vector Memory / In Process Vector Memory (vect... | 导入依赖 / import_depends |
| 55 | 将 D-DATA 实现注册到 ServiceRegistry / Service Registrati... | → | D_INTELLIGENCE 上下文管理: Reranker / Reranker (model_evaluation/reranker.py) | 导入依赖 / import_depends |
| 56 | 公共接口：wasserstein_1d / Burn Rate Monitor (ops_governa... | → | D_OPS 反馈循环: Budget Models / Budget Models (ops_governance/budget_mode... | 导入依赖 / import_depends |
| 57 | 只读：attributions / Cost Attributor (ops_governance/cost... | → | D_OPS 反馈循环: Budget Models / Budget Models (ops_governance/budget_mode... | 导入依赖 / import_depends |
| 58 | Degradation Manager / Degradation Manager (ops_governance... | → | D_OPS 反馈循环: Budget Models / Budget Models (ops_governance/budget_mode... | 导入依赖 / import_depends |
| 59 | 44 个阶段门控检查映射. / Phase Check Registry (ops_govern... | → | D_OPS 反馈循环: —5.133.2 DI 注入契约 / Budget Engine (ops_governance/bud... | 导入依赖 / import_depends |
| 60 | 44 个阶段门控检查映射. / Phase Check Registry (ops_govern... | → | D_OPS 反馈循环: Budget Models / Budget Models (ops_governance/budget_mode... | 导入依赖 / import_depends |
| 61 | Adversarial Tester / Adversarial Tester (security_governa... | → | D_OPS 反馈循环: —5.133.2 DI 注入契约 / Budget Engine (ops_governance/bud... | 导入依赖 / import_depends |
| 62 | Adversarial Tester / Adversarial Tester (security_governa... | → | D_OPS 反馈循环: Budget Models / Budget Models (ops_governance/budget_mode... | 导入依赖 / import_depends |
| 63 | Test Burn Rate Monitor / Test Burn Rate Monitor (budget/t... | → | D_OPS 反馈循环: Budget Models / Budget Models (ops_governance/budget_mode... | 测试依赖 / test_depends |
| 64 | Test Cost Attributor / Test Cost Attributor (budget/test_... | → | D_OPS 反馈循环: Budget Models / Budget Models (ops_governance/budget_mode... | 测试依赖 / test_depends |
| 65 | Test Cost Budget Root / Test Cost Budget Root (budget/tes... | → | D_OPS 反馈循环: 成本预算超出硬性熔断阈值时抛出 / Cost Budget (ops_governa... | 测试依赖 / test_depends |
| 66 | Test Cost Budget Unit / Test Cost Budget Unit (budget/tes... | → | D_OPS 反馈循环: 成本预算超出硬性熔断阈值时抛出 / Cost Budget (ops_governa... | 测试依赖 / test_depends |
| 67 | Test Degradation Manager / Test Degradation Manager (budg... | → | D_OPS 反馈循环: Budget Models / Budget Models (ops_governance/budget_mode... | 测试依赖 / test_depends |
| 68 | GCT-024 硬检查：验证 BudgetEngine 实例化、三维覆盖、策略... | → | D_OPS 反馈循环: —5.133.2 DI 注入契约 / Budget Engine (ops_governance/bud... | 测试依赖 / test_depends |
| 69 | GCT-024 硬检查：验证 BudgetEngine 实例化、三维覆盖、策略... | → | D_OPS 反馈循环: Budget Handler / Budget Handler (ops_governance/budget_ha... | 测试依赖 / test_depends |
| 70 | Test Governance Budget Tracker / Test Governance Budget T... | → | D_OPS 反馈循环: Budget Tracker / Budget Tracker (ops_governance/budget_tr... | 测试依赖 / test_depends |
| 71 | Test Pre Flight Gate / Test Pre Flight Gate (budget/test_... | → | D_OPS 反馈循环: —5.133.2 DI 注入契约 / Budget Engine (ops_governance/bud... | 测试依赖 / test_depends |
| 72 | Test Pre Flight Gate / Test Pre Flight Gate (budget/test_... | → | D_OPS 反馈循环: Budget Models / Budget Models (ops_governance/budget_mode... | 测试依赖 / test_depends |
| 73 | 空 Protocol 作为 12 个异构 detector 类的鸭子类型标记 / Es... | → | D_SECURITY 对抗验证: Gateway / Gateway (llm_security/gateway.py) | 导入依赖 / import_depends |
...
| 75 | " in finding` 语法 / Default Security Gateway (security_g... | → | D_SECURITY 对抗验证: Gateway / Gateway (llm_security/gateway.py) | 导入依赖 / import_depends |
| 76 | " in finding` 语法 / Default Security Gateway (security_g... | → | D_SECURITY 对抗验证: path whitelist + command whitelist + token budget guard. ... | 导入依赖 / import_depends |
| 77 | G-CT-003/004/006/008 消费端.""" / Contracts (escalation/c... | → | D_SHARED 共享服务: Budget Alert / Budget Alert (escalation/budget_alert.py) | 导入依赖 / import_depends |
...
| 79 | Triage / Triage (escalation/triage.py) | → | D_SHARED 共享服务: vocabulary YAML 加载公共工具 / Yaml Utils (io/yaml_utils.py) | 导入依赖 / import_depends |
| 80 | Environment Manager / Environment Manager (ops_governance... | → | D_SHARED 共享服务: Secrets / Secrets (security/secrets.py) | 导入依赖 / import_depends |
...
| 83 | 44 个阶段门控检查映射. / Phase Check Registry (ops_govern... | → | D_SHARED 共享服务: Session Continuity / Session Continuity (session/session_... | 导入依赖 / import_depends |
...
| 86 | depgraph YAML 加载或结构校验失败.""" / Blast Radius (resi... | → | D_SHARED 共享服务: 从当前文件向上查找项目根目录 / Paths (io/paths.py) | 导入依赖 / import_depends |
...
| 89 | " in finding` 语法 / Default Security Gateway (security_g... | → | D_SHARED 共享服务: Security Decision / Security Decision (security/security_... | 导入依赖 / import_depends |
...
| 91 | 5.17.5 修复：解析 HMAC 密钥 / Tamper Evident Log (securit... | → | D_SHARED 共享服务: Secrets / Secrets (security/secrets.py) | 导入依赖 / import_depends |
| 92 | 从 _registry.yaml 动态计算期望的 gate_id 集合 / Test Gate... | → | D_SHARED 共享服务: —蓝图 MOD-TASK_SYSTEM §3.2.2""" / Models (foundation/mo... | 测试依赖 / test_depends |
| 93 | P0 inflation guard + block_sessions_count + timeout exemp... | → | D_SHARED 共享服务: —蓝图 MOD-TASK_SYSTEM §3.2.2""" / Models (foundation/mo... | 测试依赖 / test_depends |
| 94 | P0 inflation guard + block_sessions_count + timeout exemp... | → | D_SHARED 共享服务: re-homed from infrastructure_runtime_integration.db.circu... | 测试依赖 / test_depends |

### 依赖本域的其他域（入边）/ Depended By

| # | 外部域-源模块 / Source Module | → | 本域模块 / Target Module | 依赖类型 / Type |
|:--:|---------|:--:|---------|---------|
| 1 | D_COMPLIANCE 合规: 异步intercept队列 / async_intercept_queue (compliance/asy... | → | 审计动作类型""" / Security Gateway Base (security_governa... | runtime / runtime |
...
| 3 | D_FEEDBACK_LOOP 反馈循环引擎: 调度器act / scheduler_act (feedback_loop/scheduler_act.py) | → | Escalation Models / Escalation Models (escalation/escalat... | 导入依赖 / import_depends |
| 4 | D_GOVERNANCE 生命周期管理: A2A故障 / a2a_failure (agent_spec/a2a_failure.py) | → | G-CT-003/004/006/008 消费端.""" / Contracts (escalation/c... | 导入依赖 / import_depends |
...
| 6 | D_GOVERNANCE 生命周期管理: delegation引擎 / Delegation Engine — MOD-INF-022 (intell... | → | Escalation Models / Escalation Models (escalation/escalat... | 导入依赖 / import_depends |
...
| 8 | D_GOVERNANCE 生命周期管理: 自测试 / Escalation Protocol Self-Test — MOD-INF-022. (i... | → | Escalation Models / Escalation Models (escalation/escalat... | 导入依赖 / import_depends |
...
| 13 | D_GOVERNANCE 生命周期管理: 适配器 / adapter (services/adapter.py) | → | Escalation Models / Escalation Models (escalation/escalat... | 导入依赖 / import_depends |
...
| 15 | D_GOVERNANCE 生命周期管理: 治理服务端 / governance_server (mcp/governance_server.py) | → | Escalation Models / Escalation Models (escalation/escalat... | 导入依赖 / import_depends |
| 16 | D_GOVERNANCE 生命周期管理: Test Account Isolator / Test Account Isolator (access_con... | → | Account Isolator / Account Isolator (resilience_governanc... | 测试依赖 / test_depends |
| 17 | D_GOVERNANCE 生命周期管理: Test Credential Guard / Test Credential Guard (access_con... | → | Credential Guard / Credential Guard (security_governance/... | 测试依赖 / test_depends |
| 18 | D_GOVERNANCE 生命周期管理: Test Adversarial Tester / Test Adversarial Tester (advers... | → | Adversarial Tester / Adversarial Tester (security_governa... | 测试依赖 / test_depends |
| 19 | D_GOVERNANCE 生命周期管理: Test Anti Automation Bias / Test Anti Automation Bias (ad... | → | D-022-09 mandatory human oversight enforcement. / Anti Au... | 测试依赖 / test_depends |
| 20 | D_GOVERNANCE 生命周期管理: Test Compositional Safety Tester / Test Compositional Saf... | → | Compositional Safety Tester / Compositional Safety Tester... | 测试依赖 / test_depends |
| 21 | D_GOVERNANCE 生命周期管理: Test Persuasion Detector / Test Persuasion Detector (adve... | → | Persuasion Detector / Persuasion Detector (security_gover... | 测试依赖 / test_depends |
| 22 | D_GOVERNANCE 生命周期管理: Test Poison Cascade Detector / Test Poison Cascade Detect... | → | 只读：suspicion_threshold / Poison Cascade Detector (secu... | 测试依赖 / test_depends |
| 23 | D_GOVERNANCE 生命周期管理: Test Vibe Security Verify / Test Vibe Security Verify (ad... | → | Vibe Security Verify / Vibe Security Verify (security_gov... | 测试依赖 / test_depends |
| 24 | D_GOVERNANCE 生命周期管理: Test Vibe Verify Integration / Test Vibe Verify Integrati... | → | 只读：scan_count / Vibe Verify Integration (security_gove... | 测试依赖 / test_depends |
| 25 | D_GOVERNANCE 生命周期管理: Test Human Factors / Test Human Factors (compliance/test_... | → | 每 owner 通知计数 / Human Factors (escalation/human_facto... | 测试依赖 / test_depends |
| 26 | D_GOVERNANCE 生命周期管理: Test Owner Absent / Test Owner Absent (compliance/test_ow... | → | 只读：data_dir / Owner Absent (escalation/owner_absent.py) | 测试依赖 / test_depends |
| 27 | D_GOVERNANCE 生命周期管理: Test Consequence Manager / Test Consequence Manager (dele... | → | Consequence Manager / Consequence Manager (escalation/con... | 测试依赖 / test_depends |
| 28 | D_GOVERNANCE 生命周期管理: Test Delegation Engine / Test Delegation Engine (delegati... | → | Escalation Models / Escalation Models (escalation/escalat... | 测试依赖 / test_depends |
| 29 | D_GOVERNANCE 生命周期管理: Test Parent Child Attributor / Test Parent Child Attribut... | → | 只读：max_depth / Parent Child Attributor (ops_governance... | 测试依赖 / test_depends |
| 30 | D_GOVERNANCE 生命周期管理: G-CT GCT集成契约测试.""" / Test Gct Integration (drift/te... | → | G-CT-003/004/006/008 消费端.""" / Contracts (escalation/c... | 测试依赖 / test_depends |
| 31 | D_GOVERNANCE 生命周期管理: G-CT GCT集成契约测试.""" / Test Gct Integration (drift/te... | → | Result Types / Result Types (escalation/result_types.py) | 测试依赖 / test_depends |
| 32 | D_GOVERNANCE 生命周期管理: Test Ghost Scan / Test Ghost Scan (drift/test_ghost_scan.py) | → | 只读：registered_pids / Ghost Scan (security_governance/g... | 测试依赖 / test_depends |
| 33 | D_GOVERNANCE 生命周期管理: Test Alternative Path Blocker / Test Alternative Path Blo... | → | Alternative Path Blocker / Alternative Path Blocker (esca... | 测试依赖 / test_depends |
| 34 | D_GOVERNANCE 生命周期管理: Test Result Types / Test Result Types (escalation/test_re... | → | Result Types / Result Types (escalation/result_types.py) | 测试依赖 / test_depends |
| 35 | D_GOVERNANCE 生命周期管理: Rollback → Escalation 集成测试.""" / Test Gct 003 Rollba... | → | G-CT-003/004/006/008 消费端.""" / Contracts (escalation/c... | 测试依赖 / test_depends |
| 36 | D_GOVERNANCE 生命周期管理: Rollback → Escalation 集成测试.""" / Test Gct 003 Rollba... | → | Result Types / Result Types (escalation/result_types.py) | 测试依赖 / test_depends |
| 37 | D_GOVERNANCE 生命周期管理: Test Gov 5system Integration / Test Gov 5system Integrati... | → | 空 Protocol 作为 12 个异构 detector 类的鸭子类型标记 / Es... | 测试依赖 / test_depends |
| 38 | D_GOVERNANCE 生命周期管理: Test Api Response Sanitizer / Test Api Response Sanitizer... | → | Api Response Sanitizer / Api Response Sanitizer (security... | 测试依赖 / test_depends |
| 39 | D_GOVERNANCE 生命周期管理: Test Bandwidth Optimizer / Test Bandwidth Optimizer (inte... | → | 每维度 self-normalize 到 [0,1] 范围 / Bandwidth Optimizer... | 测试依赖 / test_depends |
| 40 | D_GOVERNANCE 生命周期管理: Test Coldstart Manager / Test Coldstart Manager (lifecycl... | → | Coldstart Manager / Coldstart Manager (ops_governance/col... | 测试依赖 / test_depends |
| 41 | D_GOVERNANCE 生命周期管理: Test Maintenance Window Adapter / Test Maintenance Window... | → | Maintenance Window Adapter / Maintenance Window Adapter (... | 测试依赖 / test_depends |
| 42 | D_GOVERNANCE 生命周期管理: Test Startup Shutdown Cli / Test Startup Shutdown Cli (li... | → | Startup Shutdown Cli / Startup Shutdown Cli (ops_governan... | 测试依赖 / test_depends |
| 43 | D_GOVERNANCE 生命周期管理: Test Time Sync / Test Time Sync (lifecycle/test_time_sync... | → | Time Sync / Time Sync (ops_governance/time_sync.py) | 测试依赖 / test_depends |
| 44 | D_GOVERNANCE 生命周期管理: Test Clock Guard / Test Clock Guard (ops/test_clock_guard... | → | 只读：monotonic_start / Clock Guard (ops_governance/clock... | 测试依赖 / test_depends |
| 45 | D_GOVERNANCE 生命周期管理: Test Daily Ops / Test Daily Ops (ops/test_daily_ops.py) | → | Daily Ops / Daily Ops (ops_governance/daily_ops.py) | 测试依赖 / test_depends |
| 46 | D_GOVERNANCE 生命周期管理: Test Incident Response / Test Incident Response (ops/test... | → | Incident Response / Incident Response (escalation/inciden... | 测试依赖 / test_depends |
| 47 | D_GOVERNANCE 生命周期管理: Test Ops Foundation / Test Ops Foundation (ops/test_ops_f... | → | Ops Foundation / Ops Foundation (ops_governance/ops_found... | 测试依赖 / test_depends |
| 48 | D_GOVERNANCE 生命周期管理: Test Broker Resilience / Test Broker Resilience (resilien... | → | Broker Resilience / Broker Resilience (resilience_governa... | 测试依赖 / test_depends |
| 49 | D_GOVERNANCE 生命周期管理: Test Deadlock Detector / Test Deadlock Detector (resilien... | → | Escalation Models / Escalation Models (escalation/escalat... | 测试依赖 / test_depends |
| 50 | D_GOVERNANCE 生命周期管理: Test Deadlock Detector / Test Deadlock Detector (resilien... | → | Deadlock Detector / Deadlock Detector (resilience_governa... | 测试依赖 / test_depends |
| 51 | D_GOVERNANCE 生命周期管理: Test Fail Mode Manager / Test Fail Mode Manager (resilien... | → | 只读：state / Fail Mode Manager (resilience_governance/fa... | 测试依赖 / test_depends |
| 52 | D_GOVERNANCE 生命周期管理: Test Fault Tolerance / Test Fault Tolerance (resilience/t... | → | Fault Tolerance / Fault Tolerance (resilience_governance/... | 测试依赖 / test_depends |
| 53 | D_GOVERNANCE 生命周期管理: Test Interrupt Handler / Test Interrupt Handler (resilien... | → | 只读：signal / Interrupt Handler (ops_governance/interrup... | 测试依赖 / test_depends |
| 54 | D_GOVERNANCE 生命周期管理: Test Last Resort Watchdog / Test Last Resort Watchdog (re... | → | 只读：activated / Last Resort Watchdog (resilience_govern... | 测试依赖 / test_depends |
| 55 | D_GOVERNANCE 生命周期管理: Test Policy Sandbox / Test Policy Sandbox (resilience/tes... | → | 初始化 PolicySandbox / Policy Sandbox (resilience_governa... | 测试依赖 / test_depends |
| 56 | D_GOVERNANCE 生命周期管理: Test Process Isolator / Test Process Isolator (resilience... | → | 只读：processes / Process Isolator (resilience_governance... | 测试依赖 / test_depends |
| 57 | D_GOVERNANCE 生命周期管理: Test Spof Checker / Test Spof Checker (resilience/test_sp... | → | Spof Checker / Spof Checker (escalation/spof_checker.py) | 测试依赖 / test_depends |
| 58 | D_GOVERNANCE 生命周期管理: Test Stream Abort Guard / Test Stream Abort Guard (resili... | → | 流式中断守卫 / Stream Abort Guard (ops_governance/stream_... | 测试依赖 / test_depends |
| 59 | D_GOVERNANCE 生命周期管理: Test Timeout Guard / Test Timeout Guard (resilience/test_... | → | 只读：timeouts / Timeout Guard (ops_governance/timeout_gu... | 测试依赖 / test_depends |
| 60 | D_GOVERNANCE 生命周期管理: Test Witness Isolation / Test Witness Isolation (resilien... | → | 只读：witnesses / Witness Isolation (resilience_governanc... | 测试依赖 / test_depends |
| 61 | D_GOVERNANCE 生命周期管理: Test Github Api Guard / Test Github Api Guard (security/t... | → | 只读：allowed_commands / Github Api Guard (security_gover... | 测试依赖 / test_depends |
| 62 | D_GOVERNANCE 生命周期管理: Test Hooks Integrity Guard / Test Hooks Integrity Guard (... | → | 只读：hooks_hashes / Hooks Integrity Guard (security_gove... | 测试依赖 / test_depends |
| 63 | D_GOVERNANCE 生命周期管理: Test Ipi Defense / Test Ipi Defense (security/test_ipi_de... | → | 只读：block_threshold / Ipi Defense (security_governance/... | 测试依赖 / test_depends |
| 64 | D_GOVERNANCE 生命周期管理: G-CT-001~008 每条契约的端到端数据流通断言""" / Test P0 U1... | → | G-CT-003/004/006/008 消费端.""" / Contracts (escalation/c... | 测试依赖 / test_depends |
| 65 | D_GOVERNANCE 生命周期管理: G-CT-001~008 每条契约的端到端数据流通断言""" / Test P0 U1... | → | Result Types / Result Types (escalation/result_types.py) | 测试依赖 / test_depends |
| 66 | D_GOVERNANCE 生命周期管理: Test Sbom Guard / Test Sbom Guard (security/test_sbom_gua... | → | 只读：sbom / Sbom Guard (security_governance/sbom_guard.py) | 测试依赖 / test_depends |
| 67 | D_GOVERNANCE 生命周期管理: Test Security Config Scanner / Test Security Config Scann... | → | Security Config Scanner / Security Config Scanner (securi... | 测试依赖 / test_depends |
| 68 | D_GOVERNANCE 生命周期管理: governance core / Test Governance Core (shared/test_gover... | → | 44 个阶段门控检查映射. / Phase Check Registry (ops_govern... | 测试依赖 / test_depends |
| 69 | D_GOVERNANCE 生命周期管理: Phase Gates + 依赖审计隔离 + A2A Phase 4 Hold 测试.""" / ... | → | G-CT-003/004/006/008 消费端.""" / Contracts (escalation/c... | 测试依赖 / test_depends |
| 70 | D_GOVERNANCE 生命周期管理: Phase Gates + 依赖审计隔离 + A2A Phase 4 Hold 测试.""" / ... | → | Result Types / Result Types (escalation/result_types.py) | 测试依赖 / test_depends |
| 71 | D_GOVERNANCE 生命周期管理: Test Triage Unit / Test Triage Unit (shared/test_triage_u... | → | Triage / Triage (escalation/triage.py) | 测试依赖 / test_depends |
| 72 | D_GOVERNANCE 生命周期管理: Test E2e Pipeline / Test E2e Pipeline (trading/test_e2e_p... | → | " in finding` 语法 / Default Security Gateway (security_g... | 测试依赖 / test_depends |
...
| 75 | D_GOV_AUDIT 审计追踪: DOM-GOV-001 §8.4.""" / Test P0 I2 Construction Order (au... | → | G-CT-003/004/006/008 消费端.""" / Contracts (escalation/c... | 测试依赖 / test_depends |
| 76 | D_GOV_AUDIT 审计追踪: Test Tamper Evident Log / Test Tamper Evident Log (audit/... | → | 5.17.5 修复：解析 HMAC 密钥 / Tamper Evident Log (securit... | 测试依赖 / test_depends |
| 77 | D_GOV_CODE_QUALITY 代码质量治理: Test Bare Repo Scanner / Test Bare Repo Scanner (governan... | → | Bare Repo Scanner / Bare Repo Scanner (security_governanc... | 测试依赖 / test_depends |
| 78 | D_GOV_CODE_QUALITY 代码质量治理: Test Governance Result Types / Test Governance Result Typ... | → | Result Types / Result Types (escalation/result_types.py) | 测试依赖 / test_depends |
...
| 80 | D_GOV_REPAIR 治理修复: 延迟导入 BudgetEngine 避免循环依赖.""" / Budget Enforceme... | → | 公共接口：wasserstein_1d / Burn Rate Monitor (ops_governa... | 导入依赖 / import_depends |
| 81 | D_GOV_REPAIR 治理修复: 延迟导入 BudgetEngine 避免循环依赖.""" / Budget Enforceme... | → | Degradation Manager / Degradation Manager (ops_governance... | 导入依赖 / import_depends |
| 82 | D_GOV_REPAIR 治理修复: 延迟导入 BudgetEngine 避免循环依赖.""" / Budget Enforceme... | → | 只读：timeouts / Timeout Guard (ops_governance/timeout_gu... | 导入依赖 / import_depends |
...
| 85 | D_INFRA_A2A A2A通信: priority -> rule -> escalation / Arbitrator (layer3_coord... | → | Escalation Models / Escalation Models (escalation/escalat... | 导入依赖 / import_depends |
...
| 88 | D_INFRA_RUNTIME 运行时集成: Auto Runtime Core / Auto Runtime Core (trading/auto_runti... | → | Coldstart Manager / Coldstart Manager (ops_governance/col... | 导入依赖 / import_depends |
...
| 91 | D_OPS 反馈循环: —5.133.2 DI 注入契约 / Budget Engine (ops_governance/bud... | → | 只读：block_threshold / Ipi Defense (security_governance/... | 导入依赖 / import_depends |
| 92 | D_OPS 反馈循环: Budget Handler / Budget Handler (ops_governance/budget_ha... | → | G-CT-003/004/006/008 消费端.""" / Contracts (escalation/c... | 导入依赖 / import_depends |
| 93 | D_ORCHESTRATOR 代理编排器: Failure Matcher / Failure Matcher (resilience/failure_mat... | → | 声明式事件钩子注册表 / Event Hook (ops_governance/event_h... | 导入依赖 / import_depends |
| 94 | D_SECURITY 对抗验证: Escalation Bridge / Escalation Bridge (orphan_judge/escal... | → | 空 Protocol 作为 12 个异构 detector 类的鸭子类型标记 / Es... | 导入依赖 / import_depends |
| 95 | D_SECURITY 对抗验证: Game Day Scheduler / Game Day Scheduler (adversarial_vali... | → | ZephyrAlpha 施工阶段门控引擎. / Phase Manager (ops_govern... | 导入依赖 / import_depends |
