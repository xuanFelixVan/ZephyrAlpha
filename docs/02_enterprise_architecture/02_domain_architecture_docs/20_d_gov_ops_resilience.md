...
flowchart TD
    src_zephyr_governance_budget_enforcer_init_py["governance/budget-enforcer 包入口<br/>budget-enforcer<br/>包入口，整合budget-enforcer相关子模块导出<br/>文件: budget-enforcer/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_escalation_git_hook_pre_scanner_py["Git钩子预扫描器<br/>Git Hook预扫描不可跳过;risky_patterns必须匹配<br/>git_hook_pre_scanner<br/>文件: escalation/git_hook_pre_scanner.py<br/>(生产态 / production)"]
    src_zephyr_governance_escalation_human_factors_py["Human Factors — v0.7.0 人因工程:<br/>通知疲劳管理+上下文简<br/>洁性+多通道notifications<br/>human_factors<br/>文件: escalation/human_factors.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_escalation_incident_response_py["incident响应<br/>事件响应分级器，定义 L1即时/L2降级/L3部分<br/>/L4全面四级事件级别与响应策略，支持升级裁决与<br/>Kill Switch。<br/>incident_response<br/>文件: escalation/incident_response.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_escalation_result_types_py["结果类型定义<br/>结果类型定义;不可随意扩展<br/>文件: escalation/result_types.py<br/>(生产态 / production)"]
    src_zephyr_governance_escalation_spof_checker_py["spof检查器<br/>治理的类型，定义数据类型和枚举（spof）<br/>spof_checker<br/>文件: escalation/spof_checker.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_ops_governance_bandwidth_optimizer_py["bandwidth优化器<br/>Token/Cost/Time三维预算;超预算拒绝<br/>bandwidth_optimizer<br/>文件: ops_governance/bandwidth_optimizer.py<br/>(生产态 / production)"]
    src_zephyr_governance_ops_governance_burn_rate_monitor_py["burn速率监控器<br/>保障运维韧性与容错（burn rate）<br/>Burn Rate Monitor — MOD-INF-024<br/>文件: ops_governance/burn_rate_monitor.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_ops_governance_cost_attributor_py["成本attributor<br/>依赖预算模型工作（cost attributor）<br/>cost_attributor<br/>文件: ops_governance/cost_attributor.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_ops_governance_daily_ops_py["daily运维<br/>日常运维阶段定义，P1盘前/P2主交易/P3收市核实<br/>/P4盘后维护四阶段，含 Token/Cost/Time<br/>三维预算管控。<br/>daily_ops<br/>文件: ops_governance/daily_ops.py<br/>(生产态 / production)"]
    src_zephyr_governance_ops_governance_decision_fatigue_py["决策疲劳<br/>治理的核心类，封装EisenhowerPriority相关逻辑<br/>decision_fatigue<br/>文件: ops_governance/decision_fatigue.py<br/>(生产态 / production)"]
    src_zephyr_governance_ops_governance_degradation_manager_py["退化管理器<br/>依赖预算模型工作（degradation）<br/>degradation_manager<br/>文件: ops_governance/degradation_manager.py<br/>(生产态 / production)"]
    src_zephyr_governance_ops_governance_environment_manager_py["环境管理器<br/>治理管控（environment）<br/>environment_manager<br/>文件: ops_governance/environment_manager.py<br/>(生产态 / production)"]
    src_zephyr_governance_ops_governance_error_budget_burst_limiter_py["错误预算burst限制器<br/>治理管控（error budget burst limiter）<br/>error_budget_burst_limiter<br/>文件: ops_governance<br/>/error_budget_burst_limiter.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_ops_governance_maintenance_window_adapter_py["maintenancewindow适配器<br/>Maintenance Window Adapter — v0.10.0<br/>计划维护窗口适配器。<br/>maintenance_window_adapter<br/>文件: ops_governance<br/>/maintenance_window_adapter.py<br/>(生产态 / production)"]
    src_zephyr_governance_ops_governance_ops_foundation_py["运维foundation<br/>运维基础设施，定义 BackupLayer 备份层（CloudZip<br/>/DbDump/SecretsVault）与 LogCategory 日志分类。<br/>ops_foundation<br/>文件: ops_governance/ops_foundation.py<br/>(生产态 / production)"]
    src_zephyr_governance_ops_governance_parent_child_attributor_py["父子attributor<br/>保障运维韧性（parent child attributor）<br/>parent_child_attributor<br/>文件: ops_governance/parent_child_attributor.py<br/>(生产态 / production)"]
    src_zephyr_governance_ops_governance_roi_calculator_py["roi计算器<br/>治理的结果，封装操作结果的数据结构（roi<br/>calculator）<br/>roi_calculator<br/>文件: ops_governance/roi_calculator.py<br/>(生产态 / production)"]
    src_zephyr_governance_ops_governance_self_budget_tracker_py["自预算追踪器<br/>治理管控（self budget）<br/>self_budget_tracker<br/>文件: ops_governance/self_budget_tracker.py<br/>(生产态 / production)"]
    src_zephyr_governance_ops_governance_service_registration_py["服务registration<br/>使 D-INFRA 能通过 ServiceRegistry.get()<br/>获取实现，<br/>service_registration<br/>文件: ops_governance/service_registration.py<br/>(生产态 / production)"]
    src_zephyr_governance_ops_governance_startup_shutdown_py["ops_governance/startup_shutdown<br/>启动关机，治理管控（startup shutdown）<br/>文件: ops_governance/startup_shutdown.py<br/>(生产态 / production)"]
    src_zephyr_governance_ops_governance_startup_shutdown_cli_py["启动关机命令行<br/>治理管控（startup shutdown cli）<br/>startup_shutdown_cli<br/>文件: ops_governance/startup_shutdown_cli.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_resilience_governance_engine_sandbox_py["引擎沙箱<br/>为升级引擎提供 OS<br/>级沙箱隔离，防升级操作越界影响宿主系统<br/>文件: resilience_governance/engine_sandbox.py<br/>(生产态 / production)"]
    src_zephyr_governance_resilience_governance_f5_boot_integration_py["f5启动集成<br/>F5 自动启动集成，把升级引擎接入 F5 键的自动启动<br/>/关闭流程。<br/>f5_boot_integration<br/>文件: resilience_governance<br/>/f5_boot_integration.py<br/>(生产态 / production)"]
    src_zephyr_governance_resilience_governance_f5_event_subscriber_py["f5事件订阅器<br/>保障运维韧性与容错（f5 event subscriber）<br/>f5_event_subscriber<br/>文件: resilience_governance<br/>/f5_event_subscriber.py<br/>(生产态 / production)"]
    src_zephyr_governance_resilience_governance_f5_shutdown_manager_py["f5关机管理器<br/>F5 关机管理器，管理自动关闭流程，含状态持久化与<br/>信号处理，保证关机可恢复。<br/>f5_shutdown_manager<br/>文件: resilience_governance<br/>/f5_shutdown_manager.py<br/>(生产态 / production)"]
    src_zephyr_governance_resilience_governance_fail_mode_manager_py["failmode管理器<br/>实现业务功能（fail mode）<br/>fail_mode_manager<br/>文件: resilience_governance/fail_mode_manager.py<br/>(生产态 / production)"]
    src_zephyr_governance_resilience_governance_fault_tolerance_py["故障容错<br/>治理管控（fault tolerance）<br/>fault_tolerance<br/>文件: resilience_governance/fault_tolerance.py<br/>(生产态 / production)"]
    src_zephyr_governance_resilience_governance_last_resort_watchdog_py["末resortwatchdog<br/>Last Resort Watchdog — v0.8.0 终极逃生舱:<br/>所有escalation失败后的final fallback+shutdown。<br/>last_resort_watchdog<br/>文件: resilience_governance<br/>/last_resort_watchdog.py<br/>(生产态 / production)"]
    src_zephyr_governance_resilience_governance_offline_autonomy_py["离线autonomy<br/>依赖离线autonomy工作<br/>offline_autonomy<br/>文件: resilience_governance/offline_autonomy.py<br/>(生产态 / production)"]
    src_zephyr_governance_resilience_governance_offline_resilience_py["离线韧性<br/>依赖离线韧性工作<br/>offline_resilience<br/>文件: resilience_governance<br/>/offline_resilience.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_resilience_governance_witness_isolation_py["Witness Isolation — v0.8.0 Witness隔离: N版<br/>本decision验证+投票机制+majority判定<br/>witness_isolation<br/>文件: resilience_governance/witness_isolation.py<br/>(生产态 / production)"]
    src_zephyr_governance_security_governance_init_py["governance/security_governance 包入口<br/>security governance 包入口，整合security<br/>governance相关子模块导出<br/>文件: security_governance/__init__.py<br/>(生产态 / production)"]
    src_zephyr_governance_security_governance_adversarial_tester_py["对抗测试器<br/>依赖ipi防御、流中止守卫、预算引擎工作<br/>adversarial_tester<br/>文件: security_governance/adversarial_tester.py<br/>(生产态 / production)"]
    src_zephyr_governance_security_governance_anti_automation_bias_py["anti自动化bias<br/>保障运维韧性（anti automation bias）<br/>文件: security_governance<br/>/anti_automation_bias.py<br/>(生产态 / production)"]
    src_zephyr_governance_security_governance_api_response_sanitizer_py["API响应清洗器<br/>API Response Sanitizer — v0.9.0 API响应清洗器:<br/>外部API返回内容清洗+injection检测。<br/>api_response_sanitizer<br/>文件: security_governance<br/>/api_response_sanitizer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_security_governance_default_security_gateway_py["默认安全网关<br/>治理管控（default security gateway）<br/>default_security_gateway<br/>文件: security_governance<br/>/default_security_gateway.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_security_governance_hooks_integrity_guard_py["钩子完整性守卫<br/>Hooks自编辑防护不可禁用;外部hash必须验证<br/>hooks_integrity_guard<br/>文件: security_governance<br/>/hooks_integrity_guard.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_security_governance_poison_cascade_detector_py["poison级联检测器<br/>安全的事件，定义和分发事件<br/>poison_cascade_detector<br/>文件: security_governance<br/>/poison_cascade_detector.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_security_governance_security_config_scanner_py["安全配置扫描器<br/>Security Config Scanner — v0.13.0<br/>缺失安全配置扫描器。<br/>security_config_scanner<br/>文件: security_governance<br/>/security_config_scanner.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_security_governance_vibe_security_verify_py["vibe安全verify<br/>Vibe Security Verifier — v0.9.0 Vibe<br/>Coding安全验证器: AI生成代码安全基线检查。<br/>vibe_security_verify<br/>文件: security_governance<br/>/vibe_security_verify.py<br/>(生产态 / production)"]
    src_zephyr_governance_security_governance_vibe_verify_integration_py["vibeverify集成<br/>VibeVerify Integration — v0.9.0<br/>VibeVerify集成器:<br/>auto_guard级别+增量修复+confidence回传。<br/>vibe_verify_integration<br/>文件: security_governance<br/>/vibe_verify_integration.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_security_governance_compositional_safety_tester_py ~~~ src_zephyr_governance_security_governance_credential_guard_py
    ...
    src_zephyr_governance_resilience_governance_decision_fatigue_py ~~~ src_zephyr_governance_security_governance_config_scanner_py
    src_zephyr_governance_security_governance_config_scanner_py ~~~ src_zephyr_governance_security_governance_ipi_defense_py
    ...
    src_zephyr_governance_escalation_escalation_models_py["升级模型<br/>保障运维韧性与容错（escalation models）<br/>文件: escalation/escalation_models.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_resilience_governance_circuit_breaker_py["熔断断路器<br/>在错误率超阈值时熔断请求，防止级联故障扩散<br/>Circuit Breaker — MOD-INF-022<br/>文件: resilience_governance/circuit_breaker.py<br/>(生产态 / production)"]
    ...
    class D_GOV_AUDIT,D_INTEGRATION,D_INFRA_A2A,D_GOVERNANCE,D_SECURITY,D_SHARED,D_INFRA_RECOVERY,D_GOV_ENFORCEMENT,D_GOV_REPAIR,D_OPS,D_GOV_SCRIPTS,D_INFRA_RUNTIME,D_ORCHESTRATOR external_prod
