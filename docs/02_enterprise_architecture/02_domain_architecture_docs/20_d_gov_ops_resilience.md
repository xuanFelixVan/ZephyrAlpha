...
flowchart TD
    src_zephyr_governance_budget_enforcer_init_py["governance/budget-enforcer 包入口<br/>治理的包入口，把这一层的子模块归到一起统一管理，<br/>用到谁才加载谁，避免一次性全加载拖慢启动<br/>文件: budget-enforcer/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_escalation_incident_response_py["incident响应<br/>incident_response<br/>文件: escalation/incident_response.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_ops_governance_daily_ops_py["daily运维<br/>daily_ops<br/>文件: ops_governance/daily_ops.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_ops_governance_environment_manager_py["环境管理器<br/>管理多环境配置和切换<br/>environment_manager<br/>文件: ops_governance/environment_manager.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_ops_governance_parent_child_attributor_py["父子attributor<br/>父子归因器，追踪父子任务的资源消耗归属<br/>parent_child_attributor<br/>文件: ops_governance/parent_child_attributor.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_ops_governance_self_budget_tracker_py["自预算追踪器<br/>跟踪系统自身资源消耗防止超支<br/>self_budget_tracker<br/>文件: ops_governance/self_budget_tracker.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_ops_governance_startup_shutdown_py["启动关机<br/>编排，管理系统启动顺序和优雅关机流程<br/>startup_shutdown<br/>文件: ops_governance/startup_shutdown.py<br/>(生产态 / production)"]
    src_zephyr_governance_ops_governance_startup_shutdown_cli_py["启动关机命令行<br/>工具，提供启动关机的CLI入口<br/>startup_shutdown_cli<br/>文件: ops_governance/startup_shutdown_cli.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_ops_governance_timeout_guard_py["超时守卫<br/>监控操作超时并触发熔断降级<br/>timeout_guard<br/>文件: ops_governance/timeout_guard.py<br/>(生产态 / production)"]
    src_zephyr_governance_resilience_governance_init_py["governance/resilience_governance 包入口<br/>治理的包入口，把这一层的子模块归到一起统一管理，<br/>用到谁才加载谁，避免一次性全加载拖慢启动<br/>文件: resilience_governance/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_resilience_governance_broker_resilience_py["经纪人韧性<br/>broker_resilience<br/>文件: resilience_governance/broker_resilience.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_resilience_governance_decision_fatigue_cli_py["决策疲劳命令行<br/>decision_fatigue_cli<br/>文件: resilience_governance/decision_fatigue_<br/>cli.py<br/>(生产态 / production)"]
    src_zephyr_governance_resilience_governance_engine_sandbox_py["引擎沙箱<br/>引擎sandbox。EngineSandbox — D-022-08 OS-level<br/>sandboxing for the escalation engine<br/>EngineSandbox — D-022-08 OS-level sandboxing<br/>for the escalation engine.<br/>文件: resilience_governance/engine_sandbox.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_resilience_governance_fail_mode_manager_py["failmode管理器<br/>失败模式管理器，定义和管理组件各失败模式的行为<br/>fail_mode_manager<br/>文件: resilience_governance/fail_mode_manager.py<br/>(生产态 / production)"]
    src_zephyr_governance_resilience_governance_fault_tolerance_py["故障容错<br/>检测故障并自动恢复保证服务可用<br/>fault_tolerance<br/>文件: resilience_governance/fault_tolerance.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_security_governance_init_py["governance/security_governance 包入口<br/>安全的包入口，把这一层的子模块归到一起统一管理，<br/>用到谁才加载谁，避免一次性全加载拖慢启动<br/>文件: security_governance/__init__.py<br/>(生产态 / production)"]
    src_zephyr_governance_security_governance_anti_automation_bias_py["anti自动化bias<br/>强制人工监督，防止过度自动化导致决策偏差<br/>Anti-Automation Bias — D-022-09 mandatory human<br/>oversight enforcement.<br/>文件: security_governance/anti_automation_<br/>bias.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_resilience_governance_decision_fatigue_py["决策疲劳<br/>decision_fatigue<br/>文件: resilience_governance/decision_fatigue.py<br/>(生产态 / production)"]
