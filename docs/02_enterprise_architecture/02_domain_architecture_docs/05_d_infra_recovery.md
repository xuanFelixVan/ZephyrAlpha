...
flowchart TD
    ...
    class D_SHARED,D_GOV_AUDIT,D_GOV_OPS_RESILIENCE,D_INFRA_RUNTIME,D_GOVERNANCE,D_OPS,D_FEEDBACK_LOOP,D_GOV_DRIFT external_prod
```
...
| 26 | Rollback Integration (rollback/rollback_integration.py) | → | D_SHARED 共享服务: 密钥 / Secrets (security/secrets.py) | 导入依赖 / import_depends |
...
| 5 | D_GOVERNANCE 生命周期管理: Test Credential Rotation Trigger (access_control/test_cre... | → | 5.62.5 治本名实分离） / Credential Rotation Trigger (roll... | 测试依赖 / test_depends |
...
| 10 | D_GOVERNANCE 生命周期管理: Test Sqlite Dumper (data_layer/test_sqlite_dumper.py) | → | 表名无法参数化，用白名单替代） / Sqlite Dumper (rollback/... | 测试依赖 / test_depends |
