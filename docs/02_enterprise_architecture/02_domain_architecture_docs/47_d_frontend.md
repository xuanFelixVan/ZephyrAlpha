...
flowchart TD
    scripts_tests_test_frontend_components_py["(生产态 / production)<br/>测试前端components / test_frontend_<br/>components<br/>5个前端组件综合验证脚本（TTL=task_<br/>bound，施工完成后退役）<br/>文件: tests/test_frontend_components.py"]
    src_zephyr_frontend_dashboard_app_py["(生产态 / production) 应用 / app<br/>ZephyrAlpha Dashboard · Streamlit<br/>仪表盘（已弃用 v3.1.0）<br/>文件: dashboard/app.py"]
    src_zephyr_frontend_dashboard_app_panel_py["(生产态 / production) 应用面板 / app_<br/>panel<br/>app_panel · Panel 仪表盘主应用入口<br/>（v3.1.0, #ARCH-047）<br/>文件: dashboard/app_panel.py"]
    src_zephyr_frontend_implementations_default_approval_gateway_py["(设计态 / design) 默认审批网关 /<br/>default_approval_gateway<br/>默认approval网关，前端的门禁，在关键节点<br/>检查是否放行。<br/>文件: implementations/default_approval_<br/>gateway.py<br/>⛔ 该域，设计已就绪，等待开发排期"]
    src_zephyr_frontend_implementations_default_notification_manager_py["(设计态 / design) 默认通知管理器 /<br/>default_notification_manager<br/>默认通知管理器，前端的通知器，把消息推送<br/>给接收方。<br/>文件: implementations/default_<br/>notification_manager.py<br/>⛔ 该域，设计已就绪，等待开发排期"]
    ...
    src_zephyr_frontend_dashboard_components_backtest_performance_py["(生产态 / production) 回测绩效 /<br/>backtest_performance<br/>backtest_performance ·<br/>掘金量化风格绩效分析可视化（v1.0.0,<br/>#ARCH-047）<br/>文件: components/backtest_performance.py"]
    src_zephyr_frontend_dashboard_components_backtest_results_py["(生产态 / production) 回测results /<br/>backtest_results<br/>backtest_results · 回测结果可视化组件<br/>（v3.0.0 Panel+HoloViz 重构, #ARCH-047）<br/>文件: components/backtest_results.py"]
    src_zephyr_frontend_dashboard_components_fitness_functions_py["(生产态 / production) 适应度functions /<br/>fitness_functions<br/>fitness_functions · Fitness Functions<br/>仪表盘组件（v3.1.0 Panel 迁移,<br/>#ARCH-047）<br/>文件: components/fitness_functions.py"]
    src_zephyr_frontend_dashboard_components_order_book_py["(生产态 / production) 订单book / order_<br/>book<br/>order_book · 5档盘口实时展示组件<br/>（v3.0.0 Panel+HoloViz 重构, #ARCH-047）<br/>文件: components/order_book.py"]
    src_zephyr_frontend_dashboard_components_position_monitor_py["(生产态 / production) 持仓监控 /<br/>position_monitor<br/>position_monitor · 实盘持仓监控组件<br/>（v3.0.0 Panel+HoloViz 重构, #ARCH-047）<br/>文件: components/position_monitor.py"]
    src_zephyr_frontend_dashboard_components_tick_replay_py["(生产态 / production) 逐笔replay / tick_<br/>replay<br/>tick_replay · Tick 回放可视化组件<br/>（v3.0.0 Panel+HoloViz 重构, #ARCH-047）<br/>文件: components/tick_replay.py"]
    src_zephyr_frontend_dashboard_components_trade_panel_py["(生产态 / production) 交易面板 / trade_<br/>panel<br/>trade_panel · 实盘交易面板组件（v3.0.0<br/>Panel+HoloViz 重构, #ARCH-047, human_<br/>gated）<br/>文件: components/trade_panel.py"]
    src_zephyr_frontend_interface_base_py["(生产态 / production) 接口基类 / D_<br/>FRONTEND — Human-AI Interface Layer<br/>Skeleton<br/>接口基类。D_FRONTEND — Human-AI<br/>Interface Layer Skeleton<br/>文件: frontend/interface_base.py"]
    ...
    src_zephyr_frontend_dashboard_components_chart_factory_py["(生产态 / production) chart工厂 / chart_<br/>factory<br/>chart_factory · 图表统一工厂<br/>（v3.0.0新增, #ARCH-047）<br/>文件: components/chart_factory.py"]
