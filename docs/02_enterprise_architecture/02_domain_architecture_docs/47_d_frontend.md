...
flowchart TD
    scripts_tests_test_frontend_components_py["(生产态 / production) 测试frontendcomponents / Test Frontend Components<br/>5个前端组件综合验证脚本（TTL=task_bound，施工完成后退役）<br/>文件: tests/test_frontend_components.py"]
    src_zephyr_frontend_dashboard_app_py["(生产态 / production) app / App<br/>ZephyrAlpha Dashboard · Streamlit 仪表盘（已弃用 v3.1.0）<br/>文件: dashboard/app.py"]
    src_zephyr_frontend_dashboard_app_panel_py["(生产态 / production) apppanel / App Panel<br/>app_panel · Panel 仪表盘主应用入口（v3.1.0, #ARCH-047）<br/>文件: dashboard/app_panel.py"]
    src_zephyr_frontend_interface_base_py["(生产态 / production) interface基础 / Interface Base<br/>D_FRONTEND — Human-AI Interface Layer Skeleton<br/>文件: frontend/interface_base.py"]
    ...
    src_zephyr_frontend_dashboard_components_backtest_performance_py["(生产态 / production) backtest性能 / Backtest Performance<br/>backtest_performance · 掘金量化风格绩效分析可视化（v1.0.0, #ARCH-047）<br/>文件: components/backtest_performance.py"]
    src_zephyr_frontend_dashboard_components_backtest_results_py["(生产态 / production) backtestresults / Backtest Results<br/>backtest_results · 回测结果可视化组件（v3.0.0 Panel+HoloViz 重构, #ARCH-047）<br/>文件: components/backtest_results.py"]
    src_zephyr_frontend_dashboard_components_fitness_functions_py["(生产态 / production) 适应度函数 / Fitness Functions<br/>fitness_functions · Fitness Functions 仪表盘组件（v3.1.0 Panel 迁移, #ARCH-047）<br/>文件: components/fitness_functions.py"]
    src_zephyr_frontend_dashboard_components_order_book_py["(生产态 / production) orderbook / Order Book<br/>order_book · 5档盘口实时展示组件（v3.0.0 Panel+HoloViz 重构, #ARCH-047）<br/>文件: components/order_book.py"]
    src_zephyr_frontend_dashboard_components_position_monitor_py["(生产态 / production) position监控器 / Position Monitor<br/>position_monitor · 实盘持仓监控组件（v3.0.0 Panel+HoloViz 重构, #ARCH-047）<br/>文件: components/position_monitor.py"]
    src_zephyr_frontend_dashboard_components_tick_replay_py["(生产态 / production) tickreplay / Tick Replay<br/>tick_replay · Tick 回放可视化组件（v3.0.0 Panel+HoloViz 重构, #ARCH-047）<br/>文件: components/tick_replay.py"]
    src_zephyr_frontend_dashboard_components_trade_panel_py["(生产态 / production) tradepanel / Trade Panel<br/>trade_panel · 实盘交易面板组件（v3.0.0 Panel+HoloViz 重构, #ARCH-047, human_...<br/>文件: components/trade_panel.py"]
    ...
    src_zephyr_frontend_dashboard_components_chart_factory_py["(生产态 / production) chart工厂 / Chart Factory<br/>chart_factory · 图表统一工厂（v3.0.0新增, #ARCH-047）<br/>文件: components/chart_factory.py"]
...
|:--:|---------|:--:|---------|---------|
| 1 | 适应度函数 / Fitness Functions (components/fitness_functi... | → | D_FEEDBACK_LOOP 反馈循环引擎: 适应度函数 / Fitness Functions (feedback_loop/fitness_fun... | 导入依赖 / import_depends |
| 2 | apppanel / App Panel (dashboard/app_panel.py) | → | D_GOVERNANCE 生命周期管理: sqliteschema / Sqlite Schema (persistence/sqlite_schema.py) | 导入依赖 / import_depends |
| 3 | apppanel / App Panel (dashboard/app_panel.py) | → | D_GOVERNANCE 生命周期管理: 任务repo / Task Repo (persistence/task_repo.py) | 导入依赖 / import_depends |
| 4 | chart工厂 / Chart Factory (components/chart_factory.py) | → | D_SHARED 共享服务: serialization / Serialization (io/serialization.py) | 导入依赖 / import_depends |
| 5 | tradepanel / Trade Panel (components/trade_panel.py) | → | D_SHARED 共享服务: 时间utils / Time Utils (utils/time_utils.py) | 导入依赖 / import_depends |
| 6 | tradepanel / Trade Panel (components/trade_panel.py) | → | D_TRADING 交易运营: order / Order (execution/order.py) | 导入依赖 / import_depends |
