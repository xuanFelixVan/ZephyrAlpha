...
> 展示全部 14 个模块（生产态 12 + 设计态 2），节点含成熟度+中英文名+大白话+文件路径。
...
flowchart TD
    ...
    src_zephyr_frontend_implementations_default_approval_gateway_py["(设计态 / design)<br/>文件: implementations/default_approval_gateway.py"]
    src_zephyr_frontend_implementations_default_notification_manager_py["(设计态 / design)<br/>文件: implementations/default_notification_manager.py"]
