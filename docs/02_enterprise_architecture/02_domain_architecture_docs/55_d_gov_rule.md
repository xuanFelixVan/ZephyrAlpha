...
flowchart TD
    ...
    class D_GOV_ENFORCEMENT,D_SHARED,D_GOV_AUDIT,D_GOV_DRIFT,D_GOVERNANCE,D_SECURITY,D_GOV_SCRIPTS,D_GOV_OPS_RESILIENCE external_prod
```
...
| 6 | 门禁引擎模块集 / Gate Engine Package (gate_engine/__init_... | → | D_GOV_DRIFT 漂移检测: —per-gate SLI 报告、误报率、延迟分布、1人+AI运维视图 / G... | config_depends / config_depends |
| 7 | 门禁裁决引擎 / Gate Engine (gate_engine/gate_engine.py) | → | D_GOV_DRIFT 漂移检测: Drift Infrastructure (gov_drift/drift_infrastructure.py) | 导入依赖 / import_depends |
...
| 9 | 规则引擎模块集 / Rule Engine Package (rule_engine/__init_... | → | D_GOV_ENFORCEMENT 规则执行: Rule Debt Auditor (rule_engine/rule_debt_auditor.py) | config_depends / config_depends |
...
| 14 | 门禁裁决引擎 / Gate Engine (gate_engine/gate_engine.py) | → | D_INFRA_RECOVERY 回滚恢复: Contract (rollback/contract.py) | 导入依赖 / import_depends |
| 15 | 任务完成门禁 / Task Completion Gate (rule_enforcement/tas... | → | D_INFRA_RUNTIME 运行时集成: Task Lifecycle Manager (lifecycle/task_lifecycle_manager.py) | 导入依赖 / import_depends |
...
| 18 | 单向熔断器 / Circuit Breaker (rule_enforcement/circuit_br... | → | D_SHARED 共享服务: Capability (security/capability.py) | 导入依赖 / import_depends |
...
| 21 | 契约模板管理器 / Contract Template Manager (rule_enforcem... | → | D_SHARED 共享服务: Schemas (schema/schemas.py) | 导入依赖 / import_depends |
| 22 | 门禁裁决引擎 / Gate Engine (gate_engine/gate_engine.py) | → | D_SHARED 共享服务: Io Cache (io/io_cache.py) | 导入依赖 / import_depends |
...
| 25 | 门禁类型定义 / Gate Types (rule_enforcement/gate_types.py) | → | D_SHARED 共享服务: Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 32 | 任务类型定义 / Task Types (rule_enforcement/task_types.py) | → | D_SHARED 共享服务: Base Config (schema/base_config.py) | 导入依赖 / import_depends |
| 33 | 任务类型定义 / Task Types (rule_enforcement/task_types.py) | → | D_SHARED 共享服务: Execution Model (schema/execution_model.py) | 导入依赖 / import_depends |
| 34 | 任务类型定义 / Task Types (rule_enforcement/task_types.py) | → | D_SHARED 共享服务: Severity Types (schema/severity_types.py) | 导入依赖 / import_depends |
