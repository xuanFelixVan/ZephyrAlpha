...
flowchart TD
    ...
    class D_SHARED,D_GOV_AUDIT,D_INFRA_RUNTIME,D_GOVERNANCE,D_INTEGRATION,D_INTELLIGENCE,D_SECURITY,D_GOV_SCRIPTS external_prod
```
...
| 8 | 规则引擎模块集 / Rule Engine Package (rule_engine/__init_... | → | D_GOV_ENFORCEMENT 规则执行: RuleWatcher — YAML 规则文件变更检测与自动同步 (rule_engi... | config_depends / config_depends |
