...
flowchart TD
    ...
    src_zephyr_gov_enforcement_rule_enforcement_adaptive_threshold_py["自适应阈值<br/>给告警算什么程度该报警的红线。两种算法：一种看成<br/>功率自动调，一种看最近一周的平均值定线。设了最低<br/>底线，防止红线越降越低、把问题掩盖掉<br/>Adaptive Threshold<br/>dual mode: probability-based (PASS/FAIL outcome<br/>adjustment) + count-based (EWMA baseline x<br/>factor)<br/>文件: rule_enforcement/adaptive_threshold.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_enforcement_rule_enforcement_anti_pattern_guard_py["反模式防护引擎<br/>把蓝图里列的 8 条 AI<br/>集成禁止行为做成自动检查，挂进门禁流程。防止 AI<br/>集成时踩常见的坑，比如绕过门禁、擅自越权<br/>Anti-Pattern Guard<br/>engine - detects and blocks common<br/>architectural anti-patterns<br/>文件: rule_enforcement/anti_pattern_guard.py<br/>(生产态 / production)"]
    src_zephyr_gov_enforcement_rule_enforcement_can_i_deploy_py["预部署门禁<br/>部署前问一句现在能部署吗。检查四样：别人对我的期<br/>望满足没、版本兼容不、契约一致不、服务健康不。避<br/>免一部署就出事<br/>Can-I-Deploy<br/>pre-deployment gate (GATE-CDC-1)<br/>文件: rule_enforcement/can_i_deploy.py<br/>(生产态 / production)"]
    src_zephyr_gov_enforcement_rule_enforcement_capability_checker_py["能力检查器<br/>运行时核对模块有没有真的声明所需权限，再校验权限<br/>表没被偷偷改过。确保声明的能力和实际用的对得上，<br/>防止钻空子<br/>Capability Checker<br/>verifies modules/scripts declare required<br/>capabilities<br/>文件: rule_enforcement/capability_checker.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_enforcement_rule_enforcement_contract_template_manager_py["契约模板管理器<br/>管理工具契约模板：注册新模板、按名字查模板、校验<br/>调用对不对、存成文件。让每个工具有统一的接口说明<br/>书，调用前能核对参数<br/>Contract Template Manager<br/>for MCP tool contracts<br/>文件: rule_enforcement/contract_template_<br/>manager.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_enforcement_rule_enforcement_gate_engine_gate_simulator_py["门禁模拟器<br/>门禁的演习工具。把全链路门禁空跑一遍，但不改任何<br/>状态不写库。让开发者提前看门禁会怎么判，避免真跑<br/>时才出问题<br/>Gate Simulator<br/>dry-run full-chain gate rehearsal without<br/>modifying any state<br/>文件: gate_engine/gate_simulator.py<br/>(生产态 / production)"]
    src_zephyr_gov_enforcement_rule_enforcement_integration_test_runner_py["集成测试运行器<br/>跑跨模块集成测试的引擎，加载契约、执行断言、对接<br/>门禁。分四级：最关键的冒烟、全量核心、契约校验、<br/>健康探针，不同场景跑不同级别<br/>Integration Test Runner<br/>runs cross-module integration tests<br/>文件: rule_enforcement/integration_test_<br/>runner.py<br/>(生产态 / production)"]
...
| 10 | 脚本清单自动生成器 / Script Manifest Generator (generator... | → | D_GOV_SCRIPTS 脚本治理: 编码安全工具 / encoding (_shared/encoding.py) | 导入依赖 / import_depends |
| 11 | 脚本清单自动生成器 / Script Manifest Generator (generator... | → | D_GOV_SCRIPTS 脚本治理: 编码安全工具 / file_utils (_shared/file_utils.py) | 导入依赖 / import_depends |
| 12 | 脚本清单自动生成器 / Script Manifest Generator (generator... | → | D_GOV_SCRIPTS 脚本治理: 目录遍历工具 / yaml_utils (_shared/yaml_utils.py) | 导入依赖 / import_depends |
