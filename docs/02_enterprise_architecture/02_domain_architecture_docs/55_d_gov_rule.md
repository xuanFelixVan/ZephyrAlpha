...
flowchart TD
    ...
    class D_SHARED,D_GOV_SCRIPTS,D_GOVERNANCE,D_INFRA_RECOVERY,D_GOV_AUDIT,D_INFRA_RUNTIME,D_GOV_DRIFT,D_GOV_OPS_RESILIENCE,D_INTEGRATION,D_INTELLIGENCE,D_SECURITY external_prod
```
...
| 6 | 门禁引擎模块集 / Gate Engine Package (gate_engine/__init_... | → | D_GOV_DRIFT 漂移检测: 门禁完整性守卫 / gate_integrity_guard (gate_engine/gate_i... | config_depends / config_depends |
...
| 9 | 规则引擎模块集 / Rule Engine Package (rule_engine/__init_... | → | D_GOV_ENFORCEMENT 规则执行: 规则debt审计器 / rule_debt_auditor (rule_engine/rule_debt... | config_depends / config_depends |
