...
flowchart TD
    ...
    src_zephyr_gov_enforcement_rule_enforcement_can_i_deploy_py["预部署门禁<br/>部署前问一句现在能部署吗。检查四样：别人对我的期<br/>望满足没、版本兼容不、契约一致不、服务健康不。避<br/>免一部署就出事<br/>Can-I-Deploy<br/>Can-I-Deploy pre-deployment gate (GATE-CDC-1)<br/>文件: rule_enforcement/can_i_deploy.py<br/>(生产态 / production)"]
