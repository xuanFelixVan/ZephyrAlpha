...
flowchart TD
    ...
    D_GOV_CODE_QUALITY -->|测试依赖 / test_depends| src_zephyr_gov_enforcement_rule_enforcement_invariants_en_003_contract_compatibility_py
    D_GOV_CODE_QUALITY -->|测试依赖 / test_depends| src_zephyr_gov_enforcement_rule_enforcement_invariants_zero_residue_check_py
    ...
    class D_SHARED,D_GOV_DRIFT,D_GOV_AUDIT,D_GOV_SCRIPTS,D_SECURITY,D_GOVERNANCE,D_GOV_CODE_QUALITY,D_AUTONOMY_CORE,D_INFRA_RUNTIME external_prod
...
| 31 | D_GOV_CODE_QUALITY 代码质量治理: rule_enforcement/test_integration_test_runner.py | → | 集成测试运行器 / Integration Test Runner (rule_enforcemen... | 测试依赖 / test_depends |
| 32 | D_GOV_CODE_QUALITY 代码质量治理: rule_enforcement/test_kiss_enforcer.py | → | KISS 约束执行器 / KISS Enforcer (rule_enforcement/kiss_en... | 测试依赖 / test_depends |
| 33 | D_GOV_CODE_QUALITY 代码质量治理: rule_enforcement/test_secrets_guard.py | → | 密钥守卫 / Secrets Guard (rule_enforcement/secrets_guard.py) | 测试依赖 / test_depends |
| 34 | D_GOV_CODE_QUALITY 代码质量治理: rule_enforcement/test_triple_alignment.py | → | 三方对齐门禁 / Triple Alignment (rule_enforcement/triple_... | 测试依赖 / test_depends |
...
| 36 | D_GOV_ENFORCEMENT 规则执行: test_shadow_canary_deploy.py — Shadow Canary 部署运行器... | → | 预部署门禁 / Can-I-Deploy (rule_enforcement/can_i_deploy.py) | 测试依赖 / test_depends |
