...
flowchart TD
    ...
    class D_SHARED,D_GOV_AUDIT,D_GOV_DRIFT,D_GOVERNANCE,D_GOV_SCRIPTS,D_SECURITY,D_GOV_OPS_RESILIENCE external_prod
...
| 20 | 契约模板管理器 / Contract Template Manager (rule_enforcem... | → | D_SHARED 共享服务: Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 24 | 门禁类型定义 / Gate Types (rule_enforcement/gate_types.py) | → | D_SHARED 共享服务: Schemas (schema/schemas.py) | 导入依赖 / import_depends |
