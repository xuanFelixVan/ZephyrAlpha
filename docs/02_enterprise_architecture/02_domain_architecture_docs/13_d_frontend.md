...
> 展示全部 31 个模块（生产态 13 + 设计态 6 + 原型态 12），标签标注成熟度。
...
graph TD
    ...
    class D_BACKTEST,D_GOVERNANCE,D_EX_CORE,D_GOV_DOCS,D_AUDITTEST external_design
```
...
graph TD
    ...
    classDef external_prod fill:#e8f5e9,stroke:#1b5e20,stroke-width:1px,color:#000
    classDef external_design fill:#fce4ec,stroke:#880e4f,stroke-width:1px,color:#000,stroke-dasharray: 5 5
    class src_zephyr_frontend_services_init_py design
```
...
| 6 | ZephyrAlpha Dashboard · Streamlit 仪表盘 (app.py) | → | D_GOVERNANCE 生命周期管理: SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 7 | ZephyrAlpha Dashboard · Streamlit 仪表盘 (app.py) | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 导入依赖 / import_depends |
| 8 | app_panel · Panel 仪表盘主应用入口（v3.1.0, #A... | → | D_GOVERNANCE 生命周期管理: SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 9 | app_panel · Panel 仪表盘主应用入口（v3.1.0, #A... | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 导入依赖 / import_depends |
| 10 |  | → | D_GOVERNANCE 生命周期管理:  | 导入依赖 / import_depends |
| 11 |  | → | D_GOVERNANCE 生命周期管理:  | 导入依赖 / import_depends |
| 12 |  | → | D_GOV_DOCS 架构文档治理: blueprint.md | runtime / runtime |
| 13 | chart_factory · 图表统一工厂（v3.0.0新增, #ARC... | → | D_SHARED 共享服务: serialization.py —— 统一序列化/反序列化基础设... | 导入依赖 / import_depends |
| 14 | task_progress · 任务进度看板组件（v3.1.0 Panel... | → | D_SHARED 共享服务: constants.py —— 共享枚举 & 常量集中 re-export... | 导入依赖 / import_depends |
| 15 | trade_panel · 实盘交易面板组件（v3.0.0 Panel+H... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
...
> 本域与 8 个外部域直接连接（出边 16 条 + 入边 8 条 = 24 条）。只显示直接连接的域，不展开具体节点。
