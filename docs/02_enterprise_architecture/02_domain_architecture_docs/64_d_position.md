...
| 3 | D_PF_CORE 组合核心: Rebalance调度器 / Rebalance Scheduler (core/rebalance_sch... | → | —事件驱动 / Position Reconciler (position/position_recon... | 导入依赖 / import_depends |
...
| 6 | D_TRADING 交易运营: Pnl计算器 / Pnl Calculator (trading/pnl_calculator.py) | → | —事件驱动 / Position Reconciler (position/position_recon... | 导入依赖 / import_depends |
