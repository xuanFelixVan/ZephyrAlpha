...
> 展示全部 12 个模块（生产态 4 + 设计态 8），含跨域依赖外部节点。节点含成熟度+名称+大白话/简介+文件路径。
...
flowchart TD
    ...
    D_EX_CORE -->|导入依赖 / import_depends| src_zephyr_reporting_realtime_pnl_dashboard_py
    D_TRADING["交易运营<br/>交易运营，负责交易生命周期管理、订单状态和成交处<br/>理<br/>Trading Operations<br/>跨域节点 / cross-domain<br/>(生产态 / production)"]
    D_TRADING -->|导入依赖 / import_depends| src_zephyr_reporting_realtime_pnl_dashboard_py
    D_INFRASTRUCTURE -->|contract / contract| src_zephyr_reporting_realtime_pnl_dashboard_py
    D_INFRASTRUCTURE -->|contract / contract| src_zephyr_reporting_realtime_pnl_dashboard_py
    D_EX_CORE -.->|data / data| src_zephyr_reporting_report_publisher_py
    D_TRADING -->|contract / contract| src_zephyr_reporting_realtime_pnl_dashboard_py
    D_SHARED["共享服务<br/>共享服务，负责跨域共享的工具、协议和基础服务<br/>Shared Services<br/>跨域节点 / cross-domain<br/>(生产态 / production)"]
    D_SHARED -->|导入依赖 / import_depends| src_zephyr_reporting_realtime_pnl_dashboard_py
    ...
    class src_zephyr_reporting_analytics_base_py,src_zephyr_reporting_default_attribution_engine_py,src_zephyr_reporting_default_tca_engine_py,src_zephyr_reporting_realtime_pnl_dashboard_py production
    ...
    class D_INFRASTRUCTURE,D_EX_CORE,D_TRADING,D_SHARED,D_GOV_AUDIT,D_GOVERNANCE external_prod
```

### 运营态的图（仅 design_maturity=production 的模块和域内依赖）

> 仅展示已上线运行的模块（共 4 个），不含跨域外部节点。跨域依赖见下方跨域依赖章节。
...
flowchart TD
    ...
    src_zephyr_reporting_default_tca_engine_py ~~~ src_zephyr_reporting_realtime_pnl_dashboard_py
    src_zephyr_reporting_analytics_base_py["analytics基类<br/>盘后分析层。负责交易执行后的绩效评估与归因分析。<br/>D_REPORTING — Post-Trade Analytics Layer<br/>文件: reporting/analytics_base.py<br/>(生产态 / production)"]
    src_zephyr_reporting_default_tca_engine_py -->|导入依赖 / import_depends| src_zephyr_reporting_analytics_base_py
    src_zephyr_reporting_default_attribution_engine_py -->|导入依赖 / import_depends| src_zephyr_reporting_analytics_base_py
    classDef production fill:#e1f5fe,stroke:#01579b,stroke-width:2px,color:#000
...
|:--:|---------|:--:|---------|---------|
| 1 | analytics基类 / D_REPORTING — Post-Trade Analytics Layer... | → | D_INFRASTRUCTURE 跨层契约基础设施: 执行报告 / execution_report (contracts/execution_report.py) | 导入依赖 / import_depends |
| 2 | analytics基类 / D_REPORTING — Post-Trade Analytics Layer... | → | D_INFRASTRUCTURE 跨层契约基础设施: 成交 / fill (contracts/fill.py) | 导入依赖 / import_depends |
| 3 | analytics基类 / D_REPORTING — Post-Trade Analytics Layer... | → | D_INFRASTRUCTURE 跨层契约基础设施: 订单 / order (contracts/order.py) | 导入依赖 / import_depends |
| 4 | analytics基类 / D_REPORTING — Post-Trade Analytics Layer... | → | D_INFRASTRUCTURE 跨层契约基础设施: 绩效attribution报告 / performance_attribution_report (con... | 导入依赖 / import_depends |
| 5 | 默认attribution引擎 / D_REPORTING — Default Attribution ... | → | D_INFRASTRUCTURE 跨层契约基础设施: 绩效attribution报告 / performance_attribution_report (con... | 导入依赖 / import_depends |
| 6 | 默认tca引擎 / D_REPORTING — Default TCA Engine (reportin... | → | D_INFRASTRUCTURE 跨层契约基础设施: 执行报告 / execution_report (contracts/execution_report.py) | 导入依赖 / import_depends |
| 7 | 默认tca引擎 / D_REPORTING — Default TCA Engine (reportin... | → | D_INFRASTRUCTURE 跨层契约基础设施: 成交 / fill (contracts/fill.py) | 导入依赖 / import_depends |
| 8 | 默认tca引擎 / D_REPORTING — Default TCA Engine (reportin... | → | D_INFRASTRUCTURE 跨层契约基础设施: 订单 / order (contracts/order.py) | 导入依赖 / import_depends |
...
| 2 | D_EX_CORE 执行核心: D_EXECUTION_CORE — Position Tracker (持仓跟踪器) (positi... | → | 实时盈亏仪表盘 / realtime_pnl_dashboard (reporting/realti... | 导入依赖 / import_depends |
| 3 | D_GOVERNANCE 生命周期管理: analytics基类 / Re-export wrapper: analytics_base canonic... | → | analytics基类 / D_REPORTING — Post-Trade Analytics Layer... | 导入依赖 / import_depends |
| 4 | D_GOV_AUDIT 审计追踪: 默认attribution引擎 / Re-export wrapper: default_attribut... | → | 默认attribution引擎 / D_REPORTING — Default Attribution ... | 导入依赖 / import_depends |
| 5 | D_GOV_AUDIT 审计追踪: 默认tca引擎 / Re-export wrapper: default_tca_engine canon... | → | 默认tca引擎 / D_REPORTING — Default TCA Engine (reportin... | 导入依赖 / import_depends |
| 6 | D_INFRASTRUCTURE 跨层契约基础设施: 成交 / fill (contracts/fill.py) | → | 实时盈亏仪表盘 / realtime_pnl_dashboard (reporting/realti... | contract / contract |
| 7 | D_INFRASTRUCTURE 跨层契约基础设施: 持仓 / position (contracts/position.py) | → | 实时盈亏仪表盘 / realtime_pnl_dashboard (reporting/realti... | contract / contract |
| 8 | D_SHARED 共享服务: 错误 / errors (foundation/errors.py) | → | 实时盈亏仪表盘 / realtime_pnl_dashboard (reporting/realti... | 导入依赖 / import_depends |
| 9 | D_TRADING 交易运营: D_TRADING — PnL Calculator (盈亏计算器) (trading/pnl_cal... | → | 实时盈亏仪表盘 / realtime_pnl_dashboard (reporting/realti... | 导入依赖 / import_depends |
| 10 | D_TRADING 交易运营: 风险仪表盘快照 / risk_dashboard_snapshot (risk/risk_dashb... | → | 实时盈亏仪表盘 / realtime_pnl_dashboard (reporting/realti... | contract / contract |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 6 个外部域直接连接（出边 8 条 + 入边 10 条 = 18 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_EX_CORE -->|2条 data / data, 导入依赖 / import_depends| D_REPORTING
    D_GOV_AUDIT -->|2条 导入依赖 / import_depends| D_REPORTING
    D_INFRASTRUCTURE -->|2条 contract / contract| D_REPORTING
