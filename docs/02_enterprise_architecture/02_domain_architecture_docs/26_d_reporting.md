...
> 展示全部 13 个模块（生产态 5 + 设计态 8），含跨域依赖外部节点。节点含成熟度+名称+大白话/简介+文件路径。
...
flowchart TD
    ...
    src_zephyr_reporting_report_version_manager_py["报告版本管理器<br/>报告的报告器，汇总数据生成报告（report version）<br/>⛔ 报告域，设计已就绪，等待开发排期<br/>report_version_manager<br/>文件: reporting/report_version_manager.py<br/>(设计态 / design)"]
    ...
    tests_reporting_test_realtime_pnl_dashboard_py -->|测试依赖 / test_depends| D_SHARED
    src_zephyr_reporting_default_tca_engine_py -->|导入依赖 / import_depends| D_INFRASTRUCTURE
    D_SHARED -.->|导入依赖 / import_depends| src_zephyr_reporting_report_version_manager_py
    ...
    class src_zephyr_reporting_analytics_base_py,src_zephyr_reporting_default_attribution_engine_py,src_zephyr_reporting_default_tca_engine_py,src_zephyr_reporting_realtime_pnl_dashboard_py,tests_reporting_test_realtime_pnl_dashboard_py production
    class src_zephyr_reporting_ashare_performance_audit_py,src_zephyr_reporting_ashare_trade_record_template_py,src_zephyr_reporting_performance_attribution_report_py,src_zephyr_reporting_regulatory_report_generator_py,src_zephyr_reporting_report_publisher_py,src_zephyr_reporting_report_version_manager_py,src_zephyr_reporting_report_watermark_tracker_py,src_zephyr_reporting_risk_report_engine_py design
    class D_INFRASTRUCTURE,D_EX_CORE,D_SHARED,D_TRADING,D_GOVERNANCE,D_GOV_AUDIT external_prod
```
...
| 16 | MOD-RPT-004 Real-time P&L Dashboard 单元测试. (reporting/... | → | D_SHARED 共享服务: 订单枚举 / order_enums (enums/order_enums.py) | 测试依赖 / test_depends |
| 17 | MOD-RPT-004 Real-time P&L Dashboard 单元测试. (reporting/... | → | D_SHARED 共享服务: 风险仪表盘快照 / Backward-compat shim — canonical locati... | 测试依赖 / test_depends |
| 18 | 实时盈亏仪表盘 / realtime_pnl_dashboard (reporting/realti... | → | D_TRADING 交易运营: D_TRADING — PnL Calculator (盈亏计算器) (trading/pnl_cal... | 导入依赖 / import_depends |
...
| 5 | D_SHARED 共享服务: 错误 / errors (foundation/errors.py) | → | 报告版本管理器 / report_version_manager (reporting/report... | 导入依赖 / import_depends |
