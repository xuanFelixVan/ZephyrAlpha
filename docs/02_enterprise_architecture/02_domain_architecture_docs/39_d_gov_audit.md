...
graph TD
    ...
    class D_INFRA_RUNTIME,D_FBL_DIAGNOSERS,D_GOV_OPS_RESILIENCE,D_FEEDBACK_LOOP,D_GOV_DRIFT,D_AUTONOMY_CORE,D_FBL_DETECTORS,D_GOVERNANCE external_prod
```
...
graph TD
    ...
    class D_SHARED,D_GOV_DRIFT,D_SECURITY,D_GOVERNANCE,D_FEEDBACK_LOOP,D_GOV_REPAIR,D_GOV_OPS_RESILIENCE external_prod
...
| 40 | D_GOVERNANCE 生命周期管理: test_integrity_root.py | → | audit-trail.integrity — MOD-INF-020 · 密码学.... | 测试依赖 / test_depends |
...
| 90 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | finding_ingest.py | 导入依赖 / import_depends |
...
> 本域与 27 个外部域直接连接（出边 270 条 + 入边 143 条 = 413 条）。只显示直接连接的域，不展开具体节点。
