...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_OPS_RESILIENCE,D_GOV_DRIFT,D_TRADING,D_INFRA_RECOVERY,D_SECURITY,D_INFRA_RUNTIME external_prod
...
graph TD
    ...
    class D_GOVERNANCE,D_SHARED,D_GOV_ENFORCEMENT,D_SECURITY,D_GOV_CODE_QUALITY,D_GOV_OPS_RESILIENCE,D_INFRA_RECOVERY,D_INTEGRATION,D_INFRA_RUNTIME external_prod
...
graph TD
    ...
    class D_GOV_CODE_QUALITY,D_SHARED,D_GOV_DRIFT,D_GOVERNANCE,D_SECURITY,D_GOV_ENFORCEMENT,D_FEEDBACK_LOOP,D_GOV_OPS_RESILIENCE,D_TRADING,D_INFRA_RECOVERY external_prod
...
| 68 | remediation_progress_reconciler.py — 治本进度.... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
...
> 本域与 19 个外部域直接连接（出边 72 条 + 入边 57 条 = 129 条）。只显示直接连接的域，不展开具体节点。
