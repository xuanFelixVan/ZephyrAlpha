...
graph TD
    ...
    class D_GOV_OPS_RESILIENCE,D_SHARED,D_GOVERNANCE,D_FEEDBACK_LOOP,D_TRADING,D_INFRA_RECOVERY,D_SECURITY,D_INFRA_RUNTIME external_prod
    class D_GOV_DRIFT,D_GOV_ENFORCEMENT,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_GOV_CODE_QUALITY,D_SHARED,D_GOV_ENFORCEMENT,D_GOVERNANCE,D_SECURITY,D_GOV_RULE,D_GOV_OPS_RESILIENCE,D_INFRA_RECOVERY,D_INFRA_RUNTIME,D_INTEGRATION external_prod
...
graph TD
    ...
    D_GOV_DOCS["(设计态 / design) D_GOV_DOCS"]
    D_GOV_DOCS -.->|runtime / runtime| src_zephyr_governance_semantic_audit_self_health_py
    ...
    class D_SHARED,D_GOV_DOCS,D_INTEGRATION external_design
```
...
graph TD
    ...
    class D_GOV_CODE_QUALITY,D_SHARED,D_FEEDBACK_LOOP,D_GOVERNANCE,D_GOV_ENFORCEMENT,D_GOV_DRIFT,D_TRADING,D_SECURITY,D_GOV_OPS_RESILIENCE,D_INFRA_RECOVERY,D_INFRA_RUNTIME external_prod
...
| 10 | D_GOV_DOCS 架构文档治理: blueprint.md | → | 7 SLI + 5 容量 SLI + 退化检测。定时自检,输出 HE... | runtime / runtime |
...
| 40 | D_INFRASTRUCTURE 跨层契约基础设施: backup_reconciler.py — 灾备备份系统事件触发器.... | → | reconciliation_registry.py — GitCommitGateway ... | 导入依赖 / import_depends |
...
> 本域与 19 个外部域直接连接（出边 75 条 + 入边 58 条 = 133 条）。只显示直接连接的域，不展开具体节点。
