...
graph TD
    ...
    class D_GOV_DOCS,D_GOV_CODE_QUALITY,D_GOV_DRIFT,D_GOV_SCRIPTS,D_INFRA_RECOVERY external_design
```
...
graph TD
    ...
    class D_SHARED,D_FEEDBACK_LOOP,D_GOV_OPS_RESILIENCE,D_SECURITY,D_TRADING,D_INFRA_RECOVERY,D_GOV_DRIFT,D_INFRA_RUNTIME external_prod
...
graph TD
    ...
    class D_GOVERNANCE,D_SHARED,D_SECURITY,D_GOV_ENFORCEMENT,D_INFRA_RUNTIME,D_INFRA_RECOVERY,D_INTEGRATION,D_GOV_OPS_RESILIENCE external_prod
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_ENFORCEMENT,D_GOV_DRIFT,D_SECURITY,D_GOV_RULE,D_INFRA_RUNTIME,D_INFRA_RECOVERY,D_GOV_OPS_RESILIENCE,D_INTEGRATION external_prod
...
| 70 | SnapshotManager — Event Sourcing 快照管理（DW-... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 71 | SnapshotManager — Event Sourcing 快照管理（DW-... | → | D_SHARED 共享服务: serialization.py —— 统一序列化/反序列化基础设... | 导入依赖 / import_depends |
| 72 | 收集各阶段审计结果，去重合并排序输出。 (issue_a... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 73 | Stage 7 自愈闭环 — 修复->自测->回滚. (self_hea... | → | D_SHARED 共享服务: yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
...
> 本域与 19 个外部域直接连接（出边 73 条 + 入边 55 条 = 128 条）。只显示直接连接的域，不展开具体节点。
