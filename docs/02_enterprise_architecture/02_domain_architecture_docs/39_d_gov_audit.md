...
| 跨域出边 | 267 | Cross-domain Outgoing | 267 |
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_DRIFT,D_SECURITY,D_GOV_REPAIR,D_INFRA_RUNTIME,D_GOV_RULE external_prod
    class D_COMPLIANCE,D_INFRA_A2A,D_INFRA_RECOVERY,D_AUTONOMY_CORE external_design
```
...
graph TD
    ...
    src_zephyr_gov_audit_pipeline_runner_py -->|导入依赖 / import_depends| D_INTEGRATION
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_OPS_RESILIENCE,D_INTEGRATION,D_GOV_REPAIR,D_GOV_DRIFT,D_INFRA_RECOVERY,D_TRADING external_prod
    class D_FEEDBACK_LOOP,D_GOV_ENFORCEMENT external_design
```
...
graph TD
    ...
    class D_INTEGRATION,D_SHARED,D_SECURITY,D_GOV_RULE,D_GOV_REPAIR,D_AUTONOMY_CORE,D_GOV_DRIFT,D_GOV_OPS_RESILIENCE external_prod
    class D_GOVERNANCE,D_REPORTING,D_INFRA_A2A,D_GOV_ENFORCEMENT external_design
```
...
graph TD
    ...
    class D_GOVERNANCE,D_GOV_DRIFT,D_GOV_RULE,D_FBL_DIAGNOSERS,D_FBL_VERIFICATION,D_SHARED,D_GOV_REPAIR,D_GOV_OPS_RESILIENCE external_prod
...
graph TD
    ...
    class D_FBL_VERIFICATION,D_FBL_DIAGNOSERS,D_GOV_DRIFT,D_SECURITY,D_GOV_RULE,D_GOV_ENFORCEMENT,D_SHARED external_prod
```
...
graph TD
    ...
    class D_GOV_DRIFT,D_FBL_DIAGNOSERS,D_FBL_DETECTORS,D_FBL_VERIFICATION external_prod
```
...
graph TD
    ...
    class D_GOVERNANCE,D_FBL_DIAGNOSERS,D_GOV_DRIFT,D_GOV_RULE,D_FBL_VERIFICATION,D_FEEDBACK_LOOP,D_INFRA_RUNTIME,D_AUTONOMY_CORE external_prod
```
...
graph TD
    ...
    class D_GOV_DRIFT,D_INFRA_RUNTIME,D_GOV_OPS_RESILIENCE,D_INFRA_A2A,D_SHARED external_prod
```
...
graph TD
    ...
    class D_ORCHESTRATOR,D_FEEDBACK_LOOP,D_GOV_DRIFT,D_AUTONOMY_CORE,D_FBL_DIAGNOSERS,D_GOV_OPS_RESILIENCE,D_GOVERNANCE,D_FBL_DETECTORS external_prod
```
...
graph TD
    ...
    class D_GOVERNANCE,D_SHARED,D_SECURITY,D_GOV_OPS_RESILIENCE,D_GOV_DRIFT,D_GOV_REPAIR,D_AUTONOMY_CORE external_prod
...
graph TD
    ...
    class D_SHARED,D_FBL_VERIFICATION,D_FBL_DIAGNOSERS,D_GOV_DRIFT,D_FBL_DETECTORS,D_INFRA_RUNTIME,D_GOV_REPAIR external_prod
...
| 249 | test_audit_red_blue_e2e.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 250 | F18 治理脚本系统自动化测试. (test_f18_automatio... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 251 | F18 红蓝极限对抗测试. (test_f18_redblue.py) | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 252 | F21 自动关闭测试 — DM-201250 (test_f21_auto_sh... | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 测试依赖 / test_depends |
| 253 | F21 自动关闭测试 — DM-201250 (test_f21_auto_sh... | → | D_SHARED 共享服务: health.py —— ZephyrAlpha 聚合健康检查 (health.py) | 测试依赖 / test_depends |
| 254 | F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | D_SHARED 共享服务: health.py —— ZephyrAlpha 聚合健康检查 (health.py) | 测试依赖 / test_depends |
| 255 | F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | D_SHARED 共享服务: CT-HEALTH-001: System-wide Health Discovery Reg... | 测试依赖 / test_depends |
| 256 | F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | D_SHARED 共享服务: Healthcheck Service — 运行时健康检查服务。 (he... | 测试依赖 / test_depends |
| 257 | F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | D_SHARED 共享服务: longevity_monitor.py | 测试依赖 / test_depends |
| 258 | F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | D_SHARED 共享服务: Autonomy Monitor — AI 自主等级监控与降级。 (au... | 测试依赖 / test_depends |
| 259 | F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | D_SHARED 共享服务: metrics.py —— 轻量级 Metrics 收集基础设施（Ph... | 测试依赖 / test_depends |
| 260 | F21 事件启动测试 — DM-201250 (test_f21_event_d... | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 测试依赖 / test_depends |
| 261 | F21 事件启动测试 — DM-201250 (test_f21_event_d... | → | D_SHARED 共享服务: health.py —— ZephyrAlpha 聚合健康检查 (health.py) | 测试依赖 / test_depends |
| 262 | F21 事件启动测试 — DM-201250 (test_f21_event_d... | → | D_SHARED 共享服务: metrics.py —— 轻量级 Metrics 收集基础设施（Ph... | 测试依赖 / test_depends |
| 263 | F5 端到端集成测试 — boot→run→shutdown→resta... | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 测试依赖 / test_depends |
| 264 | test_f5_event_startup.py | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 测试依赖 / test_depends |
| 265 | F21 自动运行测试 — DM-201250 (test_f21_auto_ru... | → | D_TRADING 交易运营: __init__.py | 测试依赖 / test_depends |
| 266 | F21 自动关闭测试 — DM-201250 (test_f21_auto_sh... | → | D_TRADING 交易运营: __init__.py | 测试依赖 / test_depends |
| 267 | F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | D_TRADING 交易运营: __init__.py | 测试依赖 / test_depends |
...
| 23 | D_GOVERNANCE 生命周期管理: GitCommitGateway — 全项目唯一合法 git commit .... | → | reconciliation_registry.py — GitCommitGateway ... | 导入依赖 / import_depends |
...
> 本域与 26 个外部域直接连接（出边 267 条 + 入边 141 条 = 408 条）。只显示直接连接的域，不展开具体节点。
