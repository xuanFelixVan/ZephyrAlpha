...
graph TD
    ...
    class D_GOVERNANCE,D_GOV_ENFORCEMENT,D_TRADING,D_INFRASTRUCTURE,D_GOV_DOCS,D_COMPLIANCE,D_INTEGRATION,D_DATA external_design
```
...
graph TD
    ...
    class D_GOV_DRIFT,D_FBL_VERIFICATION,D_FBL_DIAGNOSERS,D_GOV_RULE,D_SECURITY external_prod
...
graph TD
    ...
    class D_FEEDBACK_LOOP,D_GOV_DRIFT,D_FBL_VERIFICATION,D_FBL_DIAGNOSERS external_prod
```
...
graph TD
    ...
    class D_FBL_DIAGNOSERS,D_GOV_DRIFT,D_FBL_DETECTORS,D_FEEDBACK_LOOP,D_FBL_VERIFICATION external_prod
```
...
graph TD
    ...
    class D_FBL_DIAGNOSERS,D_FBL_DETECTORS,D_FBL_VERIFICATION,D_GOV_DRIFT,D_SECURITY,D_GOV_RULE,D_AUTONOMY_CORE external_prod
```
...
graph TD
    ...
    class D_FBL_DETECTORS,D_GOVERNANCE,D_SHARED,D_GOV_OPS_RESILIENCE,D_INTELLIGENCE,D_INFRA_RUNTIME,D_TRADING external_prod
```
...
graph TD
    ...
    class D_FBL_DETECTORS,D_FBL_DIAGNOSERS,D_SECURITY,D_FEEDBACK_LOOP,D_GOV_DRIFT,D_GOV_CODE_QUALITY,D_GOVERNANCE,D_GOV_OPS_RESILIENCE external_prod
```
...
graph TD
    ...
    class D_SHARED,D_TRADING,D_GOV_DRIFT,D_INFRA_RUNTIME external_prod
    class D_GOVERNANCE,D_REPORTING,D_GOV_DOCS,D_SECURITY,D_COMPLIANCE external_design
```
...
| 5 | test_emergent_behavior_detector.py | → | D_FBL_DETECTORS: Emergent Behavior Detector — v0.38.0 R473 (eme... | 测试依赖 / test_depends |
| 6 | test_intermittent_failure_pattern.py | → | D_FBL_DETECTORS: Intermittent Failure Pattern Detector — v0.40.... | 测试依赖 / test_depends |
| 7 | test_traffic_replay_validator.py | → | D_FBL_DETECTORS: Traffic Replay Validator — v0.14.0 R202 (traff... | 测试依赖 / test_depends |
| 8 | test_concept_drift.py | → | D_FBL_DETECTORS: Concept Drift Detector — v0.5.0 R42 (concept_d... | 测试依赖 / test_depends |
| 9 | test_version_migrator.py | → | D_FBL_DETECTORS: Version Migrator — v0.12.0 R169 (version_migra... | 测试依赖 / test_depends |
| 10 | test_flag_lifecycle.py | → | D_FBL_DETECTORS: Flag Lifecycle Detector — v0.13.0 R180 (flag_l... | 测试依赖 / test_depends |
| 11 | test_openfeature.py | → | D_FBL_DETECTORS: OpenFeature Integration — v0.13.0 R181 (openfe... | 测试依赖 / test_depends |
| 12 | test_self_audit.py | → | D_FBL_DETECTORS: Self Audit — v0.13.0 R183 (self_audit.py) | 测试依赖 / test_depends |
| 13 | test_self_diagnosis_data_leak_detector.py | → | D_FBL_DETECTORS: R530: SelfDiagnosisDataLeakDetector (self_diagn... | 测试依赖 / test_depends |
| 14 | test_self_ha.py | → | D_FBL_DETECTORS: Self HA — v0.13.0 R173 (self_ha.py) | 测试依赖 / test_depends |
| 15 | test_amplification_guard.py | → | D_FBL_DIAGNOSERS: Amplification Guard — v0.10.0 R134 (amplificat... | 测试依赖 / test_depends |
| 16 | test_api_dependency_metrics.py | → | D_FBL_DIAGNOSERS: API Dependency Metrics — v0.17.0+ R237-R242 (a... | 测试依赖 / test_depends |
| 17 | test_burn_rate_alerter.py | → | D_FBL_DIAGNOSERS: Burn Rate Alerter — v0.14.0 R200 (burn_rate_al... | 测试依赖 / test_depends |
| 18 | test_burnout_alarm.py | → | D_FBL_DIAGNOSERS: Burnout Alarm — v0.8.0 R100 (burnout_alarm.py) | 测试依赖 / test_depends |
| 19 | test_causal_inference_engine.py | → | D_FBL_DIAGNOSERS: Causal Inference Engine — v0.3.0 R5-R7 (causal... | 测试依赖 / test_depends |
| 20 | test_cognitive_load_budget.py | → | D_FBL_DIAGNOSERS: Cognitive Load Budget — v0.16.0 R223 (cognitiv... | 测试依赖 / test_depends |
| 21 | test_diagnosis_kpi.py | → | D_FBL_DIAGNOSERS: Diagnosis KPI — v0.9.0 R116 (diagnosis_kpi.py) | 测试依赖 / test_depends |
| 22 | test_global_health_map.py | → | D_FBL_DIAGNOSERS: Global Health Map — v0.8.0 R103 (global_health... | 测试依赖 / test_depends |
| 23 | test_human_anomaly_flood_detector.py | → | D_FBL_DIAGNOSERS: Human Anomaly Flood Detector — v0.40.0 R500 (h... | 测试依赖 / test_depends |
| 24 | test_interactive_diagnosis.py | → | D_FBL_DIAGNOSERS: Interactive Diagnosis — v0.7.0 R80 (interactiv... | 测试依赖 / test_depends |
| 25 | test_latency_slo.py | → | D_FBL_DIAGNOSERS: Latency SLO Monitor — v0.14.0 R192 (latency_sl... | 测试依赖 / test_depends |
| 26 | test_mtti_tracker.py | → | D_FBL_DIAGNOSERS: MTTI Tracker — v0.16.0 R221 (mtti_tracker.py) | 测试依赖 / test_depends |
| 27 | test_regime_gain_scheduling.py | → | D_FBL_DIAGNOSERS: Regime Gain Scheduling — v0.37.0 R453 (regime_... | 测试依赖 / test_depends |
| 28 | test_socratic_questions.py | → | D_FBL_DIAGNOSERS: Socratic Questions — v0.7.0 R81 (socratic_ques... | 测试依赖 / test_depends |
| 29 | test_statistical_hygiene_auditor.py | → | D_FBL_DIAGNOSERS: Statistical Hygiene Auditor — v0.38.0 R476 (st... | 测试依赖 / test_depends |
| 30 | test_toil_quantification.py | → | D_FBL_DIAGNOSERS: Toil Quantification — v0.37.0 R457 (toil_quant... | 测试依赖 / test_depends |
| 31 | test_tone_adapter.py | → | D_FBL_DIAGNOSERS: Tone Adapter — v0.9.0 R127 (tone_adapter.py) | 测试依赖 / test_depends |
| 32 | test_tone_adapter_v2.py | → | D_FBL_DIAGNOSERS: Tone Adapter v2 — v0.10.0 R141 (tone_adapter_v... | 测试依赖 / test_depends |
| 33 | test_value_added_baseline.py | → | D_FBL_DIAGNOSERS: Value Added Baseline — v0.10.0 R138 (value_add... | 测试依赖 / test_depends |
| 34 | test_zombie_fle_detector.py | → | D_FBL_DIAGNOSERS: Zombie FLE Detector — v0.16.0 R222 (zombie_fle... | 测试依赖 / test_depends |
| 35 | test_self_benchmark.py | → | D_FBL_DIAGNOSERS: Self Benchmark — v0.9.0 R115 (self_benchmark.py) | 测试依赖 / test_depends |
| 36 | test_self_bottleneck_detector.py | → | D_FBL_DIAGNOSERS: Self-Bottleneck Detector — v0.38.0 R479 (self_... | 测试依赖 / test_depends |
| 37 | test_self_health_monitor.py | → | D_FBL_DIAGNOSERS: Self Health Monitor — v0.4.0 R29 (self_health_... | 测试依赖 / test_depends |
| 38 | test_self_llm_observability.py | → | D_FBL_DIAGNOSERS: Self LLM Observability — v0.12.0 R160 (self_ll... | 测试依赖 / test_depends |
| 39 | test_ab_test.py | → | D_FBL_VERIFICATION 反馈验证: A/B Test Verifier — v0.9.0 R117 (ab_test.py) | 测试依赖 / test_depends |
| 40 | test_build_reproducibility_verifier.py | → | D_FBL_VERIFICATION 反馈验证: Build Reproducibility Verifier — v0.38.0 R484 ... | 测试依赖 / test_depends |
| 41 | test_build_reproducibility_verifier_v2.py | → | D_FBL_VERIFICATION 反馈验证: Build Reproducibility Verifier — v0.38.0 R484 ... | 测试依赖 / test_depends |
| 42 | test_pre_flight_simulator.py | → | D_FBL_VERIFICATION 反馈验证: Pre-Flight Simulator — v0.12.0 R169b (pre_flig... | 测试依赖 / test_depends |
| 43 | test_preventive_repair.py | → | D_FBL_VERIFICATION 反馈验证: Preventive Repair — v0.6.0 R69 (preventive_rep... | 测试依赖 / test_depends |
| 44 | test_sim2real_calibration.py | → | D_FBL_VERIFICATION 反馈验证: Sim2Real Calibration — v0.6.0 R56 (sim2real_ca... | 测试依赖 / test_depends |
| 45 | test_toctou_revalidation.py | → | D_FBL_VERIFICATION 反馈验证: TOCTOU Revalidation — v0.37.0 R458 (toctou_rev... | 测试依赖 / test_depends |
| 46 | test_verification_engine.py | → | D_FBL_VERIFICATION 反馈验证: verification_engine.py | 测试依赖 / test_depends |
| 47 | feedback_bridge.py | → | D_FEEDBACK_LOOP 反馈循环引擎: Feedback Loop Engine — MOD-FEEDBACK_LOOP. (__i... | 导入依赖 / import_depends |
| 48 | test_audit_spec_auditor.py | → | D_FEEDBACK_LOOP 反馈循环引擎: protocols.py | 测试依赖 / test_depends |
| 49 | test_crypto_bootstrap.py | → | D_FEEDBACK_LOOP 反馈循环引擎: Cryptographic Bootstrap — v0.15.0 R204 (crypto... | 测试依赖 / test_depends |
| 50 | test_deterministic_replay.py | → | D_FEEDBACK_LOOP 反馈循环引擎: Deterministic Replay — v0.15.0 R206 (determini... | 测试依赖 / test_depends |
| 51 | test_point_in_time_reconstructor.py | → | D_FEEDBACK_LOOP 反馈循环引擎: Point-in-Time Reconstructor — v0.37.0 R465 (po... | 测试依赖 / test_depends |
| 52 | test_serialization_format_tracker.py | → | D_FEEDBACK_LOOP 反馈循环引擎: Serialization Format Tracker — v0.39.0 R488 (s... | 测试依赖 / test_depends |
| 53 | test_sub_agent_collusion.py | → | D_FEEDBACK_LOOP 反馈循环引擎: Sub-Agent Collusion Detector — v0.15.0 R213 (s... | 测试依赖 / test_depends |
| 54 | test_schema_evolution_root.py | → | D_FEEDBACK_LOOP 反馈循环引擎: Schema Evolution — v0.9.0 R111 (schema_evoluti... | 测试依赖 / test_depends |
| 55 | test_self_api_throttle_defense.py | → | D_FEEDBACK_LOOP 反馈循环引擎: Self API Throttle Defense — v0.39.0 R491 (self... | 测试依赖 / test_depends |
| 56 | test_self_diagnosis.py | → | D_FEEDBACK_LOOP 反馈循环引擎: self_diagnosis.py — 自我诊断 (DD120, TASK-020)... | 测试依赖 / test_depends |
...
| 78 | __init__.py | → | D_GOVERNANCE 生命周期管理: AtomicTransactionManager — SQLite + 文件系统的... | config_depends / config_depends |
...
| 83 | __init__.py | → | D_GOVERNANCE 生命周期管理: Escalation Adapter — MOD-INF-022 统一集成入口.... | config_depends / config_depends |
| 84 | Re-export wrapper: true source is zephyr.pf_cor... | → | D_GOVERNANCE 生命周期管理: D_PORTFOLIO_CORE — StrategyBase + StrategyMeta... | config_depends / config_depends |
...
> 本域与 27 个外部域直接连接（出边 314 条 + 入边 111 条 = 425 条）。只显示直接连接的域，不展开具体节点。
