...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_CODE_QUALITY,D_SECURITY,D_GOV_ENFORCEMENT,D_GOV_OPS_RESILIENCE,D_INFRA_RECOVERY,D_INTEGRATION,D_INFRA_RUNTIME external_prod
...
graph TD
    ...
    D_GOV_DOCS["(设计态 / design) D_GOV_DOCS"]
    D_GOV_DOCS -.->|runtime / runtime| src_zephyr_governance_semantic_audit_reference_extractor_py
...
graph TD
    ...
    class D_GOV_CODE_QUALITY,D_FEEDBACK_LOOP,D_SHARED,D_GOVERNANCE,D_GOV_ENFORCEMENT,D_SECURITY,D_GOV_DRIFT,D_TRADING,D_GOV_OPS_RESILIENCE,D_INFRA_RECOVERY external_prod
...
graph TD
    ...
    class D_GOV_DRIFT,D_SHARED,D_GOVERNANCE,D_GOV_OPS_RESILIENCE,D_GOV_ENFORCEMENT,D_SECURITY,D_INFRA_RUNTIME external_prod
...
| 69 | remediation_progress_reconciler.py — 治本进度.... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
...
| 10 | D_GOV_DOCS 架构文档治理: blueprint.md | → | AST 解析文件，提取 9 个维度的引用信息。 (refere... | runtime / runtime |
...
> 本域与 19 个外部域直接连接（出边 73 条 + 入边 58 条 = 131 条）。只显示直接连接的域，不展开具体节点。
