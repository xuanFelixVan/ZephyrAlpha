...
graph TD
    ...
    class D_REPORTING,D_GOV_DOCS,D_GOV_ENFORCEMENT,D_GOV_SCRIPTS,D_INFRASTRUCTURE external_design
```
...
graph TD
    ...
    class D_SHARED,D_SECURITY,D_GOV_DRIFT,D_GOVERNANCE,D_FEEDBACK_LOOP,D_GOV_RULE,D_GOV_REPAIR,D_TRADING,D_GOV_OPS_RESILIENCE external_prod
...
| 67 | D_GOV_DOCS 架构文档治理: blueprint.md | → | 三元对齐检测：蓝图声明清单 vs 磁盘实际文件 vs i... | runtime / runtime |
...
> 本域与 27 个外部域直接连接（出边 269 条 + 入边 145 条 = 414 条）。只显示直接连接的域，不展开具体节点。
