...
graph TD
    ...
    class D_INTEGRATION,D_SHARED,D_FEEDBACK_LOOP,D_GOV_OPS_RESILIENCE,D_TRADING,D_INFRA_RECOVERY,D_GOV_DRIFT,D_SECURITY,D_INFRA_RUNTIME external_prod
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_INTEGRATION,D_SECURITY,D_GOV_ENFORCEMENT,D_INFRA_RUNTIME,D_INFRA_RECOVERY,D_GOV_OPS_RESILIENCE external_prod
...
graph TD
    ...
    class D_GOV_DRIFT,D_SHARED,D_GOV_ENFORCEMENT,D_SECURITY,D_GOVERNANCE,D_INFRA_RUNTIME,D_INFRA_RECOVERY,D_GOV_OPS_RESILIENCE,D_TRADING external_prod
...
| 30 | finding_model.py | → | D_INTEGRATION 管线路由: base_config.py | 导入依赖 / import_depends |
| 31 | pipeline_runner.py | → | D_INTEGRATION 管线路由: base_config.py | 导入依赖 / import_depends |
| 32 | text_to_finding_adapter.py | → | D_INTEGRATION 管线路由: base_config.py | 导入依赖 / import_depends |
| 33 | Re-export wrapper: default_attribution_engine c... | → | D_REPORTING 报告: D_REPORTING — Default Attribution Engine (defa... | 导入依赖 / import_depends |
| 34 | Re-export wrapper: default_tca_engine canonical... | → | D_REPORTING 报告: D_REPORTING — Default TCA Engine (default_tca_... | 导入依赖 / import_depends |
| 35 | cli.py | → | D_SECURITY 对抗验证: judge.py | 导入依赖 / import_depends |
| 36 | cli.py | → | D_SECURITY 对抗验证: validator.py | 导入依赖 / import_depends |
| 37 | reconciliation_registry.py — GitCommitGateway ... | → | D_SECURITY 对抗验证: Session 级并发协调模块（P2-SES 落地）。 (sessio... | 导入依赖 / import_depends |
| 38 | [INVARIANTS] 仅接受depgraph.backup.*路径; 回滚.... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
