...
graph TD
    ...
    class D_SHARED,D_FEEDBACK_LOOP,D_GOV_DRIFT,D_SECURITY,D_GOV_OPS_RESILIENCE,D_TRADING,D_INFRA_RECOVERY,D_INFRA_RUNTIME external_prod
    class D_GOVERNANCE,D_GOV_ENFORCEMENT,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_SECURITY,D_GOV_ENFORCEMENT,D_AUTONOMY_CORE,D_INFRA_RECOVERY,D_GOV_OPS_RESILIENCE,D_INFRA_RUNTIME,D_INTEGRATION external_prod
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_DRIFT,D_GOV_ENFORCEMENT,D_SECURITY,D_AUTONOMY_CORE,D_GOV_RULE,D_INFRA_RECOVERY,D_GOV_OPS_RESILIENCE,D_INFRA_RUNTIME external_prod
...
| 66 | reconciliation_registry.py — GitCommitGateway ... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
...
| 12 | D_GOV_DRIFT 漂移检测: blueprint.md | → | blueprint.md | runtime / runtime |
| 13 | D_GOV_DRIFT 漂移检测: Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | → | finding_ingest.py | 导入依赖 / import_depends |
| 14 | D_GOV_DRIFT 漂移检测: Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | → | finding_model.py | 导入依赖 / import_depends |
| 15 | D_GOV_DRIFT 漂移检测: 真源优先级裁决器（Truth Source Validator） (tru... | → | bridge.py | 导入依赖 / import_depends |
| 16 | D_GOV_DRIFT 漂移检测: integrity.py | → | audit-trail.merkle_hourly — MOD-INF-020 · 每.... | 导入依赖 / import_depends |
| 17 | D_GOV_DRIFT 漂移检测: integrity.py | → | models.py | 导入依赖 / import_depends |
| 18 | D_GOV_DRIFT 漂移检测: integrity.py | → | trust_bridge.py | 导入依赖 / import_depends |
...
> 本域与 18 个外部域直接连接（出边 71 条 + 入边 54 条 = 125 条）。只显示直接连接的域，不展开具体节点。
