...
graph TD
    ...
    class D_GOVERNANCE,D_FEEDBACK_LOOP,D_FBL_DETECTORS,D_FBL_DIAGNOSERS,D_COMPLIANCE,D_FBL_VERIFICATION,D_INTEGRATION,D_GOV_AUDIT,D_SECURITY,D_GOV_DRIFT,D_FRONTEND,D_INFRASTRUCTURE external_design
```
...
| 27 | blueprint.md | → | D_INTEGRATION 管线路由: MCP 错误码集中注册（MOD-INF-013 §3.4）。 (erro... | runtime / runtime |
| 28 | blueprint.md | → | D_INTEGRATION 管线路由: MCP 错误码集中注册（MOD-INF-013 §3.4）。 (erro... | contract / contract |
...
> 本域与 15 个外部域直接连接（出边 37 条 + 入边 12 条 = 49 条）。只显示直接连接的域，不展开具体节点。
