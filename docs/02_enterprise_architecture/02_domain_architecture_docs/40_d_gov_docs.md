...
graph TD
    ...
    class D_GOVERNANCE,D_GOV_AUDIT,D_SECURITY,D_FBL_DIAGNOSERS,D_GOV_DRIFT external_design
```
...
| 2 | blueprint.md | → | D_FBL_DIAGNOSERS 反馈诊断器: __init__.py | runtime / runtime |
...
| 16 | blueprint.md | → | D_INTEGRATION 管线路由: MCP 错误码集中注册（MOD-INF-013 §3.4）。 (erro... | runtime / runtime |
| 17 | blueprint.md | → | D_INTEGRATION 管线路由: MCP 错误码集中注册（MOD-INF-013 §3.4）。 (erro... | contract / contract |
...
| 20 | D_ML_TRAIN 训练: D_ML_TRAIN — ML Training Base (trainer_base.py) | → | blueprint.md | data / data |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 10 个外部域直接连接（出边 23 条 + 入边 20 条 = 43 条）。只显示直接连接的域，不展开具体节点。
