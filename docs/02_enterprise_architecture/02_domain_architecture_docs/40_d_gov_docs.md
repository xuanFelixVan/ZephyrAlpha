...
graph TD
    ...
    class D_GOVERNANCE,D_FEEDBACK_LOOP,D_FBL_DIAGNOSERS,D_COMPLIANCE,D_GOV_CODE_QUALITY,D_INTEGRATION,D_GOV_AUDIT,D_SECURITY,D_GOV_DRIFT,D_FRONTEND,D_ML_TRAIN external_design
```
...
| 2 | blueprint.md | → | D_FBL_DETECTORS 反馈检测器: Flapping Detector — v0.40.0 R494 (flapping_det... | runtime / runtime |
| 3 | blueprint.md | → | D_FBL_DIAGNOSERS 反馈诊断器: __init__.py | runtime / runtime |
| 4 | blueprint.md | → | D_FEEDBACK_LOOP 反馈循环引擎: feedback-loop.collectors — auto-generated pack... | runtime / runtime |
...
| 8 | blueprint.md | → | D_FEEDBACK_LOOP 反馈循环引擎: feedback-loop.verifiers — auto-generated packa... | runtime / runtime |
...
| 32 | blueprint.md | → | D_INTEGRATION 管线路由: Handoff 自动加载器——从 handoff 包恢复 AI sess... | runtime / runtime |
| 33 | blueprint.md | → | D_INTEGRATION 管线路由: Handoff 自动加载器——从 handoff 包恢复 AI sess... | contract / contract |
...
| 10 | D_ML_TRAIN 训练: D_ML_TRAIN — ML Training Base (trainer_base.py) | → | blueprint.md | data / data |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 13 个外部域直接连接（出边 41 条 + 入边 10 条 = 51 条）。只显示直接连接的域，不展开具体节点。
