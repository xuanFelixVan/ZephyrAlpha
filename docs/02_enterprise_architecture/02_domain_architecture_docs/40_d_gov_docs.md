...
graph TD
    ...
    docs_03_modules_domain_governance_code_dedup_engine_blueprint_md -.->|data / data| D_GOVERNANCE
...
> 仅展示蓝图阶段、代码未写的设计态模块（共 26 个，29 条域内依赖）。
...
|:--:|---------|:--:|---------|---------|
| 1 | blueprint.md | → | D_FEEDBACK_LOOP 反馈循环引擎: FLE->Orc 告警分派器 — dispatch() 生产者 (alert... | runtime / runtime |
| 2 | blueprint.md | → | D_FEEDBACK_LOOP 反馈循环引擎: FLE->Orc 告警分派器 — dispatch() 生产者 (alert... | runtime / runtime |
| 3 | blueprint.md | → | D_FEEDBACK_LOOP 反馈循环引擎: FLE->Orc 告警分派器 — dispatch() 生产者 (alert... | runtime / runtime |
| 4 | blueprint.md | → | D_GOVERNANCE 生命周期管理: depgraph_reader.py — 依赖图数据库查询工具模块 ... | data / data |
| 5 | blueprint.md | → | D_GOVERNANCE 生命周期管理: G-CT-008 契约：A2A -> Audit 审计 Agent 间通信. ... | runtime / runtime |
| 6 | blueprint.md | → | D_GOVERNANCE 生命周期管理: post_sync_validator — post_sync_standard 命令.... | data / data |
| 7 | blueprint.md | → | D_GOVERNANCE 生命周期管理: depgraph_reader.py — 依赖图数据库查询工具模块 ... | data / data |
| 8 | blueprint.md | → | D_GOVERNANCE 生命周期管理: Batch2 治理层契约 — 15条 Pydantic v2 Schema（P... | runtime / runtime |
| 9 | blueprint.md | → | D_GOVERNANCE 生命周期管理: depgraph_reader.py — 依赖图数据库查询工具模块 ... | data / data |
...
| 23 | blueprint.md | → | D_INTEGRATION 管线路由: MCP Prompt 模板提供者（MOD-INF-013 Phase 6 — .... | runtime / runtime |
| 24 | blueprint.md | → | D_INTEGRATION 管线路由: MCP Prompt 模板提供者（MOD-INF-013 Phase 6 — .... | contract / contract |
...
| 10 | D_ML_TRAIN 训练: D_ML_TRAIN — ML Inference Base (inference_base.py) | → | blueprint.md | data / data |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 10 个外部域直接连接（出边 33 条 + 入边 10 条 = 43 条）。只显示直接连接的域，不展开具体节点。
