...
graph TD
    ...
    class D_GOVERNANCE,D_SECURITY_LLM,D_GOV_DRIFT,D_GOV_AUDIT,D_COMPLIANCE,D_FBL_VERIFICATION,D_ML_TRAIN,D_INFRA_RUNTIME,D_AUTONOMY_CORE external_design
```
...
> 仅展示蓝图阶段、代码未写的设计态模块（共 26 个，5 条域内依赖）。
...
graph TD
    ...
    class D_GOVERNANCE,D_COMPLIANCE,D_FBL_VERIFICATION,D_GOV_AUDIT,D_GOV_DRIFT,D_SECURITY_LLM,D_AUTONOMY_CORE external_design
```
...
| 20 | blueprint.md | → | D_INTEGRATION 管线路由: MCP 全量工具调用审计日志（MOD-INF-013 §12 Step... | runtime / runtime |
| 21 | blueprint.md | → | D_INTEGRATION 管线路由: MCP 全量工具调用审计日志（MOD-INF-013 §12 Step... | contract / contract |
| 22 | blueprint.md | → | D_ML_TRAIN 训练: blueprint.md | runtime / runtime |
| 23 | blueprint.md | → | D_SECURITY 对抗验证: NovelAttackGuard — 新型攻击行为画像. (novel_at... | contract / contract |
...
| 9 | D_GOVERNANCE 生命周期管理: backup_config.yaml | → | blueprint.md | runtime / runtime |
| 10 | D_GOVERNANCE 生命周期管理: Construction Verifier — 施工验证器: 任务卡完成... | → | blueprint.md | contract / contract |
| 11 | D_GOVERNANCE 生命周期管理: post_sync_validator — post_sync_standard 命令.... | → | blueprint.md | contract / contract |
| 12 | D_GOVERNANCE 生命周期管理: model_provider_data.py | → | blueprint.md | runtime / runtime |
| 13 | D_GOVERNANCE 生命周期管理: model_provider_data.py | → | blueprint.md | data / data |
...
| 16 | D_GOVERNANCE 生命周期管理: Self-Benchmark (W3-7) — 5 组已知对自验证 + 引.... | → | blueprint.md | contract / contract |
| 17 | D_GOV_AUDIT 审计追踪: blueprint.md | → | blueprint.md | runtime / runtime |
...
> 本域与 12 个外部域直接连接（出边 25 条 + 入边 20 条 = 45 条）。只显示直接连接的域，不展开具体节点。
