...
graph TD
    ...
    class D_GOVERNANCE,D_COMPLIANCE,D_GOV_CODE_QUALITY,D_GOV_AUDIT,D_GOV_DRIFT,D_INTEGRATION external_design
```
...
|:--:|---------|:--:|---------|---------|
| 1 | blueprint.md | → | D_AUTONOMY_CORE 自治核心: sensitivity_classifier.py — 数据分级 (B9, DD83... | contract / contract |
...
| 7 | blueprint.md | → | D_FEEDBACK_LOOP 反馈循环引擎: FLE->Orc 告警分派器 — dispatch() 生产者 (alert... | runtime / runtime |
| 8 | blueprint.md | → | D_GOVERNANCE 生命周期管理: Construction Verifier — 施工验证器: 任务卡完成... | runtime / runtime |
| 9 | blueprint.md | → | D_GOVERNANCE 生命周期管理: model_provider_data.py | data / data |
| 10 | blueprint.md | → | D_GOVERNANCE 生命周期管理: Self-Benchmark (W3-7) — 5 组已知对自验证 + 引.... | runtime / runtime |
| 11 | blueprint.md | → | D_GOVERNANCE 生命周期管理: model_provider_data.py | runtime / runtime |
...
| 14 | blueprint.md | → | D_GOV_AUDIT 审计追踪: 三元对齐检测：蓝图声明清单 vs 磁盘实际文件 vs i... | runtime / runtime |
| 15 | blueprint.md | → | D_GOV_CODE_QUALITY 代码质量治理: encoding_gate.py — 编码安全校验门禁（治本：弥.... | runtime / runtime |
| 16 | blueprint.md | → | D_GOV_CODE_QUALITY 代码质量治理: encoding_gate.py — 编码安全校验门禁（治本：弥.... | contract / contract |
...
| 24 | blueprint.md | → | D_SECURITY 对抗验证: MonotonicClock — 单调时钟. (monotonic_clock.py) | contract / contract |
| 25 | blueprint.md | → | D_SECURITY 对抗验证: MonotonicClock — 单调时钟. (monotonic_clock.py) | runtime / runtime |
...
> 本域与 11 个外部域直接连接（出边 25 条 + 入边 20 条 = 45 条）。只显示直接连接的域，不展开具体节点。
