...
graph TD
    ...
    class D_INTEGRATION,D_GOVERNANCE,D_FEEDBACK_LOOP,D_FBL_DETECTORS,D_FBL_DIAGNOSERS,D_COMPLIANCE,D_GOV_AUDIT,D_SECURITY,D_GOV_DRIFT,D_FRONTEND,D_INFRASTRUCTURE,D_ML_TRAIN external_design
```
...
| 4 | blueprint.md | → | D_FBL_VERIFICATION 反馈验证: CI/CD Pre-Scanner — v0.8.0 R107 (ci_cd_pre_sca... | runtime / runtime |
...
| 10 | blueprint.md | → | D_GOVERNANCE 生命周期管理: G-CT-008 契约：A2A -> Audit 审计 Agent 间通信. ... | runtime / runtime |
...
| 15 | blueprint.md | → | D_GOV_AUDIT 审计追踪: blueprint.md | runtime / runtime |
...
| 24 | blueprint.md | → | D_INTEGRATION 管线路由: Structural Protocol interfaces for cross-module... | runtime / runtime |
| 25 | blueprint.md | → | D_INTEGRATION 管线路由: MCP Prompt 模板提供者（MOD-INF-013 Phase 6 — .... | runtime / runtime |
| 26 | blueprint.md | → | D_INTEGRATION 管线路由: MCP Prompt 模板提供者（MOD-INF-013 Phase 6 — .... | contract / contract |
| 27 | blueprint.md | → | D_INTEGRATION 管线路由: Degradation Spiral Detector — 模型幻觉-容量正.... | runtime / runtime |
| 28 | blueprint.md | → | D_INTEGRATION 管线路由: Degradation Spiral Detector — 模型幻觉-容量正.... | runtime / runtime |
...
| 32 | blueprint.md | → | D_SECURITY 对抗验证: phase_executor.py | contract / contract |
| 33 | blueprint.md | → | D_SECURITY 对抗验证: LLM Security Gateway - Streamlit Dashboard. (ap... | contract / contract |
| 34 | blueprint.md | → | D_SECURITY 对抗验证: phase_executor.py | runtime / runtime |
...
> 本域与 15 个外部域直接连接（出边 37 条 + 入边 13 条 = 50 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_GOV_DOCS -->|5条 contract / contract, runtime / runtime| D_INTEGRATION
