...
graph TD
    ...
    class D_GOVERNANCE,D_FEEDBACK_LOOP,D_FBL_DETECTORS,D_FBL_DIAGNOSERS,D_COMPLIANCE,D_GOV_CODE_QUALITY,D_INTEGRATION,D_GOV_AUDIT,D_GOV_DRIFT,D_FRONTEND,D_INFRASTRUCTURE,D_ML_TRAIN external_design
```
...
| 4 | blueprint.md | → | D_FBL_VERIFICATION 反馈验证: Canary Repair — v0.8.0 R104b (canary_repair.py) | runtime / runtime |
| 5 | blueprint.md | → | D_FEEDBACK_LOOP 反馈循环引擎: feedback-loop.collectors — auto-generated pack... | runtime / runtime |
| 6 | blueprint.md | → | D_FEEDBACK_LOOP 反馈循环引擎: feedback-loop.evolution — auto-generated packa... | runtime / runtime |
...
| 9 | blueprint.md | → | D_GOVERNANCE 生命周期管理: G-CT-008 契约：A2A -> Audit 审计 Agent 间通信. ... | runtime / runtime |
...
| 21 | blueprint.md | → | D_GOV_CODE_QUALITY 代码质量治理: ch_final_gate.py — ch_writer.query() 直接调用.... | contract / contract |
| 22 | blueprint.md | → | D_GOV_CODE_QUALITY 代码质量治理: ch_final_gate.py — ch_writer.query() 直接调用.... | runtime / runtime |
...
| 28 | blueprint.md | → | D_GOV_DRIFT 漂移检测: blueprint.md | contract / contract |
...
| 31 | blueprint.md | → | D_INTEGRATION 管线路由: Structural Protocol interfaces for cross-module... | runtime / runtime |
| 32 | blueprint.md | → | D_INTEGRATION 管线路由: MCP Prompt 模板提供者（MOD-INF-013 Phase 6 — .... | runtime / runtime |
| 33 | blueprint.md | → | D_INTEGRATION 管线路由: MCP Prompt 模板提供者（MOD-INF-013 Phase 6 — .... | contract / contract |
| 34 | blueprint.md | → | D_KNOWLEDGE 知识管理: blueprint.md | contract / contract |
| 35 | blueprint.md | → | D_ML_TRAIN 训练: blueprint.md | runtime / runtime |
| 36 | blueprint.md | → | D_ML_TRAIN 训练: blueprint.md | data / data |
| 37 | blueprint.md | → | D_SECURITY 对抗验证: AgentRbac 异常类型. (exceptions.py) | contract / contract |
| 38 | blueprint.md | → | D_SECURITY 对抗验证: LLM Security Gateway - Streamlit Dashboard. (ap... | contract / contract |
| 39 | blueprint.md | → | D_SECURITY 对抗验证: AgentRbac 异常类型. (exceptions.py) | runtime / runtime |
| 40 | blueprint.md | → | D_SECURITY 对抗验证: LLM Security Gateway - Streamlit Dashboard. (ap... | runtime / runtime |
| 41 | blueprint.md | → | D_SECURITY 对抗验证: AgentRbac 异常类型. (exceptions.py) | runtime / runtime |
| 42 | blueprint.md | → | D_SECURITY 对抗验证: LLM Security Gateway - Streamlit Dashboard. (ap... | contract / contract |
...
| 10 | D_INFRASTRUCTURE: TCP relay: expose localhost-only MinIO to the C... | → | blueprint.md | runtime / runtime |
| 11 | D_ML_TRAIN 训练: D_ML_TRAIN — ML Inference Base (inference_base.py) | → | blueprint.md | data / data |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 16 个外部域直接连接（出边 42 条 + 入边 11 条 = 53 条）。只显示直接连接的域，不展开具体节点。
