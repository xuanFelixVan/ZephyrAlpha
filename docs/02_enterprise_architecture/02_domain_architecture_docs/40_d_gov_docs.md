...
graph TD
    ...
    docs_03_modules_cross_layer_resource_optimization_engine_blueprint_md -.->|contract / contract| D_COMPLIANCE
    ...
    class D_FBL_VERIFICATION,D_AUTONOMY_CORE,D_INTEGRATION external_prod
    class D_INFRA_RUNTIME,D_COMPLIANCE,D_GOV_AUDIT,D_GOVERNANCE,D_INFRA_A2A,D_GOV_DRIFT,D_FEEDBACK_LOOP external_design
```
...
|:--:|---------|:--:|---------|---------|
| 1 | blueprint.md | → | D_AUTONOMY_CORE 自治核心: poisoning_monitor.py — Embed 污染检测 (DD97, T... | contract / contract |
...
| 7 | blueprint.md | → | D_FBL_VERIFICATION 反馈验证: GateVerdict — GateVerdict (parameterized_safet... | runtime / runtime |
| 8 | blueprint.md | → | D_FBL_VERIFICATION 反馈验证: GateVerdict — GateVerdict (parameterized_safet... | contract / contract |
| 9 | blueprint.md | → | D_FBL_VERIFICATION 反馈验证: GateVerdict — GateVerdict (parameterized_safet... | runtime / runtime |
...
| 17 | blueprint.md | → | D_GOV_AUDIT 审计追踪: 三元对齐检测：蓝图声明清单 vs 磁盘实际文件 vs i... | runtime / runtime |
...
| 23 | blueprint.md | → | D_INTEGRATION 管线路由: MCP 错误码集中注册（MOD-INF-013 §3.4）。 (erro... | runtime / runtime |
| 24 | blueprint.md | → | D_INTEGRATION 管线路由: MCP 错误码集中注册（MOD-INF-013 §3.4）。 (erro... | contract / contract |
...
> 本域与 11 个外部域直接连接（出边 24 条 + 入边 19 条 = 43 条）。只显示直接连接的域，不展开具体节点。
