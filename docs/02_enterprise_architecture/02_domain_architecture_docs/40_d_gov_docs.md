...
| 64 | docs/02_enterprise_architecture/migration-registry.yaml | migration-registry.yaml | 生产态 / production |  |
...
| 68 | docs/03_modules/_master_blueprint/blueprint_agent_spec.md/ |  | 设计态 / design |  |
| 69 | docs/03_modules/_master_blueprint/blueprint_baseline.md/ |  | 设计态 / design |  |
| 70 | docs/03_modules/_master_blueprint/blueprint_capacity.md/ |  | 设计态 / design |  |
...
graph TD
    ...
    docs_03_modules_cross_layer_resource_optimization_engine_blueprint_md -.->|contract / contract| D_FBL_VERIFICATION
    ...
    class D_INFRA_RUNTIME,D_FBL_VERIFICATION,D_COMPLIANCE,D_GOV_AUDIT,D_GOVERNANCE,D_INFRA_A2A,D_GOV_DRIFT,D_SECURITY,D_INTEGRATION,D_FEEDBACK_LOOP,D_AUTONOMY_CORE,D_FRONTEND external_design
```
...
graph TD
    ...
    class D_INFRA_TELEMETRY external_prod
    class D_GOVERNANCE,D_INTEGRATION,D_SECURITY,D_GOV_DRIFT,D_INFRA_A2A,D_GOV_AUDIT,D_COMPLIANCE,D_FBL_VERIFICATION,D_INFRA_RUNTIME,D_AUTONOMY_CORE,D_FRONTEND external_design
```
...
|:--:|---------|:--:|---------|---------|
| 1 | blueprint.md | → | D_AUTONOMY_CORE 自治核心: Agent Spec -> Pipeline 集成桥接层 (__init__.py) | contract / contract |
...
| 5 | blueprint.md | → | D_FBL_VERIFICATION 反馈验证: _operational_gates.py | runtime / runtime |
| 6 | blueprint.md | → | D_FBL_VERIFICATION 反馈验证: _operational_gates.py | contract / contract |
| 7 | blueprint.md | → | D_FBL_VERIFICATION 反馈验证: _operational_gates.py | runtime / runtime |
...
| 11 | blueprint.md | → | D_GOVERNANCE 生命周期管理: Self-Benchmark (W3-7) — 5 组已知对自验证 + 引.... | runtime / runtime |
| 12 | blueprint.md | → | D_GOVERNANCE 生命周期管理: model_provider_data.py | runtime / runtime |
| 13 | blueprint.md | → | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | runtime / runtime |
...
| 16 | blueprint.md | → | D_GOV_AUDIT 审计追踪: 三元对齐检测：蓝图声明清单 vs 磁盘实际文件 vs i... | runtime / runtime |
...
| 19 | blueprint.md | → | D_INFRA_A2A A2A通信: test_a2a_check.py | contract / contract |
| 20 | blueprint.md | → | D_INFRA_A2A A2A通信: test_a2a_check.py | runtime / runtime |
| 21 | blueprint.md | → | D_INFRA_RUNTIME 运行时集成: blueprint.md | runtime / runtime |
| 22 | blueprint.md | → | D_INFRA_TELEMETRY 可观测性: logs — 结构化日志流（structlog + JSONL + trace... | runtime / runtime |
...
| 25 | blueprint.md | → | D_SECURITY 对抗验证: LLM Security Gateway - Streamlit Dashboard. (ap... | contract / contract |
...
| 3 | D_AUTONOMY_CORE 自治核心: Agent Spec -> Pipeline 集成桥接层 (__init__.py) | → | blueprint.md | contract / contract |
| 4 | D_FRONTEND 前端:  | → | blueprint.md | runtime / runtime |
| 5 | D_GOVERNANCE 生命周期管理: Construction Verifier — 施工验证器: 任务卡完成... | → | blueprint.md | contract / contract |
| 6 | D_GOVERNANCE 生命周期管理: post_sync_validator — post_sync_standard 命令.... | → | blueprint.md | contract / contract |
| 7 | D_GOVERNANCE 生命周期管理: Self-Benchmark (W3-7) — 5 组已知对自验证 + 引.... | → | blueprint.md | contract / contract |
| 8 | D_GOV_AUDIT 审计追踪: blueprint.md | → | blueprint.md | runtime / runtime |
| 9 | D_GOV_DRIFT 漂移检测: blueprint.md | → | blueprint.md | runtime / runtime |
| 10 | D_INFRA_TELEMETRY 可观测性: logs — 结构化日志流（structlog + JSONL + trace... | → | blueprint.md | runtime / runtime |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 13 个外部域直接连接（出边 25 条 + 入边 10 条 = 35 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_AUTONOMY_CORE -->|3条 contract / contract, runtime / runtime| D_GOV_DOCS
