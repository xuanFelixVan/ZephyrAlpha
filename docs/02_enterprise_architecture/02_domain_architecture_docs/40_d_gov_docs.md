...
graph TD
    ...
    class D_GOV_AUDIT,D_GOV_DRIFT,D_KNOWLEDGE,D_INFRA_RUNTIME,D_ML_TRAIN,D_FRONTEND external_design
```
...
> 仅展示蓝图阶段、代码未写的设计态模块（共 26 个，21 条域内依赖）。
...
| 8 | blueprint.md | → | D_INFRA_RUNTIME 运行时集成: blueprint.md | runtime / runtime |
| 9 | blueprint.md | → | D_INFRA_RUNTIME 运行时集成: blueprint.md | runtime / runtime |
| 10 | blueprint.md | → | D_KNOWLEDGE 知识管理: blueprint.md | contract / contract |
| 11 | blueprint.md | → | D_ML_TRAIN 训练: blueprint.md | runtime / runtime |
...
> 本域与 6 个外部域直接连接（出边 11 条 + 入边 10 条 = 21 条）。只显示直接连接的域，不展开具体节点。
