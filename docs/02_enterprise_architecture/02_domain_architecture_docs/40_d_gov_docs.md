...
graph TD
    ...
    class D_GOV_DRIFT,D_ML_TRAIN,D_INFRA_RUNTIME,D_GOV_AUDIT external_design
```
...
graph TD
    ...
    class D_GOV_DRIFT,D_INFRA_RUNTIME,D_ML_TRAIN,D_GOV_AUDIT external_design
```
...
| 4 | blueprint.md | → | D_ML_TRAIN 训练: blueprint.md | runtime / runtime |
...
> 本域与 4 个外部域直接连接（出边 4 条 + 入边 3 条 = 7 条）。只显示直接连接的域，不展开具体节点。
