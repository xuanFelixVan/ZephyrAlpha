...
graph TD
    ...
    class D_GOVERNANCE,D_GOV_AUDIT,D_SECURITY,D_FEEDBACK_LOOP,D_GOV_DRIFT,D_INTEGRATION,D_FRONTEND external_design
```
...
| 4 | blueprint.md | → | D_GOVERNANCE 生命周期管理: audit_logger.py | runtime / runtime |
| 5 | blueprint.md | → | D_GOVERNANCE 生命周期管理: post_sync_validator — post_sync_standard 命令.... | data / data |
| 6 | blueprint.md | → | D_GOVERNANCE 生命周期管理: Batch2 治理层契约 — 15条 Pydantic v2 Schema（P... | runtime / runtime |
| 7 | blueprint.md | → | D_GOVERNANCE 生命周期管理: post_sync_validator — post_sync_standard 命令.... | runtime / runtime |
...
> 本域与 10 个外部域直接连接（出边 30 条 + 入边 11 条 = 41 条）。只显示直接连接的域，不展开具体节点。
