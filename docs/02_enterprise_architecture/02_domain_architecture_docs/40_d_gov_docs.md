...
| # | 模块路径 / Module Path | 模块名称 / Module Name (功能简介 / Description) | 成熟度 / Maturity | 蓝图 / Blueprint |
|:--:|---------|---------|:---:|:---:|
| 1 | docs/03_modules/_master_blueprint/blueprint_agent_spec.md/ |  | 设计态 / design |  |
| 2 | docs/03_modules/_master_blueprint/blueprint_baseline.md/ |  | 设计态 / design |  |
| 3 | docs/03_modules/_master_blueprint/blueprint_capacity.md/ |  | 设计态 / design |  |
...
graph TD
    ...
    class D_GOVERNANCE,D_FEEDBACK_LOOP,D_FBL_DETECTORS,D_FBL_DIAGNOSERS,D_COMPLIANCE,D_FBL_VERIFICATION,D_INTEGRATION,D_GOV_AUDIT,D_GOV_DRIFT,D_FRONTEND,D_INFRASTRUCTURE external_design
```
...
| 4 | blueprint.md | → | D_FBL_VERIFICATION 反馈验证: _safety_gates.py | runtime / runtime |
...
| 16 | blueprint.md | → | D_GOV_AUDIT 审计追踪: 7 SLI + 5 容量 SLI + 退化检测。定时自检,输出 HE... | runtime / runtime |
...
| 28 | blueprint.md | → | D_INTEGRATION 管线路由: MCP Prompt 模板提供者（MOD-INF-013 Phase 6 — .... | runtime / runtime |
| 29 | blueprint.md | → | D_INTEGRATION 管线路由: MCP Prompt 模板提供者（MOD-INF-013 Phase 6 — .... | contract / contract |
| 30 | blueprint.md | → | D_ML_TRAIN 训练: blueprint.md | runtime / runtime |
| 31 | blueprint.md | → | D_ML_TRAIN 训练: blueprint.md | data / data |
| 32 | blueprint.md | → | D_SECURITY 对抗验证: DryRun — 权限模拟与影响分析. (dry_run.py) | contract / contract |
| 33 | blueprint.md | → | D_SECURITY 对抗验证: LLM Security Gateway - Streamlit Dashboard. (ap... | contract / contract |
| 34 | blueprint.md | → | D_SECURITY 对抗验证: DryRun — 权限模拟与影响分析. (dry_run.py) | runtime / runtime |
| 35 | blueprint.md | → | D_SECURITY 对抗验证: LLM Security Gateway - Streamlit Dashboard. (ap... | runtime / runtime |
| 36 | blueprint.md | → | D_SECURITY 对抗验证: DryRun — 权限模拟与影响分析. (dry_run.py) | runtime / runtime |
...
| 12 | D_INFRASTRUCTURE 跨层契约基础设施: TCP relay: expose localhost-only MinIO to the C... | → | blueprint.md | runtime / runtime |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 14 个外部域直接连接（出边 37 条 + 入边 12 条 = 49 条）。只显示直接连接的域，不展开具体节点。
