...
graph TD
    ...
    class D_GOV_AUDIT,D_SECURITY,D_GOV_REPAIR,D_GOVERNANCE,D_GOV_OPS_RESILIENCE,D_GOV_CODE_QUALITY external_prod
    class D_SHARED,D_COMPLIANCE,D_DATA external_design
```
...
graph TD
    ...
    class D_GOV_AUDIT,D_SECURITY,D_GOVERNANCE,D_FBL_DIAGNOSERS,D_FBL_DETECTORS,D_ORCHESTRATOR,D_GOV_REPAIR,D_GOV_OPS_RESILIENCE,D_PF_CORE external_prod
...
> 本域与 23 个外部域直接连接（出边 134 条 + 入边 116 条 = 250 条）。只显示直接连接的域，不展开具体节点。
