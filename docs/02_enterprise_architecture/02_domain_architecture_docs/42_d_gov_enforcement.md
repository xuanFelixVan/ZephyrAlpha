...
| 跨域入边 | 117 | Cross-domain Incoming | 117 |
...
graph TD
    ...
    class D_GOV_CODE_QUALITY,D_GOV_RULE,D_GOV_DRIFT,D_SECURITY,D_SHARED,D_GOV_AUDIT,D_GOV_OPS_RESILIENCE external_prod
...
graph TD
    ...
    class D_SHARED,D_GOV_SCRIPTS,D_GOV_AUDIT,D_GOVERNANCE,D_AUTONOMY_CORE external_design
```
...
graph TD
    ...
    class D_FBL_DIAGNOSERS,D_ORCHESTRATOR,D_SHARED,D_FBL_VERIFICATION,D_GOV_RULE,D_GOV_OPS_RESILIENCE,D_GOV_REPAIR,D_PF_CORE,D_GOVERNANCE external_prod
...
| 108 | D_GOV_RULE 规则治理: rule_engine package — 规则引擎模块集合（ARCH-0... | → | Rule Canary Manager — v0.10.0 规则金丝雀: 1%用... | config_depends / config_depends |
...
| 111 | D_INFRASTRUCTURE: backup_reconciler.py — 灾备备份系统事件触发器.... | → | gov_enforcement package — 执行治理域（D_GOV_EN... | 导入依赖 / import_depends |
| 112 | D_PF_CORE 组合核心: Re-export wrapper: compliance_rule has migrated... | → | Re-export shim — ComplianceRule 真源已合并至 z... | 导入依赖 / import_depends |
| 113 | D_SHARED 共享服务: test_e_gov_approval.py | → | G-CT-004 — Backward-compat re-export of Approv... | 测试依赖 / test_depends |
| 114 | D_SHARED 共享服务: test_e_slo_contract.py | → | SLO-Driven Escalation Contract — D-022-12. (sl... | 测试依赖 / test_depends |
| 115 | D_TRADING 交易运营: zephyr.trading.trading_contracts — trading-dom... | → | Re-export shim — ComplianceRule 真源已合并至 z... | 导入依赖 / import_depends |
| 116 | D_TRADING 交易运营: trading-contracts.risk — risk management domai... | → | Re-export shim — ComplianceRule 真源已合并至 z... | 导入依赖 / import_depends |
| 117 | D_TRADING 交易运营: tests.test_trading_session_lifecycle — F1 Sess... | → | session_lifecycle.py | 测试依赖 / test_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 23 个外部域直接连接（出边 133 条 + 入边 117 条 = 250 条）。只显示直接连接的域，不展开具体节点。
