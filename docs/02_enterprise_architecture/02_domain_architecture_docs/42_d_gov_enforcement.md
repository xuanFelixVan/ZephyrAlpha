...
> 展示全部 40 个模块（生产态 40 + 设计态 0），标签标注成熟度。
...
| 64 | RuleWatcher — YAML 规则文件变更检测与自动同步 ... | → | D_SHARED 共享服务: SQLite 连接工厂真源（SSoT） (sqlite_factory.py) | 导入依赖 / import_depends |
...
| 25 | D_GOV_CODE_QUALITY 代码质量治理: bare_subprocess_gate.py — 裸 subprocess 调用 w... | → | commit_gate_registry.py — GitCommitGateway pre... | 导入依赖 / import_depends |
...
> 本域与 14 个外部域直接连接（出边 64 条 + 入边 110 条 = 174 条）。只显示直接连接的域，不展开具体节点。
