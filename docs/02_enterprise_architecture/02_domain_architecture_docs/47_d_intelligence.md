...
graph TD
    ...
    D_ML_TRAIN -.->|config_depends / config_depends| src_zephyr_intelligence_model_evaluation_sync_engine_py
    ...
    class D_KNOWLEDGE,D_GOV_AUDIT,D_ML_TRAIN,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_SHARED,D_GOV_DRIFT,D_GOV_RULE,D_FBL_VERIFICATION,D_OPS,D_FRONTEND,D_INTEGRATION,D_GOVERNANCE external_prod
...
graph TD
    ...
    class D_SHARED,D_SECURITY,D_GOV_CODE_QUALITY,D_FBL_DETECTORS,D_OPS,D_BACKTEST,D_GOV_OPS_RESILIENCE,D_FBL_DIAGNOSERS external_prod
```
...
graph TD
    ...
    class D_FBL_DIAGNOSERS,D_INFRA_RUNTIME,D_GOVERNANCE,D_ORCHESTRATOR,D_FEEDBACK_LOOP,D_AUTONOMY_CORE,D_FUNDAMENTAL_SIGNAL external_prod
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_SHARED,D_GOV_RULE,D_GOV_KB,D_INTEGRATION external_prod
    class D_ML_TRAIN,D_GOVERNANCE,D_KNOWLEDGE,D_GOV_AUDIT,D_GOV_SCRIPTS,D_AUTONOMY_CORE external_design
```
...
graph TD
    ...
    class D_SHARED,D_FBL_DIAGNOSERS,D_SECURITY,D_GOV_CODE_QUALITY,D_GOV_DRIFT,D_BACKTEST,D_FBL_DETECTORS,D_OPS,D_INFRA_RUNTIME external_prod
...
| 58 | D_ML_TRAIN — ML Training Domain (__init__.py) | → | D_ML_TRAIN 训练: D_ML_TRAIN — ML Inference Base (inference_base.py) | config_depends / config_depends |
...
| 36 | D_ML_TRAIN 训练: Intelligence — Model Evaluation Domain (__init... | → | KB->VMS 同步引擎 — sync_to_vms() 生产者 (sync_... | config_depends / config_depends |
...
> 本域与 26 个外部域直接连接（出边 97 条 + 入边 38 条 = 135 条）。只显示直接连接的域，不展开具体节点。
