...
| 12 | src/zephyr/intelligence/model_evaluation/implementations/... | D_ML_TRAIN — Default Inference Engine | 原型态 / prototype | [MOD-INF-036](../../03_modules/_cross_layer/model_capability_exam/blueprint.md) |
...
> 展示全部 109 个模块（生产态 20 + 设计态 0 + 原型态 89），标签标注成熟度。
...
graph TD
    subgraph D_INTELLIGENCE["D_INTELLIGENCE 上下文管理"]
        ...
        src_zephyr_intelligence_model_evaluation_implementations_default_inference_engine_py["(原型态 / prototype) D_ML_TRAIN — Default Inference Engine<br/>文件: default_inference_engine.py"]
...
graph TD
    ...
    class D_SHARED,D_INFRA_RUNTIME,D_GOV_RULE,D_GOV_DRIFT,D_FBL_VERIFICATION,D_OPS,D_FRONTEND,D_INTEGRATION,D_GOVERNANCE external_prod
...
graph TD
    ...
    class D_OPS,D_SHARED,D_GOV_OPS_RESILIENCE,D_GOV_DRIFT,D_GOV_CODE_QUALITY,D_SECURITY,D_FEEDBACK_LOOP,D_FBL_DETECTORS external_prod
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_FBL_DIAGNOSERS,D_GOVERNANCE,D_ORCHESTRATOR,D_FEEDBACK_LOOP,D_AUTONOMY_CORE external_prod
```

### 运营态子图（仅 design_maturity=production 的模块和依赖）

> 仅展示已上线运行的模块（共 20 个，12 条域内依赖）。
...
graph TD
    ...
    class D_SHARED,D_GOV_KB,D_GOV_RULE,D_INFRA_RUNTIME,D_INTEGRATION external_prod
    class D_ML_TRAIN,D_GOVERNANCE,D_GOV_SCRIPTS,D_GOV_AUDIT,D_KNOWLEDGE external_design
```
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 89 个，6 条域内依赖）。
...
graph TD
    ...
    class D_INTEGRATION,D_BACKTEST,D_SHARED,D_GOV_RULE,D_INFRA_RUNTIME,D_GOV_DRIFT,D_FBL_VERIFICATION,D_OPS external_prod
...
| 89 | ModelProfiler — 核心性能分析引擎 (profiler.py) | → | D_SHARED 共享服务: constants.py —— 共享枚举 & 常量集中 re-export... | 导入依赖 / import_depends |
| 90 | ModelProfiler — 核心性能分析引擎 (profiler.py) | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 91 | Results Writer — 持久化 benchmark 结果，支持历... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 92 | test_ai_capability_guard.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 93 | DM-201503: F4 事件驱动预算执行——超限/IPI/螺旋... | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 测试依赖 / test_depends |
| 94 | test_budget_handler.py | → | D_SHARED 共享服务: budget_alert.py | 测试依赖 / test_depends |
...
| 36 | D_ML_TRAIN 训练: Intelligence — Model Evaluation Domain (__init... | → | Cross-Encoder 重排序层 — BGE-reranker-v2-m3（T... | config_depends / config_depends |
