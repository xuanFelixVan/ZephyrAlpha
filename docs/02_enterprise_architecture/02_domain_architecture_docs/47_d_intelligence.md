...
| 域内依赖 | 57 | Internal Dependencies | 57 |
...
graph TD
    ...
    class D_BACKTEST,D_FBL_DIAGNOSERS,D_OPS,D_SHARED,D_GOV_OPS_RESILIENCE,D_GOV_DRIFT,D_GOV_CODE_QUALITY,D_SECURITY external_prod
```
...
graph TD
    ...
    class D_INFRA_RUNTIME,D_GOV_RULE,D_SHARED,D_GOV_KB,D_INTEGRATION external_prod
    class D_ML_TRAIN,D_AUTONOMY_CORE,D_GOVERNANCE,D_KNOWLEDGE,D_GOV_AUDIT,D_GOV_SCRIPTS external_design
```
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 87 个，2 条域内依赖）。
...
| 19 | test_alpha_signal_pipeline.py | → | D_FUNDAMENTAL_SIGNAL 基本面信号: AlphaSignalPipeline D_FACTOR->D_SIGNAL跨层集成... | 测试依赖 / test_depends |
...
| 95 | test_model_discovery.py | → | D_SHARED 共享服务: constants.py —— 共享枚举 & 常量集中 re-export... | 测试依赖 / test_depends |
...
| 38 | D_ML_TRAIN 训练: Intelligence — Model Evaluation Domain (__init... | → | G4 Activate 门禁 — 人工激活（T-2-13-D） (activ... | config_depends / config_depends |
...
> 本域与 25 个外部域直接连接（出边 96 条 + 入边 40 条 = 136 条）。只显示直接连接的域，不展开具体节点。
