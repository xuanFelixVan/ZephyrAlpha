...
graph TD
    ...
    class D_GOV_DRIFT,D_INFRA_RUNTIME,D_GOV_RULE,D_FBL_VERIFICATION,D_OPS,D_FRONTEND,D_INTEGRATION,D_GOVERNANCE external_prod
...
graph TD
    ...
    class D_OPS,D_SHARED,D_GOV_OPS_RESILIENCE,D_GOV_CODE_QUALITY,D_SECURITY,D_FEEDBACK_LOOP,D_FBL_DETECTORS,D_BACKTEST external_prod
```
...
graph TD
    ...
    class D_GOV_RULE,D_INFRA_RUNTIME,D_GOV_KB,D_INTEGRATION external_prod
    class D_SHARED,D_ML_TRAIN,D_GOVERNANCE,D_GOV_AUDIT,D_KNOWLEDGE,D_AUTONOMY_CORE,D_GOV_SCRIPTS external_design
```
...
graph TD
    ...
    class D_OPS,D_SHARED,D_GOV_OPS_RESILIENCE,D_INTEGRATION,D_BACKTEST,D_GOV_CODE_QUALITY,D_SECURITY,D_FEEDBACK_LOOP,D_FBL_DETECTORS,D_INFRA_RUNTIME external_prod
...
| 96 | D_ML_TRAIN — Default Inference Engine (default... | → | D_TRADING 交易运营: model_serving_request.py | 导入依赖 / import_depends |
...
> 本域与 26 个外部域直接连接（出边 96 条 + 入边 38 条 = 134 条）。只显示直接连接的域，不展开具体节点。
