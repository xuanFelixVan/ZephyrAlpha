...
graph TD
    ...
    D_GOVERNANCE -->|import_depends| src_zephyr_shared_io_paths_py
    D_GOVERNANCE -->|import_depends| src_zephyr_shared_io_paths_py
    D_GOVERNANCE -.->|import_depends| src_zephyr_shared_io_sqlite_factory_py
    classDef production fill:#e1f5fe,stroke:#01579b,stroke-width:2px,color:#000
    classDef design fill:#fff3e0,stroke:#e65100,stroke-width:2px,color:#000,stroke-dasharray: 5 5
    classDef external_prod fill:#e8f5e9,stroke:#1b5e20,stroke-width:1px,color:#000
    classDef external_design fill:#fce4ec,stroke:#880e4f,stroke-width:1px,color:#000,stroke-dasharray: 5 5
...
graph TD
    ...
    class D_GOVERNANCE,D_AUTONOMY_CORE,D_GOV_ENFORCEMENT,D_INFRA_RUNTIME external_prod
```
...
| D_GOVERNANCE | 95 | import_depends |
| D_INFRA_RUNTIME | 68 | import_depends |
| D_TRADING | 59 | import_depends |
| D_GOV_SCRIPTS | 40 | import_depends |
| D_INTEGRATION | 34 | import_depends |
...
| D_INTELLIGENCE | 9 | import_depends |
...
> 域内模块依赖关系（共 168 条 / 168 edges）。按依赖类型分组，使用 → 表示方向。

```

┌──────────────────────────────────────────────────────────────────┐
│      依赖关系图 / Dependency Graph (共 168 条 / 168 edges)       │
├──────────────────────────────────────────────────────────────────┤
