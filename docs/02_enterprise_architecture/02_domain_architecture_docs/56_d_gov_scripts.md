...
| 域内依赖 | 764 | Internal Dependencies | 764 |
| 跨域入边 | 75 | Cross-domain Incoming | 75 |
| 跨域出边 | 140 | Cross-domain Outgoing | 140 |
| 设计态模块 | 1 | Design Modules | 1 |
| 生产态模块 | 393 | Production Modules | 393 |
| 容量 | 393/150 (超容) | Capacity | 393/150 (超容) |
...
> 展示全部 394 个模块（生产态 393 + 设计态 1），含跨域依赖外部节点。节点含成熟度+名称+大白话/简介+文件路径。
...
flowchart TD
    ...
    scripts_governance_archive_prototype_check_audit_rbac_isolation_py ~~~ scripts_governance_archive_prototype_generate_asset_index_py
    ...
    scripts_governance_archive_prototype_construction_gate_py ~~~ scripts_governance_archive_vms_ri_ri_boundary_check_py
    ...
    class D_DATA,D_SHARED,D_GOVERNANCE,D_INTEGRATION,D_GOV_AUDIT,D_INFRA_RUNTIME,D_GOV_ENFORCEMENT,D_GOV_CODE_QUALITY,D_GOV_DRIFT,D_GOV_RULE,D_OPS,D_AUTONOMY_PERM external_prod
```

### 运营态的图（仅 design_maturity=production 的模块和域内依赖）

> 仅展示已上线运行的模块（共 393 个），不含跨域外部节点。跨域依赖见下方跨域依赖章节。
