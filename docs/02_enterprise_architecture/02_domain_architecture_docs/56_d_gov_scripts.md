...
flowchart TD
    ...
    scripts_governance_d5_architecture_check_budget_health_py["(INVARIANTS)<br/>预算健康检查不可跳过;检查结果必须可机器解析<br/>预算健康检查器，检查预算执行情况是否健康，结果必<br/>须可机器解析且不可跳过（MOD-INF-024）。<br/>check_budget_health<br/>文件: d5_architecture/check_budget_health.py<br/>(生产态 / production)"]
    ...
    scripts_governance_d5_architecture_validators_validate_dependency_graph_template_py["(INVARIANTS) 治理脚本执行正确<br/>依赖图模板验证器，校验依赖图数据是否符合模板规范<br/>，保证治理脚本执行正确（MOD-INF-005）。<br/>validate_dependency_graph_template<br/>文件: validators<br/>/validate_dependency_graph_template.py<br/>(生产态 / production)"]
    ...
    scripts_governance_sync_check_p0_status_py["检查p0状态<br/>P0 优先级任务状态检查器，扫描任务仓库统计 P0<br/>级任务的待办与完成情况，供治理同步使用。<br/>文件: _sync/check_p0_status.py<br/>(生产态 / production)"]
