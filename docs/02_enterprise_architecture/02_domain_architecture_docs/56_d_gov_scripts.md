...
flowchart TD
    ...
    scripts_governance_archive_one_off_verify_final_delivery_py["(INVARIANTS) 设计态节点数>=1128; 规则表各表>0<br/>- 设计态节点数 >= 1128<br/>verify_final_delivery<br/>文件: one_off/verify_final_delivery.py<br/>(生产态 / production)"]
    ...
    scripts_governance_ast_import_rewriter_py["ast导入rewriter<br/>治理的ast导入rewriter。AST-based import<br/>rewriter for governance directory<br/>migration.，ast导入rewriter。AST-based import<br/>rewriter for governance directory<br/>migration.相关功能的实现。<br/>文件: governance/ast_import_rewriter.py<br/>(生产态 / production)"]
    ...
    scripts_governance_d1_structure_batch_create_index_md_py["批次创建索引md<br/>治理的批次创建索引md。Batch create index.md for<br/>all directories under docs/ that lack<br/>one.，批次创建索引md。Batch create index.md for<br/>all directories under docs/ that lack<br/>one.相关功能的实现。<br/>文件: d1_structure/batch_create_index_md.py<br/>(生产态 / production)"]
    ...
    scripts_governance_d3_metadata_generate_rule_catalog_py["生成规则目录<br/>治理的生成规则目录。Scan docs<br/>/01_policies_and_standards and emit _registry<br/>/catalogs<br/>/rule_catalog_registry.yaml.，生成规则目录。Scan<br/>docs/01_policies_and_standards and emit<br/>_registry/catalogs<br/>/rule_catalog_registry.yaml.相关功能的实现。<br/>文件: d3_metadata/generate_rule_catalog.py<br/>(生产态 / production)"]
    ...
    scripts_governance_d3_metadata_validate_blueprint_provenance_py["校验蓝图溯源<br/>治理的校验蓝图provenance。Blueprint Provenance<br/>Gate - V-12: validate provenance triples in<br/>blueprint<br/>frontmatter，校验蓝图provenance。Blueprint<br/>Provenance Gate - V-12: validate provenance<br/>triples in blueprint frontmatter相关功能的实现。<br/>文件: d3_metadata<br/>/validate_blueprint_provenance.py<br/>(生产态 / production)"]
    scripts_governance_d3_metadata_validate_module_id_py["校验模块id<br/>治理的校验模块id。GATE-MODULEID: Validate<br/>module_id uniqueness and index/file<br/>consistency.，校验模块id。GATE-MODULEID:<br/>Validate module_id uniqueness and index/file<br/>consistency.相关功能的实现。<br/>文件: d3_metadata/validate_module_id.py<br/>(生产态 / production)"]
    ...
    scripts_governance_d5_architecture_audit_agent_spec_py["审计代理spec<br/>治理的(INVARIANTS) agent-spec<br/>审计完整性，(INVARIANTS) agent-spec<br/>审计完整性相关功能的实现。<br/>audit_agent_spec<br/>文件: d5_architecture/audit_agent_spec.py<br/>(生产态 / production)"]
    scripts_governance_d5_architecture_check_budget_health_py["(INVARIANTS)<br/>预算健康检查不可跳过;检查结果必须可机器解析<br/>治理的(INVARIANTS)<br/>预算健康检查不可跳过;检查结果必须可机器解析，(IN<br/>VARIANTS)<br/>预算健康检查不可跳过;检查结果必须可机器解析相关<br/>功能的实现。<br/>check_budget_health<br/>文件: d5_architecture/check_budget_health.py<br/>(生产态 / production)"]
    ...
    scripts_governance_d7_code_validate_docstring_coverage_py["校验docstringcoverage<br/>AGENTS.md §5.1 — 零记忆重启标准（AI 靠<br/>docstring 理解代码意图）<br/>validate_docstring_coverage<br/>文件: d7_code/validate_docstring_coverage.py<br/>(生产态 / production)"]
    ...
    scripts_governance_d8_doc_sync_sync_rule_registry_py["同步规则注册表<br/>治理的同步规则注册表。Checks that every<br/>RULE-ZERO through RULE-N in .trae/rules<br/>/project_rules.md，同步规则注册表。Checks that<br/>every RULE-ZERO through RULE-N in .trae/rules<br/>/project_rules.md相关功能的实现。<br/>文件: d8_doc_sync/sync_rule_registry.py<br/>(生产态 / production)"]
    ...
    scripts_governance_extract_decisiongraph_py["提取decisiongraph<br/>治理的提取decisiongraph。extract_decisiongraph<br/>- decisiongraph on-demand extraction<br/>tool，提取decisiongraph。extract_decisiongraph<br/>- decisiongraph on-demand extraction<br/>tool相关功能的实现。<br/>文件: governance/extract_decisiongraph.py<br/>(生产态 / production)"]
    ...
    scripts_governance_meta_phase_e_context_check_py["阶段e上下文检查<br/>治理的阶段e上下文检查。Phase E: AI context<br/>injection verification<br/>script，阶段e上下文检查。Phase E: AI context<br/>injection verification script相关功能的实现。<br/>文件: meta/phase_e_context_check.py<br/>(生产态 / production)"]
    ...
    scripts_governance_sync_cleanup_p0_auto_bridged_py["清理历史 P0 自动桥接任务<br/>治理的清理历史 P0 自动桥接任务，清理历史 P0<br/>自动桥接任务相关功能的实现。<br/>cleanup_p0_auto_bridged<br/>文件: _sync/cleanup_p0_auto_bridged.py<br/>(生产态 / production)"]
    ...
    scripts_governance_shared_encoding_py["encoding.py — UTF-8 编码安全工具<br/>对标 AGENTS.md §6.7（审计脚本编码铁律）<br/>文件: _shared/encoding.py<br/>(生产态 / production)"]
    ...
    class D_DATA,D_GOVERNANCE,D_SHARED,D_GOV_AUDIT,D_GOV_ENFORCEMENT,D_GOV_DRIFT,D_GOV_RULE,D_AUTONOMY_PERM,D_OPS external_prod
```
