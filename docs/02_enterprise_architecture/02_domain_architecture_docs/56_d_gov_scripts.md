...
flowchart TD
    ...
    tests_governance_scripts_governance_test_validate_truth_source_cascade_governance_py -->|测试依赖 / test_depends| D_GOV_DRIFT
    ...
    class D_SHARED,D_GOV_DRIFT,D_GOVERNANCE,D_DATA,D_GOV_AUDIT,D_GOV_ENFORCEMENT,D_INFRA_RUNTIME external_prod
```
...
| 124 | G-panorama-align: 四图对齐检测器（ARCH-053 + ARCH-056 四... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 | 盲点 B19... | 导入依赖 / import_depends |
| 125 | G13: 从 depgraph (PostgreSQL) 生成资产清单全景图 (generat... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 126 | 从 candidate_module_registry.yaml 生成候选模块清单报告（... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 127 | Code Wiki 统计数据生成器（半自动维护机制）。 (generators/... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 128 | G12: 从 depgraph (PostgreSQL) 生成契约目录全景图 (generat... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 129 | generate_contracts.py -- SSoT to Codegen pipeline (genera... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 130 | G-panorama-registry: 自动生成全景图清单总表 (generators/g... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 131 | validate_module_lifecycle.py — 模块生命周期校验 (lifecyc... | → | D_SHARED 共享服务: yaml_utils.py — vocabulary YAML 加载公共工具（SSoT 真源... | 导入依赖 / import_depends |
| 132 | validate_interface_contracts.py — 接口契约校验 (validato... | → | D_SHARED 共享服务: yaml_utils.py — vocabulary YAML 加载公共工具（SSoT 真源... | 导入依赖 / import_depends |
| 133 | extract_decisiongraph - decisiongraph on-demand extractio... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 134 | [INVARIANTS] 禁止AI直接Read 157MB depgraph文件；提取输出... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 135 | [INVARIANTS] YAML 是唯一真源; DB 为只读缓存; 同步单向 YAM... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 136 | # [BLUEPRINT] MOD-INF-005 | scripts/governance/generate_p... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP servers and... | 导入依赖 / import_depends |
| 137 | # [BLUEPRINT] MOD-INF-005 | scripts/governance/generate_p... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 138 | # [BLUEPRINT] MOD-INF-005 | scripts/governance/generate_p... | → | D_SHARED 共享服务: yaml_utils.py — vocabulary YAML 加载公共工具（SSoT 真源... | 导入依赖 / import_depends |
| 139 | check_gate_inventory_drift.py — commit_gates 模块清单漂... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 140 | 从场外草稿 CSV 抓取候选模块入候选库（一次性 harvest 脚本... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP servers and... | 导入依赖 / import_depends |
| 141 | Module docstring — see module-level docstring for detail... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP servers and... | 导入依赖 / import_depends |
| 142 | create_task_from_finding.py — Finding → 任务卡自动创建... | → | D_SHARED 共享服务: ZephyrAlpha 任务系统核心数据模型 (foundation/models.py) | 导入依赖 / import_depends |
| 143 | create_task_from_finding.py — Finding → 任务卡自动创建... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 144 | SQLite → PostgreSQL 运营数据迁移脚本 (migrate_sqlite_to_... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 145 | concurrent_commit_test.py — 幽灵提交红蓝对抗脚本（OPS-20... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 146 | sync_panorama_module.py — 四图模块同步引擎（ARCH-056） (... | → | D_SHARED 共享服务: converters.py — 类型转换工具（消除 '' vs None 语义鸿沟）... | 导入依赖 / import_depends |
| 147 | 生成器触发路径验证脚本 (governance/verify_generator_paths... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP servers and... | 导入依赖 / import_depends |
