...
| 模块数 | 396 | Module Count | 396 |
| 域内依赖 | 765 | Internal Dependencies | 765 |
| 跨域入边 | 75 | Cross-domain Incoming | 75 |
| 跨域出边 | 141 | Cross-domain Outgoing | 141 |
| 设计态模块 | 0 | Design Modules | 0 |
| 生产态模块 | 396 | Production Modules | 396 |
| 容量 | 396/150 (超容) | Capacity | 396/150 (超容) |
...
> 展示全部 396 个模块（生产态 396 + 设计态 0），含跨域依赖外部节点。节点含成熟度+名称+大白话/简介+文件路径。
...
flowchart TD
    ...
    scripts_governance_oneoff_data_domain_design_state_complete_py -->|导入依赖 / import_depends| D_GOVERNANCE
    ...
    class D_DATA,D_GOVERNANCE,D_SHARED,D_GOV_ENFORCEMENT,D_INFRA_RUNTIME,D_GOV_AUDIT,D_GOV_RULE,D_GOV_DRIFT,D_GOV_DOCS external_prod
```

### 运营态的图（仅 design_maturity=production 的模块和域内依赖）

> 仅展示已上线运行的模块（共 396 个），不含跨域外部节点。跨域依赖见下方跨域依赖章节。
