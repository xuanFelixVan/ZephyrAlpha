...
| 模块数 | 397 | Module Count | 397 |
| 域内依赖 | 766 | Internal Dependencies | 766 |
| 跨域入边 | 75 | Cross-domain Incoming | 75 |
| 跨域出边 | 141 | Cross-domain Outgoing | 141 |
| 设计态模块 | 0 | Design Modules | 0 |
| 生产态模块 | 397 | Production Modules | 397 |
| 容量 | 397/150 (超容) | Capacity | 397/150 (超容) |
...
> 展示全部 397 个模块（生产态 397 + 设计态 0），含跨域依赖外部节点。节点含成熟度+名称+大白话/简介+文件路径。
...
flowchart TD
    ...
    scripts_governance_verify_generator_paths_py["governance/verify_generator_paths<br/>verify_generator_paths.py —<br/>生成器三条触发路径冒烟验证（手动/CI，不接<br/>commit...<br/>文件: governance/verify_generator_paths.py<br/>(生产态 / production)"]
    ...
    scripts_governance_archive_prototype_check_audit_rbac_isolation_py ~~~ scripts_governance_archive_vms_ri_vms_build_completion_check_py
    ...
    scripts_governance_apply_depgraph_py ~~~ scripts_governance_d5_architecture_generators_common_py
    ...
    class D_GOVERNANCE,D_SHARED,D_DATA,D_GOV_AUDIT,D_GOV_ENFORCEMENT,D_INFRA_RUNTIME,D_GOV_DRIFT,D_GOV_RULE,D_AUTONOMY_PERM,D_OPS external_prod
```

### 运营态的图（仅 design_maturity=production 的模块和域内依赖）

> 仅展示已上线运行的模块（共 397 个），不含跨域外部节点。跨域依赖见下方跨域依赖章节。
...
| 77 | CBG 熔断器重置 CLI (CircuitBreakerGateway Reset Command) ... | → | D_GOV_RULE 规则治理: 单向熔断器 / Circuit Breaker (rule_enforcement/circuit_br... | 导入依赖 / import_depends |
| 78 | create_task_from_finding.py — Finding → 任务卡自动创建... | → | D_GOV_RULE 规则治理: 任务类型定义 / Task Types (rule_enforcement/task_types.py) | 导入依赖 / import_depends |
| 79 | Gate Engine Bootstrap Self-Check — Quis custodiet ipsos ... | → | D_GOV_RULE 规则治理: 门禁裁决引擎 / Gate Engine (gate_engine/gate_engine.py) | 导入依赖 / import_depends |
| 80 | validate_gate_engine_external.py — Gate Engine 外部完整... | → | D_GOV_RULE 规则治理: 单向熔断器 / Circuit Breaker (rule_enforcement/circuit_br... | 导入依赖 / import_depends |
| 81 | validate_gate_engine_external.py — Gate Engine 外部完整... | → | D_GOV_RULE 规则治理: 门禁裁决引擎 / Gate Engine (gate_engine/gate_engine.py) | 导入依赖 / import_depends |
...
> 本域与 18 个外部域直接连接（出边 141 条 + 入边 75 条 = 216 条）。只显示直接连接的域，不展开具体节点。
