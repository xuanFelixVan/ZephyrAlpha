...
flowchart TD
    ...
    scripts_governance_archive_one_off_create_alignment_tasks_py["创建对齐任务<br/>供governance automation; alignme使用<br/># (BLUEPRINT) MOD-INF-005 / scripts/governance<br/>/create_alignment_tasks.py / §7<br/>文件: one_off/create_alignment_tasks.py<br/>(生产态 / production)"]
    ...
    scripts_governance_archive_one_off_verify_rule_yaml_migration_py["verify规则yamlmigration<br/>6维校验规则YAML迁移正确性，确保MD→YAML迁移无遗漏<br/>verify_rule_yaml_migration.py - 6-dimensional<br/>verification of rule YAML migration.<br/>文件: one_off/verify_rule_yaml_migration.py<br/>(生产态 / production)"]
    ...
    scripts_governance_archive_prototype_scan_ground_truth_deps_py["扫描groundtruthdeps<br/>供Task card system; governance a使用<br/># (BLUEPRINT) MOD-INF-005 / scripts/governance<br/>/scan_ground_truth_deps.py / §7<br/>文件: prototype/scan_ground_truth_deps.py<br/>(生产态 / production)"]
    ...
    scripts_governance_d11_compliance_validate_vocabulary_coverage_py["校验vocabularycoverage<br/>模块<br/>Module docstring — see module-level docstring<br/>for details.<br/>文件: d11_compliance/validate_vocabulary_<br/>coverage.py<br/>(生产态 / production)"]
    ...
    scripts_governance_d1_structure_detect_temp_files_py["检测tempfiles<br/>模块<br/>Module docstring — see module-level docstring<br/>for details.<br/>文件: d1_structure/detect_temp_files.py<br/>(生产态 / production)"]
    ...
    scripts_governance_d5_architecture_checkers_check_g6_ctr_compliance_py["检查g6ctr合规<br/>治理的检查器，检查某项条件是否满足<br/>check_g6_ctr_compliance.py - G6 CTR Contract<br/>Compliance Gate Engine<br/>文件: checkers/check_g6_ctr_compliance.py<br/>(生产态 / production)"]
    ...
    scripts_governance_d5_architecture_detect_causal_conflicts_py["检测causalconflicts<br/>模块<br/>Module docstring — see module-level docstring<br/>for details.<br/>文件: d5_architecture/detect_causal_conflicts.py<br/>(生产态 / production)"]
    ...
    scripts_governance_d5_architecture_validators_blueprint_validate_blueprint_path_consistency_py["校验蓝图路径一致性<br/>模块<br/>Module docstring — see module-level docstring<br/>for details.<br/>文件: blueprint/validate_blueprint_path_<br/>consistency.py<br/>(生产态 / production)"]
    ...
    scripts_governance_d5_architecture_validators_session_validate_session_log_index_integrity_py["校验会话日志索引完整性<br/>模块<br/>Module docstring — see module-level docstring<br/>for details.<br/>文件: session/validate_session_log_index_<br/>integrity.py<br/>(生产态 / production)"]
    ...
    scripts_governance_d5_architecture_validators_validate_directory_structure_py["校验directorystructure<br/>模块<br/>Module docstring — see module-level docstring<br/>for details.<br/>文件: validators/validate_directory_structure.py<br/>(生产态 / production)"]
    ...
    scripts_governance_d5_architecture_validators_validate_load_path_integrity_py["校验加载路径完整性<br/>模块<br/>Module docstring — see module-level docstring<br/>for details.<br/>文件: validators/validate_load_path_integrity.py<br/>(生产态 / production)"]
    ...
    scripts_governance_d5_architecture_validators_validate_nested_flat_dirs_py["校验nestedflatdirs<br/>模块<br/>Module docstring — see module-level docstring<br/>for details.<br/>文件: validators/validate_nested_flat_dirs.py<br/>(生产态 / production)"]
    ...
    scripts_governance_meta_governance_watchdog_py["治理watchdog<br/>模块<br/>Module docstring — see module-level docstring<br/>for details.<br/>文件: meta/governance_watchdog.py<br/>(生产态 / production)"]
    ...
    scripts_governance_meta_validate_script_onboarding_py["校验scriptonboarding<br/>模块<br/>Module docstring — see module-level docstring<br/>for details.<br/>文件: meta/validate_script_onboarding.py<br/>(生产态 / production)"]
    ...
    scripts_governance_run_all_py["统一入口脚本<br/>脚本系统统一入口，一键执行全部治理脚本<br/>run_all<br/>文件: governance/run_all.py<br/>(生产态 / production)"]
    scripts_governance_run_gate_chain_py["统一入口脚本<br/>顺序运行多个门禁脚本，任一失败即整体失败<br/>run_gate_chain<br/>文件: governance/run_gate_chain.py<br/>(生产态 / production)"]
    ...
    scripts_governance_shared_yaml_utils_py["目录遍历工具<br/>YAML 文件加载共享工具<br/>yaml_utils<br/>文件: _shared/yaml_utils.py<br/>(生产态 / production)"]
    ...
    scripts_governance_meta_benchmark_test_fixtures_bad_imports_py["无效导入<br/>（bad_imports.py）<br/>Module docstring — see module-level docstring<br/>for details.<br/>文件: test_fixtures/bad_imports.py<br/>(生产态 / production)"]
    ...
    scripts_governance_shared_encoding_py["编码安全工具<br/>UTF-8 编码安全工具，统一文件读写编码防止乱码<br/>encoding<br/>文件: _shared/encoding.py<br/>(生产态 / production)"]
    scripts_governance_shared_file_utils_py["编码安全工具<br/>原子写入共享工具<br/>file_utils<br/>文件: _shared/file_utils.py<br/>(生产态 / production)"]
    scripts_governance_shared_thresholds_py["阈值配置加载器<br/>阈值集中配置加载器，统一管理各门禁的阈值参数<br/>thresholds<br/>文件: _shared/thresholds.py<br/>(生产态 / production)"]
    scripts_governance_shared_walk_py["目录遍历工具<br/>目录遍历共享工具，递归扫描目录树并过滤文件<br/>walk<br/>文件: _shared/walk.py<br/>(生产态 / production)"]
    scripts_governance_sync_check_p0_status_py["检查p0状态<br/>模块<br/>Module docstring — see module-level docstring<br/>for details.<br/>文件: _sync/check_p0_status.py<br/>(生产态 / production)"]
    ...
    class D_DATA,D_GOVERNANCE,D_SHARED,D_GOV_AUDIT,D_GOV_ENFORCEMENT,D_GOV_RULE,D_GOV_DRIFT,D_AUTONOMY_PERM,D_OPS external_prod
```
...
| 87 | 统一入口脚本 / run_all (governance/run_all.py) | → | D_INFRA_RUNTIME 运行时集成: 发现任务桥接 / finding_task_bridge (infrastructure/findin... | 导入依赖 / import_depends |
| 88 | 统一入口脚本 / run_all (governance/run_all.py) | → | D_INFRA_RUNTIME 运行时集成: 发现 / finding (script_system/finding.py) | 导入依赖 / import_depends |
...
| 110 | 编码安全工具 / file_utils (_shared/file_utils.py) | → | D_SHARED 共享服务: 文件工具 / file_utils (io/file_utils.py) | 导入依赖 / import_depends |
| 111 | 目录遍历工具 / yaml_utils (_shared/yaml_utils.py) | → | D_SHARED 共享服务: yaml工具 / yaml_utils (io/yaml_utils.py) | 导入依赖 / import_depends |
...
| 3 | D_AUTONOMY_PERM 自治保护: 管理终止开关 / manage_kill_switch (meta/manage_kill_switc... | → | 编码安全工具 / file_utils (_shared/file_utils.py) | 导入依赖 / import_depends |
...
| 13 | D_GOVERNANCE 生命周期管理: checkscaffold退出门禁 / check_scaffold_exit_gates (arch_g... | → | 目录遍历工具 / yaml_utils (_shared/yaml_utils.py) | 导入依赖 / import_depends |
...
| 31 | D_GOVERNANCE 生命周期管理: 修复孤儿all / fix_orphan_all (scripts/fix_orphan_all.py) | → | 编码安全工具 / file_utils (_shared/file_utils.py) | 导入依赖 / import_depends |
...
| 34 | D_GOVERNANCE 生命周期管理: 检查pureshim / check_pure_shim (d7_code/check_pure_shim.py) | → | 编码安全工具 / encoding (_shared/encoding.py) | 导入依赖 / import_depends |
...
| 46 | D_GOVERNANCE 生命周期管理: scaffold.py — ZephyrAlpha 唯一创建入口（RULE-TW / scaffo... | → | 目录遍历工具 / yaml_utils (_shared/yaml_utils.py) | 导入依赖 / import_depends |
...
| 62 | D_GOV_DRIFT 漂移检测: 校验ssot / validate_ssot (validators/validate_ssot.py) | → | 编码安全工具 / encoding (_shared/encoding.py) | 导入依赖 / import_depends |
...
| 64 | D_GOV_DRIFT 漂移检测: 校验ssot / validate_ssot (validators/validate_ssot.py) | → | 目录遍历工具 / yaml_utils (_shared/yaml_utils.py) | 导入依赖 / import_depends |
...
| 68 | D_GOV_RULE 规则治理: 脚本清单自动生成器 / Script Manifest Generator (generator... | → | 编码安全工具 / encoding (_shared/encoding.py) | 导入依赖 / import_depends |
| 69 | D_GOV_RULE 规则治理: 脚本清单自动生成器 / Script Manifest Generator (generator... | → | 编码安全工具 / file_utils (_shared/file_utils.py) | 导入依赖 / import_depends |
| 70 | D_GOV_RULE 规则治理: 脚本清单自动生成器 / Script Manifest Generator (generator... | → | 目录遍历工具 / yaml_utils (_shared/yaml_utils.py) | 导入依赖 / import_depends |
| 71 | D_OPS 反馈循环: 门禁缓存 / Module docstring — see module-level docstring... | → | 常量 / constants (_shared/constants.py) | 导入依赖 / import_depends |
| 72 | D_OPS 反馈循环: 门禁缓存 / Module docstring — see module-level docstring... | → | 编码安全工具 / file_utils (_shared/file_utils.py) | 导入依赖 / import_depends |
