...
> 展示全部 385 个模块（生产态 384 + 设计态 1），含跨域依赖外部节点。节点含成熟度+名称+大白话/简介+文件路径。

```mermaid
%%{init: {'theme': 'base', 'themeVariables': {'primaryColor': '#eaeaea', 'primaryTextColor': '#333333', 'primaryBorderColor': '#666666', 'lineColor': '#666666', 'secondaryColor': '#eaeaea', 'tertiaryColor': '#eaeaea', 'fontSize': '14px'}}}%%
flowchart TD
    docs_01_policies_and_standards_registry_catalogs_scripts_registry_yaml["(生产态 / production) scripts注册表 / scripts_registry<br/>scripts注册表，机器学习的注册表，登记和查询已注册的条目。<br/>文件: catalogs/scripts_registry.yaml"]
    scripts_archive_governance_dm106_p2b_verification_py["(生产态 / production) DM-106: P2-B 迁移全量验证脚本 / dm106_p2b_verification<br/>DM-106: P2-B 迁移全量验证脚本<br/>文件: governance/dm106_p2b_verification.py"]
    scripts_governance_archive_one_off_audit_post_sync_commands_py["(生产态 / production) 审计提交同步commands.py — post同步 / audit_post_sync_commands<br/>post_sync_standard 命令可执行性巡检（防幻觉/CLI漂移）<br/>文件: one_off/audit_post_sync_commands.py"]
    scripts_governance_archive_one_off_check_exam_case_consistency_py["(生产态 / production) 考试题库一致性检查——根因治本，防止'定义-注册脱钩'复发。 / check_exam_case_consistency<br/>考试题库一致性检查——根因治本，防止'定义-注册脱钩'复发。<br/>文件: one_off/check_exam_case_consistency.py"]
    scripts_governance_archive_one_off_create_alignment_tasks_py["(生产态 / production) 创建alignment任务 / # (BLUEPRINT) MOD-INF-005 / scripts/governance/create_alignm<br/>创建alignment任务，治理的功能模块。<br/>文件: one_off/create_alignment_tasks.py"]
    scripts_governance_archive_one_off_dm105_depgraph_triage_py["(生产态 / production) DM-105: depgraph 未分配节点三策略处理脚本 / dm105_depgraph_triage<br/>DM-105: depgraph 未分配节点三策略处理脚本<br/>文件: one_off/dm105_depgraph_triage.py"]
    scripts_governance_archive_one_off_fix_broken_post_sync_py["(生产态 / production) 修复broken提交sync.py — 批量修复历史 broken  / fix_broken_post_sync<br/>批量修复历史 broken post_sync_standard 命令<br/>文件: one_off/fix_broken_post_sync.py"]
    scripts_governance_archive_one_off_list_phase0_tasks_py["(生产态 / production) (INVARIANTS) 仅查询不修改; 连接失败→exit 1 / list_phase0_tasks<br/>(INVARIANTS) 仅查询不修改; 连接失败→exit 1<br/>文件: one_off/list_phase0_tasks.py"]
    scripts_governance_archive_one_off_phase_a_backup_py["(生产态 / production) 阶段abackup.py — 阶段A安全网 Tier0/Tier1 关 / phase_a_backup<br/>阶段A安全网 Tier0/Tier1 关键文件备份<br/>文件: one_off/phase_a_backup.py"]
    scripts_governance_archive_one_off_rename_kebab_to_snake_py["(生产态 / production) renamekebabtosnake.py — 全项目文件名/目录名 ke / rename_kebab_to_snake<br/>全项目文件名/目录名 kebab-case → snake_case 批量重命名。<br/>文件: one_off/rename_kebab_to_snake.py"]
    scripts_governance_archive_one_off_rename_whitelist_cleanup_py["(生产态 / production) 命名规范白名单清理 - 全文替换脚本。 / rename_whitelist_cleanup<br/>命名规范白名单清理 - 全文替换脚本。<br/>文件: one_off/rename_whitelist_cleanup.py"]
    scripts_governance_archive_one_off_test_lock_scenarios_py["(生产态 / production) 测试锁scenarios.py — RULE-ZERO 锁协议场景 / test_lock_scenarios<br/>RULE-ZERO 锁协议场景 B/C 验证<br/>文件: one_off/test_lock_scenarios.py"]
    scripts_governance_archive_one_off_verify_final_delivery_py["(生产态 / production) (INVARIANTS) 设计态节点数>=1128; 规则表各表>0 / verify_final_delivery<br/>(INVARIANTS) 设计态节点数>=1128; 规则表各表>0<br/>文件: one_off/verify_final_delivery.py"]
    scripts_governance_archive_one_off_verify_rule_yaml_migration_py["(生产态 / production) 验证规则yaml迁移 / verify_rule_yaml_migration.py - 6-dimensional verification o<br/>验证规则yaml迁移，治理的功能模块。<br/>文件: one_off/verify_rule_yaml_migration.py"]
    scripts_governance_archive_prototype_adversarial_log_py["(生产态 / production) 红白对抗闭环记录——攻击→根源分析→修复→回归验证→知识注入全链路追踪 / adversarial_log<br/>红白对抗闭环记录——攻击→根源分析→修复→回归验证→知识注入全链路追踪<br/>文件: prototype/adversarial_log.py"]
    scripts_governance_archive_prototype_audit_domain_nodes_py["(生产态 / production) 审计域节点 / SRC-100200: Audit 13 over-capacity domains granularity distr<br/>审计域节点。SRC-100200: Audit 13 over-capacity domains granularity distribution.<br/>文件: prototype/audit_domain_nodes.py"]
    scripts_governance_archive_prototype_changelog_py["(生产态 / production) changelog.py — 治理域变更日志生成/追加工具. / changelog<br/>治理域变更日志生成/追加工具.<br/>文件: prototype/changelog.py"]
    scripts_governance_archive_prototype_check_audit_rbac_isolation_py["(生产态 / production) 检查审计rbacisolation.py — 静态分析 aud / check_audit_rbac_isolation<br/>静态分析 audit-trail 是否直接 import agent-rbac.<br/>文件: prototype/check_audit_rbac_isolation.py"]
    scripts_governance_archive_prototype_construction_gate_py["(生产态 / production) Construction Gate — 施工前路径校验门禁 / construction_gate<br/>Construction Gate — 施工前路径校验门禁<br/>文件: prototype/construction_gate.py"]
    scripts_governance_archive_prototype_generate_asset_index_py["(生产态 / production) 全项目资产索引生成器 / generate_asset_index<br/>全项目资产索引生成器<br/>文件: prototype/generate_asset_index.py"]
    scripts_governance_archive_prototype_generate_nav_table_py["(生产态 / production) 生成navtable.py — 全流程导航表自动生成器 v1.0 / generate_nav_table<br/>全流程导航表自动生成器 v1.0.0<br/>文件: prototype/generate_nav_table.py"]
    scripts_governance_archive_prototype_rebuild_audit_index_py["(生产态 / production) scripts/governance/rebuild审计index.p / rebuild_audit_index<br/>重建 audit-trail SQLite 派生索引<br/>文件: prototype/rebuild_audit_index.py"]
    scripts_governance_archive_prototype_scan_ground_truth_deps_py["(生产态 / production) 扫描groundtruthdeps / # (BLUEPRINT) MOD-INF-005 / scripts/governance/scan_ground_t<br/>扫描groundtruthdeps，治理的功能模块。<br/>文件: prototype/scan_ground_truth_deps.py"]
    scripts_governance_archive_prototype_session_simulator_py["(生产态 / production) 会话simulator — 30 个模拟开发 session 的蓝图 / session_simulator<br/>session_simulator — 30 个模拟开发 session 的蓝图读取事件生成器<br/>文件: prototype/session_simulator.py"]
    scripts_governance_archive_prototype_sync_blueprint_status_py["(生产态 / production) 机械强制：constructionplan=phase2complete  / sync_blueprint_status<br/>机械强制：construction_plan=phase_2_complete → blueprint.status=Active.<br/>文件: prototype/sync_blueprint_status.py"]
    scripts_governance_archive_vms_ri_ri_build_completion_check_py["(生产态 / production) Runtime Integration Phase 2 完工验证 — MOD-I / ri_build_completion_check<br/>Runtime Integration Phase 2 完工验证 — MOD-INF-002<br/>文件: vms_ri/ri_build_completion_check.py"]
    scripts_governance_archive_vms_ri_vms_blindspot_check_py["(生产态 / production) VMS 盲点闭合检查器 — MOD-INF-011 · R1(33) + R2( / vms_blindspot_check<br/>VMS 盲点闭合检查器 — MOD-INF-011 · R1(33) + R2(22) + R4(6)<br/>文件: vms_ri/vms_blindspot_check.py"]
    scripts_governance_archive_vms_ri_vms_build_completion_check_py["(生产态 / production) vms构建completion检查 / VMS Build Completion Check — MOD-INF-011 · TASK-INF-0217<br/>vms构建completion检查。VMS Build Completion Check — MOD-INF-011 · TASK-INF-0217<br/>文件: vms_ri/vms_build_completion_check.py"]
    scripts_governance_archive_vms_ri_vms_cron_monitor_py["(生产态 / production) VMS Cron 监控器 — MOD-INF-011 · TASK-INF-02 / vms_cron_monitor<br/>VMS Cron 监控器 — MOD-INF-011 · TASK-INF-0224<br/>文件: vms_ri/vms_cron_monitor.py"]
    scripts_governance_archive_vms_ri_vms_health_check_py["(生产态 / production) VMS Health Check 脚本 — MOD-INF-011 · Phas / vms_health_check<br/>VMS Health Check 脚本 — MOD-INF-011 · Phase 3 运维自动化<br/>文件: vms_ri/vms_health_check.py"]
    scripts_governance_archive_vms_ri_vms_migrate_py["(生产态 / production) VMS Phase 2 数据迁移脚本 — MOD-INF-011 / vms_migrate<br/>VMS Phase 2 数据迁移脚本 — MOD-INF-011<br/>文件: vms_ri/vms_migrate.py"]
    scripts_governance_archive_vms_ri_vms_migration_dry_run_py["(生产态 / production) VMS 迁移 dry-run 脚本 — MOD-INF-011 Phase 2  / vms_migration_dry_run<br/>VMS 迁移 dry-run 脚本 — MOD-INF-011 Phase 2 前置检查<br/>文件: vms_ri/vms_migration_dry_run.py"]
    scripts_governance_archive_vms_ri_vms_phase_rollback_py["(生产态 / production) VMS Phase 回滚方案 — MOD-INF-011 · TASK-INF- / vms_phase_rollback<br/>VMS Phase 回滚方案 — MOD-INF-011 · TASK-INF-0217<br/>文件: vms_ri/vms_phase_rollback.py"]
    scripts_governance_archive_vms_ri_vms_version_sync_check_py["(生产态 / production) VMS 版本同步检查器 — MOD-INF-011 · TASK-INF-022 / vms_version_sync_check<br/>VMS 版本同步检查器 — MOD-INF-011 · TASK-INF-0222<br/>文件: vms_ri/vms_version_sync_check.py"]
    scripts_governance_shared_base_py["(生产态 / production) base.py — 审计脚本基类 / base<br/>审计脚本基类<br/>文件: _shared/base.py"]
    scripts_governance_shared_module_translation_loader_py["(生产态 / production) 模块translationloader.py — 模块级翻译共享加载 / module_translation_loader<br/>模块级翻译共享加载器（SSoT 真源）<br/>文件: _shared/module_translation_loader.py"]
    scripts_governance_sync_check_p0_status_py["(生产态 / production) 检查p0状态 / Module docstring — see module-level docstring for details.<br/>检查p0状态。Module docstring — see module-level docstring for details.<br/>文件: _sync/check_p0_status.py"]
    scripts_governance_sync_cleanup_p0_ops_pending_py["(生产态 / production) 清理p0运维pending.py - 一次性：将所有 OPS- / cleanup_p0_ops_pending<br/>一次性：将所有 OPS-* P0+PENDING 任务降级+完成<br/>文件: _sync/cleanup_p0_ops_pending.py"]
    scripts_governance_tasks_list_phase0_tasks_py["(生产态 / production) (INVARIANTS) 仅查询不修改; 连接失败→exit 1 / list_phase0_tasks<br/>(INVARIANTS) 仅查询不修改; 连接失败→exit 1<br/>文件: _tasks/list_phase0_tasks.py"]
    scripts_governance_tasks_task_show_py["(生产态 / production) governance/taskshow 脚本 — 任务卡详情查询 CLI。 / task_show<br/>governance/task_show 脚本 — 任务卡详情查询 CLI。<br/>文件: _tasks/task_show.py"]
    scripts_governance_tasks_task_summary_py["(生产态 / production) 任务summary.py — 任务系统全局摘要 CLI / task_summary<br/>任务系统全局摘要 CLI<br/>文件: _tasks/task_summary.py"]
    scripts_governance_add_deferred_design_edges_py["(生产态 / production) 为暂缓模块添加设计态依赖边（depmaturity='design'）。 / add_deferred_design_edges<br/>为暂缓模块添加设计态依赖边（dep_maturity='design'）。<br/>文件: governance/add_deferred_design_edges.py"]
    scripts_governance_apply_dataflowgraph_py["(生产态 / production) 应用dataflowgraph.py — dataflowgraph 变 / apply_dataflowgraph<br/>dataflowgraph 变更写入工具<br/>文件: governance/apply_dataflowgraph.py"]
    scripts_governance_apply_decisiongraph_py["(生产态 / production) (INVARIANTS) pgadvisorylock 写锁; build / apply_decisiongraph<br/>(INVARIANTS) pg_advisory_lock 写锁; build_status 单调推进; DEC-INV-001~005 校验; 事务回滚<br/>文件: governance/apply_decisiongraph.py"]
    scripts_governance_apply_depgraph_py["(生产态 / production) (INVARIANTS) 原子写入（RULE-ONE）；变更前验证；禁止直接覆盖 / apply_depgraph<br/>(INVARIANTS) 原子写入（RULE-ONE）；变更前验证；禁止直接覆盖<br/>文件: governance/apply_depgraph.py"]
    scripts_governance_architecture_health_dashboard_py["(生产态 / production) 架构健康dashboard.py — 架构健康度 / architecture_health_dashboard<br/>架构健康度仪表盘（自动化检测基线）<br/>文件: governance/architecture_health_dashboard.py"]
    scripts_governance_ast_import_rewriter_py["(生产态 / production) ast导入rewriter / AST-based import rewriter for governance directory migration<br/>ast导入rewriter。AST-based import rewriter for governance directory migration.<br/>文件: governance/ast_import_rewriter.py"]
    scripts_governance_audit_return_contract_usage_py["(生产态 / production) 审计return契约usage.py — 返回契约 ok / audit_return_contract_usage<br/>返回契约 ok 键调用方审计<br/>文件: governance/audit_return_contract_usage.py"]
    scripts_governance_audit_worktree_ops_telemetry_py["(生产态 / production) 审计worktree运维telemetry.py — 主工作区文件 / audit_worktree_ops_telemetry<br/>主工作区文件级擦除操作遥测完整性审计<br/>文件: governance/audit_worktree_ops_telemetry.py"]
    scripts_governance_check_commit_message_py["(生产态 / production) 检查提交message / check_commit_message.py — GitHub Actions PR commit message g<br/>从 commit message 提取 (GW:session_id) 标记中的 session_id。<br/>文件: governance/check_commit_message.py"]
    scripts_governance_check_ssot_gate_py["(生产态 / production) GATE-SSOT: SSoT 创建门禁（pre-commit hook 双保险 / check_ssot_gate<br/>GATE-SSOT: SSoT 创建门禁（pre-commit hook 双保险）。<br/>文件: governance/check_ssot_gate.py"]
    scripts_governance_d10_performance_collect_system_threads_py["(生产态 / production) collectsystemthreads.py — 全系统线程数快照采集器 / collect_system_threads<br/>全系统线程数快照采集器<br/>文件: d10_performance/collect_system_threads.py"]
    scripts_governance_d11_compliance_audit_registration_py["(生产态 / production) 审计registration.py — 孤儿注册检测（RULE-TWO  / audit_registration<br/>孤儿注册检测（RULE-TWO 防线 2）<br/>文件: d11_compliance/audit_registration.py"]
    scripts_governance_d11_compliance_ci_self_check_py["(生产态 / production) CI Entry: Self-Check — Drift Detector 自身 / ci_self_check<br/>CI Entry: Self-Check — Drift Detector 自身完整性验证<br/>文件: d11_compliance/ci_self_check.py"]
    scripts_governance_d11_compliance_fix_shared_bypass_py["(生产态 / production) 修复共享bypass / fix_shared_bypass.py - D-D-07 auto-fix tool (validate_script<br/>检测赋值节点是否包含 Path(__file__).parents(N) 模式（不限变量名）。<br/>文件: d11_compliance/fix_shared_bypass.py"]
    scripts_governance_d11_compliance_g9_compliance_check_py["(生产态 / production) G9 四蓝图跨模块集成合规门禁执行器. / g9_compliance_check<br/>G9 四蓝图跨模块集成合规门禁执行器.<br/>文件: d11_compliance/g9_compliance_check.py"]
    scripts_governance_d11_compliance_task_self_check_py["(生产态 / production) 任务自check.py — 任务系统自身健康检查 / task_self_check<br/>任务系统自身健康检查<br/>文件: d11_compliance/task_self_check.py"]
    scripts_governance_d11_compliance_validate_commit_gateway_py["(生产态 / production) 校验提交gateway.py — GATE-COMMIT / validate_commit_gateway<br/>GATE-COMMIT-GW 门禁<br/>文件: d11_compliance/validate_commit_gateway.py"]
    scripts_governance_d11_compliance_validate_commit_message_py["(生产态 / production) 校验提交message.py — Conventiona / validate_commit_message<br/>Conventional Commits 校验（commit-msg hook）+ AI 归因 trailer 检测（warn-only）<br/>文件: d11_compliance/validate_commit_message.py"]
    scripts_governance_d11_compliance_validate_exit_codes_py["(生产态 / production) 校验出口codes.py — 审计脚本退出码规范门禁 / validate_exit_codes<br/>审计脚本退出码规范门禁<br/>文件: d11_compliance/validate_exit_codes.py"]
    scripts_governance_d11_compliance_validate_frozen_requirements_py["(生产态 / production) 校验frozenrequirements.py — 依赖版本锁定 / validate_frozen_requirements<br/>依赖版本锁定与验证（蓝图 §34.2）<br/>文件: d11_compliance/validate_frozen_requirements.py"]
    scripts_governance_d11_compliance_validate_manifest_admission_py["(生产态 / production) 校验manifest准入 / Module docstring — see module-level docstring for details.<br/>校验manifest准入。Module docstring — see module-level docstring for details.<br/>文件: d11_compliance/validate_manifest_admission.py"]
    scripts_governance_d11_compliance_validate_no_utf8_bom_py["(生产态 / production) 校验noutf8bom.py — UTF-8 BOM 检测门禁 / validate_no_utf8_bom<br/>UTF-8 BOM 检测门禁<br/>文件: d11_compliance/validate_no_utf8_bom.py"]
    scripts_governance_d11_compliance_validate_script_naming_py["(生产态 / production) 校验scriptnaming.py — 审计脚本命名规范门禁 / validate_script_naming<br/>审计脚本命名规范门禁<br/>文件: d11_compliance/validate_script_naming.py"]
    scripts_governance_d11_compliance_validate_script_quality_py["(生产态 / production) 校验scriptquality.py — 治理脚本质量合规检查 / validate_script_quality<br/>治理脚本质量合规检查<br/>文件: d11_compliance/validate_script_quality.py"]
    scripts_governance_d11_compliance_validate_task_decomposition_bypass_py["(生产态 / production) 校验任务decompositionbypass.py —  / validate_task_decomposition_bypass<br/>Task Decomposition Bypass 检测<br/>文件: d11_compliance/validate_task_decomposition_bypass.py"]
    scripts_governance_d11_compliance_validate_vocabulary_coverage_py["(生产态 / production) 校验vocabularycoverage / Module docstring — see module-level docstring for details.<br/>校验vocabularycoverage。Module docstring — see module-level docstring for details.<br/>文件: d11_compliance/validate_vocabulary_coverage.py"]
    scripts_governance_d11_compliance_verify_audit_integrity_py["(生产态 / production) 验证审计integrity.py — MOD-INF-020  / verify_audit_integrity<br/>MOD-INF-020 · 零依赖外部独立验证器<br/>文件: d11_compliance/verify_audit_integrity.py"]
    scripts_governance_d11_compliance_verify_schema_health_py["(生产态 / production) 验证结构health.py — depgraph (Post / verify_schema_health<br/>depgraph (PostgreSQL) Schema 健康度校验门禁（#ARCH-016 治本）<br/>文件: d11_compliance/verify_schema_health.py"]
    scripts_governance_d12_ai_hallucination_check_logger_kwargs_py["(生产态 / production) 检查日志器kwargs / ========================================================<br/>检查日志器kwargs。========================================================<br/>文件: d12_ai_hallucination/check_logger_kwargs.py"]
    scripts_governance_d12_ai_hallucination_validate_gate_prompt_conflict_py["(生产态 / production) 校验门禁提示conflict.py — Gate- / validate_gate_prompt_conflict<br/>Gate-Prompt 冲突检测<br/>文件: d12_ai_hallucination/validate_gate_prompt_conflict.py"]
    scripts_governance_d12_ai_hallucination_validate_session_budget_py["(生产态 / production) 校验会话budget.py — Session 操作预 / validate_session_budget<br/>Session 操作预算校验（已废弃）<br/>文件: d12_ai_hallucination/validate_session_budget.py"]
    scripts_governance_d12_ai_hallucination_validate_session_gate_check_py["(生产态 / production) 校验会话门禁check.py — Session / validate_session_gate_check<br/>Session 门禁检查完整性校验<br/>文件: d12_ai_hallucination/validate_session_gate_check.py"]
    scripts_governance_d1_structure_archive_drafts_zone_py["(生产态 / production) 草稿区生命周期归档器——扫描 arbitrated 草稿，按 age 判定 wa / archive_drafts_zone<br/>草稿区生命周期归档器——扫描 arbitrated 草稿，按 age 判定 warn/archive/skip。<br/>文件: d1_structure/archive_drafts_zone.py"]
    scripts_governance_d1_structure_audit_config_format_py["(生产态 / production) 审计配置format.py — config/ 目录格式/注释 / audit_config_format<br/>config/ 目录格式/注释/边界快速扫描<br/>文件: d1_structure/audit_config_format.py"]
    scripts_governance_d1_structure_audit_directory_integrity_py["(生产态 / production) 审计directoryintegrity.py — 01polici / audit_directory_integrity<br/>01_policies_and_standards/ 目录结构完整性审计<br/>文件: d1_structure/audit_directory_integrity.py"]
    scripts_governance_d1_structure_audit_directory_scalability_py["(生产态 / production) 审计directoryscalability.py -- 物理结构可扩 / audit_directory_scalability<br/>- 物理结构可扩展性审计 (1500模块支撑能力检查)<br/>文件: d1_structure/audit_directory_scalability.py"]
    scripts_governance_d1_structure_audit_findings_by_scope_py["(生产态 / production) 审计findingsbyscope.py — 按目录范围筛选 Fin / audit_findings_by_scope<br/>按目录范围筛选 Finding 报告<br/>文件: d1_structure/audit_findings_by_scope.py"]
    scripts_governance_d1_structure_batch_create_index_md_py["(生产态 / production) 批次创建索引md / Batch create index.md for all directories under docs/ that l<br/>批次创建索引md。Batch create index.md for all directories under docs/ that lack one.<br/>文件: d1_structure/batch_create_index_md.py"]
    scripts_governance_d1_structure_cbg_reset_py["(生产态 / production) CBG 熔断器重置 CLI (CircuitBreakerGateway Res / cbg_reset<br/>CBG 熔断器重置 CLI (CircuitBreakerGateway Reset Command)<br/>文件: d1_structure/cbg_reset.py"]
    scripts_governance_d1_structure_check_directory_contract_py["(生产态 / production) 检查directory契约 / GATE-DIRECTORY-CONTRACT: Directory Contract validation gate.<br/>检查directory契约。GATE-DIRECTORY-CONTRACT: Directory Contract validation gate.<br/>文件: d1_structure/check_directory_contract.py"]
    scripts_governance_d1_structure_check_handoff_manifests_py["(生产态 / production) 检查handoffmanifests.py — AI Session  / check_handoff_manifests<br/>AI Session Handoff Manifest 完整性校验.<br/>文件: d1_structure/check_handoff_manifests.py"]
    scripts_governance_d1_structure_check_index_integrity_py["(生产态 / production) 检查索引integrity.py — 索引完整性校验 / check_index_integrity<br/>索引完整性校验<br/>文件: d1_structure/check_index_integrity.py"]
    scripts_governance_d1_structure_cleanup_stash_py["(生产态 / production) 清理stash.py — git stash 堆积治理（OPS-20 / cleanup_stash<br/>git stash 堆积治理（OPS-2026062501 治本）<br/>文件: d1_structure/cleanup_stash.py"]
    scripts_governance_d1_structure_detect_orphan_py_py["(生产态 / production) 检测orphanpy.py — 全库孤儿 .py 文件检测 / detect_orphan_py<br/>全库孤儿 .py 文件检测<br/>文件: d1_structure/detect_orphan_py.py"]
    scripts_governance_d1_structure_detect_residual_files_py["(生产态 / production) 检测residualfiles.py — 残留物检测 / detect_residual_files<br/>残留物检测<br/>文件: d1_structure/detect_residual_files.py"]
    scripts_governance_d1_structure_detect_temp_files_py["(生产态 / production) 检测tempfiles / Module docstring — see module-level docstring for details.<br/>检测tempfiles。Module docstring — see module-level docstring for details.<br/>文件: d1_structure/detect_temp_files.py"]
    scripts_governance_d1_structure_drafts_zone_archiver_py["(生产态 / production) 草稿区生命周期归档器 (Drafts Zone Lifecycle Archiv / drafts_zone_archiver<br/>草稿区生命周期归档器 (Drafts Zone Lifecycle Archiver · V-16)<br/>文件: d1_structure/drafts_zone_archiver.py"]
    scripts_governance_d1_structure_generate_missing_index_md_py["(生产态 / production) 生成missing索引md.py — 扫描目录树，为缺失 / generate_missing_index_md<br/>扫描目录树，为缺失 index.md 的目录自动生成索引文件。<br/>文件: d1_structure/generate_missing_index_md.py"]
    scripts_governance_d1_structure_reset_cbg_py["(生产态 / production) CBG 熔断器重置 CLI (CircuitBreakerGateway Res / reset_cbg<br/>CBG 熔断器重置 CLI (CircuitBreakerGateway Reset Command)<br/>文件: d1_structure/reset_cbg.py"]
    scripts_governance_d1_structure_run_script_smoke_test_py["(生产态 / production) 运行scriptsmoketest.py — 治理脚本冒烟测试运行器 / run_script_smoke_test<br/>治理脚本冒烟测试运行器<br/>文件: d1_structure/run_script_smoke_test.py"]
    scripts_governance_d1_structure_sync_index_from_manifest_py["(生产态 / production) 同步索引frommanifest.py — 从 scriptm / sync_index_from_manifest<br/>从 script_manifest.yaml (SSoT) 自动同步 index.md 的脚本数量。<br/>文件: d1_structure/sync_index_from_manifest.py"]
    scripts_governance_d1_structure_sync_policies_index_py["(生产态 / production) 同步policiesindex.py — 从磁盘实际扫描，自动同步 PS / sync_policies_index<br/>从磁盘实际扫描，自动同步 PS-IDX-001 §二 文件数量表格。<br/>文件: d1_structure/sync_policies_index.py"]
    scripts_governance_d1_structure_validate_config_integrity_py["(生产态 / production) 校验配置integrity.py — 运行时配置完整性十 / validate_config_integrity<br/>运行时配置完整性十一层纵深审计 + 自动同步检测<br/>文件: d1_structure/validate_config_integrity.py"]
    scripts_governance_d1_structure_validate_d1_output_sanity_py["(生产态 / production) 校验d1outputsanity.py — D1 产出物合理性 / validate_d1_output_sanity<br/>D1 产出物合理性校验（蓝图 §31 B93）<br/>文件: d1_structure/validate_d1_output_sanity.py"]
    scripts_governance_d1_structure_validate_immutable_core_py["(生产态 / production) 校验不可变core.py — immutablec / validate_immutable_core<br/>immutable_core 文件修改检测<br/>文件: d1_structure/validate_immutable_core.py"]
    scripts_governance_d1_structure_validate_index_reality_py["(生产态 / production) 校验索引reality / Module docstring — see module-level docstring for details.<br/>校验索引reality。Module docstring — see module-level docstring for details.<br/>文件: d1_structure/validate_index_reality.py"]
    scripts_governance_d1_structure_validate_read_before_write_py["(生产态 / production) 校验readbeforewrite.py — 先读后写校验（I / validate_read_before_write<br/>先读后写校验<br/>文件: d1_structure/validate_read_before_write.py"]
    scripts_governance_d2_links_audit_broken_links_py["(生产态 / production) 检测文档/数据文件中的断链与幽灵引用。 / audit_broken_links<br/>检测文档/数据文件中的断链与幽灵引用。<br/>文件: d2_links/audit_broken_links.py"]
    scripts_governance_d2_links_detect_relative_references_py["(生产态 / production) 检测relativereferences.py — 相对路径引用检测 / detect_relative_references<br/>相对路径引用检测<br/>文件: d2_links/detect_relative_references.py"]
    scripts_governance_d3_metadata_auto_generate_index_py["(生产态 / production) 自动生成索引 / GATE-INDEX: Validate and auto-fix index.md factual accuracy.<br/>自动生成索引。GATE-INDEX: Validate and auto-fix index.md factual accuracy.<br/>文件: d3_metadata/auto_generate_index.py"]
    scripts_governance_d3_metadata_backfill_doctype_metadata_py["(生产态 / production) 批量回填 frontmatter doctype 字段（doctype 存量 / backfill_doctype_metadata<br/>批量回填 frontmatter doc_type 字段（doc_type 存量治理 Stage 2.1）<br/>文件: d3_metadata/backfill_doctype_metadata.py"]
    scripts_governance_d3_metadata_backfill_ttl_metadata_py["(生产态 / production) 批量回填/重判 ttl 字段（6 格式统一入口，GATE-15 存量治理 + G / backfill_ttl_metadata<br/>批量回填/重判 ttl 字段（6 格式统一入口，GATE-15 存量治理 + GATE-VOCAB-CHANGE 纠偏）<br/>文件: d3_metadata/backfill_ttl_metadata.py"]
    scripts_governance_d3_metadata_check_blueprint_compliance_py["(生产态 / production) (INVARIANTS) REQUIREDSECTIONS 必须与蓝图+施工图 / check_blueprint_compliance<br/>(INVARIANTS) REQUIRED_SECTIONS 必须与蓝图+施工图模板 v2.1.0 COMPLIANCE_CHECKLIST 一致<br/>文件: d3_metadata/check_blueprint_compliance.py"]
    scripts_governance_d3_metadata_check_frontmatter_metadata_py["(生产态 / production) GATE-15: Frontmatter metadata validation / check_frontmatter_metadata<br/>GATE-15: Frontmatter metadata validation（ttl + doc_type 字段校验）<br/>文件: d3_metadata/check_frontmatter_metadata.py"]
    scripts_governance_d3_metadata_check_module_singlesource_py["(生产态 / production) GATE-SSOT-SINGLESOURCE: SSoT 单一真源门禁（Phas / check_module_singlesource<br/>GATE-SSOT-SINGLESOURCE: SSoT 单一真源门禁（Phase 7 治本防复发）。<br/>文件: d3_metadata/check_module_singlesource.py"]
    scripts_governance_d3_metadata_check_naming_convention_py["(生产态 / production) GATE-11 命名规范门禁 — 全类型命名检测。 / check_naming_convention<br/>GATE-11 命名规范门禁 — 全类型命名检测。<br/>文件: d3_metadata/check_naming_convention.py"]
    scripts_governance_d3_metadata_check_registry_consistency_py["(生产态 / production) 检查注册表consistency — 跨登记表一致性校验。 / check_registry_consistency<br/>check_registry_consistency — 跨登记表一致性校验。<br/>文件: d3_metadata/check_registry_consistency.py"]
    scripts_governance_d3_metadata_check_schema_version_writes_py["(生产态 / production) GTRAE059 验证脚本：结构version 写入保护 + 版 / check_schema_version_writes<br/>G_TRAE_059 验证脚本：_schema_version 写入保护 + 版本一致性检查。<br/>文件: d3_metadata/check_schema_version_writes.py"]
    scripts_governance_d3_metadata_check_vocab_hardcode_py["(生产态 / production) GATE-VOCAB: 词表合法值硬编码检测（trae060 §2） / check_vocab_hardcode<br/>GATE-VOCAB: 词表合法值硬编码检测（trae_060 §2）<br/>文件: d3_metadata/check_vocab_hardcode.py"]
    scripts_governance_d3_metadata_classify_ttl_by_content_py["(生产态 / production) 基于内容关键词的 ttl 精细分类审查脚本。 / classify_ttl_by_content<br/>基于内容关键词的 ttl 精细分类审查脚本。<br/>文件: d3_metadata/classify_ttl_by_content.py"]
    scripts_governance_d3_metadata_deep_content_scanner_py["(生产态 / production) deepcontentscanner.py — 深度内容扫描器 / deep_content_scanner<br/>深度内容扫描器<br/>文件: d3_metadata/deep_content_scanner.py"]
    scripts_governance_d3_metadata_generate_derived_files_py["(生产态 / production) 生成derivedfiles.py — 枚举自动派生生成器（Le / generate_derived_files<br/>枚举自动派生生成器（Level 3 终极防御）<br/>文件: d3_metadata/generate_derived_files.py"]
    scripts_governance_d3_metadata_generate_rule_catalog_py["(生产态 / production) 生成规则目录 / Scan docs/01_policies_and_standards and emit _registry/catal<br/>生成规则目录。Scan docs/01_policies_and_standards and emit _registry/catalogs/rule_catalog_registry.yaml.<br/>文件: d3_metadata/generate_rule_catalog.py"]
    scripts_governance_d3_metadata_migrate_illegal_doctype_py["(生产态 / production) 批量迁移非法 doctype 值（doctype 存量治理 Stage 2. / migrate_illegal_doctype<br/>批量迁移非法 doc_type 值（doc_type 存量治理 Stage 2.2）<br/>文件: d3_metadata/migrate_illegal_doctype.py"]
    scripts_governance_d3_metadata_validate_architecture_py["(生产态 / production) 校验架构 / validate_architecture.py - Validate rule files against archi<br/>从 .md / .yaml 文件读取 frontmatter 字段（统一返回 dict）。<br/>文件: d3_metadata/validate_architecture.py"]
    scripts_governance_d3_metadata_validate_blueprint_provenance_py["(生产态 / production) 校验蓝图provenance / Blueprint Provenance Gate - V-12: validate provenance triple<br/>校验蓝图provenance。Blueprint Provenance Gate - V-12: validate provenance triples in blueprint frontmatter<br/>文件: d3_metadata/validate_blueprint_provenance.py"]
    scripts_governance_d3_metadata_validate_module_id_py["(生产态 / production) 校验模块id / GATE-MODULEID: Validate module_id uniqueness and index/file <br/>校验模块id。GATE-MODULEID: Validate module_id uniqueness and index/file consistency.<br/>文件: d3_metadata/validate_module_id.py"]
    scripts_governance_d3_metadata_validate_registry_master_index_py["(生产态 / production) 登记表总索引自校验门禁 (Registry Master Index Self- / validate_registry_master_index<br/>登记表总索引自校验门禁 (Registry Master Index Self-Check Gate · V-18).<br/>文件: d3_metadata/validate_registry_master_index.py"]
    scripts_governance_d3_metadata_validate_tool_contracts_consistency_py["(生产态 / production) Tool Contract 一致性校验脚本（MOD-INF-013 §9 R3） / validate_tool_contracts_consistency<br/>Tool Contract 一致性校验脚本（MOD-INF-013 §9 R3）。<br/>文件: d3_metadata/validate_tool_contracts_consistency.py"]
    scripts_governance_d4_paths_detect_deprecated_path_writes_py["(生产态 / production) 检测废弃路径writes.py — 废弃路径写 / detect_deprecated_path_writes<br/>废弃路径写入检测<br/>文件: d4_paths/detect_deprecated_path_writes.py"]
    scripts_governance_d4_paths_detect_excessive_file_moves_py["(生产态 / production) 检测excessive文件moves.py — 文件过度搬迁检 / detect_excessive_file_moves<br/>文件过度搬迁检测<br/>文件: d4_paths/detect_excessive_file_moves.py"]
    scripts_governance_d4_paths_detect_ruins_references_py["(生产态 / production) 检测ruinsreferences.py — 残骸/废弃路径引用检测 / detect_ruins_references<br/>残骸/废弃路径引用检测<br/>文件: d4_paths/detect_ruins_references.py"]
    scripts_governance_d4_paths_detect_split_delete_ref_commit_py["(生产态 / production) 检测拆分删除refcommit.py — 删除引用 / detect_split_delete_ref_commit<br/>删除引用分离提交检测<br/>文件: d4_paths/detect_split_delete_ref_commit.py"]
    scripts_governance_d5_architecture_analyze_change_impact_py["(生产态 / production) analyze变更冲击 / Module docstring — see module-level docstring for details.<br/>analyze变更冲击。Module docstring — see module-level docstring for details.<br/>文件: d5_architecture/analyze_change_impact.py"]
    scripts_governance_d5_architecture_analyzers_analyze_contract_impact_py["(生产态 / production) analyze契约impact.py — 契约变更影响分析器 / analyze_contract_impact<br/>契约变更影响分析器<br/>文件: analyzers/analyze_contract_impact.py"]
    scripts_governance_d5_architecture_analyzers_audit_depends_on_chain_depth_py["(生产态 / production) 审计depends开链depth.py — depend / audit_depends_on_chain_depth<br/>depends_on 依赖链路深度审计<br/>文件: analyzers/audit_depends_on_chain_depth.py"]
    scripts_governance_d5_architecture_analyzers_measure_deprecation_cascade_py["(生产态 / production) measuredeprecationcascade.py — 废弃级联影响度 / measure_deprecation_cascade<br/>废弃级联影响度量<br/>文件: analyzers/measure_deprecation_cascade.py"]
    scripts_governance_d5_architecture_audit_agent_spec_py["(生产态 / production) (INVARIANTS) agent-spec 审计完整性 / audit_agent_spec<br/>(INVARIANTS) agent-spec 审计完整性<br/>文件: d5_architecture/audit_agent_spec.py"]
    scripts_governance_d5_architecture_check_budget_health_py["(生产态 / production) (INVARIANTS) 预算健康检查不可跳过;检查结果必须可机器解析 / check_budget_health<br/>(INVARIANTS) 预算健康检查不可跳过;检查结果必须可机器解析<br/>文件: d5_architecture/check_budget_health.py"]
    scripts_governance_d5_architecture_check_drift_e2e_py["(生产态 / production) 检查漂移端到端 / CI Entry: Drift Detector E2E Pipeline Check<br/>检查漂移端到端。CI Entry: Drift Detector E2E Pipeline Check<br/>文件: d5_architecture/check_drift_e2e.py"]
    scripts_governance_d5_architecture_checkers_check_architecture_gates_py["(生产态 / production) 检查架构门禁 / v2.4.0 — 2026-05-03<br/>检查架构门禁。v2.4.0 — 2026-05-03<br/>文件: checkers/check_architecture_gates.py"]
    scripts_governance_d5_architecture_checkers_check_blueprint_automation_sync_py["(生产态 / production) (INVARIANTS) 蓝图§5.5自动化触发机制状态列必须与代码实际实现一致 / check_blueprint_automation_sync<br/>(INVARIANTS) 蓝图§5.5自动化触发机制状态列必须与代码实际实现一致; ⚠️待实现但代码已实现=DRIFT; ✅已实现但代码不存在=DRIFT<br/>文件: checkers/check_blueprint_automation_sync.py"]
    scripts_governance_d5_architecture_checkers_check_blueprint_code_alignment_py["(生产态 / production) (INVARIANTS) 代码(BLUEPRINT)头部moduleid必须与 / check_blueprint_code_alignment<br/>(INVARIANTS) 代码(BLUEPRINT)头部module_id必须与蓝图注册表一致; 蓝图§4已实现文件必须在磁盘存在; frontmatter.build_status 必须与 depgraph 聚合 build_status<br/>文件: checkers/check_blueprint_code_alignment.py"]
    scripts_governance_d5_architecture_checkers_check_blueprint_template_compliance_py["(生产态 / production) (INVARIANTS) 蓝图模板合规检查不可绕过;52项检查全覆盖 / check_blueprint_template_compliance<br/>(INVARIANTS) 蓝图模板合规检查不可绕过;52项检查全覆盖<br/>文件: checkers/check_blueprint_template_compliance.py"]
    scripts_governance_d5_architecture_checkers_check_canonical_yaml_drift_py["(生产态 / production) 检查canonicalyaml漂移 / check_canonical_yaml_drift.py — GATE-CANONICAL-YAML-DRIFT<br/>安全加载 YAML，返回解析对象（dict/list）。<br/>文件: checkers/check_canonical_yaml_drift.py"]
    scripts_governance_d5_architecture_checkers_check_code_duplication_py["(生产态 / production) (INVARIANTS) 扫描 src/zephyr/ 下所有包; 检测跨包同名 / check_code_duplication<br/>(INVARIANTS) 扫描 src/zephyr/ 下所有包; 检测跨包同名文件代码重复<br/>文件: checkers/check_code_duplication.py"]
    scripts_governance_d5_architecture_checkers_check_contract_code_drift_py["(生产态 / production) 检查契约代码drift.py —— 契约-代码双写漂 / check_contract_code_drift<br/>— 契约-代码双写漂移阻断（盲点 C2 修复）<br/>文件: checkers/check_contract_code_drift.py"]
    scripts_governance_d5_architecture_checkers_check_contract_physical_path_py["(生产态 / production) 检查契约physical路径 / check_contract_physical_path.py — GATE-CONTRACT-PHYSICAL-PAT<br/>检查单个 physical_path 是否含连字符目录段, 返回违规信息列表.<br/>文件: checkers/check_contract_physical_path.py"]
    scripts_governance_d5_architecture_checkers_check_dependency_direction_py["(生产态 / production) 检查依赖direction.py — 依赖方向校验（I / check_dependency_direction<br/>依赖方向校验<br/>文件: checkers/check_dependency_direction.py"]
    scripts_governance_d5_architecture_checkers_check_g6_ctr_compliance_py["(生产态 / production) 检查g6ctr合规 / check_g6_ctr_compliance.py - G6 CTR Contract Compliance Gate<br/>检查g6ctr合规，治理的检查器，检查某项条件是否满足。<br/>文件: checkers/check_g6_ctr_compliance.py"]
    scripts_governance_d5_architecture_checkers_check_orphan_outputs_py["(生产态 / production) (INVARIANTS) 扫描蓝图 §11 产出物 consumermin;  / check_orphan_outputs<br/>(INVARIANTS) 扫描蓝图 §11 产出物 consumer_min; 检测零消费者孤儿产出物<br/>文件: checkers/check_orphan_outputs.py"]
    scripts_governance_d5_architecture_checkers_check_precommit_id_uniqueness_py["(生产态 / production) 检查precommitiduniqueness / check_precommit_id_uniqueness.py — GATE-ID-UNIQ<br/>扫描 .pre-commit-config.yaml 文本,返回 (line_no, hook_id, repo_url, repo_line) 列表。<br/>文件: checkers/check_precommit_id_uniqueness.py"]
    scripts_governance_d5_architecture_checkers_check_rule_four_way_alignment_py["(生产态 / production) 检查规则fourwayalignment.py —— 规则四方 / check_rule_four_way_alignment<br/>— 规则四方对齐门禁（ARCH-020 补建）<br/>文件: checkers/check_rule_four_way_alignment.py"]
    scripts_governance_d5_architecture_checkers_check_ssot_uniqueness_py["(生产态 / production) (INVARIANTS) 扫描所有蓝图 ssotclaims 字段; 检测跨蓝 / check_ssot_uniqueness<br/>(INVARIANTS) 扫描所有蓝图 ssot_claims 字段; 检测跨蓝图 SSoT 冲突<br/>文件: checkers/check_ssot_uniqueness.py"]
    scripts_governance_d5_architecture_checkers_check_trace_context_propagation_py["(生产态 / production) 检查追踪上下文propagation.py — Tra / check_trace_context_propagation<br/>TraceContext 传播强制执行 CI 检查<br/>文件: checkers/check_trace_context_propagation.py"]
    scripts_governance_d5_architecture_checkers_check_vms_ssot_py["(生产态 / production) GATE-VMS-SSOT: VMS 单一真源门禁——三重检测。 / check_vms_ssot<br/>GATE-VMS-SSOT: VMS 单一真源门禁——三重检测。<br/>文件: checkers/check_vms_ssot.py"]
    scripts_governance_d5_architecture_dependency_graph_py["(生产态 / production) 治理域有向依赖图 — 扫描 governance/ 下所有 import 生成依 / dependency_graph<br/>治理域有向依赖图 — 扫描 governance/ 下所有 import 生成依赖图.<br/>文件: d5_architecture/dependency_graph.py"]
    scripts_governance_d5_architecture_detect_causal_conflicts_py["(生产态 / production) 检测causalconflicts / Module docstring — see module-level docstring for details.<br/>检测causalconflicts。Module docstring — see module-level docstring for details.<br/>文件: d5_architecture/detect_causal_conflicts.py"]
    scripts_governance_d5_architecture_detect_constraint_violations_py["(生产态 / production) G9-Detect: 架构约束违规检测器（对照 depgraph 实际数据检测  / detect_constraint_violations<br/>G9-Detect: 架构约束违规检测器（对照 depgraph 实际数据检测 6 类违规）<br/>文件: d5_architecture/detect_constraint_violations.py"]
    scripts_governance_d5_architecture_detectors_analyze_same_name_module_relations_py["(生产态 / production) analyzesamename模块relations.py -- / analyze_same_name_module_relations<br/>-- 同名模块语义关系分析<br/>文件: detectors/analyze_same_name_module_relations.py"]
    scripts_governance_d5_architecture_detectors_detect_depends_on_cycles_py["(生产态 / production) 检测depends开cycles.py - depends开 / detect_depends_on_cycles<br/>depends_on 环检测.<br/>文件: detectors/detect_depends_on_cycles.py"]
    scripts_governance_d5_architecture_detectors_detect_deprecated_adr_references_py["(生产态 / production) 检测废弃adrreferences.py — 废弃 / detect_deprecated_adr_references<br/>废弃 ADR 引用检测<br/>文件: detectors/detect_deprecated_adr_references.py"]
    scripts_governance_d5_architecture_detectors_detect_duplicate_module_names_py["(生产态 / production) 检测duplicate模块names.py --- 同名模 / detect_duplicate_module_names<br/>-- 同名模块语义关系分析<br/>文件: detectors/detect_duplicate_module_names.py"]
    scripts_governance_d5_architecture_diagnose_depgraph_py["(生产态 / production) diagnose依赖图 / # (BLUEPRINT) MOD-INF-005 / scripts/governance/diagnose_depg<br/>diagnose依赖图，治理的功能模块。<br/>文件: d5_architecture/diagnose_depgraph.py"]
    scripts_governance_d5_architecture_generators_align_panoramas_py["(生产态 / production) G-panorama-align: 四图对齐检测器（ARCH-053 + ARC / align_panoramas<br/>G-panorama-align: 四图对齐检测器（ARCH-053 + ARCH-056 四图升级）<br/>文件: generators/align_panoramas.py"]
    scripts_governance_d5_architecture_generators_generate_asset_catalog_py["(生产态 / production) G13: 从 depgraph (PostgreSQL) 生成资产清单全景图 / generate_asset_catalog<br/>G13: 从 depgraph (PostgreSQL) 生成资产清单全景图<br/>文件: generators/generate_asset_catalog.py"]
    scripts_governance_d5_architecture_generators_generate_blueprint_panorama_py["(生产态 / production) G-panorama-gen: 蓝图 §0.6 四图对齐视图生成器（ARCH-0 / generate_blueprint_panorama<br/>G-panorama-gen: 蓝图 §0.6 四图对齐视图生成器（ARCH-053 + ARCH-056 + 模板 v2.1.0）<br/>文件: generators/generate_blueprint_panorama.py"]
    scripts_governance_d5_architecture_generators_generate_code_wiki_stats_py["(生产态 / production) Code Wiki 统计数据生成器（半自动维护机制）。 / generate_code_wiki_stats<br/>Code Wiki 统计数据生成器（半自动维护机制）。<br/>文件: generators/generate_code_wiki_stats.py"]
    scripts_governance_d5_architecture_generators_generate_contract_catalog_py["(生产态 / production) G12: 从 depgraph (PostgreSQL) 生成契约目录全景图 / generate_contract_catalog<br/>G12: 从 depgraph (PostgreSQL) 生成契约目录全景图<br/>文件: generators/generate_contract_catalog.py"]
    scripts_governance_d5_architecture_generators_generate_contracts_py["(生产态 / production) 生成契约 / generate_contracts.py -- SSoT to Codegen pipeline<br/>生成契约，治理的生成器，按规则生成所需的数据或报告。<br/>文件: generators/generate_contracts.py"]
    scripts_governance_d5_architecture_generators_generate_data_acquisition_flow_py["(生产态 / production) G-acqflow: 从 tasks.yaml 生成业务数据采集流图 MD（人类 / generate_data_acquisition_flow<br/>G-acqflow: 从 tasks.yaml 生成业务数据采集流图 MD（人类可读版，内嵌 Mermaid）<br/>文件: generators/generate_data_acquisition_flow.py"]
    scripts_governance_d5_architecture_generators_generate_data_inventory_py["(生产态 / production) G-inventory: 扫描 ClickHouse 生成业务数据清单 MD / generate_data_inventory<br/>G-inventory: 扫描 ClickHouse 生成业务数据清单 MD<br/>文件: generators/generate_data_inventory.py"]
    scripts_governance_d5_architecture_generators_generate_dataflow_diagram_py["(生产态 / production) G-dataflow: 从 dataflowgraph (PostgreSQL) / generate_dataflow_diagram<br/>G-dataflow: 从 dataflowgraph (PostgreSQL) 生成数据流图 Markdown 文档（内嵌 Mermaid）<br/>文件: generators/generate_dataflow_diagram.py"]
    scripts_governance_d5_architecture_generators_generate_decision_diagram_py["(生产态 / production) G-decision: 从 decisiongraph (PostgreSQL) / generate_decision_diagram<br/>G-decision: 从 decisiongraph (PostgreSQL) 生成决策流图(.md 文档，Mermaid 内嵌)<br/>文件: generators/generate_decision_diagram.py"]
    scripts_governance_d5_architecture_generators_generate_panorama_registry_py["(生产态 / production) G-panorama-registry: 自动生成全景图清单总表 / generate_panorama_registry<br/>G-panorama-registry: 自动生成全景图清单总表<br/>文件: generators/generate_panorama_registry.py"]
    scripts_governance_d5_architecture_generators_generate_policies_py["(生产态 / production) #183: 从 datasourcesregistry.yaml 派生 po / generate_policies<br/>#183: 从 data_sources_registry.yaml 派生 policies.yaml<br/>文件: generators/generate_policies.py"]
    scripts_governance_d5_architecture_generators_generate_trading_flow_diagram_py["(生产态 / production) G-trading-flow: 从 decisiongraph + 叙事YAML / generate_trading_flow_diagram<br/>G-trading-flow: 从 decisiongraph + 叙事YAML + 候选库 生成交易决策架构视图(.md)<br/>文件: generators/generate_trading_flow_diagram.py"]
    scripts_governance_d5_architecture_pre_delete_safety_check_py["(生产态 / production) 安全删除门禁脚本——RULE-THREE 强制执行器。 / pre_delete_safety_check<br/>安全删除门禁脚本——RULE-THREE 强制执行器。<br/>文件: d5_architecture/pre_delete_safety_check.py"]
    scripts_governance_d5_architecture_pre_write_gate_py["(生产态 / production) AI写入前强制门禁钩子: lock协议检查+GateEngine Phase评估 / pre_write_gate<br/>AI写入前强制门禁钩子: lock协议检查+GateEngine Phase评估+注册完整性验证<br/>文件: d5_architecture/pre_write_gate.py"]
    scripts_governance_d5_architecture_syncers_archive_rationale_log_py["(生产态 / production) 对标 HDEBT-01：rationale-log.md 体积 >150KB / / archive_rationale_log<br/>对标 HDEBT-01：rationale-log.md 体积 >150KB / 行数 >300 时，<br/>文件: syncers/archive_rationale_log.py"]
    scripts_governance_d5_architecture_syncers_merge_readme_to_index_py["(生产态 / production) 合并readmeto索引 / Strategy:<br/>合并readmeto索引。Strategy:<br/>文件: syncers/merge_readme_to_index.py"]
    scripts_governance_d5_architecture_syncers_sync_blueprint_code_index_py["(生产态 / production) 对标：AGENTS.md §6.1 蓝图-代码同步强制约定 / sync_blueprint_code_index<br/>对标：AGENTS.md §6.1 蓝图-代码同步强制约定<br/>文件: syncers/sync_blueprint_code_index.py"]
    scripts_governance_d5_architecture_syncers_sync_registry_from_blueprints_py["(生产态 / production) 同步注册表fromblueprints.py -- 从 bl / sync_registry_from_blueprints<br/>- 从 blueprint.md frontmatter 同步 blueprint_registry.yaml<br/>文件: syncers/sync_registry_from_blueprints.py"]
    scripts_governance_d5_architecture_validators_blueprint_validate_blueprint_code_sync_py["(生产态 / production) AGENTS.md §6.1 蓝图-代码同步强制约定的 CI 门禁脚本。 / validate_blueprint_code_sync<br/>AGENTS.md §6.1 蓝图-代码同步强制约定的 CI 门禁脚本。<br/>文件: blueprint/validate_blueprint_code_sync.py"]
    scripts_governance_d5_architecture_validators_blueprint_validate_blueprint_implementation_docs_py["(生产态 / production) AGENTS.md 6.4 铁律五 + 铁律六：蓝图中声称的文件路径必须在磁盘上 / validate_blueprint_implementation_docs<br/>AGENTS.md 6.4 铁律五 + 铁律六：蓝图中声称的文件路径必须在磁盘上真实存在。<br/>文件: blueprint/validate_blueprint_implementation_docs.py"]
    scripts_governance_d5_architecture_validators_blueprint_validate_blueprint_path_consistency_py["(生产态 / production) 校验蓝图路径一致性 / Module docstring — see module-level docstring for details.<br/>校验蓝图路径一致性。Module docstring — see module-level docstring for details.<br/>文件: blueprint/validate_blueprint_path_consistency.py"]
    scripts_governance_d5_architecture_validators_blueprint_validate_blueprint_placement_py["(生产态 / production) 蓝图物理位置与归属链完整性校验器 (Blueprint Placement &  / validate_blueprint_placement<br/>蓝图物理位置与归属链完整性校验器 (Blueprint Placement & BelongsTo Validator)<br/>文件: blueprint/validate_blueprint_placement.py"]
    scripts_governance_d5_architecture_validators_blueprint_validate_blueprint_tag_uniqueness_py["(生产态 / production) 校验蓝图标签uniqueness / GATE-TAG-UNIQUE - Blueprint tag uniqueness validation gate.<br/>校验蓝图标签uniqueness。GATE-TAG-UNIQUE - Blueprint tag uniqueness validation gate.<br/>文件: blueprint/validate_blueprint_tag_uniqueness.py"]
    scripts_governance_d5_architecture_validators_lifecycle_validate_lifecycle_refs_py["(生产态 / production) 校验生命周期refs.py — 生命周期引用约束合规检 / validate_lifecycle_refs<br/>生命周期引用约束合规检查<br/>文件: lifecycle/validate_lifecycle_refs.py"]
    scripts_governance_d5_architecture_validators_lifecycle_validate_module_lifecycle_py["(生产态 / production) 校验模块lifecycle.py — 模块生命周期校验 / validate_module_lifecycle<br/>模块生命周期校验<br/>文件: lifecycle/validate_module_lifecycle.py"]
    scripts_governance_d5_architecture_validators_session_validate_session_log_index_integrity_py["(生产态 / production) 校验会话日志索引完整性 / Module docstring — see module-level docstring for details.<br/>校验会话日志索引完整性。Module docstring — see module-level docstring for details.<br/>文件: session/validate_session_log_index_integrity.py"]
    scripts_governance_d5_architecture_validators_session_validate_session_log_updated_py["(生产态 / production) 校验会话日志updated.py — Sessio / validate_session_log_updated<br/>Session Log 更新状态校验<br/>文件: session/validate_session_log_updated.py"]
    scripts_governance_d5_architecture_validators_validate_adr_frontmatter_consistency_py["(生产态 / production) 校验adrfrontmatterconsistency.py  / validate_adr_frontmatter_consistency<br/>ADR frontmatter 一致性闸门<br/>文件: validators/validate_adr_frontmatter_consistency.py"]
    scripts_governance_d5_architecture_validators_validate_arch_review_gate_py["(生产态 / production) 校验架构审查gate.py — 架构评审门控校验 / validate_arch_review_gate<br/>架构评审门控校验<br/>文件: validators/validate_arch_review_gate.py"]
    scripts_governance_d5_architecture_validators_validate_architecture_contract_internal_py["(生产态 / production) 校验架构契约内部 / GATE-CONTRACT: CI gate for architecture_contract.yaml intern<br/>校验架构契约内部。GATE-CONTRACT: CI gate for architecture_contract.yaml internal consistency.<br/>文件: validators/validate_architecture_contract_internal.py"]
    scripts_governance_d5_architecture_validators_validate_autonomy_gate_py["(生产态 / production) 校验autonomygate.py — 变更级别 vs AI 自 / validate_autonomy_gate<br/>变更级别 vs AI 自治权限交叉校验<br/>文件: validators/validate_autonomy_gate.py"]
    scripts_governance_d5_architecture_validators_validate_b_track_packages_py["(生产态 / production) 校验btrackpackages.py — B 轨 btra / validate_b_track_packages<br/>B 轨 b_track 一致性校验<br/>文件: validators/validate_b_track_packages.py"]
    scripts_governance_d5_architecture_validators_validate_blind_spot_status_py["(生产态 / production) 校验blindspot状态 / GATE-BS: Blind Spot Reality Check<br/>校验blindspot状态。GATE-BS: Blind Spot Reality Check<br/>文件: validators/validate_blind_spot_status.py"]
    scripts_governance_d5_architecture_validators_validate_code_yaml_alignment_py["(生产态 / production) 校验代码yamlalignment.py — GATE-A / validate_code_yaml_alignment<br/>GATE-A: 实际代码 ↔ YAML SSoT 对账<br/>文件: validators/validate_code_yaml_alignment.py"]
    scripts_governance_d5_architecture_validators_validate_cross_references_py["(生产态 / production) 校验跨references.py — 架构模型 YAML / validate_cross_references<br/>架构模型 YAML + 治理文档跨引用完整性闸门<br/>文件: validators/validate_cross_references.py"]
    scripts_governance_d5_architecture_validators_validate_dependency_graph_template_py["(生产态 / production) (INVARIANTS) 治理脚本执行正确 / validate_dependency_graph_template<br/>(INVARIANTS) 治理脚本执行正确<br/>文件: validators/validate_dependency_graph_template.py"]
    scripts_governance_d5_architecture_validators_validate_depends_on_format_py["(生产态 / production) 校验depends开format.py — depends / validate_depends_on_format<br/>depends_on 条目结构化格式校验<br/>文件: validators/validate_depends_on_format.py"]
    scripts_governance_d5_architecture_validators_validate_deprecated_dependents_py["(生产态 / production) 校验废弃dependents.py — 废弃文件 / validate_deprecated_dependents<br/>废弃文件活跃引用检测<br/>文件: validators/validate_deprecated_dependents.py"]
    scripts_governance_d5_architecture_validators_validate_directory_structure_py["(生产态 / production) 校验directorystructure / Module docstring — see module-level docstring for details.<br/>校验directorystructure。Module docstring — see module-level docstring for details.<br/>文件: validators/validate_directory_structure.py"]
    scripts_governance_d5_architecture_validators_validate_field_ownership_py["(生产态 / production) 校验字段ownership.py — frontmatte / validate_field_ownership<br/>frontmatter 字段归属校验<br/>文件: validators/validate_field_ownership.py"]
    scripts_governance_d5_architecture_validators_validate_gate_yaml_py["(生产态 / production) 校验门禁yaml / Module docstring — see module-level docstring for details.<br/>校验门禁yaml。Module docstring — see module-level docstring for details.<br/>文件: validators/validate_gate_yaml.py"]
    scripts_governance_d5_architecture_validators_validate_handoff_package_py["(生产态 / production) 校验handoffpackage.py — HandoffPac / validate_handoff_package<br/>HandoffPackage 完整性校验<br/>文件: validators/validate_handoff_package.py"]
    scripts_governance_d5_architecture_validators_validate_interface_contracts_py["(生产态 / production) 校验接口contracts.py — 接口契约校验 / validate_interface_contracts<br/>接口契约校验<br/>文件: validators/validate_interface_contracts.py"]
    scripts_governance_d5_architecture_validators_validate_load_path_integrity_py["(生产态 / production) 校验加载路径完整性 / Module docstring — see module-level docstring for details.<br/>校验加载路径完整性。Module docstring — see module-level docstring for details.<br/>文件: validators/validate_load_path_integrity.py"]
    scripts_governance_d5_architecture_validators_validate_module_schema_py["(生产态 / production) 校验模块schema.py — 模块 Schema 校验 / validate_module_schema<br/>模块 Schema 校验<br/>文件: validators/validate_module_schema.py"]
    scripts_governance_d5_architecture_validators_validate_nested_flat_dirs_py["(生产态 / production) 校验nestedflatdirs / Module docstring — see module-level docstring for details.<br/>校验nestedflatdirs。Module docstring — see module-level docstring for details.<br/>文件: validators/validate_nested_flat_dirs.py"]
    scripts_governance_d5_architecture_validators_validate_p0_module_contracts_py["(生产态 / production) 校验p0模块contracts.py — P0 模块契 / validate_p0_module_contracts<br/>P0 模块契约校验<br/>文件: validators/validate_p0_module_contracts.py"]
    scripts_governance_d5_architecture_validators_validate_static_manifest_drift_py["(生产态 / production) 校验静态清单drift.py — GATE / validate_static_manifest_drift<br/>GATE-21 静态清单漂移阻断<br/>文件: validators/validate_static_manifest_drift.py"]
    scripts_governance_d5_architecture_validators_validate_target_layer_py["(生产态 / production) 对标：target层vocabulary.yaml v1.0.0—— / validate_target_layer<br/>对标：target_layer_vocabulary.yaml v1.0.0——target_layer 字段值体系多真源不一致修复<br/>文件: validators/validate_target_layer.py"]
    scripts_governance_d5_architecture_validators_validate_three_way_consistency_py["(生产态 / production) 校验threewayconsistency.py — 三方一致 / validate_three_way_consistency<br/>三方一致性检查<br/>文件: validators/validate_three_way_consistency.py"]
    scripts_governance_d5_architecture_validators_yaml_md_validate_md_yaml_number_drift_py["(生产态 / production) 校验mdyamlnumberdrift.py — MD 视图 / validate_md_yaml_number_drift<br/>MD 视图与 YAML SSoT 数字漂移检测闸门<br/>文件: yaml_md/validate_md_yaml_number_drift.py"]
    scripts_governance_d5_architecture_validators_yaml_md_validate_yaml_interface_uniqueness_py["(生产态 / production) 校验yaml接口uniqueness.py —  / validate_yaml_interface_uniqueness<br/>YAML 模块接口唯一性闸门<br/>文件: yaml_md/validate_yaml_interface_uniqueness.py"]
    scripts_governance_d5_architecture_validators_yaml_md_validate_yaml_summaries_py["(生产态 / production) 校验yaml摘要 / v1.0.0 -- 2026-05-03<br/>校验yaml摘要。v1.0.0 -- 2026-05-03<br/>文件: yaml_md/validate_yaml_summaries.py"]
    scripts_governance_d6_security_check_protected_paths_py["(生产态 / production) 检查protectedpaths.py — 受保护路径写入检查（IRN / check_protected_paths<br/>受保护路径写入检查<br/>文件: d6_security/check_protected_paths.py"]
    scripts_governance_d6_security_detect_anchor_file_deletion_py["(生产态 / production) 检测anchor文件deletion.py — 锚点文件删除检 / detect_anchor_file_deletion<br/>锚点文件删除检测<br/>文件: d6_security/detect_anchor_file_deletion.py"]
    scripts_governance_d6_security_detect_git_dangerous_py["(生产态 / production) 检测gitdangerous.py — 危险 Git 命令检测 / detect_git_dangerous<br/>危险 Git 命令检测<br/>文件: d6_security/detect_git_dangerous.py"]
    scripts_governance_d6_security_detect_keywords_in_logs_py["(生产态 / production) 检测keywords入logs.py — 日志输出敏感关键词检测 / detect_keywords_in_logs<br/>日志输出敏感关键词检测<br/>文件: d6_security/detect_keywords_in_logs.py"]
    scripts_governance_d6_security_detect_permanent_file_deletion_py["(生产态 / production) 检测permanent文件deletion.py — 永久文件 / detect_permanent_file_deletion<br/>永久文件删除检测<br/>文件: d6_security/detect_permanent_file_deletion.py"]
    scripts_governance_d6_security_detect_secrets_py["(生产态 / production) 检测secrets.py — 密钥/Token/凭证硬编码检测 / detect_secrets<br/>密钥/Token/凭证硬编码检测<br/>文件: d6_security/detect_secrets.py"]
    scripts_governance_d6_security_detect_shell_dangerous_py["(生产态 / production) 检测shelldangerous.py — 危险 Shell 命令检 / detect_shell_dangerous<br/>危险 Shell 命令检测<br/>文件: d6_security/detect_shell_dangerous.py"]
    scripts_governance_d6_security_detect_shell_true_py["(生产态 / production) 检测shelltrue.py — shell=True 调用检测 / detect_shell_true<br/>shell=True 调用检测<br/>文件: d6_security/detect_shell_true.py"]
    scripts_governance_d6_security_detect_threading_lock_py["(生产态 / production) 检测threadinglock.py — threading.Loc / detect_threading_lock<br/>threading.Lock 导入检测<br/>文件: d6_security/detect_threading_lock.py"]
    scripts_governance_d6_security_detect_vague_terms_py["(生产态 / production) 检测vagueterms.py — 模糊/不确定术语检测 / detect_vague_terms<br/>模糊/不确定术语检测<br/>文件: d6_security/detect_vague_terms.py"]
    scripts_governance_d6_security_retire_tmp_artifacts_py["(生产态 / production) retiretmpartifacts — tmp/ + logs/ 退役区  / retire_tmp_artifacts<br/>retire_tmp_artifacts — tmp/ + logs/ 退役区 TTL 执行器（AI-03 审计 P2/P3 治本）<br/>文件: d6_security/retire_tmp_artifacts.py"]
    scripts_governance_d6_security_run_adversarial_checks_py["(生产态 / production) 运行adversarialchecks / CI Entry: Adversarial Validation — Red-Blue Drift Test<br/>运行adversarialchecks。CI Entry: Adversarial Validation — Red-Blue Drift Test<br/>文件: d6_security/run_adversarial_checks.py"]
    scripts_governance_d6_security_scan_runtime_log_secrets_py["(生产态 / production) 对标 architectureprinciples.md §2bis R2 安 / scan_runtime_log_secrets<br/>对标 architecture_principles.md §2bis R2 安全红线：<br/>文件: d6_security/scan_runtime_log_secrets.py"]
    scripts_governance_d6_security_scan_secret_leak_py["(生产态 / production) 对标 06-securityarchitecture.md §6.3 L3-A / scan_secret_leak<br/>对标 06-security_architecture.md §6.3 L3-Audit：<br/>文件: d6_security/scan_secret_leak.py"]
    scripts_governance_d6_security_validate_gate_discipline_py["(生产态 / production) 校验门禁discipline.py — 门禁纪律校验 / validate_gate_discipline<br/>门禁纪律校验<br/>文件: d6_security/validate_gate_discipline.py"]
    scripts_governance_d7_code_any_type_inferrer_py["(生产态 / production) 裸 Any 类型推断辅助工具 — #ARCH-ANY-GOVERNANCE-001 / any_type_inferrer<br/>裸 Any 类型推断辅助工具 — #ARCH-ANY-GOVERNANCE-001 Phase 1.<br/>文件: d7_code/any_type_inferrer.py"]
    scripts_governance_d7_code_check_ai_capability_boundary_py["(生产态 / production) 行为说明 / check_ai_capability_boundary<br/>行为说明<br/>文件: d7_code/check_ai_capability_boundary.py"]
    scripts_governance_d7_code_check_encoding_py["(生产态 / production) 检查encoding.py — 编码合规校验（INJ-007） / check_encoding<br/>编码合规校验<br/>文件: d7_code/check_encoding.py"]
    scripts_governance_d7_code_check_idempotency_py["(生产态 / production) 检查idempotency.py — 幂等性缺失检查（HC-9） / check_idempotency<br/>幂等性缺失检查<br/>文件: d7_code/check_idempotency.py"]
    scripts_governance_d7_code_check_merge_conflict_py["(生产态 / production) 检查合并conflict.py — 合并冲突标记检测（local / check_merge_conflict<br/>合并冲突标记检测（local 替代 external pre-commit-hooks）<br/>文件: d7_code/check_merge_conflict.py"]
    scripts_governance_d7_code_check_no_tests_unit_py["(生产态 / production) 检查notestsunit.py — 禁止 tests/unit/  / check_no_tests_unit<br/>禁止 tests/unit/ 旧路径重引入检测（local 替代 pygrep）<br/>文件: d7_code/check_no_tests_unit.py"]
    scripts_governance_d7_code_check_pit_compliance_py["(生产态 / production) 检查pitcompliance.py — PIT 合规检查（HC-10 / check_pit_compliance<br/>PIT 合规检查<br/>文件: d7_code/check_pit_compliance.py"]
    scripts_governance_d7_code_detect_absolute_path_hardcoding_py["(生产态 / production) 检测absolute路径hardcoding.py — 绝对路 / detect_absolute_path_hardcoding<br/>绝对路径硬编码检测（蓝图 §34.1 操作陷阱）<br/>文件: d7_code/detect_absolute_path_hardcoding.py"]
    scripts_governance_d7_code_detect_direct_llm_calls_py["(生产态 / production) 检测directllmcalls.py — 裸调 LLM API  / detect_direct_llm_calls<br/>裸调 LLM API 检测门禁<br/>文件: d7_code/detect_direct_llm_calls.py"]
    scripts_governance_d7_code_detect_forward_reference_py["(生产态 / production) 检测前reference — 前向引用检测扫描器。 / detect_forward_reference<br/>detect_forward_reference — 前向引用检测扫描器。<br/>文件: d7_code/detect_forward_reference.py"]
    scripts_governance_d7_code_detect_missing_encoding_py["(生产态 / production) 检测missingencoding.py — open() 缺 en / detect_missing_encoding<br/>open() 缺 encoding 检测<br/>文件: d7_code/detect_missing_encoding.py"]
    scripts_governance_d7_code_detect_private_key_py["(生产态 / production) 检测私有key.py — 私钥意外提交检测（local 替 / detect_private_key<br/>私钥意外提交检测（local 替代 external pre-commit-hooks）<br/>文件: d7_code/detect_private_key.py"]
    scripts_governance_d7_code_detect_pydantic_any_fields_py["(生产态 / production) 检测pydanticanyfields.py — Pydantic / detect_pydantic_any_fields<br/>Pydantic Any 类型字段检测<br/>文件: d7_code/detect_pydantic_any_fields.py"]
    scripts_governance_d7_code_detect_silent_degradation_py["(生产态 / production) 检测silentdegradation.py — 静默降级检测 / detect_silent_degradation<br/>静默降级检测<br/>文件: d7_code/detect_silent_degradation.py"]
    scripts_governance_d7_code_fix_n06_scope_py["(生产态 / production) N-06 moduleid scope 前缀检测修复脚本。 / fix_n06_scope<br/>N-06 module_id scope 前缀检测修复脚本。<br/>文件: d7_code/fix_n06_scope.py"]
    scripts_governance_d7_code_fix_n12_ke_naming_py["(生产态 / production) N-12 KE 条目命名格式批量修复脚本。 / fix_n12_ke_naming<br/>N-12 KE 条目命名格式批量修复脚本。<br/>文件: d7_code/fix_n12_ke_naming.py"]
    scripts_governance_d7_code_fix_n13_snake_case_py["(生产态 / production) N-13 YAML/JSON/MD 文件名 snakecase 批量修复脚本。 / fix_n13_snake_case<br/>N-13 YAML/JSON/MD 文件名 snake_case 批量修复脚本。<br/>文件: d7_code/fix_n13_snake_case.py"]
    scripts_governance_d7_code_fix_n14_init_all_py["(生产态 / production) N-14 初始化.py 缺少 all 批量修复脚本。 / fix_n14_init_all<br/>N-14 __init__.py 缺少 __all__ 批量修复脚本。<br/>文件: d7_code/fix_n14_init_all.py"]
    scripts_governance_d7_code_fix_n15_blueprint_path_py["(生产态 / production) N-15 BLUEPRINT 头部路径不存在批量修复脚本。 / fix_n15_blueprint_path<br/>N-15 BLUEPRINT 头部路径不存在批量修复脚本。<br/>文件: d7_code/fix_n15_blueprint_path.py"]
    scripts_governance_d7_code_fix_naming_manual_py["(生产态 / production) 修复namingmanual — 手动修复少量命名违规(N-11/N-10 / fix_naming_manual<br/>fix_naming_manual — 手动修复少量命名违规(N-11/N-10/N-03/N-09/N-16)。<br/>文件: d7_code/fix_naming_manual.py"]
    scripts_governance_d7_code_fix_orphan_exports_py["(生产态 / production) 修复orphanexports.py — 批量修复孤儿模块导出（RULE- / fix_orphan_exports<br/>批量修复孤儿模块导出（RULE-TWO 防线 2 修复器）<br/>文件: d7_code/fix_orphan_exports.py"]
    scripts_governance_d7_code_rewrite_imports_py["(生产态 / production) rewriteimports.py — 批量重写 Python import  / rewrite_imports<br/>批量重写 Python import 路径（AST-based）<br/>文件: d7_code/rewrite_imports.py"]
    scripts_governance_d7_code_scan_complexity_py["(生产态 / production) 全量循环复杂度扫描器 — §5.158 暗债监控（裁定#214 Phase 4  / scan_complexity<br/>全量循环复杂度扫描器 — §5.158 暗债监控（裁定#214 Phase 4 引入）。<br/>文件: d7_code/scan_complexity.py"]
    scripts_governance_d7_code_scan_consumers_accuracy_py["(生产态 / production) 扫描消费者accuracy.py — CONSUMERS 字 / scan_consumers_accuracy<br/>CONSUMERS 字段准确性 baseline-scan 脚本<br/>文件: d7_code/scan_consumers_accuracy.py"]
    scripts_governance_d7_code_scan_debt_py["(生产态 / production) 架构债务扫描器 — 5.96 维度防御门闸（R67 引入）。 / scan_debt<br/>架构债务扫描器 — 5.96 维度防御门闸（R67 引入）。<br/>文件: d7_code/scan_debt.py"]
    scripts_governance_d7_code_validate_contracts_purity_py["(生产态 / production) 校验契约purity.py — 契约纯度校验 / validate_contracts_purity<br/>契约纯度校验<br/>文件: d7_code/validate_contracts_purity.py"]
    scripts_governance_d7_code_validate_docstring_coverage_py["(生产态 / production) 校验docstringcoverage.py — Docstri / validate_docstring_coverage<br/>Docstring 覆盖率校验<br/>文件: d7_code/validate_docstring_coverage.py"]
    scripts_governance_d7_code_validate_fle_action_metadata_py["(生产态 / production) 校验fle动作metadata.py — FLE Ac / validate_fle_action_metadata<br/>FLE Action 元数据校验<br/>文件: d7_code/validate_fle_action_metadata.py"]
    scripts_governance_d7_code_validate_fle_imports_py["(生产态 / production) 校验fleimports.py — FLE import 接口合 / validate_fle_imports<br/>FLE import 接口合规检测<br/>文件: d7_code/validate_fle_imports.py"]
    scripts_governance_d7_code_validate_import_style_py["(生产态 / production) 校验导入style.py — 导入风格一致性校验 / validate_import_style<br/>导入风格一致性校验<br/>文件: d7_code/validate_import_style.py"]
    scripts_governance_d7_code_validate_init_all_py["(生产态 / production) 校验初始化all.py — 初始化.py all / validate_init_all<br/>__init__.py __all__ 完整性校验<br/>文件: d7_code/validate_init_all.py"]
    scripts_governance_d7_code_validate_kb_write_provenance_py["(生产态 / production) 校验kbwriteprovenance.py — 知识库写入  / validate_kb_write_provenance<br/>知识库写入 provenance 校验<br/>文件: d7_code/validate_kb_write_provenance.py"]
    scripts_governance_d7_code_validate_python_syntax_py["(生产态 / production) 校验pythonsyntax.py — Python 语法完整性 / validate_python_syntax<br/>Python 语法完整性校验<br/>文件: d7_code/validate_python_syntax.py"]
    scripts_governance_d7_code_validate_test_assertion_depth_py["(生产态 / production) 校验测试assertiondepth.py — 测试断言深 / validate_test_assertion_depth<br/>测试断言深度校验<br/>文件: d7_code/validate_test_assertion_depth.py"]
    scripts_governance_d7_code_validate_test_coverage_py["(生产态 / production) 校验测试coverage.py — 测试覆盖率治理校验器 / validate_test_coverage<br/>测试覆盖率治理校验器<br/>文件: d7_code/validate_test_coverage.py"]
    scripts_governance_d7_code_validate_type_annotation_coverage_py["(生产态 / production) 校验typeannotationcoverage.py — 类 / validate_type_annotation_coverage<br/>类型注解覆盖率校验<br/>文件: d7_code/validate_type_annotation_coverage.py"]
    scripts_governance_d7_code_validate_unused_imports_py["(生产态 / production) 校验unusedimports.py — 未使用导入检测 / validate_unused_imports<br/>未使用导入检测<br/>文件: d7_code/validate_unused_imports.py"]
    scripts_governance_d8_doc_sync_auto_sync_all_registries_py["(生产态 / production) 全自动注册表同步器 / auto_sync_all_registries<br/>全自动注册表同步器<br/>文件: d8_doc_sync/auto_sync_all_registries.py"]
    scripts_governance_d8_doc_sync_detect_ai_products_in_docs_py["(生产态 / production) 检测AIproducts入docs.py — AI 产物位置检 / detect_ai_products_in_docs<br/>AI 产物位置检测<br/>文件: d8_doc_sync/detect_ai_products_in_docs.py"]
    scripts_governance_d8_doc_sync_detect_dated_snapshots_py["(生产态 / production) 检测datedsnapshots.py — 带日期快照文件检测 / detect_dated_snapshots<br/>带日期快照文件检测<br/>文件: d8_doc_sync/detect_dated_snapshots.py"]
    scripts_governance_d8_doc_sync_sync_rule_registry_py["(生产态 / production) 同步规则注册表 / Checks that every RULE-ZERO through RULE-N in .trae/rules/pr<br/>同步规则注册表。Checks that every RULE-ZERO through RULE-N in .trae/rules/project_rules.md<br/>文件: d8_doc_sync/sync_rule_registry.py"]
    scripts_governance_d8_doc_sync_update_progress_py["(生产态 / production) 更新progress.py — 从 domainprogress.j / update_progress<br/>从 domain_progress.json 批量更新施工进度.<br/>文件: d8_doc_sync/update_progress.py"]
    scripts_governance_d8_doc_sync_validate_document_lifecycle_py["(生产态 / production) 校验documentlifecycle.py — 文档生命周期校 / validate_document_lifecycle<br/>文档生命周期校验<br/>文件: d8_doc_sync/validate_document_lifecycle.py"]
    scripts_governance_d8_doc_sync_validate_document_ttl_py["(生产态 / production) 校验documentttl.py — 文档 TTL 过期检测 / validate_document_ttl<br/>文档 TTL 过期检测<br/>文件: d8_doc_sync/validate_document_ttl.py"]
    scripts_governance_d9_knowledge_detect_duplicated_normative_language_py["(生产态 / production) 检测duplicatednormativelanguage.py  / detect_duplicated_normative_language<br/>规范用语重复定义检测<br/>文件: d9_knowledge/detect_duplicated_normative_language.py"]
    scripts_governance_d9_knowledge_detect_orphan_documents_py["(生产态 / production) 检测orphandocuments.py — 孤立文档检测 / detect_orphan_documents<br/>孤立文档检测<br/>文件: d9_knowledge/detect_orphan_documents.py"]
    scripts_governance_data_quality_check_tick_duplication_py["(生产态 / production) 逐笔data 表真重复检查工具（RULE-DATA-OPS 配套，TRAE / check_tick_duplication<br/>tick_data 表真重复检查工具（RULE-DATA-OPS 配套，TRAE-063 §invariants DATA-OPS-INV-002）.<br/>文件: data_quality/check_tick_duplication.py"]
    scripts_governance_extract_decisiongraph_py["(生产态 / production) 提取decisiongraph / extract_decisiongraph - decisiongraph on-demand extraction t<br/>提取decisiongraph。extract_decisiongraph - decisiongraph on-demand extraction tool<br/>文件: governance/extract_decisiongraph.py"]
    scripts_governance_extract_depgraph_py["(生产态 / production) (INVARIANTS) 禁止AI直接Read 157MB depgraph文件 / extract_depgraph<br/>(INVARIANTS) 禁止AI直接Read 157MB depgraph文件；提取输出必须可被AI安全消费<br/>文件: governance/extract_depgraph.py"]
    scripts_governance_generate_decision_graph_py["(生产态 / production) (INVARIANTS) YAML 是唯一真源; DB 为只读缓存; 同步单向  / generate_decision_graph<br/>(INVARIANTS) YAML 是唯一真源; DB 为只读缓存; 同步单向 YAML→DB; 不变量校验前置<br/>文件: governance/generate_decision_graph.py"]
    scripts_governance_generate_project_depgraph_py["(生产态 / production) 生成project依赖图 / # (BLUEPRINT) MOD-INF-005 / scripts/governance/generate_proj<br/>Scan 结果缓存。线程安全（ThreadPoolExecutor 并发 put）。<br/>文件: governance/generate_project_depgraph.py"]
    scripts_governance_generate_project_path_tree_py["(生产态 / production) 从磁盘扫描生成路径全景图的tree段（运营态目录结构）。 / generate_project_path_tree<br/>从磁盘扫描生成路径全景图的tree段（运营态目录结构）。<br/>文件: governance/generate_project_path_tree.py"]
    scripts_governance_generators_check_gate_inventory_drift_py["(生产态 / production) 检查门禁inventorydrift.py — commitg / check_gate_inventory_drift<br/>commit_gates 模块清单漂移检测（ARCH-055 治本）<br/>文件: generators/check_gate_inventory_drift.py"]
    scripts_governance_generators_fix_module_manifest_layout_py["(生产态 / production) 修复模块清单layout.py — 校正治理脚本模块 / fix_module_manifest_layout<br/>校正治理脚本模块 docstring 与 ``__manifest__`` 的顺序<br/>文件: generators/fix_module_manifest_layout.py"]
    scripts_governance_generators_generate_gate_registry_py["(生产态 / production) 生成门禁registry.py — 门禁登记表自动生成器 / generate_gate_registry<br/>门禁登记表自动生成器<br/>文件: generators/generate_gate_registry.py"]
    scripts_governance_generators_generate_path_ownership_map_py["(生产态 / production) 从蓝图§0.1聚合生成 pathownershipmap.yaml 路径归属 / generate_path_ownership_map<br/>从蓝图§0.1聚合生成 path_ownership_map.yaml 路径归属声明。<br/>文件: generators/generate_path_ownership_map.py"]
    scripts_governance_generators_generate_registry_master_index_py["(生产态 / production) 生成注册表主index.py — 登记表总 / generate_registry_master_index<br/>登记表总索引自动生成器<br/>文件: generators/generate_registry_master_index.py"]
    scripts_governance_generators_inject_manifests_py["(生产态 / production) injectmanifests.py — 清单 批量注入器 / inject_manifests<br/>__manifest__ 批量注入器<br/>文件: generators/inject_manifests.py"]
    scripts_governance_generators_refresh_master_entries_py["(生产态 / production) refresh主entries.py — 登记表总索引 entri / refresh_master_entries<br/>登记表总索引 entries 自动刷新器<br/>文件: generators/refresh_master_entries.py"]
    scripts_governance_generators_sync_audit_protocol_numbers_py["(生产态 / production) 同步审计协议numbers.py — 从 SSoT  / sync_audit_protocol_numbers<br/>从 SSoT 注册表自动同步审计协议中的硬编码数字。<br/>文件: generators/sync_audit_protocol_numbers.py"]
    scripts_governance_git_health_smoke_py["(生产态 / production) git健康smoke.py — Git 健康度 smoke test / git_health_smoke<br/>Git 健康度 smoke test（ARCH-GIT-CALL-BUDGET P3.2）<br/>文件: governance/git_health_smoke.py"]
    scripts_governance_meta_arbitrate_findings_py["(生产态 / production) arbitratefindings.py — Finding 仲裁器（跨脚本冲 / arbitrate_findings<br/>Finding 仲裁器（跨脚本冲突解决引擎）<br/>文件: meta/arbitrate_findings.py"]
    scripts_governance_meta_benchmark_test_fixtures_incomplete_module_py["(生产态 / production) incomplete模块 / Module docstring — see module-level docstring for details.<br/>incomplete模块。Module docstring — see module-level docstring for details.<br/>文件: test_fixtures/incomplete_module.py"]
    scripts_governance_meta_compute_sla_metrics_py["(生产态 / production) computeslametrics.py — SLA/SLO 指标计算引擎（ / compute_sla_metrics<br/>SLA/SLO 指标计算引擎（蓝图 §8.4）<br/>文件: meta/compute_sla_metrics.py"]
    scripts_governance_meta_create_task_from_finding_py["(生产态 / production) 创建任务fromfinding.py — Finding →  / create_task_from_finding<br/>Finding → 任务卡自动创建引擎<br/>文件: meta/create_task_from_finding.py"]
    scripts_governance_meta_detect_config_deviation_py["(生产态 / production) 检测配置deviation.py — 配置文件结构完整性检测 / detect_config_deviation<br/>配置文件结构完整性检测（蓝图 §28 B65 + B87）<br/>文件: meta/detect_config_deviation.py"]
    scripts_governance_meta_detect_fix_oscillation_py["(生产态 / production) 检测修复oscillation.py — 自修复振荡检测（蓝图 § / detect_fix_oscillation<br/>自修复振荡检测（蓝图 §28 B64）<br/>文件: meta/detect_fix_oscillation.py"]
    scripts_governance_meta_detect_hallucinated_packages_py["(生产态 / production) 检测hallucinatedpackages.py — 幻觉包（Sl / detect_hallucinated_packages<br/>幻觉包（Slopsquatting）防御引擎<br/>文件: meta/detect_hallucinated_packages.py"]
    scripts_governance_meta_detect_script_divergence_py["(生产态 / production) 检测scriptdivergence.py — 脚本实现与蓝图规范分 / detect_script_divergence<br/>脚本实现与蓝图规范分歧检测（蓝图 §27.3 B81）<br/>文件: meta/detect_script_divergence.py"]
    scripts_governance_meta_detect_script_rot_py["(生产态 / production) 检测scriptrot.py — Script Rot（脚本静默失效 / detect_script_rot<br/>Script Rot（脚本静默失效）检测器<br/>文件: meta/detect_script_rot.py"]
    scripts_governance_meta_env_check_py["(生产态 / production) 环境check.py — 环境就绪检查门禁 (Environment Rea / env_check<br/>环境就绪检查门禁 (Environment Readiness Gate)<br/>文件: meta/env_check.py"]
    scripts_governance_meta_finding_state_machine_py["(生产态 / production) 发现状态machine.py — Finding 全生命周期 / finding_state_machine<br/>Finding 全生命周期状态机<br/>文件: meta/finding_state_machine.py"]
    scripts_governance_meta_gate_engine_selfcheck_py["(生产态 / production) 门禁引擎selfcheck / Gate Engine Bootstrap Self-Check — Quis custodiet ipsos cust<br/>门禁引擎selfcheck。Gate Engine Bootstrap Self-Check — Quis custodiet ipsos custodes?<br/>文件: meta/gate_engine_selfcheck.py"]
    scripts_governance_meta_governance_watchdog_py["(生产态 / production) 治理watchdog / Module docstring — see module-level docstring for details.<br/>治理watchdog。Module docstring — see module-level docstring for details.<br/>文件: meta/governance_watchdog.py"]
    scripts_governance_meta_manage_error_budget_py["(生产态 / production) 管理错误budget.py — Error Budget +  / manage_error_budget<br/>Error Budget + Burn Rate 管理引擎<br/>文件: meta/manage_error_budget.py"]
    scripts_governance_meta_manage_finding_timeseries_py["(生产态 / production) 管理发现timeseries.py — Finding 时 / manage_finding_timeseries<br/>Finding 时序数据库 + 趋势分析引擎<br/>文件: meta/manage_finding_timeseries.py"]
    scripts_governance_meta_manage_script_ab_test_py["(生产态 / production) 管理scriptabtest.py — 脚本 A/B 对照模式 ( / manage_script_ab_test<br/>脚本 A/B 对照模式 (Kayenta-style)<br/>文件: meta/manage_script_ab_test.py"]
    scripts_governance_meta_manage_script_retirement_py["(生产态 / production) 管理scriptretirement.py — 脚本退役/废弃生命周 / manage_script_retirement<br/>脚本退役/废弃生命周期管理<br/>文件: meta/manage_script_retirement.py"]
    scripts_governance_meta_manage_shadow_mode_py["(生产态 / production) 管理shadowmode.py — Shadow Mode 渐进激活 / manage_shadow_mode<br/>Shadow Mode 渐进激活管理<br/>文件: meta/manage_shadow_mode.py"]
    scripts_governance_meta_mutation_test_post_sync_validator_py["(生产态 / production) mutation测试提交同步validator.py — S / mutation_test_post_sync_validator<br/>SSoT 变异测试（独立 oracle）<br/>文件: meta/mutation_test_post_sync_validator.py"]
    scripts_governance_meta_mutation_test_reconciliation_registry_py["(生产态 / production) mutation测试reconciliationregistry.py / mutation_test_reconciliation_registry<br/>ReconciliationRegistry SSoT 变异测试<br/>文件: meta/mutation_test_reconciliation_registry.py"]
    scripts_governance_meta_phase_e_context_check_py["(生产态 / production) 阶段e上下文检查 / Phase E: AI context injection verification script<br/>阶段e上下文检查。Phase E: AI context injection verification script<br/>文件: meta/phase_e_context_check.py"]
    scripts_governance_meta_pre_op_check_py["(生产态 / production) AI操作前准入控制器 — 写/删文件前的机械门禁检查. / pre_op_check<br/>AI操作前准入控制器 — 写/删文件前的机械门禁检查.<br/>文件: meta/pre_op_check.py"]
    scripts_governance_meta_score_script_effectiveness_py["(生产态 / production) 评分scripteffectiveness.py — 脚本有效性评分（ / score_script_effectiveness<br/>脚本有效性评分（蓝图 §27.12 B90）<br/>文件: meta/score_script_effectiveness.py"]
    scripts_governance_meta_session_startup_check_py["(生产态 / production) Session 冷启动自检 — 运行 Phase 0 全部 14 个检查并输出状 / session_startup_check<br/>Session 冷启动自检 — 运行 Phase 0 全部 14 个检查并输出状态报告.<br/>文件: meta/session_startup_check.py"]
    scripts_governance_meta_trace_finding_lifecycle_py["(生产态 / production) 追踪发现lifecycle.py — Finding C1→ / trace_finding_lifecycle<br/>Finding C1→C5 全链路追踪引擎<br/>文件: meta/trace_finding_lifecycle.py"]
    scripts_governance_meta_track_script_costs_py["(生产态 / production) trackscriptcosts.py — 脚本执行 AI 费用追踪 / track_script_costs<br/>脚本执行 AI 费用追踪<br/>文件: meta/track_script_costs.py"]
    scripts_governance_meta_validate_automation_boundary_py["(生产态 / production) 校验automationboundary / Module docstring — see module-level docstring for details.<br/>校验automationboundary。Module docstring — see module-level docstring for details.<br/>文件: meta/validate_automation_boundary.py"]
    scripts_governance_meta_validate_cross_model_consensus_py["(生产态 / production) 校验跨模型consensus.py — 多AI模 / validate_cross_model_consensus<br/>多AI模型共识验证引擎<br/>文件: meta/validate_cross_model_consensus.py"]
    scripts_governance_meta_validate_dependency_chain_py["(生产态 / production) 校验依赖chain.py — 依赖链拓扑顺序验证 / validate_dependency_chain<br/>依赖链拓扑顺序验证<br/>文件: meta/validate_dependency_chain.py"]
    scripts_governance_meta_validate_emergency_bypass_log_py["(生产态 / production) 校验紧急bypasslog.py — 应急绕过审 / validate_emergency_bypass_log<br/>应急绕过审计脚本<br/>文件: meta/validate_emergency_bypass_log.py"]
    scripts_governance_meta_validate_end_to_end_benchmark_py["(生产态 / production) 校验结束to结束benchmark.py — END-T / validate_end_to_end_benchmark<br/>END-TO-END 基准测试引擎<br/>文件: meta/validate_end_to_end_benchmark.py"]
    scripts_governance_meta_validate_environment_health_py["(生产态 / production) 校验环境health.py — 脚本运行环境健 / validate_environment_health<br/>脚本运行环境健康检查<br/>文件: meta/validate_environment_health.py"]
    scripts_governance_meta_validate_false_negatives_py["(生产态 / production) 校验falsenegatives.py — 假阴性检测引擎 (F / validate_false_negatives<br/>假阴性检测引擎 (Fitness Functions)<br/>文件: meta/validate_false_negatives.py"]
    scripts_governance_meta_validate_gate_engine_external_py["(生产态 / production) 校验门禁引擎external.py — Gate  / validate_gate_engine_external<br/>Gate Engine 外部完整性验证<br/>文件: meta/validate_gate_engine_external.py"]
    scripts_governance_meta_validate_mutation_testing_py["(生产态 / production) 校验mutationtesting.py — 变异测试引擎（蓝图 / validate_mutation_testing<br/>变异测试引擎（蓝图 §19.2 + B75）<br/>文件: meta/validate_mutation_testing.py"]
    scripts_governance_meta_validate_rule_freshness_py["(生产态 / production) 校验规则freshness.py — AI Session  / validate_rule_freshness<br/>AI Session 注入文件新鲜度检查（蓝图 §22.3 + B62）<br/>文件: meta/validate_rule_freshness.py"]
    scripts_governance_meta_validate_rules_file_backdoor_py["(生产态 / production) 校验rules文件backdoor.py — Rules  / validate_rules_file_backdoor<br/>Rules File Backdoor 检测器<br/>文件: meta/validate_rules_file_backdoor.py"]
    scripts_governance_meta_validate_rules_integrity_py["(生产态 / production) 校验rulesintegrity.py — 规则文件完整性保护 / validate_rules_integrity<br/>规则文件完整性保护<br/>文件: meta/validate_rules_integrity.py"]
    scripts_governance_meta_validate_script_onboarding_py["(生产态 / production) 校验scriptonboarding / Module docstring — see module-level docstring for details.<br/>校验scriptonboarding。Module docstring — see module-level docstring for details.<br/>文件: meta/validate_script_onboarding.py"]
    scripts_governance_meta_validate_script_provenance_py["(生产态 / production) 校验scriptprovenance.py — 脚本 Prove / validate_script_provenance<br/>脚本 Provenance 溯源链<br/>文件: meta/validate_script_provenance.py"]
    scripts_governance_meta_validate_script_system_health_py["(生产态 / production) 校验scriptsystemhealth.py — 脚本系统健 / validate_script_system_health<br/>脚本系统健康自检（Meta 维度 / 第 13 维度）<br/>文件: meta/validate_script_system_health.py"]
    scripts_governance_meta_validate_threshold_changes_py["(生产态 / production) 校验阈值changes.py — 阈值变更审计日志 / validate_threshold_changes<br/>阈值变更审计日志<br/>文件: meta/validate_threshold_changes.py"]
    scripts_governance_meta_validate_trust_tier_py["(生产态 / production) 校验信任tier.py — Trust-Tier 门禁执行 / validate_trust_tier<br/>Trust-Tier 门禁执行器<br/>文件: meta/validate_trust_tier.py"]
    scripts_governance_meta_verify_reconciliation_registry_py["(生产态 / production) 验证reconciliationregistry.py — Reco / verify_reconciliation_registry<br/>ReconciliationRegistry 轻量结构 audit<br/>文件: meta/verify_reconciliation_registry.py"]
    scripts_governance_migrate_sqlite_to_pg_migrate_data_py["(生产态 / production) SQLite → PostgreSQL 运营数据迁移脚本 / migrate_data<br/>SQLite → PostgreSQL 运营数据迁移脚本<br/>文件: migrate_sqlite_to_pg/migrate_data.py"]
    scripts_governance_migrate_sqlite_to_pg_seed_from_yaml_py["(生产态 / production) 种子fromyaml.py — 从 YAML 真源灌种子表（5.32.1 / seed_from_yaml<br/>从 YAML 真源灌种子表（5.32.10 治本：种子与迁移拆分）<br/>文件: migrate_sqlite_to_pg/seed_from_yaml.py"]
    scripts_governance_migrate_to_metadata_tables_py["(生产态 / production) migrateto元数据tables.py — 裁定#209 S / migrate_to_metadata_tables<br/>裁定#209 Stage 2 一次性迁移脚本<br/>文件: governance/migrate_to_metadata_tables.py"]
    scripts_governance_oneoff_data_domain_audit_query_py["(生产态 / production) 数据域设计态排查 - DB 现状查询（Phase 2，只读不写）。 / data_domain_audit_query<br/>数据域设计态排查 - DB 现状查询（Phase 2，只读不写）。<br/>文件: oneoff/data_domain_audit_query.py"]
    scripts_governance_oneoff_data_domain_design_state_complete_py["(设计态 / design) 数据域四图设计态补全——一次性执行脚本。 / data_domain_design_state_complete<br/>数据域四图设计态补全——一次性执行脚本。<br/>文件: oneoff/data_domain_design_state_complete.py"]
    scripts_governance_query_module_panorama_py["(生产态 / production) 查询模块panorama.py — 模块全景查询入口（四图模块 / query_module_panorama<br/>模块全景查询入口（四图模块对齐 Step 5）<br/>文件: governance/query_module_panorama.py"]
    scripts_governance_register_deferred_modules_py["(生产态 / production) 将42项暂缓模块写入 depgraph 设计态，含3图对齐设计。 / register_deferred_modules<br/>将42项暂缓模块写入 depgraph 设计态，含3图对齐设计。<br/>文件: governance/register_deferred_modules.py"]
    scripts_governance_repair_concurrent_commit_test_py["(生产态 / production) 并发提交test.py — 幽灵提交红蓝对抗脚本（O / concurrent_commit_test<br/>幽灵提交红蓝对抗脚本<br/>文件: repair/concurrent_commit_test.py"]
    scripts_governance_run_all_py["(生产态 / production) run_all.py — 脚本系统统一入口脚本 / run_all<br/>脚本系统统一入口脚本<br/>文件: governance/run_all.py"]
    scripts_governance_run_gate_chain_py["(生产态 / production) 运行门禁chain.py — 顺序运行多个门禁脚本，任一失败即整体失败 / run_gate_chain<br/>顺序运行多个门禁脚本，任一失败即整体失败。<br/>文件: governance/run_gate_chain.py"]
    scripts_governance_run_silent_failure_regression_py["(生产态 / production) 运行silentfailureregression.py — silen / run_silent_failure_regression<br/>silent-failure 回归套件一键执行入口<br/>文件: governance/run_silent_failure_regression.py"]
    scripts_governance_session_startup_health_check_py["(生产态 / production) 会话启动健康check.py — AI ses / session_startup_health_check<br/>AI session 启动健康度自检（ARCH-TOOL-HEALTH-V1 Phase 6）<br/>文件: governance/session_startup_health_check.py"]
    scripts_governance_status_py["(生产态 / production) status.py — 审计系统状态仪表盘 / status<br/>审计系统状态仪表盘<br/>文件: governance/status.py"]
    scripts_governance_verify_sync_integrity_py["(生产态 / production) sync 完整性校验脚本：验证 YAML→DB 同步的一致性。 / verify_sync_integrity<br/>sync 完整性校验脚本：验证 YAML→DB 同步的一致性。<br/>文件: governance/verify_sync_integrity.py"]
    scripts_governance_vms_vms_blindspot_check_py["(生产态 / production) VMS 盲点闭合检查器 — MOD-INF-011 · R1(33) + R2( / vms_blindspot_check<br/>VMS 盲点闭合检查器 — MOD-INF-011 · R1(33) + R2(22) + R4(6)<br/>文件: vms/vms_blindspot_check.py"]
    scripts_governance_vms_vms_build_completion_check_py["(生产态 / production) vms构建completion检查 / VMS Build Completion Check — MOD-INF-011 · TASK-INF-0217<br/>vms构建completion检查。VMS Build Completion Check — MOD-INF-011 · TASK-INF-0217<br/>文件: vms/vms_build_completion_check.py"]
    scripts_governance_vms_vms_cron_monitor_py["(生产态 / production) VMS Cron 监控器 — MOD-INF-011 · TASK-INF-02 / vms_cron_monitor<br/>VMS Cron 监控器 — MOD-INF-011 · TASK-INF-0224<br/>文件: vms/vms_cron_monitor.py"]
    scripts_governance_vms_vms_cross_file_check_py["(生产态 / production) VMS 跨文件内容一致性检查器 — MOD-INF-011 · TASK-INF / vms_cross_file_check<br/>VMS 跨文件内容一致性检查器 — MOD-INF-011 · TASK-INF-0211<br/>文件: vms/vms_cross_file_check.py"]
    scripts_governance_vms_vms_health_check_py["(生产态 / production) VMS Health Check 脚本 — MOD-INF-011 · Phas / vms_health_check<br/>VMS Health Check 脚本 — MOD-INF-011 · Phase 3 运维自动化<br/>文件: vms/vms_health_check.py"]
    scripts_governance_vms_vms_migrate_py["(生产态 / production) VMS Phase 2 数据迁移脚本 — MOD-INF-011 / vms_migrate<br/>VMS Phase 2 数据迁移脚本 — MOD-INF-011<br/>文件: vms/vms_migrate.py"]
    scripts_governance_vms_vms_migration_dry_run_py["(生产态 / production) VMS 迁移 dry-run 脚本 — MOD-INF-011 Phase 2  / vms_migration_dry_run<br/>VMS 迁移 dry-run 脚本 — MOD-INF-011 Phase 2 前置检查<br/>文件: vms/vms_migration_dry_run.py"]
    scripts_governance_vms_vms_phase_rollback_py["(生产态 / production) VMS Phase 回滚方案 — MOD-INF-011 · TASK-INF- / vms_phase_rollback<br/>VMS Phase 回滚方案 — MOD-INF-011 · TASK-INF-0217<br/>文件: vms/vms_phase_rollback.py"]
    scripts_governance_vms_vms_version_sync_check_py["(生产态 / production) VMS 版本同步检查器 — MOD-INF-011 · TASK-INF-022 / vms_version_sync_check<br/>VMS 版本同步检查器 — MOD-INF-011 · TASK-INF-0222<br/>文件: vms/vms_version_sync_check.py"]
    tests_governance_scripts_governance_test_any_type_inferrer_py["(生产态 / production) 测试anytypeinferrer.py — anytypeinf / test_any_type_inferrer<br/>any_type_inferrer.py 单元测试。<br/>文件: scripts_governance/test_any_type_inferrer.py"]
    tests_governance_scripts_governance_test_check_canonical_yaml_drift_py["(生产态 / production) 测试检查canonicalyamldrift.py — GAT / test_check_canonical_yaml_drift<br/>GATE-CANONICAL-YAML-DRIFT 单元测试（Phase B，2026-07-24）<br/>文件: scripts_governance/test_check_canonical_yaml_drift.py"]
    tests_governance_scripts_governance_test_check_vocab_hardcode_py["(生产态 / production) 测试检查vocabhardcode.py — GATE-VOCA / test_check_vocab_hardcode<br/>GATE-VOCAB 检测7 单元测试（2026-06-30 治本补全）<br/>文件: scripts_governance/test_check_vocab_hardcode.py"]
    tests_governance_scripts_governance_test_pre_write_gate_py["(生产态 / production) 测试预writegate.py — 检查会话 / test_pre_write_gate<br/>_check_session_overlap 单元测试（claim 前移协议防线）<br/>文件: scripts_governance/test_pre_write_gate.py"]
    tests_governance_test_check_blueprint_code_alignment_py["(生产态 / production) 测试检查蓝图代码alignment / tests for check_blueprint_code_alignment.py — ARCH-FRONTMATT<br/>隔离 BLUEPRINTS_DIR 和 REPO_ROOT，防止扫描真实项目文件。<br/>文件: governance/test_check_blueprint_code_alignment.py"]
    ...
    scripts_governance_archive_vms_ri_vms_cron_monitor_py ~~~ scripts_governance_archive_vms_ri_vms_health_check_py
    ...
    scripts_governance_shared_module_translation_loader_py ~~~ scripts_governance_sync_check_p0_status_py
    scripts_governance_sync_check_p0_status_py ~~~ scripts_governance_sync_cleanup_p0_ops_pending_py
    scripts_governance_sync_cleanup_p0_ops_pending_py ~~~ scripts_governance_tasks_list_phase0_tasks_py
    ...
    scripts_governance_archive_prototype_adversarial_sys_master_test_py["(生产态 / production) adversarialsys主测试 / Red/Blue Team Adversarial Test v3: SYS-MASTER-001 + MOD-MAST<br/>adversarialsys主测试。Red/Blue Team Adversarial Test v3: SYS-MASTER-001 + MOD-MASTER_BLUEPRINT Integration Hardening<br/>文件: prototype/adversarial_sys_master_test.py"]
    scripts_governance_archive_vms_ri_ri_boundary_check_py["(生产态 / production) Runtime Integration 边界验证脚本 — MOD-INF-002 / ri_boundary_check<br/>Runtime Integration 边界验证脚本 — MOD-INF-002<br/>文件: vms_ri/ri_boundary_check.py"]
    scripts_governance_shared_frontmatter_py["(生产态 / production) 文件头部格式解析 SSoT（Single Source of Truth） / frontmatter<br/>文件头部格式解析 SSoT（Single Source of Truth）<br/>文件: _shared/frontmatter.py"]
    scripts_governance_shared_libcst_docstring_adder_py["(生产态 / production) libcstdocstringadder / libcst_docstring_adder.py — Lossless docstring addition usin<br/>libcstdocstringadder，治理的功能模块。<br/>文件: _shared/libcst_docstring_adder.py"]
    scripts_governance_shared_registry_entry_count_py["(生产态 / production) 登记表主条目计数——与 generate注册表主ind / registry_entry_count<br/>登记表主条目计数——与 generate_registry_master_index 单一真源对齐。<br/>文件: _shared/registry_entry_count.py"]
    scripts_governance_shared_terminology_loader_py["(生产态 / production) 术语loader.py — 架构文档术语词汇表共享加载器（S / terminology_loader<br/>架构文档术语词汇表共享加载器（SSoT 真源）<br/>文件: _shared/terminology_loader.py"]
    scripts_governance_shared_yaml_utils_py["(生产态 / production) shared/yamlutils.py — YAML 文件加载共享工具 / yaml_utils<br/>YAML 文件加载共享工具<br/>文件: _shared/yaml_utils.py"]
    scripts_governance_sync_fix_orphan_deps_py["(生产态 / production) 修复orphandeps.py — 一次性修复孤儿依赖引用 / fix_orphan_deps<br/>一次性修复孤儿依赖引用<br/>文件: _sync/fix_orphan_deps.py"]
    scripts_governance_d3_metadata_validate_module_id_naming_py["(生产态 / production) 模块id / domainid / submoduleid 格式校 / validate_module_id_naming<br/>module_id / domain_id / submodule_id 格式校验真源（裁定#208 双轨制 + R2 治本修订）<br/>文件: d3_metadata/validate_module_id_naming.py"]
    scripts_governance_d5_architecture_generators_common_py["(生产态 / production) 生成器公共工具（向内收：消除重复）。 / _common<br/>生成器公共工具（向内收：消除重复）。<br/>文件: generators/_common.py"]
    scripts_governance_d7_code_check_any_abuse_py["(生产态 / production) 类型注解 Any 滥用扫描器 — 5.145 维度防御门闸（R70 引入，#AR / check_any_abuse<br/>类型注解 Any 滥用扫描器 — 5.145 维度防御门闸（R70 引入，#ARCH-ANY-GOVERNANCE-001 Phase 3 升级为 commit 阻断）。<br/>文件: d7_code/check_any_abuse.py"]
    scripts_governance_d8_doc_sync_audit_rename_completeness_py["(生产态 / production) 审计renamecompleteness.py — 改名完整性审计（裁 / audit_rename_completeness<br/>改名完整性审计（裁定#207 R1）。<br/>文件: d8_doc_sync/audit_rename_completeness.py"]
    scripts_governance_d8_doc_sync_sync_yaml_to_depgraph_py["(生产态 / production) (INVARIANTS) YAML→DB单向同步; 27项同步; try/fin / sync_yaml_to_depgraph<br/>(INVARIANTS) YAML→DB单向同步; 27项同步; try/finally恢复触发器<br/>文件: d8_doc_sync/sync_yaml_to_depgraph.py"]
    scripts_governance_meta_concurrency_py["(生产态 / production) concurrency / Module docstring — see module-level docstring for details.<br/>S0-S3 分级超时——对齐 K8s QoS Classes。<br/>文件: meta/_concurrency.py"]
    scripts_governance_meta_backup_runtime_state_py["(生产态 / production) 备份运行时state.py — 运行时状态备份（蓝图 §33 / backup_runtime_state<br/>运行时状态备份（蓝图 §33 灾备）<br/>文件: meta/backup_runtime_state.py"]
    scripts_governance_meta_benchmark_test_fixtures_bad_imports_py["(生产态 / production) badimports / Module docstring — see module-level docstring for details.<br/>badimports，测试的功能模块。<br/>文件: test_fixtures/bad_imports.py"]
    scripts_governance_meta_manage_baseline_py["(生产态 / production) 管理baseline.py — Finding 基线快照管理 / manage_baseline<br/>Finding 基线快照管理<br/>文件: meta/manage_baseline.py"]
    scripts_governance_sync_panorama_module_py["(生产态 / production) 同步panoramamodule.py — 四图模块同步引擎（ARCH- / sync_panorama_module<br/>四图模块同步引擎<br/>文件: governance/sync_panorama_module.py"]
    ...
    scripts_governance_shared_yaml_utils_py ~~~ scripts_governance_sync_fix_orphan_deps_py
    scripts_governance_sync_fix_orphan_deps_py ~~~ scripts_governance_d3_metadata_validate_module_id_naming_py
    ...
    scripts_governance_archive_vms_ri_vms_cross_file_check_py["(生产态 / production) VMS 跨文件内容一致性检查器 — MOD-INF-011 · TASK-INF / vms_cross_file_check<br/>VMS 跨文件内容一致性检查器 — MOD-INF-011 · TASK-INF-0211<br/>文件: vms_ri/vms_cross_file_check.py"]
    scripts_governance_shared_encoding_py["(生产态 / production) encoding.py — UTF-8 编码安全工具 / encoding<br/>UTF-8 编码安全工具<br/>文件: _shared/encoding.py"]
    scripts_governance_shared_file_utils_py["(生产态 / production) shared/fileutils.py — 原子写入共享工具（ARCH-03 / file_utils<br/>原子写入共享工具<br/>文件: _shared/file_utils.py"]
    scripts_governance_shared_thresholds_py["(生产态 / production) thresholds.py — 阈值集中配置加载器 / thresholds<br/>阈值集中配置加载器<br/>文件: _shared/thresholds.py"]
    scripts_governance_shared_walk_py["(生产态 / production) walk.py — 目录遍历共享工具 / walk<br/>目录遍历共享工具<br/>文件: _shared/walk.py"]
    scripts_governance_sync_cleanup_p0_auto_bridged_py["(生产态 / production) 清理历史 P0 自动桥接任务 / cleanup_p0_auto_bridged<br/>清理历史 P0 自动桥接任务<br/>文件: _sync/cleanup_p0_auto_bridged.py"]
    scripts_governance_d5_architecture_syncers_blueprint_frontmatter_reconciler_py["(生产态 / production) 蓝图frontmatterreconciler.py — 蓝图 / blueprint_frontmatter_reconciler<br/>蓝图 frontmatter 核心字段对齐（ARCH-056 Phase 3）<br/>文件: syncers/blueprint_frontmatter_reconciler.py"]
    scripts_governance_meta_benchmark_test_fixtures_orphan_file_without_module_registration_py["(生产态 / production) orphan文件without模块registration / Module docstring — see module-level docstring for details.<br/>orphan文件without模块registration。Module docstring — see module-level docstring for details.<br/>文件: test_fixtures/orphan_file_without_module_registration.py"]
    scripts_governance_archive_vms_ri_vms_cross_file_check_py ~~~ scripts_governance_shared_encoding_py
    ...
    scripts_governance_shared_walk_py ~~~ scripts_governance_sync_cleanup_p0_auto_bridged_py
    scripts_governance_sync_cleanup_p0_auto_bridged_py ~~~ scripts_governance_d5_architecture_syncers_blueprint_frontmatter_reconciler_py
    scripts_governance_d5_architecture_syncers_blueprint_frontmatter_reconciler_py ~~~ scripts_governance_meta_benchmark_test_fixtures_orphan_file_without_module_registration_py
    scripts_governance_shared_constants_py["(生产态 / production) constants.py — 审计脚本共享常量 / constants<br/>审计脚本共享常量<br/>文件: _shared/constants.py"]
    scripts_governance_d5_architecture_panorama_common_py["(生产态 / production) panoramacommon.py — 四图投票共享工具（ARCH-056 引 / panorama_common<br/>四图投票共享工具（ARCH-056 引擎加固）<br/>文件: d5_architecture/panorama_common.py"]
    ...
    scripts_governance_archive_vms_ri_ri_boundary_check_py -->|config_depends / config_depends| scripts_governance_archive_vms_ri_vms_cross_file_check_py
    ...
    D_AUTONOMY_PERM["(生产态 / production) 自治保护 / Autonomy Protection<br/>自治保护，负责 AI 自治行为的权限控制和安全边界<br/>跨域节点 / cross-domain"]
    ...
    class D_DATA,D_GOVERNANCE,D_SHARED,D_GOV_AUDIT,D_GOV_ENFORCEMENT,D_AUTONOMY_PERM,D_GOV_RULE,D_OPS,D_GOV_DRIFT external_prod
```

### 运营态的图（仅 design_maturity=production 的模块和域内依赖）

> 仅展示已上线运行的模块（共 384 个），不含跨域外部节点。跨域依赖见下方跨域依赖章节。
...
|:--:|---------|:--:|---------|---------|
| 1 | Code Wiki 统计数据生成器（半自动维护机制）。 / generate_c... | → | D_DATA 数据接入层: 表名/品类注册表消费层（裁定 #ARCH-CH-024 Phase 2）。 / ta... | 导入依赖 / import_depends |
| 2 | G-inventory: 扫描 ClickHouse 生成业务数据清单 MD / genera... | → | D_DATA 数据接入层: zephyr.data — 数据源集成器（MOD-L00-004）。 / __init__ (... | 导入依赖 / import_depends |
| 3 | G-inventory: 扫描 ClickHouse 生成业务数据清单 MD / genera... | → | D_DATA 数据接入层: ClickHouse 统一读取层（裁定 #ARCH-CH-007）。 / ch_reader ... | 导入依赖 / import_depends |
| 4 | 逐笔data 表真重复检查工具（RULE-DATA-OPS 配套，TRAE / che... | → | D_DATA 数据接入层: zephyr.data — 数据源集成器（MOD-L00-004）。 / __init__ (... | 导入依赖 / import_depends |
| 5 | 逐笔data 表真重复检查工具（RULE-DATA-OPS 配套，TRAE / che... | → | D_DATA 数据接入层: ClickHouse 统一读取层（裁定 #ARCH-CH-007）。 / ch_reader ... | 导入依赖 / import_depends |
| 6 | 审计提交同步commands.py — post同步 / audit_post_sync_com... | → | D_GOVERNANCE 生命周期管理: 提交同步validator — post同步标准 / post_sync_validator (... | 导入依赖 / import_depends |
| 7 | 创建alignment任务 / # [BLUEPRINT] MOD-INF-005 | scripts/g... | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1-04 / task... | 导入依赖 / import_depends |
| 8 | 修复broken提交sync.py — 批量修复历史 broken  / fix_broke... | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1-04 / task... | 导入依赖 / import_depends |
| 9 | Construction Gate — 施工前路径校验门禁 / construction_ga... | → | D_GOVERNANCE 生命周期管理: PathResolver — 模块路径解析器 / path_resolver (architect... | 导入依赖 / import_depends |
| 10 | constants.py — 审计脚本共享常量 / constants (_shared/con... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 / depgraph_schema (g... | 导入依赖 / import_depends |
| 11 | governance/taskshow 脚本 — 任务卡详情查询 CLI。 / task_s... | → | D_GOVERNANCE 生命周期管理: SQLite 元数据层 Schema DDL + 版本化迁移框架（T-1-02  / sq... | 导入依赖 / import_depends |
| 12 | governance/taskshow 脚本 — 任务卡详情查询 CLI。 / task_s... | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1-04 / task... | 导入依赖 / import_depends |
| 13 | 任务summary.py — 任务系统全局摘要 CLI / task_summary (_t... | → | D_GOVERNANCE 生命周期管理: SQLite 元数据层 Schema DDL + 版本化迁移框架（T-1-02  / sq... | 导入依赖 / import_depends |
| 14 | 任务summary.py — 任务系统全局摘要 CLI / task_summary (_t... | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1-04 / task... | 导入依赖 / import_depends |
| 15 | 为暂缓模块添加设计态依赖边（depmaturity='design'）。 / ad... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 / depgraph_schema (g... | 导入依赖 / import_depends |
| 16 | 应用dataflowgraph.py — dataflowgraph 变 / apply_dataflow... | → | D_GOVERNANCE 生命周期管理: dataflowgraph Schema DDL + 连接入口 / dataflowgraph_schem... | 导入依赖 / import_depends |
| 17 | [INVARIANTS] pgadvisorylock 写锁; build / apply_decisiong... | → | D_GOVERNANCE 生命周期管理: decisiongraph Schema DDL + 不变量声明 / decisiongraph_sch... | 导入依赖 / import_depends |
| 18 | GATE-SSOT: SSoT 创建门禁（pre-commit hook 双保险 / check_... | → | D_GOVERNANCE 生命周期管理: CapabilityLookup — 能力->真源文件反查注册表的查询 API  /... | 导入依赖 / import_depends |
| 19 | 任务自check.py — 任务系统自身健康检查 / task_self_check ... | → | D_GOVERNANCE 生命周期管理: SQLite 元数据层 Schema DDL + 版本化迁移框架（T-1-02  / sq... | 导入依赖 / import_depends |
| 20 | 任务自check.py — 任务系统自身健康检查 / task_self_check ... | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1-04 / task... | 导入依赖 / import_depends |
| 21 | 验证结构health.py — depgraph (Post / verify_schema_healt... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 / depgraph_schema (g... | 导入依赖 / import_depends |
| 22 | 验证结构health.py — depgraph (Post / verify_schema_healt... | → | D_GOVERNANCE 生命周期管理: decisiongraph Schema DDL + 不变量声明 / decisiongraph_sch... | 导入依赖 / import_depends |
| 23 | GTRAE059 验证脚本：结构version 写入保护 + 版 / check_sche... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 / depgraph_schema (g... | 导入依赖 / import_depends |
| 24 | analyze变更冲击 / Module docstring — see module-level do... | → | D_GOVERNANCE 生命周期管理: LLMImpactAnalyzer — LLM-based commit 语义影 / llm_impact... | 导入依赖 / import_depends |
| 25 | G-panorama-align: 四图对齐检测器（ARCH-053 + ARC / align_... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 / depgraph_schema (g... | 导入依赖 / import_depends |
| 26 | G-panorama-align: 四图对齐检测器（ARCH-053 + ARC / align_... | → | D_GOVERNANCE 生命周期管理: dataflowgraph Schema DDL + 连接入口 / dataflowgraph_schem... | 导入依赖 / import_depends |
| 27 | G-panorama-align: 四图对齐检测器（ARCH-053 + ARC / align_... | → | D_GOVERNANCE 生命周期管理: decisiongraph Schema DDL + 不变量声明 / decisiongraph_sch... | 导入依赖 / import_depends |
| 28 | G-panorama-gen: 蓝图 §0.6 四图对齐视图生成器（ARCH-0 / g... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 / depgraph_schema (g... | 导入依赖 / import_depends |
| 29 | G-panorama-gen: 蓝图 §0.6 四图对齐视图生成器（ARCH-0 / g... | → | D_GOVERNANCE 生命周期管理: dataflowgraph Schema DDL + 连接入口 / dataflowgraph_schem... | 导入依赖 / import_depends |
| 30 | G-panorama-gen: 蓝图 §0.6 四图对齐视图生成器（ARCH-0 / g... | → | D_GOVERNANCE 生命周期管理: decisiongraph Schema DDL + 不变量声明 / decisiongraph_sch... | 导入依赖 / import_depends |
| 31 | G-dataflow: 从 dataflowgraph (PostgreSQL) / generate_data... | → | D_GOVERNANCE 生命周期管理: dataflowgraph Schema DDL + 连接入口 / dataflowgraph_schem... | 导入依赖 / import_depends |
| 32 | G-decision: 从 decisiongraph (PostgreSQL) / generate_deci... | → | D_GOVERNANCE 生命周期管理: decisiongraph Schema DDL + 不变量声明 / decisiongraph_sch... | 导入依赖 / import_depends |
| 33 | G-trading-flow: 从 decisiongraph + 叙事YAML / generate_tr... | → | D_GOVERNANCE 生命周期管理: 决策图reader.py — 决策流图数据库只读查询工具 / decision_... | 导入依赖 / import_depends |
| 34 | 蓝图frontmatterreconciler.py — 蓝图 / blueprint_frontmat... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 / depgraph_schema (g... | 导入依赖 / import_depends |
| 35 | [INVARIANTS] YAML→DB单向同步; 27项同步; try/fin / sync_y... | → | D_GOVERNANCE 生命周期管理: dataflowgraph Schema DDL + 连接入口 / dataflowgraph_schem... | 导入依赖 / import_depends |
| 36 | 提取decisiongraph / extract_decisiongraph - decisiongraph... | → | D_GOVERNANCE 生命周期管理: 决策图reader.py — 决策流图数据库只读查询工具 / decision_... | 导入依赖 / import_depends |
| 37 | 提取decisiongraph / extract_decisiongraph - decisiongraph... | → | D_GOVERNANCE 生命周期管理: decisiongraph Schema DDL + 不变量声明 / decisiongraph_sch... | 导入依赖 / import_depends |
| 38 | [INVARIANTS] YAML 是唯一真源; DB 为只读缓存; 同步单向  / ... | → | D_GOVERNANCE 生命周期管理: decisiongraph Schema DDL + 不变量声明 / decisiongraph_sch... | 导入依赖 / import_depends |
| 39 | 生成project依赖图 / # [BLUEPRINT] MOD-INF-005 | scripts/g... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 / depgraph_schema (g... | 导入依赖 / import_depends |
| 40 | 从蓝图§0.1聚合生成 pathownershipmap.yaml 路径归属 / gene... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 / depgraph_schema (g... | 导入依赖 / import_depends |
| 41 | 从蓝图§0.1聚合生成 pathownershipmap.yaml 路径归属 / gene... | → | D_GOVERNANCE 生命周期管理: 规则patterns.py — 治理规则正则 + 安全审计模式唯一真源 ( ... | 导入依赖 / import_depends |
| 42 | 备份运行时state.py — 运行时状态备份（蓝图 §33 / backup_... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 / depgraph_schema (g... | 导入依赖 / import_depends |
| 43 | 创建任务fromfinding.py — Finding →  / create_task_from_... | → | D_GOVERNANCE 生命周期管理: SQLite 元数据层 Schema DDL + 版本化迁移框架（T-1-02  / sq... | 导入依赖 / import_depends |
| 44 | 创建任务fromfinding.py — Finding →  / create_task_from_... | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1-04 / task... | 导入依赖 / import_depends |
| 45 | migrateto元数据tables.py — 裁定#209 S / migrate_to_metad... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 / depgraph_schema (g... | 导入依赖 / import_depends |
| 46 | 数据域设计态排查 - DB 现状查询（Phase 2，只读不写）。 / d... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 / depgraph_schema (g... | 导入依赖 / import_depends |
| 47 | 查询模块panorama.py — 模块全景查询入口（四图模块 / query... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 / depgraph_schema (g... | 导入依赖 / import_depends |
| 48 | 查询模块panorama.py — 模块全景查询入口（四图模块 / query... | → | D_GOVERNANCE 生命周期管理: dataflowgraph Schema DDL + 连接入口 / dataflowgraph_schem... | 导入依赖 / import_depends |
| 49 | 查询模块panorama.py — 模块全景查询入口（四图模块 / query... | → | D_GOVERNANCE 生命周期管理: decisiongraph Schema DDL + 不变量声明 / decisiongraph_sch... | 导入依赖 / import_depends |
| 50 | 将42项暂缓模块写入 depgraph 设计态，含3图对齐设计。 / reg... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 / depgraph_schema (g... | 导入依赖 / import_depends |
| 51 | 同步panoramamodule.py — 四图模块同步引擎（ARCH- / sync_p... | → | D_GOVERNANCE 生命周期管理: depgraph Schema DDL + 版本化迁移框架 / depgraph_schema (g... | 导入依赖 / import_depends |
| 52 | 同步panoramamodule.py — 四图模块同步引擎（ARCH- / sync_p... | → | D_GOVERNANCE 生命周期管理: dataflowgraph Schema DDL + 连接入口 / dataflowgraph_schem... | 导入依赖 / import_depends |
| 53 | 同步panoramamodule.py — 四图模块同步引擎（ARCH- / sync_p... | → | D_GOVERNANCE 生命周期管理: decisiongraph Schema DDL + 不变量声明 / decisiongraph_sch... | 导入依赖 / import_depends |
| 54 | adversarialsys主测试 / Red/Blue Team Adversarial Test v3:... | → | D_GOV_AUDIT 审计追踪: sys主合规 / SYS-MASTER-001 Compliance Checker (rule_enfor... | 导入依赖 / import_depends |
| 55 | scripts/governance/rebuild审计index.p / rebuild_audit_ind... | → | D_GOV_AUDIT 审计追踪: 索引重建结果——治本（裁定#18 G5）：对齐 testa / indexer ... | 导入依赖 / import_depends |
| 56 | 架构健康dashboard.py — 架构健康度 / architecture_health_... | → | D_GOV_AUDIT 审计追踪: 运行时违规snapshot.py — trae060 / runtime_violation_snap... | 导入依赖 / import_depends |
| 57 | 会话启动健康check.py — AI ses / session_startup_health_c... | → | D_GOV_AUDIT 审计追踪: reconciliation_registry.py — GitCommitGa / reconciliatio... | 导入依赖 / import_depends |
| 58 | 扫描消费者accuracy.py — CONSUMERS 字 / scan_consumers_ac... | → | D_GOV_CODE_QUALITY 代码质量治理: 差异helpers.py — gate 共享 diff 解析工具模块 / _diff_hel... | 导入依赖 / import_depends |
| 59 | 扫描消费者accuracy.py — CONSUMERS 字 / scan_consumers_ac... | → | D_GOV_CODE_QUALITY 代码质量治理: 消费者accuracygate.py — CONSUMERS 字 / consumers_accurac... | 导入依赖 / import_depends |
| 60 | 并发提交test.py — 幽灵提交红蓝对抗脚本（O / concurrent_c... | → | D_GOV_ENFORCEMENT 规则执行: GitCommitGateway — 全项目唯一合法 git commit 入口 / git_... | 导入依赖 / import_depends |
| 61 | Session 冷启动自检 — 运行 Phase 0 全部 14 个检查并输出状... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: PhaseManager->GateEngine 检查注册表桥梁 — 44 个阶 / phas... | 导入依赖 / import_depends |
| 62 | Session 冷启动自检 — 运行 Phase 0 全部 14 个检查并输出状... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Phase Manager — ZephyrAlpha 施工阶段门控引擎. / phase_ma... | 导入依赖 / import_depends |
| 63 | [INVARIANTS] 预算健康检查不可跳过;检查结果必须可机器解析 ... | → | D_GOV_REPAIR 治理修复: 延迟导入 BudgetEngine 避免循环依赖. / budget_enforcement ... | 导入依赖 / import_depends |
| 64 | CBG 熔断器重置 CLI (CircuitBreakerGateway Res / cbg_reset... | → | D_GOV_RULE 规则治理: 单向熔断器 / Circuit Breaker (rule_enforcement/circuit_br... | 导入依赖 / import_depends |
| 65 | CBG 熔断器重置 CLI (CircuitBreakerGateway Res / reset_cbg... | → | D_GOV_RULE 规则治理: 单向熔断器 / Circuit Breaker (rule_enforcement/circuit_br... | 导入依赖 / import_depends |
| 66 | 创建任务fromfinding.py — Finding →  / create_task_from_... | → | D_GOV_RULE 规则治理: 任务类型定义 / Task Types (rule_enforcement/task_types.py) | 导入依赖 / import_depends |
| 67 | 门禁引擎selfcheck / Gate Engine Bootstrap Self-Check — Q... | → | D_GOV_RULE 规则治理: 门禁裁决引擎 / Gate Engine (gate_engine/gate_engine.py) | 导入依赖 / import_depends |
| 68 | 校验门禁引擎external.py — Gate  / validate_gate_engine_e... | → | D_GOV_RULE 规则治理: 单向熔断器 / Circuit Breaker (rule_enforcement/circuit_br... | 导入依赖 / import_depends |
| 69 | 校验门禁引擎external.py — Gate  / validate_gate_engine_e... | → | D_GOV_RULE 规则治理: 门禁裁决引擎 / Gate Engine (gate_engine/gate_engine.py) | 导入依赖 / import_depends |
| 70 | 会话simulator — 30 个模拟开发 session 的蓝图 / session_s... | → | D_INFRA_RUNTIME 运行时集成: 蓝图metrics — 蓝图使用追踪 instrumentati / blueprint_met... | 导入依赖 / import_depends |
| 71 | base.py — 审计脚本基类 / base (_shared/base.py) | → | D_INFRA_RUNTIME 运行时集成: Finding Schema — 审计发现标准化数据模型 / finding (scrip... | 导入依赖 / import_depends |
| 72 | 检查注册表consistency — 跨登记表一致性校验。 / check_reg... | → | D_INFRA_RUNTIME 运行时集成: Finding Schema — 审计发现标准化数据模型 / finding (scrip... | 导入依赖 / import_depends |
| 73 | 发现状态machine.py — Finding 全生命周期 / finding_state_... | → | D_INFRA_RUNTIME 运行时集成: Finding Schema — 审计发现标准化数据模型 / finding (scrip... | 导入依赖 / import_depends |
| 74 | 校验紧急bypasslog.py — 应急绕过审 / validate_emergency_b... | → | D_INFRA_RUNTIME 运行时集成: Finding Schema — 审计发现标准化数据模型 / finding (scrip... | 导入依赖 / import_depends |
| 75 | run_all.py — 脚本系统统一入口脚本 / run_all (governance/... | → | D_INFRA_RUNTIME 运行时集成: Finding->TaskCard 桥接器 / finding_task_bridge (infrastru... | 导入依赖 / import_depends |
| 76 | run_all.py — 脚本系统统一入口脚本 / run_all (governance/... | → | D_INFRA_RUNTIME 运行时集成: Finding Schema — 审计发现标准化数据模型 / finding (scrip... | 导入依赖 / import_depends |
| 77 | VMS Cron 监控器 — MOD-INF-011 · TASK-INF-02 / vms_cron_... | → | D_INTEGRATION 管线路由: CollectionManager — MOD-INF-011 八大 Colle / collection_... | 导入依赖 / import_depends |
| 78 | VMS Cron 监控器 — MOD-INF-011 · TASK-INF-02 / vms_cron_... | → | D_INTEGRATION 管线路由: IndexHealthMonitor — MOD-INF-011 索引健康自检与 / index_... | 导入依赖 / import_depends |
| 79 | VMS Health Check 脚本 — MOD-INF-011 · Phas / vms_health... | → | D_INTEGRATION 管线路由: CollectionManager — MOD-INF-011 八大 Colle / collection_... | 导入依赖 / import_depends |
| 80 | VMS Health Check 脚本 — MOD-INF-011 · Phas / vms_health... | → | D_INTEGRATION 管线路由: IndexHealthMonitor — MOD-INF-011 索引健康自检与 / index_... | 导入依赖 / import_depends |
| 81 | VMS Phase 2 数据迁移脚本 — MOD-INF-011 / vms_migrate (vm... | → | D_INTEGRATION 管线路由: BridgeLayer — MOD-INF-011 kb/ ↔ VMS 过渡桥接 / bridge_l... | 导入依赖 / import_depends |
| 82 | VMS Phase 2 数据迁移脚本 — MOD-INF-011 / vms_migrate (vm... | → | D_INTEGRATION 管线路由: CollectionManager — MOD-INF-011 八大 Colle / collection_... | 导入依赖 / import_depends |
| 83 | VMS 迁移 dry-run 脚本 — MOD-INF-011 Phase 2  / vms_migra... | → | D_INTEGRATION 管线路由: BridgeLayer — MOD-INF-011 kb/ ↔ VMS 过渡桥接 / bridge_l... | 导入依赖 / import_depends |
| 84 | VMS Cron 监控器 — MOD-INF-011 · TASK-INF-02 / vms_cron_... | → | D_INTEGRATION 管线路由: CollectionManager — MOD-INF-011 八大 Colle / collection_... | 导入依赖 / import_depends |
| 85 | VMS Cron 监控器 — MOD-INF-011 · TASK-INF-02 / vms_cron_... | → | D_INTEGRATION 管线路由: IndexHealthMonitor — MOD-INF-011 索引健康自检与 / index_... | 导入依赖 / import_depends |
| 86 | VMS Health Check 脚本 — MOD-INF-011 · Phas / vms_health... | → | D_INTEGRATION 管线路由: CollectionManager — MOD-INF-011 八大 Colle / collection_... | 导入依赖 / import_depends |
| 87 | VMS Health Check 脚本 — MOD-INF-011 · Phas / vms_health... | → | D_INTEGRATION 管线路由: IndexHealthMonitor — MOD-INF-011 索引健康自检与 / index_... | 导入依赖 / import_depends |
| 88 | VMS Phase 2 数据迁移脚本 — MOD-INF-011 / vms_migrate (vm... | → | D_INTEGRATION 管线路由: BridgeLayer — MOD-INF-011 kb/ ↔ VMS 过渡桥接 / bridge_l... | 导入依赖 / import_depends |
| 89 | VMS Phase 2 数据迁移脚本 — MOD-INF-011 / vms_migrate (vm... | → | D_INTEGRATION 管线路由: CollectionManager — MOD-INF-011 八大 Colle / collection_... | 导入依赖 / import_depends |
| 90 | VMS 迁移 dry-run 脚本 — MOD-INF-011 Phase 2  / vms_migra... | → | D_INTEGRATION 管线路由: BridgeLayer — MOD-INF-011 kb/ ↔ VMS 过渡桥接 / bridge_l... | 导入依赖 / import_depends |
| 91 | 考试题库一致性检查——根因治本，防止"定义-注册脱钩"复发。... | → | D_INTELLIGENCE 上下文管理: ExamTestCases --- v3.0.5 扩展考试题库（96 题 / 2 / exam_t... | 导入依赖 / import_depends |
| 92 | 检查handoffmanifests.py — AI Session  / check_handoff_ma... | → | D_ORCHESTRATOR 代理编排器: 集成契约注册表（Contract Registry） / contract_registry (... | 导入依赖 / import_depends |
| 93 | AI写入前强制门禁钩子: lock协议检查+GateEngine Phase评估 /... | → | D_SECURITY 对抗验证: Session 级并发协调模块（P2-SES 落地）。 / session_concurr... | 导入依赖 / import_depends |
| 94 | DM-106: P2-B 迁移全量验证脚本 / dm106_p2b_verification (g... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 95 | 审计提交同步commands.py — post同步 / audit_post_sync_com... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 96 | DM-105: depgraph 未分配节点三策略处理脚本 / dm105_depgrap... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 97 | constants.py — 审计脚本共享常量 / constants (_shared/con... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 98 | shared/fileutils.py — 原子写入共享工具（ARCH-03 / file_u... | → | D_SHARED 共享服务: 文件utils.py —— 安全文件操作工具（Phase 3 新增 | 盲 / f... | 导入依赖 / import_depends |
| 99 | shared/yamlutils.py — YAML 文件加载共享工具 / yaml_utils... | → | D_SHARED 共享服务: yamlutils.py — vocabulary YAML 加载公共工具（S / yaml_ut... | 导入依赖 / import_depends |
| 100 | [INVARIANTS] pgadvisorylock 写锁; build / apply_decisiong... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 101 | [INVARIANTS] 原子写入（RULE-ONE）；变更前验证；禁止直接覆... | → | D_SHARED 共享服务: 仅在 dev 环境下为 True——生产环境永远 False / env (found... | 导入依赖 / import_depends |
| 102 | [INVARIANTS] 原子写入（RULE-ONE）；变更前验证；禁止直接覆... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 103 | [INVARIANTS] 原子写入（RULE-ONE）；变更前验证；禁止直接覆... | → | D_SHARED 共享服务: yamlutils.py — vocabulary YAML 加载公共工具（S / yaml_ut... | 导入依赖 / import_depends |
| 104 | GATE-SSOT: SSoT 创建门禁（pre-commit hook 双保险 / check_... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 105 | GATE-SSOT-SINGLESOURCE: SSoT 单一真源门禁（Phas / check_m... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 106 | diagnose依赖图 / # [BLUEPRINT] MOD-INF-005 | scripts/gove... | → | D_SHARED 共享服务: yamlutils.py — vocabulary YAML 加载公共工具（S / yaml_ut... | 导入依赖 / import_depends |
| 107 | G-panorama-align: 四图对齐检测器（ARCH-053 + ARC / align_... | → | D_SHARED 共享服务: converters.py — 类型转换工具（消除 '' vs None 语义鸿 / c... | 导入依赖 / import_depends |
| 108 | G13: 从 depgraph (PostgreSQL) 生成资产清单全景图 / genera... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 109 | Code Wiki 统计数据生成器（半自动维护机制）。 / generate_c... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 110 | G12: 从 depgraph (PostgreSQL) 生成契约目录全景图 / genera... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 111 | 生成契约 / generate_contracts.py -- SSoT to Codegen pipel... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 112 | G-panorama-registry: 自动生成全景图清单总表 / generate_pa... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 113 | 校验模块lifecycle.py — 模块生命周期校验 / validate_modul... | → | D_SHARED 共享服务: yamlutils.py — vocabulary YAML 加载公共工具（S / yaml_ut... | 导入依赖 / import_depends |
| 114 | 校验接口contracts.py — 接口契约校验 / validate_interface... | → | D_SHARED 共享服务: yamlutils.py — vocabulary YAML 加载公共工具（S / yaml_ut... | 导入依赖 / import_depends |
| 115 | 提取decisiongraph / extract_decisiongraph - decisiongraph... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 116 | [INVARIANTS] 禁止AI直接Read 157MB depgraph文件 / extract_... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 117 | [INVARIANTS] YAML 是唯一真源; DB 为只读缓存; 同步单向  / ... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 118 | 生成project依赖图 / # [BLUEPRINT] MOD-INF-005 | scripts/g... | → | D_SHARED 共享服务: 进程池 / process_pool.py - Shared process pool for MCP se... | 导入依赖 / import_depends |
| 119 | 生成project依赖图 / # [BLUEPRINT] MOD-INF-005 | scripts/g... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 120 | 生成project依赖图 / # [BLUEPRINT] MOD-INF-005 | scripts/g... | → | D_SHARED 共享服务: yamlutils.py — vocabulary YAML 加载公共工具（S / yaml_ut... | 导入依赖 / import_depends |
| 121 | 检查门禁inventorydrift.py — commitg / check_gate_invento... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 122 | concurrency / Module docstring — see module-level docstr... | → | D_SHARED 共享服务: 进程池 / process_pool.py - Shared process pool for MCP se... | 导入依赖 / import_depends |
| 123 | 创建任务fromfinding.py — Finding →  / create_task_from_... | → | D_SHARED 共享服务: ZephyrAlpha 任务系统核心数据模型 / models (foundation/mod... | 导入依赖 / import_depends |
| 124 | 创建任务fromfinding.py — Finding →  / create_task_from_... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 125 | SQLite → PostgreSQL 运营数据迁移脚本 / migrate_data (mig... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 126 | 并发提交test.py — 幽灵提交红蓝对抗脚本（O / concurrent_c... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 127 | 同步panoramamodule.py — 四图模块同步引擎（ARCH- / sync_p... | → | D_SHARED 共享服务: converters.py — 类型转换工具（消除 '' vs None 语义鸿 / c... | 导入依赖 / import_depends |

### 依赖本域的其他域（入边）/ Depended By

| # | 外部域-源模块 / Source Module | → | 本域模块 / Target Module | 依赖类型 / Type |
|:--:|---------|:--:|---------|---------|
| 1 | D_AUTONOMY_PERM 自治保护: 检查终止开关latency.py — Kill Swit / check_kill_switch_l... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 2 | D_AUTONOMY_PERM 自治保护: 管理终止switch.py — Kill Switch 管理工具 / manage_kill_s... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 3 | D_AUTONOMY_PERM 自治保护: 管理终止switch.py — Kill Switch 管理工具 / manage_kill_s... | → | shared/fileutils.py — 原子写入共享工具（ARCH-03 / file_u... | 导入依赖 / import_depends |
| 4 | D_GOVERNANCE 生命周期管理: 架构guard 共享：仓库根路径、capacityslo / invar / _arch_s... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 5 | D_GOVERNANCE 生命周期管理: 从 cross层contracts.yaml 生成 OCP 冻结契约 / build_ocp_ma... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 6 | D_GOVERNANCE 生命周期管理: 为所有 P0/P1 契约添加 idempotencykey 字段——状态感知版 /... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 7 | D_GOVERNANCE 生命周期管理: 一次性工具——为 9 个 P1 契约补齐 physicalpath 并运行 c /... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 8 | D_GOVERNANCE 生命周期管理: 检查aclboundary.py — Broker ACL 边界强制执 / check_acl_b... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 9 | D_GOVERNANCE 生命周期管理: 检查跨planecommunication.py — INV / check_cross_plane_co... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 10 | D_GOVERNANCE 生命周期管理: 检查feaclboundary.py — INV-006 前端 AC / check_fe_acl_bo... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 11 | D_GOVERNANCE 生命周期管理: 检查hot路径purity.py — INV-012 Hot 路 / check_hot_path_p... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 12 | D_GOVERNANCE 生命周期管理: 检查scaffold出口gates.py — scaffold→ / check_scaffold_e... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 13 | D_GOVERNANCE 生命周期管理: 检查scaffold出口gates.py — scaffold→ / check_scaffold_e... | → | shared/yamlutils.py — YAML 文件加载共享工具 / yaml_utils... | 导入依赖 / import_depends |
| 14 | D_GOVERNANCE 生命周期管理: 检查结构consistency.py — INV-010 契约 / check_schema_con... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 15 | D_GOVERNANCE 生命周期管理: 检查aisggateway.py — AISG 拦截门禁 (INV-0 / check_aisg_g... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 16 | D_GOVERNANCE 生命周期管理: 检查审计日志immutability.py — 审计日志不可 / check_audit... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 17 | D_GOVERNANCE 生命周期管理: 检查daily亏损limit.py — 日损失限额自动暂停 (I / check_da... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 18 | D_GOVERNANCE 生命周期管理: 检查hotwarmipc.py — INV-018 Hot↔Warm / check_hot_warm_i... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 19 | D_GOVERNANCE 生命周期管理: 检查idempotencykey.py — 幂等 Key 字段存在性检 / check_id... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 20 | D_GOVERNANCE 生命周期管理: 检查日志密钥leak.py — R2 日志不写 secre / check_log_secr... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 21 | D_GOVERNANCE 生命周期管理: 检查no跨plane可变state.py —  / check_no_cross_plane_muta... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 22 | D_GOVERNANCE 生命周期管理: 检查ocpsignatures.py — OCP 冻结契约指纹校验 ( / check_oc... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 23 | D_GOVERNANCE 生命周期管理: 检查pitcompliance.py — PIT（Point-in-T / check_pit_compl... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 24 | D_GOVERNANCE 生命周期管理: 检查持仓limit.py — 单一持仓限制 ≤ 5% NA / check_positio... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 25 | D_GOVERNANCE 生命周期管理: 检查风险paramsconsistency.py — 风控参数真 / check_risk_p... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 26 | D_GOVERNANCE 生命周期管理: 检查warm冷async.py — INV-019 Warm→ / check_warm_cold_as... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 27 | D_GOVERNANCE 生命周期管理: 重置测试任务 / reset_test_task (construction/reset_test_t... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 28 | D_GOVERNANCE 生命周期管理: 启动brain.py — ZephyrAlpha 系统大脑一键启动 / start_brai... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 29 | D_GOVERNANCE 生命周期管理: dm90971新增测试headers / DM-90971: Batch add module_id sc... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 30 | D_GOVERNANCE 生命周期管理: 修复orphanall.py — 自动修复 初始化.py a / fix_orphan_all... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 31 | D_GOVERNANCE 生命周期管理: 修复orphanall.py — 自动修复 初始化.py a / fix_orphan_all... | → | shared/fileutils.py — 原子写入共享工具（ARCH-03 / file_u... | 导入依赖 / import_depends |
| 32 | D_GOVERNANCE 生命周期管理: 从所有 MOD 蓝图的 §路径索引 章节自动生成 system-pathway-... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 33 | D_GOVERNANCE 生命周期管理: 检查pureshim.py — GATE-NO-PURE-SHIM 检 / check_pure_shim... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 34 | D_GOVERNANCE 生命周期管理: 检查pureshim.py — GATE-NO-PURE-SHIM 检 / check_pure_shim... | → | encoding.py — UTF-8 编码安全工具 / encoding (_shared/enc... | 导入依赖 / import_depends |
| 35 | D_GOVERNANCE 生命周期管理: 生成规则AIperceptionindex.py — 规 / generate_rule_ai_per... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 36 | D_GOVERNANCE 生命周期管理: 执行 git 命令并返回 stdout（UTF-8 解码）。 / auto_handoff... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 37 | D_GOVERNANCE 生命周期管理: 从 config/mcp.json 生成各 IDE MCP 配置文件（MOD-I / gener... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 38 | D_GOVERNANCE 生命周期管理: MCP DAG 编排启动器（MOD-INF-013 §14 拓扑排序 + Pro / lau... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 39 | D_GOVERNANCE 生命周期管理: MCP 全 Server 启动脚本 — DEPRECATED. / start_all (mcp/st... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 40 | D_GOVERNANCE 生命周期管理: MCP 全 Server 停止脚本（MOD-INF-013 §14）。 / stop_all (... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 41 | D_GOVERNANCE 生命周期管理: DM-311: autonomycore/ 拆分迁移执行脚本。 / dm311_autonomy... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 42 | D_GOVERNANCE 生命周期管理: DM-314: infraops/ 拆分迁移执行脚本。 / dm314_infra_ops_sp... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
...
| 44 | D_GOVERNANCE 生命周期管理: 预commit 验证脚本 — 委托给 code-dedup-engine  / verify_d... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 45 | D_GOVERNANCE 生命周期管理: scaffold.py — ZephyrAlpha 唯一创建入口（RULE-TW / scaffo... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 46 | D_GOVERNANCE 生命周期管理: scaffold.py — ZephyrAlpha 唯一创建入口（RULE-TW / scaffo... | → | shared/yamlutils.py — YAML 文件加载共享工具 / yaml_utils... | 导入依赖 / import_depends |
...
| 48 | D_GOVERNANCE 生命周期管理: 测试生成门禁registry.py — generat / test_generate_gate_r... | → | 生成门禁registry.py — 门禁登记表自动生成器 / generate_ga... | 测试依赖 / test_depends |
| 49 | D_GOV_AUDIT 审计追踪: [INVARIANTS] 按path精确匹配+按功能名模糊匹配; 输出差距报... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 50 | D_GOV_AUDIT 审计追踪: [INVARIANTS] 20项红蓝对抗测试 / red_blue_test (repair/red... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 51 | D_GOV_AUDIT 审计追踪: [INVARIANTS] 仅接受depgraph.backup.*路径; 回滚前 / rollba... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 52 | D_GOV_AUDIT 审计追踪: 测试remediationprogresssmoke.py — Pha / test_remediation... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 53 | D_GOV_AUDIT 审计追踪: reconciliation_registry.py — GitCommitGa / reconciliatio... | → | 模块id / domainid / submoduleid 格式校 / validate_module_... | 导入依赖 / import_depends |
| 54 | D_GOV_AUDIT 审计追踪: reconciliation_registry.py — GitCommitGa / reconciliatio... | → | 检查门禁inventorydrift.py — commitg / check_gate_invento... | 导入依赖 / import_depends |
| 55 | D_GOV_CODE_QUALITY 代码质量治理: 检查模块idconsistency.py — module / check_module_id_cons... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 56 | D_GOV_DOCS 架构文档治理: 测试guc触发器fix.py — GUC 触发器缺陷修复的端到 / test_gu... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 测试依赖 / test_depends |
| 57 | D_GOV_DOCS 架构文档治理: 测试同步savepointisolation.py — sync / test_sync_savepoi... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 测试依赖 / test_depends |
| 58 | D_GOV_DRIFT 漂移检测: 校验蓝图overlap / Module docstring — see module-level do... | → | 文件头部格式解析 SSoT（Single Source of Truth） / frontma... | 导入依赖 / import_depends |
| 59 | D_GOV_DRIFT 漂移检测: 校验truth源cascade.py — 真源级联一 / validate_truth_sour... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 60 | D_GOV_DRIFT 漂移检测: 校验truth源cascade.py — 真源级联一 / validate_truth_sour... | → | 文件头部格式解析 SSoT（Single Source of Truth） / frontma... | 导入依赖 / import_depends |
| 61 | D_GOV_DRIFT 漂移检测: SSoT 文件头一致性校验器. / validate_ssot (validators/vali... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 62 | D_GOV_DRIFT 漂移检测: SSoT 文件头一致性校验器. / validate_ssot (validators/vali... | → | encoding.py — UTF-8 编码安全工具 / encoding (_shared/enc... | 导入依赖 / import_depends |
| 63 | D_GOV_DRIFT 漂移检测: SSoT 文件头一致性校验器. / validate_ssot (validators/vali... | → | 文件头部格式解析 SSoT（Single Source of Truth） / frontma... | 导入依赖 / import_depends |
| 64 | D_GOV_DRIFT 漂移检测: SSoT 文件头一致性校验器. / validate_ssot (validators/vali... | → | shared/yamlutils.py — YAML 文件加载共享工具 / yaml_utils... | 导入依赖 / import_depends |
| 65 | D_GOV_ENFORCEMENT 规则执行: 指标数量漂移reconciler.py — dashb / metric_count_drift_r... | → | 架构健康dashboard.py — 架构健康度 / architecture_health_... | 导入依赖 / import_depends |
| 66 | D_GOV_ENFORCEMENT 规则执行: 会话worktreecli.py — session worktr / session_worktree_c... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 67 | D_GOV_RULE 规则治理: 脚本清单自动生成器 / Script Manifest Generator (generator... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
...
| 69 | D_GOV_RULE 规则治理: 脚本清单自动生成器 / Script Manifest Generator (generator... | → | shared/fileutils.py — 原子写入共享工具（ARCH-03 / file_u... | 导入依赖 / import_depends |
| 70 | D_GOV_RULE 规则治理: 脚本清单自动生成器 / Script Manifest Generator (generator... | → | shared/yamlutils.py — YAML 文件加载共享工具 / yaml_utils... | 导入依赖 / import_depends |
| 71 | D_OPS 反馈循环: 门禁缓存 / Module docstring — see module-level docstring... | → | constants.py — 审计脚本共享常量 / constants (_shared/con... | 导入依赖 / import_depends |
| 72 | D_OPS 反馈循环: 门禁缓存 / Module docstring — see module-level docstring... | → | shared/fileutils.py — 原子写入共享工具（ARCH-03 / file_u... | 导入依赖 / import_depends |
