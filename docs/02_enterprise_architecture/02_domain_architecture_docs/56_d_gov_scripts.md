...
| 模块数 | 412 | Module Count | 412 |
| 域内依赖 | 774 | Internal Dependencies | 774 |
| 跨域入边 | 85 | Cross-domain Incoming | 85 |
| 跨域出边 | 146 | Cross-domain Outgoing | 146 |
| 设计态模块 | 0 | Design Modules | 0 |
| 生产态模块 | 412 | Production Modules | 412 |
| 容量 | 412/150 (超容) | Capacity | 412/150 (超容) |
...
> 展示全部 412 个模块（生产态 412 + 设计态 0），含跨域依赖外部节点。节点含成熟度+名称+大白话/简介+文件路径。
...
flowchart TD
    ...
    tests_governance_scripts_governance_test_staged_walk_py["scripts_governance/test_staged_walk<br/>test_staged_walk.py — iter_staged_files()<br/>共享函数 + 5 扫描器 --staged 模式...<br/>文件: scripts_governance/test_staged_walk.py<br/>(生产态 / production)"]
    ...
    tests_governance_scripts_governance_test_staged_walk_py -->|测试依赖 / test_depends| scripts_governance_shared_walk_py
    ...
    class D_GOVERNANCE,D_DATA,D_SHARED,D_GOV_ENFORCEMENT,D_INFRA_RUNTIME,D_GOV_AUDIT external_prod
```

### 运营态的图（仅 design_maturity=production 的模块和域内依赖）

> 仅展示已上线运行的模块（共 412 个），不含跨域外部节点。跨域依赖见下方跨域依赖章节。
...
> 本域与 18 个外部域直接连接（出边 146 条 + 入边 85 条 = 231 条）。只显示直接连接的域，不展开具体节点。
