...
graph TD
    ...
    class D_SECURITY,D_INFRA_A2A,D_GOV_OPS_RESILIENCE,D_INFRA_RUNTIME,D_INTEGRATION,D_ORCHESTRATOR,D_SHARED external_prod
...
graph TD
    ...
    class D_SECURITY,D_INTEGRATION,D_ORCHESTRATOR,D_INFRA_RUNTIME,D_GOV_OPS_RESILIENCE,D_INFRA_A2A,D_SHARED external_prod
    class D_GOVERNANCE external_design
```
...
> 本域与 12 个外部域直接连接（出边 103 条 + 入边 8 条 = 111 条）。只显示直接连接的域，不展开具体节点。
