...
graph TD
    ...
    D_INFRA_RUNTIME -.->|runtime / runtime| src_zephyr_security_llm_defense_llm_security_dashboard_init_py
    ...
    class D_SECURITY,D_INFRA_RUNTIME,D_AUTONOMY_CORE,D_GOVERNANCE,D_GOV_DOCS external_design
```
...
graph TD
    ...
    tests_llm_security_test_l8_multi_agent_py -.->|测试依赖 / test_depends| D_SECURITY
    tests_llm_security_test_fail_closed_py -.->|测试依赖 / test_depends| D_SECURITY
    D_SHARED["(生产态 / production) D_SHARED"]
    tests_llm_security_test_security_secrets_py -.->|测试依赖 / test_depends| D_SHARED
    D_INTEGRATION["(生产态 / production) D_INTEGRATION"]
    tests_llm_security_test_db_py -.->|测试依赖 / test_depends| D_INTEGRATION
    src_zephyr_security_llm_defense_llm_security_init_py -.->|导入依赖 / import_depends| D_SECURITY
    src_zephyr_security_llm_defense_llm_security_dashboard_init_py -.->|config_depends / config_depends| D_SECURITY
    ...
    class D_INFRA_RUNTIME,D_FEEDBACK_LOOP,D_SECURITY,D_SHARED,D_INTEGRATION external_prod
...
| 3 | D_GOVERNANCE 生命周期管理: model_provider_data.py | → | LLM Security Gateway Dashboard Module. (__init_... | contract / contract |
| 4 | D_GOV_DOCS 架构文档治理: blueprint.md | → | LLM Security Gateway Dashboard Module. (__init_... | contract / contract |
| 5 | D_INFRA_RUNTIME 运行时集成: 遥测 · traces/span_stub — W3C TraceContext 分... | → | LLM Security Gateway Dashboard Module. (__init_... | runtime / runtime |
