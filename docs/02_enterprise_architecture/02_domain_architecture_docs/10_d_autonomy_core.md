...
flowchart TD
    ...
    src_zephyr_autonomy_core_agent_observability_py["代理可观测性<br/>Agent Trace 全链路可观测性<br/>文件: autonomy_core/agent_observability.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_context_atomic_injector_py["atomicinjector.py — 原子注入 (DD101, TASK-0<br/>自主核心的原子注入，原子注入相关功能的实现。<br/>atomic_injector<br/>文件: context/atomic_injector.py<br/>(生产态 / production)"]
    src_zephyr_autonomy_core_context_ce_bootstrap_py["ce自举<br/>三级递进建造序列: CE-MVP -> Functional -> FullCE<br/>ce_bootstrap<br/>文件: context/ce_bootstrap.py<br/>(生产态 / production)"]
    src_zephyr_autonomy_core_context_ce_explain_cli_py["ceexplain命令行<br/>自主核心的KE inclusion rationale 解释 CLI，KE<br/>inclusion rationale 解释 CLI相关功能的实现。<br/>ce_explain_cli<br/>文件: context/ce_explain_cli.py<br/>(生产态 / production)"]
    src_zephyr_autonomy_core_context_ce_file_lister_py["ce文件lister<br/>自主核心的CE 文件清单生成器，CE<br/>文件清单生成器相关功能的实现。<br/>ce_file_lister<br/>文件: context/ce_file_lister.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_context_ce_vibe_shortcuts_py["cevibeshortcuts.py — Vibe/Strict 模式切换<br/>自主核心的Vibe/Strict 模式切换，Vibe/Strict<br/>模式切换相关功能的实现。<br/>ce_vibe_shortcuts<br/>文件: context/ce_vibe_shortcuts.py<br/>(生产态 / production)"]
    src_zephyr_autonomy_core_context_checkpoint_manager_py["检查点管理器<br/>Inject 前 snapshot; 回滚到注入前<br/>checkpoint_manager<br/>文件: context/checkpoint_manager.py<br/>(生产态 / production)"]
    src_zephyr_autonomy_core_context_cold_start_booster_py["冷启动booster<br/>build 发现 KE count < min_count -> 自动种子 KE<br/>cold_start_booster<br/>文件: context/cold_start_booster.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_context_context_budget_py["上下文预算<br/>超预算截断策略<br/>TruncationStrategy — TruncationStrategy<br/>文件: context/context_budget.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_context_context_health_score_py["上下文健康评分<br/>自主核心的统一健康分 (B6, DD80, TASK-015 beta<br/>v)，统一健康分 (B6, DD80, TASK-015 beta<br/>v)相关功能的实现。<br/>context_health_score<br/>文件: context/context_health_score.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_context_context_outcome_tracker_py["上下文结果追踪器<br/>ContextBlock -> Agent Action -> Action Success<br/>三级因果关联<br/>context_outcome_tracker<br/>文件: context/context_outcome_tracker.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_context_context_playground_py["上下文playground<br/>自主核心的上下文沙箱 dry-run (B5, DD79,<br/>TASK-015 beta v)，上下文沙箱 dry-run (B5, DD79,<br/>TASK-015 beta v)相关功能的实现。<br/>context_playground<br/>文件: context/context_playground.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_context_context_value_attribution_py["上下文valueattribution<br/>自主核心的KE 级 ROI 归因 (B2, DD76, TASK-015<br/>beta v)，KE 级 ROI 归因 (B2, DD76, TASK-015<br/>beta v)相关功能的实现。<br/>context_value_attribution<br/>文件: context/context_value_attribution.py<br/>(生产态 / production)"]
    src_zephyr_autonomy_core_context_contextual_fetch_api_py["contextual获取API<br/>自主核心的HTTP FE 对外 API，HTTP FE 对外<br/>API相关功能的实现。<br/>contextual_fetch_api<br/>文件: context/contextual_fetch_api.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_context_diff_injector_py["差异injector<br/>Continuous session: 设定 prefix_size=1000,<br/>后续注入 diff<br/>diff_injector<br/>文件: context/diff_injector.py<br/>(生产态 / production)"]
    src_zephyr_autonomy_core_context_diversity_constraint_py["diversity约束<br/>自主核心的多样性约束，多样性约束相关功能的实现。<br/>diversity_constraint<br/>文件: context/diversity_constraint.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_context_mode_manager_py["mode管理器<br/>mode_manager.py — 模式管理器 (DD102, TASK-019)<br/>文件: context/mode_manager.py<br/>(生产态 / production)"]
    src_zephyr_autonomy_core_context_position_optimizer_py["持仓优化器<br/>自主核心的位置优化，位置优化相关功能的实现。<br/>position_optimizer<br/>文件: context/position_optimizer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_integration_pipeline_bridge_py["管线桥接<br/>1. 接收 TaskCard -> 通过 TriggerRouter 匹配<br/>Domain Skill + Role Skill<br/>pipeline_bridge<br/>文件: integration/pipeline_bridge.py<br/>(生产态 / production)"]
    src_zephyr_autonomy_core_phase_planner_py["阶段规划器<br/>自主核心的阶段planner。MOD-INF-019: Agent Spec<br/>— Phase Planner，阶段planner。MOD-INF-019:<br/>Agent Spec — Phase Planner相关功能的实现。<br/>文件: autonomy_core/phase_planner.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_skill_rbac_registry_py["skillRBAC注册表<br/>自主核心的技能rbac注册表。G-CT-003: Agent Spec<br/>-> RBAC capability<br/>check.，技能rbac注册表。G-CT-003: Agent Spec -><br/>RBAC capability check.相关功能的实现。<br/>文件: autonomy_core/skill_rbac_registry.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_skills_skill_canary_py["技能金丝雀<br/>自主核心的技能canary。MOD-INF-019: Agent Spec —<br/>Skill Canary，技能canary。MOD-INF-019: Agent<br/>Spec — Skill Canary相关功能的实现。<br/>文件: skills/skill_canary.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_skills_skill_cross_model_py["技能跨模型<br/>自主核心的技能跨模型。MOD-INF-019: Agent Spec —<br/>Skill Cross-Model，技能跨模型。MOD-INF-019:<br/>Agent Spec — Skill Cross-Model相关功能的实现。<br/>文件: skills/skill_cross_model.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_skills_skill_durable_py["技能durable<br/>确保 Skill 执行不丢失:<br/>文件: skills/skill_durable.py<br/>(生产态 / production)"]
    src_zephyr_autonomy_core_skills_skill_economics_py["技能economics<br/>自主核心的技能economics。MOD-INF-019: Agent<br/>Spec — Skill<br/>Economics，技能economics。MOD-INF-019: Agent<br/>Spec — Skill Economics相关功能的实现。<br/>文件: skills/skill_economics.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_skills_skill_explain_py["技能explain<br/>让 Skill 决策不再黑箱:<br/>文件: skills/skill_explain.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_skills_skill_freshness_ext_py["技能freshness扩展<br/>自主核心的技能freshness扩展。MOD-INF-019: Agent<br/>Spec — Skill Freshness<br/>Extensions，技能freshness扩展。MOD-INF-019:<br/>Agent Spec — Skill Freshness<br/>Extensions相关功能的实现。<br/>文件: skills/skill_freshness_ext.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_skills_skill_guardrails_py["技能guardrails<br/>自主核心的技能guardrails。MOD-INF-019: Agent<br/>Spec — Skill<br/>Guardrails，技能guardrails。MOD-INF-019: Agent<br/>Spec — Skill Guardrails相关功能的实现。<br/>文件: skills/skill_guardrails.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_skills_skill_kya_py["技能kya<br/>公共接口：assess（Stage 4 公共化）<br/>MOD-INF-019: Agent Spec — Skill KYA<br/>文件: skills/skill_kya.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_skills_skill_risk_mitigator_py["skill风险mitigator<br/>R1-R65 风险缓解引擎<br/>文件: skills/skill_risk_mitigator.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_skills_skill_security_py["技能安全<br/>Skill 安全审计与防护<br/>MOD-INF-019: Agent Spec — Skill Security<br/>文件: skills/skill_security.py<br/>(生产态 / production)"]
    src_zephyr_autonomy_core_skills_skill_shadow_py["技能影子<br/>安全发布 Skill 更新:<br/>文件: skills/skill_shadow.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_skills_skill_team_optimizer_py["技能团队优化器<br/>Skill 团队组合优化器<br/>文件: skills/skill_team_optimizer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_skills_skill_tokenomics_py["技能tokenomics<br/>自主核心的技能tokenomics。MOD-INF-019: Agent<br/>Spec — Skill<br/>Tokenomics，技能tokenomics。MOD-INF-019: Agent<br/>Spec — Skill Tokenomics相关功能的实现。<br/>文件: skills/skill_tokenomics.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_spec_engine_py["spec引擎<br/>SpecEngine 蓝图->Skill 升级引擎<br/>spec_engine<br/>文件: autonomy_core/spec_engine.py<br/>(生产态 / production)"]
    ...
    src_zephyr_infrastructure_system_telemetry_otel_instrumentation_py["otelinstrumentation.py — 全链路 OTel (B12,<br/>基础设施的全链路 OTel (B12, DD86, TASK-015 beta<br/>v)，全链路 OTel (B12, DD86, TASK-015 beta<br/>v)相关功能的实现。<br/>otel_instrumentation<br/>文件: system_telemetry/otel_instrumentation.py<br/>(生产态 / production)"]
    ...
    src_zephyr_security_llm_defense_llm_security_adversarial_robustness_py["对抗鲁棒性<br/>Fuzz + 语义对抗样本 + 3 轮 penTest<br/>adversarial_robustness<br/>文件: llm_security/adversarial_robustness.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_dependency_dependency_tracker_py["依赖追踪器<br/>共享层的依赖追踪，依赖追踪相关功能的实现。<br/>dependency_tracker<br/>文件: dependency/dependency_tracker.py<br/>(生产态 / production)"]
    ...
    src_zephyr_autonomy_core_skills_skill_freshness_py["技能freshness<br/>公共接口：save（Stage 4 公共化）<br/>文件: skills/skill_freshness.py<br/>(生产态 / production)"]
