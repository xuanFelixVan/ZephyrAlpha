...
flowchart TD
    ...
    class D_SHARED,D_INFRA_RUNTIME,D_GOV_AUDIT,D_INTEGRATION,D_FEEDBACK_LOOP,D_GOV_CODE_QUALITY,D_ORCHESTRATOR,D_GOVERNANCE,D_SECURITY external_prod
```
...
> 本域与 11 个外部域直接连接（出边 59 条 + 入边 19 条 = 78 条）。只显示直接连接的域，不展开具体节点。
