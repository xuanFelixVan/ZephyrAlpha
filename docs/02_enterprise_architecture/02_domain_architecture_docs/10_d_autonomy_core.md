...
flowchart TD
    ...
    src_zephyr_autonomy_core_agent_observability_py["(生产态 / production) 代理可观测性 /<br/>MOD-INF-019: Agent Spec — Agent<br/>Observability<br/>代理observability。MOD-INF-019: Agent<br/>Spec — Agent Observability<br/>文件: autonomy_core/agent_<br/>observability.py"]
    src_zephyr_autonomy_core_all_skill_modules_py["(生产态 / production) all技能modules /<br/>MOD-INF-019: Agent Spec — All Skill<br/>Modules<br/>all技能modules。MOD-INF-019: Agent Spec<br/>— All Skill Modules<br/>文件: autonomy_core/all_skill_modules.py"]
    src_zephyr_autonomy_core_context_atomic_injector_py["(生产态 / production) atomicinjector.py<br/>— 原子注入 (DD101, TASK-0 / atomic_<br/>injector<br/>原子注入<br/>文件: context/atomic_injector.py"]
    src_zephyr_autonomy_core_context_ce_bootstrap_py["(生产态 / production) ce自举 / ce_<br/>bootstrap<br/>CE 自举架构 (B1, DD75, TASK-015 beta v)<br/>文件: context/ce_bootstrap.py"]
    src_zephyr_autonomy_core_context_ce_explain_cli_py["(生产态 / production) ceexplain命令行 /<br/>ce_explain_cli<br/>KE inclusion rationale 解释 CLI<br/>文件: context/ce_explain_cli.py"]
    src_zephyr_autonomy_core_context_ce_file_lister_py["(生产态 / production) ce文件lister / ce_<br/>file_lister<br/>CE 文件清单生成器<br/>文件: context/ce_file_lister.py"]
    src_zephyr_autonomy_core_context_ce_playground_v2_py["(生产态 / production) CE演练场v2 / ce_<br/>playground_v2.py — V2 Playground with<br/>full decision chain<br/>CE演练场v2，context的结果，封装操作结果<br/>的数据结构。<br/>文件: context/ce_playground_v2.py"]
    src_zephyr_autonomy_core_context_ce_vibe_shortcuts_py["(生产态 / production)<br/>cevibeshortcuts.py — Vibe/Strict<br/>模式切换  / ce_vibe_shortcuts<br/>Vibe/Strict 模式切换<br/>文件: context/ce_vibe_shortcuts.py"]
    src_zephyr_autonomy_core_context_checkpoint_manager_py["(生产态 / production) 检查点管理器 /<br/>checkpoint_manager<br/>Inject 前快照<br/>文件: context/checkpoint_manager.py"]
    src_zephyr_autonomy_core_context_cold_start_booster_py["(生产态 / production) 冷启动booster /<br/>cold_start_booster<br/>冷启动<br/>文件: context/cold_start_booster.py"]
    ...
    src_zephyr_autonomy_core_context_context_budget_tracker_py["(生产态 / production) 上下文预算追踪器<br/>/ ContextBudgetTracker: token budget<br/>management with 3-level t<br/>上下文预算追踪器。ContextBudgetTracker:<br/>token budget management with 3-level<br/>thresholds.<br/>文件: context/context_budget_tracker.py"]
    src_zephyr_autonomy_core_context_context_debt_score_py["(生产态 / production) 上下文debtscore /<br/>context_debt_score<br/>上下文债务评分<br/>文件: context/context_debt_score.py"]
    src_zephyr_autonomy_core_context_context_evaluator_py["(生产态 / production) 上下文evaluator /<br/>context_evaluator<br/>AI 引用率评估 (TASK-014 beta b)<br/>文件: context/context_evaluator.py"]
    src_zephyr_autonomy_core_context_context_evictor_py["(生产态 / production) 上下文驱逐器 /<br/>context_evictor<br/>三维逐出器 (DD9, TASK-014 beta a)<br/>文件: context/context_evictor.py"]
    src_zephyr_autonomy_core_context_context_health_score_py["(生产态 / production) 上下文健康评分 /<br/>context_health_score<br/>统一健康分 (B6, DD80, TASK-015 beta v)<br/>文件: context/context_health_score.py"]
    src_zephyr_autonomy_core_context_context_model_strategy_py["(生产态 / production) 上下文模型策略 /<br/>context_model_strategy<br/>模型选择策略<br/>文件: context/context_model_strategy.py"]
    ...
    src_zephyr_autonomy_core_context_context_pipeline_auto_py["(生产态 / production) 上下文管线自动 /<br/>context_pipeline_auto<br/>ContextPipeline 三层自动化机制<br/>文件: context/context_pipeline_auto.py"]
    src_zephyr_autonomy_core_context_context_playground_py["(生产态 / production) 上下文playground<br/>/ context_playground<br/>上下文沙箱 dry-run (B5, DD79, TASK-015<br/>beta v)<br/>文件: context/context_playground.py"]
    src_zephyr_autonomy_core_context_context_rot_model_py["(生产态 / production) 上下文rot模型 /<br/>context_rot_model<br/>Context Rot 注意力衰减数学模型<br/>文件: context/context_rot_model.py"]
    src_zephyr_autonomy_core_context_context_value_attribution_py["(生产态 / production)<br/>上下文valueattribution / context_value_<br/>attribution<br/>KE 级 ROI 归因 (B2, DD76, TASK-015 beta<br/>v)<br/>文件: context/context_value_<br/>attribution.py"]
    ...
    src_zephyr_autonomy_core_context_curation_loop_py["(生产态 / production) curation循环 /<br/>curation_loop<br/>Per-Turn Curation 策展 (DD10, TASK-014<br/>beta b)<br/>文件: context/curation_loop.py"]
    src_zephyr_autonomy_core_context_diff_injector_py["(生产态 / production) 差异injector /<br/>diff_injector<br/>增量注入<br/>文件: context/diff_injector.py"]
    src_zephyr_autonomy_core_context_diversity_constraint_py["(生产态 / production) diversity约束 /<br/>diversity_constraint<br/>多样性约束<br/>文件: context/diversity_constraint.py"]
    src_zephyr_autonomy_core_context_domain_decay_config_py["(生产态 / production) domaindecay配置 /<br/>domain_decay_config<br/>每领域半衰期<br/>文件: context/domain_decay_config.py"]
    ...
    src_zephyr_autonomy_core_context_integrity_check_py["(生产态 / production) 完整性检查 /<br/>integrity_check<br/>注入后完整性<br/>文件: context/integrity_check.py"]
    src_zephyr_autonomy_core_context_memory_bank_py["(生产态 / production) 记忆bank / memory_<br/>bank<br/>AI 读写结构化持久上下文 (DD: memory_<br/>bank, TASK-014 beta c)<br/>文件: context/memory_bank.py"]
    src_zephyr_autonomy_core_context_mode_manager_py["(生产态 / production) mode管理器 / mode_<br/>manager<br/>模式管理器<br/>文件: context/mode_manager.py"]
    src_zephyr_autonomy_core_context_position_optimizer_py["(生产态 / production) 持仓优化器 /<br/>position_optimizer<br/>位置优化<br/>文件: context/position_optimizer.py"]
    src_zephyr_autonomy_core_context_shadow_canary_py["(生产态 / production) 影子金丝雀 /<br/>shadow_canary<br/>金丝雀部署 (B4, DD78, TASK-015 beta w)<br/>文件: context/shadow_canary.py"]
    ...
    src_zephyr_autonomy_core_context_vector_bridge_py["(生产态 / production) 向量桥接 / vector_<br/>bridge<br/>VectorBridge — CE↔VMS 检索桥接 (Connect<br/>CT-CE-VMS-001)<br/>文件: context/vector_bridge.py"]
    src_zephyr_autonomy_core_file_autoregister_py["(生产态 / production) 文件autoregister<br/>/ file_autoregister<br/>文件autoregister，主要提供注册等功能<br/>文件: autonomy_core/file_autoregister.py"]
    src_zephyr_autonomy_core_ide_watcher_py["(生产态 / production) IDE监视器 /<br/>MOD-INF-019: Agent Spec — IDE Watcher<br/>IDE 热重载监视器——Skill<br/>文件变更自动刷新 AGENTS.md<br/>文件: autonomy_core/ide_watcher.py"]
    src_zephyr_autonomy_core_integration_pipeline_bridge_py["(生产态 / production) 管线桥接 /<br/>pipeline_bridge<br/>PipelineSkillBridge — Agent Spec -><br/>Pipeline 双向桥接<br/>文件: integration/pipeline_bridge.py"]
    src_zephyr_autonomy_core_phase_planner_py["(生产态 / production) 阶段规划器 /<br/>MOD-INF-019: Agent Spec — Phase Planner<br/>阶段planner。MOD-INF-019: Agent Spec —<br/>Phase Planner<br/>文件: autonomy_core/phase_planner.py"]
    src_zephyr_autonomy_core_progressive_disclosure_injector_py["(生产态 / production)<br/>progressivedisclosureinjector.py —<br/>渐进式 / progressive_disclosure_injector<br/>渐进式披露 (B7, DD81, TASK-015 beta w)<br/>文件: autonomy_core/progressive_<br/>disclosure_injector.py"]
    src_zephyr_autonomy_core_prompt_registry_py["(生产态 / production) 提示注册表 /<br/>prompt_registry<br/>PromptRegistry: YAML-driven Prompt<br/>模板注册表<br/>文件: autonomy_core/prompt_registry.py"]
    src_zephyr_autonomy_core_self_evolution_fidelity_gate_py["(生产态 / production)<br/>self进化fidelity门禁 / MOD-INF-019:<br/>Agent Spec — Self Evolution Fidelity<br/>Gate<br/>自进化fidelity门禁。MOD-INF-019: Agent<br/>Spec — Self Evolution Fidelity Gate<br/>文件: autonomy_core/self_evolution_<br/>fidelity_gate.py"]
    src_zephyr_autonomy_core_skill_rbac_registry_py["(生产态 / production) skillRBAC注册表 /<br/>G-CT-003: Agent Spec -> RBAC capability<br/>check.<br/>技能rbac注册表。G-CT-003: Agent Spec -><br/>RBAC capability check.<br/>文件: autonomy_core/skill_rbac_<br/>registry.py"]
    src_zephyr_autonomy_core_skills_skill_attention_py["(生产态 / production) 技能attention /<br/>MOD-INF-019: Agent Spec — Skill<br/>Attention Management<br/>技能attention。MOD-INF-019: Agent Spec<br/>— Skill Attention Management<br/>文件: skills/skill_attention.py"]
    src_zephyr_autonomy_core_skills_skill_breakage_checker_py["(生产态 / production)<br/>skillbreakage检查器 / MOD-INF-019:<br/>Agent Spec — Skill Breakage Checker<br/>技能breakage检查器。MOD-INF-019: Agent<br/>Spec — Skill Breakage Checker<br/>文件: skills/skill_breakage_checker.py"]
    src_zephyr_autonomy_core_skills_skill_cache_provider_py["(生产态 / production) 技能缓存提供器 /<br/>MOD-INF-019: Agent Spec — Skill Cache<br/>Provider<br/>技能缓存提供器。MOD-INF-019: Agent Spec<br/>— Skill Cache Provider<br/>文件: skills/skill_cache_provider.py"]
    src_zephyr_autonomy_core_skills_skill_calibration_py["(生产态 / production) 技能calibration /<br/>MOD-INF-019: Agent Spec — Skill<br/>Calibration<br/>技能calibration。MOD-INF-019: Agent<br/>Spec — Skill Calibration<br/>文件: skills/skill_calibration.py"]
    src_zephyr_autonomy_core_skills_skill_canary_py["(生产态 / production) 技能金丝雀 /<br/>MOD-INF-019: Agent Spec — Skill Canary<br/>技能canary。MOD-INF-019: Agent Spec —<br/>Skill Canary<br/>文件: skills/skill_canary.py"]
    src_zephyr_autonomy_core_skills_skill_cognitive_preservation_py["(生产态 / production)<br/>skill认知preservation / MOD-INF-019:<br/>Agent Spec — Skill Cognitive<br/>Preservation<br/>技能cognitivepreservation。MOD-INF-019:<br/>Agent Spec — Skill Cognitive<br/>Preservation<br/>文件: skills/skill_cognitive_<br/>preservation.py"]
    src_zephyr_autonomy_core_skills_skill_compliance_py["(生产态 / production) 技能合规 /<br/>MOD-INF-019: Agent Spec — Skill<br/>Compliance<br/>技能合规。MOD-INF-019: Agent Spec —<br/>Skill Compliance<br/>文件: skills/skill_compliance.py"]
    src_zephyr_autonomy_core_skills_skill_consensus_py["(生产态 / production) 技能共识 /<br/>MOD-INF-019: Agent Spec — Skill<br/>Consensus<br/>技能共识。MOD-INF-019: Agent Spec —<br/>Skill Consensus<br/>文件: skills/skill_consensus.py"]
    src_zephyr_autonomy_core_skills_skill_constructor_py["(生产态 / production) 技能constructor /<br/>MOD-INF-019: Agent Spec — Skill<br/>Constructor<br/>技能constructor。MOD-INF-019: Agent<br/>Spec — Skill Constructor<br/>文件: skills/skill_constructor.py"]
    src_zephyr_autonomy_core_skills_skill_context_isolation_py["(生产态 / production)<br/>skill上下文isolation / MOD-INF-019:<br/>Agent Spec — Context Isolation<br/>技能上下文isolation。MOD-INF-019: Agent<br/>Spec — Context Isolation<br/>文件: skills/skill_context_isolation.py"]
    src_zephyr_autonomy_core_skills_skill_contract_py["(生产态 / production) 技能契约 /<br/>MOD-INF-019: Agent Spec — Skill Contract<br/>技能契约。MOD-INF-019: Agent Spec —<br/>Skill Contract<br/>文件: skills/skill_contract.py"]
    src_zephyr_autonomy_core_skills_skill_cross_model_py["(生产态 / production) 技能跨模型 /<br/>MOD-INF-019: Agent Spec — Skill<br/>Cross-Model<br/>技能跨模型。MOD-INF-019: Agent Spec —<br/>Skill Cross-Model<br/>文件: skills/skill_cross_model.py"]
    src_zephyr_autonomy_core_skills_skill_di_py["(生产态 / production) 技能di /<br/>MOD-INF-019: Agent Spec — Skill<br/>Dependency Injection<br/>技能di。MOD-INF-019: Agent Spec — Skill<br/>Dependency Injection<br/>文件: skills/skill_di.py"]
    src_zephyr_autonomy_core_skills_skill_discovery_py["(生产态 / production) 技能discovery /<br/>MOD-INF-019: Agent Spec — Skill<br/>Discovery<br/>技能discovery。MOD-INF-019: Agent Spec<br/>— Skill Discovery<br/>文件: skills/skill_discovery.py"]
    src_zephyr_autonomy_core_skills_skill_durable_py["(生产态 / production) 技能durable /<br/>MOD-INF-019: Agent Spec — Durable<br/>Execution<br/>技能durable。MOD-INF-019: Agent Spec —<br/>Durable Execution<br/>文件: skills/skill_durable.py"]
    src_zephyr_autonomy_core_skills_skill_economics_py["(生产态 / production) 技能economics /<br/>MOD-INF-019: Agent Spec — Skill<br/>Economics<br/>技能economics。MOD-INF-019: Agent Spec<br/>— Skill Economics<br/>文件: skills/skill_economics.py"]
    src_zephyr_autonomy_core_skills_skill_efficacy_calibrator_py["(生产态 / production)<br/>skillefficacy校准器 / MOD-INF-019:<br/>Agent Spec — Skill Efficacy Calibrator<br/>技能efficacycalibrator。MOD-INF-019:<br/>Agent Spec — Skill Efficacy Calibrator<br/>文件: skills/skill_efficacy_<br/>calibrator.py"]
    src_zephyr_autonomy_core_skills_skill_executor_py["(生产态 / production) 技能执行器 /<br/>skill_executor<br/>Skill 加载前创建回滚检查点<br/>文件: skills/skill_executor.py"]
    src_zephyr_autonomy_core_skills_skill_explain_py["(生产态 / production) 技能explain /<br/>MOD-INF-019: Agent Spec — XAI<br/>Explainable Skill Engine<br/>技能explain。MOD-INF-019: Agent Spec —<br/>XAI Explainable Skill Engine<br/>文件: skills/skill_explain.py"]
    src_zephyr_autonomy_core_skills_skill_feature_flags_py["(生产态 / production) 技能功能标志 /<br/>MOD-INF-019: Agent Spec — Skill Feature<br/>Flags<br/>技能功能标志。MOD-INF-019: Agent Spec —<br/>Skill Feature Flags<br/>文件: skills/skill_feature_flags.py"]
    src_zephyr_autonomy_core_skills_skill_feedback_py["(生产态 / production) 技能反馈 /<br/>MOD-INF-019: Agent Spec — Skill<br/>Feedback Loop<br/>技能反馈。MOD-INF-019: Agent Spec —<br/>Skill Feedback Loop<br/>文件: skills/skill_feedback.py"]
    src_zephyr_autonomy_core_skills_skill_freshness_ext_py["(生产态 / production) 技能freshness扩展<br/>/ MOD-INF-019: Agent Spec — Skill<br/>Freshness Extensions<br/>技能freshness扩展。MOD-INF-019: Agent<br/>Spec — Skill Freshness Extensions<br/>文件: skills/skill_freshness_ext.py"]
    src_zephyr_autonomy_core_skills_skill_gitops_py["(生产态 / production) 技能gitops /<br/>MOD-INF-019: Agent Spec — Skill GitOps<br/>技能gitops。MOD-INF-019: Agent Spec —<br/>Skill GitOps<br/>文件: skills/skill_gitops.py"]
    src_zephyr_autonomy_core_skills_skill_guardrails_py["(生产态 / production) 技能guardrails /<br/>MOD-INF-019: Agent Spec — Skill<br/>Guardrails<br/>技能guardrails。MOD-INF-019: Agent Spec<br/>— Skill Guardrails<br/>文件: skills/skill_guardrails.py"]
    src_zephyr_autonomy_core_skills_skill_idempotency_py["(生产态 / production) 技能幂等性 /<br/>MOD-INF-019: Agent Spec — Skill<br/>Idempotency<br/>技能idempotency。MOD-INF-019: Agent<br/>Spec — Skill Idempotency<br/>文件: skills/skill_idempotency.py"]
    src_zephyr_autonomy_core_skills_skill_kya_py["(生产态 / production) 技能kya /<br/>MOD-INF-019: Agent Spec — Skill KYA<br/>技能kya。MOD-INF-019: Agent Spec —<br/>Skill KYA<br/>文件: skills/skill_kya.py"]
    src_zephyr_autonomy_core_skills_skill_learning_py["(生产态 / production) 技能learning /<br/>MOD-INF-019: Agent Spec — Skill<br/>Self-Learning Engine<br/>技能learning。MOD-INF-019: Agent Spec —<br/>Skill Self-Learning Engine<br/>文件: skills/skill_learning.py"]
    src_zephyr_autonomy_core_skills_skill_lineage_py["(生产态 / production) 技能lineage /<br/>MOD-INF-019: Agent Spec — Skill Lineage<br/>技能lineage。MOD-INF-019: Agent Spec —<br/>Skill Lineage<br/>文件: skills/skill_lineage.py"]
    src_zephyr_autonomy_core_skills_skill_locking_py["(生产态 / production) 技能locking /<br/>MOD-INF-019: Agent Spec — Skill Locking<br/>(Production Hardenin<br/>技能locking。MOD-INF-019: Agent Spec —<br/>Skill Locking (Production Hardening)<br/>文件: skills/skill_locking.py"]
    src_zephyr_autonomy_core_skills_skill_observability_py["(生产态 / production) 技能可观测性 /<br/>MOD-INF-019: Agent Spec — Skill<br/>Observability<br/>技能observability。MOD-INF-019: Agent<br/>Spec — Skill Observability<br/>文件: skills/skill_observability.py"]
    src_zephyr_autonomy_core_skills_skill_ontology_py["(生产态 / production) 技能ontology /<br/>MOD-INF-019: Agent Spec — Skill Ontology<br/>技能ontology。MOD-INF-019: Agent Spec —<br/>Skill Ontology<br/>文件: skills/skill_ontology.py"]
    src_zephyr_autonomy_core_skills_skill_postmortem_py["(生产态 / production) 技能postmortem /<br/>skill_postmortem<br/>MOD-INF-019: Agent Spec — Skill<br/>Postmortem (追问到底)<br/>文件: skills/skill_postmortem.py"]
    src_zephyr_autonomy_core_skills_skill_prompt_cache_py["(生产态 / production) 技能提示缓存 /<br/>MOD-INF-019: Agent Spec — Skill Prompt<br/>Cache<br/>技能提示缓存。MOD-INF-019: Agent Spec —<br/>Skill Prompt Cache<br/>文件: skills/skill_prompt_cache.py"]
    src_zephyr_autonomy_core_skills_skill_prompt_opt_py["(生产态 / production) 技能提示opt /<br/>MOD-INF-019: Agent Spec — Skill Prompt<br/>Optimizer<br/>技能提示opt。MOD-INF-019: Agent Spec —<br/>Skill Prompt Optimizer<br/>文件: skills/skill_prompt_opt.py"]
    src_zephyr_autonomy_core_skills_skill_resilience_py["(生产态 / production) 技能韧性 /<br/>MOD-INF-019: Agent Spec — Skill<br/>Resilience<br/>技能韧性。MOD-INF-019: Agent Spec —<br/>Skill Resilience<br/>文件: skills/skill_resilience.py"]
    src_zephyr_autonomy_core_skills_skill_risk_mitigator_py["(生产态 / production)<br/>skill风险mitigator / MOD-INF-019: Agent<br/>Spec — Skill Risk Mitigator<br/>技能风险mitigator。MOD-INF-019: Agent<br/>Spec — Skill Risk Mitigator<br/>文件: skills/skill_risk_mitigator.py"]
    src_zephyr_autonomy_core_skills_skill_router_py["(生产态 / production) 技能路由器 /<br/>skill_router<br/>技能路由器，主要提供from标签等功能<br/>文件: skills/skill_router.py"]
    src_zephyr_autonomy_core_skills_skill_sandbox_py["(生产态 / production) 技能沙箱 /<br/>MOD-INF-019: Agent Spec — Skill Sandbox<br/>技能sandbox。MOD-INF-019: Agent Spec —<br/>Skill Sandbox<br/>文件: skills/skill_sandbox.py"]
    src_zephyr_autonomy_core_skills_skill_schema_registry_py["(生产态 / production) 技能模式注册表 /<br/>MOD-INF-019: Agent Spec — Skill Schema<br/>Registry<br/>技能模式注册表。MOD-INF-019: Agent Spec<br/>— Skill Schema Registry<br/>文件: skills/skill_schema_registry.py"]
    src_zephyr_autonomy_core_skills_skill_security_py["(生产态 / production) 技能安全 /<br/>MOD-INF-019: Agent Spec — Skill Security<br/>技能安全。MOD-INF-019: Agent Spec —<br/>Skill Security<br/>文件: skills/skill_security.py"]
    src_zephyr_autonomy_core_skills_skill_shadow_py["(生产态 / production) 技能影子 /<br/>MOD-INF-019: Agent Spec — Skill Shadow<br/>Deployment<br/>技能shadow。MOD-INF-019: Agent Spec —<br/>Skill Shadow Deployment<br/>文件: skills/skill_shadow.py"]
    src_zephyr_autonomy_core_skills_skill_silent_failure_py["(生产态 / production) skillsilent故障 /<br/>MOD-INF-019: Agent Spec — Silent<br/>Failure Detector<br/>技能silentfailure。MOD-INF-019: Agent<br/>Spec — Silent Failure Detector<br/>文件: skills/skill_silent_failure.py"]
    src_zephyr_autonomy_core_skills_skill_team_optimizer_py["(生产态 / production) 技能团队优化器 /<br/>MOD-INF-019: Agent Spec — Skill Team<br/>Optimizer<br/>技能团队优化器。MOD-INF-019: Agent Spec<br/>— Skill Team Optimizer<br/>文件: skills/skill_team_optimizer.py"]
    src_zephyr_autonomy_core_skills_skill_telemetry_py["(生产态 / production) 技能遥测 /<br/>MOD-INF-019: Agent Spec — Skill<br/>Telemetry<br/>技能遥测。MOD-INF-019: Agent Spec —<br/>Skill Telemetry<br/>文件: skills/skill_telemetry.py"]
    src_zephyr_autonomy_core_skills_skill_temperature_py["(生产态 / production) 技能temperature /<br/>MOD-INF-019: Agent Spec — Skill<br/>Temperature<br/>技能temperature。MOD-INF-019: Agent<br/>Spec — Skill Temperature<br/>文件: skills/skill_temperature.py"]
    src_zephyr_autonomy_core_skills_skill_tokenomics_py["(生产态 / production) 技能tokenomics /<br/>MOD-INF-019: Agent Spec — Skill<br/>Tokenomics<br/>技能tokenomics。MOD-INF-019: Agent Spec<br/>— Skill Tokenomics<br/>文件: skills/skill_tokenomics.py"]
    src_zephyr_autonomy_core_skills_skill_translator_py["(生产态 / production) 技能translator /<br/>MOD-INF-019: Agent Spec — Skill<br/>Translator<br/>技能translator。MOD-INF-019: Agent Spec<br/>— Skill Translator<br/>文件: skills/skill_translator.py"]
    src_zephyr_autonomy_core_skills_skill_workflow_py["(生产态 / production) 技能工作流 /<br/>MOD-INF-019: Agent Spec — Skill<br/>Workflow Orchestrator<br/>技能工作流。MOD-INF-019: Agent Spec —<br/>Skill Workflow Orchestrator<br/>文件: skills/skill_workflow.py"]
    src_zephyr_autonomy_core_spec_engine_py["(生产态 / production) spec引擎 / spec_<br/>engine<br/>MOD-INF-019: Agent Spec — SpecEngine<br/>蓝图->Skill 升级引擎<br/>文件: autonomy_core/spec_engine.py"]
    src_zephyr_autonomy_core_vibe_coding_quality_gate_py["(生产态 / production)<br/>vibecoding质量门禁 / vibe_coding_<br/>quality_gate<br/>VibeCodingQualityGate — 代码质量门禁<br/>（stub, tests 待实装后补全实现）<br/>文件: autonomy_core/vibe_coding_quality_<br/>gate.py"]
    src_zephyr_governance_persistence_intent_parser_py["(生产态 / production) intent解析器 /<br/>intent_parser<br/>IntentParser · 意图三阶段级联解析器<br/>（V-09）<br/>文件: persistence/intent_parser.py"]
    src_zephyr_infrastructure_system_snapshot_py["(生产态 / production) 系统快照 / system_<br/>snapshot<br/>SystemSnapshotter — M1 系统状态镜像<br/>（CL-017 RI 扩展模式）<br/>文件: infrastructure/system_snapshot.py"]
    src_zephyr_infrastructure_system_telemetry_otel_instrumentation_py["(生产态 / production)<br/>otelinstrumentation.py — 全链路 OTel<br/>(B12, / otel_instrumentation<br/>全链路 OTel (B12, DD86, TASK-015 beta v)<br/>文件: system_telemetry/otel_<br/>instrumentation.py"]
    src_zephyr_integration_vector_memory_vector_writer_py["(生产态 / production) 向量写入器 /<br/>vector_writer<br/>CE 向量写入器 — vectorize_and_store()<br/>生产者<br/>文件: vector_memory/vector_writer.py"]
    src_zephyr_security_llm_defense_llm_security_adversarial_robustness_py["(生产态 / production) 对抗鲁棒性 /<br/>adversarial_robustness<br/>对抗鲁棒性 (B8, DD82, TASK-015 beta w)<br/>文件: llm_security/adversarial_<br/>robustness.py"]
    src_zephyr_security_llm_defense_llm_security_alignment_scorer_py["(生产态 / production) 对齐评分器 /<br/>alignment_scorer<br/>对齐评分 (B11, DD85, TASK-015 beta w)<br/>文件: llm_security/alignment_scorer.py"]
    src_zephyr_security_llm_defense_llm_security_lsg_pattern_tracker_py["(生产态 / production) lsg模式追踪器 /<br/>lsg_pattern_tracker<br/>LSG 模式逃逸追踪<br/>文件: llm_security/lsg_pattern_<br/>tracker.py"]
    ...
    src_zephyr_security_llm_defense_llm_security_sensitivity_classifier_py["(生产态 / production) sensitivity分类器<br/>/ sensitivity_classifier<br/>数据分级 (B9, DD83, TASK-015 beta w)<br/>文件: llm_security/sensitivity_<br/>classifier.py"]
    src_zephyr_security_llm_defense_llm_security_solo_dev_safety_net_py["(生产态 / production) solodev安全net /<br/>solo_dev_safety_net<br/>单人无审查安全网<br/>文件: llm_security/solo_dev_safety_<br/>net.py"]
    src_zephyr_shared_ai_guards_config_safety_guard_py["(生产态 / production) 配置安全守卫 /<br/>config_safety_guard<br/>配置自毁防护<br/>文件: ai_guards/config_safety_guard.py"]
    src_zephyr_shared_dependency_dependency_tracker_py["(生产态 / production) 依赖追踪器 /<br/>dependency_tracker<br/>依赖追踪<br/>文件: dependency/dependency_tracker.py"]
    src_zephyr_shared_io_cache_invalidation_py["(生产态 / production) 缓存invalidation<br/>/ cache_invalidation<br/>缓存一致性<br/>文件: io/cache_invalidation.py"]
    src_zephyr_shared_utils_verify_paths_py["(生产态 / production) 校验paths /<br/>verify_paths<br/>代码路径索引验证<br/>文件: utils/verify_paths.py"]
    tests_automation_test_auto_runtime_e2e_py["(生产态 / production)<br/>测试自动运行时端到端 / test_auto_<br/>runtime_e2e<br/>F1 AutoRuntimeCore 非mock端到端集成测试<br/>文件: automation/test_auto_runtime_<br/>e2e.py"]
    tests_f_lifecycle_test_f1_event_trigger_py["(生产态 / production) F1<br/>事件触发启动测试 / test_f1_event_trigger<br/>F1 事件触发启动测试<br/>文件: f_lifecycle/test_f1_event_<br/>trigger.py"]
    tests_trading_extreme_test_f14_pipeline_extreme_py["(生产态 / production) F14 管线编排<br/>/反馈环 — 红蓝对抗端到端极端测试 / test_<br/>f14_pipeline_extreme<br/>F14 管线编排/反馈环 —<br/>红蓝对抗端到端极端测试<br/>文件: extreme/test_f14_pipeline_<br/>extreme.py"]
    tests_trading_extreme_test_f1_extreme_py["(生产态 / production) F1 自动驾驶<br/>/运行时大脑 — 红蓝对抗端到端极端测试 /<br/>test_f1_extreme<br/>F1 自动驾驶/运行时大脑 —<br/>红蓝对抗端到端极端测试<br/>文件: extreme/test_f1_extreme.py"]
    ...
    src_zephyr_autonomy_core_context_context_pipeline_py["(生产态 / production) 上下文管线 /<br/>context_pipeline<br/>上下文管线 — Context Engine<br/>**四段流水线组合根**<br/>文件: context/context_pipeline.py"]
    src_zephyr_autonomy_core_skills_skill_evaluator_py["(生产态 / production) 技能evaluator /<br/>MOD-INF-019: Agent Spec — Skill<br/>Evaluator<br/>技能evaluator。MOD-INF-019: Agent Spec<br/>— Skill Evaluator<br/>文件: skills/skill_evaluator.py"]
    src_zephyr_autonomy_core_skills_skill_factory_py["(生产态 / production) 技能工厂 / skill_<br/>factory<br/>技能工厂，主要提供read蓝图、提取模块信息<br/>、findsection等功能<br/>文件: skills/skill_factory.py"]
    src_zephyr_autonomy_core_skills_skill_kill_switch_py["(生产态 / production) 技能终止开关 /<br/>MOD-INF-019: Agent Spec — Skill Kill<br/>Switch<br/>技能终止开关。MOD-INF-019: Agent Spec —<br/>Skill Kill Switch<br/>文件: skills/skill_kill_switch.py"]
    src_zephyr_autonomy_core_skills_skill_lifecycle_py["(生产态 / production) 技能生命周期 /<br/>MOD-INF-019: Agent Spec — Skill<br/>Lifecycle<br/>技能生命周期。MOD-INF-019: Agent Spec —<br/>Skill Lifecycle<br/>文件: skills/skill_lifecycle.py"]
    src_zephyr_autonomy_core_skills_skill_model_evolution_py["(生产态 / production) 技能模型进化 /<br/>MOD-INF-019: Agent Spec — Skill Model<br/>Evolution<br/>技能模型进化。MOD-INF-019: Agent Spec —<br/>Skill Model Evolution<br/>文件: skills/skill_model_evolution.py"]
    src_zephyr_autonomy_core_skills_skill_registry_py["(生产态 / production) 技能注册表 /<br/>skill_registry<br/>技能注册表.py —— Skill 注册基座（Phase<br/>14 / 盲点 B34）<br/>文件: skills/skill_registry.py"]
    src_zephyr_autonomy_core_trigger_router_py["(生产态 / production) 触发器路由器 /<br/>trigger_router<br/>触发器路由器，主要提供from标签等功能<br/>文件: autonomy_core/trigger_router.py"]
    src_zephyr_governance_persistence_intent_keyword_mapper_py["(生产态 / production) 意图关键词映射器<br/>/ IntentKeywordMapper - Stage 1 of<br/>three-stage intent parsing<br/>意图识别域（D0-D9 + UNKNOWN，与<br/>metadata_registry.yaml §9.2 domain<br/>枚举对齐）。<br/>文件: persistence/intent_keyword_<br/>mapper.py"]
    ...
    src_zephyr_autonomy_core_context_context_assembler_py["(生产态 / production) 上下文assembler /<br/>context_assembler<br/>ContextAssembler —<br/>上下文装配、校验、影子留档<br/>文件: context/context_assembler.py"]
    src_zephyr_autonomy_core_context_context_injector_py["(生产态 / production) 上下文injector /<br/>ContextInjector: retrieve and inject<br/>relevant knowledge into<br/>上下文injector。ContextInjector:<br/>retrieve and inject relevant knowledge<br/>into prompt context<br/>文件: context/context_injector.py"]
    src_zephyr_autonomy_core_skills_skill_freshness_py["(生产态 / production) 技能freshness /<br/>MOD-INF-019: Agent Spec — Skill<br/>Freshness Decay<br/>技能freshness。MOD-INF-019: Agent Spec<br/>— Skill Freshness Decay<br/>文件: skills/skill_freshness.py"]
    src_zephyr_autonomy_core_skills_skill_loader_py["(生产态 / production) 技能加载器 /<br/>skill_loader<br/>技能加载器，主要提供提取体、compressto严<br/>重rules、解析技能路径等功能<br/>文件: skills/skill_loader.py"]
    src_zephyr_autonomy_core_skills_skill_model_py["(生产态 / production) 技能模型 / skill_<br/>model<br/>技能模型，提供包入口和模块加载功能<br/>文件: skills/skill_model.py"]
    src_zephyr_shared_blueprint_tools_architecture_context_loader_py["(生产态 / production) 架构上下文加载器<br/>/ architecture_context_loader<br/>架构上下文加载器 — 加载 ``generate_<br/>architecture_context.py`` 产出的预编译<br/>JSON<br/>文件: blueprint_tools/architecture_<br/>context_loader.py"]
    ...
    src_zephyr_autonomy_core_context_context_rule_registry_py["(生产态 / production) 上下文规则注册表<br/>/ context_rule_registry<br/>上下文规则注册表，context的注册表，登记<br/>和查询已注册条目。<br/>文件: context/context_rule_registry.py"]
    src_zephyr_shared_io_doc_compressor_py["(生产态 / production) doc压缩器 / doc_<br/>compressor<br/>DocCompressor — 文档压缩服务（CL-018 RI<br/>扩展模式）<br/>文件: io/doc_compressor.py"]
    ...
    D_ORCHESTRATOR["(生产态 / production) 代理编排器 /<br/>Agent Orchestrator<br/>代理编排器，负责 Agent<br/>任务全生命周期：任务入队、调度、沙箱执行<br/>、幻觉检测和收尾归档<br/>跨域节点 / cross-domain"]
    ...
    D_GOV_REPAIR["(生产态 / production) 治理修复 /<br/>Governance Repair<br/>治理修复，负责治理问题自动修复和修复策略<br/>管理<br/>跨域节点 / cross-domain"]
    ...
    class D_SHARED,D_INFRA_RUNTIME,D_ORCHESTRATOR,D_FEEDBACK_LOOP,D_INTEGRATION,D_GOV_CODE_QUALITY,D_SECURITY,D_GOV_REPAIR external_prod
```
