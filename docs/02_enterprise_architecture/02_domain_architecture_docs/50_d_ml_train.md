...
> 展示全部 4 个模块（生产态 0 + 设计态 1 + 原型态 3），标签标注成熟度。
...
graph TD
    ...
    class docs_03_modules_cross_layer_model_profiler_blueprint_md,src_zephyr_ml_train_implementations_default_inference_engine_py,src_zephyr_ml_train_inference_base_py,src_zephyr_ml_train_trainer_base_py design
    class D_TRADING,D_INFRASTRUCTURE,D_SHARED,D_INTELLIGENCE external_prod
    class D_GOV_DOCS,D_BACKTEST,D_ML_SERVE external_design
```
...
graph TD
    ...
    classDef external_prod fill:#e8f5e9,stroke:#1b5e20,stroke-width:1px,color:#000
    classDef external_design fill:#fce4ec,stroke:#880e4f,stroke-width:1px,color:#000,stroke-dasharray: 5 5
    class src_zephyr_ml_train_implementations_default_inference_engine_py,src_zephyr_ml_train_inference_base_py,src_zephyr_ml_train_trainer_base_py design
    class D_INFRASTRUCTURE,D_TRADING,D_SHARED,D_INTELLIGENCE external_prod
    class D_GOV_DOCS,D_ML_SERVE external_design
```
...
| 2 | D_ML_TRAIN — Default Inference Engine (default... | → | D_INFRASTRUCTURE: model_serving_response.py | 导入依赖 / import_depends |
| 3 | D_ML_TRAIN — ML Inference Base (inference_base.py) | → | D_INFRASTRUCTURE: model_serving_response.py | 导入依赖 / import_depends |
...
> 本域与 7 个外部域直接连接（出边 6 条 + 入边 8 条 = 14 条）。只显示直接连接的域，不展开具体节点。
