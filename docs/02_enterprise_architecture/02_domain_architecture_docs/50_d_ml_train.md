...
graph TD
    ...
    class D_SHARED,D_GOV_DOCS,D_BACKTEST external_design
```
...
graph TD
    ...
    class D_GOV_DOCS,D_BACKTEST external_design
```
...
| 7 | D_INTELLIGENCE 上下文管理: D_ML_TRAIN — ML Training Domain (__init__.py) | → | D_ML_TRAIN — ML Inference Base (inference_base.py) | config_depends / config_depends |
| 8 | D_INTELLIGENCE 上下文管理: D_ML_TRAIN — ML Training Concrete Implementati... | → | D_ML_TRAIN — Default Inference Engine (default... | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 5 个外部域直接连接（出边 6 条 + 入边 8 条 = 14 条）。只显示直接连接的域，不展开具体节点。
