...
| 5 | tests/ml_experiment/test_l13_experimentation.py | test_l13_experimentation.py | 原型态 / prototype | [MOD-L13-001](../../03_modules/_domain_simulation/blueprint.md) |
...
> 展示全部 6 个模块（生产态 0 + 设计态 1 + 原型态 5），标签标注成熟度。
...
graph TD
    subgraph D_ML_TRAIN["D_ML_TRAIN 训练"]
        ...
        tests_ml_experiment_test_l13_experimentation_py["(原型态 / prototype) test_l13_experimentation.py"]
    end
    ...
    class docs_03_modules_cross_layer_model_profiler_blueprint_md,src_zephyr_intelligence_model_evaluation_init_py,src_zephyr_ml_train_implementations_default_inference_engine_py,src_zephyr_ml_train_inference_base_py,src_zephyr_ml_train_trainer_base_py,tests_ml_experiment_test_l13_experimentation_py design
    class D_TRADING external_prod
    class D_INTELLIGENCE,D_SHARED,D_GOV_DOCS,D_GOVERNANCE,D_BACKTEST,D_ML_SERVE external_design
```
...
graph TD
    ...
    class D_GOVERNANCE,D_GOV_DOCS,D_BACKTEST external_design
```

### 原型态子图（仅 design_maturity=prototype 的模块和依赖）

> 仅展示代码已写、验证中未稳定上线的原型态模块（共 5 个，3 条域内依赖）。
...
graph TD
    ...
    classDef external_prod fill:#e8f5e9,stroke:#1b5e20,stroke-width:1px,color:#000
    classDef external_design fill:#fce4ec,stroke:#880e4f,stroke-width:1px,color:#000,stroke-dasharray: 5 5
    class src_zephyr_intelligence_model_evaluation_init_py,src_zephyr_ml_train_implementations_default_inference_engine_py,src_zephyr_ml_train_inference_base_py,src_zephyr_ml_train_trainer_base_py,tests_ml_experiment_test_l13_experimentation_py design
    class D_TRADING external_prod
    class D_INTELLIGENCE,D_SHARED,D_GOV_DOCS,D_ML_SERVE external_design
```
...
| 2 | Intelligence — Model Evaluation Domain (__init... | → | D_INTELLIGENCE 上下文管理: KB->VMS 同步引擎 — sync_to_vms() 生产者 (sync_... | config_depends / config_depends |
| 3 | D_ML_TRAIN — Default Inference Engine (default... | → | D_SHARED 共享服务: model_serving_response.py | 导入依赖 / import_depends |
| 4 | D_ML_TRAIN — Default Inference Engine (default... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 5 | D_ML_TRAIN — ML Inference Base (inference_base.py) | → | D_SHARED 共享服务: model_serving_response.py | 导入依赖 / import_depends |
| 6 | D_ML_TRAIN — Default Inference Engine (default... | → | D_TRADING 交易运营: model_serving_request.py | 导入依赖 / import_depends |
| 7 | D_ML_TRAIN — ML Inference Base (inference_base.py) | → | D_TRADING 交易运营: model_serving_request.py | 导入依赖 / import_depends |
...
| 2 | D_GOVERNANCE 生命周期管理: model_provider_data.py | → | blueprint.md | data / data |
| 3 | D_GOV_DOCS 架构文档治理: blueprint.md | → | blueprint.md | runtime / runtime |
...
| 8 | D_INTELLIGENCE 上下文管理: D_ML_TRAIN — ML Training Domain (__init__.py) | → | D_ML_TRAIN — ML Inference Base (inference_base.py) | config_depends / config_depends |
| 9 | D_INTELLIGENCE 上下文管理: D_ML_TRAIN — ML Training Concrete Implementati... | → | D_ML_TRAIN — Default Inference Engine (default... | 导入依赖 / import_depends |
| 10 | D_ML_SERVE 推理: __init__.py | → | D_ML_TRAIN — ML Inference Base (inference_base.py) | runtime / runtime |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 7 个外部域直接连接（出边 7 条 + 入边 10 条 = 17 条）。只显示直接连接的域，不展开具体节点。
