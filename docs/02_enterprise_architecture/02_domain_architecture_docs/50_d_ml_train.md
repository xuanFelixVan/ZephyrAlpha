...
|:--:|---------|:--:|---------|---------|
| 1 | D_ML_TRAIN — Default Inference Engine (default... | → | D_INFRASTRUCTURE: model_serving_response.py | 导入依赖 / import_depends |
| 2 | D_ML_TRAIN — ML Inference Base (inference_base.py) | → | D_INFRASTRUCTURE: model_serving_response.py | 导入依赖 / import_depends |
...
> 本域与 6 个外部域直接连接（出边 5 条 + 入边 7 条 = 12 条）。只显示直接连接的域，不展开具体节点。
