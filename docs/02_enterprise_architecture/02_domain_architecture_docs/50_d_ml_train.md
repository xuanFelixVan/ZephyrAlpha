...
| 5 | D_INTELLIGENCE 上下文管理: D_ML_TRAIN — ML Training Domain (__init__.py) | → | D_ML_TRAIN — ML Inference Base (inference_base.py) | config_depends / config_depends |
...
> 本域与 3 个外部域直接连接（出边 6 条 + 入边 6 条 = 12 条）。只显示直接连接的域，不展开具体节点。
