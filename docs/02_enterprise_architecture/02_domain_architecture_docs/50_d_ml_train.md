...
graph TD
    ...
    class D_INTELLIGENCE,D_TRADING,D_GOVERNANCE external_prod
...
|:--:|---------|:--:|---------|---------|
| 1 | D_GOVERNANCE 生命周期管理: G-CT-006 — BudgetAlert re-exported from shared... | → | blueprint.md | data / data |
...
> 本域与 4 个外部域直接连接（出边 6 条 + 入边 7 条 = 13 条）。只显示直接连接的域，不展开具体节点。
