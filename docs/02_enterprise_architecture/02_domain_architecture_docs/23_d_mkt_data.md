...
flowchart TD
    src_zephyr_market_data_init_py["zephyr/market_data 包入口<br/>行情数据域包入口，归集子模块按需懒加载<br/>文件: market_data/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_market_data_connectors["连接器<br/>⛔ 行情数据域，设计已就绪，等待开发排期<br/>文件: connectors/<br/>(设计态 / design)"]
