...
flowchart TD
    src_zephyr_market_data_init_py["(生产态 / production) market_data/__init__.py<br/>文件: market_data/__init__.py"]
    src_zephyr_market_data_extensions_init_py["(生产态 / production) _extensions/__init__.py<br/>文件: _extensions/__init__.py"]
    src_zephyr_market_data_api_init_py["(生产态 / production) api/__init__.py<br/>文件: api/__init__.py"]
    src_zephyr_market_data_autoload_py["(设计态 / design) market_data/autoload.py<br/>文件: market_data/autoload.py"]
    src_zephyr_market_data_connectors["(设计态 / design) connectors/<br/>文件: connectors/"]
    src_zephyr_market_data_core_init_py["(生产态 / production) core/__init__.py<br/>文件: core/__init__.py"]
    src_zephyr_market_data_failover["(设计态 / design) failover/<br/>文件: failover/"]
    src_zephyr_market_data_infrastructure_init_py["(生产态 / production) infrastructure/__init__.py<br/>文件: infrastructure/__init__.py"]
    src_zephyr_market_data_models_init_py["(生产态 / production) models/__init__.py<br/>文件: models/__init__.py"]
    ...
    src_zephyr_market_data_services_init_py["(生产态 / production) services/__init__.py<br/>文件: services/__init__.py"]
    ...
    src_zephyr_market_data_vendor_registry_py["(设计态 / design) market_data/vendor_registry.py<br/>文件: market_data/vendor_registry.py"]
    src_zephyr_market_data_normalized_market_data_producer_producer_py ~~~ src_zephyr_market_data_vendor_registry_py
    src_zephyr_market_data_raw_data_cache["(设计态 / design) raw_data_cache/<br/>文件: raw_data_cache/"]
    src_zephyr_market_data_vendor_base_py["(设计态 / design) market_data/vendor_base.py<br/>文件: market_data/vendor_base.py"]
...
|:--:|---------|:--:|---------|---------|
| 1 | market_data/autoload.py | → | D_DATA 数据接入层: 表名/品类注册表消费层（裁定 #ARCH-CH-024 Phase 2）。 (dat... | runtime / runtime |
| 2 | NormalizedMarketData 生产者——D_MKT_DATA→D_FACTOR 数据... | → | D_DATA 数据接入层: zephyr.data — 数据源集成器（MOD-L00-004）。 (data/__init... | 导入依赖 / import_depends |
| 3 | NormalizedMarketData 生产者——D_MKT_DATA→D_FACTOR 数据... | → | D_DATA 数据接入层: ClickHouse 统一读取层（裁定 #ARCH-CH-007）。 (data/ch_rea... | 导入依赖 / import_depends |
| 4 | NormalizedMarketData 生产者——D_MKT_DATA→D_FACTOR 数据... | → | D_DATA 数据接入层: 表名/品类注册表消费层（裁定 #ARCH-CH-024 Phase 2）。 (dat... | 导入依赖 / import_depends |
| 5 | raw_data_cache/ | → | D_DATA 数据接入层: 数据源 Provider 抽象基类（MOD-L00-004 §4）。 (data/provi... | data / data |
| 6 | market_data/__init__.py | → | D_INFRASTRUCTURE 跨层契约基础设施: contracts/market_data.py | 导入依赖 / import_depends |
...
|:--:|---------|:--:|---------|---------|
| 1 | D_EX_SOR 执行路由: core/__init__.py | → | failover/ | runtime / runtime |
