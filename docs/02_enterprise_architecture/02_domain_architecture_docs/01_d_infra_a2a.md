...
graph TD
    ...
    class D_GOVERNANCE,D_SECURITY,D_GOV_AUDIT,D_INFRA_TELEMETRY,D_INFRA_RUNTIME external_prod
...
| 38 | D_INFRA_TELEMETRY 可观测性: logs — 结构化日志流（structlog + JSONL + trace... | → | test_a2a_check.py | contract / contract |
...
> 本域与 12 个外部域直接连接（出边 29 条 + 入边 40 条 = 69 条）。只显示直接连接的域，不展开具体节点。
