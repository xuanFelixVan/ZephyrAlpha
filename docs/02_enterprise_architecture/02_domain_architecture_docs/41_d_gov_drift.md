...
# 41_d_gov_drift / 漂移检测 / Drift Detection
...
| 72 | src/zephyr/governance/integrity.py | integrity.py | 原型态 / prototype | [MOD-INF-020](../../03_modules/_domain_governance/audit_trail/blueprint.md) |
...
> 展示全部 74 个模块（生产态 69 + 设计态 1 + 原型态 4），标签标注成熟度。
...
graph TD
    ...
    class D_GOV_DOCS,D_GOV_CODE_QUALITY,D_SECURITY,D_GOV_AUDIT,D_COMPLIANCE external_design
```
...
graph TD
    ...
    class D_GOVERNANCE,D_SHARED,D_OPS,D_INFRA_RUNTIME,D_GOV_RULE external_prod
...
graph TD
    ...
    class D_SHARED,D_GOV_AUDIT,D_SECURITY,D_INFRA_RECOVERY,D_GOVERNANCE,D_ORCHESTRATOR,D_FEEDBACK_LOOP external_prod
    class D_COMPLIANCE external_design
```

### 运营态子图（仅 design_maturity=production 的模块和依赖）

> 仅展示已上线运行的模块（共 69 个，19 条域内依赖）。
...
graph TD
    ...
    class D_SHARED,D_GOV_SCRIPTS,D_GOVERNANCE,D_INFRA_RUNTIME,D_OPS external_prod
...
graph TD
    ...
    class D_GOV_DOCS,D_GOV_AUDIT,D_SECURITY,D_GOV_CODE_QUALITY external_design
```
...
graph TD
    ...
    classDef external_prod fill:#e8f5e9,stroke:#1b5e20,stroke-width:1px,color:#000
    classDef external_design fill:#fce4ec,stroke:#880e4f,stroke-width:1px,color:#000,stroke-dasharray: 5 5
    class src_zephyr_gov_drift_detector_core_init_py,src_zephyr_gov_drift_detector_core_bridges_drift_bridge_py,src_zephyr_gov_enforcement_rule_enforcement_drift_detector_py,src_zephyr_governance_integrity_py design
    class D_GOV_AUDIT,D_SHARED,D_SECURITY,D_INFRA_RECOVERY,D_ORCHESTRATOR,D_FEEDBACK_LOOP external_prod
```
...
| 15 | blueprint.md | → | D_GOV_CODE_QUALITY 代码质量治理: ch_final_gate.py — ch_writer.query() 直接调用.... | runtime / runtime |
...
> 本域与 16 个外部域直接连接（出边 49 条 + 入边 113 条 = 162 条）。只显示直接连接的域，不展开具体节点。
