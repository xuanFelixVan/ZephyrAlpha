...
graph TD
    ...
    class D_GOV_AUDIT,D_SECURITY,D_SHARED,D_GOVERNANCE,D_GOV_SCRIPTS,D_COMPLIANCE,D_INFRA_RUNTIME external_prod
```
...
graph TD
    ...
    class D_GOV_AUDIT,D_GOVERNANCE,D_SHARED,D_INTEGRATION,D_SECURITY,D_COMPLIANCE,D_FEEDBACK_LOOP external_prod
```
...
graph TD
    ...
    class D_GOV_AUDIT,D_SHARED,D_SECURITY,D_GOVERNANCE,D_GOV_SCRIPTS,D_GOV_ENFORCEMENT,D_INFRA_RECOVERY,D_COMPLIANCE,D_FEEDBACK_LOOP,D_INFRA_RUNTIME external_prod
```
...
| 32 | Coverage Dashboard — dashboard.py (dashboard.py) | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
...
| 42 | Scan Mutex — scan_mutex.py (scan_mutex.py) | → | D_SHARED 共享服务: lock.py —— 分布式锁抽象（Phase 10 新增 | 盲点... | 导入依赖 / import_depends |
...
| 46 | Gate-side Drift Detector Recovery — zephyr.gov... | → | D_SHARED 共享服务: async_utils.py — async/sync 边界桥接（5.12.8 .... | 导入依赖 / import_depends |
...
> 本域与 14 个外部域直接连接（出边 49 条 + 入边 106 条 = 155 条）。只显示直接连接的域，不展开具体节点。
