...
graph TD
    ...
    class D_GOV_DOCS,D_SECURITY,D_GOV_AUDIT,D_COMPLIANCE,D_INTELLIGENCE,D_INFRA_RECOVERY external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_REPAIR,D_GOV_RULE external_prod
...
graph TD
    ...
    class D_SHARED,D_INTELLIGENCE,D_KNOWLEDGE,D_COMPLIANCE external_design
```
...
graph TD
    ...
    class D_GOVERNANCE,D_GOV_SCRIPTS,D_SHARED,D_GOV_ENFORCEMENT,D_GOV_REPAIR external_prod
...
| 20 | __init__.py | → | D_SECURITY 对抗验证: Alert Router — alert_router.py (alert_router.py) | config_depends / config_depends |
...
> 本域与 22 个外部域直接连接（出边 47 条 + 入边 199 条 = 246 条）。只显示直接连接的域，不展开具体节点。
