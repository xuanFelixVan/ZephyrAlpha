...
graph TD
    ...
    D_AUTONOMY_CORE -.->|测试依赖 / test_depends| src_zephyr_gov_drift_autonomy_regressor_py
    ...
    class D_GOV_DOCS,D_SECURITY,D_GOV_AUDIT,D_AUTONOMY_CORE,D_COMPLIANCE external_design
```
...
> 本域与 22 个外部域直接连接（出边 46 条 + 入边 198 条 = 244 条）。只显示直接连接的域，不展开具体节点。
