...
graph TD
    ...
    D_GOV_AUDIT -.->|测试依赖 / test_depends| src_zephyr_gov_drift_canary_controller_py
    ...
    D_GOV_AUDIT -.->|测试依赖 / test_depends| src_zephyr_gov_drift_backcompat_checker_py
    D_SECURITY -.->|导入依赖 / import_depends| src_zephyr_gov_drift_config_consistency_py
    D_INFRA_RECOVERY["(原型态 / prototype) D_INFRA_RECOVERY"]
    D_INFRA_RECOVERY -.->|测试依赖 / test_depends| src_zephyr_gov_drift_canary_controller_py
    ...
    D_GOV_AUDIT -.->|测试依赖 / test_depends| src_zephyr_gov_drift_cascade_detector_py
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_SCRIPTS,D_INFRA_RUNTIME external_prod
    class D_SECURITY,D_GOV_DOCS,D_GOV_AUDIT,D_COMPLIANCE,D_INFRA_A2A,D_INFRA_RECOVERY,D_INTELLIGENCE external_design
```
...
graph TD
    ...
    D_GOVERNANCE -->|导入依赖 / import_depends| src_zephyr_gov_drift_drift_engine_py
    D_GOV_AUDIT -.->|测试依赖 / test_depends| src_zephyr_gov_drift_detector_dispatcher_py
    ...
    D_OPS -->|导入依赖 / import_depends| src_zephyr_gov_drift_drift_infrastructure_py
    D_INFRA_RUNTIME["(生产态 / production) D_INFRA_RUNTIME"]
    D_INFRA_RUNTIME -->|导入依赖 / import_depends| src_zephyr_gov_drift_drift_models_py
    D_GOV_AUDIT -.->|导入依赖 / import_depends| src_zephyr_gov_drift_drift_models_py
    D_INTELLIGENCE["(原型态 / prototype) D_INTELLIGENCE"]
    ...
    class D_SHARED,D_GOVERNANCE,D_OPS,D_INFRA_RUNTIME external_prod
    class D_INTEGRATION,D_GOV_AUDIT,D_GOV_SCRIPTS,D_COMPLIANCE,D_SECURITY,D_INTELLIGENCE external_design
```
...
graph TD
    ...
    class D_GOV_AUDIT,D_SECURITY,D_INFRA_RECOVERY,D_GOVERNANCE,D_INTEGRATION,D_ORCHESTRATOR,D_FEEDBACK_LOOP external_prod
    class D_SHARED,D_KNOWLEDGE,D_COMPLIANCE external_design
```
...
graph TD
    ...
    D_GOV_ENFORCEMENT -.->|测试依赖 / test_depends| src_zephyr_gov_enforcement_rule_enforcement_gate_engine_gate_health_py
    D_GOV_AUDIT -.->|测试依赖 / test_depends| src_zephyr_gov_drift_roi_engine_py
    D_INFRA_RUNTIME["(生产态 / production) D_INFRA_RUNTIME"]
    D_INFRA_RUNTIME -->|导入依赖 / import_depends| src_zephyr_gov_drift_contract_drift_detector_py
    D_INFRA_RUNTIME -->|导入依赖 / import_depends| src_zephyr_gov_audit_self_monitor_py
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_SCRIPTS,D_INFRA_RUNTIME external_prod
    class D_GOV_AUDIT,D_COMPLIANCE,D_SECURITY,D_GOV_ENFORCEMENT external_design
```
...
graph TD
    ...
    class D_GOV_DOCS,D_GOVERNANCE,D_COMPLIANCE,D_GOV_CODE_QUALITY,D_AUTONOMY_CORE external_design
```
...
| 2 | blueprint.md | → | D_GOVERNANCE 生命周期管理: Construction Verifier — 施工验证器: 任务卡完成... | runtime / runtime |
| 3 | blueprint.md | → | D_GOVERNANCE 生命周期管理: dataflowgraph Schema DDL + 连接入口 (dataflowgr... | runtime / runtime |
| 4 | Correlation Engine — correlation_engine.py (co... | → | D_GOVERNANCE 生命周期管理: SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 5 | Coverage Dashboard — dashboard.py (dashboard.py) | → | D_GOVERNANCE 生命周期管理: SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 6 | Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | → | D_GOVERNANCE 生命周期管理: SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 7 | Drift Detector 结果类型 + 专项检测函数 — drift... | → | D_GOVERNANCE 生命周期管理: SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 8 | Gate Persistence — gate_persistence.py (gate_p... | → | D_GOVERNANCE 生命周期管理: SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 9 | Tamper-Proof Audit — 防篡改审计 D-023-37 · §... | → | D_GOVERNANCE 生命周期管理: SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 10 | Trend Analyzer — trend_analyzer.py (trend_anal... | → | D_GOVERNANCE 生命周期管理: SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 11 | Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | → | D_GOV_AUDIT 审计追踪: finding_ingest.py | 导入依赖 / import_depends |
| 12 | Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | → | D_GOV_AUDIT 审计追踪: finding_model.py | 导入依赖 / import_depends |
| 13 | 真源优先级裁决器（Truth Source Validator） (tru... | → | D_GOV_AUDIT 审计追踪: bridge.py | 导入依赖 / import_depends |
| 14 | integrity.py | → | D_GOV_AUDIT 审计追踪: audit-trail.merkle_hourly — MOD-INF-020 · 每.... | 导入依赖 / import_depends |
| 15 | integrity.py | → | D_GOV_AUDIT 审计追踪: models.py | 导入依赖 / import_depends |
| 16 | integrity.py | → | D_GOV_AUDIT 审计追踪: trust_bridge.py | 导入依赖 / import_depends |
| 17 | blueprint.md | → | D_GOV_CODE_QUALITY 代码质量治理: encoding_gate.py — 编码安全校验门禁（治本：弥.... | runtime / runtime |
...
| 24 | blueprint.md | → | D_SECURITY 对抗验证: MonotonicClock — 单调时钟. (monotonic_clock.py) | runtime / runtime |
