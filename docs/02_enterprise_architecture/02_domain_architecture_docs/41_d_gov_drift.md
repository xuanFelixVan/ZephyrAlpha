...
graph TD
    ...
    class D_GOV_DOCS,D_GOV_AUDIT,D_SECURITY,D_INTELLIGENCE,D_INFRA_RECOVERY external_design
```
...
graph TD
    ...
    class D_GOV_AUDIT,D_GOV_SCRIPTS,D_GOVERNANCE,D_GOV_ENFORCEMENT external_prod
    class D_SHARED,D_GOV_RULE,D_INTELLIGENCE external_design
```
...
| 68 | D_GOV_AUDIT 审计追踪: test_audit_integrity.py | → | governance.integrity — 兼容垫片层。 (integrity.py) | 测试依赖 / test_depends |
...
| 121 | D_GOV_DOCS 架构文档治理: blueprint.md | → | blueprint.md | contract / contract |
...
| 134 | D_GOV_RULE 规则治理: gate_engine package — 门禁引擎模块集合（ARCH-0... | → | 门禁健康仪表板——per-gate SLI 报告、误报率、延... | config_depends / config_depends |
...
| 152 | D_KNOWLEDGE 知识管理: test_kb_integrity.py | → | governance.integrity — 兼容垫片层。 (integrity.py) | 测试依赖 / test_depends |
...
> 本域与 22 个外部域直接连接（出边 51 条 + 入边 203 条 = 254 条）。只显示直接连接的域，不展开具体节点。
