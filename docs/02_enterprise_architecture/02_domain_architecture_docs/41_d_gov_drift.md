...
graph TD
    ...
    class D_GOV_DOCS,D_SECURITY,D_INFRASTRUCTURE,D_GOV_AUDIT,D_COMPLIANCE,D_INFRA_RECOVERY external_design
```
...
graph TD
    ...
    class D_GOV_AUDIT,D_SHARED,D_INFRA_RECOVERY,D_GOVERNANCE,D_INTEGRATION,D_GOV_ENFORCEMENT,D_ORCHESTRATOR,D_FEEDBACK_LOOP external_prod
...
> 本域与 22 个外部域直接连接（出边 47 条 + 入边 200 条 = 247 条）。只显示直接连接的域，不展开具体节点。
