...
| 76 | src/zephyr/governance/integrity.py | governance.integrity — 兼容垫片层。 | 生产态 / production | [MOD-INF-020](../../03_modules/_domain_governance/audit_trail/blueprint.md) |
...
graph TD
    ...
    class D_GOV_DOCS,D_INFRA_A2A,D_GOV_AUDIT,D_SECURITY,D_INFRA_RUNTIME,D_COMPLIANCE,D_INTELLIGENCE external_design
```
...
graph TD
    ...
    class D_INTEGRATION,D_GOV_AUDIT,D_INTELLIGENCE external_design
```
...
graph TD
    subgraph D_GOV_DRIFT["D_GOV_DRIFT 漂移检测"]
        ...
        src_zephyr_governance_integrity_py["(生产态 / production) governance.integrity — 兼容垫片层。<br/>文件: integrity.py"]
    end
...
graph TD
    ...
    class D_GOV_AUDIT,D_GOV_SCRIPTS,D_GOVERNANCE,D_INFRA_RUNTIME,D_OPS,D_GOV_ENFORCEMENT external_prod
    class D_SHARED,D_COMPLIANCE,D_INTELLIGENCE external_design
```
...
graph TD
    ...
    class D_GOV_DOCS,D_GOV_AUDIT,D_INFRA_A2A,D_INFRA_RUNTIME external_design
```
...
| 12 | governance.integrity — 兼容垫片层。 (integrity.py) | → | D_GOV_AUDIT 审计追踪: audit-trail.integrity — MOD-INF-020 · 密码学.... | 导入依赖 / import_depends |
| 13 | governance.integrity — 兼容垫片层。 (integrity.py) | → | D_GOV_AUDIT 审计追踪: audit-trail.merkle_hourly — MOD-INF-020 · 每.... | 导入依赖 / import_depends |
| 14 | governance.integrity — 兼容垫片层。 (integrity.py) | → | D_GOV_AUDIT 审计追踪: models.py | 导入依赖 / import_depends |
| 15 | governance.integrity — 兼容垫片层。 (integrity.py) | → | D_GOV_AUDIT 审计追踪: trust_bridge.py | 导入依赖 / import_depends |
...
| 21 | validate_ssot.py | → | D_GOV_SCRIPTS 脚本治理: 文件头部格式解析 SSoT（Single Source of Truth）... | 导入依赖 / import_depends |
| 22 | blueprint.md | → | D_INFRA_A2A A2A通信: test_a2a_check.py | runtime / runtime |
| 23 | Gate-side Drift Detector Recovery — zephyr.gov... | → | D_INFRA_RECOVERY 回滚恢复: drift_fix.py | 导入依赖 / import_depends |
| 24 | Drift Hotfix Bypass — drift_hotfix_bypass.py (... | → | D_INTEGRATION 管线路由: Structural Protocol interfaces for cross-module... | 导入依赖 / import_depends |
| 25 | EN-002 — Enforcement Mode Validator (en_002_en... | → | D_INTEGRATION 管线路由: schemas.py | 导入依赖 / import_depends |
...
| 46 | D_COMPLIANCE 合规: Re-export wrapper: integrity has migrated to ze... | → | governance.integrity — 兼容垫片层。 (integrity.py) | 导入依赖 / import_depends |
| 47 | D_FEEDBACK_LOOP 反馈循环引擎: FLE 全链路调度器 —— collect->detect->diagnose... | → | Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | 导入依赖 / import_depends |
| 48 | D_FEEDBACK_LOOP 反馈循环引擎: FLE 全链路调度器 —— collect->detect->diagnose... | → | governance.integrity — 兼容垫片层。 (integrity.py) | 导入依赖 / import_depends |
...
| 54 | D_GOVERNANCE 生命周期管理: test_integrity_root.py | → | governance.integrity — 兼容垫片层。 (integrity.py) | 测试依赖 / test_depends |
...
| 60 | D_GOV_AUDIT 审计追踪: blueprint.md | → | blueprint.md | runtime / runtime |
| 61 | D_GOV_AUDIT 审计追踪: audit-orchestrator 兼容重导出层（ARCH-042 阶段4... | → | self_monitor.py | 导入依赖 / import_depends |
| 62 | D_GOV_AUDIT 审计追踪: bridge.py | → | drift_bridge.py | 导入依赖 / import_depends |
| 63 | D_GOV_AUDIT 审计追踪: G-CT-007 Audit ↔ Drift 双向桥接 — MOD-INF-020... | → | Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | 导入依赖 / import_depends |
| 64 | D_GOV_AUDIT 审计追踪: G-CT-007 Audit ↔ Drift 双向桥接 — MOD-INF-020... | → | Drift Detector 数据模型 — drift_models.py (dri... | 导入依赖 / import_depends |
| 65 | D_GOV_AUDIT 审计追踪: cli.py | → | Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | 导入依赖 / import_depends |
| 66 | D_GOV_AUDIT 审计追踪: cli.py | → | governance.integrity — 兼容垫片层。 (integrity.py) | 导入依赖 / import_depends |
| 67 | D_GOV_AUDIT 审计追踪: Merkle Audit — 兼容别名，SSoT已迁移至 zephyr.g... | → | governance.integrity — 兼容垫片层。 (integrity.py) | 导入依赖 / import_depends |
| 68 | D_GOV_AUDIT 审计追踪: test_absence_manager.py | → | Owner Absence Manager — Owner缺席模式 §6.32。... | 测试依赖 / test_depends |
| 69 | D_GOV_AUDIT 审计追踪: test_audit_integrity.py | → | governance.integrity — 兼容垫片层。 (integrity.py) | 测试依赖 / test_depends |
...
| 122 | D_GOV_DOCS 架构文档治理: blueprint.md | → | blueprint.md | contract / contract |
...
| 129 | D_GOV_OPS_RESILIENCE 运维弹性治理: PhaseManager->GateEngine 检查注册表桥梁 — 44 .... | → | Drift Chaos Injector — 混沌工程主动漂移注入 §... | 导入依赖 / import_depends |
| 130 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | benchmark_integrity.py | 导入依赖 / import_depends |
| 131 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | model_drift_monitor.py | 导入依赖 / import_depends |
| 132 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | performance_baseline.py | 导入依赖 / import_depends |
| 133 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | regime_detector.py | 导入依赖 / import_depends |
| 134 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | Drift Detector — 兼容别名，SSoT已迁移至 zephyr... | 导入依赖 / import_depends |
| 135 | D_GOV_RULE 规则治理: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | → | Drift Detector 基础设施 — drift_infrastructure... | 导入依赖 / import_depends |
| 136 | D_GOV_RULE 规则治理: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | → | EN-002 — Enforcement Mode Validator (en_002_en... | 导入依赖 / import_depends |
| 137 | D_GOV_SCRIPTS 脚本治理: test_git_bisector.py | → | Git Bisector — git_bisector.py (git_bisector.py) | 测试依赖 / test_depends |
| 138 | D_INFRASTRUCTURE: test_config_consistency.py | → | Config Consistency Checker — 配置多源一致性 D-... | 测试依赖 / test_depends |
| 139 | D_INFRASTRUCTURE: test_contract_drift_detector.py | → | contract_drift_detector — 契约漂移检测器。 (co... | 测试依赖 / test_depends |
| 140 | D_INFRA_A2A A2A通信: test_bridges_drift_bridge.py | → | drift_bridge.py | 测试依赖 / test_depends |
| 141 | D_INFRA_RECOVERY 回滚恢复: test_canary_controller.py | → | Detector Canary Controller — 检测器金丝雀部署 ... | 测试依赖 / test_depends |
| 142 | D_INFRA_RECOVERY 回滚恢复: test_chaos_injector.py | → | Drift Chaos Injector — 混沌工程主动漂移注入 §... | 测试依赖 / test_depends |
| 143 | D_INFRA_RECOVERY 回滚恢复: test_rollback_bridge.py | → | G-CT-006 契约：Drift -> Rollback 漂移触发回滚. ... | 测试依赖 / test_depends |
| 144 | D_INFRA_RUNTIME 运行时集成: 遥测 · ai_behavior/event_sink — AI 行为遥测事... | → | blueprint.md | runtime / runtime |
...
| 153 | D_KNOWLEDGE 知识管理: test_kb_integrity.py | → | governance.integrity — 兼容垫片层。 (integrity.py) | 测试依赖 / test_depends |
...
> 本域与 22 个外部域直接连接（出边 52 条 + 入边 204 条 = 256 条）。只显示直接连接的域，不展开具体节点。
