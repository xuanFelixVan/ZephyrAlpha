...
graph TD
    ...
    class D_GOV_AUDIT,D_INTEGRATION,D_SECURITY,D_COMPLIANCE external_design
```
...
graph TD
    ...
    class D_COMPLIANCE,D_GOV_RULE external_design
```
...
graph TD
    ...
    class D_GOV_AUDIT,D_SECURITY,D_GOV_RULE,D_COMPLIANCE external_design
```
...
> 本域与 15 个外部域直接连接（出边 48 条 + 入边 114 条 = 162 条）。只显示直接连接的域，不展开具体节点。
