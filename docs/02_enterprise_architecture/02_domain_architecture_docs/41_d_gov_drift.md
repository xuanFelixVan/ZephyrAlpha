...
graph TD
    ...
    src_zephyr_gov_drift_init_py -.->|config_depends / config_depends| src_zephyr_gov_drift_artifact_scanner_py
    ...
    class D_GOV_DOCS,D_SECURITY,D_GOV_AUDIT,D_COMPLIANCE,D_INTELLIGENCE external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_INFRA_RUNTIME external_prod
    class D_INTEGRATION,D_GOV_AUDIT,D_SECURITY,D_COMPLIANCE,D_GOV_ENFORCEMENT external_design
```
...
> 本域与 21 个外部域直接连接（出边 46 条 + 入边 198 条 = 244 条）。只显示直接连接的域，不展开具体节点。
