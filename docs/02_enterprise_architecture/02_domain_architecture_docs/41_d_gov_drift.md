...
| 跨域出边 | 47 | Cross-domain Outgoing | 47 |
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_INFRA_RUNTIME,D_OPS,D_GOV_RULE external_prod
...
graph TD
    ...
    class D_GOVERNANCE,D_SHARED,D_GOV_SCRIPTS,D_INFRA_RUNTIME,D_OPS,D_GOV_RULE external_prod
...
graph TD
    ...
    class D_GOV_AUDIT,D_SHARED,D_SECURITY,D_INFRA_RECOVERY,D_FEEDBACK_LOOP,D_ORCHESTRATOR external_prod
```
...
| 20 | Gate-side Drift Detector Recovery — zephyr.gov... | → | D_INFRA_RECOVERY 回滚恢复: drift_fix.py | 导入依赖 / import_depends |
| 21 | Drift Hotfix Bypass — drift_hotfix_bypass.py (... | → | D_INTEGRATION 管线路由: Structural Protocol interfaces for cross-module... | 导入依赖 / import_depends |
| 22 | ProbeHierarchy - K8s 3-Probe + Terraform Reconc... | → | D_SECURITY 对抗验证: Cold Start Bootstrapper — 冷启动引导 §6.31。 ... | 导入依赖 / import_depends |
...
| 33 | DriftBridge — 漂移检测器事件桥接 (MOD-INF-023)... | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 导入依赖 / import_depends |
...
| 46 | EN-002 — Enforcement Mode Validator (en_002_en... | → | D_SHARED 共享服务: schemas.py | 导入依赖 / import_depends |
| 47 | 真源优先级裁决器（Truth Source Validator） (tru... | → | D_SHARED 共享服务: schemas.py | 导入依赖 / import_depends |
...
> 本域与 15 个外部域直接连接（出边 47 条 + 入边 114 条 = 161 条）。只显示直接连接的域，不展开具体节点。
