...
graph TD
    ...
    class D_GOV_DOCS,D_SECURITY,D_COMPLIANCE,D_INFRA_A2A,D_GOV_AUDIT,D_INTELLIGENCE,D_INFRA_RECOVERY external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_OPS external_prod
    class D_INTEGRATION,D_GOV_AUDIT,D_SECURITY,D_COMPLIANCE,D_INTELLIGENCE external_design
```
...
graph TD
    ...
    class D_GOVERNANCE,D_SHARED,D_GOV_SCRIPTS,D_OPS external_prod
...
| 130 | D_GOV_RULE 规则治理: gate_engine package — 门禁引擎模块集合（ARCH-0... | → | 门禁健康仪表板——per-gate SLI 报告、误报率、延... | config_depends / config_depends |
