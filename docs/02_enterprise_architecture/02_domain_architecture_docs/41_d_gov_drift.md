...
graph TD
    ...
    D_INFRA_RUNTIME -.->|runtime / runtime| docs_03_modules_domain_governance_drift_detector_blueprint_md
    ...
    D_INFRA_RUNTIME -->|导入依赖 / import_depends| src_zephyr_gov_audit_self_monitor_py
    ...
    D_INTELLIGENCE -.->|测试依赖 / test_depends| src_zephyr_gov_drift_ai_context_injector_py
    ...
    class D_FBL_VERIFICATION,D_SECURITY,D_SHARED external_prod
    class D_GOVERNANCE,D_GOV_DOCS,D_COMPLIANCE,D_AUTONOMY_CORE,D_INFRA_RUNTIME,D_INTELLIGENCE,D_GOV_AUDIT external_design
```
...
graph TD
    ...
    D_GOV_SCRIPTS -.->|测试依赖 / test_depends| src_zephyr_gov_drift_git_bisector_py
    D_GOV_AUDIT -.->|测试依赖 / test_depends| src_zephyr_gov_drift_detector_core_regime_detector_py
    D_GOV_AUDIT -.->|测试依赖 / test_depends| src_zephyr_gov_drift_roi_engine_py
    D_GOV_AUDIT -.->|测试依赖 / test_depends| src_zephyr_gov_drift_headless_scanner_py
    D_INTELLIGENCE["(原型态 / prototype) D_INTELLIGENCE"]
    D_INTELLIGENCE -.->|测试依赖 / test_depends| src_zephyr_gov_drift_drift_models_py
    D_GOV_AUDIT -.->|测试依赖 / test_depends| src_zephyr_gov_drift_scan_mutex_py
    D_GOV_ENFORCEMENT["(原型态 / prototype) D_GOV_ENFORCEMENT"]
    D_GOV_ENFORCEMENT -.->|测试依赖 / test_depends| src_zephyr_gov_drift_gate_persistence_py
    D_GOVERNANCE -->|导入依赖 / import_depends| src_zephyr_gov_drift_drift_models_py
    D_GOV_REPAIR["(生产态 / production) D_GOV_REPAIR"]
    D_GOV_REPAIR -->|导入依赖 / import_depends| src_zephyr_gov_drift_detector_core_regime_detector_py
    D_GOV_AUDIT -.->|测试依赖 / test_depends| src_zephyr_gov_drift_drift_training_py
    D_SECURITY["(原型态 / prototype) D_SECURITY"]
    D_SECURITY -.->|导入依赖 / import_depends| src_zephyr_gov_drift_forensics_engine_py
    ...
    class D_INTEGRATION,D_GOV_AUDIT,D_GOV_SCRIPTS,D_INTELLIGENCE,D_GOV_ENFORCEMENT,D_SECURITY,D_COMPLIANCE external_design
```
...
graph TD
    ...
    D_INTELLIGENCE["(原型态 / prototype) D_INTELLIGENCE"]
    D_INTELLIGENCE -.->|测试依赖 / test_depends| src_zephyr_gov_drift_spiral_ews_py
    ...
    class D_GOV_AUDIT,D_SECURITY,D_INFRA_RECOVERY,D_GOVERNANCE,D_INTEGRATION,D_ORCHESTRATOR,D_FEEDBACK_LOOP,D_OPS external_prod
    class D_SHARED,D_KNOWLEDGE,D_COMPLIANCE,D_INTELLIGENCE external_design
```
...
graph TD
    ...
    D_GOV_AUDIT -.->|测试依赖 / test_depends| src_zephyr_gov_drift_roi_engine_py
    D_GOV_AUDIT -.->|测试依赖 / test_depends| src_zephyr_gov_drift_headless_scanner_py
    D_GOVERNANCE -.->|测试依赖 / test_depends| src_zephyr_gov_enforcement_rule_enforcement_breaking_change_detector_py
    D_GOV_AUDIT -.->|测试依赖 / test_depends| src_zephyr_gov_drift_brain_integration_py
    ...
    class D_GOVERNANCE,D_GOV_SCRIPTS,D_INFRA_RUNTIME external_prod
    class D_SHARED,D_GOV_AUDIT,D_SECURITY,D_COMPLIANCE,D_INTELLIGENCE,D_GOV_ENFORCEMENT external_design
```
...
graph TD
    ...
    class D_GOV_DOCS,D_GOVERNANCE,D_COMPLIANCE,D_AUTONOMY_CORE,D_INFRA_RUNTIME external_design
```
...
| 2 | blueprint.md | → | D_FBL_VERIFICATION 反馈验证: Safety Gates L50-L55 — Coherence + Integrity L... | runtime / runtime |
...
| 21 | Gate-side Drift Detector Recovery — zephyr.gov... | → | D_INFRA_RECOVERY 回滚恢复: drift_fix.py | 导入依赖 / import_depends |
| 22 | Drift Hotfix Bypass — drift_hotfix_bypass.py (... | → | D_INTEGRATION 管线路由: Structural Protocol interfaces for cross-module... | 导入依赖 / import_depends |
| 23 | EN-002 — Enforcement Mode Validator (en_002_en... | → | D_INTEGRATION 管线路由: schemas.py | 导入依赖 / import_depends |
...
| 2 | D_AUTONOMY_CORE 自治核心: test_autonomy_regressor.py | → | Autonomy Regressor — v0.10.0 渐进自治可逆性管.... | 测试依赖 / test_depends |
| 3 | D_COMPLIANCE 合规: Re-export wrapper: artifact_scanner has migrate... | → | ArtifactScanner — SSRF / Path Traversal / Cred... | 导入依赖 / import_depends |
| 4 | D_COMPLIANCE 合规: Audit Trail — MOD-INF-020 (__init__.py) | → | blueprint.md | runtime / runtime |
| 5 | D_COMPLIANCE 合规: __init__.py | → | Owner Absence Manager — Owner缺席模式 §6.32。... | 导入依赖 / import_depends |
| 6 | D_COMPLIANCE 合规: __init__.py | → | Drift Detector AI 施工检测器 — ai_construction... | 导入依赖 / import_depends |
| 7 | D_COMPLIANCE 合规: __init__.py | → | AI Context Injector — 施工前预检D-023-16 · §... | 导入依赖 / import_depends |
| 8 | D_COMPLIANCE 合规: __init__.py | → | Backward Compatibility Checker — 向后兼容策略.... | 导入依赖 / import_depends |
| 9 | D_COMPLIANCE 合规: __init__.py | → | Baseline Manager — baseline_manager.py (baseli... | 导入依赖 / import_depends |
| 10 | D_COMPLIANCE 合规: __init__.py | → | Baseline Poisoning Guard — 基线投毒防护 D-023-... | 导入依赖 / import_depends |
| 11 | D_COMPLIANCE 合规: __init__.py | → | Detector Canary Controller — 检测器金丝雀部署 ... | 导入依赖 / import_depends |
| 12 | D_COMPLIANCE 合规: __init__.py | → | Cascade Failure Detector — 级联故障检测 D-023-... | 导入依赖 / import_depends |
| 13 | D_COMPLIANCE 合规: __init__.py | → | Drift Chaos Injector — 混沌工程主动漂移注入 §... | 导入依赖 / import_depends |
| 14 | D_COMPLIANCE 合规: __init__.py | → | Config Consistency Checker — 配置多源一致性 D-... | 导入依赖 / import_depends |
| 15 | D_COMPLIANCE 合规: __init__.py | → | contract_drift_detector — 契约漂移检测器。 (co... | 导入依赖 / import_depends |
| 16 | D_COMPLIANCE 合规: __init__.py | → | Correlation Engine — correlation_engine.py (co... | 导入依赖 / import_depends |
| 17 | D_COMPLIANCE 合规: __init__.py | → | Credibility Engine — credibility_engine.py (cr... | 导入依赖 / import_depends |
| 18 | D_COMPLIANCE 合规: __init__.py | → | Cross Module Score — cross_module_score.py (cr... | 导入依赖 / import_depends |
| 19 | D_COMPLIANCE 合规: __init__.py | → | Coverage Dashboard — dashboard.py (dashboard.py) | 导入依赖 / import_depends |
| 20 | D_COMPLIANCE 合规: __init__.py | → | Detector Dispatcher — detector_dispatcher.py (... | 导入依赖 / import_depends |
| 21 | D_COMPLIANCE 合规: __init__.py | → | Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | 导入依赖 / import_depends |
| 22 | D_COMPLIANCE 合规: __init__.py | → | Drift Hotfix Bypass — drift_hotfix_bypass.py (... | 导入依赖 / import_depends |
| 23 | D_COMPLIANCE 合规: __init__.py | → | Drift Detector 基础设施 — drift_infrastructure... | 导入依赖 / import_depends |
| 24 | D_COMPLIANCE 合规: __init__.py | → | Drift Detector 数据模型 — drift_models.py (dri... | 导入依赖 / import_depends |
| 25 | D_COMPLIANCE 合规: __init__.py | → | Drift Detector 结果类型 + 专项检测函数 — drift... | 导入依赖 / import_depends |
| 26 | D_COMPLIANCE 合规: __init__.py | → | Drift Detector AI 训练闭环 + 跨语言检测 — drif... | 导入依赖 / import_depends |
| 27 | D_COMPLIANCE 合规: __init__.py | → | File Attribute Integrity — 文件底层属性完整性 ... | 导入依赖 / import_depends |
| 28 | D_COMPLIANCE 合规: __init__.py | → | Drift Forensics Engine — 漂移取证引擎 §6.17。... | 导入依赖 / import_depends |
| 29 | D_COMPLIANCE 合规: __init__.py | → | Gate Persistence — gate_persistence.py (gate_p... | 导入依赖 / import_depends |
| 30 | D_COMPLIANCE 合规: __init__.py | → | Git Bisector — git_bisector.py (git_bisector.py) | 导入依赖 / import_depends |
| 31 | D_COMPLIANCE 合规: __init__.py | → | .gitignore Integrity Auditor — gitignore完整性... | 导入依赖 / import_depends |
| 32 | D_COMPLIANCE 合规: __init__.py | → | Cross-Session Handoff Manager — 跨Session修复.... | 导入依赖 / import_depends |
| 33 | D_COMPLIANCE 合规: __init__.py | → | Headless Scanner — headless_scanner.py (headle... | 导入依赖 / import_depends |
| 34 | D_COMPLIANCE 合规: __init__.py | → | Incremental Scanner — incremental_scanner.py (... | 导入依赖 / import_depends |
| 35 | D_COMPLIANCE 合规: __init__.py | → | Naming Magic Checker — 命名魔数与隐式约定检测 ... | 导入依赖 / import_depends |
| 36 | D_COMPLIANCE 合规: __init__.py | → | Orphan Resource Scanner — 孤儿资源检测 §6.28... | 导入依赖 / import_depends |
| 37 | D_COMPLIANCE 合规: __init__.py | → | Python Compatibility Checker — Python版本兼容.... | 导入依赖 / import_depends |
| 38 | D_COMPLIANCE 合规: __init__.py | → | Resource Guard — 资源上限与优雅降级 D-023-23 .... | 导入依赖 / import_depends |
| 39 | D_COMPLIANCE 合规: __init__.py | → | ROI Engine — roi_engine.py (roi_engine.py) | 导入依赖 / import_depends |
| 40 | D_COMPLIANCE 合规: __init__.py | → | G-CT-006 契约：Drift -> Rollback 漂移触发回滚. ... | 导入依赖 / import_depends |
| 41 | D_COMPLIANCE 合规: __init__.py | → | Scan Mutex — scan_mutex.py (scan_mutex.py) | 导入依赖 / import_depends |
| 42 | D_COMPLIANCE 合规: __init__.py | → | Self-Drift Check — self_check.py (self_check.py) | 导入依赖 / import_depends |
| 43 | D_COMPLIANCE 合规: __init__.py | → | Suppression Learner — suppression_learner.py (... | 导入依赖 / import_depends |
| 44 | D_COMPLIANCE 合规: __init__.py | → | Symlink Integrity Checker — 软链接完整性检测 .... | 导入依赖 / import_depends |
| 45 | D_COMPLIANCE 合规: __init__.py | → | Tamper-Proof Audit — 防篡改审计 D-023-37 · §... | 导入依赖 / import_depends |
| 46 | D_COMPLIANCE 合规: __init__.py | → | Test Fixture Checker — 测试夹具漂移检测 D-023-... | 导入依赖 / import_depends |
| 47 | D_COMPLIANCE 合规: __init__.py | → | Trend Analyzer — trend_analyzer.py (trend_anal... | 导入依赖 / import_depends |
| 48 | D_COMPLIANCE 合规: Re-export wrapper: integrity has migrated to ze... | → | integrity.py | 导入依赖 / import_depends |
| 49 | D_FEEDBACK_LOOP 反馈循环引擎: FLE 全链路调度器 —— collect->detect->diagnose... | → | Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | 导入依赖 / import_depends |
| 50 | D_FEEDBACK_LOOP 反馈循环引擎: FLE 全链路调度器 —— collect->detect->diagnose... | → | integrity.py | 导入依赖 / import_depends |
| 51 | D_GOVERNANCE 生命周期管理: model_provider_data.py | → | blueprint.md | contract / contract |
| 52 | D_GOVERNANCE 生命周期管理: GovernanceServer: 治理域统一MCP入口 (governance... | → | Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | 导入依赖 / import_depends |
| 53 | D_GOVERNANCE 生命周期管理: GovernanceServer: 治理域统一MCP入口 (governance... | → | Drift Detector 基础设施 — drift_infrastructure... | 导入依赖 / import_depends |
| 54 | D_GOVERNANCE 生命周期管理: GovernanceServer: 治理域统一MCP入口 (governance... | → | Drift Detector 数据模型 — drift_models.py (dri... | 导入依赖 / import_depends |
| 55 | D_GOVERNANCE 生命周期管理: test_reward_hacking_rebound_detector.py | → | Reward Hacking Rebound Detector — v0.14.0 §2.... | 测试依赖 / test_depends |
| 56 | D_GOVERNANCE 生命周期管理: test_vigil_runtime.py | → | Vigil Runtime — v0.6.0 VIGIL维护运行时: 运维to... | 测试依赖 / test_depends |
| 57 | D_GOVERNANCE 生命周期管理: test_bootstrapping_calibrator.py | → | bootstrapping_calibrator.py | 测试依赖 / test_depends |
| 58 | D_GOVERNANCE 生命周期管理: test_silence_detector.py | → | Silence Detector — v0.8.0 静默窗口检测器: agen... | 测试依赖 / test_depends |
| 59 | D_GOVERNANCE 生命周期管理: test_spiral_ews.py | → | spiral_ews.py | 测试依赖 / test_depends |
| 60 | D_GOVERNANCE 生命周期管理: test_en_002_enforcement_validator.py | → | EN-002 — Enforcement Mode Validator (en_002_en... | 测试依赖 / test_depends |
| 61 | D_GOVERNANCE 生命周期管理: test_breaking_change_detector.py | → | Breaking Change 检测器（GATE-CDC-2）——字段删.... | 测试依赖 / test_depends |
| 62 | D_GOV_AUDIT 审计追踪: audit-orchestrator 兼容重导出层（ARCH-042 阶段4... | → | self_monitor.py | 导入依赖 / import_depends |
| 63 | D_GOV_AUDIT 审计追踪: bridge.py | → | drift_bridge.py | 导入依赖 / import_depends |
| 64 | D_GOV_AUDIT 审计追踪: G-CT-007 Audit ↔ Drift 双向桥接 — MOD-INF-020... | → | Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | 导入依赖 / import_depends |
| 65 | D_GOV_AUDIT 审计追踪: G-CT-007 Audit ↔ Drift 双向桥接 — MOD-INF-020... | → | Drift Detector 数据模型 — drift_models.py (dri... | 导入依赖 / import_depends |
| 66 | D_GOV_AUDIT 审计追踪: cli.py | → | Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | 导入依赖 / import_depends |
| 67 | D_GOV_AUDIT 审计追踪: cli.py | → | integrity.py | 导入依赖 / import_depends |
| 68 | D_GOV_AUDIT 审计追踪: Merkle Audit — 兼容别名，SSoT已迁移至 zephyr.g... | → | integrity.py | 导入依赖 / import_depends |
| 69 | D_GOV_AUDIT 审计追踪: test_absence_manager.py | → | Owner Absence Manager — Owner缺席模式 §6.32。... | 测试依赖 / test_depends |
| 70 | D_GOV_AUDIT 审计追踪: test_backcompat_checker.py | → | Backward Compatibility Checker — 向后兼容策略.... | 测试依赖 / test_depends |
| 71 | D_GOV_AUDIT 审计追踪: test_baseline_manager.py | → | Baseline Manager — baseline_manager.py (baseli... | 测试依赖 / test_depends |
| 72 | D_GOV_AUDIT 审计追踪: test_baseline_poisoning_guard.py | → | Baseline Poisoning Guard — 基线投毒防护 D-023-... | 测试依赖 / test_depends |
| 73 | D_GOV_AUDIT 审计追踪: test_benchmark_integrity.py | → | benchmark_integrity.py | 测试依赖 / test_depends |
| 74 | D_GOV_AUDIT 审计追踪: test_brain_integration_root.py | → | ProbeHierarchy - K8s 3-Probe + Terraform Reconc... | 测试依赖 / test_depends |
| 75 | D_GOV_AUDIT 审计追踪: test_cascade_detector.py | → | Cascade Failure Detector — 级联故障检测 D-023-... | 测试依赖 / test_depends |
| 76 | D_GOV_AUDIT 审计追踪: test_correlation_engine.py | → | Correlation Engine — correlation_engine.py (co... | 测试依赖 / test_depends |
| 77 | D_GOV_AUDIT 审计追踪: test_credibility_engine.py | → | Credibility Engine — credibility_engine.py (cr... | 测试依赖 / test_depends |
| 78 | D_GOV_AUDIT 审计追踪: test_detector_dispatcher.py | → | Detector Dispatcher — detector_dispatcher.py (... | 测试依赖 / test_depends |
| 79 | D_GOV_AUDIT 审计追踪: test_detector_dispatcher.py | → | Drift Detector 数据模型 — drift_models.py (dri... | 测试依赖 / test_depends |
| 80 | D_GOV_AUDIT 审计追踪: test_forensics_engine.py | → | Drift Forensics Engine — 漂移取证引擎 §6.17。... | 测试依赖 / test_depends |
| 81 | D_GOV_AUDIT 审计追踪: test_gitignore_auditor.py | → | .gitignore Integrity Auditor — gitignore完整性... | 测试依赖 / test_depends |
| 82 | D_GOV_AUDIT 审计追踪: test_handoff_manager.py | → | Cross-Session Handoff Manager — 跨Session修复.... | 测试依赖 / test_depends |
| 83 | D_GOV_AUDIT 审计追踪: test_headless_scanner.py | → | Drift Detector 数据模型 — drift_models.py (dri... | 测试依赖 / test_depends |
| 84 | D_GOV_AUDIT 审计追踪: test_headless_scanner.py | → | Headless Scanner — headless_scanner.py (headle... | 测试依赖 / test_depends |
| 85 | D_GOV_AUDIT 审计追踪: test_incremental_scanner.py | → | Incremental Scanner — incremental_scanner.py (... | 测试依赖 / test_depends |
| 86 | D_GOV_AUDIT 审计追踪: test_ml_engineering.py | → | ml_engineering.py | 测试依赖 / test_depends |
| 87 | D_GOV_AUDIT 审计追踪: test_naming_magic_checker.py | → | Naming Magic Checker — 命名魔数与隐式约定检测 ... | 测试依赖 / test_depends |
| 88 | D_GOV_AUDIT 审计追踪: test_orphan_scanner.py | → | Orphan Resource Scanner — 孤儿资源检测 §6.28... | 测试依赖 / test_depends |
| 89 | D_GOV_AUDIT 审计追踪: test_performance_baseline.py | → | performance_baseline.py | 测试依赖 / test_depends |
| 90 | D_GOV_AUDIT 审计追踪: test_python_compat.py | → | Python Compatibility Checker — Python版本兼容.... | 测试依赖 / test_depends |
| 91 | D_GOV_AUDIT 审计追踪: test_regime_detector.py | → | regime_detector.py | 测试依赖 / test_depends |
| 92 | D_GOV_AUDIT 审计追踪: test_roi_engine.py | → | ROI Engine — roi_engine.py (roi_engine.py) | 测试依赖 / test_depends |
| 93 | D_GOV_AUDIT 审计追踪: test_scan_mutex.py | → | Drift Detector 数据模型 — drift_models.py (dri... | 测试依赖 / test_depends |
| 94 | D_GOV_AUDIT 审计追踪: test_scan_mutex.py | → | Scan Mutex — scan_mutex.py (scan_mutex.py) | 测试依赖 / test_depends |
| 95 | D_GOV_AUDIT 审计追踪: test_state_machine.py | → | Drift Detector 数据模型 — drift_models.py (dri... | 测试依赖 / test_depends |
| 96 | D_GOV_AUDIT 审计追踪: test_suppression_learner.py | → | Suppression Learner — suppression_learner.py (... | 测试依赖 / test_depends |
| 97 | D_GOV_AUDIT 审计追踪: test_symlink_checker.py | → | Symlink Integrity Checker — 软链接完整性检测 .... | 测试依赖 / test_depends |
| 98 | D_GOV_AUDIT 审计追踪: test_tamper_proof_audit.py | → | Tamper-Proof Audit — 防篡改审计 D-023-37 · §... | 测试依赖 / test_depends |
| 99 | D_GOV_AUDIT 审计追踪: test_test_fixture_checker.py | → | Test Fixture Checker — 测试夹具漂移检测 D-023-... | 测试依赖 / test_depends |
| 100 | D_GOV_AUDIT 审计追踪: test_trend_analyzer.py | → | Trend Analyzer — trend_analyzer.py (trend_anal... | 测试依赖 / test_depends |
| 101 | D_GOV_AUDIT 审计追踪: test_ba_canary_controller.py | → | Detector Canary Controller — 检测器金丝雀部署 ... | 测试依赖 / test_depends |
| 102 | D_GOV_AUDIT 审计追踪: test_ba_chaos_injector.py | → | Drift Chaos Injector — 混沌工程主动漂移注入 §... | 测试依赖 / test_depends |
| 103 | D_GOV_AUDIT 审计追踪: test_ba_dashboard.py | → | Coverage Dashboard — dashboard.py (dashboard.py) | 测试依赖 / test_depends |
| 104 | D_GOV_AUDIT 审计追踪: test_ba_handoff_manager.py | → | Cross-Session Handoff Manager — 跨Session修复.... | 测试依赖 / test_depends |
| 105 | D_GOV_AUDIT 审计追踪: test_ba_state_machine.py | → | Drift Detector 数据模型 — drift_models.py (dri... | 测试依赖 / test_depends |
| 106 | D_GOV_AUDIT 审计追踪: test_drift_bridge.py | → | drift_bridge.py | 测试依赖 / test_depends |
| 107 | D_GOV_AUDIT 审计追踪: test_drift_detector_ee.py | → | Drift Detector — 兼容别名，SSoT已迁移至 zephyr... | 测试依赖 / test_depends |
| 108 | D_GOV_AUDIT 审计追踪: test_drift_detector_gate.py | → | Drift Detector — 兼容别名，SSoT已迁移至 zephyr... | 测试依赖 / test_depends |
| 109 | D_GOV_AUDIT 审计追踪: test_drift_engine.py | → | Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | 测试依赖 / test_depends |
| 110 | D_GOV_AUDIT 审计追踪: test_drift_engine.py | → | Drift Detector 数据模型 — drift_models.py (dri... | 测试依赖 / test_depends |
| 111 | D_GOV_AUDIT 审计追踪: test_drift_hotfix_bypass.py | → | Drift Hotfix Bypass — drift_hotfix_bypass.py (... | 测试依赖 / test_depends |
| 112 | D_GOV_AUDIT 审计追踪: test_drift_infrastructure.py | → | Drift Detector 基础设施 — drift_infrastructure... | 测试依赖 / test_depends |
| 113 | D_GOV_AUDIT 审计追踪: test_drift_models.py | → | Drift Detector 数据模型 — drift_models.py (dri... | 测试依赖 / test_depends |
| 114 | D_GOV_AUDIT 审计追踪: test_drift_result_types.py | → | Drift Detector 结果类型 + 专项检测函数 — drift... | 测试依赖 / test_depends |
| 115 | D_GOV_AUDIT 审计追踪: test_drift_training.py | → | Drift Detector AI 训练闭环 + 跨语言检测 — drif... | 测试依赖 / test_depends |
| 116 | D_GOV_AUDIT 审计追踪: test_self_check.py | → | Self-Drift Check — self_check.py (self_check.py) | 测试依赖 / test_depends |
| 117 | D_GOV_AUDIT 审计追踪: test_self_monitor.py | → | self_monitor.py | 测试依赖 / test_depends |
| 118 | D_GOV_AUDIT 审计追踪: test_self_test_verifier.py | → | Self Test Verifier — self_test_verifier.py (se... | 测试依赖 / test_depends |
...
| 121 | D_GOV_ENFORCEMENT 规则执行: ZephyrAlpha 门禁子包 (__init__.py) | → | Breaking Change 检测器（GATE-CDC-2）——字段删.... | 导入依赖 / import_depends |
| 122 | D_GOV_ENFORCEMENT 规则执行: ZephyrAlpha 门禁子包 (__init__.py) | → | 门禁健康仪表板——per-gate SLI 报告、误报率、延... | 导入依赖 / import_depends |
| 123 | D_GOV_ENFORCEMENT 规则执行: ZephyrAlpha 门禁子包 (__init__.py) | → | 门禁引擎完整性守卫——自检SHA-256校验+trust roo... | 导入依赖 / import_depends |
| 124 | D_GOV_ENFORCEMENT 规则执行: test_gate_health.py | → | 门禁健康仪表板——per-gate SLI 报告、误报率、延... | 测试依赖 / test_depends |
| 125 | D_GOV_ENFORCEMENT 规则执行: test_gate_integrity_guard.py | → | 门禁引擎完整性守卫——自检SHA-256校验+trust roo... | 测试依赖 / test_depends |
| 126 | D_GOV_ENFORCEMENT 规则执行: test_gate_persistence.py | → | Gate Persistence — gate_persistence.py (gate_p... | 测试依赖 / test_depends |
| 127 | D_GOV_OPS_RESILIENCE 运维弹性治理: PhaseManager->GateEngine 检查注册表桥梁 — 44 .... | → | Drift Chaos Injector — 混沌工程主动漂移注入 §... | 导入依赖 / import_depends |
| 128 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | benchmark_integrity.py | 导入依赖 / import_depends |
| 129 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | model_drift_monitor.py | 导入依赖 / import_depends |
| 130 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | performance_baseline.py | 导入依赖 / import_depends |
| 131 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | regime_detector.py | 导入依赖 / import_depends |
| 132 | D_GOV_REPAIR 治理修复: Agent 治理八件套 · Governance Domain — DOM-GO... | → | Drift Detector — 兼容别名，SSoT已迁移至 zephyr... | 导入依赖 / import_depends |
| 133 | D_GOV_RULE 规则治理: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | → | Drift Detector 基础设施 — drift_infrastructure... | 导入依赖 / import_depends |
| 134 | D_GOV_RULE 规则治理: GateEngine — KMS G1-G6 + Orc G0/G7 + 交易 G10-... | → | EN-002 — Enforcement Mode Validator (en_002_en... | 导入依赖 / import_depends |
| 135 | D_GOV_SCRIPTS 脚本治理: test_git_bisector.py | → | Git Bisector — git_bisector.py (git_bisector.py) | 测试依赖 / test_depends |
| 136 | D_INFRASTRUCTURE: test_config_consistency.py | → | Config Consistency Checker — 配置多源一致性 D-... | 测试依赖 / test_depends |
| 137 | D_INFRASTRUCTURE: test_contract_drift_detector.py | → | contract_drift_detector — 契约漂移检测器。 (co... | 测试依赖 / test_depends |
| 138 | D_INFRA_A2A A2A通信: test_bridges_drift_bridge.py | → | drift_bridge.py | 测试依赖 / test_depends |
| 139 | D_INFRA_RECOVERY 回滚恢复: test_canary_controller.py | → | Detector Canary Controller — 检测器金丝雀部署 ... | 测试依赖 / test_depends |
| 140 | D_INFRA_RECOVERY 回滚恢复: test_chaos_injector.py | → | Drift Chaos Injector — 混沌工程主动漂移注入 §... | 测试依赖 / test_depends |
| 141 | D_INFRA_RECOVERY 回滚恢复: test_rollback_bridge.py | → | G-CT-006 契约：Drift -> Rollback 漂移触发回滚. ... | 测试依赖 / test_depends |
| 142 | D_INFRA_RUNTIME 运行时集成: state_machine.py | → | Drift Detector 数据模型 — drift_models.py (dri... | 导入依赖 / import_depends |
| 143 | D_INFRA_RUNTIME 运行时集成: ZephyrAlpha — system-telemetry/contract_metric... | → | contract_drift_detector — 契约漂移检测器。 (co... | 导入依赖 / import_depends |
| 144 | D_INFRA_RUNTIME 运行时集成: 遥测 · traces/span_stub — W3C TraceContext 分... | → | blueprint.md | runtime / runtime |
...
| 152 | D_KNOWLEDGE 知识管理: test_kb_integrity.py | → | integrity.py | 测试依赖 / test_depends |
| 153 | D_OPS 反馈循环: Budget Enforcer core engine — MOD-INF-024 (bud... | → | Drift Detector 基础设施 — drift_infrastructure... | 导入依赖 / import_depends |
| 154 | D_OPS 反馈循环: Budget Enforcer core engine — MOD-INF-024 (bud... | → | spiral_ews.py | 导入依赖 / import_depends |
| 155 | D_ORCHESTRATOR 代理编排器: TriggerRouter — RI-03 触发路由器（M3 跨模块触.... | → | Gate-side Drift Detector Recovery — zephyr.gov... | 导入依赖 / import_depends |
| 156 | D_SECURITY 对抗验证: Drift Detector MOD-INF-023 CLI — 漂移扫描入口... | → | Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | 导入依赖 / import_depends |
| 157 | D_SECURITY 对抗验证: Drift Detector MOD-INF-023 CLI — 漂移扫描入口... | → | Drift Detector 基础设施 — drift_infrastructure... | 导入依赖 / import_depends |
| 158 | D_SECURITY 对抗验证: Drift Detector MOD-INF-023 CLI — 漂移扫描入口... | → | Self-Drift Check — self_check.py (self_check.py) | 导入依赖 / import_depends |
| 159 | D_SECURITY 对抗验证: Drift Detector MOD-INF-023 CLI — 漂移扫描入口... | → | Self Test Verifier — self_test_verifier.py (se... | 导入依赖 / import_depends |
| 160 | D_SECURITY 对抗验证: _analysis.py | → | Correlation Engine — correlation_engine.py (co... | 导入依赖 / import_depends |
| 161 | D_SECURITY 对抗验证: _analysis.py | → | Credibility Engine — credibility_engine.py (cr... | 导入依赖 / import_depends |
| 162 | D_SECURITY 对抗验证: _analysis.py | → | Cross Module Score — cross_module_score.py (cr... | 导入依赖 / import_depends |
| 163 | D_SECURITY 对抗验证: _analysis.py | → | Drift Forensics Engine — 漂移取证引擎 §6.17。... | 导入依赖 / import_depends |
| 164 | D_SECURITY 对抗验证: _analysis.py | → | Git Bisector — git_bisector.py (git_bisector.py) | 导入依赖 / import_depends |
| 165 | D_SECURITY 对抗验证: _analysis.py | → | ROI Engine — roi_engine.py (roi_engine.py) | 导入依赖 / import_depends |
| 166 | D_SECURITY 对抗验证: _analysis.py | → | G-CT-006 契约：Drift -> Rollback 漂移触发回滚. ... | 导入依赖 / import_depends |
| 167 | D_SECURITY 对抗验证: _analysis.py | → | Self-Drift Check — self_check.py (self_check.py) | 导入依赖 / import_depends |
| 168 | D_SECURITY 对抗验证: _analysis.py | → | Suppression Learner — suppression_learner.py (... | 导入依赖 / import_depends |
| 169 | D_SECURITY 对抗验证: _analysis.py | → | Tamper-Proof Audit — 防篡改审计 D-023-37 · §... | 导入依赖 / import_depends |
| 170 | D_SECURITY 对抗验证: _analysis.py | → | Trend Analyzer — trend_analyzer.py (trend_anal... | 导入依赖 / import_depends |
| 171 | D_SECURITY 对抗验证: _core.py | → | Config Consistency Checker — 配置多源一致性 D-... | 导入依赖 / import_depends |
| 172 | D_SECURITY 对抗验证: _core.py | → | Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | 导入依赖 / import_depends |
| 173 | D_SECURITY 对抗验证: _core.py | → | Drift Detector 数据模型 — drift_models.py (dri... | 导入依赖 / import_depends |
| 174 | D_SECURITY 对抗验证: _drift.py | → | contract_drift_detector — 契约漂移检测器。 (co... | 导入依赖 / import_depends |
| 175 | D_SECURITY 对抗验证: _drift.py | → | Drift Hotfix Bypass — drift_hotfix_bypass.py (... | 导入依赖 / import_depends |
| 176 | D_SECURITY 对抗验证: _drift.py | → | Drift Detector 基础设施 — drift_infrastructure... | 导入依赖 / import_depends |
| 177 | D_SECURITY 对抗验证: _drift.py | → | Drift Detector 结果类型 + 专项检测函数 — drift... | 导入依赖 / import_depends |
| 178 | D_SECURITY 对抗验证: _drift.py | → | Drift Detector AI 训练闭环 + 跨语言检测 — drif... | 导入依赖 / import_depends |
| 179 | D_SECURITY 对抗验证: _infrastructure.py | → | Owner Absence Manager — Owner缺席模式 §6.32。... | 导入依赖 / import_depends |
| 180 | D_SECURITY 对抗验证: _infrastructure.py | → | AI Context Injector — 施工前预检D-023-16 · §... | 导入依赖 / import_depends |
| 181 | D_SECURITY 对抗验证: _infrastructure.py | → | Baseline Manager — baseline_manager.py (baseli... | 导入依赖 / import_depends |
| 182 | D_SECURITY 对抗验证: _infrastructure.py | → | Detector Canary Controller — 检测器金丝雀部署 ... | 导入依赖 / import_depends |
| 183 | D_SECURITY 对抗验证: _infrastructure.py | → | Config Consistency Checker — 配置多源一致性 D-... | 导入依赖 / import_depends |
| 184 | D_SECURITY 对抗验证: _infrastructure.py | → | Coverage Dashboard — dashboard.py (dashboard.py) | 导入依赖 / import_depends |
| 185 | D_SECURITY 对抗验证: _infrastructure.py | → | Gate Persistence — gate_persistence.py (gate_p... | 导入依赖 / import_depends |
| 186 | D_SECURITY 对抗验证: _infrastructure.py | → | Cross-Session Handoff Manager — 跨Session修复.... | 导入依赖 / import_depends |
| 187 | D_SECURITY 对抗验证: _infrastructure.py | → | Resource Guard — 资源上限与优雅降级 D-023-23 .... | 导入依赖 / import_depends |
| 188 | D_SECURITY 对抗验证: _scanners.py | → | Incremental Scanner — incremental_scanner.py (... | 导入依赖 / import_depends |
| 189 | D_SECURITY 对抗验证: _scanners.py | → | Naming Magic Checker — 命名魔数与隐式约定检测 ... | 导入依赖 / import_depends |
| 190 | D_SECURITY 对抗验证: _scanners.py | → | Orphan Resource Scanner — 孤儿资源检测 §6.28... | 导入依赖 / import_depends |
| 191 | D_SECURITY 对抗验证: _scanners.py | → | Python Compatibility Checker — Python版本兼容.... | 导入依赖 / import_depends |
| 192 | D_SECURITY 对抗验证: _scanners.py | → | Scan Mutex — scan_mutex.py (scan_mutex.py) | 导入依赖 / import_depends |
| 193 | D_SECURITY 对抗验证: _scanners.py | → | Symlink Integrity Checker — 软链接完整性检测 .... | 导入依赖 / import_depends |
| 194 | D_SECURITY 对抗验证: _scanners.py | → | Test Fixture Checker — 测试夹具漂移检测 D-023-... | 导入依赖 / import_depends |
| 195 | D_SECURITY 对抗验证: Cold Start Bootstrapper — 冷启动引导 §6.31。 ... | → | Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | 导入依赖 / import_depends |
| 196 | D_SECURITY 对抗验证: Auto Reconciler — reconciler.py (reconciler.py) | → | Drift Detector 数据模型 — drift_models.py (dri... | 导入依赖 / import_depends |
| 197 | D_SECURITY 对抗验证: Drift Runbook Generator — 漂移演练手册自动生成... | → | Drift Detector 数据模型 — drift_models.py (dri... | 导入依赖 / import_depends |
| 198 | D_SECURITY 对抗验证: Drift State Machine — state_machine.py (state_... | → | Drift Detector 数据模型 — drift_models.py (dri... | 导入依赖 / import_depends |
| 199 | D_SECURITY 对抗验证: drift_bridge.py | → | Gate-side Drift Detector Recovery — zephyr.gov... | 导入依赖 / import_depends |
| 200 | D_SHARED 共享服务: test_cross_module_score.py | → | Cross Module Score — cross_module_score.py (cr... | 测试依赖 / test_depends |
| 201 | D_SHARED 共享服务: test_e_reward_hacking.py | → | Reward Hacking Rebound Detector — v0.14.0 §2.... | 测试依赖 / test_depends |
| 202 | D_SHARED 共享服务: test_e_silence_detector.py | → | Silence Detector — v0.8.0 静默窗口检测器: agen... | 测试依赖 / test_depends |
| 203 | D_SHARED 共享服务: test_file_attr_checker.py | → | File Attribute Integrity — 文件底层属性完整性 ... | 测试依赖 / test_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 23 个外部域直接连接（出边 51 条 + 入边 203 条 = 254 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_AUTONOMY_CORE -->|2条 runtime / runtime, 测试依赖 / test_depends| D_GOV_DRIFT
