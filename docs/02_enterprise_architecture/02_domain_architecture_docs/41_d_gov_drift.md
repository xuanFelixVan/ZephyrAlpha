...
graph TD
    ...
    D_GOV_RULE -.->|config_depends / config_depends| src_zephyr_gov_enforcement_rule_enforcement_gate_engine_gate_integrity_guard_py
    D_COMPLIANCE["(原型态 / prototype) D_COMPLIANCE"]
    ...
    class D_SHARED,D_GOV_AUDIT,D_SECURITY,D_INFRA_RECOVERY,D_GOVERNANCE,D_OPS,D_ORCHESTRATOR,D_FEEDBACK_LOOP external_prod
    class D_GOV_RULE,D_COMPLIANCE external_design
```
...
graph TD
    ...
    class D_GOV_AUDIT,D_GOV_RULE,D_COMPLIANCE,D_SECURITY external_design
```
...
| 62 | D_GOV_RULE 规则治理: gate_engine package — 门禁引擎模块集合（ARCH-0... | → | 门禁引擎完整性守卫——自检SHA-256校验+trust roo... | config_depends / config_depends |
