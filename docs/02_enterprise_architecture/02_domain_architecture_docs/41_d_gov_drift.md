...
graph TD
    ...
    class D_SECURITY,D_SHARED,D_GOV_OPS_RESILIENCE,D_INFRA_RUNTIME external_prod
    class D_GOV_DOCS,D_GOV_AUDIT,D_COMPLIANCE external_design
```
...
graph TD
    ...
    class D_INTEGRATION,D_GOV_AUDIT,D_SECURITY,D_INTELLIGENCE,D_COMPLIANCE external_design
```
...
graph TD
    ...
    class D_INFRA_RECOVERY,D_SECURITY,D_GOV_AUDIT,D_SHARED,D_GOVERNANCE,D_INTEGRATION,D_ORCHESTRATOR,D_FEEDBACK_LOOP external_prod
...
graph TD
    ...
    class D_SHARED,D_GOV_AUDIT,D_GOV_SCRIPTS,D_GOVERNANCE,D_GOV_ENFORCEMENT,D_GOV_RULE external_prod
    class D_INTEGRATION,D_COMPLIANCE,D_SECURITY external_design
```
...
graph TD
    ...
    class D_GOV_DOCS,D_GOV_AUDIT,D_AUTONOMY_CORE external_design
```
...
graph TD
    ...
    class D_INFRA_RECOVERY,D_SECURITY,D_SHARED,D_ORCHESTRATOR external_prod
```
...
| 8 | blueprint.md | → | D_GOV_AUDIT 审计追踪: blueprint.md | runtime / runtime |
| 9 | Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | → | D_GOV_AUDIT 审计追踪: finding_ingest.py | 导入依赖 / import_depends |
| 10 | Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | → | D_GOV_AUDIT 审计追踪: finding_model.py | 导入依赖 / import_depends |
| 11 | 真源优先级裁决器（Truth Source Validator） (tru... | → | D_GOV_AUDIT 审计追踪: bridge.py | 导入依赖 / import_depends |
| 12 | integrity.py | → | D_GOV_AUDIT 审计追踪: audit-trail.merkle_hourly — MOD-INF-020 · 每.... | 导入依赖 / import_depends |
| 13 | integrity.py | → | D_GOV_AUDIT 审计追踪: models.py | 导入依赖 / import_depends |
| 14 | integrity.py | → | D_GOV_AUDIT 审计追踪: trust_bridge.py | 导入依赖 / import_depends |
...
| 24 | blueprint.md | → | D_SECURITY 对抗验证: RiskMitigation — 风险评估与缓解策略. (risk_mit... | runtime / runtime |
...
| 50 | D_GOVERNANCE 生命周期管理: GovernanceServer: 治理域统一MCP入口 (governance... | → | Drift Engine — 编排器核心 (SRC-0030 精简后) (d... | 导入依赖 / import_depends |
| 51 | D_GOVERNANCE 生命周期管理: GovernanceServer: 治理域统一MCP入口 (governance... | → | Drift Detector 基础设施 — drift_infrastructure... | 导入依赖 / import_depends |
| 52 | D_GOVERNANCE 生命周期管理: GovernanceServer: 治理域统一MCP入口 (governance... | → | Drift Detector 数据模型 — drift_models.py (dri... | 导入依赖 / import_depends |
| 53 | D_GOVERNANCE 生命周期管理: test_reward_hacking_rebound_detector.py | → | Reward Hacking Rebound Detector — v0.14.0 §2.... | 测试依赖 / test_depends |
| 54 | D_GOVERNANCE 生命周期管理: test_vigil_runtime.py | → | Vigil Runtime — v0.6.0 VIGIL维护运行时: 运维to... | 测试依赖 / test_depends |
| 55 | D_GOVERNANCE 生命周期管理: test_integrity_root.py | → | integrity.py | 测试依赖 / test_depends |
| 56 | D_GOVERNANCE 生命周期管理: test_bootstrapping_calibrator.py | → | bootstrapping_calibrator.py | 测试依赖 / test_depends |
| 57 | D_GOVERNANCE 生命周期管理: test_silence_detector.py | → | Silence Detector — v0.8.0 静默窗口检测器: agen... | 测试依赖 / test_depends |
| 58 | D_GOVERNANCE 生命周期管理: test_spiral_ews.py | → | spiral_ews.py | 测试依赖 / test_depends |
| 59 | D_GOVERNANCE 生命周期管理: test_en_002_enforcement_validator.py | → | EN-002 — Enforcement Mode Validator (en_002_en... | 测试依赖 / test_depends |
| 60 | D_GOVERNANCE 生命周期管理: test_breaking_change_detector.py | → | Breaking Change 检测器（GATE-CDC-2）——字段删.... | 测试依赖 / test_depends |
| 61 | D_GOV_AUDIT 审计追踪: blueprint.md | → | blueprint.md | runtime / runtime |
...
| 70 | D_GOV_AUDIT 审计追踪: test_audit_integrity.py | → | integrity.py | 测试依赖 / test_depends |
...
| 123 | D_GOV_DOCS 架构文档治理: blueprint.md | → | blueprint.md | contract / contract |
...
> 本域与 22 个外部域直接连接（出边 51 条 + 入边 204 条 = 255 条）。只显示直接连接的域，不展开具体节点。
