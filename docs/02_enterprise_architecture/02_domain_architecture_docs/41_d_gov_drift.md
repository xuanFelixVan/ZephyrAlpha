...
graph TD
    ...
    class D_GOV_DOCS,D_SECURITY,D_GOV_AUDIT,D_COMPLIANCE,D_INFRA_A2A,D_INTELLIGENCE external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_RULE,D_GOV_REPAIR external_prod
...
graph TD
    ...
    class D_GOVERNANCE,D_GOV_SCRIPTS,D_FEEDBACK_LOOP external_prod
    class D_SHARED,D_GOV_AUDIT,D_COMPLIANCE,D_SECURITY,D_GOV_ENFORCEMENT external_design
```
...
> 本域与 22 个外部域直接连接（出边 46 条 + 入边 199 条 = 245 条）。只显示直接连接的域，不展开具体节点。
