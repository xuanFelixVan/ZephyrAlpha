...
flowchart TD
    src_zephyr_factor_alpha_signal_pipeline_py["(生产态 / production) 阿尔法信号管线 /<br/>alpha_signal_pipeline<br/>阿尔法信号管线，依赖管线、D-SIGLEGACY-01<br/>工作<br/>文件: factor/alpha_signal_pipeline.py"]
    src_zephyr_factor_analysis_correlation_dedup_py["(生产态 / production) D-FACTOR-ANA-05<br/>因子相关性去重——基于相关性矩阵去除冗余因<br/>子。 / correlation_dedup<br/>D-FACTOR-ANA-05<br/>因子相关性去重——基于相关性矩阵去除冗余因<br/>子。<br/>文件: analysis/correlation_dedup.py"]
    src_zephyr_factor_analysis_decay_monitor_py["(生产态 / production) D-FACTOR-ANA-08<br/>衰减监控——监控因子 IC<br/>衰减速度，半衰期低于 / decay_monitor<br/>D-FACTOR-ANA-08 衰减监控——监控因子 IC<br/>衰减速度，半衰期低于阈值告警。<br/>文件: analysis/decay_monitor.py"]
    src_zephyr_factor_analysis_factor_attribution_py["(生产态 / production) D-FACTOR-ANA-09<br/>因子归因——按时间和行业维度分解因子表现。<br/>/ factor_attribution<br/>D-FACTOR-ANA-09<br/>因子归因——按时间和行业维度分解因子表现。<br/>文件: analysis/factor_attribution.py"]
    src_zephyr_factor_analysis_factor_optimization_py["(生产态 / production) D-FACTOR-ANA-11<br/>因子优化——优化多因子合成权重以最大化目标<br/>函数。 / factor_optimization<br/>D-FACTOR-ANA-11<br/>因子优化——优化多因子合成权重以最大化目标<br/>函数。<br/>文件: analysis/factor_optimization.py"]
    src_zephyr_factor_analysis_ic_ir_calc_py["(生产态 / production) D-FACTOR-ANA-01<br/>IC/IR 批量计算器——多因子 IC/IR 指 / ic_<br/>ir_calc<br/>D-FACTOR-ANA-01 IC/IR<br/>批量计算器——多因子 IC/IR 指标汇总表。<br/>文件: analysis/ic_ir_calc.py"]
    src_zephyr_factor_analysis_ic_ir_evaluator_py["(生产态 / production) D-FACTOR-ANA-02<br/>多因子评估报告器——批量评估+格式化报告。<br/>/ ic_ir_evaluator<br/>D-FACTOR-ANA-02<br/>多因子评估报告器——批量评估+格式化报告。<br/>文件: analysis/ic_ir_evaluator.py"]
    src_zephyr_factor_analysis_layered_backtest_py["(生产态 / production) D-FACTOR-ANA-06<br/>分层回测——按因子值分组计算各层收益与多空<br/>收益差 / layered_backtest<br/>D-FACTOR-ANA-06<br/>分层回测——按因子值分组计算各层收益与多空<br/>收益差。<br/>文件: analysis/layered_backtest.py"]
    src_zephyr_factor_analysis_three_level_judgment_py["(生产态 / production) D-FACTOR-ANA-07<br/>三级判定——按 IC 均值将因子分为优秀/合格<br/>/ / three_level_judgment<br/>D-FACTOR-ANA-07 三级判定——按 IC<br/>均值将因子分为优秀/合格/淘汰。<br/>文件: analysis/three_level_judgment.py"]
    src_zephyr_factor_bus_factor_defense_py["(生产态 / production) 总线因子防御 /<br/>bus_factor_defense<br/>总线因子防御，供MOD-INF-027;MOD-INF-020;<br/>MOD-IN使用<br/>文件: factor/bus_factor_defense.py"]
    src_zephyr_factor_core_backpressure_init_py["(生产态 / production) 包入口 / __init__<br/>D_FACTOR core backpressure<br/>子包——进程内在途并发限流器。<br/>文件: backpressure/__init__.py"]
    src_zephyr_factor_core_batch_output_init_py["(生产态 / production) 包入口 / __init__<br/>D_FACTOR core batch_output<br/>子包——FactorSignal 批量缓冲写入器。<br/>文件: batch_output/__init__.py"]
    src_zephyr_factor_core_config_manager_init_py["(生产态 / production) 包入口 / __init__<br/>D_FACTOR core config_manager 子包——core<br/>基础设施模块策略参数加载器。<br/>文件: config_manager/__init__.py"]
    src_zephyr_factor_core_ctr001_consumer_init_py["(生产态 / production) 包入口 / __init__<br/>CTR-001 NormalizedMarketData<br/>消费者包入口。<br/>文件: ctr001_consumer/__init__.py"]
    src_zephyr_factor_core_ctr002_producer_init_py["(生产态 / production) 包入口 / __init__<br/>CTR-002 FactorSignal 生产者包入口。<br/>文件: ctr002_producer/__init__.py"]
    src_zephyr_factor_core_dag_manager_init_py["(生产态 / production) 包入口 / __init__<br/>D_FACTOR core dag_manager 子包——DAG<br/>调度执行器。<br/>文件: dag_manager/__init__.py"]
    src_zephyr_factor_core_dist_feature_eng_init_py["(生产态 / production) 包入口 / __init__<br/>D_FACTOR core dist_feature_eng<br/>子包——分布式特征工程引擎。<br/>文件: dist_feature_eng/__init__.py"]
    src_zephyr_factor_core_evaluation_init_py["(生产态 / production) D-FACTOR-03<br/>因子评估包——IC/IR/OOS 正率/过拟合检测。<br/>/ __init__<br/>D-FACTOR-03 因子评估包——IC/IR/OOS 正率<br/>/过拟合检测。<br/>文件: evaluation/__init__.py"]
    src_zephyr_factor_core_factor_dag_init_py["(生产态 / production) 包入口 / __init__<br/>D_FACTOR core factor_dag 子包——因子 DAG<br/>数据结构 + Kahn 拓扑分层。<br/>文件: factor_dag/__init__.py"]
    src_zephyr_factor_governance_engine_py["(生产态 / production) D-FACTOR-GOV-05<br/>因子治理引擎——顶层编排六步流程+灰度发布<br/>。 / engine<br/>D-FACTOR-GOV-05<br/>因子治理引擎——顶层编排六步流程+灰度发布<br/>。<br/>文件: governance/engine.py"]
    src_zephyr_factor_governance_factor_pool_manager_py["(生产态 / production) D-FACTOR-08<br/>因子池容量管理——活跃池/休眠池 +<br/>IC末位淘汰 +  / factor_pool_manager<br/>D-FACTOR-08 因子池容量管理——活跃池<br/>/休眠池 + IC末位淘汰 + 批量裁剪。<br/>文件: governance/factor_pool_manager.py"]
    src_zephyr_factor_momentum_factor_py["(生产态 / production) 动量因子 / D_<br/>FACTOR — Momentum Factor<br/>动量因子。D_FACTOR — Momentum Factor<br/>文件: factor/momentum_factor.py"]
    src_zephyr_factor_value_factor_py["(生产态 / production) 价值因子 / D_<br/>FACTOR — Value Factor<br/>价值因子。D_FACTOR — Value Factor<br/>文件: factor/value_factor.py"]
    ...
    src_zephyr_factor_analysis_init_py["(生产态 / production) 包入口 / __init__<br/>D_FACTOR analysis<br/>子包——因子分析与评估工具链。<br/>文件: analysis/__init__.py"]
    src_zephyr_factor_analysis_correlation_analyzer_py["(生产态 / production) D-FACTOR-ANA-04<br/>因子相关性分析——计算因子间相关性矩阵。<br/>/ correlation_analyzer<br/>D-FACTOR-ANA-04<br/>因子相关性分析——计算因子间相关性矩阵。<br/>文件: analysis/correlation_analyzer.py"]
    src_zephyr_factor_analysis_ic_decay_py["(生产态 / production) D-FACTOR-ANA-03<br/>IC 衰减分析——不同 lag 的 IC 衰减曲 / ic_<br/>decay<br/>D-FACTOR-ANA-03 IC 衰减分析——不同 lag<br/>的 IC 衰减曲线与半衰期。<br/>文件: analysis/ic_decay.py"]
    src_zephyr_factor_analysis_multifactor_synthesis_py["(生产态 / production) D-FACTOR-ANA-10<br/>多因子合成——将多个因子值合成为综合信号。<br/>/ multifactor_synthesis<br/>D-FACTOR-ANA-10<br/>多因子合成——将多个因子值合成为综合信号。<br/>文件: analysis/multifactor_synthesis.py"]
    src_zephyr_factor_core_ctr001_consumer_converter_py["(生产态 / production) 转换器 / converter<br/>CTR-001 NormalizedMarketData<br/>消费者——数据适配层。<br/>文件: ctr001_consumer/converter.py"]
    src_zephyr_factor_core_ctr002_producer_converter_py["(生产态 / production) 转换器 / converter<br/>CTR-002 FactorSignal<br/>生产者——信号适配层。<br/>文件: ctr002_producer/converter.py"]
    src_zephyr_factor_governance_grayscale_rollout_py["(生产态 / production) D-FACTOR-GOV-03<br/>灰度发布——管理因子从 10% → 30% →  /<br/>grayscale_rollout<br/>D-FACTOR-GOV-03 灰度发布——管理因子从<br/>10% → 30% → 100% 的放量阶梯。<br/>文件: governance/grayscale_rollout.py"]
    src_zephyr_factor_governance_six_step_flow_py["(生产态 / production) D-FACTOR-GOV-04<br/>六步流程编排——因子从研究到实盘的治理流程<br/>。 / six_step_flow<br/>D-FACTOR-GOV-04<br/>六步流程编排——因子从研究到实盘的治理流程<br/>。<br/>文件: governance/six_step_flow.py"]
    ...
    src_zephyr_factor_governance_abs001_gate_py["(生产态 / production) D-FACTOR-GOV-02<br/>ABS001 上线门禁——因子进入灰度前的质量检<br/>/ abs001_gate<br/>D-FACTOR-GOV-02 ABS001<br/>上线门禁——因子进入灰度前的质量检查。<br/>文件: governance/abs001_gate.py"]
    src_zephyr_factor_governance_lifecycle_state_machine_py["(生产态 / production)<br/>生命周期状态machine / lifecycle_state_<br/>machine<br/>D-FACTOR-GOV-01<br/>因子生命周期状态机——复用项目级<br/>StateMachine 泛型基类。<br/>文件: governance/lifecycle_state_<br/>machine.py"]
    src_zephyr_factor_governance_abs001_gate_py ~~~ src_zephyr_factor_governance_lifecycle_state_machine_py
    src_zephyr_factor_core_evaluation_backtest_py["(生产态 / production) D-FACTOR-03<br/>因子评估回测运行器——端到端因子评估。 /<br/>backtest<br/>D-FACTOR-03<br/>因子评估回测运行器——端到端因子评估。<br/>文件: evaluation/backtest.py"]
    src_zephyr_factor_governance_init_py["(生产态 / production) 包入口 / __init__<br/>D_FACTOR governance<br/>子包——因子生命周期治理工具链。<br/>文件: governance/__init__.py"]
    src_zephyr_factor_core_evaluation_backtest_py ~~~ src_zephyr_factor_governance_init_py
    src_zephyr_factor_core_evaluation_metrics_py["(生产态 / production) D-FACTOR-03<br/>因子评估指标——纯函数模块（无 IO<br/>依赖）。 / metrics<br/>D-FACTOR-03 因子评估指标——纯函数模块<br/>（无 IO 依赖）。<br/>文件: evaluation/metrics.py"]
    src_zephyr_factor_factor_base_py["(生产态 / production) 因子基类 /<br/>ZephyrAlpha — D_FACTOR Alpha Factor<br/>Layer<br/>因子基类。ZephyrAlpha — D_FACTOR Alpha<br/>Factor Layer<br/>文件: factor/factor_base.py"]
