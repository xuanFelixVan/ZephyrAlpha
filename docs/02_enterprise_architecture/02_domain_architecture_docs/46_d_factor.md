...
flowchart TD
    src_zephyr_factor_alpha_signal_pipeline_py["(生产态 / production) Alpha信号管线 / alpha_signal_pipeline<br/>Alpha信号管线，因子的组成部分，依赖管线、D-SIGLEGACY-01工作。<br/>文件: factor/alpha_signal_pipeline.py"]
    src_zephyr_factor_analysis_correlation_dedup_py["(生产态 / production) D-FACTOR-ANA-05 因子相关性去重——基于相关性矩阵去除冗余因子。 / correlation_dedup<br/>D-FACTOR-ANA-05 因子相关性去重——基于相关性矩阵去除冗余因子。<br/>文件: analysis/correlation_dedup.py"]
    src_zephyr_factor_analysis_decay_monitor_py["(生产态 / production) D-FACTOR-ANA-08 衰减监控——监控因子 IC 衰减速度，半衰期低于 / decay_monitor<br/>D-FACTOR-ANA-08 衰减监控——监控因子 IC 衰减速度，半衰期低于阈值告警。<br/>文件: analysis/decay_monitor.py"]
    src_zephyr_factor_analysis_factor_attribution_py["(生产态 / production) D-FACTOR-ANA-09 因子归因——按时间和行业维度分解因子表现。 / factor_attribution<br/>D-FACTOR-ANA-09 因子归因——按时间和行业维度分解因子表现。<br/>文件: analysis/factor_attribution.py"]
    src_zephyr_factor_analysis_factor_optimization_py["(生产态 / production) D-FACTOR-ANA-11 因子优化——优化多因子合成权重以最大化目标函数。 / factor_optimization<br/>D-FACTOR-ANA-11 因子优化——优化多因子合成权重以最大化目标函数。<br/>文件: analysis/factor_optimization.py"]
    src_zephyr_factor_analysis_ic_ir_calc_py["(生产态 / production) D-FACTOR-ANA-01 IC/IR 批量计算器——多因子 IC/IR 指 / ic_ir_calc<br/>D-FACTOR-ANA-01 IC/IR 批量计算器——多因子 IC/IR 指标汇总表。<br/>文件: analysis/ic_ir_calc.py"]
    src_zephyr_factor_analysis_ic_ir_evaluator_py["(生产态 / production) D-FACTOR-ANA-02 多因子评估报告器——批量评估+格式化报告。 / ic_ir_evaluator<br/>D-FACTOR-ANA-02 多因子评估报告器——批量评估+格式化报告。<br/>文件: analysis/ic_ir_evaluator.py"]
    src_zephyr_factor_analysis_layered_backtest_py["(生产态 / production) D-FACTOR-ANA-06 分层回测——按因子值分组计算各层收益与多空收益差 / layered_backtest<br/>D-FACTOR-ANA-06 分层回测——按因子值分组计算各层收益与多空收益差。<br/>文件: analysis/layered_backtest.py"]
    src_zephyr_factor_analysis_three_level_judgment_py["(生产态 / production) D-FACTOR-ANA-07 三级判定——按 IC 均值将因子分为优秀/合格/ / three_level_judgment<br/>D-FACTOR-ANA-07 三级判定——按 IC 均值将因子分为优秀/合格/淘汰。<br/>文件: analysis/three_level_judgment.py"]
    src_zephyr_factor_bus_factor_defense_py["(生产态 / production) 总线因子防御 / bus_factor_defense<br/>总线因子防御，因子的功能模块。<br/>文件: factor/bus_factor_defense.py"]
    src_zephyr_factor_core_backpressure_init_py["(生产态 / production) DFACTOR core backpressure 子包——进程内在途并发限流 / __init__<br/>D_FACTOR core backpressure 子包——进程内在途并发限流器。<br/>文件: backpressure/__init__.py"]
    src_zephyr_factor_core_batch_output_init_py["(生产态 / production) DFACTOR core batchoutput 子包——FactorSig / __init__<br/>D_FACTOR core batch_output 子包——FactorSignal 批量缓冲写入器。<br/>文件: batch_output/__init__.py"]
    src_zephyr_factor_core_config_manager_init_py["(生产态 / production) DFACTOR core configmanager 子包——core 基础 / __init__<br/>D_FACTOR core config_manager 子包——core 基础设施模块策略参数加载器。<br/>文件: config_manager/__init__.py"]
    src_zephyr_factor_core_ctr001_consumer_init_py["(生产态 / production) CTR-001 NormalizedMarketData 消费者包入口。 / __init__<br/>CTR-001 NormalizedMarketData 消费者包入口。<br/>文件: ctr001_consumer/__init__.py"]
    src_zephyr_factor_core_ctr002_producer_init_py["(生产态 / production) CTR-002 FactorSignal 生产者包入口。 / __init__<br/>CTR-002 FactorSignal 生产者包入口。<br/>文件: ctr002_producer/__init__.py"]
    src_zephyr_factor_core_dag_manager_init_py["(生产态 / production) DFACTOR core dagmanager 子包——DAG 调度执行器。 / __init__<br/>D_FACTOR core dag_manager 子包——DAG 调度执行器。<br/>文件: dag_manager/__init__.py"]
    src_zephyr_factor_core_dist_feature_eng_init_py["(生产态 / production) DFACTOR core dist特征eng 子包——分布式特征 / __init__<br/>D_FACTOR core dist_feature_eng 子包——分布式特征工程引擎。<br/>文件: dist_feature_eng/__init__.py"]
    src_zephyr_factor_core_evaluation_init_py["(生产态 / production) D-FACTOR-03 因子评估包——IC/IR/OOS 正率/过拟合检测。 / __init__<br/>D-FACTOR-03 因子评估包——IC/IR/OOS 正率/过拟合检测。<br/>文件: evaluation/__init__.py"]
    src_zephyr_factor_core_factor_dag_init_py["(生产态 / production) DFACTOR core factordag 子包——因子 DAG 数据结构 / __init__<br/>D_FACTOR core factor_dag 子包——因子 DAG 数据结构 + Kahn 拓扑分层。<br/>文件: factor_dag/__init__.py"]
    src_zephyr_factor_governance_engine_py["(生产态 / production) D-FACTOR-GOV-05 因子治理引擎——顶层编排六步流程+灰度发布。 / engine<br/>D-FACTOR-GOV-05 因子治理引擎——顶层编排六步流程+灰度发布。<br/>文件: governance/engine.py"]
    src_zephyr_factor_governance_factor_pool_manager_py["(生产态 / production) D-FACTOR-08 因子池容量管理——活跃池/休眠池 + IC末位淘汰 +  / factor_pool_manager<br/>D-FACTOR-08 因子池容量管理——活跃池/休眠池 + IC末位淘汰 + 批量裁剪。<br/>文件: governance/factor_pool_manager.py"]
    src_zephyr_factor_momentum_factor_py["(生产态 / production) 动量因子 / D_FACTOR — Momentum Factor<br/>动量因子。D_FACTOR — Momentum Factor<br/>文件: factor/momentum_factor.py"]
    src_zephyr_factor_value_factor_py["(生产态 / production) 价值因子 / D_FACTOR — Value Factor<br/>价值因子。D_FACTOR — Value Factor<br/>文件: factor/value_factor.py"]
    ...
    src_zephyr_factor_analysis_init_py["(生产态 / production) DFACTOR analysis 子包——因子分析与评估工具链。 / __init__<br/>D_FACTOR analysis 子包——因子分析与评估工具链。<br/>文件: analysis/__init__.py"]
    src_zephyr_factor_analysis_correlation_analyzer_py["(生产态 / production) D-FACTOR-ANA-04 因子相关性分析——计算因子间相关性矩阵。 / correlation_analyzer<br/>D-FACTOR-ANA-04 因子相关性分析——计算因子间相关性矩阵。<br/>文件: analysis/correlation_analyzer.py"]
    src_zephyr_factor_analysis_ic_decay_py["(生产态 / production) D-FACTOR-ANA-03 IC 衰减分析——不同 lag 的 IC 衰减曲 / ic_decay<br/>D-FACTOR-ANA-03 IC 衰减分析——不同 lag 的 IC 衰减曲线与半衰期。<br/>文件: analysis/ic_decay.py"]
    src_zephyr_factor_analysis_multifactor_synthesis_py["(生产态 / production) D-FACTOR-ANA-10 多因子合成——将多个因子值合成为综合信号。 / multifactor_synthesis<br/>D-FACTOR-ANA-10 多因子合成——将多个因子值合成为综合信号。<br/>文件: analysis/multifactor_synthesis.py"]
    src_zephyr_factor_core_ctr001_consumer_converter_py["(生产态 / production) CTR-001 NormalizedMarketData 消费者——数据适配层。 / converter<br/>CTR-001 NormalizedMarketData 消费者——数据适配层。<br/>文件: ctr001_consumer/converter.py"]
    src_zephyr_factor_core_ctr002_producer_converter_py["(生产态 / production) CTR-002 FactorSignal 生产者——信号适配层。 / converter<br/>CTR-002 FactorSignal 生产者——信号适配层。<br/>文件: ctr002_producer/converter.py"]
    src_zephyr_factor_governance_grayscale_rollout_py["(生产态 / production) D-FACTOR-GOV-03 灰度发布——管理因子从 10% → 30% →  / grayscale_rollout<br/>D-FACTOR-GOV-03 灰度发布——管理因子从 10% → 30% → 100% 的放量阶梯。<br/>文件: governance/grayscale_rollout.py"]
    src_zephyr_factor_governance_six_step_flow_py["(生产态 / production) D-FACTOR-GOV-04 六步流程编排——因子从研究到实盘的治理流程。 / six_step_flow<br/>D-FACTOR-GOV-04 六步流程编排——因子从研究到实盘的治理流程。<br/>文件: governance/six_step_flow.py"]
    ...
    src_zephyr_factor_governance_abs001_gate_py["(生产态 / production) D-FACTOR-GOV-02 ABS001 上线门禁——因子进入灰度前的质量检 / abs001_gate<br/>D-FACTOR-GOV-02 ABS001 上线门禁——因子进入灰度前的质量检查。<br/>文件: governance/abs001_gate.py"]
    src_zephyr_factor_governance_lifecycle_state_machine_py["(生产态 / production) D-FACTOR-GOV-01 因子生命周期状态机——复用项目级 StateMa / lifecycle_state_machine<br/>D-FACTOR-GOV-01 因子生命周期状态机——复用项目级 StateMachine 泛型基类。<br/>文件: governance/lifecycle_state_machine.py"]
    src_zephyr_factor_governance_abs001_gate_py ~~~ src_zephyr_factor_governance_lifecycle_state_machine_py
    src_zephyr_factor_core_evaluation_backtest_py["(生产态 / production) D-FACTOR-03 因子评估回测运行器——端到端因子评估。 / backtest<br/>D-FACTOR-03 因子评估回测运行器——端到端因子评估。<br/>文件: evaluation/backtest.py"]
    src_zephyr_factor_governance_init_py["(生产态 / production) DFACTOR governance 子包——因子生命周期治理工具链。 / __init__<br/>D_FACTOR governance 子包——因子生命周期治理工具链。<br/>文件: governance/__init__.py"]
    src_zephyr_factor_core_evaluation_backtest_py ~~~ src_zephyr_factor_governance_init_py
    src_zephyr_factor_core_evaluation_metrics_py["(生产态 / production) D-FACTOR-03 因子评估指标——纯函数模块（无 IO 依赖）。 / metrics<br/>D-FACTOR-03 因子评估指标——纯函数模块（无 IO 依赖）。<br/>文件: evaluation/metrics.py"]
    src_zephyr_factor_factor_base_py["(生产态 / production) 因子基类 / ZephyrAlpha — D_FACTOR Alpha Factor Layer<br/>因子基类。ZephyrAlpha — D_FACTOR Alpha Factor Layer<br/>文件: factor/factor_base.py"]
...
|:--:|---------|:--:|---------|---------|
| 1 | D-FACTOR-03 因子评估回测运行器——端到端因子评估。 / back... | → | D_DATA 数据接入层: zephyr.data — 数据源集成器（MOD-L00-004）。 / __init__ (... | 导入依赖 / import_depends |
| 2 | D-FACTOR-03 因子评估回测运行器——端到端因子评估。 / back... | → | D_DATA 数据接入层: ClickHouse 统一读取层（裁定 #ARCH-CH-007）。 / ch_reader ... | 导入依赖 / import_depends |
| 3 | D-FACTOR-03 因子评估回测运行器——端到端因子评估。 / back... | → | D_DATA 数据接入层: 表名/品类注册表消费层（裁定 #ARCH-CH-024 Phase 2）。 / ta... | 导入依赖 / import_depends |
| 4 | Alpha信号管线 / alpha_signal_pipeline (factor/alpha_signa... | → | D_FUNDAMENTAL_SIGNAL 基本面信号: Alpha信号管线 / Alpha Signal Pipeline (signal_fundamental... | 导入依赖 / import_depends |
| 5 | CTR-001 NormalizedMarketData 消费者——数据适配层。 / con... | → | D_INFRASTRUCTURE 跨层契约基础设施: 市场数据 / market_data (contracts/market_data.py) | 导入依赖 / import_depends |
| 6 | CTR-002 FactorSignal 生产者——信号适配层。 / converter (... | → | D_INFRASTRUCTURE 跨层契约基础设施: 因子信号 / factor_signal (contracts/factor_signal.py) | 导入依赖 / import_depends |
| 7 | D-FACTOR-GOV-01 因子生命周期状态机——复用项目级 StateMa ... | → | D_SHARED 共享服务: StateMachine[S] — 通用状态机泛型基类 (MOD-INF-038 / stat... | 导入依赖 / import_depends |
| 8 | D-FACTOR-GOV-04 六步流程编排——因子从研究到实盘的治理流... | → | D_SHARED 共享服务: StateMachine[S] — 通用状态机泛型基类 (MOD-INF-038 / stat... | 导入依赖 / import_depends |

### 依赖本域的其他域（入边）/ Depended By

| # | 外部域-源模块 / Source Module | → | 本域模块 / Target Module | 依赖类型 / Type |
|:--:|---------|:--:|---------|---------|
| 1 | D_EX_CORE 执行核心: DCORE — 信号源 / 价格源 callable 工厂 / signal_providers... | → | D-FACTOR-ANA-10 多因子合成——将多个因子值合成为综合信号... | 导入依赖 / import_depends |
| 2 | D_EX_CORE 执行核心: DCORE — 信号源 / 价格源 callable 工厂 / signal_providers... | → | D-FACTOR-03 因子评估回测运行器——端到端因子评估。 / back... | 导入依赖 / import_depends |
| 3 | D_EX_CORE 执行核心: DCORE — 信号源 / 价格源 callable 工厂 / signal_providers... | → | 因子基类 / ZephyrAlpha — D_FACTOR Alpha Factor Layer (fa... | 导入依赖 / import_depends |
| 4 | D_FUNDAMENTAL_SIGNAL 基本面信号: Alpha信号管线 / Alpha Signal Pipeline (signal_fundamental... | → | 因子基类 / ZephyrAlpha — D_FACTOR Alpha Factor Layer (fa... | 导入依赖 / import_depends |
...
| 6 | D_PF_CORE 组合核心: DCORE — StrategyRunner 策略运行器（ / strategy_runner (s... | → | D-FACTOR-ANA-10 多因子合成——将多个因子值合成为综合信号... | 导入依赖 / import_depends |
| 7 | D_PF_CORE 组合核心: DCORE — StrategyRunner 策略运行器（ / strategy_runner (s... | → | D-FACTOR-03 因子评估回测运行器——端到端因子评估。 / back... | 导入依赖 / import_depends |
| 8 | D_PF_CORE 组合核心: DCORE — StrategyRunner 策略运行器（ / strategy_runner (s... | → | 因子基类 / ZephyrAlpha — D_FACTOR Alpha Factor Layer (fa... | 导入依赖 / import_depends |
