...
| # | 模块路径 / Module Path | 模块名称 / Module Name (功能简介 / Description) | 成熟度 / Maturity | 蓝图 / Blueprint |
|:--:|---------|---------|:---:|:---:|
| 1 | src/zephyr/factor/core/backpressure/ |  | 设计态 / design |  |
| 2 | src/zephyr/factor/core/backpressure/limiter.py | D_FACTOR core backpressure.limiter——进程内在... | 设计态 / design |  |
| 3 | src/zephyr/factor/core/batch_output/ |  | 设计态 / design |  |
| 4 | src/zephyr/factor/core/batch_output/buffer.py | D_FACTOR core batch_output.buffer——FactorSign... | 设计态 / design |  |
| 5 | src/zephyr/factor/core/config_manager/ |  | 设计态 / design |  |
| 6 | src/zephyr/factor/core/config_manager/loader.py | D_FACTOR core config_manager 加载器——加载 cor... | 设计态 / design |  |
| 7 | src/zephyr/factor/core/dag_manager/ |  | 设计态 / design |  |
| 8 | src/zephyr/factor/core/dag_manager/executor.py | D_FACTOR core dag_manager.executor——DAG 调度... | 设计态 / design |  |
| 9 | src/zephyr/factor/core/dist_feature_eng/ |  | 设计态 / design |  |
| 10 | src/zephyr/factor/core/dist_feature_eng/engine.py | D_FACTOR core dist_feature_eng.engine——分布式... | 设计态 / design |  |
| 11 | src/zephyr/factor/core/factor_dag/ |  | 设计态 / design |  |
| 12 | src/zephyr/factor/core/factor_dag/dag.py | D_FACTOR core factor_dag.dag——因子 DAG 数据结... | 设计态 / design |  |
...
> 展示全部 49 个模块（生产态 37 + 设计态 12），标签标注成熟度。
...
graph TD
    subgraph D_FACTOR["D_FACTOR 因子"]
        ...
        src_zephyr_factor_core_backpressure["(设计态 / design) "]
        src_zephyr_factor_core_backpressure_init_py["(生产态 / production) D_FACTOR core backpressure 子包——进程内在途并...<br/>文件: __init__.py"]
        src_zephyr_factor_core_backpressure_limiter_py["(设计态 / design) D_FACTOR core backpressure.limiter——进程内在...<br/>文件: limiter.py"]
        src_zephyr_factor_core_batch_output["(设计态 / design) "]
        src_zephyr_factor_core_batch_output_init_py["(生产态 / production) D_FACTOR core batch_output 子包——FactorSignal...<br/>文件: __init__.py"]
        src_zephyr_factor_core_batch_output_buffer_py["(设计态 / design) D_FACTOR core batch_output.buffer——FactorSign...<br/>文件: buffer.py"]
        src_zephyr_factor_core_config_manager["(设计态 / design) "]
        src_zephyr_factor_core_config_manager_init_py["(生产态 / production) D_FACTOR core config_manager 子包——core 基础...<br/>文件: __init__.py"]
        src_zephyr_factor_core_config_manager_loader_py["(设计态 / design) D_FACTOR core config_manager 加载器——加载 cor...<br/>文件: loader.py"]
        ...
        src_zephyr_factor_core_dag_manager["(设计态 / design) "]
        src_zephyr_factor_core_dag_manager_init_py["(生产态 / production) D_FACTOR core dag_manager 子包——DAG 调度执行器。<br/>文件: __init__.py"]
        src_zephyr_factor_core_dag_manager_executor_py["(设计态 / design) D_FACTOR core dag_manager.executor——DAG 调度...<br/>文件: executor.py"]
    end
    src_zephyr_factor_core_dag_manager_executor_py -.->|导入依赖 / import_depends| src_zephyr_factor_core_backpressure_limiter_py
    src_zephyr_factor_alpha_signal_pipeline_py -.->|导入依赖 / import_depends| src_zephyr_factor_core_backpressure
    ...
    src_zephyr_factor_core_backpressure_limiter_py -.->|导入依赖 / import_depends| D_SHARED
...
graph TD
    subgraph D_FACTOR["D_FACTOR 因子"]
        src_zephyr_factor_core_dist_feature_eng["(设计态 / design) "]
        src_zephyr_factor_core_dist_feature_eng_init_py["(生产态 / production) D_FACTOR core dist_feature_eng 子包——分布式特...<br/>文件: __init__.py"]
        src_zephyr_factor_core_dist_feature_eng_engine_py["(设计态 / design) D_FACTOR core dist_feature_eng.engine——分布式...<br/>文件: engine.py"]
        ...
        src_zephyr_factor_core_factor_dag["(设计态 / design) "]
        src_zephyr_factor_core_factor_dag_init_py["(生产态 / production) D_FACTOR core factor_dag 子包——因子 DAG 数据...<br/>文件: __init__.py"]
        src_zephyr_factor_core_factor_dag_dag_py["(设计态 / design) D_FACTOR core factor_dag.dag——因子 DAG 数据结...<br/>文件: dag.py"]
    ...
    src_zephyr_factor_core_dist_feature_eng -.->|导入依赖 / import_depends| src_zephyr_factor_factor_base_py
...
graph TD
    ...
    class D_INFRASTRUCTURE,D_DATA,D_SHARED,D_FUNDAMENTAL_SIGNAL,D_EX_CORE,D_GOV_OPS_RESILIENCE external_prod
```

### 设计态子图（仅 design_maturity=design 的模块和依赖）

> 仅展示蓝图阶段、代码未写的设计态模块（共 12 个，4 条域内依赖）。
...
graph TD
    ...
    classDef external_prod fill:#e8f5e9,stroke:#1b5e20,stroke-width:1px,color:#000
    classDef external_design fill:#fce4ec,stroke:#880e4f,stroke-width:1px,color:#000,stroke-dasharray: 5 5
    class src_zephyr_factor_core_backpressure,src_zephyr_factor_core_backpressure_limiter_py,src_zephyr_factor_core_batch_output,src_zephyr_factor_core_batch_output_buffer_py,src_zephyr_factor_core_config_manager,src_zephyr_factor_core_config_manager_loader_py,src_zephyr_factor_core_dag_manager,src_zephyr_factor_core_dag_manager_executor_py,src_zephyr_factor_core_dist_feature_eng,src_zephyr_factor_core_dist_feature_eng_engine_py,src_zephyr_factor_core_factor_dag,src_zephyr_factor_core_factor_dag_dag_py design
    class D_SHARED,D_DATA,D_INFRASTRUCTURE external_prod
```
...
|:--:|---------|:--:|---------|---------|
| 1 | D_FACTOR core batch_output.buffer——FactorSign... | → | D_DATA 数据接入层: ClickHouse 写入器（MOD-L00-004 §3.2 数据流第6.... | 导入依赖 / import_depends |
| 2 | D-FACTOR-03 因子评估回测运行器——端到端因子评... | → | D_DATA 数据接入层: zephyr.data — 数据源集成器（MOD-L00-004）。 (_... | 导入依赖 / import_depends |
| 3 | D-FACTOR-03 因子评估回测运行器——端到端因子评... | → | D_DATA 数据接入层: ClickHouse 统一读取层（裁定 #ARCH-CH-007）。 (c... | 导入依赖 / import_depends |
| 4 | D-FACTOR-03 因子评估回测运行器——端到端因子评... | → | D_DATA 数据接入层: 表名/品类注册表消费层（裁定 #ARCH-CH-024 Phase ... | 导入依赖 / import_depends |
| 5 | alpha_signal_pipeline.py | → | D_FUNDAMENTAL_SIGNAL 基本面信号: AlphaSignalPipeline D_FACTOR->D_SIGNAL跨层集成... | 导入依赖 / import_depends |
| 6 | D_FACTOR core batch_output.buffer——FactorSign... | → | D_INFRASTRUCTURE 跨层契约基础设施: factor_signal.py | 导入依赖 / import_depends |
| 7 | CTR-001 NormalizedMarketData 消费者——数据适配... | → | D_INFRASTRUCTURE 跨层契约基础设施: market_data.py | 导入依赖 / import_depends |
| 8 | CTR-002 FactorSignal 生产者——信号适配层。 (co... | → | D_INFRASTRUCTURE 跨层契约基础设施: factor_signal.py | 导入依赖 / import_depends |
| 9 | D_FACTOR core backpressure.limiter——进程内在.... | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 导入依赖 / import_depends |
| 10 | D_FACTOR core factor_dag.dag——因子 DAG 数据结... | → | D_SHARED 共享服务: schemas.py | 导入依赖 / import_depends |
| 11 | D-FACTOR-GOV-01 因子生命周期状态机——复用项目.... | → | D_SHARED 共享服务: StateMachine[S] — 通用状态机泛型基类 (MOD-INF-... | 导入依赖 / import_depends |
| 12 | D-FACTOR-GOV-04 六步流程编排——因子从研究到实.... | → | D_SHARED 共享服务: StateMachine[S] — 通用状态机泛型基类 (MOD-INF-... | 导入依赖 / import_depends |
...
> 本域与 6 个外部域直接连接（出边 12 条 + 入边 5 条 = 17 条）。只显示直接连接的域，不展开具体节点。
