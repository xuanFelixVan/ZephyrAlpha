...
graph TD
    ...
    class D_SHARED,D_DATA,D_PF_CORE,D_EX_CORE,D_FUNDAMENTAL_SIGNAL external_prod
```
...
graph TD
    ...
    class D_INFRASTRUCTURE,D_DATA,D_SHARED,D_FUNDAMENTAL_SIGNAL,D_PF_CORE,D_EX_CORE,D_GOV_OPS_RESILIENCE external_prod
```
...
> 本域与 7 个外部域直接连接（出边 12 条 + 入边 8 条 = 20 条）。只显示直接连接的域，不展开具体节点。
