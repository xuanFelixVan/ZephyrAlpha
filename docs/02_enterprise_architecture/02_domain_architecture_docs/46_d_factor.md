...
flowchart TD
    src_zephyr_factor_alpha_signal_pipeline_py["(生产态 / production) 阿尔法信号管线 / alpha_<br/>signal_pipeline<br/>阿尔法信号管线，依赖管线、D-SIGLEGACY-01工作<br/>文件: factor/alpha_signal_pipeline.py"]
    src_zephyr_factor_analysis_correlation_dedup_py["(生产态 / production) D-FACTOR-ANA-05<br/>因子相关性去重——基于相关性矩阵去除冗余因子。 /<br/>correlation_dedup<br/>D-FACTOR-ANA-05<br/>因子相关性去重——基于相关性矩阵去除冗余因子。<br/>文件: analysis/correlation_dedup.py"]
    src_zephyr_factor_analysis_decay_monitor_py["(生产态 / production) D-FACTOR-ANA-08<br/>衰减监控——监控因子 IC 衰减速度，半衰期低于 /<br/>decay_monitor<br/>D-FACTOR-ANA-08 衰减监控——监控因子 IC<br/>衰减速度，半衰期低于阈值告警。<br/>文件: analysis/decay_monitor.py"]
    ...
    src_zephyr_factor_analysis_factor_optimization_py["(生产态 / production) D-FACTOR-ANA-11<br/>因子优化——优化多因子合成权重以最大化目标函数。<br/>/ factor_optimization<br/>D-FACTOR-ANA-11<br/>因子优化——优化多因子合成权重以最大化目标函数。<br/>文件: analysis/factor_optimization.py"]
    src_zephyr_factor_analysis_ic_ir_calc_py["(生产态 / production) D-FACTOR-ANA-01 IC/IR<br/>批量计算器——多因子 IC/IR 指 / ic_ir_calc<br/>D-FACTOR-ANA-01 IC/IR 批量计算器——多因子 IC/IR<br/>指标汇总表。<br/>文件: analysis/ic_ir_calc.py"]
    src_zephyr_factor_analysis_ic_ir_evaluator_py["(生产态 / production) D-FACTOR-ANA-02<br/>多因子评估报告器——批量评估+格式化报告。 / ic_ir_<br/>evaluator<br/>D-FACTOR-ANA-02<br/>多因子评估报告器——批量评估+格式化报告。<br/>文件: analysis/ic_ir_evaluator.py"]
    src_zephyr_factor_analysis_layered_backtest_py["(生产态 / production) D-FACTOR-ANA-06<br/>分层回测——按因子值分组计算各层收益与多空收益差<br/>/ layered_backtest<br/>D-FACTOR-ANA-06<br/>分层回测——按因子值分组计算各层收益与多空收益差。<br/>文件: analysis/layered_backtest.py"]
    ...
    src_zephyr_factor_bus_factor_defense_py["(生产态 / production) 总线因子防御 / bus_factor_<br/>defense<br/>总线因子防御，供MOD-INF-027;MOD-INF-020;MOD-IN使<br/>用<br/>文件: factor/bus_factor_defense.py"]
    ...
    src_zephyr_factor_core_batch_output_init_py["(生产态 / production) 包入口 / __init__<br/>D_FACTOR core batch_output 子包——FactorSignal<br/>批量缓冲写入器。<br/>文件: batch_output/__init__.py"]
    ...
    src_zephyr_factor_core_ctr001_consumer_init_py["(生产态 / production) 包入口 / __init__<br/>CTR-001 NormalizedMarketData 消费者包入口。<br/>文件: ctr001_consumer/__init__.py"]
    ...
    src_zephyr_factor_core_dag_manager_init_py["(生产态 / production) 包入口 / __init__<br/>D_FACTOR core dag_manager 子包——DAG 调度执行器。<br/>文件: dag_manager/__init__.py"]
    ...
    src_zephyr_factor_core_evaluation_init_py["(生产态 / production) D-FACTOR-03<br/>因子评估包——IC/IR/OOS 正率/过拟合检测。 / __<br/>init__<br/>D-FACTOR-03 因子评估包——IC/IR/OOS 正率<br/>/过拟合检测。<br/>文件: evaluation/__init__.py"]
    ...
    src_zephyr_factor_governance_engine_py["(生产态 / production) D-FACTOR-GOV-05<br/>因子治理引擎——顶层编排六步流程+灰度发布。 /<br/>engine<br/>D-FACTOR-GOV-05<br/>因子治理引擎——顶层编排六步流程+灰度发布。<br/>文件: governance/engine.py"]
    src_zephyr_factor_governance_factor_pool_manager_py["(生产态 / production) D-FACTOR-08<br/>因子池容量管理——活跃池/休眠池 + IC末位淘汰 +  /<br/>factor_pool_manager<br/>D-FACTOR-08 因子池容量管理——活跃池/休眠池 +<br/>IC末位淘汰 + 批量裁剪。<br/>文件: governance/factor_pool_manager.py"]
    src_zephyr_factor_momentum_factor_py["(生产态 / production) 动量因子 / D_FACTOR —<br/>Momentum Factor<br/>动量因子。D_FACTOR — Momentum Factor<br/>文件: factor/momentum_factor.py"]
    src_zephyr_factor_value_factor_py["(生产态 / production) 价值因子 / D_FACTOR —<br/>Value Factor<br/>价值因子。D_FACTOR — Value Factor<br/>文件: factor/value_factor.py"]
    ...
    src_zephyr_factor_analysis_init_py["(生产态 / production) 包入口 / __init__<br/>D_FACTOR analysis 子包——因子分析与评估工具链。<br/>文件: analysis/__init__.py"]
    ...
    src_zephyr_factor_analysis_ic_decay_py["(生产态 / production) D-FACTOR-ANA-03 IC<br/>衰减分析——不同 lag 的 IC 衰减曲 / ic_decay<br/>D-FACTOR-ANA-03 IC 衰减分析——不同 lag 的 IC<br/>衰减曲线与半衰期。<br/>文件: analysis/ic_decay.py"]
    ...
    src_zephyr_factor_core_ctr002_producer_converter_py["(生产态 / production) 转换器 / converter<br/>CTR-002 FactorSignal 生产者——信号适配层。<br/>文件: ctr002_producer/converter.py"]
    src_zephyr_factor_governance_grayscale_rollout_py["(生产态 / production) D-FACTOR-GOV-03<br/>灰度发布——管理因子从 10% → 30% →  / grayscale_<br/>rollout<br/>D-FACTOR-GOV-03 灰度发布——管理因子从 10% → 30%<br/>→ 100% 的放量阶梯。<br/>文件: governance/grayscale_rollout.py"]
    src_zephyr_factor_governance_six_step_flow_py["(生产态 / production) D-FACTOR-GOV-04<br/>六步流程编排——因子从研究到实盘的治理流程。 /<br/>six_step_flow<br/>D-FACTOR-GOV-04<br/>六步流程编排——因子从研究到实盘的治理流程。<br/>文件: governance/six_step_flow.py"]
    ...
    src_zephyr_factor_governance_abs001_gate_py["(生产态 / production) D-FACTOR-GOV-02 ABS001<br/>上线门禁——因子进入灰度前的质量检 / abs001_gate<br/>D-FACTOR-GOV-02 ABS001<br/>上线门禁——因子进入灰度前的质量检查。<br/>文件: governance/abs001_gate.py"]
    src_zephyr_factor_governance_lifecycle_state_machine_py["(生产态 / production) 生命周期状态machine /<br/>lifecycle_state_machine<br/>D-FACTOR-GOV-01 因子生命周期状态机——复用项目级<br/>StateMachine 泛型基类。<br/>文件: governance/lifecycle_state_machine.py"]
    src_zephyr_factor_governance_abs001_gate_py ~~~ src_zephyr_factor_governance_lifecycle_state_machine_py
    src_zephyr_factor_core_evaluation_backtest_py["(生产态 / production) D-FACTOR-03<br/>因子评估回测运行器——端到端因子评估。 / backtest<br/>D-FACTOR-03 因子评估回测运行器——端到端因子评估。<br/>文件: evaluation/backtest.py"]
    ...
    src_zephyr_factor_core_evaluation_metrics_py["(生产态 / production) D-FACTOR-03<br/>因子评估指标——纯函数模块（无 IO 依赖）。 /<br/>metrics<br/>D-FACTOR-03 因子评估指标——纯函数模块（无 IO<br/>依赖）。<br/>文件: evaluation/metrics.py"]
    src_zephyr_factor_factor_base_py["(生产态 / production) 因子基类 / ZephyrAlpha —<br/>D_FACTOR Alpha Factor Layer<br/>因子基类。ZephyrAlpha — D_FACTOR Alpha Factor<br/>Layer<br/>文件: factor/factor_base.py"]
