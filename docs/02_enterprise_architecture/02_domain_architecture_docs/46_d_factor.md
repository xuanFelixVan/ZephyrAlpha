...
flowchart TD
    ...
    D_RISK -->|导入依赖 / import_depends| src_zephyr_factor_analysis_ic_decay_py
    ...
    class D_SHARED,D_INFRA_RUNTIME,D_DATA,D_INFRASTRUCTURE,D_RISK,D_EX_CORE,D_BACKTEST,D_GOVERNANCE,D_PF_CORE,D_GOV_OPS_RESILIENCE,D_FUNDAMENTAL_SIGNAL external_prod
...
| 16 | D_RISK 风控: D_RISK — Model Risk Auditor (MOD-RK-18) (core/model_risk... | → | D-FACTOR-ANA-03 IC 衰减分析——不同 lag 的 IC 衰减曲 / ic... | 导入依赖 / import_depends |
| 17 | D_SELL_DECISION 卖出决策: T交易协调器 / T Trade Coordinator (core/t_trade_coordinat... | → | 3秒拉 tick → DataFrame → DagExecutor → H1 Redis / Intr... | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 13 个外部域直接连接（出边 23 条 + 入边 17 条 = 40 条）。只显示直接连接的域，不展开具体节点。
