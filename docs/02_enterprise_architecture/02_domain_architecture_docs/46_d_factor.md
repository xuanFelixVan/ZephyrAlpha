...
flowchart TD
    src_zephyr_factor_alpha_signal_pipeline_py["(生产态 / production) 阿尔法信号管线 / alpha_signal_pipeline<br/>阿尔法信号管线，依赖管线、D-SIGLEGACY-01工作<br/>文件: factor/alpha_signal_pipeline.py"]
    ...
    src_zephyr_factor_bus_factor_defense_py["(生产态 / production) 总线因子防御 / bus_factor_defense<br/>总线因子防御，供MOD-INF-027;MOD-INF-020;MOD-IN使用<br/>文件: factor/bus_factor_defense.py"]
    src_zephyr_factor_core_backpressure_init_py["(生产态 / production) 包入口 / __init__<br/>D_FACTOR core backpressure 子包——进程内在途并发限流器。<br/>文件: backpressure/__init__.py"]
    src_zephyr_factor_core_batch_output_init_py["(生产态 / production) 包入口 / __init__<br/>D_FACTOR core batch_output 子包——FactorSignal 批量缓冲写入器。<br/>文件: batch_output/__init__.py"]
    src_zephyr_factor_core_config_manager_init_py["(生产态 / production) 包入口 / __init__<br/>D_FACTOR core config_manager 子包——core 基础设施模块策略参数加载器。<br/>文件: config_manager/__init__.py"]
    ...
    src_zephyr_factor_core_dist_feature_eng_init_py["(生产态 / production) 包入口 / __init__<br/>D_FACTOR core dist_feature_eng 子包——分布式特征工程引擎。<br/>文件: dist_feature_eng/__init__.py"]
    ...
    src_zephyr_factor_core_factor_dag_init_py["(生产态 / production) 包入口 / __init__<br/>D_FACTOR core factor_dag 子包——因子 DAG 数据结构 + Kahn 拓扑分层。<br/>文件: factor_dag/__init__.py"]
    ...
    src_zephyr_factor_core_ctr001_consumer_converter_py["(生产态 / production) 转换器 / converter<br/>CTR-001 NormalizedMarketData 消费者——数据适配层。<br/>文件: ctr001_consumer/converter.py"]
    ...
    src_zephyr_factor_governance_lifecycle_state_machine_py["(生产态 / production) 生命周期状态machine / lifecycle_state_machine<br/>D-FACTOR-GOV-01 因子生命周期状态机——复用项目级 StateMachine 泛型基类。<br/>文件: governance/lifecycle_state_machine.py"]
    ...
    src_zephyr_factor_governance_init_py["(生产态 / production) 包入口 / __init__<br/>D_FACTOR governance 子包——因子生命周期治理工具链。<br/>文件: governance/__init__.py"]
