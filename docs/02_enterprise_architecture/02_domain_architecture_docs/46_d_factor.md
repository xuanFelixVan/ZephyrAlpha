...
flowchart TD
    ...
    class D_FUNDAMENTAL_SIGNAL,D_DATA,D_INFRASTRUCTURE,D_INFRA_RUNTIME,D_SHARED,D_EX_CORE,D_GOV_OPS_RESILIENCE,D_PF_CORE external_prod
```
...
> 本域与 8 个外部域直接连接（出边 14 条 + 入边 10 条 = 24 条）。只显示直接连接的域，不展开具体节点。
