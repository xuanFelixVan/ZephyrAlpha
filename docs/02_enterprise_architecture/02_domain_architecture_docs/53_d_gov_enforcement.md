...
flowchart TD
    ...
    class D_GOV_RULE,D_GOVERNANCE,D_AUTONOMY_CORE,D_SECURITY,D_GOV_CODE_QUALITY,D_GOV_SCRIPTS external_prod
...
| 129 | D_GOV_SCRIPTS 脚本治理: concurrent_commit_test.py — 幽灵提交红蓝对抗脚本（OPS-20... | → | GitCommitGateway — 全项目唯一合法 git commit 入口（OPS-2... | 导入依赖 / import_depends |
| 130 | D_ML_TRAIN 训练: training_dataset_manager/ | → | D_DATA — Data Quality Gate (rule_enforcement/quality_gat... | data / data |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 16 个外部域直接连接（出边 159 条 + 入边 130 条 = 289 条）。只显示直接连接的域，不展开具体节点。
