...
flowchart TD
    ...
    class D_SHARED,D_GOV_AUDIT,D_GOV_OPS_RESILIENCE,D_OPS,D_GOVERNANCE,D_SECURITY,D_GOV_SCRIPTS,D_GOV_CODE_QUALITY,D_DATA external_prod
```
...
| 111 | D_GOV_SCRIPTS 脚本治理: 并发提交测试 / concurrent_commit_test (repair/concurrent_... | → | Git提交网关 / git_commit_gateway (rule_bridge/git_commit_... | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 13 个外部域直接连接（出边 70 条 + 入边 111 条 = 181 条）。只显示直接连接的域，不展开具体节点。
