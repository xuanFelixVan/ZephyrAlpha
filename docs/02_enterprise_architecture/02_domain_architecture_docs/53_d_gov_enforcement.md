...
flowchart TD
    docs_01_policies_and_standards_registry_catalogs_rule_enforcement_registry_yaml["(生产态 / production) 规则执行注册表 / rule_<br/>enforcement_registry<br/>规则enforcement注册表，规则执行的注册表，登记和<br/>查询已注册的条目。<br/>文件: catalogs/rule_enforcement_registry.yaml"]
    scripts_governance_d8_doc_sync_metric_count_drift_reconciler_py["(生产态 / production) 指标数量漂移协调器 /<br/>metric_count_drift_reconciler<br/>dashboard 指标数描述派生校验 reconciler<br/>文件: d8_doc_sync/metric_count_drift_<br/>reconciler.py"]
    scripts_governance_d8_doc_sync_readme_version_sync_reconciler_py["(生产态 / production) readme版本sync对账器 /<br/>readme_version_sync_reconciler<br/>README 版本号派生展示校验 reconciler<br/>文件: d8_doc_sync/readme_version_sync_<br/>reconciler.py"]
    scripts_governance_d8_doc_sync_requirements_version_sync_reconciler_py["(生产态 / production)<br/>requirements版本sync对账器 / requirements_<br/>version_sync_reconciler<br/>requirements版本sync对账器.txt ↔ pyproject.toml<br/>依赖一致性校验 reconciler<br/>文件: d8_doc_sync/requirements_version_sync_<br/>reconciler.py"]
    scripts_governance_session_worktree_cli_py["(生产态 / production) 会话worktreecli / session_<br/>worktree_cli<br/>会话 worktree 管理 CLI<br/>（治本遗留项#2，2026-07-17）<br/>文件: governance/session_worktree_cli.py"]
    src_zephyr_gov_enforcement_init_py["(生产态 / production) 包入口 / __init__<br/>gov_enforcement package — 执行治理域（D_GOV_<br/>ENFORCEMENT）<br/>文件: gov_enforcement/__init__.py"]
    src_zephyr_gov_enforcement_behavioral_admission_init_py["(生产态 / production) 包入口 / __init__<br/>治理执行的包入口，把这一层的子模块归到一起统一管<br/>理，用到谁才加载谁，避免一次性全加载拖慢启动。<br/>文件: behavioral_admission/__init__.py"]
    src_zephyr_gov_enforcement_commit_gates_depgraph_pre_registration_gate_py["(设计态 / design) depgraphpreregistration门禁 /<br/>depgraph_pre_registration_gate<br/>depgraph planned→production 流转强制门禁<br/>文件: commit_gates/depgraph_pre_registration_<br/>gate.py<br/>⛔ 治理执行域，设计已就绪，等待开发排期"]
    src_zephyr_gov_enforcement_commit_gates_stash_accumulation_gate_py["(生产态 / production) stashaccumulation门禁 /<br/>stash_accumulation_gate<br/>stash 堆积阈值检测门禁<br/>文件: commit_gates/stash_accumulation_gate.py"]
    src_zephyr_gov_enforcement_rule_enforcement_approval_py["(生产态 / production) 审批 / G-CT-004 —<br/>Backward-compat re-export of ApprovalRequest<br/>from<br/>审批，供zephyr.governance.services.ada使用<br/>文件: rule_enforcement/approval.py"]
    src_zephyr_gov_enforcement_rule_enforcement_compliance_rule_py["(生产态 / production) 合规规则 / compliance_rule<br/>Re-export shim — ComplianceRule 真源已合并至<br/>zephyr.shared.contracts.compliance_rule。<br/>文件: rule_enforcement/compliance_rule.py"]
    src_zephyr_gov_enforcement_rule_enforcement_default_quality_gate_py["(生产态 / production) 默认质量门禁 / D_DATA —<br/>Default Data Quality Gate<br/>默认质量门禁。D_DATA — Default Data Quality Gate<br/>文件: rule_enforcement/default_quality_gate.py"]
    src_zephyr_gov_enforcement_rule_enforcement_dlq_retry_policy_py["(生产态 / production) dlq重试策略 / dlq_retry_<br/>policy<br/>DLQ 重试策略 — 对接 shared/events<br/>/dlq.DeadLetterQueue 的真重试。<br/>文件: rule_enforcement/dlq_retry_policy.py"]
    src_zephyr_gov_enforcement_rule_enforcement_output_quality_gate_py["(生产态 / production) output质量门禁 / output_<br/>quality_gate<br/>output质量门禁，提供包入口和模块加载功能<br/>文件: rule_enforcement/output_quality_gate.py"]
    src_zephyr_gov_enforcement_rule_enforcement_pre_flight_gate_py["(生产态 / production) preflight门禁 / pre_<br/>flight_gate<br/>preflight门禁，依赖预算模型、预算引擎工作<br/>文件: rule_enforcement/pre_flight_gate.py"]
    src_zephyr_gov_enforcement_rule_enforcement_rule_engine_rule_canary_manager_py["(生产态 / production) 规则金丝雀管理器 / rule_<br/>canary_manager<br/>Rule Canary Manager — v0.10.0 规则金丝雀:<br/>1%用户先上新规则->A/B对比->rollback。<br/>文件: rule_engine/rule_canary_manager.py"]
    src_zephyr_gov_enforcement_rule_enforcement_rule_engine_rule_debt_auditor_py["(生产态 / production) 规则debt审计器 / rule_<br/>debt_auditor<br/>Rule Debt Auditor — v0.7.0 规则债务审计器:<br/>分析escalation_rules.yaml维护债务指标。<br/>文件: rule_engine/rule_debt_auditor.py"]
    src_zephyr_gov_enforcement_rule_enforcement_rule_engine_rule_shadow_runner_py["(生产态 / production) 规则影子运行器 / rule_<br/>shadow_runner<br/>Rule Shadow Runner — v0.10.0 规则影子模式:<br/>新规则shadow运行3天->diff old vs new->promote。<br/>文件: rule_engine/rule_shadow_runner.py"]
    src_zephyr_gov_enforcement_rule_enforcement_rule_engine_rule_watcher_py["(生产态 / production) 规则监视器 / rule_watcher<br/>RuleWatcher — YAML 规则文件变更检测与自动同步<br/>文件: rule_engine/rule_watcher.py"]
    src_zephyr_gov_enforcement_rule_enforcement_slo_contract_py["(生产态 / production) SLO契约 / SLO-Driven<br/>Escalation Contract — D-022-12.<br/>SLO契约。SLO-Driven Escalation Contract —<br/>D-022-12.<br/>文件: rule_enforcement/slo_contract.py"]
    tests_governance_commit_gates_test_create_guard_py["(生产态 / production) 测试创建守卫 / test_<br/>create_guard<br/>CREATE-GUARD 门禁单元测试（2026-06-30 治本补全）<br/>文件: commit_gates/test_create_guard.py"]
    tests_governance_commit_gates_test_r5_digit_suffix_gate_py["(生产态 / production) 测试r5digitsuffix门禁 /<br/>test_r5_digit_suffix_gate<br/>R5-DIGIT-SUFFIX 门禁单元测试<br/>文件: commit_gates/test_r5_digit_suffix_gate.py"]
    tests_governance_rule_bridge_test_claim_files_for_edit_py["(生产态 / production) 测试claimfilesforedit /<br/>test_claim_files_for_edit<br/>P2-2 并发 session 文件级原子性测试<br/>文件: rule_bridge/test_claim_files_for_edit.py"]
    tests_governance_rule_bridge_test_emergency_commit_py["(生产态 / production) 测试紧急提交 / test_<br/>emergency_commit<br/>emergency_commit API 测试<br/>（Ruling:100PCT-AI-GOVERNANCE P2-1）<br/>文件: rule_bridge/test_emergency_commit.py"]
    tests_governance_rule_bridge_test_heartbeat_daemon_py["(生产态 / production) 测试heartbeatdaemon /<br/>test_heartbeat_daemon<br/>heartbeat daemon + 成本递增 smoke test<br/>（Ruling:100PCT-AI-GOVERNANCE P3-1）<br/>文件: rule_bridge/test_heartbeat_daemon.py"]
    ...
    src_zephyr_gov_enforcement_behavioral_admission_admission_response_py["(生产态 / production) 准入响应 / admission_<br/>response<br/>准入响应，供GovernanceServer;run_all.py使用<br/>文件: behavioral_admission/admission_response.py"]
    src_zephyr_gov_enforcement_behavioral_admission_code_review_ai_py["(生产态 / production) 代码审查AI / code_review_<br/>ai<br/>代码审查AI，治理执行的核心类，封装ReviewLevel相<br/>关逻辑。<br/>文件: behavioral_admission/code_review_ai.py"]
    src_zephyr_gov_enforcement_behavioral_admission_gate_event_adapter_py["(生产态 / production) 门禁事件适配器 / gate_<br/>event_adapter<br/>GateEventAdapter — GateRepo 事件适配器<br/>（DW-0006）<br/>文件: behavioral_admission/gate_event_adapter.py"]
    src_zephyr_gov_enforcement_behavioral_admission_gpu_consensus_scheduler_py["(生产态 / production) GPU共识调度器 / gpu_<br/>consensus_scheduler<br/>GPU共识调度器，IO的调度器，按时间或优先级安排任<br/>务。<br/>文件: behavioral_admission/gpu_consensus_<br/>scheduler.py"]
    src_zephyr_gov_enforcement_behavioral_admission_protection_index_py["(生产态 / production) 保护索引 / protection_<br/>index<br/>保护索引，供zephyr.gov_enforcement.behavio使用<br/>文件: behavioral_admission/protection_index.py"]
    src_zephyr_gov_enforcement_rule_bridge_commit_gate_registry_py["(生产态 / production) 提交门禁注册表 / commit_<br/>gate_registry<br/>GitCommitGateway pre-commit 门禁注册表<br/>（架构债务 #AD-001 治本）<br/>文件: rule_bridge/commit_gate_registry.py"]
    src_zephyr_gov_enforcement_rule_bridge_session_worktree_py["(生产态 / production) 会话worktree / session_<br/>worktree<br/>AI 对话 worktree 物理隔离 helper<br/>（FP-ISO.4C，2026-07-01 治本）<br/>文件: rule_bridge/session_worktree.py"]
    src_zephyr_gov_enforcement_rule_enforcement_quality_gate_py["(生产态 / production) 质量门禁 / D_DATA — Data<br/>Quality Gate<br/>质量门禁。D_DATA — Data Quality Gate<br/>文件: rule_enforcement/quality_gate.py"]
    ...
    src_zephyr_gov_enforcement_behavioral_admission_admission_controller_py["(生产态 / production) 准入控制器 / admission_<br/>controller<br/>准入控制器，IO的控制器，协调组件按流程执行。<br/>文件: behavioral_admission/admission_<br/>controller.py"]
    src_zephyr_gov_enforcement_behavioral_admission_verdict_engine_py["(生产态 / production) 裁定引擎 / verdict_engine<br/>verdict引擎，治理执行的事件，定义和分发事件。<br/>文件: behavioral_admission/verdict_engine.py"]
    src_zephyr_gov_enforcement_rule_bridge_emergency_commit_py["(生产态 / production) 紧急提交 / emergency_<br/>commit<br/>紧急提交通道（Ruling:100PCT-AI-GOVERNANCE<br/>P2-1，2026-07-19）<br/>文件: rule_bridge/emergency_commit.py"]
    src_zephyr_gov_enforcement_rule_bridge_heartbeat_daemon_py["(生产态 / production) 心跳守护 / heartbeat_<br/>daemon<br/>心跳守护.py — session heartbeat 独立进程<br/>（Ruling:100PCT-AI-GOVERNANCE P3-1，2026-07-20）<br/>文件: rule_bridge/heartbeat_daemon.py"]
    src_zephyr_gov_enforcement_rule_bridge_session_claim_py["(生产态 / production) 会话claim / session_claim<br/>AI 对话并发声明 helper（FP-ISO.4B<br/>件2改，2026-07-01 治本）<br/>文件: rule_bridge/session_claim.py"]
    src_zephyr_gov_enforcement_rule_bridge_worktree_manager_py["(生产态 / production) worktree管理器 / worktree_<br/>manager<br/>会话 worktree 物理隔离管理器（阶段3 治本 stash<br/>循环）<br/>文件: rule_bridge/worktree_manager.py"]
    src_zephyr_gov_enforcement_rule_bridge_worktree_pool_py["(生产态 / production) worktree池 / worktree_pool<br/>Worktree 预创建池（ARCH-GIT-CALL-BUDGET<br/>P3.3，2026-07-19）<br/>文件: rule_bridge/worktree_pool.py"]
    ...
    src_zephyr_gov_enforcement_rule_bridge_git_commit_gateway_py["(生产态 / production) Git提交网关 / git_commit_<br/>gateway<br/>GitCommitGateway — 全项目唯一合法 git commit<br/>入口（OPS-2026062512 治本）<br/>文件: rule_bridge/git_commit_gateway.py"]
    src_zephyr_gov_enforcement_rule_bridge_batched_auto_committer_py["(生产态 / production) batched自动committer /<br/>batched_auto_committer<br/>Reconciler 批量化 auto-commit 拦截器<br/>（ARCH-GIT-CALL-BUDGET P2.3，2026-07-19）<br/>文件: rule_bridge/batched_auto_committer.py"]
    ...
    class D_GOV_AUDIT,D_GOV_CODE_QUALITY,D_GOVERNANCE,D_OPS,D_GOV_SCRIPTS,D_SHARED,D_GOV_OPS_RESILIENCE,D_SECURITY,D_DATA external_prod
```
