...
graph TD
    ...
    class D_GOV_DRIFT,D_SHARED,D_INTEGRATION,D_GOV_AUDIT,D_INFRA_RECOVERY,D_GOV_OPS_RESILIENCE,D_INTELLIGENCE,D_GOVERNANCE,D_GOV_KB external_prod
...
| 13 | task_types.py | → | D_INTEGRATION 管线路由: base_config.py | 导入依赖 / import_depends |
| 14 | task_types.py | → | D_INTEGRATION 管线路由: execution_model.py | 导入依赖 / import_depends |
| 15 | task_types.py | → | D_INTEGRATION 管线路由: severity_types.py | 导入依赖 / import_depends |
| 16 | ZephyrAlpha — gates/ai_capability_guard.py (ai... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
