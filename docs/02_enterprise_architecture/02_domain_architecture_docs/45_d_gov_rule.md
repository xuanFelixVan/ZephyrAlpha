...
graph TD
    ...
    class D_SHARED,D_INTEGRATION,D_GOV_DRIFT,D_GOV_AUDIT,D_INFRA_RECOVERY,D_GOV_ENFORCEMENT,D_GOV_OPS_RESILIENCE,D_INTELLIGENCE,D_GOVERNANCE,D_GOV_KB external_prod
...
graph TD
    ...
    class D_GOVERNANCE,D_INTEGRATION,D_INFRA_RUNTIME,D_SHARED external_prod
    class D_GOV_SCRIPTS,D_SECURITY external_design
```
...
graph TD
    ...
    class D_SHARED,D_INTEGRATION,D_GOVERNANCE,D_GOV_DRIFT,D_GOV_AUDIT,D_INFRA_RECOVERY,D_INFRA_RUNTIME,D_GOV_OPS_RESILIENCE,D_INTELLIGENCE external_prod
...
> 本域与 14 个外部域直接连接（出边 29 条 + 入边 35 条 = 64 条）。只显示直接连接的域，不展开具体节点。
