...
graph TD
    ...
    class D_SHARED,D_GOV_AUDIT,D_GOV_DRIFT,D_INFRA_RECOVERY,D_GOVERNANCE,D_AUTONOMY_CORE,D_GOV_OPS_RESILIENCE external_prod
    class D_GOV_SCRIPTS,D_INTELLIGENCE,D_SECURITY,D_INTEGRATION external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_INFRA_RUNTIME,D_GOV_AUDIT,D_GOV_DRIFT,D_INTEGRATION external_prod
    class D_GOV_SCRIPTS,D_INTELLIGENCE,D_SECURITY external_design
```
...
> 本域与 13 个外部域直接连接（出边 29 条 + 入边 29 条 = 58 条）。只显示直接连接的域，不展开具体节点。
