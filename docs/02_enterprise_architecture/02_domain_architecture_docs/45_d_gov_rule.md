...
graph TD
    ...
    class D_SHARED,D_GOV_DRIFT,D_INFRA_RECOVERY,D_GOV_AUDIT,D_GOV_OPS_RESILIENCE,D_INTELLIGENCE,D_GOVERNANCE,D_GOV_KB external_prod
    class D_GOV_ENFORCEMENT,D_GOV_SCRIPTS,D_SECURITY external_design
```
...
graph TD
    ...
    class D_SHARED,D_GOV_DRIFT,D_INFRA_RECOVERY,D_INFRA_RUNTIME,D_GOV_OPS_RESILIENCE,D_INTELLIGENCE,D_GOVERNANCE,D_INTEGRATION external_prod
...
| 8 | rule_engine package — 规则引擎模块集合（ARCH-0... | → | D_GOV_ENFORCEMENT 规则执行: RuleWatcher — YAML 规则文件变更检测与自动同步 ... | config_depends / config_depends |
...
| 24 | zero_residue_check.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 25 | task_types.py | → | D_SHARED 共享服务: base_config.py | 导入依赖 / import_depends |
| 26 | task_types.py | → | D_SHARED 共享服务: execution_model.py | 导入依赖 / import_depends |
| 27 | task_types.py | → | D_SHARED 共享服务: severity_types.py | 导入依赖 / import_depends |
...
> 本域与 14 个外部域直接连接（出边 30 条 + 入边 35 条 = 65 条）。只显示直接连接的域，不展开具体节点。
