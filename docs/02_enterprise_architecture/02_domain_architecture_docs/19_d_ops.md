...
| 2 | Budget Enforcer core engine — MOD-INF-024 (bud... | → | D_GOV_DRIFT 漂移检测: drift_infrastructure.py | 导入依赖 / import_depends |
...
> 本域与 9 个外部域直接连接（出边 10 条 + 入边 44 条 = 54 条）。只显示直接连接的域，不展开具体节点。
