...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_DATA external_prod
    class D_INFRA_RUNTIME,D_AUDITTEST,D_EX_CORE,D_FRONTEND,D_INTELLIGENCE external_design
```
...
graph TD
    ...
    class D_ML_TRAIN,D_FRONTEND external_design
```
...
> 本域与 9 个外部域直接连接（出边 10 条 + 入边 21 条 = 31 条）。只显示直接连接的域，不展开具体节点。
