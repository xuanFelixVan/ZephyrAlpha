...
flowchart TD
    ...
    src_zephyr_orchestrator_agent_health_monitor_py["代理健康监控<br/>1. **三态健康判定**：HEALTHY / DEGRADED /<br/>UNHEALTHY<br/>agent_health_monitor<br/>文件: orchestrator/agent_health_monitor.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_contracts_init_py["orchestrator/contracts 包入口<br/>编排的包入口。contracts — orchestrator<br/>contracts subpackage.，包入口。contracts —<br/>orchestrator contracts<br/>subpackage.相关功能的实现。<br/>文件: contracts/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_contracts_design_decisions_py["设计decisions<br/>编排/契约包的design_decisions模块<br/>文件: contracts/design_decisions.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_core_init_py["orchestrator/core 包入口<br/>编排的包入口。orchestrator.core —<br/>auto-generated package<br/>init.，包入口。orchestrator.core —<br/>auto-generated package init.相关功能的实现。<br/>文件: core/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_execution_init_py["orchestrator/execution 包入口<br/>执行的包入口。execution — orchestrator<br/>execution subpackage.，包入口。execution —<br/>orchestrator execution<br/>subpackage.相关功能的实现。<br/>文件: execution/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_execution_dlq_manager_py["dlq管理器<br/>执行的依据：MOD-MASTER-002 蓝图<br/>§十六，依据：MOD-MASTER-002 蓝图<br/>§十六相关功能的实现。<br/>dlq_manager<br/>文件: execution/dlq_manager.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_execution_reconciliation_loop_py["对账循环<br/>编排/执行包的reconciliation_loop模块<br/>文件: execution/reconciliation_loop.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_fault_tolerance_init_py["orchestrator/fault_tolerance 包入口<br/>编排的包入口。fault_tolerance — orchestrator<br/>fault_tolerance<br/>subpackage.，包入口。fault_tolerance —<br/>orchestrator fault_tolerance<br/>subpackage.相关功能的实现。<br/>文件: fault_tolerance/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_fault_tolerance_canary_manager_py["金丝雀发布管理器<br/>（CT-CANARY）——权重分流+指标对比+自动回滚。<br/>编排的金丝雀发布管理器<br/>（CT-CANARY）——权重分流+指标对比+自动回滚。，金<br/>丝雀发布管理器（CT-CANARY）——权重分流+指标对比+<br/>自动回滚。相关功能的实现。<br/>canary_manager<br/>文件: fault_tolerance/canary_manager.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_fault_tolerance_chaos_hooks_py["chaos钩子<br/>编排的chaos钩子。ChaosHook — integrates<br/>ChaosEngine with the orchestrator execution<br/>loop.，chaos钩子。ChaosHook — integrates<br/>ChaosEngine with the orchestrator execution<br/>loop.相关功能的实现。<br/>文件: fault_tolerance/chaos_hooks.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_governance_init_py["orchestrator/governance 包入口<br/>治理的包入口。governance — orchestrator<br/>governance subpackage.，包入口。governance —<br/>orchestrator governance<br/>subpackage.相关功能的实现。<br/>文件: governance/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_governance_risk_registry_py["风险注册表<br/>编排/治理包的risk_registry模块<br/>文件: governance/risk_registry.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_lifecycle_init_py["orchestrator/lifecycle 包入口<br/>编排的包入口。lifecycle — orchestrator<br/>lifecycle subpackage.，包入口。lifecycle —<br/>orchestrator lifecycle<br/>subpackage.相关功能的实现。<br/>文件: lifecycle/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_lifecycle_startup_sequencer_py["启动sequencer<br/>编排/lifecycle包的startup_sequencer模块<br/>文件: lifecycle/startup_sequencer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_lifecycle_teardown_manager_py["teardown管理器<br/>编排/lifecycle包的teardown_manager模块<br/>文件: lifecycle/teardown_manager.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_quality_blind_spot_closure_py["盲点闭合<br/>编排/quality包的blind_spot_closure模块<br/>文件: quality/blind_spot_closure.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_quality_ke_quality_py["知识质量评分契约<br/>（CT-KE-QUALITY）——KE完整性+准确性+时效性三维<br/>编排的知识质量评分契约<br/>（CT-KE-QUALITY）——KE完整性+准确性+时效性三维评<br/>分。，知识质量评分契约<br/>（CT-KE-QUALITY）——KE完整性+准确性+时效性三维评<br/>分。相关功能的实现。<br/>ke_quality<br/>文件: quality/ke_quality.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_quality_lean_scanner_py["死代码/孤儿文件/僵尸引用三扫描<br/>（CT-LEAN）——三款扫描器+自动化清理建议<br/>编排的死代码/孤儿文件/僵尸引用三扫描<br/>（CT-LEAN）——三款扫描器+自动化清理建议。，死代码<br/>/孤儿文件/僵尸引用三扫描<br/>（CT-LEAN）——三款扫描器+自动化清理建议。相关功能<br/>的实现。<br/>lean_scanner<br/>文件: quality/lean_scanner.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_task_queue_py["任务队列<br/>编排包的task_queue模块<br/>文件: orchestrator/task_queue.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_core_task_queue_py["任务队列<br/>编排/核心包的task_queue模块<br/>文件: core/task_queue.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_execution_memory_writer_py["Orc->VMS 记忆写入器<br/>执行的Orc->VMS 记忆写入器，Orc->VMS<br/>记忆写入器相关功能的实现。<br/>memory_writer<br/>文件: execution/memory_writer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_fault_tolerance_fault_types_py["故障类型定义<br/>编排的fault类型定义。Fault type registry and<br/>preset templates for chaos<br/>engineering.，fault类型定义。Fault type<br/>registry and preset templates for chaos<br/>engineering.相关功能的实现。<br/>文件: fault_tolerance/fault_types.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_quality_init_py["orchestrator/quality 包入口<br/>编排的包入口。quality — orchestrator quality<br/>subpackage.，包入口。quality — orchestrator<br/>quality subpackage.相关功能的实现。<br/>文件: quality/__init__.py<br/>(生产态 / production)"]
    ...
    class D_SHARED,D_GOV_OPS_RESILIENCE,D_AUTONOMY_CORE,D_TRADING,D_GOV_SCRIPTS,D_INFRA_RUNTIME,D_FEEDBACK_LOOP external_prod
