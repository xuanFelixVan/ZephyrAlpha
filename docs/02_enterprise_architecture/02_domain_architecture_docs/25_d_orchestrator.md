...
graph TD
    ...
    src_zephyr_orchestrator_agent_orchestrator_py -->|导入依赖 / import_depends| D_SECURITY_LLM
    src_zephyr_orchestrator_agent_orchestrator_py -->|导入依赖 / import_depends| D_SECURITY_LLM
    ...
    D_STRUCT_SCRIPTS -->|导入依赖 / import_depends| src_zephyr_orchestrator_contracts_contract_registry_py
    D_TRADING["(生产态 / production) D_TRADING"]
    D_TRADING -->|导入依赖 / import_depends| src_zephyr_orchestrator_core_task_queue_py
    D_TRADING -->|导入依赖 / import_depends| src_zephyr_orchestrator_execution_memory_writer_py
    ...
    class D_SHARED,D_INFRA_RUNTIME,D_SECURITY_LLM,D_GOVERNANCE,D_STRUCT_SCRIPTS,D_TRADING,D_AUTONOMY_CORE,D_FEEDBACK_LOOP external_prod
```
...
graph TD
    ...
    src_zephyr_orchestrator_fault_tolerance_chaos_hooks_py -->|导入依赖 / import_depends| D_CONTRACTS
    ...
    class D_SHARED,D_CONTRACTS external_prod
```
...
graph TD
    ...
    D_CONTRACTS["(生产态 / production) D_CONTRACTS"]
    src_zephyr_orchestrator_contracts_alert_handler_py -->|导入依赖 / import_depends| D_CONTRACTS
    ...
    class D_SECURITY_LLM,D_SHARED,D_CONTRACTS,D_GOVERNANCE,D_STRUCT_SCRIPTS,D_TRADING,D_AUTONOMY_CORE,D_FEEDBACK_LOOP external_prod
```
...
| 3 | Orc 告警接收器 — handle_alert() 消费者 (alert_... | → | D_CONTRACTS 共享契约: TaskRepositoryProtocol — TaskRepository 的 Pro... | 导入依赖 / import_depends |
| 4 | ActiveTaskQueue — 后台任务轮询与自动分发 (task... | → | D_CONTRACTS 共享契约: TaskRepositoryProtocol — TaskRepository 的 Pro... | 导入依赖 / import_depends |
| 5 | BatchOrchestrator — 多 Worker 批量任务协调器（... | → | D_CONTRACTS 共享契约: TaskRepositoryProtocol — TaskRepository 的 Pro... | 导入依赖 / import_depends |
| 6 | ChaosHook — integrates ChaosEngine with the or... | → | D_CONTRACTS 共享契约: orchestration_protocol.py | 导入依赖 / import_depends |
| 7 | TriggerRouter — RI-03 触发路由器（M3 跨模块触.... | → | D_FEEDBACK_LOOP 反馈循环引擎: Feedback Loop Decision Engine (decision_engine.py) | 导入依赖 / import_depends |
| 8 | Orc 告警接收器 — handle_alert() 消费者 (alert_... | → | D_GOVERNANCE 生命周期管理: SQLite 元数据层 Schema DDL + 版本化迁移框架（T-... | 导入依赖 / import_depends |
| 9 | Orc 告警接收器 — handle_alert() 消费者 (alert_... | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 导入依赖 / import_depends |
| 10 | CT-ORC-SCRIPT-001 运行时桥接 (finding_bridge.py) | → | D_GOVERNANCE 生命周期管理: TaskRepository — 任务登记表 CRUD + 状态机（T-1... | 导入依赖 / import_depends |
| 11 | TriggerRouter — RI-03 触发路由器（M3 跨模块触.... | → | D_GOV_DRIFT 漂移检测: Gate-side Drift Detector Recovery — zephyr.gov... | 导入依赖 / import_depends |
| 12 | FailurePatternMatcher — 任务失败模式识别与纠正... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: EventHook — 声明式任务系统事件订阅 (event_hook.py) | 导入依赖 / import_depends |
| 13 | AgentOrchestrator · 多角色 Agent 路由、工具链.... | → | D_INFRA_RUNTIME 运行时集成: token_budget.py — Token 估算工具 SSoT (token_b... | 导入依赖 / import_depends |
| 14 | Orc->Script 脚本执行器 — run_audit() 生产者 (s... | → | D_INFRA_RUNTIME 运行时集成: Script->Gate 门禁桥接器 — submit_findings() 生... | 导入依赖 / import_depends |
| 15 | Orc->VMS 记忆写入器 (memory_writer.py) | → | D_INTEGRATION 管线路由: InMemoryFakeVMS — MOD-INF-011 · 零依赖测试双... | 导入依赖 / import_depends |
| 16 | AgentOrchestrator · 多角色 Agent 路由、工具链.... | → | D_SECURITY_LLM LLM防御: gateway.py | 导入依赖 / import_depends |
| 17 | AgentOrchestrator · 多角色 Agent 路由、工具链.... | → | D_SECURITY_LLM LLM防御: InputSanitizer: path whitelist + command whitel... | 导入依赖 / import_depends |
| 18 | AgentHealthMonitor · Agent 健康监控（三态 + 5 ... | → | D_SHARED 共享服务: schemas.py | 导入依赖 / import_depends |
| 19 | AgentHealthMonitor · Agent 健康监控（三态 + 5 ... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 20 | AgentOrchestrator · 多角色 Agent 路由、工具链.... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 21 | AgentOrchestrator · 多角色 Agent 路由、工具链.... | → | D_SHARED 共享服务: serialization.py —— 统一序列化/反序列化基础设... | 导入依赖 / import_depends |
| 22 | AgentOrchestrator · 多角色 Agent 路由、工具链.... | → | D_SHARED 共享服务: schemas.py | 导入依赖 / import_depends |
| 23 | AgentOrchestrator · 多角色 Agent 路由、工具链.... | → | D_SHARED 共享服务: async_utils.py — async/sync 边界桥接（5.12.8 .... | 导入依赖 / import_depends |
| 24 | AgentOrchestrator · 多角色 Agent 路由、工具链.... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 25 | Orc 告警接收器 — handle_alert() 消费者 (alert_... | → | D_SHARED 共享服务: ZephyrAlpha 任务系统核心数据模型 (models.py) | 导入依赖 / import_depends |
| 26 | Orc 告警接收器 — handle_alert() 消费者 (alert_... | → | D_SHARED 共享服务: base_config.py | 导入依赖 / import_depends |
| 27 | Orc 告警接收器 — handle_alert() 消费者 (alert_... | → | D_SHARED 共享服务: execution_model.py | 导入依赖 / import_depends |
| 28 | Orc 告警接收器 — handle_alert() 消费者 (alert_... | → | D_SHARED 共享服务: severity_types.py | 导入依赖 / import_depends |
...
| 36 | Orc->Script 脚本执行器 — run_audit() 生产者 (s... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP s... | 导入依赖 / import_depends |
| 37 | CE 任务上下文构建器 — build_from_task() 消费者... | → | D_SHARED 共享服务: schemas.py | 导入依赖 / import_depends |
| 38 | TriggerRouter — RI-03 触发路由器（M3 跨模块触.... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP s... | 导入依赖 / import_depends |
...
| 3 | D_STRUCT_SCRIPTS 结构治理脚本: check_handoff_manifests.py — AI Session Handof... | → | 集成契约注册表（Contract Registry） (contract_r... | 导入依赖 / import_depends |
| 4 | D_TRADING 交易运营: AutoDispatcher — 守护进程内的轻量 PipelineDisp... | → | ActiveTaskQueue — 后台任务轮询与自动分发 (task... | 导入依赖 / import_depends |
| 5 | D_TRADING 交易运营: AutoDispatcher — 守护进程内的轻量 PipelineDisp... | → | Orc->CE 上下文桥接 — request_context() 生产者 ... | 导入依赖 / import_depends |
| 6 | D_TRADING 交易运营: AutoDispatcher — 守护进程内的轻量 PipelineDisp... | → | Orc->Script 脚本执行器 — run_audit() 生产者 (s... | 导入依赖 / import_depends |
| 7 | D_TRADING 交易运营: boot_hooks.py | → | Orc->VMS 记忆写入器 (memory_writer.py) | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 12 个外部域直接连接（出边 55 条 + 入边 7 条 = 62 条）。只显示直接连接的域，不展开具体节点。
