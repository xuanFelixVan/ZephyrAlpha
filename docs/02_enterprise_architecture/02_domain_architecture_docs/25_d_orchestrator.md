...
flowchart TD
    src_zephyr_orchestrator_init_py["zephyr/orchestrator 包入口<br/>编排器的包入口，把这一层的子模块归到一起统一管理<br/>，用到谁才加载谁，避免一次性全加载拖慢启动。<br/>文件: orchestrator/__init__.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_agent_health_monitor_py["代理健康监控<br/>编排调度多组件协作（agent health）<br/>agent_health_monitor<br/>文件: orchestrator/agent_health_monitor.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_contracts_init_py["orchestrator/contracts 包入口<br/>contracts 包入口，整合契约定义相关模块<br/>文件: contracts/__init__.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_contracts_construction_guide_py["施工指南引擎（Construction Guide）<br/>依据：MOD-MASTER-002 蓝图 §八 施工指南<br/>construction_guide<br/>文件: contracts/construction_guide.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_contracts_contract_router_py["契约路由器<br/>依据：MOD-MASTER-002 蓝图 §二 集成契约登记表<br/>contract_router<br/>文件: contracts/contract_router.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_contracts_design_decisions_py["设计decisions<br/>编排调度组件协作（design decisions）<br/>design_decisions<br/>文件: contracts/design_decisions.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_contracts_finding_bridge_py["CT-ORC-SCRIPT-001 运行时桥接<br/>Script System -> Orchestrator: Finding -><br/>TaskCard 转换管道<br/>finding_bridge<br/>文件: contracts/finding_bridge.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_core_init_py["orchestrator/core 包入口<br/>core 包入口，整合核心逻辑相关模块<br/>文件: core/__init__.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_deferred_queue_py["deferred队列<br/>DeferredQueue 内部任务状态（非全局<br/>TaskStatus，仅 WAITING->READY->RUNNING->DONE<br/>/FAILED）。<br/>文件: orchestrator/deferred_queue.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_execution_init_py["orchestrator/execution 包入口<br/>execution 包入口，整合执行管理相关模块<br/>文件: execution/__init__.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_execution_batch_orchestrator_py["批次编排器<br/>多 Worker 批量任务协调器，分派批量任务到多个<br/>Worker 并发执行，汇总结果。<br/>batch_orchestrator<br/>文件: execution/batch_orchestrator.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_execution_data_lifecycle_py["数据生命周期<br/>主要提供获取策略、列表类型定义、应该清除等功能<br/>data_lifecycle<br/>文件: execution/data_lifecycle.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_execution_dispatch_table_py["分发table<br/>依据：MOD-MASTER-002 蓝图 §零 AI Agent<br/>分派与阅读指南<br/>dispatch_table<br/>文件: execution/dispatch_table.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_execution_phase_executor_py["阶段执行器<br/>依据：MOD-MASTER-002 蓝图 §六 施工 Phase 规划<br/>phase_executor<br/>文件: execution/phase_executor.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_execution_reconciliation_loop_py["对账循环<br/>执行管理（reconciliation loop）<br/>reconciliation_loop<br/>文件: execution/reconciliation_loop.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_fault_tolerance_init_py["orchestrator/fault_tolerance 包入口<br/>容错包入口，整合编排调度的容错恢复与故障转移模块<br/>文件: fault_tolerance/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_fault_tolerance_canary_manager_py["金丝雀发布管理器<br/>（CT-CANARY）——权重分流+指标对比+自动回滚。<br/>金丝雀发布管理器，按权重分流流量到新旧版本，对比<br/>指标异常时自动回滚。<br/>canary_manager<br/>文件: fault_tolerance/canary_manager.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_fault_tolerance_chaos_hooks_py["chaos钩子<br/>混沌工程钩子，把 ChaosEngine<br/>故障注入集成到编排器执行循环，验证容错能力。<br/>文件: fault_tolerance/chaos_hooks.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_fault_tolerance_disk_guard_py["disk守卫<br/>主要提供检查、应该enter只读等功能<br/>disk_guard<br/>文件: fault_tolerance/disk_guard.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_fault_tolerance_network_partition_py["网络分区容忍（CT-NETWORK-PARTITION）——CAP定理CP<br/>优先+<br/>脑裂检测+quorum write<br/>network_partition<br/>文件: fault_tolerance/network_partition.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_governance_init_py["orchestrator/governance 包入口<br/>governance 包入口，整合治理管控相关模块<br/>文件: governance/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_governance_model_registry_py["模型注册表<br/>主要提供获取、列表all、获取by提供器等功能<br/>model_registry<br/>文件: governance/model_registry.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_governance_path_index_py["路径索引<br/>主要提供lookup、注册等功能<br/>path_index<br/>文件: governance/path_index.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_governance_risk_registry_py["风险注册表<br/>治理管控（risk registry）<br/>risk_registry<br/>文件: governance/risk_registry.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_hallucination_detector_py["hallucination检测器<br/>在不外接任何生产 LLM SDK 的前提下，提供一个<br/>**可注入调用者（ModelCaller）**<br/>hallucination_detector<br/>文件: orchestrator/hallucination_detector.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_lifecycle_init_py["orchestrator/lifecycle 包入口<br/>生命周期包入口，整合编排调度的组件生命周期管理模<br/>块<br/>文件: lifecycle/__init__.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_lifecycle_housekeeping_py["文件卫生保洁管理器<br/>（CT-HOUSEKEEPING）——临时文件扫描+日志轮转+<br/>废弃目录清理<br/>文件: lifecycle/housekeeping.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_lifecycle_incident_postmortem_py["事件复盘管理器（CT-INCIDENT）——incident记录+time<br/>lin<br/>e+action_items+postmortem<br/>incident_postmortem<br/>文件: lifecycle/incident_postmortem.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_lifecycle_startup_sequencer_py["启动sequencer<br/>编排调度组件协作（startup sequencer）<br/>startup_sequencer<br/>文件: lifecycle/startup_sequencer.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_lifecycle_state_propagation_py["状态propagation<br/>依据：MOD-MASTER-002 蓝图 §四 全局状态传播链<br/>(CT-ORC-DB)<br/>state_propagation<br/>文件: lifecycle/state_propagation.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_lifecycle_state_synchronizer_py["状态synchronizer<br/>StateSynchronizer — 同步 SQLite<br/>状态与文件系统实际状态（T-2-04）<br/>state_synchronizer<br/>文件: lifecycle/state_synchronizer.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_lifecycle_system_transfer_py["系统迁移<br/>系统移交恢复（CT-TRANSFER）——系统Owner变更+配置<br/>迁移+密钥轮转+健康验证。<br/>system_transfer<br/>文件: lifecycle/system_transfer.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_lifecycle_teardown_manager_py["teardown管理器<br/>编排调度组件协作（teardown）<br/>teardown_manager<br/>文件: lifecycle/teardown_manager.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_quality_agent_quality_py["代理质量<br/>AI Agent 质量反馈闭环<br/>（CT-AGENT-QUALITY）——task完成质量评分+agent绩效<br/>追踪。<br/>agent_quality<br/>文件: quality/agent_quality.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_quality_benchmark_runner_py["基准运行器<br/>benchmark运行器，主要提供获取基线、检测回归等功<br/>能<br/>benchmark_runner<br/>文件: quality/benchmark_runner.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_quality_blind_spot_closure_py["盲点闭合<br/>编排调度（blind spot closure）<br/>blind_spot_closure<br/>文件: quality/blind_spot_closure.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_quality_ke_quality_py["知识质量评分契约<br/>（CT-KE-QUALITY）——KE完整性+准确性+时效性三维<br/>知识条目质量评分器，按完整性、准确性、时效性三维<br/>度评分，判定是否可接受或需人工复核。<br/>ke_quality<br/>文件: quality/ke_quality.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_quality_knowledge_freshness_py["知识新鲜度废止管理器<br/>（CT-KNOWLEDGE-FRESHNESS）——KE过期<br/>标记+自动失效<br/>knowledge_freshness<br/>文件: quality/knowledge_freshness.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_quality_lean_scanner_py["死代码/孤儿文件/僵尸引用三扫描<br/>（CT-LEAN）——三款扫描器+自动化清理建议<br/>精简扫描器，扫描死代码、孤儿文件、僵尸引用三类浪<br/>费，并产出自动化清理建议。<br/>lean_scanner<br/>文件: quality/lean_scanner.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_resilience_init_py["orchestrator/resilience 包入口<br/>159.4 修复: 死副本文件已删除 (deferred_queue<br/>/rollback_manager/hallucination_detector<br/>与顶层重复).<br/>文件: resilience/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_task_queue_py["任务队列<br/>编排相关功能（task queue）<br/>task_queue<br/>文件: orchestrator/task_queue.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_execution_dispatch_table_py ~~~ src_zephyr_orchestrator_execution_phase_executor_py
    ...
    src_zephyr_orchestrator_governance_capacity_budget_py ~~~ src_zephyr_orchestrator_governance_model_registry_py
    ...
    src_zephyr_orchestrator_agent_orchestrator_py["代理编排器<br/>AgentOrchestrator · 多角色 Agent<br/>路由、工具链编排与健康监控<br/>agent_orchestrator<br/>文件: orchestrator/agent_orchestrator.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_contracts_alert_handler_py["告警处理器<br/>Orc 告警接收器 — handle_alert() 消费者<br/>alert_handler<br/>文件: contracts/alert_handler.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_contracts_contract_registry_py["契约注册表<br/>拆包方案（contract_registry/ 包，enums/models<br/>/registry 分离）需新建 .py<br/>文件: contracts/contract_registry.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_core_task_queue_py["任务队列<br/>核心相关功能（task queue）<br/>task_queue<br/>文件: core/task_queue.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_execution_dlq_manager_py["dlq管理器<br/>编排调度多组件协作（dlq）<br/>dlq_manager<br/>文件: execution/dlq_manager.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_execution_memory_writer_py["Orc->VMS 记忆写入器<br/>编排器到向量记忆的写入器，任务完成时把结果归档到<br/>VMS，VMS 不可用则降级不阻塞。<br/>memory_writer<br/>文件: execution/memory_writer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_fault_tolerance_degrade_cascade_py["degrade级联<br/>主要提供检测级联、break级联等功能<br/>degrade_cascade<br/>文件: fault_tolerance/degrade_cascade.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_fault_tolerance_fault_types_py["故障类型定义<br/>编排调度多组件协作（fault types）<br/>文件: fault_tolerance/fault_types.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_lifecycle_rolling_upgrade_py["零停机滚动升级（CT-DEPLOY）——graceful<br/>shutdown+流量<br/>摘除+health check wait<br/>rolling_upgrade<br/>文件: lifecycle/rolling_upgrade.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_quality_init_py["orchestrator/quality 包入口<br/>质量包入口，整合编排调度的服务质量保障模块<br/>文件: quality/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_execution_context_bridge_py ~~~ src_zephyr_orchestrator_execution_dlq_manager_py
    src_zephyr_orchestrator_execution_dlq_manager_py ~~~ src_zephyr_orchestrator_execution_memory_writer_py
    ...
    src_zephyr_orchestrator_file_task_mapper_py ~~~ src_zephyr_orchestrator_governance_dependency_lock_py
    src_zephyr_orchestrator_governance_dependency_lock_py ~~~ src_zephyr_orchestrator_lifecycle_rolling_upgrade_py
    ...
    src_zephyr_orchestrator_execution_init_py -->|config_depends / config_depends| src_zephyr_orchestrator_execution_dlq_manager_py
    ...
    class D_SHARED,D_GOV_OPS_RESILIENCE,D_TRADING,D_GOV_SCRIPTS,D_INFRA_RUNTIME,D_AUTONOMY_CORE,D_FEEDBACK_LOOP external_prod
...
| 5 | D_ML_TRAIN 训练: 训练管线 (training_pipeline/) | → | 模型注册表 / model_registry (governance/model_registry.py) | runtime / runtime |
