...
flowchart TD
    src_zephyr_orchestrator_init_py["Init<br/>管理zephyr.orchestrator子包的加载和懒导入<br/>文件: orchestrator/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_contracts_init_py["orchestrator contracts subpackage.'''<br/>contracts — orchestrator contracts subpackage.<br/>Init<br/>文件: contracts/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_contracts_contract_router_py["Contract Router<br/>契约路由（Contract Router）<br/>文件: contracts/contract_router.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_contracts_finding_bridge_py["Finding Bridge<br/>CT-ORC-SCRIPT-001 运行时桥接<br/>文件: contracts/finding_bridge.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_core_init_py["auto-generated package init.'''<br/>orchestrator.core — auto-generated package init.<br/>文件: core/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_execution_init_py["orchestrator execution subpackage.'''<br/>execution — orchestrator execution subpackage.<br/>Init<br/>文件: execution/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_execution_phase_executor_py["Phase Executor<br/>Phase 执行引擎（Phase Executor）<br/>文件: execution/phase_executor.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_execution_wave_generator_py["Wave Generator<br/>WaveGenerator — 根据 Task 依赖图生成执行 Wave<br/>（T-2-03）<br/>文件: execution/wave_generator.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_fault_tolerance_init_py["orchestrator fault_tolerance subpackage.'''<br/>fault_tolerance — orchestrator fault_tolerance<br/>subpackage.<br/>Init<br/>文件: fault_tolerance/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_fault_tolerance_chaos_hooks_py["integrates ChaosEngine with the orchestrator<br/>execution loop.'''<br/>ChaosHook — integrates ChaosEngine with the<br/>orchestrator execution loop.<br/>Chaos Hooks<br/>文件: fault_tolerance/chaos_hooks.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_governance_init_py["orchestrator governance subpackage.'''<br/>governance — orchestrator governance subpackage.<br/>Init<br/>文件: governance/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_governance_risk_registry_py["Risk Registry<br/>编排/治理包的risk_registry模块<br/>文件: governance/risk_registry.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_hallucination_detector_py["Hallucination Detector<br/>HallucinationDetector · Chain-of-Verification<br/>（CoVe）幻觉检测器<br/>文件: orchestrator/hallucination_detector.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_lifecycle_init_py["orchestrator lifecycle subpackage.'''<br/>lifecycle — orchestrator lifecycle subpackage.<br/>Init<br/>文件: lifecycle/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_lifecycle_state_propagation_py["State Propagation<br/>全局状态传播链（State Propagation Chain）<br/>文件: lifecycle/state_propagation.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_lifecycle_teardown_manager_py["Teardown Manager<br/>编排/lifecycle包的teardown_manager模块<br/>文件: lifecycle/teardown_manager.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_resilience_init_py["auto-generated package init.<br/>orchestrator.resilience — auto-generated<br/>package init.<br/>文件: resilience/__init__.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_rollback_manager_py["Rollback Manager<br/>RollbackManager — 仅调试用途的 DB-state<br/>快照，不用于自动回滚。<br/>文件: orchestrator/rollback_manager.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_task_queue_py["Task Queue<br/>ActiveTaskQueue — 后台任务轮询与自动分发<br/>文件: orchestrator/task_queue.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_governance_model_registry_py ~~~ src_zephyr_orchestrator_governance_risk_registry_py
    ...
    src_zephyr_orchestrator_agent_orchestrator_py["Agent Orchestrator<br/>AgentOrchestrator · 多角色 Agent<br/>路由、工具链编排与健康监控<br/>文件: orchestrator/agent_orchestrator.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_contracts_alert_handler_py["handle_alert<br/>Orc 告警接收器 — handle_alert() 消费者<br/>Alert Handler<br/>文件: contracts/alert_handler.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_contracts_contract_registry_py["Contract Registry<br/>集成契约注册表（Contract Registry）<br/>文件: contracts/contract_registry.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_execution_context_bridge_py["request_context<br/>Orc->CE 上下文桥接 — request_context() 生产者<br/>Context Bridge<br/>文件: execution/context_bridge.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_execution_script_runner_py["run_audit<br/>Orc->Script 脚本执行器 — run_audit() 生产者<br/>Script Runner<br/>文件: execution/script_runner.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_fault_tolerance_fault_types_py["Fault Types<br/>Fault type registry and preset templates for<br/>chaos engineering.<br/>文件: fault_tolerance/fault_types.py<br/>(生产态 / production)"]
    src_zephyr_orchestrator_file_task_mapper_py["File Task Mapper<br/>FileTaskMapper — 文件路径 ↔ Task N:N 映射器<br/>（#21 裁定重写）<br/>文件: orchestrator/file_task_mapper.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_quality_init_py["orchestrator quality subpackage.'''<br/>quality — orchestrator quality subpackage.<br/>Init<br/>文件: quality/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_file_task_mapper_py ~~~ src_zephyr_orchestrator_governance_path_index_py
    src_zephyr_orchestrator_governance_path_index_py ~~~ src_zephyr_orchestrator_lifecycle_housekeeping_py
    ...
    src_zephyr_orchestrator_execution_task_context_builder_py["build_from_task<br/>CE 任务上下文构建器 — build_from_task() 消费者<br/>Task Context Builder<br/>文件: execution/task_context_builder.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_governance_init_py -->|config_depends / config_depends| src_zephyr_orchestrator_governance_path_index_py
    ...
    class D_SHARED,D_AUTONOMY_CORE,D_SECURITY,D_GOV_SCRIPTS,D_INFRA_RUNTIME,D_FEEDBACK_LOOP,D_TRADING external_prod
...
|:--:|---------|:--:|---------|---------|
| 1 | request_context / Context Bridge (execution/context_bridg... | → | D_AUTONOMY_CORE 自治核心: 向量写入器 / vector_writer (vector_memory/vector_writer.py) | 导入依赖 / import_depends |
| 2 | Orc->VMS 记忆写入器""" / Memory Writer (execution/memory_... | → | D_AUTONOMY_CORE 自治核心: 向量桥接 / vector_bridge (context/vector_bridge.py) | 导入依赖 / import_depends |
...
| 4 | handle_alert / Alert Handler (contracts/alert_handler.py) | → | D_GOVERNANCE 生命周期管理: sqlite结构 / sqlite_schema (persistence/sqlite_schema.py) | 导入依赖 / import_depends |
| 5 | handle_alert / Alert Handler (contracts/alert_handler.py) | → | D_GOVERNANCE 生命周期管理: 任务repo / task_repo (persistence/task_repo.py) | 导入依赖 / import_depends |
| 6 | Finding Bridge / Finding Bridge (contracts/finding_bridge... | → | D_GOVERNANCE 生命周期管理: 任务repo / task_repo (persistence/task_repo.py) | 导入依赖 / import_depends |
...
| 8 | Failure Matcher / Failure Matcher (resilience/failure_mat... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: 声明式事件钩子注册表 / Event Hook (ops_governance/event_h... | 导入依赖 / import_depends |
| 9 | Agent Orchestrator / Agent Orchestrator (orchestrator/age... | → | D_INFRA_RUNTIME 运行时集成: Token Budget / Token Budget (capacity_assurance/token_bud... | 导入依赖 / import_depends |
| 10 | run_audit / Script Runner (execution/script_runner.py) | → | D_INFRA_RUNTIME 运行时集成: submit_findings / Gate Bridge (script_system/gate_bridge.py) | 导入依赖 / import_depends |
| 11 | Orc->VMS 记忆写入器""" / Memory Writer (execution/memory_... | → | D_INTEGRATION 管线路由: 只读：store_size / In Memory Fake Vms (vector_memory/in_m... | 导入依赖 / import_depends |
| 12 | Agent Orchestrator / Agent Orchestrator (orchestrator/age... | → | D_SECURITY 对抗验证: Gateway / Gateway (llm_security/gateway.py) | 导入依赖 / import_depends |
| 13 | Agent Orchestrator / Agent Orchestrator (orchestrator/age... | → | D_SECURITY 对抗验证: path whitelist + command whitelist + token budget guard. ... | 导入依赖 / import_depends |
| 14 | SLO 违规记录模型 / Agent Health Monitor (orchestrator/age... | → | D_SHARED 共享服务: Schemas / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 16 | Agent Orchestrator / Agent Orchestrator (orchestrator/age... | → | D_SHARED 共享服务: 从当前文件向上查找项目根目录 / Paths (io/paths.py) | 导入依赖 / import_depends |
| 17 | Agent Orchestrator / Agent Orchestrator (orchestrator/age... | → | D_SHARED 共享服务: 序列化/反序列化过程中类型不兼容或格式错误 / Serialization... | 导入依赖 / import_depends |
| 18 | Agent Orchestrator / Agent Orchestrator (orchestrator/age... | → | D_SHARED 共享服务: Schemas / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
| 19 | Agent Orchestrator / Agent Orchestrator (orchestrator/age... | → | D_SHARED 共享服务: async/sync 边界桥接 / Async Utils (utils/async_utils.py) | 导入依赖 / import_depends |
| 20 | Agent Orchestrator / Agent Orchestrator (orchestrator/age... | → | D_SHARED 共享服务: 注册 datetime/date→sqlite3 str 适配器 / Time Utils (util... | 导入依赖 / import_depends |
| 21 | handle_alert / Alert Handler (contracts/alert_handler.py) | → | D_SHARED 共享服务: D-ORCH / D-GOV / D-RESILIENCE 通过此接口访问任务持久化 / ... | 导入依赖 / import_depends |
| 22 | handle_alert / Alert Handler (contracts/alert_handler.py) | → | D_SHARED 共享服务: —蓝图 MOD-TASK_SYSTEM §3.2.2""" / Models (foundation/mo... | 导入依赖 / import_depends |
| 23 | handle_alert / Alert Handler (contracts/alert_handler.py) | → | D_SHARED 共享服务: Base Config / Base Config (schema/base_config.py) | 导入依赖 / import_depends |
| 24 | handle_alert / Alert Handler (contracts/alert_handler.py) | → | D_SHARED 共享服务: Execution Model / Execution Model (schema/execution_model... | 导入依赖 / import_depends |
| 25 | handle_alert / Alert Handler (contracts/alert_handler.py) | → | D_SHARED 共享服务: re-homed from infrastructure_runtime_integration.db.circu... | 导入依赖 / import_depends |
| 26 | handle_alert / Alert Handler (contracts/alert_handler.py) | → | D_SHARED 共享服务: Task Types / Task Types (schema/task_types.py) | 导入依赖 / import_depends |
| 27 | Finding Bridge / Finding Bridge (contracts/finding_bridge... | → | D_SHARED 共享服务: —蓝图 MOD-TASK_SYSTEM §3.2.2""" / Models (foundation/mo... | 导入依赖 / import_depends |
| 28 | Finding Bridge / Finding Bridge (contracts/finding_bridge... | → | D_SHARED 共享服务: 从当前文件向上查找项目根目录 / Paths (io/paths.py) | 导入依赖 / import_depends |
| 29 | 打破 pipeline↔orchestrator 循环依赖的协议接口 / Task Que... | → | D_SHARED 共享服务: D-ORCH / D-GOV / D-RESILIENCE 通过此接口访问任务持久化 / ... | 导入依赖 / import_depends |
| 30 | WAITING -> READY task scheduler. / Deferred Queue (orches... | → | D_SHARED 共享服务: Observer / Observer (infra/observer.py) | 导入依赖 / import_depends |
| 31 | WAITING -> READY task scheduler. / Deferred Queue (orches... | → | D_SHARED 共享服务: 对连接应用 KBG-0030 §4.3 PRAGMA 基线 / Sqlite Factory (i... | 导入依赖 / import_depends |
...
| 33 | 多 Worker 批量任务协调器 / Batch Orchestrator (execution/... | → | D_SHARED 共享服务: —蓝图 MOD-TASK_SYSTEM §3.2.2""" / Models (foundation/mo... | 导入依赖 / import_depends |
| 34 | Orc->VMS 记忆写入器""" / Memory Writer (execution/memory_... | → | D_SHARED 共享服务: 序列化/反序列化过程中类型不兼容或格式错误 / Serialization... | 导入依赖 / import_depends |
| 35 | run_audit / Script Runner (execution/script_runner.py) | → | D_SHARED 共享服务: 返回 Windows 无窗口 creationflags；POSIX 返回 0 / Process... | 导入依赖 / import_depends |
| 36 | build_from_task / Task Context Builder (execution/task_co... | → | D_SHARED 共享服务: Schemas / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 39 | Wave Generator / Wave Generator (execution/wave_generator... | → | D_SHARED 共享服务: 从当前文件向上查找项目根目录 / Paths (io/paths.py) | 导入依赖 / import_depends |
| 40 | Wave Generator / Wave Generator (execution/wave_generator... | → | D_SHARED 共享服务: 确保数据库 schema 已初始化 / Db Utils (utils/db_utils.py) | 导入依赖 / import_depends |
| 41 | integrates ChaosEngine with the orchestrator execution lo... | → | D_SHARED 共享服务: Orchestration Protocol / Orchestration Protocol (contract... | 导入依赖 / import_depends |
| 42 | File Task Mapper / File Task Mapper (orchestrator/file_ta... | → | D_SHARED 共享服务: 从当前文件向上查找项目根目录 / Paths (io/paths.py) | 导入依赖 / import_depends |
| 43 | File Task Mapper / File Task Mapper (orchestrator/file_ta... | → | D_SHARED 共享服务: vocabulary YAML 加载公共工具 / Yaml Utils (io/yaml_utils.py) | 导入依赖 / import_depends |
| 44 | File Task Mapper / File Task Mapper (orchestrator/file_ta... | → | D_SHARED 共享服务: Task Types / Task Types (schema/task_types.py) | 导入依赖 / import_depends |
| 45 | File Task Mapper / File Task Mapper (orchestrator/file_ta... | → | D_SHARED 共享服务: 确保数据库 schema 已初始化 / Db Utils (utils/db_utils.py) | 导入依赖 / import_depends |
| 46 | File Task Mapper / File Task Mapper (orchestrator/file_ta... | → | D_SHARED 共享服务: 注册 datetime/date→sqlite3 str 适配器 / Time Utils (util... | 导入依赖 / import_depends |
| 47 | Hallucination Detector / Hallucination Detector (orchestr... | → | D_SHARED 共享服务: Schemas / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
| 48 | Hallucination Detector / Hallucination Detector (orchestr... | → | D_SHARED 共享服务: 注册 datetime/date→sqlite3 str 适配器 / Time Utils (util... | 导入依赖 / import_depends |
...
| 53 | Rollback Manager / Rollback Manager (orchestrator/rollbac... | → | D_SHARED 共享服务: 从当前文件向上查找项目根目录 / Paths (io/paths.py) | 导入依赖 / import_depends |
| 54 | Rollback Manager / Rollback Manager (orchestrator/rollbac... | → | D_SHARED 共享服务: 确保数据库 schema 已初始化 / Db Utils (utils/db_utils.py) | 导入依赖 / import_depends |
| 55 | Rollback Manager / Rollback Manager (orchestrator/rollbac... | → | D_SHARED 共享服务: 注册 datetime/date→sqlite3 str 适配器 / Time Utils (util... | 导入依赖 / import_depends |

### 依赖本域的其他域（入边）/ Depended By

| # | 外部域-源模块 / Source Module | → | 本域模块 / Target Module | 依赖类型 / Type |
|:--:|---------|:--:|---------|---------|
| 1 | D_AUTONOMY_CORE 自治核心: 上下文assembler / context_assembler (context/context_asse... | → | orchestrator contracts subpackage.""" / Init (contracts/_... | 导入依赖 / import_depends |
| 2 | D_FEEDBACK_LOOP 反馈循环引擎: alert分发器 / alert_dispatcher (feedback_loop/alert_dispa... | → | handle_alert / Alert Handler (contracts/alert_handler.py) | 导入依赖 / import_depends |
| 3 | D_GOV_SCRIPTS 脚本治理: Check Handoff Manifests / Check Handoff Manifests (d1_str... | → | Contract Registry / Contract Registry (contracts/contract... | 导入依赖 / import_depends |
| 4 | D_INFRA_RUNTIME 运行时集成: 从 TaskRepository 查询 task 的 source_blueprint，失败返回... | → | Orc->VMS 记忆写入器""" / Memory Writer (execution/memory_... | 导入依赖 / import_depends |
| 5 | D_ML_TRAIN 训练: Training Pipeline / Training Pipeline (training_pipeline/) | → | —deepseek/opus/gpt等模型版本+性能基线 / Model Registry (... | runtime / runtime |
...
| 7 | D_TRADING 交易运营: 执行 TaskCard 并触发整条基础设施管道 / Auto Dispatcher (t... | → | request_context / Context Bridge (execution/context_bridg... | 导入依赖 / import_depends |
| 8 | D_TRADING 交易运营: 执行 TaskCard 并触发整条基础设施管道 / Auto Dispatcher (t... | → | run_audit / Script Runner (execution/script_runner.py) | 导入依赖 / import_depends |
