...
flowchart TD
    src_zephyr_orchestrator_init_py["(生产态 / production) 包入口 / __init__<br/>编排器的包入口，把这一层的子模块归到一起<br/>统一管理，用到谁才加载谁，避免一次性全加<br/>载拖慢启动。<br/>文件: orchestrator/__init__.py"]
    src_zephyr_orchestrator_agent_health_monitor_py["(生产态 / production) 代理健康监控 /<br/>agent_health_monitor<br/>AgentHealthMonitor · Agent 健康监控<br/>（三态 + 5 项 SLO）<br/>文件: orchestrator/agent_health_<br/>monitor.py"]
    src_zephyr_orchestrator_contracts_init_py["(生产态 / production) 包入口 /<br/>contracts — orchestrator contracts<br/>subpackage.<br/>包入口。contracts — orchestrator<br/>contracts subpackage.<br/>文件: contracts/__init__.py"]
    src_zephyr_orchestrator_contracts_construction_guide_py["(生产态 / production) 施工指南引擎<br/>（Construction Guide） / construction_<br/>guide<br/>施工指南引擎（Construction Guide）<br/>文件: contracts/construction_guide.py"]
    src_zephyr_orchestrator_contracts_contract_router_py["(生产态 / production) 契约路由器 /<br/>contract_router<br/>契约路由（Contract Router）<br/>文件: contracts/contract_router.py"]
    src_zephyr_orchestrator_contracts_design_decisions_py["(生产态 / production) 设计decisions /<br/>design_decisions<br/>设计decisions，提供包入口和模块加载功能<br/>文件: contracts/design_decisions.py"]
    ...
    src_zephyr_orchestrator_contracts_prompt_version_py["(生产态 / production) 提示版本 / prompt_<br/>version<br/>AI Prompt 版本控制<br/>（CT-PROMPT-VERSION）——prompt<br/>template版本化+部署前diff。<br/>文件: contracts/prompt_version.py"]
    src_zephyr_orchestrator_core_init_py["(生产态 / production) 包入口 /<br/>orchestrator.core — auto-generated<br/>package init.<br/>包入口。orchestrator.core —<br/>auto-generated package init.<br/>文件: core/__init__.py"]
    src_zephyr_orchestrator_deferred_queue_py["(生产态 / production) deferred队列 /<br/>DeferredQueue: WAITING -> READY task<br/>scheduler.<br/>deferred队列。DeferredQueue: WAITING -><br/>READY task scheduler.<br/>文件: orchestrator/deferred_queue.py"]
    src_zephyr_orchestrator_execution_init_py["(生产态 / production) 包入口 /<br/>execution — orchestrator execution<br/>subpackage.<br/>包入口。execution — orchestrator<br/>execution subpackage.<br/>文件: execution/__init__.py"]
    src_zephyr_orchestrator_execution_data_lifecycle_py["(生产态 / production) 数据生命周期 /<br/>data_lifecycle<br/>数据生命周期，主要提供获取策略、列表类型<br/>定义、应该清除等功能<br/>文件: execution/data_lifecycle.py"]
    src_zephyr_orchestrator_execution_dispatch_table_py["(生产态 / production) 分发table /<br/>dispatch_table<br/>AI Agent 冷启动分派表（Dispatch Table）<br/>文件: execution/dispatch_table.py"]
    src_zephyr_orchestrator_execution_dlq_manager_py["(生产态 / production) dlq管理器 / dlq_<br/>manager<br/>DLQ 管理器（Dead Letter Queue Manager —<br/>CT-DLQ-001）<br/>文件: execution/dlq_manager.py"]
    src_zephyr_orchestrator_execution_phase_executor_py["(生产态 / production) 阶段执行器 /<br/>phase_executor<br/>Phase 执行引擎（Phase Executor）<br/>文件: execution/phase_executor.py"]
    src_zephyr_orchestrator_execution_reconciliation_loop_py["(生产态 / production) 对账循环 /<br/>reconciliation_loop<br/>对账循环，提供包入口和模块加载功能<br/>文件: execution/reconciliation_loop.py"]
    src_zephyr_orchestrator_execution_trigger_router_py["(生产态 / production) 触发器路由器 /<br/>trigger_router<br/>TriggerRouter — RI-03 触发路由器（M3<br/>跨模块触发分派）<br/>文件: execution/trigger_router.py"]
    src_zephyr_orchestrator_execution_wave_generator_py["(生产态 / production) wave生成器 / wave_<br/>generator<br/>WaveGenerator — 根据 Task<br/>依赖图生成执行 Wave（T-2-03）<br/>文件: execution/wave_generator.py"]
    src_zephyr_orchestrator_fault_tolerance_init_py["(生产态 / production) 包入口 / fault_<br/>tolerance — orchestrator fault_<br/>tolerance subpackage.<br/>包入口。fault_tolerance — orchestrator<br/>fault_tolerance subpackage.<br/>文件: fault_tolerance/__init__.py"]
    src_zephyr_orchestrator_fault_tolerance_canary_manager_py["(生产态 / production) 金丝雀发布管理器<br/>（CT-CANARY）——权重分流+指标对比+自动回<br/>滚。 / canary_manager<br/>金丝雀发布管理器<br/>（CT-CANARY）——权重分流+指标对比+自动回<br/>滚。<br/>文件: fault_tolerance/canary_manager.py"]
    src_zephyr_orchestrator_fault_tolerance_chaos_hooks_py["(生产态 / production) chaos钩子 /<br/>ChaosHook — integrates ChaosEngine with<br/>the orchestrator exe<br/>chaos钩子。ChaosHook — integrates<br/>ChaosEngine with the orchestrator<br/>execution loop.<br/>文件: fault_tolerance/chaos_hooks.py"]
    src_zephyr_orchestrator_fault_tolerance_degrade_cascade_py["(生产态 / production) degrade级联 /<br/>degrade_cascade<br/>degrade级联，主要提供检测级联、break级联<br/>等功能<br/>文件: fault_tolerance/degrade_cascade.py"]
    src_zephyr_orchestrator_fault_tolerance_disk_guard_py["(生产态 / production) disk守卫 / disk_<br/>guard<br/>disk守卫，主要提供检查、应该enter只读等<br/>功能<br/>文件: fault_tolerance/disk_guard.py"]
    src_zephyr_orchestrator_fault_tolerance_network_partition_py["(生产态 / production) 网络分区容忍<br/>（CT-NETWORK-PARTITION）——CAP定理CP优先+<br/>/ network_partition<br/>网络分区容忍（CT-NETWORK-PARTITION）——CA<br/>P定理CP优先+脑裂检测+quorum write。<br/>文件: fault_tolerance/network_<br/>partition.py"]
    src_zephyr_orchestrator_governance_init_py["(生产态 / production) 包入口 /<br/>governance — orchestrator governance<br/>subpackage.<br/>包入口。governance — orchestrator<br/>governance subpackage.<br/>文件: governance/__init__.py"]
    src_zephyr_orchestrator_governance_autonomy_guard_py["(生产态 / production) autonomy守卫 /<br/>autonomy_guard<br/>Owner 缺位分级自治<br/>（CT-AUTONOMY）——Owner离线->自动降级->最<br/>小安全运行。<br/>文件: governance/autonomy_guard.py"]
    src_zephyr_orchestrator_governance_capacity_budget_py["(生产态 / production) 容量预算 /<br/>capacity_budget<br/>全局容量预算控制器（Capacity Budget<br/>Controller）<br/>文件: governance/capacity_budget.py"]
    src_zephyr_orchestrator_governance_dependency_lock_py["(生产态 / production) 依赖锁 /<br/>dependency_lock<br/>外部依赖版本锁（CT-DEPS）——Python包版本<br/>锁定+hash验证+安全审计。<br/>文件: governance/dependency_lock.py"]
    src_zephyr_orchestrator_governance_path_index_py["(生产态 / production) 路径索引 / path_<br/>index<br/>路径索引，主要提供lookup、注册等功能<br/>文件: governance/path_index.py"]
    src_zephyr_orchestrator_governance_risk_registry_py["(生产态 / production) 风险注册表 / risk_<br/>registry<br/>风险注册表，提供包入口和模块加载功能<br/>文件: governance/risk_registry.py"]
    src_zephyr_orchestrator_governance_schema_migration_py["(生产态 / production) 模式迁移 / schema_<br/>migration<br/>数据库 Schema 演化契约<br/>（CT-SCHEMA-MIGRATE）——向后兼容迁移+回滚<br/>脚本。<br/>文件: governance/schema_migration.py"]
    src_zephyr_orchestrator_governance_version_manifest_py["(生产态 / production) 版本清单 /<br/>version_manifest<br/>版本manifest，主要提供获取版本、获取路径<br/>、列表systems等功能<br/>文件: governance/version_manifest.py"]
    src_zephyr_orchestrator_hallucination_detector_py["(生产态 / production)<br/>hallucination检测器 / hallucination_<br/>detector<br/>HallucinationDetector ·<br/>Chain-of-Verification（CoVe）幻觉检测器<br/>文件: orchestrator/hallucination_<br/>detector.py"]
    src_zephyr_orchestrator_lifecycle_init_py["(生产态 / production) 包入口 /<br/>lifecycle — orchestrator lifecycle<br/>subpackage.<br/>包入口。lifecycle — orchestrator<br/>lifecycle subpackage.<br/>文件: lifecycle/__init__.py"]
    src_zephyr_orchestrator_lifecycle_incident_postmortem_py["(生产态 / production) 事件复盘管理器<br/>（CT-INCIDENT）——incident记录+timelin /<br/>incident_postmortem<br/>事件复盘管理器（CT-INCIDENT）——incident<br/>记录+timeline+action_items+postmortem。<br/>文件: lifecycle/incident_postmortem.py"]
    src_zephyr_orchestrator_lifecycle_rolling_upgrade_py["(生产态 / production) 零停机滚动升级<br/>（CT-DEPLOY）——graceful shutdown+流量 /<br/>rolling_upgrade<br/>零停机滚动升级（CT-DEPLOY）——graceful<br/>shutdown+流量摘除+health check wait。<br/>文件: lifecycle/rolling_upgrade.py"]
    src_zephyr_orchestrator_lifecycle_session_conflict_py["(生产态 / production) 会话冲突 /<br/>session_conflict<br/>Session 冲突预防契约<br/>（CT-SESSION-CONFLICT）——文件锁+并发sess<br/>ion检测+冲突resolution。<br/>文件: lifecycle/session_conflict.py"]
    src_zephyr_orchestrator_lifecycle_startup_sequencer_py["(生产态 / production) 启动sequencer /<br/>startup_sequencer<br/>启动sequencer，提供包入口和模块加载功能<br/>文件: lifecycle/startup_sequencer.py"]
    src_zephyr_orchestrator_lifecycle_state_propagation_py["(生产态 / production) 状态propagation /<br/>state_propagation<br/>全局状态传播链（State Propagation<br/>Chain）<br/>文件: lifecycle/state_propagation.py"]
    src_zephyr_orchestrator_lifecycle_state_synchronizer_py["(生产态 / production) 状态synchronizer<br/>/ state_synchronizer<br/>StateSynchronizer — 同步 SQLite<br/>状态与文件系统实际状态（T-2-04）<br/>文件: lifecycle/state_synchronizer.py"]
    src_zephyr_orchestrator_lifecycle_system_transfer_py["(生产态 / production) 系统迁移 / system_<br/>transfer<br/>系统移交恢复（CT-TRANSFER）——系统Owner变<br/>更+配置迁移+密钥轮转+健康验证。<br/>文件: lifecycle/system_transfer.py"]
    src_zephyr_orchestrator_lifecycle_teardown_manager_py["(生产态 / production) teardown管理器 /<br/>teardown_manager<br/>teardown管理器，提供包入口和模块加载功能<br/>文件: lifecycle/teardown_manager.py"]
    src_zephyr_orchestrator_quality_agent_quality_py["(生产态 / production) 代理质量 / agent_<br/>quality<br/>AI Agent 质量反馈闭环<br/>（CT-AGENT-QUALITY）——task完成质量评分+a<br/>gent绩效追踪。<br/>文件: quality/agent_quality.py"]
    src_zephyr_orchestrator_quality_benchmark_runner_py["(生产态 / production) 基准运行器 /<br/>benchmark_runner<br/>benchmark运行器，主要提供获取基线、检测<br/>回归等功能<br/>文件: quality/benchmark_runner.py"]
    src_zephyr_orchestrator_quality_blind_spot_closure_py["(生产态 / production) 盲点闭合 / blind_<br/>spot_closure<br/>盲点闭合，提供包入口和模块加载功能<br/>文件: quality/blind_spot_closure.py"]
    src_zephyr_orchestrator_quality_ke_quality_py["(生产态 / production) 知识质量评分契约<br/>（CT-KE-QUALITY）——KE完整性+准确性+时效<br/>性三维 / ke_quality<br/>知识质量评分契约<br/>（CT-KE-QUALITY）——KE完整性+准确性+时效<br/>性三维评分。<br/>文件: quality/ke_quality.py"]
    src_zephyr_orchestrator_quality_knowledge_freshness_py["(生产态 / production)<br/>知识新鲜度废止管理器<br/>（CT-KNOWLEDGE-FRESHNESS）——KE过期 /<br/>knowledge_freshness<br/>知识新鲜度废止管理器<br/>（CT-KNOWLEDGE-FRESHNESS）——KE过期标记+<br/>自动失效。<br/>文件: quality/knowledge_freshness.py"]
    src_zephyr_orchestrator_quality_lean_scanner_py["(生产态 / production) 死代码/孤儿文件<br/>/僵尸引用三扫描<br/>（CT-LEAN）——三款扫描器+自动化清理建议<br/>/ lean_scanner<br/>死代码/孤儿文件/僵尸引用三扫描<br/>（CT-LEAN）——三款扫描器+自动化清理建议。<br/>文件: quality/lean_scanner.py"]
    src_zephyr_orchestrator_quality_stability_guard_py["(生产态 / production) stability守卫 /<br/>stability_guard<br/>API 稳定性守护（CT-STABILITY）——public<br/>API签名锁+breaking change检测。<br/>文件: quality/stability_guard.py"]
    src_zephyr_orchestrator_resilience_init_py["(生产态 / production) 包入口 /<br/>orchestrator.resilience —<br/>auto-generated package init.<br/>包入口。orchestrator.resilience —<br/>auto-generated package init.<br/>文件: resilience/__init__.py"]
    src_zephyr_orchestrator_rollback_manager_py["(生产态 / production) 回滚管理器 /<br/>rollback_manager<br/>RollbackManager — 仅调试用途的 DB-state<br/>快照，不用于自动回滚。<br/>文件: orchestrator/rollback_manager.py"]
    src_zephyr_orchestrator_task_queue_py["(生产态 / production) 任务队列 / task_<br/>queue<br/>ActiveTaskQueue — 后台任务轮询与自动分发<br/>文件: orchestrator/task_queue.py"]
    ...
    src_zephyr_orchestrator_governance_autonomy_guard_py ~~~ src_zephyr_orchestrator_governance_capacity_budget_py
    src_zephyr_orchestrator_governance_capacity_budget_py ~~~ src_zephyr_orchestrator_governance_dependency_lock_py
    src_zephyr_orchestrator_governance_dependency_lock_py ~~~ src_zephyr_orchestrator_governance_path_index_py
    ...
    src_zephyr_orchestrator_agent_orchestrator_py["(生产态 / production) 代理编排器 /<br/>agent_orchestrator<br/>AgentOrchestrator · 多角色 Agent<br/>路由、工具链编排与健康监控<br/>文件: orchestrator/agent_orchestrator.py"]
    src_zephyr_orchestrator_contracts_alert_handler_py["(生产态 / production) 告警处理器 /<br/>alert_handler<br/>Orc 告警接收器 — handle_alert() 消费者<br/>文件: contracts/alert_handler.py"]
    src_zephyr_orchestrator_contracts_contract_registry_py["(生产态 / production) 契约注册表 /<br/>contract_registry<br/>集成契约注册表（Contract Registry）<br/>文件: contracts/contract_registry.py"]
    src_zephyr_orchestrator_core_task_queue_py["(生产态 / production) 任务队列 / task_<br/>queue<br/>ActiveTaskQueue — 后台任务轮询与自动分发<br/>文件: core/task_queue.py"]
    src_zephyr_orchestrator_execution_batch_orchestrator_py["(生产态 / production) 批次编排器 /<br/>batch_orchestrator<br/>BatchOrchestrator — 多 Worker<br/>批量任务协调器（MOD-INF-016）<br/>文件: execution/batch_orchestrator.py"]
    src_zephyr_orchestrator_execution_context_bridge_py["(生产态 / production) 上下文桥接 /<br/>context_bridge<br/>Orc->CE 上下文桥接 — request_context()<br/>生产者<br/>文件: execution/context_bridge.py"]
    src_zephyr_orchestrator_execution_memory_writer_py["(生产态 / production) Orc->VMS<br/>记忆写入器 / memory_writer<br/>Orc->VMS 记忆写入器<br/>文件: execution/memory_writer.py"]
    src_zephyr_orchestrator_execution_script_runner_py["(生产态 / production) script运行器 /<br/>script_runner<br/>Orc->Script 脚本执行器 — run_audit()<br/>生产者<br/>文件: execution/script_runner.py"]
    src_zephyr_orchestrator_fault_tolerance_bulkhead_manager_py["(生产态 / production) 舱壁管理器 /<br/>bulkhead_manager<br/>bulkhead管理器，主要提供获取配额、列表sy<br/>stems、检测slowcall等功能<br/>文件: fault_tolerance/bulkhead_<br/>manager.py"]
    src_zephyr_orchestrator_fault_tolerance_chaos_engine_py["(生产态 / production) chaos引擎 / chaos_<br/>engine<br/>Chaos 故障注入引擎<br/>（CT-CHAOS-001）——4注入点×月度执行。<br/>文件: fault_tolerance/chaos_engine.py"]
    src_zephyr_orchestrator_fault_tolerance_fault_types_py["(生产态 / production) 故障类型定义 /<br/>Fault type registry and preset<br/>templates for chaos engineeri<br/>fault类型定义。Fault type registry and<br/>preset templates for chaos engineering.<br/>文件: fault_tolerance/fault_types.py"]
    src_zephyr_orchestrator_file_task_mapper_py["(生产态 / production) 文件任务mapper /<br/>file_task_mapper<br/>FileTaskMapper — 文件路径 ↔ Task N:N<br/>映射器（#21 裁定重写）<br/>文件: orchestrator/file_task_mapper.py"]
    src_zephyr_orchestrator_governance_model_registry_py["(生产态 / production) 模型注册表 /<br/>model_registry<br/>模型注册表，主要提供获取、列表all、获取b<br/>y提供器等功能<br/>文件: governance/model_registry.py"]
    src_zephyr_orchestrator_lifecycle_housekeeping_py["(生产态 / production)<br/>文件卫生保洁管理器<br/>（CT-HOUSEKEEPING）——临时文件扫描+日志轮<br/>转+ / housekeeping<br/>文件卫生保洁管理器<br/>（CT-HOUSEKEEPING）——临时文件扫描+日志轮<br/>转+废弃目录清理。<br/>文件: lifecycle/housekeeping.py"]
    ...
    src_zephyr_orchestrator_quality_blueprint_scorer_py["(生产态 / production) 蓝图评分器 /<br/>blueprint_scorer<br/>BlueprintScorer — 蓝图路由统一打分逻辑<br/>文件: quality/blueprint_scorer.py"]
    src_zephyr_orchestrator_resilience_failure_matcher_py["(生产态 / production) 故障匹配器 /<br/>failure_matcher<br/>FailurePatternMatcher —<br/>任务失败模式识别与纠正建议<br/>文件: resilience/failure_matcher.py"]
    ...
    src_zephyr_orchestrator_file_task_mapper_py ~~~ src_zephyr_orchestrator_governance_model_registry_py
    src_zephyr_orchestrator_governance_model_registry_py ~~~ src_zephyr_orchestrator_lifecycle_housekeeping_py
    ...
    src_zephyr_orchestrator_execution_task_context_builder_py["(生产态 / production) 任务上下文构建器<br/>/ task_context_builder<br/>CE 任务上下文构建器 — build_from_task()<br/>消费者<br/>文件: execution/task_context_builder.py"]
    ...
    src_zephyr_orchestrator_governance_init_py -->|config_depends / config_depends| src_zephyr_orchestrator_governance_model_registry_py
