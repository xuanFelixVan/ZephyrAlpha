...
flowchart TD
    src_zephyr_orchestrator_init_py["zephyr/orchestrator 包入口<br/>文件: orchestrator/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_contracts_design_decisions_py["设计decisions<br/>编排器/契约包的设计decisions模块<br/>design_decisions<br/>文件: contracts/design_decisions.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_execution_reconciliation_loop_py["对账循环<br/>编排器/execution包的对账循环模块<br/>reconciliation_loop<br/>文件: execution/reconciliation_loop.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_governance_risk_registry_py["风险注册表<br/>编排器/治理包的风险注册表模块<br/>risk_registry<br/>文件: governance/risk_registry.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_lifecycle_incident_postmortem_py["事件复盘管理器（CT-INCIDENT）——incident记录+time<br/>lin<br/>incident_postmortem<br/>文件: lifecycle/incident_postmortem.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_lifecycle_startup_sequencer_py["启动sequencer<br/>编排器/lifecycle包的启动sequencer模块<br/>startup_sequencer<br/>文件: lifecycle/startup_sequencer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_lifecycle_teardown_manager_py["teardown管理器<br/>编排器/lifecycle包的teardown管理器模块<br/>teardown_manager<br/>文件: lifecycle/teardown_manager.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_quality_blind_spot_closure_py["盲点闭合<br/>编排器/quality包的盲点闭合模块<br/>blind_spot_closure<br/>文件: quality/blind_spot_closure.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_task_queue_py["任务队列<br/>编排器包的任务队列模块<br/>task_queue<br/>文件: orchestrator/task_queue.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_core_task_queue_py["任务队列<br/>编排器/核心包的任务队列模块<br/>task_queue<br/>文件: core/task_queue.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_execution_memory_writer_py["Orc->VMS 记忆写入器<br/>编排器/execution包的Orc->VMS 记忆写入器模块<br/>memory_writer<br/>文件: execution/memory_writer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_orchestrator_lifecycle_rolling_upgrade_py["零停机滚动升级（CT-DEPLOY）——graceful<br/>shutdown+流量<br/>rolling_upgrade<br/>文件: lifecycle/rolling_upgrade.py<br/>(生产态 / production)"]
