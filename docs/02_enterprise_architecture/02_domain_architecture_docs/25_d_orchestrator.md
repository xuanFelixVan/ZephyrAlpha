...
flowchart TD
    ...
    class D_AUTONOMY_CORE,D_SHARED,D_GOV_DRIFT,D_INTEGRATION,D_GOV_SCRIPTS,D_INFRA_RUNTIME,D_FEEDBACK_LOOP,D_TRADING external_prod
...
| 13 | Agent Orchestrator (orchestrator/agent_orchestrator.py) | → | D_SECURITY 对抗验证: 输入净化器 / Input Sanitizer (llm_security/input_sanitize... | 导入依赖 / import_depends |
...
| 18 | Agent Orchestrator (orchestrator/agent_orchestrator.py) | → | D_SHARED 共享服务: 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 36 | Task Context Builder (execution/task_context_builder.py) | → | D_SHARED 共享服务: 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 41 | Chaos Hooks (fault_tolerance/chaos_hooks.py) | → | D_SHARED 共享服务: 编排协议 / Orchestration Protocol (contracts/orchestratio... | 导入依赖 / import_depends |
...
| 47 | Hallucination Detector (orchestrator/hallucination_detect... | → | D_SHARED 共享服务: 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 5 | D_ML_TRAIN 训练: Training Pipeline (training_pipeline/) | → | deepseek/opus/gpt等模型版本+性能基线 / Model Registry (go... | runtime / runtime |
