...
# 32_d_ex_core / 执行核心 / 执行核心 / Execution Core
...
> 展示全部 16 个模块（生产态 6 + 设计态 1 + 原型态 9），标签标注成熟度。
...
graph TD
    ...
    classDef external_design fill:#fce4ec,stroke:#880e4f,stroke-width:1px,color:#000,stroke-dasharray: 5 5
    class src_zephyr_ex_core_init_py,src_zephyr_ex_core_adapters_broker_interface_py,src_zephyr_ex_core_adapters_simulation_broker_py,src_zephyr_ex_core_execution_engine_py,src_zephyr_ex_core_order_manager_py,src_zephyr_governance_escalation_order_state_escalator_py production
    class D_INFRASTRUCTURE,D_GOVERNANCE,D_SHARED,D_AUDITTEST external_design
```
...
| 14 | D_EXECUTION_CORE — Execution Engine (execution... | → | D_INFRASTRUCTURE: order.py | 导入依赖 / import_depends |
| 15 | D_EXECUTION_CORE — Execution Engine (execution... | → | D_INFRASTRUCTURE: risk_limits.py | 导入依赖 / import_depends |
| 16 | D_EXECUTION_CORE — Order Manager (order_manage... | → | D_INFRASTRUCTURE: fill.py | 导入依赖 / import_depends |
| 17 | D_EXECUTION_CORE — Order Manager (order_manage... | → | D_INFRASTRUCTURE: order.py | 导入依赖 / import_depends |
| 18 | MiniQMT 实盘券商适配器（对接 xttrader，A股实盘.... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 19 | D_EXECUTION_CORE — Order Manager (order_manage... | → | D_SHARED 共享服务: OrderSide/OrderStatus/OrderType — 交易枚举真源... | 导入依赖 / import_depends |
...
|:--:|---------|:--:|---------|---------|
| 1 | D_AUDITTEST 审计测试套件: test_order_state_escalator.py | → | Order State Escalator — v0.10.0 订单状态机升级... | 测试依赖 / test_depends |
| 2 | D_AUDITTEST 审计测试套件: test_cli.py | → | D_EXECUTION_CORE Trade Execution — Re-export w... | 测试依赖 / test_depends |
| 3 | D_FRONTEND 前端:  | → |  | 导入依赖 / import_depends |
| 4 | D_FRONTEND 前端:  | → |  | 导入依赖 / import_depends |
| 5 | D_GOVERNANCE 生命周期管理: C-track 端到端演示 —— 全流水线一次性运行 (dem... | → | D_EXECUTION_CORE Trade Execution — Re-export w... | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 7 个外部域直接连接（出边 22 条 + 入边 5 条 = 27 条）。只显示直接连接的域，不展开具体节点。
