...
|:--:|---------|---------|:---:|:---:|
| 1 | src/zephyr/signal_quality/__init__.py | D_SIGQC — Signal Quality Domain | 设计态 / design |  |
| 2 | src/zephyr/signal_quality/degradation_monitor_base.py | D_SIGQC — Signal Quality Degradation Monitor Base | 设计态 / design |  |
...
graph TD
    subgraph D_SIGQC["D_SIGQC 信号质量控制"]
        src_zephyr_signal_quality_init_py["(设计态 / design) D_SIGQC — Signal Quality Domain<br/>文件: __init__.py"]
        src_zephyr_signal_quality_degradation_monitor_base_py["(设计态 / design) D_SIGQC — Signal Quality Degradation Monitor Base<br/>文件: degradation_monitor_base.py"]
    end
    ...
    class src_zephyr_signal_quality_init_py,src_zephyr_signal_quality_degradation_monitor_base_py design
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 2 个，1 条域内依赖）。
...
|:--:|---------|:--:|---------|---------|
| 1 | D_SIGQC — Signal Quality Degradation Monitor B... | → | D_INFRASTRUCTURE 跨层契约基础设施: synthesized_signal.py | 导入依赖 / import_depends |
