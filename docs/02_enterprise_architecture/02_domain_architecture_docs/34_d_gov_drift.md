...
graph TD
    ...
    class D_GOVERNANCE,D_GOV_ENFORCEMENT,D_AUTONOMY_CORE,D_AUDITTEST external_design
```
...
| 4 | blueprint.md | → | D_GOVERNANCE 生命周期管理: Blueprint Bloat Monitor — v0.11.0 蓝图膨胀监控... | runtime / runtime |
| 5 | blueprint.md | → | D_GOVERNANCE 生命周期管理: GateEventAdapter — GateRepo 事件适配器（DW-000... | runtime / runtime |
| 6 | blueprint.md | → | D_GOV_ENFORCEMENT 规则执行: Audit Trail — MOD-INF-020 (__init__.py) | runtime / runtime |
| 7 | blueprint.md | → | D_SECURITY 对抗验证: InputGuard — 输入参数守卫. (input_guard.py) | runtime / runtime |
