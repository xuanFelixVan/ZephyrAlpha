...
> 展示全部 16 个模块（生产态 0 + 设计态 0 + 原型态 16），标签标注成熟度。
...
graph TD
    ...
    scripts_governance_meta_manage_kill_switch_py -.->|config_depends / config_depends| D_GOV_SCRIPTS
...
| 2 | manage_kill_switch.py — Kill Switch 管理工具 (... | → | D_GOV_SCRIPTS 脚本治理: meta/ — 脚本系统自我审计维度（第 13 维度） (__... | config_depends / config_depends |
| 3 | Re-export wrapper: red-blue-validator has migra... | → | D_SECURITY 对抗验证: attack_registry.py | 导入依赖 / import_depends |
| 4 | Re-export wrapper: red-blue-validator has migra... | → | D_SECURITY 对抗验证: bypass_recorder.py | 导入依赖 / import_depends |
| 5 | Re-export wrapper: red-blue-validator has migra... | → | D_SECURITY 对抗验证: constitution_guard.py | 导入依赖 / import_depends |
| 6 | Re-export wrapper: red-blue-validator has migra... | → | D_SECURITY 对抗验证: convergence_checker.py | 导入依赖 / import_depends |
| 7 | Re-export wrapper: red-blue-validator has migra... | → | D_SECURITY 对抗验证: defense_runner.py | 导入依赖 / import_depends |
| 8 | Re-export wrapper: red-blue-validator has migra... | → | D_SECURITY 对抗验证: game_day_runner.py | 导入依赖 / import_depends |
| 9 | Re-export wrapper: attack_registry has migrated... | → | D_SECURITY 对抗验证: attack_registry.py | 导入依赖 / import_depends |
| 10 | Re-export wrapper: bypass_recorder has migrated... | → | D_SECURITY 对抗验证: bypass_recorder.py | 导入依赖 / import_depends |
| 11 | Re-export wrapper: constitution_guard has migra... | → | D_SECURITY 对抗验证: constitution_guard.py | 导入依赖 / import_depends |
| 12 | Re-export wrapper: convergence_checker has migr... | → | D_SECURITY 对抗验证: convergence_checker.py | 导入依赖 / import_depends |
| 13 | Re-export wrapper: defense_runner has migrated ... | → | D_SECURITY 对抗验证: defense_runner.py | 导入依赖 / import_depends |
| 14 | Re-export wrapper: game_day_runner has migrated... | → | D_SECURITY 对抗验证: game_day_runner.py | 导入依赖 / import_depends |
...
> 本域与 3 个外部域直接连接（出边 14 条 + 入边 0 条 = 14 条）。只显示直接连接的域，不展开具体节点。
