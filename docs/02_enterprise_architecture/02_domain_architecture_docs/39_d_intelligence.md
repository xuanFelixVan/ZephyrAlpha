...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_ENFORCEMENT,D_TRADING,D_AUTONOMY_CORE,D_INTEGRATION external_prod
    class D_ML_TRAIN,D_SECURITY external_design
```
...
graph TD
    ...
    class D_AUTONOMY_CORE,D_GOVERNANCE,D_INTEGRATION,D_BACKTEST,D_SHARED,D_TRADING external_prod
```
...
> 本域与 12 个外部域直接连接（出边 32 条 + 入边 59 条 = 91 条）。只显示直接连接的域，不展开具体节点。
