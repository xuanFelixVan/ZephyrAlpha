...
> 展示全部 15 个模块（生产态 11 + 设计态 4），节点含成熟度+中英文名+大白话+文件路径。

```mermaid
%%{init: {'theme': 'base', 'themeVariables': {'primaryColor': '#eaeaea', 'primaryTextColor': '#333333', 'primaryBorderColor': '#666666', 'lineColor': '#666666', 'secondaryColor': '#eaeaea', 'tertiaryColor': '#eaeaea', 'fontSize': '14px'}}}%%
flowchart TD
    src_zephyr_risk_ashare_stop_loss_rule_engine_py["(设计态 / design)<br/>文件: risk/ashare_stop_loss_rule_engine.py"]
    src_zephyr_risk_ashare_systemic_risk_detector_py["(设计态 / design)<br/>文件: risk/ashare_systemic_risk_detector.py"]
    ...
    src_zephyr_risk_drawdown_realtime_tracker_py["(设计态 / design)<br/>文件: risk/drawdown_realtime_tracker.py"]
    src_zephyr_risk_drawdown_tracker["(设计态 / design)<br/>文件: drawdown_tracker/"]
...
> 仅展示蓝图阶段、代码未写的设计态模块（共 4 个，0 条域内依赖）。
...
flowchart TD
    ...
    classDef external_design fill:#fff8e7,stroke:#ef6c00,stroke-width:1px,color:#000,stroke-dasharray: 5 5
    class src_zephyr_risk_ashare_stop_loss_rule_engine_py,src_zephyr_risk_ashare_systemic_risk_detector_py,src_zephyr_risk_drawdown_realtime_tracker_py,src_zephyr_risk_drawdown_tracker design
    class D_TRADING,D_EX_CORE,D_POSITION external_design
```
