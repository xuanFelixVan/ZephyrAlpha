...
flowchart TD
    ...
    src_zephyr_compliance_behavioral_auditor_init_py["包入口<br/>compliance/behavioral_auditor<br/>包入口，管理该层子模块的统一加载和懒导入<br/>__init__<br/>文件: behavioral_auditor/__init__.py<br/>(生产态 / production)"]
    src_zephyr_compliance_zero_knowledge_audit_stub_init_py["包入口<br/>包入口。D_COMPLIANCE Compliance<br/>文件: zero_knowledge_audit_stub/__init__.py<br/>(生产态 / production)"]
