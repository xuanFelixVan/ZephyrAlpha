...
flowchart TD
    ...
    src_zephyr_compliance_behavioral_auditor_init_py["包入口<br/>审计的包入口，把这一层的子模块归到一起统一管理，<br/>用到谁才加载谁，避免一次性全加载拖慢启动。<br/>__init__<br/>文件: behavioral_auditor/__init__.py<br/>(生产态 / production)"]
    src_zephyr_compliance_zero_knowledge_audit_stub_init_py["包入口<br/>包入口。D_COMPLIANCE Compliance<br/>D_COMPLIANCE Compliance<br/>D_COMPLIANCE Compliance<br/>文件: zero_knowledge_audit_stub/__init__.py<br/>(生产态 / production)"]
