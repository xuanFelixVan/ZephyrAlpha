...
flowchart TD
    src_zephyr_compliance_async_intercept_queue_py["compliance/async_intercept_queue<br/>compliance包的async_intercept_queue模块<br/>⛔ 合规域，设计已就绪，等待开发排期<br/>文件: compliance/async_intercept_queue.py<br/>(设计态 / design)"]
    src_zephyr_compliance_behavioral_auditor_init_py["compliance/behavioral_auditor 包入口<br/>管理compliance.behavioral_auditor子包的加载和懒<br/>导入<br/>文件: behavioral_auditor/__init__.py<br/>(生产态 / production)"]
    src_zephyr_compliance_zero_knowledge_audit_stub_init_py["compliance/zero_knowledge_audit_stub 包入口<br/>D_COMPLIANCE Compliance<br/>文件: zero_knowledge_audit_stub/__init__.py<br/>(生产态 / production)"]
...
| 44 | compliance/async_intercept_queue.py | → | D_GOV_OPS_RESILIENCE 运维弹性治理: D_COMPLIANCE — Governance & Compliance Layer (security_g... | runtime / runtime |
| 45 | behavioral_auditor/__init__.py | → | D_INFRA_RUNTIME 运行时集成: auto_fix_engine/state_machine.py | 导入依赖 / import_depends |
| 46 | behavioral_auditor/__init__.py | → | D_SECURITY 对抗验证: Alert Router — alert_router.py (gov_drift/alert_router.py) | 导入依赖 / import_depends |
| 47 | behavioral_auditor/__init__.py | → | D_SECURITY 对抗验证: Cold Start Bootstrapper — 冷启动引导 §6.31。 (gov_drift... | 导入依赖 / import_depends |
| 48 | behavioral_auditor/__init__.py | → | D_SECURITY 对抗验证: G-CT-005 — ManagedDriftEvent Pydantic V2 BaseModel 漂移... | 导入依赖 / import_depends |
| 49 | behavioral_auditor/__init__.py | → | D_SECURITY 对抗验证: Auto Reconciler — reconciler.py (gov_drift/reconciler.py) | 导入依赖 / import_depends |
| 50 | behavioral_auditor/__init__.py | → | D_SECURITY 对抗验证: Drift Runbook Generator — 漂移演练手册自动生成。 (gov_dr... | 导入依赖 / import_depends |
