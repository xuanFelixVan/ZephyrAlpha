...
flowchart TD
    ...
    src_zephyr_feedback_loop_detectors_anomaly_flapping_detector_py["flapping检测器<br/>flapping检测器。Flapping Detector — v0.40.0 R494<br/>文件: anomaly/flapping_detector.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_anomaly_heisenbug_detector_py["heisenbug检测器<br/>heisenbug检测器。Heisenbug Detector — v0.38.0<br/>R470<br/>文件: anomaly/heisenbug_detector.py<br/>(生产态 / production)"]
    ...
    src_zephyr_feedback_loop_detectors_anomaly_silent_corruption_detector_py["silentcorruption检测器<br/>silentcorruption检测器。Silent Corruption<br/>Detector — v0.40.0 R499<br/>文件: anomaly/silent_corruption_detector.py<br/>(生产态 / production)"]
    ...
    src_zephyr_feedback_loop_detectors_anomaly_temporal_pattern_py["temporal模式<br/>temporal模式。Temporal Pattern Detector —<br/>v0.12.0 R164<br/>文件: anomaly/temporal_pattern.py<br/>(生产态 / production)"]
    ...
    src_zephyr_feedback_loop_detectors_correlation_agent_trajectory_anomaly_detector_py["代理trajectory异常检测器<br/>代理trajectory异常检测器。R503:<br/>AgentTrajectoryAnomalyDetector<br/>文件: correlation/agent_trajectory_anomaly_<br/>detector.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_correlation_cross_signal_validator_py["跨信号校验器<br/>跨信号校验器。Cross-Signal Validator — v0.6.0<br/>R63<br/>文件: correlation/cross_signal_validator.py<br/>(生产态 / production)"]
    ...
    src_zephyr_feedback_loop_detectors_correlation_ensemble_detector_py["ensemble检测器<br/>ensemble检测器。Ensemble Detector — v0.4.0 R21<br/>文件: correlation/ensemble_detector.py<br/>(生产态 / production)"]
    ...
    src_zephyr_feedback_loop_detectors_correlation_external_validation_checkpoint_py["外部验证检查点<br/>外部验证检查点。R524:<br/>ExternalValidationCheckpoint<br/>文件: correlation/external_validation_<br/>checkpoint.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_correlation_fle_performance_regression_detector_py["fle绩效回归检测器<br/>fle绩效回归检测器。R532:<br/>FLEPerformanceRegressionDetector<br/>文件: correlation/fle_performance_regression_<br/>detector.py<br/>(生产态 / production)"]
    ...
    src_zephyr_feedback_loop_detectors_correlation_trace_causal_bridge_py["追踪causal桥接<br/>追踪causal桥接。Trace Causal Bridge — v0.6.0 R62<br/>文件: correlation/trace_causal_bridge.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_correlation_traffic_replay_validator_py["trafficreplay校验器<br/>trafficreplay校验器。Traffic Replay Validator —<br/>v0.14.0 R202<br/>文件: correlation/traffic_replay_validator.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_drift_concept_drift_py["concept漂移<br/>concept漂移。Concept Drift Detector — v0.5.0 R42<br/>文件: drift/concept_drift.py<br/>(生产态 / production)"]
    ...
    src_zephyr_feedback_loop_detectors_drift_context_window_contamination_detector_py["上下文windowcontamination检测器<br/>上下文windowcontamination检测器。Context Window<br/>Contamination Detector — v0.38.0 R471<br/>文件: drift/context_window_contamination_<br/>detector.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_drift_diminishing_returns_detector_py["diminishingreturns检测器<br/>diminishingreturns检测器。R528:<br/>DiminishingReturnsDetector<br/>文件: drift/diminishing_returns_detector.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_drift_ensemble_drift_py["ensemble漂移<br/>ensemble漂移。Ensemble Drift — v0.5.0 R43<br/>文件: drift/ensemble_drift.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_drift_gradual_poisoning_detector_py["gradualpoisoning检测器<br/>gradualpoisoning检测器。Gradual Poisoning<br/>Detector — v0.15.0 R210<br/>文件: drift/gradual_poisoning_detector.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_drift_trend_cycle_separator_py["趋势cycleseparator<br/>趋势cycleseparator。Trend-Cycle Separator —<br/>v0.9.0 R113<br/>文件: drift/trend_cycle_separator.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_guard_alert_desensitization_curve_py["告警desensitizationcurve<br/>告警desensitizationcurve。Alert Desensitization<br/>Curve — v0.37.0 R492<br/>文件: guard/alert_desensitization_curve.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_guard_guard_cascade_detector_py["守卫级联检测器<br/>守卫级联检测器。R520: GuardCascadeDetector<br/>文件: guard/guard_cascade_detector.py<br/>(生产态 / production)"]
    ...
    src_zephyr_feedback_loop_detectors_guard_positive_feedback_defense_py["positive反馈防御<br/>positive反馈防御。Positive Feedback Defense —<br/>v0.4.0 R28<br/>文件: guard/positive_feedback_defense.py<br/>(生产态 / production)"]
    ...
    src_zephyr_feedback_loop_detectors_reliability_autoscale_remediation_py["自动伸缩修复<br/>自动伸缩修复，提供包入口和模块加载功能<br/>Autoscale Remediation — v0.13.0 R174<br/>文件: reliability/autoscale_remediation.py<br/>(生产态 / production)"]
    ...
    src_zephyr_feedback_loop_detectors_reliability_blast_radius_budget_py["blastradius预算<br/>blastradius预算。Blast Radius Budget — v0.13.0<br/>R178<br/>文件: reliability/blast_radius_budget.py<br/>(生产态 / production)"]
    ...
    src_zephyr_feedback_loop_detectors_reliability_flag_lifecycle_py["标志生命周期<br/>标志生命周期。Flag Lifecycle Detector — v0.13.0<br/>R180<br/>文件: reliability/flag_lifecycle.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_reliability_maintenance_coordinator_py["maintenance协调器<br/>maintenance协调器。Maintenance Coordinator —<br/>v0.12.0 R168<br/>文件: reliability/maintenance_coordinator.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_reliability_metric_cardinality_guard_py["指标cardinality守卫<br/>指标cardinality守卫。Metric Cardinality Guard —<br/>v0.40.0 R495<br/>文件: reliability/metric_cardinality_guard.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_reliability_openfeature_py["OpenFeature<br/>OpenFeature特性开关检测器，基于OpenFeature标准检<br/>测可靠性相关特性标志<br/>OpenFeature Integration — v0.13.0 R181<br/>文件: reliability/openfeature.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_reliability_otel_adapter_py["otel适配器<br/>otel适配器。OTel Adapter — v0.12.0 R170<br/>文件: reliability/otel_adapter.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_reliability_regulatory_audit_py["regulatory审计<br/>regulatory审计。Regulatory Audit Detector —<br/>v0.13.0 R184<br/>文件: reliability/regulatory_audit.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_reliability_resolution_tracker_py["resolution追踪器<br/>resolution追踪器。Resolution Tracker — v0.12.0<br/>R165<br/>文件: reliability/resolution_tracker.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_reliability_runbook_executor_py["runbook执行器<br/>runbook执行器。Runbook Executor — v0.13.0 R186a<br/>文件: reliability/runbook_executor.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_reliability_version_migrator_py["版本migrator<br/>版本migrator。Version Migrator — v0.12.0 R169<br/>文件: reliability/version_migrator.py<br/>(生产态 / production)"]
    ...
    src_zephyr_feedback_loop_detectors_anomaly_init_py["包入口<br/>detectors/anomaly<br/>包入口，管理该层子模块的统一加载和懒导入<br/>__init__<br/>文件: anomaly/__init__.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_correlation_init_py["包入口<br/>detectors/correlation<br/>包入口，管理该层子模块的统一加载和懒导入<br/>__init__<br/>文件: correlation/__init__.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_drift_init_py["包入口<br/>detectors/drift<br/>包入口，管理该层子模块的统一加载和懒导入<br/>__init__<br/>文件: drift/__init__.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_guard_init_py["包入口<br/>detectors/guard<br/>包入口，管理该层子模块的统一加载和懒导入<br/>__init__<br/>文件: guard/__init__.py<br/>(生产态 / production)"]
    src_zephyr_feedback_loop_detectors_reliability_init_py["包入口<br/>detectors/reliability<br/>包入口，管理该层子模块的统一加载和懒导入<br/>__init__<br/>文件: reliability/__init__.py<br/>(生产态 / production)"]
