...
flowchart TD
    ...
    D_RISK -->|导入依赖 / import_depends| src_zephyr_feedback_loop_detectors_anomaly_emergent_behavior_detector_py
    ...
    class D_FEEDBACK_LOOP,D_RISK,D_GOVERNANCE external_prod
```
...
| 8 | D_RISK 风控: D_RISK — AI/Agent Risk Monitor (MOD-RK-14) (core/ai_agen... | → | emergentbehavior检测器 / Emergent Behavior Detector — v0... | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 3 个外部域直接连接（出边 3 条 + 入边 8 条 = 11 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_RISK -->|1条 导入依赖 / import_depends| D_FBL_DETECTORS
```
