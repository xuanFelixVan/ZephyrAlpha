...
flowchart TD
    docs_01_policies_and_standards_registry_catalogs_rule_registry_collection_yaml["(生产态 / production) 规则注册表收集 /<br/>rule_registry_collection<br/>规则注册表收集，机器学习的注册表，登记和<br/>查询已注册的条目。<br/>文件: catalogs/rule_registry_<br/>collection.yaml"]
    scripts_a2a_full_verification_py["(生产态 / production) A2Afull验证 / a2a_<br/>full_verification<br/>A2A Protocol 全链路满分验证脚本<br/>文件: scripts/a2a_full_verification.py"]
    scripts_arch_guard_tools_build_ocp_manifest_py["(生产态 / production) buildocp清单 /<br/>build_ocp_manifest<br/>从 cross_layer_contracts.yaml 生成 OCP<br/>冻结契约指纹（INV-009）。<br/>文件: _tools/build_ocp_manifest.py"]
    scripts_arch_guard_tools_inject_idempotency_py["(生产态 / production) inject幂等性 /<br/>inject_idempotency<br/>为所有 P0/P1 契约添加 idempotency_key<br/>字段——状态感知版本。<br/>文件: _tools/inject_idempotency.py"]
    scripts_arch_guard_tools_patch_p1_paths_py["(生产态 / production) 补丁p1paths /<br/>patch_p1_paths<br/>一次性工具——为 9 个 P1 契约补齐<br/>physical_path 并运行 codegen。<br/>文件: _tools/patch_p1_paths.py"]
    scripts_arch_guard_check_acl_boundary_py["(生产态 / production) 检查aclboundary /<br/>check_acl_boundary<br/>Broker ACL 边界强制执行<br/>文件: arch_guard/check_acl_boundary.py"]
    scripts_arch_guard_check_cross_plane_communication_py["(生产态 / production)<br/>check跨planecommunication / check_cross_<br/>plane_communication<br/>INV-011 拓扑 + 静态越界 import 嗅探<br/>文件: arch_guard/check_cross_plane_<br/>communication.py"]
    scripts_arch_guard_check_fe_acl_boundary_py["(生产态 / production) 检查feaclboundary<br/>/ check_fe_acl_boundary<br/>INV-006 前端 ACL（仓库内有前端树则启用）<br/>文件: arch_guard/check_fe_acl_<br/>boundary.py"]
    scripts_arch_guard_check_hot_path_purity_py["(生产态 / production) 检查hot路径purity<br/>/ check_hot_path_purity<br/>INV-012 Hot 路径 Python 禁 asyncio<br/>（配置驱动）<br/>文件: arch_guard/check_hot_path_<br/>purity.py"]
    scripts_arch_guard_check_scaffold_exit_gates_py["(生产态 / production)<br/>checkscaffold退出门禁 / check_scaffold_<br/>exit_gates<br/>scaffold→experimental 安全门禁检查<br/>文件: arch_guard/check_scaffold_exit_<br/>gates.py"]
    scripts_arch_guard_check_schema_consistency_py["(生产态 / production) 检查模式一致性 /<br/>check_schema_consistency<br/>INV-010 契约物理路径存在性（Schema<br/>canonical 基线）<br/>文件: arch_guard/check_schema_<br/>consistency.py"]
    scripts_arch_guard_fitness_functions_check_aisg_gateway_py["(生产态 / production) 检查aisg网关 /<br/>check_aisg_gateway<br/>AISG 拦截门禁 (INV-015) Phase B 升级<br/>文件: fitness_functions/check_aisg_<br/>gateway.py"]
    scripts_arch_guard_fitness_functions_check_audit_log_immutability_py["(生产态 / production)<br/>check审计日志immutability / check_audit_<br/>log_immutability<br/>审计日志不可篡改检查<br/>文件: fitness_functions/check_audit_log_<br/>immutability.py"]
    scripts_arch_guard_fitness_functions_check_capacity_slo_ssot_py["(生产态 / production) check容量slossot<br/>/ check_capacity_slo_ssot<br/>check容量slossot.yaml 注册表 + 与<br/>invariants 数字对齐（SSoT 闭环）<br/>文件: fitness_functions/check_capacity_<br/>slo_ssot.py"]
    scripts_arch_guard_fitness_functions_check_daily_loss_limit_py["(生产态 / production)<br/>checkdaily损失limit / check_daily_loss_<br/>limit<br/>日损失限额自动暂停<br/>文件: fitness_functions/check_daily_<br/>loss_limit.py"]
    scripts_arch_guard_fitness_functions_check_hot_warm_ipc_py["(生产态 / production) 检查hotwarmipc /<br/>check_hot_warm_ipc<br/>INV-018 Hot↔Warm IPC 协议检查<br/>文件: fitness_functions/check_hot_warm_<br/>ipc.py"]
    scripts_arch_guard_fitness_functions_check_idempotency_key_py["(生产态 / production) 检查幂等性密钥 /<br/>check_idempotency_key<br/>幂等 Key 字段存在性检查<br/>文件: fitness_functions/check_<br/>idempotency_key.py"]
    scripts_arch_guard_fitness_functions_check_log_secret_leak_py["(生产态 / production) check日志密钥leak<br/>/ check_log_secret_leak<br/>R2 日志不写 secret 适应度函数<br/>文件: fitness_functions/check_log_<br/>secret_leak.py"]
    scripts_arch_guard_fitness_functions_check_no_cross_plane_mutable_state_py["(生产态 / production)<br/>checkno跨planemutable状态 / check_no_<br/>cross_plane_mutable_state<br/>INV-020 跨平面共享可变状态检查<br/>文件: fitness_functions/check_no_cross_<br/>plane_mutable_state.py"]
    scripts_arch_guard_fitness_functions_check_ocp_signatures_py["(生产态 / production) 检查ocpsignatures<br/>/ check_ocp_signatures<br/>OCP 冻结契约指纹校验<br/>文件: fitness_functions/check_ocp_<br/>signatures.py"]
    scripts_arch_guard_fitness_functions_check_pit_compliance_py["(生产态 / production) 检查pit合规 /<br/>check_pit_compliance<br/>检查pit合规（Point-in-Time）铁律强制执行<br/>文件: fitness_functions/check_pit_<br/>compliance.py"]
    scripts_arch_guard_fitness_functions_check_position_limit_py["(生产态 / production) 检查持仓限制 /<br/>check_position_limit<br/>单一持仓限制 ≤ 5% NAV<br/>文件: fitness_functions/check_position_<br/>limit.py"]
    scripts_arch_guard_fitness_functions_check_risk_params_consistency_py["(生产态 / production)<br/>check风险paramsconsistency / check_risk_<br/>params_consistency<br/>风控参数真源 (INV-013) + 与 INV-002<br/>声明对齐<br/>文件: fitness_functions/check_risk_<br/>params_consistency.py"]
    scripts_arch_guard_fitness_functions_check_survivorship_bias_py["(生产态 / production)<br/>检查survivorshipbias / check_<br/>survivorship_bias<br/>Survivorship 策略门禁<br/>文件: fitness_functions/check_<br/>survivorship_bias.py"]
    scripts_arch_guard_fitness_functions_check_warm_cold_async_py["(生产态 / production) checkwarm冷异步 /<br/>check_warm_cold_async<br/>INV-019 Warm→Cold 异步通信检查<br/>文件: fitness_functions/check_warm_cold_<br/>async.py"]
    scripts_arch_guard_run_all_py["(生产态 / production) 运行all / run_all<br/>Architecture Guard 编排器<br/>文件: arch_guard/run_all.py"]
    scripts_construction_e2e_check_py["(生产态 / production) 端到端检查 / _e2e_<br/>check<br/>端到端检查，construction的检查器，检查某<br/>项条件是否满足。<br/>文件: construction/_e2e_check.py"]
    scripts_construction_e2e_deep_py["(生产态 / production) 端到端deep / _e2e_<br/>deep<br/>端到端deep，依赖检查statuses工作<br/>文件: construction/_e2e_deep.py"]
    scripts_construction_check_statuses_py["(生产态 / production) 检查statuses /<br/>check_statuses<br/>检查statuses，construction的检查器，检查<br/>某项条件是否满足。<br/>文件: construction/check_statuses.py"]
    scripts_construction_d_init_task_system_py["(生产态 / production)<br/>初始化任务系统数据库 +<br/>创建任务系统自身的施工任务卡（吃狗粮）<br/>/ d_init_task_system<br/>初始化任务系统数据库 +<br/>创建任务系统自身的施工任务卡（吃狗粮）<br/>文件: construction/d_init_task_system.py"]
    scripts_construction_demo_a2a_chat_py["(生产态 / production) A2A 多 Agent<br/>聊天演示 - Alpha 和 Beta 讨论项目评估 /<br/>demo_a2a_chat<br/>A2A 多 Agent 聊天演示 - Alpha 和 Beta<br/>讨论项目评估<br/>文件: construction/demo_a2a_chat.py"]
    scripts_construction_demo_a2a_coordination_py["(生产态 / production) A2A<br/>协议协调任务演示 / demo_a2a_coordination<br/>A2A 协议协调任务演示<br/>文件: construction/demo_a2a_<br/>coordination.py"]
    scripts_construction_demo_e2e_pipeline_py["(生产态 / production) demoe2e管线 /<br/>demo_e2e_pipeline<br/>C-track 端到端演示 —— 全流水线一次性运行<br/>文件: construction/demo_e2e_pipeline.py"]
    scripts_construction_finalize_tasks_py["(生产态 / production) finalize任务 /<br/>finalize_tasks<br/>finalize任务，依赖任务repo、sqlite模式、<br/>包入口工作<br/>文件: construction/finalize_tasks.py"]
    scripts_construction_local_layer_daemon_py["(生产态 / production) 本地层daemon /<br/>local_layer_daemon<br/>L2 本地模型层守护进程<br/>（薄包装，DEPRECATED）<br/>文件: construction/local_layer_daemon.py"]
    scripts_construction_reset_test_task_py["(生产态 / production) 重置测试任务 /<br/>reset_test_task<br/>重置测试任务，依赖sqlite模式工作<br/>文件: construction/reset_test_task.py"]
    scripts_construction_start_brain_py["(生产态 / production) 启动brain / start_<br/>brain<br/>ZephyrAlpha 系统大脑一键启动<br/>文件: construction/start_brain.py"]
    scripts_construction_test_event_hook_py["(生产态 / production) 测试事件钩子 /<br/>test_event_hook<br/>测试事件钩子，construction的事件，定义和<br/>分发事件。<br/>文件: construction/test_event_hook.py"]
    scripts_context_generate_architecture_context_py["(生产态 / production) 生成架构上下文 /<br/>generate_architecture_context<br/>预编译架构上下文包生成器<br/>文件: context/generate_architecture_<br/>context.py"]
    scripts_diagnose_breadth_failed_py["(生产态 / production)<br/>diagnosebreadth失败 / diagnose_breadth_<br/>failed<br/>诊断 breadth_failed 能力的根因。<br/>文件: scripts/diagnose_breadth_failed.py"]
    scripts_dm90971_add_test_headers_py["(生产态 / production)<br/>dm90971add测试headers / DM-90971: Batch<br/>add module_id scope prefix + governance<br/>anch<br/>dm90971新增测试headers。DM-90971: Batch<br/>add module_id scope prefix + governance<br/>anchor headers to test files.<br/>文件: scripts/dm90971_add_test_<br/>headers.py"]
    scripts_fix_freeze_manifest_py["(生产态 / production) 修复freeze清单 /<br/>Fix freezemanifest.yaml - comprehensive<br/>repair of all corrup<br/>修复freezemanifest。Fix<br/>freezemanifest.yaml - comprehensive<br/>repair of all corrupted desc fields.<br/>文件: scripts/fix_freeze_manifest.py"]
    scripts_fix_orphan_all_py["(生产态 / production) 修复孤儿all / fix_<br/>orphan_all<br/>自动修复 __init__.py __all__ 孤儿模块<br/>文件: scripts/fix_orphan_all.py"]
    scripts_generate_manifest_py["(生产态 / production) generate清单 /<br/>Generate complete script_manifest.yaml<br/>from scripts/ tree sc<br/>生成manifest。Generate complete script_<br/>manifest.yaml from scripts/ tree scan.<br/>文件: scripts/generate_manifest.py"]
    scripts_generate_pathway_registry_py["(生产态 / production)<br/>generatepathway注册表 / generate_<br/>pathway_registry<br/>从所有 MOD 蓝图的 §路径索引<br/>章节自动生成 system-pathway-registry.yam<br/>l。<br/>文件: scripts/generate_pathway_<br/>registry.py"]
    scripts_governance_d5_architecture_generators_zoomable_html_py["(生产态 / production) 可缩放 Mermaid<br/>HTML 生成器（共享模块）。 / zoomable_<br/>html<br/>可缩放 Mermaid HTML 生成器（共享模块）。<br/>文件: generators/zoomable_html.py"]
    scripts_governance_d7_code_check_pure_shim_py["(生产态 / production) 检查pureshim /<br/>check_pure_shim<br/>GATE-NO-PURE-SHIM 检测器（治本漏洞1<br/>2026-06-29）<br/>文件: d7_code/check_pure_shim.py"]
    scripts_governance_generators_generate_rule_ai_perception_index_py["(生产态 / production)<br/>generate规则aiperception索引 / generate_<br/>rule_ai_perception_index<br/>规则AI感知索引生成器<br/>（#ARCH-GOV-CONVERGENCE-META Phase<br/>3.2a）<br/>文件: generators/generate_rule_ai_<br/>perception_index.py"]
    scripts_hooks_auto_handoff_log_py["(生产态 / production) 自动handoff日志 /<br/>auto_handoff_log<br/>执行 git 命令并返回 stdout（UTF-8<br/>解码）。<br/>文件: hooks/auto_handoff_log.py"]
    scripts_lock_files_py["(生产态 / production) 锁files / lock_<br/>files<br/>— AI 对话文件锁协议（硬规则执行工具）<br/>文件: scripts/lock_files.py"]
    scripts_mcp_launcher_py["(生产态 / production) MCP DAG<br/>编排启动器（MOD-INF-013 §14 拓扑排序 +<br/>Pro / launcher<br/>MCP DAG 编排启动器（MOD-INF-013 §14<br/>拓扑排序 + ProcessLifecycleGateway<br/>管理）。<br/>文件: mcp/launcher.py"]
    scripts_mcp_start_all_py["(生产态 / production) 启动all / start_<br/>all<br/>MCP 全 Server 启动脚本 — DEPRECATED.<br/>文件: mcp/start_all.py"]
    scripts_mcp_status_all_py["(生产态 / production) 状态all / status_<br/>all<br/>MCP 全 Server 状态检查脚本（MOD-INF-013<br/>§14）。<br/>文件: mcp/status_all.py"]
    scripts_mcp_stop_all_py["(生产态 / production) 停止all / stop_all<br/>MCP 全 Server 停止脚本（MOD-INF-013<br/>§14）。<br/>文件: mcp/stop_all.py"]
    scripts_migration_dm311_autonomy_core_split_py["(生产态 / production)<br/>dm311autonomy核心split / dm311_autonomy_<br/>core_split<br/>DM-311: autonomy_core/<br/>拆分迁移执行脚本。<br/>文件: migration/dm311_autonomy_core_<br/>split.py"]
    scripts_migration_governance_root_split_py["(生产态 / production) 治理根拆分 /<br/>ARCH-031: governance/ root flat-files<br/>split migration orches<br/>治理根拆分。ARCH-031: governance/ root<br/>flat-files split migration orchestrator.<br/>文件: migration/governance_root_split.py"]
    scripts_ops_verify_header_completeness_py["(生产态 / production)<br/>文件头部完整性校验（6 格式统一入口） /<br/>verify_header_completeness<br/>文件头部完整性校验（6 格式统一入口）<br/>文件: ops/verify_header_completeness.py"]
    scripts_post_checkout_guard_py["(生产态 / production) postcheckout守卫<br/>/ post_checkout_guard<br/>Post-checkout Guard — 事后检测 checkout<br/>是否覆盖了其他 session 的文件锁。<br/>文件: scripts/post_checkout_guard.py"]
    scripts_pre_commit_verify_dedup_py["(生产态 / production) verify去重 /<br/>verify_dedup<br/>pre_commit 验证脚本 — 委托给<br/>code-dedup-engine CLI verify 子命令.<br/>文件: pre_commit/verify_dedup.py"]
    scripts_rollback_py["(生产态 / production) 回滚 / rollback<br/>Rollback System CLI — MOD-INF-021<br/>v0.10.0 Git-native+SQLite Checkpoint<br/>操作入口。<br/>文件: scripts/rollback.py"]
    scripts_run_deepseek_v4_exam_py["(生产态 / production)<br/>运行deepseekv4exam / run_deepseek_v4_<br/>exam<br/>DeepSeek V4 入职考试运行脚本<br/>文件: scripts/run_deepseek_v4_exam.py"]
    scripts_run_ollama_exam_py["(生产态 / production) 运行ollamaexam /<br/>run_ollama_exam<br/>Ollama 入职考试运行脚本<br/>文件: scripts/run_ollama_exam.py"]
    scripts_scaffold_py["(生产态 / production) scaffold.py —<br/>ZephyrAlpha 唯一创建入口（RULE-TW /<br/>scaffold<br/>ZephyrAlpha 唯一创建入口（RULE-TWO<br/>强制执行器）<br/>文件: scripts/scaffold.py"]
    scripts_setup_git_guard_aliases_py["(生产态 / production)<br/>setupGit守卫aliases / setup_git_guard_<br/>aliases<br/>Setup/Remove Git Aliases for Git Guard<br/>— 自动化集成入口。<br/>文件: scripts/setup_git_guard_aliases.py"]
    src_zephyr_governance_a2a_init_py["(生产态 / production) 包入口 / __init__<br/>治理的包入口，把这一层的子模块归到一起统<br/>一管理，用到谁才加载谁，避免一次性全加载<br/>拖慢启动。<br/>文件: a2a/__init__.py"]
    src_zephyr_governance_adapters_risk_validation_bridge_py["(生产态 / production) 风险验证桥接 / D_<br/>EXECUTION_CORE — Risk Validation Bridge<br/>(DW-239)<br/>风险验证桥接。D_EXECUTION_CORE — Risk<br/>Validation Bridge (DW-239)<br/>文件: adapters/risk_validation_bridge.py"]
    src_zephyr_governance_adapters_simulation_broker_py["(生产态 / production) 仿真经纪人 / D_<br/>EXECUTION_CORE — Simulation Broker<br/>Adapter<br/>仿真经纪人。D_EXECUTION_CORE —<br/>Simulation Broker Adapter<br/>文件: adapters/simulation_broker.py"]
    src_zephyr_governance_agent_spec_init_py["(生产态 / production) 包入口 / __init__<br/>治理的包入口，把这一层的子模块归到一起统<br/>一管理，用到谁才加载谁，避免一次性全加载<br/>拖慢启动。<br/>文件: agent-spec/__init__.py"]
    src_zephyr_governance_agent_spec_a2a_failure_py["(生产态 / production) A2A故障 / a2a_<br/>failure<br/>G-CT-008 消费端 — Escalation.on_a2a_<br/>failure() 跨 agent 通信失败升级.<br/>文件: agent_spec/a2a_failure.py"]
    src_zephyr_governance_agent_spec_rbac_bridge_py["(生产态 / production) RBAC桥接 / rbac_<br/>bridge<br/>G-CT-007 契约：Budget -> RBAC 配额限制.<br/>文件: agent_spec/rbac_bridge.py"]
    src_zephyr_governance_agent_spec_registry_py["(生产态 / production) 注册表 / registry<br/>G-CT-003 契约：Agent Spec -> RBAC<br/>能力检查.<br/>文件: agent_spec/registry.py"]
    src_zephyr_governance_architecture_governance_architecture_contracts_py["(生产态 / production) 架构契约 /<br/>architecture_contracts<br/>架构契约，治理的状态机，管理状态流转。<br/>文件: architecture_governance<br/>/architecture_contracts.py"]
    src_zephyr_governance_architecture_governance_architecture_principles_py["(生产态 / production)<br/>装饰器：为函数标记适用的架构原则。 /<br/>architecture_principles<br/>装饰器：为函数标记适用的架构原则。<br/>文件: architecture_governance<br/>/architecture_principles.py"]
    src_zephyr_governance_architecture_governance_blueprint_bloat_monitor_py["(生产态 / production) 蓝图bloat监控器 /<br/>blueprint_bloat_monitor<br/>Blueprint Bloat Monitor — v0.11.0<br/>蓝图膨胀监控器。<br/>文件: architecture_governance/blueprint_<br/>bloat_monitor.py"]
    src_zephyr_governance_architecture_governance_blueprint_code_consistency_py["(生产态 / production) 蓝图代码一致性 /<br/>Blueprint-Code Consistency Gate —<br/>MOD-INF-022.<br/>蓝图代码一致性。Blueprint-Code<br/>Consistency Gate — MOD-INF-022.<br/>文件: architecture_governance/blueprint_<br/>code_consistency.py"]
    src_zephyr_governance_architecture_governance_blueprint_reconciler_py["(生产态 / production) 蓝图协调器 /<br/>blueprint_reconciler<br/>Blueprint Reconciler — v0.10.0<br/>蓝图实现一致性校验器。<br/>文件: architecture_governance/blueprint_<br/>reconciler.py"]
    src_zephyr_governance_architecture_governance_construction_verifier_py["(生产态 / production)<br/>construction验证器 / construction_<br/>verifier<br/>Construction Verifier — 施工验证器:<br/>任务卡完成度+蓝图一致性检查。<br/>文件: architecture_governance<br/>/construction_verifier.py"]
    src_zephyr_governance_architecture_governance_cross_env_consistency_py["(生产态 / production) 跨环境一致性 /<br/>cross_env_consistency<br/>跨环境一致性，提供包入口和模块加载功能<br/>文件: architecture_governance/cross_env_<br/>consistency.py"]
    src_zephyr_governance_architecture_governance_dependency_manager_py["(生产态 / production) 依赖管理器 /<br/>dependency_manager<br/>依赖管理器，治理子系统的依赖关系管理工具<br/>文件: architecture_governance<br/>/dependency_manager.py"]
    src_zephyr_governance_architecture_governance_formal_verifier_py["(生产态 / production) formal验证器 /<br/>formal_verifier<br/>Formal Verifier — v0.6.0 形式验证器:<br/>升级规则形式化验证->一致性+完备性检测。<br/>文件: architecture_governance/formal_<br/>verifier.py"]
    src_zephyr_governance_architecture_governance_gap_analyzer_py["(生产态 / production) gap分析器 / gap_<br/>analyzer<br/>Gap Analyzer — v0.8.0 间隙分析器:<br/>escalation覆盖缺口扫描+新操作类型识别。<br/>文件: architecture_governance/gap_<br/>analyzer.py"]
    src_zephyr_governance_architecture_governance_llm_impact_analyzer_py["(生产态 / production) LLM冲击分析器 /<br/>llm_impact_analyzer<br/>LLMImpactAnalyzer — LLM-based commit<br/>语义影响分析器。<br/>文件: architecture_governance/llm_<br/>impact_analyzer.py"]
    src_zephyr_governance_architecture_governance_local_first_arch_py["(生产态 / production) 本地首架构 /<br/>local_first_arch<br/>本地首架构，提供包入口和模块加载功能<br/>文件: architecture_governance/local_<br/>first_arch.py"]
    src_zephyr_governance_architecture_governance_path_resolver_py["(生产态 / production) 路径解析器 / path_<br/>resolver<br/>PathResolver — 模块路径解析器<br/>文件: architecture_governance/path_<br/>resolver.py"]
    ...
    src_zephyr_governance_bridges_spec_auditor_py["(生产态 / production) spec审计器 / spec_<br/>auditor<br/>G-CT-007 — Audit.record_agent_spec()<br/>记录 Agent Spec 注册与变更.<br/>文件: bridges/spec_auditor.py"]
    src_zephyr_governance_compliance_gate_a6_compliance_manager_py["(生产态 / production) 合规管理器 /<br/>compliance_manager<br/>ZephyrAlpha — D_COMPLIANCE Compliance<br/>Layer — 合规规则管理器接口<br/>文件: compliance_gate_a6/compliance_<br/>manager.py"]
    src_zephyr_governance_compliance_gate_a6_compliance_mapper_py["(生产态 / production) 合规mapper /<br/>compliance_mapper<br/>Compliance Mapper — D-022-13<br/>合规映射器: 操作->法规(SOX/GDPR<br/>/MiFID)映射+审计迹。<br/>文件: compliance_gate_a6/compliance_<br/>mapper.py"]
    src_zephyr_governance_context_governance_command_chain_length_gate_py["(生产态 / production) 命令链长度门禁 /<br/>command_chain_length_gate<br/>Command Chain Length Gate — v0.13.0<br/>命令体积Deny退化防御器。<br/>文件: context_governance/command_chain_<br/>length_gate.py"]
    src_zephyr_governance_context_governance_context_budget_py["(生产态 / production) 上下文预算 /<br/>context_budget<br/>— 上下文预算管理与超预算截断（Phase 11<br/>/ 盲点 B28）<br/>文件: context_governance/context_<br/>budget.py"]
    src_zephyr_governance_context_governance_context_manager_py["(生产态 / production) 上下文管理器 /<br/>context_manager<br/>上下文管理器，治理的管理器，统一管理资源<br/>生命周期。<br/>文件: context_governance/context_<br/>manager.py"]
    src_zephyr_governance_context_governance_context_package_py["(生产态 / production) 上下文包 /<br/>context_package<br/>Context Package — D-022-08<br/>委托上下文包: 升级原因+证据链+历史try_<br/>trace。<br/>文件: context_governance/context_<br/>package.py"]
    src_zephyr_governance_context_governance_context_recycling_py["(生产态 / production) 上下文recycling /<br/>context_recycling<br/>上下文recycling，主要提供is验证等功能<br/>文件: context_governance/context_<br/>recycling.py"]
    src_zephyr_governance_context_governance_context_switch_governor_py["(生产态 / production)<br/>上下文switchgovernor / context_switch_<br/>governor<br/>Context Switch Governor — v0.11.0<br/>Owner上下文切换预算管理器。<br/>文件: context_governance/context_switch_<br/>governor.py"]
    src_zephyr_governance_context_governance_context_waste_detector_py["(生产态 / production) 上下文waste检测器<br/>/ context_waste_detector<br/>上下文waste检测器，治理的报告器，汇总数<br/>据生成报告。<br/>文件: context_governance/context_waste_<br/>detector.py"]
    src_zephyr_governance_context_governance_conversation_tax_detector_py["(生产态 / production)<br/>conversationtax检测器 / conversation_<br/>tax_detector<br/>conversationtax检测器，提供包入口和模块<br/>加载功能<br/>文件: context_governance/conversation_<br/>tax_detector.py"]
    src_zephyr_governance_context_governance_instruction_bloat_detector_py["(生产态 / production)<br/>instructionbloat检测器 / instruction_<br/>bloat_detector<br/>InstructionBloatDetector — 指令膨胀检测<br/>文件: context_governance/instruction_<br/>bloat_detector.py"]
    src_zephyr_governance_context_governance_multi_turn_intent_analyzer_py["(生产态 / production)<br/>多turnintent分析器 / multi_turn_intent_<br/>analyzer<br/>Multi-Turn Intent Analyzer — v0.13.0<br/>多轮分布式意图分析器。<br/>文件: context_governance/multi_turn_<br/>intent_analyzer.py"]
    src_zephyr_governance_context_governance_prompt_lifecycle_py["(生产态 / production) 提示生命周期 /<br/>prompt_lifecycle<br/>提示生命周期，提供包入口和模块加载功能<br/>文件: context_governance/prompt_<br/>lifecycle.py"]
    src_zephyr_governance_context_governance_protocol_self_context_py["(生产态 / production) 协议自上下文 /<br/>protocol_self_context<br/>Protocol Self Context — v0.10.0<br/>协议自维护上下文管理器。<br/>文件: context_governance/protocol_self_<br/>context.py"]
    src_zephyr_governance_context_governance_think_time_model_py["(生产态 / production) thinktime模型 /<br/>think_time_model<br/>thinktime模型，提供包入口和模块加载功能<br/>文件: context_governance/think_time_<br/>model.py"]
    src_zephyr_governance_data_governance_data_classification_py["(生产态 / production) 数据分类 / data_<br/>classification<br/>检查 self_level 是否有权限访问 target_<br/>level 的数据。<br/>文件: data_governance/data_<br/>classification.py"]
    src_zephyr_governance_data_governance_data_lifecycle_py["(生产态 / production) 数据生命周期 /<br/>data_lifecycle<br/>数据生命周期，提供包入口和模块加载功能<br/>文件: data_governance/data_lifecycle.py"]
    src_zephyr_governance_data_governance_data_pipeline_guard_py["(生产态 / production) 数据管线守卫 /<br/>data_pipeline_guard<br/>Data Pipeline Guard — v0.10.0<br/>数据管道完整性防护: schema<br/>validation+row count check+checksum<br/>verify。<br/>文件: data_governance/data_pipeline_<br/>guard.py"]
    src_zephyr_governance_data_governance_data_quality_py["(生产态 / production) 数据质量 / data_<br/>quality<br/>数据质量，提供包入口和模块加载功能<br/>文件: data_governance/data_quality.py"]
    src_zephyr_governance_data_governance_data_source_reliability_py["(生产态 / production) 数据源可靠性 /<br/>data_source_reliability<br/>数据源可靠性，提供包入口和模块加载功能<br/>文件: data_governance/data_source_<br/>reliability.py"]
    src_zephyr_governance_data_governance_exchange_partition_detector_py["(生产态 / production)<br/>交易所partition检测器 / exchange_<br/>partition_detector<br/>Exchange Partition Detector — v0.12.0<br/>交易所网络分区检测器。<br/>文件: data_governance/exchange_<br/>partition_detector.py"]
    src_zephyr_governance_data_governance_exchange_reg_monitor_py["(生产态 / production) 交易所reg监控器 /<br/>exchange_reg_monitor<br/>Exchange Reg Monitor — v0.11.0<br/>交易所规则变更监控器。<br/>文件: data_governance/exchange_reg_<br/>monitor.py"]
    src_zephyr_governance_data_governance_miniqmt_provider_py["(生产态 / production) miniqmt提供器 /<br/>miniqmt_provider<br/>MiniQMT 实盘行情 Provider（Tick +<br/>5档盘口）<br/>文件: data_governance/miniqmt_<br/>provider.py"]
    src_zephyr_governance_data_governance_pricing_sync_py["(生产态 / production) pricing同步 /<br/>pricing_sync<br/>pricing同步，提供包入口和模块加载功能<br/>文件: data_governance/pricing_sync.py"]
    src_zephyr_governance_data_governance_realtime_streaming_py["(生产态 / production) 实时流式 /<br/>realtime_streaming<br/>实时流式，提供包入口和模块加载功能<br/>文件: data_governance/realtime_<br/>streaming.py"]
    src_zephyr_governance_evidence_pack_py["(生产态 / production) 证据包 / evidence_<br/>pack<br/>证据包，主要提供pack、验证、列表packs等<br/>功能，供audit-orchestrator.integrity;<br/>使用<br/>文件: governance/evidence_pack.py"]
    src_zephyr_governance_financial_governance_arbitrage_asymmetry_detector_py["(生产态 / production)<br/>arbitrageasymmetry检测器 / arbitrage_<br/>asymmetry_detector<br/>Arbitrage Asymmetry Detector — v0.11.0<br/>跨交易所套利不对称检测器。<br/>文件: financial_governance/arbitrage_<br/>asymmetry_detector.py"]
    src_zephyr_governance_financial_governance_atomic_transaction_manager_py["(生产态 / production) atomic交易管理器<br/>/ atomic_transaction_manager<br/>AtomicTransactionManager — SQLite +<br/>文件系统的跨介质原子事务管理器 v2.0<br/>（ATM）。<br/>文件: financial_governance/atomic_<br/>transaction_manager.py"]
    src_zephyr_governance_financial_governance_flash_crash_guard_py["(生产态 / production) flashcrash守卫 /<br/>flash_crash_guard<br/>Flash Crash Guard — v0.12.0<br/>闪崩双轨熔断器。<br/>文件: financial_governance/flash_crash_<br/>guard.py"]
    src_zephyr_governance_financial_governance_fsm_verifier_py["(生产态 / production) fsm验证器 / fsm_<br/>verifier<br/>fsm验证器，治理的状态机，管理状态流转。<br/>文件: financial_governance/fsm_<br/>verifier.py"]
    src_zephyr_governance_financial_governance_instrument_py["(生产态 / production) 标的合约 /<br/>instrument<br/>标的合约，供data ; factor ; pf_core ;<br/>ex_c使用<br/>文件: financial_governance/instrument.py"]
    src_zephyr_governance_financial_governance_microstructure_defense_py["(生产态 / production)<br/>microstructure防御 / microstructure_<br/>defense<br/>microstructure防御，治理的类型，定义数据<br/>类型和枚举。<br/>文件: financial_governance<br/>/microstructure_defense.py"]
    src_zephyr_governance_financial_governance_oms_risk_engine_py["(生产态 / production) oms风险引擎 / oms_<br/>risk_engine<br/>oms风险引擎，提供包入口和模块加载功能<br/>文件: financial_governance/oms_risk_<br/>engine.py"]
    src_zephyr_governance_financial_governance_risk_matrix_py["(生产态 / production) 风险矩阵 / risk_<br/>matrix<br/>风险矩阵，供MOD-INF-027;MOD-INF-020;MOD-<br/>IN使用<br/>文件: financial_governance/risk_<br/>matrix.py"]
    src_zephyr_governance_financial_governance_strategy_portfolio_py["(生产态 / production) 策略组合 /<br/>strategy_portfolio<br/>策略组合，提供包入口和模块加载功能<br/>文件: financial_governance/strategy_<br/>portfolio.py"]
    src_zephyr_governance_financial_governance_strategy_scoper_py["(生产态 / production) 策略scoper /<br/>strategy_scoper<br/>Strategy Scoper — v0.6.0<br/>策略范围隔离器: SIG/Strat<br/>/Capital多层策略隔离。<br/>文件: financial_governance/strategy_<br/>scoper.py"]
    src_zephyr_governance_implementations_default_experiment_pipeline_py["(生产态 / production) 默认实验管线 /<br/>default_experiment_pipeline<br/>实验 — Default Experiment Pipeline<br/>文件: implementations/default_<br/>experiment_pipeline.py"]
    src_zephyr_governance_implementations_default_security_gateway_py["(生产态 / production) 默认安全网关 /<br/>default_security_gateway<br/>默认安全网关，治理的门禁，在关键节点检查<br/>是否放行。<br/>文件: implementations/default_security_<br/>gateway.py"]
    src_zephyr_governance_intelligence_governance_agent_debate_py["(生产态 / production) 代理debate /<br/>agent_debate<br/>代理debate，治理的核心类，封装DebateVerd<br/>ict相关逻辑。<br/>文件: intelligence_governance/agent_<br/>debate.py"]
    src_zephyr_governance_intelligence_governance_ai_self_diagnosis_py["(生产态 / production) AI自诊断 / ai_<br/>self_diagnosis<br/>AI自诊断，提供包入口和模块加载功能<br/>文件: intelligence_governance/ai_self_<br/>diagnosis.py"]
    src_zephyr_governance_intelligence_governance_aisg_sandbox_py["(生产态 / production) aisg沙箱 / aisg_<br/>sandbox<br/>AISG Sandbox Testing — AI Security<br/>Gateway 沙箱验证 (INV-015 升级)<br/>文件: intelligence_governance/aisg_<br/>sandbox.py"]
    src_zephyr_governance_intelligence_governance_autonomy_dashboard_py["(生产态 / production) autonomy仪表盘 /<br/>autonomy_dashboard<br/>Autonomy Dashboard — AI<br/>自主感知健康仪表。<br/>文件: intelligence_governance/autonomy_<br/>dashboard.py"]
    src_zephyr_governance_intelligence_governance_confidence_estimator_py["(生产态 / production) confidence估算器<br/>/ confidence_estimator<br/>Confidence Estimator — D-022-05<br/>置信度评估器:<br/>certainty×evidence×risk三维评估。<br/>文件: intelligence_governance<br/>/confidence_estimator.py"]
    src_zephyr_governance_intelligence_governance_confidence_quantifier_py["(生产态 / production)<br/>ConfidenceQuantifier — AI 置信度量化。<br/>/ confidence_quantifier<br/>ConfidenceQuantifier — AI 置信度量化。<br/>文件: intelligence_governance<br/>/confidence_quantifier.py"]
    src_zephyr_governance_intelligence_governance_continuous_trust_py["(生产态 / production) continuous信任 /<br/>continuous_trust<br/>Continuous Trust Ledger —<br/>持续信任评估引擎。<br/>文件: intelligence_governance<br/>/continuous_trust.py"]
    src_zephyr_governance_intelligence_governance_cross_agent_conflict_detector_py["(生产态 / production) 跨代理冲突检测器<br/>/ cross_agent_conflict_detector<br/>CrossAgentConflictDetector — 多 Agent<br/>并发冲突检测。<br/>文件: intelligence_governance/cross_<br/>agent_conflict_detector.py"]
    src_zephyr_governance_intelligence_governance_cross_assistant_adapter_py["(生产态 / production) 跨assistant适配器<br/>/ cross_assistant_adapter<br/>Cross-Assistant Adapter — v0.6.0 Trae<br/>/Cursor/Windsurf/Codex<br/>/Wedata统一升级接口。<br/>文件: intelligence_governance/cross_<br/>assistant_adapter.py"]
    src_zephyr_governance_intelligence_governance_delegation_manager_py["(生产态 / production) delegation管理器<br/>/ delegation_manager<br/>Delegation Manager — D-022-02<br/>自动委托协议。<br/>文件: intelligence_governance<br/>/delegation_manager.py"]
    src_zephyr_governance_intelligence_governance_memory_provider_py["(生产态 / production) 记忆提供器 / D_<br/>DATA — Memory Provider<br/>记忆提供器。D_DATA — Memory Provider<br/>文件: intelligence_governance/memory_<br/>provider.py"]
    src_zephyr_governance_intelligence_governance_meta_confidence_py["(生产态 / production) 元confidence /<br/>meta_confidence<br/>Meta-Confidence — D-022-10<br/>Agent对自身判定置信度的自评+历史校准。<br/>文件: intelligence_governance/meta_<br/>confidence.py"]
    src_zephyr_governance_intelligence_governance_model_provider_data_py["(生产态 / production) 模型提供器数据 /<br/>model_provider_data<br/>模型提供器数据，治理的模型，定义数据结构<br/>和字段。<br/>文件: intelligence_governance/model_<br/>provider_data.py"]
    src_zephyr_governance_intelligence_governance_model_router_py["(生产态 / production) 模型路由器 /<br/>model_router<br/>模型路由器，依赖预算模型、提供器数据、re<br/>sultswriter工作<br/>文件: intelligence_governance/model_<br/>router.py"]
    src_zephyr_governance_intelligence_governance_model_version_detector_py["(生产态 / production) 模型版本检测器 /<br/>model_version_detector<br/>Model Version Detector — v0.10.0<br/>模型版本突变检测: model version<br/>change->degraded auto_guard。<br/>文件: intelligence_governance/model_<br/>version_detector.py"]
    src_zephyr_governance_intelligence_governance_multi_model_consensus_py["(生产态 / production) 多模型共识 /<br/>multi_model_consensus<br/>多模型共识，提供包入口和模块加载功能<br/>文件: intelligence_governance/multi_<br/>model_consensus.py"]
    src_zephyr_governance_intelligence_governance_mvep_orchestrator_py["(生产态 / production) mvep编排器 / mvep_<br/>orchestrator<br/>MVEP Orchestrator — v0.11.0 Minimum<br/>Viable Escalation Protocol调度器。<br/>文件: intelligence_governance/mvep_<br/>orchestrator.py"]
    src_zephyr_governance_intelligence_governance_provider_failover_py["(生产态 / production) 提供器故障切换 /<br/>provider_failover<br/>Provider Failover — v0.7.0 多LLM<br/>Provider容灾: deepseek->claude->gpt<br/>fallback链。<br/>文件: intelligence_governance/provider_<br/>failover.py"]
    src_zephyr_governance_intelligence_governance_self_benchmark_py["(生产态 / production) 自基准 / self_<br/>benchmark<br/>自基准 (W3-7) — 5 组已知对自验证 +<br/>引擎退化告警.<br/>文件: intelligence_governance/self_<br/>benchmark.py"]
    src_zephyr_governance_intelligence_governance_self_test_py["(生产态 / production) 自测试 /<br/>Escalation Protocol Self-Test —<br/>MOD-INF-022.<br/>自测试。Escalation Protocol Self-Test —<br/>MOD-INF-022.<br/>文件: intelligence_governance/self_<br/>test.py"]
    src_zephyr_governance_intelligence_governance_self_validator_py["(生产态 / production) 自校验器 / self_<br/>validator<br/>Self Validator — v0.10.0<br/>升级协议自验证器:<br/>protocol自身规则+代码一致性自检。<br/>文件: intelligence_governance/self_<br/>validator.py"]
    src_zephyr_governance_intelligence_governance_subagent_hook_propagator_py["(生产态 / production)<br/>subagent钩子propagator / subagent_hook_<br/>propagator<br/>Subagent Hook Propagator — v0.13.0<br/>子Agent Hook旁路防护器。<br/>文件: intelligence_governance/subagent_<br/>hook_propagator.py"]
    src_zephyr_governance_lifecycle_governance_api_lifecycle_py["(生产态 / production) API生命周期 / api_<br/>lifecycle<br/>API生命周期，治理的状态机，管理状态流转<br/>。<br/>文件: lifecycle_governance/api_<br/>lifecycle.py"]
    src_zephyr_governance_lifecycle_governance_migration_strategy_py["(生产态 / production) 迁移策略 /<br/>migration_strategy<br/>迁移策略，提供包入口和模块加载功能<br/>文件: lifecycle_governance/migration_<br/>strategy.py"]
    src_zephyr_governance_lifecycle_governance_paper_live_transition_py["(生产态 / production) paper实盘转换 /<br/>paper_live_transition<br/>检查是否可跳Phase——不可跳,<br/>只允许顺序next。<br/>文件: lifecycle_governance/paper_live_<br/>transition.py"]
    src_zephyr_governance_lifecycle_governance_post_live_verification_py["(生产态 / production) 提交实时验证 /<br/>post_live_verification<br/>提交实时验证，治理的检查器，检查某项条件<br/>是否满足。<br/>文件: lifecycle_governance/post_live_<br/>verification.py"]
    src_zephyr_governance_lifecycle_governance_transition_py["(生产态 / production) 转换 / transition<br/>transition — 状态机转换 Mixin（从 task_<br/>repo.py 拆分，SRC-0066）<br/>文件: lifecycle_governance/transition.py"]
    src_zephyr_governance_observability_governance_analytics_base_py["(生产态 / production) analytics基类 /<br/>Re-export wrapper: analytics_base<br/>canonical at zephyr.report<br/>analytics基类。Re-export wrapper:<br/>analytics_base canonical at<br/>zephyr.reporting.analytics_base.<br/>文件: observability_governance<br/>/analytics_base.py"]
    src_zephyr_governance_observability_governance_objective_tracker_py["(生产态 / production) objective追踪器 /<br/>objective_tracker<br/>Objective Tracker — v0.9.0<br/>目标漂移检测器:<br/>agent目标函数稳定性+变更检测+rollback。<br/>文件: observability_governance<br/>/objective_tracker.py"]
    src_zephyr_governance_persistence_database_manager_py["(生产态 / production) 数据库管理器 /<br/>database_manager<br/>DatabaseManager — 连接池 + 健康检查 +<br/>自动备份 + WAL checkpoint（SH-DB-001<br/>v2.0）<br/>文件: persistence/database_manager.py"]
    src_zephyr_governance_persistence_database_service_py["(生产态 / production) 数据库服务 /<br/>database_service<br/>DatabaseService 真源收敛（AI-14 审计 P1<br/>修复）<br/>文件: persistence/database_service.py"]
    src_zephyr_governance_persistence_dataflowgraph_schema_py["(生产态 / production) dataflowgraph结构<br/>/ dataflowgraph_schema<br/>dataflowgraph Schema DDL + 连接入口<br/>文件: persistence/dataflowgraph_<br/>schema.py"]
    src_zephyr_governance_persistence_decision_graph_reader_py["(生产态 / production) 决策graph读取器 /<br/>decision_graph_reader<br/>决策流图数据库只读查询工具模块<br/>文件: persistence/decision_graph_<br/>reader.py"]
    src_zephyr_governance_persistence_depgraph_reader_py["(生产态 / production) depgraph读取器 /<br/>depgraph_reader<br/>依赖图数据库查询工具模块<br/>文件: persistence/depgraph_reader.py"]
    src_zephyr_governance_persistence_protocol_state_store_py["(生产态 / production) 协议状态存储 /<br/>protocol_state_store<br/>Protocol State Store — v0.10.0<br/>协议运行时状态持久化: JSON<br/>snapshot+recovery state+crash恢复。<br/>文件: persistence/protocol_state_<br/>store.py"]
    src_zephyr_governance_services_adapter_py["(生产态 / production) 适配器 / adapter<br/>Escalation Adapter — MOD-INF-022<br/>统一集成入口.<br/>文件: services/adapter.py"]
    src_zephyr_governance_services_cross_session_correlator_py["(生产态 / production) 跨会话关联器 /<br/>cross_session_correlator<br/>Cross-Session Correlator — v0.9.0<br/>跨会话Coreset关联器:<br/>多session行为模式+异常跨session模式检测<br/>。<br/>文件: services/cross_session_<br/>correlator.py"]
    src_zephyr_governance_services_memory_provenance_py["(生产态 / production) 记忆溯源 / memory_<br/>provenance<br/>Memory Provenance — v0.9.0<br/>记忆溯源追踪: 每条memory<br/>record的来源agent+timestamp+hash链。<br/>文件: services/memory_provenance.py"]
    src_zephyr_governance_strategies_strategy_registry_py["(生产态 / production) 策略注册表 /<br/>strategy_registry<br/>StrategyRegistry 卫星模块（OCP-002）<br/>文件: strategies/strategy_registry.py"]
    src_zephyr_infrastructure_a2a_protocol_governance_base_server_py["(生产态 / production) 基类服务端 / _<br/>base_server<br/>基类服务端，主要提供注册tool、处理请求等<br/>功能<br/>文件: governance/_base_server.py"]
    src_zephyr_infrastructure_a2a_protocol_governance_audit_logger_py["(生产态 / production) 审计日志器 /<br/>audit_logger<br/>审计日志器，主要提供日志、查询、数量等功<br/>能<br/>文件: governance/audit_logger.py"]
    src_zephyr_infrastructure_a2a_protocol_governance_auditor_py["(生产态 / production) 审计器 / auditor<br/>G-CT-008 契约：A2A -> Audit 审计 Agent<br/>间通信.<br/>文件: governance/auditor.py"]
    src_zephyr_infrastructure_a2a_protocol_governance_error_codes_py["(生产态 / production) 错误codes / error_<br/>codes<br/>错误codes，治理的异常，定义本模块的异常<br/>类型。<br/>文件: governance/error_codes.py"]
    src_zephyr_infrastructure_a2a_protocol_governance_governance_adapter_py["(生产态 / production) 治理适配器 /<br/>governance_adapter<br/>A2A GovernanceAdapter — Phase 4<br/>治理集成桥接器<br/>文件: governance/governance_adapter.py"]
    src_zephyr_infrastructure_a2a_protocol_governance_phase_hold_py["(生产态 / production) 阶段hold / phase_<br/>hold<br/>Phase 4 Hold — A2A Phase 4 锁定标记模块<br/>与其他 Phase 3 模块不可并发施工.<br/>文件: governance/phase_hold.py"]
    src_zephyr_infrastructure_a2a_protocol_governance_policy_engine_py["(生产态 / production) 策略引擎 / policy_<br/>engine<br/>策略引擎，主要提供评估、新增策略、移除策<br/>略等功能<br/>文件: governance/policy_engine.py"]
    ...
    src_zephyr_infrastructure_a2a_protocol_governance_rate_limiter_py["(生产态 / production) 速率限制器 / rate_<br/>limiter<br/>Sliding window 速率限制器，支持 per-key<br/>分桶。<br/>文件: governance/rate_limiter.py"]
    src_zephyr_infrastructure_a2a_protocol_governance_session_manager_py["(生产态 / production) 会话管理器 /<br/>session_manager<br/>会话管理器，主要提供创建会话、获取会话、<br/>结束会话等功能<br/>文件: governance/session_manager.py"]
    src_zephyr_infrastructure_a2a_protocol_layer3_coordination_governance_integration_py["(生产态 / production) 治理集成 /<br/>Re-export bridge for layer3_<br/>coordination governance integrat<br/>治理集成。Re-export bridge for layer3_<br/>coordination governance integration<br/>symbols.<br/>文件: layer3_coordination/_governance_<br/>integration.py"]
    src_zephyr_infrastructure_capacity_assurance_contracts_batch2_governance_py["(生产态 / production) batch2治理 /<br/>batch2_governance<br/>Batch2 治理层契约 — 15条 Pydantic v2<br/>Schema（Provenance/AI审计守卫<br/>/TechStackValidator/Governance Loop<br/>/Sandbox资源限制）.<br/>文件: contracts/batch2_governance.py"]
    src_zephyr_integration_mcp_governance_server_py["(生产态 / production) 治理服务端 /<br/>governance_server<br/>GovernanceServer: 治理域统一MCP入口<br/>文件: mcp/governance_server.py"]
    src_zephyr_shared_capacity_governance_capacity_governance_loop_py["(生产态 / production) 容量治理循环 /<br/>capacity_governance_loop<br/>容量治理loop，容量治理的循环，循环执行的<br/>流程。<br/>文件: capacity_governance/capacity_<br/>governance_loop.py"]
    src_zephyr_shared_protocols_a2a_a2a_governance_py["(生产态 / production) A2A治理 / A2A<br/>Governance — shared interface<br/>definitions for governance<br/>A2A治理。A2A Governance — shared<br/>interface definitions for governance<br/>layer.<br/>文件: a2a/a2a_governance.py"]
    tests_agent_rbac_test_session_aware_stash_red_blue_py["(生产态 / production)<br/>测试会话感知stashredblue / test_session_<br/>aware_stash_red_blue<br/>会话 隔离 stash 红蓝对抗极限测试。<br/>文件: agent_rbac/test_session_aware_<br/>stash_red_blue.py"]
    tests_git_test_git_commit_concurrent_py["(生产态 / production) 测试Git提交并发 /<br/>test_git_commit_concurrent<br/>幽灵提交红蓝对抗测试<br/>文件: git/test_git_commit_concurrent.py"]
    tests_git_test_git_commit_extreme_py["(生产态 / production)<br/>测试Gitcommitextreme / test_git_commit_<br/>extreme<br/>GitCommitGateway 极端故障注入测试<br/>文件: git/test_git_commit_extreme.py"]
    tests_git_test_git_commit_gateway_py["(生产态 / production) 测试Git提交网关 /<br/>test_git_commit_gateway<br/>GitCommitGateway 单元测试<br/>（OPS-2026062512 验收）<br/>文件: git/test_git_commit_gateway.py"]
    tests_git_test_reconciler_verify_autosync_py["(生产态 / production)<br/>测试对账器verifyautosync / test_<br/>reconciler_verify_autosync<br/>--reconciler-verify auto-sync<br/>产物豁免测试。<br/>文件: git/test_reconciler_verify_<br/>autosync.py"]
    tests_governance_generators_test_check_gate_inventory_drift_py["(生产态 / production)<br/>测试check门禁inventory漂移 / test_check_<br/>gate_inventory_drift<br/>commit_gates<br/>模块清单漂移检测脚本单元测试<br/>文件: generators/test_check_gate_<br/>inventory_drift.py"]
    tests_governance_generators_test_generate_gate_registry_py["(生产态 / production)<br/>测试生成门禁注册表 / test_generate_gate_<br/>registry<br/>测试生成门禁注册表.py 单元测试<br/>（CommitGate 同步治本 2026-07-17）<br/>文件: generators/test_generate_gate_<br/>registry.py"]
    tests_governance_rule_bridge_test_worktree_lifecycle_py["(生产态 / production)<br/>测试worktree生命周期 / test_worktree_<br/>lifecycle<br/>#ARCH-WORKTREE-LIFECYCLE-001 状态机测试<br/>文件: rule_bridge/test_worktree_<br/>lifecycle.py"]
    tests_governance_test_ast_import_rewriter_py["(生产态 / production)<br/>测试astimportrewriter / Tests for<br/>scripts/governance/ast_import_<br/>rewriter.py.<br/>测试astimportrewriter，提供testexactmatc<br/>h、testnomatch、testprefixmatch等方法<br/>文件: governance/test_ast_import_<br/>rewriter.py"]
    tests_io_test_depgraph_schema_py["(生产态 / production) 测试依赖图模式 /<br/>test_depgraph_schema<br/>测试依赖图模式.py DDL<br/>真源与迁移框架单元测试<br/>文件: io/test_depgraph_schema.py"]
    tests_io_test_verify_schema_health_py["(生产态 / production) 测试校验模式健康<br/>/ test_verify_schema_health<br/>测试校验模式健康.py 门禁可靠性单元测试<br/>文件: io/test_verify_schema_health.py"]
    tests_rollback_test_concurrency_guard_red_blue_py["(生产态 / production)<br/>测试并发守卫redblue / test_concurrency_<br/>guard_red_blue<br/>红蓝对抗极端测试 — git_guard +<br/>concurrency_guard 端到端防护能力验证。<br/>文件: rollback/test_concurrency_guard_<br/>red_blue.py"]
    tests_rollback_test_concurrent_mv_guard_py["(生产态 / production)<br/>并发红蓝极限对抗测试 — 多 AI 并发执行<br/>git mv 时的防护能力验证。 / test_<br/>concurrent_mv_guard<br/>并发红蓝极限对抗测试 — 多 AI 并发执行<br/>git mv 时的防护能力验证。<br/>文件: rollback/test_concurrent_mv_<br/>guard.py"]
    tests_task_test_task_repo_gateway_e2e_py["(生产态 / production)<br/>测试taskrepogatewaye2e / test_task_repo_<br/>gateway_e2e<br/>端到端链路测试<br/>文件: task/test_task_repo_gateway_e2e.py"]
    tests_test_align_panoramas_py["(生产态 / production)<br/>测试alignpanoramas / test_align_<br/>panoramas<br/>测试alignpanoramas.py 单元测试<br/>文件: tests/test_align_panoramas.py"]
    tests_test_dataflow_design_layout_py["(生产态 / production)<br/>测试dataflowdesignlayout / test_<br/>dataflow_design_layout<br/>设计态数据流文档视觉风格测试<br/>文件: tests/test_dataflow_design_<br/>layout.py"]
    tests_test_generate_dataflow_diagram_py["(生产态 / production)<br/>测试generatedataflowdiagram / test_<br/>generate_dataflow_diagram<br/>测试generatedataflowdiagram.py 单元测试<br/>文件: tests/test_generate_dataflow_<br/>diagram.py"]
    tests_test_generate_decision_diagram_py["(生产态 / production)<br/>测试generate决策diagram / test_generate_<br/>decision_diagram<br/>测试generate决策diagram.py 单元测试<br/>文件: tests/test_generate_decision_<br/>diagram.py"]
    ...
    scripts_arch_guard_arch_ssot_py["(生产态 / production) 架构ssot / _arch_<br/>ssot<br/>arch_guard 共享：仓库根路径、capacity_<br/>slo / invariants / contracts 装载。<br/>文件: arch_guard/_arch_ssot.py"]
    scripts_check_naming_convention_py["(生产态 / production)<br/>检查namingconvention / check_naming_<br/>convention<br/>检查namingconvention，scripts的检查器，<br/>检查某项条件是否满足。<br/>文件: scripts/check_naming_convention.py"]
    scripts_construction_check_transition_code_py["(生产态 / production) 检查转换代码 /<br/>check_transition_code<br/>检查转换代码，construction的检查器，检查<br/>某项条件是否满足。<br/>文件: construction/check_transition_<br/>code.py"]
    scripts_git_commit_py["(生产态 / production) Git提交 / git_<br/>commit<br/>GitCommitGateway CLI 封装<br/>文件: scripts/git_commit.py"]
    scripts_git_guard_py["(生产态 / production) Git守卫 / git_<br/>guard<br/>Git Guard — 拦截危险 git<br/>命令，防止破坏其他 session 的文件锁。<br/>文件: scripts/git_guard.py"]
    scripts_mcp_generate_ide_config_py["(生产态 / production) 生成ide配置 /<br/>generate_ide_config<br/>从 config/mcp.json 生成各 IDE MCP<br/>配置文件（MOD-INF-013 §5.3 Step 2）。<br/>文件: mcp/generate_ide_config.py"]
    scripts_migration_dm314_infra_ops_split_py["(生产态 / production)<br/>dm314基础设施运维拆分 / dm314_infra_ops_<br/>split<br/>DM-314: infra_ops/ 拆分迁移执行脚本。<br/>文件: migration/dm314_infra_ops_split.py"]
    src_zephyr_gov_enforcement_rule_bridge_worktree_lifecycle_py["(生产态 / production) worktree生命周期<br/>/ worktree_lifecycle<br/>WorktreeLifecycle — worktree<br/>生命周期状态机（5态 + 8转换）<br/>文件: rule_bridge/worktree_lifecycle.py"]
    src_zephyr_governance_capability_lookup_py["(生产态 / production) 能力lookup /<br/>capability_lookup<br/>CapabilityLookup —<br/>能力->真源文件反查注册表的查询 API +<br/>扫描/派生逻辑（合一）<br/>文件: governance/capability_lookup.py"]
    src_zephyr_governance_data_governance_akshare_provider_py["(生产态 / production) akshare提供器 / D_<br/>DATA — Akshare Data Provider<br/>akshare提供器。D_DATA — Akshare Data<br/>Provider<br/>文件: data_governance/akshare_<br/>provider.py"]
    src_zephyr_governance_engine_pipeline_base_py["(生产态 / production) 管线基类 /<br/>pipeline_base<br/>实验 — Experimentation Pipeline Layer<br/>文件: engine/pipeline_base.py"]
    src_zephyr_governance_intelligence_governance_delegation_engine_py["(生产态 / production) delegation引擎 /<br/>Delegation Engine — MOD-INF-022<br/>delegation引擎。Delegation Engine —<br/>MOD-INF-022<br/>文件: intelligence_governance<br/>/delegation_engine.py"]
    src_zephyr_governance_observability_governance_query_metrics_py["(生产态 / production) 查询指标 / query_<br/>metrics<br/>QueryMetrics — SQL 查询性能监控装饰器<br/>（SH-DB-001 v2.0）<br/>文件: observability_governance/query_<br/>metrics.py"]
    src_zephyr_governance_persistence_base_repo_py["(生产态 / production) 基类repo / base_<br/>repo<br/>base_repo —<br/>异常类、状态机常量、工具函数（从 task_<br/>repo.py 拆分，SRC-0066）<br/>文件: persistence/base_repo.py"]
    src_zephyr_governance_persistence_decisiongraph_schema_py["(生产态 / production) decisiongraph结构<br/>/ decisiongraph_schema<br/>decisiongraph Schema DDL + 不变量声明<br/>文件: persistence/decisiongraph_<br/>schema.py"]
    src_zephyr_governance_persistence_pg_wrapper_py["(生产态 / production) pg包装 / pg_<br/>wrapper<br/>psycopg2 connection 的 sqlite3 兼容<br/>execute() 包装器（单一规范副本）。<br/>文件: persistence/pg_wrapper.py"]
    src_zephyr_governance_rule_patterns_py["(生产态 / production) 规则模式 / rule_<br/>patterns<br/>治理规则正则 + 安全审计模式唯一真源<br/>(SSoT)<br/>文件: governance/rule_patterns.py"]
    src_zephyr_governance_strategies_strategy_base_py["(生产态 / production) 策略基类 / D_<br/>PORTFOLIO_CORE — StrategyBase +<br/>StrategyMeta + StrategyReg<br/>策略基类。D_PORTFOLIO_CORE —<br/>StrategyBase + StrategyMeta +<br/>StrategyRegistry<br/>文件: strategies/strategy_base.py"]
    src_zephyr_infrastructure_a2a_protocol_layer3_coordination_a2a_governance_adapter_py["(生产态 / production) A2A治理适配器 /<br/>a2a_governance_adapter<br/>A2A 治理适配器 — 连接 A2A 协议与<br/>Governance 层<br/>文件: layer3_coordination/a2a_<br/>governance_adapter.py"]
    src_zephyr_infrastructure_registry_governance_py["(生产态 / production) 注册表治理 /<br/>Registry Governance — MOD-INF-037<br/>注册表治理。Registry Governance —<br/>MOD-INF-037<br/>文件: infrastructure/registry_<br/>governance.py"]
    ...
    src_zephyr_governance_depgraph_schema_py["(生产态 / production) 依赖图模式 /<br/>depgraph_schema<br/>depgraph Schema DDL + 版本化迁移框架<br/>文件: governance/depgraph_schema.py"]
    src_zephyr_governance_intelligence_governance_provider_base_py["(生产态 / production) 提供器基类 / D_<br/>DATA — Data Source Layer<br/>提供器基类。D_DATA — Data Source Layer<br/>文件: intelligence_governance/provider_<br/>base.py"]
    src_zephyr_governance_persistence_task_repo_py["(生产态 / production) 任务repo / task_<br/>repo<br/>TaskRepository — 任务登记表 CRUD +<br/>状态机（T-1-04）<br/>文件: persistence/task_repo.py"]
    ...
    src_zephyr_governance_architecture_governance_post_sync_validator_py["(生产态 / production) 提交同步校验器 /<br/>post_sync_validator<br/>post_sync_validator — post_sync_<br/>standard 命令校验逻辑的唯一真源<br/>（SSoT）。<br/>文件: architecture_governance/post_sync_<br/>validator.py"]
    src_zephyr_governance_observability_governance_projection_engine_py["(生产态 / production) projection引擎 /<br/>projection_engine<br/>ProjectionEngine — 事件折叠为当前状态<br/>（DW-0003）<br/>文件: observability_governance<br/>/projection_engine.py"]
    src_zephyr_governance_persistence_sqlite_schema_py["(生产态 / production) sqlite结构 /<br/>sqlite_schema<br/>SQLite 元数据层 Schema DDL +<br/>版本化迁移框架（T-1-02 + SH-DB-001<br/>v2.0）<br/>文件: persistence/sqlite_schema.py"]
    ...
    D_INFRA_A2A["(生产态 / production) A2A通信 / A2A<br/>Communication<br/>Agent 与 Agent 之间的通信协议层，负责<br/>AI 代理间的消息传递、请求路由和协议适配<br/>跨域节点 / cross-domain"]
    src_zephyr_infrastructure_a2a_protocol_layer3_coordination_governance_integration_py -->|导入依赖 / import_depends| D_INFRA_A2A
    D_RISK["(生产态 / production) 风控 / Risk<br/>Control<br/>风控，负责风险指标计算、风险限额管理和风<br/>险预警<br/>跨域节点 / cross-domain"]
    ...
    class D_SECURITY,D_SHARED,D_INFRA_A2A,D_RISK,D_GOV_SCRIPTS,D_GOV_RULE,D_TRADING,D_GOV_OPS_RESILIENCE,D_GOV_AUDIT,D_GOV_ENFORCEMENT,D_FEEDBACK_LOOP external_prod
