...
flowchart TD
    ...
    tests_governance_integration_test_contract_template_manager_unit_py -->|测试依赖 / test_depends| D_GOV_RULE
    ...
    tests_governance_shared_test_triage_unit_py -->|测试依赖 / test_depends| D_GOV_OPS_RESILIENCE
    ...
    class D_SHARED,D_GOV_CODE_QUALITY,D_GOV_RULE,D_GOV_OPS_RESILIENCE,D_INFRASTRUCTURE,D_TRADING,D_GOV_ENFORCEMENT,D_INFRA_RUNTIME,D_GOV_SCRIPTS,D_EX_CORE,D_PF_CORE external_prod
```
...
> 本域与 34 个外部域直接连接（出边 495 条 + 入边 172 条 = 667 条）。只显示直接连接的域，不展开具体节点。
