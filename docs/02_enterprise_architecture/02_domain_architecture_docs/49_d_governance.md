...
flowchart TD
    ...
    scripts_arch_guard_check_acl_boundary_py["检查aclboundary<br/>Broker ACL 边界强制执行 (INV-005)<br/>check_acl_boundary<br/>文件: arch_guard/check_acl_boundary.py<br/>(生产态 / production)"]
    ...
    scripts_arch_guard_check_scaffold_exit_gates_py["checkscaffold退出门禁<br/>对标 architecture_model/cross_cutting<br/>/invariants.yaml 安全不变量<br/>check_scaffold_exit_gates<br/>文件: arch_guard/check_scaffold_exit_gates.py<br/>(生产态 / production)"]
    ...
    scripts_arch_guard_fitness_functions_check_position_limit_py["检查持仓限制<br/>单一持仓限制 ≤ 5% NAV (INV-002)<br/>check_position_limit<br/>文件: fitness_functions/check_position_limit.py<br/>(生产态 / production)"]
    ...
    scripts_arch_guard_fitness_functions_check_survivorship_bias_py["检查survivorshipbias<br/>- 读取 config/data/survivorship_policy.yaml<br/>（真源，经 _arch_ssot.SURVIVORSHIP_POLICY_PATH<br/>引用）<br/>check_survivorship_bias<br/>文件: fitness_functions<br/>/check_survivorship_bias.py<br/>(生产态 / production)"]
    ...
    scripts_arch_guard_run_all_py["运行all<br/>对标 Google Tricorder Presubmit——在 CI Pipeline<br/>中调用<br/>run_all<br/>文件: arch_guard/run_all.py<br/>(生产态 / production)"]
    ...
    scripts_construction_demo_a2a_chat_py["A2A 多 Agent 聊天演示 - Alpha 和 Beta<br/>讨论项目评估<br/>共享消息路由器<br/>demo_a2a_chat<br/>文件: construction/demo_a2a_chat.py<br/>(生产态 / production)"]
    ...
    scripts_generate_manifest_py["generate清单<br/>双 manifest 体系说明（P1-T4 校正，2026-06-26）<br/>文件: scripts/generate_manifest.py<br/>(生产态 / production)"]
    ...
    scripts_rollback_py["回滚<br/>Rollback System CLI — MOD-INF-021 v0.10.0<br/>Git-native+SQLite Checkpoint 操作入口。用法:<br/>文件: scripts/rollback.py<br/>(生产态 / production)"]
    ...
    scripts_setup_git_guard_aliases_py["setupGit守卫aliases<br/>将危险 git 命令（reset/checkout/stash/revert<br/>/restore）的 alias 设置为通过 git_guard.py 执行<br/>setup_git_guard_aliases<br/>文件: scripts/setup_git_guard_aliases.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_adapters_risk_validation_bridge_py["风险验证桥接<br/>治理的风险验证桥接。D_EXECUTION_CORE — Risk<br/>Validation Bridge<br/>(DW-239)，风险验证桥接。D_EXECUTION_CORE — Risk<br/>Validation Bridge (DW-239)相关功能的实现。<br/>文件: adapters/risk_validation_bridge.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_architecture_governance_blueprint_bloat_monitor_py["蓝图bloat监控器<br/>治理的Blueprint Bloat Monitor — v0.11.0<br/>蓝图膨胀监控器。，Blueprint Bloat Monitor —<br/>v0.11.0 蓝图膨胀监控器。相关功能的实现。<br/>blueprint_bloat_monitor<br/>文件: architecture_governance<br/>/blueprint_bloat_monitor.py<br/>(生产态 / production)"]
    src_zephyr_governance_architecture_governance_blueprint_code_consistency_py["蓝图代码一致性<br/>公共接口：file_exists（Stage 4 公共化）<br/>文件: architecture_governance<br/>/blueprint_code_consistency.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_architecture_governance_llm_impact_analyzer_py["LLM冲击分析器<br/>commit diff -> LLM API -> 语义级风险评估:<br/>llm_impact_analyzer<br/>文件: architecture_governance<br/>/llm_impact_analyzer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_financial_governance_flash_crash_guard_py["flashcrash守卫<br/>治理的Flash Crash Guard — v0.12.0<br/>闪崩双轨熔断器。，Flash Crash Guard — v0.12.0<br/>闪崩双轨熔断器。相关功能的实现。<br/>flash_crash_guard<br/>文件: financial_governance/flash_crash_guard.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_financial_governance_instrument_py["financial_governance/instrument<br/>治理的标的合约，供data ; factor ; pf_core ;<br/>ex_c使用，标的合约，供data ; factor ; pf_core ;<br/>ex_c使用相关功能的实现。<br/>文件: financial_governance/instrument.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_financial_governance_risk_matrix_py["风险矩阵<br/>治理的风险矩阵，供MOD-INF-027;MOD-INF-020;MOD-IN<br/>使用，风险矩阵，供MOD-INF-027;MOD-INF-020;MOD-IN<br/>使用相关功能的实现。<br/>risk_matrix<br/>文件: financial_governance/risk_matrix.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_intelligence_governance_cross_assistant_adapter_py["跨assistant适配器<br/>治理的Cross-Assistant Adapter — v0.6.0 Trae<br/>/Cursor/Windsurf/Codex<br/>/Wedata统一升级接口。，Cross-Assistant Adapter<br/>— v0.6.0 Trae/Cursor/Windsurf/Codex<br/>/Wedata统一升级接口。相关功能的实现。<br/>cross_assistant_adapter<br/>文件: intelligence_governance<br/>/cross_assistant_adapter.py<br/>(生产态 / production)"]
    src_zephyr_governance_intelligence_governance_delegation_manager_py["delegation管理器<br/>delegate(task, capability)->Agent +<br/>四级安全约束：<br/>delegation_manager<br/>文件: intelligence_governance<br/>/delegation_manager.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_intelligence_governance_mvep_orchestrator_py["mvep编排器<br/>治理的MVEP Orchestrator — v0.11.0 Minimum<br/>Viable Escalation Protocol调度器。，MVEP<br/>Orchestrator — v0.11.0 Minimum Viable<br/>Escalation Protocol调度器。相关功能的实现。<br/>mvep_orchestrator<br/>文件: intelligence_governance<br/>/mvep_orchestrator.py<br/>(生产态 / production)"]
    src_zephyr_governance_intelligence_governance_provider_failover_py["提供器故障切换<br/>治理的Provider Failover — v0.7.0 多LLM<br/>Provider容灾: deepseek->claude->gpt<br/>fallback链。，Provider Failover — v0.7.0 多LLM<br/>Provider容灾: deepseek->claude->gpt<br/>fallback链。相关功能的实现。<br/>provider_failover<br/>文件: intelligence_governance<br/>/provider_failover.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_intelligence_governance_self_test_py["自测试<br/>治理的自测试。Escalation Protocol Self-Test —<br/>MOD-INF-022.，自测试。Escalation Protocol<br/>Self-Test — MOD-INF-022.相关功能的实现。<br/>文件: intelligence_governance/self_test.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_intelligence_governance_subagent_hook_propagator_py["subagent钩子propagator<br/>治理的Subagent Hook Propagator — v0.13.0<br/>子Agent Hook旁路防护器。，Subagent Hook<br/>Propagator — v0.13.0 子Agent<br/>Hook旁路防护器。相关功能的实现。<br/>subagent_hook_propagator<br/>文件: intelligence_governance<br/>/subagent_hook_propagator.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_services_adapter_py["适配器<br/>治理的Escalation Adapter — MOD-INF-022<br/>统一集成入口.，Escalation Adapter — MOD-INF-022<br/>统一集成入口.相关功能的实现。<br/>文件: services/adapter.py<br/>(生产态 / production)"]
    ...
    src_zephyr_infrastructure_a2a_protocol_governance_auditor_py["审计器<br/>治理的G-CT-008 契约：A2A -> Audit 审计 Agent<br/>间通信.，G-CT-008 契约：A2A -> Audit 审计 Agent<br/>间通信.相关功能的实现。<br/>auditor<br/>文件: governance/auditor.py<br/>(生产态 / production)"]
    ...
    src_zephyr_infrastructure_a2a_protocol_governance_protocol_py["协议<br/>治理的G-CT-008 — A2ACommunication Pydantic V2<br/>BaseModel agent-to-agent<br/>通信数据结构.，G-CT-008 — A2ACommunication<br/>Pydantic V2 BaseModel agent-to-agent<br/>通信数据结构.相关功能的实现。<br/>protocol<br/>文件: governance/protocol.py<br/>(生产态 / production)"]
    ...
    src_zephyr_integration_mcp_governance_server_py["治理服务端<br/>Backend  : zephyr.governance.* 八件套治理模块<br/>governance_server<br/>文件: mcp/governance_server.py<br/>(生产态 / production)"]
    ...
    tests_governance_rule_bridge_test_worktree_lifecycle_py["测试worktree生命周期<br/>1. WorktreeState 5态枚举完整性<br/>test_worktree_lifecycle<br/>文件: rule_bridge/test_worktree_lifecycle.py<br/>(生产态 / production)"]
    tests_governance_test_ast_import_rewriter_py["测试astimportrewriter<br/>治理的测试astimportrewriter，提供testexactmatch<br/>、testnomatch、testprefixmatch等方法，测试astimp<br/>ortrewriter，提供testexactmatch、testnomatch、te<br/>stprefixmatch等方法相关功能的实现。<br/>文件: governance/test_ast_import_rewriter.py<br/>(生产态 / production)"]
    ...
    tests_task_test_task_repo_gateway_e2e_py["测试taskrepogatewaye2e<br/>端到端链路测试（OPS-2026062516）<br/>test_task_repo_gateway_e2e<br/>文件: task/test_task_repo_gateway_e2e.py<br/>(生产态 / production)"]
    tests_test_align_panoramas_py["测试alignpanoramas<br/>- PanoramaNode 数据模型字段<br/>test_align_panoramas<br/>文件: tests/test_align_panoramas.py<br/>(生产态 / production)"]
    ...
    scripts_mcp_start_all_py["启动all<br/>启动顺序（遵守 MOD-INF-013 §14 DAG<br/>顺序）：SQLite → ChromaDB → 7 Server → Gateway。<br/>start_all<br/>文件: mcp/start_all.py<br/>(生产态 / production)"]
    ...
    src_zephyr_infrastructure_registry_governance_py["注册表治理<br/>基础设施的注册表治理。Registry Governance —<br/>MOD-INF-037，注册表治理。Registry Governance —<br/>MOD-INF-037相关功能的实现。<br/>文件: infrastructure/registry_governance.py<br/>(生产态 / production)"]
    ...
    scripts_arch_guard_fitness_functions_check_position_limit_py -->|导入依赖 / import_depends| D_GOV_SCRIPTS
    ...
    class D_SHARED,D_GOV_SCRIPTS,D_GOV_RULE,D_INFRA_A2A,D_SECURITY,D_INTELLIGENCE,D_EX_CORE external_prod
```
