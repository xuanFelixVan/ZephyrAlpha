...
flowchart TD
    docs_01_policies_and_standards_registry_catalogs_rule_registry_collection_yaml["规则注册表收集<br/>rule_registry_collection模块<br/>文件: catalogs/rule_registry_collection.yaml<br/>(生产态 / production)"]
    ...
    scripts_construction_e2e_check_py["端到端检查<br/>e2e_check模块<br/>_e2e_check<br/>文件: construction/_e2e_check.py<br/>(生产态 / production)"]
    ...
    scripts_construction_check_transition_code_py["检查转换代码<br/>check_transition_code模块<br/>文件: construction/check_transition_code.py<br/>(生产态 / production)"]
    scripts_construction_d_init_task_system_py["初始化任务系统数据库 +<br/>创建任务系统自身的施工任务卡（吃狗粮）<br/>d_init_task_system模块<br/>文件: construction/d_init_task_system.py<br/>(生产态 / production)"]
    scripts_construction_demo_a2a_chat_py["A2A 多 Agent 聊天演示 - Alpha 和 Beta<br/>讨论项目评估<br/>demo_a2a_chat模块<br/>文件: construction/demo_a2a_chat.py<br/>(生产态 / production)"]
    scripts_construction_demo_a2a_coordination_py["A2A 协议协调任务演示<br/>demo_a2a_coordination模块<br/>文件: construction/demo_a2a_coordination.py<br/>(生产态 / production)"]
    ...
    scripts_governance_d5_architecture_generators_zoomable_html_py["可缩放 Mermaid HTML 生成器（共享模块）<br/>zoomable_html模块<br/>文件: generators/zoomable_html.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_adapters_risk_validation_bridge_py["风险验证桥接<br/>治理/adapters包的risk_validation_bridge模块<br/>D_EXECUTION_CORE — Risk Validation Bridge<br/>(DW-239)<br/>文件: adapters/risk_validation_bridge.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_architecture_governance_architecture_contracts_py["架构契约<br/>治理/architecture governance包的architecture_<br/>contracts模块<br/>文件: architecture_governance/architecture_<br/>contracts.py<br/>(生产态 / production)"]
    src_zephyr_governance_architecture_governance_architecture_principles_py["装饰器：为函数标记适用的架构原则<br/>治理/architecture governance包的architecture_<br/>principles模块<br/>文件: architecture_governance/architecture_<br/>principles.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_architecture_governance_blueprint_code_consistency_py["蓝图代码一致性<br/>治理/architecture governance包的blueprint_code_<br/>consistency模块<br/>Blueprint-Code Consistency Gate — MOD-INF-022.<br/>文件: architecture_governance/blueprint_code_<br/>consistency.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_architecture_governance_cross_env_consistency_py["跨环境一致性<br/>治理/architecture governance包的cross_env_<br/>consistency模块<br/>文件: architecture_governance/cross_env_<br/>consistency.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_architecture_governance_local_first_arch_py["本地首架构<br/>治理/architecture governance包的local_first_<br/>arch模块<br/>文件: architecture_governance/local_first_<br/>arch.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_context_governance_conversation_tax_detector_py["conversationtax检测器<br/>治理/context governance包的conversation_tax_<br/>detector模块<br/>文件: context_governance/conversation_tax_<br/>detector.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_evidence_pack_py["证据包<br/>证据包，主要提供pack、验证、列表packs等功能<br/>evidence_pack<br/>文件: governance/evidence_pack.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_financial_governance_instrument_py["标的合约<br/>治理/financial governance包的instrument模块<br/>文件: financial_governance/instrument.py<br/>(生产态 / production)"]
    src_zephyr_governance_financial_governance_microstructure_defense_py["microstructure防御<br/>治理/financial governance包的microstructure_<br/>defense模块<br/>文件: financial_governance/microstructure_<br/>defense.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_financial_governance_strategy_portfolio_py["策略组合<br/>治理/financial governance包的strategy_<br/>portfolio模块<br/>文件: financial_governance/strategy_portfolio.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_implementations_default_experiment_pipeline_py["默认实验管线<br/>治理/implementations包的default_experiment_<br/>pipeline模块<br/>文件: implementations/default_experiment_<br/>pipeline.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_intelligence_governance_ai_self_diagnosis_py["AI自诊断<br/>治理/intelligence governance包的ai_self_<br/>diagnosis模块<br/>文件: intelligence_governance/ai_self_<br/>diagnosis.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_intelligence_governance_confidence_quantifier_py["ConfidenceQuantifier — AI 置信度量化<br/>治理/intelligence governance包的confidence_<br/>quantifier模块<br/>文件: intelligence_governance/confidence_<br/>quantifier.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_intelligence_governance_multi_model_consensus_py["多模型共识<br/>治理/intelligence governance包的multi_model_<br/>consensus模块<br/>文件: intelligence_governance/multi_model_<br/>consensus.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_lifecycle_governance_migration_strategy_py["迁移策略<br/>治理/lifecycle governance包的migration_<br/>strategy模块<br/>文件: lifecycle_governance/migration_strategy.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_lifecycle_governance_post_live_verification_py["提交实时验证<br/>治理/lifecycle governance包的post_live_<br/>verification模块<br/>文件: lifecycle_governance/post_live_<br/>verification.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_observability_governance_analytics_base_py["analytics基类<br/>治理/observability governance包的analytics_<br/>base模块<br/>Re-export wrapper: analytics_base canonical at<br/>zephyr.reporting.analytics_base.<br/>文件: observability_governance/analytics_base.py<br/>(生产态 / production)"]
    ...
    tests_rollback_test_concurrent_mv_guard_py["并发红蓝极限对抗测试 — 多 AI 并发执行 git mv<br/>时的防护能力验证<br/>test_concurrent_mv_guard模块<br/>文件: rollback/test_concurrent_mv_guard.py<br/>(生产态 / production)"]
    ...
    tests_test_align_panoramas_py["测试alignpanoramas<br/>test_align_panoramas模块<br/>文件: tests/test_align_panoramas.py<br/>(生产态 / production)"]
    ...
    tests_test_generate_dataflow_diagram_py["测试generatedataflowdiagram<br/>test_generate_dataflow_diagram模块<br/>文件: tests/test_generate_dataflow_diagram.py<br/>(生产态 / production)"]
    tests_test_generate_decision_diagram_py["测试generate决策diagram<br/>test_generate_decision_diagram模块<br/>文件: tests/test_generate_decision_diagram.py<br/>(生产态 / production)"]
    ...
    scripts_construction_check_statuses_py["检查statuses<br/>check_statuses模块<br/>文件: construction/check_statuses.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_data_governance_akshare_provider_py["akshare提供器<br/>治理/数据治理包的akshare_provider模块<br/>D_DATA — Akshare Data Provider<br/>文件: data_governance/akshare_provider.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_intelligence_governance_delegation_engine_py["delegation引擎<br/>治理/intelligence governance包的delegation_<br/>engine模块<br/>Delegation Engine — MOD-INF-022<br/>文件: intelligence_governance/delegation_<br/>engine.py<br/>(生产态 / production)"]
    ...
    class D_SHARED,D_INFRA_A2A,D_GOV_SCRIPTS,D_SECURITY,D_RISK,D_EX_CORE external_prod
```
...
| 68 | D_GOV_SCRIPTS 脚本治理: G-battle-map-align: 作战地图对齐检测器（battle_map_panora... | → | 依赖图模式 / depgraph_schema (governance/depgraph_schema.py) | 导入依赖 / import_depends |
| 69 | D_GOV_SCRIPTS 脚本治理: G-battle-map-align: 作战地图对齐检测器（battle_map_panora... | → | battle_map_reader.py — 作战地图数据库只读查询工具模块 (p... | 导入依赖 / import_depends |
| 70 | D_GOV_SCRIPTS 脚本治理: G-battle-map-align: 作战地图对齐检测器（battle_map_panora... | → | dataflowgraph结构 / dataflowgraph_schema (persistence/dat... | 导入依赖 / import_depends |
| 71 | D_GOV_SCRIPTS 脚本治理: G-battle-map-align: 作战地图对齐检测器（battle_map_panora... | → | decisiongraph结构 / decisiongraph_schema (persistence/dec... | 导入依赖 / import_depends |
