...
flowchart TD
    ...
    scripts_arch_guard_run_all_py["运行all<br/>读取 manifest.yaml，执行所有 status=active 的<br/>fitness function。<br/>run_all<br/>文件: arch_guard/run_all.py<br/>(生产态 / production)"]
    ...
    tests_governance_rule_bridge_test_worktree_lifecycle_py["测试worktree生命周期<br/>WorktreeState 5态枚举完整性<br/>test_worktree_lifecycle<br/>文件: rule_bridge/test_worktree_lifecycle.py<br/>(生产态 / production)"]
    ...
    class D_GOV_SCRIPTS,D_SHARED,D_GOV_RULE,D_GOV_OPS_RESILIENCE,D_FEEDBACK_LOOP,D_EX_CORE,D_GOV_AUDIT external_prod
```
