...
| 跨域出边 | 494 | Cross-domain Outgoing | 494 |
...
flowchart TD
    ...
    tests_governance_shared_test_execution_tuner_py -->|测试依赖 / test_depends| D_SHARED
    ...
    class D_SHARED,D_INFRA_RECOVERY,D_GOV_OPS_RESILIENCE,D_INFRASTRUCTURE,D_TRADING,D_GOV_ENFORCEMENT,D_INFRA_RUNTIME,D_EX_CORE,D_PF_CORE,D_PF_ALLOC external_prod
```
...
| 9 | D_EX_CORE 执行核心: 交易会话 / trading_session (ex_core/trading_session.py) | → | 策略基类 / D_PORTFOLIO_CORE — StrategyBase + StrategyMet... | 导入依赖 / import_depends |
...
> 本域与 34 个外部域直接连接（出边 494 条 + 入边 172 条 = 666 条）。只显示直接连接的域，不展开具体节点。
