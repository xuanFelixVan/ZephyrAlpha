...
graph TD
    ...
    D_GOVERNANCE["(设计态 / design) D_GOVERNANCE"]
    D_GOVERNANCE -.->|runtime / runtime| src_zephyr_trading_feedback_loop_evolution_engine_py
    D_GOVERNANCE -.->|runtime / runtime| src_zephyr_trading_feedback_loop_forensic_init_py
...
graph TD
    ...
    class D_SHARED,D_SECURITY_LLM,D_INTEGRATION,D_AUTONOMY_CORE,D_FRONTEND external_prod
    class D_GOVERNANCE,D_INFRA_RUNTIME,D_EX_CORE external_design
```
...
| 585 | D_GOVERNANCE 生命周期管理: blueprint.md | → | evolution_engine.py | runtime / runtime |
| 586 | D_GOVERNANCE 生命周期管理: blueprint.md | → | feedback-loop.forensic — auto-generated packag... | runtime / runtime |
...
> 本域与 21 个外部域直接连接（出边 204 条 + 入边 717 条 = 921 条）。只显示直接连接的域，不展开具体节点。
