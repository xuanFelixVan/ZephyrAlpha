...
| 跨域入边 | 716 | Cross-domain Incoming | 716 |
...
graph TD
    ...
    class D_GOVERNANCE,D_INFRA_RECOVERY,D_INFRA_RUNTIME,D_AUTONOMY_CORE,D_INTEGRATION external_prod
...
graph TD
    ...
    class D_SHARED,D_INTEGRATION,D_INFRA_RUNTIME,D_OPS,D_GOVERNANCE,D_SECURITY external_prod
...
graph TD
    ...
    class D_INTEGRATION,D_INFRA_RUNTIME,D_AUDITTEST external_design
```
...
graph TD
    ...
    D_INFRA_RUNTIME["(生产态 / production) D_INFRA_RUNTIME"]
    src_zephyr_trading_boot_hooks_py -->|导入依赖 / import_depends| D_INFRA_RUNTIME
    src_zephyr_trading_boot_hooks_py -->|导入依赖 / import_depends| D_INFRA_RUNTIME
    D_INFRA_RUNTIME -.->|导入依赖 / import_depends| src_zephyr_trading_feedback_loop_security_secret_rotation_py
    D_EX_CORE["(原型态 / prototype) D_EX_CORE"]
    D_EX_CORE -.->|导入依赖 / import_depends| src_zephyr_trading_trading_contracts_execution_fill_py
    ...
    class D_SHARED,D_INTEGRATION,D_AUTONOMY_CORE,D_SECURITY,D_INFRA_TELEMETRY,D_INFRA_RUNTIME,D_FRONTEND,D_GOVERNANCE external_prod
    class D_EX_CORE external_design
```
...
| 583 | D_GOVERNANCE 生命周期管理: blueprint.md | → | Cognitive Load Estimator — v0.6.0 R68 (cogniti... | runtime / runtime |
| 584 | D_GOVERNANCE 生命周期管理: blueprint.md | → | feedback-loop.evolution — auto-generated packa... | runtime / runtime |
| 585 | D_GOVERNANCE 生命周期管理: blueprint.md | → | TOCTOU Guard — v0.15.0 R207 (toctou_guard.py) | runtime / runtime |
...
> 本域与 21 个外部域直接连接（出边 204 条 + 入边 716 条 = 920 条）。只显示直接连接的域，不展开具体节点。
