...
| 跨域入边 | 634 | Cross-domain Incoming | 634 |
...
graph TD
    ...
    class D_GOVERNANCE,D_AUTONOMY_CORE,D_INTEGRATION external_prod
...
> 本域与 21 个外部域直接连接（出边 204 条 + 入边 634 条 = 838 条）。只显示直接连接的域，不展开具体节点。
