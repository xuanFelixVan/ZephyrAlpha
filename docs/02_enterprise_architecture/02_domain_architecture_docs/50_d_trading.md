...
| 跨域出边 | 202 | Cross-domain Outgoing | 202 |
...
graph TD
    ...
    class D_OPS,D_INFRA_RUNTIME,D_SHARED,D_GOVERNANCE,D_INTEGRATION,D_SECURITY external_prod
...
graph TD
    ...
    src_zephyr_trading_orchestrator_governance_init_py -.->|config_depends / config_depends| src_zephyr_trading_orchestrator_governance_autonomy_guard_py
    D_SHARED["(生产态 / production) D_SHARED"]
    ...
    class D_SHARED,D_GOVERNANCE,D_AUTONOMY_CORE external_prod
...
graph TD
    ...
    src_zephyr_trading_orchestrator_lifecycle_init_py -.->|config_depends / config_depends| src_zephyr_trading_orchestrator_lifecycle_incident_postmortem_py
    src_zephyr_trading_orchestrator_quality_init_py -.->|config_depends / config_depends| src_zephyr_trading_orchestrator_quality_benchmark_runner_py
...
graph TD
    ...
    class D_INTEGRATION,D_GOVERNANCE,D_SHARED,D_RISK external_prod
...
graph TD
    ...
    class D_SECURITY_LLM,D_INTEGRATION,D_GOVERNANCE,D_INFRA_RUNTIME,D_INTELLIGENCE,D_GOV_ENFORCEMENT,D_EX_CORE,D_FRONTEND external_prod
...
graph TD
    ...
    class D_SHARED,D_INFRA_TELEMETRY,D_SECURITY external_design
```
...
| 141 | HealthMonitor — 健康监控 + 自愈 (health_monito... | → | D_SHARED 共享服务: telemetry_emitter.py | 导入依赖 / import_depends |
| 142 | HealthMonitor — 健康监控 + 自愈 (health_monito... | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 导入依赖 / import_depends |
| 143 | HealthMonitor — 健康监控 + 自愈 (health_monito... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 144 | HealthMonitor — 健康监控 + 自愈 (health_monito... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 145 | ide_health_daemon.py — TRAE IDE 幽灵窗口守护线... | → | D_SHARED 共享服务: TaskRepositoryProtocol — TaskRepository 的 Pro... | 导入依赖 / import_depends |
| 146 | ide_health_daemon.py — TRAE IDE 幽灵窗口守护线... | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 导入依赖 / import_depends |
| 147 | ide_health_daemon.py — TRAE IDE 幽灵窗口守护线... | → | D_SHARED 共享服务: constants.py —— 共享枚举 & 常量集中 re-export... | 导入依赖 / import_depends |
| 148 | ide_health_daemon.py — TRAE IDE 幽灵窗口守护线... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 149 | lifecycle_manager.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 150 | NightShiftQueue — 夜班登记表持久化 (night_shif... | → | D_SHARED 共享服务: serialization.py —— 统一序列化/反序列化基础设... | 导入依赖 / import_depends |
| 151 | NightShiftQueue — 夜班登记表持久化 (night_shif... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 152 | AgentHealthMonitor · Agent 健康监控（三态 + 5 ... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 153 | AgentOrchestrator · 多角色 Agent 路由、工具链.... | → | D_SHARED 共享服务: __init__.py | 导入依赖 / import_depends |
| 154 | AgentOrchestrator · 多角色 Agent 路由、工具链.... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 155 | AgentOrchestrator · 多角色 Agent 路由、工具链.... | → | D_SHARED 共享服务: serialization.py —— 统一序列化/反序列化基础设... | 导入依赖 / import_depends |
| 156 | AgentOrchestrator · 多角色 Agent 路由、工具链.... | → | D_SHARED 共享服务: async_utils.py — async/sync 边界桥接（5.12.8 .... | 导入依赖 / import_depends |
| 157 | AgentOrchestrator · 多角色 Agent 路由、工具链.... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 158 | Orc 告警接收器 — handle_alert() 消费者 (alert_... | → | D_SHARED 共享服务: TaskRepositoryProtocol — TaskRepository 的 Pro... | 导入依赖 / import_depends |
| 159 | CT-ORC-SCRIPT-001 运行时桥接 (finding_bridge.py) | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 160 | agent_orchestrator.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 161 | agent_orchestrator.py | → | D_SHARED 共享服务: serialization.py —— 统一序列化/反序列化基础设... | 导入依赖 / import_depends |
| 162 | agent_orchestrator.py | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 163 | ActiveTaskQueue — 后台任务轮询与自动分发 (task... | → | D_SHARED 共享服务: TaskRepositoryProtocol — TaskRepository 的 Pro... | 导入依赖 / import_depends |
| 164 | DeferredQueue: WAITING -> READY task scheduler.... | → | D_SHARED 共享服务: Zero-dependency Observer pattern (subscribe/emi... | 导入依赖 / import_depends |
| 165 | DeferredQueue: WAITING -> READY task scheduler.... | → | D_SHARED 共享服务: SQLite 连接工厂真源（SSoT） (sqlite_factory.py) | 导入依赖 / import_depends |
| 166 | BatchOrchestrator — 多 Worker 批量任务协调器（... | → | D_SHARED 共享服务: TaskRepositoryProtocol — TaskRepository 的 Pro... | 导入依赖 / import_depends |
| 167 | Orc->VMS 记忆写入器 (memory_writer.py) | → | D_SHARED 共享服务: serialization.py —— 统一序列化/反序列化基础设... | 导入依赖 / import_depends |
| 168 | TriggerRouter — RI-03 触发路由器（M3 跨模块触.... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 169 | WaveGenerator — 根据 Task 依赖图生成执行 Wave.... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 170 | WaveGenerator — 根据 Task 依赖图生成执行 Wave.... | → | D_SHARED 共享服务: db_utils.py — SQLite 连接公共 API（SSoT: zephy... | 导入依赖 / import_depends |
| 171 | ChaosHook — integrates ChaosEngine with the or... | → | D_SHARED 共享服务: orchestration_protocol.py | 导入依赖 / import_depends |
| 172 | FileTaskMapper — 文件路径 ↔ Task N:N 映射器（... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 173 | FileTaskMapper — 文件路径 ↔ Task N:N 映射器（... | → | D_SHARED 共享服务: db_utils.py — SQLite 连接公共 API（SSoT: zephy... | 导入依赖 / import_depends |
| 174 | FileTaskMapper — 文件路径 ↔ Task N:N 映射器（... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 175 | HallucinationDetector · Chain-of-Verification.... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 176 | SessionManager — AI Agent 会话生命周期管理（CT... | → | D_SHARED 共享服务: errors.py —— ZephyrAlpha 统一错误层次（Tradit... | 导入依赖 / import_depends |
| 177 | SessionManager — AI Agent 会话生命周期管理（CT... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 178 | StateSynchronizer — 同步 SQLite 状态与文件系统... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 179 | StateSynchronizer — 同步 SQLite 状态与文件系统... | → | D_SHARED 共享服务: db_utils.py — SQLite 连接公共 API（SSoT: zephy... | 导入依赖 / import_depends |
| 180 | StateSynchronizer — 同步 SQLite 状态与文件系统... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 181 | RollbackManager — 仅调试用途的 DB-state 快照，... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 182 | RollbackManager — 仅调试用途的 DB-state 快照，... | → | D_SHARED 共享服务: db_utils.py — SQLite 连接公共 API（SSoT: zephy... | 导入依赖 / import_depends |
| 183 | RollbackManager — 仅调试用途的 DB-state 快照，... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 184 | resource_optimization.py - MAPE-K autonomic res... | → | D_SHARED 共享服务: capacity_calibrator.py | 导入依赖 / import_depends |
| 185 | resource_optimization.py - MAPE-K autonomic res... | → | D_SHARED 共享服务: capacity_digital_twin.py | 导入依赖 / import_depends |
| 186 | resource_optimization.py - MAPE-K autonomic res... | → | D_SHARED 共享服务: capacity_fingerprint.py | 导入依赖 / import_depends |
| 187 | resource_optimization.py - MAPE-K autonomic res... | → | D_SHARED 共享服务: capacity_governance_loop.py | 导入依赖 / import_depends |
| 188 | resource_optimization.py - MAPE-K autonomic res... | → | D_SHARED 共享服务: capacity_runbook_generator.py | 导入依赖 / import_depends |
| 189 | resource_optimization.py - MAPE-K autonomic res... | → | D_SHARED 共享服务: model_capacity_probe.py | 导入依赖 / import_depends |
| 190 | resource_optimization.py - MAPE-K autonomic res... | → | D_SHARED 共享服务: EventBus — 事件总线（带背压控制）(M-07) (event... | 导入依赖 / import_depends |
| 191 | resource_optimization.py - MAPE-K autonomic res... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP s... | 导入依赖 / import_depends |
| 192 | resource_optimization.py - MAPE-K autonomic res... | → | D_SHARED 共享服务: io_cache.py - File-level I/O cache with LRU evi... | 导入依赖 / import_depends |
| 193 | runtime_config.py | → | D_SHARED 共享服务: runtime_types.py | 导入依赖 / import_depends |
| 194 | speed_baseline_checker.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 195 | StatusDashboard — 实时状态面板 (status_dashboa... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 196 | StopGate — 质量闸门 (stop_gate.py) | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 197 | risk_limit_violation_error.py | → | D_SHARED 共享服务: trace_context.py | 导入依赖 / import_depends |
| 198 | risk_validator_protocol.py | → | D_SHARED 共享服务: risk_limits.py | 导入依赖 / import_depends |
| 199 | work_orchestrator.py | → | D_SHARED 共享服务: serialization.py —— 统一序列化/反序列化基础设... | 导入依赖 / import_depends |
| 200 | work_orchestrator.py | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 201 | zombie_scanner.py — 僵尸 Python 进程检测与自动... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
...
| 68 | D_AUDITTEST 审计测试套件: test_auto_runtime_core.py | → | runtime_config.py | 测试依赖 / test_depends |
| 69 | D_AUDITTEST 审计测试套件: F1 AutoRuntimeCore 非mock端到端集成测试 (test_a... | → | AutoRuntimeCore — 三层运行时运营中心（系统大脑... | 测试依赖 / test_depends |
| 70 | D_AUDITTEST 审计测试套件: F1 AutoRuntimeCore 非mock端到端集成测试 (test_a... | → | CapabilityRegistry — 能力注册中心 (capability_... | 测试依赖 / test_depends |
| 71 | D_AUDITTEST 审计测试套件: F1 AutoRuntimeCore 非mock端到端集成测试 (test_a... | → | DreamCycle — 知识固化引擎 (dream_cycle.py) | 测试依赖 / test_depends |
| 72 | D_AUDITTEST 审计测试套件: F1 AutoRuntimeCore 非mock端到端集成测试 (test_a... | → | HealthMonitor — 健康监控 + 自愈 (health_monito... | 测试依赖 / test_depends |
| 73 | D_AUDITTEST 审计测试套件: F1 AutoRuntimeCore 非mock端到端集成测试 (test_a... | → | runtime_config.py | 测试依赖 / test_depends |
| 74 | D_AUDITTEST 审计测试套件: F1 AutoRuntimeCore 非mock端到端集成测试 (test_a... | → | WorkDAG + WorkItem — 工作编排数据模型 (work_da... | 测试依赖 / test_depends |
| 75 | D_AUDITTEST 审计测试套件: F1 AutoRuntimeCore 非mock端到端集成测试 (test_a... | → | work_orchestrator.py | 测试依赖 / test_depends |
| 76 | D_AUDITTEST 审计测试套件: AutoRuntimeCore → FeedbackLoopScheduler 自动启... | → | AutoRuntimeCore — 三层运行时运营中心（系统大脑... | 测试依赖 / test_depends |
| 77 | D_AUDITTEST 审计测试套件: AutoRuntimeCore → FeedbackLoopScheduler 自动启... | → | FLE 全链路调度器 —— collect->detect->diagnose... | 测试依赖 / test_depends |
| 78 | D_AUDITTEST 审计测试套件: AutoRuntimeCore → FeedbackLoopScheduler 自动启... | → | runtime_config.py | 测试依赖 / test_depends |
| 79 | D_AUDITTEST 审计测试套件: test_auto_task_generator.py | → | AutoTaskGenerator — 自动任务生成器 (auto_task_... | 测试依赖 / test_depends |
| 80 | D_AUDITTEST 审计测试套件: test_autonomy_credit.py | → | Autonomy Credit System — v0.7.0 R87 (autonomy_... | 测试依赖 / test_depends |
| 81 | D_AUDITTEST 审计测试套件: test_autonomy_guard.py | → | Owner 缺位分级自治（CT-AUTONOMY）——Owner离线-... | 测试依赖 / test_depends |
| 82 | D_AUDITTEST 审计测试套件: test_autonomy_maturity.py | → | Autonomy Maturity Ladder — v0.7.0 R86 (autonom... | 测试依赖 / test_depends |
| 83 | D_AUDITTEST 审计测试套件: test_dispatch_table_root.py | → | AI Agent 冷启动分派表（Dispatch Table） (dispat... | 测试依赖 / test_depends |
| 84 | D_AUDITTEST 审计测试套件: test_blueprint_code_reconciler.py | → | Blueprint-Code Reconciler — v0.14.0 R195 (blue... | 测试依赖 / test_depends |
| 85 | D_AUDITTEST 审计测试套件: test_blueprint_scorer.py | → | BlueprintScorer — 蓝图路由统一打分逻辑 (bluepr... | 测试依赖 / test_depends |
| 86 | D_AUDITTEST 审计测试套件: test_blueprint_validator.py | → | Blueprint Validator — v0.8.0 R108 (blueprint_v... | 测试依赖 / test_depends |
| 87 | D_AUDITTEST 审计测试套件: test_gen_inherited.py | → | _gen_inherited.py | 测试依赖 / test_depends |
| 88 | D_AUDITTEST 审计测试套件: test_bridges_spec_auditor.py | → | protocols.py | 测试依赖 / test_depends |
| 89 | D_AUDITTEST 审计测试套件: DM-201504: F4 BudgetEngine自动关闭——shutdown.... | → | boot_hooks.py | 测试依赖 / test_depends |
| 90 | D_AUDITTEST 审计测试套件: test_error_budget.py | → | Error Budget 状态机——monthly budget + burn_ra... | 测试依赖 / test_depends |
| 91 | D_AUDITTEST 审计测试套件: test_canary_manager.py | → | 金丝雀发布管理器（CT-CANARY）——权重分流+指标.... | 测试依赖 / test_depends |
| 92 | D_AUDITTEST 审计测试套件: test_canary_repair.py | → | Canary Repair — v0.8.0 R104b (canary_repair.py) | 测试依赖 / test_depends |
| 93 | D_AUDITTEST 审计测试套件: test_capability_card.py | → | CapabilityCard — 能力卡片数据模型 (capability_... | 测试依赖 / test_depends |
| 94 | D_AUDITTEST 审计测试套件: test_capability_registry.py | → | CapabilityCard — 能力卡片数据模型 (capability_... | 测试依赖 / test_depends |
| 95 | D_AUDITTEST 审计测试套件: test_capability_registry.py | → | CapabilityRegistry — 能力注册中心 (capability_... | 测试依赖 / test_depends |
| 96 | D_AUDITTEST 审计测试套件: test_capability_sync.py | → | CapabilityCard — 能力卡片数据模型 (capability_... | 测试依赖 / test_depends |
| 97 | D_AUDITTEST 审计测试套件: test_capability_sync.py | → | CapabilityRegistry — 能力注册中心 (capability_... | 测试依赖 / test_depends |
| 98 | D_AUDITTEST 审计测试套件: test_capability_sync.py | → | capability_sync.py | 测试依赖 / test_depends |
| 99 | D_AUDITTEST 审计测试套件: test_capacity_aware_repair.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 100 | D_AUDITTEST 审计测试套件: test_capacity_budget_root.py | → | 全局容量预算控制器（Capacity Budget Controller... | 测试依赖 / test_depends |
| 101 | D_AUDITTEST 审计测试套件: test_capacity_forecast.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 102 | D_AUDITTEST 审计测试套件: test_chaos_engine.py | → | Chaos 故障注入引擎（CT-CHAOS-001）——4注入点×... | 测试依赖 / test_depends |
| 103 | D_AUDITTEST 审计测试套件: test_chaos_engine_ops.py | → | Chaos 故障注入引擎（CT-CHAOS-001）——4注入点×... | 测试依赖 / test_depends |
| 104 | D_AUDITTEST 审计测试套件: test_chaos_engineering.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 105 | D_AUDITTEST 审计测试套件: test_chaos_hooks.py | → | Chaos 故障注入引擎（CT-CHAOS-001）——4注入点×... | 测试依赖 / test_depends |
| 106 | D_AUDITTEST 审计测试套件: test_chaos_hooks.py | → | ChaosHook — integrates ChaosEngine with the or... | 测试依赖 / test_depends |
| 107 | D_AUDITTEST 审计测试套件: test_cold_start_conservative_mode.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 108 | D_AUDITTEST 审计测试套件: test_config_complexity_budget.py | → | Config Complexity Budget — v0.16.0 R227 (confi... | 测试依赖 / test_depends |
| 109 | D_AUDITTEST 审计测试套件: test_config_drift.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 110 | D_AUDITTEST 审计测试套件: test_config_governance.py | → | Config Governance — v0.3.0 R8 (config_governan... | 测试依赖 / test_depends |
| 111 | D_AUDITTEST 审计测试套件: test_config_hot_reload_guard.py | → | Config Hot-Reload Guard — v0.40.0 R498 (config... | 测试依赖 / test_depends |
| 112 | D_AUDITTEST 审计测试套件: test_context_truncation.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 113 | D_AUDITTEST 审计测试套件: test_context_window_contamination_detector.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 114 | D_AUDITTEST 审计测试套件: test_context_window_pressure_manager.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 115 | D_AUDITTEST 审计测试套件: test_api_version_contract.py | → | API Version Contract — v0.14.0 R188 (api_versi... | 测试依赖 / test_depends |
| 116 | D_AUDITTEST 审计测试套件: test_contract_registry_root.py | → | 集成契约注册表（Contract Registry） (contract_r... | 测试依赖 / test_depends |
| 117 | D_AUDITTEST 审计测试套件: test_contract_router_root.py | → | 集成契约注册表（Contract Registry） (contract_r... | 测试依赖 / test_depends |
| 118 | D_AUDITTEST 审计测试套件: test_contract_router_root.py | → | 契约路由（Contract Router） (contract_router.py) | 测试依赖 / test_depends |
| 119 | D_AUDITTEST 审计测试套件: test_cross_blueprint_contract_drift.py | → | Cross-Blueprint Contract Drift Monitor — v0.39... | 测试依赖 / test_depends |
| 120 | D_AUDITTEST 审计测试套件: test_cross_gen_validation.py | → | Cross-Gen Validation — v0.7.0 R78 (cross_gen_v... | 测试依赖 / test_depends |
| 121 | D_AUDITTEST 审计测试套件: test_cross_guard_conflict_detector.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 122 | D_AUDITTEST 审计测试套件: test_cross_module_integration_root.py | → | Cross-Module Integration Verifier — v0.5.0 R39... | 测试依赖 / test_depends |
| 123 | D_AUDITTEST 审计测试套件: test_cross_session_consistency_validator.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 124 | D_AUDITTEST 审计测试套件: test_cross_session_knowledge_integrity.py | → | Cross-Session Knowledge Integrity — v0.16.0 R2... | 测试依赖 / test_depends |
| 125 | D_AUDITTEST 审计测试套件: test_cross_signal_validator.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 126 | D_AUDITTEST 审计测试套件: test_cross_system_correlator.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 127 | D_AUDITTEST 审计测试套件: test_data_quality_gate.py | → | Data Quality Gate — v0.11.0 R143 (data_quality... | 测试依赖 / test_depends |
| 128 | D_AUDITTEST 审计测试套件: test_data_volume_growth_monitor.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 129 | D_AUDITTEST 审计测试套件: Tests for zephyr.trading.feedback_loop.db_bridg... | → | FLE DB契约适配器 — 通过规范zephyr.governance.s... | 测试依赖 / test_depends |
| 130 | D_AUDITTEST 审计测试套件: test_db_integrity.py | → | DB Integrity Gate — v0.3.0 R17 (db_integrity.py) | 测试依赖 / test_depends |
| 131 | D_AUDITTEST 审计测试套件: test_decision_engine.py | → | Feedback Loop Decision Engine (decision_engine.py) | 测试依赖 / test_depends |
| 132 | D_AUDITTEST 审计测试套件: test_decision_engine.py | → | protocols.py | 测试依赖 / test_depends |
| 133 | D_AUDITTEST 审计测试套件: test_decision_provenance.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 134 | D_AUDITTEST 审计测试套件: test_dependency_freshness_monitor.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 135 | D_AUDITTEST 审计测试套件: test_dependency_lock.py | → | 外部依赖版本锁（CT-DEPS）——Python包版本锁定+h... | 测试依赖 / test_depends |
| 136 | D_AUDITTEST 审计测试套件: test_concept_drift.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 137 | D_AUDITTEST 审计测试套件: test_schema_evolution_root.py | → | Schema Evolution — v0.9.0 R111 (schema_evoluti... | 测试依赖 / test_depends |
| 138 | D_AUDITTEST 审计测试套件: test_version_migrator.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 139 | D_AUDITTEST 审计测试套件: test_incident_priority_triage_automator.py | → | Incident Priority Triage Automator — v0.37.0 R... | 测试依赖 / test_depends |
| 140 | D_AUDITTEST 审计测试套件: test_owner_absence_escalation.py | → | Owner Absence Escalation — v0.37.0 R462 (owner... | 测试依赖 / test_depends |
| 141 | D_AUDITTEST 审计测试套件: test_external_health.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 142 | D_AUDITTEST 审计测试套件: test_external_validation_checkpoint.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 143 | D_AUDITTEST 审计测试套件: test_external_verifier.py | → | External Verifier — v0.15.0 R203 (external_ver... | 测试依赖 / test_depends |
| 144 | D_AUDITTEST 审计测试套件: F21 自动运行测试 — DM-201250 (test_f21_auto_ru... | → | __init__.py | 测试依赖 / test_depends |
| 145 | D_AUDITTEST 审计测试套件: F21 自动运行测试 — DM-201250 (test_f21_auto_ru... | → | HealthMonitor — 健康监控 + 自愈 (health_monito... | 测试依赖 / test_depends |
| 146 | D_AUDITTEST 审计测试套件: F21 自动关闭测试 — DM-201250 (test_f21_auto_sh... | → | __init__.py | 测试依赖 / test_depends |
| 147 | D_AUDITTEST 审计测试套件: F21 自动关闭测试 — DM-201250 (test_f21_auto_sh... | → | Finalizer — 优雅清理器 (finalizer.py) | 测试依赖 / test_depends |
| 148 | D_AUDITTEST 审计测试套件: F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | __init__.py | 测试依赖 / test_depends |
| 149 | D_AUDITTEST 审计测试套件: F21 自动启动测试 — DM-201250 (test_f21_auto_st... | → | Finalizer — 优雅清理器 (finalizer.py) | 测试依赖 / test_depends |
| 150 | D_AUDITTEST 审计测试套件: test_flag_lifecycle.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 151 | D_AUDITTEST 审计测试套件: test_openfeature.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 152 | D_AUDITTEST 审计测试套件: test_fl_action_reversibility.py | → | Action Reversibility — v0.15.0 R208 (action_re... | 测试依赖 / test_depends |
| 153 | D_AUDITTEST 审计测试套件: test_fl_action_selector.py | → | action_selector.py | 测试依赖 / test_depends |
| 154 | D_AUDITTEST 审计测试套件: test_fl_action_selector.py | → | protocols.py | 测试依赖 / test_depends |
| 155 | D_AUDITTEST 审计测试套件: test_fl_agent_lifecycle.py | → | Agent Lifecycle Manager — v0.12.0 R159c (agent... | 测试依赖 / test_depends |
| 156 | D_AUDITTEST 审计测试套件: test_fl_anomaly_detector.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 157 | D_AUDITTEST 审计测试套件: test_fl_anomaly_detector.py | → | FeedbackCollector: collect task execution feedb... | 测试依赖 / test_depends |
| 158 | D_AUDITTEST 审计测试套件: test_fl_anomaly_detector.py | → | MetricsCollector: append-only metrics recording... | 测试依赖 / test_depends |
| 159 | D_AUDITTEST 审计测试套件: test_fl_anomaly_detector.py | → | protocols.py | 测试依赖 / test_depends |
| 160 | D_AUDITTEST 审计测试套件: test_fl_api_version_contract.py | → | API Version Contract — v0.14.0 R188 (api_versi... | 测试依赖 / test_depends |
| 161 | D_AUDITTEST 审计测试套件: test_fl_auto_evolution.py | → | auto_evolution.py | 测试依赖 / test_depends |
| 162 | D_AUDITTEST 审计测试套件: test_fl_auto_evolution.py | → | evolution_engine.py | 测试依赖 / test_depends |
| 163 | D_AUDITTEST 审计测试套件: test_fl_autonomy_credit.py | → | Autonomy Credit System — v0.7.0 R87 (autonomy_... | 测试依赖 / test_depends |
| 164 | D_AUDITTEST 审计测试套件: test_fl_autonomy_maturity.py | → | Autonomy Maturity Ladder — v0.7.0 R86 (autonom... | 测试依赖 / test_depends |
| 165 | D_AUDITTEST 审计测试套件: test_fl_backpressure_bridge.py | → | FLE -> Pipeline 背压桥接（CTR-BP-001~003） (bac... | 测试依赖 / test_depends |
| 166 | D_AUDITTEST 审计测试套件: test_fl_backpressure_bridge.py | → | evolution_engine.py | 测试依赖 / test_depends |
| 167 | D_AUDITTEST 审计测试套件: test_fl_blueprint_code_reconciler.py | → | Blueprint-Code Reconciler — v0.14.0 R195 (blue... | 测试依赖 / test_depends |
| 168 | D_AUDITTEST 审计测试套件: test_fl_blueprint_validator.py | → | Blueprint Validator — v0.8.0 R108 (blueprint_v... | 测试依赖 / test_depends |
| 169 | D_AUDITTEST 审计测试套件: test_fl_calendar_adapter.py | → | Calendar Adapter — v0.8.0 R102b (calendar_adap... | 测试依赖 / test_depends |
| 170 | D_AUDITTEST 审计测试套件: test_fl_checkpoint_manager.py | → | Checkpoint Manager — v0.3.0 R18 (checkpoint_ma... | 测试依赖 / test_depends |
| 171 | D_AUDITTEST 审计测试套件: test_fl_ci_cd_pre_scanner.py | → | CI/CD Pre-Scanner — v0.8.0 R107 (ci_cd_pre_sca... | 测试依赖 / test_depends |
| 172 | D_AUDITTEST 审计测试套件: test_fl_concurrent_change_deconfliction.py | → | Concurrent Change Deconfliction — v0.16.0 R230... | 测试依赖 / test_depends |
| 173 | D_AUDITTEST 审计测试套件: test_fl_config.py | → | config.py | 测试依赖 / test_depends |
| 174 | D_AUDITTEST 审计测试套件: test_fl_config_complexity_budget.py | → | Config Complexity Budget — v0.16.0 R227 (confi... | 测试依赖 / test_depends |
| 175 | D_AUDITTEST 审计测试套件: test_fl_config_governance.py | → | Config Governance — v0.3.0 R8 (config_governan... | 测试依赖 / test_depends |
| 176 | D_AUDITTEST 审计测试套件: test_fl_config_timeline.py | → | Config Timeline — v0.8.0 R99 (config_timeline.py) | 测试依赖 / test_depends |
| 177 | D_AUDITTEST 审计测试套件: test_fl_conflict_arbitration.py | → | Conflict Arbitration — v0.10.0 R130 (conflict_... | 测试依赖 / test_depends |
| 178 | D_AUDITTEST 审计测试套件: test_fl_cve_scanner.py | → | CVE Scanner — v0.8.0 R106 (cve_scanner.py) | 测试依赖 / test_depends |
| 179 | D_AUDITTEST 审计测试套件: test_fl_data_quality_gate.py | → | Data Quality Gate — v0.11.0 R143 (data_quality... | 测试依赖 / test_depends |
| 180 | D_AUDITTEST 审计测试套件: test_fl_data_quality_validator.py | → | Data Quality Validator — v0.9.0 R110 (data_qua... | 测试依赖 / test_depends |
| 181 | D_AUDITTEST 审计测试套件: test_fl_db_bridge.py | → | FLE DB契约适配器 — 通过规范zephyr.governance.s... | 测试依赖 / test_depends |
| 182 | D_AUDITTEST 审计测试套件: test_fl_db_integrity.py | → | DB Integrity Gate — v0.3.0 R17 (db_integrity.py) | 测试依赖 / test_depends |
| 183 | D_AUDITTEST 审计测试套件: test_fl_decision_engine.py | → | Feedback Loop Decision Engine (decision_engine.py) | 测试依赖 / test_depends |
| 184 | D_AUDITTEST 审计测试套件: test_fl_decision_engine.py | → | protocols.py | 测试依赖 / test_depends |
| 185 | D_AUDITTEST 审计测试套件: test_fl_deployment_suppression.py | → | Deployment Suppression — v0.37.0 R464 (deploym... | 测试依赖 / test_depends |
| 186 | D_AUDITTEST 审计测试套件: test_fl_dynamic_llm_cost_router.py | → | Dynamic LLM Cost Router — v0.8.0 R109 (dynamic... | 测试依赖 / test_depends |
| 187 | D_AUDITTEST 审计测试套件: test_fl_emergency_takeover.py | → | Emergency Takeover — v0.7.0 R88 (emergency_tak... | 测试依赖 / test_depends |
| 188 | D_AUDITTEST 审计测试套件: test_fl_error_budget.py | → | Error Budget 状态机——monthly budget + burn_ra... | 测试依赖 / test_depends |
| 189 | D_AUDITTEST 审计测试套件: test_fl_eval_harness.py | → | eval_harness.py | 测试依赖 / test_depends |
| 190 | D_AUDITTEST 审计测试套件: test_fl_evolution_engine.py | → | evolution_engine.py | 测试依赖 / test_depends |
| 191 | D_AUDITTEST 审计测试套件: test_fl_exceptions.py | → | exceptions.py | 测试依赖 / test_depends |
| 192 | D_AUDITTEST 审计测试套件: test_fl_federated_security.py | → | Federated Security — v0.10.0 R131 (federated_s... | 测试依赖 / test_depends |
| 193 | D_AUDITTEST 审计测试套件: test_fl_financial_stratification.py | → | Financial Stratification — v0.5.0 R50 (financi... | 测试依赖 / test_depends |
| 194 | D_AUDITTEST 审计测试套件: test_fl_fitness_functions.py | → | fitness_functions.py | 测试依赖 / test_depends |
| 195 | D_AUDITTEST 审计测试套件: test_fl_flag_lifecycle_manager.py | → | Flag Lifecycle Manager — v0.3.0 R11 (flag_life... | 测试依赖 / test_depends |
| 196 | D_AUDITTEST 审计测试套件: test_fl_generator.py | → | generator.py | 测试依赖 / test_depends |
| 197 | D_AUDITTEST 审计测试套件: test_fl_global_action_scheduler.py | → | Global Action Scheduler — v0.16.0 R226 (global... | 测试依赖 / test_depends |
| 198 | D_AUDITTEST 审计测试套件: test_fl_incident_priority_triage_automator.py | → | Incident Priority Triage Automator — v0.37.0 R... | 测试依赖 / test_depends |
| 199 | D_AUDITTEST 审计测试套件: test_fl_intent_driven_ops.py | → | Intent-Driven Ops — v0.12.0 R159 (intent_drive... | 测试依赖 / test_depends |
| 200 | D_AUDITTEST 审计测试套件: test_fl_kb_provenance.py | → | KB Provenance — v0.10.0 R136 (kb_provenance.py) | 测试依赖 / test_depends |
| 201 | D_AUDITTEST 审计测试套件: test_fl_license_compliance.py | → | License Compliance — v0.14.0 R198 (license_com... | 测试依赖 / test_depends |
| 202 | D_AUDITTEST 审计测试套件: test_fl_llm_cost_router.py | → | LLM Cost Router — v0.3.0 R20 (llm_cost_router.py) | 测试依赖 / test_depends |
| 203 | D_AUDITTEST 审计测试套件: test_fl_merkle_audit_root.py | → | Merkle Audit Root — v0.8.0 R104 (merkle_audit_... | 测试依赖 / test_depends |
| 204 | D_AUDITTEST 审计测试套件: test_fl_meta_performance_gate.py | → | Meta Performance Gate — v0.11.0 R158 (meta_per... | 测试依赖 / test_depends |
| 205 | D_AUDITTEST 审计测试套件: test_fl_multi_agent_orchestrator.py | → | Multi-Agent Orchestrator — v0.12.0 R159b (mult... | 测试依赖 / test_depends |
| 206 | D_AUDITTEST 审计测试套件: test_fl_notification_personalizer.py | → | Notification Personalizer — v0.6.0 R67 (notifi... | 测试依赖 / test_depends |
| 207 | D_AUDITTEST 审计测试套件: test_fl_owner_absence_escalation.py | → | Owner Absence Escalation — v0.37.0 R462 (owner... | 测试依赖 / test_depends |
| 208 | D_AUDITTEST 审计测试套件: test_fl_parameterized_safety_gate.py | → | GateVerdict — GateVerdict (parameterized_safet... | 测试依赖 / test_depends |
| 209 | D_AUDITTEST 审计测试套件: test_fl_protocols.py | → | protocols.py | 测试依赖 / test_depends |
| 210 | D_AUDITTEST 审计测试套件: test_fl_safety_gate_l1_l27.py | → | Safety Gates L1-L27 — Unified Pipeline (MOD-FE... | 测试依赖 / test_depends |
| 211 | D_AUDITTEST 审计测试套件: test_fl_scheduler.py | → | FLE 全链路调度器 —— collect->detect->diagnose... | 测试依赖 / test_depends |
| 212 | D_AUDITTEST 审计测试套件: test_fl_scheduler_act.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 213 | D_AUDITTEST 审计测试套件: test_fl_scheduler_act.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 214 | D_AUDITTEST 审计测试套件: test_fl_scheduler_act.py | → | R522: SelfModificationRateLimiter (self_modific... | 测试依赖 / test_depends |
| 215 | D_AUDITTEST 审计测试套件: test_fl_scheduler_act.py | → | Graceful Degradation Planner — v0.40.0 R496 (g... | 测试依赖 / test_depends |
| 216 | D_AUDITTEST 审计测试套件: test_fl_scheduler_act.py | → | Self API Throttle Defense — v0.39.0 R491 (self... | 测试依赖 / test_depends |
| 217 | D_AUDITTEST 审计测试套件: test_fl_scheduler_act.py | → | scheduler_act.py | 测试依赖 / test_depends |
| 218 | D_AUDITTEST 审计测试套件: test_fl_scheduler_collect_detect.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 219 | D_AUDITTEST 审计测试套件: test_fl_scheduler_collect_detect.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 220 | D_AUDITTEST 审计测试套件: test_fl_scheduler_collect_detect.py | → | FeedbackCollector: collect task execution feedb... | 测试依赖 / test_depends |
| 221 | D_AUDITTEST 审计测试套件: test_fl_scheduler_collect_detect.py | → | MetricsCollector: append-only metrics recording... | 测试依赖 / test_depends |
| 222 | D_AUDITTEST 审计测试套件: test_fl_scheduler_collect_detect.py | → | scheduler_collect_detect.py | 测试依赖 / test_depends |
| 223 | D_AUDITTEST 审计测试套件: test_fl_scheduler_health.py | → | scheduler_health.py | 测试依赖 / test_depends |
| 224 | D_AUDITTEST 审计测试套件: test_fl_scheduler_safety.py | → | scheduler_safety.py | 测试依赖 / test_depends |
| 225 | D_AUDITTEST 审计测试套件: test_fl_scope_creep_monitor.py | → | Scope Creep Monitor — v0.15.0 R220 (scope_cree... | 测试依赖 / test_depends |
| 226 | D_AUDITTEST 审计测试套件: test_fl_slo_manager.py | → | slo_manager.py | 测试依赖 / test_depends |
| 227 | D_AUDITTEST 审计测试套件: test_fl_template.py | → | template.py | 测试依赖 / test_depends |
| 228 | D_AUDITTEST 审计测试套件: test_fl_validator.py | → | template.py | 测试依赖 / test_depends |
| 229 | D_AUDITTEST 审计测试套件: test_fl_validator.py | → | validator.py | 测试依赖 / test_depends |
| 230 | D_AUDITTEST 审计测试套件: test_actors_init.py | → | feedback-loop.actors — auto-generated package ... | 测试依赖 / test_depends |
| 231 | D_AUDITTEST 审计测试套件: test_adaptive_param_tuning.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 232 | D_AUDITTEST 审计测试套件: test_alert_desensitization_curve.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 233 | D_AUDITTEST 审计测试套件: test_anomaly_clustering.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 234 | D_AUDITTEST 审计测试套件: test_architectural_sod.py | → | Architectural SoD — v0.15.0 R205 (architectura... | 测试依赖 / test_depends |
| 235 | D_AUDITTEST 审计测试套件: test_automated_rca_postmortem_generator.py | → | Automated RCA Postmortem Generator — v0.38.0 R... | 测试依赖 / test_depends |
| 236 | D_AUDITTEST 审计测试套件: test_autoscale_remediation.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 237 | D_AUDITTEST 审计测试套件: test_backpressure_bridge_root.py | → | FLE -> Pipeline 背压桥接（CTR-BP-001~003） (bac... | 测试依赖 / test_depends |
| 238 | D_AUDITTEST 审计测试套件: test_backpressure_bridge_root.py | → | evolution_engine.py | 测试依赖 / test_depends |
| 239 | D_AUDITTEST 审计测试套件: test_blast_radius_budget.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 240 | D_AUDITTEST 审计测试套件: test_boot_integrity_attestation.py | → | Boot Integrity Attestation — v0.38.0 R487 (boo... | 测试依赖 / test_depends |
| 241 | D_AUDITTEST 审计测试套件: test_cascading_rollback_analyzer.py | → | Cascading Rollback Analyzer — v0.38.0 R482 (ca... | 测试依赖 / test_depends |
| 242 | D_AUDITTEST 审计测试套件: test_cognitive_load.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 243 | D_AUDITTEST 审计测试套件: test_collaborative_learning.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 244 | D_AUDITTEST 审计测试套件: test_collectors.py | → | Calendar Adapter — v0.8.0 R102b (calendar_adap... | 测试依赖 / test_depends |
| 245 | D_AUDITTEST 审计测试套件: test_collectors.py | → | Config Timeline — v0.8.0 R99 (config_timeline.py) | 测试依赖 / test_depends |
| 246 | D_AUDITTEST 审计测试套件: test_collectors.py | → | Data Quality Validator — v0.9.0 R110 (data_qua... | 测试依赖 / test_depends |
| 247 | D_AUDITTEST 审计测试套件: test_collectors.py | → | Financial Stratification — v0.5.0 R50 (financi... | 测试依赖 / test_depends |
| 248 | D_AUDITTEST 审计测试套件: test_collectors.py | → | KB Provenance — v0.10.0 R136 (kb_provenance.py) | 测试依赖 / test_depends |
| 249 | D_AUDITTEST 审计测试套件: test_collectors.py | → | Knowledge Capture — v0.4.0 R30 (knowledge_capt... | 测试依赖 / test_depends |
| 250 | D_AUDITTEST 审计测试套件: test_collectors.py | → | Knowledge Freshness — v0.5.0 R47 (knowledge_fr... | 测试依赖 / test_depends |
| 251 | D_AUDITTEST 审计测试套件: test_collectors.py | → | Knowledge Injection — v0.8.0 R102 (knowledge_i... | 测试依赖 / test_depends |
| 252 | D_AUDITTEST 审计测试套件: test_collectors.py | → | Knowledge Packaging — v0.9.0 R123 (knowledge_p... | 测试依赖 / test_depends |
| 253 | D_AUDITTEST 审计测试套件: test_collectors.py | → | Known-Unknown Registry — v0.16.0 R229 (known_u... | 测试依赖 / test_depends |
| 254 | D_AUDITTEST 审计测试套件: test_collectors.py | → | LLM Cost Accounting — v0.4.0 R35 (llm_cost_acc... | 测试依赖 / test_depends |
| 255 | D_AUDITTEST 审计测试套件: test_collectors.py | → | Market Calendar — v0.5.0 R48 (market_calendar.py) | 测试依赖 / test_depends |
| 256 | D_AUDITTEST 审计测试套件: test_collectors.py | → | Market Event Integrator — v0.14.0 R197 (market... | 测试依赖 / test_depends |
| 257 | D_AUDITTEST 审计测试套件: test_collectors.py | → | Notification Feedback — v0.9.0 R118 (notificat... | 测试依赖 / test_depends |
| 258 | D_AUDITTEST 审计测试套件: test_collectors.py | → | Schema Evolution — v0.9.0 R111 (schema_evoluti... | 测试依赖 / test_depends |
| 259 | D_AUDITTEST 审计测试套件: test_collectors.py | → | Schema Migration — v0.14.0 R190 (schema_migrat... | 测试依赖 / test_depends |
| 260 | D_AUDITTEST 审计测试套件: test_collectors.py | → | Temporal Event Store — v0.3.0 R9 (temporal_eve... | 测试依赖 / test_depends |
| 261 | D_AUDITTEST 审计测试套件: test_collectors.py | → | Token FinOps — v0.12.0 R162 (token_finops.py) | 测试依赖 / test_depends |
| 262 | D_AUDITTEST 审计测试套件: test_collectors.py | → | FeedbackCollector: collect task execution feedb... | 测试依赖 / test_depends |
| 263 | D_AUDITTEST 审计测试套件: test_collectors.py | → | MetricsCollector: append-only metrics recording... | 测试依赖 / test_depends |
| 264 | D_AUDITTEST 审计测试套件: test_confidence_decomposer.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 265 | D_AUDITTEST 审计测试套件: test_config_feedback_loop.py | → | config.py | 测试依赖 / test_depends |
| 266 | D_AUDITTEST 审计测试套件: test_conformal_prediction.py | → | Conformal Prediction — v0.7.0 R74 (conformal_p... | 测试依赖 / test_depends |
| 267 | D_AUDITTEST 审计测试套件: test_counterfactual.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 268 | D_AUDITTEST 审计测试套件: test_deadman_switch.py | → | Deadman Switch — v0.15.0 R212 (deadman_switch.py) | 测试依赖 / test_depends |
| 269 | D_AUDITTEST 审计测试套件: test_diagnosers.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 270 | D_AUDITTEST 审计测试套件: test_diagnosis_engine.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 271 | D_AUDITTEST 审计测试套件: test_digital_twin_sandbox.py | → | Digital Twin Sandbox — v0.6.0 R55 (digital_twi... | 测试依赖 / test_depends |
| 272 | D_AUDITTEST 审计测试套件: test_diminishing_returns_detector.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 273 | D_AUDITTEST 审计测试套件: test_docs_init.py | → | feedback-loop.docs — auto-generated package in... | 测试依赖 / test_depends |
| 274 | D_AUDITTEST 审计测试套件: test_docs_init.py | → | cold_start_manual.py | 测试依赖 / test_depends |
| 275 | D_AUDITTEST 审计测试套件: test_dr_automation.py | → | DR Automation — v0.14.0 R187 (dr_automation.py) | 测试依赖 / test_depends |
| 276 | D_AUDITTEST 审计测试套件: test_dr_resilience_metrics.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 277 | D_AUDITTEST 审计测试套件: test_dry_run_sandbox.py | → | Dry Run Sandbox — v0.3.0 R19 (dry_run_sandbox.py) | 测试依赖 / test_depends |
| 278 | D_AUDITTEST 审计测试套件: test_dynamic_threshold.py | → | Dynamic Threshold — v0.7.0 R71 (dynamic_thresh... | 测试依赖 / test_depends |
| 279 | D_AUDITTEST 审计测试套件: test_e2e_integration_health.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 280 | D_AUDITTEST 审计测试套件: test_ebpf_monitor.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 281 | D_AUDITTEST 审计测试套件: test_ensemble_detector.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 282 | D_AUDITTEST 审计测试套件: test_ensemble_drift.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 283 | D_AUDITTEST 审计测试套件: test_eval_harness_root.py | → | eval_harness.py | 测试依赖 / test_depends |
| 284 | D_AUDITTEST 审计测试套件: test_evolution_engine_root.py | → | evolution_engine.py | 测试依赖 / test_depends |
| 285 | D_AUDITTEST 审计测试套件: test_evolution_init.py | → | Auto Reward — v0.7.0 R76 (auto_reward.py) | 测试依赖 / test_depends |
| 286 | D_AUDITTEST 审计测试套件: test_evolution_init.py | → | Conformal Prediction — v0.7.0 R74 (conformal_p... | 测试依赖 / test_depends |
| 287 | D_AUDITTEST 审计测试套件: test_evolution_init.py | → | Dynamic Threshold — v0.7.0 R71 (dynamic_thresh... | 测试依赖 / test_depends |
| 288 | D_AUDITTEST 审计测试套件: test_evolution_init.py | → | R522: SelfModificationRateLimiter (self_modific... | 测试依赖 / test_depends |
| 289 | D_AUDITTEST 审计测试套件: test_evolution_init.py | → | Self Reflection — v0.7.0 R75 (self_reflection.py) | 测试依赖 / test_depends |
| 290 | D_AUDITTEST 审计测试套件: test_ewc_kb_review.py | → | EWC KB Review — v0.6.0 R51 (ewc_kb_review.py) | 测试依赖 / test_depends |
| 291 | D_AUDITTEST 审计测试套件: test_exceptions_feedback_loop.py | → | exceptions.py | 测试依赖 / test_depends |
| 292 | D_AUDITTEST 审计测试套件: test_failure_replay.py | → | Failure Replay — v0.7.0 R77 (failure_replay.py) | 测试依赖 / test_depends |
| 293 | D_AUDITTEST 审计测试套件: test_federated_protocol.py | → | Federated Protocol — v0.10.0 R129 (federated_p... | 测试依赖 / test_depends |
| 294 | D_AUDITTEST 审计测试套件: test_feedback_collector_root.py | → | FeedbackCollector: collect task execution feedb... | 测试依赖 / test_depends |
| 295 | D_AUDITTEST 审计测试套件: Test suite: feedback-loop core (FeedbackCollect... | → | FeedbackCollector: collect task execution feedb... | 测试依赖 / test_depends |
| 296 | D_AUDITTEST 审计测试套件: Test suite: feedback-loop core (FeedbackCollect... | → | FLE 全链路调度器 —— collect->detect->diagnose... | 测试依赖 / test_depends |
| 297 | D_AUDITTEST 审计测试套件: test_feedback_delay_compensator.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 298 | D_AUDITTEST 审计测试套件: test_feedback_loop.py | → | Feedback Loop Engine — MOD-FEEDBACK_LOOP. (__i... | 测试依赖 / test_depends |
| 299 | D_AUDITTEST 审计测试套件: test_flapping_detector.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 300 | D_AUDITTEST 审计测试套件: test_gamification.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 301 | D_AUDITTEST 审计测试套件: test_global_action_scheduler.py | → | Global Action Scheduler — v0.16.0 R226 (global... | 测试依赖 / test_depends |
| 302 | D_AUDITTEST 审计测试套件: test_golden_test_external.py | → | Golden Test External — v0.15.0 R214 (golden_te... | 测试依赖 / test_depends |
| 303 | D_AUDITTEST 审计测试套件: test_gradual_poisoning_detector.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 304 | D_AUDITTEST 审计测试套件: test_graduated_activation_protocol.py | → | Graduated Activation Protocol — v0.38.0 R485 (... | 测试依赖 / test_depends |
| 305 | D_AUDITTEST 审计测试套件: test_heisenbug_detector.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 306 | D_AUDITTEST 审计测试套件: test_hypernetwork.py | → | HyperNetwork — v0.7.0 R72 (hypernetwork.py) | 测试依赖 / test_depends |
| 307 | D_AUDITTEST 审计测试套件: test_impact_predictor.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 308 | D_AUDITTEST 审计测试套件: test_incident_knowledge_injector.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 309 | D_AUDITTEST 审计测试套件: test_infinite_loop_detector.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 310 | D_AUDITTEST 审计测试套件: test_interrupt_coherence_validator.py | → | R531: InterruptCoherenceValidator (interrupt_co... | 测试依赖 / test_depends |
| 311 | D_AUDITTEST 审计测试套件: test_known_unknown_registry.py | → | Known-Unknown Registry — v0.16.0 R229 (known_u... | 测试依赖 / test_depends |
| 312 | D_AUDITTEST 审计测试套件: test_log_anomaly.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 313 | D_AUDITTEST 审计测试套件: test_maintenance_coordinator.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 314 | D_AUDITTEST 审计测试套件: test_market_calendar.py | → | Market Calendar — v0.5.0 R48 (market_calendar.py) | 测试依赖 / test_depends |
| 315 | D_AUDITTEST 审计测试套件: test_market_event_integrator.py | → | Market Event Integrator — v0.14.0 R197 (market... | 测试依赖 / test_depends |
| 316 | D_AUDITTEST 审计测试套件: test_meta_guard_latency_budget.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 317 | D_AUDITTEST 审计测试套件: test_metric_cardinality_guard.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 318 | D_AUDITTEST 审计测试套件: test_metrics_collector.py | → | MetricsCollector: append-only metrics recording... | 测试依赖 / test_depends |
| 319 | D_AUDITTEST 审计测试套件: test_no_llm_degradation.py | → | No-LLM Degradation Mode — v0.8.0 R94 (no_llm_d... | 测试依赖 / test_depends |
| 320 | D_AUDITTEST 审计测试套件: test_nonstationary_effectiveness.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 321 | D_AUDITTEST 审计测试套件: test_notification_feedback.py | → | Notification Feedback — v0.9.0 R118 (notificat... | 测试依赖 / test_depends |
| 322 | D_AUDITTEST 审计测试套件: test_notification_personalizer.py | → | Notification Personalizer — v0.6.0 R67 (notifi... | 测试依赖 / test_depends |
| 323 | D_AUDITTEST 审计测试套件: test_numerical_stability_guard.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 324 | D_AUDITTEST 审计测试套件: test_online_feature_importance.py | → | Online Feature Importance — v0.7.0 R73 (online... | 测试依赖 / test_depends |
| 325 | D_AUDITTEST 审计测试套件: test_operational_seasonality.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 326 | D_AUDITTEST 审计测试套件: test_oscillation_damping.py | → | Oscillation Damping — v0.37.0 R450 (oscillatio... | 测试依赖 / test_depends |
| 327 | D_AUDITTEST 审计测试套件: test_otel_adapter.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 328 | D_AUDITTEST 审计测试套件: test_placebo_action_detector.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 329 | D_AUDITTEST 审计测试套件: test_positive_feedback_defense.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 330 | D_AUDITTEST 审计测试套件: test_protocols.py | → | protocols.py | 测试依赖 / test_depends |
| 331 | D_AUDITTEST 审计测试套件: test_recovery_time_stats.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 332 | D_AUDITTEST 审计测试套件: test_recursive_diagnosis_trust_evaluator.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 333 | D_AUDITTEST 审计测试套件: test_regulatory_audit.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 334 | D_AUDITTEST 审计测试套件: test_resolution_tracker.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 335 | D_AUDITTEST 审计测试套件: test_retirement_planner.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 336 | D_AUDITTEST 审计测试套件: test_rumor_noise_filter.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 337 | D_AUDITTEST 审计测试套件: test_runbook_executor.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 338 | D_AUDITTEST 审计测试套件: test_scheduler_collect_detect.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 339 | D_AUDITTEST 审计测试套件: test_scheduler_collect_detect.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 340 | D_AUDITTEST 审计测试套件: test_scheduler_collect_detect.py | → | FeedbackCollector: collect task execution feedb... | 测试依赖 / test_depends |
| 341 | D_AUDITTEST 审计测试套件: test_scheduler_collect_detect.py | → | MetricsCollector: append-only metrics recording... | 测试依赖 / test_depends |
| 342 | D_AUDITTEST 审计测试套件: test_scheduler_collect_detect.py | → | scheduler_collect_detect.py | 测试依赖 / test_depends |
| 343 | D_AUDITTEST 审计测试套件: test_scheduler_health.py | → | scheduler_health.py | 测试依赖 / test_depends |
| 344 | D_AUDITTEST 审计测试套件: Integration tests: FeedbackLoopScheduler start/... | → | FLE 全链路调度器 —— collect->detect->diagnose... | 测试依赖 / test_depends |
| 345 | D_AUDITTEST 审计测试套件: test_secondary_alert_channel.py | → | Secondary Alert Channel — v0.37.0 R461 (second... | 测试依赖 / test_depends |
| 346 | D_AUDITTEST 审计测试套件: test_silent_corruption_detector.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 347 | D_AUDITTEST 审计测试套件: test_slo_capacity_metrics.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 348 | D_AUDITTEST 审计测试套件: test_slo_manager_root.py | → | slo_manager.py | 测试依赖 / test_depends |
| 349 | D_AUDITTEST 审计测试套件: test_state_migration_validator.py | → | State Migration Validator — v0.40.0 R497 (stat... | 测试依赖 / test_depends |
| 350 | D_AUDITTEST 审计测试套件: test_stochastic_diagnosis_verifier.py | → | Stochastic Diagnosis Verifier — v0.38.0 R483 (... | 测试依赖 / test_depends |
| 351 | D_AUDITTEST 审计测试套件: test_stochastic_diagnosis_verifier_v2.py | → | Stochastic Diagnosis Verifier — v0.38.0 R483 (... | 测试依赖 / test_depends |
| 352 | D_AUDITTEST 审计测试套件: test_synthetic_anomaly_generator.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 353 | D_AUDITTEST 审计测试套件: test_system_entropy_monitor.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 354 | D_AUDITTEST 审计测试套件: test_teacher_transfer.py | → | Teacher Transfer — v0.6.0 R53 (teacher_transfe... | 测试依赖 / test_depends |
| 355 | D_AUDITTEST 审计测试套件: test_timezone_semantic_reasoner.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 356 | D_AUDITTEST 审计测试套件: test_token_finops.py | → | Token FinOps — v0.12.0 R162 (token_finops.py) | 测试依赖 / test_depends |
| 357 | D_AUDITTEST 审计测试套件: test_training_data_gov.py | → | Training Data Governance — v0.14.0 R191 (train... | 测试依赖 / test_depends |
| 358 | D_AUDITTEST 审计测试套件: test_trend_cycle_separator.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 359 | D_AUDITTEST 审计测试套件: test_validator.py | → | template.py | 测试依赖 / test_depends |
| 360 | D_AUDITTEST 审计测试套件: test_validator.py | → | validator.py | 测试依赖 / test_depends |
| 361 | D_AUDITTEST 审计测试套件: test_vertical_self_assessment.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 362 | D_AUDITTEST 审计测试套件: test_worm_write_integrity.py | → | WORM Write Integrity — v0.15.0 R216 (worm_writ... | 测试依赖 / test_depends |
| 363 | D_AUDITTEST 审计测试套件: test_file_task_mapper_root.py | → | FileTaskMapper — 文件路径 ↔ Task N:N 映射器（... | 测试依赖 / test_depends |
| 364 | D_AUDITTEST 审计测试套件: test_fle_anomaly_detector.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 365 | D_AUDITTEST 审计测试套件: test_fle_anomaly_detector.py | → | FeedbackCollector: collect task execution feedb... | 测试依赖 / test_depends |
| 366 | D_AUDITTEST 审计测试套件: test_fle_anomaly_detector.py | → | MetricsCollector: append-only metrics recording... | 测试依赖 / test_depends |
| 367 | D_AUDITTEST 审计测试套件: test_fle_anomaly_detector.py | → | protocols.py | 测试依赖 / test_depends |
| 368 | D_AUDITTEST 审计测试套件: test_fle_chaos_engineering.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 369 | D_AUDITTEST 审计测试套件: test_fle_config.py | → | config.py | 测试依赖 / test_depends |
| 370 | D_AUDITTEST 审计测试套件: test_fle_dogfood_monitor.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 371 | D_AUDITTEST 审计测试套件: test_fle_exceptions.py | → | exceptions.py | 测试依赖 / test_depends |
| 372 | D_AUDITTEST 审计测试套件: test_fle_feedback_collector.py | → | FeedbackCollector: collect task execution feedb... | 测试依赖 / test_depends |
| 373 | D_AUDITTEST 审计测试套件: test_fle_generator.py | → | generator.py | 测试依赖 / test_depends |
| 374 | D_AUDITTEST 审计测试套件: test_fle_metrics_collector.py | → | MetricsCollector: append-only metrics recording... | 测试依赖 / test_depends |
| 375 | D_AUDITTEST 审计测试套件: test_fle_performance_regression_detector.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 376 | D_AUDITTEST 审计测试套件: test_fle_protocols.py | → | protocols.py | 测试依赖 / test_depends |
| 377 | D_AUDITTEST 审计测试套件: test_fle_regime_detector.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 378 | D_AUDITTEST 审计测试套件: test_fle_self_slo_metrics.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 379 | D_AUDITTEST 审计测试套件: test_fle_template.py | → | template.py | 测试依赖 / test_depends |
| 380 | D_AUDITTEST 审计测试套件: test_fle_upgrade_safety_validator.py | → | R529: FLEUpgradeSafetyValidator (fle_upgrade_sa... | 测试依赖 / test_depends |
| 381 | D_AUDITTEST 审计测试套件: test_fle_validator.py | → | template.py | 测试依赖 / test_depends |
| 382 | D_AUDITTEST 审计测试套件: test_fle_validator.py | → | validator.py | 测试依赖 / test_depends |
| 383 | D_AUDITTEST 审计测试套件: test_ci_cd_pre_scanner.py | → | CI/CD Pre-Scanner — v0.8.0 R107 (ci_cd_pre_sca... | 测试依赖 / test_depends |
| 384 | D_AUDITTEST 审计测试套件: test_concurrent_change_deconfliction.py | → | Concurrent Change Deconfliction — v0.16.0 R230... | 测试依赖 / test_depends |
| 385 | D_AUDITTEST 审计测试套件: test_conflict_arbitration.py | → | Conflict Arbitration — v0.10.0 R130 (conflict_... | 测试依赖 / test_depends |
| 386 | D_AUDITTEST 审计测试套件: test_cve_scanner.py | → | CVE Scanner — v0.8.0 R106 (cve_scanner.py) | 测试依赖 / test_depends |
| 387 | D_AUDITTEST 审计测试套件: test_deployment_suppression.py | → | Deployment Suppression — v0.37.0 R464 (deploym... | 测试依赖 / test_depends |
| 388 | D_AUDITTEST 审计测试套件: test_dynamic_llm_cost_router.py | → | Dynamic LLM Cost Router — v0.8.0 R109 (dynamic... | 测试依赖 / test_depends |
| 389 | D_AUDITTEST 审计测试套件: test_emergency_takeover.py | → | Emergency Takeover — v0.7.0 R88 (emergency_tak... | 测试依赖 / test_depends |
| 390 | D_AUDITTEST 审计测试套件: test_federated_security.py | → | Federated Security — v0.10.0 R131 (federated_s... | 测试依赖 / test_depends |
| 391 | D_AUDITTEST 审计测试套件: test_flag_lifecycle_manager.py | → | Flag Lifecycle Manager — v0.3.0 R11 (flag_life... | 测试依赖 / test_depends |
| 392 | D_AUDITTEST 审计测试套件: test_license_compliance.py | → | License Compliance — v0.14.0 R198 (license_com... | 测试依赖 / test_depends |
| 393 | D_AUDITTEST 审计测试套件: test_merkle_audit_root.py | → | Merkle Audit Root — v0.8.0 R104 (merkle_audit_... | 测试依赖 / test_depends |
| 394 | D_AUDITTEST 审计测试套件: test_meta_performance_gate.py | → | Meta Performance Gate — v0.11.0 R158 (meta_per... | 测试依赖 / test_depends |
| 395 | D_AUDITTEST 审计测试套件: test_parameterized_safety_gate.py | → | GateVerdict — GateVerdict (parameterized_safet... | 测试依赖 / test_depends |
| 396 | D_AUDITTEST 审计测试套件: test_scope_creep_monitor.py | → | Scope Creep Monitor — v0.15.0 R220 (scope_cree... | 测试依赖 / test_depends |
| 397 | D_AUDITTEST 审计测试套件: test_spec_auditor.py | → | protocols.py | 测试依赖 / test_depends |
| 398 | D_AUDITTEST 审计测试套件: test_verdict_engine.py | → | verdict_engine.py | 测试依赖 / test_depends |
| 399 | D_AUDITTEST 审计测试套件: test_guard_cascade_detector.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 400 | D_AUDITTEST 审计测试套件: test_guard_complexity_budget.py | → | R523: GuardComplexityBudget (guard_complexity_b... | 测试依赖 / test_depends |
| 401 | D_AUDITTEST 审计测试套件: test_guard_configuration_drift_monitor.py | → | R521: GuardConfigurationDriftMonitor (guard_con... | 测试依赖 / test_depends |
| 402 | D_AUDITTEST 审计测试套件: test_guard_interaction_topology_mapper.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 403 | D_AUDITTEST 审计测试套件: test_guard_oscillation_detector.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 404 | D_AUDITTEST 审计测试套件: test_guard_self_consistency_auditor.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 405 | D_AUDITTEST 审计测试套件: test_graceful_degradation_planner.py | → | Graceful Degradation Planner — v0.40.0 R496 (g... | 测试依赖 / test_depends |
| 406 | D_AUDITTEST 审计测试套件: DM-202910: MCP boot_hooks 集成测试——验证10进.... | → | __init__.py | 测试依赖 / test_depends |
| 407 | D_AUDITTEST 审计测试套件: DM-202914: MCP boot→FLE→MCP→shutdown全链路E2... | → | __init__.py | 测试依赖 / test_depends |
| 408 | D_AUDITTEST 审计测试套件: DM-202914: MCP boot→FLE→MCP→shutdown全链路E2... | → | AutoRuntimeCore — 三层运行时运营中心（系统大脑... | 测试依赖 / test_depends |
| 409 | D_AUDITTEST 审计测试套件: test_split_brain_quorum.py | → | Split-Brain Quorum — v0.37.0 R451 (split_brain... | 测试依赖 / test_depends |
| 410 | D_AUDITTEST 审计测试套件: test_intent_driven_ops.py | → | Intent-Driven Ops — v0.12.0 R159 (intent_drive... | 测试依赖 / test_depends |
| 411 | D_AUDITTEST 审计测试套件: test_ke_quality.py | → | 知识质量评分契约（CT-KE-QUALITY）——KE完整性+.... | 测试依赖 / test_depends |
| 412 | D_AUDITTEST 审计测试套件: test_knowledge_bus_factor_monitor.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 413 | D_AUDITTEST 审计测试套件: test_knowledge_capture.py | → | Knowledge Capture — v0.4.0 R30 (knowledge_capt... | 测试依赖 / test_depends |
| 414 | D_AUDITTEST 审计测试套件: test_knowledge_distillation.py | → | Knowledge Distillation — v0.6.0 R52 (knowledge... | 测试依赖 / test_depends |
| 415 | D_AUDITTEST 审计测试套件: test_knowledge_freshness.py | → | 知识新鲜度废止管理器（CT-KNOWLEDGE-FRESHNESS）.... | 测试依赖 / test_depends |
| 416 | D_AUDITTEST 审计测试套件: test_knowledge_injection.py | → | Knowledge Injection — v0.8.0 R102 (knowledge_i... | 测试依赖 / test_depends |
| 417 | D_AUDITTEST 审计测试套件: test_knowledge_injection_pre_flight_verifier.py | → | R515: KnowledgeInjectionPreFlightVerifier (know... | 测试依赖 / test_depends |
| 418 | D_AUDITTEST 审计测试套件: test_knowledge_market.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 419 | D_AUDITTEST 审计测试套件: test_knowledge_packaging.py | → | Knowledge Packaging — v0.9.0 R123 (knowledge_p... | 测试依赖 / test_depends |
| 420 | D_AUDITTEST 审计测试套件: test_cross_module_integration_llm_security.py | → | AgentOrchestrator · 多角色 Agent 路由、工具链.... | 测试依赖 / test_depends |
| 421 | D_AUDITTEST 审计测试套件: test_dep_cve_correlator.py | → | Dependency CVE Correlator — v0.14.0 R196 (dep_... | 测试依赖 / test_depends |
| 422 | D_AUDITTEST 审计测试套件: test_llm_cost_accounting.py | → | LLM Cost Accounting — v0.4.0 R35 (llm_cost_acc... | 测试依赖 / test_depends |
| 423 | D_AUDITTEST 审计测试套件: test_llm_cost_router.py | → | LLM Cost Router — v0.3.0 R20 (llm_cost_router.py) | 测试依赖 / test_depends |
| 424 | D_AUDITTEST 审计测试套件: test_llm_provider_integrity.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 425 | D_AUDITTEST 审计测试套件: test_llm_quality_regression.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 426 | D_AUDITTEST 审计测试套件: test_metric_prompt_scanner.py | → | Metric-Prompt Scanner — v0.15.0 R215 (metric_p... | 测试依赖 / test_depends |
| 427 | D_AUDITTEST 审计测试套件: test_orphan_detector.py | → | ModuleOnboardingScanner — 模块接入扫描器 (modu... | 测试依赖 / test_depends |
| 428 | D_AUDITTEST 审计测试套件: test_remote_attestation.py | → | Remote Attestation — v0.15.0 R211 (remote_atte... | 测试依赖 / test_depends |
| 429 | D_AUDITTEST 审计测试套件: test_secret_rotation.py | → | Secret Rotation — v0.14.0 R189 (secret_rotatio... | 测试依赖 / test_depends |
| 430 | D_AUDITTEST 审计测试套件: test_security.py | → | Agent Skill Guard — v0.14.0 R201 (agent_skill_... | 测试依赖 / test_depends |
| 431 | D_AUDITTEST 审计测试套件: test_security.py | → | Dependency CVE Correlator — v0.14.0 R196 (dep_... | 测试依赖 / test_depends |
| 432 | D_AUDITTEST 审计测试套件: test_security.py | → | Metric-Prompt Scanner — v0.15.0 R215 (metric_p... | 测试依赖 / test_depends |
| 433 | D_AUDITTEST 审计测试套件: test_security.py | → | Remote Attestation — v0.15.0 R211 (remote_atte... | 测试依赖 / test_depends |
| 434 | D_AUDITTEST 审计测试套件: test_security.py | → | Secret Rotation — v0.14.0 R189 (secret_rotatio... | 测试依赖 / test_depends |
| 435 | D_AUDITTEST 审计测试套件: test_security.py | → | Wireheading Prevention — v0.37.0 R486 (wirehea... | 测试依赖 / test_depends |
| 436 | D_AUDITTEST 审计测试套件: test_wireheading_prevention.py | → | Wireheading Prevention — v0.37.0 R486 (wirehea... | 测试依赖 / test_depends |
| 437 | D_AUDITTEST 审计测试套件: test_memory_self_check.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 438 | D_AUDITTEST 审计测试套件: test_model_health.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 439 | D_AUDITTEST 审计测试套件: test_model_rotation.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 440 | D_AUDITTEST 审计测试套件: test_model_rotation_v2.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 441 | D_AUDITTEST 审计测试套件: test_model_version_semantic_drift.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 442 | D_AUDITTEST 审计测试套件: test_multi_agent_orchestrator.py | → | Multi-Agent Orchestrator — v0.12.0 R159b (mult... | 测试依赖 / test_depends |
| 443 | D_AUDITTEST 审计测试套件: test_multi_instance_coord.py | → | Multi-Instance Coordinator — v0.14.0 R199 (mul... | 测试依赖 / test_depends |
| 444 | D_AUDITTEST 审计测试套件: test_multi_signal_correlator.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 445 | D_AUDITTEST 审计测试套件: test_trace_causal_bridge.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 446 | D_AUDITTEST 审计测试套件: test_deferred_queue.py | → | DeferredQueue: WAITING -> READY task scheduler.... | 测试依赖 / test_depends |
| 447 | D_AUDITTEST 审计测试套件: test_orchestrator_data_lifecycle.py | → | data_lifecycle.py | 测试依赖 / test_depends |
| 448 | D_AUDITTEST 审计测试套件: test_orchestrator_failure_matcher.py | → | FailurePatternMatcher — 任务失败模式识别与纠正... | 测试依赖 / test_depends |
| 449 | D_AUDITTEST 审计测试套件: test_orchestrator_hallucination_detector.py | → | HallucinationDetector · Chain-of-Verification.... | 测试依赖 / test_depends |
| 450 | D_AUDITTEST 审计测试套件: test_orchestrator_model_registry.py | → | model_registry.py | 测试依赖 / test_depends |
| 451 | D_AUDITTEST 审计测试套件: test_orchestrator_rollback_manager.py | → | RollbackManager — 仅调试用途的 DB-state 快照，... | 测试依赖 / test_depends |
| 452 | D_AUDITTEST 审计测试套件: test_orchestrator_task_queue.py | → | ActiveTaskQueue — 后台任务轮询与自动分发 (task... | 测试依赖 / test_depends |
| 453 | D_AUDITTEST 审计测试套件: test_orchestrator_trigger_router.py | → | TriggerRouter — RI-03 触发路由器（M3 跨模块触.... | 测试依赖 / test_depends |
| 454 | D_AUDITTEST 审计测试套件: test_orchestrator_wave_generator.py | → | WaveGenerator — 根据 Task 依赖图生成执行 Wave.... | 测试依赖 / test_depends |
| 455 | D_AUDITTEST 审计测试套件: test_path_index.py | → | path_index.py | 测试依赖 / test_depends |
| 456 | D_AUDITTEST 审计测试套件: test_phase_executor_root.py | → | Phase 执行引擎（Phase Executor） (phase_executo... | 测试依赖 / test_depends |
| 457 | D_AUDITTEST 审计测试套件: test_integration_test_pipeline.py | → | E2E Integration Test Pipeline — TASK-MOD-FEEDB... | 测试依赖 / test_depends |
| 458 | D_AUDITTEST 审计测试套件: test_pipeline_agent_bridge.py | → | AgentOrchestrator · 多角色 Agent 路由、工具链.... | 测试依赖 / test_depends |
| 459 | D_AUDITTEST 审计测试套件: test_prompt_factory_governance.py | → | Prompt Factory Governance — v0.16.0 R224 (prom... | 测试依赖 / test_depends |
| 460 | D_AUDITTEST 审计测试套件: test_prompt_fingerprint.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 461 | D_AUDITTEST 审计测试套件: test_prompt_optimization_regression_detector.py | → | R514: PromptOptimizationRegressionDetector (pro... | 测试依赖 / test_depends |
| 462 | D_AUDITTEST 审计测试套件: test_prompt_sanitizer.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 463 | D_AUDITTEST 审计测试套件: test_prompt_self_optimization_loop.py | → | R502: PromptSelfOptimizationLoop (prompt_self_o... | 测试依赖 / test_depends |
| 464 | D_AUDITTEST 审计测试套件: test_prompt_version.py | → | AI Prompt 版本控制（CT-PROMPT-VERSION）——prom... | 测试依赖 / test_depends |
| 465 | D_AUDITTEST 审计测试套件: test_resource_optimization.py | → | resource_optimization.py - MAPE-K autonomic res... | 测试依赖 / test_depends |
| 466 | D_AUDITTEST 审计测试套件: test_resource_starvation_aware.py | → | Resource Starvation Aware — v0.15.0 R209 (reso... | 测试依赖 / test_depends |
| 467 | D_AUDITTEST 审计测试套件: test_blast_radius_detector.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 468 | D_AUDITTEST 审计测试套件: test_risk_registry_root.py | → | risk_registry.py | 测试依赖 / test_depends |
| 469 | D_AUDITTEST 审计测试套件: test_rollback_integrity.py | → | Rollback Integrity — v0.3.0 R18b (rollback_int... | 测试依赖 / test_depends |
| 470 | D_AUDITTEST 审计测试套件: test_attack_simulator.py | → | Attack Simulator — v0.6.0 R57 (attack_simulato... | 测试依赖 / test_depends |
| 471 | D_AUDITTEST 审计测试套件: test_safety_gate_l1_l27.py | → | Safety Gates L1-L27 — Unified Pipeline (MOD-FE... | 测试依赖 / test_depends |
| 472 | D_AUDITTEST 审计测试套件: test_scheduler_safety.py | → | scheduler_safety.py | 测试依赖 / test_depends |
| 473 | D_AUDITTEST 审计测试套件: test_self_api_throttle_defense.py | → | Self API Throttle Defense — v0.39.0 R491 (self... | 测试依赖 / test_depends |
| 474 | D_AUDITTEST 审计测试套件: test_self_audit.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 475 | D_AUDITTEST 审计测试套件: test_self_benchmark.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 476 | D_AUDITTEST 审计测试套件: test_self_bottleneck_detector.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 477 | D_AUDITTEST 审计测试套件: test_self_diagnosis.py | → | self_diagnosis.py — 自我诊断 (DD120, TASK-020)... | 测试依赖 / test_depends |
| 478 | D_AUDITTEST 审计测试套件: test_self_diagnosis_data_leak_detector.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 479 | D_AUDITTEST 审计测试套件: test_self_ha.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 480 | D_AUDITTEST 审计测试套件: test_self_health_monitor.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 481 | D_AUDITTEST 审计测试套件: test_self_llm_observability.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 482 | D_AUDITTEST 审计测试套件: test_self_modification_audit.py | → | Self-Modification Audit — v0.15.0 R218 (self_m... | 测试依赖 / test_depends |
| 483 | D_AUDITTEST 审计测试套件: test_self_modification_rate_limiter.py | → | R522: SelfModificationRateLimiter (self_modific... | 测试依赖 / test_depends |
| 484 | D_AUDITTEST 审计测试套件: test_self_reflection.py | → | Self Reflection — v0.7.0 R75 (self_reflection.py) | 测试依赖 / test_depends |
| 485 | D_AUDITTEST 审计测试套件: test_self_upgrade_canary.py | → | Self Upgrade Canary — v0.14.0 R194 (self_upgra... | 测试依赖 / test_depends |
| 486 | D_AUDITTEST 审计测试套件: test_semantic_intent_preservation_guard.py | → | R505: SemanticIntentPreservationGuard (semantic... | 测试依赖 / test_depends |
| 487 | D_AUDITTEST 审计测试套件: test_session_conflict.py | → | Session 冲突预防契约（CT-SESSION-CONFLICT）——... | 测试依赖 / test_depends |
| 488 | D_AUDITTEST 审计测试套件: test_session_learner.py | → | session_learner.py — 在线学习 (DD114, TASK-020... | 测试依赖 / test_depends |
| 489 | D_AUDITTEST 审计测试套件: test_session_manager.py | → | SessionManager — AI Agent 会话生命周期管理（CT... | 测试依赖 / test_depends |
| 490 | D_AUDITTEST 审计测试套件: test_task_gate.py | → | TaskGate --- 任务门控 (task_gate.py) | 测试依赖 / test_depends |
| 491 | D_AUDITTEST 审计测试套件: test_temporal_coherence_of_self_model.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 492 | D_AUDITTEST 审计测试套件: test_temporal_event_store.py | → | Temporal Event Store — v0.3.0 R9 (temporal_eve... | 测试依赖 / test_depends |
| 493 | D_AUDITTEST 审计测试套件: test_temporal_integrity_guard.py | → | feedback-loop.diagnosers — GOV-DOC-018: 71个叶... | 测试依赖 / test_depends |
| 494 | D_AUDITTEST 审计测试套件: test_temporal_pattern.py | → | feedback-loop.detectors — GOV-DOC-018: 60个叶.... | 测试依赖 / test_depends |
| 495 | D_AUDITTEST 审计测试套件: miniqmt_broker 正式测试（原 scripts/tests/ 临时... | → | order.py | 测试依赖 / test_depends |
| 496 | D_AUDITTEST 审计测试套件: test_admission_controller.py | → | admission_controller.py | 测试依赖 / test_depends |
| 497 | D_AUDITTEST 审计测试套件: test_admission_controller.py | → | verdict_engine.py | 测试依赖 / test_depends |
| 498 | D_AUDITTEST 审计测试套件: test_batch_orchestrator.py | → | BatchOrchestrator — 多 Worker 批量任务协调器（... | 测试依赖 / test_depends |
| 499 | D_AUDITTEST 审计测试套件: test_benchmark_runner.py | → | benchmark_runner.py | 测试依赖 / test_depends |
| 500 | D_AUDITTEST 审计测试套件: test_blind_spot_closure.py | → | blind_spot_closure.py | 测试依赖 / test_depends |
| 501 | D_AUDITTEST 审计测试套件: test_boot_cron_jobs.py | → | __init__.py | 测试依赖 / test_depends |
| 502 | D_AUDITTEST 审计测试套件: test_boot_hooks.py | → | boot_hooks.py | 测试依赖 / test_depends |
| 503 | D_AUDITTEST 审计测试套件: test_bulkhead_manager.py | → | bulkhead_manager.py | 测试依赖 / test_depends |
| 504 | D_AUDITTEST 审计测试套件: test_construction_guide.py | → | 施工指南引擎（Construction Guide） (constructio... | 测试依赖 / test_depends |
| 505 | D_AUDITTEST 审计测试套件: test_degrade_cascade.py | → | degrade_cascade.py | 测试依赖 / test_depends |
| 506 | D_AUDITTEST 审计测试套件: test_design_decisions_root.py | → | design_decisions.py | 测试依赖 / test_depends |
| 507 | D_AUDITTEST 审计测试套件: test_disk_guard.py | → | disk_guard.py | 测试依赖 / test_depends |
| 508 | D_AUDITTEST 审计测试套件: test_dlq_manager_root.py | → | DLQ 管理器（Dead Letter Queue Manager — CT-DLQ... | 测试依赖 / test_depends |
| 509 | D_AUDITTEST 审计测试套件: test_dream_cycle.py | → | DreamCycle — 知识固化引擎 (dream_cycle.py) | 测试依赖 / test_depends |
| 510 | D_AUDITTEST 审计测试套件: F14 管线编排/反馈环 — 红蓝对抗端到端极端测试 (... | → | Error Budget 状态机——monthly budget + burn_ra... | 测试依赖 / test_depends |
| 511 | D_AUDITTEST 审计测试套件: F14 管线编排/反馈环 — 红蓝对抗端到端极端测试 (... | → | FLE 全链路调度器 —— collect->detect->diagnose... | 测试依赖 / test_depends |
| 512 | D_AUDITTEST 审计测试套件: F1 自动驾驶/运行时大脑 — 红蓝对抗端到端极端测... | → | DreamCycle — 知识固化引擎 (dream_cycle.py) | 测试依赖 / test_depends |
| 513 | D_AUDITTEST 审计测试套件: F1 自动驾驶/运行时大脑 — 红蓝对抗端到端极端测... | → | HealthMonitor — 健康监控 + 自愈 (health_monito... | 测试依赖 / test_depends |
| 514 | D_AUDITTEST 审计测试套件: F1 自动驾驶/运行时大脑 — 红蓝对抗端到端极端测... | → | WorkDAG + WorkItem — 工作编排数据模型 (work_da... | 测试依赖 / test_depends |
| 515 | D_AUDITTEST 审计测试套件: F1 自动驾驶/运行时大脑 — 红蓝对抗端到端极端测... | → | work_orchestrator.py | 测试依赖 / test_depends |
| 516 | D_AUDITTEST 审计测试套件: test_fault_types.py | → | Fault type registry and preset templates for ch... | 测试依赖 / test_depends |
| 517 | D_AUDITTEST 审计测试套件: test_feature_flag.py | → | FeatureFlag 管理器（CT-FEATUREFLAG-001）——CT-... | 测试依赖 / test_depends |
| 518 | D_AUDITTEST 审计测试套件: test_finalizer.py | → | Finalizer — 优雅清理器 (finalizer.py) | 测试依赖 / test_depends |
| 519 | D_AUDITTEST 审计测试套件: test_finding_bridge.py | → | CT-ORC-SCRIPT-001 运行时桥接 (finding_bridge.py) | 测试依赖 / test_depends |
| 520 | D_AUDITTEST 审计测试套件: test_gpu_consensus_scheduler.py | → | gpu_consensus_scheduler.py | 测试依赖 / test_depends |
| 521 | D_AUDITTEST 审计测试套件: test_gpu_consensus_scheduler.py | → | verdict_engine.py | 测试依赖 / test_depends |
| 522 | D_AUDITTEST 审计测试套件: test_housekeeping.py | → | 文件卫生保洁管理器（CT-HOUSEKEEPING）——临时文... | 测试依赖 / test_depends |
| 523 | D_AUDITTEST 审计测试套件: IdeHealthDaemon 测试. (test_ide_health_daemon.py) | → | ide_health_daemon.py — TRAE IDE 幽灵窗口守护线... | 测试依赖 / test_depends |
| 524 | D_AUDITTEST 审计测试套件: test_incident_postmortem.py | → | 事件复盘管理器（CT-INCIDENT）——incident记录+t... | 测试依赖 / test_depends |
| 525 | D_AUDITTEST 审计测试套件: test_integration_registry.py | → | IntegrationRegistry — 集成注册表 (integration_... | 测试依赖 / test_depends |
| 526 | D_AUDITTEST 审计测试套件: test_lean_scanner.py | → | 死代码/孤儿文件/僵尸引用三扫描（CT-LEAN）——三... | 测试依赖 / test_depends |
| 527 | D_AUDITTEST 审计测试套件: test_lifecycle_manager.py | → | CapabilityRegistry — 能力注册中心 (capability_... | 测试依赖 / test_depends |
| 528 | D_AUDITTEST 审计测试套件: test_lifecycle_manager.py | → | DreamCycle — 知识固化引擎 (dream_cycle.py) | 测试依赖 / test_depends |
| 529 | D_AUDITTEST 审计测试套件: test_lifecycle_manager.py | → | Feedback Loop Engine — MOD-FEEDBACK_LOOP. (__i... | 测试依赖 / test_depends |
| 530 | D_AUDITTEST 审计测试套件: test_lifecycle_manager.py | → | Finalizer — 优雅清理器 (finalizer.py) | 测试依赖 / test_depends |
| 531 | D_AUDITTEST 审计测试套件: test_lifecycle_manager.py | → | HealthMonitor — 健康监控 + 自愈 (health_monito... | 测试依赖 / test_depends |
| 532 | D_AUDITTEST 审计测试套件: test_lifecycle_manager.py | → | IntegrationRegistry — 集成注册表 (integration_... | 测试依赖 / test_depends |
| 533 | D_AUDITTEST 审计测试套件: test_lifecycle_manager.py | → | lifecycle_manager.py | 测试依赖 / test_depends |
| 534 | D_AUDITTEST 审计测试套件: test_lifecycle_manager.py | → | NightShiftQueue — 夜班登记表持久化 (night_shif... | 测试依赖 / test_depends |
| 535 | D_AUDITTEST 审计测试套件: test_lifecycle_manager.py | → | StopGate — 质量闸门 (stop_gate.py) | 测试依赖 / test_depends |
| 536 | D_AUDITTEST 审计测试套件: test_lifecycle_manager.py | → | work_orchestrator.py | 测试依赖 / test_depends |
| 537 | D_AUDITTEST 审计测试套件: test_module_onboarding_scanner.py | → | CapabilityCard — 能力卡片数据模型 (capability_... | 测试依赖 / test_depends |
| 538 | D_AUDITTEST 审计测试套件: test_module_onboarding_scanner.py | → | CapabilityRegistry — 能力注册中心 (capability_... | 测试依赖 / test_depends |
| 539 | D_AUDITTEST 审计测试套件: test_module_onboarding_scanner.py | → | ModuleOnboardingScanner — 模块接入扫描器 (modu... | 测试依赖 / test_depends |
| 540 | D_AUDITTEST 审计测试套件: test_network_partition.py | → | 网络分区容忍（CT-NETWORK-PARTITION）——CAP定理... | 测试依赖 / test_depends |
| 541 | D_AUDITTEST 审计测试套件: test_night_shift_queue.py | → | NightShiftQueue — 夜班登记表持久化 (night_shif... | 测试依赖 / test_depends |
| 542 | D_AUDITTEST 审计测试套件: test_protection_index.py | → | protection_index.py | 测试依赖 / test_depends |
| 543 | D_AUDITTEST 审计测试套件: test_protection_index.py | → | verdict_engine.py | 测试依赖 / test_depends |
| 544 | D_AUDITTEST 审计测试套件: test_reconciliation_loop.py | → | reconciliation_loop.py | 测试依赖 / test_depends |
| 545 | D_AUDITTEST 审计测试套件: test_rolling_upgrade.py | → | 零停机滚动升级（CT-DEPLOY）——graceful shutdow... | 测试依赖 / test_depends |
| 546 | D_AUDITTEST 审计测试套件: test_runtime_config.py | → | runtime_config.py | 测试依赖 / test_depends |
| 547 | D_AUDITTEST 审计测试套件: test_schema_migration.py | → | 数据库 Schema 演化契约（CT-SCHEMA-MIGRATE）——... | 测试依赖 / test_depends |
| 548 | D_AUDITTEST 审计测试套件: test_stability_guard.py | → | API 稳定性守护（CT-STABILITY）——public API签.... | 测试依赖 / test_depends |
| 549 | D_AUDITTEST 审计测试套件: test_staging_area.py | → | StagingArea — 多AI并发草稿写入+提交+冲突检测模... | 测试依赖 / test_depends |
| 550 | D_AUDITTEST 审计测试套件: test_startup_sequencer.py | → | startup_sequencer.py | 测试依赖 / test_depends |
| 551 | D_AUDITTEST 审计测试套件: test_state_propagation_root.py | → | 全局状态传播链（State Propagation Chain） (stat... | 测试依赖 / test_depends |
| 552 | D_AUDITTEST 审计测试套件: test_state_synchronizer_root.py | → | StateSynchronizer — 同步 SQLite 状态与文件系统... | 测试依赖 / test_depends |
| 553 | D_AUDITTEST 审计测试套件: test_status_dashboard.py | → | HealthMonitor — 健康监控 + 自愈 (health_monito... | 测试依赖 / test_depends |
| 554 | D_AUDITTEST 审计测试套件: test_status_dashboard.py | → | StatusDashboard — 实时状态面板 (status_dashboa... | 测试依赖 / test_depends |
| 555 | D_AUDITTEST 审计测试套件: test_stop_gate.py | → | StopGate — 质量闸门 (stop_gate.py) | 测试依赖 / test_depends |
| 556 | D_AUDITTEST 审计测试套件: test_system_transfer.py | → | 系统移交恢复（CT-TRANSFER）——系统Owner变更+配... | 测试依赖 / test_depends |
| 557 | D_AUDITTEST 审计测试套件: test_teardown_manager.py | → | teardown_manager.py | 测试依赖 / test_depends |
| 558 | D_AUDITTEST 审计测试套件: test_version_manifest.py | → | version_manifest.py | 测试依赖 / test_depends |
| 559 | D_AUDITTEST 审计测试套件: test_work_dag.py | → | WorkDAG + WorkItem — 工作编排数据模型 (work_da... | 测试依赖 / test_depends |
| 560 | D_AUDITTEST 审计测试套件: test_work_orchestrator.py | → | WorkDAG + WorkItem — 工作编排数据模型 (work_da... | 测试依赖 / test_depends |
| 561 | D_AUDITTEST 审计测试套件: test_work_orchestrator.py | → | work_orchestrator.py | 测试依赖 / test_depends |
| 562 | D_EX_CORE 执行核心: MiniQMT 实盘券商适配器（对接 xttrader，A股实盘.... | → | fill.py | 导入依赖 / import_depends |
| 563 | D_EX_CORE 执行核心: MiniQMT 实盘券商适配器（对接 xttrader，A股实盘.... | → | order.py | 导入依赖 / import_depends |
| 564 | D_EX_CORE 执行核心: MiniQMT 实盘券商适配器（对接 xttrader，A股实盘.... | → | position.py | 导入依赖 / import_depends |
| 565 | D_EX_CORE 执行核心: D_EXECUTION_CORE — Execution Engine (execution... | → | order.py | 导入依赖 / import_depends |
| 566 | D_EX_CORE 执行核心: D_EXECUTION_CORE — Order Manager (order_manage... | → | fill.py | 导入依赖 / import_depends |
| 567 | D_EX_CORE 执行核心: D_EXECUTION_CORE — Order Manager (order_manage... | → | order.py | 导入依赖 / import_depends |
| 568 | D_FRONTEND 前端: fitness_functions · Fitness Functions 仪表盘组... | → | fitness_functions.py | 导入依赖 / import_depends |
| 569 | D_FRONTEND 前端: trade_panel · 实盘交易面板组件（v3.0.0 Panel+H... | → | order.py | 导入依赖 / import_depends |
| 570 | D_FUNDAMENTAL_SIGNAL 基本面信号: D_SIGNAL Signal Combiner (__init__.py) | → | synthesized_signal.py | 导入依赖 / import_depends |
| 571 | D_FUNDAMENTAL_SIGNAL 基本面信号: D_SIGNAL — Signal Generation Layer (aggregator... | → | capital_allocation_result.py | 导入依赖 / import_depends |
| 572 | D_FUNDAMENTAL_SIGNAL 基本面信号: D_SIGNAL — Signal Generation Layer (aggregator... | → | factor_signal.py | 导入依赖 / import_depends |
| 573 | D_FUNDAMENTAL_SIGNAL 基本面信号: D_SIGNAL — Signal Generation Layer (aggregator... | → | signal_degradation_warning.py | 导入依赖 / import_depends |
| 574 | D_FUNDAMENTAL_SIGNAL 基本面信号: D_SIGNAL — Signal Generation Layer (aggregator... | → | synthesized_signal.py | 导入依赖 / import_depends |
| 575 | D_FUNDAMENTAL_SIGNAL 基本面信号: D_SIGNAL — Default Signal Aggregator (default_... | → | factor_signal.py | 导入依赖 / import_depends |
| 576 | D_FUNDAMENTAL_SIGNAL 基本面信号: D_SIGNAL — Default Signal Aggregator (default_... | → | synthesized_signal.py | 导入依赖 / import_depends |
| 577 | D_FUNDAMENTAL_SIGNAL 基本面信号: AlphaSignalPipeline D_FACTOR->D_SIGNAL跨层集成... | → | factor_signal.py | 导入依赖 / import_depends |
| 578 | D_FUNDAMENTAL_SIGNAL 基本面信号: AlphaSignalPipeline D_FACTOR->D_SIGNAL跨层集成... | → | synthesized_signal.py | 导入依赖 / import_depends |
| 579 | D_FUNDAMENTAL_SIGNAL 基本面信号: D_SIGNAL — Capital Allocator（兼容导出） (capi... | → | capital_allocation_result.py | 导入依赖 / import_depends |
| 580 | D_FUNDAMENTAL_SIGNAL 基本面信号: D_SIGNAL — Default Capital Allocator (default_... | → | capital_allocation_result.py | 导入依赖 / import_depends |
| 581 | D_FUNDAMENTAL_SIGNAL 基本面信号: D_SIGNAL — Default Capital Allocator (default_... | → | synthesized_signal.py | 导入依赖 / import_depends |
| 582 | D_FUNDAMENTAL_SIGNAL 基本面信号: D_SIGNAL — Signal Synthesizer (signal_synthesi... | → | factor_signal.py | 导入依赖 / import_depends |
| 583 | D_FUNDAMENTAL_SIGNAL 基本面信号: D_SIGNAL — Signal Synthesizer (signal_synthesi... | → | synthesized_signal.py | 导入依赖 / import_depends |
| 584 | D_GOVERNANCE 生命周期管理: start_brain.py — ZephyrAlpha 系统大脑一键启动 ... | → | AutoRuntimeCore — 三层运行时运营中心（系统大脑... | 导入依赖 / import_depends |
| 585 | D_GOVERNANCE 生命周期管理: start_brain.py — ZephyrAlpha 系统大脑一键启动 ... | → | AutoTaskGenerator — 自动任务生成器 (auto_task_... | 导入依赖 / import_depends |
| 586 | D_GOVERNANCE 生命周期管理: IDE健康守护进程CLI包装器 (ide_health_service.py) | → | ide_health_daemon.py — TRAE IDE 幽灵窗口守护线... | 导入依赖 / import_depends |
| 587 | D_GOVERNANCE 生命周期管理: D_EXECUTION_CORE — Simulation Broker Adapter (... | → | fill.py | 导入依赖 / import_depends |
| 588 | D_GOVERNANCE 生命周期管理: D_EXECUTION_CORE — Simulation Broker Adapter (... | → | order.py | 导入依赖 / import_depends |
| 589 | D_GOVERNANCE 生命周期管理: D_EXECUTION_CORE — Simulation Broker Adapter (... | → | position.py | 导入依赖 / import_depends |
| 590 | D_GOVERNANCE 生命周期管理: feedback_bridge.py | → | Feedback Loop Engine — MOD-FEEDBACK_LOOP. (__i... | 导入依赖 / import_depends |
| 591 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | zephyr.trading.trading_contracts — trading-dom... | 导入依赖 / import_depends |
| 592 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | capital_allocation_result.py | 导入依赖 / import_depends |
| 593 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | execution_rejection_error.py | 导入依赖 / import_depends |
| 594 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | execution_report.py | 导入依赖 / import_depends |
| 595 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | fill.py | 导入依赖 / import_depends |
| 596 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | model_serving_request.py | 导入依赖 / import_depends |
| 597 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | order.py | 导入依赖 / import_depends |
| 598 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | position.py | 导入依赖 / import_depends |
| 599 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | factor_monitor_report.py | 导入依赖 / import_depends |
| 600 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | factor_signal.py | 导入依赖 / import_depends |
| 601 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | instrument.py | 导入依赖 / import_depends |
| 602 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | macro_factor_signal.py | 导入依赖 / import_depends |
| 603 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | market_data.py | 导入依赖 / import_depends |
| 604 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | signal_degradation_warning.py | 导入依赖 / import_depends |
| 605 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | synthesized_signal.py | 导入依赖 / import_depends |
| 606 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | risk_dashboard_snapshot.py | 导入依赖 / import_depends |
| 607 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | risk_limit_violation_error.py | 导入依赖 / import_depends |
| 608 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | risk_limits.py | 导入依赖 / import_depends |
| 609 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | risk_metrics.py | 导入依赖 / import_depends |
| 610 | D_GOVERNANCE 生命周期管理: zephyr.trading.trading_contracts — trading-dom... | → | risk_validator_protocol.py | 导入依赖 / import_depends |
| 611 | D_GOVERNANCE 生命周期管理: D_EXECUTION_CORE — BrokerInterface (broker_int... | → | fill.py | 导入依赖 / import_depends |
| 612 | D_GOVERNANCE 生命周期管理: D_EXECUTION_CORE — BrokerInterface (broker_int... | → | order.py | 导入依赖 / import_depends |
| 613 | D_GOVERNANCE 生命周期管理: D_EXECUTION_CORE — BrokerInterface (broker_int... | → | position.py | 导入依赖 / import_depends |
| 614 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | capital_allocation_result.py | 导入依赖 / import_depends |
| 615 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | execution_rejection_error.py | 导入依赖 / import_depends |
| 616 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | execution_report.py | 导入依赖 / import_depends |
| 617 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | fill.py | 导入依赖 / import_depends |
| 618 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | model_serving_request.py | 导入依赖 / import_depends |
| 619 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | order.py | 导入依赖 / import_depends |
| 620 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | position.py | 导入依赖 / import_depends |
| 621 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | trading-contracts/factories.py — 交易域数据契.... | 导入依赖 / import_depends |
| 622 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | factor_monitor_report.py | 导入依赖 / import_depends |
| 623 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | factor_signal.py | 导入依赖 / import_depends |
| 624 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | instrument.py | 导入依赖 / import_depends |
| 625 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | macro_factor_signal.py | 导入依赖 / import_depends |
| 626 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | market_data.py | 导入依赖 / import_depends |
| 627 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | signal_degradation_warning.py | 导入依赖 / import_depends |
| 628 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | synthesized_signal.py | 导入依赖 / import_depends |
| 629 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | compliance_rule.py | 导入依赖 / import_depends |
| 630 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | risk_dashboard_snapshot.py | 导入依赖 / import_depends |
| 631 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | risk_limit_violation_error.py | 导入依赖 / import_depends |
| 632 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | risk_limits.py | 导入依赖 / import_depends |
| 633 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | risk_metrics.py | 导入依赖 / import_depends |
| 634 | D_GOVERNANCE 生命周期管理: Re-export shim — 真源已合并至 zephyr.trading.t... | → | risk_validator_protocol.py | 导入依赖 / import_depends |
| 635 | D_GOV_SCRIPTS 脚本治理: check_handoff_manifests.py — AI Session Handof... | → | 集成契约注册表（Contract Registry） (contract_r... | 导入依赖 / import_depends |
| 636 | D_GOV_SCRIPTS 脚本治理: [INVARIANTS] 使用测试数据库副本，不污染生产数据... | → | StagingArea — 多AI并发草稿写入+提交+冲突检测模... | 导入依赖 / import_depends |
| 637 | D_INFRA_RUNTIME 运行时集成: ZephyrAlpha 核心包索引 + 模块懒加载器 (M-04) (_... | → | Secret Rotation — v0.14.0 R189 (secret_rotatio... | 导入依赖 / import_depends |
| 638 | D_INTELLIGENCE 上下文管理: D_ML_TRAIN — Default Inference Engine (default... | → | model_serving_request.py | 导入依赖 / import_depends |
| 639 | D_ML_TRAIN 训练: D_ML_TRAIN — Default Inference Engine (default... | → | model_serving_request.py | 导入依赖 / import_depends |
| 640 | D_ML_TRAIN 训练: D_ML_TRAIN — ML Inference Base (inference_base.py) | → | model_serving_request.py | 导入依赖 / import_depends |
| 641 | D_PF_CORE 组合核心: D_PORTFOLIO_CORE — Default Equity Long-Only St... | → | order.py | 导入依赖 / import_depends |
| 642 | D_REPORTING 报告: D_REPORTING — Post-Trade Analytics Layer (anal... | → | execution_report.py | 导入依赖 / import_depends |
| 643 | D_REPORTING 报告: D_REPORTING — Post-Trade Analytics Layer (anal... | → | fill.py | 导入依赖 / import_depends |
| 644 | D_REPORTING 报告: D_REPORTING — Post-Trade Analytics Layer (anal... | → | order.py | 导入依赖 / import_depends |
| 645 | D_REPORTING 报告: D_REPORTING — Default TCA Engine (default_tca_... | → | execution_report.py | 导入依赖 / import_depends |
| 646 | D_REPORTING 报告: D_REPORTING — Default TCA Engine (default_tca_... | → | fill.py | 导入依赖 / import_depends |
| 647 | D_REPORTING 报告: D_REPORTING — Default TCA Engine (default_tca_... | → | order.py | 导入依赖 / import_depends |
| 648 | D_RISK 风控: ZephyrAlpha — D_RISK Risk Management Layer — ... | → | risk_dashboard_snapshot.py | 导入依赖 / import_depends |
| 649 | D_RISK 风控: ZephyrAlpha — D_RISK Risk Management Layer — ... | → | risk_limit_violation_error.py | 导入依赖 / import_depends |
| 650 | D_RISK 风控: ZephyrAlpha — D_RISK Risk Management Layer — ... | → | risk_metrics.py | 导入依赖 / import_depends |
| 651 | D_SECURITY 对抗验证: feedback_bridge.py | → | Feedback Loop Engine — MOD-FEEDBACK_LOOP. (__i... | 导入依赖 / import_depends |
| 652 | D_SECURITY 对抗验证: [INVARIANTS] 蓝图 §4 文件清单与代码双向对齐 (o... | → | CapabilityRegistry — 能力注册中心 (capability_... | 导入依赖 / import_depends |
| 653 | D_SECURITY 对抗验证: [INVARIANTS] 蓝图 §4 文件清单与代码双向对齐 (o... | → | ModuleOnboardingScanner — 模块接入扫描器 (modu... | 导入依赖 / import_depends |
| 654 | D_SHARED 共享服务: order.py | → | order.py | 导入依赖 / import_depends |
| 655 | D_SHARED 共享服务: secrets.py —— Secrets 管理抽象（Phase 7 新增 ... | → | Secret Rotation — v0.14.0 R189 (secret_rotatio... | 导入依赖 / import_depends |
| 656 | D_SIGQC 信号质量控制: D_SIGQC — Signal Quality Degradation Monitor B... | → | signal_degradation_warning.py | 导入依赖 / import_depends |
| 657 | D_SIGQC 信号质量控制: D_SIGQC — Signal Quality Degradation Monitor B... | → | synthesized_signal.py | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 23 个外部域直接连接（出边 202 条 + 入边 735 条 = 937 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_TRADING -->|99条 导入依赖 / import_depends| D_SHARED
    ...
    D_GOVERNANCE -->|51条 导入依赖 / import_depends| D_TRADING
    D_FUNDAMENTAL_SIGNAL -->|14条 导入依赖 / import_depends| D_TRADING
    D_REPORTING -->|6条 导入依赖 / import_depends| D_TRADING
    ...
    D_SHARED -->|2条 导入依赖 / import_depends| D_TRADING
