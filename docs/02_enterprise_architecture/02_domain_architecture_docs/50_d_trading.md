...
| 跨域入边 | 639 | Cross-domain Incoming | 639 |
...
graph TD
    ...
    src_zephyr_trading_orchestrator_fault_tolerance_init_py -.->|config_depends / config_depends| src_zephyr_trading_orchestrator_fault_tolerance_degrade_cascade_py
...
graph TD
    ...
    src_zephyr_trading_orchestrator_lifecycle_init_py -.->|config_depends / config_depends| src_zephyr_trading_orchestrator_lifecycle_session_conflict_py
...
graph TD
    ...
    class D_SHARED,D_INTEGRATION,D_GOVERNANCE,D_INFRA_RUNTIME,D_INTELLIGENCE,D_GOV_ENFORCEMENT,D_FRONTEND external_prod
    class D_EX_CORE external_design
```
...
| 585 | D_GOVERNANCE 生命周期管理: blueprint.md | → | feedback-loop.evolution — auto-generated packa... | runtime / runtime |
...
> 本域与 21 个外部域直接连接（出边 203 条 + 入边 639 条 = 842 条）。只显示直接连接的域，不展开具体节点。
