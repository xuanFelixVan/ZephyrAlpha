...
graph TD
    ...
    src_zephyr_trading_orchestrator_lifecycle_init_py -.->|config_depends / config_depends| src_zephyr_trading_orchestrator_lifecycle_session_manager_py
...
> 本域与 21 个外部域直接连接（出边 203 条 + 入边 634 条 = 837 条）。只显示直接连接的域，不展开具体节点。
