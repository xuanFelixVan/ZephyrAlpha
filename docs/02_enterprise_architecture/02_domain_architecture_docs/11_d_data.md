...
| 18 | D_INFRA_RUNTIME 运行时集成: 盘中运行时编排器 (runtime/intraday_main.py) | → | 告警管理（MOD-L00-004 §6.5 失败重试与告警 + §8 可观测性... | 导入依赖 / import_depends |
| 19 | D_INFRA_RUNTIME 运行时集成: 盘中运行时编排器 (runtime/intraday_main.py) | → | 逐笔订阅器 / tick_subscriber (data/tick_subscriber.py) | 导入依赖 / import_depends |
| 20 | D_INFRA_RUNTIME 运行时集成: 盘中运行时编排器 (runtime/intraday_main.py) | → | A 股交易日历守卫（MOD-L00-004）。 / trading_calendar (dat... | 导入依赖 / import_depends |
| 21 | D_INFRA_RUNTIME 运行时集成: TickRedisCache 单元测试——tick→Redis tick:{symbol}:late... | → | 逐笔订阅器 / tick_subscriber (data/tick_subscriber.py) | 测试依赖 / test_depends |
| 22 | D_MKT_DATA 行情数据: 自动加载 / autoload (market_data/autoload.py) | → | table注册表 / table_registry (data/table_registry.py) | runtime / runtime |
| 23 | D_MKT_DATA 行情数据: 生产者 / producer (normalized_market_data_producer/produc... | → | 包入口 / __init__ (data/__init__.py) | 导入依赖 / import_depends |
| 24 | D_MKT_DATA 行情数据: 生产者 / producer (normalized_market_data_producer/produc... | → | ch读取器 / ch_reader (data/ch_reader.py) | 导入依赖 / import_depends |
| 25 | D_MKT_DATA 行情数据: 生产者 / producer (normalized_market_data_producer/produc... | → | table注册表 / table_registry (data/table_registry.py) | 导入依赖 / import_depends |
| 26 | D_MKT_DATA 行情数据: raw数据缓存 (raw_data_cache/) | → | 提供器基类 / provider_base (data/provider_base.py) | data / data |
