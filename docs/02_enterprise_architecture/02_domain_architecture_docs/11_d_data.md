...
flowchart TD
    ...
    class D_SHARED,D_GOV_ENFORCEMENT,D_MKT_DATA,D_INFRA_OPS,D_BACKTEST,D_INFRA_RUNTIME,D_FACTOR,D_GOV_SCRIPTS,D_GOV_CODE_QUALITY,D_GOVERNANCE external_prod
...
| 9 | ch配置 / ch_config (data/ch_config.py) | → | D_SHARED 共享服务: Secrets (security/secrets.py) | 导入依赖 / import_depends |
...
| 18 | 可观测性指标采集（MOD-L00-004 §11）。 / metrics (data/me... | → | D_SHARED 共享服务: 从当前文件向上查找项目根目录 / Paths (io/paths.py) | 导入依赖 / import_depends |
...
| 28 | wal写入器 / wal_writer (data/wal_writer.py) | → | D_SHARED 共享服务: 线程安全的轻量级 Metrics 注册表 / Metrics (observability/... | 导入依赖 / import_depends |
...
| 21 | D_INFRA_OPS 基础设施运维: Download Models (ops/download_models.py) | → | 7×24 CH 健康探针守护进程 / Ch Health Probe (ops/ch_healt... | config_depends / config_depends |
...
| 23 | D_INFRA_RUNTIME 运行时集成: —单进程串起 tick_subscriber + IntradayFactorLoop / Intra... | → | 告警管理（MOD-L00-004 §6.5 失败重试与告警 + §8 可观测性... | 导入依赖 / import_depends |
| 24 | D_INFRA_RUNTIME 运行时集成: —单进程串起 tick_subscriber + IntradayFactorLoop / Intra... | → | 逐笔订阅器 / tick_subscriber (data/tick_subscriber.py) | 导入依赖 / import_depends |
| 25 | D_INFRA_RUNTIME 运行时集成: —单进程串起 tick_subscriber + IntradayFactorLoop / Intra... | → | A 股交易日历守卫（MOD-L00-004）。 / trading_calendar (dat... | 导入依赖 / import_depends |
| 26 | D_INFRA_RUNTIME 运行时集成: —tick→Redis tick:{symbol}:latest 双写器 / Test Tick Red... | → | 逐笔订阅器 / tick_subscriber (data/tick_subscriber.py) | 测试依赖 / test_depends |
| 27 | D_MKT_DATA 行情数据: Autoload (market_data/autoload.py) | → | table注册表 / table_registry (data/table_registry.py) | runtime / runtime |
| 28 | D_MKT_DATA 行情数据: —D_MKT_DATA→D_FACTOR 数据供给 / Producer (normalized_ma... | → | 包入口 / __init__ (data/__init__.py) | 导入依赖 / import_depends |
| 29 | D_MKT_DATA 行情数据: —D_MKT_DATA→D_FACTOR 数据供给 / Producer (normalized_ma... | → | ch读取器 / ch_reader (data/ch_reader.py) | 导入依赖 / import_depends |
| 30 | D_MKT_DATA 行情数据: —D_MKT_DATA→D_FACTOR 数据供给 / Producer (normalized_ma... | → | table注册表 / table_registry (data/table_registry.py) | 导入依赖 / import_depends |
| 31 | D_ML_TRAIN 训练: Training Dataset Manager (training_dataset_manager/) | → | pit查询 / pit_query (data/pit_query.py) | data / data |
| 32 | D_ML_TRAIN 训练: Training Dataset Manager (training_dataset_manager/) | → | table注册表 / table_registry (data/table_registry.py) | data / data |
| 33 | D_ML_TRAIN 训练: Training Pipeline (training_pipeline/) | → | pit查询 / pit_query (data/pit_query.py) | data / data |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 11 个外部域直接连接（出边 28 条 + 入边 33 条 = 61 条）。只显示直接连接的域，不展开具体节点。
