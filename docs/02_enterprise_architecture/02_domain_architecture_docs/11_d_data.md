...
flowchart TD
    ...
    class D_SHARED,D_GOV_ENFORCEMENT,D_INFRA_RUNTIME,D_GOV_SCRIPTS,D_GOV_CODE_QUALITY,D_FACTOR external_prod
...
| 18 | D_INFRA_RUNTIME 运行时集成: 盘中运行时编排器 (runtime/intraday_main.py) | → | 逐笔订阅器 / tick_subscriber (data/tick_subscriber.py) | 导入依赖 / import_depends |
| 19 | D_INFRA_RUNTIME 运行时集成: 盘中运行时编排器 (runtime/intraday_main.py) | → | A 股交易日历守卫（MOD-L00-004）。 / trading_calendar (dat... | 导入依赖 / import_depends |
