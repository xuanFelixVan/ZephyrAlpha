...
flowchart TD
    ...
    class D_SHARED,D_GOV_ENFORCEMENT,D_MKT_DATA,D_GOV_SCRIPTS,D_GOV_CODE_QUALITY,D_FACTOR,D_BACKTEST,D_INFRA_RUNTIME,D_GOVERNANCE external_prod
...
> 本域与 10 个外部域直接连接（出边 25 条 + 入边 32 条 = 57 条）。只显示直接连接的域，不展开具体节点。
