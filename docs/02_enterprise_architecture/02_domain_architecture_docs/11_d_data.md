...
flowchart TD
    ...
    scripts_register_aux_tasks_ps1["(生产态 / production) scripts/register_aux_tasks.ps1<br/>文件: scripts/register_aux_tasks.ps1"]
    scripts_register_guard_tasks_ps1["(生产态 / production) scripts/register_guard_tasks.ps1<br/>文件: scripts/register_guard_tasks.ps1"]
    scripts_start_scheduler_ps1["(生产态 / production) scripts/start_scheduler.ps1<br/>文件: scripts/start_scheduler.ps1"]
    scripts_start_tick_subscriber_ps1["(生产态 / production) scripts/start_tick_subscriber.ps1<br/>文件: scripts/start_tick_subscriber.ps1"]
    src_zephyr_data_main_py["(生产态 / production) python -m zephyr.data — 数据源集成器 CLI 入口。<br/>文件: data/__main__.py"]
    src_zephyr_data_config_policies_yaml["(生产态 / production) config/policies.yaml<br/>文件: config/policies.yaml"]
    src_zephyr_data_config_schedule_yaml["(生产态 / production) config/schedule.yaml<br/>文件: config/schedule.yaml"]
    src_zephyr_data_config_tasks_yaml["(生产态 / production) config/tasks.yaml<br/>文件: config/tasks.yaml"]
    src_zephyr_data_connectors["(设计态 / design) connectors/<br/>文件: connectors/"]
    ...
    src_zephyr_data_pit_query_py_1["(设计态 / design) pit_query.py/<br/>文件: pit_query.py/"]
    ...
    scripts_ch_recovery_drill_py ~~~ scripts_ch_apply_market_tables_ddl_py
    ...
    src_zephyr_data_normalizers["(设计态 / design) normalizers/<br/>文件: normalizers/"]
    ...
    src_zephyr_data_storage["(设计态 / design) storage/<br/>文件: storage/"]
    ...
    src_zephyr_data_cache["(设计态 / design) cache/<br/>文件: cache/"]
    ...
    scripts_ch_lint_symbol_convention_py -->|config_depends / config_depends| scripts_ch_apply_fundamental_tables_ddl_py
...
flowchart TD
    src_zephyr_data_connectors["(设计态 / design) connectors/<br/>文件: connectors/"]
    src_zephyr_data_pit_query_py["(设计态 / design) pit_query.py/<br/>文件: pit_query.py/"]
...
| 18 | D_MKT_DATA 行情数据: market_data/autoload.py | → | 表名/品类注册表消费层（裁定 #ARCH-CH-024 Phase 2）。 (dat... | runtime / runtime |
| 19 | D_MKT_DATA 行情数据: NormalizedMarketData 生产者——D_MKT_DATA→D_FACTOR 数据... | → | zephyr.data — 数据源集成器（MOD-L00-004）。 (data/__init... | 导入依赖 / import_depends |
| 20 | D_MKT_DATA 行情数据: NormalizedMarketData 生产者——D_MKT_DATA→D_FACTOR 数据... | → | ClickHouse 统一读取层（裁定 #ARCH-CH-007）。 (data/ch_rea... | 导入依赖 / import_depends |
| 21 | D_MKT_DATA 行情数据: NormalizedMarketData 生产者——D_MKT_DATA→D_FACTOR 数据... | → | 表名/品类注册表消费层（裁定 #ARCH-CH-024 Phase 2）。 (dat... | 导入依赖 / import_depends |
| 22 | D_MKT_DATA 行情数据: raw_data_cache/ | → | 数据源 Provider 抽象基类（MOD-L00-004 §4）。 (data/provi... | data / data |
| 23 | D_ML_TRAIN 训练: training_pipeline/ | → | 财报 Point-In-Time (PIT) 查询能力（#ARCH-CH-021 P0-5）。 ... | data / data |
| 24 | D_TRADING 交易运营: trading/corporate_action_processor.py | → | pit_query.py/ | runtime / runtime |
| 25 | D_TRADING 交易运营: trading/corporate_action_processor.py | → | pit_query.py/ | runtime / runtime |
