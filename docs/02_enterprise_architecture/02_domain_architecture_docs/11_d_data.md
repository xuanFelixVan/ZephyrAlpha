...
flowchart TD
    ...
    class D_SHARED,D_GOV_ENFORCEMENT,D_GOV_SCRIPTS,D_MKT_DATA,D_FACTOR,D_BACKTEST,D_GOV_CODE_QUALITY,D_GOVERNANCE,D_INFRA_RUNTIME external_prod
...
> 本域与 10 个外部域直接连接（出边 32 条 + 入边 32 条 = 64 条）。只显示直接连接的域，不展开具体节点。
