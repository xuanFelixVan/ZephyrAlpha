...
flowchart TD
    ...
    scripts_ch_data_inventory_py["全库数据盘点：逐表审计行数/日期范围/空表<br/>/缺失日期/引擎/大小<br/>data_inventory模块<br/>_data_inventory<br/>文件: ch/_data_inventory.py<br/>(生产态 / production)"]
    scripts_ch_recovery_drill_py["恢复演练：轮询备份完成 → 恢复小表到临时库 →<br/>行数校验 → 清理<br/>recovery_drill模块<br/>_recovery_drill<br/>文件: ch/_recovery_drill.py<br/>(生产态 / production)"]
    ...
    scripts_ch_lint_symbol_convention_py["Symbol 约定 lint 门禁（TRAE-082 GATE-SYMBOL-C<br/>lint_symbol_convention模块<br/>文件: ch/lint_symbol_convention.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_connectors["连接器<br/>connectors模块<br/>⛔ 该域，设计已就绪，等待开发排期<br/>文件: connectors/<br/>(设计态 / design)"]
    ...
    src_zephyr_data_redundant_source_init_py["数据源冗余与热切换模块（MOD-L00-005）<br/>redundant source包的包入口模块，管理子模块加载<br/>文件: redundant_source/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_wal_codec_init_py["WAL 段编解码模块（MOD-L00-006）<br/>wal codec包的包入口模块，管理子模块加载<br/>文件: wal_codec/__init__.py<br/>(生产态 / production)"]
    tests_data_test_market_quality_validator_py["#ARCH-CH-021 P0-4:<br/>写入路径异常值校验器四门禁测试<br/>test_market_quality_validator模块<br/>文件: data/test_market_quality_validator.py<br/>(生产态 / production)"]
    tests_data_test_pit_query_py["#ARCH-CH-021 P0-5: 财报 PIT 查询能力测试<br/>test_pit_query模块<br/>文件: data/test_pit_query.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_alerter_py["告警管理（MOD-L00-004 §6.5 失败重试与告警 + §8<br/>可观测性）<br/>alerter模块<br/>文件: data/alerter.py<br/>(生产态 / production)"]
    src_zephyr_data_cli_py["数据源集成器 CLI（MOD-L00-004 §8.4）<br/>cli模块<br/>文件: data/cli.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_sector_ranking_engine_py["880xxx 板块动态排名引擎——5因子复合排名调整99只推<br/>送池<br/>sector_ranking_engine模块<br/>文件: data/sector_ranking_engine.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_progress_store_py["统一进度存储（MOD-L00-004 §7）<br/>progress_store模块<br/>文件: data/progress_store.py<br/>(生产态 / production)"]
    src_zephyr_data_scheduler_py["数据源调度编排层（MOD-L00-004 §6）<br/>scheduler模块<br/>文件: data/scheduler.py<br/>(生产态 / production)"]
    src_zephyr_data_speed_tester_py["数据源测速器（MOD-L00-004 §8.5）<br/>speed_tester模块<br/>文件: data/speed_tester.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_backfill_checker_py["L10 周末补下载检测器——检测过去N天缺失数据并精准<br/>补下载<br/>backfill_checker模块<br/>文件: data/backfill_checker.py<br/>(生产态 / production)"]
    src_zephyr_data_buffered_writer_py["批量聚合写入器（MOD-L00-004 §18.3 裁定<br/>#ARCH-CH-003<br/>buffered_writer模块<br/>文件: data/buffered_writer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_error_classifier_py["数据源错误分类器——根据错误字符串判断可恢复性<br/>error_classifier模块<br/>文件: data/error_classifier.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_integrity_checker_py["数据完整性巡检器——每天盘后检测全表当日数据是否达<br/>标<br/>integrity_checker模块<br/>文件: data/integrity_checker.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_metrics_py["可观测性指标采集（MOD-L00-004 §11）<br/>metrics模块<br/>文件: data/metrics.py<br/>(生产态 / production)"]
    src_zephyr_data_news_dedup_py["新闻数据去重模块（MOD-L00-004 §4.3）<br/>news_dedup模块<br/>文件: data/news_dedup.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_trading_calendar_py["A 股交易日历守卫（MOD-L00-004）<br/>trading_calendar模块<br/>文件: data/trading_calendar.py<br/>(生产态 / production)"]
