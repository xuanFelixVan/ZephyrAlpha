...
| 域内依赖 | 267 | Internal Dependencies | 267 |
...
flowchart TD
    ...
    tests_data_test_source_health_check_py["data/test_source_health_check<br/>test_source_health_check.py —<br/>数据源健康检查模块单测。<br/>文件: data/test_source_health_check.py<br/>(生产态 / production)"]
...
|:--:|---------|:--:|---------|---------|
| 1 | 质量门禁 / quality_gate (data/quality_gate.py) | → | D_GOV_ENFORCEMENT 规则执行: Data Quality Gate / Quality Gate (rule_enforcement/qualit... | 导入依赖 / import_depends |
| 2 | 包入口 / D_DATA Data Source (satellite_geospatial_engine/... | → | D_GOV_ENFORCEMENT 规则执行: Data Quality Gate / Quality Gate (rule_enforcement/qualit... | 导入依赖 / import_depends |
| 3 | #ARCH-CH-021 P0-4: 写入路径异常值校验器四门禁测试。 / tes... | → | D_GOV_ENFORCEMENT 规则执行: Data Quality Gate / Quality Gate (rule_enforcement/qualit... | 测试依赖 / test_depends |
...
| 6 | 告警管理（MOD-L00-004 §6.5 失败重试与告警 + §8 可观测性... | → | D_SHARED 共享服务: Secrets / Secrets (security/secrets.py) | 导入依赖 / import_depends |
...
| 9 | ch配置 / ch_config (data/ch_config.py) | → | D_SHARED 共享服务: Secrets / Secrets (security/secrets.py) | 导入依赖 / import_depends |
| 10 | ch写入器 / ch_writer (data/ch_writer.py) | → | D_SHARED 共享服务: 线程安全的轻量级 Metrics 注册表 / Metrics (observability/... | 导入依赖 / import_depends |
| 11 | ch写入器 / ch_writer (data/ch_writer.py) | → | D_SHARED 共享服务: Secrets / Secrets (security/secrets.py) | 导入依赖 / import_depends |
| 12 | cls提供器 / cls_provider (implementations/cls_provider.py) | → | D_SHARED 共享服务: Constants / Constants (foundation/constants.py) | 导入依赖 / import_depends |
| 13 | rss提供器 / rss_provider (implementations/rss_provider.py) | → | D_SHARED 共享服务: Constants / Constants (foundation/constants.py) | 导入依赖 / import_depends |
| 14 | tushare提供器 / tushare_provider (implementations/tushare... | → | D_SHARED 共享服务: Secrets / Secrets (security/secrets.py) | 导入依赖 / import_depends |
| 15 | 本地replay / local_replay (data/local_replay.py) | → | D_SHARED 共享服务: 从当前文件向上查找项目根目录 / Paths (io/paths.py) | 导入依赖 / import_depends |
| 16 | 可观测性指标采集（MOD-L00-004 §11）。 / metrics (data/me... | → | D_SHARED 共享服务: 从当前文件向上查找项目根目录 / Paths (io/paths.py) | 导入依赖 / import_depends |
| 17 | 统一进度存储（MOD-L00-004 §7）。 / progress_store (data/... | → | D_SHARED 共享服务: 从当前文件向上查找项目根目录 / Paths (io/paths.py) | 导入依赖 / import_depends |
| 18 | 统一进度存储（MOD-L00-004 §7）。 / progress_store (data/... | → | D_SHARED 共享服务: 注册 datetime/date→sqlite3 str 适配器 / Time Utils (util... | 导入依赖 / import_depends |
| 19 | 数据源调度编排层（MOD-L00-004 §6）。 / scheduler (data/s... | → | D_SHARED 共享服务: 从当前文件向上查找项目根目录 / Paths (io/paths.py) | 导入依赖 / import_depends |
| 20 | 数据源健康检查 (data/source_health_check.py) | → | D_SHARED 共享服务: Constants / Constants (foundation/constants.py) | 导入依赖 / import_depends |
| 21 | 数据源健康检查 (data/source_health_check.py) | → | D_SHARED 共享服务: 注册 datetime/date→sqlite3 str 适配器 / Time Utils (util... | 导入依赖 / import_depends |
| 22 | 数据源测速器（MOD-L00-004 §8.5）。 / speed_tester (data/... | → | D_SHARED 共享服务: Constants / Constants (foundation/constants.py) | 导入依赖 / import_depends |
| 23 | table注册表 / table_registry (data/table_registry.py) | → | D_SHARED 共享服务: 从当前文件向上查找项目根目录 / Paths (io/paths.py) | 导入依赖 / import_depends |
| 24 | 逐笔订阅器 / tick_subscriber (data/tick_subscriber.py) | → | D_SHARED 共享服务: 线程安全的轻量级 Metrics 注册表 / Metrics (observability/... | 导入依赖 / import_depends |
| 25 | 逐笔订阅器 / tick_subscriber (data/tick_subscriber.py) | → | D_SHARED 共享服务: Prometheus /metrics HTTP 端点 / Metrics Server (observabi... | 导入依赖 / import_depends |
| 26 | wal写入器 / wal_writer (data/wal_writer.py) | → | D_SHARED 共享服务: 线程安全的轻量级 Metrics 注册表 / Metrics (observability/... | 导入依赖 / import_depends |
...
| 12 | D_GOVERNANCE 生命周期管理: Test E2e Pipeline / Test E2e Pipeline (trading/test_e2e_p... | → | 质量门禁 / quality_gate (data/quality_gate.py) | 测试依赖 / test_depends |
...
| 26 | D_MKT_DATA 行情数据: Autoload / Autoload (market_data/autoload.py) | → | table注册表 / table_registry (data/table_registry.py) | runtime / runtime |
...
| 30 | D_ML_TRAIN 训练: Training Dataset Manager / Training Dataset Manager (trai... | → | pit查询 / pit_query (data/pit_query.py) | data / data |
| 31 | D_ML_TRAIN 训练: Training Dataset Manager / Training Dataset Manager (trai... | → | table注册表 / table_registry (data/table_registry.py) | data / data |
| 32 | D_ML_TRAIN 训练: Training Pipeline / Training Pipeline (training_pipeline/) | → | pit查询 / pit_query (data/pit_query.py) | data / data |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 10 个外部域直接连接（出边 26 条 + 入边 32 条 = 58 条）。只显示直接连接的域，不展开具体节点。
