...
flowchart TD
    ...
    src_zephyr_data_redundant_source_init_py["数据源冗余与热切换模块（MOD-L00-005）。<br/>data/redundant_source<br/>包入口，管理该层子模块的统一加载和懒导入<br/>文件: redundant_source/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_wal_codec_init_py["WAL 段编解码模块（MOD-L00-006）。<br/>data/wal_codec<br/>包入口，管理该层子模块的统一加载和懒导入<br/>文件: wal_codec/__init__.py<br/>(生产态 / production)"]
