...
| 域内依赖 | 272 | Internal Dependencies | 272 |
...
> 展示全部 173 个模块（生产态 169 + 设计态 4），含跨域依赖外部节点。节点含成熟度+名称+大白话/简介+文件路径。
...
flowchart TD
    ...
    src_zephyr_data_source_health_check_py["data/source_health_check<br/>数据源健康检查模块（每日调度器启动时执行）。<br/>文件: data/source_health_check.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_quality_gate_py ~~~ src_zephyr_data_source_health_check_py
    src_zephyr_data_source_health_check_py ~~~ src_zephyr_data_task_queue_py
    ...
    schemas_categories_market_hog_spot_index_py -->|config_depends / config_depends| schemas_categories_fundamental_balance_sheet_py
    schemas_categories_market_hog_futures_core_py -->|config_depends / config_depends| schemas_categories_fundamental_balance_sheet_py
    schemas_categories_market_hog_province_spot_py -->|config_depends / config_depends| schemas_categories_fundamental_balance_sheet_py
    ...
    schemas_categories_market_kline_etf_daily_py -->|config_depends / config_depends| schemas_categories_fundamental_balance_sheet_py
    ...
    src_zephyr_data_implementations_ifind_provider_py -->|导入依赖 / import_depends| D_SHARED
    ...
    class D_SHARED,D_GOV_ENFORCEMENT,D_GOV_SCRIPTS,D_MKT_DATA,D_INFRA_OPS,D_INFRA_RUNTIME,D_GOVERNANCE,D_GOV_CODE_QUALITY,D_FACTOR,D_BACKTEST external_prod
...
| 11 | ch写入器 / ch_writer (data/ch_writer.py) | → | D_SHARED 共享服务: secrets.py —— Secrets 管理抽象（Phase 7 新增 | 盲点 B12... | 导入依赖 / import_depends |
| 12 | cls提供器 / cls_provider (implementations/cls_provider.py) | → | D_SHARED 共享服务: constants.py —— 共享枚举 & 常量集中 re-export（Single S... | 导入依赖 / import_depends |
| 13 | ifind提供器 / ifind_provider (implementations/ifind_provi... | → | D_SHARED 共享服务: secrets.py —— Secrets 管理抽象（Phase 7 新增 | 盲点 B12... | 导入依赖 / import_depends |
| 14 | rss提供器 / rss_provider (implementations/rss_provider.py) | → | D_SHARED 共享服务: constants.py —— 共享枚举 & 常量集中 re-export（Single S... | 导入依赖 / import_depends |
| 15 | tushare提供器 / tushare_provider (implementations/tushare... | → | D_SHARED 共享服务: secrets.py —— Secrets 管理抽象（Phase 7 新增 | 盲点 B12... | 导入依赖 / import_depends |
| 16 | 本地replay / local_replay (data/local_replay.py) | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 17 | 可观测性指标采集（MOD-L00-004 §11）。 / metrics (data/me... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 18 | 统一进度存储（MOD-L00-004 §7）。 / progress_store (data/... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 19 | 统一进度存储（MOD-L00-004 §7）。 / progress_store (data/... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 | 盲点 B19... | 导入依赖 / import_depends |
| 20 | 数据源调度编排层（MOD-L00-004 §6）。 / scheduler (data/s... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 21 | 数据源测速器（MOD-L00-004 §8.5）。 / speed_tester (data/... | → | D_SHARED 共享服务: constants.py —— 共享枚举 & 常量集中 re-export（Single S... | 导入依赖 / import_depends |
| 22 | table注册表 / table_registry (data/table_registry.py) | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of Truth） (... | 导入依赖 / import_depends |
| 23 | 逐笔订阅器 / tick_subscriber (data/tick_subscriber.py) | → | D_SHARED 共享服务: metrics.py —— 轻量级 Metrics 收集基础设施（Phase 9 新增... | 导入依赖 / import_depends |
| 24 | 逐笔订阅器 / tick_subscriber (data/tick_subscriber.py) | → | D_SHARED 共享服务: Prometheus /metrics HTTP 端点（P1-5 可观测性改造）。 (obs... | 导入依赖 / import_depends |
| 25 | wal写入器 / wal_writer (data/wal_writer.py) | → | D_SHARED 共享服务: metrics.py —— 轻量级 Metrics 收集基础设施（Phase 9 新增... | 导入依赖 / import_depends |
...
> 本域与 11 个外部域直接连接（出边 25 条 + 入边 33 条 = 58 条）。只显示直接连接的域，不展开具体节点。
