...
flowchart TD
    ...
    schemas_categories_fundamental_equity_pledge_detail_py["fundamentalequitypledge详情<br/>（股权质押明细）DDL-as-Code（category_id:<br/>fundamental_equity_pledge_detail）<br/>文件: categories/fundamental_equity_pledge_<br/>detail.py<br/>(生产态 / production)"]
    schemas_categories_fundamental_income_statement_py["fundamentalincome报表<br/>（利润表）DDL-as-Code（category_id: fundamental_<br/>income_statement, calc_mode: preload）<br/>文件: categories/fundamental_income_statement.py<br/>(生产态 / production)"]
    ...
    schemas_categories_fundamental_industry_class_suppl_py["fundamentalindustry类suppl<br/>（补充行业分类）DDL-as-Code（category_id:<br/>fundamental_industry_class_suppl）<br/>文件: categories/fundamental_industry_class_<br/>suppl.py<br/>(生产态 / production)"]
    schemas_categories_fundamental_restricted_shares_py["restrictedshares（限售股明细）DDL-as-Code（cate<br/>（限售股明细）DDL-as-Code（cate<br/>（限售股明细）DDL-as-Code（category_id:<br/>fundamental_restricted_shares）<br/>文件: categories/fundamental_restricted_<br/>shares.py<br/>(生产态 / production)"]
    ...
    scripts_register_aux_tasks_ps1["注册aux任务<br/>register_aux_<br/>tasks.ps1的注册表，登记和查询已注册的条目<br/>文件: scripts/register_aux_tasks.ps1<br/>(生产态 / production)"]
    ...
    src_zephyr_data_main_py["主入口<br/>data — 数据源集成器 CLI 入口<br/>文件: data/__main__.py<br/>(生产态 / production)"]
    src_zephyr_data_config_policies_yaml["数据源CLI<br/>策略（policies.yaml）<br/>文件: config/policies.yaml<br/>(生产态 / production)"]
    ...
    src_zephyr_data_connectors["连接器<br/>连接器的子目录，归集相关子模块<br/>⛔ 该域，设计已就绪，等待开发排期<br/>文件: connectors/<br/>(设计态 / design)"]
    ...
    src_zephyr_data_pit_query_py_1["PIT查询<br/>数据的子目录，归集相关子模块<br/>⛔ 该域，设计已就绪，等待开发排期<br/>文件: pit_query.py/<br/>(设计态 / design)"]
    src_zephyr_data_redundant_source_init_py["数据源冗余<br/>与热切换模块，主备数据源自动切换保证可用性<br/>文件: redundant_source/__init__.py<br/>(生产态 / production)"]
    src_zephyr_data_satellite_geospatial_engine_init_py["数据源冗余<br/>包入口。D_DATA Data Source<br/>文件: satellite_geospatial_engine/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_wal_codec_init_py["WAL编解码<br/>WAL 段编解码模块，实现预写日志的编码解码保障持久<br/>化<br/>文件: wal_codec/__init__.py<br/>(生产态 / production)"]
    ...
    schemas_categories_fundamental_balance_sheet_py ~~~ schemas_categories_fundamental_equity_pledge_detail_py
    ...
    src_zephyr_data_alerter_py["告警管理（MOD-L00-004 §6.5 失败重试与告警 + §8<br/>可观测性）<br/>alerter<br/>文件: data/alerter.py<br/>(生产态 / production)"]
    src_zephyr_data_cli_py["数据源CLI<br/>数据源集成器 CLI，提供命令行操作数据源的入口<br/>文件: data/cli.py<br/>(生产态 / production)"]
    ...
    schemas_categories_fundamental_cashflow_statement_py["fundamentalcashflow报表<br/>（现金流量表）DDL-as-Code（category_id:<br/>fundamental_cashflow_statement, calc_mode:<br/>preload）<br/>文件: categories/fundamental_cashflow_<br/>statement.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_symbol_normalizer_normalizer_py["归一化器<br/>symbol 标准化核心实现——TRAE-082 symbol 约定铁律<br/>normalizer<br/>文件: symbol_normalizer/normalizer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_implementations_akshare_provider_py["akshare提供器<br/>AKShare 数据源 Provider 实现（MOD-L00-004 §4.3）<br/>akshare_provider<br/>文件: implementations/akshare_provider.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_implementations_tushare_provider_py["tushare提供器<br/>Tushare 数据源 Provider 实现（MOD-L00-004 §4.3）<br/>tushare_provider<br/>文件: implementations/tushare_provider.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_buffered_writer_py["批量聚合写入器（MOD-L00-004 §18.3 裁定<br/>#ARCH-CH-003<br/>）<br/>buffered_writer<br/>文件: data/buffered_writer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_wal_writer_py["WAL编解码<br/>主动 WAL 写入器（P0-1 Phase A）<br/>wal_writer<br/>文件: data/wal_writer.py<br/>(生产态 / production)"]
...
flowchart TD
    src_zephyr_data_connectors["连接器<br/>连接器的子目录，归集相关子模块<br/>⛔ 该域，设计已就绪，等待开发排期<br/>文件: connectors/<br/>(设计态 / design)"]
    src_zephyr_data_pit_query_py["PIT查询<br/>数据的子目录，归集相关子模块<br/>⛔ 该域，设计已就绪，等待开发排期<br/>文件: pit_query.py/<br/>(设计态 / design)"]
...
| 2 | 数据源冗余 / D_DATA Data Source (satellite_geospatial_eng... | → | D_GOV_ENFORCEMENT 规则执行: 质量门禁 / D_DATA — Data Quality Gate (rule_enforcement/... | 导入依赖 / import_depends |
...
| 22 | WAL编解码 / wal_writer (data/wal_writer.py) | → | D_SHARED 共享服务: 指标 / metrics (observability/metrics.py) | 导入依赖 / import_depends |
