...
flowchart TD
    ...
    schemas_categories_fundamental_balance_sheet_py["fundamental余额sheet<br/>（资产负债表）DDL-as-Code（category_id:<br/>fundamental_balance_sheet, calc_mode:<br/>preload）。<br/>文件: categories/fundamental_balance_sheet.py<br/>(生产态 / production)"]
    ...
    scripts_start_tick_subscriber_ps1["启动逐笔订阅器<br/>start_tick_<br/>subscriber.ps1的订阅器，订阅接收数据。<br/>文件: scripts/start_tick_subscriber.ps1<br/>(生产态 / production)"]
    ...
    src_zephyr_data_connectors["连接器<br/>data包的connectors模块<br/>⛔ 该域，设计已就绪，等待开发排期<br/>文件: connectors/<br/>(设计态 / design)"]
    src_zephyr_data_implementations_init_py["data/implementations 包入口<br/>管理data.implementations子包加载<br/>文件: implementations/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_pit_query_py_1["PIT查询<br/>，供数据处理器、zephy使用<br/>⛔ 该域，设计已就绪，等待开发排期<br/>文件: pit_query.py/<br/>(设计态 / design)"]
    src_zephyr_data_redundant_source_init_py["数据源冗余与热切换模块（MOD-L00-005）。<br/>data/redundant_source<br/>包入口，管理data.redundant_source子包加载<br/>文件: redundant_source/__init__.py<br/>(生产态 / production)"]
    src_zephyr_data_satellite_geospatial_engine_init_py["data/satellite_geospatial_engine 包入口<br/>管理data.satellite_geospatial_engine子包加载<br/>D_DATA Data Source<br/>文件: satellite_geospatial_engine/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_symbol_normalizer_init_py["data/symbol_normalizer 包入口<br/>管理data.symbol_normalizer子包加载<br/>文件: symbol_normalizer/__init__.py<br/>(生产态 / production)"]
    src_zephyr_data_wal_codec_init_py["WAL 段编解码模块（MOD-L00-006）。<br/>data/wal_codec 包入口，管理data.wal_<br/>codec子包加载<br/>文件: wal_codec/__init__.py<br/>(生产态 / production)"]
    tests_data_test_market_quality_validator_py["#ARCH-CH-021 P0-4:<br/>写入路径异常值校验器四门禁测试。<br/>data包的test_market_quality_validator模块<br/>文件: data/test_market_quality_validator.py<br/>(生产态 / production)"]
    tests_data_test_pit_query_py["#ARCH-CH-021 P0-5: 财报 PIT 查询能力测试。<br/>data包的test_pit_query模块<br/>文件: data/test_pit_query.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_alerter_py["告警管理（MOD-L00-004 §6.5 失败重试与告警 + §8<br/>可观测性）<br/>data包的alerter模块<br/>文件: data/alerter.py<br/>(生产态 / production)"]
    src_zephyr_data_cli_py["数据源集成器 CLI（MOD-L00-004 §8.4）。<br/>data包的cli模块<br/>文件: data/cli.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_sector_ranking_engine_py["880xxx 板块动态排名引擎——5因子复合排名调整99只推<br/>送池。<br/>data包的sector_ranking_engine模块<br/>文件: data/sector_ranking_engine.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_progress_store_py["统一进度存储（MOD-L00-004 §7）。<br/>data包的progress_store模块<br/>文件: data/progress_store.py<br/>(生产态 / production)"]
    src_zephyr_data_scheduler_py["数据源调度编排层（MOD-L00-004 §6）。<br/>data包的scheduler模块<br/>文件: data/scheduler.py<br/>(生产态 / production)"]
    src_zephyr_data_speed_tester_py["数据源测速器（MOD-L00-004 §8.5）。<br/>data包的speed_tester模块<br/>文件: data/speed_tester.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_init_py["zephyr/data 包入口<br/>管理zephyr.data子包加载<br/>文件: data/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_backfill_checker_py["L10 周末补下载检测器——检测过去N天缺失数据并精准<br/>补下载。<br/>data包的backfill_checker模块<br/>文件: data/backfill_checker.py<br/>(生产态 / production)"]
    src_zephyr_data_buffered_writer_py["批量聚合写入器（MOD-L00-004 §18.3 裁定<br/>#ARCH-CH-003<br/>data包的buffered_writer模块<br/>文件: data/buffered_writer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_error_classifier_py["数据源错误分类器——根据错误字符串判断可恢复性。<br/>data包的error_classifier模块<br/>文件: data/error_classifier.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_integrity_checker_py["数据完整性巡检器——每天盘后检测全表当日数据是否达<br/>标。<br/>data包的integrity_checker模块<br/>文件: data/integrity_checker.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_metrics_py["可观测性指标采集（MOD-L00-004 §11）。<br/>data包的metrics模块<br/>文件: data/metrics.py<br/>(生产态 / production)"]
    src_zephyr_data_news_dedup_py["新闻数据去重模块（MOD-L00-004 §4.3）。<br/>data包的news_dedup模块<br/>文件: data/news_dedup.py<br/>(生产态 / production)"]
    ...
    src_zephyr_data_trading_calendar_py["A 股交易日历守卫（MOD-L00-004）。<br/>data包的trading_calendar模块<br/>文件: data/trading_calendar.py<br/>(生产态 / production)"]
...
flowchart TD
    src_zephyr_data_connectors["连接器<br/>data包的connectors模块<br/>⛔ 该域，设计已就绪，等待开发排期<br/>文件: connectors/<br/>(设计态 / design)"]
    src_zephyr_data_pit_query_py["PIT查询<br/>，供数据处理器、zephy使用<br/>⛔ 该域，设计已就绪，等待开发排期<br/>文件: pit_query.py/<br/>(设计态 / design)"]
