...
flowchart TD
    ...
    class D_SHARED,D_GOV_ENFORCEMENT,D_MKT_DATA,D_GOV_SCRIPTS,D_INFRA_RUNTIME,D_GOVERNANCE,D_BACKTEST,D_GOV_CODE_QUALITY,D_FACTOR external_prod
...
| 30 | D_ML_TRAIN 训练: training_pipeline/ | → | pit查询 / pit_query (data/pit_query.py) | data / data |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 10 个外部域直接连接（出边 23 条 + 入边 30 条 = 53 条）。只显示直接连接的域，不展开具体节点。
