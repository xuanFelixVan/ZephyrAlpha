...
flowchart TD
    ...
    src_zephyr_data_implementations_fred_provider_py["implementations/fred_provider<br/>数据/implementations包的fred_provider模块<br/>文件: implementations/fred_provider.py<br/>(设计态 / design)"]
    ...
    class D_SHARED,D_GOV_ENFORCEMENT,D_MKT_DATA,D_BACKTEST,D_GOV_SCRIPTS,D_GOV_CODE_QUALITY,D_INFRA_RUNTIME,D_FACTOR external_prod
...
| 15 | tushare提供器 / tushare_provider (implementations/tushare... | → | D_SHARED 共享服务: 密钥 / Secrets (security/secrets.py) | 导入依赖 / import_depends |
...
> 本域与 10 个外部域直接连接（出边 28 条 + 入边 32 条 = 60 条）。只显示直接连接的域，不展开具体节点。
