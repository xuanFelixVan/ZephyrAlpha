...
flowchart TD
    ...
    tests_governance_audit_test_integrity_audit_reconciler_py -->|测试依赖 / test_depends| D_GOV_ENFORCEMENT
    ...
    src_zephyr_governance_audit_git_performance_monitor_reconciler_py -->|导入依赖 / import_depends| D_GOV_ENFORCEMENT
    ...
    class D_GOV_SCRIPTS,D_SHARED,D_SECURITY,D_GOV_CODE_QUALITY,D_GOVERNANCE,D_GOV_ENFORCEMENT,D_INFRA_RECOVERY,D_INFRA_OPS,D_GOV_DRIFT external_prod
```
...
| 71 | D_INFRA_OPS 基础设施运维: cleanup_runtime_tmp_residue.py — 一次性清理 .runtime/tmp... | → | 对账注册表 / reconciliation_registry (audit/reconciliatio... | 导入依赖 / import_depends |
...
> 本域与 20 个外部域直接连接（出边 139 条 + 入边 87 条 = 226 条）。只显示直接连接的域，不展开具体节点。
