...
flowchart TD
    docs_03_modules_cross_layer_audit_orchestrator_blueprint_md["蓝图<br/>blueprint模块蓝图文档，描述该模块的设计意图和架<br/>构决策<br/>⛔ 该域，设计已就绪，等待开发排期<br/>文件: audit_orchestrator/blueprint.md<br/>(设计态 / design)"]
    docs_03_modules_domain_governance_audit_trail_blueprint_md["蓝图<br/>blueprint模块蓝图文档，描述该模块的设计意图和架<br/>构决策<br/>⛔ 该域，设计已就绪，等待开发排期<br/>文件: audit_trail/blueprint.md<br/>(设计态 / design)"]
    ...
    scripts_governance_test_remediation_progress_smoke_py["测试修复进度smoke<br/>测试修复进度smoke.1 治本进度 reconciler<br/>end-to-end smoke test。<br/>test_remediation_progress_smoke<br/>文件: governance/test_remediation_progress_<br/>smoke.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_audit_changelog_manager_py["changelog管理器<br/>changelog管理器，审计的日志器，记录运行日志。<br/>changelog_manager<br/>文件: gov_audit/changelog_manager.py<br/>(生产态 / production)"]
    src_zephyr_gov_audit_cli_py["gov_audit/cli<br/>gov audit包的cli模块<br/>文件: gov_audit/cli.py<br/>(生产态 / production)"]
    src_zephyr_gov_audit_code_archaeology_py["代码archaeology<br/>代码archaeology，审计的记录器，把发生的事件<br/>/结果记下来留档。<br/>code_archaeology<br/>文件: gov_audit/code_archaeology.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_audit_genesis_py["audit-trail.genesis — MOD-INF-020 · 创世块管<br/>audit-trail.genesis — MOD-INF-020 ·<br/>创世块管.genesis — MOD-INF-020 ·<br/>创世块管.genesis — MOD-INF-020 · 创世块管理<br/>文件: gov_audit/genesis.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_enforcement_behavioral_admission_ai_code_standards_py["ai代码standards<br/>ai代码standards（ai_code_standards.py）<br/>文件: behavioral_admission/ai_code_standards.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_enforcement_behavioral_admission_vibe_coding_enforcer_py["vibecoding执行器<br/>vibecoding执行器，治理执行的核心类，封装VibeRule<br/>Level相关逻辑。<br/>vibe_coding_enforcer<br/>文件: behavioral_admission/vibe_coding_<br/>enforcer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_audit_default_attribution_engine_py["默认attribution引擎<br/>默认attribution引擎。Re-export wrapper: default_<br/>attribution_engine canonical at<br/>zephyr.reporting.default_attribution_engi<br/>文件: audit/default_attribution_engine.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_semantic_audit_fix_result_prioritizer_py["修复结果prioritizer<br/>修复结果prioritizer。fix_prioritizer —<br/>MOD-INF-028 §3.1 Stage 8<br/>文件: semantic_audit/fix_result_prioritizer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_semantic_audit_semantic_cache_py["semantic缓存<br/>semantic缓存，审计的缓存，暂存常用数据加速访问。<br/>semantic_cache<br/>文件: semantic_audit/semantic_cache.py<br/>(生产态 / production)"]
    ...
    tests_governance_audit_test_error_pattern_id_column_py["测试错误patternidcolumn<br/>测试错误patternidcolumn.error_pattern_id<br/>列幂等迁移单测（P4-1a）<br/>test_error_pattern_id_column<br/>文件: audit/test_error_pattern_id_column.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_audit_bridge_py["写入核心审计链——治本（裁定#18 G7 + 5.37.1）<br/>写入核心审计链——治本（裁定#18 G7 +<br/>5.37.1）：真实落盘 events.jsonl。<br/>bridge<br/>文件: gov_audit/bridge.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_semantic_audit_self_health_py["7 SLI + 5 容量 SLI + 退化检测。定时自检,输出<br/>HEALTHY/<br/>7 SLI + 5 容量 SLI + 退化检测。定时自检,输出<br/>HEALTHY/DEGRADED/CRITICAL。<br/>self_health<br/>文件: semantic_audit/self_health.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_audit_delegation_bridge_py["delegation桥接<br/>delegation桥接，主要提供报告delegationfailure、<br/>报告delegation超时、isavailable等功能，供audit-o<br/>rchestrator.delegation_使用<br/>delegation_bridge<br/>文件: gov_audit/delegation_bridge.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_audit_merkle_hourly_py["audit-trail.merkle每小时<br/>audit-trail.merkle每小时.merkle每小时· 每小时<br/>Merkle 聚合<br/>merkle_hourly<br/>文件: gov_audit/merkle_hourly.py<br/>(生产态 / production)"]
    src_zephyr_gov_audit_models_py["审计事件类型枚举——治本（裁定#18 G2）：转为真 Enu<br/>审计事件类型枚举——治本（裁定#18 G2）：转为真<br/>Enum，values 全部小写。<br/>models<br/>文件: gov_audit/models.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_audit_writer_py["不可变审计写入器——JSONL 追加 + SHA-256 哈<br/>不可变审计写入器——JSONL 追加 + SHA-256 哈希链 +<br/>HMAC-SHA256 签名 + Lamport 时钟。<br/>writer<br/>文件: gov_audit/writer.py<br/>(生产态 / production)"]
    ...
    class D_SHARED,D_GOV_CODE_QUALITY,D_GOV_SCRIPTS,D_GOVERNANCE,D_SECURITY,D_GOV_DRIFT,D_GOV_ENFORCEMENT,D_GOV_OPS_RESILIENCE,D_TRADING,D_INTEGRATION,D_AUTONOMY_CORE external_prod
```
...
> 本域与 18 个外部域直接连接（出边 108 条 + 入边 69 条 = 177 条）。只显示直接连接的域，不展开具体节点。
