...
flowchart TD
    ...
    scripts_governance_repair_audit_design_completeness_py["(生产态 / production)<br/>审计designcompleteness / audit_design_<br/>completeness<br/>(INVARIANTS)<br/>按path精确匹配+按功能名模糊匹配;<br/>输出差距报告; 提取所有ID格式<br/>文件: repair/audit_design_<br/>completeness.py"]
    ...
    scripts_governance_repair_rollback_depgraph_py["(生产态 / production) 回滚依赖图 /<br/>rollback_depgraph<br/>(INVARIANTS)<br/>仅接受depgraph.backup.*路径;<br/>回滚前自动备份当前depgraph<br/>文件: repair/rollback_depgraph.py"]
    scripts_governance_test_remediation_progress_smoke_py["(生产态 / production) 测试修复进度smoke<br/>/ test_remediation_progress_smoke<br/>测试修复进度smoke.1 治本进度 reconciler<br/>end-to-end smoke test。<br/>文件: governance/test_remediation_<br/>progress_smoke.py"]
    src_zephyr_gov_audit_orchestrator_compat_py["(生产态 / production) 编排器兼容 / _<br/>orchestrator_compat<br/>audit-orchestrator 兼容重导出层<br/>（ARCH-042 阶段4 修复双<br/>MODULE，ARCH-043 Risk3 改名）<br/>文件: gov_audit/_orchestrator_compat.py"]
    src_zephyr_gov_audit_action_history_py["(生产态 / production) 行为历史 / action_<br/>history<br/>ActionHistory — 操作历史持久化审计 +<br/>去重 + 循环检测<br/>文件: gov_audit/action_history.py"]
    src_zephyr_gov_audit_api_lifecycle_py["(生产态 / production) API生命周期 / api_<br/>lifecycle<br/>API生命周期，审计的状态机，管理状态流转<br/>。<br/>文件: gov_audit/api_lifecycle.py"]
    src_zephyr_gov_audit_audit_schema_py["(生产态 / production) 审计模式 / audit_<br/>schema<br/>audit_schema — 审计视图与查询入口<br/>（SH-DB-001 v2.0）<br/>文件: gov_audit/audit_schema.py"]
    src_zephyr_gov_audit_audit_write_failure_protector_py["(生产态 / production)<br/>审计write故障protector / audit_write_<br/>failure_protector<br/>Audit Write Failure Protector — v0.13.0<br/>审计写入失败保护器。<br/>文件: gov_audit/audit_write_failure_<br/>protector.py"]
    src_zephyr_gov_audit_bridges_audit_anomaly_py["(生产态 / production) 审计异常 / audit_<br/>anomaly<br/>G-CT-002 Audit 异常检测器 —<br/>AnomalyEvent Pydantic V2 BaseModel.<br/>文件: bridges/audit_anomaly.py"]
    src_zephyr_gov_audit_bridges_audit_contracts_py["(生产态 / production) 审计契约 / audit_<br/>contracts<br/>G-CT-001 契约消费端 — Audit.write()<br/>公共接口.<br/>文件: bridges/audit_contracts.py"]
    src_zephyr_gov_audit_bridges_audit_delegation_bridge_py["(生产态 / production)<br/>审计delegation桥接 / audit_delegation_<br/>bridge<br/>Audit ↔ DelegationManager<br/>委托链审计桥接.<br/>文件: bridges/audit_delegation_bridge.py"]
    src_zephyr_gov_audit_bridges_audit_drift_bridge_py["(生产态 / production) 审计漂移桥接 /<br/>audit_drift_bridge<br/>G-CT-007 Audit ↔ Drift 双向桥接 —<br/>MOD-INF-020 ↔ MOD-INF-023<br/>文件: bridges/audit_drift_bridge.py"]
    src_zephyr_gov_audit_bridges_audit_feedback_bridge_py["(生产态 / production) 审计反馈桥接 /<br/>audit_feedback_bridge<br/>Audit ↔ Feedback Loop 三角闭环桥接.<br/>文件: bridges/audit_feedback_bridge.py"]
    src_zephyr_gov_audit_bridges_audit_tiered_storage_bridge_py["(生产态 / production)<br/>审计tiered存储桥接 / audit_tiered_<br/>storage_bridge<br/>Audit ↔ WarmHotGate 三层存储桥接.<br/>文件: bridges/audit_tiered_storage_<br/>bridge.py"]
    src_zephyr_gov_audit_bridges_audit_trust_bridge_py["(生产态 / production) 审计信任桥接 /<br/>audit_trust_bridge<br/>Audit ↔ ContinuousTrust 信任分数桥接.<br/>文件: bridges/audit_trust_bridge.py"]
    src_zephyr_gov_audit_changelog_manager_py["(生产态 / production) changelog管理器 /<br/>changelog_manager<br/>changelog管理器，审计的日志器，记录运行<br/>日志。<br/>文件: gov_audit/changelog_manager.py"]
    src_zephyr_gov_audit_cli_py["(生产态 / production) 命令行 / cli<br/>命令行，供End users; CI/CD; MCP tool<br/>wra使用<br/>文件: gov_audit/cli.py"]
    src_zephyr_gov_audit_code_archaeology_py["(生产态 / production) 代码archaeology /<br/>code_archaeology<br/>代码archaeology，审计的记录器，把发生的<br/>事件/结果记下来留档。<br/>文件: gov_audit/code_archaeology.py"]
    src_zephyr_gov_audit_cold_start_py["(生产态 / production) 冷启动 / cold_<br/>start<br/>BootstrapCache —<br/>审计冷启动共享单例缓存。<br/>文件: gov_audit/cold_start.py"]
    src_zephyr_gov_audit_compliance_map_py["(生产态 / production) 合规map /<br/>compliance_map<br/>合规map.compliance_map — MOD-INF-020 ·<br/>合规框架映射<br/>文件: gov_audit/compliance_map.py"]
    src_zephyr_gov_audit_corporate_actions_py["(生产态 / production) 公司行为 /<br/>corporate_actions<br/>公司行为，审计的类型，定义数据类型和枚举<br/>。<br/>文件: gov_audit/corporate_actions.py"]
    src_zephyr_gov_audit_delegation_auditor_py["(生产态 / production) delegation审计器<br/>/ delegation_auditor<br/>委托链升级类型 -- str+Enum 使 ==<br/>'string_value' 可用.<br/>文件: gov_audit/delegation_auditor.py"]
    src_zephyr_gov_audit_dora_metrics_py["(生产态 / production) dora指标 / dora_<br/>metrics<br/>dora指标，供MOD-INF-027;MOD-INF-015;MOD-<br/>FE使用<br/>文件: gov_audit/dora_metrics.py"]
    src_zephyr_gov_audit_evidence_pack_py["(生产态 / production) 证据包 / evidence_<br/>pack<br/>证据包.evidence_pack — MOD-INF-020 ·<br/>证据包导出器<br/>文件: gov_audit/evidence_pack.py"]
    src_zephyr_gov_audit_external_tool_audit_py["(生产态 / production) externaltool审计<br/>/ external_tool_audit<br/>外部tool审计，主要提供审计tool、审计模块<br/>、摘要等功能，供audit-orchestrator.pipel<br/>ine_ru使用<br/>文件: gov_audit/external_tool_audit.py"]
    src_zephyr_gov_audit_feedback_policy_py["(生产态 / production) 反馈策略 /<br/>feedback_policy.py — Audit-findings →<br/>policy recommendation<br/>反馈策略，审计的策略，定义决策规则。<br/>文件: gov_audit/feedback_policy.py"]
    src_zephyr_gov_audit_feedback_self_audit_py["(生产态 / production) 反馈自审计 /<br/>feedback_self_audit<br/>反馈自审计.feedback_self_audit —<br/>MOD-INF-020 · 反馈自审计<br/>文件: gov_audit/feedback_self_audit.py"]
    src_zephyr_gov_audit_forensic_package_py["(生产态 / production) 取证包 / forensic_<br/>package<br/>Forensic Package — v0.8.0 取证就绪:<br/>escalation event bundle+hash<br/>chain+timestamp。<br/>文件: gov_audit/forensic_package.py"]
    src_zephyr_gov_audit_genesis_py["(生产态 / production)<br/>audit-trail.genesis — MOD-INF-020 ·<br/>创世块管 / genesis<br/>audit-trail.genesis — MOD-INF-020 ·<br/>创世块管.genesis — MOD-INF-020 ·<br/>创世块管.genesis — MOD-INF-020 ·<br/>创世块管理<br/>文件: gov_audit/genesis.py"]
    src_zephyr_gov_audit_glossary_matrix_py["(生产态 / production) 词汇表矩阵 /<br/>glossary_matrix<br/>词汇表矩阵，供MOD-INF-027;MOD-INF-015;MO<br/>D-FE使用<br/>文件: gov_audit/glossary_matrix.py"]
    src_zephyr_gov_audit_incremental_review_py["(生产态 / production) incremental审查 /<br/>incremental_review<br/>incremental审查，供MOD-INF-027;MOD-INF-0<br/>15;MOD-FE使用<br/>文件: gov_audit/incremental_review.py"]
    src_zephyr_gov_audit_integrity_verifier_py["(生产态 / production) 完整性验证器 /<br/>integrity_verifier<br/>Integrity Verifier — v0.8.0<br/>代码完整性验证器: hash校验+diff<br/>detection+rollback。<br/>文件: gov_audit/integrity_verifier.py"]
    src_zephyr_gov_audit_kb_gate_py["(生产态 / production) 知识库门禁 / kb_<br/>gate<br/>知识库门禁.kb_gate — MOD-INF-020 · KB<br/>审计门控<br/>文件: gov_audit/kb_gate.py"]
    src_zephyr_gov_audit_log_rotation_py["(生产态 / production) 日志rotation /<br/>log_rotation<br/>审计日志轮转管理器——按天轮转<br/>events.jsonl，支持压缩和过期清理。<br/>文件: gov_audit/log_rotation.py"]
    src_zephyr_gov_audit_merkle_audit_py["(生产态 / production) merkle审计 /<br/>merkle_audit<br/>Merkle Audit — 兼容别名，SSoT已迁移至<br/>zephyr.gov_audit (MOD-INF-020).<br/>文件: gov_audit/merkle_audit.py"]
    src_zephyr_gov_audit_observability_dashboard_py["(生产态 / production) 可观测性仪表盘 /<br/>observability_dashboard<br/>可观测性仪表盘，供MOD-INF-027;MOD-INF-01<br/>5;MOD-FE使用<br/>文件: gov_audit/observability_<br/>dashboard.py"]
    src_zephyr_gov_audit_pipeline_runner_py["(生产态 / production) 管线运行器 /<br/>pipeline_runner<br/>管线运行器，审计的结果，封装操作结果的数<br/>据结构。<br/>文件: gov_audit/pipeline_runner.py"]
    ...
    src_zephyr_gov_audit_provenance_tracker_py["(生产态 / production) 溯源追踪器 /<br/>provenance_tracker<br/>provenance追踪器，审计的记录器，把发生的<br/>事件/结果记下来留档。<br/>文件: gov_audit/provenance_tracker.py"]
    ...
    src_zephyr_gov_audit_spec_auditor_py["(生产态 / production) spec审计器 / spec_<br/>auditor<br/>spec审计器，供zephyr.gov_audit使用<br/>文件: gov_audit/spec_auditor.py"]
    src_zephyr_gov_audit_supply_chain_py["(生产态 / production) supply链 / supply_<br/>chain<br/>supply链.supply_chain — MOD-INF-020 ·<br/>供应链审计<br/>文件: gov_audit/supply_chain.py"]
    src_zephyr_gov_audit_supply_chain_security_py["(生产态 / production) supplychain安全 /<br/>supply_chain_security<br/>supplychain安全，供MOD-INF-027;MOD-INF-0<br/>15;MOD-FE使用<br/>文件: gov_audit/supply_chain_security.py"]
    src_zephyr_gov_audit_trust_ring_manager_py["(生产态 / production) trustring管理器 /<br/>trust_ring_manager<br/>trustring管理器，提供包入口和模块加载功<br/>能<br/>文件: gov_audit/trust_ring_manager.py"]
    src_zephyr_gov_audit_wqa_scorer_py["(生产态 / production) wqa评分器 / wqa_<br/>scorer<br/>wqa评分器，主要提供composite、rating等功<br/>能<br/>文件: gov_audit/wqa_scorer.py"]
    src_zephyr_gov_enforcement_behavioral_admission_ai_code_standards_py["(生产态 / production) ai代码standards /<br/>ai_code_standards<br/>ai代码standards（ai_code_standards.py）<br/>文件: behavioral_admission/ai_code_<br/>standards.py"]
    src_zephyr_gov_enforcement_behavioral_admission_mcp_result_push_py["(生产态 / production) MCP结果推送 / mcp_<br/>result_push<br/>MCP结果推送，治理执行的异常，定义本模块<br/>的异常类型。<br/>文件: behavioral_admission/mcp_result_<br/>push.py"]
    src_zephyr_gov_enforcement_behavioral_admission_post_process_py["(生产态 / production) 提交进程 / post_<br/>process<br/>— AI 生成代码后处理管道（Phase 13 /<br/>盲点 B31）<br/>文件: behavioral_admission/post_<br/>process.py"]
    src_zephyr_gov_enforcement_behavioral_admission_vibe_coding_enforcer_py["(生产态 / production) vibecoding执行器<br/>/ vibe_coding_enforcer<br/>vibecoding执行器，治理执行的核心类，封装<br/>VibeRuleLevel相关逻辑。<br/>文件: behavioral_admission/vibe_coding_<br/>enforcer.py"]
    src_zephyr_gov_enforcement_rule_enforcement_audit_chain_verifier_py["(生产态 / production) 审计链验证器 /<br/>audit_chain_verifier<br/>审计链验证工具——独立重放门禁判定+Hash链<br/>完整性校验（beta）<br/>文件: rule_enforcement/audit_chain_<br/>verifier.py"]
    src_zephyr_gov_enforcement_rule_enforcement_sys_master_compliance_py["(生产态 / production) sys主合规 /<br/>SYS-MASTER-001 Compliance Checker<br/>sys主合规。SYS-MASTER-001 Compliance<br/>Checker<br/>文件: rule_enforcement/sys_master_<br/>compliance.py"]
    src_zephyr_governance_audit_trail_contracts_py["(生产态 / production) 契约 / contracts<br/>契约.py — G-CT-002 Audit 契约<br/>（re-export）。<br/>文件: audit-trail/contracts.py"]
    src_zephyr_governance_audit_ai_error_pattern_library_py["(生产态 / production) AI错误模式库 / ai_<br/>error_pattern_library<br/>AI 错误模式库（只读查询接口）。<br/>文件: audit/ai_error_pattern_library.py"]
    src_zephyr_governance_audit_blueprint_status_transition_reconciler_py["(生产态 / production)<br/>蓝图状态转换协调器 / blueprint_status_<br/>transition_reconciler<br/>蓝图状态单调推进 reconciler<br/>（P1-d，2026-07-21）。<br/>文件: audit/blueprint_status_transition_<br/>reconciler.py"]
    src_zephyr_governance_audit_cross_layer_contract_signature_reconciler_py["(生产态 / production)<br/>跨layercontractsignature对账器 / cross_<br/>layer_contract_signature_reconciler<br/>跨层契约签名漂移检测 reconciler<br/>（P1-b，2026-07-21）。<br/>文件: audit/cross_layer_contract_<br/>signature_reconciler.py"]
    src_zephyr_governance_audit_default_attribution_engine_py["(生产态 / production)<br/>默认attribution引擎 / Re-export<br/>wrapper: default_attribution_engine<br/>canonical at z<br/>默认attribution引擎。Re-export wrapper:<br/>default_attribution_engine canonical at<br/>zephyr.reporting.default_attribution_<br/>engi<br/>文件: audit/default_attribution_<br/>engine.py"]
    src_zephyr_governance_audit_default_tca_engine_py["(生产态 / production) 默认tca引擎 /<br/>Re-export wrapper: default_tca_engine<br/>canonical at zephyr.re<br/>默认tca引擎。Re-export wrapper: default_<br/>tca_engine canonical at<br/>zephyr.reporting.default_tca_engine.<br/>文件: audit/default_tca_engine.py"]
    src_zephyr_governance_audit_git_performance_monitor_reconciler_py["(生产态 / production) Git绩效监控协调器<br/>/ git_performance_monitor_reconciler<br/>git 性能持续监控 + 早期预警<br/>（ARCH-GIT-CALL-BUDGET<br/>P3.5，2026-07-19）。<br/>文件: audit/git_performance_monitor_<br/>reconciler.py"]
    src_zephyr_governance_audit_runtime_violation_snapshot_reconciler_py["(生产态 / production)<br/>运行时违规快照协调器 / runtime_<br/>violation_snapshot_reconciler<br/>trae_060 §5 evidence 运行时快照<br/>post-commit reconciler。<br/>文件: audit/runtime_violation_snapshot_<br/>reconciler.py"]
    src_zephyr_governance_audit_snapshot_manager_py["(生产态 / production) 快照管理器 /<br/>snapshot_manager<br/>SnapshotManager — Event Sourcing<br/>快照管理（DW-0005）<br/>文件: audit/snapshot_manager.py"]
    src_zephyr_governance_audit_workspace_hygiene_reconciler_py["(生产态 / production)<br/>工作区hygiene对账器 / workspace_hygiene_<br/>reconciler<br/>工作区卫生自动清理 reconciler<br/>（DEBT-WORKSPACE-001/002<br/>消除，2026-07-20）。<br/>文件: audit/workspace_hygiene_<br/>reconciler.py"]
    src_zephyr_governance_financial_governance_financial_compliance_py["(生产态 / production) 金融合规 /<br/>financial_compliance<br/>financial合规，治理的核心类，封装Complia<br/>nceLayer相关逻辑。<br/>文件: financial_governance/financial_<br/>compliance.py"]
    src_zephyr_governance_semantic_audit_compliance_map_py["(生产态 / production) 合规map /<br/>compliance_map<br/>合规map.compliance_map — MOD-INF-020 ·<br/>合规框架映射<br/>文件: semantic_audit/compliance_map.py"]
    src_zephyr_governance_semantic_audit_feedback_self_audit_py["(生产态 / production) 反馈自审计 /<br/>feedback_self_audit<br/>反馈自审计.feedback_self_audit —<br/>MOD-INF-020 · 反馈自审计<br/>文件: semantic_audit/feedback_self_<br/>audit.py"]
    src_zephyr_governance_semantic_audit_fix_result_prioritizer_py["(生产态 / production)<br/>修复结果prioritizer / fix_prioritizer —<br/>MOD-INF-028 §3.1 Stage 8<br/>修复结果prioritizer。fix_prioritizer —<br/>MOD-INF-028 §3.1 Stage 8<br/>文件: semantic_audit/fix_result_<br/>prioritizer.py"]
    src_zephyr_governance_semantic_audit_orchestrator_py["(生产态 / production) 编排器 /<br/>orchestrator<br/>SemanticAuditor<br/>编排器——9阶段管道统一调度.<br/>文件: semantic_audit/orchestrator.py"]
    ...
    src_zephyr_governance_semantic_audit_semantic_cache_py["(生产态 / production) semantic缓存 /<br/>semantic_cache<br/>semantic缓存，审计的缓存，暂存常用数据加<br/>速访问。<br/>文件: semantic_audit/semantic_cache.py"]
    src_zephyr_governance_semantic_audit_spec_auditor_py["(生产态 / production) spec审计器 / spec_<br/>auditor<br/>G-CT-007 — Audit.record_agent_spec()<br/>记录 Agent Spec 注册与变更.<br/>文件: semantic_audit/spec_auditor.py"]
    tests_governance_audit_test_error_pattern_id_column_py["(生产态 / production)<br/>测试错误patternidcolumn / test_error_<br/>pattern_id_column<br/>测试错误patternidcolumn.error_pattern_<br/>id 列幂等迁移单测（P4-1a）<br/>文件: audit/test_error_pattern_id_<br/>column.py"]
    tests_governance_audit_test_p3_integration_smoke_py["(生产态 / production) 测试p3集成smoke /<br/>test_p3_integration_smoke<br/>Phase 3 全链路集成 smoke test<br/>文件: audit/test_p3_integration_smoke.py"]
    tests_governance_audit_test_reconcile_async_py["(生产态 / production) 测试对账异步 /<br/>test_reconcile_async<br/>P2-3 reconciler 链路异步化测试<br/>文件: audit/test_reconcile_async.py"]
    tests_governance_audit_test_reconcile_worker_selfheal_py["(生产态 / production)<br/>测试对账工作进程selfheal / test_<br/>reconcile_worker_selfheal<br/>#ARCH-RECONCILER-ALERT-SELFHEAL-001<br/>Phase 1 测试<br/>文件: audit/test_reconcile_worker_<br/>selfheal.py"]
    tests_governance_audit_test_trae_069_threshold_sync_smoke_py["(生产态 / production)<br/>测试trae069thresholdsyncsmoke / test_<br/>trae_069_threshold_sync_smoke<br/>trae_069 YAML 真源→代码常量同步 smoke<br/>test<br/>文件: audit/test_trae_069_threshold_<br/>sync_smoke.py"]
    tests_governance_rule_bridge_test_session_worktree_async_reconcile_py["(生产态 / production)<br/>测试会话worktree异步对账 / test_session_<br/>worktree_async_reconcile<br/>_run_reconcilers_after_merge<br/>异步化测试。<br/>文件: rule_bridge/test_session_worktree_<br/>async_reconcile.py"]
    tests_governance_test_workspace_telemetry_shared_py["(生产态 / production)<br/>测试工作区遥测共享 / test_workspace_<br/>telemetry_shared<br/>shared workspace_telemetry 公共 API 单测<br/>文件: governance/test_workspace_<br/>telemetry_shared.py"]
    ...
    src_zephyr_gov_audit_anomaly_py["(生产态 / production) 异常 / anomaly<br/>异常签名枚举——治本（裁定#18<br/>G3）：转为真 Enum 对齐 test_audit_<br/>anomaly.py 契约。<br/>文件: gov_audit/anomaly.py"]
    src_zephyr_gov_audit_audit_admission_controller_py["(生产态 / production) 审计准入控制器 /<br/>audit_admission_controller<br/>审计准入控制器，审计的结果，封装操作结果<br/>的数据结构。<br/>文件: gov_audit/audit_admission_<br/>controller.py"]
    src_zephyr_gov_audit_bridge_py["(生产态 / production)<br/>写入核心审计链——治本（裁定#18 G7 +<br/>5.37.1） / bridge<br/>写入核心审计链——治本（裁定#18 G7 +<br/>5.37.1）：真实落盘 events.jsonl。<br/>文件: gov_audit/bridge.py"]
    src_zephyr_gov_audit_event_store_py["(生产态 / production) 事件存储 / event_<br/>store<br/>EventStore — Event Sourcing<br/>事件追加与回放（DW-0002）<br/>文件: gov_audit/event_store.py"]
    src_zephyr_gov_audit_query_py["(生产态 / production) 旧版查询引擎<br/>（保留以兼容现有调用方）。 / query<br/>旧版查询引擎（保留以兼容现有调用方）。<br/>文件: gov_audit/query.py"]
    src_zephyr_gov_audit_resource_aware_pool_py["(生产态 / production) 资源感知池 /<br/>resource_aware_pool<br/>资源感知池，供audit_admission_<br/>controller; or使用<br/>文件: gov_audit/resource_aware_pool.py"]
    src_zephyr_gov_audit_text_to_finding_adapter_py["(生产态 / production)<br/>texttofinding适配器 / text_to_finding_<br/>adapter<br/>textto发现适配器，审计的解析器，把文本<br/>/数据解析成结构化对象。<br/>文件: gov_audit/text_to_finding_<br/>adapter.py"]
    src_zephyr_governance_audit_git_helpers_py["(生产态 / production) Git辅助 / _git_<br/>helpers<br/>审计 reconciler 共享 git 工具模块<br/>文件: audit/_git_helpers.py"]
    src_zephyr_governance_audit_commit_gateway_abuse_monitor_reconciler_py["(生产态 / production)<br/>commitgatewayabuse监控器对账器 / commit_<br/>gateway_abuse_monitor_reconciler<br/>commit gateway 持续滥用监控<br/>（ARCH-TOOL-HEALTH-V1 Phase<br/>5b，2026-07-19）。<br/>文件: audit/commit_gateway_abuse_<br/>monitor_reconciler.py"]
    src_zephyr_governance_audit_error_pattern_consumer_reconciler_py["(生产态 / production)<br/>错误模式消费者协调器 / error_pattern_<br/>consumer_reconciler<br/>AI 行为遥测 JSONL 错误事件聚合<br/>consumer。<br/>文件: audit/error_pattern_consumer_<br/>reconciler.py"]
    src_zephyr_governance_audit_reconcile_worker_py["(生产态 / production) 对账工作器 /<br/>reconcile_worker<br/>异步 reconciler worker<br/>（Ruling:100PCT-AI-GOVERNANCE<br/>P2-3，2026-07-19）<br/>文件: audit/reconcile_worker.py"]
    src_zephyr_governance_audit_remediation_progress_reconciler_py["(生产态 / production) 修复进度对账器 /<br/>remediation_progress_reconciler<br/>治本进度持久化 + 新鲜度对账<br/>（#ARCH-GOV-CONVERGENCE-META Phase<br/>3.1）。<br/>文件: audit/remediation_progress_<br/>reconciler.py"]
    src_zephyr_governance_audit_runtime_violation_snapshot_py["(生产态 / production) 运行时违规快照 /<br/>runtime_violation_snapshot<br/>trae_060 §5 evidence 运行时快照<br/>（#ARCH-GOV-CONVERGENCE-META Phase<br/>3.4b）。<br/>文件: audit/runtime_violation_<br/>snapshot.py"]
    src_zephyr_governance_semantic_audit_alignment_engine_py["(生产态 / production) 对齐引擎 /<br/>alignment_engine<br/>三元对齐检测：蓝图声明清单 vs<br/>磁盘实际文件 vs import 引用链。<br/>文件: semantic_audit/alignment_engine.py"]
    src_zephyr_governance_semantic_audit_fix_prioritizer_py["(生产态 / production) 修复prioritizer /<br/>fix_prioritizer<br/>按 severity -> certainty -> blast_<br/>radius 三级排序,分组输出批次。<br/>文件: semantic_audit/fix_prioritizer.py"]
    src_zephyr_governance_semantic_audit_issue_aggregator_py["(生产态 / production)<br/>收集各阶段审计结果，去重合并排序输出。<br/>/ issue_aggregator<br/>收集各阶段审计结果，去重合并排序输出。<br/>文件: semantic_audit/issue_aggregator.py"]
    src_zephyr_governance_semantic_audit_kb_gate_py["(生产态 / production) 知识库门禁 / kb_<br/>gate<br/>知识库门禁.kb_gate — MOD-INF-020 · KB<br/>审计门控<br/>文件: semantic_audit/kb_gate.py"]
    src_zephyr_governance_semantic_audit_llm_bridge_py["(生产态 / production) 接收 RED<br/>问题,生成修复文本。LLM<br/>只润色不做判断。不可用时降级为模板生成<br/>/ llm_bridge<br/>接收 RED 问题,生成修复文本。LLM<br/>只润色不做判断。不可用时降级为模板生成。<br/>文件: semantic_audit/llm_bridge.py"]
    src_zephyr_governance_semantic_audit_safety_boundary_py["(生产态 / production) 安全boundary /<br/>safety_boundary<br/>禁碰规则过滤 + 置信度阈值。输入<br/>TriggerResult 列表,输出 SafetyDecision<br/>分类。<br/>文件: semantic_audit/safety_boundary.py"]
    src_zephyr_governance_semantic_audit_self_healer_py["(生产态 / production) self愈合器 / self_<br/>healer<br/>Stage 7 自愈闭环 — 修复->自测->回滚.<br/>文件: semantic_audit/self_healer.py"]
    src_zephyr_governance_semantic_audit_self_health_py["(生产态 / production) 7 SLI + 5 容量<br/>SLI + 退化检测。定时自检,输出 HEALTHY/<br/>/ self_health<br/>7 SLI + 5 容量 SLI +<br/>退化检测。定时自检,输出 HEALTHY<br/>/DEGRADED/CRITICAL。<br/>文件: semantic_audit/self_health.py"]
    src_zephyr_governance_semantic_audit_trigger_engine_py["(生产态 / production)<br/>监听文件变更，判定是否触发语义审计。 /<br/>trigger_engine<br/>监听文件变更，判定是否触发语义审计。<br/>文件: semantic_audit/trigger_engine.py"]
    ...
    src_zephyr_gov_audit_delegation_bridge_py["(生产态 / production) delegation桥接 /<br/>delegation_bridge<br/>delegation桥接，主要提供报告delegationfa<br/>ilure、报告delegation超时、isavailable等<br/>功能，供audit-orchestrator.delegation_<br/>使用<br/>文件: gov_audit/delegation_bridge.py"]
    src_zephyr_gov_audit_feedback_bridge_py["(生产态 / production) 反馈桥接 /<br/>feedback_bridge<br/>反馈桥接。Bridge between audit-trail<br/>anomaly findings and the Feedback Loop<br/>Engine.<br/>文件: gov_audit/feedback_bridge.py"]
    src_zephyr_gov_audit_finding_ingest_py["(生产态 / production) 发现ingest /<br/>finding_ingest<br/>发现ingest，审计的结果，封装操作结果的数<br/>据结构。<br/>文件: gov_audit/finding_ingest.py"]
    src_zephyr_gov_audit_indexer_py["(生产态 / production)<br/>索引重建结果——治本（裁定#18 G5）：对齐<br/>testa / indexer<br/>索引重建结果——治本（裁定#18 G5）：对齐<br/>test_audit_indexer.py 契约。<br/>文件: gov_audit/indexer.py"]
    src_zephyr_gov_audit_merkle_hourly_py["(生产态 / production)<br/>audit-trail.merkle每小时 / merkle_hourly<br/>audit-trail.merkle每小时.merkle每小时·<br/>每小时 Merkle 聚合<br/>文件: gov_audit/merkle_hourly.py"]
    src_zephyr_gov_audit_models_py["(生产态 / production)<br/>审计事件类型枚举——治本（裁定#18<br/>G2）：转为真 Enu / models<br/>审计事件类型枚举——治本（裁定#18<br/>G2）：转为真 Enum，values 全部小写。<br/>文件: gov_audit/models.py"]
    src_zephyr_gov_audit_tiered_storage_bridge_py["(生产态 / production) tiered存储桥接 /<br/>tiered_storage_bridge<br/>tieredstorage桥接，主要提供find报告、mig<br/>rate、stats等功能，供audit-orchestrator.<br/>bridge; ret使用<br/>文件: gov_audit/tiered_storage_bridge.py"]
    src_zephyr_gov_audit_trust_bridge_py["(生产态 / production) 信任桥接 / trust_<br/>bridge<br/>信任桥接，主要提供评估、记录、获取趋势等<br/>功能，供audit-orchestrator.bridge;<br/>int使用<br/>文件: gov_audit/trust_bridge.py"]
    src_zephyr_governance_audit_health_score_calculator_py["(生产态 / production) 健康评分计算器 /<br/>health_score_calculator<br/>commit gateway 滥用 6 维加权健康度评分<br/>（P3-2，#ARCH-PREVENTABILITY-LAYER-001<br/>Phase 3）。<br/>文件: audit/health_score_calculator.py"]
    src_zephyr_governance_audit_reconcile_runner_py["(生产态 / production) 对账运行器 /<br/>reconcile_runner<br/>Reconciler 链路异步化<br/>（Ruling:100PCT-AI-GOVERNANCE<br/>P2-3，2026-07-19）<br/>文件: audit/reconcile_runner.py"]
    src_zephyr_governance_semantic_audit_reference_extractor_py["(生产态 / production) AST<br/>解析文件，提取 9 个维度的引用信息。 /<br/>reference_extractor<br/>AST 解析文件，提取 9 个维度的引用信息。<br/>文件: semantic_audit/reference_<br/>extractor.py"]
    ...
    src_zephyr_gov_audit_contracts_py["(生产态 / production) 契约 / contracts<br/>核心审计链写入器——桥接 contracts 层到<br/>writer 实现。<br/>文件: gov_audit/contracts.py"]
    src_zephyr_gov_audit_finding_model_py["(生产态 / production) 发现模型 /<br/>finding_model<br/>发现模型，审计的模型，定义数据结构和字段<br/>。<br/>文件: gov_audit/finding_model.py"]
    ...
    src_zephyr_gov_audit_tiered_storage_py["(生产态 / production) 旧版分层存储<br/>（保留以兼容现有调用方）。 / tiered_<br/>storage<br/>旧版分层存储（保留以兼容现有调用方）。<br/>文件: gov_audit/tiered_storage.py"]
    ...
    src_zephyr_gov_audit_writer_py["(生产态 / production)<br/>不可变审计写入器——JSONL 追加 + SHA-256<br/>哈 / writer<br/>不可变审计写入器——JSONL 追加 + SHA-256<br/>哈希链 + HMAC-SHA256 签名 + Lamport<br/>时钟。<br/>文件: gov_audit/writer.py"]
    src_zephyr_governance_audit_reconciliation_registry_py["(生产态 / production) 对账注册表 /<br/>reconciliation_registry<br/>GitCommitGateway post-commit<br/>漂移对账注册表<br/>文件: audit/reconciliation_registry.py"]
    src_zephyr_governance_semantic_audit_models_py["(生产态 / production)<br/>语义审计管线数据模型 — MOD-INF-028 §4.2<br/>/ models<br/>语义审计管线数据模型 — MOD-INF-028 §4.2<br/>文件: semantic_audit/models.py"]
    ...
    src_zephyr_gov_audit_agent_signer_py["(生产态 / production) 代理signer /<br/>agent_signer<br/>代理signer.agent_signer — MOD-INF-020 ·<br/>Agent Ed25519 签名器<br/>文件: gov_audit/agent_signer.py"]
    ...
    D_TRADING["(生产态 / production) 交易运营 /<br/>Trading Operations<br/>交易运营，负责交易生命周期管理、订单状态<br/>和成交处理<br/>跨域节点 / cross-domain"]
