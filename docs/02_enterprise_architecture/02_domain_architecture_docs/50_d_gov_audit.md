...
flowchart TD
    ...
    scripts_governance_repair_red_blue_test_py["(INVARIANTS) 20项红蓝对抗测试<br/>repair包的red_blue_test模块<br/>文件: repair/red_blue_test.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_audit_dora_metrics_py["dora指标<br/>gov audit包的dora_metrics模块<br/>文件: gov_audit/dora_metrics.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_audit_incremental_review_py["incremental审查<br/>gov audit包的incremental_review模块<br/>文件: gov_audit/incremental_review.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_audit_replay_engine_py["重放快照（补全测试期望接口）。<br/>gov audit包的replay_engine模块<br/>文件: gov_audit/replay_engine.py<br/>(生产态 / production)"]
    src_zephyr_gov_audit_retention_py["保留策略（补全测试期望接口）。<br/>gov audit包的retention模块<br/>文件: gov_audit/retention.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_audit_spec_auditor_py["spec审计器<br/>供zephyr.gov_audit使用<br/>spec_auditor<br/>文件: gov_audit/spec_auditor.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_audit_supply_chain_security_py["supplychain安全<br/>gov audit包的supply_chain_security模块<br/>文件: gov_audit/supply_chain_security.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_enforcement_behavioral_admission_ai_code_standards_py["ai代码standards<br/>（ai_code_standards.py）<br/>文件: behavioral_admission/ai_code_standards.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_enforcement_rule_enforcement_sys_master_compliance_py["sys主合规<br/>SYS-MASTER-001 Compliance Checker<br/>文件: rule_enforcement/sys_master_compliance.py<br/>(生产态 / production)"]
    src_zephyr_governance_audit_trail_contracts_py["契约<br/>py — G-CT-002 Audit 契约（re-export）<br/>contracts<br/>文件: audit-trail/contracts.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_audit_default_attribution_engine_py["默认attribution引擎<br/>Re-export wrapper: default_attribution_engine<br/>canonical at<br/>zephyr.reporting.default_attribution_engi<br/>文件: audit/default_attribution_engine.py<br/>(生产态 / production)"]
    src_zephyr_governance_audit_default_tca_engine_py["默认tca引擎<br/>Re-export wrapper: default_tca_engine canonical<br/>at zephyr.reporting.default_tca_engine<br/>文件: audit/default_tca_engine.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_audit_runtime_violation_snapshot_reconciler_py["运行时违规快照协调器<br/>trae_060 §5 evidence 运行时快照 post-commit<br/>reconciler。<br/>runtime_violation_snapshot_reconciler<br/>文件: audit<br/>/runtime_violation_snapshot_reconciler.py<br/>(生产态 / production)"]
    ...
    tests_governance_rule_bridge_test_session_worktree_async_reconcile_py["测试会话worktree异步对账<br/>_run_reconcilers_after_merge 异步化测试。<br/>test_session_worktree_async_reconcile<br/>文件: rule_bridge<br/>/test_session_worktree_async_reconcile.py<br/>(生产态 / production)"]
    tests_governance_test_workspace_telemetry_shared_py["测试工作区遥测共享<br/>shared workspace_telemetry 公共 API 单测<br/>test_workspace_telemetry_shared<br/>文件: governance<br/>/test_workspace_telemetry_shared.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_audit_query_py["旧版查询引擎（保留以兼容现有调用方）。<br/>gov audit包的query模块<br/>文件: gov_audit/query.py<br/>(生产态 / production)"]
    ...
    src_zephyr_governance_audit_commit_gateway_abuse_monitor_reconciler_py["commitgatewayabuse监控器对账器<br/>commit gateway 持续滥用监控<br/>（ARCH-TOOL-HEALTH-V1 Phase 5b，2026-07-19）。<br/>commit_gateway_abuse_monitor_reconciler<br/>文件: audit<br/>/commit_gateway_abuse_monitor_reconciler.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_audit_feedback_bridge_py["反馈桥接<br/>Bridge between audit-trail anomaly findings and<br/>the Feedback Loop Engine<br/>feedback_bridge<br/>文件: gov_audit/feedback_bridge.py<br/>(生产态 / production)"]
    ...
    src_zephyr_gov_audit_tiered_storage_py["旧版分层存储（保留以兼容现有调用方）。<br/>gov audit包的tiered_storage模块<br/>文件: gov_audit/tiered_storage.py<br/>(生产态 / production)"]
    src_zephyr_gov_audit_trust_engine_py["信任评分调整记录（补全测试期望接口）。<br/>gov audit包的trust_engine模块<br/>文件: gov_audit/trust_engine.py<br/>(生产态 / production)"]
    ...
    class D_SHARED,D_GOV_CODE_QUALITY,D_GOV_SCRIPTS,D_GOVERNANCE,D_SECURITY,D_GOV_DRIFT,D_GOV_ENFORCEMENT,D_GOV_OPS_RESILIENCE,D_TRADING,D_AUTONOMY_CORE external_prod
```
