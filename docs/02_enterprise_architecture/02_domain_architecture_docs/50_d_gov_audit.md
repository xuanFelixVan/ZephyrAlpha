...
graph TD
    ...
    class D_SHARED,D_GOV_DRIFT,D_SECURITY,D_FBL_VERIFICATION,D_GOV_RULE,D_INFRA_RECOVERY,D_INFRA_RUNTIME,D_GOVERNANCE,D_AUTONOMY_CORE external_prod
```
...
graph TD
    ...
    class D_GOVERNANCE,D_GOV_OPS_RESILIENCE,D_SHARED,D_GOV_DRIFT,D_SECURITY,D_GOV_SCRIPTS,D_GOV_ENFORCEMENT,D_INFRA_RUNTIME,D_INFRA_RECOVERY,D_TRADING external_prod
```
...
graph TD
    ...
    class D_SHARED,D_REPORTING,D_SECURITY,D_AUTONOMY_CORE,D_GOV_ENFORCEMENT,D_GOV_DRIFT,D_INTEGRATION,D_GOVERNANCE,D_INFRA_RUNTIME,D_INFRA_RECOVERY external_prod
```
...
graph TD
    ...
    class D_SHARED,D_SECURITY,D_GOVERNANCE,D_GOV_ENFORCEMENT,D_GOV_CODE_QUALITY,D_INFRA_RUNTIME,D_GOV_SCRIPTS,D_INFRASTRUCTURE,D_INTEGRATION external_prod
```
...
graph TD
    ...
    class D_SHARED,D_GOVERNANCE,D_GOV_OPS_RESILIENCE,D_SECURITY,D_GOV_ENFORCEMENT,D_GOV_CODE_QUALITY,D_GOV_DRIFT,D_AUTONOMY_CORE,D_FBL_VERIFICATION,D_GOV_RULE external_prod
```
...
| 36 | [INVARIANTS] 按path精确匹配+按功能名模糊匹配; .... | → | D_GOV_SCRIPTS 脚本治理: concurrent_commit_test.py — 幽灵提交红蓝对抗脚... | config_depends / config_depends |
| 37 | reconciliation_registry.py — GitCommitGateway ... | → | D_GOV_SCRIPTS 脚本治理: module_id / domain_id / submodule_id 格式校验真... | 导入依赖 / import_depends |
...
| 70 | pipeline_runner.py | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP s... | 导入依赖 / import_depends |
| 71 | pipeline_runner.py | → | D_SHARED 共享服务: base_config.py | 导入依赖 / import_depends |
| 72 | query.py | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 73 | retention.py | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 74 | audit-trail.supply_chain — MOD-INF-020 · 供应... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP s... | 导入依赖 / import_depends |
| 75 | text_to_finding_adapter.py | → | D_SHARED 共享服务: base_config.py | 导入依赖 / import_depends |
| 76 | tiered_storage.py | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 77 | writer.py | → | D_SHARED 共享服务: serialization.py —— 统一序列化/反序列化基础设... | 导入依赖 / import_depends |
| 78 | writer.py | → | D_SHARED 共享服务: session_audit.py —— Session 审计轨迹（Phase 1... | 导入依赖 / import_depends |
| 79 | writer.py | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 80 | mcp_result_push.py | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 81 | post_process.py —— AI 生成代码后处理管道（Pha... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP s... | 导入依赖 / import_depends |
| 82 | 审计链验证工具——独立重放门禁判定+Hash链完整性... | → | D_SHARED 共享服务: serialization.py —— 统一序列化/反序列化基础设... | 导入依赖 / import_depends |
| 83 | SYS-MASTER-001 Compliance Checker (sys_master_c... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP s... | 导入依赖 / import_depends |
| 84 | SYS-MASTER-001 Compliance Checker (sys_master_c... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 85 | _git_helpers.py — audit reconciler 共享 git 工... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP s... | 导入依赖 / import_depends |
| 86 | blueprint_status_transition_reconciler.py — 蓝... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 87 | commit_gateway_abuse_monitor_reconciler.py — c... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP s... | 导入依赖 / import_depends |
| 88 | cross_layer_contract_signature_reconciler.py —... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 89 | git_performance_monitor_reconciler.py — git 性... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP s... | 导入依赖 / import_depends |
| 90 | reconcile_runner.py — Reconciler 链路异步化（R... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP s... | 导入依赖 / import_depends |
| 91 | reconcile_runner.py — Reconciler 链路异步化（R... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 92 | reconciliation_registry.py — GitCommitGateway ... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP s... | 导入依赖 / import_depends |
| 93 | reconciliation_registry.py — GitCommitGateway ... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 94 | reconciliation_registry.py — GitCommitGateway ... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 95 | remediation_progress_reconciler.py — 治本进度.... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 96 | runtime_violation_snapshot.py — trae_060 §5 e... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP s... | 导入依赖 / import_depends |
| 97 | SnapshotManager — Event Sourcing 快照管理（DW-... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 导入依赖 / import_depends |
| 98 | SnapshotManager — Event Sourcing 快照管理（DW-... | → | D_SHARED 共享服务: serialization.py —— 统一序列化/反序列化基础设... | 导入依赖 / import_depends |
| 99 | workspace_hygiene_reconciler.py — 工作区卫生自... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP s... | 导入依赖 / import_depends |
| 100 | 收集各阶段审计结果，去重合并排序输出。 (issue_a... | → | D_SHARED 共享服务: time_utils.py —— 时间/日期工具（Phase 9 新增 ... | 导入依赖 / import_depends |
| 101 | Stage 7 自愈闭环 — 修复->自测->回滚. (self_hea... | → | D_SHARED 共享服务: process_pool.py - Shared process pool for MCP s... | 导入依赖 / import_depends |
| 102 | Stage 7 自愈闭环 — 修复->自测->回滚. (self_hea... | → | D_SHARED 共享服务: yaml_utils.py — vocabulary YAML 加载公共工具（... | 导入依赖 / import_depends |
| 103 | test_workspace_telemetry_shared.py — shared wo... | → | D_SHARED 共享服务: paths.py — 项目路径常量 SSoT（Single Source of... | 测试依赖 / test_depends |
| 104 | test_workspace_telemetry_shared.py — shared wo... | → | D_SHARED 共享服务: workspace_telemetry.py — 主工作区文件操作遥测.... | 测试依赖 / test_depends |
...
> 本域与 18 个外部域直接连接（出边 104 条 + 入边 68 条 = 172 条）。只显示直接连接的域，不展开具体节点。
