...
| 95 | D_FACTOR 因子: 纯逻辑模块 / Test Lifecycle State Machine (factor/test_li... | → | State Machine (lifecycle/state_machine.py) | 测试依赖 / test_depends |
...
| 205 | D_GOVERNANCE 生命周期管理: Test Schema Schema Registry (integration/test_schema_sche... | → | schema 不存在、版本冲突、兼容性违规 / Schema Registry (sc... | 测试依赖 / test_depends |
| 206 | D_GOVERNANCE 生命周期管理: Test Schema Schemas (integration/test_schema_schemas.py) | → | 模式定义 / Schemas (schema/schemas.py) | 测试依赖 / test_depends |
...
| 336 | D_GOV_DRIFT 漂移检测: Truth Source Validator (rule_enforcement/truth_source_val... | → | 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 365 | D_GOV_OPS_RESILIENCE 运维弹性治理: Environment Manager (ops_governance/environment_manager.py) | → | 密钥 / Secrets (security/secrets.py) | 导入依赖 / import_depends |
...
| 452 | D_INFRA_A2A A2A通信: A2a State (layer2_communication/a2a_state.py) | → | A2A模式定义 / A2a Schemas (a2a/a2a_schemas.py) | 导入依赖 / import_depends |
...
| 458 | D_INFRA_A2A A2A通信: Multi Agent (a2a_protocol/multi_agent.py) | → | A2A协调 / A2a Coordination (a2a/a2a_coordination.py) | 导入依赖 / import_depends |
...
| 476 | D_INFRA_RECOVERY 回滚恢复: Rollback Integration (rollback/rollback_integration.py) | → | 密钥 / Secrets (security/secrets.py) | 导入依赖 / import_depends |
...
| 553 | D_INFRA_RUNTIME 运行时集成: Finding Task Bridge (infrastructure/finding_task_bridge.py) | → | 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 563 | D_INFRA_RUNTIME 运行时集成: Ct Pipe Routing (pipeline/ct_pipe_routing.py) | → | 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 566 | D_INFRA_RUNTIME 运行时集成: Llm Gateway (pipeline/llm_gateway.py) | → | 密钥 / Secrets (security/secrets.py) | 导入依赖 / import_depends |
...
| 569 | D_INFRA_RUNTIME 运行时集成: Models (pipeline/models.py) | → | 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 632 | D_INFRA_RUNTIME 运行时集成: 水平触发调和循环 / Health Monitor (trading/health_monitor... | → | Longevity Monitor (lifecycle/longevity_monitor.py) | 导入依赖 / import_depends |
...
| 641 | D_INFRA_RUNTIME 运行时集成: 端口 / Ports (trading/ports.py) | → | Task Types (schema/task_types.py) | 导入依赖 / import_depends |
...
| 659 | D_INTEGRATION 管线路由: 封装 /api/embed，兼容 SentenceTransformer.encode / Ollama... | → | Constants (foundation/constants.py) | 导入依赖 / import_depends |
...
| 675 | D_INTEGRATION 管线路由: Task Manager Server (mcp/task_manager_server.py) | → | 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 686 | D_INTEGRATION 管线路由: Pipeline Orchestrator (integration/pipeline_orchestrator.py) | → | 观察者 / Observer (infra/observer.py) | 导入依赖 / import_depends |
...
| 699 | D_INTEGRATION 管线路由: Dlq Bridge (events/dlq_bridge.py) | → | 观察者 / Observer (infra/observer.py) | 导入依赖 / import_depends |
...
| 703 | D_INTEGRATION 管线路由: Chunk Strategy Router (vector_memory/chunk_strategy_route... | → | 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 705 | D_INTEGRATION 管线路由: Collection Manager (vector_memory/collection_manager.py) | → | 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 707 | D_INTEGRATION 管线路由: Collection Schemas (vector_memory/collection_schemas.py) | → | 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 710 | D_INTEGRATION 管线路由: Index Health Monitor (vector_memory/index_health_monitor.py) | → | 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 728 | D_INTELLIGENCE 上下文管理: Deepseek V4 Chat (model_profiling/deepseek_v4_chat.py) | → | 密钥 / Secrets (security/secrets.py) | 导入依赖 / import_depends |
...
| 731 | D_INTELLIGENCE 上下文管理: 枚举所有可用模型 / Model Discovery (model_profiling/model... | → | Constants (foundation/constants.py) | 导入依赖 / import_depends |
...
| 756 | D_ORCHESTRATOR 代理编排器: Agent Orchestrator (orchestrator/agent_orchestrator.py) | → | 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 774 | D_ORCHESTRATOR 代理编排器: Task Context Builder (execution/task_context_builder.py) | → | 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 779 | D_ORCHESTRATOR 代理编排器: Chaos Hooks (fault_tolerance/chaos_hooks.py) | → | 编排协议 / Orchestration Protocol (contracts/orchestratio... | 导入依赖 / import_depends |
...
| 785 | D_ORCHESTRATOR 代理编排器: Hallucination Detector (orchestrator/hallucination_detect... | → | 模式定义 / Schemas (schema/schemas.py) | 导入依赖 / import_depends |
...
| 872 | D_SECURITY 对抗验证: L6可观测性 / L6 Observability (layers/l6_observability.py) | → | Security Decision (security/security_decision.py) | 导入依赖 / import_depends |
...
| 906 | D_TRADING 交易运营: 认领 + 冲突检测 + 并行分组 + 状态管理 / Conductor (tradin... | → | Constants (foundation/constants.py) | 导入依赖 / import_depends |
