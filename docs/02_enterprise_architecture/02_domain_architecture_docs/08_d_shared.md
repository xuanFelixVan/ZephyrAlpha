...
flowchart TD
    ...
    src_zephyr_shared_alerts_alert_escalation_py["alert升级<br/>告警escalation。AlertEscalation — re-homed to<br/>eliminate shared->infrastructure circular import<br/>文件: alerts/alert_escalation.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_alerts_dual_channel_alert_py["双通道告警<br/>alerts/dual_channel_alert 模块<br/>文件: alerts/dual_channel_alert.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_capacity_governance_adaptive_sampler_py["adaptive采样器<br/>capacity_governance/adaptive_sampler 模块<br/>文件: capacity_governance/adaptive_sampler.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_capacity_governance_capacity_fingerprint_py["容量指纹<br/>capacity_governance/capacity_fingerprint 模块<br/>文件: capacity_governance/capacity_<br/>fingerprint.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_contracts_errors_signal_degradation_warning_py["信号退化警告<br/>errors/signal_degradation_warning 模块<br/>文件: errors/signal_degradation_warning.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_escalation_budget_alert_py["预算告警<br/>escalation/budget_alert 模块<br/>文件: escalation/budget_alert.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_execution_capital_allocation_result_py["资本分配结果<br/>资本allocation结果<br/>Backward-compat shim — canonical location is<br/>zephyr.trading.<br/>文件: execution/capital_allocation_result.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_execution_execution_report_py["执行报告<br/>execution/execution_report 模块<br/>Backward-compat shim — canonical location is<br/>zephyr.trading.<br/>文件: execution/execution_report.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_execution_fill_py["成交<br/>execution/fill 模块<br/>Backward-compat shim — canonical location is<br/>zephyr.trading.<br/>文件: execution/fill.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_execution_model_serving_request_py["模型服务请求<br/>execution/model_serving_request 模块<br/>Backward-compat shim — canonical location is<br/>zephyr.trading.<br/>文件: execution/model_serving_request.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_execution_order_py["订单<br/>execution/order 模块<br/>文件: execution/order.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_contracts_identity_agent_identity_py["代理identity<br/>identity/agent_identity 模块<br/>文件: identity/agent_identity.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_contracts_market_instrument_py["标的合约<br/>（instrument.py）<br/>Backward-compat shim — canonical location is<br/>zephyr.trading.<br/>文件: market/instrument.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_contracts_portfolio_position_py["持仓<br/>portfolio/position 模块<br/>Backward-compat shim — canonical location is<br/>zephyr.trading.<br/>文件: portfolio/position.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_risk_compliance_rule_py["合规规则<br/>risk/compliance_rule 模块<br/>Backward-compat shim — canonical location is<br/>zephyr.trading.<br/>文件: risk/compliance_rule.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_risk_risk_dashboard_snapshot_py["风险仪表盘快照<br/>risk/risk_dashboard_snapshot 模块<br/>Backward-compat shim — canonical location is<br/>zephyr.trading.<br/>文件: risk/risk_dashboard_snapshot.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_risk_risk_limits_py["风险limits<br/>risk/risk_limits 模块<br/>Backward-compat shim — canonical location is<br/>zephyr.trading.<br/>文件: risk/risk_limits.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_risk_risk_metrics_py["风险指标<br/>risk/risk_metrics 模块<br/>Backward-compat shim — canonical location is<br/>zephyr.trading.<br/>文件: risk/risk_metrics.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_risk_risk_validator_protocol_py["风险校验器协议<br/>risk/risk_validator_protocol 模块<br/>Backward-compat shim — canonical location is<br/>zephyr.trading.<br/>文件: risk/risk_validator_protocol.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_foundation_migration_py["迁移<br/>依赖迁移工作<br/>migration.py —— Re-export wrapper -> canonical:<br/>zephyr.share<br/>文件: foundation/migration.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_io_streaming_reader_py["流式读取器<br/>IO的读取器，读取数据流<br/>streaming_reader.py - Memory-efficient<br/>streaming file reader<br/>文件: io/streaming_reader.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_lifecycle_task_heartbeat_py["任务heartbeat<br/>lifecycle/task_heartbeat 模块<br/>文件: lifecycle/task_heartbeat.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_protocols_a2a_a2a_coordination_py["A2A协调<br/>提供fromstring等方法<br/>A2A Coordination — shared interface definitions<br/>for multi-ag<br/>文件: a2a/a2a_coordination.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_protocols_capability_py["能力<br/>依赖能力工作<br/>capability.py —— Re-export wrapper -><br/>canonical: zephyr.shar<br/>文件: protocols/capability.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_resilience_limiter_py["限制器<br/>依赖限制器工作<br/>limiter.py —— Re-export wrapper -> canonical:<br/>zephyr.shared.<br/>文件: resilience/limiter.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_security_idempotency_py["幂等性<br/>依赖幂等性工作<br/>idempotency.py —— Re-export wrapper -><br/>canonical: zephyr.sha<br/>文件: security/idempotency.py<br/>(生产态 / production)"]
    src_zephyr_shared_security_lock_py["锁<br/>依赖zephyr.shared.infra.lock<br/>lock.py —— Re-export wrapper -> canonical:<br/>zephyr.shared.inf<br/>文件: security/lock.py<br/>(生产态 / production)"]
    src_zephyr_shared_security_sandbox_executor_py["沙箱执行器<br/>sandbox执行器。SandboxExecutor — re-homed to<br/>eliminate shared->infrastructure circular import<br/>文件: security/sandbox_executor.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_events_observer_py["观察者<br/>事件的服务端，接收并处理请求<br/>observer.py —— Re-export wrapper -> canonical:<br/>zephyr.shared<br/>文件: events/observer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_infra_process_pool_py["进程池<br/>返回 Windows 无窗口 creationflags；POSIX 返回 0<br/>process_pool.py - Shared process pool for MCP<br/>servers and su<br/>文件: infra/process_pool.py<br/>(生产态 / production)"]
    ...
    class D_FEEDBACK_LOOP,D_INFRA_RUNTIME,D_GOV_RULE,D_INFRASTRUCTURE,D_ML_TRAIN,D_DATA,D_INTEGRATION,D_SELL_DECISION,D_PF_ALLOC,D_POSITION,D_GOV_CODE_QUALITY,D_GOVERNANCE,D_AUTONOMY_CORE external_prod
```
