...
flowchart TD
    ...
    src_zephyr_shared_blueprint_tools_blueprint_scorer_py["蓝图评分器<br/>蓝图评分器，提供包入口和模块加载功能<br/>blueprint_scorer.py — Re-export wrapper -><br/>canonical: zephyr<br/>文件: blueprint_tools/blueprint_scorer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_contracts_enums_init_py["contracts/enums 包入口<br/>shared/contracts/enums — 跨切面交易枚举真源<br/>(5.152 #1 修复)<br/>__init__<br/>文件: enums/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_contracts_market_instrument_py["标的合约<br/>标的合约（instrument.py）<br/>Backward-compat shim — canonical location is<br/>zephyr.trading.<br/>文件: market/instrument.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_orchestration_protocol_py["orchestration协议<br/>orchestration_protocol<br/>文件: contracts/orchestration_protocol.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_database_init_py["shared/database 包入口<br/>共享数据库工具包：提供 DatabaseService 共用的<br/>CRUD mixin。<br/>__init__<br/>文件: database/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_foundation_migration_py["迁移<br/>迁移，依赖迁移工作<br/>migration.py —— Re-export wrapper -> canonical:<br/>zephyr.share<br/>文件: foundation/migration.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_io_streaming_reader_py["流式读取器<br/>流式读取器，IO的读取器，读取数据流。<br/>streaming_reader.py - Memory-efficient<br/>streaming file reader<br/>文件: io/streaming_reader.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_lifecycle_healthcheck_service_py["healthcheck服务<br/>healthcheck_service<br/>文件: lifecycle/healthcheck_service.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_observability_dashboard_init_py["observability/dashboard 包入口<br/>Grafana 双数据源仪表盘模块（MOD-INF-044）。<br/>__init__<br/>文件: dashboard/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_protocols_a2a_a2a_coordination_py["A2A协调<br/>A2A协调，提供fromstring等方法，供zephyr.shared.p<br/>rotocols.a使用<br/>A2A Coordination — shared interface definitions<br/>for multi-ag<br/>文件: a2a/a2a_coordination.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_protocols_capability_py["能力<br/>能力，依赖能力工作<br/>capability.py —— Re-export wrapper -><br/>canonical: zephyr.shar<br/>文件: protocols/capability.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_resilience_limiter_py["限制器<br/>限制器，依赖限制器工作<br/>limiter.py —— Re-export wrapper -> canonical:<br/>zephyr.shared.<br/>文件: resilience/limiter.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_security_idempotency_py["幂等性<br/>幂等性，依赖幂等性工作<br/>idempotency.py —— Re-export wrapper -><br/>canonical: zephyr.sha<br/>文件: security/idempotency.py<br/>(生产态 / production)"]
    src_zephyr_shared_security_lock_py["锁<br/>锁，依赖zephyr.shared.infra.lock<br/>lock.py —— Re-export wrapper -> canonical:<br/>zephyr.shared.inf<br/>文件: security/lock.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_events_observer_py["观察者<br/>观察者，事件的服务端，接收并处理请求。<br/>observer.py —— Re-export wrapper -> canonical:<br/>zephyr.shared<br/>文件: events/observer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_infra_process_pool_py["进程池<br/>返回 Windows 无窗口 creationflags；POSIX 返回<br/>0。<br/>process_pool.py - Shared process pool for MCP<br/>servers and su<br/>文件: infra/process_pool.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_schema_severity_types_py["severity类型<br/>severity_types<br/>文件: schema/severity_types.py<br/>(生产态 / production)"]
    ...
    D_GOV_RULE -->|导入依赖 / import_depends| src_zephyr_shared_schema_schemas_py
    ...
    class D_FEEDBACK_LOOP,D_INFRA_RUNTIME,D_GOV_RULE,D_INFRASTRUCTURE,D_ML_TRAIN,D_SELL_DECISION,D_PF_ALLOC,D_POSITION,D_GOV_CODE_QUALITY,D_GOVERNANCE,D_AUTONOMY_CORE,D_GOV_SCRIPTS external_prod
```
...
| 745 | D_SELL_DECISION 卖出决策: Replacement & Rebalance Seller — 置换与再平衡卖出 (MOD-S... | → | 错误 / errors (foundation/errors.py) | 导入依赖 / import_depends |
| 746 | D_SELL_DECISION 卖出决策: Sell Conflict Arbitrator — 买卖冲突仲裁器 (MOD-SELL-008)... | → | 错误 / errors (foundation/errors.py) | 导入依赖 / import_depends |
| 747 | D_TRADING 交易运营: 包入口 / __init__ (action_dispatcher/__init__.py) | → | 进程池 / process_pool.py - Shared process pool for MCP se... | 导入依赖 / import_depends |
| 748 | D_TRADING 交易运营: 包入口 / __init__ (action_dispatcher/__init__.py) | → | paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 749 | D_TRADING 交易运营: 包入口 / __init__ (action_dispatcher/__init__.py) | → | serialization.py —— 统一序列化/反序列化基础设施（Phase ... | 导入依赖 / import_depends |
| 750 | D_TRADING 交易运营: 包入口 / __init__ (action_dispatcher/__init__.py) | → | 任务类型定义 / task_types (schema/task_types.py) | 导入依赖 / import_depends |
| 751 | D_TRADING 交易运营: 自动分发器 / auto_dispatcher (trading/auto_dispatcher.py) | → | 任务仓库协议 / task_repository_protocol (contracts/task_r... | 导入依赖 / import_depends |
| 752 | D_TRADING 交易运营: 自动分发器 / auto_dispatcher (trading/auto_dispatcher.py) | → | 常量 / constants (foundation/constants.py) | 导入依赖 / import_depends |
| 753 | D_TRADING 交易运营: AutoPilot — AI session 自动找活干、认领任务。 / autopilo... | → | 任务仓库协议 / task_repository_protocol (contracts/task_r... | 导入依赖 / import_depends |
| 754 | D_TRADING 交易运营: AutoPilot — AI session 自动找活干、认领任务。 / autopilo... | → | 事件总线 / event_bus (shared/event_bus.py) | 导入依赖 / import_depends |
| 755 | D_TRADING 交易运营: AutoPilot — AI session 自动找活干、认领任务。 / autopilo... | → | 常量 / constants (foundation/constants.py) | 导入依赖 / import_depends |
| 756 | D_TRADING 交易运营: AutoPilot — AI session 自动找活干、认领任务。 / autopilo... | → | 模型 / models (foundation/models.py) | 导入依赖 / import_depends |
| 757 | D_TRADING 交易运营: Conductor — AI session 全自动指挥官。 / conductor (tradi... | → | 常量 / constants (foundation/constants.py) | 导入依赖 / import_depends |
| 758 | D_TRADING 交易运营: Conductor — AI session 全自动指挥官。 / conductor (tradi... | → | 模型 / models (foundation/models.py) | 导入依赖 / import_depends |
| 759 | D_TRADING 交易运营: GPU共识调度器 / gpu_consensus_scheduler (trading/gpu_cons... | → | 常量 / constants (foundation/constants.py) | 导入依赖 / import_depends |
| 760 | D_TRADING 交易运营: GPU监控 / gpu_monitor (trading/gpu_monitor.py) | → | 进程池 / process_pool.py - Shared process pool for MCP se... | 导入依赖 / import_depends |
| 761 | D_TRADING 交易运营: ide健康daemon / ide_health_daemon (trading/ide_health_dae... | → | 任务仓库协议 / task_repository_protocol (contracts/task_r... | 导入依赖 / import_depends |
| 762 | D_TRADING 交易运营: ide健康daemon / ide_health_daemon (trading/ide_health_dae... | → | 事件总线 / event_bus (shared/event_bus.py) | 导入依赖 / import_depends |
| 763 | D_TRADING 交易运营: ide健康daemon / ide_health_daemon (trading/ide_health_dae... | → | 常量 / constants (foundation/constants.py) | 导入依赖 / import_depends |
| 764 | D_TRADING 交易运营: ide健康daemon / ide_health_daemon (trading/ide_health_dae... | → | 进程池 / process_pool.py - Shared process pool for MCP se... | 导入依赖 / import_depends |
| 765 | D_TRADING 交易运营: ide健康daemon / ide_health_daemon (trading/ide_health_dae... | → | 时间工具 / time_utils (utils/time_utils.py) | 导入依赖 / import_depends |
| 766 | D_TRADING 交易运营: 异步运行时 / async_runtime (runtime/async_runtime.py) | → | 异步工具 / async_utils (utils/async_utils.py) | 导入依赖 / import_depends |
| 767 | D_TRADING 交易运营: 测速基线检查器 / speed_baseline_checker (trading/speed_ba... | → | paths.py — 项目路径常量 SSoT（Single Source of  / paths ... | 导入依赖 / import_depends |
| 768 | D_TRADING 交易运营: 订单 / order (execution/order.py) | → | 订单枚举 / order_enums (enums/order_enums.py) | 导入依赖 / import_depends |
| 769 | D_TRADING 交易运营: 过渡兼容层（DEPRECATED）—— Money 契约 canonical 真 / mo... | → | 金额精度错误（如试图用 float 构造 Money）。 / money (port... | 导入依赖 / import_depends |
