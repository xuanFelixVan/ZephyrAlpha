...
flowchart TD
    ...
    src_zephyr_shared_alerts_dual_channel_alert_py["双通道告警<br/>共享层包的dual_channel_alert模块<br/>文件: alerts/dual_channel_alert.py<br/>(生产态 / production)"]
    src_zephyr_shared_alerts_heartbeat_server_py["heartbeat服务端<br/>共享层包的heartbeat_server模块<br/>文件: alerts/heartbeat_server.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_blueprint_tools_blueprint_scorer_py["蓝图评分器<br/>共享层包的blueprint_scorer模块<br/>blueprint_scorer.py — Re-export wrapper -><br/>canonical: zephyr<br/>文件: blueprint_tools/blueprint_scorer.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_capacity_governance_capacity_fingerprint_py["容量指纹<br/>供zephyr.trading.resource_optimi使用<br/>capacity_fingerprint<br/>文件: capacity_governance/capacity_<br/>fingerprint.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_capacity_governance_model_capacity_probe_py["模型容量probe<br/>共享层包的model_capacity_probe模块<br/>文件: capacity_governance/model_capacity_<br/>probe.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_contracts_backpressure_pause_py["暂停<br/>共享层包的pause模块<br/>文件: backpressure/pause.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_backpressure_resume_py["恢复<br/>共享层包的resume模块<br/>文件: backpressure/resume.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_backpressure_throttle_py["限流器<br/>共享层包的throttle模块<br/>文件: backpressure/throttle.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_contracts_enums_init_py["contracts/enums 包入口<br/>管理contracts.enums子包加载<br/>文件: enums/__init__.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_errors_contract_violation_error_py["契约违规错误<br/>共享层包的contract_violation_error模块<br/>文件: errors/contract_violation_error.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_errors_data_quality_error_py["数据质量错误<br/>共享层包的data_quality_error模块<br/>文件: errors/data_quality_error.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_errors_execution_rejection_error_py["执行拒绝错误<br/>共享层包的execution_rejection_error模块<br/>文件: errors/execution_rejection_error.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_errors_factor_computation_error_py["因子computation错误<br/>共享层包的factor_computation_error模块<br/>文件: errors/factor_computation_error.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_errors_risk_limit_violation_error_py["风险限制违规错误<br/>共享层包的risk_limit_violation_error模块<br/>文件: errors/risk_limit_violation_error.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_errors_signal_degradation_warning_py["信号退化警告<br/>共享层包的signal_degradation_warning模块<br/>文件: errors/signal_degradation_warning.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_contracts_execution_execution_report_py["执行报告<br/>共享层包的execution_report模块<br/>文件: execution/execution_report.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_execution_fill_py["成交<br/>共享层包的fill模块<br/>文件: execution/fill.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_execution_model_serving_request_py["模型服务请求<br/>共享层包的model_serving_request模块<br/>文件: execution/model_serving_request.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_execution_order_py["订单<br/>共享层包的order模块<br/>文件: execution/order.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_contracts_market_instrument_py["标的合约<br/>（instrument.py）<br/>文件: market/instrument.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_contracts_portfolio_money_py["金额精度错误（如试图用 float 构造 Money）。<br/>共享层包的money模块<br/>文件: portfolio/money.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_portfolio_performance_attribution_report_py["绩效attribution报告<br/>共享层包的performance_attribution_report模块<br/>文件: portfolio/performance_attribution_<br/>report.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_portfolio_position_py["持仓<br/>共享层包的position模块<br/>文件: portfolio/position.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_risk_compliance_rule_py["合规规则<br/>共享层/风控包的compliance_rule模块<br/>文件: risk/compliance_rule.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_risk_risk_dashboard_snapshot_py["风险仪表盘快照<br/>共享层/风控包的risk_dashboard_snapshot模块<br/>文件: risk/risk_dashboard_snapshot.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_risk_risk_limits_py["风险limits<br/>共享层/风控包的risk_limits模块<br/>文件: risk/risk_limits.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_risk_risk_metrics_py["风险指标<br/>共享层/风控包的risk_metrics模块<br/>文件: risk/risk_metrics.py<br/>(生产态 / production)"]
    src_zephyr_shared_contracts_risk_risk_validator_protocol_py["风险校验器协议<br/>共享层/风控包的risk_validator_protocol模块<br/>文件: risk/risk_validator_protocol.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_database_init_py["shared/database 包入口<br/>管理shared.database子包加载<br/>文件: database/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_draft_draft_assistant_py["Draft Assistant — 想法 -> MTH-012 蓝图骨架生成。<br/>共享层包的draft_assistant模块<br/>文件: draft/draft_assistant.py<br/>(生产态 / production)"]
    src_zephyr_shared_events_dlq_bridge_py["dlq桥接<br/>共享层/事件包的dlq_bridge模块<br/>CT-DLQ-001: DeadLetterQueue -> System Event Bus<br/>integration<br/>文件: events/dlq_bridge.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_lifecycle_healthcheck_service_py["healthcheck服务<br/>共享层包的healthcheck_service模块<br/>文件: lifecycle/healthcheck_service.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_lifecycle_task_heartbeat_py["任务heartbeat<br/>共享层包的task_heartbeat模块<br/>文件: lifecycle/task_heartbeat.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_maintenance_code_economy_analyzer_py["代码economy分析器<br/>共享层包的code_economy_analyzer模块<br/>文件: maintenance/code_economy_analyzer.py<br/>(生产态 / production)"]
    src_zephyr_shared_maintenance_dogfooding_py["Dogfooding — 自举测试：用 TaskCard 管理 TaskCard<br/>共享层包的dogfooding模块<br/>文件: maintenance/dogfooding.py<br/>(生产态 / production)"]
    src_zephyr_shared_maintenance_handbook_py["Onboarding Handbook — AI Agent 施工手册生成。<br/>共享层包的handbook模块<br/>文件: maintenance/handbook.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_maintenance_slo_review_assistant_py["SLO审查assistant<br/>共享层包的slo_review_assistant模块<br/>文件: maintenance/slo_review_assistant.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_observability_dashboard_init_py["observability/dashboard 包入口<br/>管理observability.dashboard子包加载<br/>文件: dashboard/__init__.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_protocols_a2a_a2a_coordination_py["A2A协调<br/>提供fromstring等方法，供zephyr.shared.protocols.<br/>a使用<br/>A2A Coordination — shared interface definitions<br/>for multi-ag<br/>文件: a2a/a2a_coordination.py<br/>(生产态 / production)"]
    ...
    src_zephyr_shared_resilience_fault_isolator_py["故障隔离器<br/>共享层包的fault_isolator模块<br/>文件: resilience/fault_isolator.py<br/>(生产态 / production)"]
