...
graph TD
    ...
    class D_GOVERNANCE,D_TRADING,D_INFRA_RUNTIME,D_GOV_OPS_RESILIENCE,D_GOV_DRIFT external_prod
    class D_INFRA_RECOVERY,D_GOV_ENFORCEMENT,D_GOV_AUDIT,D_GOV_SCRIPTS,D_AUTONOMY_CORE external_design
```
...
graph TD
    ...
    classDef external_design fill:#fce4ec,stroke:#880e4f,stroke-width:1px,color:#000,stroke-dasharray: 5 5
    class src_zephyr_governance_init_py,src_zephyr_governance_financial_governance_budget_enforcement_py,src_zephyr_governance_persistence_init_py production
    class D_INFRA_RUNTIME,D_GOVERNANCE,D_GOV_OPS_RESILIENCE,D_GOV_DRIFT external_prod
    class D_INFRA_RECOVERY,D_GOV_ENFORCEMENT,D_GOV_AUDIT,D_GOV_SCRIPTS,D_AUTONOMY_CORE external_design
```
...
> 本域与 16 个外部域直接连接（出边 124 条 + 入边 17 条 = 141 条）。只显示直接连接的域，不展开具体节点。
