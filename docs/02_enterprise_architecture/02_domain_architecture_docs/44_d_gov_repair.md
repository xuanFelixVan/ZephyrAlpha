...
| 2 | src/zephyr/governance/adapters/__init__.py | __init__.py | 原型态 / prototype | [MOD-L06-001](../../03_modules/_domain_execution_core/blueprint.md) |
...
> 展示全部 16 个模块（生产态 3 + 设计态 0 + 原型态 13），标签标注成熟度。
...
graph TD
    ...
    class D_GOVERNANCE,D_FACTOR,D_OPS,D_GOV_AUDIT,D_GOV_ENFORCEMENT,D_INFRA_RUNTIME,D_GOV_OPS_RESILIENCE external_prod
...
graph TD
    ...
    classDef external_design fill:#fce4ec,stroke:#880e4f,stroke-width:1px,color:#000,stroke-dasharray: 5 5
    class src_zephyr_governance_init_py,src_zephyr_governance_financial_governance_budget_enforcement_py,src_zephyr_governance_persistence_init_py production
    class D_FACTOR,D_GOV_AUDIT,D_GOV_ENFORCEMENT,D_GOVERNANCE,D_INFRA_RUNTIME,D_GOV_OPS_RESILIENCE external_prod
    class D_GOV_SCRIPTS external_design
```
...
> 仅展示代码已写、验证中未稳定上线的原型态模块（共 13 个，0 条域内依赖）。
...
| 31 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOVERNANCE 生命周期管理: DatabaseService 真源收敛（AI-14 审计 P1 修复） ... | 导入依赖 / import_depends |
...
| 34 | Agent Spec — MOD-INF-019 (__init__.py) | → | D_GOVERNANCE 生命周期管理: G-CT-003 契约：Agent Spec -> RBAC 能力检查. (re... | 导入依赖 / import_depends |
| 35 | __init__.py | → | D_GOVERNANCE 生命周期管理: G-CT-007 — Audit.record_agent_spec() 记录 Agen... | config_depends / config_depends |
| 36 | __init__.py | → | D_GOVERNANCE 生命周期管理: context_budget.py —— 上下文预算管理与超预算截... | config_depends / config_depends |
...
| 41 | __init__.py | → | D_GOVERNANCE 生命周期管理: ai_self_diagnosis.py | config_depends / config_depends |
| 42 | __init__.py | → | D_GOVERNANCE 生命周期管理: post_live_verification.py | config_depends / config_depends |
| 43 | __init__.py | → | D_GOVERNANCE 生命周期管理: QueryMetrics — SQL 查询性能监控装饰器（SH-DB-0... | config_depends / config_depends |
...
| 54 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_AUDIT 审计追踪: finding_ingest.py | 导入依赖 / import_depends |
| 55 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_AUDIT 审计追踪: glossary_matrix.py | 导入依赖 / import_depends |
| 56 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_AUDIT 审计追踪: audit-trail.kb_gate — MOD-INF-020 · KB 审计门... | 导入依赖 / import_depends |
| 57 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_AUDIT 审计追踪: audit-trail.privacy — MOD-INF-020 · PII 检测... | 导入依赖 / import_depends |
| 58 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_AUDIT 审计追踪: LicenseType 枚举——许可证类型定义（P3 价值审判... | 导入依赖 / import_depends |
| 59 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_AUDIT 审计追踪: spec_auditor.py | 导入依赖 / import_depends |
| 60 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_AUDIT 审计追踪: audit-trail.supply_chain — MOD-INF-020 · 供应... | 导入依赖 / import_depends |
| 61 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_AUDIT 审计追踪: wqa_scorer.py | 导入依赖 / import_depends |
| 62 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_AUDIT 审计追踪: ai_code_standards.py | 导入依赖 / import_depends |
| 63 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_AUDIT 审计追踪: mcp_result_push.py | 导入依赖 / import_depends |
| 64 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_AUDIT 审计追踪: post_process.py —— AI 生成代码后处理管道（Pha... | 导入依赖 / import_depends |
| 65 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_AUDIT 审计追踪: vibe_coding_enforcer.py | 导入依赖 / import_depends |
| 66 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_AUDIT 审计追踪: SnapshotManager — Event Sourcing 快照管理（DW-... | 导入依赖 / import_depends |
| 67 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_AUDIT 审计追踪: financial_compliance.py | 导入依赖 / import_depends |
| 68 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_AUDIT 审计追踪: fix_prioritizer — MOD-INF-028 §3.1 Stage 8 (f... | 导入依赖 / import_depends |
| 69 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_AUDIT 审计追踪: Stage 7 自愈闭环 — 修复->自测->回滚. (self_hea... | 导入依赖 / import_depends |
| 70 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_AUDIT 审计追踪: 7 SLI + 5 容量 SLI + 退化检测。定时自检,输出 HE... | 导入依赖 / import_depends |
| 71 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_CODE_QUALITY 代码质量治理: 金丝雀工厂——生成已知oracle 文件 用于引擎检出+... | 导入依赖 / import_depends |
| 72 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_CODE_QUALITY 代码质量治理: code-dedup-engine CLI——子命令映射+退出码+扫描... | 导入依赖 / import_depends |
| 73 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_CODE_QUALITY 代码质量治理: 6Phase施工执行器 — Phase 0~5 执行状态追踪. (ph... | 导入依赖 / import_depends |
| 74 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_CODE_QUALITY 代码质量治理: 盲点关闭追踪器 — 自动验证各轮盲点是否已覆盖. (... | 导入依赖 / import_depends |
| 75 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_DRIFT 漂移检测: benchmark_integrity.py | 导入依赖 / import_depends |
| 76 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_DRIFT 漂移检测: model_drift_monitor.py | 导入依赖 / import_depends |
| 77 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_DRIFT 漂移检测: performance_baseline.py | 导入依赖 / import_depends |
| 78 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_DRIFT 漂移检测: regime_detector.py | 导入依赖 / import_depends |
| 79 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_DRIFT 漂移检测: Drift Detector — 兼容别名，SSoT已迁移至 zephyr... | 导入依赖 / import_depends |
| 80 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_ENFORCEMENT 规则执行: admission_response.py | 导入依赖 / import_depends |
| 81 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_ENFORCEMENT 规则执行: code_review_ai.py | 导入依赖 / import_depends |
| 82 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_ENFORCEMENT 规则执行: GateEventAdapter — GateRepo 事件适配器（DW-000... | 导入依赖 / import_depends |
| 83 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_ENFORCEMENT 规则执行: DLQ 重试策略 — 指数退避自动重试 (dlq_retry_pol... | 导入依赖 / import_depends |
| 84 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_KB 知识库治理: knowledge_engine.py | 导入依赖 / import_depends |
| 85 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: consequence_manager.py | 导入依赖 / import_depends |
| 86 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Escalation Engine — MOD-INF-022 (escalation_en... | 导入依赖 / import_depends |
| 87 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: incident_response.py | 导入依赖 / import_depends |
| 88 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: spof_checker.py | 导入依赖 / import_depends |
| 89 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: agent_dispatch.py | 导入依赖 / import_depends |
| 90 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: bandwidth_optimizer.py | 导入依赖 / import_depends |
| 91 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: environment_manager.py | 导入依赖 / import_depends |
| 92 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: ops_foundation.py | 导入依赖 / import_depends |
| 93 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: PhaseManager->GateEngine 检查注册表桥梁 — 44 .... | 导入依赖 / import_depends |
| 94 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Phase Manager — ZephyrAlpha 施工阶段门控引擎. ... | 导入依赖 / import_depends |
| 95 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: startup_shutdown_cli.py | 导入依赖 / import_depends |
| 96 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: broker_resilience.py | 导入依赖 / import_depends |
| 97 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: bus_factor_defense.py | 导入依赖 / import_depends |
| 98 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: decision_fatigue.py | 导入依赖 / import_depends |
| 99 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: decision_fatigue_cli.py | 导入依赖 / import_depends |
| 100 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: fault_tolerance.py | 导入依赖 / import_depends |
| 101 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: offline_autonomy.py | 导入依赖 / import_depends |
| 102 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_OPS_RESILIENCE 运维弹性治理: offline_resilience.py | 导入依赖 / import_depends |
| 103 | budget_enforcement.py | → | D_GOV_OPS_RESILIENCE 运维弹性治理: Burn Rate Monitor — MOD-INF-024 (burn_rate_mon... | 导入依赖 / import_depends |
| 104 | budget_enforcement.py | → | D_GOV_OPS_RESILIENCE 运维弹性治理: degradation_manager.py | 导入依赖 / import_depends |
| 105 | budget_enforcement.py | → | D_GOV_OPS_RESILIENCE 运维弹性治理: timeout_guard.py | 导入依赖 / import_depends |
| 106 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_GOV_RULE 规则治理: constitutional_update.py —— 宪法自愈（Phase 1... | 导入依赖 / import_depends |
| 107 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_INFRA_A2A A2A通信: 基础设施 Infrastructure — A2A Protocol 模块 (M... | 导入依赖 / import_depends |
| 108 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_INFRA_RECOVERY 回滚恢复: MOD-INF-021 Rollback System — ZephyrAlpha 回滚... | 导入依赖 / import_depends |
| 109 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_INFRA_RECOVERY 回滚恢复: ComplexityBudget — 回滚复杂度元 Budget 监控。 ... | 导入依赖 / import_depends |
| 110 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_INFRA_RUNTIME 运行时集成: AssetClassifier — MOD-INF-026 L2 资产自动分类... | 导入依赖 / import_depends |
| 111 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_INFRA_RUNTIME 运行时集成: AssetDashboard — MOD-INF-026 资产健康仪表盘生... | 导入依赖 / import_depends |
| 112 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_INFRA_RUNTIME 运行时集成: MOD-INF-026 §18 — 资产依赖图。 (dependency.py) | 导入依赖 / import_depends |
| 113 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_INFRA_RUNTIME 运行时集成: UnifiedAssetIndex — MOD-INF-026 L3 统一资产索.... | 导入依赖 / import_depends |
| 114 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_INFRA_RUNTIME 运行时集成: AssetLifecycle — MOD-INF-026 L5 ITIL生命周期自... | 导入依赖 / import_depends |
| 115 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_INFRA_RUNTIME 运行时集成: MOD-INF-026 §24-25 — Git 历史元数据提取 + 多 ... | 导入依赖 / import_depends |
| 116 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_INFRA_RUNTIME 运行时集成: AssetInventoryModels — MOD-INF-026 Pydantic V2... | 导入依赖 / import_depends |
| 117 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_INFRA_RUNTIME 运行时集成: ReconciliationEngine — MOD-INF-026 L4 注册表 v... | 导入依赖 / import_depends |
| 118 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_INFRA_RUNTIME 运行时集成: MOD-INF-026 §17 — 24 个异构注册表统一解析适配... | 导入依赖 / import_depends |
| 119 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_INFRA_RUNTIME 运行时集成: MOD-INF-026 §26 — 三重信任锚验证门 R20。 (tru... | 导入依赖 / import_depends |
| 120 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_INFRA_RUNTIME 运行时集成: startup_shutdown.py | 导入依赖 / import_depends |
| 121 | Agent 治理八件套 · Governance Domain — DOM-GO... | → | D_OPS 反馈循环: token_budget.py | 导入依赖 / import_depends |
| 122 | budget_enforcement.py | → | D_OPS 反馈循环: Budget Enforcer core engine — MOD-INF-024 (bud... | 导入依赖 / import_depends |
| 123 | budget_enforcement.py | → | D_OPS 反馈循环: Budget Enforcer data models — MOD-INF-024 (bud... | 导入依赖 / import_depends |
| 124 | budget_enforcement.py | → | D_OPS 反馈循环: budget_tracker.py | 导入依赖 / import_depends |
| 125 | __init__.py | → | D_TRADING 交易运营: D_EXECUTION_CORE — BrokerInterface (broker_int... | 导入依赖 / import_depends |

### 依赖本域的其他域（入边）/ Depended By

| # | 外部域-源模块 / Source Module | → | 本域模块 / Target Module | 依赖类型 / Type |
|:--:|---------|:--:|---------|---------|
| 1 | D_GOVERNANCE 生命周期管理: C-track 端到端演示 —— 全流水线一次性运行 (dem... | → | Agent 治理八件套 · Governance Domain — DOM-GO... | 导入依赖 / import_depends |
| 2 | D_GOVERNANCE 生命周期管理: scaffold.py — ZephyrAlpha 唯一创建入口（RULE-T... | → | Agent 治理八件套 · Governance Domain — DOM-GO... | 导入依赖 / import_depends |
| 3 | D_GOVERNANCE 生命周期管理: test_ssot_redefinition_gate.py — SSoT 符号重复... | → | Agent 治理八件套 · Governance Domain — DOM-GO... | 测试依赖 / test_depends |
| 4 | D_GOVERNANCE 生命周期管理: test_governance_result_types.py | → | Agent 治理八件套 · Governance Domain — DOM-GO... | 测试依赖 / test_depends |
| 5 | D_GOV_SCRIPTS 脚本治理: VMS Cron 监控器 — MOD-INF-011 · TASK-INF-0224... | → | Agent 治理八件套 · Governance Domain — DOM-GO... | 导入依赖 / import_depends |
| 6 | D_GOV_SCRIPTS 脚本治理: VMS Health Check 脚本 — MOD-INF-011 · Phase 3... | → | Agent 治理八件套 · Governance Domain — DOM-GO... | 导入依赖 / import_depends |
| 7 | D_GOV_SCRIPTS 脚本治理: VMS Phase 2 数据迁移脚本 — MOD-INF-011 (vms_mi... | → | Agent 治理八件套 · Governance Domain — DOM-GO... | 导入依赖 / import_depends |
| 8 | D_GOV_SCRIPTS 脚本治理: VMS 迁移 dry-run 脚本 — MOD-INF-011 Phase 2 前... | → | Agent 治理八件套 · Governance Domain — DOM-GO... | 导入依赖 / import_depends |
| 9 | D_GOV_SCRIPTS 脚本治理: verify_schema_health.py — depgraph (PostgreSQL... | → | Agent 治理八件套 · Governance Domain — DOM-GO... | 导入依赖 / import_depends |
| 10 | D_GOV_SCRIPTS 脚本治理: [INVARIANTS] 预算健康检查不可跳过;检查结果必须.... | → | budget_enforcement.py | 导入依赖 / import_depends |
| 11 | D_GOV_SCRIPTS 脚本治理: VMS Cron 监控器 — MOD-INF-011 · TASK-INF-0224... | → | Agent 治理八件套 · Governance Domain — DOM-GO... | 导入依赖 / import_depends |
| 12 | D_GOV_SCRIPTS 脚本治理: VMS Health Check 脚本 — MOD-INF-011 · Phase 3... | → | Agent 治理八件套 · Governance Domain — DOM-GO... | 导入依赖 / import_depends |
| 13 | D_GOV_SCRIPTS 脚本治理: VMS Phase 2 数据迁移脚本 — MOD-INF-011 (vms_mi... | → | Agent 治理八件套 · Governance Domain — DOM-GO... | 导入依赖 / import_depends |
| 14 | D_GOV_SCRIPTS 脚本治理: VMS 迁移 dry-run 脚本 — MOD-INF-011 Phase 2 前... | → | Agent 治理八件套 · Governance Domain — DOM-GO... | 导入依赖 / import_depends |

### 跨域依赖图 / Cross-domain Dependency Diagram

> 本域与 16 个外部域直接连接（出边 125 条 + 入边 14 条 = 139 条）。只显示直接连接的域，不展开具体节点。
...
graph LR
    ...
    D_GOV_REPAIR -->|24条 导入依赖 / import_depends| D_GOV_AUDIT
