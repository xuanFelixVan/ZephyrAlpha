...
graph TD
    ...
    class D_GOV_AUDIT,D_GOV_OPS_RESILIENCE,D_GOV_RULE,D_GOVERNANCE,D_GOV_KB external_prod
    class D_GOV_ENFORCEMENT,D_GOV_SCRIPTS,D_AUTONOMY_CORE,D_DATA external_design
```
...
graph TD
    ...
    class src_zephyr_governance_init_py,src_zephyr_governance_financial_governance_budget_enforcement_py,src_zephyr_governance_observability_governance_init_py,src_zephyr_governance_persistence_init_py production
    class D_GOV_AUDIT,D_GOV_OPS_RESILIENCE,D_GOV_RULE,D_GOVERNANCE,D_GOV_KB external_prod
    class D_GOV_ENFORCEMENT,D_INFRA_RUNTIME,D_GOV_SCRIPTS,D_AUTONOMY_CORE external_design
```
...
| 39 | __init__.py | → | D_GOVERNANCE 生命周期管理: AtomicTransactionManager — SQLite + 文件系统的... | config_depends / config_depends |
...
| 42 | __init__.py | → | D_GOVERNANCE 生命周期管理: transition — 状态机转换 Mixin（从 task_repo.py... | config_depends / config_depends |
...
> 本域与 19 个外部域直接连接（出边 170 条 + 入边 17 条 = 187 条）。只显示直接连接的域，不展开具体节点。
