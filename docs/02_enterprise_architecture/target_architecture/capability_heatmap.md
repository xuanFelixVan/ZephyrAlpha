...
| 达到顶级机构对标水平还需要多少投入？| §6 投入估算（人月 + KB 决策记录 数 + Sprint 数）|
| 能力成熟度何时刷新？谁负责？| §7 季度 review 机制 |
| 本视图与 01-BA 的 `capability-map` / 业务能力清单是什么关系？| §8 与 01-BA / `capability_heatmap.yaml` 的承载关系 |
...
- **KBG-0012** Capability Maturity Heatmap View v1.0.0 accepted（本视图同源）
...
|---|---|---|---|---|
| **L0** ⚪ | **Missing** 缺失 | 能力完全不存在，无代码 / 无文档 / 无 KB 决策记录 | — | — |
| **L1** 🔵 | **Designed** 设计级 | 有 KB 决策记录 / 有架构视图 / 有 canonical 设计稿 / 无代码 | KBG-00XX accepted + 视图定义 | Emerging / Draft / Initial |
...
> **📊 能力热力图可视化**：见 [`diagrams/capability_heatmap_visual.mmd`](diagrams/capability_heatmap_visual.mmd)
...
4. **投入重估**：更新 §6 投入估算
5. **修订入账**：落盘 v1.X.0 + R/OQ/KB 决策记录 登记 + handoff-log 条目
...
## 8. 与 01-BA / `capability_heatmap.yaml` 的数据承载关系
