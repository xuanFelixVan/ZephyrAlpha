...
> 按平面维度的反查视图同样收录于 [`runtime_planes.yaml`](../../../architecture_model/cross_cutting/runtime_planes.yaml)（`planes.hot.modules[]` / `planes.warm.modules[]` / `planes.cold.modules[]`）。以下为可读摘要：
...
> 前端子层的平面归属数据见 [`runtime_planes.yaml`](../../../architecture_model/cross_cutting/runtime_planes.yaml) 中 `planes.cold.modules[]` 的 `fe-ssr-reports` / `fe-codegen` 条目（其余前端子层默认 Warm）。
...
> 已删除（git log 是真源）。v1.0.0 首次发布于 2026-04-19，详见 git log。
