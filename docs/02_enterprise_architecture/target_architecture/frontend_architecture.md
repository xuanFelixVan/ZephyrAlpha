...
# Target Architecture — Frontend Architecture / 目标架构：前端架构 （被恢复）
...
| 前端安全策略（CSP / CORS / CSRF / XSS / OAuth）| `../04_architecture_principles_decisions/security_principles.md`（SEC 原则）+ `architecture_model/security/threat_model.yaml`（威胁映射，后续深化时必须包含前端安全一节）|
...
**接触点**：仅在 L08 `api_gateway/` 子模块（FastAPI + WebSocket + OpenAPI Spec 生成）——这是前后端**唯一合法对接点**。任何试图让前端直接访问 L00-L07/L09-L13 的设计均违反 KBG-0007。

### 1.4 决策溯源

- **KBG-0007** 前端层不进 src/ 14 层，作为独立 `frontend/` 平台层（2026-04-18 accepted）
