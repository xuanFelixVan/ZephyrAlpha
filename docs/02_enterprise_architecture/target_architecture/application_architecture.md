...
| L03 策略研究 | 检索历史策略 + KB 决策记录 | 回测任务 | 回测成功率 |
| L05 执行 | 写执行日志摘要 | （实时流，不走 Orc）| 滑点 / 延迟 |
| L08 反思学习 | 读写 `lessons` Collection | 复盘任务 | — |
| L10 治理 | 读 KB 决策记录 / 写治理规范 | SSoT 校验任务 | 规范违规率 |
...
> **📊 异常处理时序图**：见 [`seq_exception_handling.mmd`](./diagrams/seq_exception_handling.mmd) — 跨层异常传播与降级处理完整时序
