...
- vibe-coding-2.0
- 5-core-services
summary: TOGAF Application Architecture 视图（v3.0.1 轻度瘦身版）。v3.0.1：修复4处断链（domain_index→domains/index、cross_domain_matrix/capacity_report迁移路径）、删除§11修订记录（git log是真源）、修复§4A.1标题（6→5服务）、§4.1数字标注时点快照。v3.0.0：基于§2.1裁定，模块清单改为53域派生，数据源depgraph。
date: '2026-07-04'
ttl: permanent
---
...
> 完整域索引见[`generated/domains/index.md`](../generated/domains/index.md)。

### 4.1 域统计概览

> **注**：以下数字为 2026-06-26 时点快照，会随 depgraph 更新而漂移。最新数字以 [`generated/domains/index.md`](../generated/domains/index.md) 为准。

| 指标 | 值 | 数据源 |
|------|:---:|--------|
| 域总数 | 53 | depgraph `domains` 表 |
| 节点总数 | 6501（时点快照）| depgraph `nodes` 表 |
| 依赖边总数 | 7191（时点快照）| depgraph `edges` 表 |
| production 节点 | 1404（时点快照）| depgraph `nodes.design_maturity` |
| design 节点 | 89（时点快照）| depgraph `nodes.design_maturity` |
| prototype 节点 | 5008（时点快照）| depgraph `nodes.design_maturity` |
...
> 完整域清单（含模块数/容量/描述）见 [`generated/domains/index.md`](../generated/domains/index.md)。
> 单域详细文档见 [`generated/domains/*.md`](../generated/domains/)（53个）。
> 单域依赖图见 [`generated/domains/*_dependency.mmd`](../generated/domains/)（53个）。

### 4.4 跨域依赖矩阵

> 完整跨域依赖矩阵见 [`../01_global_architecture_diagram/cross_domain_matrix.md`](../01_global_architecture_diagram/cross_domain_matrix.md)（由`generate_cross_domain_matrix.py`从`edges`表派生）。
...
- 容量报告见 [`../03_governance_reports/capacity_report.md`](../03_governance_reports/capacity_report.md)
- 超容域必须拆分（需Owner批准）
