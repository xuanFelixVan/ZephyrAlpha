...
- domain-driven
- depgraph-derived
summary: 架构文档组的总览视图。v2.0.0：基于§2.1裁定，14层降级为域属性，53域成为唯一物理分类体系。结构化数据由depgraph.db全景图派生。
date: '2026-06-26'
ttl: permanent
---
...
**系统定位**：ZephyrAlpha 是个人量化投资系统的 AI-native 重构，采用**53域唯一物理分类体系**（基于depgraph.db全景图），Python 全栈，Vibe Coding 驱动（Cursor + Trae 双 AI IDE）。
...
| 维度 | 状态 | 说明 |
|------|------|------|
| **53域物理分类** | ✅ **已定义** | depgraph.db `domains` 表为SSoT |
...
|------------|-------------------|
| 2026-06-26 | **v2.0.0（DM-200912 Phase4-A）**：基于§2.1裁定重写——14层降级为域属性，53域成为唯一物理分类体系；结构化数据由depgraph.db全景图派生；新增§1.2唯一分类体系裁定、§1.3全景图派生机制、§6.4数据派生原则。 |
