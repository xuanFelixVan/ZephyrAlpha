...
def run_backtest(args):
    """运行回测"""
    logger.info(f"开始回测 - 股票:{args.code} 日期:{args.start}~{args.end}")
    ...
    def _load_from_cache(self, code: str, start: str, end: str) -> Optional[pd.DataFrame]:
        ...
        if os.path.exists(cache_file):
            try:
                ...
                df = df[start:end]
                if len(df) > 0:
                    logger.info(f"从缓存加载数据: {len(df)}条")
                    return df
            except Exception as e:
...
class MaCrossStrategy(bt.Strategy):
    ...
    def stop(self):
        """策略结束时调用"""
        self.log(f'策略停止, 快速均线周期: {self.params.fast_period}, '
                 f'慢速均线周期: {self.params.slow_period}', dt=None)
```

---

### 3.4 绩效分析器 (performance.py)

```python
"""
绩效分析器 - Backtrader分析器集合
"""
import backtrader as bt
...
def print_analyzer_results(strategy):
    ...
    if 'returns' in results:
        ...
        print(f"  年化收益率: {results['returns'].get('rnorm100', 0):.2f}%")

    # 打印回撤
    print("\n【回撤指标】")
    if 'drawdown' in results:
        print(f"  最大回撤: {results['drawdown'].get('max', {}).get('drawdown', 0):.2f}%")
        print(f"  最大回撤时长: {results['drawdown'].get('max', {}).get('len', 0)}天")
    ...
    if 'trades' in results:
        ta = results['trades']
        print(f"  总交易次数: {ta.get('total', {}).get('total', 0)}")
...
class RiskManager(bt.RiskRules):
    ...
    params = (
        ('max_position_pct', 0.2),      # 最大持仓比例20%
        ('max_single_trade_pct', 0.1),  # 单笔交易最大比例10%
        ('daily_loss_limit_pct', 0.05), # 日内最大亏损5%
        ('printlog', True),
    )
...
backtest:
  ...
  # 风控配置
  risk:
    max_position_pct: 0.2          # 最大持仓20%
    max_single_trade_pct: 0.1     # 单笔交易最大10%
    daily_loss_limit_pct: 0.05    # 日内最大亏损5%
...
```
2024-03-29 10:00:00 - __main__ - INFO - 开始回测 - 股票:000001.SZ 日期:2024-01-01~2024-12-31
2024-03-29 10:00:00 - __main__ - INFO - 初始资金: 100000.00
2024-03-29 10:00:01 - __main__ - INFO - 最终资金: 125000.00
...
1. **修改策略参数**: 尝试不同的均线周期组合
2. **添加新策略**: 在strategies目录创建新策略
3. **学习因子选股**: 进入Phase 2
...
**下一步文档**: [因子计算详细设计](./factor_design.md)
