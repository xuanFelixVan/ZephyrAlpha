...
本文档定义了清风量化系统中各AI智能体之间的标准化调用协议，确保智能体协作高效、数据传递规范、错误处理统一。协议遵循**职责驱动、接口明确、数据标准化**的原则。

## 2. 智能体角色定义

| 智能体标识名 | 中文名称 | 核心职责 | 所属架构层 |
|--------------|----------|----------|------------|
| `blueprint-architect` | 蓝图架构师 | 负责系统蓝图设计、架构规划、模块职责定义 | Layer 0-8 架构设计层 |
| `spec-approver` | 审批智能体 | 负责技术规格书撰写、方案评审、技术可行性评估 | 技术评审层 |
| `development-agent` | 开发智能体 | 负责代码实现、模块开发、测试编写 | 开发实施层 |
| `audit-sentinel` | 审计哨兵 | 负责系统审计、代码质量检查、安全扫描 | 质量监控层 |

## 3. 调用触发条件

### 3.1 蓝图架构师 → 审批智能体
- **触发条件**：蓝图设计完成，需要进行技术规格书撰写和方案评审
- **传递数据**：蓝图文档路径、设计概要、关键需求点
- **预期输出**：技术规格书、技术评审报告

### 3.2 审批智能体 → 开发智能体
- **触发条件**：技术评审通过，需要开始代码实现
- **传递数据**：技术规格书路径、评审报告、实施约束条件
- **预期输出**：代码实现、模块文档、测试用例

### 3.3 开发智能体 → 审计哨兵
- **触发条件**：代码开发完成，需要进行质量检查
- **传递数据**：代码路径、测试报告、构建结果
- **预期输出**：审计报告、质量评分、改进建议
...
| `assess_technical_feasibility` | 评估技术可行性 | spec-approver | spec-approver |
| `analyze_risk` | 分析风险 | spec-approver | spec-approver |
| `calculate_implementation_complexity` | 计算实施复杂度 | spec-approver | spec-approver |
...
| 错误代码 | 说明 | 处理建议 |
|----------|------|----------|
| `AGENT_NOT_FOUND` | 目标智能体不存在 | 检查智能体标识名是否正确 |
| `INVALID_PARAMETERS` | 参数格式错误 | 检查参数是否符合要求 |
| `FILE_NOT_FOUND` | 文件不存在 | 检查文件路径是否正确 |
| `PERMISSION_DENIED` | 权限不足 | 检查文件读写权限 |
| `EXECUTION_FAILED` | 执行过程失败 | 查看详细错误信息，重新执行 |
...
2. **模板使用**：必须使用标准模板生成技术规格书和评审报告
3. **文件管理**：必须遵循系统的文件路径标准

### 8.3 质量保证
1. **协议版本管理**：协议版本必须明确标识，支持向后兼容
2. **测试验证**：必须定期进行协议兼容性测试
...
| 版本 | 日期 | 说明 | 作者 |
|------|------|------|------|
| v1.0 | 2026-04-02 | 初始版本，定义基础调用协议 | 审批智能体 (Spec-Approver) |
