...
class BacktestValidator:
    ...
    def validate_result(result: dict) -> ValidationReport:
        ...
        checks.append(ValidationCheck(
            ...
            message=f"总收益率 {total_return:.2%} 在合理范围"
        ))
        ...
        checks.append(ValidationCheck(
            name="最大回撤检查",
            passed=max_drawdown <= 0.5,  # 不超过50%
            message=f"最大回撤 {max_drawdown:.2%} 可接受"
        ))
...
| 参数 | 合理范围 | 异常检测 |
|------|----------|----------|
| 初始资金 | 10,000 - 100,000,000 | < 最低门槛或 > 异常高 |
| 收益率 | -100% ~ +1000% | > 1000% 标记可疑 |
| 最大回撤 | 0% ~ 100% | > 50% 警告 |
| 夏普比率 | -10 ~ +10 | 绝对值 > 10 异常 |
| 交易次数 | > 0 | = 0 表示无交易 |
| 胜率 | 0% ~ 100% | < 5% 或 > 95% 可疑 |
```
