...
> **版本**: v1.0
> **创建日期**: 2026-04-03
> **目标**: 建立专业化的文档治理体系，确保文档质量、一致性和可维护性
...
**命名规则**:
1. **全大写**: 所有字母使用大写
2. **下划线分隔**: 使用下划线(_)分隔单词
3. **描述性命名**: 文件名应清晰反映文档内容
...
❌ 错误示例:
- T.01.DS001.free_data_sources.md  (编号格式不规范)
- ths_bd_complete_indicator_list.md (小写命名)
- data-source-management.md         (连字符分隔)
...
def check_yaml_header(file_path: str) -> dict:
    ...
    if yaml_end == -1:
        return {"valid": False, "error": "YAML头部未正确关闭"}
    ...
    # 检查必要字段
    required_fields = ['module_id', 'version', 'created_date', 'last_updated', 'owner']
...
def check_file_naming(file_path: str) -> dict:
    ...
    if not file_name.isupper():
        return {"valid": False, "error": "文件名应全大写"}
    
    # 检查是否使用下划线分隔
    if ' ' in file_name or '-' in file_name:
        return {"valid": False, "error": "文件名应使用下划线分隔"}
    
    # 检查是否包含特殊字符
    if not re.match(r'^[A-Z0-9_]+$', file_name):
...
| 审计类型 | 频率 | 审计范围 | 审计人 |
|---------|------|---------|--------|
| **快速审计** | 每周一 | 新增/修改文档 | 文档维护者 |
| **标准审计** | 每月1日 | 全部文档 | 首席架构师 |
| **深度审计** | 每季度 | 全部文档+代码对应 | 审计团队 |
...
| `link_fixer.py` | 链接修复 | 每月1日 |
| `metadata_enhancer.py` | 元数据增强 | 按需 |
| `document_classifier.py` | 文档分类 | 按需 |
| `doc_quality_checker.py` | 文档质量检查 | 每周一 |
...
```bash
# 快速审计
python scripts/document_auditor.py --mode quick
...
```bash
# 检查链接
python scripts/link_fixer.py --check
...
```bash
# 增强元数据
python scripts/metadata_enhancer.py --file docs/XXX.md
```
...
1. **及时归档**: 旧版本文档及时归档
2. **标注状态**: 在YAML头部标注status为Archived
3. **保留历史**: 保留版本历史记录
4. **更新索引**: 从INDEX.md中移除归档文档
...
| 指标 | 当前值 | 目标值 | 改进措施 |
|------|--------|--------|---------|
| **文档健康度** | 72.5分 | 90分 | 定期审计+优化 |
| **职责清晰度** | 60分 | 90分 | 职责梳理+合并 |
| **索引完备性** | 70分 | 95分 | 创建缺失索引 |
| **命名规范性** | 85分 | 95分 | 重命名不规范文档 |
...
| **文档审计时间** | 30分钟 | 10分钟 | 自动化工具 |
| **链接修复时间** | 10分钟 | 2分钟 | 自动化工具 |
