...
| 密钥类型 | 用途 | 安全要求 |
|----------|------|----------|
| `DATA_API_KEY` | 行情数据API | 每日使用量限制 |
| `TRADE_API_KEY` | 交易API (模拟/实盘) | 最高安全等级 |
| `DATABASE_KEY` | 数据库连接 | 定期轮换 |
...
class SecretManager:
    ...
    def validate(self) -> bool:
        ...
        if missing:
            raise SecurityError(f"缺少必需的環境變量: {', '.join(missing)}")
        return True
...
| 角色 | 权限范围 | 典型用户 |
|------|----------|----------|
| `ADMIN` | 系统配置、用户管理、回测执行 | 系统管理员 |
| `TRADER` | 策略执行、持仓查询、资金查询 | 交易员 |
| `RESEARCHER` | 因子研究、回测分析、策略开发 | 研究员 |
| `VIEWER` | 只读数据访问 | 观察员 |
...
| `INFO` | 正常业务流程 | 策略信号、执行结果 |
| `WARNING` | 异常但可处理 | 数据缺失、配置警告 |
| `ERROR` | 错误需要处理 | API失败、计算异常 |
| `CRITICAL` | 系统级严重问题 | 资金风险、认证失败 |
