...
| 级别 | 数值 | 使用场景 | 示例 |
|------|------|----------|------|
| `CRITICAL` | 50 | 系统级严重问题 | 资金风险、认证失败、数据丢失 |
| `ERROR` | 40 | 错误需要处理 | API调用失败、数据解析异常 |
| `WARNING` | 30 | 异常但可处理 | 数据缺失、配置使用默认值 |
| `INFO` | 20 | 正常业务流程 | 策略启动、信号生成、订单执行 |
| `DEBUG` | 10 | 开发调试信息 | 函数入参、中间变量、循环次数 |
...
logger.critical(f"账户 {account_id} 亏损超过阈值 {threshold}")
logger.error(f"THS API 调用失败: {error_code} - {message}")
logger.warning(f"数据源 {source} 返回空数据，使用缓存")
logger.info(f"策略 {strategy_id} 在 {stock_code} 生成买入信号")
logger.debug(f"因子计算参数: window={window}, threshold={threshold}")

# ✗ 错误示例

logger.info(f"用户 {user_id} 登录成功")  # 过于琐碎
logger.error(f"出错了")  # 信息不足
logger.debug(f"循环第 {i} 次")  # 生产环境不应输出
```
...
# 输出示例
[2026-03-28 10:15:30] [INFO] [backtest_engine] [run_strategy:156] [strategy=S001] 策略执行开始
[2026-03-28 10:15:31] [ERROR] [data_fetcher] [fetch_ohlcv:89] [stock=000001] 数据获取失败: 超时
[2026-03-28 10:15:32] [WARNING] [risk_manager] [check_position:203] [position=0.95] 仓位超过阈值
```
...
| 文件 | 内容 | 级别 | 保留策略 |
|------|------|------|----------|
| `logs/app.log` | 应用主日志 | INFO+ | 5个备份 |
| `logs/error.log` | 错误日志 | ERROR+ | 10个备份 |
| `logs/audit/audit.log` | 审计日志 | INFO+ | 20个备份 |
| `logs/backtest/backtest_{date}.log` | 回测日志 | DEBUG | 30天 |
| `logs/trade/trade_{date}.log` | 交易日志 | INFO+ | 90天 |
| `logs/data/data_{date}.log` | 数据获取日志 | WARNING+ | 14天 |
```
