...
│  阶段3: 盘后处理    15:00-20:00   下载当日数据、数据清洗     │
│  阶段4: 夜间处理    20:00-06:00   因子计算、数据备份         │
└─────────────────────────────────────────────────────────────┘
```
...
class TradingSessionMonitor:
    ...
    def __init__(self):
        self.realtime_tasks = ['行情监控', '分钟线采集']
        self.scheduled_tasks = [
            {'time': '11:30', 'name': '上午汇总', 'timeout': 600},
...
| 时间 | 任务 | 超时 | 重试 | 优先级 |
|------|------|------|------|--------|
| 15:05 | 下载当日完整行情 | 30min | 3 | 高 |
| 15:45 | 更新资金流向数据 | 45min | 2 | 中 |
| 16:30 | 下载龙虎榜数据 | 30min | 2 | 中 |
| 17:30 | 更新融资融券数据 | 30min | 2 | 中 |
| 18:30 | 数据质量校验 | 60min | 1 | 高 |
...
class AfterHoursPipeline:
    ...
    def __init__(self):
        self.tasks = [
            ...
            {'time': '16:30', 'name': '龙虎榜', 'priority': 'MED', 'timeout': 1800, 'retry': 2},
...
| 错误类型 | 检测方式 | 恢复策略 | 降级方案 |
|----------|----------|----------|----------|
| 网络中断 | 超时检测 | 自动重试+切换数据源 | 使用缓存数据 |
| 数据缺失 | 完整性校验 | 多源补全 | 标记并跳过 |
...
```python
ALERT_RULES = [
    {'condition': 'success_rate < 0.95', 'level': 'WARNING', 'message': '任务成功率低于95%'},
    {'condition': 'success_rate < 0.90', 'level': 'CRITICAL', 'message': '任务成功率低于90%'},
    {'condition': 'data_completeness < 0.98', 'level': 'WARNING', 'message': '数据完整性低于98%'},
    {'condition': 'consecutive_failures >= 3', 'level': 'CRITICAL', 'message': '连续失败超过3次'},
]
```
...
├── DAILY_PIPELINE.md          # 本文档
└── DATA_QUALITY_CONTROL.md    # 数据质量控制(待创建)
```
