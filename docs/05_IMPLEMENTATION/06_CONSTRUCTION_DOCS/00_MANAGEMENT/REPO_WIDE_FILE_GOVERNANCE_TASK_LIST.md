...
| **D′（蓝图）** | 跑 `scan_blueprint_d_overlap_candidates.py`，从 **TOP-N 候选对**起做 Owner 评审与合稿排期；**低置信**产出登记 [待审登记](./D_CLASS_CONSOLIDATION_PENDING_REVIEW_REGISTER.md) | 与 **§3.4**、Playbook **§5** 对齐；可与 **§7** 同窗口 |
...
- [ ] D：候选簇已登记（`BLUEPRINT_D_OVERLAP_*` / Playbook）；**已裁决**簇完成叙事归并 + 链接 + 台账；**低置信**合稿若有进行，[待审登记](./D_CLASS_CONSOLIDATION_PENDING_REVIEW_REGISTER.md) 与实现一致。（2026-04-12：**机器候选已刷新** — [`BLUEPRINT_D_OVERLAP_CANDIDATES_20260412.md`](../../../09_AUDIT/STATE/BLUEPRINT_D_OVERLAP_CANDIDATES_20260412.md)：扫描蓝图 **758**、截断前 **12244** 对、写入 **400** 对；**最终裁决与合稿仍待 Owner** 按 [Playbook](./D_CLASS_BLUEPRINT_OVERLAP_PLAYBOOK.md) 从高分起评审。）  
...
- [ ] [办公室 README](./README.md)：**治理流程编号**仍覆盖蓝图、孤儿/重复、扩展轨、根卫生、**整仓文件尽治**、**文档地图与放置**；**办公室文件一览**表与磁盘一致（含 **D 类合稿待审登记**）；[治理工具总索引](./GOVERNANCE_TOOLS_INDEX.md) 与 `scripts/` 实际脚本同步；[文档地图与放置规程](./DOCUMENT_MAP_AND_PLACEMENT_GOVERNANCE.md) **§1.5** 与 [LAYOUT 标准](../../../09_AUDIT/STANDARDS/DOCUMENT_REPOSITORY_LAYOUT_STANDARD.md) **§1 第 5 条**互指无断链、无第二套 Layer 放置真源。  
