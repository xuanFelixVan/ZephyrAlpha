---
module_id: D_CLASS_OVERLAP_SECOND_PASS_PROMPT_TEMPLATE_001
version: 1.0.0
status: Active
...
standard_type: 提示词模板
applicable_scope: 与 `BLUEPRINT_D_OVERLAP_SECOND_PASS_QUEUE_*.jsonl` 及 [D 类蓝图重叠 Playbook](./D_CLASS_BLUEPRINT_OVERLAP_PLAYBOOK.md) 联用
---
...
> **真源规程**：[D 类蓝图重叠 Playbook](./D_CLASS_BLUEPRINT_OVERLAP_PLAYBOOK.md)（高/低置信双轨、待审登记）。
...
| `low_confidence_register` | boolean | 是 | 若建议走 Playbook **低置信**合稿链，`true` 表示应写入 [D 类合稿待审登记](./D_CLASS_CONSOLIDATION_PENDING_REVIEW_REGISTER.md)。 |
...
- Schema 字段不足以表达合并粒度；  
- 与 [Playbook](./D_CLASS_BLUEPRINT_OVERLAP_PLAYBOOK.md) 表述冲突；  
