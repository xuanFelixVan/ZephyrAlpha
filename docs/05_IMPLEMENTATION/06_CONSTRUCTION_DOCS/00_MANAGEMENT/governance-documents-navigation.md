...
> **本页不做什么**：**不**把 `docs/09_AUDIT` 等整树搬进 `00_MANAGEMENT`，也**不**把治理正文塞进 `01_BLUEPRINTS`（图纸柜只放 `*BLUEPRINT.md` 模块蓝图，见 图纸柜规则）。
...
| 施工门禁（蓝图终稿 / 阶段模型） | `CONSTRUCTION_GATE_CRITERIA_20260408.md`（项目办公室 **CANON** 真源） |

| 蓝图交付标准（机构精华版，目标态） | `BLUEPRINT_DELIVERY_STANDARD_INSTITUTIONAL_LITE.md` |

| 文档治理架构（L0～L5 分层与边界） | `DOCUMENT_GOVERNANCE_ARCHITECTURE.md` |

| 文档仓库目录与放置标准（§2～§4 目录表；**§1 第 5～6 条** Layer/路径分立 + 位置 vs 入链） | `docs/09_AUDIT/STANDARDS/DOCUMENT_REPOSITORY_LAYOUT_STANDARD.md` |

| 文档地图与放置（办公室规程 · **§1.5** / **§1.6** · 与 REPO_WIDE **§2.3.2** 互文 · 与扫描/§7 衔接） | `DOCUMENT_MAP_AND_PLACEMENT_GOVERNANCE.md` |

| 全局文件治理会话交接（新对话粘贴） | `GLOBAL_FILE_GOVERNANCE_SESSION_HANDOFF.md` |

| 已锁定治理裁决 | `docs/09_AUDIT/STATE/GOVERNANCE_DECISIONS_LOCKED_20260408.md` |

| OpenClaw 整改执行手册 | `docs/09_AUDIT/PROCEDURES/OPENCLAW_REMEDIATION_EXECUTION_PLAYBOOK_20260408.md` |

| 全系统文档审计方案 | `docs/09_AUDIT/PROCEDURES/FULL_SYSTEM_DOCUMENT_AUDIT_PLAN_20260408.md` |

| 全系统审计全案 | `docs/09_AUDIT/PROCEDURES/FULL_SYSTEM_AUDIT_COMPLETE_CASE_20260408.md` |

| 蓝图阶段文档清洁总案 | `BLUEPRINT_PHASE_DOCUMENT_HYGIENE_MASTER_PLAN_20260408.md`（项目办公室 **CANON** 真源） |

| 整改任务指令 | `docs/09_AUDIT/PROCEDURES/DOC_REMEDIATION_TASK_DIRECTIVE_20260408.md` |

| Trae×Cursor 蓝图任务台账 | `docs/09_AUDIT/STATE/TRAE_BLUEPRINT_TASK_LEDGER_20260408.md` |

| 接口契约真源（蓝图常链） | `docs/03_TRADING_TACTICS/API_Contract.md` |
...
| L1 治理扫描脚本 | `scripts/governance/sentinel_l1_governance_scan.py`（根目录 `sentinel_l1_governance_scan.py` 可转发） |

| L1 扫描产物（运行生成；路径以仓库内实际为准） | `docs/09_AUDIT/STATE/SENTINEL_L1_SCAN_20260408.json`、`SENTINEL_L1_SCAN_20260408.md` 等 |

| 蓝图 D 类重叠候选（启发式） | `scripts/governance/scan_blueprint_d_overlap_candidates.py` → `docs/09_AUDIT/STATE/BLUEPRINT_D_OVERLAP_CANDIDATES_*`；规程见 D 类 Playbook **§2.5、§5**；**方案文件索引**见 REPO_WIDE §3.4.1 |

| 蓝图 D 类 A 档分流 + 二审队列 JSONL | `scripts/governance/triage_blueprint_d_overlap_pairs.py` → `BLUEPRINT_D_OVERLAP_TRIAGE_*`、`BLUEPRINT_D_OVERLAP_SECOND_PASS_QUEUE_*.jsonl`；二审配合 D 类二审提示词模板；与上条同读 D 类 Playbook **§3.5** |
...
| 文档编码标准（UTF-8 / 防乱码全文） | `docs/10_GOVERNANCE_COMPLIANCE/DOCUMENT_ENCODING_STANDARD.md` — 办公室 AI 交接 §0.1.3、**§0.1.4**（`.gitignore` 与 `STATE/` 两条线）、**§0.2**（全委托 AI）；任务清单 REPO_WIDE §1.2（含末段两条线）、§7.2 末 |

| 知识库入口 | [`docs/10_GOVERNANCE_COMPLIANCE/KNOWLEDGE_BASE/INDEX.md`](../../../10_GOVERNANCE_COMPLIANCE/KNOWLEDGE_BASE/INDEX.md) |

| 文档治理知识库 | `docs/10_GOVERNANCE_COMPLIANCE/KNOWLEDGE_BASE/DOCUMENT_GOVERNANCE_KNOWLEDGE_BASE.md` |

| 职责审查机制 | `docs/10_GOVERNANCE_COMPLIANCE/RESPONSIBILITY_REVIEW_MECHANISM.md` |
...
| 路径标准 | `docs/05_IMPLEMENTATION/02_DEVELOPMENT/PATH_STANDARD.md` |

| 文档质量门 | `docs/05_IMPLEMENTATION/02_DEVELOPMENT/DOCUMENT_QUALITY_GATE_STANDARD.md` |

| 开发标准总览 | `docs/05_IMPLEMENTATION/02_DEVELOPMENT/DEVELOPMENT_STANDARDS.md` |
...
| 图纸柜规则 | **仅约束** `01_BLUEPRINTS` 摆放 |

| 蓝图终稿定义 | 终稿门禁（与施工门禁文档互补） |

| 任务清单 | 阶段勾选 |

| 文档治理架构 | L0～L5 分层与审计边界（机构参照模型） |

| 文档地图与放置规则 | LAYOUT 真源 + **§1.5** + **§1.6**（位置正确性分桶）+ REPO_WIDE **§2.3.2** + 扫描/§7 批次衔接 |

| D 类蓝图重叠 Playbook + REPO_WIDE §3.4.1 | D 类：**§2.5** 置信度、**§5** 双轨与 **高置信可合并**；办公室内 **D 类合稿方案文件** 与工具、待审登记、二审模板的一页索引 |
...
| 1.0.9 | 2026-04-11 | 「合规与编码」表文档编码行增 **§0.1.4 / §0.2**、REPO_WIDE **§7.2 末** |

| 1.0.8 | 2026-04-11 | 「合规与编码」表文档编码行互指 AI 交接 **§0.1.3**、REPO_WIDE **§1.2** |

| 1.0.7 | 2026-04-11 | 「脚本与扫描产物」表 D 类两行补 **§2.5 / §5** 与 REPO_WIDE §3.4.1；办公室关系表增 D 类 + §3.4.1 一行 |
...
| 1.0.4 | 2026-04-10 | 施工与蓝图表、办公室关系表互指 LAYOUT **§1 第 5 条**与 放置规程 §1.5 同口径 |

| 1.0.3 | 2026-04-10 | 施工与蓝图表增 全局文件治理会话交接 |

| 1.0.2 | 2026-04-10 | 施工与蓝图表增 文档地图与放置规则；脚本表 L1 路径改 `scripts/governance/`；办公室关系表增一行 |
