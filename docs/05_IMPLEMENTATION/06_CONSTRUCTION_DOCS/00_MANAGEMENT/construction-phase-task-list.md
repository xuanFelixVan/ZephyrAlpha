...
- [ ] **2.2** 对每个扫描发现分类处理策略：
  - **C1（重复内容）**: ✅ 已验证 0 个重复簇，跳过（无需 F2 脚本）
  - **C2（Basename 碰撞）**: 6 个非导航目标 → F3 碰撞报告脚本
  - **C3（孤儿文件）**: 存在口径差异，需重扫后决策 → F4
  - **C4（D 类重叠）**: 400 对候选，9 对高置信 → F5 triage
  - **L1（断链）**: 4558 条可自动修复 → F1 `fix_dead_links.py --apply`
  - 自动可修（断链 → 批量路径替换）
