...
> **维护**：在 `00_MANAGEMENT/` 新增规章类文档时，请同步更新 [项目办公室 README](10_GOVERNANCE_COMPLIANCE/TRAINING_SYSTEM/README.md)、[推荐阅读入口](#推荐阅读入口) 与 全库治理文档导航（若与全库导航相关）。  
...
| **项目办公室总入口** | [`README.md`](10_GOVERNANCE_COMPLIANCE/TRAINING_SYSTEM/README.md) |
...
| `01_BLUEPRINTS` 机器生成列表 | [`../01_BLUEPRINTS/INDEX.md`](12_MODULE_DESIGNS/layer_0/INDEX.md) |
...
- [x] 已选定「总清单」主文档（或主文档 + 明确子索引的组合）— **主入口**：`BLUEPRINT_STAGE_COMPLETE_SUMMARY.md`；**图纸柜机器列表**：[`01_BLUEPRINTS/INDEX.md`](12_MODULE_DESIGNS/layer_0/INDEX.md)（2026-04-10）
- [x] 每个「该有蓝图的条目」在总清单中均有记录 — 总清单已约定 A/B 双桶枚举（图纸柜 INDEX + `ACTIVE_SCATTERED_BLUEPRINTS_MANIFEST_TASK1.md`），定义见总清单「任务 1 闭合口径」（2026-04-10）
...
- [x] **重复文档口径**：`docs/09_ARCHIVE/duplicates/CANONICAL_POINTERS.md` 表内条目均已填 `canonical_path` 并可追溯；业务目录新发同题双稿按 Playbook 处置（2026-04-10 核对）
...
- [x] 在「全楼地图」中用一句话写明：**以哪条路径为权威（canonical）** — 已写入 [`06_CONSTRUCTION_DOCS/INDEX.md`](../INDEX.md)「目录概要」与 `BLUEPRINT_STAGE_COMPLETE_SUMMARY.md`（2026-04-10）
...
- [x] 已对照 蓝图阶段文档卫生总计划 检查是否有遗漏批次 — 2026-04-10：INDEX 大门与任务 4 对齐属目录卫生/P0 相关入口整理（细项批次仍可按卫生总案 P1～P3 推进）
- [x] 若仍存在 TODO/TBD 清理台账，已更新状态或注明「仅归档范围保留」— 已在 `TODO_CLEANUP_INVENTORY_20260406.md` 文首增加 **2026-04-10 治理衔接备注**（与任务 5 联动；原 30 文件统计未重算）
...
- [x] **第1级：文件系统基线验证** — 所有文档编码统一为 UTF‑8‑SIG、零乱码、零不可见控制字符、行尾统一为 LF 格式、无重复文件、无冗余文件（2026‑04‑12）
- [x] **第2级：目录架构验证** — 11层垂直架构完整、所有目录均包含 INDEX.md、目录命名规范符合 [序号]_[主题] 标准、无稀疏目录（文件数≥3）（2026‑04‑12）
- [x] **第3级：元数据完整性验证** — 所有文档具备 module_id 唯一标识、layer 归属、version 版本号、responsibility 职责描述、module_id 全局唯一无重复（2026‑04‑12）
- [x] **第4级：全局索引闭环验证** — 所有文档可从根索引到达、所有文档包含反向索引、所有内部链接有效、无悬空引用、无循环引用（2026‑04‑12）
- [x] **第5级：架构边界验证** — 模块职责边界清晰、无职责重叠、数据流单向传递、无跨层耦合（2026‑04‑12）
- [x] **第6级：文档质量验证** — 每个模块至少3项具体职责、所有文档具备标题和摘要、变更记录完整、关键设计决策记录在案（2026‑04‑12）
- [x] **第7级：施工准入最终校验** — P0 问题 = 0、P1 问题 ≤ 3、总体评分 ≥ 90/100、施工准入证生成（2026‑04‑12）
...
- [x] **仓库根事故处置**已形成 仓库根治理 Playbook 并与 W2/W4 互指（2026-04-10，commit `b24eebce`）
...
- [x] [项目办公室 README](10_GOVERNANCE_COMPLIANCE/TRAINING_SYSTEM/README.md) 与 AI 交接说明 已能指到 §1.5 与本节（2026-04-10）

### W1：R0 文档层整体验收（超出蓝图清单者）

- [x] 已对照 全系统文档审计方案（若执行）或等效自查：**孤儿 / 断链 / 大门口索引** 有关闭标准或登记例外 — 2026-04-10：**等效自查** = 建设文档 [`INDEX.md`](../INDEX.md) 已与磁盘对齐（任务 4）+ 本批次 L1；全案 A～H **不**在本条代替正式审计签字，由 Owner 按方案排期
...
- [x] 大改后 **L1** 扫描已跑且无效内链为 0（报告路径可指认）— 2026-04-10：`python scripts/governance/sentinel_l1_governance_scan.py` → `docs/09_AUDIT/STATE/SENTINEL_L1_SCAN_20260408.md`；并修复 `HUMAN_AI_LAYER_DEEP_AUDIT_20260407_163712.md` 中 3 条错误相对路径（`../../../docs/09_AUDIT/` → `../../../09_AUDIT/`）

### W2：R1 仓库配置与门面

> 操作细则与事故分类：**仓库根治理 Playbook**。

- [x] `.gitignore`（及若有的 `.gitattributes`）已覆盖应排除目录与密钥模式；无已知的误提交大目录 — 2026-04-10：复核根 `.gitignore`；根目录误提交清理证据见 commit `9c2a9108` 与 仓库根治理 Playbook
- [x] 根 `README`（或等价入口）能指到建设文档 canonical 与办公室入口 — 2026-04-10：根 [`README.md`](10_GOVERNANCE_COMPLIANCE/TRAINING_SYSTEM/README.md) 已增「治理与建设文档」表（含 `06_CONSTRUCTION_DOCS/INDEX.md`、`00_MANAGEMENT/README.md`、交付标准、任务清单、施工门禁）
- [x] （可选）已运行密钥/秘密扫描或等价检查，发现问题已修或已登记风险 — 2026-04-11：**等价检查** = 仓库内 Python 对常见密钥型字面量模式扫描，命中文件数 **0**，记录见 `W2_SECRET_PATTERN_SPOTCHECK_20260410.md`；**不**替代专业 scanner（如 gitleaks）；若外规要求须另跑并登记。与 全仓库文件治理任务清单 §6 推荐阅读互指。

### W3：R2 脚本与 R3 工程

- [x] `scripts/` 内与治理相关的脚本在交付标准或任务清单中有**互指**或维护说明（不必逐文件作文档，但**用途可查**）— 2026-04-10：[`scripts/README.md`](10_GOVERNANCE_COMPLIANCE/TRAINING_SYSTEM/README.md) 已增「文档治理与门禁」表（互指本清单与仓库根 Playbook）
- [x] 源码根目录约定（文件夹结构、生成物位置）已写在根 README 或工程文档中，且与门禁 **§0.3** 可追溯要求不冲突 — 2026-04-10：根 README「项目结构」已列 `src/` 等；契约真源仍以 `API_Contract.md` 与施工门禁 §0.3 为准
...
- [x] 若使用 pre-commit / CI 拦截，配置已入库或文档中指明真源路径 — 2026-04-10：根目录 `.pre-commit-config.yaml`、`.github/workflows/` 已存在（文档质量/审计等工作流）
...
- [x] **P7.1.1** `DOCUMENT_REPOSITORY_LAYOUT_STANDARD` 已阅读并确认无与 Owner 意图冲突。
- [x] **P7.1.2** `03_CONSTRUCTION_PLANS/` 已存在且含 `INDEX.md`。
- [x] **P7.1.3** 蓝图终稿范围内各篇路径与链接符合 LAYOUT 表。

### §3 A 元数据风险验收

- [x] **P7.2.1 (A1)** L1：Markdown 内链判定无效 = 0。
- [x] **P7.2.2 (A2)** 双 YAML：`merge_double_yaml_frontmatter.py --list` → 0（已修复 14 个）。
- [x] **P7.2.3 (A3)** 首道 FM 重复 `module_id` 组 = 0。
- [x] **P7.2.4 (A4)** `audit_state` 仅 `04_OPERATIONS/audit_state` 为权威工作区。

### §3 B 架构/模块审核验收

- [x] **P7.3.1 (B1)** `ARCH_MODULE_GAP_REGISTER` 内登记项均为已修正或已 defer。
- [x] **P7.3.2 (B2)** Layer 11：`LAYER11_CAPABILITY_TO_BLUEPRINT_MAP` 与 `ARCHITECTURE.md` 一致（22/22，TBD=0）。
- [x] **P7.3.3 (B3)** `01_BLUEPRINTS` 全量对账完成（164 entries, 0 missing）。
- [x] **P7.3.4 (B4)** `docs/module_designs/` — **Owner 签字：N/A（当前阶段无垂直切片绑定需求），2026-04-11**。

### §3 C～F 审计阶段验收

- [x] **P7.4.1～P7.4.9 (C1～C9)** 审计阶段 A～H 已完成 — **Owner 签字：通过，2026-04-11**。

### §3 D～F 归档/技术审批/Backlog 验收

- [x] **P7.5.1～P7.5.6 (D1～D6)** 归档与历史验收完成 — **Owner 签字：通过，2026-04-11**。
- [x] **P7.6.1～P7.6.2 (E1～E2)** 技术审批验收完成 — **Owner 签字：通过，2026-04-11**。
- [x] **P7.7.1 (F1)** P2 Backlog — **Owner 签字：不阻塞施工（复审频率：每季度），2026-04-11**。

### 卫生总案退出标准验收

- [x] **P7.8.1～P7.8.5** 卫生总案退出标准已满足 — **Owner 签字：通过，2026-04-11**。
