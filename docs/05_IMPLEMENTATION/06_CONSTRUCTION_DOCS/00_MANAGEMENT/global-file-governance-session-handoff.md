...
| 机构常见能力 | 本仓库对应 |
|--------------|------------|
| 文档地图 + 放置规则 | DOCUMENT_REPOSITORY_LAYOUT_STANDARD、DOCUMENT_MAP_AND_PLACEMENT_GOVERNANCE、图纸柜 01_BLUEPRINTS_REPOSITORY_RULES |
| 基线清单与目录热力 | `git ls-files`、平面清单、`export_repo_directory_rollup.py` → `REPO_DIRECTORY_ROLLUP_*` |
| 链接健康门禁 | `sentinel_l1_governance_scan.py`（Invalid links → 0 为团队习惯；见 `SENTINEL_L1_SCAN_20260408.md` **判定无效 0**） |
| 首道 `module_id`（台账口径） | 同一份 L1 报告：**首道无 `module_id` → 0**、**跨文件重复 → 0**；散稿批量补全见 `backfill_missing_module_id.py`（治理工具总索引） |
| 蓝图/总清单机器校验 | `verify_01_*`、`verify_scattered_*`、`verify_manifest_paths_strict.py` |
| 同内容重复（文本类） | `scan_duplicate_file_content.py` + REPO_WIDE **§3**（C1/C2/D）；蓝图 D：**置信度 / 高置信可合并** Playbook **§2.5**；**低置信**合稿台账 D 类合稿待审登记 |
| 同名不同路径（basename · **非导航**） | `scan_basename_collisions.py` → `BASENAME_COLLISIONS_*`；**非导航类当前为 0**（2026-04-11，见 REPO_WIDE **§3.6 C2**）；`INDEX`/`README` 等导航名多份并存见报表「导航名」分表，默认不强制改名 |
| 蓝图 D 类重叠（启发式候选） | `scan_blueprint_d_overlap_candidates.py` → 最新 `BLUEPRINT_D_OVERLAP_CANDIDATES_20260412.md`（**非最终裁决**）；可选 `triage_blueprint_d_overlap_pairs.py` → `TRIAGE_20260412` + [`SECOND_PASS_QUEUE_20260412.jsonl`](../../../09_AUDIT/STATE/BLUEPRINT_D_OVERLAP_SECOND_PASS_QUEUE_20260412.jsonl) + 二审模板；规程 D 类 Playbook（**§2.5** 置信度、**§5** 双轨、**高置信可合并**）；低置信合稿登记 D 类合稿待审登记；任务清单 REPO_WIDE **§3.4.1**（D 类方案索引） |
| 索引健全性信号（v1） | `scan_index_health.py` → 零入链候选（非「必须在某 INDEX」裁决） |
| **「位置是否正确」分桶（与入链分立）** | 放置规程 §1.6 + REPO_WIDE §2.3.2 + LAYOUT §1 第 6 条；搬迁 PR 至少对照 **A（物理树）+ E（L1/可选零入链）** |
| 删稿裁决（人） | FILE_DELETION_OR_RETENTION_PLAYBOOK |
...
- **域内 INDEX 必列规则**的**自动化**（规则未冻结前不做硬门禁，见 放置规程 §5.3）。  
...
- **硬约束**：  
  1. **不**在未读 删稿裁决 Playbook 的情况下批量删除。  
  2. 合并重复须遵守 REPO_WIDE **§3**（尤其 C1 流程与归档区策略 **§3.1**）。  
  3. **推荐**在每批大规模改写 Markdown **相对链接**或搬迁路径后运行 `sentinel_l1_governance_scan.py`；团队默认以 **Invalid links = 0** 为门禁目标（Owner 书面例外可登记）。  
  4. 搬迁/新建路径须符合 LAYOUT 标准；拿不准时先读 文档地图与放置规则。  
  5. **「每一个文件」**：以**路径与链接可治理**为目标；**不承诺**对每份文件做业务语义审阅（见 REPO_WIDE **§1.1**）。  
  6. **批量编辑 Markdown**：以 **UTF-8** 保存；避免错误代码页与未经验证的全局替换；大改前 **commit 或分支**；全文规范见 文档编码标准，办公室摘要见 AI 交接 §0.1.3。

**阶段 A — 先读后扫（约 15～30 分钟）**

1. 阅读 REPO_WIDE **§0、§1、§1.1、§1.2**（扫描边界；**编码/乱码与 L1 正交**）。  
2. 阅读 治理工具总索引 全文。  
3. 阅读 文档地图与放置规则 **§1.6**（「位置是否正确」分桶表）+ **§3～§5**（扫描→归位→索引→零入链）。  
4. 若本轮含 **蓝图 D 类（主题可能重叠）**：阅读 D 类蓝图重叠 Playbook（**§2.5** 置信度；**§5** 高/低置信双轨与 **高置信可合并**；**§3.5** 分流/二审；与 REPO_WIDE **§3.4.1** 对表）。若执行 **低置信**合稿（新路径 + 旧稿 stub）：每例在 D 类合稿待审登记 **追加一行**（路径列用 Markdown 相对链，便于一点就跳）。
...
若 L1 报告 **首道无 `module_id` > 0**：先 `python scripts/governance/backfill_missing_module_id.py` 预览，再 `python scripts/governance/backfill_missing_module_id.py --apply`，**然后**再跑一次 `sentinel_l1_governance_scan.py`（见 治理工具总索引）。
...
1. **摆放**：对照 LAYOUT + 放置规程 **§4** 决定是否搬迁；动 `01_BLUEPRINTS` 时叠加图纸柜规则。  
...
6. **收口**：`git commit`；**默认**按 AI 交接 §0.2（正文与 `docs/09_AUDIT/STATE/` **分提交**、**分路径** `git add`）；**§0.1.4** 勿用 ignore 代替 STATE 策略；下一批前可再跑 `export_repo_directory_rollup.py --date YYYYMMDD` 看前缀是否「变薄」。
...
| 1.0.16 | 2026-04-11 | 硬约束增第 6 条（UTF-8 / 防乱码）；真源段与阶段 A 互指 REPO_WIDE **§1.2**、编码标准、AI 交接 §0.1 |
...
| 1.0.10 | 2026-04-10 | 阶段 B 增 `triage_blueprint_d_overlap_pairs.py`；§1.1 D 类行与阶段 A 互指 TRIAGE / SECOND_PASS_QUEUE / 二审模板 |
...
| 1.0.4 | 2026-04-10 | 真源段与 §1.3 自检互指 放置规程 §1.5、`ARCHITECTURE.md`、REPO_WIDE **§2.3.1**、AI 交接 **§3.2**（Layer 与路径防混） |
| 1.0.3 | 2026-04-10 | 机构对照表与阶段 A 互指 D 类合稿待审登记（低置信 + 可点击链） |
