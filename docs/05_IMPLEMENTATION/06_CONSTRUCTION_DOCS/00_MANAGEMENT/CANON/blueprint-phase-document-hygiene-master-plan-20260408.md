...
layer: layer_05
responsibility: "处理BLUEPRINT_PHASE_DOCUMENT_HYGIENE_MASTER_PLAN_20260408相关业务"
---
...
| **B. 重复（duplicates 池）** | `docs/09_ARCHIVE/duplicates/*.md` | `CANONICAL_POINTERS.md` | 表中 **无未裁决 TBD**（或全部有 Owner 与截止日）；非真源已 **Superseded** 声明 |
| **C. 重叠（overlap）** | `docs/06_ARCHIVE/overlap_*.md` | `OVERLAP_ORPHAN_PARALLEL_REMEDIATION_SCHEDULE_20260408.md` | 每篇有 **非真源 + canonical_path**（或登记过的 TBD） |
...
- `docs/09_AUDIT/REPORTS/`：以 `INDEX_GROUPED_REPORTS_20260408.md` 为枢纽，**禁止**把数百报告逐篇堆进单一 `INDEX.md`。
- `docs/07_RESEARCH/`、`docs/08_KNOWLEDGE/`：按子目录 `INDEX.md` 分批补链。

### P2 — duplicates 池「结案」

按 `DUPLICATE_DOCUMENT_HANDLING_STANDARD.md` 执行 **merge_then_delete / retain_trace**；删除前必须在台账更新 **disposition** 与日期。
