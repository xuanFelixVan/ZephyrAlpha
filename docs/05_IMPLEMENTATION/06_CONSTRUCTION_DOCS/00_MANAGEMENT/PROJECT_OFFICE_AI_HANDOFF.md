...
- **接力说明**：给**下一轮对话里的 AI** 或**下一位人类协作者**看的**短交代**——上一轮做到哪、**本批建议纳入提交的路径列表**、下一步先读哪份真源、跑哪几条门禁命令。**Git 暂存、何时复跑 L1、Markdown 如何保存避免乱码** 的统一口径见下文 **§0.1**；**人类只看结果、AI 默认怎么执行与怎么拆 commit** 见 **§0.2**。跨会话进度以 **PR / commit 描述** 与 [全仓库文件治理任务清单](./REPO_WIDE_FILE_GOVERNANCE_TASK_LIST.md) **§7** 勾选为准；**不**再维护「运行队列 / 当前指针」类独立附件。常见载体：[全局文件治理会话交接](./GLOBAL_FILE_GOVERNANCE_SESSION_HANDOFF.md)、本文件。  
- **接力说明不是**：把「未裁决的合并 / 删稿」交给下一任默认执行。**D 类**：**置信度与「高置信可合并」** 见 [D 类 Playbook](./D_CLASS_BLUEPRINT_OVERLAP_PLAYBOOK.md) **§2.5、§5.1**；**低置信**合稿仍须 [待审登记](./D_CLASS_CONSOLIDATION_PENDING_REVIEW_REGISTER.md) 与 Owner 书面口径。  
...
> **定位**：把「还能不能一锅端 add」「改链要不要立刻跑 L1」「乱码怎么少发生」收成**可执行摘要**；与 **§0「接力说明」**、任务清单 **§1.1 / §1.2 / §7.2**、合规 [文档编码标准](../../../10_GOVERNANCE_COMPLIANCE/DOCUMENT_ENCODING_STANDARD.md) **互指**，不另立第二套真源。
...
- 团队默认门禁目标：**无效内链 = 0**；首道 front matter **`module_id` 缺失 / 跨文件重复 → 0**（Owner 书面例外可登记）。L1 扫描的是工作区内 `*.md`，与 `git ls-files` 仅已跟踪可能不一致，见 [任务清单 §1.1](./REPO_WIDE_FILE_GOVERNANCE_TASK_LIST.md)。散稿补 id 见 [治理工具总索引](./GOVERNANCE_TOOLS_INDEX.md)。

### 0.1.3 Markdown 编码与防乱码（薄层）

- **全文真源**：[文档编码标准](../../../10_GOVERNANCE_COMPLIANCE/DOCUMENT_ENCODING_STANDARD.md)。  
- **实操摘要**：`.md` 以 **UTF-8** 保存；不以错误代码页打开/另存；**大范围替换或脚本改写**前先小范围验证，并**先 commit 或开分支**便于回退；从 PDF/网页粘贴时注意隐式错误编码与不可见字符。  
- **任务清单侧**（致因、与 L1 正交）：[REPO_WIDE §1.2](./REPO_WIDE_FILE_GOVERNANCE_TASK_LIST.md)。**全局尽治会话**中凡批量写 `.md`，编辑端亦应遵守 UTF-8，见 [GLOBAL 会话交接](./GLOBAL_FILE_GOVERNANCE_SESSION_HANDOFF.md) 文首硬约束与阶段 D。

---

## 0.2 Solo Owner · 全委托 AI — 默认执行协议（推荐真源 · 机械清单）

> **适用**：个人仓库、个人使用；人类**主要验收结果**，日常命令与提交由 **AI 执行**。目标是：**历史干净、diff 可读、门禁可复查**，且步骤**无需人类记命令**。  
> **冲突处理**：若当轮用户对话中**明文**要求（例如「不要提交 STATE」「允许 `git add -A`」），**以用户当轮为准**；否则执行下列顺序。

### 0.2.1 每一轮任务结束前，AI 默认按序执行

1. **`git status`**：列出变更；若出现与当轮任务**无关**的已修改文件，**不得**将其纳入本轮提交（应在回复中说明「未纳入的路径」供人类处理）。  
2. **若本轮改动了 Markdown 相对链接、搬迁 `.md`、合并/删稿影响外链**：在收口前运行一次 `python scripts/governance/sentinel_l1_governance_scan.py`。若报告 **判定无效 > 0**：**先修链**再复跑至 **0**（无用户明文例外则**不得**带着无效链收尾）。  
3. **暂存（禁止默认一锅端）**：仅使用 `git add <路径1> <路径2> …`（逐条列出当轮路径），**禁止**在未获用户**明文授权**时使用 `git add -A` / `git add .`。  
4. **双提交拆噪（推荐默认）**  
   - **提交 A — 正文与逻辑**：仅包含当轮**业务/文档正文**、脚本、配置等（**不包含** `docs/09_AUDIT/STATE/` 下因扫描新生成或大面积刷新的报表，**除非**当轮交付物本身就是某份 STATE 报告）。  
   - **提交 B — 证据快照（可选）**：若步骤 2 已更新 `docs/09_AUDIT/STATE/`（如 `SENTINEL_L1_SCAN_*.md/.json`）：在提交 A 之后，**再**单独 `git add` 本次变更的 STATE 路径并 **单独 `git commit`**，说明须标明「审计快照 / L1」等可识别字样。  
   - 若用户当轮明文 **「不要提交 STATE」**：跳过提交 B，仅在回复中报告 L1 结果文件路径与**判定无效**数值。  
5. **`commit message`**：完整中文句（做了什么、为何）；若 A/B 两次提交，**各写一条**，勿混写。  
6. **回复人类（验收用）**：用短列表列出——**已提交路径摘要**、**L1 判定无效数**、若有提交 B 则说明其与 A 的分工。

### 0.2.2 与人类习惯对齐的一句话

- **要「干净」**：默认 **正文与机器报表分 commit**；默认 **只 add 明确路径**；L1 **无效链归零**再交付。  
- **任务清单互指**：[REPO_WIDE §7.2](./REPO_WIDE_FILE_GOVERNANCE_TASK_LIST.md) 勾选模板与 **STATE 分提交** 说明。
...
| ①″-D（蓝图 D 类） | [D 类蓝图重叠 Playbook](./D_CLASS_BLUEPRINT_OVERLAP_PLAYBOOK.md) + [二审提示词模板](./D_CLASS_OVERLAP_SECOND_PASS_PROMPT_TEMPLATE.md) + [D 类合稿待审登记](./D_CLASS_CONSOLIDATION_PENDING_REVIEW_REGISTER.md) + [治理工具总索引](./GOVERNANCE_TOOLS_INDEX.md) §2 步 7～7′ | 主题可能重叠（非 C1/C2）：`scan_blueprint_d_overlap_candidates.py` → 可选 `triage_blueprint_d_overlap_pairs.py`（`TRIAGE_*` + `SECOND_PASS_QUEUE_*.jsonl`）+ **§3.5**；**§2.5 置信度** 与 **§5 双轨**（**高置信可合并** / 低置信待审）；**低置信**合稿须在登记表用 **Markdown 相对链**（一点就跳） |
...
若用户要你**改代码**而非文档，仍建议至少读完 ②；若动 `01_BLUEPRINTS`，再读 ③。若任务涉及**新建/搬迁 `docs/` 下路径、不确定应放哪棵树**，读 **①‴** 并对照 [LAYOUT 标准](../../../09_AUDIT/STANDARDS/DOCUMENT_REPOSITORY_LAYOUT_STANDARD.md)。若任务同时要**核对蓝图 Layer 字段或正文「Layer 定位」**，读 **①‴ §1.5** 与下文 **§3.2**（并对照 `ARCHITECTURE.md`）。若任务涉及**蓝图主题可能重叠（D 类）或低置信合稿**，读 **①″-D**。若任务涉及**重复/归档/ canonical 裁决**，再读 ⑦。若任务为**整仓文件一次尽治、按目录拆队列、合并重复**，读 **⑤′** 并对照 [办公室 README](./README.md)「办公室文件一览」。**凡将批量改 `.md`**：先过 **§0.1**（Git 暂存、L1 收口时复跑、UTF-8/防乱码）再动手。
...
与 **§3.1 文档治理 L0～L5** 不同：量化系统 **Layer 0～11** 指 [`ARCHITECTURE.md`](../../../01_FRAMEWORK/ARCHITECTURE.md) 中的**运行架构分层**。LAYOUT 标准 **§1 第 5 条**载有同主题摘要；归位或写蓝图时请按 [放置规程 §1.5](./DOCUMENT_MAP_AND_PLACEMENT_GOVERNANCE.md)：**先**对照 `ARCHITECTURE.md` 定模块主责 Layer，**再**按 LAYOUT 标准定 `docs/` 路径；**不得**从 `10_AI_WORKFLOW` 等路径前缀推断 Layer 编号。**勿**新建与上述链路并行的「Layer 放置标准」文件。
...
| 用户要你做的事 | 建议动作 |
|----------------|----------|
| 不知道某份文档应放在 `docs/` 哪棵子树 | 先查 [`DOCUMENT_REPOSITORY_LAYOUT_STANDARD.md`](../../../09_AUDIT/STANDARDS/DOCUMENT_REPOSITORY_LAYOUT_STANDARD.md) §2～§4；再按 [文档地图与放置规则](./DOCUMENT_MAP_AND_PLACEMENT_GOVERNANCE.md)（**§1.5**）与 rollup/§7 批次衔接；若仍无格，先走 LAYOUT **§6** 变更再建目录。 |
| 要标「模块在第几层」或核对 Layer 是否写对 | 以 [`ARCHITECTURE.md`](../../../01_FRAMEWORK/ARCHITECTURE.md) 分层表为准；**路径名 ≠ Layer** — 见 [放置规程 §1.5](./DOCUMENT_MAP_AND_PLACEMENT_GOVERNANCE.md) 与上文 **§3.2**。 |
...
| 处理同题多稿 / 重复 / 重叠 | 按 §1.5 与 [任务清单](./BLUEPRINT_PHASE_CLOSURE_TASK_LIST.md) 任务 1～2；程序与模板见 Playbook、[重复标准](../../../09_AUDIT/STANDARDS/DUPLICATE_DOCUMENT_HANDLING_STANDARD.md)、[`CANONICAL_POINTERS`](../../../09_ARCHIVE/duplicates/CANONICAL_POINTERS.md)。蓝图 **D 类**另见 [D 类 Playbook](./D_CLASS_BLUEPRINT_OVERLAP_PLAYBOOK.md)、可选 `triage_blueprint_d_overlap_pairs.py` + [二审模板](./D_CLASS_OVERLAP_SECOND_PASS_PROMPT_TEMPLATE.md)；**低置信**合稿登记 [待审登记](./D_CLASS_CONSOLIDATION_PENDING_REVIEW_REGISTER.md)。 |
| 全库 Markdown 内链 + 首道 `module_id` | 见上文 **§0.1.2**；命令与补 id 流程：[治理工具总索引](./GOVERNANCE_TOOLS_INDEX.md)；边界见 [任务清单](./REPO_WIDE_FILE_GOVERNANCE_TASK_LIST.md) **§1.1**。 |
...
| 版本 | 日期 | 说明 |
|------|------|------|
| 1.4.0 | 2026-04-11 | 新增 **§0.2**（Solo + 全委托 AI 机械清单：status→L1→分路径 add→正文/STATE 双提交→验收回复）；文首与 §0 互指 §0.2 |
...
| 1.3.2 | 2026-04-10 | 新增 **§3.2**（系统 Layer 0～11 vs `docs/` 路径 vs 文档治理 L0～L5）；①‴ / 常见任务 / 阅读顺序末段互指 [放置规程 §1.5](./DOCUMENT_MAP_AND_PLACEMENT_GOVERNANCE.md) |
