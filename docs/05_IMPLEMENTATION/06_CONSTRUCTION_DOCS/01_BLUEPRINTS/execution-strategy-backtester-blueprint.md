...
| TRANSACTION_COST_ANALYSIS_ENGINE_BLUEPRINT.md | 交易成本分析引擎蓝图 |
...
- **策略配置入口**：支持接收由 `STRATEGY_AUTHORING_ASSISTANT_BLUEPRINT.md` 生成并通过 Schema 校验的 `StrategyConfig` + `backtest_plan`（含 `dataset_id`/`seed`），确保回测可复现与可审计。
...
- 对至少 1 个执行策略与 1 段历史数据能够完成回测运行并输出可检查的绩效/成本指标（如成交率、滑点、冲击成本），且结果可复现（固定配置/随机种子）。
...
- 回测结果对撮合假设与成本模型高度敏感；实施阶段需在契约真源中固化撮合规则、成本模型与校准流程。
