...
** (Layer 2):
情情情情绪因子   - 与现有 5700+ 因子无缝集成
   - 供多因子模型使用

** (Layer 5):
   - 生成买卖信号、仓位建议、风险预警   - 与现有策略框架协同工作   - 增强策略信号质量
...
- **对外接口边界**：本模块负责将“参与者仿真”能力以可集成方式暴露给回测/研究/验证链路；不负责真实交易执行与生产撮合，不作为真实市场参与者画像的最终权威。

## 验收标准（可检查）

- 能够将至少 1 类参与者行为模型接入仿真/回测流程，并输出可追溯的参与者行为日志与统计摘要（含配置、随机种子与时间范围）。

## 已知限制

- 参与者行为模型高度依赖数据与假设，初期覆盖有限；实施阶段需通过契约真源扩展模型族与校准流程，并明确适用场景边界。
