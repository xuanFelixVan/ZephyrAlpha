...
- 全库 API 与事件约定真源：[`API_Contract.md`](../../../03_TRADING_TACTICS/API_Contract.md)。数据域、数据产品、共享接口、质量状态联动与变更事件等对外约定需以该真源或其子契约为准。
- 邻层协同边界：与 **Layer 0（数据源）**、**Layer 1（数据层/预处理）**、**Layer 10（治理与合规）** 的交互以契约为准（避免域口径与治理口径冲突）。

## 验收标准（可检查）

- 能定义至少一个数据域与一个数据产品，并能输出可复核的元数据（所有者、SLO、更新频率任一）。
- 能对数据产品发布/变更产生事件记录（版本、影响范围、回滚策略）并可追溯。
- 能关联质量状态（评分/告警任一）并可追溯到质量监控来源。
- 在违反策略或质量阈值时能触发告警/阻断策略并留痕。

## 已知限制

- 域模型字段字典、事件载荷与共享接口细化将在施工阶段固化到 `API_Contract.md` 子契约；本蓝图先确保边界、接口闭合点与验收闭环清晰。
