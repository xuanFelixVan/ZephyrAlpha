---
module_id: CI_CD_PIPELINE_001_1125
version: 1.0.0
status: Active
priority: P2
...
- 全库 API 与事件约定真源：`API_Contract.md`。CI/CD 与外部系统（代码托管、制品库、部署目标、告警通知）交互的对外口径（事件、回调、状态上报）以该真源为准。
...
- 全库 API 与事件约定真源：`API_Contract.md`。构建、测试、发布、回滚与通知事件等对外约定需以该真源或其子契约为准。
