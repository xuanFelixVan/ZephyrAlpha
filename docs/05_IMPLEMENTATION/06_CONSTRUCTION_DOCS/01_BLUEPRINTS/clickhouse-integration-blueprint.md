---
module_id: CLICKHOUSE_INTEGRATION_001_3461
version: 1.0.0
status: Active
...
- 全库 API 与事件约定真源：`API_Contract.md`。数据写入（ingest）、查询（query）、指标出数（metrics）、血缘/审计（audit）等对外交互口径以该真源为准。
...
- 全库 API 与事件约定真源：`API_Contract.md`。数据写入、查询、分区/TTL、资源配额与运行事件等对外约定需以该真源或其子契约为准。
