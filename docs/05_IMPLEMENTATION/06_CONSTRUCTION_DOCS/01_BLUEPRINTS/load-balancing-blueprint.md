---
module_id: LOAD_BALANCING_001_9498
version: 1.0.0
status: Active
...
| 路由/策略配置 | API.LB.001 | 配置路由规则与负载策略 |

| 健康检查状态 | API.LB.002 | 查询实例健康与摘除状态 |
...
- 输入格式: `route_rules/strategy_config/healthcheck_config`

- 输出格式: `routing_decision/instance_health/metrics`
...
- 在至少 2 个后端实例的测试环境中，能按策略分发请求，并在实例不健康时自动摘除，且有可观测指标/日志。

- 限流/熔断任一能力可验证：达到阈值后产生明确的阻断与记录。
...
- 负载均衡效果依赖真实流量分布与实例异构性；实施阶段需结合压测与观测数据调整参数并固化默认策略。
