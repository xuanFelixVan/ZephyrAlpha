...
| 接口名称 | 索引 | 说明 |
|----------|------|------|
| 流动性分析 | API.PCE.001 | estimate_liquidity接口 |
| 市场冲击评估 | API.PCE.002 | estimate_market_impact接口 |
| 容量计算 | API.PCE.003 | calculate_capacity接口 |
| 限制识别 | API.PCE.004 | identify_limitations接口 |

### 数据格式规范

- 输入格式: Dict (portfolio_weights), Dict (liquidity_data), Dict (volatility_data)
...
1. **流动性分析**: 能够计算平均成交量、成交量趋势、流动性评分
2. **市场冲击评估**: 使用平方根法则和线性模型估算市场冲击成本
3. **容量计算**: 能够计算组合总容量，识别瓶颈资产
4. **优化建议**: 能够提供降低权重、替代资产等优化建议

### 性能验收

| 指标 | 目标值 | 验证方法 |
|------|--------|----------|
| 容量计算时间 | <1s (100资产) | 性能测试 |
| 优化建议生成 | <2s | 性能测试 |
| 内存占用 | <300MB | 资源监控 |
...
1. **数据依赖**: 需要成交量、波动率等市场数据
2. **模型假设**: 市场冲击模型基于历史数据假设
3. **流动性变化**: 极端市场条件下流动性可能急剧变化
4. **实时性**: 容量估算基于历史数据，实时性有限

### 功能限制

1. **冲击模型**: 当前仅支持平方根和线性模型，非线性模型待扩展
2. **参与率限制**: 默认参与率上限10%，不支持动态调整
3. **多市场**: 不支持跨市场容量估算
