...
- 全库 API 与事件约定真源：[`API_Contract.md`](../../../03_TRADING_TACTICS/API_Contract.md)。质量规则、质量评分、告警事件、报告输出等对外约定需以该真源或其子契约为准。
- 邻层协同边界：与 **Layer 0（数据源）**、**Layer 1（预处理）**、**Layer 10（治理与合规）** 的交互以契约为准（避免规则口径与治理口径冲突）。

## 验收标准（可检查）

- 至少定义一组可执行的质量规则（完整性/准确性/及时性任一维度），并能对目标数据集产出可复核结果。
- 能生成质量评分与趋势（至少一个时间窗口），并能说明评分计算口径与阈值来源。
- 能对质量异常触发告警事件并留痕（含证据样本、影响范围、处理建议）。
- 能输出质量报告，并可追溯到规则版本、数据版本与运行记录。

## 已知限制

- 规则 DSL、字段级字典与事件载荷将在施工阶段固化到 `API_Contract.md` 子契约；本蓝图先保证边界清晰、接口闭合点明确与验收闭环可执行。
