...
- 全库 API 与事件约定真源：[`API_Contract.md`](../../../03_TRADING_TACTICS/API_Contract.md)。因子工厂对外输入（数据源、因子配置、回看窗口、频率、版本）与输出（因子值、评估指标、元数据、发布事件）如以接口/事件对外提供，其字段口径以该真源为准。

## 验收标准（可检查）

- 给定一组因子配置与样例数据，能生成非空因子输出（含时间戳/标的维度），并能复现（固定版本与参数）。
- 至少提供 1 个因子评估口径（如 IC/IR/覆盖率/换手）并输出结构化结果，便于下游筛选与审计。
- 对外输出/事件能在 `API_Contract.md` 中定位到契约入口（或在“已知限制”列出未闭合项）。

## 已知限制

- 因子效果与数据质量/样本区间强相关；落地阶段需固化数据口径、缺失处理与版本策略，并回填契约真源与回归用例。
## 设计目标
