...
- 全库 API 与事件约定真源：[`API_Contract.md`](../../../03_TRADING_TACTICS/API_Contract.md)。配置读取/写入/发布/回滚/热更新通知等对外口径（接口与事件）以该真源为准。

## 验收标准（可检查）

- 配置变更可追溯：任意一次变更可定位到版本号、变更人、变更时间、diff（至少可通过记录字段核对）。
- 支持回滚：指定版本可一键回滚并在目标服务侧生效（可通过版本号与生效时间验证）。
- 热更新延迟可测量并达标（例如 P95 < 5s，或按实际约束设定），失败时有降级策略（继续使用旧配置）。
- 对外接口/事件（配置发布、变更通知等）能在 `API_Contract.md` 中定位契约入口（或在“已知限制”列出未闭合项）。

## 已知限制

- 文档正文中存在较多历史占位与断行（如空表格/不完整条目），不影响本节门禁，但可读性与一致性需要另开专项批次清理。
...
**任务**:
- [ ] 集成验收项：联调阶段按部署环境与 API 契约补齐用例（见施工测试计划）
...
- 全库 API 与事件约定真源：[`API_Contract.md`](../../../03_TRADING_TACTICS/API_Contract.md)。配置版本管理、变更审批、热更新事件与审计留痕等对外约定需以该真源或其子契约为准。
- 邻层协同边界：与 **配置中心/配置消费方**、**Layer 10（治理与合规）** 的交互以契约为准（避免配置口径与审计口径冲突）。

## 验收标准（可检查）

- 能对一次配置变更形成版本号并可回滚（给出可复核的“变更前后 diff + 生效范围”）。
- 能对关键配置进行校验（schema/范围/依赖任一），失败时能阻断并告警留痕。
- 能记录变更审批或责任主体信息（若无审批流程则记录提交人/时间/原因），并可追溯。
- 能在发布后验证配置消费方已生效，并记录验证结果。
