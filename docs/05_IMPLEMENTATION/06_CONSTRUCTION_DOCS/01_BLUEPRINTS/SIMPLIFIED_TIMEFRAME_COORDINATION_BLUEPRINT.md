...
> 清风量化系统 v5.2 - 简化版多时间框架协同优化架构设计
> **索引**: `TIMEFRAME_COORD_001`
> **开发时间**: 80h（约2周）
> **核心定位**: 双时间框架协同（宏观季度 + 中观日度），实现信号融合与冲突解决
> **个人开发可行性**: ⭐⭐⭐ 部分可行（简化版）
> **AI维护难度**: 中
...
**原版设计**（Two Sigma实现）：
- 三级时间框架协同（宏观季度 + 中观日度 + 微观分钟）
- 复杂的信号融合机制
- 时间框架间的风险传递控制
- 开发时间：120h

**简化版设计**（个人开发）：
- ✅ **保留**: 双时间框架协同（宏观季度 + 中观日度）
- ✅ **保留**: 信号融合机制
- ✅ **保留**: 冲突检测与解决
- ❌ **放弃**: 微观分钟级时间框架
- ❌ **放弃**: 复杂的风险传递机制
...
- 作为信号冲突解决器，协调不同时间框架的决策
- 作为组合优化的输入，提供协同后的信号

### 1.4 核心功能清单

1. **信号融合**: 融合宏观和中观时间框架的信号
2. **冲突检测**: 检测不同时间框架的信号冲突
3. **冲突解决**: 解决信号冲突，生成最终决策
4. **权重调整**: 动态调整时间框架权重
...
class SimplifiedTimeframeCoordinator:
    ...
    def coordinate_signals(
        ...
        """
        # 1. 信号标准化
        normalized_macro = self._normalize_signals(macro_signals)
        normalized_meso = self._normalize_signals(meso_signals)
        
        # 2. 动态权重调整
        timeframe_weights = self.weight_adjuster.adjust_weights(market_state)
...
class SignalFusionEngine:
    ...
    def fuse(
        ...
        # 获取所有信号名称
        all_signal_names = set(macro_signals.keys()) | set(meso_signals.keys())
    ...
    def resolve(
        ...
        for conflict in conflicts:
            if conflict.conflict_type == 'direction':
                # 方向冲突：优先使用宏观信号（长期趋势）
                resolved_signals[conflict.signal_name] = Signal(
...
class TimeframeWeightAdjuster:
    ...
    def adjust_weights(
        ...
        if market_state.volatility == 'high':
            # 高波动市场：增加宏观权重（长期趋势更可靠）
            weights = {'macro': 0.5, 'meso': 0.5}
        elif market_state.trend == 'strong':
            # 强趋势市场：增加中观权重（短期趋势更明显）
            weights = {'macro': 0.2, 'meso': 0.8}
...
class ConflictConfig:
    ...
    direction_threshold: float = 0.1  # 方向差异阈值
...
| **M2: 冲突解决完成** | Day 4 | 冲突解决器 | 解决有效 |
| **M3: 权重调整完成** | Day 5 | 权重调整器 | 调整合理 |
