...
- **对外接口边界**：本模块对外提供“优化请求 → 优化结果（权重/约束满足性/诊断元数据）”的能力；不负责行情数据采集与清洗，不负责下单执行。

## 验收标准（可检查）

- 给定一组可行约束与输入数据时，优化器能够返回一组权重且满足约束可行性校验；对同一输入重复运行结果具备可重复性（在固定随机种子/求解器配置下）。

## 已知限制

- 交易成本、滑点、流动性等实盘因素属于实施阶段的精细建模项；蓝图阶段仅定义接口边界与验证口径，具体模型选择与参数默认值以契约真源为准。
