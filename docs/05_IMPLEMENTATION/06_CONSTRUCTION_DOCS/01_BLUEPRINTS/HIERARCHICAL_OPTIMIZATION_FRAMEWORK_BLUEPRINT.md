...
**单一职责**: 实现层次化优化框架，将大规模资产池分解为多个子问题进行分层优化

**职责边界**:
- ✅ 负责: 资产分组、分层优化、结果聚合、一致性协调
- ❌ 不负责: 单层优化求解（由MEAN_VARIANCE_OPTIMIZATION负责）
- ❌ 不负责: 因子模型构建（由BARRA_RISK_MODEL负责）
- ❌ 不负责: 约束定义（由CONSTRAINT_SOLVER负责）

### 1.2 业务价值

| 场景 | 问题 | 解决方案 |
|------|------|----------|
| 大规模资产池 | 500+资产导致协方差矩阵维度灾难 | 分层优化降低计算复杂度 |
| 多策略组合 | 策略间相关性难以建模 | 策略层→资产层双层优化 |
| 全球配置 | 跨市场资产相关性不稳定 | 区域层→国家层→资产层三层优化 |
| 行业配置 | 行业内资产高度相关 | 行业层→个股层分层优化 |

### 1.3 开源依赖

| 库名 | 版本 | 用途 | GitHub Stars |
|------|------|------|--------------|
| PyPortfolioOpt | >=1.5.0 | 单层优化引擎 | 4.2k+ |
| cvxpy | >=1.4.0 | 凸优化求解器 | 4.5k+ |
| scipy | >=1.10.0 | 层次聚类 | 12k+ |
| scikit-learn | >=1.3.0 | 聚类算法 | 58k+ |

### 1.4 与现有模块关系

```
HIERARCHICAL_OPTIMIZATION_FRAMEWORK (本模块)
├── 调用 MEAN_VARIANCE_OPTIMIZATION 进行子层优化
├── 调用 CONSTRAINT_SOLVER 处理跨层约束
├── 调用 DYNAMIC_CORRELATION_MODELING 获取相关性矩阵
├── 为 MULTI_STRATEGY_HIERARCHICAL_SYSTEM 提供优化引擎
└── 为 STRATEGIC_ALLOCATION_ENGINE 提供分层求解能力
```
