...
- 全库 API 与事件约定真源：[`API_Contract.md`](../../../03_TRADING_TACTICS/API_Contract.md)。数据编织的统一数据访问、虚拟化查询、跨域集成与变更事件等对外约定需以该真源或其子契约为准。
- 邻层协同边界：与 **Layer 0（数据源）**、**Layer 1（数据层/预处理）**、**Layer 10（治理与合规）** 的交互以契约为准（避免访问口径与治理口径冲突）。

## 验收标准（可检查）

- 能对至少一个跨域数据资产提供统一访问入口，并可复核“来源/权限/口径”元信息。
- 能执行一次虚拟化查询并返回可复核结果（含时间窗与口径说明），且性能目标可测量。
- 能记录一次跨域数据集成的变更事件（版本、影响范围、回滚策略）并可追溯。
- 在上游数据异常或权限不足时，能触发明确降级/阻断策略并留痕。

## 已知限制

- 统一访问 API、字段级字典与事件载荷将在施工阶段固化到 `API_Contract.md` 子契约；本蓝图先确保边界、接口闭合点与验收闭环清晰。
