...
- 全库 API 与事件约定真源：[`API_Contract.md`](../../../03_TRADING_TACTICS/API_Contract.md)。CI/CD 与外部系统（代码托管、制品库、部署目标、告警通知）交互的对外口径（事件、回调、状态上报）以该真源为准。

## 验收标准（可检查）

- 能从一次 PR/提交触发流水线，产出可审计的结果（构建日志、测试报告、覆盖率报告），并能定位到对应 run。
- 质量门禁可量化：例如 lint=0、单元测试通过率=100%、覆盖率≥阈值（项目自定），未达标则阻断合并/发布。
- 部署阶段有明确的“成功判定”与回滚触发条件（例如健康检查通过、关键接口 smoke test 通过）。
- 对外通知/状态上报（如 webhook、通知事件）能在 `API_Contract.md` 中定位到契约入口（或在“已知限制”列出未闭合项）。

## 已知限制

- 不同运行环境（GitHub Actions / 自托管 runner）对缓存、密钥注入与网络访问限制不同；本文不在蓝图阶段闭合全部环境差异，需在落地阶段固化。
...
- 全库 API 与事件约定真源：[`API_Contract.md`](../../../03_TRADING_TACTICS/API_Contract.md)。构建、测试、发布、回滚与通知事件等对外约定需以该真源或其子契约为准。
- 邻层协同边界：与 **Layer 5（服务/运行）**、**Layer 10（治理与合规）** 的交互以契约为准（避免发布口径与审计口径冲突）。

## 验收标准（可检查）

- 能对一次提交触发流水线并产出可复核结果（构建产物、测试报告、覆盖率任一）。
- 能在失败时给出明确失败原因与重试/回滚路径，并有事件留痕。
- 能在发布后执行健康检查并记录结果（成功/失败可复现）。
- 能对关键指标（构建时间、部署成功率）给出统计口径与一段时间窗内的结果。
