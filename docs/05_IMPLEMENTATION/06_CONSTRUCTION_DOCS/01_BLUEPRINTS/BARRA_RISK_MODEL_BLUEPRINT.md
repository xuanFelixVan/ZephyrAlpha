...
- 全库 API 与事件约定真源：[`API_Contract.md`](../../../03_TRADING_TACTICS/API_Contract.md)。Barra 风险模型对外输入（收益/因子暴露、样本区间、频率、约束、模型版本）与输出（因子收益、协方差/特质风险、风险预测、归因结果）如以接口/事件对外提供，其口径以该真源为准。

## 验收标准（可检查）

- 给定最小样例数据（若为伪数据亦可），能输出非空的风险预测结果（至少包含因子协方差或特质风险估计中的一项）且维度一致。
- 风险归因结果可复核：至少能输出“因子/行业/特质”任一维度的贡献拆解，并说明口径与窗口。
- 对外输出/事件能在 `API_Contract.md` 中定位到契约入口（或在“已知限制”列出未闭合项）。

## 已知限制

- 模型效果对样本选择与去极值/标准化等预处理高度敏感；落地阶段需固化数据口径、预处理流程与回测验证，并回填契约真源。
