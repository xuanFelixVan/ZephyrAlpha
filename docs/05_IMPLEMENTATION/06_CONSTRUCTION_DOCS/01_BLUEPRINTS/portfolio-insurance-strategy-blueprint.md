---
module_id: PORTFOLIO_INSURANCE_STRATEGY_001_6056
version: 1.0.0
status: Active
priority: P0
...
- **对外接口边界**：本模块输出保险策略建议（保护比例、风险预算、对冲建议）与策略参数；不直接执行对冲交易，不负责实时风控的最终裁决。
...
- 在给定组合价值、保护底线与风险乘数等输入时，能够产出可复现的目标风险资产配置/对冲建议，并生成可追溯的策略决策记录（含输入参数与时间戳）。
...
- 保险策略对冲工具选择（期权/期货/动态对冲）与交易成本模型属于实施阶段细化项；蓝图阶段仅定义接口边界与验收口径，具体实现以契约真源与 TDR/ADR 为准。
