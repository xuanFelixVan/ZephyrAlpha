...
| 接口名称 | 索引 | 说明 |
|----------|------|------|
| 加权求和优化 | API.MOO.001 | optimize_weighted_sum接口 |
| Pareto前沿优化 | API.MOO.002 | optimize_pareto_front接口 |
| Epsilon约束优化 | API.MOO.003 | optimize_epsilon接口 |
| 目标权衡分析 | API.MOO.004 | analyze_tradeoffs接口 |

### 数据格式规范

- 输入格式: numpy.ndarray (returns, cov_matrix), Dict (objectives)
...
1. **加权求和优化**: 能够根据风险权重生成有效前沿上的最优组合，权重和为1.0
2. **Pareto前沿**: NSGA-II算法生成至少50个非支配解，解集分布均匀
3. **目标权衡**: 能够分析收益-风险-流动性等多目标之间的权衡关系
4. **约束满足**: 所有优化结果满足约束条件（权重和=1，权重非负等）

### 性能验收

| 指标 | 目标值 | 验证方法 |
|------|--------|----------|
| 加权优化时间 | <1s (100资产) | 性能测试 |
| Pareto优化时间 | <30s (50解) | 性能测试 |
| 内存占用 | <500MB | 资源监控 |
...
1. **计算复杂度**: Pareto前沿优化计算量大，大规模资产(>200)建议减少解数量
2. **内存需求**: NSGA-II算法需要较大内存存储种群，建议≥4GB
3. **收敛性**: 复杂约束场景下可能需要增加迭代次数
4. **求解器依赖**: cvxpy需要ECOS/OSQP求解器支持

### 功能限制

1. **目标数量**: 建议目标数量≤5个，过多目标会导致Pareto前沿稀疏
2. **约束类型**: 当前仅支持线性约束，非线性约束待扩展
3. **动态优化**: 不支持时变目标权重调整
