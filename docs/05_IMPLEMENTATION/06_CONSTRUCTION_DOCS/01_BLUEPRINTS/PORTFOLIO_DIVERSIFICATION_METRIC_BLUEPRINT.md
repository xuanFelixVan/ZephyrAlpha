...
- 给定固定输入（权重/协方差/相关矩阵），分散度指标输出可复现（允许数值误差容差）。
- 至少覆盖 2 个核心指标（如 `effective_number_assets`、`diversification_ratio`）并提供可验证的计算示例或测试用例。
- 输出字段/单位/范围约束明确，可被下游直接消费（例如非负约束、归一化范围等）。

## 已知限制

- 指标解释依赖风险模型与输入数据口径；实施阶段需固化数据源、缺失处理与口径校准，并在契约真源中登记。
