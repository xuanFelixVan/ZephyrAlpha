...
| 工作包 | 周期（估） | 人时（估） | 价值摘要 | 优先级 |
|--------|------------|------------|----------|--------|
| Feast 因子中间库 | 4–6 周 | 96–160h | 因子数据管理统一 | **P0** |
| IC 分析引擎 | 3–4 周 | 72–120h | 因子有效性评估 | **P0** |
| PyPortfolioOpt 集成 | 2–3 周 | 48–80h | 组合优化能力 | **P1** |
| QLib AI 集成 | 6–8 周 | 240–320h | AI 因子挖掘 | **P2** |
| Streamlit 可视化 | 2–3 周 | 48–72h | 交互与可视化 | **P1** |

> 人时为个人开发+AI 协作的**粗估区间**，立项时按实际排期收紧。
...
| 差距项 | 机构级标准 | 当前实现 | 差距评级 | 优先级 |
|------|--------------|-----------------|----------|------------|
| **因子数据管理** | 专用Feature Store + 版本控制 | 文件系统存储，无版本管理 | **大** | **P0** |
| **计算架构** | 分布式计算 + 流批一体 | 单机 Pandas 计算 | **中** | **P2** |
| **回测系统** | 事件驱动 + 成本模型完整 | Backtrader 蓝图，未实现 | **大** | **P0** |
| **监控告警** | 实时监控 + 自动告警 | 与 `MONITORING_ALERTING_SYSTEM_BLUEPRINT` 对齐；本模块未单建栈 | **中** | **P1** |
| **AI集成** | AI 工作流完整集成 | 蓝图有，未实现 | **中** | **P2** |
| **回测覆盖** | 全策略/全资产覆盖 | 概念层 | **中** | **P1** |
...
2. **AI引擎**: 以 **QLib** 为参考架构，选择性集成 AI 因子挖掘能力
...
[5] 因子开发：量化投资的基础 - 今日头条。因子生命周期管理的最佳实践
...
- **策略配置入口**：支持接收由 `STRATEGY_AUTHORING_ASSISTANT_BLUEPRINT.md` 生成并通过 Schema 校验的 `StrategyConfig`，以便将“文字/对话”表达的策略意图落到可复现的因子回测任务。

## 验收标准（可检查）

- 至少 1 个因子策略能够完成“因子信号→回测→报告/指标输出”的端到端集成验证，并产出可复现的回测结果（含数据范围、参数与版本信息）。

## 已知限制

- 因子口径、数据对齐与回测假设差异会显著影响结果；实施阶段需在契约真源中固化数据契约与回测假设，并建立回归对账用例。
