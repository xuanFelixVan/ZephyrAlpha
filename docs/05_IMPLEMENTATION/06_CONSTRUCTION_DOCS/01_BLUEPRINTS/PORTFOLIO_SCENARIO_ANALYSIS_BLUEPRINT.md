...
- **对外接口边界**：本模块提供情景/敏感性分析的请求、计算与报告生成接口；不负责优化器求解与交易执行。

## 验收标准（可检查）

- 在给定组合标识与至少 1 个情景配置时，能够生成包含关键指标变化的情景结果，并可生成可下载/可追溯的报告输出（含情景参数与时间戳）。

## 已知限制

- 情景库与参数模板需要随策略域扩展逐步完善；初期可能只覆盖常见宏观/因子冲击场景，更多自定义情景在实施阶段按契约扩展。
