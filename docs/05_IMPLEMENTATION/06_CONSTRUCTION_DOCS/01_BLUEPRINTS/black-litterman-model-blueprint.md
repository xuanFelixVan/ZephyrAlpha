...
- 全库 API 与事件约定真源：`API_Contract.md`。本模块若以服务方式对外提供“观点输入/后验收益输出/风险贡献输出”等能力，接口字段与事件口径以该真源（及其子契约）为准。
...
**索引条目**:
```markdown
| Black-Litterman模型蓝图 | BLACK_LITTERMAN_MODEL_001 | v1.0.0 | Active | 2026-04-06 | 链接 |
```
...
- 全库 API 与事件约定真源：`API_Contract.md`。观点输入、置信度、先验/后验收益、权重输出与运行事件等对外约定需以该真源或其子契约为准。
