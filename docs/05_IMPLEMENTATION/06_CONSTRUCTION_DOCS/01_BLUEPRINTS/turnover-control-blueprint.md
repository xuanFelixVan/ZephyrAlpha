...
responsibility:
- 换手率控制
module_id: TURNOVER_CONTROL_001_1558
version: 1.0.0
status: Active
priority: P0
...
- **换手率计算接口**: `calculate_turnover()` - 计算组合换手率

- **约束设置接口**: `set_turnover_constraint()` - 设置换手率约束

- **优化接口**: `optimize_with_turnover_constraint()` - 带换手率约束的优化
...
- 对至少 1 组权重变化，能够输出可检查的换手率计算结果，并在优化场景下输出满足约束的权重方案（可复核）。
...
- 换手率约束与交易成本、跟踪误差存在权衡；实施阶段需在契约真源中固化约束口径、计算方法与降级策略。
