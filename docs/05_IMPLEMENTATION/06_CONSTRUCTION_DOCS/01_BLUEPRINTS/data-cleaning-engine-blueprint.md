---
module_id: 05_IMPLEMENTATION_06_CONSTRUCTION_DOCS_01_BLUEPRINTS_DATA_CLEANING_ENGINE_BLUEPRINT
layer: layer_05
version: 1.0.0
status: Active
priority: P2
responsibility:
  - Data Cleaning Engine Blueprint相关业务
...
- 全库 API 与事件约定真源：`API_Contract.md`。数据清洗的输入输出口径、清洗规则表达、异常处理事件与质量告警等对外约定需以该真源或其子契约为准。

- 邻层协同边界：与 **Layer 0（数据源）**、**Layer 1（预处理流水线）**、**Layer 10（治理与合规）** 的交互以契约为准（避免清洗口径与治理口径不一致）。
...
- 能对至少一个数据集执行清洗并输出“清洗前后对比指标”（缺失率、重复率、异常值命中率任一），结果可复核。

- 能记录清洗规则版本与运行记录（时间窗、范围、版本号），并可复现一次运行。

- 在检测到异常（缺失突增/极端值/格式错误）时，能触发告警事件并留痕（含证据样本与处置建议）。

- 能定义并验证降级策略（跳过、回填、冻结、隔离任一），并保留决策记录。
...
- 规则 DSL、字段字典与事件载荷将在施工阶段固化到 `API_Contract.md` 子契约；本蓝图先确保边界、接口闭合点与验收闭环清晰。
