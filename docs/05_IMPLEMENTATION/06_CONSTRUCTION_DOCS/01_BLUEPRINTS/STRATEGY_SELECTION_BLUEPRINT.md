...
> 清风量化交易系统 v5.1 - 策略排名与选择系统详细技术设计
> **索引**: `STRAT.SELECTION.001`
> **开发周期**: 80小时（胶合代码开发）
> **核心定位**: 策略工厂核心组件，支持120+策略多维度评分、智能排名、动态选择和市场适应性分析
> **参考开源**: vn.py的StrategyRanking + quant-system策略选择模块 + 多准则决策分析(MCDA)
...
| 目标 | 优先级 | 技术实现 |
|------|--------|----------|
| **多维度评分体系** | P0 | 收益、风险、稳定性、适应性等20+维度评分 |
| **动态权重调整** | P0 | 基于市场状态、风险偏好动态调整评分权重 |
| **策略相关性分析** | P1 | 收益相关性矩阵、风险分散度计算 |
| **AI辅助决策** | P1 | AI推荐策略组合、风险评估、市场匹配度分析 |
| **实时性能监控** | P2 | 策略运行状态监控、性能衰减检测 |
| **用户友好界面** | P2 | 可视化排名仪表盘、自然语言策略推荐 |

### 1.2 技术约束与原则

1. **客观公正原则**：评分标准透明可解释，避免黑箱决策
2. **动态适应性原则**：评分权重随市场状态、用户风险偏好动态调整
3. **风险分散原则**：避免选择高度相关的策略组合
4. **持续优化原则**：基于实盘表现持续更新策略评分
5. **用户友好原则**：不懂编程的用户也能理解评分逻辑和推荐理由

### 1.3 与现有系统集成

| 已有模块 | 集成方式 | 接口定义 |
|----------|----------|----------|
| **BatchEvaluation系统** | 绩效数据源 | 获取策略历史绩效数据 |
| **ParameterOptimization系统** | 参数优化结果 | 获取最优参数配置 |
| **StrategyEngine核心** | 策略元数据 | 获取策略类型、参数空间等信息 |
| **MarketStateDetector** | 市场状态输入 | 获取当前市场状态（牛市/熊市/震荡市） |
...
├─────────────────────────────────────────────────────────┤
│ 1. RankingDashboard - 排名仪表盘                        │
│ 2. RecommendationEngine - 推荐引擎                       │
│ 3. NaturalLanguageExplainer - 自然语言解释器             │
└─────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────┐
│                决策逻辑层 (Decision Logic)               │
├─────────────────────────────────────────────────────────┤
│ 1. MultiCriteriaEvaluator - 多准则评估器                 │
│ 2. WeightOptimizer - 权重优化器                          │
│ 3. PortfolioConstructor - 组合构建器                     │
│ 4. RiskDiversifier - 风险分散器                          │
└─────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────┐
│                评分计算层 (Scoring Layer)                │
├─────────────────────────────────────────────────────────┤
│ 1. PerformanceScorer - 绩效评分器                        │
│ 2. RiskScorer - 风险评分器                               │
...
class TOPSISEvaluator:
    ...
    def _default_weights(self) -> Dict[str, float]:
        """默认评分权重分配"""
        return {
            'sharpe_ratio': 0.25,      # 夏普比率：收益风险平衡
            'max_drawdown': 0.20,       # 最大回撤：风险控制
            'annual_return': 0.15,      # 年化收益：盈利能力
            'win_rate': 0.10,           # 胜率：交易质量
            'stability_score': 0.10,    # 稳定性：绩效一致性
            'adaptability_score': 0.10, # 适应性：市场环境适应能力
            'complexity_score': 0.05,   # 复杂度：策略简洁性
    ...
    def _calculate_ideal_solutions(self, weighted_matrix: pd.DataFrame) -> Tuple[pd.Series, pd.Series]:
        ...
        对于成本型指标（越小越好）：正理想解取最小值，负理想解取最大值
        """
        # 定义指标类型
...
class DynamicWeightOptimizer:
    ...
    职责: 基于市场状态和用户偏好动态调整评分权重
    特点: 自适应权重分配，提高策略选择准确性
    """
    ...
    def _calculate_base_weights(self, market_state: MarketState) -> Dict[str, float]:
        ...
        weight_templates = {
            ...
            'high_volatility': {
                'sharpe_ratio': 0.25,
                'max_drawdown': 0.30,   # 高波动市场特别重视风险控制
                'annual_return': 0.10,
                'win_rate': 0.10,
                'stability_score': 0.15,
        ...
        # 根据市场状态强度调整权重
        strength_factor = market_state.strength  # 0-1之间的强度值
        adjusted_weights = {}
        ...
        for criterion, base_weight in template.items():
            ...
            adjusted_weights[criterion] = min(adjusted, 0.5)  # 限制单指标最大权重
    ...
    def _adjust_for_user_preferences(self, base_weights: Dict[str, float],
        ...
        # 风险偏好调整
        risk_tolerance = user_prefs.risk_tolerance  # 1-5，1为极度保守，5为极度激进
...
class TimeWeightedPerformanceScorer:
    ...
    职责: 计算绩效指标评分，近期表现权重更高
    特点: 指数衰减加权，重视近期表现
    """
    
    def __init__(self, decay_factor: float = 0.9, min_periods: int = 20):
        self.decay_factor = decay_factor  # 衰减因子，越大表示历史权重越高
        self.min_periods = min_periods     # 最小计算周期
    ...
    def _time_weighted_annual_return(self, equity_curve: pd.Series) -> float:
        ...
        # 将权益曲线分段计算收益
        n_segments = min(10, len(equity_curve) // 30)  # 每段约30个交易日
        segment_returns = []
    ...
    def _time_weighted_max_drawdown(self, equity_curve: pd.Series) -> float:
        ...
        # 计算滚动最大回撤
        rolling_dd = self._rolling_drawdown(equity_curve, window=60)  # 60日滚动窗口
    ...
    def _generate_recommendations(self, corr_matrix: pd.DataFrame,
        ...
        for group in high_corr_groups:
            if len(group) > 1:
                recommendations.append(
                    Recommendation(
                        type='warning',
                        message=f"策略 {', '.join(group)} 高度相关，建议最多选择其中一个",
                        priority='high',
        ...
        if top_diversifiers:
            recommendations.append(
                Recommendation(
                    type='suggestion',
                    message=f"策略 {', '.join(top_diversifiers)} 具有高分散潜力",
                    priority='medium',
        ...
        if balanced_strategies:
            recommendations.append(
                Recommendation(
                    type='suggestion',
                    message=f"策略 {', '.join(balanced_strategies)} 提供良好的收益-风险平衡",
                    priority='medium',
...
class AIStrategyRecommender:
    ...
    def _construct_ai_portfolio(self, strategies: List[Strategy],
        ...
        # 根据用户风险偏好确定组合规模
        if user_prefs.risk_tolerance <= 2:  # 保守型
    ...
    def _generate_explanation(self, portfolio: Portfolio,
        ...
        explanations.append(
            f"组合风险等级: {risk_level}，最大回撤预期: {risk_assessment.expected_max_dd:.1%}"
        )
        ...
        if market_adaptability >= 0.7:
            ...
            adapt_explanation = "适应性中等"
        else:
            adapt_explanation = "适应性有限"
            
        explanations.append(f"市场适应性: {adapt_explanation}")
        
        # 策略选择理由
        top_strategy = portfolio.strategies[0]
        explanations.append(
            f"首选策略 {top_strategy.name}，因为其在当前{risk_assessment.current_market_state}市场环境下表现稳健"
        )
...
- [ ] StrategyCorrelationAnalyzer 相关性分析
- [ ] 时间加权评分算法
- [ ] 可视化排名报表
