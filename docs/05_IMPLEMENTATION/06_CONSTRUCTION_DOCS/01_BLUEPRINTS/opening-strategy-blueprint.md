...
- 全库 API 与事件约定真源：`API_Contract.md`。开盘策略对外输入（竞价/集合数据、盘口、交易时段状态）与输出（开盘信号、目标仓位、执行指令）如以接口/事件提供，其字段口径以该真源为准。
...
- 给定最小样例的开盘竞价/开盘行情数据，能输出非空的开盘信号结果，且可复现（固定 seed/参数）。

- 信号输出包含必要元数据（时间戳、标的、方向、强度/置信度、适用时段），便于下游执行/风控消费。
...
- 开盘微结构与撮合规则高度市场相关；落地阶段需固定数据源口径与回测/仿真验证流程，并回填契约真源与回归用例。
