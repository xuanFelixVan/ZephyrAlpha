...
负责实时数据湖的设计与构建和运行和操作，构建实时数据存储和查询平台，生成和输出低延迟数据访问，兼容和适配实时分析和决策。
...
- 能完成至少 1 路实时数据端到端入湖：流式接入→落盘→可查询（SQL/接口任一）并可追溯（含时间范围与分区）。

- 延迟与吞吐指标可测量：给出目标阈值与验证方法（例如 P95 写入延迟、每秒事件数）。

- 数据版本/回溯策略可验证：至少支持按时间点查询或版本标识查询（或在“已知限制”明确暂缺）。
...
- 多引擎（Trino/Spark/Delta 等）兼容与一致性需要实施阶段压测与参数调优；字段字典、分区策略与留存策略需回填契约真源。
