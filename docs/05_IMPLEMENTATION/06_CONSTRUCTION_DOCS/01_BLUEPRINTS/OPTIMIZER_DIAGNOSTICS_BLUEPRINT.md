---
module_id: OPTIMIZER_DIAGNOSTICS_001
version: 1.0.0
status: Active
...
- 全库 API 与事件约定真源：[`API_Contract.md`](../../../03_TRADING_TACTICS/API_Contract.md)。优化器诊断对外输入（优化问题、求解状态、解向量、约束与参数）与输出（诊断结论、问题清单、改进建议、告警事件）如以接口/事件对外提供，其口径以该真源为准。
