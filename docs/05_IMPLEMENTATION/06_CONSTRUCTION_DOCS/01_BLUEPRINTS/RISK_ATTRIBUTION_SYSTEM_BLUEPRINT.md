...
**业务需求**：
- 当前系统仅有绩效归因（在Layer 7），缺乏风险归因
- 无法分解组合风险来源（因子风险、行业风险、特质风险）
- 无法识别风险驱动因素，导致风险管理缺乏针对性
- 无法评估风险预算执行情况
...
**架构角色**: 
- 作为Two Sigma模式的核心组件，提供多维度风险归因
- 作为风险管理的分析工具，识别风险驱动因素
...
class RiskAttributionSystem:
    ...
    def attribute_risk(self,
        ...
        # 4. 汇总归因结果
        total_attribution = self._aggregate_attribution(
    ...
    def monitor_risk_budget(self,
        ...
        # 3. 识别超预算风险
        over_budget_risks = self._identify_over_budget(budget_utilization)
...
class FactorRiskAttributor:
    ...
    def _calculate_factor_risk_contribution(self,
        ...
        # 计算风险贡献百分比
        risk_contribution = marginal_risk_contribution / portfolio_risk
...
class AttributionReportGenerator:
    ...
    def generate(self,
        ...
        """
        # 1. 生成可视化图表
        charts = self._generate_charts(attribution_result)
    ...
    def _generate_charts(self, attribution_result: AttributionResult) -> Dict[str, str]:
        ...
        # 1. 因子风险贡献图
        charts['factor_risk'] = self._plot_factor_risk_contribution(
            attribution_result.factor_attribution
        )
        
        # 2. 行业风险贡献图
        charts['industry_risk'] = self._plot_industry_risk_contribution(
...
class RiskBudgetMonitorResult:
    ...
    over_budget_risks: List[Dict]  # 超预算风险
...
|------|------|--------|
| **风险归因** | 分析风险来源 | 风险分解 |
| **边际风险贡献** | 单位权重增加带来的风险增加 | 风险度量 |
| **主动风险** | 组合相对基准的风险 | 相对风险 |
| **风险预算** | 分配给各因子的风险限额 | 风险管理 |
