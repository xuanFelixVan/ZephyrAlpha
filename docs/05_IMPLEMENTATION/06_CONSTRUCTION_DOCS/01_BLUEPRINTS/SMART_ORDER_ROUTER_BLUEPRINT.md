...
- **对外接口边界**：本模块对外提供路由决策与候选路径的输出能力（含约束与诊断）；不执行撮合，不替代交易网关对最终成交回报的权威口径。

## 验收标准（可检查）

- 在给定订单与市场/通道状态输入时，能够输出可复核的路由选择结果（含候选与原因摘要），并记录输入摘要与版本信息以便追溯。

## 已知限制

- 路由效果依赖外部通道质量与延迟；实施阶段需在契约真源或子契约中固化通道健康指标、降级策略与回滚策略。
