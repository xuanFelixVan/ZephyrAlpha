---
module_id: PORTFOLIO_OPTIMIZATION_DIAGNOSTICS_001_0289
version: 1.0.0
status: Active
priority: P0
...
- **对外接口边界**：本模块输出诊断结论（数值稳定性、约束可行性、建议项）与可追溯证据；不负责替代优化器生成权重，也不负责执行交易或改变组合状态。
...
- 对至少两类典型失败场景（例如协方差矩阵病态、约束不可行）能够给出明确的诊断分类与建议项，并返回可用于复现的诊断元数据（输入摘要、检测项命中情况）。
