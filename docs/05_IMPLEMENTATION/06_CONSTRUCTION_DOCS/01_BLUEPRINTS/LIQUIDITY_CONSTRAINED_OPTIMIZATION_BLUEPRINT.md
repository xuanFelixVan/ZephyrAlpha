...
open_source_dependency: Riskfolio-Lib, 自研流动性评分
estimated_effort: 1周
layer: "Layer 4 (机器学习层)"
---
# 流动性约束优化蓝图
...
**单一职责**: 在组合优化中添加流动性约束，确保组合可执行性

**职责边界**:
- ✅ 负责: 流动性评分、流动性约束、大额交易冲击、分批执行计划
- ❌ 不负责: 市场冲击建模（由MARKET_IMPACT_MODEL负责）
- ❌ 不负责: 执行策略（由SMART_EXECUTION_ENGINE负责）
