...
responsibility:
- 简化风险预算系统
module_id: SIMPLIFIED_RISK_BUDGET_SYSTEM_001_9748
version: 1.0.0
status: Active
priority: P0
...
- **对外接口边界**：本模块对外提供简化风险预算的计算与监控结果输出；不执行交易，不替代风险管理对预算口径的最终定义。
...
- 在给定资产池与风险预算输入时，能够输出可检查的预算分配结果与基本诊断（约束满足性），并记录输入摘要与版本信息以便追溯。
...
- 简化模型对协方差估计与口径统一依赖较强；实施阶段需在契约真源或子契约中固化默认估计方法与降级策略。
