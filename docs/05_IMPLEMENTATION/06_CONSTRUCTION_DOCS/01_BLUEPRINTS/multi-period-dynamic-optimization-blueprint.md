---
module_id: MULTI_PERIOD_DYNAMIC_OPTIMIZATION_001_5918
version: 1.0.0
status: Active
priority: P0
...
- 全库 API 与事件约定真源：`API_Contract.md`。多期动态优化的输入（初始权重、各期收益/协方差、交易成本模型、再平衡频率、约束）与输出（各期最优权重轨迹、交易计划、诊断）如对外提供接口/事件，其字段口径以该真源为准。
