---
module_id: MARKET_REGIME_DETECTION_001_0501
version: 1.0.0
status: Active
...
- **对外接口边界**：本模块输出市场状态/ regime 分类与置信度、状态切换事件；不负责直接生成交易信号，不替代因子/策略侧的最终决策。

## 验收标准（可检查）

- 能够对至少 1 段历史数据完成状态识别并输出状态序列（含置信度或评分），并在检测到切换时产生可追溯事件记录（含时间与输入范围）。

## 已知限制

- regime 定义与标签集需要随资产类别与策略族扩展；实施阶段需在契约真源中固定标签口径、训练/校准流程与评估指标。
