---
module_id: COVARIANCE_ESTIMATION_ENHANCEMENT_001_3047
version: 1.0.0
status: Active
priority: P2
...
- 全库 API 与事件约定真源：`API_Contract.md`。协方差估计输入数据口径、输出矩阵结构、版本标识与运行事件等对外约定需以该真源或其子契约为准。

- 邻层协同边界：与 **Layer 6（组合优化）**、**Layer 10（治理与合规）** 的交互以契约为准（避免统计口径与审计口径不一致）。
...
- 不同估计方法（样本/EWMA/收缩/GARCH/因子模型）的参数与输出字段字典将在施工阶段固化到 `API_Contract.md` 子契约；本蓝图先确保边界、接口闭合点与验收闭环。
