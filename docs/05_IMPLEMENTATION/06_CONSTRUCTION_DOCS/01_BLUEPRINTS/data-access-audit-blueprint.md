...
- 全库 API 与事件约定真源：`API_Contract.md`。数据访问审计涉及的访问事件、权限变更、告警通知与合规报告输出等对外约定需以该真源或其子契约为准。
- 邻层协同边界：与 **Layer 0（数据源）**、**Layer 1（数据层/预处理）**、**Layer 10（治理与合规）** 的交互以契约为准（避免审计口径与权限口径冲突）。

## 验收标准（可检查）

- 能记录并查询至少一种访问事件（读取/导出/变更任一），并可追溯到主体、对象与时间范围。
- 能对高风险访问（越权/高频/异常时间窗）触发告警事件并留痕（含证据与处置建议）。
- 能生成审计报告并可复现（报告时间窗、统计口径、数据来源说明）。
- 能记录权限或策略变更的审计链路（变更前后、审批/提交人、版本号）。

## 已知限制

- 事件载荷、字段级分类分级字典与告警通道细化将在施工阶段固化到 `API_Contract.md` 子契约；本蓝图先确保边界、接口闭合点与验收闭环清晰。
