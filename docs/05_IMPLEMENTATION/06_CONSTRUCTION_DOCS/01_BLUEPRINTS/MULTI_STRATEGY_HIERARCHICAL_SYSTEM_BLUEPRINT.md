...
- 全库 API 与事件约定真源：[`API_Contract.md`](../../../03_TRADING_TACTICS/API_Contract.md)。多策略分层系统对外输入（策略收益/信号、风险约束/预算、市场状态）与输出（策略权重、融合信号、协调诊断、预警事件）如以接口/事件对外提供，其口径以该真源为准。

## 验收标准（可检查）

- 给定至少 2 条策略信号与一组风险约束，能输出一组策略权重（权重和为 1 或满足声明的归一化规则），且可复现（固定 seed/参数）。
- 能输出融合信号与权重决策的可解释摘要（至少包含输入版本、关键约束、选中策略/组合）。
- 对外输出/事件能在 `API_Contract.md` 中定位到契约入口（或在“已知限制”列出未闭合项）。

## 已知限制

- 策略间冲突的自动化裁决需要明确优先级/风控兜底规则；蓝图阶段不闭合所有冲突分支，需在落地阶段固化并同步契约真源。
...
| 指标 | 目标值 | 验证方法 |
|------|--------|----------|
| **权重调整延迟** | **P99 < 500ms**（单账户、常规持仓规模） | 性能测试 |
| **信号融合延迟** | **P99 < 200ms**（单次融合批） | 性能测试 |
| **策略夏普比率** | ≥1.0 | 回测验证 |
...
- 权重调整延迟：与 §5.3 表格一致（**P99 < 500ms**）
- 信号融合延迟：与 §5.3 表格一致（**P99 < 200ms**）
- 策略夏普比率 ≥ 1.0
