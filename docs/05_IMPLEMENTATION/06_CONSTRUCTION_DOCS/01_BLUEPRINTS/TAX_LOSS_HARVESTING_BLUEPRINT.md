...
负责税收损失收割模块的设计与实现，优化税务效率，实现损失收割策略，提升税后收益。本模块通过合理的损失实现策略，降低投资组合的税务负担。
...
- **对外接口边界**：本模块提供税务亏损收割策略的识别与执行建议接口；不替代合规审计的权威记录，不直接执行交易。

## 验收标准（可检查）

- 在给定持仓与市场数据输入时，能够输出可检查的收割建议（标的、数量、预期税务收益），并提供收割前后的税务影响对比（可复核）。

## 已知限制

- 税务亏损收割效果依赖税务规则与持仓成本数据；实施阶段需在契约真源中固化税务规则与数据口径。
