---
module_id: STRATEGIC_WEIGHTING_001_8767
version: 1.0.0
status: Active
priority: P0
...
- **对外接口边界**：本模块对外提供“战略权重/长期配置建议 + 约束/假设 → 权重与诊断”的输出能力；不执行交易，不替代风控对约束口径的最终裁决。
...
- 对至少 1 个组合与 1 套输入假设（目标风险、约束、基准/资产池）能够产出可复核的战略权重结果，并记录输入摘要与版本信息以便追溯。
