---
module_id: LIQUIDITY_MANAGEMENT_SYSTEM_001_8993
version: 1.0.0
status: Active
priority: P0
...
| 接口名称 | 索引 | 说明 |
|----------|------|------|
| 流动性画像查询 | API.LIQM.001 | 输出关键指标、窗口、阈值命中情况 |
| 流动性预警事件 | API.LIQM.002 | 输出预警级别、原因、建议动作 |

### 数据格式规范

- 输入格式: `account_id/portfolio_id/time_range`
- 输出格式: `liquidity_profile/alerts/recommendations`
- 时间戳格式: ISO 8601 UTC

## 验收标准（可检查）

- 能对至少 1 个账户/组合产出可追溯的流动性画像（指标、时间范围、数据来源），并可检索历史记录。
- 阈值预警可验证：触发条件与告警事件字段齐全，且具备去重/抑制策略说明（蓝图层可给原则）。
- 对外接口/事件能在 `API_Contract.md` 中定位到契约入口（或在“已知限制”列出未闭合项）。

## 已知限制

- 不同资产类别流动性指标口径差异较大；实施阶段需固化指标定义、粒度与缺失处理规则，并回填契约真源。
...
| **流动性计算速度** | 单次计算 | <50ms |
| **预测生成速度** | 30天预测 | <1s|
| **并发监控能力** | 同时监控账户数 | **实施期压测核定**；蓝图目标下限 **≥32** 账户（告警路径延迟满足上表） |
...
- REALTIME_RISK_HEDGE_ENGINE_BLUEPRINT.md - 实时风险对冲引擎
