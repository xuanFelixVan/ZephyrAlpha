...
### 2.1 情景分析器 vs 压力测试报告器

#### 职责边界定义

| 维度 | ScenarioAnalyzer | StressTestReporter |
|------|------------------|-------------------|
| **核心职责** | 情景定义和模拟 | 压力测试执行和报告 |
| **输入** | 情景类型、冲击参数 | 测试配置、组合数据 |
| **输出** | 情景分析报告 | 压力测试报告 |
| **调用频率** | 按需/周度 | 月度/季度 |
| **关注点** | 单一情景影响评估 | 多情景综合压力测试 |
...
EconomicRegimeReporter (经济范式分析)
        ↓ 提供宏观层报告
MultiTimeframeReportFusion (报告融合)
        ↓ 生成融合报告
```
...
| 版本 | 日期 | 作者 | 变更说明 |
|------|------|------|----------|
| v1.0.0 | 2026-04-03 | 蓝图架构师 | 初始版本，定义核心模块职责边界 |
