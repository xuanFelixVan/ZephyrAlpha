...
- 全库 API 与事件约定真源：[`API_Contract.md`](../../../03_TRADING_TACTICS/API_Contract.md)。CDC 事件（insert/update/delete）、变更流元数据（source、lsn/offset、timestamp、schema_version）与下游订阅方式的契约口径以该真源为准。

## 验收标准（可检查）

- 能从源端捕获三类变更（insert/update/delete），并在变更事件中包含可追溯的 offset/时间戳/源表标识（可通过日志或样例事件检查）。
- 端到端延迟（源端提交 → 下游可消费）在目标环境下可测量，并给出明确阈值（例如 P95 < 5s，或按实际约束设定）。
- 失败重试与幂等策略有明确约定：同一 offset 的重复投递不会导致下游重复入库（至少提供处理原则/去重键）。
- 对外事件/接口能在 `API_Contract.md` 中定位到对应契约入口（或在“已知限制”列出未闭合项与补全计划）。

## 已知限制

- 若源端数据库/中间件的 binlog/WAL 权限与保留策略不足，可能导致断点续传失败；需要在实现阶段固化运维前置条件并纳入契约/运行手册。
- 不同数据源（MySQL/Postgres/ClickHouse 等）对 CDC 语义支持差异较大，跨源一致性需要额外抽象层；本文不在蓝图阶段闭合所有源端差异。
...
- 全库 API 与事件约定真源：[`API_Contract.md`](../../../03_TRADING_TACTICS/API_Contract.md)。CDC 事件载荷、游标语义、幂等键、重放机制与告警事件等对外约定需以该真源或其子契约为准。
- 邻层协同边界：与 **Layer 0（数据源）**、**Layer 1（数据层/预处理）**、**Layer 10（治理与合规）** 的交互以契约为准（避免变更口径不一致）。

## 验收标准（可检查）

- 能对至少一个数据源产生 CDC 变更事件，并可复现（含游标位置与事件序号）。
- 能保证幂等处理（同一变更重复投递不产生重复落地），并给出验证方法。
- 能在中断后从游标恢复继续捕获，并记录恢复事件与耗时。
- 能在异常（延迟突增/积压/解析失败）时触发告警并留痕（证据与处置建议）。

## 已知限制

- 事件字段字典、游标模型与重放策略细化将在施工阶段固化到 `API_Contract.md` 子契约；本蓝图先确保边界、接口闭合点与验收闭环清晰。
