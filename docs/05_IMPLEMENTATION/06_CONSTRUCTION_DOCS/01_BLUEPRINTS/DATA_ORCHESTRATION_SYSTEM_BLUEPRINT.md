...
- 全库 API 与事件约定真源：[`API_Contract.md`](../../../03_TRADING_TACTICS/API_Contract.md)。数据编排/调度相关的任务定义、依赖关系、运行状态事件、失败重试与告警等对外约定需以该真源或其子契约为准。
- 邻层协同边界：与 **Layer 0（数据源）**、**Layer 1（预处理）**、**Layer 10（治理与合规）** 的交互以契约为准（避免调度状态与治理口径不一致）。

## 验收标准（可检查）

- 能对任一工作流给出“输入数据集 → 关键变换 → 输出数据集”的最小闭环，并可复现一次运行记录（含参数与版本）。
- 能记录并查询任务状态事件（开始/成功/失败/重试/超时），并能关联到告警通知记录。
- 能对依赖关系变更（上游/下游）生成变更记录与审批留痕（若有审批流程）。
- 在上游数据缺失或质量异常时，能执行明确的降级/阻断策略并留痕。

## 已知限制

- 任务 DSL/字段字典/运行事件载荷等细化将在施工阶段固化到 `API_Contract.md` 的子契约；本蓝图先锁定职责边界、接口闭合点与验收闭环。
