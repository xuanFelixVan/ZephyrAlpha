...
**评审结论**: 当前设计符合?5%，距离专业量化机构标准（?0%）还?*15%的差?*，需要进行优化调�?
...
2. **数据一�?*: `daily_pnl`在account_snapshots表中已有记录
...
5. `idx_orders_account_status`: 复合索引（账?�?时间，支持账户订单查询）
