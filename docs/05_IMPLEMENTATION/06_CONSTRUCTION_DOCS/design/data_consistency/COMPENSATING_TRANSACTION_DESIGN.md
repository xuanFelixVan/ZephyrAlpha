...
>
> **核心原则**: 幂等性、可重试性、业务语义完整�?
> **设计目标**: 保证数据一致性，支持自动恢复，最小化人工干预
> **技术架�?*: 逆序补偿 + 状态机 + 重试机制 + 监控告警
...
class CompensationTransaction:
    ...
    def __init__(self, original_tx_id: str, participant_id: str):
        ...
        self.status: str = "pending"           # 状�? pending, executing, completed, failed
        self.retry_count: int = 0              # 重试次数
        self.max_retries: int = 3              # 最大重试次�?
```
...
| 原则 | 描述 | 实现要求 |
|------|------|----------|
| **幂等�?* | 同一补偿操作执行多次的结果相�?| 补偿前检查状态，避免重复补偿 |
| **可重试�?* | 补偿失败后可安全重试 | 最大重试次数限制，指数退避策�?|
| **业务语义完整�?* | 补偿操作完全撤销原操作的业务效果 | 反向操作逻辑正确，无副作�?|
| **原子�?* | 补偿操作本身是原子的 | 单个参与方补偿要么成功要么失�?|
| **可观测�?* | 补偿过程完全可监�?| 详细日志记录，状态跟踪，指标监控 |
...
#### 2.2.1 补偿协调�?(CompensationCoordinator)
```python
class CompensationCoordinator:
    """补偿事务协调�?""
    
    def __init__(self, storage_client, event_bus):
        self.storage = storage_client  # PostgreSQL客户�?
        self.event_bus = event_bus     # Redis Streams客户�?
...
#### 2.2.2 补偿执行�?(CompensationExecutor)
```python
...
#### 2.2.3 补偿重试管理�?(CompensationRetryManager)
```python
...
class PositionTransferCommand:
    ...
    async def execute(self) -> TransferResult:
        ...
        if not source_result.success:
            return TransferResult.failed(f"源引擎扣减失�? {source_result.error}")
...
class PositionTransferCompensation:
    ...
    async def compensate(self) -> CompensationResult:
        ...
        if current_state.already_compensated:
            return CompensationResult.skipped("已补�?)
        ...
        try:
            ...
            if not source_credit.success:
                ...
                return CompensationResult.failed(f"源引擎增加失�? {source_credit.error}")
...
class CapitalAdjustmentCommand:
    ...
    def __init__(self, engine_id: str, adjustment_type: str, amount: float):
        ...
        self.adjustment_type = adjustment_type  # "increase" �?"decrease"
        self.amount = amount
    ...
    async def execute(self, tx_id: str, participant_id: str) -> CompensationResult:
        ...
        if not check_result.can_proceed:
            if check_result.already_compensated:
                ...
                return CompensationResult.skipped("正在执行�?)
            else:
                return CompensationResult.failed(f"幂等性检查失�? {check_result.reason}")
...
class AdaptiveRetryStrategy:
    ...
    def get_delay(self, retry_count: int, error_type: str) -> float:
        ...
        # 根据成功率调整延�?
        if success_rate < 0.3:  # 成功率低，增加延�?
            base_delay = 10.0
        elif success_rate < 0.7:  # 成功率中�?
            base_delay = 5.0
        else:  # 成功率高，减少延�?
            base_delay = 2.0
...
| 失败类型 | 原因 | 自动恢复可能�?| 人工干预需�?|
|----------|------|----------------|--------------|
| **临时性失�?* | 网络抖动、资源暂时不可用 | 高（可重试） | �?|
| **业务逻辑失败** | 业务规则冲突、数据不一�?| 中（需检查） | �?|
| **系统错误** | 数据库故障、服务宕�?| 低（需修复�?| �?|
| **数据损坏** | 数据丢失、损�?| 极低 | 高（需数据修复�?|
...
class ManualInterventionTrigger:
    ...
    async def trigger_intervention(
        ...
        await self.alert.send_alert(
            f"需要人工干�? {reason}",
            f"事务 {tx_id} 需要人工干预。详�? {json.dumps(details)}",
            severity="critical"
        )
...
COMPENSATION_METRICS = {
    # 计数�?
    "compensation_triggered_total": "补偿触发总次�?,
    "compensation_success_total": "补偿成功总次�?,
    "compensation_failed_total": "补偿失败总次�?,
    "compensation_retry_total": "补偿重试总次�?,
    ...
    # 仪表�?
    "compensation_success_rate": "补偿成功�?,
    "compensation_retry_rate": "补偿重试�?,
    "compensation_pending_count": "待处理补偿数�?,
    "compensation_in_progress_count": "进行中补偿数�?,
}
```
...
BUSINESS_METRICS = {
    ...
    "position_transfer_compensation_success_rate": "持仓转移补偿成功�?,
    
    # 资金相关
    "capital_adjustment_compensation_total": "资金调整补偿次数",
    "capital_adjustment_compensation_success_rate": "资金调整补偿成功�?,
    
    # 引擎相关
    "engine_compensation_total": "各引擎补偿次�?,
...
class TestRetryMechanism:
    ...
    async def test_max_retry_limit(self):
        ...
        result = await compensation_executor.execute_compensation(tx_id, participant_id)
        assert result.failed
        assert "超过最大重试次�? in result.error_message
```
...
class TestCompensationPerformance:
    ...
    async def test_concurrent_compensation(self):
        ...
        assert success_count >= 45, f"并发补偿成功率过�? {success_count}/50"
```
...
| **Python** | 3.9+ | 安装asyncpg, redis-py等依�?| 应用服务�?|
| **监控系统** | Prometheus 2.0+ | 配置抓取和告警规�?| 独立部署 |
...
| 故障现象 | 可能原因 | 排查步骤 | 解决方案 |
|----------|----------|----------|----------|
| 补偿成功率下�?| 网络问题、引擎故�?| 1. 检查网络连�?br>2. 检查引擎状�?br>3. 查看错误日志 | 1. 修复网络<br>2. 重启引擎<br>3. 调整重试策略 |
| 补偿延迟增加 | 数据库性能问题、资源不�?| 1. 检查数据库性能<br>2. 检查系统资�?br>3. 分析慢查�?| 1. 优化数据�?br>2. 扩容资源<br>3. 优化查询 |
| 补偿堆积 | 处理能力不足、频繁失�?| 1. 检查处理线�?br>2. 分析失败原因<br>3. 查看队列长度 | 1. 增加处理能力<br>2. 修复根本原因<br>3. 临时增加资源 |
...
class CompensationDisasterRecovery:
    ...
    async def _recover_from_database_outage(self) -> RecoveryResult:
        ...
        return RecoveryResult.success("数据库故障恢复完�?)
```
...
1. [多引擎数据一致性设计方�?md](多引擎数据一致性设计方�?md) - 主设计文�?
2. [Saga模式实现流程�?md](Saga模式实现流程�?md) - 流程图文�?
3.  - 引擎适配器设�?
4. [STORAGE_TIER.md](../../05_IMPLEMENTATION/04_INFRASTRUCTURE/STORAGE_TIER.md) - 存储层设�?

---

**文档版本历史**:
- v1.0.0 (2026-04-02): 初始版本，完整补偿事务设�?
