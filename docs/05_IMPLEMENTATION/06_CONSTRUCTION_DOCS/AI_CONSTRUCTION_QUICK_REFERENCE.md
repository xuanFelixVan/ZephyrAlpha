...
owner: 首席架构师
standard_type: AI施工快速参考
applicable_scope: AI智能体施工必读
...
> **完整版**: [蓝图施工说明书](./CONSTRUCTION_SPECIFICATION.md)
...
□ 3. 文件命名是否正确？
     - Python文件: 小写+下划线 (strategy_factory.py)
     - 文档文件: 大写+下划线 (STRATEGY_FACTORY_GUIDE.md)
     - 配置文件: 小写+下划线 (strategy_config.yaml)
...
│
├── src/                     # 所有源代码
│   ├── strategy/           # 策略模块（不是strategies/）
│   ├── event_bus/          # 事件总线（不是event/或events/）
│   ├── backtest/           # 回测引擎（不是backtesting/）
│   ├── risk/               # 风险管理（不是risk_management/）
│   └── execution/          # 执行引擎（不是execution_engine/）
│
├── tests/                   # 所有测试
├── config/                  # 所有配置
├── scripts/                 # 所有脚本
...
| **类** | `StrategyFactory` | `strategy_factory` |
...
1. **查看完整版**: [蓝图施工说明书](./CONSTRUCTION_SPECIFICATION.md)
2. **检查现有结构**: 使用 LS 命令
3. **参考已有文件**: 查看类似文件的结构
4. **询问用户**: 如果不确定，先询问用户
