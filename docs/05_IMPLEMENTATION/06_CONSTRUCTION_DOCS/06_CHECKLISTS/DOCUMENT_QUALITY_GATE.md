...
| 检查项 | 标准 | 错误级别 | 自动化 |
|--------|------|---------|--------|
| module_id | 必须存在，格式正确 | 🔴 阻断 | ✅ |
| version | 必须存在，格式: x.y.z | 🔴 阻断 | ✅ |
| status | 必须存在，值为Active/Inactive | 🔴 阻断 | ✅ |
| created_date | 必须存在，格式: YYYY-MM-DD | 🔴 阻断 | ✅ |
| last_updated | 必须存在，格式: YYYY-MM-DD | 🔴 阻断 | ✅ |
| owner | 必须存在，非空 | 🔴 阻断 | ✅ |
| standard_type | 必须存在，非空 | 🟡 警告 | ✅ |
| applicable_scope | 必须存在，非空 | 🟡 警告 | ✅ |
| compliance_level | 必须存在，非空 | 🟡 警告 | ✅ |
| parent_document | 必须存在，路径有效 | 🟡 警告 | ✅ |

#### 1.2 字段格式检查

| 检查项 | 标准 | 错误级别 | 自动化 |
|--------|------|---------|--------|
| module_id格式 | 大写字母+下划线+数字 | 🔴 阻断 | ✅ |
| version格式 | 语义化版本: x.y.z | 🔴 阻断 | ✅ |
...
| 检查项 | 标准 | 错误级别 | 自动化 |
|--------|------|---------|--------|
| API一致性 | API文档与代码一致 | 🔴 阻断 | ⚪ |
| 配置一致性 | 配置文档与实际配置一致 | 🔴 阻断 | ⚪ |
| 示例可运行 | 示例代码可运行 | 🟡 警告 | ⚪ |
...
class DocumentQualityGate:
    ...
    def check_structure(self) -> bool:
        ...
        if h1_count == 0:
            self.errors.append("缺少一级标题")
        elif h1_count > 1:
            self.warnings.append("存在多个一级标题")
...
```bash
# 检查单个文档
python scripts/document_quality_gate.py docs/05_IMPLEMENTATION/06_CONSTRUCTION_DOCS/README.md

# 批量检查所有文档
find docs -name "*.md" -exec python scripts/document_quality_gate.py {} \;
```
...
| 日期 | 版本 | 更新内容 | 更新人 |
|------|------|---------|--------|
| 2026-04-02 | v1.0 | 创建文档质量门禁机制 | 首席文档架构师 |
