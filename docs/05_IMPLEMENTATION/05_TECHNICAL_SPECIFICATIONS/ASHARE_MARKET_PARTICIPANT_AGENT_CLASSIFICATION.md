...
- **核心组件**:
  ...
  - MarketMicrostructureAnalyzer (市场微观结构分析器)
...
- **核心组件**:
  - BehavioralFinanceModel (行为金融学模型)
  - HerdingEffectSimulator (羊群效应模拟器)
...
### 2.4 外资智能体 (Foreign Investor Agent) ⭐ 新增
...
- **核心组件**:
  - GlobalMacroAnalyzer (全球宏观经济分析器)
  - ExchangeRateMonitor (汇率监控器)
...
### 3.1 保险资金智能体 (Insurance Fund Agent) ⭐ 新增
...
- **核心组件**:
  ...
  - DurationMatcher (久期匹配器)
  - RiskBudgetAllocator (风险预算分配器)
  - GLM47Flash (LLM辅助决策)
...
### 3.2 产业资本智能体 (Industrial Capital Agent)
...
- **核心组件**:
  ...
  - IndustrialLogicAnalyzer (产业逻辑分析器)
...
### 3.3 社保/养老金智能体 (Pension Fund Agent)
...
class MarketSimulationEngine:
    ...
    def simulate_market(self, market_state: MarketState) -> MarketResult:
        ...
        # 2. 订单簿聚合
        self.order_book.aggregate_orders(agent_decisions)
...
**数据源定位**: 系统核心数据源，提供5700+专业因子、舆情数据、财务数据、宏观数据

**iFind数据能力**:

| 数据类型 | iFind支持情况 | 数据内容 | 更新频率 | 适用智能体 |
|---------|-------------|---------|---------|-----------|
| **因子数据** | ✅ 完全支持 | 5700+专业因子（价值、质量、成长、情绪、技术等） | 日频 | 所有智能体 |
| **财务数据** | ✅ 完全支持 | 财务报表、基本面数据、估值数据 | 季频 | 外资、保险资金、主力 |
| **舆情数据** | ✅ 完全支持 | 新闻、公告、研报、社交媒体 | 实时 | 外资、散户、主力 |
| **宏观数据** | ✅ 完全支持 | 中国宏观经济指标、汇率数据 | 月频 | 国家队、外资、主力 |
| **市场数据** | ✅ 完全支持 | 行情、交易数据、北向资金 | 日频/实时 | 所有智能体 |
| **全球宏观** | ⚠️ 部分支持 | 人民币汇率、北向资金（全球宏观数据需补充） | 日频 | 外资智能体 |

**iFind连接器**: 详见 [IFIND_CONNECTOR.md](./IFIND_CONNECTOR.md)

### 5.2 智能体数据需求与iFind映射

#### 5.2.1 外资智能体数据需求映射

| 数据需求 | iFind数据接口 | 因子ID/指标代码 | 优先级 |
|---------|-------------|---------------|--------|
| **全球宏观评估** | | | |
| 美债收益率 | ⚠️ 需补充 | FRED API (GS10) | 高 |
| 美联储政策 | ⚠️ 需补充 | FRED API (FEDFUNDS) | 高 |
| 美元指数 | ⚠️ 需补充 | Yahoo Finance (DXY) | 中 |
| VIX恐慌指数 | ⚠️ 需补充 | Yahoo Finance (^VIX) | 中 |
| **汇率分析** | | | |
| 人民币汇率 | ✅ get_macro_data | USDCNY | 高 |
| 离岸人民币 | ✅ get_macro_data | USDCNH | 高 |
| **北向资金** | | | |
| 北向资金流入 | ✅ get_market_data | northbound_flow | 高 |
| 北向资金持仓 | ✅ get_market_data | northbound_holdings | 高 |
| **财务数据** | | | |
| 财务报表 | ✅ get_financial_statements | income_statement/balance_sheet/cash_flow | 高 |
| 自由现金流 | ✅ get_factor_data | FCF_TTM | 高 |
| ROE/ROA | ✅ get_factor_data | ROE_TTM, ROA_TTM | 高 |
| **估值因子** | | | |
| PE/PB/PS | ✅ get_factor_data | PE_TTM, PB_LF, PS_TTM | 高 |
| EV/EBITDA | ✅ get_factor_data | EV_EBITDA | 中 |
...
|---------|-------------|---------------|--------|
| **股息数据** | | | |
| 股息率 | ✅ get_factor_data | DIV_YIELD | 高 |
| 分红率 | ✅ get_factor_data | DIV_PAYOUT_RATIO | 高 |
| 分红增长率 | ✅ get_factor_data | DIV_GROWTH_3Y, DIV_GROWTH_5Y | 高 |
| 分红稳定性 | ✅ get_factor_data | DIV_STABILITY_5Y | 高 |
| **财务质量** | | | |
| 资产负债率 | ✅ get_factor_data | DEBT_RATIO | 高 |
| 流动比率 | ✅ get_factor_data | CURRENT_RATIO | 高 |
| 速动比率 | ✅ get_factor_data | QUICK_RATIO | 中 |
| 经营现金流 | ✅ get_factor_data | OPERATING_CASH_FLOW | 高 |
| 自由现金流 | ✅ get_factor_data | FREE_CASH_FLOW | 高 |
| **估值因子** | | | |
| PE/PB | ✅ get_factor_data | PE_TTM, PB_LF | 高 |
| **久期数据** | | | |
| 股票久期 | ⚠️ 需计算 | 基于股息数据计算 | 中 |
| Beta系数 | ✅ get_factor_data | BETA | 中 |
| 波动率 | ✅ get_factor_data | VOLATILITY_1Y | 中 |

#### 5.2.3 其他智能体数据需求映射

| 智能体 | 核心数据需求 | iFind支持情况 | 备注 |
|-------|------------|-------------|------|
| **国家队智能体** | 市场稳定指标、宏观政策 | ✅ 完全支持 | 宏观数据、市场数据 |
| **主力智能体** | 基本面数据、机构持仓 | ✅ 完全支持 | 财务数据、因子数据 |
| **散户智能体** | 市场情绪、技术指标 | ✅ 完全支持 | 舆情数据、技术因子 |
| **产业资本智能体** | 内部人交易、产业政策 | ✅ 完全支持 | 公告数据、舆情数据 |
| **社保/养老金智能体** | 稳健收益、长期价值 | ✅ 完全支持 | 财务数据、因子数据 |
| **量化私募智能体** | 高频数据、技术指标 | ✅ 完全支持 | 市场数据、技术因子 |

### 5.3 补充数据源方案

**补充原因**: iFind主要覆盖中国A股市场数据，全球宏观数据需要补充
...
class MarketParticipantDataFetcher:
    ...
    def fetch_insurance_fund_data(self,
        ...
        # 估值数据
        data['valuation'] = self.ifind.get_factor_data(
            symbols=symbols,
