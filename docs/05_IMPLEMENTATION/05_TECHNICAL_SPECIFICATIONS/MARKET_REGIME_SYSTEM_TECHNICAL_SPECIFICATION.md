...
> 清风量化系统 v5.2 - 市场状态识别系统详细技术设�?> **索引**: `MARKET_REGIME_001`
> **开发时�?*: 160h
...
**预期价�?*�?- 实现对市场状态的准确识别（准确率�?0%�?- 提前3-5天预警市场状态转�?- 为策略选择提供市场环境上下�?- 降低策略在不同市场状态下的失效风�?
### 1.2 技术定位与架构层归�?
...
| **Alpha因子工厂** | 下游依赖 | 事件发布 | v1.0+ | 提供市场状�?|
...
class MarketStateOutput:
    """市场状态输�?""
    market_regime: str                   # 市场状�?(bull/bear/sideways/transition)
...
CREATE TABLE market_regime_results (
    ...
    market_regime VARCHAR(20) NOT NULL COMMENT '市场状�?,
...
CREATE TABLE market_regime_history (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    trade_date DATE NOT NULL COMMENT '交易日期',
    actual_regime VARCHAR(20) COMMENT '实际状�?,
    predicted_regime VARCHAR(20) COMMENT '预测状�?,
...
class TechnicalRegimeIndicator:
    ...
    def analyze(self, trend_indicators: List[str],
        ...
        # 综合判断市场状�?        if trend_score > 0.6 and momentum_score > 0.6:
            regime = 'bull'
...
def _fuse_decisions(self, hmm_state: str, hmm_prob: float,
    ...
    Args:
        hmm_state: HMM识别状�?        hmm_prob: HMM置信�?        tech_state: 技术指标状�?        microstructure: 微观结构分析结果
