...
> 清风量化系统 v5.2 - DataValidator数据校验器模块详细技术设计
> **模块ID**: `PREP_VAL_001`
> **版本**: v1.0.0
> **状态**: ✅ 正式
...
### 1.1 设计背景与业务目标
- **业务需求**: 系统需要全面的数据质量校验能力，确保数据可靠性、完整性和准确性
- **技术痛点**: 
  - 数据缺失、异常值影响分析结果
  - 数据源不一致导致数据冲突
  - 缺乏统一的数据质量评估标准
...
- **职责范围**: 负责数据质量校验、质量评估、问题诊断
- **上下层接口**: 
  - 上层依赖: Layer 2 因子计算引擎、Layer 4 机器学习层 (提供质量保证的数据)
  - 下层依赖: Layer 1 DataCleaner、DataNormalizer (接收清洗和标准化后数据)

### 2.3 模块职责与边界定义
- **核心职责**: 数据质量校验、质量评估、问题诊断、报告生成
- **职责边界**: 
  - ✅ 本模块负责: 数据质量校验、质量评估、问题诊断、报告生成
  - ❌ 本模块不负责: 数据清洗、数据标准化、数据转换
...
| pydantic | 弱依赖 | Python库 | >=1.8.0 | 数据验证 |
...
| 性能指标 | 目标值 | 测量方法 |
|----------|--------|----------|
| 单数据集校验时间 | < 3秒 | 10000行×100列 |
| 批量校验时间 | < 30秒 | 10个数据集 |
| 流数据校验延迟 | < 100ms | 单批次流数据 |
| 质量评分计算时间 | < 500ms | 单数据集 |
| 报告生成时间 | < 5秒 | HTML格式 |
| 缓存命中率 | ≥ 70% | 重复校验场景 |
...
| 缓存类型 | TTL | 淘汰策略 | 最大容量 |
|----------|-----|----------|----------|
| 校验结果缓存 | 24小时 | LRU | 1000条 |
| 规则缓存 | 永久 | 无 | 100条 |

### 4.3 数据持久化
- **持久化需求**: 校验规则、校验结果需要持久化存储
- **存储格式**: JSON或YAML格式
...
|--------|----------|------------|
| 完整性检查 | 缺失值、空值、重复 | 100% |
| 一致性检查 | 跨源、时间、逻辑一致性 | 100% |
| 有效性检查 | 范围、格式、类型检查 | 100% |
| 准确性检查 | 基准对比、统计检查 | 100% |
| 质量评分 | 评分计算、等级映射 | 100% |
| 问题分类 | 分类算法、优先级排序 | 100% |
...
| 风险ID | 风险描述 | 风险等级 | 缓解措施 |
|--------|----------|----------|----------|
| R001 | 校验规则配置复杂 | P1 | 提供默认规则、规则模板 |
| R002 | 大数据量校验性能 | P1 | 分块校验、并行处理 |
| R003 | 校验标准不统一 | P2 | 建立校验标准库、规则版本管理 |
| R004 | 误报和漏报 | P2 | 规则调优、机器学习辅助 |
...
| 功能项 | 验收标准 | 验证方法 |
|--------|----------|----------|
| 完整性检查 | 缺失值检测准确率100% | 单元测试 |
| 一致性检查 | 一致性检测准确率≥95% | 单元测试 |
| 有效性检查 | 有效性检测准确率100% | 单元测试 |
...
- **Day 1**: 完整性检查、一致性检查、有效性检查
- **Day 2**: 准确性检查、逻辑检查、统计检查
- **Day 3**: 质量评分、报告生成、测试
...
| 错误码 | 错误类型 | 错误描述 | 处理方式 |
|--------|----------|----------|----------|
| ERR_VAL_001 | ValidationError | 校验失败 | 记录日志，返回校验报告 |
| ERR_VAL_002 | RuleLoadError | 规则加载失败 | 使用默认规则 |
| ERR_VAL_003 | DataFormatError | 数据格式错误 | 数据预处理 |
...
- [DataValidator设计文档](../../module_designs/layer_1/L1_VALIDATOR.md)
