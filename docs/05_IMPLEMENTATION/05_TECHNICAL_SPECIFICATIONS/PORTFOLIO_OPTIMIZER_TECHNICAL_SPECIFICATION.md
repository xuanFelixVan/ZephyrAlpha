---
module_id: PORTFOLIO_OPTIMIZER_TECHNICAL_SPECIFICATION
version: 1.0.0
status: Active
...
owner: 首席文档架构师
responsibility:
  - PORTFOLIO_OPTIMIZER_TECHNICAL技术规范
---

﻿---
module_id: IMPL_PORTFOLIO_OPT_TECH_SPEC_001
version: 1.0.1
status: Active
...
> 清风量化系统 v5.3 - PortfolioOptimizer组合优化器模块详细技术设计
...
### 2.3 模块职责与边界定义
- **核心职责**: 组合优化、风险控制、约束处理、结果输出
- **职责边界**: 
  - ✓本模块负责: 组合优化、风险控制、约束处理、结果输出
  - ✗本模块不负责: 交易执行、策略决策、数据获取、风险模型
- **接口契约**: 提供统一的Python API接口

### 2.4 与其他优化器的关系

本模块是**通用组合优化器**，与以下专用优化器形成互补关系：

| 优化器 | 核心定位 | 适用场景 | 主要方法 |
|--------|----------|----------|----------|
| **PORTFOLIO_OPTIMIZER** (本模块) | 通用组合优化 | 单一资产类别、多种优化方法 | 均值方差、风险平价、最大夏普等 |
| **ALL_WEATHER_OPTIMIZER** | 全天候配置优化 | 多资产类别、桥水模式 | 风险平价、Black-Litterman |
| **DAILY_PORTFOLIO_OPTIMIZER** | 日线组合优化 | 文艺复兴模式、Alpha驱动 | 风险模型+Alpha信号优化 |

**选择指南**:
- 单一资产类别（如仅股票）→ 使用 **PORTFOLIO_OPTIMIZER**
- 多资产类别配置（股票+债券+商品）→ 使用 **ALL_WEATHER_OPTIMIZER**
- Alpha信号驱动的统计套利 → 使用 **DAILY_PORTFOLIO_OPTIMIZER**

### 2.5 依赖关系
| 依赖模块 | 依赖类型 | 接口方式 | 版本要求 | 备注 |
|----------|----------|----------|----------|------|
| PyPortfolioOpt | 强依?| Python?| >=1.5.0 | 均值方差优化|
...
| PyPortfolioOpt | >=1.5.0 | 均值方差优化| 成熟稳定 |
