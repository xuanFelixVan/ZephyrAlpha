...
> 清风量化系统 v5.2 - PortfolioOptimizer组合优化器模块详细技术设计
...
- **职责范围**: 组合优化、风险控制、约束处理、结果输出
- **上下层接口**: 
  - 上层依赖: Layer 5 PositionManager (提供持仓信息)
  - 下层依赖: Layer 7 AI报告层 (接收优化结果)
...
|----------|----------|----------|----------|------|
| PyPortfolioOpt | 强依赖 | Python库 | >=1.5.0 | 均值方差优化 |
| Riskfolio-Lib | 强依赖 | Python库 | >=3.0.0 | 风险平价优化 |
| CVXPY | 强依赖 | Python库 | >=1.4.0 | 凸优化求解 |
...
| 缓存类型 | TTL | 淘汰策略 | 最大容量 |
|----------|-----|----------|----------|
| 优化结果缓存 | 1小时 | LRU | 1000条记录 |
| 协方差矩阵缓存 | 1天 | LRU | 365份矩阵 |
...
| R002 | 数值不稳定 | P1 | 使用数值稳定算法 |
| R003 | 约束冲突 | P2 | 实现约束冲突检测 |

### 8.2 约束条件
- **技术约束**: 依赖PyPortfolioOpt、Riskfolio-Lib、CVXPY
...
| 错误码 | 错误类型 | 错误描述 | 处理方式 |
|--------|----------|----------|----------|
| ERR_OPT_001 | OptimizationError | 优化错误 | 记录日志，返回错误 |
| ERR_OPT_002 | ConstraintError | 约束错误 | 记录日志，返回错误 |
| ERR_OPT_003 | NumericalError | 数值错误 | 记录日志，返回错误 |
