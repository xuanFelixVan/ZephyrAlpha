...
**接口契约**: 遵循 INTERFACE_CONTRACT_BLUEPRINT.md 中定义的 `IAlphaFactorFactory` 接口
...
> 本模块专注于因子筛选、合成和Alpha信号生成?> 基础因子计算?FACTOR_CALCULATOR 模块负责?> QlibAlpha158因子?QLIB_ALPHA158 模块提供?
...
class FactorScreener:
    ...
    - IC计算调用FactorIC模块，本模块不重复实?    - ? FACTOR_IC
