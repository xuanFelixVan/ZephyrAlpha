---
module_id: EVENT_DETECTOR_001
version: 1.0.0
status: Active
created_date: 2026-04-02
last_updated: 2026-04-02
owner: 首席技术评审官
standard_type: 专业量化机构技术规格书
applicable_scope: Layer 3 舆情分析层 | 业务架构: 三级时间框架融合架构
compliance_level: 专业标准
parent_document: ../INDEX.md
implementation_status: 进行中
---

# EventDetector事件检测模块技术规格书

> 清风量化系统 v5.2 - EventDetector事件检测模块详细技术设计
> **模块ID**: `EVENT_DETECTOR_001`
> **版本**: v1.0.0
> **状态**: ✅ 正式


## 1. 概述

### 1.1 设计背景与业务目标
- **业务需求**: 系统需要专业的事件检测能力，从新闻文本中识别和分类财经事件
- **技术痛点**: 
  - 事件识别困难：缺乏统一的事件分类体系
  - 事件影响评估不足：难以量化事件对股价的影响
  - 事件关联分析缺失：无法识别事件之间的关联关系
  - 实时性要求高：需要快速识别和响应重要事件
- **预期价值**: 
  - 建立统一的事件分类体系
  - 提供事件影响评估能力
  - 实现事件关联分析
  - 支持实时事件检测和预警

### 1.2 技术定位与架构层归属
- **Layer定位**: Layer 3 - 舆情分析层 (符合ARCHITECTURE.md定义)
- **模块类别**: 核心分析模块
- **架构角色**: Layer 3事件分析组件，为策略执行层提供事件信号

### 1.3 版本信息
| 版本 | 日期 | 作者 | 变更说明 | 状态 |
|------|------|------|----------|------|
| v1.0.0 | 2026-04-02 | 首席技术评审官 | 初始版本 | Active |

---

## 2. 详细架构设计

### 2.1 系统架构图
```
┌─────────────────────────────────────────────────────────────┐
│                    Layer 3: 舆情分析层                       │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  ┌──────────────────────────────────────────────────────┐  │
│  │          EventDetector (主事件检测器)                  │  │
│  │  - 事件分类                                           │  │
│  │  - 事件抽取                                           │  │
│  │  - 影响评估                                           │  │
│  └──────────────────────────────────────────────────────┘  │
│                           ↓                                  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │          事件分析引擎                                 │  │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  │  │
│  │  │EventClass   │  │EventExtract │  │ImpactAssess │  │  │
│  │  └─────────────┘  └─────────────┘  └─────────────┘  │  │
│  └──────────────────────────────────────────────────────┘  │
│                           ↓                                  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │          支撑服务                                     │  │
│  │  - EventPatternLib (事件模式库)                      │  │
│  │  - ImpactModel (影响模型)                            │  │
│  │  - EventMonitor (事件监控)                           │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### 2.2 Layer定位详细说明
- **Layer归属**: Layer 3 - 舆情分析层
- **职责范围**: 负责事件分类、事件抽取、事件影响评估、事件关联分析
- **上下层接口**: 
  - 上层依赖: Layer 5 策略执行层 (提供事件信号)
  - 下层依赖: Layer 3 新闻爬虫、情感分析 (接收新闻数据)

### 2.3 模块职责与边界定义
- **核心职责**: 事件分类、事件抽取、事件影响评估、事件关联分析
- **职责边界**: 
  - ✅ 本模块负责: 事件分类、事件抽取、事件影响评估、事件关联分析
  - ❌ 本模块不负责: 新闻爬取、情感分析、股票匹配
- **接口契约**: 提供统一的Python API接口

### 2.4 依赖关系
| 依赖模块 | 依赖类型 | 接口方式 | 版本要求 | 备注 |
|----------|----------|----------|----------|------|
| pandas | 强依赖 | Python库 | >=1.3.0 | 数据处理核心 |
| numpy | 强依赖 | Python库 | >=1.21.0 | 数值计算 |
| jieba | 强依赖 | Python库 | >=0.42.0 | 中文分词 |
| re | 强依赖 | Python标准库 | - | 正则表达式 |

---

## 3. 接口定义

### 3.1 API接口规范

#### 3.1.1 主接口类
```python
from typing import Dict, List, Optional, Any, Tuple
from datetime import datetime
from dataclasses import dataclass
import pandas as pd
import re


@dataclass
class EventConfig:
    """事件检测配置"""
    event_types: List[str]
    detection_threshold: float
    impact_threshold: float
    enable_realtime: bool


@dataclass
class Event:
    """事件数据"""
    event_id: str
    event_type: str
    event_level: str
    title: str
    content: str
    keywords: List[str]
    mentioned_stocks: List[str]
    impact_score: float
    publish_time: datetime


class EventDetector:
    """事件检测主类"""
    
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        """初始化事件检测器"""
        pass
    
    def detect_event(
        self,
        news_text: str
    ) -> Optional[Event]:
        """检测事件"""
        pass
    
    def classify_event(
        self,
        news_text: str
    ) -> str:
        """分类事件"""
        pass
    
    def extract_event_info(
        self,
        news_text: str,
        event_type: str
    ) -> Dict[str, Any]:
        """抽取事件信息"""
        pass
    
    def assess_event_impact(
        self,
        event: Event,
        stock_code: Optional[str] = None
    ) -> float:
        """评估事件影响"""
        pass
    
    def analyze_event_correlation(
        self,
        events: List[Event]
    ) -> Dict[str, List[str]]:
        """分析事件关联"""
        pass
    
    def batch_detect(
        self,
        news_list: List[str]
    ) -> List[Event]:
        """批量检测"""
        pass
    
    def monitor_events(
        self
    ) -> Dict[str, Any]:
        """监控事件状态"""
        pass


class EventClassifier:
    """事件分类器"""
    
    EVENT_PATTERNS = {
        '财报发布': {
            'keywords': ['财报', '业绩', '净利润', '营收', '每股收益'],
            'pattern': r'(财报|业绩|利润).*(发布|公告|显示)',
            'sentiment_impact': 'positive'
        },
        '分红送转': {
            'keywords': ['分红', '送股', '转增', '派息'],
            'pattern': r'(\d+)派(\d+)|(\d+)送(\d+)',
            'sentiment_impact': 'positive'
        },
        '并购重组': {
            'keywords': ['并购', '重组', '收购', '定增'],
            'pattern': r'(并购|重组|收购).*(完成|通过|公告)',
            'sentiment_impact': 'uncertain'
        },
        '政策利好': {
            'keywords': ['政策', '支持', '鼓励', '补贴', '规划'],
            'pattern': r'(支持|鼓励|补贴|政策).*(行业|产业|公司)',
            'sentiment_impact': 'positive'
        },
        '监管函': {
            'keywords': ['监管函', '问询函', '警示函', '处罚'],
            'pattern': r'(监管|问询|警示).*函',
            'sentiment_impact': 'negative'
        },
        '减持': {
            'keywords': ['减持', '卖出', '转让'],
            'pattern': r'(股东|高管|实际控制人).*(减持|卖出)',
            'sentiment_impact': 'negative'
        }
    }
    
    def classify_event(self, news: str) -> str:
        """识别事件类型"""
        text = news
        
        for event_type, config in self.EVENT_PATTERNS.items():
            keywords = config['keywords']
            pattern = config['pattern']
            
            if any(kw in text for kw in keywords):
                if re.search(pattern, text):
                    return event_type
        
        return '其他'
```

### 3.2 性能指标要求
| 性能指标 | 目标值 | 测量方法 |
|----------|--------|----------|
| 单条检测时间 | < 100ms | 单条新闻检测 |
| 批量检测时间 | < 5秒 | 100条新闻检测 |
| 分类准确率 | ≥ 85% | 100条新闻分类 |
| 影响评估准确率 | ≥ 80% | 100条事件评估 |
| 监控响应时间 | < 1秒 | 事件状态查询 |

### 3.3 安全机制
- **数据安全**: 事件数据不包含敏感信息
- **访问控制**: 事件检测接口需要认证
- **日志审计**: 记录所有事件检测操作

---

## 4. 数据模型与存储

### 4.1 核心数据结构

#### 4.1.1 事件数据模型
```python
@dataclass
class EventData:
    """事件数据模型"""
    event_id: str
    event_type: str
    event_level: str
    title: str
    content: str
    keywords: List[str]
    mentioned_stocks: List[str]
    mentioned_amount: float
    impact_score: float
    publish_time: datetime
    processed_time: datetime
```

#### 4.1.2 事件分类模型
```python
@dataclass
class EventCategory:
    """事件分类模型"""
    category_id: str
    category_name: str
    parent_category: str
    keywords: List[str]
    patterns: List[str]
    sentiment_impact: str
```

#### 4.1.3 事件影响模型
```python
@dataclass
class EventImpact:
    """事件影响模型"""
    event_id: str
    stock_code: str
    impact_score: float
    impact_direction: str
    confidence: float
    impact_duration: int
```

### 4.2 缓存策略
| 缓存类型 | TTL | 淘汰策略 | 最大容量 |
|----------|-----|----------|----------|
| 事件数据缓存 | 24小时 | LRU | 10000条 |
| 分类结果缓存 | 7天 | LRU | 5000条 |
| 影响评估缓存 | 1小时 | LRU | 1000条 |

### 4.3 数据持久化
- **持久化需求**: 事件数据、分类结果需要持久化存储
- **存储格式**: JSON或Parquet格式

---

## 5. 算法实现说明

### 5.1 核心算法

#### 5.1.1 事件分类算法
```python
def classify_event(
    self, 
    news_text: str
) -> str:
    """
    事件分类算法
    
    算法原理:
    1. 关键词匹配
    2. 正则表达式匹配
    3. 返回事件类型
    
    复杂度: O(n) n为关键词数量
    """
    for event_type, config in self.EVENT_PATTERNS.items():
        keywords = config['keywords']
        pattern = config['pattern']
        
        if any(kw in news_text for kw in keywords):
            if re.search(pattern, news_text):
                return event_type
    
    return '其他'
```

#### 5.1.2 事件抽取算法
```python
def extract_event_info(
    self, 
    news_text: str, 
    event_type: str
) -> Dict[str, Any]:
    """
    事件抽取算法
    
    算法原理:
    1. 识别事件类型
    2. 提取关键信息
    3. 结构化输出
    
    复杂度: O(n) n为文本长度
    """
    event_info = {
        'event_type': event_type,
        'keywords': [],
        'mentioned_stocks': [],
        'mentioned_amount': 0.0
    }
    
    config = self.EVENT_PATTERNS.get(event_type, {})
    keywords = config.get('keywords', [])
    
    for kw in keywords:
        if kw in news_text:
            event_info['keywords'].append(kw)
    
    stock_pattern = r'([0-9]{6}\.[A-Z]{2}|[A-Z]{2}[0-9]{6})'
    stocks = re.findall(stock_pattern, news_text)
    event_info['mentioned_stocks'] = stocks
    
    amount_pattern = r'(\d+\.?\d*)\s*(亿|万|千万)'
    amounts = re.findall(amount_pattern, news_text)
    if amounts:
        event_info['mentioned_amount'] = float(amounts[0][0])
    
    return event_info
```

#### 5.1.3 影响评估算法
```python
def assess_event_impact(
    self, 
    event: Event, 
    stock_code: Optional[str] = None
) -> float:
    """
    影响评估算法
    
    算法原理:
    1. 基于事件类型评估
    2. 基于关键词强度评估
    3. 基于历史数据评估
    
    复杂度: O(1)
    """
    config = self.EVENT_PATTERNS.get(event.event_type, {})
    sentiment_impact = config.get('sentiment_impact', 'neutral')
    
    base_score = {
        'positive': 0.7,
        'negative': -0.7,
        'uncertain': 0.0,
        'neutral': 0.0
    }.get(sentiment_impact, 0.0)
    
    keyword_boost = len(event.keywords) * 0.05
    
    impact_score = base_score + keyword_boost
    
    return max(-1.0, min(1.0, impact_score))
```

---

## 6. 实施技术栈

### 6.1 语言与框架
| 技术选型 | 版本要求 | 用途 | 选择理由 |
|----------|----------|------|----------|
| Python | >=3.8 | 主要开发语言 | 量化系统标准语言 |
| pandas | >=1.3.0 | 数据处理 | 数据分析标准库 |
| numpy | >=1.21.0 | 数值计算 | 科学计算基础库 |
| jieba | >=0.42.0 | 中文分词 | 中文NLP基础库 |
| re | - | 正则表达式 | Python标准库 |

### 6.2 第三方依赖
```yaml
requirements:
  - pandas>=1.3.0
  - numpy>=1.21.0
  - jieba>=0.42.0
```

---

## 7. 测试策略

### 7.1 单元测试
| 测试项 | 测试内容 | 覆盖率目标 |
|--------|----------|------------|
| 事件分类 | 分类正确性 | 100% |
| 事件抽取 | 抽取正确性 | 100% |
| 影响评估 | 评估正确性 | 100% |
| 事件监控 | 监控正确性 | 100% |

### 7.2 集成测试
```python
def test_event_detector_integration():
    """集成测试示例"""
    detector = EventDetector()
    
    news = "某公司发布2025年财报，净利润同比增长50%"
    event = detector.detect_event(news)
    
    assert event is not None
    assert event.event_type == "财报发布"
    assert len(event.keywords) > 0
```

---

## 8. 风险与约束

### 8.1 技术风险
| 风险ID | 风险描述 | 风险等级 | 缓解措施 |
|--------|----------|----------|----------|
| R001 | 分类准确率不足 | P1 | 多种分类方法交叉验证 |
| R002 | 影响评估偏差 | P1 | 历史数据验证、模型优化 |
| R003 | 实时性不足 | P2 | 性能优化、并行处理 |
| R004 | 新事件类型识别 | P2 | 动态扩展事件模式库 |

### 8.2 约束条件
- **技术约束**: 依赖jieba分词、re正则表达式
- **资源约束**: 内存使用<1GB（批量检测）
- **时间约束**: 预计开发时间15小时
- **质量约束**: 分类准确率≥85%

---

## 9. 验收标准

### 9.1 功能验收标准
| 功能项 | 验收标准 | 验证方法 |
|--------|----------|----------|
| 事件分类 | 分类正确 | 单元测试 |
| 事件抽取 | 抽取正确 | 单元测试 |
| 影响评估 | 评估正确 | 单元测试 |
| 事件监控 | 监控正确 | 单元测试 |

### 9.2 性能验收标准
| 性能指标 | 验收标准 | 验证方法 |
|----------|----------|----------|
| 单条检测时间 | < 100ms | 性能测试 |
| 分类准确率 | ≥ 85% | 性能测试 |

### 9.3 质量验收标准
| 质量指标 | 验收标准 | 验证方法 |
|----------|----------|----------|
| 分类准确率 | ≥ 85% | 质量检查 |
| 测试覆盖率 | ≥ 90% | pytest-cov |

---

## 10. 实施路线图

### 10.1 Phase 1: 核心功能开发 (2天)
- **Day 1**: 事件分类、事件抽取
- **Day 2**: 影响评估、测试、优化

---

## 附录

### A. 配置示例
```yaml
event_detector:
  event_types:
    - "财报发布"
    - "分红送转"
    - "并购重组"
    - "政策利好"
    - "监管函"
    - "减持"
  
  detection:
    threshold: 0.5
    enable_realtime: true
  
  impact:
    threshold: 0.3
    duration_days: 5
```

### B. 错误码定义
| 错误码 | 错误类型 | 错误描述 | 处理方式 |
|--------|----------|----------|----------|
| ERR_EVENT_001 | ClassifyError | 分类失败 | 记录日志，返回错误 |
| ERR_EVENT_002 | ExtractError | 抽取失败 | 记录日志，返回错误 |
| ERR_EVENT_003 | ImpactError | 评估失败 | 记录日志，返回错误 |
| ERR_EVENT_004 | MonitorError | 监控失败 | 记录日志，返回错误 |

### C. 参考文档
- [架构定义](../../01_FRAMEWORK/ARCHITECTURE.md)
- [模块职责边界](../../01_FRAMEWORK/MODULE_RESPONSIBILITY_BOUNDARIES.md)
- [事件总线设计](../../04_EXECUTION/01_EVENT_ENGINE/EVENT_BUS.md)


**文档版本**: v1.0.0 | **创建日期**: 2026-04-02 | **维护者**: 舆情分析层负责人
