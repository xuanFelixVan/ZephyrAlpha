...
| **上游依赖** | QMT数据接口, iFind连接器, SuperCommand |
...
│  │  ├── IndexDataCollector (指数数据采集)                      │  │
│  │  ├── SectorDataCollector (板块数据采集)                     │  │
│  │  └── SentimentDataCollector (情绪数据采集)                  │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                              ↓                                      │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │                    数据处理层                                │  │
│  │  ├── MarketIndicatorCalculator (市场指标计算)               │  │
│  │  ├── SectorRotationAnalyzer (板块轮动分析)                  │  │
│  │  ├── SentimentAnalyzer (情绪分析)                           │  │
│  │  └── StyleIdentifier (风格识别)                             │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                              ↓                                      │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │                    AI解读层                                  │  │
│  │  ├── MarketInterpreter (市场解读)                           │  │
│  │  ├── TrendForecaster (趋势预测)                             │  │
│  │  └── RiskAlerter (风险预警)                                 │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                              ↓                                      │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │                    输出层                                    │  │
│  │  ├── MarketReportGenerator (市场报告生成)                   │  │
│  │  ├── AlertSender (预警发送)                                 │  │
│  │  └── DataExporter (数据导出)                                │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                                                                     │
...
| 维度 | 定义 |
|------|------|
| **Layer定位** | Layer 7: AI报告层 - 市场分析与解读 |
| **核心职责** | 市场行情分析、板块轮动分析、市场情绪分析、AI市场解读 |
| **职责边界** | 不负责实时交易（Layer 5）、不负责风险控制（贯穿各层）、不负责数据采集（Layer 0） |
| **数据流向** | 上游数据源 → MarketAnalyzer → 下游模块/用户 |
...
- iFind连接器 (iFindConnector): 提供板块数据、因子数据
- SuperCommand (SuperCommand): 提供实时行情推送
- Baostock (Baostock): 提供历史行情数据

下游服务:
- DailyReporter (日报生成器): 提供市场分析数据
- MonthlyReporter (月报生成器): 提供月度市场分析
- StreamlitDashboard (可视化仪表板): 展示市场分析
- NotificationSystem (通知系统): 发送市场预警
```
...
沪深300指数收盘 **4,234.56**，上涨 **+0.89%**。
成交量 **1234.56亿**，成交额 **2345.67亿**。
...
| 板块 | 涨跌幅 | 成交额 | 领涨股 |
|------|--------|--------|--------|
| 科技 | +2.34% | 456.78亿 | XXX科技 |
| 金融 | +1.23% | 345.67亿 | YYY银行 |
| 消费 | -0.45% | 234.56亿 | ZZZ食品 |

### 2.2 板块轮动

当前强势板块：**科技、金融**
轮动信号：**科技板块持续强势，消费板块出现回调**
...
| 指标 | 目标值 | 说明 |
|------|--------|------|
| **分析生成时间** | ≤10秒 | 从数据采集到分析完成 |
| **数据采集时间** | ≤5秒 | 从上游模块获取所有数据 |
| **AI解读时间** | ≤10秒 | LLM生成市场解读 |
| **并发支持** | ≥20 | 同时分析请求数量 |
...
-- 板块轮动记录表
CREATE TABLE sector_rotation_record (
...
-- 市场预警配置表
CREATE TABLE market_alert_config (
...
│  输出                                                                │
│  ├── DailyReporter (日报数据)                                       │
│  ├── MonthlyReporter (月报数据)                                     │
│  ├── StreamlitDashboard (可视化展示)                                │
│  └── NotificationSystem (预警推送)                                  │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
## 市场情绪
- 涨跌比: {market_sentiment.advance_decline_ratio:.2f}
...
| 参数 | 默认值 | 调优范围 | 说明 |
|------|--------|----------|------|
| **max_tokens** | 2000 | 1000-3000 | LLM最大生成token数 |
| **temperature** | 0.7 | 0.5-0.9 | LLM生成温度 |
| **lookback_days** | 20 | 10-60 | 板块轮动回溯天数 |
| **sentiment_threshold** | 0.7 | 0.6-0.8 | 强势板块识别阈值 |
...
| 测试场景 | 性能目标 | 测试方法 |
|----------|----------|----------|
| **市场分析** | ≤10秒 | 分析100次，计算平均时间 |
| **并发分析** | ≥20并发 | 同时分析20次，验证成功率 |
...
| **R003** | 分析结果不准确 | P2 | 算法验证、人工审核 |
| **R004** | 市场风格识别错误 | P2 | 多维度验证、人工确认 |
...
| 任务 | 工时 | 交付物 |
|------|------|--------|
| 数据采集模块开发 | 1天 | MarketDataCollector |
| 市场指标计算模块开发 | 1天 | MarketIndicatorCalculator |
| 板块轮动分析模块开发 | 1天 | SectorRotationAnalyzer |
| AI解读模块开发 | 1天 | MarketInterpreter |
...
2. [DAILY_REPORTER_TECHNICAL_SPECIFICATION.md](./DAILY_REPORTER_TECHNICAL_SPECIFICATION.md) - 日报生成器技术规格
3. [MONTHLY_REPORTER_TECHNICAL_SPECIFICATION.md](./MONTHLY_REPORTER_TECHNICAL_SPECIFICATION.md) - 月报生成器技术规格
