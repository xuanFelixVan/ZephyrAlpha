---
module_id: ECONOMIC_REGIME_REPORTER_TECHNICAL_SPECIFICATION
version: 1.0.0
status: Active
...
**职责说明**: 本模块负责经济范式分析报告生成，调用经济范式判断引擎（Layer 5）提供的接口获取经济范式判断结果，然后生成分析报告。
...
1. **经济范式报告生成**: 调用经济范式判断引擎接口，生成经济范式分析报告





2. **宏观因子暴露计算**: 计算组合对宏观因子的风险暴露





3. **战略配置建议生成**: 基于经济范式提供资产配置建议





4. **范式转换风险报告**: 生成范式转换风险评估报告
...
- ✅ 本模块负责: 报告生成、因子暴露计算、配置建议生成





- ❌ 本模块不负责: 经济范式判断（由 Layer 5 经济范式判断引擎负责）





- ❌ 本模块不负责: 范式转换预警（由 Layer 5 经济范式判断引擎负责）
...
- 上游依赖: Layer 5 经济范式判断引擎（ECONOMIC_REGIME_ENGINE_BLUEPRINT.md）
