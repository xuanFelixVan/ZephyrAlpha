...
|----------|----------|----------|-------|
| FactorCalculator | calculate_factors | 价值、成长、动量、技术指标 | Layer 2 |
...
class IModelTrainer(ABC):
    ...
    实现模块:
    - LSTMTrainer: LSTM模型训练器
    - TransformerTrainer: Transformer模型训练器
    
    调用方:
    - ModelTrainingPipeline: 通用训练流水线
    """
...
| 实现模块 | 接口方法 | 服务类型 | 职责 |
|----------|----------|----------|------|
| FeatureStore | get_online_features() | 在线服务 | 实时特征检索 |
| FeatureStore | get_historical_features() | 离线服务 | 批量特征检索 |

### 4.3 与FeatureEngineering的协作
...
| 版本 | 日期 | 变更内容 | 影响模块 |
|------|------|----------|----------|
| v1.0 | 2026-04-03 | 初始版本 | 因子计算、模型训练、特征服务 |
