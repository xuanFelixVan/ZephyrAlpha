...
> **版本**: v2.0
> **创建日期**: 2026-04-03
> **更新内容**: 术语标准化、资金流向监控接口、智能体参数配置、数据获取模块
> **依据文档**: MARKET_PARTICIPANT_BEHAVIOR_RESEARCH_SUPPLEMENT.md
...
| 原术语 | 新术语（专业标准） | 英文术语 | 替换范围 |
|--------|-------------------|---------|---------|
| 主力 | **机构资金** | Institutional Capital | 全文档 |
| 主力资金 | **机构资金** | Institutional Capital | 全文档 |
| 主力/游资智能体 | **机构/热钱智能体** | Institutional/Hot Money Agent | 智能体名称 |
| 主力控盘 | **机构控盘** | Institutional Control | 行为描述 |
| 主力行为 | **机构资金行为** | Institutional Capital Behavior | 研究领域 |
| 国家队 | **主权基金** | Sovereign Funds | 学术场景 |
| 游资 | **热钱** | Hot Money | 风险监控场景 |
| 散户 | **零售投资者** | Retail Investors | 学术场景 |
...
│   ├── 主权基金智能体（Sovereign Fund Agent）
│   │   └── 代号：AGENT.SOVEREIGN_FUND.001
│   ├── 公募基金智能体（Mutual Fund Agent）
│   │   └── 代号：AGENT.MUTUAL_FUND.001
│   ├── 私募基金智能体（Private Equity Agent）
│   │   ├── 量化基金智能体（Quantitative Fund Agent）
│   │   │   └── 代号：AGENT.QUANT_FUND.001
│   │   └── 主观私募智能体（Discretionary PE Agent）
│   │       └── 代号：AGENT.DISCRETIONARY_PE.001
│   ├── 外资智能体（Foreign Capital Agent）
│   │   └── 代号：AGENT.FOREIGN_CAPITAL.001
│   ├── 保险资金智能体（Insurance Fund Agent）
│   │   └── 代号：AGENT.INSURANCE_FUND.001
│   └── 产业资本智能体（Industrial Capital Agent）
│       └── 代号：AGENT.INDUSTRIAL_CAPITAL.001
└── 零售投资者（Retail Investors）
    ├── 高净值个人智能体（High Net Worth Agent）
    │   └── 代号：AGENT.HNW.001
    └── 散户智能体（Retail Investor Agent）
        └── 代号：AGENT.RETAIL.001
```
...
class DDXIndicator:
    ...
    stock_code: str
    timestamp: datetime
    ddx_value: float  # DDX值
    ddx_ma5: float  # 5日均值
    ddx_ma10: float  # 10日均值
    ddx_consecutive_days: int  # 连续翻红/翻绿天数
...
class DDEIndicator:
    ...
    stock_code: str
    timestamp: datetime
    large_order_net_ratio: float  # 大单净量（占流通盘比率）
    retail_participation: float  # 散户数量（参与度）
    large_order_amount: float  # 大单金额（万元）
...
class BBDIndicator:
    ...
    stock_code: str
    timestamp: datetime
    bbd_value: float  # BBD值（万元）
    super_large_inflow: float  # 特大单流入（万元）
    super_large_outflow: float  # 特大单流出（万元）
    total_amount: float  # 总成交额（万元）
    cannibalization_rate: float  # 通吃率 = BBD / 成交额 × 100
    confidence: float
...
class CapitalFlowDataFetcher(ABC):
    ...
    def fetch_dde(self,
        ...
        数据源映射:
            iFinD函数: THS_DDE
            字段映射:
                - large_order_net_ratio: 大单净量
                - retail_participation: 散户数量
    ...
    def fetch_bbd(self,
        ...
        数据源映射:
            ...
            字段映射:
                - bbd_value: BBD值
                - super_large_inflow: 特大单流入
                - super_large_outflow: 特大单流出
        """
        pass
...
class IFindCapitalFlowFetcher(CapitalFlowDataFetcher):
            ...
            try:
                ...
                for _, row in df.iterrows():
                    indicator = DDEIndicator(
                        stock_code=stock_code,
                        timestamp=row['time'],
                        large_order_net_ratio=row['大单净量'],
            ...
            try:
                ...
                for _, row in df.iterrows():
                    ...
                    indicator = BBDIndicator(
                        ...
                        timestamp=row['time'],
                        bbd_value=row['BBD'],
                        super_large_inflow=row['特大单流入'],
                        super_large_outflow=row['特大单流出'],
    ...
    def fetch_realtime_capital_flow(self,
        ...
        iFinD调用示例:
            ths.HQ_query(stock_codes, '最新价,涨跌幅,DDX,DDE,BBD')
        """
        result = {}
        ...
        try:
            df = self.ifs_client.HQ_query(
                stock_codes,
                '最新价,涨跌幅,DDX,DDE,BBD,超大单净流入,大单净流入'
            )
...
class DragonTigerListItem:
    ...
    hot_money_flag: bool  # 是否有知名游资
    institutional_flag: bool  # 是否有机构专用席位

class DragonTigerDataParser(ABC):
    """龙虎榜数据解析器抽象基类
    
    索引: INTERFACE.DRAGON_TIGER.001
    职责: 解析龙虎榜数据，识别机构席位和游资席位
    数据源: 同花顺iFinD、交易所公开数据
    """
    ...
    def fetch_dragon_tiger_list(self,
        ...
        参数:
            start_date: 开始日期
            end_date: 结束日期
            reason_filter: 上榜原因过滤（如 ['涨停', '跌停', '换手率异常']）
            
        返回:
            List[DragonTigerListItem]: 龙虎榜数据列表
    ...
    def identify_hot_money_seats(self,
        ...
        识别规则:
            1. 知名游资营业部名单（如：华鑫上海分公司、财通杭州体育场路等）
            2. 历史操作风格匹配
        """
        pass
...
class IFindDragonTigerParser(DragonTigerDataParser):
    ...
    def __init__(self, config: Dict):
        ...
        self.known_hot_money_seats = {
            '华鑫证券有限责任公司上海分公司': '知名游资',
            '财通证券股份有限公司杭州体育场路证券营业部': '知名游资',
            '国泰君安证券股份有限公司上海分公司': '知名游资',
            '中国中金财富证券有限公司北京分公司': '知名游资',
            '华泰证券股份有限公司深圳分公司': '知名游资',
        }
    ...
    def fetch_dragon_tiger_list(self,
        ...
        iFinD调用示例:
            ths.ED_query(
                'ths龙虎榜',
                '',
                '证券代码,证券简称,交易日期,收盘价,涨跌幅,换手率,上榜原因,买入席位,卖出席位',
                start_date,
                end_date
            )
        """
...
class NorthboundCapitalFlow:
    ...
    shanghai_connect_balance: float  # 沪股通余额（亿元）
    shenzhen_connect_balance: float  # 深股通余额（亿元）
    
    top_buy_stocks: List[Dict]  # 买入前10股票
    top_sell_stocks: List[Dict]  # 卖出前10股票
...
class NorthboundHolding:
    ...
    hold_value: float  # 持股市值（万元）
    hold_ratio: float  # 持股占比（%）
    change_amount: float  # 持股变化（万股）
...
class NorthboundCapitalMonitor(ABC):
    ...
    def detect_smart_money_signal(self,
        ...
        信号规则:
            1. 连续5日净买入超60亿元：新能源、电子板块胜率80%
            2. 单日净流出超80亿元：高估值消费股平均回撤5.3%
        """
        pass
```
    ...
    def fetch_daily_flow(self,
        ...
        iFinD调用示例:
            ths.ED_query(
                'ths北向资金',
                '',
                '交易日期,北向资金净买入,沪股通净买入,深股通净买入,沪股通余额,深股通余额',
                start_date,
                end_date
            )
        """
        ...
        try:
            ...
            for _, row in df.iterrows():
                flow = NorthboundCapitalFlow(
                    ...
                    shanghai_connect_balance=row['沪股通余额'] / 10000,
                    shenzhen_connect_balance=row['深股通余额'] / 10000,
    ...
    def fetch_holdings(self,
        ...
        iFinD调用示例:
            ths.ED_query(
                'ths北向持股',
                '',
                '证券代码,证券简称,持股数量,持股市值,持股占比,持股变化,持股变化比例',
                '',
                ''
            )
        """
    ...
    def detect_smart_money_signal(self,
        ...
        if recent_flow.total_net_buy < -80:
            return {
                ...
                'reasoning': f'北向资金单日净流出超80亿元，历史数据显示高估值消费股平均回撤5.3%'
            }
        
        return {
            'signal_type': 'NEUTRAL',
            'confidence': 0.50,
            'reasoning': '北向资金流向未达到显著信号阈值'
        }
...
mutual_fund_agent:
  ...
  sector_focus:
    ai_computing:
      ...
      target_stocks: ["浪潮信息", "中科曙光", "寒武纪"]
    
    medical_tech:
      weight: 0.25
      keywords: ["医疗新科技", "创新药", "医疗器械"]
      target_stocks: ["恒瑞医药", "迈瑞医疗", "药明康德"]
    
    humanoid_robot:
      weight: 0.20
      keywords: ["人形机器人", "伺服电机", "减速器"]
      target_stocks: ["三花智控", "汇川技术", "绿的谐波"]
    
    new_energy:
      weight: 0.20
      keywords: ["新能源", "光伏", "储能"]
      target_stocks: ["宁德时代", "隆基绿能", "比亚迪"]
...
class MarketParticipantDataBundle:
    ...
    dragon_tiger_data: List  # 龙虎榜数据
    northbound_flow_data: Dict  # 北向资金流向
...
class MarketParticipantDataFetcher:
    ...
    def fetch_all_data(self,
        ...
        返回:
            MarketParticipantDataBundle: 整合数据包
        """
    ...
    def fetch_realtime_data(self,
        ...
        返回:
            MarketParticipantDataBundle: 实时数据包
        """
...
| 版本 | 日期 | 更新内容 | 作者 |
|------|------|----------|------|
| v2.0 | 2026-04-03 | 术语标准化、资金流向监控接口、智能体参数配置、数据获取模块 | Spec-Approver |

---

**版本**: v2.0 | **更新**: 2026-04-03 | **状态**: ✅ 已完成
