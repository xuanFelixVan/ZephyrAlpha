...
> 清风量化系统 v5.2 - SignalGenerator信号生成器模块详细技术设计
...
### 1.1 设计背景与业务目标
- **业务需求**: 系统需要将策略产生的因子信号转化为可执行的交易信号
- **技术痛点**: 
  - 信号质量不稳定：因子信号缺乏标准化和验证
  - 信号过滤缺失：缺乏有效的信号过滤和确认机制
...
- **职责范围**: 信号生成、信号过滤、信号确认、信号合成
- **上下层接口**: 
  - 上层依赖: Layer 2 Alpha因子层 (提供因子信号)
  - 下层依赖: Layer 6 组合优化层 (接收交易信号)

### 2.3 模块职责与边界定义
- **核心职责**: 将因子信号转化为可执行的交易信号
- **职责边界**: 
  - ✅ 本模块负责: 信号生成、过滤、确认、合成、衰减
  - ❌ 本模块不负责: 因子计算、策略执行、交易执行、风险控制
...
| 缓存类型 | TTL | 淘汰策略 | 最大容量 |
|----------|-----|----------|----------|
| 信号历史缓存 | 1天 | LRU | 1000只股票 |
| 因子信号缓存 | 1小时 | LRU | 50个因子 |
| 市场数据缓存 | 5分钟 | LRU | 100只股票 |

### 4.3 数据持久化
- **持久化需求**: 信号历史、质量指标需要持久化存储
- **存储格式**: Parquet文件 + SQLite数据库
...
def calculate(
    ...
    算法原理:
    1. 选择标准化方法 (zscore, minmax, rank)
    2. 对因子信号进行标准化
    3. 将信号值限制在[-1, 1]区间
    
    复杂度: O(n*m) n为日期数，m为股票数
    """
    if method == "zscore":
        normalized = (factor_signals - factor_signals.mean()) / factor_signals.std()
        return normalized.clip(-1, 1)
...
| pandas | >=1.3.0 | 数据处理 | 强大的数据处理能力 |
| numpy | >=1.21.0 | 数值计算 | 高性能数值计算 |
| scipy | >=1.7.0 | 统计分析 | 丰富的统计函数 |
...
| 风险ID | 风险描述 | 风险等级 | 缓解措施 |
|--------|----------|----------|----------|
| R001 | 信号质量不稳定 | P1 | 实现信号质量监控和告警 |
| R002 | 信号过滤过度 | P2 | 实现过滤参数动态调整 |
| R003 | 信号衰减过快 | P2 | 实现衰减参数优化 |
| R004 | 多因子信号冲突 | P2 | 实现信号合成优化 |

### 8.2 约束条件
- **技术约束**: 依赖pandas、numpy、scipy库
...
| 错误码 | 错误类型 | 错误描述 | 处理方式 |
|--------|----------|----------|----------|
| ERR_SIGNAL_001 | CalculateError | 信号计算失败 | 记录日志，返回错误 |
| ERR_SIGNAL_002 | FilterError | 信号过滤失败 | 记录日志，返回错误 |
| ERR_SIGNAL_003 | ConfirmError | 信号确认失败 | 记录日志，返回错误 |
| ERR_SIGNAL_004 | SynthesisError | 信号合成失败 | 记录日志，返回错误 |
