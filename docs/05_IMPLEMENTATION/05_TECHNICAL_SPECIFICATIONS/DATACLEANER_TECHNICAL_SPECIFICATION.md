...
> 清风量化系统 v5.2 - DataCleaner数据清洗器模块详细技术设�?
> **模块ID**: `PREP_CLEAN_001`
> **版本**: v1.0.0
> **状�?*: �?正式
...
def detect_outliers(
    ...
    **kwargs
    ...
    算法原理:
    1. iqr: 四分位距法，识别超出Q1-1.5*IQR和Q3+1.5*IQR的�?
    2. zscore: Z分数法，识别|Z|>threshold的�?
    3. mad: 中位数绝对偏差法，识别超出中位数±threshold*MAD的�?
...
| 错误�?| 错误类型 | 错误描述 | 处理方式 |
|--------|----------|----------|----------|
| ERR_CLEAN_001 | CleaningError | 清洗失败 | 记录日志，返回原始数�?|
| ERR_CLEAN_002 | MissingValueError | 缺失值处理失�?| 使用默认策略 |
| ERR_CLEAN_003 | OutlierDetectionError | 异常值检测失�?| 跳过异常值检�?|
