...
**文档版本**: v1.1  
**创建日期**: 2026-04-03  
**更新内容**: 新增外资智能体和保险资金智能体  
**参考文档**: MARKET_PARTICIPANT_SIMULATION_SPEC.md v1.0
...
### 2.1 外资智能体 (Foreign Investor Agent) ⭐ 新增
...
class ForeignInvestorConfig:
    ...
    agent_name: str = "外资智能体"
    market_share: float = 0.0376
    
    # 投资风格
    investment_style: str = "value_investing"  # 价值投资
    holding_period: str = "long_term"  # 长期持有
    risk_appetite: str = "moderate"  # 中等风险偏好
    
    # 核心资产偏好
    preferred_sectors: List[str] = ['消费', '科技', '金融', '医疗健康', '新能源']
    min_market_cap: float = 50e9  # 最小市值500亿
    min_dividend_yield: float = 0.02  # 最小股息率2%
    max_pe_ratio: float = 30  # 最大PE 30倍
    
    # 汇率敏感度
    exchange_rate_sensitivity: float = 0.7
...
**1. GlobalMacroAnalyzer (全球宏观经济分析器)**
...
**核心数据源**: 同花顺iFind (5700+因子)

**数据需求映射**:

| 数据需求 | iFind数据接口 | 因子ID/指标代码 | 补充数据源 |
|---------|-------------|---------------|-----------|
| 全球宏观经济 | ⚠️ 需补充 | - | FRED API (GS10, FEDFUNDS) |
| 美债收益率 | ⚠️ 需补充 | - | FRED API (GS10) |
| 美联储政策 | ⚠️ 需补充 | - | FRED API (FEDFUNDS) |
| 美元指数 | ⚠️ 需补充 | - | Yahoo Finance (DXY) |
| VIX恐慌指数 | ⚠️ 需补充 | - | Yahoo Finance (^VIX) |
| 人民币汇率 | ✅ get_macro_data | USDCNY | iFind原生支持 |
| 离岸人民币 | ✅ get_macro_data | USDCNH | iFind原生支持 |
| 北向资金流入 | ✅ get_market_data | northbound_flow | iFind原生支持 |
| A股财务数据 | ✅ get_financial_statements | income_statement/balance_sheet/cash_flow | iFind原生支持 |
| 估值因子 | ✅ get_factor_data | PE_TTM, PB_LF, PS_TTM | iFind原生支持 |
| 质量因子 | ✅ get_factor_data | ROE_TTM, ROA_TTM, FCF_TTM | iFind原生支持 |

**补充数据源 (免费)**:
- FRED API: https://fred.stlouisfed.org/docs/api/fred/
- Yahoo Finance: yfinance Python库
...
详见: [外资智能体完整实现代码](#任务2设计外资智能体完整实现)

---

### 2.2 保险资金智能体 (Insurance Fund Agent) ⭐ 新增
...
class InsuranceFundConfig:
    ...
    agent_name: str = "保险资金智能体"
    market_share: float = 0.0377
    
    # 投资风格
    investment_style: str = "income_investing"  # 收益投资
    holding_period: str = "very_long_term"  # 超长期持有
    risk_appetite: str = "conservative"  # 保守风险偏好
    
    # 负债特性
    liability_duration: float = 15.0  # 负债久期15年
    asset_liability_matching: bool = True  # 资产负债匹配
    
    # 高股息偏好
    min_dividend_yield: float = 0.04  # 最小股息率4%
    min_dividend_growth_rate: float = 0.05  # 最小股息增长率5%
    max_payout_ratio: float = 0.7  # 最大分红比例70%
    
    # 行业偏好
    preferred_sectors: List[str] = ['银行', '公用事业', '交通运输', '消费', '房地产']
    min_market_cap: float = 100e9  # 最小市值1000亿
    max_pe_ratio: float = 15  # 最大PE 15倍
    max_pb_ratio: float = 1.5  # 最大PB 1.5倍
    
    # 信用评级要求
    min_credit_rating: str = "AA"  # 最低信用评级AA
    
    # 流动性要求
    min_daily_volume: float = 100e6  # 最小日均成交额1亿
    max_position_concentration: float = 0.10  # 最大持仓集中度10%
```

#### 2.2.2 行为模式

**核心行为特征**:
- **典型的"长钱"**: 负债久期长，持仓周期以年为单位
- **高股息偏好**: 银行、公用事业、消费龙头
- **低换手率**: 极少调仓，是市场的稳定力量
- **绝对收益导向**: 追求稳定的绝对收益
- **资产负债匹配**: 久期匹配管理
...
| 数据需求 | iFind数据接口 | 因子ID/指标代码 | 备注 |
|---------|-------------|---------------|------|
| 股息率 | ✅ get_factor_data | DIV_YIELD | iFind原生支持 |
| 分红率 | ✅ get_factor_data | DIV_PAYOUT_RATIO | iFind原生支持 |
| 分红增长率 | ✅ get_factor_data | DIV_GROWTH_3Y, DIV_GROWTH_5Y | iFind原生支持 |
| 分红稳定性 | ✅ get_factor_data | DIV_STABILITY_5Y | iFind原生支持 |
| 财务数据 | ✅ get_financial_statements | income_statement/balance_sheet | iFind原生支持 |
| 估值数据 | ✅ get_factor_data | PE_TTM, PB_LF | iFind原生支持 |
| 资产负债率 | ✅ get_factor_data | DEBT_RATIO | iFind原生支持 |
| 流动比率 | ✅ get_factor_data | CURRENT_RATIO | iFind原生支持 |
| 经营现金流 | ✅ get_factor_data | OPERATING_CASH_FLOW | iFind原生支持 |
| 自由现金流 | ✅ get_factor_data | FREE_CASH_FLOW | iFind原生支持 |
| 信用评级 | ⚠️ 需补充 | - | 中诚信 (付费，可选) |
| 久期数据 | ⚠️ 需计算 | - | 基于股息数据计算 |

**补充说明**:
- 信用评级数据为可选数据，可使用iFind的财务质量因子替代
- 久期数据可通过iFind的股息数据计算得出
...
详见: [保险资金智能体完整实现代码](#任务3设计保险资金智能体完整实现)
...
```
┌─────────────────────────────────────────────────────────────┐
│          Layer 2.5: 市场参与者模拟层 (v1.1)                │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  ┌─────────────────────────────────────────────────────┐   │
│  │         核心智能体 (Core Agents) - 4类              │   │
│  ├─────────────────────────────────────────────────────┤   │
│  │                                                     │   │
│  │  国家队智能体 ████████ 4.08%                       │   │
│  │  └─ 逆周期调节、市场稳定                           │   │
...
| 因子名称 | 数据来源 | 因子含义 | 应用场景 |
|---------|---------|---------|---------|
| **外资流向因子** ⭐ | 外资智能体 | 北向资金流向、全球宏观评分、汇率影响 | 判断外资动向，预测核心资产走势 |
| **保险资金配置因子** ⭐ | 保险资金智能体 | 高股息股票配置、久期匹配度、风险预算分配 | 判断保险资金动向，预测高股息股票走势 |
...
class ForeignInvestorActivityFactor(BaseFactor):
    ...
    数据源: Layer 2.5 外资智能体输出
    
    因子构成:
    1. 北向资金流向强度 (NorthboundCapitalFlowIntensity)
    2. 全球宏观评分 (GlobalMacroScore)
    3. 汇率影响度 (ExchangeRateImpact)
    4. 核心资产偏好度 (CoreAssetPreference)
    """
    ...
    def calculate(self, data: pd.DataFrame) -> pd.Series:
        ...
        输入:
            data: 包含北向资金、汇率、全球宏观数据
            
        输出:
            pd.Series: 外资动向因子值 (范围[-1, 1])
            - 正值: 外资流入
            - 负值: 外资流出
            - 绝对值越大,强度越大
        """
...
| 信号类型 | 生成方式 | 信号含义 | 应用场景 |
|---------|---------|---------|---------|
| **外资配置信号** ⭐ | 外资智能体 | 外资买入/卖出建议 | 用于核心资产配置 |
| **保险资金配置信号** ⭐ | 保险资金智能体 | 保险资金买入/卖出建议 | 用于高股息股票配置 |
...
```
┌─────────────────────────────────────────────────────────────┐
│              数据源集成架构 (Data Integration)              │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  ┌──────────────────────────────────────────────────────┐  │
│  │         核心数据源 (Primary Source)                  │  │
│  │  ┌────────────────────────────────────────────────┐  │  │
│  │  │  同花顺iFind (5700+因子、财务、舆情、宏观)     │  │  │
│  │  │  - 因子数据: get_factor_data()                │  │  │
│  │  │  - 财务数据: get_financial_statements()       │  │  │
│  │  │  - 舆情数据: get_news_data()                  │  │  │
│  │  │  - 宏观数据: get_macro_data()                 │  │  │
...
│  │  ┌────────────────────────────────────────────────┐  │  │
│  │  │  MarketParticipantDataFetcher                  │  │  │
│  │  │  - fetch_foreign_investor_data()              │  │  │
│  │  │  - fetch_insurance_fund_data()                │  │  │
│  │  │  - fetch_national_team_data()                 │  │  │
...
**核心类**: `MarketParticipantDataFetcher`

**职责**:
- 统一管理各类智能体的数据获取
- 封装iFind、FRED API、Yahoo Finance数据源
- 提供统一的数据获取接口
...
class MarketParticipantDataFetcher:
    ...
    职责: 统一管理各类智能体的数据获取
    数据源: iFind (核心) + FRED API + Yahoo Finance (补充)
    """
    ...
    def __init__(self, 
        ...
        Args:
            ifind_connector: iFind连接器 (核心数据源)
...
| 数据质量指标 | 目标值 | 检查方法 | 告警阈值 |
|------------|--------|---------|---------|
| **数据完整性** | ≥95% | 每日检查缺失值 | <90% 告警 |
| **数据及时性** | ≥90% | 检查数据更新时间 | 延迟>1天 告警 |
| **数据准确性** | ≥98% | 交叉验证多个数据源 | 偏差>5% 告警 |
| **数据一致性** | ≥95% | 检查数据逻辑一致性 | 异常值 告警 |
...
**施工阶段 (后续)**:
- 🚧 实现MarketParticipantDataFetcher类
- 🚧 集成iFind连接器
- 🚧 测试数据获取流程
- 🚧 注册FRED API账号 (如需要)
- 🚧 测试Yahoo Finance数据获取 (如需要)
