...
### 1.1 设计背景与业务目标
- **业务需求**: 将训练好的模型部署为服务,提供实时预测能力
- **技术痛点**: 
  - 缺乏模型服务化: 模型无法在线服务,仅支持离线预测
  - 缺乏版本管理: 模型版本混乱,无法回滚
  - 缺乏性能监控: 模型性能无监控,退化无感知
  - 缺乏热更新机制: 模型更新需要停服
- **预期价值**: 
  - 提供实时模型预测服务
  - 支持模型版本管理和回滚
...
class ModelServingService:
    ...
    def _get_model_version(self, model_id: str) -> str:
        """获取模型版本"""
        # 从Redis获取当前激活版本
        version = self.redis_client.get(f"model:{model_id}:active_version")
    ...
    def _activate_version(self, model_id: str, version: str):
        ...
        # 2. 更新Redis中的激活版本
        self.redis_client.set(f"model:{model_id}:active_version", version)
...
| 指标 | 目标值 | 测量方法 | 备注 |
|------|--------|----------|------|
| **响应时间** | ≤100ms | P95延迟 | 单次预测 |
| **吞吐量** | ≥1000 QPS | 每秒请求数 | 并发预测 |
| **可用性** | ≥99.9% | 每月宕机时间 | SLA要求 |
| **模型加载时间** | ≤5秒 | 冷启动时间 | 新模型加载 |
