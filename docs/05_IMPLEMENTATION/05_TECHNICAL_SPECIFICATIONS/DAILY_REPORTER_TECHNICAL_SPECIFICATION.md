...
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │                    数据采集层                                │  │
│  │  ├── TradingDataCollector (交易数据采集)                    │  │
│  │  ├── PerformanceDataCollector (绩效数据采集)                │  │
│  │  ├── MarketDataCollector (市场数据采集)                     │  │
│  │  └── RiskDataCollector (风险数据采集)                       │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                              ↓                                      │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │                    数据处理层                                │  │
...
| 维度 | 定义 |
|------|------|
| **Layer定位** | Layer 7: AI报告层 - 绩效分析与报告生成 |
| **核心职责** | 每日交易数据汇总、绩效指标计算、AI报告生成、定时调度 |
...
```python
上游依赖:
- PerformanceAnalyzer (绩效分析器): 提供绩效指标数据
- PositionManager (仓位管理器): 提供持仓数据
- TradeAuditor (交易审计器): 提供交易记录
- MarketAnalyzer (市场分析器): 提供市场行情数据
- RiskMonitor (风险监控器): 提供风险指标数据

下游服务:
- StreamlitDashboard (可视化仪表板): 展示日报
- NotificationSystem (通知系统): 推送日报
- ReportArchive (报告归档): 存储历史报告
```
...
今日账户总资产 **¥1,234,567.89**，日收益率 **+1.23%**，累计收益率 **+23.45%**。
市场整体震荡上行，沪深300指数上涨0.89%。
...
| 类型 | 数量 | 成交额 | 手续费 |
|------|------|--------|--------|
| 买入 | 5笔 | ¥234,567 | ¥234.57 |
| 卖出 | 3笔 | ¥156,789 | ¥156.79 |
| 合计 | 8笔 | ¥391,356 | ¥391.36 |

### 2.2 持仓情况

| 股票代码 | 股票名称 | 持仓数量 | 市值 | 权重 | 日收益 |
|----------|----------|----------|------|------|--------|
| 000001.SZ | 平安银行 | 1000股 | ¥12,345 | 1.0% | +2.3% |
| ... | ... | ... | ... | ... | ... |
...
| 指标 | 目标值 | 说明 |
|------|--------|------|
| **报告生成时间** | ≤30秒 | 从数据采集到报告生成完成 |
| **数据采集时间** | ≤10秒 | 从上游模块获取所有数据 |
| **AI解读时间** | ≤15秒 | LLM生成洞察和建议 |
| **报告大小** | ≤5MB | 单个报告文件大小 |
| **并发支持** | ≥10 | 同时生成报告数量 |
...
│                                                                     │
│  上游数据源                                                          │
│  ├── PerformanceAnalyzer → 绩效指标数据                             │
│  ├── PositionManager → 持仓数据                                     │
│  ├── TradeAuditor → 交易记录                                        │
│  ├── MarketAnalyzer → 市场行情数据                                  │
│  └── RiskMonitor → 风险指标数据                                     │
│                              ↓                                      │
...
│                              ↓                                      │
│  输出                                                                │
│  ├── 报告存储 (data/reports/daily/)                                 │
│  ├── 通知推送 (NotificationSystem)                                  │
│  └── 可视化展示 (StreamlitDashboard)                                │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
- 总资产: ¥{trading_summary.total_assets:,.2f}
- 日收益率: {trading_summary.daily_pnl_pct:.2%}
- 累计收益率: {trading_summary.cumulative_pnl_pct:.2%}
- 交易次数: {trading_summary.total_trades}笔
- 持仓数量: {trading_summary.positions_count}只
...
| 参数 | 默认值 | 调优范围 | 说明 |
|------|--------|----------|------|
| **max_tokens** | 2000 | 1000-4000 | LLM最大生成token数 |
...
| 类别 | 技术选型 | 版本要求 | 说明 |
|------|----------|----------|------|
| **编程语言** | Python | ≥3.10 | 类型提示、dataclass支持 |
| **数据处理** | Pandas | ≥2.0 | 数据聚合、计算 |
| **模板引擎** | Jinja2 | ≥3.1 | 报告模板渲染 |
| **任务调度** | APScheduler | ≥3.10 | 定时任务调度 |
| **图表生成** | Plotly | ≥5.18 | 交互式图表 |
| **LLM接口** | zhipuai | ≥2.0 | GLM-4-Flash API |
...
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │                    数据存储层                                │  │
│  │  ├── 报告文件存储 (data/reports/daily/)                     │  │
│  │  ├── 元数据数据库 (SQLite/MySQL)                            │  │
│  │  └── 模板文件 (templates/daily/)                            │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                              ↓                                      │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │                    输出层                                    │  │
│  │  ├── StreamlitDashboard (可视化展示)                        │  │
...
| 测试场景 | 性能目标 | 测试方法 |
|----------|----------|----------|
| **报告生成** | ≤30秒 | 生成100份报告，计算平均时间 |
| **并发生成** | ≥10并发 | 同时生成10份报告，验证成功率 |
| **大数据量** | ≤60秒 | 1000只持仓、100笔交易的报告生成 |
...
| 风险ID | 风险描述 | 风险等级 | 缓解措施 |
|--------|----------|----------|----------|
| **R001** | LLM API调用失败或超时 | P1 | 实现重试机制、降级方案（使用预定义模板） |
| **R002** | 上游数据源不可用 | P1 | 实现数据缓存、降级方案（使用历史数据） |
| **R003** | 报告生成超时 | P2 | 设置超时限制、异步生成 |
| **R004** | 模板渲染错误 | P2 | 模板验证、异常捕获 |
| **R005** | 存储空间不足 | P3 | 定期归档、容量监控 |
...
| 功能项 | 验收标准 | 验证方法 |
|--------|----------|----------|
| **日报生成** | 每日收盘后自动生成报告 | 自动化测试 |
| **数据聚合** | 准确汇总交易、持仓、绩效数据 | 数据验证 |
| **AI解读** | 生成有价值的洞察和建议 | 人工评审 |
| **定时调度** | 按时触发报告生成 | 调度测试 |
| **报告导出** | 支持多种格式导出 | 功能测试 |
...
| 任务 | 工时 | 交付物 |
|------|------|--------|
| 数据采集模块开发 | 1天 | TradingDataCollector |
| 数据聚合模块开发 | 1天 | DataAggregator |
| 报告生成模块开发 | 1天 | TemplateRenderer |
| AI解读模块开发 | 1天 | LLMInterpreter |
| 单元测试编写 | 1天 | 测试用例、测试报告 |
...
2. [RISK_REPORT.md](../../03_TRADING_TACTICS/09_RISK_RULES/RISK_REPORT.md) - 风险报告生成器蓝图
3. [AI_FACTOR_AGENT.md](../../02_FACTOR_LIBRARY/07_FACTOR_MONITORING/AI_FACTOR_AGENT.md) - AI因子管家
4. [PERFORMANCE_ANALYZER_TECHNICAL_SPECIFICATION.md](./PERFORMANCE_ANALYZER_TECHNICAL_SPECIFICATION.md) - 绩效分析器技术规格
