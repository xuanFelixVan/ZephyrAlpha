...
```
┌─────────────────────────────────────────────────────────────────────┐
│                    MonthlyReporter月报生成器                         │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │                    数据采集层                                │  │
│  │  ├── MonthlyDataCollector (月度数据采集)                    │  │
│  │  ├── DailyReportAggregator (日报数据聚合)                   │  │
│  │  ├── HistoricalDataLoader (历史数据加载)                    │  │
│  │  └── BenchmarkDataCollector (基准数据采集)                  │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                              ↓                                      │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │                    数据处理层                                │  │
│  │  ├── TrendAnalyzer (趋势分析)                               │  │
│  │  ├── MonthOverMonthCalculator (环比计算)                    │  │
│  │  ├── YearOverYearCalculator (同比计算)                      │  │
│  │  └── PerformanceAttributor (绩效归因)                       │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                              ↓                                      │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │                    AI解读层                                  │  │
│  │  ├── TrendInterpreter (趋势解读)                            │  │
...
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │                    报告生成层                                │  │
│  │  ├── MonthlyTemplateRenderer (月报模板渲染)                 │  │
│  │  ├── TrendChartGenerator (趋势图表生成)                     │  │
│  │  └── ReportFormatter (报告格式化)                           │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                              ↓                                      │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │                    调度与输出层                              │  │
│  │  ├── MonthlyScheduleManager (月度调度管理)                  │  │
│  │  ├── ReportStorage (报告存储)                               │  │
...
| 维度 | 定义 |
|------|------|
| **Layer定位** | Layer 7: AI报告层 - 月度绩效分析与报告生成 |
| **核心职责** | 月度数据汇总、趋势分析、策略评估、AI月报生成 |
...
```python
上游依赖:
- PerformanceAnalyzer (绩效分析器): 提供月度绩效指标
- PositionManager (仓位管理器): 提供月末持仓数据
- DailyReporter (日报生成器): 提供每日报告数据
- MarketAnalyzer (市场分析器): 提供月度市场数据
- RiskMonitor (风险监控器): 提供月度风险指标

下游服务:
- StreamlitDashboard (可视化仪表板): 展示月报
- NotificationSystem (通知系统): 推送月报
...
2026年4月，账户总资产 **¥1,345,678.90**，月收益率 **+8.92%**，累计收益率 **+34.57%**。
本月跑赢沪深300指数 **+3.21%**，夏普比率 **2.56**，最大回撤 **-4.12%**。

### 关键指标

| 指标 | 本月数值 | 上月数值 | 环比变化 | 状态 |
|------|----------|----------|----------|------|
| 月收益率 | +8.92% | +5.67% | +3.25% | ✅ 改善 |
| 夏普比率 | 2.56 | 2.34 | +0.22 | ✅ 改善 |
| 最大回撤 | -4.12% | -3.45% | -0.67% | ⚠️ 恶化 |
| 胜率 | 68.5% | 65.2% | +3.3% | ✅ 改善 |
...
本月收益率 **+8.92%**，环比上月 **+3.25%**，同比去年 **+2.34%**。
收益趋势 **向上**，趋势强度 **中等**。

### 3.2 风险趋势

最大回撤 **-4.12%**，环比上月 **-0.67%**，风险有所上升。
波动率 **13.45%**，环比上月 **+1.23%**，波动率略有上升。

### 3.3 效率趋势

夏普比率 **2.56**，环比上月 **+0.22**，风险调整收益改善。
胜率 **68.5%**，环比上月 **+3.3%**，交易效率提升。
...
| 均值回归 | +6.78% | 65.4% | 2.12 | ✅ 良好 |
| 因子策略 | +5.23% | 61.2% | 1.89 | ⚠️ 一般 |

### 4.2 策略建议

1. **动量策略**: 表现优秀，建议保持当前配置
2. **均值回归**: 表现良好，可适当增加权重
...
| 指标 | 目标值 | 说明 |
|------|--------|------|
| **报告生成时间** | ≤60秒 | 从数据采集到报告生成完成 |
| **数据采集时间** | ≤20秒 | 从上游模块获取所有数据 |
| **AI解读时间** | ≤30秒 | LLM生成洞察和建议 |
| **报告大小** | ≤10MB | 单个报告文件大小 |
| **并发支持** | ≥5 | 同时生成报告数量 |
...
-- 月报访问日志表
CREATE TABLE monthly_report_access_log (
...
│                                                                     │
│  上游数据源                                                          │
│  ├── PerformanceAnalyzer → 月度绩效指标                             │
│  ├── PositionManager → 月末持仓数据                                 │
│  ├── DailyReporter → 每日报告数据                                   │
│  ├── MarketAnalyzer → 月度市场数据                                  │
│  └── RiskMonitor → 月度风险指标                                     │
│                              ↓                                      │
...
- 基准收益率: {monthly_summary.benchmark_return:.2%}
- 超额收益: {monthly_summary.excess_return:.2%}
- 夏普比率: {monthly_summary.sharpe_ratio:.2f}
- 最大回撤: {monthly_summary.max_drawdown:.2%}
...
| 参数 | 默认值 | 调优范围 | 说明 |
|------|--------|----------|------|
| **max_tokens** | 3000 | 2000-5000 | LLM最大生成token数 |
...
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │                    数据存储层                                │  │
│  │  ├── 报告文件存储 (data/reports/monthly/)                   │  │
│  │  ├── 元数据数据库 (SQLite/MySQL)                            │  │
│  │  └── 模板文件 (templates/monthly/)                          │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                              ↓                                      │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │                    输出层                                    │  │
...
| 测试场景 | 性能目标 | 测试方法 |
|----------|----------|----------|
| **报告生成** | ≤60秒 | 生成12份报告，计算平均时间 |
| **并发生成** | ≥5并发 | 同时生成5份报告，验证成功率 |
| **大数据量** | ≤90秒 | 12个月数据、1000只持仓的报告生成 |
...
| 风险ID | 风险描述 | 风险等级 | 缓解措施 |
|--------|----------|----------|----------|
| **R001** | LLM API调用失败或超时 | P1 | 实现重试机制、降级方案 |
| **R002** | 上游数据源不可用 | P1 | 实现数据缓存、降级方案 |
...
| 功能项 | 验收标准 | 验证方法 |
|--------|----------|----------|
| **月报生成** | 每月第1个工作日自动生成报告 | 自动化测试 |
| **趋势分析** | 准确分析环比、同比趋势 | 数据验证 |
...
| 任务 | 工时 | 交付物 |
|------|------|--------|
| 数据采集模块开发 | 1天 | MonthlyDataCollector |
| 趋势分析模块开发 | 1.5天 | TrendAnalyzer |
| 绩效归因模块开发 | 1.5天 | PerformanceAttributor |
| 报告生成模块开发 | 1天 | MonthlyTemplateRenderer |
| AI解读模块开发 | 1天 | TrendInterpreter |
...
| 任务 | 工时 | 交付物 |
|------|------|--------|
| 调度模块开发 | 1天 | MonthlyScheduleManager |
| 上游模块集成 | 1天 | 集成接口 |
| 下游模块集成 | 0.5天 | 输出接口 |
| 集成测试 | 0.5天 | 集成测试报告 |
...
3. [DAILY_REPORTER_TECHNICAL_SPECIFICATION.md](./DAILY_REPORTER_TECHNICAL_SPECIFICATION.md) - 日报生成器技术规格
