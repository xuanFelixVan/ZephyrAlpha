...
owner: 首席文档架构�?standard_type: 专业量化机构审计报告
applicable_scope: 代码目录和临时文件审�?compliance_level: 深度审计
parent_document: ../INDEX.md
implementation_status: 已完�?---

# 深度审计报告：代码目录与临时文件

> 清风量化系统 v5.3 代码目录与临时文件审�?>
> **审计日期**: 2026-04-03
> **审计类型**: 深度审计
> **审计范围**: notebooks, scripts, src, temp, tests, tools, 根目录文�?> **审计目标**: 识别重复文件、职责不清内容、敏感信息风�?
...
| 风险等级 | 数量 | 说明 |
|----------|------|------|
| 🔴 **P0 高风�?* | 3 | 敏感信息、版本隔离、临时文件漂�?|
| 🟡 **P1 中风�?* | 4 | 重复脚本、空目录、文件漂�?|
| 🟢 **P2 低风�?* | 2 | 文档位置、命名规�?|
| **总计** | **9** | - |
...
| **建议** | 1. 确保不提交到版本控制 2. 定期更换密码 3. 考虑使用环境变量或密钥管理服�?|

**敏感信息内容**（已脱敏�?
```
QMT_SIMULATION_ACCOUNT=8886156677
QMT_SIMULATION_PASSWORD=***（已脱敏�?QMT_LIVE_ACCOUNT=8887871993
QMT_LIVE_PASSWORD=***（已脱敏�?```

**安全检�?*:
- �?.gitignore已配�? `.env.qmt  # QMT账号密码配置`
- �?文件未被git追踪
...
| 文件 | 版本 | 建议 |
|------|------|------|
| `test_qmt_connection.py` | 原版 | 🗑�?删除（旧版本�?|
| `test_qmt_connection_v2.py` | v2 | 🗑�?删除（旧版本�?|
| `test_qmt_connection_v3.py` | v3 | 🗑�?删除（旧版本�?|
| `test_qmt_connection_v4.py` | v4 | 🗑�?删除（旧版本�?|
| `test_qmt_connection_v5.py` | v5 | 🗑�?删除（旧版本�?|
| `test_qmt_connection_v6.py` | v6 | �?保留（最新版本） |
| `test_qmt_connection_full.py` | 完整�?| ⚠️ 检查是否与v6重复 |
...
| 文件 | 职责 | 建议 |
|------|------|------|
| `temp_a_stock_blueprint.md` | A股数据处理蓝�?| 📦 归档�?`docs/06_CONSTRUCTION_DOCS/` 或删�?|
| `temp_check_file.py` | 临时文件检查脚�?| 🗑�?删除（一次性使用） |
| `temp_modify_file.py` | 临时文件修改脚本 | 🗑�?删除（一次性使用） |
...
| `quick_diagnosis.py` | 快速诊�?| ⚠️ 检查是否与其他重复 |

**建议**: 整合诊断脚本，或明确各脚本的差异化职�?
#### 问题5: 空目�?
| 属�?| 详情 |
|------|------|
| **位置** | `temp/` |
...
| **位置** | 根目�?|
| **风险** | 根目录污�?|
| **建议** | 移动�?`data/archive/` 或删�?|

#### 问题7: 文件漂移 - PDF文档

| 属�?| 详情 |
|------|------|
| **文件** | `迅投QMT极速策略交易系统说明文�?pdf` |
| **位置** | 根目�?|
| **风险** | 根目录污�?|
| **建议** | 移动�?`docs/09_EXTERNAL_DOCS/` �?`data/references/` |
...
| **位置** | 根目�?|
| **现状** | �?`docs/09_AUDIT/` 目录功能重叠 |
| **建议** | 移动�?`docs/09_AUDIT/REPORTS/` 或保持现�?|
...
| 文件 | 职责 | 状�?|
|------|------|------|
| `automated_audit_tool.py` | 自动化审计工�?| �?正常 |
...
|------|--------|------|
| **职责驱动原则** | 85% | 诊断脚本职责重叠 |
| **索引完备性原�?* | 90% | 各目录有README |
| **版本隔离原则** | 60% | test_qmt_connection系列版本混乱 |
| **文档代码对应原则** | 95% | 代码与文档同�?|
| **命名规范原则** | 80% | temp_*文件命名不规�?|
...
| `.env.example` | 配置模板 | �?无敏感信�?|
| `.env.qmt.example` | 配置模板 | �?无敏感信�?|
...
| 优先�?| 操作 | 文件 | 风险 |
|--------|------|------|------|
| P0 | 删除旧版本测试脚�?| `test_qmt_connection.py` ~ `test_qmt_connection_v5.py` | 版本隔离 |
...
| P1 | 移动PDF文档 | `迅投QMT极速策略交易系统说明文�?pdf` | 目录污染 |
...
| 文件 | 原因 | 风险等级 |
|------|------|----------|
| `scripts/test_qmt_connection.py` | 旧版�?| P0 |
| `scripts/test_qmt_connection_v2.py` | 旧版�?| P0 |
| `scripts/test_qmt_connection_v3.py` | 旧版�?| P0 |
| `scripts/test_qmt_connection_v4.py` | 旧版�?| P0 |
| `scripts/test_qmt_connection_v5.py` | 旧版�?| P0 |
...
| `迅投QMT极速策略交易系统说明文�?pdf` | `docs/09_EXTERNAL_DOCS/` | P1 |
...
- **内容深度**: 每个文件内容已审�?
### 8.2 审计局限�?
1. 未执行代码功能测�?2. 未验证脚本执行结�?3. 敏感信息已脱敏处�?
### 8.3 后续审计建议

1. 执行清理操作后进行验证审�?2. 建立定期审计机制（每月一次）
...
├── test_qmt_connection.py        # 旧版�?├── test_qmt_connection_full.py
├── test_qmt_connection_official_api.py
├── test_qmt_connection_v2.py     # 旧版�?├── test_qmt_connection_v3.py     # 旧版�?├── test_qmt_connection_v4.py     # 旧版�?├── test_qmt_connection_v5.py     # 旧版�?├── test_qmt_connection_v6.py     # 最新版�?�?├── verify_qmt_environment.py
