...
> **补充文档**: 本蓝图是 [STRATEGY_ENGINE_CORE_BLUEPRINT.md](../../03_TRADING_TACTICS/01_STRATEGY_FRAMEWORK/STRATEGY_ENGINE_CORE_BLUEPRINT.md) 的技术补充，专注于策略排名与选择功能
...
- 基于市场状态动态调整评分权?- 考虑用户风险偏好个性化权重
- 使用优化算法寻找最优权重分?
**PerformanceScorer (绩效评分?**
- 计算收益类指标评分（夏普比率、年化收益、胜率等?- 时间衰减加权（近期绩效权重更高）
- 统计显著性检?
**RiskScorer (风险评分?**
- 计算风险类指标评分（最大回撤、波动率、下行风险等?- 极端风险事件检?- 风险调整收益计算

**StabilityScorer (稳定性评分器)**
- 评估策略绩效稳定?- 收益序列自相关性分?- 参数敏感性测?
**AdaptabilityScorer (适应性评分器)**
- 评估策略对不同市场状态的适应?- 市场状态切换检?- 策略鲁棒性分?
...
class TOPSISEvaluator:
    ...
    职责: 使用TOPSIS算法进行多维度策略排?    特点: 客观权重分配，避免主观偏?    """
    ...
    def evaluate(self, strategies: List[Strategy], 
        ...
        1. 构建决策矩阵
        2. 标准化决策矩?        3. 计算加权标准化矩?        4. 确定正理想解和负理想?        5. 计算各方案到理想解的距离
        6. 计算相对接近度并排序
        """
        ...
        # 3. 计算加权标准化矩?        weighted_matrix = self._apply_weights(normalized_matrix)
        
        # 4. 确定正理想解和负理想?        positive_ideal, negative_ideal = self._calculate_ideal_solutions(weighted_matrix)
    ...
    def _calculate_ideal_solutions(self, weighted_matrix: pd.DataFrame) -> Tuple[pd.Series, pd.Series]:
        """计算正理想解和负理想?        
        对于效益型指标（越大越好）：正理想解取最大值，负理想解取最小?        对于成本型指标（越小越好）：正理想解取最小值，负理想解取最大?        """
        # 定义指标类型
...
class DynamicWeightOptimizer:
    ...
    def _calculate_base_weights(self, market_state: MarketState) -> Dict[str, float]:
        ...
        weight_templates = {
            'bull_market': {
                'sharpe_ratio': 0.20,
                'max_drawdown': 0.15,
                'annual_return': 0.25,  # 牛市更看重收?                'win_rate': 0.10,
            ...
            'bear_market': {
                'sharpe_ratio': 0.25,
                'max_drawdown': 0.25,   # 熊市更看重风险控?                'annual_return': 0.10,
                'win_rate': 0.10,
            ...
            'high_volatility': {
                'sharpe_ratio': 0.25,
                'max_drawdown': 0.30,   # 高波动市场特别重视风险控?                'annual_return': 0.10,
                'win_rate': 0.10,
                'stability_score': 0.15,
        ...
        for criterion, base_weight in template.items():
            if criterion in ['max_drawdown', 'stability_score']:
                # 风险相关指标随市场波动强度增加而增?                adjusted = base_weight * (1 + strength_factor * 0.5)
    ...
    def _adjust_for_user_preferences(self, base_weights: Dict[str, float],
        ...
        if risk_tolerance <= 2:  # 保守?            adjusted['max_drawdown'] *= 1.5
...
class TimeWeightedPerformanceScorer:
    ...
    def _time_weighted_sharpe(self, returns: pd.Series) -> float:
        ...
        # 年化处理（假设日收益?        annualized_sharpe = weighted_returns / weighted_volatility * np.sqrt(252)
    ...
    def _time_weighted_annual_return(self, equity_curve: pd.Series) -> float:
        ...
        # 将权益曲线分段计算收?        n_segments = min(10, len(equity_curve) // 30)  # 每段?0个交易日
        segment_returns = []
        ...
        # 加权平均并年?        weighted_return = sum(segment_returns) / sum(self.decay_factor ** i for i in range(n_segments))
    ...
    def _time_weighted_max_drawdown(self, equity_curve: pd.Series) -> float:
        ...
        # 计算滚动最大回?        rolling_dd = self._rolling_drawdown(equity_curve, window=60)  # 60日滚动窗?        
        if rolling_dd.empty:
            return 0
            
        # 计算时间加权平均最大回?        weights = self._exponential_decay_weights(len(rolling_dd))
    ...
    def _exponential_decay_weights(self, n_periods: int) -> np.ndarray:
        ...
        # 反转使最近期权重最?        weights = weights[::-1]
        # 归一?        weights = weights / weights.sum()
        return weights
```
...
class StrategyCorrelationAnalyzer:
    ...
    def analyze(self, strategies: List[Strategy], 
        ...
        # 1. 计算收益相关性矩?        corr_matrix = self._calculate_correlation_matrix(returns_data)
        
        # 2. 识别高度相关策略?        high_corr_groups = self._find_high_correlation_groups(corr_matrix)
        
        # 3. 计算风险分散潜力
        diversification_potential = self._calculate_diversification_potential(corr_matrix)
        
        # 4. 动态相关性分析（滚动窗口?        dynamic_corr = self._analyze_dynamic_correlation(returns_data)
        
        # 5. 市场状态相关性分?        market_state_corr = self._analyze_market_state_correlation(returns_data)
    ...
    def _calculate_diversification_potential(self, corr_matrix: pd.DataFrame) -> pd.DataFrame:
        ...
        for strategy in corr_matrix.index:
            # 计算该策略与其他策略的平均相关?            other_correlations = [corr_matrix.loc[strategy, other] 
    ...
    def _generate_recommendations(self, corr_matrix: pd.DataFrame,
        ...
        # 推荐3：平衡收益相关性和风险相关?        risk_corr_matrix = self._calculate_risk_correlation(corr_matrix)
...
class AIStrategyRecommender:
    ...
    def _construct_ai_portfolio(self, strategies: List[Strategy],
        ...
        # 根据用户风险偏好确定组合规模
        if user_prefs.risk_tolerance <= 2:  # 保守?            n_strategies = min(5, len(strategies))
        ...
        for i, (strategy, score) in enumerate(sorted_strategies[:n_strategies * 2]):
            ...
            # 检查相关?            if self._is_diversified(selected, strategy, diversification_weight):
...
strategy_selection:
  # 用户偏好配置
  user_preferences:
    risk_tolerance: 3  # 1-5?为中?    investment_horizon: "medium_term"  # short_term, medium_term, long_term
...
# 相关性分?python strategy_selector.py analyze-correlation \
  --strategies "strategies/*.yaml" \
  --period "1y" \
  --threshold 0.7 \
...
### 5.3 Web仪表盘功?
**核心功能模块**?1. **实时排名看板**：策略综合评分动态展?2. **相关性热力图**：策略间收益相关性可视化
3. **AI推荐面板**：AI生成的策略组合推?4. **绩效对比图表**：多策略绩效对比分析
5. **风险分析仪表**：组合风险指标监?6. **自然语言问答**：用户问题AI解答

**技术栈建议**?- 前端：Streamlit（快速原型）?Vue.js + D3.js（生产环境）
- 后端：FastAPI + 异步任务队列
...
- [ ] 基础命令行接?
### Phase 2: 高级分析功能?周）
- [ ] WeightOptimizer 动态权重调?- [ ] StrategyCorrelationAnalyzer 相关性分?- [ ] 时间加权评分算法
- [ ] 可视化排名报?
### Phase 3: AI决策支持?周）
...
```markdown
#### Layer 0: 系统架构
##### 0.001. Tactics Strategy Selection Bp
- **模块ID**: TACTICS_STRATEGY_SELECTION_BP_001
...
| 模块 | 职责 | 边界 |
|------|------|------|
| **Tactics Strategy Selection Bp** | 全系统架构设?compliance_level: 初始标准 | **核心模块** |
