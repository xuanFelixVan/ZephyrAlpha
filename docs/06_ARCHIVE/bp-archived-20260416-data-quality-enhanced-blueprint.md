...
- 全库 API 与事件约定真源：`API_Contract.md`。质量增强涉及的质量规则、修复建议输出、告警事件与报告口径等对外约定需以该真源或其子契约为准。

- 邻层协同边界：与 **Layer 0（数据源）**、**Layer 1（预处理）**、**Layer 10（治理与合规）** 的交互以契约为准（避免增强规则与治理规则冲突）。
...
- 能对至少一个数据集产出“质量问题清单 + 修复建议 + 影响范围”，并可复核证据样本。

- 能对修复动作（回填/剔除/重算/冻结任一）给出可追溯记录，并可复现一次修复前后对比。

- 能输出质量增强后的评分/指标改善结果，并能解释计算口径。

- 能对重大质量异常触发告警并留痕（含证据、处置建议与责任边界说明）。
...
- 修复策略的字段级字典、事件载荷与自动化执行接口将在施工阶段固化到 `API_Contract.md` 子契约；本蓝图先确保边界、接口闭合点与验收闭环清晰。
