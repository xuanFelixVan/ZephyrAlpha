---
module_id: 01_FRAMEWORK_P0_CORE_MODULES_BLUEPRINT_COLLECTION
layer: layer_01
version: 1.0.0
status: Active
priority: P0
responsibility:
  - P0 Core Modules Blueprint Collection相关业务
...
- 全库 API 与事件约定真源：`API_Contract.md`。本文汇总的 P0 模块若对外暴露接口/事件（数据查询、下单、风控、审计、报告等），必须以该真源为最终口径；本文仅提供汇总入口与对齐要求。
...
- 本文“模块清单”中的每个 P0 模块条目，必须能定位到其对应蓝图入口（至少文件名与路径可点击或可检索定位）。

- 若本文中描述某模块存在“对外接口/事件”，则应能在 `API_Contract.md` 中找到对应契约入口（或在“已知限制”中列出未闭合项与原因）。
...
- 本文是“汇总型蓝图集合”，不承诺覆盖每个模块的所有字段级契约与实现细节；细节以各模块蓝图与 `API_Contract.md` 为准。

- 若后续发生模块蓝图重命名/拆分，本文件中的清单与链接可能短期不同步；以索引治理专项批次的统一映射为准。
