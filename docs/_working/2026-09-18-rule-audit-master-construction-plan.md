...
> **怎么用这份文件**（三条，读完再动手）：
> 1. **裁定已封口**：第 1 节 D-1～D-12 是 Max 已做完的全部判断。施工队**不得自行判断**、不得"顺手优化"。
...
### WP10 · 判决执行
- Flash 只做 **B 类**（改文案、改指向、补二值判据措辞）与 **D 类的登记同步部分**（`superseded_by` 填值、ROOR/capability/翻译登记增删）。
- **C 类（加牙）与 D3/D4（退役/判无效）一律 Max**；涉及注册表净删行的逐条走门位（D-3 授权范围仅限 DB 死数据，不含规则条目净删）。
- 每族一提交，commit message 写明案卷号与出口类别。
...
- **可做**：补缺栏，栏位值**必须实读** import / registry / 现场（`[DEPENDENCIES]` 从真实 import 取、`[CONSUMERS]` 从全仓反向引用取、`[DOMAIN]` 从 depgraph 或 `functional_domain_registry` 取）。
- **禁止**：**编造栏位值**（假身份证比缺栏更坏）；拿不准的栏位留空并记入回执，回流 Max；不得顺手改代码逻辑。
- **验收**：每批跑 `python scripts/ops/verify_header_completeness.py`，缺栏数**必须严格下降**；不降即返工。
