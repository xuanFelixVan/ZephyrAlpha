...
>
> **✅ 已完成（1 条，摘录）**
> - L76: | 源（海外） | SEC EDGAR EFTS 全文检索（efts.sec.gov，免注册 JSON API，需声明 User-Agent）；QuantsPlaybook 社区已验证此类文本因子工程 |
>
> **⚠️ 未完成（6 条，逐条摘录）**
> - L102: | IMF | 未接 | 免费数据门户 | 新增：接入 IMF WEO/IFS 精选序列（汇率/外储/全球通胀），落 macro_data 共表（indicator_name 前缀区分，复用 FRED 模式） |
> - L103: | OECD | 未接 | 免费 SDMX API | 低优先：部分指标与 FRED/世行重叠 |
> - L104: | GDELT | 未接 | 100% 免费免注册，15 分钟更新 | 新增：全球新闻事件/GKG 情绪 → 港美股+币圈影子增强（与 news_sentiment_window 同构，英文事件流） |
> - L105: | SEC EDGAR EFTS | 未接 | 免注册 JSON API | 新增：美股申报全文（配合已知缺口"美股深度"） |
> - L106: | pytrends/Google Trends | 未接 | 免费（pytrends 已归档，脆弱） | P2：美股/加密辅助关键词 |
> - L107: | Alpha Vantage/EODHD/FMP/Finnhub 免费层 | 未接 | 免费配额层 | P2：仅用于美股深度补缺（kline_us_daily 源上限已知缺口），配额内做增量 |
>
