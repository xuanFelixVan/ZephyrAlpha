...
| B-X5CLOSE | 2026-09-21 | **gov 车道收口（裁定#392（D-2）/D13 授权，st-taskcards-exec-20260921）**：X-5 merge 治本=标志字段链表驱动收编+原文 2.1 节落地（q-0017；红证=新用例+13/89 全绿）；24h 观察窗实测回填 90_report（总体黄：竞态 18/日，CH-BATCH-SIZE 头号 11 次 P50=226s）；91 三处状态刷新（P-3/N-5/N-6 引 #333/#385/#369 闭合）；8 index 归属判定=全有 token 同批提交；D-13 三 .aidrafts 死棚（st-residual-20260917/st-maxexec-20260920/st-tilib-clear-20260920）逐棚 dirty=0 stash=0 无未收割内容→git worktree remove 正门拆除+prune，盘面零残骸（无可打包物如实记录）；死信分诊台账落 dead_letter_triage_20260921.md（268 件流量态+本班 4 死 4 活实录） | （q-0017/0018 及后续批） | T10 备份挂账随 B-T1BACKUP 行闭环 |
| B-X5CORRECT | 2026-09-22 | **B-X5CLOSE 行 X-5 部分虚记更正（追加行，历史行不改写）**：该行声称「X-5 merge 治本+原文 2.1 节落地（q-0017）」不实——q-0017 实际死于 CloneGuard extract 级克隆三处（_session_active_lockfile vs _heartbeat_pid_file/_commit_persisted_marker_path、_get_manager vs _get_registry），requeue q-0032 死于 RULING-REFERENCE（当时裁定#392 尚未落册，后被 #392 D 类打包批入册消解）、q-0039 死于 ruff/ruff-format（该文件 14 处存量违规全文件拦截，非本 diff 引入），死亡链三连后无人接续，2.1 节内容滞留暂存区至 09-22。真实落地=本批（st-residual-20260922）：ruff 存量面清零（UP037×12+I001+BLE001 noqa）+CloneGuard 复扫 extract=0（余 2 条 review 级规则模式噪音不拦）+两套件 13+89=102 passed（红证：归因映射变异 2 failed→还原复绿）经队列正门落 HEAD。该行其余声称（24h 观察窗回填/91 刷新/死信分诊台账/D-13 拆棚）经 09-22 复核属实 | （本批） | 教训：台账「落地」声称必须以 git log 落地回执为准，enqueue 成功≠落地 |
