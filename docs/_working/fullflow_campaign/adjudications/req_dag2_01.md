...
- **后果若未拦**：按当时工作区提交 = 净删 2 个在册任务 + 塞入 4 个外来任务（触发注册表净删门/Owner 位，
  或在门下游静默改排班）。
- **同窗口复发（实测于本车道落地 `7894946ae0` 之后约 4 分钟）**：推导器已在 HEAD 且磁盘==HEAD，
  索引面却被写入对该文件的 `D`（暂存删除），同时产物文档的 index 面退回我提交前的旧内容。
  另登记他人 staged 删除 2 件（未触碰）。⇒ 与 R-038"12 件消失"同签名，**且本役首次在同一会话内连发两起**。
