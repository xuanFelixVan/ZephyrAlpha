...
| E2 | 产业链 T1：R1 精确化（官方对照表→92 部门补桥） | ⬜ | 92 零命中部门补桥 match_method=official_map；抽 30 ≥90% → verified=true | | 07 §1 R1 |
| E3 | 产业链 T2：R5 IO 2020 回填 flow_wan/coefficient | ⬜ | 非空率 100%+行和校验；as_of=2020 PIT | | 07 §1 R5 |
...
| E9 | alpha T9：研报喂链解锁（脚本修复+适配器+50 篇暂存） | ⬜ | graph_enrich_staging 不崩；50 篇暂存台账不写正图 | | a_mining_workbook |
| E10 | alpha T8：E4 送审（lane_chain_candidates.csv+intake 注册） | ⬜ | 消费点注册+csv schema 合规 | | a_mining_workbook §7 |
| E11 | alpha T7：商品两任务端到端复验 | ⬜ | manual 触发 akshare→CH 闭环实跑 | | c94b3e44c3 基线 |
