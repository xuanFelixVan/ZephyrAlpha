...
## 7. W3 车道：高波资产做T重测（03:0x Owner 裁定增补）

- Owner 裁定原文：做T 测试对象不应是 510300（全市场最低波资产=最不利样本），应为**行业/板块 ETF（半导体、券商等）乃至高波个股**。T3 判定不外推到高波宇宙。
- 新假设卡族：「高波资产的分钟振幅 economics 达成本门」——**宇宙选择规则先于测量写死**（禁事后挑）：①行业/主题 ETF：按 IS 期中位 15min bar 振幅排名取 top10，前置流动性门槛（ADV 阈值写死）；②T+0 类单列（跨境/黄金/债券 ETF=A股唯一可真日内回转的标的，恒生科技/纳指/黄金振幅大且 T+0）；③高波个股 top20（IS 期波动率排名+ADV 门槛，执行模型=底仓T，T+1 约束写明）。
- 成本：各标的按自身 ADV 分档取滑点（Q1-Q5，禁统一 Q5）、佣金万0.854、个股卖出加印花税万5、¥5 地板；判定沿用脱红/绿区带（所需捕获率 ≤0.50 脱红 / ≤0.25 绿区）+ 全量披露（含 RED 标的）；多重检验位置（宇宙数×桶数）写进报告。
- 执行代理=st-bizmine-etft0-20260919；落点 `docs/_working/bizmine_night/etf_t0_retest/`。

## 8. 全夜执行总条款（Owner 令，全程生效）

- 全夜自主执行不停顿、零提问；遇问题按自裁框架裁定：第一性原理→项目文档→专业机构实践→量化社区做法→开源先例，给出分析过程与裁定结果；仍不可裁→登记+跳下一件，最终留 Owner。
- 施工法=「线内先挖后干、线间并行流水」：谁挖干谁先开工；挖干判据=作业簿六向台账+自审闸三态。
- 收官判据：全部车道收口→循环检查已完内容，修到**连续两轮测试问题=0**→红蓝对抗，测出即修→GitCommitGateway 全落地→临时文件清零→端到端完整交付报告（诚实条款：没做到的如实列）。
