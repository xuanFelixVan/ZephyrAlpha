...
| 2 | 106 gate 全链耗时 | **41.4 秒**（当前共享暂存区含他人 WIP 真实混合场景，2 文件 commit 范围） | 只读计时 harness 逐 gate 实测（§8.2） |
...
| 5 | 子进程启动税 | ~65 个 gate 呈 0.16-0.23s 均匀分布 = `run_checker_script` 每次 spawn python 解释器（~0.19s/个），**合计 ≈12s/次提交** | 计时 harness + commit_gate_registry.py L222-277 |
...
- **沙箱原型已验证**：timeout=3.0 → 3.09s 抛 GatewayError；默认 60.0s 复核通过；锁自清后可重获（§8.1）。
...
| A2 checker 合并执行（消子进程税） | 新增 checker supervisor：单个 python 进程顺序/并行跑完全部子进程 checker 脚本，替代 65 次 spawn | **≈12s → <2s**（启动税归零，checker 实际工作时间保留） | 改动面 = run_checker_script 基建 + 各 gate 调用方式；需保持 CREATE_NO_WINDOW 与超时语义 |
...
**风险**：A1 低；A2 中（基建改动，需回归 65 gate 语义等价）；A3 中（语义设计已收敛如上，剩余风险=白名单误纳，需逐 gate 审批准入）。
...
**现状取证**
- 锁内耗时主体（实测）：DECISION-MAP 8.24s + ENCODING-SAFETY 5.37s + RULE-FOUR-WAY-ALIGN 3.82s + TTL-METADATA 3.34s + DIRECTORY-CONTRACT 2.96s + ARCH-REFERENCE 1.76s + 子进程税 ~12s + 其余零散 ≈ 41.4s（+ commit git ops + drift-watch 指纹快照 ×2）。
...
| 方案 | 内容 | 锁内时间预估 | 风险 |
|------|------|-------------|------|
| A（推荐） | 锁外预跑子集 + 锁内指纹采信，不一致回退全量重跑 | ~45-50s → **~5-8s**（-85%） | 中：白名单分类错误=漏检，需 §4 分级清单逐 gate 审批 + 红蓝测试 |
...
| RULING-REFERENCE 以下零散结构/内容 gate | — | 0.15-0.23 | （见 §8.3 JSON 全量） | 逐 gate 审批 |
...
| RULE-FOUR-WAY 以下快路径族（RULING-COMMIT-VERIFIED 109 / CAPABILITY-LOOKUP-REQUIRED 110 / GATE-PRECOMMIT-OFFLINE 111 / RECONCILER-FILE-OPS 117 / ISSUE-RESOLVED-INTEGRITY 130 / GATE-ERRCODE-CONSISTENCY 131 / META-TESTS-COVERAGE 99 / SCHEMA-FILE-EXISTS 121 / BUSINESS-REGISTRY 139 / GATE-BATTLE-MAP-ALIGNMENT 833 / GATE-PANORAMA-ALIGNMENT 830 0.181） | — | 0.000-0.181 | 信号/触发式 | 各自标注（快路径无优化价值） |
...
- 结果：总计 **41.378s**，明细见表格与 JSON；该 harness 可复用为 ⑥A2 的验收工具（改造前后逐 gate 耗时对比）

### 6.3 未做的事（合规边界）
- 未修改任何 tracked 文件；未获取真实全局锁；未做打点日志落码（check_all 无逐 gate 计时埋点——harness 已达成同目的取证，正式打点建议随 ⑥A2 一并设计，避免两次改动）
