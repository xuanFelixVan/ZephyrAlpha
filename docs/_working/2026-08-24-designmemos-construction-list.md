...
>
> **✅ 已完成（4 条，摘录）**
> - L92: - 来源文档：64_data_source_download_spec.md §16.2（Q18 已闭环，余三件）
> - L119: - 建议 MOD 号：按域顺延（factor 族待统筹 / MOD-SIG-086+ / MOD-RK-028+），派单前逐件 grep 核销已落地者。
> - L121: - 验收标准：逐件「已落地核销 or 落码 testing」二分闭环；90 号 P2 表逐项标注终态。
> - L183: - 验收标准：grep 复核确认未落后落码 testing；分层标签消费 BM-SEL-23-B 输出；22 号三件与既有 sector_* 口径一致（不重复造轮子，已落地则核销）。
>
> **⚠️ 未完成（13 条，逐条摘录）**
> - L5: # design_memos 待施工清单（2026-08-24，T3 审查产出）
> - L8: > 收录口径：**未施工 / 部分落地中的未落项**；已正式裁定暂缓且重评条件未触发、纯等实盘数据的校准项不列（在来源文档结案报告中已登记）。
> - L10: > 标注「待复核」者：08-19 复核后代码演进快，施工前须先 grep 实证当前状态再派单。
> - L16: > - 仍有效：#17 数据期挂账（读取器/评估器已就绪，纯等日历）/ #23/#24/#26 远期 / #25 PNG 退役待 Owner 确认 / #28 Owner 窗口四项；施工前须 grep 实证再派单。
> - L131: ### 14. 13 号 NLP Phase 5-8 + P2-E8（待复核后派单） ✅ 已核销（2026-08-30 实证在案）
> - L134: - 建议模块落点：`src/zephyr/nlp/` + `scripts/ml/`（sentiment_aggregator.py 已存在，Phase 5-8 链路完整度待复核）
> - L141: - 设计声明摘要：DSR 双实现统编（0.5 vs 0.95，待 Owner 裁定 SSoT）→ DSR 入 DecisionGate 判定链（随裁定）；四核心模块零单测（walk_forward/decision_gate/overfitting_detector/pit_manager）。
> - L171: ### 18. 24 号 DynamicCapacityCalculator + #10 PIT 回测框架全规格（待复核） ✅ 已核销（2026-08-30 实证在案）
> - L178: ### 19. 23 号情绪周期分层标签器（待复核）+ 22 号残余三小件（待复核） ✅ 已核销（2026-08-30 实证在案）
> - L185: ### 20. 26 号六因子矩阵待施工项（待复核） ✅ 已核销（2026-08-30 实证在案）
> - （另有 3 条，见正文）
>
...
### 1. RUN-05 断网断电断点恢复演练入 57 号日循环 SOP ✅ 已核销（2026-08-30 实证在案）
...
### 2. P1-E9 S2 评分算法重设计五子项（E9a~E9e） ✅ 已核销（2026-08-30 实证在案：五子项落码 + B4 EVT-2024 design_match 翻 true、13 号升 active）
...
### 3. 因子 IC 实证回填批（factor_registry 154 条） ✅ 已核销（2026-08-30 实证：IC 回填 code-anchored 4/4 + NL 143 条锚定裁定登记，.runtime/factor_ic_backfill/20260830_report.md）
...
### 4. 50 号五零件 experiment_tracking adapters ✅ 已核销（2026-08-30 实证在案）
...
### 5. Champion-Challenger mSPRT 晋升通道（前置：晋升载体裁定） ✅ 已核销（2026-08-30 实证在案）
...
### 6. 44 号残余两件：FCT-sentiment 12 条因子登记 + 外盘 8 标的采集接线 ✅ 已核销（2026-08-30 实证：FCT 12 条已入册 + 外盘 7/8 落库 kline_global，USDCNH 全源失效登记跳过）
...
### 8. 43 号盘中操纵实时检测（对敲/拉抬/洗售） ✅ 已核销（2026-08-30 实证在案）
...
### 9. 64 号 Q8/Q16/Q17 数据韧性三件 ✅ 已核销（2026-08-30 实证在案）
...
### 10. 55 号 §6 残余四件 ✅ 已核销（2026-08-30 实证在案）
...
### 11. 36 号 daily_auditor 集成包装层 ✅ 已核销（2026-08-30 实证在案）
...
### 12. 90 号 P2 十一项复核清偿专项 ✅ 已核销（2026-08-30 实证在案）
...
### 13. 63 号三波补文档施工 + 审查工具链 ✅ 已核销（2026-08-30 实证在案）
...
### 14. 13 号 NLP Phase 5-8 + P2-E8（待复核后派单） ✅ 已核销（2026-08-30 实证在案）
...
### 15. 52 号残余三件 ✅ 已核销（2026-08-30 实证在案）
...
### 16. 61 号其余生命周期残余（等触发类除外） ✅ 已核销（2026-08-30 实证在案）
...
### 18. 24 号 DynamicCapacityCalculator + #10 PIT 回测框架全规格（待复核） ✅ 已核销（2026-08-30 实证在案）
...
### 19. 23 号情绪周期分层标签器（待复核）+ 22 号残余三小件（待复核） ✅ 已核销（2026-08-30 实证在案）
...
### 20. 26 号六因子矩阵待施工项（待复核） ✅ 已核销（2026-08-30 实证在案）
...
### 21. 15 号 DQ_SPECS 八维 check_func 绑定 + Embargo 真日历切换核销（待复核） ✅ 已核销（2026-08-30 实证在案）
...
### 22. 17 号残余四项 ✅ 已核销（2026-08-30 实证在案）
...
### 27. 66 号 task_board 死信标签联动 + 00_index 版本同步（治理小件打包） ✅ 已核销（2026-08-30 实证在案）
