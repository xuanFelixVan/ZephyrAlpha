...
>
> **⚠️ 未完成（2 条，逐条摘录）**
> - L187: ### 9.3 处置（破坏性操作留 Owner 门位）
> - L194: 3. 关联修正待办：pit_query research_report 锚列参数化（D3）追加"EPS 槽位禁消费"标注；
>
...
3. 关联修正待办：pit_query research_report 锚列参数化（D3）追加"EPS 槽位禁消费"标注；
   data_asset_registry DS-228/229 补 PIT 限制标注。
