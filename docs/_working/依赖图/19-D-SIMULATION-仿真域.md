...
> ⚠️ **架构决策更新**（2026-07-02）：回测已从D-FACTOR/D-SIGNAL分离，独立为D_BACKTEST域（见[32-D-BACKTEST-回测引擎域.md](32-D-BACKTEST-回测引擎域.md)）。D-SIMULATION仅保留仿真（what-if）职责。
...
| 数据 | 历史数据 | 生成数据+历史数据 |
| 方法 | 重放 | What-if分析 |
| 归属 | **D_BACKTEST**（已独立，16/16 production） | D-SIMULATION（未启动） |
