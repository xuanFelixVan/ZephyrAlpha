...
> **一句话**: 把市场数据接进来
>
> **内容分层标注**：`[全景]`=全景依赖图颗粒度（域/子域/模块关系结构）→写入depgraph v3；`[蓝图]`=蓝图施工细节（具体实现方案/代码/配置）→写入模块蓝图；`[混合]`=两者兼有，需拆分
...
> ID/名称/职责/优先级/建设状态 → [全景]；门禁/对标/合并自 → [蓝图]
