...
### §7.3 市场状态→仓位上限映射 🆕v8.1 (2026-08-02 裁定: 12态对齐D-SIGNAL-04)

**裁定**: 12种Regime = 9基础网格(3×3, 趋势方向×波动率) + 3特殊态(CRISIS/RECOVERY/BREAKOUT),
与 D-SIGNAL-04 定义方对齐(原v8.0的11态中⑩事件驱动/⑪板块轮动为正交overlay维度,
改为 RegimeSnapshot 标志位 `is_event_driven`/`is_sector_rotation`, 不再占enum槽位——
事件可叠加在任意基础态上, 正交维度不该挤进同一enum)。
...
overlay 标志位(正交修饰, 不占enum):
| 标志 | 效果 | 消费方 |
|------|------|--------|
| is_event_driven | 基础仓位×70% | POS-001 (total_cap×0.70) |
| is_sector_rotation | 行业集中度放宽至±15% | POS-010 (POS-001不改cap, 仅透传) |

> 状态→仓位映射为immutable(不可AI修改), 调整需Trader审批
> 样本量顾虑归 D-SIGNAL-04(估计方) + D-SIGNAL-65(83/220天守门), 非消费方职责

#### §7.3.1 样本量论证 (为何 12 态不违反"种类别太多"原则) 🆕v8.1

**顾虑来源**: 早期讨论中确认"市场状态种类太多 → 每种状态样本率不够, 参数估计不可靠"。
本裁定通过 4 个设计动作正面回应, 12 态方案反而比 v8.0 旧 11 态(事件驱动/板块轮动进 enum)
更省样本:

**① overlay 标志位不进 enum (核心样本保护)**
若 v8.0 旧方案将事件驱动/板块轮动当作 enum 第⑩⑪态, 状态叠加会产生
"事件驱动+平稳牛市" vs "事件驱动+阴跌" 等独立场景, 理论上需拆成
11×2×2=44 种组合, 样本量直接崩盘。改为 `is_event_driven`/`is_sector_rotation`
两个正交布尔标志位后: enum 保持 12 态不变, 样本仍按 12 个基础态统计,
overlay 只对基础仓位做乘法修饰(×0.70), 不需为每种组合单独积累样本。

**② 9 基础网格是业界标准 3×3 分类**
趋势方向(牛/震荡/熊) × 波动率(低/中/高), 与 Bridgewater/AQR 等
机构 regime 切分法一致。A 股 5 年日线(~1200 交易日)平均每格 130+ 样本,
足够估计波动率/趋势参数。
...
| 特殊态 | 样本需求 | 原因 |
|--------|---------|------|
| ⑩CRISIS | 几乎不需要 | 硬编码 5% 上限+"仅减仓不开新", 不依赖参数估计 |
| ⑪RECOVERY | 低 | 50% 固定上限+逐步重建, 规则驱动非统计驱动 |
| ⑫BREAKOUT | 低 | 70% 固定上限, 趋势确立即加仓, 判定靠价格突破非分布拟合 |

只有 9 基础网格态"吃样本", 而 3×3 是标准分类, 样本充足。

**④ 样本量守门有专责模块 D-SIGNAL-65**
样本量是否够不是 POS-001(消费方)职责。D-SIGNAL-65 专责守门:
每种 regime 要求最少 83 天(参数估计)/220 天(尾部风险)样本,
不够则降级或合并相邻态。职责分离——POS-001 只管"给我什么 regime 查什么 cap"。

> **防遗忘锚点**: 本论证随裁定落入真源(2026-08-02)。未来若有人质疑
> "12 态是否违反样本量原则", 须先读本节确认 overlay 设计已化解该顾虑,
> 禁止凭"种类多=样本少"直觉回退到更少状态数(回退需重过 D-SIGNAL-04 裁定)。
