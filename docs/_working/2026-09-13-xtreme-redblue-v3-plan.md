...
>
> **✅ 已完成（3 条，摘录）**
> - L11: > 上一轮（v2，2026-09-13 白天）已验证：拆分协调协议 / TRACKED-DRIFT 降级 /
> - L275: # 假设 S1.3 的 f_stage.md 已提交成功；现在直接改它再提交（同一 session）
> - L489: # 6.3 lab 文件处理：已入库的 xt_lab3 文件整批删除并走网关提交删除
>
> **⚠️ 未完成（2 条，逐条摘录）**
> - L29: 5. **卡死就跳**。任何命令挂起超过 2 分钟：Ctrl+C（或让系统停掉它），把"挂起"本身
> - L30: 记为一条发现，跳到下一场景。挂起也是缺陷线索。
>
> **核验方式**：全文扫描完成/待办信号 + 引用文件存在性核验（引用 17 个，其中判废弃 4、路径漂移 0）+ commit 提及 5 处。
> **判废弃引用（C-1）**：`docs/_working/xt_lab3/f_bypass.md`；`docs/_working/xt_lab3/f_lock.md`；`docs/_working/xt_lab3/f_para_1.md`；`docs/_working/xt_lab3/sb_1.md`
>
