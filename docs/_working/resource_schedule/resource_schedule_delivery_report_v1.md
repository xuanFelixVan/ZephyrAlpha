...
- 状态: D 在飞保留（生产数据面+战役文档混合区）——初判 A 有误已即时勘正
- 依据: config/resource_profile_registry.yaml 33 处+src 采样器/晨报生成器/schedule_overview 等生产代码功能性引用本目录为写入面（W8-0 原台账本列 D 类在飞）；战役交付文档部分已结案（见下）
- 归档/留场: docs/_working/resource_schedule/（留场；文档部分待其 owning 线把生产写入面迁往 data/ 或 var/ 后再拆分归档）
- 备注: 排班/排期二分口径已由裁定#388 收编（术语册）；本块由 st-workclean-20260921 二次修订（改链批误扩回滚勘正）
