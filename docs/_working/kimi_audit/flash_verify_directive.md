...
| V10 | **长任务编排与补跑** | 需要小时级回测/重考/聚合的项。**注意：Kimi 白天会自己后台启动一批实验（等待不耗它的 token），实验台账在 `docs/_working/kimi_audit/experiments_ledger.md`——已登记的别重复跑**，你负责它没覆盖的批量面、可并行的大面、以及明早必须出证的项 | 夜间无人值守任务登记（`data/runtime/process_reaper_keep.txt` 防误杀）+ 产物落 `data/backtest_artifacts/runs/`；每条实验完成回填台账一行 | 轻 |
