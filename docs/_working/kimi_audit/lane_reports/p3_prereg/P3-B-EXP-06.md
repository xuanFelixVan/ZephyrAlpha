...
title: P3 预注册卡 P3-B-EXP-06（FCT-EXP-006 评级动量 点火考试，frozen）
...
n_eff: 6
parent_context: p3_prereg/P3-B-NARROWING.md（窄化裁定） + factor_registry.yaml FCT-EXP-006 + expectation_consumption_design_policy.md §8/§9
---

# P3-B-EXP-06：FCT-EXP-006 评级动量 的毛边际/DSR/经济解释三关点火考试

## 1. 假设（H1，唯一）

**按 `PROXY 单腿：rating_score_mean_t − rating_score_mean_{t−60td}（值大→多）` 构造的日频因子，月末 Top50 等权多头口径下有正且过成本线的毛边际，
且统计上非运气（DSR>0.5，N_eff=6）。**

零假设 H0：OOS 段日均毛超额 ≤ 2.7bp/日，或 DSR ≤ 0.5。

- 机制一句话（关③自述）：评级均值上移=机构观点动量，买入评级组合有超额（Jegadeesh 2002；华泰改进评级 RankIC 2.26%）。
- 注册表原式与实现注意：PROXY 披露：registry 原式含『窗口内上调事件计数』腿，报告级事件不可得，降级为评分变化单腿；值域实测 1-7（7/5/3/2/1 制）。
