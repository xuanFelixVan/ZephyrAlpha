...
| H4-B | ✅ 本轮补完（此前半落地） | **坑**：`_collect_timeseries` 造的 `cash_curve` 只随内存 `ts` 返回，`sink_backtest_result` 只收 equity/trade/drawdown/benchmark → 落盘 JSON **无现金腿**（端到端探针实测 `persisted cash_curve points: 0`）。旧测试只断 `ts["cash_curve"]`，故对此回归全盲——正是"验收断内存对象、产物却丢字段"的同族自欺（见 `gate_self_deception_mining.md`）。修法：`metrics["cash_curve"]` 落盘（`BacktestRunArtifact` 顶层受 `[MODIFY-GUARD]` 结构冻结，不加键），测试 `test_artifact_metrics_carry_chain_evidence` 升级为**读盘断言**（落盘==内存、与 `equity_curve` 等长、点结构含 timestamp/cash）。<br>**残余 R-H4B-p**：`positions_curve` 未加——引擎无逐日持仓快照序列（`Portfolio` 只有 `nav_series`/`cash_history`/`trades_log`），市值只能由 `equity−cash` 反推，故"任一时点 `cash+Σqty×px≈equity` 独立复算"仍需真独立量（要引擎侧新增逐日持仓快照），非产物侧可补<br>**残余 R-H4B-s（同族第二生产路径）＝✅ 已清偿 `ce14e814`**：单策略回测走 `scripts/run_backtest.py:_collect_timeseries`（旧 docstring 自述"四条时序"，连 `cash_curve` 都不构造 ⇒ 该路径产物无现金腿）。现已与整装路径同口径：构造 `cash_curve`（丢建仓前 `None` 快照行）+ 新增 CLI 侧 `_PERSISTED_VIA_METRICS_TS_KEYS` 名册经 `artifact.metrics` 落盘；GT-15 结构差集闸同步扩到这条第二生产路径（`test_cli_backtest_artifact_carries_cash_leg`：逐日等值 + 与 `equity_curve` 等长 + 名册在册三判据）。<br>**已加结构闸防再犯**：`framework_composer._PERSISTED_VIA_METRICS_TS_KEYS` 登记表（落盘键由它推导，登记与落盘无法漂移）+ 测试断 `_collect_timeseries` 每个返回键 ∈ `sink_backtest_result` 形参 ∪ 登记表，新增"只造不落"的序列当天即红 |
| H4-C | ✅ 通道已建 + 语义划界 | 持现金意图经 `ShrinkageBacktestEngine`（`config.shrinkage_by_date`，裁定#270）；`vectorized_engine.py` 四处明示"全零行=当日不下单≠清仓"（`:451`/`:473` 属 docstring，`:93` 是 `EXECUTION_MODEL_CAPABILITY` 字典条目，`:575` 是行内注释——四处均在、措辞一致，但只有两处是 docstring，原记"四处 docstring"归类不实，已修正），`target_weight_renormalization.rows_all_zero` 在产物侧计数披露 |
