...
- S05-G1 未修（核验 factory_intake_pipeline.py:212 仍 `--created-by st-facbe-20260914` 硬编码）——09-19 周六首跑前若不修，token 将继续记到死会话名下，**提请列入下批必做**。
- G2-G5 未动，登记维持；`--seeds` 仍为 5（run_factory_lane_c.ps1:37）。
