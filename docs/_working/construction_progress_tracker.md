...
| 第 4 批 | 34 RegimeMeta / 60 跨切（骨架需重建）/ 43 合规 | ⏳ 待开工（治理批+文档压缩批均已完工 merge，阻塞解除） |
...
| 55 | 【已闭环】65 号 Phase 1 wrapper 层全部 7 项施工落地 | AI-GIT-001 第二批（2026-08-14 晚，用户授权"遗留+P1/P2 全处理"） | scripts/git_safety_wrapper.ps1（唯一真源）+ install_git_safety_wrapper.ps1（§7.7 幂等安装）+ RULE-GIT-SAFE 写入 AGENTS.md/.trae/rules（§7.2）+ d6 三 hook 接入 pre-commit（§7.13）+ Session ID（§7.32）——commits 611227d5/21f447c1，40 验收测试两轮全绿。PS5.1 实证修正 memo 三处假设：①Alias>Function（AllScope 别名需 Remove-Item Alias: 删除，Set-Alias/function 覆盖均无效）②裸 `--` 被吞（checkout 路径/分支区分改 rev-parse 校验）③Add-Content UTF8 带 BOM 坏 JSONL 首行（改 AppendAllText 无 BOM）。偏差登记：§7.1.4 ProxyCommand 未采纳；.git 阻断挂删除类。**待办：merge 后跑 install_git_safety_wrapper.ps1 激活** | ✅ 施工闭环；⏳ 激活待 merge 后执行 |
| 56 | 【新事故机制·已修复】CLI session_worktree create 不做 SessionRegistry 活性登记——治理层 sweep/base_sync 误判死 session 残留 | AI-GIT-001 实证（2026-08-14 深夜，wrapper 施工期间三文件两度被抹） | 病根：scripts/session_worktree.py（CLI 入口）create 只备三件套，不注册 SessionRegistry/不 spawn heartbeat daemon（rule_bridge 的 session_worktree_start 才有 daemon）——CLI 创建的 worktree 在治理层=无注册无心跳死残留，GATE-WORKTREE-LIFECYCLE sweep（post-commit 触发）+ base_sync（无 session commit→git reset --hard 主仓 HEAD，reflog 铁证）反复抹除未提交工作（staged A 文件随 index 重置消失，git fsck dangling blob 找回）。与 #53 rogue worker 删除链同族。**已修复**：d7844786 create 补 SessionRegistry.register（锚主仓根 --git-common-dir，失败不阻断）；登记后网关 commit/claim 自动续期心跳（TTL 3600s）。**残留风险登记**：①sweep 对 >24h 死 worktree 的 force-clean 未走四证 SOP（仅有 quarantine ref 72h 窗）——建议下轮治理批将 sweep 清理动作接入四证；②长时间不 commit 的会话 TTL 到期仍有误杀面（heartbeat daemon 仅 rule_bridge start 路径有） | ✅ 机制修复闭环（d7844786）；⏳ sweep 四证接入 + daemon 普及留治理排期 |
| 57 | 65 号 Phase 2 项 8（lock_files.py TTL 五命令+§7.28 Mutex 原子写）+ 66 号 MVP（commit_queue/plumbing 扩展/wrapper 安装已在 65 覆盖） | 65/66 memo 既定范围 | 66 memo §2.4 #16：git_guard 缺 read-tree/update-index/write-tree/hash-object 4 个 plumbing 命令拦截（66 事故 6 根因）；lock_files 无 TTL/Mutex（26 会话并发 RMW 必丢锁） | ⏳ 待排期（下轮治理批承接；wrapper 层落地后 plumbing 可在 shell 函数层扩展覆盖） |
