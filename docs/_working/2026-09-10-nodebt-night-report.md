...
>
> **✅ 已完成（6 条，摘录）**
> - L57: 【问题】tdm.js/tdm.html/decision_map.py/回测协议备忘录工作区是陈旧回退（缺已提交的 tdm-count 实时计数、decision_map 97 行门禁、D118-D122 PIT 终裁）。
> - L58: 【分析过程】直接编辑+提交=revert 他人已提交工作（#ARCH-308 判读器定性 stale_rollback）；裸 git checkout -- 被禁；safe_write_text 是热文件合规写通道。
> - L77: 【分析过程与结果】详见 §三——①衰减巡检=验证批尾随事件（已施工+测试）；②"关风控回放开关"是伪命题（DecisionGate 是策略晋升闸不在成交路径），改为信号消融对照器，随 X 流验证批施工；③trade_log 不加 node_id（语义错），加 decision_price+order
> - L82: - **三项裁定全部放行**（"这些都没有问题，都可以执行"）：①衰减巡检尾随事件已落地；②信号消融对照器、③decision_price+order_type 两字段——按晨报裁定的时机（X 流验证批启动时）执行。
> - L86: > Owner 惯例推回：能裁的 AI 自己裁，留给 Owner 的只有执行放行。三条裁定如下，①已施工完毕。
> - L88: ### 裁定 ①衰减巡检挂载（已施工+测试+提交）
>
> **⚠️ 未完成（2 条，逐条摘录）**
> - L25: | P2-3 | 反事实对照 | **登记遗留**（真源 §8.3 授权"按需可延"） | exit_counterfactual 方法学已在 P1-1 落好（开关未建前保持 pending） | — |
> - L103: ## 四、未完成项与原因（提交状态）
>
> **核验方式**：全文扫描完成/待办信号 + 引用文件存在性核验（引用 2 个，其中判废弃 0、路径漂移 0）+ commit 提及 19 处。
>
...
## 二·五、Owner 晨间验收与放行（2026-09-10）

- **前端验收 ✅**：Owner 重启面板后亲验 tdm 页抽屉「验证档案」区正常显示（ACC-F-TDM-DRAWER 条目 10 目检通过）。
- **三项裁定全部放行**（"这些都没有问题，都可以执行"）：①衰减巡检尾随事件已落地；②信号消融对照器、③decision_price+order_type 两字段——按晨报裁定的时机（X 流验证批启动时）执行。
