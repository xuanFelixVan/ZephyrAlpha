...
1. **P2-1 衰减巡检调度挂载二选一**：①DataScheduler 加日频作业（ schedules 配置一行，cron 语义）；②trigger_router 新增 backtest_completed 触发器（事件驱动正道，但路由表=Human-Gated 不可自改）。核心逻辑已落盘（run_decay_check 无状态批函数），只差一行挂载。
2. **P2-3 反事实对照**：回测引擎"关风控回放"开关（PB-11 中等工作量）。exit_counterfactual 方法学已落好，开关建成前 X 流节点验证保持 pending。
3. **执行流水节点归因**：trade_log 加 order_type/node 归因字段（引擎侧），L4 子节点才能逐节点真验证（当前全量统计口径）。
