...
| 事件 | 维度 | 治理前 | 治理后（路A已接线/capi+three_yang 仍 legacy） |
|---|---|---|---|
| EVT-2015（09-15） | capitulation / valuation / three_yang | 0 / 0 / 0 | 0 / **0** / 0 |
| EVT-2020（04-10） | 同上 | 0 / 0 / 0 | 0 / **0** / 0 |
| EVT-2024（09-24） | 同上 | 0 / 0 / 0 | 0 / **60 ✓（过 confirm≥40）** / 0 |
...
- capitulation/three_yang 列仍为 legacy 恒 0 是**预期态**（生产接线未切换，待本报告 §七 Owner 裁定后由统筹执行）。

## 七、结论与待裁定项

**算法层结论**：恒 0 锁死已解开（2020 事件峰值 40.2、2024 事件 90），推荐组合四项门槛过三项（WFE/fp/参数平移），MC 显著性未过（p=0.87），按 14 号 §4.5 纪律属**低置信部分通过**。

**待 Owner 裁定（三选一）**：
1. **接受低置信接线（推荐）**：builder 切换推荐组合 + three_yang v2_index；B4 验收 2020/2024 两事件翻 true（2015 因簇早于事件日 3 周+trigger halflife=10 衰减仍 0，按 14 号开放问题 3 另行裁定事件标注/阶段期望）；MC p 未过写入 13 号 §3.5.4 风险注记，DSR N=17 备案。
2. **扩候选再验证**：触发侧补"长 halflife（20/30）变体"与"事件日可达性加权重"再跑一轮（DSR 预算增至 ~30）；代价是 2015 场景或可覆盖但过拟合风险上升。
3. **维持 legacy 不接线**：保持现状恒 0，S2 三事件继续 design_match=false（不推荐——修复已被实证严格优于现状且误报率 0.8%）。
