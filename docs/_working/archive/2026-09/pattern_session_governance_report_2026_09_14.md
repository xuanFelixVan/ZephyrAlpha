...
>
> **✅ 已完成（1 条，摘录）**
> - L136: ## 六、applicable_series 标注批（已完成，2026-09-14）
>
> **⚠️ 未完成**：无待办信号
>
> **核验方式**：全文扫描完成/待办信号 + 引用文件存在性核验（引用 5 个，其中判废弃 0、路径漂移 0）+ commit 提及 11 处。
>
...
## 七、弱证据族 regime 细看与裁定（st-pattern-20260914，2026-09-14）

数据：market_pattern_win_rate 全 regime 切片（w10/w20，n≥1000）。
裁定：**头肩/三重族标记为"弱证据观察族"**——

- 头肩顶 w10：整体 51.7%（基线 53.4%）；最好切片 r3=56.3% ≈ 基线 56.8%（打平不赢）。
- 头肩底 w10：整体 46.2%（基线 48.5%）；最好 r2=51.0%（+2.6pts），最差 r4=40.0%（-4.9pts）。
- w20 更弱（头肩底各 regime 全部 <51.4%）。
- 处置：检测照跑、事件照记、evidence 如实在册；**不给信号绿灯**（provider 返回
  真实低胜率，映射器加权自动降权）；**调参候选挂观察项**：swing_window 2→3~5、
  肩对称容差收紧，下一回测批对比后再定，不盲调。
- 对照：强信号族=塔形顶/底（69.6%/65.6%）、一字跌停板（67.9%）、三空（64.9%）
  ——A股特色族证据显著强于国际经典族（与 37 万检测实证及 JFQA regime 依赖一致）。
- EOC3 原书核对：thepatternsite 超时+Wiley 403（限流窗口），§四候选清单维持
  "待核对"，下次网络窗口重试。
