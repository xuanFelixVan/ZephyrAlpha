...
> Q10 收口记录（外审修复 2026-09-07）：情绪通道拆分已事实实施——S2 市场内部结构与 S3 赚钱效应为两个独立传感器节点。
...
### 2.7 列轴定稿与灰度判定（v1.2.0，Owner 2026-09-05 裁定）（D7-D9，外审修复 2026-09-07 补标 G-11③）
...
### 2.8 L1 预算带与灰度仓位（v1.2.1，Owner 2026-09-05 裁定）（D10-D12，外审修复 2026-09-07 补标 G-11③）
...
### 2.9 真源对齐裁定：B 方案（六段定稿）+ 已有资产引用（v1.2.2，Owner 2026-09-05 裁定）（D13-D15，外审修复 2026-09-07 补标 G-11③）
...
**SOP 沉淀**：本轮撞车教训固化为《交易决策地图逐层讨论 SOP》（docs/01_policies_and_standards/sop/trading_decision_map_sop/trading_decision_map_layering_policy.md）——已有资产清单先行+四路外部调研+上层完整性检查+枝干分级检查。
...
> 计数口径说明（外审修复 2026-09-07，m-33）：E 流实测 74 节点（cn_a，不含 crypto 空壳 4），总 122；历史小计 64/78 为不同施工时点快照，以真源 YAML 实测为准。
...
5. **次日竞价即决协议**（随 X 流 S1 消化，接口先登记）：隔夜仓去留 9:15-9:30 竞价判（高开 3-5%+竞价量≥昨涨停量 10-15%→持有观察 15 分钟；平低开+量萎缩→竞价即出；超短仓 10 点前未走强一律离场）【D67 纠错：量能基准修正为"昨日总成交量 5-10%"（社区共识三口径），完整决策表见 §2.30】
...
- 做T（P2-01/02 注释）：时段准入矩阵（9:30-9:40 禁开新T 开盘价差最宽/9:50-10:10 sell-first 黄金窗/10:20-11:00 胜率最高段/14:40 后只平不开/一天最多 2 笔）；双轨止损（价差 0.3-0.5%+时间止损）+连败 2 笔当日熔断+sell-first 未接回尾盘强制复原（禁 T+0 变 T+1）；T量≤一档深度 50%/单日≤1-5% ADV；量能<5日均量 0.8 倍禁T；主策略减仓信号优先于做T overlay
...
- 组合结构：持仓角色分层（底仓/进攻/机动/备用+层间资金禁互相救援"不补底仓亏损"）；板块梯队标签（龙头/跟风角色，跟风止损严于龙头，板块≥2 天地板全梯队降仓（D99 终裁 2026-09-07：板块家数规则整体废除改个股级，实证证伪 c=2 组 T+5 +0.97% 与基线无差异））；组合加仓预算池（多票同时加仓总闸，池空转报备待批）；持仓只数上限=情绪周期函数（试错期多标的分散/主升期集中 1-2 只/退潮 0 只）；账龄字段+超期升级
...
### 2.30 X 流第二轮审计：竞价即决落地+确认层+流动性感知+执行工程（D67-D73，Owner 2026-09-07 终裁）

**背景**：X 流血肉落盘后 Owner 再指令三路深查（确认机制与去抖/流动性感知/特殊场景与卖出后清理+竞价即决落地细化）。三路并行产出约 30 件新发现（与第一轮零重叠），含 1 条必改正确性缺陷（D68）与 1 个遗留接口正式落地（D67）。Owner 裁定"按建议执行"。

**D67 竞价即决协议完整决策表（真源=本节；修正 §2.28 D55-5 骨架"昨涨停量 10-15%"旧口径）**：
- **盘前固化基准**（T-1 收盘后）：昨收/涨停价、昨日总量、昨日竞价量、5 日均量、封板时间→次日预期高开映射（9:30-10:00 秒板→5-9%／10:00-11:00→3-6%／13:00-14:00→0-3%／14:00 后烂板→0 至 -3%）、昨日量能形态（缩量加速板→预期高开 7%+；放量分歧板→2-4%；巨量烂板→低开 -5% 以下）
- **竞价量能三口径**（主判据=占昨日总量）：5-10% 健康／<3% 缩量（诱多高发）／>15% 爆量（看承接）；辅口径=量比≥3 倍（>5 确认，熊市>8）+超前 5 日竞价均值 10%；回测锚：烂板+高开 2-5%+量比>5+竞价额>1000 万=弱转强，428 笔胜率 58.6% 盈亏比 3.2:1
- **溢价×量能决策矩阵**（9:25 定格 30 秒内出动作）：≥+9% 一字位=持有（开盘 3 秒封单骤减即离场）／+5~9% 爆量=竞价减半仓+10 分钟不封板全清／+5~7% 温和放量=持有观察 15 分钟（+6% 减半锁利/破 +3% 离场）／+3~5% 放量=健康区（高开低走破开盘价卖）／+3~5% 缩量=诱多高发区（竞价即出）／+1~3%=30 分钟窗口无放量上攻走人／平开=9:45 未拉升卖／-2~0 放量翻红=可留观察／-5~-2% 缩量=竞价即出或 9:38 前不翻红无条件卖／<-5% 核按钮=竞价直接挂跌停价卖出（按开盘价撮合非跌停价，未成开盘第一波反弹高点卖）
- **9:20-9:25 不可撤段挂单策略**：9:15-9:20 可撤段只观察（大单 90% 诱多诱空）；决定离场→9:20-9:25 直接挂跌停价排队；决定持有→不挂单等开盘 3 秒承接确认
- **"10 点前离场"例外清单**（6 条）：一字板/缩量加速板封单稳固／市场总龙头多给 5-10 分钟／情绪极热+板块批量涨停绝对龙头放宽至 10:00／龙头首次分歧爆量红盘=留仓（连续加速后爆量高开=顶部不留）／弱转强确认（烂板+高开 3%+首笔成交额>1000 万）按买入逻辑重评／除权除息日溢价率分母改除权参考价（D68）

**D68 除权除息日卖出口径修正（必改，正确性）**：除息日 K 线向下缺口=价格规则调整非下跌——溢价率/跳空判定分母改用**除权除息参考价**（参考价=(登记日收盘−红利)÷(1+送转比例)），否则除息日健康持仓被误判核按钮低开 -5% 触发清仓；登记日收盘后卖出仍享分红；红利税跨线：X 流情绪短线仓一律不等税线（每股分红几分钱 vs 日内波动可忽略），仅红利/波段仓（分红率>2%）卖出确认时提示"距税线 N 天+预估节税额"。

**D69 确认层三段式+分桶确认模式**：S1-05 加 triggered→confirmed→released 三段语义（加权分达阈≠允许卖）；分桶确认模式定稿——risk/time/volatility 桶=**immediate 不辩论**（Qullamaggie："decisive close below VWAP = dead trade, exit immediately"）、signal/target/trailing 桶=**close_confirm**（收盘确认非影线穿越，约 30-50% 突破是假的）；去抖三参数 confirm_bars∈{0,1}/buffer_pct 0.2-0.5%/量能≥20 日均量 1.5 倍（NSE 实证单此条过滤 65% 亏损交易）；**去抖与紧迫度互斥——S1-06 强制清仓绕过确认层**（防去抖拖住救命腿）；时段可靠性系数——早盘 9:30-10:00 假信号高发窗（北大光华：开盘 5 分钟波动为全天 4 倍）确认从严，尾盘从宽。

**D70 流动性感知卖出包**：前置三检查（单笔量≤买一档挂单量／当日量×参与率 10% 硬顶／持仓市值÷20 日均成交额>1% 禁一次性市价——冲击公式 Impact≈σ×√(Q/V)×I）；**IS 急卖 preset**（紧迫度 0.8-1.0 映射高 λ 前重后轻：首 1/3 时间窗完成≥50%，价格漂移不利参与率上调、有利减速；λ→∞ 等价一次性扫单）；ICEBERG 随机化（显示量 ±10-20%/释放间隔 0.5-2s 抖动/显示量占一档量封顶——防冰山猎人识别，实测降冲击 40-70%）；流动性枯竭检测（点差突宽/深度秒撤→挂低价等而非市价砸，薄市 stop 单级联放大跌幅）。

**D71 执行工程四件套+特殊标的**：
- **SellChase 状态机**（S2-04 注释）：触发后不成交处置——挂单→计时→撤→追 1 tick 重挂→预算 15-20 轮（60s）耗尽转最优五档即成剩撤市价兜底；内嵌 Escape 检查（本方前 N 档量<对面 X%→撤单避损防挂单被扫）
- **QMT 工程三坑**（联动 93 号 playbook）：成交回报重连全量重推须 business_id 幂等去重（柜台流水号重推可能不变，回调内无法过滤）/市价单 price 字段=保护限价（卖出给足空间 ×97-98% 与笼子取交集，否则快速跳动即废单）/on_cancel_error 兜底+回调内禁同步下单+tick 超 3s 丢弃
- **队列工程**：多持仓卖出队列每 tick 重排（非信号时一次性排序）+停牌/临停标的自动顺延；**板块联动卖出顺序=跟风→补涨→中军→龙头**（退潮跌幅跟风 40-60%>中军 25-40%，跟风见顶早龙头半天；龙头留 IS 慢速通道；龙头断板=全板块紧急升级）；紧急卖出前 5× 深度检验（最优两档累计量≥5×委托量才可市价，否则激进限价 chase）
- **特殊标的分支**（S2-02 注释）：新股首日卖出状态机（分板块：首日仅限价单市价废单/双创北交所 ±30%/±60% 临停**基准=当日开盘价非发行价**/主板竞价限发行价±20%/中签仓首日可卖当日买入仓不可卖/社区分层开盘 4 成冲高 4 成留 2 成）；临停复牌子状态机（停牌期可挂可撤/复牌竞价限停牌前成交价±10%/预埋限价规避跳水/北交所只能撤）；风险警示板（ST 主板 5% 双创 20%/0.01 元低价股跌停仅 1 分钱秒级排队/强制限价/个人只能卖不能买）；碎股整单卖出拆分废单+科创板 200 股基准

**D72 卖出纪律闸门+复盘结构化**：
- **卖出冷却期**：同标的卖出后≥2 交易日禁追高买回；大盈利（>20%）清仓后≥1 个月（实证：3 日内买回→后续两周平均回撤 11.7%/间隔 3 个月以上→盈利保持率 68.4%）；回补门槛=双逻辑+小仓 20-30% 试错
- **参数漂移锁**：一次卖飞/过山车禁改任何参数（Chandelier M 值/止盈线/时间止损），调整必须走 42号§3.11 A/B 显著性 p<0.05；冷却期 2 日内禁提交参数变更
- **恐慌卖出拦截**：大幅低开（<-3%）触发开盘 30 分钟等待窗口，窗口内禁市价卖出（KillSwitch/黑天鹅/跌停封死三类除外绕过）——与 P 流跳空统计底座衔接（P 流出概率 X 流出闸门，10:00 后按跳空分档预案执行）
- **卖出理由六分类封闭枚举**：STOP_LOSS/TAKE_PROFIT/TIME_STOP/LOGIC_FAIL/REPLACE/**PANIC**（每笔必填禁自由文本）；**PANIC 健康占比应为 0，滚动>10%=纪律失效告警**（禁先调参数，先查闸门）；理由×结果交叉统计（止损类卖后大涨=止损过紧喂 G04/置换类原标的跑赢=排序偏差喂 BM-SELL-05）
- **信号 N 日前瞻跟踪分桶归因**：每条信号持久化{桶ID/触发价/触发时刻/实际卖价}，T+5 回填未卖前瞻收益，任一桶 30 日避损率斜率降 30%→该桶紧迫度权重自动降档（社区反例：总体胜率 20.7% 靠分桶才发现方向系统性反向，反转后 79.3%）
...
> 已裁定（D95，2026-09-07）：竞价段豁免 30 分钟恐慌窗口；D72 豁免清单扩至与 S1-06 四触发完全对齐。
...
### 2.32 F 流第二轮审计：切账纪律+运行时五件+升降级实操（D84-D89，Owner 2026-09-07 终裁）

**背景**：F 流落盘后 Owner 再指令三路深查（聚合运行时与资金核算/升降级调权实操/归因扩展与极端情况），产出约 26 件新发现（与第一轮零重叠），含 3 件正确性级必改。Owner 裁定"按建议执行"。

**D84 正确性三件必改**：
1. **T+1 隔夜漂移切账**（最关键）：纸面组合从 decision price（信号日收盘）起算策略 alpha，隔夜漂移单列 delay cost **不进 PerfScore 不进执行考核**（Perold/Kissell IS 框架）——不切账则信号好执行差、信号差运气好全部混账，月度升降级评审全部失真
2. **单 sleeve 回撤协议**（组合未到 KillSwitch 线的中间地带，Millennium 5%/7.5% 事实标准）：sleeve 回撤 5%→权重减半/7.5%→冻结只减不加/冻结后连续两个月度 verdict 不达标→清退归零；sleeve 线自动执行、组合线全停，分层不冲突
3. **对账 suspense 桶+fail-safe**：break 未清标的进未归因桶（house account 不参与策略考核）；中签仓独立打新 sleeve；手工单次日强制补标签否则永久 house account；**数据质量门未过→当月不调权维持现状记"未归因月份"**（禁部分数据粗调——错配诊断会指向错误 sleeve）

> 已裁定（D96，2026-09-07）：档内自动动作=减半/冻结/降权/停新开仓（可逆减动作）；清退归零=系统自动产出评审提案+Owner 签字执行。

**D85 C2 运行时五件（缺省即坑）**：①聚合批量窗口（1 分钟 bar 收盘或 5 分钟一拍批量净额防 thrash，风险事件走即时通道）；②intent TTL≤信号半衰期（过期作废不重发等下一拍重评）；③净指令下发前资金状态校验（本地可用/可取/冻结三态模拟账本镜像券商；先卖后买排序；额度不足只削买入；与现金 sleeve 逆回购互斥）→C2-02 注释；④re-netting 循环（残余进下一批次不即时追单，偏差<0.5% NAV 不补、补单上限 2 次）；⑤强平插队三语义（先撤全部挂单后平仓/三态 kill switch ACTIVE-HALTED-REDUCING/exit 绕过削减性闸门仍过价格保护）→C2-01 注释。

**D86 升降级调权实操六件（细节补）**：①composite 公式落地（60+120 日双窗口对自身历史分布分位/阈值打分——8 sleeve 同伴分位无意义；0.05 硬线 2 连月；三类一票否决：信号 3σ 衰减隔离 5 日/回撤期否决加仓/残差 CUSUM 红）；②观察名单（非业绩触发也进观察——参数大改/数据源/通道变更；**半仓缓刑 3 个月**收集证据；恢复=50+ 笔 forward+PF>1.1-1.2；观察期不得加仓）；③滞回带（恢复线高于降权线一档+5 交易日锁定期+单次±10pp+REDUCED 跟踪仓/paper 双轨重启）；④过渡期（no-trade band 相对 25%/频率≤每周/>10pp 分日过渡/成本>改善 50% 缓调；三层仲裁——KillSwitch>资格约束>调权，**减权立即执行加仓等资格**）；⑤小样本防线（3 月<15 笔=数据不足沿用缓刑；CI 呈现禁裸报点估计；8 sleeve×切片过 BH-FDR 校正）；⑥冷启动衔接（三条件：≥2 评审周期+笔数门槛+composite 入线；shadow 只做验证门不做排名输入、与 live 同权重口径记账）。

**D87 归因扩展四件**：①选股漏斗分层归因（决策时快照+按来源聚合胜率+**未入选影子组合对照** paper T+5——差值即打分层贡献，等权对照防集中度效应，本轮最值得新增）；②L2 板块命中表（推荐板块→5/20 日收益 vs 行业等权基准，月度胜率+平均超额，中信建投月报模板）；③sleeve 内信号级归因并入 G04（信号分桶赚亏表+三分瀑布+SHARP 纪律单参数原子改）；④反馈边治理分级三档（自动执行/预授权/纯人工，防 governance drift）+错配双列表（收益贡献%|风险贡献%|比值升序=风险调整后排序）+Component VaR 边际检查（负 Component 对冲型降权需人工确认）+归因有效期标注（协方差窗口+波动状态元数据，高波动标低置信度+stressed 复算）+币圈 schema 预留（funding/basis 独立行 A 股恒 0+UTC 日历日，不预建引擎）。

**D88 现金层两件**：现金作为独立 sleeve（GC001 逆回购，14:00 后利率高 38%、1 天期 T+1 盘前可用与次日买入不冲突、与 C2 资金校验互斥）；预备金三桶存放+书面动用条件≥2 条触发+回补规则（sleeve 冻结释放仓位先进预备金层）；月报现金贡献行（涨/跌月分开标注）。

**落盘方式（D89）**：YAML 8 处注释（C1 预备金管理/C2-01 运行时五件/C2-02 资金校验/C3-01 切账五件/C3-02 单 sleeve 协议+升降级实操/C3-03 过渡期仲裁+错配双列表/C3-04 小样本+冷启动/C3-05 反馈扩展）+本节；validate error=0。

### 2.33 外审后 Owner 终裁包（v2.16.0，2026-09-07，D90-D110）

外审 Round 1/2 产出 17 项待裁定+7 项 Owner 经验量化假说，经全网证据调研与天地板实证研究后全部终裁。协议口径见 docs/_working/2026-09-07-tdm-backtest-protocol.md；实证报告 docs/_working/2026-09-07-tiandiban-threshold-study.md；归档报告 docs/_working/2026-09-07-tdm-review-round-report.md。

- **D90** 冲突场景=听弱的（降一档机械执行）+量×轮动二维状态机（成交额 250 日滚动分位<25% 存量博弈/>60% 主线放行；轮动强度>80% 分位=电风扇，MOD-SIG-060 挂 E-L2-02）+二值门滞回（进段连续 2 日>60%/退出<40%）
- **D91** 止损双触发（同层取最严；fallback 仅 ATR 缺失；2% 红线移作复盘对照；支撑破位=硬底-7%~-8%/2×ATR 立即离场+软确认次日 30 分钟观察窗收复开盘价；站稳=连续 2 日收盘于支撑上方；修订 D69 risk 桶）
- **D92** 总仓位=min(预算带,60% 硬顶)，aggregator 1.0=工程天花板
- **D93** 单策略常态 25%（删 40%）；主线确认+滚动 20/60 日评分双达标→60%；禁 100%；一致性约束日盈亏贡献>70% 自动降权；主线退出=轮动突升+跌破 5 日线清零；修订 D82=禁日级追归因调权，主线倾斜通道允许当日生效
- **D94** 单票默认≤10%、凯利波段≤3%、earned 豁免≤20% 留痕、6%=大资金容量参考非闸门、清单 8% 作废
- **D95** 竞价段豁免恐慌窗、豁免清单与 S1-06 四触发对齐
- **D96** 档内自动=可逆减动作、清退=评审提案+Owner 签字、0.05 硬线 2 连月重置=单月≥0.05 清零重计、sleeve 崩溃默认冻结最后目标
- **D97** 做T 配对 ID 穿透轧平、资金校验必过 C2-02、买回阻断挂 E-L4-12
- **D98** 切入 distribution=Tier3 立即强裁、其他降档走 Tier2、earned 存量降档日减半 7 交易日归位
- **D99** 天地板改个股级（废除板块家数规则，实证证伪：c=2 组 n=50 T+5 +0.97% 与基线无差异，报告 docs/_working/2026-09-07-tiandiban-threshold-study.md；个股自身天地板→次日竞价即决重点观察）
- **D100** 相关性 0.6=预警线只告警/0.70-0.85=动作线
- **D101** 资金 T+0 回转可用、当日卖回款以成交回报释放
- **D102** 合规双线（300 笔/秒+2 万笔/日=高频认定不适用；15 笔/秒+撤单率≤15%=内部自评红线；年度复核）
- **D103** 断流=不开新仓（昨日已计划卖出照做）
- **D104** Triage 以 D58 为准（WATCH 秒级/MONITOR 1min/HOLD 5min）、3 秒级列 V2 观察
- **D105** 长停复牌首日默认不自动卖出转人工、临停±10% 与长停复牌两子状态
- **D106** 时间口径全交易日化（月=20 交易日滚动窗无额外顺延）、废日历型冷却、改迟滞带+连败阶梯+regime 开关、参数回测标定后生效
- **D107** 废固定预留比例、闲置资金 GC001 收益化、仓位=预算带×波动率调整（杠杆上限 1.0）、尾部弹药=D110 触发+回测验证后动用、修订 D88 预备金不固定比例
- **D108** 设计原则=判定用离散状态机/程度用灰度插值/情况细化用决策表状态机非加树深/新节点拆分三条件
- **D109** 攻防板块特征入 L1（券商/银行 5 日超额、涨幅比、哑铃市检测）、先产特征不联动仓位待回测验证
- **D110** 超跌反转信号（KDJ J<-10+量能>20 日均量 2 倍+无系统性利空过滤）=反转候选信号层、仅限新开仓不补套牢仓、仓位波动率目标+分批、回测验证前不生效
- **D111** 做T 终裁包：以 D58 为框架修订——成本门改绝对金额制（保本价差逐笔现算+单笔下限 5.86 万）/笔数波动自适应（步长 K×ATR、n_plan≤10 受最低佣金约束）/新增趋势过滤+失败腿+流动性门+资金冗余+拥挤熔断风控层；1.5% 族标为代码历史参数；proposed 假说待回测验证（证据：财联社 T0 产业/东财门槛公式/ATR 网格/A-S 模型）

### 2.34 内审漏账收尾包（v2.17.0，2026-09-07，D112-D117）

交接总汇报交叉核对（M-01~M-61 × D90-D111 × W1-W11）发现 8 项"无裁定/无工单/无 §八登记"真缺口，Owner 逐项终裁+全网四路证据调研后收口：

- **D112 熔断五级状态机定义（M-36 收口，树上新分叉）**：X-R1 下开 3 子节点（树深 3），判定轴=组合日盈亏+组合回撤双轴（与六段情绪市场轴正交）——L0 正常/L1 警戒（日亏≥2%：禁加仓）/L2 禁开仓（日亏≥4%：停新开仓+白名单窄门开启）/L3 减仓（日亏≥6%：梯度减仓）/L4 保命（回撤≥25% 或生死线：清仓+Owner 人工接管）；迟滞解除=当日无新低+修复当日跌幅 50% 才降一级禁 V 型回满；全系统"熔断禁加期"统一定义=L1 及以上持续期间，解除=熔断解除即解除无独立计时器（P3-01 资格门④消费 R1-01 输出）；判定用离散状态机（D108 原则）
- **D113 熔断期减仓与持仓处置（Owner"熔断还得减仓"裁定）**：L3 减仓顺序=重亏仓→高波动仓→压舱石保留（私募实践：核心仓大市值高分红低波减缓下跌斜率）；梯度减仓参考机构软去杠杆阶梯（breakingalpha：DD 10-12%→80% 仓/12-15%→60%/15-18%→40%）；恢复=组合回撤修复 50% 后逐级加回风险预算（机构 recover-50% 标准），禁 V 型回满；减仓指令路由 P2-04 执行（冲突优先级仍=风控减仓最高，D44）
- **D114 护盘资产定向加仓白名单（proposed 假说，Owner 假说"熔断可加仓国家队护盘资产"）**：三重门全过才开窄门——①L2/L3 状态②D110 超跌反转信号或国家队明牌信号（ETF 天量成交/官方增持公告）③金字塔法分批（首笔 1/3 预算确认收复再加）；白名单=宽基 ETF（300/500/1000/红利）优先+银行/高股息次之；仓位=D107 尾部弹药预算上限总资金 5-10%；证据链=NT 买入降低标的波动率 3.45-5.65%/崩盘风险 -30~45%（JFQA 2023 Dang-Li-Wang、La Trobe 2025 Michael Li）、护盘范式切换实证（2026 半年报：证金前十股东 77→22 家退出个股、诚通/国新借道宽基 ETF 维稳、NT 总持仓 80% 银行底仓仍在）、九次出手七次见底但政策底滞后市场底 40 天~半年（2008 滞后 40 天/2015 滞后半年/2018 滞后 2.5 月，财联社复盘）→分批+确认信号铁律严禁一次到位；回测验证前整节点休眠（数据地基=ETF 成交量天量检测需 ETF 日线登记+银行 5 日超额 D109 已有 DS-170）
- **D115 sleeve 回撤协议触发时点（M-22 收口）**：5% 减半/7.5% 冻结=收盘核算触发（净值结算后统一评估防盘中假摔误杀）；盘中例外=触及 7.5% 冻结线即时禁新开仓（可逆保护不砍存量）；恢复对齐 D113 修复 50% 规则
- **D116 新股首日排除（M-30 收口）**：上市首日无昨收/历史量能基准→竞价即决十行矩阵全部"无法计算→默认观察不动作"；买入侧 E-L4-12 预检清单新增"标的上市首日"硬拦截；次日起正常规则
- **D117 小修打包收口**：M-53 E-L4-10 模块锚修正——MOD-L06-001 实体=order_manager.py（订单管理器，能力匹配），module_ref 文件路径从 async_fill_dispatcher.py 改正（文件路径写错非能力失配）；M-24 跳空分档预案挂载点=X-S2-03；M-42 HMM 核码 PASS——sentiment_cycle.py 实现为 filtered 递归贝叶斯（prior=T-1 外部传入的昨日后验+今日证据，`smoothed_prior` 命名实为转移惯性平滑非 forward-backward，回测侧只需按协议 §4 冻结标签序列滚动传入）；M-55 人工干预入口登记 X-R1 欠账（paper 升档前增设 Owner 手动接管入口节点）；M-32 桥断持仓保护补登交接报告；M-54 配股/红利税/ST 降级补入协议 §6 盲区清单
...
| 税务批次管理（FIFO/LIFO/HIFO 选批卖出）+税损收割 | A 股个人转让上市股票差价暂免个税，无批次选择收益；亏损不抵税不结转，收割纯增印花税【D54，2026-09-07 三路审计裁定】（边界注：红利税 FIFO 补扣预警不在拒绝范围——差价个税免、红利税收，外审 G-08） |
...
| 卖出冷却期固定天数（2 日/1 月/3 月） | D106：日历型冷却无实证标定且过宽有实证伤害——废除固定天数，改迟滞带+连败阶梯+regime 开关，参数回测标定（2026-09-07） |
