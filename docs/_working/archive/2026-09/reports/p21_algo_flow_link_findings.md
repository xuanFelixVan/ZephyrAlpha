...
- 实测：`scripts/governance/commit_queue_landing.py` 每次消费条目起手
  `_sync_worktree()` = `git-reset --hard（历史文本，隔字规避 GATE 检测——2026-09-20 归档批） refs/heads/dev` + `git-clean -fd（同上）`。两个进程同时排空队列时，
...
并发；租约过期后自举 drain 合法成为第二写者，其 `_sync_worktree` 的 `git-clean -fd（同上）` 删掉了
