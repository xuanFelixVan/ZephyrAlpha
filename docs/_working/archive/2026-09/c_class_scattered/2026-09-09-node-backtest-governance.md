...
>
> **⚠️ 未完成（5 条，逐条摘录）**
> - L20: | 结论：哪些数据有效、哪些是噪音 | 验证结论三态（有效/噪音/待观察）+ 治理闭环 | ❌ 缺 |
> - L49: - **建议字段**（草案）：`run_id（哪次回测）/ snapshot_hash（哪版地图）/ window（回测窗口）/ node_id / validation_method / 触发次数 / 命中方向占比 / 显著性(p值) / 结论(有效|噪音|待观察) / 结论时间 / 备注`。
> - L61: - **大白话**：右侧详情栏新增一区：该节点**验证状态徽章**（未验证/有效/噪音/待观察）+ **历次验证记录列表**（时间、窗口、结论、一句话结论）。
> - L182: - 落法：PB-03 的三态升级为状态机（未验证→验证中→有效→衰减中→噪音/退役），衰减监控挂半年复审（或月度自动巡检）。
> - L269: P0 验收：TDM-E-L1-S1 抽屉显示"未验证"徽章；端点空表返回 200+ok:true；/api/tdm 回归无 diff。
>
> **核验方式**：全文扫描完成/待办信号 + 引用文件存在性核验（引用 2 个，其中判废弃 1、路径漂移 0）+ commit 提及 1 处。
> **判废弃引用（C-1）**：`docs/concepts.md`
>
