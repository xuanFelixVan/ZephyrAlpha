...
>
> **✅ 已完成（6 条，摘录）**
> - L80: | G6 | 流根四节点 | 四流末端 | TDM-E-FLOW 建仓流/TDM-P-FLOW 持仓流/TDM-X-FLOW 离场流/TDM-F-FLOW 组合流（aggregation，输出=今日买卖清单） | 设计已完成（上轮方案） | **已落地**（他会话合并批次 23da636683 入图
> - L94: ### §3.2 盲点自查记录（2026-09-09 深夜 Owner 发问触发，已补入本文档）
> - L96: 1. **六向寻路协议缺失**（原稿只有增长方法论，没有 per-node 作业矩阵）→ 已补 §1.5
> - L97: 2. **防噪音审查闸缺失**（全网搜索可溯源/交叉验证/A 股适配/可回测四闸）→ 已补 §1.6
> - L204: | G6 流根四节点 | 已落地（他会话 23da636683） | — | 23da636683 | 已落图 |
> - L245: **纪律**：真源 YAML 只在批次边界动；禁越级立节点（父未定不立子）；新节点必须有数据来源或标 [ASSUMPTION]；每批台账+地图同 commit；本文档 ttl: task_bound，增长收官归档。
>
> **⚠️ 未完成（15 条，逐条摘录）**
> - L30: **循环终止判据（什么叫"画完了"）**：§2 全部职能有节点承载 + 每条"输入→决策"边之间无未建模的中间判断 + 已有代码能力全部入图或入 D 类不建清单。**节点数不是判据**（业界无决策地图形态，"机构有多少节点"无真源，禁为多而多）。
> - L52: 4. **可回测+数据可得闸**：新节点必须能回答"怎么算对错"（validation_method_registry 五类之一）**且**所需字段已登记或可登记；任一不满足=标 GAP-METHOD / DATA-GAP 挂起，禁带病入图。
> - L69: | 币圈 | TDM-C-* 四节点骨架 | G3：展开设计先行，实现待 Owner 排期 |
> - L77: | G3 | 币圈四节点展开 | TDM-C-L1~L4 | 每节点下设计 L1 传感器/赛道/币对/执行的子骨架（设计稿，实现待 Owner 立项） | 待做 | 挂起 |
> - L79: | G5 | 现有节点有据拆分 | L4-02 等 | 竞价/盘中/尾盘三段——**先挂起**：流根落地看过全景再定（Owner 已裁先不动） | — | 挂起 |
> - L117: | 收工 | 增长台账全部三态标注（已落图/已驳回/挂起）+交接包 | R1/R21 清零复核 |
> - L150: > **纪律**：晨审只审不改——产出=复核报告+逐项裁定（批准/驳回/挂起+理由），代码/地图改动由裁定后的执行批落地。外部佐证 URL 逐条复核（来源可溯闸是四闸第一闸，晨审强模型复核正是 SOP §1.6 规定的通道）。超能力项（统计检验类）挂 Owner，禁硬编。
> - L183: 2. **红节点 18 个逐一判定"预期红（设计态）vs 真断链红"**：4 流根（聚合结构，预期）+TDM-E-L0-04（等 C13，预期）+TDM-E-L2-09-1/09-2（等接线规格卡，预期）+TDM-E-L3-06/07-1/07-3/08/09/10+TDM-E-L4-02/08（8
> - L187: ### D. 裁定清单（6 项，逐项给"批准/驳回/挂起+理由"）
> - L201: | G2 资讯-产业链传导 | 复核（2026-09-10 夜班批2）：L2-09-1/L2-09-2 红节点在图（bc0e322b 骨架）；今晚活跃会话无接线轨（session registry 核实），W3/W4 规格卡未交付 | — | — | 挂起（维持，等交付） |
> - （另有 5 条，见正文）
>
> **核验方式**：全文扫描完成/待办信号 + 引用文件存在性核验（引用 9 个，其中判废弃 0、路径漂移 1）+ commit 提及 4 处。
> **路径漂移（非缺失，勿误判）**：`tests/signal_ashare/test_next_day_8state_forecast.py` 路径漂移→tests/signal_ashare/ml_forecast/test_next_day_8state_forecast.py
>
...
**晨审销项（st-tdm-review-20260911，2026-09-11）**：上列 6 项全部裁定完毕——①R21 维持 warn 欠账（depgraph 实测 273 种下划线 id，建议 Owner 专项批）②X-R1 三方法批准方向、实现挂 Owner 排期 ③RL 执行挂起（2027-03 T2 复审再评）④DAL 两件批准补登/入施工批 ⑤前端两卡批准派单 st-tdmfe ⑥C13 升施工批批准、R39 随批实测。详见 [晨审报告](2026-09-11-tdm-morning-review-report.md) §四。

## §6 晨审指令（2026-09-11 晨，Owner 2026-09-10 深夜令："把昨天建的模块+今天寻找的都加入审查清单，检查完毕，链路全通"）

> **纪律**：晨审只审不改——产出=复核报告+逐项裁定（批准/驳回/挂起+理由），代码/地图改动由裁定后的执行批落地。外部佐证 URL 逐条复核（来源可溯闸是四闸第一闸，晨审强模型复核正是 SOP §1.6 规定的通道）。超能力项（统计检验类）挂 Owner，禁硬编。
> **会话命名**：st-tdm-review-20260911。环境 RULE-ENV/RULE-GUARDIAN 照旧。
> **覆盖口径（诚实交底）**：寻路为时间盒制（每向≤20 分钟），已达"核心文献锚点+双源交叉验证"标准，非穷举——穷举不在本批目标内，T2 半年复审（2027-03）为制度兜底，晨审勿按"全网穷尽"标准苛求，按"每向有产出或查无留痕"标准验收。

### 0. 基线与必读（按序）
1. 地图真源 `config/trading_decision_map.yaml` = **138 节点/194 边**（增长批2 后终态；施工文档/前夜汇报里"137/192"等旧数字已被推进，作废勿混用）
2. 必读顺序：[病菌寻路 SOP](../01_policies_and_standards/sop/trading_decision_map_pathfinding_sop.md)（方法论+四闸）→ 本蓝图 §5 台账+本 §6 → 施工清单 C1-C14 → `.runtime/handoffs/handoff_night-pathfinding-2300.json`
3. 今晚增长批2 三 commit：26d20f8c（C14 行）→ 84ac62dc（地图+台账主提交）→ 01bd0a6e（台账哈希回填）

### A. 昨晚施工轨 10 模块复检（Owner 点名范围，commit/登记以施工清单 C 行为准）
| # | 模块（Owner 大白话） | 施工清单 | commit/登记 | 复检重点 |
|---|---|---|---|---|
| 1 | 五档水温（11 盘面信号→档位） | C1 | 065a2615 / MOD-SIG-135 | 24 测试复跑；D20 月/周封顶约束在码 |
| 2 | 指数传感器（-2~+2 趋势分） | C2 | 5c5b99f2 / MOD-REGIME-016 | 样本<61 fail-closed；与地图 algo_refs=DAL-MA-ALIGN 三角一致 |
| 3 | 强度传导（强板块弱票加分） | C5 | 065a2615 / MOD-SIG-136 | L2-10 同源补涨比价输入接线到位 |
| 4 | 扩散进度（站上 20 日线占比） | C6 | 065a2615（tracker 扩展） | 40/60/80% 阈值与节点 decision_question 语义一致 |
| 5 | 负面否决器（暴雷/立案/减持一票否决） | C7 | 48c913a8 / MOD-SIG-137 | 六规则挂 STR-MULTIFACTOR-034~041 有效；VetoVerdict 改名无残留 |
| 6 | 买入逻辑存活判定（成立/弱化/失效） | C8 | 994e96f9+ae9ccb83 / MOD-PLAN-024 | 失效→X 流转场边已接 |
| 7 | 金字塔加仓（资格门+递减） | C9+C10 | 4105b58e / MOD-POS-027 | "禁补亏损仓"红线在码；总加仓≤首仓 |
| 8 | 护盘白名单（熔断窄门） | C11 | bc4c0770 / MOD-POS-026 | D114 休眠默认关实证（回测验证前不开门）；三重门齐 |
| 9 | 执行时段路由（卖出看钟表） | C12 | a9be7524 / MOD-XS-016 | 14:57 深市不可撤等时段口径与交易所规则核对 |
| 10 | G1 明日情绪滚动预测（红节点） | 红节点 c8f678d8 + C13 组合器待建 | — | 红节点=预期态非丢件；C13 是否升施工批→D6 裁定 |

每模块六点全过：①15 字段头完整 ②tests 复跑 2 轮 0 错 ③depgraph 登记与 build_status 核对 ④地图 module_ref/module_id 交叉锚一致 ⑤algo_note_zh 与代码行为一致（ALGO-NOTE-SYNC 纪律）⑥入边有源/出边有消费者（链路通）。

### B. 今晚增长轨产物复检（增长批2=84ac62dc 系）
1. **TDM-E-L4-14 执行成本反馈与选型回写**：D108 三条件回检（独立输入输出/可回测/非参数变体）；feedback 环合理性（L4-06→L4-10→L4-14→L4-06，S1↔S3 双向 feedback 同款先例）；**断链实证复核**（algo_execution_selector.py 零消费 execution_quality_scorer——grep 复跑留痕）。
2. **L1-S1/L1-S2 挂载**：DAL-MA-ALIGN/DAL-BREADTH-SCORE/DS-108 id 在册+语义对位（S2 挂 design 态算法=广度计分核缺代码，是否入施工批→D4 裁定）。
3. **外部佐证 9 源**逐一可达+题录核对（重点两处期刊勘误：Seasholes-Wu 2007=*Journal of Empirical Finance* 非 JFM；Grossman-Zhou 1993=*Mathematical Finance* 非 JF）。
4. **四关复跑**：`check_decision_map.run_checks()` errors=0 / 双测试 98 绿 / `align_all.py` exit0。

### C. 链路全通总检（Owner 终审目标"检查完毕，链路全通"）
1. **8890 api_server 实测**：/api/tdm 全量接口吐出=138/194（前端 30s 轮询自动生效；昨实测 137/192 已被增长批2 推进，L4-14+2 边应可见）。
2. **红节点 18 个逐一判定"预期红（设计态）vs 真断链红"**：4 流根（聚合结构，预期）+TDM-E-L0-04（等 C13，预期）+TDM-E-L2-09-1/09-2（等接线规格卡，预期）+TDM-E-L3-06/07-1/07-3/08/09/10+TDM-E-L4-02/08（8 个树枝锚定移交批，逐一审定归属模块）+TDM-C-L2/L3/L4（币圈 D 类待 Owner 立项）。
3. **十职能族对照**（SOP §7）：逐族核对"有节点承载、无断链"——已知未闭合=资讯传导族（G2 等交付）与明日推演族（G1 等 C13），其余八族应全通，发现新断链即登记。
4. **边引用完整性**：validate R 系已覆盖，抽读复跑报告确认 error=0、无悬空引用。

### D. 裁定清单（6 项，逐项给"批准/驳回/挂起+理由"）
1. **R21 module_id 格式冲突**：depgraph blueprint_id `MOD-EX_SOR_EXT-002`（带下划线）vs 地图正则 `MOD-[A-Z0-9]+(-[A-Z0-9]+)*` 互斥——写原 id 报格式错、去下划线报对账错。三选一：a) 地图正则放宽支持下划线 b) depgraph blueprint_id 改名去下划线（牵 44 号域文档+模块蓝图头+缓存重建）c) 维持 warn 欠账。
2. **X-R1 三方法 A 股离散化**：CPPI（Black-Perold 1992 JEDC）/回撤控制（Grossman-Zhou 1993 MathFin）/趋势危机 alpha（Hurst-Ooi-Pedersen 2017 JPM）——各给采纳改造/留调研/驳回（A 股 T+1+跌停不可卖是硬约束）。
3. **RL 执行立项**：arXiv 2411.06389 (2024) / 2507.06345 (2025)，远期候选。
4. **DAL 补登两件**：DAL-MA-ALIGN code_ref 补 `src/zephyr/regime/features/index_sensor.py`（registry 现 code_ref=None 而代码在产）；DAL-BREADTH-SCORE 广度计分核是否入施工批（MOD-DATA-062 采集在/计分缺）。
5. **前端两卡派单**：指数趋势分卡（L1-S1，-2~+2+四时点刷新）/涨停板生态卡（L1-S2，梯队/断层/晋级率）→ st-tdmfe（只登记未派单）。
6. **G1 latency_budget**：R39 欠账随 C13 实测补——连带裁定 C13 是否升施工批。

### E. 产出契约
- 复核报告落 `docs/_working/2026-09-11-tdm-morning-review-report.md`（新建 .md 记得 CREATE-GUARD token 登记顶层 creation_tokens 节）。
- 裁定结果回写：本蓝图 §5 台账状态列 + 施工清单 C13/C14 状态 + 上 D 清单逐项销项——**回写动作由晨审会话执行**（只审不改指代码/地图，台账/文档裁定留痕属于晨审本职）。
- 汇报格式：大白话——链路全通结论（预期红 X 处/真断链 Y 处各是哪些）+ 裁定 N 批/M 驳/K 挂 + 六项裁定一览表。
- **终止判据**：A 十模块×六点全有结论 + B 四项复检全过 + C 三项全过（或断链全部有归属）+ D 六项全裁定。
