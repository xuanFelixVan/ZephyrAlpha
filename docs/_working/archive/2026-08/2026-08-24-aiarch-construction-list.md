...
| ~~W4~~ | ~~三套自治等级标尺统一裁定（15 号文 Q3）；A-L 成熟度与既有标尺映射裁定（16 号文 Q4）~~ ✅ 已闭环（2026-08-29，#ARCH-284） | 15/16 号文 §6 | 实证 Owner 已于 2026-08-18 裁定关闭（裁定8/裁定#224，commit 56319b2f42：00_index 主标尺 L0~L3+L4 保留+§3.3 映射定稿）；roadmap B17 同项已追认 |
| ~~W5~~ | ~~模块工厂 Phase 0→1 / 业务 Agent 细化排期裁定（13 号文 Q1 / 14 号文 Q1）~~ ✅ 已闭环（2026-08-29，#ARCH-285/#ARCH-286） | 00/13/14 号文 | GP1 排期批裁定开门（I1-1~I1-3 全达成）；13 号文 Q1~Q9 全裁定；module_factory 已落地（MOD-FACTORY-001/002）；roadmap B18 同项已闭环 |

---

## 1. P1（高——GP1 半自动主体，保障类优先）

### 1.1 Agentic Drift 防护四件套（15 号文 Phase 1） ✅ 已核销（2026-08-30 实证：agentic_drift_guard.py + drift_semantic_reviewer.py 在位）
...
### 1.2 自治运维 Diagnose→Remediate 接线 + 知识库落盘（16 号文 Phase 1） ✅ 已核销（2026-08-30 实证：security/ops/incident_pipeline.py + data/fix_patterns/ 知识库在位）
...
### 1.3 llm_runtime_gateway Phase 1：预算硬门 + ModelRouter 接线 + LLMDeg 降级注入（10 号文 Phase 1） ✅ 已核销（2026-08-30 实证：pre_flight_check 预算硬门 + LLMDeg 降级注入在位）
...
### 1.4 模型路由级联控制器（11 号文 Phase 1） ✅ 已核销（2026-08-30 实证：intelligence/model_routing/ 目录在位，cascade_orchestrator.py 落码）
...
### 1.5 触发式考试调度器（06 号文 Phase 1） ✅ 已核销（2026-08-30 实证：exam_trigger_scheduler.py 在位）
...
### 1.6 Context Engine inject 段接线 + 压缩三档（07 号文 Phase 1） ✅ 已核销（2026-08-30 实证：inject_by_keyword() 在位 + doc_compressor llm_summary/rule_based/truncate 三档）
...
### 1.7 LSG 蓝图剩余缺口收尾（09 号文 Phase 1，80%→95%） ✅ 已核销（2026-08-30 实证：lsg_security_gate.yml + LSGPerformanceGuard + check_lsg_security 全在位）
...
### 2.1 KILLSWITCH 三级响应编排层（16 号文 Phase 2，P2-3） ✅ 已核销（2026-08-30 实证在案）
...
### 2.2 Learn 回写闭环 + 成熟度 A-L0→A-L2 解锁（16 号文 Phase 2，P2-1/P2-2） ✅ 已核销（2026-08-30 实证：fix_pattern_miner 本批落码）
...
### 2.3 ReflCtrl 频率闸门 + PreFlect 失败模式库（12 号文 Phase 1，P1-1/P1-3） ✅ 已核销（2026-08-30 实证在案）
...
### 2.4 模块工厂 Phase 1：knowledge_classifier + module_mapper（13 号文 Phase 1） ✅ 已核销（2026-08-30 实证：已由 W1 批落地 autonomy_core/module_factory（MOD-FACTORY-001/002），本批错误码转正）
...
### 2.5 涌现检测「告警→人工介入」末段接线（12 号文 §4.4 + 16 号文协同） ✅ 已核销（2026-08-30 实证在案）
...
### 2.6 MCP Client 动态发现 + 漂移对账入遥测（10 号文 Phase 2，步骤 2.1/2.2） ✅ 已核销（2026-08-30 实证在案）
...
### 2.7 技能库自动生成路径（AutoSkill+Voyager，11 号文 Phase 2） ✅ 已核销（2026-08-30 实证：skill_trajectory_miner.py 落码 testing）
...
### 2.8 04 号文遗留：boot watchdog 存量缺陷修复 + 冷启动 SLA 复测 ✅ 已核销（2026-08-30 实证：缺陷已于 08-22 修复实证）
...
### 2.9 task_gate 护照 ID 口径统一（06 号文遗留） ✅ 已核销（2026-08-30 实证在案）
...
| 3.9 | derived_graphs/ 派生图 6 篇生成 ✅ 已核销（2026-08-30 实证：derived_graphs/ 6 篇已生成落盘） | 00 号文 §5.2 | 目录实测不存在；含 AI 层依赖拓扑/三层运行时编排/模型生命周期/上下文记忆流/多 AI 协作时序/LLM 安全栈纵深图 | 复用 `scripts/governance/` 派生图生成器族；MOD-GOV-049（若独立成件） | 00 号文 §5.2 六篇清单+源真源标注 |
