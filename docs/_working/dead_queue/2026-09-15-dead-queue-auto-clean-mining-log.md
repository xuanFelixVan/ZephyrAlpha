...
## 封矿决议（Owner 2026-09-15 04:09 "封库"）

四步施工清单**废止**——Owner 复核判定为过度工程：

- **规模账**：dead/ 全量 1016 条共 19.66MB，按当前增速（09-11 峰值 805 条/日后日均递减）攒到 1GB 需约 10 年——磁盘非约束。
- **收益账**：首轮抽样 200/200 全部已收敛，retirement_audit.yaml 已是"识别该清不该清"能力的完整产出；自动化只是反复证明一个稳定结论。
- **成本账**：裁定变更 + 钉死测试改写 + 两轮冷静期状态机 + 常驻维护，收益≈0。

处置：
1. 维持现状——"dead/ 永不清理"不变量原样保留，字节永可取。
2. retirement_audit.yaml 为识别能力终稿；后续审计窗口自然跑完（已接线+24h 节流，不追加投入），全量底账拿齐即停。
3. guard_recycle 通道与全部挖矿证据留档备查——未来真需清理时按本案重启，不必重新挖矿。
