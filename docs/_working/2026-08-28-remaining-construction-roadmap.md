...
| A7 | 日历扩展消费点注入改造 | [94号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/94_crypto_quant_expansion.md) | tracker #261：A/B/C/D 类 20+ 文件日历消费点注入式改造（W0 只覆盖四类核心） | data/calendar/ 包已在位；消费点清单 docs/_working/reports/2026-08-26-calendar-consumers-inventory.md | 波 3 |
| A8 | 盘中实时操纵检测 | [43号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/43_compliance_discipline.md) | MOD-CMP-011 intraday_manipulation_detector 骨架已在，实时流驱动未接 | 骨架文件已在位 | 波 3 |
| A9 | 绩效归因全链+元级迭代 | [54号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/54_reconciliation_attribution.md) | BM-REC-02-B 绩效归因全链残余阻塞；BM-REC-03-D 元级迭代（依赖 61 号 Champion-Challenger 通道，与 A10 联动） | SettlementReconciler/PositionReconciler/DailyAuditor 已在位 | 波 3 |
| A10 | 生命周期治理编排层 | [61号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/61_lifecycle_multi_ai.md) | Drift Observatory 四层编排零命中；mSPRT 晋升编排层/消费方未建（内核 msprt_champion_challenger.py 在库） | lifecycle_governance 五件 production 已在位 | 波 4 |
| A11 | Panel 作战室前端页 | [45号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/45_warroom_playbook.md) | **最大前端缺口**：app_panel.py 11 Tab 无作战室页；缺口⑥~⑩（9 格概率模型/批量边界/辩论实例化/相关性前端消费/禁做清单）按 P1-P3 分期 | daily_warroom_pipeline（testing）+dashboard_feeds W5 已在位 | 波 4（前端批，可与其他波次并行） |
