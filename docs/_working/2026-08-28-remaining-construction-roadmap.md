...
|---|---|---|---|---|---|
| ~~A1~~ | ~~B4 S2 翻 true 收尾重验~~ ✅ 已完工（2026-08-30 长城批，greatwall_20260830，commit 2a16988d） | [13号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/13_regime_phase3_engineering_plan.md) / [14号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/14_regime_s2_diagnosis.md) | ~~⚠️ 2026-08-28 重验 0/3~~ → 治本施工（08-29 AI-WAVE4-001）+ walk-forward 验证（08-29）+ **长城批收尾（08-30）**：B4 全量重跑 PASS 4/4（S1 3/3+S2 1/1，EVT-2024 confirm Δ=+3d 触发）；design_match 裁定=低置信部分通过（EVT-2024 翻 true，2015/2020 超 halflife=10 物理极限维持 false+边界注记）；known_data_gaps 补登 erp 断点；14 号参数终值回写；13 号 draft→active。调查报告/验证报告/估值管道报告已归档 docs/_archive/（2026-08-30）。批次报告：reports/2026-08-30-s2-closure-report.md | overlay_features.py 五函数+keys_or_gte 已在位 | ~~校准专项~~ 已闭环 |
...
| ~~A6~~ | ~~GAP-2 常驻服务化~~ ✅ 已闭环（2026-08-30 长城批实证勾销，commit 2a16988d） | [57号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/57_daily_cycle_sop.md) | ~~◐ 部分闭环~~ → 全链实证：LiveStrategyAdapter 库级件（波 3b）+**CLI 接线已落**（start_paper_session.py `--service`+`assemble_service`：assemble_session 包 StrategySlot 交 adapter 监督，异常隔离/退避熔断/biz 心跳/run(close_at) 有界收场）；残余仅 **Owner 窗口项**——挂计划任务/调度+deadman_switch 第四路纳入监控清单（tracker #273 已登记留痕） | start_paper_session.py 已在位 | ~~波 3~~ 已闭环（残余仅 Owner 窗口） |
| ~~A7~~ | ~~日历扩展消费点注入改造~~ ✅ 已完工（2026-08-29 波 4，AI-WAVE4-001） | [94号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/94_crypto_quant_expansion.md) | ~~首批已注入~~ → 首批 7 点（commit fd792ddb）+ 终盘补齐 3 点（commit f58fc7ed5a：LPR 公告日/MLF 操作日非交易日顺延 + 涨停池收盘钟改从 session_windows 派生，50 用例绿）。**终盘盘点（2026-08-29 重扫，替代丢失的 08-26 清单）**：消费点 30 个=已覆盖 21 + 本批补 3 + 跳过 6——跳过项全为**非 A 股交易日语义**（SGX A50 交割 Mon-Fri 自算口径/周计划调度 weekday 语义/访问控制/前端图表过滤）或**文档化向后兼容默认**（pit_manager BDay 默认、trading_calendar 旧 fallback），不构成缺口 | data/calendar/ 包已在位；16 文件已接入（grep 实证） | ~~波 4~~ 已闭环 |
...
| ~~A9~~ | ~~绩效归因全链+元级迭代~~ ✅ 已闭环 | [54号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/54_reconciliation_attribution.md) | ✅ BM-REC-02-B 已闭环（2026-08-29 波 4，AI-WAVE4-001，commit c5628ff6）：MOD-RPT-036 归因计算（Brinson/Carino/T+1）+ MOD-RPT-037 落库查询（34 用例+728 关联域绿）；**BM-REC-03-D 元级迭代已闭环（2026-08-30，AI-WAVE5）**：MOD-RPT-038 attribution_meta_iteration（归因 delta 序贯投喂 A10 PromotionChannelManager→晋升/降级评审建议，human_gated，+24 用例）；残余=双实现收敛裁定（54号 §6，Owner 窗口） | SettlementReconciler/PositionReconciler/DailyAuditor 已在位 | 已闭环（残余仅 Owner 裁定项） |
| ~~A10~~ | ~~生命周期治理编排层~~ ✅ 已完工（2026-08-29 波 4，AI-WAVE4-001） | [61号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/61_lifecycle_multi_ai.md) | ~~零命中~~ → drift_observatory_orchestrator（四层编排）+ msprt_promotion_channel + factor_promotion_wiring 落码（commit c9d0b346）+ Owner 批准 production 启用（B-007 同 B5/B6 通道）；BM-REC-03-D 依赖已解锁 | lifecycle_governance 五件 production 已在位 | ~~波 4~~ 已闭环 |
| ~~A11~~ | ~~Panel 作战室前端页（P1/P2/P3）~~ ✅ 已完工（2026-08-30，AI-WAVE5） | [45号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/45_warroom_playbook.md) | ✅ P1（2026-08-29 波 4）：作战室 Tab 居首位+四指数卡（B21 搭车）+P2/P3 折叠占位；✅ P2（commit 8ca858c166）：缺口⑥ 9 格矩阵展示层/缺口⑦ 批量边界/缺口⑨ 相关性净额/缺口⑩ 禁做清单四缺口落码；✅ P3（commit aedeb4eb1e）：缺口⑧ W4 多空辩论台展示层——llm_daily_analysis v2 辩论落库行只读消费（多头/空头陈词+综合席三情景裁决），全通道 fail-open，+7 用例 36 绿；残余=MOD-PLAN-013 四角色链交易员/风控位接日循环跑批（testing 件，待 MOD-PLAN-018 装配窗口）+W0/W6 历史预案库随 prediction_log 样本积累接入 | daily_warroom_pipeline 已随 B5 转 production +dashboard_feeds W5 已在位 | ~~波 4/5~~ 已闭环 |
| ~~A12~~ | ~~回撤延期项清偿~~ ✅ 已完工（2026-08-30，AI-WAVE5） | [35号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/35_drawdown_protocol_impl.md) | ~~延期三项~~ → 持久化+归因前序批次已闭（acc0b21335），本批补 L2/L3 兜底：drawdown_broker_side_stop（挂损意图+对账）+drawdown_watchdog（幽灵仓位强平裁决），+33 用例 2416 绿；残余=L2 直连 broker 待实盘验证+§7 ㉓ Owner 裁定 | drawdown_tracker/controller 已在位 | ~~波 5~~ 已闭环 |
| ~~A13~~ | ~~BudgetChange 接线尾巴~~ ✅ 已完工（2026-08-30，AI-WAVE5） | [33号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/33_budget_change_handler.md) | ~~三缺口~~ → TierState 跨日持久化（原子快照+fail-closed 恢复）+E-POS-40/41 事件分发+sync_from_allocator 生产接线（duck-typed 依赖倒置），+23 用例 666 绿；domain_events 25→27 | budget_change_handler.py 已在位 | ~~波 5~~ 已闭环 |
| ~~A14~~ | ~~执行层 Phase 2 项~~ ✅ 已完工（2026-08-30，AI-WAVE5） | [40号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/40_execution_broker.md) | ~~三缺口~~ → 盘后全量对账 MOD-L06-003（四步+报告态默认）+拒单分类 Saga 接管（rejection_executor 可选注入）+expire_open_orders；盘后固定价格交易实证为外部条件项（40号文 §6.1 MVP 暂缓+真实申报通道），不施工留痕；+28 用例 1192 绿 | order_manager/saga/cancel_rate_guard 已在位 | ~~波 5~~ 已闭环 |
| ~~A15~~ | ~~监控暂缓项复评~~ ✅ 已闭环（2026-08-30 复评，AI-WAVE5） | [55号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/55_monitoring_review.md) | 复评结论=**四项全部维持暂缓**（重评条件均未触发：实盘未上线/QMT 探针待 40 号 P0/偏离告警零真实触发/周复盘未满 12 期）——纪律正确性确认，无施工面 | MOD-RK-23/MOD-RPT-009/threshold_loader 已在位 | ~~波 5~~ 已闭环 |
| ~~A16~~ | ~~CPCV+PBO 过拟合增强~~ ✅ 已闭环（2026-08-30 复评，AI-WAVE5） | [52号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/52_backtest_framework_docking.md) | 复评结论=**维持设计内延期**：重评条件（策略变体>50 或 DSR 频繁误报）未触发——回测跑批（B1）未进行，变体量未达阈值；52号文 §6 裁定原文有效，零施工面 | deflated_sharpe/overfitting_adjudicator 已在位 | ~~波 5~~ 已闭环（条件触发才转 A） |
...
| ~~A22~~ | ~~期指 tick 采集+A50 数据源评估~~ ✅ 已完工（2026-08-30 核验，AI-WAVE5） | [44号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/44_premarket_intraday_decision_upgrade.md) | 实证前序已落（7ee214ad55/51aca6c3f3）：futures_tick_intraday（IF/IC/IM/IH 主力连续 3 秒 tick 盘中实时）+a50_futures_daily_incremental（akshare 新浪源十年回填 2594 行）+a50_futures_daily 表；A50 三通道评估报告落盘（新浪主源+东财兜底+不可用通道置空留痕；日韩不评估=44号 §11 裁定） | 期指日频/ES/NQ 通道已在位 | ~~波 3~~ 已闭环 |
...
| ~~B3~~ | ~~因子 IC 实证回填（factor_registry ic: null）~~ ✅ 已闭环（2026-08-30 长城批，commit 2a16988d） | [62号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/62_business_registry_construction.md) | ~~回测跑批产出 IC~~ → 阻塞解除口径扩：BTRUN 范式直算批（不等 C1 全量跑批）——code-anchored 4/4 七字段回填（窗 2024-09~2026-08，horizon=5，Spearman），143 条 NL 锚定裁定登记（.runtime/factor_ic_backfill/20260830_report.md） |
...
| B13 | 机构级 regime 维度 | [10号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/10_regime_detector_spec.md) | ~~数据源（daily_valuation 管道等）~~ → **拆分（2026-08-30 裁定）**：B13a（CAPE/IV/两融三维，数据已在仓）→ 转 A 立即排期；B13b（credit_spread/COT/PageRank，无数据源）→ 维持 B |
...
| B15 | 情绪周期定位器准确率评估 | [28号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/28_sentiment_cycle_trading.md) | 实盘数据；**挂链 B1**——B1 跑批立项时同步评估（2026-08-30 裁定） |
| B16 | 外部 Feed 事件因子（dReport/Jump on PEAD） | [26号](../02_enterprise_architecture/07_trading_decision_architecture/design_memos/26_event_driven_strategy_detail.md) | 外部 Feed 接入（iFind 已裁定不续费，源待选）；**dReport 披露日期输入部分已在仓**（daily_event 族披露计划/财报快报任务），可预做勘探裁定（2026-08-30） |
...
**GP1 第一波已落码（2026-08-29 当日派单当日完工，AI-GP1-W1，三笔）**：13号文模块工厂两件（5a35776499：MOD-FACTORY-001/002 testing，50 用例+autonomy 1521 绿）∥ 10号文门面 complexity/max_cost 路由+L3 route 三字段（5bd46ce96a，25 用例+关联域 3200+ 绿；实证大半前序 f8121421d7 已建）∥ 09号文 L6 飞书接线+check_lsg_security 专测（24f639fc6b，12 用例+llm_security 1205 绿；P1-1~P1-4 主体实证前序已落）。残余：13号文 P1-S4 ≥5 条半自动入库（潘潘 260 条负载，含人审环节）+ 分类器 50 条人评一致率验收（Owner 参与）+ L6 飞书 webhook 生产启用（ZEPHYR_FEISHU_WEBHOOK 配置=Owner 窗口）+ 14号 S1.1 接口冻结
**GP1 第二波已落码（2026-08-29，AI-GP1-W2，三笔）**：11号文级联运行时装配（runtime_assembly.py 四缝接线 cascade→agent_router，26 用例+intelligence 640 绿）∥ 12号文投票评审壳 MOD-VOTE_REVIEW_SHELL（97 行纯编排+A2AVoting 零改动+§SOP，10 用例）∥ 15号文 Drift 内联挂接 S0.2 gate（P95 增量 0.292ms 达标，WARNING 降级/DETECTED 阻断）+嵌入适配器（21 用例+autonomy 1542 绿）。**重要实证**：三文人结案报告"未做"清单均系快照滞后——cascade_orchestrator/drift_guard 两件/ReflCtrl/PreFlect 前序已建，本波只补真缺口。残余：投票壳真实评审首跑（Owner 多会话窗口）+ S1.3 语义复核首跑（盘后批量）+ S1.1 基线合成异常注入演练
**GP1 第三波已落码（2026-08-29，AI-GP1-W3，三笔）**：16号文自愈管线真缺口（冷启动 15 条模式导入+假阳性落盘+GOV-AI-001 实质对接，+16 用例 391 两轮绿；Q2 关闭+anomaly_diagnoser 支路显式排除裁定留痕）∥ 07号文 CE 补缺（inject task/module 两模式接通+local_llm_summarizer 生产档 integrity 校验串联，+48 用例 1803 两轮绿；P1-3 sync 主用裁定回填+Q6 关闭）∥ 06号文触发闭环接线（TaskGate 硬门挂 agent_router 可选缝+考试调度器 CLI 盘中拒跑，+15 用例 1498 两轮绿；MOD-INF-054 实证已 production）。残余：考试扫描定时注册（裁定=不挂 tasks.yaml 数据调度器，可选 Windows 计划任务盘后周频=Owner 窗口）+ 16号文生产首跑留痕（.runtime/security_events 首跑观察）+ 07号文 llm_summary 生产启用（ContextAssembler 注入 llm_summarizer=Owner 窗口）
**GP1 第四波已落码（2026-08-29，AI-GP1-W4，两笔）**：08号文队列联动（级联标记+死信 requeue CLI+done TTL 清理+61/65/66号文升硬联动修订，+20 用例 147 绿）∥ 14号文 Phase 1 全三步（S1.1 接口接线+S1.2 ticket_queue 四态队列+S1.3 G04 运营核对，+37 用例 1614 绿；端到端冒烟过）。**GP1 退出条件判定（17号文 §4.3 已回写）：E1-2/E1-3 ✅ 达成（E2E 演示+human_gated 抽检实证）；E1-1 ◐ 11/12 绿——M1 里程碑仅剩 13号文两项人审验收（50 条人评一致率+≥5 条半自动入库）+ MOD-FACTORY production 翻转，均 Owner 窗口**。GP1 代码施工至此全量闭环
        │
