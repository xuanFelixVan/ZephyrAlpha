...
> **文档元信息**（_working 临时区豁免规范，EXEMPT-ZONE-FM）：doc_type=asset_ledger · owner=ZephyrAlpha-Owner · status=active · version=1.5.0 · date=2026-08-28 · topic=dashboard_asset_ledger · scope=dashboard。
>
...
- **需求族（§5）**：14 条（R1~R14）全"进"（R5 附 IA 第六组拍板条件；R1 标第一优先；R11 含 1 项远期不做=全量穿透图；R12=页 1 重构令，连带⑥审批/告警形态实质定为横切层；R13=页 2 重构令，新增施工模块 2 件=时序分段状态机+明日推演联动；R14=页 3 重构令=盘中实时改名大盘分析，基本零新模块全是接线件）；随批已决 3 项（美联储不开页/币圈不做地图/天气不改名）；新增配套资产 1 件（算法注册表，R2）+配套清单 2 件（反馈点清单 R1/进度条清单 R4，映射批产出）。
