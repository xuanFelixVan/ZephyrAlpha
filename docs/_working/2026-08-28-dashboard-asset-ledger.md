...
> **文档元信息**（_working 临时区豁免规范，EXEMPT-ZONE-FM）：doc_type=asset_ledger · owner=ZephyrAlpha-Owner · status=active · version=2.2.0 · date=2026-08-29 · topic=dashboard_asset_ledger · scope=dashboard。
>
...
- **需求族（§5）**：21 条（R1~R21）全"进"（R5 附 IA 第六组拍板条件→已拍板；R1 标第一优先；R11 含 1 项远期不做=全量穿透图；R12~R15=页 1~4 重构令；R16=页 5 情绪页重构令[涨停强度今昨对比/连板天梯/涨停质量三维/涨停基因/算法查看图标泛化]；R17=页 6 新闻页重构令[两天滚动窗/点击展开详情+分析/整体新闻分析/预期差/远期=产业趋势甘特图+订单季报前瞻页]；**R18=页 1 二次再定位**[项目全景驾驶舱：资金条+模块全景地图+数据流遥测，天气/校验卡撤出]；**R19=全站模块化组合**[布局引擎一期+试点页，全站卡片化搭便车不突击]；**R20=4K 屏密度**[转视觉线]；**R21=首页结论汇报墙+改名全景总览**[每页 verdict 卡标准化输出+首页按交易顺序聚合抓取+AI 对话框常驻+一屏硬约束]）；随批已决 5 项；新增配套资产 1 件（算法注册表，R2）+配套清单 2 件（反馈点清单 R1/进度条清单 R4，映射批产出）。
