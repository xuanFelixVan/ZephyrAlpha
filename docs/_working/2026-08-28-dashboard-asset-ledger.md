...
> **文档元信息**（_working 临时区豁免规范，EXEMPT-ZONE-FM）：doc_type=asset_ledger · owner=ZephyrAlpha-Owner · status=active · version=1.6.0 · date=2026-08-28 · topic=dashboard_asset_ledger · scope=dashboard。
>
...
- **需求族（§5）**：15 条（R1~R15）全"进"（R5 附 IA 第六组拍板条件；R1 标第一优先；R11 含 1 项远期不做=全量穿透图；R12~R15=页 1~4 重构令：全景总览实盘化/作战指挥两板块/盘中实时改名大盘分析/板块全景相对强度地图）；随批已决 5 项（美联储不开页/币圈不做地图/天气不改名/地图颜色右红左绿/每页结果+证据链铁律）；新增配套资产 1 件（算法注册表，R2）+配套清单 2 件（反馈点清单 R1/进度条清单 R4，映射批产出）。
