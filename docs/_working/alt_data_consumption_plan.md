...
## 8. 验证台账（信号毕业状态，2026-09-14 C-1/C-2 落地后）

| 信号 | 状态 | 验证证据 | 毕业路径 |
|------|------|---------|---------|
| F4 BDI 动量 z20 | live（z=1.10 risk_on） | 描述性统计；无收益预测声明 | OOS 2024-2026 regime 切换胜率检验 |
| F7 台风事件 | live（116 场） | **P0 全样本证伪收益可预测**（c1a962e9：季节调整后各切面 t 全不过线）；定位=风险日历/状态标记 | 复开三条件（更长路径史/更优基线/002 衍生方向）登记于 EVT-TYPHOON-BDI-001 |
| F8 极端高温 | live v1（高温日计数） | 待积累 | 季节距平版需多年数据（2027）；迎峰度夏 regime 检验 |
| F10 生猪期现 basis | warmup（130 日暖机） | 描述性 | 现货腿积累后 IC/分层 |
| F11 猪周期相位 | live（531 天） | 描述性 | 养殖股轮动分层检验 |
| F12 分省离散度 | live（dispersion_high） | CV 口径 | 调运受阻事件对齐检验 |
| F14 BTC 动量 | live（risk_on） | 描述性 | risk_on/off 切换胜率 |
| F15 恐贪 | live（greed 61） | C4 恐慌反弹双窗及格（1.15→0.83） | <20 极值窗样本积累 |
| F23 涨停情绪周期 | live（退潮 0.214） | 阈值=社区共识 provisional | OOS 阶段状态机转移检验；毕业前不进决策硬链 |
| F25 转债溢价率分位 | warmup（匿名覆盖 30 只） | 文献分位择时 | CB_JSL_COOKIE 全市场后分位检验 |

登记纪律：全部信号 graduation 前仅作 regime/研究输入，不进 TDM 决策硬链；F7 遵循 P0 证伪校准（风险日历定位）。
