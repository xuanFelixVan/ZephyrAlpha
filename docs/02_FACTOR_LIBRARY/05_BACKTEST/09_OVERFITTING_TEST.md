...
> Layer 2: Alpha因子计算 - Walk-Forward、蒙特卡洛、参数敏感性、泛化能力评估
...
class WalkForwardAnalyzer:
    ...
    def generate_report(self, results: List[WalkForwardResult]) -> str:
        ...
        lines.append("")
        lines.append(f"平均样本外夏普: {avg_oos:.3f}")
        lines.append(f"过拟合比率: {of_ratio:.1%}")

        if of_ratio < 0.2:
            lines.append("状态: ✅ 良好 (过拟合比率 < 20%)")
        elif of_ratio < 0.4:
            lines.append("状态: ⚠️ 警告 (过拟合比率 20-40%)")
        else:
            lines.append("状态: ❌ 危险 (过拟合比率 > 40%)")
