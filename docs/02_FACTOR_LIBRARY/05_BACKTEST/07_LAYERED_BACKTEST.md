...
> Layer 2: Alpha因子计算 - 分组测试、多空组合、收益单调性分析
...
class BaseGrouper(ABC):
    ...
    def group(
        ...
        参数:
            factor_values: 因子值 (index=stock_code)
            stock_returns: 股票收益率 (index=date, columns=stock_code)
            market_caps: 市值 (index=stock_code)
...
class ReturnAnalyzer:
    ...
    def generate_returns_report(
        ...
        lines.append("分组收益:")
        lines.append("-" * 80)
        lines.append(f"{'组号':<8}{'收益率':<12}{'年化波动':<12}{'夏普比率':<12}{'最大回撤':<12}")
        lines.append("-" * 80)
        ...
        lines.append(f"  是否单调递增: {'是' if monotonicity['is_monotonic_increasing'] else '否'}")
...
class StatisticalTester:
    ...
    def cross_sectional_regression(
        ...
        参数:
            factor_values: 因子值 (index=date, columns=factor_names)
            stock_returns: 股票收益率
            risk_factors: 风险因子
