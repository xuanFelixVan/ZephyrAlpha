...
| 阶段 | 输入 | 输出 | 评判标准 |
|------|------|------|----------|
| 单因子验证 | 因子值、收益率 | IC、IR、收益 | IC>0.03, IR>0.3 |
| 多因子合成 | 多个单因子 | 合成因子 | IC提升>20% |
| 样本外验证 | 因子值、收益率 | OOS IC、衰减率 | 衰减<30% |
...
class SingleFactorValidator:
    ...
    def validate(self, factor_data: DataFrame, price_data: DataFrame) -> ValidationReport:
        ...
        参数:
            factor_data: 因子值 (index=date, columns=symbols)
            price_data: 价格数据
    ...
    def monitor(self, factor_id: str, ic_metrics: ICMetrics) -> List[Alert]:
        ...
        if positive_rate < self.thresholds['ic_positive_rate_critical']:
            alerts.append(Alert(
                level='CRITICAL',
                factor_id=factor_id,
                message=f"因子{factor_id}IC正收益比例低于50%",
...
| 指标 | 值 | 评判 |
|------|-----|------|
| IC均值 | {ic_mean:.4f} | {ic_judge} |
| IC_IR | {ic_ir:.4f} | {ir_judge} |
| IC衰减(5日) | {ic_decay_5:.2%} | {decay_judge} |

### 收益指标
| 指标 | 值 | 评判 |
|------|-----|------|
| 年化收益率 | {annual_return:.2%} | {return_judge} |
| 最大回撤 | {max_drawdown:.2%} | {dd_judge} |
| 夏普比率 | {sharpe:.2f} | {sharpe_judge} |

### 换手率
| 指标 | 值 | 评判 |
|------|-----|------|
| 月均换手率 | {turnover:.2%} | {turnover_judge} |
...
| 模块 | 接口 | 说明 |
|------|------|------|
| FactorCalculator | calculate() | 获取因子值 |
...
| 模块 | 接口 | 说明 |
|------|------|------|
| FactorRepository | save() | 存储验证通过的因子 |
