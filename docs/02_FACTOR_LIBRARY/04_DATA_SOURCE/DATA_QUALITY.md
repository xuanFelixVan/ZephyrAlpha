...
owner: 首席文档架构师
standard_type: 数据质量管理标准
applicable_scope: 数据质量控制与管理
...
### 1.1 目标
确保进入系统的数据满足量化分析的质量要求，防止"垃圾进，垃圾出"（GIGO: Garbage In, Garbage Out）。
...
class DataQualityResult:
    ...
    record_count: int = 0
    issue_count: int = 0
    quality_score: float = 1.0  # 0-1的质量分数
    ...
    def check_and_raise(self, df: pd.DataFrame, data_type: str = "ohlcv") -> pd.DataFrame:
        ...
        if not result.passed:
            ...
            raise DataQualityError(
                f"数据质量检查失败 ({result.issue_count} issues):\n{issues_str}"
            )
        return df
```
...
class MissingValueConfig:
    ...
    critical_fields: List[str] = None     # 关键字段，缺失直接失败
    fill_strategies: Dict[str, str] = None  # 填充策略
        ...
        for col in df.columns:
            ...
            if config.critical_fields and col in config.critical_fields:
                if missing_ratio > 0:
                    result.add_issue(DataIssue(
                        ...
                        category="missing",
                        message=f"关键字段缺失 {missing_count} 条 ({missing_ratio:.2%})",
                        field=col
                    ))
                    continue
            ...
            if missing_ratio > config.max_missing_ratio:
                result.add_issue(DataIssue(
                    severity=Severity.CRITICAL,
                    category="missing",
                    message=f"缺失率 {missing_ratio:.2%} 超过阈值 {config.max_missing_ratio:.2%}",
                    field=col
                ))
            else:
                result.add_issue(DataIssue(
                    ...
                    category="missing",
                    message=f"字段缺失 {missing_count} 条 ({missing_ratio:.2%})",
                    field=col
                ))
```
...
class MissingValueFiller:
    ...
    STRATEGIES = {
        ...
        "linear": lambda s: s.interpolate(method='linear'), # 线性插值
        "mean": lambda s: s.fillna(s.mean()),               # 均值填充
        "median": lambda s: s.fillna(s.median()),           # 中位数填充
        "zero": lambda s: s.fillna(0),                      # 零填充
        "ffill_then_mean": lambda s: s.fillna(method='ffill').fillna(s.mean()),
    }
        ...
        if 'high' in df.columns and 'low' in df.columns:
            ...
            if len(invalid_range) > 0:
                result.add_issue(DataIssue(
                    severity=Severity.CRITICAL,
                    category="validity",
                    message=f"存在 {len(invalid_range)} 条最高价 < 最低价的记录",
                    field="high/low"
                ))
        ...
        for _, row in df.iterrows():
            ...
            if not (row['low'] <= row['close'] <= row['high']):
                result.add_issue(DataIssue(
                    severity=Severity.WARNING,
                    category="validity",
                    message=f"收盘价 {row['close']} 超出范围 [{row['low']}, {row['high']}]",
                    field="close",
                    row_index=_
                ))
        ...
        if 'volume' in df.columns:
            ...
            if len(invalid_volume) > 0:
                result.add_issue(DataIssue(
                    severity=Severity.CRITICAL,
                    category="validity",
                    message=f"存在 {len(invalid_volume)} 条负成交量记录",
                    field="volume"
                ))
```
...
class SecurityConfig:
    ...
    valid_tick_pattern: str = r"^\d{6}\.[A-Z]$"  # 6位数字.大写字母
            ...
            if len(missing_dates) > 0:
                ...
                if missing_ratio > 0.1:  # 超过10%缺失
                    result.add_issue(DataIssue(
                        severity=Severity.WARNING,
                        category="coverage",
                        message=f"日期覆盖缺失 {len(missing_dates)} 天 ({missing_ratio:.1%})",
                        field="date"
                    ))
        ...
        if 'symbol' in df.columns:
            ...
            if missing_symbols:
                result.add_issue(DataIssue(
                    severity=Severity.WARNING,
                    category="coverage",
                    message=f"缺少 {len(missing_symbols)} 只股票数据: {list(missing_symbols)[:5]}",
                    field="symbol"
                ))
        ...
        if 'date' in df.columns and 'symbol' in df.columns:
            ...
            if key_dup_count > 0:
                result.add_issue(DataIssue(
                    severity=Severity.CRITICAL,
                    category="duplicate",
                    message=f"存在 {key_dup_count} 条日期+股票代码重复的记录",
                    field="date+symbol"
                ))

        # 3. 重复日期检查 (同一股票同一日期多条记录)
        if 'date' in df.columns and 'symbol' in df.columns:
            ...
            if len(multi_records) > 0:
                result.add_issue(DataIssue(
                    severity=Severity.WARNING,
                    category="duplicate",
                    message=f"存在 {len(multi_records)} 个日期存在多条记录",
                    field="date+symbol"
                ))
```
...
class OutlierDetector:
    ...
    def iqr(series: pd.Series, factor: float = 1.5) -> pd.Series:
        """
        IQR方法
        超过 Q1 - factor*IQR 或 Q3 + factor*IQR 的视为异常
        """
    ...
    def get_outlier_stats(cls, df: pd.DataFrame,
        ...
        for method, outliers in results.items():
            count = outliers.sum()
            ratio = count / len(df)
            indices = df[outliers].index.tolist()[:10]  # 最多显示10个
...
================================================================================
数据名称: {data_name}
检查时间: {result.checked_at.strftime('%Y-%m-%d %H:%M:%S')}
数据记录数: {result.record_count}
--------------------------------------------------------------------------------
检查结果: {'✅ 通过' if summary['passed'] else '❌ 失败'}
质量评分: {summary['quality_score']:.1%}

问题汇总:
  - 致命问题: {summary['critical_count']} 个
  - 警告问题: {summary['warning_count']} 个
  - 提示信息: {summary['info_count']} 个
...
"""
                if issue.value is not None:
                    report += f"    当前值: {issue.value}\n"
                if issue.expected is not None:
                    report += f"    期望值: {issue.expected}\n"
...
"""
    ...
    def generate_html(result: DataQualityResult,
        ...
        <h1>数据质量检查报告</h1>
        <table border="1">
            <tr><th>检查时间</th><td>{result.checked_at}</td></tr>
            <tr><th>记录数</th><td>{result.record_count}</td></tr>
...
class DataQualityConfig:
    ...
    max_daily_change: float = 0.5  # 日内最大变化50%
...
# 4. 生成报告
report = DataQualityReport.generate(result, df, "A股数据")
print(report)
...
data_quality:
  ...
  business_rules:
    ...
    max_volume_spike: 10.0      # 10倍
    max_high_low_range: 0.5     # 50%
```
