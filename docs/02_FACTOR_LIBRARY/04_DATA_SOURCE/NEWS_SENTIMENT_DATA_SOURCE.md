...
| 频率级别 | 更新频率 | 因子类别 | 代表因子 |
|----------|----------|----------|----------|
| **高频** | tick/分钟 | 行情因子 | 价格/成交量/波动率/订单簿 |
| **日频** | 日更(盘后) | 资金流/技术 | 北向/融资融券/换手率/RSI/MACD |
| **周频** | 周更 | 部分另类 | 搜索指数(周级) |
| **低频** | 季更/月更 | 基本面/宏观 | ROE/PE/GDP/CPI |

#### 8.4.2 详细因子更新频率表

| 因子类别 | 具体因子 | 更新频率 | 计算来源 | 数据源 |
|----------|----------|----------|----------|--------|
| **行情类** | 实时价格 | tick | 实时行情 | iFinD |
| | 成交量/成交额 | tick | 实时行情 | iFinD |
| | 波动率(HV) | 分钟 | 分钟数据计算 | iFinD |
| | K线(1/5/15/60分钟) | 分钟 | K线合成 | iFinD |
| **资金流** | 北向资金 | **日更(盘后)** | 沪深港通 | 东方财富 |
| | 融资融券 | **日更(盘后)** | 交易所 | 东方财富 |
| | 龙虎榜 | **日更(盘后)** | 交易所 | 上交所/深交所 |
| | 大宗交易 | **日更(盘后)** | 交易所 | 东方财富 |
| **技术类** | RSI/MACD | 分钟 | K线计算 | iFinD |
| | 均线(MA) | 分钟 | K线计算 | iFinD |
| | 布林带 | 分钟 | K线计算 | iFinD |
| **基本面** | PE/PB/PS | **日更** | 行情+财务 | iFinD |
| | ROE/毛利率 | **季更** | 财报 | iFinD |
| | 资产负债率 | **季更** | 财报 | iFinD |
| | 营收/利润 | **季更** | 财报 | iFinD |
...
|----------|----------|-----------|----------|
| **分钟级** | ~20% | 技术指标、趋势类 | iFinD实时 |
| **日频** | ~50% | 大部分量化因子 | iFinD日更 |
...
| 因子类型 | 数量估算 | 计算策略 | 存储位置 |
|----------|----------|----------|----------|
| **日频因子** | ~3000个 | iFinD预计算 | 本地可选存储 |
| **分钟因子** | ~1000个 | iFinD实时计算 | Redis缓存 |
| **基础因子** | ~2000个 | iFinD历史计算 | 本地存储K线后计算 |
...
│  Step 2: 消息队列 (10-50ms)                                        │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ Redis Pub/Sub 或 Kafka → 异步分发到各计算节点               │   │
│  │                                                              │   │
│  │ 用途:                                                        │   │
│  │ ├── 分发给多个策略实例                                       │   │
│  │ └── 临时缓存(防止丢包)                                       │   │
...
│  │ 数据分层存储:                                               │   │
│  │ ├── 热数据: Redis (近60日1分钟K线)                         │   │
│  │ ├── 温数据: ClickHouse (近1年5分钟K线)                    │   │
│  │ └── 冷数据: ClickHouse压缩 (1年+历史)                      │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                              ↓                                      │
│  Step 4: 因子计算 (批处理)                                           │
│  ┌─────────────────────────────────────────────────────────────┐   │
