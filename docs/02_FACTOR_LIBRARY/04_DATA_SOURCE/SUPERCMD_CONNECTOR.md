...
> **优先级**: P1 (重要)
> **预计开发时间**: 12小时
> **迁移来源**: [L0_SUPERCMD.md](../../../06_ARCHIVE/architecture_v4/module_designs/layer_0/L0_SUPERCMD.md) (已归档)
...
| 错误类型 | 错误码 | 处理方式 | 恢复策略 |
|----------|--------|----------|----------|
| API认证失败 | ERR_SUPERCMD_001 | 记录日志，告警通知 | 检查API密钥，人工介入 |
| 网络连接超时 | ERR_SUPERCMD_002 | 自动重试，指数退避 | 检查网络连接 |
| 数据限制超限 | ERR_SUPERCMD_003 | 返回缓存数据，告警 | 升级API套餐或优化调用 |
| 数据格式错误 | ERR_SUPERCMD_004 | 记录详细错误，返回空数据 | 联系SuperCommand技术支持 |
...
| websocket-client | 强依赖 | >=1.0.0 | 无（实时推送） |
...
| 集成对象 | 集成方式 | 协议 | 频率 |
|----------|----------|------|------|
| 实时行情（微观执行层） | 行情数据推送 | WebSocket | 实时 |
| 选股策略（中观策略层） | 选股结果推送 | REST API | 日频 |
...
| API兼容性测试 | 100% | 手动测试 | 每次SuperCommand API更新 |
...
| 决策ID | 决策内容 | 决策理由 | 决策日期 |
|--------|----------|----------|----------|
| DEC_001 | 使用requests+websocket | 成熟稳定，支持实时推送 | 2026-04-02 |
