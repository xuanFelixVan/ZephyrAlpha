...
│  │  - 实时流接入  - 批量导入  - API拉取  - 文件上传         │
│  └───────────────────────────────────────────────────────────┘
│                           │                                  │
│                           ↓                                  │
│  ┌───────────────────────────────────────────────────────────┐
│  │              数据清洗层 (Data Cleaning)                   │
│  │  - 缺失值处理  - 异常值检测  - 数据对齐  - 复权处理       │
│  └───────────────────────────────────────────────────────────┘
│                           │                                  │
│                           ↓                                  │
│  ┌───────────────────────────────────────────────────────────┐
│  │              数据存储层 (Data Storage)                    │
│  │  - Redis(热数据)  - PostgreSQL(温数据)  - 文件(冷数据)   │
│  └───────────────────────────────────────────────────────────┘
│                           │                                  │
│                           ↓                                  │
│  ┌───────────────────────────────────────────────────────────┐
...
|----------|----------|----------|----------|------|
| OHLCV | 实时/日级 | Redis+DB | 10年 | 股价、成交量 |
| 财务数据 | 季级 | DB | 永久 | 营收、利润 |
| 因子数据 | 日级 | DB | 5年 | Alpha因子值 |
| 风险数据 | 实时 | Redis | 1年 | VaR、波动率 |
...
class DataStorage:
    ...
    def __init__(self):
        self.stores = {
            'hot': RedisStore(),      # 热数据: Redis
            'warm': PostgresStore(),   # 温数据: PostgreSQL
            'cold': FileStore()       # 冷数据: 文件
        }
...
class DataService:
    ...
    索引: DAT_001-M04
    上游: DataStorage
    下游: FactorCalculator, StrategyEngine等
    """
...
| 数据源 | 接口 | 说明 |
|--------|------|------|
| 交易所API | REST/WebSocket | 实时行情、历史K线 |
| 财务数据库 | SQL | 财报数据 |
