---
module_id: DOC_FACTOR_LIBRARY_INDEX_001
version: 2.0.0
status: Active
created_date: 2026-04-03
last_updated: 2026-04-03
owner: 因子库架构师
standard_type: 专业量化机构目录索引
applicable_scope: 02_FACTOR_LIBRARY目录
compliance_level: 专业标准
parent_document: ../INDEX.md
implementation_status: 活跃维护
---

# 因子库目录索引

> **版本**: v5.3  
> **架构**: 三级时间框架融合架构  
> **最后更新**: 2026-04-03  
> **维护者**: 因子库架构师

---

## 🎯 目录职责

本目录存放因子库相关文档，包括因子方法论、因子计算、因子回测、数据源接口等内容。

---

## 📚 核心文档

### 系统清单

| 文档名称 | 说明 | 重要度 |
|---------|------|--------|
| [系统清单](../System_Manifest.md) | 系统清单（全局） | ⭐⭐⭐⭐⭐ |
| [README](./README.md) | 因子库概述 | ⭐⭐⭐⭐⭐ |
| [SITEMAP](./SITEMAP.md) | 因子库文档地图 | ⭐⭐⭐⭐⭐ |
| [因子库对接蓝图](./00_INDEX/FACTOR_LIBRARY.md) | 因子库对接蓝图 | ⭐⭐⭐⭐⭐ |
| [FAQ](./FAQ.md) | 常见问题解答 | ⭐⭐⭐⭐ |
| [交接文档](./HANDOVER.md) | 项目交接文档 | ⭐⭐⭐⭐ |
| [知识管理](./KNOWLEDGE_MANAGEMENT.md) | 知识管理体系 | ⭐⭐⭐⭐ |
| [模块设计计划](./MODULE_DESIGN_PLAN.md) | 模块设计规划 | ⭐⭐⭐⭐ |
| [优化总结](./OPTIMIZATION_SUMMARY.md) | 优化工作总结 | ⭐⭐⭐⭐ |
| [审计报告](./99_AUDIT_REPORT.md) | 审计报告汇总 | ⭐⭐⭐⭐ |

---

## 📖 因子方法论 (01_STANDARDS)

### 核心标准

| 文档名称 | 说明 | 重要度 |
|---------|------|--------|
| [因子分类学](./01_STANDARDS/FACTOR_TAXONOMY.md) | 因子注册表与分类体系 | ⭐⭐⭐⭐⭐ |
| [因子计算框架](./01_STANDARDS/FACTOR_CALCULATION_FRAMEWORK.md) | 因子计算框架 | ⭐⭐⭐⭐⭐ |
| [因子管理标准](./01_STANDARDS/FACTOR_MANAGEMENT_STANDARD.md) | 因子管理标准（含筛选策略） | ⭐⭐⭐⭐⭐ |
| [因子定义框架](./01_STANDARDS/T.02.FE001.factor_definition.md) | 因子定义框架 | ⭐⭐⭐⭐⭐ |

### 因子研究

| 文档名称 | 说明 | 重要度 |
|---------|------|--------|
| [因子筛选策略](./01_STANDARDS/FACTOR_SCREENING_STRATEGY.md) | 5900因子筛选流程（4阶段） | ⭐⭐⭐⭐⭐ |
| [因子挖掘指南](./01_STANDARDS/FACTOR_MINING_GUIDE.md) | 因子挖掘方法论 | ⭐⭐⭐⭐ |
| [因子验证指南](./01_STANDARDS/FACTOR_VALIDATION_GUIDE.md) | 因子验证流程 | ⭐⭐⭐⭐ |
| [研究管理](./01_STANDARDS/RESEARCH_MANAGEMENT.md) | 研究管理规范 | ⭐⭐⭐⭐ |
| [未来因子工具](./01_STANDARDS/FUTURE_FACTOR_TOOLS.md) | 未来因子工具规划 | ⭐⭐⭐ |

### 因子处理

| 文档名称 | 说明 | 重要度 |
|---------|------|--------|
| [因子预处理](./01_STANDARDS/FACTOR_PREPROCESSING.md) | 因子预处理方法 | ⭐⭐⭐⭐ |
| [因子中性化](./01_STANDARDS/FACTOR_NEUTRALIZATION.md) | 因子中性化处理 | ⭐⭐⭐⭐ |
| [因子收益分析](./01_STANDARDS/FACTOR_RETURN_ANALYSIS.md) | 因子收益分析 | ⭐⭐⭐⭐ |
| [因子合成](./01_STANDARDS/FACTOR_SYNTHESIS.md) | 因子合成方法 | ⭐⭐⭐⭐ |
| [IC分析](./01_STANDARDS/IC_ANALYSIS.md) | IC分析方法 | ⭐⭐⭐⭐⭐ |

### 回测标准

| 文档名称 | 说明 | 重要度 |
|---------|------|--------|
| [回测标准](./01_STANDARDS/BACKTEST_STANDARDS.md) | 回测标准规范 | ⭐⭐⭐⭐⭐ |
| [技术指标](./01_STANDARDS/TECHNICAL_INDICATORS.md) | 技术指标因子 | ⭐⭐⭐⭐ |

---

## 🎯 Alpha因子 (02_ALPHA_FACTORS_INDEX)

| 文档名称 | 说明 | 重要度 |
|---------|------|--------|
| [Alpha因子索引](./02_ALPHA_FACTORS_INDEX.md) | Alpha因子索引 | ⭐⭐⭐⭐⭐ |

---

## ⚠️ 风险因子 (03_RISK_FACTORS)

| 文档名称 | 说明 | 重要度 |
|---------|------|--------|
| [Barra风格因子](./03_RISK_FACTORS/T.03.RF001.barra_style_factors.md) | Barra风格因子 | ⭐⭐⭐⭐⭐ |
| [行业因子](./03_RISK_FACTORS/T.03.RF002.industry_factors.md) | 行业因子 | ⭐⭐⭐⭐⭐ |
| [尾部风险因子](./03_RISK_FACTORS/T.03.RF003.tail_risk_factors.md) | 尾部风险因子 | ⭐⭐⭐⭐⭐ |
| [Barra优化器](./03_RISK_FACTORS/T.03.RM003.barra_optimizer.md) | Barra优化器 | ⭐⭐⭐⭐ |
| [因子透明度报告](./03_RISK_FACTORS/T.03.RM004.factor_transparency_report.md) | 因子透明度报告 | ⭐⭐⭐⭐ |

---

## 📊 数据源 (04_DATA_SOURCE)

### 核心文档

| 文档名称 | 说明 | 重要度 |
|---------|------|--------|
| [数据源概述](./04_DATA_SOURCE/README.md) | 数据源概述 | ⭐⭐⭐⭐⭐ |
| [数据需求](./04_DATA_SOURCE/DATA_REQUIREMENTS.md) | 数据需求分析 | ⭐⭐⭐⭐⭐ |
| [数据采集](./04_DATA_SOURCE/DATA_ACQUISITION.md) | 数据采集流程 | ⭐⭐⭐⭐⭐ |
| [数据质量](./04_DATA_SOURCE/DATA_QUALITY.md) | 数据质量管理 | ⭐⭐⭐⭐⭐ |
| [数据源适配器](./04_DATA_SOURCE/DATA_SOURCE_ADAPTERS.md) | 数据源适配器 | ⭐⭐⭐⭐⭐ |

### 数据接口

| 文档名称 | 说明 | 重要度 |
|---------|------|--------|
| [QMT接口](./04_DATA_SOURCE/QMT_INTERFACE.md) | QMT数据接口 | ⭐⭐⭐⭐⭐ |
| [iFind连接器](./04_DATA_SOURCE/IFIND_CONNECTOR.md) | iFind数据接口 | ⭐⭐⭐⭐⭐ |
| [Baostock连接器](./04_DATA_SOURCE/BAOSTOCK_CONNECTOR.md) | Baostock数据接口 | ⭐⭐⭐⭐⭐ |
| [SuperCmd连接器](./04_DATA_SOURCE/SUPERCMD_CONNECTOR.md) | SuperCmd数据接口 | ⭐⭐⭐⭐ |
| [免费数据源](./04_DATA_SOURCE/FREE_DATA_SOURCES.md) | 免费数据源列表 | ⭐⭐⭐⭐ |

### 数据分析

| 文档名称 | 说明 | 重要度 |
|---------|------|--------|
| [相关性分析](./04_DATA_SOURCE/CORRELATION_ANALYSIS.md) | 相关性分析方法 | ⭐⭐⭐⭐ |
| [统计工具](./04_DATA_SOURCE/STATISTICAL_TOOLS.md) | 统计工具集 | ⭐⭐⭐⭐ |
| [宏观数据](./04_DATA_SOURCE/MACRO_DATA.md) | 宏观数据源 | ⭐⭐⭐⭐ |
| [新闻情感数据源](./04_DATA_SOURCE/NEWS_SENTIMENT_DATA_SOURCE.md) | 新闻情感数据源 | ⭐⭐⭐⭐ |
| [A股历史数据处理蓝图](./04_DATA_SOURCE/A_SHARE_HISTORICAL_DATA_PROCESSING_BLUEPRINT.md) | A股历史数据处理蓝图 | ⭐⭐⭐⭐ |

### iFind数据

| 文档名称 | 说明 | 重要度 |
|---------|------|--------|
| [因子主索引](./04_DATA_SOURCE/IFIND/FACTOR_MASTER_INDEX.md) | iFind因子主索引 | ⭐⭐⭐⭐⭐ |
| [THS_BD完整指标列表](./04_DATA_SOURCE/IFIND/financial_statements/THS_BD_COMPLETE_INDICATOR_LIST.md) | THS_BD完整指标列表 | ⭐⭐⭐⭐⭐ |

### 质量管理

| 文档名称 | 说明 | 重要度 |
|---------|------|--------|
| [数据质量控制体系](./04_DATA_SOURCE/QUALITY_MANAGEMENT/DATA_QUALITY_CONTROL_SYSTEM.md) | 数据质量控制体系 | ⭐⭐⭐⭐⭐ |

---

## 🔬 因子回测 (05_BACKTEST)

### 核心文档

| 文档名称 | 说明 | 重要度 |
|---------|------|--------|
| [回测概述](./05_BACKTEST/README.md) | 回测概述 | ⭐⭐⭐⭐⭐ |
| [因子验证蓝图](./05_BACKTEST/FACTOR_VALIDATION_BLUEPRINT.md) | 因子验证蓝图 | ⭐⭐⭐⭐⭐ |
| [因子衰减](./05_BACKTEST/06_FACTOR_DECAY.md) | 因子衰减分析 | ⭐⭐⭐⭐⭐ |
| [分层回测](./05_BACKTEST/07_LAYERED_BACKTEST.md) | 分层回测方法 | ⭐⭐⭐⭐⭐ |
| [过拟合测试](./05_BACKTEST/09_OVERFITTING_TEST.md) | 过拟合测试 | ⭐⭐⭐⭐⭐ |
| [相关性矩阵](./05_BACKTEST/CORRELATION_MATRIX.md) | 相关性矩阵 | ⭐⭐⭐⭐⭐ |

### 回测重组

| 文档名称 | 说明 | 重要度 |
|---------|------|--------|
| [回测重组](./05_BACKTEST_REORGANIZATION.md) | 回测重组方案 | ⭐⭐⭐⭐ |
| [宽度指标](./05_BREADTH_INDICATORS.md) | 宽度指标 | ⭐⭐⭐⭐ |

### 价值因子回测

| 文档名称 | 说明 | 重要度 |
|---------|------|--------|
| [PE_TTM回测](./05_BACKTEST/value_factors/PE_TTM_BACKTEST.md) | PE_TTM回测报告 | ⭐⭐⭐⭐ |
| [PE_TTM IC分析](./05_BACKTEST/value_factors/PE_TTM_IC.md) | PE_TTM IC分析 | ⭐⭐⭐⭐ |

---

## 📋 因子注册 (06_REGISTRY)

| 文档名称 | 说明 | 重要度 |
|---------|------|--------|
| [因子目录](./06_REGISTRY/FACTOR_CATALOG.md) | 因子目录 | ⭐⭐⭐⭐⭐ |

---

## 📈 因子监控 (07_FACTOR_MONITORING)

| 文档名称 | 说明 | 重要度 |
|---------|------|--------|
| [因子监控](./07_FACTOR_MONITORING/FACTOR_MONITORING.md) | 因子监控体系 | ⭐⭐⭐⭐⭐ |
| [AI因子管家](./07_FACTOR_MONITORING/AI_FACTOR_AGENT.md) | AI因子管家 | ⭐⭐⭐⭐⭐ |

---

## 🤖 AI增强 (10_MANUAL)

| 文档名称 | 说明 | 重要度 |
|---------|------|--------|
| [因子库手册](./10_MANUAL/FACTOR_LIBRARY_MANUAL.md) | 因子库使用手册 | ⭐⭐⭐⭐⭐ |

---

## 🗂️ 子目录

| 目录名称 | 说明 | 文档数量 |
|---------|------|---------|
| [00_GOVERNANCE/](./00_GOVERNANCE/) | 治理文档 | 1 |
| [00_INDEX/](./00_INDEX/) | 索引文档 | 3 |
| [01_STANDARDS/](./01_STANDARDS/) | 方法论文档 | 13 |
| [03_RISK_FACTORS/](./03_RISK_FACTORS/) | 风险因子 | 5 |
| [04_DATA_SOURCE/](./04_DATA_SOURCE/) | 数据源 | 15+ |
| [05_BACKTEST/](./05_BACKTEST/) | 回测文档 | 10+ |
| [06_REGISTRY/](./06_REGISTRY/) | 因子注册 | 1 |
| [07_FACTOR_MONITORING/](./07_FACTOR_MONITORING/) | 因子监控 | 2 |
| [10_MANUAL/](./10_MANUAL/) | 手册文档 | 1 |

---

## 📖 快速导航

### 新手入门

1. 阅读 [README.md](./README.md) - 因子库概述
2. 阅读 [系统清单](../System_Manifest.md) - 系统清单（全局）
3. 阅读 [因子分类学](./01_STANDARDS/FACTOR_TAXONOMY.md) - 因子分类体系

### 因子开发

1. 阅读 [01_STANDARDS/FACTOR_CALCULATION_FRAMEWORK.md](./01_STANDARDS/FACTOR_CALCULATION_FRAMEWORK.md) - 因子计算框架
2. 阅读 [05_BACKTEST/FACTOR_VALIDATION_BLUEPRINT.md](./05_BACKTEST/FACTOR_VALIDATION_BLUEPRINT.md) - 因子验证
3. 阅读 [01_STANDARDS/IC_ANALYSIS.md](./01_STANDARDS/IC_ANALYSIS.md) - IC分析

---

## 🔗 相关链接

- [系统主索引](../INDEX.md)
- [框架设计索引](../01_FRAMEWORK/INDEX.md)
- [实施层索引](../05_IMPLEMENTATION/README.md)
