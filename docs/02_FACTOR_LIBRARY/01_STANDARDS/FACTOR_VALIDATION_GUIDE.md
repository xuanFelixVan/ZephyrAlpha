...
def calculate_ic_series(factor_data, returns_data, forward_period=5):
    ...
    Args:
        ...
        returns_data: 收益率数据 (date x stock)
...
| 指标 | 计算方法 | 最低要求 | 良好标准 | 优秀标准 |
|------|----------|----------|----------|----------|
| **IC均值** | IC序列均值 | > 0.02 | > 0.035 | > 0.05 |
| **ICIR** | IC均值 / IC标准差 | > 0.3 | > 0.5 | > 1.0 |
| **IC胜率** | IC > 0的比例 | > 55% | > 60% | > 65% |
| **IC t值** | IC均值 / IC标准误 | > 2.0 | > 3.0 | > 5.0 |
...
def analyze_ic_decay(ic_series, recent_window=60, baseline_window=252):
    ...
    Args:
        ic_series: IC序列
        recent_window: 近期窗口（天）
        baseline_window: 基准窗口（天）
...
| 指标 | 计算方法 | 最低要求 | 良好标准 | 优秀标准 |
|------|----------|----------|----------|----------|
| **年化收益** | 复合年化收益率 | > 5% | > 10% | > 15% |
| **夏普比率** | 年化收益 / 年化波动 | > 0.5 | > 1.0 | > 1.5 |
| **最大回撤** | 最大回撤幅度 | < 30% | < 20% | < 10% |
| **胜率** | 盈利月份比例 | > 50% | > 55% | > 60% |
| **换手率** | 年化换手率 | < 500% | < 300% | < 200% |
...
def test_time_stability(ic_series, n_periods=4):
    ...
    # 计算IC相关性
    ic_correlation = np.corrcoef(period_ics[:-1], period_ics[1:])[0, 1]
    ...
    return {
        ...
        'stability_status': '稳定' if ic_correlation > 0.7 else 
                           '中等' if ic_correlation > 0.5 else '不稳定'
    }
```
...
def test_industry_stability(factor_data, returns_data, industry_mapping):
    ...
    Args:
        factor_data: 因子数据
        returns_data: 收益率数据
        industry_mapping: 行业映射 (stock -> industry)
    
    Returns:
        行业稳定性测试结果
    """
    # 按行业分组
    industries = industry_mapping.unique()
    industry_ics = {}
    ...
    # 计算IC标准差
    ic_std = np.std(list(industry_ics.values()))
...
def calculate_factor_correlation(new_factor, existing_factors):
    ...
    Args:
        new_factor: 新因子值
        existing_factors: 现有因子字典 {factor_id: factor_values}
    ...
    for factor_id, factor_values in existing_factors.items():
        # 对齐日期和股票
        common_index = new_factor.index.intersection(factor_values.index)
...
def out_of_sample_test(factor_data, returns_data, train_ratio=0.7):
    ...
    return {
        ...
        'overfit_status': '严重过拟合' if ic_decay > 0.7 else 
                         '过拟合' if ic_decay > 0.5 else '正常'
    }
```
...
def parameter_sensitivity_test(factor_func, param_ranges, data):
    ...
    return {
        ...
        'sensitivity_status': '高敏感' if ic_std > 0.02 else 
                             '中等敏感' if ic_std > 0.01 else '低敏感'
    }
```
...
class FactorValidator:
    ...
    def validate(self):
        ...
        # 相关性分析
        results['correlation'] = self.validate_correlation()
        
        # 过拟合检验
        results['overfit'] = self.validate_overfit()
