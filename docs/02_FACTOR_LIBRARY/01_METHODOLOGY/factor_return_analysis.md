...
class GroupBacktest:
    ...
    def run_group_test(
        ...
        if 0 in results['group_returns'] and n_groups - 1 in results['group_returns']:
            long_returns = pd.Series(results['group_returns'][0])  # Top组(因子值最高)
            short_returns = pd.Series(results['group_returns'][n_groups - 1])  # Bottom组
...
class TurnoverAnalyzer:
    ...
    def estimate_transaction_cost(
        ...
        return {
            ...
            'cost_adjusted_sharpe': None  # 需要结合收益计算
        }
```
...
├── factor_return_analysis.md   # 本文档 ⭐
└── factor_neutralization.md    # 中性化处理(待创建)
```
