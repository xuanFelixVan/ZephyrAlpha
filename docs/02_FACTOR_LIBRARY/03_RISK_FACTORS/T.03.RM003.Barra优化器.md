...
>
> **策略编号**：T.03.RM003
> **所属模块**：03_RISK_FACTORS
> **文档类型**：风险模型优化
> **优先级**：P1
>
> **配套文档**：
> - [1_Barra风格因子.md](./1_Barra风格因子.md) - 十大风格因子定义
> - [2_行业因子.md](./2_行业因子.md) - 申万行业分类
> - [3_尾部风险因子.md](./3_尾部风险因子.md) - CVaR/ES尾部风险
...
class BarraRiskModel:
    ...
    def calc_style_exposures(self, market_data: pd.DataFrame) -> pd.DataFrame:
        ...
        返回:
            style_exposures: 风格因子暴露度 (N x 10)
        """
    ...
    def calc_industry_exposures(self, industry_codes: pd.Series) -> pd.DataFrame:
        """
        计算行业因子暴露度 (dummy variable encoding)

        参数:
            industry_codes: 申万行业代码 Series

        返回:
            industry_exposures: 行业因子暴露度 (N x 28)
        """
    ...
    def calc_portfolio_risk(self, weights: np.ndarray,
        ...
        参数:
            weights: 组合权重 (N,)
            factor_exposures: 因子暴露度 (N, K)
            factor_cov: 因子协方差矩阵 (K, K)，使用实例缓存
            idio_var: 特异性方差 (N,)，使用实例缓存
...
class BarraOptimizer:
    """
    Barra优化器 - 基于Barra风险模型的组合权重优化

    支持的优化目标:
    - Mean-Variance: 最大化 (μ'w - λ * w'Σw)
    - Risk Parity: 各因子/资产风险贡献相等
    ...
    def mean_variance_optimize(
        ...
        参数:
            ...
            factor_exposures: 因子暴露度矩阵 (N, K)
            constraints: 额外约束列表
            bounds: 权重边界 (min, max)
            max_weight: 单只股票最大权重

        返回:
            optimal_weights: 最优权重 (N,)
        """
    ...
    def risk_parity_optimize(
        ...
        参数:
            ...
            asset_volatility: 资产波动率 (N,)，用于计算协方差矩阵
            constraints: 额外约束
        ...
        if result.success:
            ...
            print(f"风险平价优化未收敛: {result.message}")
            return x0
    ...
    def maximum_diversification_optimize(
        ...
        if result.success:
            ...
            print(f"最大分散化优化未收敛: {result.message}")
            return x0
...
def example_portfolio_optimization():
    ...
    print(f"系统性风险: {risk_decomposition['systematic_risk']:.4f}")
    print(f"特异性风险: {risk_decomposition['idiosyncratic_risk']:.4f}")
    print("\n因子风险贡献:")
...
| shrinkage_intensity | 0.3 | 协方差矩阵收缩强度 |
...
| 约束类型 | 实现方法 | 说明 |
|----------|----------|------|
| 权重和 | sum(weights) = 1 | 必须满足 |
| 边界约束 | bounds | min/max权重限制 |
| 行业中性 | industry_exposure @ w = 1/n | 各行业权重相等 |
| 风格中性 | factor_exposure @ w = target | 风格因子暴露度 |
| 换手率 | \|w - w_prev\| <= turnover | 交易成本控制 |
| 现金流 | price @ w <= cash | 最大买入金额 |
