---
module_id: INDEX_RISK_FACTORS_001
version: 1.0.1
status: Active
created_date: 2026-04-04
last_updated: 2026-04-04
owner: 首席文档架构师
standard_type: 目录索引
applicable_scope: 风险因子
compliance_level: 专业标准
parent_document: ../INDEX.md
implementation_status: 已完成
---

# 风险因子目录索引

> 风险因子的核心索引文件，提供Barra风格因子、行业因子等风险管理工具

---

## 📂 目录结构

### 核心文档

| 文档名称 | 说明 | 重要度 |
|---------|------|--------|
| [Barra风格因子](./T.03.RF001.barra_style_factors.md) | Barra风格因子定义和计算 | ⭐⭐⭐⭐⭐ |
| [行业因子](./T.03.RF002.industry_factors.md) | 行业分类因子和暴露度 | ⭐⭐⭐⭐⭐ |
| [尾部风险因子](./T.03.RF003.tail_risk_factors.md) | 极端市场风险因子 | ⭐⭐⭐⭐ |
| [Barra优化器](./T.03.RM003.barra_optimizer.md) | Barra模型优化工具 | ⭐⭐⭐⭐ |
| [因子透明度报告](./T.03.RM004.factor_transparency_report.md) | 因子透明度分析 | ⭐⭐⭐ |

---

## 🔍 快速导航

### Barra风格因子

- **规模因子 (Size)**: 市值规模效应
- **价值因子 (Value)**: B/P、E/P等估值指标
- **动量因子 (Momentum)**: 价格动量效应
- **波动率因子 (Volatility)**: 波动率效应
- **流动性因子 (Liquidity)**: 交易流动性

### 行业因子

- **中信一级行业**: 30个行业分类
- **中信二级行业**: 100+细分行业
- **申万行业**: 替代分类体系

---

## 📊 风险管理流程

```
风险因子识别 → 风险暴露计算 → 风险优化 → 风险监控
```

---

## 📚 相关文档

- [因子分类体系](../01_STANDARDS/FACTOR_TAXONOMY.md)
- [风险管理规则](../03_TRADING_TACTICS/09_RISK_RULES/RISK_RULE_ENGINE.md)
- [因子监控](../07_FACTOR_MONITORING/FACTOR_MONITORING.md)

---

> **最后更新**: 2026-04-04  
> **维护者**: 首席文档架构师
