---
module_id: KE-3563
title: 3.1 受控词表
category: governance_rule
ttl: permanent
doc_type: knowledge_entry
---

# 3.1 受控词表

3.1 受控词表

| 值 | 含义 | 对应目录 | 示例文件 |
|---|------|---------|---------|
| `meta` | 元规则——定义规则怎么写、怎么分类、怎么变更 | `meta/` | PS-STD-002, PS-STD-003, PS-STD-004 |
| `document` | 文档治理——命名、路径、编码、生命周期 | `governance/document/` | trae_028_doc_structure_naming.yaml |
| `ai` | AI 治理——自治权限、入职、幻觉自检 | `governance/ai/` | ai-onboarding-guide.md |
| `task` | 任务治理——任务卡、交接、裁定 | `governance/task/` | task-card-standard.md |
| `vibe_coding` | Vibe Coding 操作——上下文规则、状态机、可验证性 | `operational/vibe_coding/` | vibe-coding-session-state-runbook.md |
| `architecture` | 架构治理——模块注入、入职规则 | `governance/module/` | module-injection-rules-policy.md |
| `migration` | 迁移——迁移审计（该文件从未创建，2026-05-02 已标记 deprecated） | `operational/migration/` | — |
| `devops` | DevOps——pre-commit、CI、部署 | `operational/devops/` | pre-commit-simplification-plan.md |
| `coding` | 编码——代码风格、安全编码 | 待建立 | trae_028_doc_structure_naming.yaml |
