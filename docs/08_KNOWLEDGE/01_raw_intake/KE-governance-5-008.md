...
| 日期 | 版本 | 修改内容 |
|------|------|---------|
| 2026-05-06 | 2.7.0 | **唯一真源操作化升格——从声明式原则到五层操作闭环**。根源：MOD-INF-003/004→006 的反模式暴露了 "事后 SSoT 冲突裁决（MTH-008）治标不治根" 的病根——唯一真源只说了"发现重复就合并"，没说"创建前先查有没有重复"。修复：(1) 唯一真源规则新增事前检查维度——创建前 MUST 执行功能域重叠扫描（GOV-MOD-001 §7 #5）→ 禁止平行蓝图（PS-STD-005 §10 #7）；(2) 禁止行为新增第 4 条——禁止创建平行文件后合并；(3) 形成五层操作闭环：唯一真源原则 → GOV-MOD-001 §7 #5 准入门控 → PS-STD-005 §10 #7 架构标准 → GOV-MOD-003 MLC-001 生命周期前置 → blueprint-registry.yaml responsibility_domain 数据字段。对标：Lean（事前预防优于事后检查）+ ITIL Change Enablement（变更前影响评估）+ Toyota Jidoka（内置质检而非最终检验）。版本号 minor +1。 |
