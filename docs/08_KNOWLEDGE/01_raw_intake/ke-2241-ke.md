...
> 通俗解释：现在知识存进去后跟石沉大海一样——不知道 AI 到底用没用、用了有没有用。加了这四个字段，知识从"入库时猜质量"变成"运行时验证质量"。就像餐厅——不仅做出菜（G2），还要看客人吃没吃（`adoption_count`）、好不好吃（`helpfulness_score`）。`extraction_generation` 管的是另一个问题：知识被反复"蒸馏"时会不会变味——gen=0（Owner 原话）→真金；gen=3（AI 基于 AI 输出的再提取）→可能是镀金，需要重审原始来源。
