---
module_id: KE-governance-project_cleanliness_principle-003
...
| 规则 | 内容 | 大白话 |
|------|------|--------|
| **唯一真源** | 同一概念只在一个文件中定义。**事前**：创建新文件前，必须先检查责任是否已被已有文件覆盖（GOV-MOD-001 §7 #5 功能域重叠扫描 → PS-STD-005 §10 #7 禁止平行蓝图）。**事后**：发现重复定义 → 立即合并，不留"副本" | 一件事只有一本说明书。创建前先问"有人管这事了吗？"，创建后发现有人管了立刻合并 |
