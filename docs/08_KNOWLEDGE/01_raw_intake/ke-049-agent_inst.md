...
| **创建/操作任务卡** | `blueprint_decomposer.py` + `docs/03_modules/_domain-infra_ops/task-system/blueprint.md` | ~600 | ⚠️ **强制路径**：所有任务卡 MUST 通过 `decompose_blueprint()` 生成。禁止 AI 直接生成 `.md` 作为任务卡。唯一例外：MOD-INF-001（AI-GLM-5.1 直接产出，通过 `import_task_cards.py` 回填过一次） |
| **代码去重检查 / Monoculture 免疫**（MOD-INF-017） | `docs/03_modules/_domain-infra_ops/code-dedup-engine/blueprint.md` + `src/zephyr/infra_ops/code_dedup_engine/` | ~500 | ⚠️ 创建新代码前 MUST 先跑去重检查——RULE-EIGHT 强制。覆盖 65+ 模块：MinHash+LSH 扫描、BRS 单体文化评分、决策审计链、自我扫描。`pre-commit/verify_dedup.py` 自动触发 |
| **系统遥测 / 可观测性接入**（MOD-INF-015） | `docs/03_modules/_domain-infra_ops/system-telemetry/blueprint.md` + `src/zephyr/infra_ops/` | ~400 | ⚠️ 新模块需要遥测接入时必读。9 子系统设计（metrics/logs/traces/ai_behavior/archive/profiles/health/alerts/schema）。注意：蓝图 v0.9.0 设计已完备但代码实现滞后（~8%），施工时以蓝图为准 |
| **创建新脚本工具** | `scripts/governance/index.md` + `scripts/governance/script-manifest.yaml` + **`_shared/` API 速查（见 §8.2.1）** | ~1000 | ❌ 禁止创建脚本后不注册——违反 §6.5 入库强制约定 / ❌ 禁止本地重定义 `_shared/` 已有函数/常量 |
