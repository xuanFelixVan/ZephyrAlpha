...
| 枚举值 | 含义 | 进入条件（摘要） | 退出条件 |
|--------|------|---------|---------|
| `planned` | 规划中 | 新模块 ID 分配 | 准入通过（GOV-MOD-ALPHA_SIGNAL_DOMAIN）→ in_design |
...
**受控约束**：新增或删除阶段需要创建 KB 决策记录（见 §12 修改条件）。所有使用 `status` 字段的文件（GOV-MOD-ALPHA_SIGNAL_DOMAIN §7 #3 准入否决条件、GOV-MOD-005 INJ-004）必须从本枚举表中消费值。
