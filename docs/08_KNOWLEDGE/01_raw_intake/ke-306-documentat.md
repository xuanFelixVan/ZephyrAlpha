...
> 注：宪法层 ABS 编号为全局唯一前缀。领域级操作控制原则使用各自领域前缀（如 GOV-DOC-009 用 DOC-001~DOC-008，PS-STD-011 用 MTH-001），与 ABS 编号全局不冲突。详见 `../meta/behavior_boundaries_standard.yaml`（PS-STD-003）§3.4 编号注册表。
>
