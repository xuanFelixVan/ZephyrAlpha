...
- 跨表共享字段的 SSoT 归属 → 以 [registry_consistency_contract.yaml](../../_registry/catalogs/registry_consistency_contract.yaml) `cross_registry_rules` 为准
- 登记表存量清单 → 以 [registry-master-index.yaml](../../_registry/catalogs/registry-master-index.yaml) 为准
- AI 操作工具的具体调用方式 → 以 AGENTS.md 为准
- 各类工件的准入审批 → 以对应的准入标准为准（模块=GOV-MOD-ALPHA_SIGNAL_DOMAIN，规则=PS-STD-004，ADR=ADR 治理流程）
