...
| 2026-04-17 | **Stage 14 骨架对齐机构终局**：做埋雷清单审查（13 处）→ 全修 🔴🟡 7 条；建 KB 决策记录 canonical 家 + 写 ADR-0001/0002/0003；workspace 子目录改语义命名；文档标准升级到 v2.0.0（单 schema + 分阶段必填 + 受控词表 + append-only supersedes + 四符号状态）；triage-guide 从 4 类扩到 8 类；新增 adr-drafts / roadmaps / risk-registers / session-logs / archive 5 个骨架目录；新增 terminology-mapping-reference.md；新增结论 R26-R30。 |
