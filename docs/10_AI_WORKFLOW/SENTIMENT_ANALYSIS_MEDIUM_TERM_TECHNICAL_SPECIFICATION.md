...
**模块ID**: L3_MSA_001
**模块名称**: Multilingual Sentiment Analyzer (多语言情感分析器)
**版本**: v1.0.0
**状态**: 设计中
...
| properties | TEXT | 属性(JSON) | {"ticker": "AAPL", "sector": "Technology"} |
...
| properties | TEXT | 属性(JSON) | {"role": "CEO"} |
...
| partition | INTEGER | 分区号 | 0 |
| offset | INTEGER | 偏移量 | 12345 |
| key | TEXT | 消息键 | "news_001" |
| value | TEXT | 消息值(JSON) | {...} |
| timestamp | TIMESTAMP | 时间戳 | "2026-04-02 10:00:00" |
| processed | INTEGER | 是否处理 | 1 |

#### 处理结果表 (stream_results)
...
| 指标名称 | 目标值 | 测量方法 | 说明 |
|---------|--------|---------|------|
| 实体识别准确率 | > 85% | 人工标注验证 | 正确识别数/总数 |
| 关系抽取准确率 | > 80% | 人工标注验证 | 正确抽取数/总数 |
| 图谱查询响应时间 | < 1秒 | 记录查询耗时 | 平均响应时间 |
| 图谱构建速度 | > 100实体/分钟 | 统计构建速度 | 构建吞吐量 |
| 图谱存储大小 | < 10GB | 监控存储空间 | 图谱数据库大小 |
| 社区发现准确率 | > 75% | 人工评估 | 模块度评估 |

### 7.2 流式处理模块性能指标

| 指标名称 | 目标值 | 测量方法 | 说明 |
|---------|--------|---------|------|
| 消息吞吐量 | > 1000条/秒 | 统计处理速度 | 每秒处理消息数 |
| 处理延迟 | < 100ms | 记录端到端延迟 | 从接收到处理完成 |
| 系统可用性 | > 99.9% | 监控运行时间 | 正常时间/总时间 |
| 消息丢失率 | < 0.01% | 统计丢失消息 | 丢失数/总数 |
| Kafka延迟 | < 10ms | 监控Kafka延迟 | 消息在Kafka中的延迟 |
| Spark处理时间 | < 50ms | 记录Spark处理耗时 | 每批次处理时间 |
...
| 翻译速度 | > 50句/秒 | 统计翻译速度 | 每秒翻译句子数 |
| 多语言情感分析准确率 | > 80% | 测试集评估 | 各语言平均准确率 |
| 支持语言数量 | > 5 | 统计支持语言 | 支持的语言种类 |
| 内存使用 | < 8GB | 监控内存使用 | 峰值内存 |
