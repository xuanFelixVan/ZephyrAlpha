...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。实时质量检查结果上报、告警事件与审计留痕若通过接口/事件实现，须在该真源或本文后续接口说明中闭合。

## 验收标准（可检查）

- 能在本文中明确“实时检查触发 → 指标上报 → 告警 → 趋势分析/报告”最小闭环，并能在 `API_Contract.md` 中定位到相应的上报/告警/审计契约入口（或在本文写明豁免与补全计划）。

## 已知限制

- 指标口径与采样频率需在施工文档阶段与数据层实现对齐；以本节门禁为准。
