...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。数据浏览、查询、导出与权限控制若通过接口/事件暴露，须在该真源或本文后续接口说明中闭合。
...
| [Streamlit回测界面蓝图](./STREAMLIT_BACKTEST_INTERFACE_BLUEPRINT.md) | 回测系统 | 回测界面设计 |
| [性能分析界面蓝图](./PERFORMANCE_ANALYSIS_INTERFACE_BLUEPRINT.md) | 性能分析 | 性能分析界面设计 |
