...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。质量规则下发、豁免审批、审计查询与跨层协同事件若通过接口/事件实现，须在该真源或本文后续接口说明中闭合。

## 验收标准（可检查）

- Owner 能从本文中抽取“Layer 0/1/4/10 的分工与协同接口”清单，并能将关键接口映射到 `API_Contract.md`（或在本文写明豁免与补全计划）。
