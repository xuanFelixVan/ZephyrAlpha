---
module_id: 01_FRAMEWORK_P0_MODULES_INTEGRATION_CONFIG
layer: layer_01
version: 1.0.0
status: Active
priority: P0
...
- 全库 API 与事件约定真源：`API_Contract.md`。本文涉及的健康检查、服务端口、对外调用口径若形成“事实接口”，最终必须与该真源一致；若为临时约定，应在“已知限制”中标注并给出迁移到契约真源的计划。
...
- 本文中列出的 Docker Compose / 环境变量 / 依赖清单能组成一个自洽的“最小可运行集”（至少能解释：哪些服务启动、哪些端口暴露、健康检查如何判定）。

- 若本文出现健康检查 URL（如 `http://localhost:3000/health`、`http://localhost:5000/health`），应能在 `API_Contract.md` 中找到对应契约入口或在本文“已知限制”中列为未闭合项。

- 本文内所有相对链接应可点击跳转，且不引入新的无效链接。
...
- 本文包含若干实现侧文件路径示例（如 `scripts/monitor_all_p0_modules.py`、`config/*.yaml` 等），这些文件未必在当前阶段真实存在或与仓库结构一致；以施工阶段实际落地与 `API_Contract.md` 的契约口径为准。
