...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。文档上传/检索、权限变更、版本发布与归档若通过接口/事件实现，须在该真源或本文后续接口说明中闭合。

## 验收标准（可检查）

- 能在本文中明确“文档入库 → 权限控制 → 版本变更 → 归档留痕”的最小闭环，并能在 `API_Contract.md` 中定位到对应的文档/审计契约入口（或在本文写明豁免与补全计划）。
