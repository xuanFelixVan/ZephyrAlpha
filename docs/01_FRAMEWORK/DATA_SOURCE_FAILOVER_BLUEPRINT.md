...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。数据源健康检查、故障切换、回切与审计留痕若通过接口/事件实现，须在该真源或本文后续接口说明中闭合。

## 验收标准（可检查）

- 能在本文中明确至少一条“健康检查触发 → 故障判定 → 切换执行 → 一致性验证 → 审计留痕”的可检查闭环，并能在 `API_Contract.md` 中定位到相应的事件/审计契约入口（或在本文写明豁免与补全计划）。

## 已知限制

- 不同数据源的一致性与时延权衡需在施工文档阶段明确；以本节门禁为准。
