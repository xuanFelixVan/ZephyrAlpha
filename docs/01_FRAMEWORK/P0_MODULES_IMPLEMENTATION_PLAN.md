...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。本实施方案中涉及的服务对外口径（审计查询、模型登记/审批、报告生成/投递等）最终必须与该真源一致；如方案中新增接口/事件，应在真源中补齐或在“已知限制”中列出补全计划。

## 验收标准（可检查）

- 能从本文的 Phase 划分中抽取“模块 → 实施步骤 → 交付物”的清单，并可对每个交付物给出明确的完成判定（例如：服务可启动、接口可调用、测试可运行、报告可生成）。
- 本文若引用到对外接口/健康检查 URL/事件流，需能在 `API_Contract.md` 中找到对应契约入口或在本文“已知限制”中列出未闭合项。
- 本文内所有相对链接应可点击跳转，且不引入新的无效链接。

## 已知限制

- 本文为“实施方案”文档，部分命令/目录结构为示意（例如 `config/`、`tests/`、`docker-compose.*.yml` 等）；真实落地以施工阶段的实际目录与配置为准。
- 若后续对 P0 模块拆分/重命名，本文件中的模块清单与路径可能需要同步更新；以 `System_Manifest.md` 的索引与 `API_Contract.md` 的契约口径为权威。
