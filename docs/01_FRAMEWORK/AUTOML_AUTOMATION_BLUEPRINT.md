...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。AutoML 的任务提交、训练触发、模型注册与评估结果输出若通过接口/事件暴露，须在该真源或本文后续接口说明中闭合。

## 验收标准（可检查）

- 能在本文中明确“数据输入 → 特征/训练 → 评估 → 模型注册/部署”最小闭环，并能在 `API_Contract.md` 中定位到任务/结果/模型注册的契约入口（或在本文写明豁免与补全计划）。
