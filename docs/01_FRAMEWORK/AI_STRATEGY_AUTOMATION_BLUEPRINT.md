...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。策略生成、回测验证、部署、监控与人审节点若涉及对外调用或事件流转，须在该真源或本文后续接口说明中闭合。
...
| [STRATEGY_SELECTION_TACTICS_ENTRY.md](../03_TRADING_TACTICS/01_STRATEGY_FRAMEWORK/STRATEGY_SELECTION_TACTICS_ENTRY.md) | 策略选择（stub → 图纸柜） | ⭐⭐⭐⭐ |
