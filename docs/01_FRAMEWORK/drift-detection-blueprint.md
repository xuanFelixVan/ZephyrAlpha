---
module_id: DRIFT_DETECTION_BLUEPRINT_0767
version: 1.0.0
status: Active
...
- 全库 API 与事件约定真源：`API_Contract.md`。漂移检测任务配置、告警/处置触发、模型/特征回滚与审计事件若通过接口/事件实现，须在该真源或本文后续接口说明中闭合。
