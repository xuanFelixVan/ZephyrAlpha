...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。本补充方案中新增/补齐模块的接口约定最终必须回写到该真源（或在本文给出豁免与补全计划）。
...
| 模块职责边界 | [MODULE_RESPONSIBILITY_BOUNDARIES.md](./MODULE_RESPONSIBILITY_BOUNDARIES.md) | 模块职责边界定义 |
