...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。记忆写入、查询、生命周期与隐私相关的接口假设，须在该真源或本文各子模块的接口段落中闭合。

## 验收标准（可检查）

- Owner 能从本文的 8 个补充模块中，逐一指出“输入/输出/存取边界与依赖”，并能将关键边界映射到 `API_Contract.md`（或在对应子模块下给出豁免说明）。

## 已知限制

- 本文为模块合集，子模块粒度的接口细节可能分散在后续小节；以本节门禁为准，后续若需统一抽取“接口总表”再开专项批次。
