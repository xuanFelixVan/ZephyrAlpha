...
priority: P1
responsibility_boundary: '本文档负责Layer 4机器学习层的对抗鲁棒性设计，包括对抗攻击、对抗防御、鲁棒训练等核心功能。
...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。对抗鲁棒性相关的推理入口、训练数据管道与监控告警须与该文档或本文「接口」小节对齐。

## 验收标准（可检查）

- 能列出至少两类攻击面（如训练投毒、推理期扰动）对应的检测或缓解措施，并标明与模型推理与数据入口相关的契约条目。
