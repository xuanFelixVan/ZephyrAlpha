...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。质量评分、评估报告导出与异常上报若通过接口/事件暴露，须在该真源或本文后续接口说明中闭合。

## 验收标准（可检查）

- Owner 能抽检任一数据表/特征，得到可复现的质量评分与维度分解，并能在 `API_Contract.md` 中定位到质量上报/查询/审计契约入口（或在本文写明豁免与补全计划）。
