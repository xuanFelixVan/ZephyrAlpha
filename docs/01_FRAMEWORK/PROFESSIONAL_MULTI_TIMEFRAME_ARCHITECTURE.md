...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。若本文中出现“信号产出/下单/风控拦截/监控上报”等需要跨模块协作的能力，最终应能在该真源中定位到对应契约入口（或在本文“已知限制”中声明暂未闭合的原因与补全方向）。

## 验收标准（可检查）

- 读者能从本文中明确区分三层时间框架（宏观/中观/微观）的**输入、输出、关键职责**，并可映射到 `docs/01_FRAMEWORK/BLUEPRINT_ARCHITECTURE_MAPPING.md` 中的 Layer 对照表。
- 本文中每个“可执行组件/系统/引擎”若被描述为对外提供能力，需能在 `API_Contract.md` 中找到对应的接口/事件契约入口（或在“已知限制”中列出未闭合清单）。
- Mermaid 图（如有）应能被渲染（语法闭合），且不应出现明显的占位节点导致图结构断裂。

## 已知限制

- 正文存在历史内容断裂与占位符（如 `?`、未闭合的代码块/图表片段、未完整的标题与表格行）。在未进行专项清理前，本文只作为“架构意图与分层边界”的说明文档；细节实现与契约闭合以 `API_Contract.md` 与各模块蓝图为准。
- 本文中部分组件名仅为示意（例如 *Economic Regime Engine*、*Minute Execution Optimizer* 等），若后续发生拆分/重命名，应以 `System_Manifest.md` 与对应模块蓝图为准，并回补本文件的引用链接。
