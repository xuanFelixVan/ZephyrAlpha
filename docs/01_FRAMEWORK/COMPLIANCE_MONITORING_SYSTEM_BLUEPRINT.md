...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。合规规则下发、监控事件、拦截与审计查询若通过接口/事件实现，须在该真源或本文后续接口说明中闭合。

## 验收标准（可检查）

- 能在本文中明确至少一条“合规触发 → 检查/拦截 → 记录/审计 → 报告”的可检查闭环，并能在 `API_Contract.md` 中定位到对应契约入口（或在本文写明豁免与补全计划）。
