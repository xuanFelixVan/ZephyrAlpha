...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。对手方评级、敞口上报、限额检查与告警/审计记录若通过接口/事件实现，须在该真源或本文后续接口说明中闭合。

## 验收标准（可检查）

- 能从本文中明确“对手方信息 → 评级/PD → 敞口计算（CVA/DVA）→ 限额/缓释 → 审计留痕”的最小闭环，并能在 `API_Contract.md` 中定位到相应的风险事件/审计契约入口（或在本文写明豁免与补全计划）。
