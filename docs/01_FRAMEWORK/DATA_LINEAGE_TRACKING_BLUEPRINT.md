...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。血缘采集、查询、影响分析与审计留痕若通过接口/事件实现，须在该真源或本文后续接口说明中闭合。

## 验收标准（可检查）

- Owner 能从本文中抽检任一数据表/特征，追溯其“来源 → 转换 → 使用方”，并能在 `API_Contract.md` 中定位到相应的元数据/审计契约入口（或在本文写明豁免与补全计划）。

## 已知限制

- 不同数据工具的血缘采集方式差异较大；以本节门禁为准，适配细节在施工文档阶段落定。
---
