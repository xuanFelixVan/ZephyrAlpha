...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。执行质量上报、查询、告警与报告输出若通过接口/事件实现，须在该真源或本文后续接口说明中闭合。

## 验收标准（可检查）

- 能在本文中明确“订单执行数据输入 → 质量评估 → 告警/报告输出”的最小闭环，并能在 `API_Contract.md` 中定位到执行质量/审计相关契约入口（或在本文写明豁免与补全计划）。
...
| [TRANSACTION_COST_ANALYSIS_FRAMEWORK_ENTRY.md](./TRANSACTION_COST_ANALYSIS_FRAMEWORK_ENTRY.md) | 交易成本分析（→ canonical） |
