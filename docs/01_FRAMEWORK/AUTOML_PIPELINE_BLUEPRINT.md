...
priority: P2
responsibility_boundary: '本文档负责Layer 4机器学习层的AutoML流水线设计，包括自动特征工程、自动模型选择、自动调参等核心功能。
...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。流水线的任务触发、数据读写、产物（模型/指标）输出若通过接口/事件暴露，须在该真源或本文后续接口说明中闭合。

## 验收标准（可检查）

- 能从本文中明确 AutoML Pipeline 的输入输出与阶段边界（特征工程/模型选择/调参/评估），并能指向 `API_Contract.md` 的对应契约入口或补全计划。
