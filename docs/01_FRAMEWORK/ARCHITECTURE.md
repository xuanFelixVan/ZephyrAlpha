...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。本文是架构权威说明，跨层数据流、模块调用与事件边界的最终口径以契约真源为准；若本文与契约冲突，以契约真源为准并在架构治理批次中同步修订。

## 验收标准（可检查）

- 本文列出的 Layer 0-11 分层表（§1）应与正文“完整分层架构（§2）”一致，且不出现编号/名称冲突（可人工核对）。
- 本文“跨层级数据流（§4）”中对外约定的统一入口应能定位到 `API_Contract.md`（并且该链接可点击跳转）。
- 本文“相关文档（§5）”中的相对链接应可点击跳转且不引入新的无效内链。

## 已知限制

- 部分历史内容仍包含实现侧路径与审计侧路径的引用（例如指向施工目录或审计状态目录）；本文只提供架构与边界权威说明，具体实施细节与索引维护由对应专项批次负责。
- 本文中“旧层架构 ↔ 终极蓝图架构”映射属于治理过程的兼容说明，后续若发生分层调整，需以 `API_Contract.md` 的契约边界与最新架构审计结论同步更新。
