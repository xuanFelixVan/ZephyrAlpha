...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。权限与访问控制若通过接口（鉴权、授权、审计查询）或事件（审批流、拦截通知）体现，对外口径以该真源（及其子契约）为准；本文仅定义权限策略与操作规范。

## 验收标准（可检查）

- 本文“可写/只读/禁止”三类路径矩阵应互斥且覆盖关键目录；任意路径只能落入一类（可通过人工抽样检查与工具脚本校验）。
- 对于本文中声明“不可修改”的关键文件（例如 `API_Contract.md`、`System_Manifest.md` 等），应在权限矩阵中有明确归类与理由，且不出现自相矛盾条目。
- 本文中出现的相对链接应可点击跳转且不引入新的无效内链。

## 已知限制

- 本文件头部存在重复 front matter 与历史编码断裂符号（例如 `?`、未闭合的行），属于格式历史遗留；本批次仅补齐门禁三段，不在此处进行排版/编码专项修复。
- 文中提到的“必须记录到审计日志/更新变更日志”等流程为规范性要求，实际落地仍需结合实现侧工具链与 `API_Contract.md` 的契约化审批/审计接口进行闭合。
