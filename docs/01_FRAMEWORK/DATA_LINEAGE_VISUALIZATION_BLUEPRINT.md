---
module_id: DATA_LINEAGE_VISUALIZATION_001
version: 1.0.0
status: Active
...
responsibility_boundary: '本文档负责数据血缘可视化，包括：
...
responsibility:
- DATA_LINEAGE_VISUALIZATION蓝图设计
---
# 数据血缘可视化蓝图
...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。血缘图谱的查询、过滤、导出与权限控制若通过接口/事件暴露，须在该真源或本文后续接口说明中闭合。

## 验收标准（可检查）

- Owner 能从界面或查询接口中定位任一数据资产的上游/下游，并能将关键字段口径映射到 `API_Contract.md` 的相关契约入口（或在本文写明豁免与补全计划）。

## 已知限制

- 可视化框架与图谱存储（Neo4j 等）选型在施工文档阶段落定；以本节门禁为准。
