---
module_id: COMPLIANCE_TRAINING_MANAGEMENT_001
version: 1.0.0
status: Active
...
> **职责边界**: 
> - ✅ 本文档负责：Compliance Training Management蓝图设计相关内容
> - ❌ 本文档不负责：其他模块内容
...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。培训课程发布、考试评估、认证发放与审计留痕若通过接口/事件实现，须在该真源或本文后续接口说明中闭合。

## 验收标准（可检查）

- 能在本文中明确“课程发布 → 学习记录 → 考试评估 → 认证结果 → 审计留痕”的最小闭环，并能在 `API_Contract.md` 中定位到相应的培训/审计契约入口（或在本文写明豁免与补全计划）。
