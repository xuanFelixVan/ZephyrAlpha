...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。质量规则配置、验证结果上报、告警与改进工单若通过接口/事件实现，须在该真源或本文后续接口说明中闭合。

## 验收标准（可检查）

- 能在本文中明确“规则定义 → 验证执行 → 异常告警 → 改进跟踪 → 报告输出”的最小闭环，并能在 `API_Contract.md` 中定位到相应的上报/告警/审计契约入口（或在本文写明豁免与补全计划）。
