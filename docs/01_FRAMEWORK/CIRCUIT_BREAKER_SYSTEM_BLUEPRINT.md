...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。熔断触发、解除、冷却期状态与审计记录若通过接口/事件实现，须在该真源或本文后续接口说明中闭合。

## 验收标准（可检查）

- 能在本文中明确至少一条“触发条件 → 熔断动作 → 冷却期管理 → 恢复条件 → 审计记录”的可检查闭环，并能在 `API_Contract.md` 中定位到对应的风险事件/审计契约入口（或在本文写明豁免与补全计划）。
