...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。因子生成、评估、组合与监控若通过接口/事件提供给上层策略或研究工作流，须在该真源或本文后续接口说明中闭合。

## 验收标准（可检查）

- 能从本文中明确 Layer 2 对外提供的最小能力清单（因子输出格式、评估指标口径、组合输入输出），并能在 `API_Contract.md` 中定位到相关的数据契约/调用契约入口（或在本文写明豁免与补全计划）。

## 已知限制

- 因子口径与命名空间需与后续“因子注册表/因子契约”统一；以本节门禁为准，统一工作放在施工文档阶段落定。
