...
- 全库 API 与事件约定真源：[`API_Contract.md`](../03_TRADING_TACTICS/API_Contract.md)。报告生成的输入数据、模板选择、权限与分发通道若对外暴露接口，须在此真源或本文后续接口说明中闭合。

## 验收标准（可检查）

- Owner 能在任一次报告产出前，依据本文档明确指出报告生成模块的输入来源、模板规则与输出分发路径，并能在 `API_Contract.md` 中定位到对应契约条目（或在本文给出豁免说明）。

## 已知限制

- 文首存在重复的分隔线（`---`）与历史排版噪声；以本节门禁为准，结构清理留待专项批次统一处理。
