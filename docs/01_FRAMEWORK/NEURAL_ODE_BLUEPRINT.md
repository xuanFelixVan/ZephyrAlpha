...
│  │                    ODE求解器                                        │   │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐              │   │
│  │  │ Euler方法    │  │ RK45方法     │  │ 自适应方法   │              │   │
│  │  │ (快速)       │  │ (精确)       │  │ (平衡)       │              │   │
│  │  └──────────────┘  └──────────────┘  └──────────────┘              │   │
│  │  • 伴随敏感性方法 (Adjoint Sensitivity)                             │   │
│  │  • O(1)内存反向传播                                                 │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                    ↓                                        │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
...
| 模块 | 职责 | 输入 | 输出 |
|------|------|------|------|
| **ODE函数网络** | 参数化状态演化 | (t, x) | dx/dt |
| **ODE求解器** | 数值积分求解 | f, x0, t_span | x(t) |
...
class NeuralODE(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            solver: ODE求解器 ('euler', 'rk4', 'dopri5')
            rtol: 相对容差
            atol: 绝对容差
            adjoint: 是否使用伴随方法
        """
    ...
    def forward(
        ...
        Args:
            x0: 初始状态 (batch_size, state_dim)
            t_span: 时间点序列 (num_time_points,)
            external_inputs: 外部输入 (batch_size, num_time_points, input_dim)
            
        Returns:
            torch.Tensor: 状态轨迹 (batch_size, num_time_points, state_dim)
        """
        pass
...
class IrregularTimeSeriesData:
    ...
    timestamps: np.ndarray      # 时间戳 (不规则)
    values: np.ndarray          # 观测值
    features: Optional[np.ndarray] = None  # 附加特征
