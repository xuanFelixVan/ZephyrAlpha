...
| 契约项 | 契约内容 | 验证方法 |
|--------|---------|---------|
| **权重归一化** | 所有权重之和 = 1.0 | 数学验证 |
| **风险预算约束** | 风险贡献符合预算 | 风险检查 |
| **约束满足** | 所有约束条件都满足 | 约束验证 |
| **优化收敛** | 优化算法必须收敛 | 优化测试 |
| **性能保证** | 优化时间 ≤ 10秒 | 性能测试 |
...
class MarketStateOutput:
    """市场状态输出"""
    market_regime: str                   # 市场状态 (bull/bear/sideways/volatile)
...
| **状态识别准确率** | ≥ 70% | 回测验证 |
| **实时性保证** | 识别时间 ≤ 1秒 | 性能测试 |
| **状态一致性** | 连续状态识别一致性 ≥ 80% | 一致性测试 |
| **推荐策略有效性** | 推荐策略夏普比率 ≥ 1.5 | 策略验证 |
...
class FactorOutput:
    ...
    factor_correlation: pd.DataFrame     # 因子相关性
...
| **因子有效性** | IC均值 ≥ 0.03 | IC检验 |
| **因子正交性** | 因子相关性 ≤ 0.5 | 相关性检查 |
| **计算性能** | 因子计算时间 ≤ 30秒 | 性能测试 |
...
class ExecutionPlan:
    ...
    execution_schedule: Dict[str, any]    # 执行时间表
    algorithm_selection: Dict[str, str]   # 算法选择
...
class AlgorithmOutput:
    """算法输出"""
    child_orders: List[Dict[str, any]]    # 子订单
    execution_progress: float             # 执行进度
...
| **市场冲击控制** | 市场冲击 ≤ 预估值×1.5 | 冲击分析 |
| **执行稳定性** | 执行成功率 ≥ 99% | 稳定性测试 |
...
class RiskHedgeOutput:
    ...
    hedge_effectiveness: float            # 对冲有效性
    remaining_risk: Dict[str, float]      # 剩余风险
    timestamp: datetime                   # 时间戳
...
|--------|---------|---------|
| **对冲有效性** | ≥ 80% | 有效性验证 |
| **对冲及时性** | 对冲响应时间 ≤ 1秒 | 性能测试 |
| **成本控制** | 对冲成本 ≤ 预算 | 成本检查 |
| **风险覆盖** | 对冲覆盖 ≥ 90%风险 | 风险检查 |
...
class MacroToTacticalOutput:
    """宏观→中观输出"""
    strategy_selection_context: Dict[str, any]  # 策略选择上下文
    portfolio_constraints: Dict[str, any]       # 组合约束
...
| 契约项 | 契约内容 | 验证方法 |
|--------|---------|---------|
| **上下文完整性** | 所有必需上下文都传递 | 完整性检查 |
| **约束一致性** | 约束条件与宏观层一致 | 一致性验证 |
| **传递及时性** | 传递延迟 ≤ 1分钟 | 性能测试 |
...
class TacticalToExecutionInput:
    ...
    execution_priority: pd.Series         # 执行优先级
    hedge_requirements: Dict[str, any]    # 对冲需求
...
class ITacticalToExecutionBridge(ABC):
    ...
    def transfer_targets(self, tactical_input: TacticalToExecutionInput) -> TacticalToExecutionOutput:
        ...
        Args:
            tactical_input: 中观层输入
            
        Returns:
            TacticalToExecutionOutput: 微观层输出
        """
        pass
```
...
| **新增参数(可选)** | MINOR | 向后兼容 | 无需迁移 |
| **新增参数(必需)** | MAJOR | 不兼容 | 必须迁移 |
| **删除接口** | MAJOR | 不兼容 | 必须迁移 |
| **修改接口签名** | MAJOR | 不兼容 | 必须迁移 |
...
通过明确三级时间框架的接口契约,我们实现了:

1. **接口规范化**: 所有模块间通信都有明确的接口定义
2. **契约明确化**: 每个接口都有明确的契约和验证方法
3. **版本管理**: 所有接口都有版本管理机制
4. **质量保证**: 接口质量有明确的验证标准

### 7.2 实施建议

1. **Phase 1**: 实施宏观配置层接口契约
2. **Phase 2**: 实施中观策略层接口契约
3. **Phase 3**: 实施微观执行层接口契约
4. **Phase 4**: 实施跨层接口契约
