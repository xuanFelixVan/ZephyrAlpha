...
| G-013 | G4 | Layer 11 子能力映射 | `ARCHITECTURE.md` §Layer 11 | 逐项↔蓝图 | 表项极多未核验 | P2 | 主入口链到 STRATEGIC_DECISION_LAYER_BLUEPRINT；细项留待下轮 | **部分完成** |
| G-014 | G5 | 舆情 vs 策略 | `MODULE_RESPONSIBILITY_BOUNDARIES.md` | 读者理解 | 易混淆 | P2 | Layer3→5 段 + API_Contract | **已修正**（2026-04-08） |
| G-015 | G2 | 正文乱码 | 多文档正文 | 可读性 | 历史 `?` 替换字 | P1 | 文首已治理；正文分批 UTF-8 修复 | **部分完成**（本轮未全文重打） |
...
- **G-015 正文**：优先在每次编辑某节时顺带修正乱码，避免一次性 280 行人工重打。

## 质量门（本轮执行后）

- [x] L1：**无效内链 = 0**（2026-04-08 扫描）  
- [x] 权威栈：`ARCHITECTURE` ↔ `MODULE_RESPONSIBILITY` ↔ `BLUEPRINT_ARCHITECTURE_MAPPING` ↔ `API_Contract` 可点击通达  
- [ ] Q3 长期：Layer 11 子表与蓝图逐条核验（G-013）
