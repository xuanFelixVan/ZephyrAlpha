---
blueprint_id: GOV-061
title: Research Notes
module_id: GOV-061
updated: "2026-06-22"
ttl: task_bound
---

# Research Notes

> 研究笔记与调研文档归档。迁移自原 `临时工作区（不能删除）`（AUDIT-06 清理）。

## 目录

| 文件 | 说明 |
|------|------|
| [type_migration_master_plan.yaml](type_migration_master_plan.yaml) | 类型迁移主计划 |
| [38_blueprint_split_card_commands.txt](38_blueprint_split_card_commands.txt) | 38 蓝图一键拆卡指令集 |
| [three_principles_implementation_research.md](three_principles_implementation_research.md) | 三条原则落地——业界调研与完整方案 |
| [all_blueprint_structure_audit_overview.md](all_blueprint_structure_audit_overview.md) | 全体蓝图结构审计与施工总览 |
| [physical_structure_1500_module_scalability_fix.md](physical_structure_1500_module_scalability_fix.md) | 物理结构与 1500 模块可扩展性——完整修复方案 |
| [blueprint_discussion_audit_division_plan.md](blueprint_discussion_audit_division_plan.md) | 蓝图讨论与代码审计分工方案 |

## 竞品分析

| 文件 | 说明 |
|------|------|
| [competitive_analysis/ballethip_system_comparison.md](competitive_analysis/ballethip_system_comparison.md) | BalletHip 系统对比分析 |
| [competitive_analysis/architecture_blindspot_analysis.md](competitive_analysis/architecture_blindspot_analysis.md) | 架构盲点补全分析 |
| [competitive_analysis/ballethip_raw_material_visual_log.md](competitive_analysis/ballethip_raw_material_visual_log.md) | BalletHip 原始素材图文记录 |
| [competitive_analysis/observability_contract_todo.md](competitive_analysis/observability_contract_todo.md) | 可观测性契约备忘 |

## 技术可行性

| 文件 | 说明 |
|------|------|
| [technical_feasibility/vibe_coding_ai_implementation_assessment.md](technical_feasibility/vibe_coding_ai_implementation_assessment.md) | AI 氛围编程实现评估 |
