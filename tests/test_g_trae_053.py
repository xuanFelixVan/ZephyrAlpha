"""Test gate g_trae_053 for rule TRAE-053 — calls gate_engine.evaluate().
...
GATE_YAML = PROJECT_ROOT / "src" / "zephyr" / "governance" / "rule_enforcement" / "g_trae_053.yaml"
RULE_YAML = PROJECT_ROOT / "docs" / "01_policies_and_standards" / "rules" / "trae_053_automation_dual_track.yaml"
MOCK_YAML = PROJECT_ROOT / "tests" / "fixtures" / "g_trae_053_mock.yaml"


class TestGTrae053:
    """Red-Blue extreme tests for g_trae_053 gate."""
    ...
    def test_gate_registered_in_gate_files(self):
        ...
        assert "G_TRAE_053" in GateEngine._GATE_FILES, "Gate G_TRAE_053 not in _GATE_FILES"
    ...
    def test_gate_loadable_by_engine(self):
        ...
        try:
            if "G_TRAE_053" not in GateEngine._GATE_FILES:
                GateEngine._GATE_FILES["G_TRAE_053"] = "g_trae_053.yaml"
                engine._gate_cache = None  # Clear cache
            gates = engine.load_gates()
            assert "G_TRAE_053" in gates, "Gate G_TRAE_053 not loaded"
        finally:
    ...
    def test_gate_evaluates_pass(self):
        ...
        try:
            ...
            valid_task = Task(
                ...
                acceptance=["pytest"],
                applicable_rules=[{"module_id": "TRAE-053", "section": "all", "reason": "gate test"}],
            ...
            result = engine.evaluate(valid_task, "G_TRAE_053")
    ...
    def test_gate_evaluates_fail(self):
        ...
        try:
            ...
            result = engine.evaluate(sparse_task, "G_TRAE_053")
    ...
    def test_gate_bypass_detection(self):
        ...
        try:
            ...
            gates = engine.load_gates()
            assert "G_TRAE_053" in gates, "Gate should be loadable when YAML exists"
        finally:
            GateEngine._GATE_FILES = original
            engine._gate_cache = None
