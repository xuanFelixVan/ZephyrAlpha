...
class TestAssertPit:
    ...
    def test_pit_rules_cover_six_sources(self):
        assert set(DabanPITBacktestFramework.PIT_RULES) == {
            'dragon_tiger', 'emotion_cycle_score', 'echelon_data',
            'seal_data', 'next_day_auction', 'news_sentiment',
        }
...
def _make_framework(echelon_health='PERFECT', emotion_phase='主升', emotion_score=50.0,
                    tech_score=70.0, auction_open=11.0):
    t0 = date(2026, 8, 18)
    t1 = date(2026, 8, 19)
    t2 = date(2026, 8, 20)
    ...
    def data_loader(source, d):
        ...
        if source == 'echelon_data':
            return {'date': d, 'health': echelon_health, 'height': 2,
                    'sector_resonance': 0.8, 'follow_count': 5, 'tech_score': tech_score, 'price': 10.0}
