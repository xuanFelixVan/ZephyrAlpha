# [A_test] module_id: SRC-TST-0671 | layer=test | stability=volatile | safety=L | ai_autonomy=ai_modifiable
# [BLUEPRINT] MOD-GATE_ENGINE | docs/03_modules/_cross_layer/gate_engine/blueprint.md | §3-§7
# [MODULE] tests.test_ct_fle_gate
...
# [TESTS] pytest tests/test_ct_fle_gate.py
# [TTL] task_bound
...
class TestFleGateHandlerImport:
    def test_import_module(self):
        import importlib

        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_fle_gate")
        assert hasattr(mod, "FleGateHandler")

    def test_class_is_check_type_handler(self):
        import importlib

        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_fle_gate")
        assert issubclass(mod.FleGateHandler, CheckTypeHandler)


class TestFleGateHandlerInstantiation:
    def test_instantiate(self):
        import importlib

        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_fle_gate")
        handler = mod.FleGateHandler()
    ...
    def test_name_attribute(self):
        ...
        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_fle_gate")
        handler = mod.FleGateHandler()
        assert handler.name == "fle_gate"

    def test_registered_in_registry(self):
        cls = get_check_type("fle_gate")
        assert cls is not None


class TestFleGateHandlerRun:
...
class TestFleGateHandlerFleGate:
    def test_missing_gate_module(self):
        cls = get_check_type("fle_gate")
        handler = cls()
        task = _make_task()
        result = handler.run(task, {}, _MockCheck(), Path("."))
        assert len(result) >= 1
        assert any("gate_module" in v["message"] for v in result)

    def test_nonexistent_gate_module(self):
        cls = get_check_type("fle_gate")
        handler = cls()
        task = _make_task()
        result = handler.run(task, {"gate_module": "zephyr.nonexistent_module_xyz"}, _MockCheck(), Path("."))
        assert isinstance(result, list)
