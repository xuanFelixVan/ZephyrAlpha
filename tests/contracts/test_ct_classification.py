# [A_test] module_id: SRC-TST-0661 | layer=test | stability=volatile | safety=L | ai_autonomy=ai_modifiable
# [BLUEPRINT] MOD-INF-007 | docs/03_modules/_cross_layer/gate_engine/blueprint.md | §3-§7
# [MODULE] tests.test_ct_classification
...
# [TESTS] pytest tests/test_ct_classification.py
# [TTL] task_bound
...
class TestClassificationHandlerImport:
    def test_import_module(self):
        import importlib

        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_classification")
        assert hasattr(mod, "ClassificationHandler")

    def test_class_is_check_type_handler(self):
        import importlib

        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_classification")
        assert issubclass(mod.ClassificationHandler, CheckTypeHandler)


class TestClassificationHandlerInstantiation:
    def test_instantiate(self):
        import importlib

        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_classification")
        handler = mod.ClassificationHandler()
    ...
    def test_registered_in_registry(self):
        cls = get_check_type("classification")
        assert cls is not None


class TestClassificationHandlerRun:
...
class TestClassificationHandlerClassification:
    def test_valid_value_passes(self):
        cls = get_check_type("classification")
        handler = cls()
        task = _make_task()
        result = handler.run(task, {"field": "task_id", "allowed_values": ["KBG-001"]}, _MockCheck(), Path("."))
        assert len(result) == 0
    ...
    def test_invalid_value_fails(self):
        ...
        handler = cls()
        task = _make_task()
        result = handler.run(task, {"field": "task_id", "allowed_values": ["XYZ-999"]}, _MockCheck(), Path("."))
        assert len(result) >= 1
