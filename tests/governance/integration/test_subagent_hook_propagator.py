...
class TestRegisterHook:
    def test_register_with_propagate_true(self):
        ...
        assert prop.hooks["parent_a"]["name"] == "safety_check"
        assert prop.hooks["parent_a"]["propagate_to_subagents"] is True

    def test_register_with_propagate_false(self):
        prop = SubagentHookPropagator()
        prop.register_hook("parent_b", "audit_log", propagate=False)
        assert prop.hooks["parent_b"]["name"] == "audit_log"
        assert prop.hooks["parent_b"]["propagate_to_subagents"] is False

    def test_register_default_propagate_is_true(self):
        prop = SubagentHookPropagator()
        prop.register_hook("parent_c", "default_hook")
        assert prop.hooks["parent_c"]["propagate_to_subagents"] is True
    ...
    def test_register_overwrites_previous(self):
        ...
        assert prop.hooks["parent_d"]["name"] == "second_hook"
        assert prop.hooks["parent_d"]["propagate_to_subagents"] is False
