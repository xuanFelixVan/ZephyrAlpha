...
# ============================================================================
# TestOnlyOwnSessionScanned — own-scope 语义（#ARCH-GATE-OWN-SCOPE-001 第四批，
# #ARCH-310，2026-09-12；对标 test_bare_sql_gate.py 同名类）
# ============================================================================
...
class TestOnlyOwnSessionScanned:
    ...
    def _make_session_gateway(self, tmp_path, staged_am, staged_a, contents):
        for name, content in contents.items():
            (tmp_path / name).write_text(content, encoding="utf-8")
        gw = _make_mock_gateway(
            staged_files_am=staged_am,
            staged_files_a=staged_a,
            file_contents={},
            project_root=str(tmp_path),
        )
    ...
    def test_foreign_violation_does_not_block_own_commit(self, tmp_path):
        ...
        own_clean = "own_good.py"
        foreign_bad = "foreign_bad.py"
        gw = self._make_session_gateway(
            tmp_path,
            staged_am=[own_clean, foreign_bad],
            staged_a=[own_clean, foreign_bad],
            contents={
                own_clean: 'raise ValueError("bad input")\n',
                foreign_bad: 'raise FileNotFoundError(f"missing: {file_path}")\n',
            },
        )
        passed, detail = make_msg_exposure_gate().check(gw, [own_clean], session_id="sess-A")
        assert passed is True
        assert "file_path" not in detail
        audit = tmp_path / ".runtime" / "gate_audit" / "msg_exposure_foreign_staged.jsonl"
        assert audit.exists()
    ...
    def test_own_violation_still_blocks(self, tmp_path):
        own_bad = "own_bad.py"
        foreign_clean = "foreign_good.py"
        gw = self._make_session_gateway(
            tmp_path,
            staged_am=[own_bad, foreign_clean],
            staged_a=[own_bad, foreign_clean],
            contents={
                own_bad: 'raise FileNotFoundError(f"missing: {file_path}")\n',
                foreign_clean: 'raise ValueError("ok")\n',
            },
        )
        passed, detail = make_msg_exposure_gate().check(gw, [own_bad], session_id="sess-A")
        assert passed is False
        assert "file_path" in detail
    ...
    def test_no_session_degrades_to_full_scan(self, tmp_path):
        ...
        gw = self._make_session_gateway(
            ...
            staged_am=[bad],
            staged_a=[bad],
            contents={bad: 'raise FileNotFoundError(f"missing: {file_path}")\n'},
        )
        passed, detail = make_msg_exposure_gate().check(gw, [])
        assert passed is False
        assert "file_path" in detail
