...
class TestHumanFactorsInit:
    def test_instantiation(self):
        hf = HumanFactors()
        assert hf.notification_count == {}
        assert hf.last_notified == {}
        assert hf.min_interval_s == 300
        assert hf.max_per_hour == 12

    def test_multiple_instances_independent(self):
        hf1 = HumanFactors()
        hf2 = HumanFactors()
        hf1.should_notify("owner_a")
        assert "owner_a" not in hf2.last_notified
...
class TestShouldNotify:
    ...
    def test_notification_count_increments_across_intervals(self):
        ...
        assert hf.notification_count["owner_1"] == 1
        hf.should_notify("owner_1")
        assert hf.notification_count["owner_1"] == 2
    ...
    def test_different_owners_independent(self):
        ...
        assert hf.notification_count["owner_a"] == 1
        assert hf.notification_count["owner_b"] == 1
    ...
    def test_allowed_after_interval_passes(self):
        ...
        hf.min_interval_s = 1
        hf.should_notify("owner_1")
        hf.last_notified["owner_1"] = time.time() - 2
        allowed, reason = hf.should_notify("owner_1")
        assert allowed is True
        assert reason == "OK"
    ...
    def test_rate_limit_check_uses_last_notified_window(self):
        hf = HumanFactors()
        hf.max_per_hour = 1
        hf.last_notified["owner_1"] = time.time() - 100
        allowed, reason = hf.should_notify("owner_1")
        assert allowed is False
    ...
    def test_rate_limit_not_triggered_when_outside_window(self):
        hf = HumanFactors()
        hf.max_per_hour = 1
        hf.last_notified["owner_1"] = time.time() - 3601
        allowed, reason = hf.should_notify("owner_1")
        assert allowed is True
        assert reason == "OK"
...
class TestShouldNotifyBoundary:
    ...
    def test_min_interval_just_below_boundary(self):
        ...
        hf.min_interval_s = 100
        hf.should_notify("owner_1")
        hf.last_notified["owner_1"] = time.time() - 99.9
        allowed, reason = hf.should_notify("owner_1")
        assert allowed is False
        assert reason == "Too frequent"
