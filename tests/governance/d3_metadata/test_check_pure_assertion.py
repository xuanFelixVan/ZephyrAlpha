...
def test_is_in_scope_include_dirs():
    assert _mod.is_in_scope("docs/03_modules/gov_engine/blueprint.md")
    assert _mod.is_in_scope(".trae/rules/onboarding_detail.md")
    assert _mod.is_in_scope("docs/01_policies_and_standards/standards/naming.md")


def test_is_in_scope_include_files():
    assert _mod.is_in_scope("AGENTS.md")
    assert _mod.is_in_scope("README.md")


def test_is_in_scope_exclude_dirs():
    assert not _mod.is_in_scope("docs/_working/temp.md")
    assert not _mod.is_in_scope("docs/_archive/old.md")
    assert not _mod.is_in_scope("docs/01_policies_and_standards/rules/trae_030.md")


def test_is_in_scope_exclude_files():
    assert not _mod.is_in_scope("docs/_archive/architecture_debt_registry_v2.md")


def test_is_in_scope_exclude_basenames():
    assert not _mod.is_in_scope("docs/03_modules/CHANGELOG.md")
    assert not _mod.is_in_scope("CHANGELOG.md")


def test_is_in_scope_non_md():
    assert not _mod.is_in_scope("scripts/governance/d3_metadata/check_pure_assertion.py")


def test_check_file_violation_regex1():
    """已废止/已废弃/已弃用"""
    v = _mod.check_file("这是已废止的规则。\n", None)
    assert len(v) == 1 and "已废止" in v[0]


def test_check_file_violation_regex3():
    """之前是X现在改为Y"""
    v = _mod.check_file("之前是手动触发，现在是自动触发。\n", None)
    assert len(v) == 1 and "之前是" in v[0]


def test_check_file_violation_regex6():
    """从X迁移到Y"""
    v = _mod.check_file("从旧路径迁移到新路径。\n", None)
    assert len(v) == 1 and "迁移" in v[0]


def test_check_file_skip_frontmatter():
    content = "---\ntitle: 已废止的旧规则\n---\n正文无违规。\n"
    assert _mod.check_file(content, None) == []
...
def test_check_file_incremental_added_lines():
    """只检 added_lines 指定的行"""
    content = "已废止的旧行。\n这行是新增的已废止。\n"
    v = _mod.check_file(content, {2})  # 只检第 2 行
    assert len(v) == 1 and "新增" in v[0]


def test_check_file_no_violation():
    assert _mod.check_file("这是当前有效的规则。\n", None) == []
...
def test_walk_scope_files_finds_md(tmp_path):
    ...
    files = _mod.walk_scope_files(str(tmp_path))
...
def test_check_file_non_table_deprecated_still_caught():
    """非表格行的'已废弃'仍被检测。"""
    content = "这是已废弃的旧规则。\n"
    v = _mod.check_file(content, None)
