...
def _load_test_cases(script_entries: list[dict]) -> list[tuple[str, int, str, str]]:
    return [
        (e["name"], e.get("timeout_seconds", 60),
         e.get("description", ""), ",".join(e.get("dimensions", [])))
...
class TestAllScripts:
    ...
    def test_script_count(self, test_cases):
        assert len(test_cases) >= 60, f"注册脚本数异常: {len(test_cases)}"
                    ...
                    try:
                        ...
                        if "severity" not in data:
                            invalid_jsonl.append(
                                f"{script_name}: JSONL 缺少 severity: {str(data)[:80]}")
                    except json.JSONDecodeError:
                        invalid_jsonl.append(
                            f"{script_name}: 无效 JSONL: {line[:80]}")
            else:
                unsupported += 1
