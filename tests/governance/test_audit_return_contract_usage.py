...
class TestKnownMisusePatterns:
    ...
    def test_patterns_includes_all_session_worktree_funcs(self, au):
        ...
        for func in required:
            assert func in au.KNOWN_MISUSE_PATTERNS, \
                f"KNOWN_MISUSE_PATTERNS 缺少 {func}"
    ...
    def test_patterns_values_are_sets(self, au):
        ...
        for func, forbidden in au.KNOWN_MISUSE_PATTERNS.items():
            assert isinstance(forbidden, set), \
                f"{func} 对应 forbidden 必须是 set，实际是 {type(forbidden)}"
...
class TestAuditReturnContractUsage:
    ...
    def test_directory_traversal(self, au, tmp_path):
        ...
        sub = tmp_path / "sub"
        sub.mkdir()
        (sub / "misuse.py").write_text(textwrap.dedent("""
            r = session_worktree_commit("s", ["f"], "m")
            print(r["committed"])
        """), encoding="utf-8")
...
class TestAuditDirectory:
    ...
    def test_excludes_git_dir(self, au, tmp_path):
        ...
        git_dir.mkdir()
        (git_dir / "hook.py").write_text(textwrap.dedent("""
            r = session_worktree_commit("s", ["f"], "m")
            print(r["committed"])
        """), encoding="utf-8")
        violations = au.audit_directory(tmp_path)
        assert all(".git" not in v.file for v in violations), \
            ".git/ 目录文件不应被扫描"
    ...
    def test_excludes_pycache(self, au, tmp_path):
        ...
        pc.mkdir()
        (pc / "cached.py").write_text(textwrap.dedent("""
            r = session_worktree_commit("s", ["f"], "m")
            print(r["committed"])
        """), encoding="utf-8")
        violations = au.audit_directory(tmp_path)
    ...
    def test_custom_exclude_dirs(self, au, tmp_path):
        ...
        exclude_me.mkdir()
        (exclude_me / "misuse.py").write_text(textwrap.dedent("""
            r = session_worktree_commit("s", ["f"], "m")
            print(r["committed"])
        """), encoding="utf-8")
