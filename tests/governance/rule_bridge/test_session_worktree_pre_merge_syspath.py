...
def test_pre_merge_skip_all_commit_gates_flag_is_true():
    ...
    assert hasattr(sw_mod, "_PRE_MERGE_SKIP_ALL_COMMIT_GATES"), (
        "_PRE_MERGE_SKIP_ALL_COMMIT_GATES 常量必须存在"
    )
    assert _PRE_MERGE_SKIP_ALL_COMMIT_GATES is True, (
        "_PRE_MERGE_SKIP_ALL_COMMIT_GATES 必须为 True（治本默认启用）"
    )
...
def test_force_param_skips_pre_merge_topo_check(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        sw_mod, "_execute_merge_and_build_msg",
    ...
    assert not topo_called["called"], (
        "force=True 时不应调用 _run_pre_merge_topo_check（逃生通道应跳过）"
    )
    # 验证 commit gate 未被调用（_PRE_MERGE_SKIP_ALL_COMMIT_GATES=True 跳过）
    assert not gate_called["called"], (
        "_PRE_MERGE_SKIP_ALL_COMMIT_GATES=True 时不应调用 _pre_merge_gate_check"
    )
    # 验证 merge 成功
...
def test_worktree_skip_gates_unchanged():
    ...
    assert _WORKTREE_SKIP_GATES == expected, (
        f"_WORKTREE_SKIP_GATES 应保持 {expected}，实际 {_WORKTREE_SKIP_GATES}"
    )
