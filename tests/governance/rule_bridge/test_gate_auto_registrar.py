...
# [TESTS] self
# [A_module] module_id=MOD-TEST_gate_auto_registrar | layer=module | stability=evolving | safety=L | ai_autonomy=ai_modifiable
# [TTL] permanent
...
class TestRealProjectIntegration:
    ...
    def test_load_real_yaml_entries(self) -> None:
        """真实 YAML 可加载且有 76 个条目。"""
        from zephyr.shared.io.paths import REPO_ROOT
        entries = load_gate_entries(Path(REPO_ROOT))
        assert len(entries) == 76, f"expected 76 entries, got {len(entries)}"
    ...
    def test_auto_register_all_76_gates(self) -> None:
        """auto_register_gates 注册全部 76 个 gate（无失败）。"""
        from zephyr.shared.io.paths import REPO_ROOT
        registry = CommitGateRegistry()
        ...
        # 验证至少注册了 71 个
        assert len(registry._specs) >= 71, f"only {len(registry._specs)} gates registered"
