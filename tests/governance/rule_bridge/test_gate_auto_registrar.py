...
class TestRealProjectIntegration:
    ...
    def test_load_real_yaml_entries(self) -> None:
        """真实 YAML 可加载且条目数与 registry 演进同步（2026-08-15：83→92，治理批②新增 RECONCILER-FILE-OPS 等 9 gate；2026-08-16：92→93；2026-08-17：93→97，AI-AUDIT11 补登 4 个死 gate；2026-08-21：97→98 +GATE-ERRCODE-CONSISTENCY；2026-08-23：98→99 +HOT-FILE-BASE-FRESHNESS）。"""
        from zephyr.shared.io.paths import REPO_ROOT

        entries = load_gate_entries(Path(REPO_ROOT))
        assert len(entries) == 99, f"expected 99 entries, got {len(entries)}"
