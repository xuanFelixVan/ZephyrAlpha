...
class TestNoqaRegistrySmoke:
    """Registry smoke test——真实 YAML + 真实 94 豁免 + 真实匹配逻辑。"""

    def test_registry_loads_successfully(self):
        """registry YAML 可加载 + 94 个 exemptions + 5 个 categories。
        ...
        2026-09-17：82→94（真源 config/governance/noqa_exempt_registry.yaml 于 4efe2ba568
        （2026-09-13）净增 12 条 business_subset/no_vocab_source 类豁免未同步本探针基线；
        本批按真源计数对齐，条目内容未改）。
        """
