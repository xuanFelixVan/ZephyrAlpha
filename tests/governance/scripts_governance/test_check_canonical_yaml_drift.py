...
def _write_tech_yaml(path: Path, ta03_version: str = ">=3.12", g05_count: int = 5,
                     tb03_physical: str = "src/zephyr/data/implementations/") -> None:
    """合成 technology_landscape.yaml。"""
    techs = [
        {"id": "T-A03", "name": "Python / 主语言", "quadrant": "adopt",
         "category": "language", "version": ta03_version},
    ]
    g05_names = ["Panel", "HoloViz", "Plotly", "plotly_resampler", "TradingView Lightweight Charts",
                 "HoloViews", "Datashader"]
    for i in range(g05_count):
        techs.append({"id": f"T-A1{i+3}", "name": g05_names[i % len(g05_names)],
                      "quadrant": "adopt", "category": "g0.5-visualization"})
    techs.append({"id": "T-B03", "name": "Vendor Registry", "quadrant": "build",
                  "category": "core-engine", "physical_path": tb03_physical})
...
def test_a1_python_version_match(tmp_path):
    tech = tmp_path / "tech.yaml"; _write_tech_yaml(tech, ta03_version=">=3.12")
    pp = tmp_path / "pyproject.toml"; _write_pyproject(pp, ">=3.12")
    assert g.assert_a1_python_version(tech, pp) == []


def test_a1_python_version_mismatch(tmp_path):
    tech = tmp_path / "tech.yaml"; _write_tech_yaml(tech, ta03_version=">=3.11")
    pp = tmp_path / "pyproject.toml"; _write_pyproject(pp, ">=3.12")
...
def test_a2_g05_present(tmp_path):
    tech = tmp_path / "tech.yaml"; _write_tech_yaml(tech, g05_count=5)
    assert g.assert_a2_g05_presence(tech) == []


def test_a2_g05_absent(tmp_path):
    tech = tmp_path / "tech.yaml"; _write_tech_yaml(tech, g05_count=0)
...
def test_a3_path_exists(tmp_path):
    ...
    tech = tmp_path / "tech.yaml"; _write_tech_yaml(tech)
    _dump(tmp_path / "c.yaml", {"external_contracts": [
...
def test_a3_path_missing(tmp_path):
    tech = tmp_path / "tech.yaml"; _write_tech_yaml(tech, tb03_physical="src/zephyr/data/connectors/")
    _dump(tmp_path / "c.yaml", {"external_contracts": [
...
def test_a3_skips_py_and_frontend(tmp_path):
    tech = tmp_path / "tech.yaml"; _write_tech_yaml(tech, tb03_physical="src/zephyr/planned.py")
    _dump(tmp_path / "c.yaml", {"external_contracts": [
...
def test_a5_clean(tmp_path):
    ...
    gov = tmp_path / "gov.yaml"; gov.write_text("note: .trae/rules/\n", encoding="utf-8")
    assert g.assert_a5_governance_config(gov, tmp_path) == []


def test_a5_stale_cursor(tmp_path):
    (tmp_path / ".trae/rules").mkdir(parents=True)
    gov = tmp_path / "gov.yaml"; gov.write_text('location: ".cursorrules"\n', encoding="utf-8")
...
def test_a5_missing_trae_dir(tmp_path):
    gov = tmp_path / "gov.yaml"; gov.write_text("note: ok\n", encoding="utf-8")
    issues = g.assert_a5_governance_config(gov, tmp_path)
...
def test_run_all_clean_synthetic_repo(tmp_path, monkeypatch):
    ...
    _dump(tmp_path / g.CONTRACTS_YAML, {"external_contracts": [
        {"id": "EXT-002", "acl_landing": "src/zephyr/data/implementations/"},
    ]})
    (tmp_path / g.RUNTIME_PLANES_YAML).parent.mkdir(parents=True, exist_ok=True)
    (tmp_path / g.RUNTIME_PLANES_YAML).write_text(
        f"submodule: {g._RUNTIME_TAG_FULL}\n", encoding="utf-8")
