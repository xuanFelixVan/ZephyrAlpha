...
# conftest.py 只加了 d3_metadata，本测试还需 _shared.walk 和 d7_code 扫描器
...
from _shared.walk import iter_staged_files  # noqa: E402, I001
...
class TestIterStagedFiles:
    """iter_staged_files() 共享函数——变更检测核心。"""

    def test_empty_staged_returns_empty(self, tmp_path: Path) -> None:
        """无 staged 文件时返回空列表。"""
        with patch("_shared.walk.REPO_ROOT", tmp_path), \
             patch("_shared.walk.subprocess.run", return_value=_mock_git_output([])):
    ...
    def test_git_failure_returns_empty(self, tmp_path: Path) -> None:
        ...
        with patch("_shared.walk.REPO_ROOT", tmp_path), \
             patch("_shared.walk.subprocess.run", return_value=_mock_git_output([], returncode=1)):
            assert iter_staged_files() == []
    ...
    def test_returns_staged_files(self, tmp_path: Path) -> None:
        ...
        with patch("_shared.walk.REPO_ROOT", tmp_path), \
             patch("_shared.walk.subprocess.run", return_value=_mock_git_output(files)):
    ...
    def test_backslash_paths_normalized(self, tmp_path: Path) -> None:
        ...
        with patch("_shared.walk.REPO_ROOT", tmp_path), \
             patch("_shared.walk.subprocess.run",
...
class TestCheckAnyAbuseStaged:
    """check_any_abuse.py --staged 模式——变更检测集成（内联 git diff，纯 stdlib）。"""

    def test_staged_no_files_passes(self) -> None:
        """--staged 无 staged .py 文件时 exit 0（跳过）。"""
        import check_any_abuse

        # check_any_abuse 在函数内局部 import subprocess，patch stdlib 模块级 run
