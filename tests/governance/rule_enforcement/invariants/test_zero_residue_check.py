# [A_test] module_id: MOD-GOV_zero_residue_check | layer=test | stability=volatile | safety=L | ai_autonomy=ai_modifiable
...
class TestZeroResidueScanner:
    ...
    def test_scripts_dir_set(self, tmp_path):
        scanner = ZeroResidueScanner(project_root=tmp_path)
        assert scanner.scripts_dir == tmp_path / "scripts" / "governance"

    @patch.object(ZeroResidueScanner, "scan_temp_files")
    @patch.object(ZeroResidueScanner, "scan_residual_files")
    @patch.object(ZeroResidueScanner, "scan_ruins_references")
    @patch.object(ZeroResidueScanner, "scan_orphan_py")
    @patch.object(ZeroResidueScanner, "scan_orphan_docs")
...
class TestRunScript:
    def test_script_not_found(self, tmp_path):
        scanner = ZeroResidueScanner(project_root=tmp_path)
        code, out, err = scanner.run_script("nonexistent/script.py")
        assert code == 1
    ...
    def test_script_success(self, mock_run, tmp_path):
        ...
        scanner = ZeroResidueScanner(project_root=tmp_path)
        code, out, err = scanner.run_script("d1_structure/detect_temp_files.py")
        assert code == 0
...
class TestParseFindings:
    ...
    def test_empty_stderr(self):
        scanner = ZeroResidueScanner(project_root=Path("/tmp"))
        result = scanner.parse_findings(1, "")
        assert result == []
...
class TestScanTempFiles:
    @patch.object(ZeroResidueScanner, "run_script")
    def test_no_issues(self, mock_run):
        mock_run.return_value = (0, "", "")
        scanner = ZeroResidueScanner(project_root=Path("/tmp"))
        report = ResidueReport()
        for rule_id, message, severity, file_rel in scanner.scan_temp_files():
...
class TestScanResidualFiles:
    ...
    def test_no_issues(self, mock_run):
        ...
        scanner = ZeroResidueScanner(project_root=Path("/tmp"))
        report = ResidueReport()
        for rule_id, message, severity, file_rel in scanner.scan_residual_files():
            report.add(rule_id, message, severity, file_rel)
        assert report.is_clean is True
...
class TestScanRuinsReferences:
    ...
    def test_no_issues(self, mock_run):
        ...
        scanner = ZeroResidueScanner(project_root=Path("/tmp"))
        report = ResidueReport()
        for rule_id, message, severity, file_rel in scanner.scan_ruins_references():
            report.add(rule_id, message, severity, file_rel)
        assert report.is_clean is True
...
class TestScanOrphanPy:
    ...
    def test_no_issues(self, mock_run):
        ...
        scanner = ZeroResidueScanner(project_root=Path("/tmp"))
        report = ResidueReport()
        for rule_id, message, severity, file_rel in scanner.scan_orphan_py():
            report.add(rule_id, message, severity, file_rel)
        assert report.is_clean is True
...
class TestScanOrphanDocs:
    ...
    def test_no_issues(self, mock_run):
        ...
        scanner = ZeroResidueScanner(project_root=Path("/tmp"))
        report = ResidueReport()
        for rule_id, message, severity, file_rel in scanner.scan_orphan_docs():
            report.add(rule_id, message, severity, file_rel)
        assert report.is_clean is True
