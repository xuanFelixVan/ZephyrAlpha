...
class TestObjectiveTrackerInit:
    ...
    def test_multiple_instances_independent(self):
        ...
        assert "agent_a" not in ot2.objectives
...
class TestSetObjective:
    def test_set_first_objective(self):
        ...
        assert ot.objectives["agent_1"] == ["optimize latency"]
        assert ot.versions["agent_1"] == 1

    def test_set_multiple_objectives_same_agent(self):
        ot = ObjectiveTracker()
        ot.set_objective("agent_1", "obj_a")
        ot.set_objective("agent_1", "obj_b")
        assert ot.objectives["agent_1"] == ["obj_a", "obj_b"]
        assert ot.versions["agent_1"] == 2

    def test_set_objectives_different_agents(self):
        ot = ObjectiveTracker()
        ot.set_objective("agent_1", "obj_a")
        ot.set_objective("agent_2", "obj_b")
        assert ot.objectives["agent_1"] == ["obj_a"]
        assert ot.objectives["agent_2"] == ["obj_b"]
        assert ot.versions["agent_1"] == 1
        assert ot.versions["agent_2"] == 1
