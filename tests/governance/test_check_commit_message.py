...
class TestExtractSessionId:
    ...
    def test_extract_basic(self) -> None:
        ...
        assert ccm._extract_session_id(msg) == "sess-12345-20260720"
    ...
    def test_extract_with_modifier(self) -> None:
        ...
        assert ccm._extract_session_id(msg) == "sess-99999-20260720"

    def test_extract_none(self) -> None:
        """无 [GW: 标记返回 None。"""
        assert ccm._extract_session_id("docs: update readme") is None
        assert ccm._extract_session_id("") is None
...
class TestLoadRegisteredSessions:
    ...
    def test_load_valid(self, tmp_path: Path) -> None:
        ...
        result = ccm._load_registered_sessions(registry)
        assert result == {"sess-aaa-20260720", "sess-bbb-20260720"}

    def test_load_missing_file(self, tmp_path: Path) -> None:
        """文件不存在返回空 set。"""
        result = ccm._load_registered_sessions(tmp_path / "missing.json")
        assert result == set()
    ...
    def test_load_corrupt_json(self, tmp_path: Path) -> None:
        ...
        assert ccm._load_registered_sessions(registry) == set()
...
class TestExtractCommitType:
    ...
    def test_basic(self) -> None:
        assert ccm._extract_commit_type("feat(audit): add library") == "feat"

    def test_no_scope(self) -> None:
        assert ccm._extract_commit_type("docs: update readme") == "docs"

    def test_non_conventional(self) -> None:
        """非 conventional 格式返回 None。"""
        assert ccm._extract_commit_type("update readme") is None
        assert ccm._extract_commit_type("Merge branch 'dev'") is None
...
class TestIsMergeCommit:
    ...
    def test_github_merge(self) -> None:
        assert ccm._is_merge_commit("Merge pull request #123 from xuanFelixVan/feature")

    def test_branch_merge(self) -> None:
        assert ccm._is_merge_commit("Merge branch 'dev' into main")

    def test_squash_merge(self) -> None:
        assert ccm._is_merge_commit("Squashed commit of the following:")

    def test_non_merge(self) -> None:
        assert not ccm._is_merge_commit("feat(audit): add library")
        assert not ccm._is_merge_commit("docs: update readme")
...
class TestCheckCommit:
    ...
    def test_ci_mode_format_valid(self) -> None:
        ...
        violations = ccm._check_commit("abc1234", msg, set(), strict=False)
        assert violations == []
