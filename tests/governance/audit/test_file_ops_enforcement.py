...
# ---------------------------------------------------------------------------
# 已知环境差异登记（2026-09-05 AI-19 测试域审计）——已修复关闭（B22① STEWARD）
# 原 13 用例在 session worktree（.worktrees/）内条件性失败（ops_guard 第二项目根
# 决策点不随 monkeypatch 的 _PROJECT_ROOT_CACHE 联动）。2026-09-05 B22① 根修：
# worktree 段剥离收敛到 _get_project_root 缓存填充时（单一决策点），缓存即权威，
# 测试可控性恢复。13 个条件 xfail 标记全部移除（XPASS 预案兑现）；主仓 38/38 不变。
# ---------------------------------------------------------------------------
