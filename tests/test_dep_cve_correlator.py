...
# [INVARIANTS] none
# [MODIFY-GUARD] none
# [CONSUMERS] zephyr.observability.feedback_loop.security.dep_cve_correlator
# [STABILITY] evolving
# [SAFETY] L
...
from zephyr.ops.security.dep_cve_correlator import (
    CVEAlert,
    CVESeverity,
    DepCVECorrelator,
)
...
class TestCheckCritical:
    def test_no_critical_alerts(self):
        corr = DepCVECorrelator()
        corr.alerts.append(CVEAlert(
            cve_id="CVE-2024-0001", dependency="lib",
            severity=CVESeverity.LOW, cvss_score=2.0,
            description="low issue", affected_version="1.0",
        ))
    ...
    def test_critical_alerts_found(self):
        corr = DepCVECorrelator()
        corr.alerts.append(CVEAlert(
            cve_id="CVE-2024-0002", dependency="lib",
            severity=CVESeverity.CRITICAL, cvss_score=9.8,
            description="critical issue", affected_version="1.0",
        ))
    ...
    def test_mixed_severity_alerts(self):
        corr = DepCVECorrelator()
        corr.alerts.append(CVEAlert(
            cve_id="CVE-2024-L", dependency="a",
            severity=CVESeverity.LOW, cvss_score=2.0,
            description="low", affected_version="1.0",
        ))
        corr.alerts.append(CVEAlert(
            cve_id="CVE-2024-C", dependency="b",
            severity=CVESeverity.CRITICAL, cvss_score=9.8,
            description="critical", affected_version="1.0",
        ))
...
class TestAutoFixAvailable:
    def test_no_fix_available(self):
        corr = DepCVECorrelator()
        corr.alerts.append(CVEAlert(
            cve_id="CVE-2024-0003", dependency="lib",
            severity=CVESeverity.HIGH, cvss_score=7.5,
            description="no fix", affected_version="1.0", fixed_version="",
        ))
    ...
    def test_fix_available(self):
        corr = DepCVECorrelator()
        corr.alerts.append(CVEAlert(
            cve_id="CVE-2024-0004", dependency="lib",
            severity=CVESeverity.HIGH, cvss_score=7.5,
            description="fix available", affected_version="1.0", fixed_version="1.1",
        ))
...
class TestCVEAlert:
    def test_alert_defaults(self):
        alert = CVEAlert(
            cve_id="CVE-2024-0005", dependency="x",
            severity=CVESeverity.MEDIUM, cvss_score=5.0,
            description="test", affected_version="2.0",
        )
