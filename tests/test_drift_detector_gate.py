...
class TestTriggerRecoveryImport:
    def test_function_importable(self):
        from zephyr.governance.drift_detector import trigger_recovery as fn
...
class TestDriftEngineImportError:
    def test_drift_engine_unavailable_causes_unbound_local(self):
        with patch.dict("sys.modules", {"zephyr.behavioral_audit.drift_engine": None}):
...
class TestTriggerRecoveryWithDriftEvents:
    @patch("zephyr.behavioral_audit.reconciler.AutoFixer")
    @patch("zephyr.behavioral_audit.cascade_detector.is_auto_fix_paused", return_value=False)
...
class TestCascadeLockout:
    @patch("zephyr.behavioral_audit.cascade_detector.is_auto_fix_paused", return_value=True)
...
class TestFixerUnavailable:
    ...
    def test_fixer_import_error_sets_unavailable(self, mock_bypass_cls, mock_scan, mock_detect_cascade, mock_is_paused):
        ...
        with patch.dict("sys.modules", {"zephyr.behavioral_audit.reconciler": None}):
            result = trigger_recovery({"module_id": "MOD-TEST"})
