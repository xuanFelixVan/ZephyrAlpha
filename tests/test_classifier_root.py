...
class TestClassifierClassify:
    ...
    def test_classify_gate(self):
        entry = _make_entry(relative_path="src/zephyr/gates/G001.yaml", extension=".yaml")
        scan = _make_scan(entries=[entry])
        c = Classifier()
        result = c.classify(scan)
    ...
    def test_classify_registry(self):
        entry = _make_entry(relative_path="src/zephyr/gates/_registry.yaml", extension=".yaml")
        scan = _make_scan(entries=[entry])
        c = Classifier()
        result = c.classify(scan)
