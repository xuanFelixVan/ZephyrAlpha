...
class TestDropPartitionSql:
    def test_plain(self):
        assert archiver._drop_partition_sql("c1_market.kline_1min", "201901", None) == \
            "ALTER TABLE c1_market.kline_1min DROP PARTITION 201901"

    def test_tuple_literal_form(self):
        assert archiver._drop_partition_sql("c1_market.technical_indicator", "201901", "60min") == \
...
class TestVerifySampleFieldComparison:
    ...
    def test_count_mismatch_fails(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(archiver.ch_reader, "query",
                            lambda sql: "51" if sql.startswith("SELECT count()") else _ch_json(rows[:1]))
        assert archiver.verify_partition("c1_market.kline_1min", "202101", pq_path) is False

    def test_missing_parquet_fails(self, tmp_path):
        assert archiver.verify_partition(
            "c1_market.kline_1min", "202101", tmp_path / "nope.parquet") is False
...
class TestManifestExtendedFields:
    def test_rows_ch_size_compress_ratio_checksum(self, tmp_path):
        ...
        rec = archiver._manifest_record(
            "c1_market.kline_1min", "202101", pq, True, None,
            rows=20, ch_size_bytes=pq.stat().st_size * 4,
        )
        assert rec["rows"] == 20
...
class TestExportOnlySubcommand:
    ...
    def test_cli_export_wiring(self, monkeypatch):
        ...
        calls = []
        monkeypatch.setattr(
            archiver, "export_only_partition",
            lambda table, partition, dry_run=False, period=None: calls.append((table, partition, dry_run, period)) or True,
        )
        monkeypatch.setattr("sys.argv", ["archiver.py", "export", "--table", "c1_market.kline_1min", "--partition", "202101"])
        archiver.main()
