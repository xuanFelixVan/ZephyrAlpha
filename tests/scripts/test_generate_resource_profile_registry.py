...
_SPEC = importlib.util.spec_from_file_location(
    "gen_resched_profile_test",
    REPO_ROOT / "scripts" / "governance" / "generators" / "generate_resource_profile_registry.py",
)
...
def test_parse_ps1_entities_covers_all_task_names():
    ...
    assert "sch_gate_full_tree_audit" in tids
    assert len(ents) == 26  # 2026-09-21 追认：gate_fulltree_audit ps1 排班落地（aca8c71fad，
    # 裁定#354 九台全树审计）→ 25→26（环境敏感计数随真源走）
    # 杂音捕获杜绝：无变量名误捕实体
...
def test_parse_schedule_slots_21_slots_cron_verbatim():
    ...
    assert len(ents) == 26  # 2026-09-22 追认：calendar_coverage_check（st-data-fix 批）+dloop_post（丁线 Owner 批A be05d1b01f）第 25/26 槽；前值 24=2026-09-18 追认 daily_alt_fx 第 22 槽（4058b7b1e0）
    # 2026-09-21 追认：data_supply_sentinel 第 23 槽（85ef0962d0 供数哨兵）+
    # eod_reconciliation 第 24 槽（R-015 日终对账，全流通战役排班批）→ 22→24
    by = {e["task_id"]: e for e in ents}
...
def test_full_registry_build_and_drift_check(tmp_path):
    ...
    r = subprocess.run(
        [
            sys.executable,
            str(REPO_ROOT / "scripts" / "governance" / "generators" / "generate_resource_profile_registry.py"),
            "--check",
            "--output",
            str(out),
            "--existing",
...
def test_drill_avoidance_shifts_cron_and_marks_notes(tmp_path, monkeypatch):
    ...
    for key in list(gen.DRILL_OVERRIDES):
        monkeypatch.setitem(gen.DRILL_OVERRIDES, key, dict(gen.DRILL_OVERRIDES[key], dmin=60))
    ents, warns = gen.parse_drill_entities(_drill_file(tmp_path, _DRILL_TRUTH_SAMPLES))
    assert warns == []  # 避让不是降级，不该出归一告警
    assert all(e["window_expr"].startswith("0 11 ") for e in ents), [
        e["window_expr"] for e in ents
    ]  # 06:30+60min → 11:00 起跑（时点进 cron）
...
def test_drill_production_source_parses_as_cron_or_manual_only():
    ...
    assert not any(
        w.startswith(("drill_source_missing", "drill_source_unreadable", "drill_source_empty")) for w in warns
    ), warns
    for e in ents:
...
def fake_repo(tmp_path, monkeypatch):
    ...
    w("scripts/fake/gated_varroot.ps1", '$RepoRoot = "D:\\ZephyrAlpha"\n$a = "$RepoRoot\\scripts\\fake\\gated.py"\n')
    w("scripts/fake/gated_driver.ps1", 'python "D:/ZephyrAlpha/scripts/fake/decision.py"\n')
...
def test_c11_ledger_separates_gated_gap_unresolved(fake_repo):
    ...
    assert by["data_slot_host_probe"] == {
        "task_id": "data_slot_host_probe",
        "verdict": "gap",
        "how": "data_slot_host",
        "sources": [gen.EXECUTOR_SOURCE_RELPATH],
    }
    assert {by[t]["verdict"] for t in ("sch_unreach_py", "sch_unreach_chain", "sch_unreach_nosrc")} == {"unresolved"}
    assert all(by[t]["sources"] == [] for t in ("sch_unreach_py", "sch_unreach_chain", "sch_unreach_nosrc"))
    # findings 只属于 gap（unresolved 是"查不动"，不是"违规"）
    assert sorted(tid for f in findings for tid in f["task_ids"]) == ["data_slot_host_probe", "sch_gap_chain"]
...
def test_c11_recognizes_gate_call_shape_not_symbol_name(fake_repo):
    ...
    ents = [
        _ent("sch_bare", "scripts/fake/bare.py"),
    ...
    assert verdicts == {"sch_bare": "gap", "sch_check": "gated", "sch_decision": "gated", "sch_e0call": "gated"}
...
def test_c11_annotation_is_idempotent_and_spares_human_text(fake_repo):
    ...
    ents = [
        ...
        _ent("sch_gated_direct", "scripts/fake/gated.py"),
        _ent(
            "sch_human_stale_note",
            "scripts/fake/gated.py",
            notes_zh="人写的正文 保留 " + gen.GAP_NOTE_MARK + " 上一轮的旧账尾注",
        ),
        _ent("sch_unreach_py", "scripts/fake/gone.py"),
        {
            "task_id": "sch_no_notes_key",
            "status": "active",
            "trading_sensitive": True,
...
def test_c11_script_token_normalization(fake_repo):
    ...
    assert [
        p for p in gen._ps1_chain_targets('python "scripts/fake/bare.py" "scripts/fake/hop.ps1"') if p.endswith(".ps1")
    ] == ["scripts/fake/hop.ps1"]
