...
class TestTriggerYamlWiring:
    """测试 _trigger() 从 YAML 读取触发参数（F-06 Track A 治本）

    验证 _trigger 调用 _load_config() 读取 trigger.important_prefixes /
    ...
    def test_trigger_reads_custom_prefixes_from_yaml(self, reconciler_module, tmp_path):
        """_trigger 应从 YAML 读取自定义 important_prefixes"""
        ...
        with patch.object(reconciler_module, "_load_config", return_value=fake_config):
            with patch.object(reconciler_module, "_project_root", tmp_path):
                with patch.object(reconciler_module, "_load_state", return_value={}):
                    # custom/ 在 YAML 中 -> 触发
                    assert reconciler_module._trigger(committed_custom) is True
                    # src/ 不在 YAML 中 -> 不触发
                    assert reconciler_module._trigger(committed_src) is False

    def test_trigger_reads_custom_min_interval_from_yaml(self, reconciler_module, tmp_path):
        """_trigger 应从 YAML 读取自定义 min_interval_seconds"""
    ...
    def test_trigger_falls_back_to_defaults_when_yaml_empty(self, reconciler_module, tmp_path):
        ...
        committed = [str(tmp_path / "src" / "foo.py")]
        with patch.object(reconciler_module, "_load_config", return_value={}):
            with patch.object(reconciler_module, "_project_root", tmp_path):
                with patch.object(reconciler_module, "_load_state", return_value={}):
                    # 硬编码 _IMPORTANT_PREFIXES 含 src/ -> 触发
                    assert reconciler_module._trigger(committed) is True
...
class TestStatePersistence:
    ...
    F-06 Track A（2026-07-17）：_load_state/_update_state 通过 _get_state_file() 读取
    backup_config.yaml §trigger.state_file，fallback 到 _STATE_FILE。测试 patch
    _get_state_file 以隔离 YAML 依赖。
    """

    def test_load_state_returns_empty_when_no_file(self, reconciler_module, tmp_path):
        """状态文件不存在时返回空dict"""
        with patch.object(reconciler_module, "_get_state_file", return_value=tmp_path / "nonexistent.json"):
    ...
    def test_load_state_returns_dict_when_file_exists(self, reconciler_module, tmp_path):
        ...
        with patch.object(reconciler_module, "_get_state_file", return_value=state_file):
...
class TestGetStateFileYamlWiring:
    """测试 _get_state_file() 从 YAML 读取 state_file（F-06 Track A 治本）"""

    def test_get_state_file_reads_from_yaml(self, reconciler_module, tmp_path):
        """_get_state_file 应从 YAML trigger.state_file 读取路径"""
        fake_config = {"trigger": {"state_file": "custom/path/state.json"}}
        with patch.object(reconciler_module, "_load_config", return_value=fake_config):
            with patch.object(reconciler_module, "_project_root", tmp_path):
                result = reconciler_module._get_state_file()
    ...
    def test_get_state_file_falls_back_to_constant(self, reconciler_module, tmp_path):
        """YAML 缺失 state_file 时 fallback 到 _STATE_FILE"""
        with patch.object(reconciler_module, "_load_config", return_value={}):
            with patch.object(reconciler_module, "_STATE_FILE", tmp_path / "fallback.json"):
                result = reconciler_module._get_state_file()
                assert result == tmp_path / "fallback.json"

    def test_get_state_file_falls_back_when_config_none(self, reconciler_module, tmp_path):
        """_load_config 返回 None 时 fallback 到 _STATE_FILE"""
        with patch.object(reconciler_module, "_load_config", return_value=None):
            with patch.object(reconciler_module, "_STATE_FILE", tmp_path / "fallback.json"):
                result = reconciler_module._get_state_file()
                assert result == tmp_path / "fallback.json"
