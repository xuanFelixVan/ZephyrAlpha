# [A_test] module_id: SRC-TST-0666 | layer=test | stability=volatile | safety=L | ai_autonomy=ai_modifiable
# [BLUEPRINT] MOD-INF-007 | docs/03_modules/_cross_layer/gate-engine/blueprint.md | §3-§7
# [MODULE] tests.test_ct_drift_budget
...
# [TESTS] pytest tests/test_ct_drift_budget.py
...
class TestDriftBudgetHandlerImport:
    def test_import_module(self):
        import importlib
        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_drift_budget")
        assert hasattr(mod, "DriftBudgetHandler")

    def test_class_is_check_type_handler(self):
        import importlib
        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_drift_budget")
        assert issubclass(mod.DriftBudgetHandler, CheckTypeHandler)

class TestDriftBudgetHandlerInstantiation:
    def test_instantiate(self):
        import importlib
        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_drift_budget")
        handler = mod.DriftBudgetHandler()
    ...
    def test_name_attribute(self):
        ...
        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_drift_budget")
        handler = mod.DriftBudgetHandler()
        assert handler.name == "drift_budget"

    def test_registered_in_registry(self):
        cls = get_check_type("drift_budget")
        assert cls is not None

class TestDriftBudgetHandlerRun:
    ...
    def test_run_violations_have_message(self):
        ...
        for v in result:
            assert "message" in v
class TestDriftBudgetHandlerExternalDep:
