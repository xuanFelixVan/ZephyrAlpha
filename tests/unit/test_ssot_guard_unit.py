# [A_test] module_id: SRC-TST-2070 | layer=test | stability=volatile | safety=L | ai_autonomy=ai_modifiable
# [BLUEPRINT] SRC-687 | docs/03_modules/_domain-governance/blueprint.md | §
...
def _make_guard_with_staged(
    ...
    patcher = patch("zephyr.integration.shared_08.security.ssot_guard._staged_files", return_value=staged)
    mock = patcher.start()
...
class TestSsotGuardRunIntegration:
    ...
    def test_no_staged_files_all_pass(self, tmp_repo: Path) -> None:
        with patch("zephyr.integration.shared_08.security.ssot_guard._staged_files", return_value={}):
            guard = SsotGuard(repo_root=tmp_repo)
            report = guard.run()
        assert report.passed is True
    ...
    def test_new_watched_file_without_registry_fails(self, tmp_repo: Path) -> None:
        ...
        with patch("zephyr.integration.shared_08.security.ssot_guard._staged_files", return_value=staged):
            guard = SsotGuard(repo_root=tmp_repo)
            report = guard.run()
        assert report.passed is False
    ...
    def test_git_error_produces_failed_report(self, tmp_repo: Path) -> None:
        with patch(
            "zephyr.integration.shared_08.security.ssot_guard._staged_files",
