...
class TestN01Uppercase:
    def test_lowercase_passes(self) -> None:
        assert check_file("docs/01_policies_and_standards/governance/document/file-naming-standard.md") == []
...
class TestN05AdrMissingSuffix:
    def test_missing_suffix_detected(self) -> None:
        assert "N-05" in _rules(check_file("docs/adr/adr-0042.md"))

    def test_template_exempt(self) -> None:
        assert "N-05" not in _rules(check_file("docs/adr/_template.md"))
...
class TestN06ModuleIdScope:
    ...
    def test_adr_prefix_legal(self, tmp_path: Path) -> None:
        fake = tmp_path / "adr-0001-something.md"
        fake.write_text("---\nmodule_id: ADR-0001\n---\n", encoding="utf-8")
...
class TestN07AdrIdFilenameMismatch:
    def test_mismatch_detected(self, tmp_path: Path) -> None:
        ...
        fake.write_text("---\nmodule_id: ADR-0007\n---\n", encoding="utf-8")
        assert "N-07" in _rules(check_file("adr-0005-something.md", fake))
...
class TestIntegration:
    ...
    def test_clean_file_zero_violations(self, tmp_path: Path) -> None:
        fake = tmp_path / "adr-0050-example-decision.md"
        fake.write_text("---\nmodule_id: ADR-0050\n---\n", encoding="utf-8")
        assert check_file("adr-0050-example-decision.md", fake) == []
