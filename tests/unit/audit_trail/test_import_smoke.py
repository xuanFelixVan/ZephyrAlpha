...
class TestAuditTrailImportSmoke:
    """验证 audit_trail 核心模块可以被成功导入."""

    def test_import_audit_trail_init(self) -> None:
        """测试导入 zephyr.audit_trail 包自身."""
        _ensure_path()
        import zephyr.audit_trail  # noqa: F401
        assert hasattr(zephyr.audit_trail, "__all__")

    def test_import_models(self) -> None:
        """Can import AuditEntryV1, AuditEventType, LamportClock from audit_trail.models."""
        _ensure_path()
        from zephyr.audit_trail.models import AuditEntryV1, AuditEventType, LamportClock, ProvenanceLevel
