...
class TestAsOfJoin:
    ...
    def test_event_time_beyond_query_excluded(self):
        ...
        visible = mgr.as_of_join(df, pd.Timestamp("2024-03-01"), available_time_col="avail")
...
class TestConsistency:
    ...
    def test_missing_factor_col_raises(self):
        mgr = PITManager()
        with pytest.raises(PITError):
            mgr.pit_consistency_test(pd.DataFrame({"g": [1]}), pd.DataFrame({"f": [1]}), "f")
...
class TestApplyEmbargoTradingCalendar:
    ...
    def test_holiday_not_counted_with_calendar(self):
        mgr = PITManager(PITConfig(embargo_days=5))
        data = _daily_labels("2024-09-20", "2024-10-09")
        safe = mgr.apply_embargo(data, pd.Timestamp("2024-10-09"), trading_calendar=_TRADING_DAYS)
    ...
    def test_calendar_stricter_than_bday_across_holiday(self):
        mgr = PITManager(PITConfig(embargo_days=5))
        data = _daily_labels("2024-09-20", "2024-10-09")
        safe_cal = mgr.apply_embargo(data, pd.Timestamp("2024-10-09"), trading_calendar=_TRADING_DAYS)
    ...
    def test_current_on_non_trading_day_anchors_last_session(self):
        ...
        safe = mgr.apply_embargo(data, pd.Timestamp("2024-10-12"), trading_calendar=_TRADING_DAYS)
    ...
    def test_calendar_input_forms_equivalent(self):
        ...
        mgr = PITManager(PITConfig(embargo_days=5))
        data = _daily_labels("2024-09-20", "2024-10-09")
        by_str = mgr.apply_embargo(data, pd.Timestamp("2024-10-09"), trading_calendar=_TRADING_DAYS)
    ...
    def test_current_before_calendar_start_blocks_all(self):
        ...
        safe = mgr.apply_embargo(data, pd.Timestamp("2024-09-01"), trading_calendar=_TRADING_DAYS)
        assert safe.empty
