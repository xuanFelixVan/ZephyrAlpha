...
结构性防御（本守卫钉扎，防配置回退）：
  ...
  2. scratch ini（.runtime/tmp/pytest_min.ini，-c 引用时整体替换 pyproject）
...
def test_scratch_ini_mirrors_hygiene():
    """scratch ini（-c 引用时替换 pyproject）必须自带同款 norecursedirs。"""
    ini = _REPO / ".runtime" / "tmp" / "pytest_min.ini"
    if not ini.exists():
        pytest.skip("scratch ini 未创建（本地工具件）")
...
def test_stray_copy_never_collected(tmp_path):
    ...
    try:
        # 注意：scratch ini addopts 已含 -q，命令行再加 -q 会变 -qq（collect-only
        # 连 nodeid 行都吞掉），故此处不再传 -q。
        out = _pytest("tests/backtest/test_engine_base.py", "--collect-only",
                      "--no-header", "-c", ".runtime/tmp/pytest_min.ini")
