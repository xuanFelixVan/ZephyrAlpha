...
class TestEvaluate:
    ...
    def test_evaluate_threshold_rule_reject(self):
        psg = ParameterizedSafetyGate(rules=[
            {"layer": "L_SEV", "type": "threshold", "field": "severity", "threshold": 8, "op": "gt", "gate_type": "HARD", "reject_reason": "Severity {value} > {threshold}"},
        ])
    ...
    def test_evaluate_threshold_rule_pass(self):
        psg = ParameterizedSafetyGate(rules=[
            {"layer": "L_SEV", "type": "threshold", "field": "severity", "threshold": 8, "op": "gt", "gate_type": "HARD"},
        ])
    ...
    def test_evaluate_boolean_rule_reject(self):
        psg = ParameterizedSafetyGate(rules=[
            {"layer": "L_RBAC", "type": "boolean", "field": "rbac_authorized", "expected": True, "gate_type": "HARD", "reject_reason": "RBAC denied"},
        ])
    ...
    def test_evaluate_frequency_rule(self):
        psg = ParameterizedSafetyGate(rules=[
            {"layer": "L_FREQ", "type": "frequency", "limit": 2, "gate_type": "SOFT", "reject_reason": "Rate limit exceeded"},
        ])
        ctx = ActionContext(action_id="a1", action_type="REPAIR")
        psg.evaluate(ctx)
        psg.evaluate(ctx)
    ...
    def test_hard_reject_short_circuits(self):
        psg = ParameterizedSafetyGate(rules=[
            ...
            {"layer": "L2", "type": "threshold", "field": "severity", "threshold": 0, "op": "gt", "gate_type": "HARD", "reject_reason": "blocked"},
...
class TestProperties:
    ...
    def test_is_blocked_true_when_reject(self):
        psg = ParameterizedSafetyGate(rules=[
            {"layer": "L1", "type": "threshold", "field": "severity", "threshold": 0, "op": "gt", "gate_type": "HARD", "reject_reason": "blocked"},
        ])
        ctx = ActionContext(action_id="a1", action_type="REPAIR", severity=5)
        psg.results = psg.evaluate(ctx)
...
class TestBoundaries:
    ...
    def test_evaluate_enum_rule(self):
        psg = ParameterizedSafetyGate(rules=[
            {"layer": "L_ENUM", "type": "enum", "field": "action_type", "allowed_values": ["REPAIR", "DIAGNOSE"], "gate_type": "HARD", "reject_reason": "Invalid action {value}"},
        ])
