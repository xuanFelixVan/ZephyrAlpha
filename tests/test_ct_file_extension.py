# [A_test] module_id: SRC-TST-0670 | layer=test | stability=volatile | safety=L | ai_autonomy=ai_modifiable
# [BLUEPRINT] MOD-INF-007 | docs/03_modules/_cross_layer/gate-engine/blueprint.md | §3-§7
# [MODULE] tests.test_ct_file_extension
...
# [TESTS] pytest tests/test_ct_file_extension.py
...
class TestFileExtensionHandlerImport:
    def test_import_module(self):
        import importlib
        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_file_extension")
        assert hasattr(mod, "FileExtensionHandler")

    def test_class_is_check_type_handler(self):
        import importlib
        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_file_extension")
        assert issubclass(mod.FileExtensionHandler, CheckTypeHandler)

class TestFileExtensionHandlerInstantiation:
    def test_instantiate(self):
        import importlib
        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_file_extension")
        handler = mod.FileExtensionHandler()
    ...
    def test_name_attribute(self):
        ...
        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_file_extension")
        handler = mod.FileExtensionHandler()
        assert handler.name == "file_extension"

    def test_registered_in_registry(self):
        cls = get_check_type("file_extension")
        assert cls is not None

class TestFileExtensionHandlerRun:
    ...
    def test_run_violations_have_message(self):
        ...
        for v in result:
            assert "message" in v
class TestFileExtensionHandlerFileExtension:
    def test_allowed_extension_passes(self):
        cls = get_check_type("file_extension")
        handler = cls()
        task = _make_task(deliverables=["src/main.py"])
        result = handler.run(task, {"allowed_extensions": [".py"]}, _MockCheck(), Path("."))
        assert len(result) == 0

    def test_disallowed_extension_fails(self):
        cls = get_check_type("file_extension")
        handler = cls()
        task = _make_task(deliverables=["src/data.exe"])
        result = handler.run(task, {"allowed_extensions": [".py"]}, _MockCheck(), Path("."))
        assert len(result) >= 1

    def test_no_allowed_list_no_violation(self):
        cls = get_check_type("file_extension")
        handler = cls()
        task = _make_task(deliverables=["src/data.exe"])
        result = handler.run(task, {"allowed_extensions": []}, _MockCheck(), Path("."))
        assert len(result) == 0
