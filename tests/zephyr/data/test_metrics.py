...
class TestIntegratorMetrics:
    ...
    def test_record_task_counter(self, metrics):
        ...
        metrics.record_task("t1", "akshare", "SUCCESS", 5.0, 100)
        metrics.record_task("t1", "akshare", "SUCCESS", 6.0, 200)
        metrics.record_task("t1", "akshare", "FAILED", 1.0, 0)
        rendered = metrics.render()
        assert 'integrator_task_total{task_id="t1",source="akshare",status="SUCCESS"} 2' in rendered
        assert 'integrator_task_total{task_id="t1",source="akshare",status="FAILED"} 1' in rendered

    def test_record_task_duration_histogram(self, metrics):
        """record_task 更新耗时直方图。"""
        # 3 个耗时样本：0.05s, 2.0s, 100s
        metrics.record_task("t1", "akshare", "SUCCESS", 0.05, 10)
        metrics.record_task("t1", "akshare", "SUCCESS", 2.0, 20)
        metrics.record_task("t1", "akshare", "SUCCESS", 100.0, 30)
        rendered = metrics.render()
        # count=3, sum=102.05
        assert 'integrator_task_duration_seconds_count{task_id="t1",source="akshare"} 3' in rendered
        assert 'integrator_task_duration_seconds_sum{task_id="t1",source="akshare"} 102.05' in rendered
        # le=0.1 桶应含 1 个（0.05）
        assert 'integrator_task_duration_seconds_bucket{le="0.1",task_id="t1",source="akshare"} 1' in rendered
        # le=5.0 桶应含 2 个（0.05+2.0）
        assert 'integrator_task_duration_seconds_bucket{le="5.0",task_id="t1",source="akshare"} 2' in rendered
        # le="+Inf" 桶应含 3 个
        assert 'integrator_task_duration_seconds_bucket{le="+Inf",task_id="t1",source="akshare"} 3' in rendered

    def test_record_task_rows(self, metrics):
        """record_task 累加 rows_fetched_total。"""
        metrics.record_task("t1", "akshare", "SUCCESS", 1.0, 100)
        metrics.record_task("t1", "akshare", "SUCCESS", 1.0, 200)
        rendered = metrics.render()
    ...
    def test_record_retry(self, metrics):
        """record_retry 累加重试次数。"""
        metrics.record_retry("t1", "akshare")
        metrics.record_retry("t1", "akshare")
        rendered = metrics.render()
        assert 'integrator_retry_total{task_id="t1",source="akshare"} 2' in rendered
...
class TestIntegratorMetricsThreadSafety:
    ...
    def test_concurrent_record_task(self, metrics):
        ...
        def worker():
            for i in range(100):
                metrics.record_task("t1", "akshare", "SUCCESS", 0.5, 10)
        ...
        rendered = metrics.render()
        # 10 线程 × 100 次 = 1000
        assert 'integrator_task_total{task_id="t1",source="akshare",status="SUCCESS"} 1000' in rendered
        # rows: 1000 × 10 = 10000
        assert 'integrator_rows_fetched_total{task_id="t1",source="akshare"} 10000' in rendered
