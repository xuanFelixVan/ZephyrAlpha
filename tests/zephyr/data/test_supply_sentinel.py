...
def _run(
    tmp_path: Path, entries: list[dict[str, Any]], runner: _ScriptedChRunner, *, today: date = _TODAY
) -> dict:
...
def test_stale_business_date_alarms_even_when_ingest_leg_is_fresh(tmp_path: Path) -> None:
    ...
    entries = [
        {"table": "c1_market.rate_decision_calendar", "date_col": "ingest_ts",
         "max_lag_days": 2, "past_only": False, "heartbeat_leg": True},
        {"table": "c1_market.rate_decision_calendar", "date_col": "decision_date",
         "max_lag_days": 75, "past_only": False},
    ]
    runner = _ScriptedChRunner(
        {"max(ingest_ts)": "2026-09-18 10:04:15.000", "max(decision_date)": "2025-10-30"}
    )
...
def test_heartbeat_only_coverage_is_reported_as_blind_spot(tmp_path: Path) -> None:
    ...
    entries = [{"table": "c1_market.rate_decision_calendar", "date_col": "ingest_ts",
                "max_lag_days": 2, "heartbeat_leg": True}]
...
def test_business_leg_clears_the_blind_spot_flag(tmp_path: Path) -> None:
    entries = [{"table": "t.a", "date_col": "ingest_ts", "max_lag_days": 2, "heartbeat_leg": True},
...
def test_heartbeat_column_as_date_col_requires_explicit_declaration(tmp_path: Path) -> None:
    ...
    with pytest.raises(SupplySentinelError, match="heartbeat_leg"):
        _run(tmp_path, [{"table": "t.a", "date_col": "ingest_ts", "max_lag_days": 2}],
             _ScriptedChRunner({}))
...
def test_all_heartbeat_columns_are_guarded() -> None:
    ...
    assert find_heartbeat_only_blind_spots([{"table": "t.a", "date_col": "ingest_ts",
                                             "heartbeat_leg": True}]) == ["t.a"]
...
def test_row_filter_leg_detects_single_exchange_outage(tmp_path: Path) -> None:
    ...
    entries = [
        {"table": "c1_market.futures_warehouse_receipt", "date_col": "trade_date",
         "max_lag_days": 10, "past_only": True},
        {"table": "c1_market.futures_warehouse_receipt", "date_col": "trade_date",
         "max_lag_days": 10, "past_only": True, "row_filter": "exchange = 'SHFE'",
         "leg_name": "SHFE"},
    ]
    runner = _ScriptedChRunner(
...
def test_row_filter_rejects_statement_escape(tmp_path: Path) -> None:
    with pytest.raises(SupplySentinelError, match="row_filter"):
        _run(tmp_path, [{"table": "t.a", "date_col": "trade_date", "max_lag_days": 1,
                         "row_filter": "1=1; DROP TABLE c1_market.kline_daily"}], _ScriptedChRunner({}))
...
def test_trading_day_basis_absorbs_holiday_gap(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    entry = {"table": "t.a", "date_col": "trade_date", "max_lag_days": 2,
             "past_only": True, "lag_basis": "trading_days"}
...
def test_static_backfill_cadence_uses_row_floor_not_date_lag(tmp_path: Path) -> None:
    entry = {"table": "t.hist", "date_col": "end_date", "max_lag_days": 5,
             "cadence": "static_backfill", "min_rows_in_window": {"window_days": 0, "min_rows": 1000}}
...
def test_source_row_floor_detects_synth_only_window(tmp_path: Path) -> None:
    ...
    entry = {"table": "c1_market.kline_sector_intraday", "date_col": "trade_date",
             "max_lag_days": 5, "past_only": True, "row_filter": "data_source = 'tdx'",
             "min_rows_in_window": {"window_days": 5, "min_rows": 1000}, "leg_name": "tdx 真值"}
...
def test_fill_ratio_flags_all_zero_key_columns(tmp_path: Path) -> None:
    ...
    entry = {"table": "c1_market.daily_valuation", "date_col": "trade_date",
             "max_lag_days": 5, "past_only": True,
             "column_fill_ratio": {"cols": ["close", "amount", "turnover"],
                                   "min_ratio": 0.95, "window_days": 10}}
    runner = _ScriptedChRunner(
        {"max(trade_date)": "2026-09-17",
         "SELECT count(), countIf(ifNull(close, 0) != 0)": "259238\t0\t0\t0"}
    )
    summary = _run(tmp_path, [entry], runner)
...
def test_fill_ratio_passes_when_columns_are_filled(tmp_path: Path) -> None:
    entry = {"table": "t.a", "date_col": "trade_date", "max_lag_days": 5,
...
def test_fill_ratio_notnull_mode_for_computed_columns(tmp_path: Path) -> None:
    ...
    entry = {"table": "t.iv", "date_col": "trade_date", "max_lag_days": 7,
             "column_fill_ratio": {"cols": ["cape_5y", "erp"], "min_ratio": 0.9,
                                   "window_days": 0, "treat_zero_as_missing": False}}
    runner = _ScriptedChRunner(
        {"max(trade_date)": "2026-09-17", "SELECT count(), countIf(isNotNull": "8125\t0\t0"}
    )
    summary = _run(tmp_path, [entry], runner)
    assert summary["breached"] == 1
...
def test_unknown_field_fails_closed(tmp_path: Path) -> None:
    with pytest.raises(SupplySentinelError, match="未知字段"):
        _run(tmp_path, [{"table": "t.a", "date_col": "trade_date", "max_lag_days": 5,
                         "max_lag_dayz": 9}], _ScriptedChRunner({}))
...
def test_bad_enum_and_ratio_value_domains_fail_closed(tmp_path: Path) -> None:
    bad = [
        ...
        {"table": "t.a", "date_col": "d", "max_lag_days": 1,
         "column_fill_ratio": {"cols": ["x"], "min_ratio": 0}},
        {"table": "t.a", "date_col": "d", "max_lag_days": 1,
...
def test_shipped_config_has_business_leg_for_the_four_measured_blind_spots() -> None:
    ...
    table_level = [
        ...
        and e["date_col"] == "decision_date" and not e.get("row_filter")
    ]
...
def test_shipped_config_daily_tables_are_not_loosened_past_ten_days() -> None:
    ...
    daily = {"c1_market.alt_sz_reservoir_level", "c1_market.daily_valuation",
             "c1_market.index_valuation_daily", "c1_market.kline_sector_intraday",
...
def test_shipped_config_covers_every_measured_table_with_a_row_floor_or_fill_leg() -> None:
    ...
    assert sorted(dv.get("cols") or []) == ["amount", "close", "turnover"], (
        "填充率腿必须钉住实测全 0 的那三列本身——变异 μ3 实测：换成 pe_ttm/pb_mrq 也"
        "能过「有 column_fill_ratio」这种粗断言，等于把 N-1 型重新放走"
    )
    iv = by_table["c1_market.index_valuation_daily"].get("column_fill_ratio") or {}
    assert sorted(iv.get("cols") or []) == ["cape_5y", "erp", "pe_pct"]
    floored = [e for e in entries if e["table"] == "c1_market.kline_sector_intraday"
               and e.get("min_rows_in_window")]
