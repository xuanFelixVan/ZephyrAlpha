...
def test_kline_capabilities_include_bj_sector():
    ...
    for cap in ("kline_daily", "kline_1min", "kline_5min", "kline_15min", "kline_30min", "kline_60min"):
...
def test_write_tsv_outcome_http_fail_invalidates_cache(monkeypatch):
    ...
    monkeypatch.setattr(ch_writer, "save_fallback", lambda *a, **k: True) if hasattr(
        ch_writer, "save_fallback"
    ) else None
    # create_fallback=False 走 no-fallback 分支（不依赖 local_replay）
    out = ch_writer.write_tsv_outcome("c1_market.t2", "(a)", b"1\n", create_fallback=False)
...
class _FakeScheduler:
    ...
    def __init__(self):
        ...
        # WO-12/C5：BRK-049 收口后告警经 _deliver_alert_with_latch 投递，替身须同步绑定
        self._deliver_alert_with_latch = IntegratorSchedulerMixin._deliver_alert_with_latch.__get__(self)
        self.verify_daily_kline_coverage = IntegratorSchedulerMixin.verify_daily_kline_coverage.__get__(self)
