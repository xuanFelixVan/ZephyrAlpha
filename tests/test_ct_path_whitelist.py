# [A_test] module_id: SRC-TST-0678 | layer=test | stability=volatile | safety=L | ai_autonomy=ai_modifiable
# [BLUEPRINT] MOD-INF-007 | docs/03_modules/_cross_layer/gate-engine/blueprint.md | §3-§7
# [MODULE] tests.test_ct_path_whitelist
...
# [TESTS] pytest tests/test_ct_path_whitelist.py
...
class TestPathWhitelistHandlerImport:
    def test_import_module(self):
        import importlib
        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_path_whitelist")
        assert hasattr(mod, "PathWhitelistHandler")

    def test_class_is_check_type_handler(self):
        import importlib
        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_path_whitelist")
        assert issubclass(mod.PathWhitelistHandler, CheckTypeHandler)

class TestPathWhitelistHandlerInstantiation:
    def test_instantiate(self):
        import importlib
        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_path_whitelist")
        handler = mod.PathWhitelistHandler()
    ...
    def test_name_attribute(self):
        ...
        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_path_whitelist")
        handler = mod.PathWhitelistHandler()
        assert handler.name == "path_whitelist"

    def test_registered_in_registry(self):
        cls = get_check_type("path_whitelist")
        assert cls is not None

class TestPathWhitelistHandlerRun:
    ...
    def test_run_violations_have_message(self):
        ...
        for v in result:
            assert "message" in v
class TestPathWhitelistHandlerSkipBehavior:
