...
from zephyr.governance.batch_ingest import BatchIngestEntry, BatchIngestReport, BatchIngestor
...
def _mock_ingest_gate(passed: bool = True) -> MagicMock:
    ...
    if passed:
        ...
        gate.ingest.return_value = IngestResult(
            passed=False, violations=["some violation"]
        )
    return gate
...
class TestBatchIngestEntry:
    ...
    def test_custom_values(self):
        e = BatchIngestEntry(
            ke_id="KE-002", title="Test2", category="adr", source_file="a.md",
            priority="P0", status="succeeded", error="err"
        )
...
class TestBatchIngestReport:
    ...
    def test_to_markdown(self):
        r = BatchIngestReport(
            total=3, succeeded=2, failed=1, skipped=0, success_rate=2 / 3,
            entries=[
                BatchIngestEntry(ke_id="KE-001", title="T1", category="g", source_file="a.md", status="succeeded"),
                BatchIngestEntry(ke_id="KE-002", title="T2", category="g", source_file="b.md", status="failed", error="bad"),
            ],
        )
        md = r.to_markdown()
        assert "批量入库报告" in md
