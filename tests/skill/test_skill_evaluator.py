# [A_test] module_id: SRC-TST-1620 | layer=test | stability=volatile | safety=L | ai_autonomy=ai_modifiable
...
class TestSkillEvaluatorEvaluateStructure:
    def test_full_structure(self):
        ...
        body = "核心操作\n约束\n常见错误\n前置条件\n返回格式\n"
        score, issues = SkillEvaluator._evaluate_structure(body, l1)
        assert score == 100.0
        assert issues == []
    ...
    def test_empty_l1(self):
        score, issues = SkillEvaluator._evaluate_structure("body", {})
...
class TestSkillEvaluatorEvaluateDensity:
    def test_dense_body(self):
        ...
        score, detail = SkillEvaluator._evaluate_density(body)
        assert score > 0
    ...
    def test_empty_body(self):
        score, detail = SkillEvaluator._evaluate_density("")
        assert score == 0.0
...
class TestSkillEvaluatorEvaluateConstraints:
    def test_all_categories_covered(self):
        ...
        score, missing = SkillEvaluator._evaluate_constraints(body)
        assert score == 100.0
        assert missing == []

    def test_no_constraints(self):
        score, missing = SkillEvaluator._evaluate_constraints("hello world")
        assert score == 0.0
...
class TestSkillEvaluatorEvaluateFreshness:
    def test_no_freshness_data(self):
        score, detail = SkillEvaluator._evaluate_freshness(None)
        assert score == 50.0
        assert detail["detail"] == "no_freshness_data"

    def test_with_freshness_data(self):
        data = {"freshness_score": 80.0}
        score, detail = SkillEvaluator._evaluate_freshness(data)
        assert score == 80.0
...
class TestSkillEvaluatorEvaluateTokenEfficiency:
    def test_zero_tokens(self):
        score, detail = SkillEvaluator._evaluate_token_efficiency("body", 0)
        assert score == 0.0
        assert detail["detail"] == "zero_tokens"

    def test_efficient_body(self):
        body = "MUST do\n必须 done\n```py\nx=1\n```\n"
        score, detail = SkillEvaluator._evaluate_token_efficiency(body, 50)
        assert score > 0
        assert "tokens" in detail

    def test_inefficient_body(self):
        body = "plain text with no directives"
        score, detail = SkillEvaluator._evaluate_token_efficiency(body, 10000)
        assert score < 50.0
