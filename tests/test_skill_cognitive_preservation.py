# [A_test] module_id: SRC-TST-1608 | layer=test | stability=volatile | safety=L | ai_autonomy=ai_modifiable
...
class TestSkillCognitivePreservationInit:
    def test_instantiation_creates_empty_memory(self):
        with patch.object(SkillCognitivePreservation, "_load_all"):
            scp = SkillCognitivePreservation()
            assert scp._memory == {}

    def test_max_snapshots_constant(self):
        assert SkillCognitivePreservation._MAX_SNAPSHOTS_PER_SKILL == 20
...
class TestSave:
    def test_save_returns_version_and_persisted(self):
        with patch.object(SkillCognitivePreservation, "_load_all"):
            scp = SkillCognitivePreservation()
        with patch.object(scp, "_persist_snapshot"), patch.object(scp, "_prune_old_snapshots"):
    ...
    def test_save_increments_version_when_state_carries_version(self):
        ...
        with patch.object(scp, "_persist_snapshot"), patch.object(scp, "_prune_old_snapshots"):
            ...
            saved_state = scp._memory["sk-b"].state
    ...
    def test_save_stores_in_memory(self):
        ...
        assert "sk-e" in scp._memory
        assert scp._memory["sk-e"].state["val"] == 42
...
class TestRestore:
    ...
    def test_restore_returns_version(self):
        ...
        with patch.object(scp, "_persist_snapshot"), patch.object(scp, "_prune_old_snapshots"):
            ...
            saved_state = scp._memory["sk-g"].state
...
class TestMerge:
    ...
    def test_merge_into_nonexistent_creates_new(self):
        ...
        assert result["persisted"] is True
        assert scp._memory["sk-i"].state["x"] == 10
    ...
    def test_merge_empty_delta(self):
        ...
        assert result["persisted"] is True
        assert scp._memory["sk-j"].state["a"] == 1
...
class TestForget:
    def test_forget_removes_skill(self):
        ...
        with patch.object(scp, "_delete_snapshots"):
            scp.forget("sk-o")
        assert "sk-o" not in scp._memory
