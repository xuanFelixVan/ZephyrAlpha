"""Test gate g_trae_016 for rule TRAE-016 — calls gate_engine.evaluate().
...
GATE_YAML = PROJECT_ROOT / "src" / "zephyr" / "governance" / "rule_enforcement" / "g_trae_016.yaml"
RULE_YAML = PROJECT_ROOT / "docs" / "01_policies_and_standards" / "rules" / "trae_016_arch_drift_detection.yaml"
MOCK_YAML = PROJECT_ROOT / "tests" / "fixtures" / "g_trae_016_mock.yaml"


class TestGTrae016:
    """Red-Blue extreme tests for g_trae_016 gate."""
    ...
    def test_gate_registered_in_gate_files(self):
        ...
        assert "G_TRAE_016" in GateEngine._GATE_FILES, "Gate G_TRAE_016 not in _GATE_FILES"
    ...
    def test_gate_loadable_by_engine(self):
        ...
        try:
            if "G_TRAE_016" not in GateEngine._GATE_FILES:
                GateEngine._GATE_FILES["G_TRAE_016"] = "g_trae_016.yaml"
                engine._gate_cache = None  # Clear cache
            gates = engine.load_gates()
            assert "G_TRAE_016" in gates, "Gate G_TRAE_016 not loaded"
        finally:
    ...
    def test_gate_evaluates_pass(self):
        ...
        try:
            ...
            valid_task = Task(
                ...
                acceptance=["pytest"],
                applicable_rules=[{"module_id": "TRAE-016", "section": "all", "reason": "gate test"}],
            ...
            result = engine.evaluate(valid_task, "G_TRAE_016")
    ...
    def test_gate_evaluates_fail(self):
        ...
        try:
            ...
            result = engine.evaluate(sparse_task, "G_TRAE_016")
    ...
    def test_gate_bypass_detection(self):
        ...
        try:
            ...
            gates = engine.load_gates()
            assert "G_TRAE_016" in gates, "Gate should be loadable when YAML exists"
        finally:
            GateEngine._GATE_FILES = original
            engine._gate_cache = None
