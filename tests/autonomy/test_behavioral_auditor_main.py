...
class TestMainFunction:
    def test_status_command(self):
        with (
            patch(
                "zephyr.governance.drift_detection.drift_engine.load_detector_registry", return_value=[MagicMock(status="active")]
            ),
            patch("zephyr.governance.drift_detection.self_test_verifier.SelfTestVerifier") as mock_cls,
        ):
            mock_inst = MagicMock()
            mock_inst.run_all.return_value = MagicMock(summary="8/8 checks passed")
            mock_cls.return_value = mock_inst
            with patch("zephyr.governance.drift_detection.self_check.bootstrap_self_check", return_value=True):
...
class TestCmdScan:
    ...
    def test_scan_success(self):
        ...
        with patch("zephyr.governance.drift_detection.drift_engine.ScanLevel", mock_scan_level):
            with patch("zephyr.governance.drift_detection.drift_engine.build_report", return_value=mock_report):
                with patch("zephyr.governance.drift_detection.drift_engine.scan", return_value=mock_result):
...
class TestCmdSelfTest:
    ...
    def test_self_test_pass(self):
        ...
        with patch("zephyr.governance.drift_detection.self_test_verifier.SelfTestVerifier", return_value=mock_verifier):
            result = _cmd_self_test(args)
            assert result == 0
...
class TestCmdList:
    ...
    def test_list_text(self):
        ...
        with patch("zephyr.governance.drift_detection.drift_engine.load_detector_registry", return_value=[mock_det]):
            result = _cmd_list(args)
            assert result == 0
    ...
    def test_list_empty(self):
        args = argparse.Namespace(json=False)
        with patch("zephyr.governance.drift_detection.drift_engine.load_detector_registry", return_value=[]):
            result = _cmd_list(args)
            assert result == 0
...
class TestCmdStatus:
    def test_status_all_ok(self):
        ...
        with (
            ...
            patch("zephyr.governance.drift_detection.self_test_verifier.SelfTestVerifier", return_value=mock_verifier),
        ):
            with patch("zephyr.governance.drift_detection.self_check.bootstrap_self_check", return_value=True):
    ...
    def test_status_degraded(self):
        ...
        with (
            ...
            with patch("zephyr.governance.drift_detection.self_check.bootstrap_self_check", return_value=False):
                result = _cmd_status(MagicMock())
                assert result == 1

    def test_status_registry_fail(self):
        with patch("zephyr.governance.drift_detection.drift_engine.load_detector_registry", side_effect=RuntimeError("fail")):
            with patch("zephyr.governance.drift_detection.self_test_verifier.SelfTestVerifier") as mock_cls:
                mock_inst = MagicMock()
                mock_inst.run_all.return_value = MagicMock(summary="8/8 checks passed")
                mock_cls.return_value = mock_inst
                with patch("zephyr.governance.drift_detection.self_check.bootstrap_self_check", return_value=True):
