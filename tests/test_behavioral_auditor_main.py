...
class TestCmdSelfTest:
    def test_import_error(self):
        ...
        with patch.dict("sys.modules", {"zephyr.behavioral_audit.self_test_verifier": None}):
...
class TestCmdBudget:
    def test_import_error(self):
        ...
        with patch.dict("sys.modules", {"zephyr.behavioral_audit.drift_infrastructure": None}):
...
class TestCmdList:
    ...
    def test_list_empty(self):
        args = argparse.Namespace(json=False)
        with patch("zephyr.behavioral_audit.drift_engine.load_detector_registry", return_value=[]):
            result = _cmd_list(args)
            assert result == 0
