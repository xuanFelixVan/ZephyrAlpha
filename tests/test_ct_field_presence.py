# [A_test] module_id: SRC-TST-0669 | layer=test | stability=volatile | safety=L | ai_autonomy=ai_modifiable
# [BLUEPRINT] MOD-INF-007 | docs/03_modules/_cross_layer/gate-engine/blueprint.md | §3-§7
# [MODULE] tests.test_ct_field_presence
...
# [TESTS] pytest tests/test_ct_field_presence.py
...
class TestFieldPresenceHandlerImport:
    def test_import_module(self):
        import importlib
        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_field_presence")
        assert hasattr(mod, "FieldPresenceHandler")

    def test_class_is_check_type_handler(self):
        import importlib
        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_field_presence")
        assert issubclass(mod.FieldPresenceHandler, CheckTypeHandler)

class TestFieldPresenceHandlerInstantiation:
    def test_instantiate(self):
        import importlib
        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_field_presence")
        handler = mod.FieldPresenceHandler()
    ...
    def test_name_attribute(self):
        ...
        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_field_presence")
        handler = mod.FieldPresenceHandler()
        assert handler.name == "field_presence"

    def test_registered_in_registry(self):
        cls = get_check_type("field_presence")
        assert cls is not None

class TestFieldPresenceHandlerRun:
    ...
    def test_run_violations_have_message(self):
        ...
        for v in result:
            assert "message" in v
class TestFieldPresenceHandlerFieldPresence:
    def test_missing_field_violation(self):
        cls = get_check_type("field_presence")
        handler = cls()
        task = _make_task()
        task.directive = ""
        result = handler.run(task, {"required_fields": ["directive"]}, _MockCheck(), Path("."))
        assert len(result) >= 1
    ...
    def test_present_field_passes(self):
        ...
        handler = cls()
        task = _make_task()
        result = handler.run(task, {"required_fields": ["task_id"]}, _MockCheck(), Path("."))
        assert len(result) == 0
    ...
    def test_empty_required_fields(self):
        ...
        handler = cls()
        task = _make_task()
        result = handler.run(task, {"required_fields": []}, _MockCheck(), Path("."))
        assert len(result) == 0
