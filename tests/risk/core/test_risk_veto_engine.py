...
def _snapshot(
    *,
    ...
    quotes = {s: _quote(s, p, suspended=(s in suspended)) for s, p in prices.items()}
    return assemble_risk_snapshot(
        RiskSnapshotInput(
            ...
            limits=limits,
            as_of=_NOW,
            sellable_quantities=({s: Decimal(v) for s, v in sellable.items()} if sellable else None),
        )
    )
...
class TestApprovePath:
    def test_small_buy_approved(self):
        ...
        limits = RiskLimits(
            as_of_date=_NOW,
            idempotency_key="lim-x",
            max_single_position=0.30,
            max_gross_leverage=1.0,
        )
        snap = _snapshot(
            holdings={"600519.SH": "10"},
            prices={"600519.SH": "1700"},
            limits=limits,
        )
    ...
    def test_sell_within_position_approved(self):
        ...
        decision = evaluate_vetoes(
            _request(side=OrderSide.SELL, quantity="5"),
            snap,
        )
        assert decision.approved
...
class TestVetoRules:
    ...
    def test_sell_exceeds_position_veto(self):
        snap = _snapshot(holdings={"600519.SH": "10"}, prices={"600519.SH": "1700"})
        decision = evaluate_vetoes(
            _request(side=OrderSide.SELL, quantity="20"),
            snap,
        )
        codes = [v.reason_code for v in decision.vetoes]
    ...
    def test_t1_sellable_exceeded_veto(self):
        ...
        decision = evaluate_vetoes(
            _request(side=OrderSide.SELL, quantity="6"),
            snap,
        )
        codes = [v.reason_code for v in decision.vetoes]
    ...
    def test_vetoes_sorted_by_priority(self):
        ...
        snap = _snapshot(holdings={"600519.SH": "10"}, prices={})
        decision = evaluate_vetoes(
            _request(side=OrderSide.SELL, quantity="100"),
            snap,
        )
