...
class TestEvaluate:
    ...
    def test_threshold_rule_reject(self):
        psg = ParameterizedSafetyGate(rules=[
            {"layer": "L5", "gate_type": "HARD", "type": "threshold", "field": "severity", "threshold": 8, "op": "gt", "reject_reason": "Severity {value} > {threshold}"},
        ])
    ...
    def test_threshold_rule_pass(self):
        psg = ParameterizedSafetyGate(rules=[
            {"layer": "L5", "gate_type": "HARD", "type": "threshold", "field": "severity", "threshold": 8, "op": "gt", "reject_reason": "Severity too high"},
        ])
    ...
    def test_boolean_rule_mismatch_reject(self):
        psg = ParameterizedSafetyGate(rules=[
            {"layer": "L6", "gate_type": "HARD", "type": "boolean", "field": "has_rollback", "expected": True, "reject_reason": "No rollback"},
        ])
    ...
    def test_boolean_rule_match_reject(self):
        psg = ParameterizedSafetyGate(rules=[
            {"layer": "L20", "gate_type": "HARD", "type": "boolean", "field": "in_circuit_breaker", "expected": True, "reject_on": "match", "reject_reason": "In CB"},
        ])
    ...
    def test_frequency_rule_reject(self):
        psg = ParameterizedSafetyGate(rules=[
            {"layer": "L2", "gate_type": "SOFT", "type": "frequency", "key_field": "action_type", "limit": 2, "reject_reason": "Freq exceeded"},
        ])
        ctx = ActionContext(action_id="a1", action_type="REPAIR")
        psg.evaluate(ctx)
        psg.evaluate(ctx)
    ...
    def test_hard_reject_short_circuits(self):
        psg = ParameterizedSafetyGate(rules=[
            {"layer": "L1", "gate_type": "HARD", "type": "boolean", "field": "has_rollback", "expected": True, "reject_reason": "No rollback"},
    ...
    def test_enum_rule_reject(self):
        psg = ParameterizedSafetyGate(rules=[
            {"layer": "L17", "gate_type": "HARD", "type": "enum", "field": "action_type", "allowed_values": ["REPAIR", "NOTIFY"], "reject_reason": "Invalid type"},
        ])
    ...
    def test_observe_rule(self):
        psg = ParameterizedSafetyGate(rules=[
            {"layer": "L21", "gate_type": "HARD", "type": "observe", "field": "cve_alerts", "reject_reason": "CVE alert: {value}"},
        ])
