"""Test gate g_trae_022 for rule TRAE-022 — calls gate_engine.evaluate().
...
GATE_YAML = PROJECT_ROOT / "src" / "zephyr" / "governance" / "rule_enforcement" / "g_trae_022.yaml"
RULE_YAML = PROJECT_ROOT / "docs" / "01_policies_and_standards" / "rules" / "trae_022_behavior_conditional_code.yaml"
MOCK_YAML = PROJECT_ROOT / "tests" / "fixtures" / "g_trae_022_mock.yaml"


class TestGTrae022:
    """Red-Blue extreme tests for g_trae_022 gate."""
    ...
    def test_gate_registered_in_gate_files(self):
        ...
        assert "G_TRAE_022" in GateEngine._GATE_FILES, "Gate G_TRAE_022 not in _GATE_FILES"
    ...
    def test_gate_loadable_by_engine(self):
        ...
        try:
            if "G_TRAE_022" not in GateEngine._GATE_FILES:
                GateEngine._GATE_FILES["G_TRAE_022"] = "g_trae_022.yaml"
                engine._gate_cache = None  # Clear cache
            gates = engine.load_gates()
            assert "G_TRAE_022" in gates, "Gate G_TRAE_022 not loaded"
        finally:
    ...
    def test_gate_evaluates_pass(self):
        ...
        try:
            ...
            valid_task = Task(
                ...
                acceptance=["pytest"],
                applicable_rules=[{"module_id": "TRAE-022", "section": "all", "reason": "gate test"}],
            ...
            result = engine.evaluate(valid_task, "G_TRAE_022")
    ...
    def test_gate_evaluates_fail(self):
        ...
        try:
            ...
            result = engine.evaluate(sparse_task, "G_TRAE_022")
    ...
    def test_gate_bypass_detection(self):
        ...
        try:
            ...
            gates = engine.load_gates()
            assert "G_TRAE_022" in gates, "Gate should be loadable when YAML exists"
        finally:
            GateEngine._GATE_FILES = original
            engine._gate_cache = None
