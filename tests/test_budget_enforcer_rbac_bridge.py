...
class TestCheckBudgetWithinLimit:
    def test_returns_allow_when_under_limit(self):
        bridge = BudgetRBACBridge()
        result = bridge.check_budget("agent-1", token_used=50, token_limit=100)
        assert result["action"] == "ALLOW"

    def test_returns_allow_when_at_exact_limit(self):
        bridge = BudgetRBACBridge()
        result = bridge.check_budget("agent-1", token_used=100, token_limit=100)
        assert result["action"] == "ALLOW"
    ...
    def test_zero_usage_within_limit(self):
        bridge = BudgetRBACBridge()
        result = bridge.check_budget("agent-1", token_used=0, token_limit=100)
        assert result["exceeded"] is False
        assert result["action"] == "ALLOW"
...
class TestCheckBudgetExceeded:
    def test_returns_revoke_write_when_over_limit(self):
        bridge = BudgetRBACBridge()
        result = bridge.check_budget("agent-1", token_used=150, token_limit=100)
    ...
    def test_returns_correct_fields_when_exceeded(self):
        bridge = BudgetRBACBridge()
        result = bridge.check_budget("agent-1", token_used=200, token_limit=100)
        assert result["agent_id"] == "agent-1"
    ...
    def test_one_over_limit(self):
        bridge = BudgetRBACBridge()
        result = bridge.check_budget("agent-1", token_used=101, token_limit=100)
        assert result["exceeded"] is True
        assert result["action"] == "REVOKE_WRITE"
...
class TestCheckBudgetBoundaryCases:
    def test_empty_agent_id(self):
        bridge = BudgetRBACBridge()
        result = bridge.check_budget("", token_used=50, token_limit=100)
        assert result["agent_id"] == ""
        assert result["action"] == "ALLOW"

    def test_zero_token_limit_with_zero_usage(self):
        bridge = BudgetRBACBridge()
        result = bridge.check_budget("agent-1", token_used=0, token_limit=0)
        assert result["exceeded"] is False
        assert result["action"] == "ALLOW"

    def test_zero_token_limit_with_nonzero_usage(self):
        bridge = BudgetRBACBridge()
        result = bridge.check_budget("agent-1", token_used=1, token_limit=0)
        assert result["exceeded"] is True
        assert result["action"] == "REVOKE_WRITE"

    def test_large_token_values(self):
        bridge = BudgetRBACBridge()
        result = bridge.check_budget("agent-1", token_used=999999999, token_limit=1000000000)
        assert result["exceeded"] is False
        assert result["action"] == "ALLOW"

    def test_negative_token_used(self):
        bridge = BudgetRBACBridge()
        result = bridge.check_budget("agent-1", token_used=-1, token_limit=100)
        assert result["exceeded"] is False
        assert result["action"] == "ALLOW"

    def test_negative_token_limit(self):
        bridge = BudgetRBACBridge()
        result = bridge.check_budget("agent-1", token_used=0, token_limit=-1)
        assert result["exceeded"] is True
        assert result["action"] == "REVOKE_WRITE"

    def test_none_agent_id_stored_as_none(self):
        bridge = BudgetRBACBridge()
        result = bridge.check_budget(None, token_used=50, token_limit=100)
    ...
    def test_none_token_used_raises(self):
        ...
        with pytest.raises(TypeError):
            bridge.check_budget("agent-1", token_used=None, token_limit=100)

    def test_none_token_limit_raises(self):
        bridge = BudgetRBACBridge()
        with pytest.raises(TypeError):
            bridge.check_budget("agent-1", token_used=50, token_limit=None)
...
class TestCheckBudgetReturnStructure:
    ...
    def test_agent_id_preserved(self):
        bridge = BudgetRBACBridge()
        result = bridge.check_budget("special-agent-42", token_used=10, token_limit=100)
