...
# [TTL] permanent
"""TransactionCostOptimizer 单元测试 (MOD-XS-019)。全成本计算+分解+优化建议。"""
...
class TestFeeSchedule:
    def test_defaults(self):
        f = FeeSchedule()
        assert f.commission_rate_bps == Decimal("0.854")
...
class TestCommission:
    def test_commission_applies_min(self):
        ...
        r = opt.calculate("O1", "000001.SZ", OrderSide.BUY, Decimal("100"), Decimal("10.00"), now=NOW)
    ...
    def test_commission_above_min(self):
        """大单佣金超过最低收费 → 按费率计算。"""
        opt = TransactionCostOptimizer()
        # 100000 股 × 10 元 = 1000000 notional, 佣金 = 1000000×0.854/10000 = 85.40
        r = opt.calculate("O1", "000001.SZ", OrderSide.BUY, Decimal("100000"), Decimal("10.00"), now=NOW)
        comm = r.breakdown_for(CostComponent.COMMISSION)
        assert comm.amount == Decimal("85.40")

    def test_commission_custom_min(self):
        opt = TransactionCostOptimizer(FeeSchedule(commission_min=Decimal("0")))
        r = opt.calculate("O1", "000001.SZ", OrderSide.BUY, Decimal("100"), Decimal("10.00"), now=NOW)
        comm = r.breakdown_for(CostComponent.COMMISSION)
        # 100×10×0.854/10000 = 0.0854 → round 0.09
        assert comm.amount == Decimal("0.09")

    def test_commission_both_sides(self):
        """佣金买卖双方都收。"""
        opt = TransactionCostOptimizer()
        rb = opt.calculate("O1", "X", OrderSide.BUY, Decimal("100000"), Decimal("10.00"), now=NOW)
        rs = opt.calculate("O2", "X", OrderSide.SELL, Decimal("100000"), Decimal("10.00"), now=NOW)
        assert rb.breakdown_for(CostComponent.COMMISSION).amount == rs.breakdown_for(CostComponent.COMMISSION).amount
...
class TestStampDuty:
    def test_buy_no_stamp_duty(self):
        """买方免征印花税。"""
        opt = TransactionCostOptimizer()
        r = opt.calculate("O1", "X", OrderSide.BUY, Decimal("100000"), Decimal("10.00"), now=NOW)
    ...
    def test_sell_stamp_duty_charged(self):
        ...
        r = opt.calculate("O1", "X", OrderSide.SELL, Decimal("100000"), Decimal("10.00"), now=NOW)
        sd = r.breakdown_for(CostComponent.STAMP_DUTY)
...
class TestExplicitCost:
    def test_buy_explicit(self):
        ...
        r = opt.calculate("O1", "X", OrderSide.BUY, Decimal("100000"), Decimal("10.00"), now=NOW)
        # 佣金 85.40 + 印花税 0 + 过户费 10 + 监管费 20 = 115.40
        assert r.explicit_cost == Decimal("115.40")

    def test_sell_explicit(self):
        """SELL 显性 = 佣金 + 印花税 + 过户费 + 监管费。"""
        opt = TransactionCostOptimizer()
        r = opt.calculate("O1", "X", OrderSide.SELL, Decimal("100000"), Decimal("10.00"), now=NOW)
        # 佣金 85.40 + 印花税 500 + 过户费 10 + 监管费 20 = 615.40
        assert r.explicit_cost == Decimal("615.40")

    def test_small_order_explicit(self):
        """小单佣金取最低值。"""
        opt = TransactionCostOptimizer()
        # 100 股 × 10 = 1000 notional
        r = opt.calculate("O1", "X", OrderSide.BUY, Decimal("100"), Decimal("10.00"), now=NOW)
...
class TestImpactCost:
    def test_buy_impact_with_decision_price(self):
        ...
        r = opt.calculate(
            "O1", "X", OrderSide.BUY, Decimal("1000"), Decimal("10.50"), decision_price=Decimal("10.40"), now=NOW
        )
    ...
    def test_buy_impact_zero_when_better_than_decision(self):
        ...
        opt = TransactionCostOptimizer()
        r = opt.calculate(
            "O1", "X", OrderSide.BUY, Decimal("1000"), Decimal("10.30"), decision_price=Decimal("10.40"), now=NOW
        )
        imp = r.breakdown_for(CostComponent.IMPACT)
    ...
    def test_sell_impact_with_decision_price(self):
        ...
        r = opt.calculate(
            "O1", "X", OrderSide.SELL, Decimal("1000"), Decimal("10.30"), decision_price=Decimal("10.40"), now=NOW
        )
        imp = r.breakdown_for(CostComponent.IMPACT)
        assert imp.amount == Decimal("100.00")
    ...
    def test_sell_impact_zero_when_better_than_decision(self):
        ...
        opt = TransactionCostOptimizer()
        r = opt.calculate(
            "O1", "X", OrderSide.SELL, Decimal("1000"), Decimal("10.50"), decision_price=Decimal("10.40"), now=NOW
        )
        imp = r.breakdown_for(CostComponent.IMPACT)
        assert imp.amount == Decimal("0.00")
    ...
    def test_impact_zero_without_any_input(self):
        """无决策价无 ADV → 冲击 = 0。"""
        opt = TransactionCostOptimizer()
        r = opt.calculate("O1", "X", OrderSide.BUY, Decimal("1000"), Decimal("10.00"), now=NOW)
        imp = r.breakdown_for(CostComponent.IMPACT)
        assert imp.amount == Decimal("0.00")
...
class TestOpportunityCost:
    def test_no_opportunity_when_fully_filled(self):
        ...
        opt = TransactionCostOptimizer()
        r = opt.calculate(
            "O1", "X", OrderSide.BUY, Decimal("1000"), Decimal("10.00"), decision_price=Decimal("10.00"), now=NOW
        )
    ...
    def test_opportunity_zero_without_decision_price(self):
        ...
        opt = TransactionCostOptimizer()
        r = opt.calculate(
            "O1", "X", OrderSide.BUY, Decimal("800"), Decimal("10.00"), unfilled_quantity=Decimal("200"), now=NOW
        )
        opp = r.breakdown_for(CostComponent.OPPORTUNITY)
        assert opp.amount == Decimal("0.00")
...
class TestTotalCost:
    ...
    def test_total_cost_bps(self):
        ...
        opt = TransactionCostOptimizer()
        r = opt.calculate("O1", "X", OrderSide.BUY, Decimal("100000"), Decimal("10.00"), now=NOW)
        # notional = 1000000, explicit = 115.40, implicit = 0
        # bps = 115.40 / 1000000 × 10000 = 1.154
        assert r.total_cost_bps == pytest.approx(Decimal("1.154"), abs=0.1)
    ...
    def test_explicit_cost_bps(self):
        opt = TransactionCostOptimizer()
        r = opt.calculate("O1", "X", OrderSide.BUY, Decimal("100000"), Decimal("10.00"), now=NOW)
        # explicit = 115.40, notional = 1000000 → 1.154 bps
        assert r.explicit_cost_bps == pytest.approx(Decimal("1.154"), abs=0.1)
...
class TestOptimizationAdvice:
    ...
    def test_advise_impact_driven(self):
        ...
        opt = TransactionCostOptimizer()
        r = opt.calculate(
            "O1", "X", OrderSide.BUY, Decimal("1000"), Decimal("11.00"), decision_price=Decimal("10.00"), now=NOW
        )
    ...
    def test_advise_zero_cost(self):
        ...
        opt = TransactionCostOptimizer(
            FeeSchedule(
                ...
                transfer_fee_rate_bps=Decimal("0"),
                regulatory_fee_rate_bps=Decimal("0"),
            )
        )
        r = opt.calculate("O1", "X", OrderSide.BUY, Decimal("1000"), Decimal("10.00"), now=NOW)
        advice = opt.advise(r)
...
class TestHistory:
    def test_history_accumulates(self):
        opt = TransactionCostOptimizer()
        for i in range(3):
            opt.calculate(f"O{i}", "X", OrderSide.BUY, Decimal("100"), Decimal("10.00"), now=NOW)
        assert len(opt.history) == 3

    def test_history_filtered_by_symbol(self):
        opt = TransactionCostOptimizer()
        opt.calculate("O1", "000001.SZ", OrderSide.BUY, Decimal("100"), Decimal("10.00"), now=NOW)
        opt.calculate("O2", "600519.SH", OrderSide.BUY, Decimal("100"), Decimal("10.00"), now=NOW)
    ...
    def test_clear_history(self):
        opt = TransactionCostOptimizer()
        opt.calculate("O1", "X", OrderSide.BUY, Decimal("100"), Decimal("10.00"), now=NOW)
        opt.clear_history()
...
class TestErrorsAndEdgeCases:
    def test_zero_quantity_raises(self):
        ...
        with pytest.raises(InvalidCostInputError, match="成交数量"):
            opt.calculate("O1", "X", OrderSide.BUY, Decimal("0"), Decimal("10.00"), now=NOW)

    def test_negative_quantity_raises(self):
        opt = TransactionCostOptimizer()
        with pytest.raises(InvalidCostInputError, match="成交数量"):
            opt.calculate("O1", "X", OrderSide.BUY, Decimal("-1"), Decimal("10.00"), now=NOW)

    def test_zero_price_raises(self):
        opt = TransactionCostOptimizer()
        with pytest.raises(InvalidCostInputError, match="成交价"):
            opt.calculate("O1", "X", OrderSide.BUY, Decimal("100"), Decimal("0"), now=NOW)

    def test_negative_unfilled_raises(self):
        opt = TransactionCostOptimizer()
        with pytest.raises(InvalidCostInputError, match="未成交"):
            opt.calculate(
                "O1", "X", OrderSide.BUY, Decimal("100"), Decimal("10.00"), unfilled_quantity=Decimal("-1"), now=NOW
            )

    def test_default_now(self):
        opt = TransactionCostOptimizer()
        r = opt.calculate("O1", "X", OrderSide.BUY, Decimal("100"), Decimal("10.00"))
    ...
    def test_fee_schedule_property(self):
        opt = TransactionCostOptimizer()
        assert opt.fee_schedule.commission_rate_bps == Decimal("0.854")
