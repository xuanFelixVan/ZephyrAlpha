...
def _rl(**kw) -> RiskLimits:
    base = dict(
        as_of_date=T0, idempotency_key="rl-1",
        max_single_position=0.99, max_gross_leverage=1.0,
    )
    base.update(kw)
...
def test_no_trigger_when_close():
    ...
    result = scheduler.evaluate(
        {"A": 0.50, "B": 0.50}, {"A": 0.51, "B": 0.49},
        market_state=3, now=T0,
    )
...
def test_drift_threshold_trigger_single_asset():
    ...
    # 用 3 标的让组合漂移小但单标的大
    result = scheduler.evaluate(
        {"A": 0.30, "B": 0.35, "C": 0.35}, {"A": 0.25, "B": 0.35, "C": 0.40},
        market_state=3, now=T0,
    )
...
def test_event_trigger():
    ...
    result = scheduler.evaluate(
        {"A": 0.50, "B": 0.50}, {"A": 0.50, "B": 0.50},
        market_state=3, now=T0, event_trigger=True,
    )
...
def test_risk_breach_highest_priority():
    ...
    result = scheduler.evaluate(
        {"A": 0.50, "B": 0.50}, {"A": 0.50, "B": 0.50},
        market_state=3, now=FRIDAY, risk_alert=True,
    )
...
def test_drift_overrides_calendar_and_event():
    scheduler = RebalanceScheduler()
    result = scheduler.evaluate(
        {"A": 0.30, "B": 0.70}, {"A": 0.50, "B": 0.50},
        market_state=3, now=FRIDAY, event_trigger=True,
    )
    assert result.trigger_source == RebalanceTriggerSource.DRIFT_THRESHOLD
...
def test_reoptimize_produces_new_target_portfolio():
    ...
    result = scheduler.evaluate(
        {"A": 0.20, "B": 0.80}, {"A": 0.50, "B": 0.50},
        market_state=3, now=T0,
        covariance=cov, risk_limits=_rl(),
...
def test_no_reoptimize_when_skipped():
    ...
    result = scheduler.evaluate(
        {"A": 0.50, "B": 0.50}, {"A": 0.50, "B": 0.50},
        market_state=3, now=T0,
        covariance=np.eye(2), risk_limits=_rl(),
    )
...
def test_reoptimize_reason_matches_trigger():
    ...
    result = scheduler.evaluate(
        {"A": 0.50, "B": 0.50}, {"A": 0.50, "B": 0.50},
        market_state=3, now=FRIDAY,  # 周五触发
        covariance=np.eye(2), risk_limits=_rl(),
    )
...
def test_should_rebalance_property():
    ...
    triggered = scheduler.evaluate(
        {"A": 0.2, "B": 0.8}, {"A": 0.5, "B": 0.5}, now=T0,
    )
    not_triggered = scheduler.evaluate(
        {"A": 0.5, "B": 0.5}, {"A": 0.5, "B": 0.5}, now=T0,
    )
