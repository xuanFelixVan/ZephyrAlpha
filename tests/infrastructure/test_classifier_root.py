...
class TestClassifierClassify:
    ...
    def test_classify_gate(self):
        entry = _make_entry(
            relative_path="src/zephyr/gov_enforcement/rule_enforcement/g1_ingest.yaml", extension=".yaml"
        )
        scan = _make_scan(entries=[entry])
        c = Classifier()
        result = c.classify(scan)
    ...
    def test_classify_registry(self):
        entry = _make_entry(
            relative_path="src/zephyr/gov_enforcement/rule_enforcement/_registry.yaml", extension=".yaml"
        )
        scan = _make_scan(entries=[entry])
        c = Classifier()
        result = c.classify(scan)
