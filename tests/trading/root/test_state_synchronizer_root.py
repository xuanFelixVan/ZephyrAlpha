# [A_test] module_id: MOD-GOV_state_synchronizer_root | layer=test | stability=volatile | safety=L | ai_autonomy=ai_modifiable
...
class TestSyncAll:
    def test_sync_all_no_tasks(self):
        ...
        with patch("zephyr.orchestrator.lifecycle.state_synchronizer.get_db_connection", return_value=mock_conn):
            sync = StateSynchronizer()
    ...
    def test_sync_all_with_consistent_task(self):
        ...
        with (
            patch("zephyr.orchestrator.lifecycle.state_synchronizer.get_db_connection", return_value=mock_conn),
            patch("zephyr.orchestrator.lifecycle.state_synchronizer.REPO_ROOT", Path("/nonexistent")),
        ):
            sync = StateSynchronizer()
