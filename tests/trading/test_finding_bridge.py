# [A_test] module_id: MOD-GOV_finding_bridge | layer=test | stability=volatile | safety=L | ai_autonomy=ai_modifiable
...
class TestReportFinding:
    def test_report_finding_calls_report_findings(self):
        ...
        with patch("zephyr.orchestrator.contracts.finding_bridge.report_findings", return_value=mock_result) as mock_rf:
            finding = SimpleNamespace(
                dimension="test",
                severity="LOW",
                description="test",
...
class TestReportFindings:
    def test_report_findings_with_audit_finding_objects(self):
        ...
        with (
            patch("zephyr.orchestrator.contracts.finding_bridge.FindingTaskBridge") as mock_bridge_cls,
            patch("zephyr.governance.persistence.task_repo.TaskRepository") as mock_repo_cls,
        ):
            mock_repo = MagicMock()
