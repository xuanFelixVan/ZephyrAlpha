...
class TestSnapshot:
    ...
    def test_normal_status_when_low_memory(self):
        with patch("zephyr.gov_drift.resource_guard.get_memory_usage_mb", return_value=10.0):
            snap = snapshot()
            assert snap.status == ResourceStatus.OK
            assert snap.degradation_level == DegradationLevel.NORMAL
    ...
    def test_warning_at_l1(self):
        ...
        with patch("zephyr.gov_drift.resource_guard.get_memory_usage_mb", return_value=mem):
            snap = snapshot()
