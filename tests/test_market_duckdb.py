...
def test_all():
    ...
    for i in range(100):
        ...
        ticks.append((f"600519.SH", ts, 1800.0 + i*0.1, 100+i*10, 180000.0+i*10000,
                       1799.5+i*0.1, 1800.5+i*0.1, 500+i*10, 600+i*10, 'test', 100))
    
    conn.executemany("""INSERT INTO tick_data 
    ...
    conn.execute("UPDATE orders SET status='FILLED', fill_price=1800.5, fill_qty=100.0, updated_at=? WHERE order_id='ORD-001'", (now,))
    conn.commit()
    ...
    conn.execute("""INSERT INTO backtest_results 
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        ('BT-001', 'STRAT-001', '2025-01-01', '2026-06-01', 1000000.0, 1250000.0,
         0.25, 1.5, 0.12, 0.55, 200, now))
    conn.commit()
    ...
    conn.execute("""INSERT INTO backtest_trades 
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (1, 'BT-001', '600519.SH', 'BUY', '2025-06-01T09:30:00', '2025-06-05T15:00:00',
         1800.0, 1900.0, 100.0, 10000.0, 50.0))
    conn.commit()
    ...
    # === 6. risk_snapshots ===
    print("\n=== 6. risk_snapshots ===")
    conn.execute("""INSERT INTO risk_snapshots 
    ...
    conn.executemany("""INSERT INTO factor_values (factor_id, symbol, timestamp, value, quality)
        VALUES (?, ?, ?, ?, ?)""", factors)
    ...
    for i in range(10000):
        ts = (base_time + timedelta(seconds=i*3)).isoformat()
        large_ticks.append(('600000.SH', ts, 10.0+i*0.01, 1000+i, 10000.0+i*100,
                            9.99+i*0.01, 10.01+i*0.01, 5000+i, 6000+i, 'perf_test', 100))
    conn.executemany("""INSERT INTO tick_data 
        (symbol, timestamp, price, volume, amount, bid1, ask1, bid_vol1, ask_vol1, data_source, quality_score)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", large_ticks)
    elapsed = time.time() - start
    check(f"10000 tick INSERT < 20s", elapsed < 20.0, f"took {elapsed:.2f}s")
    
    start = time.time()
    kline_count = conn.execute("SELECT COUNT(*) FROM kline_3s WHERE symbol='600519.SH'").fetchone()[0]
    elapsed = time.time() - start
    check(f"kline_3s aggregation < 2s", elapsed < 2.0, f"took {elapsed:.2f}s")
