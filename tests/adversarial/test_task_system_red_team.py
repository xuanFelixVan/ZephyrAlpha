...
def test_01_taskcard_creation():
    ...
    from zephyr.core.models import TaskCard, TaskStatus, TaskNamespace, GateLevel, TaskAuditFinding
    ...
    try:
        tc_full = TaskCard(
            ...
            downstream_outputs=[
                {"path": "D:/test/output.py", "description": "生成的代码文件"}
            ],
            allowed_touch=["D:/test/allowed/"],
            forbidden_touch=["D:/test/forbidden/"],
            applicable_rules=[
                {"module_id": "MOD-INF-006", "section": "§3", "reason": "任务模型规范"}
            ],
            context_assembly_manifest=[
                {"file_path": "D:/test/context.md", "reason": "上下文文件"}
            ],
...
def test_02_task_repo():
    ...
    from zephyr.db.task_repo import TaskRepository, TaskNotFoundError, InvalidTransitionError
    from zephyr.shared.schemas import Task, TaskStatus, TaskNamespace, Priority, SafetyLevel
    ...
    try:
        ...
        try:
            ...
            results.append(("WARN", "2f: TaskCard→Repo扩展字段丢失",
                f"source_blueprint保存为: {has_source} (预期: MOD-TEST-001)"))
        except Exception as e:
        ...
        results.append(("PASS", "2h: list_by_status PENDING", f"count={len(pending)}, ids={[t.task_id for t in pending]}"))
...
def test_03_pipeline_orchestrator():
    """测试管线编排器"""
    from zephyr.core.models import TaskCard, TaskStatus, TaskNamespace, GateLevel
    from zephyr.pipeline.pipeline_orchestrator import PipelineOrchestrator
    from zephyr.pipeline.models import PipelineOrchestratorConfig, PipelineStatus
    from zephyr.shared.schemas import Priority, SafetyLevel
    ...
    try:
        ...
        results.append(("PASS", "3a: A区管线dispatch", 
            f"status={result.overall_status}, modules={len(result.modules_executed)}"))
    except Exception as e:
    ...
    try:
        ...
        results.append(("PASS", "3b: B区管线dispatch",
            f"status={result.overall_status}, modules={len(result.modules_executed)}"))
    except Exception as e:
    ...
    try:
        ...
        results.append(("PASS", "3c: security标签→Claude救援",
    ...
    try:
        ...
        results.append(("PASS", "3d: experimental标签→Claude救援",
            f"rescue={result.needs_claude_rescue}"))
    except Exception as e:
    ...
    try:
        ...
        results.append(("PASS", "3e: 无效管线被拒绝",
...
def test_04_context_assembler():
    ...
    try:
        ...
        results.append(("PASS", "4a: 基本上下文装配",
    ...
    try:
        ...
        results.append(("PASS", "4b: 空manifest",
            f"files={ctx.file_count}, errors_count={len(ctx.errors)}"))
    except Exception as e:
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            results.append(("PASS", "4d: shadow生成",
                f"path={shadow_path}, exists={shadow_path.exists()}"))
    except Exception as e:
...
def test_05_blueprint_decomposer():
    ...
    from zephyr.core.models import TaskStatus, TaskNamespace
    ...
    # 5d: check_gate G0 / G7
    try:
        from zephyr.core.models import TaskCard, TaskNamespace, GateLevel
        from zephyr.shared.schemas import Priority, SafetyLevel
        ...
        results.append(("PASS", "5d: check_gate",
...
def test_06_task_manager_mcp():
    ...
    from zephyr.core.models import TaskCard, TaskStatus, TaskNamespace
    from zephyr.shared.schemas import Priority, SafetyLevel
    ...
    try:
        ...
        if recovered is None:
            results.append(("WARN", "6b: md→TaskCard回读失败",
                "_parse_md_to_taskcard返回None——JSON解析.mdfile不适用——这是已知Bug！"))
        else:
            results.append(("PASS", "6b: md→TaskCard回读", f"recovered_task_id={recovered.task_id if hasattr(recovered, 'task_id') else '?'}"))
    except Exception as e:
    ...
    try:
        ...
        results.append(("PASS", "6c: _extract_triage_profile",
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if md_path.exists():
                results.append(("WARN", "6d: .md文件残留",
                    "v2已移除.md双轨——此文件可能是旧版本遗留"))
            else:
                results.append(("PASS", "6d: .md未被生成",
                    "v2已确认：_persist不再写.md文件"))
            
            loaded = mcp._load("CP-500")
            if loaded is None:
                results.append(("PASS", "6d: _load返回None(无repo,无SQLite)",
                    "v2已确认：_load不再回退读.md"))
            else:
...
def test_07_integration_scan():
    ...
    integration_points = [
        ...
        ("TaskRepository", "create(task: Task) / transition(task_id, status)", "只消费Task基类——扩展字段在SQLite中丢失"),
...
def test_08_triage_integration():
    ...
    try:
        ...
        results.append(("PASS", "8b: Task(name=...)正确被拒", 
            f"extra=forbid生效——name字段不被接受"))
