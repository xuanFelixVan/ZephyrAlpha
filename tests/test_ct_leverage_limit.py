# [A_test] module_id: SRC-TST-0673 | layer=test | stability=volatile | safety=L | ai_autonomy=ai_modifiable
# [BLUEPRINT] MOD-INF-007 | docs/03_modules/_cross_layer/gate-engine/blueprint.md | §3-§7
# [MODULE] tests.test_ct_leverage_limit
...
# [TESTS] pytest tests/test_ct_leverage_limit.py
...
class TestLeverageLimitHandlerImport:
    def test_import_module(self):
        import importlib

        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_leverage_limit")
        assert hasattr(mod, "LeverageLimitHandler")

    def test_class_is_check_type_handler(self):
        import importlib

        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_leverage_limit")
        assert issubclass(mod.LeverageLimitHandler, CheckTypeHandler)


class TestLeverageLimitHandlerInstantiation:
    def test_instantiate(self):
        import importlib

        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_leverage_limit")
        handler = mod.LeverageLimitHandler()
    ...
    def test_name_attribute(self):
        ...
        mod = importlib.import_module("zephyr.governance.rule_enforcement.check_types.ct_leverage_limit")
        handler = mod.LeverageLimitHandler()
        assert handler.name == "leverage_limit"

    def test_registered_in_registry(self):
        cls = get_check_type("leverage_limit")
        assert cls is not None


class TestLeverageLimitHandlerRun:
...
class TestLeverageLimitHandlerExternalDep:
