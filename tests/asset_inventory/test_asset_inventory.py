# [A_test] module_id: MOD-GOV_asset_inventory | layer=test | stability=volatile | safety=L | ai_autonomy=ai_modifiable
...
class TestDependencyExtractor:
    ...
    def test_classify_stdlib(self):
        ext = dependency.DependencyExtractor()
        result = ext.classify_import("os")
        assert result == "stdlib"

    def test_classify_project_import(self):
        ext = dependency.DependencyExtractor()
        result = ext.classify_import("zephyr.orchestrator.core")
        assert result == "absolute"

    def test_classify_third_party(self):
        ext = dependency.DependencyExtractor()
        result = ext.classify_import("numpy")
...
class TestGitMetadataExtractor:
    def test_is_ai_commit(self):
        assert metadata_mod.GitMetadataExtractor.is_ai_commit("[AI] auto fix")
        assert metadata_mod.GitMetadataExtractor.is_ai_commit("generated by copilot")
        assert not metadata_mod.GitMetadataExtractor.is_ai_commit("manual fix")

    def test_parse_date_valid(self):
        result = metadata_mod.GitMetadataExtractor.parse_date("2026-01-15 10:30:00")
        assert result.year == 2026
        assert result.month == 1
        assert result.day == 15

    def test_parse_date_invalid(self):
        result = metadata_mod.GitMetadataExtractor.parse_date("not-a-date")
...
class TestTripleTrustAnchorGate:
    def test_calculate_trust_full(self):
        assert trust_anchor.TripleTrustAnchorGate.calculate_trust({"git_ok": True, "test_ok": True, "audit_ok": True}) is trust_anchor.TrustLevel.FULL

    def test_calculate_trust_partial(self):
        assert trust_anchor.TripleTrustAnchorGate.calculate_trust({"git_ok": True, "test_ok": True, "audit_ok": False}) is trust_anchor.TrustLevel.PARTIAL
        assert trust_anchor.TripleTrustAnchorGate.calculate_trust({"git_ok": True, "test_ok": False, "audit_ok": True}) is trust_anchor.TrustLevel.PARTIAL

    def test_calculate_trust_broken(self):
        assert (
            trust_anchor.TripleTrustAnchorGate.calculate_trust({"git_ok": False, "test_ok": False, "audit_ok": False}) is trust_anchor.TrustLevel.BROKEN
        )

    def test_recommend_full(self):
        rec = trust_anchor.TripleTrustAnchorGate.recommend(trust_anchor.TrustLevel.FULL)
        assert "完全可信" in rec

    def test_recommend_broken(self):
        rec = trust_anchor.TripleTrustAnchorGate.recommend(trust_anchor.TrustLevel.BROKEN)
        assert "不可信" in rec
