"""Test gate g_trae_021 for rule TRAE-021 — calls gate_engine.evaluate().
...
GATE_YAML = PROJECT_ROOT / "src" / "zephyr" / "gov_enforcement" / "rule_enforcement" / "g_trae_021.yaml"
RULE_YAML = PROJECT_ROOT / "docs" / "01_policies_and_standards" / "rules" / "trae_021_behavior_other_prohibition.yaml"
MOCK_YAML = PROJECT_ROOT / "tests" / "fixtures" / "g_trae_021_mock.yaml"


class TestGTrae021:
    """Red-Blue extreme tests for g_trae_021 gate."""
    ...
    def test_gate_registered_in_gate_files(self):
        ...
        assert "G_TRAE_021" in GateEngine._GATE_FILES, "Gate G_TRAE_021 not in _GATE_FILES"
    ...
    def test_gate_loadable_by_engine(self):
        ...
        try:
            if "G_TRAE_021" not in GateEngine._GATE_FILES:
                GateEngine._GATE_FILES["G_TRAE_021"] = "g_trae_021.yaml"
                engine._gate_cache = None  # Clear cache
            gates = engine.load_gates()
            assert "G_TRAE_021" in gates, "Gate G_TRAE_021 not loaded"
        finally:
    ...
    def test_gate_evaluates_pass(self):
        ...
        try:
            ...
            valid_task = Task(
                ...
                acceptance=["pytest"],
                applicable_rules=[{"module_id": "TRAE-021", "section": "all", "reason": "gate test"}],
            ...
            result = engine.evaluate(valid_task, "G_TRAE_021")
    ...
    def test_gate_evaluates_fail(self):
        ...
        try:
            ...
            result = engine.evaluate(sparse_task, "G_TRAE_021")
    ...
    def test_gate_bypass_detection(self):
        ...
        try:
            ...
            gates = engine.load_gates()
            assert "G_TRAE_021" in gates, "Gate should be loadable when YAML exists"
        finally:
            GateEngine._GATE_FILES = original
            engine._gate_cache = None
