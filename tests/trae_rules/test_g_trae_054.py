"""Test gate g_trae_054 for rule TRAE-054 — calls gate_engine.evaluate().
...
GATE_YAML = PROJECT_ROOT / "src" / "zephyr" / "gov_enforcement" / "rule_enforcement" / "g_trae_054.yaml"
RULE_YAML = PROJECT_ROOT / "docs" / "01_policies_and_standards" / "rules" / "trae_054_depgraph_access_protocol.yaml"
MOCK_YAML = PROJECT_ROOT / "tests" / "fixtures" / "g_trae_054_mock.yaml"


class TestGTrae054:
    """Red-Blue extreme tests for g_trae_054 gate."""
    ...
    def test_gate_registered_in_gate_files(self):
        ...
        assert "G_TRAE_054" in GateEngine._GATE_FILES, "Gate G_TRAE_054 not in _GATE_FILES"
    ...
    def test_gate_loadable_by_engine(self):
        ...
        try:
            if "G_TRAE_054" not in GateEngine._GATE_FILES:
                GateEngine._GATE_FILES["G_TRAE_054"] = "g_trae_054.yaml"
                engine._gate_cache = None  # Clear cache
            gates = engine.load_gates()
            assert "G_TRAE_054" in gates, "Gate G_TRAE_054 not loaded"
        finally:
    ...
    def test_gate_evaluates_pass(self):
        ...
        try:
            ...
            valid_task = Task(
                ...
                acceptance=["pytest"],
                applicable_rules=[{"module_id": "TRAE-054", "section": "all", "reason": "gate test"}],
                rollback_instructions="git checkout",
                blocked_by=["DM-100000"],
                post_sync_standard=["python scripts/governance/d11_compliance/audit_registration.py"],
                created_at=datetime.now(),
                updated_at=datetime.now(),
            )
            result = engine.evaluate(valid_task, "G_TRAE_054")
    ...
    def test_gate_evaluates_fail(self):
        ...
        try:
            ...
            result = engine.evaluate(sparse_task, "G_TRAE_054")
    ...
    def test_gate_bypass_detection(self):
        ...
        try:
            ...
            gates = engine.load_gates()
            assert "G_TRAE_054" in gates, "Gate should be loadable when YAML exists"
        finally:
            GateEngine._GATE_FILES = original
            engine._gate_cache = None
