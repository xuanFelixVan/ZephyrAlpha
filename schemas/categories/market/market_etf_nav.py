...
# [MODULE] schemas.categories.market.market_etf_nav
# [DOMAIN] D_DATA
# [DEPENDENCIES] none
# [CONSUMERS] apply_market_tables_ddl; zephyr.data.c1_market_writer
# [STARTUP] imported
# [MATURITY] production
# [INVARIANTS] etf_nav 表 DDL 唯一真源；本文件 DDL 必须与 ClickHouse 实际表结构一致；变更需经 apply_schema.py 执行
...
# [TTL] permanent
"""etf_nav 表 DDL-as-Code（category_id: market_etf_nav, calc_mode: lazy）。

本文件是 c1_market.etf_nav 表结构的唯一真源（DDL-as-Code 模式）。
...
# category_id: market_etf_nav
# calc_mode: lazy

MARKET_ETF_NAV_DDL = """
CREATE TABLE IF NOT EXISTS c1_market.etf_nav
(
    trade_date               Date,
    symbol                   String,
    nav                      Decimal(18, 4),
...
INSERT_COLUMNS = "(trade_date, symbol, nav, total_assets, data_source)"
