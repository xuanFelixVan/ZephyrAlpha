...
# [MODULE] schemas.categories.market.market_futures_kline_qmt
# [DOMAIN] D_DATA
# [DEPENDENCIES] none
# [CONSUMERS] apply_market_tables_ddl; zephyr.data.c1_market_writer
# [STARTUP] imported
# [MATURITY] production
# [INVARIANTS] futures_kline_qmt 表 DDL 唯一真源；本文件 DDL 必须与 ClickHouse 实际表结构一致；变更需经 apply_schema.py 执行
...
# [TTL] permanent
"""futures_kline_qmt 表 DDL-as-Code（category_id: market_futures_kline_qmt, calc_mode: lazy）。

本文件是 c1_market.futures_kline_qmt 表结构的唯一真源（DDL-as-Code 模式）。
...
# category_id: market_futures_kline_qmt
# calc_mode: lazy

MARKET_FUTURES_KLINE_QMT_DDL = """
CREATE TABLE IF NOT EXISTS c1_market.futures_kline_qmt
(
    trade_date               Date,
    symbol                   String,
    open                     Decimal(18, 4),
