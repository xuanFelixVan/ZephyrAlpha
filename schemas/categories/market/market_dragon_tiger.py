...
# [MODULE] schemas.categories.market.market_dragon_tiger
# [DOMAIN] D_DATA
# [DEPENDENCIES] none
# [CONSUMERS] apply_market_tables_ddl; zephyr.data.c1_market_writer
# [STARTUP] imported
# [MATURITY] production
# [INVARIANTS] dragon_tiger 表 DDL 唯一真源；本文件 DDL 必须与 ClickHouse 实际表结构一致；变更需经 apply_schema.py 执行
...
# [TTL] permanent
"""dragon_tiger 表 DDL-as-Code（category_id: market_dragon_tiger, calc_mode: lazy）。

本文件是 c1_market.dragon_tiger 表结构的唯一真源（DDL-as-Code 模式）。
...
# category_id: market_dragon_tiger
# calc_mode: lazy

MARKET_DRAGON_TIGER_DDL = """
...
INSERT_COLUMNS = "(trade_date, symbol, name, reason, net_buy, buy_amount, sell_amount)"
