...
# [MODULE] schemas.categories.market.market_hk_stock_list
# [DOMAIN] D_DATA
# [DEPENDENCIES] none
# [CONSUMERS] apply_market_tables_ddl
# [STARTUP] imported
# [MATURITY] production
# [INVARIANTS] hk_stock_list 表 DDL 唯一真源；本文件 DDL 必须与 ClickHouse 实际表结构一致；变更需经 apply_schema.py 执行
...
# [TTL] permanent
"""hk_stock_list 表 DDL-as-Code（category_id: market_hk_stock_list, calc_mode: preload）.

P0-6 SCD-2 真源回写（#ARCH-CH-021/#ARCH-CH-025, 2026-07-25）：
    本文件从 ClickHouse 实际表结构转录为 DDL-as-Code 真源，消除"DB 有表无真源"漂移债务。
    hk_stock_list 采用 SCD-2 时点版本化（valid_from/valid_to），消除幸存者偏差。
...
# category_id: market_hk_stock_list
# calc_mode: preload（SCD 维度表，回测时预加载到内存）

HK_STOCK_LIST_DDL = """
CREATE TABLE IF NOT EXISTS c1_market.hk_stock_list
(
...
INSERT_COLUMNS = "(code, name, valid_to)"
