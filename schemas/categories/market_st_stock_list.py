...
# [MODULE] schemas.categories.market_st_stock_list
# [DOMAIN] D_DATA
# [DEPENDENCIES] none
# [CONSUMERS] apply_market_tables_ddl
# [STARTUP] imported
# [MATURITY] production
# [INVARIANTS] st_stock_list 表 DDL 唯一真源；本文件 DDL 必须与 ClickHouse 实际表结构一致；变更需经 apply_schema.py 执行
...
P0-6 SCD-2 真源回写（#ARCH-CH-021/#ARCH-CH-025, 2026-07-25）：
    ...
    st_stock_list 采用 SCD-2 时点版本化（valid_from/valid_to），消除幸存者偏差。
...
# category_id: market_st_stock_list
# calc_mode: preload（SCD 维度表，回测时预加载到内存）

ST_STOCK_LIST_DDL = """
CREATE TABLE IF NOT EXISTS c1_market.st_stock_list
(
    trade_date   Date                   COMMENT 'trade_date',
    symbol       String                 COMMENT 'symbol',
    name         String                 COMMENT 'name',
    st_type      LowCardinality(String) COMMENT 'st_type',
...
INSERT_COLUMNS = "(trade_date, symbol, name, st_type, valid_to)"
