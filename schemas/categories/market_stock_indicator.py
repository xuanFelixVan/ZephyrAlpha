...
# [MODULE] schemas.categories.market_stock_indicator
# [DOMAIN] D_DATA
# [DEPENDENCIES] none
# [CONSUMERS] apply_market_tables_ddl; zephyr.data.c1_market_writer
# [STARTUP] imported
# [MATURITY] production
# [INVARIANTS] stock_indicator 表 DDL 唯一真源；本文件 DDL 必须与 ClickHouse 实际表结构一致；变更需经 apply_schema.py 执行
...
# [TTL] permanent
"""stock_indicator 表 DDL-as-Code（category_id: market_stock_indicator, calc_mode: lazy）。

本文件是 c1_market.stock_indicator 表结构的唯一真源（DDL-as-Code 模式）。
...
# category_id: market_stock_indicator
# calc_mode: lazy

MARKET_STOCK_INDICATOR_DDL = """
CREATE TABLE IF NOT EXISTS c1_market.stock_indicator
(
    trade_date               Date,
    symbol                   String,
    pe                       Nullable(Float64),
...
INSERT_COLUMNS = "(trade_date, symbol, pe, pb, ps, pcf, dividend_yield)"
