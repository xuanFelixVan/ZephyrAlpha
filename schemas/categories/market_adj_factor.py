...
# [MODULE] schemas.categories.market_adj_factor
# [DOMAIN] D_DATA
# [DEPENDENCIES] none
# [CONSUMERS] apply_market_tables_ddl; zephyr.data.c1_market_writer
# [STARTUP] imported
# [MATURITY] production
# [INVARIANTS] adj_factor 表 DDL 唯一真源；本文件 DDL 必须与 ClickHouse 实际表结构一致；变更需经 apply_schema.py 执行
...
# [TTL] permanent
"""adj_factor 表 DDL-as-Code（category_id: market_adj_factor, calc_mode: lazy）。

本文件是 c1_market.adj_factor 表结构的唯一真源（DDL-as-Code 模式）。
...
MARKET_ADJ_FACTOR_DDL = """
CREATE TABLE IF NOT EXISTS c1_market.adj_factor
(
    trade_date               Date,
    symbol                   String  COMMENT '6位数字证券代码',
    adj_factor               Decimal(18, 8)  COMMENT '复权因子',
    data_source              String,
...
INSERT_COLUMNS = "(trade_date, symbol, adj_factor, data_source)"
