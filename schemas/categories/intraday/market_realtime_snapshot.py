...
# [MODULE] schemas.categories.intraday.market_realtime_snapshot
# [DOMAIN] D_DATA
# [DEPENDENCIES] none
# [CONSUMERS] apply_market_tables_ddl; zephyr.data.c1_market_writer
# [STARTUP] imported
# [MATURITY] production
# [INVARIANTS] realtime_snapshot 表 DDL 唯一真源；本文件 DDL 必须与 ClickHouse 实际表结构一致；变更需经 apply_schema.py 执行
...
# [TTL] permanent
"""realtime_snapshot 表 DDL-as-Code（category_id: market_realtime_snapshot, calc_mode: lazy）。

本文件是 c1_market.realtime_snapshot 表结构的唯一真源（DDL-as-Code 模式）。
...
MARKET_REALTIME_SNAPSHOT_DDL = """
CREATE TABLE IF NOT EXISTS c1_market.realtime_snapshot
(
    snapshot_time            DateTime64(3, 'Asia/Shanghai'),
    symbol                   String,
    open                     Decimal(18, 4),
