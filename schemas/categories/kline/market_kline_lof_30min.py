...
# [MODULE] schemas.categories.kline.market_kline_lof_30min
# [DOMAIN] D_DATA
# [DEPENDENCIES] none
# [CONSUMERS] apply_market_tables_ddl; zephyr.data.c1_market_writer
# [STARTUP] imported
# [MATURITY] production
# [INVARIANTS] kline_lof_30min 表 DDL 唯一真源；本文件 DDL 必须与 ClickHouse 实际表结构一致；变更需经 apply_schema.py 执行
...
本文件是 c1_market.kline_lof_30min 表结构的唯一真源（DDL-as-Code 模式）。
...
# category_id: market_kline_lof_30min
# calc_mode: lazy

MARKET_KLINE_LOF_30MIN_DDL = """
CREATE TABLE IF NOT EXISTS c1_market.kline_lof_30min
(
    trade_date               Date,
    trade_time               DateTime64(3, 'Asia/Shanghai'),
    symbol                   String,
