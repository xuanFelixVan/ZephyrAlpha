...
- 全库 API 与事件约定真源：`API_Contract.md`。CDC 事件（insert/update/delete）、变更流元数据（source、lsn/offset、timestamp、schema_version）与下游订阅方式的契约口径以该真源为准。
...
- 全库 API 与事件约定真源：`API_Contract.md`。CDC 事件载荷、游标语义、幂等键、重放机制与告警事件等对外约定需以该真源或其子契约为准。
