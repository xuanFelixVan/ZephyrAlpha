...
module_id: QUARTERLY_REBALANCE_001_7455
...
- 输入目标权重/当前权重/阈值与成本模型后，可生成再平衡建议（交易列表或权重调整）并可复核（满足权重归一、约束检查）。

- 再平衡触发条件可验证：偏离度≥阈值时触发，否则不触发（提供示例或测试用例）。

- 交易成本估计与换手控制口径明确，可在报告/日志中检查关键指标（如 turnover、预估成本）。
...
- 再平衡效果依赖交易成本模型与流动性假设；实施阶段需用历史数据校准，并在契约真源中固化字段/单位与默认参数。
