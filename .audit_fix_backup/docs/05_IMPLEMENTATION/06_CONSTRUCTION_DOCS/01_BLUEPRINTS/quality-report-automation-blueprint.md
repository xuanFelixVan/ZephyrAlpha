...
质量报告自动化模块，自动生成各类质量报告，包括数据质量报告、模型质量报告、系统质量报告等，支持报告模板和自动化调度。
...
- 至少生成 1 类报告（如 daily/weekly）从数据汇总到产出（HTML/PDF/Excel 任一）可自动化执行并可追溯（包含时间范围与输入数据来源）。

- 报告模板渲染与数据校验失败时能输出明确错误并阻断发布（日志/告警/返回码其一可检查）。

- 报告归档与检索可验证：按日期/类型能检索到历史报告或元数据记录。
...
- 报告指标口径与字段字典依赖上游数据治理成熟度；实施阶段需固化指标定义、脱敏规则与留存策略，并回填契约真源。
