...
- 全库 API 与事件约定真源：`API_Contract.md`。测试任务提交、结果上报、报告生成与门禁放行若通过接口/事件实现，须在该真源或本文后续接口说明中闭合。
...
- 能在本文中明确测试覆盖维度与最小放行条件（例如：策略级回测、回放、风控回归、合规模拟），并能在 `API_Contract.md` 中定位到测试结果/报告的契约入口或写明补全计划。
...
- 不同回测/执行引擎的适配策略需在施工文档阶段落定；以本节门禁为准。
