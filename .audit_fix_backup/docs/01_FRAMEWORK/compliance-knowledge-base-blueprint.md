...
- 全库 API 与事件约定真源：`API_Contract.md`。法规文档入库、检索、解读、问答与版本变更若通过接口/事件实现，须在该真源或本文后续接口说明中闭合。
...
- Owner 能从本文中抽取“法规入库 → 检索 → 解读 → 引用留痕”的最小闭环，并能在 `API_Contract.md` 中定位到相应的检索/审计契约入口（或在本文写明豁免与补全计划）。
