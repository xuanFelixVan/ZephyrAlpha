...
- 全库 API 与事件约定真源：`API_Contract.md`。清洗、对齐、特征工程与质量上报若通过接口/事件向上游/下游暴露，须在该真源或本文后续接口说明中闭合。
...
- 能从本文中抽取“输入 → 清洗/对齐 → 输出字段口径 → 质量指标”的最小闭环，并能在 `API_Contract.md` 中定位到相应的数据契约入口（或在本文写明豁免与补全计划）。
...
- 特征命名空间与口径统一需与因子/模型侧协同；以本节门禁为准，统一工作在施工文档阶段落定。
...
| DATA_SOURCE_LAYER_BLUEPRINT.md | 数据源层蓝图 |
