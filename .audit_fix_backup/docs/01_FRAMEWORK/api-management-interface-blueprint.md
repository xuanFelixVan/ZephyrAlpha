...
- 全库 API 与事件约定真源：`API_Contract.md`。API 文档、测试、密钥与调用日志若通过接口/事件暴露，须在该真源或本文后续接口说明中闭合。
...
- Owner 能从本文中明确“API 列表/文档/测试/密钥/日志”的最小闭环，并能在 `API_Contract.md` 中定位到对应的接口契约入口（或在本文写明豁免与补全计划）。
...
- 文中 UI 形态与实现框架在施工文档阶段再落定；以本节门禁为准。
