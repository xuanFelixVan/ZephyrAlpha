---
module_id: 06_ARCHIVE_BLUEPRINTS_P0_01_DATABASE_DESIGN_DOCUMENT_V1_BACKUP
layer: layer_06
version: 1.0.0
status: Active
responsibility:
  - P0 01 Database Design Document V1 Backup相关业务
...
| total_assets | DECIMAL(18,2) | NOT NULL | 0.00 | 总资?|

| total_market_value | DECIMAL(18,2) | NOT NULL | 0.00 | 总市?|

| daily_pnl | DECIMAL(18,2) | NOT NULL | 0.00 | 当日盈亏 |

| total_pnl | DECIMAL(18,2) | NOT NULL | 0.00 | 累计盈亏 |

| max_drawdown | DECIMAL(10,4) | NOT NULL | 0.0000 | 最大回?|
...
| total_assets | DECIMAL(18,2) | NOT NULL | - | 总资?|

| available_cash | DECIMAL(18,2) | NOT NULL | - | 可用现金 |

| total_market_value | DECIMAL(18,2) | NOT NULL | - | 总市?|
...
| max_drawdown | DECIMAL(10,4) | NOT NULL | - | 最大回?|
...
| quantity_before | INTEGER | NOT NULL | - | 变更前数?|

| quantity_after | INTEGER | NOT NULL | - | 变更后数?|
...
| commission | DECIMAL(10,2) | NOT NULL | 0.00 | 手续?|

| stamp_tax | DECIMAL(10,2) | NOT NULL | 0.00 | 印花?|

| transfer_fee | DECIMAL(10,2) | NOT NULL | 0.00 | 过户?|

| total_cost | DECIMAL(18,2) | NOT NULL | 0.00 | 总成?|
...
| total_cost | DECIMAL(18,2) | NOT NULL | - | 总成?|
...
| strength | DECIMAL(10,4) | NOT NULL | - | 信号强度?.0-1.0?|
...
| confidence | DECIMAL(10,4) | - | NULL | 置信?|

| is_executed | BOOLEAN | NOT NULL | FALSE | 是否已执?|
...
| priority | INTEGER | NOT NULL | 0 | 优先?|
...
| status | VARCHAR(20) | NOT NULL | - | 当前状?|
...
| total_trades_today | INTEGER | NOT NULL | 0 | 今日交易?|

| total_volume_today | DECIMAL(18,2) | NOT NULL | 0.00 | 今日交易?|
...
#### 2.5.3 Saga事务?(saga_transactions)



**表说?*: 存储Saga分布式事务的状态和执行记录
...
| participants | JSONB | NOT NULL | '[]' | 参与方列?|

| executed_steps | JSONB | NOT NULL | '[]' | 已执行步?|
...
| triggered_rules | JSONB | NOT NULL | '[]' | 触发的规?|
...
| is_resolved | BOOLEAN | NOT NULL | FALSE | 是否已解?|
...
| metric_value | DECIMAL(18,4) | NOT NULL | - | 指标?|
...
| is_acknowledged | BOOLEAN | NOT NULL | FALSE | 是否已确?|
...
| 表名 | 分区类型 | 分区?| 分区粒度 | 保留策略 |

|------|----------|--------|----------|----------|

| **account_snapshots** | RANGE | snapshot_date | 按季?| 保留3?|

| **position_history** | RANGE | created_at | 按季?| 保留3?|

| **trades** | RANGE | traded_at | 按季?| 保留5?|

| **system_metrics** | RANGE | recorded_at | 按季?| 保留1?|
...
| accounts | positions | CASCADE | 删除账户时删除所有持?|

| accounts | orders | CASCADE | 删除账户时删除所有订?|

| orders | trades | CASCADE | 删除订单时删除所有交易记?|

| accounts | signals | SET NULL | 删除账户时信号保留，account_id置空 |

| engines | engine_states | CASCADE | 删除引擎时删除所有状态记?|
...
| **_id** | 主键或外?| account_id, order_id |

| **_code** | 业务编码 | account_code, order_code |

| **_at** | 时间?| created_at, updated_at |

| **_date** | 日期 | snapshot_date, trade_date |

| **_pct** | 百分?| daily_pnl_pct, position_pct |
...
| **_value** | 价?| market_value, total_assets |
...
#### Saga事务状?(saga_transaction.status)
...
1. **表结构确?*: 请确认所有表结构设计是否符合业务需?

2. **字段类型确认**: 请确认字段类型和长度是否合?

3. **索引策略确认**: 请确认索引设计是否满足查询需?
