...
|------|----------|----------|----------|------|
| 01 | 探索性数据分析模板 | [01_EDA_TEMPLATE.py](./01_EDA_TEMPLATE.py) | 数据质量检查、分布分析、相关性研究 | ✅ 可用 |
| 02 | 因子开发模板 | [02_FACTOR_TEMPLATE.py](./02_FACTOR_TEMPLATE.py) | 新因子计算、IC分析、回测验证 | ✅ 可用 |
| 03 | 策略原型模板 | (预留) | 策略逻辑实现、参数优化、可视化 | 📅 计划中 |
| 04 | 报告生成模板 | (预留) | 自动化报告、图表生成、文档输出 | 📅 计划中 |
