    ...
    def __post_init__(self) -> None:
        ...
        if not (0.0 < self.elevated_percentile < self.overheat_percentile <= 1.0):
            raise SectorCrowdingError(
                f"分位阈值须 0<elevated<overheat≤1: "
                f"{self.elevated_percentile}/{self.overheat_percentile}"
            )
        if not (0.0 < self.momentum_decay_threshold < 1.0):
            raise SectorCrowdingError(
                f"momentum_decay_threshold 须在 (0,1): {self.momentum_decay_threshold}"
            )
...
class SectorCrowdingLauncher:
    ...
    def assess_crowding(
        ...
        *,
        ...
        wsum = w[0] + w[1] + w[2]
        composite = (
            w[0] * components.turnover_pct
            + w[1] * components.margin_pct
            + w[2] * components.correlation_pct
        ) / wsum
        ...
        if overheated and momentum_decay > self._config.momentum_decay_threshold:
            warning = OverheatWarning(
                ...
                reason=(
                    f"拥挤度{composite:.3f}>90%分位且动量衰减"
                    f"{momentum_decay:.3f}>30%，回撤概率高"
                ),
