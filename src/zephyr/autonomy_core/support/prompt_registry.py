...
# [MODULE] zephyr.autonomy_core.support.prompt_registry
...
class PromptTemplate(BaseModel):
    ...
    def render(self, variables: dict[str, str]) -> Self:
        """用给定变量渲染模板。
...
class PromptRegistry:
    ...
    def load_yaml(self, path: Path | str) -> int:
        ...
        _log.info("yaml_loaded: path=%s count=%d", str(resolved), count)
        return count
    ...
    def register(self, template: PromptTemplate, *, allow_overwrite: bool = False) -> None:
        ...
        _log.debug(
            "template_registered: template_id=%s version=%s stability=%s",
            template.template_id,
            template.version,
            template.stability,
        )

    def get(self, template_id: str, version: str | None = None) -> Self:
    ...
    def render(
        ...
        _log.info(
            "template_rendered: template_id=%s version=%s token_count=%d",
            template_id,
            template.version,
            result.token_count,
        )
    ...
    def render_with_context(
        ...
        _log.info(
            "template_rendered_with_context: template_id=%s version=%s token_count=%d context_sources=%s",
            template_id,
            template.version,
            result.token_count,
            injected_ctx.sources,
        )
