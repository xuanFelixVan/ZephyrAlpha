...
# [MODULE] zephyr.autonomy_core.agents._registration_status
# [DOMAIN] D_AUTONOMY_CORE
# [DEPENDENCIES] pyyaml ; zephyr.autonomy_core.agents._run_store
# [CONSUMERS] zephyr.autonomy_core.agents.business_agent_entry（B16 拆出后再导出，公共 API 不变；_g04_ops_check 经入口沿用 ADVICE_ONLY_DISCLAIMER）
# [STARTUP] imported
# [MATURITY] production
# [INVARIANTS] 注册表只读（真源=62号文 18 业务注册表）；产出 100% 落盘且标"仅建议"+human_gated；零交易执行路径——不 import 任何下单/执行域包（zephyr.ex_core/ex_sor/trading）
...
# [TESTS] tests/autonomy/test_execution_layer_agent_entries.py（经 business_agent_entry 公共 API 覆盖；B16 拆分未新增测试）
# [A_module] module_id=MOD-EXE-BIZ-001 | layer=module | stability=evolving | safety=M | ai_autonomy=human_gated
# [TTL] permanent
"""
B16 治本拆分（长城审计 §6#18，2026-09-05）：自 business_agent_entry.py（原 304 行，超
200 行薄入口上限）拆出的注册状态实现件。入口保持薄组装/CLI 分发，本模块承载
factor/strategy 注册表只读读取与两类工单实现：
  - registration_status：factor/strategy registry 状态汇总+单条目登记状态；
  - factor_candidate_eval：status=candidate 候选条目评估建议文本。
产出物一律"仅建议"（advice_only=true），不做自动交易决策/下单（交易决策属交易决策侧，§5-3）。
"""
