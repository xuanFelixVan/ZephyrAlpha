...
# [MODULE] zephyr.autonomy_core.agents._algorithm_experiment
# [DOMAIN] D_AUTONOMY_CORE
# [DEPENDENCIES] zephyr.autonomy_core.agents._run_store ; zephyr.experiment_tracking.query ; zephyr.experiment_tracking.config ; zephyr.trading.gpu_monitor（默认显存采集，函数内懒加载） ; zephyr.autonomy_core.agents._s11_wiring（§4-S1.1 可选接线，函数内懒加载）
# [CONSUMERS] zephyr.autonomy_core.agents.algorithm_agent_entry（B16 拆出后再导出，公共 API 不变）
# [STARTUP] imported
# [MATURITY] production
...
# [TESTS] tests/autonomy/test_execution_layer_agent_entries.py（经 algorithm_agent_entry 公共 API 覆盖；B16 拆分未新增测试） ; tests/autonomy/test_execution_layer_s11_wiring.py（同经入口覆盖）
# [A_module] module_id=MOD-EXE-ALGO-001 | layer=module | stability=evolving | safety=M | ai_autonomy=human_gated
# [TTL] permanent
"""
B16 治本拆分（长城审计 §6#18，2026-09-05）：自 algorithm_agent_entry.py（原 256 行，
超 200 行薄入口上限）拆出的算法实验工单实现件。入口保持薄组装/CLI 分发，本模块
承载实验工单端到端实现（三步纪律 + S1.1 可选接线）：
  ①实验登记先于执行——落 experiment_registration.pending.json 待登记片段；
  ②显存守卫——memory_used/memory_total >= 0.90 拒启动（refused_vram）；
  ③执行步只读——experiment_tracking.query.get_run 读既有实验记录出评估报告。
产出物 100% 落盘 human_gated；不写注册表本体（REG-EXP-001 登记交统筹）。
"""
