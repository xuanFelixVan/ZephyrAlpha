...
# [MODULE] zephyr.autonomy_core.agents._self_iteration_review
# [DOMAIN] D_AUTONOMY_CORE
# [DEPENDENCIES] stdlib ; zephyr.autonomy_core.agents._run_store
# [CONSUMERS] zephyr.autonomy_core.agents.self_iteration_agent_entry（B16 拆出后再导出，公共 API 不变）
# [STARTUP] imported
# [MATURITY] production
# [INVARIANTS] 只读形态：仅消费落盘证据（读白名单=.runtime/logs/docs 内文件），不写 src/ 不写注册表；零代码自改路径（不 import 执行/编辑链模块）；建议工单 100% human_gated 标记（Phase 2 前一律人审）
# [MODIFY-GUARD] Owner approval required; 变更须同步 14号文 §4 S0.5 验收口径
# [STABILITY] evolving
# [SAFETY] M
# [AI_AUTONOMY] human_gated
# [ERROR_CONTRACT] 证据路径越白名单/不可读→跳过并如实记 skipped_evidence，不抛
# [TESTS] tests/autonomy/test_execution_layer_agent_entries.py（经 self_iteration_agent_entry 公共 API 覆盖；B16 拆分未新增测试） ; tests/autonomy/test_execution_layer_s11_wiring.py（同经入口覆盖）
# [A_module] module_id=MOD-EXE-ITER-001 | layer=module | stability=evolving | safety=M | ai_autonomy=human_gated
# [TTL] permanent
"""
B16 治本拆分（长城审计 §6#18，2026-09-05）：自 self_iteration_agent_entry.py（原 284
行，超 200 行薄入口上限）拆出的迭代评估实现件。入口保持薄组装/CLI 分发，本模块
承载只读证据评估实现：白名单（.runtime/logs/docs）路径解析与跳过留痕、json/jsonl
证据解析、gate 判定与实验通过情况汇总、模板化 human_gated 建议工单（只复述缺口，
不下执行结论）。零代码自改路径，不做权重自更新/自进化搜索/自动改码。
"""
