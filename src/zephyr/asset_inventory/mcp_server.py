...
# [MODULE] zephyr.asset_inventory.mcp_server
...
INDEX_PATH = ROOT / "data" / "asset_index" / "unified_asset_index.yaml"
DASHBOARD_PATH = ROOT / "data" / "reports" / "dashboard.json"
SCAN_PATH = ROOT / "data" / "scans" / "raw_asset_scan.json"
...
def get_asset_summary() -> str:
    index = _load_index()
    if not index:
        return json.dumps({"error": "unified_asset_index.yaml not found — run index_generator first"})
...
MCP_TOOLS = {
    ...
    "get_asset_detail": {
        ...
        "params": {"path": "str (required) — 资产相对路径如 'src/zephyr/asset_inventory/scanner.py'"},
    },
