...
# [MODULE] zephyr.asset_inventory.reconciler
...
蓝图 §3.4：比对新扫描结果 vs unified_asset_index.yaml → 检测三类偏移
（孤儿/幽灵/漂移），产出 reconciliation_report.md。
"""
...
class Reconciler:
        ...
        for path, idx in index_assets.items():
            if path not in scan_sha:
                ...
                ghosts.append(GhostEntry(
                    registry_id="unified_asset_index",
                    registry_path=path,
    ...
    def save(self, report: ReconciliationReport, output_path: Optional[Path] = None) -> Path:
        ...
        target = output_path or (reports_dir / "reconciliation_report.md")
