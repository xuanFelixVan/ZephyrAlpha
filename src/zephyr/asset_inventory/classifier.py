...
# [MODULE] zephyr.asset_inventory.classifier
...
蓝图 §3.2：读取扫描结果，按 config/capacity/asset_inventory.yaml 中
...
class Classifier:
    ...
    def main(self) -> None:
        ...
        scan_path = scans_dir / "raw_asset_scan.json"

        if not scan_path.exists():
            print("警告: 扫描文件不存在，运行 scanner 先")
            print(f"  python -c \"from zephyr.asset_inventory.scanner import Scanner; r = Scanner().scan(); Scanner().save(r)\"")
            return
        ...
        out = klass_dir / "classified_assets.json"
