...
用法:
    python -m zephyr.asset_inventory scan       # 全量文件系统扫描
    python -m zephyr.asset_inventory classify    # 资产自动分类
    python -m zephyr.asset_inventory reconcile   # 注册表 vs 磁盘对账
    python -m zephyr.asset_inventory dashboard   # 健康仪表盘
    python -m zephyr.asset_inventory check       # Gate 检查 (exit 0=GREEN, 1=RED)
    python -m zephyr.asset_inventory bootstrap   # 从零自举 (scan→classify→reconcile→dashboard)
    python -m zephyr.asset_inventory clean       # 清理过期产物
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="python -m zephyr.asset_inventory",
...
def _load_scan() -> str | None:
    p = ROOT / "data" / "scans" / "raw_asset_scan.json"
    return str(p) if p.exists() else None


def _load_classified() -> str | None:
    p = ROOT / "data" / "classified" / "classified_assets.json"
    return str(p) if p.exists() else None
...
def _cmd_classify(args: argparse.Namespace) -> int:
    ...
    if not scan_path:
        ...
        print("  python -m zephyr.asset_inventory scan", file=sys.stderr)
        return 2
    ...
    out = out_dir / "classified_assets.json"
...
def _cmd_reconcile(args: argparse.Namespace) -> int:
    ...
    idx_path = ROOT / "data" / "asset_index" / "unified_asset_index.yaml"
    existing = None
    if idx_path.exists():
        import yaml
...
def _cmd_dashboard(args: argparse.Namespace) -> int:
    idx_p = ROOT / "data" / "asset_index" / "unified_asset_index.yaml"
    if not idx_p.exists():
    ...
    from zephyr.asset_inventory.models import UnifiedAssetIndex
...
def _cmd_deps(args: argparse.Namespace) -> int:
    ...
    from zephyr.asset_inventory.dependency import build_dependency_graph
    ...
    out_path = ROOT / "data" / "asset_index" / "dependency_graph.json"
...
def _cmd_registries(args: argparse.Namespace) -> int:
    from zephyr.asset_inventory.registry_adapter import RegistryManager
