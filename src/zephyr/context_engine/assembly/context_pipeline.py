...
--------------------------------------------------------------------
1. ``zephyr.context_engine`` 包文档与 ``docs/.../context-engine-interface.md``
...
from zephyr.context_engine.support.architecture_context_loader import (
...
def run_context_four_stage(
    ...
    *,
    ...
    include_architecture_context
        为 True 时尝试加载 ``architecture_context.json`` 并前置到 ``final_context``。
