# [BLUEPRINT] MOD-INF-018 | (auto-injected by S4 reconciler) | §
# [TTL] permanent
# [A_module] module_id=MOD-SEC-security | layer=module | stability=evolving | safety=L | ai_autonomy=ai_modifiable
# [MODULE] zephyr.security

"""


# [ALGO_FLOW]
# 层: 输入
# - id: I1
#   name: Python 包导入请求
#   fields: 无数据字段（解释器 import 机制触发，不读任何数据表）
#   code: import zephyr.security
# 层: 算法
# - id: A1
#   name_zh: ① 模块命名空间声明
#   name_en: zephyr.security.__init__
#   intro: MOD-INF-018 包入口
#   desc: MOD-INF-018 包入口，模块命名空间声明并声明 __all__（2项）
#   inputs: I1
#   outputs: zephyr.security 包级公共命名空间
#   invariant: 包级导出以 __all__ 声明为准（2项）
# 层: 输出
# - id: O1
#   name_zh: zephyr.security 包公共 API
#   name_en: __all__ 2项
#   intro: MOD-INF-018 包入口——对外统一出口
#   downstream: 见蓝图头 [CONSUMERS] 声明
# [/ALGO_FLOW]
# 边:
# I1 --> A1
# A1 --> O1
"""

from . import access_control, adversarial_validation

__all__ = [
    "access_control",
    "adversarial_validation",
]
