# [A_module] module_id=MOD-SEC_fix_reliability | layer=module | stability=evolving | safety=L | ai_autonomy=ai_modifiable
# [BLUEPRINT] MOD-INF-031 | docs/03_modules/_cross_layer/auto-fix-engine/blueprint.md | §3

# [MODULE] zephyr.security.access_control.auto_fix_engine_03.fix_reliability
...
class ConflictResolver:
    ...
    def resolve(self, actions: list[FixAction]) -> list[FixAction]:
        ...
        for target, target_actions in by_target.items():
            sorted_actions = sorted(target_actions, key=lambda a: (a.level.value, _CONFIDENCE_ORDER.get(a.confidence.value, 1), a.timestamp.isoformat() if isinstance(a.timestamp, datetime) else str(a.timestamp)))
