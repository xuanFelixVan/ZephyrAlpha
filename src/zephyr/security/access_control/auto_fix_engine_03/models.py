...
class ShadowResult(BaseModel):
    ...
    type_result: Optional[object] = None
...
_FROZEN_PUBLIC_API = frozenset({
    "FixLevel", "FixConfidence", "FixStatus", "BlastRadius",
    "ValidationResult", "BudgetInfo", "SafetyDecision", "BudgetDecision",
    "FixAction", "FixHistory", "FixDeadLetter", "FixReport",
    "FixHealthReport", "ShadowResult", "ComplianceEvidence", "BaseFixer",
})
...
def __getattr__(name: str):
    if name in _FROZEN_PUBLIC_API:
        ...
        logging.getLogger("zephyr.stability_guard").warning(
            "STABILITY VIOLATION: Public API attribute '%s' removed from frozen module zephyr.security.access_control.auto_fix_engine_03.models", name
        )
    raise AttributeError(f"module 'zephyr.security.access_control.auto_fix_engine_03.models' has no attribute {name!r}")
