...
class AlphaSignalPipeline:
    ...
    def run(
        ...
        if not self._factors:
            try:
                from zephyr.l02_alpha_factor.base import FactorBase as FB
    ...
    def _run_synthesis(self, factor_signals: list, idempotency_key: str) -> list:
        if not self._synthesizers:
            try:
                from zephyr.l03_signal_generation.signal_synthesizer import SignalSynthesizerBase as SSB
        ...
        for synth_cls in self._synthesizers:
            ...
            if hasattr(synth, "synthesize"):
                result = synth.synthesize(factor_signals)
