...
# [ALGO_FLOW] external: docs/03_modules/_domain_regime/algo_flow/overlay_features.yaml
"""
...
# 值格式："<状态> | <CH 只读实证依据> | <处置>"
#   状态 ALERT    = 现行阈值影响生产且「未经本土 walk-forward 复推」或「已复推但未过预注册验收带」
#                   → 运行期一次性告警，值沿用（过闸前换值属 Owner 门）
...
THRESHOLD_CALIBRATION_LEDGER: dict[str, str] = {
    "s2_breadth_thrust": (
        "ALERT | 0.615/0.40 为美股 NYSE 标准；A 股本土 walk-forward 复推**已跑完**"
        "（scripts/regime_breadth_thrust_walkforward.py，预注册 s2_breadth_thrust_walkforward_v1 "
        "hash 锁，网格 5×3，候选映射与生产函数逐日等值自检 5272/5272 日，母体 2005-04-07 至 "
        "2026-09-15 共 5212 有效日、死日剔除 0）：全样本 thrust 分位 p≈87 / washout 分位 p≈8.7 "
        "⇒ 旧述 p88/p10 方向正确（量级差 1 个百分点内），但 on_share≈0.126 超验收上界 ⇒ 全样本不过；"
        "post_2014 与 post_2019 窗口过；跨折 on_share 极差比≈6.96 超上限 ⇒ 2005-2013 早期段偏松是主因；"
        "训练段 4/4 折可选值、样本外仅 2/4 折过 ⇒ 按闸门判**不采纳** | 保留现值 + 告警（告警语义自"
        "「A 股从未复推」收窄为「跨时代稳定性未过、近十年窗口实测可用」）；采纳新值属 Owner 门且须新一次"
        "预注册（禁覆盖本预注册名=事后改考卷），重推前置=早期段广度进料覆盖度与该段 EMA 分布漂移单列归因"
        "（实证明细见 docs/_working/full-auto-chain/S11_assembled_backtest/"
        "breadth_thrust_walkforward_20260916.md「§4 裁定口径」）"
    ),
    ...
    "t3_money_effect": (
        "ALERT | 涨停家数门槛 30/50/100 单调（CH 全历史命中≈62%/45%/10%，但该组数是混合时代均值——"
        "2015-2018 三档占比≈2%／≈0.5%／≈0%，2019-2026 为≈94%／≈69%／≈15%，>30 档跨时代差≈54 倍，"
        "「全历史命中」掩盖了分时代漂移）；本土 walk-forward 预注册扫描已完成（68×6 候选点仅 1 个过"
        "严格接受带，其语义=几乎每天命中、无判别力）⇒ 按闸门判**不采纳任何新档位**（实证明细与两个"
        "备选案见 docs/_working/full-auto-chain/S11_assembled_backtest/"
        "t3_threshold_walkforward_20260917.md「§5 裁定要点」） | 保留现值 + 告警（告警语义自「无 A 股预注册"
        "标定」收窄为「已标定但无候选过带：现值与滚动分位口径孰对未裁定」）；改滚动分位口径属 Owner 门，"
        "禁施工期降阈值凑分"
    ),
    "t3_mainline": (
        "ALERT | HHI 阶梯 0.08/0.10/0.15 依赖板块分类法版本（596 板块口径下每日 HHI 中枢≈p35 恒低，"
        "三档几乎不区分）；本土 walk-forward 已完成：生产观测口径三档占比≈0.087／≈0.048／≈0.032，"
        "而这些命中日 11/11、6/6、4/4 **全部**落在进料口坏板块代码日（以 880005.SH/880006.SH 为主），"
        "剔伪后三档占比均为 0（干净 hhi 最大≈0.0049，远低于最松档）⇒ 无候选过接受带，按闸门判"
        "**不采纳重标定**；候选替代量 hhi_norm=(hhi−1/n)/(1−1/n) 与 hhi 的 Spearman≈0.9994（在本窗"
        "近于化妆，仍随板块数 n 变化），不足以单独承担「按板块数归一」（实证明细见 docs/"
        "_working/full-auto-chain/S11_assembled_backtest/t3_threshold_walkforward_20260917.md"
        "「§0/§6」） | 保留现值 + 告警（告警语义收窄为「数值未裁定，但观测量已知被进料口伪值污染」）；"
        "治本前置=板块覆盖度门（缺盘板块日不参与打分）+ 坏代码供应商侧修复，改分位口径属 Owner 门"
    ),
}
