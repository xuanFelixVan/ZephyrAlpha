...
# [MODULE] zephyr.pipeline.model_profiler.task_model_learner
...
class ModelTaskMatrix:
    ...
    def recommend(self, task_type: str) -> TaskRecommendation:
        ...
        from zephyr.pipeline.models import M_MODULE_SPECS
    ...
    def recommend_all(self) -> list[TaskRecommendation]:
        """为所有已知任务类型推荐最佳模型。"""
        from zephyr.pipeline.models import M_MODULES
    ...
    def persistence_path(self) -> str:
        return str(self._dir / "task_model_matrix.json")
    ...
    def _save(self) -> None:
        try:
            ...
            (self._dir / "task_model_matrix.json").write_text(
    ...
    def _load(self) -> None:
        path = self._dir / "task_model_matrix.json"
        if not path.exists():
            return
        try:
