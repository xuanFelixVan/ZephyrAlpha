...
----
    ...
    python -m zephyr.pipeline.model_profiler.cli quick qwen3:8b  # 快速测试单个模型
    python -m zephyr.pipeline.model_profiler.cli benchmark    # 全量 benchmark
    python -m zephyr.pipeline.model_profiler.cli drift qwen3:8b   # 漂移检测
    python -m zephyr.pipeline.model_profiler.cli best         # 显示最佳模型
    python -m zephyr.pipeline.model_profiler.cli history      # 查看历史 benchmark 记录
"""
...
def main() -> None:
    ...
    if cmd == "discover":
        ...
        if len(args) < 2:
            print("用法: python -m zephyr.pipeline.model_profiler.cli quick <model_name>")
            return
        cmd_quick(args[1])
    elif cmd == "benchmark":
        cmd_benchmark()
        ...
        if len(args) < 2:
            print("用法: python -m zephyr.pipeline.model_profiler.cli drift <model_name>")
            return
        cmd_drift(args[1])
    elif cmd == "history":
        cmd_history()
