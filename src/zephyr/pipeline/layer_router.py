...
依据: cross-layer-contracts.yaml v3.0 — 31 条跨层契约
...
设计原则:
  - 路由表从 cross-layer-contracts.yaml 一次加载后不可变（frozen）
...
def load_route_map(yaml_path: Path | None = None) -> LayerRouteMap:
    """从 cross-layer-contracts.yaml 加载冻结的层间路由表。
