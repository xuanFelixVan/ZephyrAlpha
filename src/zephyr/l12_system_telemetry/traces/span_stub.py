...
def noop_span(
    ...
    if parent is None:
        parent = parent_span.context if parent_span else TraceContext.new_root()

    ctx = TraceContext.new_child(parent) if parent_span else TraceContext.new_root()
...
def _flush_span(span: Span) -> None:
    try:
        from zephyr.l12_system_telemetry.logs.structured_sink import append_jsonl_record
        append_jsonl_record(span.snapshot(), labels={"__type": "trace_span"})
    except Exception:
        pass
