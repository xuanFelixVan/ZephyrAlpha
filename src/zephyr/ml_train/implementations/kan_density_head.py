...
class KanDensityHead:
    ...
    def __init__(
        ...
        *,
        ...
        if not 1 <= self.config.spline_order <= MAX_SPLINE_ORDER:
            raise KanHeadError(
                f"样条阶数越界: {self.config.spline_order}（须 ∈[1,{MAX_SPLINE_ORDER}] 护栏）"
            )
        if self.config.n_grid < 1:
            raise KanHeadError(f"n_grid 越界: {self.config.n_grid}（须 ≥1）")
        if not self.config.quantiles or any(
            not 0.0 < q < 1.0 for q in self.config.quantiles
        ):
    ...
    def _build_knots(x_min: float, x_max: float, n_grid: int, degree: int) -> np.ndarray:
        ...
        return np.concatenate([
            np.full(degree + 1, x_min), internal, np.full(degree + 1, x_max),
        ])

    @classmethod
    def _basis_matrix(
        cls, x: np.ndarray, knots: np.ndarray, degree: int, n_basis: int
    ) -> np.ndarray:
        ...
        for p in range(1, degree + 1):
            ...
            for i in range(len(knots) - 1 - p):
                ...
                term_left = (
                    (x - knots[i]) / denom_left * basis[:, i] if denom_left > 0 else 0.0
                )
                term_right = (
                    (knots[i + p + 1] - x) / denom_right * basis[:, i + 1]
    ...
    def fit(self, x: np.ndarray, y: np.ndarray) -> dict[str, Any]:
        ...
        self._offsets = {
            q: float(np.quantile(residuals, q)) for q in self.config.quantiles
        }
    ...
    def predict_quantiles(self, x: np.ndarray) -> dict[float, np.ndarray]:
        ...
        if arr.ndim != 2 or arr.shape[1] != self._n_features:
            raise KanHeadError(
                f"x 维度不符: 期望 (n,{self._n_features})，实得 shape={arr.shape}"
            )
        base = self._design_matrix(arr) @ self._coef
        raw = np.column_stack([
            base + self._offsets[q] for q in self.config.quantiles
        ])
        monotone = np.maximum.accumulate(raw, axis=1)
        return {
            q: monotone[:, i] for i, q in enumerate(self.config.quantiles)
        }
