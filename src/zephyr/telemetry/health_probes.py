...
依据：MOD-MASTER-001 蓝图 §十四 标准化 HealthCheck
...
SYSTEMS: tuple[str, ...] = (
    "orchestrator", "script_system", "knowledge_base", "context_engine",
    "gate_engine", "pipeline", "feedback_loop", "vector_memory",
    "database", "llm_security", "system_telemetry", "mcp_servers",
)

SPECIAL_RULES: dict[str, dict] = {
    "orchestrator": {"degraded_when": "pending_queue > 100"},
    "context_engine": {"degraded_when": "token_budget > 7200"},
    "llm_security": {"no_degraded": True},
...
class HealthProbeManager:
    ...
    def healthz(self, system: str, metrics: dict[str, Any] | None = None) -> dict:
        ...
        if metrics:
            ...
            elif system == "context_engine" and metrics.get("token_budget", 0) > 7200:
                degraded = True
