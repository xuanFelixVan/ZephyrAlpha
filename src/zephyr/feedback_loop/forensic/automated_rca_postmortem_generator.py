...
using temporal ordering + correlation. Generate timeline, 5 Whys analysis,
...
class AutomatedRCAPostmortemGenerator:
    ...
    def close_incident(
        ...
        five_whys = self._generate_five_whys(timeline)
        ...
        postmortem = {
            ...
            "five_whys": five_whys,
            "action_items": self._generate_action_items(timeline, five_whys),
        }

        self.generated_postmortems.append(postmortem)
        return postmortem

    def _generate_five_whys(self, timeline: list[dict]) -> list[dict]:
        whys = []
        ...
        for i in range(min(self.five_whys_depth, len(root_events) + 1)):
            whys.append({
                "level": i + 1,
    ...
    def _generate_action_items(self, timeline: list[dict], five_whys: list[dict]) -> list[dict]:
        items = []
