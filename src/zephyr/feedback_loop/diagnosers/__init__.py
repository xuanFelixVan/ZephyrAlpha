...
# [ALGO_FLOW] external: docs/03_modules/_domain_feedback_loop/algo_flow/diagnosers/diagnosers__init__.yaml
"""
...
def __getattr__(name):
    if name in _LEAF_TO_SUBPKG:
        mod = importlib.import_module(f"zephyr.feedback_loop.diagnosers.{_LEAF_TO_SUBPKG[name]}.{name}")
        globals()[name] = mod
        return mod
