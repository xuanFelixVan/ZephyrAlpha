...
# [TTL] permanent
# noqa: m11-perm-manual-legitimate  M11豁免: 生成器继承产物，由生成管线事件驱动再生成，CLI 仅调试入口
...
class KnowledgeFreshness:
    ...
    def score(self, entry_id: str, created_at: float) -> float:
        ...
        age_days = (time.time() - created_at) / 86400.0  # noqa: m46-time  M46豁免: epoch秒浮点时间戳用于存活心跳与时效计算，非本地时区展示
...
from zephyr.shared.foundation.constants import DEFAULT_OTLP_ENDPOINT
