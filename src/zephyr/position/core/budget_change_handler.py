...
# [ALGO_FLOW] external: docs/03_modules/_domain_position/algo_flow/budget_change_handler.yaml
"""
...
class ForcedTrim:
    ...
    执行口径（PFA-1 车道 D 治本：原 docstring "所有仓位 × trim_ratio" 与字段注
    "trim_ratio=削 20%" 相互矛盾——照前者执行会把仓位削到 20%，照后者是削掉 20%）：

      - trim_ratio   = (current_exposure − target_budget) / current_exposure
                       = **要削掉的比例**（0.2 → 削 20%）
      - retain_ratio = 1 − trim_ratio = **执行侧唯一应乘的乘子**（所有仓位 × retain_ratio）

    消费方（FirmRiskAggregator / 执行层）必须用 retain_ratio 做等比缩放，
    trim_ratio 仅用于审计与告警文案。
    """

    strategy_id: str
    trim_ratio: float  # 要削掉的比例（0.2 = 所有仓位削 20%），= 1 − retain_ratio
    reason: str  # 触发原因（Tier 2 超时 / firm 风险违例）
    retain_ratio: float | None = None  # 执行乘子（None=由 trim_ratio 反推，见 __post_init__）
    timestamp: datetime = field(default_factory=datetime.now)
    schema_version: str = "1.0"

    def __post_init__(self) -> None:
        """retain_ratio 缺省时按 1−trim_ratio 反推（构造方无须记住两口径）。"""
        if self.retain_ratio is None:
            object.__setattr__(self, "retain_ratio", 1.0 - float(self.trim_ratio))
...
class BudgetChangeHandler:
    ...
    def _issue_tier3_force_trim(self, strategy_id: str, trim_ratio: float, reason: str) -> ForcedTrim:
        ...
        trim_ratio=要削掉的比例；retain_ratio=执行乘子（消费方乘这个）。
        """
        return ForcedTrim(
            strategy_id=strategy_id,
            trim_ratio=trim_ratio,
            reason=reason,
            retain_ratio=1.0 - float(trim_ratio),
        )
