...
# [MODULE] zephyr.drift_detector.integration_test_runner
...
随声执行器：`pip install -e . \r\n python -m drift_detector --mode test`。
...
class IntegrationTestRunner:
    ...
    def import_check(self) -> SelfTestResult:
        ...
        for mod in modules:
            try:
                __import__(f"zephyr.drift_detector.{mod}")
    ...
    def type_check(self) -> SelfTestResult:
        ...
        file_path = os.path.join("src", "zephyr", "drift_detector", "self_check.py")
