...
# [MODULE] zephyr.drift_detector.__main__
...
用法
----
    python -m zephyr.drift_detector scan                    # 默认 LIGHT 扫描
    python -m zephyr.drift_detector scan --level STANDARD   # 标准扫描
    python -m zephyr.drift_detector scan --level DEEP       # 深度扫描
    python -m zephyr.drift_detector self-test               # SelfTestVerifier 自检
    python -m zephyr.drift_detector budget [module_id]      # 检查漂移预算
    python -m zephyr.drift_detector list                    # 列出所有已注册检测器
    python -m zephyr.drift_detector status                  # 模块健康状态
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        prog="python -m zephyr.drift_detector",
