...
# [MODULE] zephyr.drift_detector.self_test_verifier
...
class SelfTestVerifier:
            ...
            for fname in os.listdir(self._base_dir):
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.ImportFrom):
                        if node.module and node.module.startswith("zephyr.drift_detector"):
                            target = node.module.replace("zephyr.drift_detector.", "") + ".py"
    ...
    def check_file_completeness(self) -> dict[str, str]:
        required = [
            ...
            "integration_test_runner.py", "_detector_registry.yaml", "__init__.py",
        ]
