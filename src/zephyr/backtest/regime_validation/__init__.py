...
"""D_BACKTEST — Regime 回测验证包（discussion_002 验证方案落地）。

对接现有 BM-BT 回测框架，为 regime 检测器 (MOD-REGIME-001) 提供验证工具：
  - ShrinkageProvider 系列 (B2): 对接 RegimeDetector / mock / 查表
  - C1ShrinkageComparator (C1): Shrinkage 开/关对比 + 一票否决（discussion_002 §4.3/§5）

依赖方向: backtest → regime（消费 regime 的 ShrinkageResult），符合
discussion_002 §2.1 "regime 验证复用现有 BM-BT 框架" 的对接约定。
"""
