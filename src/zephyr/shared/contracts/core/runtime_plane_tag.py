...
# [ERROR_CONTRACT]
# [TESTS]
# [A_module] module_id=MOD-SHR-runtime_plane_tag | layer=module | stability=evolving | safety=L | ai_autonomy=ai_modifiable
# [TTL] permanent
...
🔒 **契约预留（Reserved Contract）**：本文件定义正交视图所用的
...
1. 为每个 D_DATA~实验 + shared + frontend + scripts 模块声明所属 **Runtime Plane**
   （Hot / Warm / Cold），作为正交视图的机器可读元数据（架构视图真源：`architecture_model/cross_cutting/runtime_planes.yaml`）。
...
═══════════════════════════════════════════════════════════════════════
【三档 Runtime Plane 定义】（与 architecture_model/cross_cutting/runtime_planes.yaml §2 对齐；原 runtime_planes_principles.md 已删 2026-07-30）
═══════════════════════════════════════════════════════════════════════
- **HOT**（Hot Path）:
...
【与架构视图的关系】
═══════════════════════════════════════════════════════════════════════
- 架构视图真源：`architecture_model/cross_cutting/runtime_planes.yaml`（原 runtime_planes_principles.md 已删 2026-07-30，git 历史可查），定义平面划分方法论、业务归属矩阵、通信协议、激活触发器
