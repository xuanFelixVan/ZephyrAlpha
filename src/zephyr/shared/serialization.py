...
def _deserialize_value(value: Any) -> Any:
    ...
    if isinstance(value, (int, float, bool, str)):
        return value
    raise SerializationError(
        f"deserialization without schema requires typed values. "
        f"Use from_dict(value, model=YourModel) for Pydantic models, "
        f"or explicit deserialize_decimal/deserialize_datetime for scalars.",
        details={"value": str(value)[:200], "type": type(value).__name__},
    )
