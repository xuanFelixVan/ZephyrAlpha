...
from zephyr.shared.resilience.circuit_breaker import CircuitBreaker, CircuitState
from zephyr.shared.resilience.retry import RetryConfig, async_retry
from zephyr.shared.serialization import from_dict, to_dict, to_json
...
class ApiClient:
        ...
        async def _execute(_attempt: int = 1) -> ApiResponse:
            ...
            try:
                ...
                metrics = ApiCallMetrics(
                    ...
                    duration_ms=duration_ms, attempt=_attempt,
            ...
            metrics = ApiCallMetrics(
                ...
                duration_ms=round(duration_ms, 2), attempt=_attempt,
            ...
            if not resp.is_success:
                raise ApiCallError(
                    ...
                    details={"url": url, "method": method.value, "status_code": resp.status_code,
    ...
    async def get(self, path: str, *, headers: dict[str, str] | None = None, timeout_seconds: float | None = None) -> ApiResponse:
        return await self.request(HttpMethod.GET, path, headers=headers, timeout_seconds=timeout_seconds)

    async def post(self, path: str, *, body: dict[str, Any] | None = None, headers: dict[str, str] | None = None, timeout_seconds: float | None = None) -> ApiResponse:
        return await self.request(HttpMethod.POST, path, headers=headers, body=body, timeout_seconds=timeout_seconds)

    async def put(self, path: str, *, body: dict[str, Any] | None = None, headers: dict[str, str] | None = None, timeout_seconds: float | None = None) -> ApiResponse:
        return await self.request(HttpMethod.PUT, path, headers=headers, body=body, timeout_seconds=timeout_seconds)

    async def patch(self, path: str, *, body: dict[str, Any] | None = None, headers: dict[str, str] | None = None, timeout_seconds: float | None = None) -> ApiResponse:
        return await self.request(HttpMethod.PATCH, path, headers=headers, body=body, timeout_seconds=timeout_seconds)

    async def delete(self, path: str, *, headers: dict[str, str] | None = None, timeout_seconds: float | None = None) -> ApiResponse:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.request(
                ...
                return ApiResponse(
                    ...
                    body=response_body,
                    metrics=ApiCallMetrics(
                        url=url, method=method, status_code=resp.status,
                        duration_ms=0, attempt=1, success=200 <= resp.status < 300,
                    ),
                )
