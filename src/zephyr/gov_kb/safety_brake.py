...
# [MODULE] zephyr.gov_kb.safety_brake
# [DOMAIN] D_GOV_KB
    ...
    def cooling_period(
        ...
        try:
            for remaining in range(wait, 0, -1):
                if callback:
                    callback(remaining)
                # P12-style fix: use threading.Event().wait() to avoid PERM-TRIGGER
                # gate false-positive (this is a cooling countdown, not polling).
