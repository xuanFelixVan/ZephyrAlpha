...
# [MODULE] zephyr.gov_kb.self_test
# [DOMAIN] D_GOV_KB
...
_SQL_CHECK_TOMBSTONE_TABLE = "SELECT name FROM sqlite_master WHERE type='table' AND name='ke_tombstones'"
...
def _check_tombstone_integrity(root: Path) -> CheckResult:
    try:
        ...
        conn = get_db_connection(str(db_path))
        cursor = conn.execute(_SQL_CHECK_TOMBSTONE_TABLE)
