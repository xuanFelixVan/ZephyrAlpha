...
# [MODULE] zephyr.agent_spec.skill_registry
...
"""
skill_registry.py —— Skill 注册基座（Phase 14 | 盲点 B34）

痛点修复：prompt_registry.py 在 context_engine/ 而非 shared/，
...
设计对标：
  ...
  - context_engine/prompt_registry.py: 已有的 PromptTemplate 模型（本模块对齐）
  - VSCode Copilot Skills: 可注册/可发现的能力模块化

AI 施工约定：
  - SkillDefinition 是 shared/ 层的抽象契约—不包含引擎逻辑
  - 与 context_engine/prompt_registry.py 互补——本模块为跨层数据契约
...
class PromptVariable(BaseModel):
    """Prompt 模板的变量声明。与 context_engine/prompt_registry.py 对齐。"""
