...
用法:
    python -m zephyr.kb --self-test        # 全量13项
    python -m zephyr.kb --self-test --json # JSON输出
"""
...
def _check_sqlite_integrity(root: Path) -> CheckResult:
    try:
        db_path = root / "data" / "zalpha_metadata.db"
        ...
        return CheckResult(
            1, "SQLite Integrity", CheckStatus.FAIL,
            str(e), "检查 data/zalpha_metadata.db 是否存在且可读写"
        )
    ...
    try:
        ...
        if bytes_per_entry > 4096:
            return CheckResult(
                ...
                "建议运行 reindex 重建HNSW图: python -m zephyr.kb.embedding_migrate reindex"
            )
        return CheckResult(
...
def _check_freeze_state(root: Path) -> CheckResult:
    try:
        ...
        return CheckResult(
            ...
            "若冻结已解决问题，运行 python -m zephyr.kb --unfreeze 恢复"
        )
    except Exception as e:
