...
# [MODULE] zephyr.kb.migration.embedding_migrate
...
from zephyr.kb.storage.chromadb_init import COLLECTION_NAMES
...
class EmbeddingMigrator:
    ...
    def register_version(self, model_name: str, dimension: int, provider: str) -> Self:
    ...
    def rollback(self, plan: MigrationPlan) -> Self:
        source_ver = self._versions.get(plan.source_model)
        target_ver = self._versions.get(plan.target_model)
        if target_ver:
