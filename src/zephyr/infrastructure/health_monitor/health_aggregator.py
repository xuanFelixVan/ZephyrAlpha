...
__all__ = ["HealthAggregator", "SystemHealth", "HealthReport", "check_all"]

@dataclass
class SystemHealth:
    system: str; status: str = "OK"; latency_ms: float = 0; details: str = ""

@dataclass  
class HealthReport:
    systems: list[SystemHealth] = field(default_factory=list)
    ok_count: int = 0; warn_count: int = 0; fail_count: int = 0
    overall: str = "OK"; total_ms: float = 0

SYSTEMS_CHECK = [
    "orchestrator","gate_engine","pipeline","script_system","context-engine",
    "knowledge_base","vector-memory","mcp","llm-security","telemetry","feedback-loop","database",
]
    ...
    def check_all(self, max_workers: int = 6) -> HealthReport:
        ...
        with ThreadPoolExecutor(max_workers=min(max_workers, len(SYSTEMS_CHECK))) as ex:
            ...
            for f in as_completed(futures):
                ...
                if h.status == "FAIL": report.fail_count += 1
                elif h.status == "WARN": report.warn_count += 1
                else: report.ok_count += 1
        ...
        logger.info("[HEALTH] %s ok=%d warn=%d fail=%d (%dms)", report.overall, report.ok_count, report.warn_count, report.fail_count, report.total_ms)
        return report
    ...
    def _check_one(self, system: str) -> SystemHealth:
        ...
        try:
            ...
            if hasattr(mod,"health_check"):
                r = mod.health_check()
                return SystemHealth(system, r.get("status","OK"), round((time.perf_counter()-t0)*1000), str(r.get("details","?"))[:200])
