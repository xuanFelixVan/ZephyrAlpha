...
class TaskManagerMCP:
    ...
    def _register_tools(self) -> None:
        ...
        async def create_task(
            ...
            TaskNamespace = getattr(_mod, "TaskNamespace")
            normalize_execution_model = getattr(_mod, "normalize_execution_model")
            ...
            tc = TaskCard(
                ...
                safety_level=getattr(
                    __import__("zephyr.integration.shared_08.schemas", fromlist=["SafetyLevel"]).SafetyLevel,
                    safety_level,
                    __import__("zephyr.integration.shared_08.schemas", fromlist=["SafetyLevel"]).SafetyLevel.L,
                ),
                ...
                forbidden_touch=[],
                applicable_rules=[{"module_id": source_blueprint, "section": source_section, "reason": "MCP create_task"}],
            ...
            if downstream_outputs:
                try:
                    ...
                    PathResolver = getattr(_mod, "PathResolver")
        ...
        async def claim_task(batch_id: str, worker_id: str) -> dict:
            """原子认领下一个依赖已满足的 READY 任务——多AI并发安全（MOD-INF-006 §16.7）"""
        ...
        async def list_dependents(task_id: str) -> dict:
            """查询所有依赖指定 task_id 的下游任务（MOD-INF-006 §16.7 依赖感知）"""
        ...
        async def write_draft(session_id: str, file_path: str, content: str) -> dict:
            ...
            StagingArea = getattr(_mod, "StagingArea")
...
def _parse_yaml_frontmatter_to_taskcard(fm: dict) -> TaskCard:
    ...
    classification = getattr(
        __import__("zephyr.integration.shared_08.schemas", fromlist=["Classification"]).Classification,
        classification_str.upper(),
        __import__("zephyr.integration.shared_08.schemas", fromlist=["Classification"]).Classification.INTERNAL,
    )

    evolution_str = str(fm.get("evolution_policy", "extendable"))
    evolution_policy = getattr(
        __import__("zephyr.integration.shared_08.schemas", fromlist=["EvolutionPolicy"]).EvolutionPolicy,
        evolution_str.upper(),
        __import__("zephyr.integration.shared_08.schemas", fromlist=["EvolutionPolicy"]).EvolutionPolicy.EXTENDABLE,
    )
