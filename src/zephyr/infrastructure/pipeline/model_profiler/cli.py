...
# [ERROR_CONTRACT]
# [TESTS]
# [A_module] module_id=MOD-INF_cli | layer=module | stability=evolving | safety=L | ai_autonomy=ai_modifiable
...
用法
----
    python -m zephyr.intelligence.model_profiling.pipeline.cli discover     # 列出所有可用模型
    python -m zephyr.intelligence.model_profiling.pipeline.cli quick qwen3:8b  # 快速测试单个模型
    python -m zephyr.intelligence.model_profiling.pipeline.cli benchmark    # 全量 benchmark
    python -m zephyr.intelligence.model_profiling.pipeline.cli drift qwen3:8b   # 漂移检测
    python -m zephyr.intelligence.model_profiling.pipeline.cli best         # 显示最佳模型
    python -m zephyr.intelligence.model_profiling.pipeline.cli history      # 查看历史 benchmark 记录
"""
...
def cmd_discover() -> None:
    _mod = importlib.import_module("zephyr.intelligence.model_profiling.pipeline.model_discovery")
...
def cmd_quick(model_name: str) -> None:
    _mod = importlib.import_module("zephyr.intelligence.model_profiling.pipeline.profiler")
...
def main() -> None:
    ...
    if cmd == "discover":
        ...
        if len(args) < 2:
            print("用法: python -m zephyr.intelligence.model_profiling.pipeline.cli quick <model_name>")
            return
        cmd_quick(args[1])
    elif cmd == "benchmark":
        cmd_benchmark()
        ...
        if len(args) < 2:
            print("用法: python -m zephyr.intelligence.model_profiling.pipeline.cli drift <model_name>")
            return
        cmd_drift(args[1])
    elif cmd == "history":
        cmd_history()
