...
使用：
    ...
    from zephyr.infrastructure.pipeline import PipelineOrchestrator, PipelineOrchestratorConfig
...
try:
    from zephyr.integration.vector_memory.local_model_scheduler import LocalModelScheduler
...
class PipelineOrchestrator:
    ...
    def _ensure_local_models(self) -> None:
        if self._cfg.local_model_always_on and self._embedding_router is None:
            try:
                from zephyr.integration.vector_memory.embedding_router import EmbeddingRouter
    ...
    def rerank_documents(self, query: str, documents: list[str]) -> list:
        ...
        if self._reranker_instance is None:
            try:
                ...
                self._reranker_instance = ServiceRegistry.get("reranker")
            except Exception as exc:
    ...
    def _lsg_sanitize_input(text: str) -> str:
        ...
        try:
            ...
            _proto_mod = importlib.import_module("zephyr.shared.contracts.security.security_decision")
            SecurityDecision = _proto_mod.SecurityDecision
    ...
    def _assess_impact(self, task_card: TaskCard) -> AIImpactAssessment:
        ...
        stakeholders = []
        if "infra_runtime" in tags_lower or "infrastructure" in tags_lower:
