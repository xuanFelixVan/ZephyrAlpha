...
# [MODULE] zephyr.integration.shared_08.lifecycle.daemon_registry
...
# [ERROR_CONTRACT]
# [TESTS]
# [A_module] module_id=MOD-INT_daemon_registry | layer=module | stability=evolving | safety=L | ai_autonomy=ai_modifiable
...
class ResourceSnapshot:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "ide_ghost_windows": self.ide_ghost_windows,
...
class _Thresholds:
    ...
    gpu_warning_pct: float = 85.0
    gpu_critical_pct: float = 95.0
    gpu_emergency_pct: float = 98.0
...
class DaemonRegistry:
    ...
    def snapshot_resources(cls) -> ResourceSnapshot:
        ...
        try:
            ...
            ide_health = importlib.import_module("zephyr.trading.ide_health_daemon")
            ghosts = ide_health.scan_ghost_windows()
    ...
    def _classify_pressure(cls, snap: ResourceSnapshot) -> PressureLevel:
        ...
        if snap.process_count >= t.process_emergency:
            return PressureLevel.EMERGENCY
        if snap.gpu_available and snap.gpu_percent >= t.gpu_emergency_pct:
            return PressureLevel.EMERGENCY
        ...
        if snap.cpu_percent >= t.cpu_critical_pct:
            return PressureLevel.CRITICAL
        if snap.gpu_available and snap.gpu_percent >= t.gpu_critical_pct:
            return PressureLevel.CRITICAL
        ...
        if snap.cpu_percent >= t.cpu_warning_pct:
            return PressureLevel.WARNING
        if snap.gpu_available and snap.gpu_percent >= t.gpu_warning_pct:
            return PressureLevel.WARNING
