...
def _bs_price(S, K, T, r, sigma, opt_type):
    ...
    return K * math.exp(-r * T) * _bs_cdf(-d2) - S * _bs_cdf(-d1)
...
class MiniQmtIngestProvider(IngestProviderBase):
    ...
    meta: IngestProviderMeta = IngestProviderMeta(
        ...
        capabilities=[
            ...
            CapabilityContract("kline_weekly", supports_symbols_null=True),
            CapabilityContract("kline_monthly", supports_symbols_null=True),
            ...
            CapabilityContract("futures_position", supports_symbols_null=True),
            CapabilityContract("shareholder", supports_symbols_null=True),
            CapabilityContract("earnings_forecast", supports_symbols_null=True),
            CapabilityContract("express_report", supports_symbols_null=True),
            ...
            CapabilityContract("kline_etf_1min", supports_symbols_null=True),
            CapabilityContract("kline_etf_5min", supports_symbols_null=True),
            ...
            CapabilityContract("kline_lof_30min", supports_symbols_null=True),
            CapabilityContract("kline_lof_60min", supports_symbols_null=True),
            CapabilityContract("kline_weekly_hfq", supports_symbols_null=True),
            CapabilityContract("kline_monthly_hfq", supports_symbols_null=True),
        ],
        known_issues=[
            "需XtMiniQmt.exe进程",
            "单线程",
    ...
    def _compute_futures_position_rows(
        ...
        if kline_df is not None and len(kline_df) > 0:
            kline_rows = self._build_futures_kline_rows(kline_df, symbol, exchange)
    ...
    def _compute_iv_rows(ctx: _OptionCtx) -> list[tuple]:
        ...
        for dt in common_dates:
            ...
            if ctx.exp_date:
                ...
                days = (ctx.exp_date - cur_date).days
    ...
    @staticmethod
    def _calc_hist_vol(ul_df, ul_ret, dt) -> float:
        """计算截止当日的 20 日历史波动率（年化）。"""
