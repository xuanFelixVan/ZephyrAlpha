...
----
    ...
    python -m zephyr.behavioral_audit scan                    # 默认 LIGHT 扫描

    python -m zephyr.behavioral_audit scan --level STANDARD   # 标准扫描

    python -m zephyr.behavioral_audit scan --level DEEP       # 深度扫描

    python -m zephyr.behavioral_audit self-test               # SelfTestVerifier 自检

    python -m zephyr.behavioral_audit budget [module_id]      # 检查漂移预算

    python -m zephyr.behavioral_audit list                    # 列出所有已注册检测器

    python -m zephyr.behavioral_audit status                  # 模块健康状态"""
...
def _cmd_scan(args: argparse.Namespace) -> int:
    try:
        from zephyr.behavioral_audit.drift_engine import ScanLevel, build_report, scan
...
def _cmd_list(args: argparse.Namespace) -> int:
    try:
        from zephyr.behavioral_audit.drift_engine import load_detector_registry
...
def _cmd_status(args: argparse.Namespace) -> int:
    ...
    try:
        from zephyr.behavioral_audit.self_check import bootstrap_self_check
...
def main() -> int:
    parser = argparse.ArgumentParser(
        prog="python -m zephyr.behavioral_audit",
