...
class MarketData:
    ...
    __slots__ = ("_instrument_id", "_bars", "_reports", "_version")
    ...
    def append_bar(self, bar: Bar) -> MarketData:
        ...
        if bar.instrument_id != self._instrument_id:
            raise MarketDataAggregateError(
                f"Bar 标的不符: {bar.instrument_id!r} 非聚合 {self._instrument_id!r}"
            )
        return MarketData(
            self._instrument_id,
            bars=self._bars + (bar,),
    ...
    def append_report(self, report: FinancialReport) -> MarketData:
        ...
        if report.instrument_id != self._instrument_id:
            raise MarketDataAggregateError(
                f"财报标的不符: {report.instrument_id!r} 非聚合 {self._instrument_id!r}"
            )
        return MarketData(
            self._instrument_id,
            bars=self._bars,
...
class Instrument:
    ...
    __slots__ = ("_instrument_id", "_symbol", "_exchange", "_version")
    ...
    def update_profile(self, *, symbol: str, exchange: str) -> Instrument:
        ...
        return Instrument(
            self._instrument_id, symbol=symbol, exchange=exchange, version=self._version + 1
        )
    ...
    def save(self, aggregate: MarketData, *, expected_version: int) -> None:
        ...
        if stored is None:
            if expected_version != 0:
                raise MarketDataAggregateError(
                    f"新建聚合 expected_version 须为 0，实收 {expected_version}"
                )
        ...
        if aggregate.version != expected_version + 1:
            raise MarketDataAggregateError(
                f"版本不变量违反: aggregate.version={aggregate.version} 须为 "
...
class RetentionPolicyRegistry:
    ...
    def register(self, policy: RetentionPolicy) -> None:
        ...
        if policy.drill_interval_days <= 0:
            raise MarketDataAggregateError(
                f"drill_interval_days 非正: {policy.drill_interval_days}"
            )
...
class RecoveryDrillLog:
    ...
    def record(self, rec: RecoveryDrillRecord) -> None:
        ...
        if rec.finished_at < rec.started_at:
            raise MarketDataAggregateError(
                f"演练时间倒置: finished_at {rec.finished_at} < started_at {rec.started_at}"
            )
