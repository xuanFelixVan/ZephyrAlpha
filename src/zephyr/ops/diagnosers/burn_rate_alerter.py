...
# [MODULE] zephyr.observability.feedback_loop.diagnosers.burn_rate_alerter
...
class BurnRateAlerter:
    slo_pct: float = 99.9
    windows: list[BurnWindow] = field(default_factory=lambda: [
    ...
    def record(self, success: bool) -> None:
        for w in self.windows:
            ...
            if w.total_count > 0:
                ...
                w.current_burn_rate = (w.error_count / w.total_count) / error_budget_pct if error_budget_pct > 0 else 0.0

    def alerts(self) -> list[str]:
        return [f"{w.name} burn rate {w.current_burn_rate:.1f}x > {w.target_burn_rate}x" for w in self.windows if w.current_burn_rate > w.target_burn_rate]
