...
#   desc: 梯队健康度四档判定（§3.1，v1.5.0）：PERFECT/FRACTURE/LONE_DRAGON/COLLAPSE。 height_counts: {连板高度: 家数}，如…；源码 L158-L176
...
#   desc: 中位股死亡池（§3.1，v1.5.0）：梯队断层/孤龙/崩塌时 3 板扣 40、4 板扣 30；完美梯队维持原评分。 COLLAPSE 比 FRACTURE 更凶险（梯队全崩）…；源码 L179-L197
...
#   desc: 集合竞价三维 100 分（§3.9，v1.6.0）：大盘 30+板块 30+个股 40，9:25 竞价定格后调用。 输入为各维原始分（自动夹取到 0~上限）；总分≥80 打板确认…；源码 L200-L222
...
#   desc: 纸老虎识别（§3.9，v1.6.0）：竞价涨幅 7-8% 但匹配量<总量 3%=主力演戏，一票否决（IC 胜率 95%+）。 auction_gain: 竞价涨幅（小数，0.07…；源码 L225-L236
...
#   desc: 封单结构双指标（§3.11①，v1.8.0）：封流比≥5%稳定/<2%薄弱 + 封成比>10稳定/<1不牢。 双指标各 50 分：封流比≥5%→50 / 2%-5%→25 / <…；源码 L239-L267
...
#   desc: 次日溢价三维预测（§3.11②，v1.8.0）：封板时间×量能×封单→预期溢价区间+操作建议。 只输出预测不做决策（决策归 §3.13#1 NextDayExitDecision…；源码 L270-L306
...
#   desc: 回封生死线决策（§3.11③，v1.8.0）：15 分钟内回封+封单递增=良性；20-30 分钟无法回封=承接崩塌。；源码 L309-L333
...
#   desc: 龙虎榜量化席位双阈值预警（§3.11④，v1.8.0）：hard 70% 降权 30%+预警 / soft 58% 降权 15%。 PIT 注意：本函数只判定占比，龙虎榜数据的…；源码 L336-L360
