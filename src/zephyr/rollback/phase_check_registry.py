...
# [MODULE] zephyr.rollback.phase_check_registry
...
def check_observability_baseline() -> GateResult:
    mod = _PROJECT_ROOT / "src/zephyr/l01_infrastructure/system_telemetry"
    return GateResult.GREEN if mod.is_dir() else GateResult.YELLOW
...
def check_budget_enforcer() -> GateResult:
    try:
        from zephyr.budget_enforcer.budget_engine import BudgetEngine
...
def check_chaos_test() -> GateResult:
    try:
        from zephyr.behavioral_auditor.chaos_injector import ChaosInjection
