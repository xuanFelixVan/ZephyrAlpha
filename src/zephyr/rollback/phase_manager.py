...
# [MODULE] zephyr.rollback.phase_manager
...
class PhaseGate(BaseModel):
    ...
    def run_checks(self, check_fn: Callable[[str], GateResult] | None = None) -> GateResult:
        if check_fn is None:
            from zephyr.rollback.phase_check_registry import run_check as _run_check
            check_fn = _run_check
...
def session_startup(quick: bool = True) -> dict:
    from zephyr.rollback.phase_check_registry import (
