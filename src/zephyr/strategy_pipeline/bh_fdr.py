...
# [INVARIANTS] 纯函数（无 IO）；BH 程序按升序 p×m/k 步进比较（Benjamini-Hochberg 1995 标准式）；
#   q 值语义=FDR 上界控制；零假设族=同一批次内全部待检策略（含已过 §8 门者——选择偏差必须全员入族）
# [MODIFY-GUARD] tests/strategy_pipeline/test_bh_fdr.py
# [STABILITY] experimental
# [SAFETY] L
# [AI_AUTONOMY] ai_modifiable
# [ERROR_CONTRACT] ValueError(空族/q 越界)
...
"""BH-FDR 门——全自动入库管线的假发现率控制（挖矿真源 §1③，q≤0.10 预授权默认）。
...
def bh_filter(p_values: dict[str, float], q: float = 0.10) -> tuple[set[str], dict[str, dict[str, Any]]]:
    """Benjamini-Hochberg FDR 过滤：返回（通过集，逐条报告）。

    标准程序：p 升序排名 k=1..m，通过条件 p(k) <= k/m × q；
    通过集=最大通过 rank 之前的全部条目（步进单调性，非逐条独立判）。
    """
    if not p_values:
