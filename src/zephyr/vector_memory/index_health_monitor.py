...
class IndexHealthMonitor:
    ...
    def snapshot_backup(self, backup_dir: Path | str | None = None) -> Path:
        backup_root = Path(backup_dir) if backup_dir else Path("data/vector_db/_snapshots")
        backup_root.mkdir(parents=True, exist_ok=True)
        timestamp = datetime.now(UTC).strftime("%Y%m%dT%H%M%S")
        snapshot_path = backup_root / f"snapshot_{timestamp}"

        if self._collection_manager.persist_dir.exists():
            shutil.copytree(str(self._collection_manager.persist_dir), str(snapshot_path), dirs_exist_ok=False)
    ...
    def check_ttl_expiry(self) -> list[TTLExpiryReport]:
        from zephyr.vector_memory.collection_manager import TTL_MAP
