...
class ColdPlaneIsolator:
    ...
    def declare_quota(self, quota: ResourceQuota) -> ResourceQuota:
        ...
        if not (0 < quota.memory_gb <= MAX_MEMORY_GB):
            raise ColdPlaneError(
                f"内存配额越界: {quota.memory_gb}GB（合法区间 (0, {MAX_MEMORY_GB}]）"
            )
        if quota.io_priority != REQUIRED_IO_PRIORITY:
            raise ColdPlaneError(
                f"IO 优先级非法: {quota.io_priority!r}（Cold 平面须为 below_normal）"
            )
        if not (0 < quota.ifind_qps <= IFIND_MAX_QPS):
            raise ColdPlaneError(
                f"iFind QPS 越界: {quota.ifind_qps}（合法区间 (0, {IFIND_MAX_QPS}]）"
            )
        ...
        *,
        ...
        if source is Plane.COLD and target is Plane.WARM:
            if not channel.startswith(CHANNEL_PREFIX):
                reason = (
                    f"Cold→Warm 白名单外通道拒绝: {channel!r}"
                    f"（仅允许 {CHANNEL_PREFIX}* 前缀）"
                )
                self._alert(source, target, channel, reason)
                raise ColdPlaneError(reason)
            if poll_interval_s != CHANNEL_POLL_INTERVAL_S:
                raise ColdPlaneError(
                    f"Cold→Warm 轮询间隔非法: {poll_interval_s}s"
                    f"（须为 {CHANNEL_POLL_INTERVAL_S}s）"
                )
    ...
    def acquire_ifind(self) -> None:
        ...
        if self._tokens < 1.0:
            raise ColdPlaneError(
                f"iFind QPS 超限: 令牌耗尽（上限 {self._qps} QPS，Fail-Closed）"
            )
        self._tokens -= 1.0
    ...
    def pending_activations(
        ...
        entries = sorted(
            self._pending.values(), key=lambda e: (e.produced_at, e.artifact_id)
        )
