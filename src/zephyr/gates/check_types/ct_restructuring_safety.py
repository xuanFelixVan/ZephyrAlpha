...
# [MODULE] zephyr.gates.check_types.ct_restructuring_safety
...
参数（通过 check.params 传入）：
  ...
  - old_import_path: str — 旧 import 路径（如 "zephyr.telemetry"）


  - canonical_path: str — 真源路径（如 "zephyr.system_telemetry"）
...
class RestructuringSafetyHandler(CheckTypeHandler):
        ...
        if canonical_path and expected_classes:
            ...
            for cls_name in missing:
                ...
                violations.append({
                    ...
                    "detail": f"GOV-ENG-002 §3.3 #2: 合并后 class 列表必须完整",
    ...
    def _check_import_residual(


        self, old_path: str, project_root: Any
        ...
        pattern_from = re.compile(
            ...
            rf"^\s*from\s+{re.escape(old_path)}\b", re.MULTILINE
        ...
        pattern_import = re.compile(
            ...
            rf"^\s*import\s+{re.escape(old_path)}\b", re.MULTILINE
