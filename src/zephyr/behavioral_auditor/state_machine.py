...
# [MODULE] zephyr.behavioral_auditor.state_machine
...
_VALID_TRANSITIONS: dict[DriftState, set[DriftState]] = {
    ...
    DriftState.DETECTED: {DriftState.TRIAGED, DriftState.ACKNOWLEDGED, DriftState.FALSE_POSITIVE, DriftState.DEAD_LETTER},
...
class DriftStateMachine:
    ...
    def transition(
        ...
        if record.state != from_state:
            ...
            raise InvalidTransitionError(
                ...
                f"State mismatch: expected {from_state.value}, actual {record.state.value}"
    ...
    def auto_transition(self, event_id: uuid.UUID) -> Optional[DriftState]:
    ...
    def trigger_rollback(self, event_id: uuid.UUID, record: DriftEventRecord) -> None:
        ...
        try:
            ...
            rollback_module = importlib.import_module("zephyr.rollback.engine")
    ...
    def get_state(self, event_id: uuid.UUID) -> Optional[DriftState]:
...
class DriftEventRecord:
    ...
    def __init__(
        ...
        self.resolution_detail: Optional[str] = None
        ...
        self.suppressed_until: Optional[datetime] = None
