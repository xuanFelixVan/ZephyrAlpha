...
# [ERROR_CONTRACT]
# [TESTS]
# [A_module] module_id=MOD-UNK-value_factor | layer=module | stability=evolving | safety=L | ai_autonomy=ai_modifiable
# [TTL] permanent
...
# [ALGO_FLOW] external: docs/03_modules/_domain_factor/algo_flow/value_factor.yaml
"""
...
class ValueFactor(FactorBase):
    ...
    def compute(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        if "eps_basic" in data.columns:
            earnings_estimate = data["eps_basic"].fillna(kwargs.get("earnings_per_share", 5.0))
        else:
