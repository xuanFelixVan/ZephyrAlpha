...
# [MODULE] zephyr.l01_infrastructure.system_telemetry.contract_metrics

# [INVARIANTS] non-invasive trace-context-based measurement; statistical drift detection window; SSoT: cross-layer-contracts.yaml
...
# [TESTS] tests/system_telemetry/test_contract_metrics.py

"""
ZephyrAlpha — system_telemetry/contract_metrics.py
...
class ContractMetricsCollector:
    ...
    def detect_contract_drift(
        ...
        from zephyr.behavioral_auditor.contract_drift_detector import detect_contract_drift as _detect
        return _detect(
            contract_id=contract_id,
...
CT_TEL_SLA: dict[str, dict] = {
    "CT-TEL-001": {
        ...
        "physical_path": "src/zephyr/system_telemetry/contract_metrics.py",
    },
    "CT-TEL-002": {
        "name": "TelemetryLogs",
        "sla_p99_us": 5_000_000,
        "description": "日志持久化延迟<5s",
        "physical_path": "src/zephyr/system_telemetry/logs/structured_sink.py",
    },
    "CT-TEL-003": {
        "name": "TelemetryTraces",
        "sla_p99_us": 2_000_000,
        "description": "链路追踪采样率可配置",
        "physical_path": "src/zephyr/system_telemetry/traces/span_stub.py",
    },
    "CT-TEL-004": {
        "name": "TelemetryHealth",
        "sla_p99_us": 30_000_000,
        "description": "健康检查心跳间隔30s",
        "physical_path": "src/zephyr/system_telemetry/health_probes.py",
    },
}
