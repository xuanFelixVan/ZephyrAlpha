...
# [MODULE] zephyr.l01_infrastructure.code_dedup_engine.integration_hub
...
def register_ce_rules() -> int:
    try:
        from zephyr.context_engine.context_rule_registry import ContextRuleRegistry, ContextRule
    except ImportError:
        return 0
    ...
    registry.register(ContextRule(
        ...
        content="影子 API 清单: 见 shadow_api_manifest.yaml; 策略树: config/policy_tree.yaml R001-R005",
        priority=70,
    ...
    registry.register(ContextRule(
        ...
        content="完整去重策略树: config/policy_tree.yaml; 修复流程: extraction_safety→auto_fixer→atomic_fixer→verifier",
        priority=30,
