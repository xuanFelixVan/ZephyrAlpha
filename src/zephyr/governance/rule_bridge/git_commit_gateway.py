...
from zephyr.governance.commit_gates.pure_shim_gate import make_pure_shim_gate
...
def _filter_existing_files_for_commit(gw, abs_files):
    existing: list[str] = []
    for f in abs_files:
        if os.path.isfile(f):
            existing.append(f)
        else:
            rel = os.path.relpath(f, str(gw.project_root)).replace("\\", "/")
            if gw._is_git_tracked(rel) or gw._is_staged_delete(rel):
                existing.append(f)
    return existing


def _evaluate_commit_gate_results(gate_results):
...
def _run_post_commit_reconcile(gw, result, existing, session_id):
    try:
        reconcile_results = gw._reconciliation_registry.reconcile_for(existing, session_id)
...
def _stage_normal_files_for_commit(gw, normal_files):
    add_ok = True
    fail_result = None
    if normal_files:
        ...
        if existing_files:
            add_pathspec_file = gw._write_pathspec_file(existing_files)
            try:
                add_result = gw._run_git(
        ...
        if add_ok and deleted_files:
            del_pathspec = gw._write_pathspec_file(deleted_files)
            try:
                rm_result = gw._run_git(
    ...
    return add_ok, fail_result


def _filter_auto_commit_files(gw, files):
    abs_files = [
        os.path.abspath(f) if os.path.isabs(f) else str(gw.project_root / f)
        for f in files
    ]
    existing: list[str] = []
    for f in abs_files:
        if os.path.isfile(f):
            ...
            rel = os.path.relpath(f, str(gw.project_root)).replace("\\", "/")
            if gw._is_git_tracked(rel):
                existing.append(f)
    return existing


def _check_auto_commit_gate(gw, gate_id, existing, session_id, fail_template, warn_template, allow_promote):
    spec = gw._gate_registry.get(gate_id)
    if spec is not None:
        passed, detail = spec.check(gw, existing, allow_promote=True) if allow_promote else spec.check(gw, existing)
        if not passed:
            return CommitResult(
                status=CommitStatus.NAMING_VIOLATION,
                message=fail_template.format(detail),
            )
    else:
        logger.warning(warn_template, session_id, len(existing))
    return None


def _stage_gitignored_deleted(gw, deleted):
    if deleted:
        del_rels = [
            os.path.relpath(f, str(gw.project_root)).replace("\\", "/")
            for f in deleted
        ]
        del_tracked = [f for f, rel in zip(deleted, del_rels) if gw._is_git_tracked(rel)]
        if del_tracked:
            del_pathspec = gw._write_pathspec_file(del_tracked)
            try:
                r = gw._run_git(
                    ["git", "rm", "--cached", "--ignore-unmatch",
                     f"--pathspec-from-file={del_pathspec}"]
                ...
                try:
                    ...
                    pass
            if r.returncode != 0:
                return f"git rm --cached failed: {r.stderr.strip()}"
    return None


def _stage_gitignored_existing(gw, existing):
    if existing:
        ex_rels = [
            os.path.relpath(f, str(gw.project_root)).replace("\\", "/")
            for f in existing
        ...
        ex_tracked = [
            ...
            if gw._is_git_tracked(rel) and not gw._is_staged_delete(rel)
        ]
        if ex_tracked:
            ex_pathspec = gw._write_pathspec_file(ex_tracked)
            try:
                r = gw._run_git(
                ...
                try:
                    ...
                    pass
            if r.returncode != 0:
                return f"git add -f failed: {r.stderr.strip()}"
    return None
...
class GitCommitGateway:
    ...
    def commit(
        ...
        existing = _filter_existing_files_for_commit(self, abs_files)
        if not existing:
            return CommitResult(
        ...
        gate_failure = _evaluate_commit_gate_results(gate_results)
        ...
        if result.status == CommitStatus.OK:
            _run_post_commit_reconcile(self, result, existing, session_id)
        return result
    ...
    def _stage_gitignored_tracked(
        ...
        del_err = _stage_gitignored_deleted(self, deleted)
        if del_err is not None:
            return False, del_err, normal_files
        ex_err = _stage_gitignored_existing(self, existing)
        if ex_err is not None:
            return False, ex_err, normal_files
    ...
    def _commit_locked(
        ...
        try:
            ...
            if not gi_ok:
                ...
                add_ok, add_fail = _stage_normal_files_for_commit(self, normal_files)
    ...
    def _commit_auto(
        ...
        existing = _filter_auto_commit_files(self, files)
        if not existing:
            return CommitResult(
                status=CommitStatus.NOTHING_TO_COMMIT,
        ...
        fpt_fail = _check_auto_commit_gate(
            self, "FILE-PLACEMENT-TTL", existing, session_id,
            "file placement ttl violation (auto-commit): {}",
            "_commit_auto: FILE-PLACEMENT-TTL gate 未注册，跳过文件放置校验"
            "（session=%s, files=%d）——检查 __init__ 的 gate 注册",
            True,
        )
        if fpt_fail is not None:
            return fpt_fail
