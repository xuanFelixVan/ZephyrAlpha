...
class PathResolver:
    ...
    def _build_index(self):
        ignore = {'__pycache__', '.git', '.pytest_cache', '.mypy_cache', '.ruff_cache',
                   '.tox', 'dist', 'build', 'egg-info', 'node_modules', '.venv', 'venv'}
    ...
    def resolve_module(self, module_name: str) -> list:
        ...
        或模块ID（"MOD-INF-021"）。
        返回所有匹配的目录路径列表，governance/ 和 l01_infrastructure/ 下的优先。
        """
        ...
        # Sort: governance/ and l01_infrastructure/ first (they're the canonical locations)
        def _priority(p):
            rel = os.path.relpath(p, self.src_root)
            if 'governance' in rel or 'l01_infrastructure' in rel:
                return 0
            return 1
        
        return sorted(matches, key=_priority) if matches else []
    
    def resolve_path(self, module_name: str, filename: str) -> Optional[str]:
        """
        给定模块名 + 文件名，返回应该落盘的完整路径。
        模块有多处存在时优先 governance/ 或 l01_infrastructure/。
        """
    ...
    def validate_path(self, expected_path: str, module_hint: str = "") -> PathResolution:
        ...
        exact_elsewhere = [fp for fp in self._file_index.get(key, [])
                          if os.path.normcase(fp) != os.path.normcase(expected_path)]
                ...
                if stripped.startswith('- path:'):
                    ...
                    if pm:
                        ...
                        if resolution.status == "OK":
                            ...
                            corrections.append({
                                'old': expected,
                                'new': resolution.suggested_path,
                                'status': resolution.status
                            })
                        else:
                            all_resolved = False
                            corrections.append({
                                'old': expected, 'new': None, 'status': resolution.status
                            })
...
def reslove_path(module: str, filename: str, project_root: str = None) -> Optional[str]:
    """快捷函数：解析模块路径"""
...
if __name__ == "__main__":
    ...
    for mod in ["rollback", "agent_rbac", "audit_trail", "code_dedup_engine", 
                 "escalation", "budget_enforcer", "drift_detector", "a2a",
                 "pipeline", "context_engine", "feedback_loop", "kb"]:
        dirs = resolver.resolve_module(mod)
        print(f"  {mod:25s} → {[os.path.relpath(d, resolver.project_root) for d in dirs[:2]]}")
    
    print("\nPath validation test:")
    test_paths = [
        r"D:\ZephyrAlpha\src\zephyr\agent_rbac\phase_executor.py",
        r"D:\ZephyrAlpha\src\zephyr\governance\audit_trail\anomaly.py",
    ...
    for tp in test_paths:
        ...
        print(f"    status={r.status}, suggested={os.path.relpath(r.suggested_path, resolver.project_root) if r.suggested_path else 'None'}")
