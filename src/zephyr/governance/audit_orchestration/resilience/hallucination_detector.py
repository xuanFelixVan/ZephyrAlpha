...
# [MODULE] zephyr.governance.audit_orchestration.resilience.hallucination_detector
# [DOMAIN] D_GOV_AUDIT
...
# [ERROR_CONTRACT]
# [TESTS]
# [A_module] module_id=MOD-RES_hallucination_detector | layer=module | stability=evolving | safety=L | ai_autonomy=ai_modifiable
# [TTL] task_bound
...
Task ID      : T-3-07
ADR          : ADR-0039（CoVe 四步 + Sonnet 4.6 × GLM-5.1 双模型交叉）
safety_level : H
Depends      : ADR-0035（Intent 三阶段）、ADR-0039（CoVe 策略）、
               ADR-0040（Pydantic 契约）、T-2-32（ai_behavior_audit_logger）
...
_FROZEN_ASSET_PATTERN = re.compile(
    r"(tool-contracts\.yaml|tool_contracts\.yaml|ADR-\d+-\d+\.md|ADR-\d{4}-[a-z0-9-]+\.md)",
    re.IGNORECASE,
)
_SUSPECT_CITATIONS = [
...
def _frozen_asset_mutation(claim: str, handoff_approved: bool) -> list[str]:
    """frozen 资产（tool-contracts.yaml / ADR-*.md）未经 Handoff 修改建议标红。"""
    if handoff_approved:
        return []
    evidence: list[str] = []
...
class HallucinationDetector:
    ...
    def _ensure_min_questions(self, claim: str, existing: list[str]) -> list[str]:
        ...
        templates = [
            ...
            "此 claim 中引用的文件 / ADR / 任务 ID 是否真实存在？",
        ]
