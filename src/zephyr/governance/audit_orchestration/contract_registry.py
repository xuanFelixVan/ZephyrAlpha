...
CONTRACTS: dict[str, Contract] = {
    ...
    "CT-FLE-DB-001": Contract(
        ...
        producer="Feedback Loop Engine",
        consumer="Database",
        status="部分实现:metrics_collector已sqlite3持久化metrics表(create/record/bulk_record/query/aggregate),未通过正式zephyr.infrastructure.db契约路径",
        ai_read_only_hint=AIReadOnlyHint.CAUTION_STUB,
        trigger="FLE 持久化指标数据",
        ...
        telemetry=TelemetryType.USE,
        telemetry_metrics=["utilization", "saturation", "error"],
        ai_prompt="FLE 指标采集 → db 持久化 → 历史分析(metrics_collector已有sqlite3实现,需迁移到zephyr.infrastructure.db正式契约)",
