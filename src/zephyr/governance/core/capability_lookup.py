...
# [MODULE] zephyr.governance.core.capability_lookup
# [DOMAIN] D_GOVERNANCE
...
设计原则（第一性原理 / 向内收）：
  ...
  3. 自动维护：CapabilityLookup() 初始化时自动扫磁盘头部 + 派生 canonical/duplicates，
     removed_duplicates 惰性派生（首次查询时触发 git log + git show，避免 init 超时）。
     canonical 文件迁移时自动跟随（basename 启发式 + 成熟度排序）。
...
class CapabilityLookup:
    ...
    def _derive_all(self) -> None:
        ...
        git show ≈ 8s）。manual 条目始终加载（真源数据，不依赖 git）。
        """
    ...
    def _ensure_removed_derived(self) -> None:
        ...
        __init__ 不调用 git log/git show（避免 10s 超时），
        首次 _entry_to_dict() 调用时触发全量派生（git log 1.5s + 73 次 git show ≈ 8s），
        后续查询命中 _removed_derived 标志直接返回。
        """
    ...
    def _git_show_header(self, commit: str, path: str) -> HeaderInfo | None:
        ...
        缓存：同一 (commit, path) 仅 subprocess 一次，多能力匹配同一删除文件时复用。
        """
