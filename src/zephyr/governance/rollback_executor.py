# [A_module] module_id=MOD-RES_rollback_executor | layer=module | stability=evolving | safety=L | ai_autonomy=ai_modifiable
...
# 方案C：并发安全守卫——回滚前检测 .ailocks/ 文件锁冲突，防止覆盖其他 session 工作
...
class RollbackExecutor:
    ...
    def discard_changes(
        ...
        # === 并发安全守卫（方案C）：检测 discard 文件是否被其他 session 锁定 ===
        ...
        if conflict.has_conflict:
            files_blocked = conflict.blocked_files
            audit_record = self._build_discard_audit(
                decision=DiscardDecision.BLOCKED_BY_OWNER,
                files=file_list,
                blocked=files_blocked,
                reason=f"Files locked by other session: {conflict.locked_by}",
                audit_session=audit_session,
            )
            return DiscardResult(
                success=False,
