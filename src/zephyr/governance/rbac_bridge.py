...
def __getattr__(name):
    """延迟导入避免循环依赖."""
    if name == "EscalationRBACBridge":
        from zephyr.governance.governance.rbac_bridge import EscalationRBACBridge

        return EscalationRBACBridge
    if name == "RBACCheckResult":
        from zephyr.governance.governance.rbac_bridge import RBACCheckResult
