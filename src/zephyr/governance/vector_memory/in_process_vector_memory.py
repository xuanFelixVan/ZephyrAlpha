...
# [MODULE] zephyr.data.knowledge_management.vector_memory.in_process_vector_memory
# [DOMAIN] D-KNOWLEDGE
# [DEPENDENCIES] zephyr.governance.__init__; zephyr.integration.local_model.embedding_router; zephyr.integration.local_model.cache_layer
# [CONSUMERS]
# [STARTUP] imported
# [MATURITY] prototype
# [INVARIANTS] none
...
# [ERROR_CONTRACT]
# [TESTS]
# [A_module] module_id=MOD-DAT_in_process_vector_memory | layer=module | stability=evolving | safety=L | ai_autonomy=ai_modifiable
# [TTL] task_bound
...
from zephyr.governance.vector_memory.collection_manager import (
    COLLECTION_NAMES,
    VMS_PERSIST_DIR,
    CollectionInfo,
    CollectionManager,
...
_VMS_CONFIG_PATH = Path(__file__).resolve().parent / "vms-config.yaml"


def _load_vms_config() -> dict:
    if not _VMS_CONFIG_PATH.exists():
        return {}
    with open(_VMS_CONFIG_PATH, encoding="utf-8") as f:
        return yaml.safe_load(f).get("vms", {})


def _is_auto_backup_enabled() -> bool:
    config = _load_vms_config()
    return bool(config.get("backup", {}).get("auto_backup_enabled", False))
...
class InProcessVectorMemory:
    ...
    def __init__(self, persist_dir: Path | str | None = None) -> None:
        ...
        self._chunk_strategy_router: ChunkStrategyRouter = self._init_chunk_router()
        self._hybrid_retriever: HybridRetriever | None = None
        self._provenance_enforcer = self._init_provenance_enforcer()
        self._index_health_monitor: IndexHealthMonitor | None = None
    ...
    def start(self) -> None:
        ...
        from zephyr.governance.vector_memory.bridge_layer import BridgeLayer
    ...
    def _maintenance_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            if now - last_daily_ts >= DAILY_INTERVAL:
                ...
                try:
                    self._index_health_monitor.cleanup_snapshots()
                except Exception:
                    _logger.debug("VMS maintenance: cleanup_snapshots failed", exc_info=True)
