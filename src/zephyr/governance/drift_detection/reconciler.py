# [A_module] module_id=MOD-GOV_reconciler | layer=module | stability=evolving | safety=L | ai_autonomy=ai_modifiable
...
class AutoFixer:
        ...
        for fp in affected_files:
            if os.path.exists(fp):
                try:
                    ...
                    snapshot.checksums[fp] = hashlib.sha256(content).hexdigest()
                    snapshot.mtimes[fp] = os.path.getmtime(fp)
                except OSError:
                    pass
    ...
    def verify_rollback(self, event: DriftEvent) -> bool:
        ...
        if not snapshot:
            return True
        for fp, expected_hash in snapshot.checksums.items():
