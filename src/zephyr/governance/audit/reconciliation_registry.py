...
def make_capability_lookup_health_reconciler(gateway: "object") -> ReconcilerSpec:
    ...
    # DEPRECATED（SSoT 漂移修复，2026-07-22）: _is_exempt_bypass 已委托给
    # capability_lookup_bypass_policy.is_exempt_reason（16 关键词 + _→- 归一化）。
    # 此常量保留仅向后兼容，不再被 _is_exempt_bypass 使用。MUST NOT 新增依赖。
    ...
    def _is_exempt_bypass(reason: str) -> bool:
        ...
        from zephyr.gov_enforcement.commit_gates.capability_lookup_bypass_policy import is_exempt_reason

        return is_exempt_reason(reason)
