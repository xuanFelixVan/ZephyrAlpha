...
# 层: 算法
# - id: A1
#   name_zh: ① HookRegistry
#   name_en: HookRegistry
#   intro: 声明式事件钩子注册表（内部事件总线）。
#   desc: 声明式事件钩子注册表（内部事件总线）。 线程安全：所有操作通过 _lock 保护。 回调异常会记录日志，不会传播——一个钩子的崩溃不影响其他。；公共方法（定义序）: active, hooks, register, u…
#   inputs: 无参数
#   outputs: 返回值
