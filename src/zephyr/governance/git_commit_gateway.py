...
class GitCommitGateway:
    ...
    def _register_default_reconcilers(self) -> None:
        ...
        P2-T7: domain_doc 重生（commit depgraph 后自动重生域 .md/.mmd 制品，治手工生成漂移）。
        ...
        P3收尾: rule_catalog 同步（GATE-RULE-CATALOG，commit rules/ 下文件后自动重新生成 rule_catalog_registry.yaml，治 catalog stale 导致 depgraph 数据污染）。
        """
    ...
    def _check_working_docs_completes_when(self, files: list[str]) -> tuple[bool, str]:
        ...
        规则真源：AGENTS.md §7（docs/_working/ completes_when 规则）+ trae_028 §归档与废弃流程。
        本方法是该规则的可执行实现——改规则先改 AGENTS.md/trae_028，再同步本方法。
