...
# [ALGO_FLOW] external: docs/03_modules/_domain_governance/algo_flow/engine/pipeline_base.yaml
"""
...
class ExperimentPipelineBase(abc.ABC):
    ...
    def compute_effect_size(control: float, treatment: float, pooled_std: float) -> float:
        ...
        if (
            abs(pooled_std) < 1e-9
        ):  # 5.167 修复: 浮点==0精确比较改 < epsilon（对标 simulation/pipeline_base.py 5.167.6）
            return 0.0
