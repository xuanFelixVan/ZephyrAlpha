...
# [MODULE] zephyr.governance.engine.pipeline_base
# [DOMAIN] D_GOVERNANCE
...
# [ERROR_CONTRACT]
# [TESTS]
# [A_module] module_id=MOD-DAT-pipeline_base | layer=module | stability=evolving | safety=L | ai_autonomy=ai_modifiable
# [TTL] permanent
...
class ExperimentPipelineBase(abc.ABC):
    ...
    def compute_effect_size(control: float, treatment: float, pooled_std: float) -> float:
        """Cohen's d 效应量计算"""
        if abs(pooled_std) < 1e-9:  # 5.167 修复: 浮点==0精确比较改 < epsilon（对标 simulation/pipeline_base.py 5.167.6）
            return 0.0
