                ...
                if not gate_module:
                    ...
                    try:
                        ...
                        if not candidates:
                            ...
                            violations.append({"message": f"FLE module {gate_module} has no usable class", "severity": check.severity})
                            ...
                            if method is None:
                                ...
                                violations.append({"message": f"FLE {gate_module} has no {gate_method} method", "severity": check.severity})
                                ...
                                if isinstance(result, dict):
                                    ...
                                    if not result.get("allowed", result.get("passed", result.get("ok", True))):
                                        ...
                                        violations.append({"message": f"FLE {gate_module}.{gate_method} rejected", "severity": check.severity, "detail": str(result)})


                                elif isinstance(result, bool) and not result:


                                    violations.append({"message": f"FLE {gate_module}.{gate_method} returned False", "severity": check.severity})
