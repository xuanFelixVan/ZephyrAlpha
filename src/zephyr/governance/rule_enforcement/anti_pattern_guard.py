...
# [MODULE] zephyr.governance.rule_enforcement.anti_pattern_guard
...
class AntiPatternGuard:
    ...
    def check_ap1_bypass_contract(self, target_system: str, has_ct_contract: bool, agent_id: str = "") -> bool:
        if not has_ct_contract:
            self._violations.append(APViolation(
                ap_id="AP1",
    ...
    def check_ap4_code_over_document(self, tier0_value: object, code_value: object,
                                      fact_id: str = "", agent_id: str = "") -> bool:
    ...
    def check_ap5_modify_upstream(self, is_upstream_modification: bool,
                                   has_finding: bool, agent_id: str = "") -> bool:
    ...
    def check_ap6_shared_mutable_state(self, share_path: str, is_ct_path: bool,
                                        agent_id: str = "") -> bool:
        if not is_ct_path:
            self._violations.append(APViolation(
                ap_id="AP6",
    ...
    def check_ap8_session_orphan_tasks(self, orphan_count: int, session_id: str = "",
                                        agent_id: str = "") -> bool:
        if orphan_count > 0:
            self._violations.append(APViolation(
                ap_id="AP8",
