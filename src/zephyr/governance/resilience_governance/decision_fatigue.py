# [BLUEPRINT] MOD-INF-022 | docs/03_modules/_domain_autonomy_perm/escalation_protocol/blueprint.md
# [MODULE] zephyr.governance.resilience_governance.decision_fatigue
# [DOMAIN] D_GOV_OPS_RESILIENCE
# [DEPENDENCIES]
# [CONSUMERS] MOD-INF-027;MOD-INF-020;MOD-INF-018
# [STARTUP] imported
# [MATURITY] production
# [INVARIANTS] 升级裁决;四级约束;Kill Switch
# [MODIFY-GUARD] docs/03_modules/_domain-autonomy_perm/escalation-protocol/blueprint.md;src/zephyr/escalation-engine/__init__.py
# [STABILITY] evolving
# [SAFETY] H
# [AI_AUTONOMY] human_gated
# [ERROR_CONTRACT] EscalationError;TimeoutError
# [TESTS] tests/decision/test_decision_fatigue.py; tests/e/test_e_decision_fatigue.py; tests/governance/integration/test_bandwidth_optimizer.py
# [A_module] module_id=MOD-INF-022 | layer=module | stability=evolving | safety=L | ai_autonomy=ai_modifiable
# [TTL] permanent

"""


# [ALGO_FLOW] external: docs/03_modules/_domain_governance/algo_flow/resilience_governance/decision_fatigue.yaml
"""

from __future__ import annotations

from enum import Enum

from pydantic import BaseModel


class EisenhowerPriority(str, Enum):
    P0_DO_NOW = "P0"
    P1_SCHEDULE = "P1"
    P2_DELEGATE = "P2"
    P3_ELIMINATE = "P3"


class TaskTriage(BaseModel):
    task_id: str
    description: str
    urgent: bool = False
    important: bool = False
    priority: EisenhowerPriority = EisenhowerPriority.P3_ELIMINATE

    def classify(self) -> EisenhowerPriority:
        if self.urgent and self.important:
            self.priority = EisenhowerPriority.P0_DO_NOW
        elif self.important and not self.urgent:
            self.priority = EisenhowerPriority.P1_SCHEDULE
        elif self.urgent and not self.important:
            self.priority = EisenhowerPriority.P2_DELEGATE
        else:
            self.priority = EisenhowerPriority.P3_ELIMINATE
        return self.priority


def triage(tasks: list[TaskTriage]) -> dict[EisenhowerPriority, list[TaskTriage]]:
    result: dict[EisenhowerPriority, list[TaskTriage]] = {p: [] for p in EisenhowerPriority}
    for t in tasks:
        t.classify()
        result[t.priority].append(t)
    return result


def filter_priority(tasks: list[TaskTriage], level: EisenhowerPriority) -> list[TaskTriage]:
    return [t for t in tasks if t.priority == level]
