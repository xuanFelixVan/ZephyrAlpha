...
SSoT: cross-layer-contracts.yaml → CTR-004 + CTR-005 + CTR-006 + CTR-P1-007
"""
...
class ExecutionEngine:
    ...
    def execute_order(
        ...
        if halt_violations:
            _logger.warning("Order rejected by risk: order_id=%s violations=%s",
                            order.order_id, len(halt_violations))
    ...
    def get_engine_run_record(self, order_id: str) -> Optional[ExecutionEngineRunRecord]:
...
__all__ = ["ExecutionEngine", "ExecutionEngineRunRecord", "ExecutionConfig", "AlgoType"]
