...
#   code: profiler.py 顶层公共函数形参（AST 提取）
# - id: I2
#   name: timeout_per_case_s 参数
#   fields: 参数 timeout_per_case_s（无注解）
#   code: profiler.py 顶层公共函数形参（AST 提取）
# - id: I3
#   name: max_ollama_models 参数
#   fields: 参数 max_ollama_models（无注解）
#   code: profiler.py 顶层公共函数形参（AST 提取）
# 层: 算法
# - id: A1
#   name_zh: ① ModelProfiler
#   name_en: ModelProfiler
#   intro: 核心性能分析引擎——对每个模型运行全维度 benchmark。
#   desc: 核心性能分析引擎——对每个模型运行全维度 benchmark。；公共方法（定义序）: url, timeout, max_models, should_skip_model, score_output, percent…
#   inputs: ollama_url timeout_per_case_s max_ollama_models
#   outputs: 返回值
...
class ModelProfiler:
    ...
    def _populate_profile_stats(
        ...
        _set_latency_percentiles(profile, all_latencies, self._percentile)

        profile.throughput_tokens_per_sec = (total_tokens / total_time * 1000) if total_time > 0 else 0.0

        _set_category_scores(profile, case_results)
        _set_hallucination_rate(profile, case_results)
        profile.refusal_rate = _compute_refusal_rate(case_results)
...
def _set_latency_percentiles(
    ...
    if all_latencies:
        ...
        profile.latency_p50_ms = percentile(sorted_lat, 0.50)
        profile.latency_p95_ms = percentile(sorted_lat, 0.95)
        profile.latency_p99_ms = percentile(sorted_lat, 0.99)
