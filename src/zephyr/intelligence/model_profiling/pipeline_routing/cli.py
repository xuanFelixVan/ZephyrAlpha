...
# [ERROR_CONTRACT]
# [TESTS]
# [A_module] module_id=MOD-INF-034-cli | layer=module | stability=evolving | safety=L | ai_autonomy=ai_modifiable
# [TTL] permanent
...
用法
----
    python -m zephyr.intelligence.model_profiling.pipeline_routing.cli discover     # 列出所有可用模型
    python -m zephyr.intelligence.model_profiling.pipeline_routing.cli quick qwen3:8b  # 快速测试单个模型
    python -m zephyr.intelligence.model_profiling.pipeline_routing.cli benchmark    # 全量 benchmark
    python -m zephyr.intelligence.model_profiling.pipeline_routing.cli drift qwen3:8b   # 漂移检测
    python -m zephyr.intelligence.model_profiling.pipeline_routing.cli best         # 显示最佳模型
    python -m zephyr.intelligence.model_profiling.pipeline_routing.cli history      # 查看历史 benchmark 记录
"""
...
def main() -> None:
    ...
    if cmd == "discover":
        ...
        if len(args) < 2:
            print("用法: python -m zephyr.intelligence.model_profiling.pipeline_routing.cli quick <model_name>")
            return
        cmd_quick(args[1])
    elif cmd == "benchmark":
        cmd_benchmark()
        ...
        if len(args) < 2:
            print("用法: python -m zephyr.intelligence.model_profiling.pipeline_routing.cli drift <model_name>")
            return
        cmd_drift(args[1])
    elif cmd == "history":
        cmd_history()
