...
# [ALGO_FLOW] external: docs/03_modules/_domain_gov_enforcement/algo_flow/rule_bridge/session_worktree.yaml
"""
...
def _wt_block_gate_id(result: dict) -> str:
    ...
    判定链（红蓝 v3 P1-2 治本 2026-09-14）：gate_results[0].gate_id 直取优先——
    pre-commit gate 阻断等路径的 result 自带精确归因，其 message 为多 gate 拼接的
    "pre-commit gate 阻断（…）: XXX: …" 格式，正则恒失配落 UNKNOWN（实证 11/13）；
    直取失败（无 gate_results/条目无 gate_id）才回退原判定链：标志字段 > message 正则。
    """
    _gate_results = result.get("gate_results")
    if _gate_results:
        _first = _gate_results[0] if isinstance(_gate_results, list) else None
