...
# [ALGO_FLOW] external: docs/03_modules/_domain_gov_enforcement/algo_flow/commit_gates/o/open_without_with_gate.yaml
"""
...
def make_open_without_with_gate() -> GateSpec:
    ...
    def _check(gateway, files: list[str], **kwargs) -> tuple[bool, str]:
        ...
        if own_scope is None:
            ...
            if foreign_staged:
                _audit_foreign_staged(gateway, session_id, foreign_staged, gate_name="OPEN-WITHOUT-WITH")
                logger.warning(
                    "OPEN-WITHOUT-WITH: %d 个外来 session staged 文件未检查（warn+审计，不阻断）: %s",
                    len(foreign_staged),
