...
升硬口径注记（66 号备忘 §10 P1 + §12 #2，2026-08-29 联动修订——只改注释不改逻辑）
--------------------------------------------------------------------------------
提交队列 MVP 已验收生产在跑，会话提交=入队（会话不再直接 commit），"软门禁阈值
可调 5→3"的 THRESHOLD 概念废除。本 gate 适用对象收敛为 **Serializer 专用 worktree
之外的会话直接 git 操作**——并发非 worktree commit 仍 fail-closed 阻断不变
（下文分级阻断逻辑/双逃生通道/fail-open 降级全部维持原逻辑）。
...
# [ALGO_FLOW] external: docs/03_modules/_domain_gov_enforcement/algo_flow/commit_gates/w/worktree_required_gate.yaml
"""
...
def make_worktree_required_gate() -> GateSpec:
    ...
    def _check(gateway, files: list[str], **kwargs) -> tuple[bool, str]:
        ...
        # reconcile_runner._count_inflight_workers 同源）。worker 是 commit 的下游产物——
