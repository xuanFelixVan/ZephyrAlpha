...
# [STARTUP] imported
# [MATURITY] prototype
# [INVARIANTS] 只检测 staged 文件中**新增的** 裁定#NNN 引用（不阻断已有的悬空引用，防阻塞大量历史文件）；fail-closed——registry 缺失或 git 异常时阻断；跳过 tests/ 豁免区；扫描文件类型 .py/.yaml/.yml/.md；正则支持纯数字/带字母后缀（裁定#6 / 裁定#19-A / 裁定#203-B 等）；ruling_id 从工作区 ruling_registry.yaml 提取（commit 后的新真源）；L1 编号空洞检测（RULING_GAP_WARNING）——WARNING 不阻断；L2 同提交原子性门禁（RULING_ATOMICITY_VIOLATION）——新引用不在 HEAD registry 时要求 registry 同 commit，否则**阶段2 硬阻断**；L2 非 git 仓库（如测试 tmp_path）跳过检测返回 None；**阶段1 manual stage**（裁定#20-B，2026-07-18）——所有违规返回 passed=True + WARNING detail 不阻断，建立基线；阶段2 移除 _MANUAL_STAGE 标记后 hard block
# [MODIFY-GUARD] gate_id="RULING-REFERENCE"；check 闭包签名 (gateway, files, **kwargs) -> tuple[bool, str]；_MANUAL_STAGE=True 时只 warn 不阻断，阶段2 设 False 启用 hard block；priority=74（紧跟 DANGLING-REFERENCE(70) + NOQA-VALIDATION(71) 之后，ARCH-REFERENCE(75) 之前——同属"引用完整性"类检查，集中执行）
# [STABILITY] evolving
# [SAFETY] L
# [AI_AUTONOMY] ai_modifiable
# [ERROR_CONTRACT] check 永不抛异常——registry 读取异常/git 异常降级为 fail-closed 阻断（passed=False，detail 含修复指引）；_MANUAL_STAGE=True 时违规不阻断
