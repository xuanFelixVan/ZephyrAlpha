...
def make_unsafe_dict_spread_gate() -> GateSpec:
    ...
    def _check(gateway, files: list[str], **kwargs) -> tuple[bool, str]:
        staged_all = _get_staged_py_files(gateway)
        if not staged_all:
            return True, ""
        ...
        if own_scope is None:
            ...
            if foreign_staged:
                _audit_foreign_staged(gateway, session_id, foreign_staged, gate_name="UNSAFE-DICT-SPREAD")
                logger.warning(
                    "UNSAFE-DICT-SPREAD: %d 个外来 session staged 文件未检查（warn+审计，不阻断）: %s",
                    len(foreign_staged),
