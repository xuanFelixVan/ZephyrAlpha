...
# [STARTUP] imported
# [MATURITY] production
# [INVARIANTS] 硬阻断——staged .py added 行含 DOMAIN_NAME_ZH 字典直接访问(.get(/[/pop/items/keys/values)时阻断 commit(passed=False)；定义文件 domain_name_mapping.py 豁免(SSoT 定义位置)；tests/ 豁免；import/注释/docstring 行豁免；git diff 不可达 fail-open(logger.warning)；检出违规则 fail-closed(passed=False)；own 化 2026-09-23(st-gslim P2)：扫描范围=全暂存∩本 session，外来 staged warn+审计不阻断(_split_own_foreign)
...
# [ALGO_FLOW] external: docs/03_modules/_domain_gov_enforcement/algo_flow/commit_gates/d/domain_name_zh_direct_access_gate.yaml
"""
...
def make_domain_name_zh_direct_access_gate() -> GateSpec:
    ...
    def _check(gateway, files: list[str], **kwargs) -> tuple[bool, str]:
        ...
        py_files = _split_own_foreign(gateway, py_files, files, kwargs.get("session_id"), gate_name="NO-DOMAIN-NAME-ZH")[0]
        if not py_files:
            return True, ""
        if not py_files:
            return True, ""
