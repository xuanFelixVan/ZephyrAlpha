...
# [ALGO_FLOW] external: docs/03_modules/_domain_gov_enforcement/algo_flow/commit_gates/m/msg_exposure_gate.yaml
"""
...
# 原 _check 151 行 McCabe=33（preamble + for 循环内 new/modified 双分支 + 格式化）。
...
def _scan_modified_file(gateway, rel_path: str, abs_path: str, content: str) -> list[str]:
    ...
    result: list[str] = []
    for lineno, exc_name, hits in _detect_msg_exposure(tree):
        if lineno not in added_line_nos:
            continue
        if _NOQA_MARKER in added_content_map.get(lineno, ""):
            continue
        result.append(_format_violation_entry(rel_path, lineno, exc_name, hits, " (modified)"))
    return result


def _format_msg_exposure_violations(violations: list[str]) -> tuple[bool, str]:
...
def make_msg_exposure_gate() -> GateSpec:
    ...
    def _check(gateway, files: list[str], **kwargs) -> tuple[bool, str]:
        ...
        if violations_all:
            return _format_msg_exposure_violations(violations_all)
        return True, ""
