...
# [STARTUP] imported
# [MATURITY] production
# [INVARIANTS] 硬阻断——staged 新增 .py/.yaml 文件含 # [DERIVES_FROM] <path> 声明但 <path> 源文件不存在时阻断 commit；tests/ 豁免；git diff 不可达 fail-open；文件读取失败 fail-open；检出违规则 fail-closed（passed=False）；own 化 2026-09-23(st-gslim P2)：扫描范围=全暂存∩本 session，外来 staged warn+审计不阻断(_split_own_foreign)
...
# [ALGO_FLOW] external: docs/03_modules/_domain_gov_enforcement/algo_flow/commit_gates/d/derivation_annotation_gate.yaml
"""
...
def make_derivation_annotation_gate() -> GateSpec:
    ...
    def _check(gateway, files: list[str], **kwargs) -> tuple[bool, str]:
        ...
        new_files = _split_own_foreign(gateway, new_files, files, kwargs.get("session_id"), gate_name="DERIVATION-ANNOTATION")[0]
        if not new_files:
            return True, ""
        if not new_files:
            return True, ""
