...
# [ALGO_FLOW] external: docs/03_modules/_domain_pf_alloc/algo_flow/multi_strategy_capital_allocator.yaml
"""
...
class AllocationConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.max_rebalance_per_day < 1:
            raise InvalidAllocationInputError(f"max_rebalance_per_day must be >= 1, got {self.max_rebalance_per_day}")
        if self.total_risk_budget <= 0:
            raise InvalidAllocationInputError(f"total_risk_budget must be positive, got {self.total_risk_budget}")
...
class MultiStrategyCapitalAllocator:
    ...
    def allocate(
        ...
        cold_start_active = cold_start_days_elapsed is not None and cold_start_days_elapsed < cfg.cold_start_days
    ...
    def _validate(
        ...
        if cold_start_days_elapsed is not None and cold_start_days_elapsed < 0:
            raise InvalidAllocationInputError(f"cold_start_days_elapsed must be >= 0, got {cold_start_days_elapsed}")
