...
├── A股_分时数据/           # 分钟级行情数据 (5分钟、15分钟、30分钟、60分钟)
├── A股数据/               # 原始数据（可能为备份或未分类数据）
├── A股数据_zip/           # 压缩的行情数据文件
│   ├── daily.zip         # 日线数据（未复权）
...
**ZIP文件结构**:
- `daily.zip`: 日线未复权数据
- `daily_qfq.zip`: 日线前复权数据  
- `daily_hfq.zip`: 日线后复权数据
- `weekly.zip`: 周线数据
- `monthly.zip`: 月线数据
...
| **数据清洗** | `pandas`, `numpy` | 强大的数据操作能力 |
| **数据存储** | **SQLite** (元数据) + **Parquet** (行情数据) | 查询效率+存储压缩 |
| **任务调度** | Python `multiprocessing` | 简单易用，适合个人开发 |
| **监控日志** | Python `logging` + 进度条 | 实时反馈，易于调试 |
...
class FinancialDataReader:
    ...
    def read_quarterly_data(self):
        ...
        for file_path in quarter_files:
            # 从文件名提取季度信息
            quarter_str = file_path.stem.split('_')[-1]  # 如 20250930
...
| 字段 | 清洗规则 | 异常处理 |
|------|----------|----------|
| **date** | 转换为datetime格式，验证日期有效性 | 无效日期标记为NaT，可插值 |
| **code** | 标准化为`交易所.代码`格式 (如`SZ.000002`) | 无效代码删除整行 |
| **price** | 价格>0，开盘≤最高≥最低≤收盘 | 价格异常标记并插值 |
| **volume** | 成交量≥0 | 负值设为0 |
| **amount** | 成交额≥0，与价格*volume一致 | 不一致时重新计算 |
| **pct_chg** | 涨跌幅在[-10%, 10%]合理范围 | 超出范围使用前值填充 |

#### 4.1.2 财务数据清洗规则
| 字段类型 | 清洗规则 | 说明 |
|----------|----------|------|
| **代码字段** | 统一为6位数字代码，补前导0 | 便于关联行情数据 |
| **数值字段** | 替换"NaN"、"-"、空值为np.nan | 保持数据一致性 |
...
class DataClassifier:
    ...
    def classify_by_content(self, df):
        ...
        # 财务数据特征列
        financial_keywords = ['资产', '负债', '利润', '收入', '现金流', '比率']
...
```
存储架构:
├── SQLite数据库 (轻量级，用于元数据和查询)
│   ├── metadata.db    # 元数据表
│   ├── stock_info.db  # 股票基本信息
│   └── index_info.db  # 指数信息
├── Parquet文件存储 (高性能，用于大量行情数据)
│   ├── daily/        # 日线数据
│   ├── weekly/       # 周线数据
│   ├── monthly/      # 月线数据
│   └── intraday/     # 分钟数据
...
# config.yaml
data_pipeline:
  source_dir: "D:/ZephyrAlpha/A股数据/量化交易数据"
  ...
  cleaning:
    ...
    volume_threshold: 1e9    # 成交量异常阈值
    pct_chg_limit: 0.2       # 涨跌幅限制
...
### 9.3 处理速度优化
1. **并行处理**: 多进程处理不同股票或时间段
2. **向量化操作**: 使用pandas向量化函数，避免循环
...
1. **全面性**: 覆盖行情数据、财务数据、分钟数据等多种数据类型
2. **实用性**: 提供即用的代码示例和配置模板
3. **可扩展性**: 模块化设计，易于扩展新的数据类型
4. **高效性**: 采用混合存储策略，平衡查询效率和存储成本
5. **健壮性**: 包含错误处理、检查点、备份恢复机制
