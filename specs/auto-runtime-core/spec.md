...
| 元数据 | `_index.yaml` | 蓝图索引 |
| 元数据 | `_schema.yaml` | 蓝图 Schema |
| 元数据 | `SCOPE.yaml` | 项目范围 |
| 元数据 | `technology-landscape.yaml` | 技术全景 |
...
4. CircadianScheduler 定时触发
   ...
   → 例如 00:00 注入 "daily_dream_cycle" DAG
```
...
├── health_monitor.py              # 健康监控 + 自愈（含水平触发调和）
...
└── work_dags/                     # DAG 定义（新增）
    ├── daily_dream_cycle.yaml
    ├── daily_health_check.yaml
    ├── daily_code_dedup.yaml
    ├── daily_kb_maintenance.yaml
    ├── daily_compliance.yaml
    ├── daily_feedback_loop.yaml
    ├── pipeline_full_run.yaml
    ├── kb_ingest_pipeline.yaml
    ├── security_scan.yaml
    └── model_drift_check.yaml
```
