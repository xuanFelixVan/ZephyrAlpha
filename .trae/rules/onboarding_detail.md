...
**session_id 格式**: `session-YYYYMMDD-NNN`。从 `session-logs/` 目录找到对应编号。
...
**核心**: `scaffold.py` 是唯一创建入口。绕过它创建的文件 = 孤儿——无注册，下一个 AI session 看不见。

**scaffold.py 自动完成**: 查重（文件名冲突 + 功能重复检测）→ 创建（temp-file + 原子 rename）→ 注册（__init__.py / script_manifest.yaml / _registry.yaml）→ 返回路径+导入命令。
...
STEP 2: 注册表精确匹配 → registry-of-registries.yaml → 对应 REG-* → 对照
STEP 3: 复用决策（四选一）:
  - 完全覆盖 → 直接用
  - 80% 覆盖 → 扩展已有
  - 50% 覆盖 → 重构已有 + 扩展
...
**后果量化**: 项目已有 388 脚本 + 20 门禁 + 41 模块 + 7 MCP 服务器。不搜索就新建 = 在 388 个脚本里造第 389 个重复。
...
**建卡方式**: 唯一入口 — `TaskRepository.create()` 写入 SQLite（`data/zalpha_metadata.db`）。`.md` 仅伴读副本，禁止作为创建入口。规则来源：task-system/blueprint.md RULE-ZERO-TASK。
...
```
STEP 1  — 读 docs/registry-of-registries.yaml → 了解全项目 29 个注册表
STEP 1.1 — 读 docs/03_modules/template-registry.yaml → 了解可用模板（蓝图/任务卡/依赖图/策略/标准等）
STEP 1.2 — 读 project-path-tree.yaml → 项目现在长什么样（物理路径树快照）
...
STEP 4.6 — Skill 发现: python -m zephyr.agent_spec list（12 个已注册 Skill）
...
| 按领域施工 | `python -m zephyr.agent_spec list` → 匹配 → `progressive_load(skill_id)` | 未加载 Skill → 盲目施工 |
...
| **新建/修改代码文件** | 添加防幻觉头部：`[BLUEPRINT]`/`[MODULE]`/`[INVARIANTS]`/`[MODIFY-GUARD]`/`[CONSUMERS]`/`[STABILITY]`/`[SAFETY]`/`[AI_AUTONOMY]`/`[ERROR_CONTRACT]`/`[TESTS]` 十字段 | 缺失 = 孤儿文件 → 禁止关闭任务 |
| **规格化蓝图** | 先 Layer 1（蓝图+施工图模板 v4.0 合规）→ 后 Layer 2（规格化砍削） | Layer 1 不通过 → 禁止砍削 |
| **规格化代码文件** | STEP 5.5：检查文件头部十字段完整性 | 缺失 → 必须补充（规格化的"加"方向） |
...
- 原子事务：关联修改同一批完成
- 脚本入库：新建 .py 立即注册到 script_manifest.yaml
- AI 受众优先：输出格式优先让 AI 零歧义消费

### 根源分析（MTH-006 触发）

**触发条件**：遇到任何 bug / 失败 / 漂移 / 异常
...
**详见**：[governance-methodology-standard.md MTH-006](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/meta/governance-methodology-standard.md)
...
| 蓝图正文 | ≤3000/章 | 模板强约束 | blueprint-construction-template.md（蓝图+施工图模板） |
| 施工指导 | ≤2000 | 步骤式。禁止"为什么"段落 | onboarding_detail.md |
| TaskCard | ≤500 | Pydantic 模型直写 SQLite | `Task` model in schemas.py |
...
> **完整方法论** → [GOV-DOC-011 compression-workflow-standard.md](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/governance/document/compression-workflow-standard.md)（触发条件 / 优化前三步 / 分类型策略 / 安全原则 / 优化后步骤）。
...
| 可解析引用 | 项目术语 ADR/KB/TaskCard/Gate 后必须有路径 |
...
| 3 | `agent_spec/AGENTS.md` | 保留 L0/L1/L2/L3 架构，压缩拓扑树，保留 Session 恢复 |
| 3 | `governance-methodology-standard.md` | 保留 frontmatter+总则+决策链+13 原则+框架映射表 |
| 4 | `code-construction-standards.md` | 保留 frontmatter+命名表+文件组织+类型层级+导入+SSoT 守卫 |
| 4 | `quality-standard.md` | 保留 frontmatter+8 维度+6 点验收+3 模板+禁止清单+41 条提纲 |
| 5 | `vibe_coding/` (4 个文件) | 逐文件处理，session-state-runbook 完整保留 |
| 6 | `agent_spec/skills/domain/` (13 个) | 保留 CRITICAL Rules+Core Ops+Constraints+Error Patterns+Checklist+Constants+References |
| 7 | `agent_spec/skills/role/` (3 个) | 保留 CRITICAL Rules+Workflow+References |
...
| 内容 | 位置 | Token |
|------|------|:---:|
| 治理方法论 13 原则 (MTH-001~013) | [`governance-methodology-standard.md`](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/meta/governance-methodology-standard.md) | ~3500 |
| 代码构建标准（命名/文件组织/类型注解） | [`code-construction-standards.md`](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/governance/engineering/code-construction-standards.md) | ~1500 |
| 编码约定（L3 implementer 专属） | `src/zephyr/agent_spec/references/coding_conventions.md` | ~350 |
| 审计脚本质量标准（8 维度） | [`quality-standard.md`](file:///d:/ZephyrAlpha/scripts/governance/quality-standard.md) | ~3500 |
| Vibe Coding 施工方法论 | `docs/03_modules/_sys-master/blueprint.md` §十五 | ~2000 |
| Session 状态机（5状态/7转换） | [`session-state-runbook.md`](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/operational/vibe_coding/vibe-coding-session-state-runbook.md) | ~800 |
| 会话门禁检查清单（12项） | [`gate-checklist.md`](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/operational/vibe_coding/vibe-coding-gate-checklist.md) | ~600 |
| AI 事故响应手册（P0/P1/P2） | [`incident-playbook.md`](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/operational/vibe_coding/ai-incident-and-emergency-playbook.md) | ~1500 |
...
| 术语表 | `docs/01_policies_and_standards/meta/glossary.md` | ~1000 |
| 规则体系总索引 | `docs/01_policies_and_standards/index.md` | ~500 |
| **AI 压缩工作流标准** | [`compression-workflow-standard.md`](file:///d:/ZephyrAlpha/docs/01_policies_and_standards/governance/document/compression-workflow-standard.md) | ~800 |
...
已在 L0 + code-construction-standards.md §7 完整定义。此处补充十字段总览：
...
> ZephyrAlpha 的规则体系对标以下业界系统。了解这些对标有助于理解规则设计的意图。

| ZephyrAlpha 概念 | 对标 | 说明 |
|------|------|------|
| 注册表体系 (registry-of-registries.yaml) | K8s `kubectl api-resources` + Linux `man hier` | 进系统先看有什么资源 |
| 文件锁协议 (RULE-ZERO) | K8s ResourceQuota + etcd 分布式锁 + Git pre-commit hooks | 防止并发冲突写入 |
| 门禁引擎 (GateEngine) | CI/CD pipeline deployment gate + K8s admission webhook | 所有检查 pass/fail，无 maybe |
| scaffold.py 唯一入口 | `kubectl create` + Rails `rails generate` | 绕开 API → CONFLICT |
| 删除审判 (RULE-THREE) | 法庭"排除合理怀疑" + Git "never lose data" | 删前证死 |
| 反孤儿 (RULE-TWO) | Unix 哲学"写只做一件事的程序" + K8s 声明式自愈 | 每个组件独立可被发现和校验 |
| 零残留 (RULE-FIVE) | Unix 哲学"做完就退出不留痕迹" | 不留垃圾 |
| Session Continuity | K8s Pod Init Containers + 医院手术 Checklist | 主流程启动前必须跑完初始化 |
| 三层蓝图体系 | TOGAF（Enterprise/ Segment/ Capability）三层架构 | 系统蓝图 → 集成蓝图 → 模块蓝图 |
| Agent Skill 渐进披露 | agentskills.io + Anthropic Claude Skills | L0 永久加载 / L1 触发加载 / L2 组合 / L3 按需 |
...
| # | 问题 | 判定标准 |
|---|------|---------|
| 1 | 变更后是否产生新的循环依赖？ | 任何新循环 → **BLOCKED**，必须先解决循环 |
| 2 | 变更后跨包违规是增还是减？ | 减少或不变 → **SAFE**；增加但都是向下依赖 → **CAUTION**；增加且是向上依赖 → **BLOCKED** |
...
- **BLOCKED**：产生新循环或向上依赖 → 禁止执行，必须先解决阻塞
...
# 5. 关键模块导入测试
python -c "
for mod in ['zephyr.shared', 'zephyr.budget_enforcer', 'zephyr.escalation_engine',
            'zephyr.llm_security.gateway', 'zephyr.system_telemetry.auto_bootstrap',
            'zephyr.kb.storage.unified_memory_api', 'zephyr.governance']:
    __import__(mod)
    print(f'OK: {mod}')
"
```
