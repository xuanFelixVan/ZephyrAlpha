...
- **模块ID**: {module_id}
- **蓝图文档**: [{blueprint_filename}](../05_IMPLEMENTATION/06_CONSTRUCTION_DOCS/01_BLUEPRINTS/{blueprint_filename})
- **技术规格书**: 待创建
- **职责**: {responsibility}
- **状态**: {status}
```
...
def get_layer_info(layer_str: str) -> Tuple[str, str]:
    ...
    if match:
        ...
        layer_name = layer_str.split('(')[-1].split(')')[0] if '(' in layer_str else "组合优化层"
...
def process_blueprint_file(filepath: Path) -> bool:
    ...
    try:
        ...
        layer_str = yaml_header.get('layer', 'Layer 6 (组合优化层)')
        ...
        created_date = yaml_header.get('created_date', '2026-04-06')
...
def main():
    ...
    blueprint_files = list(BLUEPRINTS_DIR.glob("*BLUEPRINT.md"))
