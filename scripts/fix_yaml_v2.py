...
def fix_yaml_structure(file_path, new_responsibilities):
    ...
    for i, line in enumerate(lines):
        if line.strip() == '---':
            if first_yaml_end == -1 and i > 0:
                first_yaml_end = i
            elif second_yaml_end == -1 and second_yaml_start != -1:
                second_yaml_end = i
                break
        elif 'module_id:' in line and first_yaml_end != -1 and second_yaml_start == -1:
            second_yaml_start = i
    
    if second_yaml_start == -1 or second_yaml_end == -1:
        return False, '未找到第二个YAML块'
    
    second_yaml_lines = lines[second_yaml_start:second_yaml_end]
    second_yaml_content = '\n'.join(second_yaml_lines)
    ...
    new_lines = ['---'] + new_yaml_content.split('\n') + ['---'] + lines[second_yaml_end + 1:]
    new_content = '\n'.join(new_lines)
...
for filename in files_to_fix:
    ...
    if os.path.exists(file_path):
        ...
        success, msg = fix_yaml_structure(file_path, new_responsibilities)
        if success:
            fixed_count += 1
