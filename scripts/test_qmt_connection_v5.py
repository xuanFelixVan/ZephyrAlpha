"""
QMT连接测试脚本 v5 - 使用正确的API（无XtAccount类）

修复：
1. ✅ 移除XtAccount导入（不存在）
2. ✅ 使用账户字符串而不是XtAccount对象
3. ✅ 更新subscribe和query_stock_asset调用
"""
...
# 检查xttrader模块是否有XtAccount类（应该没有）
print()
print("检查xttrader模块结构:")
print("-" * 80)
try:
    from xtquant.xttrader import XtAccount
    print("⚠️  XtAccount类存在（这与预期不符）")
except ImportError:
    print("✅ XtAccount类不存在（这是正确的）")
    print("   说明API使用账户字符串而不是XtAccount对象")
...
print("  2. 如果连接失败 → 运行诊断脚本: scripts/diagnose_qmt_permission.py")
print("  3. 检查日志文件: userdata_mini 文件夹中的日志")
print()