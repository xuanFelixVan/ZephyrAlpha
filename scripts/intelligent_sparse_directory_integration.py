...
def load_analysis_report() -> Dict:
    report_path = PROJECT_ROOT / "docs/09_AUDIT/STATE" / "intelligent_sparse_directory_analysis_20260407_031239.json"
...
def integrate_directory(dir_info: Dict) -> Dict:
    ...
    try:
        ...
        if dir_info['reason'] == "仅有INDEX.md，无实际内容":
            if len(md_files) == 1 and md_files[0].name.lower() == 'index.md':
                if len(all_files) == 1:
                    shutil.rmtree(sparse_path)
                    result["action"] = "delete_empty"
                    result["success"] = True
        ...
        elif dir_info['reason'] == "仅有INDEX.md和README.md，无实际内容":
            if len(md_files) == 2:
        ...
        elif "深度" in dir_info['reason'] and "过深" in dir_info['reason']:
            ...
            result["action"] = "merge_to_parent"
            result["success"] = True
        
        elif "深度3层，文件少，无蓝图" in dir_info['reason']:
            parent_path = sparse_path.parent
            ...
            result["error"] = f"未知原因: {dir_info['reason']}"
...
def main():
    ...
    final_report_path = PROJECT_ROOT / "docs/09_AUDIT/STATE" / f"intelligent_sparse_directory_integration_result_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
