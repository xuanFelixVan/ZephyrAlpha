...
def check_dim1_field_registry() -> None:
    ...
    for field in data.get("fields", []):
        ...
        if not derived:
            ...
            _warn(f"DIM-1 field_registry.{fname}: derived_from='{derived}' 未指向 {VOCAB_FIELD_MAP[fname]}")
...
def check_dim2_arch_contract() -> None:
    ...
    for field in fm_schema.get("required_fields", []):
        ...
        if not derived:
            ...
            _warn(f"DIM-2 arch_contract.{fname}: derived_from='{derived}' 未指向 {VOCAB_FIELD_MAP[fname]}")
...
def check_dim5_vocab_contains_field() -> None:
    ...
    for fname, vocab_file in VOCAB_FIELD_MAP.items():
        ...
        if vocab_name != fname:
            _warn(f"DIM-5 {vocab_file}: vocabulary_name='{vocab_name}' 与期望的字段名 '{fname}' 不匹配")
