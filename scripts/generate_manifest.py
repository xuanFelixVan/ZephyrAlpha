...
_buf = _io.StringIO()
_buf.write("# Auto-generated by scripts/generate_manifest.py\n")
_buf.write("# DO NOT EDIT MANUALLY — run generate_manifest.py to regenerate\n")
_buf.write("#\n")
yaml.dump(manifest, _buf, default_flow_style=False, allow_unicode=True, sort_keys=False, width=200)
content = _buf.getvalue()

# P0②（AI-20 2026-09-05）：幂等写——内容未变跳写（generated_at 时间戳行不参与
# 比对），消除每波 reconciler churn（原实现无条件 tmp+replace 落盘）。
import os as _os
import re as _re
...
try:
    _existing = _out.read_text(encoding="utf-8")
    _rx = _re.compile(r"^generated_at: .*$", _re.MULTILINE)
    if _rx.sub("", content) == _rx.sub("", _existing):
        print(f"OK: {len(scripts)} scripts unchanged, skipped {OUTPUT_PATH} (idempotent)")
        sys.exit(0)
