...
from _shared.constants import REPO_ROOT, EXCLUDE_DIRS, SCAN_EXTENSIONS_CODE
from _shared.walk import iter_files
ensure_utf8_stdout()
import argparse
_SHARED_DIR = REPO_ROOT / 'scripts' / 'governance' / '_shared'
_DEPRECATED_PATHS_YAML = _SHARED_DIR / 'deprecated_paths.yaml'
_WHITELIST_FILES = {'AGENTS.md', 'architecture-rationale-log.md', 'vibe-coding-script-system-design.md', 'detect_ruins_references.py', 'deprecated_paths.yaml', 'blueprint.md'}
...
def scan_file(filepath: Path) -> list[dict]:
    ...
    for pattern, label in _get_ruins_patterns():
        for match in re.finditer(pattern, content, re.IGNORECASE):
            ...
            findings.append({'file': str(filepath.relative_to(REPO_ROOT)), 'line': line_num, 'pattern': label, 'context': context[:120]})
    for pattern in _get_obsolete_markers():
        if re.search(pattern, content, re.IGNORECASE):
            findings.append({'file': str(filepath.relative_to(REPO_ROOT)), 'line': 1, 'pattern': f'废弃跳转占位符: {pattern}', 'context': '占位跳转文件应删除或走 superseded_by 字段'})
    return findings
    '扫描单个文件并返回发现列表.'
