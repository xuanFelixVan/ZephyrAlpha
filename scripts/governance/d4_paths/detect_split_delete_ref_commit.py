...
def check_split_delete_ref(depth: int=20) -> list[dict]:
    ...
    try:
        result = subprocess.run(['git', 'log', f'-{depth}', '--oneline', '--diff-filter=D', '--name-only'], capture_output=True, text=True, cwd=str(REPO_ROOT), timeout=30)
    ...
    for commit_hash, deleted_files in delete_commits.items():
        try:
            result = subprocess.run(['git', 'log', '-1', '--oneline', '--name-only', f'{commit_hash}^..{commit_hash}'], capture_output=True, text=True, cwd=str(REPO_ROOT), timeout=15)
            if result.returncode == 0:
                all_files_in_commit = [l.strip() for l in result.stdout.strip().split('\n') if l.strip()]
                has_ref_update = any((f not in deleted_files and (f.endswith('.md') or f.endswith('.yaml')) for f in all_files_in_commit[1:]))
                if not has_ref_update and len(deleted_files) > 0:
                    for df in deleted_files:
                        findings.append({'commit': commit_hash, 'deleted_file': df, 'detail': f'删除 {df} 但同 commit 无引用更新', 'severity': 'MEDIUM'})
        except (subprocess.TimeoutExpired, FileNotFoundError, OSError):
            pass
    return findings
    'check split delete ref.'
