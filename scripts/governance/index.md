...
scripts/governance/
├── _shared/                        共用工具（5 文件）
├── d1_structure                   D1 结构完整性（17 脚本）
├── d2_links                       D2 链接完整性（2 脚本）
├── d3_metadata                    D3 元数据合规（18 脚本）
├── d4_paths                       D4 路径有效性（4 脚本）
├── d5_architecture                D5 架构合规（36 脚本）
├── d6_security                    D6 安全漏洞（10 脚本）
├── d7_code                        D7 代码质量（15 脚本）
├── d8_doc_sync                    D8 文档代码同步（4 脚本）
├── d9_knowledge                   D9 知识覆盖（2 脚本）
├── d10_performance                D10 性能容量（待建设）
├── d11_compliance                 D11 合规完整性（6 脚本）
├── d12_ai_hallucination           D12 AI 幻觉（2 脚本）
├── run_all.py                     全维度扫描入口
├── status.py                      健康仪表盘
├── check_registry_consistency.py  跨登记表一致性校验
├── script_manifest.yaml           脚本注册表（SSoT — 117 条目）
