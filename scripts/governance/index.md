...
scripts/governance/
├── _shared/                        共用工具（5 文件）
├── d1_structure                   D1 结构完整性（25 脚本）
├── d2_links                       D2 链接完整性（2 脚本）
├── d3_metadata                    D3 元数据合规（28 脚本）
├── d4_paths                       D4 路径有效性（4 脚本）
├── d5_architecture                D5 架构合规（12 脚本）
├── d6_security                    D6 安全漏洞（15 脚本）
├── d7_code                        D7 代码质量（43 脚本）
├── d8_doc_sync                    D8 文档代码同步（15 脚本）
...
├── script_manifest.yaml           脚本注册表（SSoT — 233 条目）
...
5. **创建新脚本工具**（AGENTS.md CREATE-GUARD 入库登记铁律）：
