...
CARDS = [
    # ===== 5项待修复 =====
    {
        "task_id": "DM-100242", "namespace": "DM", "seq": 100242,
        "title": "G5: 生成器域名双源统一——移除硬编码域名字典改为从DB读取",
        "priority": "P1", "phase": 4, "safety_level": "M", "directive": "G5",
        "source_blueprint": "MOD-GOV-DEPGRAPH", "source_section": "§九 G5",
        "files_in_scope": json.dumps([
            "D:/ZephyrAlpha/scripts/governance/generate_project_depgraph.py", REGISTRY,
        ], ensure_ascii=False),
    ...
    {
        "task_id": "SRC-100295", "namespace": "SRC", "seq": 100295,
        "title": "B2: 安全敏感文件[AI_AUTONOMY]标记——认证/加密/回滚文件标human_gated",
        "priority": "P1", "phase": 4, "safety_level": "H", "directive": "B2",
        "source_blueprint": "MOD-GOV-DEPGRAPH", "source_section": "§五 B2",
        "files_in_scope": json.dumps([
            REGISTRY, "D:/ZephyrAlpha/src/zephyr/security/llm_security_01/gateway.py",
        ], ensure_ascii=False),
    ...
    {
        "task_id": "DM-100243", "namespace": "DM", "seq": 100243,
        "title": "E1: arch_layers表10条旧层名记录清理——4标准层已覆盖100%",
        "priority": "P2", "phase": 4, "safety_level": "L", "directive": "E1",
        "source_blueprint": "MOD-GOV-DEPGRAPH", "source_section": "§七 E1",
    ...
    {
        "task_id": "OPS-2026061804", "namespace": "OPS", "seq": 2026061804,
        "title": "C3: 门禁增量扫描——仅扫描变更文件而非全量扫描",
        "priority": "P2", "phase": 4, "safety_level": "M", "directive": "C3",
        "source_blueprint": "MOD-GOV-DEPGRAPH", "source_section": "§六 C3",
        "files_in_scope": json.dumps([
            "D:/ZephyrAlpha/scripts/governance/audit_registration.py", REGISTRY,
        ], ensure_ascii=False),
        "deliverables": json.dumps([
    ...
    {
        "task_id": "DM-100244", "namespace": "DM", "seq": 100244,
        "title": "K1: 3项待定架构决策文档——T6事件类型/T7三级配置/T17模块DOMAIN字段",
        "priority": "P3", "phase": 4, "safety_level": "L", "directive": "K1",
        "source_blueprint": "MOD-ARCH-002", "source_section": "§十一 K1",
        "files_in_scope": json.dumps([
            REGISTRY,
    ...
    # ===== 7项需重新校验 =====
    {
        "task_id": "DM-100245", "namespace": "DM", "seq": 100245,
        "title": "I5-RECHECK: belongs_to校验逻辑修正——存模块ID非node_id需用正确字段校验",
        "priority": "P2", "phase": 4, "safety_level": "L", "directive": "I5",
        "source_blueprint": "MOD-GOV-DEPGRAPH", "source_section": "§四 I5",
        "files_in_scope": json.dumps([DB, REGISTRY], ensure_ascii=False),
        "deliverables": json.dumps([], ensure_ascii=False),
    ...
    {
        "task_id": "DM-100246", "namespace": "DM", "seq": 100246,
        "title": "I8-RECHECK: build_status消费者确认——生成器默认填draft是否有消费者",
        "priority": "P2", "phase": 4, "safety_level": "L", "directive": "I8",
        "source_blueprint": "MOD-GOV-DEPGRAPH", "source_section": "§四 I8",
        "files_in_scope": json.dumps([DB, REGISTRY], ensure_ascii=False),
        "deliverables": json.dumps([], ensure_ascii=False),
        "allowed_touch": json.dumps([], ensure_ascii=False),
        "description": make_desc(
    ...
    {
        "task_id": "DM-100247", "namespace": "DM", "seq": 100247,
        "title": "I13-RECHECK: 孤儿节点过滤——需过滤出node_type=module的production代码文件",
        "priority": "P2", "phase": 4, "safety_level": "L", "directive": "I13",
        "source_blueprint": "MOD-GOV-DEPGRAPH", "source_section": "§四 I13",
        "files_in_scope": json.dumps([DB, REGISTRY], ensure_ascii=False),
        "deliverables": json.dumps([], ensure_ascii=False),
        "allowed_touch": json.dumps([], ensure_ascii=False),
        "description": make_desc(
    ...
    {
        "task_id": "DM-100248", "namespace": "DM", "seq": 100248,
        "title": "I15-RECHECK: tags字段消费者确认——88.8%为空是否影响AI决策",
        "priority": "P2", "phase": 4, "safety_level": "L", "directive": "I15",
        "source_blueprint": "MOD-GOV-DEPGRAPH", "source_section": "§四 I15",
        "files_in_scope": json.dumps([DB, REGISTRY], ensure_ascii=False),
        "deliverables": json.dumps([], ensure_ascii=False),
        "allowed_touch": json.dumps([], ensure_ascii=False),
        "description": make_desc(
    ...
    {
        "task_id": "DM-100249", "namespace": "DM", "seq": 100249,
        "title": "E5-RECHECK: last_verified消费者确认——与I15合并处理",
        "priority": "P2", "phase": 4, "safety_level": "L", "directive": "E5",
        "source_blueprint": "MOD-GOV-DEPGRAPH", "source_section": "§七 E5",
        "files_in_scope": json.dumps([DB, REGISTRY], ensure_ascii=False),
        "deliverables": json.dumps([], ensure_ascii=False),
        "allowed_touch": json.dumps([], ensure_ascii=False),
        "description": make_desc(
    ...
    {
        "task_id": "DM-100250", "namespace": "DM", "seq": 100250,
        "title": "A4-RECHECK: 循环依赖过滤——需排除同域内循环确认有无跨域循环",
        "priority": "P2", "phase": 4, "safety_level": "L", "directive": "A4",
        "source_blueprint": "MOD-GOV-DEPGRAPH", "source_section": "§三 A4",
        "files_in_scope": json.dumps([DB, REGISTRY], ensure_ascii=False),
        "deliverables": json.dumps([], ensure_ascii=False),
        "allowed_touch": json.dumps([], ensure_ascii=False),
        "description": make_desc(
    ...
    {
        "task_id": "DM-100251", "namespace": "DM", "seq": 100251,
        "title": "C1-RECHECK: 3个索引查询频率确认——coupling_strength仅7个不同值索引效果有限",
        "priority": "P2", "phase": 4, "safety_level": "L", "directive": "C1",
        "source_blueprint": "MOD-GOV-DEPGRAPH", "source_section": "§六 C1",
        "files_in_scope": json.dumps([DB, REGISTRY], ensure_ascii=False),
        "deliverables": json.dumps([], ensure_ascii=False),
        "allowed_touch": json.dumps([], ensure_ascii=False),
        "description": make_desc(
            ...
            cur.execute("""
                INSERT INTO tasks (
                    ...
                    requires_rb_check
                          ...
                          'A', 8000, 30, 0, 0, 0)
            """, (
                c["task_id"], c["namespace"], c["seq"], c["title"],
                c["description"], c["priority"], c["phase"],
                c["safety_level"], c["directive"],
                c["source_blueprint"], c["source_section"],
                c["files_in_scope"], c["deliverables"], c["allowed_touch"],
                RULES_BASE, c["rollback"],
                POST_SYNC, ACCEPTANCE_STD,
                NOW, NOW,
            ))
