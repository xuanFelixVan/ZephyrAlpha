...
# [DEPENDENCIES] scripts.governance._shared.file_utils
# [CONSUMERS]
# [STARTUP] manual
# [MATURITY] production
# [INVARIANTS] 本脚本是 data/asset_index/unified-asset-index.yaml 的唯一标准写者（2026-09-06 Owner 裁定"救活老脚本当唯一标准"）——GATE-ASSET-INDEX reconciler 已禁写该文件
# [MODIFY-GUARD]
# [STABILITY] evolving
# [SAFETY] M
# [AI_AUTONOMY] ai_modifiable
# [ERROR_CONTRACT] 扫描只读；写入经 atomic_write_if_changed 幂等落盘（generated_at 波动豁免）
# [TESTS] none
# [A_module] module_id=MOD-INF-005 | layer=script | stability=evolving | safety=M | ai_autonomy=ai_modifiable
# [TTL] permanent
"""
全项目资产索引生成器（2026-09-06 自 _archive/prototype 恢复为唯一真源写者，Owner 裁定）
===================
RULE-NINE 合规：生成 unified-asset-index.yaml 供新 AI session 冷启动时了解项目全盘资产规模。

宽扫描口径（六目录全文件：src/scripts/tests/docs/config/data），历史好态口径对照
commit 73dbf90f（total_assets≈31847 / Health B）。
...
from _shared.file_utils import atomic_write_if_changed  # noqa: E402  P0② 幂等写（AI-20 范式）
...
def _scan_directory(dir_path: Path) -> dict[str, int]:
    """只读扫描目录，按扩展名分类计数。"""
...
def generate(output_path: Path | None = None) -> Path:
    ...
    with ThreadPoolExecutor(max_workers=_MAX_WORKERS) as executor:
        ...
        for cat_name, rel_dirs in _SCAN_DIRECTORIES.items():
            for rel_dir in rel_dirs:
                ...
                future = executor.submit(_scan_directory, abs_dir)
    ...
    content = yaml.safe_dump(index, sort_keys=False, allow_unicode=True, default_flow_style=False)
    # 幂等写（P0② AI-20 范式）：generated_at 波动豁免，内容未变则跳写
    atomic_write_if_changed(output, content, volatile_line_pattern=r"^\s*generated_at: .*$")
