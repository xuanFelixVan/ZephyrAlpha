...
def _resolve_absolute_import(module_path: str) -> Optional[dict]:
...
def _resolve_relative_import(filepath: Path, level: int, module: Optional[str]) -> Optional[dict]:
    try:
            ...
            if node.level and node.level > 0:
                ...
                if resolved:
                    ...
                    if imported_names and resolved["resolved_file"] and "__init__.py" in resolved["resolved_file"]:
                        ...
                        rest = node.module[len("zephyr."):] if node.module.startswith("zephyr.") else node.module[len("scripts."):]
...
def main():
    ...
    parser.add_argument("--scan-roots", nargs="+", default=["src/zephyr", "scripts"],
    ...
    parser.add_argument("--check", action="store_true",
                        help="Check mode: compare with existing depgraph and report drift")
    parser.add_argument("--warn-only", action="store_true",
                        help="Exit 0 even if issues found (for CI non-blocking)")
    ...
    print(f"[SCAN] Scanned {len(scan_results)} files, "
    ...
    stats = graph["stats"]
    print(f"[BUILD] Ground-truth graph: {stats['resolved_edges']} edges, "
    ...
    print(f"[VIOLATION] C→C upward: {len(violations['c_to_c_upward'])}, "
