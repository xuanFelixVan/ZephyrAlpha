...
def scan_file(rel_path: str) -> Optional[ScanResult]:
    basename = os.path.basename(rel_path)
    if basename in EXEMPT_FILE_PATTERNS:
        return None
    if any(basename.startswith(p) for p in ("_temp", "_check", "_fix", "_phase_", "_deep", "_construction", "_rebuild", "_audit")):
...
def main():
    ...
    parser.add_argument("--category", type=str, default="", help="Filter by file_category (code,script,test,gate,registry,data,config,infra)")
    ...
    print(f"[HEADER-BATCH] mode={'DRY-RUN' if dry_run else 'APPLY'} category={args.category or 'ALL'} blueprint={args.blueprint or 'ALL'}")
    ...
    if args.json_output:
        output = {
            ...
            "details": [
                {"path": r.path, "category": r.file_category, "format": r.header_format,
                 "blueprint_id": r.blueprint_id, "status": apply_results.get(r.path, "UNMAPPED")}
                for r in need_header
            ],
        }
