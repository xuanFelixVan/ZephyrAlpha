...
def merge_domain(domain_id: str) -> bool:
    ...
    cur.execute("SELECT COUNT(*) FROM nodes WHERE domain_id=? AND design_maturity='design' AND node_type IN ('feature','reference','implementation')", (domain_id,))
    ...
    if fri_count > 0:
        ...
        for child_id, child_name, child_type, belongs_to in children:
            ...
            if parent_id:
                ...
                cur.execute("UPDATE nodes SET type_specific_data=? WHERE node_id=?", (json.dumps(tsd, ensure_ascii=False), parent_id))
    ...
    cur.execute("SELECT node_id, node_name, type_specific_data FROM nodes WHERE domain_id=? AND design_maturity='design' AND node_type='module' ORDER BY node_id", (domain_id,))
    ...
    if len(modules) > 80:
        ...
        for pfx, mods in groups.items():
            ...
            cur.execute("UPDATE nodes SET type_specific_data=? WHERE node_id=?", (json.dumps(rep_tsd, ensure_ascii=False), rep_id))
    ...
    cur.execute("SELECT node_type, COUNT(*) FROM nodes WHERE domain_id=? AND design_maturity='design' GROUP BY node_type ORDER BY COUNT(*) DESC", (domain_id,))
