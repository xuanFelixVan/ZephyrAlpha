...
def sync_cross_module_dependencies(cur):
    ...
    for dep in deps:
        ...
        cur.execute(
            "SELECT node_id FROM nodes WHERE blueprint_id = ? LIMIT 1",
            (source,)
        )
        from_row = cur.fetchone()
        if not from_row and source_name:
            cur.execute(
                "SELECT node_id FROM nodes WHERE path LIKE ? LIMIT 1",
                (f'%{source_name}%',)
            )
            from_row = cur.fetchone()
...
def sync_functional_domain_registry(cur):
    ...
    for d in entries:
        ...
        if not domain_id.startswith('D-'):
            print(f"  SKIP: 跳过小写类别域 '{domain_id}' (subdomain={d.get('subdomain', '')})——待 DM-100256 重构为 D-XXX 格式")
            skipped += 1
            continue
