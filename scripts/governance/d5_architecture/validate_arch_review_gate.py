...
def get_active_adrs() -> dict[str, dict]:
    """获取活跃 ADR 列表"""
    adrs = {}
    adr_dirs = [REPO_ROOT / '' / 'docs' / '01_policies_and_standards' / 'governance' / 'architecture' / 'adr', REPO_ROOT / 'docs' / '01_policies_and_standards' / 'governance' / 'architecture' / 'adr']
...
def scan_arch_review_triggers() -> list[dict]:
    ...
    if not docs_dir.exists():
        docs_dir = REPO_ROOT / 'docs'
    trigger_keywords = {'新增模块': ['new module', 'add module', '新增模块'], '删除模块': ['remove module', 'delete module', '删除模块', '退役'], '修改接口': ['interface change', 'API change', '修改接口', '接口变更'], '更换技术栈': ['technology change', 'tech stack', '更换技术', '技术选型'], '修改数据流': ['data flow change', '数据流', 'pipeline change']}
        ...
        for trigger_type, keywords in trigger_keywords.items():
            for kw in keywords:
                if kw.lower() in content.lower():
                    has_adr_ref = bool(re.search('(?:ADR|adr)[-_]?\\d{1,4}', content, re.IGNORECASE))
                    if not has_adr_ref:
...
def main() -> None:
    ...
    if findings:
        ...
        for f in findings:
            ...
            print(f'    未找到 ADR 引用', file=sys.stderr)
    else:
