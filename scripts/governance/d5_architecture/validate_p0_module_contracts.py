...
def load_contracts() -> dict:
    ...
    paths = [REPO_ROOT / '' / 'docs' / '01_policies_and_standards' / '_registry' / 'contracts' / 'cross-layer-contracts.yaml', REPO_ROOT / 'docs' / '01_policies_and_standards' / '_registry' / 'contracts' / 'cross-layer-contracts.yaml']
    for p in paths:
        if p.exists():
            try:
...
def scan_p0_modules() -> list[dict]:
    ...
    for filepath in iter_files(docs_dir, extensions=frozenset({'.md'})):
        ...
        if not provider_contracts:
            findings.append({'file': rel, 'module_id': module_id, 'type': 'P0_NO_CONTRACT', 'detail': 'P0 模块无接口契约', 'severity': 'HIGH'})
        elif not has_frozen:
            findings.append({'file': rel, 'module_id': module_id, 'type': 'P0_NO_FROZEN_CONTRACT', 'detail': 'P0 模块无 frozen 状态的接口契约', 'severity': 'HIGH'})
        if not runtime_plane:
            findings.append({'file': rel, 'module_id': module_id, 'type': 'P0_NO_RUNTIME_PLANE', 'detail': 'P0 模块未分配 runtime_plane', 'severity': 'MEDIUM'})
        elif runtime_plane not in VALID_RUNTIME_PLANES:
            findings.append({'file': rel, 'module_id': module_id, 'type': 'P0_INVALID_RUNTIME_PLANE', 'detail': f"runtime_plane='{runtime_plane}' 不在合法枚举中（合法值: {', '.join(sorted(VALID_RUNTIME_PLANES))}）", 'severity': 'MEDIUM'})
    return findings
    '扫描 P0 模块合约合规性.'
