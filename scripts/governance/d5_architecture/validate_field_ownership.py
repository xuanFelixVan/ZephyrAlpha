...
SSOT_AUTHORITY_FILES = {'metadata-registry.md': 'PS-STD-001 — frontmatter schema 唯一真源', 'rule-classification-and-arbitration-standard.md': 'PS-STD-011 — 规则分类唯一真源', 'glossary.md': 'PS-STD-011 配套 — 术语定义唯一真源', 'document-structure-standard.md': 'PS-STD-002 — 文档结构唯一真源', 'directory-structure-standard.md': 'GOV-DOC-002 — 目录结构唯一真源'}
...
def scan_field_usage() -> tuple[list[dict], int]:
    ...
    for field_name, files in sorted(field_usages.items()):
        ...
        for ssot_file, ssot_desc in SSOT_AUTHORITY_FILES.items():
            ...
            if non_authority:
                findings.append({'field': field_name, 'ssot_file': ssot_file, 'ssot_description': ssot_fields.get(field_name, ''), 'defined_in': non_authority, 'severity': 'HIGH', 'message': f'SSOT字段在非权威文件中被定义（应由 {ssot_file} 定义）'})
