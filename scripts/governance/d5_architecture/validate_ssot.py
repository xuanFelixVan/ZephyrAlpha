...
_HISTORICAL_TYPOS: frozenset[str] = frozenset({'layer_00', 'layer_01', 'layer_02', 'layer_03', 'layer_04', 'layer_05', 'layer_06', 'layer_07', 'layer_08', 'layer_09', 'layer_10', 'layer_11', 'layer_12', 'layer_13'})
...
_VALID_LAYERS_CACHE: Optional[frozenset[str]] = None
...
VALID_STATUSES: frozenset[str] = frozenset({'Draft', 'Review', 'Active', 'Superseded', 'Deprecated', 'Retired', 'proposed', 'active', 'deprecated'})
...
SEVERITY_P0_STATUS_CONFLICT = frozenset({('Active', 'Deprecated'), ('Active', 'Retired'), ('Deprecated', 'Active'), ('Retired', 'Active')})
...
def parse_file(path: Path, repo_root: Path) -> Optional[FileMeta]:
    """
    ...
    try:
        ...
        rel = str(path)
    return FileMeta(path=path, rel_path=rel, module_id=fm.get('module_id'), layer=fm.get('layer'), status=fm.get('status'), priority=fm.get('priority'), version=fm.get('version'), owner=fm.get('owner'))
...
def check_p0_layer_invalid(metas: list[FileMeta]) -> list[Contradiction]:
    ...
    for m in metas:
        ...
        if m.layer not in _get_valid_layers():
            result.append(Contradiction(severity='P0', check_id='P0-1', description=f'layer 字段值 `{m.layer}` 不在有效层 ID 集合（L00-L13/cross_layer）中', files=[m.rel_path], values=[m.layer], suggestion=f'将 `layer: {m.layer}` 修改为有效的层 ID（参见 ssot-authority-map.md §一）'))
    return result
...
def check_p0_duplicate_active_module_id(metas: list[FileMeta]) -> list[Contradiction]:
    ...
    for mid, paths in active_by_id.items():
        if len(paths) > 1:
            result.append(Contradiction(severity='P0', check_id='P0-2', description=f'module_id `{mid}` 在 {len(paths)} 个 Active 文件中重复声明（违反零冗余原则）', files=paths, values=[mid] * len(paths), suggestion=f'保留唯一真源，其余文件改为引用或删除。module_id: {mid}'))
    return result
...
def check_p1_status_invalid(metas: list[FileMeta]) -> list[Contradiction]:
    ...
    for m in metas:
        ...
        if m.status not in VALID_STATUSES:
            result.append(Contradiction(severity='P1', check_id='P1-1', description=f'status 字段值 `{m.status}` 不在有效状态集合中', files=[m.rel_path], values=[m.status], suggestion='将 status 修改为有效值：Draft/Review/Active/Superseded/Deprecated/Retired'))
    return result
...
def check_p1_module_id_layer_conflict(metas: list[FileMeta]) -> list[Contradiction]:
    ...
    for mid, layer_map in layer_by_id.items():
        if len(layer_map) > 1:
            ...
            result.append(Contradiction(severity='P1', check_id='P1-2', description=f'module_id `{mid}` 在不同文件中 layer 字段不一致：{conflict_desc}', files=all_files, values=list(layer_map.keys()), suggestion='以 docs/02_enterprise_architecture/target-architecture/architecture-model/_index.yaml + layers/l*.yaml 中的层归属为准，统一修正'))
    return result
...
def check_p1_module_id_status_conflict(metas: list[FileMeta]) -> list[Contradiction]:
    ...
    for mid, status_map in status_by_id.items():
        ...
        if is_conflict:
            ...
            result.append(Contradiction(severity='P1', check_id='P1-3', description=f'module_id `{mid}` 在不同文件中 status 字段矛盾：{conflict_desc}', files=all_files, values=list(statuses), suggestion='以权威来源为准，废弃版本改为 Deprecated 或 Retired'))
    return result
...
def check_p2_priority_invalid(metas: list[FileMeta]) -> list[Contradiction]:
    ...
    for m in metas:
        ...
        if m.priority not in VALID_PRIORITIES:
            result.append(Contradiction(severity='P2', check_id='P2-1', description=f'priority 字段值 `{m.priority}` 不在有效集合 (P0/P1/P2/P3) 中', files=[m.rel_path], values=[m.priority], suggestion='将 priority 修改为 P0/P1/P2/P3 之一'))
    return result
...
def check_p2_version_format(metas: list[FileMeta]) -> list[Contradiction]:
    ...
    for m in metas:
        ...
        if not VERSION_PATTERN.match(m.version):
            result.append(Contradiction(severity='P2', check_id='P2-2', description=f'version 字段值 `{m.version}` 格式不符合语义版本规范', files=[m.rel_path], values=[m.version], suggestion='将 version 修改为 MAJOR.MINOR.PATCH 格式（如 1.0.0）'))
    return result
...
class SsotValidator:
    ...
    def __init__(self, scan_dir: Optional[Path]=None, repo_root: Optional[Path]=None) -> None:
        self._repo_root = repo_root or REPO_ROOT
        self._scan_dir = scan_dir or DEFAULT_SCAN_DIR

    def run(self) -> ScanReport:
        """执行扫描"""
        report = ScanReport(scan_dir=str(self._scan_dir.relative_to(self._repo_root)), scan_time=datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        ...
        checks = [check_p0_layer_invalid, check_p0_duplicate_active_module_id, check_p1_status_invalid, check_p1_module_id_layer_conflict, check_p1_module_id_status_conflict, check_p2_priority_invalid, check_p2_version_format]
...
def render_report(report: ScanReport) -> str:
    """渲染报告"""
    lines: list[str] = []
    lines += ['---', 'type: generated', 'ttl: 7d', 'generated_by: scripts/governance/validate_ssot.py', f'scan_time: {report.scan_time}', '---', '', '# SSoT 矛盾扫描报告', '', f'> **扫描目录**：`{report.scan_dir}`  ', f'> **扫描时间**：{report.scan_time}  ', f'> **扫描文件**：{report.scanned_files} 个 .md 文件，{report.parsed_files} 个含 frontmatter  ', '', '---', '', '## 摘要', '', '| 严重级别 | 数量 | 处置要求 |', '|---------|------|---------|', f'| 🔴 P0（严重）| {report.p0_count} | 必须立即修复，阻塞 Phase 2 完成门禁 |', f'| 🟡 P1（重要）| {report.p1_count} | 需尽快修复，影响可信度 |', f'| 🔵 P2（建议）| {report.p2_count} | 低优先级，可按计划处理 |', f'| **合计** | **{report.total_count}** | |', '']
    ...
    for severity in ('P0', 'P1', 'P2'):
        ...
        for i, c in enumerate(items, 1):
            lines += [f'### {severity}-{i}：{c.description[:80]}', '', f'- **检查 ID**：`{c.check_id}`', '- **涉及文件**：']
            for f in c.files[:10]:
                lines.append(f'  - `{f}`')
            if len(c.files) > 10:
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--report', type=Path, default=DEFAULT_REPORT_PATH, help=f'报告输出路径（默认：{DEFAULT_REPORT_PATH.relative_to(REPO_ROOT)}）')
...
def main() -> None:
    ...
    if args.ci and report.has_p0:
        print(f'\n❌ CI 门禁阻塞：发现 {report.p0_count} 条 P0 矛盾，必须修复后才能通过 Phase 2 完成门禁。', file=sys.stderr)
        sys.exit(1)
    if report.total_count == 0:
