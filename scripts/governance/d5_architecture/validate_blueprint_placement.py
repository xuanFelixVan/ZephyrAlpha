...
INFRA_DIR = BLUEPRINTS_DIR / "l01_infrastructure"

VALID_LAYERS: frozenset[str] = frozenset(
    {f"L{i:02d}" for i in range(0, 14)} | {"cross_layer"}
)

VALID_BELONGS_TO: frozenset[str] = frozenset(
    {"SYS-MASTER-001", "MOD-MASTER-001", "DOM-GOV-001"}
)
...
def main() -> int:
    ...
    for module_id, (filepath, fm) in sorted(blueprints.items()):
        ...
        if bt is None or (isinstance(bt, str) and not bt.strip()):
            violations_p0.append(
                f"[P0-1] 蓝图 {module_id} 缺少 belongs_to 字段 → {filepath.relative_to(REPO_ROOT)}"
            )
    ...
    for module_id, (filepath, fm) in sorted(blueprints.items()):
        ...
        if layer.startswith("L") and layer != "cross_layer":
            # 检查完整路径中是否包含 l{NN}_ 前缀（适配 l01_infrastructure/task-system/ 这种嵌套结构）
    ...
    for module_id, bt_target in module_bt.items():
        ...
        while True:
            ...
            if next_target == current:
                violations_p1.append(
                    f"[P1-1] belongs_to 链自环——{module_id} 的 belongs_to 指向自身 ({current})"
                )
                break
    ...
    for module_id, bt_target in module_bt.items():
        if bt_target not in blueprints:
            violations_p1.append(
                f"[P1-2] believes_to 目标 {bt_target} (来自 {module_id}) 不在已注册蓝图列表中"
            )
