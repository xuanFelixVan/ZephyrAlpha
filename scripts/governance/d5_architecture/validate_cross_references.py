...
检查维度：
  ...
  DIM-3: adr_ref 引用链    —— 所有 YAML 中的 adr_ref → adr/ 实际文件
...
def _collect_all_adr_refs() -> list[tuple[str, str, str]]:
    ...
    for fname, data in _load_all_layer_yamls():
        ...
        for mod in modules:
            module_id = mod.get("id", "?")
            adr_ref = mod.get("adr_ref", [])
            if isinstance(adr_ref, list):
                for a in adr_ref:
                    refs.append((fname, module_id, a))
            elif isinstance(adr_ref, str) and adr_ref:
                refs.append((fname, module_id, adr_ref))
...
def main() -> None:
    ...
    checks = [
        ...
        ("DIM-3: adr_ref → ADR 文件存在性", check_dim3_adr_refs),
