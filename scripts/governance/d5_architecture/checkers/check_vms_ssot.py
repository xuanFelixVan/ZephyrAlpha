...
# 检测2：snapshot 已删除方法名清单（AST 检测，防重建）
# 规则真源见 AGENTS.md §11.2 遗留项-4 snapshot 整体删除治本
SNAPSHOT_METHOD_NAMES = frozenset(
    {
        "snapshot_backup",
        "cleanup_snapshots",
...
# 检测2：AST 扫描范围——仅 VMS 真源目录的 .py 文件（治本范围限定，不误报其他模块）
SNAPSHOT_CHECK_PREFIX = "src/zephyr/integration/vector_memory/"
...
def check_snapshot_rebuild(files):
    """检测2：VMS 真源目录 .py 文件是否重建已删除的 snapshot 方法（AST 检测）

    扫描 src/zephyr/integration/vector_memory/ 下 staged 的 .py 文件，
    用 ast.walk 检测 FunctionDef/AsyncFunctionDef 节点名字是否命中黑名单。
    无论方法体是否实现，命中方法名即阻断（防空桩绕过 / 伪装重建）。
    """
    violations = []
    for f in files:
        if not f.lower().startswith(SNAPSHOT_CHECK_PREFIX):
            continue
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                if node.name in SNAPSHOT_METHOD_NAMES:
...
def main():
    parser = argparse.ArgumentParser(
        description="GATE-VMS-SSOT: VMS 单一真源门禁（governance/vector_memory 漂移副本 + snapshot 方法重建 防复发）"
    )
    ...
    violations_snapshot = check_snapshot_rebuild(files)

    if not violations_ssot and not violations_snapshot:
        return 0

    print("[GATE-VMS-SSOT] 违规：VMS 单一真源 / snapshot 治本约束被破坏", file=sys.stderr)
    ...
    if violations_snapshot:
        print(
            f"  [检测2·snapshot 重建] VMS 真源目录下检测到已删除的 snapshot 方法被重建\n"
            f"    违规：{violations_snapshot}\n"
            f"    真源：AGENTS.md §11.2 遗留项-4 snapshot 整体删除治本\n"
            f"    原因：snapshot_backup/cleanup_snapshots/_cleanup_old_snapshots 三方法已删除治本"
            f"（commit dddd1813）——30GB 递归自复制 + 零消费方 + R4 被 ChromaDB ACID+WAL 覆盖\n"
            f"    修复：删除重建的方法。数据损坏恢复靠 auto_repair() 尝试修复，"
            f"勿重建 snapshot（audit_chain 回放重写未实现，亦勿虚构）",
            file=sys.stderr,
        )
