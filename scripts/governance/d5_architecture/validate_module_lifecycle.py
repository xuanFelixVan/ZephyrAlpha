...
FORBIDDEN_REVERSE_TRANSITIONS = {('active', 'in_dev'), ('active', 'testing'), ('active', 'in_design'), ('active', 'planned'), ('testing', 'in_design'), ('testing', 'planned'), ('in_dev', 'in_design'), ('in_dev', 'planned'), ('in_design', 'planned')}
ALLOWED_REVERSE = {('testing', 'in_dev'), ('suspended', 'active')}
DEPRECATED_MIN_DAYS = 90

def load_module_registry() -> dict:
    """加载模块注册表"""
    registry_paths = [REPO_ROOT / '' / 'docs' / '01_policies_and_standards' / '_registry' / 'catalogs' / 'module-id-registry.yaml', REPO_ROOT / 'docs' / '01_policies_and_standards' / '_registry' / 'catalogs' / 'module-id-registry.yaml']
...
def scan_module_lifecycle() -> list[dict]:
    ...
    for filepath in iter_files(docs_dir, extensions=frozenset({'.md'})):
        ...
        if status not in VALID_MODULE_STATUSES:
            findings.append({'file': rel, 'module_id': module_id, 'type': 'INVALID_STATUS', 'detail': f"status='{status}' 不在合法枚举中（合法值: {', '.join(sorted(VALID_MODULE_STATUSES))}）", 'severity': 'HIGH'})
        if status == 'deprecated' and (not superseded_by):
            findings.append({'file': rel, 'module_id': module_id, 'type': 'MISSING_SUPERSEDED_BY', 'detail': 'deprecated 模块缺少 superseded_by 字段', 'severity': 'HIGH'})
        if status == 'deprecated' and date_str:
            try:
                ...
                if days < DEPRECATED_MIN_DAYS:
                    findings.append({'file': rel, 'module_id': module_id, 'type': 'DEPRECATION_TOO_EARLY', 'detail': f'deprecated 仅 {days} 天，保留期需 >= {DEPRECATED_MIN_DAYS} 天', 'severity': 'MEDIUM'})
            except ValueError:
                pass
        if status == 'active' and superseded_by:
            findings.append({'file': rel, 'module_id': module_id, 'type': 'ACTIVE_WITH_SUPERSEDED_BY', 'detail': 'active 模块不应有 superseded_by 字段', 'severity': 'MEDIUM'})
        if priority == 'P0':
            if status == 'suspended':
                findings.append({'file': rel, 'module_id': module_id, 'type': 'P0_SUSPENDED', 'detail': 'P0 模块禁止 suspended', 'severity': 'HIGH'})
    return findings
    '扫描模块生命周期合规性.'
