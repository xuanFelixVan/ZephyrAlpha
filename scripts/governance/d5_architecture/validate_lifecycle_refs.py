...
def _resolve_target(target: str, source_file: Path, module_id_to_file: dict[str, Path], file_to_fm: dict[str, dict]) -> tuple[Path | None, dict | None]:
...
def scan_lifecycle_violations(docs_dir: Path) -> tuple[list[dict], int, int]:
        ...
        for dep in depends_on:
            ...
            if resolved_fp and resolved_fm:
                ...
                if dep_status == 'draft':
                    ...
                    findings.append({'file': rel, 'severity': 'MEDIUM', 'violation': f'LRC-001: active 文件引用 draft 文件 {dep_mid or dep_rel} via depends_on', 'depends_on_target': target, 'target_file': dep_rel, 'target_module_id': dep_mid, 'target_status': 'draft'})
                    draft_key = dep_rel
    ...
    for draft_key, refs in draft_ref_counts.items():
        if len(refs) >= LRC_HIGH_REF_THRESHOLD:
            ...
            findings.append({'file': draft_rel, 'severity': 'MEDIUM', 'violation': f'LRC-004: draft 文件被 {len(refs)} 个 active 文件引用（阈值: {LRC_HIGH_REF_THRESHOLD}）——实质活跃，应评估升格', 'draft_module_id': draft_mid, 'reference_count': len(refs), 'referenced_by': refs[:10]})
...
def main() -> None:
    ...
    parser.add_argument('--scope', default='docs', help='扫描范围: docs（默认，docs/ 下所有 .md）| policies（仅 docs/01_policies_and_standards/）')
    ...
    for f in lrc001[:10]:
        print(f'\n  [MEDIUM] {f['file']}\n     LRC-001: 引用 draft 文件 {f.get('target_module_id') or f.get('target_file')}', file=sys.stderr)
    for f in lrc004[:10]:
