...
import argparse
import yaml
CONTRACTS_YAML = REPO_ROOT / 'docs/02_enterprise_architecture/target-architecture/architecture-model/contracts/cross-layer-contracts.yaml'
...
def build_impact_matrix(data: dict) -> dict[str, dict]:
    ...
    for ctr in data.get('contracts', []):
        ...
        matrix[cid] = {'name': ctr.get('name', cid), 'source': source, 'targets': targets, 'priority': ctr.get('priority', '?'), 'field_count': len(ctr.get('fields', []))}
    return matrix
    ...
    if args.field:
        ...
        if consumers:
            ...
            for c in consumers:
                print(f'  - {c}')
    print(f'\n建议操作:')
    print(f'  1. 更新 cross-layer-contracts.yaml 并 bump schema_version')
    print(f'  2. 运行 python scripts/context/generate_architecture_context.py 重新生成上下文包')
    print(f'  3. 运行 pytest tests/architecture/ 确认架构适应度函数仍然通过')
    print(f'  4. 运行 python -m pytest tests/unit/test_enforcer.py 确认 enforcer 测试通过')
