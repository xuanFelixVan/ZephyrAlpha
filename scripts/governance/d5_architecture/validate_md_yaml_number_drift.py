...
def get_yaml_truth() -> dict:
    ...
    if idx:
        ...
        truth['total_modules'] = gs.get('total_modules_p0', 0) + gs.get('total_modules_p1', 0) + gs.get('total_modules_p2', 0) + gs.get('total_modules_p3', 0) + gs.get('total_modules_deferred', 0)
    ...
    for fpath in sorted(md_files):
        ...
        for m in re.finditer('(\\d+)\\s*大\\s*(?:核心\\s*)?服务', content):
            ...
            if truth.get('core_services') and n != truth['core_services']:
                findings.append({'dim': 1, 'file': rel, 'found': n, 'expected': truth['core_services'], 'context': m.group(0)})
