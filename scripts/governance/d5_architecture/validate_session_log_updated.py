...
REQUIRED_SECTIONS = [('(?:做了什么|做了什么|完成[了的]|完成[了的]|做了什么|DONE|Completed|Accomplished)', '三段论 — 做了什么'), ('(?:为什么|原因|理由|为什么这样做|WHY|Rationale|Reason)', '三段论 — 为什么这样做'), ('(?:下一步|接下来|待办|TODO|Next|Pending|下一步做什么)', '三段论 — 下一步做什么')]
...
def main() -> None:
    ...
    parser.add_argument('--warn-hours', type=int, default=WARN_HOURS_STALE, help=f'警告阈值小时数（默认 {WARN_HOURS_STALE}）')
    ...
    if age_hours > WARN_HOURS_NO_LOG:
        ...
        findings.append({'file': str(latest_log.relative_to(REPO_ROOT)), 'line': 0, 'pattern': f'超过 {WARN_HOURS_NO_LOG}h 无 session log', 'matched': f'last_modified={mtime.isoformat()}, age_hours={age_hours:.1f}'})
    elif age_hours > args.warn_hours:
        print(f'  WARNING: Session Log 已 {age_hours:.1f} 小时未更新（>={args.warn_hours}h 阈值）', file=sys.stderr)
        findings.append({'file': str(latest_log.relative_to(REPO_ROOT)), 'line': 0, 'pattern': f'Session Log 超过 {args.warn_hours}h 未更新', 'matched': f'last_modified={mtime.isoformat()}, age_hours={age_hours:.1f}'})
    else:
    ...
    if not structure['has_structure']:
        ...
        findings.append({'file': str(latest_log.relative_to(REPO_ROOT)), 'line': 0, 'pattern': 'Session Log 缺少三段论结构', 'matched': f'missing={structure['missing']}'})
    else:
