...
REGISTRY_PATH = REPO_ROOT / 'docs' / '01_policies_and_standards' / 'governance' / 'ai' / 'ai-autonomy-authority-registry.md'
...
HEADER_ROW_KEYWORDS = {'模块', '组件', '路径', '权限', '判定理由', '审批要求', '层级', '语义', 'AI 自主修改权限', '修改流程', 'layer', 'module', 'authority', 'rationale'}
SKIP_FIRST_CELL_PATTERNS = [re.compile('^`[^`]*`$'), re.compile('^（'), re.compile('^见\\s'), re.compile('^同上$'), re.compile('^来源文档$')]
...
def parse_registry_tables(path: Path) -> list[AuthorityEntry]:
    ...
    for line in text.splitlines():
        ...
        for pat in SKIP_FIRST_CELL_PATTERNS:
            ...
            entries.append(AuthorityEntry(module=first_cell, authority=authority_val, rationale=rationale, section=current_section))
    return entries
