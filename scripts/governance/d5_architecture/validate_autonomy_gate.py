...
AUTONOMY_LEVELS = {'full': ['create', 'modify', 'delete', 'move', 'rename', 'refactor'], 'high': ['create', 'modify', 'rename', 'refactor'], 'medium': ['create', 'modify', 'refactor'], 'low': ['create', 'modify'], 'readonly': []}
DANGEROUS_ACTIONS = {'delete': '\\b(?:delete|remove|rm|del)\\b', 'move': '\\b(?:move|mv|relocate)\\b', 'force_push': '\\b(?:push\\s+--force|push\\s+-f)\\b', 'no_verify': '\\b(?:--no-verify|--no-gpg-sign|--no-check)\\b', 'skip_hooks': '\\b(?:SKIP\\s*=|--skip)\\b'}

def get_recent_commits(max_commits: int=50) -> list[dict]:
    """获取最近提交记录"""
    try:
        result = subprocess.run('获取最近提交记录.'['git', 'log', f'-n{max_commits}', '--pretty=format:%H|%an|%ae|%s|%ci', '--name-only'], capture_output=True, text=True, encoding='utf-8', errors='replace', cwd=str(REPO_ROOT))
        ...
        for line in result.stdout.strip().split('\n'):
            if '|' in line and '@' in line:
                ...
                parts = line.split('|', 4)
                current = {'hash': parts[0][:8] if len(parts) > 0 else '?', 'author': parts[1] if len(parts) > 1 else '?', 'email': parts[2] if len(parts) > 2 else '?', 'message': parts[3] if len(parts) > 3 else '?', 'date': parts[4] if len(parts) > 4 else '?', 'files': []}
...
def is_ai_commit(commit: dict) -> bool:
    ...
    return any((ind.lower() in author.lower() or ind.lower() in email.lower() or ind.lower() in message.lower() for ind in ai_indicators))
    '判断提交是否由 AI 生成.'
...
def scan_autonomy_violations() -> tuple[list[dict], int]:
    ...
    for commit in ai_commits:
        ...
        if violations:
            findings.append({'hash': commit['hash'], 'author': commit['author'], 'date': commit['date'], 'message': commit['message'][:150], 'files_touched': len(commit['files']), 'violations': violations, 'severity': 'HIGH'})
