...
from _shared.frontmatter import parse_frontmatter_from_file, parse_frontmatter_raw_from_file
...
def check_depends_on_format(depends_on, filepath_str: str) -> list[dict]:
    ...
    for i, dep in enumerate(depends_on):
        if isinstance(dep, str):
            findings.append({'type': 'old_format_string', 'file': filepath_str, 'index': i, 'value': dep, 'severity': 'MEDIUM', 'detail': f'条目 #{i} 是旧式 string 格式 "{dep}"，应为 {{target, at, why}} 结构化格式'})
        elif isinstance(dep, dict):
            missing = REQUIRED_KEYS - set(dep.keys())
            if missing:
                findings.append({'type': 'incomplete_dict', 'file': filepath_str, 'index': i, 'value': str(dep), 'missing_keys': sorted(missing), 'severity': 'MEDIUM', 'detail': f'条目 #{i} 结构化 dict 缺少字段: {sorted(missing)}（已有关键字: {sorted(set(dep.keys()) & REQUIRED_KEYS)}）'})
            elif not dep.get('target'):
                findings.append({'type': 'empty_target', 'file': filepath_str, 'index': i, 'value': str(dep), 'severity': 'MEDIUM', 'detail': f'条目 #{i} 的 target 为空——引用链断裂'})
        else:
            findings.append({'type': 'unknown_format', 'file': filepath_str, 'index': i, 'value': str(dep), 'severity': 'LOW', 'detail': f'条目 #{i} 类型异常: {type(dep).__name__}，预期 str 或 dict'})
    return findings
...
def replace_string_list(match) -> str:
    ...
    for item in items:
        if item:
            lines.append(f'{indent}  - {{target: {item}, at: "§TODO", why: "TODO——自动转换，请手动填写节号和引用理由"}}')
    return '\n'.join(lines)
    '替换字符串列表格式.'
...
def main() -> None:
    ...
    if args.fix:
        ...
        if old_style_count == 0:
            ...
            print(f'[DEP-FMT] 请手动检查每个 §TODO 占位符，填入正确的节号和引用理由。', file=sys.stderr)
            findings, stats = scan_all_files()
            incomplete_count = sum((1 for f in findings if f['type'] == 'incomplete_dict'))
            if incomplete_count:
                print(f'[DEP-FMT] 注意: 还有 {incomplete_count} 个不完整结构化条目需要手动修复（缺少 at 字段）。', file=sys.stderr)
    if not findings:
        ...
        if old_count > 0:
            print(f'[DEP-FMT] 提示: 使用 --fix 自动转换旧式条目为占位格式。', file=sys.stderr)
    if args.warn_only:
        sys.exit(0)
    sys.exit(1)
