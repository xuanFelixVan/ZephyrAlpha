...
def check_dim5_field_stage_completeness(contract) -> list[dict]:
    ...
    return ('PASS', f'stage counts: draft={stage_counts['draft']}, active={stage_counts['active']}, deprecated_only={stage_counts['deprecated_only']}')
...
def main() -> None:
    ...
    dimensions = [('DIM-1 module_id regex', check_dim1_module_id_regex), ('DIM-2 doc_type consistency (VR-002)', check_dim2_doc_type_consistency), ('DIM-3 VR sequential numbering', check_dim3_vr_sequential), ('DIM-4 total_vr_rules', check_dim4_total_vr_count), ('DIM-5 field stage completeness', check_dim5_field_stage_completeness), ('DIM-6 vocab derived_from reachable', check_dim6_vocab_derived_from), ('DIM-7 doc_type subset of vocabulary', check_dim7_doc_type_subset)]
    fails = 0
