...
__manifest__ = """
args: []
description: 'G7: 从 depgraph 库 domains 表生成域容量报告MD文档'
dimensions:
- D5
priority: P2
timeout_seconds: 60
...
def generate_capacity_report() -> str:
    ...
    near_capacity = [
        ...
        for d in domains
        if d["max_modules"] > 0
        and d["production_nodes"] / d["max_modules"] > 0.8
        and d["production_nodes"] <= d["max_modules"]
    ]
    ...
    if empty_domains:
        ...
        for d in empty_domains:
            lines.append(
                f"| {d['domain_id']} | {get_domain_name_zh(d['domain_id'], d['domain_name'])} | {d['layer_id']} | {d['max_modules']} |"
            )
        lines.append("")
