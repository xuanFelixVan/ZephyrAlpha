...
# [MODULE] scripts.governance.d5_architecture.generators.generate_trigger_wiring_view
# [DOMAIN] D-GOVERNANCE
...
def update_trigger_router(wiring: list[dict]) -> tuple[int, int]:
    ...
    tmp_path = f"{TRIGGER_ROUTER_PATH}.{os.getpid()}.tmp"
    try:
        Path(tmp_path).write_text("\n".join(new_lines), encoding="utf-8")
        os.replace(tmp_path, TRIGGER_ROUTER_PATH)
