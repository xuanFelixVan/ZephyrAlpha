...
NODE_TYPES = {
    "application", "module", "package", "script", "service", "library",
    "config", "data", "gate", "domain", "aggregate"
}

LAYERS = {
    "L0_infrastructure", "L1_foundation", "L2_domain", "L3_application",
    "shared", "contracts", "meta", "domain_integration"
}

DEP_TYPES = {
    "import_depends", "blueprint_depends", "script_depends", "data_depends",
    "runtime_depends", "config_depends", "test_depends", "event_depends",
    "contract_depends", "shared_kernel"
}
...
COMPLETENESS_VALUES = {
    ...
    "incomplete_third_party_only", "unknown"
}
...
def main() -> int:
    ...
    if dep_graph_json.exists():
        try:
            import json
            with open(dep_graph_json, "r", encoding="utf-8") as f:
                data = json.load(f)
    ...
    cross_module_yaml = REPO_ROOT / "docs" / "01_policies_and_standards" / "_registry" / "catalogs" / "cross-module-dependency-registry.yaml"
