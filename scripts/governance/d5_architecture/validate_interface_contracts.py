    ...
    for contract in contracts:
        ...
        for field in REQUIRED_CONTRACT_FIELDS:
            if field not in contract or contract[field] is None:
                findings.append({'contract_id': cid, 'type': 'MISSING_FIELD', 'detail': f'契约缺少必填字段: {field}', 'severity': 'HIGH'})
        if 'contract_id' in contract:
            if not CONTRACT_ID_PATTERN.match(str(contract['contract_id'])):
                findings.append({'contract_id': cid, 'type': 'INVALID_CONTRACT_ID', 'detail': f'contract_id 格式错误（应为 {{provider}}.{{consumer}}.{{name}}）', 'severity': 'MEDIUM'})
        status = contract.get('status', '')
        if status and status not in VALID_CONTRACT_STATUSES:
            findings.append({'contract_id': cid, 'type': 'INVALID_STATUS', 'detail': f"契约 status='{status}' 不在合法枚举中（合法值: {', '.join(sorted(VALID_CONTRACT_STATUSES))}）", 'severity': 'HIGH'})
        version = contract.get('version', '')
        if version and (not SEMVER_PATTERN.match(str(version))):
            findings.append({'contract_id': cid, 'type': 'INVALID_SEMVER', 'detail': f"version='{version}' 不符合 semver 格式（X.Y.Z）", 'severity': 'MEDIUM'})
        if status == 'deprecated':
            consumers = contract.get('consumers', [])
            if isinstance(consumers, list) and len(consumers) > 0:
                findings.append({'contract_id': cid, 'type': 'DEPRECATED_WITH_CONSUMERS', 'detail': f'deprecated 契约仍有 {len(consumers)} 个 consumer', 'severity': 'MEDIUM'})
    return findings
    '校验合约.'
