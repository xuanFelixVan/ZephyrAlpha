...
from _shared.constants import REPO_ROOT, GOV_DOCS_DIR, SCAN_EXTENSIONS_MD
...
def scan_three_way_consistency(docs_dir: Path, registry_path: Path) -> tuple[list[dict], int, int]:
    ...
    for filepath in iter_files(docs_dir, extensions=SCAN_EXTENSIONS_MD):
        ...
        if bq and fm_status and bq_status and (fm_status != bq_status):
            findings.append({'file': rel, 'module_id': mid, 'severity': 'MEDIUM', 'violation': f'frontmatter.status={fm_status} ≠ blockquote.status={bq_status}', 'check_type': 'fm_vs_blockquote', 'field': 'status'})
        if bq and fm_version and bq_version and (fm_version != bq_version):
            findings.append({'file': rel, 'module_id': mid, 'severity': 'MEDIUM', 'violation': f'frontmatter.version={fm_version} ≠ blockquote.version={bq_version}', 'check_type': 'fm_vs_blockquote', 'field': 'version'})
        if reg_entry and fm_status and reg_status and (fm_status != reg_status):
            findings.append({'file': rel, 'module_id': mid, 'severity': 'MEDIUM', 'violation': f'frontmatter.status={fm_status} ≠ registry.status={reg_status}', 'check_type': 'fm_vs_registry', 'field': 'status'})
        if reg_entry and fm_version and reg_version and (fm_version != reg_version):
            findings.append({'file': rel, 'module_id': mid, 'severity': 'HIGH', 'violation': f'frontmatter.version={fm_version} ≠ registry.version={reg_version}', 'check_type': 'fm_vs_registry', 'field': 'version'})
        if bq and reg_entry and bq_version and reg_version and (bq_version != reg_version):
            findings.append({'file': rel, 'module_id': mid, 'severity': 'MEDIUM', 'violation': f'blockquote.version={bq_version} ≠ registry.version={reg_version}', 'check_type': 'blockquote_vs_registry', 'field': 'version'})
