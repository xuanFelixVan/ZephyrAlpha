...
FORBIDDEN_CLASS_NAMES_BY_LAYER = {'l00_data_source': {'NormalizedMarketData', 'MarketData', 'Tick', 'Bar', 'TradeData', 'Candle', 'Quote'}, 'l02_alpha_factor': {'FactorSignal', 'Factor', 'FactorValue', 'AlphaSignal', 'Signal'}, 'l04_risk_management': {'RiskLimits', 'Limit', 'Constraint', 'RiskConstraint'}, 'l05_portfolio_construction': {'Order', 'OrderRequest', 'OrderCmd', 'TradeInstruction'}, 'l06_trade_execution': {'Order', 'OrderRequest', 'OrderCmd', 'Fill', 'FillReport', 'Execution', 'Position', 'PositionSnapshot', 'Holding'}, 'l07_post_trade_analytics': {'Position', 'PositionSnapshot', 'Holding'}}
MONEY_FIELD_PATTERNS = {'price', 'amount', 'cost', 'fee', 'commission', 'quantity', 'volume', 'value', 'notional', 'position_value', 'market_value', 'nav', 'cash', 'balance', 'equity', 'open', 'high', 'low', 'close', 'limit_price', 'fill_price', 'avg_price', 'entry_price', 'exit_price'}
ALLOWED_CONTRACT_DIR = 'shared/contracts'
ALLOWED_SHARED_DIR = 'shared'
LAYER_DIRS = ['l00_data_source', 'l01_infrastructure', 'l02_alpha_factor', 'l03_signal_generation', 'l04_risk_management', 'l05_portfolio_construction', 'l06_trade_execution', 'l07_post_trade_analytics']
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    if node.name in forbidden_names:
                        ...
                        findings.append({'file': rel, 'layer': layer_name, 'class_name': node.name, 'line': node.lineno, 'violation': f"{layer_name} 自造了数据类型 '{node.name}'——请使用 shared/contracts/ 中的对应 CTR 契约", 'severity': 'HIGH'})
    return findings
                ...
                if isinstance(node, ast.AnnAssign) and node.annotation:
                    ...
                    if field_name and _is_money_field(field_name):
                        if _annotation_is_float(node.annotation):
                            rel = str(filepath.relative_to(REPO_ROOT)).replace('\\', '/')
                            findings.append({'file': rel, 'layer': layer_name, 'field_name': field_name, 'line': node.lineno, 'violation': f"金额相关字段 '{field_name}' 使用了 float 注解——必须使用 Decimal（CTR 全域强制，参见 CTR-001~006）", 'severity': 'HIGH'})
                    ...
                    for arg in node.args.args:
                        if arg.annotation:
                            ...
                            if _is_money_field(arg_name) and _annotation_is_float(arg.annotation):
                                rel = str(filepath.relative_to(REPO_ROOT)).replace('\\', '/')
                                findings.append({'file': rel, 'layer': layer_name, 'field_name': f'{node.name}.{arg_name}', 'line': node.lineno, 'violation': f"函数 '{node.name}' 的金额参数 '{arg_name}' 使用了 float 注解——必须使用 Decimal", 'severity': 'HIGH'})
    return findings
            ...
            for node in ast.walk(tree):
                if isinstance(node, (ast.Import, ast.ImportFrom)):
                    ...
                    if import_path and _is_cross_layer_import(import_path, layer_name):
                        rel = str(filepath.relative_to(REPO_ROOT)).replace('\\', '/')
                        findings.append({'file': rel, 'layer': layer_name, 'import_path': import_path, 'line': node.lineno, 'violation': f"跨层导入 '{import_path}' 直接指向了其他层目录——跨层数据交换必须通过 shared/contracts/", 'severity': 'MEDIUM'})
    return findings
...
def _is_cross_layer_import(import_path: str, current_layer: str) -> bool:
    ...
    if ALLOWED_CONTRACT_DIR in normalized or ALLOWED_SHARED_DIR == normalized.split('/')[-1]:
        return False
...
def check_required_imports() -> list[dict]:
    ...
    findings: list[dict] = []
    src_dir = REPO_ROOT / 'src' / 'zephyr'
    required_imports = {'l00_data_source': {'shared.contracts.market_data': 'NormalizedMarketData'}, 'l02_alpha_factor': {'shared.contracts.market_data': 'NormalizedMarketData', 'shared.contracts.factor_signal': 'FactorSignal'}, 'l04_risk_management': {'shared.contracts.risk_limits': 'RiskLimits'}, 'l05_portfolio_construction': {'shared.contracts.order': 'Order'}, 'l06_trade_execution': {'shared.contracts.order': 'Order', 'shared.contracts.fill': 'Fill', 'shared.contracts.position': 'PositionSnapshot'}}
...
def main() -> None:
    ...
    parser.add_argument('--checks', type=str, default='all', help='检查项：all | class_names | money_fields | cross_imports')
