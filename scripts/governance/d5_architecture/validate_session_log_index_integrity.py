...
def write_blind_spot_timeline(timeline: list[dict], resolved: int, open_count: int) -> None:
    ...
    with open(BLIND_SPOT_FILE, "w", encoding="utf-8") as fh:
...
def write_index(index_data: dict) -> None:
    ...
    data_lines = yaml.dump(
        index_data, allow_unicode=True, default_flow_style=False, sort_keys=False, width=120
    ).split("\n")
...
def validate(index_data: dict, disk_sessions: dict[str, Path], warn_only: bool) -> int:
    ...
    if declared_total != actual_total:
        errors.append(
            f"TOTAL-COUNT-MISMATCH: index={declared_total}, disk={actual_total}"
        )
...
def main() -> None:
    ...
    parser.add_argument("--generate", action="store_true", help="自动从 session log YAML 汇总生成 blind_spot_timeline + by_date")
    ...
    if args.generate:
        ...
        if BLIND_SPOT_FILE.exists():
            try:
                with open(BLIND_SPOT_FILE, "r", encoding="utf-8") as fh:
        ...
        print(f"[GENERATE] OK: {len(disk_sessions)} sessions → by_date synced, "
