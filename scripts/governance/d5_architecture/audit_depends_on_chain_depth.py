...
def analyze_chains(adjacency: dict[str, set[str]], reverse_adj: dict[str, set[str]], depths: dict[str, int], files_with_deps: set[str], file_to_module_id: dict[str, str], max_depth: int) -> list[dict]:
    """分析超标链路。"""
    findings = []
    ...
    for root in sorted(root_files):
        ...
        if depth_val > max_depth:
            findings.append({'type': 'chain_violation', 'chain_id': chain_id, 'root_file': root, 'root_module_id': file_to_module_id.get(root, ''), 'depth': depth_val, 'max_allowed': max_depth, 'chain': ' → '.join(node_labels), 'chain_files': chain, 'severity': 'MEDIUM' if depth_val == max_depth + 1 else 'HIGH'})
    return findings
...
def main() -> None:
    ...
    if args.verbose:
        for vl in violations:
            print(f'\n  [{vl['severity']}] 链路#{vl['chain_id']} — {vl['depth']}层 (上限{vl['max_allowed']})', file=sys.stderr)
