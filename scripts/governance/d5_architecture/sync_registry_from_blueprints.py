...
def rebuild_summary(entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "last_recomputed": datetime.now(timezone.utc).strftime("%Y-%m-%d"),
    }


def run_dry_run(registry: dict[str, Any], new_entries: list[dict[str, Any]],
                file_paths: list[Path]) -> int:
...
def run_write(
    ...
    *,
    ...
    old_entries: list[dict[str, Any]] = (
        [dict(e) for e in old_entries_raw] if isinstance(old_entries_raw, list) else []
    )

    reg_meta = registry.get("registry", {})
    reg_meta["total_blueprints"] = len(new_entries)
    reg_meta["last_updated"] = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    ...
    if "metadata" not in registry or not isinstance(registry["metadata"], dict):
        registry["metadata"] = {}
    registry["metadata"]["generated_at"] = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    ...
    BLUEPRINT_REGISTRY_PATH.write_text(content, encoding="utf-8")
