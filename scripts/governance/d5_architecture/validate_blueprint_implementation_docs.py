...
IMPLEMENTATION_SECTION_TITLES = ['实际代码实现情况', 'Code Implementation Status', '代码实现情况', '已实现代码完整路径索引']
SECTION_RE = re.compile('^##\\s+(?:\\d+\\.\\s+)?' + '(?:' + '|'.join((re.escape(t) for t in IMPLEMENTATION_SECTION_TITLES)) + ')', re.IGNORECASE | re.MULTILINE)

FILE_PATH_RE = re.compile(r'`(src/zephyr/[^`*]+\.(?:py|yaml|yml|json|toml))`|`(tests/[^`*]+\.py)`|`(config/[^`*]+\.(?:yaml|yml|json))`')
...
def _has_negative_indicator(content: str, path: str) -> bool:
    ...
    return bool(re.search(r'❌|未实现|not.implemented|待实现|待创建|not.started|Phase\s*[2-9]|📋|新模块|计划', window, re.IGNORECASE))
...
def main() -> None:
    ...
    for bp in sorted(blueprints):
        ...
        missing_files = [
            (p, exists) for p, exists in disk
            if not exists and not _has_negative_indicator(content, p)
        ]
    ...
    if not errors and not warnings:
        print(f'OK: All {len(blueprints)} blueprints have consistent construction_progress ↔ implementation documentation + verified file paths.')
        sys.exit(0)
