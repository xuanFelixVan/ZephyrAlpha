...
SCAN_ENTRIES = [('layers', ARCH_MODEL / 'layers', True), ('infra', ARCH_MODEL / 'infra', True), ('scripts', ARCH_MODEL / 'scripts', True), ('frontend', ARCH_MODEL / 'frontend', True), ('cross-cutting', ARCH_MODEL / 'cross-cutting', True), ('technology', ARCH_MODEL / 'technology', True)]
...
def validate_yaml_summaries() -> tuple[bool, list[str]]:
    ...
    for dir_label, dir_path, _ in SCAN_ENTRIES:
        ...
        for yaml_file in sorted(dir_path.glob('*.yaml')):
            ...
            declared_total = summary.get('total') or summary.get('total_technologies') or summary.get('total_capabilities') or summary.get('total_services')
            ...
            if isinstance(declared_by_priority, dict) and declared_by_priority:
                ...
                for priority, actual_count in sorted(actual.items()):
                    if priority not in declared_by_priority:
                        errors.append(f'{file_label}: priority={priority} 存在 {actual_count} 个，summary.by_priority 未声明')
        ...
        if data:
            ...
            if isinstance(registered_ids, list):
                ...
                if declared is not None and declared != actual:
                    errors.append(f'module-id-registry.yaml: total_registered={declared}，实际 registered_ids 条目数={actual}')
        ...
        if data:
            ...
            if isinstance(global_stats, dict):
                ...
                if declared is not None:
                    ...
                    if declared != actual:
                        errors.append(f'_index.yaml: global_stats.total_partitions={declared}，实际 partitions 条目数={actual}')
                cross_p0 = 0
                cross_p1 = 0
                cross_p2 = 0
                cross_p3 = 0
