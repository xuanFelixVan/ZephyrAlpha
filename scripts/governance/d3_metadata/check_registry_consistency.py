...
try:
    from zephyr.l01_infrastructure.script_system.finding import Finding, FindingCollection, Severity, Dimension, BlastRadius, RemediationAction
...
def get_nested_value(data: dict, yaml_path: str) -> Optional[str]:
    """get nested value"""
...
def get_registry_value(registry_data: dict, yaml_path: str, module_id: str) -> Optional[str]:
    """get registry value"""
...
def get_physical_value(module_path_relative: str, frontmatter_key: str) -> Optional[str]:
    """get physical value"""
...
def get_module_path(ror: dict, module_id: str) -> Optional[str]:
    """get module path"""
    ...
    for module_id in sorted(module_ids):
        ...
        if len(unique_values) > 1:
            ...
            if ssoT_source == 'REG-001':
                for field in fields:
                    val = get_registry_value(load_yaml(REPO_ROOT / next((r for r in ror['registries'] if r['id'] == 'REG-001'))['path']), 'modules[].blueprint.status' if field == 'status' else 'modules[].priority' if field == 'priority' else f'modules[].{field}', module_id)
                    if val:
                        ssoT_value = val
            ...
            f = Finding(dimension=Dimension.D3, severity=severity, category=f'跨登记表一致性 — {rule_id}', target_file=f'docs/{module_path}/blueprint.md', description=description, evidence=evidence, blast_radius=blast, remediation_action=RemediationAction.FIX, remediation_priority='P0' if severity == Severity.CRITICAL else 'P1')
            collection.add(f)
    return collection
