...
def check_migration_duplicates() -> list[dict]:
    ...
    for f in iter_files(GOV_DOCS_DIR, extensions=('.yaml', '.yml', '.md')):
        ...
        if is_stub:
            ...
            if line_count > 25:
                _warnings.append(f'{f.relative_to(GOV_DOCS_DIR)}: deprecated stub has {line_count} lines (should be <=20)')
        else:
            registry_copy = registry_files[f.name]
            _errors.append(f'{f.relative_to(GOV_DOCS_DIR)}: active file also exists at {registry_copy.relative_to(GOV_DOCS_DIR)} — migration incomplete')
    'check migration duplicates.'
