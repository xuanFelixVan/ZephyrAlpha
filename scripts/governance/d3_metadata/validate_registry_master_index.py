...
MASTER_INDEX_PATH = REPO_ROOT / 'docs' / '01_policies_and_standards' / '_registry' / 'catalogs' / 'registry-master-index.yaml'
...
def validate_master_index(data: dict[str, Any], verbose: bool=False) -> tuple[list[str], list[dict[str, Any]]]:
    ...
    if declared_total != actual_total:
        findings.append({'type': 'warn', 'target': 'total_registries', 'issue': f'声明 {declared_total} 张登记表，实际 registries 列表有 {actual_total} 条'})
        if verbose:
    ...
    for reg in registries:
        ...
        if not path_str:
            findings.append({'type': 'error', 'registry_id': rid, 'issue': f'physical_path 为空——无法校验此登记表'})
            continue
        full_path = REPO_ROOT / path_str
        if not full_path.exists():
            finding = {'type': 'error' if status == 'active' else 'warn', 'registry_id': rid, 'name': name, 'issue': f'登记表文件不存在: {path_str}', 'declared_status': status}
        ...
        if entry_key:
            ...
            if actual_entries > 0 and actual_entries != entry_count:
                findings.append({'type': 'warn', 'registry_id': rid, 'name': name, 'issue': f"entry_count 不一致: 声明={entry_count} 实际={actual_entries} (key='{entry_key}')"})
                if verbose:
                    print(f'  [WARN]  {rid}: entry_count 声明={entry_count} 实际={actual_entries}', file=sys.stderr)
            elif actual_entries == 0 and entry_count > 0:
                findings.append({'type': 'info', 'registry_id': rid, 'name': name, 'issue': f"entry_count 校验跳过——key='{entry_key}' 未在 YAML 中找到（可能是 key 名变更）"})
        ...
        if isinstance(depends_on, list):
            for dep in depends_on:
                ...
                if not target or target.startswith('http') or target.startswith('NIST') or target.startswith('OWASP') or target.startswith('ITIL'):
                    continue
        ...
        for cat, ids in by_cat.items():
            ...
            for rid in ids:
                if rid not in all_reg_ids:
                    findings.append({'type': 'warn', 'registry_id': rid, 'issue': f'quick_lookup.by_category.{cat} 中的 {rid} 不在 registries 列表中'})
    return (errors, findings)
    'validate master index.'
...
def _auto_detect_entry_key(data: dict[str, Any]) -> str | None:
    skip_keys = {'depends_on', 'tags', 'keywords', 'aliases', 'uncovered_ids', 'changes', 'changelog', 'blockers', 'allowed_subdirs', 'forbidden_doc_types', 'allowed_doc_types'}
    best_key = None
    best_len = 0
...
def main() -> None:
    ...
    print(f'[validate_registry_master_index] {len(data.get('registries', []))} 张登记表, {errs} 错误, {warns} 警告', file=sys.stderr)
    if errors:
        for err in errors:
