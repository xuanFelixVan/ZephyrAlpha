...
def _try_old_layer_mapping(declared_path: str) -> Optional[str]:
...
def _try_prefix_fix(declared_path: str) -> Optional[str]:
...
def _try_module_name_search(declared_path: str, index: BlueprintIndex) -> Optional[str]:
...
def _try_module_id_search(module_id: str, index: BlueprintIndex) -> Optional[str]:
...
def _try_combined_old_layer_and_prefix(declared_path: str) -> Optional[str]:
...
def process_file(
    ...
    try:
        ...
        results.append(FixResult(
            filepath=filepath, old_path="", new_path=None,
            status="UNRESOLVED", reason=f"读取失败: {e}",
        ))
        return results
    ...
    for match in matches:
        ...
        if strategy == "ALREADY_OK":
            results.append(FixResult(
                filepath=filepath, old_path=declared_path,
                new_path=declared_path, status="ALREADY_OK",
            ))
            continue
        ...
        if new_path:
            old_full = full_line
            new_full = (
                f"{match.group(1)}{module_id}{match.group(3)}{new_path}{match.group(5)}"
            )
            ...
            results.append(FixResult(
                filepath=filepath, old_path=declared_path,
                new_path=new_path, status="FIXED",
                reason=strategy,
            ))
        else:
            results.append(FixResult(
                filepath=filepath, old_path=declared_path,
                new_path=None, status="UNRESOLVED",
...
def main() -> int:
    ...
    with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        futures = {
            executor.submit(process_file, fp, index, args.apply): fp
            for fp in py_files
        }
