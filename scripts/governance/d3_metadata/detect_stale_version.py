...
def get_staged_modified_files() -> list[str]:
    ...
    try:
        result = subprocess.run('get staged modified files.'['git', 'diff', '--cached', '--name-only', '--diff-filter=M'], capture_output=True, text=True, cwd=str(REPO_ROOT), timeout=30)
    ...
    for rel_path in staged:
        ...
        if old_ver and new_ver and (old_ver == new_ver):
            if old_content != new_content:
                findings.append({'file': rel_path, 'type': 'STALE_VERSION', 'detail': f'内容已变更但 version 未更新（仍为 {old_ver}）', 'severity': 'MEDIUM'})
        if old_date and new_date and (old_date == new_date):
            if old_content != new_content:
                findings.append({'file': rel_path, 'type': 'STALE_DATE', 'detail': f'内容已变更但 date 未更新（仍为 {old_date}）', 'severity': 'MEDIUM'})
    return findings
    'check stale versions.'
