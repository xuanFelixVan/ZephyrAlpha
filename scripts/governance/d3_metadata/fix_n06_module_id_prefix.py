...
VALID_SCOPE_PREFIX_RE = re.compile(
    r"^(ADR|CP|KE|STD|DW|SRC|OPS|MOD|PSP|GOV|ARCH|VIEW)-[A-Z0-9-]*\d+$"
)
...
EXCLUDE_DIRS: frozenset[str] = frozenset({
    "__pycache__", ".git", ".ruff_cache", ".mypy_cache",
    ".pytest_cache", "node_modules", ".venv", "venv",
    ".tox", ".eggs", ".idea", ".vscode", ".trae",
    ".ailocks", ".aidrafts", "_reorg_snapshots", "archive",
})
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="修复 N-06 module_id scope 前缀违规"
    )
    parser.add_argument(
        "--apply",
        action="store_true",
