...
def check_uniqueness(module_id: str) -> list[str]:
    ...
    if len(id_map[module_id]) > 1:
        ...
        findings.append(f"INJ-001 FAIL: module_id '{module_id}' is not unique — found in {len(id_map[module_id])} files: {locations}")
    return findings
                        ...
                        if isinstance(arg, ast_mod.Constant) and isinstance(arg.value, str):
                            if arg.value.startswith(("GOV-", "PS-", "OPS-", "MOD-", "META-")):
                                if arg.value not in registry_ids:
                                    ...
                                    findings.append(f"HC-1 WARNING: {rel} references module_id '{arg.value}' not found in any registry file")
    return findings
...
def main() -> None:
    ...
    parser.add_argument("--check-ast", action="store_true", help="Check AST code for unregistered module_id references (HC-1)")
