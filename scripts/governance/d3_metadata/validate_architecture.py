...
from _shared.constants import REPO_ROOT, EXCLUDE_DIRS, SCAN_EXTENSIONS_MD_YAML
from _shared.encoding import ensure_utf8_stdout
ensure_utf8_stdout()
DEFAULT_CONTRACT = REPO_ROOT / 'docs' / '01_policies_and_standards' / '_registry' / 'contracts' / 'architecture-contract.yaml'
...
def get_required_fields_for_doc_type(doc_type: str) -> set[str]:
    ...
    rule_doc_fields = common | {'layer', 'owner', 'classification', 'language', 'created_by', 'date', 'ttl', 'summary', 'tags', 'rule_form', 'scope', 'stability', 'verifiability'}
    mapping = {'policy': rule_doc_fields, 'standard': rule_doc_fields, 'operational_rule': rule_doc_fields, 'protocol': rule_doc_fields, 'register': common, 'index': {'title', 'doc_type', 'status', 'version'}, 'template': common, 'terminology': common, 'vocabulary': {'title', 'doc_type', 'status'}, 'contract': common}
...
def validate_frontmatter(fm: dict, file_path: str, contract: dict, result: ValidationResult):
    ...
    for field_def in fm_schema.get('required_fields', []):
        ...
        if val_str not in allowed:
            result.add_error('VR-002' if name == 'doc_type' else 'VR-005', file_path, f"Field '{name}' value '{val}' not in allowed values: {field_def['allowed_values']}")
...
def validate_doc_type_rule_form_consistency(fm: dict, file_path: str, result: ValidationResult, rule_form_mapping: dict[str, str]):
    ...
    if expected and rule_form != expected:
        result.add_error('VR-004', file_path, f"doc_type='{doc_type}' should have rule_form='{expected}', got '{rule_form}'")
