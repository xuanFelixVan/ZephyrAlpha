...
def main() -> int:
    ...
    if warn_only_count:
        print(f"（另有 {warn_only_count} 个 warn-only 命名违规，不阻断 commit，走 gate-naming-audit CI/manual 清零）")
    # 返回逻辑与原版一致：有违规时根据 warn_only/N-16 决定，无违规时 PASS
    if actual_blocking or warn_only_count:
