...
import argparse
import yaml
SKIP_PATTERNS = [('draft', 'deprecated', 'draft→deprecated 跳过 active'), ('draft', 'archived', 'draft→archived 跳过 active+deprecated'), ('active', 'archived', 'active→archived 跳过 deprecated')]
...
def get_staged_status_changes() -> list[dict]:
    ...
    try:
        'get staged status changes.'
        '获取数据.'
        result = subprocess.run(['git', 'diff', '--cached', '--name-only'], capture_output=True, text=True, cwd=str(REPO_ROOT), timeout=30)
    ...
    for rel_path in staged_files:
        ...
        new_status = None
        try:
            old_result = subprocess.run(['git', 'show', f'HEAD:{rel_path}'], capture_output=True, text=True, cwd=str(REPO_ROOT), timeout=15)
