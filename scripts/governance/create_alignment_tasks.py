...
from zephyr.db.task_repo import TaskRepository, Task
...
TASKS = [
    {
        ...
        "upstream_files": [
            ...
            str(PROJECT_ROOT / "data" / "asset_index" / "project-entity-depgraph.yaml"),
        ],
        "downstream_outputs": [
    ...
    },
    {
        ...
        "downstream_outputs": [
            {"path": str(PROJECT_ROOT / "src" / "zephyr" / "asset_inventory" / "__init__.py"), "desc": "移除自导入"},
        ...
        "allowed_touch": [
            "src/zephyr/asset_inventory/__init__.py",
    ...
    },
    {
        ...
        "description": "escalation_engine/__init__.py与adapter.py、self_test.py之间有2条循环：__init__↔adapter, self_test↔__init__。修复方案：__init__.py改用延迟导入（lazy import），self_test.py改为直接import adapter而非通过__init__。",
        "priority": "P1",
        "source_blueprint": "MOD-INF-022",
        "source_section": "§19",
        "upstream_files": [
            str(PROJECT_ROOT / "src" / "zephyr" / "escalation_engine" / "__init__.py"),
            str(PROJECT_ROOT / "src" / "zephyr" / "escalation_engine" / "adapter.py"),
            str(PROJECT_ROOT / "src" / "zephyr" / "escalation_engine" / "self_test.py"),
        ],
        "downstream_outputs": [
            {"path": str(PROJECT_ROOT / "src" / "zephyr" / "escalation_engine" / "__init__.py"), "desc": "改为延迟导入"},
            {"path": str(PROJECT_ROOT / "src" / "zephyr" / "escalation_engine" / "self_test.py"), "desc": "直接import adapter"},
        ],
        "allowed_touch": [
            "src/zephyr/escalation_engine/__init__.py",
            "src/zephyr/escalation_engine/self_test.py",
        ],
        "forbidden_touch": [
            "src/zephyr/escalation_engine/adapter.py",
        ],
        "applicable_rules": [
            {"module_id": "RULE-ZERO", "section": "§1", "reason": "写入前获取锁"},
        ...
        "context_assembly_manifest": [
            {"file_path": str(PROJECT_ROOT / "src" / "zephyr" / "escalation_engine" / "__init__.py"), "reason": "当前导入结构"},
        ],
        "assigned_model": "deepseek",
        "assigned_pipeline": "A",
        ...
        "acceptance_criteria": [
            ...
            "AC2: python -c 'from zephyr.escalation_engine import EscalationEngine' 成功",
    ...
    },
    {
        ...
        "description": "llm_security/gateway.py与self_protection/l7_validation.py双向import。修复方案：提取公共接口到llm_security/protocol.py（已存在），gateway.py和l7_validation.py都只依赖protocol.py，不直接互相import。",
        "priority": "P1",
        "source_blueprint": "MOD-INF-014",
        "source_section": "§19",
        "upstream_files": [
            str(PROJECT_ROOT / "src" / "zephyr" / "llm_security" / "gateway.py"),
            str(PROJECT_ROOT / "src" / "zephyr" / "llm_security" / "protocol.py"),
        ],
        "downstream_outputs": [
            {"path": str(PROJECT_ROOT / "src" / "zephyr" / "llm_security" / "gateway.py"), "desc": "移除对l7_validation的直接import"},
        ],
        "allowed_touch": [
            "src/zephyr/llm_security/gateway.py",
        ],
        "forbidden_touch": [
            "src/zephyr/llm_security/protocol.py",
        ],
        "applicable_rules": [
            {"module_id": "RULE-ZERO", "section": "§1", "reason": "写入前获取锁"},
        ],
        "context_assembly_manifest": [
            {"file_path": str(PROJECT_ROOT / "src" / "zephyr" / "llm_security" / "gateway.py"), "reason": "当前导入结构"},
            {"file_path": str(PROJECT_ROOT / "src" / "zephyr" / "llm_security" / "protocol.py"), "reason": "已有接口定义"},
        ],
        "assigned_model": "deepseek",
        "assigned_pipeline": "A",
        "pipeline_modules": ["read", "refactor", "verify"],
        ...
        "acceptance_criteria": [
            ...
            "AC2: python -c 'from zephyr.llm_security import gateway' 成功",
            "AC3: diagnose_depgraph.py验证该循环消失",
        ],
    ...
    },
    {
        ...
        "acceptance_criteria": [
            ...
            "AC3: python -c 'from zephyr.l01_infrastructure.system_telemetry.traces import span_stub' 成功",
    ...
    },
    {
        ...
        "source_section": "§19",
        "upstream_files": [
            str(PROJECT_ROOT / "src" / "zephyr" / "system_telemetry" / "auto_bootstrap.py"),
            str(PROJECT_ROOT / "src" / "zephyr" / "rollback" / "phase_manager.py"),
            str(PROJECT_ROOT / "src" / "zephyr" / "rollback" / "phase_check_registry.py"),
            str(PROJECT_ROOT / "src" / "zephyr" / "budget_enforcer" / "__init__.py"),
            str(PROJECT_ROOT / "src" / "zephyr" / "budget_enforcer" / "budget_engine.py"),
        ],
        "downstream_outputs": [
            {"path": str(PROJECT_ROOT / "src" / "zephyr" / "rollback" / "phase_check_registry.py"), "desc": "改为注册制"},
            {"path": str(PROJECT_ROOT / "src" / "zephyr" / "budget_enforcer" / "budget_engine.py"), "desc": "移除对system_telemetry的直接import"},
        ],
        "allowed_touch": [
            "src/zephyr/rollback/phase_check_registry.py",
            "src/zephyr/budget_enforcer/budget_engine.py",
            "src/zephyr/system_telemetry/auto_bootstrap.py",
        ],
        "forbidden_touch": [
        ...
        "context_assembly_manifest": [
            ...
            {"file_path": str(PROJECT_ROOT / "src" / "zephyr" / "budget_enforcer" / "budget_engine.py"), "reason": "当前对system_telemetry的依赖"},
        ],
        "assigned_model": "deepseek",
        "assigned_pipeline": "A",
        ...
        "acceptance_criteria": [
            ...
            "AC2: budget_engine不再直接import system_telemetry",
    ...
    },
    {
        ...
        "description": "model_profiler/profiler.py→model_discovery.py→budget_enforcer/model_router.py→model_profiler/results_writer.py→profiler.py。修复方案：提取model_router接口到model_profiler/model_router_interface.py，budget_enforcer只依赖接口不依赖实现。",
        "priority": "P1",
        "source_blueprint": "MOD-INF-034",
        "source_section": "§19",
        "upstream_files": [
            str(PROJECT_ROOT / "src" / "zephyr" / "model_profiler" / "profiler.py"),
            str(PROJECT_ROOT / "src" / "zephyr" / "model_profiler" / "model_discovery.py"),
            str(PROJECT_ROOT / "src" / "zephyr" / "budget_enforcer" / "model_router.py"),
        ],
        "downstream_outputs": [
            {"path": str(PROJECT_ROOT / "src" / "zephyr" / "model_profiler" / "model_router_interface.py"), "desc": "提取的接口"},
        ],
        "allowed_touch": [
            "src/zephyr/model_profiler/profiler.py",
            "src/zephyr/model_profiler/model_discovery.py",
            "src/zephyr/budget_enforcer/model_router.py",
        ],
        "forbidden_touch": [],
        "applicable_rules": [
            {"module_id": "RULE-ZERO", "section": "§1", "reason": "写入前获取锁"},
        ...
        "context_assembly_manifest": [
            {"file_path": str(PROJECT_ROOT / "src" / "zephyr" / "budget_enforcer" / "model_router.py"), "reason": "当前依赖结构"},
        ],
        "assigned_model": "deepseek",
        "assigned_pipeline": "A",
        "pipeline_modules": ["read", "extract", "refactor", "verify"],
        ...
        "timeout_minutes": 60,
        "acceptance_criteria": [
            "AC1: budget_enforcer/model_router.py不再import model_profiler",
            "AC2: python -c 'from zephyr.model_profiler import profiler' 成功",
            "AC3: diagnose_depgraph.py验证该循环消失",
        ],
    ...
    },
    {
        ...
        "upstream_files": [
            ...
            str(PROJECT_ROOT / "src" / "zephyr" / "agent_spec" / "skill_feedback.py"),
        ],
        "downstream_outputs": [
            {"path": str(PROJECT_ROOT / "src" / "zephyr" / "kb" / "unified_memory_api.py"), "desc": "移除对storage的反向import"},
            {"path": str(PROJECT_ROOT / "src" / "zephyr" / "agent_spec" / "skill_feedback.py"), "desc": "改为事件通知"},
        ],
        "allowed_touch": [
            "src/zephyr/kb/unified_memory_api.py",
            "src/zephyr/kb/vms_memory_backend.py",
            "src/zephyr/agent_spec/skill_feedback.py",
        ],
        "forbidden_touch": [
        ...
        "acceptance_criteria": [
            ...
            "AC3: python -c 'from zephyr.kb import unified_memory_api' 成功",
    ...
    },
    {
        ...
        "description": "l06_trade_execution/execution_engine.py直接import l04_risk_management/implementations/default_risk_validator.py，跳过了L05层。修复方案：execution_engine通过l04_risk_management/risk_validator.py（公共接口）间接引用，不直接import implementations内部模块。",
        "priority": "P1",
        "source_blueprint": "MOD-L06-001",
        "source_section": "§19",
        "upstream_files": [
            str(PROJECT_ROOT / "src" / "zephyr" / "l06_trade_execution" / "execution_engine.py"),
            str(PROJECT_ROOT / "src" / "zephyr" / "l04_risk_management" / "risk_validator.py"),
        ],
        "downstream_outputs": [
            {"path": str(PROJECT_ROOT / "src" / "zephyr" / "l06_trade_execution" / "execution_engine.py"), "desc": "改为通过公共接口引用"},
        ],
        "allowed_touch": [
            "src/zephyr/l06_trade_execution/execution_engine.py",
            "tests/l06_trade_execution/test_execution_engine_unit.py",
            "tests/l06_trade_execution/test_execution_engine_deep.py",
        ],
        "forbidden_touch": [
        ...
        "context_assembly_manifest": [
            {"file_path": str(PROJECT_ROOT / "src" / "zephyr" / "l06_trade_execution" / "execution_engine.py"), "reason": "当前跨层import"},
            {"file_path": str(PROJECT_ROOT / "src" / "zephyr" / "l04_risk_management" / "risk_validator.py"), "reason": "应使用的公共接口"},
        ],
        "assigned_model": "deepseek",
        "assigned_pipeline": "A",
        "pipeline_modules": ["read", "refactor", "verify"],
        ...
        "acceptance_criteria": [
            ...
            "AC3: python -c 'from zephyr.l06_trade_execution import execution_engine' 成功",
    ...
    },
    {
        ...
        "title": "Phase1.5a: 补齐directory-registry.yaml 500个缺登记目录",
        "description": "磁盘上存在500+个目录未在directory-registry.yaml中登记。需要扫描磁盘目录、与注册表对比、补登记缺失项。每个目录登记需包含：path, owner_blueprint, category, status。",
        "priority": "P1",
        "source_blueprint": "PS-REG-012",
        "source_section": "§19",
        "upstream_files": [
            str(PROJECT_ROOT / "docs" / "01_policies_and_standards" / "_registry" / "catalogs" / "directory-registry.yaml"),
        ],
        "downstream_outputs": [
            {"path": str(PROJECT_ROOT / "docs" / "01_policies_and_standards" / "_registry" / "catalogs" / "directory-registry.yaml"), "desc": "补登记后的完整注册表"},
        ],
        "allowed_touch": [
            "docs/01_policies_and_standards/_registry/catalogs/directory-registry.yaml",
        ],
        "forbidden_touch": [],
        "applicable_rules": [
            {"module_id": "RULE-ZERO", "section": "§1", "reason": "写入前获取锁"},
        ...
        "context_assembly_manifest": [
            {"file_path": str(PROJECT_ROOT / "docs" / "01_policies_and_standards" / "_registry" / "catalogs" / "directory-registry.yaml"), "reason": "当前注册表"},
        ],
        "assigned_model": "deepseek",
        "assigned_pipeline": "A",
        ...
        "timeout_minutes": 60,
        "acceptance_criteria": [
            "AC1: 磁盘上所有scan_dirs下的目录都在directory-registry.yaml中登记",
            "AC2: 每个登记项包含path, owner_blueprint, category, status",
            "AC3: YAML格式正确（python -c 'import yaml; yaml.safe_load(...)'验证）",
        ],
        "rollback_instructions": "git checkout恢复directory-registry.yaml",
    ...
    },
    {
        ...
        "context_assembly_manifest": [
            {"file_path": str(PROJECT_ROOT / "data" / "asset_index" / "project-entity-depgraph.yaml"), "reason": "消费者关系数据"},
        ],
        "assigned_model": "deepseek",
        "assigned_pipeline": "A",
    ...
    },
    {
        ...
        "upstream_files": [],
        "downstream_outputs": [
            {"path": str(PROJECT_ROOT / "data" / "asset_index" / "project-entity-depgraph.yaml"), "desc": "最终依赖图"},
