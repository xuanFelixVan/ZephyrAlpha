...
ensure_utf8_stdout()
import argparse
DANGEROUS_SHELL_PATTERNS = [('rm\\s+-rf\\s+/', 'rm -rf / 根目录破坏命令 (ABS-38)'), ('rm\\s+-rf\\s+[~/]', 'rm -rf 家目录/根目录 危险变体 (ABS-38)'), ('rm\\s+-rf\\s+\\*', 'rm -rf * 通配删除 (ABS-38)'), ('rm\\s+-rf\\s+--no-preserve-root', 'rm -rf --no-preserve-root 危险选项 (ABS-38)'), ('del\\s+/[fs](?:\\s+/[qs])?\\s+\\*\\.\\*', 'Windows 递归强制删除 (ABS-38)'), ('del\\s+/[fs](?:\\s+/[qs])?\\s+[A-Z]:\\\\', 'Windows 盘符强制删除 (ABS-38)'), ('\\bformat\\s+[A-Z]:', 'Windows format 格式化命令 (ABS-39)'), ('\\bmkfs\\.', 'mkfs 磁盘格式化命令 (ABS-39)'), ('\\bdd\\s+if=', 'dd 磁盘覆写命令 (ABS-39)'), (':\\(\\)\\s*\\{.*:\\|:&\\s*\\};:', 'Fork Bomb 模式 (ABS-39)'), ('chmod\\s+(777|o\\+w\\s+/|a\\+w\\s+/)', 'chmod 777 / 过度授权 (ABS-39)'), ('>\\s*/dev/sd[a-z]', '覆写块设备 (ABS-39)')]
...
def scan_file(filepath: Path) -> list[dict]:
    ...
    for pattern, label in DANGEROUS_SHELL_PATTERNS:
        for match in re.finditer(pattern, content, re.IGNORECASE | re.MULTILINE):
            findings.append({'file': str(filepath.relative_to(REPO_ROOT)), 'line': content[:match.start()].count('\n') + 1, 'pattern': label, 'matched': match.group(0)[:120]})
    return findings
    '扫描单个文件并返回发现列表.'
