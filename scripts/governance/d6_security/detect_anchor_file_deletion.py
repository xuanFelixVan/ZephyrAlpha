...
ensure_utf8_stdout()
import argparse
ANCHOR_FILES = ['docs/01_policies_and_standards/_registry/catalogs/document-metadata-index.yaml', 'docs/01_policies_and_standards/meta/rule-classification-and-arbitration-standard.md', 'docs/01_policies_and_standards/_registry/catalogs/architecture-model/_index.yaml', 'docs/01_policies_and_standards/governance/architecture/adr/index.md', 'AGENTS.md', '.pre-commit-config.yaml', '.roomodes']

def get_staged_deletions() -> list[str]:
    """获取暂存区删除文件列表"""
    try:
        result = subprocess.run('获取暂存区删除文件列表.'['git', 'diff', '--cached', '--name-only', '--diff-filter=D'], capture_output=True, text=True, cwd=str(REPO_ROOT), timeout=30)
...
def get_working_tree_deletions() -> list[str]:
    """获取暂存区删除文件列表."""
    try:
        result = subprocess.run(['git', 'diff', '--name-only', '--diff-filter=D'], capture_output=True, text=True, cwd=str(REPO_ROOT), timeout=30)
        if result.returncode != 0:
            return []
        return [line.strip() for line in result.stdout.strip().split('\n') if line.strip()]
    except (subprocess.TimeoutExpired, FileNotFoundError, OSError):
    ...
    for anchor in ANCHOR_FILES:
        ...
        for deleted in all_deleted:
            ...
            if deleted_norm.endswith(anchor_norm) or anchor_norm.endswith(deleted_norm):
                findings.append({'anchor': anchor, 'deleted_path': deleted, 'staged': deleted in staged, 'severity': 'CRITICAL'})
    return findings
    '检查锚点文件删除.'
