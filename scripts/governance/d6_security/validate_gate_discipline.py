            ...
            for kw in node.keywords:
                if kw.arg in ('enable_gate', 'gate_enabled', 'skip_gate', 'bypass_gate'):
                    if isinstance(kw.value, ast.Constant) and kw.value.value is False:
                        if not is_test:
                            findings.append({'file': rel, 'line': node.lineno, 'type': 'GATE_DISABLED', 'detail': f'非测试环境中 {kw.arg}=False', 'severity': 'HIGH'})
    return findings
    '检查门禁绕过.'
                    ...
                    for stmt in node.body:
                        ...
                        if isinstance(stmt, ast.Expr) and isinstance(stmt.value, ast.Call):
                            ...
                            if isinstance(func, ast.Name) and func.id in ('print', 'logging', 'logger'):
                                is_silent = False
                                break
                            if isinstance(func, ast.Attribute) and func.attr in ('error', 'warning', 'info', 'debug', 'critical'):
                                is_silent = False
                                break
                    if is_silent:
                        findings.append({'file': rel, 'line': node.lineno, 'type': 'PYDANTIC_SILENT', 'detail': 'ValidationError 被静默吞没（无 raise/log）', 'severity': 'MEDIUM'})
    return findings
    '检查 Pydantic 静默.'
