...
ensure_utf8_stdout()
import argparse
LOG_SENSITIVE_PATTERNS = [('(?:print|log(?:ger)?\\.(?:info|debug|warn(?:ing)?|error|critical|exception))\\s*\\(\\s*.*\\b(password|passwd|secret|token|api[_-]?key|credential|private[_-]?key|access[_-]?key)\\b', '日志输出包含敏感关键词 (ABS-31)'), ('(?:print|log(?:ger)?\\.(?:info|debug|warn(?:ing)?|error|critical|exception))\\s*\\(\\s*.*\\b(PASSWORD|SECRET|TOKEN|API_KEY|CREDENTIAL|PRIVATE_KEY|ACCESS_KEY)\\b', '日志输出包含大写敏感变量名 (ABS-31)'), ('print\\s*\\(\\s*f["\\\'].*\\{(?:password|secret|token|api_key|credential|key)\\}', 'f-string 日志输出包含敏感变量 (ABS-31)')]
