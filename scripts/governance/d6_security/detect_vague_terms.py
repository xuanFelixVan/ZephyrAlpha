...
ensure_utf8_stdout()
import argparse
VAGUE_TERMS_CN = [('等等(?!原则|级|制度|标准|规范|协议|模型|框架|体系|系统|结构|模式)', '中文模糊词「等等」'), ('(?:诸如|诸如|比如|例如).*之类', '中文模糊词「之类」'), ('(?:大概|大约|大致)(?!符合|相当|等于)', '中文模糊词「大概/大约/大致」'), ('(?:可能)[，,。；;！!？?\\s]|可能$', '中文模糊词「可能」'), ('(?:也许|或许)(?!是|有|会|要|能)', '中文模糊词「也许/或许」'), ('(?:差不多|差不多就行|差不太多)', '中文模糊词「差不多」'), ('\\d{2,}\\s*左右(?!手|脚|边|面|方|侧|角|游)', '中文模糊词「数字+左右」')]
VAGUE_TERMS_EN = [('\\betc\\.?(?!\\s*\\)|\\s*\\})', '英文模糊词「etc.」'), ('\\band so on\\b', '英文模糊词「and so on」'), ('\\band so forth\\b', '英文模糊词「and so forth」'), ('\\bmaybe\\b(?!\\s+(?:the|a|an|this|that|these|those|some|any))', '英文模糊词「maybe」'), ('\\bprobably\\b(?!\\s+(?:the|a|an|this|that))', '英文模糊词「probably」'), ('\\bapproximately\\b', '英文模糊词「approximately」'), ('\\broughly\\b(?!\\s+(?:the|a|an|equal|equivalent|same))', '英文模糊词「roughly」'), ('\\bsimilar(?:ly)?\\b(?!\\s+(?:to|as|in|for|approach|pattern|structure|model))', '英文模糊词「similar(ly)」')]
    ...
    for pattern, label in VAGUE_TERMS_CN + VAGUE_TERMS_EN:
        for match in re.finditer(pattern, content, re.IGNORECASE):
            ...
            if is_in_quote(line_text):
                continue
            findings.append({'file': str(filepath.relative_to(REPO_ROOT)), 'line': line_idx + 1, 'pattern': label, 'matched': match.group(0)[:80]})
    return findings
    '扫描单个文件并返回发现列表.'
