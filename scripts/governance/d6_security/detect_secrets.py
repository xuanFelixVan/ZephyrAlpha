...
ensure_utf8_stdout()
import argparse
SECRET_PATTERNS = [('(?:api[_-]?key|apikey|API_KEY)\\s*[:=]\\s*[\'\\"]([^\'\\"]{8,})[\'\\"]', 'API Key 硬编码'), ('(?:secret|SECRET)\\s*[:=]\\s*[\'\\"]([^\'\\"]{8,})[\'\\"]', 'Secret 硬编码'), ('(?:token|TOKEN)\\s*[:=]\\s*[\'\\"]([^\'\\"]{8,})[\'\\"]', 'Token 硬编码'), ('(?:password|PASSWORD|passwd)\\s*[:=]\\s*[\'\\"]([^\'\\"]{3,})[\'\\"]', 'Password 硬编码'), ('(?:access[_-]?key|ACCESS_KEY)\\s*[:=]\\s*[\'\\"]([^\'\\"]{8,})[\'\\"]', 'Access Key 硬编码'), ('(?:private[_-]?key|PRIVATE_KEY)[\'\\"]?\\s*[:=]\\s*[\'\\"]([^\'\\"]{16,})[\'\\"]', 'Private Key 硬编码'), ('sk-[a-zA-Z0-9]{32,}', 'OpenAI API Key 格式'), ('AKIA[0-9A-Z]{16}', 'AWS Access Key ID 格式'), ('(?:ghp|gho|ghu|ghs|ghr)_[A-Za-z0-9_]{36,}', 'GitHub Token 格式')]
