...
THREADING_LOCK_PATTERNS = [('from\\s+threading\\s+import\\s+.*\\bLock\\b', '导入 threading.Lock（含 from import）'), ('import\\s+threading\\b', '导入 threading 模块（可能使用 threading.Lock）'), ('threading\\.Lock\\s*\\(', '直接使用 threading.Lock()')]
...
def scan_file(filepath: Path) -> list[dict]:
    ...
    for pattern, label in THREADING_LOCK_PATTERNS:
        for match in re.finditer(pattern, content):
            ...
            findings.append({'file': str(filepath.relative_to(REPO_ROOT)), 'line': line_num, 'label': label, 'line_content': line_content})
    return findings
    '扫描单个文件并返回发现列表.'
