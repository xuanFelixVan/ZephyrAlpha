...
ensure_utf8_stdout()
import argparse
DANGEROUS_GIT_PATTERNS = [('git\\s+push\\s+.*(--force|-f)', 'git push --force 危险操作 (ABS-26)'), ('git\\s+reset\\s+--hard', 'git reset --hard 危险操作 (ABS-27)'), ('git\\s+clean\\s+(-fdx|-fd\\b.*-)', 'git clean -fdx/fd 危险操作 (ABS-28)'), ('git\\s+branch\\s+-D\\b', 'git branch -D 强制删除分支'), ('git\\s+rebase\\s+.*(--onto|--root|-i.*origin)', 'git rebase 高危变体'), ('git\\s+push\\s+--delete\\s+origin', 'git push --delete 远程分支删除')]
