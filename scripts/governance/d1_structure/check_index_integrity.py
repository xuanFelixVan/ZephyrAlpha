...
from _shared.constants import REPO_ROOT, SCAN_EXTENSIONS_MD, SCAN_EXTENSIONS_MD_YAML
...
def check_index_integrity() -> list[dict]:
    ...
    for index_path in index_files:
        ...
        for name in missing_from_disk:
            ...
            findings.append({'index_file': rel, 'type': 'INDEX_ENTRY_MISSING', 'detail': f"index.md 列出 '{name}' 但磁盘不存在", 'severity': 'MEDIUM'})
        missing_from_index = siblings - entry_basenames
        for name in sorted(missing_from_index)[:20]:
            rel = str(index_path.relative_to(REPO_ROOT)).replace('\\', '/')
            findings.append({'index_file': rel, 'type': 'FILE_NOT_IN_INDEX', 'detail': f"文件 '{name}' 存在但未在 index.md 中列出", 'severity': 'LOW'})
    return findings
    'check index integrity.'
