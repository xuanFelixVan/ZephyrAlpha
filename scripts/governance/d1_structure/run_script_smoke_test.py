...
def run_script(script_name: str, timeout: int=60) -> tuple[int, str, str]:
    ...
    try:
        result = subprocess.run([sys.executable, str(script_path), '--warn-only'], capture_output=True, text=True, timeout=timeout, cwd=str(REPO_ROOT), encoding='utf-8', errors='replace')
