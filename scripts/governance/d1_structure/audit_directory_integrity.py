...
_DOMAIN_PARENT_DIRS = frozenset({_PS_ROOT / 'domains' / d for d in ('L00_data_source', 'L02_alpha_factor', 'L04_risk_management', 'L07_post_trade_analytics')})
_DOC_TYPE_SUFFIX_MAP = {'policy': '-policy.md', 'standard': '-standard.md', 'protocol': '-protocol.md', 'playbook': '-playbook.md', 'runbook': '-runbook.md', 'checklist': '-checklist.md', 'template': '-template.md', 'index': 'index.md', 'register': '.yaml', 'contract': '.yaml', 'vocabulary': '.yaml', 'schema': '.json'}
_LEGIT_MULTI_WORD_SUFFIXES = {'-state-machine.md', '-session-state-machine.md', '-standard-constitution.md'}
_FILENAME_EXCEPTIONS = frozenset({'metadata-registry.md'})
...
def _extract_module_id(filepath: Path) -> Optional[str]:
    ...
    for entry in root.rglob('*'):
        ...
        if entry.suffix.lower() in ('.md', '.yaml', '.yml', '.json'):
            ...
            if not skip:
                result.append(entry)
    return result
_MODULE_ID_PATTERN = re.compile('^(?:PS-STD|PS-REG|META-(?:GLS|IDX|TERM)|GOV-(?:ARCH|SEC|DATA|CMP|DOC|AI|TASK|MOD|OP|REG)|CAT|OPS-(?:IDX|VC|DEV|MIG)|DOM|TPL|ARCH|REG)-\\d+$')
_FILE_HEADER_NAMES = frozenset({'文件', '文件名', '文档名称', '文档', 'file', 'filename', 'file_name', 'document', 'name', '索引文件'})
...
def _find_index_for_dir(dir_path: Path) -> Optional[Path]:
...
def check_d1_ghost_files() -> list[Finding]:
    ...
    for fpath in all_files:
        ...
        if not found_elsewhere:
            findings.append(Finding('D1-幽灵文件', 'CRITICAL', _rel(fpath), '文件存在磁盘上，但未在任何 index.md 的 §2 文件清单中注册'))
    return findings
...
def check_d2_id_conflicts() -> list[Finding]:
    ...
    for mid, files in id_map.items():
        if len(files) > 1:
            findings.append(Finding('D2-ID冲突', 'CRITICAL', mid, f'module_id 被 {len(files)} 个文件同时占用: {', '.join(files)}'))
    return findings
...
def check_d3_index_counts() -> list[Finding]:
    ...
    for index_path in _PS_ROOT.rglob('index.md'):
        ...
        if index_dir in _DOMAIN_PARENT_DIRS:
            actual_count = sum((1 for sub in ('governance', 'operational') for _ in (index_dir / sub / 'index.md').parent.iterdir() if (index_dir / sub / 'index.md').exists() and (index_dir / sub / 'index.md').parent == index_dir / sub))
            sub_gov_files = sum((1 for f in (index_dir / 'governance').iterdir() if f.is_file() and f.name != 'index.md' and (f.suffix.lower() in ('.md', '.yaml', '.yml', '.json')))) if (index_dir / 'governance').exists() else 0
            sub_ops_files = sum((1 for f in (index_dir / 'operational').iterdir() if f.is_file() and f.name != 'index.md' and (f.suffix.lower() in ('.md', '.yaml', '.yml', '.json')))) if (index_dir / 'operational').exists() else 0
            actual_count = sub_gov_files + sub_ops_files
        else:
            disk_files = [f for f in index_dir.iterdir() if f.is_file() and f.name != 'index.md' and (f.suffix.lower() in ('.md', '.yaml', '.yml', '.json'))]
        ...
        if claimed_count != actual_count:
            findings.append(Finding('D3-索引对账', 'HIGH', _rel(index_path), f'index.md 登记 {claimed_count} 个文件，但目录实际有 {actual_count} 个文件（不含 index.md）。差額: {actual_count - claimed_count:+d}'))
    return findings
...
def check_d4_naming_convention() -> list[Finding]:
    ...
    for fpath in all_files:
        ...
        if not doc_type:
            if suffix == '.md':
                findings.append(Finding('D4-命名规范', 'MEDIUM', _rel(fpath), 'doc_type 缺失，无法校验文件名后缀是否匹配'))
            continue
            ...
            if not ok:
                for legit in _LEGIT_MULTI_WORD_SUFFIXES:
                    if fname_lower.endswith(legit):
                        ...
                        break
            if not ok:
                findings.append(Finding('D4-命名规范', 'MEDIUM', _rel(fpath), f'文件名后缀不匹配 doc_type。doc_type={doc_type}，期望后缀 {expected}，实际文件名 {fpath.name}'))
    return findings
    ...
    for index_path in _PS_ROOT.rglob('index.md'):
        ...
        for fpath in disk_files:
            if fpath.name not in table:
                findings.append(Finding('D5-登记缺漏', 'HIGH', _rel(fpath), f'文件存在于 {_rel(index_dir)}，但在 {_rel(index_path)} 中没有对应条目'))
    return findings
    'check d5 missing entries.'
...
def _print_report(findings: list[Finding]) -> None:
    ...
    for dim in ordered_dims:
        ...
        print(f'  [{dim}] {len(items)} finding(s)  (CRITICAL={critical}, HIGH={high}, MEDIUM={medium})', file=sys.stderr)
    print(file=sys.stderr)
    print('-' * 60, file=sys.stderr)
    for dim in ordered_dims:
