...
def scan_drafts(root: Path, warn_days: int=WARN_DAYS, archive_days: int=ARCHIVE_DAYS) -> list[dict[str, Any]]:
    ...
    for md_file in sorted(root.rglob('*.md')):
        ...
        if fm is None:
            results.append({'path': md_file, 'relative': md_file.relative_to(root), 'audit_status': None, 'arbitrated_date': None, 'age_days': None, 'action': 'skip'})
            continue
        ...
        if arbitrated_date_str:
            try:
                arb_date = datetime.strptime(str(arbitrated_date_str), '%Y-%m-%d').replace(tzinfo=timezone.utc)
                age_days = (datetime.now(timezone.utc) - arb_date).days
            except ValueError:
                pass
        action = 'skip'
        if audit_status == STATUS_ARBITRATED and age_days is not None:
            if age_days >= archive_days:
                ...
                action = 'warn'
        results.append({'path': md_file, 'relative': md_file.relative_to(root), 'audit_status': audit_status, 'arbitrated_date': arbitrated_date_str, 'age_days': age_days, 'action': action})
    return results
...
def write_audit_log(entry: dict[str, Any]) -> None:
    ...
    log_entry = {'timestamp': datetime.now(timezone.utc).isoformat(), 'action': entry.get('action', 'unknown'), 'source': str(entry.get('relative', '')), 'audit_status': entry.get('audit_status'), 'age_days': entry.get('age_days')}
