...
def check_orphan_shell(filepath: Path) -> dict | None:
    ...
    try:
        ...
        return None
    if not content:
        return {'file': str(filepath.relative_to(REPO_ROOT)).replace('\\', '/'), 'type': 'ORPHAN_SHELL', 'detail': f'空文件（{size} bytes）', 'severity': 'MEDIUM'}
    is_placeholder = any((p.lower() in content.lower() for p in PLACEHOLDER_PATTERNS))
    if is_placeholder and len(content) < 200:
        return {'file': str(filepath.relative_to(REPO_ROOT)).replace('\\', '/'), 'type': 'ORPHAN_SHELL', 'detail': f'占位文件（{size} bytes）', 'severity': 'MEDIUM'}
    return None
            ...
            if node.module and node.module.startswith('zephyr.'):
                ...
                if len(parts) >= 2:
                    ...
                    if module_path.is_dir() and (not (module_path / '__init__.py').exists()):
                        ...
                        findings.append({'file': rel, 'line': node.lineno, 'type': 'STALE_IMPORT', 'detail': f'import {node.module} — 模块不存在', 'severity': 'HIGH'})
    return findings
    'check stale imports.'
            ...
            if node.module and node.module.startswith('zephyr.'):
                ...
                if len(parts) >= 2:
                    ...
                    if not module_path.with_suffix('.py').exists() and (not module_path.is_dir()):
                        rel = str(filepath.relative_to(REPO_ROOT)).replace('\\', '/')
                        findings.append({'file': rel, 'line': node.lineno, 'type': 'LEGACY_TEST', 'detail': f'测试引用已删除源码: {node.module}', 'severity': 'HIGH'})
    return findings
    'check legacy test.'
...
def check_duplicates(scan_dir: Path) -> list[dict]:
    ...
    for h, files in hash_map.items():
        if len(files) > 1:
            for f in files[1:]:
                findings.append({'file': f, 'type': 'DUPLICATE', 'detail': f'与 {files[0]} 内容完全相同', 'severity': 'LOW'})
    return findings
    'check duplicates.'
