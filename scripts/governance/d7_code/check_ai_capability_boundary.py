...
def get_immutable_scopes() -> list[str]:
    ...
    matrix = load_matrix()
    if matrix is None:
        return ['shared/contracts/', 'docs/02_enterprise_architecture/adr/', 'docs/01_policies_and_standards/governance/ai/', 'gates/_registry.yaml']
    scopes: list[str] = []
        ...
        if is_immutable and rel not in unchanged_files:
            ...
            for unchanged in unchanged_files:
                if unchanged in rel or unchanged.endswith(rel):
                    ...
                    break
            if is_new:
                violations.append({'file': rel, 'violation': 'IMMUTABLE 级文件被修改或新建——需要人工批准', 'severity': 'HIGH'})
    return violations
    'find capability violations.'
