...
def scan_ttl_violations() -> list[dict]:
    ...
    for filepath in iter_files(docs_dir, extensions=frozenset({'.md', '.yaml', '.yml'})):
        ...
        if ttl and ttl not in VALID_TTL_VALUES:
            findings.append({'file': rel, 'type': 'INVALID_TTL', 'detail': f"ttl='{ttl}' 不在合法枚举中（合法值: {', '.join(sorted(VALID_TTL_VALUES))}）", 'severity': 'MEDIUM'})
            ...
            try:
                ...
                if (now - file_date).days > 30:
                    if 'archive' not in rel.lower() and '09_audit' not in rel:
                        findings.append({'file': rel, 'type': 'TTL_30D_EXPIRED', 'detail': f'ttl=30d 文件已过期 {(now - file_date).days} 天，应归档', 'severity': 'MEDIUM'})
            except ValueError:
                pass
        if ttl == '7d' and date_str:
            try:
                ...
                if (now - file_date).days > 7:
                    findings.append({'file': rel, 'type': 'TTL_7D_EXPIRED', 'detail': f'ttl=7d 文件已过期 {(now - file_date).days} 天，应删除', 'severity': 'HIGH'})
            except ValueError:
                pass
        if ttl == 'session':
            findings.append({'file': rel, 'type': 'TTL_SESSION_IN_REPO', 'detail': 'ttl=session 文件不应提交到 git', 'severity': 'MEDIUM'})
    return findings
    '扫描文档 TTL 违规.'
    ...
    for scan_dir in scan_dirs:
        ...
        for filepath in iter_files(scan_dir, extensions=frozenset({'.json', '.yaml', '.yml', '.md'})):
            if DATED_SNAPSHOT_PATTERN.search(filepath.name):
                rel = str(filepath.relative_to(REPO_ROOT)).replace('\\', '/')
                findings.append({'file': rel, 'type': 'DATED_SNAPSHOT', 'detail': f'状态快照应使用 LATEST 命名（当前: {filepath.name}）', 'severity': 'LOW'})
    return findings
    '扫描过期快照.'
