...
def match_file_to_domain(rel_path, bp_index, domain_index, bp_domain_map, panorama_mapping):
    ...
    if rel_path in bp_index:
        ...
        for key, info in bp_domain_map.items():
            if info["blueprint_id"] == bp_id:
                ...
                break
        if matched_entry:
            return (rel_path, "", bp_id, matched_entry["domain_id"], matched_entry["subdomain_id"],
                    "depgraph_blueprint", "high")
        if did:
    ...
    for ssot_mod, pinfo in panorama_mapping.items():
        ...
        if ssot_path and rel_path.startswith(ssot_path):
            return (rel_path, "", ssot_mod, pinfo["domain_id"], pinfo["subdomain_id"],
                    "ssot_path_prefix", "high")
...
def main():
    ...
    with open(tmp_csv, "w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["file_path", "module_id", "blueprint_id", "domain_id", "subdomain_id", "match_method", "confidence"])
