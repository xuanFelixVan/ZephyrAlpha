...
| 1.0.0 | 2026-05-02 | 初始创建。(1) 定义 7 个质量维度（D-A~D-G）及 MUST/SHOULD/MAY 条款。(2) 对标 Google Python Style Guide + PEP 484/257/540 + OWASP + Clean Code + ISO 25010。(3) 含 AI 自检清单——AI 创建/修复脚本前逐项勾选。(4) 注册到 _index.yaml + AGENTS.md §8.2 任务路由。(5) 互补 AGENTS.md §6.5（入库流程）+ §6.7（UTF-8 强制声明）。 |
