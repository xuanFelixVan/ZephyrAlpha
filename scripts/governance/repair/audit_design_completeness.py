...
def match_in_db(conn, declaration):
    ...
    row = cur.execute(
        "SELECT node_id FROM nodes WHERE design_maturity='design' AND path LIKE ? LIMIT 1",
        (f"%{mod_id}%",)
    ).fetchone()
    if row:
        return "path_exact"
    ...
    row = cur.execute(
        "SELECT node_id FROM nodes WHERE design_maturity='design' AND node_name = ? LIMIT 1",
        (mod_id,)
    ).fetchone()
    if row:
        return "name_exact"
...
def main():
    ...
    try:
        ...
        for i, m in enumerate(missing, 1):
            report.append(f"| {i} | {m['source_md']} | {m['id_format']} | {m['module_id']} | {m['module_name']} | path和功能名均未命中 |")
        report.append("")
        ...
        for i, m in enumerate(info_insufficient, 1):
            report.append(f"| {i} | {m['source_md']} | {m['id_format']} | {m['module_id']} | module_name | 记录到待人工补全清单 |")
        report.append("")
        ...
        if len(unique_declarations) > 0:
            report.append(f"匹配率: {matched}/{len(unique_declarations)} = {matched*100/len(unique_declarations):.1f}%")
