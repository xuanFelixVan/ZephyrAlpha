...
"""
SQL_SELECT_MIGRATION_STATUS = (
    f"SELECT status FROM {MIGRATION_LOG_TABLE} WHERE migration_id = %s"
)
...
SQL_INSERT_VALUES_OVERRIDING = (
    'INSERT INTO "{table}" ({cols}) OVERRIDING SYSTEM VALUE VALUES %s'
)
...
def record_migration_result(pg_conn, migration_id, status, report):
    ...
    pg_conn.commit()
    print(f'[LOG] migration_log 已记录: {migration_id} status={status} '
          f'tables={len(report)} rows={rows_total}')
...
def migrate_table(sqlite_conn, pg_conn, table_name, identity_cols):
    ...
    rows = sqlite_conn.execute(
        SQL_SELECT_ALL_ROWS.format(cols=col_list, table=table_name)
    ).fetchall()
    ...
    if pg_count != sqlite_count:
        raise RuntimeError(
            f'表 {table_name} 行数校验失败: SQLite={sqlite_count} PG={pg_count}'
        )
    pg_conn.commit()
...
def run_migration(sqlite_conn, pg_conn, tables=None, migration_id=MIGRATION_ID, force=False):
    ...
    if not force and migration_already_completed(pg_conn, migration_id):
        print(f'[SKIP] 迁移 {migration_id} 已完成（migration_log 记录存在），跳过。'
              f'如需重跑请使用 --force')
        return EXIT_PASS
