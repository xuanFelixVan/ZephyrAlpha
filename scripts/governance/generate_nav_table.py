...
DOC_META_INDEX = PROJECT_ROOT / 'docs' / '01_policies_and_standards' / '_registry' / 'catalogs' / 'document-metadata-index.yaml'
...
def load_registry_modules(registry_path) -> dict:
    ...
    for entry in data.get('documents', []):
        ...
        if mid:
            modules[mid] = {'path': entry.get('path', ''), 'title': entry.get('title', ''), 'status': entry.get('status', 'unknown'), 'version': entry.get('version', '')}
    return modules
...
def format_scripts_list(script_items, known_scripts) -> None:
    ...
    for s in script_items:
        ...
        if name in known_scripts:
            parts.append(f'`{name}`')
        elif name.startswith('p') and 'pre-commit' in name.lower():
            parts.append(name)
        elif 'CI ' in name or name.startswith('CI '):
            parts.append(name)
        elif 'run_all.py' in name:
            parts.append(name)
        elif name == '人工':
            parts.append(name)
        else:
