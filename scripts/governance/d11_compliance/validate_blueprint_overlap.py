...
def run_validation(verbose: bool=False) -> tuple[list[dict[str, Any]], int]:
    ...
    if verbose:
        ...
        print(f'[validate_blueprint_overlap] 扫描 {unique_components} 个唯一组件，共 {total_components} 个映射，发现 {len(overlaps)} 处重叠', file=sys.stderr)
...
def main() -> None:
    ...
    for overlap in overlaps:
        print(f"  - OVERLAP: component '{overlap['component_id']}' 出现在 {overlap['draft_count']} 个草稿中:", file=sys.stderr)
