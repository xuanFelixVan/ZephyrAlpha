...
from _shared.constants import SCAN_EXTENSIONS_CODE
ensure_utf8_stdout()
import argparse
...
class ClauseCheck:
    ...
    def add_failure(self, filepath: Path, detail: str='') -> None:
        """add failure"""
        self.failures.append(f'{filepath.relative_to(REPO_ROOT)}: {detail}' if detail else str(filepath.relative_to(REPO_ROOT)))
...
_SHARED_API_NAMES: frozenset[str] = frozenset({'REPO_ROOT', 'EXCLUDE_DIRS', 'SCAN_EXTENSIONS_MD_YAML', 'SCAN_EXTENSIONS_CODE', 'SCAN_EXTENSIONS_DOCS', 'SCAN_EXTENSIONS_PY', 'SCAN_EXTENSIONS_MD', 'SCAN_EXTENSIONS_DATA', 'GOV_DOCS_DIR', 'SRC_DIR', 'CONFIG_DIR', 'SCRIPTS_DIR', 'MANIFEST_PATH', 'find_repo_root', 'parse_frontmatter', 'parse_frontmatter_from_file', 'parse_frontmatter_raw_from_file', 'parse_yaml_header', 'ensure_utf8_stdout', 'iter_files', 'load_yaml'})
...
def check_shared_bypass(content: str, filepath: Path, result: ClauseCheck) -> None:
    ...
    if overlaps:
        result.add_failure(filepath, f'本地重定义了 _shared 工具: {', '.join(sorted(overlaps))} — 请改为 from _shared.xxx import ...')
...
def check_oswalk_bypass(content: str, filepath: Path, result: ClauseCheck) -> None:
    ...
    if has_oswalk and has_exclude and (not uses_iter_files):
        result.add_failure(filepath, '使用 os.walk() + 手动 EXCLUDE_DIRS 过滤 — 请改为 from _shared.walk import iter_files')
...
def check_lossy_transform(content: str, filepath: Path, result: ClauseCheck) -> None:
    ...
    if has_unparse and has_write:
        result.add_failure(filepath, '使用 ast.unparse() + 文件写入——有损代码变换，丢失行内注释/格式。请改用 LibCST (libcst.parse_module → CSTTransformer → tree.code)')
CLAUSE_CHECKS: list[tuple[str, str, callable, bool]] = [('D-A-01', 'UTF-8 输出强制声明', check_utf8, False), ('D-A-02', '禁止裸 except', check_bare_except, False), ('D-A-03', '禁止 shell=True', check_shell_true, False), ('D-A-04', "I/O 操作显式 encoding='utf-8'", check_io_encoding, True), ('D-A-05', '禁止重复 import', check_duplicate_imports, False), ('D-B-01', '公共函数返回类型注解', check_public_func_annotations, True), ('D-B-02', 'main() 返回类型标注', check_main_return_type, True), ('D-C-01', '模块级 docstring', check_docstring, False), ('D-C-02', '公共函数 docstring', check_public_func_docstring, True), ('D-D-06', 'if __name__ 守卫', check_main_guard, True), ('D-D-07', '禁止绕过 _shared 工具', check_shared_bypass, False), ('D-D-08', '禁止 os.walk + EXCLUDE_DIRS 手动过滤', check_oswalk_bypass, False), ('D-F-01', '--warn-only 支持', check_warn_only, True), ('D-F-02', 'POSIX 退出码', check_exit_codes, True), ('D-G-06', '禁止 ast.unparse 重写文件', check_lossy_transform, False)]
...
def main() -> None:
    ...
    parser.add_argument('--scripts-dir', type=str, default=str(REPO_ROOT / 'scripts' / 'governance'), help='治理脚本目录路径')
