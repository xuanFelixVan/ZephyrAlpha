...
tasks = [
    # ═══ Phase 7 ═══
    ("DM-260", 260, "src/zephyr/ 缺失文件价值裁定（2,372个文件）", "PENDING", "P1", 7, "M",
     ...
     '["src/zephyr/"]', '["裁定记录 + depgraph更新"]', '["2,372个文件均有裁定结果"]', '["DM-256"]',
     '["migration","value-assessment","src"]', "EA-MIG-001", "Phase 7"),

    ("DM-261", 261, "docs/ 缺失文件价值裁定+恢复（1,412个.md）", "PENDING", "P1", 7, "M",
     ...
     '["docs/"]', '["裁定记录 + 蓝图恢复 + depgraph更新"]', '["1,412个文件均有裁定结果，蓝图100%恢复"]', '["DM-258"]',
     '["migration","value-assessment","docs"]', "EA-MIG-001", "Phase 7"),

    ("DM-262", 262, "scripts/ 缺失文件价值裁定+恢复", "COMPLETED", "P1", 7, "L",
     "已完成。409/409注册脚本全部存在（之前报告406缺失为路径解析错误，script_manifest.yaml路径相对于scripts/目录）。",
     '["scripts/"]', '["验证报告：409/409存在"]', '["409个脚本全部存在"]', '[]',
     '["migration","scripts","verified"]', "EA-MIG-001", "Phase 7"),

    ("DM-263", 263, "data/ 缺失文件价值裁定", "COMPLETED", "P2", 7, "L",
     "已完成。31个文件已审查：29保留(23迁移脚本备份+6审计/日志/映射)，2个无价值已删除。",
     '["data/"]', '["裁定结果：29保留/2删除"]', '["data/无孤儿文件"]', '[]',
     '["migration","data","verified"]', "EA-MIG-001", "Phase 7"),

    ("DM-264", 264, "恢复文件导入路径更新+depgraph对齐", "COMPLETED", "P1", 7, "M",
     "已完成。218个.py文件352处旧导入路径替换(LAYER_TO_DOMAIN映射14条)。验证：Grep残留旧导入0匹配。depgraph+path-tree已重新生成，0循环依赖。",
     '["src/zephyr/","data/databases/depgraph.db"]', '["218文件352处替换"]', '["0残留旧导入"]', '["DM-260","DM-261"]',
     '["migration","import-fix","verified"]', "EA-MIG-001", "Phase 7"),

    ("DM-265", 265, "depgraph physical_files 全面校准", "COMPLETED", "P1", 7, "M",
     "已完成。physical_files 4,898→52,743（48,199从nodes添加，354不存在已移除）。17个旧DOM-*节点标记deprecated。navigation.quick_facts已更新。",
     '["data/databases/depgraph.db"]', '["physical_files校准完成"]', '["depgraph与nodes对齐"]', '["DM-264"]',
     '["migration","depgraph","verified"]', "EA-MIG-001", "Phase 7"),

    # ═══ Phase 8 ═══
    ("DM-266", 266, "R4 场外30域文档与10架构图内容矛盾", "BLOCKED", "P1", 8, "H",
     "风险R4：场外30域文档与10架构图存在内容矛盾，需Owner人工逐条核对。执行：1)列出30域文档定义 2)列出10架构图定义 3)逐条对比标记矛盾 4)生成矛盾清单供Owner决策。状态：BLOCKED-需Owner介入",
     '["docs/02_enterprise_architecture/"]', '["矛盾清单+Owner决策记录"]', '["Owner逐条确认"]', '["DM-265"]',
     '["risk","R4","human-review"]', "EA-MIG-001", "Phase 8"),

    ("DM-267", 267, "R7 场外域文档与能力定位书45能力映射有缺口", "BLOCKED", "P1", 8, "H",
     "风险R7：场外域文档与能力定位书45能力映射有缺口，需Owner人工逐域验证。执行：1)列出45能力定义和映射域 2)列出30域能力覆盖 3)逐域对比标记缺口 4)生成缺口清单供Owner决策。状态：BLOCKED-需Owner介入",
     '["docs/02_enterprise_architecture/"]', '["缺口清单+Owner决策记录"]', '["Owner逐域确认"]', '["DM-265"]',
     '["risk","R7","human-review"]', "EA-MIG-001", "Phase 8"),

    # ═══ Phase 9 ═══
    ("DM-268", 268, "shared/ 同名文件去重（31组/90+文件）", "PENDING", "P1", 9, "M",
     ...
     '["src/zephyr/shared/"]', '["31组裁定记录+去重目录+import更新"]', '["shared/根目录仅剩__init__.py+无子包归属模块"]', '["DM-260"]',
     '["dedup","shared","phase9"]', "EA-MIG-001", "Phase 9 Layer 1"),

    ("DM-269", 269, "空壳/桩文件识别与处置", "PENDING", "P2", 9, "M",
     ...
     '["src/zephyr/","scripts/"]', '["空壳文件裁定记录+清理后文件列表"]', '["所有EMPTY/STUB文件均有裁定记录"]', '["DM-268"]',
     '["cleanup","stubs","phase9"]', "EA-MIG-001", "Phase 9 Layer 2"),

    ("DM-270", 270, "文件内容旧层名引用清理（~100个文件）", "PENDING", "P2", 9, "L",
     ...
     '["src/zephyr/"]', '["旧层名清理报告+替换记录"]', '["Grep l0[0-9]_在src/zephyr/ .py中0匹配"]', '["DM-269"]',
     '["cleanup","layer-names","phase9"]', "EA-MIG-001", "Phase 9 Layer 3"),

    ("DM-271", 271, "src/zephyr/ 孤儿文件裁定（0~60个潜在孤儿）", "PENDING", "P2", 9, "M",
     ...
     '["src/zephyr/"]', '["孤儿裁定记录+depgraph更新"]', '["所有潜在孤儿均有裁定结果，depgraph与磁盘一致"]', '["DM-270"]',
     '["orphans","value-assessment","phase9"]', "EA-MIG-001", "Phase 9 Layer 4"),

    ("DM-272", 272, "depgraph生成器SCAN_EXTENSIONS过滤+精简后全量生成", "PENDING", "P1", 9, "M",
     ...
     '["scripts/governance/generate_project_depgraph.py","data/databases/depgraph.db"]', '["修复后生成器+全量depgraph+path-tree"]', '["生成器120秒内完成，0循环，扫描量<5000"]', '["DM-271"]',
     '["depgraph","generator","phase9"]', "EA-MIG-001", "Phase 9 Layer 5"),
]
...
for t in tasks:
    ...
    try:
        ...
        """, (task_id, seq, title, status, priority, phase, safety,
              fis, deliv, acc, deps, tags,
              now, now, src_bp, src_sec, desc))
        created += 1
