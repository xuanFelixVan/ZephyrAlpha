...
class Layer5ResponsibilityFormatFixer:
    """Layer 5职责描述格式修复器"""
    ...
    def has_core_responsibility(self, content: str) -> bool:
        """检查文档是否已有核心职责章节"""
        core_match = re.search(r'>\s*\*\*核心职责\*\*[：:]\s*(.+?)\n', content)
        return core_match is not None
...
'''
    ...
    def run(self):
        """运行修复"""
        print('=' * 80)
        print('Layer 5 职责描述格式修复工具')
        print('=' * 80)
        print(f'修复时间: {self._get_timestamp()}')
        print()
        
        print('阶段1: 修复缺少核心定位章节的文档...')
    ...
    def _generate_report(self):
        """生成修复报告"""
        report_path = self.audit_dir / 'LAYER5_RESPONSIBILITY_FORMAT_FIX_REPORT_20260407.md'
        
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write('# Layer 5 职责描述格式修复报告\n\n')
            f.write(f'> **修复时间**: {self._get_timestamp()}\n')
            f.write(f'> **修复范围**: docs/05_IMPLEMENTATION/06_CONSTRUCTION_DOCS/01_BLUEPRINTS\n')
            f.write(f'> **修复目的**: 为缺少核心定位章节的文档添加标准化职责描述\n\n')
            f.write('---\n\n')
            f.write('## 📊 修复概要\n\n')
            ...
            f.write('## 🎯 后续建议\n\n')
            f.write('### 立即行动\n')
            f.write('- ✅ 已完成：为10个缺少核心定位章节的文档添加标准化职责描述\n\n')
            f.write('### 近期改进\n')
            f.write('- 验证修复效果\n')
            f.write('- 检查职责描述相似度\n')
            f.write('- 优化职责描述个性化程度\n\n')
...
def main():
    """主函数"""
    fixer = Layer5ResponsibilityFormatFixer()
    fixer.run()
