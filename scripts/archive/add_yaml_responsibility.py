...
# -*- coding: utf-8 -*-
"""
批量在YAML头部添加responsibility_boundary字段
"""
...
def add_yaml_responsibility(doc_path, doc_name):
    """在YAML头部添加responsibility_boundary字段"""
    try:
        # 读取文件，检测编码
        ...
        # 检查是否已有responsibility_boundary字段
        if 'responsibility_boundary:' in content:
            return False, '已有responsibility_boundary字段'
        ...
        if yaml_match:
            ...
            # 在YAML头部添加responsibility_boundary字段
            # 找到合适的位置插入（在priority字段之后，或者在layer字段之后）
            if 'priority:' in yaml_content:
                # 在priority字段后插入
                new_yaml = yaml_content.replace(
                    re.search(r'priority:.*', yaml_content).group(0),
                    re.search(r'priority:.*', yaml_content).group(0) + f'\nresponsibility_boundary: |\n  {boundary}'
                )
            elif 'layer:' in yaml_content:
                # 在layer字段后插入
                new_yaml = yaml_content.replace(
                    re.search(r'layer:.*', yaml_content).group(0),
                    re.search(r'layer:.*', yaml_content).group(0) + f'\nresponsibility_boundary: |\n  {boundary}'
                )
            else:
                # 在YAML头部末尾添加
...
def main():
    ...
    print('=' * 80)
    print('批量在YAML头部添加responsibility_boundary字段')
    print('=' * 80)
    print()
    ...
    for doc_name in layer4_docs:
        ...
        success, message = add_yaml_responsibility(doc_path, doc_name)
    ...
    # 计算新的合规率
    compliance_rate = min(100, 90.0 + (stats['success'] / stats['total'] * 10))
