...
class ResponsibilityPersonalizerV2:
    """职责描述个性化优化器 v2.0"""
    ...
    def __init__(self):
        ...
        self.category_templates = {
            'data_management': {
                'prefixes': ['主导', '构建', '设计', '开发', '实施', '建立', '创建', '部署'],
                'actions': ['实现', '提供', '支持', '保障', '优化', '管理', '维护', '监控'],
                'suffixes': ['提升数据资产可见性', '确保数据质量合规', '加速数据价值释放', '优化数据治理流程', '增强数据安全性'],
                'tech_keywords': ['Apache Atlas', 'DataHub', 'Amundsen', 'Delta Lake', 'Apache Iceberg', 'Apache Hudi', 'LakeFS', 'Nessie']
            },
            'risk_control': {
                'prefixes': ['构建', '设计', '实现', '开发', '部署', '建立', '创建', '配置'],
                'actions': ['监控', '识别', '评估', '预警', '控制', '管理', '分析', '报告'],
                'suffixes': ['降低投资风险', '提升风控效率', '保障资产安全', '优化风险收益比', '增强风险意识'],
                'tech_keywords': ['Barra', 'RiskMetrics', 'VaR', '压力测试', '蒙特卡洛', 'CVaR', '风险平价', '因子模型']
            },
            'trading_execution': {
                'prefixes': ['实现', '构建', '开发', '优化', '部署', '设计', '配置', '实施'],
                'actions': ['执行', '优化', '监控', '管理', '调度', '路由', '撮合', '清算'],
                'suffixes': ['提升交易效率', '降低交易成本', '优化执行质量', '增强交易能力', '提高成交率'],
                'tech_keywords': ['算法交易', '智能订单路由', 'TWAP', 'VWAP', '执行算法', '市场冲击模型', '交易成本分析', '高频交易']
            },
            'portfolio_management': {
                'prefixes': ['设计', '构建', '优化', '实现', '管理', '配置', '调整', '监控'],
                'actions': ['配置', '优化', '调整', '监控', '评估', '分析', '选择', '组合'],
                'suffixes': ['优化投资组合', '提升收益风险比', '实现投资目标', '增强组合表现', '优化资产配置'],
                'tech_keywords': ['均值方差优化', 'Black-Litterman', '风险平价', '因子投资', '多因子模型', '组合优化', '资产配置', '再平衡']
            },
            'factor_research': {
                'prefixes': ['研究', '开发', '构建', '验证', '优化', '设计', '测试', '分析'],
                'actions': ['挖掘', '分析', '测试', '应用', '监控', '评估', '选择', '组合'],
                'suffixes': ['发现投资机会', '提升因子有效性', '增强策略表现', '优化因子组合', '提高预测能力'],
                'tech_keywords': ['因子挖掘', '机器学习', '特征工程', '回测框架', '因子库', '因子分析', 'IC分析', '因子正交化']
            },
            'infrastructure': {
                'prefixes': ['构建', '部署', '维护', '优化', '管理', '设计', '实施', '配置'],
                'actions': ['支持', '保障', '监控', '扩展', '升级', '管理', '维护', '优化'],
                'suffixes': ['确保系统稳定', '提升系统性能', '优化资源利用', '增强系统可靠性', '提高系统可用性'],
                'tech_keywords': ['Kubernetes', 'Docker', '微服务', '分布式系统', '消息队列', '缓存系统', '负载均衡', '服务网格']
            },
            'monitoring': {
                'prefixes': ['建立', '实现', '部署', '优化', '扩展', '设计', '配置', '实施'],
                'actions': ['监控', '告警', '分析', '诊断', '预测', '检测', '追踪', '可视化'],
                'suffixes': ['及时发现异常', '快速定位问题', '预防系统故障', '提升运维效率', '增强系统可观测性'],
                'tech_keywords': ['Prometheus', 'Grafana', 'ELK Stack', '时序数据库', '分布式追踪', '日志聚合', '指标采集', '告警系统']
            },
            'default': {
                'prefixes': ['负责', '实现', '管理', '维护', '优化', '设计', '开发', '部署'],
                'actions': ['提供', '支持', '保障', '确保', '提升', '管理', '监控', '分析'],
                'suffixes': ['确保系统稳定运行', '提升系统性能', '优化用户体验', '增强系统功能', '提高工作效率'],
                'tech_keywords': []
            }
        }
        ...
        self.module_category_map = {
            ...
            'INTRADAY': 'trading_execution',
            'OPENING': 'trading_execution',
            'AUTO': 'infrastructure',
            'QUARTERLY': 'portfolio_management',
            'GOVERNANCE': 'data_management',
            'OBSERVABILITY': 'monitoring',
            'SECURITY': 'data_management',
            'QUALITY': 'data_management',
            'ORCHESTRATION': 'infrastructure',
            'VALIDATION': 'data_management',
            'VERSION': 'data_management',
            'STANDARDIZATION': 'data_management',
            'SUBSCRIPTION': 'infrastructure',
            'MASKING': 'data_management',
            ...
            'COST': 'data_management',
            'DISTRIBUTED': 'infrastructure',
            'UNIFIED': 'infrastructure',
            'METADATA': 'data_management',
            'HEALTH': 'monitoring',
    ...
    def run(self):
        ...
        print('阶段2: 选择需要优化的文档（相似度>80%）...')
        high_similarity_pairs = [p for p in pairs if p['similarity'] > 80.0]
    ...
    def _generate_report(self):
        """生成优化报告"""
        report_file = self.audit_dir / 'RESPONSIBILITY_PERSONALIZATION_REPORT_v2_20260407.md'
        
        content = f"""# 职责描述个性化优化报告 v2.0
...
**高相似度对数**: {len([d for d in self.optimization_details if d['similarity'] > 80.0])}
...
if __name__ == '__main__':
    personalizer = ResponsibilityPersonalizerV2()
    personalizer.run()
