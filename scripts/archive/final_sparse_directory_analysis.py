...
def analyze_sparse_directory_final(sparse_path: Path) -> Dict:
...
def main():
    ...
    for root, dirs, files in os.walk(DOCS_DIR):
        ...
        if len(md_files) < 3 and len(md_files) > 0:
            ...
            analysis = analyze_sparse_directory_final(sparse_path)
    ...
    for i, d in enumerate(keep_dirs[:20], 1):
        print(f"\n{i}. {d['path']}")
        print(f"   深度: {d['depth']}")
        print(f"   文件数: {d['md_count']}")
        print(f"   原因: {d['reason']}")
    ...
    if len(keep_dirs) > 20:
        print(f"\n... 还有 {len(keep_dirs) - 20} 个目录")
    
    report_path = PROJECT_ROOT / "docs/09_AUDIT/STATE" / f"final_sparse_directory_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
