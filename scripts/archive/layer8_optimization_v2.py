    ...
    def refine_responsibility(self, filepath: Path, content: str) -> tuple:
        ...
        if detailed_responsibility:
            ...
            if responsibility_match:
                ...
                if old_responsibility.strip() != new_responsibility.strip():
    ...
    def consolidate_sparse_directories(self):
        ...
        for dirname in self.sparse_directories:
            ...
            if dir_path.exists():
                ...
                print(f"    - Markdown文件数: {len(files)}")
                
                if len(files) == 1 and files[0].name.upper() == "INDEX.MD":
                    print(f"    - 状态: 稀疏目录（仅有INDEX.md）")
                    
                    target_dirname = consolidation_map.get(dirname)
                    if target_dirname:
                        target_path = LAYER8_DIR / target_dirname
                        if target_path.exists():
                            print(f"    - 整合到: {target_dirname}")
                            ...
                            if index_file.exists():
                                try:
                                    index_file.unlink()
                                    print(f"    - 已删除: {index_file.name}")
                                except Exception as e:
                                    print(f"    - 删除文件失败: {e}")
                                    self.stats['errors'].append(f"删除 {index_file} 失败: {e}")
                            
                            try:
                                dir_path.rmdir()
                                print(f"    - 已删除空目录: {dirname}")
                                self.stats['directories_consolidated'] += 1
                            except Exception as e:
                                print(f"    - 无法删除目录: {e}")
                                self.stats['errors'].append(f"删除目录 {dirname} 失败: {e}")
...
def main():
    """主函数"""
    optimizer = Layer8OptimizerV2()
    stats = optimizer.run_optimization()
    
    report_path = OUTPUT_DIR / f"LAYER8_OPTIMIZATION_REPORT_{datetime.now().strftime('%Y%m%d')}.md"
