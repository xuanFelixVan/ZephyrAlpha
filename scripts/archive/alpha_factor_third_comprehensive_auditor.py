...
class ThirdComprehensiveAuditor:
    def __init__(self):
        self.results = {
            'L1': {
                '目录结构': [],
    ...
    def clean_yaml_header(self, content):
        """清理YAML头部，保留最后一个有效的YAML块"""
        yaml_pattern = r'---\s*\n(.*?)\n---\s*\n'
        matches = list(re.finditer(yaml_pattern, content, re.DOTALL))
        
        if matches:
            last_match = matches[-1]
            yaml_content = last_match.group(1)
            body_content = content[last_match.end():]
    ...
    def scan_documents(self):
        ...
        for file_path in FACTOR_LIBRARY.rglob('*.md'):
            try:
                ...
                yaml_content, body_content = self.clean_yaml_header(content)
                yaml_dict = self.parse_yaml(yaml_content)
                ...
                if responsibility:
                    resp_str = '|'.join(sorted(responsibility))
    ...
    def audit_L2_content(self):
        ...
        for doc in self.documents:
            ...
            elif len(responsibility) == 1 and '文档支持' in responsibility[0]:
                self.results['L2']['职责驱动'].append({
                    'type': '职责模糊',
                    'path': doc['path'],
                    'responsibility': responsibility,
...
"""
        ...
        for item in self.results['L1']['目录结构'][:20]:  # 只显示前20个
...
"""
        ...
        report_path = Path(rf'D:\ZephyrAlpha\docs\09_AUDIT\STATE\THIRD_COMPREHENSIVE_AUDIT_REPORT_{timestamp}.md')
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write(report)
        
        # 保存JSON数据
        json_path = Path(rf'D:\ZephyrAlpha\docs\09_AUDIT\STATE\THIRD_COMPREHENSIVE_AUDIT_REPORT_{timestamp}.json')
...
if __name__ == '__main__':
    auditor = ThirdComprehensiveAuditor()
    auditor.run()
