...
builder = RegimeFeatureBuilder(backtest_start="2020-01-01", backtest_end="2021-12-31", data_load_start="2010-01-01")
...
print(
    f"分布: 1.0={100 * (vals == 1.0).mean():.1f}% [0.8,1)={100 * ((vals >= 0.8) & (vals < 1.0)).mean():.1f}% [0.6,0.8)={100 * ((vals >= 0.6) & (vals < 0.8)).mean():.1f}% <0.6={100 * (vals < 0.6).mean():.1f}%"
)
