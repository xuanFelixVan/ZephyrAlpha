"""
L1文件命名最终修复脚本
用途：修复剩余10个文件命名问题
创建时间：2026-04-07
"""

import os
import re
from pathlib import Path
from datetime import datetime

PROJECT_ROOT = Path("D:/ZephyrAlpha")
DOCS_DIR = PROJECT_ROOT / "docs"

class L1NamingFinalFixer:
    def __init__(self):
        self.fixed_files = []
        self.failed_files = []
        
    def find_layer_files(self):
        print("扫描包含Layer关键词的文件...")
        layer_files = []
        
        for root, dirs, files in os.walk(DOCS_DIR):
            for file in files:
                if file.endswith('.md'):
                    if re.search(r'layer\s*[0-9]+', file, re.IGNORECASE):
                        file_path = Path(root) / file
                        layer_files.append(file_path)
        
        print(f"发现 {len(layer_files)} 个包含Layer关键词的文件")
        return layer_files
    
    def clean_filename(self, filename):
        new_name = filename
        
        patterns = [
            (r'layer25_', ''),
            (r'layer11_', ''),
            (r'layer10_', ''),
            (r'layer9_', ''),
            (r'layer8_', ''),
            (r'layer7_', ''),
            (r'layer6_', ''),
            (r'layer5_', ''),
            (r'layer4_', ''),
            (r'layer3_', ''),
            (r'layer2_', ''),
            (r'layer1_', ''),
            (r'layer0_', ''),
            (r'Layer25_', ''),
            (r'Layer11_', ''),
            (r'Layer10_', ''),
            (r'Layer9_', ''),
            (r'Layer8_', ''),
            (r'Layer7_', ''),
            (r'Layer6_', ''),
            (r'Layer5_', ''),
            (r'Layer4_', ''),
            (r'Layer3_', ''),
            (r'Layer2_', ''),
            (r'Layer1_', ''),
            (r'Layer0_', ''),
            (r'LAYER25_', ''),
            (r'LAYER11_', ''),
            (r'LAYER10_', ''),
            (r'LAYER9_', ''),
            (r'LAYER8_', ''),
            (r'LAYER7_', ''),
            (r'LAYER6_', ''),
            (r'LAYER5_', ''),
            (r'LAYER4_', ''),
            (r'LAYER3_', ''),
            (r'LAYER2_', ''),
            (r'LAYER1_', ''),
            (r'LAYER0_', ''),
        ]
        
        for pattern, replacement in patterns:
            new_name = re.sub(pattern, replacement, new_name, flags=re.IGNORECASE)
        
        new_name = re.sub(r'_\d{8}_\d{6}', '', new_name)
        
        return new_name
    
    def fix_file(self, file_path):
        try:
            old_name = file_path.name
            new_name = self.clean_filename(old_name)
            
            if old_name == new_name:
                return False
            
            new_path = file_path.parent / new_name
            
            if new_path.exists():
                print(f"  ⚠️ 目标文件已存在，跳过: {new_name}")
                self.failed_files.append({
                    "path": str(file_path.relative_to(DOCS_DIR)),
                    "error": "目标文件已存在"
                })
                return False
            
            os.rename(file_path, new_path)
            
            self.fixed_files.append({
                "old_path": str(file_path.relative_to(DOCS_DIR)),
                "new_path": str(new_path.relative_to(DOCS_DIR)),
                "old_name": old_name,
                "new_name": new_name
            })
            
            print(f"  ✅ {old_name} -> {new_name}")
            return True
        
        except Exception as e:
            print(f"  ❌ 处理失败: {file_path} - {e}")
            self.failed_files.append({
                "path": str(file_path.relative_to(DOCS_DIR)),
                "error": str(e)
            })
            return False
    
    def run(self):
        print("=" * 80)
        print("L1文件命名最终修复")
        print("=" * 80)
        print(f"开始时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("=" * 80)
        
        layer_files = self.find_layer_files()
        
        print(f"\n开始修复...")
        success_count = 0
        
        for i, file_path in enumerate(layer_files, 1):
            print(f"[{i}/{len(layer_files)}] 处理: {file_path.name}")
            
            if self.fix_file(file_path):
                success_count += 1
        
        print("\n" + "=" * 80)
        print("修复统计")
        print("=" * 80)
        print(f"总文件数: {len(layer_files)}")
        print(f"成功修复: {success_count}")
        print(f"失败: {len(self.failed_files)}")
        
        if self.fixed_files:
            print("\n成功修复:")
            for item in self.fixed_files:
                print(f"  ✅ {item['old_name']} -> {item['new_name']}")
        
        if self.failed_files:
            print("\n失败文件:")
            for item in self.failed_files:
                print(f"  ❌ {item['path']}: {item['error']}")
        
        print("\n" + "=" * 80)
        print(f"完成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("=" * 80)
        
        return {
            "total": len(layer_files),
            "success": success_count,
            "failed": len(self.failed_files)
        }

if __name__ == "__main__":
    fixer = L1NamingFinalFixer()
    fixer.run()
