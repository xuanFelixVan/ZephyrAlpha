...
def main() -> int:
    ap = argparse.ArgumentParser(description="数据源上架流水线编排器（probe/apply/verify）")
    ap.add_argument("--card", required=True, help="源卡片路径 config/source_cards/*.yaml")
    ...
    if args.mode == "probe":
        ...
        print(json.dumps({"mode": "probe", "ingest_exit": rc}, ensure_ascii=False))
        return rc
    ...
    if args.mode == "apply":
        ...
        if rc != 0:
            ...
            print(json.dumps({"backfill_exit": rc, "task_registered": False,
                              "reason": "backfill 失败，不登记定时任务"}, ensure_ascii=False))
            return 2
        cmd = build_task_cmd(card)
        task = run_subprocess_hidden(cmd, capture_output=True, text=True)
        task_ok = task.returncode == 0
        print(json.dumps({"backfill_exit": rc, "task_registered": task_ok,
                          "task_err": (task.stderr or "")[-200:] if not task_ok else ""},
                         ensure_ascii=False))
        if rc != 0 or not task_ok:
            return 2
        info = verify(card)
        print(json.dumps({"mode": "apply", **info}, ensure_ascii=False))
        return 0

    print(json.dumps({"mode": "verify", **verify(card)}, ensure_ascii=False))
    return 0
