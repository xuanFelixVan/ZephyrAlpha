...
for file in os.listdir(blueprints_dir):
    if file.endswith('.md') and 'BLUEPRINT' in file:
        ...
        if resp_match:
            ...
            resp_items = [item.strip() for item in resp_items if item.strip() and not item.strip().startswith('?')]
            
            if '?' in resp_text:
                issues['yaml_format_error'].append({
                    'file': file,
                    'issue': 'responsibility字段包含非法字符(?)'
                })
        ...
        boundary_match = re.search(r'>\s*\*\*职责边界\*\*:', content)
...
if issues['yaml_format_error']:
    print(f"\n[P0] YAML格式错误 ({len(issues['yaml_format_error'])}个):")
    for item in issues['yaml_format_error']:
        print(f"  - {item['file']}: {item['issue']}")
    total_issues += len(issues['yaml_format_error'])
