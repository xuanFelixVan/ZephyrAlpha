...
# [MODULE] scripts.ch.apply_research_report_ddl
# [DOMAIN] D_DATA
# [DEPENDENCIES] zephyr.data.ch_reader; schemas.categories.fundamental.fundamental_research_report
# [CONSUMERS]
# [STARTUP] manual
# [MATURITY] design
# [INVARIANTS] DDL-as-Code: research_report DDL 真源为 schemas/categories/fundamental/fundamental_research_report.py
# [MODIFY-GUARD] none
# [STABILITY] evolving
# [SAFETY] L
...
"""research_report 建表 DDL 部署 + 验证脚本（研报+分析师预测数据侧施工，2026-09-12）。

DDL 真源：schemas/categories/fundamental/fundamental_research_report.py（DDL-as-Code）。
施工方案：docs/_working/2026-09-12-research-report-data-plan.md。
裁定沿用：writer 账号无 CREATE 权限，DDL 用 base 账号直连（#ARCH-CH-027 RBAC 三账号体系，
同 scripts/ch/apply_regime_snapshot_ddl.py 先例）。

用法::

    python scripts/ch/apply_research_report_ddl.py           # 建表 + 验证
    python scripts/ch/apply_research_report_ddl.py --verify  # 仅验证（表存在且引擎正确）
"""
