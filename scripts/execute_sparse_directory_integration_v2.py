...
def integrate_directory(sparse_dir: str, target_dir: str) -> Dict:
    ...
    try:
        ...
        if not remaining_files:
            ...
            result["error"] = f"目录非空，无法删除: {remaining_names}"
...
def main():
    ...
    report_path = PROJECT_ROOT / "docs/09_AUDIT/STATE" / f"sparse_directory_integration_result_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
