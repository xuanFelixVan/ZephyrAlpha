...
# [DOMAIN] D_EX_CORE
# [DEPENDENCIES] stdlib；zephyr.ex_core.trading_session（TradingSession/TradingSessionConfig 真源）；zephyr.ex_core.adapters.miniqmt_broker（延迟 import）；zephyr.ex_core.order_manager；zephyr.ex_core.signal_providers；zephyr.governance.adapters.risk_validation_bridge；zephyr.risk.implementations.default_risk_validator；zephyr.governance.strategies.strategy_base；zephyr.pf_core.topn_momentum_strategy（--strategy 可选）；zephyr.shared.infra.process_pool（run_subprocess_hidden SSoT）
# [CONSUMERS] 57 号文 §2 盘中模拟盘过渡形态——交易日 09:25 前人工拉起（常驻服务/LiveStrategyAdapter 属后续批，本脚本不挂任何调度）
# [STARTUP] manual
# [MATURITY] testing
# [INVARIANTS] 仅连 QMT 模拟账户（config/.env.qmt QMT_SIM_*，实盘 QMT_REAL_* 永不触碰）；默认纯会话保活不自动 rebalance（--strategy 缺省=安全默认）；--dry-run 只连不打任何单；有界保活循环 15:05 自动 stop；KeyboardInterrupt 优雅 stop（stop 自动撤未成交单语义保留）
...
功能
----
``python scripts/start_paper_session.py [--strategy NAME] [--dry-run] [--universe ...]``：
...
- ``--strategy NAME``：默认空=纯会话保活不自动 rebalance（安全默认——
  LiveStrategyAdapter 真信号源未施工，57 号文 GAP-2 原文登记）。
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        description="模拟盘交易日启动脚本（57 号文 GAP-2 过渡形态；默认纯保活不下单）",
    )
    parser.add_argument(
        "--strategy",
        default="",
...
def assemble_session(args: argparse.Namespace, broker: object) -> TradingSession:
    ...
    if args.strategy == "":
        ...
        # 真信号源=LiveStrategyAdapter 后续批，57 号文 GAP-2 原文登记）
