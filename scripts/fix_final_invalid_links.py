...
def fix_final_invalid_links():
    ...
    fix_plan = [
        ...
        {
            'file': '09_AUDIT/STANDARDS/RESPONSIBILITY_TEMPLATE_LIBRARY.md',
            'links': [
                {'text': '文档版本号命名标准', 'path': '../09_AUDIT/STANDARDS/DOCUMENT_VERSION_NAMING_STANDARD.md', 'action': 'fix', 'new_path': './DOCUMENT_VERSION_NAMING_STANDARD.md'},
                {'text': '文档治理审计指南', 'path': '../09_AUDIT/TEMPLATES/PROFESSIONAL_DOCUMENT_GOVERNANCE_AUDIT_GUIDE.md', 'action': 'fix', 'new_path': '../TEMPLATES/PROFESSIONAL_DOCUMENT_GOVERNANCE_AUDIT_GUIDE.md'},
                {'text': '职责边界定义标准', 'path': '../09_AUDIT/STANDARDS/RESPONSIBILITY_DESCRIPTION_STANDARD.md', 'action': 'fix', 'new_path': './RESPONSIBILITY_DESCRIPTION_STANDARD.md'}
            ]
        }
    ]
...
def generate_fix_report(fixed_count, removed_count, issues):
    ...
    report_path = OUTPUT_DIR / f'FINAL_LINK_FIX_REPORT_{timestamp}.md'
    
    report_content = f"""---
module_id: FINAL_LINK_FIX_REPORT_{timestamp}
version: 1.0.0
status: Active
...
| 版本 | 日期 | 变更内容 | 变更人 |
|------|------|----------|--------|
| v1.0.0 | {datetime.now().strftime('%Y-%m-%d')} | 初始版本，最后无效链接修复报告 | 首席文档架构师 |
"""
...
if __name__ == '__main__':
    # 修复最后无效链接
    fixed_count, removed_count, issues = fix_final_invalid_links()
