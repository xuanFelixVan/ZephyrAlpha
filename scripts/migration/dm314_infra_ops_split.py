...
用法:
    python scripts/migration/dm314_infra_ops_split.py --dry-run --all --move
    python scripts/migration/dm314_infra_ops_split.py --subdir asset-inventory --move
    python scripts/migration/dm314_infra_ops_split.py --subdir asset-inventory --update-imports
    python scripts/migration/dm314_infra_ops_split.py --subdir asset-inventory --update-headers
    python scripts/migration/dm314_infra_ops_split.py --all --fix-external-refs
    python scripts/migration/dm314_infra_ops_split.py --update-registry-status --subdir asset-inventory
"""
...
SUBDIR_TO_NEW_MODULE_PREFIX = {
    ...
    "auto-fix-engine": "zephyr.infrastructure.auto_fix_engine.",
    ...
    "cicd_pipeline/code_dedup_engine": "zephyr.testing.code_dedup.",
    "cicd_pipeline/script_system": "zephyr.infrastructure.script_system.",
    "cicd_pipeline/system-telemetry": "zephyr.infrastructure.system_telemetry.",
    # shared -> shared_services
    "shared/observability_02": "zephyr.infrastructure.shared_services.observability_02.",
}

# Top-level subdirs that need further disambiguation
CICD_SUBDIRS = {"a2a_protocol", "code_dedup_engine", "script_system", "system-telemetry"}

# Cross-reference mapping for external import fixes
# old import prefix -> new import prefix
CROSS_REFS = {
    "zephyr.infrastructure.a2a_protocol.": "zephyr.infrastructure.a2a_protocol.",
    "zephyr.testing.code_dedup.": "zephyr.testing.code_dedup.",
    "zephyr.infrastructure.script_system.": "zephyr.infrastructure.script_system.",
    "zephyr.infrastructure.system_telemetry.": "zephyr.infrastructure.system_telemetry.",
    "zephyr.infrastructure.": "zephyr.infrastructure.",
    "zephyr.infrastructure.asset_inventory.": "zephyr.infrastructure.asset_inventory.",
    "zephyr.infrastructure.auto_fix_engine.": "zephyr.infrastructure.auto_fix_engine.",
...
def get_infra_ops_entries(data: dict, subdir: str | None = None) -> list[dict]:
    ...
    for e in entries:
        ...
        if e.get("status") != "pending":
            continue
        if subdir:
            subdir_key = _determine_subdir_key(op)
            if subdir_key != subdir:
                continue
        filtered.append(e)
    return filtered


def _determine_subdir_key(old_path: str) -> str:
    """Determine the subdir key for a given old_path.

    For cicd_pipeline files, we need to include the second-level subdir
    (e.g., 'cicd_pipeline/a2a_protocol') for proper mapping.
    """
    ...
    if not parts:
        return ""
    first = parts[0]
    # Check if it's a cicd_pipeline subdirectory
    if first == "cicd_pipeline" and len(parts) > 1:
        second = parts[1]
        if second in CICD_SUBDIRS:
            return f"{first}/{second}"
    # Check shared/observability_02
    if first == "shared" and len(parts) > 1:
        return f"{first}/{parts[1]}"
    return first


def determine_subdir(old_path: str) -> str:
    return _determine_subdir_key(old_path)
...
def update_imports_in_file(new_path: str, subdir: str, dry_run: bool = False) -> dict:
    ...
    for old_ref, new_ref in sorted(CROSS_REFS.items(), key=lambda x: -len(x[0])):
...
def update_headers_in_file(new_path: str, subdir: str, dry_run: bool = False) -> dict:
    ...
    old_module_prefix = f"{OLD_PREFIX}{subdir.replace('/', '.')}."
    ...
    # Also update [BLUEPRINT] path references
    old_path_pattern = OLD_PATH_PREFIX + subdir + "/"
    new_path_replacement_map = {
        "asset-inventory": "src/zephyr/infrastructure/runtime_integration/asset-inventory/",
        "auto-fix-engine": "src/zephyr/infrastructure/runtime_integration/auto-fix-engine/",
        "capacity-assurance": "src/zephyr/infrastructure/runtime_integration/capacity-assurance/",
        "health-monitor": "src/zephyr/infrastructure/runtime_integration/health-monitor/",
        "model-profiler": "src/zephyr/infrastructure/runtime_integration/model-profiler/",
        "model-capability-exam": "src/zephyr/infrastructure/runtime_integration/model-capability-exam/",
        "observability": "src/zephyr/observability/telemetry/",
        "cicd_pipeline/a2a_protocol": "src/zephyr/infrastructure/runtime_integration/a2a_protocol/",
        "cicd_pipeline/code_dedup_engine": "src/zephyr/testing/code_dedup/",
        "cicd_pipeline/script_system": "src/zephyr/infrastructure/runtime_integration/script_system/",
        "cicd_pipeline/system-telemetry": "src/zephyr/observability/telemetry/",
        "shared/observability_02": "src/zephyr/infrastructure/shared_services/observability_02/",
    }
    if subdir in new_path_replacement_map:
        content = content.replace(old_path_pattern, new_path_replacement_map[subdir])
...
def fix_external_refs(subdir: str | None = None, dry_run: bool = False) -> tuple[int, int]:
    ...
    refs_to_fix = {}
    if subdir:
        old_ref = f"{OLD_PREFIX}{subdir.replace('/', '.')}."
    ...
    def _fix_file(filepath: str) -> dict:
        ...
        original = content
        # Sort by length descending to replace longest matches first
        for old_ref, new_ref in sorted(refs_to_fix.items(), key=lambda x: -len(x[0])):
            content = content.replace(old_ref, new_ref)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="DM-314: infra_ops split migration (cut-paste mode)")
    parser.add_argument("--dry-run", action="store_true", help="Dry run - no actual changes")
    parser.add_argument("--subdir", type=str,
                        help="Only process specific subdirectory (e.g., asset-inventory, cicd_pipeline/a2a_protocol)")
    parser.add_argument("--all", action="store_true", help="Process all infra_ops subdirectories")
    ...
    parser.add_argument("--update-headers", action="store_true", help="Update [MODULE]/[BLUEPRINT] headers in moved files")
    ...
    if need_registry:
        data = load_registry()
        entries = get_infra_ops_entries(data, subdir=args.subdir if args.subdir else None)
    ...
    print(f"=== DM-314: infra_ops Split Migration ===")
