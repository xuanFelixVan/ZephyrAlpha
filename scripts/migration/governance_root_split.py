...
_spec = importlib.util.spec_from_file_location("ast_import_rewriter", _AST_REWRITER_PATH)
...
def update_blueprint_header(new_path: str, subdir: str, dry_run: bool = False, old_path: str = "") -> dict:
...
def rewrite_imports_project(entries: list[MigrationEntry], dry_run: bool = False, quiet: bool = False) -> dict:
    ...
    if not quiet:
        ...
        print(f"  [{mode}] {len(results)} files modified, {total_changes} replacements")
    return {
...
def update_registry_status(entries: list[MigrationEntry], dry_run: bool = False) -> int:
...
def execute_move_batch(
    entries: list[MigrationEntry], dry_run: bool = False, force: bool = False
) -> tuple[int, int, int]:
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = {}
        for e in entries:
            futures[executor.submit(move_file, e.old_path, e.new_path, dry_run, force)] = e
...
def execute_header_updates(entries: list[MigrationEntry], dry_run: bool = False) -> tuple[int, int]:
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = {}
        for e in entries:
            futures[executor.submit(update_blueprint_header, e.new_path, e.subdir, dry_run, e.old_path)] = e
        for future in as_completed(futures):
            result = future.result()
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--dry-run", action="store_true", help="Dry run - report changes, do not modify files")
    parser.add_argument("--batch", type=str, help="Process single subdir batch (e.g., escalation, budget)")
    parser.add_argument("--all", action="store_true", help="Process all pending entries (all subdirs)")
    parser.add_argument("--move", action="store_true", help="Execute file moves (shutil.move, cut-paste)")
    parser.add_argument(
        "--rewrite-imports",
        action="store_true",
        help="Rewrite imports across whole project via ast_import_rewriter (also updates [MODULE] header)",
    )
    parser.add_argument(
        "--update-headers",
        action="store_true",
        help="Update [BLUEPRINT] path in moved files' frontmatter ([MODULE] handled by --rewrite-imports)",
    )
    parser.add_argument(
        "--fix-external-refs",
        action="store_true",
    ...
    parser.add_argument(
        "--update-registry-status", action="store_true", help="Mark entries as done in migration_registry.yaml"
    )
    ...
    need_registry = args.move or args.rewrite_imports or args.update_headers or args.update_registry_status
    ...
    if do_rewrite:
        ...
        result = rewrite_imports_project(entries, dry_run=args.dry_run, quiet=args.quiet)
    ...
    if not (args.move or do_rewrite or args.update_headers or args.update_registry_status):
        print(
