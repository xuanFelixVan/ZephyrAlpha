...
# [A_script] module_id=MOD-GOV-post_checkout_guard | layer=script | stability=evolving | safety=M | ai_autonomy=ai_modifiable
# [TTL] permanent
# noqa: m11-perm-manual-legitimate  M11豁免: 由 git post-checkout 钩子事件驱动执行，CLI 入口即事件入口
"""
...
__manifest__ = """
args: [pre_head, post_head, is_branch_switch]
description: >
  事后检测 checkout 是否覆盖其他 session 的文件锁（.ailocks/ 只读扫描）；
  由 .git/hooks/post-checkout 事件驱动，只警告不阻断（exit 恒 0）。
dimensions:
- D7
priority: P2
timeout_seconds: 10
...
def _get_project_root() -> Path:
    """获取 git 仓库根目录。"""
    try:
        result = subprocess.run(  # noqa: bare-subprocess  git 钩子进程只读 rev-parse，process_pool 循环依赖不可用
...
def _get_changed_files(pre_head: str, post_head: str) -> list[str]:
    """获取 checkout 涉及的文件列表。"""
    try:
        result = subprocess.run(  # noqa: bare-subprocess  git 钩子进程只读 diff 列表，process_pool 循环依赖不可用
...
def check_checkout_conflict(pre_head: str, post_head: str, is_branch_switch: int) -> int:
